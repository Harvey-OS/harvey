...
if [ -n "$(git remote -v | awk '$1=="origin"{print $2}' | grep gerrithub)" ]; then
	...
	git config remote.origin.pushurl | grep -v -q ssh && cat <<EOF >&2
Set up your pushurl to use SSH for automatic reviewer adding:

	git config remote.origin.pushurl ssh://\${USERNAME}@review.gerrithub.io:29418/Harvey-OS/harvey
