...
static char*
myreadcons(char *prompt, char *def, int raw, char *buf, int nbuf)
{
...
//		err |= check(safe->config, CONFIGLEN, safe->configsum, "bad secstore key");
	...
	if((flag&NVwrite) || (err && (flag&NVwriteonerr))){
		myreadcons("authid", nil, 0, safe->authid, sizeof(safe->authid));
		myreadcons("authdom", nil, 0, safe->authdom, sizeof(safe->authdom));
		myreadcons("secstore key", nil, 1, safe->config, sizeof(safe->config));
		for(;;){
			if(myreadcons("password", nil, 1, in, sizeof in) == nil)
				goto Out;
