...
func process(f, which string) []build {
	...
	for n, build := range builds {
		...
		log.Printf("build.ObjectFiles after adjust %v", build.ObjectFiles)
		...
		for _, v := range includes {
			...
			log.Printf("build.ObjectFiles before %v = %v", v, build.ObjectFiles)
			include(v, &build)
			log.Printf("build.ObjectFiles after %v = %v", v, build.ObjectFiles)
		}
		...
		log.Printf("build.ObjectFiles after build.SourceFiles loop = %v", build.ObjectFiles)
