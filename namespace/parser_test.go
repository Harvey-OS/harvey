	...
	tests := []struct {
	...
	}{
		{
			...
			want1: File{
				cmd{
					syscall: MOUNT,
					flag:    harvey.REPL | harvey.AFTER | harvey.CACHE,
					args:    []string{"#s/boot", "/root", "$rootspec"},
				},
				cmd{
					syscall: BIND,
					flag:    harvey.REPL | harvey.AFTER,
					args:    []string{"$rootdir", "/"},
				},
				cmd{
					syscall: BIND,
					flag:    harvey.REPL | harvey.CREATE,
				...
				cmd{
					syscall: BIND,
					flag:    harvey.REPL | harvey.BEFORE,
					args:    []string{"#k", "/dev"},
				},
				cmd{
					syscall: BIND,
