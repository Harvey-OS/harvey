...
%%%     FAX             = "401-331-3842",
%%%     checksum        = "21012 2322 7744 81297",
%%%     email           = "tech-support@math.ams.org (Internet)",
%%%     codetable       = "ISO/ASCII",
%%%     keywords        = "latex, amslatex, ams-latex, amsmath, documentation",
%%%     supported       = "yes",
%%%     abstract        = "This is part of the AMS-\LaTeX{} distribution.
%%%                        It contains extensive examples of mathematical
%%%                        constructs supported by the amsmath package.",
...
\[\int_{\mathcal{D}}|\overline\partial u|^2\Phi_ 0(z) e^{\alpha\abs{z}^2}
\geq c_4\alpha\int_{\mathcal{D}}|u|^2\Phi_ 0e^{\alpha\abs{z}^2}
+c_5\delta^{-2}\int_ A|u|^2\Phi_ 0e^{\alpha\abs{z}^2}.\]

The boundedness, property (\ref{boundab}) of $\Phi_ 0$, then yields
\[\int_{\mathcal{D}}|\overline\partial u|^2e^{\alpha\abs{z}^2}\geq c_6\alpha
\int_{\mathcal{D}}|u|^2e^{\alpha\abs{z}^2}
+c_7\delta^{-2}\int_ A|u|^2e^{\alpha\abs{z}^2}.\]
...
T^u_x=\left\{y\in\mathbf{R}^m:
 y=\tilde u(x)+\left\langle \frac{Du}{|Du|}(x),z
...
\end{equation}
In addition, for $|\wt{D}u|$-almost every $x\in\Omega $ the
restriction of the function $f$ to $T^u_x$ is differentiable at $\tilde
u(x)$ and
\begin{equation}
\wt{D}v=\nabla (f|_{T^u_x})(\tilde u)\frac{\wt{D}u}{|
\wt{D}u|}\cdot|\wt{D}u|.\end{equation}
\end{thm}
...
|Dv|(B)\le K|Du|(B)\qquad\forall B\in\mathbf{B}(\Omega ),
\end{equation}
...
$B\in\mathbf{B}(\Omega)$, because $|Dv|$, $|Du|$ are Radon measures. To
...
yields that $|\wt{D}v|(S_u\backslash S_v)=0$, so that
...
singular part with respect to $|\wt{D} u|$. By
\thmref{th-weak-ske-owf}, we have
\begin{equation*}
\frac{\wt{D}v}{|\wt{D}u|}(t)=\lim_{s\to t^+}
\frac{Dv(\interval{\left[t,s\right[})}
{|\wt{D}u|(\interval{\left[t,s\right[})},\qquad
\frac{\wt{D}u}{|\wt{D}u|}(t)=\lim_{s\to t^+}
\frac{Du(\interval{\left[t,s\right[})}
{|\wt{D}u|(\interval{\left[t,s\right[})}
\end{equation*}
$|\wt{D}u|$-almost everywhere in $\mathbf{R}$. It is well known
...
$|\wt{D}u|(\interval{\left[t,s\right[})>0$ for every $s>t$ and
assume that the limits in \eqref{joe} exist. By \eqref{j:mark} and
\eqref{far-d} we get
\begin{equation*}\begin{split}
\frac{\hat v(s)-\hat
v(t)}{|\wt{D}u|(\interval{\left[t,s\right[})}&=\frac {f(\hat
u(s))-f(\hat u(t))}{|\wt{D}u|(\interval{\left[t,s\right[})}\\
&=\frac{f(\hat u(s))-f(\hat
u(t)+\dfrac{\wt{D}u}{|\wt{D}u|}(t)|\wt{D}u
|(\interval{\left[t,s\right[}))}%
{|\wt{D}u|(\interval{\left[t,s\right[})}\\
&+\frac
{f(\hat u(t)+\dfrac{\wt{D}u}{|\wt{D}u|}(t)|\wt{D}
u|(\interval{\left[t,s\right[}))-f(\hat
u(t))}{|\wt{D}u|(\interval{\left[t,s\right[})}
\end{split}\end{equation*}
...
v(t)}{|\wt{D}u|(\interval{\left[t,s\right[})} -\frac{f(\hat
u(t)+\dfrac{\wt{D}u}{|\wt{D}u|}(t)
|\wt{D}u|(\interval{\left[t,s\right[}))-f(\hat
u(t))}{|\wt{D}u|(\interval{\left[t,s\right[})}\right|\\
\le K\left|
\frac{\hat u(s)-\hat u(t)}
  {|\wt{D}u|(\interval{\left[t,s\right[})}
-\frac{\wt{D}u}{|
\wt{D}u|}(t)\right|.\end{multline*}
}% end of group with \multlinegap=0pt
By \eqref{e:bomb}, the function $s\to
|\wt{D}u|(\interval{\left[t,s\right[})$ is continuous and
converges to 0 as $s\downarrow t$. Therefore Remark~\ref{r:omb} and the
previous inequality imply
\[\frac{\wt{D}v}{|\wt{D}u|}(t)=\lim_{h\to 0^+}
\frac{f(\hat u(t)+h\dfrac{\wt{D}u}{|\wt{D}u|}
(t))-f(\hat u(t))}h\quad|\wt{D}u|\text{-a.e. in }\mathbf{R}.\]
...
\[\frac{\wt{D}v}{|\wt{D}u|}(t)=\lim_{h\to 0}
\frac{f(\tilde u(t)
+h\dfrac{\wt{D}u}{|\wt{D}u|}(t))-f(\tilde u(t))}{h}
\qquad|\wt{D}u|\text{-a.e. in }\mathbf{R}\]
...
\wt{D}u_y|}(t)\qquad|\wt{D}u_y|\text{-a.e. in }\mathbf{R}\]
for $\mathcal{H}_{n-1}$-almost every $y\in \pi_\nu$. We claim that
\begin{equation}
\frac{\langle \wt{D}u,\nu\rangle }{|\langle \wt{D}u,\nu\rangle
|}(y+t\nu)=\frac{\wt{D}u_y}
{|\wt{D}u_y|}(t)\qquad|\wt{D}u_y|\text{-a.e. in }\mathbf{R}
\end{equation}
for $\mathcal{H}_{n-1}$-almost every $y\in\pi_\nu$. In fact, by
...
Since the function $|\langle \wt{D}u,\nu\rangle |/|\wt{D}u|$
is strictly positive $|\langle \wt{D}u,\nu\rangle |$-almost everywhere,
we obtain also
\begin{multline*}
\lim_{h\to 0}\frac{f(\tilde u(x)+h\dfrac{|\langle
\wt{D}u,\nu\rangle |}{|\wt{D}u|}(x)\dfrac{\langle \wt{D}
u,\nu\rangle }{|\langle \wt{D}u,\nu\rangle |}(x))-f(\tilde u(x))}{h}\\
=\frac{|\langle \wt{D}u,\nu\rangle |}{|\wt{D}u|}(x)\frac
{\langle \wt{D}v,\nu\rangle }{|\langle
\wt{D}u,\nu\rangle |}(x)
\end{multline*}
...
\frac{\langle \wt{D}u,\nu\rangle }{|\langle \wt{D}u,\nu\rangle|}
=\frac{\langle \wt{D}u,\nu\rangle }{|\wt{D}u|}
=\left\langle \frac{\wt{D}u}{|\wt{D}u|},\nu\right\rangle
        \qquad|\wt{D}u|\text{-a.e. in }\mathbf{R}^n\\
&\frac{|\langle \wt{D}u,\nu\rangle |}{|\wt{D}u|}
\frac{\langle \wt{D}v,\nu\rangle }{|\langle \wt{D}u,\nu\rangle|}
=\frac{\langle \wt{D}v,\nu\rangle }{|\wt{D}u|}
=\left\langle \frac{\wt{D}v}{|\wt{D}u|},\nu\right\rangle
        \qquad|\wt{D}u|\text{-a.e. in }\mathbf{R}^n
\end{align*}
and since both sides of \eqref{alimo}
are zero $|\wt{D}u|$-almost everywhere
...
=\left\langle \frac{\wt{D}v}{|\wt{D}u|}(x),\nu\right\rangle,
\]
$|\wt{D}u|$-a.e. in $\mathbf{R}^n$.
Since $\nu$ is arbitrary, by Remarks \ref{r:dif} and~\ref{r:dif0}
the restriction of $f$ to
the affine space $T^u_x$ is differentiable at $\tilde u(x)$ for $|\wt{D}
...
\verb=\lim=.
The \pkg{amsmath} package provides \cn{operatorname} and
\cn{operatorname*}
...
The following special operatornames are predefined in the \pkg{amsmath}
...
&\le C_6\left|\left|f\int_\Omega\left|\wt{S}^{-1,0}_{a,-}
  W_2(\Omega,\Gamma_l)\right|\right|
  \left||u|\overset{\circ}\to W_2^{\wt{A}}
...
&\quad\times \left[\int_\Omega\left\{u^2_x+\frac{1}{k}
  ...
  c\Omega\right]^{1/2}\\
&\le C_4\left|\left|f\left|\wt{S}^{-1,0}_{a,-}
  W_2(\Omega,\Gamma_l)\right|\right|
  \left||u|\overset{\circ}\to W_2^{\wt{A}}
  (\Omega;\Gamma_r,T)\right|\right|.
...
M.~Marcus and H.~Ming, \emph{A survey of matrix theory and matrix
