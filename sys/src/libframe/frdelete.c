...
frdelete(Frame* f, uint32_t p0, uint32_t p1)
{
	...
	if(n1 == f->nbox &&
	   pt0.x != pt1.x) /* deleting last thing in window; must clean up */
	...
	if(pt1.y != pt0.y) {
		...
		if(n1 < f->nbox) {
			...
			if(q2 > f->r.max.y)
				q2 = f->r.max.y;
			draw(f->b, Rect(pt0.x, pt0.y,
			                pt0.x + (f->r.max.x - pt1.x), q0),
			     f->b, nil, pt1);
			draw(f->b,
			     Rect(f->r.min.x, q0, f->r.max.x, q0 + (q2 - q1)),
			     f->b, nil, Pt(f->r.min.x, q1));
			frselectpaint(f, Pt(pt2.x, pt2.y - (pt1.y - pt0.y)),
			              pt2, f->cols[BACK]);
		} else
	...
	n = f->nlines;
	f->nlines =
	    (pt0.y - f->r.min.y) / f->font->height + (pt0.x > f->r.min.x);
	return n - f->nlines;
}
