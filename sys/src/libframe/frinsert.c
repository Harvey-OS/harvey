...
frinsert(Frame* f, Rune* sp, Rune* ep, uint32_t p0)
{
	...
	if(n0 == f->nbox)
		f->nlines = (pt1.y - f->r.min.y) / f->font->height +
		            (pt1.x > f->r.min.x);
	else if(pt1.y != pt0.y) {
		int q0, q1;
	 ...
	 * The draw()s above moved everything down after the point they lined
	 * up.
	 */
	for((y = pt1.y == f->r.max.y ? pt1.y : 0), b = &f->box[n0 - 1];
		...
		if(b->nrune > 0) {
			...
			if(npts == 0 && pt.y > pt0.y) {
				/*
				 * first new char is bigger than first char
				 * we're
				 * displacing, causing line wrap. ugly special
				 * case.
				 */
				r.min = opt0;
				r.max = opt0;
				...
				r.max.y += f->font->height;
				if(f->p0 <= cn0 &&
				   cn0 < f->p1) /* b+1 is inside selection */
					col = f->cols[HIGH];
				else
					col = f->cols[BACK];
			...
			cn0--;
			if(f->p0 <= cn0 &&
			   cn0 < f->p1) /* b is inside selection */
				col = f->cols[HIGH];
			else
				col = f->cols[BACK];
			draw(f->b, r, col, nil, r.min);
	...
	/* insertion can extend the selection, so the condition here is
	 * different */
	if(f->p0 < p0 && p0 <= f->p1)
		col = f->cols[HIGH];
	else
