...
mktree(Mkaux* mkaux, File* me, int rec)
{
	...
	while((n = dirread(fd, &d)) > 0) {
		for(i = 0; i < n; i++) {
			...
			child.elem = d[i].name;
			setnames(mkaux, &child);
			if((!(d[i].mode & DMDIR) || rec) &&
			   copyfile(mkaux, &child, &d[i], 1) && rec)
				mktree(mkaux, &child, rec);
			free(child.new);
			if(child.old)
				free(child.old);
...
setname(Mkaux* mkaux, Name* name, char* s1, char* s2)
{
	...
	snprint(name->s, name->n, "%s%s%s", s1,
	        s1[0] == 0 || s1[strlen(s1) - 1] != '/' ? "/" : "", s2);
}
...
getname(Mkaux* mkaux, char* p, char** buf)
{
	...
	*buf = malloc(
	    p + 2 -
	    start); /* +2: need at least 2 bytes; might strcpy "-" into buf */
	if(*buf == nil)
		return nil;
	...
	if(**buf == '$') {
		s = getenv(*buf + 1);
		if(s == 0) {
			warn(mkaux, "can't read environment variable %s",
			     *buf + 1);
			skipdir(mkaux);
			free(*buf);
			return nil;
