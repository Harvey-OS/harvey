...
sparc64inst(Map *map, uint64_t pc, char modifier, char *buf, int n)
{
	...
	if((asstype == ASUNSPARC && modifier == 'i') || (asstype == ASPARC && modifier == 'I'))
		dascase = 'a' - 'A';
	else
		dascase = 0;
...
address(Instr *i)
{
	...
	if(i->rs1 == 2 && findsym(off, CANY, &s) && s.value - off < 4096 && (s.class == CDATA || s.class == CTEXT)) {
...
add(Instr *i, char *m) /* page 82 */
{
	if(i->i == 0) {
		...
		else if(i->op3 == 0 && i->rd && i->rs1 == 0) /* ADD $x, R0, R1 */
