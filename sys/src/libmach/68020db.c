...
static char* excep[] = {
        ...
        [5] "zero divide", [6] "CHK", [7] "TRAP", [8] "privilege violation",
        [9] "Trace", [10] "line 1010", [11] "line 1011",
        [13] "coprocessor protocol violation", [24] "spurious", [25] "incon",
        [26] "tac", [27] "auto 3", [28] "clock", [29] "auto 5", [30] "parity",
        [31] "mouse", [32] "system call", [33] "system call 1",
        [48] "FPCP branch", [49] "FPCP inexact", [50] "FPCP zero div",
        [51] "FPCP underflow", [52] "FPCP operand err", [53] "FPCP overflow",
        [54] "FPCP signal NAN",
};
...
m68020ufix(Map* map)
{
	...
	for(i = 3; i < 100; i++) {
		...
		if(machdata->swal(l) == 0xBADC0C0A) {
			if(get1(map, stktop - (i - 1) * 4, (uint8_t*)&efl[0],
			        4) < 0)
				return -1;
			if(get1(map, stktop - (i - 2) * 4, (uint8_t*)&efl[1],
			        4) < 0)
				return -1;
			fvo = (ef[6] << 8) | ef[7];
			vec = fvo & 0xfff;
...
%j	-	Dq:Dr if Dq != Dr; else Dr => Dr [word 1: 0-2] Dq [word 1:
12-14]
...
*/
/* Operand classes */
enum { EAPI = 1, /* extended address: pre decrement only */
...
struct inst {
	...
	uint16_t
	    raw[4 + 12]; /* longest instruction: 24 byte packed immediate */
	Operand and [2];
...
static Optable t0[] = {
    ...
    {0x0cfc,
     ...
     0x0000,
     {C16, C16},
     "CAS2W	R%a:R%A,R%u:R%U,(%r):(%R)"},
    {0x0efc,
     0xffff,
     0x0000,
     0x0000,
     {C16, C16},
     "CAS2L	R%a:R%A,R%u:R%U,(%r):(%R)"},
...
getshorts(Inst* ip, void* where, int n)
{
	if(ip->n + n < nelem(ip->raw)) {
		if(get1(mymap, ip->addr + ip->n * 2, (uint8_t*)&ip->raw[ip->n],
		        n * 2) < 0) {
			werrstr("can't read instruction: %r");
			return 0;
		}
...
static char* regname[] = {"R0", "R1", "R2", "R3", "R4", "R5", "R6", "R7", "A0",
...
pidx(Inst* ip, int ext, int reg, char* bfmt, char* ifmt, char* nobase)
{
	...
	sprint(buf, "%s.%c%s", regname[(ext >> 12) & 0x0f],
	       (ext & 0x800) ? 'L' : 'W', s);
	if(!printed)
		bprint(ip, ifmt, buf);
	else
...
prindex(Inst* ip, int reg, Operand* ap)
{
	...
	case 0x33:
		...
		if(reg == 15)
			...
			ip->curr +=
			    symoff(ip->curr, ip->end - ip->curr, disp, CANY);
		pidx(ip, ext, reg, "(%s)", "(%s)", 0);
		bprint(ip, ")");
		break;
	case 0x25:
...
pea(int reg, Inst* ip, Operand* ap)
{
	...
	case PDisp:
		ip->curr +=
		    symoff(ip->curr, left, ip->addr + 2 + ap->disp, CANY);
		break;
	case PXD:
		prindex(ip, 16, ap);
		break;
	case ADisp: /* references off the static base */
		if(reg == 6 && mach->sb && ap->disp) {
			ip->curr +=
			    symoff(ip->curr, left, ap->disp + mach->sb, CANY);
			bprint(ip, "(SB)");
			break;
		}
	...
	case IEXT:
		bprint(ip, "$#");
		ip->curr +=
		    beieee80ftos(ip->curr, left - 2, (void*)ap->floater);
		break;
	default:
...
static char* fcond[] = {"F",    "EQ",  "OGT", "OGE", "OLT", "OLE", "OGL", "OR",
		...
		if(*fmt != '%')
			*ip->curr++ = *fmt;
			...
			case 'E': /* destination EA; opcode: [6-11] */
				...
				break;
			case 'F': /* float dest register(s); word 1: [7-9] &
			             [10-12] */
				r1 = (w1 >> 7) & 0x07;
				r2 = (w1 >> 10) & 0x07;
				if(r1 == r2)
