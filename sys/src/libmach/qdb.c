...
static char* excname[] = {
    "reserved 0", "system reset", "machine check", "data access",
    "instruction access", "external interrupt", "alignment",
    "program exception", "floating-point unavailable", "decrementer",
    "i/o controller interface error", "reserved B", "system call", "trace trap",
    "floating point assist", "reserved", "ITLB miss", "DTLB load miss",
    "DTLB store miss", "instruction address breakpoint"
                       "SMI interrupt"
                       "reserved 15",
    "reserved 16", "reserved 17", "reserved 18", "reserved 19", "reserved 1A",
    /* the following are made up on a program exception */
    "floating point exception", /* FPEXC */
    "illegal instruction", "privileged instruction", "trap",
    "illegal operation",
};
...
#define IBF(v, a, b)                                                           \
	(((unsigned long)(v) >> (32 - (b)-1)) & ~(~0L << (((b) - (a) + 1))))
...
pglobal(Instr* i, uint64_t off, int anyoff, char* reg)
{
	...
	if(findsym(off, CANY, &s) && off - s.value < 4096 &&
...
address(Instr* i)
{
	...
	if(i->ra == REGSB && mach->sb &&
	   pglobal(i, mach->sb + i->imm64, 0, "(SB)"))
		return;
	if(i->simm < 0)
...
sw(Opcode* o, Instr* i, char r)
{
	...
	if(i->rs == REGSP) {
		if(findsym(i->addr, CTEXT, &s) &&
		   findlocal(&s, FRAMENAME, &s)) {
			offset = s.value - i->imm64;
			if(offset > 0 && getauto(&s, offset, CAUTO, &s)) {
				format(o->mnemonic, i, nil);
				bprint(i, "\t%c%d,%s-%d(SP)", r, i->rd, s.name,
				       offset);
				return;
			}
		}
...
static void and (Opcode * o, Instr* i)
{
	if(i->op == 31) {
		/* Rb,Rs,Ra */
		if(i->ra == i->rs)
...
static Opcode opcodes[] = {
    ...
    {31, 598, ALL, "SYNC", gen,
     0}, /* TO DO: there's a parameter buried in there */
...
shmask(uint64_t* m)
{
	...
	if(i > 63)
		return 0;
	if(*m &
	   ~((uint64_t)1 << i)) { /* more than one bit: do multiples of bytes */
		i = (i / 8) * 8;
		if(i == 0)
			return 0;
	}
...
format(char* mnemonic, Instr* i, char* f)
{
	...
	for(; *f; f++) {
		...
		case 'E':
			switch(IBF(i->w0, 27, 30)) { /* low bit is top bit of
			                                shift in rldiX cases */
			case 8:
				i->mb = i->xmbe;
				i->me = 63;
				break; /* rldcl */
		...
		case 'z':
			if(i->mb <= i->me)
				mask = ((uint32_t)~0L >> i->mb) &
				       (~0L << (31 - i->me));
			else
				mask = ~(((uint32_t)~0L >> (i->me + 1)) &
				         (~0L << (31 - (i->mb - 1))));
			bprint(i, "%lux", mask);
			break;
