...
fpformat(Map* map, Reglist* rp, char* buf, int n, int modif)
{
	...
	case 'F': /* first reg of double reg pair */
		if(modif == 'F')
			if((rp->rformat == 'F') || (((rp + 1)->rflags & RFLT) &&
			                            (rp + 1)->rformat == 'f')) {
...
ieeedftos(char* buf, int n, uint32_t h, uint32_t l)
{
	...
	if(exp == ((1L << 11) - 1L)) {
		if(l == 0 && (h & ((1L << 20) - 1L)) == 0)
			...
			return snprint(buf, n, "NaN(%.8lux%.8lux)",
			               h & ((1L << 20) - 1L), l);
	}
	exp -= (1L << 10) - 2L;
	fr = l & ((1L << 16) - 1L);
...
riscframe(Map* map, uint64_t addr, uint64_t pc, uint64_t sp, uint64_t link)
{
	...
	while(findsym(pc, CTEXT, &s)) {
		...
		if(s.type == 'L' || s.type == 'l' ||
		   pc - s.value <= mach->szaddr * 2)
			pc = link;
		else if(geta(map, sp - f.value, &pc) < 0)
			break;
	}
	return 0;
}
