...
struct hist {				/* Stack of include files & #line directives */
	...
	i32	line;			/* line # where it was included */
	i32	offset;			/* line # of #line directive */
};

struct file {				/* Per input file header to history stack */
	u64	addr;		/* address of first text sym */
	union {
...
static	i32		nauto;		/* number of automatics */
static	i32		nfiles;		/* number of files */
static	i32		nglob;		/* number of globals */
static	i32		nhist;		/* number of history stack entries */
static	i32		nsym;		/* number of symbols */
static	int		ntxt;		/* number of text symbols */
static	u8		*pcline;	/* start of pc-line state table */
static	u8 	*pclineend;	/* end of pc-line table */
static	u8		*spoff;		/* start of pc-sp state table */
static	u8		*spoffend;	/* end of pc-sp offset table */
static	Sym		*symbols;	/* symbol table */
static	char		*strings;	/* string table */
static	Txtsym		*txt;		/* Base of text symbol table */
static	u64	txtstart;	/* start of text segment */
static	u64	txtend;		/* end of text segment */

static void	cleansyms(void);
//static i32	decodename(Biobuf*, Sym*);
static i16	*encfname(char*);
static int 	fline(char*, int, i32, Hist*, Hist**);
...
static int	hcomp(Hist*, i16*);
static int	hline(File*, i16*, i32*);
...
syminit(int fd, Fhdr *fp)
{
	...
	u16 (*swab)(u16) = fp->bigendian ? beswab : leswab;
	u32 (*swal)(u32) = fp->bigendian ? beswal : leswal;
	u64 (*swav)(u64) = fp->bigendian ? beswav : leswav;
	...
	for (int i = 1; i < nsym; i++) {
		...
		u8 binding = esyms[i].st_info >> 4;
		u8 symtype = esyms[i].st_info & 0xf;
		u16 sectionidx = esyms[i].st_shndx;
...
filesym(int index, char *buf, int n)
{
	...
	if(!hp || !hp->name)
		return 0;
	return fileelem(fnames, (u8*)hp->name, buf, n);
}
 ...
 */
int
findsym(u64 val, int type, Symbol *s)
{
	int i;
...
encfname(char *file)
{
	...
	for(i = 0; *cp; i++) {
		...
		if(j < 0)
			return 0;	/* not found */
		dest = realloc(dest, (i+1)*sizeof(i16));
		dest[i] = j;
		cp = cp2;
 ...
 */
static int
hline(File *fp, i16 *name, i32 *line)
{
	Hist *hp;
	int offset, depth;
	i32 ln;
 ...
 */
i32
fileline(char *str, int n, u64 dot)
{
	i32 line, top, bot, mid;
	File *f;
 ...
 */
static int
fline(char *str, int n, i32 line, Hist *base, Hist **ret)
{
	Hist *start;			/* start of current level */
	Hist *h;			/* current entry */
	i32 delta;			/* sum of size of files this level */
	int k;
	...
	if(!h->name)
		...
		k = fileelem(fnames, (u8*)start->name, str, n);
		if(k+8 < n)
 ...
 */
int
fileelem(Sym **fp, u8 *cp, char *buf, int n)
{
	int i, j;
	char *c, *bp, *end;
	Sym *sym;
 ...
 */
u64
line2addr(i32 line, u64 basepc, u64 endpc)
{
	u8 *c,  u;
	u64 currpc, pc;
	i32 currline;
...
printhist(char *msg, Hist *hp, int count)
{
	...
	while(hp->name) {
		...
		for(cp = (u8 *)hp->name+1; (*cp<<8)|cp[1]; cp += 2) {
			if (cp != (u8 *)hp->name+1)
				print("/");
			print("%x", (*cp<<8)|cp[1]);
		}
		fileelem(fnames, (u8 *) hp->name, buf, sizeof(buf));
		print(" (%s)\n", buf);
		hp++;
	}
}
