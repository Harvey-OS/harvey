/*
 * Intel 8254[013467]N Gigabit Ethernet PCI Controllers:
...
enum {
	...
	Nrb		= 1024,		/* max. private receive buffers */

	Descalign	= 128,
	Rbsz		= ETHERMAXTU+32, /* was 2048 */
...
static Lock igberblock;		/* lock for free receive Blocks */
...
igbeifstat(Ether* edev, void* a, long n, ulong offset)
{
	...
	for(i = 0; i < Nstatistics; i++){
		...
		case Totl:
			...
			ruvl = (uvlong)csr32r(ctlr, Statistics+(i+1)*4)<<32 | r;
...
igbetransmit(Ether* edev)
{
	 ...
	 */
	tdh = ctlr->tdh;
	while(NEXT(tdh, Ntd) != csr32r(ctlr, Tdh)){
	 ...
	 */
	tdt = ctlr->tdt;
	while(NEXT(tdt, Ntd) != tdh){
		if((bp = qget(edev->oq)) == nil)
			break;

		td = &ctlr->tdba[tdt];
		/* td->status&Tdd should have been cleared by i82563cleanup() */
...
igbereplenish(Ctlr* ctlr)
{
	...
	while(NEXT(rdt, Nrd) != ctlr->rdh){
		...
		if(*rb == nil){
			*rb = bp = igberballoc();
			if(bp == nil){
				iprint("#l%d: igbe: no available rx buffers\n",
					ctlr->edev->ctlrno);
				break;
			}
...
igberxinit(Ctlr* ctlr)
{
	...
	csr32w(ctlr, Rxdctl, 8<<WthreshSHIFT|8<<HthreshSHIFT|4<<PthreshSHIFT);
...
igberproc(void* arg)
{
	...
	for(;;){
		...
		if(ctlr->rdfree < Nrd/2 || (ctlr->rim & Rxdmt0))
			igbereplenish(ctlr);
...
igbeattach(Ether* edev)
{
	...
	ctlr->alloc = malloc(Nrd*sizeof(Rd)+Ntd*sizeof(Td) + Descalign - 1);
	if(ctlr->alloc == nil) {
		print("igbepci: can't allocate ctlr->alloc\n");
		error(Enomem);
	}
	ctlr->rdba = (Rd*)ROUNDUP((uintptr)ctlr->alloc, Descalign);
	ctlr->tdba = (Td*)(ctlr->rdba+Nrd);

	ctlr->rb = malloc(Nrd*sizeof(Block*));
	ctlr->tb = malloc(Ntd*sizeof(Block*));
	if (ctlr->rb == nil || ctlr->tb == nil) {
		print("igbepci: can't allocate ctlr->rb or ctlr->tb\n");
		error(Enomem);
	}
...
/*
 * 82543 has a different MII management interface from all the others.
 */
...
igbemii(Ctlr* ctlr)
{
	...
	case i82543gc:
		...
		if(!(r & Mdro)) {
			print("igbepci: 82543gc Mdro not set\n");
			return -1;
		}
...
at93c46r(Ctlr* ctlr)
{
	...
	eecd = csr32r(ctlr, Eecd);
	if(eecd & Spi){
		print("igbepci: SPI EEPROM access not implemented\n");
		return 0;
	}
	...
	case i82547gi:
		...
		if(!(eecd & Agnt)){
			print("igbepci: not granted EEPROM access\n");
			goto release;
		}
		break;
	}
	...
	for(addr = 0; addr < 0x40; addr++){
		...
		if(at93c46io(ctlr, rop, (0x06<<bits)|addr) != 0){
			print("igbepci: can't set EEPROM address 0x%2.2X\n", addr);
			goto release;
		}
...
igbedetach(Ctlr* ctlr)
{
	...
	/* parallels 10.1.3 and 13 are broken: Eerst never goes to 0 */
	if(csr32r(ctlr, Ctrlext) & Eerst) {
		vmbotch(Parallels, "your actual igbe's Ctrlext's Eerst bit stuck on");
		if (!conf.vm)
			return -1;
	}
...
static void
igbeloaddefs(Ctlr *ctlr)
{
	int ctrl, pausectl, swdpio, txcw;
	...
	if(ctlr->eeprom[Icw2] & 0x0800)
		txcw |= TxcwAne;
	pausectl = (ctlr->eeprom[Icw2] & 0x3000)>>12;
	txcw |= pausectl<<TxcwPauseSHIFT;
	switch(pausectl){
	default:
...
igbereset(Ctlr* ctlr)
{
	...
	if((r = at93c46r(ctlr)) != 0xBABA){
		print("igbepci: bad EEPROM checksum - 0x%4.4uX\n", r);
		return -1;
	}
...
igbepci(void)
{
	...
	while(p = pcimatch(p, 0, 0)){
		...
		if(mem == nil){
			print("igbepci: can't map %8.8luX\n", p->mem[0].bar);
			continue;
		}
		...
		default:
			...
			print("igbepci: p->cls %d, setting to %d longs\n",
				p->cls, cls);
			p->cls = cls;
...
void
etherigbepcilink(void)
{
	addethercard("i82543", igbepnp);
	addethercard("igbepci", igbepnp);
}
