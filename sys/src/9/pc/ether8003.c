...
wd8003reset(Ctlr *ctlr)
{
	...
	if(memcmp(&ctlr->ea[1], &ic[1], 5) == 0){
		...
		if(inb(wd8003+Gp2) != 0xAA){
			...
			ctlr->card.irq = intrmap[((ic[Irr]>>5) & 0x3)|(ic[Icr] & Ir2)];
	...
	if(getisa(ctlr->card.mem, ctlr->card.size, 0) == 0)
		panic("ether8003: 0x%lux reused", ctlr->card.mem);
