...
serial(ulong id1, ulong id2)
{
	...
	if (i1 > 0 && i1 < 27 && i2 > 0 && i2 < 27 && i3 > 0 && i3 < 27 &&
	    (id1 & (1<<7)) == 0)
		snprint(buf, sizeof(buf), "%c%c%c%.4lux.%lux",
			'A'+i1-1, 'A'+i2-1, 'A'+i3-1, x, id2);
	else
		snprint(buf, sizeof(buf), "%.4lux%.4lux.%lux",
			(id1<<8)&0xff00|(id1>>8)&0x00ff, x, id2);
	return buf;
}
...
pnpscan(int rddata, int dawn)
{
	...
	cmd(0x02, 0x04+0x01);  /* reset CSN on all cards & reset logical devs */
	...
	while(isolate(rddata, &id1, &id2)) {
		...
		/* Wake all cards with a CSN of 0, putting this card to sleep */
		cmd(0x03, 0);
	}
...
pnpreset(void)
{
	...
	for(csn = 1; csn < 256; csn++) {
		...
		if(s == 0)
			continue;
		if(strlen(s) < 8 || s[7] != '.' || s[0] < 'A' || s[0] > 'Z' ||
		    s[1] < 'A' || s[1] > 'Z' || s[2] < 'A' || s[2] > 'Z') {
bad:
		...
		i3 = s[2]-'A'+1;
		x = strtoul(&s[3], 0, 16);
		id1 = i1<<2 | (i2>>3)&3 | (i2&7)<<13 | i3<<8 | (x&0xff)<<24 |
			(x&0xff00)<<8;
...
wrconfig(Card *, char *)
{
	/* Should implement setting of I/O bases, etc. by executing string */
	return 1;
}
