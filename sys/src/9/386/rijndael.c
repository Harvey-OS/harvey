...
#define ls_box(x)							 \
	((uint32_t)sbx_tab[byte(x, 0)] <<  0) ^	 \
	((uint32_t)sbx_tab[byte(x, 1)] <<  8) ^	 \
	((uint32_t)sbx_tab[byte(x, 2)] << 16) ^	 \
	((uint32_t)sbx_tab[byte(x, 3)] << 24)

#define f_rl(bo, bi, n, k)											\
	(bo)[n] = (uint32_t)sbx_tab[byte((bi)[n],0)] ^					\
		rotl(((uint32_t)sbx_tab[byte((bi)[((n) + 1) & 3],1)]),  8) ^	\
		rotl(((uint32_t)sbx_tab[byte((bi)[((n) + 2) & 3],2)]), 16) ^	\
		rotl(((uint32_t)sbx_tab[byte((bi)[((n) + 3) & 3],3)]), 24) ^ *((k) + (n))

#define i_rl(bo, bi, n, k)											\
	(bo)[n] = (uint32_t)isb_tab[byte((bi)[n],0)] ^					\
		rotl(((uint32_t)isb_tab[byte((bi)[((n) + 3) & 3],1)]),  8) ^	\
		rotl(((uint32_t)isb_tab[byte((bi)[((n) + 2) & 3],2)]), 16) ^	\
		rotl(((uint32_t)isb_tab[byte((bi)[((n) + 1) & 3],3)]), 24) ^ *((k) + (n))
#endif
...
rijndael_ctx *
rijndael_set_key(rijndael_ctx *ctx, const uint32_t *in_key, const uint32_t key_len,
				 int encrypt)
{
	uint32_t		i,
				t,
...
void
rijndael_encrypt(rijndael_ctx *ctx, const uint32_t *in_blk, uint32_t *out_blk)
{
...
void
rijndael_decrypt(rijndael_ctx *ctx, const uint32_t *in_blk, uint32_t *out_blk)
{
	uint32_t		b0[4],
				b1[4],
			   *kp;
