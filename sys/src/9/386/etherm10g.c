...
enum { AercCCR = 0x18, /* control register */
};
...
m10gifstat(Ether* e, void* v, int32_t n, uint32_t off)
{
	...
	snprint(p + l, lim, "txcnt = %ud\n"
	        ...
	        gbit32(s.dnosm), gbit32(s.dnobg), gbit32(s.nrdma), s.txstopped,
	        s.down, s.updated, s.valid, c->tx.npkt, c->tx.nbytes, c->tx.cnt,
	        c->tx.n, c->tx.i, c->sm.cnt, c->sm.i, c->sm.pool->n,
	        lstcount(c->sm.pool->head), c->bg.cnt, c->bg.i, c->bg.pool->n,
	        lstcount(c->bg.pool->head), c->tx.segsz,
	        gbit32((uint8_t*)c->coal));
...
static Cmdtab ctab[] = {
    CMdebug,  "debug",  2, CMcoal,     "coal",     2,
    CMwakeup, "wakeup", 1, CMtxwakeup, "txwakeup", 1,
    //	CMqsummary,	"q",		1,
    CMrxring, "rxring", 1,
};
...
m10gctl(Ether* e, void* v, int32_t n)
{
	...
	case CMwakeup:
		wakeup(
		    &((Ctlr*)e->ctlr)->rxrendez); /* you're kidding, right? */
		break;
	case CMtxwakeup:
		wakeup(
		    &((Ctlr*)e->ctlr)->txrendez); /* you're kidding, right? */
		break;
	//	case CMqsummary:
	//		summary(e);
	//		break;
