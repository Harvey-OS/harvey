...
static char stab[] = {[0] 'i', 'm', [8] 't', 'c', 'p', 'e', [16] 'N', 'I', 'W',
                      'B',     'D', 'C',     'H', 'S', 'T', 'F',      'X'};
...
updatedrive(Drive* d)
{
	...
	if(cause & Adhrs) {
		if(p->task & (1 << 5 | 1)) {
			dprint("ahci: %s: Adhrs cause %#lux serr %#lux task "
			       "%#lux\n",
...
resetdisk(Drive* d)
{
	...
	dprint("ahci: %s: resetdisk: %s → %s\n",
	       (d->unit ? d->unit->name : nil), diskstates[state],
	       diskstates[d->state]);
	qunlock(&d->portm);
}
...
#define has(x, str)                                                            \
	...
	return seprint(p, e, "iss %ld ncs %ld np %ld; ghc %#lux isr %#lux pi "
	                     "%#lux %s ver %#lux\n",
