...
static char* statistics[Nstatistics] = {
    ...
    "Packets Received (1024-1522 Bytes)", "Good Packets Received",
    ...
    "Receive Jabber", nil, nil, nil, "Total Octets Received", nil,
    ...
    "Packets Transmitted (1024-1522 Bytes)", "Multicast Packets Transmitted",
...
static void
igbemulticast(void* arg, unsigned char* addr, int add)
{
	int bit, x;
	Ctlr* ctlr;
	Ether* edev;
...
igbereplenish(Ctlr* ctlr)
{
	...
	while(NEXT(rdt, ctlr->nrd) != ctlr->rdh) {
		...
		if(ctlr->rb[rdt] == nil) {
			bp = igberballoc();
			if(bp == nil) {
				iprint("#l%d: igbereplenish: no available "
				       "buffers\n",
				       ctlr->edev->ctlrno);
				break;
			}
...
igbeattach(Ether* edev)
{
	...
	ctlr->alloc =
	    malloc(ctlr->nrd * sizeof(Rd) + ctlr->ntd * sizeof(Td) + 127);
	if(ctlr->alloc == nil) {
