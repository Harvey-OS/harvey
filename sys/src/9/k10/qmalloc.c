...
enum { Unitsz = sizeof(Header), /* 16 bytes on amd64 */
};
...
static char* qstatstr[QSmax] = {
        [QSmalign] "malign", [QSmalignquick] "malignquick",
        [QSmalignrover] "malignrover", [QSmalignfront] "malignfront",
        [QSmalignback] "malignback", [QSmaligntail] "maligntail",
        [QSmalignnottail] "malignnottail", [QSmalloc] "malloc",
        [QSmallocrover] "mallocrover", [QSmalloctail] "malloctail",
        [QSfree] "free", [QSfreetail] "freetail", [QSfreequick] "freequick",
        [QSfreenext] "freenext", [QSfreeprev] "freeprev",
};
...
#define tailalloc(p, n)                                                        \
	((p) = tailptr, tailsize -= (n), tailptr += (n), (p)->s.size = (n),    \
	 (p)->s.next = &checkval)
...
mallocreadfmt(char* s, char* e)
{
	...
	p = seprint(s, e, "%llud memory\n"
	                  "%d pagesize\n"
	                  "%llud kernel\n",
	            (uint64_t)conf.npage * PGSZ, PGSZ,
