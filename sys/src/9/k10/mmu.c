...
mmuflushtlb(uint64_t u)
{
	...
	machp()->tlbpurge++;
	if(machp()->pml4->daddr) {
		memset(UINT2PTR(machp()->pml4->va), 0,
		       machp()->pml4->daddr * sizeof(PTE));
		machp()->pml4->daddr = 0;
	}
	...
	for(i = 3; i > 0; i--) {
		...
		for(pg = p->mmuptp[i]; pg != nil; pg = pg->next)
			print("\tpg %#p = va %#ullx pa %#ullx"
			      ...
			      pg, pg->va, pg->pa, pg->daddr, pg->next,
			      pg->prev);
	}
	print("pml4 %#ullx\n", machp()->pml4->pa);
	if(0)
...
checkpte(uintmem ppn, void* a)
{
	...
	*s = 0;
	if((l = mmuwalk(pml4, addr, 3, &pte, nil)) < 0 || (*pte & PteP) == 0)
		goto Panic;
	s = seprint(buf, buf + sizeof buf, "check3: l%d pte %#p = %llux\n", l,
	            pte, pte ? *pte : ~0);
	if((l = mmuwalk(pml4, addr, 2, &pte, nil)) < 0 || (*pte & PteP) == 0)
		goto Panic;
	s = seprint(s, buf + sizeof buf, "check2: l%d  pte %#p = %llux\n", l,
	            pte, pte ? *pte : ~0);
	if(*pte & PtePS)
		return;
	if((l = mmuwalk(pml4, addr, 1, &pte, nil)) < 0 || (*pte & PteP) == 0)
		goto Panic;
	seprint(s, buf + sizeof buf, "check1: l%d  pte %#p = %llux\n", l, pte,
	        pte ? *pte : ~0);
	return;
Panic:
...
pteflags(uint attr)
{
	...
	flags = 0;
	if(attr &
	   ~(PTEVALID | PTEWRITE | PTERONLY | PTEUSER | PTEUNCACHED |
	     PTENOEXEC))
...
int
mmuwalk(PTE* pml4, uintptr_t va, int level, PTE** ret, uint64_t (*alloc)(usize))
{
	int l;
	uintmem pa;
	PTE* pte;
...
mmuinit(void)
{
	...
	if(machp()->machno != 0) {
		...
		machp()->pml4->daddr =
		    mach0pml4.daddr; /* # of user mappings in pml4 */
	...
	assert((pdeget(PDMAP) & ~(PteD | PteA)) ==
	       (PADDR(sys->pd) | PteRW | PteP));
