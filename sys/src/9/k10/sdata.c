...
enum { BMspan = 64 * 1024, /* must be power of 2 <= 64*1024 */
...
enum { Last28 = (1 << 28) - 1 - 1, /* all-ones mask is not addressible */
};
...
atadumpstate(Drive* drive, uint8_t* cmd, int64_t lba, int count)
{
	...
	print("data %8.8p limit %8.8p dlen %d status %uX error %uX\n",
	      drive->data, drive->limit, drive->dlen, drive->status,
	      drive->error);
	if(cmd != nil) {
		print("lba %d -> %lld, count %d -> %d (%d)\n",
		      (cmd[2] << 24) | (cmd[3] << 16) | (cmd[4] << 8) | cmd[5],
		      lba, (cmd[7] << 8) | cmd[8], count, drive->count);
	}
	...
	if(ctlr->pcidev && ctlr->pcidev->vid == 0x8086) {
		p = ctlr->pcidev;
		print("0x40: %4.4uX 0x42: %4.4uX", pcicfgr16(p, 0x40),
		      pcicfgr16(p, 0x42));
		print("0x48: %2.2uX\n", pcicfgr8(p, 0x48));
		print("0x4A: %4.4uX\n", pcicfgr16(p, 0x4A));
	}
...
atadebug(int cmdport, int ctlport, char* fmt, ...)
{
	...
	if(cmdport) {
		if(buf[n - 1] == '\n')
			n--;
		n +=
		    snprint(buf + n, PRINTSIZE - n, " ataregs 0x%uX:", cmdport);
...
ataready(int cmdport, int ctlport, int dev, int reset, int ready, int micro)
{
	...
	atadebug(cmdport, ctlport, "ataready: dev %uX reset %uX ready %uX", dev,
	         reset, ready);
...
tryedd1:
	if(ataready(cmdport, ctlport, dev, Bsy | Drq, 0, 105 * 1000) < 0) {
		 ...
		 */
		atasrst(ctlport);
		if(ataready(cmdport, ctlport, dev, Bsy | Drq, 0, 106 * 1000) <
		   0)
			goto release;
	}
...
atastandby(Drive* drive, int period)
{
	...
	while(waserror())
		;
	tsleep(&ctlr->_rendez, atadone, ctlr, 60 * 1000);
	poperror();
...
atapktio(Drive* drive, uint8_t* cmd, int clen)
{
	...
	as =
	    ataready(cmdport, ctlport, drive->dev, Bsy | Drq, Drdy, 107 * 1000);
...
static uint8_t cmd48[256] = {
        [Crs] Crs48, [Crd] Crd48, [Crdq] Crdq48, [Crsm] Crsm48,
        [Cws] Cws48, [Cwd] Cwd48, [Cwdq] Cwdq48, [Cwsm] Cwsm48,
};
...
atageniostart(Drive* drive, uint64_t lba)
{
	...
	cmdport = ctlr->cmdport;
	ctlport = ctlr->ctlport;
	if(ataready(cmdport, ctlport, drive->dev, Bsy | Drq, Drdy, 101 * 1000) <
	   0)
		return -1;

	ilock(ctlr);
	if(drive->dmactl &&
	   !atadmasetup(drive, drive->count * drive->secsize)) {
		if(drive->write)
			drive->command = Cwd;
		else
			drive->command = Crd;
...
static void
atainterrupt(Ureg* ureg, void* arg)
{
	Ctlr* ctlr;
	Drive* drive;
...
atapnp(void)
{
	...
	while(p = pcimatch(p, 0, 0)) {
		...
		/*FALLTHROUGH*/
		case(0x0680 << 16) |
		    0x1095: /* SiI 0680/680A PATA133 ATAPI/RAID */
		...
		case(0x2363 << 16) | 0x197b: /* JMicron SATA */
			break; /* TODO: verify that this should be here; wasn't
			          in original patch */
		...
		//		case (0x27C4<<16)|0x8086:	/* 82801GBM SATA
		//(ICH7) */
		case(0x27C5 << 16) | 0x8086: /* 82801GBM SATA AHCI (ICH7) */
		case(0x2920 << 16) |
		    0x8086: /* 82801(IB)/IR/IH/IO SATA IDE (ICH9) */
	...
	if(0){
	         ...
	         * /
	        memset(&isa, 0, sizeof(isa));
	        isa.port = 0x180;		/ * change this for your
	machine * /
	        isa.irq = 11;			/ * change this for your
	machine * /
	        ...
	        if(channel >= 0 && (sdev = ataprobe(isa.port, port, isa.irq)) !=
	nil){
	                if(head != nil)
	                        tail->next = sdev;
	                else
...
atarctl(SDunit* unit, char* p, int l)
{
	...
	n += snprint(p + n, l - n, "\n");
	n +=
	    snprint(p + n, l - n, "interrupts read %lud write %lud cmds %lud\n",
	            drive->intrd, drive->intwr, drive->intcmd);
	if(drive->sectors) {
		n += snprint(p + n, l - n, "geometry %lld %d", drive->sectors,
		             drive->secsize);
		if(drive->pkt == 0)
			n += snprint(p + n, l - n, " %d %d %d", drive->c,
			             drive->h, drive->s);
		n += snprint(p + n, l - n, "\n");
	}
	qunlock(drive);
