...
noted(Ureg* cur, uintptr_t arg0)
{
	...
	if(nur->cs != SSEL(SiUCS, SsRPL3) || nur->ss != SSEL(SiUDS, SsRPL3)) {
		qunlock(&up->debug);
		pprint("suicide: bad segment selector (cs %p want %p, ss %p "
		       "want %p), in noted\n",
		       nur->cs, SSEL(SiUCS, SsRPL3), nur->ss,
		       SSEL(SiUDS, SsRPL3));
		pexit("Suicide", 0);
	}
...
notify(Ureg* ureg)
{
	...
	sp = ureg->sp - ROUNDUP(sizeof(NFrame), 16) -
	     128; // amd64 red zone, also wanted by go stack traces
...
syscall(int badscallnr, Ureg* ureg)
{
	...
	if(0)
		iprint("Syscall %d, %lx, %lx, %lx %lx %lx\n", scallnr, a0, a1,
		       a2, a3, a4);
	char* e;
	uintptr_t sp;
	int s;
	...
	if(up->procctl == Proc_tracesyscall) {
		 ...
		 */
		if(sp < (USTKTOP - BIGPGSZ) ||
		   sp > (USTKTOP - sizeof(up->arg) - BY2SE))
	...
	if(printallsyscalls) {
		stopns = todget(nil);
		sysretfmt(scallnr, &ar0, startns, stopns, a0, a1, a2, a3, a4,
		          a5);
		if(up->syscalltrace) {
			print("X %s\n", up->syscalltrace);
			free(up->syscalltrace);
...
sysexecregs(uintptr_t entry, uint32_t ssize, void* tos)
{
	...
	if(ssize & 0xf) {
		print("your stack is wrong: stacksize is not 16-byte aligned: "
		      "%d\n",
		      ssize);
...
#define STACKPAD 1 /* for return PC? */
	            ...
	            */
	child->sched.sp = PTR2UINT(child->kstack + KSTACK -
	                           ((sizeof(Ureg) + STACKPAD * BY2SE)));
