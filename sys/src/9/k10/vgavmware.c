...
static void
vmwarelinear(VGAscr* scr, int i, int n)
{
	char err[64];
	Pcidev *p;
...
vmfifowr(Vmware *vm, uint32_t v)
{
	...
	if(mm[FifoNextCmd]+sizeof(uint32_t) == mm[FifoStop]
	|| (mm[FifoNextCmd]+sizeof(uint32_t) == mm[FifoMax]
	    && mm[FifoStop] == mm[FifoMin]))
		vmwait(vm);

	mm[mm[FifoNextCmd]/sizeof(uint32_t)] = v;

	/* must do this way so mm[FifoNextCmd] is never mm[FifoMax] */
	v = mm[FifoNextCmd] + sizeof(uint32_t);
	if(v == mm[FifoMax])
		v = mm[FifoMin];
	mm[FifoNextCmd] = v;
}
...
static void
vmwareflush(VGAscr *scr, Rectangle r)
{
	if(vm->mmio == nil)
		return;
...
static void
vmwareload(VGAscr *scr, Cursor *c)
{
	int i;
	uint32_t clr, set;
	uint32_t and[16];
...
static int
vmwarescroll(VGAscr *scr, Rectangle r, Rectangle sr)
{
	if(vm->mmio == nil)
		return 0;
...
static int
vmwarefill(VGAscr *scr, Rectangle r, uint32_t sval)
{
	if(vm->mmio == nil)
		return 0;
...
vmwaredrawinit(VGAscr *scr)
{
	...
	scr->mmio[FifoMin] = 4*sizeof(uint32_t);
	scr->mmio[FifoMax] = vm->mmiosize;
	scr->mmio[FifoNextCmd] = 4*sizeof(uint32_t);
	scr->mmio[FifoStop] = 4*sizeof(uint32_t);
