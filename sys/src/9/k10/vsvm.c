...
enum { Ngdt = 16,  /* max. entries in gdt */
...
vsvminit(int size, int nixtype, Mach* mach)
{
	...
	*sd = mksd(PTR2UINT(mach->tss), sizeof(Tss) - 1, SdP | SdDPL0 | SdaTSS,
	           sd + 1);
	*(uintptr_t*)mach->stack = STACKGUARD;
	tssinit(mach, mach->stack + size);
	gdtput(sizeof(gdt64) - 1, PTR2UINT(mach->gdt),
	       SSEL(SiCS, SsTIGDT | SsRPL0));
