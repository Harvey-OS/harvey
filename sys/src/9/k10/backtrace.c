...
backtrace_list(uintptr_t pc, uintptr_t fp, uintptr_t* pcs, size_t nr_slots)
{
	...
	while(fp && nr_pcs < nr_slots) {
		...
		if(fp < KTZERO)
			break;
		/* PC becomes the retaddr - 1.  the -1 is to put our PC back
		 * inside the
		 * function that called us.  this was necessary in case we
		 * called as the
		 * last instruction in a function (would have to never return).
		 * not
