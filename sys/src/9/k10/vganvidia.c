...
static void
nvidialinear(VGAscr *scr, int i, int n)
{
}
...
nvidiaenable(VGAscr* scr)
{
	...
	case 0x0020:
	case 0x00A0:
		q = (void*)((unsigned char*)scr->mmio + Pfb);
		tmp = *q;
		if (tmp & 0x0100) {
	...
	case 0x01F0:
		...
		break;
	default:
		q = (void*)((unsigned char*)scr->mmio + Pfb + 0x020C);
		tmp = (*q >> 20) & 0xFFF;
		if (tmp == 0)
			tmp = 16;
...
nvidiacurload(VGAscr* scr, Cursor* curs)
{
	...
	case 0x0020:
	case 0x00A0:
		p = (void*)((unsigned char*)scr->mmio + Pramin + 0x1E00 * 4);
		break;
	default:
		/*
		...
		p = (void*)((unsigned char*)scr->vaddr + tmp);
...
nvidiacurmove(VGAscr* scr, Point p)
{
	...
	cursorpos = (void*)((unsigned char*)scr->mmio + hwCurPos);
...
writeput(VGAscr *scr, int data)
{
	...
	p = scr->vaddr;
	scratch = *p;
	fifo = (void*)((unsigned char*)scr->mmio + Fifo);
	fifo[0x10] = (data << 2);
	USED(scratch);
}
...
waitforidle(VGAscr *scr)
{
	...
	pgraph = (void*)((unsigned char*)scr->mmio + Pgraph);
...
nvresetgraphics(VGAscr *scr)
{
	...
	if(nv.dmabase == nil){
		if(scr->storage <= scr->apsize)
			nv.dmabase = (uint32_t*)((unsigned char*)scr->vaddr + scr->storage - 128*1024);
		else{
