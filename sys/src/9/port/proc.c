...
char* statename[] = {
    ...
    "Dead", "Moribund", "Ready", "Scheding", "Running", "Queueing", "QueueingR",
    "QueueingW", "Wakeme", "Broken", "Stopped", "Rendez", "Waitrelease",
    "Exotic", "Down",
};
...
sched(void)
{
	...
	if(!islo() && machp()->ilockdepth)
		panic("cpu%d: ilockdepth %d, last lock %#p at %#p, sched "
		      "called from %#p",
		      machp()->machno, machp()->ilockdepth,
		      up ? up->lastilock : nil,
	...
	if(up) {
		...
		if(up->nlocks)
			if(up->state != Moribund)
				if(up->delaysched < 20 || pga.Lock.p == up ||
				   procalloc.Lock.p == up) {
					up->delaysched++;
					run.delayedscheds++;
					return;
...
preempted(void)
{
	...
	if(up && up->state == Running)
		if(up->preempted == 0)
			if(anyhigher())
				if(!active.exiting) {
					/*  Core 0 is dispatching all
					interrupts, so no core
					 *  actually running a user process is
					ever going call preempted, unless
					 *  we consider IPIs for preemption or
					we distribute interrupts.
					 *  But we are going to use SMP for
...
updatecpu(Proc* p)
{
	...
	t = sys->machptr[0]->ticks * Scaling +
	    Scaling / 2; // Originally MACHP(0)
	n = t - p->lastupdate;
	p->lastupdate = t;
...
rebalance(void)
{
	...
	another:
		...
		if(p == nil)
			continue;
		if(p->mp !=
		   sys->machptr[machp()->machno]) // MACHP(machp()->machno)
			continue;
		if(pri == p->basepri)
			continue;
		updatecpu(p);
 ...
 * 3. rescheduling one that run more than HZ, in the hope he gets his priority
 * lowered.
 */
static void
preemptfor(Proc* p)
...
loop:
	...
	for(rq = &run.runq[Nrq - 1]; rq >= run.runq; rq--)
		for(p = rq->head; p; p = p->rnext) {
			/*
			 * wired processes may only run when their core is
			 * available.
			 */
			if(p->wired != nil) {
				if(p->wired->proc == nil)
			...
			/*
			 * find a ready process that did run at an available
			 * core
...
found:
	...
	if(mp != nil) {
		...
		for(i = 0; i < MACHMAX; i++) {
			...
			mp->nixtype == NIXTC){*/
			if((mp = sys->machptr[i]) !=
			   nil) { // && mp->online && mp->nixtype == NIXTC){
...
found:
	...
	if(edflock(p)) {
		edfrun(p,
		       rq ==
		           &run.runq[PriEdf]); /* start deadline timer and do
		                                  admin */
		edfunlock();
	}
	if(p->trace)
...
found:
	...
	p->state = Scheding;
	if(0)
		hi("runproc, pm->mp = sys->machptr[machp()->machno]\n");
	p->mp = sys->machptr[machp()->machno];
	if(0) {
		hi("runproc, sys->machptr[machp()->machno] = ");
		put64((uint64_t)p->mp);
		hi("\n");
	}
...
runproc(void)
{
	...
	if(machp()->machno != 0) {
		...
		if(edflock(p)) {
			edfrun(p, rq == &run.runq[PriEdf]); /* start deadline
			                                       timer and do
			                                       admin */
			edfunlock();
		}
...
sleep(Rendez* r, int (*f)(void*), void* arg)
{
	...
	if(up->nlocks)
		print("process %d sleeps with %d locks held, last lock %#p "
		      ...
		      up->pid, up->nlocks, up->lastlock, up->lastlock->_pc,
		      getcallerpc(&r));
	lock(r);
	lock(&up->rlock);
	if(r->_p) {
	...
	for(;;) {
		...
		if(canlock(r)) {
			if(p->state != Wakeme || r->_p != p)
				panic("postnote: state %d %d %d", r->_p != p,
				      p->r != r, p->state);
			p->r = nil;
			r->_p = nil;
			ready(p);
		...
		/* give other process time to get out of critical section and
		 * try again */
		unlock(&p->rlock);
		splx(pl);
		sched();
...
addbroken(Proc* p)
{
	...
	if(broken.n == NBROKEN) {
		ready(broken.p[0]);
		memmove(&broken.p[0], &broken.p[1],
		        sizeof(Proc*) * (NBROKEN - 1));
		--broken.n;
	}
	broken.p[broken.n++] = p;
	qunlock(&broken);
...
pexit(char* exitstr, int freemem)
{
	...
	if(up->kp == 0) {
		...
		wq->w.time[TReal] =
		    tk2ms(sys->machptr[0]->ticks - up->time[TReal]);
...
dumpaproc(Proc* p)
{
	...
	for(sno = 0; sno < NSEG; sno++)
		if(p->seg[sno] != nil &&
		   (p->seg[sno]->type & SG_TYPE) == SG_BSS)
			bss = p->seg[sno]->top;

	s = p->psstate;
	if(s == 0)
		s = statename[p->state];
	print("%3d:%10s pc %#p dbgpc %#p  %8s (%s) ut %ld st %ld bss %#p qpc "
	      "%#p nl %d nd %lud lpc %#p pri %lud\n",
	      p->pid, p->text, p->pc, dbgpc(p), s, statename[p->state],
	      p->time[0], p->time[1], bss, p->qpc, p->nlocks, p->delaysched,
	      p->lastlock ? p->lastlock->_pc : 0, p->priority);
}
...
killbig(char* why)
{
	...
	for(x = 0; (p = psincref(x)) != nil; x++) {
		...
		if(l > max &&
		   ((p->procmode & 0222) || strcmp(eve, p->user) != 0)) {
			if(kp != nil)
				psdecref(kp);
			kp = p;
			max = l;
	...
	for(x = 0; (p = psincref(x)) != nil; x++) {
		...
		/* TODO(aki): figure out what this was for. the oom killer is
...
accounttime(void)
{
	...
	machp()->perf.avg_inidle =
	    (machp()->perf.avg_inidle * (HZ - 1) + machp()->perf.inidle) / HZ;
	machp()->perf.inidle = 0;

	machp()->perf.avg_inintr =
	    (machp()->perf.avg_inintr * (HZ - 1) + machp()->perf.inintr) / HZ;
	machp()->perf.inintr = 0;
