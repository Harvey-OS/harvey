	...
	if(up) {
		...
		if(up->nlocks)
			if(up->state != Moribund)
				if(up->delaysched < 20 || pga.Lock.p == up || procalloc.Lock.p == up) {
					up->delaysched++;
					run.delayedscheds++;
					return;
				}
...
loop:
	...
	for(i = 0;; i++) {
		...
		for(rq = &run.runq[Nrq - 1]; rq >= run.runq; rq--) {
			for(p = rq->head; p; p = p->rnext) {
				if(p->mp == nil || p->mp == sys->machptr[machp()->machno] || (!p->wired && i > 0))
					goto found;
			}
		}
...
singlerunproc(void)
{
	...
	if((p = machp()->readied) && p->mach == 0 && p->state == Ready && &run.runq[Nrq - 1].head == nil && &run.runq[Nrq - 2].head == nil) {
		skipscheds++;
