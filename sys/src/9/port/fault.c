...
char* faulttypes[] = {[FT_WRITE] "write", [FT_READ] "read", [FT_EXEC] "exec"};
...
fail:
	if(s != nil) {
		...
		print("%s fault fail %s(%c%c%c) pid %d addr 0x%p pc 0x%p\n",
		      faulttypes[ftype], segtypes[s->type & SG_TYPE],
		      (s->type & SG_READ) != 0 ? 'r' : '-',
		      (s->type & SG_WRITE) != 0 ? 'w' : '-',
		      (s->type & SG_EXEC) != 0 ? 'x' : '-', up->pid, addr, pc);
	} else {
		print("%s fault fail, no segment, pid %d addr 0x%p pc 0x%p\n",
		      faulttypes[ftype], up->pid, addr, pc);
	}
	splhi();
	up->psstate = sps;
	return -1;
...
faulterror(char* s, Chan* c, int freemem)
{
	...
	if(c && c->path) {
		snprint(buf, sizeof buf, "%s accessing %s: %s", s, c->path->s,
		        up->errstr);
		s = buf;
	}
...
fixfault(Segment* s, uintptr_t addr, int ftype, int dommuput, int color)
{
	...
	common: /* Demand load/pagein/copy on write */
		...
		if(ftype != FT_WRITE) {
			...
			/* delay copy if we are the only user (copy on write
			 * when it happens) */
		...
		if((s->type & SG_TYPE) != SG_SHARED) {
			...
			if(ref >
			   1) { /* page is shared but segment is not: copy for
			           write */
				int pgref = lkp->ref;
				unlock(lkp);

				DBG("fixfault %d: copy on %s, %s(%c%c%c) 0x%p "
				    "segref %d pgref %d\n",
				    up->pid, faulttypes[ftype], segtypes[stype],
				    (s->type & SG_READ) != 0 ? 'r' : '-',
				    (s->type & SG_WRITE) != 0 ? 'w' : '-',
				    (s->type & SG_EXEC) != 0 ? 'x' : '-', addr,
				    s->ref, pgref);
...
pio(Segment* s, uintptr_t addr, uint32_t soff, Page** p, int color)
{
	...
	if(ask > doff) {
		...
		DBG("pio %d %s(%c%c%c) addr+doff 0x%p daddr+doff 0x%x ask-doff "
		    "%d\n",
		    up->pid, segtypes[s->type & SG_TYPE],
		    (s->type & SG_READ) != 0 ? 'r' : '-',
		    (s->type & SG_WRITE) != 0 ? 'w' : '-',
		    (s->type & SG_EXEC) != 0 ? 'x' : '-', addr + doff,
		    daddr + doff, ask - doff);
