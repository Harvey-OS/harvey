...
static char* tlsnames[] = {
        [Qclonus] "clone",  [Qencalgs] "encalgs", [Qhashalgs] "hashalgs",
        [Qdata] "data",     [Qctl] "ctl",         [Qhand] "hand",
        [Qstatus] "status", [Qstats] "stats",
};
...
tlsrecread(TlsRec* tr)
{
	...
	switch(type) {
	default:
		rcvError(tr, EIllegalParameter, "invalid record message %#x",
		         type);
		break;
...
static DigestState*
nomac(uint8_t* i, uint32_t j, uint8_t* k, uint32_t l, uint8_t* m,
      DigestState* ds)
{
	return nil;
}
