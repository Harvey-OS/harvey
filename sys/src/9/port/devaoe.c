		...
		for(; f < e; f++) {
			...
			if(a->scnt > Dbcnt / Aoesectsz &&
			   ++f->nl->lostjumbo > (d->nframes << 1)) {
				...
				eventlog(
				    "%æ: jumbo failure on %s:%E; lba%lld\n", d,
				    f->nl->path, ea, f->lba);
				d->maxbcnt = Dbcnt;
				d->flag &= ~Djumbo;
			}
...
aoecfg(void)
{
	...
	// if((p = getconf("aoeif")) == nil || (n = tokenize(p, f, nelem(f))) <
	// 1)
	return;
	/* goo! */
	for(i = 0; i < n; i++) {
...
pstat(Aoedev* d, char* db, int len, int off)
{
	...
	p = seprint(p, e, "state: %s\n"
	                  ...
	                  "serial: %s\n"
	                  "firmware: %s\n",
	            state, d->nopen, d->nout, d->maxout, d->nframes, d->maxbcnt,
	            d->fwver, d->model, d->serial, d->firmware);
	p = seprint(p, e, "flag: ");
	p = pflag(p, e, d->flag);
...
unitctlwrite(Aoedev* d, void* db, int32_t n)
{
	...
	case Setsize:
		...
		if(cb->nf == 2) {
			...
			if(bsize % Aoesectsz)
				cmderror(cb,
				         "disk size must be sector aligned");
		}
		d->bsize = bsize;
		break;
	default:
	...
	if(ahin->cmdstat & 0xa9) {
		...
		case Cid:
			if(BLEN(b) - AOEATASZ < 512) {
				eventlog(
				    "%æ: runt identify blen %ld expect %d\n", d,
				    BLEN(b), n);
				goto bail;
			}
...
Dev aoedevtab = {
    ...
    devreset,  devinit,   devshutdown, aoeattach, aoewalk,   aoestat,
    aoeopen,   devcreate, aoeclose,    aoeread,   devbread,  aoewrite,
