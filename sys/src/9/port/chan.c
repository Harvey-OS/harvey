...
undomount(Chan* c, Path* path)
{
	...
	if(path->ref != 1 || path->mlen == 0)
		print("undomount: path %s ref %d mlen %d caller %#p\n", path->s,
		      path->ref, path->mlen, getcallerpc(&c));
	...
	for(nhave = 0; nhave < nnames; nhave += n) {
		...
		if((wq = ewalk(c, nil, names + nhave, ntry)) == nil) {
			...
			if(mh && !nomount) {
				...
				rlock(&mh->lock);
				if(mh->mount) {
					for(f = mh->mount->next; f != nil;
					    f = f->next) {
						if((wq = ewalk(f->to, nil,
						               names + nhave,
						               ntry)) != nil) {
							dc = f->to->dev->dc;
							devno = f->to->devno;
	...
	if(waserror()) {
		...
		if(DBGFLG > 0) {
			DBG("showing %d+%d/%d (of %d) of %s (%d %d)\n",
			    e.prefix, e.off[e.nerror], e.nerror, e.nelems,
			    aname, e.off[0], e.off[1]);
		}
