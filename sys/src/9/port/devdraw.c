...
struct FChar {
	...
	unsigned char miny;  /* first non-zero scan-line */
	unsigned char maxy;  /* last non-zero scan-line + 1 */
...
static int
drawgen(Chan *c, char *cc, Dirtab *dt, int i, int s, Dir *dp)
{
	Proc *up = externup();
	int t;
	Qid q;
...
drawgoodname(DImage *d)
{
	...
	if(d->dscreen)
		if(drawgoodname(d->dscreen->dimage) == 0 || drawgoodname(d->dscreen->dfill) == 0)
			return 0;
	if(d->name == nil)
		return 1;
...
Memimage *
drawimage(Client *client, unsigned char *a)
{
	DImage *d;
...
void
drawrectangle(Rectangle *r, unsigned char *a)
{
...
void
drawpoint(Point *p, unsigned char *a)
{
	p->x = BGLONG(a + 0 * 4);
	p->y = BGLONG(a + 1 * 4);
}
...
static int
drawstat(Chan *c, unsigned char *db, int n)
{
...
drawwrite(Chan *c, void *a, i32 n, i64 nn)
{
	...
	case Qctl:
		...
		cl->infoid = BGLONG((unsigned char *)a);
		break;
...
unsigned char *
drawcoord(unsigned char *p, unsigned char *maxp, int oldx, int *newx)
{
	int b, x;
...
static void
printmesg(char *fmt, unsigned char *a, int plsprnt)
{
	char buf[256];
	char *p, *q;
	int left;
...
drawactive(int active)
{
	if(active){
		drawblankscreen(0);
		sdraw.blanktime = machp()->ticks;
	} else {
		if(blanktime && sdraw.blanktime && TK2SEC(machp()->ticks - sdraw.blanktime) / 60 >= blanktime)
			drawblankscreen(1);
	}
}
