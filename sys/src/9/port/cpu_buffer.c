                            ...
                            struct op_entry* entry, int size)
{
	...
	int totalsize =
	    sizeof(struct op_sample) + size * sizeof(entry->sample->data[0]);
...
static inline int
op_add_sample(struct oprofile_cpu_buffer* cpu_buf, unsigned long pc,
              unsigned long event)
{
	Proc* up = externup();
	// print_func_entry();
...
void
oprofile_add_ext_hw_sample(unsigned long pc, Ureg* regs, unsigned long event,
                           int is_kernel, Proc* proc)
{
	// print_func_entry();
...
void
oprofile_add_ext_sample(unsigned long pc, void /*struct pt_regs */* const regs,
...
void
oprofile_add_sample(void /*struct pt_regs */* const regs, unsigned long event)
{
	// print_func_entry();
	int is_kernel;
	unsigned long pc;
 ...
 */
void
oprofile_write_reserve(struct op_entry* entry, Ureg* regs, unsigned long pc,
                       int code, int size)
{
	Proc* up = externup();
	// print_func_entry();
...
/* return # bytes read, or 0 if profiling is off, or block if profiling on and
 * no data.
 */
int
