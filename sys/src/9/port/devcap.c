...
capwrite(Chan* c, void* va, int32_t n, int64_t m)
{
	...
	case Quse:
		...
		*key++ = 0;
		...
		p = remcap(hash);
		if(p == nil) {
			snprint(err, sizeof err, "invalid capability %s@%s",
			        from, key);
			error(err);
		}
...
Dev capdevtab = {L'¤',     "cap",
                 ...
                 devreset, devinit,  devshutdown, capattach, capwalk,
                 capstat,  capopen,  devcreate,   capclose,  capread,
                 devbread, capwrite, devbwrite,   capremove, devwstat};
