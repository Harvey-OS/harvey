...
static Dirtab topdir[] = {
    ".", {Qtopdir, 0, QTDIR}, 0, 0555, "pci", {Qpcidir, 0, QTDIR}, 0, 0555,
};
...
pciread(Chan* c, void* va, int32_t n, int64_t offset)
{
	...
	case Qpcictl:
		...
		w = seprint(buf, ebuf, "%.2x.%.2x.%.2x %.4x/%.4x %3d", p->ccrb,
		            p->ccru, p->ccrp, p->vid, p->did, p->intl);
...
Dev pcidevtab = {
    ...
    devreset, devinit,  devshutdown, pciattach, pciwalk,
