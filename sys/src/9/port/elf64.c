...
enum { EI_MAG0 = 0, // File identification
       ...
       EM_ARC_COMPACT =
	   93, //  ARC International ARCompact processor (old spelling/synonym:
       ...
       EM_TI_ARP32 =
	   143, //  Texas Instruments Application Specific RISC Processor, 32bit
       ...
       EM_STXP7X = 166, //  STMicroelectronics STxP7x family of configurable and
                        //  extensible RISC
       EM_NDS32 =
	   167, //  Andes Technology compact code size embedded RISC processor
...
static struct {
    ...
    {"amd64", EM_X86_64}, {"arm64", EM_AARCH64}, {"power64", EM_PPC64},
};
...
elf64ldseg(Chan* c, uintptr_t* entryp, Ldseg** rp, char* mach, uint32_t minpgsz)
{
	...
	fp = ehdr.e_ident;
	if(fp[EI_MAG0] == '\x7f' && fp[EI_MAG1] == 'E' && fp[EI_MAG2] == 'L' &&
	   fp[EI_MAG3] == 'F') {
		...
		if(fp[EI_CLASS] == ELFCLASS64) {
			...
			if(mach != nil) {
				for(i = 0; i < nelem(elfmachs); i++)
					if(elfmachs[i].e_machine == e_machine &&
					   !strcmp(mach, elfmachs[i].mach))
						break;
				if(i == nelem(elfmachs)) {
					print("elf64ldseg: e_machine %d "
					      "incorrect for host %s\n",
					      e_machine, mach);
					error(Ebadexec);
				}
			}
			...
			if(phentsize * phnum > minpgsz) {
				print("elf64ldseg: phentsize %d phnum %d "
				      "exceeds page size %d\n",
			...
			if(c->dev->read(c, phbuf, phentsize * phnum, phoff) !=
			   phentsize * phnum) {
			...
			for(fp = phbuf; fp < phend; fp += phentsize) {
				...
				phdr = (Elf64_Phdr*)fp;
				if(get32(phdr->p_type) == PT_LOAD) {
					uint64_t offset, vaddr, align, filesz,
					    memsz;
					uint32_t flags;

					flags = get32(
					    phdr->p_flags); /* Segment
					                       attributes */
					offset = get64(
					    phdr->p_offset); /* Offset in file
					                        */
					vaddr = get64(
					...
					filesz =
					    get64(
					        phdr->p_filesz); /* Size of
					                            segment in
					                            file */
					memsz = get64(
					    phdr->p_memsz); /* Size of segment
					                       in memory */
					align = get64(
					    phdr->p_align); /* Alignment of
					                       segment */
					...
					if(offset & (align - 1) != vaddr &
					   (align - 1)) {
						print("elf64ldseg: va offset "
						      "0x%x != file offset "
						      "0x%x (align 0x%x)\n",
						      offset & (align - 1),
						      vaddr & (align - 1),
