...
sched(void)
{
	...
	if(m->ilockdepth)
		panic("cpu%d: ilockdepth %d, last lock %#p at %#p, sched "
		      "called from %#p",
		      machp()->machno, m->ilockdepth, up ? up->lastilock : nil,
...
accounttime(void)
{
	...
	m->perf.avg_inidle =
	    (m->perf.avg_inidle * (HZ - 1) + m->perf.inidle) / HZ;
	m->perf.inidle = 0;

	m->perf.avg_inintr =
	    (m->perf.avg_inintr * (HZ - 1) + m->perf.inintr) / HZ;
	m->perf.inintr = 0;
