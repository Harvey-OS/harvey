...
lock(Lock* l)
{
	...
	for(;;) {
		...
		i = 0;
		while(l->key) {
			if(sys->nmach < 2 && up && up->edf &&
			   (up->edf->flags & Admitted)) {
				/*
				 * Priority inversion, yield on a uniprocessor;
				 * on a
				 * multiprocessor, the other processor will
				 * unlock
				 */
				print("inversion %#p pc %#p proc %d held by pc "
				      "%#p proc %d\n",
				      l, pc, up ? up->pid : 0, l->_pc,
				      l->p ? l->p->pid : 0);
				up->edf->d = todget(
				    nil); /* yield to process with lock */
			}
			if(i++ > 100000000) {
				i = 0;
				lockloop(l, pc);
...
unlock(Lock* l)
{
	...
	if(l->isilock)
		print("unlock of ilock: pc %#p, held by %#p\n", getcallerpc(&l),
		      l->_pc);
	if(l->p != up)
		print("unlock: up changed: pc %#p, acquired at pc %#p, lock p "
		      "%#p, unlock up %#p\n",
	...
	if(up && adec(&up->nlocks) == 0 && up->delaysched && islo()) {
		 ...
		 * But, don't do it from interrupt routines, hence the islo()
		 * test
		 */
		sched();
	}
...
iunlock(Lock* l)
{
	...
	if(l->m != machp()) {
		print(
		    "iunlock by cpu%d, locked by cpu%d: pc %#p, held by %#p\n",
