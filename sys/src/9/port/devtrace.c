...
static const int PIDWATCHSIZE =
    32; /* The number of PIDS that can be watched. Pretty arbitrary. */
...
traceread(Chan* c, void* a, int32_t n, int64_t offset)
{
	...
	if(c->qid.type == QTDIR) {
		int32_t l =
		    devdirread(c, a, n, tracedir, nelem(tracedir), devgen);
		poperror();
		traceactive = saveactive;
		return l;
	}
	...
	case Qdata:
		...
		// Set the printsize
		/* 32-bit E PCPCPCPC TIMETIMETIMETIME PID# CR XXARG1XX XXARG2XX
		 * XXARG3XX XXARG4XX\n */
...
tracewrite(Chan* c, void* a, int32_t n, int64_t mm)
{
	...
	case Qctl:
		...
		if(!strcmp(tok[0], "trace")) { /* 'trace' ktextaddr
		                                  'on'|'off'|'mk'|'del' [name]
		                                  */
			if(ntok < 3) {
				error("devtrace: usage: 'trace' "
			...
			if((ntok > 3) && (!strcmp(tok[3], "new"))) {
				...
				if(*ep) {
					error("devtrace: start address not in "
					      "recognized format");
				}
				...
				if(*ep) {
					error("devtrace: end address not in "
					      "recognized format");
				}

				if(start > end || start > etext || end > etext)
					error(
					    "devtrace: invalid address range");

				/* What do we do here? start and end are weird *
				if((addr < (uintptr)start) || (addr >
				(uintptr)end)
				        error("devtrace: address out of
				bounds");
				 */
				if(p) {
				...
				if(!overlapping(p)) {
					p->next = traces;
					if(ntok < 5)
						snprint(p->name, sizeof p->name,
						        "%p", func);
					else
						strncpy(p->name, tok[4],
						        sizeof p->name);
					traces = p;
				} else {
					error("devtrace: given range overlaps "
					      "with existing trace");
				}
			} else if(!strcmp(tok[2], "remove")) {
				if(ntok != 3)
					error("devtrace: usage: trace <name> "
					      "remove");
				if(p == nil) {
			...
			if(*ep) {
				error(
				    "devtrace: size not in recognized format");
			}
			size = 1 << l;
			/* sort of foolish. Alloc new trace first, then free
			 * old. */
			...
			if(ntok != 6)
				error("devtrace: usage: testtracein <pc> "
				      "<arg1> <arg2> <arg3> <arg4>");
			...
			if(pid == 0) {
				pidwatch =
				    mallocz(sizeof(int) * PIDWATCHSIZE, 1);
				numpids = 0;
			} else if(pid < 0) {
...
Dev tracedevtab = {
    'T',       "trace",    devreset,  devinit,   devshutdown, traceattach,
    tracewalk, tracestat,  traceopen, devcreate, traceclose,  traceread,
    devbread,  tracewrite, devbwrite, devremove, devwstat,
};
