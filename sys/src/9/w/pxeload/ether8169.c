...
rtl8169init(Ether* edev)
{
	...
	csr16w(ctlr, Imr,
	       Serr | Timeout | Fovw | Punlc | Rdu | Ter | Rer | Rok);
...
rtl8169transmit(Ether* edev)
{
	...
	while(ctlr->ntq < (ctlr->ntd - 1)) {
		...
		coherence();
		d->control |=
		    Own | Fs | Ls | ((BLEN(bp) << TxflSHIFT) & TxflMASK);
...
rtl8169interrupt(Ureg*, void* arg)
{
	...
	while((isr = csr16r(ctlr, Isr)) != 0 && isr != 0xFFFF) {
		 ...
		 */
		if(isr & (Serr | Timeout | Tdu | Fovw | Punlc | Rdu | Ter |
		          Tok | Rer | Rok))
