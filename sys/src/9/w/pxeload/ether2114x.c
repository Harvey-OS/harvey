			...
			while((des->status & Own) == 0) {
				...
				if(des->status & Es) {
					...
					if(ring->owner == Interface) {
						ring->owner = Host;
						ring->len = len;
						memmove(ring->pkt, des->bp,
						        len);
						ether->ri =
						    NEXT(ether->ri, ether->nrb);
					}
				}
...
ctlrinit(Ether* ether)
{
	 ...
	 */
	ctlr->mask =
	    Nis | Ais | Fbe | Rwt | Rps | Ru | Ri | Unf | Tjt | Tps | Ti;
...
miiw(Ctlr* ctlr, int phyad, int regad, int data)
{
	...
	miimdo(ctlr, 0xFFFFFFFF, 32);
	data &= 0xFFFF;
	data |= (0x05 << (5 + 5 + 2 + 16)) | (phyad << (5 + 2 + 16)) |
	        (regad << (2 + 16)) | (0x02 << 16);
	miimdo(ctlr, data, 32);
	csr9w(ctlr, Mdc);
	csr9w(ctlr, 0);
...
typephylink(Ctlr* ctlr, uint8_t*)
{
	...
	if(bmcr & 0x1000) {
		...
		debug("an 0x%2.uX 0x%2.2uX 0x%2.2uX\n",
		      ...
		      miir(ctlr, ctlr->curphyad, Anlpar), an);
...
typesymmode(Ctlr* ctlr, uint8_t* block, int wait)
{
	...
	if(command & 0x8000) {
		print("ether2114x.c: FIXME: handle type 4 mode blocks where "
		      "cmd.active_invalid != 0\n");
		return -1;
	}
...
type0mode(Ctlr* ctlr, uint8_t* block, int wait)
{
	...
	debug(
	    "type0: medium 0x%uX, fd %d: 0x%2.2uX 0x%2.2uX 0x%2.2uX 0x%2.2uX\n",
...
media21041(Ether* ether, int wait)
{
	...
	debug(
	    "media21041: csr6 %8.8uX csr13 %4.4uX csr14 %4.4uX csr15 %4.4uX\n",
	...
	if(block[0] & 0x80) {
		...
		case 0:
			if(ctlr->medium >= 0 && block[2] != ctlr->medium)
				return 0;
			/* need this test? */ if(ctlr->sct != 0x0800 &&
			                         (ctlr->sct & 0x3F) != block[2])
				return 0;
...
static uint8_t leafpnic[] = {
    ...
    0x00, 0x00, 0x00,       /* controller 0 device number */
...
srom(Ctlr* ctlr)
{
	...
	debug("sct 0x%uX medium 0x%uX k %d curk %d phy %d\n", ctlr->sct,
	      ctlr->medium, ctlr->k, ctlr->curk, phy);
