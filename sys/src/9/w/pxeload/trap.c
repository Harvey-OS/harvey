...
#define DATASEGM(p)                                                            \
	{                                                                      \
		0xFFFF, SEGG | SEGB | (0xF << 16) | SEGP | SEGPL(p) |          \
		            SEGDATA | SEGW                                     \
	}
#define EXECSEGM(p)                                                            \
	{                                                                      \
		0xFFFF, SEGG | SEGD | (0xF << 16) | SEGP | SEGPL(p) |          \
		            SEGEXEC | SEGR                                     \
	}
 ...
 */
enum { Int0ctl = 0x20, /* control port (ICW1, OCW2, OCW3) */
...
enum { Maxhandler = 32, /* max number of interrupt handlers */
};
...
dumpregs(Ureg* ur)
{
	...
	print("  CS %4.4lux DS %4.4lux  ES %4.4lux  FS %4.4lux  GS %4.4lux\n",
	      ur->cs & 0xFF, ur->ds & 0xFFFF, ur->es & 0xFFFF, ur->fs & 0xFFFF,
	      ur->gs & 0xFFFF);
...
trap(Ureg* ur)
{
	...
	if(v >= 256 || (h = halloc.ivec[v]) == 0) {
		...
		case 0x02: /* NMI */
			print("NMI: nmisc=0x%2.2ux, nmiertc=0x%2.2ux, "
			      "nmiesc=0x%2.2ux\n",
