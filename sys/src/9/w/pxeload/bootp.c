...
tftpopen(int ctlrno, Netaddr* a, char* name, Tftp* tftp)
{
	...
	for(i = 0; i < 5; i++) {
		...
		a->port = 0;
		if((rlen = udprecv(ctlrno, a, tftp, sizeof(Tftp))) <
		   sizeof(tftp->header))
			continue;
...
bootpopen(int ctlrno, char* file, Bootp* brep, int dotftpopen)
{
	...
	if(rep->sname[0] != '\0')
		print("%s ", rep->sname);
	print("(%d.%d.%d.%d!%d): %s\n", rep->siaddr[0], rep->siaddr[1],
	      rep->siaddr[2], rep->siaddr[3], server.port, filename);
...
bootpboot(int ctlrno, char* file, Boot* b)
{
	...
	if(0 < n && n < sizeof(tftpb.data)) /* got to end of file */
		...
		nak(ctlrno, &server, 3, "ok",
		    0);      /* tftpclose to abort transfer */
...
pxeread(File* f, void* va, int32_t len)
{
	...
	while(n > 0) {
		...
		if(n != TFTP_SEGSIZE)
			break;
		if((n = tftpread(f->fs->dev, &server, &tftpb,
		                 sizeof(tftpb.data))) < 0)
			return -1;
	}
	return p - v;
...
interesting(void* data)
{
	...
	if((h->vihl & 0xf0) != IP_VER) {
		if(debug)
			print("not ipv4 (%#4.4ux) %#2.2ux...", nhgets(h->type),
			      h->vihl);
		return 0;
	}
	if((h->vihl & 0x0f) != IP_HLEN) {
