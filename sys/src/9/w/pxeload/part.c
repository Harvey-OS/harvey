...
oldp9part(SDunit* unit)
{
	...
	if(strncmp((char*)partbuf, MAGIC, sizeof(MAGIC) - 1) != 0) {
		...
		print("%s: using old plan9 partition table on 2nd-to-last "
		      "sector\n",
		      unit->name);
...
isdos(int t)
{
	return t == FAT12 || t == FAT16 || t == FATHUGE || t == FAT32 ||
	       t == FAT32X;
}
...
mbrpart(SDunit* unit)
{
	...
	if(1) {
		...
		if(tsdbio(unit, &unit->part[0], mbrbuf,
		          (int64_t)taboffset * unit->secsize, 1) < 0)
			return -1;
		for(i = 0; i < 4; i++)
			if(dp[i].type == DMDDO) {
				if(trace)
				...
				taboffset = 63;
				if(tsdbio(unit, &unit->part[0], mbrbuf,
				          (int64_t)taboffset * unit->secsize,
				          1) < 0)
					return -1;
				i = -1; /* start over */
			}
	...
	for(;;) {
		...
		for(i = 0; i < 4; i++) {
			...
			/*
			 * We used to take the active partition (and then the
			 * first
			 * when none are active).  We have to take the first
			 * here,
			...
			/* nxtxpart is relative to firstxpart (or 0), not
			 * taboffset */
...
part9660(SDunit* unit)
{
	...
	if(memcmp(buf, "\x01\x00\x00\x00", 4) != 0 ||
	   memcmp(buf + 30, "\x55\xAA", 2) != 0 || buf[0x20] != 0x88)
		return -1;
