...
#define OP(o, m, ro, rm)                                                       \
	BYTE $o; /* op + modr/m byte */                                        \
	BYTE $(((m) << 6) | ((ro) << 3) | (rm))
#define OPrm(o, r, m)                                                          \
	OP(o, 0x00, r, 0x06); /* general r <-> m */                            \
	WORD $m;
#define OPrr(o, r0, r1) OP(o, 0x03, r0, r1); /* general r -> r */

#define LW(m, rX) OPrm(0x8B, rX, m) /* m -> rX */
#define LXW(x, rI, r)                                                          \
	OP(0x8B, 0x02, r, rI); /* x(rI) -> r */                                \
	WORD $x
#define LBPW(x, r)                                                             \
	OP(0x8B, 0x02, r, xBP); /* x(rBP) -> r */                              \
	WORD $x
#define LB(m, rB) OPrm(0x8A, rB, m) /* m -> r[HL] */
#define LXB(x, rI, r)                                                          \
	OP(0x8A, 0x01, r, rI); /* x(rI) -> r */                                \
	BYTE $x
#define LBPB(x, r)                                                             \
	OP(0x8A, 0x01, r, xBP); /* x(rBP) -> r */                              \
	BYTE $x
#define SW(rX, m) OPrm(0x89, rX, m) /* rX -> m */
#define SXW(r, x, rI)                                                          \
	OP(0x89, 0x02, r, rI); /* r -> x(rI) */                                \
	WORD $x
#define SBPW(r, x)                                                             \
	OP(0x89, 0x02, r, xBP); /* r -> x(rBP) */                              \
	WORD $(x)
#define SBPWI(i, x)                                                            \
	OP(0xC7, 0x01, 0, xBP); /* i -> x(rBP) */                              \
	BYTE $(x);                                                             \
	WORD $(i)
#define STB(rB, m) OPrm(0x88, rB, m) /* rB -> m */
#define SXB(r, x, rI)                                                          \
	OP(0x88, 0x01, r, rI); /* rB -> x(rI) */                               \
	BYTE $x
#define SBPB(r, x)                                                             \
	OP(0x88, 0x01, r, xBP); /* r -> x(rBP) */                              \
	BYTE $x
#define SBPBI(i, x)                                                            \
	OP(0xC6, 0x01, 0, xBP); /* i -> x(rBP) */                              \
	BYTE $(x);                                                             \
	BYTE $(i)
#define LWI(i, rX)                                                             \
	BYTE $(0xB8 + rX); /* i -> rX */                                       \
...
#define ORI(i, r)                                                              \
	OP(0x81, 0x03, 0x01, r); /* i|r -> r */                                \
	WORD $i;
#define ROLI(i, r)                                                             \
	OPrr(0xC1, 0x00, r); /* r<<>>i -> r */                                 \
	BYTE $i;
#define SHLI(i, r)                                                             \
	OPrr(0xC1, 0x04, r); /* r<<i -> r */                                   \
	BYTE $i;
#define SHLBI(i, r)                                                            \
	OPrr(0xC0, 0x04, r); /* r<<i -> r */                                   \
	BYTE $i;
#define SHRI(i, r)                                                             \
	OPrr(0xC1, 0x05, r); /* r>>i -> r */                                   \
	BYTE $i;
#define SHRBI(i, r)                                                            \
	OPrr(0xC0, 0x05, r); /* r>>i -> r */                                   \
	BYTE $i;
#define SUB(r0, r1) OPrr(0x29, r0, r1) /* r1-r0 -> r1 */
#define SUBI(i, r)                                                             \
	OP(0x81, 0x03, 0x05, r); /* r-i -> r */                                \
	WORD $i;
...
#define CALL16(f)                                                              \
	...
	BYTE $0xFF;  /* (*rDI) */                                              \
	BYTE $0xD7;
#define FARJUMP16(s, o)                                                        \
	BYTE $0xEA; /* jump to ptr16:16 */                                     \
	WORD $o;                                                               \
	WORD $s
#define FARJUMP32(s, o)                                                        \
	BYTE $0x66; /* jump to ptr32:16 */                                     \
	BYTE $0xEA;                                                            \
	LONG $o;                                                               \
	WORD $s
#define DELAY                                                                  \
...
#define PEEKW                                                                  \
	BYTE $0x26; /* MOVW	rES:[rBX], rAX  */                                 \
	BYTE $0x8B;                                                            \
	BYTE $0x07
#define POKEW                                                                  \
	BYTE $0x26; /* MOVW	rAX, rES:[rBX] */                                  \
	BYTE $0x89;                                                            \
	BYTE $0x07
#define OUTPORTB(p, d)                                                         \
	LBI(d, rAL); /* d -> I/O port p */                                     \
	BYTE $0xE6;                                                            \
	BYTE $p;                                                               \
	DELAY
