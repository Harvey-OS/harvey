...
enum { Timeout = 50000, // 10000000, //
       ...
       RxConfig_VlanOpt1 =
	   (1 << 1), /* - TX: no tag insert, RX: tagged pkts, no extr */
       RxConfig_VlanOpt2 =
	   (2 << 1), /* - TX: tag insert, RX: all, extract tags */
       RxConfig_VlanOpt3 =
	   (3 << 1), /* - TX: tag insert, RX: tagged pkts, with extr */
       ...
       isr_Mask = Ptx0 | Prx | Shdn | Ovf | Phyint | Src | Lste | Rxstl | Txstl
};
...
enum { Vidm = (1 << 0),  /* VLAN tag filter miss */
...
enum { Rblen64K = 0x00001800, /* 64KB+16 */
...
static char* vgbeisr_info[] = {
    "hi prio Rx int", "hi prio Tx int", "Rx queue completed",
    "One of Tx queues completed", "Tx queue 0 completed",
    "Tx queue 1 completed", "Tx queue 2 completed", "Tx queue 3 completed",
    "reserved", "reserved", "Rx packet count overflow", "pause frame Rx'ed",
    "RX FIFO overflow", "ran out of Rx descriptors",
    "running out of Rx descriptors", "link status change",
    "one shot timer expired", "periodic timer expired", "wake up power event",
    "PHY interrupt", "software shutxbufown complete",
    "MIB counter overflow warning", "software interrupt", "reload hold timer",
    "Rx DMA stall", "Tx DMA stall", "reserved", "reserved",
    "interrupt source indication 0", "interrupt source indication 1",
    "interrupt source indication 2", "interrupt source indication 3",
};
...
vgberxeof(Ether* edev)
{
	...
	for(i = 0; i < RxCount; i++) {
		...
		if(status & Rxok) {
			...
			if(ctlr->debugflags & DumpRx)
				print("vgbe: Rx-rd[%03d] status=%#08ulx "
				      "ctl=%#08ulx len=%uld bytes\n",
				      i, status, rd->control, length);

			rb = ctlr->rxbuf[i];
			ctlr->stats.rx++;
			if(ctlr->debugflags & DumpRx) {
				print("&edev->rb[edev->ri] %ulx rb %ulx length "
				      "%uld\n",
			...
			print("vgbe: Rx-rd[%#02x] *BAD FRAME* status=%#08ulx "
			      "ctl=%#08ulx\n",
			      i, status, rd->control);
...
vgbetransmit(Ether* edev)
{
	...
	for(count = 0, i = 0; i < TxCount; i++) {
		...
		desc->control = htole32(Td_Control_Intr | Td_Control_Normal |
		                        ((1 + 1) << 28));
...
{
	/* now we need to do something special to empty the ring buffer since we
