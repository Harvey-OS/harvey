...
setvector(Vector *v)
{
	...
	vector[nvector++] = v;
	irq->mask0 |= v->mask & 0xFF;
	irq->mask1 |= (v->mask>>8) & 0xFF;
}
...
interrupt(Ureg *ur)
{
	...
	for(;;) {
		mask = (irq->mask0 & irq->status0) & 0xFF;
		mask |= ((irq->mask1 & irq->status1) & 0xFF)<<8;
		if(mask == 0)
			break;
		for(v = vector; *v; v++){
			if((*v)->mask & mask) {
	...
	case 0x01:				/* exception */
		...
		case EXC_FETCH:			/* fetch fault */
			...
			if (ur->pc == (ulong)_cret)
				ur->pc = *(unsigned *)(USERADDR+SPOFFSET);
			addr = ur->pc;
			/*
