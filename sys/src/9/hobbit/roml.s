#define rom					/* STUPID FUCKING COMPILER SYSTEM */
#include "mem.h"
#include "machine.h"

#define PHYSADDR(v)	(((v)-KZERO-0x400000)+RAMBASE)
#define VIRTADDR(p)	(((p)-RAMBASE)+KZERO+0x400000)
...
TEXT _startup(SB), $0
	...
	MOV	%MSP, *$PPAGE0(0x040)
	MOV	%ISP, *$PPAGE0(0x044)
	MOV	%SP, *$PPAGE0(0x048)

	MOV	%SHAD, *$PPAGE0(0x054)
	MOV	%VB, *$PPAGE0(0x058)
	MOV	%STB, *$PPAGE0(0x05C)
	MOV	%FAULT, *$PPAGE0(0x060)
	MOV	%ID, *$PPAGE0(0x064)

	MOV	$fprt0(SB), *$PPAGE0(0x74)
...
l0:
	...
	ADD3	$(SEGNUM(KZERO)<<2), R0			/* ROM STE */
	MOV	$NPSTE(PROMBASE, 64, STE_V), *R4

	ADD3	$(SEGNUM(KDATABASE)<<2), R0		/* data copied to 2Mb */
	MOV	$NPSTE(RAMBASE, 1024, STE_C|STE_U|STE_W|STE_V), *R4

	ADD3	$(SEGNUM(IOBASE)<<2), R0		/* I/O ACK STE */
	MOV	$NPSTE(IOBASE, 4, STE_U|STE_W|STE_V), *R4

	ADD3	$(SEGNUM(IOBASE+0x08000000)<<2), R0	/* I/O NACK STE */
	MOV	$NPSTE(IOBASE+0x08000000, 4, STE_U|STE_W|STE_V), *R4
...
TEXT _virtual(SB), $16
	...
	MOV	$etext(SB), R8
	MOV	$(KDATABASE+0x4000), R4		/* bdata */
_copy:
	CMPHI	R4, $edata(SB)			/* copy ROM -> RAM */
	JMPT	_vbinit
	MOV	*R8.UH, *R4.UH
	ADD	$2, R8
	ADD	$2, R4
...
_vbinit:
	...
	ADD	$4, R4
	MOV	$fprt0(SB), *R4			/* niladic unimp */
	ADD	$4, R4
	MOV	$fprt0(SB), *R4			/* unimplemented  */
	ADD	$4, R4
...
TEXT softreset(SB), $0
_softreset:
	MOV	%PSW, *$VIRTADDR(RAMBASE+0x0124)
	MOV	%CONFIG, *$VIRTADDR(RAMBASE+0x0128)
	MOV	R0, *$VIRTADDR(RAMBASE+0x012C)
