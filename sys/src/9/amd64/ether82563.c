...
i82563transmit(Ether *edev)
{
	...
	if(!edev->Netif.link && ctlr->type == i218 && !ctlr->didk1fix){
		qunlock(&ctlr->tlock);
		return;
	}
...
phywrite(Ctlr *ctlr, int reg, u16 val)
{
	...
	if(reg >= 32)
		iprint("phyread: reg %d >= 32\n", reg);
	csr32w(ctlr, Mdic, MDIwop | phynum(ctlr) << MDIpSHIFT | reg << MDIrSHIFT | val);
	phy = 0;
	for(i = 0; i < 64; i++){
		phy = csr32r(ctlr, Mdic);
		if(phy & (MDIe | MDIready))
...
k1fix(Ctlr *ctlr)
{
	...
	if(edev->Netif.link && status & (Sspeed1000 >> 2)){
	...
	phywrite(ctlr, I217inbandctlreg, reg | txtmout << I217inbandctllnkststxtmoutshift);
	csr32w(ctlr, Fextnvm6, fextnvm6);
...
i82563attach(Ether *edev)
{
	...
	for(i = 0; i < Nrb; i++){
		if((bp = allocb(ETHERMAXTU + Slop + PGSZ)) == nil)
			error(Enomem);
...
static void
i82575interrupt(Ureg *_1, void *_2)
{
	Ctlr *ctlr;
...
pnp(Ether *edev, int type)
{
	...
	edev->interrupt = (ctlr->type == i82575 ? i82575interrupt : i82563interrupt);
