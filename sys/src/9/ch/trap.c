...
ptlb(ulong phys)
{
	...
	k = (k+1)&3;
	p = buf[k];
	p += sprint(p, "(0x%lux %lud ", (phys<<6) & ~(BY2PG-1), (phys>>3)&7);
	if(phys & 4)
		*p++ = 'd';
	if(phys & 2)
		*p++ = 'v';
...
kpteprint(Ureg *ur)
{
	...
	print("i=%lud v=0x%lux p0=%s p1=%s\n",
		i, tlbstuff[0], ptlb(tlbstuff[1]), ptlb(tlbstuff[2]));

	i = (ur->badvaddr & ~KMAPADDR)>>15;
	if(i > KPTESIZE){
		print("kpte index = 0x%lud ?\n", i);
		return;
	}
	k = &kpte[i];
	print("i=%lud, &k=0x%lux, k={v=0x%lux, p0=%s, p1=%s, pg=0x%lux}\n",
...
void (*handler[128])(void);

/*
 *  set handlers for things at INTR0
 */
void
sethandler(int level, void (*h)(void))
{
	if(level >= 128)
		panic("bad handler %d %lux", level, h);
	if(handler[level])
		print("sethandler: replaced handler at %d\n", level);
	handler[level] = h;
}
...
intr(Ureg *ur)
{
	...
	if(cause & INTR3){
		x = 2;
		uvmove(&x, &CPUREGS->cipl124);	/* reset interrupt on IP19 */
		cause &= ~INTR3;
	}
	if(cause & (INTR2|INTR1)){
		for(;;){
			uvmove(&(CPUREGS->hpil), &hpil);
			uvmove(&hpil, &CPUREGS->cipl0);
			hpillo = hpil;
			if(hpillo == 0)
				break;
			h = handler[hpillo];
			if(h)
				...
				iprint("hpil.lo %d with no handler\n", hpillo);
		}
		cause &= ~(INTR2|INTR1);
	}
	if(cause & INTR7){
