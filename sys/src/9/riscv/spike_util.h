...
usize translate_address(usize vAddr);
usize mcall_query_memory(usize id, memory_block_info *p);
usize mcall_hart_id(void);
usize htif_interrupt(usize mcause, usize *regs);
usize mcall_console_putchar(u8 ch);
void putchar(void);
usize mcall_dev_req(sbi_device_message *m);
usize mcall_dev_resp(void);
usize mcall_set_timer(unsigned long long when);
usize mcall_clear_ipi(void);
usize mcall_send_ipi(usize recipient);
usize mcall_shutdown(void);
