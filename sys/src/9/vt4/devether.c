...
//				intrenable(ether->irq, ether->interrupt, ether,
//					ether->tbdf, name);
				/* done by the driver */
//				intrenable(Inttemac, ether->interrupt, "ether");
			...
			if(ether->mbps == 100){
				/*
				 * our ether has no ring buffers, so make sure
				 * that we have adequate buffering.
				 */
				netifinit(ether, name, Ntypes, 1024*1024);
				if(ether->oq == 0)
					ether->oq = qopen(1024*1024, Qmsg, 0, 0);
			}
			else{
...
enum {
	Pktwin = 60, /* must be ether activity within this interval, in sec.s */
};
...
etherclock(void)
{
	Ether *ether = etherxx[0];
	static int cnt, oldin, oldout;

	if (++cnt < Pktwin)
		return;
	cnt = 0;
	if ((ether->inpackets == oldin || ether->outpackets == oldout) &&
	    ether->inpackets && ether->outpackets) {
		print("no ether pkts in last %d s.  ", Pktwin);
		/*
		 * this is really unlikely, so probably the ethernet controller
		 * has silently wedged.  whack it hard.
		 */
		whackether(ether);
	}
	oldin =  ether->inpackets;
	oldout = ether->outpackets;
}
