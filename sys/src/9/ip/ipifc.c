...
enum { Maxmedia = 32,
       ...
       QMAX = 192 * 1024 - 1,
       Maxv6repr =
	   (128 / (4 * 4)) * (4 + 1), /* limit of xxxx:xxxx:⋯ notation */
};
...
char sfixedformat[] = "device %s maxtu %d sendra %d recvra %d mflag %d oflag"
                      " %d maxraint %d minraint %d linkmtu %d reachtime %d "
                      "rxmitra %d ttl %d routerlt"
...
ipifcstate(Conv* c, char* state, int n)
{
	...
	ifc = (Ipifc*)c->ptcl;
	m = snprint(state, n, sfixedformat, ifc->dev, ifc->maxtu, ifc->sendra6,
...
ipifcadd(Ipifc* ifc, char** argv, int argc, int tentative, Iplifc* lifcp)
{
	...
	*l = lifc;
	...
	/* add local routes */
	if(isv4(ip))
		v4addroute(f, tifc, rem + IPv4off, mask + IPv4off,
		           rem + IPv4off, type);
	else
...
ipselftabread(Fs* f, char* cp, uint32_t offset, int n)
{
	...
	for(i = 0; i < NHASH && m < n; i++) {
		for(p = f->self->hash[i]; p != nil && m < n; p = p->next) {
			...
			m +=
			    snprint(cp + m, n - m, stformat, p->a, nifc, state);
			if(off > 0) {
				off -= m;
				m = 0;
			}
