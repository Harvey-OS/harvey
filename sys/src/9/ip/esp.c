...
static Algorithm espalg[] = {
    "null", 0, nullespinit, "des3_cbc", 192,
...
static Algorithm ahalg[] = {
    "null", 0, nullahinit, "hmac_sha1_96", 128, shaahinit, /* rfc2404 */
...
espkick(void* x)
{
	...
	/* compute secure hash */
	ecb->auth(ecb, bp->rp + vers.iphdrlen,
	          (vers.hdrlen - vers.iphdrlen) + payload + pad + Esptaillen,
	          auth);
...
espiput(Proto* esp, Ipifc* ipifc, Block* bp)
{
	...
	if(payload <= 0 || payload % 4 != 0 || payload % ecb->espblklen != 0) {
		qunlock(c);
		netlog(f, Logesp,
		       "esp: bad length %I -> %I!%lud payload=%d BLEN=%lud\n",
	...
	if(payload <= 0) {
		qunlock(c);
		netlog(f, Logesp,
		       "esp: short packet after decrypt %I -> %I!%lud\n",
		       vers.raddr, vers.laddr, vers.spi);
		freeb(bp);
		return;
	}
...
static void
seanq_hmac_sha1(uint8_t hash[SHA1dlen], uint8_t* t, int32_t tlen, uint8_t* key,
                int32_t klen)
{
	int i;
...
aesahauth(Espcb* ecb, uint8_t* t, int tlen, uint8_t* auth)
{
	...
	ecb->ds = hmac_aes(t, tlen, (uint8_t*)ecb->ahstate, BITS2BYTES(96),
	                   hash, ecb->ds);
	r = memcmp(auth, hash, ecb->ahlen) == 0;
	memmove(auth, hash, ecb->ahlen);
	return r;
...
static int aescbccipher(Espcb* ecb, uint8_t* p, int n) /* 128-bit blocks */
{
...
static int aesctrcipher(Espcb* ecb, uint8_t* p, int n) /* 128-bit blocks */
{
	uint8_t tmp[AESbsize], q[AESbsize];
	uint8_t* pp, *tp, *ip, *eip, *ep;
	AESstate* ds = ecb->espstate;
...
static void
seanq_hmac_md5(uint8_t hash[MD5dlen], uint8_t* t, int32_t tlen, uint8_t* key,
               int32_t klen)
{
	int i;
