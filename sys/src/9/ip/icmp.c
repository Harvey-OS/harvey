...
enum { MinAdvise = 24, /* minimum needed for us to advise another protocol */
};

char* icmpnames[Maxtype + 1] = {
        [EchoReply] "EchoReply", [Unreachable] "Unreachable",
...
static char* statnames[Nstats] = {
        [InMsgs] "InMsgs",     [InErrors] "InErrors", [OutMsgs] "OutMsgs",
        [CsumErrs] "CsumErrs", [LenErrs] "LenErrs",   [HlenErrs] "HlenErrs",
};
...
icmpttlexceeded(Fs* f, uint8_t* ia, Block* bp)
{
	...
	hnputs(np->cksum,
	       ptclcsum(nbp, ICMP_IPSIZE, blocklen(nbp) - ICMP_IPSIZE));
...
icmpiput(Proto* icmp, Ipifc* ipifc, Block* bp)
{
	...
	netlog(icmp->f, Logicmp, "icmpiput %s (%d) %d\n",
	       (p->type < nelem(icmpnames) ? icmpnames[p->type] : ""), p->type,
	       p->code);
	n = blocklen(bp);
