...
enum { Resetdelay = 100,  /* delay after a controller reset (ms) */
...
#define ddiprint                                                               \
	...
	print
#define dqprint                                                                \
	if(debug || (qh->io && qh->io->debug))                                 \
	print
#define ddqprint                                                               \
	if(debug > 1 || (qh->io && qh->io->debug > 1))                         \
	print
...
seprinttd(char* s, char* se, Td* td)
{
	...
	s = seprint(s, se, " ndata %uld sbuff %#p buff %#p", td->ndata,
	            td->sbuff, td->buff);
	if(td->ndata > 0)
...
isodump(Isoio* iso, int all)
{
	...
	print("iso %#p %s state %d nframes %d"
	      ...
	      iso, iso->tok == Tdtokin ? "in" : "out", iso->state, iso->nframes,
	      iso->tdps[iso->td0frno], iso->tdu, iso->tdi, iso->data);
	if(iso->err != nil)
		print("\terr='%s'\n", iso->err);
	if(all == 0) {
...
dumptd(Td* td, char* pref)
{
	...
	for(; td != nil; td = td->next) {
		...
		if(!sameptr(td->next, td->link))
			seprint(s, se, " next %#p != link %#ulx %#p", td->next,
			        td->link, TPTR(td->link));
		print("%std %s\n", pref, buf);
		if(i++ > 20) {
			print("...more tds...\n");
			break;
...
qhdump(Qh* qh, char* pref)
{
	...
	s = buf;
	se = buf + sizeof(buf);
	s = seprint(s, se, "%sqh %s %#p state %s link %#ulx", pref, qh->tag, qh,
	            qhsname[qh->state], qh->link);
	if(!sameptr(qh->tds, qh->elink))
		s = seprint(s, se, " [tds %#p != elink %#ulx %#p]", qh->tds,
		            qh->elink, TPTR(qh->elink));
	if(!sameptr(qh->next, qh->link))
		s = seprint(s, se, " [next %#p != link %#ulx %#p]", qh->next,
		            qh->link, QPTR(qh->link));
	if((qh->link & Tdterm) != 0)
		s = seprint(s, se, "T");
...
xdump(Ctlr* ctlr, int doilock)
{
	...
	print("uhci port %#x frames %#p nintr %d ntdintr %d", ctlr->port,
	      ctlr->frames, ctlr->nintr, ctlr->ntdintr);
	print(" nqhintr %d nisointr %d\n", ctlr->nqhintr, ctlr->nisointr);
	print("cmd %#ux sts %#ux fl %#ulx ps1 %#ux ps2 %#ux frames[0] %#ulx\n",
	      INS(Cmd), INS(Status), INL(Flbaseadd), INS(PORT(0)), INS(PORT(1)),
	      ctlr->frames[0]);
...
isocanread(void* a)
{
	...
	return iso->state == Qclose ||
	       (iso->state == Qrun && iso->tok == Tdtokin &&
	        iso->tdi != iso->tdu);
}
...
isocanwrite(void* a)
{
	...
	iso = a;
	return iso->state == Qclose ||
	       (iso->state == Qrun && iso->tok == Tdtokout &&
	        iso->tdu->next != iso->tdi);
}
...
tdisoinit(Isoio* iso, Td* td, int32_t count)
{
	td->ndata = count;
	td->token =
	    ((count - 1) << 21) | ((iso->usbid & 0x7FF) << 8) | iso->tok;
...
seprintep(char* s, char* e, Ep* ep)
{
	...
	case Tctl:
		...
		s = seprint(s, e, "cio %#p qh %#p"
		            ...
		            cio, cio->qh, cio->usbid, cio->toggle, cio->tok,
		            cio->err);
		break;
	case Tbulk:
	case Tintr:
	...
	case Tiso:
		iso = ep->aux;
		s = seprint(
		    s, e, "iso %#p id %#x tok %#x tdu %#p tdi %#p err %s\n",
...
init(Hci* hp)
{
	...
	uhcirun(ctlr, 1);
	dprint("uhci: init: cmd %#ux sts %#ux sof %#ux", INS(Cmd), INS(Status),
	       INS(SOFmod));
	dprint(" flb %#ulx frno %#ux psc0 %#ux psc1 %#ux", INL(Flbaseadd),
	       INS(Frnum), INS(PORT(0)), INS(PORT(1)));
	/* guess other ports */
	for(i = 2; i < 6; i++) {
		sts = INS(PORT(i));
