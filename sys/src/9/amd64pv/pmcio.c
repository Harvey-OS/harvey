...
enum { PeNreg = 4, /* Number of Pe/Pct regs */
};
...
#define SetEvMsk(v, e)                                                         \
	((v) | (((e)&PeEvMskL) | (((e) << (PeEvMsksh - 8)) & PeEvMskH)))
...
pmctrans(PmcCtl* p)
{
	...
	for(pi = &pmcids[0]; pi->portdesc[0] != '\0'; pi++) {
		if(strncmp(p->descstr, pi->portdesc, strlen(pi->portdesc)) ==
		   0) {
			strncpy(p->descstr, pi->archdesc,
			        strlen(pi->archdesc) + 1);
			return 0;
		}
	}
	return 1;
