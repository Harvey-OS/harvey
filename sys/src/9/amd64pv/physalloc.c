...
seprintphysstats(char* s, char* e)
{
	...
	for(i = 0; i < Ndoms; i++) {
		...
		if(b->size > 0)
			s = seprint(s, e,
			            "%uld/%uld %ulldK color %d blocks avail\n",
...
S1:
	...
	DBG("\tbsl: BLOCK(b,i) %d index %ulld kval %d\n", BLOCK(b, i),
	    BLOCK(b, i) / ((1 << l->kval) / b->bminsz), l->kval);
	 ...
	 */
	if(l->kval == b->kmax || p->tag == Used ||
	   (p->tag == Avail && p->kval != l->kval)) {
		/*
		 * Put on list.
		 */
		l->tag = Avail;
		...
		DBG("bsl: free @ i %d BLOCK(b,i) %d kval %d next %d %s\n", i,
		    BLOCK(b, i), l->kval, l->next, l->tag ? "avail" : "used");
		return;
	}
...
iimbchunk(Bal* b, uintmem a, uintmem e, int type)
{
	...
	for(k = b->kmin, s = b->bminsz; a + s < e && k < b->kmax;
	    s <<= 1, k += 1) {
		if(a & s) {
			plop(b, a, k, type);
			a += s;
	...
	for(addr = a; addr < a + size; addr += len) {
		...
		if(b->kmin == 0) {
			...
			if(!ISPOWEROF2(sys->pmend))
				b->kspan++;
			dtsz =
			    sizeof(Buddy) * (UNO << (b->kspan - b->kmin + 1));
