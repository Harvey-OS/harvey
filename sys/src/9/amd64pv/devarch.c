...
/* shit */
	...
	static int32_t archwrite(Chan * c, void* a, int32_t n, int64_t offset)
	{
		char* p;
		int port;
		uint16_t* sp;
		...
		default:
			if(c->qid.path < narchdir &&
			   (fn = writefn[c->qid.path]))
				return fn(c, a, n, offset);
			error(Eperm);
			break;
		}
	...
	static int32_t cputyperead(Chan * c, void* a, int32_t n, int64_t off)
	{
		Proc* up = externup();
		char buf[512], *s, *e;
		int i, k;
	...
	void archreset(void)
	{
		 ...
		 * chips. The actual location and sequence could be extracted
		from
	 ...
	 */
	uint64_t fastticks(uint64_t * hz)
	{
		Proc* up = externup();
		if(hz != nil)
			*hz = m->cpuhz;
	...
	uint32_t ms(void) { return fastticks2us(rdtsc()); }

	/*
	 *  set next timer interrupt
	 */
	void timerset(uint64_t x)
	{
	...
	void cycles(uint64_t * t) { *t = rdtsc(); }
