...
static Ctlrtype cttab[Nctlrtype] = {
    i82563, 9014, Fpba, "i82563", i82566, 1514, Fload, "i82566", i82567, 9234,
    Fload, "i82567", i82567m, 1514, 0, "i82567m", i82571, 9234, Fpba, "i82571",
    i82572, 9234, Fpba, "i82572", i82573, 8192, Fert,
    "i82573", /* terrible perf above 8k */
    i82574, 9018, 0, "i82574", i82575, 9728, F75 | Fflashea, "i82575", i82576,
    9728, F75, "i82576", i82577, 4096, Fload | Fert, "i82577", i82577m, 1514,
    Fload | Fert, "i82577", i82578, 4096, Fload | Fert, "i82578", i82578m, 1514,
    Fload | Fert, "i82578", i82579, 9018, Fload | Fert, "i82579", i82580, 9728,
    F75, "i82580", i82583, 1514, 0, "i82583",
};
...
static char* statistics[Nstatistics] = {
    "CRC Error", "Alignment Error", "Symbol Error", "RX Error",
    "Missed Packets", "Single Collision", "Excessive Collisions",
    "Multiple Collision", "Late Collisions", nil, "Collision",
    "Transmit Underrun", "Defer", "Transmit - No CRS", "Sequence Error",
    "Carrier Extension Error", "Receive Error Length", nil, "XON Received",
    "XON Transmitted", "XOFF Received", "XOFF Transmitted",
    "FC Received Unsupported", "Packets Received (64 Bytes)",
    "Packets Received (65-127 Bytes)", "Packets Received (128-255 Bytes)",
    "Packets Received (256-511 Bytes)", "Packets Received (512-1023 Bytes)",
    "Packets Received (1024-mtu Bytes)", "Good Packets Received",
    "Broadcast Packets Received", "Multicast Packets Received",
    "Good Packets Transmitted", nil, "Good Octets Received", nil,
    "Good Octets Transmitted", nil, nil, nil, "Receive No Buffers",
    "Receive Undersize", "Receive Fragment", "Receive Oversize",
    "Receive Jabber", "Management Packets Rx", "Management Packets Drop",
    "Management Packets Tx", "Total Octets Received", nil,
    "Total Octets Transmitted", nil, "Total Packets Received",
    "Total Packets Transmitted", "Packets Transmitted (64 Bytes)",
    "Packets Transmitted (65-127 Bytes)", "Packets Transmitted (128-255 Bytes)",
    "Packets Transmitted (256-511 Bytes)",
    "Packets Transmitted (512-1023 Bytes)",
    "Packets Transmitted (1024-mtu Bytes)", "Multicast Packets Transmitted",
    "Broadcast Packets Transmitted", "TCP Segmentation Context Transmitted",
    "TCP Segmentation Context Fail", "Interrupt Assertion",
    "Interrupt Rx Pkt Timer", "Interrupt Rx Abs Timer",
    "Interrupt Tx Pkt Timer", "Interrupt Tx Abs Timer",
    "Interrupt Tx Queue Empty", "Interrupt Tx Desc Low", "Interrupt Rx Min",
...
i82563txinit(Ctlr* ctlr)
{
	...
	if(cttab[ctlr->type].flag & F75)
		...
		csr32w(ctlr, Tctl,
		       0x0F << CtSHIFT | Psp | 66 << ColdSHIFT | Mulr);
	...
	while(Next(rdt, m) != ctlr->rdh) {
		...
		if(bp == nil) {
			...
			if(now - lasttime > 2000)
				iprint(
				    "#l%d: 82563: all %d rx buffers in use\n",
...
i82563rxinit(Ctlr* ctlr)
{
	...
	if(cttab[ctlr->type].flag & F75)
		csr32w(ctlr, Rxdctl, 1 << WthreshSHIFT | 8 << PthreshSHIFT |
		                         1 << HthreshSHIFT | Qenable);
	else
	...
	for(;;) {
		...
		for(;;) {
			...
			if((rd->status & Reop) && rd->errors == 0) {
				bp->wp += rd->length;
				/* bp->lim = bp->wp;	lie like a dog.  avoid
				 * packblock. */
				if(!(rd->status & Ixsm)) {
					ctlr->ixsm++;
					if(rd->status & Ipcs) {
...
i82563attach(Ether* edev)
{
	...
	if((ctlr->rdba = mallocalign(ctlr->nrd * sizeof(Rd), 128, 0, 0)) ==
	       nil ||
	   (ctlr->tdba = mallocalign(ctlr->ntd * sizeof(Td), 128, 0, 0)) ==
	       nil ||
...
defaultea(Ctlr* ctlr, uint8_t* ra)
{
	...
	if(cttab[ctlr->type].flag & Fflashea) {
		/* intel mb bug */
		u = (uint64_t)csr32r(ctlr, Rah) << 32u |
		    (uint32_t)csr32r(ctlr, Ral);
...
static Cmdtab i82563ctlmsg[] = {
    CMrdtr, "rdtr", 2, CMradv, "radv", 2, CMpause, "pause", 1, CMan, "an", 1,
};
