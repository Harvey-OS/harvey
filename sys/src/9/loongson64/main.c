...
/* args passed by boot loader for main(), addr in 32 bit */
int		_argc;
long	*_argv;
long	*_env;
...
plan9iniinit(void)
{
	...
	nconf = 0;
	if(_argv != nil){
		memmove(confbuf, (char*)_argv[0], sizeof(confbuf));
		for(i=1; i<_argc && nconf<MAXCONF; i++){
			_argv[i] += (long)confbuf - _argv[0];
			if(*(char*)_argv[i] == '#')
				continue;
			cp = strchr((char*)_argv[i], '=');
			if(cp == nil)
				continue;
			*cp++ = '\0';
			confname[nconf] = (char*)_argv[i];
			confval[nconf] = cp;
			nconf++;
		}
	}
	...
	nenv = 0;
	if(_env != nil)
		for(i=0; (char*)_env[i] != nil && nenv<MAXBOOTENV; i++){
			if(*(char*)_env[i] == '#')
				continue;
			cp = strchr((char*)_env[i], '=');
			if(cp == nil)
				continue;
			*cp++ = '\0';
			bootenvname[nenv] = (char*)_env[i];
			bootenvval[nenv] = cp;
			nenv++;
		}
}
...
init0(void)
{
	...
	if(!waserror()){
		ksetenv("cputype", "spim64", 0);
		snprint(buf, sizeof buf, "spim64 %s loongson 2f", conffile);
...
userinit(void)
{
	...
	p->sched.sp = (uintptr)p->kstack+KSTACK-Stkheadroom;
...
writeconf(void)
{
	...
	_argv = (void*)CONFARGV;
	_argv[0] = (long)BOOTARGS;
	for(q=p; *q; q++) {
		_argv[_argc] = (long)((uintptr)BOOTARGS + (q - p));
		_argc++;
		q += strlen(q);
		*q = '=';
		q += strlen(q);
	...
	if(n >= BOOTARGSLEN || (uintptr)_env >= CONFARGV + CONFARGVLEN)
		error("kernel configuration too large");
	memmove(BOOTARGS, p, n);
	memset(BOOTARGS + n, '\0', BOOTARGSLEN - n);
	memset(_env, 0, CONFARGV + CONFARGVLEN - (uintptr)_env);
