...
static void
ckrring(char *title, int pflag)
{
	Rdesc *rp = l.rhead, *rn;
	int i, errs = 0;

	for(i=0; i<NRRING && errs < 10; i++, rp++){
		if(rp->base != l.rbase + i*(BY2PG/2)){
			print("ckrring %s: %d base 0x%ux expect 0x%ux\n",
				title, i, rp->base, l.rbase + i*(BY2PG/2));
			++errs;
		}
		rn = (i<NRRING-1) ? rp+1 : l.rhead;
		if(rp->next != rn){
			print("ckrring %s: %d next 0x%ux expect 0x%ux\n",
				title, i, rp->next, rn);
			++errs;
		}
	}
...
seqreset(void)
{
	...
	if(!l.inited){
		...
		l.rring = rp = pxspanalloc(NRRING*sizeof(Rdesc), BY2PG, BIGMEG);
		l.rhead = rp;
		l.rbase = pxspanalloc(NRRING*BY2PG/2, BY2PG, BIGMEG);
		...
		l.tring = pxspanalloc(NXRING*sizeof(Xdesc), BY2PG, BIGMEG);
		l.ttail = xp = l.tring;
		l.tbase = pxspanalloc(NXRING*BY2PG/2, BY2PG, BIGMEG);
...
iskpwork(void *arg)
{
	...
	if(l.self.first)
		return 1;
	if(l.thead != KADDR1(dev->cpfxbdp) && (l.thead->count&Busy))
		return 1;
	if(l.closeline)
		return 1;
	return 0;
