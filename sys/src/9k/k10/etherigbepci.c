...
igbetxinit(Ctlr* ctlr)
{
	...
	csr32w(ctlr, Tdbal, phys);
	csr32w(ctlr, Tdbah, phys>>32);
...
igberxinit(Ctlr* ctlr)
{
	...
	csr32w(ctlr, Rdbal, phys);
	csr32w(ctlr, Rdbah, phys>>32);
...
at93c46r(Ctlr* ctlr)
{
	...
	for(addr = 0; addr < 0x40; addr++){
		...
		if(at93c46io(ctlr, rop, (0x06<<bits)|addr) != 0){
			print("igbepci: can't set EEPROM address %#2.2ux\n", addr);
			goto release;
		}
...
igbereset(Ctlr* ctlr)
{
	...
	if((r = at93c46r(ctlr)) != 0xBABA){
		print("igbepci: bad EEPROM checksum - %#4.4ux\n", r);
		return -1;
	}
