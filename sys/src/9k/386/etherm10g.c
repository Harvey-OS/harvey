...
reset(Ether *e, Ctlr *c)
{
	...
	*c->coal = pbit32(25);
	...
	print("m10g: read %ud MB/s;", ((i>>16)*sz*2) / (i&0xffff));
	i = dmatestcmd(c, DMAwrite, c->done.busaddr, sz);
	print(" write %ud MB/s;", ((i>>16)*sz*2) / (i&0xffff));
...
replenish(Rx *rx)
{
	...
	while(p->n >= 8 && e){
		...
		for(i = 0; i < 8; i++){
			b = balloc(rx);
			buf[i*2]   = pbit32(PCIWADDRH(b->wp));
			buf[i*2+1] = pbit32(PCIWADDRL(b->wp));
			rx->host[idx+i] = b;
			assert(b);
		}
