...
static vlong
mkpath(ushort tree, ushort devno)
{
	return tree << 16 | devno;
}

static int
path2treeno(ulong path)
{
	return path >> 16;
}

static int
path2devno(ushort path)
{
	return path;
}
...
rdconf(void)
{
	...
	cc->dev->read(cc, confstr, sizeof confstr, 0);
	cclose(cc);
	cc = nil;
...
io(Fsdev *mp, Inner *in, int isread, void *a, long l, vlong off)
{
	...
	if (isread)
		wl = mc->dev->read(mc, a, l, off);
	else
		wl = mc->dev->write(mc, a, l, off);
	poperror();
	return wl;
}
...
interio(Fsdev *mp, int isread, void *a, long n, vlong off)
{
	...
	blk  = (uvlong)off / Blksize;
	boff = (uvlong)off % Blksize;
	wsz  = Blksize - boff;
	res = n;
	while(n > 0){
		mblk = blk / mp->ndevs;
