 ...
 * Use color c.  Centered if cen is non-zero, right-justified if right is
 * non-zero.
...
m_text(int x, int y, char* p, char* q, int c, int cen, int right)
{
	...
	if(cen)
		...
		x -= tsize.x;
	stringn(offscreen, Pt(x, y - tsize.y / 2), getcolor(c), ZP, font, p,
	        q - p);
	return y + tsize.y;
}
/*
...
{
	line(offscreen, Pt(x0, y0), Pt(x1, y1), Endsquare, Endsquare, 0,
	     getcolor(c), ZP);
}
char*
scanint(char* s, int* n)
{
	while(*s < '0' || '9' < *s) {
		if(*s == '\0') {
...
{
	if(offscreen == screen) {
		offscreen = allocimage(display, insetrect(screen->r, 4),
		                       screen->chan, 0, -1);
		if(offscreen == 0) {
