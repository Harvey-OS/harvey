...
static void
polygon(int cnt[], double* pts[], Windrule w, int v)
{
	...
	for(cntp = cnt, ptsp = pts; *cntp; cntp++, ptsp++) {
		...
		for(xp = *ptsp, i = 0; i != nvert; xp += 2, i++) {
			...
			if(p1.y <= screen->r.min.y)
				continue;
			if(p0.y >= screen->r.max.y)
				continue;
			ep->p = p0;
			...
			if(p10.x >= 0) {
				...
				p10.x = -p10.x;
				ep->dx =
				    -(p10.x / p10.y); /* this nonsense rounds
				                         toward zero */
				ep->dx1 = ep->dx - 1;
			}
			ep->x = 0;
	...
	for(yp = ylist, y = screen->r.min.y; yp != eylist; yp++, y++) {
		...
		for(ep = *yp; ep; ep = nextep) {
			...
			if(nwind & w) { /* inside */
				if(!(wind & w)) {
					left = ep->p.x;
					if(left < screen->r.min.x)
						left = screen->r.min.x;
				}
			} else if(wind & w) {
				right = ep->p.x;
				if(right >= screen->r.max.x)
					right = screen->r.max.x;
#define BART_BUG_FIXED /* what goes on here?? -rob */
#ifdef BART_BUG_FIXED
				if(right > left)
					line(screen, Pt(left, y), Pt(right, y),
