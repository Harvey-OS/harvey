...
#define OP_RRR(op, r1, r2, r3)                                                 \
	(0x00000000 | op | (((r1)&31L) << 0) | (((r2)&31L) << 14) |            \
	 (((r3)&31L) << 25))
#define OP_IRR(op, i, r2, r3)                                                  \
	(0x00002000L | (op) | (((i)&0x1fffL) << 0) | (((r2)&31L) << 14) |      \
	 (((r3)&31L) << 25))
#define OP_BRA(op, pc) ((op) | (((pc)&0x3fffff) << 0))
...
asmout(Prog* p, Optab* o, int aflag)
{
	...
	case 4: /* mov r, soreg */
		...
		v = regoff(&p->to);
		if(v == 0 && p->reg != NREG)
			o1 = OP_RRR(opcode(p->as + AEND), p->reg, r,
			            p->from.reg);
		else
	...
	case 7: /* mov [+r]r, asi */
		o1 = OP_RRR(opcode(p->as + AEND), p->reg, p->to.reg,
		            p->from.reg);
	...
	case 12: /* mov r, loreg */
		...
		o1 = 0x1000000 | (REGTMP << 25) |
		     ((v >> 10) & 0x3fffff); /* sethi */
		o2 = OP_RRR(opcode(AADD), r, REGTMP, REGTMP);
		o3 = OP_IRR(opcode(p->as + AEND), (v & 0x3ff), REGTMP,
		            p->from.reg);
		break;

	case 13: /* mov $ucon, r */
		v = regoff(&p->from);
		o1 = 0x1000000 | (p->to.reg << 25) |
		     ((v >> 10) & 0x3fffff); /* sethi */
		break;
	...
	case 31: /* ba jmp */
		...
		if(r == ABA && p->link && p->cond && isnop(p->link)) {
			...
			/* cant set annul here because pc has already been
			 * counted */
		}
		break;
...
opcode(int a)
{
	...
	case ATAS:
		return OP3(13); /* tas is really ldstub */
