...
Dirtab dirtab[] = {{".", QTDIR, Qdir, 0500 | DMDIR},
...
Xfid* (*fcall[Tmax])(Filsys*, Xfid*, Fid*) = {
        [Tflush] = filsysflush,   [Tversion] = filsysversion,
        [Tauth] = filsysauth,     [Tattach] = filsysattach,
        [Twalk] = filsyswalk,     [Topen] = filsysopen,
        [Tcreate] = filsyscreate, [Tread] = filsysread,
        [Twrite] = filsyswrite,   [Tclunk] = filsysclunk,
        [Tremove] = filsysremove, [Tstat] = filsysstat,
        [Twstat] = filsyswstat,
};
...
filsysproc(void* arg)
{
	...
	x = nil;
	for(;;) {
		buf = emalloc(messagesize + UTFmax); /* UTFmax for appending
		                                        partial rune in
		                                        xfidwrite */
...
filsysversion(Filsys* fs, Xfid* x, Fid* f)
{
	...
	if(!firstmessage)
		return filsysrespond(x->fs, x, &t,
		                     "version request not first message");
	if(x->msize < 256)
		return filsysrespond(x->fs, x, &t,
		                     "version: message size too small");
	messagesize = x->msize;
	t.msize = messagesize;
	...
	if(x->nwname > 0) {
		for(i = 0; i < x->nwname; i++) {
			...
			if(snarffd >= 0 && strcmp(x->wname[i], "snarf") == 0)
				break; /* don't serve /dev/snarf if it's
...
filsysread(Filsys* fs, Xfid* x, Fid* f)
{
	...
	case Qwsysdir:
		...
		for(i = 0; d->name != nil && i < e; i += len) {
			len = dostat(fs, WIN(x->f->qid), d, b + n, x->count - n,
			             clock);
			if(len <= BIT16SZ)
				break;
			if(i >= o)
...
filsysstat(Filsys* fs, Xfid* x, Fid* f)
{
	...
	t.nstat = dostat(fs, WIN(x->f->qid), f->dir, t.stat,
	                 messagesize - IOHDRSZ, getclock());
