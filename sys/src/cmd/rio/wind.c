...
enum { HiWater = 640000, /* max size of history */
...
wmk(Image* i, Mousectl* mc, Channel* ck, Channel* cctl, int scrolling)
{
	...
	if(cols[0] == nil) {
		...
		darkgrey =
		    allocimage(display, Rect(0, 0, 1, 1), CMAP8, 1, 0x666666FF);
		cols[BACK] = display->white;
		cols[HIGH] =
		    allocimage(display, Rect(0, 0, 1, 1), CMAP8, 1, 0xCCCCCCFF);
		cols[BORD] =
		    allocimage(display, Rect(0, 0, 1, 1), CMAP8, 1, 0x999999FF);
		cols[TEXT] = display->black;
		cols[HTEXT] = display->black;
		titlecol =
		    allocimage(display, Rect(0, 0, 1, 1), CMAP8, 1, DGreygreen);
		lighttitlecol = allocimage(display, Rect(0, 0, 1, 1), CMAP8, 1,
		                           DPalegreygreen);
		holdcol =
		    allocimage(display, Rect(0, 0, 1, 1), CMAP8, 1, DMedblue);
		lightholdcol =
		    allocimage(display, Rect(0, 0, 1, 1), CMAP8, 1, DGreyblue);
		paleholdcol = allocimage(display, Rect(0, 0, 1, 1), CMAP8, 1,
		                         DPalegreyblue);
	}
...
wrefresh(Window* w, Rectangle rec)
{
	...
	if(w->mouseopen)
		return;
	draw(w->i, insetrect(w->i->r, Borderwidth), w->cols[BACK], nil,
	     w->i->r.min);
	w->ticked = 0;
	if(w->p0 > 0)
	...
	for(;;) {
		...
		case WMouse:
			if(w->mouseopen) {
				...
				/* queue click events */
				if(!w->mouse.qfull &&
				   lastb != w->mc.buttons) { /* add to ring */
			...
			break;
		case WMouseread:
			/* send a queued event or, if the queue is empty, the
			 * current state */
			/* if the queue has filled, we discard all the events it
			 * contained. */
		...
		case WCread:
			...
			while(i < nb && (w->qh < w->nr || w->nraw > 0)) {
				if(w->qh == w->nr) {
					...
					wid =
					    runetochar(t + i, &w->run[w->qh++]);
				c = t[i]; /* knows break characters fit in a
				             byte */
				i += wid;
				if(!w->rawing && (c == '\n' || c == '\004')) {
					if(c == '\004')
 ...
 * is dying and trying to print tombstone, kernel is blocked holding p->debug
 * lock.
 */
void
...
wplumb(Window* w)
{
	...
	if(w->q1 > w->q0)
		...
		while(p0 > 0 && w->run[p0 - 1] != ' ' &&
		      w->run[p0 - 1] != '\t' && w->run[p0 - 1] != '\n')
			p0--;
		while(p1 < w->nr && w->run[p1] != ' ' && w->run[p1] != '\t' &&
		      w->run[p1] != '\n')
			p1++;
	...
	if(dl < 0) {
		...
		q0 = w->org +
		     frcharofpt(w, Pt(w->Frame.r.min.x,
		                      w->Frame.r.min.y + dl * w->font->height));
...
wselect(Window* w)
{
	...
	if(w->mc.buttons == b) {
		...
		frselect(w, &w->mc);
		/* horrible botch: while asleep, may have lost selection
		 * altogether */
		if(selectq > w->nr)
			selectq = w->org + w->p0;
		w->Frame.scroll = nil;
	...
	if(q0 == q1) {
		if(q0 == w->q0 && clickwin == w &&
		   w->mc.msec - clickmsec < 500) {
			wdoubleclick(w, &q0, &q1);
			clickwin = nil;
		} else {
			clickwin = w;
...
wclosewin(Window* w)
{
	...
	for(i = 0; i < nhidden; i++)
		if(hidden[i] == w) {
			--nhidden;
			memmove(hidden + i, hidden + i + 1,
			        (nhidden - i) * sizeof(hidden[0]));
			hidden[nhidden] = nil;
			break;
		}
	for(i = 0; i < nwindow; i++)
		if(window[i] == w) {
			...
			memmove(window + i, window + i + 1,
			        (nwindow - i) * sizeof(Window*));
			w->deleted = TRUE;
			r = w->i->r;
			/* move it off-screen to hide it, in case client is slow
			 * in letting it go */
...
Rune* left[] = {left1, left2, left3, nil};
Rune* right[] = {right1, left2, left3, nil};
...
wsetorigin(Window* w, uint org, int exact)
{
	...
	if(a >= 0 && a < w->nchars) {
		frdelete(w, 0, a);
		fixup = 1; /* frdelete can leave end of last line in wrong
		              selection mode; it doesn't know what follows */
...
winsert(Window* w, Rune* r, int n, uint q0)
{
	...
	w->nr += n;
	/* if output touches, advance selection, not qh; works best for keyboard
	 * and output */
	if(q0 <= w->q1)
		w->q1 += n;
	if(q0 <= w->q0)
