...
xfidflush(Xfid* x)
{
	...
	for(xf = xfid; xf; xf = xf->next)
		if(xf->flushtag == x->oldtag) {
			xf->flushtag = -1;
			xf->flushing = TRUE;
			incref(xf); /* to hold data structures up at tail of
			               synchronization */
			if(xf->ref == 1)
...
xfidattach(Xfid* x)
{
	...
	Allocate:
		if(!goodrect(r))
			...
			if(hideit)
				i = allocimage(display, r, screen->chan, 0,
				               DWhite);
			else
			...
			if(i) {
				...
				if(pid == 0)
					pid = -1; /* make sure we don't pop a
					             shell! - UGH */
				w = new(i, hideit, scrolling, pid, nil, nil,
				        nil);
	...
	} else if(strncmp(x->aname, "new", 3) ==
	          0) { /* new -dx -dy - new syntax, as in wctl */
		pid = 0;
		if(parsewctl(nil, ZR, &r, &pid, nil, &hideit, &scrollit, &dir,
		             x->aname, errbuf) < 0)
			err = errbuf;
		else
			goto Allocate;
...
xfidopen(Xfid* x)
{
	...
	case Qwctl:
		if(x->mode == OREAD || x->mode == ORDWR) {
			/*
			 * It would be much nicer to implement fan-out for wctl
			 * reads,
			 * so multiple people can see the resizings, but rio
			 * just isn't
			 * structured for that.  It's structured for /dev/cons,
			 * which gives
			 * alternate data to alternate readers.  So to keep
			 * things sane for
			 * wctl, we compromise and give an error if two people
			 * try to
			 * open it.  Apologies.
			 */
			if(w->wctlopen) {
...
xfidclose(Xfid* x)
{
	...
	case Qsnarf:
		if(x->f->mode == ORDWR || x->f->mode == OWRITE) {
			...
			cvttorunes(tsnarf, ntsnarf, snarf, &nb, &nsnarf,
			           &nulls);
			free(tsnarf);
			tsnarf = nil;
			ntsnarf = 0;
...
xfidwrite(Xfid* x)
{
	...
	case Qcons:
		...
		if(nr > 0) {
			memmove(
			    x->data + nr, x->data,
			    cnt); /* there's room: see malloc in filsysproc */
		...
		if(x->flushing) {
			...
			pair.ns = 0;
			send(cwm.cw,
			     &pair); /* wake up window with empty data */
			filsyscancel(x);
			return;
		}
		qlock(&x->active);
	...
	case Qlabel:
		if(off != 0) {
			filsysrespond(x->fs, x, &fc,
			              "non-zero offset writing label");
			return;
		}
		free(w->label);
...
xfidread(Xfid* x)
{
	...
	case Qcons:
		...
		c2 = crm.c2;
		t = malloc(cnt + UTFmax +
		           1); /* room to unpack partial rune plus */
		pair.s = t;
		pair.ns = cnt;
		send(c1, &pair);
		if(x->flushing) {
	...
	case Qmouse:
		...
		if(w->resized)
			c = 'r';
		n = sprint(buf, "%c%11d %11d %11d %11ld ", c, ms.xy.x, ms.xy.y,
		           ms.buttons, ms.msec);
		w->resized = 0;
		fc.data = buf;
		fc.count = min(n, cnt);
	...
	/* The algorithm for snarf and text is expensive but easy and rarely
	 * used */
	case Qsnarf:
		getsnarf();
		if(nsnarf)
	...
	caseImage:
		if(off < 5 * 12) {
			n = sprint(buf, "%11s %11d %11d %11d %11d ",
			           chantostr(cbuf, screen->chan), i->r.min.x,
		...
		t = malloc(cnt);
		fc.data = t;
		n = readwindow(i, t, r, off,
		               cnt); /* careful; fc.count is unsigned */
		if(n < 0) {
			buf[0] = 0;
