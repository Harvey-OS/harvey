...
#define PROC                                                                   \
	...
	EXIT(((nerrors && squawk) ? 1 : 0),
	     ((nerrors && squawk) ? "conversion error" : 0));
...
list(void)
{
	...
	for(c = convert; c->name; c++) {
		if((c->flags & From) && c[1].name &&
		   (strcmp(c[1].name, c->name) == 0)) {
			EPR "%c%s", ch, c->name);
			c++;
...
int32_t tabascii[256] = {
    ...
    0x78, 0x79, 0x7a, 0x7b, 0x7c, 0x7d, 0x7e, 0x7f, -1,   -1,   -1,   -1,
...
int32_t tabmsdos[256] = /* from jhelling@cs.ruu.nl (Jeroen Hellingman) */
    {
     ...
     0x7e,   0x7f,   0x00c7, 0x00fc, 0x00e9, 0x00e2, 0x00e4, 0x00e0, 0x00e5,
     0x00e7, /* latin */
     ...
     0x2561, 0x2562, 0x2556, /* forms */
     0x2555, 0x2563, 0x2551, 0x2557, 0x255d, 0x255c, 0x255b, 0x2510, 0x2514,
     0x2534, 0x252c, 0x251c, 0x2500, 0x253c, 0x255e, 0x255f, 0x255a, 0x2554,
     0x2569, 0x2566, 0x2560, 0x2550, 0x256c, 0x2567, 0x2568, 0x2564, 0x2565,
     0x2559, 0x2558, 0x2552, 0x2553, 0x256b, 0x256a, 0x2518, 0x250c, 0x2588,
     0x2584, 0x258c, 0x2590, 0x2580, 0x03b1, 0x00df, 0x0393, 0x03c0, 0x03a3,
     0x03c3, 0x00b5, 0x03c4, /* greek */
     0x03a6, 0x0398, 0x2126, 0x03b4, 0x221e, 0x2205, 0x2208, 0x2229, 0x2261,
     0x00b1, 0x2265, 0x2264, 0x2320, 0x2321, 0x00f7, 0x2248, /* math */
...
int32_t tabmsdos2[256] = /* from jhelling@cs.ruu.nl (Jeroen Hellingman) */
    {
     ...
     0x2195, 0x203c, 0x00b6, 0x00a7, 0x2043, 0x21a8, 0x2191, 0x2193, 0x2192,
     0x2190, 0x2319, 0x2194, 0x25b2, 0x25bc, 0x20,   0x21,   0x22,   0x23,
     0x24,   0x25,   0x26,   0x27,   0x28,   0x29,   0x2a,   0x2b,   0x2c,
     0x2d,   0x2e,   0x2f,   0x30,   0x31,   0x32,   0x33,   0x34,   0x35,
     0x36,   0x37,   0x38,   0x39,   0x3a,   0x3b,   0x3c,   0x3d,   0x3e,
     0x3f,   0x40,   0x41,   0x42,   0x43,   0x44,   0x45,   0x46,   0x47,
...
struct convert convert[] = {
    ...
    {"8859-1", "Latin-1 (Western and Northern Europe including Italian)", Table,
     (void*)tab8859_1},
    {"8859-2",
     "Latin-2 (Eastern Europe except Turkey and the Baltic countries)", Table,
     (void*)tab8859_2},
    {"8859-3", "Latin-3 (Mediterranean, South Africa, Esperanto)", Table,
     (void*)tab8859_3},
    {"8859-4", "Latin-4 (Scandinavia and the Baltic countries; obsolete)",
     Table, (void*)tab8859_4},
    ...
    {"ebcdic", "EBCDIC", Table,
     (void*)tabebcdic}, /* 6f is recommended bad map */
    {"euc-k", "Korean EUC: ASCII+KS C 5601 1987", From | Func, 0,
     (Fnptr)uksc_in},
    {"euc-k", "Korean EUC: ASCII+KS C 5601 1987", Func, 0, (Fnptr)uksc_out},
    {"euc-kr", "Korean EUC: ASCII+KS C 5601 1987", From | Func, 0,
     (Fnptr)uksc_in},
    {"euc-kr", "Korean EUC: ASCII+KS C 5601 1987", Func, 0, (Fnptr)uksc_out},
    {"ks_c_5601-1987", "Korean EUC: ASCII+KS C 5601 1987", From | Func, 0,
     (Fnptr)uksc_in},
    {"ks_c_5601-1987", "Korean EUC: ASCII+KS C 5601 1987", Func, 0,
     (Fnptr)uksc_out},
    ...
    {"ibm850", "IBM Code Page 850 (Multilingual Latin I)", Table,
     (void*)tabcp850},
    ...
    {"ibm858", "IBM Code Page 858 (Multilingual Latin I+Euro)", Table,
     (void*)tabcp858},
    ...
    {"iso-2022-jp", "alias for jis-kanji (MIME)", From | Func, 0,
     (Fnptr)jisjis_in},
    ...
    {"macrom", "Macintosh Standard Roman character set", Table,
     (void*)tabmacroman},
    ...
    {"unicode-be", "Unicode 1.1 big-endian", From | Func, 0,
     (Fnptr)unicode_in_be},
    {"unicode-be", "Unicode 1.1 big-endian", Func, 0, (Fnptr)unicode_out_be},
    {"unicode-le", "Unicode 1.1 little-endian", From | Func, 0,
     (Fnptr)unicode_in_le},
    ...
    {"utf-16be", "alias for unicode-be (MIME)", From | Func, 0,
     (Fnptr)unicode_in_be},
    ...
    {"windows-1250", "Windows Code Page 1250 (Central Europe)", Table,
     (void*)tabcp1250},
    {"windows-1251", "Windows Code Page 1251 (Cyrillic)", Table,
     (void*)tabcp1251},
    {"windows-1252", "Windows Code Page 1252 (Latin I)", Table,
     (void*)tabcp1252},
    {"windows-1253", "Windows Code Page 1253 (Greek)", Table, (void*)tabcp1253},
    {"windows-1254", "Windows Code Page 1254 (Turkish)", Table,
     (void*)tabcp1254},
    {"windows-1255", "Windows Code Page 1255 (Hebrew)", Table,
     (void*)tabcp1255},
    {"windows-1256", "Windows Code Page 1256 (Arabic)", Table,
     (void*)tabcp1256},
    {"windows-1257", "Windows Code Page 1257 (Baltic)", Table,
     (void*)tabcp1257},
    {"windows-1258", "Windows Code Page 1258 (Vietnam)", Table,
     (void*)tabcp1258},
    {0},
};
