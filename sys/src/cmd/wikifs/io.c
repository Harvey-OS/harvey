...
static void
gencurrent(Wcache* w, Qid* q, char* file, char* lock, uint32_t* t, Whist** wp,
           int n)
{
	Dir* d;
	Whist* wh;
	...
	if(((d = wdirstat(file)) == nil) ||
	   (d->qid.path == q->path && d->qid.vers == q->vers)) {
		*t = time(0);
		wunlock(w);
		free(d);
		return;
...
currentmap(int force)
{
	...
	if(map && d->qid.path == map->qid.path &&
	   d->qid.vers == map->qid.vers) {
		map->t = time(0);
		goto Return;
	}
...
writepage(int num, uint32_t t, String* s, char* title)
{
	...
	if(b = wBopen(tmp, OREAD)) {
		...
		if(p == nil || p[0] != 'D') {
			...
			s_free(os);
			snprint(err, sizeof err, "update conflict %lud != %s",
			        t, p + 1);
			conflict = 1;
		}
		Bterm(b);
