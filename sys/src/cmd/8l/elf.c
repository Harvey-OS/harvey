...
elf32sectab(void (*putl)(long))
{
	...
	elf32shdr(putl, Stitext, Progbits, Salloc | Sexec, INITTEXT, HEADR,
	          textsize, 0, 0, 0x10000, 0);
	...
	elf32shdr(putl, Stistrtab, Strtab, 1 << 5, 0,
	          HEADR + textsize + datsize + symsize + 3 * Shdr32sz, 14, 0, 0,
	          1, 0);
	elfstrtab();
}
...
elf32(int mach, int bo, int addpsects, void (*putpsects)(Putl))
{
	...
	if(debug['S'])
		putl(HEADR + textsize + datsize +
		     symsize); /* offset to first shdr */
	else
		putl(0);
	putl(0L); /* flags */
	putw(Ehdr32sz);
	 ...
	 */
	elf32phdr(putl, PT_LOAD, HEADR, INITTEXT, INITTEXTP, textsize, textsize,
	          R | X, INITRND); /* text */
	                           /*
	...
	elf32phdr(putl, PT_LOAD, HEADR + textsize, INITDAT, phydata, datsize,
	          datsize + bsssize, R | W | X, INITRND); /* data */
	elf32phdr(putl, NOPTYPE, HEADR + textsize + datsize, 0, 0, symsize,
	          lcsize, R, 4); /* symbol table */
	if(addpsects > 0)
		putpsects(putl);
	cflush();
...
elf64sectab(void (*putl)(long), void (*putll)(vlong))
{
	...
	elf64shdr(putl, putll, Stistrtab, Strtab, 1 << 5, 0,
	          HEADR + textsize + datsize + symsize + 3 * Shdr64sz, 14, 0, 0,
	          1, 0);
	elfstrtab();
}
...
elf64(int mach, int bo, int addpsects, void (*putpsects)(Putl))
{
	...
	if(debug['S'])
		putll(HEADR + textsize + datsize +
		      symsize); /* offset to 1st shdr */
	else
		putll(0);
	putl(0L); /* flags */
	putw(Ehdr64sz);
	...
	elf64phdr(putl, putll, PT_LOAD, HEADR, INITTEXT, INITTEXTP, textsize,
	          textsize, R | X, INITRND); /* text */
	                                     /*
