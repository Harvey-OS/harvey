...
main(int argc, char* argv[])
{
	...
	Usage:
		fprint(2,
		       "usage: syscall [-ox] entry [args; buf==1MB buffer]\n");
	...
	for(i = 0; tab[i].name; i++)
		if(strcmp(tab[i].name, argv[0]) == 0) {
			/* special case for seek, pread, pwrite; vlongs are
			 * problematic */
			if(strcmp(argv[0], "seek") == 0)
				r = seek(arg[0], strtoll(argv[2], 0, 0),
				         arg[2]);
			else if(strcmp(argv[0], "pread") == 0)
				r = pread(arg[0], (void*)arg[1], arg[2],
				          strtoll(argv[4], 0, 0));
			else if(strcmp(argv[0], "pwrite") == 0)
				r = pwrite(arg[0], (void*)arg[1], arg[2],
				           strtoll(argv[4], 0, 0));
			else
				r = (*tab[i].func)(arg[0], arg[1], arg[2],
				                   arg[3], arg[4]);
			if(r == -1) {
			...
			if(sflag && r > 0) {
				...
				if(r <= BIT16SZ)
					...
					print("[%s] ", d.muid);
					print("(%.16llux %lud %.2ux) ",
					      d.qid.path, d.qid.vers,
					      d.qid.type);
