...
enum {
	...
	Rsetconf = 9,
	Rgetiface =
	    10, // set/get interface(index) + alt setting(value) (1 byte)
	Rsetiface = 11,
	Rsynchframe = 12,
...
struct Usbstringdesc0 {
	...
	uint8_t wLANGID0[2];
	//	Supported Language Code Zero. bLength indicates how many
	//wLANGIDs there are.
};
...
static int
cmdreq(int fd, int type, int req, int value, int index, uint8_t* data,
       int count)
{
	int ndata, n;
	uint8_t* wp;
...
static int
usbcmd(int fd, int type, int req, int value, int index, uint8_t* data,
       int count)
{
	int i, r, nerr;
	char err[64];
...
{
	return usbcmd(fd, Rd2h | Rstd | Rdevice, Rgetdesc, desctype << 8 | 0,
	              index, buf, len);
}
...
usbconfprint(int fd, Usbconfig* cp)
{
	...
	for(i = 0; i < cp->nendpt; i++) {
		nwr = fprint(
		    ...
		    cp->endpt[i].addr, cp->endpt[i].type, cp->endpt[i].maxpkt,
		    cp->endpt[i].pollival);
		if(nwr == -1)
			return -1;
		ntot += nwr;
	...
	if(cp->nextra > 0) {
		for(i = 0; i < cp->nextra; i++) {
			nwr = fprint(fd, "%s0x%02x%s", i % 16 == 0 ? "\t" : "",
			             cp->extra[i],
			             i < cp->nextra - 1 ? ", " : "\n");
			if(nwr == -1)
				return -1;
	...
	for(cfg = 0; cfg < ncfg; cfg++) {
		...
		ifskip:
			...
			if(nconfs == aconfs) {
				aconfs += 4;
				confs =
				    realloc(confs, aconfs * sizeof confs[0]);
			}
			cp = &confs[nconfs++];
			...
			if(cp->nendpt > nelem(cp->endpt)) {
				print("out of endpoints in usbconfig (need %d "
				      "have %d)\n",
			...
			epskip:
				...
				if(buf[off + 1] != Dendpt) {
					if(cp->nextra + buf[off] <=
					   sizeof cp->extra) {
						memmove(cp->extra + cp->nextra,
						        buf + off, buf[off]);
						cp->nextra += buf[off];
					} else {
						print("out of extra space in "
						      "usbconfig, desc type "
						      "0x%02x\n",
						      buf[off + 1]);
					}
				...
				cp->endpt[epi].addr =
				    endptdesc.bEndpointAddress;
				cp->endpt[epi].type = endptdesc.bmAttributes;
				cp->endpt[epi].maxpkt =
				    get16(endptdesc.wMaxPacketSize);
...
usbopen(int fd, Usbconfig* cp, int epi, int* ctlp)
{
	...
	if(usbcmd(fd, Rh2d | Rstd | Rdevice, Rsetconf, cp->config, 0, nil, 0) ==
	   -1) {
		fprint(2, "usbopen: Rsetconf fail\n");
		goto fail;
	}

	if(usbcmd(fd, Rh2d | Rstd | Riface, Rsetiface, cp->alt, cp->iface, nil,
	          0) == -1) {
	...
	n = snprint(buf, sizeof buf, "new %d %d %s", epaddr, eptype,
	            isread ? "r" : "w");
	...
	if(eptype == Eintr || eptype == Eiso) {
		n = snprint(buf, sizeof buf, "pollival %d",
		            cp->endpt[epi].pollival);
		if(write(epctl, buf, n) != n) {
