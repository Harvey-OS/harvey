...
sync(int connection, Fcall *rep, OpQueue *queue, Buffer *data)
{
	...
	if(linecontrol && !rawmode){
		switch(data->data[data->written-1]){
			case '\004':
				--data->written;	// skip ctrl-d
				break;
			case '\r':
				bwrite(data, &nl, 1);
				break;
...
{
	if(b->written > 0){
		if(b->data[b->written-1] == '\004')
			return;			// skip everything after a ctrl-d
		if(b->data[b->written-1] == '\r'){
			b->data[b->written-1] = '\n';
			if(c == '\n')
				return;
		}
...
fsserve(int connection, char *owner)
{
	...
	do
	{
		...
		if((status == Unmounted || ISCLOSED(inputfid)) && (w = qclose(connection, &rep, &consreads)) <= 0){
			...
			if((w = sync(connection, &rep, &consreads, input)) <= 0){
