...
keybscan(uint8_t code, uint8_t *out, int len)
{
	...
	if(len < 16){
		debug("keybscan: out buffer too short to be safe\n");
		return -1;
	}
...
readkeyboard(int src, int consreads, int conswrites)
{
	...
	do
	{
		...
		for(i = 0; i < r; ++i){
			...
			if(scan == -1){
				debug("readkeyboard %d: keybscan(0x%x) returns -1, interrupt %s\n", pid, ibuf[i], argv0);
				if(postnote(PNGROUP, getpid(), "keybscan") == -1)
					debug("readkeyboard %d: postnote(PNGROUP, getpid(), 'keybscan') returns -1, %r\n", pid);
				exits("keybscan");
			} else {
				produced += scan;
			}
