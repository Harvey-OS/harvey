...
struct Buffer
{
	...
	uint32_t written;
	uint32_t read;
	uint32_t ctrld;		/* index of first byte after a ^D (input only) */
	int newlines;		/* number of newlines (input only) */
	int linewidth;
...
extern char *breadln(Buffer *b, uint32_t *length);
extern int blineready(Buffer *);
#define bempty(b) (b->read == b->written && b->ctrld == b->size)
