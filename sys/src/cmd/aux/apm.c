...
static char* error[256] = {
        ...
        [0x09] "unrecognized device id", [0x0A] "parameter value out of range",
        [0x0B] "interface not engaged", [0x0C] "function not supported",
        [0x0D] "resume timer disabled",
        [0x60] "unable to enter requested state",
        [0x80] "no power mgmt events pending", [0x86] "apm not present",
};
...
_apmcall(int fd, Ureg* u)
{
	...
	if(apmdebug)
		fprint(2, "flags 0x%lux ax 0x%lux bx 0x%lux cx 0x%lux\n",
		       u->flags & 0xFFFF, u->ax & 0xFFFF, u->bx & 0xFFFF,
		       u->cx & 0xFFFF);
...
static char* acstatusstr[] = {
        [AcUnknown] "unknown", [AcOffline] "offline", [AcOnline] "online",
        [AcBackup] "backup",
};

static char* batterystatusstr[] = {
        [BatteryUnknown] "unknown",   [BatteryHigh] "high",
        [BatteryLow] "low",           [BatteryCritical] "critical",
        [BatteryCharging] "charging",
};

static char* powerstatestr[] = {
        [PowerOff] "off", [PowerSuspend] "suspend", [PowerStandby] "standby",
        [PowerEnabled] "on",
};
...
powerprint(void)
{
	...
	if(apmgetpowerstatus(&apm, DevAll) == 0) {
		...
		print("battery 0: status %s percent %d time %d:%.2d\n",
		      batterystatus(apm.battery[0].status),
		      apm.battery[0].percent, apm.battery[0].time / 60,
		      apm.battery[0].time % 60);
	}
}
...
iscmd(char* p, char* cmd)
{
	...
	l = strlen(cmd);
	return strncmp(p, cmd, l) == 0 && p[l] == '\0' || p[l] == ' ' ||
	       p[l] == '\t';
}
...
eventthread(void* v)
{
	...
	for(;;) {
		while((e = recvul(cevent)) >= 0) {
			snprint(eventq[wp], sizeof(eventq[wp]) - 1, "%s",
			        apmevent(e));
			strcat(eventq[wp], "\n");
			wp++;
			if(wp == nelem(eventq))
...
usage(void)
{
	fprint(2, "usage: aux/apm [-ADPi] [-d /dev/apm] [-m /mnt/apm] [-s "
	          "service]\n");
	exits("usage");
}
