...
numfmt(int fmt, int min, int max, double num)
{
	...
	if(fmt == 9)
		...
		tm = bifftime(num);
		snprint(buf, sizeof(buf), "%d-%s-%d", tm->mday, Months[tm->mon],
		        tm->year + 1900);
	} else if((fmt >= 18 && fmt <= 21) || (fmt >= 45 && fmt <= 47)) {
		tm = bifftime(num);
		snprint(buf, sizeof(buf), "%02d:%02d:%02d", tm->hour, tm->min,
		        tm->sec);

	} else if(fmt == 22) {
		tm = bifftime(num);
		snprint(buf, sizeof(buf), "%02d:%02d:%02d %d-%s-%d", tm->hour,
		        tm->min, tm->sec, tm->mday, Months[tm->mon],
		        tm->year + 1900);
...
dump(void)
{
	...
	for(r = Root; r; r = r->next) {
		...
		for(c = r->col; c; c = c->next) {
			...
			if(Trunc && min > 2)
				max = min -
				      2; // FIXME: -2 because of bug %q format ?
			...
			case Tlabel:
				...
				break;
			case Tbool:
				Bprint(bo, strfmt, min, max,
				       (c->bool) ? "True" : "False");
				break;
			case Tindex:
				if(c->index < 0 || c->index >= Nstrtab)
					sysfatal("SST string out of range - "
					         "corrupt file?");
			...
			if(!last) {
				if(c->next->c == c->c) // bar charts
					...
					Bprint(bo, "%s", Delim);
					for(i = c->c;
					    c->next && i < c->next->c - 1; i++)
						Bprint(bo, "%-*.*s%s", min, max,
						       "", Delim);
				}
			}
		}
...
gstr(Biff* b, int len_width)
{
	...
	if(b->len < len_width) {
		...
		if(b->op != 0x03c)
			sysfatal(
			    "starting STRING expected CONTINUE, got op=0x%x",
			    b->op);
	}
...
eof(Biff* b)
{
	...
	struct {
	    ...
	    0x005, "Workbook globals", 0x006, "Visual Basic module", 0x010,
	    "Worksheet", 0x020, "Chart", 0x040, "Macro sheet", 0x100,
	    "Workspace file",
	};
	static int sheet = 0;
...
xls2csv(Biobuf* bp)
{
	...
	struct {
	    ...
	    0x000a, eof,         0x0022, datemode,    0x0042, codepage,
	    0x0055, defcolwidth, 0x005c, writeaccess, 0x007d, colinfo,
	    0x00bd, mulrk,       0x00fc, sst,         0x00fd, labelsst,
	    0x0203, number,      0x0204, label,       0x0205, boolerr,
	    0x027e, rk,          0x0809, bof,         0x00e0, xf,
	};
...
usage(void)
{
	fprint(2, "usage: %s [-Danqt] [-w worksheets] [-c columns] [-d delim] "
	          "/mnt/doc/Workbook\n",
	       argv0);
	exits("usage");
}
