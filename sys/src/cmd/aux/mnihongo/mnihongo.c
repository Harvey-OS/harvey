...
main(void)
{
	...
	while((c = Bgetc(&bin)) >= 0) {
		...
		case 'c': /* single character */
			c = Bgetrune(&bin);
			if(c ==
			   ' ') /* why does this happen? it's troff - bwk */
				break;
		...
		case 'x': /* device control */
			Bgetline(&bin, str);
			Bprint(&bout, "%c%s", c, str);
			if(tokenize(str, args, 10) > 2 && args[0][0] == 'f' &&
			   ('0' <= args[1][0] && args[1][0] <= '9')) {
...
{                  /* dump as kanji char if looks like one */
	...
	Bprint(&bout,
	       "x X PS currentpoint translate %d %d scale ptsize dup scale\n",
	       size, size);
	Bprint(&bout, "x X PS %d %d true [%d 0 0 -%d 0 %d]\n", wid, ht, wid,
	       wid, ht - 2); /* kludge; ought to use ->ascent */
...
pschar(char* s, char* hex, int* wid, int* ht)
{
	...
	*wid = ((chpt.x + 7) / 8) * 8;
	*ht = chpt.y;
	/* postscript is backwards to video, so draw white (ones) on black
	 * (zeros) */
	b = allocimage(display, Rpt(ZP, chpt), GREY1, 0,
	               DBlack); /* place to put it */
	spt =
	    string(b, Pt(0, 0), display->white, ZP, font, s); /* put it there */
	/* Bprint(&bout, "chpt %P, spt %P, wid,ht %d,%d\n", chpt, spt, *wid,
	 * *ht); */
	/* Bflush(&bout); */
	for(y = 0; y < chpt.y; y++) { /* read bits a row at a time */
		memset(rowdata, 0, sizeof rowdata);
		unloadimage(b, Rect(0, y, chpt.x, y + 1), rowdata,
		            sizeof rowdata);
