...
enum { Fs,
       ...
       HZAvgSecs =
	   3 * 60, /* target averaging period for frequency in seconds */
...
enum { NTPSIZE = 48,       /* basic ntp packet */
...
static int gettime(int64_t* nsec, uint64_t* ticks,
                   uint64_t* hz); /* returns time, ticks, hz */
...
static void settime(int64_t now, uint64_t hz, int64_t delta,
                    int n); /* set time, hz, delta, period */
...
main(int argc, char** argv)
{
	...
	case 'L':
		...
		tg.yday -= tl.yday;
		gmtdelta =
		    tg.sec + 60 * (tg.min + 60 * (tg.hour + tg.yday * 24));
	...
	for(;; background(), sleep(tsecs * 1000)) {
		...
		if(diff > 10 * SEC || diff < -10 * SEC) {
			 ...
			 *
			 * any difference greater than etha we work off during
			 *the
			 * sampling period.
			 */
			if(abs(diff) > etha)
				if(diff > 0)
					settime(-1, 0, diff - ((3 * etha) / 4),
					        secs);
				else
					settime(-1, 0, diff + ((3 * etha) / 4),
					        secs);
		...
		for(x = first; x != nil; x = x->next)
			if(s->stime - x->stime < period || x->next == nil ||
			   s->stime - x->next->stime < period)
				break;
		if(x != nil) {
			nhz = whatisthefrequencykenneth(
			    hz, minhz, maxhz, s->stime - x->stime,
			    s->ticks - x->ticks, period);
			tsecs = caperror(vabs(nhz - hz), tsecs, taccuracy);
			hz = nhz;
			writefreqfile(fd, hz, (s->stime - x->stime) / SEC,
			              diff);
		}
		...
		*l = s;
		...
		if(logging)
			syslog(0, logfile, "δ %lld avgδ %lld hz %lld", diff,
			       avgerr, hz);
	}
}
...
settime(int64_t now, uint64_t hz, int64_t delta, int n)
{
	...
	if(debug)
		fprint(2,
		       "settime(now=%lld, hz=%llud, delta=%lld, period=%d)\n",
		       now, hz, delta, n);
	if(impotent)
		return;
	switch(ifc) {
...
ntpsample(void)
{
	...
	for(tns = ntpservers; tns != nil; tns = tns->next) {
		if(ntptimediff(tns) < 0)
			continue;
		x = vabs(tns->rootdisp) +
		    (vabs(tns->rtt + tns->rootdelay) >> 1);
		if(debug)
			fprint(
			    2,
			    "ntp %s rootdelay %lld rootdisp %lld metric %lld\n",
...
ntpserver(char* servenet)
{
	...
	if(Rootid != nil)
		memmove(rootid, Rootid,
		        strlen(Rootid) > 4 ? 4 : strlen(Rootid));
