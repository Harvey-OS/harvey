...
fsopen(Req *r)
{
fprint(2, "fsopen path %d\n", r->fid->qid.path);
fprint(2, "opencount %d\n", opencount);
	if (r->fid->qid.path == Qcons){
...
cpuproc(void *data)
{
	...
	while(r = recvp(reqchan)){
		if (chatty9p) fprint(logfd, "cpuproc: got req\n");
		if(flushed(r)){
			respond(r, Eintr);
			continue;
		}
		...
		if (opval == (Tread<<8) | Qcons) {
			...
			fprint(2, "cpuproc: bad op\n");
		}
	}
}


// The trick with dispatch: this ensures we don't block.
// if the channel is full, we return immediately. If not,
// we pass it on to the cpuproc and it responds, leaving
// us free to handle other requests. The downside: nconsole
// is 2 procs, not one. FIXME
static void
dispatch(Req *r)
{
...
threadmain(int argc, char *argv[])
{
	...
	// This is a hack as a test. The lib9p needs to support ORCLOSE
	decref(Cons);
	decref(Cons);
	Qcons = Cons->qid.path;
...
fprint(2, "setup Q %d %d %d\n", Qroot, Qcons, Qconsctl);
	...
	// We want the server and stdin to share FDs so the server can close stdin when all users of cons
	// finish.
