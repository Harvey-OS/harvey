...
fprint(2, "attach r %p", r);
	spec = r->ifcall.aname;
fprint(2, "attach spec %s", r->ifcall.aname);
	if(spec && spec[0]){
		respond(r, Ebadspec);
		return;
	}
...
fprint(2, "kbscan: code 0x%x out %p len %d\n", code, out, len);
	c = code;
	off = 0;
	if(len < 16){
...
threadmain(int argc, char *argv[])
{
	...
	if (! testmode)
		...
		remove("/srv/test-nconsole");
		threadpostmountsrv(&fs, "test-nconsole", nil, MBEFORE);
	}
...
fprint(2, "psfd reads %d\n", len);
			for(i = 0; i < len; i++){
fprint(2, " %d\n", i);
fprint(2, " ibuf[i] 0x%x buf %p buf + off %p sizeof buf - off %d\n", ibuf[i], buf, buf+off, sizeof buf - off);
				if((n = keybscan(ibuf[i], buf+off, sizeof buf - off)) == -1){
					//fprint(2, "keybscan -1\n");
					break;
				}
fprint(2, "n is %d so off will be %d\n", n, off + n);
				off += n;
			}
fprint(2, "off is now %d\n", off);
			if(off > 0){
fprint(2, "write %d, %p, %d\n", tochld[0], buf, off);
				write(tochld[0], buf, off);
				off = 0;
			}
		}
fprint(2, "psfd read returned <= 0\n");
