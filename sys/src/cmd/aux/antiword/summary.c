 ...
 */
static UCHAR*
pucAnalyseSummaryInfoHeader(FILE* pFile, ULONG ulStartBlock, ULONG ulSize,
                            ...
                            const ULONG* aulSBD, size_t tSBDLen)
{
	...
	if(!bReadBuffer(pFile, ulStartBlock, aulBlockDepot, tBlockDepotLen,
	                tBlockSize, aucHdr, 0, P_HEADER_SZ)) {
		return NULL;
	}
	...
	if(!bReadBuffer(pFile, ulStartBlock, aulBlockDepot, tBlockDepotLen,
	                tBlockSize, aucSecLst, P_HEADER_SZ,
	...
	if(!bReadBuffer(pFile, ulStartBlock, aulBlockDepot, tBlockDepotLen,
	                tBlockSize, aucBuffer, ulOffset, tLength)) {
...
vSet2SummaryInfo(FILE* pFile, int iWordVersion, const UCHAR* aucHeader)
{
	...
	if(usLid < 999 && iWordVersion == 1) {
		...
		case 1:
			usLid = 0x0409;
			break; /* American English */
		case 2:
			usLid = 0x0c0c;
			break; /* Canadian French */
		case 31:
			usLid = 0x0413;
			break; /* Dutch */
		case 33:
	...
	for(tCounter = 0; tCounter < 17; tCounter++) {
		...
		if(tLen != 0) {
			...
			case 3:
				...
				strncpy(szTitle, (char*)aucBuffer + tStart + 1,
				        tLen);
				szTitle[tLen] = '\0';
				break;
			case 4:
			...
			case 7:
				...
				strncpy(szAuthor, (char*)aucBuffer + tStart + 1,
				        tLen);
				szAuthor[tLen] = '\0';
				break;
			default:
 ...
 */
static void
vSetSummaryInfoOLE(FILE* pFile, const pps_info_type* pPPS, const ULONG* aulBBD,
                   size_t tBBDLen, const ULONG* aulSBD, size_t tSBDLen)
{
	...
	/* Summary Information */
	pucBuffer = pucAnalyseSummaryInfoHeader(
	    pFile, pPPS->tSummaryInfo.ulSB, pPPS->tSummaryInfo.ulSize, aulBBD,
	    tBBDLen, aulSBD, tSBDLen);
	if(pucBuffer != NULL) {
	...
	pucBuffer = pucAnalyseSummaryInfoHeader(
	    pFile, pPPS->tDocSummaryInfo.ulSB, pPPS->tDocSummaryInfo.ulSize,
 ...
 */
void
vSet6SummaryInfo(FILE* pFile, const pps_info_type* pPPS, const ULONG* aulBBD,
                 size_t tBBDLen, const ULONG* aulSBD, size_t tSBDLen,
 ...
 */
void
vSet8SummaryInfo(FILE* pFile, const pps_info_type* pPPS, const ULONG* aulBBD,
                 size_t tBBDLen, const ULONG* aulSBD, size_t tSBDLen,
                 const UCHAR* aucHeader)
{
	USHORT usTmp;
...
szGetLastSaveDtm(void)
{
	...
	sprintf(szTime, "%04d-%02d-%02d", pTime->tm_year + 1900,
...
szGetModDate(void)
{
	...
	sprintf(szTime, "D:%04d%02d%02d%02d%02d", pTime->tm_year + 1900,
...
szGetLanguage(void)
{
	...
	case 0x0c04:
		return "zh_HK"; /* Hong Kong Chinese */
	case 0x1004:
		return "zh_SG"; /* Singapore Chinese */
	case 0x0807:
		return "de_CH"; /* Swiss German */
	case 0x0409:
		return "en_US"; /* American English */
	case 0x0809:
		return "en_GB"; /* British English */
	case 0x0c09:
		return "en_AU"; /* Australian English */
	case 0x080a:
		return "es_MX"; /* Mexican Spanish */
	case 0x080c:
		return "fr_BE"; /* Belgian French */
	case 0x0c0c:
		return "fr_CA"; /* Canadian French */
	case 0x100c:
		return "fr_CH"; /* Swiss French */
	case 0x0810:
		return "it_CH"; /* Swiss Italian */
	case 0x0813:
		return "nl_BE"; /* Belgian Dutch */
	case 0x0416:
