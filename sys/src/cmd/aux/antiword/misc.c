...
lGetFilesize(const char* szFilename)
{
	...
	e = SWI(2, 5, SWI_OS_File | XOS_Bit, 17, szFilename, &iType, NULL, NULL,
	        NULL, &iSize);
	if(e != NULL) {
		werr(0, "Get Filesize error %d: %s", e->errnum, e->errmess);
		return -1;
	}
	if(iType != 1) {
...
#if defined(DEBUG)
void
vPrintBlock(const char* szFile, int iLine, const UCHAR* aucBlock,
            size_t tLength)
{
	int i, j;
 ...
 */
BOOL
bReadBuffer(FILE* pFile, ULONG ulStartBlock, const ULONG* aulBlockDepot,
            size_t tBlockDepotLen, size_t tBlockSize, UCHAR* aucBuffer,
            ULONG ulOffset, size_t tToRead)
{
	...
	for(ulIndex = ulStartBlock; ulIndex != END_OF_CHAIN && tToRead != 0;
...
pSplitList(output_type* pAnchor)
{
	...
	(void)strncpy(pLeftOver->szStorage, pCurr->szStorage + iIndex + 1,
	              pLeftOver->tNextFree);
	...
	pLeftOver->lStringWidth =
	    lComputeStringWidth(pLeftOver->szStorage, pLeftOver->tNextFree,
	                        pLeftOver->tFontRef, pLeftOver->usFontSize);
	...
	pCurr->lStringWidth =
	    lComputeStringWidth(pCurr->szStorage, pCurr->tNextFree,
	                        pCurr->tFontRef, pCurr->usFontSize);
	pCurr->pNext = NULL;
...
unincpy(char* s1, const UCHAR* s2, size_t n)
{
	...
	for(pcDest = s1, tLen = 0; tLen < n; pcDest++, tLen++) {
		...
		ulChar = ulTranslateCharacters(usUni, 0, 8, conversion_unknown,
		                               encoding_neutral, FALSE);
                ...
                char* szResult, size_t tMaxResultLen)
{
	...
	if(eEncoding == encoding_utf_8) {
		...
		szResult[0] =
		    (char)ucGetBulletCharacter(eConversionType, eEncoding);
		szResult[1] = '\0';
	}
