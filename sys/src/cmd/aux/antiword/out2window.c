...
vString2Diagram(diagram_type* pDiag, output_type* pAnchor)
{
	...
	for(pOutput = pAnchor; pOutput != NULL; pOutput = pOutput->pNext) {
		...
		vSubstring2Diagram(
		    pDiag, pOutput->szStorage, pOutput->tNextFree, lWidth,
		    pOutput->ucFontColor, pOutput->usFontStyle,
		    pOutput->tFontRef, pOutput->usFontSize, usMaxFontSize);
	}
...
lComputeNetWidth(output_type* pAnchor)
{
	...
	while(pTmp->tNextFree != 0 &&
		...
		pTmp->lStringWidth =
		    lComputeStringWidth(pTmp->szStorage, pTmp->tNextFree,
		                        pTmp->tFontRef, pTmp->usFontSize);
		NO_DBG_DEC(pTmp->lStringWidth);
	}
 ...
 */
void
vAlign2Window(diagram_type* pDiag, output_type* pAnchor, int32_t lScreenWidth,
              UCHAR ucAlignment)
{
 ...
 */
void
vJustify2Window(diagram_type* pDiag, output_type* pAnchor, int32_t lScreenWidth,
                int32_t lRightIndentation, UCHAR ucAlignment)
{
	output_type* pTmp;
	...
	lSpaceWidth =
	    lComputeStringWidth(" ", 1, pAnchor->tFontRef, pAnchor->usFontSize);
	lToAdd = lScreenWidth - lNetWidth -
	         lDrawUnits2MilliPoints(pDiag->lXleft) + lRightIndentation;
#if defined(DEBUG)
              ...
              const section_block_type* pSection)
{
		...
		if(tIndex == tStyleIndex ||
		   (bNeedPrevLvl && tIndex < tStyleIndex)) {
			...
			case LIST_ORDINAL_TXT:
				pcTxt +=
				    sprintf(pcTxt, "%u", auiHdrCounter[tIndex]);
				break;
			case LIST_UPPER_ROMAN:
			case LIST_LOWER_ROMAN:
				pcTxt += tNumber2Roman(
				    auiHdrCounter[tIndex],
				    ucNFC == LIST_UPPER_ROMAN, pcTxt);
				break;
			case LIST_UPPER_ALPHA:
			case LIST_LOWER_ALPHA:
				pcTxt += tNumber2Alpha(
				    auiHdrCounter[tIndex],
				    ucNFC == LIST_UPPER_ALPHA, pcTxt);
				break;
	...
	*pcTxt = '\0';
	NO_DBG_MSG_C((int)pStyle->usIstd >= 1 && (int)pStyle->usIstd <= 9 &&
 ...
 */
static size_t
tGetBreakingPoint(const char* szString, size_t tLen, size_t tWidth,
...
tComputeColumnWidthMax(int16_t sWidth, int32_t lCharWidth, double dFactor)
{
	...
	tColumnWidthMax =
	    (size_t)((lTwips2MilliPoints(sWidth) * dFactor + lCharWidth / 2.0) /
	             lCharWidth);
                 ...
                 conversion_type eConversionType, int iParagraphBreak)
{
	...
	/* Character sizes */
	lCharWidthLarge =
	    lComputeStringWidth("W", 1, pOutput->tFontRef, pOutput->usFontSize);
	NO_DBG_DEC(lCharWidthLarge);
	lCharWidthSmall =
	    lComputeStringWidth("i", 1, pOutput->tFontRef, pOutput->usFontSize);
	...
	for(iNbrOfColumns = 1; iNbrOfColumns < TABLE_COLUMN_MAX;
	    iNbrOfColumns++) {
		aszColTxt[iNbrOfColumns] =
		    strchr(aszColTxt[iNbrOfColumns - 1], TABLE_SEPARATOR);
	...
	for(iIndex = 0; iIndex < iNbrOfColumns; iIndex++) {
		atColumnWidthMax[iIndex] = tComputeColumnWidthMax(
		    pRowInfo->asColumnWidth[iIndex], lCharWidthLarge, dMagnify);
	...
	do {
		...
		*pcTxt++ = TABLE_SEPARATOR_CHAR;
		for(iIndex = 0; iIndex < iNbrOfColumns; iIndex++) {
			...
			if(aszColTxt[iIndex] == NULL) {
				/* Add an empty column */
				for(iTmp = 0; iTmp < (int)tColumnWidthMax;
				    iTmp++) {
			...
			tLen = tComputeStringLengthMax(aszColTxt[iIndex],
			                               tColumnWidthMax);
			NO_DBG_DEC(tLen);
			while(tLen != 0 &&
			...
			fail(tWidth > tColumnWidthMax);
			tLen = tGetBreakingPoint(aszColTxt[iIndex], tLen,
			                         tWidth, tColumnWidthMax);
			tWidth = tCountColumns(aszColTxt[iIndex], tLen);
			if(tLen == 0 && *aszColTxt[iIndex] == '\0') {
				...
				/* Add the text */
				pcTxt += sprintf(pcTxt, "%.*s", (int)tLen,
				                 aszColTxt[iIndex]);
			...
			/* Fill up the rest */
			for(iTmp = 0; iTmp < (int)tColumnWidthMax - (int)tWidth;
			    iTmp++) {
				*pcTxt++ = (char)FILLER_CHAR;
			}
			/* End of column */
		...
		*pcTxt = '\0';
		...
		tRow.lStringWidth =
		    lComputeStringWidth(tRow.szStorage, tRow.tNextFree,
		                        tRow.tFontRef, tRow.usFontSize);
