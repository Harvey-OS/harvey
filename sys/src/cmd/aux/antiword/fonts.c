...
szGetDefaultFont(UCHAR ucFFN, int iEmphasis)
{
	...
	if(ucPrq == PITCH_FIXED) {
		...
		switch(iEmphasis) {
		case 1:
			return FONT_MONOSPACED_BOLD;
		case 2:
			return FONT_MONOSPACED_ITALIC;
		case 3:
			return FONT_MONOSPACED_BOLDITALIC;
		default:
			return FONT_MONOSPACED_PLAIN;
		}
		...
		case 3:
			return FONT_SERIF_BOLDITALIC;
		default:
			return FONT_SERIF_PLAIN;
		}
	} else if(ucFf == FAMILY_SWISS) {
		/* Set to the default sans serif font */
		switch(iEmphasis) {
		case 1:
			return FONT_SANS_SERIF_BOLD;
		case 2:
			return FONT_SANS_SERIF_ITALIC;
		case 3:
			return FONT_SANS_SERIF_BOLDITALIC;
		default:
			return FONT_SANS_SERIF_PLAIN;
		}
	} else {
...
bFontEqual(const UCHAR* aucWord, const char* szTable, int iBytesPerChar)
{
	...
	for(pucTmp = aucWord, pcTmp = szTable; *pucTmp != 0;
                ...
                font_table_type* pFontTableRecord)
{
	...
	if(!bMatchFound && pFontTableRecord->szWordFontname[0] == '\0' &&
	   szWordFont[0] == '*' && szWordFont[1] == '\0') {
		/*
	...
	if(bMatchFound) {
		...
		case 1:
			(void)strncpy(pFontTableRecord->szWordFontname,
			              (const char*)aucFont,
			              sizeof(pFontTableRecord->szWordFontname) -
			                  1);
			break;
		case 2:
			(void)unincpy(pFontTableRecord->szWordFontname, aucFont,
			              sizeof(pFontTableRecord->szWordFontname) -
			                  1);
			break;
		default:
		...
		pFontTableRecord
		    ->szWordFontname[sizeof(pFontTableRecord->szWordFontname) -
		                     1] = '\0';
		(void)strncpy(pFontTableRecord->szOurFontname, szOurFont,
		              sizeof(pFontTableRecord->szOurFontname) - 1);
		pFontTableRecord
		    ->szOurFontname[sizeof(pFontTableRecord->szOurFontname) -
		                    1] = '\0';
...
vCreateFontTable(void)
{
	...
	for(iNbr = 0, pTmp = pFontTable; pTmp < pFontTable + tFontTableRecords;
	    iNbr++, pTmp++) {
 ...
 */
static BOOL
bReadFontFile(FILE* pFontTableFile, char* szWordFont, int* piItalic,
              int* piBold, char* szOurFont, int* piSpecial)
{
	char* pcTmp;
	int iFields;
	...
	while(fgets(szLine, (int)sizeof(szLine), pFontTableFile) != NULL) {
		...
		iFields = sscanf(szLine, "%[^,],%d,%d,%1s%[^,],%d", szWordFont,
		                 piItalic, piBold, &szOurFont[0], &szOurFont[1],
		                 piSpecial);
		if(iFields != 6) {
		...
		if(strlen(szWordFont) >= sizeof(pFontTable[0].szWordFontname)) {
			werr(0, "Word fontname too long: '%s'", szWordFont);
			continue;
		}
		if(strlen(szOurFont) >= sizeof(pFontTable[0].szOurFontname)) {
...
vCreate0FontTable(void)
{
	...
	iBold = 0;
	iSpecial = 0;
	while(bReadFontFile(pFontTableFile, szWordFont, &iItalic, &iBold,
	                    szOurFont, &iSpecial)) {
		iEmphasis = 0;
		if(iBold != 0) {
			iEmphasis++;
		...
		for(iFtc = 0, pTmp = pFontTable + iEmphasis;
		    pTmp < pFontTable + tFontTableRecords; iFtc++, pTmp += 4) {
			if(iFtc >= 16 && iFtc <= 55) {
			...
			ucFFN = (ucFf << 4) | ucPrq;
			vFontname2Table(aucFont, NULL, 1, iEmphasis, ucFFN,
			                szWordFont, szOurFont, pTmp);
		}
	}
 ...
 */
void
vCreate6FontTable(FILE* pFile, ULONG ulStartBlock, const ULONG* aulBBD,
 ...
 */
void
vCreate8FontTable(FILE* pFile, const pps_info_type* pPPS, const ULONG* aulBBD,
                  ...
                  const UCHAR* aucHeader)
{
	...
	if(!bReadBuffer(pFile, pPPS->tTable.ulSB, aulBlockDepot, tBlockDepotLen,
	                tBlockSize, aucBuffer, ulBeginFontInfo, tFontInfoLen)) {
		aucBuffer = xfree(aucBuffer);
		(void)fclose(pFontTableFile);
		return;
	}
...
vCorrectFontTable(conversion_type eConversionType, encoding_type eEncoding)
{
	...
	if(eConversionType == conversion_ps && eEncoding == encoding_cyrillic) {
		vCorrect4CyrPS();
	}
#endif /* __riscos */
