...
iGet8InfoLength(int iByteNbr, const UCHAR* aucGrpprl)
{
	...
	case 0xc000:
		...
		if(usOpCode == 0xc615 && iTmp == 255) {
			...
			iAdd =
			    (int)ucGetByte(iByteNbr + 4 + iDel * 4, aucGrpprl);
 ...
 */
static UCHAR*
aucFillInfoBuffer(FILE* pFile, const pps_type* pTable, const ULONG* aulBBD,
                  ...
                  ULONG ulBeginInfo, size_t tInfoLen)
{
	...
	if(!bReadBuffer(pFile, pTable->ulSB, aulBlockDepot, tBlockDepotLen,
	                tBlockSize, aucBuffer, ulBeginInfo, tInfoLen)) {
 ...
 */
void
vGet8DopInfo(FILE* pFile, const pps_type* pTable, const ULONG* aulBBD,
             size_t tBBDLen, const ULONG* aulSBD, size_t tSBDLen,
             const UCHAR* aucHeader)
{
	...
	aucBuffer = aucFillInfoBuffer(pFile, pTable, aulBBD, tBBDLen, aulSBD,
	                              tSBDLen, ulBeginDocpInfo, tDocpInfoLen);
	if(aucBuffer == NULL) {
		return;
	}
 ...
 */
void
vGet8SepInfo(FILE* pFile, const pps_info_type* pPPS, const ULONG* aulBBD,
             size_t tBBDLen, const ULONG* aulSBD, size_t tSBDLen,
             const UCHAR* aucHeader)
{
	...
	aucBuffer =
	    aucFillInfoBuffer(pFile, &pPPS->tTable, aulBBD, tBBDLen, aulSBD,
	                      tSBDLen, ulBeginSectInfo, tSectInfoLen);
	if(aucBuffer == NULL) {
		return;
	}
	...
	for(tIndex = 0, tOffset = (tLen + 1) * 4; tIndex < tLen;
	    tIndex++, tOffset += 12) {
	...
	for(tIndex = 0; tIndex < tLen; tIndex++) {
		...
		if(!bReadBuffer(pFile, pPPS->tWordDocument.ulSB, aulBBD,
		                tBBDLen, BIG_BLOCK_SIZE, aucTmp,
		                aulSectPage[tIndex], 2)) {
			continue;
		}
		...
		if(!bReadBuffer(pFile, pPPS->tWordDocument.ulSB, aulBBD,
		                tBBDLen, BIG_BLOCK_SIZE, aucFpage,
		                aulSectPage[tIndex], tBytes)) {
 ...
 */
void
vGet8HdrFtrInfo(FILE* pFile, const pps_type* pTable, const ULONG* aulBBD,
                size_t tBBDLen, const ULONG* aulSBD, size_t tSBDLen,
                const UCHAR* aucHeader)
{
	ULONG* aulCharPos;
	...
	aucBuffer =
	    aucFillInfoBuffer(pFile, pTable, aulBBD, tBBDLen, aulSBD, tSBDLen,
 ...
 */
row_info_enum
eGet8RowInfo(int iFodo, const UCHAR* aucGrpprl, int iBytes,
             row_block_type* pRow)
{
	...
	while(iBytes >= iFodoOff + 2) {
		...
		case 0xd608: /* cDefTable */
			...
			if(iCol < 1 || iBytes < iFodoOff + 4 + (iCol + 1) * 2) {
				DBG_DEC(iCol);
				DBG_DEC(iFodoOff);
				iInfoLen = 2;
				break;
			...
			iPosPrev = (int)(int16_t)usGetWord(iFodo + iFodoOff + 5,
			                                   aucGrpprl);
 ...
 */
void
vGet8StyleInfo(int iFodo, const UCHAR* aucGrpprl, int iBytes,
               style_block_type* pStyle)
{
	...
	while(iBytes >= iFodoOff + 2) {
		...
		case 0x4610: /* Nest dxaLeft */
			sTmp =
			    (int16_t)usGetWord(iFodo + iFodoOff + 2, aucGrpprl);
		...
		case 0xc615: /* ChgTabs */
			...
			NO_DBG_DEC(iDel);
			iAdd = (int)ucGetByte(iFodo + iFodoOff + 4 + 2 * iDel,
			                      aucGrpprl);
		...
		case 0xc63e: /* anld */
			...
			if(iTmp >= 1) {
				tList6.ucNFC =
				    ucGetByte(iFodo + iFodoOff + 3, aucGrpprl);
			}
			if(tList6.ucNFC != LIST_BULLETS && iTmp >= 2) {
				iBefore = (int)ucGetByte(iFodo + iFodoOff + 4,
				                         aucGrpprl);
			} else {
				iBefore = 0;
			}
			...
			if(iTmp >= iBefore + 22) {
				tList6.usListChar = usGetWord(
				    iFodo + iFodoOff + iBefore + 23, aucGrpprl);
 ...
 */
void
vGet8LstInfo(FILE* pFile, const pps_info_type* pPPS, const ULONG* aulBBD,
             size_t tBBDLen, const ULONG* aulSBD, size_t tSBDLen,
             const UCHAR* aucHeader)
{
	...
	if(!bReadBuffer(pFile, pPPS->tTable.ulSB, aulBlockDepot, tBlockDepotLen,
	                tBlockSize, aucLfoInfo, ulBeginLfoInfo, tLfoInfoLen)) {
	...
	if(!bReadBuffer(pFile, pPPS->tTable.ulSB, aulBlockDepot, tBlockDepotLen,
	                tBlockSize, aucLstfInfo, ulBeginLstfInfo,
	                tLstfInfoLen)) {
	...
	for(tIndex = 0, tStart = 2; tIndex < tLstfRecords;
	    tIndex++, tStart += 28) {
		...
		for(ucListLevel = 0; ucListLevel < ucMaxLevel; ucListLevel++) {
			...
			usIstd = usGetWord(tStart + 8 + 2 * (size_t)ucListLevel,
			                   aucLstfInfo);
			DBG_DEC_C(usIstd != STI_NIL, usIstd);
			NO_DBG_HEX(ulStart);
			(void)memset(&tList, 0, sizeof(tList));
			/* Read the lvlf (List leVeL on File) */
			if(!bReadBuffer(pFile, pPPS->tTable.ulSB, aulBlockDepot,
			                tBlockDepotLen, tBlockSize, aucLvlfInfo,
			...
			if(!bReadBuffer(pFile, pPPS->tTable.ulSB, aulBlockDepot,
			                tBlockDepotLen, tBlockSize, aucXst,
			...
			if(tXstLen == 0) {
				...
				vAdd2ListInfoList(ulListID, usIstd, ucListLevel,
				                  &tList);
				continue;
			}
			...
			/* Read the XString */
			if(!bReadBuffer(pFile, pPPS->tTable.ulSB, aulBlockDepot,
			                tBlockDepotLen, tBlockSize, aucXString,
			                ulStart, tXstLen)) {
 ...
 */
void
vGet8PapInfo(FILE* pFile, const pps_info_type* pPPS, const ULONG* aulBBD,
             size_t tBBDLen, const ULONG* aulSBD, size_t tSBDLen,
             const UCHAR* aucHeader)
{
	row_block_type tRow;
	...
	aucBuffer =
	    aucFillInfoBuffer(pFile, &pPPS->tTable, aulBBD, tBBDLen, aulSBD,
	                      tSBDLen, ulBeginParfInfo, tParfInfoLen);
	if(aucBuffer == NULL) {
		return;
	}
	...
	for(iIndex = 0, tOffset = (tLen + 1) * 4; iIndex < (int)tLen;
	    iIndex++, tOffset += 4) {
	...
	for(iIndex = 0; iIndex < (int)tLen; iIndex++) {
		...
		for(iIndex2 = 0; iIndex2 < iRun; iIndex2++) {
			...
			iFodo =
			    2 * (int)ucGetByte((iRun + 1) * 4 + iIndex2 * 13,
			                       aucFpage);
			if(iFodo <= 0) {
				continue;
			}
			...
			tStyle.ulFileOffset =
			    ulCharPos2FileOffsetX(ulCharPos, &tStyle.eListID);
			vAdd2StyleInfoList(&tStyle);

			eRowInfo =
			    eGet8RowInfo(iFodo, aucFpage + 3, iLen - 3, &tRow);
			switch(eRowInfo) {
			case found_a_cell:
 ...
 */
void
vGet8FontInfo(int iFodo, USHORT usIstd, const UCHAR* aucGrpprl, int iBytes,
 ...
 */
static BOOL
bGet8PicInfo(int iFodo, const UCHAR* aucGrpprl, int iBytes,
             picture_block_type* pPicture)
{
	...
	while(iBytes >= iFodoOff + 2) {
		...
		case 0x680e: /* fcObj */
			...
			break;
		case 0x6a03: /* fcPic */
			pPicture->ulPictureOffset =
			    ulGetLong(iFodo + iFodoOff + 2, aucGrpprl);
			bFound = TRUE;
			break;
		default:
			break;
 ...
 */
void
vGet8ChrInfo(FILE* pFile, const pps_info_type* pPPS, const ULONG* aulBBD,
             size_t tBBDLen, const ULONG* aulSBD, size_t tSBDLen,
             const UCHAR* aucHeader)
{
	...
	aucBuffer =
	    aucFillInfoBuffer(pFile, &pPPS->tTable, aulBBD, tBBDLen, aulSBD,
	                      tSBDLen, ulBeginCharInfo, tCharInfoLen);
	if(aucBuffer == NULL) {
		return;
	}
	...
	for(iIndex = 0; iIndex < (int)tLen; iIndex++) {
		...
		for(iIndex2 = 0; iIndex2 < iRun; iIndex2++) {
			...
			iFodo = 2 * (int)ucGetByte((iRun + 1) * 4 + iIndex2,
			                           aucFpage);
			...
			if(iFodo != 0) {
				vGet8FontInfo(iFodo, usIstd, aucFpage + 1,
				              iLen - 1, &tFont);
			}
			...
			if(bGet8PicInfo(iFodo, aucFpage + 1, iLen - 1,
			                &tPicture)) {
