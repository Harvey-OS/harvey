...
vConvertListCharacter(UCHAR ucNFC, USHORT usListChar, char* szListChar)
{
	...
	if(eEncoding == encoding_utf_8) {
		...
		case 0x2022:
		default:
			vGetBulletValue(eConversionType, eEncoding, szListChar,
			                2);
			break;
		}
		tLen = 1;
...
vCorrectStyleValues(style_block_type* pStyleBlock)
{
	...
	if(pStyleBlock->usIstd >= 1 && pStyleBlock->usIstd <= 9 &&
	...
	vConvertListCharacter(pStyleBlock->ucNFC, pStyleBlock->usListChar,
...
vAdd2StyleInfoList(const style_block_type* pStyleBlock)
{
	...
	NO_DBG_DEC_C(pStyleBlock->sLeftIndent != 0, pStyleBlock->sLeftIndent);
	NO_DBG_DEC_C(pStyleBlock->sRightIndent != 0, pStyleBlock->sRightIndent);
...
pGetNextTextStyle(const style_block_type* pCurr)
{
	...
	} while(pRecord != NULL && (pRecord->eListID == hdrftr_list ||
