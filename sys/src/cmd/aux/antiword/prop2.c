 ...
 */
row_info_enum
eGet2RowInfo(int iFodo, const UCHAR* aucGrpprl, int iBytes,
             row_block_type* pRow)
{
	int iFodoOff, iInfoLen;
	...
	while(iBytes >= iFodoOff + 1) {
		...
		if(iInfoLen <= 0) {
			iInfoLen = iGet2InfoLength(iFodo + iFodoOff, aucGrpprl);
			fail(iInfoLen <= 0);
		}
		iFodoOff += iInfoLen;
	}
 ...
 */
void
vGet2StyleInfo(int iFodo, const UCHAR* aucGrpprl, int iBytes,
...
vGet2PapInfo(FILE* pFile, const UCHAR* aucHeader)
{
	...
	for(iIndex = 0; iIndex < (int)tLen; iIndex++) {
		...
		for(iIndex2 = 0; iIndex2 < iRun; iIndex2++) {
			...
			iFodo = 2 * (int)ucGetByte((iRun + 1) * 4 + iIndex2 * 1,
			                           aucFpage);
			if(iFodo <= 0) {
				continue;
			}
			...
			eRowInfo =
			    eGet2RowInfo(iFodo, aucFpage + 8, iLen - 8, &tRow);
 ...
 */
void
vGet1FontInfo(int iFodo, const UCHAR* aucGrpprl, size_t tBytes,
 ...
 */
void
vGet2FontInfo(int iFodo, const UCHAR* aucGrpprl, size_t tBytes,
              font_block_type* pFont)
{
	BOOL bIcoChange, bFtcChange, bHpsChange, bKulChange;
	USHORT usTmp;
 ...
 */
static BOOL
bGet1PicInfo(int iFodo, const UCHAR* aucGrpprl, size_t tBytes,
 ...
 */
static BOOL
bGet2PicInfo(int iFodo, const UCHAR* aucGrpprl, size_t tBytes,
             picture_block_type* pPicture)
{
	ULONG ulTmp;
	UCHAR aucChpx[18];
...
vGet2ChrInfo(FILE* pFile, int iWordVersion, const UCHAR* aucHeader)
{
	...
	for(iIndex = 0; iIndex < (int)tLen; iIndex++) {
		...
		for(iIndex2 = 0; iIndex2 < iRun; iIndex2++) {
			...
			if(iFodo != 0) {
				if(iWordVersion == 1) {
					vGet1FontInfo(iFodo, aucFpage + 1,
					              tChrLen, &tFont);
				} else if(iWordVersion == 2) {
					vGet2FontInfo(iFodo, aucFpage + 1,
					              tChrLen, &tFont);
				}
			}
			...
			bSuccess1 = iWordVersion == 1 &&
			            bGet1PicInfo(iFodo, aucFpage + 1, tChrLen,
			                         &tPicture);
			bSuccess2 = iWordVersion == 2 &&
			            bGet2PicInfo(iFodo, aucFpage + 1, tChrLen,
			                         &tPicture);
