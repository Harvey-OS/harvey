...
vStoreByte(UCHAR ucChar, output_type* pOutput)
{
	...
	while(pOutput->tNextFree + 2 > pOutput->tStorageSize) {
		...
		pOutput->szStorage =
		    xrealloc(pOutput->szStorage, pOutput->tStorageSize);
	}
...
vStoreChar(ULONG ulChar, BOOL bChangeAllowed, output_type* pOutput)
{
	...
	pOutput->lStringWidth +=
	    lComputeStringWidth(pOutput->szStorage + pOutput->tNextFree - tLen,
	                        tLen, pOutput->tFontRef, pOutput->usFontSize);
} /* end of vStoreChar */
            ...
            const style_block_type* pStyle)
{
	...
	if(tOptions.eConversionType == conversion_xml) {
		...
		tLen =
		    tStyle2Window(szString, sizeof(szString), pStyle, pSection);
		vStoreString(szString, tLen, pOutput);
	}
} /* end of vStoreStyle */
 ...
 */
static void
vPutIndentation(diagram_type* pDiag, output_type* pOutput, BOOL bNoMarks,
                BOOL bFirstLine, UINT uiListNumber, UCHAR ucNFC,
                const char* szListChar, int32_t lLeftIndentation,
                int32_t lLeftIndentation1)
{
	int32_t lWidth;
	...
	szLine[tNextFree++] = ' ';
	szLine[tNextFree] = '\0';
	lWidth = lComputeStringWidth(szLine, tNextFree, pOutput->tFontRef,
	                             pOutput->usFontSize);
...
ulGetChar(FILE* pFile, list_id_enum eListID)
{
	...
	bSkip = FALSE;
	for(;;) {
		usChar = usNextChar(pFile, eListID, &ulFileOffset, &ulCharPos,
		                    &usPropMod);
		...
		ulChar = ulTranslateCharacters(
		    ...
		    tOptions.eConversionType, tOptions.eEncoding, bOldMacFile);
...
/* Initialisation */
	...
	for(;;) {
		...
		if(bWasTableRow && !bIsTableRow && ulChar != PAR_END &&
		   ulChar != HARD_RETURN && ulChar != PAGE_BREAK &&
		   ulChar != COLUMN_FEED) {
			/*
			 ...
			 */
			OUTPUT_LINE();
			vEndOfParagraph(pDiag, pOutput->tFontRef,
			                pOutput->usFontSize,
		...
		if(bStartFont || (bStartFontNext && ulChar != PAR_END)) {
			...
			usTmp = pFontTmp->usFontStyle &
			        (FONT_BOLD | FONT_ITALIC | FONT_UNDERLINE |
			         FONT_STRIKE | FONT_MARKDEL | FONT_SUPERSCRIPT |
			         FONT_SUBSCRIPT);
			if(!bIsTableRow &&
			   (usFontSize != pFontTmp->usFontSize ||
				...
				pOutput->tFontRef =
				    tOpenFont(pFontTmp->ucFontNumber,
		...
		if(!bIsTableRow && lTotalStringWidth(pAnchor) == 0) {
			if(!bNoMarks) {
				usListNumber = usGetListValue(
				    iListSeqNumber, iWordVersion, pStyleTmp);
			}
		...
		case PAR_END:
			...
			if(bOutputContainsText(pAnchor)) {
				...
				vMove2NextLine(pDiag, pOutput->tFontRef,
				               pOutput->usFontSize);
				RESET_LINE();
			}
			vEndOfParagraph(pDiag, pOutput->tFontRef,
			                pOutput->usFontSize, lAfterIndentation);
			bWasEndOfParagraph = TRUE;
			break;
		case HARD_RETURN:
			if(bIsTableRow) {
		...
		case TAB:
			...
			lHalfSpaceWidth =
			    (lComputeSpaceWidth(pOutput->tFontRef,
			                        pOutput->usFontSize) +
			     1) /
			    2;
		...
		if(bWasTableRow && !bIsTableRow) {
			...
			pOutput->usFontSize = usFontSize;
			pOutput->tFontRef =
			    tOpenFont(ucFontNumber, usFontStyle, usFontSize);
		}
		bWasTableRow = bIsTableRow;
		...
		vJustify2Window(pDiag, pAnchor, lWidthMax, lRightIndentation,
		                ucAlignment);
		pAnchor = pStartNewOutput(pAnchor, pLeftOver);
		for(pOutput = pAnchor; pOutput->pNext != NULL;
...
pHdrFtrDecryptor(FILE* pFile, ULONG ulCharPosStart, ULONG ulCharPosNext)
{
	...
	while(usChar != (USHORT)EOF && ulCharPos != ulCharPosNext) {
		...
		if(bSkip || usChar == END_IGNORE || usChar == END_EMBEDDED) {
			...
			ulChar = ulTranslateCharacters(
			    usChar, ulFileOffset, iWordVersion,
			    tOptions.eConversionType, tOptions.eEncoding,
			    bOldMacFile);
		}
		/* Process character */
		if(ulChar != IGNORE_CHARACTER) {
			...
			case COLUMN_FEED:
				...
				pOutput->tFontRef = tOpenFont(
				    0, FONT_REGULAR, DEFAULT_FONT_SIZE);
		...
		usChar = usNextChar(pFile, hdrftr_list, &ulFileOffset,
		                    &ulCharPos, NULL);
	}
	vCloseFont();
	if(bOutputContainsText(pAnchor)) {
...
szFootnoteDecryptor(FILE* pFile, ULONG ulCharPosStart, ULONG ulCharPosNext)
{
	...
	while(usChar != (USHORT)EOF && ulCharPos != ulCharPosNext &&
	      (usChar == FOOTNOTE_OR_ENDNOTE || usChar == PAR_END ||
	       usChar == TAB || usChar == (USHORT)' ')) {
		usChar = usNextChar(pFile, footnote_list, &ulFileOffset,
		                    &ulCharPos, NULL);
	}
