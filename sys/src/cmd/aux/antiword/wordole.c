...
vName2String(char* szName, const UCHAR* aucBytes, size_t tNameSize)
{
	...
	for(tIndex = 0, pcChar = szName; tIndex < 2 * tNameSize;
	    tIndex += 2, pcChar++) {
 ...
 */
static size_t
tReadBlockIndices(FILE* pFile, ULONG* aulBlockDepot, size_t tMaxRec,
                  ULONG ulOffset)
{
	size_t tDone;
	int iIndex;
 ...
 */
static BOOL
bGetBBD(FILE* pFile, const ULONG* aulDepot, size_t tDepotLen, ULONG* aulBBD,
        size_t tBBDLen)
{
	ULONG ulBegin;
	size_t tToGo, tDone;
 ...
 */
static BOOL
bGetSBD(FILE* pFile, const ULONG* aulDepot, size_t tDepotLen, ULONG* aulSBD,
        size_t tSBDLen)
{
	ULONG ulBegin;
	size_t tToGo, tDone;
 ...
 */
static void
vComputePPSlevels(pps_entry_type* atPPSlist, pps_entry_type* pNode, int iLevel,
                  int iRecursionLevel)
{
	...
	if(pNode->ulDir != PPS_NUMBER_INVALID) {
		vComputePPSlevels(atPPSlist, &atPPSlist[pNode->ulDir],
		                  iLevel + 1, iRecursionLevel + 1);
	}
	if(pNode->ulNext != PPS_NUMBER_INVALID) {
		vComputePPSlevels(atPPSlist, &atPPSlist[pNode->ulNext], iLevel,
		                  iRecursionLevel + 1);
	}
	if(pNode->ulPrevious != PPS_NUMBER_INVALID) {
		vComputePPSlevels(atPPSlist, &atPPSlist[pNode->ulPrevious],
		                  iLevel, iRecursionLevel + 1);
	}
 ...
 */
static BOOL
bGetPPS(FILE* pFile, const ULONG* aulRootList, size_t tRootListLen,
        pps_info_type* pPPS)
{
	...
	for(iIndex = 0; iIndex < (int)tNbrOfPPS; iIndex++) {
		...
		ulBegin =
		    (aulRootList[iStartBlock] + 1) * BIG_BLOCK_SIZE + ulOffset;
		NO_DBG_HEX(ulBegin);
		if(!bReadBytes(aucBytes, PROPERTY_SET_STORAGE_SIZE, ulBegin,
		               pFile)) {
 ...
 */
static BOOL
bGetDocumentText(FILE* pFile, const pps_info_type* pPPS, const ULONG* aulBBD,
                 ...
                 const UCHAR* aucHeader, int iWordVersion)
{
	...
	case 7:
		if(bFastSaved) {
			bSuccess = bGet6DocumentText(
			    pFile, bFarEastWord, pPPS->tWordDocument.ulSB,
			    aulBBD, tBBDLen, aucHeader);
		} else {
			bSuccess = bAddTextBlocks(
			    ulBeginOfText,
			    ulTextLen + ulFootnoteLen + ulHdrFtrLen +
			        ulMacroLen + ulAnnotationLen + ulEndnoteLen +
			        ulTextBoxLen + ulHdrTextBoxLen,
			    bFarEastWord, IGNORE_PROPMOD,
			    pPPS->tWordDocument.ulSB, aulBBD, tBBDLen);
		}
		break;
	case 8:
		bSuccess = bGet8DocumentText(pFile, pPPS, aulBBD, tBBDLen,
		                             aulSBD, tSBDLen, aucHeader);
		break;
	default:
		werr(0, "This version of Word is not supported");
		bSuccess = FALSE;
	...
	if(bSuccess) {
		vSplitBlockList(pFile, ulTextLen, ulFootnoteLen, ulHdrFtrLen,
		                ulMacroLen, ulAnnotationLen, ulEndnoteLen,
		                ulTextBoxLen, ulHdrTextBoxLen,
 ...
 */
static void
vGetDocumentData(FILE* pFile, const pps_info_type* pPPS, const ULONG* aulBBD,
                 size_t tBBDLen, const UCHAR* aucHeader, int iWordVersion)
{
	...
	if(!bHasImages || tOptions.eConversionType == conversion_text ||
	...
	case 7:
		...
		if(bFastSaved) {
			bSuccess =
			    bGet6DocumentData(pFile, pPPS->tWordDocument.ulSB,
			                      aulBBD, tBBDLen, aucHeader);
		} else {
			bSuccess = bAddDataBlocks(
			    ulBeginOfText, (ULONG)LONG_MAX,
			    pPPS->tWordDocument.ulSB, aulBBD, tBBDLen);
		}
		break;
	case 8:
		 ...
		 */
		bSuccess = bAddDataBlocks(0, (ULONG)LONG_MAX, pPPS->tData.ulSB,
		                          aulBBD, tBBDLen);
		break;
	default:
		werr(0, "This version of Word is not supported");
...
iInitDocumentOLE(FILE* pFile, int32_t lFilesize)
{
	...
	ulSBLstartblock =
	    ulReadLong(pFile, (ulRootStartblock + 1) * BIG_BLOCK_SIZE + 0x74);
	DBG_DEC(ulSBLstartblock);
	tSBDLen = (size_t)(
	    ulReadLong(pFile, (ulRootStartblock + 1) * BIG_BLOCK_SIZE + 0x78) /
	    SMALL_BLOCK_SIZE);
	...
	while(ulAdditionalBBDlist != END_OF_CHAIN && iToGo > 0) {
		...
		vGetBbdList(pFile, min(iToGo, 127), aulBbdList + ulStart,
		            ulBdbListStart);
		ulAdditionalBBDlist =
		    ulReadLong(pFile, ulBdbListStart + 4 * 127);
	...
	if(!bReadBuffer(pFile, PPS_info.tWordDocument.ulSB, aulBBD, tBBDLen,
	                BIG_BLOCK_SIZE, aucHeader, 0, HEADER_SIZE)) {
		FREE_ALL();
		return -1;
	}
	usIdent = usGetWord(0x00, aucHeader);
	...
	bSuccess = bGetDocumentText(pFile, &PPS_info, aulBBD, tBBDLen, aulSBD,
	                            tSBDLen, aucHeader, iWordVersion);
	if(bSuccess) {
		vGetDocumentData(pFile, &PPS_info, aulBBD, tBBDLen, aucHeader,
		                 iWordVersion);
		vGetPropertyInfo(pFile, &PPS_info, aulBBD, tBBDLen, aulSBD,
		                 tSBDLen, aucHeader, iWordVersion);
		vSetDefaultTabWidth(pFile, &PPS_info, aulBBD, tBBDLen, aulSBD,
		                    tSBDLen, aucHeader, iWordVersion);
		vGetNotesInfo(pFile, &PPS_info, aulBBD, tBBDLen, aulSBD,
		              tSBDLen, aucHeader, iWordVersion);
	}
	FREE_ALL();
