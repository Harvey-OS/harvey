...
static const char* iso_8859_1_data[] = {
    ...
    "] bind def", "", "/reencdict 12 dict def", "",
};
...
static const char* iso_8859_2_data[] = {
    ...
    "255/dotaccent", "] bind def", "", "/reencdict 12 dict def", "",
};
...
static const char* iso_8859_5_data[] = {
    ...
    "255/afii10193", "] bind def", "", "/reencdict 12 dict def", "",
};
...
static const char* iso_8859_x_func[] = {
    ...
    "	findfont exch scalefont setfont", "} bind def", "", "/ReEncode", "{",
    "reencdict begin", "	/newname exch def", "	/basename exch def",
    "	/basedict basename findfont def",
    "	/newfont basedict maxlength dict def", "	basedict",
    ...
    "	newname newfont definefont pop", "end", "} bind def", "",
};

static const char* misc_func[] = {
    "% draw a line and show the string",
    "/LineShow	% string linewidth movement", "{", "	gsave",
    "		0 exch rmoveto", "		setlinewidth",
    "		dup", "		stringwidth pop",
    "		0 rlineto stroke", "	grestore", "	show", "} bind def", "",
    "% begin an EPS file (level 2 and up)", "/BeginEPSF", "{",
    "	/b4_Inc_state save def", "	/dict_count countdictstack def",
    "	/op_count count 1 sub def", "	userdict begin",
    "		/showpage { } def", "		0 setgray 0 setlinecap",
    "		1 setlinewidth 0 setlinejoin",
    "		10 setmiterlimit [ ] 0 setdash newpath",
    "		false setstrokeadjust false setoverprint", "} bind def", "",
    "% end an EPS file", "/EndEPSF {",
...
vAddHdrFtr(diagram_type* pDiag, const hdrftr_block_type* pHdrFtrInfo)
{
	...
	while(pStart != NULL) {
		pNext = pStart;
		while(pNext != NULL && (pNext->tNextFree != 1 ||
...
vAddHeader(diagram_type* pDiag)
{
	...
	pHdrInfo = pGetHdrFtrInfo(iSectionIndex, TRUE, odd(iPageCount),
	                          bFirstInSection);
	pFtrInfo = pGetHdrFtrInfo(iSectionIndex, FALSE, odd(iPageCount),
	                          bFirstInSection);
	/* Set the height of the footer of this page */
	lFooterHeight = pFtrInfo == NULL ? 0 : pFtrInfo->lHeight;
	fail(lFooterHeight < 0);

	if(pHdrInfo == NULL || pHdrInfo->pText == NULL ||
	   pHdrInfo->lHeight <= 0) {
		fail(pHdrInfo != NULL && pHdrInfo->lHeight < 0);
		fail(pHdrInfo != NULL && pHdrInfo->pText != NULL &&
		     pHdrInfo->lHeight == 0);
		return;
	}
...
vAddFooter(diagram_type* pDiag)
{
	...
	if(pFtrInfo == NULL || pFtrInfo->pText == NULL ||
	   pFtrInfo->lHeight <= 0) {
		fail(pFtrInfo != NULL && pFtrInfo->lHeight < 0);
		fail(pFtrInfo != NULL && pFtrInfo->pText != NULL &&
		     pFtrInfo->lHeight == 0);
		return;
	}
 ...
 */
void
vProloguePS(diagram_type* pDiag, const char* szTask, const char* szFilename,
...
vAddFontsPS(diagram_type* pDiag)
{
	...
	if(tGetFontTableLength() == 0) {
		...
		while((pTmp = pGetNextFontTableRecord(pTmp)) != NULL) {
			...
			iLineLen +=
			    fprintf(pOutFile, " %s", pTmp->szOurFontname);
		}
	}
	fprintf(pOutFile, "\n");
	...
	case encoding_latin_1:
		for(tIndex = 0; tIndex < elementsof(iso_8859_1_data);
		    tIndex++) {
			fprintf(pOutFile, "%s\n", iso_8859_1_data[tIndex]);
		}
		fprintf(pOutFile, "\n");
		for(tIndex = 0; tIndex < elementsof(iso_8859_x_func);
		    tIndex++) {
			fprintf(pOutFile, "%s\n", iso_8859_x_func[tIndex]);
		}
		break;
	case encoding_latin_2:
		for(tIndex = 0; tIndex < elementsof(iso_8859_2_data);
		    tIndex++) {
	...
	case encoding_cyrillic:
		for(tIndex = 0; tIndex < elementsof(iso_8859_5_data);
		    tIndex++) {
         ...
         USHORT usFontstyle)
{
	...
	for(tCount = 0; tCount < tStringLength; tCount++) {
		...
		case '\\':
			...
			break;
		default:
			if(ucBytes[tCount] < 0x20 || (ucBytes[tCount] >= 0x7f &&
			                              ucBytes[tCount] < 0x8c)) {
 ...
 */
void
vSubstringPS(diagram_type* pDiag, char* szString, size_t tStringLength,
             int32_t lStringWidth, UCHAR ucFontColor, USHORT usFontstyle,
             drawfile_fontref tFontRef, USHORT usFontSize, USHORT usMaxFontSize)
{
	...
	if(tFontRef != tFontRefCurr || usFontSize != usFontSizeCurr) {
		...
		fprintf(pDiag->pOutFile, "%.1f /%s /%s-ISO-8859-x ChgFnt\n",
		        (double)usFontSize / 2.0, szOurFontname, szOurFontname);
 ...
 */
void
vEndOfParagraphPS(diagram_type* pDiag, USHORT usFontSize,
