...
bFillPaletteDIB(FILE* pFile, imagedata_type* pImg, BOOL bNewFormat)
{
	...
	for(iIndex = 0; iIndex < pImg->iColorsUsed; iIndex++) {
		...
		if(pImg->aucPalette[iIndex][0] != pImg->aucPalette[iIndex][1] ||
		   pImg->aucPalette[iIndex][1] != pImg->aucPalette[iIndex][2]) {
...
bExamineJPEG(FILE* pFile, imagedata_type* pImg)
{
	...
	while(!bSOFDone && (iMarker = iNextMarker(pFile)) != (int)M_EOI) {
		...
		case EOF:
			...
			return FALSE;
		/* The following are not officially supported in PostScript
		 * level 2 */
		case M_SOF2:
		case M_SOF3:
		case M_SOF5:
		...
		case M_APP14:
			 ...
			 * Adobe's
			 * TN5116) to contain the string "Adobe" at the start of
			 * the
			 * APP14 marker.
			 */
			tLength = (size_t)usNextWordBE(pFile);
	...
	/* Do some sanity checks with the parameters */
	if(pImg->iHeight <= 0 || pImg->iWidth <= 0 || pImg->iComponents <= 0) {
		DBG_DEC(pImg->iHeight);
		DBG_DEC(pImg->iWidth);
		DBG_DEC(pImg->iComponents);
		return FALSE;
...
bExaminePNG(FILE* pFile, imagedata_type* pImg)
{
	...
	if(pImg->iColorsUsed == 0 && pImg->iComponents == 1 &&
	   pImg->uiBitsPerComponent <= 4) {
		...
		for(iIndex = 0, iColor = 0x00; iIndex < pImg->iColorsUsed;
...
vImage2Papersize(imagedata_type* pImg)
{
	...
	if(iNetPageHeight < 0 || iNetPageWidth < 0) {
		...
		iNetPageHeight =
		    tOptions.iPageHeight -
		    (lDrawUnits2MilliPoints(PS_TOP_MARGIN + PS_BOTTOM_MARGIN) +
		     999) /
		        1000;
		iNetPageWidth =
		    tOptions.iPageWidth -
...
eExamineImage(FILE* pFile, ULONG ulFileOffsetImage, imagedata_type* pImg)
{
	...
	fail(tWordHeaderLen != 46 && tWordHeaderLen != 58 &&
	     tWordHeaderLen != 68);
	...
	pImg->tPosition = tPos;
	pImg->iHorSizeScaled =
	    (int)(((int32_t)iHorSize * (int32_t)usHorScalingFactor + 500) /
	          1000);
	pImg->iVerSizeScaled =
	    (int)(((int32_t)iVerSize * (int32_t)usVerScalingFactor + 500) /
	          1000);
#if !defined(__riscos)
