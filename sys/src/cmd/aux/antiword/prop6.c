...
iGet6InfoLength(int iByteNbr, const UCHAR* aucGrpprl)
{
	...
	case 23:
		...
		if(iTmp == 255) {
			...
			iAdd =
			    (int)ucGetByte(iByteNbr + 3 + iDel * 4, aucGrpprl);
 ...
 */
void
vGet6DopInfo(FILE* pFile, ULONG ulStartBlock, const ULONG* aulBBD,
 ...
 */
void
vGet6SepInfo(FILE* pFile, ULONG ulStartBlock, const ULONG* aulBBD,
             size_t tBBDLen, const UCHAR* aucHeader)
{
	...
	for(tIndex = 0; tIndex < tLen; tIndex++) {
		...
		if(!bReadBuffer(pFile, ulStartBlock, aulBBD, tBBDLen,
		                BIG_BLOCK_SIZE, aucTmp, aulSectPage[tIndex],
		                2)) {
			continue;
		}
		...
		if(!bReadBuffer(pFile, ulStartBlock, aulBBD, tBBDLen,
		                BIG_BLOCK_SIZE, aucFpage, aulSectPage[tIndex],
		                tBytes)) {
 ...
 */
void
vGet6HdrFtrInfo(FILE* pFile, ULONG ulStartBlock, const ULONG* aulBBD,
                size_t tBBDLen, const UCHAR* aucHeader)
{
	ULONG* aulCharPos;
	UCHAR* aucBuffer;
 ...
 */
row_info_enum
eGet6RowInfo(int iFodo, const UCHAR* aucGrpprl, int iBytes,
             row_block_type* pRow)
{
	...
	while(iBytes >= iFodoOff + 1) {
		...
		case 190: /* cDefTable */
			...
			if(iCol < 1 || iBytes < iFodoOff + 3 + (iCol + 1) * 2) {
				DBG_DEC(iCol);
				DBG_DEC(iFodoOff);
				iInfoLen = 1;
				break;
			...
			iPosPrev = (int)(int16_t)usGetWord(iFodo + iFodoOff + 4,
			                                   aucGrpprl);
 ...
 */
void
vGet6StyleInfo(int iFodo, const UCHAR* aucGrpprl, int iBytes,
               style_block_type* pStyle)
{
	...
	while(iBytes >= iFodoOff + 1) {
		...
		case 2: /* istd */
			sTmp =
			    (int16_t)ucGetByte(iFodo + iFodoOff + 1, aucGrpprl);
			NO_DBG_DEC(sTmp);
			break;
		case 5: /* jc */
			pStyle->ucAlignment =
		...
		case 12: /* anld */
			...
			if(pStyle->ucNFC != LIST_BULLETS && iTmp >= 2) {
				iBefore = (int)ucGetByte(iFodo + iFodoOff + 3,
				                         aucGrpprl);
			} else {
				iBefore = 0;
			}
			if(iTmp >= 12) {
				pStyle->usStartAt =
				    usGetWord(iFodo + iFodoOff + 12, aucGrpprl);
			}
			if(iTmp >= iBefore + 21) {
				pStyle->usListChar = (USHORT)ucGetByte(
				    iFodo + iFodoOff + iBefore + 22, aucGrpprl);
		...
		case 23: /* ChgTabs */
			...
			NO_DBG_DEC(iDel);
			iAdd = (int)ucGetByte(iFodo + iFodoOff + 3 + 2 * iDel,
			                      aucGrpprl);
		...
		case 16: /* dxaRight */
			pStyle->sRightIndent =
			    (int16_t)usGetWord(iFodo + iFodoOff + 1, aucGrpprl);
 ...
 */
void
vGet6PapInfo(FILE* pFile, ULONG ulStartBlock, const ULONG* aulBBD,
             size_t tBBDLen, const UCHAR* aucHeader)
{
	row_block_type tRow;
	...
	for(tIndex = 0; tIndex < tLen; tIndex++) {
		...
		for(tIndex2 = 0; tIndex2 < tRun; tIndex2++) {
			...
			iFodo = 2 * (int)ucGetByte((tRun + 1) * 4 + tIndex2 * 7,
			                           aucFpage);
			if(iFodo <= 0) {
				continue;
			}
			...
			eRowInfo =
			    eGet6RowInfo(iFodo, aucFpage + 3, iLen - 3, &tRow);
			switch(eRowInfo) {
			case found_a_cell:
 ...
 */
void
vGet6FontInfo(int iFodo, USHORT usIstd, const UCHAR* aucGrpprl, int iBytes,
 ...
 */
static BOOL
bGet6PicInfo(int iFodo, const UCHAR* aucGrpprl, int iBytes,
 ...
 */
void
vGet6ChrInfo(FILE* pFile, ULONG ulStartBlock, const ULONG* aulBBD,
             size_t tBBDLen, const UCHAR* aucHeader)
{
	...
	for(tIndex = 0; tIndex < tLen; tIndex++) {
		...
		for(tIndex2 = 0; tIndex2 < tRun; tIndex2++) {
			...
			iFodo = 2 * (int)ucGetByte((tRun + 1) * 4 + tIndex2,
			                           aucFpage);
			...
			if(iFodo != 0) {
				vGet6FontInfo(iFodo, usIstd, aucFpage + 1,
				              iLen - 1, &tFont);
			}
			...
			if(bGet6PicInfo(iFodo, aucFpage + 1, iLen - 1,
			                &tPicture)) {
