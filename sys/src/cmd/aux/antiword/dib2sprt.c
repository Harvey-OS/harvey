...
pCreateBlankSprite(const imagedata_type* pImg, size_t* pSize)
{
	...
	tSize = sizeof(sprite_areainfo) +
	        Sprite_MemorySize(pImg->iWidth, pImg->iHeight, uMode,
	                          iPaletteEntries > 0 ? sprite_HASPAL
	                                              : sprite_HASNOMASKPAL);
	DBG_DEC(tSize);
	pArea = xmalloc(tSize);
...
vDecode8bpp(FILE* pFile, UCHAR* pucData, const imagedata_type* pImg)
{
	...
	for(iY = pImg->iHeight - 1; iY >= 0; iY--) {
		for(iX = 0; iX < pImg->iWidth; iX++) {
			...
			iOffset = iY * iByteWidth + iX;
			*(pucData + iOffset) =
			    iReduceColor(pImg->aucPalette[iIndex][0],
...
vDecodeDIB(diagram_type* pDiag, FILE* pFile, const imagedata_type* pImg)
{
	...
	if(pImg->uiBitsPerComponent <= 8) {
		(void)tSkipBytes(pFile, pImg->iColorsUsed *
		                            ((iHeaderSize > 12) ? 4 : 3));
	}
	...
	pucPalette =
	    (UCHAR*)pSprite + pSprite->firstoffset + sizeof(sprite_header);
	...
	vImage2Diagram(pDiag, pImg, (UCHAR*)pSprite + pSprite->firstoffset,
 ...
 */
BOOL
bTranslateDIB(diagram_type* pDiag, FILE* pFile, ULONG ulFileOffset,
