 ...
 */
os_error*
Drawfile_Bbox(drawfile_bbox_flags flags, drawfile_diagram const* diagram,
              int size, os_trfm const* trfm, wimp_box* bbox)
{
	return SWI(5, 0, DrawFile_BBox | XOS_Bit, flags, diagram, size, trfm,
	           bbox);
                       ...
                       double dScale)
{
	...
	return SWI(6, 0, DrawFile_Render | XOS_Bit, 0, pInfo->data,
	           pInfo->length, aiTransform, &pRedraw->rect, 0);
...
Drawfile_VerifyDiagram(drawfile_info* pInfo)
{
	...
	pDiag = (drawfile_diagram*)pInfo->data;
	if(strncmp(pDiag->tag, "Draw", 4) != 0 || pDiag->major_version != 201 ||
	   pDiag->minor_version != 0) {
...
Drawfile_QueryBox(drawfile_info* pInfo, wimp_box* pRect, BOOL bScreenUnits)
{
	...
	Error_CheckFatal(
	    Drawfile_Bbox(0, pInfo->data, pInfo->length, NULL, pRect));
	if(bScreenUnits) {
