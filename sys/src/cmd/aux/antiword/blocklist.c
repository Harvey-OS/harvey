...
static readinfo_type tOthers = {
    NULL, 0, 0,
};
static readinfo_type tHdrFtr = {
    NULL, 0, 0,
};
static readinfo_type tFootnote = {
    NULL, 0, 0,
};
...
bAdd2TextBlockList(const text_block_type* pTextBlock)
{
	...
	if(pTextBlock->ulFileOffset == FC_INVALID ||
	   pTextBlock->ulCharPos == CP_INVALID || pTextBlock->ulLength == 0 ||
	...
	if(pBlockLast != NULL &&
	   pBlockLast->tInfo.ulFileOffset + pBlockLast->tInfo.ulLength ==
	       pTextBlock->ulFileOffset &&
	   pBlockLast->tInfo.ulCharPos + pBlockLast->tInfo.ulLength ==
	       pTextBlock->ulCharPos &&
          ...
          ULONG ulListLen)
{
	...
	if(ulListLen != 0) {
		DBG_DEC(ulListLen);
		for(pCurr = *ppAnchorCurr; pCurr != NULL;
		    pCurr = pCurr->pNext) {
			...
			if(pCurr->tInfo.bUsesUnicode) {
				...
				lCharsToGo -=
				    (int32_t)(pCurr->tInfo.ulLength / 2);
				if(lCharsToGo < 0) {
	...
	if(ulListLen == 0) {
		...
		*ppAnchorNext = xmalloc(sizeof(list_mem_type));
		...
		(*ppAnchorNext)->tInfo.ulCharPos = pCurr->tInfo.ulCharPos +
...
bIsEmptyBox(FILE* pFile, const list_mem_type* pAnchor)
{
	...
	for(pCurr = pAnchor; pCurr != NULL; pCurr = pCurr->pNext) {
		...
		if(!bReadBytes(aucBuffer, tSize, pCurr->tInfo.ulFileOffset,
		               pFile)) {
                ...
                BOOL bMustExtend)
{
	...
	for(tIndex = 0; tIndex < elementsof(apAnchors); tIndex++) {
		for(pCurr = apAnchors[tIndex]; pCurr != NULL;
		    pCurr = pCurr->pNext) {
			if(pCurr->pNext != NULL &&
...
bExistsTextBox(void)
{
	return pTextBoxAnchor != NULL && pTextBoxAnchor->tInfo.ulLength != 0;
              ...
              ULONG* pulFileOffset, ULONG* pulCharPos, USHORT* pusPropMod)
{
	...
	if(pInfoCurrent->pBlockCurrent == NULL ||
	   ...
	   pInfoCurrent->ulBlockOffset + pInfoCurrent->tByteNext >=
		...
		tReadLen =
		    (size_t)(pInfoCurrent->pBlockCurrent->tInfo.ulLength -
		             pInfoCurrent->ulBlockOffset);
		...
		ulReadOff = pInfoCurrent->pBlockCurrent->tInfo.ulFileOffset +
		            pInfoCurrent->ulBlockOffset;
		if(!bReadBytes(pInfoCurrent->aucBlock, tReadLen, ulReadOff,
		               pFile)) {
	...
	if(pulCharPos != NULL) {
		*pulCharPos = pInfoCurrent->pBlockCurrent->tInfo.ulCharPos +
		              pInfoCurrent->ulBlockOffset +
 ...
 */
static USHORT
usGetNextChar(FILE* pFile, list_id_enum eListID, ULONG* pulFileOffset,
              ULONG* pulCharPos, USHORT* pusPropMod)
{
	...
	usLSB = usGetNextByte(pFile, pReadinfo, pAnchor, pulFileOffset,
	                      pulCharPos, pusPropMod);
	if(usLSB == (USHORT)EOF) {
		return (USHORT)EOF;
	}
	...
	if(pReadinfo->pBlockCurrent->tInfo.bUsesUnicode) {
		usMSB =
		    usGetNextByte(pFile, pReadinfo, pAnchor, NULL, NULL, NULL);
	} else {
		usMSB = 0x00;
	}
	if(usMSB == (USHORT)EOF) {
 ...
 */
USHORT
usNextChar(FILE* pFile, list_id_enum eListID, ULONG* pulFileOffset,
           ULONG* pulCharPos, USHORT* pusPropMod)
{
	USHORT usRetVal;
	...
	usRetVal = usGetNextChar(pFile, eListID, pulFileOffset, pulCharPos,
	                         pusPropMod);
...
usToHdrFtrPosition(FILE* pFile, ULONG ulCharPos)
{
	...
	do {
		usChar =
		    usNextChar(pFile, hdrftr_list, NULL, &ulCharPosCurr, NULL);
...
usToFootnotePosition(FILE* pFile, ULONG ulCharPos)
{
	...
	do {
		usChar = usNextChar(pFile, footnote_list, NULL, &ulCharPosCurr,
		                    NULL);
	} while(usChar != (USHORT)EOF && ulCharPosCurr != ulCharPos);
	return usChar;
...
ulCharPos2FileOffsetX(ULONG ulCharPos, list_id_enum* peListID)
{
	static list_id_enum eListIDs[8] = {
	    text_list,       footnote_list, hdrftr_list,  macro_list,
	    annotation_list, endnote_list,  textbox_list, hdrtextbox_list,
	};
	list_mem_type* apAnchors[8];
	list_mem_type* pCurr;
	...
	for(tIndex = 0; tIndex < elementsof(apAnchors); tIndex++) {
		for(pCurr = apAnchors[tIndex]; pCurr != NULL;
		    pCurr = pCurr->pNext) {
			if(ulCharPos ==
			       pCurr->tInfo.ulCharPos + pCurr->tInfo.ulLength &&
			   pCurr->pNext != NULL) {
				/*
			...
			*peListID = eListIDs[tIndex];
			return pCurr->tInfo.ulFileOffset + ulCharPos -
