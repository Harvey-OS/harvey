...
vDestroyHdrFtrInfoList(void)
{
	...
	for(tHdrFtr = 0; tHdrFtr < tHdrFtrLen; tHdrFtr++) {
		...
		for(tIndex = 0; tIndex < elementsof(pRecord->atElement);
		    tIndex++) {
...
vCreat6HdrFtrInfoList(const ULONG* aulCharPos, size_t tLength)
{
	...
	for(tHdrFtr = 0; tHdrFtr < tHdrFtrLen; tHdrFtr++) {
		...
		pListMember = pHdrFtrList + tHdrFtr;
		for(tTmp = 0; tTmp < elementsof(pListMember->atElement);
		    tTmp++) {
...
pGetHdrFtrInfo(int iSectionIndex, BOOL bWantHeader, BOOL bOddPage,
...
vPrepareHdrFtrText(FILE* pFile)
{
	...
	for(tHdrFtr = 0; tHdrFtr < tHdrFtrLen; tHdrFtr++) {
		...
		for(tIndex = 0; tIndex < elementsof(pHdrFtrList->atElement);
		    tIndex++) {
	...
	for(tHdrFtr = 1, pCurr = &pHdrFtrList[1]; tHdrFtr < tHdrFtrLen;
	    tHdrFtr++, pCurr++) {
		pPrev = pCurr - 1;
		for(tIndex = 0; tIndex < elementsof(pHdrFtrList->atElement);
		    tIndex++) {
