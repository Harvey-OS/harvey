...
pOpenFontTableFile(void)
{
	...
	if(!bMakeDirectory(szFontNamesFile)) {
		werr(0, "I can't make a directory for the FontNames file");
		return NULL;
	}
...
tOpenFont(UCHAR ucWordFontNumber, USHORT usFontStyle, USHORT usWordFontSize)
{
	...
	e = Font_FindFont(&tFont, (char*)szOurFontname, (int)usWordFontSize * 8,
	                  (int)usWordFontSize * 8, 0, 0);
	if(e != NULL) {
		switch(e->errnum) {
		case 523:
			werr(0, "%s", e->errmess);
			break;
		default:
			werr(0, "Open font error %d: %s", e->errnum,
			     e->errmess);
			break;
		}
		tFontCurr = (font_handle)-1;
