...
bIsMatch(const char* szStr1, const char* szStr2)
{
	...
	for(pcTmp1 = szStr1, pcTmp2 = szStr2; *pcTmp1 != '\0';
	    pcTmp1++, pcTmp2++) {
...
main(int argc, char** argv)
{
	...
	if(argc > 1) {
		...
		tMsg.header.size =
		    ROUND4(offsetof(message_block, data) +
		           offsetof(message_dataload, filename) + 1 + tArgLen);
		...
		Error_CheckFatal(
		    Wimp_SendMessage(event_SEND, &tMsg, tTaskHandle, 0));
		return EXIT_SUCCESS;
	} else {
