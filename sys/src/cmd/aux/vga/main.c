...
linear(Vga* vga)
{
	...
	if(vga->ctlr->flag & Ulinear) {
		...
		vgactlr("addr", buf);
		if(atoi(buf) == 0 &&
		   (buf[0] != 'p' || buf[1] != ' ' || atoi(buf + 2) == 0)) {
			sprint(buf, "0x%lux 0x%lux",
...
char* chanstr[32 + 1] = {
        [1] "k1",      [2] "k2",      [4] "k4",        [8] "m8",
        [16] "r5g6b5", [24] "r8g8b8", [32] "x8r8g8b8",
};

static void
usage(void)
{
	fprint(2, "usage: aux/vga [ -BcdilpvV ] [ -b bios-id ] [ -m monitor ] "
	          "[ -x db ] [ mode [ virtualsize ] ]\n");
	exits("usage");
}
...
main(int argc, char** argv)
{
	...
	if(iflag || lflag) {
		if(getenv(type))
			...
			strecpy(monitordb, monitordb + sizeof monitordb,
			        dbname);
		...
		if(virtual) {
			...
			vga->virty = atoi(p + 1);
			if(vga->virtx < vga->mode->x ||
			   vga->virty < vga->mode->y)
				error("virtual size smaller than physical "
				      "size\n");
			vga->panning = 1;
		} else {
		...
		trace("vmf %d vmdf %d vf1 %lud vbw %lud\n",
		      vga->mode->frequency, vga->mode->deffrequency, vga->f[1],
		      vga->mode->videobw);
		if(vga->mode->frequency == 0 && vga->mode->videobw != 0 &&
		   vga->f[1] != 0) {
			/*
			...
			bytes =
			    (vga->mode->x * vga->mode->y * vga->mode->z) / 8;
			membw = rr * bytes;
		...
		if(strcmp(vga->mode->chan, "") == 0) {
			if(vga->mode->z < nelem(chanstr) &&
			   chanstr[vga->mode->z])
				strcpy(vga->mode->chan, chanstr[vga->mode->z]);
			else
				error("%s: unknown channel type to use for "
				      "depth %d\n",
		...
		if(lflag) {
			trace("main->load\n");
			if(vga->vmz &&
			   (vga->virtx * vga->virty * vga->mode->z) / 8 >
			       vga->vmz)
