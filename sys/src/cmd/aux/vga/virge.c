...
snarf(Vga* vga, Ctlr* ctlr)
{
	...
	case 0x883D: /* ViRGE/VX */
		 ...
		 */
		vga->vmz = (2 * (((vga->crt[0x36] >> 5) & 0x03) + 1)) * 1 *
		           1024 * 1024;
		break;
...
init(Vga* vga, Ctlr* ctlr)
{
	...
	case 0x8812:             /* Aurora64V+ */
		...
		if(id == 0x883D) { /* ViRGE/VX */
			            ...
			            * will leave little turds on the screen when hwfill is
			            * used.
			            */
			vga->crt[0x36] &= ~0x0C;
			 ...
			 * to 0x90 seems to allow us (and more importantly the
			 *card)
		...
		case 32:
			 ...
			 * 32-bit mode (which the GX/2 calls 24-bit unpacked
			 * mode).
			 */
			if(id != 0x8A10)
				error(
				    "32-bit mode only supported on the GX/2\n");
			vga->crt[0x67] |= 0xD0;
			break;
		}
