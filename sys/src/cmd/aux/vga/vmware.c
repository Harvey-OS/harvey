...
static char* rname[Nreg] = {
    "ID",       "Enable",  "Width",       "Height",    "MaxWidth",  "MaxHeight",
    "Depth",    "Bpp",     "PseudoColor", "RedMask",   "GreenMask", "BlueMask",
    "Bpl",      "FbStart", "FbOffset",    "FbMaxSize", "FbSize",    "Cap",
    "MemStart", "MemSize", "ConfigDone",  "Sync",      "Busy",      "GuestID",
    "CursorID", "CursorX", "CursorY",     "CursorOn",  "HostBpp",
};
...
snarf(Vga* vga, Ctlr* ctlr)
{
	...
	if(vm->r[Rrmask] > vm->r[Rgmask] && vm->r[Rgmask] > vm->r[Rbmask]) {
		if(extra)
			...
			vm->chan[0] = '\0';
		sprint(vm->chan + strlen(vm->chan), "r%dg%db%d",
		       bits(vm->r[Rrmask]), bits(vm->r[Rgmask]),
		       bits(vm->r[Rbmask]));
	} else if(vm->r[Rbmask] > vm->r[Rgmask] &&
	          vm->r[Rgmask] > vm->r[Rrmask]) {
