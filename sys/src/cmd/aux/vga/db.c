...
dbctlr(char* name, Vga* vga)
{
	 ...
	 */
	pcituple = nil;
	for(tuple = ndbsearch(db, &s, "ctlr", ""); tuple;
	    tuple = ndbsnext(&s, "ctlr", "")) {
...
buggery:
	...
	for(t = tuple->entry; t; t = t->entry) {
		...
		else if(strcmp(t->attr, "defaultclock") == 0 &&
		        mode->deffrequency == 0)
		...
		else if(strcmp(t->attr, "include") ==
		        0 /*&& strcmp(t->val, val) != 0*/) {
			strcpy(attr, t->attr);
			strcpy(val, t->val);
			ndbfree(tuple);
			if(i++ > 5)
				error("dbmonitor: implausible include depth at "
				      "%s=%s\n",
				      attr, val);
			goto buggery;
		} else if(strcmp(t->attr, "include") == 0) {
			print("warning: bailed out of infinite loop in attr "
			      "%s=%s\n",
			      attr, val);
		} else
			addattr(&mode->attr, t);
...
dbmode(char* name, char* type, char* size)
{
	...
	if(mode->videobw &&
	   videobw == 0) /* we at least found that; save it away */
 ...
 */
buggery:
	for(tuple = ndbsearch(db, &s, attr, ""); tuple;
	    tuple = ndbsnext(&s, attr, "")) {
		for(t = tuple->entry; t; t = t->entry) {
...
dbdumpmode(Mode* mode)
{
	...
	Bprint(&stdout, "x=%d (0x%X), y=%d (0x%X), z=%d (0x%X)\n", mode->x,
	       mode->x, mode->y, mode->y, mode->z, mode->z);
