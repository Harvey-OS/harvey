...
{
	OUTREG8(radeon, CLOCK_CNTL_INDEX, (offset & 0x3f) | PLL_WR_EN);
...
radeonpci(int* isr300)
{
	...
	DBGPRINT(
	    "radeon: ATI Technologies Inc. Radeon [789]xxx drivers (v0.1)\n");
...
radeon_init_crtc_registers(Radeon* radeon, Mode* mode)
{
	...
	radeon->dac_cntl =
	    DAC_MASK_ALL | DAC_VGA_ADR_EN | (dac6bit ? 0 : DAC_8BIT_EN);

	radeon->crtc_h_total_disp =
	    ((mode->ht / 8 - 1) & 0x3ff) | ((mode->x / 8 - 1) & 0x1ff) << 16;
	...
	DBGPRINT("hsync_start=%d hsync_wid=%d hsync_fudge=%d\n", hsync_start,
	         hsync_wid, hsync_fudge);

	radeon->crtc_h_sync_strt_wid =
	    ((hsync_start & 0x1fff) | (hsync_wid & 0x3f) << 16 |
	     (mode->hsync ? CRTC_H_SYNC_POL : 0));
	radeon->crtc_v_total_disp =
	    ((mode->vt - 1) & 0xffff) | (mode->y - 1) << 16;
	...
	radeon->crtc_v_sync_strt_wid =
	    (((mode->vrs - 1) & 0xfff) | (vsync_wid & 0x1f) << 16 |
	     (mode->vsync ? CRTC_V_SYNC_POL : 0));
...
radeon_init_pll_registers(Radeon* radeon, uint32_t freq)
{
	...
	static struct divider post_divs[] = {{1, 0},
	                                     {2, 1},
	                                     {4, 2},
	                                     {8, 3},
	                                     {3, 4},
	...
	DBGPRINT("dc=%uld, of=%uld, fd=%uld, pd=%uld\n", radeon->dot_clock_freq,
	         radeon->pll_output_freq, radeon->feedback_div,
	         radeon->post_div);
...
init(Vga* vga, Ctlr* ctlr)
{
	...
	DBGPRINT("radeon: freq=%d deffreq=%d x=%d y=%d z=%d\n", mode->frequency,
	         mode->deffrequency, mode->x, mode->y, mode->z);
	...
	DBGPRINT("radeon: vt=%d vrs=%d vre=%d vsync='%c'\n", mode->vt,
	         mode->vrs, mode->vre, mode->vsync ? mode->vsync : ' ');
...
radeon_pllwriteupd(Radeon* radeon)
{
	while(INPLL(radeon, PPLL_REF_DIV) & PPLL_ATOMIC_UPDATE_R)
		;
	OUTPLLP(radeon, PPLL_REF_DIV, PPLL_ATOMIC_UPDATE_W,
...
{
	OUTPLLP(radeon, VCLK_ECP_CNTL, VCLK_SRC_SEL_CPUCLK, ~VCLK_SRC_SEL_MASK);
	OUTPLLP(
	    radeon, PPLL_CNTL,
	...
	if(debug) {
		Bprint(&stdout,
		       "Wrote: 0x%08ulx 0x%08ulx 0x%08ulx (0x%08ulx)\n",
	...
	OUTPLLP(radeon, VCLK_ECP_CNTL, VCLK_SRC_SEL_PPLLCLK,
	        ~VCLK_SRC_SEL_MASK);
}
...
Ctlr radeon = {
    "radeon", snarf, options, init, load, dump,
};
Ctlr radeonhwgc = {
    "radeonhwgc", 0, /* snarf */
    0,               /* options */
