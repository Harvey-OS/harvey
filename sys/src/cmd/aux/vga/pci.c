...
pciscan(int bno, Pcidev** list)
{
	...
	for(dno = 0; dno <= pcimaxdno; dno++) {
		...
		for(fno = 0; fno <= maxfno; fno++) {
			 ...
			 * bus+device+function
			 * triplet needed to address it and try to read the
			 * vendor
			 * and device ID. If successful, allocate a device
			 * struct
			 * and start to fill it in with some useful information
			 * from
	...
	*list = head;
	for(p = head; p != nil; p = p->link) {
		 ...
		 * numbers as the tree is descended. On the way down the
		 * subordinate
		...
		if(sbn == 0 || ubn == 0) {
			 ...
			 * set the primary, secondary and subordinate bus
			 *numbers
...
pcicfginit(void)
{
	...
	if(pcicfgmode == -1) {
		 ...
		 * any non-DWORD accesses as normal I/O cycles. There shouldn't
		 * be
...
pcihinv(Pcidev* p)
{
	...
	for(t = p; t != nil; t = t->link) {
		...
		for(i = 0; i < nelem(p->mem); i++) {
			if(t->mem[i].size == 0)
				continue;
			Bprint(&stdout, "%d:%.8lux %d ", i, t->mem[i].bar,
			       t->mem[i].size);
		}
		Bprint(&stdout, "\n");
	}
