...
enum { Fdigital = 1 << 0,       /* is a digital display */
...
dump(Vga* vga, Ctlr* ctlr)
{
	...
	p = vbemodes(vbe);
	if(p) {
		for(ep = p + 1024; (p[0] != 0xFF || p[1] != 0xFF) && p < ep;
		    p += 2) {
...
static Flag capabilityflag[] = {0x01, "8-bit-dac", 0x02, "not-vga", 0x04,
                                "ramdac-needs-blank", 0x08, "stereoscopic",
                                0x10, "stereo-evc", 0};

static Flag modeattributesflags[] = {
    1 << 0, "supported", 1 << 2, "tty", 1 << 3, "color", 1 << 4, "graphics",
    1 << 5, "not-vga", 1 << 6, "no-windowed-vga", 1 << 7, "linear", 1 << 8,
    "double-scan", 1 << 9, "interlace", 1 << 10, "triple-buffer", 1 << 11,
    "stereoscopic", 1 << 12, "dual-start-addr", 0};

static Flag winattributesflags[] = {1 << 0, "relocatable", 1 << 1, "readable",
                                    1 << 2, "writeable",   0};

static Flag directcolorflags[] = {1 << 0, "programmable-color-ramp", 1 << 1,
                                  "x-usable", 0};

static char* modelstr[] = {"text",   "cga",        "hercules", "planar",
...
loadpage(Vbe* vbe, int p)
{
	if(p >= MemSize / PageSize || vbe->isvalid[p])
		return;
	if(pread(vbe->memfd, vbe->mem + p * PageSize, PageSize, p * PageSize) !=
	   PageSize)
...
vbemodeinfo(Vbe* vbe, int id, Vmode* m)
{
	...
	m->directcolor = p[39];
	m->paddr = LONG(p + 40);
	snprint(m->name, sizeof m->name, "%dx%dx%d", m->dx, m->dy, m->depth);
	if(m->depth <= 8) {
...
static Flag edidflags[] = {
    Fdigital,  "digital",      Fdpmsstandby, "standby",   Fdpmssuspend,
    "suspend", Fdpmsactiveoff, "activeoff",  Fmonochrome, "monochrome",
    Fgtf,      "gtf",          0};
...
static char* estabtime[] = {
    ...
    "640x480@60Hz",   "640x480@67Hz", /* non-VESA: Apple, Mac II */
    "640x480@72Hz",   "640x480@75Hz",  "800x600@56Hz",  "800x600@60Hz",

    "800x600@72Hz",   "800x600@75Hz",  "832x624@75Hz", /* non-VESA: Apple, Mac
                                                          II */
...
decodedtb(Mode* m, uint8_t* p)
{
	...
	vso = ((p[11] & 0x0C) << 2) |
	      ((p[10] & 0xF0) >> 4); /* vertical sync offset */
	vspw = ((p[11] & 0x03) << 4) |
	       (p[10] & 0x0F); /* vertical sync pulse width */
	...
	/* p[17] & 0x01 is another stereo bit, only referenced if p[17] & 0x60
	 * != 0 */
...
{
	static uint8_t magic[8] = {0x00, 0xFF, 0xFF, 0xFF,
	                           0xFF, 0xFF, 0xFF, 0x00};
	uint8_t* p, *q, sum;
	...
	assert(p ==
	       (uint8_t*)v +
	           8); /* assertion offsets from pp. 12-13 of the standard */
	               /*
	                * Manufacturer name is three 5-bit ascii letters, packed
	                * into a big endian [sic] short in big endian order.  The high bit is
	                * unused.
	                */
	i = (p[0] << 8) | p[1];
	p += 2;
	...
	for(i = 0; i < 4; i++, p += 18) {
		fprint(2, "%.8H\n", p);
		if(p[0] ||
		   p[1]) { /* detailed timing block: p[0] or p[1] != 0 */
			if(decodedtb(&mode, p) == 0)
				e->modelist = addmode(e->modelist, mode);
		} else if(p[2] == 0) { /* monitor descriptor block */
			switch(p[3]) {
			case 0xFF: /* monitor serial number (13-byte ascii, 0A
			              terminated) */
				...
				break;
			case 0xFE: /* ascii string (13-byte ascii, 0A
			              terminated) */
				break;
			case 0xFD: /* monitor range limits */
				...
				break;
			case 0xFC: /* monitor name (13-byte ascii, 0A
			              terminated) */
				if(q = memchr(p + 5, 0x0A, 13))
					*q = '\0';
