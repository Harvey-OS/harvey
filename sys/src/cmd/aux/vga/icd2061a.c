...
enum { Prescale = 2, /* P counter prescale (default) */
...
static uint32_t index[NIndex] = {
    50000000, 51000000, 53200000, 58500000, 60700000, 64400000,  66800000,
    73500000, 75600000, 80900000, 83200000, 91500000, 100000000, 120000000,
};
...
init(Vga* vga, Ctlr* ctlr)
{
	 ...
	 * Determine index.
	 */
	for(vga->i[0] = NIndex - 1; vga->f[0] < index[vga->i[0]] && vga->i[0];
	    vga->i[0]--)
		;
