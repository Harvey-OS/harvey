...
main(int argc, char *argv[])
{
	...
	if(argc > 0){
		...
		if(strcmp(p, "ps2")){
			data = openps2();
			if(data < 0)
				sysfatal("opening ps2: %r");
			if(!isps2(data))
				sysfatal("no mouse on ps2");
			dops2();
		} else {
			data = openserial(p, &ctl, baud);
			if(data < 0)
				sysfatal("opening %s: %r", p);
			for(tries = 0; type == 0 && tries < 6; tries++){
				type = isserial(data, ctl, def);
				if(type != 0)
					break;
			}
			if(type == 0)
				sysfatal("no mouse on %s", p);
			doserial(data, ctl, type);
		}
	} else {
		/* find a port with a mouse */
		data = openps2();
		if(data >= 0){
			if(isps2()){
				dops2();
