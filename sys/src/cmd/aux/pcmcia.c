...
void (*parse[256])(int, int) = {
        [1] tdevice,  [6] tlonglnkmfc, [0x15] tvers1,    [0x17] tdevice,
        [0x1A] tcfig, [0x1B] tentry,   [Funcid] tfuncid,
};
...
uint32_t speedtab[16] = {
        [1] 250, [2] 200, [3] 150, [4] 100,
};

uint32_t mantissa[16] = {
        [1] 10,   [2] 12,   [3] 13,   [4] 15,   [5] 20,
        [6] 25,   [7] 30,   [8] 35,   [9] 40,   [0xa] 45,
        [0xb] 50, [0xc] 55, [0xd] 60, [0xe] 70, [0xf] 80,
};

uint32_t exponent[8] = {
        [0] 1,     [1] 10,     [2] 100,     [3] 1000,
        [4] 10000, [5] 100000, [6] 1000000, [7] 10000000,
};

char* typetab[256] = {
        [1] "Masked ROM", [2] "PROM", [3] "EPROM", [4] "EEPROM",
        [5] "FLASH",      [6] "SRAM", [7] "DRAM",  [0xD] "IO+MEM",
};
...
static char* funcids[] = {
    "MULTI", "MEMORY",  "SERIAL", "PARALLEL", "FIXED",
...
char* intrname[16] = {
        [0] "memory",   [1] "I/O",      [4] "Custom 0",
        [5] "Custom 1", [6] "Custom 2", [7] "Custom 3",
};

uint32_t vexp[8] = {1, 10, 100, 1000, 10000, 100000, 1000000, 10000000};
uint32_t vmant[16] = {
...
ttiming(char* name, int scale)
{
	...
	if(readc(&unscaled) != 1)
		return;
	scaled =
	    (mantissa[(unscaled >> 3) & 0xf] * exponent[unscaled & 7]) / 10;
...
char* ioaccess[4] = {
    " no access", " 8bit access only", " 8bit or 16bit access",
...
irq(void)
{
	...
	if(c & 0x10) {
		...
		irqs = 1 << (c & 0xf);
	print("\tinterrupts%s%s%s", (c & 0x20) ? ":level" : "",
	      (c & 0x40) ? ":pulse" : "", (c & 0x80) ? ":shared" : "");
	for(i = 0; i < 16; i++)
		if(irqs & (1 << i))
			print(", %d", i);
	print("\n");
...
memspace(int asize, int lsize, int host)
{
	...
	if(host) {
		...
		print(
		    "\tmemory address range 0x%lux - 0x%lux hostaddr 0x%lux\n",
