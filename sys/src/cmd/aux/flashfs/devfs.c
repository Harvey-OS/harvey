...
initdata(char* f, int i)
{
	...
	Plain:
		...
		if(sectsize < 512)
			sysfatal("unreasonable value for sectsize: %lud",
			         sectsize);
	}
}
...
{
	if(isdev == 0) {
		if(pwrite(flash.dfd, ones, sectsize, sect * sectsize) !=
		   sectsize)
...
readdata(int sect, void* buff, uint32_t count, uint32_t off)
{
	...
	if(n != count)
		sysfatal("short read at %lux, %ld instead of %lud", m, n,
		         count);
}
...
writedata(int err, int sect, void* buff, uint32_t count, uint32_t off)
{
	...
	if(n != count) {
		if(err)
			return 0;
		sysfatal("short write at %lud, %ld instead of %lud", m, n,
		         count);
	}
	return 1;
}
