...
{
	if(nv < bcFibhdr)
		sysfatal("not enough data for Fibhdr");
	...
	s->lKey = v[0xE] | (v[0xE + 1] << 8) | (v[0xE + 2] << 16) |
	          (v[0xE + 3] << 24);
	s->envr = v[0x12];
	s->fMac = ((v[0x13]) & 0x1) >> 0;
	...
	s->fcMin = v[0x18] | (v[0x18 + 1] << 8) | (v[0x18 + 2] << 16) |
	           (v[0x18 + 3] << 24);
	s->fcMac = v[0x1C] | (v[0x1C + 1] << 8) | (v[0x1C + 2] << 16) |
	           (v[0x1C + 3] << 24);
...
main(int argc, char** argv)
{
	...
	for(i = 0; i < n; i++) {
		...
		/*
		 *  these are only supposed to get used when special is set, but
		 * we
		 ...
		 * every character is followed by a zero byte.  Some have
		 * sections
		 * of text where there are no zero bytes.  Still others have
		 * both
		 * types and alternate between them.  Until we parse which
		 * characters are ``special'', page numbers lose out.
		 */
		case 0: /* Bprint(&bout, "<pageno>"); */
			break;
		case 1:
			Bprint(&bout, "<picture>");
			break;
		case 2:
			Bprint(&bout, "<footnote>");
			break;
		case 3:
			Bprint(&bout, "<footnote sep>");
			break;
		case 4:
			Bprint(&bout, "<footnote cont>");
			break;
		case 5:
			Bprint(&bout, "<animation>");
			break;
		case 6:
			Bprint(&bout, "<lineno>");
			break;
		...
		case 29:
			...
			/*
			        case 30:	Bprint(&bout, "<mm/dd/yy>");
			   break;
			        case 33:	Bprint(&bout, "<mm>");	break;
			        case 34:	Bprint(&bout, "<yyyy>");
			   break;
			        case 35:	Bprint(&bout, "<yy>");	break;
			        case 36:	Bprint(&bout, "<Feb>");	break;
			        case 37:	Bprint(&bout, "<February>");
			   break;
			        case 38:	Bprint(&bout, "<hh:mm>");
			   break;
			        case 39:	Bprint(&bout, "<long date>");
			   break;
			        case 41:				break;
			   */
		}
