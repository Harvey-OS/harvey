...
listendir(char* protodir, char* srvdir, int trusted)
{
	...
	for(;;) {
		...
		for(a = announcements; a; a = a->next) {
			...
			case -1:
				syslog(1, listenlog, "couldn't fork for %s",
				       a->a);
				break;
			case 0:
				for(;;) {
...
dolisten(char* proto, char* dir, int ctl, char* srvdir, char* dialstr)
{
	...
	for(;;) {
		 ...
		 */
		switch(rfork(RFFDG | RFPROC | RFNOWAIT | RFENVG | RFNAMEG |
		             RFNOTEG)) {
		case -1:
		...
		case 0:
			...
			if(data < 0) {
				syslog(1, listenlog, "can't open %s/data: %r",
				       ndir);
				exits(0);
			}
...
getserv(char* proto, char* dir, Service* s)
{
	 ...
	 * ../../usr/user/bin/rc/su
	 */
	if(strlen(serv) + strlen(proto) >= NAMELEN || utfrune(serv, L'/') ||
	   *serv == '.')
		return 0;
...
newcall(int fd, char* proto, char* dir, Service* s)
{
	...
	if(!quiet) {
		if(dir != nil) {
			...
			syslog(0, listenlog, "%s call for %s on chan %s", proto,
			       s->serv, dir);
	}
