...
again:
	...
	case SIDEA:
		if(KNOWN(ANGLEA|ANGLEB|ANGLEC))
			j = sph1(aa, ab, ac); else
		if(KNOWN(ANGLEA|ANGLEB|SIDEC))
			j = sph3(aa, ab, sc, &j); else
		if(KNOWN(ANGLEA|ANGLEC|SIDEB))
			j = sph3(aa, ac, sb, &j); else
		if(KNOWN(ANGLEA|SIDEB|SIDEC))
			j = sph5(aa, sb, sc); else
			j = UNKNOWN;
		break;
	...
	case ANGLEA:
		if(KNOWN(SIDEA|SIDEB|SIDEC))
			j = sph2(sa, sb, sc); else
		if(KNOWN(SIDEA|SIDEB|ANGLEC))
			j = sph4(sa, sb, ac, &j); else
		if(KNOWN(SIDEA|SIDEC|ANGLEB))
			j = sph4(sa, sc, ab, &j); else
		if(KNOWN(SIDEA|ANGLEB|ANGLEC))
			j = sph6(sa, ab, ac); else
			j = UNKNOWN;
		break;
