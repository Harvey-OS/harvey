...
usage(void)
{
	fprint(2, "usage: ssh [-CiImPpRr] [-A authlist] [-c cipherlist] "
		"[user@]hostname [cmd [args]]\n");
	exits("usage");
}
...
main(int argc, char **argv)
{
	...
	if(forwardagent && startagent(&c) < 0)
		forwardagent = 0;
	if(usepty == -1)
		usepty = cmd==nil;
	if(usepty)
...
winchanges(Conn *c)
{
	int nrow, ncol, width, height, pid;
	...
	while (readgeom(&nrow, &ncol, &width, &height) >= 0)
