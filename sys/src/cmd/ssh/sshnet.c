...
extern Cipher *allcipher[];
extern int nelemallcipher;
extern char *cipherlist;

extern Auth *allauth[];
extern int nelemallauth;
extern char *authlist;
...
void
sshreadproc(void *c)		/* Conn *c */
{
	Msg *m;

	while ((m = recvmsg(c, -1)) != nil)
		sendp(sshmsgchan, m);
...
ctlwrite(Req *r, Client *c)
{
	...
	if(strcmp(f[0], "hangup") == 0){
		if(c->state != Established || nf != 1)
			goto Badarg;
		queuereq(c, r);
		teardownclient(c);
	}else if(strcmp(f[0], "connect") == 0){
		if(c->state != Closed || nf != 2)
			goto Badarg;
		...
		c->state = Dialing;
		m = allocmsg(conn, SSH_MSG_PORT_OPEN,
			4+4+strlen(f[0])+4+4+strlen("localhost"));
		putlong(m, c->num);
		putstring(m, f[0]);
...
threadmain(int argc, char **argv)
{
	...
	for(i=0; i<c.nokauth; i++)
		c.okauth[i] = findauth(f[i], allauth, nelemallauth);
