...
/* server ssh auth configuration */
Authsrv *allauthsrv[] = {
	&authsrvpassword,
	&authsrvtis,
};
int nelemallauthsrv = nelem(allauthsrv);
char *authsrvlist = "tis";
...
main(int argc, char **argv)
{
	...
	conninit(&c, -1, nil, argv[0]);
	sshlog("connect from %s", c.host);

	/* limit of 768 bits in remote host key? TODO? crank to 2048? */
	c.serverpriv = rsagen(768, 6, 0);
	if(c.serverpriv == nil)
		sysfatal("rsagen failed: %r");
	c.serverkey = &c.serverpriv->pub;

	c.nokauthsrv = getfields(authsrvlist, f, nelem(f), 1, ", ");
	c.okauthsrv = emalloc(sizeof(Authsrv*)*c.nokauthsrv);
	for(i=0; i<c.nokauthsrv; i++)
		c.okauthsrv[i] = findauthsrv(f[i], allauthsrv, nelemallauthsrv);
