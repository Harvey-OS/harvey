...
char* m3[] = {"next",    "prev", "page n", "again", "bigger",
              "smaller", "pan",  "quit?",  0};
char* m2[] = {0};
...
pan(void)
{
	...
	do {
		...
		if(mouse.xy.x < lastxy.x) /* moved left, clear right */
			draw(screen, Rect(max.x, screen->r.min.y,
			                  screen->r.max.x, screen->r.max.y),
			     display->white, nil, ZP);
		else /* moved right, clear left*/
			draw(screen, Rect(screen->r.min.x, screen->r.min.y,
			                  min.x, screen->r.max.y),
			     display->white, nil, ZP);
		if(mouse.xy.y < lastxy.y) /* moved up, clear down */
			draw(screen, Rect(screen->r.min.x, max.y,
			                  screen->r.max.x, screen->r.max.y),
			     display->white, nil, ZP);
		else /* moved down, clear up */
			draw(screen, Rect(screen->r.min.x, screen->r.min.y,
			                  screen->r.max.x, min.y),
			     display->white, nil, ZP);
...
static int checkmouse(void) /* return button touched if any */
{
	int c, b;
	char* p;
...
Cursor deadmouse = {
    {0, 0}, /* offset */
    {
     ...
     0x00, 0x82, 0x04, 0x41, 0xFF, 0xE1, 0x5F, 0xF1, 0x3F, 0xFE, 0x17, 0xF0,
