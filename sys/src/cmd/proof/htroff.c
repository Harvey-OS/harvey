	...
	while(gonow) {
		...
		case 'D': /* draw function */
			...
			case 'l': /* draw a line */
				...
				hpos += n;
				vpos += m;
				line(screen, scale(p), scale(q), 0, 0, 0,
				     display->black, ZP);
				break;
			case 'c': /* circle */
				/*nop*/
				...
				p = Pt(hpos + m, vpos);
				hpos += 2 * m;
				ellipse(screen, scale(p), m / DIV, m / DIV, 0,
				        display->black, ZP);
				/* p=currentpt; p.x+=dmap(m/2);circle
				 * bp,p,a,ONES,Mode*/
				break;
			case 'e': /* ellipse */
				/*nop*/
				...
				p = Pt(hpos + m, vpos);
				hpos += 2 * m;
				ellipse(screen, scale(p), m / DIV, n / DIV, 0,
				        display->black, ZP);
				break;
			case 'a': /* arc */
				...
				/*
				  ...
				  * center, angle1, delta-angle
				 * counterclockwise.
				 */
				a = hypot(qq.x - q.x, qq.y - q.y);
				phi = atan2(q.y - p.y, p.x - q.x) * 180. / PI;
				alpha =
				    atan2(q.y - qq.y, qq.x - q.x) * 180. / PI -
				    phi;
				if(alpha < 0)
					alpha += 360;
				arc(screen, q, a, a, 0, display->black, ZP, phi,
				    alpha);
				break;
			case '~': /* wiggly line */
				wiggly(0);
		...
		default:
			fprint(2,
			       "unknown input character %o %c at offset %lud\n",
			       c, c, offsetc());
			exits("bad char");
		}
	}
...
static void devcntrl(void) /* interpret device control functions */
{
	char str[80];
	int n;
	...
	case 'r': /* resolution assumed when prepared */
		...
		if(DIV < 1)
			DIV = 1;
		mag = res / (100.0 *
		             DIV); /* adjust mag according to DIV coarseness */
		break;
	case 'f': /* font used */
		n = getn();
		getstr(str);
...
static void getutf(char* s) /* get next utf char, as bytes */
{
	int c, i;
...
static int botpage(int np) /* called at bottom of page np-1 == top of page np */
{
	char* p;
	int n;
	...
	while(p = getcmdstr()) {
		...
		fprint(2, "illegal;  try q, 17, +2, -1, p, m.7, /2, x1, y-.5 "
		          "or return\n");
	}
	return 0;
}
