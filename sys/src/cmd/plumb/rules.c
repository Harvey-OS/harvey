...
struct Input
{
	...
	u8	*s;		/* input string, if from /mnt/plumb/rules */
...
static void
pushinput(char *name, int fd, u8 *str)
{
	Input *in;
	int depth;
...
u8 *
morerules(u8 *text, int done)
{
	int n;
	Ruleset *rs;
	u8 *otext, *s, *endofrule;
	...
	if(input == nil)
		...
		text = (u8*)estrdup((char*)input->end);
	popinput();
	free(otext);
	return text;
}
