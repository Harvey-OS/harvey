...
SmbProcessResult
smbcomtreeconnectandx(SmbSession* s, SmbHeader* h, uint8_t* pdata, SmbBuffer* b)
{
	uint8_t andxcommand;
	uint16_t andxoffset;
	char* path = nil;
	...
	case SmbSessionNeedNegotiate:
		smblogprint(
		    -1,
		    "smbcomtreeconnectandx: called when negotiate expected\n");
	...
	default:
		smblogprint(
		    h->command,
		    "smbcomtreeconnectandx: invalid andxcommand %s (0x%.2ux)\n",
	...
	if(!smbbuffergetbytes(b, nil, passwordlength)) {
		smblogprint(
		    h->command,
		    "smbcomtreeconnectandx: not enough bytes for password\n");
		goto fmtfail;
	}
	...
	if(!smbbuffergetstring(b, h, SMB_STRING_PATH, &path) ||
	   !smbbuffergetstr(b, 0, &service)) {
		smblogprint(
		    h->command,
		    "smbcomtreeconnectandx: not enough bytes for strings\n");
		goto fmtfail;
	}
	...
	if(!smbresponseputs(s, 0) || !smbresponseputstr(s, serv->type) ||
