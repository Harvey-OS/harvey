...
smbnegotiate(SmbSession *s, SmbHeader *h, uint8_t *, SmbBuffer *b)
{
	...
	if(index != 0xffff) {
		...
		h->wordcount = 17;
		if(!smbbufferputheader(s->response, h, nil) || !smbbufferputs(s->response, index) || !smbbufferputb(s->response, 3) /* user security, encrypted */
		...
		s->tzoff = tm->tzoff;
		if(!smbbufferputl(s->response, capabilities) || !smbbufferputv(s->response, nsec() / 100 + (int64_t)10000000 * 11644473600LL) || !smbbufferputs(s->response, -s->tzoff / 60) || !smbbufferputb(s->response, 8)) /* crypt len */
			goto die;
		...
		if(!smbbufferputbytes(s->response, s->cs->chal, s->cs->nchal) || !smbbufferputstring(s->response, nil, SMB_STRING_UNICODE, smbglobals.primarydomain) || !smbbufferfixuprelatives(s->response, bytecountfixupoffset))
			goto die;
	} else {
		h->wordcount = 1;
		if(!smbbufferputheader(s->response, h, nil) || !smbbufferputs(s->response, index) || !smbbufferputs(s->response, 0))
			goto die;
	}
