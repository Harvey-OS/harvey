...
smbnegotiate(SmbSession* s, SmbHeader* h, uint8_t*, SmbBuffer* b)
{
	...
	if(s->state != SmbSessionNeedNegotiate) {
		...
		smblogprint(-1,
		            "smbnegotiate: called when already negotiated\n");
	...
	while(smbbuffergetb(b, &bufferformat)) {
		...
		if(bufferformat != 0x02) {
			smblogprint(-1, "smbnegotiate: unrecognised buffer "
			                "format 0x%.2ux\n",
			            bufferformat);
			return SmbProcessResultFormat;
		}
	...
	if(index != 0xffff) {
		...
		if(!smbbufferputheader(s->response, h, nil) ||
		   ...
		   !smbbufferputl(s->response,
		                  smbglobals.maxreceive) /* max buffer size */
		   ||
		   !smbbufferputl(s->response, 0x10000)        /* max raw */
		   || !smbbufferputl(s->response, threadid())) /* session key */
			goto die;
		/* <= Win2k insist upon this being set to ensure that they
		 * observe the prototol (!) */
		...
		if(!smbbufferputl(s->response, capabilities) ||
		   !smbbufferputv(s->response,
		                  nsec() / 100 +
		                      (int64_t)10000000 * 11644473600LL) ||
		...
		if(!smbbufferputbytes(s->response, s->cs->chal, s->cs->nchal) ||
		   !smbbufferputstring(s->response, nil, SMB_STRING_UNICODE,
