...
deliver(void*, NbDgram* s)
{
	...
	b = smbbufferinit(s->datagram.data, s->datagram.data,
	                  s->datagram.length);
	if(!smbbuffergetandcheckheader(b, &h, SMB_COM_TRANSACTION, 0, &pdata,
	                               &bytecount, &errmsg)) {
		print("ignored: %s\n", errmsg);
		goto done;
	}
	...
	*/
	...
	if(!smbbuffergetb(b, &updatecount) || !smbbuffergetl(b, &periodicity) ||
	   !smbbuffergetstrn(b, 16, &servername) ||
	   !smbbuffergetb(b, &versionmajor) ||
	   !smbbuffergetb(b, &versionminor) || !smbbuffergetl(b, &type) ||
	   !smbbuffergetl(b, &signature) || !smbbuffergetstr(b, &comment)) {
	...
	*/
	print("%s: period %ludms version %d.%d type 0x%.8lux browserversion "
	      "%d.%d comment %s\n",
	      servername, periodicity, versionmajor, versionminor, type,
	      (signature >> 8) & 0xff, signature & 0xff, comment);
done:
	free(errmsg);
	free(comment);
	free(servername);
...
threadmain(int, char**)
{
	...
	for(;;) {
		if(!smbbrowsesendhostannouncement(smbglobals.serverinfo.name,
		                                  3 * 60 * 1000, SV_TYPE_SERVER,
		                                  "Testing testing", &e)) {
