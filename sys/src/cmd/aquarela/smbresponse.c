...
{
	return smbbufferputstring(s->response, &s->peerinfo,
	                          mustalign ? 0 : SMB_STRING_UNALIGNED, string);
}
...
smbresponsesend(SmbSession* s)
{
	...
	smblogdata(cmd, smblogprint, smbbufferreadpointer(s->response),
	           smbbufferreadspace(s->response), 256);
	smblogunlock();
	if(s->nbss) {
		NbScatterGather a[2];
...
{
	return smbbufferputandxheader(s->response, h, &s->peerinfo, andxcommand,
	                              andxoffsetfixupp);
}
