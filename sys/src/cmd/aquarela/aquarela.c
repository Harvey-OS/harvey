...
logset(char* cmd)
{
	...
	for(x = 0; x < smbtrans2optablesize; x++)
		if(smbtrans2optable[x].name &&
		   strcmp(smbtrans2optable[x].name, cmd) == 0) {
			smbtrans2optable[x].debug = 1;
			return;
		}
...
threadmain(int argc, char** argv)
{
	...
	smblogprint(-1, "Aquarela %d.%d running\n", smbglobals.serverinfo.vmaj,
	            smbglobals.serverinfo.vmin);
	for(;;) {
		if(netbios &&
		   !smbbrowsesendhostannouncement(
		       smbglobals.serverinfo.name, 60 * 1000, SV_TYPE_SERVER,
