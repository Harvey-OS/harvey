...
static SmbFile*
openfile(SmbSession* s, SmbTree* t, char* path, uint16_t mode, uint16_t attr,
         uint16_t ofun, uint32_t createoptions, uint64_t createsize,
         uint16_t* fidp, Dir** dp, uint16_t* actionp)
{
	...
	p9mode =
	    (mode >> SMB_OPEN_MODE_ACCESS_SHIFT) & SMB_OPEN_MODE_ACCESS_MASK;
	share = (mode >> SMB_OPEN_MODE_SHARE_SHIFT) & SMB_OPEN_MODE_SHARE_MASK;
	if(share == SMB_OPEN_MODE_SHARE_COMPATIBILITY) {
	badshare:
		// smblogprint(-1, "%s SMB_OPEN_MODE_SHARE_COMPATIBILITY",
		// path);
...
smbcomopenandx(SmbSession* s, SmbHeader* h, uint8_t* pdata, SmbBuffer* b)
{
	...
	pdata += 2;
	attr = smbnhgets(pdata);
	pdata += 2;
	createtime = smbnhgetl(pdata);
	pdata += 4;
	ofun = smbnhgets(pdata);
	pdata += 2;
	createsize = smbnhgetl(pdata);
	pdata += 4;
	...
	switch((mode >> SMB_OPEN_MODE_ACCESS_SHIFT) &
	       SMB_OPEN_MODE_ACCESS_MASK) {
	case OREAD:
	...
	f = openfile(s, t, path, mode, attr, ofun, 0, createsize, &fid, &d,
	             &action);
	if(f == nil) {
	...
	if(andxcommand != SMB_COM_NO_ANDX_COMMAND)
		pr = smbchaincommand(s, h, andxoffsetfixupoffset, andxcommand,
		                     andxoffset, b);
	else
...
smbcomopen(SmbSession* s, SmbHeader* h, uint8_t* pdata, SmbBuffer* b)
{
	...
	f = openfile(s, t, path, mode, attr,
	             SMB_OFUN_EXIST_OPEN << SMB_OFUN_EXIST_SHIFT, 0, 0, &fid,
	             &d, nil);
	if(f == nil)
		goto error;
	h->wordcount = 7;
	if(!smbbufferputheader(s->response, h, &s->peerinfo) ||
	   ...
	   !smbbufferputl(s->response, smbplan9length2size32(d->length)) ||
	   !smbbufferputs(s->response,
	                  2) // lies - this should be the actual access allowed
	   ||
	   !smbbufferputs(s->response, 0))
		pr = SmbProcessResultMisc;
	else
...
smbcomcreate(SmbSession* s, SmbHeader* h, uint8_t* pdata, SmbBuffer* b)
{
	...
	mode =
	    (ORDWR
	     << SMB_OPEN_MODE_ACCESS_SHIFT) | // SFS: FIXME: should be OWRITE?
	    (SMB_OPEN_MODE_SHARE_EXCLUSIVE << SMB_OPEN_MODE_SHARE_SHIFT);
	ofun = SMB_OFUN_NOEXIST_CREATE |
	       (SMB_OFUN_EXIST_FAIL << SMB_OFUN_EXIST_SHIFT);
	f = openfile(s, t, path, mode, attr, ofun, SMB_CO_FILE, 0, &fid, nil,
	             nil);
	if(f == nil) {
		pr = SmbProcessResultError;
		goto done;
	...
	h->wordcount =
	    1; // SFS: FIXME: unsure of this constant, maybe should be 3
	if(!smbbufferputheader(s->response, h, &s->peerinfo) ||
	   !smbbufferputs(s->response, fid) ||
	   !smbbufferputs(s->response, 0)) { // bytecount 0
		pr = SmbProcessResultMisc;
...
static SmbSblut dasblut[] = {
    ...
    {"SMB_DA_STANDARD_READ_CONTROL_ACCESS",
     SMB_DA_STANDARD_READ_CONTROL_ACCESS},
...
static SmbSblut sasblut[] = {{"SMB_SA_SHARE_READ", SMB_SA_SHARE_READ},
...
static SmbSlut cdslut[] = {{"SMB_CD_SUPERCEDE", SMB_CD_SUPERCEDE},
...
smbcomntcreateandx(SmbSession* s, SmbHeader* h, uint8_t* pdata, SmbBuffer* b)
{
	...
	uint32_t extfileattributes, shareaccess, createdisposition,
	    createoptions, impersonationlevel;
	...
	pdata += 2;
	pdata++;
	namelength = smbnhgets(pdata);
	pdata += 2;
	flags = smbnhgetl(pdata);
	pdata += 4;
	rootdirectoryfid = smbnhgetl(pdata);
	pdata += 4;
	desiredaccess = smbnhgetl(pdata);
	pdata += 4;
	allocationsize = smbnhgetv(pdata);
	pdata += 8;
	extfileattributes = smbnhgetl(pdata);
	pdata += 4;
	shareaccess = smbnhgetl(pdata);
	pdata += 4;
	createdisposition = smbnhgetl(pdata);
	pdata += 4;
	createoptions = smbnhgetl(pdata);
	pdata += 4;
	impersonationlevel = smbnhgetl(pdata);
	pdata += 4;
	securityflags = *pdata++;
	USED(pdata);
	...
	smblogprint(h->command, "extfileattributes 0x%.8lux",
	            extfileattributes);
	...
	if(rootdirectoryfid != 0) {
		smblogprint(
		    -1, "smbcomntcreateandx: fid relative not implemented\n");
		goto unimp;
	}
	...
	else if(desiredaccess & SMB_DA_SPECIFIC_READ_DATA)
		if(desiredaccess &
		   (SMB_DA_SPECIFIC_WRITE_DATA | SMB_DA_SPECIFIC_APPEND_DATA))
			p9mode = ORDWR;
		else
			p9mode = OREAD;
	...
	mode = (sharemode << SMB_OPEN_MODE_SHARE_SHIFT) |
	       (p9mode << SMB_OPEN_MODE_ACCESS_SHIFT);

	switch(createdisposition) {
	default:
		smblogprint(-1, "smbcomntcreateandx: createdisposition "
		                "0x%.8lux not implemented\n",
		            createdisposition);
		goto unimp;
	case SMB_CD_OPEN:
	...
	f = openfile(s, t, path, mode, extfileattributes, ofun, createoptions,
	             allocationsize, &fid, &d, &action);
	...
	if(!smbbufferputandxheader(s->response, h, &s->peerinfo, andxcommand,
	   ...
	   !smbbufferputv(
	       s->response,
	       smbl2roundupvlong(d->length, smbglobals.l2allocationsize)) ||
	...
	if(andxcommand != SMB_COM_NO_ANDX_COMMAND)
		pr = smbchaincommand(s, h, andxoffsetfixup, andxcommand,
		                     andxoffset, b);
	else
		pr = SmbProcessResultReply;
