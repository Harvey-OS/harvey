...
{
	return smbglobals.unicode &&
	       (i == nil || (i->capabilities & CAP_UNICODE) != 0);
}
                ...
                uint8_t cmd, uint16_t offset, SmbBuffer* b)
{
	...
	if(ote->process == nil) {
		smblogprint(-1,
		            "smbchaincommand: %s (0x%.2ux) not implemented\n",
		            ote->name, cmd);
	...
	if(!smbbuffergetb(b, &h->wordcount)) {
		smblogprint(
		    -1, "smbchaincommand: not enough space for wordcount\n");
		return SmbProcessResultFormat;
	}
	pdata = smbbufferreadpointer(b);
	if(!smbbuffergetbytes(b, nil, h->wordcount * 2)) {
		smblogprint(
		    -1, "smbchaincommand: not enough space for parameters\n");
		return SmbProcessResultFormat;
	}
	if(!smbbuffergets(b, &bytecount)) {
		smblogprint(
		    -1, "smbchaincommand: not enough space for bytecount\n");
		return SmbProcessResultFormat;
	}
	if(!smbbufferpushreadlimit(b, smbbufferreadoffset(b) + bytecount)) {
		smblogprint(-1,
		            "smbchaincommand: not enough space for bytes\n");
		return SmbProcessResultFormat;
	}
                   ...
                   uint16_t* bytecountp)
{
	...
	if(!smbbuffergetbytes(
	       b, nil, (int32_t)offsetof(SmbRawHeader, parameterwords[0]))) {
	...
	if(h->flags &
	   ~(SmbHeaderFlagCaseless | SMB_FLAGS_SERVER_TO_REDIR |
	     SmbHeaderFlagReserved | SmbHeaderFlagServerIgnore))
		smblogprint(-1,
		            "smbgetheader: warning: unexpected flags 0x%.2ux\n",
		            h->flags);
	...
	if(!smbbuffergetbytes(b, nil, h->wordcount * 2)) {
		smblogprint(
		    -1, "smbgetheader: not enough data for parameter words\n");
		return 0;
	}
	...
	smblogprint(h->command,
	            "%s %s: tid 0x%.4ux pid 0x%.4ux uid 0x%.4ux mid 0x%.4ux\n",
	            ote->name,
	            (h->flags & SMB_FLAGS_SERVER_TO_REDIR) ? "response"
	                                                   : "request",
	            h->tid, h->pid, h->uid, h->mid);
	return 1;
}
...
smbcheckheaderdirection(SmbHeader* h, int response, char** errmsgp)
{
	if(((h->flags & SMB_FLAGS_SERVER_TO_REDIR) == 0) == response) {
		smbstringprint(errmsgp, "unexpected %s",
...
smbcheckheader(SmbHeader* h, uint8_t command, int response, char** errmsgp)
{
	if(response && h->command != command) {
		smbstringprint(errmsgp,
		               "sent %.2uc request, got %.2ux response",
		               command, h->command);
		return 0;
	}
...
smbbuffergetandcheckheader(SmbBuffer* b, SmbHeader* h, uint8_t command,
                           int response, uint8_t** pdatap, uint16_t* bytecountp,
                           char** errmsgp)
{
	if(!smbbuffergetheader(b, h, pdatap, bytecountp)) {
		smbstringprint(
		    errmsgp,
		    "smbbuffergetandcheckheader: not enough data for header");
		return 0;
	}
...
smbsuccess(SmbHeader* h, char** errmsgp)
{
	if(h->errclass != SUCCESS) {
		smbstringprint(errmsgp, "%s returned error %d/%d",
		               smboptable[h->command].name, h->errclass,
		               h->error);
		return 0;
	}
	return 1;
...
smbbufferputheader(SmbBuffer* b, SmbHeader* h, SmbPeerInfo* p)
{
	...
	if(smbbufferwriteoffset(b) == 0) {
		...
		smbhnputs(rh->flags2,
		          BASE_FLAGS |
		              (smbsendunicode(p) ? SMB_FLAGS2_UNICODE : 0));
		memset(rh->extra, 0, sizeof(rh->extra));
		if(!smbbufferputbytes(
		       b, nil, offsetof(SmbRawHeader, parameterwords[0])))
			return 0;
...
int
smbbufferputerror(SmbBuffer* s, SmbHeader* h, SmbPeerInfo* p, uint8_t errclass,
                  uint16_t error)
{
	h->errclass = errclass;
	h->error = error;
...
smbbufferputandxheader(SmbBuffer* b, SmbHeader* h, SmbPeerInfo* p,
                       uint8_t andxcommand, uint32_t* andxoffsetfixupp)
{
	if(!smbbufferputheader(b, h, p) || !smbbufferputb(b, andxcommand) ||
	   !smbbufferputb(b, 0))
		return 0;
...
{
	h->wordcount = 0;
	return smbbufferputheader(b, h, p) && smbbufferputs(b, 0)
...
SmbSlut smbopenmodeslut[] = {{"oread", OREAD},
                             {"owrite", OWRITE},
                             {"ordwr", ORDWR},
                             {"oexec", OEXEC},
                             {0}};
