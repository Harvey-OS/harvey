...
smbcomreadandx(SmbSession *s, SmbHeader *h, uint8_t *pdata, SmbBuffer *b)
{
	...
	if(!smbbufferputbytes(s->response, nil, 6) || !smbbufferfill(s->response, 0, 8)) // reserved
		return SmbProcessResultMisc;
	bytecountfixup = smbbufferwriteoffset(s->response);
	if(!smbbufferputs(s->response, 0) || !smbbufferputb(s->response, 0))
		return SmbProcessResultMisc;
	...
	if(!smbbufferputbytes(s->response, nil, nb) || !smbbufferfixuprelatives(s->response, bytecountfixup) || !smbbufferoffsetputs(s->response, datafixup, nb) || !smbbufferoffsetputs(s->response, datafixup + 4, nb >> 16))
		return SmbProcessResultMisc;
