...
run9fs(char* arg)
{
	...
	free(w);
	smblogprint(SMB_COM_TREE_CONNECT_ANDX,
	            "smbservicefind: 9fs %s executed successfully\n", arg);
	return 0;
}

SmbService*
smbservicefind(SmbSession* s, char* uncpath, char* servicetype,
               uint8_t* errclassp, uint16_t* errorp)
{
	char* p, *q;
	if((uncpath[0] == '/' && uncpath[1] == '/') ||
	   (uncpath[0] == '\\' && uncpath[1] == '\\')) {
		...
		if(q == nil)
			goto bad;
		*q++ = 0;
		//		if (cistrcmp(p, smbglobals.serverinfo.name) !=
		//0)
		//			goto bad;
	} else
		q = uncpath + 1;
