...
SmbProcessResult
smbcomsetinformation2(SmbSession* s, SmbHeader* h, uint8_t* pdata, SmbBuffer*)
{
	...
	smblogprint(h->command, "smbcomsetinformation2: fid 0x%.4ux adate "
	                        "0x%.4ux atime 0x%.4ux mdate 0x%.4ux mtime "
	                        "0x%.4ux\n",
	...
	if(adate || atime || mdate || mtime) {
		// smblogprint(-1, "smbcomsetinformation2: changing times not
		// implemented\n");
		...
		if(!(adate && atime && mdate && mtime)) {
			...
			if(adate || atime) {
				...
				if(!(adate && atime)) {
					...
					smbplan9time2datetime(d.atime, s->tzoff,
					                      &odate, &otime);
					if(adate == 0)
						adate = odate;
					if(atime == 0)
				...
				d.atime = smbdatetime2plan9time(adate, atime,
				                                s->tzoff);
			}
			if(mdate || mtime) {
				...
				if(!(mdate && mtime)) {
					...
					smbplan9time2datetime(d.mtime, s->tzoff,
					                      &odate, &otime);
					if(mdate == 0)
						mdate = odate;
					if(mtime == 0)
				...
				d.mtime = smbdatetime2plan9time(mdate, mtime,
				                                s->tzoff);
			}
			free(od);
		}
...
SmbProcessResult
smbcomsetinformation(SmbSession* s, SmbHeader* h, uint8_t* pdata, SmbBuffer* b)
{
	uint16_t attr;
	uint32_t utime;
	char* name;
