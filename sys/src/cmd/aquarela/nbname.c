...
static int
_nameextract(uint8_t* base, uint8_t* p, uint8_t* ep, int k, uint8_t* outbuf,
             int outbufmaxlen, int* outbuflenp)
{
	...
	for(;;) {
		...
		if((b & 0xc0) == 0xc0) {
			...
			if(ep - p < 2)
				return 0;
			off = nhgets(p - 1) & 0x3fff;
			p++;
			if(_nameextract(base, base + off, p, k, op, oep - ep,
			                &n) == 0)
				return 0;
			op += n;
		} else if((b & 0xc0) != 0x00)
