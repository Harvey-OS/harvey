...
int
smbsharedfilelock(SmbSharedFile* sf, SmbSession* s, uint16_t pid, int64_t base,
                  int64_t limit)
{
	...
	if(sf->locklist) {
		for(l = sf->locklist->head; l; l = l->next)
			if(lockconflict(l, &smblock)) {
				smblogprintif(smbglobals.log.locks,
				              "smbsharedfilelock: lock [%lld, "
				              "%lld) failed because conflicts "
				              "with [%lld, %lld)\n",
	...
	for(lp = &sf->locklist->head; (l = *lp) != nil; lp = &l->next)
		if(lockorder(&smblock, l) <= 0)
			break;
	smblogprintif(smbglobals.log.locks,
	              "smbsharedfilelock: lock [%lld, %lld) succeeded\n", base,
	              limit);
	...
	*lp = nl;
	...
	//	for (l = sf->locklist->head; l; l = l->next)
	//		smblogprintif(smbglobals.log.locks, "smbsharedfilelock:
	//[%lld, %lld)\n", l->base, l->limit);
	//}
	return 1;
}
...
failed:
	smblogprintif(smbglobals.log.locks,
	              "smbsharedfilelock: unlock [%lld, %lld) failed\n", base,
	              limit);
	return 0;
}
...
smbsharedfileget(Dir* d, int p9mode, int* sharep)
{
	...
	for(sfe = sharedfiletable.list; sfe; sfe = sfe->next) {
		if(sfe->type == d->type && sfe->dev == d->dev &&
		   sfe->path == d->qid.path) {
	...
	smblogprintif(smbglobals.log.sharedfiles,
	              "smbsharedfileget: ref %d share %d\n", sfe->ref,
	              sfe->share);
	qunlock(&sharedfiletable);
	return sfe;
}
...
smbsharedfileput(SmbFile* f, SmbSharedFile* sf, int share)
{
	...
	qlock(&sharedfiletable);
	for(sfep = &sharedfiletable.list; (sfe = *sfep) != nil;
	    sfep = &sfe->next) {
		if(sfe == sf) {
			sfe->ref--;
			if(sfe->ref == 0) {
