                       ...
                       SmbBuffer* b)
{
	...
	case SMB_COM_OPEN_PRINT_FILE:
		break;
	default:
		smblogprint(h->command, "smbcomsessionsetupandx: invalid "
		                        "andxcommand %s (0x%.2ux)\n",
	...
	if(!smbbuffergetbytes(b, nil, caseinsensitivepasswordlength +
	                                  casesensitivepasswordlength)) {
		smblogprint(
		    h->command,
		    "smbcomsessionsetupandx: not enough bdata for passwords\n");
		goto fmtfail;
	}
	if(!smbbuffergetstring(b, h, 0, &accountname) ||
	   ...
	   !smbbuffergetstring(b, h, 0, &nativelanman)) {
		smblogprint(
		    h->command,
		    "smbcomsessionsetupandx: not enough bytes for strings\n");
		goto fmtfail;
	}
	...
	if(s->client.accountname && accountname[0] &&
	   strcmp(s->client.accountname, accountname) != 0) {
		smblogprint(h->command, "smbcomsessionsetupandx: more than one "
		                        "user on VC (before %s, now %s)\n",
	...
	if(s->client.accountname == nil) {
		...
		if((casesensitivepasswordlength != 24 ||
		    caseinsensitivepasswordlength != 24)) {
			smblogprint(h->command, "smbcomsessionsetupandx: case "
			                        "sensitive/insensitive "
			                        "password length not 24\n");
		...
		if(auth_chuid(ai, nil) < 0) {
			smblogprint(
			    h->command,
			    "smbcomsessionsetupandx: chuid failed: %r\n");
			auth_freeAI(ai);
		miscerror:
		...
		if(caseinsensitivepasswordlength == 24 &&
		   casesensitivepasswordlength == 24 &&
		   memcmp(&s->client.mschapreply, mschapreply,
		          sizeof(MSchapreply)) != 0) {
			smblogprint(h->command,
			            "second time authentication failed\n");
			smbseterror(s, ERRSRV, ERRbadpw);
			goto errordone;
		}
	}

	/* CIFS says 4 with or without extended security, samba/ms says 3
	 * without */
	h->wordcount = 3;
	...
	if(andxcommand != SMB_COM_NO_ANDX_COMMAND)
		pr = smbchaincommand(s, h, andxfixupoffset, andxcommand,
		                     andxoffset, b);
	else
