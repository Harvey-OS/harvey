...
smbconnect(char* to, char* share, char** errmsgp)
{
	...
	h.flags2 = SMB_FLAGS2_KNOWS_LONG_NAMES | SMB_FLAGS2_IS_LONG_NAME |
	           SMB_FLAGS2_UNICODE;
	h.wordcount = 0;
	h.pid = 42;
	...
	smbbufferoffsetputs(b, bytecountfixupoffset,
	                    smbbufferwriteoffset(b) - bytecountfixupoffset - 2);
	...
	nbdumpdata(smbbufferreadpointer(b), smbbufferwriteoffset(b));
	if(!smbbuffergetandcheckheader(b, &rh, h.command, 1, &pdata, &bytecount,
	                               errmsgp))
		goto fail;
	...
	if(rh.wordcount != 17) {
		smbstringprint(
		    errmsgp,
		    "wrong number of parameters for negotiate response");
		goto fail;
	}
	...
	peerinfo.utc = (int64_t)secssince1970 * (int64_t)1000000000 +
	               (utcintenthsofaus % 10000000) * 100;
	...
	if(auth_respond(peerinfo.encryptionkey, peerinfo.encryptionkeylength,
	                nil, 0, &mschapreply, sizeof(mschapreply), auth_getkey,
	                "proto=mschap role=client server=%s",
	                "cher") != sizeof(mschapreply)) {
	...
	if(share) {
		...
		smbbufferoffsetputs(b, bytecountfixupoffset,
		                    smbbufferwriteoffset(b) -
		                        bytecountfixupoffset - 2);
		nbdumpdata(smbbufferreadpointer(b), smbbufferwriteoffset(b));
		nbsswrite(nbs, smbbufferreadpointer(b),
		          smbbufferwriteoffset(b));
		smbbufferreset(b);
		n = nbssread(nbs, smbbufferwritepointer(b),
		             smbbufferwritespace(b));
		if(n < 0) {
			smbstringprint(errmsgp, "read error: %r");
			goto fail;
		}
		smbbuffersetreadlen(b, n);
		nbdumpdata(smbbufferreadpointer(b), smbbufferwriteoffset(b));
		if(!smbbuffergetandcheckheader(b, &rh, h.command, 3, &pdata,
		                               &bytecount, errmsgp))
			goto fail;
		if(!smbsuccess(&rh, errmsgp))
			goto fail;
...
smbtransactionclientsend(void* magic, SmbBuffer* ob, char**)
{
	...
	smblogdata(-1, smblogprint, smbbufferreadpointer(ob),
	           smbbufferwriteoffset(ob), 256);
	return nbsswrite(c->nbss, smbbufferreadpointer(ob),
	                 smbbufferwriteoffset(ob)) == 0;
}
...
smbtransactionclientreceive(void* magic, SmbBuffer* ib, char**)
{
	...
	SmbClient* c = magic;
	smbbufferreset(ib);
	n = nbssread(c->nbss, smbbufferwritepointer(ib),
	             smbbufferwritespace(ib));
	if(n >= 0) {
