...
tcpreader(void* a)
{
	...
	for(;;) {
		...
		case 0x81: /* session request */ {
			...
			if(k == 0) {
				print("nbss: malformed called name in session "
				      "request\n");
				goto die;
			}
			p += k;
			...
			if(k == 0) {
				print("nbss: malformed calling name in session "
				      "request\n");
				goto die;
			}
			/*
			                        p += k;
			                        if (p != ep) {
			                                print("nbss: extra data
			   at end of session request\n");
			                                goto die;
			                        }
			*/
			called_found = 0;
...
int
nbsslisten(NbName to, NbName from,
           int (*accept)(void* magic, NbSession* s, NBSSWRITEFN** writep),
           void* magic)
{
	Listen* l;
