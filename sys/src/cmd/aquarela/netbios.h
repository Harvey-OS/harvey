...
enum { NbnsRcodeShift = 0,
       NbnsRcodeMask = 0xf,
};

enum { NbnsQuestionTypeNb = 0x0020,
       NbnsQuestionTypeNbStat = 0x0021,
       NbnsQuestionClassIn = 0x0001,
};

enum { NbnsResourceTypeA = 0x0001,
...
NbnsMessageQuestion* nbnsmessagequestionnew(NbName name, ushort type,
                                            ushort class);
...
NbnsMessageResource* nbnsmessageresourcenew(NbName name, ushort type,
                                            ushort class, uint32_t ttl,
                                            int rdcount, uchar* rdata);
...
NbnsMessage* nbnsmessagenamequeryrequestnew(ushort id, int broadcast,
                                            NbName name);
NbnsMessage* nbnsmessagenameregistrationrequestnew(ushort id, int broadcast,
                                                   NbName name, uint32_t ttl,
                                                   uchar* ipaddr);
...
int nbnsfindname(uchar* serveripaddr, NbName name, uchar* ipaddr,
                 uint32_t* ttlp);
int nbnsaddname(uchar* serveripaddr, NbName name, uint32_t ttl, uchar* ipaddr);
...
int nbsslisten(NbName to, NbName from,
               int (*accept)(void* magic, NbSession* s, NBSSWRITEFN** write),
               void* magic);
...
char* nbdgramlisten(NbName to, int (*deliver)(void* magic, NbDgram* s),
                    void* magic);
