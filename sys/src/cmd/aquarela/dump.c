...
nbnsdumpmessagequestion(NbnsMessageQuestion* q)
{
	...
	nbnsdumpname(q->name);
	switch(q->type) {
	case NbnsQuestionTypeNb:
		print(" NB");
		break;
	case NbnsQuestionTypeNbStat:
		print(" NBSTAT");
		break;
	default:
		print(" ???");
...
nbnsdumpmessageresource(NbnsMessageResource* r, char* name)
{
	...
	nbnsdumpname(r->name);
	switch(r->type) {
	case NbnsResourceTypeA:
		print(" A");
		break;
	case NbnsResourceTypeNs:
		print(" NS");
		break;
	case NbnsResourceTypeNull:
		print(" NULL");
		break;
	case NbnsResourceTypeNb:
		print(" NB");
		break;
	case NbnsResourceTypeNbStat:
		print(" NBSTAT");
		break;
	default:
		print(" ???");
...
nbnsdumpmessage(NbnsMessage* s)
{
	...
	print("0x%.4ux %s %s (%d)", s->id, opname(s->opcode),
	      s->response ? "response" : "request", s->opcode);
	if(s->broadcast)
		print(" B");
