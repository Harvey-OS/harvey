...
int
smbclienttrans2findfirst2(SmbClient* c, uint16_t searchcount, char* filename,
                          uint16_t* sidp, uint16_t* searchcountp,
                          uint16_t* endofsearchp,
                          SmbFindFileBothDirectoryInfo* ip, char** errmsgp)
{
	int rv;
	uint16_t setup;
	SmbBuffer* inparam;
	...
	rv = smbclienttrans2(c, 1, &setup, inparam, outparam, outdata, &rh,
	                     errmsgp);
	...
	if(rv) {
		...
		if(!smbbuffergets(outparam, sidp) ||
		   ...
		   !smbbuffergets(outparam, &lastnameoffset)) {
			smbstringprint(errmsgp, "smbclienttrans2findfirst2: "
		...
		smblogdata(-1, smblogprint, smbbufferreadpointer(outdata),
		           smbbufferreadspace(outdata), 256);
		for(i = 0; i < *searchcountp; i++) {
			...
			underflow:
				smbstringprint(errmsgp, "smbclientrans2findfirs"
				                        "t2: not enough data "
				                        "returned");
				rv = 0;
				goto done;
			...
			if(!smbbuffergetl(outdata, &info->extfileattributes) ||
			   ...
			   !smbbuffergetstring(outdata, &rh, SMB_STRING_REVPATH,
			                       &info->filename))
				goto underflow;
