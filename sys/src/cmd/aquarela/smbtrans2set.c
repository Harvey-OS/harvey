...
smbtrans2setfileinformation(SmbSession* s, SmbHeader* h)
{
	...
	default:
		smblogprint(-1, "smbtrans2setfileinformation: infolevel "
...
smbtrans2setpathinformation(SmbSession* s, SmbHeader* h)
{
	...
	case SMB_INFO_STANDARD:
		...
		translogprint(s->transaction.in.setup[0], "adate %d atime %d",
		              adate, atime);
		translogprint(s->transaction.in.setup[0], "mdate %d mtime %d\n",
		              mdate, mtime);

		if(size || adate || atime || mdate || mtime ||
		   mode != 0xffffffff) {
			Dir d;
			memset(&d, 0xff, sizeof(d));
			d.name = d.uid = d.gid = d.muid = nil;
	...
	default:
		smblogprint(-1, "smbtrans2setpathinformation: infolevel "
		                "0x%.4ux not implemented\n",
		            infolevel);
		smbseterror(s, ERRDOS, ERRunknownlevel);
		pr = SmbProcessResultError;
