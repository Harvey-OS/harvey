      ...
      int64_t cbo, Dir* d)
{
	...
	case SMB_QUERY_FILE_BASIC_INFO:
		...
		translogprint(s->transaction.in.setup[0],
		              "SMB_QUERY_FILE_BASIC_INFO\n");
		translogprint(s->transaction.in.setup[0], "REPLY:\n");
		translogprint(s->transaction.in.setup[0], "atime=%s",
		              ctime(d->atime));
		translogprint(s->transaction.in.setup[0], "atime=%s",
		              ctime(d->mtime));
		translogprint(s->transaction.in.setup[0],
		              "mode=0%o -> dosmode=0x%x\n", d->mode, dosmode);
	...
	case SMB_QUERY_FILE_ALL_INFO:
		...
		dosmode = smbplan9mode2dosattr(d->mode);
		allocsize =
		    (d->length + (1 << smbglobals.l2allocationsize) - 1) &
		    ~((1 << smbglobals.l2allocationsize) - 1);
		...
		if(!smbbufferputv(s->transaction.out.data, ntmtime) ||
		   ...
		   ||
		   !smbbufferputb(s->transaction.out.data,
		                  0) // TODO delete pending
		   ||
		   !smbbufferputb(s->transaction.out.data,
		                  (d->mode & DMDIR) != 0) ||
		   ...
		   ||
		   !smbbufferputl(s->transaction.out.data,
		                  (dosmode & SMB_ATTR_READ_ONLY) ? 0xa1
		                                                 : 0x1a7) ||
		...
		if(!smbbufferputl(s->transaction.out.data, 0) ||
		   !smbbufferputstring(s->transaction.out.data, &s->peerinfo,
		                       SMB_STRING_REVPATH, filename) ||
		   !smbbufferfixuprelativel(s->transaction.out.data,
		                            fnlfixupoffset))
			return SmbProcessResultMisc;
		break;
	case SMB_QUERY_FILE_STANDARD_INFO:
		...
		translogprint(s->transaction.in.setup[0], "REPLY:\n");
		translogprint(s->transaction.in.setup[0], "length=%lld",
		              d->length);
		translogprint(s->transaction.in.setup[0], "isdir=%d\n",
		              (d->qid.type & QTDIR) != 0);
		...
		if(!smbbufferputv(
		       ...
		       smbl2roundupvlong(d->length,
		                         smbglobals.l2allocationsize)) ||
		   !smbbufferputv(s->transaction.out.data, d->length) ||
	...
	case SMB_QUERY_FILE_STREAM_INFO:
		...
		goto unknownlevel;
	default:
		smblogprint(-1, "smbtrans2query%sinformation: infolevel "
		                "0x%.4ux not implemented\n",
		            cmdname, infolevel);
	unknownlevel:
...
smbtrans2queryfsinformation(SmbSession* s, SmbHeader* h)
{
	...
	case SMB_INFO_ALLOCATION:
		...
		if(!smbbufferputl(s->transaction.out.data, 0) ||
		   !smbbufferputl(s->transaction.out.data,
		                  1 << (smbglobals.l2allocationsize -
		                        smbglobals.l2sectorsize)) ||
		   !smbbufferputl(s->transaction.out.data, 0xffffffff) ||
		   !smbbufferputl(s->transaction.out.data, 0xffffffff) ||
		   !smbbufferputs(s->transaction.out.data,
		                  1 << smbglobals.l2sectorsize))
			goto misc;
		break;
	case SMB_INFO_VOLUME:
		translogprint(s->transaction.in.setup[0], "SMB_INFO_VOLUME\n");
		if(!smbbufferputl(s->transaction.out.data, 0xdeadbeef) ||
		   !smbbufferputstring(s->transaction.out.data, &s->peerinfo, 0,
		                       t->serv->name))
			goto misc;
		break;
	case SMB_QUERY_FS_VOLUME_INFO:
		...
		if(!smbbufferputstring(s->transaction.out.data, &s->peerinfo, 0,
		                       t->serv->name) ||
		   !smbbufferfixupl(
		       s->transaction.out.data, fixup,
	...
	case SMB_QUERY_FS_ATTRIBUTE_INFO:
		...
		if(!smbbufferputl(s->transaction.out.data, 0) ||
		   !smbbufferputstring(s->transaction.out.data, &s->peerinfo,
		                       SMB_STRING_UNTERMINATED,
		                       smbglobals.serverinfo.nativelanman) ||
		   !smbbufferfixuprelativel(s->transaction.out.data, fixup))
			goto misc;
		break;
	default:
		smblogprint(-1, "smbtrans2queryfsinformation: infolevel "
		                "0x%.4ux not implemented\n",
		            infolevel);
		smbseterror(s, ERRDOS, ERRunknownlevel);
		pr = SmbProcessResultError;
