...
struct RapTableEntry {
	char* name;
	SmbProcessResult (*procedure)(SmbBuffer* inparam, SmbBuffer* outparam,
	                              SmbBuffer* outdata);
};
...
serverinfoput(SmbBuffer* b, uint16_t level, void* data)
{
	...
	if(!smbbufferputstrn(b, si->name, 16, 1))
		return 0;
	if(level > 0) {
		if(!smbbufferputb(b, si->vmaj) || !smbbufferputb(b, si->vmin) ||
		   !smbbufferputl(b, si->stype) || !smbbufferputl(b, 0))
			return 0;
	}
	if(level > 1)
		return 0;
...
InfoMethod serverinfo = {
    serverinfosize, serverinfoput, serverinfoputstrings, serverinfoenumerate,
};
...
shareinfoput(SmbBuffer* b, uint16_t level, void* data)
{
	...
	if(!smbbufferputstrn(b, serv->name, 13, 0))
		return 0;
	if(level > 0) {
		if(!smbbufferputb(b, 0) || !smbbufferputs(b, serv->stype) ||
		   !smbbufferputl(b, 0))
			return 0;
	}
	if(level > 1) {
		if(!smbbufferputs(b, 7) || !smbbufferputs(b, -1) ||
		   !smbbufferputs(b, serv->ref) || !smbbufferputl(b, 0) ||
...
static InfoMethod shareinfo = {
    shareinfosize, shareinfoput, shareinfoputstrings, shareinfoenumerate,
};
          ...
          uint16_t level, void* magic)
{
	...
	if(!smbbufferputs(outparam, sentthings < totalthings
	                                ? SMB_RAP_ERROR_MORE_DATA
	                                : SMB_RAP_NERR_SUCCESS) ||
             ...
             uint16_t level, void* thing)
{
	...
	if(totalbytes <= smbbufferspace(outdata)) {
		...
		moredata = 1;
	if(!smbbufferputs(outparam, moredata ? SMB_RAP_ERROR_MORE_DATA
	                                     : SMB_RAP_NERR_SUCCESS) ||
	   !smbbufferputs(outparam, 0) || !smbbufferputs(outparam, totalbytes))
		return SmbProcessResultFormat;
	return SmbProcessResultReply;
}
...
netshareenum(SmbBuffer* inparam, SmbBuffer* outparam, SmbBuffer* outdata)
{
	...
	/* WrLeh */
	/* ushort sLevel, RCVBUF pbBuffer, RCVBUFLEN cbBuffer, ENTCOUNT
	 * pcEntriesRead, ushort *pcTotalAvail */

	if(!smbbuffergets(inparam, &level))
		return SmbProcessResultFormat;

	smblogprintif(smbglobals.log.rap2, "netshareenum(%lud, %lud)\n", level,
	              smbbufferwritespace(outdata));
...
netserverenum2(SmbBuffer* inparam, SmbBuffer* outparam, SmbBuffer* outdata)
{
	...
	*/
	...
	if(!smbbuffergets(inparam, &level) || !smbbuffergets(inparam, &rbl) ||
	...
	smblogprintif(smbglobals.log.rap2,
	              "netserverenum2(%lud, %lud, 0x%.8lux, %s)\n", level,
	              smbbufferwritespace(outdata), servertype, domain);
	...
	if((servertype & SV_TYPE_LOCAL_LIST_ONLY) != 0 &&
	   (servertype & SV_TYPE_DOMAIN_ENUM) == 0)
	...
	if((servertype & SV_TYPE_SERVER) != 0 &&
	   (domain[0] == 0 ||
	    cistrcmp(domain, smbglobals.primarydomain) == 0)) {
...
netsharegetinfo(SmbBuffer* inparam, SmbBuffer* outparam, SmbBuffer* outdata)
{
	...
	/*
	 * zWrLh
	 * char *pszNetName, ushort sLevel, RCVBUF pbBuffer, RCVBUFLEN cbBuffer,
	 * ushort *pcbTotalAvail
	*/
...
netservergetinfo(SmbBuffer* inparam, SmbBuffer* outparam, SmbBuffer* outdata)
{
	...
	/* WrLh
	 * ushort sLevel, RCVBUF pbBuffer, RCVBUFLEN cbBuffer, ushort
	 * *pcbTotalAvail
	*/
	...
	pr = onethingfill(outparam, outdata, &shareinfo, level,
...
netwkstagetinfo(SmbBuffer* inparam, SmbBuffer* outparam, SmbBuffer* outdata)
{
	...
	*/
	...
	usefulbytes = 22 + smbstrlen(smbglobals.serverinfo.name) +
	              smbstrlen(getuser()) +
	...
	assert(smbbufferputstring(outdata, nil, SMB_STRING_ASCII,
	                          smbglobals.serverinfo.name));
...
smbrap2(SmbSession* s)
{
	...
	inparam = smbbufferinit(s->transaction.in.parameters,
	                        s->transaction.in.parameters,
	                        s->transaction.in.tpcount);
	if(!smbbuffergets(inparam, &pno) ||
	   !smbbuffergetstrinline(inparam, &pstring) ||
	   !smbbuffergetstrinline(inparam, &dstring)) {
		smblogprintif(smbglobals.log.rap2,
		              "smbrap2: not enough parameters\n");
		pr = SmbProcessResultFormat;
		goto done;
	}
	...
	e = raptable + pno;
	pr = (*e->procedure)(inparam, s->transaction.out.parameters,
	                     s->transaction.out.data);
done:
