...
serverinfoput(SmbBuffer *b, uint16_t level, void *data)
{
	...
	if(!smbbufferputstrn(b, si->name, 16, 1))
		return 0;
	if(level > 0) {
		if(!smbbufferputb(b, si->vmaj) || !smbbufferputb(b, si->vmin) || !smbbufferputl(b, si->stype) || !smbbufferputl(b, 0))
			return 0;
	}
	if(level > 1)
		return 0;
...
serverinfoputstrings(SmbBuffer *b, uint16_t level, int instance, void *data)
{
	SmbServerInfo *si = data;
	if(level == 1) {
		if(!smbbufferfixupabsolutel(b, instance * 26 + 22) || !smbbufferputstring(b, nil, SMB_STRING_ASCII, si->remark))
			return 0;
	}
	return 1;
}
...
shareinfoput(SmbBuffer *b, uint16_t level, void *data)
{
	...
	if(!smbbufferputstrn(b, serv->name, 13, 0))
		return 0;
	if(level > 0) {
		if(!smbbufferputb(b, 0) || !smbbufferputs(b, serv->stype) || !smbbufferputl(b, 0))
			return 0;
	}
	if(level > 1) {
		if(!smbbufferputs(b, 7) || !smbbufferputs(b, -1) || !smbbufferputs(b, serv->ref) || !smbbufferputl(b, 0) || !smbbufferfill(b, 0, 10))
			return 0;
	}
	if(level > 2)
		return 0;
...
shareinfoputstrings(SmbBuffer *b, uint16_t level, int instance, void *data)
{
	...
	case 0:
		break;
	case 1:
		if(!smbbufferfixupabsolutel(b, instance * 20 + 16) || !smbbufferputstring(b, nil, SMB_STRING_ASCII, serv->remark))
			return 0;
		break;
	case 2:
		if(!smbbufferfixupabsolutel(b, instance * 40 + 16) || !smbbufferputstring(b, nil, SMB_STRING_ASCII, serv->remark) || !smbbufferfixupabsolutel(b, instance * 40 + 26) || !smbbufferputstring(b, nil, SMB_STRING_ASCII, serv->path))
			return 0;
		break;
	default:
		return 0;
	  ...
	  uint16_t level, void *magic)
{
	...
	if(!smbbufferputs(outparam, sentthings < totalthings ? SMB_RAP_ERROR_MORE_DATA : SMB_RAP_NERR_SUCCESS) || !smbbufferputs(outparam, 0) || !smbbufferputs(outparam, totalthings) || !smbbufferputs(outparam, sentthings))
	     ...
	     uint16_t level, void *thing)
{
	...
	if(totalbytes <= smbbufferspace(outdata)) {
		...
		moredata = 1;
	if(!smbbufferputs(outparam, moredata ? SMB_RAP_ERROR_MORE_DATA : SMB_RAP_NERR_SUCCESS) || !smbbufferputs(outparam, 0) || !smbbufferputs(outparam, totalbytes))
		return SmbProcessResultFormat;
	return SmbProcessResultReply;
}
...
netserverenum2(SmbBuffer *inparam, SmbBuffer *outparam, SmbBuffer *outdata)
{
	...
	*/
	...
	if(!smbbuffergets(inparam, &level) || !smbbuffergets(inparam, &rbl) || !smbbuffergetl(inparam, &servertype) || !smbbuffergetstr(inparam, 0, &domain)) {
	fmtfail:
	...
	if((servertype & SV_TYPE_SERVER) != 0 && (domain[0] == 0 || cistrcmp(domain, smbglobals.primarydomain) == 0)) {
...
netsharegetinfo(SmbBuffer *inparam, SmbBuffer *outparam, SmbBuffer *outdata)
{
	...
	*/
	...
	if(!smbbuffergetstrinline(inparam, &netname) || !smbbuffergets(inparam, &level)) {
	fmtfail:
		pr = SmbProcessResultFormat;
		goto done;
	}
...
netwkstagetinfo(SmbBuffer *inparam, SmbBuffer *outparam, SmbBuffer *outdata)
{
	...
	*/
	...
	usefulbytes = 22 + smbstrlen(smbglobals.serverinfo.name) + smbstrlen(getuser()) + 3 * smbstrlen(smbglobals.primarydomain);
	...
	if(!smbbufferputs(outparam, moredata ? SMB_RAP_ERROR_MORE_DATA : SMB_RAP_NERR_SUCCESS) || !smbbufferputs(outparam, 0) || !smbbufferputs(outparam, usefulbytes)) {
		pr = SmbProcessResultFormat;
		goto done;
	}
...
smbrap2(SmbSession *s)
{
	...
	if(!smbbuffergets(inparam, &pno) || !smbbuffergetstrinline(inparam, &pstring) || !smbbuffergetstrinline(inparam, &dstring)) {
