...
enum { OffsetSize = 4, /* size of block offset */
};
...
main(int argc, char* argv[])
{
	...
	case 'b':
		...
		if(blocksize < MinBlockSize)
			sysfatal("blocksize too small: must be at lease %d",
			         MinBlockSize);
		if(blocksize > MaxBlockSize)
			sysfatal(
			    "blocksize too large: must be no greater than %d",
			    MaxBlockSize);
		break;
	}
	ARGEND
...
paqDir(char* name, Dir* dir)
{
	...
	for(i = 0; i < ndir; i++) {
		...
		if(n + paqDirSize(pd) >= blocksize) {
			...
			n = 0;
			if(nb >= blocksize / OffsetSize) {
				warn("directory too big for blocksize: %s",
				     nname);
				goto Err;
			}
			putl(pointer + nb * OffsetSize, offset);
		...
		if(n + paqDirSize(pd) >= blocksize) {
			warn("directory entry does not fit in a block: %s",
			     nname);
			paqDirFree(pd);
			continue;
		}
