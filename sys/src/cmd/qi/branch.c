...
dobranch(uint32_t ir, uint32_t* r, int ctr)
{
	...
	if(condok(ir, ctr)) {
		...
		nia = reg.pc + 4;
	if(trace)
		itrace("%s%s\t%s,%s,#%.8lux", ci->name, ir & 1 ? "l" : "",
		       boname(bo), cname(bo, bi), nia);
	if(ir & 1) {
		call(nia);
		reg.lr = reg.pc + 4;
	}
...
bcx(uint32_t ir)
{
	...
	if((ir & 2) == 0) { /* not absolute address */
		ea = reg.pc + imm;
		if(trace)
			itrace("%s\t%s,%s,.%s%ld\tea = #%.8lux", opc[ir & 3],
			       boname(bo), cname(bo, bi), imm < 0 ? "" : "+",
			       imm, ea);
	} else {
		ea = imm;
		if(trace)
			itrace("%s\t%s,%s,#%.8lux", opc[ir & 3], boname(bo),
			       cname(bo, bi), ea);
	}
...
bx(uint32_t ir)
{
	...
	if((ir & 2) == 0) { /* not absolute address */
		ea = reg.pc + imm;
		if(trace)
			itrace("%s\t.%s%ld\tea = #%.8lux", opc[ir & 3],
			       imm < 0 ? "" : "+", imm, ea);
	} else {
		ea = imm;
		if(trace)
