...
initmap(void)
{
	...
	iprof = emalloc(iprofsize * sizeof(int32_t));
	textbase = s->base;
...
uint32_t roff[] = {
    REGOFF(r0),  REGOFF(r1),  REGOFF(r2),  REGOFF(r3),  REGOFF(r4),
    REGOFF(r5),  REGOFF(r6),  REGOFF(r7),  REGOFF(r8),  REGOFF(r9),
    REGOFF(r10), REGOFF(r11), REGOFF(r12), REGOFF(r13), REGOFF(r14),
    REGOFF(r15), REGOFF(r16), REGOFF(r17), REGOFF(r18), REGOFF(r19),
    REGOFF(r20), REGOFF(r21), REGOFF(r22), REGOFF(r23), REGOFF(r24),
    REGOFF(r25), REGOFF(r26), REGOFF(r27), REGOFF(r28), REGOFF(r29),
    REGOFF(r30), REGOFF(r31),
};
...
procinit(int pid)
{
	...
	if(s->base != vastart || s->end != vaend) {
		...
		free(s->table);
		s->table =
		    malloc(((s->end - s->base) / BY2PG) * sizeof(uint8_t*));
	}
...
dumpreg(void)
{
	...
	Bprint(bioout, "PC  #%-8lux SP  #%-8lux CR #%-8lux LR #%-8lux CTR "
	               "#%-8lux XER #%-8lux\n",
