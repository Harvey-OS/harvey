...
Inst op59[] = {
        [18]{fariths, "fdivs", Ifloat},   [20]{fariths, "fsubs", Ifloat},
        [21]{fariths, "fadds", Ifloat},   [22]{unimp, "fsqrts", Ifloat},
        [24]{unimp, "fres", Ifloat},      [25]{fariths, "fmuls", Ifloat},
        [28]{fariths, "fmsubs", Ifloat},  [29]{fariths, "fmadds", Ifloat},
        [30]{fariths, "fnmsubs", Ifloat}, [31]{fariths, "fnmadds", Ifloat},
};

Inset ops59 = {op59, nelem(op59)};

Inst op63a[] = {
        [12]{farith, "frsp", Ifloat},   [14]{farith, "fctiw", Ifloat},
        [15]{farith, "fctiwz", Ifloat}, [18]{farith, "fdiv", Ifloat},
        [20]{farith, "fsub", Ifloat},   [21]{farith, "fadd", Ifloat},
        [22]{unimp, "frsqrt", Ifloat},  [23]{unimp, "fsel", Ifloat},
        [25]{farith, "fmul", Ifloat},   [26]{unimp, "frsqrte", Ifloat},
        [28]{farith, "fmsub", Ifloat},  [29]{farith, "fmadd", Ifloat},
        [30]{farith, "fnmsub", Ifloat}, [31]{farith, "fnmadd", Ifloat},
};

Inset ops63a = {op63a, nelem(op63a)};

Inst op63b[] = {
        [0]{fcmp, "fcmpu", Ifloat},      [32]{fcmp, "fcmpo", Ifloat},
        [38]{mtfsb1, "mtfsb1", Ifloat},  [40]{farith2, "fneg", Ifloat},
        [64]{mcrfs, "mcrfs", Ifloat},    [70]{mtfsb0, "mtfsb0", Ifloat},
        [72]{farith2, "fmr", Ifloat},    [134]{mtfsfi, "mtfsfi", Ifloat},
        [136]{farith2, "fnabs", Ifloat}, [264]{farith2, "fabs", Ifloat},
        [583]{mffs, "mffs", Ifloat},     [711]{mtfsf, "mtfsf", Ifloat},
};
...
lfsx(uint32_t ir)
{
	...
	if(ra) {
		...
		if(upd)
			reg.r[ra] = ea;
		if(trace)
			itrace("%s\tf%d,(r%d+r%d) ea=%lux", ci->name, rd, ra,
			       rb, ea);
	} else {
		if(upd)
			undef(ir);
...
stfs(uint32_t ir)
{
	...
	if(trace)
		itrace("%s\tf%d,%ld(r%d) %lux=%g", ci->name, rd, imm, ra, ea,
		       reg.fd[rd]);
	u.f =
	    reg.fd[rd]; /* BUG: actual PPC conversion is more subtle than this
	                   */
	putmem_w(ea, u.w);
}
...
stfsx(uint32_t ir)
{
	...
	if(ra) {
		...
		if(upd)
			reg.r[ra] = ea;
		if(trace)
			itrace("%s\tf%d,(r%d+r%d) %lux=%g", ci->name, rd, ra,
			       rb, ea, (float)reg.fd[rd]);
	} else {
		if(upd)
			undef(ir);
		if(trace)
			itrace("%s\tf%d,(r%d) %lux=%g", ci->name, rd, rb, ea,
			       (float)reg.fd[rd]);
	}
...
mcrfs(uint32_t ir)
{
	...
	static uint32_t fpscr0[] = {
	    FPS_FX | FPS_OX, FPS_UX | FPS_ZX | FPS_XX | FPS_VXSNAN,
	    FPS_VXISI | FPS_VXIDI | FPS_VXZDZ | FPS_VXIMZ, FPS_VXVC, 0,
	    FPS_VXCVI,
	};
...
mtfsb1(uint32_t ir)
{
	...
	if(ir & 1)
		reg.cr &=
		    ~mkCR(
		        1,
		        0xE); /* BUG: manual unclear: leave SO, reset others? */
	if(trace)
...
mtfsf(uint32_t ir)
{
	...
	for(i = 0; i < 8; i++)
		if(fm & (1 << (7 - i)))
			reg.fpscr =
			    (reg.fpscr & ~mkCR(i, 0xF)) | mkCR(i, getCR(i, v));
...
fcmp(uint32_t ir)
{
	...
	case 32:
		...
		if(isNaN(reg.fd[ra]) ||
		   isNaN(reg.fd[rb])) { /* BUG: depends whether quiet or
		                           signalling ... */
			fc = CRFU;
...
fariths(uint32_t ir)
{
	...
	if(trace) {
		switch(fmt) {
		case 0:
			itrace("%s%s\tfr%d,fr%d,fr%d", ci->name, cc, rd, ra,
			       rb);
			break;
		case 1:
			...
			break;
		case 2:
			itrace("%s%s\tfr%d,fr%d,fr%d,fr%d", ci->name, cc, rd,
			       ra, rc, rb);
			break;
		}
	}
...
setfpcc(double r)
{
	...
	if(isNaN(r))
		c |= 1;
	reg.fpscr = (reg.fpscr & ~0xF800) | (0 << 15) |
	            (c << 11); /* unsure about class bit */
}
