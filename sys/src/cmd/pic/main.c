...
main(int argc, char* argv[])
{
	...
	setdefaults();
	objlist = (obj**)grow((char*)objlist, "objlist", nobjlist += 1000,
	                      sizeof(obj*));
	text =
	    (Text*)grow((char*)text, "text", ntextlist += 1000, sizeof(Text));
...
} defaults[] = {"scale", SCALE, 1, "lineht", HT, 1, "linewid", HT, 1, "moveht",
                HT, 1, "movewid", HT, 1, "dashwid", HT10, 1, "boxht", HT, 1,
                "boxwid", WID, 1, "circlerad", HT2, 1, "arcrad", HT2, 1,
                "ellipseht", HT, 1, "ellipsewid", WID, 1, "arrowht", HT5, 1,
                "arrowwid", HT10, 1, "arrowhead", 2, 0, /* arrowhead style */
                "textht", 0.0, 1, /* 6 lines/inch is also a useful value */
                "textwid", 0.0, 1, "maxpsht", MAXHT, 0, "maxpswid", MAXWID, 0,
	...
	while(fgets(buf, sizeof buf, curfile->fin) != NULL) {
		...
		if(*buf == '.' && *(buf + 1) == 'P' && *(buf + 2) == 'S') {
			...
			if(buf[3] == ' ') { /* next things are wid & ht */
				if(sscanf(&buf[4], "%lf %lf", &deltx, &delty) <
				   2)
