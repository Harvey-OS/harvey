                        ...
                        /* message if too big */
{
	...
	for(p = buf;;) {
		...
		if(c == EOF)
			ERROR "end of file in %s %c %.20s... %c", s, delim, buf,
			    delim FATAL;
		*p++ = c;
	}
	*p = '\0';
...
void copydef(struct symtab* p) /* remember macro symtab ptr */ { thrudef = p; }
...
void copyuntil(char* s) /* string that terminates a thru */ { untilstr = s; }
