...
print(void)
{
	...
	for(i = 0; i < nobj; i++) {
		...
		case ARC:
			...
			if(p->o_attr & HEAD1)
				arrow(x1 - (y1 - oy), y1 + (x1 - ox), x1, y1,
				      p->o_val[4], p->o_val[5],
				      p->o_val[5] / p->o_val[6] / 2,
				      p->o_nhead);
			if(invis && !fill)
			...
			if(p->o_attr & HEAD2)
				arrow(p->o_val[2] + p->o_val[3] - oy,
				      p->o_val[3] - (p->o_val[2] - ox),
				      p->o_val[2], p->o_val[3], p->o_val[4],
				      p->o_val[5],
				      -p->o_val[5] / p->o_val[6] / 2,
				      p->o_nhead);
			if(fill)
		...
		case SPLINE:
			...
			if(vis && p->o_attr & HEAD1)
				arrow(ox + p->o_val[5], oy + p->o_val[6], ox,
				      oy, p->o_val[2], p->o_val[3], 0.0,
				      p->o_nhead);
			if(invis && !fill)
				move(x1, y1);
			else if(p->o_type == SPLINE)
				spline(ox, oy, p->o_val[4], &p->o_val[5],
				       p->o_attr & (DOTBIT | DASHBIT),
				       p->o_ddval);
			else {
				dx = ox;
				dy = oy;
				for(k = 0, j = 5; k < p->o_val[4];
				    k++, j += 2) {
					ndx = dx + p->o_val[j];
					ndy = dy + p->o_val[j + 1];
...
void dotbox(double x0, double y0, double x1, double y1, int ddtype,
            double ddval) /* dotted or dashed box */
{
