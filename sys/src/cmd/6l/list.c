...
Pconv(Fmt* fp)
{
	...
	case ATEXT:
		if(p->from.scale) {
			sprint(str, "(%ld)	%A	%D,%d,%D", p->line,
			       p->as, &p->from, p->from.scale, &p->to);
			break;
		}
	default:
		sprint(str, "(%ld)	%A	%D,%D", p->line, p->as,
		       &p->from, &p->to);
		break;
	case ADATA:
	case AINIT:
	case ADYNT:
		sprint(str, "(%ld)	%A	%D/%d,%D", p->line, p->as,
...
Dconv(Fmt* fp)
{
	...
	case D_STATIC:
		sprint(str, "%s<%d>+%lld(SB)", a->sym->name, a->sym->version,
		       a->offset);
		break;
	...
	case D_PARAM:
		if(a->sym)
			sprint(str, "%s+%lld(%s)", a->sym->name, a->offset,
			       paramspace);
		else
...
char* regstr[] = {
    "AL", /* [D_AL] */
    "CL",   "DL",   "BL",   "SPB",  "BPB",  "SIB",  "DIB",  "R8B",
    "R9B",  "R10B", "R11B", "R12B", "R13B", "R14B", "R15B",

    "AX", /* [D_AX] */
    "CX",   "DX",   "BX",   "SP",   "BP",   "SI",   "DI",   "R8",
