...
enum { Maxenvname = 256, /* undocumented limit */
};

char* Signame[] = {"sigexit", "sighup", "sigint",  "sigquit", "sigalrm",
                   "sigkill", "sigfpe", "sigterm", 0};
char* syssigname[] = {"exit",                        /* can't happen */
                      "hangup", "interrupt", "quit", /* can't happen */
                      "alarm",  "kill",      "sys: fp: ", "term", 0};
...
builtin Builtin[] = {"cd",     execcd,   "whatis", execwhatis,  "eval",
                     execeval, "exec",   execexec, /* but with popword first */
                     "exit",   execexit, "shift",  execshift,   "wait",
                     execwait, ".",      execdot,  "finit",     execfinit,
                     "flag",   execflag, "rfork",  execnewpgrp, 0};
			...
			if(len && strncmp(ent[i].name, "fn#", 3) != 0) {
				...
				if((f = open(envname, 0)) >= 0) {
					...
					if(n <= 0)
						...
						buf[n] = '\0';
					val = 0;
					/* Charitably add a 0 at the end if need
					 * be */
					if(buf[len - 1])
						buf[len++] = '\0';
					s = buf + len - 1;
