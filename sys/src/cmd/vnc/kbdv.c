...
enum { Xshift = 0xFFE1, Xctl = 0xFFE3, Xmeta = 0xFFE7, Xalt = 0xFFE9 };
	...
	for(;;) {
		...
		default:
			if(r == ks && r < 0x1A) { /* control key */
				 ...
				 */
				if(!shift && r == ks && r < sizeof shiftkey &&
				   shiftkey[r]) {
					shift = 1;
					keyevent(v, Xshift, 1);
				}
				 ...
				 */
				if((ks & 0xff00) && ks < nelem(utf2ksym) &&
				   utf2ksym[ks] != 0)
					ks = utf2ksym[ks];
				keyevent(v, ks, 1);
				/*
				 * up event needed by vmware inside linux vnc
				 * server,
				 * perhaps others.
				 */
				keyevent(v, ks, 0);
