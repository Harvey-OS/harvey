...
enum { RGB12 = CHAN4(CIgnore, 4, CRed, 4, CGreen, 4, CBlue, 4),
...
choosecolor(Vnc* v)
{
	...
	if(verbose)
		fprint(2, "%d bpp, %d depth, 0x%lx chan, %d truecolor, %d "
		          "bigendian\n",
		       v->bpp, v->depth, screen->chan, v->truecolor,
		       v->bigendian);
