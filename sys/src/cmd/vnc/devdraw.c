...
Point
drawchar(Memimage* dst, Point p, Memimage* src, Point* sp, DImage* font,
         int index, int op)
{
	FChar* fc;
	Rectangle r;
...
initscreenimage(void)
{
	...
	screendata.base = nil;
	screendata.bdata =
	    attachscreen(&r, &chan, &depth, &width, &sdraw.softscreen);
...
drawread(Chan* c, void* a, int32_t n, int64_t off)
{
	...
	case Qrefresh:
		...
		for(;;) {
			...
			qunlock(&sdraw);
			if(waserror()) {
				qlock(&sdraw); /* restore lock for waserror()
				                  above */
				nexterror();
			}
...
drawmesg(Client* client, void* av, int n)
{
	...
	while((n -= m) > 0) {
		...
		case 'r':
			...
			client->nreaddata =
			    unloadmemimage(i, r, client->readdata, c);
...
Dev drawdevtab = {
    ...
    devreset,  devinit,  drawattach, drawwalk,  drawstat,  drawopen,  devcreate,
    drawclose, drawread, devbread,   drawwrite, devbwrite, devremove, devwstat,
};
