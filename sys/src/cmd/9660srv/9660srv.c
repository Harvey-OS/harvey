...
Xfsub isosub = {ireset,   iattach, iclone, iwalkup, iwalk,   iopen, icreate,
...
iattach(Xfile* root)
{
	...
	for(i = VOLDESC; i < VOLDESC + 100; i++) { /* +100 for sanity */
		...
		if(memcmp(v->byte, "\01CD001\01", 7) == 0) { /* iso */
			...
			haveplan9 =
			    (strncmp((char*)v->z.boot.sysid, "PLAN 9", 6) == 0);
			if(haveplan9) {
				if(noplan9) {
					chat("ignoring plan9");
					haveplan9 = 0;
		...
		if(memcmp(&v->byte[8], "\01CDROM\01", 7) ==
		   0) { /* high sierra */
			if(dirp)
				putbuf(dirp);
			dirp = p;
		...
		if(haveplan9 == 0 && !nojoliet &&
		   memcmp(v->byte, "\02CD001\01", 7) == 0) {
			chat("%d %d\n", haveplan9, nojoliet);
			/*
			 * The right thing to do is walk the escape sequences
			 * looking
			 * for one of 25 2F 4[035], but Microsoft seems to not
			 * honor
			 * the format, which makes it hard to walk over.
			 */
			q = v->z.desc.escapes;
			if(q[0] == 0x25 && q[1] == 0x2F &&
			   (q[2] == 0x40 || q[2] == 0x43 ||
			    q[2] == 0x45)) { /* Joliet, it appears */
				if(dirp)
					putbuf(dirp);
				dirp = p;
				fmt = 'J';
				...
				if(blksize != l16(v->z.desc.blksize))
					fprint(2, "warning: suspicious Joliet "
					          "blocksize\n");
				chat("joliet...");
				continue;
			}
...
iwalk(Xfile* f, char* name)
{
	...
	*/
	...
	while(getdrec(f, d) >= 0) {
		...
		if(casestrcmp(f->xf->isplan9 || f->xf->isrock, name,
		              dir.name) != 0)
			continue;
		newdrec(f, d);
...
iread(Xfile* f, char* buf, int64_t offset, int32_t count)
{
	...
	if(offset + count > size)
		count = size - offset;
	addr =
	    ((int64_t)l32(ip->d.addr) + ip->d.attrlen) * ip->blksize + offset;
	o = addr % Sectorsize;
	addr /= Sectorsize;
...
showdrec(int fd, int fmt, void* x)
{
	...
	if(d->reclen == 0)
		return 0;
	fprint(fd, "%d %d %ld %ld ", d->reclen, d->attrlen, l32(d->addr),
	       l32(d->size));
	fprint(fd, "%s 0x%2.2x %d %d %ld ", rdate(d->date, fmt),
	       (fmt == 'z' ? d->flags : d->r_flags), d->unitsize, d->gapsize,
	       l16(d->vseqno));
...
getdrec(Xfile* f, void* buf)
{
	...
	while(ip->offset < size) {
		addr =
		    ((int64_t)l32(ip->d.addr) + ip->d.attrlen) * ip->blksize +
		    ip->offset;
...
rzdir(Xfs* fs, Dir* d, int fmt, Drec* dp)
{
	...
	if(fs->isplan9 && sysl > 0) {
		...
		if(fmt != '9' && !(d->mode & DMDIR)) {
			/*
			 * ISO 9660 actually requires that you always have a .
			 *and a ;,
			 * even if there is no version and no extension.  Very
			 *few writers
			 * do this.  If the version is present, we use it for
			 *qid.vers.
			 * If there is no extension but there is a dot, we strip
			 *it off.
			 * (VMS heads couldn't comprehend the dot as a file name
			 *character
			 * rather than as just a separator between name and
			 *extension.)
			 *
			 * We don't do this for directory names because
			 *directories are
		...
		if(fs->issusp) {
			...
			s += fs->suspoff;
			sysl -= fs->suspoff;
			for(; sysl >= 4 && have != (Hname | Hmode);
			    sysl -= l, s += l) {
				if(s[0] == 0 && ((uintptr)s & 1)) {
					/* MacOS pads individual entries,
					 * contrary to spec */
					s++;
					sysl--;
				}
...
rdate(uint8_t* p, int fmt)
{
	...
	n = sprint(buf, "%2.2d.%2.2d.%2.2d %2.2d:%2.2d:%2.2d", p[0], p[1], p[2],
	           p[3], p[4], p[5]);
	if(fmt == 'z') {
		htz = p[6];
		if(htz >= 128) {
...
static int32_t gtime(uint8_t* p) /* yMdhmsz */
{
	int32_t t;
