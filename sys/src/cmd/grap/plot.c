...
{
	fprintf(tfd, "%s %s from %s", type == LINE ? "line" : "arrow",
	        desc_str(desc), xyname(p1));
...
void drawdesc(int type, Obj* p, Attr* desc,
              char* s) /* set line description for p */
{
	p->attr = desc;
	p->attr->sval = s;
	if(type == NEW) {
...
void next(Obj* p, Point pt, Attr* desc) /* add component to a path */
{
	...
	if(p->first == 0) {
		...
		fprintf(tfd, "line %s from L%s to %s; L%s: Here\n",
		        desc_str(desc->type ? desc : p->attr), p->name,
		        xyname(pt), p->name);
	}
