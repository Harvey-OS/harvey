...
void coordlog(int n) /* remember log scaling */ { logcoord = n; }
...
void coord(Obj* p) /* set coord range */
{
	...
	if(xcflag) {
		...
		if((logcoord & XFLAG) && p->pt.x <= 0.0)
			ERROR "can't have log of x coord %g,%g", p->pt.x,
			    p->pt1.x FATAL;
		xcflag = 0;
	}
	if(ycflag) {
		...
		if((logcoord & YFLAG) && p->pt.y <= 0.0)
			ERROR "can't have log of y coord %g,%g", p->pt.y,
			    p->pt1.y FATAL;
		ycflag = 0;
	}
	p->log = logcoord;
...
void resetcoord(Obj* p) /* reset current coordinate */ { curr_coord = p->name; }
