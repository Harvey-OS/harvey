...
format(char *mnemonic, Instr *i, char *f)
{
	...
	for ( ; *f; f++) {
		...
		case 's':
			c += Bprint(bioout, "%d", i->rs);
			break;

		case 't':
			c += Bprint(bioout, "%d", i->rt);
			break;

		case 'd':
			c += Bprint(bioout, "%d", i->rd);
			break;

		case 'a':
			c += Bprint(bioout, "%d", i->sa);
			break;

		case 'l':
			c += Bprint(bioout, "%lx(r%d)", i->immediate, i->rs);
			break;

		case 'i':
			c += Bprint(bioout, "$%lx", i->immediate);
			break;

		case 'u':
			c += Bprint(bioout, "$%lux", i->immediate & 0xFFFF);
			break;

		case 'j':
			c += Bprint(bioout, "%lux", (i->target << 2) | (dot & 0xF0000000));
			break;

		case 'b':
			c += Bprint(bioout, "%lux", (i->immediate << 2) + dot + 4);
			break;

		case 'c':
			c += Bprint(bioout, "%lux", i->cofun);
			break;

		case 'w':
			c += Bprint(bioout, "[%lux]", i->w0);
			break;

		case 'f':
			c += Bprint(bioout, "%c", fsub[i->rs & 0x0F]);
			break;
...
mipscoprintins(void)
{
	...
	return format(o[op].mnemonic, &i, o[op].mipsco);
}
