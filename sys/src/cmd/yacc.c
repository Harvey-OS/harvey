...
char
    tokname[NAMESIZE + UTFmax + 1]; /* input token name, slop for runes and 0 */
...
int temp1
    [TEMPSIZE];  /* temporary storage, indexed by terms + ntokens or states */
...
struct {
    ...
    "binary", BINARY,  "left", LEFT,    "nonassoc", BINARY, "prec",
    PREC,     "right", RIGHT,  "start", START,      "term", TERM,
    "token",  TERM,    "type", TYPEDEF, "union",    UNION,  0,
};
...
main(int argc, char* argv[])
{
	...
	cempty(); /* make a table of which nonterminals can match the empty
	             string */
...
others(void)
{
	...
	TLOOP(i)
	{
		...
		if(j >= 0 && j < 256) {
			if(temp1[j]) {
				print("yacc bug -- cant have 2 different Ts "
				      "with same value\n");
				print("	%s and %s\n", tokset[i].name,
				      tokset[temp1[j]].name);
				nerrors++;
...
summary(void)
{
	...
	if(foutput != 0) {
		...
		Bprint(foutput, "memory: states,etc. %d/%d, parser %d/%d\n",
		       (int)(zzmemsz - mem0), MEMSIZE, (int)(memp - amem),
		       ACTSIZE);
...
{
	fprint(2,
	       "usage: yacc [-Dn] [-vdS] [-o outputfile] [-s stem] grammar\n");
	exits("usage");
}
	...
	more_rule:
		...
		if(t == '=') {
			...
			if((t = gettok()) == IDENTIFIER) {
				 ...
				 * nprod+1
				 * move the contents down, and make room for the
				 * null
				 */
		...
		*mem++ = -nprod;
		...
		if(ntypes && !(levprd[nprod] & ACTFLAG) &&
		   nontrst[*prdptr[nprod] - NTBASE].value) {
			...
			if(tempty < 0)
				error("must return a value, since LHS has a "
				      "type");
			...
			if(tempty != nontrst[*prdptr[nprod] - NTBASE].value)
				error("default action causes potential type "
				      "clash");
		}
		nprod++;
		if(nprod >= NPROD)
...
begin:
	...
	default:
		...
		if(isdigit(c)) {
			numbval = c - '0';
			base = (c == '0') ? 8 : 10;
			for(c = Bgetrune(finput); isdigit(c);
			    c = Bgetrune(finput))
...
precftn(int r, int t, int s)
{
	...
	if(PLEVEL(lt) == 0 || PLEVEL(lp) == 0) {
		...
		/* conflict */
		if(foutput != 0)
			Bprint(foutput, "\n%d: shift/reduce conflict (shift "
			                "%d(%d), red'n %d(%d)) on %s",
			       s, temp1[t], PLEVEL(lt), r, PLEVEL(lp),
			       symnam(t));
		zzsrconf++;
		return;
	}
	...
	for(n = -maxoff; n < ACTSIZE; n++) {
		...
		for(r = q1; r < q2; r += 2) {
			...
			if(s > maxa)
				maxa = s;
			if(*s != 0 && *s != r[1])
				error("clobber of a array, pos'n %d, by %d",
				      s - amem, r[1]);
			*s = r[1];
		}
		indgo[i] = n;
...
osummary(void)
{
	...
	Bprint(foutput, "Optimizer space used: input %d/%d, output %d/%d\n",
	       (int)(pmem - mem0 + 1), MEMSIZE, (int)(maxa - amem + 1),
	       ACTSIZE);
