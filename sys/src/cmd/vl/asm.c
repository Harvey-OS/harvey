...
void lput(int32_t l) /* emit long in big-endian byte order */ { LBEPUT(l); }

void lputl(int32_t l) /* emit long in big-endian byte order */ { LLEPUT(l); }
...
asmb(void)
{
	...
	case 1:
		...
		plan9bootimage(0, 0407 << 16 | 0437, 0, HEADR + textsize,
		               textsize, INITTEXT, lcsize);
	...
	case 3:
		...
		plan9bootimage(3, 0407 << 16 | 0437, tm, HEADR + textsize,
		               textsize, INITTEXT, lcsize);
...
#define OP_JMP(op, i) ((op) | ((i)&0x3ffffffL))

#define OP(x, y) (((x) << 3) | ((y) << 0))
#define SP(x, y) (((x) << 29) | ((y) << 26))
#define BCOND(x, y) (((x) << 19) | ((y) << 16))
#define MMU(x, y) (SP(2, 0) | (16 << 21) | ((x) << 3) | ((y) << 0))
...
asmout(Prog* p, Optab* o, int aflag)
{
	...
	case 13: /* movbu r,r */
		if(p->as == AMOVBU)
			...
			o1 = OP_IRR(opirr(AAND), 0xffffL, p->from.reg,
			            p->to.reg);
		break;
	...
	case 32: /* fadd fr1,[fr2],fr3 */
		r = p->reg;
		if(r == NREG)
			o1 = OP_FRRR(oprrr(p->as), p->from.reg, p->to.reg,
			             p->to.reg);
		else
	...
	case 34: /* mov $con,fr ==> or/add $i,r,r2 */
		...
		if(o->a1 == C_ANDCON)
			r = AOR;
		o1 = OP_IRR(opirr(r), v, 0, REGTMP);
		o2 = OP_RRR(SP(2, 1) | (4 << 21), REGTMP, 0,
		            p->to.reg); /* mtc1 */
		break;
	...
	case 41: /* movw r,fcr */
		o1 = OP_RRR(SP(2, 1) | (2 << 21), REGZERO, 0,
		            p->to.reg); /* mfcc1 */
		o2 = OP_RRR(SP(2, 1) | (6 << 21), p->from.reg, 0,
		            p->to.reg); /* mtcc1 */
		break;

	case 42: /* movw fcr,r */
		o1 = OP_RRR(SP(2, 1) | (2 << 21), p->to.reg, 0,
		            p->from.reg); /* mfcc1 */
		break;
	...
	case 45: /* case r */
		if(p->link == P)
			...
			v = p->link->pc;
		if(v & (1 << 15))
			o1 = OP_IRR(opirr(ALAST), (v >> 16) + 1, REGZERO,
			            REGTMP);
		else
			o1 = OP_IRR(opirr(ALAST), v >> 16, REGZERO, REGTMP);
