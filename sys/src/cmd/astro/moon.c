...
moon(void)
{
	...
	dlong =
	    270.434164 + 13.1763965268 * eday - .001133 * capt2 + 2.e-6 * capt3;
	argp =
	    334.329556 + .1114040803 * eday - .010325 * capt2 - 12.e-6 * capt3;
	node =
	    259.183275 - .0529539222 * eday + .002078 * capt2 + 2.e-6 * capt3;
	lsun = 279.696678 + .9856473354 * eday + .000303 * capt2;
	psun =
	    281.220833 + .0000470684 * eday + .000453 * capt2 + 3.e-6 * capt3;
	...
	dlong += (0.84 * sin(arg1) + 0.31 * sin(arg2) + 14.27 * sin(arg3) +
	          7.261 * sin(arg4) + 0.282 * sin(arg5) + 0.237 * sin(arg6) +
	          0.108 * sin(arg7) + 0.126 * sin(arg8)) /
	         3600.;

	argp += (-2.10 * sin(arg1) - 0.118 * sin(arg3) - 2.076 * sin(arg4) -
	         0.840 * sin(arg5) - 0.593 * sin(arg6)) /
	        3600.;

	node += (0.63 * sin(arg1) + 0.17 * sin(arg3) + 95.96 * sin(arg4) +
	         15.58 * sin(arg5) + 1.86 * sin(arg9)) /
	        3600.;

	t0 += (-6.40 * sin(arg1) - 1.89 * sin(arg6)) / 3600.;

	psun += (6.40 * sin(arg1) + 1.89 * sin(arg6)) / 3600.;

	dgamma = -4.318 * cos(arg4) - 0.698 * cos(arg5) - 0.083 * cos(arg9);
	...
	for(;;) {
		if(mp->f == 0.0)
			break;
		lterms +=
		    sinx(mp->f, mp->c[0], mp->c[1], mp->c[2], mp->c[3], 0.0);
		mp++;
	}
	mp++;
	...
	for(;;) {
		if(mp->f == 0)
			break;
		sterms +=
		    sinx(mp->f, mp->c[0], mp->c[1], mp->c[2], mp->c[3], 0);
		mp++;
	}
	mp++;
	...
	for(;;) {
		if(mp->f == 0)
			break;
		cterms +=
		    cosx(mp->f, mp->c[0], mp->c[1], mp->c[2], mp->c[3], 0);
		mp++;
	}
	mp++;
	...
	ymp =
	    rp * (sin(lambda) * cos(beta) * cos(obl2) - sin(obl2) * sin(beta));
	zmp =
	    rp * (sin(lambda) * cos(beta) * sin(obl2) + cos(obl2) * sin(beta));
