...
plut(void)
{
	...
	do {
		dele = (anom - enom + ecc * sin(enom)) / (1. - ecc * cos(enom));
		enom += dele;
	...
	sd = rad *
	     (cos(beta) * sin(lambda) * sin(obliq) + sin(beta) * cos(obliq));
	sa = rad *
	     (cos(beta) * sin(lambda) * cos(obliq) - sin(beta) * sin(obliq));
	...
	sb =
	    sin(capj) * cos(delta) * sin(alpha - capn) - cos(capj) * sin(delta);
	su =
	    cos(capj) * cos(delta) * sin(alpha - capn) + sin(capj) * sin(delta);
	...
	su = sin(eye) * sin(beta) + cos(eye) * cos(beta) * sin(lambda - comg);
	...
	sb = sin(b);
	mag = -8.68 + 2.52 * fabs(up + omg - u) - 2.60 * fabs(sb) +
	      1.25 * (sb * sb);
