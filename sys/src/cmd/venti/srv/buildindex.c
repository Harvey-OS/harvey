...
{
	fprint(2,
	       "usage: buildindex [-b] [-i isect]... [-M imem] venti.conf\n");
	...
	for(i = 0; i < ix->narenas; i++) {
		...
		for(clump = a->memstats.clumps; clump > 0; clump -= n) {
			...
			for(j = n - 1; j >= 0; j--) {
				...
				ie.ia.addr = addr;
				ie.ia.blocks = (ci->size + ClumpSize +
				                (1 << ABlockLog) - 1) >>
				               ABlockLog;
		...
		if(addr != ix->amap[i].start)
			fprint(
			    2,
			    "%T arena %s: clump miscalculation %lld != %lld\n",
...
score2bucket(ISect* is, uint8_t* score)
{
	...
	if(b < is->start || b >= is->stop) {
		fprint(
		    2,
		    "score2bucket: score=%V div=%d b=%ud start=%ud stop=%ud\n",
...
static IPool*
mkipool(ISect* isect, Minibuf* mbuf, uint32_t nmbuf, uint32_t mbufbuckets,
        uint32_t bufsize)
{
	uint32_t i, nentry;
	uint8_t* data;
	...
	p = ezmalloc(sizeof(IPool) + nentry * sizeof(IEntry) +
	             nmbuf * sizeof(IEntryLink*) + nmbuf * sizeof(uint32_t) +
	             3 * bufsize);
               ...
               uint32_t bufsize)
{
	...
	for(p = buf; p < ep; p = q) {
		...
		for(q = p; q < ep && score2bucket(is, q) == b; q += IEntrySize)
			;
		if(lastb + 1 < b && zero)
			zerorange(part, bucket2offset(is, lastb + 1),
			          bucket2offset(is, b));
	...
	if(lastb + 1 < is->stop - is->start && zero)
		zerorange(part, bucket2offset(is, lastb + 1),
		          bucket2offset(is, is->stop - is->start));

	if(n != mb->nwentry)
		fprint(2, "sortminibuffer bug: n=%ud nwentry=%ud have=%ld\n", n,
		       mb->nwentry, (ep - buf) / IEntrySize);
...
isectproc(void* v)
{
	...
	fprint(
	    2,
	    "%T %s: %,ud buckets, %,ud groups, %,ud minigroups, %,ud buffer\n",
	...
	for(i = 0; i < nbuf; i++) {
		...
		for(j = 0; j < nminibuf; j++) {
			...
			mb->nentry =
			    epbuf * (mb->eoffset - mb->boffset) / bufsize;
		...
		if(!dumb && nminibuf == 1) {
			...
			ipool =
			    mkipool(is, mbuf, nminibuf, mbufbuckets, bufsize);
