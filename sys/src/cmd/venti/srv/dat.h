...
enum { ABlockLog = 9, /* log2(512), the quantum for reading arenas */
       ANameSize = 64,
       ...
       MaxAMap = 31 * 1024, /* max. allowed arenas in an address mapping; must
                               be < 32*1024 */
       Unspecified = TWID32,
       ...
       MaxBloomHash = 32,            /* bits per score */
	                             /*
                                      ...
                                      * would be faster if we bumped IBucketSize up to 8 and IEntrySize up to
                                      * 40,
       ...
       MaxClumpBlocks =
	   (VtMaxLumpSize + ClumpSize + (1 << ABlockLog) - 1) >> ABlockLog,
...
struct DBlock {
	...
	u32int size; /* amount of data available, not amount allocated; should
	                go away */
	u32int mode;
	u32int dirty;
	u32int dirtying;
 ...
 * to sync after reboot, follow clumps starting at used until ClumpFreeMagic if
 *found.
 ...
 *	<field name="compresseddata" val="s->used - s->clumps * ClumpSize"
 *type="U64int"/>
 *	<field name="storage" val="s->used + s->clumps * ClumpInfoSize"
 *type="U64int"/>
 * </struct>
 */
struct Arena {
	...
	u8int
	    score[VtScoreSize]; /* score of the entire sealed & summed arena */
...
/*
 * for kicking background processes that run one round after another after
 * another
 */
struct Round {
	QLock lock;
