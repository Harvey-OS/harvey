...
enum { ObjNameSize = 64, MaxObjs = 64 };
...
httpproc(void* v)
{
	...
	for(;;) {
		...
		for(i = 0; i < MaxObjs && objs[i].name[0]; i++) {
			...
			if((objs[i].name[n - 1] == '/' &&
			    strncmp(c->req.uri, objs[i].name, n) == 0) ||
			   (objs[i].name[n - 1] != '/' &&
			    strcmp(c->req.uri, objs[i].name) == 0)) {
				ok = (*objs[i].f)(c);
				goto found;
			}
		}
...
herror(HConnect* c)
{
	...
	hout = &c->hout;
	n = snprint(c->xferbuf, HBufSize, "<html><head><title>Error</title></"
	                                  "head>\n<body><h1>Error</"
	                                  "h1>\n<pre>%r</pre>\n</body></html>");
...
struct {
	char* ext;
	char* type;
} exttab[] = {".html", "text/html", ".txt", "text/plain", ".xml", "text/xml",
              ".png",  "image/png", ".gif", "image/gif",  0};
...
static struct {
    ...
    "compress", &compressblocks, "devnull", &writestodevnull, "logging",
    &ventilogging, "stats", &collectstats, "icachesleeptime", &icachesleeptime,
    "minicachesleeptime", &minicachesleeptime, "arenasumsleeptime",
    &arenasumsleeptime, "l0quantum", &l0quantum, "l1quantum", &l1quantum,
    "manualscheduling", &manualscheduling, "ignorebloom", &ignorebloom,
    "syncwrites", &syncwrites, "icacheprefetch", &icacheprefetch, 0};
...
estats(HConnect* c)
{
	...
	/*
	   ...
	   stats.cireads);
	        ...
	        hprint(hout, "index disk bloom filter hits=%,ld %d%%
	   falsemisses=%,ld %d%%\n",
	        ...
	        hprint(hout, "bloom filter bits=%,ld of %,ld %d%%\n",
	                stats.bloomones, stats.bloombits,
	   percent(stats.bloomones, stats.bloombits));
	   ...
	   stats.absorbedwrites,
	        ...
	        hprint(hout, "disk cache flush writes=%,ld (%,ld per flush)\n",
	                ...
	                stats.dcacheflushwrites/(stats.dcacheflushes ?
	   stats.dcacheflushes : 1));
...
sindex(HConnect* c)
{
	...
	hprint(hout, "total space=%,lld used=%,lld\n", size,
	       used + clumps * ClumpInfoSize);
	hprint(hout, "clumps=%,lld compressed clumps=%,lld data=%,lld "
	             "compressed data=%,lld\n",
...
darena(Hio* hout, Arena* arena)
{
	hprint(hout, "arena='%s' on %s at [%lld,%lld)\n\tversion=%d created=%d "
	             "modified=%d",
	       arena->name, arena->part->name, arena->base,
	       arena->base + arena->size + 2 * arena->blocksize, arena->version,
...
dindex(HConnect* c)
{
	...
	hprint(hout, "index=%s version=%d blocksize=%d tabsize=%d\n", ix->name,
	       ix->version, ix->blocksize, ix->tabsize);
	...
	for(i = 0; i < ix->narenas; i++) {
		if(ix->arenas[i] != nil &&
		   ix->arenas[i]->memstats.clumps != 0) {
			hprint(hout, "arena=%s at index [%lld,%lld)\n\t",
			       ix->amap[i].name, ix->amap[i].start,
			       ix->amap[i].stop);
...
pctdiffgraph(Stats* s, Stats* t, void* va)
{
	...
	a = va;
	return percent(t->n[a->index] - s->n[a->index],
	               t->n[a->index2] - s->n[a->index2]);
}
...
divdiffgraph(Stats* s, Stats* t, void* va)
{
	...
	a = va;
	return xdiv(t->n[a->index] - s->n[a->index],
	            t->n[a->index2] - s->n[a->index2]);
}
...
netbw(Stats* s)
{
	...
	n = s->n;
	return n[StatRpcReadBytes] +
	       n[StatRpcWriteBytes]; /* not exactly right */
}
...
static char* graphname[] = {
    "rpctotal", "rpcread", "rpcreadok", "rpcreadfail", "rpcreadbyte",
    "rpcreadtime", "rpcreadcached", "rpcreadcachedtime", "rpcreaduncached",
    "rpcreaduncachedtime", "rpcwrite", "rpcwritenew", "rpcwriteold",
    "rpcwritefail", "rpcwritebyte", "rpcwritetime", "rpcwritenewtime",
    "rpcwriteoldtime",

    "lcachehit", "lcachemiss", "lcachelookup", "lcachewrite", "lcachesize",
    "lcachestall", "lcachelookuptime",

    "dcachehit", "dcachemiss", "dcachelookup", "dcacheread", "dcachewrite",
    "dcachedirty", "dcachesize", "dcacheflush", "dcachestall",
    "dcachelookuptime",

    "dblockstall", "lumpstall",

    "icachehit", "icachemiss", "icacheread", "icachewrite", "icachefill",
    "icacheprefetch", "icachedirty", "icachesize", "icacheflush", "icachestall",
    "icachelookuptime", "icachelookup", "scachehit", "scacheprefetch",

    "bloomhit", "bloommiss", "bloomfalsemiss", "bloomlookup", "bloomones",
    "bloombits",

    "apartread", "apartreadbyte", "apartwrite", "apartwritebyte",

    "isectread", "isectreadbyte", "isectwrite", "isectwritebyte",
