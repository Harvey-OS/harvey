...
Arena*
newarena(Part* part, uint32_t vers, char* name, uint64_t base, uint64_t size,
         uint32_t blocksize)
{
	int bsize;
	Arena* arena;
 ...
 * make sure it won't run off the end, then return the number of bytes actually
 * read
 */
uint32_t
readarena(Arena* arena, uint64_t aa, uint8_t* buf, int32_t n)
{
	...
	qunlock(&arena->lock);
	if(aa >= a) {
		seterr(EOk, "reading beyond arena clump storage: clumps=%d "
		            "aa=%lld a=%lld -1 clumps=%lld\n",
		       arena->memstats.clumps, aa, a,
		       arena->size -
		           arenadirsize(arena, arena->memstats.clumps - 1));
		return -1;
	}
	if(aa + n > a)
		n = a - aa;
...
writearena(Arena* arena, uint64_t aa, uint8_t* clbuf, uint32_t n)
{
	...
	nn = 0;
	for(;;) {
		b = getdblock(arena->part, a,
		              off != 0 || off + n < blocksize ? ORDWR : OWRITE);
		if(b == nil) {
			qunlock(&arena->lock);
			return -1;
		}
...
writeaclump(Arena* arena, Clump* c, uint8_t* clbuf)
{
	...
	if(arena->memstats.sealed ||
	   aa + n + U32Size + arenadirsize(arena, arena->memstats.clumps + 1) >
	       arena->size) {
		if(!arena->memstats.sealed) {
	...
	if(clump % ArenaCIGSize == 0) {
		...
		if(clump / ArenaCIGSize != arena->ncig) {
			fprint(2, "bad arena cig computation %s: writing clump "
			          "%d but %d cigs\n",
		...
		arena->cig = vtrealloc(arena->cig, (arena->ncig + 1) *
		                                       sizeof arena->cig[0]);
...
setatailstate(AState* as)
{
	...
	for(i = 0; i < ix->narenas; i++)
		if(ix->arenas[i] == as->arena)
			break;
	if(i == ix->narenas || as->aa < ix->amap[i].start ||
	   as->aa >= ix->amap[i].stop || as->arena != ix->arenas[i]) {
...
sumarena(Arena* arena)
{
	...
	for(a = arena->base - arena->blocksize; a + arena->blocksize <= e;
	    a += bs) {
		disksched();
	...
	/*
	 * the last one is special, since it may already have the checksum
	 * included
	 */
	bs = arena->blocksize;
	if(readpart(arena->part, e, b->data, bs) < 0) {
	ReadErr:
		logerr(EOk, "sumarena can't sum %s, read at %lld failed: %r",
		       arena->name, a);
		freezblock(b);
		return;
	}
	...
	if(scorecmp(score, &b->data[bs - VtScoreSize]) != 0 &&
	   scorecmp(zeroscore, &b->data[bs - VtScoreSize]) != 0)
		logerr(EOk, "overwriting mismatched checksums for arena=%s, "
		            "found=%V calculated=%V",
...
wbarena(Arena* arena)
{
	...
	if((b = getdblock(arena->part, arena->base + arena->size, OWRITE)) ==
	   nil) {
...
wbarenahead(Arena* arena)
{
	...
	bad = packarenahead(&head, b->data) < 0 ||
	      writepart(arena->part, arena->base - arena->blocksize, b->data,
	                arena->blocksize) < 0 ||
...
loadarena(Arena* arena)
{
	...
	if(b == nil)
		return -1;
	if(readpart(arena->part, arena->base + arena->size, b->data,
	            arena->blocksize) < 0) {
		freezblock(b);
		return -1;
	}
	...
	if(readpart(arena->part, arena->base - arena->blocksize, b->data,
	            arena->blocksize) < 0) {
	...
	if(unpackarenahead(&head, b->data) < 0)
		...
		if(namecmp(arena->name, head.name) != 0)
			...
			logerr(ECorrupt,
			       "arena header inconsistent with arena data");
	}
	freezblock(b);
...
okarena(Arena* arena)
{
	...
	if(arena->diskstats.cclumps > arena->diskstats.clumps)
		logerr(ECorrupt,
		       "arena %s has more compressed clumps than total clumps",
		       arena->name);

	/*
	 * This need not be true if some of the disk is corrupted.
	 *
	if(arena->diskstats.uncsize + arena->diskstats.clumps * ClumpSize +
	arena->blocksize < arena->diskstats.used)
	        logerr(ECorrupt, "arena %s uncompressed size inconsistent with
	used space %lld %d %lld", arena->name, arena->diskstats.uncsize,
	arena->diskstats.clumps, arena->diskstats.used);
	 */
...
getcib(Arena* arena, int clump, int writing, CIBlock* rock)
{
	...
	cib->data = getdblock(arena->part, arena->base + arena->size -
	                                       (block + 1) * arena->blocksize,
	                      mode);
	if(cib->data == nil)
		return nil;
	return cib;
	...
	for(i = 0; i < ncig; i++) {
		...
		if(nci < ArenaCIGSize) {
			if(i != ncig - 1) {
				...
				fprint(
				    2,
				    "loadcig %s: got %ud cigs, expected %ud\n",
				    arena->name, i + 1, ncig);
				goto out;
			}
		}
...
asumload(Arena* arena, int g, IEntry* entries, int nentries)
{
	...
	for(i = base; i < limit; i++) {
		...
		if(ci.type != VtCorruptType) {
			...
			ie->ia.blocks =
			    (ci.size + ClumpSize + (1 << ABlockLog) - 1) >>
			    ABlockLog;
			ie->ia.addr = addr;
			ie++;
		}
