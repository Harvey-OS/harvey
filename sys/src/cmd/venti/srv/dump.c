...
printindex(int fd, Index* ix)
{
	...
	for(i = 0; i < ix->narenas; i++)
		fprint(fd, "\tarena=%s at [%lld,%lld)\n", ix->amap[i].name,
		       ix->amap[i].start, ix->amap[i].stop);
}
...
printarenapart(int fd, ArenaPart* ap)
{
	...
	fprint(fd, "arena partition=%s\n\tversion=%d blocksize=%d "
	           "arenas=%d\n\tsetbase=%d setsize=%d\n",
	       ap->part->name, ap->version, ap->blocksize, ap->narenas,
	       ap->tabbase, ap->tabsize);
	for(i = 0; i < ap->narenas; i++)
		fprint(fd, "\tarena=%s at [%lld,%lld)\n", ap->map[i].name,
		       ap->map[i].start, ap->map[i].stop);
}
...
printarena(int fd, Arena* arena)
{
	...
	fprint(fd, "\tclumps=%,d compressed clumps=%,d data=%,lld compressed "
	           "data=%,lld disk storage=%,lld\n",
