...
static int
preadblock(int fd, u8 *buf, int n, i64 off)
{
	int nr, m;
...
printheader(char *name, ArenaHead *head, int fd)
{
	...
	if(clumpmax > 0)
		lo = hi - (u64)arena.diskstats.clumps/clumpmax * head->blocksize;
	else
		lo = hi;
...
readap(int fd, ArenaPart *ap)
{
	...
	if(preadblock(fd, (u8*)table, ap->tabsize, ap->tabbase) < 0)
