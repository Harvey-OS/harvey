...
checkbucket(Index* ix, uint32_t buck, IBucket* ib)
{
	...
	eb = getdblock(is->part,
	               is->blockbase + ((uint64_t)buck << is->blocklog), OREAD);
	if(eb == nil)
		return -1;
	...
	for(i = 0; i < ib->n; i++) {
		while(ei < eib.n) {
			c = ientrycmp(&ib->data[i * IEntrySize],
			              &eib.data[ei * IEntrySize]);
			if(c == 0) {
		...
		if(zero) {
			for(; next != buck; next++) {
				if(next == ix->buckets) {
					if(buck != TWID32) {
						ok = -1;
						werrstr("internal error: "
						        "bucket out of range");
					}
					if(ok < 0)
						werrstr("%d spurious entries, "
						        "%d missing, %d wrong",
						        extra, missing, wrong);
					goto out;
				}
