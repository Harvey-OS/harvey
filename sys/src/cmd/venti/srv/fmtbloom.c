...
threadmain(int argc, char* argv[])
{
	...
	if(size > MaxBloomSize) {
		fprint(2, "warning: not using entire %,lld bytes; using only "
		          "%,lld bytes\n",
	...
	if(nblocks) {
		...
		if(size2 != size) {
			size = size2;
			fprint(2, "warning: using only %lldMB - not enough "
			          "blocks to warrant more\n",
			       size / 1024 / 1024);
		}

		/*
		 * optimal is to use ln 2 times as many hash functions as we
		 * have bits per blocks.
		 */
		bits = (8 * size) / nblocks;
		nhash = bits * 7 / 10;
		if(nhash > BloomMaxHash)
	...
	bits = nhash * 10 / 7;
	nblocks = (8 * size) / bits;
	fprint(2, "fmtbloom: using %lldMB, %d hashes/score, best up to %,lld "
	          "blocks\n",
