...
static uint32_t readarenainfo(IEBucks* ib, Arena* arena, uint64_t a, Bloom* b);
...
sortrawientries(Index* ix, Part* tmp, uint64_t* base, Bloom* bloom)
{
	...
	if(ok == 0) {
		...
		*base = (uint64_t)ib->chunks * ib->size;
		if(sorted != clumps) {
			fprint(2,
			       "sorting messed up: clumps=%lld sorted=%lld\n",
			       clumps, sorted);
			ok = -1;
		}
	}
			...
			if(ci->type == VtCorruptType) {
				if(0)
					print("! %V %22lld %3d %5d %3d\n",
					      ie.score, ie.ia.addr, ie.ia.type,
					      ie.ia.size, ie.ia.blocks);
				nskip++;
			} else
				sprayientry(ib, &ie);
...
initiebucks(Part* part, int bits, uint32_t size)
{
	...
	ib->buf =
	    (uint8_t*)(((uintptr)ib->xbuf + size - 1) & ~(uintptr)(size - 1));
	if(ib->buf == nil) {
		seterr(EOk,
		       "out of memory allocating sorting buckets' buffers");
		freeiebucks(ib);
		return nil;
	}
...
sortiebucks(IEBucks* ib)
{
	...
	if(ib->buf == nil) {
		seterr(EOk, "out of memory allocating final sorting buffer; "
		            "try more buckets");
		return TWID64;
	}
	tot = 0;
	for(i = 0; i < ib->nbucks; i++) {
		...
		if(n != ib->bucks[i].total / IEntrySize)
			fprint(2, "bucket %d changed count %d => %d\n", i,
			       (int)(ib->bucks[i].total / IEntrySize), n);
		tot += n;
	}
	return tot;
}
...
flushiebuck(IEBucks* ib, int b, int reset)
{
	...
	n += U32Size;
	USED(n);
	if(writepart(ib->part, (uint64_t)ib->chunks * ib->size,
	             ib->bucks[b].buf, ib->size) < 0) {
...
readiebuck(IEBucks* ib, int b)
{
	...
	if(0)
		if(ib->bucks[b].total)
			fprint(2, "\tbucket %d: %lld entries\n", b,
			       ib->bucks[b].total / IEntrySize);
	while(head != TWID32) {
		if(readpart(ib->part, (uint64_t)head * ib->size, &ib->buf[n],
		            m + U32Size) < 0) {
	...
	if(n != ib->bucks[b].total)
		fprint(2, "\tbucket %d: expected %d entries, got %d\n", b,
		       (int)ib->bucks[b].total / IEntrySize, n / IEntrySize);
	return n / IEntrySize;
}
