...
mirror(Arena* sa, Arena* da)
{
	...
	if(sa->diskstats.sealed && da->diskstats.sealed &&
	   scorecmp(da->score, zeroscore) != 0) {
	...
	if(sa->diskstats.used < da->diskstats.used) {
		chat("%T %s: src used %,lld < dst used %,lld\n", sa->name,
		     sa->diskstats.used, da->diskstats.used);
		status = "errors";
		return;
	}
	...
	if(da->diskstats.clumps != sa->diskstats.clumps) {
		if(ds && asha1(dst, shaoff, si, ds) < 0)
			return;
		if(copy(si, di, "directory", ds) <
		   0) /* si < di  because clumpinfo blocks grow down */
			return;
		shaoff = di;
	}
			...
			if(scorecmp(sa->score, da->score) == 0) {
				if(verbose)
					chat("%T %s: %V sealed mirrored\n",
					     sa->name, sa->score);
				if(ewritepart(dst,
				              end + blocksize - VtScoreSize,
				              da->score, VtScoreSize) < 0)
					return;
			} else {
				chat("%T %s: sealing dst: score mismatch: %V "
				     "vs %V\n",
				     sa->name, sa->score, da->score);
				memset(&xds, 0, sizeof xds);
				asha1(dst, base - blocksize,
				      end + blocksize - VtScoreSize, &xds);
