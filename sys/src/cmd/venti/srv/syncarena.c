...
{
	return c->type != d->type || c->size != d->size ||
	       c->uncsize != d->uncsize || scorecmp(c->score, d->score) != 0;
}
...
syncarena(Arena* arena, uint32_t n, int zok, int fix)
{
	...
	for(; n; n--) {
		...
		if(magic != arena->clumpmagic) {
			fprint(2, "%s: illegal clump magic number=%#8.8ux at "
			          "clump=%d\n",
			       arena->name, magic, clump);
			/* err |= SyncDataErr; */
			if(fix &&
			   writeclumpmagic(arena, aa, ClumpFreeMagic) < 0) {
				fprint(2, "%s: can't write corrected clump "
				          "free magic: %r",
				       arena->name);
				err |= SyncFixErr;
			}
		...
		if(lump == nil) {
			...
			if(scorecmp(cl.info.score, score) != 0) {
				...
				fprint(2, "%s: clump=%d has invalid type %d",
				       arena->name, clump, cl.info.type);
				err |= SyncDataErr;
				broken = 1;
			}
			if(broken && fix) {
				...
				if(writeclumphead(arena, aa, &cl) < 0) {
					fprint(2, "%s: can't write corrected "
					          "clump header: %r",
					       arena->name);
					err |= SyncFixErr;
				}
		...
		if(!broken && readclumpinfo(arena, clump, &ci) < 0) {
			...
			if(clumpinfocmp(&ci, &zci) == 0) {
				err |= SyncCIZero;
				if(!zok)
					fprint(2, "%s: unwritten clump info "
					          "for clump=%d\n",
					       arena->name, clump);
			} else {
				err |= SyncCIErr;
				fprint(2, "%s: bad clump info for clump=%d\n",
				       arena->name, clump);
				fprint(2, "\texpected score=%V type=%d size=%d "
				          "uncsize=%d\n",
				       cl.info.score, cl.info.type,
				       cl.info.size, cl.info.uncsize);
				fprint(2, "\tfound score=%V type=%d size=%d "
				          "uncsize=%d\n",
		...
		if(broken && fix) {
			...
			if(writeclumpinfo(arena, clump, &ci) < 0) {
				fprint(2, "%s: can't write correct clump "
				          "directory: %r\n",
				       arena->name);
				err |= SyncFixErr;
			}
		}
		trace(TraceProc, "syncarena unindexed clump %V %d",
		      cl.info.score, arena->memstats.clumps);
	...
	if(used != arena->memstats.used || clumps != arena->memstats.clumps ||
	   cclumps != arena->memstats.cclumps ||
	   uncsize != arena->memstats.uncsize) {
		err |= SyncHeader;
		fprint(2, "arena %s: fix=%d flush=%d %lld->%lld %ud->%ud "
		          "%ud->%ud %lld->%lld\n",
		       arena->name, fix, flush, used, arena->memstats.used,
		       clumps, arena->memstats.clumps, cclumps,
		       arena->memstats.cclumps, uncsize,
