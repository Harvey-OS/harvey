...
rdarena(Arena* arena, uint64_t offset)
{
	...
	if(offset != ~(uint64_t)0) {
		if(offset >= e - a)
			sysfatal("bad offset %llud >= %llud", offset, e - a);
		aa = offset;
	} else
		aa = 0;
	...
	for(; aa < e; aa += ClumpSize + cl.info.size) {
		...
		if(magic != arena->clumpmagic) {
			fprint(
			    2,
			    "illegal clump magic number %#8.8ux offset %llud\n",
			    magic, aa);
			break;
		}
...
threadmain(int argc, char* argv[])
{
	...
	for(p = table; p && *p; p = strchr(p, '\n')) {
		...
		if(readpart(part, offset + head.size - head.blocksize, buf,
		            head.blocksize) < 0) {
			fprint(2, "%s: read %s: %r\n", argv0, file);
			continue;
		}
		...
		print("arena %s %lld clumps=%,d cclumps=%,d used=%,lld "
		      "uncsize=%,lld%s\n",
		      tail.name, offset, tail.diskstats.clumps,
		      tail.diskstats.cclumps, tail.diskstats.used,
		      tail.diskstats.uncsize,
