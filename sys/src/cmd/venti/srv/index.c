 ...
 * random variation.  So the index disk space should be 5% of the arena disk
 *space.
 */
...
initindex(char* name, ISect** sects, int n)
{
	...
	if(namecmp(ix->name, name) != 0) {
		seterr(ECorrupt, "mismatched index name: found %s expected %s",
		       ix->name, name);
		return nil;
	}
	if(ix->nsects != n) {
		seterr(ECorrupt,
		       "mismatched number index sections: found %d expected %d",
		       n, ix->nsects);
		freeindex(ix);
		return nil;
	}
	...
	for(i = 0; i < ix->nsects; i++) {
		...
		if(namecmp(ix->name, is->index) != 0 ||
		   is->blocksize != blocksize || is->tabsize != tabsize ||
		   namecmp(is->name, ix->smap[i].name) != 0 ||
		   is->start != ix->smap[i].start ||
		   is->stop != ix->smap[i].stop || last != is->start ||
		   is->start > is->stop) {
			seterr(ECorrupt, "inconsistent index sections in %s",
			       ix->name);
			freeindex(ix);
			return nil;
		}
...
initindex1(Index* ix)
{
	...
	if(buckets != ix->buckets) {
		seterr(ECorrupt,
		       "inconsistent math for divisor and buckets in %s",
		       ix->name);
		return -1;
	}
...
wbindex(Index* ix)
{
	...
	if(outputindex(&f, ix) < 0) {
		seterr(EOk, "can't make index configuration: table storage too "
		            "small %d",
		       ix->tabsize);
		freezblock(b);
		return -1;
	...
	for(i = 0; i < ix->nsects; i++) {
		if(writepart(ix->sects[i]->part, ix->sects[i]->tabbase, b->data,
		             ix->tabsize) < 0 ||
...
parseindex(IFile* f, Index* ix)
{
	...
	if(ifileu32int(f, &v) < 0) {
		seterr(ECorrupt, "syntax error: bad version number in %s",
		       f->name);
		return -1;
	}
	ix->version = v;
	 ...
	 */
	if(ifileu32int(f, &v) < 0) {
		seterr(ECorrupt, "syntax error: bad block size number in %s",
		       f->name);
		return -1;
	}
	ix->blocksize = v;
...
newindex(char* name, ISect** sects, int n)
{
	...
	for(i = 0; i < n; i++) {
		 ...
		 */
		if(sects[i]->index[0] != '\0' &&
		   strcmp(sects[i]->index, name) != 0) {
			seterr(EOk,
			       "creating new index using non-empty section %s",
			       sects[i]->name);
			return nil;
		}
		...
		if(tabsize != sects[i]->tabsize) {
			seterr(
			    EOk,
			    "mismatched config table sizes in index sections");
			return nil;
		}
		nb += sects[i]->blocks;
	}
	...
	for(i = 0; i < n; i++) {
		for(j = i + 1; j < n; j++) {
			if(namecmp(sects[i]->name, sects[j]->name) == 0) {
				seterr(EOk,
				       "duplicate section name %s for index %s",
				       sects[i]->name, name);
				return nil;
			}
		}
	...
	if(nb >= ((uint64_t)1 << 32)) {
		fprint(
		    2,
		    "%s: index is 2^32 blocks or more; ignoring some of it\n",
		    argv0);
		nb = ((uint64_t)1 << 32) - 1;
	}
	...
	for(i = 0; i < n; i++) {
		...
		if(sects[i]->start != 0 || sects[i]->stop != 0)
			if(sects[i]->start != start || sects[i]->stop != stop) {
				seterr(EOk, "creating new index using "
				            "non-empty section %s",
				       sects[i]->name);
				return nil;
			}
...
initisect1(ISect* is)
{
	...
	is->tabbase =
	    (PartBlank + HeadSize + is->blocksize - 1) & ~(is->blocksize - 1);
	if(is->tabbase >= is->blockbase) {
		seterr(ECorrupt,
		       "index section config table overlaps bucket storage");
		freeisect(is);
		return nil;
	}
	...
	if(is->blockbase + (uint64_t)is->blocks * is->blocksize != v) {
		seterr(ECorrupt, "invalid blocks in index section %s",
		       is->name);
		/* ZZZ what to do?
		freeisect(is);
		return nil;
...
writeiclump(Index* ix, Clump* c, uint8_t* clbuf)
{
	...
	for(i = ix->mapalloc; i < ix->narenas; i++) {
		...
		if(a != TWID64) {
			...
			ia.blocks =
			    (c->info.size + ClumpSize + (1 << ABlockLog) - 1) >>
			    ABlockLog;
 ...
 */
Arena*
amapitoag(Index* ix, uint64_t a, uint64_t* gstart, uint64_t* glimit, int* g)
{
	uint64_t aa;
	Arena* arena;
...
{
	return ia1->type != ia2->type || ia1->size != ia2->size ||
	       ia1->blocks != ia2->blocks || ia1->addr != ia2->addr;
}
...
okibucket(IBucket* ib, ISect* is)
{
	...
	seterr(EICorrupt,
	       "corrupted disk index bucket: n=%ud max=%ud, range=[%lud,%lud)",
             ...
             IBucket* ib, int mode)
{
	...
	if(buck < is->start || is->stop <= buck) {
		seterr(EAdmin,
		       "index lookup out of range: %ud not found in index\n",
		       buck);
		return nil;
	}

	buck -= is->start;
	if((b = getdblock(is->part,
