...
threadmain(int argc, char* argv[])
{
		...
		for(j = 0; j < ap->narenas; j++) {
			...
			if(n < ix->narenas) {
				if(arenas[n] != ix->arenas[n])
					sysfatal("mismatched arenas %s and %s "
					         "at slot %d",
					         arenas[n]->name,
					         ix->arenas[n]->name, n);
				amap[n] = ix->amap[n];
				if(amap[n].start != addr)
					sysfatal(
					    "mis-located arena %s in index %s",
				...
				if(0)
					fprint(2,
					       "add arena %s at [%lld,%lld)\n",
					       amap[n].name, amap[n].start,
					       amap[n].stop);
			}
	...
	if(0) {
		fprint(2,
		       "configured index=%s with arenas=%d and storage=%lld\n",
		       ix->name, n, addr - IndexBase);
		fprint(2, "\tbuckets=%d\n", ix->buckets);
	}
	fprint(2,
	       "fmtindex: %,d arenas, %,d index buckets, %,lld bytes storage\n",
