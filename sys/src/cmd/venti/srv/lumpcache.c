...
downheap(int i, Lump* b)
{
	...
	for(;; i = k) {
		...
		if(k >= lumpcache.nheap)
			break;
		if(k + 1 < lumpcache.nheap &&
		   lumpcache.heap[k]->used2 - now >
		       lumpcache.heap[k + 1]->used2 - now)
			k++;
		bb = lumpcache.heap[k];
		if(b->used2 - now <= bb->used2 - now)
			break;
...
findblock(Lump* bb)
{
	...
	sysfatal("block score=%V type=%#x missing from hash table", bb->score,
	         bb->type);
}
...
checklumpcache(void)
{
	...
	for(i = 0; i < lumpcache.nheap; i++) {
		if(lumpcache.heap[i]->heap != i)
			sysfatal("lc: mis-heaped at %d: %d", i,
			         lumpcache.heap[i]->heap);
		if(i > 0 &&
		   lumpcache.heap[(i - 1) >> 1]->used2 - now >
		       lumpcache.heap[i]->used2 - now)
	...
	if(lumpcache.avail != lumpcache.allowed - size) {
		fprint(2,
		       "mismatched available=%d and allowed=%d - used=%d space",
		       lumpcache.avail, lumpcache.allowed, size);
		*(int*)0 = 0;
	}
	...
	if(lumpcache.nheap + nfree + refed != lumpcache.nblocks)
		sysfatal("lc: missing blocks: %d %d %d %d", lumpcache.nheap,
		         refed, nfree, lumpcache.nblocks);
}
