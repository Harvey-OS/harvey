	...
	for(i = 0; i < len; i += 64 * K) {
		...
		for(j = i; j < len && j < i + 64 * K; j += 4 * K) {
			...
			for(k = j; k < len && k < j + 4 * K; k += 512) {
				if(readpart(part, offset + k, buf + k, 512) >=
				   0)
					continue;
...
sbupdate(Shabuf* sb, uint8_t* p, int64_t offset, int len)
{
	...
	if(sb->offset < offset || sb->offset >= offset + len) {
		if(0)
			print("sbupdate %p %#llux+%d but offset=%#llux\n", p,
			      offset, len, sb->offset);
		return;
	}
	x = sb->offset - offset;
	if(0)
		print("sbupdate %p %#llux+%d skip %d\n", sb, offset, len, x);
	if(x) {
		p += x;
		offset += x;
		len -= x;
	...
	while(len > 0) {
		...
		if(o % (4 * M) == 0) {
			...
			if(x >= sb->nhist) {
				...
				sb->nhist += 32;
				sb->hist = vtrealloc(
				    sb->hist, sb->nhist * sizeof *sb->hist);
			}
			sb->hist[x] = sb->state;
		}
	}
...
enum { D = 0x10000, Z = 0x20000, S = 0x30000, T = 0x40000, N = 0xFFFF };
...
Info partinfo[] = {4,           "magic", D | 4,       "version", Z | 4,
                   "blocksize", 4,       "arenabase", 0};

Info headinfo4[] = {4, "magic", D | 4, "version", S | ANameSize, "name", Z | 4,
                    "blocksize", Z | 8, "size", 0};

Info headinfo5[] = {4, "magic", D | 4, "version", S | ANameSize, "name", Z | 4,
                    "blocksize", Z | 8, "size", 4, "clumpmagic", 0};

Info tailinfo4[] = {4, "magic", D | 4, "version", S | ANameSize, "name", D | 4,
                    "clumps", D | 4, "cclumps", T | 4, "ctime", T | 4, "wtime",
                    D | 8, "used", D | 8, "uncsize", 1, "sealed", 0};

Info tailinfo4a[] = {
    /* tailinfo 4 */
    4, "magic", D | 4, "version", S | ANameSize, "name", D | 4, "clumps", D | 4,
    "cclumps", T | 4, "ctime", T | 4, "wtime", D | 8, "used", D | 8, "uncsize",
    1, "sealed",

    /* mem stats */
    1, "extension", D | 4, "mem.clumps", D | 4, "mem.cclumps", D | 8,
    "mem.used", D | 8, "mem.uncsize", 1, "mem.sealed", 0};

Info tailinfo5[] = {4, "magic", D | 4, "version", S | ANameSize, "name", D | 4,
                    "clumps", D | 4, "cclumps", T | 4, "ctime", T | 4, "wtime",
                    4, "clumpmagic", D | 8, "used", D | 8, "uncsize", 1,
                    "sealed", 0};

Info tailinfo5a[] = {
    /* tailinfo 5 */
    4, "magic", D | 4, "version", S | ANameSize, "name", D | 4, "clumps", D | 4,
    "cclumps", T | 4, "ctime", T | 4, "wtime", 4, "clumpmagic", D | 8, "used",
    D | 8, "uncsize", 1, "sealed",
...
showdiffs(uint8_t* want, uint8_t* have, int len, Info* info)
{
		...
		if(memcmp(have, want, n) != 0) {
			...
			case Z | 4:
				print("\t%s: correct=%z disk=%z\n", info->name,
				      (uint64_t)u32(want), (uint64_t)u32(have));
				break;
			case D | 8:
	...
	if(len > 0 && memcmp(have, want, len) != 0) {
		if(memcmp(want, zero, len) != 0)
			print("!!\textra want data in showdiffs (bug in "
			      "fixarenas)\n");
		else
...
static int tabsizes[] = {
    16 * 1024, 64 * 1024, 512 * 1024, 768 * 1024,
};
/*
...
guessgeometry(void)
{
	...
	if(arenasize == 0 || ap.blocksize == 0) {
		/*
		 * The ArenaPart block at offset PartBlank may be corrupt or
		 *just wrong.
		 * Instead, look for the individual arena headers and tails,
		 *which there
		 * are many of, and once we've seen enough, infer the spacing.
		 *
		 * Of course, nothing in the file format requires that arenas be
		 *evenly
	...
	if(ap.arenabase == 0) {
		...
		for(i = 0; i < nelem(tabsizes); i++) {
			ap.arenabase = ROUNDUP(
			    PartBlank + HeadSize + tabsizes[i], ap.blocksize);
...
checkarenas(char* range)
{
	...
	if(range == nil) {
		...
		for(s = range; *s; s = t) {
			...
			for(i = lo; i <= hi; i++)
				checkarena(
				    ap.arenabase + (int64_t)i * arenasize, i);
		}
	}
}
...
loadarenabasics(int64_t offset0, int anum, ArenaHead* head, Arena* arena)
{
	...
	p = pagein(offset0 + arena->size - arena->blocksize, arena->blocksize);
           ...
           uint8_t* oldscore, uint8_t* score)
{
		...
		if((n = isclump(p, &cl, &magic)) > 0) {
			...
			if(inbad) {
				...
				if(needtozero) {
					...
					print("corrupt clump data - "
					      "%#llux+%#llux (%,llud bytes)\n",
					      lastclumpend, v, v);
				}
				addcicorrupt(v);
				totalcorrupt += v;
				...
				minclumps +=
				    (v + ClumpSize + VtMaxLumpSize - 1) /
				    (ClumpSize + VtMaxLumpSize);
				nb2 = (minclumps + ncib - 1) / ncib;
				eoffset -= (nb2 - nb1) * arena->blocksize;
			}

			if(haveclump(cl.info.score))
				print("warning: duplicate clump %d %V at "
				      "%#llux+%#d\n",
				      cl.info.type, cl.info.score, offset, n);

			/*
			 * If clumps use different magic numbers, we don't care.
			 * We'll just use the first one we find and make the
			 * others
			 * follow suit.
			 */
			if(arena->clumpmagic == 0) {
				print(
				    "clump type %d size %d score %V magic %x\n",
				    cl.info.type, cl.info.size, cl.info.score,
				    magic);
			...
			if(verbose > 1)
				print("\tclump %d: %d %V at %#llux+%#ux (%d)\n",
				      clumps, cl.info.type, cl.info.score,
				      offset, n, n);
			addcibuf(&cl.info, 0);
			if(minclumps % ncib == 0)
			 ...
			 * For now, just record whether it needs to be
			 * overwritten.
	...
	if(clumps < ncibuf)
		clumps = ncibuf;
	clumps +=
	    totalcorrupt / ((arena->diskstats.used - totalcorrupt) / clumps);
...
Again:
	...
	for(cit = bcit; cit < ecit && ci < eci; cit++) {
		if(cit->corrupt) {
			...
			if(smart) {
				/*
				 * If we can, just mark existing entries as
				 * corrupt.
				 */
				n = cit->corrupt;
		...
		if(clumpinfocmp(&cit->ci, ci) != 0) {
			if(verbose && (smart || verbose > 1)) {
				print("clumpinfo %d\n", (int)(ci - bci));
				print("\twant: %d %d %d %V\n", cit->ci.type,
				      cit->ci.size, cit->ci.uncsize,
				      cit->ci.score);
				print("\thave: %d %d %d %V\n", ci->type,
				      ci->size, ci->uncsize, ci->score);
			}
			*ci = cit->ci;
			nbad++;
		}
	...
	if(ci >= eci || cit < ecit) {
		print(
		    "ran out of space editing existing directory; rewriting\n");
		print("# eci %ld ci %ld ecit %ld cit %ld\n", eci - bci,
		      ci - bci, ecit - bcit, cit - bcit);
	...
	print("eoffset=%lld lastclumpend=%lld diff=%lld unseal=%d\n", eoffset,
	      lastclumpend, eoffset - lastclumpend, unseal);
...
Nocib:
	if(eoffset - lastclumpend > 64 * 1024 &&
	   (!arena->diskstats.sealed || unseal)) {
		if(arena->diskstats.sealed)
...
checkarena(int64_t offset, int anum)
{
	...
	if(memcmp(dbuf, p, arena.blocksize) != 0) {
		...
		showdiffs(dbuf, p, arena.blocksize,
		          arena.version == ArenaVersion4 ? headinfo4
		                                         : headinfo5);
	}
	...
	if(arena.diskstats.sealed) {
		if(oarena.diskstats.sealed)
			if(scorecmp(p + arena.blocksize - VtScoreSize,
			            oldscore) != 0) {
		...
		if(fix &&
		   scorecmp(p + arena.blocksize - VtScoreSize, score) != 0) {
...
buildamap(void)
{
	...
	for(o = ap.arenabase; o < partend; o += arenasize) {
		...
		if(unpackarenahead(&h, p) >= 0) {
			an->map =
			    vtrealloc(an->map, (an->n + 1) * sizeof an->map[0]);
			m = &an->map[an->n++];
			m->start = o;
			m->stop = o + h.size;
