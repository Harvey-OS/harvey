...
initpart(char* name, int mode)
{
	...
	if(parsepart(name, &file, &lo, &hi) < 0)
		return nil;
	trace(TraceDisk, "initpart %s file %s lo 0x%llx hi 0x%llx", name, file,
	      lo, hi);
	part = MKZ(Part);
	...
	if(dir->length < hi || dir->length < lo) {
		freepart(part);
		seterr(EOk, "partition '%s': bounds out of range (max %lld)",
		       name, dir->length);
		free(dir);
		free(file);
		return nil;
...
enum { Maxxfer = 64 * 1024, /* for NCR SCSI controllers; was 128K */
};

static int reopen(Part*);

int
rwpart(Part* part, int isread, uint64_t offset0, uint8_t* buf0, uint32_t count0)
{
	...
	trace(TraceDisk, "%s %s %ud at 0x%llx", isread ? "read" : "write",
	      part->name, count0, offset0);
	if(offset0 >= part->size || offset0 + count0 > part->size) {
		seterr(EStrange, "out of bounds %s offset 0x%llux count %ud to "
		                 "partition %s size 0x%llux",
	...
	while(count > 0) {
		...
		if(n <= 0) {
			seterr(EAdmin, "%s %s offset 0x%llux count %ud buf %p "
			               "returned %d: %r",
			       isread ? "read" : "write", part->filename,
			       offset, opsize, buf, n);
			return -1;
		}
		offset += n;
