...
static void allocminima(Allocs* all) /* enforce minima for sanity */
{
...
usage(void)
{
	fprint(2, "usage: venti [-Ldrsw] [-a ventiaddr] [-c config] "
	          "[-h httpaddr] [-m %%mem] [-B blockcachesize] [-C cachesize] "
	          "[-I icachesize] "
	          "[-W webroot]\n");
...
threadmain(int argc, char* argv[])
{
...
#ifdef PLAN9PORT
	{
		/* sigh - needed to avoid signals when writing to hungup
		 * networks */
		struct sigaction sa;
	 ...
	 */
	allocs =
	    sizeallocs((Allocs){mem, bcmem, icmem, stfree, mempcnt}, &config);
	mem = allocs.mem;
	bcmem = allocs.bcmem;
	icmem = allocs.icmem;
	fprint(2, "%s: mem %,ud bcmem %,ud icmem %,ud...", argv0, mem, bcmem,
	       icmem);
	...
	if(initarenasum() < 0)
		fprint(2,
		       "warning: can't initialize arena summing process: %r");
	...
	while((r = vtgetreq(ventisrv)) != nil) {
		...
		if(0)
			print("req (arenas[0]=%p sects[0]=%p) %F\n",
			      mainindex->arenas[0], mainindex->sects[0],
			      &r->tx);
		switch(r->tx.msgtype) {
		default:
		...
		case VtTread:
			ms = msec();
			r->rx.data = readlump(r->tx.score, r->tx.blocktype,
			                      r->tx.count, &cached);
			ms = msec() - ms;
			...
			if(r->rx.data == nil) {
				...
				addstat(StatRpcReadBytes,
				        packetsize(r->rx.data));
				addstat(StatRpcReadOk, 1);
				if(cached)
					addstat2(StatRpcReadCached, 1,
					         StatRpcReadCachedTime, ms);
				else
					addstat2(StatRpcReadUncached, 1,
					         StatRpcReadUncachedTime, ms);
			}
			break;
		case VtTwrite:
			if(readonly) {
