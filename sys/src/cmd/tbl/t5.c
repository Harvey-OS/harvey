...
gettbl(void)
{
	...
	textflg = 0;
	for(nlin = nslin = 0; gets1(cstore, MAXCHS - (cstore - cspace));
	    nlin++) {
		stynum[nlin] = nslin;
		if(prefix(".TE", cstore)) {
			leftover = 0;
		...
		if(nodata(nlin)) {
			if(ch = oneh(nlin))
				fullbot[nlin] = ch;
			table[nlin] = (struct colstr*)alocv(
		...
		table[nlin] =
		    (struct colstr*)alocv((ncol + 2) * sizeof(table[0][0]));
		if(cstore[1] == 0)
			switch(cstore[0]) {
			case '_':
				fullbot[nlin] = '-';
		...
		for(icol = 0; icol < ncol; icol++) {
			...
			if(match(cstore, "T{")) { /* text follows */
				...
				*cstore++ = '\0';
				switch(ctype(
				    nlin,
				    icol)) /* numerical or alpha, subcol */ {
				case 'n':
					table[nlin][icol].rcol =
