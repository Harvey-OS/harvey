	...
	while(c = get1char()) {
		...
		default:
			if(c != tab) {
				char buf[64];
				sprint(buf,
				       "bad table specification character %c",
				       c);
				error(buf);
			}
		...
		case '.': /* end of table specification */
			...
			if(c == '.') {
				...
				if(oncol == 0)
					...
					error("tried to widen table in T&, not "
					      "allowed");
				return;
			}
			sawchar = 0;
		...
		case 'a':
			...
			if(c == 's' && style[icol - 1][nclin] == 'a') {
				Bprint(&tabout, ".tm warning: can't span "
				                "a-type cols, changed to l\n");
				style[icol - 1][nclin] = 'l';
			}
			if(c == 's' && style[icol - 1][nclin] == 'n') {
				Bprint(&tabout, ".tm warning: can't span "
				                "n-type cols, changed to c\n");
		...
		case 'W':
			snp = cll[icol - 1];
			/* Dale Smith didn't like this check - possible to have
		two text blocks
		   of different widths now ....
			if (*snp)
			        {
			        Bprint(&tabout, "Ignored second width
		specification");
			        continue;
			        }
...
garray(int qcol)
{
	...
	lefline = (int(*)[])getcore(
	    MAXHEAD * (qcol + 1),
	    sizeof(int)); /*+1 for sample55 loop - others may need it too*/
...
freearr(void)
{
	...
	free(vsize);
	free(cll);
	free(--sep); /* netnews says this should be --sep because incremented
	                earlier! */
	free(used);
	free(lused);
	free(rused);
