...
void maktab(void) /* define the tab stops of the table */
{
	...
	for(icol = 0; icol < ncol; icol++) {
		...
		for(text = 0; text < 2; text++) {
			...
			for(ilin = 0; ilin < nlin; ilin++) {
				...
				case 'c':
				case 'l':
					if(real(ss = table[ilin][icol].col) &&
					   !vspen(ss)) {
						s = (int)(uintptr)ss;
						if(tx(s) != text)
							continue;
						Bprint(&tabout, ".nr %d ", TMP);
						wide(ss, FN(vforml, icol),
						     SZ(vforml, icol));
						Bprint(&tabout, "\n");
						Bprint(&tabout,
						       ".if \\n(%2s<\\n(%d .nr "
		...
		if(acase[icol]) {
			Bprint(&tabout,
			       ".if \\n(%d>=\\n(%2s .nr %2s \\n(%du+2n\n", S2,
			       reg(icol, CRIGHT), reg(icol, CRIGHT), S2);
		}
		if(doubled[icol]) {
			Bprint(&tabout, ".nr %2s \\n(%d\n", reg(icol, CMID),
			       S1);
	...
	if(evenflg) {
		...
		for(icol = 0; icol < ncol; icol++) {
			...
			/* that nonsense with the 100's and parens tries
			           to avoid overflow while proportionally
			   shifting
	...
	if(rightl)
		Bprint(&tabout, ".nr %s (\\n(%s+\\n(%s)/2\n",
		       reg(ncol - 1, CRIGHT), reg(ncol - 1, CLEFT),
		       reg(ncol - 2, CRIGHT));
	Bprint(&tabout, ".nr TW \\n(%2s\n", reg(ncol - 1, CRIGHT));
	tsep = sep[ncol - 1];
	if(boxflg || allflg || dboxflg)
		Bprint(&tabout, ".nr TW +((%d*\\n(%d)/2)\n", tsep, TMP);
	Bprint(&tabout, ".if t .if (\\n(TW>\\n(.l .tm Table at line %d file %s "
	                "is too wide - \\n(TW units\n",
	       iline - 1, ifile);
	/*
	 * Used to be:
	            ".if t .if (\\n(TW+\\n(.o)>7.65i .tm Table at line %d file
	 %s is too wide - \\n(TW units\n", iline - 1, ifile);
