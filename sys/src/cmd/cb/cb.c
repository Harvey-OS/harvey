...
work(void)
{
	...
	while((c = getch()) != Beof) {
		...
		case '}':
			...
			ct = getnl();
			if((dolevel && clev->tabs <= dotabs[dolevel]) || (structlev) || (lptr != 0 && lptr->type == ELSE && clev->pdepth == 0)) {
				if(c == ';') {
					OUTK;
...
keep(struct op *o)
{
	...
	if(o->blanks == NEVER)
		...
		ok = YES;
	if(strict && ((o->blanks & ALWAYS) || ((opflag == 0 && o->blanks & SOMETIMES) && clev->tabs != 0)))
		putspace(' ', YES);
	for(s = o->name; *s != '\0'; s++) {
		if(*(s + 1) == '\0')
			putch(*s, ok);
...
getnext(int must)
{
	...
	if(inswitch && tptr <= lastplace)
		if(isalnum(*lastplace) || ispunct(*lastplace) || isop(*lastplace))
			return (lastplace);
space:
