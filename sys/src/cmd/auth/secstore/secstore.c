...
static int
getfile(SConn* conn, char* gf, uint8_t** buf, uint32_t* buflen, uint8_t* key,
        int nkey)
{
	...
	for(nr = 0; nr < len;) {
		if((n = conn->read(conn, ibw, Maxmsg)) <= 0) {
			fprint(2, "secstore: empty file chunk n=%d nr=%d "
			          "len=%d: %r\n",
			       n, nr, len);
			return -1;
		}
		...
		if(n > 0) {
			if(buf == nil) {
				nw = write(fd, ibr, n);
				if(nw != n) {
					fprint(2, "secstore: write error on %s",
					       gf);
					return -1;
				}
			} else {
...
cmd(AuthConn* c, char** gf, int* Gflag, char** pf, char** rf)
{
	...
	while(*gf != nil) {
		...
		if(getfile(c->conn, *gf, *Gflag ? &memfile : nil, &len,
			...
			goto Out;
		if(*Gflag) {
			/* write 1 line at a time, as required by
			 * /mnt/factotum/ctl */
			memcur = memfile;
			while(len > 0) {
	...
	while(*pf != nil) {
		if(verbose)
			fprint(2, "put %s\n", *pf);
		if(putfile(c->conn, *pf, nil, 0, (uint8_t*)c->pass,
		           c->passlen) < 0)
			goto Out;
		pf++;
	}
	...
	for(;;) {
		...
		if(!pass_stdin && !pass_nvram) {
			...
			if(strlen(pass) >= sizeof c->pass) {
				fprint(2, "secstore: password too long, "
				          "skipping secstore login\n");
		...
		if(c->pass[0] == 0) {
			fprint(2, "secstore: null password, skipping secstore "
			          "login\n");
			exits("no password");
		}
...
main(int argc, char** argv)
{
	...
	if(chpass && (ngfile || npfile || nrfile)) {
		fprint(2, "secstore: Get, put, and remove invalid with "
		          "password change.\n");
		exits("usage");
	}
