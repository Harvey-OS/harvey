...
SC_read(SConn* conn, uint8_t* buf, int n)
{
	...
	if(ss->alg) {
		...
		if((nr = readn(ss->fd, buf, len)) != len) {
			werrstr("!SC_read expected %d bytes, but got %d", len,
			        nr);
			return -1;
		}
	}
...
getfile(SConn* conn, uint8_t* key, int nkey)
{
	...
	for(nr = 0; nr < len;) {
		if((n = conn->read(conn, ibw, Maxmsg)) <= 0) {
			werrstr(
			    "secstore: empty file chunk n=%d nr=%d len=%d: %r",
			    n, nr, len);
			return -1;
		}
		nr += n;
...
// This group was generated by the seed
// EB7B6E35F7CD37B511D96C67D6688CC4DD440E1E.
static void
initPAKparams(void)
{
	if(pak)
	...
	pak->q =
	    strtomp("E0F0EF284E10796C5A2A511E94748BA03C795C13", nil, 16, nil);
	pak->p = strtomp(
	    "C41CFBE4D4846F67A3DF7DE9921A49D3B42DC33728427AB159CEC8CBBD"
	    "B12B5F0C244F1A734AEB9840804EA3C25036AD1B61AFF3ABBC247CD4B384224567"
	    "A86"
	    "3A6F020E7EE9795554BCD08ABAD7321AF27E1E92E3DB1C6E7E94FAAE590AE9C48F"
	    "96D9"
	    "3D178E809401ABE8A534A1EC44359733475A36A70C7B425125062B1142D",
	    nil, 16, nil);
	pak->r = strtomp(
	    "DF310F4E54A5FEC5D86D3E14863921E834113E060F90052AD332B3241CEF"
	    "2497EFA0303D6344F7C819691A0F9C4A773815AF8EAECFB7EC1D98F039F17A32A7"
	    "E887"
	    "D97251A927D093F44A55577F4D70444AEBD06B9B45695EC23962B175F266895C67"
	    "D21"
	    "C4656848614D888A4",
	    nil, 16, nil);
	pak->g = strtomp(
	    "2F1C308DC46B9A44B52DF7DACCE1208CCEF72F69C743ADD4D2327173444"
	    "ED6E65E074694246E07F9FD4AE26E0FDDD9F54F813C40CB9BCD4338EA6F242AB94"
	    "CD41"
	    "0E676C290368A16B1A3594877437E516C53A6EEE5493A038A017E955E218E78197"
	    "34E3E"
...
static void
shorthash(char* mess, char* C, char* S, char* m, char* mu, char* sigma,
          char* Hi, uint8_t* digest)
{
	SHA1state* state;
...
{
	char* mess, *mess2, *eol, *S, *hexmu, *ks, *hexm, * hexsigma = nil,
	                                                    *hexHi;
...
secstorefetch(char* password)
{
	...
	if(getfile(conn, (uint8_t*)pass, strlen(pass)) < 0)
		goto Out;
