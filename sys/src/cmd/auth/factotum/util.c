...
canusekey(Fsstate* fss, Key* k)
{
	...
	if(_strfindattr(k->attr, "confirm")) {
		...
		if(fss->nconf % 16 == 0)
			fss->conf =
			    erealloc(fss->conf, (fss->nconf + 16) *
			                            (sizeof(fss->conf[0])));
	...
	*ret = nil;
	...
	for(i = 0; i < ring->nkey; i++) {
		...
		if(_strfindattr(k->attr, "disabled") && !ki->usedisabled)
			continue;
		if(matchattr(attr0, k->attr, k->privattr) &&
		   matchattr(attr1, k->attr, k->privattr)) {
			/* check ownership */
			if(!matchattr(attr2, k->attr, nil) &&
			   !matchattr(attr3, k->attr, nil))
				continue;
			if(nmatch++ < ki->skip)
				continue;
			if(!ki->noconf) {
	...
	flog("%d: no key matches %A %A %A %A", ki->fss->seqnum, attr0, attr1,
	     attr2, attr3);
	werrstr("no key matches %A %A", attr0, attr1);
	_freeattr(attr2);
	_freeattr(attr3);
	s = RpcFailure;
	if(askforkeys && who == nil &&
	   (hasqueries(attr0) || hasqueries(attr1))) {
		if(nmatch == 0) {
			...
			*l = attr1;
			...
			snprint(ki->fss->keyinfo, sizeof ki->fss->keyinfo, "%A",
			        attr0);
			_freeattr(attr0);
...
findp9authkey(Key** k, Fsstate* fss)
{
	...
	if(dom = _strfindattr(fss->attr, "dom"))
		return findkey(k, &ki, "proto=p9sk1 dom=%q role=server user?",
		               dom);
	else
...
getnvramkey(int flag, char** secstorepw)
{
	...
	snprint(s, 512,
	        "key proto=p9sk1 user=%q dom=%q !hex=%.*H !password=______",
	...
	for(; pat; pat = pat->next) {
		...
		case AttrNameval: /* name=val must be present */
			if(!hasnameval(a0, a1, pat->name, pat->val))
				return 0;
			break;
		case AttrDefault: /* name=val must be present if name=anything
		                     is present */
			if(hasname(a0, a1, pat->name) &&
			   !hasnameval(a0, a1, pat->name, pat->val))
				return 0;
			break;
		}
	}
...
phaseerror(Fsstate* s, char* op)
{
	...
	werrstr("protocol phase error: %s in state %s", op,
	        phasename(s, s->phase, tmp));
	return RpcPhase;
}
...
replacekey(Key* kn, int before)
{
	...
	for(i = 0; i < ring->nkey; i++) {
		k = ring->key[i];
		if(matchattr(kn->attr, k->attr, nil) &&
		   matchattr(k->attr, kn->attr, nil)) {
			closekey(k);
			kn->ref++;
			ring->key[i] = kn;
			return 0;
	...
	if(ring->nkey % 16 == 0)
		ring->key = erealloc(ring->key,
		                     (ring->nkey + 16) * sizeof(ring->key[0]));
	kn->ref++;
	if(before) {
		memmove(ring->key + 1, ring->key,
		        ring->nkey * sizeof ring->key[0]);
		ring->key[0] = kn;
		ring->nkey++;
	} else
...
disablekey(Key* k)
{
	...
	if(a)
		...
		k->attr =
		    _mkattr(AttrNameval, "disabled", "by.factotum",
		            nil); /* not reached: always a proto attribute */
}
