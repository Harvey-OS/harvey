...
convthread(void *v)
{
	...
	/*
	 * defeat russ' fix to keys being used as server keys
	 * when ment only as client auth so as not to break
	 * most keys on the system.
	 */
	if(role==nil && 1 /* accept traditional keys */)
	if(strcmp(proto, "pass") == 0)
		role = "client";
	if(role == nil){
...
rpcrespondn(Conv *c, char *verb, void *data, int count)
{
	...
	if(need > sizeof c->reply){
		print("RPC response too large; caller %#p", getcallerpc(&c));
		return;
	}
