 ...
 *	Client->Server: proto domain, tokenize separated (not proto@domain), nul
 *terminated
 *
 * Server protocol:
...
static char* phasenames[Maxphase] = {
        [CNeedProtos] "CNeedProtos", [CHaveProto] "CHaveProto",
        [CNeedOK] "CNeedOK",         [CRelay] "CRelay",
        [SHaveProtos] "SHaveProtos", [SNeedProto] "SNeedProto",
        [SHaveOK] "SHaveOK",         [SRelay] "SRelay",
};
...
p9anyread(Fsstate* fss, void* a, uint* n)
{
	...
	case SHaveProtos:
		...
		for(i = 0; i < nelem(negotiable); i++) {
			anew = setattr(_copyattr(fss->attr), "proto=%q dom?",
			               negotiable[i]->name);
			ki.attr = anew;
			for(ki.skip = 0; findkey(&k, &ki, nil) == RpcOk;
			    ki.skip++) {
				if(m++)
					s_append(negstr, " ");
...
findneg(char* name)
{
	...
	for(i = 0; i < nelem(negotiable); i++)
		if(strncmp(negotiable[i]->name, name, len) == 0 &&
		   negotiable[i]->name[len] == 0)
			return negotiable[i];
	return nil;
}
...
p9anywrite(Fsstate* fss, void* va, uint n)
{
	...
	case CNeedProtos:
		 ...
		 */
		anew =
		    _delattr(_delattr(_copyattr(fss->attr), "proto"), "role");
		...
		for(i = (s->version == 1 ? 0 : 1); i < m; i++) {
			...
			if(user == nil || strcmp(user, fss->sysuser) == 0) {
				...
				ki.user = nil;
				ret = findkey(
				    &k, &ki, "proto=%q dom=%q role=speakfor %s",
		...
		/*
		 * no acceptable key, go through the proto@domains one at a
		 * time.
		 */
		asking = 0;
		if(k == nil) {
	...
	case SNeedProto:
		...
		ki.attr = attr;
		ki.user = nil;
		ret = findkey(&k, &ki, "proto=%q dom=%q role=server", token[0],
		              token[1]);
		free(a);
		_freeattr(attr);
		if(ret == RpcConfirm)
