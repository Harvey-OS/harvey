		...
		for(last = minute(last); last <= now; last += Minute) {
			...
			for(i = 0; i < nuser; i++)
				for(j = users[i].jobs; j; j = j->next)
					if(j->time.min & t.min && j->time.hour & t.hour && j->time.wday & t.wday && j->time.mday & t.mday && j->time.mon & t.mon)
						rexec(&users[i], j);
		}
		seek(lock, 0, 0);
...
readjobs(char *file, User *user)
{
	...
	for(line = 1; savec = Brdline(b, '\n'); line++) {
		...
		if(j->time.min && (j->time.hour = gettime(0, 23)) && (j->time.mday = gettime(1, 31)) && (j->time.mon = gettime(1, 12)) && (j->time.wday = gettime(0, 6)) && getname(&j->host)) {
