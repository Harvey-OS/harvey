...
readjobs(char* file, User* user)
{
	...
	for(line = 1; savec = Brdline(b, '\n'); line++) {
		...
		if(j->time.min && (j->time.hour = gettime(0, 23)) &&
		   ...
		   (j->time.wday = gettime(0, 6)) && getname(&j->host)) {
...
rexec(User* user, Job* j)
{
	...
	if(strcmp(j->host, "local") == 0) {
		...
		clog("%s: exec failed for %s on %s: %r", user->name, j->cmd,
		     j->host);
		_exits(0);
	}
	...
	if(becomeuser(user->name) < 0) {
		clog("%s: can't change uid for %s on %s: %r", user->name,
		     j->cmd, j->host);
		_exits(0);
	}
	ai = auth_proxy(fd, nil, "proto=p9any role=client");
	if(ai == nil) {
		clog("%s: can't authenticate for %s on %s: %r", user->name,
		     j->cmd, j->host);
		_exits(0);
	}
