...
main(int argc, char* argv[])
{
	...
	for(;;) {
		...
		case AuthVNC:
			...
			break;
		default:
			syslog(0, AUTHLOG, "unknown ticket request type: %d",
			       buf[0]);
			exits(0);
		}
	}
...
ticketrequest(Ticketreq* tr)
{
	...
	if(findkey(KEYDB, tr->hostid, hkey) == 0) {
		...
		mkkey(hkey);
		if(debug)
			syslog(0, AUTHLOG, "tr-fail hostid %s(%s)", tr->hostid,
			       raddr);
	}
	...
	if(speaksfor(tr->hostid, tr->uid))
		...
		if(debug)
			syslog(0, AUTHLOG,
			       "tr-fail %s@%s(%s) -> %s@%s no speaks for",
	...
	if(write(1, tbuf, 2 * TICKETLEN + 1) < 0) {
		if(debug)
			syslog(0, AUTHLOG, "tr-fail %s@%s(%s): hangup", tr->uid,
			       tr->hostid, raddr);
		exits(0);
	}
	if(debug)
		syslog(0, AUTHLOG, "tr-ok %s@%s(%s) -> %s@%s", tr->uid,
		       tr->hostid, raddr, tr->uid, tr->authid);
...
challengebox(Ticketreq* tr)
{
	...
	if(findkey(KEYDB, tr->hostid, hkey) == 0) {
		...
		mkkey(hkey);
		if(debug)
			syslog(0, AUTHLOG, "cr-fail hostid %s %s@%s",
			       tr->hostid, tr->uid, raddr);
	}
	...
	if(tickauthreply(tr, hkey) < 0) {
		if(debug)
			syslog(0, AUTHLOG, "cr-fail %s@%s(%s): hangup", tr->uid,
			       tr->hostid, raddr);
		exits(0);
	}

	if(debug)
		syslog(0, AUTHLOG, "cr-ok %s@%s(%s)", tr->uid, tr->hostid,
		       raddr);
}
...
changepasswd(Ticketreq* tr)
{
	...
	for(;;) {
		...
		if(pr.changesecret &&
		   setsecret(KEYDB, tr->uid, pr.secret) == 0) {
...
apop(Ticketreq* tr, int type)
{
	...
	p = chal;
	p += snprint(p, sizeof(chal), "<%lux%lux.%lux%lux@%s>", rb[0], rb[1],
	             rb[2], rb[3], domainname());
	challen = p - chal;
	...
	for(tries = 0;; tries++) {
		 ...
		 */
		if(type == AuthCram) {
			hmac_md5((uint8_t*)chal, challen, (uint8_t*)secret,
			         strlen(secret), digest, nil);
		} else {
...
mschap(Ticketreq* tr)
{
	...
	hkey = findkey(KEYDB, tr->hostid, hbuf);
	if(hkey == 0 || secret == 0) {
		replyerror("mschap-fail bad response %s/%s(%s)", tr->uid,
		           tr->hostid, raddr);
		logfail(tr->uid);
		exits(0);
	}
	...
	if(debug)
		replyerror("mschap-ok %s/%s(%s) %ux", tr->uid, tr->hostid,
		           raddr);
