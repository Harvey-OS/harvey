...
main(int argc, char** argv)
{
	...
	for(;;) {
		...
		*pr.new = 0;
		...
		readln("change Inferno/POP password? (y/n) ", buf, sizeof buf,
		       0);
		if(*buf == 'y' || *buf == 'Y') {
			pr.changesecret = 1;
			readln(
			    "make it the same as your plan 9 password? (y/n) ",
			    buf, sizeof buf, 0);
			if(*buf == 'y' || *buf == 'Y') {
				if(*pr.new == 0)
				...
				readln("Secret(0 to 256 characters): ",
				       pr.secret, sizeof pr.secret, 1);
				readln("Confirm: ", buf, sizeof buf, 1);
