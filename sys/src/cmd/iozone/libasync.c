 ...
 * Date: 4/24/1998
 *
 * Two models are supported.  First model is a replacement for read() where the
 *async
 * operations are performed and the requested data is bcopy()-ed back into the
 *users
 ...
 *	stride ........	is the distance, in size units, to space the async
 *reads.
...
/***********************************************/
void async_init(gc, fd, flag) struct cache** gc;
int fd;
int flag;
{
#ifdef VXFS
...
/***********************************************/
void end_async(gc) struct cache* gc;
{
	del_cache(gc);
...
int async_read(gc, fd, ubuffer, offset, size, stride, max, depth) struct cache
    * gc;
long long fd;
char* ubuffer;
off64_t offset;
...
long long depth;
{
	...
	if((ce = (struct cache_ent*)incache(gc, fd, offset, size))) {
...
#ifdef _LARGEFILE64_SOURCE
#ifdef __LP64__
			mbcopy((char*)ce->myaiocb.aio_buf, (char*)ubuffer,
			       (size_t)retval);
#else
			mbcopy((char*)ce->myaiocb64.aio_buf, (char*)ubuffer,
			       (size_t)retval);
#endif
#else
			mbcopy((char*)ce->myaiocb.aio_buf, (char*)ubuffer,
...
#ifdef _LARGEFILE64_SOURCE
#ifdef __LP64__
			printf("aio_return error1: fd %d offset %ld buffer %lx "
			       "size %d Opcode %d\n",
			       ce->myaiocb.aio_fildes, ce->myaiocb.aio_offset,
			       (long)(ce->myaiocb.aio_buf),
			       ce->myaiocb.aio_nbytes,
			       ce->myaiocb.aio_lio_opcode
#else
			printf("aio_return error1: fd %d offset %lld buffer "
			       "%lx size %d Opcode %d\n",
...
#endif
#else
			printf("aio_return error1: fd %d offset %d buffer %lx "
			       "size %d Opcode %d\n",
			       ce->myaiocb.aio_fildes, ce->myaiocb.aio_offset,
			       (long)(ce->myaiocb.aio_buf),
			       ce->myaiocb.aio_nbytes,
		...
		del_read++;
		first_ce =
		    alloc_cache(gc, fd, offset, size, (long long)LIO_READ);
	again:
#ifdef _LARGEFILE64_SOURCE
#ifdef __LP64__
		...
		if(ret != 0) {
			if(errno == EAGAIN)
				...
				printf("error returned from aio_read(). Ret %d "
				       "errno %d\n",
				       ret, errno);
		}
	}
	...
	if(gc->count > 1)
		start = depth - 1;
	for(i = start; i < depth;
	    i++) /* Issue read-aheads for the depth specified */
	    {
...
#else
		while((ret = aio_error64(&first_ce->myaiocb64)) ==
		      EINPROGRESS) {
			async_suspend(first_ce);
		}
#endif
...
#ifdef _LARGEFILE64_SOURCE
#ifdef __LP64__
			printf("aio_return error2: fd %d offset %lld buffer "
			       "%lx size %d Opcode %d\n",
...
#endif
#else
			printf("aio_return error2: fd %d offset %d buffer %lx "
			       "size %d Opcode %d\n",
			       first_ce->myaiocb.aio_fildes,
			       first_ce->myaiocb.aio_offset,
			       (long)(first_ce->myaiocb.aio_buf),
...
#ifdef _LARGEFILE64_SOURCE
#ifdef __LP64__
			mbcopy((char*)first_ce->myaiocb.aio_buf, (char*)ubuffer,
			       (size_t)retval);
#else
			mbcopy((char*)first_ce->myaiocb64.aio_buf,
			       (char*)ubuffer, (size_t)retval);
#endif
#else
			mbcopy((char*)first_ce->myaiocb.aio_buf, (char*)ubuffer,
			       (size_t)retval);
...
struct cache_ent* alloc_cache(gc, fd, offset, size, op) struct cache* gc;
 ...
 * cache.
*************************************************************************/
struct cache_ent* incache(gc, fd, offset, size) struct cache* gc;
long long fd, size;
off64_t offset;
{
	...
	while(move) {
		if((move->fd == fd) &&
		   (move->myaiocb.aio_offset == (off64_t)offset) &&
	...
	while(move) {
		if((move->fd == fd) &&
		   (move->myaiocb64.aio_offset == (off64_t)offset) &&
	...
	while(move) {
		if((move->fd == fd) &&
		   (move->myaiocb.aio_offset == (off_t)offset) &&
		   ((size_t)size == move->myaiocb.aio_nbytes)) {
			return (move);
		}
		move = move->forward;
...
void takeoff_cache(gc, ce) struct cache* gc;
struct cache_ent* ce;
{
	struct cache_ent* move;
	long long found;
 ...
 ************************************************************************/
void del_cache(gc) struct cache* gc;
{
	struct cache_ent* ce;
	ssize_t ret;
	ce = gc->head;
...
#else
		while((ret = aio_cancel64(0, &ce->myaiocb64)) ==
		      AIO_NOTCANCELED)
#endif
#else
		while((ret = aio_cancel(0, &ce->myaiocb)) == AIO_NOTCANCELED)
 ...
 ************************************************************************/
int async_read_no_copy(gc, fd, ubuffer, offset, size, stride, max,
                       depth) struct cache* gc;
long long fd;
char** ubuffer;
off64_t offset;
long long size;
...
long long depth;
{
	...
	if((ce = (struct cache_ent*)incache(gc, fd, offset, size))) {
...
#ifdef _LARGEFILE64_SOURCE
#ifdef __LP64__
			printf("aio_return error4: fd %d offset %lld buffer "
			       "%lx size %d Opcode %d\n",
			       ce->myaiocb.aio_fildes, ce->myaiocb.aio_offset,
			       (long)(ce->myaiocb.aio_buf),
			       ce->myaiocb.aio_nbytes,
...
#endif
#else
			printf("aio_return error4: fd %d offset %d buffer %lx "
			       "size %d Opcode %d\n",
			       ce->myaiocb.aio_fildes, ce->myaiocb.aio_offset,
			       (long)(ce->myaiocb.aio_buf),
			       ce->myaiocb.aio_nbytes,
		...
		del_read++;
		first_ce =
		    alloc_cache(gc, fd, offset, size,
		                (long long)LIO_READ); /* allocate buffer */
	/*printf("allocated buffer/read %x offset
...
#ifdef _LARGEFILE64_SOURCE
#ifdef __LP64__
			printf("aio_return error5: fd %d offset %lld buffer "
			       "%lx size %d Opcode %d\n",
			       first_ce->myaiocb.aio_fildes,
			       first_ce->myaiocb.aio_offset,
			       (long)(first_ce->myaiocb.aio_buf),
...
#endif
#else
			printf("aio_return error5: fd %d offset %ld buffer %lx "
			       "size %d Opcode %d\n",
			       first_ce->myaiocb.aio_fildes,
			       first_ce->myaiocb.aio_offset,
			       (long)(first_ce->myaiocb.aio_buf),
 ...
 * reuse.
 ************************************************************************/
void async_release(gc) struct cache* gc;
{
	takeoffinuse(gc);
}
 ...
 ************************************************************************/
void putoninuse(gc, entry) struct cache* gc;
 ...
 ************************************************************************/
void takeoffinuse(gc) struct cache* gc;
{
	struct cache_ent* ce;
	if(gc->inuse_head == 0)
 ...
 *
 *************************************************************************/
size_t async_write(gc, fd, buffer, size, offset, depth) struct cache* gc;
long long fd, size;
char* buffer;
off64_t offset;
long long depth;
{
	struct cache_ent* ce;
	size_t ret;
	ce = allocate_write_buffer(gc, fd, offset, size, (long long)LIO_WRITE,
	                           depth, 0LL, (char*)0, (char*)0);
...
again:
	...
	if(ret == -1) {
		...
		if(errno == 0) {
			...
			/*
			printf("aio_write_no_copy: fd %d buffer %x offset %lld
			size %d\n",
			        ce->myaiocb64.aio_fildes,
...
struct cache_ent* allocate_write_buffer(gc, fd, offset, size, op, w_depth,
                                        direct, buffer, free_addr) struct cache
    * gc;
long long fd, size, op;
off64_t offset;
long long w_depth;
...
void async_put_on_write_queue(gc, ce) struct cache* gc;
struct cache_ent* ce;
{
	ce->forward = 0;
	ce->back = gc->w_tail;
 ...
 *************************************************************************/
void async_write_finish(gc) struct cache* gc;
{
	while(gc->w_head) {
...
void async_wait_for_write(gc) struct cache* gc;
{
	struct cache_ent* ce;
	size_t ret, retval;
	if(gc->w_head == 0)
...
*/
	...
	if(ret) {
		...
		printf("fd %d offset %lld size %d\n", ce->myaiocb.aio_fildes,
		       ce->myaiocb.aio_offset, ce->myaiocb.aio_nbytes);
#else
		printf("fd %d offset %lld size %d\n", ce->myaiocb64.aio_fildes,
		       ce->myaiocb64.aio_offset, ce->myaiocb64.aio_nbytes);
#endif
#else
		printf("fd %d offset %lld size %d\n", ce->myaiocb.aio_fildes,
		       ce->myaiocb.aio_offset, ce->myaiocb.aio_nbytes);
 ...
 *
 *************************************************************************/
size_t async_write_no_copy(gc, fd, buffer, size, offset, depth,
                           free_addr) struct cache* gc;
long long fd, size;
char* buffer;
off64_t offset;
long long depth;
...
#ifdef _LARGEFILE64_SOURCE
#ifdef __LP64__
			printf("aio_write_no_copy: fd %d buffer %lx offset "
			       "%lld size %d\n",
			       ce->myaiocb.aio_fildes,
			       (long)(ce->myaiocb.aio_buf),
			       ce->myaiocb.aio_offset, ce->myaiocb.aio_nbytes);
...
#endif
#else
			printf("aio_write_no_copy: fd %d buffer %lx offset %ld "
			       "size %d\n",
			       ce->myaiocb.aio_fildes,
			       (long)(ce->myaiocb.aio_buf),
			       ce->myaiocb.aio_offset, ce->myaiocb.aio_nbytes);
...
void mbcopy(source, dest, len) char* source, *dest;
size_t len;
{
	int i;
	for(i = 0; i < len; i++)
