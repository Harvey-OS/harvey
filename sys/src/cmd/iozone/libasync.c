...
/***********************************************/
void
    async_init(gc, fd, flag) struct cache **gc;
int fd;
int flag;
{
#ifdef VXFS
...
int
    async_read(gc, fd, ubuffer, offset, size, stride, max, depth) struct cache *gc;
long long fd;
char *ubuffer;
off64_t offset;
long long size;
...
struct cache_ent *
    alloc_cache(gc, fd, offset, size, op) struct cache *gc;
...
*************************************************************************/
struct cache_ent *
    incache(gc, fd, offset, size) struct cache *gc;
long long fd, size;
off64_t offset;
{
...
void
    takeoff_cache(gc, ce) struct cache *gc;
struct cache_ent *ce;
{
	struct cache_ent *move;
	long long found;
 ...
 ************************************************************************/
int
    async_read_no_copy(gc, fd, ubuffer, offset, size, stride, max, depth) struct cache *gc;
long long fd;
char **ubuffer;
off64_t offset;
long long size;
 ...
 ************************************************************************/
void
    async_release(gc) struct cache *gc;
{
	takeoffinuse(gc);
}
 ...
 ************************************************************************/
void
    putoninuse(gc, entry) struct cache *gc;
 ...
 ************************************************************************/
void
    takeoffinuse(gc) struct cache *gc;
{
	struct cache_ent *ce;
	if(gc->inuse_head == 0)
 ...
 *************************************************************************/
size_t
    async_write(gc, fd, buffer, size, offset, depth) struct cache *gc;
long long fd, size;
char *buffer;
off64_t offset;
long long depth;
...
struct cache_ent *
    allocate_write_buffer(gc, fd, offset, size, op, w_depth, direct, buffer, free_addr) struct cache *gc;
long long fd, size, op;
off64_t offset;
long long w_depth;
long long direct;
...
void
    async_put_on_write_queue(gc, ce) struct cache *gc;
struct cache_ent *ce;
{
	ce->forward = 0;
	ce->back = gc->w_tail;
 ...
 *************************************************************************/
void
    async_write_finish(gc) struct cache *gc;
{
	while(gc->w_head) {
...
void
    async_wait_for_write(gc) struct cache *gc;
{
	struct cache_ent *ce;
	size_t ret, retval;
	if(gc->w_head == 0)
 ...
 *************************************************************************/
size_t
    async_write_no_copy(gc, fd, buffer, size, offset, depth, free_addr) struct cache *gc;
long long fd, size;
char *buffer;
off64_t offset;
long long depth;
