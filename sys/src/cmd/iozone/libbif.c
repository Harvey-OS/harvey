...
#ifdef HAVE_ANSIC_C
void do_header(int fd) /* Stick the BOF at the beginning of the file */
{
#else
do_header(fd) int fd;
{
...
{
#else
do_int(fd, val, row, column) int fd, val, row, column;
{
#endif
...
{
#else
do_float(fd, value, row, column) int fd;
double value;
int row, column;
{
#endif
	...
	junk = write(fd, &floatrec,
	             11); /* Don't write floatrec. Padding problems */
...
{
#else
do_label(fd, string, row, column) int fd;
char* string;
int row, column;
{
#endif
