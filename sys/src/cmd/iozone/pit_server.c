...
*/
static int openSckt(const char* service, const char* protocol, int desc[],
                    size_t* descSize);
...
#define USAGE                                                                  \
	{                                                                      \
		fprintf(stderr, "Usage: %s [-v] -p service \n", pgmName);      \
		exit(127);                                                     \
	} /* End USAGE macro. */
          /*
	...
	do {                                                                   \
		if((expr) == -1) {                                             \
			fprintf(stderr,                                        \
			        ...
			        pgmName, __LINE__, strerror(errno));           \
			exit(1);                                               \
	...
	*/
	if((tScktSize > 0) || (uScktSize > 0)) {
		pit(tSckt, /* pit() never returns. */
		    tScktSize, uSckt, uScktSize);
	}
	/*
	** Since pit() never returns, execution only gets here if no sockets
	*were
	** created.
	*/
	if(verbose) {
		fprintf(stderr, "%s: No sockets opened... terminating.\n",
		        pgmName);
	}
	return 0;
} /* End main() */
...
******************************************************************************/
static int
openSckt(const char* service, const char* protocol, int desc[],
         size_t* descSize)
{
	struct addrinfo* ai;
	int aiErr;
	...
	/*
	** Initialize output parameters.  When the loop completes, *descSize is
	*0.
	*/
	while(*descSize > 0) {
	...
	*/
	...
	{
		fprintf(stderr, "%s (line %d): ERROR - Unknown transport "
		                "layer protocol \"%s\".\n",
		        pgmName, __LINE__, protocol);
		return -1;
	}
	/*
	...
	*in
	** 'hints'.  Thus, the program is requesting passive address
	*information.
	** The network address is initialized to :: (all zeros) for IPv6
	*records, or
	** 0.0.0.0 for IPv4 records.
	*/
	if((aiErr = getaddrinfo(NULL, service, &hints, &aiHead)) != 0) {
	...
	** socket.
	*/
	for(ai = aiHead; (ai != NULL) && (*descSize < maxDescs);
	    ai = ai->ai_next) {
		if(verbose) {
			/*
			** Display the current address info.   Start with the
			*protocol-
			...
			*/
			fprintf(stderr,
			        ...
			        "= %d)\n"
			        "   ai_socktype  = %d (SOCK_STREAM = %d, "
			        "SOCK_DGRAM = %d)\n"
			        "   ai_protocol  = %d (IPPROTO_TCP = %d, "
			        "IPPROTO_UDP = %d)\n"
			        "   ai_addrlen   = %d (sockaddr_in = %lu, "
			        "sockaddr_in6 = %lu)\n",
			        ai->ai_flags, ai->ai_family, PF_INET, PF_INET6,
			        ai->ai_socktype, SOCK_STREAM, SOCK_DGRAM,
			        ai->ai_protocol, IPPROTO_TCP, IPPROTO_UDP,
			        ai->ai_addrlen, sizeof(struct sockaddr_in),
			        sizeof(struct sockaddr_in6));
			/*
			** Now display the protocol-specific formatted socket
			*address.  Note
			** that the program is requesting that getnameinfo(3)
			*convert the
			** host & service into numeric strings.
			*/
			getnameinfo(ai->ai_addr, ai->ai_addrlen, hostBfr,
			            sizeof(hostBfr), servBfr, sizeof(servBfr),
			...
			case PF_INET6: /* IPv6 address record. */
			{
				...
				fprintf(stderr,
				        ...
				        "                  sin6_flowinfo: %d\n"
				        "                  sin6_scope_id: %d\n",
				        p->sin6_family, AF_INET, AF_INET6,
				        hostBfr, servBfr, p->sin6_flowinfo,
				        p->sin6_scope_id);
				break;
			}        /* End CASE of IPv6. */
			default: /* Can never get here, but just for
			            completeness. */
			{
				fprintf(stderr, "%s (line %d): ERROR - Unknown "
				                "protocol family (%d).\n",
				        pgmName, __LINE__, ai->ai_family);
				freeaddrinfo(aiHead);
				return -1;
		          ...
		          */
		CHK(desc[*descSize] = socket(ai->ai_family, ai->ai_socktype,
		                             ai->ai_protocol));
		/*
		** Here is the code that prevents "IPv4 mapped addresses", as
		*discussed
		** in Section 22.1.3.1.  If an IPv6 socket was just created,
		*then set the
		...
		*/
		if(ai->ai_family == PF_INET6) {
			...
			*/
			int v6Only = 1;
			CHK(setsockopt(desc[*descSize], IPPROTO_IPV6,
			               IPV6_V6ONLY, &v6Only, sizeof(v6Only)));
#else
			/*
			** IPV6_V6ONLY is not defined, so the socket option
			*can't be set and
			** thus IPv4 mapped addresses can't be disabled.  Print
			*a warning
			** message and close the socket.  Design note: If the
			** #if...#else...#endif construct were removed, then
			*this program
			** would not compile (because IPV6_V6ONLY isn't
			*defined).  That's an
			** acceptable approach; IPv4 mapped addresses are
			*certainly disabled
			** if the program can't build!  However, since this
			*program is also
			** designed to work for IPv4 sockets as well as IPv6, I
			*decided to
			** allow the program to compile when IPV6_V6ONLY is not
			*defined, and
			** turn it into a run-time warning rather than a
			*compile-time error.
			** IPv4 mapped addresses are still disabled because
			*_all_ IPv6 traffic
			** is disabled (all IPv6 sockets are closed here), but
			*at least this
			** way the server can still service IPv4 network
			*traffic.
			*/
			fprintf(stderr, "%s (line %d): WARNING - Cannot set "
			                "IPV6_V6ONLY socket "
			                "option.  Closing IPv6 %s socket.\n",
			        pgmName, __LINE__,
			        ai->ai_protocol == IPPROTO_TCP ? "TCP" : "UDP");
			CHK(close(desc[*descSize]));
			continue; /* Go to top of FOR loop w/o updating
			             *descSize! */
#endif            /* IPV6_V6ONLY */
		} /* End IF this is an IPv6 socket. */
		  /*
		  ** Bind the socket.  Again, the info from the addrinfo structure
		  *is used.
		  */
		CHK(bind(desc[*descSize], ai->ai_addr, ai->ai_addrlen));
		/*
		** If this is a TCP socket, put the socket into passive
		*listening mode
		** (listen is only valid on connection-oriented sockets).
		*/
		if(ai->ai_socktype == SOCK_STREAM) {
			CHK(listen(desc[*descSize], MAXCONNQLEN));
		}
		/*
		** Socket set up okay.  Bump index to next descriptor array
		*element.
		*/
		*descSize += 1;
...
******************************************************************************/
static void
pit(int tSckt[], size_t tScktSize, int uSckt[], size_t uScktSize)
{
	char bfr[256];
	ssize_t count;
	struct pollfd* desc;
	...
	*/
	for(idx = 0; idx < descSize; idx++) {
		desc[idx].fd =
		    idx < tScktSize ? tSckt[idx] : uSckt[idx - tScktSize];
	...
	*/
	...
	{
		/*
		** Wait for activity on one of the sockets.  The DO..WHILE
		*construct is
		** used to restart the system call in the event the process is
		** interrupted by a signal.
		*/
		do {
			status = poll(desc, descSize,
		 ...
		 * milli
		 * second increments. By using the Win32api one can get access
		 * to the
		 * high performance measurement interfaces. With this one can
		 * get back
		...
		*/
		for(idx = 0; idx < descSize; idx++) {
			...
			{
				fprintf(stderr, "%s (line %d): ERROR - Invalid "
				                "poll event (0x%02X).\n",
				        pgmName, __LINE__, desc[idx].revents);
				exit(1);
			}
			...
			if(idx < tScktSize) {
				/*
				** TCP connection requested.  Accept it.  Notice
				*the use of
				...
				*/
				...
				CHK(newSckt =
				        accept(desc[idx].fd, sadr, &sadrLen));
				CHK(shutdown(
				    newSckt, /* Server never recv's anything. */
				    SHUT_RD));
				if(verbose) {
					/*
					** Display the socket address of the
					*remote client.  Begin with
					** the address-independent fields.
					*/
					fprintf(stderr,
					        "Sockaddr info for new TCP "
					        "client:\n"
					        "   sa_family = %d (AF_INET = "
					        "%d, AF_INET6 = %d)\n"
					        "   addr len  = %d "
					...
					*/
					getnameinfo(sadr, sadrLen, hostBfr,
					            sizeof(hostBfr), servBfr,
					            sizeof(servBfr),
					            NI_NUMERICHOST |
					                NI_NUMERICSERV);
					/*
					** Notice that we're switching on an
					*address family now, not a
					** protocol family.
					*/
					         ...
					         completeness. */
					{
						fprintf(stderr,
						        ...
						        sadr->sa_family);
						break;
					} /* End DEFAULT case (unknown address
					     family). */
				...
				/*
				** This is a UDP socket, and a datagram is
				*available.  The funny
				** thing about UDP requests is that this server
				*doesn't require any
				** client input; but it can't send the PIT
				*unless it knows a client
				** wants the data, and the only way that can
				*occur with UDP is if
				** the server receives a datagram from the
				*client.  Thus, the
				** server must receive _something_, but the
				*content of the datagram
				** is irrelevant.  Read in the datagram.  Again
				*note the use of
				...
				*/
				...
				sadr = (struct sockaddr*)&sockStor;
				CHK(count =
				        recvfrom(desc[idx].fd, bfr, sizeof(bfr),
				                 0, sadr, &sadrLen));
				/*
				...
				if(verbose) {
					...
					*/
					fprintf(
					    ...
					    "   sa_family = %d (AF_INET = %d, "
					    "AF_INET6 = %d)\n"
					    "   addr len  = %d (sockaddr_in = "
					    "%lu, "
					    "sockaddr_in6 = %lu)\n",
					    sadr->sa_family, AF_INET, AF_INET6,
					    sadrLen, sizeof(struct sockaddr_in),
					    sizeof(struct sockaddr_in6));
					/*
					** Display the address-specific
					*information.
					*/
					getnameinfo(sadr, sadrLen, hostBfr,
					            sizeof(hostBfr), servBfr,
				...
				while(wBytes > 0) {
					do {
						count = sendto(
					        ...
					        (errno == EINTR));
					CHK(count); /* Check for a bona fide
					               error. */
					wBytes -= count;
				} /* End WHILE there is data to send. */
			}         /* End ELSE a UDP datagram is available. */
			desc[idx].revents =
			    0; /* Clear the returned poll events. */
