...
main(int argc, char* argv[])
{
	...
	snprint(cmd, sizeof cmd, "scp%F%V%F%V%F%V%F%V", dflag, "-d", pflag,
	        "-p", rflag, "-r", vflag, "-v");
...
destislocal(char* cmd, int argc, char* argv[], char* dst)
{
	...
	for(i = 0; i < argc; i++) {
		...
		if(src == nil) {
			...
			snprint(buf, sizeof buf, "exec cp%F%V%F%V %s %s", rflag,
			        "-r", pflag, "-p", argv[i], dst);
...
destisremote(char* cmd, int argc, char* argv[], char* host, char* dest)
{
	...
	for(i = 0; i < argc; i++) {
		vprint("remote destination: send %s to %s:%s", argv[i], host,
		       dest);
		...
		if(src != nil) {
			...
			snprint(buf, sizeof buf,
			        "exec %s%F%V%F%V %s %s %s '%s:%s'", ssh, iflag,
			        " -i", vflag, "-v", argv[i], cmd, src, host,
			        dest);
           ...
           uint32_t mtime, uint32_t mode)
{
	...
	if(exists) {
		if(!(d->qid.type & QTDIR)) {
			scperror(0, "%s: protocol botch: directory requrest "
			            "for non-directory",
			         dir);
			return d;
		}
...
receive(char* dest)
{
	...
	for(;;) {
		...
		case 'C':
			...
			if(*p++ != ' ') {
			Badmode:
				scperror(1, "bad mode/size format: %s",
				         buf + 1);
			}
			size = strtoll(p, &p, 10);
			if(*p++ != ' ')
			...
			if(buf[0] == 'D') {
				...
				d = receivedir(name, exists, d, settimes, atime,
				               mtime, mode);
				settimes = 0;
				continue;
			}
...
senddir(char* name, int fd, Dir* dirp)
{
	...
	vprint("directory %s mode: D%.4lo %d %.1024s", name, dirp->mode & 0777,
	       0, lastelem(name));
	...
	for(d = dir; d < &dir[n]; d++) {
		...
		if(strcmp(d->name, ".") == 0 || strcmp(d->name, "..") == 0)
			continue;
		if(snprint(file, sizeof file, "%s/%s", name, d->name) >
		   sizeof file - UTFmax) {
			scperror(0, "%.20s.../%s: name too long; skipping file",
			         file, d->name);
			continue;
		}
		send(file);
