...
send_ssh_cmsg_session_key(Conn* c)
{
	...
	if(serverkeylen + 128 <= hostkeylen) {
		...
		error("server session and host keys do not differ by at least "
		      "128 bits");
...
checkkey(Conn* c)
{
	...
	case KeyWrong:
		fprint(2,
		       "server presented public key different than expected\n");
		fprint(2, "(expected key in /sys/lib/ssh/keyring).  will not "
		          "continue.\n");
	...
	case KeyWrong:
		fprint(
		    2,
		    "server %s presented public key different than expected\n",
		    c->host);
		fprint(2, "(expected key in %s).  will not continue.\n",
		       keyfile);
		fprint(2, "this could be a man-in-the-middle attack.\n");
		switch(ask(c, "eri", "replace key in keyfile (r), continue "
		                     "without replacing key (c), or exit (e) "
		                     "[e]")) {
		case 'e':
			error("bad key");
		case 'r':
	...
	case NoKeyFile:
		...
		switch(ask(c, "eac", "add key to keyfile (a), continue without "
		                     "adding key (c), or exit (e) [e]")) {
		case 'e':
			error("bad key");
		case 'a':
...
sshclienthandshake(Conn* c)
{
	...
	if(c->cipher == nil)
		error("can't agree on ciphers: remote side supports %#lux",
		      c->ciphermask);
