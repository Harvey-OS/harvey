...
unsigned char*
    argvals[ARGSIZ / 2 + 1]; /* pointers to arguments after parsing */
...
int dbgstate = 0;
char* dbgstrings[] = {"", "sendack1", "send", "rcvack", "sendack2", "done"};
...
getjobinfo(int fd)
{
	...
	/* there may be a space after the name and host.  It will be filtered
	 * out
	 * by CPYFIELD.
	 */
...
main()
{
	...
	argvals[argcnt++] = cp;
	for(bp = (unsigned char*)LP, i = 0; (*bp != '\0') && (i < ARGSIZ - 1);
	    * cp++ = *bp++, i++)
		;
	*cp++ = '\0';
	...
	default:
		...
		do {
			...
			if(*bp == '-') {
				...
				saveflg = 0;
			/* move to next white space copying text to argument
			 * buffer */
			while(*bp != ' ' && *bp != '\t' && *bp != '\n' &&
			      *bp != '\0') {
				*cp = *bp++;
