...
copyfile(int in, int out, int32_t tosend)
{
	...
	while((n = read(in, sendbuf, SBSIZE)) > 0) {
		if(debugflag)
			fprintf(stderr,
			        "lpdsend: copyfile read %d bytes from %d\n", n,
			        in);
		alarm(TIMEOUT);
		alarmstate = 1;
		...
		if(debugflag)
			fprintf(stderr,
			        "lpdsend: copyfile wrote %d bytes to %d\n", n,
			        out);
		sent += n;
...
netmkaddr(char* linear, char* defnet, char* defsrv)
{
	...
	if(cp == 0) {
		if(defnet == 0) {
			if(defsrv)
				snprintf(addr, sizeof addr, "net!%s!%s", linear,
				         defsrv);
			else
...
main(int argc, char* argv[])
{
	...
	while((c = getopt(argc, argv, "Dd:k:qs:t:H:P:")) != -1)
		...
		case 'k':
			if(statflag) {
				fprintf(stderr,
				        "cannot have both -k and -q flags\n");
				exit(1);
			}
			killflag = 1;
			killarg = optarg;
		...
		case 'q':
			if(killflag) {
				fprintf(stderr,
				        "cannot have both -q and -k flags\n");
				exit(1);
			}
			statflag = 1;
			break;
	...
	if(optind < argc) {
		...
		usgflg++;
	if(usgflg) {
		fprintf(stderr, "usage: to send a job - %s -d printer -H "
		                "hostname -P username [-s seqno] "
		                "[-t[cdfgklnoprtvz]] desthost [filename]\n",
		        argv[0]);
		fprintf(stderr,
		        "     to check status - %s -d printer -q desthost\n",
		        argv[0]);
		fprintf(stderr, "       to kill a job - %s -d printer -P "
		                "username -k jobname desthost\n",
		        argv[0]);
		exit(1);
	}
	...
	if(!statflag && !killflag) {
		if(optind < argc) {
			...
			tmpnam(tmpfilename);
			debug("using stdin\n");
			if((inputfd = open(tmpfilename, O_RDWR | O_CREAT,
			                   0600)) < 0) {
				fprintf(stderr, "open(%s) failed\n",
				        tmpfilename);
				exit(1);
			...
			if(!copyfile(0, inputfd, 0L)) {
				fprintf(
				    stderr,
				    "failed to copy file to temporary file\n");
				exit(1);
			}
			if(lseek(inputfd, 0L, 0) < 0) {
				fprintf(stderr, "failed to seek back to the "
