	...
	for(k = log2n - 1; k >= 0; k--) {
		...
		for(i = 0; i < nxtop - oddx; i += 2) {
			...
			for(j = 0; j < nytop - oddy; j += 2) {
				/*
				 * Multiply h0,hx,hy,hc by 2 (1 the last time
				 * through).
				 */
				h0 = a[s00] << shift;
				hx = a[s10] << shift;
				 ...
				 * Add 1 to round -- note that these values are
				 * always
				 * positive so we don't need to do anything
				 * special
		...
		if(oddx) {
			...
			if(oddy) {
				/*
				 * do corner element if both row and column
				 * lengths are odd
