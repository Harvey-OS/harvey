...
qtree_decode(Biobuf* infile, Pix* a, int n, int nqx, int nqy, int nbitplanes)
{
	...
	for(bit = nbitplanes - 1; bit >= 0; bit--) {
		...
		if(b == 0) {
			...
			/*
			 * do log2n expansions, reading codes from file as
			 * necessary
			 */
			nx = 1;
			ny = 1;
			...
			for(k = 1; k < log2n; k++) {
				/*
				 * this somewhat cryptic code generates the
				 * sequence
			...
			/*
			 * copy last set of 4-bit codes to bitplane bit of array
			 * a
			 */
...
static void
read_bdirect(Biobuf* infile, Pix* a, int n, int nqx, int nqy, uint8_t* scratch,
             int bit)
{
	int i;
