...
amdinv(Header* h, Angle ra, Angle dec, float mag, float col)
{
	...
	for(i = 0; i < max_iterations; i++) {
		...
		f = h->param[Pamdx1] * x1 + h->param[Pamdx2] * y1 +
		    h->param[Pamdx3] + h->param[Pamdx4] * x2 +
		    h->param[Pamdx5] * x1 * y1 + h->param[Pamdx6] * y2 +
		    h->param[Pamdx7] * (x2 + y2) + h->param[Pamdx8] * x2 * x1 +
		    h->param[Pamdx9] * x2 * y1 + h->param[Pamdx10] * x1 * y2 +
		    h->param[Pamdx11] * y3 +
		    h->param[Pamdx12] * x1 * (x2 + y2) +
		    h->param[Pamdx13] * x1 * (x2 + y2) * (x2 + y2) +
		    h->param[Pamdx14] * mag + h->param[Pamdx15] * mag * mag +
		 ...
		 */
		fx =
		    h->param[Pamdx1] + h->param[Pamdx4] * 2 * x1 +
		    h->param[Pamdx5] * y1 + h->param[Pamdx7] * 2 * x1 +
		    h->param[Pamdx8] * 3 * x2 + h->param[Pamdx9] * 2 * x1 * y1 +
		    h->param[Pamdx10] * y2 + h->param[Pamdx12] * (3 * x2 + y2) +
		    h->param[Pamdx13] * (5 * x4 + 6 * x2 * y2 + y4) +
		    h->param[Pamdx17] * mag + h->param[Pamdx18] * mag * 2 * x1 +
		    h->param[Pamdx19] * mag * (3 * x2 + y2);

		/*
		 *  Derivative of X model wrt y
		 */
		fy = h->param[Pamdx2] + h->param[Pamdx5] * x1 +
		     h->param[Pamdx6] * 2 * y1 + h->param[Pamdx7] * 2 * y1 +
		     h->param[Pamdx9] * x2 + h->param[Pamdx10] * x1 * 2 * y1 +
		 ...
		 *  Y plate model
		 */
		g = h->param[Pamdy1] * y1 + h->param[Pamdy2] * x1 +
		    h->param[Pamdy3] + h->param[Pamdy4] * y2 +
		    h->param[Pamdy5] * y1 * x1 + h->param[Pamdy6] * x2 +
		    h->param[Pamdy7] * (x2 + y2) + h->param[Pamdy8] * y3 +
		    h->param[Pamdy9] * y2 * x1 + h->param[Pamdy10] * y1 * x3 +
		    h->param[Pamdy11] * x3 +
		    h->param[Pamdy12] * y1 * (x2 + y2) +
		    h->param[Pamdy13] * y1 * (x2 + y2) * (x2 + y2) +
		    h->param[Pamdy14] * mag + h->param[Pamdy15] * mag * mag +
		 ...
		 */
		gx = h->param[Pamdy2] + h->param[Pamdy5] * y1 +
		     h->param[Pamdy6] * 2 * x1 + h->param[Pamdy7] * 2 * x1 +
		     h->param[Pamdy9] * y2 + h->param[Pamdy10] * y1 * 2 * x1 +
		 ...
		 */
		gy =
		    h->param[Pamdy1] + h->param[Pamdy4] * 2 * y1 +
		    h->param[Pamdy5] * x1 + h->param[Pamdy7] * 2 * y1 +
		    h->param[Pamdy8] * 3 * y2 + h->param[Pamdy9] * 2 * y1 * x1 +
		    h->param[Pamdy10] * x2 + h->param[Pamdy12] * 3 * y2 +
		    h->param[Pamdy13] * (5 * y4 + 6 * x2 * y2 + x4) +
		    h->param[Pamdy17] * mag + h->param[Pamdy18] * mag * 2 * y1 +
...
ppoinv(Header* h, Angle ra, Angle dec)
{
	 ...
	 */
	h->x = h->param[Pppo1] * h->xi + h->param[Pppo2] * h->eta +
	       h->param[Pppo3];
	h->y = h->param[Pppo4] * h->xi + h->param[Pppo5] * h->eta +
	       h->param[Pppo6];
	/*
...
traneqstd(Header* h, Angle object_ra, Angle object_dec)
{
	 ...
	 *  Find divisor
	 */
	div = (sin(object_dec) * sin(h->param[Ppltdec]) +
	 ...
	 */
	h->xi = cos(object_dec) * sin(object_ra - h->param[Ppltra]) *
