...
rowinit(Row* row, Rectangle r)
{
	...
	t = &row->tag;
	textinit(t, fileaddtext(nil, t), r1, rfget(FALSE, FALSE, FALSE, nil),
	         tagcols);
	t->what = Rowtag;
	t->row = row;
	t->w = nil;
...
rowadd(Row* row, Column* c, int x)
{
	...
	if(x < r.min.x &&
	   row->ncol > 0) { /*steal 40% of last column by default */
	...
	memmove(row->col + i + 1, row->col + i,
	        (row->ncol - i) * sizeof(Column*));
	row->col[i] = c;
	row->ncol++;
	clearmouse();
	return c;
...
rowdump(Row* row, char* file)
{
	...
	for(i = 0; i < row->ncol; i++) {
		c = row->col[i];
		Bprint(b, "%11d",
		       100 * (c->r.min.x - row->r.min.x) / Dx(row->r));
		if(i == row->ncol - 1)
			Bputc(b, '\n');
		else
			Bputc(b, ' ');
		...
		for(j = 0; j < c->nw; j++) {
			...
			if(t->file->dumpid) {
				...
				Bprint(b, "x%11d %11d %11d %11d %11d %s\n", i,
				       t->file->dumpid, w->body.q0, w->body.q1,
				       100 * (w->r.min.y - c->r.min.y) /
				           Dy(c->r),
				       fontname);
			} else if(w->dumpstr) {
				dumped = FALSE;
				Bprint(b, "e%11d %11d %11d %11d %11d %s\n", i,
				       ...
				       100 * (w->r.min.y - c->r.min.y) /
				           Dy(c->r),
				       fontname);
			} else if((w->dirty == FALSE && access(a, 0) == 0) ||
			          w->isdir) {
				dumped = FALSE;
				t->file->dumpid = w->id;
				Bprint(b, "f%11d %11d %11d %11d %11d %s\n", i,
				       w->id, w->body.q0, w->body.q1,
				       100 * (w->r.min.y - c->r.min.y) /
				           Dy(c->r),
				       fontname);
			} else {
...
rowload(Row* row, char* file, int initing)
{
	...
	if(file == nil) {
		if(home == nil) {
			warning(
			    nil,
			    "can't find file for load: $home not defined\n");
			goto Rescue1;
		}
		sprint(buf, "%s/acme.dump", home);
		file = buf;
	...
	for(i = 0; i < j; i++) {
		...
		if(i < row->ncol) {
			...
			if(Dx(r1) < 50 || Dx(r2) < 50)
				continue;
			draw(screen, Rpt(r1.min, r2.max), display->white, nil,
			     ZP);
			colresize(c1, r1);
			colresize(c2, r2);
			r2.min.x = x;
	...
	for(;;) {
		...
		for(; n < nr; n++)
			if(r[n] == '|')
				break;
		wincleartag(w);
		textinsert(&w->tag, w->tag.file->nc, r + n + 1, nr - (n + 1),
		           TRUE);
		if(ndumped >= 0) {
		...
		w->maxlines =
		    min(w->body.nlines, max(w->maxlines, w->body.maxlines));
	}
	Bterm(b);
	fbuffree(buf);
	return TRUE;
