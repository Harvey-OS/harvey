...
enum { TABDIR = 3 /* width of tabs in directory windows */
};
...
textload(Text* t, uint q0, char* file, int setqid)
{
	...
	nulls = FALSE;
	if(d->qid.type & QTDIR) {
		/* this is checked in get() but it's possible the file changed
		 * underfoot */
		if(t->file->ntext > 1) {
			warning(nil, "%s is a directory; can't read with "
	...
	for(i = 0; i < t->file->ntext; i++) {
		u = t->file->text[i];
		if(u != t) {
			if(u->org > u->file->nc) /* will be 0 because of
			                            reset(), but safety first */
				u->org = 0;
			textresize(u, u->all);
...
textbsinsert(Text* t, uint q0, Rune* r, uint n, int tofile, int* nrp)
{
	...
	if(t
	       ->what == Tag) { /* can't happen but safety first: mustn't
	                           backspace over file name */
	Err:
...
textcomplete(Text* t)
{
	...
	if(t->q0 < t->file->nc &&
	   textreadc(t, t->q0) > ' ') /* must be at end of word */
		return nil;
	...
	if(!c->advance) {
		warning(nil, "%.*S%s%.*S*%s\n", dir.nr, dir.r,
		        dir.nr > 0 && dir.r[dir.nr - 1] != '/' ? "/" : "", nstr,
		        str, c->nmatch ? "" : ": no matches in:");
		for(i = 0; i < c->nfile; i++)
			warning(nil, " %s\n", c->filename[i]);
	}
...
texttype(Text* t, Rune r)
{
	...
	case_Down:
		q0 = t->org +
		     frcharofpt(
		         t, Pt(t->r.min.x, t->r.min.y + n * t->font->height));
	...
	case 0x17:             /* ^W: erase word */
		...
		q1 = t->q0;
		q0 = q1 - nnb;
		/* if selection is at beginning of window, avoid deleting
		 * invisible text */
		if(q0 < t->org) {
			q0 = t->org;
			nnb = q1 - q0;
	...
	/* otherwise ordinary character; just insert, typically in caches of all
	 * texts */
	for(i = 0; i < t->file->ntext; i++) {
		u = t->file->text[i];
		if(u->eq0 == ~0)
...
textframescroll(Text* t, int dl)
{
	...
	if(dl < 0) {
		...
		q0 = t->org +
		     frcharofpt(
		         t, Pt(t->r.min.x, t->r.min.y + dl * t->font->height));
...
textselect(Text* t)
{
	...
	if(q0 == q1) {
		if(q0 == t->q0 && clicktext == t &&
		   mouse->msec - clickmsec < 500) {
			textdoubleclick(t, &q0, &q1);
			clicktext = nil;
		} else {
			clicktext = t;
...
textshow(Text* t, uint q0, uint q1, int doselect)
{
	...
	qe = t->org + t->nchars;
	if(t->org <= q0 &&
	   (q0 < qe || (q0 == qe && qe == t->file->nc + t->ncache)))
		textscrdraw(t);
	else {
...
selrestore(Frame* f, Point pt0, uint p0, uint p1)
{
	...
	/* after selection */
	if(p1 > f->p1) {
		frdrawsel0(f, frptofchar(f, f->p1), f->p1, p1, f->cols[BACK],
		           f->cols[TEXT]);
		p1 = f->p1;
	}
	/* inside selection */
...
uint xselect(Frame* f, Mousectl* mc, Image* col,
             uint* p1p) /* when called, button is down */
{
		...
		if(p1 != q) {
			...
			if(reg !=
			   region(q, p0)) { /* crossed starting point; reset */
				...
				reg = region(q, p0);
				if(reg == 0)
					frdrawsel0(f, pt0, p0, p1, col,
					           display->white);
			}
			qt = frptofchar(f, q);
			if(reg > 0) {
				if(q > p1)
					frdrawsel0(f, pt1, p1, q, col,
					           display->white);
	...
	if(mc->msec - msec < DELAY && p0 != p1 &&
	   abs(mp.x - mc->xy.x) < MINMOVE && abs(mp.y - mc->xy.y) < MINMOVE) {
		if(reg > 0)
			selrestore(f, pt0, p0, p1);
		else if(reg < 0)
			selrestore(f, pt1, p1, p0);
