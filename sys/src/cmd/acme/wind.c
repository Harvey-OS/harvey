...
movetodel(Window* w)
{
	...
	if(n < 0)
		return;
	moveto(mousectl,
	       addpt(frptofchar(&w->tag, n), Pt(4, w->tag.font->height - 4)));
}
...
int winclean(Window* w,
             int conservative) /* as it stands, conservative is always TRUE */
{
	if(w->isscratch ||
	   w->isdir) /* don't whine if it's a guide file, error window, etc. */
		return TRUE;
	...
	if(w->dirty) {
		if(w->body.file->nname)
			...
			if(w->body.file->nc <
			   100) /* don't whine if it's too small */
				return TRUE;
