...
coladd(Column* c, Window* w, Window* clone, int y)
{
	...
	if(y < r.min.y &&
	   c->nw > 0) { /* steal half of last window by default */
		v = c->w[c->nw - 1];
	...
	if(c->nw > 0) {
		...
		r1 = r;
		y = min(y, t - (v->tag.font->height + v->body.font->height +
		                Border + 1));
		r1.max.y = min(y, v->body.r.min.y +
		                      v->body.nlines * v->body.font->height);
...
{
	moveto(mousectl,
	       divpt(addpt(c->tag.scrollr.min, c->tag.scrollr.max), 2));
}
...
colresize(Column* c, Rectangle r)
{
	...
	for(i = 0; i < c->nw; i++) {
		...
		if(i == c->nw - 1)
			...
			r1.max.y =
			    r1.min.y + (Dy(w->r) + Border) * Dy(r) / Dy(c->r);
		r2 = r1;
...
Found:
	...
	if(i == 0 && c->nw == 1)
		return; /* can't do it */
	if((i > 0 && p.y < c->w[i - 1]->r.min.y) ||
	   (i < c->nw - 1 && p.y > w->r.max.y) ||
	   (i == 0 && p.y > w->r.max.y)) {
		/* shuffle */
		colclose(c, w, FALSE);
		coladd(c, w, nil, p.y);
