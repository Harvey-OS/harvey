...
Range
address(Mntdir* md, Text* t, Range lim, Range ar, void* a, uint q0, uint q1,
	...
	while(q < q1) {
		...
		case ',':
			if(prevc == 0) /* lhs defaults to 0 */
				r.q0 = 0;
			if(q >= q1 && t != nil &&
			   t->file != nil) /* rhs defaults to $ */
				r.q1 = t->file->nc;
			else {
				nr = address(md, t, lim, ar, a, q, q1, getc,
				             evalp, &q);
				r.q1 = nr.q1;
			}
			*qp = q;
		...
		case '-':
			if(*evalp && (prevc == '+' || prevc == '-'))
				if((nc = (*getc)(a, q)) != '#' && nc != '/' &&
				   nc != '?')
					r = number(md, t, r, 1, prevc, Line,
					           evalp); /* do previous one */
			dir = c;
			break;
		case '.':
		case '$':
