...
execute(Text* t, uint aq0, uint aq1, int external, Text* argt)
{
	...
	if(q1 == q0) { /* expand to find word (actually file name) */
		...
		if(t->q1 > t->q0 && t->q0 <= q0 && q0 <= t->q1) {
			...
			while(q1 < t->file->nc &&
			      isexecc(c = textreadc(t, q1)) && c != ':')
				q1++;
			while(q0 > 0 && isexecc(c = textreadc(t, q0 - 1)) &&
			      c != ':')
				q0--;
			if(q1 == q0)
				return;
...
printarg(Text* argt, uint q0, uint q1)
{
	...
	if(q0 == q1)
		sprint(buf, "%.*S:#%d", argt->file->nname, argt->file->name,
		       q0);
	else
		sprint(buf, "%.*S:#%d,#%d", argt->file->nname, argt->file->name,
		       q0, q1);
	return buf;
}
...
delcol(Text* et, Text*, Text*, int, int, Rune*, int)
{
	...
	for(i = 0; i < c->nw; i++) {
		w = c->w[i];
		if(w->nopen[QWevent] + w->nopen[QWaddr] + w->nopen[QWdata] +
		       w->nopen[QWxdata] >
		   0) {
			warning(nil, "can't delete column; %.*S is running an "
			             "external command\n",
...
undo(Text* et, Text*, Text*, int flag1, int, Rune*, int)
{
	...
	/*
	 * Undo the executing window first. Its display will update. other
	 * windows
	 * in the same file will not call show() and jump to a different
...
getname(Text* t, Text* argt, Rune* arg, int narg, int isput)
{
	...
	if(r == nil)
		...
		/* if are doing a Put, want to synthesize name even for
		 * non-existent file */
...
get(Text* et, Text* t, Text* argt, int flag1, int, Rune* arg, int narg)
{
	...
	if(flag1)
		if(et == nil || et->w == nil)
			return;
	if(!et->w->isdir &&
	   (et->w->body.file->nc > 0 && !winclean(et->w, TRUE)))
		return;
	w = et->w;
	t = &w->body;
	...
	for(i = 0; i < t->file->ntext; i++) {
		u = t->file->text[i];
		/* second and subsequent calls with zero an already empty
		 * buffer, but OK */
		textreset(u);
		windirfree(u->w);
	}
	...
	for(i = 0; i < t->file->ntext; i++) {
		u = t->file->text[i];
		textsetselect(&u->w->tag, u->w->tag.file->nc,
		              u->w->tag.file->nc);
		textscrdraw(u);
	}
}
	...
	if(d != nil && runeeq(namer, nname, f->name, f->nname)) {
		...
		if(f->dev != d->dev || f->qidpath != d->qid.path ||
		   f->mtime + 1 < d->mtime) {
			...
			if(f->unread)
				warning(nil,
				        "%s not written; file already exists\n",
				        name);
			else
				warning(
				    nil, "%s modified%s%s since last read\n",
...
fontx(Text* et, Text* t, Text* argt, int, int, Rune* arg, int narg)
{
	...
	for(;;) {
		...
		if(runeeq(r, narg - na, L"fix", 3) ||
		   runeeq(r, narg - na, L"var", 3)) {
			free(flag);
			flag = r;
		} else {
			free(file);
...
runproc(void* argvp)
{
	...
	if(e)
		memmove(name, e + 1,
		        strlen(e + 1) + 1); /* strcpy but overlaps */
	...
	if(newns) {
		...
		incl = nil;
		if(win) {
			filename = smprint("%.*S", win->body.file->nname,
			                   win->body.file->name);
			nincl = win->nincl;
			if(nincl > 0) {
		...
		if(winid > 0 && (pipechar == '|' || pipechar == '>')) {
			...
			open("/dev/null", OREAD);
		close(1);
		if((winid > 0 || iseditcmd) &&
		   (pipechar == '|' || pipechar == '<')) {
			if(iseditcmd) {
				if(winid > 0)
					sprint(buf, "/mnt/acme/%d/editout",
					       winid);
				else
...
void
run(Window* win, char* s, Rune* rdir, int ndir, int newns, char* argaddr,
    char* xarg, int iseditcmd)
{
	...
	/* mustn't block here because must be ready to answer mount() call in
	 * run() */
