 ...
 */
enum { Minstring = 16, /* distance beneath which we merge changes */
       Maxstring =
	   RBUFSIZE, /* maximum length of change we will merge into one */
};
...
elogreplace(File* f, int q0, int q1, Rune* r, int nr)
{
	...
	gap =
	    q0 - (f->elog.q0 + f->elog.nd); /* gap between previous and this */
	if(f->elog.type == Replace && f->elog.nr + gap + nr < Maxstring) {
		if(gap < Minstring) {
			if(gap > 0) {
				bufread(f, f->elog.q0 + f->elog.nd,
				        f->elog.r + f->elog.nr, gap);
				f->elog.nr += gap;
			}
			f->elog.nd += gap + q1 - q0;
	...
	f->elog.nr = nr;
	if(nr > RBUFSIZE)
		editerror("internal error: replacement string too large(%d)",
		          nr);
...
eloginsert(File* f, int q0, Rune* r, int nr)
{
	...
	/* try to merge with previous */
	if(f->elog.type == Insert && q0 == f->elog.q0 &&
	   f->elog.nr + nr < Maxstring) {
		runemove(f->elog.r + f->elog.nr, r, nr);
		f->elog.nr += nr;
		return;
	}
...
elogapply(File* f)
{
	 ...
	 * but using coordinates relative to the unmodified buffer.  As we apply
	 * the log,
	 * we have to update the coordinates to be relative to the modified
	 * buffer.
	 * Textinsert and textdelete will do this for us; our only work is to
	 * apply the
	 * convention that an insertion at t->q0==t->q1 is intended to select
	 * the
	 * inserted text.
	 */
	 ...
	 * If changes were out of order, we expect problems so don't complain
	 * further.
	 */
