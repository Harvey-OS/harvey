...
cmdexec(Text* t, Cmd* cp)
{
	...
	if(w == nil && (cp->addr == 0 || cp->addr->type != '"') &&
	   !utfrune("bBnqUXY!", cp->cmdc) && !(cp->cmdc == 'D' && cp->text))
	...
	case '{':
		...
		for(cp = cp->cmd; cp; cp = cp->next) {
			if(dot.r.q1 > t->file->nc)
				editerror("dot extends past end of buffer "
				          "during { command");
			t->q0 = dot.r.q0;
			t->q1 = dot.r.q1;
			cmdexec(t, cp);
...
D_cmd(Text* t, Cmd* cp)
{
	...
	do {
		s = findbl(r, nr, &nr);
		*s = '\0';
		/* first time through, could be empty string, meaning delete
		 * file empty name */
		nn = runestrlen(r);
		...
		if(w == nil) {
			snprint(buf, sizeof buf, "no such file %.*S", rs.nr,
			        rs.r);
			free(rs.r);
			editerror(buf);
		}
		...
		for(i = 0; i < cp->text->n; i++)
			if((c = cp->text->r[i]) == '\\' &&
			   i < cp->text->n - 1) {
				...
				if('1' <= c && c <= '9') {
					j = c - '0';
					if(sel.r[j].q1 - sel.r[j].q0 >
					   RBUFSIZE) {
						err = "replacement string too "
						      "long";
						goto Err;
					}
					bufread(t->file, sel.r[j].q0, rbuf,
					        sel.r[j].q1 - sel.r[j].q0);
					for(k = 0;
					    k < sel.r[j].q1 - sel.r[j].q0; k++)
						Straddc(buf, rbuf[k]);
				} else
					Straddc(buf, c);
			} else if(c != '&')
				...
				if(sel.r[0].q1 - sel.r[0].q0 > RBUFSIZE) {
					err = "right hand side too long in "
					      "substitution";
					goto Err;
				}
				bufread(t->file, sel.r[0].q0, rbuf,
				        sel.r[0].q1 - sel.r[0].q0);
...
printposn(Text* t, int charsonly)
{
	...
	if(!charsonly) {
		...
		if(addr.r.q1 > 0 && addr.r.q1 > addr.r.q0 &&
...
looper(File* f, Cmd* cp, int xy)
{
	...
	rp = nil;
	for(p = r.q0; p <= r.q1;) {
		if(!rxexecute(f->curtext, nil, p, r.q1,
		              &sel)) { /* no match, but y should still run */
			if(xy || op > r.q1)
				break;
			tr.q0 = op, tr.q1 = r.q1;
...
filematch(File* f, String* r)
{
	...
	/* compile expr first so if we get an error, we haven't allocated
	 * anything */
	if(rxcompile(r->r) == FALSE)
