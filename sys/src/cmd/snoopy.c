 ...
 *  Bootp packet
 */
#include "/sys/src/cmd/ip/bootp.h"
...
sprintpup(void *a, char *buf, int len)
{
	...
	if(t==0x0806 && aflag){
		...
		n += sprintarp(p, buf+n);
	} else if((t==0x0800||(t&0xFF00)==0x1000) && (iflag||uflag||tflag||bflag||lflag)){
		if(eflag)
			n += sprint(buf+n, "%.4ux(%.4d %E > %E)", t, len, p->s, p->d);
...
sprintil(void *a, char *buf, int len)
{
	...
	if(flag9 && convM2S(il->data, &f, len)){
		n += sprint(buf+n, "%F", &f);
	} else
...
sprinttcp(void *a, char *buf, int len)
{
	...
	len -= TCP_HDRSIZE;
	n += sprint(buf+n, "TCP(%d > %d s %l8.8ux a %8.8lux f %.4ux w %.4ux l %d) ",
		NetS(tcp->sport), NetS(tcp->dport),
		NetL(tcp->seq), NetL(tcp->ack), NetS(tcp->flag),
		NetS(tcp->win), len);
...
sprintip(void *a, char *buf, int len)
{
	...
	len -= IP_HDRSIZE;
	if(ip->proto==IP_UDPPROTO && (iflag||uflag||bflag)){
