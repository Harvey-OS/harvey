...
enum {
	...
	NilBlock	= (~0UL),	// Blknum change here
	MaxBlock	= (1UL<<31),	// Blknum change here
};
...
struct Super {			/* believed to be on-disk format */
	u16int version;
	u32int epochLow;
	u32int epochHigh;
	u64int qid;			/* next qid */
	/* Blknums here? on disk? */
...
struct Entry {
	...
	u32int	snap;	/* non-zero -> entering snapshot of given epoch Blknum? on disk? */
...
struct Source {
	...
	Blknum	offset; 	/* immutable: entry offset in parent */
};


struct Header {		// Blknums here? /* believed to be on-disk format */
	ushort version;
	ushort blockSize;
...
struct Fsck
{
	...
	int	bsize;
	int	walkdepth;
	Blknum	hint;		/* where the next root probably is */
	Blknum	nseen;
	int	quantum;
	Blknum	nclre;
	Blknum	nclrp;
