...
fileSetDir(File *f, DirEntry *dir, char *uid)
{
	File *ff;
	char *oelem;
	int changed;		/* flag: content (length) changed? */
	u32int mask;
	u64int size;
...
//fprint(2, "->%x\n", f->dir.mode);
	...
	if (changed)
		fileWAccess(f, uid);	/* ensure change is a modification */
...
getEntry(Source *r, Entry *e, int checkepoch)
{
	...
	if(checkepoch){
		...
		if(b){
			if(b->l.epoch >= epoch)
				fprint(2, "warning: entry %p epoch not older %#llux/%d %V/%d in getEntry\n",
...
setEntry(Source *r, Entry *e)
{
	...
	if(0) fprint(2, "setEntry: b %#llux %lld score=%V\n", b->addr, r->offset % r->epb, e->score);
	if(b == nil)
		return 0;
