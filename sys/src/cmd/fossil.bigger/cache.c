...
struct Cache
{
	...
	Blknum	nblocks;		/* number of blocks allocated */
	...
	Blknum 	ndirty;			/* number of dirty blocks in the cache */
	Blknum 	maxdirty;		/* max number of dirty blocks */
	u32int	vers;
...
struct FreeList {
	VtLock *lk;
	Blknum last;		/* last block allocated */
	Blknum end;		/* end of data partition */
	Blknum nused;		/* number of used blocks */
 ...
 */
Cache *
cacheAlloc(Disk *disk, VtSession *z, Blknum nblocks, int mode)
{
	Blknum i;
	Cache *c;
	Block *b;
...
cacheDump(Cache *c)
{
	...
	for(i = 0; i < c->nblocks; i++){
		b = &c->blocks[i];
		fprint(2, "%d. p=%d a=%llud %V t=%d ref=%d state=%s io=%s pc=%#p\n",
...
fprint(2, "%s: cacheCheck: nheap %d refed %lld nblocks %lld\n", argv0, c->nheap, refed, c->nblocks);
cacheDump(c);
}
	...
	for(i = 0; i < c->nblocks; i++){
		b = &c->blocks[i];
		if(b->ref){
if(1)fprint(2, "%s: p=%d a=%llud %V ref=%d %L\n", argv0, b->part, b->addr, b->score, b->ref, &b->l);
			refed++;
		}
	}
if(refed > 0)fprint(2, "%s: cacheCheck: in used %lld\n", argv0, refed);
}
...
cacheBumpBlock(Cache *c)
{
	...
	if(c->nheap == 0){
		while(c->nheap == 0){
			...
			if(c->nheap == 0){
				printed = 1;
				fprint(2, "%s: entire cache is busy, %lld dirty "
					"-- waking flush thread\n",
					argv0, c->ndirty);
			}
		}
		if(printed)
			fprint(2, "%s: cache is okay again, %lld dirty\n",
				argv0, c->ndirty);
	}
...
if(0)fprint(2, "%s: dropping %d:%llx:%V\n", argv0, b->part, b->addr, b->score);
...
if(0)fprint(2, "%s: cacheLocal: %d: %d %llx\n", argv0, getpid(), b->part, b->addr);
	bwatchLock(b);
	vtLock(b->lk);
	b->nlock = 1;
...
void
cacheCountUsed(Cache *c, u32int epochLow, Blknum *used, Blknum *total, u32int *bsize)
{
	int n;
	Blknum addr, nused;
	Block *b;
	...
	*bsize = c->size;
	...
	for(addr=0; addr<fl->end; addr++){
		if(addr%n == 0){
			...
			b = cacheLocal(c, PartLabel, addr/n, OReadOnly);
			if(b == nil){
				fprint(2, "%s: flCountUsed: loading %llux: %R\n",
					argv0, addr/n);
				break;
			}
		}
...
blockDependency(Block *b, Block *bb, int index, uchar *score, Entry *e)
{
	...
	if(bb->iostate != BioDirty){
		fprint(2, "%s: %d:%llx:%d iostate is %d in blockDependency\n",
...
blockWrite(Block *b, int waitlock)
{
	...
	for(p=*pp; p; p=*pp){
		 ...
		 */
		if(bb->iostate != BioDirty){
			fprint(2, "%s: %d:%llx:%d iostate is %d in blockWrite\n",
				argv0, bb->part, bb->addr, bb->l.type, bb->iostate);
		...
		if(p->index < 0){
			 ...
			 */
			if(0) fprint(2, "%s: blockWrite skipping %d %llx %d %d; need to write %d %llx %d\n",
...
blockCopy(Block *b, u32int tag, u32int ehi, u32int elo)
{
	...
	if((b->l.state&BsClosed) || b->l.epoch >= ehi)
		fprint(2, "%s: blockCopy %#llux %L but fs is [%ud,%ud]\n",
...
flushFill(Cache *c)
{
	...
	if(ndirty != c->ndirty){
		fprint(2, "%s: ndirty mismatch expected %lld found %lld\n",
