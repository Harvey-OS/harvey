...
fsysBlock(Fsys* fsys, int argc, char* argv[])
{
	...
	if(b == nil){
		vtSetError("cacheLocal %#llux: %R", addr);
		vtRUnlock(fs->elk);
		return 0;
	}

	consPrint("\t%sblock %#llux %llud %llud %.*H\n",
		argc==4 ? "old: " : "", addr, offset, count, (int)count,
		b->data+offset);
	...
	if(argc == 4){
		...
		consPrint("\tnew: block %#llux %llud %llud %.*H\n",
			addr, offset, count, (int)count, b->data+offset);
		blockDirty(b);
	}
...
fsysBfree(Fsys* fsys, int argc, char* argv[])
{
	...
	vtRLock(fs->elk);
	while(argc > 0){
		addr = strtoull(argv[0], &p, 0);
		if(*p != '\0'){
			consPrint("bad address - '%llud'\n", addr);
		...
		b = cacheLocal(fs->cache, PartData, addr, OReadOnly);
		if(b == nil){
			consPrint("loading %#llux: %R\n", addr);
			continue;
		}
		l = b->l;
		if(l.state == BsFree)
			consPrint("%#llux is already free\n", addr);
		else{
			consPrint("label %#llux %ud %ud %ud %ud %#x\n",
				addr, l.type, l.state, l.epoch, l.epochClose, l.tag);
			l.state = BsFree;
			l.type = BtMax;
			l.tag = 0;
...
fsckClose(Fsck *fsck, Block *b, u32int epoch)
{
	...
	if(l.state == BsFree || (l.state&BsClosed)){
		consPrint("%#llux is already closed\n", b->addr);
		return;
	}
	if(epoch){	
		l.state |= BsClosed;
...
fsysCheck(Fsys *fsys, int argc, char *argv[])
{
	...
	fsCheck(&fsck);
	consPrint("fsck: %lld clri, %lld clre, %lld clrp, %lld bclose\n",
