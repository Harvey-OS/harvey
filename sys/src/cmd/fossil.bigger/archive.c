...
ventiSend(Arch *a, Block *b, uchar *data)
{
	...
	if(DEBUG > 1)
		fprint(2, "ventiSend: sending %#llux %L to venti\n", b->addr, &b->l);
	...
	if(!vtWrite(a->z, score, vtType[b->l.type], data, n)){
		fprint(2, "ventiSend: vtWrite block %#llux failed: %R\n", b->addr);
		return 0;
	}
	if(!vtSha1Check(score, data, n)){
		uchar score2[VtScoreSize];
		vtSha1(score2, data, n);
		fprint(2, "ventiSend: vtWrite block %#llux failed vtSha1Check %V %V\n",
			b->addr, score, score2);
		return 0;
	}
	if(!vtSync(a->z))
...
{
	int ret, i, x, psize, dsize;		// BLKnum i?
	...
	if(DEBUG) fprint(2, "%*sarchive(%ud, %#llux): block label %L\n",
		...
		for(i=0; nextWalk(&w, score, &type, &tag, &e); i++){
			if(e){
				...
				|| (e->flags&VtEntryNoArchive)){
					if(0) fprint(2, "snap; faking %#llux\n", b->addr);
					if(data == b->data){
			...
			switch(x){
			case ArchFailure:
				fprint(2, "archWalk %#ux failed; ptr is in %#llux offset %d\n",
					addr, b->addr, i);
				ret = ArchFailure;
				goto Out;
			case ArchFaked:
				...
				if(e==nil || !e->archive)
				if(data == b->data){
if(0) fprint(2, "faked %#ux, faking %#llux (%V)\n", addr, b->addr, p->score);
					data = copyBlock(b, p->blockSize);
					if(data == nil){
						ret = ArchFailure;
						goto Out;
