	...
	u32int epoch)
{
	...
	if(b){
		...
		if(b->l.epoch < bb->l.epoch || bb->l.epochClose <= b->l.epoch){
			error(chk, "walk: block %#llux [%ud, %ud) points at %#llux [%ud, %ud)",
		...
		if(b->l.epoch == epoch && bb->l.epoch == epoch){
			if(getBit(chk->emap, addr)){
				error(chk, "walk: epoch join detected: addr %#llux %L",
					bb->addr, &bb->l);
				goto Exit;
			}
		...
		if(!(b->l.state&BsCopied) && b->l.epoch == bb->l.epoch){
			if(getBit(chk->xmap, addr)){
				error(chk, "walk: copy join detected; addr %#llux %L",
					bb->addr, &bb->l);
				goto Exit;
			}
	...
	if(epoch == chk->fs->ehi){
		...
		if(getBit(chk->amap, addr)){
			error(chk, "walk: active join detected: addr %#llux %L",
				bb->addr, &bb->l);
			goto Exit;
		}
...
checkLeak(Fsck *chk)
{
	u32int a, nfree, nlost;		/* good to 32TB only */
	Block *b;
	Label l;
...
scanSource(Fsck *chk, char *name, Source *r)
{
	...
	for(o = 0; o < nb; o++){
		...
		if(b->addr != NilBlock && getBit(chk->errmap, b->addr)){
			warn(chk, "previously reported error in block %llux is in file %s",
				b->addr, name);
		}
		blockPut(b);
	}
...
chkDir(Fsck *chk, char *name, Source *source, Source *meta)
{
	...
	for(o = 0; o < nb; o++){
		...
		if(b == nil){
			error(chk, "could not read block in meta file: %s[%llud]: %R",
				name, o);
			continue;
		}
if(0)		fprint(2, "source %V:%lld block %lld addr %lld\n", source->score,
			source->offset, o, b->addr);
		if(b->addr != NilBlock && getBit(chk->errmap, b->addr))
			warn(chk, "previously reported error in block %llux is in %s",
				b->addr, name);

		if(!mbUnpack(&mb, b->data, meta->dsize)){
			error(chk, "could not unpack meta block: %s[%llud]: %R",
				name, o);
			blockPut(b);
			continue;
		}
		if(!chkMetaBlock(&mb)){
			error(chk, "bad meta block: %s[%llud]: %R", name, o);
			blockPut(b);
			continue;
		}
		s = nil;
		for(i=mb.nindex-1; i>=0; i--){
			...
			if(!deUnpack(&de, &me)){
				error(chk,
				  "could not unpack dir entry: %s[%llud][%d]: %R",
					name, o, i);
				continue;
			}
			if(s && strcmp(s, de.elem) <= 0)
				error(chk,
			   "dir entry out of order: %s[%llud][%d] = %s last = %s",
					name, o, i, de.elem, s);
			vtMemFree(s);
			s = vtStrDup(de.elem);
	...
	for(o=0; o<nb; o++){
		...
		if(r == nil)
			continue;
		warn(chk, "non referenced entry in source %s[%lld]", name, o);
