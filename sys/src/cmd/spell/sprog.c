...
Suftab staba[] = {{"aibohp", subst, 1, "-e+ia", "", NOUN, NOUN}, 0};

Suftab stabc[] = {{"cai", strip, 1, "", "+c", N_AFFIX, ADJ | NOUN},
...
Suftab stabe[] = {
     ...
     */
    {"ecna", subst, 1, "-t+ce", "", ADJ,
     N_AFFIX | _Y | NOUN | VERB | ACTOR | V_AFFIX},
    {"ecne", subst, 1, "-t+ce", "", ADJ,
     N_AFFIX | _Y | NOUN | VERB | ACTOR | V_AFFIX},
    ...
    {"ezi", CCe, 3, "-e+ize", "+ize", N_AFFIX | ADJ,
     V_AFFIX | VERB | ION | COMP},
    {"ekil", strip, 4, "", "+like", N_AFFIX, ADJ},
    0};
Suftab stabg[] = {{"gniee", strip, 3, "", "+ing", V_IRREG, ADJ | NOUN},
...
Suftab stabn[] = {
    ...
    {"noit", tion, 3, "-e+ion", "+ion", ION,
     NOUN | N_AFFIX | V_AFFIX | VERB | ACTOR},
    ...
    0};
Suftab stabp[] = {
    {"pihs", strip, 4, "", "+ship", NOUN | PROP_COLLECT, NOUN | N_AFFIX}, 0};
Suftab stabr[] = {
    ...
    {"re", strip, 1, "", "+r", ACTOR, NOUN | N_AFFIX | VERB | ADJ, i_to_y, 2,
     "-y+ier", "+er"},
...
Suftab stabz[] = {0};
Suftab* suftab[] = {
    staba, stabz, stabc, stabd, stabe, stabz, stabg, stabz, stabz,
    stabz, stabz, stabl, stabm, stabn, stabz, stabp, stabz, stabr,
    stabs, stabt, stabz, stabz, stabz, stabz, staby, stabz,
};

Ptab ptaba[] = {"anti", 0, "auto", 0, 0};
Ptab ptabb[] = {"bio", 0, 0};
Ptab ptabc[] = {"counter", 0, 0};
Ptab ptabd[] = {"dis", 0, 0};
Ptab ptabe[] = {"electro", 0, 0};
Ptab ptabf[] = {"femto", 0, 0};
Ptab ptabg[] = {"geo", 0, "giga", 0, 0};
Ptab ptabh[] = {"hyper", 0, 0};
Ptab ptabi[] = {"immuno", 0,  "im", IN, "intra", 0, "inter", 0,
                "in",     IN, "ir", IN, "iso",   0, 0};
Ptab ptabj[] = {0};
Ptab ptabk[] = {"kilo", 0, 0};
Ptab ptabl[] = {0};
Ptab ptabm[] = {"magneto", 0,     "mega", 0,       "meta",  0,      "micro",
                0,         "mid", 0,      "milli", 0,       "mini", 0,
                "mis",     0,     "mono", 0,       "multi", 0,      0};
Ptab ptabn[] = {"nano", 0, "neuro", 0, "non", 0, 0};
Ptab ptabo[] = {"out", 0, "over", 0, 0};
Ptab ptabp[] = {"para", 0, "photo",  0, "pico",   0, "poly", 0,
                "pre",  0, "pseudo", 0, "psycho", 0, 0};
Ptab ptabq[] = {"quasi", 0, 0};
Ptab ptabr[] = {"radio", 0, "re", 0, 0};
Ptab ptabs[] = {"semi", 0, "stereo", 0, "sub", 0, "super", 0, 0};
Ptab ptabt[] = {"tele", 0, "tera", 0, "thermo", 0, 0};
Ptab ptabu[] = {"ultra", 0,  "under", 0, /*must precede un*/
                "un",    IN, 0};
Ptab ptabv[] = {0};
Ptab ptabw[] = {0};
Ptab ptabx[] = {0};
...
Ptab* preftab[] = {
    ptaba, ptabb, ptabc, ptabd, ptabe, ptabf, ptabg, ptabh, ptabi,
    ptabj, ptabk, ptabl, ptabm, ptabn, ptabo, ptabp, ptabq, ptabr,
    ptabs, ptabt, ptabu, ptabv, ptabw, ptabx, ptaby, ptabz,
};
...
main(int argc, char* argv[])
{
	...
	while(argc > 1) {
		...
		for(i = 1; c = argv[1][i]; i++)
			...
			case 'f':
				if(argc <= 2) {
					fprint(2, "spell: -f requires another "
					          "argument\n");
					exits(Usage);
				}
				argv++;
	...
	for(;;) {
		...
		*ep = 0;
		...
		if(!low && !(h = trypref(ep, ".", 0, ALL | STOP | DONT_TOUCH)))
			for(cp = original + 1, dp = word + 1; dp < ep;
			    dp++, cp++)
				*dp = Tolower(*cp);
		if(!h)
			for(;;) { /* at most twice */
...
ordinal(void)
{
	...
	return 0 ==
	       strncmp(sp, cp[-2] == '1' ? "th" : /* out of bounds if 1 digit */
	                       *--cp == '1' ? "st" : /* harmless */
	                           *cp == '2' ? "nd" : *cp == '3' ? "rd" : "th",
	               3);
}
