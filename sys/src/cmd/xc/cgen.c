...
/*
 * compile expression n into nn
 * nn must be a register or Z
 */
void
cgen(Node *n, Node *nn)
{
	...
	case OAS:
		...
		while(r->op == OCAST && n->type->etype == r->type->etype)
			r = r->left;
...
/*
 * compile lvalue n into allocated register t
 * nn can be used for computation but not for the address
 */
void
...
/*
 * compile lvalue n into register nn
 */
void
...
boolgen(Node *n, int true, Node *nn)
{
	...
	default:
		...
		if(typefd[n->type->etype]){
			regalloc(&nod1, n, Z);
			gopcode(OAS, nodfconst(0), Z, &nod1);
...
/*
 * generate code for an expression whose type
 * is a structure or union
 * nn may not be a register
 * there are no releases in this code yet
 */
void
...
copy:
	...
	layout(&nod1, &nod2, w%c, w/c-1, &nod3);
	
	pc1 = pc;
	layout(&nod1, &nod2, c, 0, Z);

	gopcode(OSUB, nodconst(1L), Z, &nod3);
	gopcode(OGE, &nod3, Z, nodconst(0));
...
layout(Node *f, Node *t, int c, int cv, Node *cn)
{
	...
	if(c > 0) {
		gopcode(OAS, f, Z, &t1);
		gopcode(OADD, nodconst(SZ_LONG), Z, &fa);
	}
	if(cn != Z)
	...
	if(c > 0) {
		gopcode(OAS, &t1, Z, t);
		gopcode(OADD, nodconst(SZ_LONG), Z, &ta);
	}
	if(c > 2) {
		gopcode(OAS, f, Z, &t1);
		gopcode(OADD, nodconst(SZ_LONG), Z, &fa);
