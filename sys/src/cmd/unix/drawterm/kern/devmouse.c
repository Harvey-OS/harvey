...
mouseread(Chan* c, void* va, int32_t n, int64_t offset)
{
	...
	case Qmouse:
		...
		sprint(buf, "m%11d %11d %11d %11d", mouse.queue[i].xy.x,
		       mouse.queue[i].xy.y, mouse.queue[i].buttons,
		       mouse.queue[i].msec);
...
Dev mousedevtab = {
    ...
    devreset,  devinit,    devshutdown, mouseattach, mousewalk,
    mousestat, mouseopen,  devcreate,   mouseclose,  mouseread,
    devbread,  mousewrite, devbwrite,   devremove,   devwstat,
};
