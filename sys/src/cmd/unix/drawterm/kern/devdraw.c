...
Memscreen*
drawinstallscreen(Client* client, DScreen* d, int id, DImage* dimage,
                  DImage* dfill, int public)
{
	Memscreen* s;
	CScreen* c;
...
Point
drawchar(Memimage* dst, Memimage* rdst, Point p, Memimage* src, Point* sp,
         DImage* font, int index, int op)
{
	 ...
	 * directly.
	 */
	if(1 || (isvgascreen(dst) &&
	         !isvgascreen(rdst) /*&& font->image->depth > 1*/)) {
		if(tmp == nil || tmp->chan != dst->chan || Dx(tmp->r) < Dx(r) ||
		   Dy(tmp->r) < Dy(r)) {
			if(tmp)
				freememimage(tmp);
			tmp =
			    allocmemimage(Rect(0, 0, Dx(r), Dy(r)), dst->chan);
			if(tmp == nil)
				goto fallback;
		}
		memdraw(tmp, Rect(0, 0, Dx(r), Dy(r)), rdst, r.min, memopaque,
		        ZP, S);
...
initscreenimage(void)
{
	...
	screendata.base = nil;
	screendata.bdata =
	    attachscreen(&r, &chan, &depth, &width, &sdraw.softscreen, &X);
...
drawclose(Chan* c)
{
	...
	if(QID(c->qid) <
	   Qcolormap) /* Qtopdir, Qnew, Q3rd, Q2nd have no client */
		return;
	qlock(&sdraw.lk);
	if(waserror()) {
		qunlock(&sdraw.lk);
...
drawread(Chan* c, void* a, int32_t n, int64_t off)
{
	...
	case Qctl:
		...
		n = sprint(a, "%11d %11d %11s %11d %11d %11d %11d %11d %11d "
		              "%11d %11d %11d ",
		           cl->clientid, cl->infoid, chantostr(buf, i->chan),
		           (i->flags & Frepl) == Frepl, i->r.min.x, i->r.min.y,
		           i->r.max.x, i->r.max.y, i->clipr.min.x,
		           i->clipr.min.y, i->clipr.max.x, i->clipr.max.y);
		cl->infoid = -1;
		break;
	...
	case Qrefresh:
		...
		for(;;) {
			...
			if(waserror()) {
				qlock(
				    &sdraw.lk); /* restore lock for waserror()
				                   above */
				nexterror();
			}
	...
	case Qcolormap:
		...
		while(m > 0) {
			...
			*q = 0;
			...
			if(fields[3] == q)
				error(Ebadarg);
			if(red > 255 || green > 255 || blue > 255 || i < 0 ||
			   i > 255)
				error(Ebadarg);
			red |= red << 8;
			red |= red << 16;
	...
	*q++ = *a++;
	for(p = fmt; *p; p++) {
		...
		case 'L':
			...
			break;
		case 'R':
			q += sprint(q, " [%d %d %d %d]", BGLONG(a),
			            BGLONG(a + 4), BGLONG(a + 8),
			            BGLONG(a + 12));
			a += 16;
			break;
		case 'P':
...
drawmesg(Client* client, void* av, int n)
{
	...
	while((n -= m) > 0) {
		...
		/* new allocate: 'b' id[4] screenid[4] refresh[1] chan[4]
		 * repl[1] R[4*4] clipR[4*4] rrggbbaa[4] */
		case 'b':
			...
			if(scrnid) {
				...
				if(repl || chan != scrn->image->chan)
					error("image parameters incompatible "
					      "with screen");
				reffn = 0;
				switch(refresh) {
				case Refbackup:
		...
		case 'c':
			...
			if(ddst->name)
				error(
				    "can't change repl/clipr of shared image");
			dst = ddst->image;
			if(a[5])
				dst->flags |= Frepl;
		...
		case 'P':
			...
			if(pp == nil)
				error(Enomem);
			doflush = 0;
			if(dstid == 0 || (dst->layer &&
			                  dst->layer->screen->image->data ==
			                      screenimage->data))
...
Dev drawdevtab = {
    ...
    devreset, devinit,   devshutdown, drawattach, drawwalk,
    drawstat, drawopen,  devcreate,   drawclose,  drawread,
    devbread, drawwrite, devbwrite,   devremove,  devwstat,
};
...
drawactive(int active)
{
	/*
	   ...
	   TK2SEC(MACHP(0)->ticks - sdraw.blanktime)/60 >= blanktime)
	                        drawblankscreen(1);
	        }
	*/
}
