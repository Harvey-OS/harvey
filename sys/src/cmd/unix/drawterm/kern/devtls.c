...
/*
 *  devtls - record layer for transport layer security 1.0 and secure sockets
 * layer 3.0
 */
#include "u.h"
#include "lib.h"
...
enum { Statlen = 1024, /* max. length of status or stats message */
       /* buffer limits */
...
struct Secret {
	...
	DigestState* (*mac)(uint8_t*, uint32_t, uint8_t*, uint32_t, uint8_t*,
	                    DigestState*);
...
struct TlsRec {
	...
	void (*packMac)(Secret*, uint8_t*, uint8_t*, uint8_t*, uint8_t*, int,
	                uint8_t*);
...
static TlsErrs tlserrs[] = {
    ...
    {EUnexpectedMessage, EUnexpectedMessage, EUnexpectedMessage, 1,
     "unexpected message"},
    {EBadRecordMac, EBadRecordMac, EBadRecordMac, 1, "bad record mac"},
    {EDecryptionFailed, EIllegalParameter, EDecryptionFailed, 1,
     "decryption failed"},
    {ERecordOverflow, EIllegalParameter, ERecordOverflow, 1, "record too long"},
    {EDecompressionFailure, EDecompressionFailure, EDecompressionFailure, 1,
     "decompression failed"},
    {EHandshakeFailure, EHandshakeFailure, EHandshakeFailure, 1,
     "could not negotiate acceptable security parameters"},
    {ENoCertificate, ENoCertificate, ECertificateUnknown, 1,
     "no appropriate certificate available"},
    {EBadCertificate, EBadCertificate, EBadCertificate, 1,
    ...
    {ECertificateRevoked, ECertificateRevoked, ECertificateRevoked, 1,
     "revoked certificate"},
    {ECertificateExpired, ECertificateExpired, ECertificateExpired, 1,
     "expired certificate"},
    {ECertificateUnknown, ECertificateUnknown, ECertificateUnknown, 1,
     "unacceptable certificate"},
    {EIllegalParameter, EIllegalParameter, EIllegalParameter, 1,
     "illegal parameter"},
    {EUnknownCa, EHandshakeFailure, EUnknownCa, 1,
     "unknown certificate authority"},
    {EAccessDenied, EHandshakeFailure, EAccessDenied, 1, "access denied"},
    {EDecodeError, EIllegalParameter, EDecodeError, 1,
     "error decoding message"},
    {EDecryptError, EIllegalParameter, EDecryptError, 1,
     "error decrypting message"},
    {EExportRestriction, EHandshakeFailure, EExportRestriction, 1,
     "export restriction violated"},
    {EProtocolVersion, EIllegalParameter, EProtocolVersion, 1,
     "protocol version not supported"},
    {EInsufficientSecurity, EHandshakeFailure, EInsufficientSecurity, 1,
     "stronger security routines required"},
    {EInternalError, EHandshakeFailure, EInternalError, 1, "internal error"},
    {EUserCanceled, ECloseNotify, EUserCanceled, 0,
     "handshake canceled by user"},
    {ENoRenegotiation, EUnexpectedMessage, ENoRenegotiation, 0,
     "no renegotiation"},
};
...
static DigestState* sslmac_sha1(uint8_t* p, uint32_t len, uint8_t* key,
                                uint32_t klen, uint8_t* digest, DigestState* s);
static DigestState* nomac(uint8_t* p, uint32_t len, uint8_t* key, uint32_t klen,
                          uint8_t* digest, DigestState* s);
static void sslPackMac(Secret* sec, uint8_t* mackey, uint8_t* seq,
                       uint8_t* header, uint8_t* body, int len, uint8_t* mac);
...
static char* tlsnames[] = {
    /* unused */ 0,
    /* topdir */ 0,
    /* protodir */ 0, "clone", "encalgs", "hashalgs",
    /* convdir */ 0, "data", "ctl", "hand", "status", "stats",
};
...
static int
tlsstat(Chan* c, uint8_t* db, int n)
{
...
static int
tlswstat(Chan* c, uint8_t* dp, int n)
{
	Dir* d;
	TlsRec* tr;
	int rv;
...
ensure(TlsRec* s, Block** l, int n)
{
	...
	while(sofar < n) {
		bl = devtab[s->c->type]->bread(s->c,
		                               MaxCipherRecLen + RecHdrLen, 0);
		if(bl == 0)
			error(Ehungup);
		*l = bl;
		i = 0;
...
tlsrecread(TlsRec* tr)
{
	...
	if((tr->handin == 0) && (header[0] & 0x80)) {
		...
		if(type != SSL2ClientHello || len < 22)
			rcvError(tr, EProtocolVersion,
			         "invalid initial SSL2-like message");
	...
	if(ver != tr->version &&
	   (tr->verset || ver < MinProtoVersion || ver > MaxProtoVersion))
		rcvError(tr, EProtocolVersion, "devtls expected ver=%x%s, saw "
		                               "(len=%d) type=%x ver=%x "
		                               "'%.12s'",
		         tr->version, tr->verset ? "/set" : "", len, type, ver,
		         (char*)header);
	if(len > MaxCipherRecLen || len < 0)
		rcvError(tr, ERecordOverflow, "record message too long %d",
		         len);
	...
	if(in->sec != nil) {
		...
		put64(seq, in->seq);
		in->seq++;
		(*tr->packMac)(in->sec, in->sec->mackey, seq, header, p, len,
		               hmac);
		if(unpad_len < in->sec->maclen)
	...
	switch(type) {
	default:
		rcvError(tr, EIllegalParameter, "invalid record message 0x%x",
		         type);
		break;
	case RChangeCipherSpec:
		if(len != 1 || p[0] != 1)
			rcvError(tr, EDecodeError,
			         "invalid change cipher spec");
		qlock(&in->seclock);
		if(in->new == nil) {
			qunlock(&in->seclock);
			rcvError(tr, EUnexpectedMessage,
	...
	case RAlert:
		...
		if(p[0] == 2)
			rcvAlert(tr, p[1]);
		if(p[0] != 1)
			rcvError(tr, EIllegalParameter,
			         "invalid alert fatal code");
	...
	case RHandshake:
		 ...
		 * qbwrite always queues even if flow controlled and
		 *interrupted.
		 *
	...
	case SSL2ClientHello:
		...
		if(tr->handq != nil) {
			...
			/* Pass the SSL2 format data, so that the handshake code
			   can compute
			        the correct checksums.  HSSL2ClientHello =
			   HandshakeType 9 is
			        unused in RFC2246. */
			b = padblock(b, 8);
			b->rp[0] = RHandshake;
	...
	case RApplication:
		if(!tr->opened)
			rcvError(tr, EUnexpectedMessage, "application message "
			                                 "received before "
			                                 "handshake completed");
		if(BLEN(b) > 0) {
			tr->processed = b;
...
static Block*
tlsbread(Chan* c, int32_t n, uint32_t offset)
{
	int ty;
	Block* b;
	TlsRec* volatile tr;
	...
	if(ty == Qdata) {
		 ...
		 */
		while(!tr->opened && tr->hprocessed == nil &&
		      !qcanread(tr->handq))
			tlsrecread(tr);
...
tlsrecwrite(TlsRec* tr, int type, Block* b)
{
	...
	while(bb != nil) {
		...
		if(n > MaxRecLen) {
			...
			/*
			 * carefully reuse bb so it will get freed if we're out
			 * of memory
			 */
		 ...
		 * if not, we're out of sync with the receiver and will not
		 * recover.
		 */
		if(waserror()) {
...
static int32_t
tlsbwrite(Chan* c, Block* b, uint32_t offset)
{
	int ty;
	uint32_t n;
	TlsRec* tr;
...
static void
initclearmac(Hashalg* unused1, int unused2, Secret* s, uint8_t* unused3)
{
	s->maclen = 0;
	s->mac = nomac;
}
...
static void
initRC4key(Encalg* ea, Secret* s, uint8_t* p, uint8_t* unused1)
{
...
static void
initDES3key(Encalg* unused1, Secret* s, uint8_t* p, uint8_t* iv)
{
...
static void
initclearenc(Encalg* unused1, Secret* s, uint8_t* unused2, uint8_t* unused3)
{
	s->enc = noenc;
	s->dec = noenc;
	s->block = 0;
...
static Encalg encrypttab[] = {{"clear", 0, 0, initclearenc},
...
tlswrite(Chan* c, void* a, int32_t n, int64_t off)
{
	...
	if(strcmp(cb->f[0], "fd") == 0) {
		...
		if(cb->nf != 5)
			error(
			    "usage: secret hashalg encalg isclient secretdata");
		...
		if(!tos->mac || !tos->enc || !tos->dec || !toc->mac ||
		   !toc->enc || !toc->dec)
		...
		if(tr->out.new == nil)
			error(
			    "cannot change cipher spec without setting secret");
		...
		*b->wp++ = 1;
		...
		if(tr->in.sec == nil || tr->out.sec == nil)
			error("cipher must be configured before enabling data "
			      "messages");
		lock(&tr->statelk);
...
Dev tlsdevtab = {
    ...
    devreset, tlsinit,  devshutdown, tlsattach, tlswalk,
    tlsstat,  tlsopen,  devcreate,   tlsclose,  tlsread,
    tlsbread, tlswrite, tlsbwrite,   devremove, tlswstat,
};
...
static int
noenc(Secret* unused1, uint8_t* unused2, int n)
{
	return n;
}
...
static DigestState*
nomac(uint8_t* unused1, uint32_t unused2, uint8_t* unused3, uint32_t unused4,
      uint8_t* unused5, DigestState* unused6)
{
	return nil;
}
 ...
 */
static DigestState*
sslmac_x(uint8_t* p, uint32_t len, uint8_t* key, uint32_t klen, uint8_t* digest,
         DigestState* s,
