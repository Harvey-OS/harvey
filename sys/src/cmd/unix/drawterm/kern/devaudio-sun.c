...
setvolbal(double left, double right)
{
	...
	if(left < 0 || right < 0) {
		...
		bal = tosun(1.0 - left / right, AUDIO_MID_BALANCE,
		            AUDIO_RIGHT_BALANCE);
	} else if(right < left) {
		vol = tosun(left / 100.0, AUDIO_MIN_GAIN, AUDIO_MAX_GAIN);
		bal = tosun(1.0 - right / left, AUDIO_MID_BALANCE,
		            AUDIO_LEFT_BALANCE);
	}
	AUDIO_INITINFO(&info);
...
getvolbal(int* left, int* right)
{
	...
	if(bal == AUDIO_MID_BALANCE) {
		...
		l = vol;
		r = vol *
		    (1.0 - fromsun(bal, AUDIO_MID_BALANCE, AUDIO_LEFT_BALANCE));
	} else {
		r = vol;
		l = vol * (1.0 - fromsun(bal, AUDIO_MID_BALANCE,
		                         AUDIO_RIGHT_BALANCE));
	}
	*left = (int32_t)(l + 0.5);
	*right = (int32_t)(r + 0.5);
