...
bfCBCencrypt(uint8_t* buf, int n, BFstate* s)
{
	...
	bo[0] = s->ivec[0] | ((uint32_t)s->ivec[1] << 8) |
	        ((uint32_t)s->ivec[2] << 16) | ((uint32_t)s->ivec[3] << 24);
	bo[1] = s->ivec[4] | ((uint32_t)s->ivec[5] << 8) |
	        ((uint32_t)s->ivec[6] << 16) | ((uint32_t)s->ivec[7] << 24);

	for(i = 0; i < n; i += 8, buf += 8) {
		bi[0] = buf[0] | ((uint32_t)buf[1] << 8) |
		        ((uint32_t)buf[2] << 16) | ((uint32_t)buf[3] << 24);
		bi[1] = buf[4] | ((uint32_t)buf[5] << 8) |
...
bfECBencrypt(uint8_t* buf, int n, BFstate* s)
{
	...
	for(i = 0; i < n; i += 8, buf += 8) {
		b[0] = buf[0] | ((uint32_t)buf[1] << 8) |
		       ((uint32_t)buf[2] << 16) | ((uint32_t)buf[3] << 24);
		b[1] = buf[4] | ((uint32_t)buf[5] << 8) |
		       ((uint32_t)buf[6] << 16) | ((uint32_t)buf[7] << 24);
...
bfencrypt(uint32_t* b, BFstate* s)
{
	...
	for(i = 1; i < 16; i += 2) {
		r ^= pb[i];
		r ^=
		    ((sb[(uint8_t)(l >> 24)] + sb[256 + ((uint8_t)(l >> 16))]) ^
		     sb[512 + ((uint8_t)(l >> 8))]) +
		    sb[768 + ((uint8_t)l)];

		l ^= pb[i + 1];
		l ^=
		    ((sb[(uint8_t)(r >> 24)] + sb[256 + ((uint8_t)(r >> 16))]) ^
		     sb[512 + ((uint8_t)(r >> 8))]) +
		    sb[768 + ((uint8_t)r)];
	}
