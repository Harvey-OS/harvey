...
usage(void)
{
	fprint(2, "usage: drawterm [-a authserver] [-c cpuserver] [-s "
	          "secstore] [-u user]\n");
	exits("usage");
}
int fdd;
...
{
	char dat[MaxStr], buf[MaxStr], cmd[MaxStr], *err, *secstoreserver, *p,
	    *s;
	int fd, ms, data;
...
p9any(int fd)
{
	...
	*dom++ = 0;
	...
	if(auth.num != AuthAs || memcmp(auth.chal, cchal, CHALLEN) != 0 ||
	   auth.id != 0) {
