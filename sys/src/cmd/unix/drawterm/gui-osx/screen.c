...
screeninit(void)
{
	...
	dataProviderRef = CGDataProviderCreateWithData(0, gscreen->data->bdata,
	                                               dx * dy * 4, 0);
	fullScreenImage =
	    CGImageCreate(dx, dy, 8, 32, dx * 4, CGColorSpaceCreateDeviceRGB(),
	                  kCGImageAlphaNoneSkipLast, dataProviderRef, 0, 0,
...
static OSStatus ApplicationQuitEventHandler(EventHandlerCallRef nextHandler,
...
window_resized()
{
	...
	bounds = CGRectMake(0, 0, winRect.right - winRect.left,
...
winproc(void* a)
{
	...
	DrawMenuBar();
	uint32_t windowAttrs =
	    0 | kWindowCloseBoxAttribute | kWindowCollapseBoxAttribute |
	...
	InstallApplicationEventHandler(
	    ...
	    GetEventTypeCount(quit_events), quit_events, NULL, NULL);

	InstallApplicationEventHandler(
	    NewEventHandlerUPP(MainWindowEventHandler),
	    GetEventTypeCount(events), events, NULL, NULL);
	InstallWindowEventHandler(
	    theWindow, NewEventHandlerUPP(MainWindowCommandHandler),
	    GetEventTypeCount(commands), commands, theWindow, NULL);
...
// catch quit events to handle quits from menu, Cmd+Q, applescript, and task
// switcher
static OSStatus
ApplicationQuitEventHandler(EventHandlerCallRef nextHandler, EventRef event,
                            void* userData)
{
	exit(0);
                       ...
                       void* userData)
{
	...
	if(class == kEventClassKeyboard) {
		...
		GetEventParameter(event, kEventParamKeyMacCharCodes, typeChar,
		                  NULL, sizeof(macCharCodes), NULL,
		                  &macCharCodes);
		GetEventParameter(event, kEventParamKeyCode, typeUInt32, NULL,
		                  sizeof(macKeyCode), NULL, &macKeyCode);
		GetEventParameter(event, kEventParamKeyModifiers, typeUInt32,
		                  NULL, sizeof(macKeyModifiers), NULL,
		                  &macKeyModifiers);
		switch(kind) {
		...
		case kEventMouseWheelMoved: {
			...
			GetEventParameter(event, kEventParamMouseWheelDelta,
			                  typeSInt32, 0, sizeof(wheeldelta), 0,
			                  &wheeldelta);
		...
		case kEventMouseDown: {
			...
			GetEventParameter(event, kEventParamKeyModifiers,
			                  typeUInt32, 0, sizeof(modifiers), 0,
			                  &modifiers);
			GetEventParameter(event, kEventParamMouseChord,
			                  typeUInt32, 0, sizeof buttons, 0,
			                  &buttons);
...
static OSStatus
MainWindowCommandHandler(EventHandlerCallRef nextHandler, EventRef event,
                         void* userData)
{
	OSStatus result = noErr;
	UInt32 class = GetEventClass(event);
	...
	if(class == kEventClassCommand) {
		...
		case kEventWindowClosed:
			// send a quit carbon event instead of directly calling
			// cleanexit
			// so that all quits are done in
			// ApplicationQuitEventHandler
			{
				EventRef quitEvent;
				CreateEvent(NULL, kEventClassApplication,
				            kEventAppQuit, 0,
				            kEventAttributeNone, &quitEvent);
...
clipread(void)
{
	...
	for(i = 1; i <= nItems; ++i) {
		...
		if((err = PasteboardCopyItemFlavors(
		        appleclip, itemID, &flavorTypeArray)) != noErr) {
		...
		for(flavorIndex = 0; flavorIndex < flavorCount; ++flavorIndex) {
			...
			if(UTTypeConformsTo(flavorType,
				...
				if((err = PasteboardCopyItemFlavorData(
				        appleclip, itemID,
				        CFSTR("public.utf16-plain-text"),
				        &cfdata)) != noErr) {
					fprint(2, "apple pasteboard CopyItem "
					          "failed - Error %d\n",
					       err);
					return 0;
				}
				CFIndex length = CFDataGetLength(cfdata);
				if(length > sizeof rsnarf)
					length = sizeof rsnarf;
...
clipwrite(char* snarf)
{
	...
	cfdata = CFDataCreate(kCFAllocatorDefault, (uint8_t*)rsnarf,
	                      runestrlen(rsnarf) * 2);
	if(cfdata == nil) {
...
screenload(Rectangle r, int depth, uint8_t* p, Point pt, int step)
{
	...
	rbounds.origin.y =
	    winRect.bottom - winRect.top - r.min.y - rbounds.size.height;
...
// SetCursor and InitCursor are marked as deprecated in 10.4, and I can't for
// the
