...
uint32_t _imgtorgba(Memimage *, uint32_t);
uint32_t _rgbatoimg(Memimage *, uint32_t);
uint32_t _pixelbits(Memimage *, Point);
...
chanmatch(Buffer *bdst, Buffer *bsrc)
{
	 ...
	 * in the rgba word.
	 */
	drgb = (uint8_t *)bdst->rgba;
	srgb = (uint8_t *)bsrc->rgba;
	if(bdst->red - drgb != bsrc->red - srgb || bdst->blu - drgb != bsrc->blu - srgb || bdst->grn - drgb != bsrc->grn - srgb)
		return 0;
...
memoptdraw(Memdrawparam *par)
{
	...
	if((par->state & (m | Replsrc)) == m && src->depth >= 8 && src->chan == dst->chan && !(src->flags & Falpha) && (op == S || op == SoverD)) {
		uchar *sp, *dp;
	 ...
	 */
	if((par->state & (Simplemask | Simplesrc | Replmask | Replsrc)) == 0 && dst->chan == GREY1 && src->chan == GREY1 && par->mask->chan == GREY1 && (par->r.min.x & 7) == (par->sr.min.x & 7) && (par->r.min.x & 7) == (par->mr.min.x & 7)) {
		uchar *sp, *dp, *mp;
		uchar lm, rm;
...
chardraw(Memdrawparam *par)
{
	...
	if(0)
		if(drawdebug)
			iprint("chardraw? mf %lux md %d sf %lux dxs %d dys %d dd %d ddat %p sdat %p\n",
	...
	if((par->state & (Replsrc | Simplesrc | Replmask)) != (Replsrc | Simplesrc) || mask->depth != 1 || src->flags & Falpha || dst->depth < 8 || dst->data == src->data || op != SoverD)
		return 0;
...
void
_memfillcolor(Memimage *i, uint32_t val)
{
	uint32_t bits;
	int d, y;
