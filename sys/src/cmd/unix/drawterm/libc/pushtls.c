...
// given a plain fd and secrets established beforehand, return encrypted
// connection
int
pushtls(int fd, char* hashalg, char* encalg, int isclient, char* secret,
        char* dir)
{
	char buf[8];
	char dname[64];
	...
	// user-level file server can't stand in for #a; may as well hard-code
	// it.
	...
	if(fprint(ctl, "fd %d 0x301", fd) < 0 ||
	   ...
	   fprint(ctl, "secret %s %s %d %s", hashalg, encalg, isclient,
	          secret) < 0 ||
	   fprint(ctl, "changecipher") < 0 || finished(hand, isclient) < 0 ||
	   fprint(ctl, "opened") < 0) {
		close(hand);
		hand = -1;
		goto error;
