...
fcallfmt(Fmt* fmt)
{
	...
	switch(type) {
	case Tversion: /* 100 */
		seprint(buf, e, "Tversion tag %ud msize %ud version '%s'", tag,
		        f->msize, f->version);
		break;
	case Rversion:
		seprint(buf, e, "Rversion tag %ud msize %ud version '%s'", tag,
		        f->msize, f->version);
		break;
	case Tauth: /* 102 */
		...
		break;
	case Rauth:
		seprint(buf, e, "Rauth tag %ud qid " QIDFMT, tag, f->aqid.path,
	...
	case Tattach: /* 104 */
		...
		break;
	case Rattach:
		seprint(buf, e, "Rattach tag %ud qid " QIDFMT, tag, f->qid.path,
	...
	case Rflush:
		...
		break;
	case Twalk: /* 110 */
		p = seprint(buf, e, "Twalk tag %ud fid %d newfid %d nwname %d ",
	...
	case Rwalk:
		...
		if(f->nwqid <= MAXWELEM)
			for(i = 0; i < f->nwqid; i++) {
				q = &f->wqid[i];
				p = seprint(p, e, "%d:" QIDFMT " ", i, q->path,
				            q->vers, qidtype(tmp, q->type));
			}
		break;
	case Topen: /* 112 */
		seprint(buf, e, "Topen tag %ud fid %ud mode %d", tag, fid,
		        f->mode);
		break;
	case Ropen:
	...
	case Tcreate: /* 114 */
		seprint(buf, e,
		        "Tcreate tag %ud fid %ud name %s perm %M mode %d", tag,
		        fid, f->name, (uint32_t)f->perm, f->mode);
		break;
	case Rcreate:
		seprint(buf, e, "Rcreate tag %ud qid " QIDFMT " iounit %ud ",
		        tag, f->qid.path, f->qid.vers,
		        qidtype(tmp, f->qid.type), f->iounit);
		break;
	case Tread: /* 116 */
	...
	case Twrite: /* 118 */
		p = seprint(buf, e,
		            "Twrite tag %ud fid %d offset %lld count %ud ", tag,
