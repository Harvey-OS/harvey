...
enum { PMundef = ~0 /* undefined pixmap id */
};
...
getXdata(Memimage* m, Rectangle r)
{
	...
	if(xtblbit && m->chan == CMAP8)
		for(y = r.min.y; y < r.max.y; y++)
			for(x = r.min.x, p = byteaddr(m, Pt(x, y)); x < r.max.x;
			    x++, p++)
				*p = x11toplan9[*p];
...
putXdata(Memimage* m, Rectangle r)
{
	...
	XPutImage(xdisplay, xm->pmid, g, xi, xdelta.x, xdelta.y, delta.x,
	          delta.y, Dx(r), Dy(r));
...
xallocmemimage(Rectangle r, uint32_t chan, int pmid)
{
	...
	if(pmid != PMundef)
		...
		xm->pmid = XCreatePixmap(xdisplay, xscreenid, Dx(r), Dy(r),
		                         (d == 32) ? 24 : d);
	...
	xi = XCreateImage(xdisplay, xvis, m->depth == 32 ? 24 : m->depth,
	                  ZPixmap, 0, (char*)m->data->bdata, Dx(r), Dy(r), 32,
...
xdraw(Memdrawparam* par)
{
	...
	if((par->state & (m | Replsrc)) == m && src->chan == dst->chan &&
	   src->X) {
		sxm = src->X;
		...
		if(dst->chan == GREY1)
			...
			gc = xgccopy;
		XCopyArea(xdisplay, sxm->pmid, dxm->pmid, gc, sr.min.x,
		          sr.min.y, dx, dy, r.min.x, r.min.y);
		dirtyXdata(dst, par->r);
		return 1;
	}
	 ...
	 */
	if(dst->X && mask->X && !(mask->flags & Frepl) && mask->chan == GREY1 &&
	   (par->state & Simplesrc)) {
		Point p;
		...
		XFillRectangle(xdisplay, dxm->pmid, gc, r.min.x, r.min.y, dx,
		               dy);
		dirtyXdata(dst, par->r);
		return 1;
	}
...
flushmemscreen(Rectangle r)
{
	...
	if(r.min.x >= r.max.x || r.min.y >= r.max.y)
		return;
	XCopyArea(xdisplay, xscreenid, xdrawable, xgccopy, r.min.x, r.min.y,
...
setcursor(void)
{
	...
	xc = XCreatePixmapCursor(xdisplay, xsrc, xmask, &fg, &bg,
...
xproc(void* arg)
{
	...
	mask = KeyPressMask | ButtonPressMask | ButtonReleaseMask |
...
xinitscreen(void)
{
	...
	if(XMatchVisualInfo(xdisplay, rootscreennum, 16, TrueColor, &xvi) ||
	   XMatchVisualInfo(xdisplay, rootscreennum, 16, DirectColor, &xvi)) {
		...
		xscreendepth = 16;
		xtblbit = 1;
	} else if(XMatchVisualInfo(xdisplay, rootscreennum, 24, TrueColor,
	                           &xvi) ||
	          XMatchVisualInfo(xdisplay, rootscreennum, 24, DirectColor,
	                           &xvi)) {
		xvis = xvi.visual;
		xscreendepth = 24;
		xtblbit = 1;
	} else if(XMatchVisualInfo(xdisplay, rootscreennum, 8, PseudoColor,
	                           &xvi) ||
	          XMatchVisualInfo(xdisplay, rootscreennum, 8, StaticColor,
	                           &xvi)) {
		if(xscreendepth > 8)
			panic("drawterm: can't deal with colormapped depth %d "
			      "screens\n",
			      xscreendepth);
		xvis = xvi.visual;
		xscreendepth = 8;
	...
	for(i = 0; i < n; i++) {
		if(pfmt[i].depth == xscreendepth) {
			...
			case 24: /* untested (impossible?) */
				...
				break;
			case 32:
				xscreenchan = CHAN4(CIgnore, 8, CRed, 8, CGreen,
				                    8, CBlue, 8);
				break;
			}
		}
	...
	/* attrs.override_redirect = 1;*/ /* WM leave me alone!
	                                     |CWOverrideRedirect */
	xdrawable = XCreateWindow(
	    xdisplay, rootwin, 0, 0, xsize, ysize, 0, xscreendepth, InputOutput,
	    xvis, CWBackPixel | CWBorderPixel | CWColormap, &attrs);

	/* load the given bitmap data and create an X pixmap containing it. */
	icon_pixmap = XCreateBitmapFromData(
	    xdisplay, rootwin, (char*)glenda_bits, glenda_width, glenda_height);
	...
	xscreenid =
	    XCreatePixmap(xdisplay, xdrawable, Dx(r), Dy(r), xscreendepth);
 ...
 * application.  Drawterm gets the best colors here when it has the cursor
 * focus.
 */
static void
initmap(Window w)
	...
	if(xscreendepth >= 24) {
		...
		if(xtblbit == 0) {
			...
			for(i = 0; i < 128; i++) {
				c = map7[i];
				if(!XAllocColor(xdisplay, xcmap, &c)) {
					iprint("drawterm: can't alloc colors "
					       "in default map, don't use "
					       "-7\n");
					exit(0);
				}
...
xmouse(XEvent* e)
{
	...
	if(putsnarf != assertsnarf) {
		...
		if(clipboard != None)
			XSetSelectionOwner(xkmcon, clipboard, xdrawable,
			                   CurrentTime);
		XFlush(xkmcon);
	}
...
_xgetsnarf(XDisplay* xd)
{
	...
	XChangeProperty(xd, xdrawable, prop, XA_STRING, 8, PropModeReplace,
	                (uint8_t*)"", 0);
	XConvertSelection(xd, clipboard, XA_STRING, prop, xdrawable,
	                  CurrentTime);
	XFlush(xd);
	lastlen = 0;
	for(i = 0; i < 10 || (lastlen != 0 && i < 30); i++) {
		usleep(100 * 1000);
		XGetWindowProperty(xd, xdrawable, prop, 0, 0, 0,
	...
	/* get the property */
	data = nil;
	XGetWindowProperty(xd, xdrawable, prop, 0,
	                   SnarfSize / sizeof(unsigned long), 0,
...
xselect(XEvent* e, XDisplay* xd)
{
	...
	if(0)
		iprint(
		    "xselect target=%d requestor=%d property=%d selection=%d\n",
	...
	if(xe->target == targets) {
		...
		XChangeProperty(xd, xe->requestor, xe->property, XA_ATOM, 32,
		                PropModeReplace, (uint8_t*)a, sizeof a);
	} else if(xe->target == XA_STRING || xe->target == utf8string ||
	          xe->target == text || xe->target == compoundtext) {
	text:
		/* if the target is STRING we're supposed to reply with Latin1
		 * XXX */
		qlock(&clip.lk);
		XChangeProperty(xd, xe->requestor, xe->property, xe->target, 8,
		                PropModeReplace, (uint8_t*)clip.buf,
		                strlen(clip.buf));
		qunlock(&clip.lk);
	} else {
		...
		if(name) {
			if(strcmp(name, "TIMESTAMP") == 0) {
				...
				/* nothing */
			          ...
			          strcmp(name, "text/plain;charset=UTF-8") ==
			              0) {
				goto text;
			} else
				iprint("%s: cannot handle selection request "
				       "for '%s' (%d)\n",
