...
#define	BLANKTIME	10*24*60*60*1000	/* ten days */
...
drawaddname(Client *client, DImage *di, int n, char *str)
{
	...
	for(; name<ename; name++)
		if(drawcmp(name->name, str, n) == 0)
			error(Enameused);
	t = mallocz((sdraw.nname+1)*sizeof(DName));
	memmove(t, sdraw.name, sdraw.nname*sizeof(DName));
	free(sdraw.name);
	sdraw.name = t;
	new = &sdraw.name[sdraw.nname++];
	new->name = mallocz(n+1);
...
initscreenimage(void)
{
	...
	if(screenimage == nil)
		fatal("cannot allocate screen: %r\n");
...
long
drawread(Chan *c, void *a, long n, ulong off)
{
	int index, m;
...
long
drawwrite(Chan *c, void *a, long n, ulong off)
{
