...
enum { MAXCHLEN = 256,    /* max challenge length	*/
...
enum { AttrNameval, /* name=val -- when matching, must have name=val */
       AttrQuery,   /* name? -- when matching, must be present */
       AttrDefault, /* name:=val -- when matching, if present must match
                       INTERNAL */
};
struct Attr {
	int type;
