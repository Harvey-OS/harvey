...
/* already in plan9.h #include <sys/types.h> */ /* for struct passwd, struct
                                                   group, struct stat ... */
/* plan9.h is first to get the large file support definitions as early as
 * possible */
#include <plan9.h>
...
getfcallnew(int fd, Fcall* fc, int have)
{
	...
	if(have < BIT32SZ &&
	   readn(fd, rxbuf + have, BIT32SZ - have) != BIT32SZ - have)
...
getfcall(int fd, Fcall* fc)
{
	...
	if(50 <= rxbuf[0] && rxbuf[0] <= 87 && (rxbuf[0] & 1) == 0 &&
	   GBIT16(rxbuf + 1) == 0xFFFF) {
		old9p = 1;
		getfcallold(fd, fc, 3);
		return;
	}
...
rattach(Fcall* rx, Fcall* tx)
{
	...
	if((u = uname2user(rx->uname)) == nil || (!defaultuser && u->id == 0)) {
...
stat2qid(struct stat* st)
{
	...
	if(q > ep) {
		/*
		 *		fprint(2, "warning: inode number + device number
		 *too big %d+%d\n",
	...
	if(fid->dir) {
		...
		for(;;) {
			if(p + BIT16SZ >= ep)
				break;
			if(fid->dirent ==
			   nil) /* one entry cache for when convD2M fails */
			...
			if((n = (old9p ? convD2Mold
			               : convD2M)(&d, p, ep - p)) <= BIT16SZ)
				break;
			p += n;
			fid->dirent = nil;
		}
...
rwrite(Fcall* rx, Fcall* tx)
{
	...
	if(fid->omode == -1 || (fid->omode & 3) == OREAD ||
	   (fid->omode & 3) == OEXEC) {
		seterror(tx, Ebadusefid);
		return;
	}
...
rstat(Fcall* rx, Fcall* tx)
{
	...
	if((tx->nstat = (old9p ? convD2Mold : convD2M)(&d, tx->stat, msize)) <=
	   BIT16SZ)
...
rwstat(Fcall* rx, Fcall* tx)
{
	 ...
	 */
	if((old9p ? convM2Dold : convM2D)(rx->stat, rx->nstat, &d,
	                                  (char*)rx->stat) <= BIT16SZ) {
	 ...
	 * is admittedly superfluous, given the current lack of 128-bit
	 *machines).
	 */
	gid = (gid_t)-1;
	if(d.gid[0] != '\0') {
	...
	if((uint32_t)d.mode != (uint32_t)~0 &&
	   (((d.mode & DMDIR) != 0) ^ (S_ISDIR(fid->st.st_mode) != 0))) {
	...
	if((uint64_t)d.length != (uint64_t)~0 &&
	   truncate(fid->path, d.length) < 0) {
...
userperm(User* u, char* path, int type, int need)
{
	...
	if((have & need) == need)
		return 0;
	if(((have | ((st.st_mode >> 3) & 7)) & need) !=
	   need) /* group won't help */
		return -1;
...
useropen(Fid* fid, int omode, char** ep)
{
	 ...
	 */
	if((omode & ORCLOSE) &&
	   userperm(fid->u, fid->path, Tdotdot, W_OK) < 0) {
		*ep = Eperm;
		return -1;
	}
...
usage(void)
{
	fprint(
	    2,
	    "usage: u9fs [-Dnz] [-a authmethod] [-m msize] [-u user] [root]\n");
	exit(1);
}
