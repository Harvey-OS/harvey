...
int angle = 0;
int showbottom =
    0; /* on the next showpage, move the image so the bottom is visible. */

Rectangle
    ulrange; /* the upper left corner of the image must be in this rectangle */
Point
    ul; /* the upper left corner of the image is at this point on the screen */
...
Cursor reading = {
    {-1, -1},
    {
     0xff, 0x80, 0xff, 0x80, 0xff, 0x00, 0xfe, 0x00, 0xff, 0x00, 0xff, 0x80,
     0xff, 0xc0, 0xef, 0xe0, 0xc7, 0xf0, 0x03, 0xf0, 0x01, 0xe0, 0x00, 0xc0,
     0x03, 0xff, 0x03, 0xff, 0x03, 0xff, 0x03, 0xff,
    },
    {
     0x00, 0x00, 0x7f, 0x00, 0x7e, 0x00, 0x7c, 0x00, 0x7e, 0x00, 0x7f, 0x00,
     0x6f, 0x80, 0x47, 0xc0, 0x03, 0xe0, 0x01, 0xf0, 0x00, 0xe0, 0x00, 0x40,
...
Cursor query = {
    {-7, -7},
    {
     0x0f, 0xf0, 0x1f, 0xf8, 0x3f, 0xfc, 0x7f, 0xfe, 0x7c, 0x7e, 0x78, 0x7e,
     0x00, 0xfc, 0x01, 0xf8, 0x03, 0xf0, 0x07, 0xe0, 0x07, 0xc0, 0x07, 0xc0,
     0x07, 0xc0, 0x07, 0xc0, 0x07, 0xc0, 0x07, 0xc0,
    },
    {
     0x00, 0x00, 0x0f, 0xf0, 0x1f, 0xf8, 0x3c, 0x3c, 0x38, 0x1c, 0x00, 0x3c,
...
viewer(Document* dd)
{
	...
	static char* miditems[] = {"orig size", "zoom in", "fit window",
	                           "rotate 90", "upside down", "", "next",
	                           "prev", "zerox", "", "reverse", "discard",
	                           "write", "", "quit", 0};
	char* s;
	enum { Eplumb = 4 };
	Plumbmsg* pm;
	...
	for(;;) {
		/*
		 * throughout, if doc->fwdonly is set, we restrict the
		 * functionality
		...
		case Ekeyboard:
			...
			case 'r': /* reverse page order */
				 ...
				 * should be dropped and this should happen
				 * always.
				 */
			...
			/*
			 * The upper y coordinate of the image is at ul.y in
			 * screen->r.
			 * Panning up means moving the upper left corner down.
			 * If the
			 * upper left corner is currently visible, we need to go
			 * back a page.
			 */
			case Kup:
				if(screen->r.min.y <= ul.y &&
				   ul.y < screen->r.max.y) {
					if(page > 0 && !doc->fwdonly) {
						--page;
						showbottom = 1;
			...
			/*
			 * If the lower y coordinate is on the screen, we go to
			 * the next page.
		...
		case Emouse:
			...
			case Middle:
				...
				case Zin: /* zoom in */
				{
					...
					r = egetrect(Middle, &m);
					if((rectclip(&r, rectaddpt(im->r,
					                           ul)) == 0) ||
					   Dx(r) == 0 || Dy(r) == 0)
						break;
					/* use the smaller side to expand */
					if(Dx(r) < Dy(r))
						delta = (double)Dx(im->r) /
						        (double)Dx(r);
					else
						delta = (double)Dy(im->r) /
						        (double)Dy(r);

					esetcursor(&reading);
					tmp = xallocimage(
					    display,
					    Rect(0, 0, (int)((double)Dx(im->r) *
					                     delta),
					         (int)((double)Dy(im->r) *
					               delta)),
					    im->chan, 0, DBlack);
				...
				case Fit: /* fit */
				{
					...
					delta = (double)Dx(screen->r) /
					        (double)Dx(im->r);
					if((double)Dy(im->r) * delta >
					   Dy(screen->r))
						delta = (double)Dy(screen->r) /
						        (double)Dy(im->r);

					r = Rect(
					    0, 0,
					    (int)((double)Dx(im->r) * delta),
					    (int)((double)Dy(im->r) * delta));
					esetcursor(&reading);
					tmp = xallocimage(display, r, im->chan,
					                  0, DBlack);
					if(tmp == nil) {
						fprint(2, "out of memory "
						          "during fit: %r\n");
		...
		case Eplumb:
			...
			if(showdata(pm)) {
				...
				/* lose fd reference on purpose; the file is
				 * open ORCLOSE */
			} else if(pm->data[0] == '/') {
				s = estrdup(pm->data);
			} else {
				s = emalloc(strlen(pm->wdir) + 1 + pm->ndata +
				            1);
 ...
 */
static void
gendrawdiff(Image* dst, Rectangle bot, Rectangle top, Image* src, Point sp,
            Image* mask, Point mp, int op)
{
	Rectangle r;
	Point origin;
	Point delta;
	...
	if(bot.min.x < top.min.x) {
		...
		gendrawop(dst, r, src, addpt(sp, delta), mask, addpt(mp, delta),
		          op);
		bot.min.x = top.min.x;
	}
...
static void
drawdiff(Image* dst, Rectangle bot, Rectangle top, Image* src, Image* mask,
         Point p, int op)
{
...
redraw(Image* screen)
{
	...
	ul = pclip(ul, ulrange);
	drawop(screen, screen->r, im, nil,
	       subpt(im->r.min, subpt(ul, screen->r.min)), S);
	...
	/* gray for the rest */
	if(gray == nil) {
		gray = xallocimage(display, Rect(0, 0, 1, 1), RGB24, 1,
		                   0x888888FF);
		if(gray == nil) {
...
/* all code below this line should be in the library, but is stolen from colors
 * instead */
static char*
rdenv(char* name)
{
...
screenrect(void)
{
	...
	close(fd);
	return Rect(atoi(buf + 12), atoi(buf + 24), atoi(buf + 36),
	            atoi(buf + 48));
}
