...
rdbbox(char* p)
{
	...
	if(a < 300 * 300) { /* really small, probably supposed to be */
		            /* empty */
	} else if(Dx(r) <= 596 && r.max.x <= 596 && Dy(r) > 792 &&
	          Dy(r) <= 842 && r.max.y <= 842) /* A4 */
		r = Rect(0, 0, 596, 842);
...
Keepreading:
	while(p = Brdline(b, eol)) {
		...
		if(reverse == -1 && prefix(p, "%%PageOrder")) {
			...
			if(strstr(p, "Landscape"))
				landscape = 1;
		} else if(incomments && Dx(bbox) == 0 &&
		          prefix(p, q = "%%BoundingBox")) {
		...
		/*
		 * If they use the initgraphics command, we can't play our
		 * translation tricks.
		 */
		p[Blinelen(b) - 1] = 0;
		if((q = strstr(p, "initgraphics")) &&
		   ((r = strchr(p, '%')) == nil || r > q))
			cantranslate = 0;
		p[Blinelen(b) - 1] = eol;
	...
	if(goodps) {
		/*
		 * We want to only send the page (i.e. not header and trailer)
		 * information
		 * for each page, so initialize the device by sending the header
		 * now.
		 */
	...
	if(dumb) {
		...
		fprint(ps->gsfd, "(/fd/3) (w) file dup (THIS IS NOT A PLAN9 "
		                 "BITMAP "
