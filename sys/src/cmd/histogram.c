...
drawdatum(int x, double prev, double v)
{
	...
	q = datapoint(x, prev);
	if(p.y < q.y) {
		draw(screen, Rect(p.x, hrect.min.y, p.x + 1, p.y), neutral, nil,
		     ZP);
		draw(screen, Rect(p.x, p.y, p.x + 1, q.y + Dot), dark, nil, ZP);
		draw(screen, Rect(p.x, q.y + Dot, p.x + 1, hrect.max.y), light,
		     nil, ZP);
	} else {
		draw(screen, Rect(p.x, hrect.min.y, p.x + 1, q.y), neutral, nil,
		     ZP);
...
redrawhistogram(int new)
{
	...
	maxvloc =
	    Pt(r.max.x - Lx - stringwidth(display->defaultfont, "999999999"),
	       r.min.y + Ly);
...
histogram(char* rect)
{
	...
	static char* mitems[] = {"exit", nil};
	static Menu menu = {mitems, nil, -1};
