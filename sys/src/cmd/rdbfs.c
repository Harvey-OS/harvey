...
usage(void)
{
	fprint(2, "usage: rdbfs [-p procnum] [-s srvname] [-t textfile] "
	          "[serialport]\n");
	exits("usage");
}
...
eiaread(void* v)
{
	...
	while(r = recvp(rchan)) {
		...
		if(r->type == Tread &&
		   lookup(r->ifcall.offset, (uint8_t*)r->ofcall.data,
		          r->ofcall.count)) {
			respond(r, nil);
			continue;
		}
		for(tries = 0; tries < 5; tries++) {
			...
			for(;;) {
				...
				if(p[0] == 'R') {
					if(strtoul(p + 1, 0, 16) ==
					   (uint32_t)r->ifcall.offset) {
						/* we know that data can handle
						 * Readlen bytes */
						data = (uint8_t*)r->ofcall.data;
						for(i = 0; i < r->ifcall.count;
						    i++)
							data[i] = strtol(
							    p + 1 + 8 + 1 +
							        3 * i,
							    0, 16);
						insert(r->ifcall.offset, data,
						       r->ifcall.count);
						respond(r, nil);
						goto Break2;
					} else
...
fsread(Req* r)
{
	...
	case Xmem:
		...
		break;
	case Xtext:
		n = pread(textfd, r->ofcall.data, r->ifcall.count,
		          r->ifcall.offset);
		if(n < 0) {
...
struct {
    ...
    "ctl", Xctl,  0222,   "fpregs", Xfpregs, 0666,     "kregs", Xkregs,
    0666,  "mem", Xmem,   0666,     "proc",  Xproc,    0444,    "regs",
    Xregs, 0666,  "text", Xtext,    0444,    "status", Xstatus, 0444,
};
...
Srv fs = {
    .open = fsopen, .read = fsread, .write = fswrite, .end = killall,
};
...
threadmain(int argc, char** argv)
{
	...
	for(i = 0; i < nelem(tab); i++)
		closefile(createfile(dir, tab[i].s, "rdbfs", tab[i].mode,
		                     (void*)tab[i].id));
	closefile(dir);
