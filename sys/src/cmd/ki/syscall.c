...
{ 
	...
	char name[7024], old[7024];
	int n;

	pname = getmem_w(reg.r[1]+4);
	pold = getmem_w(reg.r[1]+8);
	flags = getmem_w(reg.r[1]+12);
...
sysdup(void)
{
	...
	oldfd = getmem_w(reg.r[1]+4);
	newfd = getmem_w(reg.r[1]+8);
	if(sysdbg)
...
sysread(void)
{
	...
	fd = getmem_w(reg.r[1]+4);
	a = getmem_w(reg.r[1]+8);
	size = getmem_w(reg.r[1]+12);
...
sysseek(void)
{
	...
	fd = getmem_w(reg.r[1]+4);
	off = getmem_w(reg.r[1]+8);
	mode = getmem_w(reg.r[1]+12);
	if(sysdbg)
