...
main(int argc, char* argv[])
{
	...
	case 'u':
		...
		break;
	default:
		fprint(2, "usage: ls [-dlmnpqrstuFQT] [file ...]\n");
		exits("usage");
	}
	ARGEND
...
format(Dir* db, char* name)
{
	...
	if(sflag)
		Bprint(&bin, "%*llud ", swidth, (db->length + 1023) / 1024);
	if(mflag) {
	...
	if(qflag)
		Bprint(&bin, "(%.16llux %*lud %.2ux) ", db->qid.path, qwidth,
		       db->qid.vers, db->qid.type);
	if(Tflag)
		Bprint(&bin, "%c ", (db->mode & DMTMP) ? 't' : '-');

	if(lflag)
		Bprint(&bin, "%M %C %*ud %*q %*q %*llud %s ", db->mode,
		       db->type, vwidth, db->dev, -uwidth, db->uid, -gwidth,
		       db->gid, lwidth, db->length,
