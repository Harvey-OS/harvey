...
#include <libc.h>
#include <disk.h>
#include <thread.h>	/* botch: to print error messages */
#include "scsireq.h"
#include "usb.h"	/* botch: to print error messages */
#include "usbfs.h"	/* botch: to print error messages */
#include "ums.h"	/* botch: to print error messages */
...
SRdumpReq(ScsiReq *rp)
{
	...
	e = buf+sizeof(buf);
	p = seprint(buf, e, "lun %d ", rp->lun);
	p = seprintcmd(p, e, (char*)rp->cmd.p, rp->cmd.count, 1);
	p = seprint(p, e, " [%ld]", rp->data.count);
	if(rp->cmd.write)
		seprintdata(p, e, rp->data.p, rp->data.count);
	fprint(2, "scsi⇒ %s\n", buf);
}
...
scsierr(ScsiReq *rp)
{
	...
	case 0:
		return "";
	case Status_SD:
		return scsierror(rp->sense[12], rp->sense[13]);
	case Status_SW:
...
SRrecover(ScsiReq *rp)
{
	...
	s = "";
	if(rp->umsc && rp->umsc->fs.name)
		s = rp->umsc->fs.name;

	if(rp->cmd.p[0] == ScmdRsense)
		return -1;
	if(SRreqsense(rp) != -1)
		rp->status = Status_SD;
	switch(mkascq(rp) & 0xffff){
	default:
		fprint(2, "usb/disk: ");
		if (rp->unit && rp->unit[0] != '\0')
			fprint(2, "%s: ", rp->unit);
		if (s && *s != '\0')
			fprint(2, "%s: ", s);
		fprint(2, "ascq %#ux\n", mkascq(rp));
		return -1;
	case 0x0402:
		fprint(2, "usb/disk: %s: spundown .. attempt restart\n", s);
		restart = clonereq(rp);
		SRstart(restart, 1);
		status = restart->status;
		free(restart);
		fprint(2, "usb/disk: %s: restart: %d\n", s, status);
		return 0;
	}
}
...
dirdevopen(ScsiReq *rp)
{
	...
	rmb = rp->inquiry[1] & 0x80;
	if(rmb && SRstart(rp, 3) == -1)
		return -1;
	if(SRstart(rp, 1) == -1){
		/*
	...
	if(blocks == 0xffffffff){
		...
		blocks = (uvlong)GETBELONG(data)<<32 | GETBELONG(data + 4);
	}
