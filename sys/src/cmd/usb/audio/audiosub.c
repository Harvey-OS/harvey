...
Namelist terminal_types[] = {{0x100, "USB Terminal, undefined type"},
                             {0x101, "USB Streaming"},
                             {0x201, "Microphone"},
                             {0x301, "Speaker"},
...
audio_interface(Dev*, Desc* dd)
{
	...
	case 1: // control
		...
		case 0x01:
			...
			dprint(
			    2,
			    "\tAudioDevClass release (bcd)%c%c%c%c, "
		...
		case 0x02: // input
			...
			dprint(2,
			       "\tbTerminalId %d, wTerminalType "
			       "0x%x (%s), bAssocTerminal %d bNrChannels %d, "
			       "wChannelConfig %d, iChannelNames %d iTerminal "
			       "%d\n",
			       b[3], b[4] | (b[5] << 8),
		...
		case 0x03: // output
			if(usbdebug) {
				...
				fprint(
				    2, "\tbTerminalId %d, wTerminalType 0x%x "
				       "(%s), bAssocTerminal %d bSourceId %d, "
				       "iTerminal %d\n",
				    b[3], b[4] | (b[5] << 8),
				    namefor(terminal_types, b[4] | (b[5] << 8)),
				    b[6], b[7], b[8]);
				...
				if(usbdebug) {
					fprint(2, "Channels %d, config %d, ",
					       ...
					       b[ctl + 5 + 1] |
					           b[ctl + 5 + 2] << 8);
					x = b[ctl + 5] * b[4];
					fprint(2, "programmable: %d bits, 0x",
					       x);
					x = (x + 7) >> 3;
					while(x--)
						fprint(2, "%2.2x",
		...
		case 0x05:
			...
			if(b[4]) {
				...
				if(u >= 0) {
					units[u][nunits[u]++] = b[3];
					if(selectorid[u] >= 0)
						fprint(2, "Second selector "
						          "(%d, %d) on %s\n",
						       selectorid[u], b[3],
						       u ? "record"
						         : "playback");
					selectorid[u] = b[3];
					controls[u][Selector_control].readable =
					    1;
					controls[u][Selector_control].settable =
					    1;
		...
		case 0x06: // feature
			if(verbose)
				fprint(2, "Audio Feature Unit %d", b[3]);
			dprint(2,
			       "\tbUnitId %d, bSourceId %d, bControlSize %d\n",
			       b[3], b[4], b[5]);
			u = findunit(b[4]);
			if(u >= 0) {
				if(verbose)
				...
				fprint(2, ", not known what for\n");
			p = b + 6;
			for(ctl = 1; ctl < 0x0b; ctl++)
				if((1 << (ctl - 1)) &
				   (b[6] | ((b[5] > 1) ? (b[7] << 8) : 0))) {
					if(verbose)
						fprint(2, "\t%s control on "
						          "master channel\n",
						       controls[0][ctl].name);
					if(u >= 0) {
	...
	case 2: // stream
		switch(b[2]) {
		case 0x01:
			dprint(2, "Audio stream for TerminalID %d, delay %d, "
			          "format_tag %#ux\n",
			       b[3], b[4], b[5] | (b[6] << 8));
			break;
		case 0x02:
			...
			if(verbose) {
				if(b[4] <= 2)
					...
					fprint(2, "Interface %d, %d channels, "
					          "%d bits, ",
					       dd->iface->id, b[4], b[6]);
			}
			if(b[7] == 0) {
				if(verbose)
					fprint(2, "frequency variable between "
					          "%d and %d\n",
					       b[8] | b[9] << 8 | b[10] << 16,
					       b[11] | b[12] << 8 |
					           b[13] << 16);
				...
				if(ch > 1)
					aa->caps |=
					    has_discfreq; /* more than one
					                     frequency */
				else
					aa->caps |=
					    onefreq; /* only one frequency */
			}
			aa->nchan = b[4];
			aa->res = b[6];
			aa->subframesize = b[5];
	...
	default:
		if(usbdebug) {
			hd = hexstr(bb, nb);
			fprint(2,
			       "Unknown audio stream type: CS_INTERFACE: %s\n",
			       hd);
			free(hd);
		}
	}
