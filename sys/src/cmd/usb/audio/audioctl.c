...
Audiocontrol controls[2][Ncontrol] = {
    {
     ...
     //	[Selector_control] = {		"selector",	0, {0}, 0,	0,
     //Undef},
    },
    {
         [Speed_control] = {"speed", 0, {0}, 0, 44100, Undef},
...
setaudioalt(int rec, Audiocontrol* c, int control)
{
	...
	curalt[rec] = control;
	if(usbcmd(ad, Rh2d | Rstd | Riface, Rsetiface, control, interface[rec],
	          nil, 0) < 0) {
...
setspeed(int rec, int speed)
{
	...
	if(a->caps & has_setspeed) {
		dprint(2, "Setting %s speed to %d Hz;",
		       rec ? "record" : "playback", speed);
		buf[0] = speed;
		buf[1] = speed >> 8;
		buf[2] = speed >> 16;
		...
		if(rec)
			n |= 0x80;
		if(usbcmd(ad, Rh2d | Rclass | Rep, Rsetcur,
		          sampling_freq_control << 8, n, buf, 3) < 0) {
			fprint(2, "Error in setupcmd\n");
			return Undef;
		}
		if((n = usbcmd(ad, Rd2h | Rclass | Rep, Rgetcur,
		               sampling_freq_control << 8, n, buf, 3)) < 0) {
		...
		if(n != 3)
			...
			if(buf[2] || n == 0) {
				dprint(2, "Speed out of bounds %d (0x%x)\n", n,
			          ...
			          (ad->usb->did == 0x0223 ||
			           ad->usb->did == 0x07af)) {
				/* Griffin iMic responds incorrectly to sample
				 * rate inquiry */
...
getspeed(int rec, int which)
{
	...
	if(a->caps & has_setspeed) {
		...
		if(rec)
			n |= 0x80;
		r = Rd2h | Rclass | Rep;
		if(usbcmd(ad, r, which, sampling_freq_control << 8, n, buf, 3) <
		   0)
			return Undef;
		if(n == 3) {
			if(buf[2]) {
...
setcontrol(int rec, char* name, int32_t* value)
{
	...
	case Speed_control:
		if((rec != Record || setrec) &&
		   (value[0] = setspeed(rec, value[0])) < 0)
			return -1;
		c->value[0] = value[0];
		return 0;
	...
	case Resolution_control:
		control = findalt(rec, controls[rec][Channel_control].value[0],
		                  value[0], defaultspeed[rec]);
	...
	case Channel_control:
		control = findalt(rec, value[0],
		                  controls[rec][Resolution_control].value[0],
		                  defaultspeed[rec]);
		if(control < 0 || setaudioalt(rec, c, control) < 0) {
			dprint(2, "setcontrol: can't find setting for %s\n",
			       c->name);
	...
	if(c->chans) {
		for(i = 1; i < 8; i++)
			if(c->chans & 1 << i) {
				...
				if(usbcmd(ad, type, req, control | i, index,
				          buf, count) < 0) {
					dprint(
					    2,
					    "setcontrol: setupcmd %s failed\n",
	...
	if(controls[rec][ctl].chans) {
		...
		for(i = 1; i < 8; i++) {
			...
			if(controls[rec][ctl].chans & 1 << i) {
				n = usbcmd(ad, type, req, control | i, index,
				           buf, count);
				if(n < 0)
					return Undef;
				if(n != count)
			...
			if(c->readable) {
				if(verbose)
					fprint(2, "%s %s control",
					       rec ? "Record" : "Playback",
					       controls[rec][ctl].name);
				c->min = (getspecialcontrol(rec, ctl, Rgetmin,
				                            v) < 0)
				             ? Undef
				             : v[0];
				if(verbose && c->min != Undef)
					fprint(2, ", min %ld", c->min);
				c->max = (getspecialcontrol(rec, ctl, Rgetmax,
				                            v) < 0)
				             ? Undef
				             : v[0];
				if(verbose && c->max != Undef)
					fprint(2, ", max %ld", c->max);
				c->step = (getspecialcontrol(rec, ctl, Rgetres,
				                             v) < 0)
				              ? Undef
				              : v[0];
				if(verbose && c->step != Undef)
					fprint(2, ", step %ld", c->step);
				if(getspecialcontrol(rec, ctl, Rgetcur,
				                     c->value) == 0) {
					if(verbose) {
						if(c->chans) {
							fprint(2, ", values");
...
Aconv(Fmt* fp)
{
	...
	if(c->chans) {
		...
		for(i = 1; i < 8; i++)
			if(c->chans & 1 << i) {
				p = seprint(p, str + sizeof str, "%s%ld",
				            fst ? "'" : " ", c->value[i]);
				fst = 0;
			}
