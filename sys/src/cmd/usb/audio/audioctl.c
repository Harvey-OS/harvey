...
findalt(int rec, int nchan, int res, int speed)
{
	...
	for(i = 0; i < nelem(ad->usb->ep); i++) {
		...
		if(ep->iface->csp != CSP(Claudio, 2, 0))
			continue;
		if((rec == Play && (ep->addr & 0x80)) || (rec == Record && (ep->addr & 0x80) == 0))
			continue;
		for(j = 0; j < 16; j++) {
...
setspeed(int rec, int speed)
{
	...
	ps = ((speed * da->interval + 999) / 1000) * controls[rec][Channel_control].value[0] * controls[rec][Resolution_control].value[0] / 8;
	if(ps > ep->maxpkt) {
