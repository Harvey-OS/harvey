...
enum {
	...
	FTELVEM1000DLDid =
	    0xE0F1, /* PC-Datenlogger fuer Energiemonitor (EM 1000 DL) */
...
enum { PITDEFAULT = 0, /* SIOA */
       PITA,           /* SIOA jtag if there is one */
};

enum { Rftdireq = 1 << 6, /* bit for type of request */
};
 ...
 */
enum { BMSERIAL = 0, /* reset, turn off bit-bang mode */
