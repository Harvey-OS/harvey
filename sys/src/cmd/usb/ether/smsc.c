...
wr(Dev* d, int reg, int val)
{
	...
	ret = usbcmd(d, Rh2d | Rvendor | Rdev, Writereg, 0, reg, (uint8_t*)&val,
	             sizeof(val));
	if(ret < 0)
...
rr(Dev* d, int reg)
{
	...
	ret = usbcmd(d, Rd2h | Rvendor | Rdev, Readreg, 0, reg, (uint8_t*)&rval,
	             sizeof(rval));
	if(ret < 0) {
...
smscinit(Ether* ether)
{
	...
	wr(d, Coecr,
	   rr(d, Coecr) & ~(Txcoe | Rxcoe)); /* TODO could offload checksums? */
...
smscbread(Ether* e, Buf* bp)
{
	...
	if(rbp->ndata < 4) {
		rbp->rp = rbp->data;
		rbp->ndata = read(e->epin->dfd, rbp->rp,
		                  Doburst ? Hsburst * 512 : Maxpkt);
		if(rbp->ndata < 0)
			return -1;
	}
	if(rbp->ndata < 4) {
	...
	for(ip = cinfo; ip->vid != 0; ip++)
		if(ip->vid == dev->usb->vid && ip->did == dev->usb->did) {
			...
			if(Doburst) {
				...
				ether->aux = emallocz(
				    sizeof(Buf) + ether->bufsize - Maxpkt, 1);
			} else {
