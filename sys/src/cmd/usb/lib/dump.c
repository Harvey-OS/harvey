...
static char* cnames[] = {"none", "audio",   "comms",   "hid", "",    "",
                         "",     "printer", "storage", "hub", "data"};
static char* devstates[] = {"detached", "attached", "enabled", "assigned",
                            "configured"};
...
seprintiface(char* s, char* e, Iface* i)
{
	...
	for(j = 0; j < Naltc; j++) {
		...
		if(a == nil)
			break;
		s = seprint(s, e, "\t\t  alt %d attr %d ival %d", j, a->attrib,
		            a->interval);
		if(a->aux != nil)
	...
	for(j = 0; j < Nep; j++) {
		...
		s = seprint(s, e, "\t\t  ep id %d addr %d dir %s type %s"
		            ...
		            ep->id, ep->addr, eds, ets, ep->isotype, ep->maxpkt,
		            ep->ntds);
	}
	return s;
}
...
Ufmt(Fmt* f)
{
	...
	s = seprint(s, e, " csp %s.%uld.%uld", classname(Class(ud->csp)),
	            Subclass(ud->csp), Proto(ud->csp));
