...
char* schedstatename[] = {
        [SAdmit] = "Admit",       [SSleep] = "Sleep",     [SDead] = "Dead",
        [SDeadline] = "Deadline", [SEdf] = "Edf",         [SExpel] = "Expel",
        [SReady] = "Ready",       [SRelease] = "Release", [SRun] = "Run",
        [SSlice] = "Slice",       [SInts] = "Ints",       [SInte] = "Inte",
        [SUser] = "User",         [SYield] = "Yield",
};
...
usage(void)
{
	fprint(
	    2,
	    "Usage: %s [-d profdev] [-w] [-v] [-t triggerproc] [processes]\n",
	    argv0);
	exits(nil);
}
...
threadmain(int argc, char** argv)
{
	...
	for(i = 0; i < argc; i++) {
		snprint(fname, sizeof fname - 2, "/proc/%s/ctl", argv[i]);
...
{
	mediumfont =
	    openfont(display, "/lib/font/bit/lucidasans/unicode.10.font");
	if(mediumfont == nil)
		mediumfont = font;
	...
	/* Grey */
	cols[5][0] =
	    allocimage(display, Rect(0, 0, 1, 1), screen->chan, 1, 0xEEEEEEFF);
	cols[5][1] =
	    allocimage(display, Rect(0, 0, 1, 1), screen->chan, 1, 0xCCCCCCFF);
	cols[5][2] =
	    allocimage(display, Rect(0, 0, 1, 1), screen->chan, 1, 0x888888FF);
	cols[5][3] =
	    allocimage(display, Rect(0, 0, 1, 1), screen->chan, 1, 0xAAAAAAFF);
	grey = cols[5][2];
	red =
	    allocimage(display, Rect(0, 0, 1, 1), screen->chan, 1, 0xFF0000FF);
	green =
	    allocimage(display, Rect(0, 0, 1, 1), screen->chan, 1, 0x00FF00FF);
	blue =
	    allocimage(display, Rect(0, 0, 1, 1), screen->chan, 1, 0x0000FFFF);
	bg = display->white;
	fg = display->black;
}
...
#define time2x(t) ((int)(((t)-oldestts) / ppp))
	...
	// time2x(oldestts), time2x(newestts));
	if(prevts < oldestts) {
		oldestts = newestts - period;
		...
		rtime = screen->r;
		rtime.min.x =
		    rtime.max.x - stringwidth(mediumfont, "00000000000.000s");
	...
	for(n = 0; n != ntasks; n++) {
		...
		line(screen, addpt(p, Pt(x, Height - lineht)),
		     Pt(screen->r.max.x, p.y + Height - lineht), Endsquare,
		     Endsquare, 0, cols[n % Ncolor][1], ZP);
		...
		for(i = 0; i != t->nevents; i++) {
			...
			case SAdmit:
				if(e->time > prevts && e->time <= newestts) {
					sx = time2x(e->time);
					line(screen,
					     addpt(p, Pt(sx, topmargin)),
					     addpt(p, Pt(sx, Height -
					                         bottommargin)),
			...
			case SUser:
				if(e->time > prevts && e->time <= newestts) {
					sx = time2x(e->time);
					line(
					    screen, addpt(p, Pt(sx, topmargin)),
					    addpt(p, Pt(sx,
					                Height - bottommargin)),
					    Endsquare, Endarrow, 0,
					    (e->etype == SYield) ? green : blue,
					    ZP);
				}
				break;
			case SSlice:
			...
			case SEdf:
				...
				r = Rect(sx, topmargin + 8, ex,
				         Height - lineht);
				r = rectaddpt(r, p);

				draw(screen, r,
				     cols[n % Ncolor][e->etype == SRun ? 1 : 3],
				     nil, ZP);
	...
	ts = prevts + scales[scaleno].littletics -
	     (prevts % scales[scaleno].littletics);
	x = time2x(ts);
	...
	while(x < Width) {
		...
		for(n = 0; n < ntasks; n++) {
			...
			line(screen, addpt(p, Pt(x, height)),
			     addpt(p, Pt(x, Height - lineht)), Endsquare,
			     Endsquare, width, cols[n % Ncolor][2], ZP);
	...
	draw(screen, rtime, bg, nil, ZP);
	ts = oldestts + scales[scaleno].bigtics -
	     (oldestts % scales[scaleno].bigtics);
	x = time2x(ts);
	ss = 0;
	while(x < Width) {
		snprint(buf, sizeof(buf), "%t", ss);
		string(screen, addpt(p, Pt(x - stringwidth(tinyfont, buf) / 2,
		                           -tinyfont->height - 1)),
		       fg, ZP, tinyfont, buf);
	...
	string(screen, Pt(screen->r.max.x - stringwidth(mediumfont, buf),
	                  screen->r.min.y),
...
doevent(Task* t, Traceevent* ep)
{
	...
	case SDead:
		print("task died %ld %t %s\n", event->pid, event->time,
		      schedstatename[event->etype & 0xffff]);
		free(t->events);
		free(t->name);
		ntasks--;
		...
		if(newwin)
			...
			Height =
			    ntasks ? Dy(screen->r) / ntasks : Dy(screen->r);
		prevts = 0;
	}
}
...
drawtrace(void)
{
	...
	if(newwin) {
		if((wsys = getenv("wsys")) == nil)
			sysfatal("%s: Cannot find windowing system: %r", argv0);

		if((wfd = open(wsys, ORDWR)) < 0)
			sysfatal("%s: Cannot open windowing system: %r", argv0);
	...
	for(;;) {
		...
		case 3:
			now = nsec();
			while((n = read(logfd, eventbuf,
			                Nevents * sizeof(Traceevent))) > 0) {
				assert((n % sizeof(Traceevent)) == 0);
				nevents = n / sizeof(Traceevent);
				for(ep = eventbuf; ep < eventbuf + nevents;
				    ep++) {
					if((ep->etype & 0xffff) >= Nevent) {
						print("%ld %t Illegal event "
						      "%ld\n",
						      ep->pid, ep->time,
						      ep->etype & 0xffff);
						continue;
...
timeconv(Fmt* f)
{
	...
	if(t > S(1)) {
		t += OneRound;
		sprint(buf, "%s%d.%.3ds", sign, (int)(t / S(1)),
		       (int)(t % S(1)) / 1000000);
	} else if(t > MS(1)) {
		t += MilliRound;
		sprint(buf, "%s%d.%.3dms", sign, (int)(t / MS(1)),
		       (int)(t % MS(1)) / 1000);
	} else if(t > US(1))
