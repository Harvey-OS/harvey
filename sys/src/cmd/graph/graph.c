...
struct {
	char* name;
	int next;
} palette[] = {['b']{"blue", 'b'},   ['c']{"cyan", 'c'},    ['g']{"green", 'g'},
               ['k']{"kblack", 'k'}, ['m']{"magenta", 'm'}, ['r']{"red", 'r'},
               ['w']{"white", 'w'},  ['y']{"yellow", 'y'}};
int pencolor = 'k';
...
readin(void)
{
	...
	for(;;) {
		temp = (struct val*)realloc((char*)xx, (unsigned)(n + ovlay) *
		                                           sizeof(struct val));
		if(temp == 0)
			return;
		xx = temp;
...
axlab(char c, struct xy* p, char* b)
{
	char* dir;
	dir = p->xlb < p->xub ? "<=" : ">=";
	sprintf(b + strlen(b), " %g %s %c%s %s %g", p->xlb / p->xmult, dir, c,
	        p->xf == log10 ? " (log)" : "", dir, p->xub / p->xmult);
}
