 ...
 * handled by dpost. The program is an attempt to address requirements,
 *expressed
 ...
 * on the line is taken to be a picture file pathname that's added (in
 *transparent
 * mode) to the output file. In addition each in-line picture file is preceeded
 *by
 ...
 * and filename is the pathname of the picture file. dpost uses both arguments
 *to
 * manage in-line pictures (in a big temp file). To handle pictures in
 *diversions
 * picpack reads each input file twice. The first pass looks for picture
 *inclusion
 ...
 *
 * The recognized in-line picture requests are saved in keys[] and by default
 *only
 ...
 * .BP or .PI or decided to list each picture file at the start of your input
 *file
 ...
 *
 * hits on lines that begin with .iP (rather than .BP or .PI), and the only
 *files
 * pulled in would be ones named as the second argument to the new .iP macro.
 *The
 * -k option accepts a space or comma separated list of up to 10 different key
 * strings. picpack imposes no contraints on key strings, other than not
 *allowing
 ...
 * included) don't fit the device independent language accepted by important
 *post
 * processors like proof, and that means you won't be able to reliably preview a
 * packed file on your 5620 or whatever. Another potential problem is that
 *picture
...

{
	...
	/*
	 *
	 * A picture packing pre-processor that copies input files to stdout,
	 *expanding
	 * picture requests (as listed in keys[]) to an in-line format that can
	 *be passed
...

{
	...
	/*
	 *
	 * Separates *list into space or comma separated strings and adds each
	 *to the
	 * keys[] array. The strings in keys[] are used to locate the picture
	 *inclusion
	 * requests that are translated to the in-line format. The keys array
	 *must end
	 * with a NULL pointer and by default only expands .BP and .PI macro
	 *calls.
	 *
	 */
...

{
	...
	/*
	 *
	 * Copies stdin to a temp file, unlinks the file, and returns the file
	 *pointer
	 * for the new temporary file to the caller. Needed because we read each
	 *input
...

{
	...
	/*
	 *
	 * Copies file fd_in to fd_out. Handles the second pass for each input
	 *file and
...

{
	...
	/*
	 *
	 * Finished with all the input files - unlink the temporary file that
	 *was used
...

{
	...
	/*
	 *
	 * Handles the two passes over the next input file. First pass compares
	 *the start
	 * of each line in *fp_in with the key strings saved in the keys[]
	 *array. If a
	 * match is found inline() is called to copy the picture file (ie. the
	 *file named
	 * as the second string in line[]) to stdout, provided the file hasn't
	 *previously
	 * been copied. The second pass goes back to the start of fp_in and
	 *copies it all
	 * to the output file.
	 *
	 */
...

{
	...
	/*
	 *
	 * Copies the picture file *name to the output file in an in-line format
	 *that can
	 * be passed through troff and recovered later by dpost. Transparent
	 *mode is used
	 * so each line starts with \! and all \ characters must be escaped. The
	 *in-line
	 * picture sequence begins with an "x X InlinePicture" device control
	 *command that
...

{
	...
	/*
	 *
	 * Checks the list of previously added picture files in *temp_file and
	 *returns
	 * FALSE if it's a new file and TRUE otherwise. Probably should open the
	 *temp
	 * file once for update and leave it open, rather than opening and
	 *closing it
	 * every time.
	 *
	 */
...

{
	...
	/*
	 *
	 * Adds string *name to the list of in-line picture files that's
	 *maintained in
	 * *temp_file. Should undoubtedly open the file once for update and use
	 *fseek()
