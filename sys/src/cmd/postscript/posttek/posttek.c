 ...
 * Most of the code was borrowed from the tektronix 4014 emulator that was
 *written
 ...
 *	  mark array l mark
 *
 *	    Set the line drawing mode according to the description given in
 *array.
 ...
 *	  n w -
 *
 *	    Adjusts the line width for vector drawing. Used to select normal
 *(n=0)
 ...
 *	  size f -
 *
 *	    Changes the size of the font that's used to print characters in
 *alpha
 ...
 * works well on 'write to black' engines but won't be right for 'write to
 *white'
...

{
	...
	/*
	 *
	 * A simple program that can be used to translate tektronix 4014 files
	 *into
	 * PostScript. Most of the code was taken from the DMD tektronix 4014
	 *emulator,
...

{
	...
	/*
	 *
	 * Controls the translation of the next input file. Tektronix states
	 *(dispmode)
...

{
	...
	/*
	 *
	 * Called to reset things, typically only at the beginning of each input
	 *file.
	 *
	 */
...

{
	...
	/*
	 *
	 * Special point mode permits gray scaling by varying the size of the
	 *stored
	 * point, which is controlled by an intensity character that preceeds
	 *each point
	 * address.
	 *
	 */
...

{
	...
	/*
	 *
	 * Handles incremental plot mode. It's entered after the RS control code
	 *and is
	 * used to mark points relative to our current position. It's typically
	 *followed
	 * by one or two bytes that set the pen state and are used to increment
	 *the
	 * current position.
	 *
	 */
...

{
	...
	/*
	 *
	 * All we really have to do for GIN mode is make sure it's properly
	 *ended.
	 *
	 */
...

{
	...
	/*
	 *
	 * Checks character c and does special things, like mode changes, that
	 *depend
	 * not only on the character, but also on the current state. If the mode
	 *changed
	 * becuase of c, OUTMODED is returned to the caller. In all other cases
	 *the
...

{
	...
	/*
	 *
	 * Handles tektronix escape code. Called from control() whenever an ESC
	 *character
...

{
	...
	/*
	 *
	 * Makes sure the current mode is properly ended and then sets dispmode
	 *to mode.
	 *
	 */
...

{
	...
	/*
	 *
	 * Makes sure the cursor is positioned at the upper left corner of the
	 *page.
	 *
	 */
...

{
	...
	/*
	 *
	 * Generates the call to the procedure that's responsible for changing
	 *the
...

{
	...
	/*
	 *
	 * Called whenever we need to draw a vector or plot a point. Nothing
	 *will be
...

{
	...
	/*
	 *
	 * Usually called when we've finished the last page and want to get
	 *ready for the
...

{
	...
	/*
	 *
	 * Reads the next character from the current input file and returns it
	 *to the
	 * caller. When we're finished with the file dispmode is set to EXIT and
	 *OUTMODED
