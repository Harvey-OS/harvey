 ...
 *
 * A simple program that can be used to print DMD bitmaps on PostScript
 *printers.
 ...
 * the bitfile format begins with a 10 byte header with the first two bytes set
 *to
 * zero. The next 8 bytes set the x and y coordinates of the bitmap's origin and
 * corner (ie. the upper left and lower right corners). The compressed raster
 *data
 * follows the header and consists of control bytes followed an appropriate
 *number
 * of data bytes. Control bytes (ie. n) less than 127 means read the next 2*n
 *bytes
 ...
 *
 * After each raster line is recovered postdmd encodes it in a slightly
 *different
 * format that's designed to be unpacked by a PostScript procedure that's
 *defined
 * in the prologue. By default no exclusive-or'ing is done and packing of
 *pattern
 * data can be based on any number of bytes rather than just the next two bytes.
 * By default 6 byte patterns are used, but any number can be selected with the
 *-b
 * option. A non-positive argument (eg. -b0) disables all pattern encoding.
 *Larger
 * patterns increase the size of the output file, but reduce the work load
 *that's
 * forced on the PostScript interpreter. The default choices I've made (ie. 6
 *byte
 ...
 *
 * where bytes and count are decimal integers and patterns is a hex string.
 *Bytes
 * is the number of bytes represented by the hex patterns and count is the
 *number
 ...
 *
 * all represent 10 consecutive bytes of ones. Scanlines are terminated by a 0
 *on
 * a line by itself.
 *
 ...
 *
 *	    Prints the bitmap that's read from standard input. The bitmap
 *consists
 *	    of scanlines lines, each of which includes scanlength pixels. If
 *	    v8format is true the picture is assumed to be an Eighth Edition
 *bitmap,
...

{
	...
	/*
	 *
	 * A simple program that translates DMD bitmap files into PostScript.
	 *There can
	 * be more than one bitmap per file, but none can be split across input
	 *files.
...

{
	...
	fprintf(stdout, "%s 0 0 %d %d\n", BOUNDINGBOX,
	        (bbox[0] * 72 + 100) / 100, (bbox[1] * 72 + 100) / 100);
	fprintf(stdout, "%s %d\n", PAGES, printed);
...

{
	...
	/*
	 *
	 * Reads all the bitmaps from the next input file, translates each one
	 *into
	 * PostScript, and arranges to have one bitmap printed on each page.
	 *Multiple
	...
	while(dimensions() == TRUE) {
		...
		bbox[0] =
		    MAX(bbox[0], patterns * 16); /* for BoundingBox comment */
		...
		fprintf(fp_out, "%s ", (v8format == TRUE && v8undo == FALSE)
		                           ? "true"
		                           : "false");
		...
		if(debug == ON)
			fprintf(
			    stderr,
			    "patterns = %d, scanlines = %d, patcount = %d\n",
...

{
	...
	/*
	 *
	 * Determines the dimensions and type of the next bitmap. Eighth edition
	 *bitmaps
	 * have a zero in the first 16 bits. If valid dimensions are read TRUE
	 *is returned
	 * to the caller. Changed so the check of whether we're done (by testing
	 *scanlines
...

{
	...
	/*
	 *
	 * Reads the input file and adds the appropriate number of bytes to the
	 *output
	 * raster line. If count has bit 7 on, one 16 bit pattern is read and
	 *repeated
	 * count & 0177 times. If bit 7 is off, count is the number of patterns
	 *read from
...

{
	...
	/*
	 *
	 * Compares the two n byte patterns *p1 and *(p1+n). FALSE is returned
	 *if they're
...

{
	...
	/*
	 *
	 * Reads the next two bytes from *fp_in and returns the resulting
	 *integer.
	 *
	 */
