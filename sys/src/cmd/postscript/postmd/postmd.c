 ...
 *
 * A simple program that can be used to display a matrix as a gray scale image
 *on
 * a PostScript printer using the image operator. Much of the code was borrowed
 * from postdmd, the bitmap display program DMD screen dumps. May help if you
 *have
 ...
 * file in row major order. The actual matrix elements can be preceeded by a
 *simple
 * header that sets things like the matrix dimensions, interval list, and
 *possibly
 * a window into the matrix that we'll use for display. The dimension statement
 *is
 ...
 * matrix elements to integers is accomplished using an interval list that can
 *be
 * set using the -i option. The format of the interval string is,
 *
 *		num1,num2,num3,...,numn
 *
 * where each num is a floating point number. The list must be given in
 *increasing
 * numerical order. A list of n numbers partitions the real line into 2n+1
 *regions
 * given as,
 *
 ...
 *
 * Every number in a region is mapped one integer in the range 0 to 254, and
 *that
 * number, when displayed on a printer using the image operator, prints as a
 *square
 ...
 *
 * The size of the matrix that we can display reasonably well is a function of
 *the
 * number of elements in the interval list, paper size, and printer resolution.
 * For example a 300dpi printer using 8.5x11 inch paper gives us an image area
 *of
 * about 2400x2400 pixels. An interval list of two numbers generates five
 *separate
 * regions and will therefore need that many different shades of gray. Since
 *we're
 * not using white we'll need to partion our image area into 4x4 pixel squares,
 * and that means a 600x600 matrix is about as big as we can go. In practice
 *that's
 * optimistic, but the argument illustrates some of the limitations.
 *
 * A submatrix can be selected to display by windowing into the matrix. The
 *window
 * list can be given using the -w option or can be set in the optional header
 *that
 ...
 *
 * where each element in the list must be a positive integer. Rows and columns
 *in
 * the input matrix start at 1. The dimension of the displayed window will be
 *from
 ...
 * by postdmd. See the comments at the beginning of that program if you need
 *more
 ...
 *	  mark ... setup -
 *
 *	    Handles special initialization stuff that depends on how this
 *program
 ...
 *	  columns rows bitmap -
 *
 *	    Prints the image that's read as a hex string from standard input.
 *The
 *	    image consists of rows lines, each of which includes columns
 *elements.
 ...
 *
 *	    Prints string matrixname just below the lower left corner of the
 *image
 *	    and prints string martixlimits near the lower right corner. Outlines
 *	    the entire image with a (one pixel wide) box and then draws tick
 *marks
 ...
 *
 *	    Prints the legend as a bar graph below the matrix image. n1 ... nN
 *are
 ...
 *	    the c's are trivial procedures that just leave a one character
 *string
...

{
	...
	/*
	 *
	 * Bitmap display program for matrices. Only one matrix is allowed per
	 *input file,
	 * and each one will be displayed on a page by itself. Input files
	 *consist of an
	 * optional header followed by floating point numbers that represent the
	 *matrix
...

{
	...
	/*
	 *
	 * Reads and processes the command line options. Added the -P option so
	 *arbitrary
	 * PostScript code can be passed through. Expect it could be useful for
	 *changing
...

{
	...
	/*
	 *
	 * Finished with all the input files, so mark the end of the pages, make
	 *sure the
...

{
	...
	/*
	 *
	 * Reads a matrix from *fp_in, translates it into a PostScript gray
	 *scale image,
	 * and writes the result on stdout. For now only one matrix is allowed
	 *per input
	 * file. Matrix elements are floating point numbers arranged in row
	 *major order
	 * in the input file. In addition each input file may contain an
	 *optional header
	 * that defines special things like the dimension of the matrix, a
	 *window into
	 * the matrix that will be displayed, and an interval list.
	 *
	 * If we're reading from stdin we first make a copy in a temporary file
	 *so we can
	 * can properly position ourselves after we've looked for the header.
	 *Originally
	 * wasn't always making a copy of stdin, but I've added a few things to
	 *what's
	 * accepted in the header and this simplifies the job. An alternative
	 *would be
	 * to always require a header and mark the end of it by some string.
	 *Didn't like
...

{
	...
	/*
	 *
	 * If we're reading the matrix from stdin and the matrix dimension isn't
	 *set by
	 * a dimension statement at the beginning of the file we'll copy stdin
	 *to a
	 * temporary file and reset *fp_in so reads come from the temp file.
	 *Simplifies
...

{
	...
	/*
	 *
	 * Looks for the optional header information at the beginning of the
	 *input file,
	 * reads it if it's there, and sets *fp_in to be just past the header.
	 *That should
	 * be the beginning of the matrix element list. The recognized header
	 *keywords are
	 * dimension, interval, colormap (or grayscale), window, name, and
	 *statistics. All
	 * are optional, but may be useful in a spooling environment when the
	 *user doesn't
	 * doesn't actually run the translator.
	 *
	 * The dimension statement specifies the number of rows and columns. For
	 *example
	 ...
	 *	dimension	50x50
	 *
	 * The first integer is the number of rows and the second, if given, is
	 *the number
	 * of columns. If columns are missing from the dimension statement we
	 *assume the
	 * matrix is square.
	 *
	 * interval can be used to redefine the interval list used for mapping
	 *floating
	 * point numbers into integers in the range 0 to 254. The string
	 *following the
	 * interval keyword has the same format as the -i option. For example to
	 *set the
	 * interval list to -1, 0, and 1 you can add the line,
	 *
	 *	interval	-1,0,1
	 *
	 * The numbers are floats given in increasing order, and separated by
	 *commas or
	 * blanks. The last interval list in a header takes precedence.
	 *
	 * colormap can be used to redefine the grayscale list.  The string
	 *following
	 * the colormap keyword has the same format as the -g option.  For
	 *example
	 *
	 *	colormap	0,50,100,150,200,250
	 * or	grayscale	0,50,100,150,200,250
	 *
	 * The window keyword can be used to select a submatrix. The numbers
	 *following
	 * window are the upper left and lower right matix coordinates. May not
	 *be
	 * implemented yet but shouldn't be difficult. For example
	 *
	 *	window		10 10 40 40
	 *
	 * selects the submatrix with corners at (10, 10) and (40, 40). The
	 *edges of the
	 * window are included in the display.
	 *
	 * The name keyword can be used to define the title of the display.  For
	 *example,
	 *
	 *	name		Plot Of Matrix 1
	 *
	 * prints the string "Plot Of Matrix 1" at the top of the page.
	 *Everything up to
	...
	while(fscanf(fp_in, "%s", buf) != EOF) {
		if(strncmp(buf, "dimension", strlen("dimension")) == 0)
			...
			fgets(buf, sizeof(buf), fp_in);
			buildilist(buf);
		} else if(strncmp(buf, "statistics", strlen("statistics")) ==
		          0) {
			fscanf(fp_in, "%s", buf);
...

{
	...
	/*
	 *
	 * Need to know the dimensions of the matrix before we can go any
	 *farther. If
	 * rows and columns are still 0 we'll read the entire input file,
	 *starting from
	 * the current position, count the number of elements, take the square
	 *root of it,
	 * and use it as the number of rows and columns. Then we seek back to
	 *the start
	 * of the real matrix, make sure columns is set, and allocate enough
	 *memory for
	 * storing each raster line. After we're certain we've got the number of
	 *rows and
	 * columns we check the window coordinates, and if they're not
	 *legitimate they're
...

{
	...
	/*
	 *
	 * Reads string *list and builds up the ilist[] that will be used in the
	 *next
	 * matrix. Since strtok() modifies the string it's parsing we make a
	 *copy first.
	 * The format of the interval list is described in detail in the
	 *comments at the
	 * beginning of this program. Basically consists of a comma or space
	 *separated
	 * list of floating point numbers that must be given in increasing
	 *numerical order.
	 * The list determines how floating point numbers are mapped into
	 *integers in the
	 * range 0 to 254.
	 *
	 */
...

{
	...
	/*
	 *
	 * Assigns the integers in *list to the color field for the regions
	 *defined in
...

{
	...
	return ((c >= wlist[0]) && (r >= wlist[1]) && (c <= wlist[2]) &&
	        (r <= wlist[3]));
...

{
	...
	/*
	 *
	 * Checks if the current row lies in the window. Used right before we
	 *output the
	 * raster lines.
	 *
	 */

	return (((patcount / columns) >= wlist[1]) &&
	        ((patcount / columns) <= wlist[3]));
...

{
	...
	/*
	 *
	 * Maps element into an integer in the range 0 to 255, and returns the
	 *result to
	 * the caller. Mapping is done using the color map that was saved in
	 *ilist[]. Also
	 * updates the count field for the region that contains element - not
	 *good!
	 *
	 */
...

{
	...
	/*
	 *
	 * Takes the scanline that's been saved in *raster, encodes it according
	 *to the
	 * value that's been assigned to bytespp, and writes the result to
	 **fp_out. Each
...

{
	...
	/*
	 *
	 * Responsible for generating the PostScript calls that label the
	 *matrix, generate
	...
	for(i = regions - 1; i >= 0; i--)
		fprintf(fp_out, "{(\\%.3o)} %d ", ilist[i].color,
		        ilist[i].count);
...

{
	...
	/*
	 *
	 * Compares the two n byte patterns *p1 and *(p1+n). FALSE if returned
	 *is they're
...

{
	...
	/*
	 *
	 * Copies string *str to a permanent place and returns the address to
	 *the caller.
	 *
	 */
