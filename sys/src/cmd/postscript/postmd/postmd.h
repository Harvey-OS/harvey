 ...
 * 254 representing shades of gray on a PostScript printer. The list can be
 *given
 * using the -i option or can be set in the optional header that can preceed
 *each
 ...
 *
 * Every number in a given region is mapped into an integer in the range 0 to
 *254
 * and that number, when displayed on a PostScript printer using the image
 *operator,
 * prints as a square filled with a gray scale that reflects the integer that
 *was
 * chosen. 0 maps to black and 255 white (that's why 255 is normally omitted).
 *
 * The shades of gray chosen by the program are normally generated
 *automatically,
 ...
 * integers between 0 and 255 that's used to map individual regions into
 *arbitray
 * shade of gray, thus overriding the default choice made in the program. The
 *list
 * should look like,
 *
 ...
 * than 2n+1 numbers are given the default assignments will be used for the
 *missing
 * regions. Each color must be an integer in the range 0 to 255.
 *
 * The default interval list is given below. The default grayscale maps 254
 *(almost
...
/*
 *
 * The active interval list is built from an interval string and stored in an
 *array
