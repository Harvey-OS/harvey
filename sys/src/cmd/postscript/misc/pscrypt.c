...

{
	...
	while(lastchar != EOF) {
		...
		key = (key + (mode == DECRYPT ? cypher : clear)) * MAGIC1 +
		      MAGIC2;
		if(++outoffset > 0 && lastchar != EOF) {
			if(output == HEX) {
				printf("%.2X", clear);
				if(linelength > 0 &&
				   (outoffset % linelength) == 0)
					putchar('\n');
			} else
				putchar(clear);
		} /* End if */
...

{
	...
	/*
	 *
	 * Returns the next byte. Uses cryptkey (i.e. what followed -e) while
	 *inoffset is
...

{
	...
	/*
	 *
	 * Reads the next byte from *fp, sets lastchar, and returns the
	 *character.
	 *
	 */
