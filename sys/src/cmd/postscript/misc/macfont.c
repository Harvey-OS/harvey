...

{
	...
	/*
	 *
	 * The first four bytes (in a block) are the block size, the fifth is
	 *the block
	 * type, and the sixth always appears to be NULL. Type 0 blocks are
	 *comments and
	 * are always skipped. Type 1 blocks are ASCII text, type 2 is binary
	 *data that
	 * should be converted to hex, while type 5 blocks represent the end of
	 *the font
	 * file. Commment block lengths appear to be from the first byte, while
	 *other
	 * lengths seem to be measured from block type byte (ie. the fifth
	 *byte). Type
	 * four blocks aren't used, while type 3 blocks mean an end of file
	 *indication
	 * should be sent to the printer. Haven't done anything with type 3
	 *blocks.
	 *
	 */
	...
	while(1) {
		...
		case 3:
		case 4:
			error(FATAL, "resource type %d not implemented",
			      blocktype);
			break;
...

{
	...
	/*
	 *
	 * Handles type 1 (ie. ASCII text) blocks. Changing carriage returns to
	 *newlines
	 * is all I've done.
	 *
	 */
...

{
	...
	/*
	 *
	 * Reads the next count bytes and converts each byte to hex. Also starts
	 *a new
...

{
	...
	/*
	 *
	 * Reads the next four bytes into an integer and returns the value to
	 *the caller.
