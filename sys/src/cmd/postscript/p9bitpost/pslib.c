...
preamble(Biobuf* ioutb, Rectangle bb)
{
	...
	Bprint(ioutb, "  ffffff ffffaa ffff55 ffff00 ffaaff ffaaaa ffaa55 "
	              "ffaa00 ff55ff ff55aa ff5555 ff5500\n");
	Bprint(ioutb, "  ff00ff ff00aa ff0055 ff0000 ee0000 eeeeee eeee9e "
	              "eeee4f eeee00 ee9eee ee9e9e ee9e4f\n");
	Bprint(ioutb, "  ee9e00 ee4fee ee4f9e ee4f4f ee4f00 ee00ee ee009e "
	              "ee004f dd0049 dd0000 dddddd dddd93\n");
	Bprint(ioutb, "  dddd49 dddd00 dd93dd dd9393 dd9349 dd9300 dd49dd "
	              "dd4993 dd4949 dd4900 dd00dd dd0093\n");
	Bprint(ioutb, "  cc0088 cc0044 cc0000 cccccc cccc88 cccc44 cccc00 "
	              "cc88cc cc8888 cc8844 cc8800 cc44cc\n");
	Bprint(ioutb, "  cc4488 cc4444 cc4400 cc00cc aaffaa aaff55 aaff00 "
	              "aaaaff bbbbbb bbbb5d bbbb00 aa55ff\n");
	Bprint(ioutb, "  bb5dbb bb5d5d bb5d00 aa00ff bb00bb bb005d bb0000 "
	              "aaffff 9eeeee 9eee9e 9eee4f 9eee00\n");
	Bprint(ioutb, "  9e9eee aaaaaa aaaa55 aaaa00 9e4fee aa55aa aa5555 "
	              "aa5500 9e00ee aa00aa aa0055 aa0000\n");
	Bprint(ioutb, "  990000 93dddd 93dd93 93dd49 93dd00 9393dd 999999 "
	              "99994c 999900 9349dd 994c99 994c4c\n");
	Bprint(ioutb, "  994c00 9300dd 990099 99004c 880044 880000 88cccc "
	              "88cc88 88cc44 88cc00 8888cc 888888\n");
	Bprint(ioutb, "  888844 888800 8844cc 884488 884444 884400 8800cc "
	              "880088 55ff55 55ff00 55aaff 5dbbbb\n");
	Bprint(ioutb, "  5dbb5d 5dbb00 5555ff 5d5dbb 777777 777700 5500ff "
	              "5d00bb 770077 770000 55ffff 55ffaa\n");
	Bprint(ioutb, "  4fee9e 4fee4f 4fee00 4f9eee 55aaaa 55aa55 55aa00 "
	              "4f4fee 5555aa 666666 666600 4f00ee\n");
	Bprint(ioutb, "  5500aa 660066 660000 4feeee 49dddd 49dd93 49dd49 "
	              "49dd00 4993dd 4c9999 4c994c 4c9900\n");
	Bprint(ioutb, "  4949dd 4c4c99 555555 555500 4900dd 4c0099 550055 "
	              "550000 440000 44cccc 44cc88 44cc44\n");
	Bprint(ioutb, "  44cc00 4488cc 448888 448844 448800 4444cc 444488 "
	              "444444 444400 4400cc 440088 440044\n");
	Bprint(ioutb, "  00ff00 00aaff 00bbbb 00bb5d 00bb00 0055ff 005dbb "
	              "007777 007700 0000ff 0000bb 000077\n");
	Bprint(ioutb, "  333333 00ffff 00ffaa 00ff55 00ee4f 00ee00 009eee "
	              "00aaaa 00aa55 00aa00 004fee 0055aa\n");
	Bprint(ioutb, "  006666 006600 0000ee 0000aa 000066 222222 00eeee "
	              "00ee9e 00dd93 00dd49 00dd00 0093dd\n");
	Bprint(ioutb, "  009999 00994c 009900 0049dd 004c99 005555 005500 "
	              "0000dd 000099 000055 111111 00dddd\n");
	Bprint(ioutb, "  00cccc 00cc88 00cc44 00cc00 0088cc 008888 008844 "
	              "008800 0044cc 004488 004444 004400\n");
	...
	Bprint(ioutb,
	       "iwidth xmagnification mul iheight ymagnification mul scale\n");
	Bprint(ioutb, "<<\n");
	...
	Bprint(
	    ioutb,
	    " /Decode decodemat %% Brazil/Inferno cmap or DeviceGray value\n");
...
printnewpage(int pagenum, int end, Biobuf* ioutb)
{
	...
	if(end) {
		/*		# bounding box */
		if(boxes) {
			Bprint(ioutb, "18 18 moveto 594 18 lineto 594 774 "
			              "lineto 18 774 lineto closepath "
			              "stroke\n");
		}
...
/* 			if (im!=nil){
/* 				bps := 1<<im.ldepth;
/* 				ioutb.puts(sys->sprint("%d %d %d %d %d %d %d
%d doimage\n",
/*
...
/* 		if (items[j].itype!=IMAGE || class=="button"||
class=="menubutton"){
...
/* 				ioutb.puts(sys->sprint("(%s) dup stringwidth
pop 2 div",
/* 								msg));
...
imagebits(Biobuf* ioutb, Memimage* im)
{
	...
	if(debug)
		fprint(2, "imagebits, r=%d %d %d %d, depth=%d\n", im->r.min.x,
		       im->r.min.y, im->r.max.x, im->r.max.y, im->depth);
	width = Dx(im->r);
	height = Dy(im->r);
	...
	if(bitoff != 0) {
		/* 		# Postscript image wants beginning of line at
		 * beginning of byte */
		r = im->r;
	...
	for(i = 0; i < Dy(im->r); i++) {
		/* memmove(data+bpl*i, byteaddr(im, Pt(im->r.min.x,
		 * im->r.min.y+i)), bpl); with inversion */
		dst = data + bpl * i;
...
image2psfile(int fd, Memimage* im, int dpi)
{
	...
	if(im->depth >= 8 && im->chan != CMAP8 && im->chan != GREY8) {
		/*
		 * the postscript libraries can only handle [1248]-bit grey,
		 * 8-bit cmap,
	...
	if(landscape) {
		paperaspectratio = ((double)paperlength - (ymargin * 2)) /
		                   ((double)paperwidth - (xmargin * 2));
		if(dpi > 0) {
			iwidth = width * 72 / dpi;
		...
		paperaspectratio = ((double)paperwidth - (xmargin * 2)) /
		                   ((double)paperlength - (ymargin * 2));
		if(dpi > 0) {
			iwidth = width * 72 / dpi;
			iheight = height * 72 / dpi;
		} else if(imageaspectratio > paperaspectratio) {
	...
	Bprint(&ioutb, "%d 0 %d %d %d %d %d %d %s doimage\n", iheight, iwidth,
	       ystart, xstart, height, width, bps,
	       im->flags & Fgrey ? "true" : "false");
	imagebits(&ioutb, im);
