 ...
 * if you appear to be having trouble with flow control. The idea is simple -
 *only
 ...
 * originally included in the postio.tmp directory, but can now be requested
 *with
 * the -S option. Considered eliminating this code, but some printers still
 *depend
 * on it. In particular Datakit connections made using Datakit PVCs and DACUs
 *seem
 * to have the most problems. Much of the new stuff that was added can't work
 *when
...

{
	...
	/*
	 *
	 * A slow version of send() that's very careful about when data is sent
	 *to the
	 * printer. Should help prevent overflowing the printer's input buffer,
	 *provided
	 * blocksize is sufficiently small (1024 should be safe). It's a totally
	 *kludged
	 * up routine that should ONLY be used if you have constant transmission
	 *problems.
	 * There's really no way it will be able to drive a printer much faster
	 *that about
	 * six pages a minute, even for the simplest jobs. Get it by using the
	 *-S option.
	 *
	 */
