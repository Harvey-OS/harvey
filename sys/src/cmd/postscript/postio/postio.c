 ...
 * expect is necessary, but should provide a reasonably reliable spooler
 *interface
 * that can return error indications to the caller via the program's exit
 *status.
 *
 ...
 *file
 * that will be returning useful data from the printer. The two process stuff
 *was
 ...
 * to the original program (despite the fact that there are now many
 *differences).
 *
 ...
 *it's
 * not as efficient as it could be. It depends on the system dependent
 *procedures
 ...
 *forces
 * the return of a status report. PostScript errors detected on the printer
 *result
 ...
 *be
 * data loss in stuff sent back from the printer. Usually that only caused
 *problems
 * with jobs designed to run on the printer and return useful data back over the
 * communications line. If that's the kind of job you're sending call postio
 *with
 * the -t option. That should force the program to split into separate read and
 * write processes and everything not bracketed by "%%[ " and " ]%%" strings
 *goes
 * to stdout. In otherwords the data you're expecting should be separated from
 *the
 * status stuff that goes to the log file (or stderr). The -R2 option does
 *almost
 * the same thing (ie. separate read and write processes), but everything that
 * comes back from the printer goes to the log file (stderr by default) and
 *you'll
 ...
 * used. If you have a program that will be returning data the command line
 *might
 * look like,
 *
...

{
	 ...
	 *printers. Can run
	 * as a single process or as separate read/write processes. What's done
	 *depends on
...

{
	...
	/*
	 *
	 * Makes sure we handle interrupts. The proper way to kill the program,
	 *if
	 * necessary, is to do a kill -15. That forces a call to interrupt(),
	 *which in
	 * turn tries to reset the printer and then exits with a non-zero
	 *status. If the
	 * program is running as two processes, sending SIGTERM to either the
	 *parent or
...

{
	...
	/*
	 *
	 * Reads and processes the command line options. The -R2, -t, and -i
	 *options all
	 * force separate read and write processes by eventually setting splitme
	 *to TRUE
	 * (check initialize()). The -S option is not recommended and should
	 *only be used
	 * as a last resort!
	 *
	 */
...

{
	...
	/*
	 *
	 * Called from options() to convert a baud rate string into an
	 *appropriate termio
	 * value. *rate is looked up in baudtable[] and if it's found, the
	 *corresponding
...

{
	...
	/*
	 *
	 * Initialization, a few checks, and a call to setupline() (file
	 *ifdef.c) to open
	 * and configure the communications line. Settings for interactive mode
	 *always
	 * take precedence. The setupstdin() call with an argument of 0 saves
	 *the current
	 * terminal settings if interactive mode has been requested - otherwise
	 *nothing's
	 * done. Unbuffering stdout (via the setbuf() call) isn't really needed
	 *on System V
	 * since it's flushed whenever terminal input is requested. It's more
	 *efficient if
	 * we buffer the stdout (on System V) but safer (for other versions of
	 *Unix) if we
	...
	endmesg =
	    mesg + sizeof mesg - 2; /* one byte from last position in mesg */
...

{
	...
	/*
	 *
	 * Tries to put the printer in the IDLE state before anything important
	 *is sent.
	 * Run as a single process no matter what has been assigned to splitme.
	 *Separate
	 * read and write processes, if requested, will be created after we're
	 *done here.
	 *
	 */
...

{
	...
	/*
	 *
	 * If splitme is TRUE we fork a process, make the parent handle reading,
	 *and let
	 * the child take care of writing. resetline() (file ifdef.c) contains
	 *all the
	 * system dependent code needed to reset the communications line for
	 *separate
	 * read and write processes. For now it's expected to return TRUE or
	 *FALSE and
	 * that value controls whether we try the fork. I've only tested the two
	 *process
	 * stuff for System V. Other versions of resetline() may just be dummy
	 *procedures
	 * that always return FALSE. If the fork() failed previous versions
	 *continued as
	 * a single process, although the implementation wasn't quite right, but
	 *I've now
	 * decided to quit. The main reason is a Datakit channel may be
	 *configured to
	 * flow control data in both directions, and if we run postio over that
	 *channel
...

{
	 ...
	 *If there
	 * aren't any arguments left when we get here we'll send stdin. Input
	 *files are
	 * only read and sent to the printer if canwrite is TRUE. Checking it
	 *here means
	 * we won't have to do it in send(). If interactive mode is TRUE we'll
	 *stay here
	 * forever sending stdin when we run out of files - exit with a break.
	 *Actually
	 * the loop is bogus and used at most once when we're in interactive
	 *mode because
	 * stdin is in a pseudo raw mode and the read() in readblock() should
	 *never see
	 * the end of file.
	 *
	 */
...

{
	...
	/*
	 *
	 * Sends file *name to the printer. There's nothing left here that
	 *depends on
	 * sending and receiving status reports, although it can be reassuring
	 *to know
	 * the printer is responding and processing our job. Only the writer
	 *gets here
	 * in the two process implementation, and in that case split() has reset
	 *nostatus
	 * to WRITEPROCESS and that's what getstatus() always returns. For now
	 *we accept
	 * the IDLE state and ENDOFJOB as legitimate and ignore the INITIALIZING
	 *state.
	 *
	 */
...

{
	...
	/*
	 *
	 * Tries to stay connected to the printer until we're reasonably sure
	 *the job is
	 * complete. It's the only way we can recover error messages or data
	 *generated by
	 * the PostScript program and returned over the communication line.
	 *Actually doing
	 * it correctly for all possible PostScript jobs is more difficult that
	 *it might
	 * seem. For example if we've sent several jobs, each with their own EOF
	 *mark, then
	 * waiting for ENDOFJOB won't guarantee all the jobs have completed.
	 *Even waiting
	 * for IDLE isn't good enough. Checking for the WAITING state after all
	 *the files
	 * have been sent and then sending an EOF may be the best approach, but
	 *even that
	 * won't work all the time - we could miss it or might not get there.
	 *Even sending
	 * our own special PostScript job after all the input files has it's own
	 *different
	 * set of problems, but probably could work (perhaps by printing a fake
	 *status
	 * message or just not timing out). Anyway it's probably not worth the
	 *trouble so
	 * for now we'll quit if writedone is TRUE and we get ENDOFJOB or IDLE.
	 *
	 * If we're running separate read and write processes the reader gets
	 *here after
	 * after split() while the writer goes to send() and only gets here
	 *after all the
	 * input files have been transmitted. When they're both here the writer
	 *sends the
	 * reader signal joinsig and that forces writedone to TRUE in the
	 *reader. At that
	 * point the reader can begin looking for an indication of the end of
	 *the job.
	 * The writer hangs around until the reader kills it (usually in
	 *cleanup()) sending
...

{
	...
	/*
	 *
	 * Only needed if we're running separate read and write processes. Makes
	 *sure the
	 * write process is killed after the read process has successfully
	 *finished with
	 * all the jobs. sendsignal() returns a -1 if there's nobody to signal
	 *so things
...

{
	 ...
	 *with the
	 * last one. Blocks from fd_in are stored in array block[]. head is the
	 *index
	 * of the next byte in block[] that's supposed to go to the printer.
	 *tail points
	 * one past the last byte in the current block. head is adjusted in
	 *writeblock()
	 * after each successful write, while head and tail are reset here each
	 *time
	 * a new block is read. Returns the number of bytes left in the current
	 *block.
	 * Read errors cause the program to abort. The fake status message
	 *that's put out
	 * in quiet mode is only so you can look at the log file and know
	 *something's
...

{
	 ...
	 *printer. head
	 * is adjusted after the write, and the number of bytes that were
	 *successfully
...

{
	...
	/*
	 *
	 * Looks for things coming back from the printer on the communications
	 *line, parses
	 * complete lines retrieved by readline(), and returns an integer
	 *representation
	 * of the current printer status to the caller. If nothing was available
	 *a status
	 * request (control T) is sent to the printer and nostatus is returned
	 *to the
	 * caller (provided quiet isn't TRUE). Interactive mode either never
	 *returns from
	...
	if(canread == TRUE && (gotline = readline()) == TRUE) {
		state = parsemesg();
		if(state != laststate || state == UNKNOWN || mesgptr != mesg ||
		   debug == ON)
			logit("%s", mesg);
	...
	if((quiet == FALSE || currentstate != SEND) &&
	   (tostdout == FALSE || currentstate == START) &&
	   interactive == FALSE) {
...

{
	...
	/*
	 *
	 * Parsing the lines that readline() stores in mesg[] is messy, and
	 *what's done
	 * here isn't completely correct nor as fast as it could be. The general
	 *format
	 * of lines that come back from the printer (assuming no data loss) is:
	 *
	 *		str%%[ key: val; key: val; key: val ]%%\n
	 *
	 * where str can be most anything not containing a newline and printer
	 *reports
	 * (eg. status or error messages) are bracketed by "%%[ " and " ]%%"
	 *strings and
	 * end with a newline. Usually we'll have the string or printer report
	 *but not
	 * both. For most jobs the leading string will be empty, but could be
	 *anything
	 * generated on a printer and returned over the communications line
	 *using the
	 * PostScript print operator. I'll assume PostScript jobs are well
	 *behaved and
	 * never bracket their messages with "%%[ " and " ]%%" strings that
	 *delimit status
	 * or error messages.
	 *
	 * Printer reports consist of one or more key/val pairs, and what we're
	 *interested
	 * in (status or error indications) may not be the first pair in the
	 *list. In
	 * addition we'll sometimes want the value associated with a keyword
	 *(eg. when
	 * key = status) and other times we'll want the keyword (eg. when key =
	 *Error or
	 * Flushing). The last pair isn't terminated by a semicolon and a value
	 *string
	 * often contains many space separated words and it can even include
	 *colons in
	 * meaningful places. I've also decided to continue converting things to
	 *lower
	 * case before doing the lookup in status[]. The isupper() test is for
	 *Berkeley
	 * systems.
	 *
	 */
...

{
	...
	/*
	 *
	 * Looks for *str1 in string *str2. Returns a pointer to the start of
	 *the substring
...

{
	...
	/*
	 *
	 * Sends signal sig to the other process if we're running as separate
	 *read and
	 * write processes. Returns the result of the kill if there's someone
	 *else to
...

{
	...
	/*
	 *
	 * Caught a signal - all except joinsig cause the program to quit.
	 *joinsig is the
	 * signal sent by the writer to the reader after all the jobs have been
	 *transmitted.
	 * Used to tell the read process when it can start looking for the end
	 *of the job.
	 *
	 */
...

{
	 ...
	 *printed
	 * using the control string arguments a?. If kind is FATAL and we're not
	 *ignoring
	 * errors the program will be terminated. If mesg is NULL or *mesg is
	 *the NULL
...

{
	...
	/*
	 *
	 * Makes sure everything is properly cleaned up if there's a signal or
	 *FATAL error
	 * that should cause the program to terminate. The sleep by the write
	 *process is
	 * to help give the reset sequence a chance to reach the printer before
	 *we break
	 * the connection - primarily for printers connected to Datakit. There's
	 *a very
	 * slight chance the reset sequence that's sent to the printer could get
	 *us stuck
	 * here. Simplest solution is don't bother to send it - everything works
	 *without it.
	 * Flushing ttyo would be better, but means yet another system dependent
	 *procedure
	 * in ifdef.c! I'll leave things be for now.
	 *
	 * Obscure problem on PS-810 turbos says wait a bit after sending an
	 *interrupt.
	 * Seem to remember the printer getting into a bad state immediately
	 *after the
	 * top was opened when the toner light was on. A sleep after sending the
	 *ctrl-C
...

{
	...
	/*
	 *
	 * Used to replace sleep() calls. Only needed if we're running the
	 *program as
	 * a read and write process and don't want to have the read process
	 *sleep. Most
	 * sleeps are in the code because of the non-blocking read used by the
	 *single
...

{
	...
	/*
	 *
	 * Used to replace some of the read() calls. Only needed if we're
	 *running separate
	 * read and write processes. Should only be used to replace read calls
	 *on ttyi.
	 * Always returns 0 to the caller if the process doesn't have its READ
	 *flag set.
...

{
	...
	/*
	 *
	 * Used to replace some of the write() calls. Again only needed if we're
	 *running
	 * separate read and write processes. Should only be used to replace
	 *write calls
	 * on ttyo. Always returns n to the caller if the process doesn't have
	 *its WRITE
