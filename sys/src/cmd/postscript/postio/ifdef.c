 ...
 * part of the purpose of conditional complilation directives, I think it's
 *easier
 ...
 *
 * By request I've changed the way some of the setupline() procedures (eg. in
 *the
 * System V implementation) handle things when no line has been given. If line
 *is
 ...
 * we continue on as before using stdout as ttyi and ttyo. Doesn't work when
 *we're
...

{
	...
	/*
	 *
	 * Only used if we're running the program as separate read and write
	 *processes.
	 * Called from split() after the initial connection has been made and
	 *returns
	 * TRUE if two processes should work. Don't know if the O_NDELAY stuff
	 *is really
	 * needed, but setting c_cc[VMIN] to 1 definitely is. If we leave it be
	 *(as a 0)
...

{
	...
	/*
	 *
	 * Save (mode = 0), reset (mode = 1), or restore (mode = 2) the tty
	 *settings for
	 * stdin. Expect something like raw mode with no echo will be set up.
	 *Explicit
	 * code to ensure blocking reads probably isn't needed because blocksize
	 *is set
	...
	if(interactive == TRUE)
		...
		case 0:
			if(isatty(0) != 1)
				error(FATAL, "stdin not a terminal - can't run "
				             "interactive mode");
...

{
	...
	/*
	 *
	 * Reads characters coming back from the printer on ttyi up to a newline
	 *(or EOF)
	 * or until no more characters are available. Characters are put in
	 *mesg[], the
	 * string is terminated with '\0' when we're done with a line and TRUE
	 *is returned
	 * to the caller. If complete line wasn't available FALSE is returned.
	 *Interactive
	 * mode should loop here forever, except during start(), echoing
	 *characters to
	 * stdout. If it happens to leave FALSE should be returned. The
	 *non-blocking read
	 * gets us out until split() is called.
	 *
	 * Some users (apparently just on 3B2 DKHOST systems) have had problems
	 *with the
	 * two process implementation that's forced me to kludge things up some.
	 *When a
	 * printer (on those systems) is turned off while postio is transmitting
	 *files
	 * the write process hangs in writeblock() (postio.c) - it's typically
	 *in the
	 * middle of a write() call, while the read() call (below) continually
	 *returns 0.
	 * In the original code readline() returned FALSE when read() returned 0
	 *and we
	 * get into a loop that never ends - because the write process is hung.
	 *In the
	 * one process implementation having read return 0 is legitimate because
	 *the line
	 * is opened for no delay, but with two processes the read() blocks and
	 *a return
	 * value of 0 should never occur. From my point of view the real problem
	 *is that
	 * the write() call hangs on 3B2 DKHOST systems and apparently doesn't
	 *anywhere
	 * else. If the write returned anything less than or equal to 0
	 *writeblock() would
	 * shut things down. The kludge I've implemented counts the number of
	 *consecutive
	 * times read() returns a 0 and if it exceeds a limit (100) the read
	 *process will
	 * shut things down. In fact one return of 0 from read() when we're in
	 *the two
	 * process mode is undoubtedly sufficient and no counting should be
	 *necessary!!!
	 * Moving the check to getstatus() should also work and is probably
	 *where things
	 * belong.
	 *
	 */
	...
	if(interactive == FALSE) {
		while((n = read(ttyi, ptr, 1)) != 0) {
			if(n < 0)
				if(errno == EINTR)
					...
					error(FATAL, "error reading %s", line);
			tries = 0;
			if(*ptr == '\n' || *ptr == '\004' || ptr >= endmesg) {
				*(ptr + 1) = '\0';
				if(*ptr == '\004')
		...
		if(canread == TRUE &&
		   canwrite == FALSE) /* read process kludge */
			if(++tries > 100)
...

{
	 ...
	 *the stuff
	 * for separate read and write processes on Ninth Edition systems - no
	 *guarantees
...

{
	 ...
	 * stdin. Expect something like raw mode with no echo will be set up.
	 *Need to make
	 * sure interrupt and quit still work - they're the only good way to
	 *exit when
...

{
	 ...
	 * Reads characters coming back from the printer on ttyi up to a newline
	 *(or EOF)
	 * and transfers each line to the mesg[] array. Everything available on
	 *ttyi is
	 * initially stored in tbuf[] and a line at a time is transferred from
	 *there to
	 * mesg[]. The string in mesg[] is terminated with a '\0' and TRUE is
	 *returned to
	 * the caller when we find a newline, EOF, or reach the end of the
	 *mesg[] array.
	 * If nothing is available on ttyi we return FALSE if a single process
	 *is being
	 * used for reads and writes, while in the two process implementation we
	 *force a
	 * one character read. Interactive mode loops here forever, except
	 *during start(),
	 * echoing everything that comes back on ttyi to stdout. The performance
	 *of a
	 * simple getc/putc loop for interactive mode was unacceptable when run
	 *under mux
	 * and has been replaced by more complicated code. When layers wasn't
	 *involved
		...
		while(1) {
			...
			if(n <= 0)
				if(canwrite == TRUE)
					return (FALSE);
			n = ((n < 1) ? 1
			             : ((n < sizeof(tbuf)) ? n : sizeof(tbuf)));
			if((n = read(ttyi, tbuf, n)) < 0)
				if(errno == EINTR)
					continue;
				else
					error(FATAL, "error reading line %s",
					      line);
			else
				eptr = nptr + n;
		} /* End while */
...

{
	...
	/*
	 *
	 * Line initialization for BSD4_2. As in the System V code, if no line
	 *is given
	 * (ie. line == NULL) we continue on as before using stdout as ttyi and
	 *ttyo.
	 *
	 */
...

{
	 ...
	 * TRUE if two processes should work. Haven't tested or even compiled
	 *the stuff
	 * for separate read and write processes on Berkeley systems - no
	 *guarantees
	 * even though we return TRUE!
	 *
	 */
...

{
	...
	/*
	 *
	 * Reads characters coming back from the printer on ttyo up to a newline
	 *(or EOF)
	 * or until no more characters are available. Characters are put in
	 *mesg[], the
	 * string is terminated with '\0' when we're done with a line and TRUE
	 ...
	 * mode should loop here forever, except during start(), echoing
	 *characters to
	 * stdout. If it happens to leave FALSE should be returned. Probably
	 *should read
	 * everything available on ttyi into a temporary buffer and work from
	 *there rather
 ...
 */
int strspn(string, charset) int8_t* string;
...
char* strpbrk(string, brkset) register char* string, *brkset;
{
	register char* p;
...
char* strtok(string, sepset) int8_t* string, *sepset;
{
	register char* p, *q, *r;
	static char* savept;
...

{
	...
	/*
	 *
	 * Tries to connect to a Datakit destination. The extra stuff I've added
	 *to save
	 * and later restore stderr is primarily for our spooling setup at
	 *Murray Hill.
	 * postio is usually called with stderr directed to a file that will be
	 *returned
	 * to the user when the job finishes printing. Problems encountered by
	 *dkdial(),
	 * like busy messages, go to stderr but don't belong in the user's mail.
	 *They'll
	 * be temporarily directed to the log file. After we've connected stderr
	 *will be
	 * restored.
	 *
	 */
	...
	if(window_size > 0) {
		...
		if(ioctl(ttyi, DIOURPWD, dkparm) < 0 ||
		   ioctl(ttyi, DIOCFLUSH, 0) < 0)
