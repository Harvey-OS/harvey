 ...
 *
 * Page reversal relies on being able to locate sections of a document using
 *file
 * structuring comments defined by Adobe (ie. the 1.0 and now 2.0 conventions)
 *and
 * a few I've added. Among other things a minimally conforming document,
 *according
 ...
 *
 * The most important change (at least for this program) that Adobe made in
 *going
 * from the 1.0 to the 2.0 structuring conventions was in the prologue. They now
 * say the prologue should only define things, and the global initialization
 *that
 ...
 * and %%EndSetup comments. So a document that conforms to Adobe's 2.0
 *conventions,
 *
 *	1) Marks the end of the prologue (only definitions) with %%EndProlog.
 *
 *	2) Brackets global initialization with %%BeginSetup and %%EndSetup
 *comments
 ...
 *
 * postreverse can handle documents that follow the 1.0 or 2.0 conventions, but
 *has
 * also been extended slightly so it works properly with the translators
 *(primarily
 * dpost) supplied with this package. The page independence requirement has been
 * relaxed some. In particular definitions exported to the global environment
 *from
 ...
 * of the document, immediately before it writes the %%EndProlog (for version
 *1.0)
 * or %%EndSetup (for version 2.0) comments.
 *
 * In addition postreverse accepts documents that choose to mark the end of each
 * page with a %%EndPage: comment, which from a translator's point of view is
 *often
 * a more natural approach. Both page boundary comments (ie. Page: and
 *%%EndPage:)
 * are also accepted, but be warned that everything between consecutive
 *%%EndPage:
 ...
 *
 *	2) May have a %%BeginSetup/%%EndSetup comment pair before the first
 *%%Page:
 *	   comment that brackets any global initialization.
 *
 *	3) Marks the start of each page with a %%Page: comment, or the end of
 *each
 *	   page with a %%EndPage: comment. Both page boundary comments are
 *allowed.
 *
 *	4) Marks the end of all the pages with a %%Trailer comment.
 *
 ...
 *
 * If no file arguments are given postreverse copies stdin to a temporary file
 *and
 * then processes that file. That means the input is read three times (rather
 *than
 * two) whenever we handle stdin. That's expensive, and shouldn't be too
 *difficult
...
main(agc, agv) int agc;
char* agv[];
{
	/*
	 * A simple program that reverses the pages in specially formatted
	 * PostScript
	 * files. Will work with all the translators in this package, and should
	 * handle
	 * any document that conforms to Adobe's version 1.0 or 2.0 file
	 * structuring
	 * conventions. Only one input file is allowed, and it can either be a
	 * named (on
...
options()
{
	...
	/*
	 *
	 * Reads and processes the command line options. The -r option (ie. the
	 *one that
	 * turns page reversal off) is really only useful if you want to take
	 *dpost output
	 * and produce a page independent output file. In that case global
	 *definitions
	...
	while((ch = getopt(argc, argv, optnames)) != EOF) {
		...
		case 'n': /* forms per page */
			if((forms = atoi(optarg)) <= 0)
				error(FATAL, "illegal forms request %s",
				      optarg);
			break;
...

{
	...
	/*
	 *
	 * postreverse only handles one input file at a time, so if there's more
	 *than one
	 * argument left when we get here we'll quit. If none remain we copy
	 *stdin to a
...

{
	...
	/*
	 *
	 * Cleans things up after we've finished reversing the pages in the
	 *input file.
	 * All that's really left to do is remove the temp file, provided we
	 *used one.
	 *
	 */
...

{
	...
	/*
	 *
	 * Copies stdin to a temporary file and returns the pathname of that
	 *file to the
	 * caller. It's an expensive way of doing things, because it means we
	 *end up
	 * reading the input file three times - rather than just twice. Could
	 *probably be
	 * fixed by creating the temporary file on the fly as we read the file
	 *the first
	 * time.
	 *
	 */
...

{
	...
	/*
	 *
	 * Begins by looking for the ENDPROLOG comment in the input file.
	 *Everything up to
	 * that comment is copied to the output file. If the comment isn't found
	 *the entire
	 * input file is copied and moreprolog() returns FALSE. Otherwise
	 *readpages() reads
	 * the rest of the input file and remembers (in pages[]) where each page
	 *starts and
	 * ends. In addition everything bracketed by %%BeginGlobal and
	 *%%EndGlobal comments
	 * is immediately added to the new prologue (or setup section) and ends
	 *up being
	 * removed from the individual pages. When readpages() finds the TRAILER
	 *comment
	 * or gets to the end of the input file we go back to the pages[] array
	 *and use
	 * the saved offsets to write the pages out in reverse order. Finally
	 *everything
	 * from the TRAILER comment to the end of the input file is copied to
	 *the output
	 * file.
	 *
	 */
...

{
	...
	/*
	 *
	 * Looks for string *str at the start of a line and copies everything up
	 *to that
	 * string to the output file. If *str isn't found the entire input file
	 *will end
	 * up being copied to the output file and FALSE will be returned to the
	 *caller.
	 * The first call (made from reverse()) looks for ENDPROLOG. Any other
	 *call comes
...

{
	...
	/*
	 *
	 * Records starting and ending positions of the requested pages (usually
	 *all of
	 * them), puts global definitions in the prologue, and remembers where
	 *the TRAILER
	 * was found.
	 *
	 * Page boundaries are marked by the strings PAGE, ENDPAGE, or perhaps
	 *both.
	 * Application programs will normally find one or the other more
	 *convenient, so
	 * in most cases only one kind of page delimiter will be found in a
	 *particular
	 * document.
	 *
	 */
	...
	while(fgets(buf, sizeof(buf), fp_in) != NULL)
		if(buf[0] != '%')
			...
			if(sawendpage == FALSE && in_olist(page++) == ON) {
				pages[next_page].empty = FALSE;
				pages[next_page++].stop =
				    ftell(fp_in) - strlen(buf);
			} /* End if */
...

{
	...
	/*
	 *
	 * Goes through the pages[] array, usually from the bottom up, and
	 *writes out all
	 * the pages. Documents that print more than one form per page cause
	 *things to get
	 * a little more complicated. Each physical page has to have its
	 *subpages printed
	 * in the correct order, and we have to build a few dummy subpages for
	 *the last
	 * (and now first) sheet of paper, otherwise things will only
	 *occasionally work.
	 *
	 */
	...
	if(noreverse == FALSE) /* fill out the first page */
		for(i = (forms - next_page % forms) % forms; i > 0;
		    i--, next_page++)
			pages[next_page].empty = TRUE;
	else
		forms = next_page; /* turns reversal off in next loop */
	...
	for(i = next_page - forms; i >= 0; i -= forms)
		for(j = i, k = 0; k < forms; j++, k++)
			if(pages[j].empty == TRUE) {
				...
				copypage(pages[j].start, pages[j].stop);
...

{
	...
	/*
	 *
	 * Copies the page beginning at offset start and ending at stop to the
	 *output
	 * file. Global definitions are skipped since they've already been added
	 *to the
	 * prologue.
	 *
	 */
...

{
	...
	/*
	 *
	 * Makes sure everything from the TRAILER string to EOF is copied to the
	 *output
	 * file.
	 *
	 */
