 ...
 * code was borrowed from the corresponding program that was written for
 *printers
 * that understand Impress.
 *
 * Extending the original program to handle PRISM jobs was not trivial. Graphics
 * packages that support PRISM occasionally use BGI commands that I ignored in
 *the
 * STARE implementation. Subroutines, color requests, patterns (for filling),
 *and
 * filled trapeziods were the most important omissions. All are now implemented,
 * and at present only repeats, filled slices, and raster rectangles are
 *missing.
 *
 * Pattern filling results were not always predictable or even good, unless the
 * halftone screen definitions were changed and scaling was adjusted so one
 *pixel
 * in user space mapped into an integral number of device space pixels. Doing
 *that
 * makes the resulting PostScript output device dependent, but was often
 *necessary.
 * I've added two booleans to the PostScript prologue (fixscreen and
 *scaletodevice)
 * that control what's done. By default both are false (check postbgi.ps) but
 *can
 ...
 *
 * Several other approaches are available if you want to have your spooler
 *handle
 ...
 * set fixscreen and scaletodevice to true before anything important is done.
 *That
 * means the following command line,
 *
 *	postbgi -P"/prism true" file >file.ps
 *
 * accomplishes the same things as the last example. Two different prologue
 *files,
 * one for STARE jobs and the other for PRISM, could be used and the spooler
 *could
 * point postbgi to the appropriate one using the -L option. In that case the
 *only
 * important difference in the two prologues would be the definition of prism.
 *The
 * prologue used for PRISM jobs would have prism set to true, while the STARE
 * prologue would have it set to false.
 *
 * Also included is code that ties lines to device space coordinates. What you
 *get
 * is a consistent line thickness, but placement of lines won't be exact. It's a
 * trade-off that should be right for most jobs. Everything is implemented in
 *the
 ...
 *
 * BGI supports color mixing but PostScript doesn't. BGI files that expect to
 *mix
 ...
 *
 *	    Fills a trapezoid starting at (x, y) and having relative
 *displacements
 ...
 *	  angle x y string t -
 *
 *	    Prints string starting at (x, y) using an orientation of angle
 *degrees.
 *	    The PostScript procedure can handle any angle, but BGI files will
 *only
 *	    request 0 or 90 degrees. Text printed at any other orientation will
 *be
 *	    vector generated.
 *
 *	p
 ...
 *	    array. The arrays that describe the different line styles are
 *declared
 ...
 *	  red green blue c -
 *
 *	    Sets the current PostScript RGB color using setrgbcolor. Also used
 *for
 ...
 * works well for 'write to black' engines but won't be right for 'write to
 *white'
 * engines. The line width can be changed using the -w option, or you can change
 * the initialization of linewidth in the prologue. Code in the prologue
 *supports
...

{
	...
	/*
	 *
	 * A program that converts BGI (Basic Graphical Instructions) files
	 *generated by
	 * packages like GRAFPAC and DISSPLA into PostScript. It does an
	 *adequate job but
	 * is far from perfect. A few things still haven't been implemented (eg.
	 *repeats
	 * and raster rectangles), but what's here should be good enough for
	 *most of our
	 * STARE and PRISM jobs. Color mixing (in PRISM jobs) won't work on
	 *PostScript
...

{
	...
	/*
	 *
	 * Scans the option list looking for things, like the prologue file,
	 *that we need
	 * right away but could be changed from the default. Doing things this
	 *way is an
	 * attempt to conform to Adobe's latest file structuring conventions. In
	 *particular
	 * they now say there should be nothing executed in the prologue, and
	 *they have
	 * added two new comments that delimit global initialization calls. Once
	 *we know
	 * where things really are we write out the job header, follow it by the
	 *prologue,
	...
	while((ch = getopt(argc, argv, optnames)) != EOF) {
		...
		case 'J':
			if((fp_acct = fopen(optarg, "a")) == NULL)
				error(FATAL, "can't open accounting file %s",
				      optarg);
			break;
...

{
	...
	/*
	 *
	 * Called from options() to map a user's font name into a legal
	 *PostScript name.
	 * If the lookup fails *name is returned to the caller. That should let
	 *you choose
	 * any PostScript font.
	 *
	 */
...

{
	...
	/*
	 *
	 * Handles things that must be done after the options are read but
	 *before the
...

{
	...
	/*
	 *
	 * Makes sure all the non-option command line options are processed. If
	 *we get
	 * here and there aren't any arguments left, or if '-' is one of the
	 *input files
	 * we'll process stdin.
	 *
	 */
...

{
	...
	/*
	 *
	 * Finished with the last input file, so mark the end of the pages, make
	 *sure the
...

{
	...
	/*
	 *
	 * Controls the conversion of BGI files into PostScript. Not everything
	 *has been
	 * implemented, but what's been done should be good enough for our
	 *purposes.
	 *
	 */
...

{
	...
	/*
	 *
	 * Called when we're supposed to change the BGI character size to n. The
	 *BGI
	 * size is the grid separation in a 5 by 7 array in which characters are
	 *assumed
	 * to be built.
	 *
	 */
...

{
	...
	/*
	 *
	 * Haven't implemented repeats, although it wouldn't be difficult.
	 *Apparently it's
...

{
	...
	/*
	 *
	 * Called from conv() after we've entered one of the graphical character
	 *modes.
	 * Characters are read from the input file and printed until the next
	 *mode change
	 * opcode is found (or until EOF). angle will be 90 for rotated
	 *character mode
	 * and 0 otherwise.
	 *
	 *
...

{
	...
	/*
	 *
	 * Does whatever is needed to print the last page and get ready for the
	 *next one.
	 * It's called, from conv(), after a BEND code is processed. I'm
	 *ignoring the
...

{
	...
	/*
	 *
	 * Starts a subroutine definition. All subroutines are defined as
	 *PostScript
	 * procedures that begin with the character S and end with the
	 *subroutine's id
	 * (a number between 0 and 63 - I guess). The primary, and perhaps only
	 *use of
	 * subroutines is in special color plots produced by several graphics
	 *libraries,
	 * and even there it's not all that common. I've also chosen not to
	 *worry about
	 * nested subroutine definitions - that would certainly be overkill!
	 *
	 * All subroutines set up their own (translated) coordinate system, do
	 *their work
	 * in that system, and restore things when they exit. To make everything
	 *work
	 * properly we save the current point (in shpos and svpos), set our
	 *position to
	 * (0, 0), and restore things at the end of the subroutine definition.
	 *That means
	 * hpos and vpos measure the relative displacement after a subroutine
	 *returns, and
	 * we save those values in the displacement[] array. The displacements
	 *are used
	 * (in subr_call()) to properly adjust our position after each
	 *subroutine call,
	 * and all subroutines are called with the current x and y coordinates
	 *on top of
	 * the stack.
	 *
	 */
...

{
	...
	/*
	 *
	 * Handles stuff needed at the end of each subroutine. Want to remember
	 *the change
	 * in horizontal and vertical positions for each subroutine so we can
	 *adjust our
	 * position after each call - just in case. The current position was set
	 *to (0, 0)
	 * before we started the subroutine definition, so when we get here hpos
	 *and vpos
	 * are the relative displacements after the subroutine is called.
	 *They're saved in
	 * the displacement[] array and used to adjust the current position when
	 *we return
	 * from a subroutine.
	 *
	 */
...

{
	...
	/*
	 *
	 * Handles subroutine calls. Everything that follows the BCALL opcode
	 *(up to the
	 * next opcode) is taken as a subroutine identifier - thus the loop that
	 *generates
...

{
	...
	/*
	 *
	 * Handles plotting of all types of BGI vectors. If it's a manhattan
	 *vector var
	 * specifies which coordinate will be changed by the next number in the
	 *input
	 * file.
	 *
	 */
...

{
	...
	/*
	 *
	 * Draws a rectangle and either outlines or fills it, depending on the
	 *value of
	 * mode. Would be clearer, and perhaps better, if {stroke} or {fill}
	 *were put on
	 * the stack instead of 0 or 1. R could then define the path and just do
	 *an exec
...

{
	...
	/*
	 *
	 * Handles filled trapeziods. A data byte of 0101 following the opcode
	 *means the
	 * horizontal sides are parallel, 0102 means the vertical sides are
	 *parallel.
	 * Filling is handled by eofill so we don't need to get things in the
	 *right order.
	 *
	 */
...

{
	...
	/*
	 *
	 * The two point plot modes are used to place a character at selected
	 *points. The
	 * difference in the two modes, namely BPOINT and BPOINT1, is the way we
	 *get the
	 * coordinates of the next point. In BPOINT1 the two bytes immediately
	 *following
	 * ch select a constant horizontal change, while both coordinates are
	 *given for
...

{
	...
	/*
	 *
	 * Essentially the same format as BPOINT1, except that in this case we
	 *connect
...

{
	...
	/*
	 *
	 * Called whenever we need to draw an arc. I'm ignoring filled slices
	 *for now.
	 *
	 */
...

{
	...
	/*
	 *
	 * Handles patterns by setting the current color based of the values
	 *assigned to
	 * the next four data bytes. BGI supports two kinds of patterns (fine or
	 *coarse)
	 * but I'm handling each in the same way - for now. In a fine pattern
	 *the four
	 * data bytes assign a color to four individual pixels (upperleft first)
	 *while
	 * in a coarse pattern the four colors are assigned to groups of four
	 *pixels,
	 * for a total of 16. Again the first color goes to the group in the
	 *upper left
	 * corner. The byte immediately following the BPAT opcode selects fine
	 *(040) or
	 * coarse (041) patterns. The PostScript RGB color is assigned by
	 *averaging the
	 * RED, GREEN, and BLUE components assigned to the four pixels (or
	 *groups of
	 * pixels). Acceptable results, but there's no distinction between fine
	 *and
	 * coarse patterns.
	 *
	 */
...

{
	...
	/*
	 *
	 * Picks the requested color component (RED, GREEN, or BLUE) from val
	 *and returns
	 * the result to the caller. BGI works with Cyan, Yellow, and Magenta so
	 *the one's
	 * complement stuff (following the exclusive or'ing) recovers the RED,
	 *BLUE, and
	 * GREEN components that PostScript's setrgbcolor operator needs. The
	 *PostScript
	 * interpreter in the ColorScript 100 has a setcmycolor operator, but
	 *it's not
	...
	if(debug == ON)
		fprintf(stderr,
		        "val = %o, primary = %o, plane = %o, rgbcolor = %o\n",
...

{
	...
	/*
	 *
	 * Arranges to have the color set to the value requested in the BGI data
	 *byte val.
	 *
	 */

	fprintf(fp_out, "%d %d %d c\n", get_color(val, RED),
	        get_color(val, GREEN), get_color(val, BLUE));
...

{
	...
	/*
	 *
	 * Figures out the value on the integer (sign magnitude form) that's
	 *next in the
	 * input file. If highbyte is nonzero we'll use it and the next byte to
	 *build the
	...
	return (highbyte & SGNB ? -MAG(highbyte, lowbyte)
	                        : MAG(highbyte, lowbyte));
...

{
	...
	/*
	 *
	 * Called when we expect to find a single data character in the input
	 *file. The
	 * data bit is turned off and the resulting value is returned to the
	 *caller.
	 *
	 */
...

{
	...
	/*
	 *
	 * Reads the next character from file *fp_in and returns the value to
	 *the caller.
	 * This routine isn't really needed, but we may want to deal directly
	 *with some
	 * screwball file formats so I thought it would probably be a good idea
	 *to isolate
...

{
	...
	/*
	 *
	 * If we're not supposed to print page pg, fp_out will be directed to
	 */dev/null,
