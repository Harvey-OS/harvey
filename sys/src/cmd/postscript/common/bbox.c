...
void cover(x, y) double x, y;
{
	/*
	 * Adds point (x, y) to bbox. Coordinates are in user space - the
	 * transformation
...
void writebbox(fp, keyword, slop) FILE* fp; /* the comment is written here */
...
int slop; /* expand (or contract?) the box a bit */
{
	...
	/*
	 * Transforms the numbers in the bbox[] using ctm[], adjusts the corners
	 * a bit
	 * (depending on slop) and then writes comment. If *keyword is
	 * BoundingBox use
	 * whatever's been saved in docbbox, otherwise assume the comment is
	 * just for
	 * the current page.
	 */
...
{
	/*
	 * Adds bbox to docbbox and resets bbox for the next page. Only update
	 * docbbox
...
concat(double m1[])
{
	...
	/*
	 * Replaces the ctm[] by the result of the matrix multiplication m1[] x
	 * ctm[].
	 */
