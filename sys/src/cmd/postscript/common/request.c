 ...
 * page basis. Requests are passed through to the translator using the -R
 * option.
 ...
 */
void saverequest(want) char* want; /* grab code for this stuff */
{
 ...
 */
void writerequest(page, fp_out) int page; /* write everything for this page */
 ...
 * to the end of file.
 */
void dumprequest(want, file, fp_out) char* want; /* look for this string */
...
FILE* fp_out; /* and write the value out here */
{
	...
	if((fp_in = fopen(file, "r")) != NULL) {
		while(fgets(buf, sizeof(buf), fp_in) != NULL)
			if(buf[0] == '@' &&
			   strncmp(want, &buf[1], strlen(want)) == 0)
				while(fgets(buf, sizeof(buf), fp_in) != NULL)
					if(buf[0] == '#' || buf[0] == '%')
						continue;
					else if(buf[0] != '@')
