...
Status statuslist[] = {
    "initializing", INITIALIZING, "idle",  IDLE,       "busy",
    BUSY,           "waiting",    WAITING, "printing", PRINTING,
    "printererror", PRINTERERROR, "Error", ERROR,      "flushing",
    FLUSHING,       NULL,         UNKNOWN};
...
readprinter(int printerfd, int pipefd)
{
	...
	reselect:
		...
		if(debug & 0x1)
			fprintf(stderr, "readprinter select returned %d\n", n);
		if(n == 0) {
			/* a timeout occurred */
			if(++tocount > 4) {
				fprintf(stderr, "printer appears to be "
				                "offline.\nHP4m printers may "
				                "be out of paper.\n");
				tocount = 0;
			}
		...
		if(n > 0 && FD_ISSET(printerfd, &exceptfds)) {
			...
			if(write(pipefd, Fatal_error, 1) != 1) {
				fprintf(stderr,
				        "'fatal error' write to pipe failed\n");
			}
			progstate = FATAL_ERROR;
			continue;
		}
		...
		if(n > 0 && FD_ISSET(pipefd, &readfds)) {
			...
			if(debug & 0x1)
				fprintf(stderr, "pipe wants to be read\n");
			...
			if(debug & 0x1)
				fprintf(stderr, "readprinter: proto=%c\n",
				        proto);
			/* change state? */
			switch(proto) {
			case SENT_DATA:
			...
			case WAIT_FOR_EOJ:
				...
				break;
			default:
				fprintf(stderr, "received unknown protocol "
				                "request <%c> from sendfile\n",
				        proto);
				break;
			}
			n--;
		...
		if(n > 0 && FD_ISSET(printerfd, &readfds)) {
			...
			if(debug & 0x1)
				fprintf(stderr, "printer wants to be read\n");
			...
			if(c == 1 && *buf == '\004') {
				if(progstate == WAIT_FOR_EOJ) {
					...
					if(write(pipefd, Over_and_out, 1) !=
					   1) {
			...
			case ERROR:
				...
				if(write(pipefd, Fatal_error, 1) != 1) {
					fprintf(stderr, "Fatal_error mesg "
					                "write to pipe "
					                "failed\n");
				}
				continue;
...
sendfile(int infd, int printerfd, int pipefd)
{
	...
	do {
		...
		if(debug & 02)
			fprintf(stderr, "sendfile select returned %d\n", n);
		if(n > 0 && FD_ISSET(pipefd, &readfds)) {
			/* protocol pipe wants to be read */
			if(read(pipefd, &proto, 1) != 1) {
				fprintf(stderr, "read protocol pipe failed\n");
			...
			/* change state? */
			if(debug & 02)
				fprintf(stderr, "sendfile command - <%c>\n",
				        proto);
			switch(proto) {
			case OVER_AND_OUT:
			case END_OF_JOB:
			...
			case SEND_DATA:
				...
				if(bytesread > 0) {
					if(debug & 02)
						fprintf(stderr,
						        "writing %d bytes\n",
						        bytesread);
					if(write(printerfd, buf, bytesread) !=
					   bytesread) {
						perror("sendfile:write:");
						progstate = FATAL_ERROR;
					} else if(write(pipefd, Sent_data, 1) !=
					          1) {
						perror("sendfile:write:");
						progstate = FATAL_ERROR;
					} else {
				...
				if(i == 0) {
					if(progstate != WAIT_FOR_EOJ) {
						if(write(printerfd, "\004",
						         1) != 1) {
							perror(
							    "sendfile:write:");
							progstate = FATAL_ERROR;
						} else if(write(pipefd,
						                Wait_for_eoj,
						                1) != 1) {
							perror(
							    "sendfile:write:");
...
main(int argc, char* argv[])
{
	...
	if(usgflg) {
		fprintf(stderr,
		        "usage: %s [-b baudrate] net!host!service [infile]\n",
		        argv[0]);
		exit(2);
	}
	if(optind < argc) {
