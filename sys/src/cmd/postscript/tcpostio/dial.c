...
#define USED(x)                                                                \
	...
	if((tdest = malloc(strlen(dest) + 1)) == NULL) {
		if(dial_debug)
			fprintf(stderr, "dial: could not allocate memory\n");
		return (-1);
	}
	strcpy(tdest, dest);
	...
	if(strcmp(netname, "tcp") == 0) {
		...
		fprintf(stderr, "dial: network protocol name `%s' is invalid; "
		                "must be `tcp' or `udp'\n",
		        netname);
		return (-1);
	}
	...
	if(hp == (struct hostent*)NULL) {
		if(dial_debug)
			fprintf(stderr, "host `%s' unknown by local host\n",
			        hostname);
		return (-1);
	}
