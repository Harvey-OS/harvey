 ...
 *	    from too many string/column pairs. Stays on the current line. It
 *will
 *	    not be needed often!!
 *
 *	done
...

{
	...
	/*
	 *
	 * A simple program that translates ASCII files into PostScript. If
	 *there's more
...

{
	 ...
	 *
	 * Although any PostScript font can be used, things will only work well
	 *for
...

{
	 ...
	 *before the
	 * input files are processed. linespp (lines per page) can be set using
	 *the -l
	 * option. If it's not positive we calculate a reasonable value using
	 *the
	 * requested point size - assuming LINESPP lines fit on a page in point
	 *size
	 * POINTSIZE.
	 *
	 */
...

{
	...
	/*
	 *
	 * Finished with all the input files, so mark the end of the pages with
	 *a TRAILER
	 * comment, make sure the last page prints, and add things like the
	 *PAGES comment
...

{
	...
	/*
	 *
	 * Translates *fp_in into PostScript. Intercepts space, tab, backspace,
	 *newline,
	 * return, and formfeed. Everything else goes to oput(), which handles
	 *quoting
	 * (if needed) and escapes for nonascii characters if extended is TRUE.
	 *The
	 * redirect(-1) call forces the initial output to go to /dev/null - so
	 *stuff
	 * that formfeed() does at the end of each page goes to /dev/null rather
	 *than
...

{
	 ...
	 *we have to
	 * be careful about what's done. The first time through (up to the
	 *redirect() call)
	 * output goes to /dev/null.
	 *
	 * Adobe now recommends that the showpage operator occur after the page
	 *level
	 * restore so it can be easily redefined to have side-effects in the
	 *printer's VM.
	 * Although it seems reasonable I haven't implemented it, because it
	 *makes other
	 * things, like selectively setting manual feed or choosing an alternate
	 *paper
...

{
	...
	/*
	 *
	 * Called when we've read a newline character. The call to startline()
	 *ensures
...

{
	...
	/*
	 *
	 * Counts consecutive spaces, tabs, and backspaces and figures out where
	 *the next
	 * string should start. Once that's been done we try to choose an
	 *efficient way
	 * to output the required number of spaces. The choice is between using
	 *procedure
	 * l with a single string on the stack and L with several string and
	 *column pairs.
	 * We usually break even, in terms of the size of the output file, if we
	 *need four
	 * consecutive spaces. More means using L decreases the size of the
	 *file. For now
	 * if there are less than 6 consecutive spaces we just add them to the
	 *current
	 * string, otherwise we end that string, follow it by its starting
	 *position, and
	 * begin a new one that starts at endcol. Backspacing is always handled
	 *this way.
	 *
	 */
...

{
	...
	/*
	 *
	 * Generates a call to the PostScript procedure that processes all the
	 *text on
	 * the stack - provided stringcount is positive. If one string is on the
	 *stack
	 * the fast procedure (ie. l) is used to print the line, otherwise the
	 *slower
