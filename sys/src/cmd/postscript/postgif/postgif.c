...
void puthex(c, fp) unsigned char c;
FILE* fp;
{
...
void setcolormap(bp) int bp;
{
	...
	if(gammaflag)
		for(i = 0, p = gcmap; i < 256; i += scale, p += 3) {
			*p = (unsigned char)(pow((double)*p / 256.0, gamma) *
			                     256);
			p[1] = *p;
			p[2] = *p;
		}
...
nextbyte()
{
	...
	if(byteinibuf) {
		...
		if(byteinibuf)
			fread(ibuf, sizeof(*ibuf), byteinibuf, fp_in);
		else
			error(
			    FATAL,
			    "encounter zero byte count block before end code");
		ibufindex = 0;
		byteinibuf--;
		ibufindex++;
	}
...
int masktbl[25] = {0,        01,       03,        07,        017,
                   037,      077,      0177,      0377,      0777,
                   01777,    03777,    07777,     017777,    037777,
                   077777,   0177777,  0377777,   0777777,   01777777,
                   03777777, 07777777, 017777777, 037777777, 077777777};
...
getcode()
{
	...
	if(curstblsize < 4096)
		cs = cstbl[curstblsize];
	else
		cs = 12;
	while(bitsleft < cs) {
		oldc = (oldc & masktbl[bitsleft]) |
		       ((nextbyte() & 0377) << bitsleft);
		bitsleft += 8;
	}
	c = oldc & masktbl[cs];
	oldc = oldc >> cs;
...
writeimage()
{
	...
	if(!gray) {
		fprintf(fp_out,
		        "/colortbl currentfile %d string readhexstring\n",
		        maplength * 3);
		for(i = 0; i < maplength; i++)
			puthex(cmap[i], fp_out);
		fputs("\n", fp_out);
		for(i = maplength; i < maplength * 2; i++)
			puthex(cmap[i], fp_out);
		fputs("\n", fp_out);
		for(i = maplength * 2; i < maplength * 3; i++)
			puthex(cmap[i], fp_out);
		fputs("\npop def\n", fp_out);
		fprintf(fp_out,
		        "/graytbl currentfile %d string readhexstring\n",
		        maplength);
		for(i = 0; i < maplength; i++)
			puthex(gmap[i], fp_out);
		fputs("\npop def\n", fp_out);
	}
	fprintf(fp_out, "%s %d %d %d %d gifimage\n", gray ? "true" : "false",
	        imagewidth, imageheight, scrwidth - imageleft - imagewidth,
	        scrheight - imagetop - imageheight);
	if(gray) {
		if(interlaced) {
			int* iltbl;
...
readimage()
{
	...
	do {
		...
		dbprt(stderr, "byte count: %d\n", bytecount);
		if(bytecount)
			fread(ibuf, sizeof(*ibuf), bytecount, fp_in);
		else
			zerobytecount = 1;
	} while(!zerobytecount);
...
void redirect(pg) int pg; /* next page we're printing */
{
...
readgif()
{
	...
	if(ibuf[6] != 0)
		error(FATAL, "wrong screen descriptor");
	if(gcolormap)
		readgcolormap(bitperpixel);
	else
		setcolormap(bitperpixel);
	...
	do {
		...
		if(ibuf[0] == ',')
			...
			error(FATAL, "wrong image separator character or wrong "
			             "GIF terminator");
	} while(!terminate);
