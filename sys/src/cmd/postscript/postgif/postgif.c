...
nextbyte()
{
	...
	if(byteinibuf) {
		...
		if(byteinibuf)
			...
			error(FATAL, "encounter zero byte count block before end code");
		ibufindex = 0;
		byteinibuf--;
		ibufindex++;
	}
...
void
    redirect(pg) int pg; /* next page we're printing */
{
