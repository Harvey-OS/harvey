 ...
 * has been added, and works in combination with the simple picpack
 *pre-processor
 ...
 *
 * where name is the pathname of the original picture file and size is the
 *number
 * of bytes in the picture, which begins immediately on the next line. When
 *dpost
 ...
 * approach! Perhaps the most important problem is that troff output, with
 *in-line
...
picture(Biobufhdr* inp, char* buf)
{
	...
	/*
	 * Called from devcntrl() after an 'x X PI' command is found. The syntax
	 *of that
	 * command is:
	 *
	 *	x X PI:args
	 ...
	 *	[flags]
	 *
	 * poffset, indent, length, and totrap are given in machine units.
	 *height, width,
	 * and offset refer to the picture frame in inches, unless they're
	 *followed by
	 * the u scale indicator. flags is a string that provides a little bit
	 *of control
	 * over the placement of the picture in the frame. Rotation of the
	 *picture, in
	 * clockwise degrees, is set by the a flag. If it's not followed by an
	 *angle
	 * the current rotation angle is incremented by 90 degrees, otherwise
	 *the angle
	...
	for(i = 0; flags[i]; i++)
		...
		case 'b':
			adjy = 0;
			break; /* or bottom justify */
		case 'o':
			outline = 1;
			break; /* outline the picture */
		case 'w':
			whiteout = 1;
			break; /* white out the box */
		case 's':
			scaleboth = 1;
			break; /* scale both dimensions */
		case 'a':
			if(sscanf(&flags[i + 1], "%lf", &rot) != 1)
				rot += 90;
		}
...
/*
 * Responsible for finding and opening the next picture file. If we've
 * accumulated
 * any in-line pictures fp_pic won't be NULL and we'll look there first. If
 * *path
...
/*
 * Adds an in-line picture file to the end of temporary file *Bfp_pic. All
 * pictures
 * grabbed from the input file are saved in the same temp file. Each is
 * preceeded
