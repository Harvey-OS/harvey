...
readDESC(void)
{
	...
	descfilename = galloc(descfilename, strlen(descnameformat) + strlen(FONTDIR) + strlen(devname), "readdesc");
	...
	while(Bgetfield(Bfd, 's', token, MAXTOKENSIZE) > 0) {
		...
		case 5:
			...
			if(debug)
				Bprint(Bstderr, "ignoring horizontal resolution\n");
			break;
		case 6:
			/* I don't really care about vertical motion resolution */
			if(debug)
				Bprint(Bstderr, "ignoring vertical resolution\n");
			break;
		case 7:
		...
		case 8:
			...
			if(debug)
				Bprint(Bstderr, "ignoring special character <%s>\n", token);
			break;
		default:
			if(*token == '#')
				Brdline(Bfd, '\n');
