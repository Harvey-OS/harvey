...
#define NDESCTOKS 9
static char* desctoks[NDESCTOKS] = {"PDL",   "Encoding",  "fonts",
                                    "sizes", "res",       "hor",
                                    "vert",  "unitwidth", "charset"};
...
readDESC(void)
{
	...
	if(debug)
		Bprint(Bstderr, "readDESC()\n");
	descfilename =
	    galloc(descfilename,
	           strlen(descnameformat) + strlen(FONTDIR) + strlen(devname),
	           "readdesc");
	...
	while(Bgetfield(Bfd, 's', token, MAXTOKENSIZE) > 0) {
		...
		case 0:
			...
			if(debug)
				Bprint(Bstderr, "PDL %s\n", token);
			break;
		case 1:
			encoding =
			    galloc(encoding, strlen(token) + 1, "readdesc:");
			strcpy(encoding, token);
			if(debug)
				Bprint(Bstderr, "encoding %s\n", token);
			break;
		case 2:
			if(fontmnt <= 0) {
				if(!isdigit(*token)) {
					error(WARNING, "readdesc: expecting "
					               "number of fonts in "
					               "mount table.\n");
					return FALSE;
				}
				...
				fontmtab =
				    galloc(fontmtab, fontmnt * sizeof(char*),
				           "readdesc:");
		...
		case 4:
			...
			devres = atoi(token);
			if(debug)
				Bprint(Bstderr, "res %d\n", devres);
			break;
		case 5:
			/* I don't really care about horizontal motion
			 * resolution */
			if(debug)
				Bprint(Bstderr,
		...
		case 6:
			...
			break;
		case 7:
			/* unitwidth is the font size at which the character
			 * widths are 1:1 */
			if(!isdigit(*token)) {
				error(WARNING,
			...
			if(debug)
				Bprint(Bstderr, "unitwidth %d\n", unitwidth);
			break;
		case 8:
			/* I don't really care about this list of special
			 * characters */
			if(debug)
				Bprint(Bstderr,
