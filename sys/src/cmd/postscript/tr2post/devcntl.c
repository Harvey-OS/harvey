...
devcntl(Biobufhdr* inp)
{
	 ...
	 *The
	 * "x X ..." commands are a device dependent collection generated by
	 *troff's
	 * \X'...' request.
	 *
	 */
	...
	case 'X': /* copy through - from troff */
		...
		if(strncmp(str, "PI", sizeof("PI") - 1) == 0 ||
		   strncmp(str, "PictureInclusion",
		           sizeof("PictureInclusion") - 1) == 0) {
			picture(inp, str);
		} else if(strncmp(str, "InlinePicture",
		                  sizeof("InlinePicture") - 1) == 0) {
			 ...
			 */
		          ...
		          0) {
			beginpath(buf, FALSE);
		} else if(strncmp(str, "DrawPath", sizeof("DrawPath") - 1) ==
		          0) {
			drawpath(buf, FALSE);
		} else if(strncmp(str, "BeginObject",
		                  sizeof("BeginObject") - 1) == 0) {
			beginpath(buf, TRUE);
		} else if(strncmp(str, "EndObject", sizeof("EndObject") - 1) ==
		          0) {
			drawpath(buf, TRUE);
		} else if(strncmp(str, "NewBaseline",
		                  sizeof("NewBaseline") - 1) == 0) {
			error(FATAL, "NewBaseline not implemented yet.\n");
			/*			newbaseline(buf);
			 */
		} else if(strncmp(str, "DrawText", sizeof("DrawText") - 1) ==
		          0) {
			...
			/*			settext(buf);
			 */
		} else if(strncmp(str, "SetColor", sizeof("SetColor") - 1) ==
		          0) {
			 ...
			 */
		} else if(strncmp(str, "PS", sizeof("PS") - 1) == 0 ||
		          strncmp(str, "PostScript",
		                  sizeof("PostScript") - 1) == 0) {
			if(pageon()) {
				endstring();
				Bprint(Bstdout, "%s\n", buf);
			}
		} else if(strncmp(str, "ExportPS", sizeof("ExportPS") - 1) ==
		          0) { /* dangerous!! */
		...
		/*		 else
		                        error(WARNING, "Unknown string <%s %s>
		   after x X\n", str, buf);
		*/
