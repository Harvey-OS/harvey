...
hmot(int x)
{
	...
	if((x < expecthmot - 1) || (x > expecthmot + 1)) {
		...
		if(delta ==
		       troffontab[curtrofffontid].spacewidth * fontsize / 10 &&
		   isinstring()) {
			if(pageon())
				runeout(' ');
		} else {
...
findglyph(int trfid, Rune rune, char* stoken)
{
	...
	for(cp = &(troffontab[trfid]
	               .charent[RUNEGETGROUP(rune)][RUNEGETCHAR(rune)]);
	    *cp != 0; cp = &((*cp)->next)) {
		if((*cp)->name) {
			if(debug)
				fprint(
				    2,
				    "looking for <%s>, have <%s> in font %s\n",
				    stoken, (*cp)->name,
				    troffontab[trfid].trfontid);
...
glyphout(Rune rune, char* stoken, BOOLEAN specialflag)
{
	...
	struct psfent* psfp;
	int i, t, mi, wid;
	int fontid; /* this is the troff font table index, not the mounted font
	               table index */
	Rune r;
	...
	if(specialflag) {
		...
		/* cycle through the (troff) mounted fonts starting at the next
		 * font */
		for(mi = 0; mi < fontmnt; mi++) {
			if(troffontab[fontid].trfontid == 0)
				error(WARNING, "glyphout:troffontab[%d]."
				               "trfontid=0x%x, botch!\n",
				      fontid, troffontab[fontid].trfontid);
			if(fontmtab[mi] == 0) {
				if(debug)
					fprint(2,
					       "fontmtab[%d]=%#p, fontmnt=%d\n",
		...
		if(mi == fontmnt)
			error(FATAL,
			      "current troff font is not mounted, botch!\n");
		for(i = (mi + 1) % fontmnt; i != mi; i = (i + 1) % fontmnt) {
			...
			if(debug)
				fprint(2, "	looking through special fonts: "
				          "trying %s\n",
				       troffontab[fontid].trfontid);
		...
		/* check font 1 (if current font is not font 1) for the special
		 * character */
		if(mi != 1) {
			fontid = findtfn(fontmtab[1], TRUE);
			;
			if(debug)
				fprint(2, "	looking through font at "
				          "position 1: trying %s\n",
				       troffontab[fontid].trfontid);
			cp = findglyph(fontid, rune, stoken);
			if(*cp != 0)
	...
	if(*cp == 0) {
		error(
		    WARNING,
		    "cannot find glyph, rune=0x%x stoken=<%s> troff font %s\n",
	...
	for(i = (mi + 1) % fontmnt; i != mi; i = (i + 1) % fontmnt) {
		...
		if(debug)
			fprint(
			    2,
			    "	looking through special fonts: trying %s\n",
			    troffontab[fontid].trfontid);
		if(troffontab[fontid].special) {
			cp = findglyph(fontid, rune, stoken);
			if(*cp != 0)
...
foundit:
	...
	if(debug)
		fprint(2, "runeout(0x%x)<%C> postfontid=0x%x postcharid=0x%x "
		          "troffcharwidth=%d\n",
	...
	if(t == 0x0001) { /* character is in charlib */
		...
		if(pageon()) {
			...
			/* if char is unicode character rather than name, clean
			 * up for postscript */
