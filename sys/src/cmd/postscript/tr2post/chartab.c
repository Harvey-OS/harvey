...
settrfont(void)
{
	...
	if(fontmtab[fontpos] == 0)
		error(FATAL,
		      "Font at position %d was not initialized, botch!\n",
		      fontpos);

	curtrofffontid = findtfn(fontmtab[fontpos], 1);
	if(debug)
		fprint(2, "settrfont()-> curtrofffontid=%d\n", curtrofffontid);
	curfontpos = fontpos;
	if(curtrofffontid < 0) {
		int i;
...
setpsfont(int psftid, int fontsize)
{
	if(psftid == curpostfontid && fontsize == curfontsize)
		return;
	if(psftid >= pfnamcnt)
		error(FATAL, "Postscript font index=%d used but not defined, "
		             "there are only %d fonts\n",
		      psftid, pfnamcnt);
...
findpfn(char* fontname, int insflg)
{
	...
	if(insflg) {
		tp = galloc(pfnafontmtab,
		            sizeof(struct pfnament) * (pfnamcnt + 1),
		            "findpfn():pfnafontmtab");
		if(tp == 0)
			return (-2);
		pfnafontmtab = (struct pfnament*)tp;
		i = strlen(fontname);
		pfnafontmtab[pfnamcnt].str =
		    galloc(0, i + 1, "findpfn():pfnafontmtab[].str");
...
char postroffdirname[] =
    "/sys/lib/postscript/troff"; /* "/sys/lib/postscript/troff/"; */
char troffmetricdirname[] =
    "/sys/lib/troff/font"; /* "/sys/lib/troff/font/devutf/"; */
...
readpsfontdesc(char* fontname, int trindex)
{
	...
	filename =
	    galloc(filename, strlen(postroffdirname) + 1 + strlen(fontname) + 1,
	...
	do {
		...
		if(!errorflg) {
			...
			if(startfont != endfont) {
				error(WARNING, "font descriptions must not "
			...
			tp->psfmap =
			    galloc(tp->psfmap,
			           ++tp->psfmapsize * sizeof(struct psfent),
			...
			/*
			   ...
			   i++) {
			                                tp->charent[startfont][i].postfontid
			   = pfid;
			                                tp->charent[startfont][i].postcharid
			   = i + offset - startchar;
			                        }
			 */
			if(debug) {
...
readtroffmetric(char* fontname, int trindex)
{
	...
	filename =
	    galloc(filename, strlen(troffmetricdirname) + 4 + strlen(devname) +
	                         1 + strlen(fontname) + 1,
	...
	do {
		...
		if(strcmp(stoken, "name") == 0) {
			...
			for(cp = &(troffontab[trindex]
			               .charent[thisfont][thischar]);
			    *cp != 0; cp = &((*cp)->next))
				if((*cp)->name)
					if(strcmp((*cp)->name, " ") == 0)
						break;
			...
			(*cp)->name =
			    galloc(0, 2, "readtroffmetric: char name");
			(*cp)->next = 0;
			strcpy((*cp)->name, " ");
	...
	while(!errorflg && rv >= 0) {
		if((rv = Bgetfield(Bfd, 's', stoken, 128)) == 0) {
			errorflg = 1;
			error(WARNING,
			      "file %s:%d illegal rune token <0x%x> rv=%d\n",
		...
		if(utflen(stoken) > 1)
			...
			specharflag = FALSE;
		/* if this character is a quote we have to use the previous
		 * characters info */
		if((rv = Bgetfield(Bfd, 'r', &quote, 0)) == 0) {
			errorflg = 1;
			error(WARNING, "file %s:%d illegal width or quote "
			               "token <0x%x> rv=%d\n",
		...
		if((rv = Bgetfield(Bfd, 'd', &width, 0)) == 0) {
			errorflg = 1;
			error(WARNING,
			      "file %s:%d illegal width token <0x%x> rv=%d\n",
			      filename, line, troffchar, rv);
		} else if(rv < 0)
			break;
		if((rv = Bgetfield(Bfd, 'd', &flag, 0)) == 0) {
			errorflg = 1;
			error(WARNING,
			      "file %s:%d illegal flag token <0x%x> rv=%d\n",
			      filename, line, troffchar, rv);
		} else if(rv < 0)
			break;
		if((rv = Bgetfield(Bfd, 'd', &charnum, 0)) == 0) {
			errorflg = 1;
			error(WARNING, "file %s:%d illegal character number "
			               "token <0x%x> rv=%d\n",
			      filename, line, troffchar, rv);
		} else if(rv < 0)
			break;
	flush:
		...
		(*cp)->name =
		    galloc(0, strlen(stoken) + 1, "readtroffmetric: char name");
		(*cp)->next = 0;
		...
		if(debug) {
			...
			fprint(2, " %d  %d 0x%x %s # psfontid=0x%x "
			          "pscharid=0x%x thisfont=0x%x thischar=0x%x\n",
			       width, flag, charnum, str, (*cp)->postfontid,
			       (*cp)->postcharid, thisfont, thischar);
		}
	}
	Bterm(Bfd);
	Bflush(Bstderr);
...
findtfn(char* fontname, BOOLEAN insflg)
{
	...
	for(i = 0; i < troffontcnt; i++) {
		if(troffontab[i].trfontid == 0) {
			error(WARNING,
			      "findtfn:troffontab[%d].trfontid=0x%x, botch!\n",
	...
	if(insflg) {
		tp = (struct troffont*)galloc(
		    troffontab, sizeof(struct troffont) * (troffontcnt + 1),
		...
		/* initialize new troff font entry with name and numeric fields
		 * to 0 */
