...
copy(Biobufhdr* fin, Biobufhdr* fout, Section* s)
{
	...
	while(Bseek(fin, 0L, 1) < s->end &&
		 ...
		 * just
		 * indent comments a space, which turns DSC comments into
		 * non-DSC comments
		 * and has no effect on binary encodings, which are
		 * whitespace-blind.
		 */
		if(buf[0] == '%')
			Bputc(fout, ' ');
...
ps_include(Biobufhdr* fin, Biobufhdr* fout, int page_no, int whiteout,
           int outline, int scaleboth, double cx, double cy, double sx,
           double sy, double ax, double ay, double rot)
{
...
#define grab(n)                                                                \
	((Section*)(nglobal ? realloc((char*)global, n * sizeof(Section))      \
	...
	while((buf = Brdline(fin, '\n')) != NULL) {
		...
		if(!has("%%"))
			...
			if(i == page_no) {
				foundpbox = 1;
				sscanf(buf, "%*s %lf %lf %lf %lf", &llx, &lly,
				       &urx, &ury);
			}
