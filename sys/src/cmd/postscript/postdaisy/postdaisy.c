 ...
 *
 * A program that translates Diablo 1640 files into PostScript. Absolutely
 *nothing
 ...
 *
 * Some of document comments that are generated may not be right. Most of the
 *test
 * files I used produced a trailing blank page. I've put a check in formfeed()
 *that
 * won't print the last page if it doesn't contain any text, but PAGES comments
 *may
 * not be right. The DOCUMENTFONTS comment will also be wrong if auto underline
 *or
 * bold printing have been turned on by escape commands.
 *
 * The brute force approach used to implement horizontal and vertical tabs
 *leaves
 * much to be desired, and may not work for very small initial hmi and vmi
 *values.
 * At the very least I should have used malloc() to get space for the two
 *tabstop
 ...
 *	    vertical position y. Each string is begins at the horizontal
 *position
 *	    that preceeds it.
 *
 *	f
...

{
	...
	/*
	 *
	 * A simple program that translates Diablo 1640 files into PostScript.
	 *Nothing
	 * is guaranteed - the program not well tested and doesn't implement
	 *everything.
	 *
	 */
...

{
	 ...
	 *
	 * Although any PostScript font can be used, things will only work for
	 *constant
	 * width fonts.
	 *
	 */
...

{
	 ...
	 *you choose
	 * any PostScript font, although things will only work well for constant
	 *width
	 * fonts.
	 *
	 */
...

{
	...
	/*
	 *
	 * Translates the next input file into PostScript. The redirect(-1) call
	 *forces
	 * the initial output to go to /dev/null - so the stuff formfeed() does
	 *at the
...

{
	...
	/*
	 *
	 * Initializes the horizontal and vertical tab arrays. The way tabs are
	 *handled is
...

{
	...
	/*
	 *
	 * Called whenever we've finished with the last page and want to get
	 *ready for the
	 * next one. Also used at the beginning and end of each input file, so
	 *we have to
	 * be careful about what's done. I've added a simple test before the
	 *showpage that
	 * should eliminate the extra blank page that was put out at the end of
	 *many jobs,
...

{
	...
	/*
	 *
	 * Adjust our current vertical position. If we've passed the bottom of
	 *the page
	 * or exceeded the number of lines per page, print it and go to the
	 *upper left
	 * corner of the next page. This routine is also called from carriage()
	 *if crislf
	 * is ON.
	 *
	 */
...

{
	...
	/*
	 *
	 * Handles carriage return character. If crislf is ON we'll generate a
	 *line feed
...

{
	...
	/*
	 *
	 * Tries to figure out where the next tab stop is. Wasn't positive about
	 *this
	 * one, since hmi can change. I'll assume columns are determined by the
	 *original
	 * value of hmi. That fixes them on the page, which seems to make more
	 *sense than
...

{
	...
	/*
	 *
	 * Looks for the next vertical tab stop in the vtabstops[] array and
	 *moves to that
	 * line. If we don't find a tab we'll just move down one line -
	 *shouldn't happen.
	 *
	 */
...

{
	...
	/*
	 *
	 * Moves backwards a distance equal to the current value of hmi, but
	 *don't go
...

{
	...
	/*
	 *
	 * Handles special codes that are expected to follow an escape
	 *character. The
...

{
	...
	/*
	 *
	 * Changes the current font. Used to get in and out of auto underscore
	 *and bold
	 * printing.
	 *
	 */
...

{
	...
	/*
	 *
	 * Called whenever we want to be certain we're ready to start pushing
	 *characters
	 * into an open string on the stack. If stringcount is positive we've
	 *already
	 * started, so there's nothing to do. The first string starts in column
	 *1.
	 *
	 */
...

{
	...
	/*
	 *
	 * Generates a call to the PostScript procedure that processes the text
	 *on the
...

{
	...
	/*
	 *
	 * Takes the string we've been working on and adds it to the output
	 *file. Called
	 * when we need to adjust our horizontal position before starting a new
	 *string.
...

{
	...
	/*
	 *
	 * Responsible for adding all printing characters from the input file to
	 *the
	 * open string on top of the stack. The only other characters that end
	 *up in
	 * that string are the quotes required for special characters. Reverse
	 *printing
	 * mode hasn't been tested but it should be close. hpos and lastx should
	 *disagree
	 * each time (except after startline() does something), and that should
	 *force a
