 ...
 * the active map table that point the program to a readable Unix file. A
 *request
 * for an unlisted font or inaccessible file is ignored. All font requests are
 * ignored if the map table can't be read. In that case the program simply
 *copies
...
char* agv[];
{
	/*
	 *
	 * Host resident font downloader. The input files are assumed to be part
	 *of a
...
readmap(void)
{
	...
	/*
	 * Initializes the map table by reading an ASCII mapping file. If
	 * mapname begins
	 * with a / it's the map table. Otherwise hostfontdir, mapname, and
	 * suffix are
	 * combined to build the final pathname. If we can open the file we read
	 * it all
	 * into memory, erase comments, and separate the font and file name
	 * pairs. When
	 * we leave next points to the next free slot in the map[] array. If
	 * it's zero
	...
	if((fd = open(path, 0)) != -1) {
		...
		if((stringspace = malloc(sbuf.st_size + 2)) == NULL)
			error(FATAL, "no memory for %s (%d bytes)", path,
			      sbuf.st_size + 2);
		...
		for(ptr = stringspace;; next++) {
			...
			if(map[next].file == NULL)
				error(FATAL,
				      "map table format error - check %s",
				      path);
		}
	}
}
...
readresident(void)
{
	...
	/*
	 * Reads a file that lists the resident fonts for a particular printer
	 * and marks
	 * each font as already downloaded. Nothing's done if the file can't be
	 * read or
	 * there's no mapping file. Comments, as in the map file, begin with a %
	 * and
...
arguments(void)
{
	 ...
	 *input files
	 * we'll translate stdin. Assumes input files are part of a single
	 *PostScript
...
download(void)
{
	...
	/*
	 *
	 * If next is zero the map table is empty and all we do is copy the
	 *input file
	 * to stdout. Otherwise we read the input file looking for
	 *%%DocumentFonts: or
	 * continuation comments, add any accessible fonts to the output file,
	 *and then
	 * append the input file. When reading stdin we append lines to fp_temp
	 *and
	 * recover them when we're ready to copy the input file. fp_temp will
	 *often
	 * only contain part of stdin - if there's no %%DocumentFonts: (atend)
	 *comment
...
void copyfonts(list) char* list;
{
	...
	/*
	 * list points to a %%DocumentFonts: or continuation comment. What
	 * follows the
	 * the keyword will be a list of fonts separated by white space (or
	 * (atend)).
	 * Look for each font in the map table and if it's found copy the font
	 * file to
	 * stdout (once only).
	 */
	...
	while((font = strtok(NULL, " \t\n")) != NULL) {
		...
		if((n = lookup(font)) < next) {
			if(*map[n].file != '/') {
				if((path = malloc(strlen(hostfontdir) +
				                  strlen(map[n].file) + 2)) ==
				   NULL)
					error(FATAL, "no memory");
				sprintf(path, "%s/%s", hostfontdir,
				        map[n].file);
				cat(path);
				free(path);
			} else
...
copyinput(void)
{
	/*
	 *
	 * Copies the input file to stdout. If fp_temp isn't NULL seek to the
	 *start and
	 * add it to the output file - it's a partial (or complete) copy of
	 *stdin made
	 * by download(). Then copy fp_in, but only seek to the start if it's
	 *not stdin.
	 *
	 */
...
lookup(font) char* font;
{
	int i;

	/*
	 *
	 * Looks for *font in the map table. Return the map table index if found
	 *and
