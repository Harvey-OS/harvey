...
void
struct_name(Lextok *n, Symbol *v, int xinit)
{	Symbol *tl;
	Lextok *tmp;
	...
	if (tmp->sym->type == STRUCT)
	{	printf(".");
		struct_name(tmp, tl, 0);
		return;
	}
	printf(".%s", tl->name);
	if (tmp->sym->nel > 1)
		printf("[%d]", eval(tmp->lft));
}
...
int
fill_struct(Symbol *z, int cur, int ov, char *gather, Symbol **sp)
{	Lextok *fp, *tl;
	int ix;
	int nc = cur;
	int nv = ov;
	...
	{	if (tl->sym->type == STRUCT)
		{	sprintf(newpref, "%s.%s",
				gather, tl->sym->name);
			nc = fill_struct(tl->sym, nc, nv, newpref, sp);
			if (nc < 0)
...
Lextok *
mk_explicit(Lextok *n, int Ok, int Ntyp) /* produce a single ',' list of fields */
{	Lextok *bld = ZN, *x;
	int i, cnt;
	...
	if (!Ok) fatal("incomplete structure ref %s", n->sym->name);
