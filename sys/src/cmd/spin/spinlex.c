		...
		for(tmp = seqnames; tmp; tmp = tmp->nxt)
			if(tmp->nm->type == CODE_FRAG ||
			   tmp->nm->type == CODE_DECL) {
				...
				cnt = 0;
				while(*q &&
				      cnt < 1024) /* pangen1.h allows 2048 */
				{
					switch(*q) {
					case '"':
						fprintf(fd, "\\\"");
...
void c_track(Symbol* s, Symbol* t, Symbol* stackonly) /* name, size */
{
	...
	if(stackonly != ZS) {
		if(strcmp(stackonly->name, "\"Matched\"") == 0)
			...
			non_fatal("expecting '[Un]Matched', saw %s",
			          stackonly->name);
		else
...
c_add_globinit(FILE* fd)
{
	...
	for(r = c_added; r; r = r->nxt) {
		...
		} else if(strncmp(r->t->name, " Hidden ", strlen(" Hidden ")) ==
		          0) {
			for(q = r->ival->name; *q; q++) {
				if(*q == '\"')
					*q = ' ';
...
c_preview(void)
{
	...
	if(c_tracked)
		...
		for(r = c_added; r; r = r->nxt)
			if(strncmp(r->t->name, " Global ",
			           strlen(" Global ")) != 0 &&
			   strncmp(r->t->name, " Hidden ",
			           strlen(" Hidden ")) != 0 &&
			   strncmp(r->t->name, " Local", strlen(" Local")) !=
			       0) {
...
c_add_hidden(FILE* fd)
{
	...
	for(r = c_added; r; r = r->nxt) /* pickup hidden decls */
		if(strncmp(r->t->name, "\"Hidden\"", strlen("\"Hidden\"")) ==
		   0) {
...
plunk_reverse(FILE* fd, IType* p, int matchthis)
{
	...
	if(!p->nm->context && p->nm->type == matchthis) {
		fprintf(fd, "\n/* start of %s */\n", p->nm->name);
		z = (char*)p->cn;
		while(*z == '\n' || *z == '\r' || *z == '\\')
			z++;
...
check_inline(IType* tmp)
{
	...
	for(p = rdy; p; p = p->nxt) {
		...
		if(strstr((char*)tmp->cn, buf)) {
			printf("spin: in proctype %s, ref to object in "
			       "proctype %s\n",
			       X->n->name, p->n->name);
             ...
             Lextok* n) /* check a condition for c_expr with preconditions */
{
	...
	if(n->ntyp == C_EXPR) {
		...
		if(tmp->prec) {
			fprintf(fd, "if (!(%s)) { if (!readtrail) { depth++; ",
			        tmp->prec);
			fprintf(fd, "trpt++; trpt->pr = II; trpt->o_t = t;");
			fprintf(fd,
			        "trpt->st = tt; uerror(\"%s\"); continue; } ",
			        tmp->prec);
			fprintf(fd, "else { printf(\"pan: precondition false: "
			            "%s\\n\"); ",
			        tmp->prec);
                  ...
                  int gencode) /* c_code with precondition */
{
	...
	if(how && tmp->prec) {
		...
		fprintf(fd, " uerror(\"%s\"); continue; ", tmp->prec);
		fprintf(fd, "} else { ");
	...
	case '}':
		...
		if(--nest <= 0) {
			*p = '\0';
			...
			if(firstchar)
				printf("%3d: %s, warning: empty inline "
				       "definition (%s)\n",
...
again:
	...
	case ':':
		...
		break;
	case '-':
		c = follow('>', SEMI, follow('-', DECR, '-'));
		break;
	case '+':
		c = follow('+', INCR, '+');
		break;
	case '<':
		c = follow('<', LSHIFT, follow('=', LE, LT));
		break;
	case '>':
...
check_name(char* s)
{
	...
	if(Inlining >= 0 && !ReDiRect) {
		...
		for(i = 0; t; t = t->rgt, i++) /* formal pars */
			if(!strcmp(
			       s,
			       t->lft->sym->name) /* varname matches formal */
			   &&
			   strcmp(s, Inline_stub[Inlining]->anms[i]) !=
			       0) /* actual pars */
			{
#if 0
			if (verbose&32)
...
#endif
				for(tt = Inline_stub[Inlining]->params; tt;
				    tt = tt->rgt)
					if(!strcmp(Inline_stub[Inlining]
					               ->anms[i],
					           tt->lft->sym
					               ->name)) { /* would be
					                             cyclic if
					                             ...
					                             */
						printf(
						    "spin: %s:%d replacement "
						    "value: %s\n",
						    oFname->name ? oFname->name
						                 : "--",
						    lineno, tt->lft->sym->name);
						fatal("formal par of %s "
						      "contains replacement "
						      "value",
						      Inline_stub[Inlining]
						          ->nm->name);
						yylval->ntyp = tt->lft->ntyp;
						yylval->sym =
						    lookup(tt->lft->sym->name);
						return NAME;
					}

				/* check for occurrence of param as field of
				 * struct */
				{
					char* ptr =
					    Inline_stub[Inlining]->anms[i];
...
yylex(void)
{
	...
	if(hold) {
		...
		if(last == '}' && c != PROCTYPE && c != INIT && c != CLAIM &&
		   c != SEP && c != FI && c != OD && c != '}' && c != UNLESS &&
		   c != SEMI && c != EOF) {
			hold = c;
			last = 0;
			return SEMI; /* insert ';' */
		}
		if(c == SEMI) { /* if context, we're not in a typedef
