...
announce(char* w)
{
	if(columns) {
		...
		firstrow = 1;
		if(columns == 2) {
			sprintf(Buf, "%d:%s", run->pid - Have_claim,
			        run->n->name);
	...
	if(w)
		...
		whoruns(1);
	printf("creates proc %2d (%s)", run->pid - Have_claim, run->n->name);
	if(run->priority > 1)
...
enable(Lextok* m)
{
	...
	for(p = rdy; p; p = p->nxt)
		if(strcmp(s->name, p->n->name) == 0) {
			...
			return run->pid - Have_claim +
			       Skip_claim; /* effective simu pid */
		}
...
pickproc(RunList* Y)
{
	...
	if(!interactive ||
	   depth < jumpsteps) { /* was: j = (int) Rand()%(nproc-nstop); */
...
multi_claims(void)
{
	...
	if(nclaims > 1) {
		...
		printf("  choose which one with ./pan -N name (defaults to -N "
		       "%s)\n",
		       q ? q->n->name : "--");
	}
}
...
sched(void)
{
	...
	while(X) {
		...
		go = 1;
		if(X->prov && X->pc && !(X->pc->status & D_ATOM) &&
		   !eval(X->prov)) {
...
setlocals(RunList* r)
{
	...
	for(walk = all_names; walk; walk = walk->next) {
		sp = walk->entry;
		if(sp && sp->context &&
		   strcmp(sp->context->name, r->n->name) == 0 && sp->Nid >= 0 &&
		   (sp->type == UNSIGNED || sp->type == BIT ||
		    sp->type == MTYPE || sp->type == BYTE || sp->type == CHAN ||
		    sp->type == SHORT || sp->type == INT ||
		    sp->type == STRUCT)) {
			if(!findloc(sp))
...
findloc(Symbol* s)
{
	...
	if(!X) { /* fatal("error, cannot eval '%s' (no proc)", s->name); */
		return ZS;
	}
...
p_talk(Element* e, int lnr)
{
	...
	if(Have_claim && X && X->pid == 0 && lastnever != newnever && e) {
		if(xspin) {
			printf("MSC: ~G line %d\n", newnever);
#if 0
	...
	if(e) {
		...
		printf("%s:%d (state %d)", nbuf, e->n ? e->n->ln : -1,
		       e->seqno);
		if(!xspin &&
	...
	for(Y = run; Y; Y = Y->nxt)
		if(--i == prno) {
			if(strcmp(Y->n->name, n->lft->sym->name) != 0) {
				printf("spin: remote reference error on "
				       "'%s[%d]'\n",
				       n->lft->sym->name, prno - added);
				non_fatal("refers to wrong proctype '%s'",
				          Y->n->name);
			}
