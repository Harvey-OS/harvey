...
pickproc(RunList *Y)
{
	...
	if(!interactive || depth < jumpsteps) { /* was: j = (int) Rand()%(nproc-nstop); */
	...
	try_more:
		...
		for(X = run; X; Y = X, X = X->nxt) {
			if(!X->nxt || X->nxt->pid > 255 || j < Choices[X->nxt->pid]) {
...
sched(void)
{
	...
	while(X) {
		...
		go = 1;
		if(X->prov && X->pc && !(X->pc->status & D_ATOM) && !eval(X->prov)) {
		...
		if(go && (e = eval_sub(X->pc))) {
			...
			if(oX != X || (X->pc->status & (ATOM | D_ATOM))) /* new 5.0 */
			{
				e = silent_moves(e);
				notbeyond = 0;
			}
			...
			if(X->pc && (X->pc->status & (ATOM | L_ATOM)) && (notbeyond == 0 || oX != X)) {
			...
			if(X->pc && X->pc->n && X->pc->n->ntyp == '@' && X->pid == (nproc - nstop - 1)) {
				if(X != run && Y != NULL)
					Y->nxt = X->nxt;
				else
					run = X->nxt;
...
complete_rendez(void)
{
	...
	for(j = nproc - nstop; j > 0; j--) {
		if(X != orun && (!X->prov || eval(X->prov)) && (e = eval_sub(X->pc))) {
			if(TstOnly) {
				X = orun;
				Rvous = 0;
				goto out;
...
addsymbol(RunList *r, Symbol *s)
{
	...
	for(t = r->symtab; t; t = t->next)
		if(strcmp(t->name, s->name) == 0 && (old_scope_rules || strcmp((const char *)t->bscp, (const char *)s->bscp) == 0))
...
setlocals(RunList *r)
{
	...
	for(walk = all_names; walk; walk = walk->next) {
		sp = walk->entry;
		if(sp && sp->context && strcmp(sp->context->name, r->n->name) == 0 && sp->Nid >= 0 && (sp->type == UNSIGNED || sp->type == BIT || sp->type == MTYPE || sp->type == BYTE || sp->type == CHAN || sp->type == SHORT || sp->type == INT || sp->type == STRUCT)) {
			if(!findloc(sp))
...
findloc(Symbol *s)
{
	...
	for(r = X->symtab; r; r = r->next)
		if(strcmp(r->name, s->name) == 0 && (old_scope_rules || strcmp((const char *)r->bscp, (const char *)s->bscp) == 0))
			break;
	if(!r) {
		addsymbol(X, s);
		r = X->symtab;
