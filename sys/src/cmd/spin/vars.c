...
cast_val(int t, int v, int w)
{
	...
	if(t == PREDEF || t == INT || t == CHAN)
		...
		/*	u = (unsigned)(v& ((1<<w)-1));		problem when
		 * w=32	*/
		u = (unsigned)(v &
		               (~0u >> (8 * sizeof(unsigned) - w))); /* doug */
	}
	...
	for(walk = all_names; walk; walk = walk->next) {
		...
		for(j = 0; j < sp->nel; j++) {
			...
			if(sp->nel > 1 || sp->isarray)
				printf("[%d]", j);
			printf(" = ");
			sr_mesg(stdout, prefetch, sp->type == MTYPE);
			printf("\n");
			if(limited_vis && (sp->hidden & 2)) {
				...
				printf("MSC: ~G %s %s\n", sp->name, Buf);
				printf(
				    "%3d:\tproc %3d (TRACK) line   1 \"var\" ",
				    depth, colpos);
				printf("(state 0)\t[printf('MSC: "
				       "globvar\\\\n')]\n");
				printf("\t\t%s", sp->name);
				if(sp->nel > 1 || sp->isarray)
					printf("[%d]", j);
...
dumplocal(RunList* r)
{
	...
	for(z = s; z; z = z->next) {
		...
		for(i = 0; i < z->nel; i++) {
			...
			printf("\t\t%s(%d):%s", r->n->name, r->pid - Have_claim,
			       z->name);
