...
rm_selfrefs(Symbol *s, Lextok *i)
{
	...
	if(i->ntyp == NAME && strcmp(i->sym->name, s->name) == 0 && ((!i->sym->context && !s->context) || (i->sym->context && s->context && strcmp(i->sym->context->name, s->context->name) == 0))) {
		lineno = i->ln;
		Fname = i->fn;
...
dumpglobals(void)
{
	...
	for(walk = all_names; walk; walk = walk->next) {
		sp = walk->entry;
		if(!sp->type || sp->context || sp->owner || sp->type == PROCTYPE || sp->type == PREDEF || sp->type == CODE_FRAG || sp->type == CODE_DECL || (sp->type == MTYPE && ismtype(sp->name)))
			continue;

		if(sp->type == STRUCT) {
			if((verbose & 4) && !(verbose & 64) && (sp->setat < depth && jumpsteps != depth)) {
				continue;
			}
...
dumplocal(RunList *r)
{
	...
	for(z = s; z; z = z->next) {
		...
		for(i = 0; i < z->nel; i++) {
			...
			if((verbose & 4) && !(verbose & 64) && (z->setat < depth && jumpsteps != depth))
				continue;
