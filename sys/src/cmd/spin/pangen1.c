...
here:
	...
	if(nclaims >
	   1) { /* this is the structure that goes into the state-vector
	         ...
	         * this claim records the types and states of all subclaims in
	         * an array
	         * NB: not sure if we need the first 3 fields in this structure
	         *     it's here for now to avoid breaking some possible
	         * dependence
	         * in the calculations above, we were already taking into
	         * account
...
genaddproc(void)
{
	...
	fprintf(tc, "		channels[j]->modified = 1; /* channel index "
	            "moved */\n");
	fprintf(tc, "	#endif\n");
	fprintf(tc, "	#ifndef TRIX_ORIG\n");
	fprintf(tc, "	if (whichway > 0)\n");
	fprintf(tc, "	{	for (j = now._nr_pr + now._nr_qs - 1; j >= "
	            "now._nr_pr; j--)\n");
	fprintf(tc, "			now._ids_[j] = now._ids_[j-1];\n");
	fprintf(tc, "	} else\n");
	fprintf(tc, "	{	for (j = now._nr_pr; j < now._nr_pr + "
	            "now._nr_qs; j++)\n");
	fprintf(tc, "			now._ids_[j] = now._ids_[j+1];\n");
	fprintf(tc, "	}\n");
	fprintf(tc, "	#endif\n");
...
void c_wrapper(FILE* fd) /* allow pan.c to print out global sv entries */
{
	...
	for(p = rdy; p; p = p->nxt) {
		fprintf(fd, "	case %d:\n", p->tn);
		if(c_splurge_any(p)) {
			fprintf(fd, "	\tprintf(\"local vars proc %%d "
			            "(%s):\\n\", pid);\n",
			        p->n->name);
			c_splurge(fd, p);
		} else {
	...
	for(j = 0; j < 8; j++)
		for(walk = all_names; walk; walk = walk->next) {
			...
			if((sp->hidden & 1) && sp->type == Types[j]) {
				if(sp->context || sp->owner)
					fatal("cannot hide non-globals (%s)",
					      sp->name);
				if(sp->type == CHAN)
					fatal("cannot hide channels (%s)",
					      sp->name);
...
void
do_var(FILE* ofd, int dowhat, char* s, Symbol* sp, char* pre, char* sep,
       char* ter)
{
	int i;
	char* ptr = sp ? sp->name : "";
	...
	case INIV:
		...
		if(sp->nel == 1 && sp->isarray == 0) {
			if(dowhat == LOGV) {
				...
				fprintf(ofd, "\t\t%s%s%s%s", pre, s, sp->name,
				        sep);
				do_init(ofd, sp);
			}
			fprintf(ofd, "%s", ter);
			...
			if(sp->ini && sp->ini->ntyp == CHAN) {
				...
				fprintf(
				    ...
				    sp->nel);
				fprintf(ofd, "\t\t{\n");
				fprintf(ofd, "\t\t\t%s%s%s[l_in]%s", pre, s,
				        sp->name, sep);
				if(dowhat == LOGV)
...
put_ptype(char* s, int i, int m0, int m1, enum btypes b)
{
	...
	else if(LstSet || k == 0) /* 5.0, added condition */
	{
		...
		if((LstSet->type != BIT && LstSet->type != UNSIGNED) ||
		   LstSet->nel != 1) {
			fprintf(th, "Offsetof(P%d, %s) - %d*sizeof(", i,
			        LstSet->name, LstSet->nel);
		}
		switch(LstSet->type) {
		case UNSIGNED:
			fprintf(th, "%d", (nBits + 7) / 8);
...
typ2c(Symbol* sp)
{
	...
	case BIT:
		...
		if(!(sp->hidden & 1) && (verbose & 32))
			printf("spin: warning: bit-array %s[%d] mapped to "
			       "byte-array\n",
			       sp->name, sp->nel);
	...
	case STRUCT:
		...
		fprintf(th, "\tstruct %s %s", sp->Snm->name, sp->name);
		LstSet = ZS;
		break;
	case CODE_FRAG:
	case PREDEF:
...
genaddqueue(void)
{
	...
	for(q = qtab; q; q = q->nxt) {
		...
		if(has_sorted) {
			...
			for(j = 0; j < q->nflds; j++) {
				fprintf(tc,
				        ...
				        j, buf0, j);
				fprintf(tc, "\t\t\tif (fld%d < %s%d) ", j, buf0,
				        j);
		...
		fprintf(tc, "\t\t{	if (args_given > %d)\n", q->nflds);
		fprintf(tc,
		        "\t\t		uerror(\"too many parameters in send "
		        "stmnt\");\n");
		fprintf(tc, "\t\t	else\n");
		fprintf(tc, "\t\t		uerror(\"too few parameters in "
		            "send stmnt\");\n");
		fprintf(tc, "\t\t}\n");
		fprintf(tc, "\t\tbreak;\n");
	}
