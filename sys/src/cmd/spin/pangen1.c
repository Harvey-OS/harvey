...
here:
	...
	if(nclaims > 1) { /* this is the structure that goes into the state-vector
...
end_labs(Symbol *s, int i)
{
	...
	for(l = labtab; l; l = l->nxt)
		for(j = 0; ln[j].n; j++) {
			if(strncmp(l->s->name, ln[j].s, ln[j].n) == 0 && strcmp(l->c->name, s->name) == 0) {
				fprintf(tc, "\t%s[%d][%d] = 1;\n",
					ln[j].t, i, l->e->seqno);
				acceptors += ln[j].m;
				progressors += ln[j].p;
	...
	if(dowhat == INIV) { /* initialize in order of declaration */
		...
		for(j = 0; j < 8; j++)
			for(h = 0; h <= 1; h++)
				for(walk = all_names; walk; walk = walk->next) {
					sp = walk->entry;
					if(sp->context && !sp->owner && sp->type == Types[j] && ((h == 0 && (sp->nel == 1 && sp->isarray == 0)) || (h == 1 && (sp->nel > 1 || sp->isarray == 1))) && strcmp(s, sp->context->name) == 0) {
						switch(dowhat) {
						case LOGV:
...
c_splurge_any(ProcList *p)
{
	...
	if(p->b != N_CLAIM && p->b != E_TRACE && p->b != N_TRACE)
		for(walk = all_names; walk; walk = walk->next) {
			sp = walk->entry;
			if(!sp->context || sp->type == 0 || strcmp(sp->context->name, p->n->name) != 0 || sp->owner || (sp->hidden & 1) || (sp->type == MTYPE && ismtype(sp->name)))
				continue;
...
c_wrapper(FILE *fd) /* allow pan.c to print out global sv entries */
{
	...
	for(walk = all_names; walk; walk = walk->next) {
		sp = walk->entry;
		if(sp->context || sp->owner || (sp->hidden & 1) || (sp->type == MTYPE && ismtype(sp->name)))
			continue;
	...
	case UNSIGNED:
		if(sp->hidden & 1)
			...
			fprintf(th, "\tunsigned %s : %d",
				sp->name, sp->nbits);
		LstSet = sp;
		if(nBits % wsbits > 0 && wsbits - nBits % wsbits < sp->nbits) { /* must padd to a word-boundary */
