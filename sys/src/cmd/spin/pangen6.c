...
AST_par_chans(ProcList *p) /* find local chan's init'd to chan passed as param */
{
	...
	for(walk = all_names; walk; walk = walk->next) {
		sp = walk->entry;
		if(sp && sp->context && strcmp(sp->context->name, p->n->name) == 0 && sp->Nid >= 0 /* not itself a param */
...
AST_relpar(char *s)
{
	...
	for(T = expl_par; T; T = (T == expl_par) ? expl_var : (FSM_trans *)0)
		for(t = T; t; t = t->nxt) {
			if(t->relevant & 1)
				for(u = t->Val[0]; u; u = u->nxt) {
					if(u->n->sym->type && u->n->sym->context && strcmp(u->n->sym->context->name, s) == 0) {
						if(verbose & 32) {
...
AST_tagruns(void)
{
	...
	for(a = ast; a; a = a->nxt) {
		if(a->p->b == N_CLAIM || a->p->b == I_PROC || a->p->b == E_TRACE || a->p->b == N_TRACE) {
	...
	for(f = a->fsm; f; f = f->nxt)       /* control states */
		for(t = f->t; t; t = t->nxt) /* transitions    */
		{
			if(t->step && t->step->n && t->step->n->ntyp == 's')
				for(u = t->Val[0]; u; u = u->nxt) {
					if(Sym_typ(u->n) == CHAN && ((u->special & USE) && !(u->special & DEREF_USE))) {
#if 0
				printf("%s -- (%d->%d) -- ",
...
AST_dump_rel(void)
{
	...
	for(walk = all_names; walk; walk = walk->next) {
		Symbol *s;
		s = walk->entry;
		if(!s->setat && (s->type != MTYPE || s->ini->ntyp != CONST) && s->type != STRUCT /* report only fields */
...
AST_suggestions(void)
{
	...
	for(walk = all_names; walk; walk = walk->next) {
		...
		if(s->colnr == 2 /* only used in conditionals */
		   && (s->type == BYTE || s->type == SHORT || s->type == INT || s->type == MTYPE)) {
			if(!banner) {
				banner = 1;
...
AST_var_init(void) /* initialized vars (not chans) - hidden assignments */
{
	...
	for(walk = all_names; walk; walk = walk->next) {
		...
		if(sp && !sp->context								       /* globals */
		   && sp->type != PROCTYPE && sp->ini && (sp->type != MTYPE || sp->ini->ntyp != CONST) /* not mtype defs */
		   && sp->ini->ntyp != CHAN) {
			x = nn(ZN, TYPE, ZN, ZN);
			x->sym = sp;
	...
	for(a = ast; a; a = a->nxt) {
		if(a->p->b != N_CLAIM && a->p->b != E_TRACE && a->p->b != N_TRACE) /* has no locals */
			for(walk = all_names; walk; walk = walk->next) {
				sp = walk->entry;
				if(sp && sp->context && strcmp(sp->context->name, a->p->n->name) == 0 && sp->Nid >= 0 /* not a param */
