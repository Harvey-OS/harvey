...
name_def_use(Lextok* n, int code)
{
	...
	name_def_indices(
	    n, USE | (code & (~DEF))); /* not def, but perhaps deref */
}
...
def_use(Lextok* now, int code)
{
	...
	if(now)
		...
		case RUN:
			name_def_use(now,
			             USE); /* procname - not really needed */
...
AST_indirect(FSM_use* uin, FSM_trans* t, char* cause, char* pn)
{
	...
	for(u = t->Val[0]; u; u = u->nxt)
		if(u != uin && (u->special & (USE | DEREF_USE))) {
			...
			name_AST_track(u->n,
			               u->special); /* add to slice criteria */
		}
}
		...
		for(f = a->fsm; f; f = f->nxt)
			for(t = f->t; t; t = t->nxt)
				if(t->step && AST_scoutrun(t->step->n)) {
					AST_indirect((FSM_use*)0, t, ":run:",
					             a->p->n->name);
					/* BUT, not all actual params are
					 * relevant */
				}
}
...
AST_def_use(AST* a)
{
	...
	for(f = a->fsm; f; f = f->nxt)       /* control states */
		for(t = f->t; t; t = t->nxt) /* all edges */
		{
			cur_t = t;
			rel_use(
			    t->Val[0]); /* redo Val; doesn't cover structs */
			rel_use(t->Val[1]);
...
void AST_track(Lextok* now, int code) /* called from main.c */
{
	...
	if(now)
		...
		case 'R':
			...
			for(v = now->rgt; v; v = v->rgt)
				AST_track(v->lft,
				          code); /* a deeper eval can add USE */
			break;
...
AST_dump_rel(void)
{
	...
	for(walk = all_names; walk; walk = walk->next) {
		...
		if(!s->setat && (s->type != MTYPE || s->ini->ntyp != CONST) &&
		   ...
		   && s->type != PROCTYPE && !s->owner &&
		   sputtype(buf, s->type)) {
			if(!banner) {
				banner = 1;
				printf("spin: redundant vars (for given "
				       "property):\n");
			}
			printf("\t");
			symvar(s);
	...
	no_good:
		if(banner == 1 || banner == 2) {
			printf("spin: proctype %s defines a %s process\n",
			       a->p->n->name, banner == 1 ? "source" : "sink");
			talked |= banner;
		} else if(banner == 3) {
	...
	for(t = f->t; t; t = t->nxt) {
		if(t->step && t->step->n)
			...
			default:
				...
				/*
				        AST_track is called here for a blockable
				   stmnt from which
				        a relevant stmnmt was shown to be
				   reachable
				        for a condition this makes all USEs
				   relevant
				        but for a channel operation it only
				   makes the executability
				        relevant -- in those cases, parameters
				   that aren't already
				        relevant may be replaceable with
				   arbitrary tokens
				 */
				if(AST_suspect(t)) {
					printf("spin: possibly redundant "
					       "parameters in: ");
					comment(stdout, t->step->n, 0);
					printf("\n");
				}
...
AST_ctrl(AST* a)
{
	...
	/* 2: keep only 2-marked stmnts from which 1-marked stmnts can be
	 * reached */
	for(f = a->fsm; f; f = f->nxt) {
		fsm_tbl[f->from] = f;
	...
	for(f = a->fsm; f; f = f->nxt) {
		...
		if(hit) /* at least one outgoing trans can block */
			for(t = f->t; t; t = t->nxt) {
				...
				AST_spread(
				    a, t->to); /* and spread to all guards */
			}
	}
...
static void AST_alias_analysis(void) /* aliasing of promela channels */
{
	...
	for(a = ast; a; a = a->nxt)
		AST_sends(
		    a); /* collect chan-names that are send across chans */
...
AST_slice(void)
{
	...
	if(!slicer) {
		printf("spin: warning: no slice criteria found (no assertions "
		       "and no claim)\n");
		spurious = 1;
	}
	...
	if(!spurious || (verbose & 32)) {
		...
		if(!AST_dump_rel() /* relevant variables */
		   && spurious)
			printf("spin: no redundancies found (for given "
			       "property)\n");
	}
	AST_suggestions();
	...
	for(a = ast; a; a = a->nxt) {
		...
		for(f = a->p->p; f; f = f->rgt)        /* types */
			for(t = f->lft; t; t = t->rgt) /* formals */
			{
				...
				c = (t->ntyp != ',')
				        ? t
				        : t->lft; /* the formal parameter */
				AST_mk1(a->p->n->name, c,
				        cnt); /* all matching run statements */
			}
	}
}
...
AST_var_init(void) /* initialized vars (not chans) - hidden assignments */
{
	...
	for(walk = all_names; walk; walk = walk->next) {
		...
		if(sp && !sp->context /* globals */
		   ...
		   (sp->type != MTYPE ||
		    sp->ini->ntyp != CONST) /* not mtype defs */
		   &&
		   sp->ini->ntyp != CHAN) {
			x = nn(ZN, TYPE, ZN, ZN);
			x->sym = sp;
		...
		if(a->p->b != N_CLAIM && a->p->b != E_TRACE &&
		   a->p->b != N_TRACE) /* has no locals */
			for(walk = all_names; walk; walk = walk->next) {
				sp = walk->entry;
				if(sp && sp->context &&
				   strcmp(sp->context->name, a->p->n->name) ==
				       0 &&
				   sp->Nid >= 0 /* not a param */
				   && sp->type != LABEL && sp->ini &&
				   sp->ini->ntyp != CHAN) {
					x = nn(ZN, TYPE, ZN, ZN);
					x->sym = sp;
					AST_add_explicit(x, sp->ini);
...
init_dom(AST* a)
{
	...
	for(f = a->fsm; f; f = f->nxt) {
		...
		f->dom = (uint32_t*)emalloc(a->nwords * sizeof(uint32_t));
...
dom_perculate(AST* a, FSM_state* f)
{
	...
	if(on < a->nwords) {
		on = a->nwords;
		ndom = (uint32_t*)emalloc(on * sizeof(uint32_t));
	}
	...
	if(AST_Round == 1) /* computed once, reused in every round */
		for(a = ast; a; a = a->nxt) {
			...
			if(verbose & 32) {
				...
				printf("states=%d (max %d), words = %d, bpw "
				       "%d, overflow %d\n",
				       a->nstates, o_max, a->nwords, (int)BPW,
				       (int)(a->nstates % BPW));
			}
			...
			oi = a->i_st;
			if(fsm_tbl[0]
			       ->seen) /* end-state reachable - else leave it */
				a->i_st = 0; /* becomes initial state */

			dom_forward(a); /* reverse dominance -- don't redo
			                   reachability! */
			act_dom(a);     /* mark proper subgraphs, if any */
			AST_checkpairs(
			    a); /* selectively place 2 scratch-marks */
