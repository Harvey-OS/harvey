	...
	more:
		...
		if((in2 == in1 || !isvalid(*(in2 - 1))) &&
			...
			*in2 = '\0';
			...
			if(strlen(in1) + strlen(d[i].trg) + strlen(in2 + j) >=
			   GENEROUS) {
				printf("spin: macro expansion overflow %s -> "
				       "%s ?\n",
				       d[i].src, d[i].trg);
				return in1;
			}
...
check_ifdef(char* p)
{
	...
	for(i = 0; i < nr_defs; i++)
		if(d[i].exists && !strcmp(d[i].src, q))
			return d[i].trg;
	return (char*)0;
}
...
do_if(char* p)
{
	...
	if(!is_simple(q) && !is_simple(check_ifdef(q))) {
...
process(char* q, int lno, char* fnm)
{
	...
	for(i = 0; i < (int)(sizeof(s) / sizeof(struct Directives)); i++)
		if(!strncmp(s[i].directive, p, s[i].len)) {
			if(s[i].interp && !printing[if_depth])
				return 1;
			fprintf(outpp, "#line %d \"%s\"\n", lno, fnm);
