...
disambiguate(void)
{
	...
	for(walk = all_names; walk; walk = walk->next) {
		sp = walk->entry;
		if(sp->type != 0 && sp->type != LABEL && strlen((const char *)sp->bscp) > 1) {
			char *n = (char *)emalloc(strlen((const char *)sp->name) + strlen((const char *)sp->bscp) + 1);
...
lookup(char *s)
{
	...
	if(old_scope_rules) { /* same scope - global refering to global or local to local */
		for(sp = symtab[h]; sp; sp = sp->next) {
			if(strcmp(sp->name, s) == 0 && samename(sp->context, context) && samename(sp->owner, owner)) {
				return sp; /* found */
			}
		}
	} else { /* added 6.0.0: more traditional, scope rule */
		for(sp = symtab[h]; sp; sp = sp->next) {
			if(strcmp(sp->name, s) == 0 && samename(sp->context, context) && (strcmp((const char *)sp->bscp, CurScope) == 0 || strncmp((const char *)sp->bscp, CurScope, strlen((const char *)sp->bscp)) == 0) && samename(sp->owner, owner)) {
	...
	if(context) /* in proctype, refers to global */
		for(sp = symtab[h]; sp; sp = sp->next) {
			if(strcmp(sp->name, s) == 0 && !sp->context && samename(sp->owner, owner)) {
				return sp; /* global */
			}
		}
...
setmtype(Lextok *m)
{
	...
	{
		if(!n->lft || !n->lft->sym || n->lft->ntyp != NAME || n->lft->lft) /* indexed variable */
