	...
	for(walk = all_names; walk; walk = walk->next) {
		...
		if(sp->type != 0 && sp->type != LABEL &&
		   strlen((const char*)sp->bscp) > 1) {
			char* n =
			    (char*)emalloc(strlen((const char*)sp->name) +
			                   strlen((const char*)sp->bscp) + 1);
...
lookup(char* s)
{
	...
	if(old_scope_rules) { /* same scope - global refering to global or local
	                         to local */
...
checkrun(Symbol* parnm, int posno)
{
	...
	if(!(res & 4) || !(res & 8)) {
		...
		printf("proctype %s, '%s %s' could be declared",
		       parnm->context ? parnm->context->name : "", buf,
		       parnm->name);
		printf(" '%s %s'\n", buf2, parnm->name);
	}
}
...
void setptype(Lextok* n, int t, Lextok* vis) /* predefined types */
{
	...
	while(n) {
		...
		if(t == UNSIGNED) {
			if(n->sym->nbits < 0 || n->sym->nbits >= 32)
				fatal("(%s) has invalid width-field",
				      n->sym->name);
			if(n->sym->nbits == 0) {
				n->sym->nbits = 16;
		...
		if(vis) {
			if(strncmp(vis->sym->name, ":hide:", (size_t)6) == 0) {
				...
				has_hidden++;
				if(t == BIT)
					fatal("bit variable (%s) cannot be "
					      "hidden",
					      n->sym->name);
			} else if(strncmp(vis->sym->name, ":show:",
			                  (size_t)6) == 0) {
				n->sym->hidden |= 2;
			} else if(strncmp(vis->sym->name, ":local:",
			                  (size_t)7) == 0) {
				n->sym->hidden |= 64;
			}
		}
...
setxus(Lextok* p, int t)
{
	...
	if(m_loss && t == XS) {
		printf("spin: warning, %s:%d, xs tag not compatible with -m "
		       "(message loss)\n",
	...
	for(walk = all_names; walk; walk = walk->next) {
		if(!walk->entry->owner)
			...
			case UNSIGNED:
				...
				if(!separate && !walk->entry->context &&
				   !has_code && deadvar)
					walk->entry->hidden |=
					    1; /* auto-hide */
