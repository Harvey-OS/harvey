...
hookup(void)
{
	...
	for(e = Al_El; e; e = e->Nxt)
		if(e->n && (e->n->ntyp == ATOMIC || e->n->ntyp == NON_ATOMIC || e->n->ntyp == D_STEP))
			(void)huntstart(e);
}
...
okay:
	...
	while(fscanf(fd, "%d:%d:%d\n", &depth, &pno, &nst) == 3) {
		...
		if(dothis->n->ntyp == D_STEP) {
			...
			do {
				g = eval_sub(og);
				if(g && depth >= jumpsteps && ((verbose & 32) || ((verbose & 4) && not_claim()))) {
					if(columns != 2) {
						p_talk(og, 1);
		...
		keepgoing:
			...
			if(X != NULL) {
				...
				if(X->pc)
					X->pc = huntele(X->pc, 0, a);
			}

			if(depth >= jumpsteps && ((verbose & 32) || ((verbose & 4) && not_claim()))) /* -v or -p */
			{
				if(columns != 2) {
					p_talk(dothis, 1);
		...
		if(Have_claim && X && X->pid == 0 && dothis->n && lastclaim != dothis->n->ln) {
