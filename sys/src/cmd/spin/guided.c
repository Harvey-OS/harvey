...
okay:
	...
	while(fscanf(fd, "%d:%d:%d\n", &depth, &pno, &nst) == 3) {
		...
		if(!X) {
			if(verbose & 32) {
				...
				printf(" max %d (%d - %d + %d) claim %d",
				       nproc - nstop + Skip_claim, nproc, nstop,
				       Skip_claim, Have_claim);
		...
		if(Have_claim && X && X->pid == 0 && dothis->n &&
		   lastclaim != dothis->n->ln) {
			...
			if(columns == 2) {
				...
				printf("Never claim moves to line %d\t[",
				       lastclaim);
				comment(stdout, dothis->n, 0);
				printf("]\n");
			}
