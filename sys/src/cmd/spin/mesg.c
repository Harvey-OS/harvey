...
try_slot:
	...
	for(m = n->rgt, j = 0; m && j < q->nflds; m = m->rgt, j++)
		if((m->lft->ntyp == CONST && q->contents[i * q->nflds + j] != m->lft->val) || (m->lft->ntyp == EVAL && q->contents[i * q->nflds + j] != eval(m->lft->lft))) {
...
nochan_manip(Lextok *p, Lextok *n, int d)
{
	...
	if(!n || n->ntyp == LEN || n->ntyp == RUN || n->ntyp == FULL || n->ntyp == NFULL || n->ntyp == EMPTY || n->ntyp == NEMPTY)
		return;
...
no_internals(Lextok *n)
{
	...
	if((strlen(sp) == strlen("_nr_pr") && strcmp(sp, "_nr_pr") == 0) || (strlen(sp) == strlen("_p") && strcmp(sp, "_p") == 0)) {
