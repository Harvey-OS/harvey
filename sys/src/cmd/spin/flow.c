...
is_skip(Lextok *n)
{
	return (n->ntyp == PRINT || n->ntyp == PRINTM || (n->ntyp == 'c' && n->lft && n->lft->ntyp == CONST && n->lft->val == 1));
}
...
prune_opts(Lextok *n)
{
	...
	if(!n || (context && claimproc && strcmp(context->name, claimproc) == 0))
		return;
...
do_unless(Lextok *No, Lextok *Es)
{
	...
	if(No->ntyp == NON_ATOMIC) {
		No->sl->nxt = Sl;
		Sl = No->sl;
	} else if(No->ntyp == ':' && (No->lft->ntyp == NON_ATOMIC || No->lft->ntyp == ATOMIC || No->lft->ntyp == D_STEP)) {
		int tok = No->lft->ntyp;
...
add_seq(Lextok *n)
{
	...
	innermost = n;
	e = colons(n);
	if(innermost->ntyp != IF && innermost->ntyp != DO && innermost->ntyp != UNLESS)
		add_el(e, cur_s->this);
}
...
set_lab(Symbol *s, Element *e)
{
	...
	for(l = labtab; l; l = l->nxt) {
		if(strcmp(l->s->name, s->name) == 0 && l->c == context && l->uiid == cur_uiid) {
...
get_labspec(Lextok *n)
{
	 ...
	 */
	for(l = labtab; l; l = l->nxt) {
		if(strcmp(s->name, l->s->name) == 0 && s->context == l->s->context) {
			anymatch = l;
...
has_lab(Element *e, int special)
{
	...
	for(l = labtab; l; l = l->nxt) {
		if(e != l->e)
			continue;
		if(special == 0 || ((special & 1) && !strncmp(l->s->name, "accept", 6)) || ((special & 2) && !strncmp(l->s->name, "end", 3)) || ((special & 4) && !strncmp(l->s->name, "progress", 8)))
			return (l->s);
	}
	return ZS;
}
...
fix_dest(Symbol *c, Symbol *a) /* c:label name, a:proctype name */
{
	...
	for(l = labtab; l; l = l->nxt) {
		if(strcmp(c->name, l->s->name) == 0 && strcmp(a->name, l->c->name) == 0) /* ? */
			break;
	}
	if(!l) {
...
find_lab(Symbol *s, Symbol *c, int markit)
{
	...
	for(l = labtab; l; l = l->nxt) {
		if(strcmp(s->name, l->s->name) == 0 && strcmp(c->name, l->c->name) == 0) {
			l->visible |= markit;
			return (l->e->seqno);
		}
	}
...
valid_name(Lextok *a3, Lextok *a5, Lextok *a8, char *tp)
{
	...
	if(a3->sym->type == CHAN || a3->sym->type == STRUCT || a3->sym->isarray != 0) {
...
for_index(Lextok *a3, Lextok *a5)
{
	...
	if(a3->sym->type == STRUCT) {
		...
		z0 = a5->sym->ini;
		if(!z0 || z0->val <= 0 || z0->rgt->ntyp != STRUCT || z0->rgt->rgt != NULL) {
