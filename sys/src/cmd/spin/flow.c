...
is_skip(Lextok* n)
{
	return (n->ntyp == PRINT || n->ntyp == PRINTM ||
	        (n->ntyp == 'c' && n->lft && n->lft->ntyp == CONST &&
	         n->lft->val == 1));
}
	...
	for(e = Al_El; e; e = e->Nxt) {
		...
		case D_STEP:
			...
			while(f && f->n->ntyp == '.')
				f = f->nxt;
			if(0)
				printf("link %d, {%d .. %d} -> %d (ntyp=%d) "
				       "was %d\n",
				       e->seqno, e->n->sl->this->frst->seqno,
				       e->n->sl->this->last->seqno,
				       f ? f->seqno : -1, f ? f->n->ntyp : -1,
				       e->n->sl->this->last->nxt
				           ? e->n->sl->this->last->nxt->seqno
				           : -1);
				...
				if(e->n->sl->this->last->nxt->n->ntyp != GOTO) {
					if(!f ||
					   e->n->sl->this->last->nxt->seqno !=
					       f->seqno)
						non_fatal(
						    "unexpected: loose ends",
						    (char*)0);
				} else
					e->n->sl->this->last =
					    e->n->sl->this->last->nxt;
				/*
				 * fix_dest can push a goto into the nxt
				 * position
				 * in that case the goto wins and f is not
				 * needed
...
if_seq(Lextok* n)
{
	...
	if(prev_z && ref_chans && prev_z->this->frst->n->ntyp == ELSE) {
		prev_z->this->frst->n->val = 1;
		has_badelse++;
		if(has_xu) {
			fatal("invalid use of 'else' combined with i/o and "
			      "xr/xs assertions,",
			      (char*)0);
		} else {
...
unless_seq(Lextok* n)
{
	...
	for(z = s; z; z = z->nxt) {
		Element* x;
		printf("\t%d,%d,%d :: ", z->this->frst->Seqno,
		       z->this->extent->Seqno, z->this->last->Seqno);
...
#ifdef DEBUG
	printf("add_el %d after %d -- ", e->Seqno,
	       (s->last) ? s->last->Seqno : -1);
...
set_lab(Symbol* s, Element* e)
{
	...
	for(l = labtab; l; l = l->nxt) {
		if(strcmp(l->s->name, s->name) == 0 && l->c == context &&
		   l->uiid == cur_uiid) {
...
void fix_dest(Symbol* c, Symbol* a) /* c:label name, a:proctype name */
{
	...
	if(!l) {
		...
		if(context == a)
			printf("spin: cannot remote ref a label inside the "
			       "same proctype\n");
		return;
	}
	if(!l->e || !l->e->n)
...
{
	if(!t || !t->ini || !t->ini->rgt || !t->ini->rgt->sym ||
	   t->ini->rgt->rgt) {
		fatal("chan %s in for should have only one field (a typedef)",
		      t->name);
	}
...
valid_name(Lextok* a3, Lextok* a5, Lextok* a8, char* tp)
{
	...
	if(a5->ntyp == CONST && a8->ntyp == CONST && a5->val > a8->val) {
		non_fatal("start value for %s exceeds end-value",
		          a3->sym->name);
	}
}
...
for_index(Lextok* a3, Lextok* a5)
{
	...
	if(a3->sym->type == STRUCT) {
		...
		sprintf(tmp_nm, "_f0r_t3mp%s",
		        CurScope); /* make sure it's unique */
		...
		/* retrieve  message from the right slot -- for now: rotate
		 * contents */
		in_for = 0;
		add_seq(nn(a5, 'r', a5, expand(a3, 1))); /* receive */
		add_seq(
		    nn(a5, 's', a5, expand(a3, 1))); /* put back in to rotate */
		in_for = 1;
		return z3;
	} else {
		if(a5->sym->isarray == 0 || a5->sym->nel <= 0) {
...
walk_atomic(Element* a, Element* b, int added)
{
	...
	for(f = a;; f = f->nxt) {
		...
		case ATOMIC:
			if(verbose & 32)
				printf("spin: warning, %s:%d, atomic inside %s "
				       "(ignored)\n",
				       f->n->fn->name, f->n->ln,
				       (added) ? "d_step" : "atomic");
			goto mknonat;
		...
		case UNLESS:
			if(added) {
				printf("spin: error, %s:%d, unless in d_step "
				       "(ignored)\n",
				       f->n->fn->name, f->n->ln);
			}
		}
