...
complete_transition(Succ_List *sl, Guard *g)
{
	...
	for(w = g; w; w = w->nxt) {
		if(w->t->ntyp == CONST && w->t->val == 1) {
			continue;
		} else if(w->t->ntyp == 'c' && w->t->lft->ntyp == CONST && w->t->lft->val == 1) {
			continue; /* 'true' */
		}
...
state_body(OneState *s, Guard *guard)
{
	...
	for(sl = s->succ; sl; sl = sl->nxt) {
		...
		for(i = 0; i < nclaims; i++) {
			...
			if(!e || e->n->ntyp == NON_ATOMIC || e->n->ntyp == DO || e->n->ntyp == IF) {
				s = &(sl->s->state);
				y = push_dsts(s->combo);
				if(!y) {
					if(once++ == 0) {
...
claim_has_accept(ProcList *p)
{
	...
	for(l = labtab; l; l = l->nxt) {
		if(strcmp(l->c->name, p->n->name) == 0 && strncmp(l->s->name, "accept", 6) == 0) {
			return 1;
		}
	}
	return 0;
...
prune_dead(void)
{
	...
	do {
		cnt = 0;
		for(q = sd; q; q = q->nxt) { /* if successor is deadend, remove it
