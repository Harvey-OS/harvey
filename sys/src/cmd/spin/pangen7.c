			...
			if(!e || e->n->ntyp == NON_ATOMIC || e->n->ntyp == DO ||
			   e->n->ntyp == IF) {
				...
				} else if(!y->nxt) /* self-loop transition */
				{
					if(!not_printing)
						printf(" /* self-loop */\n");
				} else { /* non_fatal("loop in state body", 0);
				            ** maybe ok */
				}
				continue;
			} else {
...
check_special(int* nrs)
{
	...
	for(j = 0; spl[j].n; j++) /* 2 special label prefixes */
	{
		...
		for(p = rdy, i = 0; p; p = p->nxt, i++) /* check each claim */
		{
			...
			{
				if(strcmp(l->c->name, p->n->name) ==
				       0 /* right claim */
				   &&
				   l->e->seqno == nrs[i] /* right state */
				   &&
				   strncmp(l->s->name, spl[j].s, spl[j].n) ==
				       0) {
					if(j == 1) /* accept state */
					{
						char buf[32];
	...
	do {
		...
		for(q = sd; q;
		    q = q->nxt) { /* if successor is deadend, remove it
		                   * unless it's a move to the end-state of the
		                   * claim
		                   */
			last = (Succ_List*)0;
...
sync_product(void)
{
	...
	matrix =
	    (Element****)emalloc(sizeof(Element***) * nclaims); /* claims */
	...
	for(n = 0; n < nclaims; n++) {
		...
		matrix[n] =
		    (Element***)emalloc(sizeof(Element**) * nst); /* rows */
		for(i = 0; i < nst; i++)                          /* cols */
		{
			matrix[n][i] =
			    (Element**)emalloc(sizeof(Element*) * nst);
		}
	}
	...
	for(; e; e = e->nxt) {
		...
		if(e->sub) /* IF or DO */
		{
			...
			for(h = e->sub; h; h = h->nxt) {
				...
				if(t->ntyp != 'c') {
					fatal("product, 'else' combined with "
					      "non-condition",
					      0);
				}
