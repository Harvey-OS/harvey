	...
	for(i = 0; i < Jt; i++) {
		...
		if(j == Tj) {
			...
			if(Jumpto[i] == OkBreak) {
				...
				non_fatal("goto %s breaks from d_step seq",
				          buf);
			}
		}
	}
...
putCode(FILE* fd, Element* f, Element* last, Element* next, int isguard)
{
	...
	for(e = f; e; e = e->nxt) {
		...
		if(!e->sub) {
			...
			case NON_ATOMIC:
				h = e->n->sl;
				putCode(fd, h->this->frst, h->this->extent,
				        e->nxt, 0);
				break;
			case BREAK:
				if(LastGoto)
					break;
				if(e->nxt) {
					i = target(huntele(e->nxt, e->status,
					                   -1))->Seqno;
			...
			case GOTO:
				if(LastGoto)
					break;
				i = huntele(get_lab(e->n, 1), e->status, -1)
				        ->Seqno;
				fprintf(fd, "\t\tgoto S_%.3d_0;	", i);
				fprintf(fd, "/* 'goto' */\n");
				Dested(i);
			...
			for(h = e->sub, i = 1; h; h = h->nxt, i++) {
				...
				putCode(fd, h->this->frst, h->this->extent, N,
				        1);
				Level--;
				fprintf(fd, "%s: /* 3 */\n", &NextOpt[5]);
				LastGoto = 0;
