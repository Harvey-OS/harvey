...
alldone(int estatus)
{
	...
	if(seedy && !analyze && !export_ast && !s_trail && !preprocessonly && depth > 0)
		printf("seed used: %d\n", SeedUsed);
	...
	if(strncmp(PreProc, "gcc -E -x c", strlen("gcc -E -x c")) == 0) {
		if(stat("/bin/gcc-4.exe", (struct stat *)&x) == 0 /* for PCs with cygwin */
		   || stat("c:/cygwin/bin/gcc-4.exe", (struct stat *)&x) == 0) {
			PreProc = "gcc-4 -E -x c";
		} else if(stat("/bin/gcc-3.exe", (struct stat *)&x) == 0 || stat("c:/cygwin/bin/gcc-3.exe", (struct stat *)&x) == 0) {
			PreProc = "gcc-3 -E -x c";
		}
	}
#endif
...
main(int argc, char *argv[])
{
	...
	if(argc > 1) {
		...
		if(strncmp(argv[1], "progress", (size_t)8) == 0 || strncmp(argv[1], "accept", (size_t)6) == 0)
...
trapwonly(Lextok *n /* , char *unused */)
{
	...
	if(i != MTYPE && i != BIT && i != BYTE && i != SHORT && i != INT && i != UNSIGNED)
		return;
