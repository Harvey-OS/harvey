...
void setutype(Lextok* p, Symbol* t, Lextok* vis) /* user-defined types */
{
	int oln = lineno;
	Symbol* ofn = Fname;
	Lextok* m, *n;
	...
	for(n = p; n; n = n->rgt) {
		...
		if(vis) {
			if(strncmp(vis->sym->name, ":hide:", (size_t)6) == 0) {
				n->sym->hidden |= 1;
				has_hidden++;
			} else if(strncmp(vis->sym->name, ":show:",
			                  (size_t)6) == 0)
				n->sym->hidden |= 2;
			else if(strncmp(vis->sym->name, ":local:", (size_t)7) ==
			        0)
				n->sym->hidden |= 64;
		}
...
int Rval_struct(Lextok* n, Symbol* v, int xinit) /* n varref, v valref */
{
	Symbol* tl;
	Lextok* tmp;
	int ix;
...
int Lval_struct(Lextok* n, Symbol* v, int xinit, int a) /* a = assigned value */
{
	Symbol* tl;
	Lextok* tmp;
	int ix;
		...
		for(tl = fp->lft; tl; tl = tl->rgt) {
			if(tl->sym->type == STRUCT) {
				if(tl->sym->nel > 1 || tl->sym->isarray)
					fatal("array of structures in param "
					      "list, %s",
					      tl->sym->name);
...
void
walk_struct(FILE* ofd, int dowhat, char* s, Symbol* z, char* a, char* b,
            char* c)
{
	Lextok* fp, *tl;
	char eprefix[128];
	...
	for(ix = 0; ix < z->nel; ix++) {
		...
		for(fp = z->Sval[ix]; fp; fp = fp->rgt)
			for(tl = fp->lft; tl; tl = tl->rgt) {
				if(tl->sym->type == STRUCT)
					walk_struct(ofd, dowhat, eprefix,
					            tl->sym, a, b, c);
				else
					do_var(ofd, dowhat, eprefix, tl->sym, a,
					       b, c);
			}
	}
}
...
retrieve(Lextok** targ, int i, int want, Lextok* n, int Ntyp)
{
	...
	for(fp = n; fp; fp = fp->rgt)
		for(tl = fp->lft; tl; tl = tl->rgt) {
			if(tl->sym->type == STRUCT) {
				j = retrieve(targ, j, want, tl->sym->Slst,
				             Ntyp);
				if(j < 0) {
