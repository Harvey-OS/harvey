...
eligible(FSM_trans* v)
{
	...
	if(!lt                      /* dead end */
	   ...
	   || lt->ntyp == ATOMIC ||
	   lt->ntyp == NON_ATOMIC /* used for inlines -- should be able to
	                             handle this */
	   ||
...
#define SINGLES
	...
	the same is not true for non-rv send operations
			...
			if(lt->ntyp == 'c' /* potentially blocking stmnts */
			   || lt->ntyp == 'r' ||
			   (lt->ntyp == 's' &&
			    u_sync == 0)) /* added !u_sync in 4.1.3 */
			{
				if(!canfill_in(
				       t)) /* atomic, non-global, etc. */
					continue;
