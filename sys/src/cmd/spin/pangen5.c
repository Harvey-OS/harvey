...
eligible(FSM_trans *v)
{
	...
	if(!lt										   /* dead end */
	   ...
	   || lt->ntyp == ATOMIC || lt->ntyp == NON_ATOMIC				   /* used for inlines -- should be able to handle this */
	   || lt->ntyp == IF || lt->ntyp == C_CODE || lt->ntyp == C_EXPR || has_lab(el, 0) /* any label at all */

	   || lt->ntyp == DO || lt->ntyp == UNLESS || lt->ntyp == D_STEP || lt->ntyp == ELSE || lt->ntyp == '@' || lt->ntyp == 'c' || lt->ntyp == 'r' || lt->ntyp == 's')
		return 0;
...
FSM_MERGER(/* char *pname */ void) /* find candidates for safely merging steps */
{
	...
	for(f = fsm; f; f = f->nxt)	  /* all states */
		for(t = f->t; t; t = t->nxt) /* all edges */
		{
			...
			if(lt->ntyp == 'c' || lt->ntyp == 'r' || lt->ntyp == 's') /* blocking stmnts */
...
#define SINGLES
	...
	the same is not true for non-rv send operations
			...
			if(lt->ntyp == 'c'					   /* potentially blocking stmnts */
			   || lt->ntyp == 'r' || (lt->ntyp == 's' && u_sync == 0)) /* added !u_sync in 4.1.3 */
			{
...
ana_var(FSM_trans *t, Lextok *now, int usage)
{
	...
	if(now->sym->name[0] == '_' && (strcmp(now->sym->name, "_") == 0 || strcmp(now->sym->name, "_pid") == 0 || strcmp(now->sym->name, "_last") == 0))
		return;
...
ana_stmnt(FSM_trans *t, Lextok *now, int usage)
{
	...
	case 'r':
		...
		for(v = now->rgt; v; v = v->rgt) {
			...
			else if(v->lft->ntyp != CONST && now->ntyp != 'R') /* was v->lft->ntyp */
	...
	for(e = s->frst; e; e = e->nxt) {
		...
		if(e->n->ntyp == UNLESS)
			...
			for(h = e->sub; h; h = h->nxt) {
				...
				if(g->n->ntyp != 'c' || g->n->lft->ntyp != CONST || g->n->lft->val != 0 || g->esc)
					FSM_EDGE(From, To, e);
				/* else it's a dead link */
			}
			for(h = e->sub; h; h = h->nxt)
				ana_seq(h->this);
		} else if(e->n->ntyp == ATOMIC || e->n->ntyp == D_STEP || e->n->ntyp == NON_ATOMIC) {
			t = e->n->sl->this;
			g = huntstart(t->frst);
			t->last->nxt = e->nxt;
			To = g->seqno;
