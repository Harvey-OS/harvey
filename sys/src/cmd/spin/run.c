...
eval_sub(Element *e)
{
	...
	} else if(e->sub) /* true for IF, DO, and UNLESS */
	{
		...
		if(interactive && !MadeChoice && !E_Check && !Escape_Check && !(e->status & (D_ATOM)) && depth >= jumpsteps) {
			printf("Select stmnt (");
			whoruns(0);
			printf(")\n");
			if(nproc - nstop > 1) {
		...
		for(z = e->sub, j = 0; z; z = z->nxt) {
			j++;
			if(interactive && !MadeChoice && !E_Check && !Escape_Check && !(e->status & (D_ATOM)) && depth >= jumpsteps && z->this->frst && (xspin || (verbose & 32) || Enabled0(z->this->frst))) {
		...
		if(interactive && depth >= jumpsteps && !Escape_Check && !(e->status & (D_ATOM)) && !E_Check) {
			if(!MadeChoice) {
				char buf[256];
				if(xspin)
		...
		for(i = 0, z = e->sub; i < j + k; i++) {
			if(z->this->frst && z->this->frst->n->ntyp == ELSE) {
				bas_else = z->this->frst;
				has_else = (Rvous) ? ZE : bas_else->nxt;
				if(!interactive || depth < jumpsteps || Escape_Check || (e->status & (D_ATOM))) {
					z = (z->nxt) ? z->nxt : e->sub;
					continue;
				}
			}
			if(z->this->frst && ((z->this->frst->n->ntyp == ATOMIC || z->this->frst->n->ntyp == D_STEP) && z->this->frst->n->sl->this->frst->n->ntyp == ELSE)) {
...
eval(Lextok *now)
{
	if(now) {
		...
		case RSHIFT:
			return (eval(now->lft) >> eval(now->rgt));
		case '?':
			return (eval(now->lft) ? eval(now->rgt->lft)
					       : eval(now->rgt->rgt));
...
interprint(FILE *fd, Lextok *n)
{
	...
	if(strlen(Buf) >= 4096)
		fatal("printf string too long", 0);
	return 1;
}
