...
eval_sub(Element* e)
{
	...
	} else if(e->sub) /* true for IF, DO, and UNLESS */
	{
		...
		for(z = e->sub, j = 0; z; z = z->nxt) {
			j++;
			if(interactive && !MadeChoice && !E_Check &&
			   !Escape_Check && !(e->status & (D_ATOM)) &&
			   depth >= jumpsteps && z->this->frst &&
			   (xspin || (verbose & 32) ||
			    Enabled0(z->this->frst))) {
				if(z->this->frst->n->ntyp == ELSE) {
					has_else =
					    (Rvous) ? ZE : z->this->frst->nxt;
					nr_else = j;
					continue;
				}
				printf("\tchoice %d: ", j);
		...
		for(i = 0, z = e->sub; i < j + k; i++) {
			...
			if(z->this->frst &&
			   ((z->this->frst->n->ntyp == ATOMIC ||
			    ...
			    z->this->frst->n->sl->this->frst->n->ntyp ==
			        ELSE)) {
