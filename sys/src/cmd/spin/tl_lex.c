	...
	while((c != want || nesting > 0) && c != -1 && i < 2047) {
		if(islower((int)c)) {
			...
			while(j < (int)sizeof(peek_buf) &&
			      isalnum((int)(c = tl_peek(i)))) {
				peek_buf[j++] = c;
				i++;
			}
			c = 0; /* make sure we don't match on z or want on the
			          peekahead */
			if(j >= (int)sizeof(peek_buf)) {
				peek_buf[j - 1] = '\0';
				fatal("name '%s' in ltl formula too long",
				      peek_buf);
			}
			peek_buf[j] = '\0';
...
read_upto_closing(int z)
{
	...
	c = tl_Getchar();
	while((c != want || nesting > 0) && c != -1 &&
	      i < 2047) /* yytext is 2048 */
	{
		yytext[i++] = c;
		if(c == z) {
			nesting++;
