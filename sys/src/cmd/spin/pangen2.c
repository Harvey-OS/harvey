...
int pid_is_claim(int p) /* Pid (p->tn) to type (p->b) */
{
	ProcList* r;
...
gensrc(void)
{
	...
	fprintf(th, "#define HAS_CODE\n"); /* doesn't seem to cause measurable
	                                      overhead */
	...
	if(has_enabled || has_pcvalue || has_badelse || has_last) {
...
doless:
	...
	switch(separate) {
	case 0:
		fprintf(tc, "#include \"pan.h\"\n");
		break;
	case 1:
		fprintf(tc, "#include \"pan_s.h\"\n");
		break;
	case 2:
		fprintf(tc, "#include \"pan_t.h\"\n");
		break;
	}
	...
	if(separate != 2) {
		...
		if(separate == 1) {
			fprintf(tm, "	if (II == 0)\n");
			fprintf(tm, "	{ _m = step_claim(trpt->o_pm, "
			            "trpt->tau, tt, ot, t);\n");
	...
	if(separate != 2) {
		...
		fprintf(tc, "\t}\n");
		fprintf(tc, "\n}\n");
		fprintf(tc, "int\nstep_claim(int o_pm, int tau, int tt, int "
		            "ot, Trans *t)\n");
		fprintf(tc, "{	int forw = t->forw; int _m = 0; extern char "
		            "*noptr; int II=0;\n");
		fprintf(tc, "	extern State now;\n");
...
putproc(ProcList* p)
{
	...
	if(pid_is_claim(Pid) && separate == 1) {
	...
	if(!pid_is_claim(Pid) && separate == 2) {
		fprintf(th, "extern short src_ln%d[];\n", Pid);
		fprintf(th, "extern uchar *loopstate%d;\n", Pid);
		return;
	}
...
put_sub(Element* e, int Tt0, int Tt1)
{
	...
	if(e->n->ntyp == D_STEP) {
		...
		fprintf(tm, "/* STATE %d - %s:%d - [", e->seqno, e->n->fn->name,
		        e->n->ln);
		comment(tm, e->n, 0);
		fprintf(tm, "] */\n\t\t");

		if(s->last->n->ntyp == BREAK)
			OkBreak = target(huntele(s->last->nxt, s->last->status,
			                         -1))->Seqno;
		else
			OkBreak = -1;
		...
		fprintf(tt, "settr(%d,%d,%d,%d,%d,\"", e->Seqno,
		        D_ATOM | inherit, a, uniq - 1, uniq - 1);
		...
		fprintf(tt, "settr(%d,%d,0,0,0,\"", e->Seqno,
		...
		blurb(tt, e);
		fprintf(tt, "\tT->nxt\t= ");
		fprintf(tt, "settr(%d,%d,%d,0,0,\"", e->Seqno,
...
nr_bup(Element* e)
{
	...
	for(u = e->dead; u; u = u->nxt) {
		...
		case 2: /* dead after write */
			if(e->n->ntyp == ASGN && e->n->rgt->ntyp == CONST &&
			   e->n->rgt->val == 0)
				break;
			nr++;
			break;
...
doforward(FILE* tm_fd, Element* e)
{
	...
	if(deadvar && !has_code)
		for(u = e->dead; u; u = u->nxt) {
			...
			case 1: /* dead after read -- add asgn of rval -- needs
			           bup */
...
case_cache(Element* e, int a)
{
	...
	if(ccache && !pid_is_claim(Pid) && Pid != eventmapnr &&
	   (Cached = prev_case(e, Pid))) {
		...
		fprintf(tm, "] (%d:%d - %d) same as %d (%d:%d - %d) */\n",
		        e->merge_start, e->merge, e->merge_in, casenr,
		        Cached->e->merge_start, Cached->e->merge,
		        Cached->e->merge_in);

		goto gotit;
	}

	fprintf(tm, "\tcase %d: /* STATE %d - %s:%d - [", uniq++, e->seqno,
	        e->n->fn->name, e->n->ln);
	comment(tm, e->n, 0);
	nrbups = (e->merge || e->merge_start) ? nrhops(e) : nr_bup(e);
	fprintf(tm, "] (%d:%d:%d - %d) */\n\t\t", e->merge_start, e->merge,
	        nrbups, e->merge_in);
	...
	if(pid_is_claim(Pid)) {
		...
		fprintf(tm, "	{\tprintf(\"depth %%ld: Claim %%s (%%d), state "
		            "%%d (line %%d)\\n\",\n\t\t");
		fprintf(tm, "	\t\tdepth, procname[spin_c_typ[nn]], nn, ");
		fprintf(tm, "(int) ((Pclaim *)this)->_p, src_claim[ (int) "
		            "((Pclaim *)this)->_p ]);\n\t\t");
		...
		fprintf(tm, "{	static int reported%d = 0;\n\t\t", e->seqno);
		fprintf(tm, "	if (verbose && !reported%d)\n\t\t", e->seqno);
		fprintf(tm, "	{	printf(\"depth %%d: Claim, state %%d "
		            "(line %%d)\\n\",\n\t\t");
		fprintf(tm, "			(int) depth, (int) ((Pclaim "
		            "*)this)->_p, ");
...
put_el(Element* e, int Tt0, int Tt1)
{
	...
	generic_case:
		fprintf(tt, "\ttrans[%d][%d]\t= ", Pid, e->seqno);
		fprintf(tt, "settr(%d,%d,%d,%d,0,\"", e->Seqno,
		        e->status & ATOM, a, casenr);
		break;
#ifndef PRINTF
	case PRINT:
		goto non_generic;
	...
	for(f = s->frst; f; f = f->nxt) {
		...
		{ /* consider jump from an atomic without globals into
		   ...
		   */
			...
			if(g &&
			   !seq_has_el(
			       s, g)) /* not internal to this atomic/dstep */

#else
			if(g && !(f->status & L_ATOM) &&
			   !(g->status & (ATOM | L_ATOM)))
#endif
			{
				fprintf(tt, "\t/* mark-down line %d status %d "
				            "= %d */\n",
				        f->n->ln, f->status,
				        (f->status & D_ATOM));
...
proc_is_safe(const Lextok* n)
{
	...
	for(p = rdy; p; p = p->nxt) {
		if(strcmp(n->sym->name, p->n->name) ==
		   0) { /* printf("proc %s safety: %d\n", p->n->name,
		           p->unsafe); */
			return (p->unsafe != 0);
		}
	}
...
putstmnt(FILE* fd, Lextok* now, int m)
{
	...
	case TIMEOUT:
		...
		if(GenCode)
			printf("spin: warning, %s:%d, 'timeout' in d_step "
			       "sequence\n",
			       Fname->name, lineno);
		/* is okay as a guard */
		break;
	...
	case RUN:
		...
		if(i > Npars) { /* printf("\t%d parameters used, max %d
		                   expected\n", i, Npars); */
	...
	case 's':
		...
		if(has_xu) {
			...
			putname(fd, "{	q_S_check(", now->lft, m,
			        ", II);\n\t\t");
			fprintf(fd, "}\n");
			if(has_sorted && now->val == 1) {
				putname(fd, "\t\tif (q_claim[", now->lft, m,
				        "]&1)\n\t\t"); /* &1 iso &2 */
				fprintf(fd, "{	uerror(\"sorted send on xr "
				            "channel violates po "
				            "reduction\");\n\t\t");
				fprintf(fd, "}\n");
			}
	...
	case 'r':
		...
		if(TestOnly) {
			...
			if(now->val == 0 || now->val == 2) {
				for(v = now->rgt, i = j = 0; v;
				    v = v->rgt, i++) {
					if(v->lft->ntyp == CONST) {
						cat3("\n\t\t&& (", v->lft,
						     " == ");
						putname(fd, "qrecv(", now->lft,
						        m, ", ");
						fprintf(fd, "0, %d, 0))", i);
		...
		if(has_xu) {
			...
			if(has_random && now->val != 0)
				fprintf(fd, "	uerror(\"rand receive on xr "
				            "channel violates po "
				            "reduction\");\n\t\t");
			fprintf(fd, "}\n");
			fprintf(fd, "#endif\n\t\t");
		...
		if(!GenCode) {
			...
			if(j == 0 && now->val == 0) {
				...
				fprintf(fd, " != %d)\n\t", i);
				fprintf(fd, "\t\tUerror(\"wrong nr of msg "
				            "fields in rcv\");\n\t\t");
			}
		...
		if(u_sync) {
			...
			fprintf(fd, "\t\t\tprintf(\"%%3d: proc %%d fairness "
			            "\", depth, II);\n");
			fprintf(fd, "\t\t\tprintf(\"Rule 2: --cnt to %%d "
			            "(%%d)\\n\",\n");
			fprintf(fd,
...
void putname(FILE* fd, char* pre, Lextok* n, int m, char* suff) /* varref */
{
	...
	if(s->nel > 1 || s->isarray == 1) {
		if(no_arrays) {
			non_fatal(
			    "ref to array element invalid in this context",
			    (char*)0);
		...
		if(evalindex == 2) {
			...
			if(terse || (n->lft && n->lft->ntyp == CONST &&
			             n->lft->val < s->nel) ||
			   (!n->lft && s->nel > 0)) {
				cat3("[", n->lft, "]");
			} else { /* attempt to catch arrays that are indexed
			          * with an array element in the same array
			          * this causes trouble in the verifier in the
			          * backtracking
			          * e.g., restoring a[?] in the assignment: a
			          * [a[1]] = x where a[1] == 1
			          * but it is hard when the array is inside a
			          * structure, so the names dont match
			          */
#if 0
				if (n->lft->ntyp == NAME)
