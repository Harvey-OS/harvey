...
all_bucky(State *a, State *b)
{
	...
	for(s = a->trans; s; s = s->nxt) {
		...
		for(t = b->trans; t; t = t->nxt) {
			...
			if(isequal(s->cond, t->cond)) {
				if(strcmp(s->name->name, b->name->name) == 0 && strcmp(t->name->name, a->name->name) == 0) {
					found = 1; /* they point to each other */
					t->marked = 1;
					break;
				}
				if(strcmp(s->name->name, t->name->name) == 0 && strcmp(s->name->name, "accept_all") == 0) {
					found = 1;
					t->marked = 1;
					break;
	...
	for(s = b->trans; s; s = s->nxt) {
		...
		for(t = a->trans; t; t = t->nxt) {
			...
			if(isequal(s->cond, t->cond)) {
				if(strcmp(s->name->name, a->name->name) == 0 && strcmp(t->name->name, b->name->name) == 0) {
					found = 1;
					t->marked = 1;
					break;
				}
		...
		for(a = never; a; a = a->nxt) {
			...
			for(b = a->nxt; b; b = b->nxt) {
				...
				if(all_bucky(a, b)) {
					...
					if(!strncmp(c->name->name, "accept", 6) && Max_Red == 0) {
						char buf[64];
		...
		for(a = never; a; a = a->nxt) {
			...
			for(b = a->nxt; b; b = b->nxt) {
				...
				if(all_trans_match(a, b)) {
					...
					if(!strncmp(a->name->name, "accept", 6) && Max_Red == 0) {
						char buf[64];
...
printstate(State *b)
{
	...
	if(strncmp(b->name->name, "accept", 6) == 0 && Max_Red == 0)
		fprintf(tl_out, "T0%s:\n", &(b->name->name[6]));
