...
#define ICM_FILE_BASE                                                          \
	...
	int (*seek)(struct _icmFile * p, long int offset);                     \
                                                                               \
	/* Read count items of size length. Return number of items             \
	 * successfully read. */                                               \
	size_t (*read)(struct _icmFile * p, void* buffer, size_t size,         \
	               size_t count);                                          \
                                                                               \
	/* write count items of size length. Return number of items            \
	 * successfully written. */                                            \
	size_t (*write)(struct _icmFile * p, void* buffer, size_t size,        \
	                size_t count);                                         \
                                                                               \
	/* flush all write data out to secondary storage. Return nz on         \
	 * failure. */                                                         \
...
typedef struct {
	...
	int** rlists; /* Array of list of fwd values that may contain output
	                 value */
...
struct _icmCurve {
	...
	int (*lookup_fwd)(struct _icmCurve* p, double* out,
	                  double* in); /* Forwards */
	int (*lookup_bwd)(struct _icmCurve* p, double* out,
	                  double* in); /* Backwards */
};
...
struct _icmData {
	...
	unsigned long
	    size; /* Allocated and used size of the array (inc ascii null) */
...
/ *DeviceSettings* /
    ...
    where
            platformN.combinationM.valid = (
    platformN.combinationM.settingstruct1.valid
                                            and
    platformN.combinationM.settingstruct2.valid
                                            and ...
                                           )

    where
            platformN.combinationM.settingstructP.valid = (
    platformN.combinationM.settingstructP.setting1.valid
                                                           or
    platformN.combinationM.settingstructP.setting2.valid
                                                           or ...
                                                          )
...
struct _icmPlatformEntry {
	...
	unsigned long
	    numCombinations; /* num of settings and allocated array size */
	icmSettingComb* data;
};
...
struct _icmDeviceSettings {
	...
	/* Public: */
	unsigned long
	    numPlatforms;       /* num of platforms and allocated array size */
...
struct _icmLut {
	...
	/* return the minimum and maximum values of the given channel in the
	 * clut */
	void (*min_max)(struct _icmLut* pp, double* minv, double* maxv,
	                int chan);

	/* Translate color values through 3x3 matrix, input tables only,
	...
	unsigned int
	    inputEnt; /* Num of in-table entries (must be 256 for Lut8) */
	unsigned int
	    outputEnt;  /* Num of out-table entries (must be 256 for Lut8) */
	double e[3][3]; /* 3 * 3 array */
	double* inputTable; /* The in-table: [inputChan * inputEnt] */
	double*
	    clutTable; /* The clut: [(clutPoints ^ inputChan) * outputChan] */
	double* outputTable; /* The out-table: [outputChan * outputEnt] */
	/* inputTable  is organized [inputChan 0..ic-1][inputEnt 0..ie-1] */
	/* clutTable   is organized [inputChan 0, 0..cp-1]..[inputChan ic-1,
	   0..cp-1]
	                                                                [outputChan
	   0..oc-1] */
	...
	int (*set_tables)(
	    ...
	    double* clutmin,
	    double* clutmax, /* Maximum range of outspace' values */
	                     /* (NULL = default) */
...
struct _icmTextDescription {
	...
	unsigned long
	    ucSize; /* Allocated and used size of ucDesc in wchars, inc null */
...
struct _icmCrdInfo {
	...
	/* Public: */
	unsigned long
	    ppsize;   /* Postscript product name size (including null) */
	char* ppname; /* Postscript product name (null terminated) */
	unsigned long crdsize[4]; /* Rendering intent 0-3 CRD names sizes
	                             (icluding null) */
...
struct _icmHeader {
	...
	/* icmUint64            attributes;		   Device attributes.h
	 * (see above) */
...
typedef enum {
	icmLuOrdNorm =
	    0, /* Normal profile preference: Lut, matrix, monochrome */
	icmLuOrdRev =
	    1 /* Reverse profile preference: monochrome, matrix, monochrome */
} icmLookupOrder;
...
#define LU_ICM_BASE_MEMBERS                                                    \
	...
	icmXYZNumber pcswht, whitePoint,                                       \
	    blackPoint;     /* White and black point info */                   \
	double toAbs[3][3]; /* Matrix to convert from relative to absolute */  \
	double                                                                 \
	    fromAbs[3][3]; /* Matrix to convert from absolute to relative */   \
	...
	void (*lutspaces)(struct _icmLuBase * p, icColorSpaceSignature * ins,  \
	                  int* inn, icColorSpaceSignature* outs, int* outn);   \
                                                                               \
	/* External effecive colorspaces */                                    \
	void (*spaces)(struct _icmLuBase * p, icColorSpaceSignature * ins,     \
	               int* inn, icColorSpaceSignature* outs, int* outn,       \
	...
	void (*get_ranges)(                                                    \
	    struct _icmLuBase * p, double* inmin,                              \
	    double* inmax, /* Maximum range of inspace values */               \
	    double* outmin,                                                    \
	    double* outmax); /* Maximum range of outspace values */            \
                                                                               \
	void (*wh_bk_points)(struct _icmLuBase * p, icmXYZNumber * wht,        \
	                     icmXYZNumber * blk);                              \
...
struct _icmLuLut {
	...
	void (*in_normf)(double* out,
	                 double* in); /* Lut input data normalizing function */
	void (*in_denormf)(
	    double* out,
	    double* in); /* Lut input data de-normalizing function */
	void (*out_normf)(
	    double* out, double* in); /* Lut output data normalizing function */
	void (*out_denormf)(
	    double* out, double* in); /* Lut output de-normalizing function */
	void (*e_in_denormf)(
	    double* out,
	    double* in); /* Effective input de-normalizing function */
	void (*e_out_denormf)(
	    double* out,
	    double* in); /* Effecive output de-normalizing function */
	/* function chosen out of lut->lookup_clut_sx and lut->lookup_clut_nl to
	 * imp. clut() */
	int (*lookup_clut)(struct _icmLut* pp, double* out,
	                   double* in); /* clut function */

	/* public: */

	/* Components of lookup */
	int (*in_abs)(struct _icmLuLut* p, double* out,
	              double* in); /* Should be in icmLut ? */
	...
	int (*out_abs)(struct _icmLuLut* p, double* out,
	               double* in); /* Should be in icmLut ? */
...
struct _icc {
	...
	unsigned int (*get_size)(
	    struct _icc* p); /* Return total size needed, 0 = err. */
	int (*read)(struct _icc* p, icmFile* fp,
	            unsigned long of); /* Returns error code */
	int (*write)(struct _icc* p, icmFile* fp,
	             unsigned long of); /* Returns error code */
	...
	icmBase* (*add_tag)(struct _icc* p, icTagSignature sig,
	                    icTagTypeSignature ttype);
	/* Returns pointer to object */
	int (*rename_tag)(struct _icc* p, icTagSignature sig,
	                  icTagSignature sigNew);
	/* Rename and existing tag */
	icmBase* (*link_tag)(struct _icc* p, icTagSignature sig,
	                     icTagSignature ex_sig);
	/* Returns pointer to object */
	int (*unread_tag)(struct _icc* p, icTagSignature sig);
	/* Unread a tag (free on refcount == 0 */
	int (*read_all_tags)(
	    struct _icc* p); /* Read all the tags, non-zero on error. */

	int (*delete_tag)(struct _icc* p, icTagSignature sig);
	/* Returns 0 if deleted OK */
	icmLuBase* (*get_luobj)(
	    struct _icc* p, icmLookupFunc func, /* Functionality */
...
void icmChromAdaptMatrix(int flags,         /* Flags as defined below */
...
#define icmAry2XYZ(xyz, ary)                                                   \
	((xyz).X = (ary)[0], (xyz).Y = (ary)[1], (xyz).Z = (ary)[2])

/* And the reverse */
#define icmXYZ2Ary(ary, xyz)                                                   \
	((ary)[0] = (xyz).X, (ary)[1] = (xyz).Y, (ary)[2] = (xyz).Z)
