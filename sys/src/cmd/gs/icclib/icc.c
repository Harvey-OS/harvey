 ...
 *
 *		Should fix all write_number failure errors to indicate failed
 *value.
 *		(Partially implemented - need to check all write_number
 *functions)
 *
 *		Make write fail error messages be specific on which element
 *failed.
 *
 ...
 *      so that the base library doesn't need to be modified (ie.
 *VideoCardGamma) ?
 *
 *		Need to add DeviceSettings and OutputResponse tags to bring up
 *to
 *		ICC.1:1998-09 [started but not complete]
 *
 */

#undef ICM_STRICT /* Not fully implimented - switch off strict checking of     \
                     file format */
 ...
 *
 * 1.30
 *      Added workaround for reading some Adobe profiles with confused header
 *DateTime.
 *      Enhanced tag allocate() methods so that they can resize allocations.
 *      Enhanced icmLut_set_tables() to access grid points in a cache friendly
 *order.
 *      Fixed bug in check_icc_legal() that caused bogus errors, removed
 *      uneccessary static declarations in icc.h, and fixed a bug in
 *      icmTable_lookup_bwd() that effected both accuracy and speed. (Thanks to
 *Andrei Frolov)
 ...
 *		Changed all the public 2str utility routines to a single
 *function
 ...
 *      it is more friendly to systems with a limited stack. (Thanks to Dave
 *White)
 *
 ...
 *      Adding simplex interpolation for non-Lab style input space
 *interpolation.
 *      Fix Sun misalignment and realloc problems (Thanks to Allan N.
 *Hessenflow)
 *      Fixed endian problem with Unicode on read and write.
 *      Expanded icmTextDescription_dump() to do hex dump of Unicode and
 *ScriptCode.
...
static int
icmFileStd_seek(icmFile* pp, long int offset)
{
...
static size_t
icmFileStd_read(icmFile* pp, void* buffer, size_t size, size_t count)
{
	icmFileStd* p = (icmFileStd*)pp;
...
/* write count items of size length. Return number of items successfully
 * written. */
static size_t
icmFileStd_write(icmFile* pp, void* buffer, size_t size, size_t count)
{
	icmFileStd* p = (icmFileStd*)pp;
...
icmFile*
new_icmFileStd_name(char* name, char* mode)
{
	FILE* fp;
	icmFile* p;
#if defined(O_BINARY)
...
/* Set current position to offset. Return 0 on success, nz on failure. */
static int
icmFileMem_seek(icmFile* pp, long int offset)
{
...
/* Read count items of size length. Return number of items successfully read. */
static size_t
icmFileMem_read(icmFile* pp, void* buffer, size_t size, size_t count)
{
	icmFileMem* p = (icmFileMem*)pp;
	size_t len;
 ...
 * written. */
static size_t
icmFileMem_write(icmFile* pp, void* buffer, size_t size, size_t count)
{
	icmFileMem* p = (icmFileMem*)pp;
	size_t len;
...
icmFile*
new_icmFileMem(void* base,   /* Pointer to base of memory buffer */
...
static void*
icmAllocStd_malloc(struct _icmAlloc* pp, size_t size)
{
	return malloc(size);
}

static void*
icmAllocStd_calloc(struct _icmAlloc* pp, size_t num, size_t size)
{
	return calloc(num, size);
}

static void*
icmAllocStd_realloc(struct _icmAlloc* pp, void* ptr, size_t size)
{
	return realloc(ptr, size);
}

static void
icmAllocStd_free(struct _icmAlloc* pp, void* ptr)
{
	free(ptr);
}
...
static void
icmAllocStd_delete(icmAlloc* pp)
{
...
static int
write_UInt8Number(unsigned int d, char* p)
{
	if(d > 255)
		return 1;
	((ORD8*)p)[0] = (ORD8)d;
...
static unsigned int
read_UInt16Number(char* p)
{
	unsigned int rv;
	rv = 256 * (unsigned int)((ORD8*)p)[0] + (unsigned int)((ORD8*)p)[1];
	return rv;
}

static int
write_UInt16Number(unsigned int d, char* p)
{
	if(d > 65535)
		return 1;
	((ORD8*)p)[0] = (ORD8)(d >> 8);
...
static unsigned int
read_UInt32Number(char* p)
{
	unsigned int rv;
	rv = 16777216 * (unsigned int)((ORD8*)p)[0] +
	     65536 * (unsigned int)((ORD8*)p)[1] +
	     256 * (unsigned int)((ORD8*)p)[2] + (unsigned int)((ORD8*)p)[3];
	return rv;
}

static int
write_UInt32Number(unsigned int d, char* p)
{
	((ORD8*)p)[0] = (ORD8)(d >> 24);
	((ORD8*)p)[1] = (ORD8)(d >> 16);
	((ORD8*)p)[2] = (ORD8)(d >> 8);
...
static void
read_UInt64Number(icmUint64* d, char* p)
{
	...
	d->l = 16777216 * (unsigned int)((ORD8*)p)[4] +
	       ...
	       256 * (unsigned int)((ORD8*)p)[6] + (unsigned int)((ORD8*)p)[7];
}

static int
write_UInt64Number(icmUint64* d, char* p)
{
...
static int
write_U8Fixed8Number(double d, char* p)
{
	ORD32 o32;
	d = d * 256.0 + 0.5;
	if(d >= 65536.0)
...
static double
read_U16Fixed16Number(char* p)
{
	ORD32 o32;
	o32 = 16777216 *
	          (ORD32)((ORD8*)p)[0] /* Read big endian 32 bit unsigned */
	      +
	      65536 * (ORD32)((ORD8*)p)[1] + 256 * (ORD32)((ORD8*)p)[2] +
	      (ORD32)((ORD8*)p)[3];
	return (double)o32 / 65536.0;
}

static int
write_U16Fixed16Number(double d, char* p)
{
	ORD32 o32;
	d = d * 65536.0 + 0.5;
	if(d >= 4294967296.0)
...
static int
write_SInt8Number(int d, char* p)
{
	if(d > 127)
		return 1;
	else if(d < -128)
...
static int
read_SInt16Number(char* p)
{
	int rv;
	rv = 256 * (int)((INR8*)p)[0] + (int)((ORD8*)p)[1];
	return rv;
}

static int
write_SInt16Number(int d, char* p)
{
	if(d > 32767)
		return 1;
	else if(d < -32768)
...
static int
read_SInt32Number(char* p)
{
	int rv;
	rv = 16777216 * (int)((INR8*)p)[0] + 65536 * (int)((ORD8*)p)[1] +
	     256 * (int)((ORD8*)p)[2] + (int)((ORD8*)p)[3];
	return rv;
}

static int
write_SInt32Number(int d, char* p)
{
	((INR8*)p)[0] = (INR8)(d >> 24);
	((ORD8*)p)[1] = (ORD8)(d >> 16);
	((ORD8*)p)[2] = (ORD8)(d >> 8);
...
static void
read_SInt64Number(icmInt64* d, char* p)
{
	d->h = 16777216 * (int)((INR8*)p)[0] + 65536 * (int)((ORD8*)p)[1] +
	       256 * (int)((ORD8*)p)[2] + (int)((ORD8*)p)[3];
	d->l = 16777216 * (unsigned int)((ORD8*)p)[4] +
	       65536 * (unsigned int)((ORD8*)p)[5] +
	       256 * (unsigned int)((ORD8*)p)[6] + (unsigned int)((ORD8*)p)[7];
...
static int
write_SInt64Number(icmInt64* d, char* p)
{
...
static double
read_S15Fixed16Number(char* p)
{
	INR32 i32;
	i32 =
	    16777216 * (INR32)((INR8*)p)[0] /* Read big endian 32 bit signed */
	    + 65536 * (INR32)((ORD8*)p)[1] + 256 * (INR32)((ORD8*)p)[2] +
	    (INR32)((ORD8*)p)[3];
	return (double)i32 / 65536.0;
}

static int
write_S15Fixed16Number(double d, char* p)
{
	INR32 i32;
	...
	i32 = (INR32)d;
	((INR8*)p)[0] =
	    (INR8)((i32) >> 24); /* Write big endian 32 bit signed */
...
static int
write_PCSXYZ16Number(double d, char* p)
{
	ORD32 o32;
	d = d * 32768.0 + 0.5;
	if(d >= 65536.0)
...
static int
write_PCSL8Number(double d, char* p)
{
	ORD32 o32;
	d = d * 2.550 + 0.5;
	if(d >= 256.0)
...
static int
write_PCSab8Number(double d, char* p)
{
	ORD32 o32;
	d = (d + 128.0) + 0.5;
	if(d >= 256.0)
...
static int
write_PCSL16Number(double d, char* p)
{
	ORD32 o32;
	d = d * 652.800 + 0.5;
	if(d >= 65536.0)
...
static int
write_PCSab16Number(double d, char* p)
{
	ORD32 o32;
	d = (d + 128.0) * 256.0 + 0.5;
	if(d >= 65536.0)
...
static int
write_DCS8Number(double d, char* p)
{
	ORD32 o32;
	d = d * 255.0 + 0.5;
	if(d >= 256.0)
...
static int
write_DCS16Number(double d, char* p)
{
	ORD32 o32;
	d = d * 65535.0 + 0.5;
	if(d >= 65536.0)
...
str2tag(const char* str)
{
	...
	tag = (((unsigned long)str[0]) << 24) +
	      (((unsigned long)str[1]) << 16) + (((unsigned long)str[2]) << 8) +
	      (((unsigned long)str[3]));
	return (int)tag;
}
...
static int
check_null_string(char* cp, int len)
{
	for(; len > 0; len--) {
		if(*cp++ == '\000')
			break;
...
/* Unicode version */
static int
check_null_string16(char* cp, int len)
{
...
/* Return 0 on error */
static unsigned int
number_ColorSpaceSignature(icColorSpaceSignature sig)
{
	switch(sig) {
	case icSigXYZData:
		return 3;
...
/* Screening Encodings */
static char*
string_ScreenEncodings(unsigned long flags)
{
	static int si = 0;      /* String buffer index */
...
/* Device attributes */
static char*
string_DeviceAttributes(unsigned long flags)
{
	static int si = 0;      /* String buffer index */
	static char buf[5][80]; /* String buffers */
	char* bp, *cp;
...
static char*
string_ProfileHeaderFlags(unsigned long flags)
{
	static int si = 0;      /* String buffer index */
	static char buf[5][80]; /* String buffers */
	char* bp, *cp;
...
static char*
string_AsciiOrBinaryData(unsigned long flags)
{
	static int si = 0;      /* String buffer index */
	static char buf[5][80]; /* String buffers */
	char* bp, *cp;
...
static const char*
string_TagSignature(icTagSignature sig)
{
	static char buf[80];
	switch(sig) {
	case icSigAToB0Tag:
...
static const char*
string_TechnologySignature(icTechnologySignature sig)
{
	static char buf[80];
	switch(sig) {
...
/* type signatures */
static const char*
string_TypeSignature(icTagTypeSignature sig)
{
	static char buf[80];
	switch(sig) {
	case icSigCurveType:
...
static const char*
string_ColorSpaceSignature(icColorSpaceSignature sig)
{
	static char buf[80];
	switch(sig) {
	case icSigXYZData:
...
char*
ColorSpaceSignature2str(icColorSpaceSignature sig)
{
...
static const char*
string_ProfileClassSignature(icProfileClassSignature sig)
{
	static char buf[80];
	switch(sig) {
...
/* Platform Signatures */
static const char*
string_PlatformSignature(icPlatformSignature sig)
{
	static char buf[80];
	switch(sig) {
	case icSigMacintosh:
...
static const char*
string_MeasurementGeometry(icMeasurementGeometry sig)
{
	static char buf[30];
	switch(sig) {
...
static const char*
string_RenderingIntent(icRenderingIntent sig)
{
	static char buf[30];
	switch(sig) {
	case icPerceptual:
...
static const char*
string_SpotShape(icSpotShape sig)
{
	static char buf[30];
	switch(sig) {
...
static const char*
string_StandardObserver(icStandardObserver sig)
{
	static char buf[30];
	switch(sig) {
...
static const char*
string_Illuminant(icIlluminant sig)
{
	static char buf[30];
	switch(sig) {
...
/* Public: */
const char*
icm2str(icmEnumType etype, int enumval)
{
	...
	case icmTechnologySignature:
		return string_TechnologySignature(
		    (icTechnologySignature)enumval);
	case icmTypeSignature:
		return string_TypeSignature((icTagTypeSignature)enumval);
	case icmColorSpaceSignature:
		return string_ColorSpaceSignature(
		    (icColorSpaceSignature)enumval);
	case icmProfileClassSignaure:
		return string_ProfileClassSignature(
		    (icProfileClassSignature)enumval);
	case icmPlatformSignature:
		return string_PlatformSignature((icPlatformSignature)enumval);
	case icmMeasurementGeometry:
		return string_MeasurementGeometry(
		    (icMeasurementGeometry)enumval);
...
static unsigned int
icmUInt8Array_get_size(icmBase* pp)
{
...
static int
icmUInt8Array_read(icmBase* pp, unsigned long len, /* tag length */
                   ...
                   )
{
	...
	if(len < 8) {
		sprintf(icp->err,
		        "icmUInt8Array_read: Tag too small to be legal");
		return icp->errc = 1;
	}
	...
	if(icp->fp->seek(icp->fp, of) != 0 ||
	   icp->fp->read(icp->fp, bp, 1, len) != len) {
		sprintf(icp->err,
		        "icmUInt8Array_read: fseek() or fread() failed");
	...
	if(((icTagTypeSignature)read_SInt32Number(bp)) != p->ttype) {
		icp->al->free(icp->al, buf);
		sprintf(icp->err,
		        "icmUInt8Array_read: Wrong tag type for icmUInt8Array");
		return icp->errc = 1;
	}
	bp += 8; /* Skip padding */
 ...
 */
static int
icmUInt8Array_write(icmBase* pp,
                    ...
                    )
{
	...
	if((rv = write_SInt32Number((int)p->ttype, bp)) != 0) {
		sprintf(icp->err,
		        "icmUInt8Array_write: write_SInt32Number() failed");
		icp->al->free(icp->al, buf);
		return icp->errc = rv;
	}
	...
	for(i = 0; i < p->size; i++, bp += 1) {
		if((rv = write_UInt8Number(p->data[i], bp)) != 0) {
			sprintf(
			    icp->err,
			    "icmUInt8Array_write: write_UInt8umber() failed");
			icp->al->free(icp->al, buf);
			return icp->errc = rv;
		}
	}
	...
	if(icp->fp->seek(icp->fp, of) != 0 ||
	   icp->fp->write(icp->fp, buf, 1, len) != len) {
		sprintf(icp->err,
		        "icmUInt8Array_write fseek() or fwrite() failed");
		icp->al->free(icp->al, buf);
		return icp->errc = 2;
	}
	icp->al->free(icp->al, buf);
...
static void
icmUInt8Array_dump(icmBase* pp, FILE* op, /* Output to dump to */
...
static int
icmUInt8Array_allocate(icmBase* pp)
{
	icmUInt8Array* p = (icmUInt8Array*)pp;
	icc* icp = p->icp;
	...
	if(p->size != p->_size) {
		...
		if((p->data = (unsigned int*)icp->al->malloc(
		        icp->al, p->size * sizeof(unsigned int))) == NULL) {
...
static void
icmUInt8Array_delete(icmBase* pp)
{
	icmUInt8Array* p = (icmUInt8Array*)pp;
	icc* icp = p->icp;
...
static icmBase*
new_icmUInt8Array(icc* icp)
{
	icmUInt8Array* p;
	if((p = (icmUInt8Array*)icp->al->calloc(icp->al, 1,
	                                        sizeof(icmUInt8Array))) == NULL)
		return NULL;
...
/* Return the number of bytes needed to write this tag */
static unsigned int
icmUInt16Array_get_size(icmBase* pp)
{
...
/* read the object, return 0 on success, error code on fail */
static int
icmUInt16Array_read(icmBase* pp, unsigned long len, /* tag length */
                    unsigned long of /* start offset within file */
                    )
{
	icmUInt16Array* p = (icmUInt16Array*)pp;
	...
	if(len < 8) {
		sprintf(icp->err,
		        "icmUInt16Array_read: Tag too small to be legal");
		return icp->errc = 1;
	}
	...
	if(icp->fp->seek(icp->fp, of) != 0 ||
	   icp->fp->read(icp->fp, bp, 1, len) != len) {
		sprintf(icp->err,
		        "icmUInt16Array_read: fseek() or fread() failed");
		icp->al->free(icp->al, buf);
		return icp->errc = 1;
	}
	...
	if(((icTagTypeSignature)read_SInt32Number(bp)) != p->ttype) {
		sprintf(
		    icp->err,
		    "icmUInt16Array_read: Wrong tag type for icmUInt16Array");
		icp->al->free(icp->al, buf);
		return icp->errc = 1;
	}
	bp += 8; /* Skip padding */
 ...
 */
static int
icmUInt16Array_write(icmBase* pp,
                     unsigned long of /* File offset to write from */
                     )
{
	icmUInt16Array* p = (icmUInt16Array*)pp;
	...
	if((rv = write_SInt32Number((int)p->ttype, bp)) != 0) {
		sprintf(icp->err,
		        "icmUInt16Array_write: write_SInt32Number() failed");
		icp->al->free(icp->al, buf);
		return icp->errc = rv;
	}
	write_SInt32Number(0, bp + 4); /* Set padding to 0 */
	...
	for(i = 0; i < p->size; i++, bp += 2) {
		if((rv = write_UInt16Number(p->data[i], bp)) != 0) {
			sprintf(
			    icp->err,
			    "icmUInt16Array_write: write_UInt16umber() failed");
			icp->al->free(icp->al, buf);
			return icp->errc = rv;
		}
	}
	...
	if(icp->fp->seek(icp->fp, of) != 0 ||
	   icp->fp->write(icp->fp, buf, 1, len) != len) {
		sprintf(icp->err,
		        "icmUInt16Array_write fseek() or fwrite() failed");
		icp->al->free(icp->al, buf);
		return icp->errc = 2;
	}
	icp->al->free(icp->al, buf);
...
/* Dump a text description of the object */
static void
icmUInt16Array_dump(icmBase* pp, FILE* op, /* Output to dump to */
                    int verb               /* Verbosity level */
                    )
{
	icmUInt16Array* p = (icmUInt16Array*)pp;
...
/* Allocate variable sized data elements */
static int
icmUInt16Array_allocate(icmBase* pp)
{
	icmUInt16Array* p = (icmUInt16Array*)pp;
	icc* icp = p->icp;
...
/* Free all storage in the object */
static void
icmUInt16Array_delete(icmBase* pp)
{
	icmUInt16Array* p = (icmUInt16Array*)pp;
	icc* icp = p->icp;
...
/* Create an empty object. Return null on error */
static icmBase*
new_icmUInt16Array(icc* icp)
{
	icmUInt16Array* p;
	if((p = (icmUInt16Array*)icp->al->calloc(
	        icp->al, 1, sizeof(icmUInt16Array))) == NULL)
		return NULL;
...
/* Return the number of bytes needed to write this tag */
static unsigned int
icmUInt32Array_get_size(icmBase* pp)
{
...
/* read the object, return 0 on success, error code on fail */
static int
icmUInt32Array_read(icmBase* pp, unsigned long len, /* tag length */
                    unsigned long of /* start offset within file */
                    )
{
	icmUInt32Array* p = (icmUInt32Array*)pp;
	...
	if(len < 8) {
		sprintf(icp->err,
		        "icmUInt32Array_read: Tag too small to be legal");
		return icp->errc = 1;
	}
	...
	if(icp->fp->seek(icp->fp, of) != 0 ||
	   icp->fp->read(icp->fp, bp, 1, len) != len) {
		sprintf(icp->err,
		        "icmUInt32Array_read: fseek() or fread() failed");
		icp->al->free(icp->al, buf);
		return icp->errc = 1;
	}
	...
	if(((icTagTypeSignature)read_SInt32Number(bp)) != p->ttype) {
		sprintf(
		    icp->err,
		    "icmUInt32Array_read: Wrong tag type for icmUInt32Array");
		icp->al->free(icp->al, buf);
		return icp->errc = 1;
	}
	bp += 8; /* Skip padding */
 ...
 */
static int
icmUInt32Array_write(icmBase* pp,
                     unsigned long of /* File offset to write from */
                     )
{
	icmUInt32Array* p = (icmUInt32Array*)pp;
	...
	if((rv = write_SInt32Number((int)p->ttype, bp)) != 0) {
		sprintf(icp->err,
		        "icmUInt32Array_write: write_SInt32Number() failed");
		icp->al->free(icp->al, buf);
		return icp->errc = rv;
	}
	write_SInt32Number(0, bp + 4); /* Set padding to 0 */
	...
	for(i = 0; i < p->size; i++, bp += 4) {
		if((rv = write_UInt32Number(p->data[i], bp)) != 0) {
			sprintf(
			    icp->err,
			    "icmUInt32Array_write: write_UInt32umber() failed");
			icp->al->free(icp->al, buf);
			return icp->errc = rv;
		}
	}
	...
	if(icp->fp->seek(icp->fp, of) != 0 ||
	   icp->fp->write(icp->fp, buf, 1, len) != len) {
		sprintf(icp->err,
		        "icmUInt32Array_write fseek() or fwrite() failed");
		icp->al->free(icp->al, buf);
		return icp->errc = 2;
	}
	icp->al->free(icp->al, buf);
...
/* Dump a text description of the object */
static void
icmUInt32Array_dump(icmBase* pp, FILE* op, /* Output to dump to */
                    int verb               /* Verbosity level */
                    )
{
	icmUInt32Array* p = (icmUInt32Array*)pp;
...
/* Allocate variable sized data elements */
static int
icmUInt32Array_allocate(icmBase* pp)
{
	icmUInt32Array* p = (icmUInt32Array*)pp;
	icc* icp = p->icp;
...
/* Free all storage in the object */
static void
icmUInt32Array_delete(icmBase* pp)
{
	icmUInt32Array* p = (icmUInt32Array*)pp;
	icc* icp = p->icp;
...
/* Create an empty object. Return null on error */
static icmBase*
new_icmUInt32Array(icc* icp)
{
	icmUInt32Array* p;
	if((p = (icmUInt32Array*)icp->al->calloc(
	        icp->al, 1, sizeof(icmUInt32Array))) == NULL)
		return NULL;
...
/* Return the number of bytes needed to write this tag */
static unsigned int
icmUInt64Array_get_size(icmBase* pp)
{
...
/* read the object, return 0 on success, error code on fail */
static int
icmUInt64Array_read(icmBase* pp, unsigned long len, /* tag length */
                    unsigned long of /* start offset within file */
                    )
{
	icmUInt64Array* p = (icmUInt64Array*)pp;
	...
	if(len < 8) {
		sprintf(icp->err,
		        "icmUInt64Array_read: Tag too small to be legal");
		return icp->errc = 1;
	}
	...
	if(icp->fp->seek(icp->fp, of) != 0 ||
	   icp->fp->read(icp->fp, bp, 1, len) != len) {
		sprintf(icp->err,
		        "icmUInt64Array_read: fseek() or fread() failed");
		icp->al->free(icp->al, buf);
		return icp->errc = 1;
	}
	...
	if(((icTagTypeSignature)read_SInt32Number(bp)) != p->ttype) {
		sprintf(
		    icp->err,
		    "icmUInt64Array_read: Wrong tag type for icmUInt64Array");
		icp->al->free(icp->al, buf);
		return icp->errc = 1;
	}
	bp += 8; /* Skip padding */
 ...
 */
static int
icmUInt64Array_write(icmBase* pp,
                     unsigned long of /* File offset to write from */
                     )
{
	icmUInt64Array* p = (icmUInt64Array*)pp;
	...
	if((rv = write_SInt32Number((int)p->ttype, bp)) != 0) {
		sprintf(icp->err,
		        "icmUInt64Array_write: write_SInt32Number() failed");
		icp->al->free(icp->al, buf);
		return icp->errc = rv;
	}
	write_SInt32Number(0, bp + 4); /* Set padding to 0 */
	...
	for(i = 0; i < p->size; i++, bp += 8) {
		if((rv = write_UInt64Number(&p->data[i], bp)) != 0) {
			sprintf(
			    icp->err,
			    "icmUInt64Array_write: write_UInt64umber() failed");
			icp->al->free(icp->al, buf);
			return icp->errc = rv;
		}
	}
	...
	if(icp->fp->seek(icp->fp, of) != 0 ||
	   icp->fp->write(icp->fp, buf, 1, len) != len) {
		sprintf(icp->err,
		        "icmUInt64Array_write fseek() or fwrite() failed");
		icp->al->free(icp->al, buf);
		return icp->errc = 2;
	}
	icp->al->free(icp->al, buf);
...
/* Dump a text description of the object */
static void
icmUInt64Array_dump(icmBase* pp, FILE* op, /* Output to dump to */
                    int verb               /* Verbosity level */
                    )
{
	icmUInt64Array* p = (icmUInt64Array*)pp;
...
/* Allocate variable sized data elements */
static int
icmUInt64Array_allocate(icmBase* pp)
{
	icmUInt64Array* p = (icmUInt64Array*)pp;
	icc* icp = p->icp;
	...
	if(p->size != p->_size) {
		if(p->data != NULL)
			icp->al->free(icp->al, p->data);
		if((p->data = (icmUint64*)icp->al->malloc(
		        icp->al, p->size * sizeof(icmUint64))) == NULL) {
...
/* Free all storage in the object */
static void
icmUInt64Array_delete(icmBase* pp)
{
	icmUInt64Array* p = (icmUInt64Array*)pp;
	icc* icp = p->icp;
...
/* Create an empty object. Return null on error */
static icmBase*
new_icmUInt64Array(icc* icp)
{
	icmUInt64Array* p;
	if((p = (icmUInt64Array*)icp->al->calloc(
	        icp->al, 1, sizeof(icmUInt64Array))) == NULL)
		return NULL;
...
/* Return the number of bytes needed to write this tag */
static unsigned int
icmU16Fixed16Array_get_size(icmBase* pp)
{
...
/* read the object, return 0 on success, error code on fail */
static int
icmU16Fixed16Array_read(icmBase* pp, unsigned long len, /* tag length */
                        unsigned long of /* start offset within file */
                        )
{
	icmU16Fixed16Array* p = (icmU16Fixed16Array*)pp;
	...
	if(len < 8) {
		sprintf(icp->err,
		        "icmU16Fixed16Array_read: Tag too small to be legal");
		return icp->errc = 1;
	}
	...
	if(icp->fp->seek(icp->fp, of) != 0 ||
	   icp->fp->read(icp->fp, bp, 1, len) != len) {
		sprintf(icp->err,
		        "icmU16Fixed16Array_read: fseek() or fread() failed");
		icp->al->free(icp->al, buf);
		return icp->errc = 1;
	}
	p->size = size = (len - 8) / 4; /* Number of elements in the array */
	...
	/* Read type descriptor from the buffer */
	if(((icTagTypeSignature)read_SInt32Number(bp)) != p->ttype) {
		sprintf(icp->err, "icmU16Fixed16Array_read: Wrong tag type for "
		                  "icmU16Fixed16Array");
		icp->al->free(icp->al, buf);
		return icp->errc = 1;
	}
 ...
 */
static int
icmU16Fixed16Array_write(icmBase* pp,
                         unsigned long of /* File offset to write from */
                         )
{
	icmU16Fixed16Array* p = (icmU16Fixed16Array*)pp;
	...
	if((rv = write_SInt32Number((int)p->ttype, bp)) != 0) {
		sprintf(
		    icp->err,
		    "icmU16Fixed16Array_write: write_SInt32Number() failed");
		icp->al->free(icp->al, buf);
		return icp->errc = rv;
	}
	write_SInt32Number(0, bp + 4); /* Set padding to 0 */
	...
	if(icp->fp->seek(icp->fp, of) != 0 ||
	   icp->fp->write(icp->fp, buf, 1, len) != len) {
		sprintf(icp->err,
		        "icmU16Fixed16Array_write fseek() or fwrite() failed");
		icp->al->free(icp->al, buf);
		return icp->errc = 2;
	}
	icp->al->free(icp->al, buf);
...
/* Dump a text description of the object */
static void
icmU16Fixed16Array_dump(icmBase* pp, FILE* op, /* Output to dump to */
                        int verb               /* Verbosity level */
                        )
{
	icmU16Fixed16Array* p = (icmU16Fixed16Array*)pp;
...
/* Allocate variable sized data elements */
static int
icmU16Fixed16Array_allocate(icmBase* pp)
{
	icmU16Fixed16Array* p = (icmU16Fixed16Array*)pp;
	icc* icp = p->icp;
	...
	if(p->size != p->_size) {
		if(p->data != NULL)
			icp->al->free(icp->al, p->data);
		if((p->data = (double*)icp->al->malloc(
		        icp->al, p->size * sizeof(double))) == NULL) {
			sprintf(icp->err, "icmU16Fixed16Array_alloc: malloc() "
			                  "of icmU16Fixed16Array data failed");
			return icp->errc = 2;
		}
		p->_size = p->size;
	}
...
/* Free all storage in the object */
static void
icmU16Fixed16Array_delete(icmBase* pp)
{
	icmU16Fixed16Array* p = (icmU16Fixed16Array*)pp;
	icc* icp = p->icp;
...
/* Create an empty object. Return null on error */
static icmBase*
new_icmU16Fixed16Array(icc* icp)
{
	icmU16Fixed16Array* p;
	if((p = (icmU16Fixed16Array*)icp->al->calloc(
	        icp->al, 1, sizeof(icmU16Fixed16Array))) == NULL)
		return NULL;
...
/* Return the number of bytes needed to write this tag */
static unsigned int
icmS15Fixed16Array_get_size(icmBase* pp)
{
...
/* read the object, return 0 on success, error code on fail */
static int
icmS15Fixed16Array_read(icmBase* pp, unsigned long len, /* tag length */
                        unsigned long of /* start offset within file */
                        )
{
	icmS15Fixed16Array* p = (icmS15Fixed16Array*)pp;
	...
	if(len < 8) {
		sprintf(icp->err,
		        "icmS15Fixed16Array_read: Tag too small to be legal");
		return icp->errc = 1;
	}
	...
	if(icp->fp->seek(icp->fp, of) != 0 ||
	   icp->fp->read(icp->fp, bp, 1, len) != len) {
		sprintf(icp->err,
		        "icmS15Fixed16Array_read: fseek() or fread() failed");
		icp->al->free(icp->al, buf);
		return icp->errc = 1;
	}
	p->size = size = (len - 8) / 4; /* Number of elements in the array */
	...
	/* Read type descriptor from the buffer */
	if(((icTagTypeSignature)read_SInt32Number(bp)) != p->ttype) {
		sprintf(icp->err, "icmS15Fixed16Array_read: Wrong tag type for "
		                  "icmS15Fixed16Array");
		icp->al->free(icp->al, buf);
		return icp->errc = 1;
	}
 ...
 */
static int
icmS15Fixed16Array_write(icmBase* pp,
                         unsigned long of /* File offset to write from */
                         )
{
	icmS15Fixed16Array* p = (icmS15Fixed16Array*)pp;
	...
	if((rv = write_SInt32Number((int)p->ttype, bp)) != 0) {
		sprintf(
		    icp->err,
		    "icmS15Fixed16Array_write: write_SInt32Number() failed");
		icp->al->free(icp->al, buf);
		return icp->errc = rv;
	}
	write_SInt32Number(0, bp + 4); /* Set padding to 0 */
	...
	for(i = 0; i < p->size; i++, bp += 4) {
		if((rv = write_S15Fixed16Number(p->data[i], bp)) != 0) {
			sprintf(icp->err, "icmS15Fixed16Array_write: "
			                  "write_S15Fixed16umber() failed");
			icp->al->free(icp->al, buf);
			return icp->errc = rv;
		}
	}
	...
	if(icp->fp->seek(icp->fp, of) != 0 ||
	   icp->fp->write(icp->fp, buf, 1, len) != len) {
		sprintf(icp->err,
		        "icmS15Fixed16Array_write fseek() or fwrite() failed");
		icp->al->free(icp->al, buf);
		return icp->errc = 2;
	}
	icp->al->free(icp->al, buf);
...
/* Dump a text description of the object */
static void
icmS15Fixed16Array_dump(icmBase* pp, FILE* op, /* Output to dump to */
                        int verb               /* Verbosity level */
                        )
{
	icmS15Fixed16Array* p = (icmS15Fixed16Array*)pp;
...
/* Allocate variable sized data elements */
static int
icmS15Fixed16Array_allocate(icmBase* pp)
{
	icmS15Fixed16Array* p = (icmS15Fixed16Array*)pp;
	icc* icp = p->icp;
	...
	if(p->size != p->_size) {
		...
		if((p->data = (double*)icp->al->malloc(
		        icp->al, p->size * sizeof(double))) == NULL) {
			sprintf(icp->err, "icmS15Fixed16Array_alloc: malloc() "
			                  "of icmS15Fixed16Array data failed");
			return icp->errc = 2;
		}
		p->_size = p->size;
	}
...
/* Free all storage in the object */
static void
icmS15Fixed16Array_delete(icmBase* pp)
{
	icmS15Fixed16Array* p = (icmS15Fixed16Array*)pp;
	icc* icp = p->icp;
...
/* Create an empty object. Return null on error */
static icmBase*
new_icmS15Fixed16Array(icc* icp)
{
	icmS15Fixed16Array* p;
	if((p = (icmS15Fixed16Array*)icp->al->calloc(
	        icp->al, 1, sizeof(icmS15Fixed16Array))) == NULL)
		return NULL;
...
static int
write_XYZNumber(icmXYZNumber* p, char* d)
{
	int rv;
...
static int
read_XYZNumber(icmXYZNumber* p, char* d)
{
...
static char*
string_XYZNumber(icmXYZNumber* p)
{
	static char buf[40];
...
static char*
string_XYZNumber_and_Lab(icmXYZNumber* p)
{
	static char buf[50];
	double lab[3];
	lab[0] = p->X;
	...
	sprintf(buf, "%f, %f, %f    [Lab %f, %f, %f]", p->X, p->Y, p->Z, lab[0],
	        lab[1], lab[2]);
	return buf;
}
...
/* Return the number of bytes needed to write this tag */
static unsigned int
icmXYZArray_get_size(icmBase* pp)
{
...
/* read the object, return 0 on success, error code on fail */
static int
icmXYZArray_read(icmBase* pp, unsigned long len, /* tag length */
                 unsigned long of                /* start offset within file */
                 )
{
	icmXYZArray* p = (icmXYZArray*)pp;
	...
	if(len < 8) {
		sprintf(icp->err,
		        "icmXYZArray_read: Tag too small to be legal");
		return icp->errc = 1;
	}
	...
	if(icp->fp->seek(icp->fp, of) != 0 ||
	   icp->fp->read(icp->fp, bp, 1, len) != len) {
		sprintf(icp->err,
		        "icmXYZArray_read: fseek() or fread() failed");
		icp->al->free(icp->al, buf);
		return icp->errc = 1;
	}
	...
	if(((icTagTypeSignature)read_SInt32Number(bp)) != p->ttype) {
		sprintf(icp->err,
		        "icmXYZArray_read: Wrong tag type for icmXYZArray");
		icp->al->free(icp->al, buf);
		return icp->errc = 1;
	}
	bp += 8; /* Skip padding */
 ...
 */
static int
icmXYZArray_write(icmBase* pp, unsigned long of /* File offset to write from */
                  )
{
	icmXYZArray* p = (icmXYZArray*)pp;
	icc* icp = p->icp;
	...
	if((rv = write_SInt32Number((int)p->ttype, bp)) != 0) {
		sprintf(icp->err,
		        "icmXYZArray_write: write_SInt32Number() failed");
		icp->al->free(icp->al, buf);
		return icp->errc = rv;
	}
	write_SInt32Number(0, bp + 4); /* Set padding to 0 */
	...
	for(i = 0; i < p->size; i++, bp += 12) {
		if((rv = write_XYZNumber(&p->data[i], bp)) != 0) {
			sprintf(icp->err,
			        "icmXYZArray_write: write_XYZumber() failed");
			icp->al->free(icp->al, buf);
			return icp->errc = rv;
		}
	}
	...
	if(icp->fp->seek(icp->fp, of) != 0 ||
	   icp->fp->write(icp->fp, buf, 1, len) != len) {
		sprintf(icp->err,
		        "icmXYZArray_write fseek() or fwrite() failed");
		icp->al->free(icp->al, buf);
		return icp->errc = 2;
	}
	icp->al->free(icp->al, buf);
...
/* Dump a text description of the object */
static void
icmXYZArray_dump(icmBase* pp, FILE* op, /* Output to dump to */
                 int verb               /* Verbosity level */
                 )
{
	icmXYZArray* p = (icmXYZArray*)pp;
	...
	if(verb >= 2) {
		...
		for(i = 0; i < p->size; i++) {
			fprintf(op, "    %lu:  %s\n", i,
			        string_XYZNumber_and_Lab(&p->data[i]));
		}
	}
}
...
/* Allocate variable sized data elements */
static int
icmXYZArray_allocate(icmBase* pp)
{
	icmXYZArray* p = (icmXYZArray*)pp;
	icc* icp = p->icp;
	...
	if(p->size != p->_size) {
		if(p->data != NULL)
			icp->al->free(icp->al, p->data);
		if((p->data = (icmXYZNumber*)icp->al->malloc(
		        icp->al, p->size * sizeof(icmXYZNumber))) == NULL) {
...
/* Create an empty object. Return null on error */
static icmBase*
new_icmXYZArray(icc* icp)
{
	icmXYZArray* p;
	if((p = (icmXYZArray*)icp->al->calloc(icp->al, 1,
	                                      sizeof(icmXYZArray))) == NULL)
		return NULL;
...
static int
icmCurve_lookup_fwd(icmCurve* p, double* out, double* in)
{
	int rv = 0;
...
static int
icmTable_setup_bwd(icc* icp,           /* Base icc object */
                   ...
                   )
{
	...
	rt->qscale = (double)rt->rsize /
	             (rt->rmax - rt->rmin); /* Scale factor to quantize to */

	/* Initialize the reverse lookup structures, and get overall min/max */
	if((rt->rlists = (int**)icp->al->calloc(
	        icp->al, 1, rt->rsize * sizeof(int*))) == NULL) {
		return 2;
	}
	...
	for(i = 0; i < (rt->size - 1); i++) {
		...
		/* For all buckets that may contain this output range, add index
		 * of this output */
		for(j = s; j <= e; j++) {
			...
			if(rt->rlists[j] == NULL) { /* No allocation */
				...
				if((rt->rlists[j] = (int*)icp->al->malloc(
				        icp->al, sizeof(int) * as)) == NULL) {
					return 2;
				}
				rt->rlists[j][0] = as;
				nf = rt->rlists[j][1] = 2;
				...
				as = rt->rlists[j][0]; /* Allocate space for
				                          this list */
				nf =
				    rt->rlists[j][1]; /* Next free location in
				                         list */
				if(nf >= as) {        /* need to expand space */
					as *= 2;
					rt->rlists[j] = (int*)icp->al->realloc(
					    icp->al, rt->rlists[j],
					    sizeof(int) * as);
					if(rt->rlists[j] == NULL) {
						return 2;
...
/* Free up any data */
static void
icmTable_delete_bwd(icc* icp,       /* Base icc */
                    icmRevTable* rt /* Reverse table data to setup */
                    )
{
	if(rt->inited != 0) {
...
/* Return 0 on success, 1 if clipping occured, 2 on other error */
static int
icmTable_lookup_bwd(icmRevTable* rt, double* out, double* in)
{
	int rv = 0;
	int ix, i, k;
	...
	if(rt->rlists[ix] != NULL) { /* There is a list of fwd candidates */
		...
		for(i = 2; i < rt->rlists[ix][1];
		    i++) { /* For all fwd indexes */
			double lv, hv;
			k = rt->rlists[ix][i]; /* Base index */
			lv = rt->data[k];
			hv = rt->data[k + 1];
			if((ival >= lv &&
			    ival <= hv) /* If this slot contains output value */
			   ||
			   (ival >= hv && ival <= lv)) {
				/* Reverse linear interpolation */
				if(hv == lv) { /* Technically non-monotonic -
				                  due to quantization ? */
					oval = (k + 0.5) / (rt->size - 1.0);
				} else
					oval = (k + ((ival - lv) / (hv - lv))) /
					       (rt->size - 1.0);
...
/* Return 0 on success, 1 if clipping occured, 2 on other error */
static int
icmCurve_lookup_bwd(icmCurve* p, double* out, double* in)
{
	icc* icp = p->icp;
	int rv = 0;
	if(p->flag == icmCurveLin) {
		*out = *in;
		...
		if(p->rt.inited == 0) {
			...
			if(rv != 0) {
				sprintf(icp->err, "icmCurve_lookup: Malloc "
				                  "failure in reverse lookup "
				                  "init.");
				return icp->errc = rv;
			}
		}
...
/* read the object, return 0 on success, error code on fail */
static int
icmCurve_read(icmBase* pp, unsigned long len, /* tag length */
              unsigned long of                /* start offset within file */
              )
{
	icmCurve* p = (icmCurve*)pp;
	...
	if(p->flag == icmCurveGamma) { /* Gamma curve */
		if((bp + 1) > end) {
			sprintf(icp->err,
			        "icmCurve_read: Data too short to curve gamma");
			icp->al->free(icp->al, buf);
			return icp->errc = 1;
		}
		...
		for(i = 0; i < p->size; i++, bp += 2) {
			if((bp + 2) > end) {
				sprintf(icp->err, "icmData_read: Data too "
				                  "short to curve value");
				icp->al->free(icp->al, buf);
				return icp->errc = 1;
			}
 ...
 */
static int
icmCurve_write(icmBase* pp, unsigned long of /* File offset to write from */
               )
{
	icmCurve* p = (icmCurve*)pp;
	icc* icp = p->icp;
	...
	if((rv = write_SInt32Number((int)p->ttype, bp)) != 0) {
		sprintf(icp->err,
		        "icmCurve_write: write_SInt32Number() failed");
		icp->al->free(icp->al, buf);
		return icp->errc = rv;
	}
	write_SInt32Number(0, bp + 4); /* Set padding to 0 */
	...
	if((rv = write_UInt32Number(p->size, bp + 8)) != 0) {
		sprintf(icp->err,
		        "icmCurve_write: write_UInt32Number() failed");
		icp->al->free(icp->al, buf);
		return icp->errc = rv;
	}
	...
	if(p->flag == icmCurveLin) {
		if(p->size != 0) {
			sprintf(icp->err, "icmCurve_write: Must be exactly 0 "
			                  "entry for Linear");
			icp->al->free(icp->al, buf);
			return icp->errc = 1;
		}
		...
		if(p->size != 1) {
			sprintf(icp->err, "icmCurve_write: Must be exactly 1 "
			                  "entry for Gamma");
			icp->al->free(icp->al, buf);
			return icp->errc = 1;
		}
		...
		if(p->size < 2) {
			sprintf(icp->err, "icmCurve_write: Must be 2 or more "
			                  "entries for Specified curve");
			icp->al->free(icp->al, buf);
			return icp->errc = 1;
		}
		for(i = 0; i < p->size; i++, bp += 2) {
			if((rv = write_DCS16Number(p->data[i], bp)) != 0) {
				sprintf(icp->err, "icmCurve_write: "
				                  "write_UInt16umber(%f) "
				                  "failed",
				        p->data[i]);
				icp->al->free(icp->al, buf);
...
/* Dump a text description of the object */
static void
icmCurve_dump(icmBase* pp, FILE* op, /* Output to dump to */
              int verb               /* Verbosity level */
              )
{
	icmCurve* p = (icmCurve*)pp;
...
icmCurve_allocate(icmBase* pp)
{
	...
	if(p->size != p->_size) {
		...
		if((p->data = (double*)icp->al->malloc(
			...
			sprintf(
			    icp->err,
			    "icmCurve_alloc: malloc() of icmCurve data failed");
			return icp->errc = 2;
		}
		p->_size = p->size;
	}
...
/* Create an empty object. Return null on error */
static icmBase*
new_icmCurve(icc* icp)
{
	icmCurve* p;
	if((p = (icmCurve*)icp->al->calloc(icp->al, 1, sizeof(icmCurve))) ==
	   NULL)
		return NULL;
...
/* read the object, return 0 on success, error code on fail */
static int
icmData_read(icmBase* pp, unsigned long len, /* tag length */
             unsigned long of                /* start offset within file */
             )
{
	icmData* p = (icmData*)pp;
	...
	if(p->size > 0) {
		if(p->flag == icmDataASCII) {
			if(check_null_string(bp, p->size) != 0) {
				sprintf(icp->err, "icmData_read: ACSII is not "
				                  "null terminated");
				icp->al->free(icp->al, buf);
				return icp->errc = 1;
			}
 ...
 */
static int
icmData_write(icmBase* pp, unsigned long of /* File offset to write from */
              )
{
	icmData* p = (icmData*)pp;
	icc* icp = p->icp;
	...
	if(p->data != NULL) {
		if(p->flag == icmDataASCII) {
			if((rv = check_null_string((char*)p->data, p->size)) !=
			   0) {
				sprintf(icp->err, "icmData_write: ASCII is not "
				                  "null terminated");
				icp->al->free(icp->al, buf);
				return icp->errc = 1;
			}
...
/* Dump a text description of the object */
static void
icmData_dump(icmBase* pp, FILE* op, /* Output to dump to */
             int verb               /* Verbosity level */
             )
{
	icmData* p = (icmData*)pp;
...
icmData_allocate(icmBase* pp)
{
	...
	if(p->size != p->_size) {
		if(p->data != NULL)
			icp->al->free(icp->al, p->data);
		if((p->data = (unsigned char*)icp->al->malloc(
		        icp->al, p->size * sizeof(unsigned char))) == NULL) {
			sprintf(
			    icp->err,
			    "icmData_alloc: malloc() of icmData data failed");
			return icp->errc = 2;
		}
		p->_size = p->size;
	}
...
/* read the object, return 0 on success, error code on fail */
static int
icmText_read(icmBase* pp, unsigned long len, /* tag length */
             unsigned long of                /* start offset within file */
             )
{
	icmText* p = (icmText*)pp;
	...
	if(p->size > 0) {
		if(check_null_string(bp, p->size) != 0) {
			sprintf(icp->err,
			        "icmText_read: text is not null terminated");
			icp->al->free(icp->al, buf);
			return icp->errc = 1;
		}
		if((rv = p->allocate((icmBase*)p)) != 0) {
 ...
 */
static int
icmText_write(icmBase* pp, unsigned long of /* File offset to write from */
              )
{
	icmText* p = (icmText*)pp;
	icc* icp = p->icp;
	...
	if(p->data != NULL) {
		if((rv = check_null_string(p->data, p->size)) != 0) {
			sprintf(icp->err,
			        "icmText_write: text is not null terminated");
			icp->al->free(icp->al, buf);
			return icp->errc = 1;
		}
		memcpy((void*)bp, (void*)p->data, p->size);
...
/* Dump a text description of the object */
static void
icmText_dump(icmBase* pp, FILE* op, /* Output to dump to */
             int verb               /* Verbosity level */
             )
{
	icmText* p = (icmText*)pp;
...
icmText_allocate(icmBase* pp)
{
	...
	if(p->size != p->_size) {
		if(p->data != NULL)
			icp->al->free(icp->al, p->data);
		if((p->data = (char*)icp->al->malloc(
		        icp->al, p->size * sizeof(char))) == NULL) {
			sprintf(
			    icp->err,
			    "icmText_alloc: malloc() of icmText data failed");
			return icp->errc = 2;
		}
		p->_size = p->size;
	}
...
/* Data conversion support functions */
static int
write_DateTimeNumber(icmDateTimeNumber* p, char* d)
{
	int rv;
	if(p->year < 1900 || p->year > 3000 || p->month == 0 || p->month > 12 ||
	   p->day == 0 || p->day > 31 || p->hours > 23 || p->minutes > 59 ||
	   p->seconds > 59)
		return 1;
...
static int
read_DateTimeNumber(icmDateTimeNumber* p, char* d)
{
	...
	if(p->year < 1900 || p->year > 3000 || p->month == 0 || p->month > 12 ||
	   ...
	   p->seconds > 59) {
		...
		if(p->month < 1900 || p->month > 3000 || p->year == 0 ||
		   p->year > 12 || p->hours == 0 || p->hours > 31 ||
		   p->day > 23 || p->seconds > 59 || p->minutes > 59)
			return 1; /* Nope */
...
static char*
string_DateTimeNumber(icmDateTimeNumber* p)
{
	static const char* mstring[13] = {"Bad", "Jan", "Feb", "Mar", "Apr",
	                                  "May", "Jun", "Jul", "Aug", "Sep",
	                                  "Oct", "Nov", "Dec"};
	static char buf[80];

	sprintf(buf, "%d %s %4d, %d:%02d:%02d", p->day,
	        mstring[p->month > 12 ? 0 : p->month], p->year, p->hours,
	        p->minutes, p->seconds);
	return buf;
}
...
static void
setcur_DateTimeNumber(icmDateTimeNumber* p)
{
	time_t cclk;
	struct tm* ctm;
...
/* Return the number of bytes needed to write this tag */
static unsigned int
icmDateTimeNumber_get_size(icmBase* pp)
{
	unsigned int len = 0;
	len += 8;  /* 8 bytes for tag and padding */
...
/* read the object, return 0 on success, error code on fail */
static int
icmDateTimeNumber_read(icmBase* pp, unsigned long len, /* tag length */
                       unsigned long of /* start offset within file */
                       )
{
	...
	if(len < 20) {
		sprintf(icp->err,
		        "icmDateTimeNumber_read: Tag too small to be legal");
		return icp->errc = 1;
	}
	...
	if(icp->fp->seek(icp->fp, of) != 0 ||
	   icp->fp->read(icp->fp, bp, 1, len) != len) {
		sprintf(icp->err,
		        "icmDateTimeNumber_read: fseek() or fread() failed");
		icp->al->free(icp->al, buf);
		return icp->errc = 1;
	}
	...
	/* Read type descriptor from the buffer */
	if(((icTagTypeSignature)read_SInt32Number(bp)) != p->ttype) {
		sprintf(icp->err, "icmDateTimeNumber_read: Wrong tag type for "
		                  "icmDateTimeNumber");
		icp->al->free(icp->al, buf);
		return icp->errc = 1;
	}
 ...
 */
static int
icmDateTimeNumber_write(icmBase* pp,
                        unsigned long of /* File offset to write from */
                        )
{
	icmDateTimeNumber* p = (icmDateTimeNumber*)pp;
	...
	if((rv = write_SInt32Number((int)p->ttype, bp)) != 0) {
		sprintf(icp->err,
		        "icmDateTimeNumber_write: write_SInt32Number() failed");
		icp->al->free(icp->al, buf);
		return icp->errc = rv;
	}
	write_SInt32Number(0, bp + 4); /* Set padding to 0 */
	...
	if((rv = write_DateTimeNumber(p, bp)) != 0) {
		sprintf(
		    icp->err,
		    "icmDateTimeNumber_write: write_DateTimeNumber() failed");
		icp->al->free(icp->al, buf);
		return icp->errc = rv;
	}
	...
	if(icp->fp->seek(icp->fp, of) != 0 ||
	   icp->fp->write(icp->fp, buf, 1, len) != len) {
		sprintf(icp->err,
		        "icmDateTimeNumber_write fseek() or fwrite() failed");
		icp->al->free(icp->al, buf);
		return icp->errc = 2;
	}
	icp->al->free(icp->al, buf);
...
/* Dump a text description of the object */
static void
icmDateTimeNumber_dump(icmBase* pp, FILE* op, /* Output to dump to */
                       int verb               /* Verbosity level */
                       )
{
	icmDateTimeNumber* p = (icmDateTimeNumber*)pp;
...
/* Allocate variable sized data elements */
static int
icmDateTimeNumber_allocate(icmBase* pp)
{
	/* Nothing to do */
	return 0;
}
...
/* Free all storage in the object */
static void
icmDateTimeNumber_delete(icmBase* pp)
{
	icmDateTimeNumber* p = (icmDateTimeNumber*)pp;
	icc* icp = p->icp;
...
/* Create an empty object. Return null on error */
static icmBase*
new_icmDateTimeNumber(icc* icp)
{
	icmDateTimeNumber* p;
	if((p = (icmDateTimeNumber*)icp->al->calloc(
	        icp->al, 1, sizeof(icmDateTimeNumber))) == NULL)
		return NULL;
...
static unsigned int
uipow(unsigned int a, unsigned int b)
{
	unsigned int rv = 1;
	for(; b > 0; b--)
		rv *= a;
...
static int
icmLut_nu_matrix(icmLut* p /* Pointer to Lut object */
                 )
{
	int i, j;
...
static void
icmLut_min_max(icmLut* p,             /* Pointer to Lut object */
               double* minp,          /* Return position of min/max */
               double* maxp, int chan /* Channel, -1 for average of all */
               )
{
	double* tp;
...
/* Return 0 on success, 1 if clipping occured, 2 on other error */
static int
icmLut_lookup_matrix(
    icmLut* p, /* Pointer to Lut object */
    double*
        out, /* Output array[outputChan] in ICC order - see Table 39 in 6.5.5 */
...
/* Return 0 on success, 1 if clipping occured, 2 on other error */
static int
icmLut_lookup_input(icmLut* p,   /* Pointer to Lut object */
    ...
    )
{
	...
	if(p->inputChan <= 8) {
		...
		if((gw = (double*)icp->al->malloc(
		        icp->al, (1 << p->inputChan) * sizeof(double))) ==
		   NULL) {
			sprintf(icp->err,
			        "icmLut_lookup_clut: malloc() failed");
			return icp->errc = 2;
		}
	}
	...
	/* Compute base index into grid and coordinate offsets */
	{
		...
		for(e = 0; e < p->inputChan; e++) {
			...
			gp +=
			    x *
			    p->dinc[e]; /* Add index offset for base of cube */
		}
	}
	...
	/* Now compute the output values */
	{
		...
		for(f = 0; f < p->outputChan; f++) /* Base of cube */
			out[f] = w * d[f];
		for(i = 1; i < (1 << p->inputChan);
		    i++) {         /* For all other corners of cube */
    ...
    )
{
	...
	/* Do selection sort on coordinates */
	{
		...
		for(e = 0; e < (p->inputChan - 1); e++) {
			...
			for(f = e + 1; f < p->inputChan;
			    f++) { /* Check against rest */
				int tt;
				tt = si[f];
				if(cosn > co[tt]) {
					si[f] = si[e]; /* Exchange */
	...
	/* Now compute the weightings, simplex vertices and output values */
	{
		...
		for(e = p->inputChan - 1; e > 0; e--) { /* Middle verticies */
			w = co[si[e]] - co[si[e - 1]];
			gp += p->dinc[si[e]]; /* Move to top of cell in next
			                         largest dimension */
			for(f = 0; f < p->outputChan; f++)
				out[f] += w * gp[f];
		}
...
/* Return 0 on success, 1 if clipping occured, 2 on other error */
static int
icmLut_lookup_output(icmLut* p,   /* Pointer to Lut object */
...
unsigned
psh_init(psh* p,       /* Pointer to structure to initialise */
...
/* Reset the counter */
void
psh_reset(psh* p /* Pointer to structure */
          )
{
	p->ix = 0;
}
...
int
psh_inc(psh* p,  /* Pointer to structure */
        ...
        )
{
	...
	do {
		...
		for(e = 0; e < di; e++) {
			...
			if(tv >= res) /* Dumbo filter - increment again if
			                 outside cube range */
				break;
			co[e] = tv;
		}
...
typedef enum {
	icmFromLuti =
	    0, /* return "fromo Lut normalized index" conversion function */
	icmToLuti =
	    1, /* return "to Lut normalized index" conversion function */
	icmFromLutv =
	    2, /* return "from Lut normalized value" conversion function */
...
static int getNormFunc(icColorSpaceSignature csig, icTagTypeSignature tagSig,
                       icmNormFlag flag,
...
icmLut_set_tables(
    ...
    double* inmin,
    double* inmax, /* Maximum range of inspace' values (NULL = default) */
    void (*clutfunc)(void* cbctx, double* out, double* in),
    /* inspace' -> outspace' transfer function */
    double* clutmin,
    double* clutmax, /* Maximum range of outspace' values (NULL = default) */
    ...
    )
{
	...
	double _iv[2 * MAX_CHAN],
	    * iv = &_iv[MAX_CHAN]; /* Real index value/table value */
	double imin[MAX_CHAN], imax[MAX_CHAN];
	double omin[MAX_CHAN], omax[MAX_CHAN];
	void (*ifromindex)(
	    double* out, double* in); /* Index to input color space function */
	void (*itoentry)(double* out,
	                 double* in); /* Input color space to entry function */
	void (*ifromentry)(
	    double* out, double* in); /* Entry to input color space function */
	void (*otoentry)(
	    double* out,
	    double* in); /* Output colorspace to table value function */
	void (*ofromentry)(
	    double* out,
	    double* in); /* Table value to output color space function */

	if(getNormFunc(insig, p->ttype, icmFromLuti, &ifromindex) != 0) {
		sprintf(icp->err, "icmLut_set_tables index to input colorspace "
		                  "function lookup failed");
		return icp->errc = 1;
	}
	if(getNormFunc(insig, p->ttype, icmToLutv, &itoentry) != 0) {
		sprintf(icp->err, "icmLut_set_tables input colorspace to table "
		                  "entry function lookup failed");
		return icp->errc = 1;
	}
	if(getNormFunc(insig, p->ttype, icmFromLutv, &ifromentry) != 0) {
		sprintf(icp->err, "icmLut_set_tables table entry to input "
		                  "colorspace function lookup failed");
		return icp->errc = 1;
	}

	if(getNormFunc(outsig, p->ttype, icmToLutv, &otoentry) != 0) {
		sprintf(icp->err, "icmLut_set_tables output colorspace to "
		                  "table entry function lookup failed");
		return icp->errc = 1;
	}
	if(getNormFunc(outsig, p->ttype, icmFromLutv, &ofromentry) != 0) {
		sprintf(icp->err, "icmLut_set_tables table entry to output "
		                  "colorspace function lookup failed");
		return icp->errc = 1;
	}
	...
	if(inmin == NULL) {
		...
		if(insig == icSigLabData) { /* Special case Lab */
			double mn[3], mx[3];
			/* Because the symetric range will cause slight
			 * clipping, */
			...
			if(p->inputEnt >= 64) {
				...
				itoentry(
				    imin,
				    mn); /* Convert from input color space to
		...
		{
			for(e = 0; e < p->inputChan; e++) {
				imin[e] = 0.0; /* We are assuming this is true
				                  for all other color spaces. */
				imax[e] = 1.0;
			}
		}
	} else {
		itoentry(imin, inmin); /* Convert from input color space to
		                          table representation */
		itoentry(imax, inmax);
	}
	...
	if(clutmin == NULL) {
		...
		if(outsig == icSigLabData) { /* Special case Lab */
			...
			if(p->outputEnt >= 64) {
				...
				otoentry(
				    omin,
				    mn); /* Convert from output color space to
				            table representation */
				otoentry(omax, mx);
			} else {
				for(e = 0; e < p->inputChan; e++) {
...
#endif
		{
			for(e = 0; e < p->outputChan; e++) {
				omin[e] = 0.0; /* We are assuming this is true
				                  for all other color spaces. */
				omax[e] = 1.0;
			}
		}
	} else {
		otoentry(omin, clutmin); /* Convert from output color space to
		                            table representation */
	...
	for(n = 0; n < p->inputEnt; n++) {
		...
		ifromindex(iv, iv); /* Convert from index value to input color
		                       space value */

		if(infunc != NULL)
			infunc(cbctx, iv, iv); /* In colorspace -> input table
			                          -> In colorspace. */

		itoentry(iv, iv); /* Convert from input color space value to
		                     table value */
	...
	psh_init(&counter, p->inputChan, p->clutPoints,
	         ii); /* Initialise counter */
	...
	for(;;) {
		...
		for(ti = e = 0; e < p->inputChan; e++) { /* Input tables */
			...
			iv[e] = ii[e] /
			        (p->clutPoints - 1.0); /* Vertex coordinates */
			iv[e] = iv[e] * (imax[e] - imin[e]) +
			        imin[e]; /* Undo expansion to 0.0 - 1.0 */
			*((int*)&iv[-e - 1]) =
			    ii[e]; /* Trick to supply grid index in iv[] */
		}

		ifromentry(
		    iv, iv); /* Convert from table value to input color space */

		/* Apply incolor -> outcolor function we want to represent */
		clutfunc(cbctx, iv, iv);

		otoentry(iv, iv); /* Convert from output color space value to
		                     table value */
	...
	for(n = 0; n < p->outputEnt; n++) {
		...
		ofromentry(iv,
		           iv); /* Convert from table value to output color
		                   space value */

		if(outfunc != NULL)
			outfunc(cbctx, iv, iv); /* Out colorspace -> output
			                           table -> out colorspace. */
...
/* read the object, return 0 on success, error code on fail */
static int
icmLut_read(icmBase* pp, unsigned long len, /* tag length */
            unsigned long of                /* start offset within file */
            )
{
	icmLut* p = (icmLut*)pp;
	...
	if(p->ttype == icSigLut8Type) {
		if(len < 48) {
			sprintf(icp->err,
			        "icmLut_read: Tag too small to be legal");
			icp->al->free(icp->al, buf);
			return icp->errc = 1;
		}
	} else {
	...
	if(p->inputChan > MAX_CHAN) {
		sprintf(icp->err,
		        "icmLut_read: Can't handle > %d input channels\n",
		        MAX_CHAN);
		return icp->errc = 1;
	}
	...
	if(p->outputChan > MAX_CHAN) {
		sprintf(icp->err,
		        "icmLut_read: Can't handle > %d output channels\n",
		        MAX_CHAN);
		return icp->errc = 1;
	}
	...
	for(j = 0; j < 3; j++) {         /* Rows */
		for(i = 0; i < 3; i++) { /* Columns */
			p->e[j][i] =
			    read_S15Fixed16Number(bp + 12 + ((j * 3 + i) * 4));
		}
	}
 ...
 */
static int
icmLut_write(icmBase* pp, unsigned long of /* File offset to write from */
             )
{
	icmLut* p = (icmLut*)pp;
	icc* icp = p->icp;
	...
	for(j = 0; j < 3; j++) {         /* Rows */
		for(i = 0; i < 3; i++) { /* Columns */
			if((rv = write_S15Fixed16Number(
			        p->e[j][i], bp + 12 + ((j * 3 + i) * 4))) !=
			   0) {
				sprintf(icp->err, "icmLut_write: "
				                  "write_S15Fixed16Number() "
				                  "failed");
				icp->al->free(icp->al, buf);
				return icp->errc = rv;
			}
	...
	if(p->ttype == icSigLut8Type) {
		if(p->inputEnt != 256 || p->outputEnt != 256) {
			sprintf(icp->err, "icmLut_write: 8 bit Input and "
			                  "Output tables must be 256 entries");
			icp->al->free(icp->al, buf);
			return icp->errc = 1;
		}
		bp = buf + 48;
		...
		if((rv = write_UInt16Number(p->inputEnt, bp + 48)) != 0) {
			sprintf(icp->err,
			        "icmLut_write: write_UInt16Number() failed");
			icp->al->free(icp->al, buf);
			return icp->errc = rv;
		}
	...
	if(p->ttype == icSigLut8Type) {
		for(i = 0; i < size; i++, bp += 1) {
			if((rv = write_DCS8Number(p->inputTable[i], bp)) != 0) {
				sprintf(icp->err, "icmLut_write: inputTable "
				                  "write_DCS8Number() failed");
				icp->al->free(icp->al, buf);
				return icp->errc = rv;
			}
		}
		...
		for(i = 0; i < size; i++, bp += 2) {
			if((rv = write_DCS16Number(p->inputTable[i], bp)) !=
			   0) {
				sprintf(icp->err, "icmLut_write: inputTable "
				                  "write_DCS16Number(%f) "
				                  "failed",
	...
	if(p->ttype == icSigLut8Type) {
		for(i = 0; i < size; i++, bp += 1) {
			if((rv = write_DCS8Number(p->clutTable[i], bp)) != 0) {
				sprintf(icp->err, "icmLut_write: clutTable "
				                  "write_DCS8Number() failed");
				icp->al->free(icp->al, buf);
				return icp->errc = rv;
			}
	...
	if(p->ttype == icSigLut8Type) {
		for(i = 0; i < size; i++, bp += 1) {
			if((rv = write_DCS8Number(p->outputTable[i], bp)) !=
			   0) {
				sprintf(icp->err, "icmLut_write: outputTable "
				                  "write_DCS8Number() failed");
				icp->al->free(icp->al, buf);
				return icp->errc = rv;
			}
		...
		for(i = 0; i < size; i++, bp += 2) {
			if((rv = write_DCS16Number(p->outputTable[i], bp)) !=
			   0) {
				sprintf(icp->err, "icmLut_write: outputTable "
				                  "write_DCS16Number(%f) "
				                  "failed",
...
/* Dump a text description of the object */
static void
icmLut_dump(icmBase* pp, FILE* op, /* Output to dump to */
            int verb               /* Verbosity level */
            )
{
	icmLut* p = (icmLut*)pp;
...
icmLut_allocate(icmBase* pp)
{
	...
	if(size != p->inputTable_size) {
		...
		if((p->inputTable = (double*)icp->al->calloc(
		        icp->al, sizeof(double), size)) == NULL) {
			sprintf(icp->err, "icmLut_alloc: calloc() of Lut "
			                  "inputTable data failed");
			return icp->errc = 2;
		}
	...
	if(size != p->clutTable_size) {
		...
		if((p->clutTable = (double*)icp->al->calloc(
		        icp->al, sizeof(double), size)) == NULL) {
			sprintf(icp->err, "icmLut_alloc: calloc() of Lut "
			                  "clutTable data failed");
			return icp->errc = 2;
		}
	...
	if(size != p->outputTable_size) {
		...
		if((p->outputTable = (double*)icp->al->calloc(
		        icp->al, sizeof(double), size)) == NULL) {
			sprintf(icp->err, "icmLut_alloc: calloc() of Lut "
			                  "outputTable data failed");
			return icp->errc = 2;
		}
...
/* Return the number of bytes needed to write this tag */
static unsigned int
icmMeasurement_get_size(icmBase* pp)
{
	unsigned int len = 0;
	len += 8;  /* 8 bytes for tag and padding */
...
/* read the object, return 0 on success, error code on fail */
static int
icmMeasurement_read(icmBase* pp, unsigned long len, /* tag length */
                    unsigned long of /* start offset within file */
                    )
{
	...
	if(len < 36) {
		sprintf(icp->err,
		        "icmMeasurement_read: Tag too small to be legal");
		return icp->errc = 1;
	}
	...
	if(icp->fp->seek(icp->fp, of) != 0 ||
	   icp->fp->read(icp->fp, bp, 1, len) != len) {
		sprintf(icp->err,
		        "icmMeasurement_read: fseek() or fread() failed");
		icp->al->free(icp->al, buf);
		return icp->errc = 1;
	}
	...
	if(((icTagTypeSignature)read_SInt32Number(bp)) != p->ttype) {
		sprintf(
		    icp->err,
		    "icmMeasurement_read: Wrong tag type for icmMeasurement");
		icp->al->free(icp->al, buf);
		return icp->errc = 1;
	}
 ...
 */
static int
icmMeasurement_write(icmBase* pp,
                     unsigned long of /* File offset to write from */
                     )
{
	icmMeasurement* p = (icmMeasurement*)pp;
	...
	if((rv = write_SInt32Number((int)p->ttype, bp)) != 0) {
		sprintf(
		    icp->err,
		    "icmMeasurement_write, type: write_SInt32Number() failed");
		icp->al->free(icp->al, buf);
		return icp->errc = rv;
	}
	write_SInt32Number(0, bp + 4); /* Set padding to 0 */
	...
	if((rv = write_SInt32Number((int)p->observer, bp + 8)) != 0) {
		sprintf(icp->err, "icmMeasurementa_write, observer: "
		                  "write_SInt32Number() failed");
		icp->al->free(icp->al, buf);
		return icp->errc = rv;
	}
	...
	if((rv = write_XYZNumber(&p->backing, bp + 12)) != 0) {
		sprintf(icp->err,
		        "icmMeasurement, backing: write_XYZNumber error");
		icp->al->free(icp->al, buf);
		return icp->errc = rv;
	}
	...
	if((rv = write_SInt32Number((int)p->geometry, bp + 24)) != 0) {
		sprintf(icp->err, "icmMeasurementa_write, geometry: "
		                  "write_SInt32Number() failed");
		icp->al->free(icp->al, buf);
		return icp->errc = rv;
	}
	...
	if((rv = write_U16Fixed16Number(p->flare, bp + 28)) != 0) {
		sprintf(icp->err, "icmMeasurementa_write, flare: "
		                  "write_U16Fixed16Number() failed");
		icp->al->free(icp->al, buf);
		return icp->errc = rv;
	}
	...
	if((rv = write_SInt32Number((int)p->illuminant, bp + 32)) != 0) {
		sprintf(icp->err, "icmMeasurementa_write, illuminant: "
		                  "write_SInt32Number() failed");
		icp->al->free(icp->al, buf);
		return icp->errc = rv;
	}
	...
	if(icp->fp->seek(icp->fp, of) != 0 ||
	   icp->fp->write(icp->fp, buf, 1, len) != len) {
		sprintf(icp->err,
		        "icmMeasurement_write fseek() or fwrite() failed");
		icp->al->free(icp->al, buf);
		return icp->errc = 2;
	}
	icp->al->free(icp->al, buf);
...
/* Dump a text description of the object */
static void
icmMeasurement_dump(icmBase* pp, FILE* op, /* Output to dump to */
                    int verb               /* Verbosity level */
                    )
{
	icmMeasurement* p = (icmMeasurement*)pp;
...
/* Allocate variable sized data elements */
static int
icmMeasurement_allocate(icmBase* pp)
{
	/* Nothing to do */
	return 0;
}
...
/* Free all storage in the object */
static void
icmMeasurement_delete(icmBase* pp)
{
	icmMeasurement* p = (icmMeasurement*)pp;
	icc* icp = p->icp;
...
/* Create an empty object. Return null on error */
static icmBase*
new_icmMeasurement(icc* icp)
{
	icmMeasurement* p;
	if((p = (icmMeasurement*)icp->al->calloc(
	        icp->al, 1, sizeof(icmMeasurement))) == NULL)
		return NULL;
...
static int
read_NamedColorVal(
    icmNamedColorVal* p, char* bp, char* end,
    ...
    )
{
	...
	if(check_null_string(bp, mxl)) {
		sprintf(
		    icp->err,
		    "icmNamedColorVal_read: Root name string not terminated");
		return icp->errc = 1;
	}
	strcpy((void*)p->root, (void*)bp);
	bp += strlen(p->root) + 1;
	if((bp + ndc) > end) {
		sprintf(icp->err, "icmNamedColorVal_read: Data too short to "
		                  "read device coords");
		return icp->errc = 1;
	}
	for(i = 0; i < ndc; i++) {
    ...
    )
{
	...
	if((bp + 32 + 6 + ndc * 2) > end) {
		sprintf(icp->err,
		        "icmNamedColorVal2_read: Data too short to read");
		return icp->errc = 1;
	}
	if(check_null_string(bp, 32)) {
		sprintf(
		    icp->err,
		    "icmNamedColorVal2_read: Root name string not terminated");
		return icp->errc = 1;
	}
    ...
    )
{
	...
	if(check_null_string(p->root, 32) != 0) {
		sprintf(icp->err, "icmNamedColorVal_write: Root string names "
		                  "is unterminated");
		return icp->errc = 1;
	}
	...
	for(i = 0; i < ndc; i++) {
		if((rv = write_DCS8Number(p->deviceCoords[i], d)) != 0) {
			sprintf(icp->err, "icmNamedColorVal_write: write of "
			                  "device coord failed");
			return icp->errc = 1;
		}
		d += 1;
...
static int
write_NamedColorVal2(
    icmNamedColorVal* p, char* bp,
    icColorSpaceSignature pcs, /* Header Profile Connection Space */
    unsigned int ndc           /* Number of device corrds */
    )
{
	...
	if(check_null_string(p->root, 32)) {
		sprintf(icp->err, "icmNamedColorVal2_write: Root string names "
		                  "is unterminated");
		return icp->errc = 1;
	}
	...
	if(rv) {
		sprintf(icp->err,
		        "icmNamedColorVal2_write: write of PCS coord failed");
		return icp->errc = 1;
	}
	for(i = 0; i < ndc; i++) {
		if((rv = write_DCS16Number(p->deviceCoords[i],
		                           bp + 32 + 6 + 2 * i)) != 0) {
			sprintf(icp->err, "icmNamedColorVal2_write: write of "
			                  "device coord failed");
			return icp->errc = 1;
		}
	}
...
/* Return the number of bytes needed to write this tag */
static unsigned int
icmNamedColor_get_size(icmBase* pp)
{
	...
	if(p->ttype == icSigNamedColorType) {
		...
		len += p->count *
		       (32 + 6 +
		        p->nDeviceCoords * 2); /* bytes for each named color */
	}
	return len;
}
...
/* read the object, return 0 on success, error code on fail */
static int
icmNamedColor_read(icmBase* pp, unsigned long len, /* tag length */
                   unsigned long of /* start offset within file */
                   )
{
	icmNamedColor* p = (icmNamedColor*)pp;
	...
	if(len < 4) {
		sprintf(icp->err,
		        "icmNamedColor_read: Tag too small to be legal");
		return icp->errc = 1;
	}
	...
	if(icp->fp->seek(icp->fp, of) != 0 ||
	   icp->fp->read(icp->fp, bp, 1, len) != len) {
		sprintf(icp->err,
		        "icmNamedColor_read: fseek() or fread() failed");
		icp->al->free(icp->al, buf);
		return icp->errc = 1;
	}
	...
	if(p->ttype != icSigNamedColorType &&
	   p->ttype != icSigNamedColor2Type) {
		sprintf(icp->err,
		        "icmNamedColor_read: Wrong tag type for icmNamedColor");
		icp->al->free(icp->al, buf);
		return icp->errc = 1;
	}
	...
	if(p->ttype == icSigNamedColorType) {
		...
		p->nDeviceCoords =
		    number_ColorSpaceSignature(icp->header->colorSpace);
		if(p->nDeviceCoords > MAX_CHAN) {
			sprintf(icp->err, "icmNamedColor_read: Can't handle "
			                  "more than %d device channels",
			        MAX_CHAN);
			icp->al->free(icp->al, buf);
			return icp->errc = 1;
		}
	...
	if(p->ttype == icSigNamedColorType) {
		...
		if(check_null_string(bp, mxl) != 0) {
			sprintf(icp->err, "icmNamedColor_read: Color prefix is "
			                  "not null terminated");
			icp->al->free(icp->al, buf);
			return icp->errc = 1;
		}
		...
		mxl = (end - bp) < 32 ? (end - bp) : 32;
		if(check_null_string(bp, mxl) != 0) {
			sprintf(icp->err, "icmNamedColor_read: Color suffix is "
			                  "not null terminated");
			icp->al->free(icp->al, buf);
			return icp->errc = 1;
		}
		...
		/* Read all the data from the buffer */
		for(i = 0; i < p->count; i++) {
			if((rv = read_NamedColorVal(p->data + i, bp, end,
			                            icp->header->pcs,
			                            p->nDeviceCoords)) != 0) {
				icp->al->free(icp->al, buf);
				return rv;
		...
		bp = bp + 84;
		for(i = 0; i < p->count;
		    i++, bp += (32 + 6 + p->nDeviceCoords * 2)) {
			if((rv = read_NamedColorVal2(p->data + i, bp, end,
			                             icp->header->pcs,
			                             p->nDeviceCoords)) != 0) {
				icp->al->free(icp->al, buf);
				return rv;
 ...
 */
static int
icmNamedColor_write(icmBase* pp,
                    unsigned long of /* File offset to write from */
                    )
{
	icmNamedColor* p = (icmNamedColor*)pp;
	...
	if((rv = write_SInt32Number((int)p->ttype, bp)) != 0) {
		sprintf(icp->err,
		        "icmNamedColor_write: write_SInt32Number() failed");
		icp->al->free(icp->al, buf);
		return icp->errc = rv;
	}
	write_SInt32Number(0, bp + 4); /* Set padding to 0 */
	...
	if((rv = write_UInt32Number(p->vendorFlag, bp + 8)) != 0) {
		sprintf(icp->err,
		        "icmNamedColor_write: write_UInt32Number() failed");
		icp->al->free(icp->al, buf);
		return icp->errc = rv;
	}
	...
	if(p->ttype == icSigNamedColorType) {
		...
		if((rv = check_null_string(p->prefix, 32)) != 0) {
			sprintf(icp->err, "icmNamedColor_write: Color prefix "
		...
		if(check_null_string(p->suffix, 32)) {
			sprintf(icp->err, "icmNamedColor_write: Color sufix is "
			                  "not null terminated");
			icp->al->free(icp->al, buf);
			return icp->errc = 1;
		}
		...
		for(i = 0; i < p->count; i++) {
			if((rv = write_NamedColorVal(p->data + i, bp,
			                             icp->header->pcs,
			                             p->nDeviceCoords)) != 0) {
				icp->al->free(icp->al, buf);
				return rv;
		...
		for(i = 0; i < p->count;
		    i++, bp += (32 + 6 + p->nDeviceCoords * 2)) {
			if((rv = write_NamedColorVal2(p->data + i, bp,
			                              icp->header->pcs,
			                              p->nDeviceCoords)) != 0) {
				icp->al->free(icp->al, buf);
				return rv;
	...
	if(icp->fp->seek(icp->fp, of) != 0 ||
	   icp->fp->write(icp->fp, buf, 1, len) != len) {
		sprintf(icp->err,
		        "icmNamedColor_write fseek() or fwrite() failed");
		icp->al->free(icp->al, buf);
		return icp->errc = 2;
	}
	icp->al->free(icp->al, buf);
...
/* Dump a text description of the object */
static void
icmNamedColor_dump(icmBase* pp, FILE* op, /* Output to dump to */
                   int verb               /* Verbosity level */
                   )
{
	icmNamedColor* p = (icmNamedColor*)pp;
...
/* Allocate variable sized data elements */
static int
icmNamedColor_allocate(icmBase* pp)
{
	icmNamedColor* p = (icmNamedColor*)pp;
	icc* icp = p->icp;
	...
	if(p->count != p->_count) {
		...
		if(p->data != NULL)
			icp->al->free(icp->al, p->data);
		if((p->data = (icmNamedColorVal*)icp->al->calloc(
		        icp->al, p->count, sizeof(icmNamedColorVal))) == NULL) {
			sprintf(icp->err, "icmNamedColor_alloc: malloc() of "
			                  "icmNamedColor data failed");
			return icp->errc = 2;
		}
		for(i = 0; i < p->count; i++) {
...
/* Free all storage in the object */
static void
icmNamedColor_delete(icmBase* pp)
{
	icmNamedColor* p = (icmNamedColor*)pp;
	icc* icp = p->icp;
...
/* Create an empty object. Return null on error */
static icmBase*
new_icmNamedColor(icc* icp)
{
	icmNamedColor* p;
	if((p = (icmNamedColor*)icp->al->calloc(icp->al, 1,
	                                        sizeof(icmNamedColor))) == NULL)
		return NULL;
	p->ttype = icSigNamedColor2Type;
	p->refcount = 1;
	...
	/* Default the the number of device coords appropriately for
	 * NamedColorType */
...
/* Return the number of bytes needed to write this tag */
static unsigned int
icmTextDescription_get_size(icmBase* pp)
{
...
/* read the object, return 0 on success, error code on fail */
static int
icmTextDescription_read(icmBase* pp, unsigned long len, /* tag length */
                        unsigned long of /* start offset within file */
                        )
{
	icmTextDescription* p = (icmTextDescription*)pp;
...
#endif
		sprintf(icp->err,
		        "icmTextDescription_read: Tag too small to be legal");
		return icp->errc = 1;
	}
	...
	if(icp->fp->seek(icp->fp, of) != 0 ||
	   icp->fp->read(icp->fp, bp, 1, len) != len) {
		sprintf(icp->err,
		        "icmTextDescription_read: fseek() or fread() failed");
		icp->al->free(icp->al, buf);
		return icp->errc = 1;
	}
    ...
    )
{
	...
	if((bp + 8) > end) {
		sprintf(icp->err, "icmTextDescription_read: Data too short to "
		                  "type descriptor");
		*bpp = bp;
		return icp->errc = 1;
	}
	...
	if(((icTagTypeSignature)read_SInt32Number(bp)) != p->ttype) {
		*bpp = bp;
		sprintf(icp->err, "icmTextDescription_read: Wrong tag type "
		                  "('%s') for icmTextDescription",
	...
	if(p->size > 0) {
		if((bp + p->size) > end) {
			*bpp = bp;
			sprintf(icp->err, "icmTextDescription_read: Data to "
			                  "short to read Ascii string");
			return icp->errc = 1;
		}
		if(check_null_string(bp, p->size)) {
			*bpp = bp;
			sprintf(icp->err, "icmTextDescription_read: ascii "
	...
	if(p->ucSize > 0) {
		...
		if((bp + 2 * p->ucSize) > end) {
			*bpp = bp;
			sprintf(icp->err, "icmTextDescription_read: Data too "
			                  "short to read Unicode string");
			return icp->errc = 1;
		}
		if(check_null_string16(bp, p->ucSize)) {
			*bpp = bp;
			sprintf(icp->err, "icmTextDescription_read: Unicode "
			                  "string is not terminated");
			return icp->errc = 1;
		}
		if((rv = p->allocate((icmBase*)p)) != 0) {
	...
	if(p->scSize > 0) {
		if(p->scSize > 67) {
			*bpp = bp;
			sprintf(icp->err, "icmTextDescription_read: ScriptCode "
			                  "string too long");
			return icp->errc = 1;
		}
		if((bp + p->scSize) > end) {
			*bpp = bp;
			sprintf(icp->err, "icmTextDescription_read: Data too "
			                  "short to read ScriptCode string");
			return icp->errc = 1;
		}
 ...
 */
static int
icmTextDescription_write(icmBase* pp,
                         unsigned long of /* File offset to write from */
                         )
{
	icmTextDescription* p = (icmTextDescription*)pp;
	...
	if(icp->fp->seek(icp->fp, of) != 0 ||
	   icp->fp->write(icp->fp, buf, 1, len) != len) {
		sprintf(icp->err,
		        "icmTextDescription_write fseek() or fwrite() failed");
		icp->al->free(icp->al, buf);
		return icp->errc = 2;
	}
	icp->al->free(icp->al, buf);
...
/* Core write the contents of the object. Return 0 on sucess, error code on
 * failure */
static int
icmTextDescription_core_write(
    icmTextDescription* p,
    char** bpp /* Pointer to buffer pointer, returns next after read */
    )
{
	...
	if((rv = write_SInt32Number((int)p->ttype, bp)) != 0) {
		sprintf(
		    icp->err,
		    "icmTextDescription_write: write_SInt32Number() failed");
		*bpp = bp;
		return icp->errc = rv;
	}
	write_SInt32Number(0, bp + 4); /* Set padding to 0 */
	...
	if((rv = write_UInt32Number(p->size, bp)) != 0) {
		sprintf(
		    icp->err,
		    "icmTextDescription_write: write_UInt32Number() failed");
		*bpp = bp;
		return icp->errc = rv;
	}
	bp += 4;
	if(p->size > 0) {
		if(check_null_string(p->desc, p->size)) {
			*bpp = bp;
			sprintf(icp->err, "icmTextDescription_write: ascii "
			                  "string is not terminated");
			return icp->errc = 1;
		}
	...
	if(p->ucSize > 0) {
		...
		if(check_null_string16((char*)p->ucDesc, p->ucSize)) {
			*bpp = bp;
			sprintf(icp->err, "icmTextDescription_write: Unicode "
			                  "string is not terminated");
			return icp->errc = 1;
		}
		for(up = p->ucDesc; *up != 0; up++, bp += 2) {
			if((rv = write_UInt16Number(((unsigned int)*up), bp)) !=
			   0) {
				sprintf(icp->err, "icmTextDescription_write: "
				                  "write_UInt16Number() "
				                  "failed");
				*bpp = bp;
				return icp->errc = rv;
	...
	if((rv = write_UInt16Number(p->scCode, bp)) != 0) {
		sprintf(
		    icp->err,
		    "icmTextDescription_write: write_UInt16Number() failed");
		*bpp = bp;
		return icp->errc = rv;
	}
	bp += 2;
	if((rv = write_UInt8Number(p->scSize, bp)) != 0) {
		sprintf(icp->err,
		        "icmTextDescription_write: write_UInt8Number() failed");
		*bpp = bp;
		return icp->errc = rv;
	}
	bp += 1;
	if(p->scSize > 0) {
		if(p->scSize > 67) {
			*bpp = bp;
			sprintf(icp->err, "icmTextDescription_write: "
			                  "ScriptCode string too long");
			return icp->errc = 1;
		}
		if(check_null_string((char*)p->scDesc, p->scSize)) {
			*bpp = bp;
			sprintf(icp->err, "icmTextDescription_write: "
			                  "ScriptCode string is not "
			                  "terminated");
			return icp->errc = 1;
		}
...
/* Dump a text description of the object */
static void
icmTextDescription_dump(icmBase* pp, FILE* op, /* Output to dump to */
                        int verb               /* Verbosity level */
                        )
{
	icmTextDescription* p = (icmTextDescription*)pp;
...
/* Allocate variable sized data elements */
static int
icmTextDescription_allocate(icmBase* pp)
{
	icmTextDescription* p = (icmTextDescription*)pp;
	icc* icp = p->icp;
	...
	if(p->size != p->_size) {
		...
		if((p->desc = (char*)icp->al->malloc(
		        icp->al, p->size * sizeof(char))) == NULL) {
			sprintf(icp->err, "icmTextDescription_alloc: malloc() "
			                  "of Ascii description failed");
			return icp->errc = 2;
		}
		p->_size = p->size;
	}
	if(p->ucSize != p->uc_size) {
		...
		if((p->ucDesc = (ORD16*)icp->al->malloc(
		        icp->al, p->ucSize * sizeof(ORD16))) == NULL) {
			sprintf(icp->err, "icmTextDescription_alloc: malloc() "
			                  "of Unicode description failed");
			return icp->errc = 2;
		}
		p->uc_size = p->ucSize;
	}
...
static void
icmTextDescription_unallocate(icmTextDescription* p)
{
	icc* icp = p->icp;
...
/* Free all storage in the object */
static void
icmTextDescription_delete(icmBase* pp)
{
	icmTextDescription* p = (icmTextDescription*)pp;
	icc* icp = p->icp;
...
static void
icmTextDescription_init(icmTextDescription* p, icc* icp)
{
...
/* Create an empty object. Return null on error */
static icmBase*
new_icmTextDescription(icc* icp)
{
	icmTextDescription* p;
	if((p = (icmTextDescription*)icp->al->calloc(
	        icp->al, 1, sizeof(icmTextDescription))) == NULL)
		return NULL;
...
/* Return the number of bytes needed to write this tag */
static unsigned int
icmDescStruct_get_size(icmDescStruct* p)
{
	unsigned int len = 0;
    ...
    )
{
	...
	if((bp + 20) > end) {
		sprintf(icp->err,
		        "icmDescStruct_read: Data too short read header");
		*bpp = bp;
		return icp->errc = 1;
	}
    ...
    )
{
	...
	if((rv = write_SInt32Number(p->deviceMfg, bp + 0)) != 0) {
		sprintf(icp->err,
		        "icmDescStruct_write: write_SInt32Number() failed");
		*bpp = bp;
		return icp->errc = rv;
	}
	if((rv = write_UInt32Number(p->deviceModel, bp + 4)) != 0) {
		sprintf(icp->err,
		        "icmDescStruct_write: write_UInt32Number() failed");
		*bpp = bp;
		return icp->errc = rv;
	}
	if((rv = write_UInt64Number(&p->attributes, bp + 8)) != 0) {
		sprintf(icp->err,
		        "icmDescStruct_write: write_UInt64Number() failed");
		*bpp = bp;
		return icp->errc = rv;
	}
...
/* Dump a text description of the object */
static void
icmDescStruct_dump(icmDescStruct* p, FILE* op, /* Output to dump to */
                   int verb,                   /* Verbosity level */
...
static int
icmDescStruct_allocate(icmDescStruct* p)
{
	int rv;
...
/* Free all storage in the object */
static void
icmDescStruct_delete(icmDescStruct* p)
{
...
static void
icmDescStruct_init(icmDescStruct* p, icc* icp)
{
...
/* Return the number of bytes needed to write this tag */
static unsigned int
icmProfileSequenceDesc_get_size(icmBase* pp)
{
...
/* read the object, return 0 on success, error code on fail */
static int
icmProfileSequenceDesc_read(icmBase* pp, unsigned long len, /* tag length */
                            unsigned long of /* start offset within file */
                            )
{
	icmProfileSequenceDesc* p = (icmProfileSequenceDesc*)pp;
	...
	if(len < 12) {
		sprintf(
		    icp->err,
		    "icmProfileSequenceDesc_read: Tag too small to be legal");
		return icp->errc = 1;
	}
	...
	if((buf = (char*)icp->al->malloc(icp->al, len)) == NULL) {
		sprintf(icp->err,
		        "icmProfileSequenceDesc_read: malloc() failed");
		return icp->errc = 2;
	}
	bp = buf;
	end = buf + len;
	...
	if(icp->fp->seek(icp->fp, of) != 0 ||
	   icp->fp->read(icp->fp, bp, 1, len) != len) {
		sprintf(
		    icp->err,
		    "icmProfileSequenceDesc_read: fseek() or fread() failed");
		icp->al->free(icp->al, buf);
		return icp->errc = 1;
	}
	...
	/* Read type descriptor from the buffer */
	if(((icTagTypeSignature)read_SInt32Number(bp)) != p->ttype) {
		sprintf(icp->err, "icmProfileSequenceDesc_read: Wrong tag type "
		                  "for icmProfileSequenceDesc");
		icp->al->free(icp->al, buf);
		return icp->errc = 1;
	}
	bp += 8; /* Skip padding */
 ...
 */
static int
icmProfileSequenceDesc_write(icmBase* pp,
                             unsigned long of /* File offset to write from */
                             )
{
	icmProfileSequenceDesc* p = (icmProfileSequenceDesc*)pp;
	...
	if((buf = (char*)icp->al->malloc(icp->al, len)) == NULL) {
		sprintf(icp->err,
		        "icmProfileSequenceDesc_write malloc() failed");
		return icp->errc = 2;
	}
	bp = buf;
	...
	/* Write type descriptor to the buffer */
	if((rv = write_SInt32Number((int)p->ttype, bp)) != 0) {
		sprintf(icp->err, "icmProfileSequenceDesc_write: "
		                  "write_SInt32Number() failed");
		icp->al->free(icp->al, buf);
		return icp->errc = rv;
	}
	...
	if(icp->fp->seek(icp->fp, of) != 0 ||
	   icp->fp->write(icp->fp, buf, 1, len) != len) {
		sprintf(
		    icp->err,
		    "icmProfileSequenceDesc_write fseek() or fwrite() failed");
		icp->al->free(icp->al, buf);
		return icp->errc = 2;
	}
	icp->al->free(icp->al, buf);
...
/* Dump a text description of the object */
static void
icmProfileSequenceDesc_dump(icmBase* pp, FILE* op, /* Output to dump to */
                            int verb               /* Verbosity level */
                            )
{
	icmProfileSequenceDesc* p = (icmProfileSequenceDesc*)pp;
...
static int
icmProfileSequenceDesc_allocate(icmBase* pp)
{
	icmProfileSequenceDesc* p = (icmProfileSequenceDesc*)pp;
	icc* icp = p->icp;
	unsigned int i;
	...
	if(p->count != p->_count) {
		if(p->data != NULL)
			icp->al->free(icp->al, p->data);
		if((p->data = (icmDescStruct*)icp->al->malloc(
		        icp->al, p->count * sizeof(icmDescStruct))) == NULL) {
			sprintf(icp->err, "icmProfileSequenceDesc_allocate "
			                  "Allocation of DescStruct array "
			                  "failed");
			return icp->errc = 2;
		}
...
/* Free all storage in the object */
static void
icmProfileSequenceDesc_delete(icmBase* pp)
{
	icmProfileSequenceDesc* p = (icmProfileSequenceDesc*)pp;
	icc* icp = p->icp;
	unsigned int i;
...
/* Create an empty object. Return null on error */
static icmBase*
new_icmProfileSequenceDesc(icc* icp)
{
	icmProfileSequenceDesc* p;
	if((p = (icmProfileSequenceDesc*)icp->al->calloc(
	        icp->al, 1, sizeof(icmProfileSequenceDesc))) == NULL)
		return NULL;
...
/* Return the number of bytes needed to write this tag */
static unsigned int
icmSignature_get_size(icmBase* pp)
{
	unsigned int len = 0;
	len += 8; /* 8 bytes for tag and padding */
...
/* read the object, return 0 on success, error code on fail */
static int
icmSignature_read(icmBase* pp, unsigned long len, /* tag length */
                  unsigned long of                /* start offset within file */
                  )
{
	...
	if(len < 12) {
		sprintf(icp->err,
		        "icmSignature_read: Tag too small to be legal");
		return icp->errc = 1;
	}
	...
	if(icp->fp->seek(icp->fp, of) != 0 ||
	   icp->fp->read(icp->fp, bp, 1, len) != len) {
		sprintf(icp->err,
		        "icmSignature_read: fseek() or fread() failed");
		icp->al->free(icp->al, buf);
		return icp->errc = 1;
	}
	...
	if(((icTagTypeSignature)read_SInt32Number(bp)) != p->ttype) {
		sprintf(icp->err,
		        "icmSignaturSignatureng tag type for icmSignature");
		icp->al->free(icp->al, buf);
		return icp->errc = 1;
	}
 ...
 */
static int
icmSignature_write(icmBase* pp, unsigned long of /* File offset to write from */
                   )
{
	icmSignature* p = (icmSignature*)pp;
	icc* icp = p->icp;
	...
	if((rv = write_SInt32Number((int)p->ttype, bp)) != 0) {
		sprintf(icp->err,
		        "icmSignature_write: write_SInt32Number() failed");
		icp->al->free(icp->al, buf);
		return icp->errc = rv;
	}
	write_SInt32Number(0, bp + 4); /* Set padding to 0 */
	...
	if((rv = write_SInt32Number((int)p->sig, bp + 8)) != 0) {
		sprintf(icp->err,
		        "icmSignaturea_write: write_SInt32Number() failed");
		icp->al->free(icp->al, buf);
		return icp->errc = rv;
	}
	...
	if(icp->fp->seek(icp->fp, of) != 0 ||
	   icp->fp->write(icp->fp, buf, 1, len) != len) {
		sprintf(icp->err,
		        "icmSignature_write fseek() or fwrite() failed");
		icp->al->free(icp->al, buf);
		return icp->errc = 2;
	}
	icp->al->free(icp->al, buf);
...
/* Dump a text description of the object */
static void
icmSignature_dump(icmBase* pp, FILE* op, /* Output to dump to */
                  int verb               /* Verbosity level */
                  )
{
	icmSignature* p = (icmSignature*)pp;
...
/* Allocate variable sized data elements */
static int
icmSignature_allocate(icmBase* pp)
{
	/* Nothing to do */
	return 0;
}
...
/* Free all storage in the object */
static void
icmSignature_delete(icmBase* pp)
{
	icmSignature* p = (icmSignature*)pp;
	icc* icp = p->icp;
...
/* Create an empty object. Return null on error */
static icmBase*
new_icmSignature(icc* icp)
{
	icmSignature* p;
	if((p = (icmSignature*)icp->al->calloc(icp->al, 1,
	                                       sizeof(icmSignature))) == NULL)
		return NULL;
...
/* Data conversion support functions */
static int
read_ScreeningData(icmScreeningData* p, char* d)
{
...
/* Return the number of bytes needed to write this tag */
static unsigned int
icmScreening_get_size(icmBase* pp)
{
...
/* read the object, return 0 on success, error code on fail */
static int
icmScreening_read(icmBase* pp, unsigned long len, /* tag length */
                  unsigned long of                /* start offset within file */
                  )
{
	icmScreening* p = (icmScreening*)pp;
	...
	if(len < 12) {
		sprintf(icp->err,
		        "icmScreening_read: Tag too small to be legal");
		return icp->errc = 1;
	}
	...
	if(icp->fp->seek(icp->fp, of) != 0 ||
	   icp->fp->read(icp->fp, bp, 1, len) != len) {
		sprintf(icp->err,
		        "icmScreening_read: fseek() or fread() failed");
		icp->al->free(icp->al, buf);
		return icp->errc = 1;
	}
	...
	if(((icTagTypeSignature)read_SInt32Number(bp)) != p->ttype) {
		sprintf(icp->err,
		        "icmScreening_read: Wrong tag type for icmScreening");
		icp->al->free(icp->al, buf);
		return icp->errc = 1;
	}
	...
	for(i = 0; i < p->channels; i++, bp += 12) {
		if((bp + 12) > end) {
			sprintf(icp->err, "icmScreening_read: Data too short "
			                  "to read Screening Data");
			icp->al->free(icp->al, buf);
			return icp->errc = 1;
		}
 ...
 */
static int
icmScreening_write(icmBase* pp, unsigned long of /* File offset to write from */
                   )
{
	icmScreening* p = (icmScreening*)pp;
	icc* icp = p->icp;
	...
	if((rv = write_SInt32Number((int)p->ttype, bp)) != 0) {
		sprintf(icp->err,
		        "icmScreening_write: write_SInt32Number() failed");
		icp->al->free(icp->al, buf);
		return icp->errc = rv;
	}
	write_SInt32Number(0, bp + 4); /* Set padding to 0 */
	...
	if((rv = write_UInt32Number(p->screeningFlag, bp + 8)) != 0) {
		sprintf(icp->err,
		        "icmScreening_write: write_UInt32Number() failed");
		icp->al->free(icp->al, buf);
		return icp->errc = rv;
	}
	if((rv = write_UInt32Number(p->channels, bp + 12)) != 0) {
		sprintf(
		    icp->err,
		    "icmScreening_write: write_UInt32NumberXYZumber() failed");
		icp->al->free(icp->al, buf);
		return icp->errc = rv;
	}
	bp = bp + 16;
	...
	for(i = 0; i < p->channels; i++, bp += 12) {
		if((rv = write_ScreeningData(&p->data[i], bp)) != 0) {
			sprintf(
			    icp->err,
			    "icmScreening_write: write_ScreeningData() failed");
			icp->al->free(icp->al, buf);
			return icp->errc = rv;
		}
	}
	...
	if(icp->fp->seek(icp->fp, of) != 0 ||
	   icp->fp->write(icp->fp, buf, 1, len) != len) {
		sprintf(icp->err,
		        "icmScreening_write fseek() or fwrite() failed");
		icp->al->free(icp->al, buf);
		return icp->errc = 2;
	}
	icp->al->free(icp->al, buf);
...
/* Dump a text description of the object */
static void
icmScreening_dump(icmBase* pp, FILE* op, /* Output to dump to */
                  int verb               /* Verbosity level */
                  )
{
	icmScreening* p = (icmScreening*)pp;
...
/* Allocate variable sized data elements */
static int
icmScreening_allocate(icmBase* pp)
{
	icmScreening* p = (icmScreening*)pp;
	icc* icp = p->icp;
	...
	if(p->channels != p->_channels) {
		if(p->data != NULL)
			icp->al->free(icp->al, p->data);
		if((p->data = (icmScreeningData*)icp->al->malloc(
		        icp->al, p->channels * sizeof(icmScreeningData))) ==
		   NULL) {
...
/* Free all storage in the object */
static void
icmScreening_delete(icmBase* pp)
{
	icmScreening* p = (icmScreening*)pp;
	icc* icp = p->icp;
...
/* Create an empty object. Return null on error */
static icmBase*
new_icmScreening(icc* icp)
{
	icmScreening* p;
	if((p = (icmScreening*)icp->al->calloc(icp->al, 1,
	                                       sizeof(icmScreening))) == NULL)
		return NULL;
...
/* read the object, return 0 on success, error code on fail */
static int
icmUcrBg_read(icmBase* pp, unsigned long len, /* tag length */
              unsigned long of                /* start offset within file */
              )
{
	icmUcrBg* p = (icmUcrBg*)pp;
	...
	if(p->UCRcount > 0) {
		...
		for(i = 0; i < p->UCRcount; i++, bp += 2) {
			if((bp + 2) > end) {
				sprintf(icp->err, "icmUcrBg_read: Data too "
				                  "short to read UCR Data");
				icp->al->free(icp->al, buf);
				return icp->errc = 1;
			}
			if(p->UCRcount == 1) /* % */
	...
	if((bp + 4) > end) {
		sprintf(icp->err,
		        "icmData_read: Data too short to read Black Gen count");
		icp->al->free(icp->al, buf);
		return icp->errc = 1;
	}
	...
	if(p->size > 0) {
		if(check_null_string(bp, p->size) != 0) {
			sprintf(icp->err,
			        "icmUcrBg_read: string is not null terminated");
			icp->al->free(icp->al, buf);
			return icp->errc = 1;
		}
		p->size = strlen(bp) + 1;
 ...
 */
static int
icmUcrBg_write(icmBase* pp, unsigned long of /* File offset to write from */
               )
{
	icmUcrBg* p = (icmUcrBg*)pp;
	icc* icp = p->icp;
	...
	if((rv = write_SInt32Number((int)p->ttype, bp)) != 0) {
		sprintf(icp->err,
		        "icmUcrBg_write: write_SInt32Number() failed");
		icp->al->free(icp->al, buf);
		return icp->errc = rv;
	}
	write_SInt32Number(0, bp + 4); /* Set padding to 0 */
	...
	if((rv = write_UInt32Number(p->UCRcount, bp)) != 0) {
		sprintf(icp->err,
		        "icmUcrBg_write: write_UInt32Number() failed");
		icp->al->free(icp->al, buf);
		return icp->errc = rv;
	}
	bp += 4;
	...
	for(i = 0; i < p->UCRcount; i++, bp += 2) {
		if(p->UCRcount == 1) { /* % */
			if((rv = write_UInt16Number(
			        (unsigned int)(p->UCRcurve[i] + 0.5), bp)) !=
			   0) {
				sprintf(icp->err, "icmUcrBg_write: "
				                  "write_UInt16umber() failed");
				icp->al->free(icp->al, buf);
				return icp->errc = rv;
			}
		} else {
	...
	for(i = 0; i < p->BGcount; i++, bp += 2) {
		if(p->BGcount == 1) { /* % */
			if((rv = write_UInt16Number(
			        (unsigned int)(p->BGcurve[i] + 0.5), bp)) !=
			   0) {
				sprintf(icp->err, "icmUcrBg_write: "
				                  "write_UInt16umber() failed");
				icp->al->free(icp->al, buf);
	...
	if(p->string != NULL) {
		if((rv = check_null_string(p->string, p->size)) != 0) {
			sprintf(icp->err,
			        "icmUcrBg_write: text is not null terminated");
			icp->al->free(icp->al, buf);
			return icp->errc = 1;
		}
...
/* Dump a text description of the object */
static void
icmUcrBg_dump(icmBase* pp, FILE* op, /* Output to dump to */
              int verb               /* Verbosity level */
              )
{
	icmUcrBg* p = (icmUcrBg*)pp;
...
icmUcrBg_allocate(icmBase* pp)
{
	...
	if(p->UCRcount != p->UCR_count) {
		...
		if((p->UCRcurve = (double*)icp->al->malloc(
		        icp->al, p->UCRcount * sizeof(double))) == NULL) {
	...
	if(p->BGcount != p->BG_count) {
		...
		if((p->BGcurve = (double*)icp->al->malloc(
		        icp->al, p->BGcount * sizeof(double))) == NULL) {
	...
	if(p->size != p->_size) {
		...
		if((p->string = (char*)icp->al->malloc(
		        icp->al, p->size * sizeof(char))) == NULL) {
			sprintf(icp->err, "icmUcrBg_allocate: malloc() of "
			                  "string data failed");
			return icp->errc = 2;
		}
		p->_size = p->size;
...
/* Create an empty object. Return null on error */
static icmBase*
new_icmUcrBg(icc* icp)
{
	icmUcrBg* p;
	if((p = (icmUcrBg*)icp->al->calloc(icp->al, 1, sizeof(icmUcrBg))) ==
	   NULL)
		return NULL;
...
static unsigned int
icmVideoCardGamma_get_size(icmBase* pp)
{
...
/* read the object, return 0 on success, error code on fail */
static int
icmVideoCardGamma_read(icmBase* pp, unsigned long len, /* tag length */
                       unsigned long of /* start offset within file */
                       )
{
	icmVideoCardGamma* p = (icmVideoCardGamma*)pp;
	...
	if(len < 18) {
		sprintf(icp->err,
		        "icmVideoCardGamma_read: Tag too small to be legal");
		return icp->errc = 1;
	}
	...
	if(icp->fp->seek(icp->fp, of) != 0 ||
	   icp->fp->read(icp->fp, bp, 1, len) != len) {
		sprintf(icp->err,
		        "icmVideoCardGamma_read: fseek() or fread() failed");
		icp->al->free(icp->al, buf);
		return icp->errc = 1;
	}
	...
	/* Read type descriptor from the buffer */
	if(((icTagTypeSignature)read_SInt32Number(bp)) != p->ttype) {
		sprintf(icp->err, "icmVideoCardGamma_read: Wrong tag type for "
		                  "icmVideoCardGamma");
		icp->al->free(icp->al, buf);
		return icp->errc = 1;
	}
	...
	case icmVideoCardGammaTableType:
		...
		if(len - 18 < p->u.table.channels * p->u.table.entryCount *
		                  p->u.table.entrySize) {
			sprintf(icp->err, "icmVideoCardGamma_read: Tag too "
			                  "small to be legal");
			return icp->errc = 1;
		}
		...
		for(c = 0, bp = bp + 18;
		    c < p->u.table.channels * p->u.table.entryCount; c++) {
			...
			case 2:
				*pshort++ = read_UInt16Number(bp);
				...
				break;
			default:
				sprintf(icp->err, "icmVideoCardGamma_read: "
				                  "unsupported table entry "
				                  "size");
				pp->del(pp);
				icp->al->free(icp->al, buf);
				return icp->errc = 1;
	...
	case icmVideoCardGammaFormulaType:
		...
		break;
	default:
		sprintf(icp->err, "icmVideoCardGammaTable_read: Unknown gamma "
		                  "format for icmVideoCardGamma");
		icp->al->free(icp->al, buf);
		return icp->errc = 1;
	}
 ...
 */
static int
icmVideoCardGamma_write(icmBase* pp,
                        unsigned long of /* File offset to write from */
                        )
{
	icmVideoCardGamma* p = (icmVideoCardGamma*)pp;
	...
	if((rv = write_SInt32Number((int)p->ttype, bp)) != 0) {
		sprintf(icp->err,
		        "icmVideoCardGamma_write: write_SInt32Number() failed");
		icp->al->free(icp->al, buf);
		return icp->errc = rv;
	}
	write_SInt32Number(0, bp + 4); /* Set padding to 0 */
	...
	if((rv = write_UInt32Number(p->tagType, bp + 8)) != 0) {
		sprintf(icp->err,
		        "icmVideoCardGamma_write: write_UInt32Number() failed");
		icp->al->free(icp->al, buf);
		return icp->errc = rv;
	}
	...
	switch((int)p->tagType) {
	case icmVideoCardGammaTableType:
		if((rv = write_UInt16Number(p->u.table.channels, bp + 12)) !=
		   0) {
			sprintf(icp->err, "icmVideoCardGamma_write: "
			                  "write_UInt16Number() failed");
			icp->al->free(icp->al, buf);
			return icp->errc = rv;
		}
		if((rv = write_UInt16Number(p->u.table.entryCount, bp + 14)) !=
		   0) {
			sprintf(icp->err, "icmVideoCardGamma_write: "
			                  "write_UInt16Number() failed");
			icp->al->free(icp->al, buf);
		...
		if((rv = write_UInt16Number(p->u.table.entrySize, bp + 16)) !=
		   0) {
			sprintf(icp->err, "icmVideoCardGamma_write: "
			                  "write_UInt16Number() failed");
			icp->al->free(icp->al, buf);
		...
		break;
	case icmVideoCardGammaFormulaType:
		if((rv = write_S15Fixed16Number(p->u.formula.redGamma,
		                                bp + 12)) != 0) {
			sprintf(icp->err, "icmVideoCardGamma_write: "
			                  "write_S15Fixed16Number() failed");
			icp->al->free(icp->al, buf);
			return icp->errc = rv;
		}
		if((rv = write_S15Fixed16Number(p->u.formula.redMin,
		                                bp + 16)) != 0) {
			sprintf(icp->err, "icmVideoCardGamma_write: "
			                  "write_S15Fixed16Number() failed");
			icp->al->free(icp->al, buf);
		...
		if((rv = write_S15Fixed16Number(p->u.formula.redMax,
		                                bp + 20)) != 0) {
			sprintf(icp->err, "icmVideoCardGamma_write: "
			                  "write_S15Fixed16Number() failed");
			icp->al->free(icp->al, buf);
		...
		if((rv = write_S15Fixed16Number(p->u.formula.greenGamma,
		                                bp + 24)) != 0) {
			sprintf(icp->err, "icmVideoCardGamma_write: "
			                  "write_S15Fixed16Number() failed");
			icp->al->free(icp->al, buf);
		...
		if((rv = write_S15Fixed16Number(p->u.formula.greenMin,
		                                bp + 28)) != 0) {
			sprintf(icp->err, "icmVideoCardGamma_write: "
			                  "write_S15Fixed16Number() failed");
			icp->al->free(icp->al, buf);
		...
		if((rv = write_S15Fixed16Number(p->u.formula.greenMax,
		                                bp + 32)) != 0) {
			sprintf(icp->err, "icmVideoCardGamma_write: "
			                  "write_S15Fixed16Number() failed");
			icp->al->free(icp->al, buf);
		...
		if((rv = write_S15Fixed16Number(p->u.formula.blueGamma,
		                                bp + 36)) != 0) {
			sprintf(icp->err, "icmVideoCardGamma_write: "
			                  "write_S15Fixed16Number() failed");
			icp->al->free(icp->al, buf);
		...
		if((rv = write_S15Fixed16Number(p->u.formula.blueMin,
		                                bp + 40)) != 0) {
			sprintf(icp->err, "icmVideoCardGamma_write: "
			                  "write_S15Fixed16Number() failed");
			icp->al->free(icp->al, buf);
		...
		if((rv = write_S15Fixed16Number(p->u.formula.blueMax,
		                                bp + 44)) != 0) {
			sprintf(icp->err, "icmVideoCardGamma_write: "
			                  "write_S15Fixed16Number() failed");
			icp->al->free(icp->al, buf);
		...
		break;
	default:
		sprintf(icp->err, "icmVideoCardGammaTable_write: Unknown gamma "
		                  "format for icmVideoCardGamma");
		icp->al->free(icp->al, buf);
		return icp->errc = 1;
	}
	...
	if(icp->fp->seek(icp->fp, of) != 0 ||
	   icp->fp->write(icp->fp, buf, 1, len) != len) {
		sprintf(
		    icp->err,
		    "icmViewingConditions_write fseek() or fwrite() failed");
		icp->al->free(icp->al, buf);
		return icp->errc = 2;
	}
	icp->al->free(icp->al, buf);
...
/* Dump a text description of the object */
static void
icmVideoCardGamma_dump(icmBase* pp, FILE* op, /* Output to dump to */
                       int verb               /* Verbosity level */
                       )
{
	icmVideoCardGamma* p = (icmVideoCardGamma*)pp;
...
/* Allocate variable sized data elements */
static int
icmVideoCardGamma_allocate(icmBase* pp)
{
	icmVideoCardGamma* p = (icmVideoCardGamma*)pp;
	icc* icp = p->icp;
	int size;
	...
	if(p->tagType == icmVideoCardGammaTableType) {
		...
		size = (p->u.table.channels * p->u.table.entryCount);
		switch(p->u.table.entrySize) {
		case 1:
		...
		if((p->u.table.data = (void*)icp->al->malloc(icp->al, size)) ==
		   NULL) {
			sprintf(icp->err, "icmVideoCardGamma_alloc: malloc() "
			                  "of table data failed");
			return icp->errc = 2;
		}
	}
...
/* Free all storage in the object */
static void
icmVideoCardGamma_delete(icmBase* pp)
{
	icmVideoCardGamma* p = (icmVideoCardGamma*)pp;
	icc* icp = p->icp;
...
/* Create an empty object. Return null on error */
static icmBase*
new_icmVideoCardGamma(icc* icp)
{
	icmVideoCardGamma* p;
	if((p = (icmVideoCardGamma*)icp->al->calloc(
	        icp->al, 1, sizeof(icmVideoCardGamma))) == NULL)
		return NULL;
...
/* Return the number of bytes needed to write this tag */
static unsigned int
icmViewingConditions_get_size(icmBase* pp)
{
	unsigned int len = 0;
	len += 8;  /* 8 bytes for tag and padding */
...
/* read the object, return 0 on success, error code on fail */
static int
icmViewingConditions_read(icmBase* pp, unsigned long len, /* tag length */
                          unsigned long of /* start offset within file */
                          )
{
	...
	if(len < 36) {
		sprintf(icp->err,
		        "icmViewingConditions_read: Tag too small to be legal");
		return icp->errc = 1;
	}
	...
	if(icp->fp->seek(icp->fp, of) != 0 ||
	   icp->fp->read(icp->fp, bp, 1, len) != len) {
		sprintf(icp->err,
		        "icmViewingConditions_read: fseek() or fread() failed");
		icp->al->free(icp->al, buf);
		return icp->errc = 1;
	}
	...
	/* Read type descriptor from the buffer */
	if(((icTagTypeSignature)read_SInt32Number(bp)) != p->ttype) {
		sprintf(icp->err, "icmViewingConditions_read: Wrong tag type "
		                  "for icmViewingConditions");
		icp->al->free(icp->al, buf);
		return icp->errc = 1;
	}
 ...
 */
static int
icmViewingConditions_write(icmBase* pp,
                           unsigned long of /* File offset to write from */
                           )
{
	icmViewingConditions* p = (icmViewingConditions*)pp;
	...
	if((rv = write_SInt32Number((int)p->ttype, bp)) != 0) {
		sprintf(
		    icp->err,
		    "icmViewingConditions_write: write_SInt32Number() failed");
		icp->al->free(icp->al, buf);
		return icp->errc = rv;
	}
	write_SInt32Number(0, bp + 4); /* Set padding to 0 */
	...
	if((rv = write_XYZNumber(&p->illuminant, bp + 8)) != 0) {
		sprintf(icp->err,
		        "icmViewingConditions: write_XYZNumber error");
		icp->al->free(icp->al, buf);
		return icp->errc = rv;
	}
	...
	if((rv = write_SInt32Number((int)p->stdIlluminant, bp + 32)) != 0) {
		sprintf(
		    icp->err,
		    "icmViewingConditionsa_write: write_SInt32Number() failed");
		icp->al->free(icp->al, buf);
		return icp->errc = rv;
	}
...
/* Dump a text description of the object */
static void
icmViewingConditions_dump(icmBase* pp, FILE* op, /* Output to dump to */
                          int verb               /* Verbosity level */
                          )
{
	icmViewingConditions* p = (icmViewingConditions*)pp;
...
/* Allocate variable sized data elements */
static int
icmViewingConditions_allocate(icmBase* pp)
{
	/* Nothing to do */
	return 0;
}
...
/* Free all storage in the object */
static void
icmViewingConditions_delete(icmBase* pp)
{
	icmViewingConditions* p = (icmViewingConditions*)pp;
	icc* icp = p->icp;
...
/* Create an empty object. Return null on error */
static icmBase*
new_icmViewingConditions(icc* icp)
{
	icmViewingConditions* p;
	if((p = (icmViewingConditions*)icp->al->calloc(
	        icp->al, 1, sizeof(icmViewingConditions))) == NULL)
		return NULL;
...
/* Return the number of bytes needed to write this tag */
static unsigned int
icmCrdInfo_get_size(icmBase* pp)
{
...
/* read the object, return 0 on success, error code on fail */
static int
icmCrdInfo_read(icmBase* pp, unsigned long len, /* tag length */
                unsigned long of                /* start offset within file */
                )
{
	icmCrdInfo* p = (icmCrdInfo*)pp;
	...
	if(((icTagTypeSignature)read_SInt32Number(bp)) != p->ttype) {
		sprintf(icp->err,
		        "icmCrdInfo_read: Wrong tag type for icmCrdInfo");
		icp->al->free(icp->al, buf);
		return icp->errc = 1;
	}
	bp = bp + 8;
	...
	if((bp + 4) > end) {
		sprintf(icp->err, "icmCrdInfo_read: Data too short to read "
		                  "Postscript product name");
		icp->al->free(icp->al, buf);
		return icp->errc = 1;
	}
	...
	if(p->ppsize > 0) {
		if((bp + p->ppsize) > end) {
			sprintf(icp->err, "icmCrdInfo_read: Data to short to "
			                  "read Postscript product string");
			icp->al->free(icp->al, buf);
			return icp->errc = 1;
		}
		if(check_null_string(bp, p->ppsize)) {
			sprintf(icp->err, "icmCrdInfo_read: Postscript product "
	...
	for(t = 0; t < 4; t++) { /* For all 4 intents */
		if((bp + 4) > end) {
			sprintf(icp->err, "icmCrdInfo_read: Data too short to "
			                  "read CRD%ld name",
			        t);
			icp->al->free(icp->al, buf);
			return icp->errc = 1;
		...
		if(p->crdsize[t] > 0) {
			if((bp + p->crdsize[t]) > end) {
				sprintf(icp->err, "icmCrdInfo_read: Data to "
				                  "short to read CRD%ld string",
				        t);
				icp->al->free(icp->al, buf);
				return icp->errc = 1;
			}
			if(check_null_string(bp, p->crdsize[t])) {
				sprintf(icp->err, "icmCrdInfo_read: CRD%ld "
				                  "name is not terminated",
				        t);
				icp->al->free(icp->al, buf);
				return icp->errc = 1;
 ...
 */
static int
icmCrdInfo_write(icmBase* pp, unsigned long of /* File offset to write from */
                 )
{
	icmCrdInfo* p = (icmCrdInfo*)pp;
	icc* icp = p->icp;
	...
	if((rv = write_SInt32Number((int)p->ttype, bp)) != 0) {
		sprintf(icp->err,
		        "icmCrdInfo_write: write_SInt32Number() failed");
		icp->al->free(icp->al, buf);
		return icp->errc = rv;
	}
	write_SInt32Number(0, bp + 4); /* Set padding to 0 */
	...
	if((rv = write_UInt32Number(p->ppsize, bp)) != 0) {
		sprintf(icp->err,
		        "icmCrdInfo_write: write_UInt32Number() failed");
		icp->al->free(icp->al, buf);
		return icp->errc = rv;
	}
	bp += 4;
	if(p->ppsize > 0) {
		if((rv = check_null_string(p->ppname, p->ppsize)) != 0) {
			sprintf(icp->err, "icmCrdInfo_write: Postscript "
			                  "product name is not terminated");
			icp->al->free(icp->al, buf);
			return icp->errc = 1;
		}
	...
	for(t = 0; t < 4; t++) { /* For all 4 intents */
		...
		bp += 4;
		if(p->ppsize > 0) {
			if((rv = check_null_string(p->crdname[t],
			                           p->crdsize[t])) != 0) {
				sprintf(icp->err, "icmCrdInfo_write: CRD%ld "
				                  "name is not terminated",
				        t);
				icp->al->free(icp->al, buf);
				return icp->errc = 1;
	...
	if(icp->fp->seek(icp->fp, of) != 0 ||
	   icp->fp->write(icp->fp, buf, 1, len) != len) {
		sprintf(icp->err,
		        "icmCrdInfo_write fseek() or fwrite() failed");
		icp->al->free(icp->al, buf);
		return icp->errc = 2;
	}
	icp->al->free(icp->al, buf);
...
/* Dump a text description of the object */
static void
icmCrdInfo_dump(icmBase* pp, FILE* op, /* Output to dump to */
                int verb               /* Verbosity level */
                )
{
	icmCrdInfo* p = (icmCrdInfo*)pp;
...
/* Allocate variable sized data elements */
static int
icmCrdInfo_allocate(icmBase* pp)
{
	icmCrdInfo* p = (icmCrdInfo*)pp;
	icc* icp = p->icp;
	unsigned int t;
	...
	if(p->ppsize != p->_ppsize) {
		...
		if((p->ppname = (char*)icp->al->malloc(
		        icp->al, p->ppsize * sizeof(char))) == NULL) {
			sprintf(
			    icp->err,
			    "icmCrdInfo_alloc: malloc() of string data failed");
			return icp->errc = 2;
		}
		p->_ppsize = p->ppsize;
	}
	for(t = 0; t < 4; t++) { /* For all 4 intents */
		if(p->crdsize[t] != p->_crdsize[t]) {
			...
			if((p->crdname[t] = (char*)icp->al->malloc(
			        icp->al, p->crdsize[t] * sizeof(char))) ==
			   NULL) {
				sprintf(icp->err, "icmCrdInfo_alloc: malloc() "
				                  "of CRD%d name string failed",
				        t);
				return icp->errc = 2;
			}
...
/* Create an empty object. Return null on error */
static icmBase*
new_icmCrdInfo(icc* icp)
{
	icmCrdInfo* p;
	if((p = (icmCrdInfo*)icp->al->calloc(icp->al, 1, sizeof(icmCrdInfo))) ==
	   NULL)
		return NULL;
...
/* Return the number of bytes needed to write this tag */
static unsigned int
icmHeader_get_size(icmHeader* p)
{
...
/* read the object, return 0 on success, error code on fail */
static int
icmHeader_read(icmHeader* p, unsigned long len, /* tag length */
               unsigned long of                 /* start offset within file */
               )
{
	icc* icp = p->icp;
	...
	p->majv =
	    (tt >> 4) * 10 + (tt & 0xf); /* Integer major version number */
	...
	p->deviceClass = (icProfileClassSignature)read_SInt32Number(
	    buf + 12); /* Type of profile */
	p->colorSpace = (icColorSpaceSignature)read_SInt32Number(
	    buf + 16); /* Color space of data */
	p->pcs = (icColorSpaceSignature)read_SInt32Number(
	    buf + 20); /* PCS: XYZ or Lab */
	if((rv = read_DateTimeNumber(&p->date, buf + 24)) !=
	   0) { /* Creation Date */
		sprintf(icp->err,
		        "icmHeader_read: read_DateTimeNumber corrupted");
		icp->al->free(icp->al, buf);
		return icp->errc = rv;
	}
	tt = read_SInt32Number(buf + 36);
	if(tt != icMagicNumber) { /* Check magic number */
		sprintf(icp->err, "icmHeader_read: wrong magic number 0x%x",
		        tt);
		icp->al->free(icp->al, buf);
		return icp->errc = 1;
	}
	p->platform = (icPlatformSignature)read_SInt32Number(
	    buf + 40);                                 /* Primary platform */
	...
	p->renderingIntent = (icRenderingIntent)read_SInt32Number(
	    buf + 64); /* Rendering intent */
	if((rv = read_XYZNumber(&p->illuminant, buf + 68)) !=
	   0) { /* Profile illuminant */
 ...
 */
static int
icmHeader_write(icmHeader* p, unsigned long of /* File offset to write from */
                )
{
	icc* icp = p->icp;
	...
	len = p->get_size(p);
	if((buf = (char*)icp->al->calloc(icp->al, 1, len)) ==
	   NULL) { /* Zero it - some CMS are fussy */
		sprintf(icp->err, "icmHeader_write calloc() failed");
		return icp->errc = 2;
	}

	/* Fill in the write buffer */
	if((rv = write_UInt32Number(p->size, buf + 0)) !=
	   0) { /* Profile size in bytes */
		sprintf(icp->err, "icmHeader_write: profile size");
		icp->al->free(icp->al, buf);
		return icp->errc = rv;
	}

	if((rv = write_SInt32Number(p->cmmId, buf + 4)) !=
	   0) { /* CMM for profile */
	...
	if(p->majv < 0 || p->majv > 99 /* Sanity check version numbers */
	   || p->minv < 0 || p->minv > 9 || p->bfv < 0 || p->bfv > 9) {
		sprintf(icp->err, "icmHeader_write: version number");
		icp->al->free(icp->al, buf);
		return icp->errc = 1;
	}
	tt = ((p->majv / 10) << 4) + (p->majv % 10);
	if((rv = write_UInt8Number(tt, buf + 8)) !=
	   0) { /* Raw major version number */
		sprintf(icp->err, "icmHeader_write: Uint8Number major version");
		icp->al->free(icp->al, buf);
		return icp->errc = rv;
	}
	tt = (p->minv << 4) + p->bfv;
	if((rv = write_UInt8Number(tt, buf + 9)) !=
	   0) { /* Raw minor/bug fix version numbers */
		sprintf(icp->err, "icmHeader_write: Uint8Number minor/bug fix");
		icp->al->free(icp->al, buf);
		return icp->errc = rv;
	}
	if((rv = write_SInt32Number((int)p->deviceClass, buf + 12)) !=
	   0) { /* Type of profile */
		sprintf(icp->err, "icmHeader_write: SInt32Number deviceClass");
		icp->al->free(icp->al, buf);
		return icp->errc = rv;
	}
	if((rv = write_SInt32Number((int)p->colorSpace, buf + 16)) !=
	   0) { /* Color space of data */
		sprintf(icp->err,
		        "icmHeader_write: SInt32Number data color space");
		icp->al->free(icp->al, buf);
		return icp->errc = rv;
	}
	if((rv = write_SInt32Number((int)p->pcs, buf + 20)) !=
	   0) { /* PCS: XYZ or Lab */
		sprintf(icp->err, "icmHeader_write: SInt32Number PCS");
		icp->al->free(icp->al, buf);
		return icp->errc = rv;
	}
	if((rv = write_DateTimeNumber(&p->date, buf + 24)) !=
	   0) { /* Creation Date */
		sprintf(icp->err, "icmHeader_write: DateTimeNumber creation");
		icp->al->free(icp->al, buf);
		return icp->errc = rv;
	}
	if((rv = write_SInt32Number(icMagicNumber, buf + 36)) !=
	   0) { /* Magic number */
		sprintf(icp->err, "icmHeader_write: SInt32Number magic");
		icp->al->free(icp->al, buf);
		return icp->errc = rv;
	}
	if((rv = write_SInt32Number((int)p->platform, buf + 40)) !=
	   0) { /* Primary platform */
		sprintf(icp->err, "icmHeader_write: SInt32Number platform");
		icp->al->free(icp->al, buf);
		return icp->errc = rv;
	}
	if((rv = write_UInt32Number(p->flags, buf + 44)) !=
	   0) { /* Various bits */
		sprintf(icp->err, "icmHeader_write: UInt32Number flags");
		icp->al->free(icp->al, buf);
		return icp->errc = rv;
	}
	if((rv = write_SInt32Number(p->manufacturer, buf + 48)) !=
	   0) { /* Dev manufacturer */
	...
	if((rv = write_UInt64Number(&p->attributes, buf + 56)) !=
	   0) { /* Device attributes */
		sprintf(icp->err, "icmHeader_write: UInt64Number attributes");
		icp->al->free(icp->al, buf);
		return icp->errc = rv;
	}
	if((rv = write_SInt32Number((int)p->renderingIntent, buf + 64)) !=
	   0) { /* Rendering intent */
		sprintf(icp->err,
		        "icmHeader_write: SInt32Number rendering intent");
		icp->al->free(icp->al, buf);
		return icp->errc = rv;
	}
	if((rv = write_XYZNumber(&p->illuminant, buf + 68)) !=
	   0) { /* Profile illuminant */
		sprintf(icp->err, "icmHeader_write: XYZNumber illuminant");
		icp->al->free(icp->al, buf);
		return icp->errc = rv;
	}
	if((rv = write_SInt32Number(p->creator, buf + 80)) !=
	   0) { /* Profile creator */
...
static void
icmHeader_dump(icmHeader* p, FILE* op, /* Output to dump to */
               int verb                /* Verbosity level */
               )
{
	if(verb <= 0)
...
/* Create an empty object. Return null on error */
static icmHeader*
new_icmHeader(icc* icp)
{
	icmHeader* p;
	if((p = (icmHeader*)icp->al->calloc(icp->al, 1, sizeof(icmHeader))) ==
	   NULL)
		return NULL;
	p->icp = icp;
...
} typetable[] = {{icSigCrdInfoType, new_icmCrdInfo},
...
static struct {
    ...
    {icSigProfileSequenceDescTag,
     {icSigProfileSequenceDescType, icMaxEnumType}},
...
static struct {
	...
	icColorSpaceSignature
	    colsig; /* Data Color space signature, icMaxEnumData for ignore, */
	            /*                           icSigPCSData for XYZ of Lab */
	icColorSpaceSignature
	    pcssig; /* PCS Color space signature, icMaxEnumData for ignore, */
    ...
    {icSigInputClass,
     ...
     icMaxEnumData,
     icSigPCSData,
     {icSigProfileDescriptionTag, icSigGrayTRCTag, icSigMediaWhitePointTag,
    ...
    {icSigInputClass,
     ...
     icMaxEnumData,
     icSigXYZData,
     {icSigProfileDescriptionTag, icSigRedColorantTag, icSigGreenColorantTag,
      icSigBlueColorantTag, icSigRedTRCTag, icSigGreenTRCTag, icSigBlueTRCTag,
      icSigMediaWhitePointTag, icSigCopyrightTag, icMaxEnumType}},

    {icSigInputClass,
     -100,
     icMaxEnumData,
     icSigPCSData,
     {icSigProfileDescriptionTag, icSigAToB0Tag, icSigMediaWhitePointTag,
      icSigCopyrightTag, icMaxEnumType}},
    ...
    {icSigDisplayClass,
     -3,
     icSigRgbData,
     icSigXYZData, /* Rgb or any 3 component space ?? */
     {icSigProfileDescriptionTag, icSigRedColorantTag, icSigGreenColorantTag,
      icSigBlueColorantTag, icSigRedTRCTag, icSigGreenTRCTag, icSigBlueTRCTag,
      icSigMediaWhitePointTag, icSigCopyrightTag, icMaxEnumType}},
    ...
    {icSigOutputClass,
     ...
     icMaxEnumData,
     icSigPCSData,
     {icSigProfileDescriptionTag, icSigAToB0Tag, icSigBToA0Tag, icSigGamutTag,
      icSigAToB1Tag, icSigBToA1Tag, icSigAToB2Tag, icSigBToA2Tag,
      icSigMediaWhitePointTag, icSigCopyrightTag, icMaxEnumType}},
    ...
    {icSigOutputClass,
     -100,
     icMaxEnumData,
     icSigPCSData, /* Assumed from Hexachrome examples */
     {icSigProfileDescriptionTag, icSigBToA0Tag, icSigBToA1Tag, icSigBToA2Tag,
      icSigMediaWhitePointTag, icSigCopyrightTag, icMaxEnumType}},
    ...
    {icSigLinkClass,
     ...
     icMaxEnumData,
     icMaxEnumData,
     {icSigProfileDescriptionTag, icSigAToB0Tag, icSigProfileSequenceDescTag,
      icSigCopyrightTag, icMaxEnumType}},
    ...
    {icSigColorSpaceClass,
     ...
     icMaxEnumData,
     icSigPCSData,
     {icSigProfileDescriptionTag, icSigBToA0Tag, icSigAToB0Tag,
      icSigMediaWhitePointTag, icSigCopyrightTag, icMaxEnumType}},
    ...
    {icSigNamedColorClass,
     ...
     icMaxEnumData,
     icMaxEnumData,
     {icSigProfileDescriptionTag, icSigNamedColor2Tag, icSigMediaWhitePointTag,
      icSigCopyrightTag, icMaxEnumType}},
    ...
    {icSigNamedColorClass,
     ...
     icMaxEnumData,
     icMaxEnumData,
     {icSigProfileDescriptionTag, icSigNamedColorTag, /* Not strictly V3.4 */
      icSigMediaWhitePointTag, icSigCopyrightTag, icMaxEnumType}},
...
check_icc_legal(icc* p)
{
	...
	for(i = 0; tagchecktable[i].sig != icMaxEnumType; i++) {
		if(tagchecktable[i].sig == sig &&
		   (tagchecktable[i].chans == dchans /* Exactly matches */
		    ||
		    tagchecktable[i].chans ==
		        -dchans /* Exactly matches, but can try next table */
		    ||
		    tagchecktable[i].chans <
		        -99) /* Doesn't have to match or try next table */
		   &&
		   ...
		   (tagchecktable[i].pcssig == pcssig ||
			...
			/* Found entry, so now check that all the required tags
			 * are present. */
			for(j = 0; tagchecktable[i].tags[j] != icMaxEnumType;
			    j++) {
				if(p->find_tag(p, tagchecktable[i].tags[j]) !=
				   0) { /* Not present! */
					if(tagchecktable[i].chans == -200 ||
					   tagchecktable[i].chans ==
					       -dchans) { /* But can try next
					                     table */
						break;
					}
					sprintf(
					    p->err, "icc_check_legal: "
					            "deviceClass %s is missing "
					            "required tag %s",
					    tag2str(sig),
					    tag2str(tagchecktable[i].tags[j]));
					return p->errc = 1;
				}
			}
	...
	/* According to the spec. if the deviceClass is:
	        ...
	        an Link Class: both in and out color spaces can be any, and
	   should
	            be the input space of the first profile in the link, and the
	            input space of the last profile in the link respectively.
	        a Named Class: in and out color spaces are not defined in the
	   spec.
...
static int
icc_read(icc* p, icmFile* fp, /* File to read from */
         ...
         )
{
	...
	if(p->fp->seek(p->fp, of + 128) != 0 ||
	   p->fp->read(p->fp, tcbuf, 1, 4) != 4) {
		sprintf(p->err,
		        "icc_read: fseek() or fread() failed on tag count");
		return p->errc = 1;
	}
	...
	if(p->count > 0) {
		char* bp, *buf;
		if((p->data = (icmTag*)p->al->malloc(
		        p->al, p->count * sizeof(icmTag))) == NULL) {
		...
		if((buf = (char*)p->al->malloc(p->al, len)) == NULL) {
			sprintf(
			    p->err,
			    "icc_read: Tag table read buffer malloc() failed");
		...
		if(p->fp->seek(p->fp, of + 128) != 0 ||
		   p->fp->read(p->fp, buf, 1, len) != len) {
			sprintf(
			    p->err,
			    "icc_read: fseek() or fread() failed on tag table");
			p->al->free(p->al, p->data);
			p->data = NULL;
			p->al->free(p->al, buf);
			return p->errc = 1;
		...
		for(i = 0; i < p->count; i++, bp += 12) {
			p->data[i].sig =
			    (icTagSignature)read_SInt32Number(bp + 0);
			...
			p->data[i].ttype =
			    read_SInt32Number(tcbuf); /* Tag type */
...
icc_get_size(icc* p)
{
	...
	for(i = 0; i < p->count; i++) {
		if(p->data[i].objp->touched ==
		   0) { /* Not alllowed for previously */
			size = DO_ALIGN(size);
			size += p->data[i].objp->get_size(p->data[i].objp);
			p->data[i].objp->touched =
			    1; /* Don't account for this again */
		}
	}
 ...
 */
static int
icc_write(icc* p, icmFile* fp, /* File to write to */
          ...
          )
{
	...
	if((rv = write_UInt32Number(p->count, bp)) != 0) { /* Tag count */
		sprintf(p->err,
		        "icc_write: write_UInt32Number() failed on tag count");
		p->al->free(p->al, buf);
		return p->errc = rv;
	}
	bp += 4;
	...
	for(i = 0; i < p->count; i++) {
		...
		if((rv = write_SInt32Number((int)p->data[i].sig, bp + 0)) !=
		   0) {
			sprintf(p->err, "icc_write: write_SInt32Number() "
			                "failed on tag signature");
			p->al->free(p->al, buf);
			return p->errc = rv;
		}
		if((rv = write_UInt32Number(p->data[i].offset, bp + 4)) != 0) {
			sprintf(p->err, "icc_write: write_UInt32Number() "
			                "failed on tag offset");
			p->al->free(p->al, buf);
			return p->errc = rv;
		}
	...
	for(i = 0; i < p->count; i++) { /* For all the tag element data */
		if(p->data[i].objp->touched == 0)
			continue; /* Must be linked, and we've already written
			             it */
		if((rv = p->data[i].objp->write(p->data[i].objp,
		                                of + p->data[i].offset)) != 0) {
			return rv;
		}
...
static icmBase*
icc_add_tag(icc* p, icTagSignature sig, /* Tag signature - may be unknown */
            ...
            )
{
	...
	for(i = 0; sigtypetable[i].sig != icMaxEnumType; i++) {
		if(sigtypetable[i].sig == sig) { /* recognized signature */
			ok = 0;
			for(j = 0; sigtypetable[i].ttypes[j] != icMaxEnumType;
			    j++) {
				if(sigtypetable[i].ttypes[j] ==
				   ttype) /* recognized type */
					ok = 1;
			}
			break;
		}
	...
	if(p->data == NULL)
		...
		tp = p->al->realloc(p->al, (void*)p->data,
		                    (p->count + 1) * sizeof(icmTag));
	if(tp == NULL) {
	...
	p->data[p->count].ttype = nob->ttype =
	    ttype;                    /* The tag type signature */
...
/* NOTE: that we prevent tag duplication */
static icmBase*
icc_link_tag(icc* p, icTagSignature sig, /* Tag signature - may be unknown */
             ...
             )
{
	...
	if(exi == p->count) {
		sprintf(p->err, "icc_link_tag: Can't find existing tag '%s'",
		        tag2str(ex_sig));
		p->errc = 1;
		return NULL;
	}
	...
	if(p->data[exi].objp == NULL) {
		sprintf(p->err, "icc_link_tag: Existing tag '%s' isn't loaded",
		        tag2str(ex_sig));
		p->errc = 1;
		return NULL;
	}
	...
	for(i = 0; sigtypetable[i].sig != icMaxEnumType; i++) {
		if(sigtypetable[i].sig == sig) { /* recognized signature */
			...
			for(j = 0; sigtypetable[i].ttypes[j] != icMaxEnumType;
			    j++) {
				if(sigtypetable[i].ttypes[j] ==
				   p->data[exi].ttype) /* recognized type */
					ok = 1;
			}
			break;
		}
	...
	/* Fill out our tag table entry */
	p->data[p->count].sig = sig; /* The tag signature */
	p->data[p->count].ttype =
	    p->data[exi].ttype; /* The tag type signature */
	p->data[p->count].offset =
	    p->data[exi].offset; /* Same offset (may not be allocated yet) */
	p->data[p->count].size =
	    p->data[exi].size; /* Same size (may not be allocated yet) */
...
/* NOTE: we don't handle tag duplication - you'll always get the first in the
 * file. */
static int
icc_find_tag(icc* p, icTagSignature sig /* Tag signature - may be unknown */
             )
{
	int i, j;
 ...
 * file */
static icmBase*
icc_read_tag(icc* p, icTagSignature sig /* Tag signature - may be unknown */
             )
{
	icmBase* nob;
	int i, j, k;
	...
	if(i >= p->count) {
		sprintf(p->err, "icc_read_tag: Tag '%s' not found",
		        string_TagSignature(sig));
		p->errc = 2;
		return NULL;
	}
	...
	for(k = 0; k < p->count; k++) {
		if(i == k)
			continue;
		if(p->data[i].ttype ==
		       p->data[k].ttype /* Exact match and already read */
		   &&
		   p->data[i].offset == p->data[k].offset &&
		   p->data[i].size == p->data[k].size &&
		   p->data[k].objp != NULL)
	...
	if(typetable[j].ttype == icMaxEnumType) {
		sprintf(p->err, "icc_read_tag: Unhandled tag type '%s'",
		        string_TypeSignature(p->data[i].ttype));
		p->errc = 1;
		return NULL;
	}
               ...
               )
{
	...
	if(k >= p->count) {
		sprintf(p->err, "icc_rename_tag: Tag '%s' not found",
		        string_TagSignature(sig));
		return p->errc = 2;
	}
	...
	for(i = 0; sigtypetable[i].sig != icMaxEnumType; i++) {
		if(sigtypetable[i].sig == sigNew) { /* recognized signature */
			...
			for(j = 0; sigtypetable[i].ttypes[j] != icMaxEnumType;
			    j++) {
				if(sigtypetable[i].ttypes[j] ==
				   p->data[k].ttype) /* recognized type */
					ok = 1;
			}
			break;
		}
...
static int
icc_unread_tag(icc* p, icTagSignature sig /* Tag signature - may be unknown */
               )
{
	int i;
	...
	if(i >= p->count) {
		sprintf(p->err, "icc_unread_tag: Tag '%s' not found",
		        string_TagSignature(sig));
		return p->errc = 2;
	}
	...
	if(p->data[i].objp == NULL) {
		sprintf(p->err, "icc_unread_tag: Tag '%s' not currently loaded",
		        string_TagSignature(sig));
		return p->errc = 2;
	}
...
/* tag not found - icc->errc will contain 2 */
static int
icc_delete_tag(icc* p, icTagSignature sig /* Tag signature - may be unknown */
               )
{
	int i;
	...
	if(i >= p->count) {
		sprintf(p->err, "icc_delete_tag: Tag '%s' not found",
		        string_TagSignature(sig));
		return p->errc = 2;
	}
	...
	if(p->data[i].objp != NULL) {
		if(--(p->data[i].objp->refcount) ==
		   0) /* decrement reference count */
			(p->data[i].objp->del)(
			    p->data[i].objp); /* Last reference */
		p->data[i].objp = NULL;
	}
...
static void
icc_dump(icc* p, FILE* op, /* Output to dump to */
         int verb          /* Verbosity level */
         )
{
	int i;
...
static void
Lut_Lut2XYZ(double* out, double* in)
{
...
static void
Lut_XYZ2Lut(double* out, double* in)
{
...
static void
Lut_Lut2Lab16(double* out, double* in)
{
...
static void
Lut_Lab2Lut16(double* out, double* in)
{
...
static void
Lut_Lut2Lab8(double* out, double* in)
{
	out[0] = in[0] * 100.0;           /* L */
...
static void
Lut_Lab2Lut8(double* out, double* in)
{
...
static void
Lut_Lut2Luv(double* out, double* in)
{
	out[0] = in[0] * 100.0;                     /* L */
...
static void
Lut_Luv2Lut(double* out, double* in)
{
	out[0] = in[0] * 1.0 / 100.0;               /* L */
...
static void
Lut_N(double* out, double* in, int nc)
{
	for(--nc; nc >= 0; nc--)
		out[nc] = in[nc];
}
...
static struct {
	...
	void (*fromLut8)(double* out,
	                 double* in); /* 8  bit from Lut index/entry */
	void (*fromLut16)(double* out,
	                  double* in); /* 16 bit from Lut index/entry */
	void (*toLut8)(double* out, double* in); /* 8  bit to Lut index/entry */
	void (*toLut16)(double* out,
	                double* in); /* 16 bit to Lut index/entry */
} colnormtable[] = {
...
static int
getNormFunc(icColorSpaceSignature csig, icTagTypeSignature tagSig,
            icmNormFlag flag, void (**nfunc)(double* out, double* in))
{
	int i;
	...
	if(flag == icmFromLuti ||
	   flag == icmFromLutv) { /* Table index/value decoding functions */
	...
	} else if(flag == icmToLuti ||
	          flag ==
	              icmToLutv) { /* Table index/value encoding functions */
		if(tagSig == icSigLut8Type) {
...
/* Return 0 on success, 1 on match failure */
static int
getRange(icColorSpaceSignature csig, double* min, double* max)
{
	int i, e, ee;
...
static void
adjoint(double out[3][3], double in[3][3])
{
...
static int
inverse3x3(double out[3][3], double in[3][3])
{
	int i, j;
	double det;
...
static void
icmMulBy3x3(double out[3], double mat[3][3], double in[3])
{
	double tt[3];
...
/* Default black point */
icmXYZNumber icmBlack = {0.0000, 0.0000, 0.0000};

/* Return the normal Delta E given two Lab values */
double
icmLabDE(double* Lab1, double* Lab2)
{
	double rv = 0.0, tt;
...
double
icmLabDEsq(double* Lab1, double* Lab2)
{
	double rv = 0.0, tt;
...
double
icmCIE94(double Lab1[3], double Lab2[3])
{
	double desq, dhsq;
	double dlsq, dcsq;
	double c12;
...
double
icmCIE94sq(double Lab1[3], double Lab2[3])
{
	double desq, dhsq;
	double dlsq, dcsq;
	double c12;
...
static void
mul3x3(double dst[3][3], double src[3][3])
{
	int i, j, k;
...
void
icmChromAdaptMatrix(int flags, icmXYZNumber d_wp, /* Destination white point */
...
static void
icmLutSpaces(struct _icmLuBase* p,       /* This */
...
/* Any pointer may be NULL if value is not to be returned */
static void
icmLuWh_bk_points(struct _icmLuBase* p, icmXYZNumber* wht, icmXYZNumber* blk)
{
	if(wht != NULL)
...
/* Get the effective (externally visible) ranges for the Monochrome or Matrix
 * profile */
/* Arguments may be NULL */
static void
icmLu_get_ranges(struct _icmLuBase* p, double* inmin,
                 double* inmax, /* Return maximum range of inspace values */
                 double* outmin,
                 double* outmax /* Return maximum range of outspace values */
                 )
{
...
static int
icmLuMonoFwd_curve(icmLuMono* p, /* This */
                   ...
                   )
{
	...
	if((rv |= p->grayCurve->lookup_fwd(p->grayCurve, &out[0], &in[0])) >
	   1) {
...
static int
icmLuMonoFwd_map(icmLuMono* p, /* This */
...
static int
icmLuMonoFwd_lookup(icmLuBase* pp, /* This */
                    double* out,   /* Vector of output values */
...
static int
icmLuMonoBwd_map(icmLuMono* p, /* This */
                 ...
                 )
{
	...
	pcsw[2] = p->pcswht.Z;
	if(p->pcs == icSigLabData)
		icmXYZ2Lab(&p->pcswht, pcsw,
		           pcsw); /* in Lab (should be 100.0!) */
...
static int
icmLuMonoBwd_curve(icmLuMono* p, /* This */
                   double* out,  /* Output value */
                   double* in    /* Input value */
                   )
{
...
static int
icmLuMonoBwd_lookup(icmLuBase* pp, /* This */
                    double* out,   /* Output value */
                    double* in     /* Vector of input values */
                    )
{
    ...
    )
{
	...
	if((p = (icmLuMono*)icp->al->calloc(icp->al, 1, sizeof(icmLuMono))) ==
	   NULL)
		return NULL;
	p->icp = icp;
	...
	if(number_ColorSpaceSignature(icp->header->colorSpace) != 1 ||
	   (icp->header->pcs != icSigXYZData &&
	    icp->header->pcs != icSigLabData)) {
		p->del((icmLuBase*)p);
		return NULL;
	}

	/* Find the appropriate tags */
	if((p->grayCurve = (icmCurve*)icp->read_tag(icp, icSigGrayTRCTag)) ==
	       NULL ||
...
{
	return new_icmLuMono(icp, inSpace, outSpace, pcs, e_inSpace, e_outSpace,
	                     e_pcs, whitePoint, blackPoint, intent, func, 0);
}
...
{
	return new_icmLuMono(icp, inSpace, outSpace, pcs, e_inSpace, e_outSpace,
	                     e_pcs, whitePoint, blackPoint, intent, func, 1);
}
...
/* Individual components of Fwd conversion: */
static int
icmLuMatrixFwd_curve(icmLuMatrix* p, /* This */
                     double* out,    /* Vector of output values */
                     double* in      /* Vector of input values */
                     )
{
	...
	if((rv |= p->redCurve->lookup_fwd(p->redCurve, &out[0], &in[0])) > 1 ||
	   (rv |= p->greenCurve->lookup_fwd(p->greenCurve, &out[1], &in[1])) >
	       1 ||
	   (rv |= p->blueCurve->lookup_fwd(p->blueCurve, &out[2], &in[2])) >
	       1) {
		sprintf(icp->err, "icc_lookup: Curve->lookup_fwd() failed");
		icp->errc = rv;
		return 2;
...
static int
icmLuMatrixFwd_matrix(icmLuMatrix* p, /* This */
                      double* out,    /* Vector of output values */
                      double* in      /* Vector of input values */
                      )
{
...
static int
icmLuMatrixFwd_lookup(icmLuBase* pp, /* This */
                      double* out,   /* Vector of output values */
                      double* in     /* Vector of input values */
                      )
{
...
static int
icmLuMatrixBwd_matrix(icmLuMatrix* p, /* This */
                      double* out,    /* Vector of output values */
                      double* in      /* Vector of input values */
                      )
{
	...
	/* Matrix */
	tt[0] =
	    p->bmx[0][0] * in[0] + p->bmx[0][1] * in[1] + p->bmx[0][2] * in[2];
	tt[1] =
	    p->bmx[1][0] * in[0] + p->bmx[1][1] * in[1] + p->bmx[1][2] * in[2];
	tt[2] =
	    p->bmx[2][0] * in[0] + p->bmx[2][1] * in[1] + p->bmx[2][2] * in[2];
...
static int
icmLuMatrixBwd_curve(icmLuMatrix* p, /* This */
                     double* out,    /* Vector of output values */
                     double* in      /* Vector of input values */
                     )
{
	...
	if((rv |= p->redCurve->lookup_bwd(p->redCurve, &out[0], &out[0])) > 1 ||
	   (rv |= p->greenCurve->lookup_bwd(p->greenCurve, &out[1], &out[1])) >
	       1 ||
	   (rv |= p->blueCurve->lookup_bwd(p->blueCurve, &out[2], &out[2])) >
	       1) {
		sprintf(icp->err, "icc_lookup: Curve->lookup_bwd() failed");
		icp->errc = rv;
		return 2;
...
static int
icmLuMatrixBwd_lookup(icmLuBase* pp, /* This */
                      double* out,   /* Vector of output values */
                      double* in     /* Vector of input values */
                      )
{
...
static void
icmLuMatrix_delete(icmLuBase* p)
{
	icc* icp = p->icp;
    ...
    )
{
	...
	if((p = (icmLuMatrix*)icp->al->calloc(icp->al, 1,
	                                      sizeof(icmLuMatrix))) == NULL)
		return NULL;
	p->icp = icp;
	...
	/* Find the appropriate tags */
	if((p->redCurve = (icmCurve*)icp->read_tag(icp, icSigRedTRCTag)) ==
	       NULL ||
	   p->redCurve->ttype != icSigCurveType ||
	   (p->greenCurve = (icmCurve*)icp->read_tag(icp, icSigGreenTRCTag)) ==
	       NULL ||
	   p->greenCurve->ttype != icSigCurveType ||
	   (p->blueCurve = (icmCurve*)icp->read_tag(icp, icSigBlueTRCTag)) ==
	       NULL ||
	   p->blueCurve->ttype != icSigCurveType ||
	   (p->redColrnt = (icmXYZArray*)icp->read_tag(
	        icp, icSigRedColorantTag)) == NULL ||
	   p->redColrnt->ttype != icSigXYZType || p->redColrnt->size < 1 ||
	   (p->greenColrnt = (icmXYZArray*)icp->read_tag(
	        icp, icSigGreenColorantTag)) == NULL ||
	   p->greenColrnt->ttype != icSigXYZType || p->greenColrnt->size < 1 ||
	   (p->blueColrnt = (icmXYZArray*)icp->read_tag(
	        icp, icSigBlueColorantTag)) == NULL ||
	...
	if(inverse3x3(p->bmx, p->mx) != 0) { /* Compute inverse */
		sprintf(icp->err,
		        "icc_new_iccLuMatrix: Matrix wasn't invertable");
		icp->errc = 2;
		p->del((icmLuBase*)p);
		return NULL;
	}
...
{
	return new_icmLuMatrix(icp, inSpace, outSpace, pcs, e_inSpace,
	                       e_outSpace, e_pcs, whitePoint, blackPoint,
	                       intent, func, 0);
}
...
static int
icmLuLut_in_abs(icmLuLut* p, double* out, double* in)
{
	icmLut* lut = p->lut;
	int rv = 0;
	...
	if(out != in) {
		int i;
		for(i = 0; i < lut->inputChan;
		    i++) /* Don't alter input values */
			out[i] = in[i];
	}

	/* If Bwd Lut, take care of Absolute color space and effective input
	 * space */
	if((p->function == icmBwd || p->function == icmGamut ||
	    p->function == icmPreview) &&
	   p->intent == icAbsoluteColorimetric) {
		...
		if(p->e_inSpace == icSigLabData && p->inSpace == icSigXYZData)
			icmLab2XYZ(&p->pcswht, out, out);
		else if(p->e_inSpace == icSigXYZData &&
...
static int
icmLuLut_matrix(icmLuLut* p, double* out, double* in)
{
	icmLut* lut = p->lut;
	int rv = 0;
...
static int
icmLuLut_input(icmLuLut* p, double* out, double* in)
{
	icmLut* lut = p->lut;
	int rv = 0;
...
static int
icmLuLut_clut(icmLuLut* p, double* out, double* in)
{
	icmLut* lut = p->lut;
...
static int
icmLuLut_output(icmLuLut* p, double* out, double* in)
{
	icmLut* lut = p->lut;
	int rv = 0;
	...
	rv |=
	    lut->lookup_output(lut, out, out); /* Lookup though output tables */
	p->out_denormf(out, out); /* De-normalize to output color space */
	return rv;
}

static int
icmLuLut_out_abs(icmLuLut* p, double* out, double* in)
{
	icmLut* lut = p->lut;
	int rv = 0;
	...
	if((p->function == icmFwd || p->function == icmPreview) &&
	   p->intent == icAbsoluteColorimetric) {
		...
		if(p->outSpace == icSigLabData && p->e_outSpace == icSigXYZData)
			icmLab2XYZ(&p->pcswht, out, out);
		else if(p->outSpace == icSigXYZData &&
...
/* Overall lookup */
static int
icmLuLut_lookup(icmLuBase* pp, /* This */
                double* out,   /* Vector of output values */
                double* in     /* Vector of input values */
                )
{
	...
	if(p->usematrix)
		rv |= lut->lookup_matrix(
		    lut, temp, temp); /* If XYZ, multiply by non-unity matrix */
	p->in_normf(temp, temp);      /* Normalize for input color space */
	rv |=
	    lut->lookup_input(lut, temp, temp); /* Lookup though input tables */
	rv |= p->lookup_clut(lut, out, temp);   /* Lookup though clut tables */
	rv |=
	    lut->lookup_output(lut, out, out); /* Lookup though output tables */
...
static int
icmLuLut_inv_out_abs(icmLuLut* p, double* out, double* in)
{
	icmLut* lut = p->lut;
	int rv = 0;
	...
	if((p->function == icmFwd || p->function == icmPreview) &&
	   p->intent == icAbsoluteColorimetric) {
		...
		if(p->e_outSpace == icSigLabData && p->outSpace == icSigXYZData)
			icmLab2XYZ(&p->pcswht, out, out);
		else if(p->e_outSpace == icSigXYZData &&
...
static int
icmLuLut_inv_output(icmLuLut* p, double* out, double* in)
{
	icc* icp = p->icp;
	icmLut* lut = p->lut;
	int rv = 0;
	...
	if(lut->rot.inited == 0) {
		rv = icmTable_setup_bwd(icp, &lut->rot, lut->outputEnt,
		                        lut->outputTable);
		if(rv != 0) {
			sprintf(icp->err, "icc_Lut_inv_input: Malloc failure "
			                  "in inverse lookup init.");
			return icp->errc = rv;
		}
	}

	p->out_normf(out, in); /* Normalize from output color space */
	rv |= icmTable_lookup_bwd(
	    &lut->rot, out, out); /* Reverse lookup though output tables */
	p->out_denormf(out, out); /* De-normalize to output color space */
	return rv;
}
...
static int
icmLuLut_inv_input(icmLuLut* p, double* out, double* in)
{
	icc* icp = p->icp;
	icmLut* lut = p->lut;
	int rv = 0;
	...
	if(lut->rit.inited == 0) {
		rv = icmTable_setup_bwd(icp, &lut->rit, lut->inputEnt,
		                        lut->inputTable);
		if(rv != 0) {
			sprintf(icp->err, "icc_Lut_inv_input: Malloc failure "
			                  "in inverse lookup init.");
	...
	rv |= icmTable_lookup_bwd(&lut->rit, out,
	                          out); /* Reverse lookup though input tables */
	p->in_denormf(out, out);        /* De-normalize to input color space */
	return rv;
}
...
static int
icmLuLut_inv_matrix(icmLuLut* p, double* out, double* in)
{
	icc* icp = p->icp;
	icmLut* lut = p->lut;
	int rv = 0;
	...
	if(p->usematrix) {
		double tt[3];
		if(p->imx_valid == 0) {
			if(inverse3x3(p->imx, lut->e) !=
			   0) { /* Compute inverse */
				sprintf(icp->err, "icc_new_iccLuMatrix: Matrix "
				                  "wasn't invertable");
				icp->errc = 2;
				return 2;
			}
		...
		/* Matrix multiply */
		tt[0] = p->imx[0][0] * in[0] + p->imx[0][1] * in[1] +
		        p->imx[0][2] * in[2];
		tt[1] = p->imx[1][0] * in[0] + p->imx[1][1] * in[1] +
		        p->imx[1][2] * in[2];
		tt[2] = p->imx[2][0] * in[0] + p->imx[2][1] * in[1] +
		        p->imx[2][2] * in[2];
...
static int
icmLuLut_inv_in_abs(icmLuLut* p, double* out, double* in)
{
	icmLut* lut = p->lut;
	int rv = 0;
	...
	if((p->function == icmBwd || p->function == icmGamut ||
	   ...
	   p->intent == icAbsoluteColorimetric) {
		...
		if(p->inSpace == icSigLabData && p->e_inSpace == icSigXYZData)
			icmLab2XYZ(&p->pcswht, out, out);
		else if(p->inSpace == icSigXYZData &&
...
icmLuLut_get_lutranges(
    struct _icmLuLut* p, double* inmin,
    double* inmax,                 /* Return maximum range of inspace values */
...
/* Arguments may be NULL */
static void
icmLuLut_get_ranges(struct _icmLuBase* pp, double* inmin,
                    double* inmax, /* Return maximum range of inspace values */
                    double* outmin,
                    double* outmax /* Return maximum range of outspace values */
                    )
{
	icmLuLut* p = (icmLuLut*)pp;
	double tinmin[MAX_CHAN], tinmax[MAX_CHAN], toutmin[MAX_CHAN],
	    toutmax[MAX_CHAN];
	int i;
...
static void
icmLuLut_get_matrix(struct _icmLuLut* p, double m[3][3])
{
	int i, j;
	icmLut* lut = p->lut;
...
static icmLuBase*
new_icmLuLut(
    icc* icp, icTagSignature ttag,    /* Target Lut tag */
    icColorSpaceSignature inSpace,    /* Native Input color space */
    icColorSpaceSignature outSpace,   /* Native Output color space */
    icColorSpaceSignature pcs,        /* Native PCS */
    icColorSpaceSignature e_inSpace,  /* Effective Input color space */
    ...
    )
{
	...
	if((p = (icmLuLut*)icp->al->calloc(icp->al, 1, sizeof(icmLuLut))) ==
	   NULL)
		return NULL;
	p->ttype = icmLutType;
	p->icp = icp;
	...
	if((p->lut = (icmLut*)icp->read_tag(icp, ttag)) == NULL ||
	   (p->lut->ttype != icSigLut8Type &&
	    p->lut->ttype != icSigLut16Type)) {
		p->del((icmLuBase*)p);
		return NULL;
	}
	...
	if(getNormFunc(e_inSpace, p->lut->ttype, icmFromLuti,
	               &p->e_in_denormf)) {
		sprintf(icp->err,
		        "icc_get_luobj: Unknown effective colorspace");
		icp->errc = 1;
		p->del((icmLuBase*)p);
		return NULL;
	}

	/* Lookup normalized Lut entry value to effective output color space
	 * function */
	if(getNormFunc(e_outSpace, p->lut->ttype, icmFromLutv,
	               &p->e_out_denormf)) {
		sprintf(icp->err,
		        "icc_get_luobj: Unknown effective colorspace");
		icp->errc = 1;
	...
	/* Determine appropriate clut lookup algorithm */
	{
		...
		icColorSpaceSignature ins,
		    outs;      /* In and out Lut color spaces */
		...
		if(use_sx == -1) {
			...
			/* Determine where the luminence is carried in the
			 * output */
			switch(outs) {

			/* Luminence is carried by the sum of all the output
			 * channels */
			case icSigRgbData:
			case icSigGrayData:
			case icSigCmykData:
			...
			/* If we know how luminance is represented in output
			 * space */
			if(lc != -2) {
				...
				/* Determine input space location of min and max
				 * of */
				/* given output channel (chan = -1 means average
				 * of all) */
				p->lut->min_max(p->lut, tout1, tout2, lc);

				/* Convert to vector and then calculate
				 * normalized */
				...
				if(tt > 0.0)
					tt = sqrt(
					    tt); /* normalizing factor for
					            maximum delta */
				else
					tt = 1.0; /* Hmm. */
				tt *= sqrt(
				    (double)inn); /* Normalizing factor for
				                     diagonal vector */
				for(diag = 0.0, n = 0; n < outn; n++)
					diag += tout1[n] / tt;
				diag = fabs(diag);

				/* I'm not really convinced that this is a
				 * reliable */
				/* indicator of whether simplex interpolation
				 * should be used ... */
				/* It does seem to do the right thing with YCC
				 * space though. */
				if(diag > 0.8) /* Diagonal is dominant ? */
					use_sx = 1;

				/* If we couldn't figure it out, use N-linear
				 * interpolation */
				if(use_sx == -1)
					use_sx = 0;
			}
...
static icmLuBase*
icc_get_luobj(icc* p,                      /* ICC */
              icmLookupFunc func,          /* Conversion functionality */
              icRenderingIntent intent,    /* Rendering intent, including
                                              icmAbsoluteColorimetricXYZ */
              ...
              )
{
	...
	/* Get White and Black points from the profile */
	{
		...
		if((whitePointTag = (icmXYZArray*)p->read_tag(
		        p, icSigMediaWhitePointTag)) == NULL ||
		   whitePointTag->ttype != icSigXYZType ||
		   whitePointTag->size < 1) {
			if(intent == icAbsoluteColorimetric) {
				sprintf(p->err, "icc_lookup: Profile is "
				                "missing Media White Point "
				                "Tag");
				p->errc = 1;
				return NULL;
			}
			...
			whitePoint =
			    whitePointTag->data[0]; /* Copy structure */

		if((blackPointTag = (icmXYZArray*)p->read_tag(
		        p, icSigMediaBlackPointTag)) == NULL ||
		   blackPointTag->ttype != icSigXYZType ||
		   blackPointTag->size < 1) {
			blackPoint = icmBlack; /* default */
		} else
			blackPoint =
			    blackPointTag->data[0]; /* Copy structure */
	}

	/* How we expect to execute the request depends firstly on the type of
	 * profile */
	...
	case icSigDisplayClass:
		...
		if(intent != icmDefaultIntent &&
		   intent != icAbsoluteColorimetric) {
			sprintf(p->err, "icc_get_luobj: Intent is "
			                "inappropriate for Input/Display "
			                "profile");
			p->errc = 1;
			return NULL;
		}
		...
		case icmFwd: /* Device to PCS */
			if(order != icmLuOrdRev) {
				...
				if((luobj = new_icmLuLut(
				        p, icSigAToB0Tag, p->header->colorSpace,
				        pcs, pcs, p->header->colorSpace, e_pcs,
				        e_pcs, whitePoint, blackPoint, intent,
				        func)) != NULL)
					break;

				/* See if it could be a matrix lookup */
				if((luobj = new_icmLuMatrixFwd(
				        p, p->header->colorSpace, pcs, pcs,
				        p->header->colorSpace, e_pcs, e_pcs,
				        whitePoint, blackPoint, intent,
				        func)) != NULL)
					break;

				/* See if it could be a monochrome lookup */
				if((luobj = new_icmLuMonoFwd(
				        p, p->header->colorSpace, pcs, pcs,
				        p->header->colorSpace, e_pcs, e_pcs,
				        whitePoint, blackPoint, intent,
				        func)) != NULL)
		...
		case icmBwd: /* PCS to Device */
			if(order != icmLuOrdRev) {
				...
				if((luobj = new_icmLuLut(
				        p, icSigBToA0Tag, pcs,
				        p->header->colorSpace, pcs, e_pcs,
				        p->header->colorSpace, e_pcs,
				        whitePoint, blackPoint, intent,
				        func)) != NULL)
					break;

				/* See if it could be a matrix lookup */
				if((luobj = new_icmLuMatrixBwd(
				        p, pcs, p->header->colorSpace, pcs,
		...
		default:
			sprintf(
			    p->err,
			    "icc_get_luobj: Inaproptiate function requested");
			p->errc = 1;
			return NULL;
		}
		break;
	...
	case icSigOutputClass:
		/* Expect BToA Lut and optional AToB Lut, All intents, expect
		 * gamut */
		/* or momochrome table profile (reversable) */
		/* Device <-> PCS */
		/* Gamut Lut - no intent */
		...
		case icmFwd: /* Device to PCS */
			...
			if(intent == icmDefaultIntent)
				intent =
				    icRelativeColorimetric; /* Make this the
				                               default */
			...
			if(order != icmLuOrdRev) {
				/* Try Lut type lookup first */
				if((luobj = new_icmLuLut(
				        p, ttag, p->header->colorSpace, pcs,
				        pcs, p->header->colorSpace, e_pcs,
				        e_pcs, whitePoint, blackPoint, intent,
				        func)) != NULL)
					break;
		...
		case icmBwd: /* PCS to Device */
			...
			if(order != icmLuOrdRev) {
				/* Try Lut type lookup first */
				if((luobj = new_icmLuLut(
				        p, ttag, pcs, p->header->colorSpace,
				        pcs, e_pcs, p->header->colorSpace,
				        e_pcs, whitePoint, blackPoint, intent,
				        func)) != NULL)
					break;
		...
		case icmGamut: /* PCS to 1D */
			...
			luobj =
			    new_icmLuLut(p, icSigGamutTag, pcs, icSigGrayData,
			                 pcs, e_pcs, icSigGrayData, e_pcs,
		...
		case icmPreview: /* PCS to PCS */
			...
			/* If the target tag exists, and it is a Lut */
			luobj = new_icmLuLut(p, ttag, pcs, pcs, pcs, e_pcs,
			                     e_pcs, e_pcs, whitePoint,
			                     blackPoint, intent, func);
			break;
	...
	case icSigLinkClass:
		...
		if(intent != p->header->renderingIntent &&
		   intent != icmDefaultIntent) {
			sprintf(p->err, "icc_get_luobj: Intent is "
			                "inappropriate for Link profile");
			p->errc = 1;
			return NULL;
		}
		...
		case icmFwd: /* Device to PCS (== Device) */
			...
			luobj = new_icmLuLut(
			    p, icSigAToB0Tag, p->header->colorSpace, pcs, pcs,
			    p->header->colorSpace, pcs, pcs, whitePoint,
			    blackPoint, intent, func);
			break;
		...
		case icmBwd: /* PCS (== Device) to Device */
			...
			luobj = new_icmLuLut(
			    p, icSigBToA0Tag, pcs, p->header->colorSpace, pcs,
			    pcs, p->header->colorSpace, pcs, whitePoint,
			    blackPoint, intent, func);
			break;
	...
	case icSigAbstractClass:
		...
		if(intent != icmDefaultIntent) {
			sprintf(p->err, "icc_get_luobj: Intent is "
			                "inappropriate for Abstract profile");
			p->errc = 1;
			return NULL;
		}
		...
		case icmFwd: /* PCS (== Device) to PCS */
			...
			luobj = new_icmLuLut(p, icSigAToB0Tag,
			                     ...
			                     e_pcs, e_pcs, e_pcs, whitePoint,
			                     blackPoint, intent, func);
			break;

		case icmBwd: /* PCS to PCS (== Device) */

			luobj = new_icmLuLut(p, icSigBToA0Tag, pcs,
			                     p->header->colorSpace, pcs, e_pcs,
			                     e_pcs, e_pcs, whitePoint,
			                     blackPoint, intent, func);
			break;
	...
	case icSigColorSpaceClass:
		...
		if(intent != icmDefaultIntent) {
			sprintf(p->err, "icc_get_luobj: Intent is "
			                "inappropriate for Colorspace profile");
			p->errc = 1;
			return NULL;
		}
		...
		case icmFwd: /* Device to PCS */
			...
			luobj = new_icmLuLut(
			    p, icSigAToB0Tag, p->header->colorSpace, pcs, pcs,
			    p->header->colorSpace, e_pcs, e_pcs, whitePoint,
			    blackPoint, intent, func);
			break;
		...
		case icmBwd: /* PCS to Device */
			...
			luobj = new_icmLuLut(
			    p, icSigBToA0Tag, pcs, p->header->colorSpace, pcs,
			    e_pcs, p->header->colorSpace, e_pcs, whitePoint,
			    blackPoint, intent, func);
			break;
...
/* Create an empty object. Return null on error */
icc*
new_icc_a(icmAlloc* al /* Optional memory allocator. NULL for default */
          )
{
	icc* p;
	int del_al = 0;
	...
	p->header->deviceClass =
	    icMaxEnumClass; /* Type of profile - must be set! */
	p->header->colorSpace =
	    icMaxEnumData;              /* Clr space of data - must be set! */
	p->header->pcs = icMaxEnumData; /* PCS: XYZ or Lab - must be set! */
	p->header->renderingIntent =
	    icMaxEnumIntent; /* Rendering intent - must be set ! */
	...
	p->header->creator =
	    str2tag("argl"); /* Profile creator - Argyll - may be set ! */
