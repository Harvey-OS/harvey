...
/* Forward references */
private
void gdev_x_setup_fontmap(gx_device_X*);
...
gdev_x_open(gx_device_X* xdev)
{
	...
	if(xdev->pwin != (Window)None) { /* pick up the destination window
		...
		char* buf;
		Atom ghostview_atom =
		    XInternAtom(xdev->dpy, "GHOSTVIEW", False);
		...
		if(XGetWindowProperty(xdev->dpy, xdev->win, ghostview_atom, 0,
		                      256, (xdev->dest != 0), XA_STRING, &type,
		   ...
		   type == XA_STRING) {
			...
			nitems =
			    sscanf(buf, "%ld %d %d %d %d %d %f %f %d %d %d %d",
			           &(xdev->bpixmap), &page_orientation, &llx,
			           &lly, &urx, &ury, &(xdev->x_pixels_per_inch),
			           &(xdev->y_pixels_per_inch), &left_margin,
			           &bottom_margin, &right_margin, &top_margin);
			if(!(nitems == 8 || nitems == 12)) {
				eprintf("Cannot get ghostview property.\n");
				return_error(gs_error_ioerror);
			}
			if(xdev->dest && xdev->bpixmap) {
				eprintf("Both destination and backing pixmap "
				        "specified.\n");
			...
			if(xdev->dest) {
				...
				if(XGetGeometry(xdev->dpy, xdev->dest, &root,
				                &x, &y, &width, &height,
				                &border_width, &depth)) {
					xdev->width = width;
					xdev->height = height;
			...
			/* The following sets the imageable area according to
			 * the */
			/* bounding box and margins sent by ghostview. */
			/* This code has been patched many times; its current
			 * state */
		...
		if(xvinfo.visual->class != TrueColor) {
			...
			if(XMatchVisualInfo(xdev->dpy, scrno, 24, TrueColor,
			                    &xvinfo) ||
			   XMatchVisualInfo(xdev->dpy, scrno, 32, TrueColor,
			                    &xvinfo) ||
			   XMatchVisualInfo(xdev->dpy, scrno, 16, TrueColor,
			                    &xvinfo) ||
			   XMatchVisualInfo(xdev->dpy, scrno, 15, TrueColor,
			                    &xvinfo)) {
				xdev->cmap = XCreateColormap(
				    xdev->dpy, DefaultRootWindow(xdev->dpy),
	...
	dpy = XtOpenDisplay(app_con, NULL, "ghostscript", "Ghostscript", NULL,
	                    0, &zero, NULL);
	toplevel = XtAppCreateShell(NULL, "Ghostscript",
	                            applicationShellWidgetClass, dpy, NULL, 0);
	XtGetApplicationResources(toplevel, (XtPointer)xdev, gdev_x_resources,
	                          gdev_x_resource_count, NULL, 0);

	/* Reserve foreground and background colors under the regular
	 * connection. */
	xc.pixel = xdev->foreground;
	XQueryColor(xdev->dpy,
	            DefaultColormap(xdev->dpy, DefaultScreen(xdev->dpy)), &xc);
	...
	if(!xdev->ghostview) {
		...
		if(xdev->x_pixels_per_inch == FAKE_RES ||
		   xdev->y_pixels_per_inch == FAKE_RES) {
			float xsize =
			    (float)xdev->width / xdev->x_pixels_per_inch;
			float ysize =
			    (float)xdev->height / xdev->y_pixels_per_inch;

			if(xdev->xResolution == 0.0 &&
			   xdev->yResolution == 0.0) {
				float dpi, xdpi, ydpi;
				...
				dpi = min(xdpi, ydpi);
				/*
				 * Some X servers provide very large "virtual
				 * screens", and
				 * return the virtual screen size for
				 * Width/HeightMM but the
				 * physical size for Width/Height.  Attempt to
				 * detect and
				 * correct for this now.  This is a KLUDGE
				 * required because
				 * the X server provides no way to read the
				 * screen
				 * resolution directly.
				 */
				if(dpi < 30)
					...
					while(
					    xsize * dpi >
					        WidthOfScreen(xdev->scr) - 32 ||
					    ysize * dpi >
					        HeightOfScreen(xdev->scr) - 32)
						dpi *= 0.95;
				}
		...
		if(xdev->geometry != NULL) {
			/*
			 * Note that border_width must be set first.  We can't
			 * use
			...
			bitmask = XWMGeometry(
			    xdev->dpy, DefaultScreen(xdev->dpy), xdev->geometry,
			    gstr, xdev->borderWidth, &sizehints, &sizehints.x,
		...
		if(xdev->pwin != (Window)None && xid_width != 0 &&
		   xid_height != 0) {
#if 0 /*************** */
		...
		(float)xid_height / (float)xdev->height;
			...
			xdev->win = XCreateWindow(
			    xdev->dpy, RootWindowOfScreen(xdev->scr),
			    sizehints.x, sizehints.y, /* upper left */
			    xdev->width, xdev->height, xdev->borderWidth,
			    xdev->vinfo->depth, InputOutput, /* class        */
			...
			XSetWMHints(xdev->dpy, xdev->win,
			            &wm_hints); /* avoid input focus */
		}
	}
	/***
	 *** According to Ricard Torres (torres@upf.es), we have to wait until
	 *here
	...
	XSetLineAttributes(xdev->dpy, xdev->gc, 0, LineSolid, CapButt,
	                   JoinMiter);
	...
	if(!xdev->ghostview) { /* Make the window appear. */
		...
		if(xdev->pwin ==
		   (Window)None) { /* there isn't a next event for existing
		                      windows */
		...
		/* Create an unmapped window, that the window manager will
		 * ignore.
		...
		xdev->mwin = XCreateWindow(
		    xdev->dpy, RootWindowOfScreen(xdev->scr), 0, 0, 1, 1, 0,
	...
	xdev->ht.no_pixmap =
	    XCreatePixmap(xdev->dpy, xdev->win, 1, 1, xdev->vinfo->depth);
...
setup:
	if(buffered) {
		...
		if(mdev == 0 ||
		   mdev->color_info.depth != xdev->color_info.depth) {
			...
			if(mdev) {
				 ...
				 */
				gx_device_set_target((gx_device_forward*)mdev,
				                     NULL);
			} else {
			...
			/*
			 * We want to forward image drawing to the memory
			 * device.
			 * That requires making the memory device forward its
			 * color
			 * mapping operations back to the X device, creating a
			 * circular
			 * pointer structure.  This is not a disaster, we just
			 * need to
			 * be aware that this is going on.
			 */
			gs_make_mem_device(mdev, mdproto, mem, 0,
			                   (gx_device*)xdev);
			gx_device_set_target((gx_device_forward*)xdev,
			                     (gx_device*)mdev);
		...
		if(mdev->width != xdev->width || mdev->height != xdev->height) {
			...
			space =
			    gdev_mem_data_size(mdev, xdev->width, xdev->height);
			if(space > xdev->MaxBitmap) {
				buffered = false;
				goto setup;
			}
			buffer = (xdev->buffer
			              ? (byte*)gs_resize_object(
			                    mem, xdev->buffer, space, "buffer")
...
gdev_x_clear_window(gx_device_X* xdev)
{
		...
		if(xdev->useBackingPixmap) {
			if(xdev->bpixmap == 0) {
				x_error_handler.oldhandler =
				    XSetErrorHandler(x_catch_alloc);
				x_error_handler.alloc_error = False;
				xdev->bpixmap = XCreatePixmap(
				    xdev->dpy, xdev->win, xdev->width,
				    xdev->height, xdev->vinfo->depth);
				XSync(xdev->dpy, False); /* Force the error */
				if(x_error_handler.alloc_error) {
					xdev->useBackingPixmap = False;
#ifdef DEBUG
					eprintf("Warning: Failed to allocated "
					        "backing pixmap.\n");
#endif
					if(xdev->bpixmap) {
						XFreePixmap(xdev->dpy,
						            xdev->bpixmap);
						xdev->bpixmap = None;
						XSync(
						    xdev->dpy,
						    False); /* Force the error
						               */
					}
				}
				x_error_handler.oldhandler = XSetErrorHandler(
				    x_error_handler.oldhandler);
			}
		} else {
			if(xdev->bpixmap != 0) {
	...
	if(xdev->dest == (Pixmap)0) {
		xdev->dest = (xdev->bpixmap != (Pixmap)0 ? xdev->bpixmap
		                                         : (Pixmap)xdev->win);
	}
	if(xdev->dest != (Pixmap)0) {
		XSetForeground(xdev->dpy, xdev->gc, xdev->background);
		XFillRectangle(xdev->dpy, xdev->dest, xdev->gc, 0, 0,
	...
	if(xdev->bpixmap != (Pixmap)0) {
		if(!xdev->ghostview)
			XSetWindowBackgroundPixmap(xdev->dpy, xdev->win,
			                           xdev->bpixmap);
		XSetForeground(xdev->dpy, xdev->gc, xdev->background);
		XFillRectangle(xdev->dpy, xdev->bpixmap, xdev->gc, 0, 0,
		               xdev->width, xdev->height);
	}
...
scan_font_resource(const char* resource, x11fontmap** pmaps, gs_memory_t* mem)
{
	...
	while((ps_name = get_ps_name(&cp, &ps_name_len)) != 0) {
		...
		if(x11_name) {
			...
			if(font == NULL)
				continue;
			font->ps_name = (char*)gs_alloc_byte_array(
			    mem, ps_name_len + 1, sizeof(char),
			    "scan_font_resource(ps_name)");
			if(font->ps_name == NULL) {
				gs_free_object(mem, font,
				               "scan_font_resource(font)");
				continue;
			}
			strncpy(font->ps_name, ps_name, ps_name_len);
			font->ps_name[ps_name_len] = '\0';
			font->x11_name = (char*)gs_alloc_byte_array(
			    mem, x11_name_len, sizeof(char),
...
gdev_x_get_params(gx_device* dev, gs_param_list* plist)
{
	...
	if(code < 0 || (code = param_write_long(plist, "WindowID", &id)) < 0 ||
	   (code = param_write_bool(plist, ".IsPageDevice",
	                            &xdev->IsPageDevice)) < 0 ||
	   (code = param_write_long(plist, "MaxBitmap", &xdev->MaxBitmap)) <
	       0 ||
	   (code = param_write_int(plist, "MaxTempPixmap",
	                           &xdev->MaxTempPixmap)) < 0 ||
	   (code = param_write_int(plist, "MaxTempImage",
	                           &xdev->MaxTempImage)) < 0 ||
	   (code = param_write_int(plist, "MaxBufferedTotal",
	                           &xdev->MaxBufferedTotal)) < 0 ||
	   (code = param_write_int(plist, "MaxBufferedArea",
	                           &xdev->MaxBufferedArea)) < 0 ||
	   (code = param_write_int(plist, "MaxBufferedCount",
	                           &xdev->MaxBufferedCount)) < 0)
		DO_NOTHING;
	return code;
}
...
gdev_x_put_params(gx_device* dev, gs_param_list* plist)
{
	...
	ecode =
	    param_put_bool(plist, ".IsPageDevice", &values.IsPageDevice, ecode);
	ecode = param_put_long(plist, "MaxBitmap", &values.MaxBitmap, ecode);
	ecode =
	    param_put_int(plist, "MaxTempPixmap", &values.MaxTempPixmap, ecode);
	ecode =
	    param_put_int(plist, "MaxTempImage", &values.MaxTempImage, ecode);
	ecode = param_put_int(plist, "MaxBufferedTotal",
	                      &values.MaxBufferedTotal, ecode);
	ecode = param_put_int(plist, "MaxBufferedArea", &values.MaxBufferedArea,
	                      ecode);
	ecode = param_put_int(plist, "MaxBufferedCount",
	                      &values.MaxBufferedCount, ecode);
	...
	if(xdev->is_open && !xdev->ghostview &&
	   (dev->width != values.width || dev->height != values.height ||
		...
		if(dw || dh) {
			XResizeWindow(xdev->dpy, xdev->win, dev->width,
			              dev->height);
			if(xdev->bpixmap != (Pixmap)0) {
				XFreePixmap(xdev->dpy, xdev->bpixmap);
				xdev->bpixmap = (Pixmap)0;
...
free_x_fontmaps(x11fontmap** pmaps, gs_memory_t* mem)
{
	while(*pmaps) {
		...
		*pmaps = font->next;
		...
		gs_free_object(mem, font->x11_name,
		               "free_x_fontmaps(x11_name)");
