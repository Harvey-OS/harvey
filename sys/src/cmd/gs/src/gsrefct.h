...
/* Copyright (C) 1993, 1994, 1996, 1997, 1998, 1999 Aladdin Enterprises.  All
  rights reserved.
...
struct rc_header_s {
	long ref_count;
	gs_memory_t* memory;
#define rc_free_proc(proc) void proc(gs_memory_t*, void*, client_name_t)
	rc_free_proc((*free));
};
...
#define rc_adjust_(vp, delta, cname, body)                                     \
	...
	END
#define rc_adjust(vp, delta, cname) rc_adjust_(vp, delta, cname, (vp) = 0)
#define rc_adjust_only(vp, delta, cname)                                       \
	rc_adjust_(vp, delta, cname, DO_NOTHING)
#define rc_adjust_const(vp, delta, cname) rc_adjust_only(vp, delta, cname)
#define rc_decrement(vp, cname) rc_adjust(vp, -1, cname)
#define rc_decrement_only(vp, cname) rc_adjust_only(vp, -1, cname)
