...
#define ref_assign_inline(pto, pfrom)                                          \
	((pto)->value = (pfrom)->value, (pto)->tas = (pfrom)->tas)
#ifdef __TURBOC__
/*
 ...
 */
#define ref_assign(pto, pfrom) discard(ref_assign_inline(pto, pfrom))
#else
/*
 ...
 */
#define ref_saving_in(mmem) ((mmem)->new_mask != 0)
...
#define ref_save_in(mem, pcont, pto, cname)                                    \
	discard((ref_must_save_in(mem, pto)                                    \
	             ? ref_do_save_in(mem, pcont, pto, cname)                  \
	             : 0))
#define ref_save(pcont, pto, cname)                                            \
	discard((ref_must_save(pto) ? ref_do_save(pcont, pto, cname) : 0))
#define ref_mark_new_in(mmem, pto) ((pto)->tas.type_attrs |= (mmem)->new_mask)
...
#define ref_assign_old_in(mem, pcont, pto, pfrom, cname)                       \
	(ref_save_in(mem, pcont, pto, cname),                                  \
	 ref_assign_new_in(mem, pto, pfrom))
...
#ifdef DEBUG
#define and_fill_s(pref) , (gs_debug['$'] ? r_set_size(pref, 0xfeed) : 0)
/*
...
#define and_fill_sv(pref)                                                      \
	, (gs_debug['$']                                                       \
	       ? (r_set_size(pref, 0xfeed), (pref)->value.intval = DEADBEEF)   \
	       : 0)
#else                     /* !DEBUG */
...
#define make_t(pref, newtype) make_ta(pref, newtype, 0)
#define make_t_new_in(mem, pref, newtype)                                      \
	make_ta(pref, newtype, imemory_new_mask(mem))
#define make_t_new(pref, newtype) make_ta(pref, newtype, ialloc_new_mask)
#define make_t_old_in(mem, pcont, pref, newtype, cname)                        \
	(ref_save_in(mem, pcont, pref, cname),                                 \
	 make_t_new_in(mem, pref, newtype))
...
#define make_tav(pref, newtype, newattrs, valfield, newvalue)                  \
	((pref)->value.valfield = (newvalue),                                  \
	 r_set_type_attrs(pref, newtype, newattrs) and_fill_s(pref))
...
#define make_tasv(pref, newtype, newattrs, newsize, valfield, newvalue)        \
	((pref)->value.valfield = (newvalue),                                  \
	 r_set_type_attrs(pref, newtype, newattrs), r_set_size(pref, newsize))
...
#define make_null_old(pcont, pref, cname) make_t_old(pcont, pref, t_null, cname)
...
#define make_empty_array(pref, attrs) make_array(pref, attrs, 0, (ref*)NULL)
...
#define make_empty_string(pref, attrs) make_string(pref, attrs, 0, (byte*)NULL)
