...
gs_pixel_image_t_init(gs_pixel_image_t* pim, const gs_color_space* color_space)
{
	...
	if(color_space == 0 ||
	   (num_components = gs_color_space_num_components(color_space)) < 0)
		num_components = 0;
...
gx_image_enum_common_init(gx_image_enum_common_t* piec,
                          ...
                          const gx_image_enum_procs_t* piep, gx_device* dev,
...
int
gx_no_plane_data(gx_image_enum_common_t* info, const gx_image_plane_t* planes,
                 int height, int* height_used)
{
...
int
gx_image_data(gx_image_enum_common_t* info, const byte** plane_data, int data_x,
                    ...
                    const gx_image_plane_t* planes, int height)
{
	...
	return gx_image_plane_data_rows(info, planes, height,
	                                &ignore_rows_used);
}
...
int
gx_image_no_sget(gs_image_common_t* pic, stream* s, const gs_color_space* pcs)
{
	return_error(gs_error_rangecheck);
}
...
debug_b_print_matrix(const gs_pixel_image_t* pim)
{
	if_debug6('b', "      ImageMatrix=[%g %g %g %g %g %g]\n",
	          pim->ImageMatrix.xx, pim->ImageMatrix.xy, pim->ImageMatrix.yx,
	          pim->ImageMatrix.yy, pim->ImageMatrix.tx,
	          pim->ImageMatrix.ty);
}
private
void
	...
	if(control & PI_Decode) {
		...
		for(i = 0; i < num_decode; i += 2) {
			...
			if(dflags >= 0x100) {
				...
				sputs(s, (const byte*)decode,
				      di * sizeof(float), &ignore);
				dflags = 1;
				di = 0;
			}
			dflags <<= 2;
 ...
 */
int
gx_pixel_image_sget(gs_pixel_image_t* pim, stream* s, const gs_color_space* pcs)
{
	uint control;
	float decode_default_1 = 1;
	...
	if(control & PI_Decode) {
		...
		for(i = 0; i < num_decode; i += 2, dp += 2, dflags <<= 2) {
			...
			case 0x00:
				dp[0] = 0,
				dp[1] = DECODE_DEFAULT(i + 1, decode_default_1);
				break;
			case 0x40:
				dp[0] = DECODE_DEFAULT(i + 1, decode_default_1),
				dp[1] = 0;
				break;
			case 0x80:
				dp[0] = 0;
				if(sgets(s, (byte*)(dp + 1), sizeof(float),
				         &ignore) < 0)
					return_error(gs_error_ioerror);
				break;
			case 0xc0:
				if(sgets(s, (byte*)dp, sizeof(float) * 2,
				         &ignore) < 0)
					return_error(gs_error_ioerror);
				break;
			}
