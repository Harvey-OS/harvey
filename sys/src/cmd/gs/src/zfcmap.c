	...
	if(pcmap->lookup) {
		...
		for(i = 0; i < pcmap->num_lookup; ++i) {
			...
			if(pclr->value_type == CODE_VALUE_GLYPH)
				gs_free_string(mem, pclr->values.data,
				               pclr->values.size,
...
acquire_code_ranges(gs_cmap_adobe1_t* cmap, const ref* pref, gs_memory_t* mem)
{
	...
	ranges = (gx_code_space_range_t*)gs_alloc_byte_array(
	...
	for(i = 0; i < r_size(pref); i++) {
		...
		for(j = 0; j < elem_sz; j += 2) {
			...
			if(!r_has_type(&rfirst, t_string) ||
			   ...
			   (size = r_size(&rfirst)) == 0 ||
			   size > MAX_CMAP_CODE_SIZE ||
			   r_size(&rlast) != size ||
			   memcmp(rfirst.value.bytes, rlast.value.bytes, size) >
			       0)
				return_error(e_rangecheck);
	...
	for(i = 0; i < r_size(pref); i++) {
		...
		for(j = 0; j < elem_sz; j += 5) {
			...
			if(!r_has_type(&rprefix, t_string) ||
			   ...
			   !(r_has_type(&rvalues, t_string) ||
			     r_is_array(&rvalues)) ||
			   !r_has_type(&rfxs, t_integer))
				return_error(e_typecheck);
			if(r_size(&rmisc) != 4 ||
			   rmisc.value.bytes[0] >
			       MAX_CMAP_CODE_SIZE - r_size(&rprefix) ||
			...
			memcpy(pclr->key_prefix, rprefix.value.bytes,
			       pclr->key_prefix_size);
			pclr->key_is_range = rmisc.value.bytes[1];
			if(pclr->key_size == 0) {
				/* This is a single entry consisting only of the
				 * prefix. */
				if(r_size(&rkeys) != 0)
					return_error(e_rangecheck);
				pclr->num_entries = 1;
			} else {
				int step = pclr->key_size *
				           (pclr->key_is_range ? 2 : 1);
			...
			if(r_has_type(&rvalues, t_string)) {
				...
				uint values_size =
				    pclr->num_entries * pclr->value_size;
				int32_t k;
				byte* pvalue;
				...
				if(pclr->value_type != CODE_VALUE_GLYPH ||
				   ...
				   pclr->value_size > sizeof(gs_glyph))
					return_error(e_rangecheck);
				pclr->values.data =
				    gs_alloc_string(mem, values_size,
				...
				for(k = 0; k < pclr->num_entries; ++k) {
					...
					/*
					 * We need a special check here because
					 * some CPUs cannot
					 * shift by the full size of an int or
					 * long.
					 */
					if(pclr->value_size < sizeof(value) &&
					   (value >> (pclr->value_size * 8)) !=
					       0)
						return_error(e_rangecheck);
...
private
int
get_cid_system_info(const gs_memory_t* mem, gs_cid_system_info_t* pcidsi,
                    const ref* psia, uint index)
{
	ref rcidsi;
...
{
	return !bytes_compare(pcs1->data, pcs1->size, pcs2->data, pcs2->size);
}
private
bool
...
int
ztype0_get_cmap(const gs_cmap_t** ppcmap, const ref* pfdepvector, const ref* op,
                gs_memory_t* imem)
{
	ref* prcmap;
	ref* pcodemap;
		...
		if(code == 0) {
...
#ifdef CHECK_CID_SYSTEM_INFO_COMPATIBILITY
			{
				...
				if(!cid_system_info_is_null(&cidsi) &&
				   !cid_system_info_compatible(
				       &cidsi, pcmap->CIDSystemInfo + i))
					return_error(e_rangecheck);
			}
#endif
		}
...
private
int
zfcmap_glyph_name(const gs_memory_t* mem, gs_glyph glyph, gs_const_string* pstr,
                  void* proc_data)
{
	ref nref, nsref;
	int code = 0;
...
zbuildcmap(i_ctx_t* i_ctx_p)
{
	...
	if(dict_find_string(op, ".CodeMapData", &pcodemapdata) <= 0 ||
	   !r_has_type(pcodemapdata, t_array) || r_size(pcodemapdata) != 3 ||
	...
	if((code = gs_cmap_adobe1_alloc(&pcmap, 0, rname.value.const_bytes,
	                                r_size(&rname), r_size(&rcidsi), 0, 0,
	                                0, 0, 0, imemory)) < 0)
		goto fail;
	if((code = dict_int_param(op, "CMapType", 0, 1, 0, &pcmap->CMapType)) <
	       0 ||
	   (code = dict_float_param(op, "CMapVersion", 0.0,
	                            &pcmap->CMapVersion)) < 0 ||
	...
	for(i = 0; i < r_size(&rcidsi); ++i) {
		code = get_cid_system_info(imemory, pcmap->CIDSystemInfo + i,
		                           &rcidsi, i);
		if(code < 0)
			goto fail;
	}
	...
	if((code = acquire_code_map(&pcmap->def, &rdefs, pcmap, imemory)) < 0)
		goto fail;
	if((code = acquire_code_map(&pcmap->notdef, &rnotdefs, pcmap,
	                            imemory)) < 0)
		goto fail;
...
const op_def zfcmap_op_defs[] = {{"1.buildcmap", zbuildcmap}, op_def_end(0)};
