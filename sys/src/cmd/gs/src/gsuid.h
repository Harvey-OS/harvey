...
#define uid_is_valid(puid) ((puid)->id != no_UniqueID)
#define uid_set_invalid(puid) ((puid)->id = no_UniqueID, (puid)->xvalues = 0)
#define uid_is_UniqueID(puid) (((puid)->id & ~0xffffff) == 0)
#define uid_is_XUID(puid) ((puid)->id < 0)

/* Initialize a uid. */
#define uid_set_UniqueID(puid, idv) ((puid)->id = idv, (puid)->xvalues = 0)
...
#define uid_free(puid, mem, cname) gs_free_object(mem, (puid)->xvalues, cname)
