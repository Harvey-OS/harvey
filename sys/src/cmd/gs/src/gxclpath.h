...
typedef enum {
	cmd_op_misc2 = 0xd0,            /* (see below) */
	                                /* obsolete */
	/* cmd_opv_set_color = 0xd0, */ /* Used if base values do not fit into 1
                                           bit */
	/* #flags,#base[0],...#base[num_comp-1] if flags */
	/* colored halftone with base colors a,b,c,d */
	/* obsolete */
	/* cmd_opv_set_color_short = 0xd1, */ /* Used if base values fit into 1
                                                 bit */
...
#define cmd_segment_op_name_strings                                            \
	"rmoveto", "rlineto", "hlineto", "vlineto", "rmlineto", "rm2lineto",   \
	    "rm3lineto", "rrcurveto", "hvcurveto", "vhcurveto", "nrcurveto",   \
	    "rncurveto", "vqcurveto", "hqcurveto", "scurveto", "closepath"

#define cmd_path_op_name_strings                                               \
	"fill", "htfill", "colorfill", "eofill", "hteofill", "coloreofill",    \
	    "stroke", "htstroke", "colorstroke", "polyfill", "htpolyfill",     \
	    "colorpolyfill", "?fc?", "?fd?", "?fe?", "?ff?"
...
#define state_neq(member) (cdev->imager_state.member != pis->member)
#define state_update(member) (cdev->imager_state.member = pis->member)
...
int cmd_put_drawing_color(gx_device_clist_writer* cldev, gx_clist_state* pcls,
                          const gx_drawing_color* pdcolor);
...
int cmd_write_ctm_return_length(gx_device_clist_writer* cldev,
                                const gs_matrix* m);
/* Write out CTM. */
...
#define cmd_do_write_unknown(cldev, pcls, must_know)                           \
	(~(pcls)->known & (must_know)                                          \
	     ? cmd_write_unknown(cldev, pcls, must_know)                       \
	     : 0)
