...
private
xfont_proc_lookup_font(x_lookup_font);
private
xfont_proc_char_xglyph(x_char_xglyph);
private
xfont_proc_char_metrics(x_char_metrics);
private
xfont_proc_render_char(x_render_char);
private
xfont_proc_release(x_release);
private
const gx_xfont_procs x_xfont_procs = {x_lookup_font, x_char_xglyph,
                                      x_char_metrics, x_render_char, x_release};
...
gs_private_st_dev_ptrs1(st_x_xfont, x_xfont, "x_xfont", x_xfont_enum_ptrs,
                        x_xfont_reloc_ptrs, xdev);
            ...
            bool* scalable_font)
{
	...
	if(fls->count == -1) {
		sprintf(x11template, "%s-*-*-*-*-*-*-%s", fmp->x11_name,
		        encoding_name);
		fls->names =
		    XListFonts(xdev->dpy, x11template, 32, &fls->count);
	}
	*scalable_font = false;
...
private
gx_xfont*
x_lookup_font(gx_device* dev, const byte* fname, uint len, int encoding_index,
              const gs_uid* puid, const gs_matrix* pmat, gs_memory_t* mem)
{
	...
	case 1:
		...
		if(fmp == NULL)
			return NULL;
		x11fontname = find_x_font(xdev, x11template, fmp, "ISO8859-1",
		                          &fmp->iso, xheight, &scalable_font);
		if(!x11fontname) {
			x11fontname = find_x_font(
			    xdev, x11template, fmp, "Adobe-fontspecific",
			    &fmp->std, xheight, &scalable_font);
			encoding_index = 0;
		}
		break;
	...
	sym:
		fmp = find_fontmap(fmp, fname, len);
		if(fmp == NULL)
			return NULL;
		x11fontname =
		    find_x_font(xdev, x11template, fmp, "Adobe-fontspecific",
	...
	if(xwidth != xheight || angle != 0 || My) {
		if(!xdev->useScalableFonts || !scalable_font)
			return NULL;
		sprintf(
		    x11template, "%s%s+%d-%d+%d-0-0-0-*-0-%s", fmp->x11_name,
		    (My ? "+My" : ""), angle * 64, xheight, xwidth,
...
private
gx_xglyph
x_char_xglyph(gx_xfont* xf, gs_char chr, int encoding_index, gs_glyph glyph,
...
private
int
x_char_metrics(gx_xfont* xf, gx_xglyph xg, int wmode, gs_point* pwidth,
               gs_int_rect* pbbox)
{
	...
	switch(xxf->angle) {
	case 0:
		pwidth->x = width, pwidth->y = 0;
		break;
	case 90:
		pwidth->x = 0, pwidth->y = -xxf->My * width;
		break;
	case 180:
		pwidth->x = -width, pwidth->y = 0;
		break;
...
private
int
x_render_char(gx_xfont* xf, gx_xglyph xg, gx_device* dev, int xo, int yo,
              gx_color_index color, int required)
{
	...
	if(dev->dname == gs_x11_device.dname &&
	   !((gx_device_X*)dev)->is_buffered) {
		gx_device_X* xdev = (gx_device_X*)dev;

		code =
		    (*xf->common.procs->char_metrics)(xf, xg, 0, &wxy, &bbox);
		if(code < 0)
			return code;
		...
		/*
		 * The following is wrong for rotated text, but it doesn't
		 * matter,
		 * because the next call of x_render_char will have a different
		 * Y.
		 */
		{
			...
			char* pchar =
			    &xdev->text.chars[xdev->text.char_count++];
		...
		raster = wbm >> 3;
		bits =
		    (byte*)gs_malloc(xdev->memory, h, raster, "x_render_char");
		if(bits == 0)
