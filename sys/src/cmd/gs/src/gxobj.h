...
/* Copyright (C) 1995, 1996, 1999, 2002 Aladdin Enterprises.  All rights
  reserved.
...
#define o_set_unmarked(pp) ((pp)->o_smark = o_unmarked)
#define o_is_unmarked(pp) ((pp)->o_smark == o_unmarked)
#define o_untraced (((uint)1 << obj_mb_bits) - 2)
#define o_set_untraced(pp) ((pp)->o_smark = o_untraced)
#define o_is_untraced(pp) ((pp)->o_smark == o_untraced)
#define o_marked 0
#define o_mark(pp) ((pp)->o_smark = o_marked)
...
#define obj_align_mod                                                          \
	(((arch_align_memory_mod - 1) | (align_bitmap_mod - 1) |               \
	  (obj_back_scale - 1)) +                                              \
	 1)
...
#define obj_align_round(siz) (uint)(((siz) + obj_align_mask) & -obj_align_mod)
#define obj_size_round(siz) obj_align_round((siz) + sizeof(obj_header_t))
 ...
 */
#define pre_obj_contents_size(pp) ((pp)->o_size)

#define pre_obj_rounded_size(pp) obj_size_round(pre_obj_contents_size(pp))
#define pre_obj_next(pp)                                                       \
	((obj_header_t*)((byte*)(pp) +                                         \
	                 obj_align_round(pre_obj_contents_size(pp) +           \
	                                 sizeof(obj_header_t))))
