...
struct stats_image_fast_s {
	int32_t
	    calls,
	    all0s, all1s, runs, lbit0, byte00, byte01, byte02, byte03,
		    ...
		    fixed xcur, fixed x_extent, byte zero /* 0 or 0xff */)
{
	...
	/* Set up the DDAs. */
	xl0 =
	    (x_extent >= 0 ? fixed_fraction(fixed_pre_pixround(xcur)) : fixed_fraction(fixed_pre_pixround(xcur + x_extent)) - x_extent);
	      ...
	      int x, int y, int w, int h, gx_device *dev)
{
	...
	if(gx_dc_is_pure(pdc0) && gx_dc_is_pure(pdc1)) {
		...
		dev_proc_copy_mono((*copy_mono)) =
		    (h == 1 || (raster & (align_bitmap_mod - 1)) == 0 ? dev_proc(dev, copy_mono) : gx_copy_mono_unaligned);
		return (*copy_mono)(dev, data, dx, raster, gx_no_bitmap_id,
     ...
     */
	{
		...
		if(DC_IS_NULL(pdc1)) {
			...
			if(!DC_IS_NULL(pdc0)) {
				int code = gx_device_color_fill_rectangle(pdc0, x, y, w, h, dev, lop_default, NULL);
		...
		return (*pdc->type->fill_masked)(pdc, data, dx, raster, gx_no_bitmap_id, x, y, w, h,
	...
	if(penum->line == 0) { /* A direct BitBlt is possible. */
		...
		*/
		...
		if(dxx < 0)
			ix -= line_width;
		for(dy = 1; dy < ih; dy++) {
			int code = (*copy_mono)(dev, line, line_x, line_size, gx_no_bitmap_id,
		       ...
		       uint w, int h, gx_device *dev)
{
	...
	if(ix != penum->xi_next || h == 0) {
		...
		int code =
		    (xinc > 0 ? copy_landscape(penum, penum->line_xy, xi, y_neg, dev) : copy_landscape(penum, xi, penum->line_xy, y_neg, dev));
