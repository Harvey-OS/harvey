...
private
irender_proc(image_render_skip);
private
irender_proc(image_render_simple);
private
irender_proc(image_render_landscape);
irender_proc_t
gs_image_class_1_simple(gx_image_enum* penum)
{
	...
	case image_portrait: { /* Use fast portrait algorithm. */
		int32_t dev_width =
		...
		if(dev_width != penum->rect.w) {
			 ...
			 */
			int32_t line_size = bitmap_raster(any_abs(dev_width)) +
			                    align_bitmap_mod;
			...
			penum->line = gs_alloc_bytes(
			    penum->memory, penum->line_size, "image line");
			if(penum->line == 0) {
				gx_default_end_image(
				    penum->dev, (gx_image_enum_common_t*)penum,
				    false);
				return 0;
			}
		}
		if_debug2(
		    'b',
		    "[b]render=simple, unpack=copy; rect.w=%d, dev_width=%ld\n",
		    penum->rect.w, dev_width);
		rproc = image_render_simple;
		break;
	}
	case image_landscape: { /* Use fast landscape algorithm. */
		int32_t dev_width =
		...
		if(penum->line == 0) {
			gx_default_end_image(
			    penum->dev, (gx_image_enum_common_t*)penum, false);
			return 0;
		}
		penum->xi_next = penum->line_xy = fixed2int_var_rounded(ox);
		if_debug3('b', "[b]render=landscape, unpack=copy; rect.w=%d, "
		               "dev_width=%ld, line_size=%ld\n",
		          penum->rect.w, dev_width, line_size);
		rproc = image_render_landscape;
		/* Precompute values needed for rasterizing. */
		penum->dxy = float2fixed(penum->matrix.xy +
	...
	if(penum->use_mask_color) {
		...
		if(penum->mask_color.values[0] == 1) {
			/* if v0 == 1, 1 is transparent since v1 must be == 1 to
			 * be a valid range */
			set_nonclient_dev_color(penum->map[0].inverted
			                            ? &penum->icolor0
			                            : &penum->icolor1,
			                        gx_no_color_index);
		} else if(penum->mask_color.values[1] == 0) {
			/* if v1 == 0, 0 is transparent since v0 must be == 0 to
			 * be a valid range */
			set_nonclient_dev_color(penum->map[0].inverted
			                            ? &penum->icolor1
			                            : &penum->icolor0,
			...
			/*
			 * The only other possible in-range value is v0 = 0, v1
			 * = 1.
...
private
int
image_render_skip(gx_image_enum* penum, const byte* buffer, int data_x, uint w,
                  int h, gx_device* dev)
{
	return h;
}
...
struct stats_image_fast_s {
	int32_t calls, all0s, all1s, runs, lbit0, byte00, byte01, byte02,
	    byte03, byte04, rbit0, lbit1, byte1, rbit1, thin, thin2, nwide,
	    bwide, nfill, bfill;
} stats_image_fast;
...
private
void
image_simple_expand(byte* line, int line_x, uint raster, const byte* buffer,
                    int data_x, uint w, fixed xcur, fixed x_extent,
                    byte zero /* 0 or 0xff */)
{
	int dbitx = data_x & 7;
	...
	/* Now (stop, stopbit) give the last bit of the row. */
	{
		...
		if(last & stopbit) {
			...
			if(~last & stopmask) { /* Transition in last byte. */
				...
				if(stop == psrc ||
				   (stop == psrc + 1 && !(~*psrc & sbitmask))) {
					/* The input is all 1s.  Clear the row
					 * and exit. */
					INCS(all1s);
			...
			stopx =
			    byte_bit_run_length_0[byte_reverse_bits[last]] - 1;
		} else {
			...
			if(last & stopmask) { /* Transition in last byte. */
				...
				if(stop == psrc ||
				   (stop == psrc + 1 && !(*psrc & sbitmask))) {
					/* The input is all 0s.  Clear the row
					 * and exit. */
					INCS(all0s);
			...
			stopx = byte_bit_run_length_0[byte_reverse_bits[last ^
			                                                0xff]] -
			        1;
		}
		if(stopx < 0)
	...
	xl0 = (x_extent >= 0
	           ...
	           : fixed_fraction(fixed_pre_pixround(xcur + x_extent)) -
	                 x_extent);
	...
	for(data = *psrc;;) {
		int x0, n, bit;
		byte* bp;
		static const byte lmasks[9] = {0xff, 0x7f, 0x3f, 0x1f, 0xf,
		                               7,    3,    1,    0};
		static const byte rmasks[9] = {0,    0x80, 0xc0, 0xe0, 0xf0,
		                               0xf8, 0xfc, 0xfe, 0xff};
...
private
int
copy_portrait(gx_image_enum* penum, const byte* data, int dx, int raster, int x,
              int y, int w, int h, gx_device* dev)
{
	...
	if(gx_dc_is_pure(pdc0) && gx_dc_is_pure(pdc1)) {
		...
		dev_proc_copy_mono((*copy_mono)) =
		    (h == 1 || (raster & (align_bitmap_mod - 1)) == 0
		         ...
		         : gx_copy_mono_unaligned);
		return (*copy_mono)(dev, data, dx, raster, gx_no_bitmap_id, x,
		                    y, w, h, pdc0->colors.pure,
		                    pdc1->colors.pure);
	}
	/*
	 ...
	 */
	{
		...
		return (*pdc->type->fill_masked)(pdc, data, dx, raster,
                    ...
                    uint w, int h, gx_device* dev)
{
	...
	if(penum->line == 0) { /* A direct BitBlt is possible. */
		...
		image_simple_expand(
		    scan_line + (line_ix >> 3), line_x, line_size, buffer,
		    data_x, w, xcur, penum->x_extent.x,
		...
		for(dy = 1; dy < ih; dy++) {
			int code = (*copy_mono)(
			    dev, line, line_x, line_size, gx_no_bitmap_id, ix,
			    iy + dy, line_width, 1, (gx_color_index)0,
			    (gx_color_index)1);
		...
		line_x = ix & (align_bitmap_mod * 8 - 1);
		image_simple_expand(penum->line, line_x, line_size, buffer,
		                    data_x, w, xcur, penum->x_extent.x, 0);
	}
	...
	if(dxx < 0)
		ix -= line_width;
	for(dy = 0; dy < ih; dy++) {
		int code = copy_portrait(penum, line, line_x, line_size, ix,
		                         iy + dy, line_width, 1, dev);
...
private
int copy_landscape(gx_image_enum*, int, int, bool, gx_device*);
private
int
                       ...
                       uint w, int h, gx_device* dev)
{
	...
	if(ix != penum->xi_next || h == 0) {
		...
		int code = (xinc > 0 ? copy_landscape(penum, penum->line_xy, xi,
		                                      y_neg, dev)
		                     : copy_landscape(penum, xi, penum->line_xy,
		                                      y_neg, dev));
	...
	for(; iw != 0; iw -= xinc) {
		...
		if(orig_row == 0) {
			image_simple_expand(row, 0, raster, buffer, data_x, w,
		...
		if(xinc > 0) {
			++ix;
			if(xmod == 7) {
				int code = copy_landscape(penum, penum->line_xy,
				                          ix, y_neg, dev);
			...
			if(xmod == 0) {
				int code = copy_landscape(
				    penum, ix, penum->line_xy, y_neg, dev);
...
private
int
copy_landscape(gx_image_enum* penum, int x0, int x1, bool y_neg, gx_device* dev)
{
	byte* line = penum->line;
	...
	if(y_neg)
		y -= line_width;
	return copy_portrait(penum, flipped, x0 & 7, align_bitmap_mod, x0, y, w,
	                     line_width, dev);
}
