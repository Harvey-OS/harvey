...
private
const gs_log2_scale_point scale_log2_1 =
    {0, 0};
		  ...
		  const gs_log2_scale_point *log2_scale, bool design_grid, cached_fm_pair **ppair)
{
	...
	while(count--) {
		...
		if(pair->mxx == mxx && pair->mxy == mxy &&
		   pair->myx == myx && pair->myy == myy && pair->design_grid == design_grid) {
			if(pair->font == 0) {
				pair->font = pfont;
		...
		if(cc->code == glyph && cc_pair(cc) == pair &&
		   ...
		   cc->wmode == wmode && cc_depth(cc) == depth) {
			if_debug4('K', "[K]found 0x%lx (depth=%d) for glyph=0x%lx, wmode=%d\n",
				  (uint32_t)cc, cc_depth(cc), (uint32_t)glyph, wmode);
			return cc;
		}
		chi++;
	}
		     ...
		     gs_char chr, gs_glyph glyph, int wmode)
{
	...
	if(font == 0)
		return NULL;
	enc_index =
	    (font->FontType == ft_composite ? -1 : ((gs_font_base *)font)->nearest_encoding_index);
...
top:
	 ...
	 */
		if(gs_color_writes_pure(pgs)) {
			...
			if_debug8('K', "[K]render_char display: xfont=0x%lx, glyph=0x%lx\n\tdev=0x%lx(%s) x,y=%d,%d, color=0x%lx => %d\n",
				  (uint32_t)xf, (uint32_t)xg, (uint32_t)imaging_dev,
				  imaging_dev->dname, cx, cy,
				  (uint32_t)pdevc->colors.pure, code);
			if(code == 0)
	...
	if(dev_proc(orig_dev, fill_mask) != gx_default_fill_mask ||
	   !lop_no_S_is_T(pgs->log_op)) {
		...
		code = gx_effective_clip_path(pgs, &pcpath);
		if(code >= 0) {
			code = (*dev_proc(orig_dev, fill_mask))(orig_dev, bits, 0, cc_raster(cc), cc->id,
		...
		if(depth > 1) {
			code = (*dev_proc(imaging_dev, copy_alpha))(imaging_dev, bits, 0, cc_raster(cc), cc->id,
								    x, y, w, h, color, depth);
			if(code >= 0)
				return_check_interrupt(penum->memory, 0);
		...
		code = (*dev_proc(imaging_dev, copy_mono))(imaging_dev, bits, 0, bitmap_raster(w), gs_no_id,
