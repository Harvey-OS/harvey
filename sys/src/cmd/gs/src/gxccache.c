...
/* Copyright (C) 1989, 1995, 1996, 1997, 1998 Aladdin Enterprises.  All rights
  reserved.
...
/* Forward references */
private
byte* compress_alpha_bits(const cached_char*, gs_memory_t*);

/* Define a scale factor of 1. */
private
const gs_log2_scale_point scale_log2_1 = {0, 0};

void
gx_compute_char_matrix(const gs_matrix* char_tm,
                       const gs_log2_scale_point* log2_scale, float* mxx,
...
{
	if(design_grid && (pfont->FontType == ft_TrueType ||
	                   pfont->FontType == ft_CID_TrueType)) {
		/*
		 * We need a special face for this case, because the TT
		 * interpreter
                  ...
                  cached_fm_pair** ppair)
{
	...
	gx_compute_ccache_key(pfont, char_tm, log2_scale, design_grid, &mxx,
	                      &mxy, &myx, &myy);
	if(font->FontType == ft_composite ||
	   font->PaintType != 0) { /* We can't cache by UID alone. */
		uid_set_invalid(&uid);
	} else {
	...
	while(count--) {
		...
		if(pair->mxx == mxx && pair->mxy == mxy && pair->myx == myx &&
		   pair->myy == myy && pair->design_grid == design_grid) {
			if(pair->font == 0) {
				pair->font = pfont;
				if_debug2(
				    'k',
				    "[k]updating pair 0x%lx with font 0x%lx\n",
				    (uint32_t)pair, (uint32_t)pfont);
			} else {
				if_debug2('k',
				          "[k]found pair 0x%lx: font=0x%lx\n",
				          (uint32_t)pair, (uint32_t)pair->font);
			}
			*ppair = pair;
			return 0;
		}
	...
	while((cc = dir->ccache.table[chi & dir->ccache.table_mask]) != 0) {
		if(cc->code == glyph && cc_pair(cc) == pair &&
		   ...
		   cc->wmode == wmode && cc_depth(cc) == depth) {
			if_debug4('K', "[K]found 0x%lx (depth=%d) for "
			               "glyph=0x%lx, wmode=%d\n",
			          (uint32_t)cc, cc_depth(cc), (uint32_t)glyph,
			          wmode);
			return cc;
		}
		chi++;
...
/* Return the cached_char or 0. */
cached_char*
gx_lookup_xfont_char(const gs_state* pgs, cached_fm_pair* pair, gs_char chr,
                     gs_glyph glyph, int wmode)
{
	...
	if(font == 0)
		return NULL;
	enc_index = (font->FontType == ft_composite
	                 ? -1
		...
		return NULL;
	{
		...
		if(code < 0)
			return NULL;
		if(enc_index >= 0 &&
		   ((gs_font_base*)font)->encoding_index < 0) {
			/*
			...
			if(gs_c_glyph_name(gs_c_known_encode(chr, enc_index),
			                   &kstr) < 0 ||
			   kstr.size != gstr.size ||
	...
	cc = gx_alloc_char_bits(font->dir, NULL, NULL,
	                        (ushort)(bbox.q.x - bbox.p.x),
	                        (ushort)(bbox.q.y - bbox.p.y), &log2_scale, 1);
	if(cc == 0)
		return NULL;
	...
	if_debug5('k', "[k]xfont %s char %d/0x%x#0x%lx=>0x%lx\n",
	          font->font_name.chars, enc_index, (int)chr, (uint32_t)glyph,
	          (uint32_t)xg);
...
top:
	...
	if(xg != gx_no_xglyph && (xf = cc_pair(cc)->xfont) != 0) {
		...
		if(gs_color_writes_pure(pgs)) {
			code = (*xf->common.procs->render_char)(
			    xf, xg, imaging_dev, cx, cy, pdevc->colors.pure, 0);
			if_debug8('K', "[K]render_char display: xfont=0x%lx, "
			               "glyph=0x%lx\n\tdev=0x%lx(%s) "
			               "x,y=%d,%d, color=0x%lx => %d\n",
			          (uint32_t)xf, (uint32_t)xg,
			          (uint32_t)imaging_dev, imaging_dev->dname, cx,
			          cy, (uint32_t)pdevc->colors.pure, code);
			if(code == 0)
		...
		if(!cc_has_bits(cc)) {
			...
			gs_make_mem_mono_device(&mdev, dev->memory,
			                        imaging_dev);
			gx_open_cache_device(&mdev, cc);
			code = (*xf->common.procs->render_char)(
			    xf, xg, (gx_device*)&mdev, cx - x, cy - y,
			    (gx_color_index)1, 1);
			if_debug7('K', "[K]render_char to bits: xfont=0x%lx, "
			               "glyph=0x%lx\n\tdev=0x%lx(%s) x,y=%d,%d "
			               "=> %d\n",
			          (uint32_t)xf, (uint32_t)xg, (uint32_t)&mdev,
			          mdev.dname, cx - x, cy - y, code);
			if(code != 0)
				return_check_interrupt(penum->memory, 1);
			gx_add_char_bits(cc_pair(cc)->font->dir, cc,
			                 &scale_log2_1);
	   ...
	   !lop_no_S_is_T(pgs->log_op)) {
		...
		if(code >= 0) {
			code = (*dev_proc(orig_dev, fill_mask))(
			    orig_dev, bits, 0, cc_raster(cc), cc->id, x, y, w,
			    h, pdevc, depth, pgs->log_op, pcpath);
			if(code >= 0)
				goto done;
		}
		...
		if(depth > 1) {
			code = (*dev_proc(imaging_dev, copy_alpha))(
			    imaging_dev, bits, 0, cc_raster(cc), cc->id, x, y,
			    w, h, color, depth);
			if(code >= 0)
				return_check_interrupt(penum->memory, 0);
			/* copy_alpha failed, construct a monobit mask. */
			bits = compress_alpha_bits(
			    cc, penum->memory->non_gc_memory);
			if(bits == 0)
				return 1; /* VMerror, but recoverable */
		}
		code = (*dev_proc(imaging_dev, copy_mono))(
		    imaging_dev, bits, 0, bitmap_raster(w), gs_no_id, x, y, w,
		    h, gx_no_color_index, color);
		goto done;
	}
	...
	{ /* Use imagemask to render the character. */
		...
		int iy;
		uint used,
		    raster = (bits == cc_bits(cc) ? cc_raster(cc)
...
#undef mat
	...
	if(bits != cc_bits(cc))
		gs_free_object(penum->memory->non_gc_memory, bits,
		               "compress_alpha_bits");
	if(code > 0)
		code = 0;
...
compress_alpha_bits(const cached_char* cc, gs_memory_t* mem)
{
	...
	byte* mask =
	    gs_alloc_bytes(mem, draster * height, "compress_alpha_bits");
