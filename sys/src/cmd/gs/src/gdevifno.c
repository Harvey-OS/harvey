...
private
WImage* initwriteimage(FILE* f, Rectangle r, int ldepth);
private
int writeimageblock(WImage* w, uchar* data, int ndata);
private
int bytesperline(Rectangle, int);
private
int rgb2cmap(int, int, int);
private
long cmap2rgb(int);
...
private
dev_proc_map_rgb_color(inferno_rgb2cmap);
private
dev_proc_map_color_rgb(inferno_cmap2rgb);
private
dev_proc_open_device(inferno_open);
private
dev_proc_close_device(inferno_close);
private
dev_proc_print_page(inferno_print_page);
private
dev_proc_put_params(inferno_put_params);
private
dev_proc_get_params(inferno_get_params);
...
private
const gx_device_procs inferno_procs = prn_color_params_procs(
    inferno_open, gdev_prn_output_page, gdev_prn_close, inferno_rgb2cmap,
    inferno_cmap2rgb, gdev_prn_get_params, gdev_prn_put_params);
/*
...
private
int
inferno_cmap2rgb(gx_device* dev, gx_color_index color, gx_color_value rgb[3])
{
	int shift, i;
	inferno_device* idev;
...
uint32_t
    p9color[Rlevs * Glevs * Blevs]; /* index blue most sig, red least sig */
...
inferno_open(gx_device* dev)
{
	...
	//	printf("inferno_open gs_inferno_device.dither = %d idev->dither
	//= %d\n",
	...
	for(y = 0; y < pdev->height; y++) {
		...
		for(x = 0; x < pdev->width; x++) {
			...
			case 3:
				if(1 || dither) {
					u = p9color[us];
					/* the ulong in p9color is a 2x2 matrix.
					 * pull the entry
		...
		if(xmod)
			p[(x - 1) / ppb[ldepth]] <<=
			    ((ppb[ldepth] - xmod) * bpp[ldepth]);
		if(writeimageblock(w, p, bpl) == ERROR) {
			gs_free(pdev->memory, buf, gsbpl, 1,
			        "inferno_print_page");
			return_error(gs_error_Fatal);
		}
	}
...
addbuf(WImage* w, uchar* buf, int nbuf)
{
	...
	if(buf == nil || w->outp + nbuf > w->eout) {
		if(w
		       ->loutp == w->outbuf) { /* can't really happen -- we
		                                  checked line length above */
...
gobbleline(WImage* w)
{
	...
	for(p = w->inbuf + w->line; p != eline;) {
		...
		for(hp = w->hash[w->h].next; hp; hp = hp->next) {
			 ...
			 * for(s=p, t=w->ibase+hp->p; s<es && *s == *t; s++,
			 * t++)
			 * 	;
			 */
...
initwriteimage(FILE* f, Rectangle r, int ldepth)
{
	...
	fprintf(f, "compressed\n%11d %11d %11d %11d %11d ", ldepth, r.min.x,
