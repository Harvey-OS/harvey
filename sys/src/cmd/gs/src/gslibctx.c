...
int
gs_lib_ctx_init(gs_memory_t* mem)
{
	gs_lib_ctx_t* pio = 0;
	...
	pio = mem->gs_lib_ctx = (gs_lib_ctx_t*)gs_alloc_bytes_immovable(
	    mem, sizeof(gs_lib_ctx_t), "gs_lib_ctx_init");
	if(pio == 0)
		return -1;
	pio->memory = mem;
	...
	pio->gs_next_id =
	    1; /* this implies that each thread has its own complete state */
...
int
errwrite(const char* str, int len)
{
	int code;
	if(len == 0)
		return 0;
	if(mem_err_print->gs_lib_ctx->stderr_fn)
		return (*mem_err_print->gs_lib_ctx->stderr_fn)(
		    mem_err_print->gs_lib_ctx->caller_handle, str, len);
...
void
outflush(const gs_memory_t* mem)
{
