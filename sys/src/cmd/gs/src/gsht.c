...
/* Copyright (C) 1989, 1996, 1997, 1998, 1999 Aladdin Enterprises.  All rights
  reserved.
...
RELOC_PTRS_WITH(halftone_reloc_ptrs, gs_halftone* hptr)
{
	...
	case ht_type_spot:
		if(hptr->params.spot.transfer == 0)
			RELOC_PTR(gs_halftone,
			          params.spot.transfer_closure.data);
		break;
	case ht_type_threshold:
		RELOC_CONST_STRING_PTR(gs_halftone,
		                       params.threshold.thresholds);
		if(hptr->params.threshold.transfer == 0)
			RELOC_PTR(gs_halftone,
			          params.threshold.transfer_closure.data);
		break;
	case ht_type_threshold2:
	...
	case ht_type_client_order:
		...
		RELOC_PTR(gs_halftone,
		          params.client_order.transfer_closure.data);
		break;
	case ht_type_multiple:
	case ht_type_multiple_colorscreen:
                         ...
                         gs_color_select_t select)
{
	if(select == gs_color_select_all) {
		...
		for(i = 0; i < gs_color_select_count; ++i)
			gx_imager_setscreenphase(pis, x, y,
			                         (gs_color_select_t)i);
		return 0;
...
{
	int code =
	    gx_imager_setscreenphase((gs_imager_state*)pgs, x, y, select);
	 ...
	 */
	if(code >= 0 &&
	   (select == gs_color_select_texture || select == gs_color_select_all))
...
gs_currentscreenphase(const gs_state* pgs, gs_int_point* pphase,
                      gs_color_select_t select)
{
	return gs_currentscreenphase_pis((const gs_imager_state*)pgs, pphase,
	                                 select);
}
                            ...
                            gs_memory_t* mem)
{
	...
	while((code = gs_screen_currentpoint(penum, &pt)) == 0)
		if((code = gs_screen_next(
		        penum, (*phsp->spot_function)(pt.x, pt.y))) < 0)
			return code;
	return 0;
}
                     ...
                     const gx_ht_order_procs_t* procs, gs_memory_t* mem)
{
	...
	if(num_levels > 0) {
		porder->levels = (uint*)gs_alloc_byte_array(
		    mem, porder->num_levels, sizeof(uint),
	...
	if(num_bits > 0) {
		porder->bit_data = gs_alloc_byte_array(
		    mem, porder->num_bits, porder->procs->bit_data_elt_size,
		    "alloc_ht_order_data(bit_data)");
		if(porder->bit_data == 0) {
			gs_free_object(mem, porder->levels,
			               "alloc_ht_order_data(levels)");
			porder->levels = 0;
			return_error(gs_error_VMerror);
		}
...
gx_ht_copy_ht_order(gx_ht_order* pdest, gx_ht_order* psrc, gs_memory_t* mem)
{
	...
	*pdest = *psrc;
	...
	code = gx_ht_alloc_ht_order(pdest, psrc->width, psrc->height,
	                            psrc->num_levels, psrc->num_bits,
	                            psrc->shift, psrc->procs, mem);
	if(code < 0)
		return code;
	if(pdest->levels != 0)
		memcpy(pdest->levels, psrc->levels,
		       psrc->num_levels * sizeof(uint));
	if(pdest->bit_data != 0)
                            ...
                            uint num_levels, gs_memory_t* mem)
{
	...
	const gx_ht_order_procs_t* procs =
	    (num_bits > 2000 && num_bits <= max_ushort
	         ? &ht_order_procs_short
	         : &ht_order_procs_default);
	int code;
                         ...
                         uint num_levels, uint num_bits, gs_memory_t* mem)
{
	...
	order.params.R1 = 1;
	gx_compute_cell_values(&order.params);
	code = gx_ht_alloc_ht_order(&order, width, height, num_levels, num_bits,
	                            0, &ht_order_procs_default, mem);
	if(code < 0)
		return code;
	*porder = order;
	return 0;
...
gx_ht_construct_spot_order(gx_ht_order* porder)
{
	...
	for(i = num_levels; i > 0;) {
		...
		levels[i] = i * copies;
		for(k = 0; k < copies; k++, bp--, hy += num_levels,
		x = (x + width - shift) % width)
			bp->offset = hy + x;
	}
...
gx_ht_construct_bit(gx_ht_bit* bit, int width, int bit_num)
{
	...
	bit->mask = 0;
	for(pb = (byte*)&bit->mask + (sizeof(mask) - 1); mask != 0;
	    mask >>= 8, pb--)
		*pb = (byte)mask;
}
...
gx_ht_construct_bits(gx_ht_order* porder)
{
	...
	for(i = 0, phb = (gx_ht_bit*)porder->bit_data; i < porder->num_bits;
	    i++, phb++)
	...
	if(pdht->components) {
		...
		for(i = 0; i < pdht->num_comp; ++i)
			if(pdht->components[i].corder.bit_data !=
			   pdht->order.bit_data) { /* Currently, all orders
			                              except the default one */
				/* own their caches. */
				gx_ht_order_release(&pdht->components[i].corder,
				                    mem, true);
			}
 ...
 */
int
gs_color_name_component_number(gx_device* dev, const char* pname, int name_size,
                               int halftonetype)
{
	int num_colorant;

#define check_colorant_name(dev, name)                                         \
	((*dev_proc(dev, get_color_comp_index))(dev, name, strlen(name),       \
	                                        NO_COMP_NAME_TYPE))

#define check_colorant_name_length(dev, name, length)                          \
	((*dev_proc(dev, get_color_comp_index))(dev, name, length,             \
	                                        NO_COMP_NAME_TYPE))
...
#define check_name(str, pname, length)                                         \
	...
	if(num_colorant >= 0) {
		 ...
		 * colorant is logically present in the device but not being
		 * used
		 * because a SeparationOrder parameter is specified.  Since we
		 * are
 ...
 */
int
gx_imager_dev_ht_install(gs_imager_state* pis, gx_device_halftone* pdht,
                         gs_halftone_type type, const gx_device* dev)
{
	...
	dht.type = type;
	dht.components =
	    gs_alloc_struct_array(pis->memory, num_comps, gx_ht_order_component,
		...
		for(i = 0; i < input_ncomps && code >= 0; i++) {
			...
			if(comp_num >= 0 &&
			   comp_num < GX_DEVICE_COLOR_MAX_COMPONENTS) {
				gx_ht_order* p_d_order =
				    &dht.components[comp_num].corder;
				...
				/*
				 * The component can be used only if it is from
				 * the
				 * proper memory
				 */
				if(mem_diff)
					code = gx_ht_copy_ht_order(
					    p_d_order, p_s_order, pis->memory);
				else {
	...
	for(i = 0; i < num_comps && code >= 0; i++) {
		...
		if(dht.components[i].comp_number != i) {
			if(used_default || mem_diff)
				code = gx_ht_copy_ht_order(porder, &pdht->order,
				                           pis->memory);
			else {
		...
		if((wse = porder->wse) != 0) {
			...
			lcm_height /= dh;
			lcm_width =
			    (w > max_int / lcm_width ? max_int : lcm_width * w);
			lcm_height =
			    (h > max_int / lcm_height ? max_int
			                              : lcm_height * h);
			...
			if(porder->cache == 0) {
				...
				num_tiles =
				    1 + max_tile_cache_bytes / tile_bytes;
				pcache =
				    gx_ht_alloc_cache(pis->memory, num_tiles,
				                      tile_bytes * num_tiles);
				if(pcache == NULL)
					code = gs_error_VMerror;
				else {
					porder->cache = pcache;
					gx_ht_init_cache(pis->memory, pcache,
					                 porder);
				}
			}
		}
	...
	if(code >= 0) {
		...
		if(pisdht != 0 && pisdht->rc.ref_count == 1) {
			if(pdht != pisdht)
				gx_device_halftone_release(pisdht,
				                           pisdht->rc.memory);
		} else {
			rc_unshare_struct(pis->dev_ht, gx_device_halftone,
			                  &st_device_halftone, pis->memory,
			                  BEGIN code = gs_error_VMerror;
			                  goto err;
			                  END, "gx_imager_dev_ht_install");
			pisdht = pis->dev_ht;
		}
		 ...
		 * pdht == pis->dev_ht, this must done before updating
		 *pis->dev_ht.
		 *
		 * If the default order has been used for a device component,
		 *and
		 * any of the source component orders share their levels or
		 *bit_data
		 * arrays with the default order, clear the pointers in those
		 *orders
		 * now. This is necessary because the default order's pointers
		 *will
		...
		if(pdht->components != 0) {
			...
			for(i = 0; i < input_ncomps; i++) {
				gx_ht_order_component* p_s_comp =
				    &pdht->components[i];
				gx_ht_order* p_s_order = &p_s_comp->corder;
				int comp_num = p_s_comp->comp_number;
 ...
 */
int
gx_ht_install(gs_state* pgs, const gs_halftone* pht, gx_device_halftone* pdht)
{
	...
	if(old_ht != 0 && old_ht->rc.memory == mem && old_ht->rc.ref_count == 1)
		...
		rc_alloc_struct_1(new_ht, gs_halftone, &st_halftone, mem,
		                  return_error(gs_error_VMerror),
		                  "gx_ht_install(new halftone)");
	code = gx_imager_dev_ht_install((gs_imager_state*)pgs, pdht, pht->type,
	                                gs_currentdevice_inline(pgs));
	if(code < 0) {
		if(new_ht != old_ht)
			gs_free_object(mem, new_ht,
			               "gx_ht_install(new halftone)");
		return code;
	}
...
gx_imager_set_effective_xfer(gs_imager_state* pis)
{
	...
	for(i = 0; i < GX_DEVICE_COLOR_MAX_COMPONENTS; i++)
		pis->effective_transfer[i] =
		    pis->set_transfer.gray; /* default */
	...
	if(pis->set_transfer.red) {
		...
		if(component_num >= 0)
			pis->effective_transfer[component_num] =
