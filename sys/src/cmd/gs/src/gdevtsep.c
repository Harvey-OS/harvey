...
/* Copyright (C) 1995, 2000 Aladdin Enterprises, 2004 Artifex Software Inc.  All
  rights reserved.
...
private
dev_proc_print_page(tiffgray_print_page);

private
const gx_device_procs tiffgray_procs = prn_color_procs(
    gdev_prn_open, gdev_prn_output_page, gdev_prn_close,
    gx_default_gray_map_rgb_color, gx_default_gray_map_color_rgb);

const gx_device_printer gs_tiffgray_device = {prn_device_body(
    gx_device_tiff, tiffgray_procs, "tiffgray", DEFAULT_WIDTH_10THS,
    DEFAULT_HEIGHT_10THS, X_DPI, Y_DPI, 0, 0, 0, 0, /* Margins */
...
private
const tiff_gray_directory dir_gray_template = {
...
private
const tiff_gray_values val_gray_template = {{8}};
...
tiffgray_print_page(gx_device_printer* pdev, FILE* file)
{
	...
	code = gdev_tiff_begin_page(
	    pdev, &tfdev->tiff, file, (const TIFF_dir_entry*)&dir_gray_template,
	    sizeof(dir_gray_template) / sizeof(TIFF_dir_entry),
	    (const byte*)&val_gray_template, sizeof(val_gray_template), 0);
	if(code < 0)
		return code;
	...
	/* Write the page data. */
	{
		...
		byte* line =
		    gs_alloc_bytes(pdev->memory, raster, "tiffgray_print_page");
		byte* row;
...
private
dev_proc_print_page(tiff32nc_print_page);

#define cmyk_procs(p_map_color_rgb, p_map_cmyk_color)                          \
	gdev_prn_open, NULL, NULL, gdev_prn_output_page, gdev_prn_close, NULL, \
	    p_map_color_rgb, NULL, NULL, NULL, NULL, NULL, NULL,               \
	    gdev_prn_get_params, gdev_prn_put_params, p_map_cmyk_color, NULL,  \
	    NULL, NULL, gx_page_device_get_page_device

/* 8-bit-per-plane separated CMYK color. */

private
const gx_device_procs tiff32nc_procs = {
    cmyk_procs(cmyk_8bit_map_color_cmyk, cmyk_8bit_map_cmyk_color)};

const gx_device_printer gs_tiff32nc_device = {prn_device_body(
    gx_device_tiff, tiff32nc_procs, "tiff32nc", DEFAULT_WIDTH_10THS,
    DEFAULT_HEIGHT_10THS, X_DPI, Y_DPI, 0, 0, 0, 0, /* Margins */
...
private
const tiff_cmyk_values val_cmyk_template = {{8, 8, 8, 8}};

private
const tiff_cmyk_directory dir_cmyk_template = {
...
tiff32nc_print_page(gx_device_printer* pdev, FILE* file)
{
	...
	/* Write the page directory. */
	code = gdev_tiff_begin_page(
	    pdev, &tfdev->tiff, file, (const TIFF_dir_entry*)&dir_cmyk_template,
	    sizeof(dir_cmyk_template) / sizeof(TIFF_dir_entry),
	    (const byte*)&val_cmyk_template, sizeof(val_cmyk_template), 0);
	if(code < 0)
		return code;
	...
	/* Write the page data. */
	{
		...
		int raster = gdev_prn_raster(pdev);
		byte* line =
		    gs_alloc_bytes(pdev->memory, raster, "tiff32nc_print_page");
		byte* row;
...
private
dev_proc_open_device(tiffsep_prn_open);
private
dev_proc_close_device(tiffsep_prn_close);
private
dev_proc_get_params(tiffsep_get_params);
private
dev_proc_put_params(tiffsep_put_params);
private
dev_proc_print_page(tiffsep_print_page);
private
dev_proc_get_color_mapping_procs(tiffsep_get_color_mapping_procs);
private
dev_proc_get_color_comp_index(tiffsep_get_color_comp_index);
private
dev_proc_encode_color(tiffsep_encode_color);
private
dev_proc_decode_color(tiffsep_decode_color);
private
dev_proc_update_spot_equivalent_colors(tiffsep_update_spot_equivalent_colors);
...
ENUM_PTRS_END
private
RELOC_PTRS_WITH(tiffsep_device_reloc_ptrs, tiffsep_device* pdev)
{
...
gs_private_st_composite_final(st_tiffsep_device, tiffsep_device,
                              "tiffsep_device", tiffsep_device_enum_ptrs,
                              tiffsep_device_reloc_ptrs,
...
#define device_procs                                                                       \
	{                                                                                  \
		tiffsep_prn_open, gx_default_get_initial_matrix,                           \
		    NULL,                 /* sync_output */                                \
...
#define tiffsep_device_body(procs, dname, ncomp, pol, depth, mg, mc, cn)       \
	std_device_full_body_type_extended(                                    \
	    tiffsep_device, &procs, dname, &st_tiffsep_device,                 \
	    (int)((long)(DEFAULT_WIDTH_10THS) * (X_DPI) / 10),                 \
	    (int)((long)(DEFAULT_HEIGHT_10THS) * (Y_DPI) / 10), X_DPI, Y_DPI,  \
 ...
 */
private
const gx_device_procs spot_cmyk_procs = device_procs;

const tiffsep_device gs_tiffsep_device = {
    tiffsep_device_body(spot_cmyk_procs, "tiffsep", NC,
                        GX_CINFO_POLARITY_SUBTRACTIVE, NC * 8, MAX_COLOR_VALUE,
                        MAX_COLOR_VALUE, "DeviceCMYK"),
...
private
void
tiffsep_rgb_cs_to_cm(gx_device* dev, const gs_imager_state* pis, frac r, frac g,
                     frac b, frac out[])
{
	int* map = ((tiffsep_device*)dev)->devn_params.separation_order_map;
...
private
void
tiffsep_cmyk_cs_to_cm(gx_device* dev, frac c, frac m, frac y, frac k,
                      frac out[])
{
	int* map = ((tiffsep_device*)dev)->devn_params.separation_order_map;
...
private
const gx_cm_color_map_procs tiffsep_cm_procs = {
    tiffsep_gray_cs_to_cm, tiffsep_rgb_cs_to_cm, tiffsep_cmyk_cs_to_cm};
...
tiffsep_put_params(gx_device* pdev, gs_param_list* plist)
{
	...
	return devn_printer_put_params(pdev, plist, &(pdevn->devn_params),
	                               &(pdevn->equiv_cmyk_colors));
}
...
private
int
tiffsep_get_color_comp_index(gx_device* dev, const char* pname, int name_size,
                             int component_type)
{
	tiffsep_device* pdev = (tiffsep_device*)dev;
	...
	return devn_get_color_comp_index(
	    dev, &(pdev->devn_params), &(pdev->equiv_cmyk_colors), pname,
	    name_size, component_type, ALLOW_EXTRA_SPOT_COLORS);
}
...
private
void
copy_separation_name(tiffsep_device* pdev, char* buffer, int max_size,
                     int sep_num)
{
...
private
int
create_separation_file_name(tiffsep_device* pdev, char* buffer, uint max_size,
                            int sep_num)
{
...
#define SUFFIX_SIZE (4 * APPEND_TIF_TO_NAME)
	...
	if(sep_num < pdev->devn_params.num_std_colorant_names) {
		if(max_size <
		   strlen(pdev->devn_params.std_colorant_names[sep_num]))
...
/*
 * This routine creates a list to map the component number to a separation
 *number.
...
tiffsep_prn_close(gx_device* pdev)
{
	...
	int comp_num;
	int num_comp = number_output_separations(
	    num_dev_comp, num_std_colorants, num_order, num_spot);
	...
	for(comp_num = 0; comp_num < num_comp; comp_num++) {
		if(pdevn->sep_file[comp_num] != NULL) {
			...
			code = create_separation_file_name(
			    pdevn, name, MAX_FILE_NAME_SIZE, sep_num);
			if(code < 0)
				return code;
			code = gx_device_close_output_file(
			    pdev, name, pdevn->sep_file[comp_num]);
			if(code < 0)
				return code;
...
private
void
build_cmyk_map(tiffsep_device* pdev, int num_comp, short* map_comp_to_sep,
               cmyk_composite_map* cmyk_map)
{
	...
	for(comp_num = 0; comp_num < num_comp; comp_num++) {
		...
		if(sep_num < pdev->devn_params.num_std_colorant_names) {
			switch(sep_num) {
			case 0:
				cmyk_map[comp_num].c = frac_1;
				break;
			case 1:
				cmyk_map[comp_num].m = frac_1;
				break;
			case 2:
				cmyk_map[comp_num].y = frac_1;
				break;
			case 3:
				cmyk_map[comp_num].k = frac_1;
				break;
			}
		} else {
			sep_num -= pdev->devn_params.num_std_colorant_names;
			if(pdev->equiv_cmyk_colors.color[sep_num]
			       .color_info_valid) {
				cmyk_map[comp_num].c =
				    pdev->equiv_cmyk_colors.color[sep_num].c;
				cmyk_map[comp_num].m =
				    pdev->equiv_cmyk_colors.color[sep_num].m;
				cmyk_map[comp_num].y =
				    pdev->equiv_cmyk_colors.color[sep_num].y;
				cmyk_map[comp_num].k =
				    pdev->equiv_cmyk_colors.color[sep_num].k;
			}
		}
	}
}
...
private
void
build_cmyk_raster_line(byte* src, byte* dest, int width, int num_comp,
                       int pixel_size, cmyk_composite_map* cmyk_map)
{
	int pixel, comp_num;
...
tiffsep_print_page(gx_device_printer* pdev, FILE* file)
{
	...
	for(sep_num = 0; sep_num < num_spot; sep_num++) {
		copy_separation_name(tfdev, name,
		                     MAX_FILE_NAME_SIZE - base_filename_length -
		                         SUFFIX_SIZE,
		                     sep_num);
	...
	*/
	...
	num_comp =
	    number_output_separations(tfdev->color_info.num_components,
	...
	for(comp_num = 0; comp_num < num_comp; comp_num++) {
		...
		if(code < 0)
			return code;
		/*
		 * Close the old separation file if we are creating individual
		 * files
		 * for each page.
		 */
		if(tfdev->sep_file[comp_num] != NULL && fmt != NULL) {
			code = gx_device_close_output_file(
			    (const gx_device*)tfdev, name,
		...
		if(tfdev->sep_file[comp_num] == NULL) {
			code = gx_device_open_output_file(
			    (gx_device*)pdev, name, true, false,
			    &(tfdev->sep_file[comp_num]));
			if(code < 0)
				return code;
		}
		...
		code = gdev_tiff_begin_page(
		    pdev, &(tfdev->tiff[comp_num]), tfdev->sep_file[comp_num],
		    (const TIFF_dir_entry*)&dir_gray_template,
		    sizeof(dir_gray_template) / sizeof(TIFF_dir_entry),
		    (const byte*)&val_gray_template, sizeof(val_gray_template),
		    0);
		pdev->color_info.depth = save_depth;
		if(code < 0)
			return code;
...

	{
		...
		int pixel, y;
		byte* line =
		    gs_alloc_bytes(pdev->memory, raster, "tiffsep_print_page");
		byte* sep_line;
		byte* row;

		if(line == NULL)
			return_error(gs_error_VMerror);
		sep_line = gs_alloc_bytes(pdev->memory, cmyk_raster,
		                          "tiffsep_print_page");
		if(sep_line == NULL) {
			gs_free_object(pdev->memory, line,
			               "tiffsep_print_page");
			return_error(gs_error_VMerror);
		}
		...
		for(y = 0; y < pdev->height; ++y) {
			...
			for(comp_num = 0; comp_num < num_comp; comp_num++) {
				...
				for(pixel = 0; pixel < width;
				    pixel++, dest++, src += bytes_pp)
					*dest = MAX_COLOR_VALUE -
					        *src; /* Gray is additive */
				fwrite((char*)sep_line, width, 1,
				       tfdev->sep_file[comp_num]);
			}
			/* Write CMYK equivalent data (tiff32nc format) */
			build_cmyk_raster_line(row, sep_line, width, num_comp,
			                       bytes_pp, cmyk_map);
