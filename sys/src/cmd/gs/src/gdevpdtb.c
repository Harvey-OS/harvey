...
struct pdf_base_font_s {
	...
	enum { DO_SUBSET_UNKNOWN = 0, DO_SUBSET_NO, DO_SUBSET_YES } do_subset;
	bool is_standard;
	/*
...
BASIC_PTRS(pdf_base_font_ptrs){
    GC_OBJ_ELT(pdf_base_font_t, copied), GC_OBJ_ELT(pdf_base_font_t, complete),
    GC_OBJ_ELT(pdf_base_font_t, CIDSet), GC_OBJ_ELT(pdf_base_font_t, FontFile),
 ...
 */
int
pdf_add_subset_prefix(const gx_device_pdf* pdev, gs_string* pstr, byte* used,
                      int count)
{
	uint size = pstr->size;
                    ...
                    bool is_standard, bool orig_name)
{
	...
	gs_font* copied;
	gs_font* complete;
	pdf_base_font_t* pbfont = gs_alloc_struct(
	    mem, pdf_base_font_t, &st_pdf_base_font, "pdf_base_font_alloc");
	const gs_font_name* pfname =
	    pdf_choose_font_name((gs_font*)font, orig_name);
	...
	memset(pbfont, 0, sizeof(*pbfont));
	{
		 ...
		 *Specification" says :
		 *
		 * There is a known bug in the TrueType rasterizer included in
		 *versions of the
		 * PostScript interpreter previous to version 2013. The problem
		 *is that the
		 * translation components of the FontMatrix, as used as an
		 *argument to the
		 * definefont or makefont operators, are ignored. Translation of
		 *user space is
		 * not affected by this bug.
		 *
		 * Besides that, we found that Adobe Acrobat Reader 4 and 5
		 *ignore
		 * FontMatrix.ty .
		 */
	...
	case ft_encrypted2:
		pbfont->do_subset =
		    (is_standard ? DO_SUBSET_NO : DO_SUBSET_UNKNOWN);
		/* We will count the number of glyphs below. */
		pbfont->num_glyphs = -1;
		break;
	case ft_TrueType:
		pbfont->num_glyphs =
		    ((gs_font_type42*)font)->data.trueNumGlyphs;
		pbfont->do_subset = (pbfont->num_glyphs <= MAX_NO_SUBSET_GLYPHS
		                         ? DO_SUBSET_UNKNOWN
		                         : DO_SUBSET_YES);
		break;
	...
	if(pbfont->do_subset != DO_SUBSET_YES) {
		/* The only possibly non-subsetted fonts are Type 1/2 and Type
		 * 42. */
		if(is_standard)
			complete = copied, code = 0;
		else
			code = gs_copy_font((gs_font*)font, &font->FontMatrix,
			                    mem, &complete);
		if(code >= 0)
	...
	pbfont->font_name.data = gs_alloc_string(
	    mem, font_name.size, "pdf_base_font_alloc(font_name)");
...
pdf_base_font_copy_glyph(pdf_base_font_t* pbfont, gs_glyph glyph,
                         gs_font_base* font)
{
	int code = gs_copy_glyph_options(
	    (gs_font*)font, glyph, (gs_font*)pbfont->copied,
...
pdf_do_subset_font(gx_device_pdf* pdev, pdf_base_font_t* pbfont, gs_id rid)
{
	...
	if(pbfont->do_subset == DO_SUBSET_UNKNOWN) {
		...
		if(do_subset && max_pct < 100) {
			/* We want to subset iff used <= total * MaxSubsetPct /
			 * 100. */
			do_subset = false;
			if(max_pct > 0) {
				int max_subset_used =
				    pbfont->num_glyphs * max_pct / 100;
				int used, index;
				gs_glyph ignore_glyph;

				do_subset = true;
				for(index = 0, used = 0;
				    (copied->procs.enumerate_glyph(
				         (gs_font*)copied, &index,
				         GLYPH_SPACE_INDEX, &ignore_glyph),
				     index != 0);)
...
#define SUFFIX_CHAR '~'
	...
	data = gs_resize_string(pdev->pdf_memory, chars, size,
	                        size + suffix_size, "pdf_adjust_font_name");
	if(data == 0)
		return_error(gs_error_VMerror);
                        ...
                        gs_int_rect* FontBBox, gs_id rid, cos_dict_t** ppcd)
{
	...
	gs_font_base* out_font =
	    (do_subset || pbfont->complete == NULL ? pbfont->copied
	                                           : pbfont->complete);
	...
	code = pdf_begin_data_stream(
	    pdev, &writer, DATA_STREAM_BINARY |
	                       /* Don't set DATA_STREAM_ENCRYPT since we write
	                          to a temporary file.
	...
	if(code < 0)
		return code;
	if(pdev->CompatibilityLevel == 1.2 && !do_subset &&
	   !pbfont->is_standard) {
		/*
		 ...
		 */
		int code =
		    pdf_adjust_font_name(pdev, writer.pres->object->id, pbfont);
		if(code < 0)
			return code;
	}
	...
	case ft_encrypted2:
		if(!pdev->HaveCFF) {
			...
			return_error(
			    gs_error_unregistered); /* Not implemented yet. */
		}
	case ft_encrypted:
		...
		if(!pdev->HaveCFF) {
			...
			if(lengths[0] > 0) {
				if(code < 0)
					return code;
				code = cos_dict_put_c_key_int(
				    (cos_dict_t*)writer.pres->object,
				    "/Length1", lengths[0]);
			}
			if(lengths[1] > 0) {
				if(code < 0)
...
#define TYPE2_OPTIONS (WRITE_TYPE2_NO_LENIV | WRITE_TYPE2_CHARSTRINGS)
			code = cos_dict_put_string_copy(
			    (cos_dict_t*)writer.pres->object, "/Subtype",
			    "/Type1C");
			if(code < 0)
				return code;
			code = psf_write_type2_font(
			    writer.binary.strm, (gs_font_type1*)out_font,
			    TYPE2_OPTIONS | (pdev->CompatibilityLevel < 1.3
			                         ? WRITE_TYPE2_AR3
			                         : 0),
...
#define TRUETYPE_OPTIONS (WRITE_TRUETYPE_NAME | WRITE_TRUETYPE_HVMTX)
		...
		const int options =
		    TRUETYPE_OPTIONS | (pdev->CompatibilityLevel <= 1.2
		                            ? WRITE_TRUETYPE_NO_TRIMMED_TABLE
		                            : 0) |
		...
		code = psf_write_truetype_font(&poss, pfont, options, NULL, 0,
		                               &fnstr);
		if(code < 0)
			return code;
	...
	case ft_CID_encrypted:
		code =
		    cos_dict_put_string_copy((cos_dict_t*)writer.pres->object,
		                             "/Subtype", "/CIDFontType0C");
		if(code < 0)
			return code;
		code = psf_write_cid0_font(writer.binary.strm,
		                           (gs_font_cid0*)out_font,
		                           TYPE2_OPTIONS, NULL, 0, &fnstr);
		goto finish;
...
pdf_write_CharSet(gx_device_pdf* pdev, pdf_base_font_t* pbfont)
{
	...
	stream_puts(s, "(");
	for(index = 0; (font->procs.enumerate_glyph((gs_font*)font, &index,
		...
		if(code >= 0 && bytes_compare(gstr.data, gstr.size,
		                              (const byte*)".notdef", 7))
