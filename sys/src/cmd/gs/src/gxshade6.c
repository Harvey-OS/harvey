...
static int min_linear_grades =
    255; /* The minimal number of device color grades,
                  ...
                  int num_vertices)
{
	...
	for(i = 0; i < num_vertices && code >= 0; ++i) {
		curves[i].vertex.cc[1] =
		    0; /* safety. (patch_fill may assume 2 arguments) */
...
shade_next_patch(shade_coord_stream_t* cs, int BitsPerFlag,
                 ...
                 gs_fixed_point interior[4] /* 0 for Coons patch */)
{
	...
	v3:
		num_colors = 2;
	vx:
		if((code = shade_next_coords(cs, curve[1].control, 2)) < 0 ||
...
init_patch_fill_state(patch_fill_state_t* pfs)
{
	...
	for(i = 0; i < pfs->num_components; i++)
		pfs->color_domain.paint.values[i] =
		    max(fcc1.paint.values[i] - fcc0.paint.values[i], 1);
	pfs->vectorization =
	    false; /* A stub for a while. Will use with pclwrite. */
...
patch_resolve_color_inline(patch_color_t* ppcr, const patch_fill_state_t* pfs)
{
	if(pfs->Function) {
		...
		gs_function_evaluate(pfs->Function, ppcr->t,
		                     ppcr->cc.paint.values);
...
patch_interpolate_color(patch_color_t* ppcr, const patch_color_t* ppc0,
                        const patch_color_t* ppc1,
                        const patch_fill_state_t* pfs, floatp t)
{
	...
	if(pfs->Function) {
		...
		for(ci = pfs->num_components - 1; ci >= 0; --ci)
			ppcr->cc.paint.values[ci] =
			    ppc0->cc.paint.values[ci] * (1 - t) +
			    t * ppc1->cc.paint.values[ci];
	}
}
             ...
             const gs_fixed_point ignore_interior[4], floatp u, floatp v)
{
	...
	curve_eval(&c1u, &curve[C1START].vertex.p, &curve[C1XCTRL].control[1],
	           &curve[C1XCTRL].control[0], &curve[C1END].vertex.p, u);
	curve_eval(&d1v, &curve[D1START].vertex.p, &curve[D1CTRL].control[0],
	           &curve[D1CTRL].control[1], &curve[D1END].vertex.p, v);
	curve_eval(&c2u, &curve[C2START].vertex.p, &curve[C2CTRL].control[0],
	           &curve[C2CTRL].control[1], &curve[C2END].vertex.p, u);
	curve_eval(&d2v, &curve[D2START].vertex.p, &curve[D2XCTRL].control[1],
	           &curve[D2XCTRL].control[0], &curve[D2END].vertex.p, v);
#define COMPUTE_COORD(xy)                                                      \
	pt->xy = (fixed)((co_v * c1u.xy + v * c2u.xy) +                        \
	                 (co_u * d1v.xy + u * d2v.xy) -                        \
                             ...
                             gs_imager_state* pis)
{
	...
	code = mesh_init_fill_state((mesh_fill_state_t*)&state,
	                            (const gs_shading_mesh_t*)psh0, rect_clip,
	                            dev, pis);
	if(code < 0)
		return code;
	...
	curve[0].straight = curve[1].straight = curve[2].straight =
	    curve[3].straight = false;
	shade_next_init(&cs, (const gs_shading_mesh_params_t*)&psh->params,
	                pis);
	while((code = shade_next_patch(&cs, psh->params.BitsPerFlag, curve,
	                               NULL)) == 0 &&
                              ...
                              gs_imager_state* pis)
{
	...
	while((code = shade_next_patch(&cs, psh->params.BitsPerFlag, curve,
	                               interior)) == 0) {
		/*
		 * The order of points appears to be consistent with that for
		 * Coons
		 * patches, which is different from that documented in Red Book
		 * 3.
		 */
		...
		code =
		    patch_fill(&state, curve, swapped_interior, Tpp_transform);
		if(code < 0)
			break;
	}
	term_patch_fill_state(&state);
...
typedef enum {
	interpatch_padding =
	    1, /* A Padding between patches for poorly designed documents. */
...
wedge_vertex_list_elem_buffer_alloc(patch_fill_state_t* pfs)
{
	...
	pfs->wedge_vertex_list_elem_buffer =
	    (wedge_vertex_list_elem_t*)gs_alloc_bytes(
	        memory, sizeof(wedge_vertex_list_elem_t) *
	                    pfs->wedge_vertex_list_elem_count_max,
...
private
inline wedge_vertex_list_elem_t*
wedge_vertex_list_elem_reserve(patch_fill_state_t* pfs)
{
	...
	if(pfs->wedge_vertex_list_elem_count <
	   pfs->wedge_vertex_list_elem_count_max)
		return pfs->wedge_vertex_list_elem_buffer +
		       pfs->wedge_vertex_list_elem_count++;
	return NULL;
}

private
inline void
wedge_vertex_list_elem_release(patch_fill_state_t* pfs,
...
release_triangle_wedge_vertex_list_elem(patch_fill_state_t* pfs,
                                        wedge_vertex_list_elem_t* beg,
                                        wedge_vertex_list_elem_t* end)
{
...
private
inline void
release_wedge_vertex_list(patch_fill_state_t* pfs, wedge_vertex_list_t* ll,
                          int n)
{
	int i;
...
private
inline wedge_vertex_list_elem_t*
wedge_vertex_list_find(wedge_vertex_list_elem_t* beg,
                       const wedge_vertex_list_elem_t* end, int level)
{
              ...
              const gs_fixed_point* p2, ulong rgbcolor)
{
	...
	if(!vd_enabled)
		return;
	vd_quad(p0->x, p0->y, p0->x, p0->y, p1->x, p1->y, p2->x, p2->y, 0,
	        rgbcolor);
#endif
}
...
{
#ifdef DEBUG
	vd_quad(p->p[0][0]->p.x, p->p[0][0]->p.y, p->p[0][1]->p.x,
	        p->p[0][1]->p.y, p->p[1][1]->p.x, p->p[1][1]->p.y,
	        p->p[1][0]->p.x, p->p[1][0]->p.y, 0, rgbcolor);
#endif
}

private
inline int
curve_samples(patch_fill_state_t* pfs, const gs_fixed_point* pole,
              int pole_step, fixed fixed_flat)
{
	curve_segment s;
	int k;
	...
	k = gx_curve_log2_samples(pole[0].x, pole[0].y, &s, fixed_flat);
	{
		...
		int k1;
		fixed L = any_abs(pole[1].x - pole[0].x) +
		          any_abs(pole[1].y - pole[0].y) +
		          any_abs(pole[2].x - pole[1].x) +
		          any_abs(pole[2].y - pole[1].y) +
		          any_abs(pole[3].x - pole[2].x) +
		          any_abs(pole[3].y - pole[2].y);
#endif
...
private
bool
intersection_of_small_bars(const gs_fixed_point q[4], int i0, int i1, int i2,
                           int i3, fixed* ry, fixed* ey)
{
	...
	/* vp2 = vp2a - vp2b; It can overflow int64_t, but we only need the
	 * sign. */
	if(vp2a > vp2b)
		s2 = 1;
	else if(vp2a < vp2b)
	...
	/* vp3 = vp3a - vp3b; It can overflow int64_t, but we only need the
	 * sign. */
	if(vp3a > vp3b)
		s3 = 1;
	else if(vp3a < vp3b)
	...
	if(s2 == 0) {
		...
		/* The intersection definitely exists, so the determinant isn't
		 * zero.  */
...
#define USE_INT64_T (1 || !USE_DOUBLE)
#if USE_DOUBLE
		{
			...
			/* The determinant can't compute in double due to
			   possible loss of all significant bits when
			   subtracting the
...
#if USE_INT64_T
		{
			/* Assuming small bars : cubes of coordinates must fit
			   into int64_t.
			...
			{ /* Likely when called form wedge_trap_decompose or
			     constant_color_quadrangle,
			     we always have det > 0 && num >= 0, but we check
			     here for a safety reason. */
				if(det < 0) {
					num = -num;
					det = -det;
				}
				...
				iy = (fixed)iiy;
				if(iy != iiy) {
					/* If it is inside the bars, it must fit
					 * into fixed. */
					return false;
				}
			}
...
private
inline void
make_trapezoid(const gs_fixed_point q[4], int vi0, int vi1, int vi2, int vi3,
               fixed ybot, fixed ytop, bool swap_axes, bool orient,
               gs_fixed_edge* le, gs_fixed_edge* re)
{
	if(!orient) {
		le->start = q[vi0];
		le->end = q[vi1];
...
private
inline int
gx_shade_trapezoid(patch_fill_state_t* pfs, const gs_fixed_point q[4], int vi0,
                   int vi1, int vi2, int vi3, fixed ybot0, fixed ytop0,
                   bool swap_axes, const gx_device_color* pdevc, bool orient)
{
	...
	if(dbg_nofill)
		return 0;
#endif
	make_trapezoid(q, vi0, vi1, vi2, vi3, ybot, ytop, swap_axes, orient,
	               &le, &re);
	if(!VD_TRACE_DOWN)
		vd_disable;
	code = dev_proc(pfs->dev, fill_trapezoid)(
	    pfs->dev, &le, &re, ybot, ytop, swap_axes, pdevc, pfs->pis->log_op);
	vd_restore;
	return code;
}
...
private
int
patch_color_to_device_color(const patch_fill_state_t* pfs,
                            const patch_color_t* c, gx_device_color* pdevc)
{
	...
	return pcs->type->remap_color(&fcc, pcs, pdevc, pfs->pis, pfs->dev,
	                              gs_color_select_texture);
}

private
inline double
color_span(const patch_fill_state_t* pfs, const patch_color_t* c0,
           const patch_color_t* c1)
{
	...
	m = any_abs(c1->cc.paint.values[0] - c0->cc.paint.values[0]) /
	    pfs->color_domain.paint.values[0];
	for(i = 1; i < n; i++)
		m = max(m, any_abs(c1->cc.paint.values[i] -
		                   c0->cc.paint.values[i]) /
		               pfs->color_domain.paint.values[i]);
	return m;
}
...
private
inline void
color_diff(const patch_fill_state_t* pfs, const patch_color_t* c0,
           const patch_color_t* c1, patch_color_t* d)
{
	int n = pfs->num_components, i;

	for(i = 0; i < n; i++)
		d->cc.paint.values[i] =
		    c1->cc.paint.values[i] - c0->cc.paint.values[i];
}
...
color_norm(const patch_fill_state_t* pfs, const patch_color_t* c)
{
	...
	for(i = 1; i < n; i++)
		m = max(m, any_abs(c->cc.paint.values[i]) /
		               pfs->color_domain.paint.values[i]);
	return m;
}
...
private
inline int
isnt_color_monotonic(const patch_fill_state_t* pfs, const patch_color_t* c0,
                     const patch_color_t* c1)
...
private
inline int
constant_color_trapezoid(patch_fill_state_t* pfs, gs_fixed_edge* le,
                         gs_fixed_edge* re, fixed ybot, fixed ytop,
                         bool swap_axes, const patch_color_t* c)
{
	patch_color_t c1 = *c;
	gx_device_color dc;
	int code;
...
/* if (dbg_nofill)
	...
	if(!VD_TRACE_DOWN)
		vd_disable;
	code = dev_proc(pfs->dev, fill_trapezoid)(
	    pfs->dev, le, re, ybot, ytop, swap_axes, &dc, pfs->pis->log_op);
	vd_restore;
	return code;
}
...
private
inline float
function_linearity(const patch_fill_state_t* pfs, const patch_color_t* c0,
                   const patch_color_t* c1)
{
	float smoothness = max(pfs->smoothness, 1.0 / min_linear_grades), s = 0;
	/* Restrict the smoothness with 1/min_linear_grades, because
	   cs_is_linear
	...
	if(pfs->Function != NULL) {
		...
		for(j = 0; j < count_of(q); j++) {
			...
			for(i = 0; i < pfs->num_components; i++) {
				float v = c0->cc.paint.values[i] * (1 - q[j]) +
				          c1->cc.paint.values[i] * q[j];
...
private
inline int
is_color_linear(const patch_fill_state_t* pfs, const patch_color_t* c0,
                const patch_color_t* c1)
	...
	if(pfs->unlinear)
		...
		gs_direct_color_space* cs =
		    (gs_direct_color_space*)
		        pfs->direct_space; /* break 'const'. */
		int code;
		float smoothness =
		    max(pfs->smoothness, 1.0 / min_linear_grades);
		/* Restrict the smoothness with 1/min_linear_grades, because
		   cs_is_linear
		   can't provide a better precision due to the color
		   representation with integers.
		...
		if(s > smoothness)
			return 0;
		code = cs_is_linear(cs, pfs->pis, pfs->dev, &c0->cc, &c1->cc,
		                    NULL, NULL, smoothness - s);
		if(code <= 0)
			return code;
		return 1;
	}
...
private
int
decompose_linear_color(patch_fill_state_t* pfs, gs_fixed_edge* le,
                       gs_fixed_edge* re, fixed ybot, fixed ytop,
                       bool swap_axes, const patch_color_t* c0,
                       const patch_color_t* c1, int level)
{
	/* Assuming a very narrow trapezoid - ignore the transversal color
	 * variation. */
	...
	if(ytop - ybot <
	   fixed_1 / 2) /* Prevent an infinite color decomposition. */
		return constant_color_trapezoid(pfs, le, re, ybot, ytop,
		                                swap_axes, &c);
	else {
		...
		if(!pfs->unlinear && pfs->linear_color) {
			...
			if(dc[0].type == &gx_dc_type_data_pure) {
				...
				code = patch_color_to_device_color(pfs, c1,
				                                   &dc[1]);
				if(code < 0)
					return code;
				dc2fc(pfs, dc[1].colors.pure, fc[1]);
				code = dev_proc(pdev,
				                fill_linear_color_trapezoid)(
				    pdev, &fa, &le->start, &le->end, &re->start,
				    &re->end, fc[0], fc[1], NULL, NULL);
				if(code == 1) {
					pfs->monotonic_color =
					    monotonic_color_save;
					pfs->linear_color = linear_color_save;
					return 0; /* The area is filled. */
				}
				if(code < 0)
					return code;
				else /* code == 0, the device requested to
				        decompose the area. */
					return_error(
		...
		if(!pfs->unlinear || !pfs->linear_color ||
		   color_span(pfs, c0, c1) > pfs->smoothness) {
			...
			code = decompose_linear_color(
			    pfs, le, re, ybot, y, swap_axes, c0, &c, level + 1);
			if(code >= 0)
				code = decompose_linear_color(
				    pfs, le, re, y, ytop, swap_axes, &c, c1,
				    level + 1);
		} else
			code = constant_color_trapezoid(pfs, le, re, ybot, ytop,
			                                swap_axes, &c);
...
private
inline int
linear_color_trapezoid(patch_fill_state_t* pfs, gs_fixed_point q[4], int i0,
                       int i1, int i2, int i3, fixed ybot, fixed ytop,
                       bool swap_axes, const patch_color_t* c0,
                       const patch_color_t* c1, bool orient)
{
	/* Assuming a very narrow trapezoid - ignore the transversal color
	 * change. */
	gs_fixed_edge le, re;

	make_trapezoid(q, i0, i1, i2, i3, ybot, ytop, swap_axes, orient, &le,
	               &re);
	return decompose_linear_color(pfs, &le, &re, ybot, ytop, swap_axes, c0,
	                              c1, 0);
}

private
int
wedge_trap_decompose(patch_fill_state_t* pfs, gs_fixed_point q[4], fixed ybot,
                     fixed ytop, const patch_color_t* c0,
                     const patch_color_t* c1, bool swap_axes,
...
#if 1
	if(!swap_axes)
		vd_quad(q[0].x, q[0].y, q[1].x, q[1].y, q[3].x, q[3].y, q[2].x,
		        q[2].y, 0, RGB(255, 0, 0));
	else
		vd_quad(q[0].y, q[0].x, q[1].y, q[1].x, q[3].y, q[3].x, q[2].y,
		        q[2].x, 0, RGB(255, 0, 0));
#endif
	if((int64_t)dx1 * dy2 != (int64_t)dy1 * dx2) {
		orient = ((int64_t)dx1 * dy2 > (int64_t)dy1 * dx2);
		return linear_color_trapezoid(pfs, q, 0, 1, 2, 3, ybot, ytop,
		                              swap_axes, c0, c1, orient);
	} else {
...
private
inline int
fill_wedge_trap(patch_fill_state_t* pfs, const gs_fixed_point* p0,
                const gs_fixed_point* p1, const gs_fixed_point* q0,
                const gs_fixed_point* q1, const patch_color_t* c0,
                const patch_color_t* c1, bool swap_axes, bool self_intersecting)
{
	...
	p[0] = *q0;
	p[1] = *q1;
	return wedge_trap_decompose(pfs, p, p[2].y, p[3].y, cc0, cc1, swap_axes,
	                            self_intersecting);
}

private
void
split_curve_s(const gs_fixed_point* pole, gs_fixed_point* q0,
              gs_fixed_point* q1, int pole_step)
{
...
#define midpoint(a, b)                                                         \
	...
	q0[1 * pole_step].x =
	    midpoint(pole[0 * pole_step].x, pole[1 * pole_step].x);
	q0[1 * pole_step].y =
	    midpoint(pole[0 * pole_step].y, pole[1 * pole_step].y);
	q1[2 * pole_step].x =
	    midpoint(pole[2 * pole_step].x, pole[3 * pole_step].x);
	q1[2 * pole_step].y =
	    midpoint(pole[2 * pole_step].y, pole[3 * pole_step].y);
	...
	q0[3 * pole_step].x = q1[0 * pole_step].x =
	    midpoint(q0[2 * pole_step].x, q1[1 * pole_step].x);
	q0[3 * pole_step].y = q1[0 * pole_step].y =
	    midpoint(q0[2 * pole_step].y, q1[1 * pole_step].y);
...
private
void
split_curve(const gs_fixed_point pole[4], gs_fixed_point q0[4],
            gs_fixed_point q1[4])
{
...
make_wedge_median(patch_fill_state_t* pfs, wedge_vertex_list_t* l,
                  wedge_vertex_list_t* l0, bool forth, const gs_fixed_point* p0,
                  const gs_fixed_point* p1, const gs_fixed_point* pm)
{
...
private
int fill_wedge_from_list(patch_fill_state_t* pfs, const wedge_vertex_list_t* l,
...
private
inline int
fill_triangle_wedge_aux(patch_fill_state_t* pfs, const shading_vertex_t* q0,
                        const shading_vertex_t* q1, const shading_vertex_t* q2)
{
	int code;
	...
	if(p0->y < p1->y) {
		code = fill_wedge_trap(pfs, p0, p2, p0, p1, &q0->c, &q2->c,
		                       swap_axes, false);
		if(code < 0)
			return code;
		return fill_wedge_trap(pfs, p2, p1, p0, p1, &q2->c, &q1->c,
		                       swap_axes, false);
	} else {
		code = fill_wedge_trap(pfs, p0, p2, p1, p0, &q0->c, &q2->c,
		                       swap_axes, false);
		if(code < 0)
			return code;
		return fill_wedge_trap(pfs, p2, p1, p1, p0, &q2->c, &q1->c,
		                       swap_axes, false);
	}
}
                        ...
                        const shading_vertex_t* p2)
{
	...
	if(!wedge) {
		...
		s012 = max(s01, s2);
		code =
		    cs_is_linear(cs, pfs->pis, pfs->dev, &p0->c.cc, &p1->c.cc,
		                 &p2->c.cc, NULL, smoothness - s012);
		if(code < 0)
			return code;
		if(code == 0)
			return 1;
	}
	{
		...
		if(!VD_TRACE_DOWN)
			vd_disable;
		code = dev_proc(pdev, fill_linear_color_triangle)(
		    pdev, &fa, &p0->p, &p1->p, &p2->p, fc[0],
		    (wedge ? NULL : fc[1]), fc[2]);
		vd_restore;
		if(code == 1)
			return 0; /* The area is filled. */
		if(code < 0)
...
private
inline int
fill_triangle_wedge(patch_fill_state_t* pfs, const shading_vertex_t* q0,
                    const shading_vertex_t* q1, const shading_vertex_t* q2)
{
...
fill_triangle_wedge_from_list(patch_fill_state_t* pfs,
                              const wedge_vertex_list_elem_t* beg,
                              const wedge_vertex_list_elem_t* end,
...
private
int
fill_wedge_from_list_rec(patch_fill_state_t* pfs, wedge_vertex_list_elem_t* beg,
                         const wedge_vertex_list_elem_t* end, int level,
                         const patch_color_t* c0, const patch_color_t* c1)
{
	if(beg->next == end)
		...
		assert(beg->next->divide_count == 1 ||
		       beg->next->divide_count == 2);
		if(beg->next->divide_count != 1)
			return 0;
		return fill_triangle_wedge_from_list(pfs, beg, end, beg->next,
		                                     c0, c1);
	} else {
		gs_fixed_point p;
		wedge_vertex_list_elem_t* e;
...
fill_wedge_from_list(patch_fill_state_t* pfs, const wedge_vertex_list_t* l,
                     const patch_color_t* c0, const patch_color_t* c1)
{
	return fill_wedge_from_list_rec(
	    pfs, l->beg, l->end, max(l->beg->level, l->end->level), c0, c1);
}
...
wedge_by_triangles(patch_fill_state_t* pfs, int ka,
                   const gs_fixed_point pole[4], const patch_color_t* c0,
                   const patch_color_t* c1)
...
is_linear_color_applicable(const patch_fill_state_t* pfs)
{
	...
	if(pfs->dev->color_info.separable_and_linear != GX_CINFO_SEP_LIN)
		return false;
	if(gx_get_cmap_procs(pfs->pis, pfs->dev)
	       ->is_halftoned(pfs->pis, pfs->dev))
		return false;
	return true;
}

int
mesh_padding(patch_fill_state_t* pfs, const gs_fixed_point* p0,
             const gs_fixed_point* p1, const patch_color_t* c0,
             const patch_color_t* c1)
{
	...
	adjust_swapped_boundary(&re.start.x, swap_axes);
	adjust_swapped_boundary(&re.end.x, swap_axes);
	return decompose_linear_color(pfs, &le, &re, le.start.y, le.end.y,
	                              swap_axes, cc0, cc1, 0);
	/* fixme : for a better performance and quality, we would like to
	   consider the bar as an oriented one and to know at what side of it
	   the spot resides.
...
fill_wedges_aux(patch_fill_state_t* pfs, int k, int ka,
                const gs_fixed_point pole[4], const patch_color_t* c0,
                const patch_color_t* c1, int wedge_type)
{
	int code;
	...
	if(k > 1) {
		...
		split_curve(pole, q[0], q[1]);
		code =
		    fill_wedges_aux(pfs, k / 2, ka, q[0], c0, &c, wedge_type);
		if(code < 0)
			return code;
		...
		if(INTERPATCH_PADDING && (wedge_type & interpatch_padding)) {
			vd_bar(pole[0].x, pole[0].y, pole[3].x, pole[3].y, 0,
			       RGB(255, 0, 0));
...
private
int
fill_wedges(patch_fill_state_t* pfs, int k0, int k1, const gs_fixed_point* pole,
            int pole_step, const patch_color_t* c0, const patch_color_t* c1,
            int wedge_type)
{
...
private
int
ordered_triangle(patch_fill_state_t* pfs, gs_fixed_edge* le, gs_fixed_edge* re,
                 patch_color_t* c)
{
	gs_fixed_edge ue;
	int code;
	...
	if(le->end.y < re->end.y) {
		code = dev_proc(pfs->dev, fill_trapezoid)(
		    pfs->dev, le, re, le->start.y, le->end.y, false, &dc,
		    pfs->pis->log_op);
		if(code >= 0) {
			ue.start = le->end;
			ue.end = re->end;
			code = dev_proc(pfs->dev, fill_trapezoid)(
			    pfs->dev, &ue, re, le->end.y, re->end.y, false, &dc,
			    pfs->pis->log_op);
		}
	} else if(le->end.y > re->end.y) {
		code = dev_proc(pfs->dev, fill_trapezoid)(
		    pfs->dev, le, re, le->start.y, re->end.y, false, &dc,
		    pfs->pis->log_op);
		if(code >= 0) {
			ue.start = re->end;
			ue.end = le->end;
			code = dev_proc(pfs->dev, fill_trapezoid)(
			    pfs->dev, le, &ue, re->end.y, le->end.y, false, &dc,
			    pfs->pis->log_op);
		}
	} else
		code = dev_proc(pfs->dev, fill_trapezoid)(
...
private
int
constant_color_triangle(patch_fill_state_t* pfs, const shading_vertex_t* p0,
                        const shading_vertex_t* p1, const shading_vertex_t* p2)
{
	patch_color_t c, cc;
	gs_fixed_edge le, re;
...
private
int
constant_color_quadrangle(patch_fill_state_t* pfs, const quadrangle_patch* p,
                          bool self_intersecting)
{
	/* Assuming the XY span is restricted with curve_samples.
	   It is important for intersection_of_small_bars to compute faster. */
	...
	}
	{
		...
		if(g13 == h13) {
			...
			if(g23 != h23) {
				orient = (g23 > h23);
				if(q[2].y <= q[3].y) {
					if((code = gx_shade_trapezoid(
					        pfs, q, 1, 2, 0, 3, q[1].y,
					        q[2].y, swap_axes, &dc,
					        orient)) < 0)
						return code;
					return gx_shade_trapezoid(
					    pfs, q, 2, 3, 0, 3, q[2].y, q[3].y,
					    swap_axes, &dc, orient);
				} else {
					if((code = gx_shade_trapezoid(
					        pfs, q, 1, 2, 0, 3, q[1].y,
						...
						return code;
					return gx_shade_trapezoid(
					    pfs, q, 1, 2, 3, 2, q[3].y, q[2].y,
					    swap_axes, &dc, orient);
				}
			} else {
				int64_t g12 = (int64_t)dx1 * dy2,
				        h12 = (int64_t)dy1 * dx2;
	...
	if(q[1].y <= q[2].y && q[2].y <= q[3].y) {
		if(self_intersecting &&
		   intersection_of_small_bars(q, 0, 3, 1, 2, &ry, &ey)) {
			if((code = gx_shade_trapezoid(pfs, q, 0, 1, 0, 3,
			                              q[0].y, q[1].y, swap_axes,
			                              &dc, orient)) < 0)
				return code;
			if((code = gx_shade_trapezoid(
			        pfs, q, 0, 3, 1, 2, q[1].y, ry + ey, swap_axes,
			        &dc, orient)) < 0)
				return code;
			if((code = gx_shade_trapezoid(pfs, q, 1, 2, 0, 3, ry,
			                              q[2].y, swap_axes, &dc,
			                              orient)) < 0)
				return code;
			return gx_shade_trapezoid(pfs, q, 0, 3, 2, 3, q[2].y,
			                          q[3].y, swap_axes, &dc,
			                          orient);
		} else {
			if((code = gx_shade_trapezoid(pfs, q, 0, 1, 0, 3,
			                              ...
			                              &dc, orient)) < 0)
				return code;
			if((code = gx_shade_trapezoid(pfs, q, 1, 2, 0, 3,
			                              q[1].y, q[2].y, swap_axes,
			                              &dc, orient)) < 0)
				return code;
			return gx_shade_trapezoid(pfs, q, 2, 3, 0, 3, q[2].y,
			                          q[3].y, swap_axes, &dc,
			                          orient);
		}
		...
		if(self_intersecting &&
		   intersection_of_small_bars(q, 0, 3, 1, 2, &ry, &ey)) {
			if((code = gx_shade_trapezoid(pfs, q, 0, 1, 0, 3,
				...
				return code;
			if((code = gx_shade_trapezoid(
			        pfs, q, 1, 2, 0, 3, q[1].y, ry + ey, swap_axes,
			        &dc, orient)) < 0)
				return code;
			if((code = gx_shade_trapezoid(pfs, q, 0, 3, 1, 2, ry,
			                              q[3].y, swap_axes, &dc,
			                              orient)) < 0)
				return code;
			return gx_shade_trapezoid(pfs, q, 3, 2, 1, 2, q[3].y,
			                          q[2].y, swap_axes, &dc,
			                          orient);
		} else {
			if((code = gx_shade_trapezoid(pfs, q, 0, 1, 0, 3,
				...
				return code;
			if((code = gx_shade_trapezoid(pfs, q, 1, 2, 0, 3,
			                              q[1].y, q[3].y, swap_axes,
			                              &dc, orient)) < 0)
				return code;
			return gx_shade_trapezoid(pfs, q, 1, 2, 3, 2, q[3].y,
			                          q[2].y, swap_axes, &dc,
			                          orient);
		}
	} else if(q[2].y <= q[1].y && q[1].y <= q[3].y) {
		if(self_intersecting &&
		   intersection_of_small_bars(q, 0, 1, 2, 3, &ry, &ey)) {
			if((code = gx_shade_trapezoid(
			        pfs, q, 0, 1, 0, 3, q[0].y, ry + ey, swap_axes,
			        &dc, orient)) < 0)
				return code;
			if((code = gx_shade_trapezoid(
			        pfs, q, 2, 1, 2, 3, q[2].y, ry + ey, swap_axes,
			        &dc, orient)) < 0)
				return code;
			if((code = gx_shade_trapezoid(pfs, q, 2, 1, 0, 1, ry,
			                              q[1].y, swap_axes, &dc,
			                              orient)) < 0)
				return code;
			return gx_shade_trapezoid(pfs, q, 2, 3, 0, 3, ry,
			                          q[3].y, swap_axes, &dc,
			                          orient);
			...
			if((code = gx_shade_trapezoid(
			        ...
			        &dc, orient)) < 0)
				return code;
			if((code = gx_shade_trapezoid(pfs, q, 0, 1, 2, 1, ry,
			                              q[1].y, swap_axes, &dc,
			                              orient)) < 0)
				return code;
			return gx_shade_trapezoid(pfs, q, 0, 3, 2, 3, ry,
			                          q[3].y, swap_axes, &dc,
			                          orient);
		} else {
			if((code = gx_shade_trapezoid(pfs, q, 0, 1, 0, 3,
			                              ...
			                              &dc, orient)) < 0)
				return code;
			if((code = gx_shade_trapezoid(pfs, q, 2, 3, 2, 1,
			                              q[2].y, q[1].y, swap_axes,
			                              &dc, orient)) < 0)
				return code;
			return gx_shade_trapezoid(pfs, q, 2, 3, 0, 3, q[1].y,
			                          q[3].y, swap_axes, &dc,
			                          orient);
		}
		...
		if(self_intersecting &&
		   intersection_of_small_bars(q, 0, 1, 2, 3, &ry, &ey)) {
			...
			return gx_shade_trapezoid(pfs, q, 2, 3, 0, 3, ry,
		          ...
		          intersection_of_small_bars(q, 0, 3, 2, 1, &ry, &ey)) {
			/* Same code as someone above. */
			if((code = gx_shade_trapezoid(
			        pfs, q, 0, 1, 0, 3, q[0].y, ry + ey, swap_axes,
			        &dc, orient)) < 0)
			...
			if((code = gx_shade_trapezoid(pfs, q, 0, 1, 0, 3,
			                              q[0].y, q[2].y, swap_axes,
			                              &dc, orient)) < 0)
				return code;
			if((code = gx_shade_trapezoid(pfs, q, 2, 3, 0, 3,
			                              q[2].y, q[3].y, swap_axes,
			                              &dc, orient)) < 0)
				return code;
			return gx_shade_trapezoid(pfs, q, 0, 1, 2, 1, q[2].y,
			                          q[1].y, swap_axes, &dc,
			                          orient);
		}
	} else if(q[3].y <= q[1].y && q[1].y <= q[2].y) {
		if(self_intersecting &&
		   intersection_of_small_bars(q, 0, 1, 3, 2, &ry, &ey)) {
			if((code = gx_shade_trapezoid(pfs, q, 0, 1, 0, 3,
			                              q[0].y, q[3].y, swap_axes,
			                              &dc, orient)) < 0)
				return code;
			if((code = gx_shade_trapezoid(
			        pfs, q, 0, 1, 3, 2, q[3].y, ry + ey, swap_axes,
			        &dc, orient)) < 0)
				return code;
			if((code = gx_shade_trapezoid(pfs, q, 3, 2, 0, 1, ry,
			                              q[1].y, swap_axes, &dc,
			                              orient)) < 0)
				return code;
			return gx_shade_trapezoid(pfs, q, 3, 2, 1, 2, q[1].y,
			                          q[2].y, swap_axes, &dc,
			                          orient);
		} else {
			if((code = gx_shade_trapezoid(pfs, q, 0, 1, 0, 3,
			                              ...
			                              &dc, orient)) < 0)
				return code;
			if((code = gx_shade_trapezoid(pfs, q, 0, 1, 3, 2,
			                              q[3].y, q[1].y, swap_axes,
			                              &dc, orient)) < 0)
				return code;
			return gx_shade_trapezoid(pfs, q, 1, 2, 3, 2, q[1].y,
			                          q[2].y, swap_axes, &dc,
			                          orient);
		}
		...
		if(self_intersecting &&
		   intersection_of_small_bars(q, 0, 1, 2, 3, &ry, &ey)) {
			...
			if((code = gx_shade_trapezoid(pfs, q, 3, 2, 0, 1, ry,
			                              ...
			                              orient)) < 0)
				return code;
			return gx_shade_trapezoid(pfs, q, 2, 1, 0, 1, q[2].y,
			                          q[1].y, swap_axes, &dc,
			                          orient);
		} else {
			if((code = gx_shade_trapezoid(pfs, q, 0, 1, 0, 3,
...
private
inline void
divide_quadrangle_by_v(patch_fill_state_t* pfs, quadrangle_patch* s0,
                       quadrangle_patch* s1, shading_vertex_t q[2],
                       const quadrangle_patch* p)
{
	...
	patch_interpolate_color(&q[0].c, &p->p[0][0]->c, &p->p[1][0]->c, pfs,
	                        0.5);
	patch_interpolate_color(&q[1].c, &p->p[0][1]->c, &p->p[1][1]->c, pfs,
	                        0.5);
	s0->p[0][0] = p->p[0][0];
	s0->p[0][1] = p->p[0][1];
...
private
inline void
divide_quadrangle_by_u(patch_fill_state_t* pfs, quadrangle_patch* s0,
                       quadrangle_patch* s1, shading_vertex_t q[2],
                       const quadrangle_patch* p)
{
	...
	patch_interpolate_color(&q[0].c, &p->p[0][0]->c, &p->p[0][1]->c, pfs,
	                        0.5);
	patch_interpolate_color(&q[1].c, &p->p[1][0]->c, &p->p[1][1]->c, pfs,
	                        0.5);
	s0->p[0][0] = p->p[0][0];
	s0->p[1][0] = p->p[1][0];
...
private
inline int
is_quadrangle_color_monotonic(const patch_fill_state_t* pfs,
                              const quadrangle_patch* p,
                              bool* not_monotonic_by_u,
                              bool* not_monotonic_by_v)
...
private
inline void
divide_bar(patch_fill_state_t* pfs, const shading_vertex_t* p0,
           const shading_vertex_t* p1, int radix, shading_vertex_t* p)
{
	p->p.x = (fixed)((int64_t)p0->p.x * (radix - 1) + p1->p.x) / radix;
	p->p.y = (fixed)((int64_t)p0->p.y * (radix - 1) + p1->p.y) / radix;
	patch_interpolate_color(&p->c, &p0->c, &p1->c, pfs,
	                        (double)(radix - 1) / radix);
}

private
inline void
bbox_of_points(gs_fixed_rect* r, const gs_fixed_point* p0,
               const gs_fixed_point* p1, const gs_fixed_point* p2,
               const gs_fixed_point* p3)
{
	r->p.x = r->q.x = p0->x;
	r->p.y = r->q.y = p0->y;
...
private
int
triangle_by_4(patch_fill_state_t* pfs, const shading_vertex_t* p0,
              const shading_vertex_t* p1, const shading_vertex_t* p2,
              wedge_vertex_list_t* l01, wedge_vertex_list_t* l12,
              wedge_vertex_list_t* l20, double cd, fixed sd)
{
	...
	if(!pfs->inside) {
		if(r.p.x == r1.p.x && r.p.y == r1.p.y && r.q.x == r1.q.x &&
		   r.q.y == r1.q.y)
			pfs->inside = true;
	}
	...
	if(LAZY_WEDGES) {
		...
		make_wedge_median(pfs, &L20, l20, false, &p2->p, &p0->p,
		                  &p20.p);
	} else {
	...
	code = triangle_by_4(pfs, p0, &p01, &p20, &L01, &L[0], &L20, cd / 2,
	                     sd / 2);
	if(code < 0)
		return code;
	if(LAZY_WEDGES) {
	...
	code = triangle_by_4(pfs, p1, &p12, &p01, &L12, &L[1], &L01, cd / 2,
	                     sd / 2);
	if(code < 0)
		return code;
	if(LAZY_WEDGES)
		move_wedge(&L12, l12, true);
	code = triangle_by_4(pfs, p2, &p20, &p12, &L20, &L[2], &L12, cd / 2,
	                     sd / 2);
	if(code < 0)
		return code;
	L[0].last_side = L[1].last_side = L[2].last_side = true;
	code = triangle_by_4(pfs, &p01, &p12, &p20, &L[1], &L[2], &L[0], cd / 2,
	                     sd / 2);
	if(code < 0)
		return code;
	if(LAZY_WEDGES) {
...
private
inline int
fill_triangle(patch_fill_state_t* pfs, const shading_vertex_t* p0,
              const shading_vertex_t* p1, const shading_vertex_t* p2,
              wedge_vertex_list_t* l01, wedge_vertex_list_t* l12,
              wedge_vertex_list_t* l20)
{
	fixed sd01 =
	    max(any_abs(p1->p.x - p0->p.x), any_abs(p1->p.y - p0->p.y));
	fixed sd12 =
	    max(any_abs(p2->p.x - p1->p.x), any_abs(p2->p.y - p1->p.y));
	fixed sd20 =
	    max(any_abs(p0->p.x - p2->p.x), any_abs(p0->p.y - p2->p.y));
...
private
int
small_mesh_triangle(patch_fill_state_t* pfs, const shading_vertex_t* p0,
                    const shading_vertex_t* p1, const shading_vertex_t* p2)
{
	int code;
...
private
int
mesh_triangle_rec(patch_fill_state_t* pfs, const shading_vertex_t* p0,
                  const shading_vertex_t* p1, const shading_vertex_t* p2)
{
	pfs->unlinear = !is_linear_color_applicable(pfs);
...
int
mesh_triangle(patch_fill_state_t* pfs, const shading_vertex_t* p0,
              const shading_vertex_t* p1, const shading_vertex_t* p2)
{
	if((*dev_proc(pfs->dev, pattern_manage))(
	       pfs->dev, gs_no_id, NULL, pattern_manage__shading_area) > 0) {
		...
		if(code >= 0)
			code = (*dev_proc(pfs->dev, fill_path))(
			    pdev, NULL, &path, NULL, NULL, NULL);
...
private
inline int
triangles4(patch_fill_state_t* pfs, const quadrangle_patch* p,
           bool dummy_argument)
{
	...
	code = fill_triangle(pfs, p->p[0][0], p->p[0][1], &q, p->l0001, &l[0],
	                     &l[3]);
	if(code < 0)
		return code;
	l[0].last_side = true;
	l[3].last_side = true;
	code = fill_triangle(pfs, p->p[0][1], p->p[1][1], &q, p->l0111, &l[1],
	                     &l[0]);
	if(code < 0)
		return code;
	l[1].last_side = true;
	code = fill_triangle(pfs, p->p[1][1], p->p[1][0], &q, p->l1110, &l[2],
	                     &l[1]);
	if(code < 0)
		return code;
	l[2].last_side = true;
	code = fill_triangle(pfs, p->p[1][0], p->p[0][0], &q, p->l1000, &l[3],
	                     &l[2]);
	if(code < 0)
		return code;
...
private
inline int
triangles2(patch_fill_state_t* pfs, const quadrangle_patch* p,
           bool dummy_argument)
{
	...
	code = fill_triangle(pfs, p->p[0][0], p->p[0][1], p->p[1][1], p->l0001,
	                     p->l0111, &l);
	if(code < 0)
		return code;
	l.last_side = true;
	code = fill_triangle(pfs, p->p[1][1], p->p[1][0], p->p[0][0], p->l1110,
	                     p->l1000, &l);
	if(code < 0)
		return code;
	code = terminate_wedge_vertex_list(pfs, &l, &p->p[1][1]->c,
	                                   &p->p[0][0]->c);
	if(code < 0)
		return code;
	return 0;
...
private
inline int
is_quadrangle_color_linear_by_u(const patch_fill_state_t* pfs,
                                const quadrangle_patch* p)
{ /* returns : 1 = linear, 0 = unlinear, <0 = error. */
	int code;
...
private
inline int
is_quadrangle_color_linear_by_v(const patch_fill_state_t* pfs,
                                const quadrangle_patch* p)
{ /* returns : 1 = linear, 0 = unlinear, <0 = error. */
	int code;
...
private
inline int
is_quadrangle_color_linear_by_diagonals(const patch_fill_state_t* pfs,
                                        const quadrangle_patch* p)
{ /* returns : 1 = linear, 0 = unlinear, <0 = error. */
	int code;
...
private
inline color_change_type_t
quadrangle_color_change(const patch_fill_state_t* pfs,
                        const quadrangle_patch* p, bool is_big_u, bool is_big_v,
                ...
                int level)
{
	...
	if(!pfs->inside) {
		bbox_of_points(&r, &p->p[0][0]->p, &p->p[0][1]->p,
		               &p->p[1][0]->p, &p->p[1][1]->p);
		r1 = r;
		rect_intersect(r, pfs->rect);
		if(r.q.x <= r.p.x || r.q.y <= r.p.y)
	...
	if(big) {
		...
		if(QUADRANGLES && pfs->maybe_self_intersecting) {
			if(size_v > pfs->max_small_coord) {
				/* constant_color_quadrangle can't handle big
				   self-intersecting areas
	...
	if(!big1) {
		...
		if(d0001x > fixed_1 || d1011x > fixed_1 || d0001y > fixed_1 ||
		   d1011y > fixed_1)
			is_big_u = true;
		if(d0010x > fixed_1 || d0111x > fixed_1 || d0010y > fixed_1 ||
		   d0111y > fixed_1)
			is_big_v = true;
		else if(!is_big_u)
		...
		if(!pfs->monotonic_color) {
			bool not_monotonic_by_u = false,
			     not_monotonic_by_v = false;

			code = is_quadrangle_color_monotonic(
			    pfs, p, &not_monotonic_by_u, &not_monotonic_by_v);
			if(code < 0)
				return code;
			if(is_big_u)
		...
		if(pfs->monotonic_color && !pfs->linear_color) {
			if(divide_v && divide_u) {
				if(d0001x + d1011x + d0001y + d1011y >
				   d0010x + d0111x + d0010y + d0111y)
					divide_v = false;
				else
					divide_u = false;
			} else if(!divide_u && !divide_v && !pfs->unlinear) {
				if(is_big_u) {
					code = is_quadrangle_color_linear_by_u(
					    pfs, p);
					if(code < 0)
						return code;
					divide_u = !code;
				...
				if(is_big_v) {
					code = is_quadrangle_color_linear_by_v(
					    pfs, p);
					if(code < 0)
						return code;
					divide_v = !code;
				...
				if(is_big_u && is_big_v) {
					code =
					    is_quadrangle_color_linear_by_diagonals(
					        pfs, p);
					if(code < 0)
						return code;
					if(!code) {
						if(d0001x + d1011x + d0001y +
						       d1011y >
						   d0010x + d0111x + d0010y +
						       d0111y) {
							divide_u = true;
							divide_v = false;
						} else {
		...
		if(!pfs->linear_color) {
			...
			switch(quadrangle_color_change(
			    pfs, p, is_big_u, is_big_v, &divide_u, &divide_v)) {
			case color_change_small:
				code = (QUADRANGLES ||
				                !pfs->maybe_self_intersecting
				            ? constant_color_quadrangle
				            : triangles4)(
				    pfs, p, pfs->maybe_self_intersecting);
				pfs->monotonic_color = monotonic_color_save;
				pfs->linear_color = linear_color_save;
				return code;
	...
	if(divide_v) {
		...
		if(LAZY_WEDGES) {
			make_wedge_median(pfs, &l1, p->l0111, true,
			                  &p->p[0][1]->p, &p->p[1][1]->p,
			                  &s0.p[1][1]->p);
			make_wedge_median(pfs, &l2, p->l1000, false,
			                  &p->p[1][0]->p, &p->p[0][0]->p,
			                  &s0.p[1][0]->p);
			...
			code = fill_triangle_wedge(pfs, s0.p[0][0], s1.p[1][0],
			                           s0.p[1][0]);
			if(code < 0)
				return code;
			code = fill_triangle_wedge(pfs, s0.p[0][1], s1.p[1][1],
			                           s0.p[1][1]);
			if(code < 0)
				return code;
		}
		...
		if(LAZY_WEDGES) {
			if(code < 0)
				return code;
			code = close_wedge_median(pfs, p->l0111, &p->p[0][1]->c,
			                          &p->p[1][1]->c);
			if(code < 0)
				return code;
			code = close_wedge_median(pfs, p->l1000, &p->p[1][0]->c,
			                          &p->p[0][0]->c);
			if(code < 0)
				return code;
			code = terminate_wedge_vertex_list(
			    pfs, &l0, &s0.p[1][0]->c, &s0.p[1][1]->c);
		}
	} else if(divide_u) {
		divide_quadrangle_by_u(pfs, &s0, &s1, q, p);
		if(LAZY_WEDGES) {
			make_wedge_median(pfs, &l1, p->l0001, true,
			                  &p->p[0][0]->p, &p->p[0][1]->p,
			                  &s0.p[0][1]->p);
			make_wedge_median(pfs, &l2, p->l1110, false,
			                  &p->p[1][1]->p, &p->p[1][0]->p,
			                  &s0.p[1][1]->p);
			...
			code = fill_triangle_wedge(pfs, s0.p[0][0], s1.p[0][1],
			                           s0.p[0][1]);
			if(code < 0)
				return code;
			code = fill_triangle_wedge(pfs, s0.p[1][0], s1.p[1][1],
			                           s0.p[1][1]);
			if(code < 0)
				return code;
		}
		...
		if(LAZY_WEDGES) {
			if(code < 0)
				return code;
			code = close_wedge_median(pfs, p->l0001, &p->p[0][0]->c,
			                          &p->p[0][1]->c);
			if(code < 0)
				return code;
			code = close_wedge_median(pfs, p->l1110, &p->p[1][1]->c,
			                          &p->p[1][0]->c);
			if(code < 0)
				return code;
			code = terminate_wedge_vertex_list(
			    pfs, &l0, &s0.p[0][1]->c, &s0.p[1][1]->c);
		}
	} else
		code = (QUADRANGLES || !pfs->maybe_self_intersecting
		            ? constant_color_quadrangle
		            : triangles4)(pfs, p, pfs->maybe_self_intersecting);
	pfs->monotonic_color = monotonic_color_save;
	pfs->linear_color = linear_color_save;
	pfs->inside = inside_save;
	return code;
...
private
inline void
split_stripe(patch_fill_state_t* pfs, tensor_patch* s0, tensor_patch* s1,
             const tensor_patch* p)
{
	...
	s0->c[0][0] = p->c[0][0];
	s0->c[1][0] = p->c[1][0];
	patch_interpolate_color(&s0->c[0][1], &p->c[0][0], &p->c[0][1], pfs,
	                        0.5);
	patch_interpolate_color(&s0->c[1][1], &p->c[1][0], &p->c[1][1], pfs,
	                        0.5);
	s1->c[0][0] = s0->c[0][1];
	s1->c[1][0] = s0->c[1][1];
	s1->c[0][1] = p->c[0][1];
...
private
inline void
split_patch(patch_fill_state_t* pfs, tensor_patch* s0, tensor_patch* s1,
            const tensor_patch* p)
{
	...
	s0->c[0][0] = p->c[0][0];
	s0->c[0][1] = p->c[0][1];
	patch_interpolate_color(&s0->c[1][0], &p->c[0][0], &p->c[1][0], pfs,
	                        0.5);
	patch_interpolate_color(&s0->c[1][1], &p->c[0][1], &p->c[1][1], pfs,
	                        0.5);
	s1->c[0][0] = s0->c[1][0];
	s1->c[0][1] = s0->c[1][1];
	s1->c[1][0] = p->c[1][0];
...
decompose_stripe(patch_fill_state_t* pfs, const tensor_patch* p, int ku)
{
	if(ku > 1) {
		...
		if(LAZY_WEDGES) {
			code = terminate_wedge_vertex_list(
			    pfs, &l[0], &q.p[0][0]->c, &q.p[0][1]->c);
			if(code < 0)
				return code;
			code = terminate_wedge_vertex_list(
			    pfs, &l[1], &q.p[0][1]->c, &q.p[1][1]->c);
			if(code < 0)
				return code;
			code = terminate_wedge_vertex_list(
			    pfs, &l[2], &q.p[1][1]->c, &q.p[1][0]->c);
			if(code < 0)
				return code;
			code = terminate_wedge_vertex_list(
			    pfs, &l[3], &q.p[1][0]->c, &q.p[0][1]->c);
			if(code < 0)
				return code;
		}
		return code;
...
fill_stripe(patch_fill_state_t* pfs, const tensor_patch* p)
{
	...
	/* We would like to apply iterations for enumerating the kum curve
	   parts,
	...
	kum = max(ku[0], ku[3]);
	code = fill_wedges(pfs, ku[0], kum, p->pole[0], 1, &p->c[0][0],
	                   &p->c[0][1], inpatch_wedge);
	if(code < 0)
		return code;
	if(INTERPATCH_PADDING) {
		vd_bar(p->pole[0][0].x, p->pole[0][0].y, p->pole[3][0].x,
		       p->pole[3][0].y, 0, RGB(255, 0, 0));
		code = mesh_padding(pfs, &p->pole[0][0], &p->pole[3][0],
		                    &p->c[0][0], &p->c[1][0]);
		if(code < 0)
			return code;
		vd_bar(p->pole[0][3].x, p->pole[0][3].y, p->pole[3][3].x,
		       p->pole[3][3].y, 0, RGB(255, 0, 0));
		code = mesh_padding(pfs, &p->pole[0][3], &p->pole[3][3],
		                    &p->c[0][1], &p->c[1][1]);
		if(code < 0)
			return code;
	}
	...
	if(code < 0)
		return code;
	return fill_wedges(pfs, ku[3], kum, p->pole[3], 1, &p->c[1][0],
	                   &p->c[1][1], inpatch_wedge);
}
...
private
inline int
vector_pair_orientation(const gs_fixed_point* p0, const gs_fixed_point* p1,
...
{
	int sign = vector_pair_orientation(&p->pole[0][0], &p->pole[0][1],
	                                   &p->pole[1][0]);

	if(neqs(&sign, vector_pair_orientation(&p->pole[0][1], &p->pole[0][2],
	                                       &p->pole[1][1])))
		return true;
	if(neqs(&sign, vector_pair_orientation(&p->pole[0][2], &p->pole[0][3],
	                                       &p->pole[1][2])))
		return true;
	if(neqs(&sign, -vector_pair_orientation(&p->pole[0][3], &p->pole[0][2],
	                                        &p->pole[1][3])))
		return true;

	if(neqs(&sign, vector_pair_orientation(&p->pole[1][1], &p->pole[1][2],
	                                       &p->pole[2][1])))
		return true;
	if(neqs(&sign, vector_pair_orientation(&p->pole[1][1], &p->pole[1][2],
	                                       &p->pole[2][1])))
		return true;
	if(neqs(&sign, vector_pair_orientation(&p->pole[1][2], &p->pole[1][3],
	                                       &p->pole[2][2])))
		return true;
	if(neqs(&sign, -vector_pair_orientation(&p->pole[1][3], &p->pole[1][2],
	                                        &p->pole[2][3])))
		return true;

	if(neqs(&sign, vector_pair_orientation(&p->pole[2][1], &p->pole[2][2],
	                                       &p->pole[3][1])))
		return true;
	if(neqs(&sign, vector_pair_orientation(&p->pole[2][1], &p->pole[2][2],
	                                       &p->pole[3][1])))
		return true;
	if(neqs(&sign, vector_pair_orientation(&p->pole[2][2], &p->pole[2][3],
	                                       &p->pole[3][2])))
		return true;
	if(neqs(&sign, -vector_pair_orientation(&p->pole[2][3], &p->pole[2][2],
	                                        &p->pole[3][3])))
		return true;

	if(neqs(&sign, -vector_pair_orientation(&p->pole[3][1], &p->pole[3][2],
	                                        &p->pole[2][1])))
		return true;
	if(neqs(&sign, -vector_pair_orientation(&p->pole[3][1], &p->pole[3][2],
	                                        &p->pole[2][1])))
		return true;
	if(neqs(&sign, -vector_pair_orientation(&p->pole[3][2], &p->pole[3][3],
	                                        &p->pole[2][2])))
		return true;
	if(neqs(&sign, vector_pair_orientation(&p->pole[3][3], &p->pole[3][2],
	                                       &p->pole[2][3])))
		return true;
	return false;
}
...
{
	int sign = vector_pair_orientation(&p->pole[0][0], &p->pole[1][0],
	                                   &p->pole[0][1]);

	if(neqs(&sign, vector_pair_orientation(&p->pole[1][0], &p->pole[2][0],
	                                       &p->pole[1][1])))
		return true;
	if(neqs(&sign, vector_pair_orientation(&p->pole[2][0], &p->pole[3][0],
	                                       &p->pole[2][1])))
		return true;
	if(neqs(&sign, -vector_pair_orientation(&p->pole[3][0], &p->pole[2][0],
	                                        &p->pole[3][1])))
		return true;

	if(neqs(&sign, vector_pair_orientation(&p->pole[1][1], &p->pole[2][1],
	                                       &p->pole[1][2])))
		return true;
	if(neqs(&sign, vector_pair_orientation(&p->pole[1][1], &p->pole[2][1],
	                                       &p->pole[1][2])))
		return true;
	if(neqs(&sign, vector_pair_orientation(&p->pole[2][1], &p->pole[3][1],
	                                       &p->pole[2][2])))
		return true;
	if(neqs(&sign, -vector_pair_orientation(&p->pole[3][1], &p->pole[2][1],
	                                        &p->pole[3][2])))
		return true;

	if(neqs(&sign, vector_pair_orientation(&p->pole[1][2], &p->pole[2][2],
	                                       &p->pole[1][3])))
		return true;
	if(neqs(&sign, vector_pair_orientation(&p->pole[1][2], &p->pole[2][2],
	                                       &p->pole[1][3])))
		return true;
	if(neqs(&sign, vector_pair_orientation(&p->pole[2][2], &p->pole[3][2],
	                                       &p->pole[2][3])))
		return true;
	if(neqs(&sign, -vector_pair_orientation(&p->pole[3][2], &p->pole[2][2],
	                                        &p->pole[3][3])))
		return true;

	if(neqs(&sign, -vector_pair_orientation(&p->pole[1][3], &p->pole[2][3],
	                                        &p->pole[1][2])))
		return true;
	if(neqs(&sign, -vector_pair_orientation(&p->pole[1][3], &p->pole[2][3],
	                                        &p->pole[1][2])))
		return true;
	if(neqs(&sign, -vector_pair_orientation(&p->pole[2][3], &p->pole[3][3],
	                                        &p->pole[2][2])))
		return true;
	if(neqs(&sign, vector_pair_orientation(&p->pole[3][3], &p->pole[2][3],
	                                       &p->pole[3][2])))
		return true;
	return false;
}
...
private
int
fill_patch(patch_fill_state_t* pfs, const tensor_patch* p, int kv, int kv0,
           int kv1)
{
	if(kv <= 1) {
...
private
void
patch_set_color(const patch_fill_state_t* pfs, patch_color_t* c,
                const float* cc)
{
	if(pfs->Function) {
		c->t[0] = cc[0];
		...
		memcpy(c->cc.paint.values, cc,
		       sizeof(c->cc.paint.values[0]) * pfs->num_components);
}

private
void
make_tensor_patch(const patch_fill_state_t* pfs, tensor_patch* p,
                  const patch_curve_t curve[4],
                    ...
                    const gx_device_color* pdevc, gs_logical_operation_t log_op)
{
	...
	return dev_proc(pdev, fill_trapezoid)(pdev, &le, &re, le.start.y,
	                                      le.end.y, false, pdevc, log_op);
}
...
/*if (dbg_patch_cnt != 67 && dbg_patch_cnt != 78)
	...
	if((*dev_proc(pfs->dev, pattern_manage))(
		...
		int s = (s1 + s2 > 0 ? 1 : 3), i, j, k, jj,
		    l = (s == 1 ? 0 : 1);

		gx_path_init_local(&path, pdev->memory);
		if(is_x_bended(&p) || is_y_bended(&p)) {
			/* The patch possibly is self-overlapping,
			   so the patch coverage may fall outside the patch
			   outline.
			   In this case we pass an empty path,
			   and the device must use a bitmap mask instead
			   clipping. */
		} else {
			code = gx_path_add_point(&path, curve[0].vertex.p.x,
			                         curve[0].vertex.p.y);
			for(i = k = 0; k < 4 && code >= 0; i = j, k++) {
				j = (i + s) % 4, jj = (s == 1 ? i : j);
				if(curve[jj].straight)
					code = gx_path_add_line(
					    &path, curve[j].vertex.p.x,
					    curve[j].vertex.p.y);
				else
					code = gx_path_add_curve(
					    &path, curve[jj].control[l].x,
					    curve[jj].control[l].y,
					    curve[jj].control[(l + 1) & 1].x,
					    curve[jj].control[(l + 1) & 1].y,
					    curve[j].vertex.p.x,
					    curve[j].vertex.p.y);
			}
			if(code >= 0)
...
#endif
	code = fill_wedges(pfs, ku[0], kum, p.pole[0], 1, &p.c[0][0],
	                   &p.c[0][1], interpatch_padding | inpatch_wedge);
	if(code >= 0) {
/* We would like to apply iterations for enumerating the kvm curve parts,
   ...
   the rounding errors do not depend on the direction. */
	...
	if(code >= 0)
		code =
		    fill_wedges(pfs, ku[3], kum, p.pole[3], 1, &p.c[1][0],
		                &p.c[1][1], interpatch_padding | inpatch_wedge);
	return code;
}
