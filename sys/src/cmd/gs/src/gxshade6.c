...
#define COMPUTE_COORD(xy)                                                              \
	pt->xy = (fixed)((co_v * c1u.xy + v * c2u.xy) + (co_u * d1v.xy + u * d2v.xy) - \
...
fill_quadrangle(patch_fill_state_t *pfs, const quadrangle_patch *p, bool big, int level)
{
	...
	if(!big1) {
		...
		if(d0010x > fixed_1 || d0111x > fixed_1 || d0010y > fixed_1 || d0111y > fixed_1)
			...
			return (QUADRANGLES || !pfs->maybe_self_intersecting ? constant_color_quadrangle : triangles4)(pfs, p,
		...
		if(!pfs->linear_color) {
			...
			switch(quadrangle_color_change(pfs, p, is_big_u, is_big_v, &divide_u, &divide_v)) {
			case color_change_small:
				code = (QUADRANGLES || !pfs->maybe_self_intersecting ? constant_color_quadrangle : triangles4)(pfs, p,
															       pfs->maybe_self_intersecting);
				pfs->monotonic_color = monotonic_color_save;
				pfs->linear_color = linear_color_save;
				return code;
