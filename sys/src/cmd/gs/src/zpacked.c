...
/* Copyright (C) 1990, 1992, 1993, 1999 Aladdin Enterprises.  All rights
  reserved.
                  ...
                  gs_dual_memory_t* idmemory, client_name_t cname)
{
	...
	code = gs_alloc_ref_array(imem, &rtemp, 0,
	                          (idest + pad) / packed_per_ref, cname);
	if(code < 0)
		return code;
	...
	for(i = size; i != 0; i--) {
		...
		case t_name: {
			...
			if(nidx >= packed_name_max_index)
				break; /* can't pack */
			*pdest++ = nidx + (r_has_attr(pref, a_executable)
		...
		/* 'skip' already ensures that we don't need to do this. */
		{
			...
			ref* pmove = (ref*)(pdest += (packed_per_ref - 1) * i);
...

	{
		...
		make_tasv_new(parr, atype, a_readonly | space, size, packed,
		              pbody + skip);
	}
	return 0;
}
...
const op_def zpacked_op_defs[] = {{"0currentpacking", zcurrentpacking},
