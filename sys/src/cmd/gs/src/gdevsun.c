...
/* Copyright (C) 1989, 1992, 1994, 1996 Aladdin Enterprises.  All rights
  reserved.
...
private
gx_device_procs sun_procs = {sun_open, NULL, /* get_initial_matrix */
                             sun_sync, NULL, /* output_page */
                             sun_close, sun_map_rgb_color, sun_map_color_rgb,
                             sun_fill_rectangle, NULL, /* tile_rectangle */
                             sun_copy_mono, sun_copy_color, sun_draw_line};
...
gx_device_sun far_data gs_sunview_device = {
    std_device_std_body(gx_device_sun, &sun_procs, "sunview",
                        (int)(PAPER_X* DEFAULT_DPI),
                        (int)(PAPER_Y* DEFAULT_DPI), /* x and y extent */
...
#define RED_MASK                                                               \
	((1 << (BLUE_BITS + GREEN_BITS + RED_BITS)) - 1 - BLUE_MASK -          \
	 GREEN_MASK)
/*
...
#define CMS_SIZE                                                               \
	ALL_COLS /* cut down to 64 or 128 for                                  \
...
#define TRUE_GREEN_MASK                                                        \
	((1 << (TRUE_RED_BITS + TRUE_GREEN_BITS)) - 1 - TRUE_RED_MASK)
#define TRUE_BLUE_MASK                                                         \
	((1 << (TRUE_RED_BITS + TRUE_GREEN_BITS + TRUE_BLUE_BITS)) - 1 -       \
...
sun_open(register gx_device* dev)
{
	...
	if(xdev->frame == (Frame)0)
		xdev->frame = window_create(
		    NULL, FRAME, FRAME_LABEL, gs_product, WIN_WIDTH,
		    min(xdev->width + 24, 900), WIN_HEIGHT,
		    min(xdev->height + 36, 900), WIN_Y, 0, WIN_X, 200, 0);
	if(xdev->frame == (Frame)0)
		return -1;
	xdev->canvas = window_create(
	    xdev->frame, CANVAS, CANVAS_AUTO_EXPAND, FALSE, CANVAS_AUTO_SHRINK,
	    FALSE, CANVAS_WIDTH, xdev->width, CANVAS_HEIGHT, xdev->height,
	...
	switch(xdev->pw->pw_pixrect->pr_depth) {
		static gx_device_color_info mono_ci = dci_black_and_white;
		/*
		 * If the pre-allocated color cube leaves room for spare
		 * entries,
	   ...
	   && !xdev->truecolor
		...
		for(j = 0; j < ALL_COLS; j++) {
			xdev->blue[j] =
			    ...
			    (TRUE_BLUE_COLS - 1) * (ALL_COLS - 1);
			xdev->green[j] =
			    (double)((j & TRUE_GREEN_MASK) >> TRUE_RED_BITS) /
			    (TRUE_GREEN_COLS - 1) * (ALL_COLS - 1);
			xdev->red[j] = (double)((j & TRUE_RED_MASK)) /
			               (TRUE_RED_COLS - 1) * (ALL_COLS - 1);
		}
		 ...
		 */
		j = CMS_SIZE - 2;
		cms_monochromeload(xdev->red + j, xdev->green + j,
		                   xdev->blue + j);
		...
		for(color = 1; color < RGB_COLS - 1; color++) {
			j--;
			xdev->red[j] = (double)((color & RED_MASK) >>
			                        (GREEN_BITS + BLUE_BITS)) /
			               (RED_COLS - 1) * (ALL_COLS - 1);
			xdev->green[j] =
			    (double)((color & GREEN_MASK) >> BLUE_BITS) /
			    (GREEN_COLS - 1) * (ALL_COLS - 1);
			xdev->blue[j] = (double)((color & BLUE_MASK)) /
			                (BLUE_COLS - 1) * (ALL_COLS - 1);
		}
		...
		pw_putcolormap(xdev->pw, 0, CMS_SIZE, xdev->red, xdev->green,
		               xdev->blue);
	} else {
		xdev->freecols = 0;
		xdev->red = (byte*)0;
	 ...
	 */
	window_set(xdev->canvas, CANVAS_RETAINED, TRUE, WIN_VERTICAL_SCROLLBAR,
 ...
 */
gx_color_index
sun_map_rgb_color(gx_device* dev, unsigned short red, unsigned short green,
                  unsigned short blue)
{
	if(!xdev->frame || !gx_device_has_color(dev))
		...
		if(xdev->freecols <= 1) {
			if(warn) {
				eprintf("gs: last spare color map entry "
				        "allocated\n");
				warn = 0;
			}
		...
		xdev->blue[i] = blue_val;
		pw_setcmsname(xdev->pw, xdev->cmsname);
		pw_putcolormap(xdev->pw, i, 1, &xdev->red[i], &xdev->green[i],
		               &xdev->blue[i]);
...
int
sun_map_color_rgb(gx_device* dev, gx_color_index color, unsigned short rgb[3])
{
	if(!xdev->frame || !gx_device_has_color(dev))
		...
		if(color < xdev->freecols || color >= CMS_SIZE) {
			eprintf1("gs: attempt to get RGB values for "
			         "unallocated color index %d\n",
			         (int)color);
			return -1;
		}
		...
		rgb[0] = (double)((unsigned short)(color & TRUE_RED_MASK)) /
		         (TRUE_RED_COLS - 1) * gx_max_color_value;
		rgb[1] = (double)((unsigned short)(color & TRUE_GREEN_MASK) >>
		                  TRUE_RED_BITS) /
		         (TRUE_GREEN_COLS - 1) * gx_max_color_value;
		rgb[2] = (double)((unsigned short)(color & TRUE_BLUE_MASK) >>
		                  (TRUE_GREEN_BITS + TRUE_RED_BITS)) /
		         (TRUE_BLUE_COLS - 1) * gx_max_color_value;
		return 0;
	}
}
...
int
sun_fill_rectangle(register gx_device* dev, int x, int y, int w, int h,
...
int
sun_copy_mono(register gx_device* dev, const byte* base, int sourcex,
...
#define BUF_WIDTH_BYTES (((int)(8.5 * DEFAULT_DPI) + 15) / 16 * 2)
	...
	if(one != gx_no_color_index) {
		pw_stencil(xdev->pw, x, y, w, h, PIX_SRC | PIX_COLOR(one),
		           &(xdev->pr), ((int)base & 1) ? sourcex + 8 : sourcex,
		           0, (Pixrect*)0, 0, 0);
	}
	if(zero != gx_no_color_index) {
		for(i = 0; i < nbytes; i++)
			non_const_base[i] = ~base[i];
		pw_stencil(xdev->pw, x, y, w, h, PIX_SRC | PIX_COLOR(zero),
		           &(xdev->pr), ((int)base & 1) ? sourcex + 8 : sourcex,
		           0, (Pixrect*)0, 0, 0);
		for(i = 0; i < nbytes; i++)
			non_const_base[i] = ~base[i];
...
/* Copy a color bitmap. */
int
sun_copy_color(register gx_device* dev, const byte* base, int sourcex,
               int raster, gx_bitmap_id id, int x, int y, int w, int h)
{
	extern struct pixrectops mem_ops;

	if(!gx_device_has_color(dev))
		return sun_copy_mono(dev, base, sourcex, raster, id, x, y, w, h,
	...
	xdev->mpr.md_linebytes = raster;
	xdev->mpr.md_image = (int16_t*)((uint32_t)base & ~1);
	pw_write(xdev->pw, x, y, w, h, PIX_SRC, &(xdev->pr),
...
/* Draw a line */
int
sun_draw_line(register gx_device* dev, int x0, int y0, int x1, int y1,
              gx_color_index color)
{
