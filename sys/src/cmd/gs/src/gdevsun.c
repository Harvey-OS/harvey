...
#define TRUE_GREEN_MASK ((1 << (TRUE_RED_BITS + TRUE_GREEN_BITS)) - 1 - TRUE_RED_MASK)
#define TRUE_BLUE_MASK ((1 << (TRUE_RED_BITS + TRUE_GREEN_BITS + TRUE_BLUE_BITS)) - 1 - TRUE_GREEN_MASK - TRUE_RED_MASK)
/*
 * number of colors on rgb dimensions
 */
	...
	if(gx_device_has_color(xdev)
	   ...
	   && !xdev->truecolor
		...
		for(j = 0; j < ALL_COLS; j++) {
			xdev->blue[j] =
			    (double)((j & TRUE_BLUE_MASK) >> (TRUE_GREEN_BITS + TRUE_RED_BITS)) / (TRUE_BLUE_COLS - 1) * (ALL_COLS - 1);
			xdev->green[j] =
			    (double)((j & TRUE_GREEN_MASK) >> TRUE_RED_BITS) / (TRUE_GREEN_COLS - 1) * (ALL_COLS - 1);
			xdev->red[j] =
			    (double)((j & TRUE_RED_MASK)) / (TRUE_RED_COLS - 1) * (ALL_COLS - 1);
		}
		...
		for(color = 1; color < RGB_COLS - 1; color++) {
			j--;
			xdev->red[j] =
			    (double)((color & RED_MASK) >> (GREEN_BITS + BLUE_BITS)) / (RED_COLS - 1) * (ALL_COLS - 1);
			xdev->green[j] =
			    (double)((color & GREEN_MASK) >> BLUE_BITS) / (GREEN_COLS - 1) * (ALL_COLS - 1);
			xdev->blue[j] =
			    (double)((color & BLUE_MASK)) / (BLUE_COLS - 1) * (ALL_COLS - 1);
		}
		  ...
		  unsigned short rgb[3])
{
	if(!xdev->frame || !gx_device_has_color(dev))
		...
		rgb[0] = (double)xdev->red[color] / (ALL_COLS - 1) * gx_max_color_value;
		rgb[1] = (double)xdev->green[color] / (ALL_COLS - 1) * gx_max_color_value;
		rgb[2] = (double)xdev->blue[color] / (ALL_COLS - 1) * gx_max_color_value;
		return 0;
	} else { /* true color mapping */
		rgb[0] = (double)((unsigned short)(color & TRUE_RED_MASK)) / (TRUE_RED_COLS - 1) * gx_max_color_value;
		rgb[1] = (double)((unsigned short)(color & TRUE_GREEN_MASK) >> TRUE_RED_BITS) / (TRUE_GREEN_COLS - 1) * gx_max_color_value;
		rgb[2] = (double)((unsigned short)(color & TRUE_BLUE_MASK) >> (TRUE_GREEN_BITS + TRUE_RED_BITS)) / (TRUE_BLUE_COLS - 1) * gx_max_color_value;
		return 0;
	}
}
