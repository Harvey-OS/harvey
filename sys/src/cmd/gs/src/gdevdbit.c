			  ...
			  int px, int py)
{
	...
	*(gx_tile_bitmap *)&tiles = *tile;
	...
	return (*dev_proc(dev, strip_tile_rectangle))(dev, &tiles, x, y, w, h, color0, color1, px, py);
}
		     ...
		     gx_color_index zero, gx_color_index one)
{
	...
	if(one != gx_no_color_index) {
		...
		if(zero != gx_no_color_index) {
			int code = (*dev_proc(dev, fill_rectangle))(dev, x, y, w, h, zero);
		     ...
		     gs_logical_operation_t lop, const gx_clip_path *pcpath)
{
	...
	if(depth > 1) {
		...
		return (*dev_proc(dev, copy_alpha))(dev, data, dx, raster, id, x, y, w, h,
...
{ /* Fill the rectangle in chunks. */
	...
	if(dev_proc(dev, tile_rectangle) != gx_default_tile_rectangle) {
		if(shift == 0) { /*
			...
			code = (*tile_proc)(dev, (const gx_tile_bitmap *)tiles, x, y, w, h,
					    color0, color1, px, py);
	...
	{ /*
				 ...
				 */
		int xoff =
		    (shift == 0 ? px : px + (y + py) / rheight * tiles->rep_shift);
...
#define real_copy_tile(srcx, tx, ty, tw, th, id)                                                                                                                         \
	code = (proc_color != 0 ? (*proc_color)(dev, row, srcx, raster, id, tx, ty, tw, th) : (*proc_mono)(dev, row, srcx, raster, id, tx, ty, tw, th, color0, color1)); \
	if(code < 0)                                                                                                                                                     \
		return_error(code);                                                                                                                                      \
