...
int
gx_default_tile_rectangle(gx_device* dev, const gx_tile_bitmap* tile, int x,
                          ...
                          gx_color_index color1, int px, int py)
{
	...
	*(gx_tile_bitmap*)&tiles = *tile;
	...
	return (*dev_proc(dev, strip_tile_rectangle))(dev, &tiles, x, y, w, h,
	                                              color0, color1, px, py);
}
...
int
gx_default_copy_mono(gx_device* dev, const byte* data, int dx, int raster,
                     ...
                     gx_color_index zero, gx_color_index one)
{
	...
	if(one != gx_no_color_index) {
		...
		if(zero != gx_no_color_index) {
			int code = (*dev_proc(dev, fill_rectangle))(dev, x, y,
			                                            w, h, zero);
	...
	return gx_dc_default_fill_masked(&devc, data, dx, raster, id, x, y, w,
	                                 h, dev, rop3_T, invert);
}
...
/* This is very inefficient, but it works as a default. */
int
gx_default_copy_color(gx_device* dev, const byte* data, int dx, int raster,
                      gx_bitmap_id id, int x, int y, int w, int h)
{
	...
	if(depth == 1)
		return (*dev_proc(dev, copy_mono))(dev, data, dx, raster, id, x,
		                                   y, w, h, (gx_color_index)0,
		                                   (gx_color_index)1);
	fit_copy(dev, data, dx, raster, id, x, y, w, h);
...
int
gx_no_copy_alpha(gx_device* dev, const byte* data, int data_x, int raster,
...
int
gx_default_copy_alpha(gx_device* dev, const byte* data, int data_x, int raster,
                      gx_bitmap_id id, int x, int y, int width, int height,
                      gx_color_index color, int depth)
...
int
gx_no_copy_rop(gx_device* dev, const byte* sdata, int sourcex, uint sraster,
...
int
gx_default_fill_mask(gx_device* orig_dev, const byte* data, int dx, int raster,
                     gx_bitmap_id id, int x, int y, int w, int h,
...
{ /* Fill the rectangle in chunks. */
	...
	if(gs_debug_c('t')) {
		...
		dlprintf4("[t]tile %dx%d raster=%d id=%lu;", tiles->size.x,
		          tiles->size.y, tiles->raster, tiles->id);
	...
	if(dev_proc(dev, tile_rectangle) != gx_default_tile_rectangle) {
		if(shift ==
		   0) { /*
		         ...
		         * device has a tile_rectangle procedure that
		         * conditionally
			...
			set_dev_proc(dev, tile_rectangle,
			             gx_default_tile_rectangle);
			code = (*tile_proc)(dev, (const gx_tile_bitmap*)tiles,
			                    x, y, w, h, color0, color1, px, py);
...
#define real_copy_tile(srcx, tx, ty, tw, th, id)                               \
	code = (proc_color != 0 ? (*proc_color)(dev, row, srcx, raster, id,    \
	                                        tx, ty, tw, th)                \
	                        : (*proc_mono)(dev, row, srcx, raster, id, tx, \
	                                       ty, tw, th, color0, color1));   \
	if(code < 0)                                                           \
		return_error(code);                                            \
...
#define copy_tile(srcx, tx, ty, tw, th, id)                                    \
	real_copy_tile(srcx, tx, ty, tw, th, id)
		...
		if(ch >= h) { /* Shallow operation */
			if(icw >=
			   w) { /* Just one (partial) tile to transfer. */
				...
				int cx = x + icw;
				uint32_t id =
				    (h == height ? tile_id : gs_no_bitmap_id);
			...
			for(cy = y;;) {
				uint32_t id =
				    (ch == height ? tile_id : gs_no_bitmap_id);
...
int
gx_no_strip_copy_rop(gx_device* dev, const byte* sdata, int sourcex,
 ...
 */
int
gx_copy_mono_unaligned(gx_device* dev, const byte* data, int dx, int raster,
                       gx_bitmap_id id, int x, int y, int w, int h,
                       gx_color_index zero, gx_color_index one)
{
	...
	if(!step) { /* No adjustment needed. */
		return (*copy_mono)(dev, data, dx, raster, id, x, y, w, h, zero,
		                    one);
	}
...
int
gx_copy_color_unaligned(gx_device* dev, const byte* data, int data_x,
                        ...
                        int height)
{
	...
	/* Adjust the raster. */
	if(!step) { /* No adjustment needed. */
		return (*copy_color)(dev, data, data_x, raster, id, x, y, width,
		                     height);
	}
	/* Do the transfer one scan line at a time. */
	{
                        ...
                        int height, gx_color_index color, int depth)
{
	...
	/* Adjust the raster. */
	if(!step) { /* No adjustment needed. */
		return (*copy_alpha)(dev, data, data_x, raster, id, x, y, width,
		                     height, color, depth);
	}
	/* Do the transfer one scan line at a time. */
	{
