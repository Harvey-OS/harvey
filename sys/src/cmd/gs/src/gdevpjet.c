...
private
dev_proc_print_page(lj250_print_page);
private
dev_proc_print_page(paintjet_print_page);
private
dev_proc_print_page(pjetxl_print_page);
private
int pj_common_print_page(gx_device_printer*, FILE*, int, const char*);
private
gx_device_procs paintjet_procs =
    prn_color_procs(gdev_prn_open, gdev_prn_output_page, gdev_prn_close,
                    gdev_pcl_3bit_map_rgb_color, gdev_pcl_3bit_map_color_rgb);
const gx_device_printer far_data gs_lj250_device =
    prn_device(paintjet_procs, "lj250", 85,    /* width_10ths, 8.5" */
               110,                            /* height_10ths, 11" */
               X_DPI, Y_DPI, 0.25, 0, 0.25, 0, /* margins */
               3, lj250_print_page);
const gx_device_printer far_data gs_paintjet_device =
    prn_device(paintjet_procs, "paintjet", 85, /* width_10ths, 8.5" */
               110,                            /* height_10ths, 11" */
               X_DPI, Y_DPI, 0.25, 0, 0.25, 0, /* margins */
               3, paintjet_print_page);
private
gx_device_procs pjetxl_procs =
    prn_color_procs(gdev_prn_open, gdev_prn_output_page, gdev_prn_close,
                    gdev_pcl_3bit_map_rgb_color, gdev_pcl_3bit_map_color_rgb);
const gx_device_printer far_data gs_pjetxl_device =
    prn_device(pjetxl_procs, "pjetxl", 85,  /* width_10ths, 8.5" */
               110,                         /* height_10ths, 11" */
               X_DPI, Y_DPI, 0.25, 0, 0, 0, /* margins */
               3, pjetxl_print_page);

/* Forward references */
private
int compress1_row(const byte*, const byte*, byte*);
...
#define DATA_SIZE (LINE_SIZE * 8)
	byte* data = (byte*)gs_malloc(pdev->memory, DATA_SIZE, 1,
	                              "paintjet_print_page(data)");
	byte* plane_data = (byte*)gs_malloc(pdev->memory, LINE_SIZE * 3, 1,
	...
	if(data == 0 || plane_data == 0) {
		if(data)
			gs_free(pdev->memory, (char*)data, DATA_SIZE, 1,
			        "paintjet_print_page(data)");
		if(plane_data)
			gs_free(pdev->memory, (char*)plane_data, LINE_SIZE * 3,
			        1, "paintjet_print_page(plane_data)");
	...
	/* Send each scan line in turn */
	{
		...
		for(lnum = 0; lnum < pdev->height; lnum++) {
			...
			gdev_prn_copy_scan_lines(pdev, lnum, (byte*)data,
			                         line_size);
			/* Remove trailing 0s. */
		...
		0, c, c * 0x100, c * 0x101, c * 0x10000L, c * 0x10001L,        \
		    c * 0x10100L, c * 0x10101L                                 \
					...
					register ulong pword =
					    ...
					    (spr40[dp[1]]) +
					    (spr40[dp[2]] >> 1) +
					    (spr8[dp[3]] << 1) + (spr8[dp[4]]) +
					    (spr8[dp[5]] >> 1) + (spr2[dp[6]]) +
					    (spr2[dp[7]] >> 1);
					odp[0] = (byte)(pword >> 16);
				...
				if(num_blank_lines >
				   0) { /* move down from current position */
				...
				for(row = plane_data + LINE_SIZE * 2, i = 0;
					...
					int count = compress1_row(
					    row, row + LINE_SIZE, temp);
					fprintf(prn_stream, "\033*b%d%c", count,
					        "VVW"[i]);
					fwrite(temp, sizeof(byte), count,
					       prn_stream);
				}
			}
		}
...
private
int
compress1_row(const byte* row, const byte* end_row, byte* compressed)
{
