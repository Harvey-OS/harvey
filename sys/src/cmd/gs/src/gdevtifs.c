...
private
const TIFF_std_directory_entries std_entries_initial = {
...
private
const TIFF_std_directory_values std_values_initial = {
    0, {0, 1}, {0, 1}, {0}, {0, 0}};
...
gdev_tiff_begin_page(gx_device_printer* pdev, gdev_tiff_state* tifs, FILE* fp,
                     ...
                     const byte* values, int value_size, int32_t max_strip_size)
{
...
#define std_value_size sizeof(TIFF_std_directory_values)
	...
	tifs->StripOffsets = (TIFF_ulong*)gs_alloc_bytes(
	    mem, (tifs->strip_count) * 2 * sizeof(TIFF_ulong),
	...
	}
	{
		...
		sprintf(std_values.dateTimeValue,
		        "%04d:%02d:%02d %02d:%02d:%02d", tms.tm_year + 1900,
		        tms.tm_mon + 1, tms.tm_mday, tms.tm_hour, tms.tm_min,
		        tms.tm_sec);
	}

	/* Write the merged directory. */
	for(pse = (const TIFF_dir_entry*)&std_entries, nse = std_entry_count,
	pce = entries, nce = entry_count;
	    nse || nce;) {
		bool std;
		...
		if(entry.tag == TIFFTAG_StripOffsets) {
			if(tifs->strip_count > 1) {
				tifs->offset_StripOffsets =
				    tifs->dir_off +
				    (ntags * sizeof(TIFF_dir_entry)) +
				    std_value_size + value_size;
	...
	fwrite(tifs->StripOffsets, sizeof(TIFF_ulong), 2 * tifs->strip_count,
	       fp);
	tifs->strip_index = 0;
...
gdev_tiff_end_page(gdev_tiff_state* tifs, FILE* fp)
{
	...
	/* The offset in the file was determined at begin_page and may be
	 * indirect */
	...
	fwrite(tifs->StripByteCounts, sizeof(TIFF_ulong), tifs->strip_count,
	       fp);
	gs_free_object(mem, tifs->StripOffsets,
	               "gdev_tiff_begin_page(StripOffsets)");
	return 0;
}
