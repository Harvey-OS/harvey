...
private
dev_proc_open_device(display_open);
private
dev_proc_get_initial_matrix(display_get_initial_matrix);
private
dev_proc_sync_output(display_sync_output);
private
dev_proc_output_page(display_output_page);
private
dev_proc_close_device(display_close);

private
dev_proc_map_rgb_color(display_map_rgb_color_device4);
private
dev_proc_map_color_rgb(display_map_color_rgb_device4);
private
dev_proc_encode_color(display_encode_color_device8);
private
dev_proc_decode_color(display_decode_color_device8);
private
dev_proc_map_rgb_color(display_map_rgb_color_device16);
private
dev_proc_map_color_rgb(display_map_color_rgb_device16);
private
dev_proc_map_rgb_color(display_map_rgb_color_rgb);
private
dev_proc_map_color_rgb(display_map_color_rgb_rgb);
private
dev_proc_map_rgb_color(display_map_rgb_color_bgr24);
private
dev_proc_map_color_rgb(display_map_color_rgb_bgr24);

private
dev_proc_fill_rectangle(display_fill_rectangle);
private
dev_proc_copy_mono(display_copy_mono);
private
dev_proc_copy_color(display_copy_color);
private
dev_proc_get_bits(display_get_bits);
private
dev_proc_get_params(display_get_params);
private
dev_proc_put_params(display_put_params);
private
dev_proc_finish_copydevice(display_finish_copydevice);

private
dev_proc_get_color_mapping_procs(display_separation_get_color_mapping_procs);
private
dev_proc_get_color_comp_index(display_separation_get_color_comp_index);
private
dev_proc_encode_color(display_separation_encode_color);
private
dev_proc_decode_color(display_separation_decode_color);
private
dev_proc_update_spot_equivalent_colors(display_update_spot_equivalent_colors);

private
const gx_device_procs display_procs = {
    display_open, display_get_initial_matrix, display_sync_output,
    display_output_page, display_close, gx_default_w_b_map_rgb_color,
    gx_default_w_b_map_color_rgb, display_fill_rectangle,
    NULL,                                        /* tile rectangle */
    display_copy_mono, display_copy_color, NULL, /* draw line */
    display_get_bits, display_get_params, display_put_params,
    gx_default_cmyk_map_cmyk_color,   /* map_cmyk_color */
    gx_default_get_xfont_procs, NULL, /* get_xfont_device */
...
if(ddev->mdev) {
	ddev->mdev = (gx_device_memory*)gx_device_reloc_ptr(
	    (gx_device*)ddev->mdev, gcst);
}
{
	int i;
	for(i = 0; i < ddev->devn_params.separations.num_separations; ++i) {
		RELOC_PTR(gx_device_display,
...
const gx_device_display gs_display_device = {
    std_device_std_body_type(gx_device_display, &display_procs, "display",
                             &st_device_display, INITIAL_WIDTH, INITIAL_HEIGHT,
...
private
int display_check_structure(gx_device_display* dev);
private
void display_free_bitmap(gx_device_display* dev);
private
int display_alloc_bitmap(gx_device_display*, gx_device*);
private
int display_set_color_format(gx_device_display* dev, int nFormat);
private
int display_set_separations(gx_device_display* dev);
private
int display_raster(gx_device_display* dev);
...
display_open(gx_device* dev)
{
	...
	ccode = (*(ddev->callback->display_presize))(
	    ddev->pHandle, dev, dev->width, dev->height, display_raster(ddev),
	    ddev->nFormat);
	if(ccode < 0) {
	...
	ccode = (*(ddev->callback->display_size))(
	    ddev->pHandle, dev, dev->width, dev->height, display_raster(ddev),
	    ddev->nFormat, ddev->mdev->base);
	if(ccode < 0) {
...
display_output_page(gx_device* dev, int copies, int flush)
{
	...
	code = (*(ddev->callback->display_page))(ddev->pHandle, dev, copies,
	                                         flush);
                               ...
                               gx_color_value prgb[3])
{
	...
	if((ddev->nFormat & DISPLAY_ENDIAN_MASK) == DISPLAY_BIGENDIAN) {
		if((ddev->nFormat & DISPLAY_555_MASK) == DISPLAY_NATIVE_555) {
			...
			prgb[0] =
			    (gx_color_value)(((value << 11) + (value << 6) +
			                      (value << 1) + (value >> 4)) >>
			...
			prgb[1] =
			    (gx_color_value)((value << 10) + (value << 4) +
			                     (value >> 2)) >>
			    (16 - gx_color_value_bits);
			value = (ushort)(color & 0x1f);
			prgb[2] =
...
display_map_rgb_color_rgb(gx_device* dev, const gx_color_value cv[])
{
	...
	case DISPLAY_UNUSED_FIRST:
		if((ddev->nFormat & DISPLAY_ENDIAN_MASK) == DISPLAY_BIGENDIAN)
			return ((gx_color_index)red << 16) + (green << 8) +
			       blue; /* xRGB */
		else
			return ((gx_color_index)blue << 16) + (green << 8) +
			       red; /* xBGR */
	case DISPLAY_ALPHA_LAST:
	case DISPLAY_UNUSED_LAST:
		if((ddev->nFormat & DISPLAY_ENDIAN_MASK) == DISPLAY_BIGENDIAN)
			return ((gx_color_index)red << 24) + (green << 16) +
			       (blue << 8); /* RGBx */
		else
			return ((gx_color_index)blue << 24) + (green << 16) +
			       (red << 8); /* BGRx */
	}
	return 0;
}
                          ...
                          gx_color_value prgb[3])
{
	...
	case DISPLAY_ALPHA_NONE:
		if((ddev->nFormat & DISPLAY_ENDIAN_MASK) == DISPLAY_BIGENDIAN)
			return gx_default_rgb_map_color_rgb(dev, color,
			                                    prgb); /* RGB */
		else {
			/* BGR */
			prgb[0] = (gx_color_value)(((color)&color_mask) *
			                           (ulong)gx_max_color_value /
			                           color_mask);
			prgb[1] = (gx_color_value)(
...
display_map_rgb_color_bgr24(gx_device* dev, const gx_color_value cv[])
{
	...
	return (gx_color_value_to_byte(b) << 16) +
	       (gx_color_value_to_byte(g) << 8) + gx_color_value_to_byte(r);
}
                       ...
                       gx_color_index color)
{
	...
	if(ddev->callback == NULL)
		return 0;
	dev_proc(ddev->mdev, fill_rectangle)((gx_device*)ddev->mdev, x, y, w, h,
	                                     color);
	if(ddev->callback->display_update)
		(*(ddev->callback->display_update))(ddev->pHandle, dev, x, y, w,
		                                    h);
	return 0;
}
...
private
int
display_copy_mono(gx_device* dev, const byte* base, int sourcex, int raster,
                  ...
                  gx_color_index zero, gx_color_index one)
{
	...
	if(ddev->callback == NULL)
		return 0;
	dev_proc(ddev->mdev, copy_mono)((gx_device*)ddev->mdev, base, sourcex,
	                                raster, id, x, y, w, h, zero, one);
	if(ddev->callback->display_update)
		(*(ddev->callback->display_update))(ddev->pHandle, dev, x, y, w,
		                                    h);
	return 0;
...
private
int
display_copy_color(gx_device* dev, const byte* base, int sourcex, int raster,
                   gx_bitmap_id id, int x, int y, int w, int h)
{
	gx_device_display* ddev = (gx_device_display*)dev;
	if(ddev->callback == NULL)
		return 0;
	dev_proc(ddev->mdev, copy_color)((gx_device*)ddev->mdev, base, sourcex,
	                                 raster, id, x, y, w, h);
	if(ddev->callback->display_update)
		(*(ddev->callback->display_update))(ddev->pHandle, dev, x, y, w,
		                                    h);
...
display_get_bits(gx_device* dev, int y, byte* str, byte** actual_data)
{
	...
	if(ddev->callback == NULL)
		return 0;
	return dev_proc(ddev->mdev, get_bits)((gx_device*)ddev->mdev, y, str,
	                                      actual_data);
}
...
display_get_params(gx_device* dev, gs_param_list* plist)
{
	...
	(void)(code < 0 ||
	       (code = param_write_string(plist, "DisplayHandle", &dhandle)) <
	           0 ||
	       (code = param_write_int(plist, "DisplayFormat",
	                               &ddev->nFormat)) < 0 ||
	       (code = param_write_float(plist, "DisplayResolution",
	                                 &ddev->HWResolution[1])) < 0);
	if(code >= 0 &&
...
display_put_params(gx_device* dev, gs_param_list* plist)
{
	...
	default:
		ecode = code;
	cfe:
		param_signal_error(plist, "DisplayFormat", ecode);
	case 1:
		break;
	}
	...
	default:
		if((code == gs_error_typecheck) && (sizeof(size_t) <= 4)) {
			...
			default:
				ecode = code;
			hdle:
				param_signal_error(plist, "DisplayHandle",
				                   ecode);
			case 1:
				break;
			}
	...
	if(ecode >= 0 &&
	   (ddev->nFormat & DISPLAY_COLORS_MASK) == DISPLAY_COLORS_SEPARATION) {
		...
		ecode =
		    devn_put_params(dev, plist, pdevn_params, pequiv_colors);
		/*
	...
	if(is_open && ddev->callback &&
	   ((old_width != dev->width) || (old_height != dev->height))) {
		...
		if((*ddev->callback->display_presize)(
		       ddev->pHandle, dev, dev->width, dev->height,
		       display_raster(ddev), ddev->nFormat) < 0) {
		...
		if(code < 0) {
			...
			(*ddev->callback->display_size)(
			    ddev->pHandle, dev, 0, 0, 0, ddev->nFormat, NULL);
			return_error(code);
		}

		/* tell caller about the new size */
		if((*ddev->callback->display_size)(
		       ddev->pHandle, dev, dev->width, dev->height,
...
{
	int* map = (int*)(&((gx_device_display*)dev)
	                       ->devn_params.separation_order_map);
...
private
void
display_separation_rgb_cs_to_cmyk_cm(gx_device* dev, const gs_imager_state* pis,
...
private
void
display_separation_cmyk_cs_to_cmyk_cm(gx_device* dev, frac c, frac m, frac y,
                                      frac k, frac out[])
{
	int* map = (int*)(&((gx_device_display*)dev)
	                       ->devn_params.separation_order_map);
...
private
const gx_cm_color_map_procs display_separation_cm_procs = {
    display_separation_gray_cs_to_cmyk_cm, display_separation_rgb_cs_to_cmyk_cm,
...
display_update_spot_equivalent_colors(gx_device* dev, const gs_state* pgs)
{
	...
	if((ddev->nFormat & DISPLAY_COLORS_MASK) == DISPLAY_COLORS_SEPARATION)
		update_spot_equivalent_cmyk_colors(dev, pgs, &ddev->devn_params,
		                                   &ddev->equiv_cmyk_colors);
	return 0;
}
...
private
int
display_separation_get_color_comp_index(gx_device* dev, const char* pname,
                                        int name_size, int component_type)
{
	return devn_get_color_comp_index(
	    dev, &(((gx_device_display*)dev)->devn_params),
	    &(((gx_device_display*)dev)->equiv_cmyk_colors), pname, name_size,
...
private
int
display_check_structure(gx_device_display* ddev)
{
	if(ddev->callback == 0)
		return_error(gs_error_rangecheck);
...
display_free_bitmap(gx_device_display* ddev)
{
	...
	if(ddev->pBitmap) {
		if(ddev->callback->display_memalloc &&
		   ddev->callback->display_memfree && ddev->pBitmap) {
...
display_alloc_bitmap(gx_device_display* ddev, gx_device* param_dev)
{
	...
	ddev->mdev =
	    gs_alloc_struct(gs_memory_stable(ddev->memory), gx_device_memory,
	                    &st_device_memory, "display_memory_device");
	if(ddev->mdev == 0)
	...
	ddev->ulBitmapSize = gdev_mem_bits_size(ddev->mdev, ddev->mdev->width,
	                                        ddev->mdev->height);

	/* allocate bitmap using an allocator not subject to GC */
	if(ddev->callback->display_memalloc &&
	   ddev->callback->display_memfree) {
		ddev->pBitmap = (*ddev->callback->display_memalloc)(
		    ddev->pHandle, ddev, ddev->ulBitmapSize);
	} else {
		ddev->pBitmap = gs_alloc_byte_array_immovable(
		    ddev->memory->non_gc_memory, (uint)ddev->ulBitmapSize, 1,
		    "display_alloc_bitmap");
	}
	...
	if(ccode == 0) {
		...
		for(i = 0; i < GX_DEVICE_COLOR_MAX_COMPONENTS; i++)
			cv[i] = (ddev->color_info.polarity ==
			         GX_CINFO_POLARITY_ADDITIVE)
			            ? gx_max_color_value
			            : 0;
		dev_proc(ddev, fill_rectangle)(
		    (gx_device*)ddev, 0, 0, ddev->width, ddev->height,
...
{
	if(((dev->nFormat & DISPLAY_COLORS_MASK) ==
	   ...
	   (dev->callback->display_separation != NULL)) {
		...
		for(sep_num = 0; sep_num < num_comp; sep_num++) {
			comp_num =
			    dev->devn_params.separation_order_map[sep_num];
			if(comp_num >= 0 &&
		...
		for(comp_num = 0; comp_num < num_comp; comp_num++) {
			...
			if(sep_num < dev->devn_params.num_std_colorant_names) {
				sep_name_size =
				    strlen(dev->devn_params
				               .std_colorant_names[sep_num]);
				if(sep_name_size > sizeof(name) - 2)
					sep_name_size = sizeof(name) - 1;
				memcpy(name, dev->devn_params
				                 .std_colorant_names[sep_num],
				       sep_name_size);
				...
				sep_num -=
				    dev->devn_params.num_std_colorant_names;
				sep_name_size =
				    dev->devn_params.separations.names[sep_num]
				        .size;
				if(sep_name_size > sizeof(name) - 2)
					sep_name_size = sizeof(name) - 1;
				memcpy(
				    ...
				    sep_name_size);
				name[sep_name_size] = '\0';
				if(dev->equiv_cmyk_colors.color[sep_num]
				       .color_info_valid) {
					c = dev->equiv_cmyk_colors
					        .color[sep_num]
					        .c *
					    65535 / frac_1;
					m = dev->equiv_cmyk_colors
					        .color[sep_num]
					        .m *
					    65535 / frac_1;
					y = dev->equiv_cmyk_colors
					        .color[sep_num]
					        .y *
					    65535 / frac_1;
					k = dev->equiv_cmyk_colors
					        .color[sep_num]
					        .k *
					    65535 / frac_1;
				}
			}
			(*dev->callback->display_separation)(
			    dev->pHandle, dev, comp_num, name,
...
private
void
set_color_info(gx_device_color_info* pdci, DISPLAY_MODEL model, int nc,
               int depth, int maxgray, int maxcolor)
{
...
void
set_color_procs(
    gx_device* pdev, dev_t_proc_encode_color((*encode_color), gx_device),
...
display_set_color_format(gx_device_display* ddev, int nFormat)
{
	...
	case DISPLAY_COLORS_NATIVE:
		...
		case DISPLAY_DEPTH_16:
			...
			if((ddev->nFormat & DISPLAY_555_MASK) ==
			   DISPLAY_NATIVE_555)
				set_color_info(&dci, DISPLAY_MODEL_RGB, 3, 16,
				               31, 31);
			else
				set_color_info(&dci, DISPLAY_MODEL_RGB, 3, 16,
				               63, 63);
			set_rgb_color_procs(pdev,
			                    display_map_rgb_color_device16,
	...
	case DISPLAY_COLORS_RGB:
		if((nFormat & DISPLAY_ALPHA_MASK) == DISPLAY_ALPHA_NONE)
			...
			bpp = bpc * 4;
		set_color_info(&dci, DISPLAY_MODEL_RGB, 3, bpp, maxvalue,
		               maxvalue);
		if(((nFormat & DISPLAY_DEPTH_MASK) == DISPLAY_DEPTH_8) &&
		   ((nFormat & DISPLAY_ALPHA_MASK) == DISPLAY_ALPHA_NONE)) {
			if((nFormat & DISPLAY_ENDIAN_MASK) == DISPLAY_BIGENDIAN)
				set_rgb_color_procs(
				    pdev, gx_default_rgb_map_rgb_color,
				    gx_default_rgb_map_color_rgb);
			else
				set_rgb_color_procs(
				    pdev, display_map_rgb_color_bgr24,
	...
	case DISPLAY_COLORS_CMYK:
		bpp = bpc * 4;
		set_color_info(&dci, DISPLAY_MODEL_CMYK, 4, bpp, maxvalue,
		               maxvalue);
		if((nFormat & DISPLAY_ALPHA_MASK) != DISPLAY_ALPHA_NONE)
			return_error(gs_error_rangecheck);
		if((nFormat & DISPLAY_ENDIAN_MASK) != DISPLAY_BIGENDIAN)
	...
	case DISPLAY_COLORS_SEPARATION:
		...
		if((nFormat & DISPLAY_DEPTH_MASK) == DISPLAY_DEPTH_8) {
			ddev->devn_params.bitspercomponent = bpc;
			set_color_procs(
			    pdev, display_separation_encode_color,
...
{
    char buf[1024];
    sprintf(buf, "gs -dDisplayFormat=16#%x examples/colorcir.ps -c quit",
test_modes[index].format);
    system(buf);
}
