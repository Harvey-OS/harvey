...
#define std_device_part1_(devtype, ptr_procs, dev_name, stype, open_init)      \
	sizeof(devtype), ptr_procs, dev_name, 0 /*memory*/, stype,             \
	    0 /*stype_is_dynamic*/, 0 /*finalize*/, {0} /*rc*/,                \
	    0 /*retained*/, open_init() /*is_open, max_fill_band*/
...
#define std_device_part3_()                                                    \
	0 /*PageCount*/, 0 /*ShowpageCount*/, 1 /*NumCopies*/,                 \
	    0 /*NumCopies_set*/, 0 /*IgnoreNumCopies*/, 0 /*UseCIEColor*/,     \
	    0 /*LockSafetyParams*/,                                            \
	{                                                                      \
 ...
 *
 */
#define std_device_body_with_macros_(dtype, pprocs, dname, stype, w, h, xdpi,  \
                                     ydpi, open_init, dci_macro,               \
                                     margins_macro)                            \
	std_device_part1_(dtype, pprocs, dname, stype, open_init),             \
	    dci_macro(), std_device_part2_(w, h, xdpi, ydpi), margins_macro(), \
	    std_device_part3_()

#define std_device_std_body_type(dtype, pprocs, dname, stype, w, h, xdpi,      \
                                 ydpi)                                         \
	std_device_body_with_macros_(dtype, pprocs, dname, stype, w, h, xdpi,  \
	                             ydpi, open_init_closed,                   \
	                             dci_black_and_white_, no_margins_)

#define std_device_std_body(dtype, pprocs, dname, w, h, xdpi, ydpi)            \
	std_device_std_body_type(dtype, pprocs, dname, 0, w, h, xdpi, ydpi)

#define std_device_std_body_type_open(dtype, pprocs, dname, stype, w, h, xdpi, \
                                      ydpi)                                    \
	std_device_body_with_macros_(dtype, pprocs, dname, stype, w, h, xdpi,  \
	                             ydpi, open_init_open,                     \
	                             dci_black_and_white_, no_margins_)
...
#define std_device_full_body_type(dtype, pprocs, dname, stype, w, h, xdpi,     \
                                  ydpi, ncomp, depth, mg, mc, dg, dc, xoff,    \
                                  yoff, lm, bm, rm, tm)                        \
...
#define std_device_full_body_type_extended(dtype, pprocs, dname, stype, w, h,  \
                                           xdpi, ydpi, mcomp, ncomp, pol,      \
                                           depth, gi, mg, mc, dg, dc, ef, cn,  \
                                           xoff, yoff, lm, bm, rm, tm)         \
	std_device_part1_(dtype, pprocs, dname, stype, open_init_closed),      \
	    dci_extended_alpha_values(mcomp, ncomp, pol, depth, gi, mg, mc,    \
	                              dg, dc, 1, 1, ef, cn),                   \
	    std_device_part2_(w, h, xdpi, ydpi),                               \
	    offset_margin_values(xoff, yoff, lm, bm, rm, tm),                  \
	    std_device_part3_()

#define std_device_full_body(dtype, pprocs, dname, w, h, xdpi, ydpi, ncomp,    \
                             depth, mg, mc, dg, dc, xoff, yoff, lm, bm, rm,    \
                             tm)                                               \
	std_device_full_body_type(dtype, pprocs, dname, 0, w, h, xdpi, ydpi,   \
	                          ncomp, depth, mg, mc, dg, dc, xoff, yoff,    \
	                          lm, bm, rm, tm)

#define std_device_dci_alpha_type_body(dtype, pprocs, dname, stype, w, h,      \
                                       xdpi, ydpi, ncomp, depth, mg, mc, dg,   \
                                       dc, ta, ga)                             \
	std_device_part1_(dtype, pprocs, dname, stype, open_init_closed),      \
	    dci_alpha_values(ncomp, depth, mg, mc, dg, dc, ta, ga),            \
	    std_device_part2_(w, h, xdpi, ydpi),                               \
	    offset_margin_values(0, 0, 0, 0, 0, 0), std_device_part3_()

#define std_device_dci_type_body(dtype, pprocs, dname, stype, w, h, xdpi,      \
                                 ydpi, ncomp, depth, mg, mc, dg, dc)           \
	std_device_dci_alpha_type_body(dtype, pprocs, dname, stype, w, h,      \
	                               xdpi, ydpi, ncomp, depth, mg, mc, dg,   \
	                               dc, 1, 1)

#define std_device_dci_body(dtype, pprocs, dname, w, h, xdpi, ydpi, ncomp,     \
                            depth, mg, mc, dg, dc)                             \
	std_device_dci_type_body(dtype, pprocs, dname, 0, w, h, xdpi, ydpi,    \
	                         ncomp, depth, mg, mc, dg, dc)

#define std_device_color_full_body(dtype, pprocs, dname, w, h, xdpi, ydpi,     \
                                   depth, max_value, dither, xoff, yoff, lm,   \
                                   bm, rm, tm)                                 \
...
#define std_device_color_body(dtype, pprocs, dname, w, h, xdpi, ydpi, depth,   \
                              max_value, dither)                               \
	std_device_color_full_body(dtype, pprocs, dname, w, h, xdpi, ydpi,     \
	                           depth, max_value, dither, 0, 0, 0, 0, 0, 0)

#define std_device_color_stype_body(dtype, pprocs, dname, stype, w, h, xdpi,   \
                                    ydpi, depth, max_value, dither)            \
	std_device_part1_(dtype, pprocs, dname, stype, open_init_closed),      \
	    dci_color(depth, max_value, dither),                               \
	    std_device_part2_(w, h, xdpi, ydpi),                               \
	    offset_margin_values(0, 0, 0, 0, 0, 0), std_device_part3_()

#define std_device_std_color_full_body_type(dtype, pprocs, dname, stype, w, h, \
                                            xdpi, ydpi, depth, xoff, yoff, lm, \
                                            bm, rm, tm)                        \
	std_device_part1_(dtype, pprocs, dname, stype, open_init_closed),      \
	    dci_std_color(depth), std_device_part2_(w, h, xdpi, ydpi),         \
	    offset_margin_values(xoff, yoff, lm, bm, rm, tm),                  \
	    std_device_part3_()

#define std_device_std_color_full_body(dtype, pprocs, dname, w, h, xdpi, ydpi, \
                                       depth, xoff, yoff, lm, bm, rm, tm)      \
	std_device_std_color_full_body_type(dtype, pprocs, dname, 0, w, h,     \
	                                    xdpi, ydpi, depth, xoff, yoff, lm, \
	                                    bm, rm, tm)
...
dev_proc_update_spot_equivalent_colors(
 ...
 */
int gx_device_open_output_file(const gx_device* dev, char* fname, bool binary,
                               bool positionable, FILE** pfile);
...
#define fit_copy_xyw(dev, data, data_x, raster, id, x, y, w, h)                \
	...
	if((x | y) < 0) {                                                      \
		if(x < 0)                                                      \
			w += x, data_x -= x, x = 0;                            \
		if(y < 0)                                                      \
			h += y, data -= y * raster, id = gx_no_bitmap_id,      \
			                            y = 0;                     \
	}                                                                      \
	if(w > (dev)->width - x)                                               \
		w = (dev)->width - x;                                          \
...
#define gdev_input_media_default_values                                        \
	{                                                                      \
		0, 0, 0, 0                                                     \
	}                                                                      \
	, 0, 0, 0
