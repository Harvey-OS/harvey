...
z11_CIDMap_proc(gs_font_cid2* pfont, gs_glyph glyph)
{
	...
	default: /* array type */
		code = string_array_access_proc(pfont->memory, pcidmap, 1,
		                                cid * gdbytes, gdbytes, &data);
...
private
int
z11_get_outline(gs_font_type42* pfont, uint glyph_index, gs_glyph_data_t* pgd)
{
	gs_font_cid2* const pfcid = (gs_font_cid2*)pfont;
	int skip = pfcid->cidata.MetricsCount << 1;
	int code =
	    pfcid->cidata.orig_procs.get_outline(pfont, glyph_index, pgd);
                ...
                float sbw[4])
{
	...
	if(wmode >= skip >> 2 ||
	   (code = pfcid->cidata.orig_procs.get_outline(pfont, glyph_index,
	                                                &gdata)) < 0 ||
	   gdata.bits.size < skip)
		return pfcid->cidata.orig_procs.get_metrics(pfont, glyph_index,
		                                            wmode, sbw);
...
private
int
z11_enumerate_glyph(gs_font* font, int* pindex, gs_glyph_space_t glyph_space,
                    gs_glyph* pglyph)
{
...
private
int
z11_glyph_outline(gs_font* font, int WMode, gs_glyph glyph,
                  const gs_matrix* pmat, gx_path* ppath, double sbw[4])
{
	return gs_type42_glyph_outline(
	    font, WMode, z11_get_glyph_index((gs_font_type42*)font, glyph) +
	                     GS_MIN_GLYPH_INDEX,
	    pmat, ppath, sbw);
}
...
zbuildfont11(i_ctx_t* i_ctx_p)
{
	...
	gs_font_cid2* pfcid;
	int MetricsCount;
	ref rcidmap, ignore_gdir, file, *pfile, cfnstr, *pCIDFontName,
	    CIDFontName;
	...
	code = build_gs_TrueType_font(
	    i_ctx_p, op, &pfont, ft_CID_TrueType, &st_gs_font_cid2,
	...
	if(MetricsCount) {
		 ...
		 * We could do not, but the number of virtual function wariants
		 * increases.
		 */
		stream* s;
		...
		pfont->data.gdcache = gs_glyph_cache__alloc(
		    pfont, s, gs_type42_get_outline_from_TT_file);
	}
	return define_gs_font((gs_font*)pfont);
}
...
zfillCIDMap(i_ctx_t* i_ctx_p)
{
	...
	code = cid_fill_CIDMap(imemory, Decoding, TT_cmap, SubstNWP,
	                       GDBytes->value.intval, CIDMap);
	pop(5);
	return code;
}
...
const op_def zfcid1_op_defs[] = {{"2.buildfont10", zbuildfont10},
