...
/* Procedures */
private
dev_proc_map_rgb_color(mem_mono_map_rgb_color);
private
dev_proc_map_color_rgb(mem_mono_map_color_rgb);
private
dev_proc_copy_mono(mem_mono_copy_mono);
private
dev_proc_fill_rectangle(mem_mono_fill_rectangle);
private
dev_proc_strip_tile_rectangle(mem_mono_strip_tile_rectangle);

/* The device descriptor. */
/* The instance is public. */
const gx_device_memory mem_mono_device = mem_full_alpha_device(
    "image1", 0, 1, mem_open, mem_mono_map_rgb_color, mem_mono_map_color_rgb,
    mem_mono_copy_mono, gx_default_copy_color, mem_mono_fill_rectangle,
    gx_default_map_cmyk_color, gx_no_copy_alpha, mem_mono_strip_tile_rectangle,
    mem_mono_strip_copy_rop, mem_get_bits_rectangle);
...
mem_mono_map_rgb_color(gx_device* dev, const gx_color_value cv[])
{
	...
	return (gx_default_w_b_map_rgb_color(dev, cv) ^ mdev->palette.data[0]) &
	       1;
}
...
{
	gx_device_memory* const mdev = (gx_device_memory*)dev;
	/* NB code doesn't make sense... map_color_rgb procedures return an
	 * error code */
...
#ifdef USE_COPY_ROP
	return mem_mono_copy_rop(dev, NULL, 0, 0, gx_no_bitmap_id, NULL, NULL,
	                         NULL, x, y, w, h, 0, 0,
	                         (color ? rop3_1 : rop3_0));
#else
 ...
 */
#define CFETCH_ALIGNED(cptr) (*(const chunk*)(cptr))
...
#define chunk uint
#define CFETCH_RIGHT(cptr, shift, cshift) (CFETCH_ALIGNED(cptr) >> shift)
#define CFETCH_LEFT(cptr, shift, cshift) (CFETCH_ALIGNED(cptr) << shift)
...
#define chunk bits16
private
const bits16 right_masks2[9] = {0xffff, 0x7f7f, 0x3f3f, 0x1f1f, 0x0f0f,
                                0x0707, 0x0303, 0x0101, 0x0000};
private
const bits16 left_masks2[9] = {0xffff, 0xfefe, 0xfcfc, 0xf8f8, 0xf0f0,
                               0xe0e0, 0xc0c0, 0x8080, 0x0000};
...
private
int
mem_mono_copy_mono(gx_device* dev, const byte* source_data, int source_x,
                   int source_raster, gx_bitmap_id id, int x, int y, int w,
                   int h, gx_color_index color0, gx_color_index color1)
{
	gx_device_memory* const mdev = (gx_device_memory*)dev;

#ifdef USE_COPY_ROP
	return mem_mono_copy_rop(
	    dev, source_data, source_x, source_raster, id, NULL, NULL, NULL, x,
	    y, w, h, 0, 0,
	    ((color0 == gx_no_color_index ? rop3_D : color0 == 0 ? rop3_0
	                                                         : rop3_1) &
	     ~rop3_S) |
	        ((color1 == gx_no_color_index ? rop3_D : color1 == 0 ? rop3_0
	                                                             : rop3_1) &
	         rop3_S));
#else /* !USE_COPY_ROP */
...
#define WRITE_OR_MASKED(bits, mask, off) optr[off] |= (CINVERT(bits) & mask)
#define WRITE_STORE_MASKED(bits, mask, off)                                    \
	optr[off] = ((optr[off] & ~mask) | (CINVERT(bits) & mask))
#define WRITE_AND_MASKED(bits, mask, off) optr[off] &= (CINVERT(bits) | ~mask)
...
#define END_Y_LOOP(sdelta, ddelta)                                             \
	...
	if((wleft = w + dbit - chunk_bits) <=
	   0) { /* The entire operation fits in one (destination) chunk. */
...
#define WRITE1_LOOP(src)                                                       \
	switch(mode.op) {                                                      \
	case COPY_OR:                                                          \
		WRITE_SINGLE(WRITE_OR_MASKED, src);                            \
		break;                                                         \
	case COPY_STORE:                                                       \
		WRITE_SINGLE(WRITE_STORE_MASKED, src);                         \
		break;                                                         \
	case COPY_AND:                                                         \
		WRITE_SINGLE(WRITE_AND_MASKED, src);                           \
		break;                                                         \
	default:                                                               \
		return FUNNY_CASE();                                           \
	}
...
#define WRITE_ALIGNED(wr_op, wr_op_masked)                                     \
	for(;;) {                                                              \
		...
		while((count -= chunk_bits) >= 0) {                            \
			NEXT_X_CHUNK();                                        \
			wr_op(CFETCH_ALIGNED(bptr));                           \
		}                                                              \
		/* Do last chunk */                                            \
		if(count > -chunk_bits) {                                      \
			wr_op_masked(CFETCH_ALIGNED(bptr + chunk_bytes),       \
			             rmask, 1);                                \
		}                                                              \
		if(--h == 0)                                                   \
			break;                                                 \
...
#undef WRITE_ALIGNED
			...
			bool case_right =
			    (skew >= 0 ? true : ((bptr += chunk_bytes), false));

			skew &= chunk_bit_mask;

#define WRITE_UNALIGNED(wr_op, wr_op_masked)                                   \
	/* Prefetch partial word. */                                           \
	bits = (case_right ? CFETCH_RIGHT(bptr, skew, cskew)                   \
...
mem_mono_strip_tile_rectangle(gx_device* dev,
                              register const gx_strip_bitmap* tiles, int tx,
                              int y, int tw, int th, gx_color_index color0,
                              gx_color_index color1, int px, int py)
{
	gx_device_memory* const mdev = (gx_device_memory*)dev;

#ifdef USE_COPY_ROP
	return mem_mono_strip_copy_rop(
	    dev, NULL, 0, 0, tile->id, NULL, tiles, NULL, tx, y, tw, th, px, py,
	    ((color0 == gx_no_color_index ? rop3_D : color0 == 0 ? rop3_0
	                                                         : rop3_1) &
	     ~rop3_T) |
	        ((color1 == gx_no_color_index ? rop3_D : color1 == 0 ? rop3_0
...
#define optr ((chunk*)dbptr)
	...
	if(color0 != (color1 ^ 1) || tiles->shift != 0)
		return gx_default_strip_tile_rectangle(
		    dev, tiles, tx, y, tw, th, color0, color1, px, py);
	fit_fill(dev, tx, y, tw, th);
	invert = (uint)(-(int)color0);
	source_raster = tiles->raster;
	source_data =
	    tiles->data + ((y + py) % tiles->rep_height) * source_raster;
...
#else /* byte swapping */
		for(;;) {
			WRITE_STORE_MASKED(CFETCH_RIGHT(bptr, skew, cskew),
			                   mask, 0);
			WRITE_STORE_MASKED(CFETCH_LEFT(bptr, cskew, skew),
			                   rmask, 1);
			if(--h == 0)
				break;
			END_Y_LOOP(source_raster, dest_raster);
	...
	dbptr += chunk_bytes
		...
		if(skew == 0) { /* optimize the aligned case */
			...
			for(;;) {
				...
				/* Do first partial chunk. */
				if(mask)
					WRITE_STORE_MASKED(CFETCH_ALIGNED(bptr),
					                   mask, 0);
				else
				...
				if(count > -chunk_bits) {
					WRITE_STORE_MASKED(
					    CFETCH_ALIGNED(bptr + chunk_bytes),
					    rmask, 1);
				}
				if(--h == 0)
					break;
...
/* Procedures */
private
dev_proc_copy_mono(mem1_word_copy_mono);
private
dev_proc_fill_rectangle(mem1_word_fill_rectangle);

#define mem1_word_strip_tile_rectangle gx_default_strip_tile_rectangle

/* Here is the device descriptor. */
const gx_device_memory mem_mono_word_device = mem_full_alpha_device(
    "image1w", 0, 1, mem_open, mem_mono_map_rgb_color, mem_mono_map_color_rgb,
    mem1_word_copy_mono, gx_default_copy_color, mem1_word_fill_rectangle,
    gx_default_map_cmyk_color, gx_no_copy_alpha, mem1_word_strip_tile_rectangle,
                         ...
                         gx_color_index color)
{
	...
	bits_fill_rectangle(base, x, raster, -(int)(mono_fill_chunk)color, w,
	                    h);
	mem_swap_byte_rect(base, raster, x, w, h, true);
	return 0;
}
...
private
int
mem1_word_copy_mono(gx_device* dev, const byte* source_data, int source_x,
                    int source_raster, gx_bitmap_id id, int x, int y, int w,
                    int h, gx_color_index color0, gx_color_index color1)
{
	gx_device_memory* const mdev = (gx_device_memory*)dev;
	...
	mem_mono_copy_mono(dev, source_data, source_x, source_raster, id, x, y,
	                   w, h, color0, color1);
