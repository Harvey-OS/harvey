...
#define chunk bits16
private
const bits16 right_masks2[9] =
    {
     0xffff, 0x7f7f, 0x3f3f, 0x1f1f, 0x0f0f, 0x0707, 0x0303, 0x0101, 0x0000};
private
const bits16 left_masks2[9] =
    {
     0xffff, 0xfefe, 0xfcfc, 0xf8f8, 0xf0f0, 0xe0e0, 0xc0c0, 0x8080, 0x0000};

#define CCONT(cptr, off) (((const chunk *)(cptr))[off])
#define CFETCH_RIGHT(cptr, shift, cshift) \
	((shift) < 8 ? ((CCONT(cptr, 0) >> (shift)) & right_masks2[shift]) + (CCONT(cptr, 0) << (cshift)) : ((chunk) * (const byte *)(cptr) << (cshift)) & 0xff00)
#define CFETCH_LEFT(cptr, shift, cshift) \
	((shift) < 8 ? ((CCONT(cptr, 0) << (shift)) & left_masks2[shift]) + (CCONT(cptr, 0) >> (cshift)) : ((CCONT(cptr, 0) & 0xff00) >> (cshift)) & 0xff)
#define CFETCH_USES_CSKEW 1
/* Fetch a chunk that straddles a chunk boundary. */
/* We can avoid testing the shift amount twice */
/* by expanding the CFETCH_LEFT/right macros in-line. */
#define CFETCH2(cptr, cskew, skew)                                                                                                                                                                                            \
	((cskew) < 8 ? ((CCONT(cptr, 0) << (cskew)) & left_masks2[cskew]) + (CCONT(cptr, 0) >> (skew)) + (((chunk)(((const byte *)(cptr))[2]) << (cskew)) & 0xff00) : (((CCONT(cptr, 0) & 0xff00) >> (skew)) & 0xff) +        \
...
#define FUNNY_CASE() \
	(invert ? gs_note_error(-1) : mem_mono_fill_rectangle(dev, x, y, w, h, color0))
		   ...
		   int x, int y, int w, int h, gx_color_index color0, gx_color_index color1)
{
	...
	return mem_mono_copy_rop(dev, source_data, source_x, source_raster,
				 id, NULL, NULL, NULL,
				 x, y, w, h, 0, 0,
				 ((color0 == gx_no_color_index ? rop3_D : color0 == 0 ? rop3_0 : rop3_1) & ~rop3_S) |
				     ((color1 == gx_no_color_index ? rop3_D : color1 == 0 ? rop3_0 : rop3_1) & rop3_S));
#else /* !USE_COPY_ROP */
...
#define WRITE_UNALIGNED(wr_op, wr_op_masked)                                                              \
	...
	bits = (case_right ? CFETCH_RIGHT(bptr, skew, cskew) : CFETCH2(bptr - chunk_bytes, cskew, skew)); \
			      ...
			      int px, int py)
{
	...
	return mem_mono_strip_copy_rop(dev, NULL, 0, 0, tile->id, NULL,
				       tiles, NULL,
				       tx, y, tw, th, px, py,
				       ((color0 == gx_no_color_index ? rop3_D : color0 == 0 ? rop3_0 : rop3_1) & ~rop3_T) |
					   ((color1 == gx_no_color_index ? rop3_D : color1 == 0 ? rop3_0 : rop3_1) & rop3_T));
#else /* !USE_COPY_ROP */
	register uint invert;
	int source_raster;
	uint tile_bits_size;
