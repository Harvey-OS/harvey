...
#define decode_sample(sample_value, cc, i)                                     \
	...
	case sd_none:                                                          \
		cc.paint.values[i] =                                           \
		    (sample_value) * (1.0 / 255.0); /* faster than / */        \
		break;                                                         \
...
struct gx_image_enum_s {
	...
	struct mc_ {
		uint
		    values[GS_IMAGE_MAX_COMPONENTS * 2]; /* MaskColor values, */
...
#define gx_image_enum_do_ptrs(m)                                               \
	m(0, pis) m(1, pcs) m(2, dev) m(3, buffer) m(4, line) m(5, clip_dev)   \
	    m(6, rop_dev) m(7, scaler)
#define gx_image_enum_num_ptrs 8
#define private_st_gx_image_enum() /* in gsimage.c */                          \
	gs_private_st_composite(st_gx_image_enum, gx_image_enum,               \
	                        "gx_image_enum", image_enum_enum_ptrs,         \
	                        image_enum_reloc_ptrs)

/* Compare two device colors for equality. */
/* We can special-case this for speed later if we care. */
#define dev_color_eq(devc1, devc2) gx_device_color_equal(&(devc1), &(devc2))
 ...
 */
void gx_image_scale_mask_colors(gx_image_enum* penum, int component_index);
 ...
 *	rect
 */
int gx_image_enum_alloc(const gs_image_common_t* pic, const gs_int_rect* prect,
 ...
 *	masked, adjust
 */
int gx_image_enum_begin(gx_device* dev, const gs_imager_state* pis,
 ...
 */
void image_init_clues(gx_image_enum* penum, int bps, int spp);
