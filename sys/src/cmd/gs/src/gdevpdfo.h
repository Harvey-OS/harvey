...
#define cos_proc_release(proc) void proc(cos_object_t* pco, client_name_t cname)
...
#define cos_proc_equal(proc)                                                   \
	int proc(const cos_object_t* pco0, const cos_object_t* pco1,           \
	         gx_device_pdf* pdev)
...
#define private_st_cos_object() /* in gdevpdfo.c */                            \
	gs_private_st_ptrs5(st_cos_object, cos_object_t, "cos_object_t",       \
	                    cos_object_enum_ptrs, cos_object_reloc_ptrs,       \
	                    elements, pieces, pdev, pres, input_strm)
...
#define private_st_cos_value() /* in gdevpdfo.c */                             \
	gs_private_st_composite(st_cos_value, cos_value_t, "cos_value_t",      \
	                        cos_value_enum_ptrs, cos_value_reloc_ptrs)
...
int cos_dict_put_no_copy(cos_dict_t*, const byte*, uint, const cos_value_t*);
...
int cos_dict_put_string_copy(cos_dict_t* pcd, const char* key,
                             const char* value);
 ...
 */
const cos_array_element_t* cos_array_element_first(const cos_array_t*);
const cos_array_element_t* cos_array_element_next(const cos_array_element_t*,
                                                  long*, const cos_value_t**);
...
int cos_stream_elements_write(const cos_stream_t*,
                              gx_device_pdf*); /* = dict_elements_write */
