...
const gx_device_printer far_data gs_cif_device =
    prn_device(prn_std_procs, "cif", DEFAULT_WIDTH_10THS, DEFAULT_HEIGHT_10THS,
               X_DPI, Y_DPI, 0, 0, 0, 0, 1, cif_print_page);
...
cif_print_page(gx_device_printer* pdev, FILE* prn_stream)
{
	...
	int lnum;
	byte* in =
	    (byte*)gs_malloc(pdev->memory, line_size, 1, "cif_print_page(in)");
	char* s;
	...
	if((s = strchr(pdev->fname, '.')) == NULL)
		...
		length = s - pdev->fname;
	s = (char*)gs_malloc(pdev->memory, length, sizeof(char),
	                     "cif_print_page(s)");
