...
private
int
pixel_resize(psdf_binary_writer* pbw, int width, int num_components, int bpc_in,
             int bpc_out)
{
	...
	if(bpc_in != 8) {
		static const stream_template* const exts[13] = {
		    0, &s_1_8_template, &s_2_8_template, 0, &s_4_8_template, 0,
		    0, 0, 0, 0, 0, 0, &s_12_8_template};
	...
	st = (stream_1248_state*)s_alloc_state(mem, template->stype,
	                                       "pixel_resize state");
	if(st == 0)
...
private
int
convert_color(gx_device* pdev, const gs_color_space* pcs,
              const gs_imager_state* pis, gs_client_color* cc, float c[3])
{
	int code;
	gx_device_color dc;

	cs_restrict_color(cc, pcs);
	code = pcs->type->remap_color(cc, pcs, &dc, pis, pdev,
	                              gs_color_select_texture);
	if(code < 0)
		return code;
	c[0] = (float)((int)(dc.colors.pure >> pdev->color_info.comp_shift[0]) &
	               ((1 << pdev->color_info.comp_bits[0]) - 1));
	c[1] = (float)((int)(dc.colors.pure >> pdev->color_info.comp_shift[1]) &
	               ((1 << pdev->color_info.comp_bits[1]) - 1));
	c[2] = (float)((int)(dc.colors.pure >> pdev->color_info.comp_shift[2]) &
	               ((1 << pdev->color_info.comp_bits[2]) - 1));
	return 0;
}
...
choose_DCT_params(gx_device* pdev, const gs_color_space* pcs,
                  const gs_imager_state* pis, gs_c_param_list* list,
                  gs_c_param_list** param, stream_state* st)
{
	...
	*param = list;
	...
	gs_make_mem_device(&mdev, gdev_mem_device_for_bits(24), pdev->memory, 0,
	                   NULL);
	...
	cc.paint.values[0] = cc.paint.values[1] = cc.paint.values[2] =
	    MIN_FLOAT;
	convert_color((gx_device*)&mdev, pcs, pis, &cc, c[3]);
	cc.paint.values[0] = MAX_FLOAT;
	cc.paint.values[1] = MIN_FLOAT;
	cc.paint.values[2] = MIN_FLOAT;
	convert_color((gx_device*)&mdev, pcs, pis, &cc, c[0]);
	cc.paint.values[0] = MIN_FLOAT;
	cc.paint.values[1] = MAX_FLOAT;
	cc.paint.values[2] = MIN_FLOAT;
	convert_color((gx_device*)&mdev, pcs, pis, &cc, c[1]);
	cc.paint.values[0] = MIN_FLOAT;
	cc.paint.values[1] = MIN_FLOAT;
	cc.paint.values[2] = MAX_FLOAT;
	...
	if(c[0][0] * domination > c[0][1] && c[0][0] * domination > c[0][2] &&
	   ...
	   c[2][2] * domination > c[2][0] && c[2][2] * domination > c[2][1]) {
		...
		code = param_write_int((gs_param_list*)list, "ColorTransform",
		                       &one);
		if(code < 0)
			return code;
		goto done;
	...
	cc.paint.values[0] = MAX_FLOAT;
	cc.paint.values[1] = cc.paint.values[2] = 0;
	convert_color((gx_device*)&mdev, pcs, pis, &cc, c[0]);
	cc.paint.values[0] /= 2;
	convert_color((gx_device*)&mdev, pcs, pis, &cc, c[1]);
	cc.paint.values[0] /= 2;
	...
	if(c[0][0] * domination > c[0][1] && c[0][0] * domination > c[0][2] &&
	   ...
	   c[2][0] * domination > c[2][1] && c[2][0] * domination > c[2][2]) {
		...
		/* Unknown color space type.
		   Replace /HSamples [1 1 1 1] /VSamples [1 1 1 1] to avoid
		   quality degradation. */
		gs_param_string a;
                        ...
                        bool lossless)
{
	...
	if(pdip->AutoFilter) {
		/*
		 * Disregard the requested filter.  What we should do at this
		 * point
		 * is analyze the image to decide whether to use JPEG encoding
		 * (DCTEncode with ACSDict) or the lossless filter.  However,
		 * since
		 * we don't buffer the entire image, we'll make the choice
		 * on-fly,
		 * forking the image data into 3 streams : (1) JPEG, (2)
		 * lossless,
		 * (3) the compression chooser. In this case this function is
		 * called 2 times with different values of the 'lossless'
		 * argument.
		 */
		if(lossless) {
	...
	if(pdev->version < psdf_version_ll3 && template == &s_zlibE_template)
		orig_template = template = lossless_template;
	if(dict) /* NB: rather than dereference NULL lets continue on without a
	            dict */
		gs_c_param_list_read(
		    dict); /* ensure param list is in read mode */
	if(template == 0)  /* no compression */
		return 0;
	if(pim->Width < 200 &&
	   pim->Height < 200) /* Prevent a fixed overflow. */
		if(pim->Width * pim->Height * Colors * pim->BitsPerComponent <=
		   160)
	...
	if(template == &s_CFE_template) {
		...
		if(!Indexed) {
			...
			if(template->set_defaults)
				(*template->set_defaults)(st);
			{
				stream_PNGP_state* const ss =
				    (stream_PNGP_state*)st;
		...
		code = choose_DCT_params((gx_device*)pbw->dev, pcs, pis, &list,
		                         &param, st);
		if(code < 0) {
			gs_c_param_list_release(&list);
			return code;
		}
		code = psdf_DCT_filter((gs_param_list*)param, st, pim->Width,
		                       pim->Height, Colors, pbw);
		gs_c_param_list_release(&list);
		if(code < 0)
			goto fail;
                   ...
                   floatp resolution, bool lossless)
{
	...
	const stream_template* template =
	    (pdip->DownsampleType == ds_Subsample ? &s_Subsample_template
	                                          : &s_Average_template);
		...
		template->set_defaults(st);
	{
		...
		if(template->init)
			template->init(st);
		pim->Width =
		    s_Downsample_size_out(pim->Width, factor, ss->padX);
		pim->Height =
		    s_Downsample_size_out(pim->Height, factor, ss->padY);
		pim->BitsPerComponent = pdip->Depth;
		gs_matrix_scale(
		    &pim->ImageMatrix, (double)pim->Width / orig_width,
		    (double)pim->Height / orig_height, &pim->ImageMatrix);
		/****** NO ANTI-ALIASING YET ******/
		if((code = setup_image_compression(pbw, pdip, pim, pis,
		                                   lossless)) < 0 ||
		   (code = pixel_resize(pbw, pim->Width, ss->Colors, 8,
		                        pdip->Depth)) < 0 ||
		   (code = psdf_encode_binary(pbw, template, st)) < 0 ||
		   (code = pixel_resize(pbw, orig_width, ss->Colors, orig_bpc,
		                        8)) < 0) {
			gs_free_object(pdev->v_memory, st,
			               "setup_image_compression");
			return code;
		}
	}
...
{
	return pdev->params.ConvertCMYKImagesToRGB && pis != 0 &&
                         ...
                         const gs_imager_state* pis, bool lossless)
{
	...
	if(pctm == 0)
		...
		code = gs_distance_transform_inverse(1.0, 0.0,
		                                     &pim->ImageMatrix, &pt);
		if(code < 0)
			return code;
	...
	if(ncomp == 1) {
		...
		if(do_downsample(&params, pim, resolution)) {
			/* Use the downsampled depth, not the original data
			 * depth. */
			if(params.Depth == 1) {
				params.Filter = pdev->params.MonoImage.Filter;
				params.filter_template =
				    pdev->params.MonoImage.filter_template;
				params.Dict = pdev->params.MonoImage.Dict;
			} else {
				params.Filter = pdev->params.GrayImage.Filter;
				params.filter_template =
				    pdev->params.GrayImage.filter_template;
				params.Dict = pdev->params.GrayImage.Dict;
			}
			code = setup_downsampling(pbw, &params, pim, pis,
			                          resolution, lossless);
		} else {
			code = setup_image_compression(pbw, &params, pim, pis,
			                               lossless);
		}
		if(code < 0)
			return code;
		...
		/* Color */
		bool cmyk_to_rgb =
		    psdf_is_converting_image_to_RGB(pdev, pis, pim);

		if(cmyk_to_rgb) {
			extern_st(st_color_space);
			gs_memory_t* mem = pdev->v_memory;
			gs_color_space* rgb_cs = gs_alloc_struct(
			    mem, gs_color_space, &st_color_space,
			    "psdf_setup_image_filters");

			gs_cspace_init_DeviceRGB(
			    mem, rgb_cs); /* idempotent initialization */
		...
		if(cmyk_to_rgb) {
			gs_memory_t* mem = pdev->v_memory;
			stream_C2R_state* ss = (stream_C2R_state*)s_alloc_state(
			    mem, s_C2R_template.stype, "C2R state");
			int code = pixel_resize(pbw, pim->Width, 3, 8, bpc_out);

			if(code < 0 ||
			   (code = psdf_encode_binary(pbw, &s_C2R_template,
			                              (stream_state*)ss)) < 0 ||
			   (code = pixel_resize(pbw, pim->Width, 4, bpc, 8)) <
			       0)
				return code;
			s_C2R_init(ss, pis);
		} else {
			code =
			    pixel_resize(pbw, pim->Width, ncomp, bpc, bpc_out);
			if(code < 0)
				return code;
		}
	}
                               ...
                               int bits_per_sample)
{
	int code;
	stream_state* ss =
	    s_alloc_state(pdev->memory, s_compr_chooser_template.stype,
	...
	pbw->dev = pdev;
	pbw->target = pbw->strm; /* Since s_add_filter may insert NullEncode to
	                            comply buffering,
...
psdf_setup_image_to_mask_filter(psdf_binary_writer* pbw, gx_device_psdf* pdev,
                                int width, int height, int depth,
                                int bits_per_sample, uint* MaskColor)
{
	int code;
	stream_state* ss =
	    s_alloc_state(pdev->memory, s__image_colors_template.stype,
	...
	if(code < 0)
		return code;
	s_image_colors_set_dimensions((stream_image_colors_state*)ss, width,
	                              height, depth, bits_per_sample);
	s_image_colors_set_mask_colors((stream_image_colors_state*)ss,
	                               MaskColor);
	return 0;
}
...
int
psdf_setup_image_colors_filter(psdf_binary_writer* pbw, gx_device_psdf* pdev,
                               gs_pixel_image_t* pim,
                               const gs_imager_state* pis,
...
{ /* fixme: currently it's a stub convertion to mask. */
	...
	s_image_colors_set_dimensions(
	    (stream_image_colors_state*)ss, pim->Width, pim->Height,
	    gs_color_space_num_components(pim->ColorSpace),
	    pim->BitsPerComponent);
	s_image_colors_set_color_space((stream_image_colors_state*)ss,
	                               (gx_device*)pdev, pim->ColorSpace, pis,
	                               pim->Decode);
	pim->BitsPerComponent =
	    pdev->color_info
	        .comp_bits[0]; /* Same precision for all components. */
