			...
			bool lossless)
{
	...
	if(template == &s_DCTE_template) {
		if(Indexed ||
		   !(pdip->Downsample ? pdip->Depth == 8 || (pdip->Depth == -1 && pim->BitsPerComponent == 8) : pim->BitsPerComponent == 8)) {
		   ...
		   floatp resolution, bool lossless)
{
	...
	const stream_template *template =
	    (pdip->DownsampleType == ds_Subsample ? &s_Subsample_template : &s_Average_template);
		...
		template->set_defaults(st);
	{
		...
		ss->Colors =
		    (pim->ColorSpace == 0 ? 1 /*mask*/ : gs_color_space_num_components(pim->ColorSpace));
