...
#include "vdtrace.h"
/*
#include "gxfilltr.h" - Do not remove this comment. "gxfilltr.h" is included
below.
#include "gxfillsl.h" - Do not remove this comment. "gxfillsl.h" is included
below.
#include "gxfillts.h" - Do not remove this comment. "gxfillts.h" is included
below.
*/
...
print_active_line(const char* label, const active_line* alp)
{
	dlprintf5("[f]%s 0x%lx(%d): x_current=%f x_next=%f\n", label,
	          (uint32_t)alp, alp->direction, fixed2float(alp->x_current),
	          fixed2float(alp->x_next));
	dlprintf5("    start=(%f,%f) pt_end=0x%lx(%f,%f)\n",
	          fixed2float(alp->start.x), fixed2float(alp->start.y),
	          (uint32_t)alp->pseg, fixed2float(alp->end.x),
	          fixed2float(alp->end.y));
...
print_line_list(const active_line* flp)
{
	...
	for(lp = flp; lp != 0; lp = lp->next) {
		...
		dlprintf3("[f]0x%lx(%d): x_current/next=%g", (uint32_t)lp,
		          lp->direction, fixed2float(xc));
		if(xn != xc)
...
private
int add_y_list(gx_path*, line_list*);
private
int add_y_line_aux(const segment* prev_lp, const segment* lp,
                   const gs_fixed_point* curr, const gs_fixed_point* prev,
                   int dir, line_list* ll);
private
void insert_x_new(active_line*, line_list*);
private
bool end_x_line(active_line*, const line_list*, bool);
private
void step_al(active_line* alp, bool move_iterator);

#define FILL_LOOP_PROC(proc) int proc(line_list*, fixed band_mask)
private
FILL_LOOP_PROC(spot_into_scan_lines);
private
FILL_LOOP_PROC(spot_into_trapezoids);
...
init_line_list(line_list* ll, gs_memory_t* mem)
{
	...
	ll->margin_set0.y = ll->margin_set1.y =
	    0; /* A stub against indeterminism. Don't use it. */
	...
	/* Do not initialize ll->bbox_left, ll->bbox_width - they were set in
	 * advance. */
	INCR(fill);
}
...
unclose_path(gx_path* ppath, int count)
{
	...
	for(psub = ppath->first_subpath; count != 0;
		...
		if(psub->last == (segment*)&psub->closer) {
			segment* prev = psub->closer.prev,
			         * next = psub->closer.next;
...
#define SMALL_CHARACTER 500
	pseudo_rasterization =
	    (is_character && !is_spotan_device(dev) &&
	...
	lst.bbox_width =
	    fixed2int(fixed_ceiling(ibox.q.x + adjust.x)) - lst.bbox_left;
	if(vd_enabled) {
		fixed x0 =
		    int2fixed(fixed2int(ibox.p.x - adjust.x - fixed_epsilon));
		fixed x1 = int2fixed(fixed2int(ibox.q.x + adjust.x +
		                               fixed_scale - fixed_epsilon));
		fixed y0 =
		    int2fixed(fixed2int(ibox.p.y - adjust.y - fixed_epsilon));
		fixed y1 = int2fixed(fixed2int(ibox.q.y + adjust.y +
		                               fixed_scale - fixed_epsilon)),
		      k;
	...
	if(!rect_within(ibox, bbox)) {
		...
		if(ibox.p.x - adjust.x >= ibox.q.x + adjust.x ||
		   ibox.p.y - adjust.y >=
		       ibox.q.y +
		           adjust.y) { /* Intersection of boxes is empty! */
			return 0;
		}
		/*
	...
	if(adjust.x == fixed_half)
		...
		fo.adjust_right =
		    fixed_half /* + fixed_epsilon */; /* see above */
	else
	...
	if(fill_by_trapezoids && !fo.is_spotan && !lop_is_idempotent(lop)) {
		...
		if(gx_path_is_rectangular(ppath, &rbox)) {
			...
			return gx_fill_rectangle_device_rop(
			    x0, y0, x1 - x0, y1 - y0, pdevc, dev, lop);
		}
		if(fo.adjust_left | fo.adjust_right | fo.adjust_below |
		   fo.adjust_above)
			fill_by_trapezoids =
			    false; /* avoid double writing pixels */
	}
	...
	if(!big_path && !gx_path_has_curves(ppath)) /* don't need to flatten */
		...
		pfpath = ppath;
	        ...
	        gx_path__check_curves(ppath, pco_small_curves, fo.fixed_flat))
		pfpath = ppath;
	else {
		code = gx_path_copy_reducing(ppath, &ffpath, fo.fixed_flat,
		                             NULL, pco_small_curves);
		if(code < 0)
			return code;
		pfpath = &ffpath;
		if(big_path) {
		...
		goto nope;
	{
		...
		/* Some short-sighted compilers won't allow a conditional
		 * here.... */
		if(fill_by_trapezoids)
			fill_loop = spot_into_trapezoids;
		else
			fill_loop = spot_into_scan_lines;
		if(lst.bbox_width > MAX_LOCAL_SECTION &&
		   fo.pseudo_rasterization) {
			 ...
			 */
			lst.margin_set0.sect = (section*)gs_alloc_struct_array(
			    pdev->memory, lst.bbox_width * 2, section,
			    &st_section, "section");
			if(lst.margin_set0.sect == 0)
				return_error(gs_error_VMerror);
			lst.margin_set1.sect =
			    lst.margin_set0.sect + lst.bbox_width;
		}
		...
		code = (*fill_loop)(&lst, (max_fill_band == 0
		                               ? NO_BAND_MASK
		                               : int2fixed(-max_fill_band)));
		if(lst.margin_set0.sect != lst.local_section0 &&
		   lst.margin_set0.sect != lst.local_section1)
			gs_free_object(pdev->memory, min(lst.margin_set0.sect,
			                                 lst.margin_set1.sect),
			               "section");
	}
nope:
...
#ifdef DEBUG
	if(gs_debug_c('f')) {
		dlputs("[f]  # alloc    up  down horiz step slowx  iter  find  "
		       "band bstep bfill\n");
		dlprintf5(" %5ld %5ld %5ld %5ld %5ld", stats_fill.fill,
		          stats_fill.fill_alloc, stats_fill.y_up,
		          stats_fill.y_down, stats_fill.horiz);
		dlprintf4(" %5ld %5ld %5ld %5ld", stats_fill.x_step,
		          stats_fill.slow_x, stats_fill.iter,
		          stats_fill.find_y);
		dlprintf3(" %5ld %5ld %5ld\n", stats_fill.band,
		          stats_fill.band_step, stats_fill.band_fill);
                     ...
                     const gx_device_color* pdevc, const gx_clip_path* pcpath)
{
	...
	if(gx_dc_is_pattern2_color(pdevc)) {
		/*  Optimization for shading fill :
		    The general path filling algorithm subdivides fill region
		   with
		    trapezoid or rectangle subregions and then paints each
		   subregion
		    with given color. If the color is shading, each subregion to
		   be
		    ...
		    subregions grows huge. Faster processing is done here by
		   changing
		    the order of subdivision cycles : we first subdivide the
		   shading into
		    areas of constant color, then apply the general path filling
		   algorithm
		    (i.e. subdivide each area into trapezoids or rectangles),
		   using the
		...
		*/
		...
		/*  Shading fill algorithm uses "current path" parameter of the
		   general
		    path filling algorithm as boundary of constant color area,
		    so we need to intersect the filling path with the clip path
		   now,
		    reducing the number of pathes passed to it :
		*/
		gx_path_init_local(&path_intersection, pdev->memory);
		gx_cpath_init_local_shared(&cpath_intersection, pcpath,
		                           pdev->memory);
		if((code = gx_cpath_intersect(&cpath_intersection, ppath,
		                              params->rule,
		                              (gs_imager_state*)pis)) >= 0)
			code = gx_cpath_to_path(&cpath_intersection,
			                        &path_intersection);

		/* Do fill : */
		if(code >= 0)
			code = gx_dc_pattern2_fill_path(
			    pdevc, &path_intersection, NULL, pdev);

		/* Destruct local data and return :*/
		gx_path_free(&path_intersection,
		             "shading_fill_path_intersection");
		gx_cpath_free(&cpath_intersection,
		              "shading_fill_cpath_intersection");
	} else {
		bool got_dc = false;
		vd_save;
		if(vd_allowed('F') || vd_allowed('f')) {
			if(!vd_enabled) {
				vd_get_dc((params->adjust.x | params->adjust.y)
				              ? 'F'
				              : 'f');
				got_dc = vd_enabled;
			}
			...
			vd_disable;
		code = gx_general_fill_path(pdev, pis, ppath, params, pdevc,
		                            pcpath);
		if(got_dc)
			vd_release_dc;
		vd_restore;
...
make_al(line_list* ll)
{
	...
	if(alp == ll->limit) { /* Allocate separately */
		alp = gs_alloc_struct(ll->memory, active_line, &st_active_line,
		                      "active line");
		if(alp == 0)
			return NULL;
...
insert_y_line(line_list* ll, active_line* alp)
{
	...
	vd_bar(alp->start.x, alp->start.y, alp->end.x, alp->end.y, 0,
	       RGB(255, 0, 0));
	if(yp == 0) {
		alp->next = alp->prev = 0;
		ll->y_list = alp;
	} else if(y_start >=
	          yp->start.y) { /* Insert the new line after y_line */
		while(INCR_EXPR(y_up),
		      ((nyp = yp->next) != NULL && y_start > nyp->start.y))
			yp = nyp;
		alp->next = nyp;
		alp->prev = yp;
		yp->next = alp;
		...
		while(INCR_EXPR(y_down),
		      ((nyp = yp->prev) != NULL && y_start < nyp->start.y))
			yp = nyp;
		alp->prev = nyp;
		alp->next = yp;
		yp->prev = alp;
	...
	ll->y_line = alp;
	vd_bar(alp->start.x, alp->start.y, alp->end.x, alp->end.y, 1,
	       RGB(0, 255, 0));
	print_al("add ", alp);
}
...
compute_dir(const fill_options* fo, fixed y0, fixed y1)
{
	...
	if(min(y0, y1) > fo->ymax)
		return 2;
	return (y0 < y1 ? DIR_UP : y0 > y1 ? DIR_DOWN : DIR_HORIZONTAL);
}
                 ...
                 bool monotonic_x)
{
	...
	if(step_back) {
		do {
			alp->more_flattened =
			    gx_flattened_iterator__prev(&alp->fi);
...
start_al_pair(line_list* ll, contour_cursor* q, contour_cursor* p)
{
	...
	if(p->monotonic_y)
		...
		code =
		    add_y_curve_part(ll, p->prev, p->pseg, DIR_UP, p->fi,
...
start_al_pair_from_min(line_list* ll, contour_cursor* q)
{
	...
	do {
		...
		if(dir == DIR_UP && ll->main_dir == DIR_DOWN &&
		   q->fi->ly0 >= fo->ymin) {
			code =
			    add_y_curve_part(ll, q->prev, q->pseg, DIR_DOWN,
			                     q->fi, true, true, q->monotonic_x);
			if(code < 0)
				return code;
			code = add_y_curve_part(ll, q->prev, q->pseg, DIR_UP,
			                        q->fi, q->more_flattened, false,
			                        q->monotonic_x);
			if(code < 0)
				return code;
			...
			code = add_y_curve_part(ll, q->prev, q->pseg, DIR_DOWN,
			                        q->fi, true, false,
			                        q->monotonic_x);
			if(code < 0)
				return code;
		...
		q->dir = dir;
		ll->main_dir =
		    (dir == DIR_DOWN ? DIR_DOWN : dir == DIR_UP ? DIR_UP
		                                                : ll->main_dir);
		if(!q->more_flattened)
			break;
...
init_contour_cursor(line_list* ll, contour_cursor* q)
{
	...
	if(q->pseg->type == s_curve) {
		...
		fixed ymin =
		    min(min(q->prev->pt.y, s->p1.y), min(s->p2.y, s->pt.y));
		fixed ymax =
		    max(max(q->prev->pt.y, s->p1.y), max(s->p2.y, s->pt.y));
		bool in_band = ymin <= fo->ymax && ymax >= fo->ymin;
		q->crossing = ymin < fo->ymin && ymax >= fo->ymin;
		q->monotonic_y =
		    !in_band || (!q->crossing &&
		                 ((q->prev->pt.y <= s->p1.y &&
		                   s->p1.y <= s->p2.y && s->p2.y <= s->pt.y) ||
		                  (q->prev->pt.y >= s->p1.y &&
		                   s->p1.y >= s->p2.y && s->p2.y >= s->pt.y)));
		q->monotonic_x = ((q->prev->pt.x <= s->p1.x &&
		                   s->p1.x <= s->p2.x && s->p2.x <= s->pt.x) ||
		                  (q->prev->pt.x >= s->p1.x &&
		                   s->p1.x >= s->p2.x && s->p2.x >= s->pt.x));
	} else
		q->monotonic_y = true;
	if(!q->monotonic_y) {
		curve_segment* s = (curve_segment*)q->pseg;
		int k = gx_curve_log2_samples(q->prev->pt.x, q->prev->pt.y, s,
		                              fo->fixed_flat);
		...
		gx_flattened_iterator__init_line(
		    q->fi, q->prev->pt.x, q->prev->pt.y, q->pseg->pt.x,
		    q->pseg->pt.y); /* fake for curves. */
		vd_bar(q->prev->pt.x, q->prev->pt.y, q->pseg->pt.x,
		       q->pseg->pt.y, 1, RGB(0, 0, 255));
	}
...
scan_contour(line_list* ll, contour_cursor* q)
{
	...
	for(pseg = q->pfirst; pseg != q->plast; pseg = pseg->next) {
		...
		if(!fo->pseudo_rasterization || only_horizontal ||
		   p.prev->pt.x != p.pseg->pt.x ||
		   p.prev->pt.y != p.pseg->pt.y || p.pseg->type == s_curve) {
			...
			if(p.monotonic_y && p.dir == DIR_HORIZONTAL &&
			   ...
			   fixed2int_pixround(p.pseg->pt.y - fo->adjust_below) <
			       fixed2int_pixround(p.pseg->pt.y +
			                          fo->adjust_above)) {
				/* Add it here to avoid double processing in
				 * process_h_segments. */
				code = add_y_line(p.prev, p.pseg,
				                  DIR_HORIZONTAL, ll);
				if(code < 0)
					return code;
			}
			...
			if(p.fi->ly0 >= fo->ymin && p.dir == DIR_UP &&
			   ll->main_dir == DIR_DOWN) {
			...
			if(p.fi->ly0 < fo->ymin && p.fi->ly1 >= fo->ymin) {
				if(p.monotonic_y)
					...
					code = add_y_curve_part(
					    ll, p.prev, p.pseg, DIR_UP, p.fi,
			...
			if(p.fi->ly0 >= fo->ymin && p.fi->ly1 < fo->ymin) {
				if(p.monotonic_y)
					...
					code = add_y_curve_part(
					    ll, p.prev, p.pseg, DIR_DOWN, p.fi,
			...
			ll->main_dir =
			    (p.dir == DIR_DOWN ? DIR_DOWN : p.dir == DIR_UP
			                                        ? DIR_UP
			                                        : ll->main_dir);
...
step_al(active_line* alp, bool move_iterator)
{
	...
	if(move_iterator) {
		if(forth)
			alp->more_flattened =
			    gx_flattened_iterator__next(&alp->fi);
		else
			alp->more_flattened =
			    gx_flattened_iterator__prev(&alp->fi);
	} else
		vd_bar(alp->fi.lx0, alp->fi.ly0, alp->fi.lx1, alp->fi.ly1, 1,
		       RGB(0, 0, 255));
...
private
void
init_al(active_line* alp, const segment* s0, const segment* s1,
        const line_list* ll)
{
	...
	if(curve) {
		if(alp->direction == DIR_UP) {
			...
			int k = gx_curve_log2_samples(s0->pt.x, s0->pt.y, cs,
			                              ll->fo->fixed_flat);

			gx_flattened_iterator__init(
			    &alp->fi, s0->pt.x, s0->pt.y,
			    (const curve_segment*)s1, k);
			step_al(alp, true);
			if(!ll->fo->fill_by_trapezoids) {
				alp->monotonic_y = (s0->pt.y <= cs->p1.y &&
				                    cs->p1.y <= cs->p2.y &&
				                    cs->p2.y <= cs->pt.y);
				alp->monotonic_x = (s0->pt.x <= cs->p1.x &&
				                    cs->p1.x <= cs->p2.x &&
				                    cs->p2.x <= cs->pt.x) ||
				                   (s0->pt.x >= cs->p1.x &&
				                    cs->p1.x >= cs->p2.x &&
			...
			int k = gx_curve_log2_samples(s1->pt.x, s1->pt.y, cs,
			                              ll->fo->fixed_flat);
			bool more;

			gx_flattened_iterator__init(
			    &alp->fi, s1->pt.x, s1->pt.y,
			    (const curve_segment*)s0, k);
			alp->more_flattened = false;
			do {
				more = gx_flattened_iterator__next(&alp->fi);
				alp->more_flattened |= more;
			} while(more);
			gx_flattened_iterator__switch_to_backscan(
			    &alp->fi, alp->more_flattened);
			step_al(alp, false);
			if(!ll->fo->fill_by_trapezoids) {
				alp->monotonic_y = (s0->pt.y >= cs->p1.y &&
				                    cs->p1.y >= cs->p2.y &&
		...
		gx_flattened_iterator__init_line(&alp->fi, s0->pt.x, s0->pt.y,
		                                 s1->pt.x, s1->pt.y);
		step_al(alp, true);
...
insert_x_new(active_line* alp, line_list* ll)
{
	...
	alp->x_next =
	    alp->start.x; /*	If the spot starts with a horizontal segment,
...
end_x_line(active_line* alp, const line_list* ll, bool update)
{
	...
	if(alp->end.y < alp->start.y) {
		/* fixme: The condition above causes a horizontal
		   ...
		   once scanning the left spot boundary and once scanning the
		   right one.
...
private
inline int
add_margin(line_list* ll, active_line* flp, active_line* alp, fixed y0,
           fixed y1)
{
	vd_bar(alp->start.x, alp->start.y, alp->end.x, alp->end.y, 1,
	       RGB(255, 255, 255));
	vd_bar(flp->start.x, flp->start.y, flp->end.x, flp->end.y, 1,
	       RGB(255, 255, 255));
...
private
inline int
continue_margin(line_list* ll, active_line* flp, active_line* alp, fixed y0,
                fixed y1)
{
	return continue_margin_common(ll, &ll->margin_set0, flp, alp, y0, y1);
}
...
private
inline int
complete_margin(line_list* ll, active_line* flp, active_line* alp, fixed y0,
                fixed y1)
{
...
private
int
fill_slant_adjust(const line_list* ll, const active_line* flp,
                  const active_line* alp, fixed y, fixed y1)
{
	const fill_options* const fo = ll->fo;
	...
	INCR(slant);
	vd_quad(flp->x_current, y, alp->x_current, y, alp->x_next, y1,
	        flp->x_next, y1, 1, VD_TRAP_COLOR); /* fixme: Wrong X. */

	/* Set up all the edges, even though we may not need them all. */

	if(flp->start.x < flp->end.x) {
		vert_left.start.x = vert_left.end.x =
		    flp->x_current - fo->adjust_left;
		vert_left.start.y = Yb, vert_left.end.y = Ya;
		vert_right.start.x = vert_right.end.x =
		    alp->x_next + fo->adjust_right;
		...
		vert_left.start.x = vert_left.end.x =
		    flp->x_next - fo->adjust_left;
		vert_left.start.y = Y1b, vert_left.end.y = Y1a;
		vert_right.start.x = vert_right.end.x =
		    alp->x_current + fo->adjust_right;
	...
	if(Ya >= Y1b) {
		...
		INCR(slant_shallow);
		if(iY1b > iYb) {
			code = fo->fill_trap(fo->dev, plbot, prbot, Yb, Y1b,
			                     false, fo->pdevc, fo->lop);
			if(code < 0)
				return code;
		}
		if(iYa > iY1b) {
			...
			int iw =
			    fixed2int_var_pixround(vert_right.start.x) - ix;

			code = gx_fill_rectangle_device_rop(
			    ix, iY1b, iw, iYa - iY1b, fo->pdevc, fo->dev,
			    fo->lop);
			if(code < 0)
				return code;
		}
		if(iY1a > iYa)
			code = fo->fill_trap(fo->dev, pltop, prtop, Ya, Y1a,
			                     false, fo->pdevc, fo->lop);
		else
			code = 0;
	} else {
		...
		if(fo->pbox->p.y < Ya) {
			code = fo->fill_trap(fo->dev, plbot, prbot, Yb, Ya,
			                     false, fo->pdevc, fo->lop);
			if(code < 0)
				return code;
			Yac = Ya;
		...
		if(fo->pbox->q.y > Y1b) {
			code =
			    fo->fill_trap(fo->dev, &slant_left, &slant_right,
			                  Yac, Y1b, false, fo->pdevc, fo->lop);
			if(code < 0)
				return code;
			code = fo->fill_trap(fo->dev, pltop, prtop, Y1b, Y1a,
			                     false, fo->pdevc, fo->lop);
		} else
			code = fo->fill_trap(fo->dev, &slant_left, &slant_right,
			                     Yac, fo->pbox->q.y, false,
			                     fo->pdevc, fo->lop);
	}
	return code;
}
	...
	for(alp = ll->x_list; alp != 0; alp = nlp) {
		...
		if(alp->start.y == y && alp->end.y == y) {
			if(ll->fo->pseudo_rasterization) {
				code = add_y_line_aux(NULL, NULL, &alp->start,
				                      &alp->end, DIR_HORIZONTAL,
				                      ll);
				if(code < 0)
					return code;
			}
...
private
inline int
loop_fill_trap_np(const line_list* ll, const gs_fixed_edge* le,
                  const gs_fixed_edge* re, fixed y, fixed y1)
{
	const fill_options* const fo = ll->fo;
	...
	if(ybot >= ytop)
		return 0;
	vd_quad(le->start.x, ybot, re->start.x, ybot, re->end.x, ytop,
	        le->end.x, ytop, 1, VD_TRAP_COLOR);
	return (*fo->fill_trap)(fo->dev, le, re, ybot, ytop, false, fo->pdevc,
	                        fo->lop);
}
...
private
bool
intersect(active_line* endp, active_line* alp, fixed y, fixed y1,
          fixed* p_y_new)
{
	fixed y_new, dy;
	...
	if_debug3('F', "[F]cross y=%g, y_new=%g, y1=%g\n", fixed2float(y),
	          fixed2float(y_new), fixed2float(y1));
	if(y_new <= y) {
		/*
		...
		if(endp->start.y < alp->start.y)
			ys = alp->start.y, xs0 = AL_X_AT_Y(endp, ys),
			xs1 = alp->start.x;
		else
			ys = endp->start.y, xs0 = endp->start.x,
			xs1 = AL_X_AT_Y(alp, ys);
		if(endp->end.y > alp->end.y)
			ye = alp->end.y, xe0 = AL_X_AT_Y(endp, ye),
			xe1 = alp->end.x;
		else
			ye = endp->end.y, xe0 = endp->end.x,
			xe1 = AL_X_AT_Y(alp, ye);
		dy = ye - ys;
		dx0 = xe0 - xs0;
		dx1 = xe1 - xs1;
...
{
	return 1 +
	       min(any_abs((int)((int64_t)alp->diff.y * 8 / alp->diff.x)), 256);
}
...
intersect_al(line_list* ll, fixed y, fixed* y_top, int draw, bool all_bands)
{
	...
	if(y == y1) {
		 ...
		 */
		for(alp = stopx = ll->x_list; INCR_EXPR(find_y), alp != 0;
			...
			if(nx >= x)
				...
				if(y_new <= y1) {
					...
					endp->x_next = alp->x_next =
					    nx; /* Ensure same X. */
					draw = 0;
					/* Can't guarantee same x for triple
					   intersections here.
		...
		/* Ensure X monotonity (particularly imporoves triple
		 * intersections). */
		if(ll->x_list != NULL) {
			for(;;) {
				fixed x1;
				double sx =
				    0xbaadf00d; /* 'fixed' can overflow. We
				                   operate 7-bytes integers. */
				int k = 0xbaadf00d, n;

				endp = ll->x_list;
				x1 = endp->x_next;
				for(alp = endp->next; alp != NULL;
				    x1 = alp->x_next, alp = alp->next)
					if(alp->x_next < x1)
						break;
				if(alp == NULL)
					break;
...
spot_into_trapezoids(line_list* ll, fixed band_mask)
{
	...
	if(fo->adjust_below | fo->adjust_above | fo->adjust_left |
	   fo->adjust_right) {
		if(fo->fill_direct)
...
range_alloc(coord_range_list_t* pcrl)
{
	...
	if(pcrl->freed) {
		...
		pcr = gs_alloc_struct(pcrl->memory, coord_range_t,
		                      &st_coord_range, "range_alloc");
		if(pcr == 0)
			return 0;
...
private
void range_list_clear(coord_range_list_t*);
private
void
...
private
int
merge_ranges(coord_range_list_t* pcrl, const line_list* ll, fixed y_min,
             fixed y_top)
{
	active_line* alp, *nlp;
	int code = 0;
	...
	for(alp = ll->x_list; alp != 0 && code >= 0; alp = nlp) {
		...
		if(alp->start.y < y_min)
			continue;
		if(alp->monotonic_x && alp->monotonic_y &&
		   alp->fi.y3 <= y_top) {
			vd_bar(alp->start.x, alp->start.y, alp->end.x,
			       alp->end.y, 0, RGB(255, 0, 0));
			x1 = alp->fi.x3;
			if(x0 > x1)
				xt = x0, x0 = x1, x1 = xt;
			code = range_list_add(
			    pcrl, fixed2int_pixround(x0 - ll->fo->adjust_left),
			    fixed2int_rounded(x1 + ll->fo->adjust_right));
			alp->more_flattened =
			    false; /* Skip all the segments left. */
		} else {
			x1 = x0;
			for(;;) {
				if(alp->end.y <= y_top)
