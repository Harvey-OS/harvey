...
#define SMALL_CHARACTER 500
	 ...
	 */
		...
		if(ibox.p.x - adjust.x >= ibox.q.x + adjust.x ||
		   ibox.p.y - adjust.y >= ibox.q.y + adjust.y) { /* Intersection of boxes is empty! */
			return 0;
		}
		/*
		...
		goto nope;
	{
		...
		code = (*fill_loop)(&lst, (max_fill_band == 0 ? NO_BAND_MASK : int2fixed(-max_fill_band)));
...
start_al_pair_from_min(line_list *ll, contour_cursor *q)
{
	...
	do {
		...
		q->dir = dir;
		ll->main_dir = (dir == DIR_DOWN ? DIR_DOWN : dir == DIR_UP ? DIR_UP : ll->main_dir);
		if(!q->more_flattened)
			break;
...
scan_contour(line_list *ll, contour_cursor *q)
{
	...
	for(;; q->pseg = q->prev, q->prev = q->prev->prev) {
		...
		while(gx_flattened_iterator__next(q->fi)) {
			...
			ll->main_dir = (q->dir == DIR_DOWN ? DIR_DOWN : q->dir == DIR_UP ? DIR_UP : ll->main_dir);
		}
		q->dir = compute_dir(fo, q->fi->ly0, q->fi->ly1);
	...
	for(pseg = q->pfirst; pseg != q->plast; pseg = pseg->next) {
		p.prev = pseg;
		p.pseg = pseg->next;
		if(!fo->pseudo_rasterization || only_horizontal || p.prev->pt.x != p.pseg->pt.x || p.prev->pt.y != p.pseg->pt.y || p.pseg->type == s_curve) {
			...
			ll->main_dir = (p.dir == DIR_DOWN ? DIR_DOWN : p.dir == DIR_UP ? DIR_UP : ll->main_dir);
...
end_x_line(active_line *alp, const line_list *ll, bool update)
{
     ...
     */
	const segment *next =
	    (alp->direction == DIR_UP ? (			      /* Upward line, go forward along path. */
...
loop_fill_trap_np(const line_list *ll, const gs_fixed_edge *le, const gs_fixed_edge *re, fixed y, fixed y1)
{
	...
	return (*fo->fill_trap)(fo->dev, le, re, ybot, ytop, false, fo->pdevc, fo->lop);
}
...
intersect(active_line *endp, active_line *alp, fixed y, fixed y1, fixed *p_y_new)
{
	...
	y_new =
	    ((dy | dx_old) < 1L << (size_of(fixed) * 4 - 1) ? dy * dx_old / dx_den : (INCR_EXPR(mq_cross), fixed_mult_quo(dy, dx_old, dx_den))) + y;
