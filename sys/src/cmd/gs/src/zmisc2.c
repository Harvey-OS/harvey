...
/* Forward references */
private
int set_language_level(i_ctx_t*, int);
...
const op_def zmisc2_op_defs[] = {{"0.languagelevel", zlanguagelevel},
 ...
 */
private
int swap_level_dict(i_ctx_t* i_ctx_p, const char* dict_name);
private
int swap_entry(i_ctx_t* i_ctx_p, ref elt[2], ref* pdict, ref* pdict2);
private
int
set_language_level(i_ctx_t* i_ctx_p, int new_level)
{
	...
	if(new_level < 1 ||
	   new_level > (dict_find_string(systemdict, "ll3dict", &level2dict) > 0
	                    ? 3
	                    : 2))
	...
	while(new_level != old_level) {
		...
		case 1: { /* 1 => 2 or 3 */
			 ...
			 */
			code =
			    dict_find_string(level2dict, "globaldict", &pdict);
			if(code > 0) {
		...
		case 1: { /* 2 => 1 */
			   ...
			   * defined
			   * in globaldict.  This will slow down future lookups,
			   * but
			   * we don't care.
			   */
			...
			while((index = dict_next(pgdict, index, &elt[0])) >= 0)
				if(r_has_type(&elt[0], t_name))
					name_invalidate_value_cache(imemory,
					                            &elt[0]);
		...
		if(r_has_type(&elt[1], t_dictionary) &&
		   ...
		   obj_eq(imemory, &elt[1], psubdict)) {
			...
			if(found <= 0)
				continue;
			rsubdict = *psubdict;
			while((isub = dict_next(&elt[1], isub, &subelt[0])) >=
			      0)
				if(!obj_eq(imemory, &subelt[0], &elt[0])) {
					/* don't swap dict itself */
					int code =
					    swap_entry(i_ctx_p, subelt,
					               &rsubdict, &elt[1]);
			...
			int code =
			    swap_entry(i_ctx_p, elt, systemdict, &rleveldict);
