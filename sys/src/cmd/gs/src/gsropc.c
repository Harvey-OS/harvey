...
private
composite_create_default_compositor_proc(c_rop_create_default_compositor);
private
composite_equal_proc(c_rop_equal);
private
composite_write_proc(c_rop_write);
private
composite_read_proc(c_rop_read);
private
const gs_composite_type_t gs_composite_rop_type = {
    {c_rop_create_default_compositor, c_rop_equal, c_rop_write, c_rop_read,
                        ...
                        gs_memory_t* mem)
{
	...
	rc_alloc_struct_0(pcte, gs_composite_rop_t, &st_composite_rop, mem,
...
gs_private_st_suffix_add1_final(st_device_composite_rop,
                                gx_device_composite_rop,
                                "gx_device_composite_rop",
                                device_c_rop_enum_ptrs, device_c_rop_reloc_ptrs,
                                gx_device_finalize, st_device_forward,
                                params.texture);
/* The device descriptor. */
private
dev_proc_close_device(dcr_close);
private
dev_proc_fill_rectangle(dcr_fill_rectangle);
private
dev_proc_copy_mono(dcr_copy_mono);
private
dev_proc_copy_color(dcr_copy_color);
private
dev_proc_copy_alpha(dcr_copy_alpha);
private
const gx_device_composite_rop gs_composite_rop_device = {
    std_device_std_body_open(gx_device_composite_rop, 0, "RasterOp compositor",
                             0, 0, 1, 1),
    {gx_default_open_device, gx_forward_get_initial_matrix,
     gx_forward_sync_output, gx_forward_output_page, dcr_close,
     gx_forward_map_rgb_color, gx_forward_map_color_rgb, dcr_fill_rectangle,
     gx_default_tile_rectangle, dcr_copy_mono, dcr_copy_color,
     gx_default_draw_line, gx_default_get_bits, gx_forward_get_params,
     gx_forward_put_params, gx_forward_map_cmyk_color,
     gx_forward_get_xfont_procs, gx_forward_get_xfont_device,
     gx_forward_map_rgb_alpha_color, gx_forward_get_page_device,
     gx_forward_get_alpha_bits, dcr_copy_alpha, gx_forward_get_band,
...
int
c_rop_create_default_compositor(const gs_composite_t* pcte, gx_device** pcdev,
...
#endif /*************** */
	cdev = gs_alloc_struct_immovable(mem, gx_device_composite_rop,
                   ...
                   gx_color_index color)
{
	...
	if(texture == 0) {
		return (*copy)(dev, (const byte*)0, 0, 0, gx_no_bitmap_id,
		               (const gx_color_index*)0,
		               (const gx_strip_bitmap*)0, colors, x, y, w, h, 0,
		               0, log_op);
	}
	...
	if(gx_dc_is_pure(texture)) {
		...
		return (*copy)(dev, (const byte*)0, 0, 0, gx_no_bitmap_id,
		               colors, (const gx_strip_bitmap*)0, tcolors, x, y,
		               w, h, 0, 0, log_op);
	} else if(gx_dc_is_binary_halftone(texture)) {
		tcolors[0] = texture->colors.binary.color[0];
		tcolors[1] = texture->colors.binary.color[1];
		return (*copy)(dev, (const byte*)0, 0, 0, gx_no_bitmap_id,
		               colors, &texture->colors.binary.b_tile->tiles,
		               tcolors, x, y, w, h, texture->phase.x,
		               texture->phase.y, log_op);
...
private
int
dcr_copy_mono(gx_device* dev, const byte* data, int dx, int raster,
...
private
int
dcr_copy_color(gx_device* dev, const byte* data, int dx, int raster,
...
private
int
dcr_copy_alpha(gx_device* dev, const byte* data, int data_x, int raster,
