...
/* Procedures */
declare_mem_procs(mem_mapped2_copy_mono, mem_mapped2_copy_color,
                  mem_mapped2_fill_rectangle);

/* The device descriptor. */
const gx_device_memory mem_mapped2_device = mem_device(
    "image2", 2, 0, mem_mapped_map_rgb_color, mem_mapped_map_color_rgb,
...
static const mono_fill_chunk tile_patterns[4] = {fpat(0x00), fpat(0x55),
                                                 fpat(0xaa), fpat(0xff)};

/* Fill a rectangle with a color. */
private
int
mem_mapped2_fill_rectangle(gx_device* dev, int x, int y, int w, int h,
...
private
int
mem_mapped2_copy_mono(gx_device* dev, const byte* base, int sourcex,
                      ...
                      gx_color_index zero, gx_color_index one)
{
	...
	while(h-- > 0) {
		...
		if(one != gx_no_color_index) {
			if(zero !=
			   gx_no_color_index) { /* Copying an opaque bitmap. */
				byte data =
...
private
int
mem_mapped2_copy_color(gx_device* dev, const byte* base, int sourcex,
                       int sraster, gx_bitmap_id id, int x, int y, int w, int h)
{
	int code;
	...
	code = (*dev_proc(&mem_mono_device, copy_mono))(
	    dev, base, sourcex << 1, sraster, id, x << 1, y, w << 1, h,
...
/* Procedures */
declare_mem_procs(mem2_word_copy_mono, mem2_word_copy_color,
                  mem2_word_fill_rectangle);

/* Here is the device descriptor. */
const gx_device_memory mem_mapped2_word_device =
    mem_full_device("image2w", 2, 0, mem_open, mem_mapped_map_rgb_color,
                    mem_mapped_map_color_rgb, mem2_word_copy_mono,
                    mem2_word_copy_color, mem2_word_fill_rectangle,
                         ...
                         gx_color_index color)
{
	...
	bits_fill_rectangle(base, x << 1, raster, tile_patterns[color], w << 1,
	                    h);
	mem_swap_byte_rect(base, raster, x << 1, w << 1, h, true);
	return 0;
}
...
private
int
mem2_word_copy_mono(gx_device* dev, const byte* base, int sourcex, int sraster,
                    ...
                    gx_color_index zero, gx_color_index one)
{
	...
	mem_mapped2_copy_mono(dev, base, sourcex, sraster, id, x, y, w, h, zero,
	                      one);
...
private
int
mem2_word_copy_color(gx_device* dev, const byte* base, int sourcex, int sraster,
                     gx_bitmap_id id, int x, int y, int w, int h)
{
	int code;
