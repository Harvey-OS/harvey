...
zrsdparams(i_ctx_t* i_ctx_p)
{
	...
	for(i = 0; i < r_size(pFilter); ++i) {
		...
		if(pDecodeParms) {
			...
			if(!(r_has_type(&dp, t_dictionary) ||
			     r_has_type(&dp, t_null)))
				return_error(e_typecheck);
		}
	}
 ...
 */
private
int make_rss(i_ctx_t* i_ctx_p, os_ptr op, const byte* data, uint size,
             int space, int32_t offset, int32_t length, bool is_bytestring);
private
int make_rfs(i_ctx_t* i_ctx_p, os_ptr op, stream* fs, int32_t offset,
             int32_t length);
private
int
zreusablestream(i_ctx_t* i_ctx_p)
{
	...
	if(r_has_type(source_op, t_string)) {
		...
		code =
		    make_rss(i_ctx_p, source_op, source_op->value.const_bytes,
		...
		if(gs_object_type(imemory, source_op->value.pstruct) !=
		   &st_bytes)
			return_error(e_rangecheck);
		check_read(*source_op);
	...
	rs:
		if(s->cbuf_string.data != 0) { /* string stream */
			...
			offset += pos;
			code = make_rss(
			    i_ctx_p, source_op, s->cbuf_string.data,
			    s->cbuf_string.size,
			...
			code = make_rfs(i_ctx_p, source_op, s,
			                offset + stell(s), length);
			...
			if(sfd_state->count != 0) {
				int32_t left =
				    max(sfd_state->count, 0) + sbufavailable(s);
...
make_rss(i_ctx_t* i_ctx_p, os_ptr op, const byte* data, uint size,
         int string_space, int32_t offset, int32_t length, bool is_bytestring)
{
	stream* s;
...
const op_def zfrsd_op_defs[] = {{"2.reusablestream", zreusablestream},
