...
#define psw_procs                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   \
	{                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           \
		psw_open, gx_upright_get_initial_matrix, NULL, /* sync_output */ psw_output_page, psw_close, gx_default_rgb_map_rgb_color, gx_default_rgb_map_color_rgb, psw_fill_rectangle, NULL, /* tile_rectangle */ psw_copy_mono, psw_copy_color, NULL, /* draw_line */ psdf_get_bits, psw_get_params, psw_put_params, NULL, /* map_cmyk_color */ NULL, /* get_xfont_procs */ NULL, /* get_xfont_device */ NULL, /* map_rgb_alpha_color */ gx_page_device_get_page_device, NULL, /* get_alpha_bits */ NULL, /* copy_alpha */ NULL, /* get_band */ NULL, /* copy_rop */ psw_fill_path, psw_stroke_path, psw_fill_mask, gdev_vector_fill_trapezoid, gdev_vector_fill_parallelogram, gdev_vector_fill_triangle, NULL /****** WRONG ******/, /* draw_thin_line */ psw_begin_image, NULL, /* image_data */ NULL, /* end_image */ NULL, /* strip_tile_rectangle */ NULL, /******psw_strip_copy_rop******/ NULL, /* get_clipping_box */ NULL, /* begin_typed_image */ psdf_get_bits_rectangle, NULL, /* map_color_rgb_alpha */ psdf_create_compositor \
	}
...
private
const char *const psw_1_procset[] = {
    0};
...
psw_check_erasepage(gx_device_pswrite *pdev)
{
	...
	if(pdev->page_fill.color != gx_no_color_index) {
		if(pdev->page_fill.rect.p.x != pdev->page_fill.rect.q.x && pdev->page_fill.rect.p.y != pdev->page_fill.rect.q.y) {
	    ...
	    gx_path_type_t type)
{
	...
	if(pdev->path_state.num_points > 0)
		stream_puts(s, (pdev->path_state.move ? (pdev->path_state.num_points == 1 ? "m\n" : "P\n") : "p\n"));
	if(dx1 == 0 && dy1 == 0) {
	      ...
	      floatp x_start, floatp y_start, gx_path_type_t type)
{
	...
	stream_puts(gdev_vector_stream(vdev),
		    (pdev->path_state.num_points > 0 && pdev->path_state.move ? "H\n" : "h\n"));
	pdev->path_state.num_points = 0;
	pdev->path_state.move = 0;
	if((gdev_vector_stream(vdev))->end_status == ERRC)
		return_error(gs_error_ioerror);
	      ...
	      gx_color_index zero, gx_color_index one)
{
	...
	if(w <= 0 || h <= 0)
		return 0;
	(*dev_proc(vdev->bbox_device, copy_mono))((gx_device *)vdev->bbox_device, data, data_x, raster, id,
						  x, y, w, h, zero, one);
	       ...
	       int x, int y, int w, int h)
{
	...
	if(w <= 0 || h <= 0)
		return 0;
	(*dev_proc(vdev->bbox_device, copy_color))((gx_device *)vdev->bbox_device, data, data_x, raster, id,
						   x, y, w, h);
	/*
	...
	if(h == 1 && !memcmp(bits, bits + 3, (w - 1) * 3)) {
		return (*dev_proc(dev, fill_rectangle))(dev, x, y, w, h, (bits[0] << 16) + (bits[1] << 8) + bits[2]);
	}
		...
		const gx_device_color *pdcolor, const gx_clip_path *pcpath)
{
	...
	return (vdev->bbox_device == 0 ? 0 : dev_proc(vdev->bbox_device, stroke_path)((gx_device *)vdev->bbox_device, pis, ppath, params,
										      pdcolor, pcpath));
}
	      ...
	      gs_logical_operation_t lop, const gx_clip_path *pcpath)
{
	...
	(*dev_proc(vdev->bbox_device, fill_mask))((gx_device *)vdev->bbox_device, data, data_x, raster, id,
						  x, y, w, h, pdcolor, depth, lop, pcpath);
	/* Update the clipping path now. */
	gdev_vector_update_clip_path(vdev, pcpath);
...
private
const gx_image_enum_procs_t psw_image_enum_procs = {
...
#define MAX_IMAGE_OP 10   /* imagemask\n */
	...
	if(prect && !(prect->p.x == 0 && prect->p.y == 0 && prect->q.x == pim->Width && prect->q.y == pim->Height))
		goto fail;
	switch(pim->format) {
