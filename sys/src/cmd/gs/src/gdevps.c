...
private
int psw_open_printer(gx_device* dev);
private
int psw_close_printer(gx_device* dev);

/* ---------------- Device definition ---------------- */

/* Device procedures */
private
dev_proc_open_device(psw_open);
private
dev_proc_output_page(psw_output_page);
private
dev_proc_close_device(psw_close);
private
dev_proc_fill_rectangle(psw_fill_rectangle);
private
dev_proc_copy_mono(psw_copy_mono);
private
dev_proc_copy_color(psw_copy_color);
private
dev_proc_put_params(psw_put_params);
private
dev_proc_get_params(psw_get_params);
private
dev_proc_fill_path(psw_fill_path);
private
dev_proc_stroke_path(psw_stroke_path);
private
dev_proc_fill_mask(psw_fill_mask);
private
dev_proc_begin_image(psw_begin_image);
...
#define psw_procs                                                              \
	{                                                                      \
		psw_open, gx_upright_get_initial_matrix,                       \
		    NULL, /* sync_output */                                    \
		    psw_output_page, psw_close, gx_default_rgb_map_rgb_color,  \
		    gx_default_rgb_map_color_rgb, psw_fill_rectangle,          \
		    NULL,                                /* tile_rectangle */  \
		    psw_copy_mono, psw_copy_color, NULL, /* draw_line */       \
		    psdf_get_bits, psw_get_params, psw_put_params,             \
		    NULL, /* map_cmyk_color */                                 \
		    ...
		    NULL,                                 /* get_band */       \
		    NULL,                                 /* copy_rop */       \
		    psw_fill_path, psw_stroke_path, psw_fill_mask,             \
		    gdev_vector_fill_trapezoid,                                \
		    gdev_vector_fill_parallelogram, gdev_vector_fill_triangle, \
		    NULL /****** WRONG ******/, /* draw_thin_line */           \
		    psw_begin_image, NULL,      /* image_data */               \
		    NULL,                       /* end_image */                \
		    NULL,                       /* strip_tile_rectangle */     \
		    NULL, /******psw_strip_copy_rop******/                     \
		    NULL, /* get_clipping_box */                               \
		    NULL, /* begin_typed_image */                              \
		    psdf_get_bits_rectangle, NULL, /* map_color_rgb_alpha */   \
		    psdf_create_compositor                                     \
	}

const gx_device_pswrite gs_pswrite_device = {
    std_device_dci_type_body(
        gx_device_pswrite, 0, "pswrite", &st_device_pswrite,
        DEFAULT_WIDTH_10THS* X_DPI / 10, DEFAULT_HEIGHT_10THS* Y_DPI / 10,
        X_DPI, Y_DPI, 3, 24, 255, 255, 256, 256),
    psw_procs, psdf_initial_values(psdf_version_default,
                                   1 /*true */), /* (ASCII85EncodePages) */
...
const gx_device_pswrite gs_epswrite_device = {
    std_device_dci_type_body(
        gx_device_pswrite, 0, "epswrite", &st_device_pswrite,
        DEFAULT_WIDTH_10THS* X_DPI / 10, DEFAULT_HEIGHT_10THS* Y_DPI / 10,
        X_DPI, Y_DPI, 3, 24, 255, 255, 256, 256),
    psw_procs, psdf_initial_values(psdf_version_default,
                                   1 /*true */), /* (ASCII85EncodePages) */
...
private
int psw_beginpage(gx_device_vector* vdev),
    ...
    psw_lineto(gx_device_vector* vdev, floatp x0, floatp y0, floatp x, floatp y,
               gx_path_type_t type),
    psw_curveto(gx_device_vector* vdev, floatp x0, floatp y0, floatp x1,
                floatp y1, floatp x2, floatp y2, floatp x3, floatp y3,
                gx_path_type_t type),
    psw_closepath(gx_device_vector* vdev, floatp x0, floatp y0, floatp x_start,
                  floatp y_start, gx_path_type_t type),
    psw_endpath(gx_device_vector* vdev, gx_path_type_t type);
private
const gx_device_vector_procs psw_vector_procs = {
    /* Page management */
    psw_beginpage,
    /* Imager state */
    psdf_setlinewidth, psdf_setlinecap, psdf_setlinejoin, psdf_setmiterlimit,
    psdf_setdash, psdf_setflat, psdf_setlogop,
    /* Other state */
    psw_can_handle_hl_color,
    psw_setcolors, /* fill & stroke colors are the same */
    psw_setcolors,
    /* Paths */
    psdf_dopath, psw_dorect, psw_beginpath, psw_moveto, psw_lineto, psw_curveto,
    psw_closepath, psw_endpath};
...
const char* const psw_procset[] = {
    ...
    "/P{N 0 gt{N -2 roll moveto p}if}!", "/h{p closepath}!/H{P closepath}!",
    /* <dx> lx - */
    /* <dy> ly - */
    /* <dx2> <dy2> <dx3> <dy3> v - */
    /* <dx1> <dy1> <dx2> <dy2> y - */
    ...
    "/|={pop exch 4 1 roll 1 array astore cvx 3 array astore cvx exch 1 index "
    "def exec}!",
...
private
const char* const psw_1_procset[] = {0};

private
const char* const psw_1_x_procset[] = {
    /* <w> <h> <name> <length> <src> |X <w> <h> <data> */
    /* <w> <h> <name> (<length>|) $X <w> <h> <data> */
    "/|X{exch string readhexstring |=}!/$X{+ @ |X}!",
    /* - @X <hexsrc> */
    "/@X{{currentfile ( ) readhexstring pop}}!", 0};

private
const char* const psw_1_5_procset[] = {
    /* <x> <y> <w> <h> <src> <bpc> Ic - */
    "/Ic{exch Ix false 3 colorimage}!", 0};

private
const char* const psw_2_procset[] = {
...
psw_begin_file(gx_device_pswrite* pdev, const gs_rect* pbbox)
{
	...
	if((code = psw_begin_file_header(
	        f, (gx_device*)pdev, pbbox, &pdev->pswrite_common,
...
private
int
image_cache_lookup(gx_device_pswrite* pdev, gx_bitmap_id id, int width,
                   int height, bool enter)
{
	int i1, i2;
...
psw_image_stream_setup(gx_device_pswrite* pdev, bool binary_ok)
{
	...
	if(pdev->pswrite_common.LanguageLevel >= 2 || binary_ok) {
		...
		code = psdf_begin_binary((gx_device_psdf*)pdev,
		                         pdev->image_writer);
	} else {
...
private
int
psw_put_image_bits(gx_device_pswrite* pdev, const char* op, const byte* data,
                   int data_x, uint raster, int width, int height, int depth)
{
	int code;
              ...
              int depth)
{
	...
	if(encode & 2) {
		code =
		    psdf_CFE_binary(pdev->image_writer, width, height, false);
		if(code < 0)
			return code;
	}
	return psw_put_image_bits(pdev, op, data, data_x, raster, width, height,
	                          depth);
}
private
int
psw_image_write(gx_device_pswrite* pdev, const char* imagestr, const byte* data,
                int data_x, uint raster, gx_bitmap_id id, int x, int y,
                int width, int height, int depth)
{
	...
	if(depth == 1 && width > 16 &&
	   pdev->pswrite_common.LanguageLevel >= 2) {
		/*
	...
	if(id == gx_no_bitmap_id || width_bits * (ulong)height > 8000) {
		static const char* const uncached[4] = {"@", "@X", "@F", "@C"};

		stream_puts(s, uncached[encode]);
		op = imagestr;
		strcpy(endstr, "\n");
	} else {
		static const char* const cached[4] = {"$", "$X", "$F", "$C"};
...
{
	pprintg6(s, "[%g %g %g %g %g %g]", pmat->xx, pmat->xy, pmat->yx,
	...
	if(pdev->page_fill.color != gx_no_color_index) {
		if(pdev->page_fill.rect.p.x != pdev->page_fill.rect.q.x &&
		   pdev->page_fill.rect.p.y != pdev->page_fill.rect.q.y) {
			code = gdev_vector_fill_rectangle(
			    (gx_device*)pdev, pdev->page_fill.rect.p.x,
			    pdev->page_fill.rect.p.y,
			    pdev->page_fill.rect.q.x - pdev->page_fill.rect.p.x,
			    pdev->page_fill.rect.q.y - pdev->page_fill.rect.p.y,
...
psw_beginpage(gx_device_vector* vdev)
{
	...
	code = psw_write_page_header(
	    s, (gx_device*)vdev, &pdev->pswrite_common, true,
              ...
              const gx_drawing_color* pdc)
{
	const gs_imager_state* pis =
	    NULL; /* High level color is not implemented yet. */
           ...
           gx_path_type_t type)
{
	...
	pprintg4(gdev_vector_stream(vdev), "%g %g %g %g rf\n", fixed2float(x0),
	         fixed2float(y0), fixed2float(x1 - x0), fixed2float(y1 - y0));
...
psw_beginpath(gx_device_vector* vdev, gx_path_type_t type)
{
	...
	if(type & gx_path_type_clip) {
		/*
		 * This approach doesn't work for clip + fill or stroke, but
		 * that
           ...
           gx_path_type_t type)
{
	...
	if(pdev->path_state.num_points > pdev->path_state.move)
		...
		/*
		 * Two consecutive movetos -- possible only if a zero-length
		 * line
		 * was discarded.
		 */
...
private
int
psw_curveto(gx_device_vector* vdev, floatp x0, floatp y0, floatp x1, floatp y1,
            floatp x2, floatp y2, floatp x3, floatp y3, gx_path_type_t type)
{
	stream* s = gdev_vector_stream(vdev);
...
psw_open(gx_device* dev)
{
	...
	pdev->image_writer =
	    gs_alloc_struct(mem, psdf_binary_writer, &st_psdf_binary_writer,
...
psw_get_params(gx_device* dev, gs_param_list* plist)
{
	...
	if(code < 0)
		return code;
	if((ecode = param_write_float(plist, "LanguageLevel",
	                              &pdev->pswrite_common.LanguageLevel)) < 0)
		return ecode;
	return code;
}
...
psw_put_params(gx_device* dev, gs_param_list* plist)
{
	...
	switch(code = param_read_float(plist, (param_name = "LanguageLevel"),
	                               &ll)) {
	case 0:
	 ...
	 */
	{
		static const psdf_version vv[3] = {psdf_version_level1,
		                                   psdf_version_level1_color,
		                                   psdf_version_level2};
...
psw_open_printer(gx_device* dev)
{
	...
	{
		int code = gdev_vector_open_file_options(
		    vdev, 512,
		    VECTOR_OPEN_FILE_SEQUENTIAL_OK | VECTOR_OPEN_FILE_BBOX);
...
private
int
psw_copy_mono(gx_device* dev, const byte* data, int data_x, int raster,
              gx_bitmap_id id, int x, int y, int w, int h, gx_color_index zero,
              gx_color_index one)
{
	gx_device_vector* const vdev = (gx_device_vector*)dev;
	gx_device_pswrite* const pdev = (gx_device_pswrite*)vdev;
	...
	if(w <= 0 || h <= 0)
		return 0;
	(*dev_proc(vdev->bbox_device, copy_mono))((gx_device*)vdev->bbox_device,
	                                          data, data_x, raster, id, x,
	                                          y, w, h, zero, one);
	if(one == gx_no_color_index) {
		...
		if(zero != gx_no_color_index) {
			code = (*dev_proc(dev, fill_rectangle))(dev, x, y, w, h,
			                                        zero);
			if(code < 0)
				return code;
		}
	...
	if(code < 0)
		return code;
	return psw_image_write(pdev, op, data, data_x, raster, id, x, y, w, h,
	                       1);
}
...
private
int
psw_copy_color(gx_device* dev, const byte* data, int data_x, int raster,
               gx_bitmap_id id, int x, int y, int w, int h)
{
	...
	(*dev_proc(vdev->bbox_device, copy_color))(
	    (gx_device*)vdev->bbox_device, data, data_x, raster, id, x, y, w,
	    h);
	/*
...
private
int
psw_fill_path(gx_device* dev, const gs_imager_state* pis, gx_path* ppath,
...
private
int
psw_stroke_path(gx_device* dev, const gs_imager_state* pis, gx_path* ppath,
                const gx_stroke_params* params, const gx_device_color* pdcolor,
                const gx_clip_path* pcpath)
{
	gx_device_vector* const vdev = (gx_device_vector*)dev;
	...
	/* Do the right thing for oddly transformed coordinate systems.... */
	{
		...
		if(!gx_dc_is_pure(pdcolor))
			return gx_default_stroke_path(dev, pis, ppath, params,
			                              pdcolor, pcpath);
		set_ctm =
		    (bool)gdev_vector_stroke_scaling(vdev, pis, &scale, &mat);
		gdev_vector_update_clip_path(vdev, pcpath);
		...
		if(set_ctm) {
			stream_puts(s, "q");
			if(is_fzero2(mat.xy, mat.yx) &&
			   is_fzero2(mat.tx, mat.ty))
		...
		code =
		    gdev_vector_dopath(vdev, ppath, gx_path_type_stroke,
	...
	return (vdev->bbox_device == 0
	            ? 0
	            : dev_proc(vdev->bbox_device,
	                       stroke_path)((gx_device*)vdev->bbox_device, pis,
	                                    ppath, params, pdcolor, pcpath));
}

/* Fill a mask. */
private
int
psw_fill_mask(gx_device* dev, const byte* data, int data_x, int raster,
              gx_bitmap_id id, int x, int y, int w, int h,
              ...
              gs_logical_operation_t lop, const gx_clip_path* pcpath)
{
	 ...
	 */
	if(depth > 1 || gdev_vector_update_clip_path(vdev, pcpath) < 0 ||
	   gdev_vector_update_fill_color(vdev, NULL, pdcolor) < 0 ||
	   gdev_vector_update_log_op(vdev, lop) < 0)
		return gx_default_fill_mask(dev, data, data_x, raster, id, x, y,
		                            w, h, pdcolor, depth, lop, pcpath);
	(*dev_proc(vdev->bbox_device, fill_mask))(
	    (gx_device*)vdev->bbox_device, data, data_x, raster, id, x, y, w, h,
	    pdcolor, depth, lop, pcpath);
	/* Update the clipping path now. */
	gdev_vector_update_clip_path(vdev, pcpath);
	return psw_image_write(pdev, ",", data, data_x, raster, id, x, y, w, h,
	                       1);
}
...
private
image_enum_proc_plane_data(psw_image_plane_data);
private
image_enum_proc_end_image(psw_image_end_image);
private
const gx_image_enum_procs_t psw_image_enum_procs = {psw_image_plane_data,
                                                    psw_image_end_image};

/* Start processing an image. */
private
int
psw_begin_image(gx_device* dev, const gs_imager_state* pis,
...
#define MAX_IMAGE_OP 10   /* imagemask\n */
	...
	if(prect &&
	   !(prect->p.x == 0 && prect->p.y == 0 && prect->q.x == pim->Width &&
	...
	if(pim->ImageMask) {
		...
		case gs_color_space_index_Indexed: {
			if(pdev->pswrite_common.LanguageLevel < 2 ||
			   pcs->params.indexed.use_proc ||
			   pim->Decode[0] != 0 ||
			   pim->Decode[1] != (1 << pim->BitsPerComponent) - 1) {
				goto fail;
			}
			pbcs = (const gs_color_space*)&pcs->params.indexed
			           .base_space;
			switch(gs_color_space_get_index(pbcs)) {
			case gs_color_space_index_DeviceGray:
				base_name = "DeviceGray";
				break;
			case gs_color_space_index_DeviceRGB:
				base_name = "DeviceRGB";
				break;
			case gs_color_space_index_DeviceCMYK:
				base_name = "DeviceCMYK";
				break;
			default:
				goto fail;
			}
	...
	if(pdev->pswrite_common.LanguageLevel < 2 && !pim->ImageMask) {
		/*
		 * Restrict ourselves to Level 1 images: bits per component <=
		 * 8,
		 * not indexed.
		 */
	...
	if(binary) {
		 ...
		 * only reason for this is the infamous "short image" problem:
		 * the
		 * image may actually have fewer rows than its height specifies.
		 * If
		 * it weren't for that, we could know the size of the binary
		 * data in
		 * advance.  However, this will change if we compress images.
		 */
		uint bsize =
		    MAX_IMAGE_OP + ((pie->bits_per_row + 7) >> 3) * pie->height;
		...
		if(buffer && bs) {
			...
			gs_free_object(mem, bs,
			               "psw_begin_image(buffer stream)");
			gs_free_object(mem, buffer, "psw_begin_image(buffer)");
			/*
			 * Rather than returning VMerror, we fall back to an
			 * ASCII
	...
	/* Write the image/colorimage/imagemask preamble. */
	{
		...
		if(pim->ImageMask) {
			...
			if(pbcs != pcs) {
				...
				/*
				 * Don't write the table in binary: it might
				 * interfere
				 * with DSC parsing.
				 */
				s_write_ps_string(
				    s, pcs->params.indexed.lookup.table.data,
				    pcs->params.indexed.lookup.table.size,
				    PRINT_ASCII85_OK);
				pprintd1(s, "\n]setcolorspace[0 %d]",
				         (int)pim->Decode[1]);
				pprints2(s, "%s %s",
				         (pim->Interpolate ? "true" : "false"),
				         source);
				op = "IC";
                     ...
                     const gx_image_plane_t* planes, int height, int* rows_used)
{
	...
	for(pi = 0; pi < pie->num_planes; ++pi) {
		...
		psw_put_bits(pdev->image_stream, planes[pi].data,
		             ...
		             planes[pi].raster, pie->bits_per_row, *rows_used);
...
psw_image_end_image(gx_image_enum_common_t* info, bool draw_last)
{
	...
	if(code > 0) {
		...
		if(bs != s) {
			/*
			 * We were buffering a binary image.  Write it now, with
			 * the
			 * DSC comments.
			 */
			...
			gs_free_object(mem, bs,
			               "psw_image_end_image(buffer stream)");
			gs_free_object(mem, buffer,
			               "psw_image_end_image(buffer)");
		}
		stream_puts(s, "\nQ\n");
		if(s->end_status == ERRC)
			return_error(gs_error_ioerror);
