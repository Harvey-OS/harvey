...
/* Test programs */
private
int test1(gs_state*, gs_memory_t*); /* kaleidoscope */
private
int test2(gs_state*, gs_memory_t*); /* pattern fill */
private
int test3(gs_state*, gs_memory_t*); /* RasterOp */
private
int test4(gs_state*, gs_memory_t*); /* set resolution */
private
int test5(gs_state*, gs_memory_t*); /* images */
private
int test6(gs_state*, gs_memory_t*); /* CIE API, snapping */
private
int test7(gs_state*, gs_memory_t*); /* non-monot HT */
private
int test8(gs_state*, gs_memory_t*); /* transp patterns */

#ifdef CAPTURE
#include "k/capture.c"
private
int test10(gs_state*, gs_memory_t*); /* captured data */

#endif
private
int (*tests[])(gs_state*, gs_memory_t*) = {test1, test2, test3, test4, test5,
                                           test6, test7, test8, 0
#ifdef CAPTURE
                                           test10
#endif
...
#define mem ((gs_memory_t*)imem)
	 ...
	 */
	{
		...
		code = param_write_string((gs_param_list*)&list, "OutputFile",
		                          &nstr);
		if(code < 0) {
...
odsf(floatp x, floatp y)
{
	static const byte dither[256] = {
	    };
...
#define rf() (rand() / (1.0 * 0x10000 * 0x8000))
		...
		double x0 = rf(), y0 = rf(), x1 = rf(), y1 = rf(), x2 = rf(),
		       y2 = rf();
...
test2(gs_state* pgs, gs_memory_t* mem)
{
	...
	/*const */ byte tpdata[] = {
	    /* Define a pattern that looks like this:
	       ...
	       x.....
	     */
	    0x3c, 0, 0, 0, 0x04, 0, 0, 0, 0x04, 0, 0, 0,
	    0x3c, 0, 0, 0, 0x40, 0, 0, 0, 0x80, 0, 0, 0};
...
#define pbytes ((byte*)pattern)
	...
	(*dev_proc(dev, copy_rop))(dev, NULL, 0, 0, gx_no_bitmap_id, black2,
	                           &tile, white_black, 100, 100, 150, 150, 0, 0,
	                           rop3_T);
	(*dev_proc(dev, copy_rop))(dev, NULL, 0, 0, gx_no_bitmap_id, black2,
	                           NULL, NULL, 120, 120, 110, 110, 0, 0,
	                           ~rop3_S& rop3_1);
	(*dev_proc(dev, copy_rop))(dev, NULL, 0, 0, gx_no_bitmap_id, black2,
	                           &tile, white_black, 110, 110, 130, 130, 0, 0,
	                           rop3_T ^ rop3_D);
#undef pbytes
	return 0;
}
...
test4(gs_state* pgs, gs_memory_t* mem)
{
	...
	code = param_write_float_array((gs_param_list*)&list, "HWResolution",
	                               &ares);
	if(code < 0) {
...
test5(gs_state* pgs, gs_memory_t* mem)
{
	...
	int code;
	static const byte data3[] = {0x00, 0x44, 0x88, 0xcc, 0x44, 0x88,
...
#define do_image(image, idata)                                                 \
	...
	code = gx_device_begin_typed_image(dev, (gs_imager_state*)pgs, NULL,   \
	                                   (gs_image_common_t*)&image, NULL,   \
	                                   &dcolor, NULL, mem, &info);         \
	/****** TEST code >= 0 ******/                                         \
	planes[0].data = idata;                                                \
	planes[0].data_x = 0;                                                  \
...
#define H 4
	...
	{
		gs_image3_t image3;
		static const byte data3mask[] = {0x60, 0x90, 0x90, 0x60};
		static const byte data3x2mask[] = {0x66, 0x99, 0x99, 0x66,
		                                   0x66, 0x99, 0x99, 0x66};
		...
		code = gx_device_begin_typed_image(
		    dev, (gs_imager_state*)pgs, NULL,
		    (gs_image_common_t*)&image3, NULL, &dcolor, NULL, mem,
		    &info);
		/****** TEST code >= 0 ******/
	...
	for(a = 0; a < n; ++a)
		for(b = 0; b < n; ++b)
			for(c = 0; c < n; ++c) {
				...
				gs_setcolor(pgs, &cc);
				fill_rect1(pgs, a / den + c / den2,
				           b / den + c / den2, size, size);
			}
}
private
float
...
test6(gs_state* pgs, gs_memory_t* mem)
{
	...
	gs_cie_abc* pabc;
	gs_cie_render* pcrd;
	static const gs_vector3 white_point = {1, 1, 1};
	...
	if(code < 0)
		return code;
	gs_cie_render1_initialize(pcrd, NULL, &white_point, NULL, NULL, NULL,
	                          NULL, NULL, NULL, NULL, NULL, &encode_abc,
	                          NULL, NULL);
	...
	cmdev = gs_alloc_struct_immovable(mem, gx_device_cmap, &st_device_cmap,
	                                  "cmap device");
...
test7(gs_state* pgs, gs_memory_t* mem)
{
	...
	static const byte masks[1 * 4 * 4] = {/* 0% */
	                                      0x00, 0x00, 0x00, 0x00,
	                                      /* 25% */
	                                      0x80, 0x40, 0x20, 0x10,
	                                      /* 50% */
	...
	code = gs_ht_set_mask_comp(pht, 0, 4, 4, 4, masks, NULL, NULL);
...
test8(gs_state* pgs, gs_memory_t* mem)
{
	...
	static const byte pdata[] = {
	    0x7f, 0xff, 0x00, 0x03, 0x7f, 0xff, 0x00, 0x0c, 0x50, 0x00, 0x00,
	    0x30, 0x50, 0x00, 0x00, 0xc0, 0x50, 0x00, 0x03, 0x00, 0x50, 0x00,
	    0x0c, 0x00, 0x50, 0x00, 0x30, 0x00, 0x50, 0x00, 0xc0, 0x00, 0xf0,
	    0x00, 0xc0, 0x00, 0xf0, 0x00, 0x30, 0x00, 0xf0, 0x00, 0x0c, 0x00,
	    0xf0, 0x00, 0x03, 0x00, 0xf0, 0x00, 0x00, 0xc0, 0xf0, 0x00, 0x00,
	    0x30, 0xea, 0x55, 0xaa, 0x5c, 0xea, 0x55, 0xaa, 0x57,
	};
...
private
const int8_t outfile[] = "t.pbm";
private
const float ypage_wid = 11.0;
private
const float xpage_len = 17.0;
private
const int rotate_value = 0;
private
const float scale_x = 0.45;
private
const float scale_y = 0.45;
private
const float xmove_origin = 0.0;
private
const float ymove_origin = 0.0;
...
test10(gs_state* pgs, gs_memory_t* mem)
{
	...
	code = param_read_int_array((gs_param_list*)&list, "HWSize", &HWSa);
	if(code < 0) {
		lprintf1("reading HWSize failed! code = %d\n", code);
		exit(1);
	}
	eprintf3("HWSize[%d] = [ %d, %d ]\n", HWSa.size, HWSa.data[0],
	         HWSa.data[1]);
	code = param_read_float_array((gs_param_list*)&list, "HWResolution",
	                              &HWRa);
	if(code < 0) {
		lprintf1("reading Resolution failed! code = %d\n", code);
		exit(1);
	}
	eprintf3("HWResolution[%d] = [ %f, %f ]\n", HWRa.size, HWRa.data[0],
	         HWRa.data[1]);
	code = param_read_float_array((gs_param_list*)&list, "PageSize", &PSa);
	if(code < 0) {
	...
	eprintf3("PageSize[%d] = [ %f, %f ]\n", PSa.size, PSa.data[0],
	         PSa.data[1]);
	code = param_read_long((gs_param_list*)&list, "MaxBitmap", &MaxBitmap);
	if(code < 0) {
	...
	PSa.data = PageSize;
	code = param_write_float_array((gs_param_list*)&list, "PageSize", &PSa);
	if(nstr.data[0] != 'v') {
		...
		code = param_write_float_array((gs_param_list*)&list,
		                               "HWResolution", &HWRa);
		code = param_write_int_array((gs_param_list*)&list, "HWSize",
		                             &HWSa);
		MaxBitmap = 1000000L;
		code = param_write_long((gs_param_list*)&list, "MaxBitmap",
		                        &MaxBitmap);
	}
	gs_c_param_list_read(&list);
	code = gs_putdeviceparams(dev, (gs_param_list*)&list);
	...
	eprintf4("	cliprect = [[%g,%g],[%g,%g]]\n", cliprect.p.x,
	         cliprect.p.y, cliprect.q.x, cliprect.q.y);
	gs_newpath(pgs);
	...
	gs_translate(pgs, 72.0 * (float)xmove_origin,
	             72.0 * (float)ymove_origin);

	gs_rotate(pgs, (float)rotate_value + 270.0);
	gs_scale(pgs, scale_x * 72.0 / 2032.0, scale_y * 72.0 / 2032.0);
