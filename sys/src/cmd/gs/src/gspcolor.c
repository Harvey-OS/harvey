...
private
cs_proc_num_components(gx_num_components_Pattern);
private
cs_proc_base_space(gx_base_space_Pattern);
private
cs_proc_remap_color(gx_remap_Pattern);
private
cs_proc_init_color(gx_init_Pattern);
private
cs_proc_restrict_color(gx_restrict_Pattern);
private
cs_proc_install_cspace(gx_install_Pattern);
private
cs_proc_set_overprint(gx_set_overprint_Pattern);
private
cs_proc_adjust_cspace_count(gx_adjust_cspace_Pattern);
private
cs_proc_adjust_color_count(gx_adjust_color_Pattern);
private
cs_proc_serialize(gx_serialize_Pattern);
...
{
	return (pcs->params.pattern.has_base_space ? -1 - cs_num_components((const gs_color_space *)&(pcs->params.pattern.base_space)) : -1 /* Pattern dictionary only */);
}
...
{
	return (pcs->params.pattern.has_base_space ? (const gs_color_space *)&(pcs->params.pattern.base_space) : NULL);
}
		 ...
		 gx_device *dev, gs_color_select_t select)
{
	...
	return pc->pattern->type->procs.remap_color(pc, pcs, pdc, pis, dev, select);
}
...
gx_install_Pattern(const gs_color_space *pcs, gs_state *pgs)
{
	if(!pcs->params.pattern.has_base_space)
		return 0;
	return (*pcs->params.pattern.base_space.type->install_cspace)((const gs_color_space *)&pcs->params.pattern.base_space, pgs);
}
...
{
	if(pcs->params.pattern.has_base_space)
		(*pcs->params.pattern.base_space.type->adjust_cspace_count)((const gs_color_space *)&pcs->params.pattern.base_space, delta);
}
			...
			const gs_color_space *pcs, int delta)
{
	...
	if(pcs && pcs->params.pattern.has_base_space)
		(*pcs->params.pattern.base_space.type->adjust_color_count)(pcc, (const gs_color_space *)&pcs->params.pattern.base_space,
									   delta);
}
...
ENUM_PTRS_END_PROC
private
RELOC_PTRS_WITH(cs_Pattern_reloc_ptrs, gs_color_space *pcs)
{
	if(!pcs->params.pattern.has_base_space)
		return;
