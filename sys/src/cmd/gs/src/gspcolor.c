...
gs_private_st_composite(st_color_space_Pattern, gs_paint_color_space,
                        "gs_color_space_Pattern", cs_Pattern_enum_ptrs,
                        cs_Pattern_reloc_ptrs);
private
...
const gs_color_space_type gs_color_space_type_Pattern = {
    gs_color_space_index_Pattern, false, false, &st_color_space_Pattern,
    gx_num_components_Pattern, gx_base_space_Pattern, gx_init_Pattern,
    gx_restrict_Pattern, gx_no_concrete_space, gx_no_concretize_color, NULL,
    gx_remap_Pattern, gx_install_Pattern, gx_set_overprint_Pattern,
    gx_adjust_cspace_Pattern, gx_adjust_color_Pattern, gx_serialize_Pattern,
    gx_cspace_no_linear};
 ...
 */
int
gs_make_pattern_common(gs_client_color* pcc, const gs_pattern_template_t* ptemp,
                       const gs_matrix* pmat, gs_state* pgs, gs_memory_t* mem,
...
{
	return (pcs->params.pattern.has_base_space
                 ...
                 gx_device* dev, gs_color_select_t select)
{
	...
	return pc->pattern->type->procs.remap_color(pc, pcs, pdc, pis, dev,
	                                            select);
}
...
gx_serialize_Pattern(const gs_color_space* pcs, stream* s)
{
	...
	if(code < 0)
		return code;
	code =
	    sputs(s, (byte*)&p->has_base_space, sizeof(p->has_base_space), &n);
	if(code < 0)
		return code;
	if(!p->has_base_space)
		return 0;
