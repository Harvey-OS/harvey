...
private
int alloc_rb(gs_memory_t* mem, cmyrow** rb, int rows);
private
int alloc_line(gs_memory_t* mem, cmyrow* row, int cols);
private
void add_cmy8(cmyrow* rb, char c, char m, char y);
private
void write_cpass(cmyrow* buf, int rows, int pass, FILE* pstream);
private
void free_rb_line(gs_memory_t* mem, cmyrow* rbuf, int rows, int cols);
...
#define ccr_prn_device(procs, dev_name, margin, num_comp, depth, max_gray,     \
                       max_rgb, print_page)                                    \
	{                                                                      \
		prn_device_body(                                               \
		    gx_device_ccr, procs, dev_name, DEFAULT_WIDTH_10THS_A3,    \
		    DEFAULT_HEIGHT_10THS_A3, X_DPI, Y_DPI, margin, margin,     \
		    margin, margin, num_comp, depth, max_gray, max_rgb,        \
		    max_gray + 1, max_rgb + 1, print_page)                     \
	}

/* For CCR, we need our own color mapping procedures. */
private
dev_proc_map_rgb_color(ccr_map_rgb_color);
private
dev_proc_map_color_rgb(ccr_map_color_rgb);

/* And of course we need our own print-page routine. */
private
dev_proc_print_page(ccr_print_page);

/* The device procedures */
private
gx_device_procs ccr_procs =
    prn_color_procs(gdev_prn_open, gdev_prn_output_page, gdev_prn_close,
                    ccr_map_rgb_color, ccr_map_color_rgb);

/* The device descriptors themselves */
gx_device_ccr far_data gs_ccr_device =
    ccr_prn_device(ccr_procs, "ccr", 0.2, 3, 8, 1, 1, ccr_print_page);

/* ------ Color mapping routines ------ */
/* map an rgb color to a ccr cmy bitmap */
private
gx_color_index
ccr_map_rgb_color(gx_device* pdev, const uint16_t cv[])
{
	uint16_t r, g, b;
...
ccr_print_page(gx_device_printer* pdev, FILE* pstream)
{
	...
	if((in = (byte*)gs_malloc(pdev->memory, line_size, 1, "gsline")) ==
	   NULL)
...
private
int
alloc_rb(gs_memory_t* mem, cmyrow** rb, int rows)
{
...
private
int
alloc_line(gs_memory_t* mem, cmyrow* row, int cols)
{
	int suc;
	suc =
	    ((row->cbuf =
	          (unsigned char*)gs_malloc(mem, cols, 1, row->cname)) &&
	     (row->mbuf =
	          (unsigned char*)gs_malloc(mem, cols, 1, row->mname)) &&
...
private
void
free_rb_line(gs_memory_t* mem, cmyrow* rbuf, int rows, int cols)
{
	int i;
	for(i = 0; i < rows; i++) {
		if(rbuf[i].is_used) {
