...
#define x_offset(ty, cdev) \
	((cdev)->phase.x + (((ty) + (cdev)->phase.y) / (cdev)->tiles.rep_height) * (cdev)->tiles.rep_shift)
		     ...
		     int x, int y, int w, int h)
{
	...
	{
		/* Copy the run. */
		int code = (*dev_proc(cdev->target, copy_color))(cdev->target, data_row, sourcex + txrun - x, raster,
		     ...
		     int x, int y, int w, int h, gx_color_index color, int depth)
{
	...
	{
		/* Copy the run. */
		int code = (*dev_proc(cdev->target, copy_alpha))(cdev->target, data_row, sourcex + txrun - x, raster,
			 ...
			 int phase_x, int phase_y, gs_logical_operation_t lop)
{
	...
	{
		/* Copy the run. */
		int code = (*dev_proc(cdev->target, strip_copy_rop))(cdev->target, data_row, sourcex + txrun - x, raster,
