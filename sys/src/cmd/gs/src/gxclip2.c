...
/* Device procedures */
private
dev_proc_fill_rectangle(tile_clip_fill_rectangle);
private
dev_proc_copy_mono(tile_clip_copy_mono);
private
dev_proc_copy_color(tile_clip_copy_color);
private
dev_proc_copy_alpha(tile_clip_copy_alpha);
private
dev_proc_strip_copy_rop(tile_clip_strip_copy_rop);

/* The device descriptor. */
private
const gx_device_tile_clip gs_tile_clip_device = {
    std_device_std_body_open(gx_device_tile_clip, 0, "tile clipper", 0, 0, 1,
                             1),
    {gx_default_open_device, gx_forward_get_initial_matrix,
     ...
     tile_clip_fill_rectangle, gx_default_tile_rectangle, tile_clip_copy_mono,
     tile_clip_copy_color, gx_default_draw_line, gx_forward_get_bits,
     ...
     gx_forward_get_alpha_bits, tile_clip_copy_alpha, gx_forward_get_band,
     ...
     tile_clip_strip_copy_rop, gx_forward_get_clipping_box,
...
{
	int code = gx_mask_clip_initialize(cdev, &gs_tile_clip_device,
	                                   (const gx_bitmap*)tiles, tdev, 0, 0,
	                                   mem); /* phase will be reset */
                         ...
                         gx_color_index color)
{
	...
	return (*dev_proc(tdev, strip_tile_rectangle))(
	    tdev, &cdev->tiles, x, y, w, h, gx_no_color_index, color,
...
private
int
tile_clip_copy_mono(gx_device* dev, const byte* data, int sourcex, int raster,
	...
	for(ty = y; ty < y + h; ty += ny) {
		...
		for(tx = x; tx < x + w; tx += nx) {
			...
			(*dev_proc(&cdev->mdev, copy_mono))(
			    (gx_device*)&cdev->mdev, data + (ty - y) * raster,
			    sourcex + tx - x, raster, gx_no_bitmap_id, cx, 0,
			    nx, ny, mcolor0, mcolor1);
			/* Now copy the color through the double mask. */
			code = (*dev_proc(cdev->target, copy_mono))(
...
private
int
tile_clip_copy_color(gx_device* dev, const byte* data, int sourcex, int raster,
                     gx_bitmap_id id, int x, int y, int w, int h)
{
	gx_device_tile_clip* cdev = (gx_device_tile_clip*)dev;
...
private
int
tile_clip_copy_alpha(gx_device* dev, const byte* data, int sourcex, int raster,
                     gx_bitmap_id id, int x, int y, int w, int h,
...
private
int
tile_clip_strip_copy_rop(gx_device* dev, const byte* data, int sourcex,
                         ...
                         gs_logical_operation_t lop)
{
	...
	FOR_RUNS(data_row, txrun, tx, ty)
	{
		...
		int code = (*dev_proc(cdev->target, strip_copy_rop))(
		    cdev->target, data_row, sourcex + txrun - x, raster,
		    gx_no_bitmap_id, scolors, textures, tcolors, txrun, ty,
		    tx - txrun, 1, phase_x, phase_y, lop);
