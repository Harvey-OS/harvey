 ...
 */
private
const clj_paper_size clj_paper_sizes[] = {
 ...
 */
private
const float supported_resolutions[] = {75.0, 100.0, 150.0, 300.0};
...
private
bool
is_supported_resolution(const float HWResolution[2])
{
	int i;
 ...
 */
private
const clj_paper_size*
get_paper_size(const float MediaSize[2], bool* rotatep)
{
	...
	for(i = 0, psize = clj_paper_sizes; i < countof(clj_paper_sizes);
	    i++, psize++) {
...
clj_get_params(gx_device* pdev, gs_param_list* plist)
{
	...
	if(code < 0)
		...
		for(i = 0; i < countof(clj_paper_sizes); ++i) {
			code = gdev_write_input_page_size(
			    i, &mdict, clj_paper_sizes[i].width,
...
private
int
clj_put_params(gx_device* pdev, gs_param_list* plist)
{
	float mediasize[2];
	bool rotate = false;
...
private
void
pack_and_compress_scanline(const byte* pin, int in_size, byte* pout[3],
                           int out_size[3])
{
#define BUFF_SIZE                                                              \
	...
	for(i = 0; i < 3; i++) {
		...
		if(p_start == p_end)
			...
			out_size[i] =
			    gdev_pcl_mode2compress(p_start, p_end, pout[i]);
	}
...
clj_print_page(gx_device_printer* pdev, FILE* prn_stream)
{
	...
	if((cdata[0] = gs_alloc_bytes(mem, 3 * clsize,
	                              "clj_print_page(cdata)")) == 0) {
	...
	if(pclj->rotated) {
		...
		imageable_height =
		    pdev->height - (2 * psize->offsets.y) * ss_res;
	} else {
		imageable_width = pdev->width - (2 * psize->offsets.y) * ss_res;
		imageable_height =
		    pdev->height - (2 * psize->offsets.x) * fs_res;
	}
	...
	fprintf(prn_stream,
	        ...
	        psize->tag, pclj->rotated, (int)(pdev->HWResolution[0]),
	...
	for(i = 0; i < imageable_height; i++) {
		...
		/* The 'lsize' bytes of data have the blank margin area at the
		 * end due	*/
...
private
gx_device_procs cljet5_procs = {CLJ_PROCS(clj_get_params, clj_put_params)};

/* CLJ device structure */
#define CLJ_DEVICE_BODY(procs, dname, rotated)                                 \
	prn_device_body(                                                       \
	    gx_device_clj, procs, /* procedures */                             \
	    dname,                /* device name */                            \
	    ...
	    0.167, 0.167, 3, /* num_components - 3 colors, 1 bit per pixel */  \
...
private
int
clj_pr_put_params(gx_device* pdev, gs_param_list* plist)
{
	float mediasize[2];
	int code = 0;
	bool rotate = false;
	...
	if(have_pagesize) {
		if(get_paper_size(mediasize, &rotate) == 0)
			return_error(gs_error_rangecheck);
		if(rotate) {
			/* We need to rotate the requested page size, so
			 * synthesize a new	*/
			/* parameter list in front of the requestor's list to
			 * force the	*/
			/* rotated page size.
			 */
			...
			code = param_write_float_array((gs_param_list*)&alist,
			                               ".MediaSize", &pf_array);
			gs_c_param_list_read(&alist);

			/* stick this synthesized parameter on the front of the
			 * existing list */
			gs_c_param_list_set_target(&alist, plist);
			if((code = gdev_prn_put_params(
			        pdev, (gs_param_list*)&alist)) >= 0)
				pclj->rotated = true;
...
private
gx_device_procs cljet5pr_procs = {
