	...
	if(r_has_type(&pfdata->u.cid0.DataSource, t_null)) {
		...
		if(r_has_type(pgdata, t_string)) { /* single string */
			...
			data = pgdata->value.bytes + base;
			  ...
			  * string_array_access_proc in zfont42.c, but it also
			  * has to
			  * deal with the case where the requested string crosses
			  * array
			  * elements.
			  */
			uint32_t skip = base;
			...
			for(;; skip -= size, ++index) {
				int code = array_get(pfont->memory, pgdata,
				                     index, &rstr);
			...
			if(count <= size) {
				...
				if(data == 0) { /* no buffer provided */
					data = gs_alloc_string(
					    pfont->memory, count,
					    "cid0_read_bytes");
					if(data == 0)
						return_error(e_VMerror);
					gdfont = (gs_font*)
					    pfont; /* newly allocated */
				}
				memcpy(data, rstr.value.bytes + skip, size);
				copied = size;
				while(copied < count) {
					int code =
					    array_get(pfont->memory, pgdata,
					              ++index, &rstr);
					...
					if(size > count - copied)
						size = count - copied;
					memcpy(data + copied, rstr.value.bytes,
					       size);
					copied += size;
				}
			}
		...
		check_read_known_file(s, &pfdata->u.cid0.DataSource,
		                      return_error);
		if(sseek(s, base) < 0)
              ...
              int* pfidx)
{
	...
	}
	{
		...
		uint num_bytes =
		    pfont->cidata.FDBytes + pfont->cidata.common.GDBytes;
		uint32_t base =
		    pfont->cidata.CIDMapOffset + glyph_index * num_bytes;
		uint32_t gidx, fidx_next, gidx_next;
		int rcode = cid0_read_bytes(
		    pfont, base, (uint32_t)(num_bytes * 2), fd_gd, &gdata);
		gs_glyph_data_t orig_data;

		if(rcode < 0)
			return rcode;
		orig_data = gdata;
		if((code = get_index(&gdata, pfont->cidata.FDBytes, &fidx)) <
		       0 ||
		   (code = get_index(&gdata, pfont->cidata.common.GDBytes,
		                     &gidx)) < 0 ||
		   (code = get_index(&gdata, pfont->cidata.FDBytes,
		                     &fidx_next)) < 0 ||
		   (code = get_index(&gdata, pfont->cidata.common.GDBytes,
		                     &gidx_next)) < 0)
			DO_NOTHING;
		...
		*pfidx = (int)fidx;
		if(pgd == 0)
			return 0;
		return cid0_read_bytes(pfont, gidx, gidx_next - gidx, NULL,
		                       pgd);
	}
}
...
private
int
z9_glyph_outline(gs_font* font, int WMode, gs_glyph glyph,
                 const gs_matrix* pmat, gx_path* ppath, double sbw[4])
{
	...
	ocode = zcharstring_outline(pfcid->cidata.FDArray[fidx], WMode, &gref,
	                            &gdata, pmat, ppath, sbw);
...
private
int
z9_glyph_info(gs_font* font, gs_glyph glyph, const gs_matrix* pmat, int members,
...
fd_array_element(i_ctx_t* i_ctx_p, gs_font_type1** ppfont, ref* prfd)
{
	...
	case 1:
		...
		if(code < 0)
			return code;
		code = build_proc_name_refs(imemory, &build, "%Type1BuildChar",
		                            "%Type1BuildGlyph");
		break;
	case 2:
		...
		if(code < 0)
			return code;
		code = build_proc_name_refs(imemory, &build, "%Type2BuildChar",
		                            "%Type2BuildGlyph");
		break;
   ...
   */
	...
	if(event_data == NULL) {
		...
		for(i = 0; i < pfcid->cidata.FDArray_size; ++i) {
			if(pfcid->cidata.FDArray[i]->data.parent ==
			   (gs_font_base*)pfcid)
...
zbuildfont9(i_ctx_t* i_ctx_p)
{
	...
	build_proc_refs build;
	int code =
	    build_proc_name_refs(imemory, &build, NULL, "%Type9BuildGlyph");
	...
	if(code < 0 ||
	   ...
	   (code = dict_int_param(op, "FDBytes", 0, MAX_FDBytes, -1,
	                          &FDBytes)) < 0)
		return code;
	/*
	...
	if(r_has_type(&GlyphDirectory, t_null)) {
		...
		if((code = dict_find_string(op, "GlyphData", &pGlyphData)) <
		       0 ||
		...
		if(r_has_type(&GlyphData, t_integer)) {
			...
			if((code = dict_find_string(op, "DataSource", &pds)) <
			   0)
				return code;
			check_read_file(ignore_s, pds);
			DataSource = *pds;
		} else {
			if(!r_has_type(&GlyphData, t_string) &&
			   !r_is_array(&GlyphData))
				return_error(e_typecheck);
	...
	if(code >= 0)
		code = gs_notify_register(&pfont->notify_list,
		                          notify_remove_font_type9, pfont);
	if(code >= 0) {
		for(i = 0; i < FDArray_size; ++i) {
...
ztype9mapcid(i_ctx_t* i_ctx_p)
{
	...
	pfcid = (gs_font_cid0*)pfont;
	gdata.memory = pfont->memory;
	code = pfcid->cidata.glyph_data(
	    (gs_font_base*)pfcid,
	    (gs_glyph)(gs_min_cid_glyph + op->value.intval), &gdata, &fidx);
	...
	if(code < 0) { /* failed to load glyph data, put CID 0 */
		...
		code = pfcid->cidata.glyph_data(
		    (gs_font_base*)pfcid,
		    (gs_glyph)(gs_min_cid_glyph + default_fallback_CID), &gdata,
		    &fidx);

		if(code < 0) {
			if_debug1('J', "[J]ztype9cidmap() could not load "
			               "default glyph (CID %d)\n",
			          op->value.intval);
			return_error(e_invalidfont);
		}
	...
	make_const_string(
	    op - 1, a_readonly | imemory_space((gs_ref_memory_t*)pfont->memory),
	    gdata.bits.size, gdata.bits.data);
	make_int(op, fidx);
	return code;
}
...
const op_def zfcid0_op_defs[] = {{"2.buildfont9", zbuildfont9},
