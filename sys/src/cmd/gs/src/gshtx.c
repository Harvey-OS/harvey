...
private
void
free_comps(gs_memory_t* pmem, void* pvht, client_name_t cname)
{
...
private
float
null_closure_transfer(floatp val,
 ...
 */
int
gs_ht_build(gs_ht** ppht, uint num_comps, gs_memory_t* pmem)
{
	gs_ht* pht;
	...
	*ppht = 0;
	...
	rc_alloc_struct_1(pht, gs_ht, &st_gs_ht, pmem,
	                  return_error(gs_error_VMerror), "gs_ht_build");
	phtc = gs_alloc_struct_array(pmem, num_comps, gs_ht_component,
	                             &st_ht_comp_element, "gs_ht_build");
	if(phtc == 0) {
 ...
 */
int
gs_ht_set_spot_comp(gs_ht* pht, int comp, floatp freq, floatp angle,
                    float (*spot_func)(floatp, floatp), bool accurate,
                    gs_ht_transfer_proc transfer, const void* client_data)
{
 ...
 */
int
gs_ht_set_threshold_comp(gs_ht* pht, int comp, int width, int height,
...
check_ht(gs_ht* pht)
{
	...
	for(i = 0; i < num_comps; i++) {
		gs_ht_component* phtc =
		    &(pht->params.ht_multiple.components[i]);
...
private
void
build_transfer_map(gs_ht_component* phtc, gx_transfer_map* pmap)
{
	...
	for(i = 0; i < transfer_map_size; i++) {
		float fval = proc(i * (1 / (double)(transfer_map_size - 1)),
		                  pmap, client_info);
...
alloc_ht_order(const gs_ht* pht, gs_memory_t* pmem, byte* comp2order)
{
	int num_comps = pht->params.ht_multiple.num_comp;
	gx_ht_order_component* pocs = gs_alloc_struct_array(
	    pmem, pht->params.ht_multiple.num_comp, gx_ht_order_component,
	    &st_ht_order_component_element, "alloc_ht_order");
	int inext = 0;
	int i;
	...
	for(i = 0; i < num_comps; i++) {
		gs_ht_component* phtc =
		    &(pht->params.ht_multiple.components[i]);
		gx_transfer_map* pmap = gs_alloc_struct(
		    pmem, gx_transfer_map, &st_transfer_map, "alloc_ht_order");

		if(pmap == 0) {
			int j;

			for(j = 0; j < inext; j++)
				gs_free_object(pmem, pocs[j].corder.transfer,
				               "alloc_ht_order");
...
private
int
build_component(gs_ht_component* phtc, gx_ht_order* porder, gs_state* pgs,
                gs_memory_t* pmem)
{
	if(phtc->type == ht_type_spot) {
		...
		code = gx_ht_process_screen_memory(
		    &senum, pgs, &phtc->params.ht_spot.screen,
		    phtc->params.ht_spot.accurate_screens, pmem);
		if(code < 0)
			return code;
		...
		*porder = senum.order;
		...
		porder->params.N1 = 0;
		porder->params.R1 = 1;
		code = gx_ht_alloc_threshold_order(
		    porder, phtc->params.ht_threshold.width,
		    phtc->params.ht_threshold.height, 256, pmem);
		if(code < 0)
			return code;
		gx_ht_construct_threshold_order(
		    porder, phtc->params.ht_threshold.thresholds.data);
		/*
		 * gx_ht_construct_threshold_order wipes out transfer map
		 * pointer,
		 * restore it here.
		 */
...
private
void
free_order_array(gx_ht_order_component* pocs, int num_comps, gs_memory_t* pmem)
{
	int i;
 ...
 */
int
gs_ht_install(gs_state* pgs, gs_ht* pht)
{
	int code = 0;
	...
	for(i = 0; i < num_comps; i++) {
		...
		code = build_component(&(pht->params.ht_multiple.components[i]),
		                       &(pocs[j].corder), pgs, pmem);

		if((code >= 0) && (j != 0)) {
			gx_ht_cache* pcache;

			pcache = gx_ht_alloc_cache(
			    pmem, 4, pocs[j].corder.raster *
...
private
int
create_mask_bits(const byte* mask1, const byte* mask2, int width, int height,
                 gx_ht_bit* bits)
{
	/*
	...
	for(y = 0; y < height; ++y)
		for(x = 0; x < width; ++x) {
			...
			if((mask1[offset] ^ mask2[offset]) & bit_mask) {
				if(bits)
					gx_ht_construct_bit(&bits[count], width,
					                    y * width + x);
				++count;
			}
		}
...
create_mask_order(gx_ht_order* porder, gs_state* pgs,
                  const gs_client_order_halftone* phcop, gs_memory_t* mem)
{
	...
	for(prev_mask = masks, num_bits = 0, i = 0; i < num_levels - 1;
	    ++i, prev_mask += bytes_per_mask)
		num_bits +=
		    create_mask_bits(prev_mask, prev_mask + bytes_per_mask,
	...
	for(prev_mask = masks, num_bits = 0, i = 0; i < num_levels - 1;
		...
		num_bits += create_mask_bits(
		    prev_mask, prev_mask + bytes_per_mask, phcop->width,
		    phcop->height, ((gx_ht_bit*)porder->bit_data) + num_bits);
	}
...
private
const gs_client_order_ht_procs_t mask_order_procs = {create_mask_order};
 ...
 */
int
gs_ht_set_mask_comp(gs_ht* pht, int component_index, int width, int height,
                    int num_levels,
