	...
	sw:
		...
		opt:
			if(pe_op == gs_pe_lineto) {
				...
				/*
				 * Merge collinear horizontal or vertical line
				 * segments
				 * going in the same direction.
				 */
				if(vs[0].x == line_end.x) {
					if(vs[0].x == line_start.x &&
					   coord_between(line_start.y,
					                 line_end.y, vs[0].y)) {
						line_end.y = vs[0].y;
						continue;
					}
			...
			if(incomplete_line) {
				if(need_moveto) { /* see gs_pe_moveto case */
					code = gdev_vector_dopath_segment(
					    &state, gs_pe_moveto, &line_start);
					if(code < 0)
						return code;
					need_moveto = false;
				}
				code = gdev_vector_dopath_segment(
				    &state, gs_pe_lineto, &line_end);
				if(code < 0)
					return code;
				line_start = line_end;
		...
		case gs_pe_moveto:
			/*
			 * A bug in Acrobat Reader 4 causes it to draw a single
			 * pixel
			 * for a fill with an isolated moveto.  If we're doing a
			 * fill
			 * without a stroke, defer emitting a moveto until we
			 * know that
			 * the subpath has more elements.
			 */
			line_start = vs[0];
			if(!(type & gx_path_type_stroke) &&
			   (type & gx_path_type_fill)) {
				need_moveto = true;
				continue;
			}
			goto draw;
                              ...
                              int open_options)
{
	...
	if((vdev->strmbuf = gs_alloc_bytes(vdev->v_memory, strmbuf_size,
	   ...
	   (vdev->strm = s_alloc(vdev->v_memory, "vector_open(strm)")) == 0 ||
	   ((open_options & VECTOR_OPEN_FILE_BBOX) &&
	    (vdev->bbox_device = gs_alloc_struct_immovable(
	         vdev->v_memory, gx_device_bbox, &st_device_bbox,
...
private
int
gdev_vector_update_color(gx_device_vector* vdev, const gs_imager_state* pis,
                         const gx_drawing_color* pdcolor, gx_hl_saved_color* sc,
                                         ...
                                         const gx_drawing_color* pdc))
{
	...
	int code;
	bool hl_color =
	    (*vdev_proc(vdev, can_handle_hl_color))(vdev, pis, pdcolor);
...
{
	return gdev_vector_update_color(vdev, pis, pdcolor,
	                                &vdev->saved_fill_color,
                           ...
                           floatp scale)
{
	if(pis) {
		...
		if(dash_offset != vdev->state.line_params.dash.offset ||
		   ...
		   (pattern_size != 0 &&
			...
			for(i = 0; i < pattern_size; ++i)
				pattern[i] =
				    pis->line_params.dash.pattern[i] * scale;
			code = (*vdev_proc(vdev, setdash))(
			    vdev, pattern, pattern_size, dash_offset);
			if(code < 0)
				return code;
			memcpy(vdev->dash_pattern, pattern,
			       pattern_size * sizeof(float));

			vdev->state.line_params.dash.pattern_size =
			    pattern_size;
		...
		if(pis->line_params.miter_limit !=
		   vdev->state.line_params.miter_limit) {
	...
	if(params) {
		if(params->flatness != vdev->state.flatness) {
			int code =
			    (*vdev_proc(vdev, setflat))(vdev, params->flatness);
	...
	if(pdcolor) {
		int code = gdev_vector_update_color(
		    vdev, pis, pdcolor, &vdev->saved_stroke_color,
		    vdev_proc(vdev, setstrokecolor));
...
gdev_vector_stroke_scaling(const gx_device_vector* vdev,
                           const gs_imager_state* pis, double* pscale,
                           gs_matrix* pmat)
{
	bool set_ctm = true;
	double scale = 1;
	...
	if(set_ctm) {
		 ...
		 * coordinates will get too big: limit the scale factor to
		 * prevent
		 * this from happening.  (This does no harm for other output
		 * formats.)
		 */
		double mxx = pis->ctm.xx / vdev->scale.x,
                        ...
                        const gs_matrix* pmat)
{
	...
	if(pmat) {
		...
		/*
		 * The path element writing procedures all divide the
		 * coordinates
		...
		gs_make_scaling(vdev->scale.x, vdev->scale.y,
		                &state->scale_mat);
	}
	state->first = true;
}
                           ...
                           gs_fixed_point vs[3])
{
	...
	case gs_pe_moveto:
		code = gs_point_transform_inverse(
		    fixed2float(vs[0].x), fixed2float(vs[0].y), pmat, &vp[0]);
		if(code < 0)
			return code;
		if(state->first)
			state->start = vp[0], state->first = false;
		code = vdev_proc(vdev, moveto)(vdev, 0 /*unused*/, 0 /*unused*/,
		                               vp[0].x, vp[0].y, state->type);
		state->prev = vp[0];
		break;
	case gs_pe_lineto:
		code = gs_point_transform_inverse(
		...
		if(code < 0)
			return code;
		code =
		    vdev_proc(vdev, lineto)(vdev, state->prev.x, state->prev.y,
		                            vp[0].x, vp[0].y, state->type);
		state->prev = vp[0];
		break;
	case gs_pe_curveto:
		...
		if(code < 0)
			return code;
		code = gs_point_transform_inverse(
		    fixed2float(vs[1].x), fixed2float(vs[1].y), pmat, &vp[1]);
		if(code < 0)
			return code;
		...
		break;
	case gs_pe_closepath:
		code = vdev_proc(vdev, closepath)(vdev, state->prev.x,
		                                  state->prev.y, state->start.x,
                          ...
                          uint count, bool close, gx_path_type_t type)
{
	...
	if(count > 0) {
		...
		code = (*vdev_proc(vdev, moveto))(vdev, 0.0, 0.0, x, y, type);
		if(code >= 0)
			for(i = 1; i < count && code >= 0; ++i) {
				x_prev = x, y_prev = y;
				code = (*vdev_proc(vdev, lineto))(
				    vdev, x_prev, y_prev,
				    (x = fixed2float(points[i].x) /
				         vdev->scale.x),
				    (y = fixed2float(points[i].y) /
				         vdev->scale.y),
				    type);
			}
		if(code >= 0 && close)
	...
	return (code >= 0 && type != gx_path_type_none
	            ? (*vdev_proc(vdev, endpath))(vdev, type)
	            : code);
}
...
gdev_vector_write_rectangle(gx_device_vector* vdev, fixed x0, fixed y0,
                            fixed x1, fixed y1, bool close,
                            gx_rect_direction_t direction)
{
	...
	if(direction == gx_rect_x_first)
		points[1].x = x1, points[1].y = y0, points[3].x = x0,
		points[3].y = y1;
	else
		points[1].x = x0, points[1].y = y1, points[3].x = x1,
		points[3].y = y0;
	...
	for(; code >= 0 && prect != 0; prect = prect->next)
		if(prect->xmax > prect->xmin && prect->ymax > prect->ymin)
			code = gdev_vector_write_rectangle(
			    vdev, int2fixed(prect->xmin),
			    int2fixed(prect->ymin), int2fixed(prect->xmax),
			    int2fixed(prect->ymax), false, gx_rect_x_first);
	if(code >= 0)
...
int
gdev_vector_update_clip_path(gx_device_vector* vdev, const gx_clip_path* pcpath)
{
	if(pcpath) {
...
int
gdev_vector_begin_image(gx_device_vector* vdev, const gs_imager_state* pis,
                        ...
                        gdev_vector_image_enum_t* pie)
{
	...
	code = gx_image_enum_common_init(
	    (gx_image_enum_common_t*)pie, (const gs_data_image_t*)pim, pprocs,
	    (gx_device*)vdev, num_components, format);
	if(code < 0)
		return code;
	pie->bits_per_pixel = bits_per_pixel * num_components / pie->num_planes;
	pie->default_info = 0;
	pie->bbox_info = 0;
...
int
gdev_vector_end_image(gx_device_vector* vdev, gdev_vector_image_enum_t* pie,
                      bool draw_last, gx_color_index pad)
{
	int code;
	...
	if(pie->default_info) {
		...
		if(pie->y < pie->height && pad != gx_no_color_index) {
			...
			byte* row =
			    gs_alloc_bytes(pie->memory, bytes_per_row,
...
gdev_vector_get_params(gx_device* dev, gs_param_list* plist)
{
	...
	if(code < 0)
		return code;
	ofns.data = (const byte*)vdev->fname, ofns.size = strlen(vdev->fname),
...
gdev_vector_put_params(gx_device* dev, gs_param_list* plist)
{
	...
	case 0:
		...
		if(ofns.size > fname_size)
			ecode = gs_error_limitcheck;
		                       ...
		                       strlen(vdev->fname))) {
			/* The new name is the same as the old name.  Do
			 * nothing. */
			ofns.data = 0;
			break;
	...
	if(ofns.data != 0) {
		...
		if(vdev->file != 0) {
			...
			if(code < 0)
				return code;
			return gdev_vector_open_file_options(
			    vdev, vdev->strmbuf_size, vdev->open_options);
		}
	}
	return 0;
}
	...
	if((code = gdev_vector_prepare_fill(vdev, pis, params, pdevc)) < 0 ||
	   ...
	   (vdev->bbox_device &&
	    (code = (*dev_proc(vdev->bbox_device, fill_path))(
	         (gx_device*)vdev->bbox_device, pis, ppath, params, pdevc,
	         pcpath)) < 0) ||
	   (code = (*vdev_proc(vdev, dopath))(
	        vdev, ppath, (params->rule > 0 ? gx_path_type_even_odd
	                                       : gx_path_type_winding_number) |
	                         gx_path_type_fill | vdev->fill_options,
	        NULL)) < 0)
                        ...
                        const gx_clip_path* pcpath)
{
	...
	if((set_ctm = gdev_vector_stroke_scaling(vdev, pis, &scale, &mat)) !=
	       0 ||
	   ...
	   (vdev->bbox_device &&
	    (code = (*dev_proc(vdev->bbox_device, stroke_path))(
	         (gx_device*)vdev->bbox_device, pis, ppath, params, pdcolor,
	         pcpath)) < 0) ||
	   (code = (*vdev_proc(vdev, dopath))(
	        vdev, ppath, gx_path_type_stroke | vdev->stroke_options,
	        NULL)) < 0)
...
#define y1 ytop
	...
	if(swap_axes)
		...
		points[0].x = points[1].x = y0, points[2].y = x1r,
		points[3].y = x1l, points[2].x = points[3].x = y1;
	else
		points[0].x = x0l, points[1].x = x0r,
		points[0].y = points[1].y = y0, points[2].x = x1r,
		points[3].x = x1l, points[2].y = points[3].y = y1;
#undef y0
#undef y1
	if(vdev->bbox_device) {
...
int
gdev_vector_fill_parallelogram(gx_device* dev, fixed px, fixed py, fixed ax,
...
int
gdev_vector_fill_triangle(gx_device* dev, fixed px, fixed py, fixed ax,
                          fixed ay, fixed bx, fixed by,
                          const gx_device_color* pdevc,
                          gs_logical_operation_t lop)
{
