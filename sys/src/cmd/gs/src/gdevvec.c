			      ...
			      int open_options)
{
	...
	if(vdev->bbox_device) {
		...
		(*dev_proc(vdev->bbox_device, open_device))((gx_device *)vdev->bbox_device);
	}
	return 0;
}
	...
	if(pis) {
		...
		if(dash_offset != vdev->state.line_params.dash.offset ||
		   ...
		   (pattern_size != 0 &&
			...
			code = (*vdev_proc(vdev, setdash))(vdev, pattern, pattern_size, dash_offset);
			if(code < 0)
				return code;
		...
		if(half_width != vdev->state.line_params.half_width) {
			int code = (*vdev_proc(vdev, setlinewidth))(vdev, half_width * 2);
		...
		if(pis->line_params.miter_limit != vdev->state.line_params.miter_limit) {
			int code = (*vdev_proc(vdev, setmiterlimit))(vdev, pis->line_params.miter_limit);
		...
		if(pis->line_params.cap != vdev->state.line_params.cap) {
			int code = (*vdev_proc(vdev, setlinecap))(vdev, pis->line_params.cap);
		...
		if(pis->line_params.join != vdev->state.line_params.join) {
			int code = (*vdev_proc(vdev, setlinejoin))(vdev, pis->line_params.join);
			   ...
			   gs_fixed_point vs[3])
{
	...
	case gs_pe_moveto:
		...
		code = vdev_proc(vdev, moveto)(vdev, 0 /*unused*/, 0 /*unused*/, vp[0].x, vp[0].y,
					       state->type);
		state->prev = vp[0];
		break;
	case gs_pe_lineto:
		...
		if(code < 0)
			return code;
		code = vdev_proc(vdev, lineto)(vdev, state->prev.x, state->prev.y, vp[0].x, vp[0].y,
					       state->type);
		state->prev = vp[0];
		break;
	case gs_pe_curveto:
		...
		code = vdev_proc(vdev, curveto)(vdev, state->prev.x, state->prev.y, vp[0].x, vp[0].y,
						vp[1].x, vp[1].y, vp[2].x, vp[2].y, state->type);
		state->prev = vp[2];
		break;
	case gs_pe_closepath:
		code = vdev_proc(vdev, closepath)(vdev, state->prev.x, state->prev.y, state->start.x,
	...
	if(count > 0) {
		...
		if(code >= 0)
			for(i = 1; i < count && code >= 0; ++i) {
				x_prev = x, y_prev = y;
				code = (*vdev_proc(vdev, lineto))(vdev, x_prev, y_prev,
								  (x = fixed2float(points[i].x) / vdev->scale.x),
								  (y = fixed2float(points[i].y) / vdev->scale.y),
								  type);
			}
		if(code >= 0 && close)
			code = (*vdev_proc(vdev, closepath))(vdev, x, y, x_start, y_start, type);
	}
	return (code >= 0 && type != gx_path_type_none ? (*vdev_proc(vdev, endpath))(vdev, type) : code);
}
			    ...
			    const gx_clip_path *pcpath)
{
	...
	if(pcpath == 0) {
		...
		return (*vdev_proc(vdev, dopath))(vdev, &pcpath->path,
						  (pcpath->rule <= 0 ? gx_path_type_clip | gx_path_type_winding_number : gx_path_type_clip | gx_path_type_even_odd),
						  NULL);
	} else {
	...
	for(; code >= 0 && prect != 0; prect = prect->next)
		if(prect->xmax > prect->xmin && prect->ymax > prect->ymin)
			code = gdev_vector_write_rectangle(vdev, int2fixed(prect->xmin), int2fixed(prect->ymin),
...
gdev_vector_close_file(gx_device_vector *vdev)
{
	...
	if(gx_device_close_output_file((gx_device *)vdev, vdev->fname, f) != 0 || err != 0)
			...
			gdev_vector_image_enum_t *pie)
{
	...
	if((code = gdev_vector_update_log_op(vdev, pis->log_op)) < 0 ||
	   ...
	   (vdev->bbox_device &&
	    (code = (*dev_proc(vdev->bbox_device, begin_image))((gx_device *)vdev->bbox_device, pis, pim, format, prect,
		...
		if(ofns.size > fname_size)
			...
			break;
			  ...
			  (dev->is_open && vdev->strm != 0 &&
			   stell(vdev->strm) != 0))
			ecode = (dev->LockSafetyParams) ? gs_error_invalidaccess : gs_error_rangecheck;
		else
			break;
		goto ofe;
	default:
			   ...
			   gx_color_index color)
{
	...
	if(vdev->bbox_device) {
		int code = (*dev_proc(vdev->bbox_device, fill_rectangle))((gx_device *)vdev->bbox_device, x, y, w, h, color);
		      ...
		      const gx_device_color *pdevc, const gx_clip_path *pcpath)
{
	...
	if((code = gdev_vector_prepare_fill(vdev, pis, params, pdevc)) < 0 ||
	   (code = gdev_vector_update_clip_path(vdev, pcpath)) < 0 ||
	   (vdev->bbox_device &&
	    (code = (*dev_proc(vdev->bbox_device, fill_path))((gx_device *)vdev->bbox_device, pis, ppath, params,
							      pdevc, pcpath)) < 0) ||
	   (code = (*vdev_proc(vdev, dopath))(vdev, ppath,
					      (params->rule > 0 ? gx_path_type_even_odd : gx_path_type_winding_number) | gx_path_type_fill |
						  vdev->fill_options,
					      NULL)) < 0)
			...
			const gx_drawing_color *pdcolor, const gx_clip_path *pcpath)
{
	...
	if((set_ctm = gdev_vector_stroke_scaling(vdev, pis, &scale, &mat)) != 0 ||
	   ...
	   (code = gdev_vector_update_clip_path(vdev, pcpath)) < 0 ||
	   (vdev->bbox_device &&
	    (code = (*dev_proc(vdev->bbox_device, stroke_path))((gx_device *)vdev->bbox_device, pis, ppath, params,
								pdcolor, pcpath)) < 0) ||
	   (code = (*vdev_proc(vdev, dopath))(vdev, ppath, gx_path_type_stroke | vdev->stroke_options, NULL)) < 0)
...
#undef y1
	if(vdev->bbox_device) {
		int code = (*dev_proc(vdev->bbox_device, fill_trapezoid))((gx_device *)vdev->bbox_device, left, right, ybot, ytop,
									  swap_axes, pdevc, lop);
			       ...
			       const gx_device_color *pdevc, gs_logical_operation_t lop)
{
	...
	if(code < 0)
		return code;
	if(vdev->bbox_device) {
		code = (*dev_proc(vdev->bbox_device, fill_parallelogram))((gx_device *)vdev->bbox_device, px, py, ax, ay, bx, by,
									  pdevc, lop);
		if(code < 0)
			return code;
	}
			  ...
			  const gx_device_color *pdevc, gs_logical_operation_t lop)
{
	...
	if(code < 0)
		return code;
	if(vdev->bbox_device) {
		code = (*dev_proc(vdev->bbox_device, fill_triangle))((gx_device *)vdev->bbox_device, px, py, ax, ay, bx, by,
								     pdevc, lop);
		if(code < 0)
			return code;
	}
