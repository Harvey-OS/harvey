...
#define gx_device_clist_common_members                                         \
	...
	gs_memory_t*                                                           \
	    bandlist_memory; /* allocator for in-memory bandlist files */      \
	byte* data;          /* buffer area */                                 \
	uint data_size;      /* size of buffer */                              \
	gx_band_params_t band_params;        /* band buffering parameters */   \
	bool do_not_open_or_close_bandfiles; /* if true, do not open/close     \
	                                        bandfiles */                   \
	bool page_uses_transparency;         /* if true then page uses PDF 1.4 \
	                                        transparency */                        \
...
typedef struct gx_device_clist_writer_s {
	...
	int error_is_retryable;      /* Extra status used to distinguish hard
	                                VMerrors */
...
#define clist_disable_pass_thru_params                                         \
	(1 << 5) /* disable EXCEPT at top of page */
#define clist_disable_copy_alpha                                               \
	(1 << 6) /* target does not support copy_alpha */
...
#define public_st_device_clist() /* in gxclist.c */                            \
	gs_public_st_complex_only(st_device_clist, gx_device_clist,            \
	                          "gx_device_clist", 0,                        \
	                          device_clist_enum_ptrs,                      \
	                          device_clist_reloc_ptrs, gx_device_finalize)
#define st_device_clist_max_ptrs                                               \
	(st_device_forward_max_ptrs + st_imager_state_num_ptrs + 1)

/* setup before opening clist device */
#define clist_init_params(xclist, xdata, xdata_size, xtarget, xbuf_procs,      \
                          xband_params, xexternal, xmemory, xfree_bandlist,    \
                          xdisable, pageusestransparency)                      \
	BEGIN(xclist)->common.data = (xdata);                                  \
 ...
 */
int clist_render_rectangle(gx_device_clist* cdev, const gs_int_rect* prect,
                           gx_device* bdev,
