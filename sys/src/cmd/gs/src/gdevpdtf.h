 ...
 */
typedef int (*pdf_font_write_contents_proc_t)(gx_device_pdf*,
                                              pdf_font_resource_t*);
...
struct pdf_font_resource_s {
	...
	pdf_font_descriptor_t*
	    FontDescriptor;         /* (not used for Type 0, Type 3, */
	...
	union {
		...
		struct /*type0*/ {
			...
			pdf_font_resource_t* DescendantFont; /* CIDFont */
			                                     /*
			                                      * The Encoding_name must be long enough to hold either
			                                      * the
			                                      * longest standard CMap name defined in the PDF
			                                      * Reference,
			...
			gs_const_string
			    CMapName; /* copied from the original CMap, */
		...
		struct /*cidfont*/ {
			...
			/* [D]W[2] is Widths. */
			long CIDSystemInfo_id; /* (written when font is
			                          allocated) */
		...
		struct /*simple*/ {
			...
			int FirstChar,
			    LastChar; /* 0 <= FirstChar <= LastChar <= 255 */
			              /*
			               * The BaseEncoding can only be ENCODING_INDEX_WINANSI,
			               * ENCODING_INDEX_MACROMAN, ENCODING_INDEX_MACEXPERT, or
			               * -1.
			               */
			gs_encoding_index_t BaseEncoding;
			pdf_encoding_element_t*
			    Encoding; /* [256], not for Type 3 */
			...
			union {
				...
				struct /*truetype*/ {
					/*
					 * No extra info needed, but the ANSI
					 * standard doesn't
					 * allow empty structs.
					 */
					int _dummy;
...
#define public_st_pdf_font_resource() /* gdevpdtf.c */                         \
	gs_public_st_composite(                                                \
	    st_pdf_font_resource, pdf_font_resource_t, "pdf_font_resource_t",  \
	    pdf_font_resource_enum_ptrs, pdf_font_resource_reloc_ptrs)
...
#define private_st_pdf_standard_font() /* gdevpdtf.c */                        \
	gs_private_st_ptrs1(st_pdf_standard_font, pdf_standard_font_t,         \
	                    "pdf_standard_font_t", pdf_std_font_enum_ptrs,     \
	                    pdf_std_font_reloc_ptrs, pdfont)
#define private_st_pdf_standard_font_element() /* gdevpdtf.c */                \
	gs_private_st_element(                                                 \
	    st_pdf_standard_font_element, pdf_standard_font_t,                 \
...
int pdf_font_std_alloc(gx_device_pdf* pdev, pdf_font_resource_t** ppfres,
                       bool is_original, gs_id rid, gs_font_base* pfont,
                       int index);
...
int pdf_font_cidfont_alloc(gx_device_pdf* pdev, pdf_font_resource_t** ppfres,
                           gs_id rid, pdf_font_descriptor_t* pfd);
int pdf_obtain_cidfont_widths_arrays(gx_device_pdf* pdev,
                                     pdf_font_resource_t* pdfont, int wmode,
                                     double** w, double** w0, double** v);
int font_resource_encoded_alloc(gx_device_pdf* pdev,
                                pdf_font_resource_t** ppfres, gs_id rid,
                                font_type ftype,
                                pdf_font_write_contents_proc_t write_contents);
