...
{
	return (glyph >= gs_min_cid_glyph || glyph == gs_no_glyph ? false : name_mark_index(mem, (uint)glyph));
}
	...
	gs_ref_memory_t *imem)
{
	...
	make_tav(&fid, t_fontID,
		 ...
		 pstruct, (void *)pfont);
	return (i_ctx_p ? idict_put_string(fp, "FID", &fid) : dict_put_string(fp, "FID", &fid, NULL));
}
