...
/* Forward references */
private
int make_font(i_ctx_t*, const gs_matrix*);
private
void make_uint_array(os_ptr, const uint*, int);
private
...
{
	return (glyph >= gs_min_cid_glyph || glyph == gs_no_glyph
	            ? false
...
private
int
zfont_global_glyph_code(const gs_memory_t* mem, gs_const_string* gstr,
                        gs_glyph* pglyph)
{
	ref v;
...
const op_def zfont_op_defs[] = {{"0currentfont", zcurrentfont},
                                {"2makefont", zmakefont},
                   ...
                   const gs_matrix* pmat, gs_font** ppfont)
{
	...
	if((code = dict_put_string(&newdict, "FontMatrix", &newmat, NULL)) <
	       0 ||
	   (code = dict_put_string(&newdict, "OrigFont",
	                           pfont_dict(oldfont->base), NULL)) < 0 ||
	   (code = dict_put_string(&newdict, "ScaleMatrix", &scalemat, NULL)) <
	       0 ||
...
font_restore(const alloc_save_t* save)
{
...

	{
		...
		for(pair = pdir->fmcache.mdata, n = pdir->fmcache.mmax; n > 0;
		    pair++, n--)
			if(!fm_pair_is_free(pair)) {
				if((uid_is_XUID(&pair->UID) &&
				    alloc_is_since_save(
				        (char*)pair->UID.xvalues, save))) {
					gs_purge_fm_pair(pdir, pair, 0);
					continue;
				}
				if(pair->font != 0 &&
				   alloc_is_since_save((char*)pair->font,
				                       save)) {
					if(!uid_is_valid(&pair->UID)) {
						gs_purge_fm_pair(pdir, pair, 0);
						continue;
					}
					/* Don't discard pairs with a surviving
					 * UID. */
					pair->font = 0;
				}
				if(pair->xfont != 0 &&
				   alloc_is_since_save((char*)pair->xfont,
				                       save))
...
ENUM_PTRS_END
private
RELOC_PTRS_WITH(unicode_decoder_reloc_ptrs, gs_unicode_decoder* pptr);
RELOC_REF_VAR(pptr->data);
r_clear_attrs(&pptr->data, l_mark);
RELOC_PTRS_END

gs_private_st_complex_only(st_unicode_decoder, gs_unicode_decoder,
                           "unicode_decoder", unicode_decoder_clear_marks,
                           unicode_decoder_enum_ptrs,
...
{
	const gs_unicode_decoder* pud =
	    (gs_unicode_decoder*)dir->glyph_to_unicode_table;
...
{
	gs_unicode_decoder* pud =
	    gs_alloc_struct(imemory, gs_unicode_decoder, &st_unicode_decoder,
	                    "setup_unicode_decoder");
	if(pud == NULL)
		return_error(e_VMerror);
