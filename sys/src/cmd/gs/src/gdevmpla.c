...
/* Copyright (C) 1993, 1994, 1997, 1998, 1999 Aladdin Enterprises.  All rights
  reserved.
...
/* procedures */
private
dev_proc_open_device(mem_planar_open);
declare_mem_procs(mem_planar_copy_mono, mem_planar_copy_color,
                  mem_planar_fill_rectangle);
private
dev_proc_strip_tile_rectangle(mem_planar_strip_tile_rectangle);
private
dev_proc_get_bits_rectangle(mem_planar_get_bits_rectangle);
                    ...
                    const gx_render_plane_t* planes /*[num_planes]*/)
{
	...
	set_dev_proc(mdev, strip_tile_rectangle,
...
#define MEM_SAVE_PARAMS(mdev, msp)                                             \
	(msp.depth = mdev->color_info.depth, msp.base = mdev->base,            \
...
#define MEM_RESTORE_PARAMS(mdev, msp)                                          \
	(mdev->color_info.depth = msp.depth, mdev->base = msp.base,            \
                          ...
                          gx_color_index color)
{
	...
	for(pi = 0; pi < mdev->num_planes; ++pi) {
		...
		dev_proc(mdproto, fill_rectangle)(
		    dev, x, y, w, h, (color >> mdev->planes[pi].shift) & mask);
...
private
int
mem_planar_copy_mono(gx_device* dev, const byte* base, int sourcex, int sraster,
                     ...
                     gx_color_index color0, gx_color_index color1)
{
	...
	for(pi = 0; pi < mdev->num_planes; ++pi) {
		...
		if(c0 == c1)
			...
			dev_proc(mdproto, copy_mono)(dev, base, sourcex,
			                             sraster, id, x, y, w, h,
			                             c0, c1);
		mdev->line_ptrs += mdev->height;
	}
			...
			for(cx = x; cx < x + w; cx += cw) {
				...
				for(iy = 0; iy < ch; ++iy) {
					sample_load_declare_setup(
					    sptr, sbit, source_base, source_bit,
					    source_depth);
					sample_store_declare_setup(
					    dptr, dbit, dbbyte, buf.b + br * iy,
					    0, plane_depth);

					for(ix = 0; ix < cw; ++ix) {
						gx_color_index value;

						sample_load_next_any(
						    value, sptr, sbit,
						    source_depth);
					...
					sample_store_flush(dptr, dbit,
					                   plane_depth, dbbyte);
					source_base += sraster;
				}
				/*
				 * Detect and bypass the possibility that
				 * copy_color is
				 * defined in terms of copy_mono.
				 */
				if(plane_depth == 1)
					dev_proc(mdproto, copy_mono)(
					    dev, buf.b, 0, br, gx_no_bitmap_id,
					    cx, cy, cw, ch, (gx_color_index)0,
					    (gx_color_index)1);
				else
                                ...
                                int px, int py)
{
	...
	for(pi = 0; pi < mdev->num_planes; ++pi) {
		...
		if(c0 == c1)
			...
			/*
			 * Temporarily replace copy_mono in case
			 * strip_tile_rectangle is
			 * defined in terms of it.
			 */
			set_dev_proc(dev, copy_mono,
			             dev_proc(mdproto, copy_mono));
...
private
int
planar_to_chunky(gx_device_memory* mdev, int x, int y, int w, int h, int offset,
                 uint draster, byte* dest)
{
	...
	int direct = (mdev->color_info.depth != num_planes * mdev->plane_depth
	                  ? 0
	                  : mdev->planes[0].shift == 0 ? -mdev->plane_depth
	                                               : mdev->plane_depth);
	int pi, ix, iy;
	...
	if(direct < 0) {
		...
		for(pi = 0; pi < num_planes; ++pi)
			if(mdev->planes[num_planes - 1 - pi].shift !=
			   pi * direct) {
				direct = 0;
				break;
			}
	...
	for(iy = y; iy < y + h; ++iy) {
		...
		for(ix = w; ix > 0; --ix) {
			...
			for(pi = 0; pi < num_planes; ++pi) {
				...
				sample_load_next16(value, sptr[pi], sbit[pi],
				                   plane_depth);
				color |= (gx_color_index)value
				         << mdev->planes[pi].shift;
			}
			sample_store_next_any(color, dptr, dbit, ddepth,
			                      dbbyte);
		}
                              ...
                              gs_int_rect** unread)
{
	 ...
	 */
	if(!(~options & (GB_COLORS_NATIVE | GB_ALPHA_NONE | GB_PACKING_CHUNKY |
	                 GB_RETURN_COPY))) {
		int offset =
		    (options & GB_OFFSET_SPECIFIED ? params->x_offset : 0);
		uint draster = (options & GB_RASTER_SPECIFIED
		                    ? params->raster
		                    : bitmap_raster((offset + w) *
		                                    mdev->color_info.depth));

		planar_to_chunky(mdev, x, y, w, h, offset, draster,
		                 params->data[0]);
	} else {
/*
...
#define BUF_BYTES (BUF_LONGS * ARCH_SIZEOF_LONG)
		...
		copy_params.options = GB_COLORS_NATIVE | GB_PACKING_CHUNKY |
		                      GB_ALPHA_NONE | GB_RASTER_STANDARD;
		...
		for(cy = y; cy < y + h; cy += ch) {
			...
			for(cx = x; cx < x + w; cx += cw) {
				cw = min(bw, x + w - cx);
				planar_to_chunky(mdev, cx, cy, cw, ch, 0, br,
				                 buf.b);
				dest_params.x_offset =
				    params->x_offset + cx - x;
				code = gx_get_bits_copy(
				    dev, 0, cw, ch, &dest_params, &copy_params,
				    buf.b, br);
				if(code < 0)
					return code;
			}
