...
private
dev_proc_close_device(san_close);
private
dev_proc_get_clipping_box(san_get_clipping_box);

/* --------------------- List management ------------------------- */
/* fixme : use something like C++ patterns to generate same functions for
 * various types. */
...
trap_reserve(gx_device_spot_analyzer* padev)
{
	...
	if(t != NULL) {
		...
		if(padev->trap_buffer_count > 10000)
			return NULL;
		t = gs_alloc_struct(padev->memory, gx_san_trap, &st_san_trap,
		                    "trap_reserve");
		if(t == NULL)
			return NULL;
		t->link = NULL;
...
{
	/* Assuming a non-empty cyclic list, and the anchor points to the first
	 * element.  */
	return list->prev;
}
...
private
const gx_device_spot_analyzer gx_spot_analyzer_device = {
    std_device_std_body(gx_device_spot_analyzer, 0, "spot analyzer", 0, 0, 1,
                        1),
    {san_open, NULL, NULL, NULL, san_close, NULL, NULL, NULL, NULL, NULL, NULL,
     NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL,
     NULL, gx_default_fill_path, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL,
     NULL, NULL, NULL, san_get_clipping_box, NULL, NULL, NULL, NULL, NULL, NULL,
     gx_default_finish_copydevice, NULL, NULL, NULL, NULL, NULL, NULL, NULL,
     NULL, NULL}};
...
try_unite_last_trap(gx_device_spot_analyzer* padev, fixed xlbot)
{
	if(padev->bot_band != NULL && padev->top_band != NULL) {
		...
		/* If the last trapezoid is a prolongation of its bottom
		   contact,
		...
		if(t != NULL && t->upper != NULL && last->xrbot < xlbot &&
		   (last->prev == last || last->prev->xrbot < last->xlbot)) {
			if((t->next == NULL || t->xrtop < t->next->xltop) &&
			   (t->upper->next == t->upper && t->l == last->l &&
			    t->r == last->r)) {
				...
				vd_quad(t->xlbot, t->ybot, t->xrbot, t->ybot,
				        t->xrtop, t->ytop, t->xltop, t->ytop, 1,
				        VD_TRAP_U_COLOR);
...
{
	return (double)(t->xrbot - t->xlbot + t->xrtop - t->xltop) *
	       (t->ytop - t->ybot) / 2;
}
...
trap_axis_length(gx_san_trap* t)
{
	...
	return hypot(xtop - xbot,
	             (double)t->ytop - t->ybot); /* See Bug 687238 */
}
...
gx_san__obtain(gs_memory_t* mem, gx_device_spot_analyzer** ppadev)
{
	...
	gx_device_init((gx_device*)padev,
	               (const gx_device*)&gx_spot_analyzer_device, mem, false);
...
int
gx_san_trap_store(gx_device_spot_analyzer* padev, fixed ybot, fixed ytop,
                  fixed xlbot, fixed xrbot, fixed xltop, fixed xrtop,
                  const segment* l, const segment* r, int dir_l, int dir_r)
{
	...
	vd_quad(last->xlbot, last->ybot, last->xrbot, last->ybot, last->xrtop,
	        last->ytop, last->xltop, last->ytop, 1, VD_TRAP_N_COLOR);
	if(padev->top_band != NULL) {
	...
	while(padev->bot_current != NULL && padev->bot_current->xrtop < xlbot)
		padev->bot_current =
		    (trap_is_last(padev->bot_band, padev->bot_current)
		         ? NULL
		         : padev->bot_current->next);
	if(padev->bot_current != 0) {
		...
		while(t->xltop <= xrbot) {
			...
			cont->lower = t;
			cont->upper = last;
			vd_bar((t->xltop + t->xrtop + t->xlbot + t->xrbot) / 4,
			       (t->ytop + t->ybot) / 2,
			       (last->xltop + last->xrtop + last->xlbot +
			        last->xrbot) /
			           4,
...
private
int
hint_by_trap(gx_device_spot_analyzer* padev, int side_mask, void* client_data,
             gx_san_trap* t0, gx_san_trap* t1, double ave_width,
...
choose_by_vector(fixed x0, fixed y0, fixed x1, fixed y1, const segment* s,
                 double* slope, double* len, const segment** store_segm,
                 fixed* store_x, fixed* store_y)
{
	if(y0 != y1) {
...
private
inline void
choose_by_tangent(const segment* p, const segment* s, double* slope,
                  double* len, const segment** store_segm, fixed* store_x,
                  fixed* store_y, fixed ybot, fixed ytop)
{
	if(s->type == s_curve) {
		...
		if(ybot <= p->pt.y && p->pt.y <= ytop)
			choose_by_vector(c->p1.x, c->p1.y, p->pt.x, p->pt.y, s,
			                 slope, len, store_segm, store_x,
			                 store_y);
		if(ybot <= s->pt.y && s->pt.y <= ytop)
			choose_by_vector(c->p2.x, c->p2.y, s->pt.x, s->pt.y, s,
			                 slope, len, store_segm, store_x,
			                 store_y);
	} else {
		vd_bar(p->pt.x, p->pt.y, s->pt.x, s->pt.y, 0, VD_HINT_COLOR);
		choose_by_vector(s->pt.x, s->pt.y, p->pt.x, p->pt.y, s, slope,
		                 len, store_segm, store_x, store_y);
	}
}
...
hint_by_tangent(gx_device_spot_analyzer* padev, int side_mask,
                void* client_data, gx_san_trap* t0, gx_san_trap* t1,
                double ave_width,
                int (*handler)(void* client_data, gx_san_sect* ss))
{
	gx_san_trap* t;
	...
	for(t = t0;; t = upper_neighbour(t, left_right)) {
		if(side_mask & 1) {
			s = t->l;
			if(t->dir_l < 0)
				s = (s->type == s_line_close
				         ? ((const line_close_segment*)s)
				               ->sub->next
				         : s->next);
			p = (s->type == s_start
			         ? ((const subpath*)s)->last->prev
			         : s->prev);
			choose_by_tangent(p, s, &slope0, &len0, &sect.l,
			                  &sect.xl, &sect.yl, t->ybot, t->ytop);
		}
		if(side_mask & 2) {
			s = t->r;
			if(t->dir_r < 0)
			...
			p = (s->type == s_start
			         ? ((const subpath*)s)->last->prev
			         : s->prev);
			choose_by_tangent(p, s, &slope1, &len1, &sect.r,
			                  &sect.xr, &sect.yr, t->ybot, t->ytop);
		}
		if(t == t1)
			break;
	}
...
private
int
gx_san_generate_stems_aux(gx_device_spot_analyzer* padev, bool overall_hints,
                          void* client_data,
                          int (*handler)(void* client_data, gx_san_sect* ss))
{
	gx_san_trap* t0;
	...
	for(k = 0; overall_hints && k < 2; k++) { /* left, right. */
		for(t0 = padev->trap_buffer; t0 != padev->trap_free;
		    t0 = t0->link) {
			if(!t0->visited &&
			   (!k ? t0->leftmost : t0->rightmost)) {
				if(is_stem_boundaries(t0, 1 << k)) {
					gx_san_trap* t1 = t0, * tt = t0,
					             * t = t0;
					int code;
					...
					if(!k ? !t->leftmost : !t->rightmost)
						continue;
					t1 = t;
					/* leftmost/rightmost boundary from t0
					 * to t1. */
					code = hint_by_tangent(padev, 1 << k,
					                       client_data, t0,
					                       t1, 0, handler);
					if(code < 0)
						return code;
	...
	for(t0 = padev->trap_buffer; t0 != padev->trap_free; t0 = t0->link) {
		if(!t0->visited) {
			if(is_stem_boundaries(t0, 3)) {
				...
				vd_quad(t0->xlbot, t0->ybot, t0->xrbot,
				        t0->ybot, t1->xrtop, t1->ytop,
				        t1->xltop, t1->ytop, 1, VD_STEM_COLOR);
				for(t = t0;; t = t->upper->upper) {
				...
				if(length > ave_width / (2.0 /* arbitrary */)) {
					...
					int code = (by_trap ? hint_by_trap
					                    : hint_by_tangent)(
					    padev, 3, client_data, t0, t1,
					    ave_width, handler);
...
int
gx_san_generate_stems(gx_device_spot_analyzer* padev, bool overall_hints,
                      void* client_data,
                      int (*handler)(void* client_data, gx_san_sect* ss))
{
	int code;
	...
	vd_set_origin(0, 0);
	code = gx_san_generate_stems_aux(padev, overall_hints, client_data,
	                                 handler);
	vd_release_dc;
	return code;
}
