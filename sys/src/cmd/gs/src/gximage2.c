...
#undef eqe
	 ...
	 */
		{
			gx_color_index trans_black =
			    (*dev_proc(sdev, map_rgb_alpha_color))(sdev, (gx_color_value)0, (gx_color_value)0,
								   (gx_color_value)0, (gx_color_value)0);

			has_alpha =
			    trans_black != (*dev_proc(sdev, map_rgb_alpha_color))(sdev, (gx_color_value)0, (gx_color_value)0,
										  (gx_color_value)0, gx_max_color_value) &&
			    trans_black != (*dev_proc(sdev, map_rgb_alpha_color))(sdev, gx_max_color_value, gx_max_color_value,
	...
	if(!direct_copy)
		code = (*dev_proc(dev, begin_typed_image))(dev, pis, pmat, (const gs_image_common_t *)&idata.image, NULL,
	...
	if(code >= 0) {
		...
		for(y = 0; code >= 0 && y < idata.image.Height; ++y) {
			...
			if(pixel_copy) {
				...
				code = (*dev_proc(sdev, get_bits_rectangle))(sdev, &rect, &params, &unread);
				if(code < 0)
					break;
				num_unread = code;
				data = params.data[0];
		     ...
		     */
					code = (*dev_proc(dev, copy_color))(dev, data, 0, row_size, gx_no_bitmap_id,
