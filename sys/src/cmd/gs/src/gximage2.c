...
/* Forward references */
private
dev_proc_begin_typed_image(gx_begin_image2);
private
image_proc_source_size(gx_image2_source_size);
...
const gx_image_type_t gs_image_type_2 = {
    &st_gs_image2, gx_begin_image2, gx_image2_source_size, gx_image_no_sput,
    gx_image_no_sget, gx_image_default_release, 2};
...
private
int
gx_begin_image2(gx_device* dev, const gs_imager_state* pis,
                ...
                gx_image_enum_common_t** pinfo)
{
	...
	gs_image_t_init(&idata.image,
	                gs_currentcolorspace((const gs_state*)pis));
	...
	if(pixel_copy) {
		...
		if(pcpath == NULL ||
		   gx_cpath_includes_rectangle(
		       pcpath, int2fixed(idata.bbox.p.x),
		       int2fixed(idata.bbox.p.y), int2fixed(idata.bbox.q.x),
		       int2fixed(idata.bbox.q.y))) {
			gs_matrix mat;

			/*
			 * Figure 7.2 of the Adobe 3010 Supplement says that we
			 * should
			 * compute CTM x ImageMatrix here, but I'm almost
			 * certain it
			 * should be the other way around.  Also see gdevx.c.
			 */
			gs_matrix_multiply(&idata.image.ImageMatrix, &smat,
			                   &mat);
			direct_copy = (is_xxyy(&dmat) || is_xyyx(&dmat)) &&
...
#undef eqe
	...
	if(smat.yy < 0) {
		 ...
		 */
		idata.image.ImageMatrix.ty +=
		    idata.image.Height * idata.image.ImageMatrix.yy;
	...
	if(!direct_copy)
		code = (*dev_proc(dev, begin_typed_image))(
		    dev, pis, pmat, (const gs_image_common_t*)&idata.image,
		    NULL, pdcolor, pcpath, mem, &info);
	if(code >= 0) {
		int y;
		gs_int_rect rect;
		...
		for(y = 0; code >= 0 && y < idata.image.Height; ++y) {
			...
			rect.q.y = rect.p.y + 1;
			/* Insist on x_offset = 0 to simplify the conversion
			 * loop. */
			...
			if(pixel_copy) {
				...
				if(direct_copy) {
					/*
					 * Copy the pixels directly to the
					 * destination.
					 * We know that the transformation is
					 * only a translation,
					 * but we must handle an inverted
					 * destination Y axis.
					 */
				...
				/*
				 * Convert the pixels to pure colors.  This may
				 * be very
				 * slow and painful.  Eventually we will use
				 * indexed color for
				 * narrow pixels.
				 */
				/* Always use RGB source color for now. */
			...
			if(num_unread > 0 && pim->UnpaintedPath) {
				...
				gs_free_object(dev->memory, unread,
				               "UnpaintedPath unread");
			}
