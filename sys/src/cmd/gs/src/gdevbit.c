 ...
 */
#define REAL_NUM_COMPONENTS(dev) (dev->dname[3] == 'c' ? 4 : dev->dname[3] == 'r' ? 3 : 1)

private
const gx_device_procs bitmono_procs =
...
private
const gx_device_procs bitrgb_procs =
...
private
const gx_device_procs bitcmyk_procs =
...
#define cvalue(c) ((gx_color_value)((ulong)(c)*gx_max_color_value / mask))
	...
	case 1: /* gray */
		cv[0] =
		    (depth == 1 ? (color ? 0 : gx_max_color_value) : cvalue(color));
		break;
	case 3: /* RGB */
	{
	...
	if(dev_proc(pdev, map_cmyk_color) == cmyk_1bit_map_cmyk_color ||
	   ...
	   dev_proc(pdev, map_cmyk_color) == bit_map_cmyk_color) {
		set_dev_proc(pdev, map_cmyk_color,
			     pdev->color_info.depth == 4 ? cmyk_1bit_map_cmyk_color : pdev->color_info.depth == 32 ? cmyk_8bit_map_cmyk_color : bit_map_cmyk_color);
	}
