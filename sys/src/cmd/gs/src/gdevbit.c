...
private
dev_proc_map_rgb_color(bit_mono_map_color);
private
dev_proc_map_color_rgb(bit_map_color_rgb);
private
dev_proc_map_cmyk_color(bit_map_cmyk_color);
private
dev_proc_get_params(bit_get_params);
private
dev_proc_put_params(bit_put_params);
private
...
#define REAL_NUM_COMPONENTS(dev)                                               \
	(dev->dname[3] == 'c' ? 4 : dev->dname[3] == 'r' ? 3 : 1)

private
const gx_device_procs bitmono_procs = bit_procs(bit_mono_map_color);
const gx_device_printer gs_bit_device = {prn_device_body(
    gx_device_printer, bitmono_procs, "bit", DEFAULT_WIDTH_10THS,
    DEFAULT_HEIGHT_10THS, X_DPI, Y_DPI, 0, 0, 0, 0, /* margins */
    1, 1, 1, 0, 2, 1, bit_print_page)};

private
const gx_device_procs bitrgb_procs = bit_procs(gx_default_rgb_map_rgb_color);
const gx_device_printer gs_bitrgb_device = {prn_device_body(
    gx_device_printer, bitrgb_procs, "bitrgb", DEFAULT_WIDTH_10THS,
    DEFAULT_HEIGHT_10THS, X_DPI, Y_DPI, 0, 0, 0, 0, /* margins */
    3, 4, 1, 1, 2, 2, bit_print_page)};

private
const gx_device_procs bitcmyk_procs = bit_procs(bit_map_cmyk_color);
const gx_device_printer gs_bitcmyk_device = {prn_device_body(
    gx_device_printer, bitcmyk_procs, "bitcmyk", DEFAULT_WIDTH_10THS,
    DEFAULT_HEIGHT_10THS, X_DPI, Y_DPI, 0, 0, 0, 0, /* margins */
...
#define cvalue(c) ((gx_color_value)((ulong)(c)*gx_max_color_value / mask))
	...
	switch(ncomp) {
	case 1: /* gray */
		cv[0] = (depth == 1 ? (color ? 0 : gx_max_color_value)
		                    : cvalue(color));
		break;
	case 3: /* RGB */
	{
...
bit_map_cmyk_color(gx_device* dev, const gx_color_value cv[])
{
	...
	gx_color_index color =
	    (((((((gx_color_index)cv[0] >> drop) << bpc) + (cv[1] >> drop))
	       << bpc) +
	      (cv[2] >> drop))
	     << bpc) +
	    (cv[3] >> drop);
...
bit_put_params(gx_device* pdev, gs_param_list* plist)
{
	...
	pdev->color_info.max_gray = pdev->color_info.max_color =
	    (pdev->color_info.dither_grays = pdev->color_info.dither_colors =
	         (1 << bpc)) -
	    1;
...
{ /* Just dump the bits on the file. */
	...
	byte* in =
	    gs_alloc_bytes(pdev->memory, line_size, "bit_print_page(in)");
	byte* data;
