...
/* Copyright (C) 1994, 1995, 1996, 1997, 1998, 1999 Aladdin Enterprises.  All
  rights reserved.
...
#define X_WRAPPER_DATA(amc_proc)                                               \
	...
	,                   /* std_procs */                                    \
	    0, /* target */ /* gx_device_X_wrapper */                          \
	    {0},            /* cache */                                        \
	    amc_proc
gs_private_st_suffix_add0_final(st_device_X_wrapper, gx_device_X_wrapper,
                                "gx_device_X_wrapper", gdevx_wrapper_enum_ptrs,
                                gdevx_wrapper_reloc_ptrs, gx_device_finalize,
                                st_device_forward);
...
private
int get_dev_target(gx_device**, gx_device*);

private
int get_target_info(gx_device*);
private
gx_color_index x_alt_map_color(gx_device*, gx_color_index);
                     ...
                     gx_color_value prgb[3])
{
	...
	return (*dev_proc(tdev, map_color_rgb))(
	    tdev, x_alt_map_color(dev, color), prgb);
}
...
private
int
x_wrap_copy_mono(gx_device* dev, const byte* base, int sourcex, int raster,
                 ...
                 gx_color_index zero, gx_color_index one)
{
	...
	if((code = get_dev_target(&tdev, dev)) < 0)
		return code;
	return (*dev_proc(tdev, copy_mono))(tdev, base, sourcex, raster, id, x,
	                                    y, w, h, x_alt_map_color(dev, zero),
	                                    x_alt_map_color(dev, one));
}

private
int
x_wrap_copy_color(gx_device* dev, const byte* base, int sourcex, int raster,
                  gx_bitmap_id id, int x, int y, int w, int h)
{
	gx_device* tdev;
		...
		for(xblock = x; xblock < x + w; xblock += block_w) {
			...
			for(ycur = yblock; ycur < yend; ++ycur)
				for(xcur = xblock; xcur < xend; ++xcur) {
					int sbit =
					    (xcur - x + sourcex) * source_bits;
					uint sbyte = base[(ycur - y) * raster +
					                  (sbit >> 3)];
					uint spixel =
					    ((sbyte << (sbit & 7)) & 0xff) >>
					    (8 - source_bits);
					gx_color_index cindex =
					    ((gx_device_X_wrapper*)dev)
					        ->color_cache[spixel];

					if(cindex == gx_no_color_index)
						cindex = x_alt_map_color(
						    dev, spixel);
					switch(depth_bytes) {
					case 4:
						*p++ = (byte)(cindex >> 24);
			...
			code = (*dev_proc(tdev, copy_color))(
			    tdev, mapped, 0, (xend - xblock) * depth_bytes,
			    gx_no_bitmap_id, xblock, yblock, xend - xblock,
			    yend - yblock);
			if(code < 0)
				return code;
		}
...
private
int
x_forward_copy_color(gx_device* dev, const byte* base, int sourcex, int raster,
                     gx_bitmap_id id, int x, int y, int w, int h)
{
	gx_device* tdev;
	int code;
	...
	if((code = get_dev_target(&tdev, dev)) < 0)
		return code;
	return (*dev_proc(tdev, copy_color))(tdev, base, sourcex, raster, id, x,
	                                     y, w, h);
}
	...
	for(sbit = 0, xi = 0; xi < width; sbit += sdepth, ++xi) {
		...
		if(pixel != pixel_in) {
			...
			if(dev->color_info.num_components <= 3)
				pixel_out =
				    (*dev_proc(dev, map_rgb_color))(dev, rgb);
			else {
				/* Convert RGB to CMYK. */
				...
				cmyk[3] = k;
				pixel_out =
				    (*dev_proc(dev, map_cmyk_color))(dev, cmyk);
			}
		}
...
get_dev_target(gx_device** ptdev, gx_device* dev)
{
	...
	if(tdev == 0) {
		...
		int code = gs_copydevice(
		    &tdev, (const gx_device*)&gs_x11_device, dev->memory);
...
/* Device procedures */
private
dev_proc_open_device(x_cmyk_open);
private
dev_proc_put_params(x_cmyk_put_params);
private
dev_proc_map_cmyk_color(x_cmyk_map_cmyk_color);
/* Extended device procedures */
private
dev_proc_map_color_rgb(x_cmyk_alt_map_color);

/* The device descriptor */
private
const gx_device_procs x_cmyk_procs = {
    x_cmyk_open, gx_forward_get_initial_matrix, x_forward_sync_output,
    x_forward_output_page, x_wrap_close, NULL, /* map_rgb_color */
    ...
    x_wrap_get_params, x_cmyk_put_params, x_cmyk_map_cmyk_color,
    gx_forward_get_xfont_procs, gx_forward_get_xfont_device,
    NULL, /* map_rgb_alpha_color */
    gx_forward_get_page_device, gx_forward_get_alpha_bits, NULL /* copy_alpha */
};
...
const gx_device_X_wrapper gs_x11cmyk_device = {
    std_device_dci_type_body(gx_device_X_wrapper, &x_cmyk_procs, "x11cmyk",
                             &st_device_X_wrapper, FAKE_RES * 85 / 10,
                             FAKE_RES * 11,      /* x and y extent (nominal) */
...
/* Extended device procedures */
private
dev_proc_map_color_rgb(x_mono_alt_map_color);

/* The device descriptor */
private
const gx_device_procs x_mono_procs = {
    x_wrap_open, gx_forward_get_initial_matrix, x_forward_sync_output,
    x_forward_output_page, x_wrap_close, gx_default_b_w_map_rgb_color,
    x_wrap_map_color_rgb, x_wrap_fill_rectangle, gx_default_tile_rectangle,
    x_wrap_copy_mono,
    gx_default_copy_color, /* this is fast for the 1-bit case */
    gx_default_draw_line, x_wrap_get_bits, x_wrap_get_params, x_wrap_put_params,
    gx_default_map_cmyk_color, gx_forward_get_xfont_procs,
    gx_forward_get_xfont_device, NULL, /* map_rgb_alpha_color */
    gx_forward_get_page_device, gx_forward_get_alpha_bits, NULL /* copy_alpha */
};
...
/* Extended device procedures */
private
dev_proc_map_color_rgb(x_gray_alt_map_color);

/* The device descriptor */
private
const gx_device_procs x_gray_procs = {
    x_wrap_open, gx_forward_get_initial_matrix, x_forward_sync_output,
    x_forward_output_page, x_wrap_close, gx_default_gray_map_rgb_color,
    x_wrap_map_color_rgb, x_wrap_fill_rectangle, gx_default_tile_rectangle,
    x_wrap_copy_mono, x_wrap_copy_color, gx_default_draw_line, x_wrap_get_bits,
    x_wrap_get_params, x_wrap_put_params, gx_default_map_cmyk_color,
    gx_forward_get_xfont_procs, gx_forward_get_xfont_device,
    NULL, /* map_rgb_alpha_color */
    gx_forward_get_page_device, gx_forward_get_alpha_bits, NULL /* copy_alpha */
};
...
private
gx_color_index
x_alpha_map_rgb_alpha_color(gx_device* dev, gx_color_value r, gx_color_value g,
                            gx_color_value b, gx_color_value alpha)
{
	gx_color_index color;
	gx_color_value cv[3];
...
/* Device procedures */
private
dev_proc_map_rgb_color(x_rg16x_map_rgb_color);
private
dev_proc_map_rgb_color(x_rg32x_map_rgb_color);
/* Extended device procedures */
private
dev_proc_map_color_rgb(x_rg16x_alt_map_color);
private
dev_proc_map_color_rgb(x_rg32x_alt_map_color);

/* The device descriptor */
#define RGBX_PROCS(map_rgb_proc)                                               \
	x_wrap_open, gx_forward_get_initial_matrix, x_forward_sync_output,     \
	    x_forward_output_page, x_wrap_close, map_rgb_proc, /* differs */   \
	    x_wrap_map_color_rgb, x_wrap_fill_rectangle,                       \
	    gx_default_tile_rectangle, x_wrap_copy_mono, x_forward_copy_color, \
	    gx_default_draw_line, x_forward_get_bits, x_wrap_get_params,       \
	    x_wrap_put_params, gx_forward_map_cmyk_color,                      \
	    gx_forward_get_xfont_procs, gx_forward_get_xfont_device,           \
	    x_alpha_map_rgb_alpha_color, gx_forward_get_page_device,           \
	    gx_default_get_alpha_bits, gx_default_copy_alpha

private
const gx_device_procs x_rg16x_procs = {RGBX_PROCS(x_rg16x_map_rgb_color)};
...
private
const gx_device_procs x_rg32x_procs = {RGBX_PROCS(x_rg32x_map_rgb_color)};
