...
#define mapped_bytes 480 /* must be a multiple of 3 & 4 */
	...
	for(yblock = y; yblock < y + h; yblock += block_h)
		for(xblock = x; xblock < x + w; xblock += block_w) {
			...
			code = (*dev_proc(tdev, copy_color))(tdev, mapped, 0, (xend - xblock) * depth_bytes, gx_no_bitmap_id,
			    ...
			    gx_color_value r, gx_color_value g, gx_color_value b, gx_color_value alpha)
{
	...
	return (abyte == 0 ? (gx_color_index)0xff << 24 : ((gx_color_index)(abyte ^ 0xff) << 24) + color);
}
...
private
const gx_device_procs x_rg16x_procs = {
...
private
const gx_device_procs x_rg32x_procs = {
