...
private
const gx_device_procs win_pr2_procs =
...
win_pr2_open(gx_device *dev)
{
	...
	if(wdev->max_dpi > 50) {
		...
		while((dev->x_pixels_per_inch > wdev->max_dpi) || (dev->y_pixels_per_inch > wdev->max_dpi)) {
			ratio += 1.0;
			wdev->ratio++;
...
win_pr2_put_params(gx_device *pdev, gs_param_list *plist)
{
	...
	if(wdev->win32_hdevmode && wdev->hdcprn) {
		if((old_duplex != wdev->Duplex) || (old_tumble != wdev->tumble) || (old_orient != wdev->user_orient) || (old_color != wdev->user_color) || (old_paper != wdev->user_paper) || (old_mx_dpi != wdev->max_dpi)) {
