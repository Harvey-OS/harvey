 ...
 * Bug fixed by Pierre Arnaud 2000-03-20 (win_pr2_set_bpp did not set
 * anti_alias)
...
private
dev_proc_open_device(win_pr2_open);
private
dev_proc_close_device(win_pr2_close);
private
dev_proc_print_page(win_pr2_print_page);
private
dev_proc_map_rgb_color(win_pr2_map_rgb_color);
private
dev_proc_map_color_rgb(win_pr2_map_color_rgb);
private
dev_proc_get_params(win_pr2_get_params);
private
dev_proc_put_params(win_pr2_put_params);

private
void win_pr2_set_bpp(gx_device* dev, int depth);

private
const gx_device_procs win_pr2_procs = prn_color_params_procs(
    win_pr2_open, gdev_prn_output_page, win_pr2_close, win_pr2_map_rgb_color,
    win_pr2_map_color_rgb, win_pr2_get_params, win_pr2_put_params);
...
gx_device_win_pr2 far_data gs_mswinpr2_device = {
    prn_device_std_body(gx_device_win_pr2, win_pr2_procs, "mswinpr2",
                        ...
                        0, 0, 0, 0, 0, win_pr2_print_page), /* depth = 0 */
    0,                                                      /* hdcprn */
    0,                                                      /* nocancel */
    0,                                                      /* doc_page_begin */
    0,                                                      /* doc_page_end */
...
private
int win_pr2_getdc(gx_device_win_pr2* dev);
private
int win_pr2_update_dev(gx_device_win_pr2* dev, LPDEVMODE pdevmode);
private
int win_pr2_update_win(gx_device_win_pr2* dev, LPDEVMODE pdevmode);
private
int win_pr2_print_setup_interaction(gx_device_win_pr2* dev, int mode);
private
int win_pr2_write_user_settings(gx_device_win_pr2* dev, gs_param_list* plist);
private
int win_pr2_read_user_settings(gx_device_win_pr2* dev, gs_param_list* plist);
private
void win_pr2_copy_check(gx_device_win_pr2* dev);
...
win_pr2_open(gx_device* dev)
{
	...
	if((wdev->win32_hdevmode) && (wdev->win32_hdevnames)) {
		...
		LPDEVNAMES devnames =
		    (LPDEVNAMES)GlobalLock(wdev->win32_hdevnames);
	...
	gx_device_set_width_height(dev, (int)(size.x / ratio),
	                           (int)(size.y / ratio));
	...
	if(depth == 0) {
		...
		/* We recognize 1, 4 (but use only 3), 8 and 24 bit color
		 * devices */
		depth = GetDeviceCaps(wdev->hdcprn, PLANES) *
		        GetDeviceCaps(wdev->hdcprn, BITSPIXEL);
	}
	...
	pfile = gp_open_scratch_file(gp_scratch_file_name_prefix, wdev->fname,
	                             "wb");
	fclose(pfile);
	...
	if(!wdev->nocancel) {
		...
		wdev->hDlgModeless =
		    CreateDialog(phInstance, "CancelDlgBox", PARENT_WINDOW,
		                 wdev->lpfnCancelProc);
...
win_pr2_print_page(gx_device_printer* pdev, FILE* file)
{
	...
	if(depth <= 8) {
		...
		for(i = 0; i != 1 << depth; i++) {
			(*dev_proc(pdev, map_color_rgb))(
			    (gx_device*)pdev, (gx_color_index)i, rgb);
			pq = &bmi.pal[i];
	...
	if(!wdev->nocancel) {
		sprintf(dlgtext, "Printing page %d",
		        (int)(pdev->PageCount) + 1);
		SetWindowText(GetDlgItem(wdev->hDlgModeless, CANCEL_PRINTING),
		              dlgtext);
	...
	for(y = 0; y < scan_lines;) {
		...
		for(i = 0; i < lines; i++)
			gdev_prn_copy_scan_lines(
			    pdev, y + i, row + (bmp_raster * (lines - 1 - i)),
			    raster);

		if(ratio > 1) {
			StretchDIBits(
			    wdev->hdcprn, 0, y * ratio, pdev->width * ratio,
			    lines * ratio, 0, 0, pdev->width, lines, row,
			    (BITMAPINFO FAR*)&bmi, DIB_RGB_COLORS, SRCCOPY);
		} else {
			SetDIBitsToDevice(
			    wdev->hdcprn, 0, y, pdev->width, lines, 0, 0, 0,
			    lines, row, (BITMAPINFO FAR*)&bmi, DIB_RGB_COLORS);
		}
		y += lines;

		if(!wdev->nocancel) {
			/* inform user of progress */
			sprintf(dlgtext, "%d%% done",
			        (int)(y * 100L / scan_lines));
			SetWindowText(
			    GetDlgItem(wdev->hDlgModeless, CANCEL_PCDONE),
			    dlgtext);
		}
		/* process message loop */
		while(PeekMessage(&msg, wdev->hDlgModeless, 0, 0, PM_REMOVE)) {
			if((wdev->hDlgModeless == 0) ||
			   !IsDialogMessage(wdev->hDlgModeless, &msg)) {
...
{
	if(depth > 8) {
		static const gx_device_color_info win_pr2_24color =
		    dci_std_color(24);
		...
		static const gx_device_color_info win_pr2_4color =
		    dci_values(3, 4, 1, 1, 2, 2);

		dev->color_info = win_pr2_4color;
		depth = 4;
	} else { /* default is black_and_white */
		static const gx_device_color_info win_pr2_1color =
		    dci_std_color(1);
...
win_pr2_get_params(gx_device* pdev, gs_param_list* plist)
{
	...
	if(code >= 0)
		code = param_write_bool(plist, "NoCancel", &(wdev->nocancel));
	if(code >= 0)
		code = param_write_int(plist, "QueryUser", &(wdev->query_user));
	if(code >= 0)
		code = win_pr2_write_user_settings(wdev, plist);

	if((code >= 0) && (wdev->Duplex_set > 0))
		code = param_write_bool(plist, "Tumble", &(wdev->tumble));
...
win_pr2_put_params(gx_device* pdev, gs_param_list* plist)
{
	...
	case 0:
		...
		} else { /* change dev->color_info is valid before device is
		            opened */
	...
	default:
		ecode = code;
	bppe:
		param_signal_error(plist, "BitsPerPixel", ecode);
	case 1:
		break;
	}
	...
	default:
		ecode = code;
	nocancele:
		param_signal_error(plist, "NoCancel", ecode);
	case 1:
		break;
	}
...
win_pr2_getdc(gx_device_win_pr2* wdev)
{
	...
	if((devices = gs_malloc(wdev->memory, 4096, 1, "win_pr2_getdc")) ==
	   (char*)NULL)
		return FALSE;
	...
	if(!is_win32s) { /* Win32 */
		if(!OpenPrinter(device, &hprinter, NULL))
			return FALSE;
		size =
		    DocumentProperties(NULL, hprinter, device, NULL, NULL, 0);
		if((podevmode =
		        gs_malloc(wdev->memory, size, 1, "win_pr2_getdc")) ==
		   (LPDEVMODE)NULL) {
		...
		if((pidevmode =
		        gs_malloc(wdev->memory, size, 1, "win_pr2_getdc")) ==
		   (LPDEVMODE)NULL) {
			gs_free(wdev->memory, podevmode, size, 1,
			        "win_pr2_getdc");
			ClosePrinter(hprinter);
			return FALSE;
		}
		DocumentProperties(NULL, hprinter, device, podevmode, NULL,
		                   DM_OUT_BUFFER);
		pfnDeviceCapabilities = (LPFNDEVCAPS)DeviceCapabilities;
	} else { /* Win16 and Win32s */
		...
		pfnExtDeviceMode =
		    (LPFNDEVMODE)GetProcAddress(hlib, "ExtDeviceMode");
		if(pfnExtDeviceMode == (LPFNDEVMODE)NULL) {
			FreeLibrary(hlib);
			return FALSE;
		}
		pfnDeviceCapabilities =
		    (LPFNDEVCAPS)GetProcAddress(hlib, "DeviceCapabilities");
		if(pfnDeviceCapabilities == (LPFNDEVCAPS)NULL) {
			FreeLibrary(hlib);
			return FALSE;
		}
		size = pfnExtDeviceMode(NULL, hlib, NULL, device, output, NULL,
		                        NULL, 0);
		if((podevmode =
		        gs_malloc(wdev->memory, size, 1, "win_pr2_getdc")) ==
		   (LPDEVMODE)NULL) {
		...
		pfnExtDeviceMode(NULL, hlib, podevmode, device, output, NULL,
		                 NULL, DM_OUT_BUFFER);
	}

	/* now find out what paper sizes are available */
	devcapsize =
	    pfnDeviceCapabilities(device, output, DC_PAPERSIZE, NULL, NULL);
	devcapsize *= sizeof(POINT);
	if((devcap = gs_malloc(wdev->memory, devcapsize, 1, "win_pr2_getdc")) ==
	   (LPBYTE)NULL)
		return FALSE;
	...
	/* get the dmPaperSize */
	devcapsize =
	    pfnDeviceCapabilities(device, output, DC_PAPERS, NULL, NULL);
	...
	/* get the paper name */
	devcapsize =
	    pfnDeviceCapabilities(device, output, DC_PAPERNAMES, NULL, NULL);
	devcapsize *= 64;
	if((devcap = gs_malloc(wdev->memory, devcapsize, 1, "win_pr2_getdc")) ==
	   (LPBYTE)NULL)
		return FALSE;
	...
	if((wdev->user_paper) && (wdev->user_paper != papersize)) {
	...
	pidevmode->dmFields &= ~(DM_PAPERSIZE | DM_ORIENTATION | DM_COLOR |
	                         DM_PAPERLENGTH | DM_PAPERWIDTH | DM_DUPLEX);
	...
	if(wdev->win32_hdevmode) {
		LPDEVMODE pdevmode =
		    (LPDEVMODE)GlobalLock(wdev->win32_hdevmode);
		if(pdevmode) {
...
win_pr2_update_dev(gx_device_win_pr2* dev, LPDEVMODE pdevmode)
{
	...
	if(pdevmode->dmFields & DM_PAPERSIZE) {
		...
		dev->user_media_size[1] =
		    pdevmode->dmPaperLength / 254.0 * 72.0;
...
#define END_ARRAY_PARAM(pa, e)                                                 \
	...
	default:                                                               \
		ecode = code;                                                  \
	e:                                                                     \
		param_signal_error(dict.list, param_name, ecode);              \
	case 1:                                                                \
...
win_pr2_read_user_settings(gx_device_win_pr2* wdev, gs_param_list* plist)
{
	...
	case 0: {
		...
		BEGIN_ARRAY_PARAM(param_read_int_array, "DocumentRange", ia, 2,
		                  ia)
		if((ia.data[0] < 0) || (ia.data[1] < 0) ||
		   (ia.data[0] > ia.data[1]))
			ecode = gs_note_error(gs_error_rangecheck);
		...
		BEGIN_ARRAY_PARAM(param_read_int_array, "SelectedRange", ia, 2,
		                  ia)
		if((ia.data[0] < 0) || (ia.data[1] < 0) ||
		   (ia.data[0] > ia.data[1]))
			ecode = gs_note_error(gs_error_rangecheck);
		...
		switch(code = param_read_string(
		           dict.list, (param_name = "DocumentName"), &docn)) {
		case 0:
...
win_pr2_write_user_settings(gx_device_win_pr2* wdev, gs_param_list* plist)
{
	...
	code = param_write_bool(dict.list, "UserChangedSettings",
	                        &wdev->user_changed_settings);
...
win_pr2_print_setup_interaction(gx_device_win_pr2* wdev, int mode)
{
	...
	if(wdev->use_old_spool_name) {
		sprintf(wdev->fname, "\\\\spool\\%s",
		        (char*)(devnames) + (devnames->wDeviceOffset));
	} else {
		sprintf(wdev->fname, "%%printer%%%s",
		        (char*)(devnames) + (devnames->wDeviceOffset));
	}
	...
	if(devmode->dmFields & DM_DUPLEX) {
		wdev->Duplex_set = 1;
		wdev->Duplex =
		    devmode->dmDuplex == DMDUP_SIMPLEX ? false : true;
		wdev->tumble =
		    devmode->dmDuplex == DMDUP_HORIZONTAL ? true : false;
	}

	{
		...
		if(hic) {
			...
			wdev->user_media_size[0] =
			    GetDeviceCaps(hic, PHYSICALWIDTH) * 72.0 / xppinch;
			wdev->user_media_size[1] =
			    GetDeviceCaps(hic, PHYSICALHEIGHT) * 72.0 / yppinch;
			DeleteDC(hic);
		}
	}
...
win_pr2_copy_check(gx_device_win_pr2* wdev)
{
	...
	if(devmode_len) {
		...
		if(wdev->win32_hdevmode) {
			memcpy(GlobalLock(wdev->win32_hdevmode),
			       GlobalLock(hdevmode), devmode_len);
			GlobalUnlock(wdev->win32_hdevmode);
	...
	if(devnames_len) {
		...
		if(wdev->win32_hdevnames) {
			memcpy(GlobalLock(wdev->win32_hdevnames),
			       GlobalLock(hdevnames), devnames_len);
			GlobalUnlock(wdev->win32_hdevnames);
