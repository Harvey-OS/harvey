...
static int
put_int32(unsigned char* p, const unsigned int q)
{
	*p++ = (q >> 24) & 0xFF;
	*p++ = (q >> 16) & 0xFF;
	*p++ = (q >> 8) & 0xFF;
...
inode_clear(romfs_inode* node)
{
	...
	if(node) {
		...
		if(node->data_lengths)
			free(node->data_lengths);
		if(node->name)
			free(node->name);
	}
}
...
inode_write(FILE* out, romfs_inode* node)
{
	...
	/* write block sizes */
	offset += fwrite(node->data_lengths, node->blocks,
	                 sizeof(*node->data_lengths), out);
...
main(int argc, char* argv[])
{
	...
	printf("compressing with %d byte blocksize (zlib output buffer %d "
	       "bytes)\n",
	...
	for(i = 1; i < argc; i++) {
		...
		fclose(in);
		node->offset = 12 + 4 * node->blocks + node->data_size +
		               strlen(node->name);
		printf("inode %d (%ld/%ld bytes) '%s'\t%ld%%\n", i,
		       node->data_size, node->length, node->name,
		       100 * node->data_size / node->length);
