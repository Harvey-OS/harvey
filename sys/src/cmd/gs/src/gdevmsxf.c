...
private
const gx_xfont_procs win_xfont_procs =
    {
     win_lookup_font,
     win_char_xglyph,
     win_char_metrics,
		...
		gs_memory_t *mem)
{
	...
	f.lf.lfWeight =
	    (string_match(fname, len, "*Bold*", 6, NULL) ? FW_BOLD : FW_REGULAR);
	f.lf.lfItalic =
		...
		int xo, int yo, gx_color_index color, int required)
{
	...
	if(dev->dname == gs_mswin_device.dname &&
	   wdev->hdctext != NULL && !wxf->invert_y) { /* Display the character directly */
