...
static const char* const type_strings[] = {REF_TYPE_DEBUG_PRINT_STRINGS};
...
debug_print_full_ref(const gs_memory_t* mem, const ref* pref)
{
	...
	switch(r_type(pref)) {
	case t_array:
		dprintf2("array(%u)0x%lx", size, (uint32_t)pref->value.refs);
		break;
	case t_astruct:
		goto strct;
	case t_boolean:
		...
		break;
	case t_device:
		dprintf1("device 0x%lx", (uint32_t)pref->value.pdevice);
		break;
	case t_dictionary:
		dprintf3("dict(%u/%u)0x%lx", dict_length(pref),
		         dict_maxlength(pref), (uint32_t)pref->value.pdict);
		break;
	case t_file:
		dprintf1("file 0x%lx", (uint32_t)pref->value.pfile);
		break;
	case t_fontID:
		goto strct;
	case t_integer:
	...
	case t_mixedarray:
		dprintf2("mixed packedarray(%u)0x%lx", size,
		         (uint32_t)pref->value.packed);
		break;
	case t_name:
		dprintf2("name(0x%lx#%u)", (uint32_t)pref->value.pname,
		         name_index(mem, pref));
	...
	case t_null:
		...
		break;
	case t_oparray:
		dprintf2("op_array(%u)0x%lx:", size,
		         (uint32_t)pref->value.const_refs);
		{
			const op_array_table* opt =
			    op_index_op_array_table(size);

			name_index_ref(
			    mem, opt->nx_table[size - opt->base_index], &nref);
		}
		debug_print_name(mem, &nref);
		break;
	case t_operator:
	...
	case t_shortarray:
		...
		break;
	case t_string:
		dprintf2("string(%u)0x%lx", size, (uint32_t)pref->value.bytes);
		break;
	case t_struct:
	strct : {
...
{
	uint16_t elt = *pref & packed_value_mask;
	ref nref;
	...
	ptn:
		...
		dprintf2("(0x%lx#%u)", (uint32_t)nref.value.pname, elt);
		debug_print_name(mem, &nref);
		break;
	default:
...
/* Dump one ref. */
private
void print_ref_data(const gs_memory_t* mem, const ref*);
void
debug_dump_one_ref(const gs_memory_t* mem, const ref* p)
{
	uint attrs = r_type_attrs(p);
	uint type = r_type(p);
	static const ref_attr_print_mask_t apm[] = {REF_ATTR_PRINT_MASKS,
	                                            {0, 0, 0}};
	...
	for(; ap->mask; ++ap)
		if((attrs & ap->mask) == ap->value)
			dputc(ap->print);
	dprintf2(" 0x%04x 0x%08lx", r_size(p), *(const uint32_t*)&p->value);
...
void
debug_dump_refs(const gs_memory_t* mem, const ref* from, uint size,
                const char* msg)
{
	const ref* p = from;
	uint count = size;
	...
	if(size && msg)
		dprintf2("%s at 0x%lx:\n", msg, (uint32_t)from);
	while(count--) {
		dprintf2("0x%lx: 0x%04x ", (uint32_t)p, r_type_attrs(p));
...
/* Dump a stack. */
void
debug_dump_stack(const gs_memory_t* mem, const ref_stack_t* pstack,
                 const char* msg)
{
	uint i;
	const char* m = msg;
	...
	for(i = ref_stack_count(pstack); i != 0;) {
		...
		dprintf2("0x%lx: 0x%02x ", (uint32_t)p, r_type(p));
		debug_dump_one_ref(mem, p);
		dputc('\n');
	}
}
...
debug_dump_array(const gs_memory_t* mem, const ref* array)
{
	...
	default:
		dprintf2("%s at 0x%lx isn't an array.\n",
		         (type < countof(type_strings) ? type_strings[type]
		                                       : "????"),
		         (uint32_t)array);
		return;
	case t_oparray:
	...
	for(len = r_size(array), pp = array->value.packed; len > 0;
	    len--, pp = packed_next(pp)) {
		ref temp;

		packed_get(mem, pp, &temp);
		if(r_is_packed(pp)) {
			dprintf2("0x%lx* 0x%04x ", (uint32_t)pp, (uint)*pp);
			print_ref_data(mem, &temp);
		} else {
			dprintf2("0x%lx: 0x%02x ", (uint32_t)pp, r_type(&temp));
