...
#define gs_color_rendering_state_common                                        \
	...
	gx_transfer_map* effective_transfer                                    \
	    [GX_DEVICE_COLOR_MAX_COMPONENTS]; /* see below */                  \
                                                                               \
	/* Color caches: */                                                    \
                                                                               \
...
#define gs_cr_state_do_rc_ptrs(m)                                              \
	m(halftone) m(dev_ht) m(cie_render) m(black_generation)                \
	    m(undercolor_removal) m(set_transfer.red) m(set_transfer.green)    \
	        m(set_transfer.blue) m(set_transfer.gray) m(cie_joint_caches)

/* Enumerate the pointers in a c.r. state. */
#define gs_cr_state_do_ptrs(m)                                                 \
	m(0, halftone) m(1, dev_ht) m(2, cie_render) m(3, black_generation)    \
	    m(4, undercolor_removal) m(5, set_transfer.red)                    \
	        m(6, set_transfer.green) m(7, set_transfer.blue)               \
	            m(8, set_transfer.gray) m(9, cie_joint_caches)             \
	                m(10, pattern_cache)
/*
...
typedef struct gs_devicen_color_map_s {
	...
	uint
	    num_components; /* Input - Duplicate of value in gs_device_n_params
	                       */
...
#define gs_imager_state_common                                                 \
	...
	float flatness;                                                        \
	gs_fixed_point                                                         \
	    fill_adjust; /* A path expansion for fill; -1 = dropout            \
	                    prevention*/                                       \
	bool stroke_adjust;                                                    \
...
#define gs_imager_state_initial(scale)                                         \
	0, 0, {gx_line_params_initial},                                        \
	    {(float)(scale), 0.0, 0.0, (float)(-(scale)), 0.0, 0.0}, false,    \
	    {0, 0}, {0, 0}, false, lop_default, gx_max_color_value,            \
	    BLEND_MODE_Compatible, {1.0, 0}, {1.0, 0}, 0, 0 /*false*/, 0, 0,   \
	    0 /*false*/, 0, 0, 1.0, {fixed_half, fixed_half}, 0 /*false*/,     \
	    0 /*false*/, 0 /*false*/, 1.0, gx_default_get_cmap_procs

/* The imager state structure is public only for subclassing. */
#define public_st_imager_state() /* in gsistate.c */                           \
	gs_public_st_composite(st_imager_state, gs_imager_state,               \
	                       "gs_imager_state", imager_state_enum_ptrs,      \
...
gs_imager_state* gs_imager_state_copy(const gs_imager_state* pis,
                                      gs_memory_t* mem);
...
int gs_currentscreenphase_pis(const gs_imager_state*, gs_int_point*,
                              gs_color_select_t);
