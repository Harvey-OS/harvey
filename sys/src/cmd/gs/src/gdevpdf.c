...
ENUM_PTRS_WITH(device_pdfwrite_enum_ptrs, gx_device_pdf* pdev)
{
	...
	if(index < NUM_RESOURCE_TYPES * NUM_RESOURCE_CHAINS)
		ENUM_RETURN(pdev->resources[index / NUM_RESOURCE_CHAINS]
		                .chains[index % NUM_RESOURCE_CHAINS]);
...
#undef e1
#define e1(i, elt)                                                             \
	ENUM_PARAM_STRING_PTR(i + gx_device_pdf_num_ptrs, gx_device_pdf, elt);
    gx_device_pdf_do_param_strings(e1)
#undef e1
#define e1(i, elt)                                                             \
	ENUM_STRING_PTR(i + gx_device_pdf_num_ptrs +                           \
...
#undef r1
	{
		...
		for(i = 0; i < NUM_RESOURCE_TYPES; ++i)
			for(j = 0; j < NUM_RESOURCE_CHAINS; ++j)
				RELOC_PTR(gx_device_pdf,
				          resources[i].chains[j]);
		for(i = 0; i <= pdev->outline_depth; ++i) {
			RELOC_PTR(gx_device_pdf,
			          outline_levels[i].first.action);
...
/* Driver procedures */
private
dev_proc_open_device(pdf_open);
private
dev_proc_output_page(pdf_output_page);
private
...
pdf_close_temp_file(gx_device_pdf* pdev, pdf_temp_file_t* ptf, int code)
{
	...
	if(ptf->strm) {
		if(s_is_valid(ptf->strm)) {
			...
			ptf->file = file =
			    0; /* file was closed by finalization */
	...
	ptf->save_strm = 0;
	return (code < 0 ? code : err != 0 ? gs_note_error(gs_error_ioerror)
	                                   : code);
}
private
int
...
pdf_reset_page(gx_device_pdf* pdev)
{
	...
	memset(
	    pdev->cs_Patterns, 0,
	    sizeof(pdev->cs_Patterns)); /* simplest to create for each page */
...
pdf_open_temp_file(gx_device_pdf* pdev, pdf_temp_file_t* ptf)
{
	...
	ptf->file = gp_open_scratch_file(gp_scratch_file_name_prefix,
	                                 ptf->file_name, fmode);
	if(ptf->file == 0)
...
pdf_initialize_ids(gx_device_pdf* pdev)
{
	 ...
	 */
	{
		struct tm tms;
		time_t t;
		char buf[1 + 2 + 4 + 2 + 2 + 2 + 2 + 2 + 2 + 1 +
		         1]; /* (D:yyyymmddhhmmss)\0 */

		time(&t);
		tms = *localtime(&t);
		sprintf(buf, "(D:%04d%02d%02d%02d%02d%02d)", tms.tm_year + 1900,
		        tms.tm_mon + 1, tms.tm_mday, tms.tm_hour, tms.tm_min,
		        tms.tm_sec);
		cos_dict_put_c_key_string(pdev->Info, "/CreationDate",
		                          (byte*)buf, strlen(buf));
...
private
const byte pad[32] = {0x28, 0xBF, 0x4E, 0x5E, 0x4E, 0x75, 0x8A, 0x41,
...
pdf_compute_encryption_data(gx_device_pdf* pdev)
{
	...
	if(pdev->EncryptionV == 2 && pdev->EncryptionR == 2 &&
	   pdev->KeyLength > 40) {
		eprintf("Encryption version 2 revision 2 with KeyLength > 40 "
		        "appears incompatible to some viewers. With long keys "
		        "use revision 3.\n");
		return_error(gs_error_rangecheck);
	}
	/* Compute O : */
	...
	if(pdev->EncryptionR == 3)
		Adobe_magic_loop_19(buf, sizeof(buf), digest,
		                    pdev->KeyLength / 8);
	...
	t = (byte)(pdev->Permissions >> 0);
	md5_append(&md5, &t, 1);
	t = (byte)(pdev->Permissions >> 8);
	md5_append(&md5, &t, 1);
	t = (byte)(pdev->Permissions >> 16);
	md5_append(&md5, &t, 1);
	t = (byte)(pdev->Permissions >> 24);
	md5_append(&md5, &t, 1);
	...
	if(pdev->EncryptionR == 3) {
		...
		md5_append(&md5, pdev->fileID, sizeof(pdev->fileID));
		md5_finish(&md5, digest);
		s_arcfour_set_key(&sarc4, pdev->EncryptionKey,
		                  pdev->KeyLength / 8);
		s_arcfour_process_buffer(&sarc4, digest, sizeof(digest));
		Adobe_magic_loop_19(digest, sizeof(digest), pdev->EncryptionKey,
		                    pdev->KeyLength / 8);
		...
		s_arcfour_set_key(&sarc4, pdev->EncryptionKey,
		                  pdev->KeyLength / 8);
		s_arcfour_process_buffer(&sarc4, pdev->EncryptionU,
		                         sizeof(pdev->EncryptionU));
	}
	return 0;
}
...
pdf_set_process_color_model(gx_device_pdf* pdev, int index)
{
	...
	switch(index) {
	case 0: /* DeviceGray */
		set_dev_proc(pdev, map_rgb_color,
...
pdf_open(gx_device* dev)
{
	 ...
	 */
	pdev->local_named_objects = pdev->global_named_objects =
	...
	pdev->Namespace_stack =
	    cos_array_alloc(pdev, "pdf_open(Namespace stack)");
	...
	if(pdev->OwnerPassword.size > 0) {
		...
		eprintf("User password is specified. Need an Owner password or "
		        "both.\n");
		return_error(gs_error_rangecheck);
	...
	pdev->sbstack =
	    gs_alloc_struct_array(mem, pdev->sbstack_size, pdf_substream_save,
	...
	memset(pdev->sbstack, 0,
	       pdev->sbstack_size * sizeof(pdf_substream_save));
...
#define Bug687800
		...
		const gs_point* pbox =
		    &(page != NULL ? page : &pdev->pages[0])->MediaBox;
...
pdf_close_page(gx_device_pdf* pdev)
{
	...
	if(pdev->MaxViewerMemorySize < 10000000) {
		...
		if(pdev->ForOPDFRead) {
			pdf_resource_t* pres = pdf_find_resource_by_resource_id(
			    pdev, resourcePage, pdev->contents_id);

			if(pres != NULL) {
				code = cos_dict_put_c_strings(
	...
	page->text_rotation.Rotate =
	    (pdev->params.AutoRotatePages == arp_PageByPage
	         ? pdf_dominant_rotation(&page->text_rotation)
	         : -1);
	{
		int i;

		for(i = 0; i < countof(page->text_rotation.counts); ++i)
			pdev->text_rotation.counts[i] +=
			    page->text_rotation.counts[i];
	}
...
pdf_close(gx_device* dev)
{
	...
	if(pdev->OwnerPassword.size > 0) {
		...
		stream_puts(s, "/O ");
		pdf_put_string(pdev, pdev->EncryptionO,
		               sizeof(pdev->EncryptionO));
		stream_puts(s, "\n/U ");
		pdf_put_string(pdev, pdev->EncryptionU,
		               sizeof(pdev->EncryptionU));
		stream_puts(s, ">>\n");
		pdf_end_obj(pdev);
		s = pdev->strm;
	...
	if(pdev->max_referred_page >= pdev->next_page + 1) {
		...
		eprintf2("ERROR: A pdfmark destination page %d points beyond "
		         "the last page %d.\n",
