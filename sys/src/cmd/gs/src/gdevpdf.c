...
pdf_close_temp_file(gx_device_pdf *pdev, pdf_temp_file_t *ptf, int code)
{
	...
	ptf->save_strm = 0;
	return (code < 0 ? code : err != 0 ? gs_note_error(gs_error_ioerror) : code);
}
private
int
...
pdf_close_page(gx_device_pdf *pdev)
{
	...
	page->text_rotation.Rotate =
	    (pdev->params.AutoRotatePages == arp_PageByPage ? pdf_dominant_rotation(&page->text_rotation) : -1);
	{
		int i;
...
pdf_output_page(gx_device *dev, int num_copies, int flush)
{
	...
	return (code < 0 ? code : pdf_ferror(pdev) ? gs_note_error(gs_error_ioerror) : gx_finish_output_page(dev, num_copies, flush));
}
