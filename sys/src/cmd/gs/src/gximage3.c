...
/* Copyright (C) 1997, 1998, 1999, 2000 Aladdin Enterprises.  All rights
  reserved.
...
/* Forward references */
private
dev_proc_begin_typed_image(gx_begin_image3);
private
image_enum_proc_plane_data(gx_image3_plane_data);
private
image_enum_proc_end_image(gx_image3_end_image);
private
image_enum_proc_flush(gx_image3_flush);
private
image_enum_proc_planes_wanted(gx_image3_planes_wanted);
...
const gx_image_type_t gs_image_type_3 = {
    &st_gs_image3, gx_begin_image3, gx_data_image_source_size, gx_image_no_sput,
    gx_image_no_sget, gx_image_default_release, 3};
private
const gx_image_enum_procs_t image3_enum_procs = {
    gx_image3_plane_data, gx_image3_end_image, gx_image3_flush,
    gx_image3_planes_wanted};
...
gs_private_st_suffix_add6(st_image3_enum, gx_image3_enum_t, "gx_image3_enum_t",
                          ...
                          st_gx_image_enum_common, mdev, pcdev, pixel_info,
                          mask_info, pixel_data, mask_data);

/* Define the default implementation of ImageType 3 processing. */
private
IMAGE3_MAKE_MID_PROC(make_mid_default); /* check prototype */
private
int
make_mid_default(gx_device** pmidev, gx_device* dev, int width, int height,
                 gs_memory_t* mem)
{
	gx_device_memory* midev = gs_alloc_struct(
	    mem, gx_device_memory, &st_device_memory, "make_mid_default");
	int code;
...
private
IMAGE3_MAKE_MCDE_PROC(make_mcde_default); /* check prototype */
private
int
                  ...
                  const gs_int_point* origin)
{
	...
	code = gx_mask_clip_initialize(mcdev, &gs_mask_clip_device,
	                               (const gx_bitmap*)&bits, dev, origin->x,
	                               origin->y, mem);
	if(code < 0) {
	...
	mcdev->tiles = bits;
	code = dev_proc(mcdev, begin_typed_image)((gx_device*)mcdev, pis, pmat,
...
private
int
gx_begin_image3(gx_device* dev, const gs_imager_state* pis,
                const gs_matrix* pmat, const gs_image_common_t* pic,
                const gs_int_rect* prect, const gx_drawing_color* pdcolor,
                const gx_clip_path* pcpath, gs_memory_t* mem,
 ...
 */
private
bool check_image3_extent(floatp mask_coeff, floatp data_coeff);
int
gx_begin_image3_generic(gx_device* dev, const gs_imager_state* pis,
                        const gs_matrix* pmat, const gs_image_common_t* pic,
                        ...
                        gx_image_enum_common_t** pinfo)
{
		...
		return_error(gs_error_rangecheck);
	{
		...
		if((code = gs_point_transform(pim->Width, pim->Height,
		                              &mi_pixel, &ep)) < 0 ||
	...
	gx_image_enum_common_init(
	    ...
	    &image3_enum_procs, dev, 1 + penum->num_components, pim->format);
	...
	if(prect) {
		...
		mask_rect.q.x =
		    (int)((data_rect.q.x + pim->Width - 1) * lmw / pim->Width);
	...
	if(pim->InterleaveType == interleave_chunky) {
		...
		penum->pixel_data = gs_alloc_bytes(
		    mem, (penum->pixel_width * pim->BitsPerComponent *
	...
	origin.x =
	    (mrect.p.x < 0) ? (int)ceil(mrect.p.x) : (int)floor(mrect.p.x);
	origin.y =
	    (mrect.p.y < 0) ? (int)ceil(mrect.p.y) : (int)floor(mrect.p.y);
	...
	}
	{
		...
		code = gx_device_begin_typed_image(
		    mdev, NULL, &m_mat, (const gs_image_common_t*)&i_mask,
		    &mask_rect, &dcolor, NULL, mem, &penum->mask_info);
		if(code < 0)
			goto out2;
	}
	...
	case interleave_chunky:
		...
		penum->plane_depths[0] = penum->pixel_info->plane_depths[0] *
		                         (penum->num_components + 1) /
		                         penum->num_components;
		break;
	case interleave_scan_lines:
		/*
		 * There is only 1 plane, with dynamically changing width &
		 * depth.
		...
		break;
	case interleave_separate_source:
		/* Insert the mask data as a separate plane before the image
		 * data. */
		penum->num_planes = penum->pixel_info->num_planes + 1;
		penum->plane_widths[0] = pim->MaskDict.Width;
		penum->plane_depths[0] = 1;
		memcpy(&penum->plane_widths[1],
		       &penum->pixel_info->plane_widths[0],
		       (penum->num_planes - 1) *
		           sizeof(penum->plane_widths[0]));
		memcpy(&penum->plane_depths[1],
...
planes_next(const gx_image3_enum_t* penum)
{
	...
	long current =
	    penum->pixel_y * (long)mask_h - penum->mask_y * (long)pixel_h;
                     ...
                     const gx_image_plane_t* planes, int height, int* rows_used)
{
	...
	int mask_used = 0;
	int h1 =
	    max(pixel_height - penum->pixel_y, mask_height - penum->mask_y);
	int h = min(height, h1);
	...
	*rows_used = 0;
	...
	case interleave_chunky:
		...
		if(h > 1) {
			...
			mask_plane = planes[0];
			do {
				code = gx_image3_plane_data(info, &mask_plane,
				                            1, rows_used);
				h -= *rows_used;
				if(code)
					break;
		...
		}
		{
			 ...
			 */
			uint bit_x =
			    bpc * (num_components + 1) * planes[0].data_x;
			...
			for(x = 0; x < width; ++x) {
				...
				sample_store_next12(value != 0, mptr, mbit, 1,
				                    mbbyte);
				for(i = 0; i < num_components; ++i) {
					sample_load_next12(value, sptr, sbit,
					                   bpc);
					sample_store_next12(value, pptr, pbit,
					                    bpc, pbbyte);
				}
			}
	...
	if(mask_plane.data) {
		 ...
		 * successfully
		 * but processing the pixel rows was interrupted, we set
		 * rows_used
		 * to indicate the number of pixel rows processed (since there
		 * is
		 * no way to return two rows_used values).  If this happened,
		 * some
		...
		if(skip >= h) {
			...
			code = gx_image_plane_data_rows(
			    penum->mask_info, &mask_plane, mask_h, &mask_used);
			mask_used += skip;
		}
		*rows_used = mask_used;
	...
	if(pixel_planes[0].data) {
		...
		code = gx_image_plane_data_rows(penum->pixel_info, pixel_planes,
		                                h, &pixel_used);
		/*
...
gx_image3_planes_wanted(const gx_image_enum_common_t* info, byte* wanted)
{
	...
	case interleave_separate_source: {
		/*
		 * We always want at least as much of the mask to be filled as
		 * the
		 * pixel data.  next > 0 iff we've processed more data than
		 * mask.
		 * Plane 0 is the mask, planes [1 .. num_planes - 1] are pixel
		 * data.
		 */
		int next = planes_next(penum);

		wanted[0] = (next >= 0 ? 0xff : 0);
		memset(wanted + 1, (next <= 0 ? 0xff : 0),
		       info->num_planes - 1);
		/*
...
gx_image3_end_image(gx_image_enum_common_t* info, bool draw_last)
{
	...
	gs_free_object(mem, penum->mask_data, "gx_image3_end_image(mask_data)");
	...
	return (pcode < 0 ? pcode : mcode < 0 ? mcode : code1 < 0 ? code1
	                                                          : code2);
}
