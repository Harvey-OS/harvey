...
private
int
current_float_value(i_ctx_t* i_ctx_p, float (*current_value)(const gs_state*))
{
	os_ptr op = osp;
...
private
int
enum_param(const gs_memory_t* mem, const ref* pnref, const char* const names[])
{
	const char* const* p;
	ref nsref;
...
private
const int8_t* const blend_mode_names[] = {GS_BLEND_MODE_NAMES, 0};
...
zbegintransparencygroup(i_ctx_t* i_ctx_p)
{
	...
	if((code = dict_bool_param(dop, "Isolated", false, &params.Isolated)) <
	       0 ||
	   (code = dict_bool_param(dop, "Knockout", false, &params.Knockout)) <
	       0)
		return code;
...
private
int tf_using_function(floatp, float*, void*);
private
int
...
/* <dict> .image3x - */
private
int mask_dict_param(const gs_memory_t* mem, os_ptr, image_params*, const char*,
                    int, gs_image3x_mask_t*);
private
int
zimage3x(i_ctx_t* i_ctx_p)
{
	...
	if((code = pixel_image_params(i_ctx_p, pDataDict,
	                              (gs_pixel_image_t*)&image, &ip_data, 12,
	                              false)) < 0 ||
	   (code = dict_int_param(pDataDict, "ImageType", 1, 1, 0, &ignored)) <
	       0)
		return code;
	/*
	 ...
	 */
	if((code = mask_dict_param(imemory, op, &ip_data, "ShapeMaskDict",
	                           num_components, &image.Shape)) < 0 ||
	   (code = mask_dict_param(imemory, op, &ip_data, "OpacityMaskDict",
	                           num_components, &image.Opacity)) < 0)
		return code;
	return zimage_setup(i_ctx_p, (gs_pixel_image_t*)&image,
	                    &ip_data.DataSource[0], image.CombineWithColor, 1);
}

/* Get one soft-mask dictionary parameter. */
private
int
mask_dict_param(const gs_memory_t* mem, os_ptr op, image_params* pip_data,
                const char* dict_name, int num_components,
                gs_image3x_mask_t* pixm)
{
	...
	if((mcode = code = data_image_params(mem, pMaskDict, &pixm->MaskDict,
	                                     &ip_mask, false, 1, 12, false)) <
	       0 ||
	   (code = dict_int_param(pMaskDict, "ImageType", 1, 1, 0, &ignored)) <
	       0 ||
