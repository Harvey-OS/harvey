...
/* Copyright (C) 1994, 1996, 1997, 1999 Aladdin Enterprises.  All rights
  reserved.
...
private
struct_proc_enum_ptrs(font_type0_enum_ptrs);
private
struct_proc_reloc_ptrs(font_type0_reloc_ptrs);
...
case fmap_SubsVector:
	ENUM_RETURN_CONST_STRING_PTR(gs_font_type0, data.SubsVector);
case fmap_CMap:
...
ENUM_PTRS_END
private
RELOC_PTRS_WITH(font_type0_reloc_ptrs, gs_font_type0* pfont)
...
gs_type0_define_font(gs_font_dir* pdir, gs_font* pfont)
{
	...
	if(pmat->xx == 1.0 && pmat->yy == 1.0 && pmat->xy == 0.0 &&
	   pmat->yx == 0.0 && pmat->tx == 0.0 && pmat->ty == 0.0)
		return 0;
