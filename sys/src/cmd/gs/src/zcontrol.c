...
/* Forward references */
private
int no_cleanup(i_ctx_t*);
private
uint count_exec_stack(i_ctx_t*, bool);
private
uint count_to_stopped(i_ctx_t*, int32_t);
private
int unmatched_exit(os_ptr, op_proc_t);
...
private
int end_runandhide(i_ctx_t*);
private
int err_end_runandhide(i_ctx_t*);
private
int
...
private
int for_pos_int_continue(i_ctx_t*), for_neg_int_continue(i_ctx_t*),
 ...
 */
private
int for_samples_continue(i_ctx_t*);
...
zinstopped(i_ctx_t* i_ctx_p)
{
	...
	if(count) {
		push(1);
		op[-1] =
		    *ref_stack_index(&e_stack, count - 2); /* default result */
		make_true(op);
	} else
		make_false(op);
	return 0;
...
private
int execstack2_continue(i_ctx_t*);
private
int
push_execstack(i_ctx_t* i_ctx_p, os_ptr op1, bool include_marks, op_proc_t cont)
{
	uint size;
	/*
...
zexecstack2(i_ctx_t* i_ctx_p)
{
	...
	check_type(*op, t_boolean);
	return push_execstack(i_ctx_p, op - 1, op->value.boolval,
	                      execstack2_continue);
}
...
do_execstack(i_ctx_t* i_ctx_p, bool include_marks, os_ptr op1)
{
	...
	for(i = 0, rq = arefs + asize; rq != arefs; ++i) {
		...
		case t_struct:
		case t_astruct: {
			const char* tname = gs_struct_type_name_string(
...
zcurrentfile(i_ctx_t* i_ctx_p)
{
	...
	} else if((fp = zget_current_file(i_ctx_p)) ==
	          0) { /* Return an invalid file object. */
...
const op_def zcontrol1_op_defs[] = {{"1.cond", zcond},
                                    ...
                                    op_def_end(0)};
const op_def zcontrol2_op_defs[] = {{"4for", zfor},
                                    {"1loop", zloop},
                                    {"2.quit", zquit},
                                    {"2repeat", zrepeat},
                                    {"0stop", zstop},
...
count_exec_stack(i_ctx_t* i_ctx_p, bool include_marks)
{
	...
	if(!include_marks) {
		...
		for(i = count; i--;)
			if(r_has_type_attrs(
			       ref_stack_index(&e_stack, (int32_t)i), t_null,
			       a_executable))
				--count;
	}
	return count;
