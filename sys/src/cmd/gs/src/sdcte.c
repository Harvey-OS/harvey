               ...
               stream_cursor_write* pw, bool last)
{
	...
	case 0: /* not initialized yet */
		if(gs_jpeg_start_compress(ss, TRUE) < 0)
			return ERRC;
		if_debug4('w', "[wde]width=%u, height=%u, components=%d, "
		               "scan_line_size=%u\n",
		          jcdp->cinfo.image_width, jcdp->cinfo.image_height,
		          jcdp->cinfo.input_components, ss->scan_line_size);
...
#undef ADOBE_MARKER_LEN
	...
	case 3: /* markers written, processing data */
		while(jcdp->cinfo.image_height > jcdp->cinfo.next_scanline) {
			...
			if(written < 0)
				return ERRC;
			if_debug3(
			    'w', "[wde]write returns %d, used=%u, written=%u\n",
			    written, (uint)(samples - 1 - pr->ptr),
	...
	case 5: /* copy the final data to the output */
		if(jcdp->fcb_pos < jcdp->fcb_size) {
			...
			memcpy(pw->ptr + 1,
			       jcdp->finish_compress_buf + jcdp->fcb_pos,
			       count);
			jcdp->fcb_pos += count;
			pw->ptr += count;
			if(jcdp->fcb_pos < jcdp->fcb_size)
...
const stream_template s_DCTE_template = {
    &st_DCT_state, s_DCTE_init,    s_DCTE_process,     1000,
    4000,          s_DCTE_release, s_DCTE_set_defaults};
