...
zsetseparationspace(i_ctx_t* i_ctx_p)
{
	...
	/*
	 * pcsa is a pointer to element 1 (2nd element)  in the Separation
	 * colorspace
	 * description array.  Thus pcsa[2] is element #3 (4th element) which is
	 * the
	 * tint transform.
	 */
	...
	if((code = name_ref(mem, (const byte*)"None", 4, &name_none, 0)) < 0)
		return code;
	sep_type = (name_eq(&sname, &name_all)
	                ? SEP_ALL
