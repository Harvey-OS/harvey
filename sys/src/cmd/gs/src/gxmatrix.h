...
#define PRECISE_CURRENTPOINT                                                   \
	1 /* Old code compatible with dropped clamping = 0, new code = 1 */
...
int gs_point_transform2fixed_rounding(const gs_matrix_fixed* pmat, floatp x,
...
#define m_fixed(v, c, fc, maxb)                                                \
	(((v) + (fixed_1 << (maxb - 1))) &                                     \
	     ...
	     : arith_rshift(fixed2int_var(v) * (fc).c + (fc).round,            \
	                    (fc).shift))
