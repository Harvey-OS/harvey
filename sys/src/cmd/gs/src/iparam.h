...
/* Copyright (C) 1993, 1995, 1998, 1999 Aladdin Enterprises.  All rights
  reserved.
...
#define iparam_list_common                                                     \
	...
	union {                                                                \
		struct {/* reading */                                          \
			...
			bool                                                   \
			    require_all; /* if true, require all params to be  \
			                    known */                           \
		} r;                                                           \
		struct {/* writing */                                          \
...
int dict_param_list_write(dict_param_list*, ref* /*t_dictionary */, const ref*,
                          gs_ref_memory_t*);
...
int array_param_list_read(array_param_list*, ref*, uint, const ref*, bool,
                          gs_ref_memory_t*);
int stack_param_list_read(stack_param_list*, ref_stack_t*, uint, const ref*,
                          bool, gs_ref_memory_t*);
int stack_param_list_write(stack_param_list*, ref_stack_t*, const ref*,
                           gs_ref_memory_t*);

#define iparam_list_release(plist)                                             \
	gs_free_object((plist)->memory, (plist)->results, "iparam_list_"       \
	                                                  "release")
