...
{
	return (int)(fWidthIn / (scale >= 1.0 ? 1.0 : max(scale, min_scale)) * 2 + 1);
}
...
#define clamp_pixel(j) \
	(j < 0 ? (-j >= limit ? limit - 1 : -j) : j >= limit ? (j >> 1 >= limit ? 0 : (limit - j) + limit - 1) : j)
		int lmin =
		    (left < 0 ? 0 : left);
		int lmax =
		    (left < 0 ? (-left >= limit ? limit - 1 : -left) : left);
		int rmin =
		    (right >= limit ? (right >> 1 >= limit ? 0 : (limit - right) + limit - 1) : right);
		int rmax =
...
#define zoom_x_loop(PixelIn, PixelIn2)                                            \
	...
	for(i = 0; i < tmp_width; tp += Colors, ++clp, ++i) {                     \
		...
		{                                                                 \
			...
			*tp = (PixelTmp)CLAMP(pixel, minPixelTmp, maxPixelTmp);   \
		}                                                                 \
	}
...
#define zoom_y_loop(PixelOut)                                                          \
	for(kc = 0; kc < kn; ++kc) {                                                   \
		...
		{                                                                      \
			...
			if_debug1('W', " %d", pixel);                                  \
			((PixelOut *)dst)[kc] = (PixelOut)CLAMP(pixel, 0, max_weight); \
		}                                                                      \
	}
...
calculate_dst_contrib(stream_IScale_state *ss, int y)
{
	...
	if(last_index < first_index_mod) { /* Shuffle the indices to account for wraparound. */
		...
		for(i = 0; i < MAX_ISCALE_SUPPORT; ++i)
			shuffle[i].weight =
			    (i <= last_index ? ss->dst_items[i + MAX_ISCALE_SUPPORT - first_index_mod].weight : i >= first_index_mod ? ss->dst_items[i - first_index_mod].weight : 0);
