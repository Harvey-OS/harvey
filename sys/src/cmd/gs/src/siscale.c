...
#define num_weight_bits                                                        \
	((sizeof(AccumTmp) - maxSizeofPixel) * 8 -                             \
	 (LOG2_MAX_ISCALE_SUPPORT + 1))
...
#define unscale_AccumTmp(atemp, fraction_bits)                                 \
...
gs_private_st_ptrs5(st_IScale_state, stream_IScale_state,
                    "ImageScaleEncode/Decode state", iscale_state_enum_ptrs,
                    iscale_state_reloc_ptrs, dst, src, tmp, contrib, items);
...
Mitchell_filter(double t)
{
	...
	if(t < 1)
		return ((12 - 9 * B - 6 * C) * (t * t2) +
		        (-18 + 12 * B + 6 * C) * t2 + (6 - 2 * B)) /
		       6;
	else if(t < 2)
		return ((-1 * B - 6 * C) * (t * t2) + (6 * B + 30 * C) * t2 +
		        (-12 * B - 48 * C) * t + (8 * B + 24 * C)) /
		       6;
	else
		return 0;
}
...
#define clamp_pixel(j)                                                         \
	(j < 0 ? (-j >= limit ? limit - 1 : -j)                                \
		...
		int rmin = (right >= limit
		                ? (right >> 1 >= limit ? 0 : (limit - right) +
		                                                 limit - 1)
		                : right);
...
#define zoom_x_loop(PixelIn, PixelIn2)                                         \
	...
	for(i = 0; i < tmp_width; tp += Colors, ++clp, ++i) {                  \
		...
		{                                                              \
			PixelIn2 pixel =                                       \
			    unscale_AccumTmp(weight, fraction_bits);           \
...
zoom_y(void /*PixelOut */* dst, int sizeofPixelOut, uint MaxValueOut,
       const PixelTmp* tmp, int WidthOut, int tmp_width, int Colors,
       const CLIST* contrib, const CONTRIB* items)
{
...
/* Forward references */
private
void s_IScale_release(stream_state* st);
...
calculate_dst_contrib(stream_IScale_state* ss, int y)
{
	...
	int last_index = calculate_contrib(
	    &ss->dst_next_list, ss->dst_items, ss->yscale, y, 1,
	    ss->params.HeightIn, MAX_ISCALE_SUPPORT, row_size,
	...
	if(last_index <
	   first_index_mod) { /* Shuffle the indices to account for wraparound.
	                         */
		...
		for(i = 0; i < MAX_ISCALE_SUPPORT; ++i)
			shuffle[i].weight =
			    (i <= last_index
			         ? ss->dst_items[i + MAX_ISCALE_SUPPORT -
			                         first_index_mod].weight
			         : i >= first_index_mod
			               ? ss->dst_items[i - first_index_mod]
			                     .weight
			               : 0);
		memcpy(ss->dst_items, shuffle,
		       MAX_ISCALE_SUPPORT * sizeof(CONTRIB));
...
s_IScale_init(stream_state* st)
{
	...
	ss->src_y = 0;
	ss->src_size =
	    ss->params.WidthIn * ss->sizeofPixelIn * ss->params.Colors;
	ss->src_offset = 0;
	ss->dst_y = 0;
	ss->dst_size =
	    ss->params.WidthOut * ss->sizeofPixelOut * ss->params.Colors;
	ss->dst_offset = 0;

	/* create intermediate image to hold horizontal zoom */
	ss->tmp = (PixelTmp*)gs_alloc_byte_array(
	    mem, min(ss->tmp_height, MAX_ISCALE_SUPPORT),
	    ss->tmp_width * ss->params.Colors * sizeof(PixelTmp),
	    "image_scale tmp");
	ss->contrib = (CLIST*)gs_alloc_byte_array(
	    mem, max(ss->params.WidthOut, ss->params.HeightOut), sizeof(CLIST),
	    "image_scale contrib");
	ss->items = (CONTRIB*)gs_alloc_byte_array(
	    mem, contrib_pixels(ss->xscale) * ss->params.WidthOut,
	    sizeof(CONTRIB), "image_scale contrib[*]");
	/* Allocate buffers for 1 row of source and destination. */
	ss->dst =
	    gs_alloc_byte_array(mem, ss->params.WidthOut * ss->params.Colors,
	                        ss->sizeofPixelOut, "image_scale dst");
	ss->src =
	    gs_alloc_byte_array(mem, ss->params.WidthIn * ss->params.Colors,
	                        ss->sizeofPixelIn, "image_scale src");
	if(ss->tmp == 0 || ss->contrib == 0 || ss->items == 0 || ss->dst == 0 ||
	   ss->src == 0) {
		s_IScale_release(st);
		return ERRC;
		/****** WRONG ******/
	...
	calculate_contrib(
	    ss->contrib, ss->items, ss->xscale, 0, ss->params.WidthOut,
	    ss->params.WidthIn, ss->params.WidthIn, ss->params.Colors,
	    (double)unitPixelTmp * fixedScaleFactor / ss->params.MaxValueIn);
...
top:
	while(ss->src_y > ss->dst_last_index) { /* We have enough horizontally
	                                           scaled temporary rows */
		...
		if(ss->dst_offset == 0) {
			...
			/* Apply filter to zoom vertically from tmp to dst. */
			zoom_y(row, ss->sizeofPixelOut, ss->params.MaxValueOut,
			       ss->tmp, ss->params.WidthOut, ss->tmp_width,
			       ss->params.Colors, &ss->dst_next_list,
			       ss->dst_items);
		...
		{ /* We're delivering a buffered output row. */
			...
			memcpy(pw->ptr + 1, (byte*)ss->dst + ss->dst_offset,
			       ncopy);
			pw->ptr += ncopy;
			ss->dst_offset += ncopy;
			if(ncopy != wcount)
...

	{
		...
		if(rleft >= rcount) { /* We're going to fill up a row. */
			...
			if(ss->src_offset ==
			   0) { /* We have a complete row.  Read the data */
				/* directly from the input. */
				row = pr->ptr + 1;
			} else { /* We're buffering a row in src. */
				row = ss->src;
				memcpy((byte*)ss->src + ss->src_offset,
				       pr->ptr + 1, rcount);
				ss->src_offset = 0;
			}
			/* Apply filter to zoom horizontally from src to tmp. */
			...
			zoom_x(ss->tmp +
			           (ss->src_y % MAX_ISCALE_SUPPORT) *
			               ss->tmp_width * ss->params.Colors,
			       row, ss->sizeofPixelIn, ss->tmp_width,
			       ss->params.WidthIn, ss->params.Colors,
			       ss->contrib, ss->items);
			pr->ptr += rcount;
			++(ss->src_y);
			goto top;
		} else { /* We don't have a complete row.  Copy data to src
		            buffer. */
...
s_IScale_release(stream_state* st)
{
	...
	gs_free_object(mem, (void*)ss->src,
	               "image_scale src"); /* no longer const */
	ss->src = 0;
...
const stream_template s_IScale_template = {
    &st_IScale_state, s_IScale_init, s_IScale_process, 1, 1, s_IScale_release,
    s_IScale_set_defaults};
