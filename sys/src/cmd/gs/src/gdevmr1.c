...
int
mem_mono_strip_copy_rop(gx_device* dev, const byte* sdata, int sourcex,
                        ...
                        gs_logical_operation_t lop)
{
	...
	if(mdev->palette.data == 0) {
		gx_color_value cv[3];
		cv[0] = cv[1] = cv[2] = 0;
		gdev_mem_mono_set_inverted(
		    mdev, (*dev_proc(dev, map_rgb_color))(dev, cv) != 0);
	}
	invert = mdev->palette.data[0] != 0;

#ifdef DEBUG
	if(gs_debug_c('b'))
		trace_copy_rop("mem_mono_strip_copy_rop", dev, sdata, sourcex,
		               sraster, id, scolors, textures, tcolors, x, y,
		               width, height, phase_x, phase_y, lop);
	if(gs_debug_c('B'))
		debug_dump_bitmap(scan_line_base(mdev, y), mdev->raster, height,
		                  "initial dest bits");
#endif
	...
	/* Handle constant source and/or texture, and other special cases. */
	{
		...
		do_copy:
			return (*dev_proc(dev, copy_mono))(
			    dev, sdata, sourcex, sraster, id, x, y, width,
			    height, color0, color1);
		case rop_usage_DS:
/* This might be a case that copy_mono can handle. */
#define copy_case(c0, c1)                                                      \
	color0 = c0, color1 = c1;                                              \
	goto do_copy;
...
#undef copy_case
			fit_copy(dev, sdata, sourcex, sraster, id, x, y, width,
			         height);
		no_T: /* Texture is not used; textures may be garbage. */
			no_texture.data =
			    scan_line_base(mdev, 0); /* arbitrary */
			no_texture.raster = 0;
			...
			break;
		case rop_usage_T:
			/* This is either T or ~T, which tile_rectangle can
			 * handle. */
			if(rop == rop3_T)
				color0 = 0, color1 = 1;
			else
...
#undef tile_case
	...
	for(; line_count-- > 0; drow += draster, srow += sraster, ++ty) {
		...
		for(; w > 0; sx += nw, dx += nw, w -= nw) {
			...
			if(tskew < 0)
				--tptr, tskew += 8;
			for(; left > 0; left -= nx, mask = 0xff, nx = 8, ++dptr,
			                            ++sptr, ++tptr) {
				byte dbyte = *dptr;
