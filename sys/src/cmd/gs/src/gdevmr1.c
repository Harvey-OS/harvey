...
#define x_offset(px, ty, textures) \
	((textures)->shift == 0 ? (px) : (px) + (ty) / (textures)->rep_height * (textures)->rep_shift)
			...
			int phase_x, int phase_y, gs_logical_operation_t lop)
{
	...
	if(mdev->palette.data == 0) {
		...
		gdev_mem_mono_set_inverted(mdev,
					   (*dev_proc(dev, map_rgb_color))(dev, cv) != 0);
	}
	...
	/* Handle constant source and/or texture, and other special cases. */
	{
		...
		case rop_usage_none:
			...
			return (*dev_proc(dev, fill_rectangle))(dev, x, y, width, height, (gx_color_index)(rop & 1));
		case rop_usage_D:
		...
		case rop_usage_S:
			...
			if(rop == rop3_S)
				...
				color0 = 1, color1 = 0;
		do_copy:
			return (*dev_proc(dev, copy_mono))(dev, sdata, sourcex, sraster, id, x, y, width, height,
							   color0, color1);
		case rop_usage_DS:
...
#undef copy_case
		...
		case rop_usage_T:
			...
			if(rop == rop3_T)
				...
				color0 = 1, color1 = 0;
		do_tile:
			return (*dev_proc(dev, strip_tile_rectangle))(dev, textures, x, y, width, height, color0, color1,
								      phase_x, phase_y);
		case rop_usage_DT:
