	...
	if(pdcolor->type == gx_dc_type_none)
		...
		dprintf5("binary(0x%lx, 0x%lx, %d/%d, index=%d)",
		         ...
		         (ci < 0 ? pdcolor->colors.binary.b_ht->order.num_bits
		                 : pdcolor->colors.binary.b_ht->components[ci]
		                       .corder.num_bits),
		         ci);
...
private
int
trace_copy_mono(gx_device* dev, const byte* data, int dx, int raster,
                gx_bitmap_id id, int x, int y, int w, int h,
                gx_color_index zero, gx_color_index one)
{
	dprintf7(
	    "copy_mono(x=%d, y=%d, w=%d, h=%d, dx=%d, raster=%d, id=0x%lx,\n",
...
private
int
trace_copy_color(gx_device* dev, const byte* data, int dx, int raster,
                 gx_bitmap_id id, int x, int y, int w, int h)
{
	dprintf7(
	    "copy_color(x=%d, y=%d, w=%d, h=%d, dx=%d, raster=%d, id=0x%lx)\n",
	    x, y, w, h, dx, raster, (uint32_t)id);
	return 0;
}
...
{
	dprintf7(
	    "copy_alpha(x=%d, y=%d, w=%d, h=%d, dx=%d, raster=%d, id=0x%lx,\n",
	    x, y, w, h, dx, raster, (uint32_t)id);
...
private
int
trace_fill_mask(gx_device* dev, const byte* data, int dx, int raster,
                gx_bitmap_id id, int x, int y, int w, int h,
                const gx_drawing_color* pdcolor, int depth,
                gs_logical_operation_t lop, const gx_clip_path* pcpath)
{
	dprintf7(
	    "fill_mask(x=%d, y=%d, w=%d, h=%d, dx=%d, raster=%d, id=0x%lx,\n",
	    x, y, w, h, dx, raster, (uint32_t)id);
...
private
int
trace_fill_trapezoid(gx_device* dev, const gs_fixed_edge* left,
...
private
int
trace_fill_parallelogram(gx_device* dev, fixed px, fixed py, fixed ax, fixed ay,
...
private
int
trace_fill_triangle(gx_device* dev, fixed px, fixed py, fixed ax, fixed ay,
                    fixed bx, fixed by, const gx_drawing_color* pdcolor,
                    gs_logical_operation_t lop)
{
	dprintf6("fill_triangle((%g,%g), (%g,%g), (%g,%g)", fixed2float(px),
	         fixed2float(py), fixed2float(ax), fixed2float(ay),
	         fixed2float(bx), fixed2float(by));
	trace_drawing_color(", ", pdcolor);
	trace_lop(lop);
	dputs(")\n");
	return 0;
...
private
int
trace_draw_thin_line(gx_device* dev, fixed fx0, fixed fy0, fixed fx1, fixed fy1,
                     const gx_drawing_color* pdcolor,
                     gs_logical_operation_t lop)
{
	dprintf4("draw_thin_line((%g,%g), (%g,%g)", fixed2float(fx0),
	         fixed2float(fy0), fixed2float(fx1), fixed2float(fy1));
	trace_drawing_color(", ", pdcolor);
	trace_lop(lop);
	dputs(")\n");
	return 0;
...
private
int
trace_strip_tile_rectangle(gx_device* dev, const gx_strip_bitmap* tiles, int x,
                           int y, int w, int h, gx_color_index color0,
                           gx_color_index color1, int px, int py)
{
	dprintf6("strip_tile_rectangle(x=%d, y=%d, w=%d, h=%d, "
	         "colors=(0x%lx,0x%lx),\n",
	         x, y, w, h, (uint32_t)color0, (uint32_t)color1);
	dprintf8(
	    "    size=(%d,%d) shift %u, rep=(%u,%u) shift %u, phase=(%d,%d))\n",
	    tiles->size.x, tiles->size.y, tiles->shift, tiles->rep_width,
	    tiles->rep_height, tiles->rep_shift, px, py);
	return 0;
}
...
private
int
trace_fill_path(gx_device* dev, const gs_imager_state* pis, gx_path* ppath,
...
private
int
trace_stroke_path(gx_device* dev, const gs_imager_state* pis, gx_path* ppath,
...
gs_private_st_suffix_add0(st_trace_image_enum, trace_image_enum_t,
                          ...
                          trace_image_enum_reloc_ptrs, st_gx_image_enum_common);
private
int
trace_plane_data(gx_image_enum_common_t* info, const gx_image_plane_t* planes,
...
private
const gx_image_enum_procs_t trace_image_enum_procs = {trace_plane_data,
                                                      trace_end_image};
private
int
                        ...
                        gx_image_enum_common_t** pinfo)
{
	...
	dprintf7("begin_typed_image(type=%d, ImageMatrix=[%g %g %g %g %g %g]",
	         ...
	         pim->ImageMatrix.yx, pim->ImageMatrix.yy, pim->ImageMatrix.tx,
	         pim->ImageMatrix.ty);
	...
	if(pie == 0)
		goto dflt;
	if(gx_image_enum_common_init(
	       (gx_image_enum_common_t*)pie, (const gs_data_image_t*)pim,
	       &trace_image_enum_procs, dev, ncomp, ppi->format) < 0)
		goto dflt;
...
private
const gs_text_enum_procs_t trace_text_procs = {
    NULL, trace_text_process, NULL, NULL, NULL, NULL, gx_default_text_release};
private
int
                 ...
                 gs_memory_t* memory, gs_text_enum_t** ppenum)
{
	static const char* const tags[sizeof(text->operation) * 8] = {
	    ...
	    "FROM_SINGLE_CHAR", "FROM_SINGLE_GLYPH", "ADD_TO_ALL_WIDTHS",
	    "ADD_TO_SPACE_WIDTH", "REPLACE_WIDTHS", "DO_NONE", "DO_DRAW",
	    "DO_CHARWIDTH", "DO_FALSE_CHARPATH", "DO_TRUE_CHARPATH",
	    "DO_FALSE_CHARBOXPATH", "DO_TRUE_CHARBOXPATH", "INTERVENE",
	    "RETURN_WIDTH"};
	int i;
	gs_text_enum_t* pte;
	int code;
	...
	if(text->operation & TEXT_ADD_TO_ALL_WIDTHS)
		dprintf2(", delta_all=(%g,%g)", text->delta_all.x,
		         text->delta_all.y);
	if(text->operation & TEXT_ADD_TO_SPACE_WIDTH) {
		dprintf3(", space=0x%lx, delta_space=(%g,%g)",
		         (text->operation & TEXT_FROM_GLYPHS
		              ? (uint32_t)text->space.s_glyph
		              : (uint32_t)text->space.s_char),
	 ...
	 */
	if(text->operation & (TEXT_DO_FALSE_CHARPATH | TEXT_DO_TRUE_CHARPATH |
	                      TEXT_DO_FALSE_CHARBOXPATH |
	                      TEXT_DO_TRUE_CHARBOXPATH | TEXT_INTERVENE))
		goto dflt;
	...
	if((text->operation & (TEXT_DO_CHARWIDTH | TEXT_RETURN_WIDTH)) &&
	   !(text->operation & TEXT_REPLACE_WIDTHS)) {
		/*
		 * Get the widths from the font.  This code is mostly copied
		 * from
		...
		for(i = 0; i < text->size; ++i) {
			...
			gs_glyph glyph =
			    ((gs_font_base*)font)
			        ->procs.encode_char(font, ch, GLYPH_SPACE_NAME);
			gs_glyph_info_t info;

			if(glyph != gs_no_glyph &&
			   (code = font->procs.glyph_info(
			        font, glyph, NULL, GLYPH_INFO_WIDTH0 << wmode,
			        &info)) >= 0) {
		...
		code = gx_path_add_point(path, origin.x + float2fixed(dpt.x),
...
#define TRACE_DEVICE_BODY(dname, ncomp, depth, map_rgb_color, map_color_rgb,   \
                          map_cmyk_color, map_rgb_alpha_color)                 \
	std_device_dci_body(                                                   \
	    gx_device, 0, dname, DEFAULT_WIDTH_10THS* X_DPI / 10,              \
	    DEFAULT_HEIGHT_10THS * Y_DPI / 10, X_DPI, Y_DPI, ncomp, depth,     \
	    (1 << (depth / ncomp)) - 1,                                        \
	    (ncomp > 1 ? (1 << (depth / ncomp)) - 1 : 0),                      \
	    1 << (depth / ncomp), (ncomp > 1 ? 1 << (depth / ncomp) : 1)),     \
	{                                                                      \
		NULL,                           /* open_device */              \
		    ...
		    trace_fill_rectangle, NULL, /* tile_rectangle */           \
		    trace_copy_mono, trace_copy_color, NULL, /* draw_line */   \
		    NULL,                                    /* get_bits */    \
		    NULL,                                    /* get_params */  \
		    NULL,                                    /* put_params */  \
		    ...
		    trace_copy_alpha, NULL,               /* get_band */       \
		    NULL,                                 /* copy_rop */       \
		    trace_fill_path, trace_stroke_path, trace_fill_mask,       \
		    trace_fill_trapezoid, trace_fill_parallelogram,            \
		    trace_fill_triangle, trace_draw_thin_line,                 \
		    NULL, /* begin_image */                                    \
		    NULL, /* image_data */                                     \
		    NULL, /* end_image */                                      \
		    trace_strip_tile_rectangle, trace_strip_copy_rop,          \
		    NULL,                          /* get_clipping_box */      \
		    trace_begin_typed_image, NULL, /* get_bits_rectangle */    \
		    NULL,                          /* map_color_rgb_alpha */   \
		    NULL,                          /* create_compositor */     \
		    NULL,                          /* get_hardware_params */   \
		    trace_text_begin, NULL         /* finish_copydevice */     \
	}

const gx_device gs_tr_mono_device = {
    TRACE_DEVICE_BODY("tr_mono", 1, 1, gx_default_b_w_map_rgb_color,
                      gx_default_b_w_map_color_rgb, NULL, NULL)};

const gx_device gs_tr_rgb_device = {
    TRACE_DEVICE_BODY("tr_rgb", 3, 24, gx_default_rgb_map_rgb_color,
                      gx_default_rgb_map_color_rgb, NULL, NULL)};

const gx_device gs_tr_cmyk_device = {
    TRACE_DEVICE_BODY("tr_cmyk", 4, 4, NULL, cmyk_1bit_map_color_rgb,
