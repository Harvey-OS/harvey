...
trace_drawing_color(const char *prefix, const gx_drawing_color *pdcolor)
{
	...
	if(pdcolor->type == gx_dc_type_none)
		...
		dprintf5("binary(0x%lx, 0x%lx, %d/%d, index=%d)",
			 ...
			 (ci < 0 ? pdcolor->colors.binary.b_ht->order.num_bits : pdcolor->colors.binary.b_ht->components[ci].corder.num_bits),
			 ci);
...
private
const gx_image_enum_procs_t trace_image_enum_procs = {
		 ...
		 gs_text_enum_t **ppenum)
{
	...
	if(text->operation & TEXT_FROM_SINGLE_CHAR)
		...
		for(i = 0; i < text->size; ++i) {
			if(text->operation & TEXT_FROM_STRING)
				...
				dprintf1("0x%lx ",
					 (text->operation & TEXT_FROM_GLYPHS ? (uint32_t)text->data.glyphs[i] : (uint32_t)text->data.chars[i]));
		}
	dprintf1(")\n           size=%u", text->size);
	...
	if(text->operation & TEXT_ADD_TO_SPACE_WIDTH) {
		dprintf3(", space=0x%lx, delta_space=(%g,%g)",
			 (text->operation & TEXT_FROM_GLYPHS ? (uint32_t)text->space.s_glyph : (uint32_t)text->space.s_char),
	...
	if(text->operation & TEXT_REPLACE_WIDTHS) {
		...
		for(i = 0; i < text->widths_size; ++i) {
			...
			if(text->y_widths)
				dprintf1("%g)",
					 (text->y_widths == text->x_widths ? text->y_widths[++i] : text->y_widths[i]));
			else
				dputs(")");
		}
	}
	 ...
	 */
		...
		for(i = 0; i < text->size; ++i) {
			...
			gs_glyph glyph =
			    ((gs_font_base *)font)->procs.encode_char(font, ch, GLYPH_SPACE_NAME);
...
#define TRACE_DEVICE_BODY(dname, ncomp, depth, map_rgb_color, map_color_rgb, map_cmyk_color, map_rgb_alpha_color)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                \
	std_device_dci_body(gx_device, 0, dname, DEFAULT_WIDTH_10THS *X_DPI / 10, DEFAULT_HEIGHT_10THS * Y_DPI / 10, X_DPI, Y_DPI, ncomp, depth, (1 << (depth / ncomp)) - 1, (ncomp > 1 ? (1 << (depth / ncomp)) - 1 : 0), 1 << (depth / ncomp), (ncomp > 1 ? 1 << (depth / ncomp) : 1)),                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        \
	{                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        \
		NULL, /* open_device */ NULL, /* get_initial_matrix */ NULL, /* sync_output */ NULL, /* output_page */ NULL, /* close_device */ map_rgb_color, /* differs */ map_color_rgb, /* differs */ trace_fill_rectangle, NULL, /* tile_rectangle */ trace_copy_mono, trace_copy_color, NULL, /* draw_line */ NULL, /* get_bits */ NULL, /* get_params */ NULL, /* put_params */ map_cmyk_color, /* differs */ NULL, /* get_xfont_procs */ NULL, /* get_xfont_device */ map_rgb_alpha_color, /* differs */ gx_page_device_get_page_device, NULL, /* get_alpha_bits */ trace_copy_alpha, NULL, /* get_band */ NULL, /* copy_rop */ trace_fill_path, trace_stroke_path, trace_fill_mask, trace_fill_trapezoid, trace_fill_parallelogram, trace_fill_triangle, trace_draw_thin_line, NULL, /* begin_image */ NULL, /* image_data */ NULL, /* end_image */ trace_strip_tile_rectangle, trace_strip_copy_rop, NULL, /* get_clipping_box */ trace_begin_typed_image, NULL, /* get_bits_rectangle */ NULL, /* map_color_rgb_alpha */ NULL, /* create_compositor */ NULL, /* get_hardware_params */ trace_text_begin, NULL /* finish_copydevice */ \
	}
