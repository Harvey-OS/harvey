...
lzw_put_code(register stream_LZW_state* ss, byte* q, uint code)
{
	...
	byte cb =
	    (ss->bits << ss->bits_left) + (code >> (size - ss->bits_left));
...
{
	ss->bits_left = 8;
	ss->table.encode = gs_alloc_struct(st->memory, lzw_encode_table,
	                                   &st_lzwe_table, "LZWEncode init");
...
#define set_limit_code()                                                       \
	...
	if(limit_code > encode_max)                                            \
	limit_code = encode_max
	set_limit_code();
	if(ss->first) { /* Emit the initial reset code. */
		if(wlimit - q < 2)
			return 1;
	...
	while(p < rlimit) {
		...
		for(tp = &table->hashed[encode_hash(code, c)];;) {
			...
			if(ep->prefix == code && ep->datum == c) {
				...
				if(++tp == table_end)
					tp =
					    &table->hashed[0]; /* wrap around */
			} else { /* end of recognized sequence */
				if(wlimit - q <= 4) {
					status = 1;
					goto out;
				}
				q = lzw_put_code(ss, q, code);
				if(ss->next_code == limit_code) { /* Reached
				                                     power of 2
				                                     or limit.
				                                     */
					/* Determine which. */
	...
	if(last && status == 0) {
		if(wlimit - q < 4)
			...
			if(code != code_eod) {
				q = lzw_put_code(ss, q,
				                 code); /* put out final code */
			}
			q = lzw_put_code(ss, q, code_eod);
			if(ss->bits_left < 8)
				*++q = ss->bits
				       << ss->bits_left; /* final byte */
		}
	}
out:
	ss->prev_code = code;
...
/* Stream template */
const stream_template s_LZWE_template = {&st_LZW_state, s_LZWE_init,
                                         s_LZWE_process, 1, 4, s_LZW_release,
                                         s_LZW_set_defaults};
