...
private
dev_proc_get_params(xcf_get_params);
private
dev_proc_put_params(xcf_put_params);
private
dev_proc_print_page(xcf_print_page);
private
dev_proc_map_color_rgb(xcf_map_color_rgb);
private
dev_proc_get_color_mapping_procs(get_spotrgb_color_mapping_procs);
#if 0
private dev_proc_get_color_mapping_procs(get_spotcmyk_color_mapping_procs);
#endif
private
dev_proc_get_color_mapping_procs(get_xcf_color_mapping_procs);
private
dev_proc_get_color_comp_index(xcf_get_color_comp_index);
private
dev_proc_encode_color(xcf_encode_color);
private
dev_proc_decode_color(xcf_decode_color);
...
#define device_procs(get_color_mapping_procs)                                  \
	{                                                                      \
		gdev_prn_open, gx_default_get_initial_matrix,                  \
		    NULL,                 /* sync_output */                    \
...
const gx_device_procs spot_rgb_procs =
    device_procs(get_spotrgb_color_mapping_procs);

const xcf_device gs_xcf_device = {
    prn_device_body_extended(
        xcf_device, spot_rgb_procs, "xcf", DEFAULT_WIDTH_10THS,
        DEFAULT_HEIGHT_10THS, X_DPI, Y_DPI, /* X and Y hardware resolution */
        0, 0, 0, 0,                         /* margins */
...
const gx_device_procs spot_cmyk_procs =
    device_procs(get_xcf_color_mapping_procs);

const xcf_device gs_xcfcmyk_device = {
    prn_device_body_extended(
        xcf_device, spot_cmyk_procs, "xcfcmyk", DEFAULT_WIDTH_10THS,
        DEFAULT_HEIGHT_10THS, X_DPI, Y_DPI, /* X and Y hardware resolution */
        0, 0, 0, 0,                         /* margins */
...
{
	/* TO_DO_DEVICEN  This routine needs to include the effects of the
	 * SeparationOrder array */
...
private
void
rgb_cs_to_spotrgb_cm(gx_device* dev, const gs_imager_state* pis, frac r, frac g,
                     frac b, frac out[])
{
	/* TO_DO_DEVICEN  This routine needs to include the effects of the
	 * SeparationOrder array */
...
private
void
cmyk_cs_to_spotrgb_cm(gx_device* dev, frac c, frac m, frac y, frac k,
                      frac out[])
{
	/* TO_DO_DEVICEN  This routine needs to include the effects of the
	 * SeparationOrder array */
...
private
const gx_cm_color_map_procs spotRGB_procs = {
...
private
const gx_cm_color_map_procs spotN_procs = {
...
private
int
xcf_param_read_fn(gs_param_list* plist, const char* name, gs_param_string* pstr,
                  int max_len)
{
...
xcf_put_params(gx_device* pdev, gs_param_list* plist)
{
	...
	if(pdev->color_info.polarity == GX_CINFO_POLARITY_SUBTRACTIVE) {
		/*
		 * Process the separation color names.  Remove any names that
		 * already
		...
		if(scna.data != 0) {
			...
			for(i = num_spot = 0; i < num_names; i++) {
				if(!check_process_color_names(pcomp_names,
				                              &scna.data[i]))
					pdevn->separation_names
					    .names[num_spot++] = &scna.data[i];
			}
		...
		pdevn->color_info.depth = bpc_to_depth(
		    pdevn->color_info.num_components, pdevn->bitspercomponent);
                         ...
                         int component_type)
{
	/* TO_DO_DEVICEN  This routine needs to include the effects of the
	 * SeparationOrder array */
	const fixed_colorant_names_list* list =
	    ((const xcf_device*)dev)->std_colorant_names;
	...
	{
		const gs_separation_names* separations =
		    &((const xcf_device*)dev)->separation_names;
...
private
void
xcf_shuffle_to_tile(xcf_write_ctx* xc, byte** tile_data, const byte* row, int y)
{
	...
	for(tile_i = 0; tile_i < xc->n_tiles_x; tile_i++) {
		int x;
		int tile_width =
		    min(TILE_WIDTH, xc->width - tile_i * TILE_WIDTH);
		int tile_height =
		    min(TILE_HEIGHT, xc->height - tile_j * TILE_HEIGHT);
		byte* base_ptr =
		    tile_data[tile_i] + yrem * tile_width * base_bytes_pp;
		int extra_stride = tile_width * tile_height;
		byte* extra_ptr = tile_data[tile_i] +
		                  extra_stride * base_bytes_pp +
		                  yrem * tile_width;
		...
		for(x = 0; x < tile_width; x++) {
			int plane_idx;
			for(plane_idx = 0; plane_idx < base_bytes_pp;
			    plane_idx++)
				base_ptr[base_idx++] = row[row_idx++];
			for(plane_idx = 0; plane_idx < n_extra_channels;
			    plane_idx++)
				extra_ptr[plane_idx * extra_stride + x] =
				    255 ^ row[row_idx++];
		}
	}
}
...
private
void
xcf_icc_to_tile(xcf_write_ctx* xc, byte** tile_data, const byte* row, int y,
                icmLuBase* luo)
{
	int tile_j = y / TILE_HEIGHT;
	int yrem = y % TILE_HEIGHT;
...
xcf_write_image_data(xcf_write_ctx* xc, gx_device_printer* pdev)
{
	...
	tile_data =
	    (byte**)gs_alloc_bytes(pdev->memory, xc->n_tiles_x * sizeof(byte*),
	...
	for(tile_j = 0; tile_j < xc->n_tiles_y; tile_j++) {
		...
		for(tile_i = 0; tile_i < xc->n_tiles_x; tile_i++) {
			...
			for(chan_idx = 0; chan_idx < n_extra_channels;
			    chan_idx++) {
				xcf_write(xc, tile_data[tile_i] + base_size +
				                  tile_size * chan_idx,
				          tile_size);
...
xcf_write_footer(xcf_write_ctx* xc, xcf_device* pdev)
{
	...
	for(chan_idx = 0; chan_idx < xc->n_extra_channels; chan_idx++) {
		...
		for(tile_idx = 0; tile_idx < xc->n_tiles; tile_idx++) {
			...
			xcf_write_32(
			    xc,
			    offset + (base_bytes_pp + chan_idx) * tile_size);
