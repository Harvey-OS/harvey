...
private
dev_proc_open_device(lvga256_open);
private
dev_proc_close_device(lvga256_close);
private
dev_proc_map_rgb_color(lvga256_map_rgb_color);
private
dev_proc_map_color_rgb(lvga256_map_color_rgb);
private
dev_proc_fill_rectangle(lvga256_fill_rectangle);
private
dev_proc_tile_rectangle(lvga256_tile_rectangle);
private
dev_proc_copy_mono(lvga256_copy_mono);
private
dev_proc_copy_color(lvga256_copy_color);
private
dev_proc_draw_line(lvga256_draw_line);

private
gx_device_procs lvga256_procs = {lvga256_open, NULL, /* get_initial_matrix */
                                 NULL,               /* sync_output */
                                 NULL,               /* output_page */
                                 lvga256_close, lvga256_map_rgb_color,
                                 lvga256_map_color_rgb, lvga256_fill_rectangle,
                                 lvga256_tile_rectangle, lvga256_copy_mono,
                                 lvga256_copy_color, lvga256_draw_line};

gx_device_lvga256 far_data gs_lvga256_device = {
    std_device_color_body(gx_device_lvga256, &lvga256_procs, "lvga256", 0,
                          0,    /* width and height are set in lvga256_open */
...
#define cv_bits(v, n) (v >> (gx_color_value_bits - n))
	...
	/* Put in the dynamic table if space available. */
	{
		...
		pdc->rgb = rgb;
		pdc->index = i;
		gl_setpalettecolor(i, cv_bits(r, 6), cv_bits(g, 6),
		                   cv_bits(b, 6));
...
{
	/*   gl_getpalettecolor (color,(int *)&prgb[0],(int *)&prgb[1],(int
	 * *)&prgb[2]); */
...
int
lvga256_copy_mono(gx_device* dev, const byte* base, int sourcex, int raster,
...
int
lvga256_copy_color(gx_device* dev, const byte* base, int sourcex, int raster,
                   gx_bitmap_id id, int x, int y, int w, int h)
{
	fit_copy(dev, base, sourcex, raster, id, x, y, w, h);
...
int
lvga256_tile_rectangle(gx_device* dev, const gx_tile_bitmap* tile, int x, int y,
