...
struct gs_indexed_map_s {
	...
	union {
		...
		int (*tint_transform)(const gs_separation_params*, floatp,
		                      float*);
	} proc;
	void* proc_data;
...
#define private_st_indexed_map() /* in gscolor2.c */                           \
	gs_private_st_ptrs2(st_indexed_map, gs_indexed_map, "gs_indexed_map",  \
	                    indexed_map_enum_ptrs, indexed_map_reloc_ptrs,     \
	                    proc_data, values)
...
int alloc_indexed_map(gs_indexed_map** ppmap, int num_values, gs_memory_t* mem,
                      client_name_t cname);
