 ...
 */
#define string_space_quanta(spacebytes) ((spacebytes) / string_space_quantum)
/*
 * Compute the size of string marks for a given number of quanta.
 */
#define string_quanta_mark_size(nquanta) ((nquanta) * (string_data_quantum / 8))
/*
...
#define public_st_chunk() /* in ialloc.c */                                    \
	gs_public_st_ptrs2(st_chunk, chunk_t, "chunk_t", chunk_enum_ptrs,      \
	                   chunk_reloc_ptrs, cprev, cnext)
...
#define SCAN_CHUNK_OBJECTS(cp)                                                 \
	{                                                                      \
		...
		for(; pre < end;                                               \
		    pre =                                                      \
		        (obj_header_t*)((char*)pre + obj_size_round(size))) {
#define DO_ALL                                                                 \
...
#define dprintf_chunk_format "%s 0x%lx (0x%lx..0x%lx, 0x%lx..0x%lx..0x%lx)\n"
#define dprintf_chunk(msg, cp)                                                 \
	dprintf7(dprintf_chunk_format, msg, (ulong)(cp), (ulong)(cp)->cbase,   \
	         (ulong)(cp)->cbot, (ulong)(cp)->ctop, (ulong)(cp)->climit,    \
	         (ulong)(cp)->cend)
#define if_debug_chunk(c, msg, cp)                                             \
	if_debug7(c, dprintf_chunk_format, msg, (ulong)(cp),                   \
	          (ulong)(cp)->cbase, (ulong)(cp)->cbot, (ulong)(cp)->ctop,    \
	          (ulong)(cp)->climit, (ulong)(cp)->cend)
...
#define public_st_ref_memory() /* in gsalloc.c */                              \
	gs_public_st_composite(st_ref_memory, gs_ref_memory_t,                 \
	                       "gs_ref_memory", ref_memory_enum_ptrs,          \
	                       ref_memory_reloc_ptrs)
...
/* contents. */
void debug_print_object(const gs_memory_t* mem, const void* obj,
                        const dump_control_t* control);

/* Print the contents of a chunk with the given options. */
/* Relevant options: all. */
void debug_dump_chunk(const gs_memory_t* mem, const chunk_t* cp,
                      const dump_control_t* control);
void debug_print_chunk(const gs_memory_t* mem,
                       const chunk_t* cp); /* default options */
