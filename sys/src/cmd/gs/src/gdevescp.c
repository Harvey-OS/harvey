...
private
dev_proc_print_page(escp2_print_page);

/* Stylus 800 device */
const gx_device_printer far_data gs_st800_device = prn_device(
    prn_std_procs, "st800", DEFAULT_WIDTH_10THS, DEFAULT_HEIGHT_10THS, X_DPI,
    Y_DPI, STYLUS_L_MARGIN, STYLUS_B_MARGIN, STYLUS_R_MARGIN, STYLUS_T_MARGIN,
    1, escp2_print_page);

/* AP3250 device */
const gx_device_printer far_data gs_ap3250_device = prn_device(
    prn_std_procs, "ap3250", DEFAULT_WIDTH_10THS, DEFAULT_HEIGHT_10THS, X_DPI,
    Y_DPI, AP3250_L_MARGIN, AP3250_B_MARGIN, AP3250_R_MARGIN, AP3250_T_MARGIN,
    1, escp2_print_page);
...
escp2_print_page(gx_device_printer* pdev, FILE* prn_stream)
{
	...
	*/
	...
	fwrite("\033(U\001\0\n\033(C\002\0t\020\033(c\004\0\0\0t\020", 1, 22,
	       prn_stream);
#endif
	...
	*/
	...
	width = ((pdev->width -
	          (int)(dev_r_margin(pdev) * pdev->x_pixels_per_inch)) >>
	         3) -
	        left;
	...
	for(lnum = top, skip = 0; lnum < bottom;) {
		...
		*/
		...
		for(outp = out, i = 0; i < band_size; i++) {
			...
			** Since we need more than 2 repeats to make the
			*compression
			** worth it, we can compare pairs, since it doesn't
			*matter if we
			**
			*/
			...
			*/
			...
			if(inp < in_end) {
				...
				/*
				** If we've had a long run of varying data
				*followed by a
				** sequence of repeated data and then hit the
				*end of line,
