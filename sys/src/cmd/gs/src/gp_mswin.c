...
private
int is_printer(const char* name);
...
/* Forward references */
private
int gp_printfile(const char*, const char*);
...
SpoolDlgProc(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_INITDIALOG:
		entry = (LPSTR)lParam;
		while(*entry) {
			SendDlgItemMessage(hDlg, SPOOL_PORT, LB_ADDSTRING, 0,
			                   (LPARAM)entry);
			entry += lstrlen(entry) + 1;
		}
		SendDlgItemMessage(hDlg, SPOOL_PORT, LB_SETCURSEL, 0,
		                   (LPARAM)0);
		return TRUE;
	case WM_COMMAND:
		...
		case SPOOL_PORT:
			...
			return FALSE;
		case IDOK:
			EndDialog(hDlg, 1 + (int)SendDlgItemMessage(
			                        hDlg, SPOOL_PORT, LB_GETCURSEL,
			                        0, 0L));
			return TRUE;
		case IDCANCEL:
			EndDialog(hDlg, 0);
...
private
int gp_printfile_win32(const char* filename, char* port);

/* Win32s: Pass to Win16 spooler via gs16spl.exe */
private
int gp_printfile_gs16spl(const char* filename, const char* port);
...
gp_printfile(const char* filename, const char* pmport)
{
	...
	if(!is_win32s) {
		if(strlen(pmport) == 0) { /* get default printer */
			...
			GetProfileString("windows", "device", "", buf,
			                 sizeof(buf));
			...
			if(strlen(pmport) >= 8)
				return gp_printfile_win32(filename,
				                          (char*)pmport + 8);
			else
				return gp_printfile_win32(filename,
				                          (char*)NULL);
		...
		if(is_spool(pmport)) {
			if(strlen(pmport) >= 8) {
				...
				GetProfileString("Devices", pmport + 8, "",
				                 driverbuf, sizeof(driverbuf));
				strtok(driverbuf, ",");
				output = strtok(NULL, ",");
				return gp_printfile_gs16spl(filename, output);
			} else
				return gp_printfile_gs16spl(filename,
				                            (char*)NULL);
		} else
			return gp_printfile_gs16spl(filename, pmport);
	}
...
get_queues(void)
{
	...
	EnumPrinters(PRINTER_ENUM_CONNECTIONS | PRINTER_ENUM_LOCAL, NULL, 1,
	             NULL, 0, &needed, &count);
	if(needed == 0) {
		/* no printers */
		enumbuffer = malloc(4);
	...
	if(enumbuffer == (char*)NULL)
		return NULL;
	if(!EnumPrinters(PRINTER_ENUM_CONNECTIONS | PRINTER_ENUM_LOCAL, NULL, 1,
	                 (LPBYTE)enumbuffer, needed, &needed, &count)) {
		char buf[256];

		free(enumbuffer);
		sprintf(buf, "EnumPrinters() failed, error code = %d",
		        GetLastError());
	...
	p = buffer;
	for(i = 0; i < count; i++) {
		if(strlen(prinfo[i].pName) + 1 <
		   (PORT_BUF_SIZE - (p - buffer))) {
...
get_queuename(char* portname, const char* queue)
{
	...
	if((queue == (char*)NULL) || (strlen(queue) == 0)) {
		/* select a queue */
		iport = DialogBoxParam(phInstance, "QueueDlgBox", (HWND)NULL,
...
get_portname(char* portname, const char* port)
{
	...
	if((port == (char*)NULL) || (strlen(port) == 0)) {
		if(buffer == (char*)NULL)
			return FALSE;
		/* select a port */
		iport = DialogBoxParam(phInstance, "SpoolDlgBox", (HWND)NULL,
		                       SpoolDlgProc, (LPARAM)buffer);
		if(!iport) {
			free(buffer);
			return FALSE;
...
gp_printfile_win32(const char* filename, char* port)
{
	...
	di.pOutputFile = NULL;
	di.pDatatype =
	    "RAW"; /* for available types see EnumPrintProcessorDatatypes */
	if(!StartDocPrinter(printer, 1, (LPBYTE)&di)) {
		char buf[256];

		sprintf(buf, "StartDocPrinter() failed, error code = %d",
		        GetLastError());
		MessageBox((HWND)NULL, buf, szAppName, MB_OK | MB_ICONSTOP);
	...
	if(!EndDocPrinter(printer)) {
		...
		sprintf(buf, "EndDocPrinter() failed, error code = %d",
		        GetLastError());
		MessageBox((HWND)NULL, buf, szAppName, MB_OK | MB_ICONSTOP);
		AbortPrinter(printer);
		return FALSE;
	...
	if(!ClosePrinter(printer)) {
		...
		sprintf(buf, "ClosePrinter() failed, error code = %d",
		        GetLastError());
		MessageBox((HWND)NULL, buf, szAppName, MB_OK | MB_ICONSTOP);
		return FALSE;
	}
...
gp_printfile_gs16spl(const char* filename, const char* port)
{
	...
	if((p = strrchr(command, '\\')) != (char*)NULL)
		...
		p = command;
	*p = '\0';
	sprintf(command + strlen(command), "gs16spl.exe %s %s", portname,
	        filename);
 ...
 */
FILE*
mswin_popen(const char* cmd, const char* mode)
{
	...
	if(handle == 0) {
		if(!DuplicateHandle(
		       hProcess, hPipeTemp, hProcess, &hChildStdinWr, 0,
		       FALSE /* not inherited */, DUPLICATE_SAME_ACCESS))
			handle = -1;
...
gp_open_scratch_file(const char* prefix, char* fname, const char* mode)
{
	...
	if(l <= sizeof(sTempDir)) {
		...
		if(n == 0) {
			...
			for(i = l - 1; i > 0; i--) {
				uint slen = gs_file_name_check_separator(
				    sTempDir + i, l, sTempDir + l);
			...
			if(i > 0)
				n = GetTempFileName(sTempDir, sTempDir + i, 0,
				                    sTempFileName);
		}
		if(n != 0) {
			hfile = CreateFile(
			 ...
			 * gdev_prn_render_pages requires multiple temporary
			 * files
			 * to exist simultaneousely, so that keeping all them
			 * opened
