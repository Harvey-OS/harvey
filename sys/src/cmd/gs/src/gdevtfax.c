...
private
dev_proc_get_params(tfax_get_params);
private
dev_proc_put_params(tfax_put_params);
private
dev_proc_print_page(tiffcrle_print_page);
private
dev_proc_print_page(tiffg3_print_page);
private
dev_proc_print_page(tiffg32d_print_page);
private
dev_proc_print_page(tiffg4_print_page);
...
private
const gx_device_procs gdev_tfax_std_procs =
...
#define TFAX_DEVICE(dname, print_page)                                         \
	{                                                                      \
		FAX_DEVICE_BODY(gx_device_tfax, gdev_tfax_std_procs, dname,    \
		                print_page),                                   \
...
tfax_get_params(gx_device* dev, gs_param_list* plist)
{
	...
	if((code = param_write_long(plist, "MaxStripSize",
	                            &tfdev->MaxStripSize)) < 0)
		ecode = code;
...
tfax_put_params(gx_device* dev, gs_param_list* plist)
{
	...
	switch(code = param_read_long(plist, (param_name = "MaxStripSize"),
	                              &mss)) {
	case 0:
		/*
	...
	switch(code = param_read_int(plist, (param_name = "FillOrder"),
	                             &fill_order)) {
	case 0:
...
gdev_stream_print_page_width(gx_device_printer* pdev, FILE* prn_stream,
                             ...
                             int width)
{
	return gdev_stream_print_page_strips(pdev, prn_stream, temp, ss, width,
	                                     pdev->height);
}
...
private
dev_proc_print_page(tifflzw_print_page);
private
dev_proc_print_page(tiffpack_print_page);

const gx_device_tfax gs_tifflzw_device = {prn_device_std_body(
    gx_device_tfax, prn_std_procs, "tifflzw", DEFAULT_WIDTH_10THS,
    DEFAULT_HEIGHT_10THS, X_DPI, Y_DPI, 0, 0, 0, 0, /* margins */
    1, tifflzw_print_page)};

const gx_device_tfax gs_tiffpack_device = {prn_device_std_body(
    gx_device_tfax, prn_std_procs, "tiffpack", DEFAULT_WIDTH_10THS,
    DEFAULT_HEIGHT_10THS, X_DPI, Y_DPI, 0, 0, 0, 0, /* margins */
...
private
const tiff_mono_directory dir_mono_template = {
...
/* Forward references */
private
int tfax_begin_page(gx_device_tfax*, FILE*, const tiff_mono_directory*, int);
                 ...
                 stream_CFE_state* pstate, tiff_mono_directory* pdir)
{
	...
	code = gdev_fax_print_page_stripped(dev, prn_stream, pstate,
	                                    tfdev->tiff.rows);
...
tifflzw_print_page(gx_device_printer* dev, FILE* prn_stream)
{
	...
	state.EarlyChange =
	    1; /* PLRM is sort of confusing, but this is correct */
                ...
                const tiff_mono_directory* pdir, int width)
{
	...
	tfdev->width = width;
	code = gdev_tiff_begin_page((gx_device_printer*)tfdev, &tfdev->tiff, fp,
