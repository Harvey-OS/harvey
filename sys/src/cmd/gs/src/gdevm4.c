...
/* Procedures */
declare_mem_procs(mem_mapped4_copy_mono, mem_mapped4_copy_color,
                  mem_mapped4_fill_rectangle);

/* The device descriptor. */
const gx_device_memory mem_mapped4_device = mem_device(
    "image4", 4, 0, mem_mapped_map_rgb_color, mem_mapped_map_color_rgb,
...
static const mono_fill_chunk tile_patterns[16] = {
    fpat(0x00), fpat(0x11), fpat(0x22), fpat(0x33), fpat(0x44), fpat(0x55),
    fpat(0x66), fpat(0x77), fpat(0x88), fpat(0x99), fpat(0xaa), fpat(0xbb),
    fpat(0xcc), fpat(0xdd), fpat(0xee), fpat(0xff)};

/* Fill a rectangle with a color. */
private
int
mem_mapped4_fill_rectangle(gx_device* dev, int x, int y, int w, int h,
...
private
int
mem_mapped4_copy_mono(gx_device* dev, const byte* base, int sourcex,
                      ...
                      gx_color_index zero, gx_color_index one)
{
	...
	if(one == gx_no_color_index) {
		...
		do {
			...
			/*
			 * If the first source bit corresponds to an odd X in
			 * the
			...
			/*
			 * Now we know the next destination X is even.  We want
			 * to
			 * process 2 source bits at a time from now on, so set
			 * things up
			 * properly depending on whether the next source X (bit)
			 * is even
			 * or odd.  In both even and odd cases, the active
			 * source bits
			...
			sbit += shift - 1;
			/*
			 * Now bit # sbit+1 is the most significant unprocessed
			 *bit
			 * in sbyte.  -1 <= sbit <= 7; sbit is odd.
			 * Note that if sbit = -1, all of sbyte has been
			 *processed.
			 *
			 * Continue processing pairs of bits in the first source
			 *byte.
			 */
			...
			/*
			 * Now sbit = -1 iff we have processed the entire first
			 *source
			 * byte.
			 *
			...
			if(!count)
				continue;
			/*
			 * Process pairs of bits in the final source byte.  Note
			 * that
			...
			/*
			 * If the final source bit corresponds to an even X
			 * value,
			 * process it now.
			 */
			if(count) {
...
private
int
mem_mapped4_copy_color(gx_device* dev, const byte* base, int sourcex,
                       int sraster, gx_bitmap_id id, int x, int y, int w, int h)
{
	...
	code = (*dev_proc(&mem_mono_device, copy_mono))(
	    dev, base, sourcex << 2, sraster, id, x << 2, y, w << 2, h,
...
/* Procedures */
declare_mem_procs(mem4_word_copy_mono, mem4_word_copy_color,
                  mem4_word_fill_rectangle);

/* Here is the device descriptor. */
const gx_device_memory mem_mapped4_word_device =
    mem_full_device("image4w", 4, 0, mem_open, mem_mapped_map_rgb_color,
                    mem_mapped_map_color_rgb, mem4_word_copy_mono,
                    mem4_word_copy_color, mem4_word_fill_rectangle,
                         ...
                         gx_color_index color)
{
	...
	bits_fill_rectangle(base, x << 2, raster, tile_patterns[color], w << 2,
	                    h);
	mem_swap_byte_rect(base, raster, x << 2, w << 2, h, true);
	return 0;
}
...
private
int
mem4_word_copy_mono(gx_device* dev, const byte* base, int sourcex, int sraster,
                    ...
                    gx_color_index zero, gx_color_index one)
{
	...
	mem_mapped4_copy_mono(dev, base, sourcex, sraster, id, x, y, w, h, zero,
	                      one);
...
private
int
mem4_word_copy_color(gx_device* dev, const byte* base, int sourcex, int sraster,
                     gx_bitmap_id id, int x, int y, int w, int h)
{
	int code;
