...
{
	if_debug7(
	    'A', "[a%d%s]%s %s(%u) %s0x%lx\n", alloc_trace_space(imem), chars,
	    client_name_string(cname),
	    (ptr == 0 || stype == 0 ? "" : struct_type_name_string(stype)),
	    size, (chars[1] == '+' ? "= " : ""), (uint32_t)ptr);
}
private
bool
...
ENUM_PTRS_END
private
RELOC_PTRS_WITH(ref_memory_reloc_ptrs, gs_ref_memory_t* mptr)
{
...
/* Forward references */
private
void remove_range_from_freelist(gs_ref_memory_t* mem, void* bottom, void* top);
private
obj_header_t* large_freelist_alloc(gs_ref_memory_t* mem, uint size);
private
obj_header_t* scavenge_low_free(gs_ref_memory_t* mem, unsigned request_size);
private
ulong compute_free_objects(gs_ref_memory_t*);
private
obj_header_t* alloc_obj(gs_ref_memory_t*, ulong, gs_memory_type_ptr_t,
                        alloc_flags_t, client_name_t);
private
void consolidate_chunk_free(chunk_t* cp, gs_ref_memory_t* mem);
private
void trim_obj(gs_ref_memory_t* mem, obj_header_t* obj, uint size, chunk_t* cp);
private
chunk_t* alloc_acquire_chunk(gs_ref_memory_t*, ulong, bool, client_name_t);
private
chunk_t* alloc_add_chunk(gs_ref_memory_t*, ulong, client_name_t);
...
private
gs_memory_proc_alloc_bytes(i_alloc_bytes_immovable);
private
gs_memory_proc_resize_object(i_resize_object);
private
gs_memory_proc_free_object(i_free_object);
private
gs_memory_proc_stable(i_stable);
private
gs_memory_proc_status(i_status);
private
gs_memory_proc_free_all(i_free_all);
private
gs_memory_proc_consolidate_free(i_consolidate_free);

/* Object memory procedures */
private
gs_memory_proc_alloc_bytes(i_alloc_bytes);
private
gs_memory_proc_alloc_struct(i_alloc_struct);
private
gs_memory_proc_alloc_struct(i_alloc_struct_immovable);
private
gs_memory_proc_alloc_byte_array(i_alloc_byte_array);
private
gs_memory_proc_alloc_byte_array(i_alloc_byte_array_immovable);
private
gs_memory_proc_alloc_struct_array(i_alloc_struct_array);
private
gs_memory_proc_alloc_struct_array(i_alloc_struct_array_immovable);
private
gs_memory_proc_object_size(i_object_size);
private
gs_memory_proc_object_type(i_object_type);
private
gs_memory_proc_alloc_string(i_alloc_string);
private
gs_memory_proc_alloc_string(i_alloc_string_immovable);
private
gs_memory_proc_resize_string(i_resize_string);
private
gs_memory_proc_free_string(i_free_string);
private
gs_memory_proc_register_root(i_register_root);
private
gs_memory_proc_unregister_root(i_unregister_root);
private
gs_memory_proc_enable_free(i_enable_free);

/* We export the procedures for subclasses. */
const gs_memory_procs_t gs_ref_memory_procs = {
    /* Raw memory procedures */
    i_alloc_bytes_immovable, i_resize_object, i_free_object, i_stable, i_status,
    i_free_all, i_consolidate_free,
    /* Object memory procedures */
    i_alloc_bytes, i_alloc_struct, i_alloc_struct_immovable, i_alloc_byte_array,
    i_alloc_byte_array_immovable, i_alloc_struct_array,
    i_alloc_struct_array_immovable, i_object_size, i_object_type,
    i_alloc_string, i_alloc_string_immovable, i_resize_string, i_free_string,
    i_register_root, i_unregister_root, i_enable_free};
 ...
 */
private
void* ialloc_solo(gs_memory_t*, gs_memory_type_ptr_t, chunk_t**);
gs_ref_memory_t*
...
{ /*
   ...
   */
	chunk_t* cp = gs_raw_alloc_struct_immovable(parent, &st_chunk,
	                                            "ialloc_solo(chunk)");
	uint csize = ROUND_UP(sizeof(chunk_head_t) + sizeof(obj_header_t) +
	                          pstype->ssize,
	                      obj_align_mod);
 ...
 */
int
ialloc_add_chunk(gs_ref_memory_t* imem, uint32_t space, client_name_t cname)
{
	chunk_t* cp;
	...
	imem->procs.alloc_struct_array_immovable =
	    imem->procs.alloc_struct_array;
...
{ /*
   ...
   */
	...
	if(mem->gc_status.enabled) {
		uint32_t limit =
		    mem->gc_allocated + mem->gc_status.vm_threshold;
		...
		mem->limit =
		    min(max_allocated, mem->gc_allocated + FORCE_GC_LIMIT);
	if_debug7('0', "[0]space=%d, max_vm=%ld, prev.alloc=%ld, enabled=%d,\n\
      gc_alloc=%ld, threshold=%ld => limit=%ld\n",
	          mem->space, (int32_t)mem->gc_status.max_vm,
	          (int32_t)mem->previous_status.allocated,
	          mem->gc_status.enabled, (int32_t)mem->gc_allocated,
	          (int32_t)mem->gc_status.vm_threshold, (int32_t)mem->limit);
}
...
void
gs_memory_set_gc_status(gs_ref_memory_t* mem,
                        const gs_memory_gc_status_t* pstat)
{
	mem->gc_status = *pstat;
...
/* Set VM threshold. */
void
gs_memory_set_vm_threshold(gs_ref_memory_t* mem, int32_t val)
{
...
#define ELSEIF_LIFO_ALLOC(ptr, imem, size, pstype)                             \
	...
	else if((imem->cc.ctop -                                               \
	         (byte*)(ptr = (obj_header_t*)imem->cc.cbot)) >=               \
...
{
	gs_ref_memory_t* const imem = (gs_ref_memory_t*)mem;
	obj_header_t* obj = alloc_obj(imem, (uint32_t)num_elements * elt_size,
	                              &st_bytes, ALLOC_DIRECT, cname);

	if_debug6('A', "[a%d:+b.]%s -bytes-*(%lu=%u*%u) = 0x%lx\n",
	          alloc_trace_space(imem), client_name_string(cname),
	          (uint32_t)num_elements * elt_size, num_elements, elt_size,
	          (uint32_t)obj);
	return (byte*)obj;
}
private
byte*
i_alloc_byte_array_immovable(gs_memory_t* mem, uint num_elements, uint elt_size,
                             client_name_t cname)
{
	gs_ref_memory_t* const imem = (gs_ref_memory_t*)mem;
	obj_header_t* obj =
	    alloc_obj(imem, (uint32_t)num_elements * elt_size, &st_bytes,
	              ALLOC_IMMOVABLE | ALLOC_DIRECT, cname);
                     ...
                     gs_memory_type_ptr_t pstype, client_name_t cname)
{
	...
	if(pstype->enum_ptrs == basic_enum_ptrs) {
		dprintf2("  i_alloc_struct_array: called with incorrect "
		         "structure type (not element), struct='%s', "
		         "client='%s'\n",
		         pstype->sname, cname);
		return NULL; /* fail */
	}
#endif
	obj = alloc_obj(imem, (uint32_t)num_elements * pstype->ssize, pstype,
	                ALLOC_DIRECT, cname);
	if_debug7('A', "[a%d:+<.]%s %s*(%lu=%u*%u) = 0x%lx\n",
	          alloc_trace_space(imem), client_name_string(cname),
	          struct_type_name_string(pstype),
	          (uint32_t)num_elements * pstype->ssize, num_elements,
	          pstype->ssize, (uint32_t)obj);
	return (char*)obj;
}
private
void*
                ...
                client_name_t cname)
{
	...
	uint32_t old_size = pre_obj_contents_size(pp);
	uint32_t new_size = (uint32_t)pstype->ssize * new_num_elements;
	uint32_t old_size_rounded = obj_align_round(old_size);
	uint32_t new_size_rounded = obj_align_round(new_size);
	void* new_obj = NULL;

	if(old_size_rounded == new_size_rounded) {
		pp->o_size = new_size;
		new_obj = obj;
	} else if((byte*)obj + old_size_rounded == imem->cc.cbot &&
	...
	if(new_obj) {
		if_debug8(
		    ...
		    alloc_trace_space(imem), (new_size > old_size ? '>' : '<'),
		    (pstype == &st_bytes ? 'b' : '<'),
		    client_name_string(cname), struct_type_name_string(pstype),
		    old_size, new_size, (uint32_t)obj);
		return new_obj;
	}
	/* Punt. */
	new_obj =
	    gs_alloc_struct_array(mem, new_num_elements, void, pstype, cname);
	if(new_obj == 0)
		return 0;
...
i_free_object(gs_memory_t* mem, void* ptr, client_name_t cname)
{
	...
	if(gs_debug_c('?')) {
		...
		if(pstype == &st_free) {
			lprintf2("%s: object 0x%lx already free!\n",
			         client_name_string(cname), (uint32_t)ptr);
			return; /*gs_abort(); */
		}
		...
		while((cld.cp = cld.memory->clast),
			...
			if(!cld.memory->saved) {
				lprintf3("%s: freeing 0x%lx, not owned by "
				         "memory 0x%lx!\n",
				         client_name_string(cname),
		...
		if(!(PTR_BETWEEN((const byte*)pp, cld.cp->cbase,
			...
			lprintf5("%s: freeing 0x%lx,\n\toutside chunk 0x%lx "
			         "cbase=0x%lx, cbot=0x%lx!\n",
			         client_name_string(cname), (uint32_t)ptr,
			         (uint32_t)cld.cp, (uint32_t)cld.cp->cbase,
			         (uint32_t)cld.cp->cbot);
			return; /*gs_abort(); */
		}
	}
#endif
	...
	if(finalize != 0) {
		if_debug3('u', "[u]finalizing %s 0x%lx (%s)\n",
		          struct_type_name_string(pstype), (uint32_t)ptr,
		          client_name_string(cname));
		(*finalize)(ptr);
	}
	...
	if(pp->o_alone) {
...
#ifdef DEBUG
		{
			...
			cld.cp = 0;
			if(gs_debug_c('a'))
				alloc_trace((chunk_locate_ptr(ptr, &cld)
				                 ? ":-oL"
				                 : ":-o~"),
		...
		if(chunk_locate(ptr, &imem->cfreed)) {
			...
			if((byte*)pp >= imem->cc.int_freed_top)
				imem->cc.int_freed_top =
				    (byte*)ptr + rounded_size;
			pp->o_type = &st_free; /* don't confuse GC */
			gs_alloc_fill(ptr, gs_alloc_fill_free, size);
			*(obj_header_t**)ptr = *pfl;
			*pfl = (obj_header_t*)ptr;
			alloc_trace(
			    (size > max_freelist_size ? ":-oF" : ":-of"), imem,
			    cname, pstype, size, ptr);
			return;
		}
		/* Don't overwrite even if gs_alloc_debug is set. */
	} else {
...
top:
	if(imem->cc.ctop - imem->cc.cbot > nbytes) {
		if_debug4('A', "[a%d:+> ]%s(%u) = 0x%lx\n",
		          alloc_trace_space(imem), client_name_string(cname),
		          nbytes, (uint32_t)(imem->cc.ctop - nbytes));
	...
	if(nbytes >
	   string_space_quanta(max_uint - sizeof(chunk_head_t)) *
	       string_data_quantum) { /* Can't represent the size in a uint! */
		return 0;
	}
	if(nbytes >= imem->large_size) { /* Give it a chunk all its own. */
		...
		chunk_t* cp = alloc_acquire_chunk(
		    imem, (uint32_t)imem->chunk_size, true, "chunk");
...
i_alloc_string_immovable(gs_memory_t* mem, uint nbytes, client_name_t cname)
{
	...
	chunk_t* cp = alloc_acquire_chunk(imem, (uint32_t)asize, true,
	                                  "large string chunk");
	...
	if_debug4('a', "[a%d|+>L]%s(%u) = 0x%lx\n", alloc_trace_space(imem),
	          client_name_string(cname), nbytes, (uint32_t)str);
	gs_alloc_fill(str, gs_alloc_fill_alloc, nbytes);
	return str;
}
private
                ...
                client_name_t cname)
{
	...
	if(data == imem->cc.ctop && /* bottom-most string */
	   (new_num < old_num ||
	    imem->cc.ctop - imem->cc.cbot >
	        new_num - old_num)) { /* Resize in place. */
		ptr = data + old_num - new_num;
		if_debug6(
		    'A', "[a%d:%c> ]%s(%u->%u) 0x%lx\n",
		    alloc_trace_space(imem), (new_num > old_num ? '>' : '<'),
		    client_name_string(cname), old_num, new_num, (uint32_t)ptr);
		imem->cc.ctop = ptr;
		...
		if(new_num > old_num)
			...
			gs_alloc_fill(data, gs_alloc_fill_free,
			              old_num - new_num);
#endif
...
private
void
i_free_string(gs_memory_t* mem, byte* data, uint nbytes, client_name_t cname)
{
	gs_ref_memory_t* const imem = (gs_ref_memory_t*)mem;
	if(data == imem->cc.ctop) {
		if_debug4('A', "[a%d:-> ]%s(%u) 0x%lx\n",
		          alloc_trace_space(imem), client_name_string(cname),
		          nbytes, (uint32_t)data);
		imem->cc.ctop += nbytes;
	} else {
		if_debug4('A', "[a%d:->#]%s(%u) 0x%lx\n",
		          alloc_trace_space(imem), client_name_string(cname),
...
{
	gs_ref_memory_t* const imem = (gs_ref_memory_t*)mem;
	uint32_t unused = imem->lost.refs + imem->lost.strings;
	uint32_t inner = 0;
	...
	pstat->used =
	    imem->allocated + inner - unused + imem->previous_status.used;
...
{
	uint32_t unused = mem->lost.objects;
	int i;
...
private
obj_header_t* /* rets obj if allocated, else 0 */
    large_freelist_alloc(gs_ref_memory_t* mem, uint size)
{
	...
	obj_header_t** best_fit_prev =
	    NULL; /* Initialize against indeteminizm. */
	...
	while((pfree = *ppfprev) != 0) {
		...
		if(free_size == aligned_size ||
		   (free_size >= aligned_min_size &&
		    free_size < best_fit_size)) {
			best_fit = pfree;
			best_fit_prev = ppfprev;
			best_fit_size = pfree[-1].o_size;
			if(best_fit_size <= aligned_max_size)
				break; /* good enough fit to spare scan of
				          entire list */
		}
	...
	if(best_fit == 0) {
		/*
		 * No single free chunk is large enough, but since we scanned
		 * the
          ...
          alloc_flags_t flags, client_name_t cname)
{
	...
	if(lsize >= mem->large_size || (flags & ALLOC_IMMOVABLE)) {
		/*
		 * Give the object a chunk all its own.  Note that this case
		 * does
		 * not occur if is_controlled is true.
		 */
		ulong asize = ((lsize + obj_align_mask) & -obj_align_mod) +
		              sizeof(obj_header_t);
		chunk_t* cp =
		    alloc_acquire_chunk(mem, asize + sizeof(chunk_head_t),
		                        false, "large object chunk");
	...
	((cp)->ctop - (byte*)(ptr = (obj_header_t*)(cp)->cbot) >               \
	 asize + sizeof(obj_header_t))
		...
		if(!allocate_success) {
			 ...
			 * It's not clear this is a good idea, since it requires
			 * quite
			 * a lot of computation and doesn't seem to improve
			 * things much.
			 */
			if(!mem->is_controlled) { /* already did this if
			                             controlled */
				chunk_t* cp = cp_orig;
...
#undef CAN_ALLOC_AT_END
		...
		if(!allocate_success) {
			/* Add another chunk. */
			chunk_t* cp = alloc_add_chunk(
			    mem, (ulong)mem->chunk_size, "chunk");
...
consolidate_chunk_free(chunk_t* cp, gs_ref_memory_t* mem)
{
	...
	if(pre->o_type == &st_free) {
		...
		if(begin_free)
			cp->int_freed_top =
			    (byte*)pre; /* first byte following internal free */
		begin_free = 0;
	}
	END_OBJECTS_SCAN
	if(begin_free) {
		...
		if_debug4('a', "[a]resetting chunk 0x%lx cbot from 0x%lx to "
		               "0x%lx (%lu free)\n",
...
private
obj_header_t* /* returns uninitialized object hdr, NULL if none found */
    scavenge_low_free(gs_ref_memory_t* mem, unsigned request_size)
{
	/* find 1st range of memory that can be glued back together to fill
	 * request */
	...
	uint need_free = request_size_rounded +
	                 sizeof(obj_header_t); /* room for GC's dummy hdr */
	chunk_t* cp;
	...
	for(cp = mem->cfirst; cp != 0; cp = cp->cnext) {
		...
		if(begin_free != 0 && found_free >= need_free) {
			...
			found_pre->o_type =
			    &st_free; /* don't confuse GC if gets lost */
			found_pre->o_size = found_free - sizeof(obj_header_t);

			/* Chop off excess tail piece & toss it back into free
			 * pool */
...
remove_range_from_freelist(gs_ref_memory_t* mem, void* bottom, void* top)
{
	...
	for(cur = bottom; cur != top;
	    cur = (const obj_header_t*)((const byte*)cur +
	                                obj_size_round(size))) {
		size = cur->o_size;
		i = (size > max_freelist_size
		...
		if(i > largest) {
			if(largest >= 0)
				memset(&num_free[largest + 1], 0,
				       (i - largest) * sizeof(int));
			largest = i;
		}
		num_free[i]++;
	}
...
/* trailing dummy obj_header */
{
	...
	uint excess_size =
	    old_rounded_size - rounded_size - sizeof(obj_header_t);
	...
	if(pre_obj->o_alone) {
		...
		if(cp) {
			...
			if(cp->cbot != (byte*)obj + old_rounded_size) {
				lprintf3("resizing 0x%lx, old size %u, new "
				         "size %u, cbot wrong!\n",
                ...
                void** up, client_name_t cname)
{
	...
	if(rp == NULL) {
		rp = gs_raw_alloc_struct_immovable(
		    imem->non_gc_memory, &st_gc_root_t, "i_register_root");
		if(rp == 0)
...
private
chunk_t*
alloc_add_chunk(gs_ref_memory_t* mem, uint32_t csize, client_name_t cname)
{
...
private
chunk_t*
alloc_acquire_chunk(gs_ref_memory_t* mem, uint32_t csize, bool has_strings,
                    client_name_t cname)
{
	...
	if(mem->gc_status.psignal != 0) {
		...
		if((uint32_t)(mem->allocated) >= mem->limit) {
			mem->gc_status.requested += csize;
			if(mem->limit >= mem->gc_status.max_vm) {
				gs_free_object(parent, cp, cname);
				return 0;
			}
			if_debug4('0', "[0]signaling space=%d, allocated=%ld, "
			               "limit=%ld, requested=%ld\n",
			          mem->space, (int32_t)mem->allocated,
			          (int32_t)mem->limit,
			          (int32_t)mem->gc_status.requested);
                 ...
                 chunk_t* outer)
{
	...
	cp->sbase = cdata;
	if(has_strings &&
	   top - cdata >= string_space_quantum + sizeof(int32_t) - 1) {
		/*
...
{
#ifdef DEBUG
	if(gs_alloc_debug) { /* Check to make sure this chunk belongs to this
	                        allocator. */
		...
		if(ap != cp) {
			lprintf2(
			    "unlink_chunk 0x%lx not owned by memory 0x%lx!\n",
			    (uint32_t)cp, (uint32_t)mem);
			return; /*gs_abort(); */
		}
	}
#endif
...
alloc_free_chunk(chunk_t* cp, gs_ref_memory_t* mem)
{
	...
	uint32_t csize = (byte*)cp->cend - cdata;
...
chunk_locate_ptr(const void* ptr, chunk_locator_t* clp)
{
	...
	if(cp == 0) {
		...
		if(cp == 0)
			return false;
		/* ptr is in the last chunk often enough to be worth checking
		 * for. */
...
{
	return ((pdc->bottom == NULL || PTR_GT(otop, pdc->bottom)) &&
	        (pdc->top == NULL || PTR_LT(obot, pdc->top)));
}

const dump_control_t dump_control_default = {dump_do_default, NULL, NULL};
const dump_control_t dump_control_all = {
    dump_do_strings | dump_do_type_addresses | dump_do_pointers |
        dump_do_pointed_strings | dump_do_contents,
    NULL, NULL};
...
private
void
debug_dump_contents(const byte* bot, const byte* top, int indent, bool as_chars)
{
	const byte* block;
...
#define block_size 16
	...
	if(bot >= top)
		return;
	for(block = bot - ((bot - (byte*)0) & (block_size - 1)); block < top;
	    block += block_size) {
		int i;
		char label[12];
...
/* contents. */
void
debug_print_object(const gs_memory_t* mem, const void* obj,
                   const dump_control_t* control)
{
	...
	if(options & dump_do_pointers) {
		...
		if(proc != gs_no_struct_enum_ptrs)
			for(; (ptype = (*proc)(mem, pre + 1, size, index, &eptr,
			                       type, NULL)) != 0;
			    ++index) {
				const void* ptr = eptr.ptr;

				dprintf1("    ptr %u: ", index);
				if(ptype == ptr_string_type ||
				   ptype == ptr_const_string_type) {
					const gs_const_string* str =
					    (const gs_const_string*)ptr;
...
void
debug_dump_chunk(const gs_memory_t* mem, const chunk_t* cp,
                 const dump_control_t* control)
{
	dprintf1("chunk at 0x%lx:\n", (ulong)cp);
	dprintf3("   chead=0x%lx  cbase=0x%lx sbase=0x%lx\n", (ulong)cp->chead,
	         (ulong)cp->cbase, (ulong)cp->sbase);
	dprintf3("    rcur=0x%lx   rtop=0x%lx  cbot=0x%lx\n", (ulong)cp->rcur,
	         (ulong)cp->rtop, (ulong)cp->cbot);
	...
	dprintf2("  sreloc=0x%lx   cend=0x%lx\n", (ulong)cp->sreloc,
	         (ulong)cp->cend);
	dprintf5(
	    "cprev=0x%lx cnext=0x%lx outer=0x%lx inner_count=%u has_refs=%s\n",
	...
	SCAN_CHUNK_OBJECTS(cp)
	DO_ALL
	if(obj_in_control_region(pre + 1, (const byte*)(pre + 1) + size,
	                         control))
...
debug_find_pointers(const gs_ref_memory_t* mem, const void* target)
{
	...
	for(mcp = mem->cfirst; mcp != 0; mcp = mcp->cnext) {
		...
		if(proc) /* doesn't trace refs */
			for(; (*proc)((const gs_memory_t*)mem, pre + 1, size,
			              index, &eptr, pre->o_type, NULL);
			    ++index)
				if(eptr.ptr == target) {
					dprintf1("Index %d in", index);
					debug_print_object(
					    (const gs_memory_t*)mem, pre + 1,
					    &control);
				}
		END_OBJECTS_SCAN_NO_ABORT
	}
