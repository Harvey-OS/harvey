...
private
obj_header_t *alloc_obj(gs_ref_memory_t *, ulong, gs_memory_type_ptr_t, alloc_flags_t, client_name_t);
private
 ...
 */
private
void *ialloc_solo(gs_memory_t *, gs_memory_type_ptr_t,
		  chunk_t **);
gs_ref_memory_t *
...
{ /*
	...
	uint32_t max_allocated =
	    (mem->gc_status.max_vm > mem->previous_status.allocated ? mem->gc_status.max_vm - mem->previous_status.allocated : 0);

	if(mem->gc_status.enabled) {
		uint32_t limit = mem->gc_allocated + mem->gc_status.vm_threshold;
...
#define IF_FREELIST_ALLOC(ptr, imem, size, pstype, pfl)                                                                  \
	if(size <= max_freelist_size && *(pfl = &imem->freelists[(size + obj_align_mask) >> log2_obj_align_mod]) != 0) { \
		ptr = *pfl;                                                                                              \
...
#define ELSEIF_BIG_FREELIST_ALLOC(ptr, imem, size, pstype)                                 \
	...
	else if(size > max_freelist_size && (ptr = large_freelist_alloc(imem, size)) != 0) \
	{                                                                                  \
		ptr[-1].o_type = pstype;                                                   \
		/* If debugging, clear the block in an attempt to */                       \
		/* track down uninitialized data errors. */                                \
...
#define ELSEIF_LIFO_ALLOC(ptr, imem, size, pstype)                                                                                                               \
	...
	else if((imem->cc.ctop - (byte *)(ptr = (obj_header_t *)imem->cc.cbot)) >= size + (obj_align_mod + sizeof(obj_header_t) * 2) && size < imem->large_size) \
	{                                                                                                                                                        \
		   ...
		   client_name_t cname)
{
	...
	if_debug6('A', "[a%d:+b.]%s -bytes-*(%lu=%u*%u) = 0x%lx\n",
		  ...
		  (uint32_t)num_elements * elt_size,
		  num_elements, elt_size, (uint32_t)obj);
	return (byte *)obj;
}
private
byte *
...
i_free_object(gs_memory_t *mem, void *ptr, client_name_t cname)
{
	...
	if(finalize != 0) {
		if_debug3('u', "[u]finalizing %s 0x%lx (%s)\n",
			  struct_type_name_string(pstype),
			  (uint32_t)ptr, client_name_string(cname));
		(*finalize)(ptr);
	}
...
top:
	...
	if(nbytes >= imem->large_size) { /* Give it a chunk all its own. */
		...
		chunk_t *cp =
		    alloc_acquire_chunk(imem, (uint32_t)imem->chunk_size, true,
					"chunk");
		...
		client_name_t cname)
{
	...
	if(data == imem->cc.ctop && /* bottom-most string */
	   (new_num < old_num ||
	    imem->cc.ctop - imem->cc.cbot > new_num - old_num)) { /* Resize in place. */
...
#define CAN_ALLOC_AT_END(cp) \
	((cp)->ctop - (byte *)(ptr = (obj_header_t *)(cp)->cbot) > asize + sizeof(obj_header_t))
	...
	for(cp = mem->clast; cp != 0; cp = cprev) {
		...
		if(cp->cbot == cp->cbase && cp->ctop == cp->climit) {
			...
			if(!mem->is_controlled) {
				...
				if(mem->pcc == cp)
					mem->pcc =
					    (cnext == 0 ? cprev : cprev == 0 ? cnext : cprev->cbot - cprev->ctop >
											       cnext->cbot - cnext->ctop
											   ? cprev
											   : cnext);
			}
		}
	...
	if(mem->gc_status.psignal != 0) {
		...
		if((uint32_t)(mem->allocated) >= mem->limit) {
			...
			if_debug4('0', "[0]signaling space=%d, allocated=%ld, limit=%ld, requested=%ld\n",
				  ...
				  (int32_t)mem->limit, (int32_t)mem->gc_status.requested);
		 ...
		 chunk_t *outer)
{
	...
	cp->has_refs = false;
	cp->sbase = cdata;
	if(has_strings && top - cdata >= string_space_quantum + sizeof(int32_t) - 1) {
		/*
		...
		if(proc != gs_no_struct_enum_ptrs)
			for(; (ptype = (*proc)(mem, pre + 1, size, index, &eptr, type, NULL)) != 0;
				...
				if(ptype == ptr_string_type || ptype == ptr_const_string_type) {
					...
					dprintf2("0x%lx(%u)", (uint32_t)str->data, str->size);
					...
					dprintf1((PTR_BETWEEN(ptr, obj, (const byte *)obj + size) ? "(0x%lx)\n" : "0x%lx\n"), (uint32_t)ptr);
				}
			}
	}
...
debug_dump_chunk(const gs_memory_t *mem, const chunk_t *cp, const dump_control_t *control)
{
	...
	if(control->options & dump_do_strings) {
		debug_dump_contents((control->bottom == 0 ? cp->ctop : max(control->bottom, cp->ctop)),
				    (control->top == 0 ? cp->climit : min(control->top, cp->climit)),
				    0, true);
	}
	SCAN_CHUNK_OBJECTS(cp)
	DO_ALL
