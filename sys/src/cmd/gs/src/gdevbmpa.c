...
#define async_device(procs, dname, w10, h10, xdpi, ydpi, lm, bm, rm, tm, color_bits, print_page)                                                                       \
	{                                                                                                                                                              \
		prn_device_std_margins_body(gx_device_async, procs, dname, w10, h10, xdpi, ydpi, lm, tm, lm, bm, rm, tm, color_bits, print_page), 0, 0, { 0, 0, 0, 0 } \
	}
...
private
const gx_device_procs bmpasep1_procs = {
...
private
const gx_device_procs bmpasep8_procs = {
			 ...
			 int raster)
{
	...
	for(plane = first_plane; plane <= last_plane; ++plane) {
		...
		code =
		    (first_plane < 0 ? write_bmp_header(pdev, prn_stream) : write_bmp_separated_header(pdev, prn_stream));
		if(code < 0)
			goto done;
	 ...
	 */
	     ...
	     */
			for(y = dev->height - 1; y >= 0; y = band_base_line - 1) {
				...
				code = crdev->buf_procs.setup_buf_device(bdev, raster_data, bmp_raster, NULL, 0, band_height,
									 band_height);
				if(code < 0)
					goto done;
