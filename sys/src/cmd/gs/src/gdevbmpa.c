...
#define async_device(procs, dname, w10, h10, xdpi, ydpi, lm, bm, rm, tm,       \
                     color_bits, print_page)                                   \
	{                                                                      \
		prn_device_std_margins_body(                                   \
		    gx_device_async, procs, dname, w10, h10, xdpi, ydpi, lm,   \
		    tm, lm, bm, rm, tm, color_bits, print_page),               \
		    0, 0,                                                      \
		{                                                              \
			0, 0, 0, 0                                             \
		}                                                              \
...
private
dev_proc_open_device(bmpa_writer_open);
private
dev_proc_open_device(bmpa_cmyk_writer_open);
private
prn_dev_proc_open_render_device(bmpa_reader_open_render_device);
private
dev_proc_print_page_copies(bmpa_reader_print_page_copies);
/* VMS limits procedure names to 31 characters. */
private
dev_proc_print_page_copies(bmpa_cmyk_reader_print_copies);
private
prn_dev_proc_buffer_page(bmpa_reader_buffer_page);
private
prn_dev_proc_buffer_page(bmpa_cmyk_reader_buffer_page);
private
dev_proc_output_page(bmpa_reader_output_page);
private
dev_proc_get_params(bmpa_get_params);
private
dev_proc_put_params(bmpa_put_params);
private
dev_proc_get_hardware_params(bmpa_get_hardware_params);
private
prn_dev_proc_start_render_thread(bmpa_reader_start_render_thread);
private
prn_dev_proc_get_space_params(bmpa_get_space_params);
#define default_print_page 0 /* not needed becoz print_page_copies def'd */

/* Monochrome. */

private
const gx_device_procs bmpamono_procs =
    prn_procs(bmpa_writer_open, gdev_prn_output_page, gdev_prn_close);
const gx_device_async gs_bmpamono_device =
    async_device(bmpamono_procs, "bmpamono", DEFAULT_WIDTH_10THS,
...
#define bmpa_cmyk_procs(p_open, p_map_color_rgb, p_map_cmyk_color)             \
	p_open, NULL, NULL, gdev_prn_output_page, gdev_prn_close, NULL,        \
	    p_map_color_rgb, NULL, NULL, NULL, NULL, NULL, NULL,               \
	    bmpa_get_params, bmpa_put_params, p_map_cmyk_color, NULL, NULL,    \
	    NULL, gx_page_device_get_page_device

private
const gx_device_procs bmpasep1_procs = {bmpa_cmyk_procs(
    bmpa_cmyk_writer_open, cmyk_1bit_map_color_rgb, cmyk_1bit_map_cmyk_color)};
const gx_device_async gs_bmpasep1_device = {prn_device_body(
    gx_device_async, bmpasep1_procs, "bmpasep1", DEFAULT_WIDTH_10THS,
    DEFAULT_HEIGHT_10THS, X_DPI, Y_DPI, 0, 0, 0, 0, /* margins */
...
private
const gx_device_procs bmpasep8_procs = {bmpa_cmyk_procs(
    bmpa_cmyk_writer_open, cmyk_8bit_map_color_rgb, cmyk_8bit_map_cmyk_color)};
const gx_device_async gs_bmpasep8_device = {prn_device_body(
    gx_device_async, bmpasep8_procs, "bmpasep8", DEFAULT_WIDTH_10THS,
    DEFAULT_HEIGHT_10THS, X_DPI, Y_DPI, 0, 0, 0, 0, /* margins */
...
private
const gx_device_procs bmpa256_procs =
    prn_color_procs(bmpa_writer_open, gdev_prn_output_page, gdev_prn_close,
                    pc_8bit_map_rgb_color, pc_8bit_map_color_rgb);
const gx_device_async gs_bmpa256_device =
    async_device(bmpa256_procs, "bmpa256", DEFAULT_WIDTH_10THS,
                 DEFAULT_HEIGHT_10THS, X_DPI, Y_DPI, 0, 0, 0, 0, /* margins */
...
private
const gx_device_procs bmpa16m_procs =
    prn_color_procs(bmpa_writer_open, gdev_prn_output_page, gdev_prn_close,
                    bmp_map_16m_rgb_color, bmp_map_16m_color_rgb);
const gx_device_async gs_bmpa16m_device =
    async_device(bmpa16m_procs, "bmpa16m", DEFAULT_WIDTH_10THS,
                 DEFAULT_HEIGHT_10THS, X_DPI, Y_DPI, 0, 0, 0, 0, /* margins */
...
private
const gx_device_procs bmpa32b_procs = {
...
const gx_device_async gs_bmpa32b_device =
    async_device(bmpa32b_procs, "bmpa32b", DEFAULT_WIDTH_10THS,
                 DEFAULT_HEIGHT_10THS, X_DPI, Y_DPI, 0, 0, 0, 0, /* margins */
                 ...
                 prn_dev_proc_buffer_page((*reader_buffer_page)))
{
	 ...
	 * there are no convenient macros for setting them up in static
	 * template.
	 */
	init_async_render_procs(pwdev, bmpa_reader_start_render_thread,
	                        reader_buffer_page, reader_print_page_copies);
	set_dev_proc(pdev, get_params, bmpa_get_params); /* because not all
	                                                    device-init macros
	                                                    allow this to be
	                                                    defined */
	 ...
	 */
	max_width =
	    DEFAULT_WIDTH_10THS * 60; /* figure max wid = default @ 600dpi */
	min_band_height = max(1, (DEFAULT_HEIGHT_10THS * 60) / 100);
	max_raster = bitmap_raster(
	    max_width *
	    pwdev->color_info.depth); /* doesn't need to be super accurate */
	...
	return gdev_prn_async_write_open((gx_device_printer*)pdev, max_raster,
	                                 min_band_height, max_src_image_row);
}
private
int
...
/* Forward declarations */
private
int bmpa_reader_buffer_planes(gx_device_printer* pdev, FILE* prn_stream,
                         ...
                         int raster)
{
	...
	if(prdev->buffered_page_exists) {
		code =
		    bmpa_reader_buffer_planes(pdev, prn_stream, num_copies,
	...
	for(plane = first_plane; plane <= last_plane; ++plane) {
		...
		code = (first_plane < 0
		            ? write_bmp_header(pdev, prn_stream)
		 ...
		 */
		if(plane >= 0)
			gx_render_plane_init(&render_plane, (gx_device*)pdev,
			                     plane);
		for(y = prdev->height - 1; y >= 0; y--) {
			uint actual_raster;

			code = gdev_prn_get_lines(
			    pdev, y, 1, row, bmp_raster, &raster_data,
			    &actual_raster, (plane < 0 ? NULL : &render_plane));
			if(code < 0)
				goto done;
			if(fwrite((const char*)raster_data, actual_raster, 1,
			          prn_stream) < 1) {
                          ...
                          int first_plane, int last_plane, int raster)
{
	...
	if(!prdev->buffered_page_exists) {
		code = bmpa_reader_print_planes(
		    pdev, file, num_copies, first_plane, last_plane, raster);
		goto done;
	}
...

	{
		...
		int padding =
		    -raster & 3; /* BMP scan lines are padded to 32 bits. */
		int bmp_raster = raster + padding;
		int plane;

		/*
		 * Get the address of the renderer's band buffer.  In the
		 * future,
		 * it will be possible to suppress the allocation of this
		 * buffer,
		...
		for(plane = first_plane; plane <= last_plane; ++plane) {
			...
			if(fseek(
			       file,
			       prdev->file_offset_to_data[plane - first_plane],
			       SEEK_SET)) {
				code = gs_note_error(gs_error_ioerror);
				goto done;
			}
			...
			if(plane >= 0)
				gx_render_plane_init(&render_plane,
				                     (gx_device*)pdev, plane);
			else
				render_plane.index = -1;

			/* Set up the buffer device. */
			code = gdev_create_buf_device(
			    crdev->buf_procs.create_buf_device, &bdev,
			    crdev->target, &render_plane, dev->memory, true);
			if(code < 0)
				goto done;

			/*
			 * Iterate thru bands from top to bottom.  As noted
			 * above, we
			 * do this an entire band at a time for efficiency.
			 */
			for(y = dev->height - 1; y >= 0;
			    y = band_base_line - 1) {
				int band_height = dev_proc(dev, get_band)(
				...
				code = crdev->buf_procs.setup_buf_device(
				    bdev, raster_data, bmp_raster, NULL, 0,
				    band_height, band_height);
				if(code < 0)
					goto done;

				/* Fill in the buffer with a band from the BMP
				 * file. */
				/* Need to do this backward since BMP is top to
				 * bottom. */
				for(line = band_height - 1; line >= 0; --line)
					if(fread(raster_data +
					             line * bmp_raster,
					         raster, 1, file) < 1 ||
				...
				/* Continue rendering on top of the existing
				 * bits. */
				band_rect.p.x = 0;
				band_rect.p.y = band_base_line;
				band_rect.q.x = pdev->width;
				band_rect.q.y = band_base_line + band_height;
				if((code = clist_render_rectangle(
				        (gx_device_clist*)pdev, &band_rect,
				        bdev, &render_plane, false)) < 0)
					goto done;

				/* Rewind & write out the updated buffer. */
				if(fseek(file, -bmp_raster * band_height,
				         SEEK_CUR)) {
					code = gs_note_error(gs_error_ioerror);
					goto done;
				}
				for(line = band_height - 1; line >= 0; --line) {
					if(fwrite(raster_data +
					              line * bmp_raster,
					          bmp_raster, 1, file) < 1 ||
					   fseek(file, padding, SEEK_CUR)) {
						code = gs_note_error(
                      ...
                      gdev_prn_space_params* space_params)
{
	...
	int min_band_height =
	    max(1, pdev->height / 100); /* make bands >= 1% of total */
	...
	/* need to include minimal writer requirements to satisfy rasterizer
	 * init */
	writer_space = /* add 5K slop for good measure */
	    5000 +
	    (72 + 8) * ((pdev->height / space_params->band.BandHeight) + 1);
...
bmpa_get_hardware_params(gx_device* dev, gs_param_list* plist)
{
	...
	if(param_requested(plist, test_name)) {
		...
		param_string_from_string(
		    param_str, test_value); /* value must be persistent to use
		                               this macro */
