...
#define PICTWriteColorTable(ptr, seed, numEntries, cspecarr)                                                                          \
	{                                                                                                                             \
		...
		for(i = 0; i < numEntries; i++)    /* ctTable */                                                                      \
			PICTWriteColorSpec(ptr, cspecarr[i].value, cspecarr[i].rgb.red, cspecarr[i].rgb.green, cspecarr[i].rgb.blue); \
	}

/* write a PixMap structure */
#define PICTWritePixMap(ptr, x, y, w, h, rowBytes, packType, packSize, hRes, vRes, pixelSize) \
	{                                                                                     \
...
#define PICTWriteDataPackBits(ptr, base, rowBytes, lines)                                                           \
	{                                                                                                           \
		...
		if(raster < 8) {/* data uncompressed */                                                             \
			...
			Ptr destBufBegin = (Ptr)malloc(raster + (raster + 126) / 127), destBuf, srcBuf = (Ptr)base; \
			short i, len;                                                                               \
                                                                                                                    \
			byteCount = 0;                                                                              \
			for(i = 0; i < lines; i++) {                                                                \
...
#define PICT_BkPat(ptr, byte1, byte2, byte3, byte4, byte5, byte6, byte7, byte8)                                   \
	{                                                                                                         \
		...
		PICTWritePattern(ptr, byte1, byte2, byte3, byte4, /* Pattern data */ byte5, byte6, byte7, byte8); \
	}
...
#define PICT_PackBitsRect_BitMap(ptr, x0, y0, w0, h0, x1, y1, w1, h1, rowBytes, mode, dataPtr, size) \
	{                                                                                            \
