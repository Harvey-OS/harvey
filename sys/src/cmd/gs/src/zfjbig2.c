...
private
void jbig2_global_data_finalize(void* vptr);
...
z_jbig2decode(i_ctx_t* i_ctx_p)
{
	...
	if(r_has_type(op, t_dictionary)) {
		...
		if(dict_find_string(op, ".jbig2globalctx", &sop) > 0) {
			...
			s_jbig2decode_set_global_ctx((stream_state*)&state,
			                             gref->global_ctx);
		}
	}

	/* we pass npop=0, since we've no arguments left to consume */
	/* we pass 0 instead of the usual rspace(sop) which will allocate
	   storage
...
z_jbig2makeglobalctx(i_ctx_t* i_ctx_p)
{
	...
	code = s_jbig2decode_make_global_ctx(data, size, &global_ctx);
	if(size > 0 && global_ctx == NULL) {
		dlprintf("failed to create parsed JBIG2GLOBALS object.");
		return_error(e_unknownerror);
	}

	st = ialloc_struct(jbig2_global_data_t, &st_jbig2_global_data_t,
...
private
void
jbig2_global_data_finalize(void* vptr)
{
	jbig2_global_data_t* st = vptr;

	if(st->global_ctx)
		jbig2_global_ctx_free(st->global_ctx);
	st->global_ctx = NULL;
}
