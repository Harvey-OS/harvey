...
void FAR* jpeg_get_large(j_common_ptr cinfo, size_t size);

void jpeg_free_large(j_common_ptr cinfo, void FAR* object, size_t size);
typedef void* backing_store_ptr;

int32_t jpeg_mem_available(j_common_ptr cinfo, int32_t min_bytes_needed,
...
int32_t jpeg_mem_init(j_common_ptr cinfo);
...
{ /* We use the offset of cinfo in jpeg_compress data here, but we */
	...
	return (jpeg_compress_data*)((byte*)cinfo -
	                             offset_of(jpeg_compress_data, cinfo));
}
...
jpeg_alloc(j_common_ptr cinfo, size_t size, const char* info)
{
	...
	jpeg_block_t* p = gs_alloc_struct_immovable(
	    mem, jpeg_block_t, &st_jpeg_block, "jpeg_alloc(block)");
