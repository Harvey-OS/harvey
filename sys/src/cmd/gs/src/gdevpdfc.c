...
#define CIE_CACHE_IS_IDENTITY(pc) ((pc)->floats.params.is_identity)
#define CIE_CACHE3_IS_IDENTITY(pca)                                            \
	(CIE_CACHE_IS_IDENTITY(&(pca)[0]) &&                                   \
	 CIE_CACHE_IS_IDENTITY(&(pca)[1]) && CIE_CACHE_IS_IDENTITY(&(pca)[2]))
...
cie_scalar_cache_is_exponential(const gx_cie_scalar_cache* pc, float* pexpt)
{
	return cie_values_are_exponential(pc->floats.values[0],
	                                  ...
	                                  pc->floats.values[CC_INDEX_1], pexpt);
}
...
cie_vector_cache_is_exponential(const gx_cie_vector_cache* pc, float* pexpt)
{
	return cie_values_are_exponential(pc->vecs.values[0].u,
	                                  ...
	                                  pc->vecs.values[CC_INDEX_1].u, pexpt);
}
...
{
	double k = CC_KEY(i);
	double g = (k >= 6.0 / 29 ? k * k * k : (k - 4.0 / 29) * (108.0 / 841));
...
cie_vector_cache_is_lab_abc(const gx_cie_vector_cache3_t* pvc, int i)
{
	...
	return (
	    fabs(cie_cached2float(pc3[0].vecs.values[i].u) - (l + 16) / 116) <
	        0.001 &&
	    fabs(cie_cached2float(pc3[1].vecs.values[i].u) - a / 500) < 0.001 &&
	    fabs(cie_cached2float(pc3[2].vecs.values[i].w) - b / -200) < 0.001);
}
...
cie_is_lab(const gs_cie_abc* pcie)
{
	...
	if(!(pcie->MatrixABC.cu.u == 1 && pcie->MatrixABC.cu.v == 1 &&
	     pcie->MatrixABC.cu.w == 1 && pcie->MatrixABC.cv.u == 1 &&
	     pcie->MatrixABC.cv.v == 0 && pcie->MatrixABC.cv.w == 0 &&
	     pcie->MatrixABC.cw.u == 0 && pcie->MatrixABC.cw.v == 0 &&
	     pcie->MatrixABC.cw.w == -1 && pcie->common.MatrixLMN.is_identity))
		return false;
                     ...
                     const gs_cie_common* pciec)
{
	...
	if(code < 0)
		return code;
	if(pciec->points.BlackPoint.u != 0 || pciec->points.BlackPoint.v != 0 ||
...
const pdf_color_space_names_t pdf_color_space_names = {PDF_COLOR_SPACE_NAMES};
 ...
 */
int
pdf_cspace_init_Device(const gs_memory_t* mem, gs_color_space* pcs,
                       int num_components)
{
	switch(num_components) {
	case 1:
		gs_cspace_init_DeviceGray(mem, pcs);
		break;
	case 3:
		gs_cspace_init_DeviceRGB(mem, pcs);
		break;
	case 4:
		gs_cspace_init_DeviceCMYK(mem, pcs);
		break;
	default:
...
private
int
pdf_separation_color_space(gx_device_pdf* pdev, cos_array_t* pca,
                           const char* csname, const cos_value_t* snames,
                           ...
                           const pdf_color_space_names_t* pcsn)
{
	...
	if((code = cos_array_add(pca, cos_c_string_value(&v, csname))) < 0 ||
	   ...
	   (code = pdf_color_space(pdev, &v, &ranges, alt_space, pcsn, false)) <
	       0 ||
                        ...
                        const gs_color_space* pcs, cos_array_t* pca)
{
	...
	if(pcs->params.indexed.use_proc) {
		...
		for(i = 0; i < num_entries; ++i) {
			...
			for(j = 0; j < num_components; ++j) {
				float v = (cc.paint.values[j] -
				           cmin.paint.values[j]) *
				          255 / (cmax.paint.values[j] -
				                 cmin.paint.values[j]);

				*pnext++ =
				    (v <= 0 ? 0 : v >= 255 ? 255 : (byte)v);
			}
		}
	} else
	...
	if((code = pdf_color_space(pdev, pvalue, NULL, base_space,
	   ...
	   (code = cos_array_add(
	        pca, cos_c_string_value(&v, pdf_color_space_names.Indexed
	                                /*pcsn->Indexed*/))) < 0 ||
	   (code = cos_array_add(pca, pvalue)) < 0 ||
	   (code = cos_array_add_int(pca, pip->hival)) < 0 ||
	   (code = cos_array_add_no_copy(
	        pca, cos_string_value(&v, table, string_used))) < 0)
		return code;
	return 0;
}
...
private
pdf_resource_t*
pdf_find_cspace_resource(gx_device_pdf* pdev, const byte* serialized,
	...
	for(i = 0; i < NUM_RESOURCE_CHAINS; i++) {
		for(pres = pchain[i]; pres != 0; pres = pres->next) {
			...
			if(ppcs->serialized_size != serialized_size)
				continue;
			if(!memcmp(ppcs->serialized, serialized,
			           ppcs->serialized_size))
				return pres;
		}
	}
	return NULL;
...
pdf_color_space_named(gx_device_pdf* pdev, cos_value_t* pvalue,
                      const gs_range_t** ppranges, const gs_color_space* pcs,
                      const pdf_color_space_names_t* pcsn, bool by_name,
                      const byte* res_name, int name_length)
{
	...
	case gs_color_space_index_CIEICC:
		/*
		 * Take a special early exit for unrecognized ICCBased color
		 * spaces,
		 * or for PDF 1.2 output (ICCBased color spaces date from PDF
		 * 1.3).
		 */
		if(pcs->params.icc.picc_info->picc == 0 ||
		   pdev->CompatibilityLevel < 1.3) {
			...
			return pdf_color_space(
			    pdev, pvalue, ppranges,
	...
	if(pcs->id != gs_no_id)
		pres = pdf_find_resource_by_gs_id(pdev, resourceColorSpace,
		                                  pcs->id);
	if(pres == NULL) {
		stream s;
		...
		if(serialized_size <= sizeof(serialized0))
			...
			serialized =
			    gs_alloc_bytes(pdev->pdf_memory, serialized_size,
			                   "pdf_color_space");
			if(serialized == NULL)
				return_error(gs_error_VMerror);
		}
		...
		sclose(&s);
		pres =
		    pdf_find_cspace_resource(pdev, serialized, serialized_size);
		if(pres != NULL) {
			if(serialized != serialized0)
				gs_free_object(pdev->pdf_memory, serialized,
				               "pdf_color_space");
			serialized = NULL;
		}
	}
	...
	case gs_color_space_index_CIEA: {
		...
		bool identityA = (pcie->MatrixA.u == 1 &&
		                  pcie->MatrixA.v == 1 && pcie->MatrixA.w == 1);
		gs_vector3 expts;

		pciec = (const gs_cie_common*)pcie;
		if(!pcie->common.MatrixLMN.is_identity) {
			code = pdf_convert_cie_space(
			    pdev, pca, pcs, "GRAY", pciec, &pcie->RangeA,
			    ONE_STEP_NOT, NULL, &ranges);
			break;
		}
		if(unitary && identityA &&
		   CIE_CACHE_IS_IDENTITY(&pcie->caches.DecodeA) &&
		   CIE_SCALAR3_CACHE_IS_EXPONENTIAL(
		       pcie->common.caches.DecodeLMN, expts) &&
		   expts.v == expts.u && expts.w == expts.u) {
			DO_NOTHING;
		} else if(unitary && identityA &&
		          CIE_CACHE3_IS_IDENTITY(
		              pcie->common.caches.DecodeLMN) &&
		          cie_vector_cache_is_exponential(&pcie->caches.DecodeA,
		                                          &expts.u)) {
			DO_NOTHING;
		} else {
			code = pdf_convert_cie_space(
	...
	case gs_color_space_index_CIEABC: {
		...
		if(unitary) {
			switch(one_step) {
			case ONE_STEP_ABC:
				if(CIE_VECTOR3_CACHE_IS_EXPONENTIAL(
				       pcie->caches.DecodeABC.caches, expts))
					goto calrgb;
				break;
			case ONE_STEP_LMN:
				if(CIE_SCALAR3_CACHE_IS_EXPONENTIAL(
				       pcie->common.caches.DecodeLMN, expts))
					goto calrgb;
			default:
				break;
		...
		if(cie_is_lab(pcie)) {
			...
			if(pcd == 0)
				return_error(gs_error_VMerror);
			code = pdf_put_lab_color_space(pca, pcd,
			                               pcie->RangeABC.ranges);
			goto cal;
		} else {
			code = pdf_convert_cie_space(
			    pdev, pca, pcs, "RGB ", pciec,
			    pcie->RangeABC.ranges, one_step, pmat, &ranges);
			break;
		}
	calrgb:
		...
		if(expts.u != 1 || expts.v != 1 || expts.w != 1) {
			code =
			    cos_dict_put_c_key_vector3(pcd, "/Gamma", &expts);
			if(code < 0)
				return code;
		}
		if(!pmat->is_identity) {
			...
			if(pcma == 0)
				return_error(gs_error_VMerror);
			if((code = cos_array_add_vector3(pcma, &pmat->cu)) <
			       0 ||
			   (code = cos_array_add_vector3(pcma, &pmat->cv)) <
			       0 ||
			   (code = cos_array_add_vector3(pcma, &pmat->cw)) <
			       0 ||
			   (code = cos_dict_put(pcd, (const byte*)"/Matrix", 7,
			                        COS_OBJECT_VALUE(&v, pcma))) <
			       0)
				return code;
		}
	}
	...
	case gs_color_space_index_CIEDEF:
		code =
		    pdf_convert_cie_space(pdev, pca, pcs, "RGB ",
	...
	case gs_color_space_index_CIEDEFG:
		code = pdf_convert_cie_space(
		    ...
		    pcs->params.defg->RangeDEFG.ranges, ONE_STEP_NOT, NULL,
		    &ranges);
		break;
	...
	case gs_color_space_index_DeviceN:
			...
			return_error(gs_error_rangecheck);
		{
			...
			if(psna == 0)
				return_error(gs_error_VMerror);
			for(i = 0; i < pcs->params.device_n.num_components;
			    ++i) {
				if((code =
				        pcs->params.device_n
				            .get_colorname_string(
				                pdev->memory,
				                pcs->params.device_n.names[i],
				                &name_string,
				                &name_string_length)) < 0 ||
				   (code = pdf_string_to_cos_name(
				        pdev, name_string, name_string_length,
				        &v)) < 0 ||
				   (code = cos_array_add_no_copy(psna, &v)) < 0)
					return code;
			}
			COS_OBJECT_VALUE(&v, psna);
			if((code = pdf_separation_color_space(
			        pdev, pca, "/DeviceN", &v,
			        (const gs_color_space*)&pcs->params.device_n
			            .alt_space,
	...
	case gs_color_space_index_Separation:
			...
			return_error(gs_error_rangecheck);
		{
			...
			if((code = pcs->params.separation.get_colorname_string(
			        pdev->memory, pcs->params.separation.sep_name,
			        &name_string, &name_string_length)) < 0 ||
			   (code = pdf_string_to_cos_name(pdev, name_string,
			                                  name_string_length,
			                                  &v)) < 0 ||
			   (code = pdf_separation_color_space(
			        pdev, pca, "/Separation", &v,
			        (const gs_color_space*)&pcs->params.separation
			            .alt_space,
			        pfn, &pdf_color_space_names)) < 0)
				return code;
		}
	...
	case gs_color_space_index_Pattern:
		if((code = pdf_color_space(
		        pdev, pvalue, ppranges,
		        (const gs_color_space*)&pcs->params.pattern.base_space,
		        &pdf_color_space_names, false)) < 0 ||
		   (code = cos_array_add(
		        pca, cos_c_string_value(&v, "/Pattern"))) < 0 ||
		   (code = cos_array_add(pca, pvalue)) < 0)
			return code;
		break;
	 ...
	 */
	{
		...
		if(ranges) {
			...
			gs_range_t* copy_ranges =
			    (gs_range_t*)gs_alloc_byte_array(
			        pdev->pdf_memory, num_comp, sizeof(gs_range_t),
			        "pdf_color_space");
			...
			memcpy(copy_ranges, ranges,
			       num_comp * sizeof(gs_range_t));
...
pdf_color_space(gx_device_pdf* pdev, cos_value_t* pvalue,
                ...
                const pdf_color_space_names_t* pcsn, bool by_name)
{
	return pdf_color_space_named(pdev, pvalue, ppranges, pcs, pcsn, by_name,
	                             NULL, 0);
}
                  ...
                  pdf_resource_t** ppres, const char* cs_name)
{
	...
	if(!*ppres) {
		int code = pdf_begin_resource_body(pdev, resourceColorSpace,
		                                   gs_no_id, ppres);
	...
	code = pdf_add_resource(pdev, pdev->substream_Resources, "/ColorSpace",
	                        *ppres);
	if(code < 0)
		return code;
...
pdf_cs_Pattern_uncolored(gx_device_pdf* pdev, cos_value_t* pvalue)
{
	...
	static const char* const pcs_names[5] = {0, "[/Pattern /DeviceGray]", 0,
	                                         "[/Pattern /DeviceRGB]",
	                                         "[/Pattern /DeviceCMYK]"};
...
int
pdf_cs_Pattern_uncolored_hl(gx_device_pdf* pdev, const gs_color_space* pcs,
                            cos_value_t* pvalue)
{
	/* Only for high level colors. */
	return pdf_color_space(pdev, pvalue, NULL, pcs, &pdf_color_space_names,
	                       true);
}
