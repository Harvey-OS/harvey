...
clist_plane_raster(const gx_device *dev, const gx_render_plane_t *render_plane)
{
	return bitmap_raster(dev->width *
			     (render_plane && render_plane->index >= 0 ? render_plane->depth : dev->color_info.depth));
}
			 ...
			 gs_get_bits_params_t *params, gs_int_rect **unread)
{
	...
	gx_device *bdev;
	int num_planes =
	    (options & GB_PACKING_CHUNKY ? 1 : options & GB_PACKING_PLANAR ? dev->color_info.num_components : options & GB_PACKING_BIT_PLANAR ? dev->color_info.depth : 0 /****** NOT POSSIBLE ******/);
	...
	code = dev_proc(bdev, get_bits_rectangle)(bdev, &band_rect, params, unread);
	...
	}
	{
		...
		while((y += lines_rasterized) < end_y) {
			...
			band_rect.p.y = my;
			band_rect.q.y = my + lines_rasterized;
			code = dev_proc(bdev, get_bits_rectangle)(bdev, &band_rect, &band_params, unread);
			if(code < 0)
				break;
			  ...
			  int *pmy)
{
	...
	if(crdev->ymin < 0 || crdev->yplane.index != plane_index ||
	   !(y >= crdev->ymin && y < crdev->ymax)) {
		...
		if(y < 0 || y > dev->height)
			return_error(gs_error_rangecheck);
		code = crdev->buf_procs.setup_buf_device(bdev, mdata, raster, NULL, 0, band_num_lines, band_num_lines);
		band_rect.p.x = 0;
	...
	code = crdev->buf_procs.setup_buf_device(bdev, mdata, raster, NULL, y - crdev->ymin, line_count,
		       ...
		       const gx_render_plane_t *render_plane, bool clear)
{
	...
	if(clear)
		dev_proc(bdev, fill_rectangle)(bdev, 0, 0, bdev->width, bdev->height, gx_device_white(bdev));
