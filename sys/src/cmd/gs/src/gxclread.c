...
/* Copyright (C) 1991, 1996, 1997, 1998, 1999 Aladdin Enterprises.  All rights
  reserved.
...
private
int clist_render_init(gx_device_clist*);
private
...
private
int clist_rasterize_lines(gx_device* dev, int y, int lineCount, gx_device* bdev,
                          const gx_render_plane_t* render_plane, int* pmy);
                          ...
                          gx_render_plane_t* render_plane, int index)
{
	if(index >= 0) {
		...
		gdev_prn_colors_used(dev, y, height, &colors_used,
		                     &ignore_start);
...
clist_setup_params(gx_device* dev)
{
	...
	code = clist_playback_file_bands(playback_action_setup, crdev,
	                                 &crdev->page_info, 0, 0, 0, 0, 0);
                         ...
                         gs_get_bits_params_t* params, gs_int_rect** unread)
{
	...
	gx_device* bdev;
	int num_planes = (options & GB_PACKING_CHUNKY
	                      ? 1
	                      : options & GB_PACKING_PLANAR
	                            ? dev->color_info.num_components
	                            : options & GB_PACKING_BIT_PLANAR
	                                  ? dev->color_info.depth
	                                  : 0 /****** NOT POSSIBLE ******/);
	...
	if(prect->p.x < 0 || prect->q.x > dev->width || y < 0 ||
	   end_y > dev->height)
	...
	clist_select_render_plane(dev, y, line_count, &render_plane,
	                          plane_index);
	code = gdev_create_buf_device(cdev->buf_procs.create_buf_device, &bdev,
	                              cdev->target, &render_plane, dev->memory,
	                              true);
	if(code < 0)
		return code;
	code =
	    clist_rasterize_lines(dev, y, line_count, bdev, &render_plane, &my);
	if(code < 0)
		return code;
	...
	code = dev_proc(bdev, get_bits_rectangle)(bdev, &band_rect, params,
	                                          unread);
	...
	if(crdev->ymin < 0 || crdev->yplane.index != plane_index ||
	   !(y >= crdev->ymin && y < crdev->ymax)) {
		...
		code = crdev->buf_procs.setup_buf_device(
		    bdev, mdata, raster, NULL, 0, band_num_lines,
		    band_num_lines);
		band_rect.p.x = 0;
		...
		if(code >= 0)
			code = clist_render_rectangle(cdev, &band_rect, bdev,
			                              render_plane, true);
                       ...
                       bool clear)
{
	...
	if(clear)
		dev_proc(bdev, fill_rectangle)(bdev, 0, 0, bdev->width,
		                               bdev->height,
		                               gx_device_white(bdev));
	...
	for(i = 0; i < num_pages && code >= 0; ++i) {
		...
		code = clist_playback_file_bands(
		    playback_action_render, crdev, &ppage->page->info, bdev,
		    band_first, band_last, prect->p.x - ppage->offset.x,
		    prect->p.y);
	}
	return code;
}
                          ...
                          int band_first, int band_last, int x0, int y0)
{
	...
	if(rs.page_cfile == 0) {
		code = clist_fopen(rs.page_cfname, gp_fmode_rb, &rs.page_cfile,
		                   cdev->bandlist_memory, cdev->bandlist_memory,
		                   true);
		opened_cfile = (code >= 0);
	}
	if(rs.page_bfile == 0 && code >= 0) {
		code = clist_fopen(rs.page_bfname, gp_fmode_rb, &rs.page_bfile,
		                   cdev->bandlist_memory, cdev->bandlist_memory,
		                   false);
		opened_bfile = (code >= 0);
	}
	if(rs.page_cfile != 0 && rs.page_bfile != 0) {
		...
		/* The stream doesn't need a memory, but we'll need to access
		 * s.memory->gs_lib_ctx. */
		s_init(&s, mem);
		...
		code =
		    clist_playback_band(action, cdev, &s, target, x0, y0, mem);
	}
