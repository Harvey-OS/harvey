...
gs_public_st_suffix_add0_final(st_device_pclxl, gx_device_pclxl,
                               "gx_device_pclxl", device_pclxl_enum_ptrs,
...
#define pclxl_device_body(dname, depth)                                        \
	std_device_dci_type_body(gx_device_pclxl, 0, dname, &st_device_pclxl,  \
	                         ...
	                         DEFAULT_HEIGHT_10THS * Y_DPI / 10, X_DPI,     \
	                         Y_DPI, (depth > 8 ? 3 : 1), depth,            \
	                         (depth > 1 ? 255 : 1), (depth > 8 ? 255 : 0), \
	                         (depth > 1 ? 256 : 2), (depth > 8 ? 256 : 1))

/* Driver procedures */
private
dev_proc_open_device(pclxl_open_device);
private
dev_proc_output_page(pclxl_output_page);
private
dev_proc_close_device(pclxl_close_device);
private
dev_proc_copy_mono(pclxl_copy_mono);
private
dev_proc_copy_color(pclxl_copy_color);
private
dev_proc_fill_mask(pclxl_fill_mask);

private
dev_proc_get_params(pclxl_get_params);
private
dev_proc_put_params(pclxl_put_params);

/*private dev_proc_draw_thin_line(pclxl_draw_thin_line); */
private
dev_proc_begin_image(pclxl_begin_image);
private
dev_proc_strip_copy_rop(pclxl_strip_copy_rop);

#define pclxl_device_procs(map_rgb_color, map_color_rgb)                       \
	{                                                                      \
		pclxl_open_device, NULL, /* get_initial_matrix */              \
		    NULL,                /* sync_output */                     \
		    pclxl_output_page, pclxl_close_device,                     \
		    map_rgb_color,                    /* differs */            \
		    map_color_rgb,                    /* differs */            \
		    gdev_vector_fill_rectangle, NULL, /* tile_rectangle */     \
		    pclxl_copy_mono, pclxl_copy_color, NULL, /* draw_line */   \
		    NULL,                                    /* get_bits */    \
		    pclxl_get_params, pclxl_put_params,                        \
		    NULL, /* map_cmyk_color */                                 \
                        ...
                        const byte* palette, uint palette_size)
{
	if(xdev->color_space != color_space ||
	   ...
	   memcmp(xdev->palette.data, palette, palette_size)) {
		stream* s = pclxl_stream(xdev);
		static const byte csp_[] = {DA(pxaColorSpace), DUB(e8Bit),
		                            DA(pxaPaletteDepth),
		                            pxt_ubyte_array};
...
pclxl_can_handle_color_space(const gs_color_space* pcs)
{
	...
	if(index == gs_color_space_index_Indexed) {
		if(pcs->params.indexed.use_proc)
			return false;
		index = gs_color_space_get_index(
...
pclxl_set_paints(gx_device_pclxl* xdev, gx_path_type_t type)
{
	...
	if(!(type & gx_path_type_fill) &&
	   (color_is_set(&xdev->saved_fill_color.saved_dev_color) ||
		...
		static const byte nac_[] = {DUB(0), DA(pxaNullBrush),
		                            pxtSetBrushSource};

		PX_PUT_LIT(s, nac_);
		color_set_null(&xdev->saved_fill_color.saved_dev_color);
		if(rule != xdev->fill_rule) {
			px_put_ub(s, (byte)(rule == gx_path_type_even_odd
			                        ? eEvenOdd
			                        : eNonZeroWinding));
	...
	if(!(type & gx_path_type_stroke) &&
	   (color_is_set(&xdev->saved_stroke_color.saved_dev_color) ||
	    !gx_dc_is_null(&xdev->saved_fill_color.saved_dev_color))) {
		static const byte nac_[] = {DUB(0), DA(pxaNullPen),
		                            pxtSetPenSource};
...
pclxl_flush_points(gx_device_pclxl* xdev)
{
	...
	if(count) {
		...
		/* Use byte values. */
		useb:
			px_put_np(s, count, data_type);
			spputc(s, (byte)op);
			px_put_data_length(s,
			                   count * 2); /* 2 bytes per point */
...
private
image_enum_proc_plane_data(pclxl_image_plane_data);
private
image_enum_proc_end_image(pclxl_image_end_image);
private
const gx_image_enum_procs_t pclxl_image_enum_procs = {pclxl_image_plane_data,
                       ...
                       uint raster, uint width_bits, int y, int height)
{
	...
	if(compress) {
		 ...
		 * temporary buffer for the compressed data.  Currently we don't
		 * go
		 * to the trouble of doing two passes if we can't allocate a
		 * buffer
		...
		for(i = 0; i < height; ++i) {
			...
			if((*s_RLE_template.process)((stream_state*)&rlstate,
			                             &r, &w, false) != 0 ||
			   r.ptr != r.limit)
				goto ncfree;
		...
		r.ptr = r.limit;
		if((*s_RLE_template.process)((stream_state*)&rlstate, &r, &w,
		                             true) != 0)
			goto ncfree;
		{
...
pclxl_define_bitmap_font(gx_device_pclxl* xdev)
{
	...
	static const byte bfh_[] = {
	    ...
	    DUS(8 + 6 + 4 + 6), DA(pxaFontHeaderLength), pxtReadFontHeader,
	    pxt_dataLengthByte, 8 + 6 + 4 + 6, 0, 0, 0, 0, 254, 0,
	    (MAX_CACHED_CHARS + 255) >> 8, 0, 'B', 'R', 0, 0, 0, 4};
...
{
	stream* s = pclxl_stream(xdev);
	static const byte sf_[] = {DA(pxaFontName),  DUB(1),
	                           DA(pxaCharSize),  DUS(0),
	                           DA(pxaSymbolSet), pxtSetFont};
...
private
void
pclxl_define_bitmap_char(gx_device_pclxl* xdev, uint ccode, const byte* data,
                         uint raster, uint width_bits, uint height)
{
	stream* s = pclxl_stream(xdev);
	uint width_bytes = (width_bits + 7) >> 3;
...
private
int
pclxl_copy_text_char(gx_device_pclxl* xdev, const byte* data, int raster,
                     gx_bitmap_id id, int w, int h)
{
	...
	if((ccode = xdev->chars.table[index]) < 2) {
		...
		while(xdev->chars.used + size > MAX_CHAR_DATA ||
			...
			index =
			    pclxl_char_index(xdev, xdev->chars.data[ccode].id);
...
private
int
pclxl_dorect(gx_device_vector* vdev, fixed x0, fixed y0, fixed x1, fixed y1,
             gx_path_type_t type)
{
	gx_device_pclxl* const xdev = (gx_device_pclxl*)vdev;
	stream* s = gdev_vector_stream(vdev);
...
#define OUT_OF_RANGE(v) (v < 0 || v >= int2fixed(0x10000))
	if(OUT_OF_RANGE(x0) || OUT_OF_RANGE(y0) || OUT_OF_RANGE(x1) ||
	   OUT_OF_RANGE(y1))
		return_error(gs_error_rangecheck);
#undef OUT_OF_RANGE
	...
	if(type & gx_path_type_clip) {
		static const byte cr_[] = {DA(pxaBoundingBox), DUB(eInterior),
		                           DA(pxaClipRegion),
		                           pxtSetClipRectangle};
             ...
             gx_path_type_t type)
{
	...
	if(code < 0)
		return code;
	return pclxl_set_cursor(xdev, xdev->points.current.x = (int)x,
...
private
int
pclxl_curveto(gx_device_vector* vdev, floatp x0, floatp y0, floatp x1,
...
private
int
pclxl_closepath(gx_device_vector* vdev, floatp x, floatp y, floatp x_start,
...
pclxl_endpath(gx_device_vector* vdev, gx_path_type_t type)
{
	...
	if(type & gx_path_type_clip) {
		static const byte scr_[] = {DUB(eInterior), DA(pxaClipRegion),
		                            pxtSetClipReplace};
...
private
const gx_device_vector_procs pclxl_vector_procs = {
    /* Page management */
    pclxl_beginpage,
    /* Imager state */
    pclxl_setlinewidth, pclxl_setlinecap, pclxl_setlinejoin,
    pclxl_setmiterlimit, pclxl_setdash, gdev_vector_setflat, pclxl_setlogop,
    /* Other state */
    pclxl_can_handle_hl_color, pclxl_setfillcolor, pclxl_setstrokecolor,
    /* Paths */
    gdev_vector_dopath, pclxl_dorect, pclxl_beginpath, pclxl_moveto,
    pclxl_lineto, pclxl_curveto, pclxl_closepath, pclxl_endpath};
...
private
const byte eBit_values[] = {0, e1Bit, 0, 0, e4Bit, 0, 0, 0, e8Bit};
                ...
                gx_color_index zero, gx_color_index one)
{
	...
	s = pclxl_stream(xdev);
	{
		static const byte mi_[] = {DUB(e1Bit), DA(pxaColorDepth),
		                           DUB(eIndexedPixel),
		                           DA(pxaColorMapping)};
...
private
int
pclxl_copy_color(gx_device* dev, const byte* base, int sourcex, int raster,
                 gx_bitmap_id id, int x, int y, int w, int h)
{
	gx_device_vector* const vdev = (gx_device_vector*)dev;
	gx_device_pclxl* const xdev = (gx_device_pclxl*)dev;
	...
	if((source_bit & 7) != 0)
		return gx_default_copy_color(dev, base, sourcex, raster, id, x,
		                             y, w, h);
	gdev_vector_update_log_op(vdev, rop3_S);
	pclxl_set_cursor(xdev, x, y);
	s = pclxl_stream(xdev);
	{
		static const byte ci_[] = {DA(pxaColorDepth), DUB(eDirectPixel),
		                           DA(pxaColorMapping)};
...
private
int
pclxl_fill_mask(gx_device* dev, const byte* data, int data_x, int raster,
                gx_bitmap_id id, int x, int y, int w, int h,
                ...
                gs_logical_operation_t lop, const gx_clip_path* pcpath)
{
	...
	code =
	    gdev_vector_update_log_op(vdev, lop | rop3_S | lop_S_transparent);
	if(code < 0)
		return 0;
...
private
int
pclxl_begin_image(gx_device* dev, const gs_imager_state* pis,
                  ...
                  gx_image_enum_common_t** pinfo)
{
	 ...
	 */
	int bits_per_pixel =
	    (pim->ImageMask ? 1 : pim->BitsPerComponent *
	...
	if(mat.xx <= 0 || mat.xy != 0 || mat.yx != 0 || mat.yy <= 0 ||
	   (pim->ImageMask ? (!gx_dc_is_pure(pdcolor) || pim->CombineWithColor)
	                   : (!pclxl_can_handle_color_space(pim->ColorSpace) ||
	                      (bits_per_pixel != 1 && bits_per_pixel != 4 &&
	                       bits_per_pixel != 8))) ||
	   format != gs_image_format_chunky || prect)
		goto use_default;
	...
	code = gdev_vector_begin_image(vdev, pis, pim, format, prect, pdcolor,
	                               pcpath, mem, &pclxl_image_enum_procs,
	...
	*pinfo = (gx_image_enum_common_t*)pie;
	{
		...
		if(pim->ImageMask) {
			const byte* palette =
			    (const byte*)(pim->Decode[0] ? "\377\000"
			                                 : "\000\377");

			code = gdev_vector_update_fill_color(
			    vdev, NULL, /* use process color */
			    pdcolor);
			if(code < 0)
				goto fail;
...
pclxl_image_write_rows(pclxl_image_enum_t* pie)
{
	...
	static const byte ii_[] = {DA(pxaColorDepth), DUB(eIndexedPixel),
	                           DA(pxaColorMapping)};
