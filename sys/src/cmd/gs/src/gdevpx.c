...
#define pclxl_device_procs(map_rgb_color, map_color_rgb)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           \
	{                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          \
		pclxl_open_device, NULL, /* get_initial_matrix */ NULL, /* sync_output */ pclxl_output_page, pclxl_close_device, map_rgb_color, /* differs */ map_color_rgb, /* differs */ gdev_vector_fill_rectangle, NULL, /* tile_rectangle */ pclxl_copy_mono, pclxl_copy_color, NULL, /* draw_line */ NULL, /* get_bits */ pclxl_get_params, pclxl_put_params, NULL, /* map_cmyk_color */ NULL, /* get_xfont_procs */ NULL, /* get_xfont_device */ NULL, /* map_rgb_alpha_color */ gx_page_device_get_page_device, NULL, /* get_alpha_bits */ NULL, /* copy_alpha */ NULL, /* get_band */ NULL, /* copy_rop */ gdev_vector_fill_path, gdev_vector_stroke_path, pclxl_fill_mask, gdev_vector_fill_trapezoid, gdev_vector_fill_parallelogram, gdev_vector_fill_triangle, NULL /****** WRONG ******/, /* draw_thin_line */ pclxl_begin_image, NULL, /* image_data */ NULL, /* end_image */ NULL, /* strip_tile_rectangle */ pclxl_strip_copy_rop \
	}
	...
	if(!(type & gx_path_type_fill) &&
	   (color_is_set(&xdev->saved_fill_color.saved_dev_color) ||
		...
		if(rule != xdev->fill_rule) {
			px_put_ub(s, (byte)(rule == gx_path_type_even_odd ? eEvenOdd : eNonZeroWinding));
...
private
const gx_image_enum_procs_t pclxl_image_enum_procs = {
		       ...
		       uint raster, uint width_bits, int y, int height)
{
	 ...
	 */
		for(i = 0; i < height; ++i) {
			...
			if((*s_RLE_template.process)((stream_state *)&rlstate, &r, &w, false) != 0 ||
			   r.ptr != r.limit)
				goto ncfree;
		...
		r.ptr = r.limit;
		if((*s_RLE_template.process)((stream_state *)&rlstate, &r, &w, true) != 0)
			goto ncfree;
		{
		  ...
		  gx_image_enum_common_t **pinfo)
{
     ...
     */
	int bits_per_pixel =
	    (pim->ImageMask ? 1 : pim->BitsPerComponent * gs_color_space_num_components(pcs));
	gs_matrix mat;
	int code;
	...
	if(mat.xx <= 0 || mat.xy != 0 || mat.yx != 0 || mat.yy <= 0 ||
	   (pim->ImageMask ? (!gx_dc_is_pure(pdcolor) || pim->CombineWithColor) : (!pclxl_can_handle_color_space(pim->ColorSpace) ||
	...
	*pinfo = (gx_image_enum_common_t *)pie;
	{
		...
		if(pim->ImageMask) {
			const byte *palette = (const byte *)(pim->Decode[0] ? "\377\000" : "\000\377");
			...
			if(code < 0)
				goto fail;
			code = gdev_vector_update_log_op(vdev, lop | rop3_S | lop_S_transparent);
			if(code < 0)
				goto fail;
			...
			code = gdev_vector_update_log_op(vdev, (pim->CombineWithColor ? lop : rop3_know_T_0(lop)));
			if(code < 0)
				goto fail;
