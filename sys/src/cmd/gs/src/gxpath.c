...
#define path_alloc_segment(pseg, ctype, pstype, stype, snotes, cname)                        \
	...
	if(!(pseg = gs_alloc_struct(gs_memory_stable(ppath->memory), ctype, pstype, cname))) \
...
gx_path_add_relative_point(gx_path *ppath, fixed dx, fixed dy)
{
	if(!path_position_in_range(ppath))
		return_error((path_position_valid(ppath) ? gs_error_limitcheck : gs_error_nocurrentpoint));
	{
