...
/* Forward references */
private
int path_alloc_copy(gx_path*);
private
int gx_path_new_subpath(gx_path*);

#ifdef DEBUG
private
void gx_print_segment(const segment*);

#define trace_segment(msg, pseg)                                               \
	if(gs_debug_c('P'))                                                    \
		dlprintf(msg), gx_print_segment(pseg);
#else
...
private
rc_free_proc(rc_free_path_segments);
private
rc_free_proc(rc_free_path_segments_local);

/*
 * Define the default virtual path interface implementation.
 */
private
int gz_path_add_point(gx_path*, fixed, fixed),
    gz_path_add_line_notes(gx_path*, fixed, fixed, segment_notes),
    gz_path_add_curve_notes(gx_path*, fixed, fixed, fixed, fixed, fixed, fixed,
                            segment_notes),
    gz_path_close_subpath_notes(gx_path*, segment_notes);
private
byte gz_path_state_flags(gx_path* ppath, byte flags);

private
gx_path_procs default_path_procs = {
    gz_path_add_point, gz_path_add_line_notes, gz_path_add_curve_notes,
    gz_path_close_subpath_notes, gz_path_state_flags};

/*
 * Define virtual path interface implementation for computing a path bbox.
 */
private
int gz_path_bbox_add_point(gx_path*, fixed, fixed),
    gz_path_bbox_add_line_notes(gx_path*, fixed, fixed, segment_notes),
    gz_path_bbox_add_curve_notes(gx_path*, fixed, fixed, fixed, fixed, fixed,
                                 fixed, segment_notes),
    gz_path_bbox_close_subpath_notes(gx_path*, segment_notes);

private
gx_path_procs path_bbox_procs = {
    gz_path_bbox_add_point, gz_path_bbox_add_line_notes,
    gz_path_bbox_add_curve_notes, gz_path_bbox_close_subpath_notes,
    gz_path_state_flags};
                    ...
                    client_name_t cname)
{
	...
	rc_alloc_struct_1(*ppsegs, gx_path_segments, &st_path_segments, mem,
                              ...
                              gs_memory_t* mem, client_name_t cname)
{
	if(shared) {
		if(shared->segments == &shared->local_segments) {
			lprintf1("Attempt to share (local) segments of path "
			         "0x%lx!\n",
			         (uint32_t)shared);
...
gx_path_assign_preserve(gx_path* ppto, gx_path* ppfrom)
{
	...
	if(fromsegs == &ppfrom->local_segments) {
		...
		if(tosegs == &ppto->local_segments || gx_path_is_shared(ppto)) {
			/* We can't use ppto's segments either.  Allocate a new
			 * one. */
...
private
void
rc_free_path_segments_local(gs_memory_t* mem, void* vpsegs, client_name_t cname)
{
...
#define path_unshare(ppath)                                                    \
	...
	if(gx_path_is_shared(ppath)) {                                         \
		int code_;                                                     \
		if((code_ = path_alloc_copy(ppath)) < 0)                       \
			return code_;                                          \
	}                                                                      \
	END
...
gx_path_new(gx_path* ppath)
{
	...
	if(gx_path_is_shared(ppath)) {
		...
		rc_free_path_segments_local(psegs->rc.memory, psegs,
		                            "gx_path_new");
	}
	gx_path_init_contents(ppath);
	return 0;
...
gx_path_add_relative_point(gx_path* ppath, fixed dx, fixed dy)
{
	if(!path_position_in_range(ppath))
		return_error((path_position_valid(ppath)
		                  ? gs_error_limitcheck
...
private
int
gz_path_bbox_add_line_notes(gx_path* ppath, fixed x, fixed y,
                            segment_notes notes)
{
                        ...
                        segment_notes notes)
{
	...
	if(lp != 0)
		ppath->position.x = lp->pt.x, ppath->position.y = lp->pt.y,
		psub->last = (segment*)lp, lp->next = 0,
		path_update_draw(ppath);
	return code;
}
...
int
gx_path_add_curve_notes(gx_path* ppath, fixed x1, fixed y1, fixed x2, fixed y2,
                        fixed x3, fixed y3, segment_notes notes)
{
	return ppath->procs->add_curve(ppath, x1, y1, x2, y2, x3, y3, notes);
}
private
int
gz_path_add_curve_notes(gx_path* ppath, fixed x1, fixed y1, fixed x2, fixed y2,
                        fixed x3, fixed y3, segment_notes notes)
{
	subpath* psub;
	curve_segment* lp;
...
private
int
gz_path_bbox_add_curve_notes(gx_path* ppath, fixed x1, fixed y1, fixed x2,
                             fixed y2, fixed x3, fixed y3, segment_notes notes)
{
 ...
 */
int
gx_path_add_partial_arc_notes(gx_path* ppath, fixed x3, fixed y3, fixed xt,
                              fixed yt, floatp fraction, segment_notes notes)
{
	...
	vd_curveto(x0 + (fixed)((xt - x0) * fraction),
	           ...
	           y3 + (fixed)((yt - y3) * fraction), x3, y3);
	return gx_path_add_curve_notes(
	    ppath, x0 + (fixed)((xt - x0) * fraction),
	    y0 + (fixed)((yt - y0) * fraction),
	    x3 + (fixed)((xt - x3) * fraction),
	    y3 + (fixed)((yt - y3) * fraction), x3, y3, notes | sn_from_arc);
}
...
gx_print_segment(const segment* pseg)
{
	...
	sprintf(out, "   0x%lx<0x%lx,0x%lx>:%u", (ulong)pseg, (ulong)pseg->prev,
	        (ulong)pseg->next, pseg->notes);
	switch(pseg->type) {
	case s_start: {
	...
	case s_curve: {
		...
		dprintf7("%s: %1.4f %1.4f %1.4f %1.4f %1.4f %1.4f curveto\n",
		         ...
		         fixed2float(pcur->p2.x), fixed2float(pcur->p2.y), px,
		         py);
		break;
	}
	case s_line:
