...
private
ENUM_PTRS_WITH(pdf_image_writer_enum_ptrs, pdf_image_writer* piw)
index -= 4;
if(index < psdf_binary_writer_max_ptrs * piw->alt_writer_count) {
	gs_ptr_type_t ret = ENUM_USING(
	    st_psdf_binary_writer,
	    &piw->binary[index / psdf_binary_writer_max_ptrs],
...
return 0;
case 0:
ENUM_RETURN(piw->pres);
case 1:
ENUM_RETURN(piw->data);
case 2:
ENUM_RETURN(piw->named);
case 3:
ENUM_RETURN(piw->pres_mask);
ENUM_PTRS_END
private
RELOC_PTRS_WITH(pdf_image_writer_reloc_ptrs, pdf_image_writer* piw)
{
	int i;
...
const pdf_image_names_t pdf_image_names_full = {
    {PDF_COLOR_SPACE_NAMES}, {PDF_FILTER_NAMES}, PDF_IMAGE_PARAM_NAMES};
const pdf_image_names_t pdf_image_names_short = {{PDF_COLOR_SPACE_NAMES_SHORT},
	...
	if(pcs) {
		...
		if(gs_color_space_get_index(pcs) ==
		   gs_color_space_index_Indexed) {
			indexed_decode[0] = 0;
			indexed_decode[1] =
			    (float)((1 << pim->BitsPerComponent) - 1);
	                             ...
	                             pim->BitsPerComponent));
	{
		...
		if(i < num_components * 2) {
			cos_array_t* pca = cos_array_alloc(
			    pdev, "pdf_put_pixel_image_values(decode)");

			if(pca == 0)
				return_error(gs_error_VMerror);
			if(pcs == NULL) {
				/* 269-01.ps sets /Decode[0 100] with a mask
				 * image. */
				for(i = 0; i < num_components * 2; ++i)
					CHECK(cos_array_add_real(
					    pca, min(pim->Decode[i], 1)));
			} else {
				for(i = 0; i < num_components * 2; ++i)
					CHECK(cos_array_add_real(
					    pca, pim->Decode[i]));
...
pdf_put_image_values(cos_dict_t* pcd, gx_device_pdf* pdev,
                     const gs_pixel_image_t* pic, const pdf_image_names_t* pin,
                     const cos_value_t* pcsvalue)
{
	...
	case 1: {
		...
		if(pim->ImageMask) {
			CHECK(cos_dict_put_c_strings(pcd, pin->ImageMask,
			                             "true"));
	...
	case 4: {
		...
		/* Masked images are only supported starting in PDF 1.3. */
		if(pdev->CompatibilityLevel < 1.3)
			break; /* Will convert into an imagemask with a pattern
			          color. */
		...
		for(i = 0; i < num_components; ++i) {
			...
			if(pim->MaskColor_is_range)
				lo = pim->MaskColor[i * 2],
				hi = pim->MaskColor[i * 2 + 1];
			else
 ...
 */
void
pdf_put_image_matrix(gx_device_pdf* pdev, const gs_matrix* pmat, floatp y_scale)
{
	gs_matrix imat;
...
int
pdf_do_image_by_id(gx_device_pdf* pdev, double scale, const gs_matrix* pimat,
                   bool in_contents, gs_id id)
{
             ...
             const gs_matrix* pimat, bool in_contents)
{
	...
	return pdf_do_image_by_id(pdev, scale, pimat, in_contents,
                      ...
                      bool in_line)
{
	...
	if(in_line) {
		...
		/*
		 * Note that if named != 0, there are two objects with the same
		 * id
		 * while the image is being accumulated: named, and
		 * pres->object.
		 */
	...
	pdev->strm =
	    cos_write_stream_alloc(data, pdev, "pdf_begin_write_image");
	if(pdev->strm == 0)
		return_error(gs_error_VMerror);
	if(!mask)
		piw->data = data;
	...
	code = psdf_begin_binary((gx_device_psdf*)pdev,
	                         &piw->binary[alt_stream_index]);
	piw->binary[alt_stream_index].target =
	    NULL; /* We don't need target with cos_write_stream. */
	pdev->strm = save_strm;
	return code;
}
                     ...
                     int alt_writer_index)
{
	...
	cos_stream_t* s =
	    cos_stream_from_pipeline(piw->binary[alt_writer_index].strm);
	cos_dict_t* pcd = cos_stream_dict(s);
	int code = pdf_put_image_values(pcd, pdev, pim, piw->pin, pcsvalue);

	if(code >= 0)
		code = pdf_put_image_filters(
		    pcd, pdev, &piw->binary[alt_writer_index], piw->pin);
	if(code < 0) {
		if(!piw->pres)
                        ...
                        int width, int bits_per_pixel)
{
	if(data_h != piw->height) {
		if(piw->binary[0].strm->procs.process ==
		       s_DCTE_template.process ||
		   piw->binary[0].strm->procs.process ==
		       s_PNGPE_template.process) {
			/* 	Since DCTE and PNGPE can't safely close with
			   incomplete data,
...
pdf_end_write_image(gx_device_pdf* pdev, pdf_image_writer* piw)
{
	...
	if(pres) { /* image resource */
		...
		if(named) {
			...
			/*
			 * This image was named by NI.  Copy any dictionary
			 * elements
			 * from the named dictionary to the image stream, and
			 * then
			...
			pres->named = true;
			/*
			 * We need to make the entry in the name dictionary
			 * point to
			 * the stream (pcs) rather than the object created by NI
			 * (named).
			 * Unfortunately, we no longer know what dictionary to
			 * use.
			 * Instead, overwrite the latter with the former's
			 * contents,
			 * and change the only relevant pointer.
			 */
			*(cos_object_t*)named = *pco;
			pres->object = COS_OBJECT(named);
		} else if(!pres->named) { /* named objects are written at the
		                             end */
			code = pdf_substitute_resource(
			    pdev, &piw->pres, resourceXObject, NULL, false);
			if(code < 0)
				return code;
			/*  Warning : If the substituted image used alternate
			   streams,
			    its space in the pdev->streams.strm file won't be
			   released. */
		...
		pdev->KeyLength =
		    0; /* Disable encryption for the inline image. */
...
int
pdf_copy_mask_bits(stream* s, const byte* base, int sourcex, int raster, int w,
                   int h, byte invert)
{
	int yi;
...
int
pdf_copy_color_bits(stream* s, const byte* base, int sourcex, int raster, int w,
                    int h, int bytes_per_pixel)
{
	int yi;
...
private
inline bool
much_bigger__DL(long l1, long l2)
{
	return l1 > 1024 * 1024 && l2 < l1 / 3;
}
private
void
pdf_choose_compression_cos(pdf_image_writer* piw, cos_stream_t* s[2],
                           bool force)
{ /*	Assume s[0] is Flate, s[1] is DCT, s[2] is chooser. */
	...
	if(force && l0 <= l1)
		...
		k0 = s_compr_chooser__get_choice(
		    (stream_compr_chooser_state*)piw->binary[2].strm->state,
		    force);
		if(k0 && l0 > 0 && l1 > 0)
			k0--;
	...
	s[k0]->cos_procs->release((cos_object_t*)s[k0],
	                          "pdf_image_choose_filter");
	s[k0]->written = 1;
...
pdf_choose_compression(pdf_image_writer* piw, bool end_binary)
{
	...
	if(end_binary) {
		...
		status = s_close_filters(&piw->binary[0].strm,
		                         piw->binary[0].target);
		if(status < 0)
			return status;
		status = s_close_filters(&piw->binary[1].strm,
		                         piw->binary[1].target);
		if(status < 0)
			return status;
	}
