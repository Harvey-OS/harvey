...
private
dev_proc_print_page(sj48_print_page);
gx_device_printer far_data gs_sj48_device =
    prn_device(prn_std_procs, "sj48", 80, /* width_10ths, 8" */
...
sj48_print_page(gx_device_printer* pdev, FILE* prn_stream)
{
	...
	int mode =
	    (yres == 180 ? (xres == 180 ? 39 : 40) : (xres == 180 ? 71 : 72));
	int bytes_per_column = (yres == 180) ? 3 : 6;
	int bits_per_column = bytes_per_column * 8;
	int skip_unit = bytes_per_column *
	                (xres == 180 ? 1 : 2); /* Skips in step of 1/180" */
	byte* in =
	    (byte*)gs_malloc(pdev->memory, 8, line_size, "sj48_print_page(in)");
	byte* out = (byte*)gs_malloc(pdev->memory, bits_per_column, line_size,
	                             "sj48_print_page(out)");
	int lnum = 0;
	int skip = 0;
	int skips;
	...
	fwrite("\033@\000\000", 1, 4,
	       prn_stream); /* <Printer reset>, <0>, <0>. */
	...
	while(lnum < last_row) {
		...
		/* them so badly that this code is faster. */
		{
			...
			for(; zcnt >= 4 * sizeof(long);
			    zip += 4, zcnt -= 4 * sizeof(long)) {
		...
		for(bnum = 0; bnum < bits_per_column; bnum += 8) {
			...
			byte* inp = in;
			byte* outp = outl;
			lcnt = gdev_prn_copy_scan_lines(pdev, lnum, in,
			                                lcnt * line_size);
			if(lcnt < 0) {
				code = lcnt;
				goto xit;
			...
			for(; inp < in_end; inp++, outp += bits_per_column) {
				gdev_prn_transpose_8x8(inp, line_size, outp,
				                       bytes_per_column);
			}
			outl++;
			lnum += lcnt;
		...
		do {
			...
			if(outl > out_beg) {
				...
				fprintf(prn_stream, "\033\\%c%c", count & 0xff,
				        count >> 8);
			}
			...
			count = outl - out_beg;
			{
				/* What to transmit is the number of columns in
				   the row.
				   Compare this with the <Esc>|*-command wich
				   expects the
				   total number of bytes in the graphic row! */
				int count1 = count / bytes_per_column;
				fprintf(prn_stream, "\033*%c%c%c", mode,
				        count1 & 0xff, count1 >> 8);
			}
		...
		fputc('\r', prn_stream);
		skip = bits_per_column; /* <CR> only moves to the beginning of
		                           the row. */
	}
...
fin:
	if(out != 0)
		gs_free(pdev->memory, (char*)out, bits_per_column, line_size,
		        "sj48_print_page(out)");
	if(in != 0)
		gs_free(pdev->memory, (char*)in, 8, line_size,
		        "sj48_print_page(in)");
	return code;
}
