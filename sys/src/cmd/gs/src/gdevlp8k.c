...
gx_device_printer far_data gs_lp8000_device = prn_device(
    prn_std_procs, "lp8000", DEFAULT_WIDTH_10THS, DEFAULT_HEIGHT_10THS, X_DPI,
    Y_DPI, L_MARGIN, B_MARGIN, R_MARGIN, T_MARGIN, 1, lp8000_print_page);
...
lp8000_print_page(gx_device_printer* pdev, FILE* prn_stream)
{
	...
	/* Calculate the PRINTER_LEFT_MARGIN = device_left_margin - 60 adjusted
	to
	...
	*/
	...
	width =
	    ((pdev->width - (int)(R_MARGIN * pdev->x_pixels_per_inch)) >> 3) -
	    left;
	...
	*/
...

	{
		...
		*/
		...
		for(p = inp, q = inp + 1; q < in_end;) {
			if(*p != *q++) {
				...
				*outp++ = *p++;
				...
				for(count = 2; (*p == *q) && (q < in_end);
				    q++, count++)
					;

				/*
				Copy repeated bytes and counts to the output
				buffer.
				As long as count is <= 255, additional step is
				necessary
		...
		/* The next line is necessary just in case of a single
		non-repeated byte at
