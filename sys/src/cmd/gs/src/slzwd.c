...
/* Copyright (C) 1993, 1996, 1998, 1999 Aladdin Enterprises.  All rights
  reserved.
...
s_LZWD_init(stream_state* st)
{
	stream_LZW_state* const ss = (stream_LZW_state*)st;
	lzw_decode* dc =
	    gs_alloc_struct_array(st->memory, lzw_decode_max + 1, lzw_decode,
	                          &st_lzw_decode_element, "LZWDecode(init)");
...
add: /* Add a new entry to the table */
	if(prev_code >= 0) {
		...
		if(next_code == lzw_decode_max) {
			/*
			 * A few anomalous files have one data item too many
			 * before the
			 * reset code.  We think this is a bug in the
			 * application that
			 * produced the files, but Acrobat accepts the files, so
			 * we do
			 * too.
			 */
			if(!ss->BlockData) { /* don't do this for GIF */
				...
				if(bits_left + ((rlimit - p) << 3) <
				   code_size) {
					/*
					 * We need more data to decide whether a
					 * reset is next.
					 * Return an error if we cannot get
					 * more.
					 */
					if(last)
						status = ERRC;
		...
		if(++next_code == switch_code) { /* Crossed a power of 2. */
			...
			if(next_code < lzw_decode_max - 1) {
				code_size++;
				set_code_size();
				if_debug2('w', "[w]crossed power of 2: new "
				               "code_size=%d, next_code=%d\n",
				          code_size, next_code);
			}
		}
	}
...
/* Stream template */
const stream_template s_LZWD_template = {&st_LZW_state, s_LZWD_init,
                                         s_LZWD_process, 3, 1, s_LZW_release,
