...
case 1:
return ENUM_OBJ(tptr->operation & TEXT_REPLACE_WIDTHS ? tptr->x_widths : NULL);
case 2:
return ENUM_OBJ(tptr->operation & TEXT_REPLACE_WIDTHS ? tptr->y_widths : NULL);
ENUM_PTRS_END
private
RELOC_PTRS_WITH(text_params_reloc_ptrs, gs_text_params_t* tptr)
{
	if(tptr->operation & TEXT_FROM_STRING) {
		gs_const_string str;
...
private
ENUM_PTRS_WITH(text_enum_enum_ptrs, gs_text_enum_t* eptr)
{
	if(index == 8) {
		if(eptr->pair != 0)
	...
	return ENUM_USING(st_gs_text_params, &eptr->text, sizeof(eptr->text),
	                  index);
}
case 0:
return ENUM_OBJ(gx_device_enum_ptr(eptr->dev));
case 1:
return ENUM_OBJ(gx_device_enum_ptr(eptr->imaging_dev));
ENUM_PTR3(2, gs_text_enum_t, pis, orig_font, path);
ENUM_PTR3(5, gs_text_enum_t, pdcolor, pcpath, current_font);
ENUM_PTRS_END

private
RELOC_PTRS_WITH(text_enum_reloc_ptrs, gs_text_enum_t* eptr)
{
	int i;
	...
	if(eptr->pair != NULL)
		eptr->pair =
		    (cached_fm_pair*)RELOC_OBJ(eptr->pair - eptr->pair->index) +
		    eptr->pair->index;
                     ...
                     gs_memory_t* mem, gs_text_enum_t** ppte)
{
	...
	{
		gx_path* tpath = ((text->operation & TEXT_DO_NONE) &&
		...
		return dev_proc(dev, text_begin)(dev, pis, text, font, tpath,
		                                 pdcolor, tcpath, mem, ppte);
	}
}
...
int
gs_text_begin(gs_state* pgs, const gs_text_params_t* text, gs_memory_t* mem,
              gs_text_enum_t** ppte)
{
	...
	if(code < 0)
		return code;
	return gx_device_text_begin(pgs->device, (gs_imager_state*)pgs, text,
	                            pgs->font, pgs->path, pgs->dev_color,
	                            pcpath, mem, ppte);
}
...
int
gs_show_begin(gs_state* pgs, const byte* str, uint size, gs_memory_t* mem,
              gs_text_enum_t** ppte)
{
	gs_text_params_t text;

	text.operation =
	    TEXT_FROM_STRING | text_do_draw(pgs) | TEXT_RETURN_WIDTH;
...
gs_widthshow_begin(gs_state* pgs, floatp cx, floatp cy, gs_char chr,
                   const byte* str, uint size, gs_memory_t* mem,
                   gs_text_enum_t** ppte)
{
	gs_text_params_t text;
...
int
gs_awidthshow_begin(gs_state* pgs, floatp cx, floatp cy, gs_char chr, floatp ax,
                    floatp ay, const byte* str, uint size, gs_memory_t* mem,
                    gs_text_enum_t** ppte)
{
	gs_text_params_t text;

	text.operation = TEXT_FROM_STRING | TEXT_ADD_TO_ALL_WIDTHS |
	                 TEXT_ADD_TO_SPACE_WIDTH | text_do_draw(pgs) |
	                 TEXT_RETURN_WIDTH;
	text.data.bytes = str, text.size = size;
	text.delta_space.x = cx;
	text.delta_space.y = cy;
...
int
gs_kshow_begin(gs_state* pgs, const byte* str, uint size, gs_memory_t* mem,
               gs_text_enum_t** ppte)
{
	gs_text_params_t text;
...
gs_xyshow_begin(gs_state* pgs, const byte* str, uint size,
                const float* x_widths, const float* y_widths, uint widths_size,
                gs_memory_t* mem, gs_text_enum_t** ppte)
{
	gs_text_params_t text;
...
int
gs_glyphshow_begin(gs_state* pgs, gs_glyph glyph, gs_memory_t* mem,
                   gs_text_enum_t** ppte)
{
	gs_text_params_t text;
	int result;
	...
	text.operation =
	    TEXT_FROM_SINGLE_GLYPH | text_do_draw(pgs) | TEXT_RETURN_WIDTH;
...
int
gs_cshow_begin(gs_state* pgs, const byte* str, uint size, gs_memory_t* mem,
               gs_text_enum_t** ppte)
{
	gs_text_params_t text;
                  ...
                  gs_memory_t* mem, gs_text_enum_t** ppte)
{
	...
	text.operation =
	    TEXT_FROM_STRING | TEXT_RETURN_WIDTH |
                     ...
                     bool stroke_path, gs_memory_t* mem, gs_text_enum_t** ppte)
{
	...
	text.operation = TEXT_FROM_STRING | TEXT_RETURN_WIDTH |
	                 (stroke_path ? TEXT_DO_TRUE_CHARBOXPATH
                   ...
                   gs_memory_t* mem, gs_text_enum_t** ppte)
{
	...
	text.operation =
	    TEXT_FROM_SINGLE_GLYPH | TEXT_RETURN_WIDTH |
	    (stroke_path ? TEXT_DO_TRUE_CHARPATH : TEXT_DO_FALSE_CHARPATH);
	text.data.d_glyph = glyph;
	text.size = 1;
	result = gs_text_begin(pgs, &text, mem, ppte);
...
int
gs_glyphwidth_begin(gs_state* pgs, gs_glyph glyph, gs_memory_t* mem,
                    gs_text_enum_t** ppte)
{
	gs_text_params_t text;
	int result;
	...
	text.operation =
	    TEXT_FROM_SINGLE_GLYPH | TEXT_DO_NONE | TEXT_RETURN_WIDTH;
	text.data.d_glyph = glyph;
	text.size = 1;
	result = gs_text_begin(pgs, &text, mem, ppte);
	if(result == 0)
