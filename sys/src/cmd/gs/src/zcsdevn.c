...
zsetdevicenspace(i_ctx_t* i_ctx_p)
{
	...
	/* Check tint transform procedure.  Note: Cheap trick to get pointer to
	   it.
	...
	/* Pick up the names of the components */
	{
		...
		for(i = 0; i < num_components; ++i) {
			...
			case t_string:
				code =
				    name_from_string(imemory, &sname, &sname);
				if(code < 0) {
					ifree_object(names,
...
const op_def zcsdevn_op_defs[] = {
    op_def_begin_ll3(), {"1.setdevicenspace", zsetdevicenspace}, op_def_end(0)};
