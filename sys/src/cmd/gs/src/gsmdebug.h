...
extern const byte gs_alloc_fill_alloc, /* allocated but not initialized */
...
extern void gs_alloc_memset(void*, int /*byte */, uint32_t);

#ifdef DEBUG
#define gs_alloc_fill(ptr, fill, len)                                          \
	BEGIN if(gs_alloc_debug) gs_alloc_memset(ptr, fill, (ulong)(len));     \
	END
#else
#define gs_alloc_fill(ptr, fill, len) DO_NOTHING
#endif
