...
#define private_st_C2R_state() /* in gdevpsds.c */                             \
	gs_private_st_ptrs1(st_C2R_state, stream_C2R_state,                    \
	                    "stream_C2R_state", c2r_enum_ptrs, c2r_reloc_ptrs, \
	                    pis)
...
#define private_st_IE_state() /* in gdevpsds.c */                              \
	gs_public_st_composite(st_IE_state, stream_IE_state,                   \
	                       "stream_IE_state", ie_state_enum_ptrs,          \
	                       ie_state_reloc_ptrs)
...
#define private_st_Average_state() /* in gdevpsds.c */                         \
	gs_private_st_ptrs1(st_Average_state, stream_Average_state,            \
	                    "stream_Average_state", avg_enum_ptrs,             \
	                    avg_reloc_ptrs, sums)
...
#define private_st_compr_chooser_state() /* in gdevpsds.c */                   \
	gs_private_st_ptrs1(                                                   \
	    st_compr_chooser_state, stream_compr_chooser_state,                \
	    "stream_compr_chooser_state", compr_chooser_enum_ptrs,             \
	    compr_chooser_reloc_ptrs, sample)

extern const stream_template s_compr_chooser_template;

/* Set image dimensions. */
int s_compr_chooser_set_dimensions(stream_compr_chooser_state* st, int width,
...
#define private_st_image_colors_state() /* in gdevpsds.c */                    \
	gs_private_st_ptrs3(                                                   \
	    st_stream_image_colors_state, stream_image_colors_state,           \
...
void s_image_colors_set_dimensions(stream_image_colors_state* st, int width,
                                   int height, int depth, int bits_per_sample);

void s_image_colors_set_mask_colors(stream_image_colors_state* ss,
                                    uint* MaskColor);

void s_image_colors_set_color_space(stream_image_colors_state* ss,
                                    gx_device* pdev, const gs_color_space* pcs,
