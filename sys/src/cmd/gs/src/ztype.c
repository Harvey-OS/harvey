...
/* Forward references */
private
int access_check(i_ctx_t*, int, bool);
private
int convert_to_string(const gs_memory_t* mem, os_ptr, os_ptr);
...
private
const double min_int_real = (ALT_MIN_LONG * 1.0 - 1);
private
const double max_int_real = (ALT_MAX_LONG * 1.0 + 1);

#define REAL_CAN_BE_INT(v) ((v) > min_int_real && (v) < max_int_real)
...
ztype(i_ctx_t* i_ctx_p)
{
	...
	if(!r_has_type(&tnref, t_name)) {
		...
		{ /* Get the type name from the structure. */
			const char* sname = gs_struct_type_name_string(
			    gs_object_type(imemory, op[-1].value.pstruct));
			int code = name_ref(imemory, (const byte*)sname,
			                    strlen(sname), (ref*)(op - 1), 0);
...
const op_def ztype_op_defs[] = {{"1cvi", zcvi},
                                {"1cvlit", zcvlit},
                                {"1cvn", zcvn},
                                {"1cvr", zcvr},
                                {"3cvrs", zcvrs},
...
private
int access_check(i_ctx_t* i_ctx_p, int access, /* mask for attrs */
