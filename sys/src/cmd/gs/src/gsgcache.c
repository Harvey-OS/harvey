...
gs_public_st_composite(st_glyph_cache_elem, gs_glyph_cache_elem,
                       "gs_glyph_cache_elem", gs_glyph_cache_elem_enum_ptrs,
                       gs_glyph_cache_elem_reloc_ptrs);
...
ENUM_PTRS_END
private
RELOC_PTRS_WITH(gs_glyph_cache_elem_reloc_ptrs, gs_glyph_cache_elem* e)
{
...
gs_private_st_ptrs4(st_glyph_cache, gs_glyph_cache, "gs_glyph_cache",
                    gs_glyph_cache_enum_ptrs, gs_glyph_cache_reloc_ptrs, list,
                    memory, pfont, s);
                      ...
                      get_glyph_data_from_file read_data)
{
	...
	gs_glyph_cache* gdcache = (gs_glyph_cache*)gs_alloc_struct(
	    mem, gs_glyph_cache, &st_glyph_cache, "gs_glyph_cache");
	if(gdcache == 0)
		return 0;
	...
	*/
	...
	gs_font_notify_register((gs_font*)pfont, gs_glyph_cache__release,
	                        (void*)gdcache);
	return gdcache;
}
...
gs_glyph_cache__release(void* data, void* event)
{
	...
	this->list = NULL;
	gs_font_notify_unregister((gs_font*)pfont, gs_glyph_cache__release,
	                          (void*)this);
...
private
inline void
gs_glyph_cache_elem__move_to_head(gs_glyph_cache* this,
                                  gs_glyph_cache_elem** pe)
{
...
gs_glyph_cache_elem__substring(gs_glyph_data_t* pgd, uint offset, uint size)
{
	...
	e->lock_count++;
	return_error(
	    gs_error_unregistered); /* Unsupported; should not happen. */
}
...
int
gs_get_glyph_data_cached(gs_font_type42* pfont, uint glyph_index,
                         gs_glyph_data_t* pgd)
{
	gs_glyph_cache* gdcache = pfont->data.gdcache;
	gs_glyph_cache_elem** pe =
	    gs_glyph_cache_elem__locate(gdcache, glyph_index);
	...
	if(pe == NULL || (*pe)->glyph_index != glyph_index) {
		...
		if(pe != NULL && gdcache->total_size > 32767 /* arbitrary */ &&
		   (*pe)->lock_count <= 0) {
			...
			e = (gs_glyph_cache_elem*)gs_alloc_struct(
			    gdcache->memory, gs_glyph_cache_elem,
			    &st_glyph_cache_elem, "gs_glyph_cache_elem");
			if(e == NULL)
		...
		code = (*gdcache->read_data)(pfont, gdcache->s, glyph_index,
		                             &e->gd);
		if(code < 0)
			return code;
		gdcache->total_size += e->gd.bits.size + sizeof(*e);
