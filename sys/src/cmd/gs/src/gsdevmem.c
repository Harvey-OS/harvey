...
gs_initialize_wordimagedevice(gx_device_memory* new_dev, const gs_matrix* pmat,
                              uint width, uint height, const byte* colors,
                              int colors_size, bool word_oriented,
                              bool page_device, gs_memory_t* mem)
{
	...
	proto_dev =
	    (word_oriented ? gdev_mem_word_device_for_bits(bits_per_pixel)
	...
	if(bits_per_pixel <= 8) {
		...
		has_color = false;
		for(i = 0, p = colors, q = palette; i < palette_count;
		    i++, q += 3) {
			int mask = 1;
...
#undef shift_mask
	...
	if(bits_per_pixel == 1) {
		...
		gdev_mem_mono_set_inverted(
		    new_dev, (palette[0] | palette[1] | palette[2]) != 0);
	} else {
		byte* dev_palette =
		    gs_alloc_string(mem, pcount, "gs_makeimagedevice(palette)");
...
int
gs_makewordimagedevice(gx_device** pnew_dev, const gs_matrix* pmat, uint width,
                       uint height, const byte* colors, int num_colors,
                       bool word_oriented, bool page_device, gs_memory_t* mem)
{
	...
	if(pnew == 0)
		return_error(gs_error_VMerror);
	code = gs_initialize_wordimagedevice(pnew, pmat, width, height, colors,
	                                     num_colors, word_oriented,
	                                     page_device, mem);
	if(code < 0) {
