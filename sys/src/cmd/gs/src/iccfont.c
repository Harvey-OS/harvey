...
/* Forward references */
private
int cfont_ref_from_string(i_ctx_t*, ref*, const char*, uint);
...
cfont_put_next(ref* pdict, key_enum* kep, const ref* pvalue)
{
	...
	if(kp->num_enc_keys) {
		...
		gs_glyph glyph =
		    gs_c_known_encode((gs_char)skp->charx, skp->encx);
		...
		if(glyph == GS_NO_GLYPH)
			...
			code =
			    name_ref(imemory, gstr.data, gstr.size, &kname, 0);
		kp->num_enc_keys--;
...
private
int
cfont_ref_dict_create(i_ctx_t* i_ctx_p, ref* pdict, const cfont_dict_keys* kp,
...
private
int
cfont_num_dict_create(i_ctx_t* i_ctx_p, ref* pdict, const cfont_dict_keys* kp,
                      cfont_string_array ksa, const ref* values,
                      const char* lengths)
{
	key_enum kenum;
	...
	while(more_keys(&kenum)) {
		...
		if(len == 0)
			...
			--len;
			make_const_array(&vnum, avm_foreign | a_readonly, len,
			                 vp);
			vp += len;
		}
...
private
int
cfont_string_array_create(i_ctx_t* i_ctx_p, ref* parray, cfont_string_array ksa,
                          int size, uint attrs)
{
	int code = ialloc_ref_array(parray, a_readonly, size,
...
private
int
cfont_scalar_array_create(i_ctx_t* i_ctx_p, ref* parray, const ref* va,
                          int size, uint attrs)
{
	int code =
	    ialloc_ref_array(parray, attrs, size, "cfont_scalar_array_create");
	ref* aptr = parray->value.refs;
...
private
const cfont_procs ccfont_procs = {
    cfont_ref_dict_create,     cfont_string_dict_create,
    cfont_num_dict_create,     cfont_name_array_create,
    cfont_string_array_create, cfont_scalar_array_create,
    cfont_name_create,         cfont_ref_from_string};
...
const op_def ccfonts_op_defs[] = {{"0.getccfont", zgetccfont}, op_def_end(0)};
