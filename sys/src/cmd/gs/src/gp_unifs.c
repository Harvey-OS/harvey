...
gs_private_st_ptrs1(st_dirstack, dirstack, "dirstack", dirstack_enum_ptrs,
                    dirstack_reloc_ptrs, next);
...
gs_private_st_ptrs3(st_file_enum, struct file_enum_s, "file_enum",
                    file_enum_enum_ptrs, file_enum_reloc_ptrs, pattern, work,
                    dstack);
...
gp_enumerate_files_init(const char* pat, uint patlen, gs_memory_t* mem)
{
	...
	pfen->pattern = (char*)gs_alloc_bytes(mem, patlen + 1,
...
top:
	...
	if(pathead < pfen->patlen) {
		...
		if(pfen->patlen ==
		   pathead + 1) { /* Listing "foo/?/" -- return this entry */
			/* if it's a directory. */
			if(!stat_is_dir(
			       stbuf)) { /* Do directoryp test the hard way */
				dp = opendir(work);
				if(!dp)
					goto top;
				closedir(dp);
		...
		if(!dp)
			...
			for(p = pattern + pathead + 1;; p++) {
				if(*p ==
				   0) { /* No more subdirectories to match */
					pathead = pfen->patlen;
					break;
				} else if(*p == '/') {
					pathead = p - pattern;
...
gp_enumerate_files_close(file_enum* pfen)
{
	...
	while(popdir(pfen)) /* clear directory stack */
		DO_NOTHING;
	gs_free_object(mem, (byte*)pfen->work, "gp_enumerate_close(work)");
