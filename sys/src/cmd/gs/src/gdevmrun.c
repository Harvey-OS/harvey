...
#define RP_TO_START(rp, data)                                                  \
	((rp).index = (data)[1].next, (rp).ptr = (data) + (rp).index)
...
private
dev_proc_copy_mono(run_copy_mono);
private
dev_proc_copy_color(run_copy_color);
private
dev_proc_fill_rectangle(run_fill_rectangle);
private
dev_proc_copy_alpha(run_copy_alpha);
private
dev_proc_strip_tile_rectangle(run_strip_tile_rectangle);
private
dev_proc_strip_copy_rop(run_strip_copy_rop);
private
dev_proc_get_bits_rectangle(run_get_bits_rectangle);
...
gdev_run_from_mem(gx_device_run* rdev, gx_device_memory* mdev)
{
	 ...
	 */
	int min_runs =
	    (mdev->width + (MAX_RUN_LENGTH - 1)) / MAX_RUN_LENGTH + 2;
	int i;
...
run_expand(gx_device_run* rdev, int y)
{
	...
	if(line->zero != gx_no_color_index) {
		rdev->save_procs.fill_rectangle((gx_device*)&rdev->md, 0, y,
	...
	for(x = 0, n = 0; x < rdev->md.width; x += w, ++n) {
		w = length[n];
		rdev->save_procs.fill_rectangle((gx_device*)&rdev->md, x, y, w,
		                                1, value[n]);
	}
	return 0;
}
              ...
              gx_color_index one)
{
	...
	return rdev->save_procs.copy_mono((gx_device*)&rdev->md, data, dx,
	                                  raster, id, x, y, w, h, zero, one);
}
private
int
run_copy_color(gx_device* dev, const byte* data, int data_x, int raster,
               gx_bitmap_id id, int x, int y, int w, int h)
{
	gx_device_run* const rdev = (gx_device_run*)dev;

	run_standardize(rdev, y, h);
	return rdev->save_procs.copy_color((gx_device*)&rdev->md, data, data_x,
	                                   raster, id, x, y, w, h);
}
private
int
               ...
               gx_color_index color, int depth)
{
	...
	run_standardize(rdev, y, h);
	return rdev->save_procs.copy_alpha((gx_device*)&rdev->md, data, data_x,
	                                   raster, id, x, y, w, h, color,
	                                   depth);
}
private
int
run_strip_tile_rectangle(gx_device* dev, const gx_strip_bitmap* tiles, int x,
                         ...
                         gx_color_index color1, int px, int py)
{
	...
	run_standardize(rdev, y, h);
	return rdev->save_procs.strip_tile_rectangle(
	    (gx_device*)&rdev->md, tiles, x, y, w, h, color0, color1, px, py);
}
private
int
run_strip_copy_rop(gx_device* dev, const byte* sdata, int sourcex, uint sraster,
                   gx_bitmap_id id, const gx_color_index* scolors,
                   const gx_strip_bitmap* textures,
                   const gx_color_index* tcolors, int x, int y, int w, int h,
                   int px, int py, gs_logical_operation_t lop)
{
	gx_device_run* const rdev = (gx_device_run*)dev;

	run_standardize(rdev, y, h);
	return rdev->save_procs.strip_copy_rop(
	    (gx_device*)&rdev->md, sdata, sourcex, sraster, id, scolors,
	    textures, tcolors, x, y, w, h, px, py, lop);
}
private
int
                       ...
                       gs_get_bits_params_t* params, gs_int_rect** unread)
{
	...
	return rdev->save_procs.get_bits_rectangle((gx_device*)&rdev->md, prect,
	                                           params, unread);
}
...
run_fill_interval(run_line* line, int xo, int xe, run_value new)
{
	...
	if(x0 < xo) {
		...
		if(RP_VALUE(rpp) == v0 &&
		   RP_LENGTH(rpp) + diff <= MAX_RUN_LENGTH)
			RP_LENGTH(rpp) += diff;
		else {
	...
	if(x1 > xe) {
		...
		if(RP_VALUE(rpn) == vc &&
		   RP_LENGTH(rpn) + diff <= MAX_RUN_LENGTH)
			RP_LENGTH(rpn) += diff;
		else {
...

	{
		...
		if(xo == x0 &&
		   RP_VALUE(rp0) ==
		       new&& RP_LENGTH(rp0) + left <= MAX_RUN_LENGTH)
			RP_LENGTH(rp0) += left;
		else {
			/*
			 * If we need more than one run, we divide up the length
			 * to
			 * create more runs with length less than MAX_RUN_LENGTH
			 * in
			 * order to improve the chances of a later merge.
			 * However,
			 * we still guarantee that we won't create more runs
			 * than
			...
			if(left <= MAX_RUN_LENGTH)
				...
				/*len = ceil(left / ceil(left /
				 * MAX_RUN_LENGTH))*/
				int pieces =
				    left +
				    (MAX_RUN_LENGTH - 1) / MAX_RUN_LENGTH;
			...
			do {
				...
				/*
				 * The allocation in rp_insert_next can't fail,
				 * because
				 * we just deleted at least as many runs as
				 * we're going
				 * to insert.
				 */
	...
	for(iy = y; iy < ye; ++iy) {
		...
		if(color != line->zero) {
			...
			if(run_fill_interval(line, x, xe, color) < 0) {
				/* We ran out of runs.  Convert to expanded
				 * form. */
				run_standardize(rdev, iy, 1);
				rdev->save_procs.fill_rectangle(
				    (gx_device*)&rdev->md, x, iy, w, 1, color);
			}
		}
	}
	return 0;
