                      ...
                      const stream_template* template, stream_state* st)
{
	...
	if(r_has_type(op, t_dictionary)) {
		if((code = dict_int_param(op, "Predictor", 0, 15, 1,
		                          &predictor)) < 0)
			return code;
		switch(predictor) {
		case 0: /* identity */
		...
		return filter_read(i_ctx_p, npop, template, st, 0);
	{
		...
		op = osp;
		code = (predictor == 2
		            ? filter_read(i_ctx_p, 0, &s_PDiffD_template,
		                          (stream_state*)&pds, 0)
		            : filter_read(i_ctx_p, 0, &s_PNGPD_template,
		                          (stream_state*)&pps, 0));
		if(code < 0) {
			/* Restore the operands.  Don't bother trying to clean
			 * up */
			/* the first stream. */
			osp = ++op;
...
zlz_setup(os_ptr op, stream_LZW_state* plzs)
{
	...
	if((code = dict_int_param(dop, "EarlyChange", 0, 1, 1,
	   ...
	   (code = dict_bool_param(dop, "BlockData", false, &plzs->BlockData)) <
	       0)
		return code;
	return 0;
}
...
zLZWD(i_ctx_t* i_ctx_p)
{
	...
	if(LL3_ENABLED && r_has_type(op, t_dictionary)) {
		...
		if((code =
		        dict_bool_param(op, "LowBitFirst", lzs.FirstBitLowOrder,
...
zpd_setup(os_ptr op, stream_PDiff_state* ppds)
{
	...
	if((code = dict_int_param(op, "Colors", 1, s_PDiff_max_Colors, 1,
	                          &ppds->Colors)) < 0 ||
	   (code = dict_int_param(op, "BitsPerComponent", 1, 16, 8, &bpc)) <
	       0 ||
	   (bpc & (bpc - 1)) != 0 ||
...
zPDiffE(i_ctx_t* i_ctx_p)
{
	...
	if(code < 0)
		return code;
	return filter_write(i_ctx_p, 0, &s_PDiffE_template, (stream_state*)&pds,
	                    0);
}
...
zPDiffD(i_ctx_t* i_ctx_p)
{
	...
	if(code < 0)
		return code;
	return filter_read(i_ctx_p, 0, &s_PDiffD_template, (stream_state*)&pds,
	                   0);
}
...
zpp_setup(os_ptr op, stream_PNGP_state* ppps)
{
	...
	check_type(*op, t_dictionary);
	check_dict_read(*op);
	if((code = dict_int_param(op, "Colors", 1, 16, 1, &ppps->Colors)) < 0 ||
	   (code = dict_int_param(op, "BitsPerComponent", 1, 16, 8, &bpc)) <
	       0 ||
	   (bpc & (bpc - 1)) != 0 ||
...
zPNGPE(i_ctx_t* i_ctx_p)
{
	...
	if(code < 0)
		return code;
	return filter_write(i_ctx_p, 0, &s_PNGPE_template, (stream_state*)&pps,
	                    0);
}
...
zPNGPD(i_ctx_t* i_ctx_p)
{
	...
	if(code < 0)
		return code;
	return filter_read(i_ctx_p, 0, &s_PNGPD_template, (stream_state*)&pps,
	                   0);
}

/* ---------------- Initialization procedure ---------------- */

const op_def zfdecode_op_defs[] = {op_def_begin_filter(),
