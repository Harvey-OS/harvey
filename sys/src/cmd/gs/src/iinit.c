...
/* Copyright (C) 1989, 1995, 1997, 1998, 1999, 2000 Aladdin Enterprises.  All
  rights reserved.
...
const char* const gs_error_names[] = {ERROR_NAMES};

/* The operator tables */
op_array_table op_array_table_global,
    op_array_table_local; /* definitions of `operator' procedures */
...
obj_init(i_ctx_t** pi_ctx_p, gs_dual_memory_t* idmem)
{
	...
	code = dict_alloc(idmem->space_global,
	                  (level >= 3 ? SYSTEMDICT_LL3_SIZE
	                              : level >= 2 ? SYSTEMDICT_LEVEL2_SIZE
	                                           : SYSTEMDICT_SIZE),
	                  &system_dict);
	if(code < 0)
		return code;
...
#define icount countof(initial_dictionaries)
		...
		if(level >= 2) {
			dsp += 2;
			/*
			 * For the moment, let globaldict be an alias for
			 * systemdict.
			 */
			dsp[-1] = system_dict;
			min_dstack_size++;
		...
		*dsp = system_dict;
		...
		for(i = 0; i < countof(initial_dstack); i++) {
			...
			ref_assign(dsp,
			           make_initial_dict(i_ctx_p, dname, idicts));
		}

		/* Enter names of referenced initial dictionaries into
		 * systemdict. */
		...
		for(i = 0; i < icount; i++) {
			...
			if(!r_has_type(idict, t_null)) {
				/*
				 * Note that we enter the dictionary in
				 * systemdict
				 * even if it is in local VM.  There is a
				 * special
				...
				code = initial_enter_name(
				    initial_dictionaries[i].name, idict);
...
#undef icount
	...
	/* Create the error name table */
	{
		...
		if(code < 0)
			return code;
		for(i = 0; i < n; i++)
			if((code = name_enter_string(
			        imemory, (const char*)gs_error_names[i],
			        era.value.refs + i)) < 0)
				return code;
                     ...
                     op_array_table* opt)
{
	...
	code =
	    ialloc_ref_array(&opt->table, a_readonly, size, "op_array table");
	ialloc_set_space(idmemory, save_space);
	if(code < 0)
		return code;
	refset_null(opt->table.value.refs, size);
	opt->nx_table = (ushort*)ialloc_byte_array(size, sizeof(ushort),
	                                           "op_array nx_table");
	if(opt->nx_table == 0)
		return_error(e_VMerror);
	opt->count = 0;
...
op_init(i_ctx_t* i_ctx_p)
{
	...
	for(tptr = op_defs_all; *tptr != 0; tptr++) {
		...
		for(def = *tptr; (nstr = def->oname) != 0; def++)
			if(op_def_is_begin_dict(def)) {
				...
				code = name_ref(imemory, (const byte*)nstr,
				                strlen(nstr), &nref, -1);
				if(code < 0)
					return code;
				...
				uint opidx =
				    (tptr - op_defs_all) * OP_DEFS_MAX_SIZE +
				    index_in_table;
				...
				/* giving the minimum acceptable number of
				 * operands. */
				...
				nstr++;
				/*
				 * Skip internal operators, and the second
				 * occurrence of
				 * operators with special indices.
				 */
				if(*nstr != '%' && r_size(&oper) == opidx) {
					code = i_initial_enter_name_in(
					    i_ctx_p, pdict, nstr, &oper);
					if(code < 0)
						return code;
				}
			}
	...
	if((code =
	        alloc_op_array_table(i_ctx_p, OP_ARRAY_TABLE_GLOBAL_SIZE,
