...
/* Copyright (C) 1994, 1996, 1998, 1999 Aladdin Enterprises.  All rights
  reserved.
...
declare_mem_procs(mem_true16_copy_mono, mem_true16_copy_color,
                  mem_true16_fill_rectangle);

/* The device descriptor. */
const gx_device_memory mem_true16_device = mem_device(
    "image16", 16, 0, mem_true16_map_rgb_color, mem_true16_map_color_rgb,
                         ...
                         gx_color_value prgb[3])
{
	...
	prgb[0] =
	    ((value << 11) + (value << 6) + (value << 1) + (value >> 4)) >>
...
private
int
mem_true16_fill_rectangle(gx_device* dev, int x, int y, int w, int h,
                          gx_color_index color)
{
	...
	if(w == 1) {
		...
		bytes_fill_rectangle(scan_line_base(mdev, y) + (x << 1),
		                     draster, (byte)color, w << 1, h);
	} else {
		while(h-- > 0) {
...
private
int
mem_true16_copy_mono(gx_device* dev, const byte* base, int sourcex, int sraster,
...
private
int
mem_true16_copy_color(gx_device* dev, const byte* base, int sourcex,
