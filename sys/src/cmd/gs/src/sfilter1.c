...
top:
	...
	case 1: /* text data */
		...
		{
			int count =
			    (wcount < rcount ? (status = 1, wcount) : rcount);
...
/* Stream template */
const stream_template s_PFBD_template = {&st_PFBD_state, s_PFBD_init,
                                         s_PFBD_process, 6, 2};
...
private
int
s_SFD_process(stream_state* st, stream_cursor_read* pr, stream_cursor_write* pw,
              bool last)
{
	stream_SFD_state* const ss = (stream_SFD_state*)st;
	register const byte* p = pr->ptr;
		...
		while(p < rlimit) {
			...
			if(c == pattern[match]) {
				if(++match == ss->eod.size) {
					...
					/*
					 * We use if/else rather than switch
					 * because the value
					 * is long, which is not supported as a
					 * switch value in
					 * pre-ANSI C.
					 */
					if(ss->count <= 0) {
