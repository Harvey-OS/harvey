			...
			gs_color_select_t select)
{
	if(pis->alpha == gx_max_color_value)
		...
		(*pis->cmap_procs->map_rgb_alpha)(pconc[0], pconc[0], pconc[0], cv2frac(pis->alpha),
						  pdc, pis, dev, select);
	return 0;
}
int
...
gx_default_gray_map_color_rgb(gx_device *dev, gx_color_index color,
			      gx_color_value prgb[3])
{
	gx_color_value gray = (gx_color_value)(color * gx_max_color_value / dev->color_info.max_gray);
...
#define CV_BIT(v) ((v) >> (gx_color_value_bits - 1))
	return (gx_color_index)(CV_BIT(cv[3]) + (CV_BIT(cv[2]) << 1) + (CV_BIT(cv[1]) << 2) + (CV_BIT(cv[0]) << 3));
#undef CV_BIT
}
