...
ENUM_PTRS_END
private
RELOC_PTRS_WITH(device_color_reloc_ptrs, gx_device_color* cptr)
{
...
gx_default_encode_color(gx_device* dev, const gx_color_value cv[])
{
	...
	if(dev->color_info.separable_and_linear != GX_CINFO_SEP_LIN) {
		dprintf("gx_default_encode_color() requires separable and "
		        "linear\n");
		return gx_no_color_index;
	}
#endif
	for(i = 0; i < ncomps; i++) {
		color |= (gx_color_index)(cv[i] >>
		                          (gx_color_value_bits - comp_bits[i]))
		         << comp_shift[i];
	}
	return color;
}
 ...
 */
int
gx_default_decode_color(gx_device* dev, gx_color_index color,
                        gx_color_value cv[])
{
	int ncomps = dev->color_info.num_components;
	int i;
...
#ifdef DEBUG
	if(dev->color_info.separable_and_linear != GX_CINFO_SEP_LIN) {
		dprintf("gx_default_decode_color() requires separable and "
		        "linear\n");
	...
	for(i = 0; i < ncomps; i++) {
		 ...
		 * Split the conversion into an integer and a fraction
		 * calculation
		 * so we can do integer arthmetic.  The calculation is
		 * equivalent
...
int
gx_error_decode_color(gx_device* dev, gx_color_index cindex,
...
{
	return cv[0] * (dev->color_info.max_gray + 1) /
	       (gx_max_color_value + 1);
}
 ...
 **/
gx_color_index
gx_backwards_compatible_gray_encode(gx_device* dev, const gx_color_value cv[])
{
...
static void
rgb_cs_to_gray_cm(gx_device* dev, const gs_imager_state* pis, frac r, frac g,
                  frac b, frac out[])
{
...
void
rgb_cs_to_rgb_cm(gx_device* dev, const gs_imager_state* pis, frac r, frac g,
                 frac b, frac out[])
{
	out[0] = r;
	out[1] = g;
...
static void
rgb_cs_to_rgbk_cm(gx_device* dev, const gs_imager_state* pis, frac r, frac g,
                  frac b, frac out[])
{
	if((r == g) && (g == b)) {
		out[0] = out[1] = out[2] = frac_0;
 ...
 */
static void
rgb_cs_to_cmyk_cm(gx_device* dev, const gs_imager_state* pis, frac r, frac g,
                  frac b, frac out[])
{
	if(pis != 0)
...
{
/*
 * We should never get here.  If we do then we do not have a
...
#define compare_color_names(pname, name_size, name_str)                        \
	(name_size == (int)strlen(name_str) &&                                 \
	 strncmp(pname, name_str, name_size) == 0)
...
int
gx_error_get_color_comp_index(gx_device* dev, const char* pname, int name_size,
                              int component_type)
{
/*
 * We should never get here.  If we do then we do not have a
...
private
cmap_proc_gray(cmap_gray_halftoned);
private
cmap_proc_gray(cmap_gray_direct);

private
cmap_proc_rgb(cmap_rgb_halftoned);
private
cmap_proc_rgb(cmap_rgb_direct);

#define cmap_cmyk_halftoned cmap_cmyk_direct
private
cmap_proc_cmyk(cmap_cmyk_direct);

private
cmap_proc_rgb_alpha(cmap_rgb_alpha_halftoned);
private
cmap_proc_rgb_alpha(cmap_rgb_alpha_direct);
...
private
cmap_proc_separation(cmap_separation_halftoned);
private
cmap_proc_separation(cmap_separation_direct);

private
cmap_proc_devicen(cmap_devicen_halftoned);
private
cmap_proc_devicen(cmap_devicen_direct);

private
cmap_proc_is_halftoned(cmap_halftoned_is_halftoned);
private
cmap_proc_is_halftoned(cmap_direct_is_halftoned);

private
const gx_color_map_procs cmap_few = {
    cmap_gray_halftoned,        cmap_rgb_halftoned,
    cmap_cmyk_halftoned,        cmap_rgb_alpha_halftoned,
    cmap_separation_halftoned,  cmap_devicen_halftoned,
    cmap_halftoned_is_halftoned};
private
const gx_color_map_procs cmap_many = {
    cmap_gray_direct,        cmap_rgb_direct,        cmap_cmyk_direct,
    cmap_rgb_alpha_direct,   cmap_separation_direct, cmap_devicen_direct,
                       ...
                       gx_device* dev, gs_color_select_t select)
{
	...
	code = (*pconcs->type->remap_concrete_color)(conc, pconcs, pdc, pis,
	                                             dev, select);
                        ...
                        gx_device* dev, gs_color_select_t select)
{
	if(pis->alpha == gx_max_color_value)
		...
		(*pis->cmap_procs->map_rgb_alpha)(pconc[0], pconc[0], pconc[0],
		                                  cv2frac(pis->alpha), pdc, pis,
		                                  dev, select);
	return 0;
}
int
                    ...
                    gx_device* dev, gs_color_select_t select)
{
	...
	if(pis->alpha == gx_max_color_value)
		(*pis->cmap_procs->map_gray)(fgray, pdc, pis, dev, select);
	else
		(*pis->cmap_procs->map_rgb_alpha)(fgray, fgray, fgray,
		                                  cv2frac(pis->alpha), pdc, pis,
		                                  dev, select);
	return 0;
}
...
{
	if(pis->alpha == gx_max_color_value)
		gx_remap_concrete_rgb(pconc[0], pconc[1], pconc[2], pdc, pis,
		                      dev, select);
	else
		gx_remap_concrete_rgb_alpha(pconc[0], pconc[1], pconc[2],
		                            cv2frac(pis->alpha), pdc, pis, dev,
		                            select);
	return 0;
}
int
...
{
	float ftemp;
	frac fred = unit_frac(pc->paint.values[0], ftemp),
	     fgreen = unit_frac(pc->paint.values[1], ftemp),
	...
	if(pis->alpha == gx_max_color_value)
		gx_remap_concrete_rgb(fred, fgreen, fblue, pdc, pis, dev,
		                      select);
	else
		gx_remap_concrete_rgb_alpha(fred, fgreen, fblue,
		                            cv2frac(pis->alpha), pdc, pis, dev,
		                            select);
	return 0;
}
...
{
	/****** IGNORE alpha ******/
	gx_remap_concrete_cmyk(pconc[0], pconc[1], pconc[2], pconc[3], pdc, pis,
	                       dev, select);
	return 0;
}
int
                    ...
                    gx_device* dev, gs_color_select_t select)
{
	...
	gx_remap_concrete_cmyk((frac)unit_frac(pc->paint.values[0], ft0),
	                       ...
	                       (frac)unit_frac(pc->paint.values[3], ft3), pdc,
	                       pis, dev, select);
	return 0;
}
...
private
void
cmap_gray_halftoned(frac gray, gx_device_color* pdc, const gs_imager_state* pis,
                    gx_device* dev, gs_color_select_t select)
{
	...
	if(dev->color_info.polarity == GX_CINFO_POLARITY_ADDITIVE)
		for(i = 0; i < ncomps; i++)
			cm_comps[i] = gx_map_color_frac(pis, cm_comps[i],
                          ...
                          gs_color_select_t select)
{
	...
	if(pis->color_component_map.sep_type == SEP_ALL) {
		...
		/* map to the color model */
		map_components_to_colorants(&all, &(pis->color_component_map),
		                            cm_comps);
	}
...
private
void
cmap_separation_direct(frac all, gx_device_color* pdc,
                       const gs_imager_state* pis, gx_device* dev,
                       gs_color_select_t select)
{
	int i, ncomps = dev->color_info.num_components;
...
private
void
cmap_devicen_halftoned(const frac* pcc, gx_device_color* pdc,
                       const gs_imager_state* pis, gx_device* dev,
                       gs_color_select_t select)
{
	int i, ncomps = dev->color_info.num_components;
...
private
void
cmap_devicen_direct(const frac* pcc, gx_device_color* pdc,
                    const gs_imager_state* pis, gx_device* dev,
                    gs_color_select_t select)
{
	int i, ncomps = dev->color_info.num_components;
...
#define cp_frac_bits (frac_bits - log2_transfer_map_size)
	...
	if(mdv < -1 << (16 - cp_frac_bits) || mdv > 1 << (16 - cp_frac_bits))
...
{ /* We round the value rather than truncating it. */
	gx_color_value gray =
	    (((cv[0] * (uint32_t)lum_red_weight) +
	      ...
	      (cv[2] * (uint32_t)lum_blue_weight) + (lum_all_weights / 2)) /
	         lum_all_weights * dev->color_info.max_gray +
...
gx_default_gray_map_color_rgb(gx_device* dev, gx_color_index color,
                              gx_color_value prgb[3])
{
	gx_color_value gray = (gx_color_value)(color * gx_max_color_value /
                             ...
                             gx_color_value prgb[3])
{
	if(dev->color_info.depth == 24) {
		...
		prgb[1] = ((color >> (bits_per_color)) & color_mask) *
		          (uint32_t)gx_max_color_value / color_mask;
		prgb[2] = (color & color_mask) * (uint32_t)gx_max_color_value /
		          color_mask;
	}
	return 0;
}
...
{ /* Convert to RGB */
	frac rgb[3];
	gx_color_value rgb_cv[3];
	color_cmyk_to_rgb(cv2frac(cv[0]), cv2frac(cv[1]), cv2frac(cv[2]),
	                  cv2frac(cv[3]), NULL, rgb);
...
#define CV_BIT(v) ((v) >> (gx_color_value_bits - 1))
	return (gx_color_index)(CV_BIT(cv[3]) + (CV_BIT(cv[2]) << 1) +
	                        (CV_BIT(cv[1]) << 2) + (CV_BIT(cv[0]) << 3));
#undef CV_BIT
}
...
{
	gx_color_index color = gx_color_value_to_byte(cv[3]) +
...
cmyk_8bit_map_color_rgb(gx_device* dev, gx_color_index color,
                        gx_color_value prgb[3])
{
	int not_k = (int)(~color & 0xff), r = not_k - (int)(color >> 24),
...
gx_color_index
gx_default_map_rgb_alpha_color(gx_device* dev, gx_color_value r,
