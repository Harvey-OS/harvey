...
/* Procedure record */
private
dev_proc_map_rgb_color(ega0_map_rgb_color);
private
dev_proc_map_rgb_color(ega1_map_rgb_color);
#define ega2_map_rgb_color pc_4bit_map_rgb_color
private
dev_proc_map_color_rgb(ega01_map_color_rgb);
...
#define ega_std_procs(get_params, put_params)                                  \
	ega_open, NULL, /* get_initial_matrix */                               \
	    NULL,       /* sync_output */                                      \
	    NULL,       /* output_page */                                      \
	    ega_close, ega_map_rgb_color, ega_map_color_rgb,                   \
	    ega_fill_rectangle, ega_tile_rectangle, ega_copy_mono,             \
	    ega_copy_color, NULL,                       /* draw_line */        \
	    ega_get_bits, get_params, put_params, NULL, /* map_cmyk_color */   \
...
private
const gx_device_procs ega_procs = {ega_std_procs(NULL, NULL)};

private
dev_proc_get_params(svga16_get_params);
private
dev_proc_put_params(svga16_put_params);
private
const gx_device_procs svga16_procs = {
...
/* Save the BIOS state */
private
pcfb_bios_state pcfb_save_state = {-1};
...
svga16_put_params(gx_device* dev, gs_param_list* plist)
{
	...
	switch(code = param_read_int(plist, (param_name = "DisplayMode"),
	                             &imode)) {
	default:
		ecode = code;
...
#if USE_ASM
void
    memrwcol2(rop_ptr); /* dest, draster, src, sraster, height, shift, invert */
#else
...
private
void fill_rectangle(rop_ptr, int, int, int);
private
void fill_row_only(byte*, int, int, int);

/* Clean up after writing */
#define dot_end() set_g_mask(0xff) /* all bits on */
...
int
ega_copy_mono(gx_device* dev, const byte* base, int sourcex, int raster,
              gx_bitmap_id id, int x, int y, int w, int h, gx_color_index izero,
              gx_color_index ione)
{
	rop_params params;
...
copy:
	set_g_mask(mask);
	if(params.shift ==
	   0) { /* optimize the aligned case */ /* Do left column */
...
fin:
	if(other_color != -1) {
		if(other_color >=
		   0) { /* Do the second pass on (T, other) or (other, T). */
			count = w - dleft;
			params.src = base + (sourcex >> 3);
			params.dest = save_dest;
			params.invert ^= -1;
...
int
ega_copy_color(gx_device* dev, const byte* base, int sourcex, int raster,
...
int
ega_tile_rectangle(gx_device* dev, const gx_tile_bitmap* tile, int x, int y,
...
#define one (int) cone
{
	...
	if((tile->size.x & 7) || one == -1 || zero == -1 || px || py)
		return gx_default_tile_rectangle(dev, tile, x, y, w, h, czero,
		                                 cone, px, py);
...
#define spread4(v)                                                             \
	...
	static const bits32 spread8[256] = {
	    spread4(0x0000), spread4(0x0800), spread4(0x8000), spread4(0x8800),
	    spread4(0x0008), spread4(0x0808), spread4(0x8008), spread4(0x8808),
	    spread4(0x0080), spread4(0x0880), spread4(0x8080), spread4(0x8880),
	    spread4(0x0088), spread4(0x0888), spread4(0x8088), spread4(0x8888)};
...
static const byte rmask_tab[9] = {0x00, 0x80, 0xc0, 0xe0, 0xf0,
                                  0xf8, 0xfc, 0xfe, 0xff};
