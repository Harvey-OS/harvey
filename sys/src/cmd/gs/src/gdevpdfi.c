...
/* Forward references */
private
image_enum_proc_plane_data(pdf_image_plane_data);
private
image_enum_proc_end_image(pdf_image_end_image);
private
image_enum_proc_end_image(pdf_image_end_image_object);
private
image_enum_proc_end_image(pdf_image_end_image_object2);
private
image_enum_proc_end_image(pdf_image_end_image_cvd);
private
IMAGE3_MAKE_MID_PROC(pdf_image3_make_mid);
private
IMAGE3_MAKE_MCDE_PROC(pdf_image3_make_mcde);
private
IMAGE3X_MAKE_MID_PROC(pdf_image3x_make_mid);
private
IMAGE3X_MAKE_MCDE_PROC(pdf_image3x_make_mcde);

private
const gx_image_enum_procs_t pdf_image_enum_procs = {pdf_image_plane_data,
                                                    pdf_image_end_image};
private
const gx_image_enum_procs_t pdf_image_object_enum_procs = {
    pdf_image_plane_data, pdf_image_end_image_object};
private
const gx_image_enum_procs_t pdf_image_object_enum_procs2 = {
    pdf_image_plane_data, pdf_image_end_image_object2};
private
const gx_image_enum_procs_t pdf_image_cvd_enum_procs = {
    gx_image1_plane_data, pdf_image_end_image_cvd, gx_image1_flush};
...
/* GC procedures */
private
ENUM_PTRS_WITH(pdf_image_enum_enum_ptrs, pdf_image_enum* pie)
if(index < pdf_image_writer_max_ptrs) {
	gs_ptr_type_t ret = ENUM_USING(st_pdf_image_writer, &pie->writer,
	                               sizeof(pie->writer), index);

	if(ret == 0) /* don't stop early */
		ENUM_RETURN(0);
	return ret;
}
return ENUM_USING_PREFIX(st_gx_image_enum_common, pdf_image_writer_max_ptrs);
ENUM_PTRS_END
private
RELOC_PTRS_WITH(pdf_image_enum_reloc_ptrs, pdf_image_enum* pie)
{
...
{
	return (!color_is_pure(pdcolor)
	            ? -1
	            : gx_dc_pure_color(pdcolor) == gx_device_black(dev)
	                  ? 0
	                  : gx_dc_pure_color(pdcolor) == gx_device_white(dev)
	                        ? 1
	                        : -1);
}
private
int
pdf_convert_image4_to_image1(gx_device_pdf* pdev, const gs_imager_state* pis,
                             const gx_drawing_color* pbcolor,
                             const gs_image4_t* pim4, gs_image_t* pim1,
                             gx_drawing_color* pdcolor)
{
	if(pim4->BitsPerComponent == 1 &&
	   (pim4->MaskColor_is_range ? pim4->MaskColor[0] | pim4->MaskColor[1]
	                             : pim4->MaskColor[0]) <= 1) {
		...
		/*
		 * Prepare the drawing color.  (pdf_prepare_imagemask will set
		 * it.)
		...
		/*
		 * The PDF imaging model doesn't support RasterOp.  We can
		 * convert a
		 * Type 4 image to a Type 1 imagemask only if the effective
		 * RasterOp
		 * passes through the source color unchanged.  "Effective" means
		 * we
		 * take into account CombineWithColor, and whether the source
		 * and/or
		 ...
		 */
		{
			...
			int black_or_white =
			    color_is_black_or_white(dev, pdcolor);
...
#define COPY_ELEMENT(e) pim1->e = pim4->e
		...
		pim1->format =
		    gs_image_format_chunky; /* BPC = 1, doesn't matter */
#undef COPY_ELEMENT
		return 0;
	}
...
private
int
pdf_begin_image_data_decoded(gx_device_pdf* pdev, int num_components,
                             const gs_range_t* pranges, int i,
                             gs_pixel_image_t* pi, cos_value_t* cs_value,
                             pdf_image_enum* pie)
{
                      ...
                      pdf_typed_image_context_t context)
{
	...
	/* An initialization for pdf_end_and_do_image :
	   We need to delay adding the "Mask" entry into a type 3 image
	   dictionary
	...
	case 1: {
		...
		if(is_mask) {
			/* If parameters are invalid, use the default
			 * implementation. */
	...
	case 3: {
		...
		if(pdev->CompatibilityLevel < 1.3 && !pdev->PatternImagemask) {
			...
			if(pim3a.Width < pim3a.MaskDict.Width &&
			   pim3a.Width > 0) {
				int sx =
				    (pim3a.MaskDict.Width + pim3a.Width - 1) /
				    pim3a.Width;

				gs_matrix_scale(&mi, 1.0 / sx, 1, &mi);
				gs_matrix_scale(&pim3a.ImageMatrix, 1.0 / sx, 1,
				                &pim3a.ImageMatrix);
			}
			if(pim3a.Height < pim3a.MaskDict.Height &&
			   pim3a.Height > 0) {
				int sy =
				    (pim3a.MaskDict.Height + pim3a.Height - 1) /
				    pim3a.Height;

				gs_matrix_scale(&mi, 1, 1.0 / sy, &mi);
				gs_matrix_scale(&pim3a.ImageMatrix, 1, 1.0 / sy,
				                &pim3a.ImageMatrix);
			}
			gs_matrix_multiply(&mi, &pim3a.MaskDict.ImageMatrix,
			                   &pim3a.MaskDict.ImageMatrix);
			pic1 = (gs_image_common_t*)&pim3a;
			/* Setting pdev->converting_image_matrix to communicate
			 * with pdf_image3_make_mcde. */
			gs_matrix_multiply(&mi, &ctm_only(pis),
			                   &pdev->converting_image_matrix);
		}
		/*
		 * We handle ImageType 3 images in a completely different way:
		 * the default implementation sets up the enumerator.
		 */
		return gx_begin_image3_generic(
		    (gx_device*)pdev, pis, pmat1, pic1, prect, pdcolor, pcpath,
		    mem, pdf_image3_make_mid, pdf_image3_make_mcde, pinfo);
	}
	case IMAGE3X_IMAGETYPE: {
		...
		return gx_begin_image3x_generic(
		    (gx_device*)pdev, pis, pmat, pic, prect, pdcolor, pcpath,
		    mem, pdf_image3x_make_mid, pdf_image3x_make_mcde, pinfo);
	}
	case 4: {
		...
		pdev->image_mask_is_SMask = false;
		if(pdf_convert_image4_to_image1(
		       pdev, pis, pdcolor, (const gs_image4_t*)pic,
			...
			if(pnamed)
				cos_array_add_object(pdev->NI_stack,
				                     COS_OBJECT(pnamed));
			/* HACK: temporary patch the color space, to allow
			   pdf_prepare_imagemask to write the right color for
			   the imagemask. */
			code = gs_gsave(pgs);
			if(code < 0)
				return code;
			code = gs_setcolorspace(
			    pgs, ((const gs_image4_t*)pic)->ColorSpace);
			if(code < 0)
				return code;
			code = pdf_begin_typed_image(
			    pdev, pis, pmat,
			    (gs_image_common_t*)&image[0].type1, prect, &icolor,
			    pcpath, mem, pinfo, context);
			if(code < 0)
				return code;
		...
		if(pdev->CompatibilityLevel < 1.3 && !pdev->PatternImagemask) {
			...
			code = pdf_setup_masked_image_converter(
			    pdev, mem, &m, &cvd, true, 0, 0, pi4.Width,
			    pi4.Height, false);
			if(code < 0)
				return code;
			...
			code = (*dev_proc(cvd->mask, fill_rectangle))(
			    (gx_device*)cvd->mask, 0, 0, cvd->mask->width,
			    cvd->mask->height, (gx_color_index)0);
			if(code < 0)
				return code;
			...
			code = gx_default_begin_typed_image(
			    (gx_device*)cvd, pis, &m1, (gs_image_common_t*)&pi4,
			    prect, pdcolor, NULL, mem, pinfo);
			if(code < 0)
				return code;
	...
	memset(
	    pie, 0,
	    sizeof(*pie)); /* cleanup entirely for GC to work in all cases. */
	*pinfo = (gx_image_enum_common_t*)pie;
	gx_image_enum_common_init(
	    *pinfo, (const gs_data_image_t*)pim,
	...
	if(pnamed != 0) /* Don't in-line the image if it is named. */
		...
		double nbytes =
		    (double)(((ulong)pie->width * pie->bits_per_pixel + 7) >>
		             3) *
		    pie->num_planes * pie->rows_left;

		in_line &= (nbytes < pdev->MaxInlineImageSize);
	}
	if(rect.p.x != 0 || rect.p.y != 0 || rect.q.x != pim->Width ||
	   rect.q.y != pim->Height || (is_mask && pim->CombineWithColor)
		...
		pmat = &ctm_only(pis);
	{
		...
		pdf_make_bitmap_matrix(&bmat, -rect.p.x, -rect.p.y, pim->Width,
		                       pim->Height, height);
		...
		/* AR3,AR4 show no image when CTM is singular; AR5 reports an
		 * error */
	...
	pie->writer.alt_writer_count =
	    (in_line || (pim->Width <= 64 && pim->Height <= 64) ||
	             pdev->transfer_not_identity
	         ? 1
	         : 2);
	image[1] = image[0];
	names =
	    (in_line ? &pdf_color_space_names_short : &pdf_color_space_names);
	if(psdf_is_converting_image_to_RGB((gx_device_psdf*)pdev, pis, pim)) {
		...
		code = pdf_color_space(pdev, &cs_value, &pranges, pcs, names,
		                       in_line);
		if(code < 0) {
			const char* sname;
			...
			case gs_color_space_index_DeviceGray:
				sname = names->DeviceGray;
				break;
			case gs_color_space_index_DeviceRGB:
				sname = names->DeviceRGB;
				break;
			case gs_color_space_index_DeviceCMYK:
				sname = names->DeviceCMYK;
				break;
			default:
	...
	if((code = pdf_begin_write_image(pdev, &pie->writer, gs_no_id, width,
	   ...
	   (code = (pie->writer.alt_writer_count == 1
	                ...
	                : psdf_setup_image_filters(
	                      (gx_device_psdf*)pdev, &pie->writer.binary[0],
	                      &image[0].pixel, pmat, pis, true))) < 0)
		goto fail;
	if(convert_to_process_colors) {
		code = psdf_setup_image_colors_filter(
		    &pie->writer.binary[0], (gx_device_psdf*)pdev,
		    &image[0].pixel, pis, pdev->pcm_color_info_index);
		if(code < 0)
			goto fail;
	}
	if(pie->writer.alt_writer_count > 1) {
		...
		if(code)
			goto fail;
		code = psdf_setup_image_filters(
		    (gx_device_psdf*)pdev, &pie->writer.binary[1],
		    &image[1].pixel, pmat, pis, false);
		if(code == gs_error_rangecheck) {
			/* setup_image_compression rejected the alternative
			 * compression. */
			pie->writer.alt_writer_count = 1;
			memset(pie->writer.binary + 1, 0,
			       sizeof(pie->writer.binary[1]));
			memset(pie->writer.binary + 2, 0,
			       sizeof(pie->writer.binary[1]));
		} else if(code)
			goto fail;
		else if(convert_to_process_colors) {
			code = psdf_setup_image_colors_filter(
			    &pie->writer.binary[1], (gx_device_psdf*)pdev,
			    &image[1].pixel, pis, pdev->pcm_color_info_index);
			if(code < 0)
				goto fail;
		}
	}
	for(i = 0; i < pie->writer.alt_writer_count; i++) {
		code = pdf_begin_image_data_decoded(pdev, num_components,
		                                    pranges, i, &image[i].pixel,
		                                    &cs_value, pie);
		if(code < 0)
			goto fail;
	}
	if(pie->writer.alt_writer_count == 2) {
		psdf_setup_compression_chooser(
		    &pie->writer.binary[2], (gx_device_psdf*)pdev, pim->Width,
		    pim->Height, num_components, pim->BitsPerComponent);
	...
	if(pic->type->index == 4 && pdev->CompatibilityLevel < 1.3) {
		...
		gs_image_t_init_mask_adjust((gs_image_t*)&image[i].type1, true,
		                            false);
		...
		if(code)
			goto fail;
		code = psdf_setup_image_filters(
		    (gx_device_psdf*)pdev, &pie->writer.binary[i],
		    &image[i].pixel, pmat, pis, true);
		if(code < 0)
			goto fail;
		psdf_setup_image_to_mask_filter(
		    &pie->writer.binary[i], (gx_device_psdf*)pdev, pim->Width,
		    pim->Height, num_components, pim->BitsPerComponent,
		    image[i].type4.MaskColor);
		code = pdf_begin_image_data_decoded(pdev, num_components,
		                                    pranges, i, &image[i].pixel,
		                                    &cs_value, pie);
		if(code < 0)
		...
		/* Note : Possible values for alt_writer_count are 1,2,3, 4.
		   ...
		   2 means the main image stream and a mask stream while
		   converting
		           an Image Type 4.
		   3 means the main image steram, alternative image compression
		   stream,
		           and the compression chooser.
		   4 meams 3 and a mask stream while convertingh an Image Type
		   4.
		 */
	}
	return 0;
...
nyi:
	return gx_default_begin_typed_image((gx_device*)pdev, pis, pmat, pic,
                         ...
                         int* rows_used, int alt_writer_index)
{
	...
	for(y = 0; y < h; ++y) {
		if(nplanes > 1) {
			/*
			 * We flip images in blocks, and each block except the
			 * last one
			 * must contain an integral number of pixels.  The
			 * easiest way
			 * to meet this condition is for all blocks except the
			 * last to
			 * be a multiple of 3 source bytes (guaranteeing an
			 * integral
			 * number of 1/2/4/8/12-bit samples), i.e., 3*nplanes
			 * flipped
...
#define ROW_BYTES max(200 /*arbitrary*/, 3 * GS_IMAGE_MAX_COMPONENTS)
			...
			for(pi = 0; pi < nplanes; ++pi)
				bit_planes[pi] =
				    planes[pi].data + planes[pi].raster * y;
			while(count) {
				uint flip_count;
				uint flipped_count;
				...
				if(count >= block_bytes) {
					...
					flip_count = count;
					flipped_count =
					    (width_bits % (block_bytes * 8) *
					         nplanes +
					     7) >>
					    3;
				}
				image_flip_planes(row, bit_planes, offset,
				                  flip_count, nplanes,
				                  pie->plane_depths[0]);
				status = sputs(
				    pie->writer.binary[alt_writer_index].strm,
				    row, flipped_count, &ignore);
				if(status < 0)
					break;
				offset += flip_count;
				count -= flip_count;
			...
			status =
			    sputs(pie->writer.binary[alt_writer_index].strm,
                     ...
                     const gx_image_plane_t* planes, int height, int* rows_used)
{
	...
	int i;
	for(i = 0; i < pie->writer.alt_writer_count; i++) {
		int code = pdf_image_plane_data_alt(info, planes, height,
		                                    rows_used, i);
		if(code)
			return code;
	}
...
{ /* See also dump_image in gdevpdfd.c . */
	...
	inst.template.XStep =
	    2; /* Set 2 times bigger step against artifacts. */
	inst.template.YStep = 2;
	code = (*dev_proc(pdev, pattern_manage))((gx_device*)pdev, id, &inst,
	                                         pattern_manage__start_accum);
	if(code >= 0)
		pprintld1(pdev->strm, "/R%ld Do\n", pdf_resource_id(pres1));
	pres = pdev->accumulating_substream_resource;
	if(code >= 0)
		code = pdf_add_resource(pdev, pdev->substream_Resources,
		                        "/XObject", pres1);
	if(code >= 0)
		code = (*dev_proc(pdev, pattern_manage))(
		    (gx_device*)pdev, id, &inst, pattern_manage__finish_accum);
	if(code >= 0)
		code = (*dev_proc(pdev, pattern_manage))(
		    (gx_device*)pdev, id, &inst, pattern_manage__load);
	if(code >= 0) {
	...
	if(code >= 0) {
		...
		m.tx = pmat->tx;
		m.ty = pmat->ty;
		code = pdf_do_image_by_id(pdev, pdev->image_mask_scale, &m,
		                          true, pdev->image_mask_id);
		stream_puts(pdev->strm, "Q\n");
	}
	return code;
}

typedef enum { USE_AS_MASK, USE_AS_IMAGE, USE_AS_PATTERN } pdf_image_usage_t;

/* Close PDF image and do it. */
private
int
pdf_end_and_do_image(gx_device_pdf* pdev, pdf_image_writer* piw,
                     const gs_matrix* mat, gs_id ps_bitmap_id,
	...
	case 0:
		if(do_image == USE_AS_IMAGE) {
			if(pdev->image_mask_id != gs_no_id) {
				...
				code = cos_dict_put_string_copy(
				    (cos_dict_t*)pres->object,
				    pdev->image_mask_is_SMask ? "/SMask"
				                              : "/Mask",
				    buf);
				if(code < 0)
					return code;
			...
			/* Provide data for pdf_do_image_by_id, which will be
			   called through
			    use_image_as_pattern during the next call to this
			   function.
			    See pdf_do_image about the meaning of 'scale'. */
			const pdf_x_object_t* const pxo =
			    (const pdf_x_object_t*)pres;

			pdev->image_mask_scale =
			    (double)pxo->data_height / pxo->height;
			pdev->image_mask_id = pdf_resource_id(pres);
			pdev->converting_image_matrix = *mat;
		} else if(do_image == USE_AS_PATTERN)
			code =
			    use_image_as_pattern(pdev, pres, mat, ps_bitmap_id);
	}
	return code;
}
                         ...
                         pdf_image_usage_t do_image)
{
	...
	if(data_height > 0) {
		...
		/* The call above possibly decreases
		 * pie->writer.alt_writer_count in 2. */
		if(code < 0)
			return code;
		if(pie->writer.alt_writer_count == 2) {
			/* We're converting a type 4 image into an imagemask
			 * with a pattern color. */
			/* Since the type 3 image writes the mask first, do so
			 * here. */
			...
			if(code < 0)
				return code;
			code = pdf_end_and_do_image(pdev, &writer, &pie->mat,
			                            info->id, USE_AS_MASK);
			if(code < 0)
				return code;
			code =
			    pdf_end_and_do_image(pdev, &pie->writer, &pie->mat,
			                         info->id, USE_AS_PATTERN);
		} else
			code = pdf_end_and_do_image(
			    pdev, &pie->writer, &pie->mat, info->id, do_image);
		pie->writer.alt_writer_count--; /* For GC. */
	}
...
pdf_image_end_image_cvd(gx_image_enum_common_t* info, bool draw_last)
{
	...
	gs_free_object(cvd->mask->memory, (gx_device*)cvd->mask,
	               "pdf_image_end_image_cvd");
	gs_free_object(cvd->mdev.memory, (gx_device*)cvd,
	               "pdf_image_end_image_cvd");
...
private
dev_proc_begin_typed_image(pdf_mid_begin_typed_image);
private
int
                    ...
                    gs_memory_t* mem)
{
	...
	if(pdev->CompatibilityLevel < 1.3 && !pdev->PatternImagemask) {
		...
		gs_make_identity(&m);
		code = pdf_setup_masked_image_converter(
		    pdev, mem, &m, &cvd, true, 0, 0, width, height, true);
		if(code < 0)
			return code;
		cvd->mask->target = (gx_device*)
		    cvd; /* Temporary, just to communicate with
		...
		*pmidev = (gx_device*)cvd->mask;
		...
		if(code < 0)
			return code;
		set_dev_proc(*pmidev, begin_typed_image,
		             pdf_mid_begin_typed_image);
		return 0;
	}
}
private
                          ...
                          gx_image_enum_common_t** pinfo)
{
	...
	gx_device_pdf* const pdev =
	    (gx_device_pdf*)((gx_device_null*)dev)->target;
	return pdf_begin_typed_image(pdev, pis, pmat, pic, prect, pdcolor,
	                             pcpath, mem, pinfo, PDF_IMAGE_TYPE3_MASK);
}
...
pdf_image3_make_mcde(gx_device* dev, const gs_imager_state* pis,
                     ...
                     const gs_int_rect* prect, const gx_drawing_color* pdcolor,
                     const gx_clip_path* pcpath, gs_memory_t* mem,
                     gx_image_enum_common_t** pinfo, gx_device** pmcdev,
                     gx_device* midev, gx_image_enum_common_t* pminfo,
                     const gs_int_point* origin)
{
	...
	if(pdev->CompatibilityLevel < 1.3 && !pdev->PatternImagemask) {
		/* pdf_image3_make_mid must set midev with a pdf_lcvd_t
		 * instance.*/
		pdf_lcvd_t* cvd =
		    (pdf_lcvd_t*)((gx_device_memory*)midev)->target;
		...
		*pmcdev = (gx_device*)&cvd->mdev;
		code = gx_default_begin_typed_image((gx_device*)&cvd->mdev, pis,
		                                    pmat, pic, prect, pdcolor,
		                                    pcpath, mem, pinfo);
	} else {
		code = pdf_make_mxd(pmcdev, midev, mem);
		if(code < 0)
			return code;
		code = pdf_begin_typed_image((gx_device_pdf*)dev, pis, pmat,
		                             pic, prect, pdcolor, pcpath, mem,
		                             pinfo, PDF_IMAGE_TYPE3_DATA);
	}
	/* Due to equal image merging, we delay the adding of the "Mask" entry
	   into
...
pdf_image3x_make_mcde(gx_device* dev, const gs_imager_state* pis,
                      ...
                      const gx_clip_path* pcpath, gs_memory_t* mem,
                      gx_image_enum_common_t** pinfo, gx_device** pmcdev,
                      gx_device* midev[2], gx_image_enum_common_t* pminfo[2],
                      const gs_int_point origin[2], const gs_image3x_t* pim)
{
	int code;
	pdf_image_enum* pmie;
	pdf_image_enum* pmce;
	...
	if(code < 0)
		return code;
	code = pdf_begin_typed_image((gx_device_pdf*)dev, pis, pmat, pic, prect,
	                             pdcolor, pcpath, mem, pinfo,
	...
	if(pixm->has_Matte) {
		...
		code = cos_dict_put_c_key_floats(
		    (cos_dict_t*)pmie->writer.pres->object, "/Matte",
		    pixm->Matte, num_components);
		if(code < 0)
			return code;
	}
...
pdf_resource_t*
pdf_substitute_pattern(pdf_resource_t* pres)
{
	pdf_pattern_t* ppat = (pdf_pattern_t*)pres;

	return (pdf_resource_t*)(ppat->substitute != 0 ? ppat->substitute
	                                               : ppat);
}

private
int
check_unsubstituted2(gx_device_pdf* pdev, pdf_resource_t* pres0,
                     pdf_resource_t* pres1)
{
                        ...
                        pattern_manage_t function)
{
	...
	case pattern_manage__start_accum:
		code = pdf_enter_substream(
		    pdev, resourcePattern, id, &pres, false,
	...
	case pattern_manage__finish_accum:
		code =
		    pdf_add_procsets(pdev->substream_Resources, pdev->procsets);
		if(code < 0)
			return code;
		...
		if(pdev->substituted_pattern_count > 300 &&
		   pdev->substituted_pattern_drop_page !=
		       pdev->next_page) { /* arbitrary */
			pdf_drop_resources(pdev, resourcePattern,
			                   check_unsubstituted1);
			pdev->substituted_pattern_count = 0;
			pdev->substituted_pattern_drop_page = pdev->next_page;
		}
		code = pdf_find_same_resource(pdev, resourcePattern, &pres,
		                              check_unsubstituted2);
		if(code < 0)
			return code;
		if(code > 0) {
			...
			code =
			    pdf_cancel_resource(pdev, pres1, resourcePattern);
			if(code < 0)
				return code;
			/* Do not remove pres1, because it keeps the
			 * substitution. */
