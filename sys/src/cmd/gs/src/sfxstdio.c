...
private
int s_file_available(stream*, int32_t*), s_file_read_seek(stream*, int32_t),
    s_file_read_close(stream*),
    s_file_read_process(stream_state*, stream_cursor_read*,
                        stream_cursor_write*, bool);
private
int s_file_write_seek(stream*, int32_t), s_file_write_flush(stream*),
    s_file_write_close(stream*),
    s_file_write_process(stream_state*, stream_cursor_read*,
                         stream_cursor_write*, bool);
private
int s_file_switch(stream*, bool);
...
{
	static const stream_procs p = {s_file_available,  s_file_read_seek,
	                               s_std_read_reset,  s_std_read_flush,
	                               s_file_read_close, s_file_read_process,
	                               s_file_switch};
	/*
                    ...
                    stream_cursor_write* pw, bool last)
{
	...
	if(s->file_limit < max_long) {
		int32_t limit_count =
		    s->file_offset + s->file_limit - ftell(file);
...
{
	static const stream_procs p = {s_std_noavailable,  s_file_write_seek,
	                               s_std_write_reset,  s_file_write_flush,
	                               s_file_write_close, s_file_write_process,
	                               s_file_switch};
...
s_file_switch(stream* s, bool writing)
{
	...
	if(writing) {
		...
		if(modes & s_mode_append) {
			sappend_file(s, file, s->cbuf,
			             s->cbsize); /* sets position */
		} else {
