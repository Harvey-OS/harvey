...
private
const int32_t max_repeated_scan = 100000;

/* Define the minimum space for creating an inner chunk. */
/* Must be at least sizeof(chunk_head_t). */
private
const int32_t min_inner_chunk_space = sizeof(chunk_head_t) + 500;
...
print_save(const char* str, uint spacen, const alloc_save_t* sav)
{
	if_debug5('u', "[u]%s space %u 0x%lx: cdata = 0x%lx, id = %lu\n", str,
	          spacen, (uint32_t)sav, (uint32_t)sav->client_data,
	          (uint32_t)sav->id);
}
...
ENUM_PTRS_END
private
RELOC_PTRS_WITH(change_reloc_ptrs, alloc_change_t* ptr)
{
	RELOC_VAR(ptr->next);
	switch(ptr->offset) {
...
gs_private_st_complex_only(st_alloc_change, alloc_change_t, "alloc_change",
                           change_clear_marks, change_enum_ptrs,
                           change_reloc_ptrs, 0);
	...
	if(r_is_packed(&cp->contents)) {
		if(print_current)
			dprintf2("saved=%x cur=%x\n",
			         *(ref_packed*)&cp->contents, *cp->where);
		else
			dprintf1("%x\n", *(ref_packed*)&cp->contents);
	} else {
		if(print_current)
			dprintf6("saved=%x %x %lx cur=%x %x %lx\n",
			         r_type_attrs(&cp->contents),
			         r_size(&cp->contents),
...
/* Forward references */
private
void restore_resources(alloc_save_t*, gs_ref_memory_t*);
private
void restore_free(gs_ref_memory_t*);
private
long save_set_new(gs_ref_memory_t*, bool);
private
void save_set_new_changes(gs_ref_memory_t*, bool);
	...
	for(i = 0; i < countof(dmem->spaces.memories.indexed); ++i)
		if((mem = dmem->spaces.memories.indexed[i]) != 0) {
			...
			if(mem->stable_memory != (gs_memory_t*)mem) {
				...
				mem->new_mask = new_mask,
				mem->test_mask = test_mask;
			}
		}
}
void
...
/* Save the state. */
private
alloc_save_t* alloc_save_space(gs_ref_memory_t* mem, gs_dual_memory_t* dmem,
                               uint32_t sid);
private
void
...
alloc_save_state(gs_dual_memory_t* dmem, void* cdata)
{
	...
	bool global =
	    lmem->save_level == 0 && gmem != lmem && gmem->num_contexts == 1;
	alloc_save_t* gsave =
	...
	if(lsave == 0 || (global && gsave == 0)) {
		if(lsave != 0)
			alloc_free_save(lmem, lsave,
			                "alloc_save_state(local save)");
		if(gsave != 0)
			alloc_free_save(gmem, gsave,
			                "alloc_save_state(global save)");
		return 0;
	}
	if(gsave != 0) {
		gsave->client_data = 0;
	...
	if(lmem->save_level > 1) {
		...
		if((lsave->state.total_scanned += scanned) >
		   max_repeated_scan) {
...
alloc_save_space(gs_ref_memory_t* mem, gs_dual_memory_t* dmem, uint32_t sid)
{
	...
	for(cp = save_mem.cfirst; cp != 0; cp = cp->cnext) {
		if(cp->ctop - cp->cbot > min_inner_chunk_space) {
			/* Create an inner chunk to cover only the unallocated
			 * part. */
			chunk_t* inner = gs_raw_alloc_struct_immovable(
			    mem->non_gc_memory, &st_chunk,
			    "alloc_save_space(inner)");

			if(inner == 0)
				break; /* maybe should fail */
			alloc_init_chunk(inner, cp->cbot, cp->ctop,
			                 cp->sreloc != 0, cp);
			alloc_link_chunk(inner, mem);
			if_debug2('u',
			          "[u]inner chunk: cbot=0x%lx ctop=0x%lx\n",
	...
	save = gs_alloc_struct((gs_memory_t*)mem, alloc_save_t, &st_alloc_save,
	                       "alloc_save_space(save)");
...
int
alloc_save_change_in(gs_ref_memory_t* mem, const ref* pcont, ref_packed* where,
                     client_name_t cname)
{
	...
	if(pcont == NULL)
		...
		lprintf3(
		    "Bad type %u for save!  pcont = 0x%lx, where = 0x%lx\n",
...
int
alloc_save_change(gs_dual_memory_t* dmem, const ref* pcont, ref_packed* where,
                  client_name_t cname)
{
	gs_ref_memory_t* mem =
	    (pcont == NULL
...
alloc_is_since_save(const void* vptr, const alloc_save_t* save)
{
	...
	if_debug2('U', "[U]is_since_save 0x%lx, 0x%lx:\n", (uint32_t)ptr,
	          (uint32_t)save);
	if(mem->saved ==
	   0) { /* This is a special case, the final 'restore' from */
		/* alloc_restore_all. */
		return true;
	}
	...
	for(;; mem = &mem->saved->state) {
		...
		for(cp = mem->cfirst; cp != 0; cp = cp->cnext) {
			if(ptr_is_within_chunk(ptr, cp)) {
				if_debug3(
				    'U',
				    "[U+]in new chunk 0x%lx: 0x%lx, 0x%lx\n",
		...
		if(mem->saved ==
		   save) { /* We've checked all the more recent saves, */
			/* must be OK. */
			break;
		}
	}
	 ...
	 */
	if(save->state.save_level ==
	       0 /* Restoring to save level 0 - see bug 688157, 688161 */ &&
...
bool
alloc_name_is_since_save(const gs_memory_t* mem, const ref* pnref,
...
bool
alloc_name_index_is_since_save(const gs_memory_t* mem, uint nidx,
                               const alloc_save_t* save)
{
	const name_string_t* pnstr;
 ...
 */
private
void restore_finalize(gs_ref_memory_t*);
private
void restore_space(gs_ref_memory_t*, gs_dual_memory_t*);
		...
		while(cp) {
			...
			if(r_is_packed(&cp->contents))
				*cp->where = *(ref_packed*)&cp->contents;
				...
				ref_assign_inline((ref*)cp->where,
				                  &cp->contents);
			cp = cp->next;
		}
	}
...
alloc_restore_all(gs_dual_memory_t* dmem)
{
	...
	if(gmem != lmem) {
		if(!--(gmem->num_contexts)) {
			restore_free(gmem);
			if((mem = (gs_ref_memory_t*)gmem->stable_memory) !=
			   gmem)
				restore_free(mem);
		}
	}
...
restore_finalize(gs_ref_memory_t* mem)
{
	...
	for(cp = mem->clast; cp != 0; cp = cp->cprev) {
		...
		DO_ALL
		struct_proc_finalize((*finalize)) = pre->o_type->finalize;
		if(finalize != 0) {
...
private
void combine_space(gs_ref_memory_t*);
private
void forget_changes(gs_ref_memory_t*);
void
...
#define RP_REF(rp) ((ref*)rp)
				...
				/* a full-size ref, so we only need the end
				 * check */
