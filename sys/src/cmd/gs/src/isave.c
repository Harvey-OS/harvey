...
/* Save the state. */
private
alloc_save_t *alloc_save_space(gs_ref_memory_t *mem,
...
{
	gs_ref_memory_t *mem =
	    (pcont == NULL ? dmem->space_local : dmem->spaces_indexed[r_space(pcont) >> r_space_shift]);
	...
	for(cp = mem->cfirst; cp != 0; cp = csucc) {
		...
		if(cp->outer == 0)
			...
			/* the GC having compacted the outer chunk. */
			{
				...
				hp->o_alone = 0;
				hp->o_size = (char *)(cp->chead + 1) - (char *)(hp + 1);
				hp->o_type = &st_bytes;
