...
/* Copyright (C) 1995, 1996, 1997, 1998 Aladdin Enterprises.  All rights
  reserved.
 ...
 */
private
const int32_t COMPRESSION_THRESHOLD = 300000;
...
#define GET_NUM_RAW_BUFFERS(f) max(f->log_length / MEMFILE_DATA_SIZE / 32, 8)
...
/* forward references */
private
void memfile_free_mem(MEMFILE* f);
private
int memfile_init_empty(MEMFILE* f);
...
void* /* allocated memory's address, 0 if failure */
    allocateWithReserve(
        ...
        int* return_code, /* RET 0 ok, -ve GS-style error, or +1 if OK but low
                             memory */
	...
	if(block == NULL) {
		...
		if(sizeofBlock == sizeof(LOG_MEMFILE_BLK)) {
			if(f->reserveLogBlockCount > 0) {
				...
				f->reserveLogBlockChain =
				    f->reserveLogBlockChain->link;
			...
			if(f->reservePhysBlockCount > 0) {
				...
				f->reservePhysBlockChain =
				    f->reservePhysBlockChain->link;
...
memfile_fopen(char fname[gp_file_name_sizeof], const char* fmode,
              clist_file_ptr /*MEMFILE * */* pf, gs_memory_t* mem,
              gs_memory_t* data_mem, bool ok_to_compress)
{
	MEMFILE* f = 0;
	int code = 0;
	...
	f->decompress_state = 0;
	if(f->ok_to_compress) {
		const stream_state* compress_proto =
		    clist_compressor_state(NULL);
		const stream_state* decompress_proto =
		    clist_decompressor_state(NULL);
		const stream_template* compress_template =
		    compress_proto->template;
		const stream_template* decompress_template =
		    decompress_proto->template;

		f->compress_state =
		    gs_alloc_struct(mem, stream_state, compress_template->stype,
		                    "memfile_open_scratch(compress_state)");
		f->decompress_state = gs_alloc_struct(
		    mem, stream_state, decompress_template->stype,
		    "memfile_open_scratch(decompress_state)");
		if(f->compress_state == 0 || f->decompress_state == 0) {
			eprintf1("memfile_open_scratch(%s): gs_alloc_struct "
			         "failed\n",
			         fname);
			code = gs_note_error(gs_error_VMerror);
			goto finish;
		...
		if(decompress_template->set_defaults)
			(*decompress_template->set_defaults)(
			    f->decompress_state);
	}
	f->total_space = 0;
...
/* Pre-alloc enough reserve mem blox to guarantee a write of N bytes will
 * succeed */
    ...
    memfile_set_memory_warning(clist_file_ptr cf, int bytes_left)
{
	...
	while(logNeeded > f->reserveLogBlockCount) {
		LOG_MEMFILE_BLK* block = MALLOC(f, sizeof(LOG_MEMFILE_BLK),
	...
	while(physNeeded > f->reservePhysBlockCount) {
		PHYS_MEMFILE_BLK* block =
		    MALLOC(f, max(sizeof(PHYS_MEMFILE_BLK), sizeof(RAW_BUFFER)),
		           "memfile_set_block_size");
...
compress_log_blk(MEMFILE* f, LOG_MEMFILE_BLK* bp)
{
	...
	start_ptr = f->wt.ptr;
	status = (*f->compress_state->template->process)(
	    f->compress_state, &(f->rd), &(f->wt), true);
	...
	if(status == 1) { /* More output space needed (see strimpl.h) */
		...
		newphys = allocateWithReserve(
		    f, sizeof(*newphys), &code, "memfile newphys",
		...
		if(status != 0) {
			/*
			 * You'd think the above line is a bug, but in real life
			 * 1 src
			 * block never ends up getting split across 3 dest
			 * blocks.
			 */
			/* CHANGE memfile_set_memory_warning if this assumption
			 * changes. */
			eprintf(
...
private
int /* ret 0 ok, -ve error, or +ve low-memory warning */
    memfile_next_blk(MEMFILE* f)
{
	...
	if(f->phys_curr == NULL) { /* means NOT compressing                */
		...
		newbp = allocateWithReserve(
		    f, sizeof(*newbp), &code, "memfile newbp",
		...
		if(NEED_TO_COMPRESS(f)) {
			...
			if(!f->compressor_initialized) {
				...
				if(f->compress_state->template->init != 0)
					code = (*f->compress_state->template
					             ->init)(f->compress_state);
				if(code < 0)
					return_error(
					    gs_error_VMerror); /****** BOGUS
					                          ******/
				if(f->decompress_state->template->init != 0)
					code =
					    (*f->decompress_state->template
					          ->init)(f->decompress_state);
				if(code < 0)
					return_error(
					    gs_error_VMerror); /****** BOGUS
					                          ******/
			 ...
			 */
			/* replace it after the loop (after some blocks are
			 * freed)     */
			f->phys_curr = newphys;
			f->wt.ptr = (byte*)(newphys->data) - 1;
			f->wt.limit = f->wt.ptr + MEMFILE_DATA_SIZE;
			...
			/* Allocate a physical block for this (last) logical
			 * block     */
			newphys = allocateWithReserve(
			    f, sizeof(*newphys), &code, "memfile newphys",
			    "memfile_next_blk: MALLOC 2 for 'newphys' "
			    "failed\n");
			if(code < 0)
				return code;
			ecode |= code; /* accumulate low-mem warnings */
	...
	if(bp->phys_blk->data_limit == NULL) {
		...
		if(f->raw_head == NULL) {
			...
			if(f->reservePhysBlockCount) {
				/* HACK: allocate reserve block that's been
				 * reserved for
				 * decompression.  This buffer's block was
				 * pre-allocated to make
				 * sure we won't come up short here. Take from
				 * chain instead of
				 * allocateWithReserve() since this buf would
				 * just be wasted if
				 * allowed to remain preallocated. */
				f->raw_head =
				    (RAW_BUFFER*)f->reservePhysBlockChain;
				f->reservePhysBlockChain =
				    f->reservePhysBlockChain->link;
				--f->reservePhysBlockCount;
			} else {
				...
				f->raw_head = allocateWithReserve(
				    f, sizeof(*f->raw_head), &code,
				    "memfile raw buffer",
				    "memfile_get_pdata: MALLOC for 'raw_head' "
				    "failed\n");
				if(code < 0)
					return code;
			}
			...
			for(i = 0; i < num_raw_buffers; i++) {
				f->raw_tail->fwd =
				    (RAW_BUFFER*)MALLOC(f, sizeof(RAW_BUFFER),
				                        "memfile raw buffer");
...
/* This block was in use, grab it                           */
				...
				tot_swap_out++;
#endif
				f->raw_tail->log_blk->raw_block =
				    NULL; /* data no longer here */
				f->raw_tail->log_blk = NULL;
			}
			/* Use the last raw block in the chain (the oldest) */
			f->raw_tail->back->fwd =
			    NULL; /* disconnect from tail */
			...
			if(f->decompress_state->template->reinit != 0)
				(*f->decompress_state->template->reinit)(
				    f->decompress_state);
			...
			if(status == 0) { /* More input data needed */
				...
				if(f->rd.ptr != f->rd.limit) {
					/* transfer remainder bytes from the
					 * previous block      */
					back_up = f->rd.limit - f->rd.ptr;
					for(i = 0; i < back_up; i++)
						*(bp->phys_blk->link->data -
						  back_up + i) = *++f->rd.ptr;
				}
				f->rd.ptr =
				    (const byte*)bp->phys_blk->link->data -
				    back_up - 1;
				f->rd.limit =
				    (const byte*)bp->phys_blk->link->data_limit;
#ifdef DEBUG
...
#endif
				status =
				    (*f->decompress_state->template->process)(
				        f->decompress_state, &(f->rd), &(f->wt),
				        true);
				if(status == 0) {
					eprintf("Decompression required more "
					        "than one full block!\n");
					return_error(gs_error_Fatal);
				}
			}
		...
		/* end if( raw_block == NULL ) meaning need to decompress data
		   */
			...
			/* data exists in the raw data cache, if not raw_head,
			 * move it */
				...
				if(bp->raw_block->fwd != NULL)
					...
					f->raw_tail =
					    bp->raw_block
					        ->back; /* tail = prev        */
				f->raw_head->back =
				    bp->raw_block; /* head.back = this     */
				bp->raw_block->fwd =
				    f->raw_head; /* this.fwd = orig head */
				f->raw_head = bp->raw_block; /* head = this */
				f->raw_head->back = NULL; /* this.back = NULL */
#ifdef DEBUG
				tot_cache_hits++; /* counting here prevents
				                     repeats since */
 ...
 ***********************************************************************/
	...
	if(f->compressor_initialized) {
		if(f->decompress_state->template->release != 0)
			(*f->decompress_state->template->release)(
			    f->decompress_state);
		if(f->compress_state->template->release != 0)
			(*f->compress_state->template->release)(
			    f->compress_state);
