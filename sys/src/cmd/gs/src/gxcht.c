...
#define SET_COLOR_HT_PROC(proc)                                                                                               \
	void proc(                                                                                                            \
	    ...
	    gx_color_index colors[MAX_DCC], /* the actual colors for the tile, */	 /* actually [nplanes] */            \
	    const gx_const_strip_bitmap *sbits[MAX_DCC] /* the bitmaps for the planes, */ /* actually [nplanes] */            \
	    )
	...
	while(w) {
		...
		for(;;) {
			set_color_ht((byte *)tbits, raster,
				     ...
				     dev, &vp, colors, sbits);
			if(no_rop) {
				code = (*dev_proc(dev, copy_color))(dev, (byte *)tbits, 0, raster, gx_no_bitmap_id,
								    x, cy, dw, ch);
			} else {
				...
				code = (*dev_proc(dev, strip_copy_rop))(dev, source->sdata, source->sourcex, source->sraster,
									source->id,
									(source->use_scolors ? source->scolors : NULL),
...
const gx_const_strip_bitmap ht_no_bitmap = {
    &ht_no_bitmap_data.bytes[0], sizeof(ulong), {sizeof(ulong) * 8, sizeof(ht_no_bitmap_data.bytes) / sizeof(ulong)}, gx_no_bitmap_id, 1, 1, 0, 0};
...
#define SET_PLANE_COLOR_CONSTANT(i)                                                                         \
	BEGIN                                                                                               \
	pvp->values[1][i] = pvp->values[0][i] = fractional_color(pdc->colors.colored.c_base[i], max_color); \
...
#define SET_PLANE_COLOR(i)                                                                                         \
	...
	if(r == 0)                                                                                                 \
		...
		int nlevels = (pdht->components ? pdht->components[i].corder.num_levels : pdht->order.num_levels); \
                                                                                                                   \
...
#define SET_PLANE_COLOR_CMYK(i, mask)                                                                              \
	...
	if(r == 0) {                                                                                               \
		...
		int nlevels = (pdht->components ? pdht->components[i].corder.num_levels : pdht->order.num_levels); \
                                                                                                                   \
		mask0 |= mask;                                                                                     \
		sbits[3 - i] = (const gx_const_strip_bitmap *)&gx_render_ht(caches[i], nlevels - r)->tiles;        \
	}                                                                                                          \
	END
...
{
	int tw = btile->size.x;
	int bx = ((ptc->tile_shift = btile->shift) == 0 ? endx : endx + lasty / btile->size.y * ptc->tile_shift) % tw;
