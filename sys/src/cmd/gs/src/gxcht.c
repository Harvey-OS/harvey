...
/* Copyright (C) 1993, 1996, 1997, 1998, 1999 Aladdin Enterprises.  All rights
  reserved.
...
#ifdef DEBUG
#define tile_longs (gs_debug_c('.') ? tile_longs_SMALL : tile_longs_LARGE)
#else
...
gs_private_st_ptrs1(st_dc_ht_colored, gx_device_color, "dc_ht_colored",
                    ...
                    colors.colored.c_ht);
private
dev_color_proc_save_dc(gx_dc_ht_colored_save_dc);
private
dev_color_proc_get_dev_halftone(gx_dc_ht_colored_get_dev_halftone);
private
dev_color_proc_load(gx_dc_ht_colored_load);
private
dev_color_proc_fill_rectangle(gx_dc_ht_colored_fill_rectangle);
private
dev_color_proc_equal(gx_dc_ht_colored_equal);
private
dev_color_proc_write(gx_dc_ht_colored_write);
private
dev_color_proc_read(gx_dc_ht_colored_read);
const gx_device_color_type_t gx_dc_type_data_ht_colored = {
    &st_dc_ht_colored, gx_dc_ht_colored_save_dc,
    gx_dc_ht_colored_get_dev_halftone, gx_dc_ht_get_phase,
    gx_dc_ht_colored_load, gx_dc_ht_colored_fill_rectangle,
    gx_dc_default_fill_masked, gx_dc_ht_colored_equal, gx_dc_ht_colored_write,
    gx_dc_ht_colored_read, gx_dc_ht_colored_get_nonzero_comps};
                         ...
                         gx_device_color_saved* psdc)
{
	...
	memcpy(psdc->colors.colored.c_base, pdevc->colors.colored.c_base,
	       sizeof(psdc->colors.colored.c_base));
	memcpy(psdc->colors.colored.c_level, pdevc->colors.colored.c_level,
	       sizeof(psdc->colors.colored.c_base));
                       ...
                       const gx_device_color* pdevc2)
{
	...
	if(pdevc2->type != pdevc1->type ||
	   ...
	   num_comp != pdevc2->colors.colored.num_components)
		return false;
	return !memcmp(pdevc1->colors.colored.c_base,
 ...
 */
private
const int dc_ht_colored_has_base = 0x01;
private
const int dc_ht_colored_has_level = 0x02;
private
const int dc_ht_colored_has_alpha = 0x04;
private
const int dc_ht_colored_alpha_is_max = 0x08;
...
private
int
gx_dc_ht_colored_write(const gx_device_color* pdevc,
                       const gx_device_color_saved* psdc0, const gx_device* dev,
                       byte* pdata, uint* psize)
{
	int req_size = 1;
	int flag_bits = 0;
	...
	if(psdc == 0 ||
	   memcmp(pdevc->colors.colored.c_base, psdc->colors.colored.c_base,
	...
	if(psdc == 0 ||
	   memcmp(pdevc->colors.colored.c_level, psdc->colors.colored.c_level,
		...
		for(i = 0, comp_bit = 0x1; i < num_comps; i++, comp_bit <<= 1) {
			if((plane_mask & comp_bit) != 0)
				req_size += enc_u_sizew(
				    pdevc->colors.colored.c_level[i]);
		}
	}
	...
	*pdata++ = (byte)flag_bits;
	...
	if((flag_bits & dc_ht_colored_has_base) != 0) {
		if(num_comps == depth) {
			...
			memcpy(pdata, pdevc->colors.colored.c_base,
			       num_comps *
			           sizeof(pdevc->colors.colored.c_base[0]));
			pdata +=
			    num_comps * sizeof(pdevc->colors.colored.c_base[0]);
		}
	}
	...
	if((flag_bits & dc_ht_colored_has_level) != 0) {
		...
		for(i = 0, code_bit = 0x1; i < num_comps; i++, code_bit <<= 1) {
			if((plane_mask & code_bit) != 0)
				enc_u_putw(pdevc->colors.colored.c_level[i],
				           pdata);
		}
	}
 ...
 */
private
int gx_dc_ht_colored_read(gx_device_color* pdevc, const gs_imager_state* pis,
                          const gx_device_color* prior_devc,
                          const gx_device* dev, const byte* pdata, uint size,
                          gs_memory_t* mem) /* ignored */
{
	...
	if((flag_bits & dc_ht_colored_has_base) != 0) {
		if(depth == num_comps) {
			...
			for(i = 0; i < num_bytes; i++, shift += 8)
				base_mask |= (gx_color_index)(*pdata++)
				             << shift;
		...
		for(i = 0; i < num_comps; i++, plane_mask >>= 1) {
			if((plane_mask & 0x1) != 0) {
				if(size - (pdata - pdata_start) < 1)
					return_error(gs_error_rangecheck);
				enc_u_getw(devc.colors.colored.c_level[i],
				           pdata);
			} else
	...
	color_set_phase_mod(&devc, pis->screen_phase[0].x,
	                    pis->screen_phase[0].y, pis->dev_ht->lcm_width,
 ...
 */
int
gx_dc_ht_colored_get_nonzero_comps(const gx_device_color* pdevc,
 ...
 * only store 2 values per color so the array size can be 2 * MAX_DCC.  Use
 * which
...
#define SET_HT_COLORS_PROC(proc)                                               \
	int proc(color_values_pair_t* pvp, gx_color_index colors[MAX_DCC_16],  \
	         const gx_const_strip_bitmap* sbits[MAX_DCC],                  \
	         const gx_device_color* pdevc, gx_device* dev,                 \
	         gx_ht_cache* caches[MAX_DCC], int nplanes)

private
SET_HT_COLORS_PROC(set_ht_colors_le_4);
private
SET_HT_COLORS_PROC(set_cmyk_1bit_colors);
private
SET_HT_COLORS_PROC(set_ht_colors_gt_4);
...
#define SET_COLOR_HT_PROC(proc)                                                \
	void proc(                                                             \
	    ...
	    int py, int w,    /* how much of the tile to set */                \
	    int h, int depth, /* depth of tile (4, 8, 16, 24, 32) */           \
	    ...
	    gx_color_index colors                                              \
	        [MAX_DCC], /* the actual colors for the tile, */ /* actually   \
	                                                            [nplanes]  \
	                                                            */         \
	    const gx_const_strip_bitmap* sbits                                 \
	        [MAX_DCC] /* the bitmaps for the planes, */ /* actually        \
	                                                       [nplanes] */    \
	    )

private
SET_COLOR_HT_PROC(set_color_ht_le_4);
private
SET_COLOR_HT_PROC(set_color_ht_gt_4);
                                ...
                                const gx_rop_source_t* source)
{
	...
	SET_HT_COLORS_PROC((*set_ht_colors)) = (
	    ...
	    (dev_proc(dev, map_cmyk_color) == gx_default_encode_color &&
	     dev->color_info.depth == 4)
	        ...
	        : nplanes <= 4 ? set_ht_colors_le_4 : set_ht_colors_gt_4
#endif
	    );
	...
	special =
	    set_ht_colors(&vp, colors, sbits, pdevc, dev, caches, nplanes);
	   ...
	   (raster = bitmap_raster(lw * depth)) <= tile_bytes / lh) {
		...
		if(w > lw || h > lh) {
			...
			set_color_ht((byte*)tbits, raster, 0, 0, lw, lh, depth,
			             special, nplanes,
			             pdevc->colors.colored.plane_mask, dev, &vp,
			             colors, sbits);
			if(no_rop)
				return (*dev_proc(dev, strip_tile_rectangle))(
				    dev, &tiles, x, y, w, h, gx_no_color_index,
				    gx_no_color_index, pdevc->phase.x,
				    pdevc->phase.y);
			if(source == NULL)
				set_rop_no_source(source, no_source, dev);
			return (*dev_proc(dev, strip_copy_rop))(
			    dev, source->sdata, source->sourcex,
			    source->sraster, source->id,
			    (source->use_scolors ? source->scolors : NULL),
			    &tiles, NULL, x, y, w, h, pdevc->phase.x,
			    pdevc->phase.y, lop);
		}
	}
	size_x = w * depth;
...
fit: /* Now the tile will definitely fit. */
	...
	while(w) {
		...
		for(;;) {
			set_color_ht((byte*)tbits, raster, x + pdevc->phase.x,
			             cy + pdevc->phase.y, dw, ch, depth,
			             special, nplanes,
			             pdevc->colors.colored.plane_mask, dev, &vp,
			             colors, sbits);
			if(no_rop) {
				code = (*dev_proc(dev, copy_color))(
				    dev, (byte*)tbits, 0, raster,
				    gx_no_bitmap_id, x, cy, dw, ch);
			} else {
				tiles.rep_height = tiles.size.y = ch;
				if(source == NULL)
					set_rop_no_source(source, no_source,
					                  dev);
				...
				code = (*dev_proc(dev, strip_copy_rop))(
				    dev, source->sdata, source->sourcex,
				    source->sraster, source->id,
				    (source->use_scolors ? source->scolors
				                         : NULL),
...
private
const gx_const_strip_bitmap ht_no_bitmap = {
    &ht_no_bitmap_data.bytes[0],
    sizeof(ulong),
...
#define SET_PLANE_COLOR(i)                                                     \
	...
	if(r == 0)                                                             \
		...
		sbits[i] = (const gx_const_strip_bitmap*)&gx_render_ht(        \
		               caches[i], r)->tiles;                           \
	} else {                                                               \
		const gx_device_halftone* pdht = pdc->colors.colored.c_ht;     \
		int nlevels =                                                  \
		    (pdht->components ? pdht->components[i].corder.num_levels  \
		                      : pdht->order.num_levels);               \
                                                                               \
		pvp->values[1][i] = pvp->values[0][i];                         \
		pvp->values[0][i] = fractional_color(q + 1, max_color);        \
		sbits[i] = (const gx_const_strip_bitmap*)&gx_render_ht(        \
		               caches[i], nlevels - r)->tiles;                 \
	}                                                                      \
	END
...
#define M(i)                                                                   \
	colors[i] = dev_proc(dev, map_rgb_alpha_color)(                        \
	    dev, pvp->values[(i)&1][0], pvp->values[((i)&2) >> 1][1],          \
...
#undef M
		...
		if(nplanes > 4) {
			/*
			 * Set colors for any planes beyond the 4th.  Since this
			 * code
			 * only handles the case of at most 4 active planes, we
			 * know
...
set_cmyk_1bit_colors(color_values_pair_t* ignore_pvp,
                     ...
                     const gx_device_color* pdc, gx_device* dev,
                     gx_ht_cache* caches[MAX_DCC /*4 used*/], int nplanes /*4*/)
{
	const gx_device_halftone* pdht = pdc->colors.colored.c_ht;
	/*
		...
		int nlevels =                                                  \
		    (pdht->components ? pdht->components[i].corder.num_levels  \
		                      : pdht->order.num_levels);               \
                                                                               \
		mask0 |= mask;                                                 \
		sbits[3 - i] = (const gx_const_strip_bitmap*)&gx_render_ht(    \
		                   caches[i], nlevels - r)->tiles;             \
	}                                                                      \
	END
...
{
	int tw = btile->size.x;
	int bx = ((ptc->tile_shift = btile->shift) == 0
	              ? endx
	              : endx + lasty / btile->size.y * ptc->tile_shift) %
	         tw;
	...
	if_debug6('h', "[h]plane %d: size=%d,%d shift=%d bx=%d by=%d\n", i, tw,
	          btile->size.y, btile->shift, bx, by);
}
	...
	if(ptc->tile_shift) {
		if((ptc->xshift += ptc->tile_shift) >= 8) {
			if((ptc->xoffset -= ptc->xshift >> 3) < 0) {
				/* wrap around in X */
				int bx = (ptc->xoffset << 3) + 8 -
				         (ptc->xshift & 7) + psbit->size.x;
...
private
const bits32 expand_8x1_to_8x4[256] = {
#define X16(c)                                                                 \
	c + 0, c + 1, c + 0x10, c + 0x11, c + 0x100, c + 0x101, c + 0x110,     \
	    c + 0x111, c + 0x1000, c + 0x1001, c + 0x1010, c + 0x1011,         \
	    c + 0x1100, c + 0x1101, c + 0x1110, c + 0x1111
    X16(0x00000000), X16(0x00010000), X16(0x00100000),
    X16(0x00110000), X16(0x01000000), X16(0x01010000),
    X16(0x01100000), X16(0x01110000), X16(0x10000000),
    X16(0x10010000), X16(0x10100000), X16(0x10110000),
    X16(0x11000000), X16(0x11010000), X16(0x11100000),
    X16(0x11110000)
#undef X16
};
...
private
void
set_color_ht_le_4(byte* dest_data, uint dest_raster, int px, int py, int w,
                  int h, int depth, int special, int nplanes,
                  ...
                  const gx_const_strip_bitmap* sbits[MAX_DCC])
{
	...
	int dbytes = depth >> 3;
	byte* dest_row = dest_data + dest_raster * (h - 1) + (w * depth) / 8;

	if(special > 0) {
		/* Planes are in reverse order. */
		plane_mask = "\000\010\004\014\002\012\006\016\001\011\005\015"
		             "\003\013\007\017"[plane_mask];
	}
	if_debug6(
	    'h',
...
private
void
set_color_ht_gt_4(byte* dest_data, uint dest_raster, int px, int py, int w,
                  int h, int depth, int special, int num_planes,
                  ...
                  const gx_const_strip_bitmap* sbits[MAX_DCC])
{
	...
	/* Do one-time cursor initialization. */
	{
		...
		for(i = pmin; i <= pmax; ++i)
			if((plane_mask >> i) & 1)
				init_tile_cursor(i, &cursor[i], sbits[i], endx,
				                 lasty);
	}
	...
	for(y = h;; dest_row -= dest_raster) {
		...
		for(x = w; x > 0;) {
			...
			for(i = pmin; i <= pmax; ++i)
				if((plane_mask >> i) & 1) {
					/* Get the next bit from an individual
					 * mask. */
