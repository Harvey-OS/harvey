...
/* Define an abstract class for accessing memory managers from the TT
 * interpreter. */
typedef struct ttfMemory_s ttfMemory;
struct ttfMemory_s {
	void* (*alloc_bytes)(ttfMemory*, int size, const char* cname);
	void* (*alloc_struct)(ttfMemory*, const ttfMemoryDescriptor*,
	                      const char* cname);
...
FontError ttfOutliner__Outline(ttfOutliner* this, int glyphIndex, float orig_x,
