...
HBITMAP make_bitmap(BMAP* pbm, ULONG left, ULONG bottom, ULONG right, ULONG top,
                    ULONG depth);
...
message_box(char* str, int icon)
{
	return WinMessageBox(HWND_DESKTOP,
	                     hwnd_frame ? hwnd_frame : HWND_DESKTOP, str,
	                     "gspmdrv.exe", 0, icon | MB_MOVEABLE | MB_OK);
}

void
error_message(char* str)
{
	WinMessageBox(HWND_DESKTOP, HWND_DESKTOP, str, "gspmdrv.exe", 0,
	              MB_MOVEABLE | MB_ICONHAND | MB_OK);
...
init_window()
{
	...
	if(DosQuerySysInfo(QSV_VERSION_MAJOR, QSV_VERSION_REVISION, &version,
	                   sizeof(version)))
	...
	if(!rc) {
		...
		hwnd_frame =
		    WinCreateStdWindow(HWND_DESKTOP, /* window type */
...
read_profile(void)
{
	...
	PrfQueryProfileString(HINI_USERPROFILE, section, "Origin", "", profile,
	                      sizeof(profile));
	if(sscanf(profile, "%d %d", &option.img_origin.x,
	          &option.img_origin.y) != 2) {
		option.img_origin.x = CW_USEDEFAULT;
		option.img_origin.y = CW_USEDEFAULT;
	}
	PrfQueryProfileString(HINI_USERPROFILE, section, "Size", "", profile,
	                      sizeof(profile));
	if(sscanf(profile, "%d %d", &option.img_size.x, &option.img_size.y) !=
	   2) {
		option.img_size.x = CW_USEDEFAULT;
		option.img_size.y = CW_USEDEFAULT;
	}
	PrfQueryProfileString(HINI_USERPROFILE, section, "Maximized", "",
...
restore_window_position(SWP* pswp)
{
	...
	if(!WinSetWindowPos(hwnd_frame, hwnd_gs, swp.x, swp.y, swp.cx, swp.cy,
	                    swp.fl))
		return 1;
	return 0;
}
...
init_display(int argc, char* argv[])
{
	...
	if(!rc) {
		...
		rc = DosGetNamedSharedMem((PVOID*)&bitmap.pbmi, name,
		                          PAG_READ | PAG_WRITE);
		if(rc) {
			sprintf(
			    buf,
			    "Failed to open: bmp shared memory \"%s\" rc = %d",
			    argv[0], rc);
			error_message(buf);
		}
	}
	if(!rc) {
		...
		if(rc) {
			sprintf(buf, "Failed to open: update event semaphore "
			             "\"%s\" rc = %d",
			        argv[1], rc);
			error_message(buf);
		}
	...
	if(!rc) {
		...
		if(rc) {
			sprintf(buf, "Failed to open: bmp mutex semaphore "
			             "\"%s\" rc = %d",
			        argv[1], rc);
			error_message(buf);
		}
...
init_bitmap(int argc, char* argv[])
{
	...
	if((rc = DosAllocMem((PPVOID)&bbuffer, length,
	                     PAG_READ | PAG_WRITE | PAG_COMMIT)) != (APIRET)0) {
	...
	DosClose(hf);
	if(rc) {
		sprintf(buf, "failed reading bitmap, error = %u, count = %u",
		        rc, count);
		error_message(buf);
		return rc;
	}
	...
	sprintf(buf, "bitmap width = %d, height = %d", bitmap.width,
	        bitmap.height);
	message_box(buf, 0);
	return rc;
}
...
make_palette(BMAP* pbm)
{
	...
	if(old_bmp) {
		...
		for(i = 0; i < palcount; i++) {
			tbl[i] = (palptr->bRed << 16) + (palptr->bGreen << 8) +
			         (palptr->bBlue);
			palptr++;
		}
	}
	...
	display.hpal =
	    GpiCreatePalette(hab, 0L, LCOLF_CONSECRGB, palcount, tbl);
...
scan_bitmap(BMAP* pbm)
{
	...
	if(old_bmp) {
		...
		default:
			pbm->valid = FALSE;
			error_message(
			    "scan_bitmap: wrong number of bits"); /* panic */
			return FALSE;
		}
	...
	if((pbm->palsize != pbm->old_palsize) ||
	   (pbm->palimportant != pbm->old_palimportant)) {
		if((pbm->depth == 8) && display.hasPalMan)
			make_palette(pbm);
		pbm->old_palimportant = pbm->palimportant;
	}
	if((pbm->width == pbm->old_width) && (pbm->height == pbm->old_height) &&
	   (pbm->planes == pbm->old_planes) && (pbm->depth == pbm->old_depth) &&
	   (pbm->palsize == pbm->old_palsize) && (pbm->old_bmp == old_bmp))
		return FALSE;
...
update_scroll_bars(void)
{
	...
	WinSendMsg(hwnd_bmp, WM_SIZE, MPFROM2SHORT(swp.cx, swp.cy),
...
copy_clipboard(void)
{
	...
	if(WinOpenClipbrd(hab)) {
		...
		hbmp = make_bitmap(&bitmap, 0, 0, bitmap.width, bitmap.height,
		                   bitmap.depth);
		if(hbmp) {
			WinEmptyClipbrd(hab);
			WinSetClipbrdData(hab, (ULONG)hbmp, CF_BITMAP,
			                  CFI_HANDLE);
		}
...
HBITMAP
make_bitmap(BMAP* pbm, ULONG left, ULONG bottom, ULONG right, ULONG top,
            ULONG depth)
{
	...
	if(hdcMem != DEV_ERROR)
		hps = GpiCreatePS(hab, hdcMem, &sizePS, PU_PELS | GPIF_DEFAULT |
		                                            GPIT_MICRO |
		                                            GPIA_ASSOC);
	...
	if(hbmr != HBM_ERROR) {
		...
		if(rc == 0) {
			...
			sprintf(
			    buf,
			    "make_bitmap: GpiDrawBits rc = %08x, eid = %08x",
			    rc, eid);
			message_box(buf, 0);
		}
	}
...
paint_bitmap(HPS ps, PRECTL prect, int scrollx, int scrolly)
{
	...
	if((display.bitcount == 4) /* standard VGA is buggy */
	   || ((os_version == 201100) && (display.bitcount == 8) &&
	       (bitmap.depth == 1)) /* S3 and ATI GU are buggy */
	   ) {
		/* slow code to dodge OS/2 bugs */
		/* this code double buffers the bitmap and works on a standard
		 * VGA
		 * but didn't work on an ATI Ultra Graphics Pro in 8514
		 * emulation
		 */
		/* This won't work for version 2.11, S3 or ATI GU, 8bit/pixel
		 * display, 8bit/pixel bitmap */
		HBITMAP hbmp;

		/* create a bitmap */
		hbmp = make_bitmap(&bitmap, apts[2].x, apts[2].y, apts[3].x,
		                   apts[3].y, bitmap.depth);
		/* Draw it to the display */
		if(hbmp) {
			WinDrawBitmap(ps, hbmp, NULL, &apts[0], CLR_BLACK,
			              CLR_WHITE, DBM_NORMAL);
			GpiDeleteBitmap(hbmp);
		}
	} else {
		/* fast code which doesn't always work */
		/* This code works on the Trident SVGA and 8514 in 256 color
		 * mode,
		 * but GpiDrawBits fails with a SYS3175 on the standard VGA.
		 */
		/* This won't work for version 2.11, S3 or ATI GU, 8bit/pixel
		 * display, 1bit/pixel bitmap */
		GpiDrawBits(ps, bitmap.bits, bitmap.pbmi, 4, apts,
		            (bitmap.depth != 1) ? ROP_SRCCOPY : ROP_NOTSRCCOPY,
		            0);
	}
...
MRESULT EXPENTRY
ClientWndProc(HWND hwnd, ULONG mess, MPARAM mp1, MPARAM mp2)
{
	char buf[256];
	...
	case WM_ERASEBACKGROUND:
		/* by returning TRUE, the Presentation Manager automatically
		 * clears
	...
	case WM_REALIZEPALETTE:
		if((bitmap.depth == 8) && display.hasPalMan &&
		   display.hpal_exists) {
			hps = WinGetPS(hwnd);
			if(hps == NULLHANDLE)
				debugbeep(1);
	...
	case WM_PAINT:
		...
		if((bitmap.depth == 8) && display.hasPalMan &&
		   display.hpal_exists) {
			...
			paint_bitmap(hps, &rect, nHscrollPos,
	...
	case WM_SIZE:
		...
		if((cxAdjust != 0 || cyAdjust != 0)) {
			...
			WinSetWindowPos(WinQueryWindow(hwnd, QW_PARENT), 0,
			                ...
			                swp.cx + cxAdjust, swp.cy + cyAdjust,
			                SWP_SIZE | SWP_MOVE);
			cxAdjust = cyAdjust = 0;
		}
	...
	case WM_VSCROLL:
		...
		if((nVscrollInc = max(
			...
			scroll_pos.y = nVscrollMax - nVscrollPos;
			lComplexity = WinScrollWindow(
			    hwnd, 0, nVscrollInc, (PRECTL)NULL, (PRECTL)NULL,
			    (HRGN)NULLHANDLE, (PRECTL)&rect, 0);
			WinSendMsg(hwndScroll, SBM_SETPOS,
			           MPFROMLONG(nVscrollPos), 0);
	...
	case WM_HSCROLL:
		...
		if((nHscrollInc = max(
			...
			scroll_pos.x = nHscrollPos;
			lComplexity = WinScrollWindow(
			    hwnd, -nHscrollInc, 0, (PRECTL)NULL, (PRECTL)NULL,
			    (HRGN)NULLHANDLE, (PRECTL)&rect, 0);
			/* need to send next message BEFORE redrawing, otherwise
			 * S3 driver screws up */
			WinSendMsg(hwndScroll, SBM_SETPOS,
			           MPFROMLONG(nHscrollPos), 0);
			if(lComplexity != RGN_RECT) {
				WinInvalidateRect(hwnd, (PRECTL)NULL, FALSE);
				WinUpdateWindow(hwnd);
			} else {
