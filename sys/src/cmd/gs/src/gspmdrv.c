...
init_bitmap(int argc, char *argv[])
{
	...
	if((rc = DosOpen(argv[2], &hf, &action, 0, FILE_NORMAL, FILE_OPEN,
			 OPEN_ACCESS_READONLY | OPEN_SHARE_DENYREADWRITE, 0)) != (APIRET)0) {
	...
	if((rc = DosAllocMem((PPVOID)&bbuffer, length, PAG_READ | PAG_WRITE | PAG_COMMIT)) != (APIRET)0) {
...
scan_bitmap(BMAP *pbm)
{
	...
	if(old_bmp) {
		...
		if((pbmi->cbFix > (&(pbmi->cclrUsed) - &(pbmi->cbFix))) && (pbmi->cclrUsed != 0) && (pbmi->cBitCount != 24))
			pbm->palsize = pbmi->cclrUsed;
		pbm->palimportant = pbm->palsize;
		if((pbmi->cbFix > (&(pbmi->cclrImportant) - &(pbmi->cbFix))) && (pbmi->cclrImportant != 0) && (pbmi->cBitCount != 24))
