...
/* Copyright (C) 1992, 1993, 1994, 1996, 1997, 1998, 1999, 2000 Aladdin
...
private
dev_proc_get_bits(pm_get_bits);
private
dev_proc_get_params(pm_get_params);
private
dev_proc_put_params(pm_put_params);

private
gx_device_procs pm_procs = {
    pm_open, pm_get_initial_matrix, pm_sync_output, pm_output_page, pm_close,
    pm_map_rgb_color, pm_map_color_rgb, pm_fill_rectangle,
    NULL,                                            /* tile rectangle */
    pm_copy_mono, pm_copy_color, NULL,               /* draw line */
    pm_get_bits, pm_get_params, pm_put_params, NULL, /* map_cmyk_color */
...
gx_device_pm far_data gs_os2dll_device = {
    std_device_std_body(gx_device_pm, &pm_procs, "os2dll", INITIAL_WIDTH,
                        INITIAL_HEIGHT, INITIAL_RESOLUTION, INITIAL_RESOLUTION),
    {0},  /* std_procs */
    24,   /* BitsPerPixel */
    5000, /* UpdateInterval */
    "\0", /* GSVIEW */
...
gx_device_pm far_data gs_os2pm_device = {
    std_device_std_body(gx_device_pm, &pm_procs, "os2pm", INITIAL_WIDTH,
                        INITIAL_HEIGHT, INITIAL_RESOLUTION, INITIAL_RESOLUTION),
    {0},  /* std_procs */
    24,   /* BitsPerPixel */
    5000, /* UpdateInterval */
...
private
void pm_makepalette(gx_device_pm*);
private
void pm_update(gx_device_pm*);
private
uint pm_set_bits_per_pixel(gx_device_pm*, int);
private
uint pm_palette_size(gx_device_pm*);
private
int pm_alloc_bitmap(gx_device_pm*, gx_device*);
private
int pm_run_gspmdrv(gx_device_pm*);
private
void pm_write_bmp(gx_device_pm*);
...
/* possible bitmap (13Mbytes = A3 x 150dpi x 24bits) */
	...
	if(pmdev->dll) {
		...
		if(DosAllocMem((PPVOID)&pmdev->bitmap, 13 * 1024 * 1024,
		               PAG_READ | PAG_WRITE)) {
...
#endif
	{
		...
		if(DosAllocSharedMem((PPVOID)&pmdev->bitmap, name,
			...
			eprintf1(
			    "pm_open: failed allocating shared BMP memory %s\n",
			    name);
			return gs_error_limitcheck;
		}
	}
	...
	if(pmdev->dll) {
		/* Create mutex - used for preventing another thread from
		 * accessing */
		/* bitmap while we are changing the bitmap size. Initially
		 * unowned. */
		...
		if(DosCreateMutexSem(name, &(pmdev->bmp_mutex), 0, FALSE)) {
			...
			eprintf1(
			    "pm_open: failed to create mutex semaphore %s\n",
			    name);
			return gs_error_limitcheck;
		}
	} else {
		if(*pmdev->GSVIEW) {
			...
			if(!rc) {
				...
				rc = DosOpenQueue(&owner_pid, &pmdev->drv_queue,
				                  name);
			}
			if(rc) {
				DosFreeMem(pmdev->bitmap);
			...
			if(DosCreateEventSem(name, &(pmdev->sync_event), 0,
			                     FALSE)) {
				DosFreeMem(pmdev->bitmap);
				eprintf1("pm_open: failed to create event "
				         "semaphore %s\n",
				         name);
				return gs_error_limitcheck;
			}
			/* Create mutex - used for preventing gspmdrv from
			 * accessing */
			/* bitmap while we are changing the bitmap size.
			 * Initially unowned. */
			sprintf(name, MUTEX_NAME, id);
			if(DosCreateMutexSem(name, &(pmdev->bmp_mutex), 0,
			                     FALSE)) {
				DosFreeMem(pmdev->bitmap);
				DosCloseEventSem(pmdev->sync_event);
				DosCloseQueue(pmdev->drv_queue);
				eprintf1("pm_open: failed to create mutex "
				         "semaphore %s\n",
				         name);
				return gs_error_limitcheck;
			}
	...
	pmdev->bmi->cbImage = 0;
	pmdev->bmi->cxResolution =
	    (ULONG)(dev->x_pixels_per_inch / 25.4 * 1000);
	pmdev->bmi->cyResolution =
	    (ULONG)(dev->y_pixels_per_inch / 25.4 * 1000);
...
pm_get_initial_matrix(gx_device* dev, gs_matrix* pmat)
{
	...
	if(*pmdev->GSVIEW)
		pm_update(
		    (gx_device_pm*)dev); /* let GSVIEW know we are drawing */
}
...
pm_do_output_page(gx_device* dev, int copies, int flush)
{
	...
	if(*pmdev->GSVIEW) {
		if(copies == -2) {
			rc =
			    DosWriteQueue(pmdev->drv_queue, GS_END, 0, NULL, 0);
			if(rc)
				eprintf1(
				    "pm_output_page: DosWriteQueue error %d\n",
				    rc);
		} else if(copies == -1) {
			rc = DosWriteQueue(pmdev->drv_queue, GS_BEGIN, 0, NULL,
			                   0);
			if(rc)
				eprintf1(
				    "pm_output_page: DosWriteQueue error %d\n",
			...
			rc = DosWriteQueue(pmdev->drv_queue, GS_PAGE, 0, NULL,
			                   0);
			if(rc)
				eprintf1(
				    "pm_output_page: DosWriteQueue error %d\n",
		...
		if(rc) {
			...
			if(rc == ERROR_SMG_NO_TARGET_WINDOW) {
				DosSleep(
				    5000); /* give gspmdrv.exe a chance to run
				              */
				rc = DosSelectSession(
				    pmdev->session_id); /* try yet again */
			}
			if((rc == ERROR_SMG_SESSION_NOT_FOUND) ||
			   (rc == ERROR_SMG_INVALID_SESSION_ID)) {
				...
				Request.ulData = 0;
				/* wait for termination queue, queue is then
				 * closed by session manager */
				DosReadQueue(pmdev->term_queue, &Request,
				             &DataLength, &DataAddress, 0,
				             DCWW_WAIT, &ElemPriority,
				             (HEV)NULL);
				...
				DosSleep(
				    2000); /* give gspmdrv.exe a chance to run
				              */
				rc = DosSelectSession(pmdev->session_id);
			}
			if(rc == ERROR_SMG_SESSION_NOT_FOREGRND)
				...
				eprintf1("pm_output_page: Select Session error "
				         "code %u\n",
				         rc);
		}
	}
...
pm_close(gx_device* dev)
{
	...
	if(pmdev->dll) {
		...
		if(pgsdll_callback)
			(*pgsdll_callback)(GSDLL_DEVICE, (unsigned char*)dev,
			                   0);
	...
	{
		if(*pmdev->GSVIEW) {
			rc = DosWriteQueue(pmdev->drv_queue, GS_CLOSE, 0, NULL,
			                   0);
			if(rc)
			...
			Request.pid = pmdev->gspid;
			Request.ulData = 0;
			/* wait for termination queue, queue is then closed by
			 * session manager */
			DosReadQueue(pmdev->term_queue, &Request, &DataLength,
			             &DataAddress, 0, DCWW_WAIT, &ElemPriority,
			             (HEV)NULL);
	...
	case 8: {
		...
		if(i < 230) { /* allow 26 for PM and other apps */
			...
			if(*pmdev->GSVIEW) {
				...
				rc = DosWriteQueue(pmdev->drv_queue,
				                   GS_PALCHANGE, 0, NULL, 0);
				if(rc)
					eprintf1("pm_sync_output: "
					         "DosWriteQueue error %d\n",
					         rc);
			}
			return (
			    (gx_color_index)i); /* return new palette index */
		}
...
int
pm_map_color_rgb(gx_device* dev, gx_color_index color, gx_color_value prgb[3])
{
	gx_color_value one;
	...
	case 8:
		...
		{
			RGB2* argb =
			    (RGB2*)((PBYTE)pmdev->bmi + pmdev->bmi->cbFix);
...
private
int
pm_copy_mono(gx_device* dev, const byte* base, int sourcex, int raster,
             gx_bitmap_id id, int x, int y, int w, int h, gx_color_index zero,
             gx_color_index one)
{
	pmmproc(copy_mono)(pmmdev, base, sourcex, raster, id, x, y, w, h, zero,
	                   one);
	pm_update((gx_device_pm*)dev);
	return 0;
}
...
private
int
pm_copy_color(gx_device* dev, const byte* base, int sourcex, int raster,
              gx_bitmap_id id, int x, int y, int w, int h)
{
	pmmproc(copy_color)(pmmdev, base, sourcex, raster, id, x, y, w, h);
	pm_update((gx_device_pm*)dev);
	return 0;
}
...
pm_get_params(gx_device* dev, gs_param_list* plist)
{
	...
	code < 0 ||
	    (code = param_write_int(plist, "UpdateInterval",
	                            &pmdev->UpdateInterval)) < 0 ||
...
pm_put_params(gx_device* dev, gs_param_list* plist)
{
	...
	default:
		ecode = code;
	uie:
		param_signal_error(plist, "UpdateInterval", ecode);
	case 1:
		break;
	}
	...
	if(ecode >=
	   0) { /* Prevent gx_default_put_params from closing the device. */
		dev->is_open = false;
	...
	if(is_open &&
	   (old_bpp != bpp || dev->width != width || dev->height != height)) {
		int ccode;

		ccode = pm_alloc_bitmap(pmdev, dev);
		if(ccode <
		   0) { /* Bad news!  Some of the other device parameters */
	...
	if(dev->is_open && reopen) {
		...
		/* erase bitmap - before window gets redrawn */
		{
			...
			for(i = 0; i < GX_DEVICE_COLOR_MAX_COMPONENTS; i++)
				cv[i] = (pmdev->color_info.polarity ==
				         GX_CINFO_POLARITY_ADDITIVE)
				            ? gx_max_color_value
				            : 0;
			dev_proc(pmdev, fill_rectangle)(
			    (gx_device*)pmdev, 0, 0, pmdev->width,
			    pmdev->height,
...
pm_run_gspmdrv(gx_device_pm* pmdev)
{
	...
	if((rc = DosGetInfoBlocks(&pptib, &pppib)) != 0) {
		eprintf1(
		    "pm_run_gspmdrv: Couldn't get module handle, rc = %d\n",
		    rc);
		return gs_error_limitcheck;
	}
	...
	sdata.InheritOpt =
	    0; /* Can't inherit from parent because different sesison type */
	...
	if(rc == ERROR_FILE_NOT_FOUND) {
		...
		rc = DosStartSession(&sdata, &pmdev->session_id,
		                     &pmdev->process_id);
	}
	if(rc) {
...
pm_alloc_bitmap(gx_device_pm* pmdev, gx_device* param_dev)
{
	...
	gs_make_mem_device(&mdev,
	                   gdev_mem_device_for_bits(pmdev->color_info.depth), 0,
	                   0, (gx_device*)pmdev);
...
/* Shared memory can't be decommitted */
	...
	if(pmdev->dll && (needed < pmdev->committed)) {
		...
		if(rc = DosSetMem(pmdev->bitmap + needed,
		                  pmdev->committed - needed, PAG_DECOMMIT)) {
			eprintf1("Failed to decommit memory in "
			         "pm_alloc_bitmap, rc = %d\n",
			         rc);
			return gs_error_limitcheck;
		}
		pmdev->committed = needed;
...
pm_makepalette(gx_device_pm* pmdev)
{
	...
	for(i = 0; i < pmdev->nColors; i++) {
		...
		case 64:
			...
			argb[i].fcOptions = 0;
			/* zero unused entries */
			argb[i + 64].bRed = argb[i + 64].bGreen =
			    argb[i + 64].bBlue = 0;
			argb[i + 64].fcOptions = 0;
			argb[i + 128].bRed = argb[i + 128].bGreen =
			    argb[i + 128].bBlue = 0;
			argb[i + 128].fcOptions = 0;
			argb[i + 192].bRed = argb[i + 192].bGreen =
			    argb[i + 192].bBlue = 0;
		...
		case 16:
			...
			if(i == 8) { /* light gray */
				argb[i].bRed = argb[i].bGreen = argb[i].bBlue =
				    192;
				argb[i].fcOptions = 0;
			}
			break;
...
private
void
set_color_info(gx_device_color_info* pdci, int nc, int depth, int maxgray,
               int maxcolor)
{
