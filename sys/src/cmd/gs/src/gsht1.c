...
private
int process_spot(gx_ht_order *, gs_state *,
		 gs_spot_halftone *, gs_memory_t *);
private
int process_threshold(gx_ht_order *, gs_state *,
		      gs_threshold_halftone *, gs_memory_t *);
private
int process_threshold2(gx_ht_order *, gs_state *,
		       gs_threshold2_halftone *, gs_memory_t *);
private
int process_client_order(gx_ht_order *, gs_state *,
		...
		return code;
	{
		...
		for(row = 0, di = 0; row < d; ++row) {
			...
			for(dx = 0; dx < sod; dx += w) {
				...
				for(j = 0; j < w; ++j, ++si, ++di) {
					uint thr =
					    (bps == 1 ? data[si] : (data[si * 2] << 8) + data[si * 2 + 1]) >> rshift;
