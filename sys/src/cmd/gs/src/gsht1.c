...
private
int process_spot(gx_ht_order*, gs_state*, gs_spot_halftone*, gs_memory_t*);
private
int process_threshold(gx_ht_order*, gs_state*, gs_threshold_halftone*,
                      gs_memory_t*);
private
int process_threshold2(gx_ht_order*, gs_state*, gs_threshold2_halftone*,
                       gs_memory_t*);
private
int process_client_order(gx_ht_order*, gs_state*, gs_client_order_halftone*,
                         gs_memory_t*);
private
int gs_sethalftone_try_wts(gs_halftone* pht, gs_state* pgs,
...
private
ENUM_PTRS_WITH(halftone_component_enum_ptrs,
               gs_halftone_component* hptr) return 0;
case 0:
switch(hptr->type) {
case ht_type_spot:
...
ENUM_PTRS_END
private
RELOC_PTRS_WITH(halftone_component_reloc_ptrs, gs_halftone_component* hptr)
{
	switch(hptr->type) {
	case ht_type_spot:
                       ...
                       gx_device_halftone* pdht)
{
	...
	case ht_type_colorscreen: {
		...
		static const char* color_names[4] = {"Gray", "Red", "Green",
		                                     "Blue"};
		int i;

		pocs = gs_alloc_struct_array(mem, 4, gx_ht_order_component,
		...
		for(i = 0; i < 4; i++) {
			...
			code = gx_ht_process_screen_memory(
			    &senum, pgs, &phc[ci], gs_currentaccuratescreens(),
			    mem);
			if(code < 0)
				break;
			poc->corder = senum.order;
			poc->comp_number = gs_color_name_component_number(
			    pgs->device, color_names[i], strlen(color_names[i]),
			    pht->type);
			poc->cname =
			    0; /* name index values are not known (or needed) */
			if(i == 0) /* Gray = Default */
				pdht->order =
				    poc->corder; /* Save default value */
		}
		if(code < 0)
			break;
	...
	case ht_type_multiple_colorscreen: {
		...
		pocs = gs_alloc_struct_array(mem, count, gx_ht_order_component,
		                             &st_ht_order_component_element,
		                             "gs_sethalftone");
		if(pocs == 0)
			return_error(gs_error_VMerror);
		...
		for(i = 0; i < count; i++, phc++) {
			...
			case ht_type_threshold:
				...
				break;
			case ht_type_threshold2:
				code = process_threshold2(
				    &poc->corder, pgs, &phc->params.threshold2,
				    mem);
				break;
			case ht_type_client_order:
				code = process_client_order(
				    &poc->corder, pgs,
...
private
int
process_transfer(gx_ht_order* porder, gs_state* pgs, gs_mapping_proc proc,
                 gs_mapping_closure_t* pmc, gs_memory_t* mem)
{
	...
	rc_alloc_struct_1(pmap, gx_transfer_map, &st_transfer_map, mem,
	                  return_error(gs_error_VMerror), "process_transfer");
...
private
int
process_spot(gx_ht_order* porder, gs_state* pgs, gs_spot_halftone* phsp,
             gs_memory_t* mem)
{
	gs_screen_enum senum;
...
#define MAX_HT_LEVELS (1 << LOG2_MAX_HT_LEVELS)
...

	{
		...
		for(i = 0; i < size; ++i) {
			uint thr = (bps == 1 ? data[i] : (data[i * 2] << 8) +
			                                     data[i * 2 + 1]);
		...
		return code;
	{
		...
		if_debug7('h', "[h]rect1=(%d,%d), rect2=(%d,%d), "
		               "strip=(%d,%d), shift=%d\n",
		...
		for(row = 0, di = 0; row < d; ++row) {
			...
			for(dx = 0; dx < sod; dx += w) {
				/* Iterate within a destination row, over source
				 * rows. */
				int si, j;
	...
	return process_transfer(porder, pgs, NULL, &phtp->transfer_closure,
	                        mem);
                     ...
                     gs_client_order_halftone* phcop, gs_memory_t* mem)
{
	...
	if(code < 0)
		return code;
	return process_transfer(porder, pgs, NULL, &phcop->transfer_closure,
	                        mem);
}

private
const gx_ht_order_procs_t wts_order_procs = {0};
                       ...
                       gx_device_halftone* pdht)
{
	...
	if(pht->type == ht_type_multiple) {
		gs_halftone_component* components =
		    pht->params.multiple.components;
		...
		for(i = 0; i < num_comp; i++) {
			if(components[i].type != ht_type_spot)
				...
				gs_spot_halftone* spot =
				    &components[i].params.spot;
				if(!spot->accurate_screens)
					return 1;
			}
		}

		pocs = gs_alloc_struct_array(
		    pgs->memory, num_comp, gx_ht_order_component,
		    &st_ht_order_component_element, "gs_sethalftone_try_wts");
		...
		for(i = 0; i < num_comp; i++) {
			...
			code = process_transfer(
			    &poc->corder, pgs, spot->transfer,
			    &spot->transfer_closure, pgs->memory);
			if(code < 0)
				break;
		}
