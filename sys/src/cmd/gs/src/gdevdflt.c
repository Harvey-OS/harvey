...
private
gx_color_index (*get_encode_color(gx_device* dev))(gx_device*,
                                                   const gx_color_value*)
{
	...
	if(encode_proc == 0) {
		if(dev->color_info.num_components == 1 &&
		   dev->color_info.depth != 0) {
			set_cinfo_polarity(dev, GX_CINFO_POLARITY_ADDITIVE);
			if(dev->color_info.max_gray ==
			   (1 << dev->color_info.depth) - 1)
...
is_like_DeviceRGB(gx_device* dev)
{
	...
	cm_procs->map_rgb(dev, 0, frac_1 / 4, frac_1 / 3, 3 * frac_1 / 4,
	                  cm_comp_fracs);
	...
	for(i = 0; i < 3 && -frac_1 / 100 < cm_comp_fracs[i] &&
...
is_like_DeviceCMYK(gx_device* dev)
{
	...
	cm_procs->map_cmyk(dev, frac_1 / 4, frac_1 / 3, 3 * frac_1 / 4,
	                   frac_1 / 8, cm_comp_fracs);
	...
	for(i = 0; i < 4 && -frac_1 / 100 < cm_comp_fracs[i] &&
	               cm_comp_fracs[i] < frac_1 / 100;
	    i++)
		;
	return i == 4;
...
private
int
gx_default_1_add_decode_color(gx_device* dev, gx_color_index color,
                              gx_color_value cv[1])
{
...
private
int
gx_default_1_sub_decode_color(gx_device* dev, gx_color_index color,
                              gx_color_value cv[1])
{
	gx_color_value rgb[3];
	int code = dev_proc(dev, map_color_rgb)(dev, color, rgb);
...
private
int
gx_default_cmyk_decode_color(gx_device* dev, gx_color_index color,
                             gx_color_value cv[4])
{
...
private
int
gx_1bit_cmyk_decode_color(gx_device* dev, gx_color_index color,
                          gx_color_value cv[4])
{
...
private
int (*get_decode_color(gx_device* dev))(gx_device*, gx_color_index,
                                        gx_color_value*)
{
	...
	/*
	 * If a map_color_rgb method has been provided, we may be able to use
	 * it.
	 * Currently this will always be the case, as a default value will be
	 * provided this method. While this default may not be correct, we are
	 * not
	...
	if(dev_proc(dev, map_color_rgb) != 0) {
		...
		if(dev->color_info.num_components == 1 &&
		   dev->color_info.gray_index == 0)
			return dev->color_info.polarity ==
		 ...
		 * arises with some frequency, so it is useful not to generate
		 *an
		 * error in this case. The mechanism below assumes full
		 *undercolor
		 * removal and black generation, which may not be accurate but
		 *are
		 * the  best that can be done in the general case in the absence
		 *of
		 * other information.
		 *
		 * As a hack to handle certain common devices, if the
		 *map_rgb_color
		 * routine is cmyk_1bit_map_color_rgb, we provide a direct
		 *one-bit
		 * decoder.
		 */
		if(is_like_DeviceCMYK(dev)) {
			if(dev_proc(dev, map_color_rgb) ==
...
#define comp_shift (dev->color_info.comp_shift)
	...
	for(i = num_components - 1 - 1; i >= 0; i--) {
		comp_shift[i] = comp_shift[i + 1] +
		                (i == gray_index ? ilog2(max_gray + 1)
		                                 : ilog2(max_color + 1));
	}
	for(i = 0; i < num_components; i++) {
		comp_bits[i] = (i == gray_index ? ilog2(max_gray + 1)
		                                : ilog2(max_color + 1));
...
check_device_separable(gx_device* dev)
{
	...
	if(pinfo->gray_index < num_components &&
	   (!pinfo->dither_grays ||
	    pinfo->dither_grays != (pinfo->max_gray + 1)))
		return;
	if((num_components > 1 || pinfo->gray_index != 0) &&
	   (!pinfo->dither_colors ||
	    pinfo->dither_colors != (pinfo->max_color + 1)))
		return;
	/*
	...
	for(i = 0; i < num_components; i++) {
		...
		for(j = 0; color_index != 0; j++) {
			if((color_index & 1) ==
			   0) /* check for non-consecutive bits */
				return;
			color_index >>= 1;
		}
		comp_bits[i] = j;
		/*
		 * We could verify that the bit count matches the dither_grays
		 * or
		 * dither_colors values, but this is not really required unless
		 * we
	 ...
	 * an appropriate value, we are setting its value based upon the data
	 * that
	 * we just determined.  Note:  In some cases the macros set max_gray to
	 * 0
...
#define CHECK_NON_DEFAULT(proc, default, procname)                             \
	...
	if(dev_proc(dev, proc) != NULL && dev_proc(dev, proc) != default)      \
		dprintf2("**** Warning: device %s implements obsolete "        \
		         "procedure %s\n",                                     \
		         dev->dname, procname);                                \
	END
#else
#define CHECK_NON_DEFAULT(proc, default, procname) DO_NOTHING
#endif
...
#undef CHECK_NON_DEFAULT
	...
	case 1: /* DeviceGray or DeviceInvertGray */
		if(dev_proc(dev, get_color_mapping_procs) == NULL) {
			 ...
			 * mapping procs.
			 */
			if(dev->color_info.polarity ==
			   GX_CINFO_POLARITY_ADDITIVE) {
				fill_dev_proc(
				    dev, get_color_mapping_procs,
	...
	default: /* Unknown color model - set error handlers */
		fill_dev_proc(dev, get_color_mapping_procs,
		              gx_error_get_color_mapping_procs);
		fill_dev_proc(dev, get_color_comp_index,
		              gx_error_get_color_comp_index);
	}
...
gx_default_create_compositor(gx_device* dev, gx_device** pcdev,
                             ...
                             gs_memory_t* memory)
{
	return pcte->type->procs.create_default_compositor(pcte, pcdev, dev,
	                                                   pis, memory);
}
int
...
int
gx_default_composite_clist_write_update(const gs_composite_t* pcte,
                                        gx_device* dev, gx_device** pcdev,
...
int
gx_default_fill_rectangle_hl_color(gx_device* pdev, const gs_fixed_rect* rect,
