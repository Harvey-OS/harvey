...
private
gx_color_index (*get_encode_color(gx_device *dev))(gx_device *, const gx_color_value *)
{
...
private
int (*get_decode_color(gx_device *dev))(gx_device *, gx_color_index, gx_color_value *)
{
...
#define comp_shift (dev->color_info.comp_shift)
	...
	for(i = 0; i < num_components; i++) {
		comp_bits[i] = (i == gray_index ? ilog2(max_gray + 1) : ilog2(max_color + 1));
...
check_device_separable(gx_device *dev)
{
     ...
     */
	if(!is_power_of_two(pinfo->dither_grays) || !is_power_of_two(pinfo->dither_colors))
		return;
	/*
...
{
	return (type == go_text ? dev->color_info.anti_alias.text_bits : dev->color_info.anti_alias.graphics_bits);
}
...
gx_default_create_compositor(gx_device *dev, gx_device **pcdev,
			     ...
			     gs_imager_state *pis, gs_memory_t *memory)
{
	return pcte->type->procs.create_default_compositor(pcte, pcdev, dev, pis, memory);
}
int
