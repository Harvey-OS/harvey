...
/* Copyright (C) 1989-1994, 1998, 1999 Aladdin Enterprises.  All rights
  reserved.
...
/* declare functions */
private
dev_proc_print_page(lxm5700m_print_page);
private
dev_proc_get_params(lxm_get_params);
private
dev_proc_put_params(lxm_put_params);
...
lxm_device far_data gs_lxm5700m_device = {
    prn_device_std_body(lxm_device, lxm5700m_procs, "lxm5700m",
                        DEFAULT_WIDTH_10THS, DEFAULT_HEIGHT_10THS, 600,
                        600,                /* x dpi, y dpi */
 ...
 * directory indicates that a bitmap for that sector follows.) In the worst
 * case,
 ...
 * addressed by even and odd columns are reversed.  I think the printheads might
 * be
...
#define init1()                                                                \
	top(), 0xA5, 0, 3, 0x40, 4, 5, 0xA5, 0, 3, 0x40, 4, 6, 0xA5, 0, 3,     \
	    0x40, 4, 7, 0xA5, 0, 3, 0x40, 4, 8, 0xA5, 0, 4, 0x40, 0xe0, 0x0b,  \
	    3

#define init2()                                                                \
	0xA5, 0, 11, 0x40, 0xe0, 0x41, 0, 0, 0, 0, 0, 0, 0, 2, 0xA5, 0, 6,     \
	    0x40, 5, 0, 0, 0x80, 0

#define init3()                                                                \
	0x1b, '*', 7, 0x73, 0x30, 0x1b, '*', 'm', 0, 0x14, 3, 0x84, 2, 0, 1,   \
	    0xf4, 0x1b, '*', 7, 0x63, 0x1b, '*', 'm', 0, 0x42, 0, 0, 0xA5, 0,  \
	    5, 0x40, 0xe0, 0x80, 8, 7, 0x1b, '*', 'm', 0, 0x40, 0x15, 7, 0x0f, \
	    0x0f

#define top() 0xA5, 0, 6, 0x40, 3, 3, 0xc0, 0x0f, 0x0f

#define fin() 0x1b, '*', 7, 0x65
...
lxm5700m_print_page(gx_device_printer* pdev, FILE* prn_stream)
{
	...
	byte* buf1 =
	    (byte*)gs_malloc(pdev->memory, in_size, 1, "lxm_print_page(buf1)");
	byte* swipeBuf = (byte*)gs_malloc(pdev->memory, swipeBuf_size, 1,
	                                  "lxm_print_page(swipeBuf)");
	byte* in = buf1;
	...
	if(buf1 == 0 || swipeBuf == 0) {
		if(buf1)
		quit_ignomiously
		    : /* and a goto into an if statement is pretty ignomious! */
		gs_free(pdev->memory, (char*)buf1, in_size, 1,
		        "lxm_print_page(buf1)");
		if(swipeBuf)
			gs_free(pdev->memory, (char*)swipeBuf, swipeBuf_size, 1,
			        "lxm_print_page(swipeBuf)");
		return_error(gs_error_VMerror);
	}

	{ /* Initialize the printer and reset the margins. */
		static const char init_string[] = {init1(), init2(), init3()};
		fwrite(init_string, 1, sizeof(init_string), prn_stream);
	}
	/* Print lines of graphics */
	for(lnum = 0;
	    lnum < pdev->height - swipeHeight;) { /* increment in body */
		byte* in_data;
		register byte* outp;
		int lcnt;
		...
		{ /* test for blank scan lines.  We  maintain the */
			...
			for(l = lnum; l < pdev->height; l++) {
				...
				if(in_data[0] != 0 ||
				   memcmp((char*)in_data, (char*)in_data + 1,
				          line_size - 1)) {
					break;
				}
			} /* end for l */
			...
			if(l >= pdev->height) { /* if there are no more bits on
			                           this page */
				lnum = l;
				break; /* end the loop and eject the page*/
			}

			/* leave room for following swipe to reinforce these
			 * bits */
			if(l - lnum > overLap)
				lnum = l - overLap;
		...
		for(l = 0; l < swipeHeight; l++) { /* for each line of swipe */
			for(i = 0; i < minX;
			    i++) { /* look for left-most non-zero byte*/
				if(in[l * line_size + i] != 0) {
					minX = i;
					break;
				}
			}
			for(i = line_size - 1; i >= maxX;
			    i--) { /* look for right-most */
				if(in[l * line_size + i] != 0) {
					maxX = i;
					break;
				}
...
#define buffer_store(x)                                                        \
	if(outp - swipeBuf >= swipeBuf_size) {                                 \
		gs_free(pdev->memory, (char*)swipeBuf, swipeBuf_size, 1,       \
		        "lxm_print_page(swipeBuf)");                           \
		swipeBuf_size *= 2;                                            \
		swipeBuf = (byte*)gs_malloc(pdev->memory, swipeBuf_size, 1,    \
		                            "lxm_print_page(swipeBuf)");       \
		if(swipeBuf == 0)                                              \
			goto quit_ignomiously;                                 \
		break;                                                         \
	} else                                                                 \
	*outp++ = (x)
		...
		{ /* work out the bytes to store for this swipe*/
			...
			/* want to set up pointers for (upto two) stripes
			...
			for(x = leastX; x <= highestX; x++) {
				...
				case(RIGHTWARD):
					sx = (x & 1) == 1
					         ...
					         : x - (((lxm_device*)pdev)
					                    ->headSeparation);
					j1 = (x &
					      1); /* even if x even, odd if x
					             odd */
					break;
				default
				    : /* shouldn't happen ... but compilation
				         checks */
				case(LEFTWARD):
					sx = (x & 1) == 0
					         ? x
				...
				/* loop through all the swipeHeight bits of this
				 * column */
				for(i = 0, b = 1, y = sxBy8 + j1 * line_size;
				    i < directorySize; i++, b <<= 1) {
					sum = false;
					for(j = j1,
					c = c1 /*,y=i*16*line_size+sxBy8*/;
					    j < 16; j += 2, y += 2 * line_size,
					c >>= 2) {
						f = (in[y] & sxMask);
						if(f) {
							words[i] |= c;
				...
				retval += 2;
				buffer_store(directory >> 8);
				buffer_store(directory & 0xff);
				if(directory != 0x3fff) {
					for(i = 0; i < directorySize; i++) {
						if(words[i] != 0) {
							buffer_store(words[i] >>
							             8);
							buffer_store(words[i] &
							             0xff);
							retval += 2;
...
#undef buffer_store
		...
		{ /* now write out header, then buffered bits */
			...
			int deltaY =
			    2 *
			    (leastY -
			     lastY); /* vert coordinates here are 1200 dpi */
			lastY = leastY;
			outByte(0x1b);
			outByte('*');
			outByte(3);
...
lxm_get_params(gx_device* pdev, gs_param_list* plist)
{
	...
	if(code < 0)
		return code;
	code = param_write_int(plist, "HeadSeparation",
...
lxm_put_params(gx_device* pdev, gs_param_list* plist)
{
	...
	int code =
	    param_read_int(plist, "HeadSeparation", &trialHeadSeparation);

	if(trialHeadSeparation < 1 || trialHeadSeparation > 32)
		param_signal_error(plist, "HeadSeparation",
		                   gs_error_rangecheck);
	/* looks like param_signal_error is not expected to return */
	ecode =
	    gdev_prn_put_params(pdev, plist); /* call super class put_params */
	if(code < 0)
		return code;
	if(ecode < 0)
		return ecode;
	...
	if(code == 1)
		return ecode; /* I guess this means there is no "HeadSeparation"
		                 parameter */
	return 0;
}
