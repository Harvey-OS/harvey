...
#define cmd_read(ptr, rsize, cbp) cbp = cmd_read_data(&cbuf, ptr, rsize, cbp)
...
#define cmd_get_value(var, cbp) cbp = cmd_copy_value(&var, sizeof(var), cbp)
 ...
 */
private
int read_set_tile_size(command_buf_t* pcb, tile_slot* bits);
private
int read_set_bits(command_buf_t* pcb, tile_slot* bits, int compress,
                  gx_clist_state* pcls, gx_strip_bitmap* tile,
                  tile_slot** pslot, gx_device_clist_reader* cdev,
                  gs_memory_t* mem);
private
int read_set_misc2(command_buf_t* pcb, gs_imager_state* pis,
                   segment_notes* pnotes);
private
int read_set_color_space(command_buf_t* pcb, gs_imager_state* pis,
                         const gs_color_space** ppcs,
                         gs_color_space* pcolor_space, gs_memory_t* mem);
private
int read_begin_image(command_buf_t* pcb, gs_image_common_t* pic,
                     const gs_color_space* pcs);
private
int read_put_params(command_buf_t* pcb, gs_imager_state* pis,
                    gx_device_clist_reader* cdev, gs_memory_t* mem);
private
int read_create_compositor(command_buf_t* pcb, gs_imager_state* pis,
                           gx_device_clist_reader* cdev, gs_memory_t* mem,
                           gx_device** ptarget);
private
int read_alloc_ht_buff(ht_buff_t*, uint, gs_memory_t*);
private
int read_ht_segment(ht_buff_t*, command_buf_t*, gs_imager_state*, gx_device*,
                    gs_memory_t*);

private
const byte* cmd_read_rect(int, gx_cmd_rect*, const byte*);
private
const byte* cmd_read_matrix(gs_matrix*, const byte*);
private
const byte* cmd_read_short_bits(command_buf_t* pcb, byte* data, int width_bytes,
                                int height, uint raster, const byte* cbp);
private
int cmd_select_map(cmd_map_index, cmd_map_contents, gs_imager_state*, int**,
                   frac**, uint*, gs_memory_t*);
private
int cmd_create_dev_ht(gx_device_halftone**, gs_memory_t*);
private
int cmd_resize_halftone(gx_device_halftone**, uint, gs_memory_t*);
private
int clist_decode_segment(gx_path*, int, fixed[6], gs_fixed_point*, int, int,
                         segment_notes);
private
int clist_do_polyfill(gx_device*, gx_path*, const gx_drawing_color*,
	...
	while(code >= 0) {
		...
		if(gs_debug_c('L')) {
			...
			if(sub)
				...
				dlprintf2("[L]%s %d:", cmd_op_names[op >> 4],
				          op & 0xf);
		}
#endif
		switch(op >> 4) {
		case cmd_op_misc >> 4:
			...
			case cmd_opv_set_bits:
				...
				if_debug4('L',
				          " compress=%d depth=%d size=(%d,%d)",
				          compress, bits.cb_depth, bits.width,
				          bits.height);
				bits.cb_raster =
			...
			case cmd_opv_set_misc: {
				...
				case cmd_set_misc_map >> 6: {
					...
					code = cmd_select_map(
					    cb & 0xf, cont, &imager_state,
					    &pcomp_num, &mdata, &count, mem);
			...
			delta2_c:
				set_colors = state.colors;
				/* See comments for cmd_put_color() in
				 * gxclutil.c. */
				{
					...
					/* For cases with an even number of
					 * bytes */
					case 8:
						data = *cbp++;
						delta =
					...
					*pcolor +=
					    delta -
					    cmd_delta_offsets[dev_depth_bytes];
				}
				if_debug1('L', " 0x%lx\n", *pcolor);
				continue;
		...
		set_color:
			set_colors = state.colors;
			/*
			 * We have a special case for gx_no_color_index. If the
			 * low
			 * order 4 bits are "cmd_no_color_index" then we really
			 * have a value of gx_no_color_index.  Otherwise the
			 * these
			 * bits indicate the number of low order zero bytes in
			 * the
			 * value.  See comments for cmd_put_color() in
			 * gxclutil.c.
			 */
		...
		copy:
			...
			if(op & 8) { /* Use the current "tile". */
				...
				bytes = clist_bitmap_bytes(
				    width_bits, state.rect.height, op & 3,
				    &width_bytes, (uint*)&raster);
/* copy_mono and copy_color/alpha */
/* ensure that the bits will fit in a single buffer, */
/* even after decompression if compressed. */
#ifdef DEBUG
				if(bytes > cbuf_size) {
					lprintf6("bitmap size exceeds buffer!  "
					         "width=%d raster=%d "
					         "height=%d\n    file pos %ld "
					         "buf pos %d/%d\n",
					         state.rect.width, raster,
					         state.rect.height, stell(s),
					         (int)(cbp - cbuf.data),
				...
				if(op & 3) { /* Decompress the image data. */
					...
					if(cleft < bytes) {
						...
						cbuf.end_status =
						    sgets(s, cbuf.data + cleft,
						          nread, &nread);
						set_cb_end(&cbuf, cbuf.data +
						                      cleft +
						                      nread);
						cbp = cbuf.data;
					}
					...
					case cmd_compress_rle: {
						...
						/* The process procedure can't
						 * fail. */
						(*s_RLD_template.process)(
						    (stream_state*)&sstate, &r,
						    &w, true);
					} break;
					...
					source = data_bits;
				          ...
				          width_bytes != raster) {
					source = data_bits;
					cbp = cmd_read_short_bits(
					    &cbuf, source, width_bytes,
		...
		sti:
			state_slot =
			    (tile_slot*)(cdev->chunk.data +
			                 cdev->tile_table[state.tile_index]
			                     .offset);
		...
		set_phase
		    : /*
		       ...
		       *	For fill_path, stroke_path,
		       *fill_triangle/p'gram,
			...
			if(state_tile.size.x)
				tile_phase.x = (state.tile_phase.x + x0) %
				               state_tile.size.x;
			if(imager_state.dev_ht &&
			   imager_state.dev_ht->lcm_width)
				color_phase.x = (state.tile_phase.x + x0) %
				                imager_state.dev_ht->lcm_width;
			/*
			...
			if(imager_state.dev_ht &&
			   imager_state.dev_ht->lcm_height)
				color_phase.y = (state.tile_phase.y + y0) %
				                imager_state.dev_ht->lcm_height;
			gx_imager_setscreenphase(
			    &imager_state, -(state.tile_phase.x + x0),
			    -(state.tile_phase.y + y0), gs_color_select_all);
			continue;
		case cmd_op_misc2 >> 4:
			switch(op) {
			...
			case cmd_opv_set_ctm: {
				...
				if_debug6('L', " [%g %g %g %g %g %g]\n", mat.xx,
				          mat.xy, mat.yx, mat.yy, mat.tx,
				          mat.ty);
			}
				continue;
			case cmd_opv_set_misc2:
				cbuf.ptr = cbp;
				code = read_set_misc2(&cbuf, &imager_state,
				                      &notes);
				cbp = cbuf.ptr;
				if(code < 0)
					goto out;
			...
			case cmd_opv_set_dash: {
				...
				gx_set_dash(&imager_state.line_params.dash,
				            dash_pattern, n, offset, NULL);
				gx_set_dash_adapt(
				    &imager_state.line_params.dash,
				    (nb & 0x80) != 0);
				gx_set_dot_length(&imager_state.line_params,
				                  dot_length, (nb & 0x40) != 0);
#ifdef DEBUG
				if(gs_debug_c('L')) {
					int i;

					dprintf4(" dot=%g(mode %d) adapt=%d "
					         "offset=%g [",
					         dot_length, (nb & 0x40) != 0,
					         (nb & 0x80) != 0, offset);
					for(i = 0; i < n; ++i)
			...
			case cmd_opv_set_color_space:
				cbuf.ptr = cbp;
				code = read_set_color_space(&cbuf,
				                            &imager_state, &pcs,
				                            &color_space, mem);
				cbp = cbuf.ptr;
				if(code < 0) {
			...
			case cmd_opv_begin_image:
				...
				if_debug2('L', " size=(%d,%d)", image.d.Width,
				          image.d.Height);
			ibegin:
				if_debug0('L', "\n");
				{
			...
			case cmd_opv_extend:
				...
				case cmd_opv_ext_put_params:
					cbuf.ptr = cbp;
					code = read_put_params(
					    &cbuf, &imager_state, cdev, mem);
					cbp = cbuf.ptr;
					if(code > 0)
						break; /* empty list */
					if(code < 0)
				...
				case cmd_opv_ext_create_compositor:
					cbuf.ptr = cbp;
					/*
					 * The screen phase may have been
					 * changed during
		...
		case cmd_op_segment >> 4: {
			...
			for(i = 0; i < op_num_operands[op & 0xf]; ++i) {
				...
				case 5:
					 ...
					 * brain-damaged coercion rules cause
					 * the
					 * result to be considered unsigned, and
					 * not
			...
			if_debug0('L', "\n");
			code = clist_decode_segment(&path, op, vs, &ppos, x0,
			                            y0, notes);
			if(code < 0)
				goto out;
		}
		...
		case cmd_op_path >> 4: {
			...
			if(in_clip) {
				...
				code = gx_path_add_flattened_accurate(
				    &path, &fpath,
			...
			fill:
				...
				code = gx_fill_path_only(
				    ppath, tdev, &imager_state, &fill_params,
				    &dev_color, pcpath);
				break;
			case cmd_opv_stroke:
				stroke_params.flatness = imager_state.flatness;
				code = gx_stroke_path_only(
				    ppath, (gx_path*)0, tdev, &imager_state,
				    &stroke_params, &dev_color, pcpath);
				break;
			case cmd_opv_polyfill:
				code =
				    clist_do_polyfill(tdev, ppath, &dev_color,
				                      imager_state.log_op);
				break;
			default:
				goto bad_op;
		...
		default:
		bad_op:
			lprintf5("Bad op %02x band y0 = %d file pos %ld buf "
			         "pos %d/%d\n",
			         op, y0, stell(s), (int)(cbp - cbuf.data),
			         (int)(cbuf.end - cbuf.data));
			{
				const byte* pp;
		...
		if_debug4('L', " x=%d y=%d w=%d h=%d\n", state.rect.x,
		          state.rect.y, state.rect.width, state.rect.height);
		switch(op >> 4) {
		case cmd_op_fill_rect >> 4:
		case cmd_op_fill_rect_short >> 4:
		case cmd_op_fill_rect_tiny >> 4:
		...
		case cmd_op_tile_rect_tiny >> 4:
			...
			code = (*dev_proc(tdev, strip_tile_rectangle))(
			    tdev, &state_tile, state.rect.x - x0,
			    state.rect.y - y0, state.rect.width,
			    state.rect.height, state.tile_colors[0],
			    state.tile_colors[1], tile_phase.x, tile_phase.y);
			break;
		case cmd_op_copy_mono >> 4:
			if(state.lop_enabled) {
				pcolor = state.colors;
			...
			if((op & cmd_copy_use_tile) ||
			   pcpath != NULL) { /*
			                      * This call of copy_mono
			                      * originated as a call
			                      * of fill_mask.
			                      */
				code = (*dev_proc(tdev, fill_mask))(
				    tdev, source, data_x, raster,
				    gx_no_bitmap_id, state.rect.x - x0,
				    state.rect.y - y0,
				    state.rect.width - data_x,
				    state.rect.height, &dev_color, 1,
				    imager_state.log_op, pcpath);
			} else
				code = (*dev_proc(tdev, copy_mono))(
				    ...
				    state.rect.y - y0,
				    state.rect.width - data_x,
				    state.rect.height, state.colors[0],
				    state.colors[1]);
			data_x = 0;
			break;
		case cmd_op_copy_color_alpha >> 4:
			if(state.color_is_alpha) {
				...
				code = (*dev_proc(tdev, copy_alpha))(
				    ...
				    state.rect.y - y0,
				    state.rect.width - data_x,
				    state.rect.height, state.colors[1], depth);
			} else {
				if(state.lop_enabled) {
					pcolor = NULL;
					log_op = state.lop;
...
out:
	if(ht_buff.pbuff != 0) {
		gs_free_object(mem, ht_buff.pbuff,
		               "clist_playback_band(ht_buff)");
		ht_buff.pbuff = 0;
		ht_buff.pcurr = 0;
	}
	ht_buff.ht_size = 0;
	...
	if(color_space.params.indexed.lookup.table.size)
		gs_free_const_string(
		    mem, color_space.params.indexed.lookup.table.data,
	...
	if(target != orig_target) {
		...
		gs_free_object(target->memory, target,
		               "gxclrast discard compositor");
		target = orig_target;
	}
	if(code < 0)
		return_error(code);
...
read_set_tile_size(command_buf_t* pcb, tile_slot* bits)
{
	...
	if(bd & 0x80)
		...
		bits->rep_shift = 0;
	if_debug6('L',
	          " depth=%d size=(%d,%d), rep_size=(%d,%d), rep_shift=%d\n",
	          bits->cb_depth, bits->width, bits->height, rep_width,
              ...
              gx_device_clist_reader* cdev, gs_memory_t* mem)
{
	...
	uint width_bytes;
	uint raster;
	uint bytes = clist_bitmap_bytes(
	    width_bits, rep_height,
	    compress | (rep_width < bits->width ? decompress_spread : 0) |
	...
	*slot = *bits;
	...
	if(compress) {
		/*
		 * Decompress the image data.  We'd like to share this code with
		 * the
		...
		stream_cursor_write w;
		/*
		 * We don't know the data length a priori, so to be
		 * conservative, we
		...
		if(cleft < bytes) {
			...
			pcb->end_status =
			    sgets(pcb->s, pcb->data + cleft, nread, &nread);
		...
		case cmd_compress_rle: {
			...
			clist_rld_init(&sstate);
			(*s_RLD_template.process)((stream_state*)&sstate, &r,
			                          &w, true);
		} break;
		case cmd_compress_cfe: {
			stream_CFD_state sstate;
			...
			clist_cfd_init(&sstate,
			               ...
			               rep_height, mem);
			(*s_CFD_template.process)((stream_state*)&sstate, &r,
			                          &w, true);
		...
		cbp = cmd_read_short_bits(pcb, data, width_bytes, rep_height,
		                          bits->cb_raster, cbp);
	} else {
		cbp = cmd_read_data(pcb, data, bytes, cbp);
	}
	if(bits->width > rep_width)
		bits_replicate_horizontally(data, rep_width * bits->cb_depth,
		                            rep_height, bits->cb_raster,
		                            bits->width * bits->cb_depth,
		                            bits->cb_raster);
	if(bits->height > rep_height)
		bits_replicate_vertically(data, rep_height, bits->cb_raster,
		                          bits->height);
#ifdef DEBUG
	if(gs_debug_c('L'))
...
read_alloc_ht_buff(ht_buff_t* pht_buff, uint ht_size, gs_memory_t* mem)
{
	...
	if(ht_size > cbuf_ht_seg_max_size) {
		pht_buff->pbuff =
		    gs_alloc_bytes(mem, ht_size, "read_alloc_ht_buff");
		if(pht_buff->pbuff == 0)
...
private
int
read_ht_segment(ht_buff_t* pht_buff, command_buf_t* pcb, gs_imager_state* pis,
                gx_device* dev, gs_memory_t* mem)
{
	const byte* cbp = pcb->ptr;
	const byte* pbuff = 0;
	...
	if(pbuff != 0) {
		...
		if(pht_buff->pbuff != 0) {
			gs_free_object(mem, pht_buff->pbuff,
			               "read_alloc_ht_buff");
			pht_buff->pbuff = 0;
			pht_buff->pcurr = 0;
		}
...
read_set_misc2(command_buf_t* pcb, gs_imager_state* pis, segment_notes* pnotes)
{
	...
	if(mask & op_bm_tk_known) {
		...
		if_debug4('L', " BM=%d TK=%d OPM=%d OP=%d\n", pis->blend_mode,
		          pis->text_knockout, pis->overprint_mode,
		          pis->overprint);
	}
                     ...
                     gs_memory_t* mem)
{
	...
	if(pcolor_space->params.indexed.use_proc) {
		...
		if(pcolor_space->params.indexed.lookup.table.size)
			gs_free_const_string(
			    mem, pcolor_space->params.indexed.lookup.table.data,
	...
	if(b & 8) {
		...
		if(use_proc) {
			...
			code = alloc_indexed_map(&map, num_values, mem,
			                         "indexed map");
			if(code < 0)
				goto out;
			...
			byte* table = gs_alloc_string(
			    mem, num_values, "color_space indexed table");
		...
		pcolor_space->type = &gs_color_space_type_Indexed;
                ...
                gx_device_clist_reader* cdev, gs_memory_t* mem)
{
	...
	if(pcb->end - cbp >= param_length) {
		...
		param_buf =
		    gs_alloc_bytes(mem, param_length, "clist put_params");
		if(param_buf == 0) {
			code = gs_note_error(gs_error_VMerror);
			goto out;
		}
		...
		pcb->end_status =
		    sgets(pcb->s, param_buf + cleft, rleft, &rleft);
	...
	code =
	    gs_param_list_unserialize((gs_param_list*)&param_list, param_buf);
...
private
int
read_create_compositor(command_buf_t* pcb, gs_imager_state* pis,
                       gx_device_clist_reader* cdev, gs_memory_t* mem,
                       gx_device** ptarget)
{
	const byte* cbp = pcb->ptr;
	...
	code = pcomp->type->procs.clist_compositor_read_update(
	    pcomp, (gx_device*)cdev, tdev, pis, mem);
	if(code < 0)
		return code;
...
private
const byte*
cmd_read_short_bits(command_buf_t* pcb, byte* data, int width_bytes, int height,
...
cmd_create_dev_ht(gx_device_halftone** ppdht, gs_memory_t* mem)
{
	...
	if(pdht == 0) {
		...
		rc_alloc_struct_1(pdht, gx_device_halftone, &st_device_halftone,
...
private
int
cmd_resize_halftone(gx_device_halftone** ppdht, uint num_comp, gs_memory_t* mem)
{
	...
	if(num_comp != pdht->num_comp) {
		...
		if(num_comp < pdht->num_comp) {
			...
			if(num_comp == 0) {
				...
				pcomp = gs_resize_object(mem, pdht->components,
				                         num_comp,
				                         "cmd_resize_halftone");
				if(pcomp == 0) {
					pdht->num_comp =
					    num_comp; /* attempt consistency */
					return_error(gs_error_VMerror);
				}
			}
		} else {
			...
			if(pdht->num_comp == 0)
				pcomp = gs_alloc_struct_array(
				    mem, num_comp, gx_ht_order_component,
...
private
int
clist_decode_segment(gx_path* ppath, int op, fixed vs[6], gs_fixed_point* ppos,
                     int x0, int y0, segment_notes notes)
{
...
#define F vs[5]
	...
	rrc:
		...
		F += (D += B);
	curve:
		code = gx_path_add_curve_notes(ppath, px + A, py + B, px + C,
		                               py + D, px + E, py + F, notes);
		px += E, py += F;
		break;
	...
	case cmd_opv_scurveto: /* (a b c d e f) => */
	{
		...
		if(A == 0) {
			...
			A = E - C, B = D - F, C = C - a, D = b - D, E = a,
			F = -b;
		} else {
			/* Previous curve was hv or hh */
			A = C - E, B = F - D, C = a - C, D = D - b, E = -a,
			F = b;
		}
	}
		goto rrc;
