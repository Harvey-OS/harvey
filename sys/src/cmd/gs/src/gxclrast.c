...
private
int read_set_bits(command_buf_t *pcb, tile_slot *bits,
...
private
const byte *cmd_read_short_bits(command_buf_t *pcb, byte *data,
				int width_bytes, int height,
				uint raster, const byte *cbp);
private
int cmd_select_map(cmd_map_index, cmd_map_contents,
		   gs_imager_state *, int **,
		   frac **, uint *, gs_memory_t *);
private
int cmd_create_dev_ht(gx_device_halftone **, gs_memory_t *);
private
int cmd_resize_halftone(gx_device_halftone **, uint,
			gs_memory_t *);
private
int clist_decode_segment(gx_path *, int, fixed[6],
			 gs_fixed_point *, int, int,
			 segment_notes);
private
int clist_do_polyfill(gx_device *, gx_path *,
	...
	while(code >= 0) {
		...
		if(cbp >= cbuf.limit) {
			if(cbuf.end_status < 0) { /* End of file or error. */
				if(cbp == cbuf.end) {
					code = (cbuf.end_status == EOFC ? 0 : gs_note_error(gs_error_ioerror));
					break;
				}
			} else {
		...
		case cmd_op_misc >> 4:
			...
			delta2_c:
				...
				/* See comments for cmd_put_color() in gxclutil.c. */
				{
					...
					case 8:
						data = *cbp++;
						delta = ((gx_color_index)((data & 0xf0) << 4) + (data & 0x0f)) << 48;
					case 6:
						data = *cbp++;
						delta |= ((gx_color_index)((data & 0xf0) << 4) + (data & 0x0f)) << 32;
					case 4:
						data = *cbp++;
						delta |= ((gx_color_index)((data & 0xf0) << 4) + (data & 0x0f)) << 16;
					case 2:
						data = *cbp++;
						delta |= ((gx_color_index)((data & 0xf0) << 4) + (data & 0x0f));
						break;
					...
					case 3:
						...
						delta |= ((gx_color_index)((data & 0xe0) << 3) + (data & 0x1f)) << odd_delta_shift;
						data = *cbp++;
...
/* even after decompression if compressed. */
				...
				if(op & 3) { /* Decompress the image data. */
					...
					case cmd_compress_rle: {
						...
						(*s_RLD_template.process)((stream_state *)&sstate, &r, &w, true);
					} break;
					case cmd_compress_cfe: {
						...
						/* The process procedure can't fail. */
						(*s_CFD_template.process)((stream_state *)&sstate, &r, &w, true);
		...
		case cmd_op_misc2 >> 4:
			 ...
			 */
				...
				imager_state.log_op =
				    (state.lop_enabled ? state.lop : lop_default);
				imager_state.fill_adjust =
				    clip_save.fill_adjust;
			...
			ibegin:
				if_debug0('L', "\n");
				{
					code = (*dev_proc(tdev, begin_typed_image))(tdev, &imager_state, NULL,
		...
		default:
		bad_op:
			lprintf5("Bad op %02x band y0 = %d file pos %ld buf pos %d/%d\n",
		...
		case cmd_op_fill_rect_tiny >> 4:
			if(!state.lop_enabled) {
				code = (*dev_proc(tdev, fill_rectangle))(tdev, state.rect.x - x0, state.rect.y - y0,
			...
			pcolor = colors;
		do_rop:
			code = (*dev_proc(tdev, strip_copy_rop))(tdev, source, data_x, raster, gx_no_bitmap_id,
								 pcolor, &state_tile,
		...
		case cmd_op_tile_rect_tiny >> 4:
			...
			code = (*dev_proc(tdev, strip_tile_rectangle))(tdev, &state_tile,
								       state.rect.x - x0, state.rect.y - y0,
								       state.rect.width, state.rect.height,
		...
		case cmd_op_copy_mono >> 4:
			...
			if((op & cmd_copy_use_tile) || pcpath != NULL) { /*
									 ...
									 * of fill_mask.
									 */
				code = (*dev_proc(tdev, fill_mask))(tdev, source, data_x, raster, gx_no_bitmap_id,
								    state.rect.x - x0, state.rect.y - y0,
								    state.rect.width - data_x, state.rect.height,
								    &dev_color, 1, imager_state.log_op, pcpath);
			} else
				code = (*dev_proc(tdev, copy_mono))(tdev, source, data_x, raster, gx_no_bitmap_id,
								    state.rect.x - x0, state.rect.y - y0,
								    state.rect.width - data_x, state.rect.height,
		...
		case cmd_op_copy_color_alpha >> 4:
			if(state.color_is_alpha) {
				...
				code = (*dev_proc(tdev, copy_alpha))(tdev, source, data_x, raster, gx_no_bitmap_id,
								     state.rect.x - x0, state.rect.y - y0,
								     state.rect.width - data_x, state.rect.height,
				...
				code = (*dev_proc(tdev, copy_color))(tdev, source, data_x, raster, gx_no_bitmap_id,
								     state.rect.x - x0, state.rect.y - y0,
								     state.rect.width - data_x, state.rect.height);
			}
			data_x = 0;
...
read_set_tile_size(command_buf_t *pcb, tile_slot *bits)
{
	...
	bits->shift =
	    (bits->rep_shift == 0 ? 0 : (bits->rep_shift * (bits->height / rep_height)) % rep_width);
	      ...
	      gx_device_clist_reader *cdev, gs_memory_t *mem)
{
	...
	uint bytes =
	    clist_bitmap_bytes(width_bits, rep_height,
			       compress |
				   (rep_width < bits->width ? decompress_spread : 0) |
		...
		gx_device_clist_reader *cdev, gs_memory_t *mem)
{
	...
	code = gs_param_list_unserialize((gs_param_list *)&param_list, param_buf);
...
#define F vs[5]
	...
	rrc:
		...
		F += (D += B);
	curve:
		code = gx_path_add_curve_notes(ppath, px + A, py + B,
					       px + C, py + D,
					       px + E, py + F, notes);
		px += E, py += F;
		break;
