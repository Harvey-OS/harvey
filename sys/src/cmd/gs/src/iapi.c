 ...
 * next instance in a proces call with *pinstance == copy of valid_instance
 * pointer
...
gsapi_delete_instance(void* lib)
{
	...
	if((ctx != NULL)) {
		...
		/* NB: notice how no deletions are occuring, good bet this isn't
		 * thread ready
		 */
...
GSDLLEXPORT int GSDLLAPI
gsapi_set_poll(void* lib, int(GSDLLCALL* poll_fn)(void* caller_handle))
{
	gs_lib_ctx_t* ctx = (gs_lib_ctx_t*)lib;
	if(lib == NULL)
		return e_Fatal;
...
gsapi_init_with_args(void* lib, int argc, char** argv)
{
	...
	if(lib == NULL)
		return e_Fatal;
	return gs_main_init_with_args(get_minst_from_memory(ctx->memory), argc,
	                              argv);
}
...
gsapi_run_string_begin(void* lib, int user_errors, int* pexit_code)
{
	...
	return gs_main_run_string_begin(
	    get_minst_from_memory(ctx->memory), user_errors, pexit_code,
	    &(get_minst_from_memory(ctx->memory)->error_object));
}

GSDLLEXPORT int GSDLLAPI
gsapi_run_string_continue(void* lib, const char* str, uint length,
                          int user_errors, int* pexit_code)
{
	...
	return gs_main_run_string_continue(
	    get_minst_from_memory(ctx->memory), str, length, user_errors,
	    pexit_code, &(get_minst_from_memory(ctx->memory)->error_object));
}
...
gsapi_run_string_end(void* lib, int user_errors, int* pexit_code)
{
	...
	return gs_main_run_string_end(
	    get_minst_from_memory(ctx->memory), user_errors, pexit_code,
	    &(get_minst_from_memory(ctx->memory)->error_object));
}

GSDLLEXPORT int GSDLLAPI
gsapi_run_string_with_length(void* lib, const char* str, uint length,
                             int user_errors, int* pexit_code)
{
	gs_lib_ctx_t* ctx = (gs_lib_ctx_t*)lib;
	if(lib == NULL)
	...
	return gs_main_run_string_with_length(
	    get_minst_from_memory(ctx->memory), str, length, user_errors,
	    pexit_code, &(get_minst_from_memory(ctx->memory)->error_object));
}

GSDLLEXPORT int GSDLLAPI
gsapi_run_string(void* lib, const char* str, int user_errors, int* pexit_code)
{
	gs_lib_ctx_t* ctx = (gs_lib_ctx_t*)lib;
	return gsapi_run_string_with_length(get_minst_from_memory(ctx->memory),
	                                    str, (uint)strlen(str), user_errors,
	                                    pexit_code);
}

GSDLLEXPORT int GSDLLAPI
gsapi_run_file(void* lib, const char* file_name, int user_errors,
               int* pexit_code)
{
	gs_lib_ctx_t* ctx = (gs_lib_ctx_t*)lib;
	if(lib == NULL)
	...
	return gs_main_run_file(
	    get_minst_from_memory(ctx->memory), file_name, user_errors,
	    pexit_code, &(get_minst_from_memory(ctx->memory)->error_object));
}
