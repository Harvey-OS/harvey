...
/* Context structure */
typedef enum { cs_active, cs_done } ctx_status_t;
...
CLEAR_MARKS_PROC(context_clear_marks)
{
	...
	(*st_context_state.clear_marks)(cmem, &pctx->state, sizeof(pctx->state),
	                                &st_context_state);
}
private
...
case 0:
return ENUM_OBJ(pctx->scheduler);
case 1: {
...
ENUM_PTRS_END
private
RELOC_PTRS_WITH(context_reloc_ptrs, gs_context_t* pctx)
...
gs_private_st_complex_only(st_context, gs_context_t, "gs_context_t",
                           context_clear_marks, context_enum_ptrs,
                           context_reloc_ptrs, 0);
...
gs_private_st_ptrs1(st_lock, gs_lock_t, "locktype", lock_enum_ptrs,
                    lock_reloc_ptrs, scheduler);
 ...
 */
private
ENUM_PTRS_WITH(scheduler_enum_ptrs, gs_scheduler_t* psched)
{
	index -= 1;
...
case 0:
return ENUM_OBJ(visible_context(psched->current));
ENUM_PTRS_END
private
RELOC_PTRS_WITH(scheduler_reloc_ptrs, gs_scheduler_t* psched)
{
...
context_reclaim(vm_spaces* pspaces, bool global)
{
	...
	for(i = countof(pspaces->memories.indexed) - 1; psched == 0 && i > 0;
	    --i) {
		gs_ref_memory_t* mem = pspaces->memories.indexed[i];
		const gs_gc_root_t* root = mem->roots;

		for(; root; root = root->next) {
			if(gs_object_type((gs_memory_t*)mem, *root->p) ==
			   &st_context) {
				pctx = *root->p;
...
/* Forward references */
private
int context_create(gs_scheduler_t*, gs_context_t**, const gs_dual_memory_t*,
                   const gs_context_state_t*, bool);
private
long context_usertime(void);
private
int context_param(const gs_scheduler_t*, os_ptr, gs_context_t**);
private
void context_destroy(gs_context_t*);
private
void stack_copy(ref_stack_t*, const ref_stack_t*, uint, uint);
private
int lock_acquire(os_ptr, gs_context_t*);
private
int lock_release(ref*);
...
private
int ctx_initialize(i_ctx_t**);
private
int ctx_reschedule(i_ctx_t**);
private
int ctx_time_slice(i_ctx_t**);
private
int
...
ctx_initialize(i_ctx_t** pi_ctx_p)
{
	...
	gs_scheduler_t* psched = gs_alloc_struct_immovable(
	    (gs_memory_t*)imem, gs_scheduler_t, &st_scheduler, "gs_scheduler");
	...
	if(context_create(psched, &psched->current, &gs_imemory, *pi_ctx_p,
	                  true) < 0) {
...
ctx_reschedule(i_ctx_t** pi_ctx_p)
{
	...
	/* Run the first ready context, taking the 'save' lock into account. */
	{
		...
		for(ready = index_context(psched, psched->active.head_index);;
		    prev = ready,
		ready = index_context(psched, ready->next_index)) {
			if(ready == 0) {
				if(current != 0)
...
private
int do_fork(i_ctx_t* i_ctx_p, os_ptr op, const ref* pstdin, const ref* pstdout,
            uint mcount, bool local),
    values_older_than(const ref_stack_t* pstack, uint first, uint last,
                      int max_space);
private
int fork_done(i_ctx_t*), fork_done_with_error(i_ctx_t*), finish_join(i_ctx_t*),
        ...
        uint mcount, bool local)
{
	...
	if(local) {
		...
		lmem_stable =
		    ialloc_alloc_state(parent, iimemory_local->chunk_size);
		...
		code =
		    context_create(psched, &pctx, &dmem, &pcur->state, false);
		if(code < 0) {
			/****** FREE lmem ******/
			return code;
		}
		 ...
		 */
		{
			...
			for(n = 0, old = igs; old != 0;
			    old = gs_state_saved(old))
				++n;
			for(old = pctx->state.pgs; old != 0;
			    old = gs_state_saved(old))
				--n;
			for(; n > 0 && code >= 0; --n)
...
fork_done(i_ctx_t* i_ctx_p)
{
	...
	if(iimemory_local->save_level) {
		...
		if(pcur->detach) {
			ref_stack_clear(
			    &o_stack); /* help avoid invalidrestore */
			op = osp;
		}
		push(1);
...
zjoin(i_ctx_t* i_ctx_p)
{
	...
	case cs_active:
		...
		return o_push_estack;
	case cs_done: {
		const ref_stack_t* ostack = (ref_stack_t*)&pctx->state.op_stack;
...
private
int monitor_cleanup(i_ctx_t*), monitor_release(i_ctx_t*), await_lock(i_ctx_t*);
private
void activate_waiting(gs_scheduler_t*, ctx_list_t* pcl);
...
zmonitor(i_ctx_t* i_ctx_p)
{
	...
	if(pctx != 0) {
		if(pctx == current || (iimemory_local->save_level != 0 &&
		                       pctx->state.memory.space_local ==
		                           current->state.memory.space_local))
			return_error(e_invalidcontext);
	}
	...
	if(code !=
	   0) { /* We didn't acquire the lock.  Re-execute this later. */
...
context_create(gs_scheduler_t* psched, gs_context_t** ppctx,
               const gs_dual_memory_t* dmem, const gs_context_state_t* i_ctx_p,
               bool copy_state)
{
	/*
	...
	pctx =
	    gs_alloc_struct(mem, gs_context_t, &st_context, "context_create");
	if(pctx == 0)
		return_error(e_VMerror);
	if(copy_state) {
		pctx->state = *i_ctx_p;
           ...
           uint from_index)
{
	...
	for(i = (long)count - 1; i >= 0; --i)
		*ref_stack_index(to, i) =
		    *ref_stack_index(from, i + from_index);
}
