                      ...
                      fixed fixed_flat)
{
	...
	if(fixed_flat == 0) { /* Use the conservative method. */
		...
		const fixed x12 = pc->p1.x - pc->p2.x,
		            y12 = pc->p1.y - pc->p2.y,
		            dx0 = x0 - pc->p1.x - x12,
		            dy0 = y0 - pc->p1.y - y12,
		            dx1 = x12 - pc->p2.x + pc->pt.x,
		            dy1 = y12 - pc->p2.y + pc->pt.y,
		            adx0 = any_abs(dx0), ady0 = any_abs(dy0),
		            adx1 = any_abs(dx1), ady1 = any_abs(dy1);
		fixed d = max(adx0, adx1) + max(ady0, ady1);
		/*
		...
		if_debug6('2', "[2]d01=%g,%g d12=%g,%g d23=%g,%g\n",
		          fixed2float(pc->p1.x - x0),
		          fixed2float(pc->p1.y - y0), fixed2float(-x12),
		          fixed2float(-y12), fixed2float(pc->pt.x - pc->p2.x),
		          fixed2float(pc->pt.y - pc->p2.y));
		if_debug2('2', "     D=%f, flat=%f,", fixed2float(d),
		          fixed2float(fixed_flat));
...
bool
curve_coeffs_ranged(fixed x0, fixed x1, fixed x2, fixed x3, fixed y0, fixed y1,
                    fixed y2, fixed y3, fixed* ax, fixed* bx, fixed* cx,
                    fixed* ay, fixed* by, fixed* cy, int k)
{
	fixed x01, x12, y01, y12;

	curve_points_to_coefficients(x0, x1, x2, x3, *ax, *bx, *cx, x01, x12);
	curve_points_to_coefficients(y0, y1, y2, y3, *ay, *by, *cy, y01, y12);
#define max_fast (max_fixed / 6)
#define min_fast (-max_fast)
#define in_range(v) (v < max_fast && v > min_fast)
	if(k > k_sample_max || !in_range(*ax) || !in_range(*ay) ||
	   !in_range(*bx) || !in_range(*by) || !in_range(*cx) || !in_range(*cy))
		return false;
#undef max_fast
#undef min_fast
#undef in_range
 ...
 */
bool
gx_flattened_iterator__init(gx_flattened_iterator* this, fixed x0, fixed y0,
                            const curve_segment* pc, int k)
{
	/* Note : Immediately after the ininialization it keeps an invalid (zero
	 * length) segment. */
	fixed x1, y1, x2, y2;
	...
	this->x3 = pc->pt.x;
	this->y3 = pc->pt.y;
	if(!curve_coeffs_ranged(this->x0, x1, x2, this->x3, this->y0, y1, y2,
	                        this->y3, &this->ax, &this->bx, &this->cx,
	...
	if(gs_debug_c('3')) {
		dlprintf4("[3]x0=%f y0=%f x1=%f y1=%f\n", fixed2float(this->x0),
		          fixed2float(this->y0), fixed2float(x1),
		          fixed2float(y1));
		dlprintf5("   x2=%f y2=%f x3=%f y3=%f  k=%d\n", fixed2float(x2),
		          fixed2float(y2), fixed2float(this->x3),
		          fixed2float(this->y3), this->k);
	}
#endif
	if(k == -1) {
	...
	if_debug6('3', "[3]ax=%f bx=%f cx=%f\n   ay=%f by=%f cy=%f\n",
	          fixed2float(this->ax), fixed2float(this->bx),
	          fixed2float(this->cx), fixed2float(this->ay),
	          fixed2float(this->by), fixed2float(this->cy));
	bx2 = this->bx << 1;
	by2 = this->by << 1;
...
bool
gx_flattened_iterator__init_line(gx_flattened_iterator* this, fixed x0,
                                 fixed y0, fixed x1, fixed y1)
{
	...
	this->y3 = y1;
	if(ox || oy) {
		/* Subdivide a long line into 2 segments, because the filling
		   algorithm
		   and the stroking algorithm need to compute differences
		   of coordinates of end points. */
		/* Note : the result of subdivision may be not strongly
		 * colinear. */
		this->ax = this->bx = 0;
		this->ay = this->by = 0;
...
gx_flattened_iterator__print_state(gx_flattened_iterator* this)
{
	...
	dlprintf4("   d2x=%f+%d, d2y=%f+%d\n", fixed2float(this->id2x),
	          this->rd2x, fixed2float(this->id2y), this->rd2y);
	dlprintf4("   d3x=%f+%d, d3y=%f+%d\n", fixed2float(this->id3x),
	          this->rd3x, fixed2float(this->id3y), this->rd3y);
}
#endif

/* Move to the next segment and store it to this->lx0, this->ly0, this->lx1,
 * this->ly1 .
...
#undef poly2
		if_debug2('3', "[3]dx=%f, dy=%f\n", fixed2float(x - this->x0),
		          fixed2float(y - this->y0));
		...
		vd_bar(this->lx0, this->ly0, this->lx1, this->ly1, 1,
		       RGB(0, 255, 0));
		return true;
	} else {
		--this->i;
...
#define accum(i, r, di, dr, rmask)                                             \
	...
	i += di
		...
		accum(this->idx, this->rdx, this->id2x, this->rd2x,
		      this->rmask);
		accum(this->idy, this->rdy, this->id2y, this->rd2y,
		      this->rmask);
		accum(this->id2x, this->rd2x, this->id3x, this->rd3x,
		      this->rmask);
		accum(this->id2y, this->rd2y, this->id3y, this->rd3y,
		      this->rmask);
		if_debug5(
		    '3', "[3]%s x=%g, y=%g x=%d y=%d\n",
...
last:
	this->lx1 = this->x3;
	this->ly1 = this->y3;
	if_debug4('3', "[3]last x=%g, y=%g x=%d y=%d\n", fixed2float(this->lx1),
	          fixed2float(this->ly1), this->lx1, this->ly1);
...
gx_flattened_iterator__prev(gx_flattened_iterator* this)
{
	...
	if(this->k <= 1) {
		/* If k==0, we have a single segment, return it.
		   ...
		   but we want to check the assertion about the last segment
		   below.
		 */
		this->i++;
		this->lx0 = this->x0;
...
/* Switching from the forward scanning to the backward scanning for the
 * filtered1. */
void
gx_flattened_iterator__switch_to_backscan(gx_flattened_iterator* this,
                                          bool not_first)
{
	/*	When scanning forth, the accumulator stands on the end of a
	   segment,
	    except for the last segment.
	    When scanning back, the accumulator should stand on the beginning of
	   a segment.
	    Asuuming that at least one forward step is done.
	*/
	if(not_first)
		if(this->i > 0 &&
		   this->k != 1 /* This case doesn't use the accumulator. */)
			gx_flattened_iterator__unaccum(this);
}
...
private
int
generate_segments(gx_path* ppath, const gs_fixed_point* points, int count,
                  segment_notes notes)
{
	...
	if(notes & sn_not_first) {
		...
		code = gx_path_add_line_notes(ppath, points[0].x, points[0].y,
		                              notes);
		if(code < 0)
			return code;
		/* vd_lineto_multi(points + 1, count - 1); */
		print_points(points + 1, count - 1);
		return gx_path_add_lines_notes(ppath, points + 1, count - 1,
		                               notes | sn_not_first);
	}
}
...
private
int
gx_subdivide_curve_rec(gx_flattened_iterator* this, gx_path* ppath, int k,
                       curve_segment* pc, segment_notes notes,
                       gs_fixed_point* points)
{
	int code;

top:
	if(!gx_flattened_iterator__init(this, ppath->position.x,
	                                ppath->position.y, pc, k)) {
		/* Curve is too long.  Break into two pieces and recur. */
		curve_segment cseg;

		k--;
		split_curve_midpoint(ppath->position.x, ppath->position.y, pc,
		                     &cseg, pc);
		code = gx_subdivide_curve_rec(this, ppath, k, &cseg, notes,
		                              points);
		if(code < 0)
			return code;
		notes |= sn_not_first;
		...
		/* fixme : Don't need to init the iterator. Just wanted to check
		 * in_range. */
		...
		for(;;) {
			...
			if(ppt == &points[max_points] || !more) {
				...
				code = generate_segments(ppath, points,
				                         pe - points, notes);
				if(code < 0)
					return code;
				if(!more)
