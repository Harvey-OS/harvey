...
/* Forward references */
private
int zcrd1_proc_params(const gs_memory_t* mem, os_ptr op,
                      ref_cie_render_procs* pcprocs);
private
int zcrd1_params(os_ptr op, gs_cie_render* pcrd, ref_cie_render_procs* pcprocs,
                 gs_memory_t* mem);
private
int cache_colorrendering1(i_ctx_t* i_ctx_p, gs_cie_render* pcrd,
...
zsetcolorrendering1(i_ctx_t* i_ctx_p)
{
	...
	if(gs_cie_cs_common(igs) != 0 &&
	   (code = cie_cache_joint(i_ctx_p, &procs, gs_cie_cs_common(igs),
	                           igs)) < 0)
		return code;
...
private
int
zcrd1_proc_params(const gs_memory_t* mem, os_ptr op,
                  ref_cie_render_procs* pcprocs)
{
	int code;
	ref* pRT;

	if((code = dict_proc3_param(mem, op, "EncodeLMN",
	                            &pcprocs->EncodeLMN)) < 0 ||
	   (code = dict_proc3_param(mem, op, "EncodeABC",
	                            &pcprocs->EncodeABC)) < 0 ||
	   (code = dict_proc3_param(mem, op, "TransformPQR",
	                            &pcprocs->TransformPQR)) < 0)
	...
	if(dict_find_string(op, "RenderTable", &pRT) > 0) {
		...
		make_const_array(&pcprocs->RenderTableT,
		                 a_readonly | r_space(pRT), size - 5, prte + 5);
	} else
...
private
int
zcrd1_params(os_ptr op, gs_cie_render* pcrd, ref_cie_render_procs* pcprocs,
             gs_memory_t* mem)
{
	int code;
	int ignore;
	...
	if((code = dict_int_param(op, "ColorRenderingType", 1, 1, 0, &ignore)) <
	       0 ||
	   (code = zcrd1_proc_params(mem, op, pcprocs)) < 0 ||
	   (code = dict_matrix3_param(mem, op, "MatrixLMN", &pcrd->MatrixLMN)) <
	       0 ||
	   (code = dict_range3_param(mem, op, "RangeLMN", &pcrd->RangeLMN)) <
	       0 ||
	   (code = dict_matrix3_param(mem, op, "MatrixABC", &pcrd->MatrixABC)) <
	       0 ||
	   (code = dict_range3_param(mem, op, "RangeABC", &pcrd->RangeABC)) <
	       0 ||
	   (code = cie_points_param(mem, op, &pcrd->points)) < 0 ||
	   (code = dict_matrix3_param(mem, op, "MatrixPQR", &pcrd->MatrixPQR)) <
	       0 ||
...
private
int cie_cache_render_finish(i_ctx_t*);
private
int
                      ...
                      gs_ref_memory_t* imem)
{
	...
	if(code < 0 ||
	   (code = cie_cache_push_finish(i_ctx_p, cie_cache_render_finish, imem,
	                                 pcrd)) < 0 ||
	   (code = cie_prepare_cache3(
	        i_ctx_p, &pcrd->DomainLMN, pcrprocs->EncodeLMN.value.const_refs,
	        pcrd->caches.EncodeLMN.caches, pcrd, imem, "Encode.LMN")) < 0 ||
	   (code = cie_prepare_cache3(
	        i_ctx_p, &pcrd->DomainABC, pcrprocs->EncodeABC.value.const_refs,
	        &pcrd->caches.EncodeABC[0], pcrd, imem, "Encode.ABC")) < 0) {
		esp = ep;
		return code;
	}
	if(pcrd->RenderTable.lookup.table != 0) {
		...
		for(i = 0; i < pcrd->RenderTable.lookup.m; i++)
			if(r_size(pcrprocs->RenderTableT.value.const_refs +
			          i) != 0) {
				is_identity = false;
				break;
			}
		...
		if(!is_identity)
			for(i = 0; i < pcrd->RenderTable.lookup.m; i++)
				if((code = cie_prepare_cache(
				        i_ctx_p, Range4_default.ranges,
				        pcrprocs->RenderTableT.value
				                .const_refs +
				            i,
	   ...
	   !pcrd->caches.RenderTableT_is_identity) {
		...
		for(j = 0; j < pcrd->RenderTable.lookup.m; j++)
			gs_cie_cache_to_fracs(
			    &pcrd->caches.RenderTableT[j].floats,
...
private
int cie_exec_tpqr(i_ctx_t*), cie_post_exec_tpqr(i_ctx_t*),
                ...
                const gs_cie_common* pcie, gs_state* pgs)
{
	...
	if(r_has_type(&pcrprocs->TransformPQR, t_null)) {
		/*
		 * This CRD came from a driver, not from a PostScript
		 * dictionary.
	...
	*++esp = pqr_procs;
	...
	for(i = 0; i < 3; i++) {
		...
		make_array(pqr_procs.value.refs + i,
		           a_readonly | a_executable | space, 4, p);
	...
	return cie_prepare_cache3(
	    i_ctx_p, &pcrd->RangePQR, pqr_procs.value.const_refs,
	    pjc->TransformPQR.caches, pjc, imem, "Transform.PQR");
}
...
cie_exec_tpqr(i_ctx_t* i_ctx_p)
{
	...
	*op = op[-4];    /* proc */
	op[-1] = op[-6]; /* v */
	for(i = 0; i < 4; i++)
		make_const_array(op - 5 + i, a_readonly | space, 6,
		                 ppt + i * 6);
	make_mark(op - 6);
