...
zPFBD(i_ctx_t* i_ctx_p)
{
	...
	return filter_read(i_ctx_p, 1, &s_PFBD_template, (stream_state*)&state,
	                   0);
}
...
zRLE(i_ctx_t* i_ctx_p)
{
	...
	return filter_write(i_ctx_p, 1, &s_RLE_template, (stream_state*)&state,
	                    0);
}
...
zRLD(i_ctx_t* i_ctx_p)
{
	...
	if(code < 0)
		return code;
	return filter_read(i_ctx_p, 0, &s_RLD_template, (stream_state*)&state,
	                   0);
}
...
zSFD(i_ctx_t* i_ctx_p)
{
	...
	if(LL3_ENABLED && r_has_type(op, t_dictionary)) {
		...
		check_dict_read(*op);
		/*
		 * The PLRM-3rd says that EODCount is a required parameter.
		 * However
		 * Adobe accepts files without this value and apparently
		 * defaults to
		 * zero.  Thus we are doing the same.
		 */
		if((code = dict_int_param(op, "EODCount", 0, max_int, 0,
		                          &count)) < 0)
			return code;
...
/* Forward references */
private
int filter_ensure_buf(stream**, uint, gs_ref_memory_t*, bool);
            ...
            stream_state* st, uint space)
{
	...
	if(r_has_type(sop, t_dictionary)) {
		check_dict_read(*sop);
		if((code = dict_bool_param(sop, "CloseSource", false, &close)) <
		   0)
			return code;
		--sop;
	}
	...
	case t_string:
		...
		sstrm =
		    file_alloc_stream(imemory, "filter_read(string stream)");
		if(sstrm == 0) {
	...
	default:
		...
		sstrm->is_temp = 2;
	ens:
		code = filter_ensure_buf(
		    &sstrm, template->min_in_size +
	...
	code = filter_open("r", min_size, (ref*)sop, &s_filter_read_procs,
	                   template, st, imemory);
	if(code < 0)
		goto out;
	s = fptr(sop);
             ...
             stream_state* st, uint space)
{
	...
	if(r_has_type(sop, t_dictionary)) {
		check_dict_read(*sop);
		if((code = dict_bool_param(sop, "CloseTarget", false, &close)) <
		   0)
			return code;
		--sop;
	}
	...
	if(min_size < 128)
		min_size = file_default_buffer_size;
	code = filter_open("w", min_size, (ref*)sop, &s_filter_write_procs,
	                   template, st, imemory);
	if(code < 0)
		goto out;
	s = fptr(sop);
...
private
const stream_template s_Null1D_template = {&st_stream_state, NULL,
                                           s_Null1D_process, 1, 1};
...
private
const stream_template s_EOFD_template = {&st_stream_state, NULL, s_EOFD_process,
                                         1, 1};
                  ...
                  bool writing)
{
	...
	if(s->cbuf == 0) {
		...
		if(writing)
			code = filter_open(
			    "w", min_size, &bsop, &s_filter_write_procs,
			    &s_NullE_template, NULL, (gs_memory_t*)imem);
		else
			code = filter_open(
			    "r", min_size, &bsop, &s_filter_read_procs,
