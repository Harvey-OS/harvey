...
/* Copyright (C) 1991, 1995, 1998, 1999 Aladdin Enterprises.  All rights
  reserved.
...
#define public_st_clip_rect() /* in gxcpath.c */                               \
	gs_public_st_ptrs2(st_clip_rect, gx_clip_rect, "clip_rect",            \
	                   clip_rect_enum_ptrs, clip_rect_reloc_ptrs, next,    \
	                   prev)
...
#define private_st_clip_list() /* in gxcpath.c */                              \
	gs_private_st_ptrs2(st_clip_list, gx_clip_list, "clip_list",           \
	                    clip_list_enum_ptrs, clip_list_reloc_ptrs, head,   \
	                    tail)
...
#define public_st_device_clip() /* in gxcpath.c */                             \
	gs_public_st_composite_use_final(                                      \
	    st_device_clip, gx_device_clip, "gx_device_clip",                  \
	    device_clip_enum_ptrs, device_clip_reloc_ptrs, gx_device_finalize)
...
#define clip_rect_print(ch, str, ar)                                           \
	if_debug7(ch, "[%c]%s 0x%lx: (%d,%d),(%d,%d)\n", ch, str,              \
	          (uint32_t)ar, (ar)->xmin, (ar)->ymin, (ar)->xmax,            \
	          (ar)->ymax)
