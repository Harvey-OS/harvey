...
private
irender_proc(image_render_mono);
irender_proc_t
gs_image_class_3_mono(gx_image_enum* penum)
{
	if(penum->spp == 1) {
		/*
		 * Use the slow loop for imagemask with a halftone or a
		 * non-default
		 * logical operation.
		 */
		penum->slow_loop =
		...
		if(!(penum->slow_loop || penum->posture != image_portrait))
			penum->clip_image &=
			    ~(image_clip_xmin | image_clip_xmax);
		if_debug0('b', "[b]render=mono\n");
		/* Precompute values needed for rasterizing. */
		penum->dxx = float2fixed(penum->matrix.xx +
		                         fixed2float(fixed_epsilon) / 2);
		/*
		 * Scale the mask colors to match the scaling of each sample to
		 * a
		 * full byte.  Also, if black or white is transparent, reset
		 * icolor0
...
private
int
image_render_mono(gx_image_enum* penum, const byte* buffer, int data_x, uint w,
                  int h, gx_device* dev)
{
	...
	uint mask_limit =
	    (penum->use_mask_color ? penum->mask_color.values[1] - mask_base + 1
	                           : 0);
/*
...
#define IMAGE_SET_GRAY(sample_value)                                           \
	...
	if(!color_is_set(pdevc)) {                                             \
		if((uint)(sample_value - mask_base) < mask_limit)              \
			...
			code = (*remap_color)(&cc, pcs, pdevc, pis, dev,       \
		...
		if(!tiles_fit) {                                               \
			code = gx_color_load_select(pdevc, pis, dev,           \
			                            gs_color_select_source);   \
			if(code < 0)                                           \
				goto err;                                      \
		}                                                              \
...
#define ytf dda_current(next.y)
		...
		if(masked) {
			...
			if(penum->posture == image_portrait) {
				...
				/*
				 * We don't have to worry about the Y DDA, and
				 * the fill
				 * regions are rectangles.  Calculate multiples
				 * of the DDA
				 * step.
				 */
				fixed ax =
				    (penum->matrix.xx < 0 ? -penum->adjust
				                          : penum->adjust);
				fixed ay =
				...
				for(;;) { /* Skip a run of zeros. */
					...
					for(; *psrc; ++psrc)
						dda_next(next.x);
					code = (*fill_pgram)(
					    dev, xrun, yrun, xl - xrun + ax,
					    fixed_0, fixed_0, dyy, pdevc, lop);
					if(code < 0)
						goto err;
					rsrc = psrc;
					if(psrc >= stop)
				...
				/*
				 * We don't have to worry about the X DDA.
				 * However, we do
				 * have to take adjustment into account.  We
				 * don't bother to
				 * optimize this as heavily as the portrait
				 * case.
				 */
				fixed ax =
				...
				fixed dyx = pdyx + (ax << 1);
				fixed ay =
				    (penum->matrix.xy < 0 ? -penum->adjust
				                          : penum->adjust);
	...
	last:
		if(stop < endp && (*stop || !masked)) {
			...
			code =
			    (*fill_pgram)(dev, xrun, yrun, xl - xrun,
			                  ytf - yrun, pdyx, pdyy, pdevc, lop);
		}
#undef xl
#undef ytf
