			...
			if(second_level_sizeof > 0) {
				...
				for(; curr_string < end_string; ++curr_string)
					if(!curr_string->persistent) {
						...
						((gs_param_string *)curr_string)->data = second_level_memory;
			     ...
			     gs_param_dict *pvalue)
{
	...
	return c_param_write(cplist, pkey, pvalue->list,
			     (dlist->coll_type == gs_param_collection_dict_int_keys ? gs_param_type_dict_int_keys : dlist->coll_type == gs_param_collection_array ? gs_param_type_array : gs_param_type_dict));
}
private
int
		    ...
		    gs_param_typed_value *pvalue)
{
	...
	return c_param_begin_write_collection(plist, pkey, &pvalue->value.d, coll_type);
}
		   ...
		   gs_param_typed_value *pvalue)
{
	...
	if(pparam == 0)
		return (cplist->target ? param_read_typed(cplist->target, pkey, pvalue) : 1);
	...
	if(code == gs_error_typecheck &&
	   ...
	   pvalue->type == gs_param_type_int_array) {
		...
		if(pparam->alternate_typed_data == 0) {
			if((pparam->alternate_typed_data = (void *)gs_alloc_bytes_immovable(cplist->memory,
			      ...
			      gs_param_dict *pvalue, gs_param_collection_type_t coll_type)
{
	...
	if(pparam == 0)
		return (cplist->target ? param_begin_read_collection(cplist->target,
