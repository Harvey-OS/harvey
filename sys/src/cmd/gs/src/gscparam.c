...
/* Lengths of *actual* data-containing type pointed to or contained by
 * gs_param_type_xxx's */
const byte gs_param_type_base_sizes[] = {GS_PARAM_TYPE_BASE_SIZES(0)};
...
gs_private_st_composite(st_c_param, gs_c_param, "gs_c_param", c_param_enum_ptrs,
                        c_param_reloc_ptrs);
ENUM_PTRS_WITH(c_param_enum_ptrs, gs_c_param* param)
{
	...
	default: {
		...
		return gs_param_typed_value_enum_ptrs(
		    mem, &value, sizeof(value), index, pep, NULL, gcst);
	}
	}
}
case 0:
return ENUM_OBJ(param->next);
case 1:
return ENUM_OBJ(param->alternate_typed_data);
case 2:
...
RELOC_PTRS_WITH(c_param_reloc_ptrs, gs_c_param* param)
{
	...
	case gs_param_type_dict_int_keys:
	case gs_param_type_array:
		RELOC_USING(st_c_param_list, &param->value.d,
		            sizeof(param->value.d));
		break;
	default: {
		gs_param_typed_value value;
		...
		value.type = param->type;
		gs_param_typed_value_reloc_ptrs(&value, sizeof(value), NULL,
		                                gcst);
...
c_param_find(const gs_c_param_list* plist, gs_param_name pkey, bool any)
{
	...
	for(; pparam != 0; pparam = pparam->next)
		if(pparam->key.size == len &&
		   !memcmp(pparam->key.data, pkey, len))
			return (pparam->type != gs_param_type_any || any
			            ? pparam
			            : 0);
	return 0;
}
...
private
param_proc_begin_xmit_collection(c_param_begin_write_collection);
private
param_proc_end_xmit_collection(c_param_end_write_collection);
private
param_proc_xmit_typed(c_param_write_typed);
private
param_proc_request(c_param_request);
private
param_proc_requested(c_param_requested);
private
const gs_param_list_procs c_write_procs = {
    c_param_write_typed, c_param_begin_write_collection,
    c_param_end_write_collection, NULL, /* get_next_key */
    c_param_request, c_param_requested};
	...
	while((pparam = plist->head) != 0) {
		...
		case gs_param_type_name_array:
			if(!pparam->value.s.persistent)
				gs_free_const_object(
				    mem, pparam->value.s.data,
...
{
	gs_c_param* pparam = gs_alloc_struct(plist->memory, gs_c_param,
	                                     &st_c_param, "c_param_add entry");
	uint len = strlen(pkey);
	...
	if(!plist->persistent_keys) {
		...
		byte* str =
		    gs_alloc_string(plist->memory, len, "c_param_add key");
              ...
              gs_param_type type)
{
	...
	case gs_param_type_int_array:
	case gs_param_type_float_array:
		if(!pparam->value.s.persistent) { /* Allocate & copy object
		                                     pointed to by array or
		                                     string */
			byte* top_level_memory = NULL;

			top_level_sizeof = pparam->value.s.size *
			                   gs_param_type_base_sizes[type];
			if(top_level_sizeof + second_level_sizeof > 0) {
				top_level_memory = gs_alloc_bytes_immovable(
				    plist->memory,
				...
				memcpy(top_level_memory, pparam->value.s.data,
				       top_level_sizeof);
			}
			...
			if(second_level_sizeof > 0) {
				...
				curr_string = pparam->value.sa.data;
				end_string =
				    curr_string + pparam->value.sa.size;
				for(; curr_string < end_string; ++curr_string)
					if(!curr_string->persistent) {
						memcpy(second_level_memory,
						       curr_string->data,
						       curr_string->size);
						((gs_param_string*)curr_string)
						    ->data =
						    second_level_memory;
						second_level_memory +=
						    curr_string->size;
                               ...
                               gs_param_collection_type_t coll_type)
{
	...
	gs_c_param_list* dlist = gs_c_param_list_alloc(
	    cplist->memory, "c_param_begin_write_collection");
                             ...
                             gs_param_dict* pvalue)
{
	...
	return c_param_write(
	    cplist, pkey, pvalue->list,
                    ...
                    gs_param_typed_value* pvalue)
{
	...
	case gs_param_type_array:
		...
		break;
	default:
		return c_param_write(cplist, pkey, &pvalue->value,
		                     pvalue->type);
	}
	return c_param_begin_write_collection(plist, pkey, &pvalue->value.d,
	                                      coll_type);
}
...
private
param_proc_begin_xmit_collection(c_param_begin_read_collection);
private
param_proc_end_xmit_collection(c_param_end_read_collection);
private
param_proc_xmit_typed(c_param_read_typed);
private
param_proc_next_key(c_param_get_next_key);
private
param_proc_get_policy(c_param_read_get_policy);
private
param_proc_signal_error(c_param_read_signal_error);
private
param_proc_commit(c_param_read_commit);
private
const gs_param_list_procs c_read_procs = {
    c_param_read_typed, c_param_begin_read_collection,
    c_param_end_read_collection, c_param_get_next_key, NULL, /* request, N/A */
    NULL, /* requested, N/A */
    c_param_read_get_policy, c_param_read_signal_error, c_param_read_commit};
                   ...
                   gs_param_typed_value* pvalue)
{
	...
	if(pparam == 0)
		return (cplist->target
		            ? param_read_typed(cplist->target, pkey, pvalue)
		            : 1);
	   ...
	   pvalue->type == gs_param_type_int_array) {
		...
		if(pparam->alternate_typed_data == 0) {
			if((pparam->alternate_typed_data =
			        (void*)gs_alloc_bytes_immovable(
			            cplist->memory, fa.size * sizeof(float),
			            "gs_c_param_read alternate float array")) ==
			   0)
				return_error(gs_error_VMerror);

			for(element = 0; element < fa.size; ++element)
				((float*)(pparam->alternate_typed_data))
				    [element] =
                              ...
                              gs_param_collection_type_t coll_type)
{
	...
	if(pparam == 0)
		return (cplist->target
		            ? param_begin_read_collection(cplist->target, pkey,
		                                          pvalue, coll_type)
		            : 1);
	switch(pparam->type) {
	case gs_param_type_dict:
                         ...
                         gs_param_key_t* key)
{
	gs_c_param_list* const cplist = (gs_c_param_list*)plist;
	gs_c_param* pparam =
	    (penum->pvoid ? ((gs_c_param*)(penum->pvoid))->next : cplist->head);
