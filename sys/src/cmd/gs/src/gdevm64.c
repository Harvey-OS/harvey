...
/* Procedures */
declare_mem_procs(mem_true64_copy_mono, mem_true64_copy_color,
                  mem_true64_fill_rectangle);

/* The device descriptor. */
const gx_device_memory mem_true64_device = mem_full_alpha_device(
    "image64", 64, 0, mem_open, gx_default_rgb_map_rgb_color,
    gx_default_rgb_map_color_rgb, mem_true64_copy_mono, mem_true64_copy_color,
    mem_true64_fill_rectangle, gx_default_map_cmyk_color, gx_default_copy_alpha,
...
#define put8(ptr, abcd, efgh) (ptr)[0] = abcd, (ptr)[1] = efgh
...
#define declare_unpack_color(abcd, efgh, color)                                \
	bits32 abcd = (bits32)(                                                \
	    (0x000000ff & ((color) >> 56)) | (0x0000ff00 & ((color) >> 40)) |  \
	    (0x00ff0000 & ((color) >> 24)) | (0xff000000 & ((color) >> 8)));   \
	bits32 efgh = (bits32)(                                                \
	    (0x000000ff & ((color) >> 24)) | (0x0000ff00 & ((color) >> 8)) |   \
	    (0x00ff0000 & ((color) << 8)) | (0xff000000 & ((color) << 24)))
#endif
...
private
int
mem_true64_fill_rectangle(gx_device* dev, int x, int y, int w, int h,
...
private
int
mem_true64_copy_mono(gx_device* dev, const byte* base, int sourcex, int sraster,
...
private
int
mem_true64_copy_color(gx_device* dev, const byte* base, int sourcex,
...
/* Procedures */
declare_mem_procs(mem64_word_copy_mono, mem64_word_copy_color,
                  mem64_word_fill_rectangle);

/* Here is the device descriptor. */
const gx_device_memory mem_true64_word_device =
    mem_full_device("image64w", 64, 0, mem_open, gx_default_rgb_map_rgb_color,
                    gx_default_rgb_map_color_rgb, mem64_word_copy_mono,
                    mem64_word_copy_color, mem64_word_fill_rectangle,
...
private
int
mem64_word_copy_mono(gx_device* dev, const byte* base, int sourcex, int sraster,
                     gx_bitmap_id id, int x, int y, int w, int h,
                     gx_color_index zero, gx_color_index one)
{
	gx_device_memory* const mdev = (gx_device_memory*)dev;
	...
	mem_true64_copy_mono(dev, base, sourcex, sraster, id, x, y, w, h, zero,
	                     one);
...
private
int
mem64_word_copy_color(gx_device* dev, const byte* base, int sourcex,
                      int sraster, gx_bitmap_id id, int x, int y, int w, int h)
{
	gx_device_memory* const mdev = (gx_device_memory*)dev;
	byte* row;
