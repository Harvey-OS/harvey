...
color_rgb_to_gray(frac r, frac g, frac b, const gs_imager_state* pis)
{
	return (r * (unsigned long)lum_red_weight +
	        ...
	        b * (unsigned long)lum_blue_weight + (lum_all_weights / 2)) /
	       lum_all_weights;
}
                  ...
                  frac cmyk[4])
{
	 ...
	 */
	frac bg = (pis == NULL ? k : pis->black_generation == NULL
	                                 ? frac_0
	                                 : gx_map_color_frac(pis, k,
	                                                     black_generation));
	signed_frac ucr =
	    (pis == NULL ? k
	                 : pis->undercolor_removal == NULL
	                       ? frac_0
	...
	if(ucr == frac_1)
		...
		cmyk[0] = (is_fneg(v) ? frac_0 : v >= (float)frac_1 ? frac_1
		                                                    : (frac)v);
		v = (float)frac_1 - g / denom; /* unscaled */
		cmyk[1] = (is_fneg(v) ? frac_0 : v >= (float)frac_1 ? frac_1
		                                                    : (frac)v);
		v = (float)frac_1 - b / denom; /* unscaled */
		cmyk[2] = (is_fneg(v) ? frac_0 : v >= (float)frac_1 ? frac_1
		                                                    : (frac)v);
#endif
	}
	cmyk[3] = bg;
	if_debug7('c', "[c]RGB 0x%x,0x%x,0x%x -> CMYK 0x%x,0x%x,0x%x,0x%x\n", r,
	          g, b, cmyk[0], cmyk[1], cmyk[2], cmyk[3]);
}
...
#define deduct_black(v) (prod = (frac_1 - (v)) * not_k, frac_1_quo(prod))
