...
const gs_color_space_type gs_color_space_type_DeviceGray = {
    gs_color_space_index_DeviceGray, true, true, &st_base_color_space,
    gx_num_components_1, gx_no_base_space, gx_init_paint_1,
    gx_restrict01_paint_1, gx_same_concrete_space, gx_concretize_DeviceGray,
    gx_remap_concrete_DGray, gx_remap_DeviceGray, gx_no_install_cspace,
    gx_spot_colors_set_overprint, gx_no_adjust_cspace_count,
    gx_no_adjust_color_count, gx_serialize_cspace_type,
    gx_cspace_is_linear_default};
private
const gs_color_space_type gs_color_space_type_DeviceRGB = {
    gs_color_space_index_DeviceRGB, true, true, &st_base_color_space,
    gx_num_components_3, gx_no_base_space, gx_init_paint_3,
    gx_restrict01_paint_3, gx_same_concrete_space, gx_concretize_DeviceRGB,
    gx_remap_concrete_DRGB, gx_remap_DeviceRGB, gx_no_install_cspace,
    gx_spot_colors_set_overprint, gx_no_adjust_cspace_count,
    gx_no_adjust_color_count, gx_serialize_cspace_type,
    gx_cspace_is_linear_default};

private
cs_proc_set_overprint(gx_set_overprint_DeviceCMYK);

private
const gs_color_space_type gs_color_space_type_DeviceCMYK = {
    gs_color_space_index_DeviceCMYK, true, true, &st_base_color_space,
    gx_num_components_4, gx_no_base_space, gx_init_paint_4,
    gx_restrict01_paint_4, gx_same_concrete_space, gx_concretize_DeviceCMYK,
    gx_remap_concrete_DCMYK, gx_remap_DeviceCMYK, gx_no_install_cspace,
    gx_set_overprint_DeviceCMYK, gx_no_adjust_cspace_count,
    gx_no_adjust_color_count, gx_serialize_cspace_type,
    gx_cspace_is_linear_default};
...
int
gs_cspace_alloc(gs_color_space** ppcspace, const gs_color_space_type* pcstype,
                gs_memory_t* mem)
{
	gs_color_space* pcspace = gs_alloc_struct(
	    mem, gs_color_space, &st_color_space, "gs_cspace_alloc");
...
gs_cspace_init_DeviceGray(const gs_memory_t* mem, gs_color_space* pcs)
{
	...
	if(dev_gray_proto.id == 0)
		gs_cspace_init(&dev_gray_proto, &gs_color_space_type_DeviceGray,
...
gs_cspace_init_DeviceRGB(const gs_memory_t* mem, gs_color_space* pcs)
{
	...
	if(dev_rgb_proto.id == 0)
		gs_cspace_init(&dev_rgb_proto, &gs_color_space_type_DeviceRGB,
		               (gs_memory_t*)mem, false);
	*pcs = dev_rgb_proto;
	return 0;
}
...
gs_cspace_init_DeviceCMYK(const gs_memory_t* mem, gs_color_space* pcs)
{
	...
	if(dev_cmyk_proto.id == 0)
		gs_cspace_init(&dev_cmyk_proto, &gs_color_space_type_DeviceCMYK,
		               (gs_memory_t*)mem, false);
	*pcs = dev_cmyk_proto;
	return 0;
}
...
check_cmyk_color_model_comps(gx_device* dev)
{
	...
	process_comps =
	    ((gx_color_index)1 << cyan_c) | ((gx_color_index)1 << magenta_c) |
	    ((gx_color_index)1 << yellow_c) | ((gx_color_index)1 << black_c);
...
gx_set_overprint_DeviceCMYK(const gs_color_space* pcs, gs_state* pgs)
{
	...
	if(!pgs->overprint || pgs->overprint_mode != 1 || pcinfo == 0 ||
...
gx_cspace_no_linear(gs_direct_color_space* cs, const gs_imager_state* pis,
                    gx_device* dev, const gs_client_color* c0,
                    const gs_client_color* c1, const gs_client_color* c2,
                    const gs_client_color* c3, float smoothness)
{
...
{
	return cs->type->remap_color(cc, (const gs_color_space*)cs, dc, pis,
	                             dev, gs_color_select_texture);
}

private
inline void
interpolate_cc(gs_client_color* c, const gs_client_color* c0,
               const gs_client_color* c1, double t, int n)
{
	int i;

	for(i = 0; i < n; i++)
		c->paint.values[i] =
		    c0->paint.values[i] * t + c1->paint.values[i] * (1 - t);
}
                    ...
                    double t, int n, float smoothness)
{
	if(c0->type == &gx_dc_type_data_pure) {
		...
		for(i = 0; i < n; i++) {
			...
			int max_color = (i == dev->color_info.gray_index
			                     ? dev->color_info.max_gray
			                     : dev->color_info.max_color);
			int b0 = (pure0 >> shift) & mask,
			    b1 = (pure1 >> shift) & mask;
...
private
int
gx_cspace_is_linear_in_line(gs_direct_color_space* cs,
                            const gs_imager_state* pis, gx_device* dev,
                            const gs_client_color* c0,
                            const gs_client_color* c1, float smoothness)
{
...
private
int
gx_cspace_is_linear_in_triangle(gs_direct_color_space* cs,
                                const gs_imager_state* pis, gx_device* dev,
                                const gs_client_color* c0,
                                const gs_client_color* c1,
                                const gs_client_color* c2, float smoothness)
{
	/* We check 4 points - the median center, and middle points of 3 sides.
	   Hopely this is enough for reasonable color spaces and color
	   renderings.
	...
	if(code < 0)
		return code;
	if(!is_dc_nearly_linear(dev, &d012, &d[2], &d01, 2.0 / 3, n,
	                        smoothness))
		return 0;
...
int
gx_cspace_is_linear_default(gs_direct_color_space* cs,
                            const gs_imager_state* pis, gx_device* dev,
                            const gs_client_color* c0,
                            const gs_client_color* c1,
                            const gs_client_color* c2,
                            const gs_client_color* c3, float smoothness)
{
	/* Assuming 2 <= nc <= 4. We don't need other cases. */
	/* With nc == 4 assuming a convex plain quadrangle in the client color
	 * space. */
	int code;
	...
	if(dev->color_info.separable_and_linear != GX_CINFO_SEP_LIN)
		return_error(gs_error_rangecheck);
	if(c2 == NULL)
		return gx_cspace_is_linear_in_line(cs, pis, dev, c0, c1,
		                                   smoothness);
	code = gx_cspace_is_linear_in_triangle(cs, pis, dev, c0, c1, c2,
	                                       smoothness);
	if(code <= 0)
		return code;
	if(c3 == NULL)
		return 1;
	return gx_cspace_is_linear_in_triangle(cs, pis, dev, c1, c2, c3,
	                                       smoothness);
}
