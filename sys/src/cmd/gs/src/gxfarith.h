...
#if USE_FPU <= 0 && arch_floats_are_IEEE &&                                    \
    (arch_sizeof_float == arch_sizeof_int ||                                   \
     arch_sizeof_float == arch_sizeof_long)
...
#define _ftest(v, f, n) (sizeof(v) == sizeof(float) ? (f) : (n))
#ifdef _d_int_t
#define _fdtest(v, f, d, n)                                                    \
	(sizeof(v) == sizeof(float) ? (f) : sizeof(v) == sizeof(double) ? (d)  \
	                                                                : (n))
#else
#define _fdtest(v, f, d, n) _ftest(v, f, n)
#endif
...
#if arch_sizeof_float == arch_sizeof_int
#define is_fneg(f)                                                             \
	(sizeof(f) == sizeof(float) ? _f_as_int(f) < 0 : _is_fnegb(f))
#else
...
#if arch_sizeof_float == arch_sizeof_int
#define is_fge1(f)                                                             \
	(sizeof(f) == sizeof(float) ? (_f_as_int(f)) >= IEEE_f1 : (f) >= 1.0)
#else /* arch_sizeof_float == arch_sizeof_long */
#define is_fge1(f)                                                             \
...
#define f_fits_in_bits(f, n)                                                   \
	_ftest(f, (_f_as_uint(f) & IEEE_expt) <                                \
	              IEEE_f1 + ((_f_uint_t)((n)-1) << 23),                    \
