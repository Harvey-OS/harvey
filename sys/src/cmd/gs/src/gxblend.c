...
static void
art_blend_luminosity_rgb_8(byte* dst, const byte* backdrop, const byte* src)
{
	...
	/*
	 * From section 7.4 of the PDF 1.5 specification, for RGB, the
	 * luminosity
	 * is:  Y = 0.30 R + 0.59 G + 0.11 B)
	 */
	delta_y =
	    ((rs - rb) * 77 + (gs - gb) * 151 + (bs - bb) * 28 + 0x80) >> 8;
	r = rb + delta_y;
	g = gb + delta_y;
	b = bb + delta_y;
	if((r | g | b) & 0x100) {
 ...
 */
static void
art_blend_luminosity_cmyk_8(byte* dst, const byte* backdrop, const byte* src)
{
...
static void
art_blend_saturation_rgb_8(byte* dst, const byte* backdrop, const byte* src)
{
	int rb = backdrop[0], gb = backdrop[1], bb = backdrop[2];
	int rs = src[0], gs = src[1], bs = src[2];
	int minb, maxb;
...
/* Our component values have already been complemented, i.e. (1 - X). */
static void
art_blend_saturation_cmyk_8(byte* dst, const byte* backdrop, const byte* src)
{
	/* Treat CMY the same as RGB */
...
const unsigned int art_blend_sq_diff_8[256] = {
    0,     256,   510,   762,   1012,  1260,  1506,  1750,  1992,  2231,  2469,
    2705,  2939,  3171,  3401,  3628,  3854,  4078,  4300,  4519,  4737,  4953,
    5166,  5378,  5588,  5795,  6001,  6204,  6406,  6606,  6803,  6999,  7192,
    7384,  7573,  7761,  7946,  8129,  8311,  8490,  8668,  8843,  9016,  9188,
    9357,  9524,  9690,  9853,  10014, 10173, 10331, 10486, 10639, 10790, 10939,
    11086, 11232, 11375, 11516, 11655, 11792, 11927, 12060, 12191, 12320, 12447,
    12572, 12695, 12816, 12935, 13052, 13167, 13280, 13390, 13499, 13606, 13711,
    13814, 13914, 14013, 14110, 14205, 14297, 14388, 14477, 14564, 14648, 14731,
    14811, 14890, 14967, 15041, 15114, 15184, 15253, 15319, 15384, 15446, 15507,
    15565, 15622, 15676, 15729, 15779, 15827, 15874, 15918, 15960, 16001, 16039,
    16075, 16110, 16142, 16172, 16200, 16227, 16251, 16273, 16293, 16311, 16327,
    16341, 16354, 16364, 16372, 16378, 16382, 16384, 16384, 16382, 16378, 16372,
    16364, 16354, 16341, 16327, 16311, 16293, 16273, 16251, 16227, 16200, 16172,
    16142, 16110, 16075, 16039, 16001, 15960, 15918, 15874, 15827, 15779, 15729,
    15676, 15622, 15565, 15507, 15446, 15384, 15319, 15253, 15184, 15114, 15041,
    14967, 14890, 14811, 14731, 14648, 14564, 14477, 14388, 14297, 14205, 14110,
    14013, 13914, 13814, 13711, 13606, 13499, 13390, 13280, 13167, 13052, 12935,
    12816, 12695, 12572, 12447, 12320, 12191, 12060, 11927, 11792, 11655, 11516,
    11375, 11232, 11086, 10939, 10790, 10639, 10486, 10331, 10173, 10014, 9853,
    9690,  9524,  9357,  9188,  9016,  8843,  8668,  8490,  8311,  8129,  7946,
    7761,  7573,  7384,  7192,  6999,  6803,  6606,  6406,  6204,  6001,  5795,
    5588,  5378,  5166,  4953,  4737,  4519,  4300,  4078,  3854,  3628,  3401,
    3171,  2939,  2705,  2469,  2231,  1992,  1750,  1506,  1260,  1012,  762,
    510,   256,   0};
...
const byte art_blend_soft_light_8[256] = {
    0,  3,  6,  9,  11, 14, 16, 19, 21, 23, 26, 28, 30, 32, 33, 35, 37, 39, 40,
    42, 43, 45, 46, 47, 48, 49, 51, 52, 53, 53, 54, 55, 56, 57, 57, 58, 58, 59,
    60, 60, 60, 61, 61, 62, 62, 62, 62, 63, 63, 63, 63, 63, 63, 63, 64, 64, 64,
    64, 64, 64, 64, 64, 64, 64, 64, 64, 64, 64, 64, 64, 64, 64, 63, 63, 63, 63,
    63, 63, 63, 63, 63, 63, 63, 62, 62, 62, 62, 62, 62, 62, 61, 61, 61, 61, 61,
    61, 60, 60, 60, 60, 60, 59, 59, 59, 59, 59, 58, 58, 58, 58, 57, 57, 57, 57,
    56, 56, 56, 56, 55, 55, 55, 55, 54, 54, 54, 54, 53, 53, 53, 52, 52, 52, 51,
    51, 51, 51, 50, 50, 50, 49, 49, 49, 48, 48, 48, 47, 47, 47, 46, 46, 46, 45,
    45, 45, 44, 44, 43, 43, 43, 42, 42, 42, 41, 41, 40, 40, 40, 39, 39, 39, 38,
    38, 37, 37, 37, 36, 36, 35, 35, 35, 34, 34, 33, 33, 33, 32, 32, 31, 31, 31,
    30, 30, 29, 29, 28, 28, 28, 27, 27, 26, 26, 25, 25, 25, 24, 24, 23, 23, 22,
    22, 21, 21, 21, 20, 20, 19, 19, 18, 18, 17, 17, 16, 16, 15, 15, 15, 14, 14,
    13, 13, 12, 12, 11, 11, 10, 10, 9,  9,  8,  8,  7,  7,  6,  6,  5,  5,  4,
    4,  3,  3,  2,  2,  1,  1,  0,  0};

void
art_blend_pixel_8(byte* dst, const byte* backdrop, const byte* src, int n_chan,
                  gs_blend_mode_t blend_mode)
{
	...
	case BLEND_MODE_SoftLight:
		for(i = 0; i < n_chan; i++) {
			...
			if(s < 0x80) {
				...
				t = ((s << 1) - 0xff) *
				    ((bits32)(art_blend_soft_light_8[b]));
				t += 0x80;
				t += (t >> 8);
				dst[i] = b + (t >> 8);
			}
	...
	case BLEND_MODE_Luminosity:
		switch(n_chan) {
		case 1: /* DeviceGray */
			dlprintf("art_blend_pixel_8: DeviceGray luminosity "
	...
	case BLEND_MODE_Color:
		switch(n_chan) {
		case 1: /* DeviceGray */
			dlprintf("art_blend_pixel_8: DeviceGray color blend "
	...
	case BLEND_MODE_Saturation:
		switch(n_chan) {
		case 1: /* DeviceGray */
			dlprintf("art_blend_pixel_8: DeviceGray saturation "
			         "blend mode not implemented\n");
			break;
		case 3: /* DeviceRGB */
			art_blend_saturation_rgb_8(dst, backdrop, src);
	...
	case BLEND_MODE_Hue: {
		...
		switch(n_chan) {
		case 1: /* DeviceGray */
			dlprintf("art_blend_pixel_8: DeviceGray hue blend mode "
			         "not implemented\n");
			break;
		case 3: /* DeviceRGB */
...
void
art_pdf_uncomposite_group_8(byte* dst, const byte* backdrop, const byte* src,
...
void
art_pdf_recomposite_group_8(byte* dst, byte* dst_alpha_g, const byte* src,
                            byte src_alpha_g, int n_chan, byte alpha,
                            gs_blend_mode_t blend_mode)
{
	byte dst_alpha;
	int i;
	...
	if(blend_mode == BLEND_MODE_Normal && alpha == 255) {
		...
		if(src_alpha_g == 255 || dst_alpha == 0) {
			...
			scale = (dst_alpha * 255 * 2 + src_alpha_g) /
			            (src_alpha_g << 1) -
			        dst_alpha;
			for(i = 0; i < n_chan; i++) {
				int si, di;
				...
				/* todo: it should be possible to optimize these
				 * cond branches */
				if(tmp < 0)
					tmp = 0;
				if(tmp > 255)
...
void
art_pdf_composite_group_8(byte* dst, byte* dst_alpha_g, const byte* src,
                          int n_chan, byte alpha, gs_blend_mode_t blend_mode)
{
	...
	if(alpha == 255) {
		...
		art_pdf_composite_pixel_alpha_8(dst, src_tmp, n_chan,
		                                blend_mode);
		if(dst_alpha_g != NULL) {
			tmp = (255 - *dst_alpha_g) * (255 - src_tmp[n_chan]) +
			      0x80;
			*dst_alpha_g = 255 - ((tmp + (tmp >> 8)) >> 8);
		}
	}
...
void
art_pdf_composite_knockout_simple_8(byte* dst, byte* dst_shape, const byte* src,
...
void
art_pdf_composite_knockout_isolated_8(byte* dst, byte* dst_shape,
                                      const byte* src, int n_chan, byte shape,
...
void
art_pdf_composite_knockout_8(byte* dst, byte* dst_alpha_g, const byte* backdrop,
                             const byte* src, int n_chan, byte shape,
                             byte alpha_mask, byte shape_mask,
                             gs_blend_mode_t blend_mode)
{
	...
	if(blend_mode == BLEND_MODE_Normal) {
		for(i = 0; i < n_chan; i++) {
			...
			c_s = src[i];
			c_b = backdrop[i];
			tmp = (c_b << 16) * scale_b + (c_s - c_b) + scale_src +
			      0x8000;
			ct[i] = tmp >> 16;
		}
	} else {
