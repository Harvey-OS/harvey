...
private
const cmap_operators_t cmap_cid_operators = {"begincidchar\n", "endcidchar\n",
                                             "begincidrange\n",
                                             "endcidrange\n"},
...
cmap_put_system_info(stream* s, const gs_cid_system_info_t* pcidsi)
{
	if(cid_system_info_is_null(pcidsi)) {
		...
		s_write_ps_string(s, pcidsi->Registry.data,
		                  pcidsi->Registry.size, 0);
		stream_puts(s, " def\n/Ordering ");
		s_write_ps_string(s, pcidsi->Ordering.data,
		                  pcidsi->Ordering.size, 0);
		pprintd1(s, " def\n/Supplement %d def\nend ",
		         pcidsi->Supplement);
	}
}
...
private
int
cmap_put_code_map(const gs_memory_t* mem, stream* s, int which,
                  const gs_cmap_t* pcmap, const cmap_operators_t* pcmo,
                  psf_put_name_chars_proc_t put_name_chars, int font_index_only)
{
	...
	for(gs_cmap_lookups_enum_init(pcmap, which, &lenum);
		...
		if(font_index_only >= 0 &&
		   lenum.entry.font_index != font_index_only)
			continue;
		if(font_index_only < 0 &&
		   lenum.entry.font_index != font_index) {
		...
		for(gi = 0; gi < num_entries; gi += 100) {
			...
			if(lenum.entry.key_is_range) {
				if(lenum.entry.value_type == CODE_VALUE_CID ||
				   lenum.entry.value_type ==
				       CODE_VALUE_NOTDEF) {
			...
			for(; i < ni; ++i) {
				...
				case CODE_VALUE_CHARS:
					stream_putc(s, '<');
					pput_hex(s, lenum.entry.value.data,
					         value_size);
					stream_putc(s, '>');
					break;
				case CODE_VALUE_GLYPH: {
					gs_const_string str;
					int code = pcmap->glyph_name(
					    mem, (gs_glyph)value, &str,
					    pcmap->glyph_name_data);

					if(code < 0)
						return code;
					stream_putc(s, '/');
					code = put_name_chars(s, str.data,
					                      str.size);
					if(code < 0)
						return code;
				} break;
...
int
psf_write_cmap(const gs_memory_t* mem, stream* s, const gs_cmap_t* pcmap,
...
#undef MAX_RANGES
...

	{
		...
		code =
		    cmap_put_code_map(mem, s, 1, pcmap, &cmap_notdef_operators,
