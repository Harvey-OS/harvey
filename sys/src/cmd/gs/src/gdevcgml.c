...
private
void begin_command(cgm_state*, cgm_op_index);

#define OP(op) begin_command(st, op)
private
cgm_result end_command(cgm_state*);

#define END_OP (void) end_command(st)
#define DONE return end_command(st)
/* Parameters */
private
void put_int(cgm_state*, cgm_int, int);
...
private
void put_real(cgm_state*, cgm_real, const cgm_precision*);

#define R(r) put_real(st, r, &st->metafile.real_precision)
private
void put_vdc(cgm_state*, const cgm_vdc*);

#define VDC(vdc) put_vdc(st, vdc)
#define VDC2(vdc1, vdc2)                                                       \
	VDC(vdc1);                                                             \
	VDC(vdc2)
#define VDC4(vdc1, vdc2, vdc3, vdc4)                                           \
	VDC2(vdc1, vdc2);                                                      \
	VDC2(vdc3, vdc4)
private
void put_vdc_r(cgm_state*, const cgm_line_marker_extent*,
               cgm_line_marker_specification_mode);

#define VDC_R(vdcr, mode) put_vdc_r(st, vdcr, mode)
private
void put_point(cgm_state*, const cgm_point*);

#define P(p) put_point(st, p)
private
void put_points(cgm_state*, const cgm_point*, int);

#define nP(p, n) put_points(st, p, n)
private
void put_string(cgm_state*, const char*, uint);

#define S(s, l) put_string(st, s, l)
private
void put_color(cgm_state*, const cgm_color*);

#define CO(co) put_color(st, co)
private
void put_rgb(cgm_state*, const cgm_rgb*);

#define CD(cd) put_rgb(st, cd)
/* Other data types */
#define put_byte(st, b)                                                        \
	if(st->command_count == command_max_count)                             \
		write_command(st, false);                                      \
	st->command[st->command_count++] = (byte)(b)
private
void put_bytes(cgm_state*, const byte*, uint);
private
void write_command(cgm_state*, bool);
private
void put_real_precision(cgm_state*, const cgm_precision*);
...
cgm_result
cgm_set_metafile_elements(cgm_state* st, const cgm_metafile_elements* meta,
                          int32_t mask)
{
	...
	if((mask & cgm_set_METAFILE_DESCRIPTION)) {
		...
		S(meta->metafile_description.chars,
		  meta->metafile_description.length);
		END_OP;
	...
	if((mask & cgm_set_METAFILE_ELEMENT_LIST)) {
		...
		for(i = 0, p = meta->metafile_element_list;
		    i < meta->metafile_element_list_count; i++, p += 2) {
			I(p[0]);
			I(p[1]);
		}
		END_OP;
...
cgm_result
cgm_set_picture_elements(cgm_state* st, const cgm_picture_elements* pic,
                         int32_t mask)
{
	...
	if((mask & cgm_set_LINE_WIDTH_SPECIFICATION_MODE)) {
		...
		END_OP;
		st->picture.line_width_specification_mode =
	...
	if((mask & cgm_set_MARKER_SIZE_SPECIFICATION_MODE)) {
		...
		END_OP;
		st->picture.marker_size_specification_mode =
		    pic->marker_size_specification_mode;
	}
	if((mask & cgm_set_EDGE_WIDTH_SPECIFICATION_MODE)) {
		OP(EDGE_WIDTH_SPECIFICATION_MODE);
		E(pic->edge_width_specification_mode);
		END_OP;
		st->picture.edge_width_specification_mode =
		    pic->edge_width_specification_mode;
	}
...
cgm_result
cgm_TEXT(cgm_state* st, const cgm_point* position, bool final, const char* str,
         uint len)
{
	OP(TEXT);
	P(position);
...
cgm_result
cgm_RESTRICTED_TEXT(cgm_state* st, const cgm_vdc* delta_width,
                    const cgm_vdc* delta_height, const cgm_point* position,
                    bool final, const char* str, uint len)
{
	OP(RESTRICTED_TEXT);
...
cgm_result
cgm_CELL_ARRAY(cgm_state* st, const cgm_point* pqr /*[3] */, cgm_int nx,
               cgm_int ny, cgm_int local_color_precision,
               cgm_cell_representation_mode mode, const byte* values,
...
cgm_result
cgm_CIRCULAR_ARC_3_POINT(cgm_state* st, const cgm_point* start,
                         const cgm_point* intermediate, const cgm_point* end)
{
...
cgm_result
cgm_CIRCULAR_ARC_3_POINT_CLOSE(cgm_state* st, const cgm_point* start,
                               const cgm_point* intermediate,
                               const cgm_point* end, cgm_arc_closure closure)
{
...
cgm_result
cgm_CIRCULAR_ARC_CENTER(cgm_state* st, const cgm_point* center,
                        const cgm_vdc* dx_start, const cgm_vdc* dy_start,
                        const cgm_vdc* dx_end, const cgm_vdc* dy_end,
                        const cgm_vdc* radius)
{
...
cgm_result
cgm_CIRCULAR_ARC_CENTER_CLOSE(cgm_state* st, const cgm_point* center,
                              const cgm_vdc* dx_start, const cgm_vdc* dy_start,
                              const cgm_vdc* dx_end, const cgm_vdc* dy_end,
                              const cgm_vdc* radius, cgm_arc_closure closure)
{
...
cgm_result
cgm_ELLIPSE(cgm_state* st, const cgm_point* center, const cgm_point* cd1_end,
            const cgm_point* cd2_end)
{
	OP(ELLIPSE);
	P(center);
	P(cd1_end);
...
cgm_result
cgm_ELLIPTICAL_ARC(cgm_state* st, const cgm_point* center,
                   const cgm_point* cd1_end, const cgm_point* cd2_end,
                   const cgm_vdc* dx_start, const cgm_vdc* dy_start,
                   const cgm_vdc* dx_end, const cgm_vdc* dy_end)
{
	OP(ELLIPTICAL_ARC);
...
cgm_result
cgm_ELLIPTICAL_ARC_CLOSE(cgm_state* st, const cgm_point* center,
                         const cgm_point* cd1_end, const cgm_point* cd2_end,
                         const cgm_vdc* dx_start, const cgm_vdc* dy_start,
                         const cgm_vdc* dx_end, const cgm_vdc* dy_end,
                         cgm_arc_closure closure)
{
...
cgm_result
cgm_CHARACTER_ORIENTATION(cgm_state* st, const cgm_vdc* x_up,
                          const cgm_vdc* y_up, const cgm_vdc* x_base,
                          const cgm_vdc* y_base)
{
...
cgm_result
cgm_TEXT_ALIGNMENT(cgm_state* st, cgm_text_alignment_horizontal align_h,
                   cgm_text_alignment_vertical align_v, cgm_real align_cont_h,
                   cgm_real align_cont_v)
{
	OP(TEXT_ALIGNMENT);
	E(align_h);
	E(align_v);
...
cgm_result
cgm_PATTERN_SIZE(cgm_state* st, const cgm_vdc* x_height,
                 const cgm_vdc* y_height, const cgm_vdc* x_width,
                 const cgm_vdc* y_width)
{
	OP(PATTERN_SIZE);
...
cgm_result
cgm_COLOR_TABLE(cgm_state* st, cgm_int index, const cgm_color* values,
                int count)
{
	int i;
...
cgm_result
cgm_ASPECT_SOURCE_FLAGS(cgm_state* st, const cgm_aspect_source_flag* flags,
                        int count)
{
	int i;
