...
#define VDC4(vdc1, vdc2, vdc3, vdc4) \
	...
	VDC2(vdc3, vdc4)
private
void put_vdc_r(cgm_state *, const cgm_line_marker_extent *, cgm_line_marker_specification_mode);
...
cgm_CELL_ARRAY(cgm_state *st, const cgm_point *pqr /*[3] */, cgm_int nx, cgm_int ny, cgm_int local_color_precision, cgm_cell_representation_mode mode, const byte *values, uint source_bit, uint raster)
{
	...
	bits_per_pixel =
	    (st->picture.color_selection_mode == cgm_color_selection_indexed ? precision : precision * 3);
