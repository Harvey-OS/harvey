...
private
int arc_add(const arc_curve_params_t* arc, bool is_quadrant);
...
{
	int code = gs_imager_arc_add(pgs->path, (gs_imager_state*)pgs, cw, xc,
	                             yc, rad, a1, a2, add);
...
int
gs_arc(gs_state* pgs, floatp xc, floatp yc, floatp r, floatp ang1, floatp ang2)
{
	return gs_arc_add_inline(pgs, false, xc, yc, r, ang1, ang2, true);
}

int
gs_arcn(gs_state* pgs, floatp xc, floatp yc, floatp r, floatp ang1, floatp ang2)
{
...
{
	return gs_arc_add_inline(pgs, clockwise, axc, ayc, arad, aang1, aang2,
	                         add_line);
}
...
next_arc_curve(arc_curve_params_t* arc, fixed anext)
{
	...
	double trad = arc->radius * tan(fixed2float(anext - arc->angle) *
...
gs_imager_arc_add(gx_path* ppath, gs_imager_state* pis, bool clockwise,
                  floatp axc, floatp ayc, floatp arad, floatp aang1,
                  floatp aang2, bool add_line)
{
	double ar = arad;
...
int
gs_arcto(gs_state* pgs, floatp ax1, floatp ay1, floatp ax2, floatp ay2,
...
#define ay0 up0.y
	...
	{ /* Now we have to compute the tangent points. */
		...
		if(denom == 0) {
			...
			if(code == 0)
				code = gx_setcurrentpoint_from_path(
...
arc_add(const arc_curve_params_t* arc, bool is_quadrant)
{
	...
	if((arc->action != arc_nothing &&
	   ...
	   (code = gs_point_transform2fixed(&pis->ctm, arc->p3.x, arc->p3.y,
	                                    &p3)) < 0 ||
#else
	    (code = gs_point_transform2fixed_rounding(&pis->ctm, x0, y0, &p0)) <
	        0) ||
	   (code = gs_point_transform2fixed_rounding(&pis->ctm, xt, yt, &pt)) <
	       0 ||
	   (code = gs_point_transform2fixed_rounding(&pis->ctm, arc->p3.x,
	                                             arc->p3.y, &p3)) < 0 ||
#endif
	   (code =
	        (arc->action == arc_nothing
	             ...
	             : arc->action == arc_lineto && path_position_valid(path)
	                   ...
	                   :
	                   /* action == arc_moveto, or lineto with no current
	                      point */
	...
	if(is_quadrant) {
		...
		if(arc->fast_quadrant > 0) {
			/*
			 * The CTM is well-behaved, and we have pre-calculated
			 * the delta
			...
			if(pt.x != p0.x)
				p0.x =
				    (pt.x > p0.x ? p0.x + delta : p0.x - delta);
			if(pt.y != p0.y)
				p0.y =
				    (pt.y > p0.y ? p0.y + delta : p0.y - delta);
			p2.x =
			    (pt.x == p3.x ? p3.x : pt.x > p3.x ? p3.x + delta
			                                       : p3.x - delta);
			p2.y =
			    (pt.y == p3.y ? p3.y : pt.y > p3.y ? p3.y + delta
			                                       : p3.y - delta);
			goto add;
		}
	} else {
...
add:
	if_debug8(
	    'r', "[r]Arc f=%f p0=(%f,%f) pt=(%f,%f) p3=(%f,%f) action=%d\n",
	    fraction, x0, y0, xt, yt, arc->p3.x, arc->p3.y, (int)arc->action);
...
void
make_quadrant_arc(gs_point* p, const gs_point* c, const gs_point* p0,
                  const gs_point* p1, double r)
{
	p[0].x = c->x + p0->x * r;
	p[0].y = c->y + p0->y * r;
...
gs_reversepath(gs_state* pgs)
{
	...
	if(pgs->current_point_valid) {
		...
		pgs->subpath_start.x =
		    fixed2float(rpath.segments->contents.subpath_current->pt.x);
		pgs->subpath_start.y =
		    fixed2float(rpath.segments->contents.subpath_current->pt.y);
	}
...
gs_path_enum_copy_init(gs_path_enum* penum, const gs_state* pgs, bool copy)
{
	...
	if(copy) {
		gx_path* copied_path = gx_path_alloc(mem, "gs_path_enum_init");
		int code;
...
gs_path_enum_next(gs_path_enum* penum, gs_point ppts[3])
{
	...
	case gs_pe_curveto:
		if((code = gs_point_transform_inverse(
		        fixed2float(fpts[1].x), fixed2float(fpts[1].y),
		        &penum->mat, &ppts[1])) < 0 ||
		   (code = gs_point_transform_inverse(
		        fixed2float(fpts[2].x), fixed2float(fpts[2].y),
		        &penum->mat, &ppts[2])) < 0)
			return code;
	/* falls through */
	case gs_pe_moveto:
