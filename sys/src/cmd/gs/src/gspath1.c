...
next_arc_quadrant(arc_curve_params_t *arc, fixed anext)
{
	 ...
	 */
		...
		if(is_fzero2(pis->ctm.xy, pis->ctm.yx) ? (scale = fabs(pis->ctm.xx)) == fabs(pis->ctm.yy) : is_fzero2(pis->ctm.xx, pis->ctm.yy) ? (scale = fabs(pis->ctm.xy)) == fabs(pis->ctm.yx) : 0) {
...
arc_add(const arc_curve_params_t *arc, bool is_quadrant)
{
...
#endif
	   (code =
		(arc->action == arc_nothing ? (p0.x = path->position.x, p0.y = path->position.y, 0) : arc->action == arc_lineto && path_position_valid(path) ? gx_path_add_line(path, p0.x, p0.y) :
	...
	if(is_quadrant) {
	     ...
	     */
			...
			p2.x = (pt.x == p3.x ? p3.x : pt.x > p3.x ? p3.x + delta : p3.x - delta);
			p2.y = (pt.y == p3.y ? p3.y : pt.y > p3.y ? p3.y + delta : p3.y - delta);
			goto add;
		}
	} else {
		double r = arc->radius;
