...
private_st_jbig2decode_state(); /* creates a gc object for our state, defined in
                                   sjbig2.h */
                    ...
                    Jbig2Severity severity, int32_t seg_idx)
{
	...
	if(seg_idx == -1)
		...
		sprintf(segment, "(segment 0x%02x)", seg_idx);
...
/* parse a globals stream packed into a gs_bytestring for us by the postscript
   layer and stuff the resulting context into a pointer for use in later
   decoding */
public
int
s_jbig2decode_make_global_ctx(byte* data, uint length,
                              Jbig2GlobalCtx** global_ctx)
{
	...
	if(length == 0) {
		if_debug0('s',
		          "[s] ignoring zero-length jbig2 global stream.\n");
		*global_ctx = NULL;
		return 0;
	}
                      ...
                      stream_cursor_write* pw, bool last)
{
	...
	if(out_size > 0) {
		...
		if(image != NULL) {
			...
			int32_t usable =
			    min(image_size - state->offset, out_size);
			memcpy(pw->ptr + 1, image->data + state->offset,
			       usable);
...
s_jbig2decode_release(stream_state* ss)
{
	...
	if(state->decode_ctx) {
		if(state->image)
			jbig2_release_page(state->decode_ctx, state->image);
...
const stream_template s_jbig2decode_template = {
    &st_jbig2decode_state, s_jbig2decode_init, s_jbig2decode_process, 1,
    1, /* min in and out buffer sizes we can handle --should be ~32k,64k for
          efficiency? */
