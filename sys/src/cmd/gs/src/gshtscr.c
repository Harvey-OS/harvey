...
ENUM_PTRS_WITH(screen_enum_enum_ptrs, gs_screen_enum* eptr)
{
	if(index < 1 + st_ht_order_max_ptrs) {
		gs_ptr_type_t ret = ENUM_USING(st_ht_order, &eptr->order,
		                               sizeof(eptr->order), index - 1);
...
ENUM_PTRS_END
private
RELOC_PTRS_WITH(screen_enum_reloc_ptrs, gs_screen_enum* eptr)
{
...
gx_compute_cell_values(gx_ht_cell_params_t* phcp)
{
	...
	if(M1 && N) {
		...
		phcp->S = 0;
	if_debug12('h', "[h]MNR=(%d,%d)/%d, M'N'R'=(%d,%d)/%d => C=%lu, D=%d, "
	                "D'=%d, W=%u, W'=%u, S=%d\n",
	           M, N, phcp->R, M1, N1, phcp->R1, C, D, D1, phcp->W, phcp->W1,
	           phcp->S);
}

/* Forward references */
private
int pick_cell_size(gs_screen_halftone* ph, const gs_matrix* pmat,
                   ulong max_size, uint min_levels, bool accurate,
...
int
gs_screen_init(gs_screen_enum* penum, gs_state* pgs, gs_screen_halftone* phsp)
{
...
{
	int code = gs_screen_order_init_memory(&penum->order, pgs, phsp,
	                                       accurate, mem);
	...
	if(!FORCE_STRIP_HALFTONES &&
	   ((uint32_t)porder->params.W1 * bitmap_raster(porder->params.W) +
		...
		code = gx_ht_alloc_order(porder, porder->params.W,
		                         porder->params.W1, 0, num_levels, mem);
                            ...
                            gs_memory_t* mem)
{
	...
	code = pick_cell_size(phsp, &imat, max_size, screen_min_screen_levels,
	                      accurate, &porder->params);
	if(code < 0)
		return code;
...
#define v0 uv0.y
...

	{
		...
		if_debug10('h', "[h]Requested: f=%g a=%g mat=[%g %g %g %g] "
		                "max_size=%lu min_levels=%u =>\n     u=%g "
		                "v=%g\n",
		           ph->frequency, ph->angle, pmat->xx, pmat->xy,
		           pmat->yx, pmat->yy, max_size, min_levels, u0, v0);
	}
...
try_size:
	better = false;
	{
		...
		for(p.M = m0 + 1; p.M >= m0; p.M--)
			for(p.N = n0 + 1; p.N >= n0; p.N--) {
				...
				double fr, ar, ft, at, f_diff, a_diff, f_err,
				    a_err;
				...
				if(landscape)
					...
					fr = 72.0 *
					     (p.M == 0
					          ...
					          : pmat->xx / p.M * cos(ar));
				ft = fabs(fr) * rt;
				/* Normalize the angle to the requested
				 * quadrant. */
				at = (ar * radians_to_degrees - rotation) *
				     reflection;
				...
				/*
				 * We used to compute the percentage difference
				 * here:
				 *      a_err = (a0 == 0 ? a_diff : a_diff /
				 * fabs(a0));
				 * but using the angle difference makes more
				 * sense:
				 */
				a_err = a_diff;

				if_debug5('h', " ==> d=%d, wt=%ld, "
				               "wt_size=%ld, f=%g, a=%g\n",
				          p.D, wt, bitmap_raster(wt) * wt, ft,
				          at);

				{
					/*
					 * Compute the error in position between
					 * ideal location.
					...
					double error =
					    ...
					    (fm0 - p.M) * (fm0 - p.M);
					/*
					 * Adjust the error by the length of the
					 * vector.  This gives
					 * a slight bias toward larger cell
					 * sizzes.
					 */
					error /= p.N * p.N + p.M * p.M;
					error = sqrt(error); /* The previous
					                        calcs. gave
					                        value squared */
					if(error > e_best)
						continue;
				...
				*phcp = p;
				f = ft, a = at;
				better = true;
				if_debug3('h', "*** best wt_size=%ld, "
				               "f_diff=%g, a_diff=%g\n",
				          wt_size, f_diff, a_diff);
				/*
				 * We want a maximum relative frequency error of
				 * 1% and a
	...
	if(phcp->C <
	   min_levels) { /* We don't have enough levels yet.  Keep going. */
		++rt;
		goto try_size;
	}
                           ...
                           gs_memory_t* mem)
{
	...
	if(porder->wse == NULL) {
		 ...
		 * (0,0), (U,V), (U-V',V+U'), (-V',U') to the square (+/-1,
		 * +/-1).
		...
		if_debug7('h', "[h]Screen: (%dx%d)/%d [%f %f %f %f]\n",
		          ...
		          penum->mat.xx, penum->mat.xy, penum->mat.yx,
		          penum->mat.yy);
	}
	return 0;
}
...
gs_screen_currentpoint(gs_screen_enum* penum, gs_point* ppt)
{
	...
	if((code = gs_point_transform(penum->x + 0.501, penum->y + 0.498,
	                              &penum->mat, &pt)) < 0)
		return code;
	...
	sx = ceil(pt.x / 2) * 2;
	sy = ceil(pt.y / 2) * 2;
	if((code = gs_point_transform(sx, sy, &penum->mat_inv, &spot_center)) <
	   0)
		return code;
...
//	sample = (ht_sample_t)((value+1) * max_ht_sample);
		...
		if(gs_debug_c('H')) {
			...
			dlprintf6("[H]sample x=%d y=%d (%f,%f): %f -> %u\n",
			          penum->x, penum->y, pt.x, pt.y, value,
			          sample);
		}
#endif
