...
*/
/* Portions Copyright (C) 1994-2000 Ghostgum Software Pty Ltd.  All rights
 * reserved. */
...
GSDLL_CALLBACK pgsdll_callback =
    NULL; /* callback for messages and stdio to caller */
...
/* local functions */
private
int GSDLLCALL gsdll_old_stdin(void* caller_handle, char* buf, int len);
private
int GSDLLCALL gsdll_old_stdout(void* caller_handle, const char* str, int len);
private
int GSDLLCALL gsdll_old_stderr(void* caller_handle, const char* str, int len);
private
int GSDLLCALL gsdll_old_poll(void* caller_handle);
...
gsdll_init(GSDLL_CALLBACK callback, HWND hwnd, int argc, char* argv[])
{
	...
	gsapi_set_stdio(pgs_minst, gsdll_old_stdin, gsdll_old_stdout,
	                gsdll_old_stderr);
...
gsdll_execute_cont(const char* str, int len)
{
	int exit_code;
	int code =
	    gsapi_run_string_continue(pgs_minst, str, len, 0, &exit_code);
	if(code == e_NeedInput)
...
int GSDLLEXPORT GSDLLAPI
gsdll_revision(const char** product, const char** copyright, int32_t* revision,
               int32_t* revisiondate)
{
	if(product)
		*product = gs_product;
	if(copyright)
