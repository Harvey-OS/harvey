...
#define stream_proc_init(proc) int proc(stream_state*)

/* Process a buffer.  See strimpl.h for details. */
#define stream_proc_process(proc)                                              \
	int proc(stream_state*, stream_cursor_read*, stream_cursor_write*, bool)

/* Release the stream state when closing. */
#define stream_proc_release(proc) void proc(stream_state*)

/* Initialize the client parameters to default values. */
#define stream_proc_set_defaults(proc) void proc(stream_state*)
...
#define stream_proc_reinit(proc) int proc(stream_state*)

/* Report an error.  Note that this procedure is stored in the state, */
/* not in the main stream structure. */
#define stream_proc_report_error(proc) int proc(stream_state*, const char*)
