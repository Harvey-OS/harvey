...
switch(pdfont->FontType) {
case ft_composite:
	return (pdfont->u.type0.cmap_is_standard ? ENUM_OBJ(0) : ENUM_CONST_STRING(&pdfont->u.type0.CMapName));
case ft_encrypted:
case ft_encrypted2:
case ft_TrueType:
		      ...
		      pdf_char_glyph_pair_t *pairs, int num_glyphs)
{
	...
	if(!embed_list_includes(&pdev->params.NeverEmbed, chars, size) ||
	   (index >= 0 &&
	    !(embed_as_standard_called ? do_embed_as_standard : (embed_as_standard_called = true,
								 (do_embed_as_standard = embed_as_standard(pdev, font, index, pairs, num_glyphs)))))
	...
	if(index >= 0 &&
	   (embed_as_standard_called ? do_embed_as_standard : embed_as_standard(pdev, font, index, pairs, num_glyphs)))
		return FONT_EMBED_STANDARD;
