...
case 0:
return ENUM_STRING(&pdfont->BaseFont);
case 1:
ENUM_RETURN(pdfont->FontDescriptor);
case 2:
ENUM_RETURN(pdfont->base_font);
case 3:
ENUM_RETURN(pdfont->Widths);
case 4:
ENUM_RETURN(pdfont->used);
case 5:
ENUM_RETURN(pdfont->res_ToUnicode);
case 6:
ENUM_RETURN(pdfont->cmap_ToUnicode);
case 7:
...
switch(pdfont->FontType) {
case ft_composite:
	return (pdfont->u.type0.cmap_is_standard
	            ? ENUM_OBJ(0)
...
private
const pdf_standard_font_info_t standard_font_info[] = {
...
private
int
find_std_appearance(const gx_device_pdf* pdev, gs_font_base* bfont, int mask,
                    pdf_char_glyph_pair_t* pairs, int num_glyphs)
{
	...
	for(i = 0; i < PDF_NUM_STANDARD_FONTS; ++psf, ++i) {
		...
		if(has_uid) {
			/*
			 * Require the UIDs to match.  The PostScript spec says
			 * this
		...
		/*
		 * Require the actual outlines to match (within the given
		 * subset).
		 */
		code = gs_copied_can_copy_glyphs(
		    (const gs_font*)cfont, (const gs_font*)bfont,
		    &pairs[0].glyph, num_glyphs, sizeof(pdf_char_glyph_pair_t),
		    true);
...
scan_for_standard_fonts(gx_device_pdf* pdev, const gs_font_dir* dir)
{
	...
	for(; orig; orig = orig->next) {
		...
		if(uid_is_UniqueID(&obfont->UID)) {
			...
			int i = pdf_find_standard_font_name(
			    orig->key_name.chars, orig->key_name.size);

			if(i >= 0 && pdf_standard_fonts(pdev)[i].pdfont == 0) {
				pdf_font_resource_t* pdfont;
				int code = pdf_font_std_alloc(
				    pdev, &pdfont, true, orig->id, obfont, i);
...
pdf_outline_fonts_alloc(gs_memory_t* mem)
{
	...
	pdf_standard_font_t* ppsf = gs_alloc_struct_array(
	    mem, PDF_NUM_STANDARD_FONTS, pdf_standard_font_t,
...
private
int
pdf_resize_array(gs_memory_t* mem, void** p, int elem_size, int old_size,
                 int new_size)
{
	void* q =
	    gs_alloc_byte_array(mem, new_size, elem_size, "pdf_resize_array");

	if(q == NULL)
		return_error(gs_error_VMerror);
	memset((char*)q + elem_size * old_size, 0,
	       elem_size * (new_size - old_size));
                    ...
                    pdf_font_write_contents_proc_t write_contents)
{
	...
	int code;
	bool is_CID_font =
	    (ftype == ft_CID_encrypted || ftype == ft_CID_TrueType);

	if(chars_count != 0) {
		uint size = (chars_count + 7) / 8;

		if(!is_CID_font) {
			widths = (void*)gs_alloc_byte_array(
			    mem, chars_count, sizeof(*widths),
			    "font_resource_alloc(Widths)");
		} else {
			/*  Delay allocation because we don't know which WMode
			 * will be used. */
		}
	...
	code =
	    pdf_alloc_resource(pdev, rtype, rid, (pdf_resource_t**)&pfres, 0L);
	if(code < 0)
		goto fail;
...
font_resource_encoded_alloc(gx_device_pdf* pdev, pdf_font_resource_t** ppfres,
                            ...
                            pdf_font_write_contents_proc_t write_contents)
{
	pdf_encoding_element_t* Encoding = gs_alloc_struct_array(
	    pdev->pdf_memory, 256, pdf_encoding_element_t,
	    &st_pdf_encoding_element, "font_resource_encoded_alloc");
	gs_point* v = (gs_point*)gs_alloc_byte_array(
	    pdev->pdf_memory, 256, sizeof(gs_point), "pdf_font_simple_alloc");
	pdf_font_resource_t* pdfont;
	int code, i;
	...
	code = font_resource_simple_alloc(pdev, &pdfont, rid, ftype, 256,
	                                  write_contents);
	if(code < 0) {
		gs_free_object(pdev->pdf_memory, Encoding,
		               "font_resource_encoded_alloc");
...
int
pdf_resize_resource_arrays(gx_device_pdf* pdev, pdf_font_resource_t* pfres,
                           int chars_count)
{
	...
	if(pfres->Widths != NULL) {
		code = pdf_resize_array(mem, (void**)&pfres->Widths,
		                        sizeof(*pfres->Widths), pfres->count,
		                        chars_count);
		if(code < 0)
			return code;
	}
	...
	if(code < 0)
		return code;
	if(pfres->FontType == ft_CID_encrypted ||
	   pfres->FontType == ft_CID_TrueType) {
		if(pfres->u.cidfont.v != NULL) {
			code =
			    pdf_resize_array(mem, (void**)&pfres->u.cidfont.v,
			                     sizeof(*pfres->u.cidfont.v),
			                     pfres->count * 2, chars_count * 2);
			if(code < 0)
				return code;
		}
		if(pfres->u.cidfont.Widths2 != NULL) {
			code = pdf_resize_array(
			    mem, (void**)&pfres->u.cidfont.Widths2,
			    sizeof(*pfres->u.cidfont.Widths2), pfres->count,
			    chars_count);
			if(code < 0)
				return code;
		}
	...
	if(pfres->FontType == ft_CID_TrueType) {
		if(pfres->u.cidfont.CIDToGIDMap != NULL) {
			code = pdf_resize_array(
			    mem, (void**)&pfres->u.cidfont.CIDToGIDMap,
			    sizeof(*pfres->u.cidfont.CIDToGIDMap), pfres->count,
			    chars_count);
			if(code < 0)
				return code;
		}
	...
	if(pfres->FontType == ft_CID_encrypted ||
	   pfres->FontType == ft_CID_TrueType) {
		if(pfres->u.cidfont.used2 != NULL) {
			code = pdf_resize_array(
			    mem, (void**)&pfres->u.cidfont.used2,
                    ...
                    uint size)
{
	...
	for(i = 0; i < psa->size; ++i)
		if(!bytes_compare(psa->data[i].data, psa->data[i].size, chars,
		                  size))
			return true;
	return false;
}
                  ...
                  pdf_char_glyph_pair_t* pairs, int num_glyphs)
{
	...
	if(find_std_appearance(pdev, (gs_font_base*)font, -1, pairs,
	                       num_glyphs) == index)
		return true;
	if(!scan_for_standard_fonts(pdev, font->dir))
		return false;
	return (find_std_appearance(pdev, (gs_font_base*)font, -1, pairs,
	                            num_glyphs) == index);
}
private
bool
 ...
 */
const gs_font_name*
pdf_choose_font_name(gs_font* font, bool key_name)
{
	return key_name ? (font->key_name.size != 0 ? &font->key_name
	                                            : &font->font_name)
	                : (font->font_name.size != 0 ? &font->font_name
	                                             : &font->key_name);
}
pdf_font_embed_t
                      ...
                      pdf_char_glyph_pair_t* pairs, int num_glyphs)
{
	...
	if(pdev->CompatibilityLevel < 1.3) {
		if(index >= 0 && (embed_as_standard_called = true,
		                  do_embed_as_standard = embed_as_standard(
		                      pdev, font, index, pairs, num_glyphs))) {
	...
	if(!embed_list_includes(&pdev->params.NeverEmbed, chars, size) ||
	   (index >= 0 &&
	    !(embed_as_standard_called
	          ? do_embed_as_standard
                     ...
                     bool finish)
{
	...
	if(pdfont->FontType == ft_composite) {
		...
		if(pdsubf->FontType == ft_CID_encrypted ||
		   pdsubf->FontType == ft_CID_TrueType)
	...
	case ft_composite:
		if(extra) {
			data[size] = '-';
			memcpy(data + size + 1, pdfont->u.type0.CMapName.data,
			       extra - 1);
			size += extra;
		}
		break;
	...
	if(finish && pdfont->FontDescriptor != NULL &&
	   ...
	   pdf_font_descriptor_embedding(pdfont->FontDescriptor)) {
		int code = pdf_add_subset_prefix(pdev, &fname, pdfont->used,
		                                 pdfont->count);
...
{
	int code =
	    font_resource_alloc(pdev, ppfres, resourceFont, rid, ft_composite,
...
pdf_font_type3_alloc(gx_device_pdf* pdev, pdf_font_resource_t** ppfres,
                     pdf_font_write_contents_proc_t write_contents)
{
	return font_resource_simple_alloc(pdev, ppfres, gs_no_id,
	                                  ft_user_defined, 256, write_contents);
}
...
{
	pdf_font_resource_t* pdfont;
	int code = font_resource_encoded_alloc(
	    pdev, &pdfont, rid, pfont->FontType, pdf_write_contents_std);
	...
	gs_matrix* orig_matrix =
	    (is_original ? &pfont->FontMatrix : &psf->orig_matrix);

	if(code < 0 ||
	   (code = pdf_base_font_alloc(pdev, &pdfont->base_font, pfont,
	                               orig_matrix, true, true)) < 0)
		return code;
                       ...
                       gs_id rid, pdf_font_descriptor_t* pfd)
{
	...
	case ft_CID_encrypted:
		chars_count =
		    ((const gs_font_cid0*)font)->cidata.common.CIDCount;
		pcidsi =
		    &((const gs_font_cid0*)font)->cidata.common.CIDSystemInfo;
		write_contents = pdf_write_contents_cid0;
		break;
	case ft_CID_TrueType:
		chars_count =
		    ((const gs_font_cid2*)font)->cidata.common.CIDCount;
		pcidsi =
		    &((const gs_font_cid2*)font)->cidata.common.CIDSystemInfo;
	...
	code = font_resource_alloc(pdev, &pdfont, resourceCIDFont, rid,
	                           FontType, chars_count, write_contents);
	if(code < 0)
		return code;
	pdfont->FontDescriptor = pfd;
	...
	/* Don' know whether the font will use WMode 1,
	   so reserve it now. */
	pdfont->u.cidfont.used2 = gs_alloc_bytes(
	    pdev->pdf_memory, (chars_count + 7) / 8, "pdf_font_cidfont_alloc");
...
int
pdf_obtain_cidfont_widths_arrays(gx_device_pdf* pdev,
                                 pdf_font_resource_t* pdfont, int wmode,
                                 double** w, double** w0, double** v)
{
	gs_memory_t* mem = pdev->pdf_memory;
	...
	*w = (wmode ? pdfont->u.cidfont.Widths2 : pdfont->Widths);
	if(*w == NULL) {
		ww = (double*)gs_alloc_byte_array(
		    mem, chars_count, sizeof(*ww),
		    "pdf_obtain_cidfont_widths_arrays");
		if(wmode) {
			vv = (double*)gs_alloc_byte_array(
			    mem, chars_count, sizeof(*vv) * 2,
			    "pdf_obtain_cidfont_widths_arrays");
			if(pdfont->Widths == 0) {
				ww0 = (double*)gs_alloc_byte_array(
				    mem, chars_count, sizeof(*ww0),
				    "pdf_obtain_cidfont_widths_arrays");
				pdfont->Widths = *w0 = ww0;
				if(ww0 != 0)
					memset(ww0, 0,
					       chars_count * sizeof(*ww));
			} else
				*w0 = ww0 = pdfont->Widths;
		}
		if(ww == 0 || (wmode && vv == 0) || (wmode && ww0 == 0)) {
			gs_free_object(mem, ww,
			               "pdf_obtain_cidfont_widths_arrays");
			gs_free_object(mem, vv,
			               "pdf_obtain_cidfont_widths_arrays");
			gs_free_object(mem, ww0,
			               "pdf_obtain_cidfont_widths_arrays");
			return_error(gs_error_VMerror);
		}
		if(wmode)
