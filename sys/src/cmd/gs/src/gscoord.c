...
private
void trace_matrix_fixed(const gs_matrix_fixed*);
private
void trace_matrix(const gs_matrix*);
...
gs_defaultmatrix(const gs_state* pgs, gs_matrix* pmat)
{
	...
	pmat->tx += dev->Margins[0] * dev->HWResolution[0] /
	            dev->MarginsHWResolution[0];
	pmat->ty += dev->Margins[1] * dev->HWResolution[1] /
...
{
	int code =
	    gs_matrix_rotate(&ctm_only(pgs), ang, &ctm_only_writable(pgs));
...
{ /* If the matrix isn't skewed, we get more accurate results */
	/* by using transform_inverse than by using the inverse matrix. */
	if(!is_skewed(&pgs->ctm)) {
		return gs_distance_transform_inverse(dx, dy, &ctm_only(pgs),
		                                     pt);
	} else {
		ensure_inverse_valid(pgs);
...
#endif
	gx_setcurrentpoint(pgs, fixed2float(pgs->ctm.tx_fixed),
	                   fixed2float(pgs->ctm.ty_fixed));
...
#undef SET_C
	...
	if(gs_debug_c('x')) {
		dlprintf6("[x]ctm: [%6g %6g %6g %6g %6g %6g]\n", ctm.xx, ctm.xy,
		          ctm.yx, ctm.yy, ctm.tx, ctm.ty);
		dlprintf6(
		    "   scale=%d fc: [0x%lx 0x%lx 0x%lx 0x%lx] shift=%d\n",
		    scale, pfc->xx, pfc->xy, pfc->yx, pfc->yy, pfc->shift);
	}
#endif
 ...
 */
fixed
fixed_coeff_mult(fixed value, int32_t coeff, const fixed_coeff* pfc, int maxb)
{
	int shift = pfc->shift;
	...
	if((value + (fixed_1 << (maxb - 1))) & (-fixed_1 << maxb)) {
		 ...
		 */
		return (fixed)arith_rshift(
		    fixed2int_var(value) * coeff +
		        fixed2int(fixed_fraction(value) * coeff) + pfc->round,
		    shift);
	}
}
