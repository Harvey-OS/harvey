...
#define update_matrix_fixed(mat, xt, yt)                                                                                     \
	((mat).txy_fixed_valid = (f_fits_in_fixed(xt) && f_fits_in_fixed(yt) ? (update_t_fixed(mat, tx, tx_fixed, xt),       \
		 ...
		 int maxb)
{
	...
	if((value + (fixed_1 << (maxb - 1))) & (-fixed_1 << maxb)) {
		...
		return (coeff < 0 ? -fixed_mult_quo(value, -coeff, fixed_1 << shift) : fixed_mult_quo(value, coeff, fixed_1 << shift));
	} else {
		/*
	 ...
	 */
		return (fixed)
		    arith_rshift(fixed2int_var(value) * coeff + fixed2int(fixed_fraction(value) * coeff) + pfc->round, shift);
	}
}
