...
/* Copyright (C) 1991, 1992, 1993, 1997, 1998, 1999, 2000 Aladdin Enterprises.
  All rights reserved.
...
up:
	...
	while(fdepth > 0) {
		...
		down:
			...
			do {
				...
				if_debug3('j', "[j]down from modal: new "
				               "depth=%d, index=%d, new "
				               "font=0x%lx\n",
	...
	while((pfont = pte->fstack.items[fdepth].font)->FontType ==
	      ft_composite) {
		pdata = &pfont0->data;
		switch(pdata->FMapType) {
...
#undef subs_loop
		...
		case fmap_CMap: {
			...
			if(*(p - 1) != chr) {
				...
				int submindex = 0;
				if_debug2('j', "[j] *(p-1) 0x%02x != chr "
				               "0x%02x, modified str should be "
				               "passed\n",
				          *(p - 1), (byte)chr);
				memcpy(substr, p - 1,
				...
				if(gs_debug_c('j')) {
					...
					debug_print_string_hex(cstr.data,
					                       cstr.size);
					dlprintf(")\n");
				}
			...
			p = str + mindex;
			if_debug3(
			    'J', "[J]CMap returns %d, chr=0x%lx, glyph=0x%lx\n",
	...
	*pglyph = glyph;
	...
	if_debug4('J', "[J]depth=%d font=0x%lx index=%d changed=%d\n", fdepth,
	          (uint32_t)pte->fstack.items[fdepth].font,
