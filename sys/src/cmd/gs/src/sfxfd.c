...
void sread_fileno(stream*, FILE*, byte*, uint),
...
private
int s_fileno_available(stream*, int32_t*), s_fileno_read_seek(stream*, int32_t),
    s_fileno_read_close(stream*),
    s_fileno_read_process(stream_state*, stream_cursor_read*,
                          stream_cursor_write*, bool);
private
int s_fileno_write_seek(stream*, int32_t), s_fileno_write_flush(stream*),
    s_fileno_write_close(stream*),
    s_fileno_write_process(stream_state*, stream_cursor_read*,
                           stream_cursor_write*, bool);
private
int s_fileno_switch(stream*, bool);
...
errno_is_retry(int errn)
{
	...
	case EAGAIN:
		return true;
#endif
#if defined(EWOULDBLOCK) && (!defined(EINTR) || EWOULDBLOCK != EINTR) &&       \
    (!defined(EAGAIN) || EWOULDBLOCK != EAGAIN)
	case EWOULDBLOCK:
		return true;
#endif
	default:
