			  ...
			  int min_band_height, int max_src_image_row)
{
	...
	if((code = alloc_render_memory(&render_memory,
				       pwdev->memory->non_gc_memory, RendererAllocationOverheadBytes + max_raster
									 ...
									 +
									 (max_raster + sizeof(void *) * 2) * min_band_height + max_src_image_row + gx_ht_cache_default_bits() * 2)) < 0)
		goto open_err;

	/* Alloc & init bandlist allocators */
	/* Bandlist mem is threadsafe & common to rdr/wtr, so it's used */
	/* for page queue & cmd list buffers. */
	if((code = alloc_bandlist_memory(&pwdev->bandlist_memory, pwdev->memory->non_gc_memory)) < 0)
		goto open_err;
	...
	if(code >= 0) {
		...
		if(!(open_semaphore = gx_semaphore_alloc(prdev->memory)))
			...
			code = (*pwdev->printer_procs.start_render_thread)(&thread_params);
			if(code >= 0)
	...
	gdev_prn_start_render_params *params)
{
	...
	/* Successful open */
	while((entry = gx_page_queue_start_dequeue(prdev->page_queue)) && entry->action != GX_PAGE_QUEUE_ACTION_TERMINATE) {
...
gdev_prn_async_write_get_hardware_params(gx_device *pdev, gs_param_list *plist)
{
	...
	if(!pwdev->is_open || !prdev)
		...
		return (dev_proc(prdev, get_hardware_params))((gx_device *)prdev, plist);
	}
}
...
gdev_prn_async_write_output_page(gx_device *pdev, int num_copies, int flush)
{
	...
	add_code = gx_page_queue_add_page(pwdev->page_queue,
					  (flush ? GX_PAGE_QUEUE_ACTION_FULL_PAGE : GX_PAGE_QUEUE_ACTION_COPY_PAGE),
	...
	while((open_code = (*gs_clist_device_procs.open_device)((gx_device *)pdev)) == gs_error_VMerror) {
		/* Open failed, try after a page gets rendered */
		if(!gx_page_queue_wait_one_page(pwdev->page_queue) && one_last_time-- <= 0)
			break;
	}

	return (flush_code < 0 ? flush_code : open_code < 0 ? open_code : add_code < 0 ? add_code : 0);
}
	...
	)
{
	...
	flush_code = clist_end_page(pcwdev);
	add_code = gx_page_queue_add_page(pwdev->page_queue,
					  (partial ? GX_PAGE_QUEUE_ACTION_PARTIAL_PAGE : GX_PAGE_QUEUE_ACTION_FULL_PAGE),
					  &pcwdev->page_info, 0);
    ...
    )
{
	...
	/* Open new band files to take the place of ones added to page queue */
	while((open_code = (*gs_clist_device_procs.open_device)((gx_device *)pwdev)) == gs_error_VMerror) {
		/* Open failed, try after a page gets rendered */
		if(!gx_page_queue_wait_one_page(pwdev->page_queue) && one_last_time-- <= 0)
			break;
	}
