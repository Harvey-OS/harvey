 ...
 */
#define RendererAllocationOverheadBytes                                        \
	503000 /* minimum is 503,000 as of 4/26/99 */

#ifdef DEBUG
/* 196000 is pretty much the minimum, given 16K phys memfile blocks */
/*# define DebugBandlistMemorySize 196000*/ /* comment out to disable fixed
                                               (debug) bandlist size */
#endif                                      /* defined(DEBUG) */

/* ---------------- Standard device procedures ---------------- */
private
dev_proc_close_device(gdev_prn_async_write_close_device);
private
dev_proc_output_page(gdev_prn_async_write_output_page);
private
dev_proc_put_params(gdev_prn_async_write_put_params);
private
dev_proc_get_hardware_params(gdev_prn_async_write_get_hardware_params);
private
dev_proc_put_params(gdev_prn_async_render_put_params);

/* ---------------- Forward Declarations ---------------------- */
private
void gdev_prn_dealloc(gx_device_printer*);
private
proc_free_up_bandlist_memory(gdev_prn_async_write_free_up_bandlist_memory);
private
int flush_page(gx_device_printer*, bool);
private
int reopen_clist_after_flush(gx_device_printer*);
private
void reinit_printer_into_printera(gx_device_printer* const);
private
int alloc_bandlist_memory(gs_memory_t**, gs_memory_t*);
private
void free_bandlist_memory(gs_memory_t*);
private
int alloc_render_memory(gs_memory_t**, gs_memory_t*, long);
private
void free_render_memory(gs_memory_t*);
private
gs_memory_recover_status_t prna_mem_recover(gs_memory_retrying_t* rmem,
                                            void* proc_data);
                          ...
                          int min_band_height, int max_src_image_row)
{
	...
	if((code = alloc_render_memory(
	        &render_memory, pwdev->memory->non_gc_memory,
	        RendererAllocationOverheadBytes + max_raster
	...
	if((code = alloc_bandlist_memory(&pwdev->bandlist_memory,
	                                 pwdev->memory->non_gc_memory)) < 0)
		goto open_err;
	...
	/* Hack: set this vector to tell gdev_prn_open to allocate for async
	 * rendering */
	pwdev->free_up_bandlist_memory =
	    &gdev_prn_async_write_free_up_bandlist_memory;

	/* prevent clist writer from queuing path graphics & force it to split
	 * images */
	pwdev->clist_disable_mask |=
	    clist_disable_fill_path | clist_disable_stroke_path |
	    clist_disable_complex_clip | clist_disable_nonrect_hl_image |
	...
	if((code = gdev_prn_open(pdev)) >= 0) {
		...
		if((pwdev->page_queue =
		        gx_page_queue_alloc(pwdev->bandlist_memory)) == 0)
			code = gs_note_error(gs_error_VMerror);
		else
			/* Allocate from clist allocator since it is thread-safe
			 */
			code = gx_page_queue_init(pwdev->page_queue,
			                          pwdev->bandlist_memory);
	}
	...
	if(code >= 0) {
		...
		/* Force writer's actual band params into reader's requested
		 * params */
		...
		/* enable renderer to accept changes to params computed by
		 * writer */
		...
		/* Start renderer thread & wait for its successful open of
		 * device */
...
prna_mem_recover(gs_memory_retrying_t* rmem, void* proc_data)
{
	...
	return (pages_remain > 0) ? RECOVER_STATUS_RETRY_OK
    ...
    )
{
	...
	if(dev_proc(pdev, close_device) == gdev_prn_close)
		set_dev_proc(pdev, close_device,
		             gdev_prn_async_write_close_device);
	...
	set_dev_proc(pdev, get_hardware_params,
	             gdev_prn_async_write_get_hardware_params);

	/* clist writer calls this if it runs out of memory & wants to retry */
	pdev->free_up_bandlist_memory =
	    &gdev_prn_async_write_free_up_bandlist_memory;
}
...
/* Deallocte dynamic memory attached to device. Aware of possible imcomplete
 * open */
private
void
...
/* (Re)set renderer device fields which get trampled by gdevprn_open &
 * put_params */
private
void
    ...
    gdev_prn_async_render_thread(gdev_prn_start_render_params* params)
{
	...
	/* The cmd list logic assumes reader's & writer's tile caches are same
	 * size */
	if(code >= 0 &&
		...
		if(!prdev->is_open) {
			if(prdev->printer_procs.open_render_device ==
			   gx_default_open_render_device)
				...
				code =
				    (*prdev->printer_procs.open_render_device)(
				        prdev);
			reinit_printer_into_renderer(prdev);

			if(code >= 0) {
				prdev->is_open = true;
				gdev_prn_output_page((gx_device*)prdev, 0,
				                     true);
			}
		}
		if(prdev->is_open) {
			/* Force retrieved entry onto render device */
			((gx_device_clist*)prdev)->common.page_info =
			    entry->page_info;

			/* Set up device geometry */
			if(clist_setup_params((gx_device*)prdev) >= 0)
				/* Go this again, since setup_params may have
				 * trashed it */
				((gx_device_clist*)prdev)->common.page_info =
				    entry->page_info;

			/* Call appropriate renderer routine to deal w/buffer */
			/* Ignore status, since we don't know how to deal
			 * w/errors! */
			switch(entry->action) {

			case GX_PAGE_QUEUE_ACTION_FULL_PAGE:
				(*dev_proc(prdev, output_page))(
				    (gx_device*)prdev, entry->num_copies, true);
				break;

			case GX_PAGE_QUEUE_ACTION_PARTIAL_PAGE:
			case GX_PAGE_QUEUE_ACTION_COPY_PAGE:
				(*dev_proc(prdev, output_page))(
				    (gx_device*)prdev, entry->num_copies,
				    false);
				break;
			}
			/*
			 * gx_page_queue_finish_dequeue will close and free the
			 * band
			 * list files, so we don't need to call
...
gdev_prn_async_write_put_params(gx_device* pdev, gs_param_list* plist)
{
	...
	pwdev->is_open = true;
	pwdev->OpenOutputFile =
	    0; /* This is wrong, because it causes the same thing in the
	          renderer */

	/* Flush device or emit to command list, depending if device changed
	 * geometry */
	if(memcmp(&pwdev->space_params, &save_sp, sizeof(save_sp)) != 0 ||
	   pwdev->width != save_width || pwdev->height != save_height) {
		...
		do {
			ecode = gdev_prn_reallocate_memory(
			    pdev, &new_sp, new_width, new_height);
			if(ecode >= 0)
				break; /* managed to recover enough memory */
			if(!pdev->is_open) {
				/* Disaster! Device was forced closed, which
				 * async drivers */
				/* aren't suppsed to do. */
				gdev_prn_async_write_close_device(pdev);
				return ecode; /* caller 'spozed to know could be
				                 closed now */
			}
			pclwdev->error_is_retryable =
	...
	/* Reset fields that got trashed by gdev_prn_put_params and/or
	 * gdev_prn_open */
	reinit_printer_into_printera(pwdev);
...
gdev_prn_async_write_get_hardware_params(gx_device* pdev, gs_param_list* plist)
{
	...
	if(!pwdev->is_open || !prdev)
		 ...
		 */
		return (dev_proc(prdev, get_hardware_params))((gx_device*)prdev,
		                                              plist);
	}
}
...
private
int /* returns -ve err code only if FATAL error (can't keep rendering) */
	...
	if(!prdev->is_open && save_is_open) {
		...
		if(prdev->printer_procs.open_render_device ==
		   gx_default_open_render_device)
			...
			code =
			    (*prdev->printer_procs.open_render_device)(prdev);
		reinit_printer_into_renderer(prdev);
		if(code >= 0)
			/****** CLEAR PAGE SOMEHOW ******/;
		else
			return code; /* this'll cause clist to stop processing
			                this band! */
	}
...
gdev_prn_async_write_output_page(gx_device* pdev, int num_copies, int flush)
{
	...
	add_code = gx_page_queue_add_page(
	    pwdev->page_queue, (flush ? GX_PAGE_QUEUE_ACTION_FULL_PAGE
	...
	return (flush_code < 0
	            ? flush_code
	            : open_code < 0 ? open_code : add_code < 0 ? add_code : 0);
}

/* Free bandlist memory waits until the rasterizer runs enough to free some mem
 */
private
int /* -ve err,  0 if no pages remain to rasterize, 1 if more pages to go */
    gdev_prn_async_write_free_up_bandlist_memory(gx_device* pdev,
                                                 bool flush_current)
{
	gx_device_printer* const pwdev = (gx_device_printer*)pdev;
...
private
int                                      /* ret 0 ok no flush, -ve error code */
    flush_page(gx_device_printer* pwdev, /* async writer device to flush */
               ...
               )
{
	...
	flush_code = clist_end_page(pcwdev);
	add_code = gx_page_queue_add_page(
	    pwdev->page_queue, (partial ? GX_PAGE_QUEUE_ACTION_PARTIAL_PAGE
...
#endif
	locked_allocator = (gs_memory_locked_t*)gs_alloc_bytes_immovable(
	    data_allocator, sizeof(gs_memory_locked_t),
...
{
	gs_memory_locked_t* const lmem =
	    (gs_memory_locked_t*)bandlist_allocator;
...
private
int
alloc_render_memory(gs_memory_t** final_allocator, gs_memory_t* base_allocator,
                    long space)
{
