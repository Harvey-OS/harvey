...
/* Copyright (C) 1992, 1993, 1994, 1997, 1999 Aladdin Enterprises.  All rights
  reserved.
...
private
unsigned int clut2mgr(int, int);
private
void swap_bwords(unsigned char*, int);
...
#define mgr_prn_device(procs, dev_name, num_comp, depth, mgr_depth, max_gray,  \
                       max_rgb, dither_gray, dither_rgb, print_page)           \
	{                                                                      \
		prn_device_body(gx_device_mgr, procs, dev_name,                \
		                DEFAULT_WIDTH_10THS, DEFAULT_HEIGHT_10THS,     \
		                X_DPI, Y_DPI, 0, 0, 0, 0, num_comp, depth,     \
		                max_gray, max_rgb, dither_gray, dither_rgb,    \
		                print_page),                                   \
		    mgr_depth                                                  \
	}
...
private
dev_proc_print_page(mgr_print_page);
private
dev_proc_print_page(mgrN_print_page);
private
dev_proc_print_page(cmgrN_print_page);

/* The device procedures */
private
gx_device_procs mgr_procs =
    prn_procs(gdev_mgr_open, gdev_prn_output_page, gdev_prn_close);
private
gx_device_procs mgrN_procs = prn_color_procs(
    gdev_mgr_open, gdev_prn_output_page, gdev_prn_close,
    gx_default_gray_map_rgb_color, gx_default_gray_map_color_rgb);
private
gx_device_procs cmgr4_procs =
    prn_color_procs(gdev_mgr_open, gdev_prn_output_page, gdev_prn_close,
                    pc_4bit_map_rgb_color, pc_4bit_map_color_rgb);
private
gx_device_procs cmgr8_procs =
    prn_color_procs(gdev_mgr_open, gdev_prn_output_page, gdev_prn_close,
...
gx_device_mgr far_data gs_mgrgray2_device = mgr_prn_device(
    mgrN_procs, "mgrgray2", 1, 8, 2, 255, 0, 4, 0, mgrN_print_page);
gx_device_mgr far_data gs_mgrgray4_device = mgr_prn_device(
    mgrN_procs, "mgrgray4", 1, 8, 4, 255, 0, 16, 0, mgrN_print_page);
gx_device_mgr far_data gs_mgrgray8_device = mgr_prn_device(
    mgrN_procs, "mgrgray8", 1, 8, 8, 255, 0, 0, 0, mgrN_print_page);
gx_device_mgr far_data gs_mgr4_device =
    mgr_prn_device(cmgr4_procs, "mgr4", 3, 8, 4, 1, 1, 2, 2, cmgrN_print_page);
gx_device_mgr far_data gs_mgr8_device = mgr_prn_device(
    cmgr8_procs, "mgr8", 3, 8, 8, 255, 255, 6, 5, cmgrN_print_page);
...
mgr_begin_page(gx_device_mgr* bdev, FILE* pstream, mgr_cursor* pcur)
{
	...
	uint line_size = gdev_prn_raster((gx_device_printer*)bdev) + 3;
	byte* data =
	    (byte*)gs_malloc(bdev->memory, line_size, 1, "mgr_begin_page");
	if(data == 0)
...
mgr_next_row(mgr_cursor* pcur)
{
	...
	gdev_prn_copy_scan_lines((gx_device_printer*)pcur->dev, pcur->lnum++,
	                         pcur->data, pcur->line_size);
	return 0;
}
...
mgrN_print_page(gx_device_printer* pdev, FILE* pstream)
{
	...
	if(bdev->mgr_depth != 8)
		data = (byte*)gs_malloc(pdev->memory, mgr_line_size, 1,
		                        "mgrN_print_page");

	while(!(code = mgr_next_row(&cur))) {
		switch(bdev->mgr_depth) {
		case 2:
			for(i = 0, dp = data, bp = cur.data; i < mgr_line_size;
			    i++) {
				*dp = *(bp++) & 0xc0;
				*dp |= (*(bp++) & 0xc0) >> 2;
				*dp |= (*(bp++) & 0xc0) >> 4;
			...
			if(fwrite(data, sizeof(byte), mgr_line_size, pstream) <
			   mgr_line_size)
				return_error(gs_error_ioerror);
			break;
		...
		case 8:
			...
			if(fwrite(cur.data, sizeof(cur.data[0]), mgr_line_size,
			          pstream) < mgr_line_size)
				return_error(gs_error_ioerror);
			break;
		}
	}
	...
	if(bdev->mgr_depth == 2) {
		for(i = 0; i < 4; i++) {
			clut[i].colnum = i;
			clut[i].red = clut[i].green = clut[i].blue =
			    clut2mgr(i, 2);
		}
	}
	if(bdev->mgr_depth == 4) {
...
cmgrN_print_page(gx_device_printer* pdev, FILE* pstream)
{
	...
	if(bdev->mgr_depth == 4) {
		for(i = 0; i < 16; i++) {
			pc_4bit_map_color_rgb((gx_device*)0, (gx_color_index)i,
			                      prgb);
			clut[i].colnum = i;
	...
	if(bdev->mgr_depth == 8) {
		for(i = 0; i < colors8; i++) {
			mgr_8bit_map_color_rgb((gx_device*)0,
			                       (gx_color_index)table[i], prgb);
...
mgr_8bit_map_rgb_color(gx_device* dev, const gx_color_value cv[])
{
	...
	return (gx_color_index)(rv == gv && gv == bv
	                            ? rv + (256 - 7)
	                            : (rv << 5) + (gv << 2) + (bv >> 1));
}
int
...
private
unsigned int
clut2mgr(register int v,   /* value in clut */
