...
#define mgr_prn_device(procs, dev_name, num_comp, depth, mgr_depth, max_gray, max_rgb, dither_gray, dither_rgb, print_page)                                                                                              \
	{                                                                                                                                                                                                                \
		prn_device_body(gx_device_mgr, procs, dev_name, DEFAULT_WIDTH_10THS, DEFAULT_HEIGHT_10THS, X_DPI, Y_DPI, 0, 0, 0, 0, num_comp, depth, max_gray, max_rgb, dither_gray, dither_rgb, print_page), mgr_depth \
	}
...
mgr_8bit_map_rgb_color(gx_device *dev, const gx_color_value cv[])
{
	...
	return (gx_color_index)(rv == gv && gv == bv ? rv + (256 - 7) : (rv << 5) + (gv << 2) + (bv >> 1));
}
int
