...
enumerate_font_next(psf_glyph_enum_t* ppge, gs_glyph* pglyph)
{
	...
	int code = font->procs.enumerate_glyph(font, &index, ppge->glyph_space,
	                                       pglyph);
                         ...
                         gs_glyph_space_t glyph_space)
{
	...
	ppge->enumerate_next =
	    (subset_list ? enumerate_glyphs_next : subset_size
	                                               ? enumerate_range_next
	                                               : enumerate_font_next);
...
enumerate_bits_next(psf_glyph_enum_t* ppge, gs_glyph* pglyph)
{
	for(; ppge->index < ppge->subset.size; ppge->index++)
		if(ppge->subset.selected.bits[ppge->index >> 3] &
		   (0x80 >> (ppge->index & 7))) {
			*pglyph = (gs_glyph)(ppge->index++ + gs_min_cid_glyph);
			return 0;
		}
                         ...
                         gs_glyph_space_t glyph_space)
{
	...
	ppge->glyph_space = glyph_space;
	ppge->enumerate_next =
	    (subset_bits ? enumerate_bits_next : subset_size
	                                             ? enumerate_range_next
	                                             : enumerate_font_next);
	psf_enumerate_glyphs_reset(ppge);
}
                      ...
                      uint max_pieces, gs_font* font)
{
	...
	for(i = 0; i < count; ++i) {
		...
		code = font->procs.glyph_info(
		    font, glyphs[i], NULL,
		    GLYPH_INFO_NUM_PIECES | GLYPH_INFO_PIECES, &info);
		if(code >= 0)
                       ...
                       glyph_data_proc_t glyph_data)
{
	...
	{
		/*
		 * Make sure that this font can be written out.  Specifically,
		 * it
	...
	if(subset_glyphs) {
		 ...
		 * 2 pieces.
		 */
		int code = psf_add_subset_pieces(
		    subset_glyphs, &subset_size,
		    countof(pglyphs->subset_data) - 1, 2, (gs_font*)pfont);
		uint keep_size, i;
