...
ENUM_PTRS_END
private
RELOC_PTRS_WITH(cie_render1_reloc_ptrs, gs_cie_render* pcrd);
...
EncodeLMN_cached_L(floatp in, const gs_cie_render* pcrd)
{
	return gs_cie_cached_value(in,
	                           &pcrd->caches.EncodeLMN.caches[0].floats);
}
private
float
...
{
	return gs_cie_cached_value(in,
	                           &pcrd->caches.EncodeLMN.caches[1].floats);
}
private
float
...
{
	return gs_cie_cached_value(in,
	                           &pcrd->caches.EncodeLMN.caches[2].floats);
}
...
{
	return pcrd->caches.RenderTableT[i]
	    .fracs.values[in * (gx_cie_cache_size - 1) / 255];
}
private
frac
...
tpqr_do_lookup(gs_cie_render* pcrd, const gx_device* dev_proto)
{
	...
	code =
	    param_request((gs_param_list*)&list, pcrd->TransformPQR.proc_name);
	if(code >= 0) {
		...
		if(code >= 0) {
			...
			code = param_read_string((gs_param_list*)&list,
			                         ...
			                         &proc_addr);
			if(code == 0 &&
			   proc_addr.size == sizeof(gs_cie_transform_proc)) {
...
private
int
tpqr_lookup(int index, floatp in, const gs_cie_wbsd* pwbsd, gs_cie_render* pcrd,
            float* out)
{
...
gs_cie_render1_init_from(
    const gs_memory_t* mem, gs_cie_render* pcrd, void* client_data,
    const gs_cie_render* pfrom_crd, const gs_vector3* WhitePoint,
    const gs_vector3* BlackPoint, const gs_matrix3* MatrixPQR,
    const gs_range3* RangePQR, const gs_cie_transform_proc3* TransformPQR,
    const gs_matrix3* MatrixLMN, const gs_cie_render_proc3* EncodeLMN,
    const gs_range3* RangeLMN, const gs_matrix3* MatrixABC,
    const gs_cie_render_proc3* EncodeABC, const gs_range3* RangeABC,
	...
	if(RenderTable) {
		...
		if(pfrom_crd &&
		   !memcmp(&pcrd->RenderTable.T, &RenderTableT_from_cache,
			...
			memcpy(pcrd->caches.RenderTableT,
			       pfrom_crd->caches.RenderTableT,
...
gs_cie_render1_initialize(
    ...
    const gs_cie_transform_proc3* TransformPQR, const gs_matrix3* MatrixLMN,
    const gs_cie_render_proc3* EncodeLMN, const gs_range3* RangeLMN,
    const gs_matrix3* MatrixABC, const gs_cie_render_proc3* EncodeABC,
    const gs_range3* RangeABC, const gs_cie_render_table_t* RenderTable)
{
	return gs_cie_render1_init_from(
	    mem, pcrd, client_data, NULL, WhitePoint, BlackPoint, MatrixPQR,
	    RangePQR, TransformPQR, MatrixLMN, EncodeLMN, RangeLMN, MatrixABC,
	    EncodeABC, RangeABC, RenderTable);
}
