...
case 2:
return (pcrd->RenderTable.lookup.table ? ENUM_CONST_STRING(&pcrd->TransformPQR.proc_data) : 0);
ENUM_PTRS_END
private
...
{
	return pcrd->caches.RenderTableT[i].fracs.values[in * (gx_cie_cache_size - 1) / 255];
}
private
frac
