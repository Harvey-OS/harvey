...
#define make_file(pref, a, id, s) make_tasv(pref, t_file, a, id, pfile, s)
...
#define check_read_known_file(svar, op, error_return)                          \
	check_read_known_file_else(svar, op, error_return,                     \
	                           svar = invalid_file_entry)
...
/* for imainarg.c */
FILE* lib_fopen(const gs_file_path_ptr pfpath, const gs_memory_t* mem,
                const char*);

/* for imain.c */
int lib_file_open(const gs_file_path_ptr pfpath, i_ctx_t*, const char*, uint,
                  byte*, uint, uint*, ref*, gs_memory_t*);
...
int filter_open(const char*, uint, ref*, const stream_procs*,
                const stream_template*, const stream_state*, gs_memory_t*);
...
/* for ziodev.c */
int zreadline_from(stream* s, gs_string* buf, gs_memory_t* bufmem, uint* pcount,
                   bool* pin_eol);
