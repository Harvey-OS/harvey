...
#define set_pixel_scale(pps, log2)                                             \
	(pps)->unit = ((pps)->half = fixed_half << ((pps)->log2_unit = log2))  \
	              << 1
#define scaled_rounded(v, pps) (((v) + (pps)->half) & -(pps)->unit)
...
#define charstring_next(ch, state, chvar, encrypted)                           \
	(encrypted                                                             \
	     ? (chvar = decrypt_this(ch, state), decrypt_skip_next(ch, state)) \
	     : (chvar = ch))
...
struct gs_type1_state_s {
	...
	gs_log2_scale_point
	    log2_subpixels;    /* log2 of the number of subpixels */
...
#define public_st_gs_type1_state() /* in gxtype1.c */                          \
	gs_public_st_composite(st_gs_type1_state, gs_type1_state,              \
	                       "gs_type1_state", gs_type1_state_enum_ptrs,     \
	                       gs_type1_state_reloc_ptrs)
...
#define CLEAR_CSTACK(cstack, csp) (csp = (cs_ptr)(cstack)-1)
...
#define decode_num1(var, c) (var = c_value_num1(c))
...
#define sign_extend_num4(lw) lw = (lw ^ 0x80000000L) - 0x80000000L
#else
...
int gs_type1_sbw(gs_type1_state* pcis, fixed sbx, fixed sby, fixed wx,
                 fixed wy);

/* blend returns the number of values to pop. */
int gs_type1_blend(gs_type1_state* pcis, fixed* csp, int num_results);

int gs_type1_seac(gs_type1_state* pcis, const fixed* cstack, fixed asb_diff,
                  ip_state_t* ipsp);
