...
gx_device_forward_fill_in_procs(register gx_device_forward* dev)
{
	...
	fill_dev_proc(dev, get_color_mapping_procs,
	              gx_forward_get_color_mapping_procs);
	fill_dev_proc(dev, get_color_comp_index,
	...
	fill_dev_proc(dev, fill_rectangle_hl_color,
	              gx_forward_fill_rectangle_hl_color);
	fill_dev_proc(dev, include_color_space, gx_forward_include_color_space);
	fill_dev_proc(dev, fill_linear_color_scanline,
	              gx_forward_fill_linear_color_scanline);
	fill_dev_proc(dev, fill_linear_color_trapezoid,
	              gx_forward_fill_linear_color_trapezoid);
	fill_dev_proc(dev, fill_linear_color_triangle,
	              gx_forward_fill_linear_color_triangle);
	fill_dev_proc(dev, update_spot_equivalent_colors,
...
int
gx_forward_tile_rectangle(gx_device* dev, const gx_tile_bitmap* tile, int x,
...
int
gx_forward_copy_mono(gx_device* dev, const byte* data, int dx, int raster,
                     ...
                     gx_color_index zero, gx_color_index one)
{
	...
	if(tdev == 0)
		return_error(gs_error_Fatal);
	return dev_proc(tdev, copy_mono)(tdev, data, dx, raster, id, x, y, w, h,
	                                 zero, one);
}

int
gx_forward_copy_alpha(gx_device* dev, const byte* data, int data_x, int raster,
                      ...
                      gx_color_index color, int depth)
{
	...
	if(tdev == 0)
		return_error(gs_error_Fatal);
	return dev_proc(tdev, copy_mono)(tdev, data, data_x, raster, id, x, y,
	                                 width, height, color, depth);
}

int
gx_forward_copy_color(gx_device* dev, const byte* data, int dx, int raster,
                      gx_bitmap_id id, int x, int y, int w, int h)
{
	gx_device_forward* const fdev = (gx_device_forward*)dev;
	gx_device* tdev = fdev->target;
	...
	if(tdev == 0)
		return_error(gs_error_Fatal);
	return dev_proc(tdev, copy_color)(tdev, data, dx, raster, id, x, y, w,
	                                  h);
}
...
gx_forward_get_bits(gx_device* dev, int y, byte* data, byte** actual_data)
{
	...
	return (tdev == 0
	            ? gx_default_get_bits(dev, y, data, actual_data)
                               ...
                               gx_color_value alpha)
{
	...
	return (tdev == 0 ? gx_default_map_rgb_alpha_color(dev, r, g, b, alpha)
	                  : dev_proc(tdev, map_rgb_alpha_color)(tdev, r, g, b,
	                                                        alpha));
}
...
gx_forward_get_band(gx_device* dev, int y, int* band_start)
{
	...
	return (tdev == 0 ? gx_default_get_band(dev, y, band_start)
	                  : dev_proc(tdev, get_band)(tdev, y, band_start));
}

int
gx_forward_copy_rop(gx_device* dev, const byte* sdata, int sourcex,
                    ...
                    gs_logical_operation_t lop)
{
	...
	return proc(tdev, sdata, sourcex, sraster, id, scolors, texture,
	            tcolors, x, y, width, height, phase_x, phase_y, lop);
}

int
gx_forward_fill_path(gx_device* dev, const gs_imager_state* pis, gx_path* ppath,
...
int
gx_forward_fill_mask(gx_device* dev, const byte* data, int dx, int raster,
                     gx_bitmap_id id, int x, int y, int w, int h,
                     ...
                     gs_logical_operation_t lop, const gx_clip_path* pcpath)
{
	...
	return proc(tdev, data, dx, raster, id, x, y, w, h, pdcolor, depth, lop,
	            pcpath);
}

int
gx_forward_fill_trapezoid(gx_device* dev, const gs_fixed_edge* left,
...
int
gx_forward_fill_parallelogram(gx_device* dev, fixed px, fixed py, fixed ax,
                              fixed ay, fixed bx, fixed by,
                              const gx_drawing_color* pdcolor,
                              gs_logical_operation_t lop)
{
	gx_device_forward* const fdev = (gx_device_forward*)dev;
...
int
gx_forward_fill_triangle(gx_device* dev, fixed px, fixed py, fixed ax, fixed ay,
...
int
gx_forward_draw_thin_line(gx_device* dev, fixed fx0, fixed fy0, fixed fx1,
                          fixed fy1, const gx_drawing_color* pdcolor,
                          gs_logical_operation_t lop)
{
	gx_device_forward* const fdev = (gx_device_forward*)dev;
	gx_device* tdev = fdev->target;
...
int
gx_forward_begin_image(gx_device* dev, const gs_imager_state* pis,
                       ...
                       gx_image_enum_common_t** pinfo)
{
	...
	return proc(tdev, pis, pim, format, prect, pdcolor, pcpath, memory,
	            pinfo);
}
                          ...
                          gs_logical_operation_t lop)
{
	...
	return proc(tdev, sdata, sourcex, sraster, id, scolors, textures,
	            tcolors, x, y, width, height, phase_x, phase_y, lop);
}
                             ...
                             gx_image_enum_common_t** pinfo)
{
	...
	return proc(tdev, pis, pmat, pim, prect, pdcolor, pcpath, memory,
	            pinfo);
}
                               ...
                               gx_color_value prgba[4])
{
	...
	return (tdev == 0
	            ? gx_default_map_color_rgb_alpha(dev, color, prgba)
                      ...
                      gs_text_enum_t** ppenum)
{
	...
	return proc(tdev, pis, text, font, path, pdcolor, pcpath, memory,
	            ppenum);
}
 ...
 */
static void
fwd_map_rgb_cs(gx_device* dev, const gs_imager_state* pis, frac r, frac g,
               frac b, frac out[])
{
	gx_device_forward* const fdev = (gx_device_forward*)dev;
	gx_device* const tdev = fdev->target;
	...
	if(tdev == 0 || dev_proc(tdev, get_color_mapping_procs) == 0 ||
	   (pprocs = dev_proc(tdev, get_color_mapping_procs(tdev))) == 0 ||
	   pprocs->map_rgb == 0)
		rgb_cs_to_rgb_cm(tdev, pis, r, g, b,
...
fwd_map_cmyk_cs(gx_device* dev, frac c, frac m, frac y, frac k, frac out[])
{
	...
	if(tdev == 0 || dev_proc(tdev, get_color_mapping_procs) == 0 ||
	   (pprocs = dev_proc(tdev, get_color_mapping_procs(tdev))) == 0 ||
	   pprocs->map_cmyk == 0)
		cmyk_cs_to_cmyk_cm(tdev, c, m, y, k,
		                   out); /* if all else fails */
	else
                                ...
                                int name_size, int component_type)
{
	...
	return (tdev == 0 ? gx_error_get_color_comp_index(dev, pname, name_size,
	                                                  component_type)
...
int
gx_forward_decode_color(gx_device* dev, gx_color_index cindex,
                        gx_color_value colors[])
{
	gx_device_forward* const fdev = (gx_device_forward*)dev;
	gx_device* tdev = fdev->target;
	...
	if(tdev == 0) /* If no device - just clear the color values */
		memset(colors, 0,
		       sizeof(gx_color_value[GX_DEVICE_COLOR_MAX_COMPONENTS]));
	else
                          ...
                          pattern_manage_t function)
{
	...
	if(tdev == 0)
		...
		return dev_proc(tdev, pattern_manage)(tdev, id, pinst,
		                                      function);
}

int
gx_forward_fill_rectangle_hl_color(gx_device* dev, const gs_fixed_rect* rect,
                                   ...
                                   const gx_clip_path* pcpath)
{
	...
	if(tdev == 0)
		...
		return dev_proc(tdev, fill_rectangle_hl_color)(tdev, rect, pis,
		                                               pdcolor, NULL);
}
                               ...
                               const byte* res_name, int name_length)
{
	...
	if(tdev == 0)
		...
		return dev_proc(tdev, include_color_space)(
		    tdev, cspace, res_name, name_length);
}

int
gx_forward_fill_linear_color_scanline(gx_device* dev,
                                      const gs_fill_attributes* fa, int i,
                                      int j, int w, const frac31* c,
                                      const int32_t* addx, const int32_t* mulx,
                                      int32_t divx)
{
	gx_device_forward* const fdev = (gx_device_forward*)dev;
	gx_device* tdev = fdev->target;
...
int
gx_forward_fill_linear_color_trapezoid(
    gx_device* dev, const gs_fill_attributes* fa, const gs_fixed_point* p0,
    const gs_fixed_point* p1, const gs_fixed_point* p2,
    const gs_fixed_point* p3, const frac31* c0, const frac31* c1,
...
gx_forward_fill_linear_color_triangle(
    gx_device* dev, const gs_fill_attributes* fa, const gs_fixed_point* p0,
    const gs_fixed_point* p1, const gs_fixed_point* p2, const frac31* c0,
    const frac31* c1, const frac31* c2)
{
	gx_device_forward* const fdev = (gx_device_forward*)dev;
	gx_device* tdev = fdev->target;
...
private
dev_proc_get_initial_matrix(gx_forward_upright_get_initial_matrix);
private
dev_proc_fill_rectangle(null_fill_rectangle);
private
dev_proc_copy_mono(null_copy_mono);
private
dev_proc_copy_color(null_copy_color);
private
dev_proc_put_params(null_put_params);
private
dev_proc_copy_alpha(null_copy_alpha);
private
dev_proc_copy_rop(null_copy_rop);
private
dev_proc_fill_path(null_fill_path);
private
dev_proc_stroke_path(null_stroke_path);
private
dev_proc_fill_trapezoid(null_fill_trapezoid);
private
dev_proc_fill_parallelogram(null_fill_parallelogram);
private
dev_proc_fill_triangle(null_fill_triangle);
private
dev_proc_draw_thin_line(null_draw_thin_line);
private
dev_proc_decode_color(null_decode_color);
...
private
dev_proc_strip_copy_rop(null_strip_copy_rop);

#define null_procs(get_initial_matrix, get_page_device)                                    \
	{                                                                                  \
		gx_default_open_device, get_initial_matrix, /* differs */                  \
		    gx_default_sync_output, gx_default_output_page,                        \
		    gx_default_close_device, gx_forward_map_rgb_color,                     \
		    gx_forward_map_color_rgb, null_fill_rectangle,                         \
		    gx_default_tile_rectangle, null_copy_mono,                             \
		    null_copy_color, gx_default_draw_line,                                 \
		    gx_default_get_bits, gx_forward_get_params,                            \
		    null_put_params, gx_forward_map_cmyk_color,                            \
		    gx_forward_get_xfont_procs, gx_forward_get_xfont_device,               \
		    gx_forward_map_rgb_alpha_color,                                        \
		    get_page_device, /* differs */                                         \
		    gx_default_get_alpha_bits, null_copy_alpha,                            \
		    gx_forward_get_band, null_copy_rop, null_fill_path,                    \
		    null_stroke_path, gx_default_fill_mask,                                \
		    null_fill_trapezoid, null_fill_parallelogram,                          \
		    null_fill_triangle, null_draw_thin_line,                               \
		    gx_default_begin_image, gx_default_image_data,                         \
		    gx_default_end_image, gx_default_strip_tile_rectangle,                 \
		    ...
		    gx_forward_get_hardware_params, gx_default_text_begin,                 \
...
const gx_device_null gs_null_device = {
    std_device_std_body_type_open(gx_device_null, 0, "null", &st_device_null, 0,
                                  0, 72, 72),
...
const gx_device_null gs_nullpage_device = {
    std_device_std_body_type_open(gx_device_null, 0, "nullpage",
                                  &st_device_null, 72 /*nominal */,
                                  72 /*nominal */, 72, 72),
...
private
int
null_decode_color(gx_device* dev, gx_color_index cindex,
                  gx_color_value colors[])
{
...
private
int
null_copy_color(gx_device* dev, const byte* data, int data_x, int raster,
                gx_bitmap_id id, int x, int y, int width, int height)
{
	return 0;
}
...
private
int
null_copy_rop(gx_device* dev, const byte* sdata, int sourcex, uint sraster,
              gx_bitmap_id id, const gx_color_index* scolors,
              const gx_tile_bitmap* texture, const gx_color_index* tcolors,
              int x, int y, int width, int height, int phase_x, int phase_y,
              gs_logical_operation_t lop)
{
	return 0;
}
private
int
null_fill_path(gx_device* dev, const gs_imager_state* pis, gx_path* ppath,
...
private
int
null_stroke_path(gx_device* dev, const gs_imager_state* pis, gx_path* ppath,
...
private
int
null_fill_trapezoid(gx_device* dev, const gs_fixed_edge* left,
                    const gs_fixed_edge* right, fixed ybot, fixed ytop,
                    bool swap_axes, const gx_drawing_color* pdcolor,
                    gs_logical_operation_t lop)
{
...
private
int
null_fill_parallelogram(gx_device* dev, fixed px, fixed py, fixed ax, fixed ay,
                        fixed bx, fixed by, const gx_drawing_color* pdcolor,
                        gs_logical_operation_t lop)
{
	return 0;
...
private
int
null_fill_triangle(gx_device* dev, fixed px, fixed py, fixed ax, fixed ay,
                   fixed bx, fixed by, const gx_drawing_color* pdcolor,
                   gs_logical_operation_t lop)
{
	return 0;
...
private
int
null_draw_thin_line(gx_device* dev, fixed fx0, fixed fy0, fixed fx1, fixed fy1,
