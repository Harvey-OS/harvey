...
gx_forward_sync_output(gx_device *dev)
{
	...
	return (tdev == 0 ? gx_default_sync_output(dev) : dev_proc(tdev, sync_output)(tdev));
}
...
gx_forward_output_page(gx_device *dev, int num_copies, int flush)
{
	...
	gx_device *tdev = fdev->target;
	int code =
	    (tdev == 0 ? gx_default_output_page(dev, num_copies, flush) : dev_proc(tdev, output_page)(tdev, num_copies, flush));
...
gx_forward_map_rgb_color(gx_device *dev, const gx_color_value cv[])
{
	...
	return (tdev == 0 ? gx_error_encode_color(dev, cv) : dev_proc(tdev, map_rgb_color)(tdev, cv));
}
			 ...
			 gx_color_value prgb[3])
{
	...
	return (tdev == 0 ? gx_default_map_color_rgb(dev, color, prgb) : dev_proc(tdev, map_color_rgb)(tdev, color, prgb));
}
			  ...
			  gx_color_index color1, int px, int py)
{
	...
	dev_proc_tile_rectangle((*proc)) =
	    (tdev == 0 ? (tdev = dev, gx_default_tile_rectangle) : dev_proc(tdev, tile_rectangle));
		     ...
		     gx_color_index zero, gx_color_index one)
{
	...
	if(tdev == 0)
		return_error(gs_error_Fatal);
	return dev_proc(tdev, copy_mono)(tdev, data, dx, raster, id, x, y, w, h, zero, one);
}
		      ...
		      gx_color_index color, int depth)
{
	...
	if(tdev == 0)
		return_error(gs_error_Fatal);
	return dev_proc(tdev, copy_mono)(tdev, data, data_x, raster, id, x, y, width, height, color, depth);
}
		      ...
		      int x, int y, int w, int h)
{
	...
	if(tdev == 0)
		return_error(gs_error_Fatal);
	return dev_proc(tdev, copy_color)(tdev, data, dx, raster, id, x, y, w, h);
}
...
gx_forward_get_bits(gx_device *dev, int y, byte *data, byte **actual_data)
{
	...
	return (tdev == 0 ? gx_default_get_bits(dev, y, data, actual_data) : dev_proc(tdev, get_bits)(tdev, y, data, actual_data));
}
...
gx_forward_get_params(gx_device *dev, gs_param_list *plist)
{
	...
	return (tdev == 0 ? gx_default_get_params(dev, plist) : dev_proc(tdev, get_params)(tdev, plist));
}
...
gx_forward_map_cmyk_color(gx_device *dev, const gx_color_value cv[])
{
	...
	return (tdev == 0 ? gx_default_map_cmyk_color(dev, cv) : dev_proc(tdev, map_cmyk_color)(tdev, cv));
}
...
gx_forward_get_xfont_procs(gx_device *dev)
{
	...
	return (tdev == 0 ? gx_default_get_xfont_procs(dev) : dev_proc(tdev, get_xfont_procs)(tdev));
}
...
gx_forward_get_xfont_device(gx_device *dev)
{
	...
	return (tdev == 0 ? gx_default_get_xfont_device(dev) : dev_proc(tdev, get_xfont_device)(tdev));
}
			       ...
			       gx_color_value alpha)
{
	...
	return (tdev == 0 ? gx_default_map_rgb_alpha_color(dev, r, g, b, alpha) : dev_proc(tdev, map_rgb_alpha_color)(tdev, r, g, b, alpha));
}
...
gx_forward_get_band(gx_device *dev, int y, int *band_start)
{
	...
	return (tdev == 0 ? gx_default_get_band(dev, y, band_start) : dev_proc(tdev, get_band)(tdev, y, band_start));
}
		    ...
		    int phase_x, int phase_y, gs_logical_operation_t lop)
{
	...
	dev_proc_copy_rop((*proc)) =
	    (tdev == 0 ? (tdev = dev, gx_default_copy_rop) : dev_proc(tdev, copy_rop));
		     ...
		     const gx_clip_path *pcpath)
{
	...
	dev_proc_fill_path((*proc)) =
	    (tdev == 0 ? (tdev = dev, gx_default_fill_path) : dev_proc(tdev, fill_path));
		       ...
		       const gx_clip_path *pcpath)
{
	...
	dev_proc_stroke_path((*proc)) =
	    (tdev == 0 ? (tdev = dev, gx_default_stroke_path) : dev_proc(tdev, stroke_path));
		     ...
		     gs_logical_operation_t lop, const gx_clip_path *pcpath)
{
	...
	dev_proc_fill_mask((*proc)) =
	    (tdev == 0 ? (tdev = dev, gx_default_fill_mask) : dev_proc(tdev, fill_mask));
			  ...
			  gs_logical_operation_t lop)
{
	...
	dev_proc_fill_trapezoid((*proc)) =
	    (tdev == 0 ? (tdev = dev, gx_default_fill_trapezoid) : dev_proc(tdev, fill_trapezoid));
			      ...
			      const gx_drawing_color *pdcolor, gs_logical_operation_t lop)
{
	...
	dev_proc_fill_parallelogram((*proc)) =
	    (tdev == 0 ? (tdev = dev, gx_default_fill_parallelogram) : dev_proc(tdev, fill_parallelogram));
			 ...
			 const gx_drawing_color *pdcolor, gs_logical_operation_t lop)
{
	...
	dev_proc_fill_triangle((*proc)) =
	    (tdev == 0 ? (tdev = dev, gx_default_fill_triangle) : dev_proc(tdev, fill_triangle));
			  ...
			  const gx_drawing_color *pdcolor, gs_logical_operation_t lop)
{
	...
	dev_proc_draw_thin_line((*proc)) =
	    (tdev == 0 ? (tdev = dev, gx_default_draw_thin_line) : dev_proc(tdev, draw_thin_line));
		       ...
		       gs_memory_t *memory, gx_image_enum_common_t **pinfo)
{
	...
	dev_proc_begin_image((*proc)) =
	    (tdev == 0 ? (tdev = dev, gx_default_begin_image) : dev_proc(tdev, begin_image));
				...
				int px, int py)
{
	...
	dev_proc_strip_tile_rectangle((*proc)) =
	    (tdev == 0 ? (tdev = dev, gx_default_strip_tile_rectangle) : dev_proc(tdev, strip_tile_rectangle));
			  ...
			  int phase_x, int phase_y, gs_logical_operation_t lop)
{
	...
	dev_proc_strip_copy_rop((*proc)) =
	    (tdev == 0 ? (tdev = dev, gx_default_strip_copy_rop) : dev_proc(tdev, strip_copy_rop));
			     ...
			     gx_image_enum_common_t **pinfo)
{
	...
	dev_proc_begin_typed_image((*proc)) =
	    (tdev == 0 ? (tdev = dev, gx_default_begin_typed_image) : dev_proc(tdev, begin_typed_image));
			      ...
			      gs_get_bits_params_t *params, gs_int_rect **unread)
{
	...
	dev_proc_get_bits_rectangle((*proc)) =
	    (tdev == 0 ? (tdev = dev, gx_default_get_bits_rectangle) : dev_proc(tdev, get_bits_rectangle));
			       ...
			       gx_color_value prgba[4])
{
	...
	return (tdev == 0 ? gx_default_map_color_rgb_alpha(dev, color, prgba) : dev_proc(tdev, map_color_rgb_alpha)(tdev, color, prgba));
}
...
gx_forward_get_hardware_params(gx_device *dev, gs_param_list *plist)
{
	...
	return (tdev == 0 ? gx_default_get_hardware_params(dev, plist) : dev_proc(tdev, get_hardware_params)(tdev, plist));
}
		      ...
		      gs_text_enum_t **ppenum)
{
	...
	dev_proc_text_begin((*proc)) =
	    (tdev == 0 ? (tdev = dev, gx_default_text_begin) : dev_proc(tdev, text_begin));
				      ...
				      const frac31 *c, const int32_t *addx, const int32_t *mulx, int32_t divx)
{
	...
	dev_proc_fill_linear_color_scanline((*proc)) =
	    (tdev == 0 ? (tdev = dev, gx_default_fill_linear_color_scanline) : dev_proc(tdev, fill_linear_color_scanline));
				       ...
				       const frac31 *c2, const frac31 *c3)
{
	...
	dev_proc_fill_linear_color_trapezoid((*proc)) =
	    (tdev == 0 ? (tdev = dev, gx_default_fill_linear_color_trapezoid) : dev_proc(tdev, fill_linear_color_trapezoid));
				      ...
				      const frac31 *c0, const frac31 *c1, const frac31 *c2)
{
	...
	dev_proc_fill_linear_color_triangle((*proc)) =
	    (tdev == 0 ? (tdev = dev, gx_default_fill_linear_color_triangle) : dev_proc(tdev, fill_linear_color_triangle));
