...
private
dev_proc_print_page(okiibm_print_page);

/* Okidata IBM device */
const gx_device_printer far_data gs_okiibm_device = prn_device(
    prn_std_procs, "okiibm", DEFAULT_WIDTH_10THS, DEFAULT_HEIGHT_10THS, X_DPI,
    Y_DPI, 0.25, 0.0, 0.25, 0.0, /* margins */
    1, okiibm_print_page);
...
/* Forward references */
private
void okiibm_output_run(byte*, int, int, char, FILE*, int);
                   ...
                   const char* end_string, int end_length)
{
	...
	byte* buf1 = (byte*)gs_malloc(pdev->memory, in_size, 1,
	                              "okiibm_print_page(buf1)");
	byte* buf2 = (byte*)gs_malloc(pdev->memory, in_size, 1,
	                              "okiibm_print_page(buf2)");
	byte* in = buf1;
	byte* out = buf2;
	int out_y_mult = 1;
	...
	while(lnum < pdev->height) {
		...
		if(y_9pin_high) { /* Shuffle the scan lines */
			byte* p;
			int i;
			static const char index[] = {0, 2, 4, 6, 8, 10, 12, 14,
			                             1, 3, 5, 7, 9, 11, 13, 15};
			for(i = 0; i < 16; i++) {
				memcpy(out + (i * line_size),
				       in + (index[i] * line_size), line_size);
			}
			p = in;
			in = out;
			out = p;
...
private
void
okiibm_output_run(byte* data, int count, int y_mult, char start_graphics,
...
private
const char okiibm_init_string[] = {0x18};
private
const char okiibm_end_string[] = {0x0c};
private
const char okiibm_one_direct[] = {0x1b, 0x55, 0x01};
private
const char okiibm_two_direct[] = {0x1b, 0x55, 0x00};
...
okiibm_print_page(gx_device_printer* pdev, FILE* prn_stream)
{
	...
	if(pdev->y_pixels_per_inch > 72 && pdev->x_pixels_per_inch > 60) {
	...
	return okiibm_print_page1(
	    pdev, prn_stream, pdev->y_pixels_per_inch > 72 ? 1 : 0, init_string,
	    init_length, end_string, end_length);
}
