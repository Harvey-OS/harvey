...
TEMPLATE_spot_into_scanlines(line_list* ll, fixed band_mask)
{
	 ...
	 * epsilon.)
	 */
	fixed y_frac_min = (fo.adjust_above == fixed_0
	                        ? fixed_half
	                        : fixed_half + fixed_epsilon - fo.adjust_above);
	fixed y_frac_max = fixed_half + fo.adjust_below;
	...
	while(code >= 0) {
		...
		if(ll->x_list == 0)
			...
			for(alp = ll->x_list; alp != 0; alp = alp->next) {
				...
				yy = max(yy,
				         alp->end.y); /* Non-monotonic curves
				                         may have an inner
				                         extreme. */
				y = min(y, yy);
			}
		...
		if(y > y_top || y >= y_limit) {
			...
			for(pcr = rlist.first.next; pcr != &rlist.last;
				...
				if_debug4('Q', "[Qr]draw 0x%lx: [%d,%d),%d\n",
				          (uint32_t)pcr, x0, x1, y0);
				VD_RECT(x0, y0, x1 - x0, 1, VD_TRAP_COLOR);
				code = LOOP_FILL_RECTANGLE_DIRECT(&fo, x0, y0,
				                                  x1 - x0, 1);
		...
		if(y <= y_top) {
			/*
			 * We're within the same Y pixel.  Merge regions for
			 * segments
			 * starting here (at y), up to y_top or the end of the
			 * segment.
			 * If this is the first sampling within the band, run
			 * the
			...
			if(new_band) {
				...
				for(alp = ll->x_list; alp != 0;
				    alp = alp->next) {
					int x0 = fixed2int_pixround(
					    alp->x_current - fo.adjust_left);
					...
					for(;;) {
						 ...
						 * closed paths, the test
						 * for alp == 0 shouldn't be
						 * needed, but we may have
						 * omitted lines that are to the
						 * right of the
						 * clipping region.
						 */
						if((alp = alp->next) == 0)
