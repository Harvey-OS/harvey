...
case 1:
return ENUM_STRING(&pclr->keys);
case 2:
return ENUM_STRING(&pclr->values);
ENUM_PTRS_END
private
...
private
void
print_msg_str_in_range(const byte* str, const byte* key_lo, const byte* key_hi,
                       int key_size)
{
...
private
int
gs_multidim_CID_offset(const byte* key_str, const byte* key_lo,
                       const byte* key_hi, int key_size)
{
...
code_map_decode_next_multidim_regime(const gx_code_map_t* pcmap,
                                     const gs_const_string* pstr, uint* pindex,
                                     uint* pfidx, gs_char* pchr,
                                     gs_glyph* pglyph)
{
	...
	for(i = pcmap->num_lookup - 1; i >= 0; --i) {
		...
		if(0 < pre_size) {
			...
			if(0 == j) /* no match, skip to next i */
				...
				if(gs_debug_c('J')) {
					dlprintf("[J]CMDNmr() partial match "
					         "with prefix:");
					print_msg_str_in_range(
					    str, prefix, prefix, pre_size);
				}
				...
				continue; /* no need to check key, skip to next
				             i */
			}

			if(gs_debug_c('J')) {
				dlprintf(
				    "[J]CMDNmr()   full match with prefix:");
				print_msg_str_in_range(str, prefix, prefix,
				                       pre_size);
			}
		...
		/* full match in prefix. check key */
		{
			...
			for(k = 0; k < pclr->num_entries; ++k, key += step) {
				...
				if(gs_debug_c('j'))
					print_msg_str_in_range(
					    str + pre_size, key,
					    key + step - key_size, key_size);

				for(l = 0; l < key_size; l++) {
					byte c = str[l + pre_size];
					if(c < key[l] ||
					   c > key[step - key_size + l])
						break;
				}
			...
			*pfidx = pclr->font_index;
			...
			if(gs_debug_c('J')) {
				dlprintf(
				    "[J]CMDNmr()     full matched pvalue=(");
				debug_print_string_hex(pvalue,
				                       pclr->value_size);
				dlprintf(")\n");
			}
	...
	*pglyph = gs_no_glyph;
	if(gs_debug_c('J')) {
		dlprintf(
		    "[J]CMDNmr()     no full match, use partial match for (");
...
gs_cmap_adobe1_decode_next(const gs_cmap_t* pcmap_in,
                           const gs_const_string* pstr, uint* pindex,
                           uint* pfidx, gs_char* pchr, gs_glyph* pglyph)
{
	...
	code = code_map_decode_next_multidim_regime(&pcmap->def, pstr, pindex,
	                                            pfidx, pchr, pglyph);
	...
	*pindex = save_index;
	code = code_map_decode_next_multidim_regime(
	    &pcmap->notdef, pstr, pindex, pfidx, pchr, pglyph);
	...
	if(save_index < pm_index) {
		...
		*pfidx = pm_fidx;
		*pchr = '\0';
		return 0; /* should return some error for partial matched
		             .notdef? */
	} else {
		/* no match */
		...
		if(chr_size_shortest <= ssize) {
			...
			*pchr = '\0';
			if(gs_debug_c('J')) {
				dlprintf1("[J]GCDN() no partial match, skip %d "
				          "byte (",
				          chr_size_shortest);
				debug_print_string_hex(str, chr_size_shortest);
				dlprintf(")\n");
			}
			return 0; /* should return some error for fallback
			             .notdef? */
		} else {
			/* Undecodable string is shorter than the shortest
			 * character,
			 * there's no way except to return error.
			 */
			if(gs_debug_c('J')) {
				dlprintf2("[J]GCDN() left data in buffer (%d) "
				          "is shorter than shortest defined "
				          "character (%d)\n",
...
private
const gs_cmap_ranges_enum_procs_t adobe1_range_procs = {adobe1_next_range};
private
void
...
{
	return adobe1_next_lookup(
	    penum, &((const gs_cmap_adobe1_t*)penum->cmap)->notdef);
}
private
int
adobe1_next_entry(gs_cmap_lookups_enum_t* penum, const gx_code_map_t* pcm)
{
	const gx_cmap_lookup_range_t* lookup =
	    &pcm->lookup[penum->index[0] - 1];
	int psize = lookup->key_prefix_size;
	int ksize = lookup->key_size;
	const byte* key =
	    lookup->keys.data +
	    penum->index[1] * ksize * (lookup->key_is_range ? 2 : 1);
	int i;
...
{
	return adobe1_next_entry(
	    penum, &((const gs_cmap_adobe1_t*)penum->cmap)->notdef);
}
private
const gs_cmap_lookups_enum_procs_t adobe1_lookup_def_procs = {
    adobe1_next_lookup_def, adobe1_next_entry_def};
private
const gs_cmap_lookups_enum_procs_t adobe1_lookup_notdef_procs = {
...
{
	gs_cmap_lookups_enum_setup(
	    pre, pcmap,
	    (which ? &adobe1_lookup_notdef_procs : &adobe1_lookup_def_procs));
}

private
const gs_cmap_procs_t cmap_adobe1_procs = {
    gs_cmap_adobe1_decode_next, gs_cmap_adobe1_enum_ranges,
    gs_cmap_adobe1_enum_lookups, gs_cmap_compute_identity};

int
gs_cmap_adobe1_alloc(gs_cmap_adobe1_t** ppcmap, int wmode, const byte* map_name,
                     uint name_size, uint num_fonts, uint num_ranges,
                     uint num_lookups, uint keys_size, uint values_size,
                     const gs_cid_system_info_t* pcidsi_in, gs_memory_t* mem)
{
	gs_cmap_t* pcmap;
	gs_cmap_adobe1_t* pcmap1;
	gx_code_space_range_t* ranges =
	    (gx_code_space_range_t*)gs_alloc_byte_array(
	        mem, num_ranges, sizeof(gx_code_space_range_t),
	        "gs_cmap_alloc(code space ranges)");
	gx_cmap_lookup_range_t* lookups =
	    (num_lookups == 0
	         ? NULL
	         : gs_alloc_struct_array(
	               mem, num_lookups, gx_cmap_lookup_range_t,
	               &st_cmap_lookup_range, "gs_cmap_alloc(lookup ranges)"));
	byte* keys =
	    (keys_size == 0 ? NULL : gs_alloc_string(mem, keys_size,
	                                             "gs_cmap_alloc(keys)"));
	byte* values =
	    (values_size == 0
	         ? NULL
	...
	if(code < 0 || ranges == 0 || (num_lookups != 0 && lookups == 0) ||
	   (keys_size != 0 && keys == 0) || (values_size != 0 && values == 0)) {
		gs_free_string(mem, values, values_size,
		               "gs_cmap_alloc(values)");
