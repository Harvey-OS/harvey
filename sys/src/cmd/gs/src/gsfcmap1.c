...
private
const gs_cmap_ranges_enum_procs_t adobe1_range_procs = {
    adobe1_next_range};
private
void
...
adobe1_next_entry(gs_cmap_lookups_enum_t *penum, const gx_code_map_t *pcm)
{
	...
	const byte *key =
	    lookup->keys.data + penum->index[1] * ksize * (lookup->key_is_range ? 2 : 1);
	int i;
		     ...
		     const gs_cid_system_info_t *pcidsi_in, gs_memory_t *mem)
{
	...
	gx_cmap_lookup_range_t *lookups =
	    (num_lookups == 0 ? NULL : gs_alloc_struct_array(mem, num_lookups, gx_cmap_lookup_range_t,
							     &st_cmap_lookup_range,
							     "gs_cmap_alloc(lookup ranges)"));
	byte *keys =
	    (keys_size == 0 ? NULL : gs_alloc_string(mem, keys_size, "gs_cmap_alloc(keys)"));
	byte *values =
	    (values_size == 0 ? NULL : gs_alloc_string(mem, values_size, "gs_cmap_alloc(values)"));
	int code =
