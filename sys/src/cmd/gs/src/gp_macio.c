...
getenv(const char* env)
{
	...
	if(strcmp(env, "GS_LIB") == 0) {
		...
		pFile.name[0] = 0;
		err =
		    FindFolder(kOnSystemDisk, kApplicationSupportFolderType,
		               kDontCreateFolder, &pFile.vRefNum, &pFile.parID);

		if(err != noErr)
			goto failed;

		//		FSMakeFSSpec(pFile.vRefNum, pFile.parID,thepfname,
		//&pfile);
		...
		sprintf(p, "%s,%sGhostscript:lib,%sGhostscript:fonts",
		        (char*)&fpath[0], (char*)&fpath[0], (char*)&fpath[0]);
...
/* Forward references */
private
void mac_std_init(void);
private
stream_proc_process(mac_stdin_read_process);
private
stream_proc_process(mac_stdout_write_process);
private
stream_proc_process(mac_stderr_write_process);
private
stream_proc_available(mac_std_available);

/* Use a pseudo IODevice to get mac_stdio_init called at the right time. */
/* This is bad architecture; we'll fix it later. */
private
iodev_proc_init(mac_stdio_init);
const gx_io_device gs_iodev_macstdio = {
    "macstdio",
    "Special",
    {mac_stdio_init, iodev_no_open_device, iodev_no_open_file, iodev_no_fopen,
     iodev_no_fclose, iodev_no_delete_file, iodev_no_rename_file,
...
{
	if(strlen(fname) == 0)
		return gp_open_scratch_file(gp_scratch_file_name_prefix, fname,
		                            binary_mode ? "wb" : "w");
	else
                     ...
                     const char* mode)
{
	...
	if(prefix_length > gp_file_name_sizeof)
		return NULL;
	strcpy(fname, (char*)prefix);
	{
		...
		tmpnam(newName);
		if(prefix_length + strlen(newName) > gp_file_name_sizeof)
			return NULL;
	...
	if(strrchr(fname, ':') == NULL) {
		...
		memmove((char*)&thepfname[1], (char*)&thefname[0],
		        strlen(thefname));
 ...
 */
int
gp_native_fontmap(char* names[], char* paths[], int* count)
{
	return 0;
}
...
{
	file_enum* pfen = (file_enum*)gs_alloc_bytes(memory, sizeof(file_enum),
	                                             "gp_enumerate_files");
	char* pattern;
	if(pfen == 0)
		return 0;
	pattern = (char*)gs_alloc_bytes(memory, patlen + 1,
...
{
	gs_free_object(pfen->memory, pfen->pattern,
...
static int
compare_UniStr(HFSUniStr255 u, const char* c, uint len)
{
	...
	for(i = 0; i < searchlen; i++) {
		...
		if(unichar & !0xFF)
			return -1;
		/* otherwise return the the index of the first non-matching
		 * character */
		if(unichar != c[i])
			break;
	}
...
gp_file_name_root(const char* fname, uint len)
{
	...
	index = 1;
	while(err == noErr) {
		err = FSGetVolumeInfo(kFSInvalidVolumeRefNum, index, NULL,
		                      kFSVolInfoNone,
		                      NULL, /* not interested in these fields */
		                      &volumeName, &rootDirectory);
		if(err == nsvErr)
			return 0; /* no more volumes */
		if(err == noErr) {
...
{
	return gp_file_name_combine_generic(prefix, plen, fname, flen,
	                                    no_sibling, buffer, blen);
}

// FIXME: there must be a system util for this!
static char*
MacStr2c(char* pstring)
{
	char* cstring;
...
static void
fond_table_free(fond_table* table)
{
	if(table != NULL) {
		if(table->refs)
			free(table->refs);
...
static fond_table*
fond_table_grow(fond_table* table, int entries)
{
	if(table == NULL) {
		...
		table->entries += entries;
		table->refs =
		    realloc(table->refs, table->entries * sizeof(fond_entry));
	}
	return table;
}

static int
get_int16(unsigned char* p)
{
	return (p[0] & 0xFF) << 8 | (p[1] & 0xFF);
}

static int
get_int32(unsigned char* p)
{
...
parse_fond(FSSpec* spec)
{
	...
	if(result != noErr || ref <= 0) {
		...
		dlprintf2("unable to open resource file '%s' for font "
		          "enumeration (error %d)\n",
		          path, result);
		goto fin;
	}
	...
	for(i = 0; i < count; i++) {
		...
		/* The FOND resource structure corresponds to the FamRec and
		   AsscEntry
		   data structures documented in the FontManager reference.
		   However,
		   access to these types is deprecated in Carbon. We therefore
		   access the
		   data by direct offset in the hope that the resource format
		   will not change
		...
		for(j = start; j < start + n; j++) {
			table->refs[j].size = get_int16(res);
			res += 2;
			table->refs[j].style = get_int16(res);
			res += 2;
			table->refs[j].id = get_int16(res);
			res += 2;
		}
		start += n;
		HUnlock(fond);
...
static int
is_ttf_file(const char* path)
{
	int len = strlen(path);
	return !memcmp(path + len - 4, ".ttf", 4);
}
static int
is_otf_file(const char* path)
{
	int len = strlen(path);
...
static void
strip_char(char* string, int len, const int c)
{
	char* bit;
	len += 1;
...
/* get the macos name for the font instance and mangle it into a PS
   fontname */
static char*
makePSFontName(FMFontFamily Family, FMFontStyle Style)
{
	Str255 Name;
	OSStatus result;
	int length;
...
gp_enumerate_fonts_free(void* enum_state)
{
	...
	if(state->path)
		free(state->path);
	if(state->last_container_path)
		free(state->last_container_path);
	if(state->last_table)
		fond_table_free(state->last_table);
...
gp_enumerate_fonts_next(void* enum_state, char** fontname, char** path)
{
	...
	if(table != NULL) {
		...
		for(i = 0; i < table->entries; i++) {
			if(table->refs[i].size ==
			   0) { /* ignore non-scalable fonts */
				if(table->refs[i].style == Style) {
					int len =
					    strlen(fontpath) +
					    strlen("%macresource%#sfnt+") + 6;
					state->path = malloc(len);
					snprintf(state->path, len,
					         "%%macresource%%%s#sfnt+%d",
	...
	if(state->path == NULL) {
		/* no matching font was found in the FOND resource table. this
		 * usually */
		/* means an LWFN file, which we don't handle yet. */
		/* we still specify these with a %macresource% path, but no res
		 * id */
