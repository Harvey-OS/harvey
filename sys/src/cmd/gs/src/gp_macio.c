...
const gx_io_device gs_iodev_macstdio =
    {
     "macstdio", "Special", {mac_stdio_init, iodev_no_open_device, iodev_no_open_file, iodev_no_fopen, iodev_no_fclose, iodev_no_delete_file, iodev_no_rename_file, iodev_no_file_status, iodev_no_enumerate_files}};
...
uint
gs_file_name_check_separator(const char *fname, int len,
			     const char *item)
{
	if(len > 0) {
		if(fname[0] == ':') {
...
int
gp_enumerate_fonts_next(void *enum_state, char **fontname,
			char **path)
{
	fontenum_t *state = (fontenum_t *)enum_state;
	FMFontIterator *Iterator = &state->Iterator;
