...
/* Copyright (C) 1992, 1993, 1994, 1997, 1998, 1999 Aladdin Enterprises.  All
  rights reserved.
...
#define TRANSPOSE(r, s, mask, shift)                                           \
	(r ^= (temp = ((s >> shift) ^ r) & mask), s ^= temp << shift)
...
const string_match_params string_match_params_default = {'*', '?', '\\', false,
                                                         false};
...
uid_equal(register const gs_uid* puid1, register const gs_uid* puid2)
{
	...
	return !memcmp((const char*)puid1->xvalues, (const char*)puid2->xvalues,
...
uid_copy(gs_uid* puid, gs_memory_t* mem, client_name_t cname)
{
	if(uid_is_XUID(puid)) {
		...
		int32_t* xvalues = (int32_t*)gs_alloc_byte_array(
		    mem, xsize, sizeof(int32_t), cname);
