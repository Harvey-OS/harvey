			  ...
			  gs_logical_operation_t lop)
{
	...
	for(py = y; py < y + height; py += block_height) {
		...
		if(row /*uses_d*/) {
			...
			code = (*dev_proc(dev, get_bits_rectangle))(dev, &rect, &bit_params, NULL);
			if(code < 0)
				break;
			code = (*dev_proc(&mdev, copy_color))((gx_device *)&mdev, bit_params.data[0], bit_params.x_offset,
		...
		if(code < 0)
			break;
		code = (*dev_proc(dev, copy_color))(dev, scan_line_base(&mdev, 0), 0, draster, gx_no_bitmap_id,
		   ...
		   int width, int depth, int src_depth)
{
	dev_proc_map_rgb_color((*map)) =
	    (dev->color_info.num_components == 4 ? map_rgb_to_color_via_cmyk : dev_proc(dev, map_rgb_color));
...
#define ALLOC_BUF(buf, prebuf, size, cname)                     \
	...
	if(gdev_mem_data_size(&mdev, width, block_height) <= sizeof(mdev_storage)) {
		...
		mdev.line_ptrs = (byte **)(mdev.base + gdev_mem_bits_size(&mdev, mdev.width, mdev.height));
	} else {
		mdev.bitmap_memory = mem;
	}
	code = (*dev_proc(&mdev, open_device))((gx_device *)&mdev);
	...
	for(py = y; py < y + height; py += loop_height) {
		...
		code = (*dev_proc(&mdev, strip_copy_rop))((gx_device *)&mdev, source_data, sx, source_raster,
		    ...
		    int phase_x, int phase_y, gs_logical_operation_t lop)
{
	...
	return (*dev_proc(dev, strip_copy_rop))(dev, sdata, sourcex, sraster, id, scolors, textures, tcolors,
						x, y, width, height, phase_x, phase_y, lop);
}
		      ...
		      int phase_x, int phase_y, gs_logical_operation_t lop)
{
	...
	return gx_strip_copy_rop_unaligned(dev, sdata, sourcex, sraster, id, scolors, textures, tcolors,
					   x, y, width, height, phase_x, phase_y, lop);
}
			    ...
			    int phase_x, int phase_y, gs_logical_operation_t lop)
{
	...
	/* Adjust the raster. */
	if(!step || sdata == 0 ||
	   (scolors != 0 && scolors[0] == scolors[1])) { /* No adjustment needed. */
...
#define pattern_transparent ((lop & lop_T_transparent) && rop3_uses_T(rop))
	gs_rop3_t mask =
	    (source_transparent ? (pattern_transparent ? So & Po : So) : (pattern_transparent ? MPo : rop3_1));
