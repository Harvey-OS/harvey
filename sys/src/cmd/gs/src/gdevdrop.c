 ...
 */
private
const uint max_rop_bitmap = 1000;
...
void
trace_copy_rop(const char* cname, gx_device* dev, const byte* sdata,
               int sourcex, uint sraster, gx_bitmap_id id,
               const gx_color_index* scolors, const gx_strip_bitmap* textures,
               const gx_color_index* tcolors, int x, int y, int width,
               int height, int phase_x, int phase_y, gs_logical_operation_t lop)
{
	dlprintf4("%s: dev=0x%lx(%s) depth=%d\n", cname, (uint32_t)dev,
	          dev->dname, dev->color_info.depth);
	...
	if(textures)
		dlprintf8(
		    "  textures=0x%lx size=%dx%d(%dx%d) raster=%u shift=%d(%d)",
	...
	dlprintf7("  rect=(%d,%d),(%d,%d) phase=(%d,%d) op=0x%x\n", x, y,
	          x + width, y + height, phase_x, phase_y, (uint)lop);
	if(gs_debug_c('B')) {
		if(sdata)
			debug_dump_bitmap(sdata, sraster, height,
			                  "source bits");
 ...
 */
int
gx_default_strip_copy_rop(gx_device* dev, const byte* sdata, int sourcex,
                          ...
                          gs_logical_operation_t lop)
{
...
#ifdef DEBUG
	if(gs_debug_c('b'))
		trace_copy_rop("gx_default_strip_copy_rop", dev, sdata, sourcex,
		               sraster, id, scolors, textures, tcolors, x, y,
		               width, height, phase_x, phase_y, lop);
#endif
	if(mdproto == 0)
	...
	if(rop3_uses_D(gs_transparent_rop(lop))) {
		row =
		    gs_alloc_bytes(mem, draster * block_height, "copy_rop row");
		if(row == 0) {
	...
	for(py = y; py < y + height; py += block_height) {
		...
		if(row /*uses_d*/) {
			...
			bit_params.options = GB_COLORS_NATIVE | GB_ALPHA_NONE |
			                     GB_DEPTH_ALL | GB_PACKING_CHUNKY |
			...
			code = (*dev_proc(&mdev, copy_color))(
			    (gx_device*)&mdev, bit_params.data[0],
			    bit_params.x_offset, draster, gx_no_bitmap_id, 0, 0,
			    width, block_height);
			if(code < 0)
				return code;
		}
		code = (*dev_proc(&mdev, strip_copy_rop))(
		    (gx_device*)&mdev, sdata + (py - y) * sraster, sourcex,
		    sraster, gx_no_bitmap_id, scolors, textures, tcolors, 0, 0,
		    width, block_height, phase_x + x, phase_y + py, lop);
		if(code < 0)
			break;
...
map_rgb_to_color_via_cmyk(gx_device* dev, const gx_color_value rgbcv[])
{
	...
	cmykcv[3] = (cmykcv[0] < cmykcv[1] ? min(cmykcv[0], cmykcv[2])
	                                   : min(cmykcv[1], cmykcv[2]));
...
{
	dev_proc_map_rgb_color((*map)) = (dev->color_info.num_components == 4
	                                      ? map_rgb_to_color_via_cmyk
 ...
 */
int
mem_default_strip_copy_rop(gx_device* dev, const byte* sdata, int sourcex,
                           uint sraster, gx_bitmap_id id,
                           const gx_color_index* scolors,
                           const gx_strip_bitmap* textures,
                           const gx_color_index* tcolors, int x, int y,
                           ...
                           gs_logical_operation_t lop)
{
	...
	const gx_bitmap_format_t expand_options =
	    (rop_depth > 8 ? GB_COLORS_RGB : GB_COLORS_GRAY) | GB_ALPHA_NONE |
	    GB_DEPTH_8 | GB_PACKING_CHUNKY | GB_RETURN_COPY |
...
#define ALLOC_BUF(buf, prebuf, size, cname)                                    \
...
#ifdef DEBUG
	if(gs_debug_c('b'))
		trace_copy_rop("mem_default_strip_copy_rop", dev, sdata,
		               sourcex, sraster, id, scolors, textures, tcolors,
	...
	if(gdev_mem_data_size(&mdev, width, block_height) <=
	   sizeof(mdev_storage)) {
		/* Use the locally allocated storage. */
		mdev.base = (byte*)mdev_storage;
		mdev.line_ptrs =
		    (byte**)(mdev.base + gdev_mem_bits_size(&mdev, mdev.width,
		                                            mdev.height));
	} else {
		mdev.bitmap_memory = mem;
	}
	...
	if(scolors && uses_s) {
		unpack_colors_to_standard(dev, source_colors, scolors,
		                          rop_depth);
	...
	if(tcolors && uses_t) {
		unpack_colors_to_standard(dev, texture_colors, tcolors,
		                          rop_depth);
	...
	for(py = y; py < y + height; py += loop_height) {
		...
		if(expand_t) {
			...
			loop_height =
			    min(block_height, rop_texture.size.y - rep_y);
			rect.q.y = py + loop_height;
			expand_params.data[0] = texture_row;
			gx_get_bits_copy(
			    dev, 0, textures->rep_width, loop_height,
			 ...
			 */
			rop_texture.data =
			    texture_row - rep_y * rop_texture.raster;
		} else {
		...
		if(expand_s) {
			...
			gx_get_bits_copy(dev, sx, width, loop_height,
			                 &expand_params, &no_expand_params,
			                 source_data, sraster);
			sx = 0;
		...
		{
			/*
			 * Convert the result back to the device's format.  We
			 * know
			 * the device is a memory device, so we can store the
			 * result
			...
			for(i = 0; i < loop_height;
			    unpacked += mdev.raster, ++i) {
				byte* packed = scan_line_base(
				    (gx_device_memory*)dev, py + i);

				pack(dev, packed, x, unpacked, width, depth,
				     rop_depth);
			}
		}
	}
...
int
gx_default_copy_rop(gx_device* dev, const byte* sdata, int sourcex,
                    uint sraster, gx_bitmap_id id,
                    const gx_color_index* scolors,
                    ...
                    gs_logical_operation_t lop)
{
	...
	return (*dev_proc(dev, strip_copy_rop))(
	    dev, sdata, sourcex, sraster, id, scolors, textures, tcolors, x, y,
	    width, height, phase_x, phase_y, lop);
}

int
gx_copy_rop_unaligned(gx_device* dev, const byte* sdata, int sourcex,
                      uint sraster, gx_bitmap_id id,
                      const gx_color_index* scolors,
                      const gx_tile_bitmap* texture,
                      const gx_color_index* tcolors, int x, int y, int width,
...
int
gx_strip_copy_rop_unaligned(gx_device* dev, const byte* sdata, int sourcex,
                            uint sraster, gx_bitmap_id id,
                            const gx_color_index* scolors,
                            const gx_strip_bitmap* textures,
                            const gx_color_index* tcolors, int x, int y,
                            ...
                            gs_logical_operation_t lop)
{
	...
	if(sdata != 0) {
		...
		if(depth == 24)
			offset += (offset % 3) * (align_bitmap_mod *
			                          (3 - (align_bitmap_mod % 3)));
		sdata -= offset;
	...
	if(!step || sdata == 0 ||
	   (scolors != 0 &&
	    scolors[0] == scolors[1])) { /* No adjustment needed. */
	...
	/* Do the transfer one scan line at a time. */
	{
		...
		for(i = 0; i < height && code >= 0;
			...
			code = (*copy_rop)(dev, p, d, sraster, gx_no_bitmap_id,
			                   scolors, textures, tcolors, x, y + i,
			                   width, 1, phase_x, phase_y, lop);
		return code;
	}
}
...
#define pattern_transparent ((lop & lop_T_transparent) && rop3_uses_T(rop))
	gs_rop3_t mask =
	    (source_transparent ? (pattern_transparent ? So & Po : So)
