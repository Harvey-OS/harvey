...
#define PDF_RESOURCE_TYPE_NAMES                                                \
	"/ColorSpace", "/ExtGState", "/Pattern", "/Shading", "/XObject", 0,    \
	    "/Font", 0, "/Font", "/CMap", "/FontDescriptor", "/Group",         \
	    "/Mask", 0, 0
#define PDF_RESOURCE_TYPE_STRUCTS                                              \
	&st_pdf_color_space,  /* gdevpdfg.h / gdevpdfc.c */                    \
	    &st_pdf_resource, /* see below */                                  \
	    &st_pdf_pattern, &st_pdf_resource,                                 \
	    ...
	    &st_pdf_resource, &st_pdf_resource, &st_pdf_resource,              \
	    &st_pdf_resource
...
#define public_st_pdf_resource() /* in gdevpdfu.c */                           \
	gs_public_st_ptrs3(st_pdf_resource, pdf_resource_t, "pdf_resource_t",  \
	                   pdf_resource_enum_ptrs, pdf_resource_reloc_ptrs,    \
	                   next, prev, object)
...
#define private_st_pdf_x_object() /* in gdevpdfu.c */                          \
	gs_private_st_suffix_add0(st_pdf_x_object, pdf_x_object_t,             \
	                          "pdf_x_object_t", pdf_x_object_enum_ptrs,    \
	                          pdf_x_object_reloc_ptrs, st_pdf_resource)
...
#define private_st_pdf_article()                                               \
	gs_private_st_ptrs2(st_pdf_article, pdf_article_t, "pdf_article_t",    \
	                    pdf_article_enum_ptrs, pdf_article_reloc_ptrs,     \
	                    next, contents)
...
typedef struct pdf_page_s {
	...
	long contents_id;
	long
	    resource_ids[resourceFont + 1]; /* resources thru Font, see above */
	long group_id;
	cos_array_t* Annots;
...
#define private_st_pdf_page() /* in gdevpdf.c */                               \
	gs_private_st_ptrs2(st_pdf_page, pdf_page_t, "pdf_page_t",             \
	                    pdf_page_enum_ptrs, pdf_page_reloc_ptrs, Page,     \
	                    Annots)
...
#define private_st_pdf_substream_save()                                        \
	gs_private_st_strings1_ptrs7(                                          \
	    st_pdf_substream_save, pdf_substream_save, "pdf_substream_save",   \
	    pdf_substream_save_enum, pdf_substream_save_reloc, objname,        \
	    text_state, clip_path, strm, substream_Resources, font3,           \
	    accumulating_substream_resource, pres_soft_mask_dict)
#define private_st_pdf_substream_save_element()                                \
	gs_private_st_element(                                                 \
	    st_pdf_substream_save_element, pdf_substream_save,                 \
...
struct gx_device_pdf_s {
	...
	long MaxClipPathSize;  /* The maximal number of elements of a clipping
	                          path
...
#define initial_num_pages 50
	 ...
	 * Usually 5 levels is enough, but patterns and charprocs may be nested
	 * recursively.
	 */
	...
	pdf_viewer_state
	    vg_initial; /* Initial values for viewer's graphic state */
	bool vg_initial_set;
	...
	gs_color_space_index
	    pcm_color_info_index; /* Index of the ProcessColorModel space. */
	bool
	    skip_colors;   /* Skip colors while a pattern/charproc accumulation.
	                      */
...
#define is_in_page(pdev) ((pdev)->contents_id != 0)
#define is_in_document(pdev) (is_in_page(pdev) || (pdev)->last_resource != 0)

/* Enumerate the individual pointers in a gx_device_pdf. */
#define gx_device_pdf_do_ptrs(m)                                               \
	m(0, asides.strm) m(1, asides.strm_buf) m(2, asides.save_strm) m(      \
	    3, streams.strm) m(4, streams.strm_buf) m(5, pictures.strm)        \
	    m(6, pictures.strm_buf) m(7, pictures.save_strm) m(8, Catalog) m(  \
	        9, Info) m(10, Pages) m(11, text) m(12, pages)                 \
	        m(13, cs_Patterns[0]) m(14, cs_Patterns[1]) m(15,              \
	                                                      cs_Patterns[3])  \
	            m(16, cs_Patterns[4]) m(17, last_resource) m(18, articles) \
	                m(19, Dests) m(20, global_named_objects) m(            \
	                    21, local_named_objects) m(22, NI_stack)           \
	                    m(23, Namespace_stack) m(24, font_cache) m(        \
	                        25, clip_path) m(26, PageLabels)               \
	                        m(27, PageLabels_current_label) m(28, sbstack) \
	                            m(29, substream_Resources) m(30, font3)    \
...
#define gx_device_pdf_do_param_strings(m)                                      \
	m(0, OPDFReadProcsetPath) m(1, OwnerPassword) m(2, UserPassword)       \
	    m(3, NoEncrypt)
#define gx_device_pdf_num_param_strings 4
#define gx_device_pdf_do_const_strings(m) m(0, objname)
...
#define private_st_device_pdfwrite() /* in gdevpdf.c */                        \
	gs_private_st_composite_final(                                         \
	    st_device_pdfwrite, gx_device_pdf, "gx_device_pdf",                \
	    device_pdfwrite_enum_ptrs, device_pdfwrite_reloc_ptrs,             \
...
int pdf_substitute_resource(gx_device_pdf* pdev, pdf_resource_t** ppres,
                            pdf_resource_type_t rtype,
                            int (*eq)(gx_device_pdf* pdev,
                                      pdf_resource_t* pres0,
                                      pdf_resource_t* pres1),
                            bool write);
...
int pdf_encrypt_init(const gx_device_pdf* pdev, gs_id object_id,
...
#define public_st_pdf_lcvd_t()                                                 \
	gs_public_st_suffix_add2(st_pdf_lcvd_t, pdf_lcvd_t, "pdf_lcvd_t",      \
	                         pdf_lcvd_t_enum_ptrs, pdf_lcvd_t_reloc_ptrs,  \
	                         st_device_memory, mask, pdev)
#define pdf_lcvd_t_max_ptrs (gx_device_memory_max_ptrs + 2)

int pdf_setup_masked_image_converter(gx_device_pdf* pdev, gs_memory_t* mem,
...
void pdf_remove_masked_image_converter(gx_device_pdf* pdev, pdf_lcvd_t* cvd,
                                       bool need_mask);
...
#define PDF_FILTER_NAMES                                                       \
	"/ASCII85Decode", "/ASCIIHexDecode", "/CCITTFaxDecode", "/DCTDecode",  \
	    "/DecodeParms", "/Filter", "/FlateDecode", "/LZWDecode",           \
	    "/RunLengthDecode"
...
pdf_put_name_chars_proc_t pdf_put_name_chars_proc(const gx_device_pdf* pdev);
int pdf_put_name_chars(const gx_device_pdf* pdev, const byte* nstr, uint size);
...
int pdf_write_value(const gx_device_pdf* pdev, const byte* vstr, uint size,
                    gs_id object_id);
...
typedef struct pdf_data_writer_s {
	...
	long length_pos;
	pdf_resource_t* pres;
	gx_device_pdf* pdev; /* temporary for backward compatibility of
...
#define DATA_STREAM_NOLENGTH                                                   \
	4 /* Skip the length reference and filter names writing. */
...
int pdf_set_charproc_attrs(gx_device_pdf* pdev, gs_font* font, const double* pw,
                           int narg, gs_text_cache_control_t control,
                           gs_char ch, gs_const_string* gnstr);
...
int pdf_enter_substream(gx_device_pdf* pdev, pdf_resource_type_t rtype,
                        ...
                        bool reserve_object_id, bool compress);
...
int pdf_add_resource(gx_device_pdf* pdev, cos_dict_t* pcd, const char* key,
                     pdf_resource_t* pres);
