...
gs_locked_alloc_bytes(gs_memory_t *mem, uint size, client_name_t cname)
{
	RETURN_MONITORED(
	    byte *,
	    (*lmem->target->procs.alloc_bytes)(lmem->target, size, cname));
}
private
byte *
				...
				client_name_t cname)
{
	RETURN_MONITORED(
	    byte *,
	    (*lmem->target->procs.alloc_bytes_immovable)(lmem->target, size, cname));
}
private
void *
		       ...
		       client_name_t cname)
{
	RETURN_MONITORED(
	    void *,
	    (*lmem->target->procs.alloc_struct)(lmem->target, pstype, cname));
}
private
void *
				 ...
				 gs_memory_type_ptr_t pstype, client_name_t cname)
{
	RETURN_MONITORED(
	    void *,
	    (*lmem->target->procs.alloc_struct_immovable)(lmem->target, pstype, cname));
}
private
byte *
			   ...
			   client_name_t cname)
{
	RETURN_MONITORED(
	    byte *,
	    (*lmem->target->procs.alloc_byte_array)(lmem->target, num_elements, elt_size, cname));
}
private
byte *
				     ...
				     uint elt_size, client_name_t cname)
{
	RETURN_MONITORED(
	    byte *,
	    (*lmem->target->procs.alloc_byte_array_immovable)(lmem->target, num_elements, elt_size, cname));
}
private
void *
			     ...
			     gs_memory_type_ptr_t pstype, client_name_t cname)
{
	RETURN_MONITORED(
	    void *,
	    (*lmem->target->procs.alloc_struct_array)(lmem->target, num_elements, pstype, cname));
}
private
void *
				       ...
				       gs_memory_type_ptr_t pstype, client_name_t cname)
{
	RETURN_MONITORED(
	    void *,
	    (*lmem->target->procs.alloc_struct_array_immovable)(lmem->target, num_elements, pstype, cname));
}
private
void *
			...
			client_name_t cname)
{
	RETURN_MONITORED(
	    void *,
	    (*lmem->target->procs.resize_object)(lmem->target, obj, new_num_elements, cname));
}
private
uint
...
gs_locked_alloc_string(gs_memory_t *mem, uint nbytes, client_name_t cname)
{
	RETURN_MONITORED(
	    byte *,
	    (*lmem->target->procs.alloc_string)(lmem->target, nbytes, cname));
}
private
byte *
				 ...
				 client_name_t cname)
{
	RETURN_MONITORED(
	    byte *,
	    (*lmem->target->procs.alloc_string_immovable)(lmem->target, nbytes, cname));
}
private
byte *
			...
			client_name_t cname)
{
	RETURN_MONITORED(
	    byte *,
	    (*lmem->target->procs.resize_string)(lmem->target, data, old_num, new_num, cname));
}
private
void
