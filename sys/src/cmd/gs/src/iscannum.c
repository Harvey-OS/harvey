 ...
 */
int
scan_number(const byte* str, const byte* end, int sign, ref* pref,
            const byte** psp, const bool PDFScanInvNum)
{
	const byte* sp = str;
...
#define NUM_POWERS_10 6
	static const float powers_10[NUM_POWERS_10 + 1] = {1e0, 1e1, 1e2, 1e3,
	                                                   1e4, 1e5, 1e6};
...
#define IS_DIGIT(d, c) ((d = decoder[c]) < 10)
...
i2r:
	...
	while(IS_DIGIT(d, c) || c == '-') {
		 ...
		 * '.'
		 * Adobe Acrobat Reader (PDF) apparently doesn't treat this as
		 * an
		 * error. Experiments show that the numbers following the '-'
		 * are
		 * ignored, so we swallow the fractional part. PDFScanInvNum
		 * enables
	...
	if(!(c == 'e' || c == 'E' ||
	     exp10 < -NUM_POWERS_10)) { /* Check for trailing garbage */
		if(c != EOFC)
			*psp = sp, code = 1;
