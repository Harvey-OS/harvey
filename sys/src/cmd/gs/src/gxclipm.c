...
/* Device procedures */
private
dev_proc_fill_rectangle(mask_clip_fill_rectangle);
private
dev_proc_copy_mono(mask_clip_copy_mono);
private
dev_proc_copy_color(mask_clip_copy_color);
private
dev_proc_copy_alpha(mask_clip_copy_alpha);
private
dev_proc_strip_tile_rectangle(mask_clip_strip_tile_rectangle);
private
dev_proc_strip_copy_rop(mask_clip_strip_copy_rop);
private
dev_proc_get_clipping_box(mask_clip_get_clipping_box);

/* The device descriptor. */
const gx_device_mask_clip gs_mask_clip_device = {
    std_device_std_body_open(gx_device_mask_clip, 0, "mask clipper", 0, 0, 1,
                             1),
    {gx_default_open_device, gx_forward_get_initial_matrix,
     gx_default_sync_output, gx_default_output_page, gx_default_close_device,
     gx_forward_map_rgb_color, gx_forward_map_color_rgb,
     mask_clip_fill_rectangle, gx_default_tile_rectangle, mask_clip_copy_mono,
     mask_clip_copy_color, gx_default_draw_line, gx_forward_get_bits,
     ...
     gx_forward_get_alpha_bits, mask_clip_copy_alpha, gx_forward_get_band,
     ...
     gx_default_end_image, mask_clip_strip_tile_rectangle,
     mask_clip_strip_copy_rop, mask_clip_get_clipping_box,
     ...
     gx_forward_include_color_space, gx_forward_fill_linear_color_scanline,
                         ...
                         gx_color_index color)
{
	...
	return (*dev_proc(tdev, copy_mono))(
	    ...
	    my0 - cdev->phase.y, mx1 - mx0, my1 - my0, gx_no_color_index,
	    color);
}
...
private
int
mask_clip_copy_mono(gx_device* dev, const byte* data, int sourcex, int raster,
                    ...
                    gx_color_index color0, gx_color_index color1)
{
	...
	for(cy = my0; cy < my1; cy += ny) {
		...
		for(cx = mx0; cx < mx1; cx += nx) {
			...
			(*dev_proc(&cdev->mdev, copy_mono))(
			    (gx_device*)&cdev->mdev, sdata + (ty - y) * raster,
			    sx + tx - x, raster, gx_no_bitmap_id, cx, 0, nx, ny,
			    mcolor0, mcolor1);
			/* Now copy the color through the double mask. */
			code = (*dev_proc(tdev, copy_mono))(
			    cdev->target, cdev->buffer.bytes, cx,
			    cdev->tiles.raster, gx_no_bitmap_id, tx, ty, nx, ny,
                    ...
                    clip_callback_data_t* pccd)
{
	...
	FIT_MASK_COPY(pccd->data, pccd->sourcex, pccd->raster, pccd->x, pccd->y,
	              pccd->w, pccd->h);
...
private
int
mask_clip_copy_color(gx_device* dev, const byte* data, int sourcex, int raster,
                     gx_bitmap_id id, int x, int y, int w, int h)
{
	gx_device_mask_clip* cdev = (gx_device_mask_clip*)dev;
...
private
int
mask_clip_copy_alpha(gx_device* dev, const byte* data, int sourcex, int raster,
                     gx_bitmap_id id, int x, int y, int w, int h,
                               ...
                               int phase_x, int phase_y)
{
	...
	return clip_runs_enumerate(cdev, clip_call_strip_tile_rectangle,
	                           &ccdata);
}

private
int
mask_clip_strip_copy_rop(gx_device* dev, const byte* data, int sourcex,
                         uint raster, gx_bitmap_id id,
                         const gx_color_index* scolors,
                         const gx_strip_bitmap* textures,
                         const gx_color_index* tcolors, int x, int y, int w,
                         int h, int phase_x, int phase_y,
