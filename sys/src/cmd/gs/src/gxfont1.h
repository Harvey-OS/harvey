...
#define stem_table(size) float_array(size)
...
typedef struct gs_type1_data_procs_s {
	...
	int (*seac_data)(gs_font_type1* pfont, int ccode, gs_glyph* pglyph,
	...
	int (*push_values)(void* callback_data, const fixed* values, int count);
...
struct gs_type1_data_s {
	...
	gs_font_base* parent; /* the type 9 font, if this font is is a type 9
	                         descendent. */
...
#define public_st_gs_font_type1() /* in gstype1.c */                           \
	gs_public_st_suffix_add2_final(                                        \
	    st_gs_font_type1, gs_font_type1, "gs_font_type1",                  \
	    font_type1_enum_ptrs, font_type1_reloc_ptrs, gs_font_finalize,     \
	    st_gs_font_base, data.parent, data.proc_data)
