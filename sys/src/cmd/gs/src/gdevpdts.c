...
typedef struct pdf_text_buffer_s {
	 ...
	 *   0 < moves[0].index < moves[1].index < ...
	 *moves[count_moves-1].index
	 *	   <= count_chars
	 *   moves[*].amount != 0
	 */
...
private
const pdf_text_state_t ts_default = {
    /* State as seen by client */
...
pdf_set_text_matrix(gx_device_pdf* pdev)
{
	...
	if(matrix_is_compatible(&pts->out.matrix, &pts->in.matrix)) {
		...
		code = set_text_distance(
		    &dist, pts->start.x - pts->line_start.x,
		...
		if(dist.x == 0 && dist.y < 0) {
		 ...
		 /*
		  * See stream_to_text in gdevpdfu.c for why we need the
		  * following
		  * matrix adjustments.
		  */
		...
		pprintg6(s, "%g %g %g %g %g %g Tm\n", pts->in.matrix.xx * sx,
		         pts->in.matrix.xy * sy, pts->in.matrix.yx * sx,
		         pts->in.matrix.yy * sy, pts->start.x * sx,
		         pts->start.y * sy);
	}
...
{
	pdf_text_state_t* pts = gs_alloc_struct(
	    mem, pdf_text_state_t, &st_pdf_text_state, "pdf_text_state_alloc");
...
flush_text_buffer(gx_device_pdf* pdev)
{
	...
	if(pts->buffer.count_moves > 0) {
		...
		pdf_put_string(pdev, pts->buffer.chars,
		               pts->buffer.count_chars);
...
sync_text_state(gx_device_pdf* pdev)
{
	...
	if(pts->out.pdfont != pts->in.pdfont || pts->out.size != pts->in.size) {
		 ...
		 */
		pts->wmode =
		    (pdfont->FontType == ft_composite ? pdfont->u.type0.WMode
		                                      : 0);
                          ...
                          const pdf_text_state_values_t* ptsv)
{
	...
	if(pts->buffer.count_chars > 0) {
		...
		if(pts->in.character_spacing == ptsv->character_spacing &&
		   pts->in.pdfont == ptsv->pdfont &&
		   pts->in.size == ptsv->size &&
 ...
 */
int
pdf_append_chars(gx_device_pdf* pdev, const byte* str, uint size, floatp wx,
                 floatp wy, bool nobreak)
{
	...
	while(left)
		if(pts->buffer.count_chars == MAX_TEXT_BUFFER_CHARS ||
		   (nobreak &&
		    pts->buffer.count_chars + left > MAX_TEXT_BUFFER_CHARS)) {
			...
			if(code < 0)
				return code;
			copy =
			    min(MAX_TEXT_BUFFER_CHARS - pts->buffer.count_chars,
			        left);
			memcpy(pts->buffer.chars + pts->buffer.count_chars, p,
			       copy);
