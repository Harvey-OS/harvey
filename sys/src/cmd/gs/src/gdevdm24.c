...
dot24_print_page(gx_device_printer *pdev, FILE *prn_stream, char *init_string, int init_len)
{
	...
	while(lnum < pdev->height) {
		...
		for(ypass = 0; ypass < y_passes; ypass++) {
			...
			/* Remove trailing 0s. */
			while(out_end - 3 >= out && out_end[-1] == 0 && out_end[-2] == 0 && out_end[-3] == 0)
				out_end -= 3;
			...
			for(out_blk = outp = out; outp < out_end;) {
				...
				if(outp[0] == 0 && outp + 12 <= out_end && outp[1] == 0 && outp[2] == 0 && outp[3] == 0 && outp[4] == 0 && outp[5] == 0 && outp[6] == 0 && outp[7] == 0 && outp[8] == 0 && outp[9] == 0 && outp[10] == 0 && outp[11] == 0) {
					byte *zp = outp;
					int tpos;
					byte *newp;
					outp += 12;
					while(outp + 3 <= out_end && outp[0] == 0 && outp[1] == 0 && outp[2] == 0)
						outp += 3;
