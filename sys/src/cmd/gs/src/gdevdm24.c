...
/* Supported printers
 *  NEC P6 and similar, implemented by Andreas Schwab
...
/* Driver for NEC P6 */
private
dev_proc_print_page(necp6_print_page);
const gx_device_printer far_data gs_necp6_device =
    prn_device(prn_std_procs, "necp6", DEFAULT_WIDTH_10THS,
               DEFAULT_HEIGHT_10THS, 360, 360, 0, 0, 0.5, 0, /* margins */
               1, necp6_print_page);

/* Driver for Epson LQ850 */
/* I've tested this driver on a BJ300 with LQ850 emulation and there it produce
 * correct 360x360dpi output. */
private
dev_proc_print_page(lq850_print_page);
const gx_device_printer gs_lq850_device =
    prn_device(prn_std_procs, "lq850", DEFAULT_WIDTH_10THS,
               DEFAULT_HEIGHT_10THS, 360, 360, 0, 0, 0.5, 0, /* margins */
               1, lq850_print_page);
...
/* Forward references */
private
void dot24_output_run(byte*, int, int, FILE*);
private
void dot24_improve_bitmap(byte*, int);

/* Send the page to the printer. */
private
int
dot24_print_page(gx_device_printer* pdev, FILE* prn_stream, char* init_string,
                 int init_len)
{
	...
	byte* in =
	    (byte*)gs_malloc(pdev->memory, in_size, 1, "dot24_print_page (in)");
	uint out_size = ((pdev->width + 7) & -8) * 3;
	byte* out = (byte*)gs_malloc(pdev->memory, out_size, 1,
	                             "dot24_print_page (out)");
	...
	if(in == 0 || out == 0) {
		if(out)
			gs_free(pdev->memory, (char*)out, out_size, 1,
			        "dot24_print_page (out)");
		if(in)
			gs_free(pdev->memory, (char*)in, in_size, 1,
			        "dot24_print_page (in)");
	...
	while(lnum < pdev->height) {
		...
		if(y_high) {
			...
			for(lcnt = 1; lcnt < 24; lcnt++, inp += line_size)
				if(!gdev_prn_copy_scan_lines(
				       pdev, lnum + lcnt * 2, inp, line_size)) {
		...
		for(ypass = 0; ypass < y_passes; ypass++) {
			...
			for(; inp < in_end; inp++, out_end += 24) {
				...
				memflip8x8(inp + line_size * 8, line_size,
				           out_end + 1, 3);
				memflip8x8(inp + line_size * 16, line_size,
				           out_end + 2, 3);
			}
			/* Remove trailing 0s. */
			...
			for(out_blk = outp = out; outp < out_end;) {
				...
				/* At least 10 are needed to make tabbing worth
				 * it. */
				...
				if(outp[0] == 0 && outp + 12 <= out_end &&
				   ...
				   outp[11] == 0) {
					...
					if(newp > zp + 10) {
						...
						/* only false at beginning of
						 * line */
						if(zp > out_blk) {
							if(x_high)
								dot24_improve_bitmap(
								    out_blk,
								    (int)(zp -
								          out_blk));
							dot24_output_run(
			...
			if(outp > out_blk) {
				...
				dot24_output_run(out_blk, (int)(outp - out_blk),
				                 x_high, prn_stream);
			}
...
necp6_print_page(gx_device_printer* pdev, FILE* prn_stream)
{
	...
	return dot24_print_page(pdev, prn_stream, necp6_init_string,
	                        sizeof(necp6_init_string));
}
...
lq850_print_page(gx_device_printer* pdev, FILE* prn_stream)
{
	...
	return dot24_print_page(pdev, prn_stream, lq850_init_string,
	                        sizeof(lq850_init_string));
}
