...
ENUM_PTRS_WITH(context_state_enum_ptrs, gs_context_state_t* pcst)
{
	...
	return ENUM_USING(st_op_stack, &pcst->op_stack, sizeof(pcst->op_stack),
	                  index);
}
ENUM_PTR(0, gs_context_state_t, pgs);
case 1:
ENUM_RETURN_REF(&pcst->stdio[0]);
case 2:
ENUM_RETURN_REF(&pcst->stdio[1]);
case 3:
ENUM_RETURN_REF(&pcst->stdio[2]);
case 4:
ENUM_RETURN_REF(&pcst->userparams);
ENUM_PTRS_END
private
RELOC_PTRS_WITH(context_state_reloc_ptrs, gs_context_state_t* pcst);
...
int
context_state_alloc(gs_context_state_t** ppcst, const ref* psystem_dict,
                    const gs_dual_memory_t* dmem)
{
	...
	if(pcst == 0) {
		pcst =
		    gs_alloc_struct((gs_memory_t*)mem, gs_context_state_t,
	...
	{ /*
	   ...
	   */
		...
		if(dict_find_string(system_dict, "userparams", &puserparams) >=
		   0)
			size = dict_length(puserparams);
		else
			size = 30;
		code = dict_alloc(pcst->memory.space_local, size,
		                  &pcst->userparams);
		if(code < 0)
			goto x2;
...
context_state_load(gs_context_state_t* i_ctx_p)
{
	 ...
	 */
	{
		...
		if(dict_find_string(puserdict, "localdicts", &plocaldicts) >
		       0 &&
