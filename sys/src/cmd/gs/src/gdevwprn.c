...
/* Copyright (C) 1989, 1995, 1996, 1997, 1998, 2000 Aladdin Enterprises.  All
  rights reserved.
...
/* Forward references */
private
void near win_prn_addtool(gx_device_win_prn*, int);
private
void near win_prn_maketools(gx_device_win_prn*, HDC);
private
void near win_prn_destroytools(gx_device_win_prn*);
...
private
dev_proc_open_device(win_prn_open);
private
dev_proc_close_device(win_prn_close);
private
dev_proc_sync_output(win_prn_sync_output);
private
dev_proc_output_page(win_prn_output_page);
private
dev_proc_map_rgb_color(win_prn_map_rgb_color);
private
dev_proc_fill_rectangle(win_prn_fill_rectangle);
private
dev_proc_tile_rectangle(win_prn_tile_rectangle);
private
dev_proc_copy_mono(win_prn_copy_mono);
private
dev_proc_copy_color(win_prn_copy_color);
private
dev_proc_draw_line(win_prn_draw_line);
...
private
const gx_device_procs win_prn_procs = {
    win_prn_open, NULL, /* get_initial_matrix */
    win_prn_sync_output, win_prn_output_page, win_prn_close,
    win_prn_map_rgb_color, win_map_color_rgb, win_prn_fill_rectangle,
    win_prn_tile_rectangle, win_prn_copy_mono, win_prn_copy_color,
    win_prn_draw_line, NULL, /* get_bits */
    NULL,                    /* get_params */
    NULL,                    /* put_params */
    NULL,                    /* map_cmyk_color */
    win_get_xfont_procs};
gx_device_win_prn far_data gs_mswinprn_device = {
    std_device_std_body(gx_device_win_prn, &win_prn_procs, "mswinprn",
                        INITIAL_WIDTH,
                        INITIAL_HEIGHT, /* win_open() fills these in later */
                        INITIAL_RESOLUTION,
                        INITIAL_RESOLUTION /* win_open() fills these in later */
                        ),
    {0}, /* std_procs */
    0,   /* BitsPerPixel */
    2,   /* nColors */
...
win_prn_open(gx_device* dev)
{
	...
	if(Escape(wdev->hdcprn, STARTDOC, strlen(szAppName), szAppName, NULL) <=
	   0) {
		DeleteDC(wdev->hdcprn);
		return gs_error_limitcheck;
	}
	f = gp_open_scratch_file(gp_scratch_file_name_prefix, wdev->mfname,
	                         "wb");
	if(f == (FILE*)NULL) {
	...
	if(depth >=
	   8) { /* use 64 static colors and 166 dynamic colors from 8 planes */
		static const gx_device_color_info win_256color =
		    dci_color(8, 31, 4);

		dev->color_info = win_256color;
		wdev->nColors = 64;
	} else if(depth >= 4) {
		static const gx_device_color_info win_16ega_color =
		    dci_color(4, 2, 3);
	...
	if(wdev->hbmmono == NULL || wdev->hdcmono == NULL) {
		...
		Escape(wdev->hdcprn, ENDDOC, 0, NULL, NULL);
		DeleteDC(wdev->hdcprn);
		gs_free((char*)(wdev->limgpalette), 1,
		        sizeof(LOGPALETTE) +
		            (1 << (wdev->color_info.depth)) *
		                sizeof(PALETTEENTRY),
		        "win_prn_open");
		return win_nomemory();
	}
...
private
int
win_prn_tile_rectangle(gx_device* dev, const gx_tile_bitmap* tile, int x, int y,
                       int w, int h, gx_color_index czero, gx_color_index cone,
                       int px, int py)
{
	fit_fill(dev, x, y, w, h);
	...
	if(tile->raster == bmWidthBytes && tile->size.y <= bmHeight &&
	   (px | py) == 0 &&
	   cone !=
	       gx_no_color_index) { /* We can do this much more efficiently */
...
#define copy_tile(srcx, srcy, tx, ty, tw, th)                                  \
	BitBlt(wdev->hdcmf, tx, ty, tw, th, wdev->hdcmono, srcx, srcy,         \
	       rop_write_at_1s)
...
private
int
win_prn_copy_mono(gx_device* dev, const byte* base, int sourcex, int raster,
                  ...
                  gx_color_index zero, gx_color_index one)
{
	...
	while((endx = sourcex + w) > bmWidthBits) {
		...
		int subw = endx - lastx;
		int code = win_prn_copy_mono(
		    dev, base, lastx, raster, gx_no_bitmap_id,
		    x + lastx - sourcex, y, subw, h, zero, one);
	...
	while(h > bmHeight) {
		...
		code = win_prn_copy_mono(dev, base + h * raster, sourcex,
		                         raster, gx_no_bitmap_id, x, y + h, w,
		                         bmHeight, zero, one);
		if(code < 0)
			return code;
	}
	...
	if(id != wdev->bm_id || id == gx_no_bitmap_id) {
		wdev->bm_id = id;
		if(raster == bmWidthBytes) { /* We can do the whole thing in a
		                                single transfer! */
			SetBitmapBits(wdev->hbmmono, (DWORD)(bmWidthBytes * h),
			              (BYTE*)base);
		} else {
			for(height = h; height--;
			    ptr_line += raster,
			aptr +=
			    bmWidthBytes) { /* Pack the bits into the bitmap. */
				switch(width_bytes) {
				default:
...
private
int
win_prn_copy_color(gx_device* dev, const byte* base, int sourcex, int raster,
                   gx_bitmap_id id, int x, int y, int w, int h)
{
	fit_copy(dev, base, sourcex, raster, id, x, y, w, h);
		...
		case 8: {
			...
			for(yi = y; yi - y < h; yi++) {
				for(xi = x; xi - x < w; xi++) {
					...
					SetPixel(wdev->hdcmf, xi, yi,
					         PALETTEINDEX(color));
				}
				sptr += skip;
			}
		} break;
		case 4: { /* color device, four bits per pixel */
			...
			while(h--) { /* for each line */
				...
				if(sourcex & 1) { /* odd nibble first */
					...
					SetPixel(wdev->hdcmf, dest_x, dest_y,
					         PALETTEINDEX(color));
					dest_x++;
				}
				/* Now do full bytes */
	...
	{ /* bitmap is the same as win_copy_mono: one bit per pixel */
		win_prn_copy_mono(
		    dev, base, sourcex, raster, id, x, y, w, h,
		    (gx_color_index)0,
		    (gx_color_index)(dev->color_info.depth == 8
		                         ? 63
