...
win_prn_open(gx_device *dev)
{
	...
	m[2] /*right */ =
	    (size.x - offset.x - GetDeviceCaps(wdev->hdcprn, HORZRES)) / dev->x_pixels_per_inch;
	m[1] /*bottom */ =
	    (size.y - offset.y - GetDeviceCaps(wdev->hdcprn, VERTRES)) / dev->y_pixels_per_inch + 0.15; /* hack to add a bit more margin for deskjet printer */
	...
	if((wdev->limgpalette = win_makepalette((gx_device_win *)dev)) == (LPLOGPALETTE)NULL) {
		       ...
		       int px, int py)
{
	...
	if(tile->raster == bmWidthBytes && tile->size.y <= bmHeight &&
	   (px | py) == 0 && cone != gx_no_color_index) { /* We can do this much more efficiently */
		  ...
		  gx_color_index zero, gx_color_index one)
{
	...
	if(id != wdev->bm_id || id == gx_no_bitmap_id) {
		...
		if(raster == bmWidthBytes) { /* We can do the whole thing in a single transfer! */
			...
			for(height = h; height--;
			    ptr_line += raster, aptr += bmWidthBytes) { /* Pack the bits into the bitmap. */
				switch(width_bytes) {
				default:
