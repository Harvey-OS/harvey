...
struct stats_dstack_s {
	...
	int32_t depth[MAX_STATS_DEPTH +
	              1]; /* stack depth of lookups requiring search */
} stats_dstack;
...
dstack_dict_is_permanent(const dict_stack_t* pds, const ref* pdref)
{
	...
	if(pds->stack.extension_size == 0) { /* Only one block of d-stack. */
		...
		for(i = count - pds->min_size; i < count; ++i)
			if(ref_stack_index(&pds->stack, i)->value.pdict ==
			   pdict)
				return true;
	}
	return false;
	...
	do {
		...
		if(gs_debug_c('D')) {
			...
			dprintf3(" in 0x%lx(%u/%u)\n", (uint32_t)pdict,
			         dict_length(pdref), dict_maxlength(pdref));
		}
#endif
			...
			for(kp = kbot + dict_hash_mod(hash, size) + 2;;) {
				...
				if(r_has_type(kp, t_name)) {
					if(name_index(mem, kp) == nidx) {
						INCR_DEPTH(pdref);
						return pdict->values.value
						           .refs +
						       (kp - kbot);
					}
				} else if(r_has_type(
				              kp,
				              t_null)) { /* Empty, deleted, or
				                            wraparound. */
					/* Figure out which. */
...
#undef INCR_DEPTH
	...
	{ /* We could use the STACK_LOOP macros, but for now, */
		...
		for(; i < size; i++) {
			if(dict_find(ref_stack_index(&pds->stack, i), &key,
			             &pvalue) > 0) {
...
private
const ref_packed no_packed_keys[2] = {packed_key_deleted, packed_key_empty};
void
dstack_set_top(dict_stack_t* pds)
{
	...
	if(dict_is_packed(pdict) && r_has_attr(dict_access_ref(dsp), a_read)) {
...
dstack_gc_cleanup(dict_stack_t* pds)
{
	...
	for(dsi = pds->min_size; dsi > 0; --dsi) {
		...
		for(i = 0; i < size; ++i, ++pvalue) {
			...
			array_get(dict_mem(pdict), &pdict->keys, (int32_t)i,
			          &key);
			       ...
			       old_pvalue =
			           key.value.pname
			               ->pvalue)) { /*
			                             ...
			                             * Check to see if
			                             * no relocation is actually
			                             * needed; if so,
			                             * we can skip the entire
			                             * dictionary.
			                             */
				if(old_pvalue == pvalue) {
					if_debug1(
					    'd',
					    "[d]skipping dstack entry %d\n",
					    dsi - 1);
					break;
				}
