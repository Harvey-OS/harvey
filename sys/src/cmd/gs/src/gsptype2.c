...
ENUM_PTRS_BEGIN(pattern2_instance_enum_ptrs)
{
	if(index < st_pattern2_template_max_ptrs) {
		gs_ptr_type_t ptype = ENUM_SUPER_ELT(
		    gs_pattern2_instance_t, st_pattern2_template, template, 0);
...
ENUM_PTRS_END
private
RELOC_PTRS_BEGIN(pattern2_instance_reloc_ptrs)
{
...
private
pattern_proc_uses_base_space(gs_pattern2_uses_base_space);
private
pattern_proc_make_pattern(gs_pattern2_make_pattern);
private
pattern_proc_get_pattern(gs_pattern2_get_pattern);
private
pattern_proc_remap_color(gs_pattern2_remap_color);
private
pattern_proc_set_color(gs_pattern2_set_color);
private
const gs_pattern_type_t gs_pattern2_type = {
    2,
    {
     gs_pattern2_uses_base_space, gs_pattern2_make_pattern,
     gs_pattern2_get_pattern, gs_pattern2_remap_color, gs_pattern2_set_color,
    }};
...
private
int
gs_pattern2_make_pattern(gs_client_color* pcc, const gs_pattern_template_t* pcp,
...
private
dev_color_proc_get_dev_halftone(gx_dc_pattern2_get_dev_halftone);
private
dev_color_proc_load(gx_dc_pattern2_load);
private
dev_color_proc_fill_rectangle(gx_dc_pattern2_fill_rectangle);
private
dev_color_proc_equal(gx_dc_pattern2_equal);
private
dev_color_proc_save_dc(gx_dc_pattern2_save_dc);
/*
...
const gx_device_color_type_t gx_dc_pattern2 = {
    &st_dc_pattern2, gx_dc_pattern2_save_dc, gx_dc_pattern2_get_dev_halftone,
    gx_dc_ht_get_phase, gx_dc_pattern2_load, gx_dc_pattern2_fill_rectangle,
    gx_dc_default_fill_masked, gx_dc_pattern2_equal, gx_dc_pattern_write,
...
int
gx_dc_pattern2_fill_path(const gx_device_color* pdevc, gx_path* ppath,
                         gs_fixed_rect* rect, gx_device* dev)
{
	gs_pattern2_instance_t* pinst =
	    (gs_pattern2_instance_t*)pdevc->ccolor.pattern;

	return gs_shading_fill_path_adjusted(
	    pinst->template.Shading, ppath, rect, dev,
...
private
int
gx_dc_pattern2_fill_rectangle(const gx_device_color* pdevc, int x, int y, int w,
...
private
void
gx_dc_pattern2_save_dc(const gx_device_color* pdevc,
...
int
gx_dc_pattern2_shade_bbox_transform2fixed(const gs_rect* rect,
...
gx_dc_pattern2_get_bbox(const gx_device_color* pdevc, gs_fixed_rect* bbox)
{
	...
	code = gx_dc_pattern2_shade_bbox_transform2fixed(
	    &pinst->template.Shading->params.BBox,
	    (gs_imager_state*)pinst->saved, bbox);
	if(code < 0)
		return code;
	return 1;
}
...
bool
gx_dc_pattern2_has_background(const gx_device_color* pdevc)
{
	gs_pattern2_instance_t* pinst;
