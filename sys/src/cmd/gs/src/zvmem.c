...
private
const bool I_VALIDATE_BEFORE_SAVE = true;
private
const bool I_VALIDATE_AFTER_SAVE = true;
private
const bool I_VALIDATE_BEFORE_RESTORE = true;
private
const bool I_VALIDATE_AFTER_RESTORE = true;
...
gs_private_st_ptrs1(st_vm_save, vm_save_t, "savetype", vm_save_enum_ptrs,
                    vm_save_reloc_ptrs, gsave);
...
/* <save> restore - */
private
int restore_check_operand(os_ptr, alloc_save_t**, gs_dual_memory_t*);
private
int restore_check_stack(const ref_stack_t*, const alloc_save_t*, bool);
private
void restore_fix_stack(ref_stack_t*, const alloc_save_t*, bool);
int
...
private
int
restore_check_operand(os_ptr op, alloc_save_t** pasave, gs_dual_memory_t* idmem)
{
	vm_save_t* vmsave;
	uint32_t sid;
	alloc_save_t* asave;
	...
	do {
		...
		for(; size; stkp++, size--) {
			...
			case t_file:
				...
				break;
			case t_name:
				/* Names are special because of how they are
				 * allocated. */
				if(alloc_name_is_since_save(
				       (const gs_memory_t*)pstack->memory, stkp,
				       asave))
					return_error(e_invalidrestore);
				continue;
			case t_string:
		...
		for(; size; stkp++, size--) {
			...
			if(is_estack) {
				...
				case t_string:
					if(r_size(stkp) == 0 &&
					   alloc_is_since_save(
					       stkp->value.bytes, asave)) {
						make_empty_const_string(
						    stkp, avm_foreign);
						break;
					}
					continue;
				case t_file:
					if(alloc_is_since_save(
					       stkp->value.pfile, asave)) {
...
const op_def zvmem_op_defs[] = {{"1.forgetsave", zforgetsave},
                                {"1restore", zrestore},
                                {"0save", zsave},
                                {"0vmstatus", zvmstatus},
                                op_def_end(0)};
