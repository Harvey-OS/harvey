...
/* Forward references */
private
int cs_next_packed_value(shade_coord_stream_t*, int, uint*);
private
int cs_next_array_value(shade_coord_stream_t*, int, uint*);
private
int cs_next_packed_decoded(shade_coord_stream_t*, int, const float[2], float*);
private
int cs_next_array_decoded(shade_coord_stream_t*, int, const float[2], float*);
private
bool cs_eod(const shade_coord_stream_t* cs);
...
cs_next_packed_value(shade_coord_stream_t* cs, int num_bits, uint* pvalue)
{
	...
	if(left >= num_bits) {
		...
		*pvalue = (bits >> left) & ((1 << num_bits) - 1);
		...
		uint value =
		    bits & ((1 << left) - 1); /* all the remaining bits */
...
int
shade_next_coords(shade_coord_stream_t* cs, gs_fixed_point* ppt, int num_points)
{
	...
	for(i = 0; i < num_points; ++i) {
		...
		if((code = cs->get_decoded(cs, num_bits, decode, &x)) < 0 ||
		   ...
		   (code = gs_point_transform2fixed(cs->pctm, x, y, &ppt[i])) <
		       0)
			break;
	}
	return code;
...
shade_next_color(shade_coord_stream_t* cs, float* pc)
{
	...
	if(index == gs_color_space_index_Indexed) {
		int ncomp =
		    gs_color_space_num_components(gs_cspace_base_space(pcs));
		uint ci;
		...
		if(ci >= gs_cspace_indexed_num_entries(pcs))
			return_error(gs_error_rangecheck);
		code = gs_cspace_indexed_lookup(&pcs->params.indexed, (int)ci,
		                                &cc);
		if(code < 0)
			return code;
		for(i = 0; i < ncomp; ++i)
		...
		int i, code;
		int ncomp = (cs->params->Function != 0
		                 ? 1
		                 : gs_color_space_num_components(pcs));

		for(i = 0; i < ncomp; ++i)
			if((code = cs->get_decoded(cs, num_bits, decode + i * 2,
			                           &pc[i])) < 0)
				return code;
	}
	return 0;
...
top:
	...
	if(num_colors <= 32) {
		...
		if(components && components[0].corder.wts)
			...
			num_colors *=
			    pis->dev_ht->components[0].corder.num_levels;
	}
	...
	for(ci = 0; ci < pfs->num_components; ++ci)
		pfs->cc_max_error[ci] =
		    (ranges == 0 ? max_error : max_error * (ranges[ci].rmax -
		                                            ranges[ci].rmin));
}
