...
private
int cs_next_packed_decoded(shade_coord_stream_t *, int,
			   const float[2], float *);
private
int cs_next_array_decoded(shade_coord_stream_t *, int,
			  const float[2], float *);
private
		       ...
		       const float decode[2], float *pvalue)
{
	...
	if(code < 0)
		return code;
	*pvalue =
	    (decode == 0 ? value / max_value : decode[0] + value * (decode[1] - decode[0]) / max_value);
	return 0;
}
...
shade_next_color(shade_coord_stream_t *cs, float *pc)
{
	...
	if(index == gs_color_space_index_Indexed) {
		...
		int i, code;
		int ncomp = (cs->params->Function != 0 ? 1 : gs_color_space_num_components(pcs));
...
top:
	...
	for(ci = 0; ci < pfs->num_components; ++ci)
		pfs->cc_max_error[ci] =
		    (ranges == 0 ? max_error : max_error * (ranges[ci].rmax - ranges[ci].rmin));
}
