...
private
const dcte_scalars_t dcte_scalars_default = {
    0, 0, -1, {0, 0}, 0 /*false */, 0, 0};
private
const gs_param_item_t s_DCTE_param_items[] = {
#define dctp(key, type, memb)                                                  \
	{                                                                      \
		key, type, offset_of(dcte_scalars_t, memb)                     \
	}
...
dcte_get_samples(gs_param_list* plist, gs_param_name key, int num_colors,
                 const jpeg_compress_data* jcdp, gs_memory_t* mem, bool is_vert,
                 bool all)
{
	...
	for(i = 0; i < num_colors; ++i)
		write |=
		    (samples[i] = (is_vert ? comp_info[i].v_samp_factor
		                           : comp_info[i].h_samp_factor)) != 1;
	if(write) {
		int* data = (int*)gs_alloc_byte_array(
		    mem, num_colors, sizeof(int), "dcte_get_samples");
...
s_DCTE_get_params(gs_param_list* plist, const stream_DCT_state* ss, bool all)
{
	...
	if(!all) {
		jpeg_compress_data* jcdp_default = gs_alloc_struct_immovable(
	...
	if((code = s_DCT_get_params(plist, ss, defaults)) < 0 ||
	   (code = gs_param_write_items(plist, &params, &dcte_scalars_default,
	                                s_DCTE_param_items)) < 0 ||
	   (code = dcte_get_samples(plist, "HSamples", params.Colors, jcdp, mem,
	                            false, all)) < 0 ||
	   (code = dcte_get_samples(plist, "VSamples", params.Colors, jcdp, mem,
	                            true, all)) < 0 ||
	   (code = s_DCT_get_quantization_tables(plist, ss, defaults, true)) <
	       0 ||
                 ...
                 jpeg_compress_data* jcdp, bool is_vert)
{
	 ...
	 */
	switch((code = s_DCT_byte_params(plist, key, 0, num_colors, samples))) {
	default: /* error */
		return code;
	case 0:
		break;
...
s_DCTE_put_params(gs_param_list* plist, stream_DCT_state* pdct)
{
	...
	if(code < 0)
		return code;
	if(params.Columns <= 0 || params.Columns > 0xffff || params.Rows <= 0 ||
	   params.Rows > 0xffff || params.Colors <= 0 || params.Colors == 2 ||
	   params.Colors > 4 || params.Resync < 0 || params.Resync > 0xffff ||
	...
	case 1:
		...
		if(pdct->QFactor != 1.0) {
			code = gs_jpeg_set_linear_quality(
			    pdct,
			    (int)(min(pdct->QFactor, 100.0) * 100.0 + 0.5),
			    TRUE);
			if(code < 0)
				return code;
		}
	...
	if((code = dcte_put_samples(plist, "HSamples", params.Colors, jcdp,
	                            false)) < 0 ||
	   (code = dcte_put_samples(plist, "VSamples", params.Colors, jcdp,
	                            true)) < 0)
		return code;
