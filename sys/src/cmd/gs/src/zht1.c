...
private
int setcolorscreen_finish(i_ctx_t*);
private
int setcolorscreen_cleanup(i_ctx_t*);
private
int
zsetcolorscreen(i_ctx_t* i_ctx_p)
{
	...
	rc_alloc_struct_0(pht, gs_halftone, &st_halftone, mem, pht = 0,
	                  "setcolorscreen(halftone)");
	rc_alloc_struct_0(pdht, gx_device_halftone, &st_device_halftone, mem,
	                  pdht = 0, "setcolorscreen(device halftone)");
	if(pht == 0 || pdht == 0)
	...
	if(code >= 0) {            /* Schedule the sampling of the screens. */
		...
		for(i = 0; i < 4; i++) {
			...
			code = zscreen_enum_init(
			    i_ctx_p, &pdht->components[(i + 1) & 3].corder,
...
setcolorscreen_cleanup(i_ctx_t* i_ctx_p)
{
	...
	gs_free_object(pht->rc.memory, pht, "setcolorscreen_cleanup(halftone)");
	return 0;
}
