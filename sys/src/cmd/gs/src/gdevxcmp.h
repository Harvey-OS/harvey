...
typedef struct x11_cman_s {
	...
	struct {
		...
		/*
		 * When possible, we precompute shift values and tables that
		 * replace
		 ...
		 * represent a TrueColor or Static Gray visual), and must free
		 * it
 ...
 * dithering.
 */
#define CUBE_INDEX(r, g, b)                                                    \
	(((r)*xdev->color_info.dither_colors + (g)) *                          \
