...
main(int argc, char* argv[])
{
	...
	if(i == argc - 1) {
		fprintf(stderr,
		        "No output file name given, last argument is %s.\n",
		        argv[i]);
		exit(1);
	}
	...
	fnarg = argv[++i];
	{
		...
		if(!append)
			fprintf(out, "/*\n * File %s created automatically by "
			             "gendev.\n */\n",
			        fname);
	}
	conf.out = out;
...
typedef enum { uniq_none = 0, uniq_first, uniq_last } uniq_mode;
void
...
#define is_cat(str) !strcmp(category, str)
	...
	case 'e':
		if(is_cat("emulator"))
			sprintf(str, "emulator_(\"%s\",%d)", item,
			        strlen(item));
		break;
	case 'f':
		if(is_cat("font"))
			sprintf(str, "font_(\"0.font_%s\",%sf_%s,zf_%s)", item,
			        pconf->name_prefix, item, item);
		break;
	case 'i':
		if(is_cat("include")) {
			int len = strlen(item);
			...
			sprintf(str, "init_(%s%s_init)", pconf->name_prefix,
			        item);
		else if(is_cat("iodev"))
			sprintf(str, "io_device_(%siodev_%s)",
			        pconf->name_prefix, item);
		break;
	case 'l':
		if(is_cat("lib")) {
	...
	case 'p':
		if(is_cat("ps"))
			sprintf(str, "psfile_(\"%s.ps\",%d)", item,
			        strlen(item) + 3);
		break;
#undef is_cat
	default:
