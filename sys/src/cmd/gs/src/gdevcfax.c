...
private
dev_proc_print_page(cfax_print_page);
private
dev_proc_close_device(cfax_prn_close);
...
/* "end page" signature */
private
const gx_device_procs gdev_cfax_std_procs =
    prn_params_procs(gdev_prn_open, gdev_prn_output_page, cfax_prn_close,
                     gdev_fax_get_params, gdev_fax_put_params);

const gx_device_fax gs_cfax_device = {FAX_DEVICE_BODY(
    gx_device_fax, gdev_cfax_std_procs, "cfax", cfax_print_page)};
...
#define OUT_SIZE 1000
	...
	for(lnum = 0; lnum < pdev->height; lnum++) {
		/* Initialize read and write pointer each time, because they're
		 * getting modified */
		r.ptr = in - 1;
		r.limit = in + col_size;
		w.ptr = out - 1;
...
cfax_print_page(gx_device_printer* pdev, FILE* prn_stream)
{
	...
	code =
	    cfax_stream_print_page_width(pdev, prn_stream, &s_CFE_template,
	                                 (stream_state*)&state, state.Columns);
	return code;
}
