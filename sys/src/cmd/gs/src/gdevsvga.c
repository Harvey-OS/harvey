...
#define svga_procs(open)                                                                                                                                                                                                                                                                                                                                                                                                                                                                        \
	{                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       \
		open, NULL /*get_initial_matrix*/, NULL /*sync_output*/, NULL /*output_page*/, svga_close, svga_map_rgb_color, svga_map_color_rgb, svga_fill_rectangle, NULL /*tile_rectangle*/, svga_copy_mono, svga_copy_color, NULL /*draw_line*/, svga_get_bits, NULL /*get_params*/, svga_put_params, NULL /*map_cmyk_color*/, NULL /*get_xfont_procs*/, NULL /*get_xfont_device*/, NULL /*map_rgb_alpha_color*/, gx_page_device_get_page_device, NULL /*get_alpha_bits*/, svga_copy_alpha \
	}
	...
	for(mip = mode_table; mip->mode >= 0; mip++) {
		if(mip->width >= fb_dev->width &&
		   ...
		   bits_include(info.win_a_attributes,
				w_supported) &&
		   info.win_a_segment == regen) { /* Make sure we can both read & write. */
			...
			if(bits_include(info.win_a_attributes,
				...
				bits_include(info.win_a_attributes |
					     ...
					     w_readable | w_writable)) { /* Two superimposed windows. */
				if(!bits_include(info.win_a_attributes,
						 w_writable))
					fb_dev->wnum_write = 1;
				else
...
private
const gx_device_procs tseng_procs =
