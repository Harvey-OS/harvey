...
/* Copyright (C) 1991, 1995, 1996, 1997, 1998, 1999 Aladdin Enterprises.  All
  rights reserved.
...
private
dc_entry dynamic_colors[dc_hash_size + 1];
...
#define svga_procs(open)                                                       \
	{                                                                      \
		open, NULL /*get_initial_matrix*/, NULL /*sync_output*/,       \
		    NULL /*output_page*/, svga_close, svga_map_rgb_color,      \
		    svga_map_color_rgb, svga_fill_rectangle,                   \
		    NULL /*tile_rectangle*/, svga_copy_mono, svga_copy_color,  \
		    NULL /*draw_line*/, svga_get_bits, NULL /*get_params*/,    \
		    svga_put_params, NULL /*map_cmyk_color*/,                  \
		    NULL /*get_xfont_procs*/, NULL /*get_xfont_device*/,       \
...
#define set_pixel_ptr(ptr, fbdev, x, y, wnum)                                  \
	{                                                                      \
		...
		if((uint)(index >> 16) != fbdev->current_page) {               \
			(*fbdev->set_page)(                                    \
			    fbdev, (fbdev->current_page = index >> 16), wnum); \
		}                                                              \
...
svga_find_mode(gx_device* dev, const mode_info* mip)
{
	for(;; mip++) {
		if(mip->width >= fb_dev->width &&
		   ...
		   mip[1].mode < 0) {
			fb_dev->mode = mip;
			gx_device_adjust_resolution(dev, mip->width,
			                            mip->height, 1);
...
svga_load_colors(gx_device* dev)
{
	...
	if(fb_dev->fixed_colors)
		...
		for(ci = 0; ci < 64; ci++) {
			static const byte c2[10] = {0, 42, 0, 0,  0,
			                            0, 0,  0, 21, 63};
...
{
	fb_dev->x_pixels_per_inch = fb_dev->y_pixels_per_inch =
...
svga_map_rgb_color(gx_device* dev, const gx_color_value cv[])
{
	...
	}
	{
		...
		for(pdc = &dynamic_colors[rgb % dc_hash_size]; pdc->rgb != 0;
		    pdc++)
			if(pdc->rgb == rgb)
		...
		if(next_dc_index ==
		   num_colors) { /* No space left, report failure. */
...
int
svga_map_color_rgb(gx_device* dev, gx_color_index color, gx_color_value prgb[3])
{
	uint cval;

	outportb(0x3c7, (byte)color);
#define dacin()                                                                \
	(cval = inportb(0x3c9) >> 1,                                           \
                    ...
                    gx_color_index color)
{
	...
	case 1:
		while(--yi >= 0 && PTR_OFF(ptr) < limit)
			ptr[0] = (byte)color, ptr += raster;
		if(!++yi)
			return 0;
		break;
	case 2:
		while(--yi >= 0 && PTR_OFF(ptr) < limit)
			ptr[0] = ptr[1] = (byte)color, ptr += raster;
		if(!++yi)
			return 0;
		break;
	case 3:
		while(--yi >= 0 && PTR_OFF(ptr) < limit)
			ptr[0] = ptr[1] = ptr[2] = (byte)color, ptr += raster;
		if(!++yi)
			return 0;
		break;
	case 4:
	...
	while(--yi >= 0) {
		if(PTR_OFF(ptr) < limit) {
			...
			set_pixel_write_ptr(ptr, fb_dev, x + left,
			                    y + h - 1 - yi);
...
int
svga_copy_mono(gx_device* dev, const byte* base, int sourcex, int sraster,
               gx_bitmap_id id, int x, int y, int w, int h,
               gx_color_index czero, gx_color_index cone)
{
	uint raster = fb_dev->raster;
	uint16_t limit;
	register int wi;
	...
	for(yi = 0; yi < h; yi++) {
		...
		if(PTR_OFF(ptr) <= skip) {
			set_pixel_write_ptr(ptr, fb_dev, x, y + yi);
		          ...
		          limit) { /* We're crossing a page boundary. */
...
int
svga_copy_color(gx_device* dev, const byte* base, int sourcex, int sraster,
                gx_bitmap_id id, int x, int y, int w, int h)
{
	int xi, yi;
	int skip;
...
private
int
svga_copy_alpha(gx_device* dev, const byte* base, int sourcex, int sraster,
                gx_bitmap_id id, int x, int y, int w, int h,
                gx_color_index color, int depth)
{
	...
	fit_copy(dev, base, sourcex, sraster, id, x, y, w, h);
	shades[0] = (byte)svga_map_rgb_color(
...
private
dev_proc_open_device(vesa_open);
private
const gx_device_procs vesa_procs = svga_procs(vesa_open);
int vesa_get_mode(void);
void vesa_set_mode(int);
private
void vesa_set_page(gx_device_svga*, int, int);
gx_device_svga far_data gs_vesa_device = svga_device(
    vesa_procs, "vesa", vesa_get_mode, vesa_set_mode, vesa_set_page);
...
typedef enum { m_supported = 1, m_graphics = 0x10 } mode_attribute;
typedef enum { w_supported = 1, w_readable = 2, w_writable = 4 } win_attribute;
typedef struct {
...
vesa_get_info(int mode, vesa_info _ss* info)
{
	...
	if(regs.h.ah == 0 && regs.h.al == 0x4f)
		dlprintf8("vesa_get_info(%x): ma=%x wa=%x/%x wg=%x ws=%x "
		          "wseg=%x/%x\n",
		          mode, info->mode_attributes, info->win_a_attributes,
		          info->win_b_attributes, info->win_granularity,
		          info->win_size, info->win_a_segment,
		          info->win_b_segment);
	else
...
{ /* Select the proper video mode */
	...
	for(mip = mode_table; mip->mode >= 0; mip++) {
		if(mip->width >= fb_dev->width &&
		   ...
		   info.win_size == 64 &&
		   bits_include(info.win_a_attributes, w_supported) &&
		   info.win_a_segment ==
		       regen) { /* Make sure we can both read & write. */
			...
			if(bits_include(info.win_a_attributes,
				...
				break;
			        ...
			        bits_include(info.win_b_attributes,
			                     w_supported) &&
			        bits_include(
			            info.win_a_attributes |
			                info.win_b_attributes,
			            w_readable |
			                w_writable)) { /* Two superimposed
			                                  windows. */
				if(!bits_include(info.win_a_attributes,
				                 w_writable))
					fb_dev->wnum_write = 1;
...
{
	static const mode_info mode_table[] = {{640, 400, 0x100},
	                                       {640, 480, 0x101},
	                                       {800, 600, 0x103},
	                                       {1024, 768, 0x105},
	                                       {1280, 1024, 0x107},
...
vesa_set_page(gx_device_svga* dev, int pn, int wnum)
{
	...
	if(dev->info.vesa.bios_set_page != NULL)
		vesa_call_set_page(dev->info.vesa.bios_set_page,
		                   pn << dev->info.vesa.pn_shift, wnum);
	else
#endif
	{
		registers regs;
...
private
dev_proc_open_device(atiw_open);
private
const gx_device_procs atiw_procs = svga_procs(atiw_open);
private
int atiw_get_mode(void);
private
void atiw_set_mode(int);
private
void atiw_set_page(gx_device_svga*, int, int);
gx_device_svga far_data gs_atiw_device = svga_device(
    atiw_procs, "atiw", atiw_get_mode, atiw_set_mode, atiw_set_page);
...
{ /* Select the proper video mode */
	{
		static const mode_info mode_table[] = {{640, 400, 0x61},
		                                       {640, 480, 0x62},
		                                       {800, 600, 0x63},
		                                       {1024, 768, 0x64},
		                                       {-1, -1, -1}};
...
private
dev_proc_open_device(tvga_open);
private
const gx_device_procs tvga_procs = svga_procs(tvga_open);

/* We can use the atiw_get/set_mode procedures. */
private
void tvga_set_page(gx_device_svga*, int, int);
gx_device_svga far_data gs_tvga_device = svga_device(
    tvga_procs, "tvga", atiw_get_mode, atiw_set_mode, tvga_set_page);
...
tvga_open(gx_device* dev)
{
	...
	/* Select the proper video mode */
	{
		static const mode_info mode_table[] = {{640, 400, 0x5c},
		                                       {640, 480, 0x5d},
		                                       {800, 600, 0x5e},
		                                       {1024, 768, 0x62},
		                                       {-1, -1, -1}};
...
private
dev_proc_open_device(tseng_open);
private
const gx_device_procs tseng_procs = svga_procs(tseng_open);

/* We can use the atiw_get/set_mode procedures. */
private
void tseng_set_page(gx_device_svga*, int, int);

/* The 256-color Tseng device */
gx_device_svga far_data gs_tseng_device = svga_device(
    tseng_procs, "tseng", atiw_get_mode, atiw_set_mode, tseng_set_page);
...
tseng_open(gx_device* dev)
{
	...
	/* Select the proper video mode */
	{
		static const mode_info mode_table[] = {{640, 350, 0x2d},
		                                       {640, 480, 0x2e},
		                                       {800, 600, 0x30},
		                                       {1024, 768, 0x38},
		                                       {-1, -1, -1}};
...
private
dev_proc_open_device(cirr_open);
private
gx_device_procs cirr_procs = svga_procs(cirr_open);

/* We can use the atiw_get/set_mode procedures. */
private
void cirr_set_page(gx_device_svga*, int, int);
gx_device_svga gs_cirr_device = svga_device(cirr_procs, "cirr", atiw_get_mode,
                                            atiw_set_mode, cirr_set_page);
...
cirr_open(gx_device* dev)
{
	...
	/* Select the proper video mode */
	{
		static const mode_info mode_table[] = {{640, 400, 0x5e},
		                                       {640, 480, 0x5f},
		                                       {800, 600, 0x5c},
		                                       {1024, 768, 0x60},
		                                       {-1, -1, -1}};
...
private
dev_proc_open_device(ali_open);
private
const gx_device_procs ali_procs = svga_procs(ali_open);

/* We can use the atiw_get/set_mode procedures. */
private
void ali_set_page(gx_device_svga*, int, int);

/* The 256-color Avance Logic device */
gx_device_svga gs_ali_device =
    svga_device(ali_procs, "ali", atiw_get_mode, atiw_set_mode, ali_set_page);
...
ali_open(gx_device* dev)
{
	...
	/* Select the proper video mode */
	{
		static const mode_info mode_table[] = {{640, 400, 0x29},
		                                       {640, 480, 0x2a},
		                                       {800, 600, 0x2c},
		                                       {1024, 768, 0x31},
		                                       {-1, -1, -1}};
