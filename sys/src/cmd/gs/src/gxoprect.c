...
private
void
unpack_scanline_lt8(gx_color_index* destp, const byte* srcp, int src_offset,
                    int width, int depth)
{
	byte buff = 0;
...
private
void
pack_scanline_lt8(const gx_color_index* srcp, byte* destp, int dest_offset,
                  int width, int depth)
{
	byte buff = 0;
...
private
void
unpack_scanline_ge8(gx_color_index* destp, const byte* srcp, int src_offset,
                    int width, int depth)
{
...
private
void
pack_scanline_ge8(const gx_color_index* srcp, byte* destp, int dest_offset,
                  int width, int depth)
{
	gx_color_index buff = 0;
	int i, j, bytes_per_p = depth >> 3;
 ...
 */
int
gx_overprint_generic_fill_rectangle(gx_device* tdev, gx_color_index drawn_comps,
                                    int x, int y, int w, int h,
                                    gx_color_index color, gs_memory_t* mem)
{
	...
	void (*unpack_proc)(gx_color_index*, const byte*, int, int, int);
	void (*pack_proc)(const gx_color_index*, byte*, int, int, int);
	...
	pcolor_buff =
	    (gx_color_index*)gs_alloc_bytes(mem, w * arch_sizeof_color_index,
	...
	gb_buff =
	    gs_alloc_bytes(mem, raster, "overprint generic fill rectangle");
	if(gb_buff == 0) {
		gs_free_object(mem, pcolor_buff,
		               "overprint generic fill rectangle");
		return gs_note_error(gs_error_VMerror);
	}
	 ...
	 */
	gb_params.options = GB_COLORS_NATIVE | GB_ALPHA_NONE | GB_DEPTH_ALL |
	...
	while(h-- > 0 && code >= 0) {
		...
		for(i = 0; i < w; i++, cp++) {
			...
			int j;
			gx_color_value
			    dest_cvals[GX_DEVICE_COLOR_MAX_COMPONENTS];

			if((code = dev_proc(tdev, decode_color)(
			        tdev, *cp, dest_cvals)) < 0)
				break;
			for(j = 0, comps = drawn_comps; comps != 0;
			    ++j, comps >>= 1) {
				if((comps & 0x1) != 0)
		...
		code = dev_proc(tdev, copy_color)(
		    tdev, gb_buff, 0, raster, gs_no_bitmap_id, x, y - 1, w, 1);
	}

	gs_free_object(mem, gb_buff, "overprint generic fill rectangle");
	gs_free_object(mem, pcolor_buff, "overprint generic fill rectangle");
...
gx_overprint_sep_fill_rectangle_1(
    gx_device* tdev, gx_color_index retain_mask,      /* already swapped */
    int x, int y, int w, int h, gx_color_index color, /* already swapped */
    gs_memory_t* mem)
{
	byte* gb_buff = 0;
	gs_get_bits_params_t gb_params;
	...
	while(h-- > 0 && code >= 0) {
		...
		if(code < 0)
			break;
		bits_fill_rectangle_masked(gb_buff, 0, raster, rep_color,
		                           rep_mask, bit_w, 1);
		code = dev_proc(tdev, copy_color)(
		    tdev, gb_buff, 0, raster, gs_no_bitmap_id, x, y - 1, w, 1);
	}
