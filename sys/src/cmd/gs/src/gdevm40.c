...
/* Procedures */
declare_mem_procs(mem_true40_copy_mono, mem_true40_copy_color,
                  mem_true40_fill_rectangle);

/* The device descriptor. */
const gx_device_memory mem_true40_device = mem_full_alpha_device(
    "image40", 40, 0, mem_open, gx_default_rgb_map_rgb_color,
    gx_default_rgb_map_color_rgb, mem_true40_copy_mono, mem_true40_copy_color,
    mem_true40_fill_rectangle, gx_default_map_cmyk_color, gx_default_copy_alpha,
...
#else
#define set_color40_cache(color, a, b, c, d, e)                                \
	mdev->color40.abcd = abcd = ((bits32)(d) << 24) |                      \
...
private
int
mem_true40_fill_rectangle(gx_device* dev, int x, int y, int w, int h,
...
private
int
mem_true40_copy_mono(gx_device* dev, const byte* base, int sourcex, int sraster,
			...
			while(count >= 8) {
				...
				if(sbyte & 0xf0) {
					if(sbyte & 0x80)
						put5(pptr, a1, b1, c1, d1, e1);
					if(sbyte & 0x40)
						put5(pptr + 5, a1, b1, c1, d1,
						     e1);
					if(sbyte & 0x20)
						put5(pptr + 10, a1, b1, c1, d1,
						     e1);
					if(sbyte & 0x10)
						put5(pptr + 15, a1, b1, c1, d1,
						     e1);
				}
				if(sbyte & 0xf) {
					if(sbyte & 8)
						put5(pptr + 20, a1, b1, c1, d1,
						     e1);
					if(sbyte & 4)
						put5(pptr + 25, a1, b1, c1, d1,
						     e1);
					if(sbyte & 2)
						put5(pptr + 30, a1, b1, c1, d1,
						     e1);
					if(sbyte & 1)
						put5(pptr + 35, a1, b1, c1, d1,
						     e1);
				}
				pptr += 8 * PIXEL_SIZE;
				count -= 8;
...
private
int
mem_true40_copy_color(gx_device* dev, const byte* base, int sourcex,
...
/* Procedures */
declare_mem_procs(mem40_word_copy_mono, mem40_word_copy_color,
                  mem40_word_fill_rectangle);

/* Here is the device descriptor. */
const gx_device_memory mem_true40_word_device =
    mem_full_device("image40w", 40, 0, mem_open, gx_default_rgb_map_rgb_color,
                    gx_default_rgb_map_color_rgb, mem40_word_copy_mono,
                    mem40_word_copy_color, mem40_word_fill_rectangle,
...
private
int
mem40_word_copy_mono(gx_device* dev, const byte* base, int sourcex, int sraster,
                     gx_bitmap_id id, int x, int y, int w, int h,
                     gx_color_index zero, gx_color_index one)
{
	gx_device_memory* const mdev = (gx_device_memory*)dev;
	...
	mem_true40_copy_mono(dev, base, sourcex, sraster, id, x, y, w, h, zero,
	                     one);
...
private
int
mem40_word_copy_color(gx_device* dev, const byte* base, int sourcex,
                      int sraster, gx_bitmap_id id, int x, int y, int w, int h)
{
	gx_device_memory* const mdev = (gx_device_memory*)dev;
	byte* row;
