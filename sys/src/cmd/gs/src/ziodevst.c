...
ENUM_PTRS_END
private
RELOC_PTRS_WITH(iostatic_state_reloc_ptrs, iostatic_state* pptr);
RELOC_REF_VAR(pptr->data);
r_clear_attrs(&pptr->data, l_mark);
RELOC_PTRS_END

gs_private_st_complex_only(st_iostatic_state, iostatic_state, "iostatic_state",
                           iostatic_state_clear_marks, iostatic_state_enum_ptrs,
...
{
	iostatic_state* pstate = gs_alloc_struct(
	    mem, iostatic_state, &st_iostatic_state, "iostatic_init");
	if(!pstate)
...
const gx_io_device gs_iodev_static = {
    "%static%",
    iodev_dtype_static,
    {iostatic_init, iodev_no_open_device, iostatic_open_file, iodev_no_fopen,
     iodev_no_fclose, iodev_no_delete_file, iodev_no_rename_file,
     iodev_no_file_status, iodev_no_enumerate_files, NULL, NULL,
...
const op_def ziodevst_op_defs[] = {{"0.setup_io_static", zsetup_io_static},
                                   op_def_end(0)};
