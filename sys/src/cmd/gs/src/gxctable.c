 ...
 */
void
gx_color_interpolate_nearest(const fixed* pi, const gx_color_lookup_table* pclt,
                             frac* pv)
{
...
private
void
interpolate_accum(const fixed* pi, const gx_color_lookup_table* pclt, frac* pv,
                  fixed factor)
{
	const int* pdim = pclt->dims;
	int m = pclt->m;
	...
	if(pclt->n > 3) {
		...
		for(j = 0; j < m; ++j, ++pa0, ++pa1) {
			...
			frac v00 = v000 + (frac)arith_rshift((int32_t)fc *
			                                         (v001 - v000),
			                                     _fixed_shift);
			frac v01 = v010 + (frac)arith_rshift((int32_t)fc *
			                                         (v011 - v010),
			                                     _fixed_shift);
			frac v10 = v100 + (frac)arith_rshift((int32_t)fc *
			                                         (v101 - v100),
			                                     _fixed_shift);
			frac v11 = v110 + (frac)arith_rshift((int32_t)fc *
			                                         (v111 - v110),
			                                     _fixed_shift);

			frac v0 =
			    v00 + (frac)arith_rshift((int32_t)fb * (v01 - v00),
			                             _fixed_shift);
			frac v1 =
			    v10 + (frac)arith_rshift((int32_t)fb * (v11 - v10),
			                             _fixed_shift);

			rv = v0 + (frac)arith_rshift((int32_t)fa * (v1 - v0),
			                             _fixed_shift);
			if(factor == fixed_1)
				pv[j] = rv;
			else
				pv[j] += (frac)arith_rshift((int32_t)factor *
				                                (rv - pv[j]),
				                            _fixed_shift);
		}
	}
...
void
gx_color_interpolate_linear(const fixed* pi, const gx_color_lookup_table* pclt,
                            frac* pv)
{
