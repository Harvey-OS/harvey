...
**	the ESC-r n value
*/
static char rgb_color[2][2][2] = {{{BLACK, VIOLET}, {GREEN, CYAN}},
...
epson_map_rgb_color(gx_device* dev, const gx_color_value cv[])
{
	...
	if(gx_device_has_color(dev))
		...
		return (gx_color_index)
		           rgb_color[r >> cv_shift][g >> cv_shift][b >>
		                                                   cv_shift] ^
		       7;
	else
...
private
dev_proc_print_page(epsc_print_page);

private
gx_device_procs epson_procs =
    prn_color_procs(gdev_prn_open, gdev_prn_output_page, gdev_prn_close,
                    epson_map_rgb_color, epson_map_color_rgb);

const gx_device_printer far_data gs_epsonc_device =
    prn_device(epson_procs, "epsonc", DEFAULT_WIDTH_10THS, DEFAULT_HEIGHT_10THS,
               X_DPI, Y_DPI, 0, 0, 0.25, 0, /* margins */
               3, epsc_print_page);
...
/* Forward references */
private
void epsc_output_run(byte*, int, int, char, FILE*, int);
...
epsc_print_page(gx_device_printer* pdev, FILE* prn_stream)
{
	...
	char start_graphics = (char)((y_24pin ? graphics_modes_24
	                                      : graphics_modes_9)[x_dpi / 60]);
	...
	if(gx_device_has_color(pdev)) {
		...
		if((color_in = (byte*)gs_malloc(pdev->memory, color_in_size + 1,
		                                1, "epsc_print_page(color)")) ==
		   0) {
			gs_free(pdev->memory, (char*)in, in_size + 1, 1,
			        "epsc_print_page(in)");
			gs_free(pdev->memory, (char*)out, out_size + 1, 1,
	...
	while(lnum < pdev->height) {
		...
		lcnt = 1 + gdev_prn_copy_scan_lines(
		               pdev, lnum + 1, color_in + color_line_size,
		...
		if(lcnt < 8 * y_mult) {
			...
			if(gx_device_has_color(
			       pdev)) /* clear the work buffer */
				memset((char*)(in + lcnt * line_size), 0,
				       in_size - lcnt * line_size);
		}

		/*
		**	We need to create a normal epson scan line from our
		*color scan line
		**	We do this by setting a bit in the "in" buffer if the
		*pixel byte is set
		**	to any color.  We then search for any more pixels of
		*that color, setting
		**	"in" accordingly.  If any other color is found, we save
		*it for the next
		**	pass.  There may be up to 7 passes.
		**	In the future, we should make the passes so as to
		*maximize the
		...
		*/
		do {
			...
			if(gx_device_has_color(pdev)) {
				...
				/*	Move to the point in the scanline that
				 * has a new color */
				if(nextcolor) {
					realbuf = nextcolor;
					outbuf = nextmono;
					...
					nextcolor = NULL;
					end_next_bits = (i / color_line_size) *
					                    color_line_size +
					                whole_bits;
				} else {
					i = 0;
				...
				/*	move thru the color buffer, turning on
				*the appropriate
				**	bit in the "mono" buffer", setting
				*pointers to the next
				**	color and changing the color output of
				*the epson
				*/
				for(current_color = 0;
				    i <= color_in_size && outbuf < in + in_size;
				    outbuf++) {
					/*	Remember, line_size is rounded
					*up to next whole byte
					**	whereas color_line_size is the
					*proper length
					**	We only want to set the proper
					*bits in the last line_size byte.
					*/
					if(spare_bits && i == end_next_bits) {
						end_next_bits =
						    whole_bits + i + spare_bits;
				...
				*outbuf =
				    0; /* zero the end, for safe keeping */
				/*	Change color on the EPSON, current_color
				*must be set
				**	but lets check anyway
				*/
				if(current_color)
					fprintf(prn_stream, "\033r%d",
					        current_color ^ 7);
			}
			...
			if(y_24pin) {
				for(; inp < in_end; inp++, out_end += 24) {
					...
					gdev_prn_transpose_8x8(
					    inp + line_size * 8, line_size,
					    out_end + 1, 3);
					gdev_prn_transpose_8x8(
					    inp + line_size * 16, line_size,
					    out_end + 2, 3);
				}
				/* Remove trailing 0s. */
			...
			for(pass = first_pass; pass <= last_pass; pass++) {
				for(out_blk = outp = out;
				    outp < out_end;) { /* Skip a run of leading
				                          0s. */
					/* At least 10 are needed to make
					 * tabbing worth it. */
					/* We do everything by 3's to avoid
					 * having to make */
...
private
void
epsc_output_run(byte* data, int count, int y_mult, char start_graphics,
                FILE* prn_stream, int pass)
{
