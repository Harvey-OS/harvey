 ...
 */
gs_private_st_composite(st_color_space_CIEICC, gs_paint_color_space,
                        "gs_color_space_CIEICC", cs_CIEICC_enum_ptrs,
...
RELOC_USING(*pcs->params.icc.alt_space.type->stype, &pcs->params.icc.alt_space,
 ...
 */
private
cs_proc_num_components(gx_num_components_CIEICC);
private
cs_proc_base_space(gx_alt_space_CIEICC);
private
cs_proc_init_color(gx_init_CIEICC);
private
cs_proc_restrict_color(gx_restrict_CIEICC);
private
cs_proc_concrete_space(gx_concrete_space_CIEICC);
private
cs_proc_concretize_color(gx_concretize_CIEICC);
private
cs_proc_adjust_cspace_count(gx_adjust_cspace_CIEICC);
private
cs_proc_serialize(gx_serialize_CIEICC);

private
const gs_color_space_type gs_color_space_type_CIEICC = {
...
private
int
gx_concretize_CIEICC(const gs_client_color* pcc, const gs_color_space* pcs,
                     frac* pconc, const gs_imager_state* pis)
{
	...
	if(picc == NULL)
		return picc_params->alt_space.type->concretize_color(
		    pcc, (const gs_color_space*)&picc_params->alt_space, pconc,
		    pis);
...
gx_adjust_cspace_CIEICC(const gs_color_space* pcs, int delta)
{
	...
	rc_adjust_const(picc_params->picc_info, delta,
...
gx_load_icc_profile(gs_cie_icc* picc_info)
{
		...
		return_error(gs_error_limitcheck);
	{
		 ...
		 * intent to be specified for both the input (device ==> pcs)
		 *and
		 * output (pcs ==> device) operations. Hence, when using
		 *ICCBased
		 ...
		 * color values to XYZ or L*a*b* values such that the
		 *relationship
		 ...
		 * power levels). To achieve relative colorimetry for the
		 *output,
		 ...
		 */
		plu = picc->get_luobj(picc, icmFwd, icmDefaultIntent,
		                      0, /* PCS override */
		                      icmLuOrdNorm);
		if(plu == NULL)
			goto return_rangecheck;
...
{
	const gs_icc_params* picc_params =
	    (const gs_icc_params*)&pcs->params.icc;
 ...
 */
int
gs_cspace_build_CIEICC(gs_color_space** ppcspace, void* client_data,
                       gs_memory_t* pmem)
{
	...
	/*
	 * The gs_cie_icc_s structure is the only CIE-based color space
	 * structure
	 ...
	 * procedure).
	 */
	picc_info = gx_build_cie_space(ppcspace, &gs_color_space_type_CIEICC,
	                               &st_cie_icc, pmem);
	 ...
	 */
	picc_info->common.points.WhitePoint.u =
	    (float)0.9642; /* Profile illuminant - D50 */
...
gx_serialize_CIEICC(const gs_color_space* pcs, stream* s)
{
	...
	if(code < 0)
		return code;
	code = sputs(s, (byte*)&picc->num_components,
	             sizeof(picc->num_components), &n);
	if(code < 0)
		return code;
	...
	return sputs(s, (byte*)&picc->pcs_is_cielab,
	             sizeof(picc->pcs_is_cielab), &n);
}
