...
#define scan_line_base(dev, y) ((dev)->line_ptrs[y])
	...
	bool foreign_line_pointers; /* if true, line_ptrs are not in GC-able
	                               space */
	/*
	...
	struct _c56 {
		...
		bits32 abcd, bcde, cdef, defg, efga, fgab,
		    gabc; /* cache value */
	} color56;
...
#define public_st_device_memory() /* in gdevmem.c */                           \
	gs_public_st_composite_use_final(                                      \
	    st_device_memory, gx_device_memory, "gx_device_memory",            \
	    device_memory_enum_ptrs, device_memory_reloc_ptrs,                 \
	    gx_device_finalize)
 ...
 */
/* bits only */
uint32_t gdev_mem_bits_size(const gx_device_memory* mdev, int width,
                            int height);
/* line pointers only */
uint32_t gdev_mem_line_ptrs_size(const gx_device_memory* mdev, int width,
                                 int height);
/* bits + line pointers */
uint32_t gdev_mem_data_size(const gx_device_memory* mdev, int width,
                            int height);
 ...
 */
int gdev_mem_max_height(const gx_device_memory* dev, int width, uint32_t size,
                        bool page_uses_transparency);

/*
 * Compute the standard raster (data bytes per line) similarly.
 */
#define gdev_mem_raster(mdev) gx_device_raster((const gx_device*)(mdev), true)
...
void gs_make_mem_mono_device(gx_device_memory* mdev, gs_memory_t* mem,
                             gx_device* target);
void gs_make_mem_device(gx_device_memory* mdev, const gx_device_memory* mdproto,
                        gs_memory_t* mem, int page_device, gx_device* target);
void gs_make_mem_abuf_device(gx_device_memory* adev, gs_memory_t* mem,
                             gx_device* target,
                             const gs_log2_scale_point* pscale, int alpha_bits,
                             int mapped_x);
 ...
 * setup_height.
 */
int gdev_mem_set_line_ptrs(gx_device_memory* mdev, byte* base, int raster,
