...
#define psdf_ds_names "Average", "Bicubic", "Subsample"
	bool Encode;
	const char* Filter;
	int Resolution;
...
#define psdf_image_param_defaults(af, res, dst, f, ft)                         \
	NULL /*ACSDict*/, 0 /*false*/, af, -1, NULL /*Dict*/, 1 /*true*/, dst, \
...
#define psdf_arp_names "None", "All", "PageByPage"
	enum psdf_binding { binding_Left, binding_Right } Binding;
#define psdf_binding_names "Left", "Right"
	bool CompressPages;
...
#define psdf_ucrbg_names "Preserve", "Remove"

#define psdf_general_param_defaults(ascii)                                     \
	ascii, arp_None, binding_Left, 1 /*true*/, ri_Default, 1 /*true*/,     \
	    0 /*false*/, 500000, 0 /*false*/, 0 /*false*/, 1, 0 /*false*/,     \
	    1 /*true*/, /* Color processing parameters */                      \
	    {0}, {0}, {0}, {0}, ccs_LeaveColorUnchanged, 0 /*false*/,          \
	    0 /*false*/, tfi_Preserve, ucrbg_Remove
...
#define psdf_color_image_param_defaults                                        \
	{                                                                      \
		psdf_image_param_defaults(1 /*true*/, 72, 1.5,                 \
		                          0 /*"DCTEncode"*/,                   \
		                          0 /*&s_DCTE_template*/)              \
	}                                                                      \
	, 0 /*false*/, 1 /*true*/
...
#define psdf_mono_image_param_defaults                                         \
	{                                                                      \
		psdf_image_param_defaults(0 /*false*/, 300, 2.0,               \
...
#define psdf_cefp_names "OK", "Warning", "Error"
	bool EmbedAllFonts;
	int MaxSubsetPct;
	bool SubsetFonts;
...
#define psdf_font_param_defaults                                               \
	...
	, {0}, cefp_Warning, 1 /*true*/, 100, 1 /*true*/
...
typedef enum {
	...
	psdf_version_level1_color =
	    1100,                   /* Level 1 + colorimage + CMYK color */
	psdf_version_level2 = 2000, /* Red Book Level 2 */
	psdf_version_level2_with_TT =
	    2010, /* Adobe release 2010 with Type 42 fonts */
...
#define psdf_initial_values(version, ascii)                                    \
	vector_initial_values, version, !(ascii), true, true, false,           \
	{                                                                      \
		psdf_general_param_defaults(ascii),                            \
		    psdf_color_image_param_defaults,                           \
		    psdf_gray_image_param_defaults,                            \
		    psdf_mono_image_param_defaults, psdf_font_param_defaults   \
	}
...
#define public_st_device_psdf() /* in gdevpsdu.c */                            \
	...
	gs_public_st_basic_super_final(st_device_psdf, gx_device_psdf,         \
	                               "gx_device_psdf", device_psdf_ptrs,     \
	                               device_psdf_data, &st_device_vector, 0, \
	                               gx_device_finalize)
...
int psdf_setdash(gx_device_vector* vdev, const float* pattern, uint count,
                 floatp offset);
...
int psdf_dorect(gx_device_vector* vdev, fixed x0, fixed y0, fixed x1, fixed y1,
                gx_path_type_t type);
int psdf_beginpath(gx_device_vector* vdev, gx_path_type_t type);
int psdf_moveto(gx_device_vector* vdev, floatp x0, floatp y0, floatp x,
                floatp y, gx_path_type_t type);
int psdf_lineto(gx_device_vector* vdev, floatp x0, floatp y0, floatp x,
                floatp y, gx_path_type_t type);
int psdf_curveto(gx_device_vector* vdev, floatp x0, floatp y0, floatp x1,
                 floatp y1, floatp x2, floatp y2, floatp x3, floatp y3,
                 gx_path_type_t type);
int psdf_closepath(gx_device_vector* vdev, floatp x0, floatp y0, floatp x_start,
...
int psdf_encode_binary(psdf_binary_writer* pbw, const stream_template* template,
                       stream_state* ss);
...
int psdf_DCT_filter(gs_param_list* plist /* may be NULL */,
                    stream_state /*stream_DCTE_state*/* st, int Columns,
                    int Rows, int Colors,
...
int psdf_setup_compression_chooser(psdf_binary_writer* pbw,
                                   gx_device_psdf* pdev, int width, int height,
                                   int depth, int bits_per_sample);

/* Set up an "image to mask" filter. */
int psdf_setup_image_to_mask_filter(psdf_binary_writer* pbw,
                                    gx_device_psdf* pdev, int width, int height,
                                    int depth, int bits_per_sample,
                                    uint* MaskColor);
...
#define psdf_alloc_position_stream(ps, mem) s_alloc_position_stream(ps, mem)
#define psdf_alloc_param_printer(pplist, ppp, s, mem)                          \
	s_alloc_param_printer(pplist, ppp, s, mem)
#define psdf_free_param_printer(plist) s_free_param_printer(plist)
...
extern const psdf_set_color_commands_t psdf_set_fill_color_commands,
