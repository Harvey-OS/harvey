...
#define cmap_proc_gray(proc)                                                   \
	void proc(frac, gx_device_color*, const gs_imager_state*, gx_device*,  \
	          gs_color_select_t)
 ...
 */
#define cm_map_proc_gray(proc) void proc(gx_device* dev, frac gray, frac* out)

#define cm_map_proc_rgb(proc)                                                  \
	void proc(gx_device* dev, const gs_imager_state* pis, frac r, frac g,  \
	          frac b, frac* out)

#define cm_map_proc_cmyk(proc)                                                 \
	void proc(gx_device* dev, frac c, frac m, frac y, frac k, frac* out)
 ...
 */
const gx_color_map_procs* gx_get_cmap_procs(const gs_imager_state*,
                                            const gx_device*);
const gx_color_map_procs* gx_default_get_cmap_procs(const gs_imager_state*,
                                                    const gx_device*);
...
#define gx_remap_concrete_rgb_alpha(cr, cg, cb, ca, pdc, pis, dev, select)     \
	((pis)->cmap_procs->map_rgb_alpha)(cr, cg, cb, ca, pdc, pis, dev,      \
	                                   select)
...
#define dev_t_proc_get_color_comp_index(proc, dev_t)                           \
	int(proc)(dev_t * dev, const char* pname, int name_size,               \
	          int component_type)
...
#define dev_proc_encode_color(proc) dev_t_proc_encode_color(proc, gx_device)
...
#define dev_proc_decode_color(proc) dev_t_proc_decode_color(proc, gx_device)
...
#define unit_frac(v, ftemp)                                                    \
	(ftemp = (v),                                                          \
	 (is_fneg(ftemp) ? frac_0 : is_fge1(ftemp) ? frac_1                    \
	                                           : float2frac(ftemp)))
