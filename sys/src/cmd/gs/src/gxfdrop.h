...
/*  The structure margin_set and related structures and functions are used for
    preventing dropouts rasterizing a character with zero fill adjustment. The
   purpose
    is to paint something along thin quazi-horizontal stems,
    which are composed of multiple small segments (such as a result of
   flattenpath).
    We call it "pseudo-rasterization".
    When fill adjustment takes place, this stuff is not required and is being
   skipped.
    ...
    boudaries, being visible through window (only extremal coordinates are
   stored
    into a section)).

    The structure margin_set snaps what has been painted inside window.
    We handle 2 instances of margin_set : margin_set0 is being prepared and
   margin_set1 is
    ...
    fixme:
    Another optimization could be applied to the regular(old) trapezoid
   algorithm.
...
typedef struct section_s {
	short y0, y1; /* Fraction part of y coordinates of intersections of the
	                 margin with line x==i + bbox_left */
...
#define VD_RECT(x, y, w, h, c)                                                 \
	vd_rect(int2fixed(x), int2fixed(y), int2fixed(x + w),                  \
	        int2fixed(y + h), 1, c)
...
int process_h_lists(line_list* ll, active_line* plp, active_line* flp,
                    active_line* alp, fixed y0, fixed y1);
int margin_interior(line_list* ll, active_line* flp, active_line* alp, fixed y0,
                    fixed y1);
int start_margin_set(gx_device* dev, line_list* ll, fixed y0);
int continue_margin_common(line_list* ll, margin_set* set, active_line* flp,
                           active_line* alp, fixed y0, fixed y1);
