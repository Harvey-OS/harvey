...
#define DebugTrace(font, fmt)                                                  \
	(void)(!font->DebugPrint ? 0 : font->DebugPrint(font, fmt))
#define DebugTrace1(font, fmt, x)                                              \
	(void)(!font->DebugPrint ? 0 : font->DebugPrint(font, fmt, x))
#else
#define DebugTrace(font, fmt)
#define DebugTrace1(font, fmt, x)
#endif
...
Load_TrueType_MaxProfile(PFace face)
{
	...
	face->maxPoints =
	    MAX(maxProfile->maxCompositePoints, maxProfile->maxPoints);
	face->maxContours =
	    MAX(maxProfile->maxCompositeContours, maxProfile->maxContours);
	face->maxComponents =
	    maxProfile->maxComponentElements + maxProfile->maxComponentDepth;
...
Load_TrueType_CVT(PFace face)
{
	...
	if(face->cvtSize > 0) { /* allow fonts with a CVT table */
		face->cvt = mem->alloc_bytes(mem, face->cvtSize * sizeof(Short),
		                             "Load_TrueType_CVT");
		if(!face->cvt)
...
Load_TrueType_Programs(PFace face)
{
	...
	if(!font->t_fpgm.nPos) {
		...
		face->fontProgram = mem->alloc_bytes(mem, face->fontPgmSize,
		                                     "Load_TrueType_Programs");
	...
	if(!font->t_prep.nPos) {
		...
		face->cvtProgram = mem->alloc_bytes(mem, face->cvtPgmSize,
		                                    "Load_TrueType_Programs");
		if(!face->cvtProgram)
			return TT_Err_Out_Of_Memory;
