...
private
dev_proc_get_params(rinkj_get_params);
private
dev_proc_put_params(rinkj_put_params);
private
dev_proc_print_page(rinkj_print_page);
private
dev_proc_map_color_rgb(rinkj_map_color_rgb);
private
dev_proc_get_color_mapping_procs(get_rinkj_color_mapping_procs);
private
dev_proc_get_color_comp_index(rinkj_get_color_comp_index);
private
dev_proc_encode_color(rinkj_encode_color);
private
dev_proc_decode_color(rinkj_decode_color);
...
const gx_device_procs spot_cmyk_procs =
    device_procs(get_rinkj_color_mapping_procs);

const rinkj_device gs_rinkj_device = {
    prn_device_body_extended(
        rinkj_device, spot_cmyk_procs, "rinkj", DEFAULT_WIDTH_10THS,
...
rinkj_get_params(gx_device* pdev, gs_param_list* plist)
{
	...
	pos.size = strlen(rdev->profile_out_fn), pos.persistent = false;
	code = param_write_string(plist, "ProfileOut", &pos);
	if(code < 0)
		return code;

	sfs.data = (const byte*)rdev->setup_fn,
	sfs.size = strlen(rdev->setup_fn), sfs.persistent = false;
...
private
int
rinkj_get_color_comp_index(const gx_device* dev, const char* pname,
                           int name_size, int src_index)
{
	/* TO_DO_DEVICEN  This routine needs to include the effects of the
	 * SeparationOrder array */
	const fixed_colorant_names_list* list =
	    ((const rinkj_device*)dev)->std_colorant_names;
	...
	/* Check if the component is in the separation names list. */
	{
		const gs_separation_names* separations =
		    &((const rinkj_device*)dev)->separation_names;
...
static double
rinkj_graph_lookup(const double* graph_x, const double* graph_y, int n_graph,
                   double x)
{
	int i;
	...
	return graph_y[i] +
	       (x - graph_x[i]) * (graph_y[i + 1] - graph_y[i]) /
...
rinkj_add_lut(rinkj_device* rdev, rinkj_lutset* lutset, char plane, FILE* f)
{
	...
	if(sscanf(linebuf, "%d", &n_graph) != 1)
		return -1;
	chain = (rinkj_lutchain*)gs_alloc_bytes(
	    rdev->memory, sizeof(rinkj_lutchain), "rinkj_add_lut");
	chain->next = NULL;
	chain->n_graph = n_graph;
	chain->graph_x = (double*)gs_alloc_bytes(
	    rdev->memory, sizeof(double) * n_graph, "rinkj_add_lut");
	chain->graph_y = (double*)gs_alloc_bytes(
	    rdev->memory, sizeof(double) * n_graph, "rinkj_add_lut");
	for(i = 0; i < n_graph; i++) {
		double x, y;
...
static int
rinkj_apply_luts(rinkj_device* rdev, RinkjDevice* cmyk_dev,
                 const rinkj_lutset* lutset)
{
	int plane_ix;
	double lut[256];
	...
	for(plane_ix = 0; plane_ix < 7; plane_ix++) {
		...
		for(i = 0; i < 256; i++) {
			...
			for(chain = lutset->lut[plane_ix]; chain;
			    chain = chain->next) {
				g = rinkj_graph_lookup(chain->graph_x,
				                       chain->graph_y,
				                       chain->n_graph, g);
			}
			lut[i] = g;
...
static int
rinkj_set_luts(rinkj_device* rdev, RinkjDevice* printer_dev,
               RinkjDevice* cmyk_dev, const char* config_fn,
               const RinkjDeviceParams* params)
{
...
rinkj_color_hash(bits32 color)
{
	...
	return (color ^ (color >> 10) ^ (color >> 20)) &
	       (RINKJ_CCACHE_SIZE - 1);
}
...
rinkj_write_image_data(gx_device_printer* pdev, RinkjDevice* cmyk_dev)
{
	...
	for(i = 0; i < n_planes_out; i++)
		plane_data[i] =
		    gs_alloc_bytes(pdev->memory, xsb, "rinkj_write_image_data");

	if(luo != NULL) {
		cache = (rinkj_color_cache_entry*)gs_alloc_bytes(
		    pdev->memory,
		    RINKJ_CCACHE_SIZE * sizeof(rinkj_color_cache_entry),
		    "rinkj_write_image_data");
		if(cache == NULL)
	...
	for(i = 0; i < n_planes_in; i++)
		gs_free_object(pdev->memory, plane_data[i],
		               "rinkj_write_image_data");
