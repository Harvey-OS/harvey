...
private
const gx_device_procs spot_cmyk_procs = device_procs(get_rinkj_color_mapping_procs);
...
rinkj_graph_lookup(const double *graph_x, const double *graph_y, int n_graph, double x)
{
	...
	return graph_y[i] + (x - graph_x[i]) * (graph_y[i + 1] - graph_y[i]) / (graph_x[i + 1] - graph_x[i]);
}
