...
TT_Error
Set_CodeRange(PExecution_Context exec, Int range, void* base, Int length)
{
	if(range < 1 || range > 3)
		return TT_Err_Bad_Argument;
...
#define ALLOC_ARRAY(ptr, old_count, count, type)                               \
	(old_count >= count ? 0                                                \
	                    : !(free_aux(mem, ptr),                            \
	                        ptr = mem->alloc_bytes(                        \
	                            mem, (count) * sizeof(type), "ttobjs.c")))
...
Context_Destroy(void* _context)
{
	...
	if(!exec->current_face) {
		/* This may happen while closing a high level device, when
		   allocator runs out of memory.
		   A test case is 01_001.pdf with pdfwrite and a small
		   vmthreshold.
		*/
...
{
	/* Note : The function name is a kind of misleading due to our
	 * improvement.
	 * Now it adjusts (enhances) the context for the specified face.
	 * We keep the old Free Type's name for easier localization of our
	 * changes.
	...
	if(ALLOC_ARRAY(exec->callStack, exec->callSize, callSize,
	               TCallRecord) ||
	   ...
	   ALLOC_ARRAY(exec->twilight.org_x, exec->twilight.n_points,
	               n_twilight, TT_F26Dot6) ||
	   ALLOC_ARRAY(exec->twilight.org_y, exec->twilight.n_points,
	               n_twilight, TT_F26Dot6) ||
	   ALLOC_ARRAY(exec->twilight.cur_x, exec->twilight.n_points,
	               n_twilight, TT_F26Dot6) ||
	   ALLOC_ARRAY(exec->twilight.cur_y, exec->twilight.n_points,
	               n_twilight, TT_F26Dot6) ||
	   ALLOC_ARRAY(exec->twilight.touch, exec->twilight.n_points,
	               n_twilight, Byte) ||
	   /* reserve twilight zone */

	   ALLOC_ARRAY(exec->pts.contours, exec->n_contours, face->maxContours,
	               UShort)
...
Fail_Memory:
	/* Context_Destroy( exec ); Don't release buffers because the context is
	 * shared. */
	return TT_Err_Out_Of_Memory;
}
...
TT_Error
Context_Load(PExecution_Context exec, PInstance ins)
{
	Int i;
...
TT_Error
Context_Save(PExecution_Context exec, PInstance ins)
{
	Int i;
...
TT_Error
Context_Run(PExecution_Context exec, Bool debug)
{
	TT_Error error;
...
const TGraphicsState Default_GraphicsState = {0,
                                              0,
                                              0,
                                              {0x4000, 0},
                                              {0x4000, 0},
...
TT_Error
Instance_Create(void* _instance, void* _face)
{
	PInstance ins = (PInstance)_instance;
	PFace face = (PFace)_face;
	ttfMemory* mem = face->font->tti->ttf_memory;
...
Instance_Init(PInstance ins)
{
	...
	Set_CodeRange(exec, TT_CodeRange_Font, face->fontProgram,
	              face->fontPgmSize);
...
TT_Error
Instance_Reset(PInstance ins, Bool debug)
{
	TT_Error error;
	Int i;
	PFace face = ins->face;
	...
	if(ins->metrics.x_ppem >= ins->metrics.y_ppem) {
		...
		ins->metrics.y_ratio = MulDiv_Round(
		    ins->metrics.y_ppem, 0x10000, ins->metrics.x_ppem);
	} else {
		ins->metrics.scale1 = ins->metrics.y_scale1;
		ins->metrics.scale2 = ins->metrics.y_scale2;
		ins->metrics.ppem = ins->metrics.y_ppem;
		ins->metrics.x_ratio = MulDiv_Round(
		    ins->metrics.x_ppem, 0x10000, ins->metrics.y_ppem);
	...
	for(i = 0; i < ins->cvtSize; i++)
		ins->cvt[i] = MulDiv_Round(face->cvt[i], ins->metrics.scale1,
		                           ins->metrics.scale2);
	...
	Set_CodeRange(exec, TT_CodeRange_Cvt, face->cvtProgram,
	              face->cvtPgmSize);
...
#define LOAD_(table) (error = Load_TrueType_##table(face))
