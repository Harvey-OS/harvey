...
/* Device procedures */
private
dev_proc_open_device(bbox_open_device);
private
dev_proc_close_device(bbox_close_device);
private
dev_proc_output_page(bbox_output_page);
private
dev_proc_fill_rectangle(bbox_fill_rectangle);
private
dev_proc_copy_mono(bbox_copy_mono);
private
dev_proc_copy_color(bbox_copy_color);
private
dev_proc_get_params(bbox_get_params);
private
dev_proc_put_params(bbox_put_params);
private
dev_proc_copy_alpha(bbox_copy_alpha);
private
dev_proc_fill_path(bbox_fill_path);
private
dev_proc_stroke_path(bbox_stroke_path);
private
dev_proc_fill_mask(bbox_fill_mask);
private
dev_proc_fill_trapezoid(bbox_fill_trapezoid);
private
dev_proc_fill_parallelogram(bbox_fill_parallelogram);
private
dev_proc_fill_triangle(bbox_fill_triangle);
private
dev_proc_draw_thin_line(bbox_draw_thin_line);
private
dev_proc_strip_tile_rectangle(bbox_strip_tile_rectangle);
private
dev_proc_strip_copy_rop(bbox_strip_copy_rop);
private
dev_proc_begin_typed_image(bbox_begin_typed_image);
private
dev_proc_create_compositor(bbox_create_compositor);
private
dev_proc_text_begin(bbox_text_begin);
...
#define MAX_RESOLUTION 4000
    gx_device_bbox gs_bbox_device = {
         ...
         */
        std_device_dci_body(gx_device_bbox, 0, "bbox", MAX_COORD, MAX_COORD,
                            MAX_RESOLUTION, MAX_RESOLUTION, 1, 8, 255, 0, 256,
                            1),
        {
         bbox_open_device, gx_upright_get_initial_matrix,
         NULL, /* sync_output */
         bbox_output_page, bbox_close_device, gx_default_gray_map_rgb_color,
         gx_default_gray_map_color_rgb, bbox_fill_rectangle,
         NULL,                                  /* tile_rectangle */
         bbox_copy_mono, bbox_copy_color, NULL, /* draw_line */
         NULL,                                  /* get_bits */
         bbox_get_params, bbox_put_params, gx_default_map_cmyk_color,
         NULL, /* get_xfont_procs */
         NULL, /* get_xfont_device */
         gx_default_map_rgb_alpha_color, gx_page_device_get_page_device,
         NULL,                  /* get_alpha_bits */
         bbox_copy_alpha, NULL, /* get_band */
         NULL,                  /* copy_rop */
         bbox_fill_path, bbox_stroke_path, bbox_fill_mask, bbox_fill_trapezoid,
         bbox_fill_parallelogram, bbox_fill_triangle, bbox_draw_thin_line,
         gx_default_begin_image, NULL, /* image_data */
         NULL,                         /* end_image */
         bbox_strip_tile_rectangle, bbox_strip_copy_rop,
         NULL,                         /* get_clipping_box */
         bbox_begin_typed_image, NULL, /* get_bits_rectangle */
         gx_default_map_color_rgb_alpha, bbox_create_compositor,
         NULL,                  /* get_hardware_params */
         bbox_text_begin, NULL, /* finish_copydevice */
...
#define BBOX_INIT_BOX(bdev) bdev->box_procs.init_box(bdev->box_proc_data)
...
private
const gx_device_bbox_procs_t box_procs_default = {
...
bbox_close_device(gx_device* dev)
{
	...
	if(bdev->box_procs.init_box != box_procs_default.init_box) {
		 ...
		 */
		int code =
		    (bdev->forward_open_close ? gs_closedevice(tdev) : 0);

		gs_free_object(dev->memory, dev,
		               "bbox_close_device(composite)");
		return code;
	} else {
		return (tdev && bdev->forward_open_close ? gs_closedevice(tdev)
		                                         : 0);
	}
}
...
gx_device_bbox_init(gx_device_bbox* dev, gx_device* target, gs_memory_t* mem)
{
	...
	if(target) {
		...
		set_dev_proc(dev, map_rgb_alpha_color,
		...
		set_dev_proc(dev, fill_rectangle_hl_color,
		             gx_forward_fill_rectangle_hl_color);
		set_dev_proc(dev, include_color_space,
...
bbox_open_device(gx_device* dev)
{
	...
	/* gx_forward_open_device doesn't exist */
	{
		gx_device* tdev = bdev->target;
		int code =
		    (tdev && bdev->forward_open_close ? gs_opendevice(tdev)
		                                      : 0);
                    ...
                    gx_color_index color)
{
	...
	int code = (tdev == 0 ? 0 : dev_proc(tdev, fill_rectangle)(
	                                tdev, x, y, w, h, color));
...
private
int
bbox_copy_mono(gx_device* dev, const byte* data, int dx, int raster,
               ...
               gx_color_index one)
{
	...
	gx_device* tdev = bdev->target;
	int code = (tdev == 0 ? 0 : dev_proc(tdev, copy_mono)(tdev, data, dx,
	                                                      raster, id, x, y,
	                                                      w, h, zero, one));
...
private
int
bbox_copy_color(gx_device* dev, const byte* data, int dx, int raster,
                gx_bitmap_id id, int x, int y, int w, int h)
{
	...
	gx_device* tdev = bdev->target;
	int code =
	    (tdev == 0 ? 0 : dev_proc(tdev, copy_color)(tdev, data, dx, raster,
	                                                id, x, y, w, h));
...
private
int
bbox_copy_alpha(gx_device* dev, const byte* data, int data_x, int raster,
                gx_bitmap_id id, int x, int y, int w, int h,
                gx_color_index color, int depth)
{
	...
	gx_device* tdev = bdev->target;
	int code = (tdev == 0 ? 0 : dev_proc(tdev, copy_alpha)(
	                                tdev, data, data_x, raster, id, x, y, w,
	                                h, color, depth));
...
private
int
bbox_strip_tile_rectangle(gx_device* dev, const gx_strip_bitmap* tiles, int x,
                          int y, int w, int h, gx_color_index color0,
                          gx_color_index color1, int px, int py)
{
	gx_device_bbox* const bdev = (gx_device_bbox*)dev;
	/* Skip the call if there is no target. */
	gx_device* tdev = bdev->target;
	int code = (tdev == 0 ? 0 : dev_proc(tdev, strip_tile_rectangle)(
	                                tdev, tiles, x, y, w, h, color0, color1,
	                                px, py));
...
private
int
bbox_strip_copy_rop(gx_device* dev, const byte* sdata, int sourcex,
                    uint sraster, gx_bitmap_id id,
                    const gx_color_index* scolors,
                    const gx_strip_bitmap* textures,
                    const gx_color_index* tcolors, int x, int y, int w, int h,
                    int phase_x, int phase_y, gs_logical_operation_t lop)
{
	gx_device_bbox* const bdev = (gx_device_bbox*)dev;
	/* gx_forward_strip_copy_rop doesn't exist */
	gx_device* tdev = bdev->target;
	int code = (tdev == 0 ? 0 : dev_proc(tdev, strip_copy_rop)(
	                                tdev, sdata, sourcex, sraster, id,
	                                scolors, textures, tcolors, x, y, w, h,
	                                phase_x, phase_y, lop));
...
bbox_put_params(gx_device* dev, gs_param_list* plist)
{
	...
	switch(code = param_read_bool(plist, (param_name = "WhiteIsOpaque"),
	                              &white_is_opaque)) {
	default:
		ecode = code;
		param_signal_error(plist, param_name, ecode);
	...
	if(code >= 0) {
		if(bba.data != 0) {
			BBOX_INIT_BOX(bdev);
			BBOX_ADD_RECT(bdev, float2fixed(bba.data[0]),
			              float2fixed(bba.data[1]),
			              float2fixed(bba.data[2]),
			              float2fixed(bba.data[3]));
		}
		bdev->white_is_opaque = white_is_opaque;
	}
...
{
	return fixed_mult_quo(edge->end.x - edge->start.x, y - edge->start.y,
	                      edge->end.y - edge->start.y) +
	       edge->start.x;
}
private
int
bbox_fill_trapezoid(gx_device* dev, const gs_fixed_edge* left,
                    ...
                    gs_logical_operation_t lop)
{
	...
	/* Skip the call if there is no target. */
	gx_device* tdev = bdev->target;
	int code = (tdev == 0 ? 0 : dev_proc(tdev, fill_trapezoid)(
	                                tdev, left, right, ybot, ytop,
	                                swap_axes, pdevc, lop));

	if(!GX_DC_IS_TRANSPARENT(pdevc, bdev)) {
		fixed x0l = (left->start.y == ybot ? left->start.x
		                                   : edge_x_at_y(left, ybot));
		fixed x1l = (left->end.y == ytop ? left->end.x
		                                 : edge_x_at_y(left, ytop));
		fixed x0r = (right->start.y == ybot ? right->start.x
		                                    : edge_x_at_y(right, ybot));
		fixed x1r = (right->end.y == ytop ? right->end.x
...
private
int
bbox_fill_parallelogram(gx_device* dev, fixed px, fixed py, fixed ax, fixed ay,
                        ...
                        gs_logical_operation_t lop)
{
	...
	gx_device* tdev = bdev->target;
	int code =
	    (tdev == 0 ? 0 : dev_proc(tdev, fill_parallelogram)(
...
private
int
bbox_fill_triangle(gx_device* dev, fixed px, fixed py, fixed ax, fixed ay,
                   fixed bx, fixed by, const gx_device_color* pdevc,
                   gs_logical_operation_t lop)
{
	gx_device_bbox* const bdev = (gx_device_bbox*)dev;
	...
	gx_device* tdev = bdev->target;
	int code =
	    (tdev == 0 ? 0 : dev_proc(tdev, fill_triangle)(tdev, px, py, ax, ay,
	                                                   bx, by, pdevc, lop));
...
private
int
bbox_draw_thin_line(gx_device* dev, fixed fx0, fixed fy0, fixed fx1, fixed fy1,
                    const gx_device_color* pdevc, gs_logical_operation_t lop)
{
	gx_device_bbox* const bdev = (gx_device_bbox*)dev;
	/* Skip the call if there is no target. */
	gx_device* tdev = bdev->target;
	int code = (tdev == 0 ? 0 : dev_proc(tdev, draw_thin_line)(
               ...
               const gx_clip_path* pcpath)
{
	...
	if(!GX_DC_IS_TRANSPARENT(pdevc, bdev) && !gx_path_is_void(ppath)) {
		...
		/*
		 * If the path lies within the already accumulated box, just
		 * draw
		 * on the target.
		 */
		if(BBOX_IN_RECT(bdev, &ibox))
			return fill_path(tdev, pis, ppath, params, pdevc,
			                 pcpath);
		/*
		...
		if(tdev != 0 && fill_path == gx_default_fill_path)
			return fill_path(dev, pis, ppath, params, pdevc,
			                 pcpath);
		...
		if(pcpath != NULL &&
		   !gx_cpath_includes_rectangle(pcpath, ibox.p.x, ibox.p.y,
			...
			set_nonclient_dev_color(
			    &devc,
			    bdev->black); /* any non-white color will do */
			bdev->target = NULL;
			code = gx_default_fill_path(dev, pis, ppath, params,
			                            &devc, pcpath);
			bdev->target = tdev;
		} else { /* Just use the path bounding box. */
			BBOX_ADD_RECT(bdev, ibox.p.x, ibox.p.y, ibox.q.x,
			              ibox.q.y);
		}
		return code;
	} else
...
bbox_stroke_path(gx_device* dev, const gs_imager_state* pis, gx_path* ppath,
                 const gx_stroke_params* params, const gx_drawing_color* pdevc,
                 const gx_clip_path* pcpath)
{
	gx_device_bbox* const bdev = (gx_device_bbox*)dev;
	gx_device* tdev = bdev->target;
	/* Skip the call if there is no target. */
	int code =
	    (tdev == 0 ? 0 : dev_proc(tdev, stroke_path)(
	...
	if(!GX_DC_IS_TRANSPARENT(pdevc, bdev)) {
		...
		if(gx_stroke_path_expansion(pis, ppath, &expand) == 0 &&
		   gx_path_bbox(ppath, &ibox) >= 0) {
			...
			/*
			 * The result is not exact.  Compute an exact result
			 * using
			 * strokepath.
			 */
			gx_path* spath =
			    gx_path_alloc(pis->memory, "bbox_stroke_path");
			int code = 0;

			if(spath)
				code = gx_imager_stroke_add(ppath, spath, dev,
				                            pis);
			else
				code = -1;
			if(code >= 0)
		...
		if(pcpath != NULL &&
		   !gx_cpath_includes_rectangle(pcpath, ibox.p.x, ibox.p.y,
			...
			set_nonclient_dev_color(
			    ...
			    bdev->black); /* any non-white color will do */
			bdev->target = NULL;
			gx_default_stroke_path(dev, pis, ppath, params, &devc,
			                       pcpath);
			bdev->target = tdev;
		} else {
...
private
int
bbox_fill_mask(gx_device* dev, const byte* data, int dx, int raster,
               gx_bitmap_id id, int x, int y, int w, int h,
               ...
               gs_logical_operation_t lop, const gx_clip_path* pcpath)
{
	...
	gx_device* tdev = bdev->target;
	/* Skip the call if there is no target. */
	int code = (tdev == 0 ? 0 : dev_proc(tdev, fill_mask)(
...
gs_private_st_suffix_add2(st_bbox_image_enum, bbox_image_enum,
                          "bbox_image_enum", bbox_image_enum_enum_ptrs,
                          bbox_image_enum_reloc_ptrs, st_gx_image_enum_common,
                          pcpath, target_info);

private
image_enum_proc_plane_data(bbox_image_plane_data);
private
image_enum_proc_end_image(bbox_image_end_image);
private
image_enum_proc_flush(bbox_image_flush);
private
image_enum_proc_planes_wanted(bbox_image_planes_wanted);
private
const gx_image_enum_procs_t bbox_image_enum_procs = {
    bbox_image_plane_data, bbox_image_end_image, bbox_image_flush,
    bbox_image_planes_wanted};
...
private
int
bbox_begin_typed_image(gx_device* dev, const gs_imager_state* pis,
                       ...
                       gx_image_enum_common_t** pinfo)
{
	 ...
	 */
	{
		...
		code =
		    (*begin_typed_image)(tdev, pis, pmat, pic, prect, pdcolor,
		                         pcpath, memory, &pbe->target_info);
		if(code) {
			bbox_image_end_image((gx_image_enum_common_t*)pbe,
			                     false);
			return code;
		}
		/*
		 * We fill in num_planes and plane_depths later.  format is
		 * irrelevant.  NOTE: we assume that if begin_typed_image
		 * returned
		 * 0, the image is a data image.
		 */
		code = gx_image_enum_common_init(
		    (gx_image_enum_common_t*)pbe, (const gs_data_image_t*)pic,
                      ...
                      int* rows_used)
{
	...
	if(pcpath != NULL &&
	   !gx_cpath_includes_rectangle(pcpath, ibox.p.x, ibox.p.y, ibox.q.x,
	                                ibox.q.y)) {
		...
		set_nonclient_dev_color(
		    &devc, bdev->black); /* any non-white color will do */
		bdev->target = NULL;
		gx_default_fill_triangle((gx_device*)&cdev, x0, y0,
		                         float2fixed(corners[1].x) - x0,
		                         float2fixed(corners[1].y) - y0, bx2,
		                         by2, &devc, lop_default);
		gx_default_fill_triangle((gx_device*)&cdev, x0, y0,
		                         float2fixed(corners[3].x) - x0,
		                         float2fixed(corners[3].y) - y0, bx2,
		                         by2, &devc, lop_default);
		bdev->target = tdev;
	} else {
...
private
const gx_device_bbox_procs_t box_procs_forward = {
    bbox_forward_init_box, bbox_forward_get_box, bbox_forward_add_rect,
    bbox_forward_in_rect};

private
int
bbox_create_compositor(gx_device* dev, gx_device** pcdev,
                       const gs_composite_t* pcte, gs_imager_state* pis,
                       gs_memory_t* memory)
{
	gx_device_bbox* const bdev = (gx_device_bbox*)dev;
	 ...
	 */
	{
		...
		/* If the target did not create a new compositor then we are
		 * done. */
