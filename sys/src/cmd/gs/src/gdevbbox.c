		    ...
		    gx_color_index color)
{
	...
	int code =
	    (tdev == 0 ? 0 : dev_proc(tdev, fill_rectangle)(tdev, x, y, w, h, color));
	       ...
	       gx_color_index zero, gx_color_index one)
{
	...
	gx_device *tdev = bdev->target;
	int code =
	    (tdev == 0 ? 0 : dev_proc(tdev, copy_mono)(tdev, data, dx, raster, id, x, y, w, h, zero, one));
		...
		int dx, int raster, gx_bitmap_id id, int x, int y, int w, int h)
{
	...
	gx_device *tdev = bdev->target;
	int code =
	    (tdev == 0 ? 0 : dev_proc(tdev, copy_color)(tdev, data, dx, raster, id, x, y, w, h));
		...
		gx_color_index color, int depth)
{
	...
	gx_device *tdev = bdev->target;
	int code =
	    (tdev == 0 ? 0 : dev_proc(tdev, copy_alpha)(tdev, data, data_x, raster, id, x, y, w, h, color, depth));
			  ...
			  int px, int py)
{
	...
	gx_device *tdev = bdev->target;
	int code =
	    (tdev == 0 ? 0 : dev_proc(tdev, strip_tile_rectangle)(tdev, tiles, x, y, w, h, color0, color1, px, py));
		    ...
		    int phase_x, int phase_y, gs_logical_operation_t lop)
{
	...
	gx_device *tdev = bdev->target;
	int code =
	    (tdev == 0 ? 0 : dev_proc(tdev, strip_copy_rop)(tdev, sdata, sourcex, sraster, id, scolors,
		    ...
		    const gx_device_color *pdevc, gs_logical_operation_t lop)
{
	...
	gx_device *tdev = bdev->target;
	int code =
	    (tdev == 0 ? 0 : dev_proc(tdev, fill_trapezoid)(tdev, left, right, ybot, ytop, swap_axes, pdevc, lop));

	if(!GX_DC_IS_TRANSPARENT(pdevc, bdev)) {
		fixed x0l =
		    (left->start.y == ybot ? left->start.x : edge_x_at_y(left, ybot));
		fixed x1l =
		    (left->end.y == ytop ? left->end.x : edge_x_at_y(left, ytop));
		fixed x0r =
		    (right->start.y == ybot ? right->start.x : edge_x_at_y(right, ybot));
		fixed x1r =
		    (right->end.y == ytop ? right->end.x : edge_x_at_y(right, ytop));
			...
			gs_logical_operation_t lop)
{
	...
	gx_device *tdev = bdev->target;
	int code =
	    (tdev == 0 ? 0 : dev_proc(tdev, fill_parallelogram)(tdev, px, py, ax, ay, bx, by, pdevc, lop));
		   ...
		   const gx_device_color *pdevc, gs_logical_operation_t lop)
{
	...
	gx_device *tdev = bdev->target;
	int code =
	    (tdev == 0 ? 0 : dev_proc(tdev, fill_triangle)(tdev, px, py, ax, ay, bx, by, pdevc, lop));
		    ...
		    const gx_device_color *pdevc, gs_logical_operation_t lop)
{
	...
	gx_device *tdev = bdev->target;
	int code =
	    (tdev == 0 ? 0 : dev_proc(tdev, draw_thin_line)(tdev, fx0, fy0, fx1, fy0, pdevc, lop));
	       ...
	       const gx_clip_path *pcpath)
{
	...
	dev_proc_fill_path((*fill_path)) =
	    (tdev == 0 ? dev_proc(&gs_null_device, fill_path) : dev_proc(tdev, fill_path));
	int code;
		 ...
		 const gx_drawing_color *pdevc, const gx_clip_path *pcpath)
{
	...
	/* Skip the call if there is no target. */
	int code =
	    (tdev == 0 ? 0 : dev_proc(tdev, stroke_path)(tdev, pis, ppath, params, pdevc, pcpath));
	       ...
	       gs_logical_operation_t lop, const gx_clip_path *pcpath)
{
	...
	/* Skip the call if there is no target. */
	int code =
	    (tdev == 0 ? 0 : dev_proc(tdev, fill_mask)(tdev, data, dx, raster, id, x, y, w, h,
		       ...
		       gs_memory_t *memory, gx_image_enum_common_t **pinfo)
{
     ...
     */
	{
		...
		code = (*begin_typed_image)(tdev, pis, pmat, pic, prect, pdcolor, pcpath, memory,
					    &pbe->target_info);
		if(code) {
		       ...
		       gs_imager_state *pis, gs_memory_t *memory)
{
     ...
     */
	{
		...
		int code = (*dev_proc(target, create_compositor))(target, &cdev, pcte, pis, memory);
