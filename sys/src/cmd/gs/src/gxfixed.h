...
#ifdef USE_FPU
#define USE_FPU_FIXED                                                          \
	(USE_FPU < 0 && arch_floats_are_IEEE && arch_sizeof_long == 4)
#else
...
#define FINISH_FMUL2FIXED_VARS(vr, dtemp) DO_NOTHING
...
#define CHECK_DFMUL2FIXED_VARS(vr, vda, vfb, dtemp)                            \
     set_dfmul2fixed_(&vr, *(const uint32_t *)&vda, *(const long *)&vfb,
                      ((const long *)&vda)[1])
#endif
#define FINISH_DFMUL2FIXED_VARS(vr, dtemp) DO_NOTHING

#else /* don't bother */

#undef NEED_SET_FMUL2FIXED
#define CHECK_FMUL2FIXED_VARS(vr, vfa, vfb, dtemp)                             \
	(dtemp = (vfa) * (vfb), (f_fits_in_bits(dtemp, fixed_int_bits)         \
	                             ? 0                                       \
	                             : gs_note_error(gs_error_limitcheck)))
#define FINISH_FMUL2FIXED_VARS(vr, dtemp) vr = float2fixed(dtemp)
#define CHECK_DFMUL2FIXED_VARS(vr, vda, vfb, dtemp)                            \
	CHECK_FMUL2FIXED_VARS(vr, vda, vfb, dtemp)
#define FINISH_DFMUL2FIXED_VARS(vr, dtemp) FINISH_FMUL2FIXED_VARS(vr, dtemp)

#endif

                      /*
                       * set_float2fixed_vars(R, F) does the equivalent of R =
                       * float2fixed(F):
                       *      R is a fixed, F is a float or a double.
                       * set_fixed2float_var(R, V) does the equivalent of R =
                       * fixed2float(V):
                       *      R is a float or a double, V is a fixed.
                       * set_ldexp_fixed2double(R, V, E) does the equivalent of
                       * R=ldexp((double)V,E):
                       *      R is a double, V is a fixed, E is an int.
                       * R and F must be variables, not expressions; V and E may
                       * be expressions.
                       */
#if USE_FPU_FIXED
...
#define set_float2fixed_vars(vr, vf)                                           \
	(sizeof(vf) == sizeof(float)                                           \
	     ...
	     : set_double2fixed_(&vr,                                          \
	                         ((const uint32_t*)&vf)[arch_is_big_endian],   \
                      ...
                      /*
                       * We need the (double *)&vf cast to prevent compile-time
                       * error messages,
                       * even though the call will only be executed if vf has
                       * the correct type.
                       */
...
#define set_ldexp_fixed2double(vd, x, exp) set_fixed2double_(&vd, x, -(exp))
#else
#define set_float2fixed_vars(vr, vf)                                           \
	(f_fits_in_bits(vf, fixed_int_bits)                                    \
	     ? (vr = float2fixed(vf), 0)                                       \
	     : gs_note_error(gs_error_limitcheck))
#define set_fixed2float_var(vf, x) (vf = fixed2float(x), 0)
#define set_ldexp_fixed2double(vd, x, exp) discard(vd = ldexp((double)(x), exp))
#endif
