...
#define CHECK_SET_FIXED_SUM(r, a, b) \
	((((a) ^ (b)) >= 0) && ((((a) + (b)) ^ (a)) < 0) ? (((r) = (((a) < 0) ? min_fixed : max_fixed)), gs_error_limitcheck) : (((r) = ((a) + (b))), 0)) /* no overflow */
/*
...
#define CHECK_FMUL2FIXED_VARS(vr, vfa, vfb, dtemp) \
	(dtemp = (vfa) * (vfb),                    \
	 (f_fits_in_bits(dtemp, fixed_int_bits) ? 0 : gs_note_error(gs_error_limitcheck)))
#define FINISH_FMUL2FIXED_VARS(vr, dtemp) \
...
#define set_float2fixed_vars(vr, vf)                                                                                                                                        \
	(sizeof(vf) == sizeof(float) ? set_float2fixed_(&vr, *(const long *)&vf, fixed_fraction_bits) : set_double2fixed_(&vr, ((const uint32_t *)&vf)[arch_is_big_endian], \
...
#define set_fixed2float_var(vf, x) \
	(sizeof(vf) == sizeof(float) ? (*(long *)&vf = fixed2float_(x, fixed_fraction_bits), 0) : (set_fixed2double_((double *)&vf, x, fixed_fraction_bits), 0))
#define set_ldexp_fixed2double(vd, x, exp) \
	set_fixed2double_(&vd, x, -(exp))
#else
#define set_float2fixed_vars(vr, vf) \
	(f_fits_in_bits(vf, fixed_int_bits) ? (vr = float2fixed(vf), 0) : gs_note_error(gs_error_limitcheck))
#define set_fixed2float_var(vf, x) \
	(vf = fixed2float(x), 0)
#define set_ldexp_fixed2double(vd, x, exp) \
