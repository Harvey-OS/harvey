...
pdf_copy_mask_data(gx_device_pdf* pdev, const byte* base, int sourcex,
                   ...
                   gs_image_t* pim, pdf_image_writer* piw, int for_pattern)
{
	uint32_t nbytes;
	int code;
	const byte* row_base;
	...
	if(for_pattern) {
		...
		if(id != gx_no_bitmap_id) {
			piw->pres = pdf_find_resource_by_gs_id(
			    pdev, resourceXObject, id);
			if(piw->pres)
				return 0;
		}
	}
	...
	if((code = pdf_begin_write_image(pdev, piw, id, w, h, NULL, in_line)) <
	       0 ||
	   ...
	   (code = pdf_begin_image_data(pdev, piw, (const gs_pixel_image_t*)pim,
	                                NULL, 0)) < 0)
		return code;
	pdf_copy_mask_bits(piw->binary[0].strm, row_base, sourcex, row_step, w,
	                   h, 0);
...
private
int
pdf_copy_mono(gx_device_pdf* pdev, const byte* base, int sourcex, int raster,
              ...
              gx_color_index one, const gx_clip_path* pcpath)
{
	...
	if(zero == gx_no_color_index) {
		...
		if(id != gx_no_bitmap_id && sourcex == 0) {
			pres = pdf_find_resource_by_gs_id(pdev,
			                                  resourceCharProc, id);
			if(pres ==
			   0) { /* Define the character in an embedded font. */
				pdf_char_proc_t* pcp;
				double x_offset;
				int y_offset;
				...
				y_offset =
				    pdf_char_image_y_offset(pdev, x, y, h);
				/*
				 * The Y axis of the text matrix is inverted,
				 * so we need to negate the Y offset
				 * appropriately.
				 */
				code = pdf_begin_char_proc(
				    pdev, w, h, 0, y_offset, id, &pcp, &ipos);
				if(code < 0)
					return code;
				y_offset = -y_offset;
				x_offset = psdf_round(pdev->char_width.x, 100,
				                      10); /* See
	       ...
	       a compatible data for Tj. */
				...
				code = pdf_begin_write_image(
				    pdev, &writer, gs_no_id, w, h, NULL, true);
				if(code < 0)
					return code;
				pres = (pdf_resource_t*)pcp;
				goto wr;
			} else if(pdev->pte) {
				/* We're under pdf_text_process. It set a high
				 * level color. */
			} else
		...
		/*
		 * We think this code is never executed when interpreting
		 * PostScript
	...
	pdf_make_bitmap_image(&image, x, y, w, h);
	{
		...
		if(code < 0)
			return code;
		pdf_image_writer_init(&writer);
		code = pdf_begin_write_image(pdev, &writer, gs_no_id, w, h,
		                             NULL, in_line);
		if(code < 0)
			return code;
	}
wr:
	...
	if(pres) {
		...
		image.Decode[1] = d0;
		psdf_CFE_binary(&writer.binary[0], image.Width, image.Height,
		                true);
		invert ^= 0xff;
	} else {
	...
	if(!pres) {
		...
		case 1:
			return 0;
		case 0:
			return pdf_do_image(pdev, writer.pres,
			                    &image.ImageMatrix, true);
		}
	}
...
int
gdev_pdf_copy_mono(gx_device* dev, const byte* base, int sourcex, int raster,
                   ...
                   gx_color_index zero, gx_color_index one)
{
	...
	if(w <= 0 || h <= 0)
		return 0;
	return pdf_copy_mono(pdev, base, sourcex, raster, id, x, y, w, h, zero,
	                     one, NULL);
}
	...
	if((code = pdf_begin_write_image(pdev, piw, id, w, h, NULL, in_line)) <
	   ...
	   (for_pattern < 2 || nbytes < 512000
	        ...
	        : (code = psdf_setup_image_filters(
	               (gx_device_psdf*)pdev, &piw->binary[0],
	               (gs_pixel_image_t*)pim, NULL, NULL, false))) < 0 ||
	   (code = pdf_begin_image_data(pdev, piw, (const gs_pixel_image_t*)pim,
	                                &cs_value, 0)) < 0)
		return code;
	pdf_copy_color_bits(piw->binary[0].strm, row_base, sourcex, row_step, w,
	                    h, bytes_per_pixel);
	pdf_end_image_binary(pdev, piw, piw->height);
	return pdf_end_write_image(pdev, piw);
}
...
int
gdev_pdf_copy_color(gx_device* dev, const byte* base, int sourcex, int raster,
                    gx_bitmap_id id, int x, int y, int w, int h)
{
	gx_device_pdf* pdev = (gx_device_pdf*)dev;
	gs_image_t image;
...
/* Fill a mask. */
int
gdev_pdf_fill_mask(gx_device* dev, const byte* data, int data_x, int raster,
                   ...
                   gs_logical_operation_t lop, const gx_clip_path* pcpath)
{
	...
	if(width <= 0 || height <= 0)
		return 0;
	if(depth > 1 ||
	   (!gx_dc_is_pure(pdcolor) != 0 && pdcolor->type != &gx_dc_pattern))
		return gx_default_fill_mask(dev, data, data_x, raster, id, x, y,
		                            width, height, pdcolor, depth, lop,
		                            pcpath);
	return pdf_copy_mono(pdev, data, data_x, raster, id, x, y, width,
	                     height, gx_no_color_index,
	                     gx_dc_pure_color(pdcolor), pcpath);
}
                              ...
                              gx_color_index color1, int px, int py)
{
	...
	bool mask;
	int depth;
	int (*copy_data)(gx_device_pdf*, const byte*, int, int, gx_bitmap_id,
	                 int, int, int, int, gs_image_t*, pdf_image_writer*,
	                 int);
	pdf_resource_t* pres;
	cos_value_t cs_value;
	int code;
	...
	if(tiles->id == gx_no_bitmap_id || tiles->shift != 0 ||
	   (w < tw && h < th) || color0 != gx_no_color_index)
		goto use_default;
	...
	if(!pres) {
		...
		uint32_t tile_id = (tw == tiles->size.x && th == tiles->size.y
		                        ? tiles->id
		                        : gx_no_bitmap_id);
		...
		if(in_line)
			...
			/*
			 * Acrobat Reader can't handle image Patterns with more
			 * than
			 * 64K of data.  :-(
			 */
			goto use_default;
		...
		code =
		    pdf_begin_resource(pdev, resourcePattern, tiles->id, &pres);
		if(code < 0)
			goto use_default;
		s = pdev->strm;
		pprintd1(
		    s, "/PatternType 1/PaintType %d/TilingType 1/Resources<<\n",
		    (mask ? 2 : 1));
		if(image_id)
			pprintld2(s, "/XObject<</R%ld %ld 0 R>>", image_id,
			          image_id);
	...
	return 0;
use_default:
	return gx_default_strip_tile_rectangle(dev, tiles, x, y, w, h, color0,
	                                       color1, px, py);
}
