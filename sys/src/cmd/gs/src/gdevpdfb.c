		    ...
		    int for_pattern)
{
	...
	if((code = pdf_begin_write_image(pdev, piw, id, w, h, NULL, in_line)) < 0 ||
	   ...
	   (for_pattern < 2 || nbytes < 512000 ? (code = psdf_setup_lossless_filters((gx_device_psdf *)pdev,
										     &piw->binary[0], (gs_pixel_image_t *)pim))
					       : (code = psdf_setup_image_filters((gx_device_psdf *)pdev,
										  &piw->binary[0], (gs_pixel_image_t *)pim, NULL, NULL, false))) < 0 ||
	   (code = pdf_begin_image_data(pdev, piw, (const gs_pixel_image_t *)pim,
					&cs_value, 0)) < 0)
		return code;
			      ...
			      int px, int py)
{
	...
	if(!pres) {
		...
		uint32_t tile_id =
		    (tw == tiles->size.x && th == tiles->size.y ? tiles->id : gx_no_bitmap_id);
