...
private
dev_proc_open_device(os2prn_open);
private
dev_proc_close_device(os2prn_close);
private
dev_proc_print_page(os2prn_print_page);
private
dev_proc_map_rgb_color(os2prn_map_rgb_color);
private
dev_proc_map_color_rgb(os2prn_map_color_rgb);
private
dev_proc_put_params(os2prn_put_params);
private
dev_proc_get_params(os2prn_get_params);

private
void os2prn_set_bpp(gx_device* dev, int depth);
private
int os2prn_get_queue_list(gs_memory_t* mem, OS2QL* ql);
private
void os2prn_free_queue_list(gs_memory_t* mem, OS2QL* ql);
int os2prn_get_printer(OS2QL* ql);

private
gx_device_procs os2prn_procs = prn_color_params_procs(
    os2prn_open, gdev_prn_output_page, os2prn_close, os2prn_map_rgb_color,
    os2prn_map_color_rgb, os2prn_get_params, os2prn_put_params);
...
gx_device_os2prn far_data gs_os2prn_device = {
    prn_device_std_body(gx_device_os2prn, os2prn_procs, "os2prn",
                        DEFAULT_WIDTH_10THS, DEFAULT_HEIGHT_10THS, 72, 72, 0, 0,
                        0, 0, 0, os2prn_print_page), /* depth = 0 */
    0,                                               /* hab */
    0,                                               /* hdc */
    0,                                               /* hps */
    ""                                               /* queue_name */
...
os2prn_open(gx_device* dev)
{
	...
	if(pppib->pib_ultype != 3) {
		...
		errprintf(
		    "os2prn device can only be used from a PM application\n");
		return gs_error_limitcheck;
	}
	...
	if(opdev->queue_name[0] != '\0') {
		for(i = 0; i < opdev->ql.nqueues; i++) {
			if(strcmp(opdev->ql.prq[i].pszName,
			          opdev->queue_name) == 0) {
				pprq = &(opdev->ql.prq[i]);
				break;
			}
	...
	opdev->hdc = DevOpenDC(opdev->hab, OD_QUEUED, "*", 9L,
	...
	opdev->clipbox[0] =
	    (int)(hcinfo.xLeftClip / 25.4 * dev->x_pixels_per_inch +
	          1); /* round inwards */
	opdev->clipbox[1] =
	    (int)(hcinfo.yBottomClip / 25.4 * dev->y_pixels_per_inch + 1);
	opdev->clipbox[2] =
	    (int)(hcinfo.xRightClip / 25.4 * dev->x_pixels_per_inch);
	opdev->clipbox[3] =
	    (int)(hcinfo.yTopClip / 25.4 * dev->y_pixels_per_inch);

	/* get presentation space */
	sizlPage.cx = dev->width;
	sizlPage.cy = dev->height;
	opdev->hps =
	    GpiCreatePS(opdev->hab, opdev->hdc, &sizlPage,
	...
	opdev->hdcMem =
	    DevOpenDC(opdev->hab, OD_MEMORY, "*", 0L, NULL, opdev->hdc);
	...
	sizlPage.cy = dev->height;
	opdev->hpsMem =
	    GpiCreatePS(opdev->hab, opdev->hdcMem, &sizlPage,
	                PU_PELS | GPIF_DEFAULT | GPIT_NORMAL | GPIA_ASSOC);
	...
	pfile = gp_open_scratch_file(gp_scratch_file_name_prefix, opdev->fname,
	                             "wb");
	fclose(pfile);
...
os2prn_get_params(gx_device* dev, gs_param_list* plist)
{
	...
	qs.persistent = false;
	code < 0 || (code = param_write_string(plist, "OS2QUEUE", &qs)) < 0;
	return code;
}
...
os2prn_put_params(gx_device* dev, gs_param_list* plist)
{
	...
	default:
		ecode = code;
	qe:
		param_signal_error(plist, "OS2QUEUE", ecode);
	case 1:
		qs.data = 0;
		break;
	}
...
os2prn_print_page(gx_device_printer* pdev, FILE* file)
{
	...
	row = (byte*)gs_malloc(pdev->memory, bmp_raster_multi, 1,
	                       "bmp file buffer");
	...
	while(y > yend) {
		...
		y -= lines;
		for(i = lines - 1; i >= 0; i--)
			gdev_prn_copy_scan_lines(pdev, ystart - 1 - (y + i),
			                         row + (bmp_raster * i),
			                         raster);
		apts[0].y = 0; /* target */
		apts[1].y = lines;
		...
		rc =
		    GpiDrawBits(opdev->hpsMem, row, (BITMAPINFO2*)&bmi, 4, apts,
		...
		aptsb[1].y = y + lines;
		aptsb[3].y = lines;
		rc = GpiBitBlt(opdev->hps, opdev->hpsMem, 4, aptsb, ROP_SRCCOPY,
		               BBO_IGNORE);
...
bmp_done:
	if(row)
		gs_free(pdev->memory, (char*)row, bmp_raster_multi, 1,
		        "bmp file buffer");
...
os2prn_get_queue_list(gs_memory_t* mem, OS2QL* ql)
{
	...
	if(splerr == ERROR_MORE_DATA || splerr == NERR_BufTooSmall) {
		pBuf = gs_malloc(mem, cbNeeded, 1,
		                 "OS/2 printer device info buffer");
		ql->prq = (PRQINFO3*)pBuf;
		if(ql->prq != (PRQINFO3*)NULL) {
			ql->len = cbNeeded;
			cbBuf = cbNeeded;
			splerr =
			    SplEnumQueue(pszComputerName, ulLevel, pBuf, cbBuf,
			                 &cReturned, &cTotal, &cbNeeded, NULL);
			if(splerr == NO_ERROR) {
				/* Set pointer to point to the beginning of the
				 * buffer.           */
				prq = (PPRQINFO3)pBuf;
				/* cReturned has the count of the number of
				 * PRQINFO3 structures.  */
		...
		/* If we are here we had a bad error code. Print it and some
		 * other info. */
		eprintf4("SplEnumQueue Error=%ld, Total=%ld, Returned=%ld, "
		         "Needed=%ld\n",
