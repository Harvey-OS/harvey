...
typedef struct FAPI_outline_handler_s {
	...
	fixed x0, y0;
	bool close_path, need_close; /* This stuff fixes unclosed paths being
	                                rendered with UFST */
} FAPI_outline_handler;

private
inline int
import_shift(int x, int n)
{
	return n > 0 ? x << n : x >> -n;
}
...
private
int
add_move(FAPI_path* I, FracInt x, FracInt y)
{
	FAPI_outline_handler* olh = (FAPI_outline_handler*)I->olh;
	int code;
	...
	if(olh->need_close && olh->close_path)
		if((code = add_closepath(I)) < 0)
			return code;
	olh->need_close = false;
	return gx_path_add_point(olh->path, import_shift(x, I->shift) + olh->x0,
	                         -import_shift(y, I->shift) + olh->y0);
}

private
int
add_line(FAPI_path* I, FracInt x, FracInt y)
{
	FAPI_outline_handler* olh = (FAPI_outline_handler*)I->olh;

	olh->need_close = true;
	return gx_path_add_line_notes(olh->path,
	                              import_shift(x, I->shift) + olh->x0,
	                              -import_shift(y, I->shift) + olh->y0, 0);
}

private
int
add_curve(FAPI_path* I, FracInt x0, FracInt y0, FracInt x1, FracInt y1,
          FracInt x2, FracInt y2)
{
	FAPI_outline_handler* olh = (FAPI_outline_handler*)I->olh;

	olh->need_close = true;
	return gx_path_add_curve_notes(
	    olh->path, import_shift(x0, I->shift) + olh->x0,
	    -import_shift(y0, I->shift) + olh->y0,
	    import_shift(x1, I->shift) + olh->x0,
	    -import_shift(y1, I->shift) + olh->y0,
	    import_shift(x2, I->shift) + olh->x0,
	    -import_shift(y2, I->shift) + olh->y0, 0);
}

private
FAPI_path path_interface_stub = {NULL,     0,         add_move,
                                 add_line, add_curve, add_closepath};

private
inline bool
IsCIDFont(const gs_font_base* pbfont)
{
...
private
inline bool
IsType1GlyphData(const gs_font_base* pbfont)
{
...
sfnts_next_elem(sfnts_reader* r)
{
	...
	r->length =
	    r_size(&s) &
	    ~(uint)1; /* See Adobe Technical Note # 5012, section 4.2. */
	r->offset = 0;
}
...
private
byte sfnts_reader_rbyte(sfnts_reader* r) /* old compiler compatibility */
{
...
private
const sfnts_writer sfnts_writer_stub = {0, 0, 0, sfnts_writer_wbyte,
                                        sfnts_writer_wword, sfnts_writer_wlong,
...
sfnts_need_copy_table(byte* tag)
{
	return memcmp(tag, "glyf", 4) &&
	       memcmp(tag, "glyx", 4) && /* Presents in files created by
	                                    AdobePS5.dll Version 5.1.2 */
	       memcmp(tag, "loca", 4) &&
	       memcmp(tag, "locx", 4) && /* Presents in files created by
	                                    AdobePS5.dll Version 5.1.2 */
	       memcmp(tag, "cmap", 4);
}
...
{ /* Note : TTC is not supported and probably is unuseful for Type 42. */
	...
	for(i = 0; i < num_tables; i++) {
		...
		if(sfnts_need_copy_table(tables[i].tag)) {
			num_tables_new++;
			size_new += (tables[i].length + alignment - 1) /
			            alignment * alignment;
		}
	}
	...
	for(i = 0; i < num_tables; i++)
		if(sfnts_need_copy_table(tables[i].tag)) {
			...
			if(r->error)
				return 0;
			if(w->p - w->buf !=
			   tables[i].offset_new + num_tables_new * 16)
...
private
ushort
FAPI_FF_serialize_tt_font(FAPI_font* ff, void* buf, int buf_size)
{
...
private
ushort
FAPI_FF_get_word(FAPI_font* ff, fapi_font_feature var_id, int index)
{
	gs_font_type1* pfont = (gs_font_type1*)ff->client_font_data;
	ref* pdr = (ref*)ff->client_font_data2;

	switch((int)var_id) {
	case FAPI_FONT_FEATURE_Weight:
		return 0; /* wrong */
	case FAPI_FONT_FEATURE_ItalicAngle:
		return 0; /* wrong */
	case FAPI_FONT_FEATURE_IsFixedPitch:
		return 0; /* wrong */
	case FAPI_FONT_FEATURE_UnderLinePosition:
		return 0; /* wrong */
	case FAPI_FONT_FEATURE_UnderlineThickness:
		return 0; /* wrong */
	case FAPI_FONT_FEATURE_FontType:
		return (pfont->FontType == 2 ? 2 : 1);
	case FAPI_FONT_FEATURE_FontBBox:
		switch(index) {
		case 0:
			return (uint16_t)pfont->FontBBox.p.x;
		case 1:
			return (uint16_t)pfont->FontBBox.p.y;
		case 2:
			return (uint16_t)pfont->FontBBox.q.x;
		case 3:
			return (uint16_t)pfont->FontBBox.q.y;
		}
		return 0;
	case FAPI_FONT_FEATURE_BlueValues_count:
		return pfont->data.BlueValues.count;
	case FAPI_FONT_FEATURE_BlueValues:
		return float_to_ushort(pfont->data.BlueValues.values[index]);
	case FAPI_FONT_FEATURE_OtherBlues_count:
		return pfont->data.OtherBlues.count;
	case FAPI_FONT_FEATURE_OtherBlues:
		return float_to_ushort(pfont->data.OtherBlues.values[index]);
	case FAPI_FONT_FEATURE_FamilyBlues_count:
		return pfont->data.FamilyBlues.count;
	case FAPI_FONT_FEATURE_FamilyBlues:
		return float_to_ushort(pfont->data.FamilyBlues.values[index]);
	case FAPI_FONT_FEATURE_FamilyOtherBlues_count:
		return pfont->data.FamilyOtherBlues.count;
	case FAPI_FONT_FEATURE_FamilyOtherBlues:
		return float_to_ushort(
		    pfont->data.FamilyOtherBlues.values[index]);
	case FAPI_FONT_FEATURE_BlueShift:
		return float_to_ushort(pfont->data.BlueShift);
	case FAPI_FONT_FEATURE_BlueFuzz:
		return float_to_ushort(pfont->data.BlueShift);
	case FAPI_FONT_FEATURE_StdHW:
		return (
		    pfont->data.StdHW.count == 0
		        ? 0
		        : float_to_ushort(
		              pfont->data.StdHW.values[0])); /* UFST bug ? */
	case FAPI_FONT_FEATURE_StdVW:
		return (
		    pfont->data.StdVW.count == 0
		        ? 0
		        : float_to_ushort(
		              pfont->data.StdVW.values[0])); /* UFST bug ? */
	case FAPI_FONT_FEATURE_StemSnapH_count:
		return pfont->data.StemSnapH.count;
	case FAPI_FONT_FEATURE_StemSnapH:
		return float_to_ushort(pfont->data.StemSnapH.values[index]);
	case FAPI_FONT_FEATURE_StemSnapV_count:
		return pfont->data.StemSnapV.count;
	case FAPI_FONT_FEATURE_StemSnapV:
		return float_to_ushort(pfont->data.StemSnapV.values[index]);
	case FAPI_FONT_FEATURE_ForceBold:
		return pfont->data.ForceBold;
	case FAPI_FONT_FEATURE_LanguageGroup:
		return pfont->data.LanguageGroup;
	case FAPI_FONT_FEATURE_lenIV:
	...
	case FAPI_FONT_FEATURE_Subrs_count: {
		...
		/* trick : we return twice maximum of n1, n2 to pass both Subrs
		 * and GlobalSubrs in same array.
		*/
...
private
ulong
FAPI_FF_get_long(FAPI_font* ff, fapi_font_feature var_id, int index)
{
	gs_font_type1* pfont = (gs_font_type1*)ff->client_font_data;
	ref* pdr = (ref*)ff->client_font_data2;

	switch((int)var_id) {
	case FAPI_FONT_FEATURE_UniqueID:
		return pfont->UID.id;
	case FAPI_FONT_FEATURE_BlueScale:
		return (uint32_t)(pfont->data.BlueScale * 65536);
...
private
float
FAPI_FF_get_float(FAPI_font* ff, fapi_font_feature var_id, int index)
{
	...
	case FAPI_FONT_FEATURE_FontMatrix: {
		double FontMatrix_div =
		    (ff->is_cid && !IsCIDFont(pbfont) ? 1000 : 1);
		switch(index) {
		case 0:
			return pbfont->base->FontMatrix.xx / FontMatrix_div;
		case 1:
			return pbfont->base->FontMatrix.xy / FontMatrix_div;
		case 2:
			return pbfont->base->FontMatrix.yx / FontMatrix_div;
		case 3:
			return pbfont->base->FontMatrix.yy / FontMatrix_div;
		case 4:
			return pbfont->base->FontMatrix.tx / FontMatrix_div;
		case 5:
			return pbfont->base->FontMatrix.ty / FontMatrix_div;
		}
	}
	}
	return 0;
...
private
ushort
get_type1_data(FAPI_font* ff, const ref* type1string, byte* buf,
               uint16_t buf_length)
{
	gs_font_type1* pfont = (gs_font_type1*)ff->client_font_data;
	...
	if(buf != 0) {
		...
		if(ff->need_decrypt && pfont->data.lenIV >= 0)
			decode_bytes(buf, type1string->value.const_bytes,
			             l + lenIV, lenIV);
		else
...
private
ushort
FAPI_FF_get_subr(FAPI_font* ff, int index, byte* buf, uint16_t buf_length)
{
	ref* pdr = (ref*)ff->client_font_data2;
	...
	n1 = (Subrs != NULL ? r_size(Subrs) : 0);
	n2 = (GlobalSubrs != NULL ? r_size(GlobalSubrs) : 0);
	/* trick : we use the maximum of n1, n2 to pass both Subrs and
	 * GlobalSubrs in same array.
	*/
	n = (n1 < n2 ? n2 : n1);
	if(index < n && Subrs != NULL) {
		...
		if(array_get(ff->memory, GlobalSubrs, index - n, &subr) < 0 ||
		   r_type(&subr) != t_string)
			return 0;
	} else
		return 0;
...
{ /* Note : TTC is not supported and probably is unuseful for Type 42. */
	...
	*offset0 = pfont42->data.glyf +
	           (glyf_elem_size == 2 ? r.rword(&r) * 2 : r.rlong(&r));
	*offset1 = pfont42->data.glyf +
	           (glyf_elem_size == 2 ? r.rword(&r) * 2 : r.rlong(&r));
	return r.error;
}
...
private
int
get_GlyphDirectory_data_ptr(const gs_memory_t* mem, ref* pdr, int char_code,
                            const byte** ptr)
{
...
private
ushort
FAPI_FF_get_glyph(FAPI_font* ff, int char_code, byte* buf, uint16_t buf_length)
{ /*
   ...
   */
	...
	if(ff->is_type1) {
		if(ff->is_cid) {
			...
			glyph_length =
			    get_type1_data(ff, glyph, buf, buf_length);
		} else {
			ref* CharStrings, char_name, *glyph;
			if(ff->char_data != NULL) {
				/*
				 * Can't use char_code in this case because
				 * hooked Type 1 fonts
				 * with 'glyphshow' may render a character which
				 * has no
				 * Encoding entry.
				 */
				if(name_ref(ff->memory, ff->char_data,
				            ff->char_data_len, &char_name,
				            -1) < 0)
					return -1;
				if(buf != NULL) {
					/*
					 * Trigger the next call to the 'seac'
					 * case below.
					 * Here we use the assumption about call
					 * sequence
				...
				if(dict_find_string(systemdict,
				                    "StandardEncoding",
				                    &StandardEncoding) <= 0 ||
				   array_get(ff->memory, StandardEncoding,
				             char_code, &char_name) < 0)
					if(name_ref(ff->memory,
					            (const byte*)".notdef", 7,
					            &char_name, -1) < 0)
						return -1;
			}
			if(dict_find_string(pdr, "CharStrings", &CharStrings) <=
			   0)
				return -1;
		...
		const byte* data_ptr;
		int l = get_GlyphDirectory_data_ptr(ff->memory, pdr, char_code,
		                                    &data_ptr);

		if(l >= 0) {
			int MetricsCount = get_MetricsCount(ff),
			    mc = MetricsCount << 1;
			...
			bool error = sfnt_get_glyph_offset(
			    pdr, pfont42, char_code, &offset0, &offset1);
...
private
const FAPI_font ff_stub = {0,     /* server_font_data */
                           0,     /* need_decrypt */
                           ...
                           0,     /* char_data */
                           0,     /* char_data_len */
                           FAPI_FF_get_word, FAPI_FF_get_long,
                           FAPI_FF_get_float, FAPI_FF_get_subr,
                           FAPI_FF_get_glyph, FAPI_FF_serialize_tt_font};
...
FAPI_get_xlatmap(i_ctx_t* i_ctx_p, char** xlatmap)
{
	...
	*xlatmap = (char*)pref->value.bytes;
	/*  Note : this supposes that xlatmap doesn't move in virtual memory.
	    Garbager must not be called while plugin executes get_scaled_font,
	   get_decodingID.
...
private
int
renderer_retcode(i_ctx_t* i_ctx_p, FAPI_server* I, FAPI_retcode rc)
{
	if(rc == 0)
		return 0;
...
private
int
FAPI_find_plugin(i_ctx_t* i_ctx_p, const char* subtype, FAPI_server** pI)
{
	i_plugin_holder* h = i_plugin_get_list(i_ctx_p);
	int code;
	...
	for(; h != 0; h = h->next)
		if(!strcmp(h->I->d->type, "FAPI"))
			if(!strcmp(h->I->d->subtype, subtype)) {
				...
				if((code = renderer_retcode(
				        i_ctx_p, I, I->ensure_open(I))) < 0)
					return code;
				return 0;
			}
...
private
int
FAPI_prepare_font(i_ctx_t* i_ctx_p, FAPI_server* I, ref* pdr,
                  gs_font_base* pbfont, const char* font_file_path,
                  const FAPI_font_scale* font_scale, const char* xlatmap,
                  int BBox[4], const char** decodingID)
{       /* Returns 1 iff BBox is set. */
	 ...
	 *
	 * Actually server's data for top level CIDFontTYpe 0 non-disk fonts
	 *should not be important,
	 * because with non-disk fonts FAPI_do_char never deals with the
	 *top-level font,
	 * but does with its descendants individually.
	 * Therefore a recommendation for the renderer is don't build any
	 *special
	 * data for the top-level non-disk font of CIDFontType 0, but return
	 *immediately
	...
	ff.server_font_data =
	    pbfont->FAPI_font_data; /* Possibly pass it from zFAPIpassfont. */
	ff.is_cid = IsCIDFont(pbfont);
	ff.is_mtx_skipped = (get_MetricsCount(&ff) != 0);
	if((code = renderer_retcode(
	        i_ctx_p, I,
	        I->get_scaled_font(I, &ff, subfont, font_scale, xlatmap, false,
	                           FAPI_TOPLEVEL_BEGIN))) < 0)
		return code;
	pbfont->FAPI_font_data =
	    ff.server_font_data; /* Save it back to GS font. */
	if(ff.server_font_data != 0) {
		if((code = renderer_retcode(
		        i_ctx_p, I, I->get_font_bbox(I, &ff, BBox))) < 0) {
	...
	if(xlatmap != NULL && pbfont->FAPI_font_data != NULL)
		if((code = renderer_retcode(
		        i_ctx_p, I, I->get_decodingID(I, &ff, decodingID))) <
		   0) {
	...
	if(font_file_path == NULL && ff.is_type1 &&
	   ff.is_cid) { /* Renderers should expect same condition. */
		gs_font_cid0* pfcid = (gs_font_cid0*)pbfont;
		gs_font_type1** FDArray = pfcid->cidata.FDArray;
		int i, n = pfcid->cidata.FDArray_size;
		ref* rFDArray, f;

		if(dict_find_string(pdr, "FDArray", &rFDArray) <= 0 ||
		...
		for(i = 0; i < n; i++) {
			...
			pbfont1->FontBBox = pbfont->FontBBox; /* Inherit
			                                         FontBBox from
			                                         the type 9
			                                         font. */
			if(array_get(imemory, rFDArray, i, &f) < 0 ||
			...
			if((code = renderer_retcode(
			        i_ctx_p, I,
			        I->get_scaled_font(I, &ff, 0, font_scale, NULL,
			                           false, i))) < 0)
				break;
			pbfont1->FAPI_font_data =
			    ff.server_font_data; /* Save it back to GS font. */
			/* Try to do something with the descendant font to
			 * ensure that it's working : */
			if((code = renderer_retcode(
			        i_ctx_p, I,
			        I->get_font_bbox(I, &ff, BBox_temp))) < 0)
				break;
		}
		if(i == n) {
			code = renderer_retcode(
			    i_ctx_p, I, I->get_scaled_font(
			                    I, &ff, subfont, font_scale, NULL,
			                    false, FAPI_TOPLEVEL_COMPLETE));
			if(code >= 0)
		...
		code = renderer_retcode(
		    i_ctx_p, I,
		    I->get_scaled_font(I, &ff, subfont, font_scale, xlatmap,
		                       false, FAPI_TOPLEVEL_COMPLETE));
		if(code < 0) {
			I->release_typeface(I, pbfont->FAPI_font_data);
			pbfont->FAPI_font_data = 0;
...
private
int
FAPI_refine_font(i_ctx_t* i_ctx_p, os_ptr op, gs_font_base* pbfont,
                 const char* font_file_path)
{
	...
	code = FAPI_prepare_font(i_ctx_p, I, pdr, pbfont, font_file_path,
	                         &font_scale, xlatmap, BBox, &decodingID);
	if(code < 0)
		return code;
	...
	if(code > 0) {
		...
		ref_assign_old(v, v->value.refs + 0, &x0,
		               "FAPI_refine_font_BBox");
		ref_assign_old(v, v->value.refs + 1, &y0,
		               "FAPI_refine_font_BBox");
		ref_assign_old(v, v->value.refs + 2, &x1,
		               "FAPI_refine_font_BBox");
		ref_assign_old(v, v->value.refs + 3, &y1,
		               "FAPI_refine_font_BBox");
	}
	...
	if(decodingID != 0 && *decodingID) {
		...
		if(IsCIDFont(pbfont)) {
			...
			if(dict_find_string(pdr, "CIDSystemInfo",
			                    &CIDSystemInfo) <= 0 ||
			   !r_has_type(CIDSystemInfo, t_dictionary))
				return_error(e_invalidfont);
			if(dict_find_string(CIDSystemInfo, "Ordering",
			                    &Ordering) <= 0 ||
			   !r_has_type(Ordering, t_string))
				return_error(e_invalidfont);
			ordering_length =
			    min(r_size(Ordering),
			        sizeof(buf) - 2 - decodingID_length);
			memcpy(buf, Ordering->value.const_bytes,
			       ordering_length);
			if((code = name_ref(imemory, buf, ordering_length,
			                    &SubstNWP, 0)) < 0)
				return code;
			if((code = dict_put_string(pdr, "SubstNWP", &SubstNWP,
			                           NULL)) < 0)
				return code;
			buf[ordering_length] = '.';
			memcpy(buf + ordering_length + 1, decodingID,
			       decodingID_length);
			buf[decodingID_length + 1 + ordering_length] =
			    0; /* Debug purpose only */
			if((code =
			        name_ref(imemory, buf, decodingID_length + 1 +
			                                   ordering_length,
			                 &Decoding, 0)) < 0)
				return code;
		          ...
		          0)
			return code;
		if((code = dict_put_string(pdr, "Decoding", &Decoding, NULL)) <
		   0)
			return code;
	}
	return 0;
...
{ /* gs_font_finalize passes event_data == NULL, so check it here. */
	if(event_data == NULL) {
		...
		if(pbfont->FAPI_font_data != 0)
			pbfont->FAPI->release_typeface(pbfont->FAPI,
			                               pbfont->FAPI_font_data);
	}
	return 0;
}
...
zFAPIrebuildfont(i_ctx_t* i_ctx_p)
{
	...
	if(pbfont->FAPI != 0) {
		...
		*/
		...
		FAPI_ID[len] = 0;
		if((code = FAPI_find_plugin(i_ctx_p, FAPI_ID, &pbfont->FAPI)) <
		   0)
			return code;
	}
	pdata = (font_data*)pfont->client_data;
	if(dict_find_string(op - 1, "Path", &v) <= 0 ||
	   !r_has_type(v, t_string))
		v = NULL;
	if(pfont->FontType == ft_CID_encrypted && v == NULL) {
		if((code = build_proc_name_refs(imemory, &build,
		                                ".FAPIBuildGlyph9",
		                                ".FAPIBuildGlyph9")) < 0)
			return code;
	} else if((code =
	               build_proc_name_refs(imemory, &build, ".FAPIBuildChar",
	                                    ".FAPIBuildGlyph")) < 0)
		return code;
	if(name_index(imemory, &pdata->BuildChar) ==
	   name_index(imemory, &build.BuildChar)) {
		...
		code =
		    FAPI_refine_font(i_ctx_p, op - 1, pbfont, font_file_path);
		if(font_file_path != NULL)
			gs_free_string(imemory_global, (byte*)font_file_path,
			               r_size(v) + 1, "font file path");
		code1 = gs_notify_register(&pfont->notify_list,
		                           notify_remove_font, pbfont);
...
private
int
outline_char(i_ctx_t* i_ctx_p, FAPI_server* I, int import_shift_v,
             gs_show_enum* penum_s, struct gx_path_s* path, bool close_path)
{
	...
	if((code = renderer_retcode(
	        i_ctx_p, I, I->get_char_outline(I, &path_interface))) < 0)
		return code;
	if(olh.need_close && olh.close_path)
...
private
void
compute_em_scale(const gs_font_base* pbfont, FAPI_metrics* metrics,
                 double FontMatrix_div, double* em_scale_x, double* em_scale_y)
{ /* optimize : move this stuff to FAPI_refine_font */
	gs_matrix* m = &pbfont->base->FontMatrix;
	int rounding_x, rounding_y; /* Striking out the 'float' representation
	                               error in FontMatrix. */
	double sx, sy;
...
private
int
fapi_copy_mono(gx_device* dev1, FAPI_raster* rast, int dx, int dy)
{
	if((rast->line_step & (align_bitmap_mod - 1)) == 0)
		return dev_proc(dev1, copy_mono)(
		    dev1, rast->p, 0, rast->line_step, 0, dx, dy, rast->width,
		    rast->height, 0, 1);
...
private
int
fapi_finish_render_aux(i_ctx_t* i_ctx_p, gs_font_base* pbfont, FAPI_server* I)
{
	gs_text_enum_t* penum = op_show_find(i_ctx_p);
	gs_show_enum* penum_s = (gs_show_enum*)penum;
	gs_state* pgs = penum_s->pgs;
	gx_device* dev1 = gs_currentdevice_inline(
	    pgs); /* Possibly changed by zchar_set_cache. */
	...
	if(SHOW_IS(penum, TEXT_DO_NONE)) {
		...
		if((code = outline_char(i_ctx_p, I, import_shift_v, penum_s,
		                        pgs->show_gstate->path,
		                        !pbfont->PaintType)) < 0)
			return code;
	} else {
		...
		if(code == e_limitcheck) {
			...
			gs_imager_state* pis =
			    (gs_imager_state*)pgs->show_gstate;
			gs_point pt;
			if((code = gs_currentpoint(pgs->show_gstate, &pt)) < 0)
				return code;
			if((code = outline_char(i_ctx_p, I, import_shift_v,
			                        penum_s, pgs->show_gstate->path,
			                        !pbfont->PaintType)) < 0)
				return code;
			if((code = gs_imager_setflat(
			        pis, gs_char_flatness(pis, 1.0))) < 0)
				return code;
			if(pbfont->PaintType) {
				if((code = gs_stroke(pgs->show_gstate)) < 0)
					return code;
			} else if((code = gs_fill(pgs->show_gstate)) < 0)
				return code;
			if((code = gs_moveto(pgs->show_gstate, pt.x, pt.y)) < 0)
				return code;
		} else {
			int rast_orig_x =
			    rast.orig_x - (int)(penum_s->fapi_glyph_shift.x *
			                        (1 << frac_pixel_shift));
			int rast_orig_y =
			    -rast.orig_y - (int)(penum_s->fapi_glyph_shift.y *
			                         (1 << frac_pixel_shift));

			if((code = renderer_retcode(i_ctx_p, I, code)) < 0)
				return code;
			if(pgs->in_cachedevice ==
			   CACHE_DEVICE_CACHING) { /* Using GS cache */
				/*  GS and renderer may transform coordinates
				   few differently.
				    The best way is to make set_cache_device to
				   take the renderer's bitmap metrics
				   immediately,
				    but we need to account CDevProc, which may
				   truncate the bitmap.
				    Perhaps GS overestimates the bitmap size,
				    so now we only add a compensating shift -
				   the dx and dy.
				*/
				int shift_rd = _fixed_shift - frac_pixel_shift;
				int rounding = 1 << (frac_pixel_shift - 1);
				int dx = arith_rshift_slow(
				    (pgs->ctm.tx_fixed >> shift_rd) +
				        rast_orig_x + rounding,
				    frac_pixel_shift);
				int dy = arith_rshift_slow(
				    (pgs->ctm.ty_fixed >> shift_rd) +
				        rast_orig_y + rounding,
				    frac_pixel_shift);
				if((code = fapi_copy_mono(dev1, &rast, dx,
				                          dy)) < 0)
					return code;
			} else { /* Not using GS cache */
				const gx_clip_path* pcpath =
				    i_ctx_p->pgs->clip_path;
				const gx_drawing_color* pdcolor =
				    penum->pdcolor;
				if((code = dev_proc(dev, fill_mask)(
				        dev, rast.p, 0, rast.line_step, 0,
				        (int)(penum_s->pgs->ctm.tx +
				              (double)rast_orig_x /
				                  (1 << frac_pixel_shift) +
				              0.5),
				        (int)(penum_s->pgs->ctm.ty +
				              (double)rast_orig_y /
				                  (1 << frac_pixel_shift) +
				              0.5),
				        rast.width, rast.height, pdcolor, 1,
				        rop3_default, pcpath)) < 0)
					return code;
			}
		}
	}
...
private
int
fapi_finish_render(i_ctx_t* i_ctx_p)
{
	os_ptr op = osp;
	gs_font* pfont;
...
private
int
FAPI_do_char(i_ctx_t* i_ctx_p, gs_font_base* pbfont, gx_device* dev,
             char* font_file_path, bool bBuildGlyph, ref* charstring)
{ /* Stack : <font> <code|name> --> - */
	...
	gs_show_enum* penum_s = (gs_show_enum*)penum;
	/*
	    fixme: the following code needs to optimize with a maintainence of
	   scaled font objects
	    in graphics library and in interpreter. Now we suppose that the
	   renderer
	...
	*/
	...
	int subfont = 0;
	bool is_TT_from_type42 =
	    (pbfont->FontType == ft_TrueType && font_file_path == NULL);
	...
	double FontMatrix_div =
	    (bCID && bIsType1GlyphData && font_file_path == NULL ? 1000 : 1);
	...
	int code;
	enum { SBW_DONE, SBW_SCALE, SBW_FROM_RENDERER } sbw_state = SBW_SCALE;
	...
	if(!SHOW_IS(penum, TEXT_DO_NONE) && !igs->in_charpath) {
		...
		gx_compute_text_oversampling(penum_s, (gs_font*)pbfont,
		                             alpha_bits, &log2_scale);
...
retry_oversampling:
	...
	scale = 1 << I->frac_shift;
	{
		...
		/*  Trick : we need to restore the font scale from ctm,
		   pbfont->FontMatrix,
		    and base_font_matrix. We assume that base_font_matrix is
		    a multiple of pbfont->FontMatrix with a constant from
		   scalefont.
		    But we cannot devide ctm by pbfont->FontMatrix for getting
		    a proper result: the base_font_matrix may be XY
		   transposition,
		    but we must not cut out the transposition from ctm.
		    Therefore we use the norm of base_font_matrix columns as the
		   divisors
		    for X and Y. It is not clear what to do when
		   base_font_matrix is anisotropic
		    (i.e. dx != dy), but we did not meet such fonts before now.
		*/
		font_scale.matrix[0] =
		    (FracInt)(ctm->xx * FontMatrix_div / dx * 72 /
		              dev->HWResolution[0] * scale);
		font_scale.matrix[1] =
		    -(FracInt)(ctm->xy * FontMatrix_div / dy * 72 /
		               dev->HWResolution[0] * scale);
		font_scale.matrix[2] =
		    (FracInt)(ctm->yx * FontMatrix_div / dx * 72 /
		              dev->HWResolution[1] * scale);
		font_scale.matrix[3] =
		    -(FracInt)(ctm->yy * FontMatrix_div / dy * 72 /
		               dev->HWResolution[1] * scale);
		font_scale.matrix[4] =
		    (FracInt)(ctm->tx * FontMatrix_div / dx * 72 /
		              dev->HWResolution[0] * scale);
		font_scale.matrix[5] =
		    (FracInt)(ctm->ty * FontMatrix_div / dy * 72 /
		              dev->HWResolution[1] * scale);
		/* Note: the ctm mapping here is upside down. */
	}
	font_scale.HWResolution[0] = (FracInt)((double)dev->HWResolution[0] *
	                                       font_scale.subpixels[0] * scale);
	font_scale.HWResolution[1] = (FracInt)((double)dev->HWResolution[1] *
	...
	if((code = renderer_retcode(
	        i_ctx_p, I, I->get_scaled_font(
	                        I, &ff, subfont, &font_scale, NULL, bVertical,
	                        (!bCID || (pbfont->FontType != ft_encrypted &&
	                                   pbfont->FontType != ft_encrypted2)
	                             ? FAPI_TOPLEVEL_PREPARED
	                             : FAPI_DESCENDANT_PREPARED)))) < 0)
		return code;
	...
	if(bCID) {
		...
		if(dict_find_string(pdr, "Encoding", &Encoding) > 0 &&
		   (r_has_type(Encoding, t_array) ||
			...
			if(array_get(imemory, Encoding, client_char_code,
			             &char_name) < 0)
				if((code = name_ref(imemory,
				                    (const byte*)".notdef", 7,
				                    &char_name, -1)) < 0)
					return code;
		} else
	...
	if(bCID) {
		if(font_file_path != NULL) {
			...
			code = cid_to_TT_charcode(imemory, Decoding, NULL,
			                          SubstNWP, client_char_code,
			                          &c, &src_type, &dst_type);
			if(code < 0)
				return code;
			cr.char_code = c;
			cr.is_glyph_index = (code == 0);
			/* fixme : process the narrow/wide/proportional mapping
			   type,
			   using src_type, dst_type. Should adjust the 'matrix'
			   above.
		...
		/* This font must not use 'cmap', so compute glyph index from
		 * CharStrings : */
		...
		if(dict_find(CharStrings, &char_name, &glyph_index) < 0) {
			...
			if((code = name_ref(imemory, (const byte*)".notdef", 7,
			                    &char_name, -1)) < 0)
				return code;
		...
		/*  Since the client passes charstring by callback using
		   ff.char_data,
		    the client doesn't need to provide a good cr here.
		    Perhaps since UFST uses char codes as glyph cache keys (UFST
		   4.2 cannot use names),
		    we provide font char codes equal to document's char codes.
		    This trick assumes that Encoding can't point different
		   glyphs
		    for same char code. The last should be true due to
		    PLRM3, "5.9.4 Subsetting and Incremental Definition of
		   Glyphs".
		*/
		if(r_has_type(op, t_integer))
			...
			/*
			 * Reverse Encoding here, because it can be an
			 * incremental one.
			 * Note that this can cause problems with UFST (see the
			 * comment above),
			 * if the encoding doesn't contain the glyph name
			 * rendered with glyphshow.
			 */
			ref* Encoding;
			if(dict_find_string(osp - 1, "Encoding", &Encoding) > 0)
				cr.char_code =
				    (uint)array_find(imemory, Encoding, op);
			else
				return_error(e_invalidfont);
		}
		...
		if((code = renderer_retcode(
		        i_ctx_p, I,
		        I->can_retrieve_char_by_name(
		            I, &ff, &cr, &can_retrieve_char_by_name))) < 0)
			return code;
		if(!can_retrieve_char_by_name) {
			/* Translate from char name to encoding used with 3d
			 * party font technology : */
			ref* Decoding, *char_code;
			if(dict_find_string(osp - 1, "Decoding", &Decoding) >
			       0 &&
			   r_has_type(Decoding, t_dictionary)) {
				if(dict_find(Decoding, &char_name,
				             &char_code) >= 0 &&
				   r_has_type(char_code, t_integer))
					int_param(char_code, 0xFFFF,
					          &cr.char_code);
			}
		}
	}
	...
	if(bCID && !bIsType1GlyphData) {
		...
		if(MetricsCount > 0) {
			const byte* data_ptr;
			int l = get_GlyphDirectory_data_ptr(
			    imemory, pdr, cr.char_code, &data_ptr);

			if(MetricsCount == 2 && l >= 4) {
				if(!bVertical0) {
					cr.sb_x =
					    GET_S16_MSB(data_ptr + 2) * scale;
					cr.aw_x =
					    GET_U16_MSB(data_ptr + 0) * scale;
	...
	if(cr.metrics_type != FAPI_METRICS_REPLACE && bVertical) {
		...
		if(code == metricsNone) {
			if(bCID) {
				...
				cr.sb_y =
				    fapi_round(pbfont->FontBBox.q.y * scale);
				cr.aw_y = fapi_round(-pbfont->FontBBox.q.x *
				                     scale); /* Sic ! */
				cr.metrics_scale =
				    (bIsType1GlyphData ? 1000 : 1);
				cr.metrics_type = FAPI_METRICS_REPLACE;
				sbw[0] = sbw[2] / 2;
	...
	if(SHOW_IS(penum, TEXT_DO_NONE)) {
		if((code = renderer_retcode(
		        i_ctx_p, I, I->get_char_width(I, &ff, &cr, &metrics))) <
		   0)
			return code;
	} else if(igs->in_charpath) {
		if((code = renderer_retcode(
		        i_ctx_p, I, I->get_char_outline_metrics(I, &ff, &cr,
		                                                &metrics))) < 0)
			return code;
	} else {
		...
		if(code == e_limitcheck) {
			if(log2_scale.x > 0 || log2_scale.y > 0) {
				penum_s->fapi_log2_scale.x = log2_scale.x =
				    penum_s->fapi_log2_scale.y = log2_scale.y =
				        0;
				I->release_char_data(I);
				goto retry_oversampling;
			}
			if((code = renderer_retcode(
			        i_ctx_p, I, I->get_char_outline_metrics(
			                        I, &ff, &cr, &metrics))) < 0)
				return code;
		} else {
			if((code = renderer_retcode(i_ctx_p, I, code)) < 0)
	...
	if(cr.metrics_type == FAPI_METRICS_REPLACE) {
		...
		if((code = renderer_retcode(
		        i_ctx_p, I, I->can_replace_metrics(
		                        I, &ff, &cr, &can_replace_metrics))) <
		   0)
			return code;
		if(!can_replace_metrics) {
			/*
			 * The renderer should replace the lsb, but it can't.
			 * To work around we compute a displacement in integral
			 * pixels
			 * and later shift the bitmap to it. The raster will be
			 * inprecise
			...
			gs_distance_transform(
			    (metrics.bbox_x0 / em_scale_x - sbw[0]), 0, ctm,
			    &penum_s->fapi_glyph_shift);
	 ...
	 */
	code = zchar_set_cache(i_ctx_p, pbfont, &char_name, NULL, sbw + 2,
	                       &char_bbox, fapi_finish_render, &exec_cont, sbw);
	...
	if(code != 0) {
		if(code < 0) {
			...
			/* Callout to CDevProc, zsetcachedevice2,
			 * fapi_finish_render. */
		}
	}
	return code;
...
{ /* Stack : <font> <code|name> --> - */
	...
	if(code == 0) {
		...
		if(dict_find_string(osp - 1, "Path", &v) > 0 &&
		   r_has_type(v, t_string))
			font_file_path =
			    ref_to_string(v, imemory, "font file path");
		code = FAPI_do_char(i_ctx_p, pbfont, dev, font_file_path,
		                    bBuildGlyph, charstring);
		if(font_file_path != NULL)
			gs_free_string(imemory, (byte*)font_file_path,
			               r_size(v) + 1, "font file path");
	}
	return code;
}
...
private
int
FAPIBuildGlyph9aux(i_ctx_t* i_ctx_p)
{
...
{ /*  The alghorithm is taken from %Type9BuildGlyph - see gs_cidfn.ps .  */
	...
	ialloc_set_space(idmemory, (r_is_local(op - 3)
	                                ? avm_global
	                                : avm_local)); /* for ztype9mapcid */
	...
	if(code != 0) { /* <font0> <dirty> <dirty> <dirty> */
		...
		pop(2); /* */
		/*  Note that this releases the charstring, and it may be
		   garbage-collected
		    before the interpreter calls fapi_finish_render. This
		   requires the server
		    to keep glyph raster internally between calls to
		   get_char_raster_metrics
		    and get_char_raster. Perhaps UFST cannot provide metrics
		   without
		    building a raster, so this constraint actually goes from
		   UFST.
		*/
	}
...
private
int
do_FAPIpassfont(i_ctx_t* i_ctx_p, char* font_file_path, bool* success)
{
	...
	if(code < 0)
		return code;
	code = FAPI_get_xlatmap(
	    i_ctx_p,
	    &xlatmap); /* Useful for emulated fonts hooked with FAPI. */
	if(code < 0)
		return code;
	...
	*success = false;
	for(; h != 0; h = h->next) {
		...
		if((code = renderer_retcode(i_ctx_p, I, I->ensure_open(I))) < 0)
			return code;
		font_scale.HWResolution[0] = font_scale.HWResolution[1] =
		    72 << I->frac_shift;
		font_scale.matrix[0] = font_scale.matrix[3] = 1
		                                              << I->frac_shift;

		code =
		    FAPI_prepare_font(i_ctx_p, I, pdr, pbfont, font_file_path,
		                      &font_scale, xlatmap, BBox, &decodingID);
		if(code < 0) {
		...
		if((code = name_ref(imemory, (const byte*)I->ig.d->subtype,
		                    strlen(I->ig.d->subtype), &FAPI_ID,
		                    false)) < 0)
			return code;
...
const op_def zfapi_op_defs[] = {{"2.FAPIavailable", zFAPIavailable},
