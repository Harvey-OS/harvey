...
private
int
add_curve(FAPI_path *I, FracInt x0, FracInt y0, FracInt x1, FracInt y1, FracInt x2, FracInt y2)
{
	FAPI_outline_handler *olh = (FAPI_outline_handler *)I->olh;
...
private
FAPI_path path_interface_stub = {NULL, 0, add_move, add_line, add_curve, add_closepath};
...
private
const sfnts_writer sfnts_writer_stub = {
    0, 0, 0,
    sfnts_writer_wbyte,
    sfnts_writer_wword,
    sfnts_writer_wlong,
...
FAPI_FF_get_word(FAPI_font *ff, fapi_font_feature var_id, int index)
{
	...
	case FAPI_FONT_FEATURE_FamilyOtherBlues:
		return float_to_ushort(pfont->data.FamilyOtherBlues.values[index]);
...
private
ushort
get_type1_data(FAPI_font *ff, const ref *type1string,
...
private
ushort
FAPI_FF_get_subr(FAPI_font *ff, int index, byte *buf,
		 uint16_t buf_length)
{
	ref *pdr = (ref *)ff->client_font_data2;
...
private
int
get_GlyphDirectory_data_ptr(const gs_memory_t *mem,
...
private
ushort
FAPI_FF_get_glyph(FAPI_font *ff, int char_code, byte *buf,
		  uint16_t buf_length)
{ /* 
...
private
int
FAPI_prepare_font(i_ctx_t *i_ctx_p, FAPI_server *I, ref *pdr, gs_font_base *pbfont,
...
private
int
FAPI_refine_font(i_ctx_t *i_ctx_p, os_ptr op, gs_font_base *pbfont, const char *font_file_path)
{
...
zFAPIrebuildfont(i_ctx_t *i_ctx_p)
{
	...
	if(pfont->FontType == ft_CID_encrypted && v == NULL) {
		if((code = build_proc_name_refs(imemory, &build, ".FAPIBuildGlyph9", ".FAPIBuildGlyph9")) < 0)
			return code;
	} else if((code = build_proc_name_refs(imemory, &build, ".FAPIBuildChar", ".FAPIBuildGlyph")) < 0)
		return code;
...
private
int
outline_char(i_ctx_t *i_ctx_p, FAPI_server *I, int import_shift_v, gs_show_enum *penum_s, struct gx_path_s *path, bool close_path)
{
...
private
void
compute_em_scale(const gs_font_base *pbfont, FAPI_metrics *metrics, double FontMatrix_div, double *em_scale_x, double *em_scale_y)
...
private
int
FAPI_do_char(i_ctx_t *i_ctx_p, gs_font_base *pbfont, gx_device *dev, char *font_file_path, bool bBuildGlyph, ref *charstring)
...
retry_oversampling:
	...
	if(cr.metrics_type != FAPI_METRICS_REPLACE && bVertical) {
		...
		if(code == metricsNone) {
			...
			cr.metrics_scale = (bIsType1GlyphData ? 1000 : 1);
			cr.metrics_type = (code == metricsSideBearingAndWidth ? FAPI_METRICS_REPLACE : FAPI_METRICS_REPLACE_WIDTH);
			sbw[0] = pwv[2];
			sbw[1] = pwv[3];
			sbw[2] = pwv[0];
			sbw[3] = pwv[1];
