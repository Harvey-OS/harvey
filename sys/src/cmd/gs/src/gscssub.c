...
{
	int index = (int)csi;
	static const uint masks[3] = {(1 << gs_color_space_index_DeviceGray) |
	...
	if(pcs_old == 0) {
		...
		if(pcs == 0 || gs_color_space_get_index(pcs) == csi)
			return 0;
		pcs_new = gs_alloc_struct(pgs->memory, gs_color_space,
		                          &st_color_space,
		...
		gs_cspace_assign(
		    pgs->device_color_spaces.indexed[index],
		    (pcs ? pcs
...
gs_current_DeviceGray_space(const gs_state* pgs)
{
	...
	return (!pgs->device->UseCIEColor ||
	            ...
	            ? pgs->shared->device_color_spaces.named.Gray
	            : pcs);
}
const gs_color_space*
gs_current_DeviceRGB_space(const gs_state* pgs)
{
	...
	return (!pgs->device->UseCIEColor ||
	            ...
	            ? pgs->shared->device_color_spaces.named.RGB
	            : pcs);
}
const gs_color_space*
gs_current_DeviceCMYK_space(const gs_state* pgs)
{
	...
	return (!pgs->device->UseCIEColor ||
	            ...
	            ? pgs->shared->device_color_spaces.named.CMYK
	            : pcs);
}
