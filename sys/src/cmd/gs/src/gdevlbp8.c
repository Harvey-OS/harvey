...
/* Copyright (C) 1991, 1994, 1996, 1997 Aladdin Enterprises.  All rights
  reserved.
...
/*
  Modifications:
    2.2.97  Lauri Paatero
            Changed CSI command into ESC [. DCS commands may still need to be
changed
            (to ESC P).
...
problems
    ...
    18.6.96 Lauri Paatero, lauri.paatero@paatero.pp.fi
            ...
            Added logic to recognize (and optimize away) long strings of 00's in
data.
            For LBP-8II removed use of 8-bit CSI (this does not work if 8-bit
character
                set has been configured in LBP-8II. (Perhaps this should also be
done
                for LBP-8III?)
  Original versions:
...
private
dev_proc_print_page(lbp8_print_page);
private
dev_proc_print_page(lips3_print_page);

const gx_device_printer far_data gs_lbp8_device = prn_device(
    prn_std_procs, "lbp8", DEFAULT_WIDTH_10THS, DEFAULT_HEIGHT_10THS, X_DPI,
    Y_DPI, 0.16, 0.2, 0.32, 0.21, /* margins: left, bottom, right, top */
    1, lbp8_print_page);

const gx_device_printer far_data gs_lips3_device =
    prn_device(prn_std_procs, "lips3", 82,           /* width_10ths, 8.3" */
               117,                                  /* height_10ths, 11.7" */
               X_DPI, Y_DPI, 0.16, 0.27, 0.23, 0.27, /* margins */
               1, lips3_print_page);
...
static const char lips3_init[] = {
    ESC, '<',                                             /* soft reset */
    DCS, '0', 'J', ST,                                    /* JOB END */
    DCS, '3', '1', ';', '3', '0', '0', ';', '2', 'J', ST, /* 300dpi, LIPS3 JOB
                                                             START */
    ESC, '<',                                             /* soft reset */
    DCS, '2', 'y', 'P', 'r', 'i', 'n', 't', 'i', 'n', 'g', '(',
    'g', 's', ')', ST,       /* Printing (gs) display */
...
private
int
can_print_page(gx_device_printer* pdev, FILE* prn_stream, const char* init,
               int init_size, const char* end, int end_size)
{
		...
		for(lnum = 0; lnum < pdev->height; lnum++) {
			...
			if(end_data != data) {
				...
				while(out_data < end_data) {
					...
					/* if there is a lot data, find if there
					 * is sequence of zeros */
					if(out_count > 22) {

						out_count = 1;

						while(out_data + out_count +
						          zero_count <
						      end_data) {
							if(out_data
							       [zero_count +
