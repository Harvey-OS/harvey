...
private
dev_proc_print_page(atx23_print_page);
private
dev_proc_print_page(atx24_print_page);
private
dev_proc_print_page(atx38_print_page);
...
const gx_device_printer gs_atx23_device = /* real width = 576 pixels */
    ATX_DEVICE("atx23", 28 /* 2.84" */, 35 /* (minimum) */, 203, 0.25, 0.125,
               atx23_print_page);

const gx_device_printer gs_atx24_device = /* real width = 832 pixels */
    ATX_DEVICE("atx24", 41 /* 4.1" */, 35 /* (minimum) */, 203, 0.193, 0.125,
               atx24_print_page);

const gx_device_printer gs_atx38_device = /* real width = 2400 pixels */
    ATX_DEVICE("atx38", 80 /* 8.0" */, 35 /* (minimum) */, 300, 0.25, 0.125,
               atx38_print_page);
...
Scan_Compressed:
	if(pair_count == MAX_COMPRESSED_SEGMENT_PAIRS || in == in_end ||
	   in[0] != in[-1] || in[1] != in[0]) { /* end the segment */
...
Scan_Uncompressed:
	if(pair_count == MAX_UNCOMPRESSED_SEGMENT_PAIRS || in == in_end ||
	   in[1] == in[0]) { /* end the segment */
...
atx_print_page(gx_device_printer* pdev, FILE* f, int max_width_bytes)
{
	 ...
	 */
	float top_bottom_skip =
	    (pdev->HWMargins[1] + pdev->HWMargins[3]) / 72.0;
	 ...
	 */
	int compressed_raster =
	    min(raster / 2, 510); /* require 50% compression */
	byte* compressed;
	...
	for(blank_lines = 0, lnum = 0; lnum < height; ++lnum) {
		...
		count =
		    atx_compress(row, end - row, compressed, compressed_raster);
		if(count >= 0) { /* compressed line */
			         /*
			          * Note that since compressed_raster can't exceed 510,
			          * count
