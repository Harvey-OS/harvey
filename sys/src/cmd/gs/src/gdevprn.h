...
/* Copyright (C) 1989, 1995, 1996, 1997, 1998, 1999, 2000, 2001 Aladdin
...
#define PRN_MIN_MEMORY_LEFT                                                    \
	(gs_debug_c('.') ? PRN_MIN_MEMORY_LEFT_SMALL                           \
...
#define prn_dev_proc_print_page(proc) int proc(gx_device_printer*, FILE*)
...
#define prn_dev_proc_open_render_device(proc) int proc(gx_device_printer*)
	prn_dev_proc_open_render_device((*open_render_device));

#define prn_dev_proc_close_render_device(proc) int proc(gx_device_printer*)
...
#define prn_dev_proc_buffer_page(proc) int proc(gx_device_printer*, FILE*, int)
...
struct gdev_prn_space_params_s {
	...
	bool params_are_read_only; /* true if put_params may not modify this
	                              struct */
	gdev_prn_banding_type
	    banding_type; /* used to force banding or bitmap */
};
...
#define gx_prn_device_common                                                   \
	...
	bool ReopenPerPage;                                                    \
	bool                                                                   \
	    page_uses_transparency; /* PDF 1.4 transparency is used on page */ \
	bool Duplex;                                                           \
	...
	proc_free_up_bandlist_memory((                                         \
	    *free_up_bandlist_memory)); /* if nz, proc to free some bandlist   \
	                                   memory */                           \
	gx_page_queue_t*                                                       \
	    page_queue; /* if <> 0,page queue for gdevprna NOT GC'd */         \
	bool                                                                   \
	    is_async_renderer; /* device is only the rendering part of async   \
	                          device */                                    \
	gx_device_printer*                                                     \
	    async_renderer; /* in async writer, pointer to async renderer */   \
...
#define public_st_device_printer() /* in gdevprn.c */                          \
	gs_public_st_complex_only(                                             \
	    st_device_printer, gx_device_printer, "gx_device_printer", 0,      \
	    device_printer_enum_ptrs, device_printer_reloc_ptrs,               \
	    gx_device_finalize)
...
prn_dev_proc_start_render_thread(gx_default_start_render_thread); /* for async
                                                                     rendering
                                                                     only, see
                                                                     gdevprna.c
                                                                     */
...
#define prn_procs(p_open, p_output_page, p_close)                              \
	prn_color_procs(p_open, p_output_page, p_close,                        \
	                gdev_prn_map_rgb_color, gdev_prn_map_color_rgb)
#define prn_params_procs(p_open, p_output_page, p_close, p_get_params,         \
                         p_put_params)                                         \
	prn_color_params_procs(p_open, p_output_page, p_close,                 \
	                       gdev_prn_map_rgb_color, gdev_prn_map_color_rgb, \
	                       p_get_params, p_put_params)
#define prn_color_procs(p_open, p_output_page, p_close, p_map_rgb_color,       \
                        p_map_color_rgb)                                       \
	prn_color_params_procs(p_open, p_output_page, p_close,                 \
	                       p_map_rgb_color, p_map_color_rgb,               \
	                       gdev_prn_get_params, gdev_prn_put_params)
/* See gdev_prn_open for explanation of the NULLs below. */
#define prn_color_params_procs(p_open, p_output_page, p_close,                 \
                               p_map_rgb_color, p_map_color_rgb, p_get_params, \
                               p_put_params)                                   \
	{                                                                      \
		p_open, NULL, /* get_initial_matrix */                         \
		    NULL,     /* sync_output */                                \
		    p_output_page, p_close, p_map_rgb_color, p_map_color_rgb,  \
		    NULL,                             /* fill_rectangle */     \
		    NULL,                             /* tile_rectangle */     \
		    NULL,                             /* copy_mono */          \
		    NULL,                             /* copy_color */         \
		    NULL,                             /* draw_line */          \
		    NULL,                             /* get_bits */           \
		    p_get_params, p_put_params, NULL, /* map_cmyk_color */     \
...
#define prn_device_body_rest2_(print_page, print_page_copies)                  \
	...
	,        /* std_procs */                                               \
	    ...
	    {print_page,                                                       \
	     print_page_copies,                                                \
	     {gx_default_create_buf_device, gx_default_size_buf_device,        \
	      gx_default_setup_buf_device, gx_default_destroy_buf_device},     \
	    ...
	    0, 0, 0, 0, 0 /*false*/, 0,                                        \
	    0, /* buffer_memory ... clist_dis'_mask */                         \
	{                                                                      \
		0                                                              \
	} /* ... orig_procs */
...
#define prn_device_margins_body(dtype, procs, dname, w10, h10, xdpi, ydpi, lo, \
                                to, lm, bm, rm, tm, ncomp, depth, mg, mc, dg,  \
                                dc, print_page)                                \
	std_device_full_body_type(                                             \
	    dtype, &procs, dname, &st_device_printer,                          \
	    (int)((float)(w10) * (xdpi) / 10 + 0.5),                           \
	    (int)((float)(h10) * (ydpi) / 10 + 0.5), xdpi, ydpi, ncomp, depth, \
	    mg, mc, dg, dc, (float)(-(lo) * (xdpi)), (float)(-(to) * (ydpi)),  \
	    (float)((lm)*72.0), (float)((bm)*72.0), (float)((rm)*72.0),        \
	    (float)((tm)*72.0)),                                               \
	    prn_device_body_rest_(print_page)

#define prn_device_body(dtype, procs, dname, w10, h10, xdpi, ydpi, lm, bm, rm, \
                        tm, ncomp, depth, mg, mc, dg, dc, print_page)          \
	prn_device_margins_body(dtype, procs, dname, w10, h10, xdpi, ydpi, lm, \
	                        tm, lm, bm, rm, tm, ncomp, depth, mg, mc, dg,  \
	                        dc, print_page)

#define prn_device_margins_body_extended(dtype, procs, dname, w10, h10, xdpi,  \
                                         ydpi, lo, to, lm, bm, rm, tm, mcomp,  \
                                         ncomp, pol, depth, gi, mg, mc, dg,    \
                                         dc, ef, cn, print_page)               \
	std_device_full_body_type_extended(                                    \
	    dtype, &procs, dname, &st_device_printer,                          \
	    (int)((long)(w10) * (xdpi) / 10),                                  \
	    (int)((long)(h10) * (ydpi) / 10), xdpi, ydpi, mcomp, ncomp, pol,   \
	    depth, gi, mg, mc, dg, dc, ef, cn, -(lo) * (xdpi), -(to) * (ydpi), \
	    (lm)*72.0, (bm)*72.0, (rm)*72.0, (tm)*72.0),                       \
	    prn_device_body_rest_(print_page)

#define prn_device_body_extended(dtype, procs, dname, w10, h10, xdpi, ydpi,    \
                                 lm, bm, rm, tm, mcomp, ncomp, pol, depth, gi, \
                                 mg, mc, dg, dc, ef, cn, print_page)           \
	prn_device_margins_body_extended(                                      \
	    dtype, procs, dname, w10, h10, xdpi, ydpi, lm, tm, lm, bm, rm, tm, \
	    mcomp, ncomp, pol, depth, gi, mg, mc, dg, dc, ef, cn, print_page)

#define prn_device_std_margins_body(dtype, procs, dname, w10, h10, xdpi, ydpi, \
                                    lo, to, lm, bm, rm, tm, color_bits,        \
                                    print_page)                                \
	std_device_std_color_full_body_type(                                   \
	    dtype, &procs, dname, &st_device_printer,                          \
	    (int)((float)(w10) * (xdpi) / 10 + 0.5),                           \
	    (int)((float)(h10) * (ydpi) / 10 + 0.5), xdpi, ydpi, color_bits,   \
	    (float)(-(lo) * (xdpi)), (float)(-(to) * (ydpi)),                  \
	    (float)((lm)*72.0), (float)((bm)*72.0), (float)((rm)*72.0),        \
	    (float)((tm)*72.0)),                                               \
	    prn_device_body_rest_(print_page)

#define prn_device_std_body(dtype, procs, dname, w10, h10, xdpi, ydpi, lm, bm, \
                            rm, tm, color_bits, print_page)                    \
	prn_device_std_margins_body(dtype, procs, dname, w10, h10, xdpi, ydpi, \
	                            lm, tm, lm, bm, rm, tm, color_bits,        \
	                            print_page)

#define prn_device_std_margins_body_copies(dtype, procs, dname, w10, h10,      \
                                           xdpi, ydpi, lo, to, lm, bm, rm, tm, \
                                           color_bits, print_page_copies)      \
	std_device_std_color_full_body_type(                                   \
	    dtype, &procs, dname, &st_device_printer,                          \
	    (int)((float)(w10) * (xdpi) / 10 + 0.5),                           \
	    (int)((float)(h10) * (ydpi) / 10 + 0.5), xdpi, ydpi, color_bits,   \
...
#define prn_device_std_body_copies(dtype, procs, dname, w10, h10, xdpi, ydpi,  \
                                   lm, bm, rm, tm, color_bits,                 \
                                   print_page_copies)                          \
	prn_device_std_margins_body_copies(dtype, procs, dname, w10, h10,      \
	                                   xdpi, ydpi, lm, tm, lm, bm, rm, tm, \
	                                   color_bits, print_page_copies)

/* Note that the following macros add { } around the data. */

#define prn_device_margins(procs, dname, w10, h10, xdpi, ydpi, lo, to, lm, bm, \
                           rm, tm, color_bits, print_page)                     \
	{                                                                      \
		prn_device_std_margins_body(                                   \
		    gx_device_printer, procs, dname, w10, h10, xdpi, ydpi, lo, \
		    to, lm, bm, rm, tm, color_bits, print_page)                \
	}

#define prn_device(procs, dname, w10, h10, xdpi, ydpi, lm, bm, rm, tm,         \
                   color_bits, print_page)                                     \
	prn_device_margins(procs, dname, w10, h10, xdpi, ydpi, lm, tm, lm, bm, \
	                   rm, tm, color_bits, print_page)

#define prn_device_margins_copies(procs, dname, w10, h10, xdpi, ydpi, lo, to,  \
                                  lm, bm, rm, tm, color_bits,                  \
                                  print_page_copies)                           \
	{                                                                      \
		prn_device_std_margins_body_copies(                            \
		    gx_device_printer, procs, dname, w10, h10, xdpi, ydpi, lo, \
		    to, lm, bm, rm, tm, color_bits, print_page_copies)         \
	}

#define prn_device_copies(procs, dname, w10, h10, xdpi, ydpi, lm, bm, rm, tm,  \
                          color_bits, print_page_copies)                       \
	prn_device_margins_copies(procs, dname, w10, h10, xdpi, ydpi, lm, tm,  \
	                          lm, bm, rm, tm, color_bits,                  \
	                          print_page_copies)
...
int gx_page_info_colors_used(const gx_device* dev,
                             const gx_band_page_info_t* page_info, int y,
                             int height, gx_colors_used_t* colors_used,
                             int* range_start);
 ...
 * an error.
 */
int gdev_prn_render_rectangle(gx_device_printer* pdev, const gs_int_rect* prect,
                              gx_device* target,
 ...
 * returned.
 */
int gdev_prn_get_lines(gx_device_printer* pdev, int y, int height, byte* buffer,
                       uint bytes_per_line, byte** actual_buffer,
...
int gdev_prn_allocate_memory(gx_device* pdev, gdev_prn_space_params* space,
                             int new_width, int new_height);
int gdev_prn_reallocate_memory(gx_device* pdev, gdev_prn_space_params* space,
                               int new_width, int new_height);
...
int gdev_create_buf_device(create_buf_device_proc_t cbd_proc, gx_device** pbdev,
                           gx_device* target,
                           const gx_render_plane_t* render_plane,
                           gs_memory_t* mem, bool for_band);

/* BACKWARD COMPATIBILITY */
#define dev_print_scan_lines(dev) gdev_prn_print_scan_lines((gx_device*)(dev))
...
void gdev_prn_init_color(gx_device*, int, dev_proc_map_rgb_color((*)),
...
#define prn_device_type_mono(dtname, dname, initproc, pageproc)                \
	...
	dev_proc_print_page(pageproc);                                         \
      private                                                                  \
	int initproc(gx_device* dev)                                           \
	{                                                                      \
		return gdev_prn_initialize(dev, dname, pageproc);              \
	}                                                                      \
	device_type(dtname, st_prn_device, initproc)

#define prn_device_type_color(dtname, dname, depth, initproc, pageproc,        \
                              rcproc, crproc)                                  \
      private                                                                  \
	dev_proc_print_page(pageproc);                                         \
      private                                                                  \
