...
/* Copyright (C) 1992, 1995, 1996, 1997, 1998, 1999, 2000 Aladdin Enterprises.
  All rights reserved.
...
{
	file_enum* pfen = gs_alloc_struct(mem, file_enum, &st_file_enum,
	                                  "gp_enumerate_files");
	...
	pattern =
	    (char*)gs_alloc_bytes(mem, pat_size, "gp_enumerate_files(pattern)");
	if(pattern == 0)
		return 0;
...
gp_enumerate_files_next(file_enum* pfen, char* ptr, uint maxlen)
{
	...
	if(pfen->first_time) {
		...
		rc = DosFindNext(pfen->hdir, &pfen->findbuf,
		                 sizeof(pfen->findbuf), &cFilenames);
	}
	if(rc)
		return -1;
...
gp_enumerate_files_close(file_enum* pfen)
{
	...
	gs_free_object(mem, pfen->pattern, "gp_enumerate_files_close(pattern)");
...
/* ------ Printer accessing ------ */
private
int is_os2_spool(const char* queue);
...
gp_open_printer(char fname[gp_file_name_sizeof], int binary_mode)
{
	...
	if((strlen(fname) == 0) || is_os2_spool(fname)) {
		if(isos2) {
			/* default or spool */
			if(pm_spool(NULL,
			            fname)) /* check if spool queue valid */
				return NULL;
			pfile = gp_open_scratch_file(
			    gp_scratch_file_name_prefix, pm_prntmp,
			    (binary_mode ? "wb" : "w"));
		} else
			...
			if(splerr == NO_ERROR) {
				...
				for(i = 0; i < cReturned; i++) {
					if(queue_name) {
						...
						if(strcmp(prq->pszName,
						          queue_name) == 0) {
							char* p;

							for(p = prq->pszDriverName;
							    *p && (*p != '.');
							    p++)
								/* do nothing */;
							*p = '\0'; /* truncate
