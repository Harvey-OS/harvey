...
dynamic_grow(da_ptr pda, byte *next, uint max_size)
{
	...
	while((code = dynamic_resize(pda, new_size)) < 0 &&
	      new_size > old_size) { /* Try trimming down the requested new size. */
...
#define try_number sstate.s_ss.s_name.s_try_number
	...
	if(pstate->s_scan_type != scanning_none) {
		...
		case scanning_binary:
			retcode = (*sstate.s_ss.binary.cont)(i_ctx_p, s, myref, &sstate);
			scan_begin_inline();
...
top:
	...
	cont_string:
		for(;;) {
			...
			w.ptr = da.next - 1;
			w.limit = da.limit - 1;
			status = (*sstate.s_ss.st.template->process)(&sstate.s_ss.st, &s->cursor.r, &w,
								     s->end_status == EOFC);
			if(!check_only)
				da.next = w.ptr + 1;
			switch(status) {
	...
	case '}':
		...
		osp--;
		{
			...
			if_debug4('S', "[S}]d=%d, s=%d->%ld, c=%d\n",
				  pdepth, pstack,
				  (pstack == pdepth ? 0 : ref_stack_index(&o_stack, size)->value.intval),
				  size + pstack);
...
#define case4(c)    \
	...
	case c + 2: \
	case c + 3
		case4(128) : case4(132) : case4(136) : case4(140) : case4(144) : case4(148) : case4(152) : case4(156) :
#undef case4
															if(recognize_btokens())
		{
			scan_end_inline();
