...
dynamic_resize(da_ptr pda, uint new_size)
{
	...
	if(pda->is_dynamic) {
		base = gs_resize_string(mem, pda->base, old_size, new_size,
		                        "scanner");
		if(base == 0)
...
dynamic_grow(da_ptr pda, byte* next, uint max_size)
{
	uint old_size = da_size(pda);
	uint new_size =
	    (old_size < 10 ? 20 : old_size >= (max_size >> 1) ? max_size
	                                                      : old_size << 1);
	int code;
	...
	while((code = dynamic_resize(pda, new_size)) < 0 &&
	      new_size >
	          old_size) { /* Try trimming down the requested new size. */
...
ENUM_PTRS_END
private
RELOC_PTRS_WITH(scanner_reloc_ptrs, scanner_state* ssptr)
{
	if(ssptr->s_scan_type != scanning_none && ssptr->s_da.is_dynamic) {
		gs_string sda;
		...
		ssptr->s_da.next =
		    sda.data + (ssptr->s_da.next - ssptr->s_da.base);
                   ...
                   bool save, bool push_file, op_proc_t cont)
{
	...
	if(s->end_status == EOFC) {
		/* More data needed, but none available, so this is a syntax
		 * error. */
	...
	case CALLC: {
		...
		return s_handle_read_exception(i_ctx_p, status, fop, rstate,
		                               nstate, cont);
	}
	}
...
int
scan_string_token_options(i_ctx_t* i_ctx_p, ref* pstr, ref* pref, int options)
{
	stream st;
	stream* s = &st;
	scanner_state state;
...
#define spop1()                                                                \
	...
	int max_name_ctype = (recognize_btokens() ? ctype_name : ctype_btoken);
...
#define ensure2(styp) ensure2_back(styp, 1)
	...
	const bool PDFScanRules =
	    (i_ctx_p->scanner_options & SCAN_PDF_RULES) != 0;
	const bool PDFScanInvNum =
	    (i_ctx_p->scanner_options & SCAN_PDF_INV_NUM) != 0;
...
#define try_number sstate.s_ss.s_name.s_try_number
	...
	if(pstate->s_scan_type != scanning_none) {
		...
		case scanning_binary:
			retcode = (*sstate.s_ss.binary.cont)(i_ctx_p, s, myref,
			                                     &sstate);
			scan_begin_inline();
...
top:
	...
	case '}':
		...
		osp--;
		{
			...
			if_debug4('S', "[S}]d=%d, s=%d->%ld, c=%d\n", pdepth,
			          pstack, (pstack == pdepth
			                       ? 0
			                       : ref_stack_index(&o_stack, size)
			                             ->value.intval),
			          size + pstack);
			...
			if(check_only) {
				...
				retcode = ialloc_ref_array(
				    myref, a_executable + a_all, size,
				    "scanner(proc)");
				if(retcode < 0) { /* must be VMerror */
					osp++;
					scan_putback();
				...
				retcode =
				    ref_stack_store(&o_stack, myref, size, 0, 1,
			...
			if(pstack ==
			   pdepth) { /* This was the top-level procedure. */
				spop1();
				pstack = 0;
			} else {
				if(osp < osbot)
	...
	case '%': { /* Scan as much as possible within the buffer. */
		...
		for(;;) {
			...
			default:
				if(c < 0)
					...
					case EOFC:
						/*
						 * One would think that an EOF
						 * in a comment
						 * should be a syntax error, but
						 * there are
						 * quite a number of files that
						 * end that way.
						 */
						goto end_comment;
					default:
			...
			end_comment:
				retcode =
				    scan_comment(i_ctx_p, myref, &sstate,
...
#undef case4
	...
	default:
		if(c < 0) {
			dynamic_init(
			    &da,
			    name_memory(imemory)); /* da state must be clean */
			scan_type = scanning_none;
			goto pause;
		}
	...
	cont_name:
		...
		while(decoder[c = scan_getc()] <= max_name_ctype) {
			if(daptr == da.limit) {
				retcode =
				    dynamic_grow(&da, daptr, name_max_string);
				if(retcode < 0) {
					dynamic_save(&da);
					if(retcode != e_VMerror)
						sreturn(retcode);
	...
	nx:
		...
		case ctype_space:
			...
			if(c == char_CR) {
				if(sptr >=
				   endptr) { /* ensure2 */ /* We have to check
				                              specially for */
		...
		if(da
		       .is_dynamic) { /* We've already allocated the string on
		                         the heap. */
			...
			if(retcode >= 0) {
				...
				retcode =
				    name_ref(imemory, da.base, size, myref, 2);
			}
		} else {
			retcode =
			    name_ref(imemory, da.base, (uint)(daptr - da.base),
			             myref, !s->foreign);
		}
	...
	if(retcode < 0) {
		...
		if(pstack != 0) {
			if(retcode == e_undefined)
				*pref =
				    *osp; /* return undefined name as error
				             token */
