                  ...
                  gs_type1_data* pdata1)
{
	...
	if((code = dict_uint_param(pfr->Private, "gsubrNumberBias", 0, max_uint,
	                           pdata1->gsubrNumberBias,
	                           &pdata1->gsubrNumberBias)) < 0 ||
	   (code = dict_float_param(pfr->Private, "defaultWidthX", 0.0, &dwx)) <
	       0 ||
	   (code = dict_float_param(pfr->Private, "nominalWidthX", 0.0, &nwx)) <
	       0)
		return code;
	...
	pdata1->nominalWidthX = float2fixed(nwx);
	{
		...
		if(dict_find_string(pfr->Private, "initialRandomSeed", &pirs) <=
		   0)
...
zbuildfont2(i_ctx_t* i_ctx_p)
{
	...
	int code = build_proc_name_refs(imemory, &build, "%Type2BuildChar",
	                                "%Type2BuildGlyph");
	gs_type1_data data1;
...
const op_def zfont2_op_defs[] = {{"2.buildfont2", zbuildfont2}, op_def_end(0)};
