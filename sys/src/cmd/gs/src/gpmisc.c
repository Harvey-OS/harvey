...
private
inline uint
search_separator(const char** ip, const char* ipe, const char* item,
                 int direction)
{
	uint slen = 0;
	for(slen = 0; (*ip - ipe) * direction < 0; (*ip) += direction)
		if((slen = gs_file_name_check_separator(*ip, ipe - *ip,
		                                        item)) != 0)
			break;
	return slen;
}
...
gp_file_name_combine_generic(const char* prefix, uint plen, const char* fname,
                             uint flen, bool no_sibling, char* buffer,
                             uint* blen)
{
	/*
	...
	slen = gs_file_name_check_separator(bp, buffer - bp,
	                                    bp); /* Backward search. */
	if(rlen != 0 && slen == 0) {
	...
	for(;;) {
		...
		if(ilen == 0 && !gp_file_name_is_empty_item_meanful()) {
			...
			/* Skip the reference to 'current', except the starting
			 * one.
			...
			if(!append(&bp, bpe, &item, ilen))
				return gp_combine_small_buffer;
			/* The 'item' pointer is now broken; it may be restored
			 * using 'ilen'. */
		} else if(bp == buffer + rlen + infix_len) {
			/* Input is a parent and the output only contains a root
			 * and an infix. */
			if(rlen != 0)
			...
			case 1:
				...
				bp = buffer +
				     rlen; /* Drop the old infix, if any. */
				infix_len = 0;
			/* Falls through. */
			case 0:
				/* We have no infix, start with parent. */
				if((no_sibling && ipe == fname + flen &&
				    flen != 0) ||
			...
			infix_type = 2;
			infix_len += ilen;
			/* Recompute the separator length. We cannot use the old
			 * slen on Mac OS. */
			slen = gs_file_name_check_separator(ip, ipe - ip, ip);
		} else {
			/* Input is a parent and the output continues after
			 * infix. */
			/* Unappend the last separator and the last item. */
			uint slen1 = gs_file_name_check_separator(
			    bp, buffer + rlen - bp, bp); /* Backward search. */
			char* bie = bp - slen1;

			bp = bie;
			DISCARD(search_separator((const char**)&bp,
			                         buffer + rlen, bp, -1));
			/* The cast above quiets a gcc warning. We believe it's
			 * a bug in the compiler. */
			/* Skip the input with separator. We cannot use slen on
			 * Mac OS. */
		...
		if(ip == ipe) {
			if(ipe == fname + flen) {
				/* All done.
				 * Note that the case (prefix + plen == fname &&
				 * flen == 0)
				...
				*blen = bp - buffer;
				...
				if(slen == 0) {
					...
					slen = search_separator(&ip, ipe, fname,
					                        1);
					sep =
					    (slen != 0
					         ? gp_file_name_directory_separator()
...
{
	return gp_file_name_combine(fname, flen, fname + flen, 0, false, buffer,
	                            blen);
}
                    ...
                    bool (*test)(const char* fname, uint flen))
{
	...
	const char* ip, *ipe;
	const char* item =
	    fname; /* plen == flen could cause an indeterminizm. */
