...
int
psdf_curveto(gx_device_vector* vdev, floatp x0, floatp y0, floatp x1, floatp y1,
...
int
psdf_closepath(gx_device_vector* vdev, floatp x0, floatp y0, floatp x_start,
               ...
               const psdf_set_color_commands_t* ppscc)
{
		...
		return_error(gs_error_rangecheck);
	{
		 ...
		 * generates incorrect code for Intel CPUs if we do this.  The
		 * code
...
#define BUF_SIZE 100 /* arbitrary */
		byte* buf =
		    gs_alloc_bytes(mem, BUF_SIZE, "psdf_begin_binary(buf)");
		stream_A85E_state* ss = (stream_A85E_state*)s_alloc_state(
		    mem, s_A85E_template.stype,
...
psdf_DCT_filter(gs_param_list* plist /* may be NULL */,
                stream_state /*stream_DCTE_state*/* st, int Columns, int Rows,
                int Colors, psdf_binary_writer* pbw /* may be NULL */)
{
	...
	if((code = param_write_int((gs_param_list*)&rcc_list, "Rows", &Rows)) <
	       0 ||
...
psdf_CFE_binary(psdf_binary_writer* pbw, int w, int h, bool invert)
{
	...
	stream_CFE_state* st = gs_alloc_struct(
	    mem, stream_CFE_state, template->stype, "psdf_CFE_binary");
	int code;
...
int
psdf_get_bits_rectangle(gx_device* dev, const gs_int_rect* prect,
 ...
 */
int
psdf_create_compositor(gx_device* dev, gx_device** pcdev,
