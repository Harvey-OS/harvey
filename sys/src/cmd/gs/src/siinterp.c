...
typedef struct stream_IIEncode_state_s {
	...
	void /*PixelOut */*
	    prev; /* previous row of input data in output fmt, */
...
gs_private_st_ptrs2(st_IIEncode_state, stream_IIEncode_state,
                    "ImageInterpolateEncode state", iiencode_state_enum_ptrs,
                    iiencode_state_reloc_ptrs, prev, cur);

/* Forward references */
private
void s_IIEncode_release(stream_state* st);
	    ...
	    (ss->params.BitsPerComponentIn == 8
	         ? (ss->params.BitsPerComponentOut == 8
	                ? (ss->params.MaxValueIn == ss->params.MaxValueOut
	                       ? SCALE_SAME
	                       : SCALE_8_8)
	                : (ss->params.MaxValueIn == 255 &&
	                           ss->params.MaxValueOut == frac_1
	                       ? (ss->params.Colors == 3
	                              ? SCALE_8_16_BYTE2FRAC_3
	                              : SCALE_8_16_BYTE2FRAC)
	                       : SCALE_8_16_GENERAL))
...
top:
	...
	if(ss->src_offset < ss->src_size) {
		uint count =
		    min(ss->src_size - ss->src_offset, pr->limit - pr->ptr);
