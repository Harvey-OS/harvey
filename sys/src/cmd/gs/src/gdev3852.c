...
/* Copyright (C) 1989, 1992, 1993, 1996, 1999 Aladdin Enterprises.  All rights
  reserved.
...
private
dev_proc_print_page(jetp3852_print_page);
private
gx_device_procs jetp3852_procs =
    prn_color_procs(gdev_prn_open, gdev_prn_output_page, gdev_prn_close,
                    gdev_pcl_3bit_map_rgb_color, gdev_pcl_3bit_map_color_rgb);
const gx_device_printer far_data gs_jetp3852_device = prn_device(
    jetp3852_procs, "jetp3852", 86, /* width_10ths, 8.6" (?) */
    110,                            /* height_10ths, 11" */
    X_DPI, Y_DPI, 0.0, 0, 0.0, 0,   /* left, bottom, right, top margins */
		    ...
		    c * 0x10100L, c * 0x10101L                                 \
				...
				if(num_blank_lines > 0) {
					if(lnum == 0) { /* Skip down the page
						...
						vtp = num_blank_lines;
						fprintf(prn_stream, "\033e%c",
						        vtp);
						num_blank_lines = 0;
					}
				}

				/* Transfer raster graphics in the order R, G,
				 * B. */
				/* Apparently it is stored in B, G, R */
				/* Calculate the amount of data to send by what
				 */
				/* Ghostscript tells us the scan line_size in
				 * (bytes) */
				...
				fprintf(prn_stream, "\033[O%c%c\200\037", cntc2,
				        cntc1);
				...
				for(row = plane_data + LINE_SIZE * 2, i = 0;
					...
					odp = row;
					/* Complement bytes */
					for(jj = 0; jj < line_size_color_plane;
					    jj++) {
						ctemp = *odp;
						*odp++ = ~ctemp;
					}
