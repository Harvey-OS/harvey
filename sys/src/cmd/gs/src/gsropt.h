...
#define rop2_D_shift 1
	rop2_1 = 0xf,
#define rop2_operand(shift, d, s) ((shift) == 2 ? (s) : (d))
	rop2_default = rop2_S
} gs_rop2_t;
...
#define rop3_use_D_when_1_(op, mask) (((op) & ~mask) | (rop3_D & mask))
 ...
 */
#define rop3_uses_(op, mask, shift) (((((op) << shift) ^ (op)) & mask) != 0)
 ...
 */
#define rop3_is_idempotent(op) !((op) & ~((op) << rop3_D_shift) & rop3_D)
...
#define lop_default                                                            \
	(rop3_default | (source_transparent_default ? lop_S_transparent : 0) | \
...
#define lop_uses_S(lop) (rop3_uses_S(lop) || ((lop)&lop_S_transparent))
#endif
#define lop_uses_T(lop) (rop3_uses_T(lop) || ((lop)&lop_T_transparent))
/* Test whether a logical operation just sets D = x if y = 0. */
#define lop_no_T_is_S(lop)                                                     \
	(((lop) & (lop_S_transparent | (rop3_1 - rop3_T))) ==                  \
	 (rop3_S & ~rop3_T))
#define lop_no_S_is_T(lop)                                                     \
	(((lop) & (lop_T_transparent | (rop3_1 - rop3_S))) ==                  \
	 (rop3_T & ~rop3_S))
 ...
 */
#define lop_know_S_0(lop) (rop3_know_S_0(lop) & ~lop_S_transparent)
#define lop_know_T_0(lop) (rop3_know_T_0(lop) & ~lop_T_transparent)
