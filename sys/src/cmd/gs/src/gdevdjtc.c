...
private
dev_proc_print_page(djet500c_print_page);

private
gx_device_procs djet500c_procs =
    prn_color_procs(gdev_prn_open, gdev_prn_output_page, gdev_prn_close,
                    gdev_pcl_3bit_map_rgb_color, gdev_pcl_3bit_map_color_rgb);

const gx_device_printer far_data gs_djet500c_device =
    prn_device(djet500c_procs, "djet500c", 85,       /* width_10ths, 8.5" */
               120,                                  /* height_10ths, 12" */
               X_DPI, Y_DPI, 0.25, 0.25, 0.25, 0.25, /* margins */
               3, djet500c_print_page);

/* Forward references */
private
int djet500c_print_page(gx_device_printer*, FILE*);
	...
	/* Send each scan line in turn */
	{
		...
		for(lnum = 0; lnum < pdev->height; lnum++) {
			...
			if(endData == bitData)
				...
				lineLen = ((endData - bitData) + 7) /
				          8; /* Round to next 8multiple */
				...
				for(k = i = 0; k < lineLen; i += 8, k++) {
					...
					/* Three smaller loops are better
					   optimizable and use less
					   vars, so most of them can be in
					   registers even on pc's */
					for(c = t = 0; t < 8; t++)
						c = (c << 1) |
						    (bitData[t + i] & 4);
					plane3[k] = ~(byte)(c >> 2);
				...
				/* in the order R, G, B. */
				/* lineLen is at least bitSize/8, so bitData can
				   easily be used to store
				   lineLen of bytes */
				/* P.s. mode9 compression is akward(??) to use,
				   because the lineLenght's
				   are different, so we are stuck with mode 2,
				   which is good enough */

				/* set the line width */
				fprintf(fprn, "\033*r%dS", lineLen * 8);

				count = mode2compress(plane1, plane1 + lineLen,
				                      bitData);
				fprintf(fprn, "\033*b%dV", count);
				fwrite(bitData, sizeof(byte), count, fprn);
				count = mode2compress(plane2, plane2 + lineLen,
				                      bitData);
				fprintf(fprn, "\033*b%dV", count);
				fwrite(bitData, sizeof(byte), count, fprn);
				count = mode2compress(plane3, plane3 + lineLen,
				                      bitData);
...
mode2compress(byte* row, byte* end_row, byte* compressed)
{
	...
	while(1) {
		...
		while((test == *exam) &&
		      (exam < end_row)) /* skip all same bytes */
			exam++;
