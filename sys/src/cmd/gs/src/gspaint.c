...
#else
#define abuf_nominal (gs_debug_c('.') ? abuf_nominal_SMALL : abuf_nominal_LARGE)
#endif
...
gs_fillpage(gs_state* pgs)
{
	...
	bool hl_color_available =
	    gx_hld_is_hl_color_available((gs_imager_state*)pgs, pgs->dev_color);
	...
	if(hl_color_available) {
		...
		code = dev_proc(pgs->device, fill_rectangle_hl_color)(
		    pgs->device, &rect, (const gs_imager_state*)pgs,
		    pgs->dev_color, NULL);
	}
...
scale_paths(gs_state* pgs, int log2_scale_x, int log2_scale_y, bool do_path)
{
	...
	const gx_path_segments* seg_effective_clip =
	    (pgs->effective_clip_path->path_valid
	         ? pgs->effective_clip_path->path.segments
	         : 0);
	...
	if(pgs->view_clip != 0 && pgs->view_clip != pgs->clip_path) {
		seg_view_clip =
		    (pgs->view_clip->path_valid ? pgs->view_clip->path.segments
		                                : 0);
		list_view_clip = pgs->view_clip->rect_list;
		gx_cpath_scale_exp2_shared(
		    pgs->view_clip, log2_scale_x, log2_scale_y,
	...
	if(pgs->effective_clip_path != pgs->clip_path &&
	   pgs->effective_clip_path != pgs->view_clip)
		gx_cpath_scale_exp2_shared(
		    pgs->effective_clip_path, log2_scale_x, log2_scale_y,
...
alpha_buffer_init(gs_state* pgs, fixed extra_x, fixed extra_y, int alpha_bits)
{
	...
	gs_make_mem_abuf_device(mdev, mem, dev, &log2_scale, alpha_bits,
	                        ibox.p.x << log2_scale.x);
	mdev->width = width;
	mdev->height = height;
...
fill_with_rule(gs_state* pgs, int rule)
{
	...
	if(pgs->in_charpath)
		...
		/* Handle separately to prevent gs_state_color_load - bug
		 * 688308. */
		gs_newpath(pgs);
		code = 0;
	} else {
...
gs_stroke(gs_state* pgs)
{
	...
	if(pgs->in_charpath) {
		...
		if(abits > 1) {
			...
			acode = alpha_buffer_init(
			    pgs, pgs->fill_adjust.x + extra_adjust,
			    pgs->fill_adjust.y + extra_adjust, abits);
			if(acode < 0)
				return acode;
...
gs_strokepath(gs_state* pgs)
{
	...
	if(code < 0)
		return code;
	gx_setcurrentpoint(pgs, fixed2float(spath.position.x),
	                   fixed2float(spath.position.y));
	return 0;
}
