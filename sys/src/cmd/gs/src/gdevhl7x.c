...
private
const int HL7X0_LENGTH = 5; /* Length of a command to tell the size of the data to be sent to the printer*/
private
void makeCommandsForSequence(Byte *pSource,
			     int16_t length,
...
private
int dumpPage(gx_device_printer *pSource,
	     Byte *pLineTmp,
	     ByteList *pCommandList,
	     Summary *pSummary);
private
void initSummary(Summary *s, int16_t pw, int16_t ph,
		 int16_t resolution);
...
private
void makeFullLine(Byte *pCurrentLine,
		  Byte *pPreviousLine,
		  int16_t lineWidth,
 ...
 */
private
void initByteList(ByteList *list, Byte *array, int16_t maxSize, int16_t initCurrent);
private
...
{
	Byte prefix[] = {
	    0x1B, '%', '-', '1', '2', '3', '4', '5', 'X', '@', 'P', 'J', 'L', 0x0A /* set PJL mode */
	    ,
	    '@', 'P', 'J', 'L', ' ', 'E', 'N', 'T', 'E', 'R', ' ', 'L', 'A', 'N', 'G', 'U', 'A', 'G', 'E', ' ', '=', ' ', 'H', 'B', 'P', 0x0A /* set GDI Printer mode */
	    ,
	    '@', 'L', 0x0};
...
private
short
horizontalOffset(short pixWidth,
		 short pixOffset,
		 short resolution)
{
...
private
int
dumpPage(gx_device_printer *pSource,
	 Byte *pLineTmp,
	 ByteList *pCommandList,
	 Summary *pSummary)
{
...
private
void
makeFullLine(Byte *pCurrentLine,
	     Byte *pPreviousLine,
	     short lineWidth,
	     ByteList *commandsList,
 ...
 */
private
void makeSequenceWithoutRepeat(
    Byte *pSequence,
...
private
void
makeCommandsForSequence(Byte *pSource,
			short length,
			ByteList *pCommandList,
			short offset,
			Byte *pNumberOfCommands,
