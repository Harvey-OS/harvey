...
/* Other constants */
private
const int DumpFinished = 0;
private
const int DumpContinue = 1;
private
const int HL7X0_LENGTH =
    5; /* Length of a command to tell the size of the data to be sent to the
          printer*/
private
void makeCommandsForSequence(Byte* pSource, int16_t length,
                             ByteList* pCommandList, int16_t offset,
                             Byte* pCommandCount, int16_t rest);

/* Auxiliary Functions */

private
int dumpPage(gx_device_printer* pSource, Byte* pLineTmp, ByteList* pCommandList,
             Summary* pSummary);
private
void initSummary(Summary* s, int16_t pw, int16_t ph, int16_t resolution);

private
void resetPreviousData(Summary* s);

private
void makeFullLine(Byte* pCurrentLine, Byte* pPreviousLine, int16_t lineWidth,
                  ByteList* commandsList, int16_t horizontalOffset);

/*
 * Initialize a list of Bytes structure
 */
private
void initByteList(ByteList* list, Byte* array, int16_t maxSize,
                  int16_t initCurrent);
private
void addByte(ByteList* list, Byte value);
private
void addArray(ByteList* list, Byte* source, int16_t nb);
private
void addNBytes(ByteList* list, Byte value, int16_t nb);
private
Byte* currentPosition(ByteList* list);
private
void addCodedNumber(ByteList* list, short number);
private
int isThereEnoughRoom(ByteList* list, short biggest);
private
short roomLeft(ByteList* list);
private
void dumpToPrinter(ByteList* list, FILE* printStream);

/* Real Print function */

private
int hl7x0_print_page(gx_device_printer*, FILE*, int, int, ByteList*);
...
private
const PaperFormat tableOfFormats[] = {
...
private
short
MaxLineLength(short resolution)
{
...
private
dev_proc_open_device(hl7x0_open);
private
dev_proc_close_device(hl7x0_close);
private
dev_proc_print_page(hl720_print_page);
private
dev_proc_print_page(hl730_print_page);

private
const gx_device_procs prn_hl_procs =
    prn_params_procs(hl7x0_open, gdev_prn_output_page, hl7x0_close,
                     gdev_prn_get_params, gdev_prn_put_params);

const gx_device_printer far_data gs_hl7x0_device =
    prn_device(prn_hl_procs, "hl7x0", DEFAULT_WIDTH_10THS, DEFAULT_HEIGHT_10THS,
               X_DPI, Y_DPI, 0, 0, 0, 0, /* margins filled in by hl7x0_open */
               1, hl720_print_page);     /* The hl720 and hl730 can both use the
                                            same print method */
...
{
	Byte prefix[] = {0x1B, '%', '-', '1', '2', '3', '4', '5', 'X', '@', 'P',
	                 'J', 'L', 0x0A /* set PJL mode */
	                 ,
	                 '@', 'P', 'J', 'L', ' ', 'E', 'N', 'T', 'E', 'R', ' ',
	                 'L', 'A', 'N', 'G', 'U', 'A', 'G', 'E', ' ', '=', ' ',
...
{
	/* UTILE*/
	/* Command for a formFeed (we can't use strings because of the
	 * zeroes...)*/
	...
	Byte* storage = (Byte*)gs_malloc(pdev->memory, sizeOfBuffer + line_size,
	                                 1, "hl7x0_print_page");
	/*	bool dup = pdev->Duplex; */
	/* bool dupset = pdev->Duplex_set >= 0; */
	Summary pageSummary;
	ByteList commandsBuffer;
	initSummary(&pageSummary, line_size, num_rows, x_dpi);
	...
	if(pdev->PageCount == 0) {
		...
		dumpToPrinter(initCommand,
		              printStream); /* send init to printer */
	}

	do {
		result = dumpPage(
		    pdev, storage + sizeOfBuffer, /* The line buffer is after
		                                     the dump buffer */
		    &commandsBuffer,
		    &pageSummary);
...
private
short
stripTrailingBlanks(Byte* line, short length)
{
...
private
short
horizontalOffset(short pixWidth, short pixOffset, short resolution)
{
	return (((LETTER_WIDTH * resolution / 600 - pixWidth) + pixOffset * 2) +
	        7) /
	       8;
}
...
private
void
initSummary(Summary* s, short pw, short ph, short resolution)
{
	s->previousSize = -1;
	s->nbBlankLines = 1;
	s->nbLinesSent = 0;
...
private
int
dumpPage(gx_device_printer* pSource, Byte* pLineTmp, ByteList* pCommandList,
         Summary* pSummary)
{
	...
	for(lineNB = pSummary->nbLinesSent /*ERROR? + nbBlankLines */;
		...
		gdev_prn_copy_scan_lines(pSource, lineNB, pLineTmp,
		                         pSummary->pageWidth);

		usefulLength =
		    stripTrailingBlanks(pLineTmp, pSummary->pageWidth);
		if(usefulLength != 0) {

			/* The line is not blank */
			/* Get rid of the precedent blank lines */
			if(pSummary->nbBlankLines != 0) {
				if(isThereEnoughRoom(pCommandList,
				                     pSummary->nbBlankLines)) {

					addNBytes(pCommandList, 0xff,
					          pSummary->nbBlankLines);
				...
				resetPreviousData(pSummary); /* Make sure there
				                                are zeroes for
				                                the previous
				                                line */
				pSummary->previousSize =
				    0; /* The previous line was empty */
			}

			/* Deal with the current line */
			if(!isThereEnoughRoom(
			       pCommandList,
			       MaxLineLength(pSummary->resolution))) {
			...
			if(pSummary->previousSize ==
			   -1) { /* This is the first line */

				Byte* save = currentPosition(pCommandList);
				addByte(pCommandList,
				        0); /* One byte for the number of
				               commands */

				makeCommandsForSequence(
				    pLineTmp, tmpLength, pCommandList,
				    pSummary->horizontalOffset, save, 0);
			} else { /*There is a previous line */

				makeFullLine(pLineTmp, pSummary->previousData,
				             tmpLength, pCommandList,
				             pSummary->horizontalOffset);
			}
			/* The present line will soon be considered as
			 * "previous" */
			pSummary->previousSize = tmpLength;
			/* Update the data representing the line will soon be
			 * the "previous line" */
...
private
void
makeFullLine(Byte* pCurrentLine, Byte* pPreviousLine, short lineWidth,
             ByteList* commandsList, short horizontalOffset)
{
	/* Declarations */
	Byte* pPreviousTmp;
	Byte* pCurrentTmp;
	...
	pNumberOfCommands = currentPosition(
	    commandsList);        /* Keep a pointer to the number of commands */
	...
	/* Find sequences that are different from the corresponding (i.e.
	 * vertically aligned)
 ...
 */
		...
		while(true) {
			...
			if(*pStartOfSequence != 0)
				break;
			pStartOfSequence++;
			horizontalOffset++; /* the offset takes count of the
			                       bytes that are not "new" */
			--remainingWidth;
		}
#endif

		pPreviousTmp = pStartOfSequence +
		               1; /* The sequence contains at least this byte */
		--remainingWidth;
...
/* Find the end of the sequence of "new" bytes */
		...
		makeCommandsForSequence(
		    pCurrentLine + (pStartOfSequence - pPreviousLine),
		    pPreviousTmp - pStartOfSequence, commandsList,
		    horizontalOffset, pNumberOfCommands, remainingWidth);
		if(*pNumberOfCommands == 0xfe /* If the number of commands has
		...
		pStartOfSequence =
		    pPreviousTmp +
		    1; /* We go on right after the sequence of "new" bytes */
		horizontalOffset = 1;
		--remainingWidth;
	} /* End of While */
 ...
 */
private
void makeSequenceWithoutRepeat(Byte* pSequence, short lengthOfSequence,
                               ByteList* pCommandList, short offset);

private
void makeSequenceWithRepeat(Byte* pSequence, short lengthOfSequence,
                            ByteList* pCommandList, short offset);

/*
 * makeCommandsForSequence :
 * Process a sequence of new bytes (i.e. different from the ones on the former
 * line)
 */

private
void
makeCommandsForSequence(Byte* pSource, short length, ByteList* pCommandList,
                        short offset, Byte* pNumberOfCommands, short rest)
{
	/* Declarations */
	Byte* pStartOfSequence;
	Byte* pEndOfSequence;
	...
	while(true) {
		...
		/* If we have already stored too many commands, make one last
		 * command with
		...
		++*pNumberOfCommands;
		offset =
		    0; /* The relative offset between two subsequences is 0 */
		pStartOfSequence =
		    pEndOfSequence++; /* we loop again from the end of this
		                         subsequence */
		--remainingLength;
...
private
void
makeSequenceWithoutRepeat(Byte* pSequence, short lengthOfSequence,
                          ByteList* pCommandList, short offset)
{
	/*
	...
	Byte* pSaveFirstByte;
	short reducedLength =
	    lengthOfSequence - 1; /* Length is alway higher than 1
...
private
void
makeSequenceWithRepeat(Byte* pSequence, short lengthOfSequence,
                       ByteList* pCommandList, short offset)
{
	/*
	 *   Constant definitions
	...
	Byte* pSaveFirstByte;
	short reducedLength =
	    lengthOfSequence - 2; /* Length is always higher than 2
	                             Therefore a reduced value is stored
	                             */
	/* Initialization */
...
private
void
initByteList(ByteList* list, Byte* array, short maxSize, short initCurrent)
{
...
private
void
addByte(ByteList* list, Byte value)
{
...
private
void
addArray(ByteList* list, Byte* source, short nb)
{
...
private
void
addNBytes(ByteList* list, Byte value, short nb)
{
	int i;
	if(list->current <= list->maxSize - nb) {
...
private
void
addCodedNumber(ByteList* list, short number)
{
	short q = number / 0xff;
	short r = number % 0xff;
...
private
int
isThereEnoughRoom(ByteList* list, short biggest)
{
...
private
void
dumpToPrinter(ByteList* list, FILE* printStream)
{
	short loopCounter;
	/* Actual dump */
