...
/* Forward references */
private
int dict_spot_params(const ref*, gs_spot_halftone*, ref*, ref*);
private
int dict_spot_results(i_ctx_t*, ref*, const gs_spot_halftone*);
private
int dict_threshold_params(const ref*, gs_threshold_halftone*, ref*);
private
int dict_threshold2_params(const ref*, gs_threshold2_halftone*, ref*,
                           gs_memory_t*);
 ...
 */
int
gs_get_colorname_string(const gs_memory_t* mem,
                        ...
                        unsigned char** ppstr, unsigned int* pname_size)
{
	...
	return obj_string_data(mem, &nref, (const unsigned char**)ppstr,
	                       pname_size);
}
...
private
int sethalftone_finish(i_ctx_t*);
private
int sethalftone_cleanup(i_ctx_t*);
private
int
zsethalftone5(i_ctx_t* i_ctx_p)
{
	...
	halftonetype = (type == 2 || type == 4) ? ht_type_multiple_colorscreen
	                                        : ht_type_multiple;
	...
	for(count = 0;;) {
		...
		if(code < 0)
			break;
		colorant_number = gs_cname_to_colorant_number(
		    pgs, pname, name_size, halftonetype);
	...
	rc_alloc_struct_0(pht, gs_halftone, &st_halftone, imemory, pht = 0,
	                  ".sethalftone5");
	phtc = gs_alloc_struct_array(mem, count, gs_halftone_component,
	                             &st_ht_component_element, ".sethalftone5");
	...
	if(pht == 0 || phtc == 0 || pdht == 0) {
		...
		for(j = 0, pc = phtc;;) {
			...
			default:
				...
				break;
			case 1:
				code = dict_spot_params(&rvalue[1],
				                        &pc->params.spot,
				                        sprocs + j, tprocs + j);
				...
				break;
			case 3:
				code = dict_threshold_params(
				    &rvalue[1], &pc->params.threshold,
				    tprocs + j);
				pc->type = ht_type_threshold;
				break;
			case 7:
				code = dict_threshold2_params(
				    &rvalue[1], &pc->params.threshold2,
				    tprocs + j, imemory);
	...
	if(code >= 0) {
		...
		for(pc = phtc;;) {
			...
			/* Get the name of the component and verify that we will
			 * use it. */
			cname = name_index(mem, &rvalue[0]);
			code = gs_get_colorname_string(mem, cname, &pname,
			                               &name_size);
			...
			if(pc->type == ht_type_spot) {
				code = dict_spot_results(i_ctx_p, &rvalue[1],
				                         &pc->params.spot);
				if(code < 0)
					break;
			}
	...
	if(code >= 0) {
		...
		for(j = 0; j < count; j++) {
			...
			if(pdht->components == 0)
				...
				/* Find the component in pdht that matches
				   component j in
				   the pht; gs_sethalftone_prepare() may permute
				   these. */
				int k;
				int comp_number = phtc[j].comp_number;
				for(k = 0; k < count; k++) {
					if(pdht->components[k].comp_number ==
					   comp_number) {
						porder =
						    &pdht->components[k].corder;
						break;
					}
				}
			}
			switch(phtc[j].type) {
			case ht_type_spot:
				code = zscreen_enum_init(
				    i_ctx_p, porder,
				    &phtc[j].params.spot.screen, &sprocs[j], 0,
				    0, space_index);
				if(code < 0)
					break;
			/* falls through */
...
const op_def zht2_l2_op_defs[] = {op_def_begin_level2(),
                 ...
                 ref* ptproc)
{
	...
	if((code = dict_float_param(pdict, "Frequency", 0.0,
	   ...
	   (code = dict_float_param(pdict, "Angle", 0.0, &psp->screen.angle)) !=
	       0 ||
	   (code = dict_proc_param(pdict, "SpotFunction", psproc, false)) !=
	       0 ||
	   (code = dict_bool_param(pdict, "AccurateScreens",
	                           gs_currentaccuratescreens(),
	                           &psp->accurate_screens)) < 0 ||
	   (code = dict_proc_param(pdict, "TransferFunction", ptproc, false)) <
	       0)
...
dict_threshold_common_params(const ref* pdict,
                             gs_threshold_halftone_common* ptp, ref** pptstring,
                             ref* ptproc)
{
	int code;

	check_dict_read(*pdict);
	if((code = dict_int_param(pdict, "Width", 1, 0x7fff, -1, &ptp->width)) <
	       0 ||
...
private
int
dict_threshold_params(const ref* pdict, gs_threshold_halftone* ptp, ref* ptproc)
{
	ref* tstring;
	int code = dict_threshold_common_params(
	    pdict, (gs_threshold_halftone_common*)ptp, &tstring, ptproc);
                       ...
                       ref* ptproc, gs_memory_t* mem)
{
	...
	case t_string:
		size = r_size(tstring);
		gs_bytestring_from_string(&ptp->thresholds,
		                          tstring->value.const_bytes, size);
		break;
	case t_astruct:
	...
	check_read(*tstring);
	if(size !=
	   (ptp->width * ptp->height + ptp->width2 * ptp->height2) *
