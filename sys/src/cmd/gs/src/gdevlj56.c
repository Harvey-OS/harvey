...
private
dev_proc_open_device(ljet5_open);
private
dev_proc_close_device(ljet5_close);
private
dev_proc_print_page(ljet5_print_page);

private
const gx_device_procs ljet5_procs =
    prn_procs(ljet5_open, gdev_prn_output_page, ljet5_close);

const gx_device_printer gs_lj5mono_device = prn_device(
    ljet5_procs, "lj5mono", DEFAULT_WIDTH_10THS, DEFAULT_HEIGHT_10THS, X_DPI,
    Y_DPI, 0, 0, 0, 0, 1, ljet5_print_page);

private
const gx_device_procs lj5gray_procs = prn_color_procs(
    ljet5_open, gdev_prn_output_page, ljet5_close,
...
const gx_device_printer gs_lj5gray_device = {
    prn_device_body(gx_device_printer, lj5gray_procs, "lj5gray",
                    ...
                    0, 0, 0, 1, 8, 255, 0, 256, 1, ljet5_print_page)};
...
ljet5_print_page(gx_device_printer* pdev, FILE* prn_stream)
{
	...
	word* line =
	    (word*)gs_alloc_byte_array(mem, line_size_words, W, "ljet5(line)");
	...
	{
		static const byte page_header[] = {pxtBeginPage, DUSP(0, 0),
		                                   DA(pxaPoint), pxtSetCursor};
		static const byte mono_header[] = {
		    DUB(eGray), DA(pxaColorSpace), DUB(e8Bit),
		    DA(pxaPaletteDepth), pxt_ubyte_array, pxt_ubyte, 2, 0xff,
		    0x00, DA(pxaPaletteData), pxtSetColorSpace};
		static const byte gray_header[] = {
		    DUB(eGray), DA(pxaColorSpace), pxtSetColorSpace};
	...
	/* Write the image header. */
	{
		static const byte mono_image_header[] = {
		    DA(pxaDestinationSize), DUB(eIndexedPixel),
		    DA(pxaColorMapping), DUB(e1Bit), DA(pxaColorDepth),
		    pxtBeginImage};
		static const byte gray_image_header[] = {
		    DA(pxaDestinationSize), DUB(eDirectPixel),
		    DA(pxaColorMapping), DUB(e8Bit), DA(pxaColorDepth),
		    pxtBeginImage};
	...
	for(lnum = 0; lnum < pdev->height; ++lnum) {
		...
		static const byte line_header[] = {
		    DA(pxaStartLine), DUS(1), DA(pxaBlockHeight),
		    DUB(eRLECompression), DA(pxaCompressMode), pxtReadImage};

		code = gdev_prn_copy_scan_lines(pdev, lnum, (byte*)line,
		                                line_size);
		if(code < 0)
			goto fin;
		px_put_us(s, lnum);
		PX_PUT_LIT(s, line_header);
		ncompr = gdev_pcl_mode2compress_padded(
		    line, line + line_size_words, out, true);
