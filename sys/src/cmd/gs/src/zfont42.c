...
/* Copyright (C) 1996, 1998, 1999, 2000 Aladdin Enterprises.  All rights
  reserved.
...
/* Forward references */
private
int z42_string_proc(gs_font_type42*, uint32_t, uint, const byte**);
private
uint z42_get_glyph_index(gs_font_type42*, gs_glyph);
private
int z42_gdir_get_outline(gs_font_type42*, uint, gs_glyph_data_t*);
private
font_proc_enumerate_glyph(z42_enumerate_glyph);
private
font_proc_enumerate_glyph(z42_gdir_enumerate_glyph);
private
font_proc_encode_char(z42_encode_char);
private
font_proc_glyph_info(z42_glyph_info);
private
font_proc_glyph_outline(z42_glyph_outline);
...
zbuildfont42(i_ctx_t* i_ctx_p)
{
	os_ptr op = osp;
	gs_font_type42* pfont;
	int code = build_gs_TrueType_font(
	    i_ctx_p, op, &pfont, ft_TrueType, &st_gs_font_type42,
	    "%Type42BuildChar", "%Type42BuildGlyph", bf_options_none);
 ...
 */
int
string_array_access_proc(const gs_memory_t* mem, const ref* psa, int modulus,
                         uint32_t offset, uint length, const byte** pdata)
{
...
const op_def zfont42_op_defs[] = {{"2.buildfont42", zbuildfont42},
                                  op_def_end(0)};
 ...
 */
int
font_gdir_get_outline(const gs_memory_t* mem, const ref* pgdir,
                      int32_t glyph_index, gs_glyph_data_t* pgd)
{
	ref iglyph;
	ref gdef;
	ref* pgdef;
	...
	if(r_has_type(pgdir, t_dictionary)) {
		...
		code =
		    dict_find(pgdir, &iglyph, &pgdef) - 1; /* 0 => not found */
	} else {
	...
	if(code < 0) {
		...
		gs_glyph_data_from_string(pgd, pgdef->value.const_bytes,
		                          r_size(pgdef), NULL);
	}
	return 0;
}
                     ...
                     gs_glyph_data_t* pgd)
{
	...
	return font_gdir_get_outline(pfont->memory, pgdir, (long)glyph_index,
	                             pgd);
}
                    ...
                    gs_glyph* pglyph)
{
	if(glyph_space == GLYPH_SPACE_INDEX)
		return gs_type42_enumerate_glyph(font, pindex, glyph_space,
		                                 pglyph);
	else {
		const ref* pcsdict = &pfont_data(font)->CharStrings;

		return zchar_enumerate_glyph(font->memory, pcsdict, pindex,
		                             pglyph);
	}
}
                         ...
                         gs_glyph_space_t glyph_space, gs_glyph* pglyph)
{
	...
	if(glyph_space == GLYPH_SPACE_INDEX) {
		...
		if(!r_has_type(pgdict, t_dictionary)) {
			...
			for(;; (*pindex)++) {
				if(array_get(font->memory, pgdict,
				             (long)*pindex, &gdef) < 0) {
					*pindex = 0;
					return 0;
				}
...
z42_encode_char(gs_font* font, gs_char chr, gs_glyph_space_t glyph_space)
{
	...
	return (glyph_space == GLYPH_SPACE_INDEX && glyph != gs_no_glyph
	            ? glyph_to_index(font, glyph)
	            : glyph);
}
private
int
z42_glyph_outline(gs_font* font, int WMode, gs_glyph glyph,
