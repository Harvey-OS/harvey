...
/* Copyright (C) 1989, 1992, 1993, 1994, 1996, 1997, 1998, 1999 Aladdin
...
ENUM_PTRS_END
private
RELOC_PTRS_WITH(transfer_map_reloc_ptrs, gx_transfer_map* mptr)
{
	if(mptr->proc == 0)
...
void
gx_no_adjust_color_count(const gs_client_color* pcc, const gs_color_space* pcs,
                         int delta)
{
}
...
load_transfer_map(gs_state* pgs, gx_transfer_map* pmap, floatp min_value)
{
	...
	for(i = 0; i < transfer_map_size; i++) {
		float fval = (*proc)((float)i / (transfer_map_size - 1), pmap,
		                     proc_data);

		values[i] =
		    (fval < min_value ? fmin : fval >= 1.0 ? frac_1
		                                           : float2frac(fval));
	}
}
