...
zwrapfont(i_ctx_t* i_ctx_p)
{
	...
	case ft_TrueType:
		code = gs_font_type0_from_type42(&font0, (gs_font_type42*)font,
		                                 wmode, true, font->memory);
		if(code < 0)
			return code;
		/*
		 ...
		 */
		{
			...
			code = name_ref((const byte*)bgstr, strlen(bgstr),
			                &temp, 1);
			if(code < 0)
				return code;
...
zwritefont9(i_ctx_t* i_ctx_p)
{
	...
	code = psf_write_cid0_font(s, pfcid, WRITE_TYPE2_NO_LENIV |
	                                         WRITE_TYPE2_CHARSTRINGS,
	                           NULL, 0, NULL);
	if(code >= 0)
		pop(2);
	return code;
...
const op_def zcidtest_op_defs[] = {{"1.wrapfont", zwrapfont},
