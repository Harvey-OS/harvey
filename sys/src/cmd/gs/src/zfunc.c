...
zscalefunction(i_ctx_t* i_ctx_p)
{
	...
	ranges = (gs_range_t*)gs_alloc_byte_array(
	    imemory, pfn->params.n, sizeof(gs_range_t), "zscalefunction");
	if(ranges == 0)
	...
	if(code < 0 || (code = make_function_proc(i_ctx_p, op - 1, psfn)) < 0) {
...
zexecfunction(i_ctx_t* i_ctx_p)
{
		...
		return_error(e_typecheck);
	{
		...
		if(diff > 0)
			check_ostack(diff);
		{
			float params[20]; /* arbitrary size, just to avoid
			                     allocs */
			float* in;
			float* out;
			int code = 0;
...
int
fn_build_function(i_ctx_t* i_ctx_p, const ref* op, gs_function_t** ppfn,
                  gs_memory_t* mem)
{
                      ...
                      int depth, gs_memory_t* mem)
{
	...
	params.Range = 0;
	code =
	    fn_build_float_array(op, "Domain", true, true, &params.Domain, mem);
	if(code < 0)
		goto fail;
	params.m = code >> 1;
	code =
	    fn_build_float_array(op, "Range", false, true, &params.Range, mem);
	if(code < 0)
		goto fail;
	params.n = code >> 1;
	/* Finish building the function. */
	/* If this fails, it will free all the parameters. */
	return (*build_function_type_table[i].proc)(i_ctx_p, op, &params,
	                                            depth + 1, ppfn, mem);
fail:
 ...
 */
int
fn_build_float_array(const ref* op, const char* kstr, bool required, bool even,
                     const float** pparray, gs_memory_t* mem)
{
	...
	*pparray = 0;
		...
		return_error(e_typecheck);
	{
		...
		float* ptr =
		    (float*)gs_alloc_byte_array(mem, size, sizeof(float), kstr);

		if(ptr == 0)
			return_error(e_VMerror);
		code = dict_float_array_check_param(mem, op, kstr, size, ptr,
		                                    NULL, 0, e_rangecheck);
                            ...
                            const float** pparray, gs_memory_t* mem)
{
	...
	*pparray = 0;
	...
	if(r_is_array(par))
		...
		code = dict_float_param(op, kstr, 0.,
		                        ptr); /* defailt cannot happen */
		if(code == 0)
			code = 1;
	}
...
const op_def zfunc_op_defs[] = {{"1.buildfunction", zbuildfunction},
#ifdef TEST
