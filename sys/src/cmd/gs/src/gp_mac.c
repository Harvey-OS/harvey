...
gp_get_usertime(int32_t* pdt)
{
	...
	pdt[0] -= (char)rand(); // was needed, if used for random generator seed
	                        // (g3 is too fast)
}
...

{
	...
	int32_t pdtmp[2];
	void do_get_clock(DateTimeRec * dateRec, int32_t * pdt);
...
gpp_get_realtime(int32_t* pdt)
{
	...
	UnsignedWide microTickCount, nMicroTickCount;

	int32_t idate;
	static const int mstart[12] = {0,   31,  59,  90,  120, 151,
	...
	idate = ((int32_t)dateRec.year - 1980) * 365 + /* days per year */
	        (((int32_t)dateRec.year - 1) / 4 -
	         1979 / 4) + /* intervening leap days */
	...
	pdt[0] = ((idate * 24 + dateRec.hour) * 60 + dateRec.minute) * 60 +
	         dateRec.second;
...
//#define DEBUG_CLOCK 1
	...
	fprintf(stderr, "b hi[0] = %ld  lo[1] = %ld\n", beginMicroTickCount.hi,
	        beginMicroTickCount.lo);
