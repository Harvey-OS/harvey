...
zbind(i_ctx_t* i_ctx_p)
{
	...
	*op = defn;
	...
	while(depth) {
		while(r_size(bsp)) {
			ref_packed* const tpp =
			    (ref_packed*)bsp->value.packed; /* break const */
			...
			if(r_is_packed(tpp)) {
				...
				if(r_packed_is_exec_name(&elt)) {
					...
					name_index_ref(imemory,
					               packed_name_index(&elt),
					               &nref);
					if((pvalue = dict_find_name(&nref)) !=
					       0 &&
					   r_is_ex_oper(pvalue)) {
						store_check_dest(bsp, pvalue);
						/*
						 * Always save the change, since
						 * this can only
						 * happen once.
						 */
				...
				case t_name: /* bind the name if an operator */
					if(r_has_attr(tp, a_executable)) {
						...
						if((pvalue = dict_find_name(
						        tp)) != 0 &&
						   r_is_ex_oper(pvalue)) {
							store_check_dest(
							    bsp, pvalue);
							ref_assign_old(bsp, tp,
							               pvalue,
							               "bind");
						}
					}
...
zpcachequery(i_ctx_t* i_ctx_p)
{
	...
	len = r_size(op);
	key = op->value.bytes;
	len = gp_cache_query(GP_CACHE_TYPE_TEST, key, len, (void**)&string,
	                     &pcache_alloc_callback, i_ctx_p);
	if(len < 0) {
		make_false(op);
		return 0;
	}
...
const op_def zmisc_op_defs[] = {{"1bind", zbind},
                                {"1getenv", zgetenv},
