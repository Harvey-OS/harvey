...
/*
 * The linear color trapezoid fill algorithm uses trap_color structures to keep
 * track of
...
compute_ldx(trap_line* tl, fixed ys)
{
	...
	if(df < YMULT_LIMIT) {
		if(df == 0) /* vertical edge, worth checking for */
			...
			tl->xf = (ys < fixed_1 ? ys * df % h
			                       : fixed_mult_rem(ys, df, h)) -
			         h;
		}
	} else {
              ...
              const trap_line* l, fixed ybot, int num_components)
{
	...
	if(e->c1 == NULL || e->c0 == NULL)
		g->den =
		    0; /* A wedge - the color is axial along another edge. */
	else {
		...
		for(i = 0; i < num_components; i++) {
			...
			c = (int64_t)g->num[i] *
			    (uint32_t)(ybot - (ends_from_fa ? fa->ystart
			                                    : e->start.y));
			d = (int32_t)(c / g->den);
			g->c[i] = e->c0[i] + d;
			c -= (int64_t)d * g->den;
...
private
inline bool
check_gradient_overflow(const gs_linear_color_edge* le,
                        const gs_linear_color_edge* re, int num_components)
{
	if(le->c1 == NULL || re->c1 == NULL) {
		/* A wedge doesn't use a gradient by X. */
		return false;
	} else {
		/* Check whether set_x_gradient, fill_linear_color_scanline can
		   overflow.

		   dev_proc(dev, fill_linear_color_scanline) can perform its
		   computation in 32-bit fractions,
		   so we assume it never overflows. Devices which implement it
		   with no this
		   assumption must implement the check in
		   gx_default_fill_linear_color_trapezoid,
		   gx_default_fill_linear_color_triangle with a function other
		   than this one.

		   Since set_x_gradient perform computations in int64_t, which
		   provides 63 bits
...
private
inline int
set_x_gradient_nowedge(trap_gradient* xg, const trap_gradient* lg,
                       const trap_gradient* rg, const trap_line* l,
                       const trap_line* r, int il, int ir, int num_components)
{
	/* Ignoring the ending coordinats fractions,
	   so the gridient is slightly shifted to the left (in <1 'fixed' unit).
	   */
	int32_t xl = l->x - (l->xf == -l->h ? 1 : 0) -
	             fixed_half; /* Revert the GX_FILL_TRAPEZOID shift. */
	int32_t xr = r->x - (r->xf == -r->h ? 1 : 0) -
	             fixed_half; /* Revert the GX_FILL_TRAPEZOID shift. */
	/* The pixel span boundaries : */
	int32_t x0 =
	    int2fixed(il) + fixed_half; /* Shift to the pixel center. */
	int32_t x1 =
	    int2fixed(ir) + fixed_half; /* Shift to the pixel center. */
	int i;

#ifdef DEBUG
	if(arith_rshift_1(xr) - arith_rshift_1(xl) >=
	   0x3FFFFFFE)                               /* Can overflow ? */
	...
	for(i = 0; i < num_components; i++) {
		...
		int32_t cr = rg->c[i];
		int32_t c0 =
		    (int32_t)(cl + ((int64_t)cr - cl) * (x0 - xl) / (xr - xl));
		int32_t c1 =
		    (int32_t)(cl + ((int64_t)cr - cl) * (x1 - xl) / (xr - xl));

		xg->c[i] = c0;
		xg->f[i] =
		    0; /* Insufficient bits to compute it better.
		          The color so the color gets a slightly smaller value
		          (in <1 'frac31' unit), but it's not important due to
		          the further conversion to [0, 1 <<
		          cinfo->comp_bits[j]],
		          which drops the fraction anyway.
...
private
inline int
set_x_gradient(trap_gradient* xg, const trap_gradient* lg,
               const trap_gradient* rg, const trap_line* l, const trap_line* r,
               int il, int ir, int num_components)
{
	if(lg->den == 0 || rg->den == 0) {
		...
		for(i = 0; i < num_components; i++) {
			...
			xg->f[i] =
			    0; /* Compatible to set_x_gradient_nowedge. */
			xg->num[i] = 0;
		}
		return 0;
	} else
		return set_x_gradient_nowedge(xg, lg, rg, l, r, il, ir,
		                              num_components);
}
...
#define GX_FILL_TRAPEZOID                                                      \
      private                                                                  \
	int gx_fill_trapezoid_as_fd
...
#define FILL_ATTRS                                                             \
	const gs_fill_attributes * /* Common for non-shading variants. */

#define GX_FILL_TRAPEZOID                                                      \
      private                                                                  \
	int gx_fill_trapezoid_ns_lc
#define CONTIGUOUS_FILL 0
#define SWAP_AXES 0
#define FILL_DIRECT 1
#include "gxdtfill.h"
                          ...
                          gs_logical_operation_t lop)
{
	...
	if(swap_axes) {
		if(fill_direct)
			return gx_fill_trapezoid_as_fd(dev, left, right, ybot,
			                               ytop, 0, pdevc, lop);
		else
			return gx_fill_trapezoid_as_nd(dev, left, right, ybot,
			                               ytop, 0, pdevc, lop);
	} else {
		if(fill_direct)
			return gx_fill_trapezoid_ns_fd(dev, left, right, ybot,
			                               ytop, 0, pdevc, lop);
		else
			return gx_fill_trapezoid_ns_nd(dev, left, right, ybot,
			                               ytop, 0, pdevc, lop);
	}
}
...
private
inline void
middle_frac31_color(frac31* c, const frac31* c0, const frac31* c2,
                    int num_components)
{
...
private
inline int
fill_linear_color_trapezoid_nocheck(gx_device* dev,
                                    const gs_fill_attributes* fa,
                                    const gs_linear_color_edge* le,
                                    const gs_linear_color_edge* re)
{
	fixed y02 = max(le->start.y, re->start.y),
	      ymin = max(y02, fa->clip->p.y);
	fixed y13 = min(le->end.y, re->end.y), ymax = min(y13, fa->clip->q.y);
	int code;

	code =
	    (fa->swap_axes ? gx_fill_trapezoid_as_lc : gx_fill_trapezoid_ns_lc)(
	        dev, le, re, ymin, ymax, 0, NULL, fa);
	if(code < 0)
		return code;
	return !code;
}
...
fill_linear_color_triangle(gx_device* dev, const gs_fill_attributes* fa,
                           ...
                           const gs_fixed_point* p2, const frac31* c0,
...
{ /* p0 must be the lowest vertex. */
	...
	if(p0->y == p1->y)
		return gx_default_fill_linear_color_trapezoid(
		    dev, fa, p0, p2, p1, p2, c0, c2, c1, c2);
	if(p1->y == p2->y)
		return gx_default_fill_linear_color_trapezoid(
		    dev, fa, p0, p2, p0, p1, c0, c2, c0, c1);
	e0.start = *p0;
	e0.end = *p2;
	e0.c0 = c0;
	e0.c1 = c2;
    ...
    const frac31* c1, const frac31* c2)
{
	...
	if(p0->y <= p1->y && p0->y <= p2->y)
		return fill_linear_color_triangle(dev, fa, p0, p1, p2, c0, c1,
		                                  c2);
	if(p1->y <= p0->y && p1->y <= p2->y)
		return fill_linear_color_triangle(dev, fa, p1, p2, p0, c1, c2,
		                                  c0);
	else
		return fill_linear_color_triangle(dev, fa, p2, p0, p1, c2, c0,
		                                  c1);
}
...
int
gx_default_fill_parallelogram(gx_device* dev, fixed px, fixed py, fixed ax,
                              ...
                              gs_logical_operation_t lop)
{
	...
	right.end.x = px + bx;
	right.end.y = py + by;
#define ROUNDED_SAME(p1, p2) (fixed_pixround(p1) == fixed_pixround(p2))
	if(ay < by) {
		if(!ROUNDED_SAME(py, left.end.y)) {
			code = (*fill_trapezoid)(dev, &left, &right, py,
			                         left.end.y, false, pdevc, lop);
			if(code < 0)
				return code;
		}
		left.start = left.end;
		...
		if(!ROUNDED_SAME(left.start.y, ym)) {
			code =
			    (*fill_trapezoid)(dev, &left, &right, left.start.y,
			                      ym, false, pdevc, lop);
			if(code < 0)
				return code;
		}
		...
		if(!ROUNDED_SAME(py, right.end.y)) {
			code =
			    (*fill_trapezoid)(dev, &left, &right, py,
			                      right.end.y, false, pdevc, lop);
			if(code < 0)
				return code;
		}
		right.start = right.end;
		...
		if(!ROUNDED_SAME(right.start.y, ym)) {
			code =
			    (*fill_trapezoid)(dev, &left, &right, right.start.y,
			                      ym, false, pdevc, lop);
			if(code < 0)
				return code;
		}
	...
	if(!ROUNDED_SAME(ym, qy))
		return (*fill_trapezoid)(dev, &left, &right, ym, qy, false,
		                         pdevc, lop);
	else
		return 0;
#undef ROUNDED_SAME
...
/* We should swap axes to get best accuracy, but we don't. */
int
gx_default_fill_triangle(gx_device* dev, fixed px, fixed py, fixed ax, fixed ay,
                         ...
                         gs_logical_operation_t lop)
{
	...
	return (*fill_trapezoid)(dev, &left, &right, ym, right.end.y, false,
	                         pdevc, lop);
}

/* Draw a one-pixel-wide line. */
int
gx_default_draw_thin_line(gx_device* dev, fixed fx0, fixed fy0, fixed fx1,
                          fixed fy1, const gx_device_color* pdevc,
                          gs_logical_operation_t lop)
{
	...
	if(itoy == iy) { /* horizontal line */
		return (ix <= itox
		            ? gx_fill_rectangle_device_rop(
		                  ix, iy, itox - ix + 1, 1, pdevc, dev, lop)
		            : gx_fill_rectangle_device_rop(
		                  itox, iy, ix - itox + 1, 1, pdevc, dev, lop));
	}
	if(itox == ix) { /* vertical line */
		return (iy <= itoy
		            ? gx_fill_rectangle_device_rop(
		                  ix, iy, 1, itoy - iy + 1, pdevc, dev, lop)
		            : gx_fill_rectangle_device_rop(
		                  ix, itoy, 1, iy - itoy + 1, pdevc, dev, lop));
	}
	{
		fixed h = fy1 - fy0;
		fixed w = fx1 - fx0;
		...
		if((w < 0 ? -w : w) <= (h < 0 ? -h : h)) {
			...
			right.start.x =
			    (left.start.x = fx0 - fixed_half) + fixed_1;
			...
			right.start.x =
			    (left.start.x = fy0 - fixed_half) + fixed_1;
		...
		return (*dev_proc(dev, fill_trapezoid))(
		    dev, &left, &right, left.start.y, left.end.y, swap_axes,
		    pdevc, lop);
	}
}
...
int
gx_default_draw_line(gx_device* dev, int x0, int y0, int x1, int y1,
...
private
RELOC_PTRS_WITH(image_enum_common_reloc_ptrs, gx_image_enum_common_t* eptr)
{
...
private
int
gx_no_begin_image(gx_device* dev, const gs_imager_state* pis,
...
int
gx_default_begin_image(gx_device* dev, const gs_imager_state* pis,
                       const gs_image_t* pim, gs_image_format_t format,
...
int
gx_default_begin_typed_image(gx_device* dev, const gs_imager_state* pis,
...
{ /*
   ...
   */
		...
		if(pmat == 0 ||
		   (pis != 0 && !memcmp(pmat, &ctm_only(pis), sizeof(*pmat)))) {
			int code = (*dev_proc(dev, begin_image))(
			    dev, pis, pim, pim->format, prect, pdcolor, pcpath,
			    memory, pinfo);
	...
	return (*pic->type->begin_typed_image)(dev, pis, pmat, pic, prect,
...
gx_default_image_data(gx_device* dev, gx_image_enum_common_t* info,
                      const byte** plane_data, int data_x, uint raster,
                      int height)
{
