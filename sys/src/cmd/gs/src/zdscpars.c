...
private
void dsc_finalize(void* vptr);
gs_private_st_simple_final(st_dsc_data_t, dsc_data_t, "dsc_data_struct",
                           dsc_finalize);

/* Define the key name for storing the instance pointer in a dictionary. */
private
const int8_t* const dsc_dict_name = "DSC_struct";
...
private
int
dsc_put_string(gs_param_list* plist, const char* keyname, const char* string)
{
	gs_param_string str;
...
dsc_page_orientation(gs_param_list* plist, const CDSC* pData)
{
	 ...
	 */
	if(page_num &&
	   pData->page[page_num - 1].orientation != CDSC_ORIENT_UNKNOWN)
		return dsc_put_int(
		    plist, "PageOrientation",
...
typedef struct cmd_list_s {
	...
	const char*
	    comment_name; /* A name to be returned to postscript caller */
 ...
 */
private
const int8_t* const BadCmdlist[] = {
    "%%BeginData:", "%%EndData", "%%BeginBinary:", "%%EndBinary",
...
#define MAX_DSC_MSG_SIZE (DSC_LINE_LENGTH + 4) /* Allow for %% and CR/LF */
	...
	if(*pBadList) {           /* If found in list, then skip comment */
		...
		/*
		 * Parse comments - use Russell Lang's DSC parser.  We need to
		 * get
		...
		if_debug1('%', "[%%].parse_dsc_comments: code = %d\n",
		          comment_code);
		/*
		 * We ignore any errors from Russell's parser.  The only value
		 * that
		 * it will return for an error is -1 so there is very little
		 * information.
		 * We also do not want bad DSC comments to abort processing of
		 * an
