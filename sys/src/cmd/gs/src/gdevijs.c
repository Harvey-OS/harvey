...
private
dev_proc_open_device(gsijs_open);
private
dev_proc_close_device(gsijs_close);
private
dev_proc_output_page(gsijs_output_page);
private
dev_proc_get_params(gsijs_get_params);
private
dev_proc_put_params(gsijs_put_params);
private
dev_proc_finish_copydevice(gsijs_finish_copydevice);

private
const gx_device_procs gsijs_procs = {
    gsijs_open, NULL, /* get_initial_matrix */
    NULL,             /* sync_output */
    gsijs_output_page, gsijs_close, gx_default_rgb_map_rgb_color,
    gx_default_rgb_map_color_rgb, NULL,       /* fill_rectangle */
    NULL,                                     /* tile_rectangle */
    NULL,                                     /* copy_mono */
    NULL,                                     /* copy_color */
    NULL,                                     /* draw_line */
    NULL,                                     /* get_bits */
    gsijs_get_params, gsijs_put_params, NULL, /* map_cmyk_color */
...
gx_device_ijs gs_ijs_device = {
    prn_device_std_body(gx_device_ijs, gsijs_procs, "ijs", DEFAULT_WIDTH_10THS,
                        DEFAULT_HEIGHT_10THS, DEFAULT_DPI, DEFAULT_DPI, 0, 0, 0,
                        0, 24 /* depth */, NULL /* print page */),
...
private
int gsijs_client_set_param(gx_device_ijs* ijsdev, const char* key,
                           const char* value);
private
int gsijs_set_color_format(gx_device_ijs* ijsdev);
private
int gsijs_read_int(gs_param_list* plist, gs_param_name pname, int* pval,
                   int min_value, int max_value, bool only_when_closed);
private
int gsijs_read_bool(gs_param_list* plist, gs_param_name pname, bool* pval,
                    bool only_when_closed);
private
int gsijs_read_string(gs_param_list* plist, gs_param_name pname, char* str,
...
gsijs_set_generic_params_hpijs(gx_device_ijs* ijsdev)
{
	...
	if(ijsdev->IjsParams) {
		code = gsijs_client_set_param(ijsdev, "IjsParams",
		                              ijsdev->IjsParams);
	}
...
gsijs_set_generic_params(gx_device_ijs* ijsdev)
{
	...
	value = NULL;
	for(i = 0, j = 0; (j < ijsdev->IjsParams_size) && (i < sizeof(buf) - 1);
	    j++) {
		...
		if(ch == '\\') {
			...
			if(ch == ',') {
				buf[i - 1] = '\0';
				if(value)
					gsijs_client_set_param(ijsdev, buf,
					                       value);
				i = 0;
				value = NULL;
			}
	...
	if(code == 0 && ijsdev->Duplex_set) {
		code = gsijs_client_set_param(
		    ijsdev, "PS:Duplex", ijsdev->Duplex ? "true" : "false");
	}
	if(code == 0 && ijsdev->IjsTumble_set) {
		code = gsijs_client_set_param(
		    ijsdev, "PS:Tumble", ijsdev->IjsTumble ? "true" : "false");
	}
	return code;
}
...
gsijs_set_margin_params_hpijs(gx_device_ijs* ijsdev)
{
	...
	if(code == 0) {
		...
		if(code == IJS_EUNKPARAM)
			...
			code = gsijs_parse_wxh(buf, code, &printable_width,
			                       &printable_height);
		}

		if(code == 0) {
			code = ijs_client_get_param(ijsdev->ctx, 0,
			                            "PrintableTopLeft", buf,
			                            sizeof(buf));
			if(code == IJS_EUNKPARAM)
				return 0;
			else if(code >= 0) {
				code = gsijs_parse_wxh(
				    buf, code, &printable_left, &printable_top);
			}
		}
...
gsijs_set_margin_params(gx_device_ijs* ijsdev)
{
	...
	if(code == 0 && ijsdev->Duplex_set) {
		code = gsijs_client_set_param(
		    ijsdev, "Duplex", ijsdev->Duplex ? "true" : "false");
	}
	if(code == 0 && ijsdev->IjsTumble_set) {
		code = gsijs_client_set_param(
		    ijsdev, "Tumble", ijsdev->IjsTumble ? "true" : "false");
	}

	if(code == 0) {
		sprintf(buf, "%gx%g", ijsdev->MediaSize[0] * (1.0 / 72),
		        ijsdev->MediaSize[1] * (1.0 / 72));
		code = ijs_client_set_param(ijsdev->ctx, 0, "PaperSize", buf,
		                            strlen(buf));
	}
...
gsijs_set_resolution(gx_device_ijs* ijsdev)
{
	...
	code = ijs_client_get_param(ijsdev->ctx, 0, "Dpi", buf, sizeof(buf));
	if(code >= 0) {
		int i;
	...
	ijsdev->is_open = true;
	code = gdev_prn_maybe_realloc_memory(
	    (gx_device_printer*)ijsdev, &ijsdev->space_params, width, height,
...
gsijs_open(gx_device* dev)
{
	...
	*/
	...
	if(use_outputfd) {
		...
		*/
		sprintf(buf, "%d", fd);
		ijs_client_set_param(ijsdev->ctx, 0, "OutputFD", buf,
		                     strlen(buf));
		close(fd);
	} else {
	...
	if(code >= 0 && ijsdev->DeviceManufacturer)
		code =
		    ijs_client_set_param(ijsdev->ctx, 0, "DeviceManufacturer",
...
gsijs_close(gx_device* dev)
{
	...
	if(ijsdev->IjsParams)
		gs_free(dev->memory, ijsdev->IjsParams, ijsdev->IjsParams_size,
...
private
int
ijs_all_white(unsigned char* data, int size)
{
	int clean = 1;
	int i;
	for(i = 0; i < size; i++) {
		...
		for(y = 0; y < ijs_height; y++) {
			...
			if(ijsdev->ijs_version == HPIJS_1_0_VERSION &&
			   ijs_all_white(actual_data, row_bytes))
				status = ijs_client_send_data_wait(ijsdev->ctx,
				                                   0, NULL, 0);
			else
				status = ijs_client_send_data_wait(
				    ijsdev->ctx, 0, (char*)actual_data,
				    row_bytes);
			if(status)
				break;
		}
...
gsijs_get_params(gx_device* dev, gs_param_list* plist)
{
	...
	if(code >= 0) {
		if(ijsdev->DeviceManufacturer) {
			param_string_from_transient_string(
			    gps, ijsdev->DeviceManufacturer);
			code = param_write_string(plist, "DeviceManufacturer",
			                          &gps);
		} else {
	...
	if(code >= 0)
		code = param_write_int(plist, "BitsPerSample",
		                       &ijsdev->BitsPerSample);
                  ...
                  uint size, bool safety, bool only_when_closed)
{
	...
	case 0:
		...
		if(new_value.size < size) {
			strncpy(str, (const char*)new_value.data,
			        new_value.size);
...
gsijs_put_params(gx_device* dev, gs_param_list* plist)
{
	 ...
	 */
	if(code >= 0)
		code = gsijs_read_string(plist, "IjsServer", ijsdev->IjsServer,
		                         sizeof(ijsdev->IjsServer),
		                         dev->LockSafetyParams, is_open);

	if(code >= 0)
		code = gsijs_read_string_malloc(
		    plist, "DeviceManufacturer", &ijsdev->DeviceManufacturer,
		    &ijsdev->DeviceManufacturer_size, is_open);

	if(code >= 0)
		code = gsijs_read_string_malloc(
		    plist, "DeviceModel", &ijsdev->DeviceModel,
		    &ijsdev->DeviceModel_size, is_open);

	if(code >= 0)
		code = gsijs_read_string_malloc(
		    plist, "IjsParams", &(ijsdev->IjsParams),
		    &(ijsdev->IjsParams_size), is_open);

	if(code >= 0)
		code = gsijs_read_int(plist, "BitsPerSample",
		                      &ijsdev->BitsPerSample, 1, 16, is_open);
	...
	if(code >= 0) {
		code = gsijs_read_string_malloc(
		    plist, "ProcessColorModel", &ijsdev->ColorSpace,
		    &ijsdev->ColorSpace_size, is_open);
	}

	if(code >= 0) {
		code =
		    gsijs_read_bool(plist, "Tumble", &ijsdev->IjsTumble, false);
		if(code == 0)
...
gsijs_client_set_param(gx_device_ijs* ijsdev, const char* key,
                       const char* value)
{
	int code = ijs_client_set_param(ijsdev->ctx, 0 /* job id */, key, value,
	                                strlen(value));
	if(code < 0)
...
gsijs_set_color_format(gx_device_ijs* ijsdev)
{
	...
	if(!strcmp(ColorSpace, "DeviceGray")) {
		components = 1;
		if(bpc == 1) {
			ijsdev->procs.map_rgb_color =
			    gx_default_w_b_map_rgb_color;
			ijsdev->procs.map_color_rgb =
