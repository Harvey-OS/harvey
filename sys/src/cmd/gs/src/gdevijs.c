...
private
int gsijs_read_int(gs_param_list *plist, gs_param_name pname,
		   int *pval, int min_value, int max_value, bool only_when_closed);
private
int gsijs_read_bool(gs_param_list *plist, gs_param_name pname,
		    bool *pval, bool only_when_closed);
private
int gsijs_read_string(gs_param_list *plist, gs_param_name pname,
...
gsijs_output_page(gx_device *dev, int num_copies, int flush)
{
	...
	if((data = gs_alloc_bytes(pdev->memory, raster, "gsijs_output_page")) == (unsigned char *)NULL)
		return gs_note_error(gs_error_VMerror);
	...
	strcpy(buf, (n_chan == 4) ? "DeviceCMYK" : ((n_chan == 3) ? "DeviceRGB" : "DeviceGray"));
	...
	endcode = (pdev->buffer_space && !pdev->is_async_renderer ? clist_finish_page(dev, flush) : 0);
