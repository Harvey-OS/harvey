...
/* Copyright (C) 1989, 1991, 1993, 1994, 1997, 1999 Aladdin Enterprises.  All
  rights reserved.
...
/* Forward references */
private
int screen_sample(i_ctx_t*);
private
int set_screen_continue(i_ctx_t*);
private
int screen_cleanup(i_ctx_t*);
                  ...
                  int (*finish_proc)(i_ctx_t*), int space_index)
{
	...
	make_struct(esp + snumpush, space_index << r_space_shift,
	            penum); /* do early for screen_cleanup in case of error */
...
const op_def zht_op_defs[] = {{"0.currenthalftone", zcurrenthalftone},
