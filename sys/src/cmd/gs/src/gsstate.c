...
/* Forward references */
private
gs_state *gstate_alloc(gs_memory_t *, client_name_t,
		       const gs_state *);
private
...
private
int gstate_copy(gs_state *, const gs_state *,
...
gs_state_do_ptrs(e1) case gs_state_num_ptrs : /* handle device specially */
...
private
const gs_imager_state gstate_initial = {
	    ...
	    gs_state_copy_reason_t reason, client_name_t cname)
{
	 ...
	 */
		parts.effective_clip_path =
		    (pfrom->effective_clip_path == pfrom->view_clip ? pto->view_clip : parts.clip_path);
	} else
