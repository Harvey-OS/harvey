...
/* Forward references */
private
gs_state* gstate_alloc(gs_memory_t*, client_name_t, const gs_state*);
private
gs_state* gstate_clone(gs_state*, gs_memory_t*, client_name_t,
                       gs_state_copy_reason_t);
private
void gstate_free_contents(gs_state*);
private
int gstate_copy(gs_state*, const gs_state*, gs_state_copy_reason_t,
                client_name_t);
...
#define GSTATE_ASSIGN_PARTS(pto, pfrom)                                        \
	((pto)->path = (pfrom)->path, (pto)->clip_path = (pfrom)->clip_path,   \
	 ...
	 (pto)->ccolor = (pfrom)->ccolor,                                      \
	 (pto)->dev_color = (pfrom)->dev_color)
...
private
ENUM_PTRS_WITH(gs_state_enum_ptrs, gs_state* gsvptr)
ENUM_PREFIX(st_imager_state, gs_state_num_ptrs + 2);
#define e1(i, elt) ENUM_PTR(i, gs_state, elt);
gs_state_do_ptrs(e1) case gs_state_num_ptrs
...
ENUM_PTRS_END
private
RELOC_PTRS_WITH(gs_state_reloc_ptrs, gs_state* gsvptr)
{
...
private
const gs_imager_state gstate_initial = {gs_imager_state_initial(1.0)};
...
gs_state_alloc(gs_memory_t* mem)
{
	...
	*(gs_imager_state*)pgs = gstate_initial;
	...
	pgs->path =
	    gx_path_alloc(gstate_path_memory(mem), "gs_state_alloc(path)");
...
{
	gs_state* pnew =
	    gstate_clone(pgs, pgs->memory, "gs_gsave", copy_for_gsave);
...
gs_gsave_for_save(gs_state* pgs, gs_state** psaved)
{
	...
	if(old_cpath) {
		new_cpath = gx_cpath_alloc_shared(
		    old_cpath, pgs->memory, "gs_gsave_for_save(view_clip)");
		if(new_cpath == 0)
			return_error(gs_error_VMerror);
	} else {
		new_cpath = 0;
...
gs_state_update_overprint(gs_state* pgs, const gs_overprint_params_t* pparams)
{
	...
	if((code = gs_create_overprint(&pct, pparams, pgs->memory)) >= 0 &&
	   (code = dev_proc(dev, create_compositor)(dev, &ovptdev, pct, pis,
	                                            pgs->memory)) >= 0) {
		if(ovptdev != dev)
...
gs_initgraphics(gs_state* pgs)
{
	...
	gs_initmatrix(pgs);
	if((code = gs_newpath(pgs)) < 0 || (code = gs_initclip(pgs)) < 0 ||
	   ...
	   (code = gs_setcurvejoin(
	        pgs, gstate_initial.line_params.curve_join)) < 0 ||
	   ...
	   (code = gs_setmiterlimit(
	        pgs, gstate_initial.line_params.miter_limit)) < 0)
		return code;
	gs_init_rop(pgs);
	return 0;
}
...
private
int
gstate_alloc_parts(gs_state* parts, const gs_state* shared, gs_memory_t* mem,
                   client_name_t cname)
{
	gs_memory_t* path_mem = gstate_path_memory(mem);

	parts->path =
	    (shared ? gx_path_alloc_shared(shared->path, path_mem,
	                                   "gstate_alloc_parts(path)")
	            : gx_path_alloc(path_mem, "gstate_alloc_parts(path)"));
	parts->clip_path =
	    (shared ? gx_cpath_alloc_shared(shared->clip_path, mem,
	...
	if(!shared || shared->effective_clip_shared) {
		...
		parts->effective_clip_path = gx_cpath_alloc_shared(
		    shared->effective_clip_path, mem,
	...
	if(parts->path == 0 || parts->clip_path == 0 ||
	   parts->effective_clip_path == 0 || parts->color_space == 0 ||
...
{
	gs_state* pgs = gs_alloc_struct(mem, gs_state, &st_gs_state, cname);
             ...
             gs_state_copy_reason_t reason)
{
	...
	*pgs = *pfrom;
	...
	if(pgs->client_data != 0) {
		...
		if(pdata == 0 ||
		   gstate_copy_client_data(pgs, pdata, pfrom->client_data,
		                           reason) < 0)
			goto fail;
	}
...
private
int
gstate_copy(gs_state* pto, const gs_state* pfrom, gs_state_copy_reason_t reason,
            client_name_t cname)
{
	gs_state_parts parts;
...
#define RCCOPY(element) rc_pre_assign(pto->element, pfrom->element, cname)
	RCCOPY(device);
	RCCOPY(clip_stack);
...
/* Accessories. */
gs_id
gx_get_clip_path_id(gs_state* pgs)
{
