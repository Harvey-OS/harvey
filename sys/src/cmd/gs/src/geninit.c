 ...
 *    geninit [-(I|i) <prefix>] <init-file.ps> <gconfig.h> -c
 *<merged-init-file.c>
 *
...
/* Forward references */
private
FILE* prefix_open(const char* prefix, const char* inname);
private
void merge_to_c(const char* prefix, const char* inname, FILE* in, FILE* config,
                FILE* out);
private
void merge_to_ps(const char* prefix, const char* inname, FILE* in, FILE* config,
                 FILE* out);
...
#define MAX_STR 0xffff /* longest possible PostScript string token */
	...
	while((c = getc(in)) >= 0) {
		if(isxdigit(c)) {
			...
			if((which = !which)) {
				if(q == strbuf + MAX_STR) {
					fprintf(stderr, "Can't handle string "
					                "token > %d bytes.\n",
					        MAX_STR);
					exit(1);
				}
			...
			fprintf(stderr,
			        "Unknown character in ASCIIHex string: %c\n",
			        c);
	}
	len = q - strbuf;
	if(len <= 255) {
		...
		if(!strncmp(line, "%% Replace", 10) &&
		   sscanf(line + 11, "%d %s", &nlines, psname) == 2) {
			...
			if(psname[0] == '(') {
				...
				if(ps == 0)
					exit(1);
				mergefile(prefix, psname + 1, ps, config, out,
				          to_c, intact || do_intact);
			} else if(!strcmp(psname, "INITFILES")) {
				/*
				 * We don't want to bind config.h into geninit,
				 * so
				 * we parse it ourselves at execution time
				 * instead.
				 */
				rewind(config);
				while(rl(config, psname, LINE_SIZE))
					if(!strncmp(psname, "psfile_(\"", 9)) {
						...
						*quote = 0;
						ps = prefix_open(prefix,
						                 psname + 9);
						if(ps == 0)
							exit(1);
						mergefile(prefix, psname + 9,
						          ps, config, out, to_c,
						          false);
					}
			} else {
			...
			if(buf[0] !=
			       '%' && /* i.e. not special retained comment */
           ...
           FILE* out)
{
	...
	fputs("10};\n", out);
	fputs("const unsigned int gs_init_string_sizeof = "
	      "sizeof(gs_init_string);\n",
	      out);
}
