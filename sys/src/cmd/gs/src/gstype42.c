...
/* Forward references */
private
int append_outline_fitted(uint glyph_index, const gs_matrix* pmat,
                          gx_path* ppath, cached_fm_pair* pair,
                          const gs_log2_scale_point* pscale, bool design_grid);
private
uint default_get_glyph_index(gs_font_type42* pfont, gs_glyph glyph);
private
int default_get_outline(gs_font_type42* pfont, uint glyph_index,
                        gs_glyph_data_t* pgd);
...
gs_type42_font_init(gs_font_type42* pfont)
{
	...
	/* Now build the len_glyphs array since 'loca' may not be properly
	 * sorted */
	pfont->data.len_glyphs = (uint*)gs_alloc_bytes(
	    pfont->memory, pfont->data.numGlyphs * sizeof(uint),
	    "gs_type42_font");
	if(pfont->data.len_glyphs == 0)
		return_error(gs_error_VMerror);
	gs_font_notify_register((gs_font*)pfont, gs_len_glyphs_release,
	                        (void*)pfont);
	...
	if(i < loca_size) {
		...
		for(i = 0; i < loca_size - 1; i++) {
			...
			for(j = 1, glyph_length = 0x80000000; j < loca_size;
			    j++) {
				glyph_offset = get_glyph_offset(pfont, j);
				trial_glyph_length = glyph_offset - glyph_start;
				if((trial_glyph_length > 0) &&
				   (trial_glyph_length < glyph_length))
					glyph_length = trial_glyph_length;
			}
			pfont->data.len_glyphs[i] =
			    glyph_length < 0x80000000 ? glyph_length : 0;
		}
	}
	/*
...
gs_len_glyphs_release(void* data, void* event)
{
	...
	gs_font_notify_unregister((gs_font*)pfont, gs_len_glyphs_release,
	                          (void*)data);
	gs_free_object(pfont->memory, pfont->data.len_glyphs,
	               "gs_len_glyphs_release");
	return 0;
}
                ...
                const gs_matrix_fixed* pmat)
{
	...
	if(flags & TT_CG_ARGS_ARE_XY_VALUES) {
		...
		gs_point_transform2fixed(pmat, arg1 * factor, arg2 * factor,
		                         &pt);
	...
	if(S16(gdata) != -1) {
		...
		do {
			...
			if(code < 0)
				return code;
			total += code;
			parse_component(&gdata, &flags, &mat, NULL, pfont,
			                &mat);
                    ...
                    gs_glyph_data_t* pgd)
{
	...
	if(glyph_length == 0)
		...
		code = (*string_proc)(pfont,
		                      (ulong)(pfont->data.glyf + glyph_start),
		                      glyph_length, &data);
		if(code < 0)
			return code;
		if(code == 0)
			gs_glyph_data_from_string(pgd, data, glyph_length,
			                          NULL);
		else {
			/*
			...
			buf = (byte*)gs_alloc_string(pgd->memory, glyph_length,
			                             "default_get_outline");
			if(buf == 0)
				return_error(gs_error_VMerror);
			gs_glyph_data_from_string(pgd, buf, glyph_length,
			                          (gs_font*)pfont);
			for(;;) {
...
int
gs_type42_get_outline_from_TT_file(gs_font_type42* pfont, stream* s,
                                   uint glyph_index, gs_glyph_data_t* pgd)
{
	uchar lbuf[8];
	ulong glyph_start;
...
int
gs_type42_glyph_outline(gs_font* font, int WMode, gs_glyph glyph,
                        const gs_matrix* pmat, gx_path* ppath, double sbw[4])
{
	...
	/* fixme : The subpixel numbers doesn't pass through the
	   font_proc_glyph_outline interface.
	   High level devices can't get a proper grid fitting with AlignToPixels
	   = 1.
	   Currently font_proc_glyph_outline is only used by pdfwrite for
	   computing a
	...
	if((code = gx_path_current_point(ppath, &origin)) < 0 ||
	   ...
	   (code = font->procs.glyph_info(
	        font, glyph, pmat, GLYPH_INFO_WIDTH0 << WMode, &info)) < 0)
		return code;
	return gx_path_add_point(ppath,
	                         origin.x + float2fixed(info.width[WMode].x),
	                         origin.y + float2fixed(info.width[WMode].y));
}

/* Get glyph info by glyph index. */
int
gs_type42_glyph_info_by_gid(gs_font* font, gs_glyph glyph,
                            const gs_matrix* pmat, int members,
                            gs_glyph_info_t* info, uint glyph_index)
{
	gs_font_type42* const pfont = (gs_font_type42*)font;
	int default_members = members &
	                      ~(GLYPH_INFO_WIDTHS | GLYPH_INFO_NUM_PIECES |
	...
	if(default_members) {
		code = gs_default_glyph_info(font, glyph, pmat, default_members,
		                             info);

		if(code < 0)
			return code;
	} else if((code = pfont->data.get_outline(pfont, glyph_index,
	                                          &outline)) < 0)
	...
	if(members & GLYPH_INFO_WIDTHS) {
		...
		for(i = 0; i < 2; ++i)
			if(members & (GLYPH_INFO_WIDTH0 << i)) {
				...
				code = gs_type42_wmode_metrics(
				    pfont, glyph_index, i, sbw);
				if(code < 0) {
					code = 0;
					continue;
				}
				if(pmat) {
					code = gs_point_transform(
					    sbw[2], sbw[3], pmat,
					    &info->width[i]);
					if(code < 0)
						return code;
					code = gs_point_transform(
					    sbw[0], sbw[1], pmat, &info->v);
				} else {
					info->width[i].x = sbw[2],
					info->width[i].y = sbw[3];
	...
	if(members & (GLYPH_INFO_NUM_PIECES | GLYPH_INFO_PIECES)) {
		...
		int code =
		    parse_pieces(pfont, glyph, pieces, &info->num_pieces);

		if(code < 0)
			return code;
		info->members |=
		    members & (GLYPH_INFO_NUM_PIECES | GLYPH_INFO_PIECES);
	}
	return code;
}
int
                     ...
                     int members, gs_glyph_info_t* info)
{
	...
	return gs_type42_glyph_info_by_gid(font, glyph, pmat, members, info,
	                                   glyph_index);
}
int
	...
	if(glyph_data.bits.size != 0 && S16(glyph_data.bits.data) == -1) {
		...
		do {
			...
			if(flags & TT_CG_USE_MY_METRICS) {
				result = gs_type42_wmode_metrics(
				    pfont, comp_index, wmode, sbw);
				goto done;
			}
		} while(flags & TT_CG_MORE_COMPONENTS);
	}
...
int
gs_type42_get_metrics(gs_font_type42* pfont, uint glyph_index, float sbw[4])
{
...
int
gs_type42_append(uint glyph_index, gs_imager_state* pis, gx_path* ppath,
                 const gs_log2_scale_point* pscale, bool charpath_flag,
...
add_quadratic_curve(gx_path* const ppath, const gs_fixed_point* const a,
                    const gs_fixed_point* const b,
                    const gs_fixed_point* const c)
{
	return gx_path_add_curve(ppath, (a->x + 2 * b->x) / 3,
	                         (a->y + 2 * b->y) / 3, (c->x + 2 * b->x) / 3,
	                         (c->y + 2 * b->y) / 3, c->x, c->y);
}
              ...
              bool subglyph)
{
...

	{
		...
		if(!subglyph) {
			int xmin = S16(
			    gdata + 2); /* We like to see it with debugger. */
		...
		for(i = 0, np = 0; i < numContours; ++i) {
			...
			for(; np <= last_point; --reps, ++np) {
				...
				if(reps == 0) {
					flags = *pflags++;
					reps =
					    (flags & gf_Repeat ? *pflags++ + 1
					                       : 1);
				}
				if(flags & gf_xShort) {
					/*
					 * A bug in the Watcom compiler prevents
					 * us from doing
					 * the following with the obvious
				...
				if(flags & gf_yShort) {
					...
					dy = 0;
				code = gs_distance_transform2fixed(pmat, dx, dy,
				                                   &dpt);
				if(code < 0)
					return code;
				...
				if(is_start_off) {
					if(off_curve) {
						...
						p.y = (cpoints[1].y +
						       pt_start_off.y) /
						      2;
						code = add_quadratic_curve(
						    ppath, cpoints, cpoints + 1,
						    &p);
						if(code < 0)
							return code;
						code = add_quadratic_curve(
						    ppath, &p, &pt_start_off,
						    &start);
						if(code < 0)
							return code;
					} else {
...
private
int
check_component(uint glyph_index, const gs_matrix_fixed* pmat, gx_path* ppath,
                gs_font_type42* pfont, gs_fixed_point* ppts,
...
private
int
append_component(uint glyph_index, const gs_matrix_fixed* pmat, gx_path* ppath,
                 gs_fixed_point* ppts, int point_index, gs_font_type42* pfont,
                 bool subglyph)
{
	gs_glyph_data_t glyph_data;
	int code;
	...
	glyph_data.memory = pfont->memory;
	code = check_component(glyph_index, pmat, ppath, pfont,
	                       ppts + point_index, &glyph_data, subglyph);
	if(code != 1)
		return code;
	/*
	 ...
	 */
	{
		...
		do {
			...
			if(mp[0] >= 0) {
				...
				const gs_fixed_point* const pfrom =
				    ppts + mp[0];
				/*
				 * Contrary to the TrueType documentation, mp[1]
				 * is not
				 * relative to the start of the compound glyph,
				 * but is
				 * relative to the start of the component.
				 */
				const gs_fixed_point* const pto =
				    ppts + point_index + mp[1];
				gs_fixed_point diff;

				code = append_component(comp_index, &mat, NULL,
				                        ppts, point_index,
				                        pfont, true);
				if(code < 0)
					break;
...
private
int
append_outline_fitted(uint glyph_index, const gs_matrix* pmat, gx_path* ppath,
                      cached_fm_pair* pair, const gs_log2_scale_point* pscale,
                      bool design_grid)
{
