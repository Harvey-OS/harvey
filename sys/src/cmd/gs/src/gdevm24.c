...
/* Procedures */
declare_mem_procs(mem_true24_copy_mono, mem_true24_copy_color,
                  mem_true24_fill_rectangle);
private
dev_proc_copy_alpha(mem_true24_copy_alpha);

/* The device descriptor. */
const gx_device_memory mem_true24_device = mem_full_alpha_device(
    "image24", 24, 0, mem_open, gx_default_rgb_map_rgb_color,
    gx_default_rgb_map_color_rgb, mem_true24_copy_mono, mem_true24_copy_color,
    mem_true24_fill_rectangle, gx_default_map_cmyk_color, mem_true24_copy_alpha,
...
#define put3(ptr, r, g, b) (ptr)[0] = r, (ptr)[1] = g, (ptr)[2] = b
...
#else
#define set_color24_cache(crgb, r, g, b)                                       \
	mdev->color24.rgbr = rgbr = ((bits32)(r) << 24) |                      \
	                            ((bits32)(b) << 16) | ((bits16)(g) << 8) | \
	                            (r),                                       \
...
private
int
mem_true24_fill_rectangle(gx_device* dev, int x, int y, int w, int h,
                          gx_color_index color)
{
	...
	if(w >= 5) {
		...
		if(r == g && r == b) {
#ifndef USE_MEMSET
			/* We think we can do better than the library's
			 * memset.... */
			int bcntm7 = w * 3 - 7;
...
#ifdef USE_MEMCPY
				/*
				 * For very wide fills, it's most efficient to
				 * fill a few
...
private
int
mem_true24_copy_mono(gx_device* dev, const byte* base, int sourcex, int sraster,
...
private
int
mem_true24_copy_color(gx_device* dev, const byte* base, int sourcex,
...
/* Procedures */
declare_mem_procs(mem24_word_copy_mono, mem24_word_copy_color,
                  mem24_word_fill_rectangle);

/* Here is the device descriptor. */
const gx_device_memory mem_true24_word_device =
    mem_full_device("image24w", 24, 0, mem_open, gx_default_rgb_map_rgb_color,
                    gx_default_rgb_map_color_rgb, mem24_word_copy_mono,
                    mem24_word_copy_color, mem24_word_fill_rectangle,
...
private
int
mem24_word_copy_mono(gx_device* dev, const byte* base, int sourcex, int sraster,
                     gx_bitmap_id id, int x, int y, int w, int h,
                     gx_color_index zero, gx_color_index one)
{
	gx_device_memory* const mdev = (gx_device_memory*)dev;
	...
	mem_true24_copy_mono(dev, base, sourcex, sraster, id, x, y, w, h, zero,
	                     one);
...
private
int
mem24_word_copy_color(gx_device* dev, const byte* base, int sourcex,
                      int sraster, gx_bitmap_id id, int x, int y, int w, int h)
{
	gx_device_memory* const mdev = (gx_device_memory*)dev;
	byte* row;
