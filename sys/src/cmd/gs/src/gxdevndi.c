...
private
const gx_color_value q0[] = {0};
private
const gx_color_value q1[] = {0, frac_color_(1, 1)};
private
const gx_color_value q2[] = {0, frac_color_(1, 2), frac_color_(2, 2)};
private
const gx_color_value q3[] = {0, frac_color_(1, 3), frac_color_(2, 3),
                             frac_color_(3, 3)};
private
const gx_color_value q4[] = {0, frac_color_(1, 4), frac_color_(2, 4),
                             frac_color_(3, 4), frac_color_(4, 4)};
private
const gx_color_value q5[] = {0, frac_color_(1, 5), frac_color_(2, 5),
                             frac_color_(3, 5), frac_color_(4, 5),
                             frac_color_(5, 5)};
private
const gx_color_value q6[] = {0, frac_color_(1, 6), frac_color_(2, 6),
                             frac_color_(3, 6), frac_color_(4, 6),
                             frac_color_(5, 6), frac_color_(6, 6)};
private
const gx_color_value q7[] = {
...
const gx_color_value* const fc_color_quo[8] = {q0, q1, q2, q3, q4, q5, q6, q7};
...
private
int
gx_render_device_DeviceN_wts(frac* pcolor, gx_device_color* pdevc,
 ...
 */
int
gx_render_device_DeviceN(frac* pcolor, gx_device_color* pdevc, gx_device* dev,
                         gx_device_halftone* pdht, const gs_int_point* ht_phase)
{
	...
	for(i = 0; i < num_colors; i++) {
		unsigned long hsize =
		    pdht ? (unsigned)pdht->components[i].corder.num_levels : 1;
	...
	if(!dither_check) {
		for(i = 0; i < num_colors; i++)
			vcolor[i] =
			    fractional_color(int_color[i], max_value[i]);
	...
	color_set_phase_mod(pdevc, ht_phase->x, ht_phase->y, pdht->lcm_width,
	                    pdht->lcm_height);
...
gx_devn_reduce_colored_halftone(gx_device_color* pdevc, gx_device* dev)
{
	...
	if(planes == 0) {
		 ...
		 * (and only if) the difference of each component from the
		 * nearest
		...
		if(invert) {
			...
			color_set_binary_halftone_component(pdevc, pdht, i, c1,
			                                    c0, level);
		} else
			color_set_binary_halftone_component(pdevc, pdht, i, c0,
			                                    c1, level);
