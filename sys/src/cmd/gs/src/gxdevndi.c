			 ...
			 gx_device_halftone *pdht, const gs_int_point *ht_phase)
{
	...
	for(i = 0; i < num_colors; i++) {
		max_value[i] = (dev->color_info.gray_index == i) ? dev->color_info.dither_grays - 1 : dev->color_info.dither_colors - 1;
	}

	for(i = 0; i < num_colors; i++) {
		unsigned long hsize = pdht ? (unsigned)pdht->components[i].corder.num_levels
					   : 1;
