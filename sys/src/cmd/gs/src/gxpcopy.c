...
private
void adjust_point_to_tangent(segment*, const segment*, const gs_fixed_point*);
                      ...
                      gx_path_copy_options options)
{
	...
	if(options & pco_for_stroke) {
		...
		expansion.x =
		    float2fixed((fabs(pis->ctm.xx) + fabs(pis->ctm.yx)) *
		                width) *
		    2;
		expansion.y =
		    float2fixed((fabs(pis->ctm.xy) + fabs(pis->ctm.yy)) *
		                width) *
		    2;
	}
			...
			if(fixed_flatness == max_fixed) { /* don't flatten */
				if(options & pco_monotonize)
					...
					code = gx_path_add_curve_notes(
					    ppath, pc->p1.x, pc->p1.y, pc->p2.x,
					    pc->p2.y, pc->pt.x, pc->pt.y,
					    pseg->notes);
			} else {
				...
				if(options & pco_for_stroke) {
					/*
					 * When flattening for stroking, the
					 * flatness
					 * must apply to the outside of the
					 * resulting
					 * stroked region.  We approximate this
					 * by
					 * dividing the flatness by the ratio of
					 * the
					 * expanded bounding box to the original
					 * bounding box.  This is crude, but
					 * pretty
					 * simple to calculate, and produces
					 * reasonably
					 * good results.
					 */
...
#define SET_EXTENT(r, c0, c1, c2, c3)                                          \
	...
	END
					SET_EXTENT(ex, x0, pc->p1.x, pc->p2.x,
					           pc->pt.x);
					SET_EXTENT(ey, y0, pc->p1.y, pc->p2.y,
					           pc->pt.y);
#undef SET_EXTENT
					/*
					 * We check for the degenerate case
					 * specially
					...
					if(ex == 0 || ey == 0)
						...
						flat_x = fixed_mult_quo(
						    fixed_flatness, ex,
						    ex + expansion.x);
						flat_y = fixed_mult_quo(
						    fixed_flatness, ey,
						    ey + expansion.y);
						flat = min(flat_x, flat_y);
						k = gx_curve_log2_samples(
						    x0, y0, pc, flat);
					}
				} else
					k = gx_curve_log2_samples(x0, y0, pc,
					                          flat);
				if(options & pco_accurate) {
					...
					notes |= sn_not_first;
					cseg = *pc;
					code = gx_subdivide_curve(ppath, k,
					                          &cseg, notes);
					if(code < 0)
						break;
					/*
					 * Adjust the first and last segments so
					 * that
					 * they line up with the tangents.
					 */
					end = ppath->current_subpath->last;
					vd_lineto(ppath->position.x,
					          ppath->position.y);
					if((code = gx_path_add_line_notes(
					        ppath, ppath->position.x,
					        ppath->position.y,
					        pseg->notes | sn_not_first)) <
					   0)
						break;
					if(start->next->pt.x != pc->p1.x ||
					   start->next->pt.y != pc->p1.y)
						adjust_point_to_tangent(
						    start, start->next,
						    &pc->p1);
					else if(start->next->pt.x != pc->p2.x ||
					        start->next->pt.y != pc->p2.y)
						adjust_point_to_tangent(
						    start, start->next,
						    &pc->p2);
					else
						adjust_point_to_tangent(
						    start, start->next,
						    &end->prev->pt);
					if(end->prev->pt.x != pc->p2.x ||
					   end->prev->pt.y != pc->p2.y)
						adjust_point_to_tangent(
						    end, end->prev, &pc->p2);
					else if(end->prev->pt.x != pc->p1.x ||
					        end->prev->pt.y != pc->p1.y)
						adjust_point_to_tangent(
						    end, end->prev, &pc->p1);
					else
						adjust_point_to_tangent(
						    end, end->prev, &start->pt);
				} else {
					cseg = *pc;
					code = gx_subdivide_curve(ppath, k,
					                          &cseg, notes);
		...
		case s_line:
			code = gx_path_add_line_notes(ppath, pseg->pt.x,
			                              pseg->pt.y, pseg->notes);
	...
	if(ppath_old->bbox_set) {
		if(ppath->bbox_set) {
			ppath->bbox.p.x =
			    min(ppath_old->bbox.p.x, ppath->bbox.p.x);
			ppath->bbox.p.y =
			    min(ppath_old->bbox.p.y, ppath->bbox.p.y);
			ppath->bbox.q.x =
			    max(ppath_old->bbox.q.x, ppath->bbox.q.x);
			ppath->bbox.q.y =
			    max(ppath_old->bbox.q.y, ppath->bbox.q.y);
		} else {
			ppath->bbox_set = true;
...
bool
gx_path__check_curves(const gx_path* ppath, gx_path_copy_options options,
                      fixed fixed_flat)
{
	...
	while(pseg) {
		...
		case s_curve: {
			...
			if(options & pco_monotonize) {
				double t[2];
				int nz = gx_curve_monotonic_points(
				    pt0.y, pc->p1.y, pc->p2.y, pc->pt.y, t);

				if(nz != 0)
					return false;
				nz = gx_curve_monotonic_points(
				    pt0.x, pc->p1.x, pc->p2.x, pc->pt.x, t);
				if(nz != 0)
					return false;
			}
			if(options & pco_small_curves) {
				fixed ax, bx, cx, ay, by, cy;
				int k = gx_curve_log2_samples(pt0.x, pt0.y, pc,
				                              fixed_flat);

				if(!curve_coeffs_ranged(
				       pt0.x, pc->p1.x, pc->p2.x, pc->pt.x,
				       pt0.y, pc->p1.y, pc->p2.y, pc->pt.y, &ax,
				       &bx, &cx, &ay, &by, &cy, k))
					return false;
			}
		} break;
...
gx_curve_monotonize(gx_path* ppath, const curve_segment* pc)
{
	...
	n1 =
	    gx_curve_monotonic_points(y0, pc->p1.y, pc->p2.y, pc->pt.y, t + n0);
	n = n0 + n1;
	if(n == 0)
		return gx_path_add_curve_notes(ppath, pc->p1.x, pc->p1.y,
		                               pc->p2.x, pc->p2.y, pc->pt.x,
		                               pc->pt.y, notes);
	if(n0 > 0)
		c[0] = 1;
	if(n0 > 1)
	...
	for(i = k + 1, j = k; i < n && t[k] < 1 - delta; i++)
		if(any_abs(t[i] - t[j]) < delta) {
			t[j] =
			    (t[j] + t[i]) / 2; /* Unlikely 3 roots are close. */
			c[j] |= c[i];
		} else {
			j++;
			t[j] = t[i];
	...
	n = j + 1;
	/* Do split : */
	curve_points_to_coefficients(x0, pc->p1.x, pc->p2.x, pc->pt.x, ax, bx,
	                             cx, v01, v12);
	curve_points_to_coefficients(y0, pc->p1.y, pc->p2.y, pc->pt.y, ay, by,
	                             cy, v01, v12);
	...
	for(i = k; i < n; i++) {
		...
		double x = x0 * omt3 + 3 * pc->p1.x * omt2 * ti +
		           3 * pc->p2.x * omt * t2 + pc->pt.x * t3;
		double y = y0 * omt3 + 3 * pc->p1.y * omt2 * ti +
		           3 * pc->p2.y * omt * t2 + pc->pt.y * t3;
		double ddx = (c[i] & 1 ? 0 : ax * t2 + bx * ti +
		                                 cx); /* Suppress noize. */
		...
		if((double)(sx - px) * rx + (double)(sy - py) * ry < 0)
			rx = -rx, ry = -qy;
		/* Do add : */
		code = gx_path_add_curve_notes(ppath, px + qx, py + qy, sx - rx,
		                               sy - ry, sx, sy, notes);
		if(code < 0)
			return code;
		notes |= sn_not_first;
	...
	if((double)(sx - px) * rx + (double)(sy - py) * ry < 0)
		rx = -rx, ry = -qy;
	return gx_path_add_curve_notes(ppath, px + qx, py + qy, sx - rx,
	                               sy - ry, sx, sy, notes);
}
 ...
 */
int
gx_curve_monotonic_points(fixed v0, fixed v1, fixed v2, fixed v3, double pst[2])
{
	/*
	   Let
	...
	/*
	   ...
	   Note that we just determined that sign(a) != sign(b), so we know t1 >
	   0.
	 */
	 ...
	 */
	{
		...
		}
		{
			 ...
			 * We know that root is non-negative, but a3f may be
			 * either
			 * positive or negative, so we need to check the
			 * ordering
			 * explicitly.
			 */
...
find_contacting_segments(const subpath* sp0, segment* sp0last,
                         const subpath* sp1, segment* sp1last, segment** sc0,
                         segment** sc1)
{
	segment* s0, *s1;
	...
	/* This is a simplified algorithm, which only checks for quazi-colinear
	   vertical lines.
	...
	for(s0 = sp0last, count0 = 0;
	    count0 < search_limit && s0 != (segment*)sp0;
	    s0 = s0->prev, count0++) {
		s0s = s0->prev;
		if(s0->type == s_line &&
		   (s0s->pt.x == s0->pt.x ||
			...
			for(s1 = sp1last, count1 = 0;
			    count1 < search_limit && s1 != (segment*)sp1;
			    s1 = s1->prev, count1++) {
				s1s = s1->prev;
				if(s1->type == s_line &&
				   (s1s->pt.x == s1->pt.x ||
				    (any_abs(s1s->pt.x - s1->pt.x) == 1 &&
				     any_abs(s1s->pt.y - s1->pt.y) >
				         min_length))) {
					if(s0s->pt.x == s1s->pt.x ||
					   s0->pt.x == s1->pt.x ||
					   s0->pt.x == s1s->pt.x ||
					   s0s->pt.x == s1->pt.x) {
						if(s0s->pt.y < s0->pt.y &&
...
gx_path_merge_contacting_contours(gx_path* ppath)
{
	...
	for(; sp0 != NULL; sp0 = (subpath*)sp0->last->next) {
		...
		for(count = 0; sp1 != NULL && count < window;
		    sp1 = spnext, count++) {
			segment* sp1last = sp1->last;
			segment* sc0, *sc1;

			spnext = (subpath*)sp1last->next;
			if(find_contacting_segments(sp0, sp0last, sp1, sp1last,
			                            &sc0, &sc1)) {
				...
				sp1->prev = 0;
				sp1last->next = 0;
				/* Change 'closepath' of the subpath 1 to a line
				 * (maybe degenerate) : */
				...
				/* Rotate the subpath 1 to sc1 : */
				{
					 ...
					 */
					if(ppath->segments->contents
					       .subpath_current == sp1) {
						ppath->segments->contents
						    .subpath_current = sp1p;
					}
					gs_free_object(ppath->memory, sp1,
					               "gx_path_merge_"
					               "contacting_contours");
					sp1 = 0; /* Safety. */
				}
				/* Insert the subpath 1 into the subpath 0
				 * before sc0 :*/
				sc0->prev->next = sc1;
				sc1->prev = sc0->prev;
				sp1last->next = sc0;
				sc0->prev = sp1last;
				/* Remove degenearte "bridge" segments : (fixme:
				 * Not done due to low importance). */
