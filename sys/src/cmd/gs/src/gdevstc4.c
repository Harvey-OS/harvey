...
escp2c_pick_best(byte* col)
{
	...
	/*
	   Note that I don't use a simple distance algorithm. That can lead to a
	   block
	   of (130,127,127) being dithered as red-cyan. This algorithm should
	   make
	   it use black-white-red. This is very important, as a coloured block
	   in
...
stc_fs2(stcolor_device* sd, int npixel, byte* in, byte* buf, byte* out)
{
	...
	if(npixel > 0) { /* npixel >  0 -> scanline-processing       */
		...
		if(in == NULL) { /* clear the error-buffer upon white-lines */
			...
			p = buf;
			if(*p != 0 || memcmp((char*)p, (char*)p + 1,
			                     fullcolor_line_size - 1)) {
