		...
		const double Metrics2_sbw_default[4])
{
	...
	if(have_cdevproc || zchar_show_width_only(penum)) {
		...
		if(have_cdevproc) {
	       ...
	       from FontBBox. */
			if(!metrics2 || (penum->current_font->FontType == ft_CID_encrypted && metrics2_use_default)) {
				w2[6] = w2[0], w2[7] = w2[1];
				w2[8] = w2[9] = 0;
			}
			nparams = 10;
	...
	{
		int code =
		    (metrics2 ? gs_text_setcachedevice2(penum, w2) : gs_text_setcachedevice(penum, w2));
