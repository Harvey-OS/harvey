...
/* Copyright (C) 1996, 1997, 1999, 2000 Aladdin Enterprises.  All rights
  reserved.
                      ...
                      double psbw[4])
{
	...
	if(dict_find_string(pfdict, "Metrics", &pmdict) > 0) {
		...
		if(dict_find(pmdict, pcnref, &pmvalue) > 0) {
			if(num_params(pmvalue, 1, psbw + 2) >=
			   0) { /* <wx> only */
				psbw[3] = 0;
				...
				case 2: /* [<sbx> <wx>] */
					code = num_params(
					    pmvalue->value.refs + 1, 2, psbw);
					psbw[2] = psbw[1];
					psbw[1] = psbw[3] = 0;
					break;
				case 4: /* [<sbx> <sby> <wx> <wy>] */
					code = num_params(
					    pmvalue->value.refs + 3, 4, psbw);
					break;
				default:
...
int
zchar_get_metrics2(const gs_font_base* pbfont, const ref* pcnref, double pwv[4])
{
	const ref* pfdict = &pfont_data(gs_font_parent(pbfont))->dict;
	ref* pmdict;
	...
	if(dict_find_string(pfdict, "Metrics2", &pmdict) > 0) {
		...
		if(dict_find(pmdict, pcnref, &pmvalue) > 0) {
			...
			if(r_size(pmvalue) == 4) {
				int code =
				    num_params(pmvalue->value.refs + 3, 4, pwv);
 ...
 */
int
zchar_set_cache(i_ctx_t* i_ctx_p, const gs_font_base* pbfont, const ref* pcnref,
                const double psb[2], const double pwidth[2],
                const gs_rect* pbbox, op_proc_t cont, op_proc_t* exec_cont,
                const double Metrics2_sbw_default[4])
{
	...
	if(have_cdevproc || zchar_show_width_only(penum)) {
		...
		if(have_cdevproc) {
			...
			/* If we have cdevproc and the font type is CID type 0,
			   we'll throw away Metrics2_sbw_default that is
			   calculated
			   from FontBBox. */
			if(!metrics2 ||
	...
	{
		int code = (metrics2 ? gs_text_setcachedevice2(penum, w2)
 ...
 */
private
bool charstring_is_notdef_proc(const gs_memory_t* mem, const ref*);
private
int charstring_make_notdef(gs_glyph_data_t*, gs_font*);
int
zchar_charstring_data(gs_font* font, const ref* pgref, gs_glyph_data_t* pgd)
{
	...
	if(!r_has_type(pcstr, t_string)) {
		 ...
		 * To prevent this from making the font unembeddable in PDF
		 *files
...
charstring_is_notdef_proc(const gs_memory_t* mem, const ref* pcstr)
{
	if(r_is_array(pcstr) && r_size(pcstr) == 4) {
		...
		if(r_has_type(&elts[0], t_name) &&
		   r_has_type(&elts[1], t_integer) &&
		   elts[1].value.intval == 0 &&
		   r_has_type(&elts[2], t_integer) &&
		   elts[2].value.intval == 0 && r_has_type(&elts[3], t_name)) {
			ref nref;
...
charstring_make_notdef(gs_glyph_data_t* pgd, gs_font* font)
{
	...
	static const byte char_data[4] = {139, /* 0 */
	                                  139, /* 0 */
	                                  c1_hsbw, cx_endchar};
	uint len = max(pfont->data.lenIV, 0) + sizeof(char_data);
	byte* chars =
	    gs_alloc_string(font->memory, len, "charstring_make_notdef");
 ...
 */
int
zchar_enumerate_glyph(const gs_memory_t* mem, const ref* prdict, int* pindex,
                      gs_glyph* pglyph)
{
	int index = *pindex - 1;
	ref elt[2];
