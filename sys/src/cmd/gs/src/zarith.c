	...
	case t_integer:
		...
		case t_integer: {
			...
			if(((op[-1].value.intval += int2) ^ int2) < 0 &&
			   ((op[-1].value.intval - int2) ^ int2) >= 0) { /* Overflow, convert to real */
	...
	case t_integer:
		...
		case t_integer: {
			...
			if((int1 ^ (op[-1].value.intval = int1 - op->value.intval)) < 0 &&
			   (int1 ^ op->value.intval) < 0) { /* Overflow, convert to real */
...
ztruncate(i_ctx_t *i_ctx_p)
{
	...
	case t_real:
		op->value.realval =
		    (op->value.realval < 0.0 ? ceil(op->value.realval) : floor(op->value.realval));
	case t_integer:
		;
	}
	return 0;
