...
zop_add(register os_ptr op)
{
	...
	case t_real:
		...
		case t_real:
			...
			break;
		case t_integer:
			make_real(op - 1, (double)op[-1].value.intval +
			                      op->value.realval);
		}
		break;
	case t_integer:
		...
		case t_integer: {
			...
			if(((op[-1].value.intval += int2) ^ int2) < 0 &&
			   ((op[-1].value.intval - int2) ^ int2) >=
			       0) { /* Overflow, convert to real */
				make_real(op - 1,
				          (double)(op[-1].value.intval - int2) +
				              int2);
			}
		}
		}
...
zdiv(i_ctx_t* i_ctx_p)
{
	...
	case t_real:
		...
		case t_real:
			...
			break;
		case t_integer:
			make_real(op1, (double)op1->value.intval /
			                   op->value.realval);
		}
		break;
	case t_integer:
...
zop_sub(register os_ptr op)
{
	...
	case t_integer:
		...
		case t_integer: {
			...
			if((int1 ^ (op[-1].value.intval =
			                int1 - op->value.intval)) < 0 &&
			   (int1 ^ op->value.intval) <
			       0) { /* Overflow, convert to real */
				make_real(op - 1,
				          (float)int1 - op->value.intval);
			}
		}
		}
	}
...
zidiv(i_ctx_t* i_ctx_p)
{
	...
	if(op->value.intval == 0)
		return_error(e_undefinedresult);
	if((op[-1].value.intval /= op->value.intval) == MIN_INTVAL &&
	   op->value.intval == -1) { /* Anomalous boundary case, fail. */
...
ztruncate(i_ctx_t* i_ctx_p)
{
	...
	case t_real:
		op->value.realval =
		    (op->value.realval < 0.0 ? ceil(op->value.realval)
...
const op_def zarith_op_defs[] = {{"1abs", zabs},
                                 {"2add", zadd},
                                 {"2.bitadd", zbitadd},
                                 {"1ceiling", zceiling},
                                 {"2div", zdiv},
