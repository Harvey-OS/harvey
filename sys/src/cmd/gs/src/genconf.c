...
/* Copyright (C) 1993, 1996, 1997, 1998, 1999, 2000 Aladdin Enterprises.  All
  rights reserved.
 ...
 * small allocations.
 */
static const config_t init_config = {0,                      /* debug */
...
/* Forward definitions */
private
void* mrealloc(void*, size_t, size_t);
...
main(int argc, char* argv[])
{
	...
	for(i = 1; i < argc; i++) {
		...
		case 'f':
			...
			fputs("/* This file was generated automatically by "
			      "genconf.c. */\n",
			      out);
			      ...
			      out);
			{
				...
				sprintf(
				    ...
				    conf.name_prefix);
				write_list(out, &conf.lists.named.fonts,
				           template);
			}
			break;
		case 'h':
			...
			write_list(out, &conf.lists.named.sorted_resources,
			           "%s\n");
			break;
		case 'l':
			lib = 1;
		...
		lo:
			...
			if(obj) {
				...
				write_list_pattern(out, &conf.lists.named.objs,
				                   &conf.obj_p);
			}
			if(lib) {
				sort_uniq(&conf.lists.named.libs, true);
				sort_uniq(&conf.lists.named.links, true);
				write_list_pattern(out,
				                   &conf.lists.named.libpaths,
				                   &conf.libpath_p);
				write_list_pattern(out, &conf.lists.named.links,
				                   &conf.obj_p);
				write_list_pattern(out, &conf.lists.named.libs,
				                   &conf.lib_p);
			}
			break;
		default:
	...
	for(i = 0; i < pconf->replaces.count; ++i) {
		...
		for(j = 0; j < pconf->file_names.count; ++j) {
			...
			if(!strcmp(fname, bufname)) {
				...
				/* Delete all resources associated with this
				 * file. */
				{
					int rn;

					for(rn = 0; rn < NUM_RESOURCE_LISTS;
					    ++rn) {
						string_item_t* items =
						    pconf->lists.indexed[rn]
						        .items;
						int count =
						    pconf->lists.indexed[rn]
						        .count;
...
#define IS_CAT(str) !strcmp(category, str)
		...
		case 'e':
			if(IS_CAT("emulator")) {
				sprintf(str, "emulator_(\"%s\",%u)", item,
				        (uint)strlen(item));
				item = str;
				break;
			}
		...
		case 'f':
			if(IS_CAT("font")) {
				...
				pat =
				    "function_type_(%%s,%sbuild_function_%%s)";
			} else
				goto err;
			goto pre;
		case 'h':
		...
		case 'p':
			if(IS_CAT("ps")) {
				sprintf(str, "psfile_(\"%s.ps\",%u)", item,
				        (uint)(strlen(item) + 3));
				item = str;
				break;
...
#undef IS_CAT
		default:
		err:
			fprintf(stderr, "Definition not recognized: %s %s.\n",
			        category, item);
			exit(1);
		}
		if(pat) {
...
add_item(string_list_t* list, const char* str, int file_index)
{
	...
	if(count >= list->max_count) {
		...
		if(list->max_count < 20)
			list->max_count = 20;
		list->items = (string_item_t*)mrealloc(
		    list->items, (list->max_count >> 1) * sizeof(string_item_t),
		    list->max_count * sizeof(string_item_t));
		assert(list->items != NULL);
	}
