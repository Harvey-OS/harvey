...
/* Copyright (C) 1989, 1995, 1996, 1997 Aladdin Enterprises.  All rights
  reserved.
...
/* Forward references */
private
void near win_addtool(gx_device_win_ddb*, int);
private
void near win_maketools(gx_device_win_ddb*, HDC);
private
void near win_destroytools(gx_device_win_ddb*);

/* Device procedures */

/* See gxdevice.h for the definitions of the procedures. */
private
dev_proc_open_device(win_ddb_open);
private
dev_proc_close_device(win_ddb_close);
private
dev_proc_map_rgb_color(win_ddb_map_rgb_color);
private
dev_proc_fill_rectangle(win_ddb_fill_rectangle);
private
dev_proc_tile_rectangle(win_ddb_tile_rectangle);
private
dev_proc_copy_mono(win_ddb_copy_mono);
private
dev_proc_copy_color(win_ddb_copy_color);

/* Windows-specific procedures */
private
win_proc_copy_to_clipboard(win_ddb_copy_to_clipboard);
private
win_proc_repaint(win_ddb_repaint);
private
win_proc_alloc_bitmap(win_ddb_alloc_bitmap);
private
win_proc_free_bitmap(win_ddb_free_bitmap);
...
private
const gx_device_procs win_ddb_procs = {
    win_ddb_open, NULL, /* get_initial_matrix */
    win_sync_output, win_output_page, win_ddb_close, win_ddb_map_rgb_color,
    win_map_color_rgb, win_ddb_fill_rectangle, win_ddb_tile_rectangle,
    win_ddb_copy_mono, win_ddb_copy_color, NULL, /* draw_line */
    NULL,                                        /* get_bits */
    win_get_params, win_put_params, NULL,        /* map_cmyk_color */
    win_get_xfont_procs};
...
private
int
win_ddb_tile_rectangle(gx_device* dev, const gx_tile_bitmap* tile, int x, int y,
...
#define copy_tile(srcx, srcy, tx, ty, tw, th)                                  \
	BitBlt(wdev->hdcbit, tx, ty, tw, th, wdev->hdcmono, srcx, srcy,        \
	       rop_write_at_1s)
...
private
int
win_ddb_copy_mono(gx_device* dev, const byte* base, int sourcex, int raster,
...
private
int
win_ddb_copy_color(gx_device* dev, const byte* base, int sourcex, int raster,
                   gx_bitmap_id id, int x, int y, int w, int h)
{
	fit_copy(dev, base, sourcex, raster, id, x, y, w, h);
		...
		case 4: { /* color device, four bits per pixel */
			...
			while(h--) { /* for each line */
				...
				if(sourcex & 1) { /* odd nibble first */
					...
					SetPixel(wdev->hdcbit, dest_x, dest_y,
					         PALETTEINDEX(color));
					dest_x++;
				}
				/* Now do full bytes */
...
{ /* make somewhere to put it and copy */
	...
	HBITMAP bitmap =
	    CreateCompatibleBitmap(hdcbit, dev->width, dev->height);
	...
	if(bitmap) {
		...
		BitBlt(mem, 0, 0, dev->width, dev->height, hdcbit, 0, 0,
		       SRCCOPY);
		DeleteDC(mem);
...
win_ddb_alloc_bitmap(gx_device_win* dev, gx_device* param_dev)
{
	...
	hdc = GetDC(wdev->hwndimg);
	for(i = 0;; i++) {
		wdev->hbitmap = CreateCompatibleBitmap(hdc, param_dev->width,
		                                       param_dev->height);
	...
	wdev->hdcbit =
	    CreateCompatibleDC(hdc); /* create Device Context for drawing */
