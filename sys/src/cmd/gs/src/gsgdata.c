...
private
ENUM_PTRS_WITH(gs_glyph_data_enum_ptrs, gs_glyph_data_t* pgd)
case 0:
return ENUM_CONST_BYTESTRING(&pgd->bits);
case 1:
return ENUM_OBJ(pgd->proc_data);
ENUM_PTRS_END
private
RELOC_PTRS_WITH(gs_glyph_data_reloc_ptrs, gs_glyph_data_t* pgd)
{
...
{
	gs_free_const_bytestring(((gs_font*)pgd->proc_data)->memory, &pgd->bits,
	                         cname);
}
private
int
glyph_data_substring_by_font(gs_glyph_data_t* pgd, uint offset, uint size)
{
	...
	pgd->bits.data = gs_resize_string(
	    font->memory, data, pgd->bits.size, size,
...
private
const gs_glyph_data_procs_t no_free_procs = {glyph_data_free_permanent,
                                             glyph_data_substring_permanent};
private
const gs_glyph_data_procs_t free_by_font_procs = {glyph_data_free_by_font,
                                                  glyph_data_substring_by_font};
 ...
 */
void
gs_glyph_data_from_string(gs_glyph_data_t* pgd, const byte* data, uint size,
                          gs_font* font)
{
...
void
gs_glyph_data_from_bytes(gs_glyph_data_t* pgd, const byte* bytes, uint offset,
