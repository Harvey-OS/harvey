...
private
int
margin_boundary(line_list *ll, margin_set *set, active_line *alp,
...
int
continue_margin_common(line_list *ll, margin_set *set, active_line *flp, active_line *alp, fixed y0, fixed y1)
{
	int code;
#if ADJUST_SERIF
	section *sect = set->sect;
...
private
inline int
mark_margin_interior(line_list *ll, margin_set *set, active_line *flp, active_line *alp, fixed y, fixed y0, fixed y1)
{
	section *sect = set->sect;
...
int
margin_interior(line_list *ll, active_line *flp, active_line *alp, fixed y0, fixed y1)
{
	int code;
	fixed yy0, yy1;
...
int
process_h_lists(line_list *ll, active_line *plp, active_line *flp, active_line *alp,
		fixed y0, fixed y1)
{
	if(y0 == y1) {
...
compute_padding(section *s)
{
	return (s->y0 < 0 || s->y1 < 0 ? -2 : /* contacts a trapezoid - don't paint */
		    s->y1 < fixed_half ? 0 : s->y0 > fixed_half ? 1 : fixed_half - s->y0 < s->y1 - fixed_half ? 1 : 0);
}
