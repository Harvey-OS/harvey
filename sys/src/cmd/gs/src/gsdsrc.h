...
#define public_st_data_source() /* in gsdsrc.c */                              \
	gs_public_st_composite(st_data_source, gs_data_source_t,               \
	                       "gs_data_source_t", data_source_enum_ptrs,      \
	                       data_source_reloc_ptrs)
...
#define data_source_init_string(psrc, strg)                                    \
	((psrc)->type = data_source_type_string, (psrc)->data.str = strg,      \
...
#define data_source_init_bytes(psrc, bytes, len)                               \
	((psrc)->type = data_source_type_bytes, (psrc)->data.str.data = bytes, \
	 (psrc)->data.str.size = len,                                          \
...
#define data_source_init_stream(psrc, s)                                       \
	((psrc)->type = data_source_type_stream, (psrc)->data.strm = s,        \
	 (psrc)->access = data_source_access_stream)
