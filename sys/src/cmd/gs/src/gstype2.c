...
#define clear CLEAR_CSTACK(cstack, csp)
	...
	case 0:
		...
		code = t1_hinter__set_mapping(
		    h, &pcis->pis->ctm, &pfont->FontMatrix,
		    &pfont->base->FontMatrix, pcis->scale.x.log2_unit,
		    pcis->scale.x.log2_unit,
		...
		if(code < 0)
			return code;
		code = t1_hinter__set_font_data(h, 2, pdata,
		                                pcis->no_grid_fitting);
		if(code < 0)
			return code;
		break;
	default /*case 1 */:
	...
	for(;;) {
		...
		if(c >= c_num1) {
			...
			if(c < c_pos2_0) { /* 1-byte number */
				...
				decode_push_num2(csp, cstack, c, cip, state,
				                 encrypted);
...
#define cnext                                                                  \
	...
	goto top
		...
		case c_callsubr:
			...
			code = pdata->procs.subr_data(pfont, c, false,
			                              &ipsp[1].cs_data);
		subr:
			if(code < 0) {
				/* Calling a Subr with an out-of-range index is
				 * clearly a error:
				 * the Adobe documentation says the results of
				 * doing this are
				 * undefined. However, we have seen a PDF file
				 * produced by Adobe
				 * PDF Library 4.16 that included a Type 2 font
				 * that called an
				 * out-of-range Subr, and Acrobat Reader did not
				 * signal an error.
			...
			goto call;
		case c_return:
			gs_glyph_data_free(&ipsp->cs_data,
			                   "gs_type2_interpret");
			--ipsp;
			goto cont;
		case c_undoc15:
		...
		case cx_vlineto:
			vertical = true;
		hvl:
			for(ap = cstack; ap <= csp;
			    vertical = !vertical, ++ap) {
				if(vertical) {
		...
		case cx_rrcurveto:
			for(ap = cstack; ap + 5 <= csp; ap += 6) {
				code =
				    t1_hinter__rcurveto(h, ap[0], ap[1], ap[2],
				                        ap[3], ap[4], ap[5]);
				if(code < 0)
					return code;
			}
		...
		case cx_endchar:
			/*
			 * It is a feature of Type 2 CharStrings that if endchar
			 * is
			 * invoked with 4 or 5 operands, it is equivalent to the
			 * Type 1 seac operator. In this case, the asb operand
			 * of
			 * seac is missing: we assume it is the same as the
			 * l.s.b. of the accented character.  This feature was
			 * undocumented until the 16 March 2000 version of the
			 * Type
			 * 2 Charstring Format specification, but, thankfully,
			 * is
			...
			if(pcis->seac_accent < 0) {
				...
				if(code < 0)
					return code;
				code = gx_setcurrentpoint_from_path(pcis->pis,
				                                    pcis->path);
				if(code < 0)
					return code;
			} else
				t1_hinter__setcurrentpoint(h, pcis->save_adxy.x,
				                           pcis->save_adxy.y);
			code = gs_type1_endchar(pcis);
			if(code == 1) {
				/*
				 * Reset the total hint count so that hintmask
				 * will
				 * parse its following data correctly.
				 * (gs_type1_endchar already reset the actual
				 * hint
				 * tables.)
				 */
				pcis->num_hints = 0;
		...
		case cx_rmoveto:
			...
			if(csp > cstack + 1) {
				/* Some Type 2 charstrings omit the vstemhm
				   operator before rmoveto,
				   even though this is only allowed before
		...
		case cx_hvcurveto:
			vertical = false;
		hvc:
			for(ap = cstack; ap + 3 <= csp;
			    vertical = !vertical, ap += 4) {
				...
				code = t1_hinter__rcurveto(h, pt[0].x, pt[0].y,
				                           ap[1], ap[2],
				                           pt[1].x, pt[1].y);
				if(code < 0)
					return code;
		...
		hstem:
			check_first_operator(!((csp - cstack) & 1));
			{
				...
				for(ap = cstack; ap + 1 <= csp;
				    x += ap[1], ap += 2) {
					code = t1_hinter__hstem(h, x += ap[0],
					                        ap[1]);
					if(code < 0)
						return code;
				}
		...
		case c2_callgsubr:
			...
			code = pdata->procs.subr_data(pfont, c, true,
			                              &ipsp[1].cs_data);
			goto subr;
		case cx_escape:
			...
			case ce2_store: {
				...
				float* to =
				    Registry[fixed2int_var(csp[-3])].values +
				    fixed2int_var(csp[-2]);
				const fixed* from = pcis->transient_array +
				                    fixed2int_var(csp[-1]);
			...
			case ce2_div:
				...
				break;
			case ce2_load:
				/* The specification says there is no j
				 * (starting index */
				/* in registry array) argument.... */
				{
					int i, n = fixed2int_var(*csp);
					const float* from =
					    Registry[fixed2int_var(csp[-2])]
					        .values;
					fixed* to = pcis->transient_array +
					            fixed2int_var(csp[-1]);
			...
			case ce2_drop:
				...
				break;
			case ce2_put:
				pcis->transient_array[fixed2int_var(*csp)] =
				    csp[-1];
				csp -= 2;
				break;
			case ce2_get:
			...
			flex : {
				...
				fixed x_end =
				    x_join + csp[-6] + csp[-4] + csp[-2];
				fixed y_end =
				    y_join + csp[-5] + csp[-3] + csp[-1];
				gs_point join, end;
				double flex_depth;

				if((code = gs_distance_transform(
				        fixed2float(x_join),
				        fixed2float(y_join),
				        &ctm_only(pcis->pis), &join)) < 0 ||
				   (code = gs_distance_transform(
				        fixed2float(x_end), fixed2float(y_end),
				        &ctm_only(pcis->pis), &end)) < 0)
					return code;
				/*
				...
				if(fabs(flex_depth) < fixed2float(*csp)) {
					/* Do flex as line. */
					code =
					    t1_hinter__rlineto(h, x_end, y_end);
				} else {
					/*
					 * Do flex as curve.  We can't jump to
					 * rrc,
					 * because the flex operators don't
					 * clear
					 * the stack (!).
					 */
					code = t1_hinter__rcurveto(
					    h, csp[-12], csp[-11], csp[-10],
					    csp[-9], csp[-8], csp[-7]);
					if(code < 0)
						return code;
					code = t1_hinter__rcurveto(
			...
			case ce2_hflex1:
				...
				*csp = csp[-2], csp[1] = csp[-1]; /* dx5, dy5 */
				...
				goto flex;
			case ce2_flex1: {
				fixed dx = csp[-10] + csp[-8] + csp[-6] +
				           csp[-4] + csp[-2];
				fixed dy = csp[-9] + csp[-7] + csp[-5] +
				           csp[-3] + csp[-1];
