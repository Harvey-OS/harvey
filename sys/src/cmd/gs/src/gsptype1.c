...
/* GC procedures */
private
ENUM_PTRS_BEGIN(pattern1_instance_enum_ptrs)
{
	if(index < st_pattern1_template_max_ptrs) {
		gs_ptr_type_t ptype = ENUM_SUPER_ELT(
		    gs_pattern1_instance_t, st_pattern1_template, template, 0);
...
ENUM_PTRS_END
private
RELOC_PTRS_BEGIN(pattern1_instance_reloc_ptrs)
{
...
private
pattern_proc_uses_base_space(gs_pattern1_uses_base_space);
private
pattern_proc_make_pattern(gs_pattern1_make_pattern);
private
pattern_proc_get_pattern(gs_pattern1_get_pattern);
private
pattern_proc_set_color(gs_pattern1_set_color);
private
const gs_pattern_type_t gs_pattern1_type = {
    1,
    {gs_pattern1_uses_base_space, gs_pattern1_make_pattern,
     gs_pattern1_get_pattern, gs_pattern1_remap_color, gs_pattern1_set_color}};
                         ...
                         const gs_color_space* pbase_cspace, gs_memory_t* pmem)
{
	...
	if(pbase_cspace != 0) {
		if(gs_color_space_num_components(pcspace) <
		   0) /* Pattern space */
	...
	if(pbase_cspace != 0) {
		...
		gs_cspace_init_from(
		    (gs_color_space*)&(pcspace->params.pattern.base_space),
		    pbase_cspace);
	} else
...
private
int compute_inst_matrix(gs_pattern1_instance_t* pinst, const gs_state* saved,
...
gs_pattern1_make_pattern(gs_client_color* pcc,
                         ...
                         const gs_matrix* pmat, gs_state* pgs, gs_memory_t* mem)
{
	const gs_pattern1_template_t* pcp =
	    (const gs_pattern1_template_t*)ptemp;
	...
	int code =
	    gs_make_pattern_common(pcc, (const gs_pattern_template_t*)pcp, pmat,
	                           pgs, mem, &st_pattern1_instance);
...
#define mat inst.step_matrix
	          ...
	          bbox.q.x, bbox.q.y);
	{
		...
		if(inst.size.x == 0 || inst.size.y == 0) {
			/*
			 * The pattern is empty: the stepping matrix doesn't
			 * matter.
			 */
			...
			if(ADJUST_SCALE_BY_GS_TRADITION && mat.xy == 0 &&
			   mat.yx == 0 && fabs(fabs(mat.xx) - bbw) < 0.5 &&
			   fabs(fabs(mat.yy) - bbh) < 0.5) {
				gs_scale(saved, fabs(inst.size.x / mat.xx),
				         fabs(inst.size.y / mat.yy));
				code = compute_inst_matrix(
				    &inst, saved, &bbox, dev_width, dev_height);
				if(code < 0)
					goto fsaved;
				if(ADJUST_SCALE_FOR_THIN_LINES) {
					/* To allow thin lines at a cell
					   boundary
					   to be painted inside the cell,
					   we adjust the scale so that
					   the scaled width is in fixed_1
					   smaller */
					gs_scale(saved, (fabs(inst.size.x) -
					                 1.0 / fixed_scale) /
					                    fabs(inst.size.x),
					         (fabs(inst.size.y) -
					          1.0 / fixed_scale) /
				...
				if_debug2('t', "[t]adjusted XStep & YStep to "
				               "size=(%d,%d)\n",
				...
				if(mat.xy == 0 && mat.yx == 0 &&
				   ...
				   fabs(fabs(mat.yy) - bbh) < 0.5) {
					if(inst.step_matrix.xx <= 2) {
						/* Prevent a degradation - see
						 * -r72 mspro.pdf */
						gs_scale(
						    saved,
						    fabs(inst.size.x / mat.xx),
						    1);
						inst.step_matrix.xx =
						    (float)inst.size.x;
					} else {
						...
						/* To allow thin lines at a cell
						   boundary
						   to be painted inside the
						   cell,
						   we adjust the scale so that
						   the scaled width is in
						   fixed_1 smaller */
						if(bbw >= inst.size.x -
						              1.0 / fixed_scale)
							gs_scale(
	...
	if_debug4('t', "[t]ibbox=(%g,%g),(%g,%g)\n", inst.bbox.p.x,
	          inst.bbox.p.y, inst.bbox.q.x, inst.bbox.q.y);
	...
	if_debug6('t',
	          ...
	          inst.step_matrix.yy, inst.size.x, inst.size.y);
...
private
int
clamp_pattern_bbox(gs_pattern1_instance_t* pinst, gs_rect* pbbox, int width,
                   int height, const gs_matrix* pmat)
{
	...
	for(;; ixpat++) {
		...
		for(iypat = iystart;; iypat++) {
			...
			/*
			 * Check if the pattern bounding box intersects the
			 * page.
			 */
			xlower = (xdev + pbbox->p.x > 0) ? pbbox->p.x : -xdev;
			xupper = (xdev + pbbox->q.x < width) ? pbbox->q.x
			                                     : -xdev + width;
			ylower = (ydev + pbbox->p.y > 0) ? pbbox->p.y : -ydev;
			yupper = (ydev + pbbox->q.y < height) ? pbbox->q.y
			                                      : -ydev + height;
			if(xlower < xupper && ylower < yupper) {
				/*
				 * The pattern intersects the page.  Expand
				 * required area if
				 * needed.
				 */
				if(xlower < xmin)
                    ...
                    gs_rect* pbbox, int width, int height)
{
	...
	code =
	    gs_bbox_transform(&pinst->template.BBox, &ctm_only(saved), pbbox);
	/*
	 ...
	 */
	if(code >= 0 && (pbbox->q.x - pbbox->p.x > width ||
	                 pbbox->q.y - pbbox->p.y > height))
		code = clamp_pattern_bbox(pinst, pbbox, width, height,
		                          &ctm_only(saved));
...
gs_getpattern(const gs_client_color* pcc)
{
	...
	return (pinst == 0 || pinst->type != &gs_pattern1_type
	            ? 0
	            : &((const gs_pattern1_instance_t*)pinst)->template);
}
...
gs_private_st_suffix_add1(st_pixmap_info, pixmap_info, "pixmap info. struct",
                          pixmap_enum_ptr, pixmap_reloc_ptr, st_gs_depth_bitmap,
                          pcspace);
...
private
void
free_pixmap_pattern(gs_memory_t* pmem, void* pvpinst, client_name_t cname)
{
 ...
 */
private
int bitmap_paint(gs_image_enum* pen, gs_data_image_t* pim,
...
image_PaintProc(const gs_client_color* pcolor, gs_state* pgs)
{
	...
	/*
	 * If the image is transparent then we want to do image type4
	 * processing.
	 * Otherwise we want to use image type 1 processing.
	 */
	int transparent = ppmap->white_index <
	                  (1 << (pbitmap->num_comps * pbitmap->pix_depth));
	...
	if((code = gs_image_begin_typed((const gs_image_common_t*)&image, pgs,
	                                false, &pie)) >= 0 &&
	   (code = gs_image_enum_init(pen, pie, (gs_data_image_t*)&image,
	                              pgs)) >= 0)
		code =
		    bitmap_paint(pen, (gs_data_image_t*)&image, pbitmap, pgs);
	gs_grestore(pgs);
	return code;
}
 ...
 */
int
gs_makepixmappattern(gs_client_color* pcc, const gs_depth_bitmap* pbitmap,
                     bool mask, const gs_matrix* pmat, long id,
                     const gs_color_space* pcspace, uint white_index,
                     gs_state* pgs, gs_memory_t* mem)
{
	...
	if((mask) || (pcspace == 0)) {
		if(pbitmap->pix_depth != 1)
			return_error(gs_error_rangecheck);
		pcspace = 0;
	} else if(gs_color_space_get_index(pcspace) !=
	...
	if(mem == 0)
		mem = gs_state_memory(pgs);
	ppmap = gs_alloc_struct(mem, pixmap_info, &st_pixmap_info,
	                        "makepximappattern");
	if(ppmap == 0)
		return_error(gs_error_VMerror);
	...
	uid_set_UniqueID(&pat.uid,
	                 (id == no_UniqueID) ? gs_next_ids(mem, 1) : id);
 ...
 */
int
gs_makebitmappattern_xform(gs_client_color* pcc, const gx_tile_bitmap* ptile,
                           bool mask, const gs_matrix* pmat, int32_t id,
                           gs_state* pgs, gs_memory_t* mem)
{
	...
	return gs_makepixmappattern(pcc, &bitmap, mask, pmat, id, 0, 0, pgs,
	                            mem);
}
 ...
 */
private
dev_color_proc_get_dev_halftone(gx_dc_pattern_get_dev_halftone);
private
dev_color_proc_load(gx_dc_pattern_load);
/*dev_color_proc_fill_rectangle(gx_dc_pattern_fill_rectangle); */ /*gxp1fill.h
                                                                     */
private
dev_color_proc_equal(gx_dc_pattern_equal);
private
dev_color_proc_load(gx_dc_pure_masked_load);

private
dev_color_proc_get_dev_halftone(gx_dc_pure_masked_get_dev_halftone);
/*dev_color_proc_fill_rectangle(gx_dc_pure_masked_fill_rect); */ /*gxp1fill.h */
private
dev_color_proc_equal(gx_dc_pure_masked_equal);
private
dev_color_proc_load(gx_dc_binary_masked_load);

private
dev_color_proc_get_dev_halftone(gx_dc_binary_masked_get_dev_halftone);
/*dev_color_proc_fill_rectangle(gx_dc_binary_masked_fill_rect); */ /*gxp1fill.h
                                                                      */
private
dev_color_proc_equal(gx_dc_binary_masked_equal);
private
dev_color_proc_load(gx_dc_colored_masked_load);

private
dev_color_proc_get_dev_halftone(gx_dc_colored_masked_get_dev_halftone);
/*dev_color_proc_fill_rectangle(gx_dc_colored_masked_fill_rect); */ /*gxp1fill.h
                                                                       */
private
dev_color_proc_equal(gx_dc_colored_masked_equal);
...
const gx_device_color_type_t gx_dc_pattern = {
    &st_dc_pattern, gx_dc_pattern_save_dc, gx_dc_pattern_get_dev_halftone,
    gx_dc_ht_get_phase, gx_dc_pattern_load, gx_dc_pattern_fill_rectangle,
    gx_dc_default_fill_masked, gx_dc_pattern_equal, gx_dc_pattern_write,
    gx_dc_pattern_read, gx_dc_pattern_get_nonzero_comps};
...
const gx_device_color_type_t gx_dc_pure_masked = {
    &st_dc_pure_masked, gx_dc_pattern_save_dc,
    gx_dc_pure_masked_get_dev_halftone, gx_dc_no_get_phase,
    gx_dc_pure_masked_load, gx_dc_pure_masked_fill_rect,
    gx_dc_default_fill_masked, gx_dc_pure_masked_equal, gx_dc_pattern_write,
    gx_dc_pattern_read, gx_dc_pure_get_nonzero_comps};
...
const gx_device_color_type_t gx_dc_binary_masked = {
    &st_dc_binary_masked, gx_dc_pattern_save_dc,
    gx_dc_binary_masked_get_dev_halftone, gx_dc_ht_get_phase,
    gx_dc_binary_masked_load, gx_dc_binary_masked_fill_rect,
    gx_dc_default_fill_masked, gx_dc_binary_masked_equal, gx_dc_pattern_write,
    gx_dc_pattern_read, gx_dc_ht_binary_get_nonzero_comps};

gs_private_st_composite_only(st_dc_colored_masked, gx_device_color,
                             "dc_colored_masked", dc_masked_enum_ptrs,
                             dc_masked_reloc_ptrs);
const gx_device_color_type_t gx_dc_colored_masked = {
    &st_dc_colored_masked, gx_dc_pattern_save_dc,
    gx_dc_colored_masked_get_dev_halftone, gx_dc_ht_get_phase,
    gx_dc_colored_masked_load, gx_dc_colored_masked_fill_rect,
    gx_dc_default_fill_masked, gx_dc_colored_masked_equal, gx_dc_pattern_write,
    gx_dc_pattern_read, gx_dc_ht_colored_get_nonzero_comps};
...
ENUM_PTRS_END
private
RELOC_PTRS_WITH(dc_pattern_reloc_ptrs, gx_device_color* cptr)
{
	gx_color_tile* tile = cptr->colors.pattern.p_tile;
	...
	if(tile != 0) {
		...
		RELOC_TYPED_OFFSET_PTR(gx_device_color, colors.pattern.p_tile,
		                       index);
	}
...
RELOC_PTRS_END
private
ENUM_PTRS_WITH(dc_masked_enum_ptrs, gx_device_color* cptr)
...
ENUM_PTRS_END
private
RELOC_PTRS_WITH(dc_masked_reloc_ptrs, gx_device_color* cptr)
{
	gx_color_tile* mask = cptr->mask.m_tile;
...
RELOC_PTRS_END
private
ENUM_PTRS_BEGIN(dc_binary_masked_enum_ptrs)
{
...
ENUM_PTRS_END
private
RELOC_PTRS_BEGIN(dc_binary_masked_reloc_ptrs)
{
	RELOC_USING(st_dc_pure_masked, vptr, size);
 ...
 */
void
gx_dc_pattern_save_dc(const gx_device_color* pdevc, gx_device_color_saved* psdc)
{
                        ...
                        gx_device* dev, gs_color_select_t select)
{
	...
	if(pcache != 0) {
		...
		if(pis->have_pattern_streams) {
			int code = dev_proc(dev, pattern_manage)(
			    dev, id, NULL, pattern_manage__load);
			internal_accum = (code == 0);
			if(code < 0)
				return false;
		}
		if(ctile->id == id && ctile->is_dummy == !internal_accum &&
 ...
 */
int
gx_dc_pattern_get_nonzero_comps(const gx_device_color* pdevc_ignored,
...
/* currently, patterns cannot be passed through the command list */
int
gx_dc_pattern_write(const gx_device_color* pdevc,
                    const gx_device_color_saved* psdc, const gx_device* dev,
...
int
gx_dc_pattern_read(gx_device_color* pdevc, const gs_imager_state* pis,
