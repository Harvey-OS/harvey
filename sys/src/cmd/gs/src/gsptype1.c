...
const gs_pattern_type_t gs_pattern1_type = {
    1, {gs_pattern1_uses_base_space, gs_pattern1_make_pattern, gs_pattern1_get_pattern, gs_pattern1_remap_color, gs_pattern1_set_color}};
...
private
int compute_inst_matrix(gs_pattern1_instance_t *pinst,
...
{
	return (const gs_pattern_template_t *)&((const gs_pattern1_instance_t *)pinst)->template;
}
...
gs_getpattern(const gs_client_color *pcc)
{
	...
	return (pinst == 0 || pinst->type != &gs_pattern1_type ? 0 : &((const gs_pattern1_instance_t *)pinst)->template);
}
	...
	if(pcache != 0) {
		...
		if(ctile->id == id &&
		   ...
		   (pdevc->type != &gx_dc_pattern ||
			...
			pdevc->mask.m_tile =
			    (ctile->tmask.data == 0 ? (gx_color_tile *)0 : ctile);
