...
#define END_PARAM(pa, e)                                                       \
	...
	default:                                                               \
		code = ncode;                                                  \
	e:                                                                     \
		param_signal_error(plist, oname, code);                        \
	case 1:                                                                \
		pa.data = 0; /* mark as not filled */                          \
		}

private
int cdj_param_check_bytes(gs_param_list*, gs_param_name, const byte*, uint,
                          bool);
private
int cdj_param_check_float(gs_param_list*, gs_param_name, floatp, bool);
...
#define DESKJET_MARGINS_LETTER                                                 \
	(float)0.25, (float)0.50, (float)0.25, (float)0.167
 ...
 *
 */
#define ESC_P_MARGINS_LETTER                                                   \
	(float)0.134, (float)(0.276 + 0.2), (float)(0.366 + 0.01), (float)0.335
#define ESC_P_MARGINS_A4                                                       \
	(float)0.134, (float)(0.276 + 0.2), (float)(0.166 + 0.01), (float)0.335
...
private
dev_proc_map_cmyk_color(gdev_cmyk_map_cmyk_color);
private
dev_proc_map_rgb_color(gdev_cmyk_map_rgb_color);

private
dev_proc_map_rgb_color(gdev_pcl_map_rgb_color);
private
dev_proc_map_color_rgb(gdev_pcl_map_color_rgb);
private
dev_proc_decode_color(gdev_cmyk_map_color_cmyk);

/* Print-page, parameters and miscellaneous procedures */
private
dev_proc_open_device(dj500c_open);
private
dev_proc_open_device(dj550c_open);
private
dev_proc_open_device(dnj650c_open);
private
dev_proc_open_device(lj4dith_open);
private
dev_proc_open_device(pj_open);
private
dev_proc_open_device(pjxl_open);
private
dev_proc_open_device(pjxl300_open);
private
dev_proc_open_device(escp_open);
private
dev_proc_open_device(bjc_open);

private
dev_proc_print_page(declj250_print_page);
private
dev_proc_print_page(dj500c_print_page);
private
dev_proc_print_page(dj550c_print_page);
private
dev_proc_print_page(dnj650c_print_page);
private
dev_proc_print_page(lj4dith_print_page);
private
dev_proc_print_page(pj_print_page);
private
dev_proc_print_page(pjxl_print_page);
private
dev_proc_print_page(pjxl300_print_page);
private
dev_proc_print_page(escp_print_page);
private
dev_proc_print_page(bjc_print_page);

private
dev_proc_get_params(cdj_get_params);
private
dev_proc_get_params(pjxl_get_params);
private
dev_proc_get_params(bjc_get_params);
#define ep_get_params cdj_get_params

private
dev_proc_put_params(cdj_put_params);
private
dev_proc_put_params(pj_put_params);
private
dev_proc_put_params(pjxl_put_params);
private
dev_proc_put_params(bjc_put_params);
...
#define bjcversion(p)                                                          \
	(((gx_device_bjc*)pdev)->ptype == BJC800 ? BJC_BJC800_VERSION          \
	                                         : BJC_BJC600_VERSION)
#define bjcversionstring(p)                                                    \
	(((gx_device_bjc*)pdev)->ptype == BJC800 ? BJC_BJC800_VERSIONSTR       \
	                                         : BJC_BJC600_VERSIONSTR)
...
#define prn_colour_device_body(dtype, procs, dname, w10, h10, xdpi, ydpi, lm,  \
                               bm, rm, tm, ncomp, depth, mg, mc, dg, dc,       \
                               print_page, cmyk, correct)                      \
	prn_device_body(dtype, procs, dname, w10, h10, xdpi, ydpi, lm, bm, rm, \
	                tm, ncomp, depth, mg, mc, dg, dc, print_page),         \
	    cmyk, depth /* default */, correct

/* Note: the computation of color_info values here must match */
/* the computation in the cdj_set_bpp procedure below. */

#define prn_hp_colour_device(dtype, procs, dev_name, x_dpi, y_dpi, bpp,        \
                             print_page, correct)                              \
	prn_colour_device_body(                                                \
	    dtype, procs, dev_name, DEFAULT_WIDTH_10THS, DEFAULT_HEIGHT_10THS, \
	    x_dpi, y_dpi, 0, 0, 0, 0,                                          \
...
#define prn_cmyk_colour_device(dtype, procs, dev_name, x_dpi, y_dpi, bpp,      \
                               print_page, correct)                            \
	prn_colour_device_body(                                                \
	    dtype, procs, dev_name, DEFAULT_WIDTH_10THS, DEFAULT_HEIGHT_10THS, \
	    x_dpi, y_dpi, 0, 0, 0, 0, ((bpp == 1 || bpp == 4) ? 1 : 4), bpp,   \
	    (bpp > 8 ? 255 : 1),                                               \
	    (1 << (bpp >> 2)) - 1, /* max_gray, max_color */                   \
	    (bpp > 8 ? 256 : 2), (bpp > 8 ? 256 : bpp > 1 ? 2 : 0),            \
...
#define cdj_device(procs, dev_name, x_dpi, y_dpi, bpp, print_page, correction, \
                   shingling, depletion)                                       \
	{                                                                      \
		prn_hp_colour_device(gx_device_cdj, procs, dev_name, x_dpi,    \
		                     y_dpi, bpp, print_page, correction),      \
		    shingling, depletion                                       \
	}

#define pjxl_device(procs, dev_name, x_dpi, y_dpi, bpp, print_page, printqual, \
                    rendertype)                                                \
	{                                                                      \
		prn_hp_colour_device(gx_device_pjxl, procs, dev_name, x_dpi,   \
		                     y_dpi, bpp, print_page, 0),               \
		    printqual, rendertype                                      \
	}

#define pj_device(procs, dev_name, x_dpi, y_dpi, bpp, print_page)              \
	{                                                                      \
		prn_hp_colour_device(gx_device_pj, procs, dev_name, x_dpi,     \
		                     y_dpi, bpp, print_page, 0)                \
	}

#define bjc600_device(procs, dev_name, x_dpi, y_dpi, bpp, print_page, t, mf,   \
                      mt, mws, mw, pq, dt, cc, pc, mp)                         \
	{                                                                      \
		bjc_device(gx_device_bjc600, procs, dev_name, x_dpi, y_dpi,    \
		           bpp, print_page, 0),                                \
		    t, 0.,                                                     \
		{                                                              \
			mf, mt, mws, mw, pq, dt, cc, pc, mp                    \
		}                                                              \
	}
#define bjc800_device(procs, dev_name, x_dpi, y_dpi, bpp, print_page, t, mf,   \
                      mt, mws, mw, pq, dt, cc, pc)                             \
	{                                                                      \
		bjc_device(gx_device_bjc800, procs, dev_name, x_dpi, y_dpi,    \
		           bpp, print_page, 0),                                \
		    t, 0.,                                                     \
		{                                                              \
			mf, mt, mws, mw, pq, dt, cc, pc                        \
		}                                                              \
	}

#define hp_colour_procs(proc_colour_open, proc_get_params, proc_put_params)    \
	{                                                                      \
		proc_colour_open, gx_default_get_initial_matrix,               \
		    gx_default_sync_output, gdev_prn_output_page,              \
		    gdev_prn_close, gdev_pcl_map_rgb_color,                    \
		    gdev_pcl_map_color_rgb, NULL, /* fill_rectangle */         \
		    NULL,                         /* tile_rectangle */         \
		    NULL,                         /* copy_mono */              \
		    NULL,                         /* copy_color */             \
		    NULL,                         /* draw_line */              \
		    gx_default_get_bits, proc_get_params, proc_put_params      \
	}
...
#define cmyk_colour_procs(proc_colour_open, proc_get_params, proc_put_params)  \
	{                                                                      \
		proc_colour_open, gx_default_get_initial_matrix,               \
		    gx_default_sync_output, gdev_prn_output_page,              \
		    gdev_prn_close, NULL /* map_rgb_color */,                  \
		    NULL /* map_color_rgb */, NULL /* fill_rectangle */,       \
		    NULL /* tile_rectangle */, NULL /* copy_mono */,           \
		    NULL /* copy_color */, NULL /* draw_line */,               \
		    gx_default_get_bits, proc_get_params, proc_put_params,     \
		    gdev_cmyk_map_cmyk_color, NULL, /* get_xfont_procs */      \
...
gx_device_cdj far_data gs_cdjmono_device = cdj_device(
    cdj500_procs, "cdjmono", 300, 300, 1, dj500c_print_page, 4, 0, 1);

gx_device_cdj far_data gs_cdeskjet_device = cdj_device(
    cdj500_procs, "cdeskjet", 300, 300, 3, dj500c_print_page, 4, 2, 1);

gx_device_cdj far_data gs_cdjcolor_device = cdj_device(
    cdj500_procs, "cdjcolor", 300, 300, 24, dj500c_print_page, 4, 2, 1);

gx_device_cdj far_data gs_cdj500_device = cdj_device(
    cdj500_procs, "cdj500", 300, 300, BITSPERPIXEL, dj500c_print_page, 4, 2, 1);

gx_device_cdj far_data gs_cdj550_device = cdj_device(
    cdj550_procs, "cdj550", 300, 300, BITSPERPIXEL, dj550c_print_page, 0, 2, 1);

#ifdef USE_CDJ550_CMYK
gx_device_cdj far_data gs_cdj550cmyk_device = {
    prn_cmyk_colour_device(cdj550cmyk_procs, "cdj550cmyk", 300, 300,
                           BITSPERPIXEL, dj550c_print_page, 0),
    2, 1};
#endif

gx_device_pj far_data gs_declj250_device = pj_device(
    pj_procs, "declj250", 180, 180, BITSPERPIXEL, declj250_print_page);

gx_device_cdj far_data gs_dnj650c_device =
    cdj_device(dnj650c_procs, "dnj650c", 300, 300, BITSPERPIXEL,
               dnj650c_print_page, 0, 2, 1);

gx_device_cdj far_data gs_lj4dith_device = cdj_device(
    lj4dith_procs, "lj4dith", 600, 600, 8, lj4dith_print_page, 4, 0, 1);

gx_device_pj far_data gs_pj_device =
    pj_device(pj_procs, "pj", 180, 180, BITSPERPIXEL, pj_print_page);

gx_device_pjxl far_data gs_pjxl_device = pjxl_device(
    pjxl_procs, "pjxl", 180, 180, BITSPERPIXEL, pjxl_print_page, 0, 0);

gx_device_pjxl far_data gs_pjxl300_device = pjxl_device(
    pjxl300_procs, "pjxl300", 300, 300, BITSPERPIXEL, pjxl300_print_page, 0, 0);

gx_device_cdj far_data gs_escp_device =
    cdj_device(escp_procs, "escp", 360, 360, 8, escp_print_page, 0, 0, 1);

gx_device_cdj far_data gs_escpc_device =
    cdj_device(escp_procs, "escpc", 360, 360, 24, escp_print_page, 0, 0, 1);
...
gx_device_bjc600 far_data gs_bjc600_device =
    bjc600_device(bjc_procs, BJC_BJC600, BJC600_DEFAULT_RESOLUTION,
                  BJC600_DEFAULT_RESOLUTION, BJC600_DEFAULT_BITSPERPIXEL,
                  bjc_print_page, BJC600, BJC600_DEFAULT_MANUALFEED,
                  BJC600_DEFAULT_MEDIATYPE, BJC600_DEFAULT_SETMEDIAWEIGHT,
                  BJC600_DEFAULT_MEDIAWEIGHT, BJC600_DEFAULT_PRINTQUALITY,
                  BJC600_DEFAULT_DITHERINGTYPE, BJC600_DEFAULT_COLORCOMPONENTS,
                  BJC600_DEFAULT_PRINTCOLORS, BJC600_DEFAULT_MONOCHROMEPRINT);

gx_device_bjc800 far_data gs_bjc800_device = bjc800_device(
    bjc_procs, BJC_BJC800, BJC800_DEFAULT_RESOLUTION, BJC800_DEFAULT_RESOLUTION,
    BJC800_DEFAULT_BITSPERPIXEL, bjc_print_page, BJC800,
    BJC800_DEFAULT_MANUALFEED, BJC800_DEFAULT_MEDIATYPE,
    BJC800_DEFAULT_SETMEDIAWEIGHT, BJC800_DEFAULT_MEDIAWEIGHT,
    BJC800_DEFAULT_PRINTQUALITY, BJC800_DEFAULT_DITHERINGTYPE,
    BJC600_DEFAULT_COLORCOMPONENTS, BJC800_DEFAULT_PRINTCOLORS);

/* Forward references */
private
int gdev_pcl_mode1compress(const byte*, const byte*, byte*);
private
int hp_colour_open(gx_device*, int);
private
int hp_colour_print_page(gx_device_printer*, FILE*, int);
private
int cdj_put_param_int(gs_param_list*, gs_param_name, int*, int, int, int);
private
uint gdev_prn_rasterwidth(const gx_device_printer*, int);
private
int cdj_put_param_bpp(gx_device*, gs_param_list*, int, int, int);
private
int cdj_set_bpp(gx_device*, int, int);
private
void cdj_expand_line(word*, int, short, int, int);
private
int bjc_fscmyk(byte**, byte * [4][4], int**, int, int);
...
private
const byte* paramValueToString(const stringParamDescription*, int);
private
int paramStringValue(const stringParamDescription*, const byte*, int, int*);

private
int put_param_string(gs_param_list*, const byte*, gs_param_string*,
                     const stringParamDescription*, int*, int);
private
int get_param_string(gs_param_list*, const byte*, gs_param_string*,
                     const stringParamDescription*, int, bool, int);
...
{ /* Change the margins if necessary. */
	...
	case BJC800:
		...
		/*
		 * The margins must be set so that the resulting page length
		 *will be
		...
		/**/ {
			...
			byte pdimen =
			    (byte)(pdev->height / pdev->y_pixels_per_inch *
			               10. -
			           bjcm[3] * 10. - bjcm[1] * 10. + .5) +
			    1;
			do {
				--pdimen;
				bjcm[1] =
				    pdev->height / pdev->y_pixels_per_inch -
...
cdj_get_params(gx_device* pdev, gs_param_list* plist)
{
	...
	if(code < 0 ||
	   (code = param_write_int(plist, "BlackCorrect",
	                           (int*)&cdj->correction)) < 0 ||
...
cdj_put_params(gx_device* pdev, gs_param_list* plist)
{
	...
	code =
	    cdj_put_param_int(plist, "BlackCorrect", &correction, 0, 9, code);
...
pjxl_get_params(gx_device* pdev, gs_param_list* plist)
{
	int code = gdev_prn_get_params(pdev, plist);
	if(code < 0 ||
	   (code = param_write_int(plist, "PrintQuality", &pjxl->printqual)) <
	       0 ||
...
pjxl_put_params(gx_device* pdev, gs_param_list* plist)
{
	...
	code =
	    cdj_put_param_int(plist, "PrintQuality", &printqual, -1, 1, code);
	...
	real_bpp = bpp;
	if(rendertype >
	   0) { /* If printer is doing the dithering, we must have a
...
private
stringParamDescription bjc_processColorsStrings[] = {
    {"DeviceGray", 1}, {"DeviceRGB", 3}, {"DeviceCMYK", 4}, {0}};
...
private
stringParamDescription bjc600_printQualityStrings[] = {
    {"Normal", 0}, {"High", 1}, {"Draft", 2}, {0}};

private
stringParamDescription bjc800_printQualityStrings[] = {
    {"Normal", 0}, {"High", 1}, {"Low", 3}, {"Draft", 4}, {0},
};

private
stringParamDescription bjc_ditheringTypeStrings[] = {
    {"None", BJC_DITHER_NONE}, {"Floyd-Steinberg", BJC_DITHER_FS}, {0}};
...
bjc_get_params(gx_device* pdev, gs_param_list* plist)
{
	...
	code = get_param_string(plist, (unsigned char*)BJC_OPTION_MEDIATYPE,
	                        &pmedia, bjc_mediaTypeStrings,
	                        bjcparams.mediaType, true, code);

	code = get_param_string(plist, (unsigned char*)BJC_OPTION_PRINTQUALITY,
	                        &pquality, (bjc->ptype == BJC800
	                                        ? bjc800_printQualityStrings
	                                        : bjc600_printQualityStrings),
	                        bjcparams.printQuality, true, code);

	code = get_param_string(plist, (unsigned char*)BJC_OPTION_DITHERINGTYPE,
	                        &dithering, bjc_ditheringTypeStrings,
	                        bjcparams.ditheringType, true, code);
	...
	if((ncode = (bjcparams.mediaWeight_isSet
	                 ...
	                 : param_write_null(plist, BJC_OPTION_MEDIAWEIGHT))) <
	   0) {
		code = ncode;
	}
...
bjc_put_params(gx_device* pdev, gs_param_list* plist)
{
	...
	if((code = cdj_put_param_int(plist, "BitsPerPixel", &bpp, 1, 32,
	                             code)) != 1) {
		bpp = pdev->color_info.depth;
	}

	if((code = put_param_string(plist, (unsigned char*)"ProcessColorModel",
	                            &pprocesscolors, bjc_processColorsStrings,
	                            &ccomps, code)) != 1) {
	...
	code = put_param_string(plist, (unsigned char*)BJC_OPTION_MEDIATYPE,
	                        &pmedia, bjc_mediaTypeStrings,
	                        &params->mediaType, code);

	code = cdj_put_param_int(plist, BJC_OPTION_PRINTCOLORS,
	                         &params->printColors, 0, 15, code);

	code = put_param_string(plist, (unsigned char*)BJC_OPTION_PRINTQUALITY,
	                        &pquality, (bjc->ptype == BJC800
	                                        ? bjc800_printQualityStrings
	                                        : bjc600_printQualityStrings),
	                        &params->printQuality, code);

	switch(ncode = param_read_int(plist, oname = BJC_OPTION_MEDIAWEIGHT,
	                              &params->mediaWeight)) {
	case 0:
	...
	if((ncode = param_read_bool(plist, oname = BJC_DEVINFO_OUTPUTFACEUP,
		...
		param_signal_error(plist, oname,
		                   code = ncode = gs_error_rangecheck);
	}
	...
	if(hwra.data[0] <= 0 || hwra.data[1] <= 0 ||
	   hwra.data[0] != hwra.data[1])
		...
		/**/ {
			...
			for(n = 0; n < 8 * sizeof(n) / BJC_RESOLUTION_BASE;
			    ++n) {
				float res =
				    (float)(BJC_RESOLUTION_BASE * (1 << n));
...
#define FSDline(scan, i, j, plane_size, cErr, mErr, yErr, kErr, cP, mP, yP,    \
                kP, n)                                                         \
	{                                                                      \
		if(scan == 0) {/* going_up */                                  \
			for(i = 0; i < plane_size; i++) {                      \
				...
				int oldErr;                                    \
				bitmask = 0x80;                                \
				for(c = m = y = k = 0; bitmask != 0;           \
				    bitmask >>= 1) {                           \
					if(n >= 4) {                           \
						FSdither(dp, k, ep, kErr,      \
						         bitmask, -n, 0);      \
					}                                      \
					if(n >= 3) {                           \
						FSdither(dp, c, ep, cErr,      \
						         bitmask, -n, n - 3);  \
						FSdither(dp, m, ep, mErr,      \
						         bitmask, -n, n - 2);  \
					}                                      \
					FSdither(dp, y, ep, yErr, bitmask, -n, \
					         n - 1);                       \
					dp += n, ep += n;                      \
				}                                              \
				if(n >= 4)                                     \
 ...
 */
#define fputshort(n, f)                                                        \
	fputc((n) % 256, f);                                                   \
	fputc((n) / 256, f)

private
int
bjc_cmd(byte cmd, int argsize, byte* arg, gx_device_printer* pdev, FILE* f)
{
	fputs("\033(", f);
	putc(cmd, f);
	fputshort(argsize, f);
               ...
               FILE* f)
{
	if(bjcparams.printColors == BJC_COLOR_ALLBLACK) {
		...
		if(bjcparams.printColors & BJC_COLOR_BLACK) {
	      ...
	      private
		byte ymckCodes[] = {
		    BJC_COLOR_YELLOW, BJC_COLOR_MAGENTA, BJC_COLOR_CYAN,
		    BJC_COLOR_BLACK,
		};
...
bjc_init_page(gx_device_printer* pdev, FILE* f)
{
	...
	pagemargins[0] =
	    (byte)((float)pdev->height / pdev->y_pixels_per_inch * 10 + .5);
	pagemargins[1] = (byte)1;
	pagemargins[2] =
	    (byte)((pdev->width / pdev->x_pixels_per_inch * 10) -
...
/* Set printing method. */
	...
	if(0) {
		...
		printmode[1] = (bjcparams.mediaType >= BJC_MEDIA_ENVELOPE
		                    ? 1
		                    : bjc800thickpaper());
		...
		printmeth[1] =
		    (bjcparams.mediaType << 4) + bjcparams.printQuality;
		printmeth[2] =
		    (bjcparams.printQuality == BJC_QUALITY_HIGH ? 0x10 : 0) +
		    (bjcparams.mediaType >= BJC_MEDIA_ENVELOPE
		         ? 1
		         : bjc600thickpaper());
...
bjc_compress(const byte* row, const byte* end_row, byte* compressed)
{
	...
	register byte* cptr =
	    compressed; /* output pointer into compressed bytes */
	...
	while(exam < end_row) {
		...
		for(;;) { /* Encode up to 128 dissimilar bytes */
			uint count =
			    end_dis - compr; /* uint for faster switch */
			switch(
			    count) { /* Use memcpy only if it's worthwhile. */
			case 6:
				cptr[6] = compr[5];
			case 5:
				cptr[5] = compr[4];
...
word* ep_storage;
private
uint ep_storage_size_words;
private
byte* ep_raster_buf[4][BJC_HEAD_ROWS], *ep_print_buf;
private
int ep_num_comps, ep_plane_size, img_rows = BJC_HEAD_ROWS;
...
ep_print_image(FILE* prn_stream, char cmd, byte* data, int size)
{
	...
	case 'B': /* blank line skip */
		if(!ln_idx) {
			vskip1 += size;
		} else if(size >= img_rows - (ln_idx + vskip2) ||
		          ln_idx + vskip2 >= min_rows) {
			/* The 'I' cmd must precede 'B' cmd! */
			vskip2 += size;
			ep_print_image(prn_stream, 'F', 0,
			               0); /* flush and reset status */
		} else {
			vskip2 += size;
		}
		return 0;
	...
	case 'F': /* flush print buffer */
		...
		for(i = 0; i < ep_num_comps; i++) {
			...
			img_rows = real_rows; /* Note that this img_rows is not
			                       * the one that
			...
			for(lnum = 0; lnum < img_rows; lnum += 8, outbuf++) {
				...
				for(outp = outbuf; inp < inbuf + ep_plane_size;
				    inp++, outp += img_rows) {
					memflip8x8(inp, ep_plane_size, outp,
					           row_bytes);
				}
			}
			...
			*(outp = ep_print_buf + ep_plane_size * img_rows) =
			    1; /* sentinel */
			...
			while(p0 < outp) {
				static const word zeros[8] = {0, 0, 0, 0,
				                              0, 0, 0, 0};
				...
				redo:
					for(p3 += row_bytes;
					    memcmp(p3, zeros, row_bytes);
					    p3 += row_bytes)
						;
					if(p3 < outp &&
					   memcmp(p3 + row_bytes, zeros,
					          row_bytes))
						goto redo;
				} else
					p1 = p2 = outp;
				...
				if(p0 <
				   p1) { /* print the image between p0 and p1 */
					print_size =
					    ((p1 < outp) ? p1 : outp) - p0;
					fputs("\033|B", prn_stream);
					putc(img_rows, prn_stream);
					fputshort(print_size, prn_stream);
					fwrite(p0, sizeof(byte), print_size,
					       prn_stream);
				}
				if(p1 <
				   p2) { /* skip running zeros from p1 to p2 */
					hskip =
					    (((p2 < outp) ? p2 : outp) - p1) /
					    row_bytes / 2;
...
hp_colour_print_page(gx_device_printer* pdev, FILE* prn_stream, int ptype)
{
	...
	case ESC_P:
		if(bits_per_pixel ==
		   24) /* prefer 3-component printing for bpp=24. */
			num_comps = 3;
		else if(num_comps != 1)
			num_comps = 4;
		break;
	...
	case PJXL180:
		if(pjxl->rendertype > 0) {
			if(bits_per_pixel < 16)
				...
				cid_string = (bits_per_pixel == 16)
				                 ? CONFIG_16BIT
				                 : CONFIG_24BIT;
				/* Pretend we're a monobit device so we send the
				 * data out unchanged */
	...
	if(cprn_device->cmyk <= 0) {
		if(storage_bpp == 8 && num_comps >= 3)
			bits_per_pixel = expanded_bpp =
			    3; /* Only 3 bits of each byte used */
	}
	...
	if(bits_per_pixel == 1) {  /* Data printed direct from i/p */
		...
		outbuff_size =
		    plane_size * 4; /* but need separate output buffers */
	}
	...
	if(bits_per_pixel > 4) { /* Error buffer for FS dithering */
		...
		if(cprn_device->cmyk > 0) { /* Use CMYK dithering algorithm. */
			...
			errbuff_size = /* 4n extra values for line ends */
			    calc_buffsize(
			        (plane_size * expanded_bpp + num_comps * 4) * I,
			        1);
		}
	}
	...
	storage_size_words = ((plane_size + plane_size) * num_comps +
	                      databuff_size + errbuff_size + outbuff_size) /
	                     W;

	storage = (ulong*)gs_malloc(pdev->memory, storage_size_words, W,
	                            "hp_colour_print_page");
	ep_storage_size_words =
	    (plane_size * (num_comps + 1)) / W * img_rows +
	    16; /* Redundant space for sentinel and aligning. */
	ep_storage = (word*)gs_malloc(pdev->memory, ep_storage_size_words, W,
	                              "ep_print_buffer");
	...
	if(storage == 0 || ep_storage == 0) /* can't allocate working area */
		...
		if(bits_per_pixel == 1) {
			...
			out_row_alt = out_row + plane_size * 2;
			data[1] += databuff_size; /* coincides with plane_data
			                             pointers */
		...
		for(i = 0; i < num_comps; i++) {
			...
			ep_print_buf = (byte*)((word)(ep_p + sizeof(word)) &
			                       ~(sizeof(word) - 1));
		}
...
#define BOFFSET (dev_t_margin(pdev) - bjc->printLimit)
	...
	case LJ4DITH:
		...
		fprintf(prn_stream,
		        "\033*p0x0Y"); /* These Offsets are hacked ! */
	...
	case DJ550C:
		...
		fprintf(prn_stream, "\033*o%dd%dQ", cdj->depletion,
		        cdj->shingling);
		/* Move to top left of printed area */
		fprintf(prn_stream, "\033*p%dY", (int)(300 * DOFFSET));
		/* Set number of planes ((-)1 is mono, (-)3 is (cmy)rgb, -4 is
		 * cmyk),
	...
	case DNJ650C:
		if(pdev->x_pixels_per_inch == 600) {
			...
			fprintf(prn_stream,
			        "\033%%-12345X@PJL SET RESOLUTION = 600\n");
		}
		...
		fprintf(prn_stream, "PS%d,%d",
		        ...
		        (int)((pdev->width / pdev->x_pixels_per_inch) *
		              1016));      /* Set length/width of page */
		fprintf(prn_stream, "PU"); /* Pen up */
		fprintf(prn_stream, "PA%d,%d", 0,
		        0);                      /* Move pen to upper-left */
		fprintf(prn_stream, "\033%%1A"); /* Enter HP-RTL mode */
		fprintf(prn_stream,
		        "\033&a1N"); /* No negative motion - allow plotting
		                               while receiving */
		if(pdev->x_pixels_per_inch == 600)
			fprintf(prn_stream,
			        "\033*t600R"); /* request 600dpi via HP RTL */
		{
			static const char temp[] = {
			    033, '*', 'v', '6', 'W', 000 /* color model */,
			    000 /* pixel encoding mode */,
			    003 /* number of bits per index */,
			    010 /* bits red */, 010 /* bits green */,
			    010 /* bits blue */
			};
	...
	case PJXL300:
		...
		if(pjxl->rendertype > 0) {
			 ...
			 * and raster width, then start raster graphics */
			fprintf(prn_stream, "\033*r%ds-%du0A", raster_width,
			        num_comps);
		}
		/* No combined escapes for raster transfers */
		combined_escapes = 0;
	...
	case DECLJ250:
		...
		break;
	case ESC_P:
		/* Move to top left of printed area (must be modified for large
		 * movement(YK))*/
		if((int)(EOFFSET * 360))
			fprintf(prn_stream, "\033|J%c%c", 0,
			        (int)(360 * EOFFSET));
		combined_escapes = 0;
		break;
	case BJC600:
	case BJC800:
		/* Move to top left of printed area */
		bjc_v_skip((int)(pdev->HWResolution[1] * BOFFSET), pdev,
		           prn_stream);
		combined_escapes = 0;
		compression =
		    2; /* BJC600 uses the same method as mode 2 compression */
		break;
	}
	...
	/* Send each scan line in turn */
	{
		 ...
		 * this case 'blank' means black!  Also disabled for XL300 due
		 * to
		...
		if(bits_per_pixel >
		   4) { /* Randomly seed initial error buffer */
			if(cprn_device->cmyk > 0 && expanded_bpp == 32) {
				bjc_fscmyk(data, plane_data, errors, plane_size,
				           -1);
			} else {
				int* ep = errors[0];
		...
		for(lnum = 0; lnum < lend; lnum++) {
			...
			gdev_prn_copy_scan_lines(pdev, lnum, data[scan],
			                         line_size);
			...
			while(end_data > data_words && end_data[-1] == 0)
				end_data--;
			if(ptype !=
			   DNJ650C) /* DesignJet can't skip blank lines ? ? */
			...
			if(num_blank_lines > 0) {
				if(ptype == ESC_P) {
					ep_print_image(prn_stream, 'B', 0,
					               num_blank_lines);
				} else if(ptype == BJC600 || ptype == BJC800) {
					bjc_v_skip(num_blank_lines, pdev,
					           prn_stream);
				} else if(num_blank_lines < this_pass) {
					/* Moving down from current position
					 * causes head motion on the DeskJets,
					 * so
...
/**************** as a replacement: ****************/
	    ...
	    num_blank_lines = 0;
					...
					if(combined_escapes)
						fprintf(prn_stream, "%dy",
						        num_blank_lines);
					else
						fprintf(prn_stream, "\033*b%dY",
						        num_blank_lines);
				}
				memset(plane_data[1 - scan][0], 0,
				       plane_size * num_comps);
				num_blank_lines = 0;
				this_pass = start_rows;
			}
			{ /* Printing non-blank lines */
				...
				if(expanded_bpp >
				   bits_per_pixel) { /* Expand line if required
				                        */
				...
				/* In colour modes, we have some bit-shuffling
				 * to do before
				 * we can print the data; in FS mode we also
				 * have the
				 * dithering to take care of. */
				switch(expanded_bpp) { /* Can be 1, 3, 8, 24 or
				                          32 */
				case 3:
					/* Transpose the data to get pixel
					 * planes. */
					for(i = 0, odp = plane_data[scan][0];
					    i < databuff_size; i += 8,
					odp++) { /* The following is for 16-bit
					          * machines */
#define spread3(c)                                                             \
	{                                                                      \
		0, c, c * 0x100, c * 0x101, c * 0x10000L, c * 0x10001L,        \
		    c * 0x10100L, c * 0x10101L                                 \
	}
						static ulong spr40[8] =
						    spread3(0x40);
						static ulong spr08[8] =
						    spread3(8);
						static ulong spr02[8] =
						    spread3(2);
						register byte* dp =
						    data[scan] + i;
				...
				case 8:
					...
					default:
						FSDline(scan, i, j, plane_size,
						        cErr, mErr, yErr, kErr,
						        cP, mP, yP, kP, 1);
					}
					break;
				case 24:
					FSDline(scan, i, j, plane_size, cErr,
					        mErr, yErr, kErr, cP, mP, yP,
					        kP, 3);
					break;
				case 32:
					if(cprn_device->cmyk > 0) {
						bjc_fscmyk(data, plane_data,
						           errors, plane_size,
						           scan);
					} else {
						FSDline(scan, i, j, plane_size,
				...
				if(num_comps == 4 && (cprn_device->cmyk <= 0 ||
				                      expanded_bpp != 32)) {
					register word* kp =
					    (word*)plane_data[scan][3];
					register word* cp =
					    (word*)plane_data[scan][2];
					register word* mp =
					    (word*)plane_data[scan][1];
					register word* yp =
					    (word*)plane_data[scan][0];
					if(bits_per_pixel >
					   4) { /* Done as 4 planes */
						for(i = 0; i < plane_size / W;
						    i++) {
							word bits =
							    *cp & *mp & *yp;
							*kp++ |= bits;
					...
					} else { /* This has really been done as
					            3 planes */
						for(i = 0; i < plane_size / W;
						    i++) {
							word bits =
				...
				/* Transfer raster graphics in the order (K), C,
				 * M, Y */

				for(zero_row_count = 0, i = num_comps - 1;
				    i >= 0; i--) {
					int output_plane = 1;
					int out_count = 0;

					switch(ptype) {
					case DJ500C: /* Always compress using
					                mode 9 */
					case DJ550C:
						out_count =
						    gdev_pcl_mode9compress(
						        plane_size,
						        plane_data[scan][i],
						        plane_data[1 - scan][i],
						        out_data);
						 ...
						 * row, but this doesn't work
						 * correctly in an alternating
						 * mode 2 / mode 3 regime, so we
						 * only use it with mode 9
						 * compression */
						if(out_count == 0) {
							output_plane =
							    0; /* No further
					...
					case DNJ650C:
						...
						out_count =
						    gdev_pcl_mode1compress(
						        ...
						        out_data);
						break;
					case PJXL180: /* Need to invert data as
					                 CMY not supported */
						if(num_comps > 1) {
							word* wp = (word*)
							    plane_data[scan][i];
					...
					/* fall through .. */
					case PJXL300: /* Compression modes 2 and
					               * 3 are both
					               * available.  Try both
					               * and see which one
					               * produces the least
					...
					case LJ4DITH: {
						...
						const word* end_row =
						    row + plane_size / W;
						int count2 =
						    gdev_pcl_mode2compress(
						        row, end_row,
						        out_row_alt);
						int count3 =
						    gdev_pcl_mode3compress(
						        plane_size, plane,
						        prev_plane, out_row);
						int penalty =
						    combined_escapes
						        ? strlen("#m")
						        : strlen("\033*b#M");
						int penalty2 = (compression == 2
						                    ? 0
						                    : penalty);
						int penalty3 = (compression == 3
						                    ? 0
						                    : penalty);
					...
					case BJC800: {
						const byte* plane =
						    (byte*)plane_data[scan][i];
						int count2 = bjc_compress(
						    plane, plane + plane_size,
						    out_row_alt);
	...
	gs_free(pdev->memory, (char*)ep_storage, ep_storage_size_words, W,
	        "ep_print_buffer");
	gs_free(pdev->memory, (char*)storage, storage_size_words, W,
	        "hp_colour_print_page");
...
#define gx_color_value_to_bits(cv, b) ((cv) >> (gx_color_value_bits - (b)))
#define gx_bits_to_color_value(cv, b) ((cv) << (gx_color_value_bits - (b)))
...
#define gx_value_cmyk_bits(v, c, m, y, k, b)                                   \
	(k) = gx_bits_to_color_value(((v) >> (3 * (b))) & ((1 << (b)) - 1),    \
	                             (b)),                                     \
	(c) = gx_bits_to_color_value(((v) >> (2 * (b))) & ((1 << (b)) - 1),    \
	                             (b)),                                     \
...
gdev_cmyk_map_cmyk_color(gx_device* pdev, const gx_color_value cv[])
{
	...
	case 1:
		color =
		    (cyan | magenta | yellow | black) > gx_max_color_value / 2
		        ? (gx_color_index)1
		        : (gx_color_index)0;
		break;
...
private
int
gdev_cmyk_map_color_cmyk(gx_device* pdev, gx_color_index color,
...
gdev_pcl_map_rgb_color(gx_device* pdev, const gx_color_value cv[])
{
	...
	if(gx_color_value_to_byte(r & g & b) == 0xff)
		 ...
		 * cartridge (on the DeskJet 500C only). We reduce the cyan
		 * component
		 * by some fraction (eg. 4/5) to correct the slightly greenish
		 * cast
...
#define gx_color_value_to_6bits(cv) ((cv) >> (gx_color_value_bits - 6))
		...
		case 32: {
			return (
			    (c == m && c == y)
			        ? ((uint32_t)gx_color_value_to_byte(c) << 24)
			        : (gx_color_value_to_byte(y) +
			           (gx_color_value_to_byte(m) << 8) +
			           ((uint32_t)gx_color_value_to_byte(c)
			            << 16)));
		}
		}
	}
                       ...
                       gx_color_value prgb[3])
{
	...
	case 24: {
		...
		prgb[1] =
		    gx_color_value_from_byte((gx_color_value)((c >> 8) & 0xff));
		prgb[2] = gx_color_value_from_byte((gx_color_value)(c & 0xff));
	} break;
	case 32:
#define gx_maxcol                                                              \
	gx_color_value_from_byte(gx_color_value_to_byte(gx_max_color_value))
	{
		gx_color_value w =
	...
	if(cmyk > 0) {
		if(bpp == 8) {
			...
			while(in > start) {
				...
				*--out =
				    (bs0 << 2) + bs0 + (bs1 << 6) + (bs1 << 4);
				*--out =
				    (bs2 << 2) + bs2 + (bs3 << 6) + (bs3 << 4);
			}
		}
...
cdj_set_bpp(gx_device* pdev, int bpp, int ccomps)
{
	...
	if(cprn_device->cmyk) {
		if(cprn_device->cmyk > 0) {
			ci->num_components =
			    ccomps ? ccomps : (bpp < 8 ? 1 : 4);
		} else {
			ci->num_components =
			    ccomps ? ccomps : (bpp < 8 ? 1 : 3);
		}
		if(bpp != 1 &&
		   ci->num_components == 1) { /* We do dithered grays. */
			bpp = bpp < 8 ? 8 : bpp;
		}
...
#define save_ccomps save_info.num_components
		...
		pdev->color_info.depth =
		    new_bpp; /* cdj_set_bpp maps 3/6 to 8 */
...
private
int
paramStringValue(const stringParamDescription* params, const byte* name,
                 int namelen, int* value)
{
...
private
int
put_param_string(gs_param_list* plist, const byte* pname,
                 gs_param_string* pstring, const stringParamDescription* params,
                 int* pvalue, int code)
{
	...
	if((ncode = param_read_string(plist, (char*)pname, pstring)) < 0) {
		...
		if(paramStringValue(params, pstring->data, pstring->size,
		                    &value) == 0) {
			param_signal_error(plist, (char*)pname,
			                   code = gs_error_rangecheck);
		} else {
			*pvalue = value;
		}
...
private
int
get_param_string(gs_param_list* plist, const byte* pname,
                 gs_param_string* pstring, const stringParamDescription* params,
                 int pvalue, bool persist, int code)
{
	...
	if(scan < 0) { /* scan <   0 -> initialize private buffer */
		...
		*/
		...
		for(i = 0; i < 4; ++i) {
			...
			float maxv = 1.0;
			/*KGHorig
			if((sd->stc.xfer[i].size < 1) || (sd->stc.xfer[i].data
			== NULL)) {
			   maxv = 1.0;
			} else {
			   maxv = 1.0/255.0;
			...
			*/
			...
			errc[3] = 0;
			FSdiffuse(CMYK_THRESHOLD(i), errv[0], errc[0],
			          errv[-4]);
			FSdiffuse(CMYK_THRESHOLD(i), errv[0], errc[0],
			          errv[-4]);
			EMAX(i) = errv[0];
...
#ifdef CDJ_DEBUG_FS
		for(i = 0; i < 4; ++i)
			errprintf("CMYK_THRESHOLD(%d)=%5d, spotsize(%d)=%5d, "
			          "emin(%d)=%5d, emax(%d)=%5d\n",
			          i, CMYK_THRESHOLD(i), i, SPOTSIZE(i), i,
			          EMIN(i), i, EMAX(i));
#endif
		...
		for(p = 0; p < NPIXEL; ++p) {
			for(i = 0; i < 4; ++i) {
				...
				*/
				if(0)
					v = 0; /* Must provide a default for
					          that. */
				else
					v = (rand() % SPOTSIZE(i)) -
					    CMYK_THRESHOLD(i);
		...
		*/
		...
		for(p = NPIXEL; p; --p) { /* loop over pixels */
			...
			FSdecide(kv, CMYK_THRESHOLD(3), SPOTSIZE(3), pixel[3],
			         bit);
			...
			if(cmy) {
				...
				if(pixel[3] & bit) { /* black known to fire */
					...
					if((cv + CMYK_THRESHOLD(2)) < 0)
						cv = -CMYK_THRESHOLD(2);
					FSdiffuse(cv, errv[2], yerr,
					          errv[2 - dir]);

				} else {

					cv = FSerror(in[IDX_C], errv[0], cerr);
					FSdecide(cv, CMYK_THRESHOLD(0),
					         SPOTSIZE(0), pixel[0], bit);
					FSdiffuse(cv, errv[0], cerr,
					          errv[0 - dir]);

					cv = FSerror(in[IDX_M], errv[1], merr);
					FSdecide(cv, CMYK_THRESHOLD(1),
					         SPOTSIZE(1), pixel[1], bit);
					FSdiffuse(cv, errv[1], merr,
					          errv[1 - dir]);

					cv = FSerror(in[IDX_Y], errv[2], yerr);
					FSdecide(cv, CMYK_THRESHOLD(2),
					         SPOTSIZE(2), pixel[2], bit);
					FSdiffuse(cv, errv[2], yerr,
					          errv[2 - dir]);
				...
				if(errv[0] > EMAX(0))
					errv[0] = EMAX(0);
				else if(errv[0] < EMIN(0))
					errv[0] = EMIN(0);

				if(errv[1] > EMAX(1))
					errv[1] = EMAX(1);
				else if(errv[1] < EMIN(1))
					errv[1] = EMIN(1);

				if(errv[2] > EMAX(2))
					errv[2] = EMAX(2);
				else if(errv[2] < EMIN(2))
					errv[2] = EMIN(2);
			}
			...
			if(bit == 0) {
				/*KGHorig
				if(((*out[0]  = pixel[0]) != 0) && (width[0] <
				w)) width[0] = w;
				if(((*out[1]  = pixel[1]) != 0) && (width[1] <
				w)) width[1] = w;
				if(((*out[2]  = pixel[2]) != 0) && (width[2] <
				w)) width[2] = w;
				if(((*out[3]  = pixel[3]) != 0) && (width[3] <
				w)) width[3] = w;
				*/
				*out[0] = pixel[0];
				*out[1] = pixel[1];
