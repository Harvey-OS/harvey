...
Where 'e' is 'fixed_epsilon', 0.5 is 'fixed_half', and fl == l.fx / l.h, fr == -
r.fx / r.h,
e <= fl < 0, e <= fr < 0.
Let
...
Note that e != fl ==> floor(xl' + e - fl) == floor(xl')  due to e - fl <
LeastSignificantBit(xl') ;
...
GX_FILL_TRAPEZOID(gx_device* dev, const EDGE_TYPE* left, const EDGE_TYPE* right,
                  fixed ybot, fixed ytop, int flags,
                  const gx_device_color* pdevc, FILL_ATTRS fa)
{
		...
		return 0; /* no scan lines to sample */
	{
		...
		int ry;
		const fixed x0l = left->start.x, x1l = left->end.x,
...
#define FILL_TRAP_RECT(x, y, w, h)                                             \
	(!(w) ? 0 : dev_proc(dev, fill_linear_color_scanline)(                 \
	                dev, fa, x, y, w, xg.c, xg.f, xg.num, xg.den))
#else
#define FILL_TRAP_RECT(x, y, w, h)                                             \
	(FILL_DIRECT ? FILL_TRAP_RECT_DIRECT(x, y, w, h)                       \
	             : FILL_TRAP_RECT_INDIRECT(x, y, w, h))
#endif

#define VD_RECT_SWAPPED(rxl, ry, rxr, iy)                                      \
	vd_rect(int2fixed(SWAP_AXES ? ry : rxl),                               \
	        int2fixed(SWAP_AXES ? rxl : ry),                               \
	        int2fixed(SWAP_AXES ? iy : rxr),                               \
	        int2fixed(SWAP_AXES ? rxr : iy), 1, VD_RECT_COLOR);
...
/* we have a rectangle. */
			...
			if(l.di == 0 && l.df == 0) {
				...
				code = FILL_TRAP_RECT(rxl, ry, rxr - rxl,
				                      iy1 - ry);
				goto xit;
			}
#endif
		...
		compute_ldx(&l, ysl);
		compute_ldx(&r, ysr);
		/* We subtracted fixed_epsilon from l.x, r.x to simplify
		   rounding
		   when the rational part is zero. Now add it back to get xl',
		   xr' */
		l.x += fixed_epsilon;
		r.x += fixed_epsilon;
#if LINEAR_COLOR
		...
		if(check_gradient_overflow(left, right, num_components)) {
			/* The caller must care of.
			   Checking it here looses some performance with
			   triangles. */
...
#define rational_floor(tl)                                                     \
	fixed2int_var(fixed_is_int(tl.x) && tl.xf == -tl.h ? tl.x - fixed_1    \
	                                                   : tl.x)
#define STEP_LINE(ix, tl)                                                      \
	...
	ix = rational_floor(tl)
		...
		while(LINEAR_COLOR ? 1 : ++iy != iy1) {
#if LINEAR_COLOR
			if(rxl != rxr) {
				code = set_x_gradient(&xg, &lg, &rg, &l, &r,
				                      rxl, rxr, num_components);
				if(code < 0)
					goto xit;
			...
			if(ixl != rxl || ixr != rxr) {
				CONNECT_RECTANGLES(ixl, ixr, rxl, rxr, iy, ry,
				                   rxr, ixl, FILL_TRAP_RECT);
				CONNECT_RECTANGLES(ixl, ixr, rxl, rxr, iy, ry,
				                   ixr, rxl, FILL_TRAP_RECT);
				VD_RECT_SWAPPED(rxl, ry, rxr, iy);
				code =
				    FILL_TRAP_RECT(rxl, ry, rxr - rxl, iy - ry);
				if(code < 0)
					goto xit;
