...
/* Changes after Apple: replaced non-portable types with ISO/IEC 988:1999
 * exact-size types */
...
#endif
typedef int16_t FUnit; /* Smallest measurable distance in em space (16-bit
                          signed integer) */
typedef int16_t
...
typedef struct {
	...
	uint32
	    magicNumber; /* signature, should always be 0x5F0F3CF5  == MAGIC */
	uint16 flags;
	...
	/* 0: fully mixed directional glyphs, 1: only strongly L->R or T->B
	   glyphs,
	   -1: only strongly R->L or B->T glyphs, 2: like 1 but also contains
	   neutrals,
...
typedef struct {
	...
	int16 extentMax; /* Max of ( SB[i] + bounds[i] ), i loops through all
	                    glyphs */
...
typedef struct {
	...
	uint16 maxStackElements;   /* max number of stack elements for any
	                              individual glyph */
	uint16 maxSizeOfInstructions; /* max size in bytes for any individual
	                                 glyph */
	uint16
	    maxComponentElements; /* number of glyphs referenced at top level */
	uint16 maxComponentDepth; /* levels of recursion, 1 for simple
	                             components */
...
typedef enum componentPacking {
	...
	WE_HAVE_INSTRUCTIONS =
	    256,             /* short count followed by instructions follows */
...
typedef enum { plat_Unicode, plat_Macintosh, plat_ISO, plat_MS } platformEnums;
