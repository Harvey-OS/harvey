...
static void draw(IMAGE* img, HDC hdc, int dx, int dy, int wx, int wy, int sx,
                 int sy);
...
/* GUI thread only */
void image_color(unsigned int format, int index, unsigned char* r,
...
void image_16RGB565_to_24BGR(int width, unsigned char* dest,
                             unsigned char* source);
void image_4CMYK_to_24BGR(int width, unsigned char* dest, unsigned char* source,
                          IMAGE_DEVICEN* devicen, int devicen_gray);
void image_32CMYK_to_24BGR(int width, unsigned char* dest,
                           unsigned char* source, IMAGE_DEVICEN* devicen,
                           int devicen_gray);
           ...
           unsigned int new_format, void* pimage)
{
	...
	case DISPLAY_COLORS_RGB:
		if((img->format & DISPLAY_DEPTH_MASK) != DISPLAY_DEPTH_8)
			return DISPLAY_ERROR;
		if(((img->format & DISPLAY_ALPHA_MASK) ==
		    DISPLAY_UNUSED_LAST) &&
	...
	img->bytewidth =
	    ((img->bmih.biWidth * img->bmih.biBitCount + 31) & ~31) >> 3;
...
int
image_separation(IMAGE* img, int comp_num, const char* name, unsigned short c,
	   ...
	   ((img->format & DISPLAY_COLORS_MASK) == DISPLAY_COLORS_SEPARATION)) {
		...
		for(i = 0; i < IMAGE_DEVICEN_MAX; i++) {
			...
			if(exist && (strcmp(img->devicen[i].name, buf) != 0)) {
				...
				RemoveMenu(sysmenu, M_SEPARATION + i,
				           MF_BYCOMMAND);
		...
		EnableMenuItem(sysmenu, M_DEVICEN_GRAY,
		               MF_BYCOMMAND | ((num_visible <= 1) ? MF_ENABLED
		                                                  : MF_GRAYED));
	} else {
		for(i = 0; i < IMAGE_DEVICEN_MAX; i++) {
			if(img->devicen[i].menu) {
...
sep_menu(IMAGE* img, int component)
{
	...
	CheckMenuItem(
	    GetSystemMenu(img->hwnd, FALSE), M_SEPARATION + component,
	...
	EnableMenuItem(GetSystemMenu(img->hwnd, FALSE), M_DEVICEN_GRAY,
	               MF_BYCOMMAND |
	                   ((num_visible <= 1) ? MF_ENABLED : MF_GRAYED));
...
create_window(IMAGE* img)
{
	...
	if(win_get_reg_value((img->device != NULL ? "Image" : "Tracer"),
	                     winposbuf, &len) == 0) {
		int x, y, cx, cy;
	...
	{ /*
	   ...
	   *   It allows to replace window title so that user can identify
	   *window
	   ...
	   *
	   *   It is useful to compare images generated with different
	   *revisions.
	   */
		...
		if(ini_path_length > 0) {
			...
			if(i < sizeof(ini_path) - 4) {
				...
				GetPrivateProfileString(
				    "Window", "Title",
				    (img->device != NULL ? (LPSTR)szImgName2
				                         : (LPSTR)szTrcName2),
	...
	/* create window */
	img->hwnd = CreateWindow(szImgName2, window_title, WS_OVERLAPPEDWINDOW,
	                         img->x, img->y, img->cx, img->cy, NULL, NULL,
	                         GetModuleHandle(NULL), (void*)img);
	if(img->device == NULL && img->x != CW_USEDEFAULT &&
	   img->y != CW_USEDEFAULT && img->cx != CW_USEDEFAULT &&
	   img->cy != CW_USEDEFAULT)
		MoveWindow(img->hwnd, img->x, img->y, img->cx, img->cy, FALSE);
	ShowWindow(img->hwnd,
	           (img->device != NULL ? SW_SHOWMINNOACTIVE : SW_SHOW));
...
image_update_now(IMAGE* img)
{
	...
	if(!IsIconic(img->hwnd)) { /* redraw window */
		...
		else if((delta >= 2) && (delta < img->update_interval / 4))
		...
		for(j = 0; j < num_comp; j++) {
			if(devicen[j].visible && devicen[j].used) {
				...
				if(show_gray)
					...
					magenta +=
					    value * devicen[j].magenta / 65535;
					yellow +=
					    value * devicen[j].yellow / 65535;
					black +=
					    value * devicen[j].black / 65535;
				}
			}
		}
		if(cyan > 255)
...
image_convert_line(IMAGE* img, unsigned char* dest, unsigned char* source)
{
	...
	BOOL bigendian =
	    (img->format & DISPLAY_ENDIAN_MASK) == DISPLAY_BIGENDIAN;
	int i;
		...
		if((img->format & DISPLAY_DEPTH_MASK) == DISPLAY_DEPTH_16) {
			if(bigendian) {
				if((img->format & DISPLAY_555_MASK) ==
				   DISPLAY_NATIVE_555)
					image_16RGB555_to_24BGR(
					    img->bmih.biWidth, dest, source);
				else
					image_16RGB565_to_24BGR(
					    img->bmih.biWidth, dest, source);
			} else {
				if((img->format & DISPLAY_555_MASK) ==
				   DISPLAY_NATIVE_555) {
					image_16BGR555_to_24BGR(
					    img->bmih.biWidth, dest, source);
				} else
					image_16BGR565_to_24BGR(
					    img->bmih.biWidth, dest, source);
			}
		}
		break;
	...
	case DISPLAY_COLORS_CMYK:
		if((img->format & DISPLAY_DEPTH_MASK) == DISPLAY_DEPTH_8)
			image_32CMYK_to_24BGR(width, dest, source, img->devicen,
			                      img->devicen_gray);
	...
	case DISPLAY_COLORS_SEPARATION:
		if((img->format & DISPLAY_DEPTH_MASK) != DISPLAY_DEPTH_8)
			return;
		image_devicen_to_24BGR(width, dest, source, img->devicen,
		                       img->devicen_gray);
		break;
	}
}
...
copy_dib(IMAGE* img)
{
	...
	pBits = (BYTE*)(pDIB + sizeof(BITMAPINFOHEADER) +
	...
	if(directcopy) {
		...
		for(i = 0; i < img->bmih.biHeight; i++) {
			image_convert_line(img, pLine,
			                   img->image + i * img->raster);
			pLine += bytewidth;
		}
	}
...
create_palette(IMAGE* img)
{
	...
	if(nColors) {
		...
		logpalette = (LPLOGPALETTE)malloc(
...
WndImg2Proc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_SYSCOMMAND:
		...
		if(LOWORD(wParam) == M_COPY_CLIP) {
			...
			CheckMenuItem(
			    GetSystemMenu(img->hwnd, FALSE), M_DEVICEN_GRAY,
	...
	case WM_SIZE:
		...
		img->cyAdjust =
		    min(img->bmih.biHeight, img->cyClient) - img->cyClient;
		...
		img->cxAdjust =
		    min(img->bmih.biWidth, img->cxClient) - img->cxClient;
		...
		if((wParam == SIZENORMAL) &&
		   (img->cxAdjust != 0 || img->cyAdjust != 0)) {
			...
			MoveWindow(GetParent(hwnd), rect.left, rect.top,
			           ...
			           rect.bottom - rect.top + img->cyAdjust,
			           TRUE);
	...
	case WM_VSCROLL:
		...
		if((nVscrollInc =
		        max(-img->nVscrollPos,
		            min(nVscrollInc,
		                img->nVscrollMax - img->nVscrollPos))) != 0) {
	...
	case WM_HSCROLL:
		...
		if((nHscrollInc =
		        max(-img->nHscrollPos,
		            min(nHscrollInc,
		                img->nHscrollMax - img->nHscrollPos))) != 0) {
	...
	case WM_DROPFILES:
		if(hwndtext)
			...
			cFiles =
			    DragQueryFile(hdrop, (UINT)(-1), (LPSTR)NULL, 0);
			for(i = 0; i < cFiles; i++) {
		...
		break;
	case WM_DESTROY: { /* Save the text window size */
		char winposbuf[64];
		sprintf(winposbuf, "%d %d %d %d", img->x, img->y, img->cx,
		        img->cy);
...
draw(IMAGE* img, HDC hdc, int dx, int dy, int wx, int wy, int sx, int sy)
{
	...
	for(; wy; dy += ny, wy -= ny, sy += ny) {
		...
		if(directcopy) {
			bits = img->image +
			       img->raster * (img->bmih.biHeight - (sy + ny));
		} else {
			image_convert_line(
			    img, line,
			    img->image +
