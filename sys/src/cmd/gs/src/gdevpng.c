...
private
dev_proc_print_page(png_print_page);
private
dev_proc_open_device(pngalpha_open);
private
dev_proc_encode_color(pngalpha_encode_color);
private
dev_proc_decode_color(pngalpha_decode_color);
private
dev_proc_copy_alpha(pngalpha_copy_alpha);
private
dev_proc_fill_rectangle(pngalpha_fill_rectangle);
private
dev_proc_get_params(pngalpha_get_params);
private
dev_proc_put_params(pngalpha_put_params);
private
dev_proc_create_buf_device(pngalpha_create_buf_device);

/* Monochrome. */

const gx_device_printer gs_pngmono_device =
    prn_device(prn_std_procs, "pngmono", DEFAULT_WIDTH_10THS,
...
private
const gx_device_procs png16_procs =
    prn_color_procs(gdev_prn_open, gdev_prn_output_page, gdev_prn_close,
                    pc_4bit_map_rgb_color, pc_4bit_map_color_rgb);
const gx_device_printer gs_png16_device = {prn_device_body(
    gx_device_printer, png16_procs, "png16", DEFAULT_WIDTH_10THS,
    DEFAULT_HEIGHT_10THS, X_DPI, Y_DPI, 0, 0, 0, 0, /* margins */
...
private
const gx_device_procs png256_procs =
    prn_color_procs(gdev_prn_open, gdev_prn_output_page, gdev_prn_close,
                    pc_8bit_map_rgb_color, pc_8bit_map_color_rgb);
const gx_device_printer gs_png256_device = {prn_device_body(
    gx_device_printer, png256_procs, "png256", DEFAULT_WIDTH_10THS,
    DEFAULT_HEIGHT_10THS, X_DPI, Y_DPI, 0, 0, 0, 0, /* margins */
...
private
const gx_device_procs pnggray_procs = prn_color_procs(
    gdev_prn_open, gdev_prn_output_page, gdev_prn_close,
    gx_default_gray_map_rgb_color, gx_default_gray_map_color_rgb);
const gx_device_printer gs_pnggray_device = {prn_device_body(
    gx_device_printer, pnggray_procs, "pnggray", DEFAULT_WIDTH_10THS,
    DEFAULT_HEIGHT_10THS, X_DPI, Y_DPI, 0, 0, 0, 0, /* margins */
...
private
const gx_device_procs png16m_procs =
    prn_color_procs(gdev_prn_open, gdev_prn_output_page, gdev_prn_close,
                    gx_default_rgb_map_rgb_color, gx_default_rgb_map_color_rgb);
const gx_device_printer gs_png16m_device =
    prn_device(png16m_procs, "png16m", DEFAULT_WIDTH_10THS,
               DEFAULT_HEIGHT_10THS, X_DPI, Y_DPI, 0, 0, 0, 0, /* margins */
               24, png_print_page);
...
private
const gx_device_procs png48_procs =
    prn_color_procs(gdev_prn_open, gdev_prn_output_page, gdev_prn_close,
                    gx_default_rgb_map_rgb_color, gx_default_rgb_map_color_rgb);
const gx_device_printer gs_png48_device =
    prn_device(png48_procs, "png48", DEFAULT_WIDTH_10THS, DEFAULT_HEIGHT_10THS,
               X_DPI, Y_DPI, 0, 0, 0, 0, /* margins */
               48, png_print_page);
...
private
const gx_device_procs pngalpha_procs = {
    pngalpha_open, NULL, /* get_initial_matrix */
    NULL,                /* sync_output */
    gdev_prn_output_page, gdev_prn_close,
    pngalpha_encode_color,                          /* map_rgb_color */
    pngalpha_decode_color,                          /* map_color_rgb */
    pngalpha_fill_rectangle, NULL,                  /* tile_rectangle */
    NULL,                                           /* copy_mono */
    NULL,                                           /* copy_color */
    NULL,                                           /* draw_line */
    NULL,                                           /* get_bits */
    pngalpha_get_params, pngalpha_put_params, NULL, /* map_cmyk_color */
    NULL,                                           /* get_xfont_procs */
    NULL,                                           /* get_xfont_device */
    NULL,                                           /* map_rgb_alpha_color */
    gx_page_device_get_page_device, NULL,           /* get_alpha_bits */
    pngalpha_copy_alpha, NULL,                      /* get_band */
    NULL,                                           /* copy_rop */
    NULL,                                           /* fill_path */
    NULL,                                           /* stroke_path */
    NULL,                                           /* fill_mask */
    ...
    gx_default_DevRGB_get_color_comp_index, pngalpha_encode_color,
...
const gx_device_pngalpha gs_pngalpha_device = {
    ...
    std_device_part2_((int)((float)(DEFAULT_WIDTH_10THS) * (X_DPI) / 10 + 0.5),
...
png_print_page(gx_device_printer* pdev, FILE* file)
{
	...
	if(info_ptr->color_type == PNG_COLOR_TYPE_PALETTE) {
		...
		info_ptr->palette = (void*)gs_alloc_bytes(
		    mem, 256 * sizeof(png_color), "png palette");
		...
		for(i = 0; i < num_colors; i++) {
			...
			info_ptr->palette[i].red =
			    gx_color_value_to_byte(rgb[0]);
			info_ptr->palette[i].green =
			    gx_color_value_to_byte(rgb[1]);
...
pngalpha_put_params(gx_device* pdev, gs_param_list* plist)
{
	...
	if(code == 0) {
		...
		if((ppdev->procs.fill_rectangle != pngalpha_fill_rectangle) &&
		   (ppdev->procs.fill_rectangle != NULL)) {
			/* Get current implementation of fill_rectangle and
			 * restore ours.
			 * Implementation is either clist or memory and can
			 * change
			 * during put_params.
			 */
			ppdev->orig_fill_rectangle =
			    ppdev->procs.fill_rectangle;
			ppdev->procs.fill_rectangle = pngalpha_fill_rectangle;
		}
	}
	return code;
...
pngalpha_encode_color(gx_device* dev, const gx_color_value cv[])
{
	 ...
	 */
	return ((uint)gx_color_value_to_byte(cv[2]) << 8) +
                        ...
                        gx_color_index color)
{
	...
	if((color == 0xffffff00) && (x == 0) && (y == 0) && (w == dev->width) &&
	   (h == dev->height)) {
...
private
int
pngalpha_copy_alpha(gx_device* dev, const byte* data, int data_x, int raster,
                    gx_bitmap_id id, int x, int y, int width, int height,
                    gx_color_index color, int depth)
{ /* This might be called with depth = 1.... */
	if(depth == 1)
		return (*dev_proc(dev, copy_mono))(dev, data, data_x, raster,
		                                   id, x, y, width, height,
	 ...
	 */
	{
		...
		for(ry = y; ry < y + height; row += raster, ++ry) {
			...
			if(code < 0)
				break;
			for(sx = data_x, rx = x; sx < data_x + width;
			    ++sx, ++rx) {
				...
				if(depth == 2) /* map 0 - 3 to 0 - 15 */
					...
					alpha2 = row[sx >> 1],
					alpha = (sx & 1 ? alpha2 & 0xf
					                : alpha2 >> 4);
				if(alpha ==
				   15) { /* Just write the new color. */
					composite = color;
				} else {
					if(previous ==
					   gx_no_color_index) { /* Extract the
					                           old color. */
						const byte* src =
						    line + (rx * (bpp >> 3));
						previous = 0;
						...
						previous +=
						    ...
						    << 16;
						previous +=
						    (gx_color_index)*src++ << 8;
						previous += *src++;
					}
					if(alpha ==
					   0) { /* Just write the old color. */
						composite = previous;
					} else { /* Blend values. */
						gx_color_value cv
						...
						(*dev_proc(dev, decode_color))(
						    dev, previous, cv);
						/* decode color doesn't give us
						 * coverage */
						cv[3] = previous & 0xff;
						...
						composite =
						    (*dev_proc(dev,
						               encode_color))(
						        dev, cv);
						/* encode color doesn't include
						 * coverage */
						composite |=
						    (255 - new_coverage) & 0xff;

						/* composite can never be
						 * gx_no_color_index
						 * because pixel is never
	...
	out:
		gs_free_object(mem, lout, "copy_alpha(lout)");
