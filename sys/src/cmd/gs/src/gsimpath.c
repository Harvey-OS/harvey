...
/* Copyright (C) 1989, 1992, 1998, 1999 Aladdin Enterprises.  All rights
  reserved.
	...
	for(y = height - 1; y >= 0; y--)
		for(x = width - 1; x >= 0; x--) {
			if(get_pixel(out, x, y) && !get_pixel(out, x, y - 1) &&
			   (!get_pixel(out, x + 1, y) ||
			    get_pixel(out, x + 1, y - 1)) &&
			   !trace_from(out, x, y,
			               1)) { /* Found a starting point */
				stat.count = 0;
				stat.dx = stat.dy = 0;
				if((code = trace_from(out, x, y, 0)) < 0)
					return code;
				add_deltas(out, 0, 0,
				           1); /* force out last segment */
...
trace_from(register status* out, int x0, int y0, int detect)
{
	...
	if(!detect) {
		part = (get_pixel(out, x + 1, y - 1) ? outline_scale - step
		                                     : step);
		code = gs_moveto(out->pgs, x + 1 - part / (float)outline_scale,
		                 (float)y);
		if(code < 0)
			return code;
	}
	while(1) { /* Relative to the current direction, */
		...
		if(get_pixel(out, x + tx,
		             y + ty)) { /* Cell at 45 degrees is full, */
...
#undef half_scale
			...
			x += tx, y += ty;
			dx = -dy, dy += tx;
		} else if(!get_pixel(out,
		                     x + dx, y + dy)) { /* Cell straight ahead
		                                           is empty, go
		                                           clockwise. */
			if(!detect) {
				add_deltas(out, dx, dy,
				           outline_scale - step - part);
		...
		if(dx == -step && dy == 0 &&
		   !(tx == -step && ty == -step)) { /* We just turned a corner
		                                       and are going west, */
