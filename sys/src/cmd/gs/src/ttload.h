...
Int LookUp_TrueType_Table(PFace face, Long tag);
...
TT_Error Load_TrueType_Any(PFace face, Long tag, Long offset, void* buffer,
                           Long* length);
...
#define DONE_Stream(_stream) TT_Done_Stream(&_stream)
...
#define ACCESS_Frame(_size_) (error = TT_Access_Frame(stream, &frame, _size_))
#define CHECK_ACCESS_Frame(_size_)                                             \
	(error = TT_Check_And_Access_Frame(stream, &frame, _size_))
#define FORGET_Frame() (error = TT_Forget_Frame(&frame))

#define FILE_Pos() TT_File_Pos(stream)

#define FILE_Seek(_position_) (error = TT_Seek_File(stream, _position_))
#define FILE_Skip(_distance_) (error = TT_Skip_File(stream, _distance_))
#define FILE_Read(buffer, count) (error = TT_Read_File(stream, buffer, count))
...
#define DEFINE_STREAM_LOCALS TT_Error error

/* Define stream locals without frame -- nothing in thread-safe mode */
#define DEFINE_STREAM_LOCALS_WO_FRAME TT_Error error

/* Define locals with a predefined stream in reentrant mode -- see ttload.c */
#define DEFINE_LOAD_LOCALS(STREAM) TT_Error error

/* Define locals without frame with a predefined stream - see ttload.c */
#define DEFINE_LOAD_LOCALS_WO_FRAME(STREAM) TT_Error error
...
#define ACCESS_Frame(_size_) (error = TT_Access_Frame(_size_))
#define CHECK_ACCESS_Frame(_size_) (error = TT_Check_And_Access_Frame(_size_))
#define FORGET_Frame() (error = TT_Forget_Frame())

#define GET_Tag4() TT_Get_Long()

#define FILE_Pos() TT_File_Pos()

#define FILE_Seek(_position_) (error = TT_Seek_File(_position_))
#define FILE_Skip(_distance_) (error = TT_Skip_File(_distance_))
#define FILE_Read(buffer, count) (error = TT_Read_File(buffer, count))
#define FILE_Read_At(pos, buffer, count)                                       \
