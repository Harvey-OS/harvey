...
gc_locate(const void* ptr, gc_state_t* gcst)
{
	...
	if(gcst->space_local != gcst->space_global) {
		gcst->loc.memory = other =
		    (mem->space == avm_local ? gcst->space_global
		                             : gcst->space_local);
		gcst->loc.cp = 0;
		if(chunk_locate(ptr, &gcst->loc))
			return gcst->loc.cp;
		...
		if(other->stable_memory != (const gs_memory_t*)other) {
			gcst->loc.memory =
			    (gs_ref_memory_t*)other->stable_memory;
			gcst->loc.cp = 0;
			if(chunk_locate(ptr, &gcst->loc))
				return gcst->loc.cp;
			gcst->loc.memory = other;
...
ialloc_validate_spaces(const gs_dual_memory_t* dmem)
{
	int i;
	gc_state_t state;
	alloc_temp_save_t save[countof(dmem->spaces_indexed)],
...
ialloc_validate_memory(const gs_ref_memory_t* mem, gc_state_t* gcst)
{
	...
	for(smem = mem, level = 0; smem != 0;
		...
		if_debug3('6',
		          "[6]validating memory 0x%lx, space %d, level %d\n",
		          (uint32_t)mem, mem->space, level);
		/* Validate chunks. */
		...
		for(i = 0; i < num_freelists; ++i) {
			...
			for(pfree = mem->freelists[i]; pfree != 0;
				...
				if(pfree[-1].o_type != &st_free) {
					lprintf3("Non-free object 0x%lx(%u) on "
					         "freelist %i!\n",
					         (uint32_t)pfree, size, i);
					break;
				}
				if((i == LARGE_FREELIST_INDEX &&
				    size < max_freelist_size) ||
				   (i != LARGE_FREELIST_INDEX &&
				    (size < free_size - obj_align_mask ||
				     size > free_size))) {
					lprintf3("Object 0x%lx(%u) size wrong "
					         "on freelist %i!\n",
					         (uint32_t)pfree, size, i);
					break;
				}
...
private
void ialloc_validate_ref(const ref*, gc_state_t*, const obj_header_t* pre_fr);
private
void ialloc_validate_ref_packed(const ref_packed*, gc_state_t*,
                                const obj_header_t* pre_fr);
#else
private
void ialloc_validate_ref(const ref*, gc_state_t*);
private
void ialloc_validate_ref_packed(const ref_packed*, gc_state_t*);
#endif
void
ialloc_validate_chunk(const chunk_t* cp, gc_state_t* gcst)
{
	...
	if(pre->o_type == &st_free) {
		if(!object_size_valid(pre, size, cp))
			lprintf3(
			    "Bad free object 0x%lx(%lu), in chunk 0x%lx!\n",
			    (uint32_t)(pre + 1), (uint32_t)size, (uint32_t)cp);
	} else
		ialloc_validate_object(pre + 1, cp, gcst);
	if_debug3('7', " [7]validating %s(%lu) 0x%lx\n",
	          struct_type_name_string(pre->o_type), (uint32_t)size,
	          (uint32_t)pre);
		...
		if(proc != gs_no_struct_enum_ptrs)
			for(; (ptype = (*proc)(gcst_get_memory_ptr(gcst),
				...
				if(eptr.ptr == 0)
					...
					ialloc_validate_object(eptr.ptr, NULL,
					                       gcst);
#if IGC_PTR_STABILITY_CHECK
					ialloc_validate_pointer_stability(
					    pre,
					    (const obj_header_t*)eptr.ptr - 1);
#endif
				} else if(ptype == ptr_ref_type)
#if IGC_PTR_STABILITY_CHECK
					ialloc_validate_ref_packed(eptr.ptr,
					                           gcst, pre);
#else
					ialloc_validate_ref_packed(eptr.ptr,
					                           gcst);
                    ...
                    )
{
...
#if IGC_PTR_STABILITY_CHECK
			ialloc_validate_pointer_stability(
			    pre_fr, (const obj_header_t*)optr - 1);
#endif
		}
		break;
	case t_name:
		if(name_index_ptr(cmem, name_index(cmem, pref)) !=
		   pref->value.pname) {
		...
		}
		{
			...
			if(r_space(&sref) != avm_foreign &&
			   !gc_locate(sref.value.const_bytes, gcst)) {
				lprintf4("At 0x%lx, bad name %u, pname = "
				         "0x%lx, string 0x%lx not in any "
				         "chunk\n",
	...
	case t_string:
		if(r_size(pref) != 0 && !gc_locate(pref->value.bytes, gcst))
			lprintf3(
			    "At 0x%lx, string ptr 0x%lx[%u] not in any chunk\n",
	...
	cka:
		...
		}
		{
			...
			for(i = 0; i < size; ++i) {
				...
				if(r_is_packed(elt))
					lprintf5("At 0x%lx, %s 0x%lx[%u] "
					         "element %u is not a ref\n",
					         (ulong)pref, tname,
					         (ulong)rptr, size, i);
			}
		}
	...
	case t_mixedarray:
		...
		if(!gc_locate(optr, gcst))
			lprintf2(
			    "At 0x%lx, packed array 0x%lx not in any chunk\n",
...
ialloc_validate_pointer_stability(const obj_header_t* ptr_fr,
                                  const obj_header_t* ptr_to)
{
	static const char* sn[] = {"undef",        "undef",         "system",
	                           "undef",        "global_stable", "global",
	                           "local_stable", "local"};

	if(ptr_fr->d.o.space_id < ptr_to->d.o.space_id) {
		const char* sn_fr = (ptr_fr->d.o.space_id < count_of(sn)
		                         ? sn[ptr_fr->d.o.space_id]
		                         : "unknown");
		const char* sn_to = (ptr_to->d.o.space_id < count_of(sn)
		                         ? sn[ptr_to->d.o.space_id]
		                         : "unknown");

		lprintf6("Reference to a less stable object 0x%lx<%s> "
		         "in the space \'%s\' from 0x%lx<%s> in the space "
		         "\'%s\' !\n",
		         (uint32_t)ptr_to, ptr_to->d.o.t.type->sname, sn_to,
		         (uint32_t)ptr_fr, ptr_fr->d.o.t.type->sname, sn_fr);
	}
}
#endif
                       ...
                       gc_state_t* gcst)
{
	...
	if(otype == &st_free) {
		lprintf3(
		    "Reference to free object 0x%lx(%lu), in chunk 0x%lx!\n",
		    (uint32_t)ptr, (uint32_t)size, (uint32_t)cp);
		gs_abort(gcst->heap);
	}
	if((cp != 0 && !object_size_valid(pre, size, cp)) ||
	   otype->ssize == 0 || size % otype->ssize != 0 ||
