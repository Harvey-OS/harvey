...
#define stream_proc_switch_mode(proc) int proc(stream*, bool)
...
#define sbuf_min_left(s)                                                       \
	((s->end_status == EOFC || s->end_status == ERRC                       \
	      ? 0                                                              \
	      : s->state->min_left))
...
#define s_declare_inline(s, cp, ep)                                            \
	...
	const byte* ep
#define s_begin_inline(s, cp, ep) cp = (s)->srptr, ep = (s)->srlimit
#define s_end_inline(s, cp, ep) (s)->srptr = cp
#define sbufavailable_inline(s, cp, ep) (ep - cp)
#define sendbufp_inline(s, cp, ep) (cp >= ep)
...
#define sputback_inline(s, cp, ep) --cp
...
int s_std_read_flush(stream*), s_std_write_flush(stream*),
    s_std_noavailable(stream*, long*), s_std_noseek(stream*, long),
    s_std_close(stream*), s_std_switch_mode(stream*, bool);
