...
/* Forward references */
private
int array_new_indexed_plist_write(dict_param_list *plist,
...
int ref_param_write_string_value(ref *, const gs_param_string *,
				 gs_ref_memory_t *);
private
int ref_param_write_name_value(const gs_memory_t *mem, ref *, const gs_param_string *);
private
int
...
private
int ref_param_read(iparam_list *, gs_param_name,
		   iparam_loc *, int);
private
int ref_param_read_string_value(const gs_memory_t *mem,
				const iparam_loc *,
				gs_param_string *);
private
		     ...
		     gs_param_typed_value *pvalue)
{
	...
	case t_dictionary:
		...
		/* fixup new dict's type & int_keys field if contents have int keys */
		{
			...
			if(!(*((iparam_list *)plist)->enumerate)((iparam_list *)pvalue->value.d.list, &enumr, &key, &keytype) && keytype == t_integer) {
