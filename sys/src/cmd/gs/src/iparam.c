...
ref_param_key(const iparam_list* plist, gs_param_name pkey, ref* pkref)
{
	if(plist->int_keys) {
		...
		return name_ref(plist->memory, (const byte*)pkey, strlen(pkey),
		                pkref, 0);
}
...
/* Forward references */
private
int array_new_indexed_plist_write(dict_param_list* plist, ref* parray,
                                  const ref* pwanted, gs_ref_memory_t* imem);

/* ---------------- Generic writing procedures ---------------- */

private
param_proc_begin_xmit_collection(ref_param_begin_write_collection);
private
param_proc_end_xmit_collection(ref_param_end_write_collection);
private
param_proc_xmit_typed(ref_param_write_typed);
private
param_proc_next_key(ref_param_get_next_key);
private
param_proc_requested(ref_param_requested);
private
const gs_param_list_procs ref_write_procs = {
    ref_param_write_typed, ref_param_begin_write_collection,
    ref_param_end_write_collection, ref_param_get_next_key, NULL, /* request */
    ref_param_requested};
private
int ref_array_param_requested(const iparam_list*, gs_param_name, ref*, uint,
                              client_name_t);
private
int ref_param_write(iparam_list*, gs_param_name, const ref*);
private
int ref_param_write_string_value(ref*, const gs_param_string*,
                                 gs_ref_memory_t*);
private
int ref_param_write_name_value(const gs_memory_t* mem, ref*,
...
{
	return ref_param_write_string_value(
	    pe, &((const gs_param_string_array*)pvalue)->data[i], imem);
}
private
int
ref_param_make_name(ref* pe, const void* pvalue, uint i, gs_ref_memory_t* imem)
{
	return ref_param_write_name_value(
	    (const gs_memory_t*)imem, pe,
                                 ...
                                 gs_param_collection_type_t coll_type)
{
	...
	dict_param_list* dlist = (dict_param_list*)gs_alloc_bytes(
	    plist->memory, size_of(dict_param_list),
	...
	if(coll_type != gs_param_collection_array) {
		...
		code = gs_alloc_ref_array(imem, &aref, a_all, pvalue->size,
		                          "ref_param_begin_write_collection");
		if(code >= 0)
			code = array_new_indexed_plist_write(dlist, &aref, NULL,
			                                     imem);
	}
	if(code < 0)
		gs_free_object(plist->memory, dlist,
		               "ref_param_begin_write_collection");
	else
                      ...
                      gs_param_typed_value* pvalue)
{
	...
	case gs_param_type_name:
		if(!ref_param_requested(plist, pkey))
			return 0;
		code = ref_param_write_name_value(iplist->memory, &value,
		                                  &pvalue->value.n);
		break;
	case gs_param_type_int_array:
		return ref_param_write_typed_array(
		    plist, pkey, &pvalue->value.ia, pvalue->value.ia.size,
		    ref_param_make_int);
	case gs_param_type_float_array:
		return ref_param_write_typed_array(
		    plist, pkey, &pvalue->value.fa, pvalue->value.fa.size,
		    ref_param_make_float);
	case gs_param_type_string_array:
		return ref_param_write_typed_array(
		    plist, pkey, &pvalue->value.sa, pvalue->value.sa.size,
		    ref_param_make_string);
	case gs_param_type_name_array:
		return ref_param_write_typed_array(
		    plist, pkey, &pvalue->value.na, pvalue->value.na.size,
		    ref_param_make_name);
	case gs_param_type_dict:
	case gs_param_type_dict_int_keys:
	case gs_param_type_array:
		return ref_param_begin_write_collection(
		    plist, pkey, &pvalue->value.d,
		    (gs_param_collection_type_t)(pvalue->type -
		                                 gs_param_type_dict));
	default:
		return_error(e_typecheck);
	}
                          ...
                          ref* pvalue, uint size, client_name_t cname)
{
	...
	if(!ref_param_requested((const gs_param_list*)iplist, pkey))
		return 0;
	code =
	    gs_alloc_ref_array(iplist->ref_memory, pvalue, a_all, size, cname);
...
private
int
ref_param_write_name_value(const gs_memory_t* mem, ref* pref,
...
private
int /* ret 0 ok, 1 if EOF, or -ve err */
...
private
param_proc_begin_xmit_collection(ref_param_begin_read_collection);
private
param_proc_end_xmit_collection(ref_param_end_read_collection);
private
param_proc_xmit_typed(ref_param_read_typed);

/*private param_proc_next_key(ref_param_get_next_key); already dec'ld above */
private
param_proc_get_policy(ref_param_read_get_policy);
private
param_proc_signal_error(ref_param_read_signal_error);
private
param_proc_commit(ref_param_read_commit);
private
const gs_param_list_procs ref_read_procs = {
    ref_param_read_typed, ref_param_begin_read_collection,
    ref_param_end_read_collection, ref_param_get_next_key, NULL, /* request */
    NULL,                                                        /* requested */
    ref_param_read_get_policy, ref_param_read_signal_error,
    ref_param_read_commit};
private
int ref_param_read(iparam_list*, gs_param_name, iparam_loc*, int);
private
int ref_param_read_string_value(const gs_memory_t* mem, const iparam_loc*,
                                gs_param_string*);
private
int ref_param_read_array(iparam_list*, gs_param_name, iparam_loc*);

#define iparam_note_error(loc, code) gs_note_error(*(loc).presult = code)
                           ...
                           gs_param_float_array* pvalue)
{
	...
	if(code < 0) {
		gs_free_object(plist->memory, pfv,
                            ...
                            gs_param_string_array* pvalue)
{
	...
	if(code != 0)
		return code;
	size = r_size(loc.pvalue);
	psv = (gs_param_string*)gs_alloc_byte_array(
	    plist->memory, size, sizeof(gs_param_string),
	...
	if(r_has_type(&aref, t_array)) {
		for(i = 0; code >= 0 && i < size; i++) {
			...
			code = ref_param_read_string_value(plist->memory, &loc,
			                                   psv + i);
		}
	} else {
		ref elt;
	...
	if(code < 0) {
		gs_free_object(plist->memory, psv,
		               "ref_param_read_string_array");
		return (*loc.presult = code);
	}
	pvalue->data = psv;
                                ...
                                gs_param_collection_type_t coll_type)
{
	...
	if(code != 0)
		return code;
	dlist = (dict_param_list*)gs_alloc_bytes(
	    plist->memory, size_of(dict_param_list),
	...
	if(r_has_type(loc.pvalue, t_dictionary)) {
		...
		code = array_indexed_param_list_read(dlist, loc.pvalue, NULL,
		                                     false, iplist->ref_memory);
		if(code >= 0)
                     ...
                     gs_param_typed_value* pvalue)
{
	...
	case t_boolean:
		...
		return 0;
	case t_dictionary:
		code = ref_param_begin_read_collection(
		    plist, pkey, &pvalue->value.d,
		...
		/* fixup new dict's type & int_keys field if contents have int
		 * keys */
		{
			...
			if(!(*((iparam_list*)plist)->enumerate)(
			       (iparam_list*)pvalue->value.d.list, &enumr, &key,
			       &keytype) &&
			   keytype == t_integer) {
				((dict_param_list*)pvalue->value.d.list)
				    ->int_keys = 1;
	...
	case t_name:
		...
		return ref_param_read_string_value(plist->memory, &loc,
		                                   &pvalue->value.n);
	case t_null:
...
private
int
ref_param_read_string_value(const gs_memory_t* mem, const iparam_loc* ploc,
                    ...
                    bool require_all, gs_ref_memory_t* imem)
{
	...
	plist->count = count;
	plist->results = (int*)gs_alloc_byte_array(
	    plist->memory, count, sizeof(int), "ref_param_read_init");
...
int
stack_param_list_read(stack_param_list* plist, ref_stack_t* pstack, uint skip,
                      const ref* ppolicies, bool require_all,
                      gs_ref_memory_t* imem)
{
	iparam_list* const iplist = (iparam_list*)plist;
