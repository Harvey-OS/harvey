                ...
                unsigned int format)
{
	...
	char buf[256];
	sprintf(buf,
	        "display_presize(0x%x, 0x%x, width=%d height=%d raster=%d\n\
  format=%d)\n",
	...
	/* insert display device defaults as first arguments */
	{
		...
		if(depth == 32)
			...
			format = DISPLAY_COLORS_NATIVE | DISPLAY_ALPHA_NONE |
			         DISPLAY_DEPTH_16 | DISPLAY_LITTLEENDIAN |
			         DISPLAY_BOTTOMFIRST | DISPLAY_NATIVE_555;
		else if(depth > 8)
...
set_font(void)
{
	...
	/* read ini file */
	GetPrivateProfileString(szIniSection, "FontName", "Courier New",
	                        fontname, sizeof(fontname), szIniName);
	fontsize =
	    GetPrivateProfileInt(szIniSection, "FontSize", 10, szIniName);

	/* set font */
	text_font(tw, fontname, fontsize);

	/* write ini file */
	WritePrivateProfileString(szIniSection, "FontName", fontname,
	                          szIniName);
...
#define MAXCMDTOKENS 128
	...
	tw = text_new();
	if(tw == NULL) {
		MessageBox((HWND)NULL, "Can't create text window", szAppName,
		           MB_OK | MB_ICONSTOP);
		return 1;
	}
	...
	if(!hPrevInstance) {
		HICON hicon =
		    LoadIcon(hInstance, (LPSTR)MAKEINTRESOURCE(GSTEXT_ICON));
	...
	if(dll_exit_status && !tw->quitnow) {
		...
		MSG msg;
		text_puts(tw, "\nClose this window with the close button on "
		              "the title bar or the system menu.\n");
		if(IsIconic(text_get_handle(tw)))
			ShowWindow(text_get_handle(tw), SW_SHOWNORMAL);
		BringWindowToTop(
		    text_get_handle(tw)); /* make text window visible */
