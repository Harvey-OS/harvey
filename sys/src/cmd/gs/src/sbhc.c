...
/* Copyright (C) 1994, 1995, 1996, 1998 Aladdin Enterprises.  All rights
  reserved.
...
{
	stream_BHCE_state* const ss = (stream_BHCE_state*)st;
	hce_code* encode = ss->encode.codes = (hce_code*)gs_alloc_byte_array(
	    st->memory, ss->definition.num_values, sizeof(hce_code),
	    "BHCE encode");
...
/* Stream template */
const stream_template s_BHCE_template = {&st_BHCE_state, s_BHCE_init,
                                         s_BHCE_process, 1, hc_bits_size >> 3,
                                         s_BHCE_release, NULL, s_BHCE_reinit};
...
s_BHCD_init(register stream_state* st)
{
	...
	hcd_code* decode = ss->decode.codes = (hcd_code*)gs_alloc_byte_array(
	    st->memory, dsize, sizeof(hcd_code), "BHCD decode");
...
z:
	...
	w1:
		...
		if((clen = cp->code_length) > initial_bits) {
			if(!hcd_bits_available(
			       clen)) { /* We don't have enough bits for */
...
/* Stream template */
const stream_template s_BHCD_template = {&st_BHCD_state, s_BHCD_init,
                                         s_BHCD_process, 1, 1, s_BHCD_release,
                                         NULL, s_BHCD_reinit};
