...
static TT_F26Dot6
Norm(TT_F26Dot6 X, TT_F26Dot6 Y)
{
	Int64 T1, T2;
...
static TT_F26Dot6
FUnits_To_Pixels(EXEC_OPS Int distance)
{
	return MulDiv_Round(distance, CUR.metrics.scale1, CUR.metrics.scale2);
}
...
static Long Current_Ratio(EXEC_OP)
{
	...
	if(CUR.GS.projVector.y == 0)
		...
		x = MulDiv_Round(CUR.GS.projVector.x, CUR.metrics.x_ratio,
		                 0x4000);
		y = MulDiv_Round(CUR.GS.projVector.y, CUR.metrics.y_ratio,
		                 0x4000);
...
static TT_F26Dot6
Read_CVT(EXEC_OPS Int index)
{
	return CUR.cvt[index];
}

static TT_F26Dot6
Read_CVT_Stretched(EXEC_OPS Int index)
{
	return MulDiv_Round(CUR.cvt[index], CURRENT_Ratio(), 0x10000);
}

static void
Write_CVT(EXEC_OPS Int index, TT_F26Dot6 value)
{
...
static void
Write_CVT_Stretched(EXEC_OPS Int index, TT_F26Dot6 value)
{
	int ov = CUR.cvt[index];
...
static void
Move_CVT(EXEC_OPS Int index, TT_F26Dot6 value)
{
	int ov = CUR.cvt[index];
	CUR.cvt[index] += value;
	DBG_PRINT3(" cvt[%d]%d:=%d", index, ov, CUR.cvt[index]);
...
static void
Move_CVT_Stretched(EXEC_OPS Int index, TT_F26Dot6 value)
{
	int ov = CUR.cvt[index];
	CUR.cvt[index] += MulDiv_Round(value, 0x10000, CURRENT_Ratio());
	DBG_PRINT3(" cvt[%d]%d:=%d", index, ov, CUR.cvt[index]);
...
static Short GetShortIns(EXEC_OP)
{
	...
	CUR.IP += 2;
	return (CUR.code[CUR.IP - 2] << 8) + CUR.code[CUR.IP - 1];
}
...
static Bool
Ins_Goto_CodeRange(EXEC_OPS Int aRange, Int aIP)
{
	TCodeRange* WITH;
...
static void
Direct_Move(EXEC_OPS PGlyph_Zone zone, Int point, TT_F26Dot6 distance)
{
	TT_F26Dot6 v;
	...
	if(v != 0) {
		zone->cur_x[point] +=
		    MulDiv_Round(distance, v * 0x10000L, CUR.F_dot_P);
...
static void
Direct_Move_X(EXEC_OPS PGlyph_Zone zone, Int point, TT_F26Dot6 distance)
{
	(void)exc;
...
static void
Direct_Move_Y(EXEC_OPS PGlyph_Zone zone, Int point, TT_F26Dot6 distance)
{
	(void)exc;
...
static TT_F26Dot6
Round_None(EXEC_OPS TT_F26Dot6 distance, TT_F26Dot6 compensation)
{
	TT_F26Dot6 val;
	(void)exc;
...
static TT_F26Dot6
Round_To_Grid(EXEC_OPS TT_F26Dot6 distance, TT_F26Dot6 compensation)
{
	TT_F26Dot6 val;
	(void)exc;
...
static TT_F26Dot6
Round_To_Half_Grid(EXEC_OPS TT_F26Dot6 distance, TT_F26Dot6 compensation)
{
	TT_F26Dot6 val;
	(void)exc;
...
static TT_F26Dot6
Round_Down_To_Grid(EXEC_OPS TT_F26Dot6 distance, TT_F26Dot6 compensation)
{
	TT_F26Dot6 val;
	(void)exc;
...
static TT_F26Dot6
Round_Up_To_Grid(EXEC_OPS TT_F26Dot6 distance, TT_F26Dot6 compensation)
{
	TT_F26Dot6 val;
	(void)exc;
...
static TT_F26Dot6
Round_To_Double_Grid(EXEC_OPS TT_F26Dot6 distance, TT_F26Dot6 compensation)
{
	TT_F26Dot6 val;
	(void)exc;
...
static TT_F26Dot6
Round_Super(EXEC_OPS TT_F26Dot6 distance, TT_F26Dot6 compensation)
{
	TT_F26Dot6 val;
...
static TT_F26Dot6
Round_Super_45(EXEC_OPS TT_F26Dot6 distance, TT_F26Dot6 compensation)
{
	TT_F26Dot6 val;
...
static void
Compute_Round(EXEC_OPS Byte round_mode)
{
	switch(round_mode) {
	case TT_Round_Off:
...
static void
SetSuperRound(EXEC_OPS TT_F26Dot6 GridPeriod, Long selector)
{
...
static TT_F26Dot6
Project(EXEC_OPS TT_F26Dot6 Vx, TT_F26Dot6 Vy)
{
	THROW_PATENTED;
	return 0;
}
...
static TT_F26Dot6
Dual_Project(EXEC_OPS TT_F26Dot6 Vx, TT_F26Dot6 Vy)
{
	THROW_PATENTED;
	return 0;
}
...
static TT_F26Dot6
Free_Project(EXEC_OPS TT_F26Dot6 Vx, TT_F26Dot6 Vy)
{
	THROW_PATENTED;
	return 0;
}
...
static TT_F26Dot6
Project_x(EXEC_OPS TT_F26Dot6 Vx, TT_F26Dot6 Vy)
{
	(void)exc;
	(void)Vy;
	return Vx;
...
static TT_F26Dot6
Project_y(EXEC_OPS TT_F26Dot6 Vx, TT_F26Dot6 Vy)
{
	(void)exc;
	(void)Vx;
	return Vy;
...
static void Compute_Funcs(EXEC_OP)
{
	if(CUR.GS.freeVector.x == 0x4000) {
		...
		if(CUR.GS.freeVector.y == 0x4000) {
			...
			CUR.F_dot_P =
			    (Long)CUR.GS.projVector.x * CUR.GS.freeVector.x *
			        4 +
...
static Bool
Normalize(EXEC_OPS TT_F26Dot6 Vx, TT_F26Dot6 Vy, TT_UnitVector* R)
{
	TT_F26Dot6 W;
	Bool S1, S2;
	...
	if(ABS(Vx) < 0x10000L && ABS(Vy) < 0x10000L) {
		...
		if(W == 0) {
			/* XXX : Undocumented. It seems that it's possible to
			 * try  */
			/*       to normalize the vector (0,0). Return
			 * immediately */
			return SUCCESS;
		}
...
static void Ins_MINDEX(INS_ARG)
{
	...
	memmove((&CUR.stack[CUR.args - L]), (&CUR.stack[CUR.args - L + 1]),
	        (L - 1) * sizeof(Long));
...
{
	args[0] = Round_None(EXEC_ARGS args[0],
...
static void Ins_ENDF(INS_ARG)
{
	...
	if(pRec->Cur_Count > 0) {
		...
		INS_Goto_CodeRange(pRec->Caller_Range, pRec->Caller_IP);
...
static void Ins_CALL(INS_ARG)
{
	...
	INS_Goto_CodeRange(CUR.FDefs[args[0]].Range, CUR.FDefs[args[0]].Start);
...
static Bool
Ins_SxVTL(EXEC_OPS Int aIdx1, Int aIdx2, Int aOpc, TT_UnitVector* Vec)
{
	Long A, B, C;

	if(BOUNDS(aIdx1, CUR.zp2.n_points) || BOUNDS(aIdx2, CUR.zp1.n_points)) {
		CUR.error = TT_Err_Invalid_Reference;
		return FAILURE;
	}
...
{
	if(INS_SxVTL(args[1], args[0], CUR.opcode, &CUR.GS.projVector) ==
	   FAILURE)
		return;
...
{
	if(INS_SxVTL((Int)(args[1]), (Int)(args[0]), CUR.opcode,
...
static void Ins_SDPVTL(INS_ARG)
{
	...
	if(BOUNDS(p2, CUR.zp1.n_points) || BOUNDS(p1, CUR.zp2.n_points)) {
		CUR.error = TT_Err_Invalid_Reference;
		return;
	}
...
static void Ins_INSTCTRL(INS_ARG)
{
	...
	CUR.GS.instruct_control =
	    (Int)((CUR.GS.instruct_control & (~K)) | (L & K));
}
...
static void Ins_GC(INS_ARG)
{
	...
	switch(CUR.opcode & 1) {
	case 0:
		L = CUR_Func_project(CUR.zp2.cur_x[L], CUR.zp2.cur_y[L]);
		break;

	case 1:
		L = CUR_Func_dualproj(CUR.zp2.org_x[L], CUR.zp2.org_y[L]);
		break;
	}
...
static void Ins_SCFS(INS_ARG)
{
	...
	K = CUR_Func_project(CUR.zp2.cur_x[L], CUR.zp2.cur_y[L]);
...
static void Ins_FLIPRGON(INS_ARG)
{
	...
	if(BOUNDS(K, CUR.pts.n_points) || BOUNDS(L, CUR.pts.n_points)) {
		CUR.error = TT_Err_Invalid_Reference;
		return;
	}
...
static Bool
Compute_Point_Displacement(EXEC_OPS PCoordinates x, PCoordinates y,
                           PGlyph_Zone zone, Int* refp)
{
	TGlyph_Zone zp;
	Int p;
	TT_F26Dot6 d;
...
static void
Move_Zp2_Point(EXEC_OPS Long point, TT_F26Dot6 dx, TT_F26Dot6 dy, Bool touch)
{
...
static void Ins_SHPIX(INS_ARG)
{
	...
	dx = MulDiv_Round(args[0], (Long)CUR.GS.freeVector.x, 0x4000);
	dy = MulDiv_Round(args[0], (Long)CUR.GS.freeVector.y, 0x4000);
...
static void Ins_MSIRP(INS_ARG)
{
	...
	distance =
	    CUR_Func_project(CUR.zp1.cur_x[point] - CUR.zp0.cur_x[CUR.GS.rp0],
	                     CUR.zp1.cur_y[point] - CUR.zp0.cur_y[CUR.GS.rp0]);
...
static void Ins_MDAP(INS_ARG)
{
	...
	if((CUR.opcode & 1) != 0) {
		cur_dist = CUR_Func_project(CUR.zp0.cur_x[point],
		                            CUR.zp0.cur_y[point]);
		distance =
		    CUR_Func_round(cur_dist, CUR.metrics.compensations[0]) -
		    cur_dist;
	} else
		distance = 0;
...
{
	Int cvtEntry, point;
	TT_F26Dot6 distance, org_dist;

	cvtEntry = (Int)args[1];
	point = (Int)args[0];

	if(BOUNDS(args[0], CUR.zp0.n_points) || BOUNDS(args[1], CUR.cvtSize)) {
		CUR.error = TT_Err_Invalid_Reference;
		return;
	}
	...
	DBG_PRINT3(" cvtEntry=%d point=%d distance=%d", cvtEntry, point,
	           distance);

	if(CUR.GS.gep0 == 0) /* If in twilight zone */
	{
		CUR.zp0.org_x[point] =
		    MulDiv_Round(CUR.GS.freeVector.x, distance, 0x4000L);
		CUR.zp0.cur_x[point] = CUR.zp0.org_x[point];

		CUR.zp0.org_y[point] =
		    MulDiv_Round(CUR.GS.freeVector.y, distance, 0x4000L);
		CUR.zp0.cur_y[point] = CUR.zp0.org_y[point];
	}

	org_dist = CUR_Func_project(CUR.zp0.cur_x[point], CUR.zp0.cur_y[point]);
	...
	if((CUR.opcode & 1) != 0) /* rounding and control cutin flag */
	{
		...
		distance =
		    CUR_Func_round(distance, CUR.metrics.compensations[0]);
	}
...
static void Ins_MDRP(INS_ARG)
{
	...
	org_dist =
	    CUR_Func_dualproj(CUR.zp1.org_x[point] - CUR.zp0.org_x[CUR.GS.rp0],
	                      CUR.zp1.org_y[point] - CUR.zp0.org_y[CUR.GS.rp0]);
	...
	if((CUR.opcode & 4) != 0)
		distance = CUR_Func_round(
		    org_dist, CUR.metrics.compensations[CUR.opcode & 3]);
	else
		distance =
		    Round_None(EXEC_ARGS org_dist,
...
static void Ins_MIRP(INS_ARG)
{
	...
	TT_F26Dot6 cvt_dist, distance, cur_dist, org_dist;
	...
	if(CUR.GS.gep1 == 0) {
		CUR.zp1.org_x[point] =
		    ...
		    MulDiv_Round(cvt_dist, CUR.GS.freeVector.x, 0x4000);

		CUR.zp1.org_y[point] =
		    CUR.zp0.org_y[CUR.GS.rp0] +
		    MulDiv_Round(cvt_dist, CUR.GS.freeVector.y, 0x4000);
	...
	if((CUR.opcode & 4) != 0) {
		...
		if(CUR.GS.gep0 == CUR.GS.gep1)
			if(ABS(cvt_dist - org_dist) >=
			   CUR.GS.control_value_cutin)
				cvt_dist = org_dist;

		distance = CUR_Func_round(
		    cvt_dist, CUR.metrics.compensations[CUR.opcode & 3]);
	} else
		distance =
		    Round_None(EXEC_ARGS cvt_dist,
		               CUR.metrics.compensations[CUR.opcode & 3]);
...
static void Ins_ALIGNRP(INS_ARG)
{
	...
	while(CUR.GS.loop > 0) {
		...
		distance = CUR_Func_project(
		    CUR.zp1.cur_x[point] - CUR.zp0.cur_x[CUR.GS.rp0],
		    CUR.zp1.cur_y[point] - CUR.zp0.cur_y[CUR.GS.rp0]);
...
static void Ins_ISECT(INS_ARG)
{
	...
	TT_F26Dot6 dx, dy, dax, day, dbx, dby;
	...
	if(BOUNDS(b0, CUR.zp0.n_points) || BOUNDS(b1, CUR.zp0.n_points) ||
	   BOUNDS(a0, CUR.zp1.n_points) || BOUNDS(a1, CUR.zp1.n_points)) {
		CUR.error = TT_Err_Invalid_Reference;
		return;
	}
	...
	discriminant =
	    MulDiv_Round(dax, -dby, 0x40L) + MulDiv_Round(day, dbx, 0x40L);

	if(ABS(discriminant) >= 0x40) {
		val = MulDiv_Round(dx, -dby, 0x40L) +
		      MulDiv_Round(dy, dbx, 0x40L);
		...
		CUR.zp2.cur_x[point] = (CUR.zp1.cur_x[a0] + CUR.zp1.cur_x[a1] +
		                        CUR.zp0.cur_x[b0] + CUR.zp1.cur_x[b1]) /
		                       4;
		CUR.zp2.cur_y[point] = (CUR.zp1.cur_y[a0] + CUR.zp1.cur_y[a1] +
		                        CUR.zp0.cur_y[b0] + CUR.zp1.cur_y[b1]) /
		                       4;
	}
}
...
static void Ins_ALIGNPTS(INS_ARG)
{
	...
	distance = CUR_Func_project(CUR.zp0.cur_x[p2] - CUR.zp1.cur_x[p1],
	                            CUR.zp0.cur_y[p2] - CUR.zp1.cur_x[p1]) /
	           2;
...
{
	TT_F26Dot6 org_a, org_b, org_x, cur_a, cur_b, cur_x, distance;
	Int point;
	(void)args;
	...
	while(CUR.GS.loop > 0) {
		...
		if((org_a <= org_b && org_x <= org_a) ||
		   (org_a > org_b && org_x >= org_a))
			...
			distance = MulDiv_Round(cur_b - cur_a, org_x - org_a,
			                        org_b - org_a) +
			           (cur_a - cur_x);
...
static void
Shift(Int p1, Int p2, Int p, struct LOC_Ins_IUP* LINK)
{
	Int i;
	TT_F26Dot6 x;
...
static void
Interp(Int p1, Int p2, Int ref1, Int ref2, struct LOC_Ins_IUP* LINK)
{
	Long i;
	...
	for(i = p1; i <= p2; i++) {
		...
		if(x <= x2)
			...
			if(x >= x1)
				...
				x = LINK->curs[ref1] +
				    MulDiv_Round(x - x1, LINK->curs[ref2] -
				                             LINK->curs[ref1],
				                 x2 - x1);
		}
		LINK->curs[i] = x;
	...
	do {
		...
		if(point <= end_point) {
			...
			if(cur_touched == first_touched)
				Shift((Int)first_point, (Int)end_point,
				      (Int)cur_touched, &V);
			else {
				Interp((Int)(cur_touched + 1), (Int)(end_point),
				       (Int)(cur_touched), (Int)(first_touched),
				       &V);
...
#else
		/* igorm changed : allow phantom points
...
static struct {
	const char* sName;
	TInstruction_Function p;
} Instruct_Dispatch[256] = {
    /* Opcodes are gathered in groups of 16. */
    /* Please keep the spaces as they are.   */
...
RunIns(PExecution_Context exc)
{
	...
	if(dbg_prt && CUR.pts.n_points) {
		save_ox = mem->alloc_bytes(
		    mem, CUR.pts.n_points * sizeof(*save_ox), "RunIns");
		save_oy = mem->alloc_bytes(
		    mem, CUR.pts.n_points * sizeof(*save_oy), "RunIns");
		save_cx = mem->alloc_bytes(
		    mem, CUR.pts.n_points * sizeof(*save_cx), "RunIns");
		save_cy = mem->alloc_bytes(
		    mem, CUR.pts.n_points * sizeof(*save_cy), "RunIns");
	...
	do {
		...
		/* `args' is the top of the stack once arguments have been
		 * popped. */
		...
		/* `new_top' is the new top of the stack, after the
		 * instruction's */
...
#ifdef DEBUG
		DBG_PRINT3("\n%%n=%5d IP=%5d OP=%s            ", nInstrCount,
		           CUR.IP, Instruct_Dispatch[CUR.opcode].sName);
		/*
		...
		*/
		if(save_ox != NULL) {
			memcpy(save_ox, CUR.pts.org_x,
			       sizeof(CUR.pts.org_x[0]) * CUR.pts.n_points);
			memcpy(save_oy, CUR.pts.org_y,
			       sizeof(CUR.pts.org_y[0]) * CUR.pts.n_points);
			memcpy(save_cx, CUR.pts.cur_x,
			       sizeof(CUR.pts.cur_x[0]) * CUR.pts.n_points);
			memcpy(save_cy, CUR.pts.cur_y,
			       sizeof(CUR.pts.cur_y[0]) * CUR.pts.n_points);
		}
#endif

		Instruct_Dispatch[CUR.opcode].p(EXEC_ARGS &
		                                CUR.stack[CUR.args]);
		...
		if(save_ox != NULL) {
			...
			pp[0] = save_ox, pp[1] = save_oy, pp[2] = save_cx,
			pp[3] = save_cy;
			qq[0] = CUR.pts.org_x;
			qq[1] = CUR.pts.org_y;
			qq[2] = CUR.pts.cur_x;
		...
		if(CUR.error != TT_Err_Ok) {
			switch(CUR.error) {
			case TT_Err_Invalid_Opcode: /* looking for redefined
			                               instructions */
				A = 0;
