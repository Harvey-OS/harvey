...
flip3x2(byte* buffer, const byte** planes, int offset, int nbytes)
{
	...
	const byte* in3 = planes[2] + offset;
	int n = nbytes;
	static const bits32 tab3x2[256] = {VTAB(
	    0x800000, 0x400000, 0x20000, 0x10000, 0x800, 0x400, 0x20, 0x10)};
...
flip3x12(byte* buffer, const byte** planes, int offset, int nbytes)
{
	...
	for(; n > 0; out += 9, pa += 3, pb += 3, pc += 3, n -= 3) {
		byte_var a1 = pa[1], b0 = pb[0], b1 = pb[1], b2 = pb[2],
		         c1 = pc[1];
           ...
           int num_planes, int bits_per_sample)
{
	...
	int bi, pi;
	sample_store_declare_setup(dptr, dbit, dbbyte, buffer, 0,
	                           bits_per_sample);
	...
	for(bi = 0; bi < nbytes * 8; bi += bits_per_sample) {
		for(pi = 0; pi < num_planes; ++pi) {
			...
			uint value =
			    (*sptr >> (8 - (bi & 7) - bits_per_sample)) & mask;

			sample_store_next8(value, dptr, dbit, bits_per_sample,
			                   dbbyte);
		}
	}
         ...
         int num_planes, int ignore_bits_per_sample)
{
	...
	for(bi = 0; bi < nbytes * 8; bi += 12) {
		for(pi = 0; pi < num_planes; ++pi) {
			const byte* sptr = planes[pi] + offset + (bi >> 3);
			uint value = (bi & 4 ? ((*sptr & 0xf) << 8) | sptr[1]
			                     : (*sptr << 4) | (sptr[1] >> 4));
...
private
const image_flip_proc image_flip3_procs[13] = {
    flip_fail, flip3x1, flip3x2,   flip_fail, flip3x4,   flip_fail, flip_fail,
    flip_fail, flip3x8, flip_fail, flip_fail, flip_fail, flip3x12};
private
const image_flip_proc image_flip4_procs[13] = {
    flip_fail, flip4x1, flip4x2,   flip_fail, flip4x4,   flip_fail, flip_fail,
    flip_fail, flip4x8, flip_fail, flip_fail, flip_fail, flip4x12};
...
private
const image_flipN_proc image_flipN_procs[13] = {
    flipN_fail, flipNx1to8, flipNx1to8, flipN_fail, flipNx1to8,
    flipN_fail, flipN_fail, flipN_fail, flipNx1to8, flipN_fail,
    flipN_fail, flipN_fail, flipNx12};
                  ...
                  int num_planes, int bits_per_sample)
{
	...
	case 3:
		return image_flip3_procs[bits_per_sample](buffer, planes,
		                                          offset, nbytes);
	case 4:
		return image_flip4_procs[bits_per_sample](buffer, planes,
		                                          offset, nbytes);
	default:
		if(num_planes < 0)
			return -1;
		return image_flipN_procs[bits_per_sample](
		    buffer, planes, offset, nbytes, num_planes,
		    bits_per_sample);
	}
}
