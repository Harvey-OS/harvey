...
/* Device procedures */
private
dev_proc_open_device(pattern_accum_open);
private
dev_proc_close_device(pattern_accum_close);
private
dev_proc_fill_rectangle(pattern_accum_fill_rectangle);
private
dev_proc_copy_mono(pattern_accum_copy_mono);
private
dev_proc_copy_color(pattern_accum_copy_color);
private
dev_proc_get_bits_rectangle(pattern_accum_get_bits_rectangle);

/* The device descriptor */
private
const gx_device_pattern_accum gs_pattern_accum_device = {
    std_device_std_body_open(gx_device_pattern_accum, 0, "pattern accumulator",
                             0, 0, 72, 72),
    {/* NOTE: all drawing procedures must be defaulted, not forwarded. */
     pattern_accum_open, NULL, NULL, NULL, pattern_accum_close, NULL, NULL,
     pattern_accum_fill_rectangle, gx_default_tile_rectangle,
     pattern_accum_copy_mono, pattern_accum_copy_color, NULL,
     gx_default_get_bits, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL,
     gx_default_copy_alpha, NULL, gx_default_copy_rop, gx_default_fill_path,
     gx_default_stroke_path, gx_default_fill_mask, gx_default_fill_trapezoid,
     ...
     gx_default_strip_copy_rop, gx_get_largest_clipping_box,
     gx_default_begin_typed_image, pattern_accum_get_bits_rectangle, NULL,
     gx_default_create_compositor, NULL, gx_default_text_begin,
    ...
    0,
    0,
    0 /* bitmap_memory, bits, mask, instance */
};
...
{
	gx_device_pattern_accum* adev = gs_alloc_struct(
	    mem, gx_device_pattern_accum, &st_device_pattern_accum, cname);

	if(adev == 0)
		return 0;
	gx_device_init((gx_device*)adev,
	               (const gx_device*)&gs_pattern_accum_device, mem, true);
...
pattern_accum_open(gx_device* dev)
{
	 ...
	 */
	gx_device* target = (padev->target == 0 ? gs_currentdevice(pinst->saved)
	                                        : padev->target);
...
#define PDSET(dev)                                                             \
	((dev)->width = width,                                                 \
	 (dev)->height =                                                       \
	     height, /*(dev)->HWResolution = target->HWResolution*/            \
...
#undef PDSET
				...
				code = (*dev_proc(bits, open_device))(
				    (gx_device*)bits);
                             ...
                             gx_color_index color)
{
	...
	if(padev->bits)
		(*dev_proc(padev->target, fill_rectangle))(padev->target, x, y,
		                                           w, h, color);
	if(padev->mask)
                        ...
                        gx_color_index color0, gx_color_index color1)
{
	...
	if(color0 == gx_no_color_index && color1 == gx_no_color_index)
		return 0;
	if(padev->bits)
		(*dev_proc(padev->target, copy_mono))(padev->target, data,
		                                      data_x, raster, id, x, y,
		                                      w, h, color0, color1);
	if(padev->mask) {
		...
		if(color0 == 1 && color1 == 1)
			return (*dev_proc(padev->mask, fill_rectangle))(
			    (gx_device*)padev->mask, x, y, w, h,
			    (gx_color_index)1);
		else
			return (*dev_proc(padev->mask, copy_mono))(
			    (gx_device*)padev->mask, data, data_x, raster, id,
...
pattern_accum_copy_color(gx_device* dev, const byte* data, int data_x,
                         int raster, gx_bitmap_id id, int x, int y, int w,
                         int h)
{
	gx_device_pattern_accum* const padev = (gx_device_pattern_accum*)dev;
...
gx_pattern_alloc_cache(gs_memory_t* mem, uint num_tiles, uint32_t max_bits)
{
	...
	gx_color_tile* tiles = gs_alloc_struct_array(
	    mem, num_tiles, gx_color_tile, &st_color_tile_element,
...
ensure_pattern_cache(gs_imager_state* pis)
{
	if(pis->pattern_cache == 0) {
		gx_pattern_cache* pcache = gx_pattern_alloc_cache(
		    pis->memory, gx_pat_cache_default_tiles(),
 ...
 */
private
void make_bitmap(gx_strip_bitmap*, const gx_device_memory*, gx_bitmap_id);
int
gx_pattern_cache_add_entry(gs_imager_state* pis, gx_device_pattern_accum* padev,
                           gx_color_tile** pctile)
{
	...
	while(pcache->bits_used + used > pcache->max_bits &&
		...
		gx_pattern_cache_free_entry(pcache,
		                            &pcache->tiles[pcache->next]);
	}
	ctile->id = id;
...
gx_pattern_cache_winnow(gx_pattern_cache* pcache,
                        bool (*proc)(gx_color_tile* ctile, void* proc_data),
                        void* proc_data)
{
	uint i;
                ...
                gx_device* dev, gs_color_select_t select)
{
	...
	if(code >= 0) {
		if(!gx_pattern_cache_lookup(pdc, pis, dev, select)) {
			lprintf(
			    "Pattern cache lookup failed after insertion!\n");
	...
	if(gs_debug_c('B')) {
		...
		if(adev->bits)
			debug_dump_bitmap(
			    ((gx_device_memory*)adev->target)->base,
