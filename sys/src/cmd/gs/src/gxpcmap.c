...
#else
	return (gs_debug_c('.') ? max_cached_patterns_SMALL : max_cached_patterns_LARGE);
#endif
}
uint32_t
...
#else
	return (gs_debug_c('.') ? max_pattern_bits_SMALL : max_pattern_bits_LARGE);
#endif
}
...
pattern_accum_open(gx_device *dev)
{
     ...
     */
	gx_device *target =
	    (padev->target == 0 ? gs_currentdevice(pinst->saved) : padev->target);
...
/*
     ...
     */
#define PDSET(dev)                                                                                    \
	((dev)->width = width, (dev)->height = height, /*(dev)->HWResolution = target->HWResolution*/ \
			     ...
			     gx_color_index color)
{
	...
	if(padev->bits)
		(*dev_proc(padev->target, fill_rectangle))(padev->target, x, y, w, h, color);
	if(padev->mask)
		return (*dev_proc(padev->mask, fill_rectangle))((gx_device *)padev->mask, x, y, w, h, (gx_color_index)1);
	else
		return 0;
}
			...
			gx_color_index color0, gx_color_index color1)
{
	...
	if(color0 == gx_no_color_index && color1 == gx_no_color_index)
		return 0;
	if(padev->bits)
		(*dev_proc(padev->target, copy_mono))(padev->target, data, data_x, raster, id, x, y, w, h,
						      color0, color1);
	if(padev->mask) {
		...
		if(color0 == 1 && color1 == 1)
			...
			return (*dev_proc(padev->mask, copy_mono))((gx_device *)padev->mask, data, data_x, raster, id, x, y, w, h,
								   color0, color1);
	} else
		return 0;
}
			 ...
			 int raster, gx_bitmap_id id, int x, int y, int w, int h)
{
	...
	if(padev->bits)
		(*dev_proc(padev->target, copy_color))(padev->target, data, data_x, raster, id, x, y, w, h);
	if(padev->mask)
		return (*dev_proc(padev->mask, fill_rectangle))((gx_device *)padev->mask, x, y, w, h, (gx_color_index)1);
	else
		return 0;
				 ...
				 gs_get_bits_params_t *params, gs_int_rect **unread)
{
	...
	if(padev->bits)
		return (*dev_proc(padev->target, get_bits_rectangle))(padev->target, prect, params, unread);
			...
			gx_device *dev, gs_color_select_t select)
{
	...
	if(pinst->template.PaintType == 2) { /* uncolored */
		code = (*pcs->params.pattern.base_space.type->remap_color)(pc, (const gs_color_space *)&pcs->params.pattern.base_space,
									   pdc, pis, dev, select);
		if(code < 0)
			return code;
