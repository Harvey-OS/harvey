...
/* Copyright (C) 1990, 1996, 1997, 1998, 1999 Aladdin Enterprises.  All rights
  reserved.
...
/* Forward references */
private
int upath_append(os_ptr, i_ctx_t*);
private
int upath_stroke(i_ctx_t*, gs_matrix*);

/* ---------------- Insideness testing ---------------- */

/* Forward references */
private
int in_test(i_ctx_t*, int (*)(gs_state*));
private
int in_path(os_ptr, i_ctx_t*, gx_device*);
private
int in_path_result(i_ctx_t*, int, int);
private
int in_utest(i_ctx_t*, int (*)(gs_state*));
private
int in_upath(i_ctx_t*, gx_device*);
private
int in_upath_result(i_ctx_t*, int, int);
...
static const byte up_nargs[UPATH_MAX_OP + 1] = {4, 2, 2, 2, 2, 6,
                                                6, 5, 5, 5, 0, 0};
...
static const op_proc_t up_ops[UPATH_MAX_OP + 1] = {
    zsetbbox,  zmoveto, zrmoveto, zlineto, zrlineto,   zcurveto,
    zrcurveto, zarc,    zarcn,    zarct,   zclosepath, zucache};
           ...
           bool with_ucache)
{
	...
	code =
	    ialloc_ref_array(rupath, a_all | a_executable, size, "make_upath");
	if(code < 0)
		return code;
	/* Construct the path. */
	...
	}
	{
		...
		if((code = gs_upathbbox(pgs, &bbox, true)) < 0) {
			/*
			 * Note: Adobe throws 'nocurrentpoint' error, but the
			 * PLRM
	...
	}
	{
		...
		while((op = gs_path_enum_next(&penum, pts)) != 0) {
			...
			if((code = name_enter_string(pgs->memory, opstr,
			                             next)) < 0)
				return code;
...
upath_append_aux(os_ptr oppath, i_ctx_t* i_ctx_p)
{
	...
	if(r_size(oppath) == 2 &&
	   ...
	   r_has_type(
	       &opcodes,
	       t_string)) { /* 1st element is operands, 2nd is operators */
		ref operands;
		int code, format;
		int repcount = 1;
		const byte* opp;
		...
		for(; index < ocount; index++) {
			...
			case t_operator:
				defp = &rup;
			xop:
				if(!r_has_attr(defp, a_executable))
					return_error(e_typecheck);
...
const op_def zupath_l2_op_defs[] = {op_def_begin_level2(),
                                    /* Insideness testing */
                                    {"1ineofill", zineofill},
                                    {"1infill", zinfill},
                                    {"1instroke", zinstroke},
