 ...
 */
#define _frac2s(fr) (((fr) >> (frac_bits - frac_1_0bits)) + (fr))
#define frac2bits(fr, nb) ((uint)(_frac2s(fr) >> (frac_bits - (nb))))
#define frac2byte(fr) ((byte)frac2bits(fr, 8))
/* bits2frac requires frac_bits / 2 <= nb <= frac_bits. */
#define bits2frac(v, nb)                                                       \
	((frac)(((frac)(v) << (frac_bits - (nb))) +                            \
	        ((v) >> ((nb)*2 - frac_bits)) - ((v) >> ((nb)-frac_1_0bits))))
...
#define frac2ushort(fr)                                                        \
	((ushort)(((fr) << (ushort_bits - frac_bits)) +                        \
	          ((fr) >> (frac_bits * 2 - ushort_bits - frac_1_0bits))))
#define ushort2frac(us)                                                        \
	((frac)(((us) >> (ushort_bits - frac_bits)) -                          \
 ...
 */
#define frac_1_quo(p)                                                          \
	((((p) >> frac_1_0bits) + ((p) >> frac_bits) + 1) >>                   \
	 (frac_bits - frac_1_0bits))
/*
