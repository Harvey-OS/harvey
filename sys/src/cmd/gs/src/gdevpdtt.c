	...
	if(penum->current_font->FontType == ft_user_defined &&
	   ...
	   !(penum->pte_default->text.operation & TEXT_DO_CHARWIDTH)) {
		...
		if(glyph != GS_NO_GLYPH && ch != GS_NO_CHAR) {
			...
			double pw1[10];
			int narg = (control == TEXT_SET_CHAR_WIDTH ? 2 : control == TEXT_SET_CACHE_DEVICE ? 6 : 10), i;
		    ...
		    gs_memory_t *mem, gs_text_enum_t **ppte)
{
	 ...
	 */
		...
		if(code < 0)
			return code;
	} else if((!(text->operation & TEXT_DO_DRAW) && pis->text_rendering_mode != 3) || path == 0 || gx_path_current_point(path, &cpt) < 0)
		      ...
		      pdf_font_resource_t *pdfont, const gs_matrix *pfmat)
{
	...
	ppts->values.render_mode = (penum->pis->text_rendering_mode == 3 ? 3 : font->PaintType == 0 ? 0 : 1);
...
{
	return fabs((font->FontMatrix.yy != 0 ? font->FontMatrix.yy : font->FontMatrix.yx));
}
int
		 ...
		 const double cdevproc_result[10])
{
	...
	int code, rcode = 0;
	gs_point v;
	int allow_cdevproc_callout = (orig_font->FontType == ft_CID_TrueType || orig_font->FontType == ft_CID_encrypted
					  ? GLYPH_INFO_CDEVPROC
...
top:
	...
	}
	{
		...
		case ft_composite:
			process =
			    (((gs_font_type0 *)font)->data.FMapType == fmap_CMap ? process_cmap_text : process_composite_text);
			break;
		default:
			goto skip;
		}
