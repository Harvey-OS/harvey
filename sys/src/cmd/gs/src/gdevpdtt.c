                   ...
                   gs_text_cache_control_t control)
{
	...
	case TEXT_SET_CACHE_DEVICE:
		gs_distance_transform(pw[0], pw[1], &ctm_only(pte->pis),
		                      &pdev->char_width);
		break;
	case TEXT_SET_CACHE_DEVICE2:
		...
		if(penum->cdevproc_callout) {
			memcpy(penum->cdevproc_result, pw,
			       sizeof(penum->cdevproc_result));
			return 0;
		}
		break;
	default:
	...
	if(penum->current_font->FontType == ft_user_defined &&
	   ...
	   !(penum->pte_default->text.operation & TEXT_DO_CHARWIDTH)) {
		...
		if(penum->text.operation & TEXT_FROM_SINGLE_GLYPH) {
			...
			code = pdf_encode_glyph((gs_font_base*)font, glyph, buf,
			                        sizeof(buf), &char_code_length);
			if(code < 0) {
				/* Must not happen, becuse pdf_encode_glyph was
				 * passed in process_plain_text.*/
				ch = GS_NO_CHAR;
			 ...
			 * a notdef glyph, but we don't know how to do with Type
			 * 3 fonts.
			 */
		}
		if(glyph != GS_NO_GLYPH && ch != GS_NO_CHAR) {
			...
			double pw1[10];
			int narg =
			        (control == TEXT_SET_CHAR_WIDTH
			             ? 2
			             : control == TEXT_SET_CACHE_DEVICE ? 6
			                                                : 10),
			    i;
			...
			if(penum->pte_default == NULL)
				return_error(
				    gs_error_unregistered); /* Must not happen.
				                               */
			/* Check to verify the structure type is really
			 * gs_show_enum */
			if(gs_object_type(penum->pte_default->memory,
			                  penum->pte_default) !=
			   &st_gs_show_enum) {
			...
			if(code < 0)
				return_error(
				    gs_error_unregistered); /* Must not happen.
				                               */
			/* BuildChar could change the scale before calling
			   setcachedevice (Bug 687290).
			   We must scale the setcachedevice arguments because we
			   assumed
			...
			*/
			for(i = 0; i < narg; i += 2) {
				...
				gs_point_transform(pw[i], pw[i + 1],
				                   &ctm_only(penum_s->pgs), &p);
				pw1[i] = p.x;
				pw1[i + 1] = p.y;
			}
			if(control != TEXT_SET_CHAR_WIDTH) {
				...
				/*
				 * We have no character bbox, but we need one to
				 * install the clipping
				 * to the graphic state of the PS interpreter.
				 * Since some fonts don't
				 * provide a proper FontBBox (Bug 687239
				 * supplies a zero one),
				 * we set an "infinite" clipping here.
				 * We also detected that min_int, max_int don't
				 * work here with
				 * comparefiles/Bug687044.ps, therefore we
				 * divide them by 2.
				 */
			...
			if(code < 0)
				return code;
			code =
			    pdf_set_charproc_attrs(pdev, pte->current_font, pw1,
			                           narg, control, ch, &gnstr);
			if(code < 0)
				return code;
			/* Prevent writing the clipping path to charproc.
			   See the comment above and bugs 687678, 688327.
			   Note that the clipping in the graphic state will be
			   used while
			   fallbacks to default implementations of graphic
			   objects.
			...
			gs_matrix m;
			pdf_resource_t* pres =
			    pdev->accumulating_substream_resource;

			/* pdf_text_process started a charproc stream
			   accumulation,
			   but now we re-decided to go with the default
			   implementation.
			...
			if(code < 0)
				return code;
			code =
			    pdf_cancel_resource(pdev, pres, resourceCharProc);
			if(code < 0)
				return code;
			...
			gs_matrix_multiply((gs_matrix*)&pdev->charproc_ctm,
			                   (gs_matrix*)&penum->pis->ctm, &m);
			gs_matrix_fixed_from_matrix(&penum->pis->ctm, &m);
		}
	}
	if(penum->pte_default) {
		if(penum->pte_default->text.operation &
		   TEXT_DO_CHARWIDTH /* See process_cmap_text.*/)
			return gs_text_set_cache(penum->pte_default, pw,
			                         TEXT_SET_CHAR_WIDTH);
		else
			return gs_text_set_cache(penum->pte_default, pw,
			                         control);
...
private
const gs_text_enum_procs_t pdf_text_procs = {
    pdf_text_resync,        pdf_text_process,   pdf_text_is_width_only,
    pdf_text_current_width, pdf_text_set_cache, pdf_text_retry,
    pdf_text_release};
...
pdf_prepare_text_drawing(gx_device_pdf* const pdev, gs_text_enum_t* pte)
{
	...
	if(text->operation & TEXT_DO_DRAW) {
		/*
		 * Set the clipping path and drawing color.  We set both the
		 * fill
		 * and stroke color, because we don't know whether the fonts
		 * will be
		 * filled or stroked, and we can't set a color while we are in
		 * text
		...
		if((code = pdf_set_drawing_color(
		        pdev, pis, pdcolor, &pdev->saved_stroke_color,
		        &pdev->stroke_used_process_color,
		        &psdf_set_stroke_color_commands)) < 0 ||
		   (code = pdf_set_drawing_color(
		        pdev, pis, pdcolor, &pdev->saved_fill_color,
...
gdev_pdf_text_begin(gx_device* dev, gs_imager_state* pis,
                    const gs_text_params_t* text, gs_font* font, gx_path* path0,
                    ...
                    gs_memory_t* mem, gs_text_enum_t** ppte)
{
	...
	if(font->FontType == ft_user_defined &&
	   ...
	   (text->operation & TEXT_RETURN_WIDTH)) {
		...
		if(code < 0)
			return code;
	} else if((!(text->operation & TEXT_DO_DRAW) &&
	           pis->text_rendering_mode != 3) ||
	          path == 0 || gx_path_current_point(path, &cpt) < 0)
		return gx_default_text_begin(dev, pis, text, font, path,
		                             pdcolor, pcpath, mem, ppte);
	...
	penum->cgp = NULL;
	code = gs_text_enum_init((gs_text_enum_t*)penum, &pdf_text_procs, dev,
	                         pis, text, font, path, pdcolor, pcpath, mem);
	if(code < 0) {
	...
	if(pdev->font3 != 0) {
		/* A text operation happens while accumulating a charproc.
		   ...
		   the font isn't converting to a raster (i.e. to a bitmap
		   font).
		   Disable the grid fitting for the convertion to get a proper
		   outlines,
		   because the viewer resolution is not known during the
		   accumulation.
		   Note we set identity CTM in pdf_text_set_cache for the
		   accumilation,
		   and therefore the font may look too small while the source
		   charproc
		   interpretation. The document tpc2.ps of the bug 687087 is an
		   example.
		*/
...
private
void
font_cache_elem_array_sizes(gx_device_pdf* pdev, gs_font* font, int* num_widths,
                            int* num_chars)
{
	...
	case ft_Chameleon:
	case ft_TrueType:
		*num_widths = *num_chars =
		    256; /* Assuming access to glyph_usage by character codes */
		break;
	case ft_CID_encrypted:
		*num_widths = *num_chars =
		    ((gs_font_cid0*)font)->cidata.common.CIDCount;
		break;
	case ft_CID_TrueType:
		*num_widths = *num_chars =
		    ((gs_font_cid2*)font)->cidata.common.CIDCount;
		break;
	default:
		*num_widths = *num_chars =
		    65536; /* No chance to determine, use max. */
	}
}
                             ...
                             gs_font* font)
{
	...
	len = (num_chars + 7) / 8;
	e->glyph_usage = gs_alloc_bytes(pdev->pdf_memory, len,
	                                "alloc_font_cache_elem_arrays");

	e->real_widths =
	    (num_widths > 0
	         ? (double*)gs_alloc_bytes(
	               pdev->pdf_memory,
	...
	if(e->glyph_usage == NULL ||
	   (num_widths != 0 && e->real_widths == NULL)) {
                         ...
                         pdf_font_resource_t* pdfont)
{
	...
	pdf_font_cache_elem_t* e,
	    ** pe = pdf_locate_font_cache_elem(pdev, font);
	...
	if(pe != NULL) {
		...
		int code;
		e = (pdf_font_cache_elem_t*)gs_alloc_struct(
		    pdev->pdf_memory, pdf_font_cache_elem_t,
		    &st_pdf_font_cache_elem, "pdf_attach_font_resource");
		if(e == NULL)
			return_error(gs_error_VMerror);
		e->pdfont = pdfont;
		...
		pdev->font_cache = e;
		code = gs_notify_register(&font->notify_list,
		                          pdf_notify_remove_font, e);
		if(code < 0)
			return code;
	}
	return 0;
...
pdf_font_orig_matrix(const gs_font* font, gs_matrix* pmat)
{
	...
	case ft_TrueType:
	case ft_CID_TrueType:
		/* The TrueType FontMatrix is 1 unit per em, which is what we
		 * want. */
	...
	case ft_user_defined:
		 ...
		 *
		 * The old code used a heuristic to detect and correct for this
		 *here.
		 ...
		 * have similar problem. Therefore we added a support to
		 *lib/gs_fonts.ps,
		 * src/zbfont.c, src/gsfont.c that provides an acces to the
		 *original
		 * font via a special key .OrigFont added to the font dictionary
		 *while definefont.
		 ...
		 */
		{
			...
			if(font->FontType == ft_user_defined)
			...
			else if(base_font->orig_FontMatrix.xx != 0 ||
			        base_font->orig_FontMatrix.xy != 0 ||
			        base_font->orig_FontMatrix.yx != 0 ||
			        base_font->orig_FontMatrix.yy != 0)
				...
				*/
				if(base_font->FontMatrix.xx == 1.0 / 2048 &&
				   ...
				   any_abs(base_font->FontMatrix.yy) ==
				       1.0 / 2048)
					*pmat = base_font->FontMatrix;
				else
...
pdf_check_encoding_compatibility(const pdf_font_resource_t* pdfont,
                                 const pdf_char_glyph_pair_t* pairs,
                                 int num_chars)
{
	int i;
...
pdf_is_compatible_encoding(gx_device_pdf* pdev, pdf_font_resource_t* pdfont,
                           gs_font* font, const pdf_char_glyph_pair_t* pairs,
                           int num_chars)
{
	/*
	...
	case ft_composite: { /*
	                      * We assume that source document don't redefine
	                      * CMap
	                      * resources and that incremental CMaps do not
	                      * exist.
	                      * Therefore we don't maintain stable CMap copies,
	                      * but just compare CMap names for equality.
	                      * A better implementation should compare the
	                      * chars->glyphs
	...
	case ft_encrypted2:
	case ft_TrueType:
		return pdf_check_encoding_compatibility(pdfont, pairs,
		                                        num_chars);
	case ft_CID_encrypted:
	case ft_CID_TrueType: {
		gs_font* font1 =
		    (gs_font*)pdf_font_resource_font(pdfont, false);
...
pdf_find_font_resource(gx_device_pdf* pdev, gs_font* font,
                       pdf_resource_type_t type, pdf_font_resource_t** ppdfont,
	...
	for(i = 0; i < NUM_RESOURCE_CHAINS; i++) {
		for(pres = pchain[i]; pres != 0; pres = pres->next) {
			...
			if(pdfont->FontType == ft_composite) {
				...
				ofont =
				    font0->data.FDepVector
				        [0]; /* See pdf_make_font_resource. */
				cfont = pdf_font_resource_font(
				    pdfont->u.type0.DescendantFont, false);
				if(font0->data.CMap->WMode !=
				   pdfont->u.type0.WMode)
					continue;
			} else
				cfont = pdf_font_resource_font(pdfont, false);
			if(!pdf_is_CID_font(ofont) &&
			   !pdf_is_compatible_encoding(
			       pdev, pdfont, font, cgp->s, cgp->num_all_chars))
				continue;
			if(cfont == 0)
				continue;
			code = gs_copied_can_copy_glyphs(
			    (const gs_font*)cfont, ofont,
			    &cgp->s[cgp->unused_offset].glyph,
			    cgp->num_unused_chars,
			    sizeof(pdf_char_glyph_pair_t), true);
			if(code ==
			   gs_error_unregistered) /* Debug purpose only. */
				return code;
			if(code > 0) {
				*ppdfont = pdfont;
				return 1;
...
private
int
pdf_find_type0_font_resource(gx_device_pdf* pdev,
                             const pdf_font_resource_t* pdsubf,
	...
	for(i = 0; i < NUM_RESOURCE_CHAINS; i++) {
		for(pres = pchain[i]; pres != 0; pres = pres->next) {
			...
			if(pdfont->BaseFont.size !=
			   pdsubf->BaseFont.size + CMapName->size + 1)
				continue;
			if(memcmp(pdfont->BaseFont.data +
			              pdsubf->BaseFont.size + 1,
                            ...
                            pdf_char_glyph_pairs_t* cgp)
{
	...
	if(*ppdsubf != NULL) {
		const gs_font_base* cfont =
		    pdf_font_resource_font(*ppdsubf, false);

		code = gs_copied_can_copy_glyphs(
		    (const gs_font*)cfont, subfont,
	...
	code = pdf_find_font_resource(pdev, subfont, resourceCIDFont, ppdsubf,
	                              cgp);
	if(code < 0)
		return code;
	if(*ppdsubf == NULL) {
...
private
int
pdf_refine_encoding_index(const gx_device_pdf* pdev, int index,
                          bool is_standard)
{
	if(pdev->ForOPDFRead) {
		/*
                        ...
                        pdf_font_resource_t** ppdfont)
{
	...
	cached = gs_alloc_bytes(pdev->pdf_memory, 256 / 8,
	                        "pdf_make_font3_resource");
	if(cached == NULL)
		return_error(gs_error_VMerror);
	...
	if(code < 0) {
		gs_free_object(pdev->pdf_memory, cached,
		               "pdf_make_font3_resource");
		return code;
	}
	...
	pdfont->u.simple.BaseEncoding = pdf_refine_encoding_index(
	    pdev, bfont->nearest_encoding_index, true);
                       ...
                       pdf_char_glyph_pairs_t* cgp)
{
	...
	int (*font_alloc)(gx_device_pdf*, pdf_font_resource_t**, gs_id,
	                  pdf_font_descriptor_t*);
	gs_font* base_font =
	    font; /* A roudiment from old code. Keep it for a while. */
	pdf_font_resource_t* pdfont;
	...
	embed = pdf_font_embed_status(pdev, base_font, &index, cgp->s,
	                              cgp->num_all_chars);
	if(embed == FONT_EMBED_STANDARD) {
		pdf_standard_font_t* psf = &psfa[index];

		if(psf->pdfont == NULL ||
		   !pdf_is_compatible_encoding(pdev, psf->pdfont, font, cgp->s,
		                               cgp->num_all_chars)) {
			code = pdf_font_std_alloc(
			    pdev, ppdfont, (psf->pdfont == NULL), base_font->id,
			    (gs_font_base*)base_font, index);
			if(code < 0)
				return code;
			if(psf->pdfont == NULL)
				psf->pdfont = *ppdfont;
			(*ppdfont)->u.simple.BaseEncoding =
			    pdf_refine_encoding_index(
			        pdev, ((const gs_font_base*)base_font)
			                  ->nearest_encoding_index,
			        true);
			code = 1;
		} else
			*ppdfont = psf->pdfont;
	...
	if(embed == FONT_EMBED_YES) {
		...
		if(font->FontType == ft_TrueType &&
		   pdev->CompatibilityLevel <= 1.2) {
			...
			for(i = 0; i <= 0xff; ++i) {
				gs_glyph glyph = font->procs.encode_char(
				    font, (gs_char)i, GLYPH_SPACE_INDEX);
	...
	if(font->FontType == ft_encrypted || font->FontType == ft_encrypted2 ||
	   font->FontType == ft_TrueType) {
		 ...
		 * PDF spec says that "symbolic font should not specify Encoding
		 * entry"
		 * (see section 5.5, the article "Encodings for True Type
		 * fonts", paragraph 3).
		 * However Acrobat Reader 4,5,6 fail when TT font with no
		 * Encoding
		 * appears in a document together with a CID font with a
		 * non-standard CMap
		 * (AR 4 and 5 claim "The encoding (CMap) specified by a font is
		 * corrupted."
		 * (we read it as "The encoding or CMap specified by a font is
		 * corrupted.",
		 * and apply the 1st alternative)). We believe that AR is buggy,
		 * and therefore we write an Encoding with non-CID True Type
		 * fonts.
		 * Hopely other viewers can ignore Encoding in such case.
	...
	if((code =
	        pdf_font_descriptor_alloc(pdev, &pfd, (gs_font_base*)base_font,
...
{
	return (
	    font->FontType == ft_encrypted || font->FontType == ft_encrypted2 ||
	    font->FontType == ft_TrueType || font->FontType == ft_user_defined);
}
                    ...
                    gs_char* char_code, gs_char* cid, gs_glyph* glyph)
{
	...
	if(font_is_simple) {
		*cid = *char_code;
		*glyph =
		    font->procs.encode_char(font, *char_code, GLYPH_SPACE_NAME);
		if(*glyph == GS_NO_GLYPH)
			return 3;
	} else {
...
private
void
store_glyphs(pdf_char_glyph_pairs_t* cgp, byte* glyph_usage,
             int char_cache_size, gs_char char_code, gs_char cid,
             gs_glyph glyph)
{
	int j;
	...
	if(glyph_usage == 0 || !(glyph_usage[cid / 8] & (0x80 >> (cid & 7)))) {
		cgp->s[cgp->unused_offset + cgp->num_unused_chars].glyph =
		    glyph;
		cgp->s[cgp->unused_offset + cgp->num_unused_chars].chr =
		    char_code;
		cgp->num_unused_chars++;
	}
...
private
int
pdf_alloc_text_glyphs_table(gx_device_pdf* pdev, pdf_text_enum_t* penum,
                            const gs_string* pstr)
{
	...
	pdf_char_glyph_pairs_t* cgp = (pdf_char_glyph_pairs_t*)gs_alloc_bytes(
	    penum->memory, struct_size, "pdf_alloc_text_glyphs_table");
	if(cgp == NULL)
		return_error(gs_error_VMerror);
	penum->cgp = cgp;
                           ...
                           byte* glyph_usage, int char_cache_size)
{
	...
	if(pstr != NULL) {
		...
		if(scan.text.operation & TEXT_FROM_CHARS)
			scan.text.operation =
			    ((scan.text.operation & ~TEXT_FROM_CHARS) |
			     TEXT_FROM_STRING);
	}
	for(;;) {
		...
		if(glyph_usage != 0 && cid > char_cache_size)
			continue;
		store_glyphs(cgp, glyph_usage, char_cache_size, char_code, cid,
		             glyph);
	}
	return 0;
}
...
private
int
pdf_make_text_glyphs_table_unencoded(pdf_char_glyph_pairs_t* cgp, gs_font* font,
                                     const gs_string* pstr,
                                     ...
                                     int* ps_encoding_index)
{
	...
	gs_glyph* gid =
	    (gs_glyph*)pstr->data; /* pdf_text_process allocs enough space. */
	...
	for(ei = 0; gs_c_known_encodings[ei]; ei++) {
		...
		for(i = 0; i < pstr->size; i++) {
			...
			if(ch == GS_NO_CHAR)
				break;
			/* pstr->data[i] = (byte)ch; Can't do because pstr->data
			   and gid
                                 ...
                                 pdf_char_glyph_pairs_t* cgp)
{
	...
	if(*ppdfont == 0) {
		...
		if(base_font != font)
			same_encoding =
			    ((base_font->procs.same_font(base_font, font,
			                                 FONT_SAME_ENCODING) &
			      FONT_SAME_ENCODING) != 0);
		/* Find or make font resource. */
		pdf_attached_font_resource(pdev, base_font, ppdfont, NULL, NULL,
		                           NULL, NULL);
		...
		if(*ppdfont == NULL || *ppdfont == pdfont_not_allowed) {
			pdf_resource_type_t type =
			    (pdf_is_CID_font(base_font) ? resourceCIDFont
			                                : resourceFont);
			*ppdfont = NULL;
			code = pdf_find_font_resource(pdev, base_font, type,
			                              ppdfont, cgp);
			if(code < 0)
				return code;
			if(*ppdfont == NULL) {
				code = pdf_make_font_resource(pdev, base_font,
				                              ppdfont, cgp);
				if(code < 0)
					return code;
			}
			if(base_font != font && same_encoding) {
				code = pdf_attach_font_resource(pdev, base_font,
				                                *ppdfont);
				if(code < 0)
					return code;
			}
                     ...
                     byte* glyph_usage, int char_cache_size)
{
	...
	for(;;) {
		int code =
		    pdf_next_char_glyph(&scan, pstr, font, font_is_simple,
		                        &char_code, &cid, &glyph);
...
private
int
pdf_mark_text_glyphs_unencoded(const gs_text_enum_t* penum,
                               const gs_string* pstr, byte* glyph_usage,
                               int char_cache_size)
{
	int i;
 ...
 */
int
pdf_obtain_font_resource(pdf_text_enum_t* penum, const gs_string* pstr,
                         pdf_font_resource_t** ppdfont)
{
	...
	if(font->FontType == ft_composite) {
		/* Must not happen, because we always split composite fonts into
		 * descendents. */
		return_error(gs_error_unregistered);
	}
	code = pdf_attached_font_resource(pdev, font, ppdfont, &glyph_usage,
	                                  &real_widths, &char_cache_size,
	                                  &width_cache_size);
	...
	if(penum->cgp == NULL) {
		...
		if(code < 0)
			return code;
		code = pdf_make_text_glyphs_table(penum, pstr, glyph_usage,
		                                  char_cache_size);
		if(code < 0)
			return code;
	}
	code =
	    pdf_obtain_font_resource_encoded(pdev, font, ppdfont, penum->cgp);
	if(code < 0)
		return code;
	code = pdf_attached_font_resource(pdev, font, ppdfont, &glyph_usage,
	                                  &real_widths, &char_cache_size,
	...
	if(code < 0)
		return code;
	return pdf_mark_text_glyphs((const gs_text_enum_t*)penum, pstr,
                                   ...
                                   const gs_glyph* gdata)
{
	...
	if(penum->cgp == NULL) {
		...
		if(code < 0)
			return code;
		code = pdf_make_text_glyphs_table_unencoded(
		    penum->cgp, font, pstr, gdata, &ps_encoding_index);
		if(code < 0)
			return code;
	}
	code =
	...
	if(code < 0)
		return code;
	return pdf_mark_text_glyphs_unencoded(
	    (const gs_text_enum_t*)penum, pstr, glyph_usage, char_cache_size);
}
 ...
 */
int
pdf_obtain_parent_type0_font_resource(gx_device_pdf* pdev,
                                      ...
                                      pdf_font_resource_t** pdfont)
{
	if(pdsubf->u.cidfont.parent != 0 &&
		...
		*pdfont = pdsubf->u.cidfont.parent;
		 ...
		 *
		 * PDF 1.2 introduces a general architecture for composite fonts
		 *that theoretically
		 * allows a Type 0 font to have multiple descendants,which might
		 *themselves be
		 * Type 0 fonts.However,in versions up to and including PDF
		 *1.4,only a single
		 * descendant is allowed,which must be a CIDFont (not a
		 *font).This restriction
		 * may be relaxed in a future PDF version.
		 */

		if(pdsubf->u.cidfont.parent == NULL ||
		   pdf_find_type0_font_resource(pdev, pdsubf, CMapName,
		                                pdfont) <= 0) {
			int code = pdf_font_type0_alloc(pdev, pdfont, gs_no_id,
			                                pdsubf, CMapName);
...
{
	int code =
	    gs_distance_transform_inverse(pdelta->x, pdelta->y, pmat, ppt);
	gs_point delta;
	...
	if(code < 0)
		return 0; /* punt */
	if(fabs(delta.x - pdelta->x) < 0.01 &&
	   fabs(delta.y - pdelta->y) < 0.01) {
...
pdf_update_text_state(pdf_text_process_state_t* ppts,
                      const pdf_text_enum_t* penum, pdf_font_resource_t* pdfont,
                      const gs_matrix* pfmat)
{
	...
	if(penum->text.operation & TEXT_ADD_TO_ALL_WIDTHS) {
		if(penum->current_font->WMode == 0) {
			...
			code = transform_delta_inverse(&penum->text.delta_all,
			                               &smat, &pt);
			if(code >= 0 && pt.y == 0)
				c_s = pt.x * size;
			else
	...
	if(penum->text.operation & TEXT_ADD_TO_SPACE_WIDTH) {
		...
		code = transform_delta_inverse(&penum->text.delta_space, &smat,
		                               &pt);
	...
	ppts->values.render_mode = (penum->pis->text_rendering_mode == 3
	                                ? 3
                           ...
                           pdf_text_process_state_t* ppts)
{
	...
	if(pdf_render_mode_uses_stroke(pdev, &ppts->values)) {
		...
		/* Note that we compute pis->line_params.half_width in device
		 * space,
		...
		scaled_width *=
		    min(hypot(pte->pis->ctm.xx, pte->pis->ctm.yx) /
		...
		if(code >= 0) {
			/*
			 * See stream_to_text in gdevpdfu.c re the computation
			 * of
			 * the scaling value.
			 */
			double scale = 72.0 / pdev->HWResolution[1];

			code = gdev_vector_prepare_stroke(
			    (gx_device_vector*)pdev, pis, NULL, NULL, scale);
		}
                 ...
                 const double cdevproc_result[10])
{
	...
	int code, rcode = 0;
	gs_point v;
	int allow_cdevproc_callout =
	    (orig_font->FontType == ft_CID_TrueType ||
	             orig_font->FontType == ft_CID_encrypted
	         ? GLYPH_INFO_CDEVPROC
	         : 0); /* fixme : allow more font types. */
	...
	code = cfont->procs.glyph_info(
	    (gs_font*)cfont, glyph, NULL,
	    GLYPH_INFO_WIDTH0 | (GLYPH_INFO_WIDTH0 << wmode) |
	        GLYPH_INFO_OUTLINE_WIDTHS | (GLYPH_INFO_VVECTOR0 << wmode),
	    &info);
	...
	if(code == gs_error_undefined ||
	   !(info.members & (GLYPH_INFO_WIDTH0 << wmode))) {
		...
		code =
		    store_glyph_width(&pwidths->Width, wmode, scale_c, &info);
		if(code < 0)
			return code;
		rcode |= code;
		...
		if(wmode && pdf_is_CID_font(ofont)) {
			if(info.members & (GLYPH_INFO_WIDTH0 << wmode)) {
				...
				if(get_missing_width(cfont, 0, scale_c,
				                     &widths1) < 0)
					v.x = 0;
				else
					v.x = widths1.Width.w / 2;
	...
	if(code > 0 && !pdf_is_CID_font(ofont))
		pwidths->Width.xy.x = pwidths->Width.xy.y = pwidths->Width.w =
		    0;
#endif
	if(cdevproc_result == NULL) {
		code = ofont->procs.glyph_info(
		    ofont, glyph, NULL,
		    (GLYPH_INFO_WIDTH0 << wmode) |
		        (GLYPH_INFO_VVECTOR0 << wmode) | allow_cdevproc_callout,
		    &info);
	...
	if(code == gs_error_undefined ||
	   !(info.members & (GLYPH_INFO_WIDTH0 << wmode)))
		...
		if((info.members &
		    (GLYPH_INFO_VVECTOR0 | GLYPH_INFO_VVECTOR1)) != 0)
			pwidths->replaced_v = true;
		else
			info.v.x = info.v.y = 0;
		code = store_glyph_width(&pwidths->real_width, wmode, scale_o,
		                         &info);
		if(code < 0)
			return code;
		rcode |= code;
                       ...
                       gs_text_enum_t** ppte)
{
	...
	if(pte->current_font->FontType == 3 &&
	   (text1.operation & TEXT_DO_NONE)) {
		/* We need a real drawing to accumulate charproc. */
		text1.operation &= ~TEXT_DO_NONE;
		text1.operation |= TEXT_DO_DRAW;
	}
	return gx_default_text_begin(pte->dev, pte->pis, &text1,
	                             pte->current_font, pte->path, pte->pdcolor,
	                             pte->pcpath, pte->memory, ppte);
}
...
#define BUF_SIZE 100 /* arbitrary > 0 */
	...
	if(!penum->pte_default && !penum->charproc_accum) {
		...
		if(penum->outer_CID != GS_NO_GLYPH) {
			/* Fallback to the default implermentation for handling
			   ...
			   font resource arrays' sizes are hardcoded to 256
			   glyphs.
			   A better solution would be to re-encode the CID text
			   with
	...
	if(!penum->pte_default) {
		...
		if(penum->cdevproc_callout) {
			/* Restore after TEXT_PROCESS_CDEVPROC in
			 * scan_cmap_text. */
...
top:
	...
	if(pte_default) {
		...
		pdev->pte =
		    pte_default; /* CAUTION: See comment in gdevpdfx.h . */
		...
		if(pte->orig_font->FontType != ft_user_defined)
			...
			penum->returned.current_char =
			    pte_default->returned.current_char;
			penum->returned.current_glyph =
			    pte_default->returned.current_glyph;
		}
		pdev->charproc_just_accumulated = false;
		if(code == TEXT_PROCESS_RENDER) {
			...
			if(penum->current_font->FontType == ft_user_defined &&
			   penum->orig_font->FontType != ft_composite &&
			   penum->outer_CID == GS_NO_GLYPH &&
			   !(penum->pte_default->text.operation &
			     TEXT_DO_CHARWIDTH)) {
				/* The condition above must be consistent with
				   one in pdf_text_set_cache,
				   which decides to apply
				   pdf_set_charproc_attrs. */
				gs_matrix m;

				code = pdf_start_charproc_accum(pdev);
				if(code < 0)
					return code;
				pdf_viewer_state_from_imager_state(
				    pdev, pte->pis, pte->pdcolor);
				/* Set line params to unallowed values so that
				   they'll synchronize with writing them out on
				   the first use.
				   Doing so because PDF viewer inherits them
				   from the
				   contents stream when executing the charproc,
				   but at this moment we don't know in what
				   contexts
				   it will be used. */
				...
				/* Must set an identity CTM for the charproc
				   accumulation.
				   The function show_proceed (called from
				   gs_text_process above)
				   executed gsave, so we are safe to change CTM
				   now.
				   Note that BuildChar may change CTM before
				   calling setcachedevice. */
				gs_make_identity(&m);
				gs_matrix_fixed_from_matrix(&penum->pis->ctm,
				                            &m);
	...
	{
		gs_font* font = pte->orig_font; /* Not sure. Changed for
		                                   CDevProc callout. Was
		                                   pte->current_font */
	...
	if(size <= sizeof(buf)) {
		...
		byte* buf =
		    gs_alloc_string(pte->memory, size, "pdf_text_process");
...
skip:
	if(code < 0 || (pte->current_font->FontType == ft_user_defined &&
	...
	return code;
	/*
	 * This function uses an unobvious algorithm while handling type 3
	 * fonts.
	 ...
	 * When the callout completes, pdf_text_process calls
	 * pdf_end_charproc_accum
	 * and later resumes the normal (non-default) text enumeration,
	 * repeating the
	 * the "callouted" glyph AT SECOND TIME. We can't do without the second
	 * pass
	 ...
	 * an interpreter callout will happen, and the function will be called
	 * again
