...
/* Copyright (C) 1991, 1995, 1997, 1998, 1999 Aladdin Enterprises.  All rights
  reserved.
...
#define sample_load_setup(sbit, bitno, sbpv) sbit = (bitno)

/* macro to eliminate compiler warning message */
#define sample_bound_shift(value, shift)                                       \
	((shift) >= 8 * sizeof(value) ? (shift) & (8 * sizeof(value) - 1)      \
	                              : (shift))
...
#define sample_load8(value, sptr, sbit, sbpv)                                  \
	sample_load8_(value, sptr, sbit, sbpv) sample_end_
...
#define sample_load12_(value, sptr, sbit, sbpv)                                \
	sample_load8_(value, sptr, sbit, sbpv) case 3                          \
	    : value = ((sbit) ? ((*(sptr)&0xf) << 8) | (sptr)[1]               \
	                      : (*(sptr) << 4) | ((sptr)[1] >> 4));            \
	break;
#define sample_load12(value, sptr, sbit, sbpv)                                 \
	sample_load12_(value, sptr, sbit, sbpv) sample_end_
...
#define sample_load16_(value, sptr, sbit, sbpv)                                \
	sample_load12_(value, sptr, sbit, sbpv) case 4                         \
	    : value = (*(sptr) << 8) | (sptr)[1];                              \
	break;
#define sample_load16(value, sptr, sbit, sbpv)                                 \
	sample_load16_(value, sptr, sbit, sbpv) sample_end_
...
#define sample_load32_(value, sptr, sbit, sbpv)                                \
	sample_load16_(value, sptr, sbit, sbpv) case 6                         \
	    : value = (*(sptr) << 16) | ((sptr)[1] << 8) | (sptr)[2];          \
	break;                                                                 \
	case 8:                                                                \
		value = (*(sptr) << 24) | ((sptr)[1] << 16) |                  \
		        ((sptr)[2] << 8) | sptr[3];                            \
		break;
...
#define sample_load64_(value, sptr, sbit, sbpv)                                \
	sample_load32_(value, sptr, sbit, sbpv);                               \
	case 10:                                                               \
		value = ((gx_color_index)((sptr)[0])                           \
		         << sample_bound_shift((value), 32)) |                 \
	...
	case 12:                                                               \
		value = ((gx_color_index)((sptr)[0])                           \
		         << sample_bound_shift((value), 40)) |                 \
		        ((gx_color_index)((sptr)[1])                           \
		         << sample_bound_shift((value), 32)) |                 \
	...
	case 14:                                                               \
		value = ((gx_color_index)((sptr)[0])                           \
		         << sample_bound_shift((value), 48)) |                 \
		        ((gx_color_index)((sptr)[1])                           \
		         << sample_bound_shift((value), 40)) |                 \
		        ((gx_color_index)((sptr)[2])                           \
		         << sample_bound_shift((value), 32)) |                 \
	...
	case 16:                                                               \
		value = ((gx_color_index)((sptr)[0])                           \
		         << sample_bound_shift((value), 56)) |                 \
		        ((gx_color_index)((sptr)[1])                           \
		         << sample_bound_shift((value), 48)) |                 \
		        ((gx_color_index)((sptr)[2])                           \
		         << sample_bound_shift((value), 40)) |                 \
		        ((gx_color_index)((sptr)[3])                           \
		         << sample_bound_shift((value), 32)) |                 \
...
#define sample_store_setup(dbit, bitno, dbpv) dbit = (bitno)
...
#define sample_store_next8_(value, dptr, dbit, dbpv, dbbyte)                   \
	...
	case 0:                                                                \
		if((dbit += (dbpv)) == 8)                                      \
			*(dptr)++ = dbbyte | (byte)(value), dbbyte = 0,        \
			dbit = 0;                                              \
		else                                                           \
...
#define sample_store_next8(value, dptr, dbit, dbpv, dbbyte)                    \
	sample_store_next8_(value, dptr, dbit, dbpv, dbbyte) case 2            \
	    :*(dptr)++ = (byte)(value);                                        \
	break;                                                                 \
	sample_end_
#define sample_store_next_12_(value, dptr, dbit, dbbyte)                       \
	if(dbit ^= 4)                                                          \
		...
		*(dptr) = dbbyte | (byte)((value) >> 8),                       \
		(dptr)[1] = (byte)(value), dptr += 2;
...
#define sample_store_next12_(value, dptr, dbit, dbpv, dbbyte)                  \
	sample_store_next8_(value, dptr, dbit, dbpv,                           \
	                    dbbyte) /* case 2 is deliberately omitted */       \
	    case 3 : sample_store_next_12_(value, dptr, dbit, dbbyte) break;
#define sample_store_next12(value, dptr, dbit, dbpv, dbbyte)                   \
	sample_store_next12_(value, dptr, dbit, dbpv, dbbyte) case 2           \
	    :*(dptr)++ = (byte)(value);                                        \
	break;                                                                 \
	sample_end_
#define sample_store_next16(value, dptr, dbit, dbpv, dbbyte)                   \
	sample_store_next12_(value, dptr, dbit, dbpv, dbbyte) case 4           \
	    :*(dptr)++ = (byte)((value) >> 8);                                 \
	case 2:                                                                \
		*(dptr)++ = (byte)(value);                                     \
		break;                                                         \
...
#define sample_store_next32(value, dptr, dbit, dbpv, dbbyte)                   \
	sample_store_next12_(value, dptr, dbit, dbpv, dbbyte) case 8           \
	    :*(dptr)++ = (byte)((value) >> 24);                                \
	case 6:                                                                \
		*(dptr)++ = (byte)((value) >> 16);                             \
	case 4:                                                                \
...
#define sample_store_next64(value, dptr, dbit, dbpv, dbbyte)                   \
	sample_store_next12_(value, dptr, dbit, dbpv, dbbyte) case 16          \
	    :*(dptr)++ = (byte)((value) >> sample_bound_shift((value), 56));   \
	case 14:                                                               \
		*(dptr)++ =                                                    \
		    (byte)((value) >> sample_bound_shift((value), 48));        \
	case 12:                                                               \
		*(dptr)++ =                                                    \
		    (byte)((value) >> sample_bound_shift((value), 40));        \
	case 10:                                                               \
		*(dptr)++ =                                                    \
		    (byte)((value) >> sample_bound_shift((value), 32));        \
	case 8:                                                                \
		*(dptr)++ = (byte)((value) >> 24);                             \
	case 6:                                                                \
		*(dptr)++ = (byte)((value) >> 16);                             \
...
void bits_compress_scaled(const byte* src, int srcx, uint width, uint height,
...
int bits_extract_plane(const bits_plane_t* dest /*write*/,
                       const bits_plane_t* source /*read*/, int shift,
...
void bytes_fill_rectangle(byte* dest, uint raster, byte value, int width_bytes,
                          int height);

/* Copy a rectangle of bytes. */
void bytes_copy_rectangle(byte* dest, uint dest_raster, const byte* src,
