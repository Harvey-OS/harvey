...
#define sgi_prn_device(procs, dev_name, num_comp, depth, max_gray, max_color,  \
                       print_page)                                             \
	{                                                                      \
		prn_device_body(gx_device_printer, procs, dev_name,            \
		                DEFAULT_WIDTH_10THS, DEFAULT_HEIGHT_10THS,     \
		                X_DPI, Y_DPI, 0, 0, 0, 0, num_comp, depth,     \
		                max_gray, max_color, max_gray + 1,             \
		                max_color + 1, print_page)                     \
	}

private
dev_proc_map_rgb_color(sgi_map_rgb_color);
private
dev_proc_map_color_rgb(sgi_map_color_rgb);

private
dev_proc_print_page(sgi_print_page);

private
gx_device_procs sgi_procs =
...
sgi_map_color_rgb(gx_device* dev, gx_color_index color, ushort prgb[3])
{
	...
	prgb[1] = (ushort)(((color >> bitspercolor) & colormask) *
	                   (ulong)gx_max_color_value / colormask);
	prgb[2] = (ushort)((color & colormask) * (ulong)gx_max_color_value /
	                   colormask);
	return 0;
}
...
sgi_begin_page(gx_device_printer* bdev, FILE* pstream, sgi_cursor* pcur)
{
	...
	byte* data =
	    (byte*)gs_malloc(bdev->memory, line_size, 1, "sgi_begin_page");
	IMAGE* header =
	    (IMAGE*)gs_malloc(bdev->memory, sizeof(IMAGE), 1, "sgi_begin_page");
	char filler = '\0';
	int i;
	...
	for(i = 0; i < 512 - sizeof(IMAGE); i++)
		fputc(filler, pstream);
	pcur->dev = bdev;
...
sgi_print_page(gx_device_printer* pdev, FILE* pstream)
{
	...
	uint bpe, mask;
	int separation;
	long* rowsizes = (long*)gs_malloc(pdev->memory, 4, 3 * bdev->height,
	                                  "sgi_print_page");
	byte* edata;
	long lastval;
	int rownumber;
#define aref2(a, b) a * bdev->height + b
	edata =
	    (byte*)gs_malloc(pdev->memory, cur.line_size, 1, "sgi_begin_page");
	if((code < 0) || (rowsizes == (long*)NULL) || (edata == (byte*)NULL))
		return (-1);
	fwrite(rowsizes, sizeof(long), 3 * bdev->height,
	       pstream); /* rowstarts */
	fwrite(rowsizes, sizeof(long), 3 * bdev->height,
	       pstream); /* rowsizes */
	...
	for(separation = 0; separation < 3; separation++) {
		...
		while(!(code = sgi_next_row(&cur))) {
			...
			*optr++ = 0;
			rowsizes[aref2(separation, rownumber++)] =
			    optr - startcol;
	...
	gs_free(pdev->memory, (char*)cur.data, cur.line_size, 1,
	        "sgi_print_page(done)");
	gs_free(pdev->memory, (char*)edata, cur.line_size, 1,
	        "sgi_print_page(done)");
	gs_free(pdev->memory, (char*)rowsizes, 4, 3 * bdev->height,
	        "sgi_print_page(done)");
