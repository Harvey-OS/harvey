...
private
int
s_AXE_process(stream_state* st, stream_cursor_read* pr, stream_cursor_write* pw,
              bool last)
{
	stream_AXE_state* const ss = (stream_AXE_state*)st;
	const byte* p = pr->ptr;
...
/* Stream template */
const stream_template s_AXE_template = {&st_AXE_state, s_AXE_init,
                                        s_AXE_process, 1, 3};
...
private
int
s_AXD_process(stream_state* st, stream_cursor_read* pr, stream_cursor_write* pw,
              bool last)
{
	stream_AXD_state* const ss = (stream_AXD_state*)st;
...
/* Stream template */
const stream_template s_AXD_template = {&st_AXD_state, s_AXD_init,
                                        s_AXD_process, 2, 1};
...
/* Stream template */
const stream_template s_PSSE_template = {&st_stream_state, NULL, s_PSSE_process,
                                         1, 4};
...
/* Stream template */
const stream_template s_PSSD_template = {&st_PSSD_state, s_PSSD_init,
                                         s_PSSD_process, 4, 1};
 ...
 */
int
s_hex_process(stream_cursor_read* pr, stream_cursor_write* pw, int* odd_digit,
              hex_syntax syntax)
{
	const byte* p = pr->ptr;
	const byte* rlimit = pr->limit;
