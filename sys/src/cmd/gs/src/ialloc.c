...
/* Copyright (C) 1993, 1995, 1996, 1998, 1999 Aladdin Enterprises.  All rights
  reserved.
            ...
            bool level2)
{
	...
	ilmem->space_id =
	    (i_vm_local << 1) + 1; /* overrides if ilmem == igmem */
...
/* Register a ref root. */
int
gs_register_ref_root(gs_memory_t* mem, gs_gc_root_t* root, void** pp,
                     client_name_t cname)
{
...
int
gs_alloc_ref_array(gs_ref_memory_t* mem, ref* parr, uint attrs, uint num_refs,
                   client_name_t cname)
{
	ref* obj;
...
int
gs_resize_ref_array(gs_ref_memory_t* mem, ref* parr, uint new_num_refs,
                    client_name_t cname)
{
	...
	if(mem->cc.rtop == mem->cc.cbot &&
	   (byte*)(obj + (old_num_refs + 1)) == mem->cc.rtop) {
		...
		ref* end =
		    (ref*)(mem->cc.cbot = mem->cc.rtop -= diff * sizeof(ref));
...
gs_free_ref_array(gs_ref_memory_t* mem, ref* parr, client_name_t cname)
{
	...
	if_debug4('A', "[a%d:-$#]%s(%u) 0x%lx\n", ialloc_trace_space(mem),
	          client_name_string(cname), num_refs, (uint32_t)obj);
	{
		uint size;
		...
		case t_shortarray:
			...
			break;
		case t_mixedarray: {
			/* We have to parse the array to compute the storage
			 * size. */
			uint i = 0;
...
int
gs_alloc_string_ref(gs_ref_memory_t* mem, ref* psref, uint attrs, uint nbytes,
                    client_name_t cname)
{
