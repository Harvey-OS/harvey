...
private
gx_device_procs att3b1_procs = {
    att3b1_open, gx_default_get_initial_matrix, gx_default_sync_output,
    att3b1_output_page, att3b1_close, gx_default_map_rgb_color,
    gx_default_map_color_rgb, att3b1_fill_rectangle, gx_default_tile_rectangle,
    att3b1_copy_mono, gx_default_copy_color, gx_default_draw_line,
    gx_default_get_bits};

gx_device_att3b1 gs_att3b1_device = {
    std_device_std_body(gx_device_att3b1, &att3b1_procs, "att3b1", XSIZE, YSIZE,
                        XDPI, YDPI),
    {0}, /* std_procs */
    -1,
    0,
    0, /* fd, screen, line_size, */
    0,
    0, /* screen size, page */
#ifdef ATT3B1_PERF
    0,
    0,
    0, /* no_output, no_fill, no_copy */
#endif
};
...
att3b1_open(gx_device* dev)
{
	...
	if(ioctl(att3b1dev->fd, WIOCGETD, &uw) < 0) {
		lprintf1("att3b1_open: can not obtain window data [%d]\n",
		         errno);
		lprintf("att3b1_open: the att3b1 device requires a console "
		        "window\n");
		att3b1_close(dev);
		return_error(gs_error_ioerror);
	}
	...
	att3b1dev->screen = (uint8_t*)gs_malloc(
	    dev->memory, att3b1dev->screen_size, 1, "att3b1_open");
	if(att3b1dev->screen == NULL) {
		att3b1_close(dev);
...
#else
#define rotate(value, count) value = (value >> count) | (value << (32 - count))
#endif

int
att3b1_copy_mono(gx_device* dev, const uint8_t* data, int data_x, int raster,
                 ...
                 gx_color_index colour0, gx_color_index colour1)
{
	...
	dst_p =
	    (uint16_t*)&att3b1dev->screen[(uint16_t)y * att3b1dev->line_size] +
	    (uint)x / 16;
	dst_o = (uint)x % 16;
	dst_b = 16 - dst_o;
	...
	while(height-- > 0) {
		...
		while(w2 > 0) {
			...
			mask = masks[w1];
			/*
			 * We are assuming that the bitmap "data" is typically
			 * aligned.
			 * Thus the test for this special case is typically a
			 * win over
			 * a 16-bit shift.
			 */
			if(src_o == 0)
			...
			if(w1 <= 8)
				...
				bits = (reverse_bits[bits & 0xff] << 8) |
				       reverse_bits[bits >> 8];
			/*
			 * While the input bit map is assumed to be typically
			 * aligned, we
			 * assume that the place in the image is not.  Thus we
			 * don't
			 * separate out the aligned case.  Doing so would cost a
			 * test,
...
att3b1_do_output_page(gx_device* dev, int num_copies, int flush)
{
	...
	if(ioctl(att3b1dev->fd, WIOCGETD, &uw) < 0) {
		lprintf1(
		    "att3b1_output_page: window WIOCGETD ioctl failed [%d]\n",
		    errno);
		att3b1_close(dev);
		return_error(gs_error_ioerror);
	}
	...
	if(!(uflags & NBORDER)) {
		uw.uw_uflags =
		    BORDHSCROLL | BORDVSCROLL | BORDHELP | BORDCANCEL;
	...
	screen_width =
	    (uw.uw_width < att3b1dev->width) ? uw.uw_width : att3b1dev->width;
	...
	while(1) {
		...
		if(ioctl(att3b1dev->fd, WIOCRASTOP, &ur) < 0) {
			lprintf1("att3b1_output_page: window WIOCRASTOP ioctl "
			         "failed [%d]\n",
			         errno);
		...
		ut.ut_num = WTXTSLK1;
		sprintf(ut.ut_text, "%s %d, top right (%d,%d), size (%d,%d), "
		                    "press '?' for help.",
		        flush ? "Showpage" : "Copypage", att3b1dev->page_num,
		        xorigin, yorigin, att3b1dev->width, att3b1dev->height);
		ioctl(att3b1dev->fd, WIOCSETTEXT, &ut);
		...
		case 'h':
			xorigin -=
			    ((uint)(int)att3b1dev->x_pixels_per_inch + 3) / 4;
			break;

		case 'k':
			yorigin -=
			    ((uint)(int)att3b1dev->y_pixels_per_inch + 1) / 2;
			break;
		...
		case '\r':
			if(flush)
				...
				for(p = (ushort*)att3b1dev->screen;
					...
					*p = ~*p;
			if(!(uflags & NBORDER)) {
				ioctl(att3b1dev->fd, WIOCGETD,
				      &uw); /*window may have moved*/
				uw.uw_uflags = uflags;
				ioctl(att3b1dev->fd, WIOCSETD, &uw);
			}
