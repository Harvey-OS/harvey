...
int zimage_data_setup(i_ctx_t* i_ctx_p, const gs_pixel_image_t* pim,
                      gx_image_enum_common_t* pie, const ref* sources,
                      int npop);
private
int image_proc_process(i_ctx_t*);
private
int image_file_continue(i_ctx_t*);
private
int image_string_continue(i_ctx_t*);
private
int image_cleanup(i_ctx_t*);
                  ...
                  bool has_alpha)
{
	...
	check_dict_read(*op);
	if((code = dict_int_param(op, "Width", 0, max_int_in_fixed / 2, -1,
	                          &pim->Width)) < 0 ||
	   (code = dict_int_param(op, "Height", 0, max_int_in_fixed / 2, -1,
	                          &pim->Height)) < 0 ||
	   ...
	   (code = decode_size =
	        dict_floats_param(mem, op, "Decode", num_components * 2,
	                          &pim->Decode[0], NULL)) < 0 ||
...
int
zimage_setup(i_ctx_t* i_ctx_p, const gs_pixel_image_t* pim, const ref* sources,
             bool uses_color, int npop)
{
	gx_image_enum_common_t* pie;
	int code = gs_image_begin_typed((const gs_image_common_t*)pim, igs,
	                                uses_color, &pie);
...
zimagemask1(i_ctx_t* i_ctx_p)
{
	...
	code = data_image_params(imemory, op, (gs_data_image_t*)&image, &ip,
	                         true, 1, 1, false);
	if(code < 0)
		return code;
...
#define ETOP_SOURCE(ep, i) ((ep)-4 - (i)*2)
	...
	for(px = 0, pp = sources; px < num_sources; px++, pp++) {
		...
		case t_file:
			...
			/* Check for aliasing. */
			{
				...
				for(pi = 0; pi < px; ++pi)
					if(sources[pi].value.pfile ==
					   pp->value.pfile) {
						/* Record aliasing */
						make_int(ep + 1, -pi);
						EBOT_SOURCE(
						    esp, pi)[1].value.intval++;
						break;
					}
			}
		/* falls through */
...
image_proc_continue(i_ctx_t* i_ctx_p)
{
	...
	if(!r_has_type_attrs(op, t_string, a_read)) {
		...
		return_error(!r_has_type(op, t_string) ? e_typecheck
		                                       : e_invalidaccess);
	}
	size = r_size(op);
	if(size == 0 && ETOP_SOURCE(esp, 0)[1].value.intval == 0)
		...
		if(code == e_RemapColor) {
			...
			ETOP_SOURCE(esp, 0)[1].value.intval =
			    0; /* RemapColor callout */
			return code;
		}
	}
	if(code) { /* Stop now. */
	...
	for(;;) {
		...
		for(px = 0, pp = ETOP_SOURCE(esp, 0); px < num_sources;
			...
			while((avail = sbufavailable(s)) <=
				...
				case INTC:
				case CALLC:
					return s_handle_read_exception(
					    i_ctx_p, next, pp, NULL, 0,
					    image_file_continue);
				default:
					/* case ERRC: */
			...
			/*
			 * Note that in the EOF case, we can get here with no
			 * data
			 * available.
			 */
			if(avail >= min_left)
				avail = (avail - min_left) /
				        num_aliases; /* may be 0 */
			if(avail < min_avail)
				min_avail = avail;
		...
		if(code) {
			...
			return (code < 0 ? code : code1 < 0 ? code1
			                                    : o_pop_estack);
		}
	}
}
