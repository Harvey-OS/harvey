...
/* Copyright (C) 1989, 1990, 1991, 1994, 1996 Aladdin Enterprises.  All rights
  reserved.
...
gx_device_procs pe_procs = {
    pe_open, NULL,           /* get_initial_matrix */
    NULL,                    /* sync_output */
    NULL,                    /* output_page */
    pe_close, NULL,          /* map_rgb_color */
    NULL,                    /* map_color_rgb */
    pe_fill_rectangle, NULL, /* tile_rectangle */
    pe_copy_mono, NULL       /* copy_color */
};

gx_device_pe far_data
    gs_pe_device = {std_device_std_body(gx_device_pe, &pe_procs, "pe", XSIZE,
                                        YSIZE, XPPI, YPPI),
                    {0}, /* std_procs */
                    DEFAULT_ADDRESS,
                    DEFAULT_REGISTERS};

static regval peinit[] = {{0x04, 0x1e},
                          {0x05, 0x00},
                          {0x04, 0x0c},
                          {0x05, 0x21},
                          {0x04, 0x0d},
...
static regval pedone[] = {{0x04, 0x1e},
                          {0x05, 0x10},
                          {0x04, 0x0a},
                          {0x05, 0x00},
                          {0x04, 0x0b},
...
pe_open(gx_device* dev)
{
	...
	if((str = getenv("PEFBADDR")) != 0) {
		if(!sscanf(str, "%lx", &(pedev->fbaddr))) {
			eprintf("Private Eye: PEFBADDR environment string "
			        "format error\n");
			exit(1);
		}
	}
	...
	if((str = getenv("PEREGS")) != 0) {
		if(!sscanf(str, "%x", &(pedev->regs))) {
			eprintf("Private Eye: PEREGS environment string format "
			        "error\n");
			exit(1);
		}
	}
...
int
pe_copy_mono(gx_device* dev, const byte* base, int sourcex, int raster,
