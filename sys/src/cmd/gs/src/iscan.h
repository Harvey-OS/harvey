...
struct scanner_state_s {
	...
	union sss_ {                      /* scanning sub-state */
		...
		struct sns_ {             /* name */
			...
			bool
			    s_try_number; /* true if should try scanning name */
			                  /* as number */
		} s_name;
...
#define public_st_scanner_state() /* in iscan.c */                             \
	gs_public_st_complex_only(st_scanner_state, scanner_state,             \
	                          "scanner state", scanner_clear_marks,        \
	                          scanner_enum_ptrs, scanner_reloc_ptrs, 0)
...
int scan_token(i_ctx_t* i_ctx_p, stream* s, ref* pref, scanner_state* pstate);
 ...
 * or an error code.
 */
int scan_handle_refill(i_ctx_t* i_ctx_p, const ref* fop, scanner_state* pstate,
