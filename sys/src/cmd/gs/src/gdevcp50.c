...
/* Copyright (C) 1991, 1994, 1996, 1998 Aladdin Enterprises.  All rights
  reserved.
...
private
dev_proc_print_page(cp50_print_page);
private
dev_proc_output_page(cp50_output_page);

private
dev_proc_map_rgb_color(cp50_rgb_color);
private
dev_proc_map_color_rgb(cp50_color_rgb);

private
gx_device_procs cp50_procs =
...
const gx_device_printer far_data gs_cp50_device =
    prn_device(cp50_procs, "cp50", 39,               /* width_10ths, 100mm */
               59,                                   /* height_10ths,150mm  */
               X_DPI, Y_DPI, 0.39, 0.91, 0.43, 0.75, /* margins */
               24, cp50_print_page);
...
cp50_print_page(gx_device_printer* pdev, FILE* prn_stream)
{
	...
	byte* out = (byte*)gs_malloc(pdev->memory, line_size, 1,
	                             "cp50_print_page(out)");
	byte* r_plane = (byte*)gs_malloc(pdev->memory, X_PIXEL * Y_PIXEL, 1,
	                                 "cp50_print_page(r_plane)");
	byte* g_plane = (byte*)gs_malloc(pdev->memory, X_PIXEL * Y_PIXEL, 1,
	                                 "cp50_print_page(g_plane)");
	byte* b_plane = (byte*)gs_malloc(pdev->memory, X_PIXEL * Y_PIXEL, 1,
	                                 "cp50_print_page(b_plane)");
	byte* t_plane = (byte*)gs_malloc(pdev->memory, X_PIXEL * Y_PIXEL, 1,
	...
	/*fprintf(prn_stream, "%d,%d,%d,", pdev->width, pdev->height,
	 * line_size);*/
	   ...
	   t_plane == 0) {
		if(out)
			gs_free(pdev->memory, (char*)out, line_size, 1,
			        "cp50_print_page(out)");
		if(r_plane)
			gs_free(pdev->memory, (char*)r_plane, X_PIXEL * Y_PIXEL,
			        1, "cp50_print_page(r_plane)");
		if(g_plane)
			gs_free(pdev->memory, (char*)g_plane, X_PIXEL * Y_PIXEL,
			        1, "cp50_print_page(g_plane)");
		if(b_plane)
			gs_free(pdev->memory, (char*)b_plane, X_PIXEL * Y_PIXEL,
			        1, "cp50_print_page(b_plane)");
		if(t_plane)
			gs_free(pdev->memory, (char*)t_plane, X_PIXEL * Y_PIXEL,
			        1, "cp50_print_page(t_plane)");
		return -1;
	}
...
int private cp50_output_page(gx_device* pdev, int num_copies, int flush)
{
	...
	if(ppdev->buffer_space) /* reinitialize clist for writing */
		code = (*gs_clist_device_procs.output_page)(pdev, num_copies,
		                                            flush);
...
private
int
cp50_color_rgb(gx_device* dev, gx_color_index color, gx_color_value prgb[3])
{
