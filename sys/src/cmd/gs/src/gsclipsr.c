...
gs_clipsave(gs_state* pgs)
{
	...
	gx_clip_path* copy = gx_cpath_alloc_shared(pgs->clip_path, mem,
	                                           "gs_clipsave(clip_path)");
	gx_clip_stack_t* stack = gs_alloc_struct(
	    mem, gx_clip_stack_t, &st_clip_stack, "gs_clipsave(stack)");
...
gs_cliprestore(gs_state* pgs)
{
	...
	if(stack) {
		...
		return gx_cpath_assign_preserve(pgs->clip_path,
		                                pgs->saved->clip_path);
	}
}
