...
/* Copyright (C) 1989, 1995, 1996, 1997, 1998, 1999, 2000 Aladdin Enterprises.
  All rights reserved.
...
zfont_encode_char(gs_font* pfont, gs_char chr, gs_glyph_space_t gspace)
{
	...
	if(pfont->FontType == ft_user_defined &&
	   r_type(&pdata->BuildGlyph) == t_null) {
		...
		if(r_size(&nsref) == 7 &&
		   !memcmp(nsref.value.const_bytes, ".notdef",
		           r_size(&nsref))) {
			/* A special support for high level devices.
			   They need a glyph name but the font doesn't provide
			   one
			   due to an instandard BuildChar.
			   Such fonts don't conform to PLRM section 5.3.7,
			   but we've got real examples that we want to handle
			   (Bug 686982).
			   Construct a name here.
			   Low level devices don't pass here, because regular PS
			   interpretation
			...
			*/
			...
			code = name_ref(pfont->memory, (const byte*)buf,
			                strlen(buf), &tname, 1);
			if(code < 0) {
				/* Can't propagate the error due to interface
				   limitation,
				   return with .notdef */
			} else
				cname = tname;
...
zfont_glyph_name(gs_font* font, gs_glyph index, gs_const_string* pstr)
{
	...
	if(index >= gs_min_cid_glyph) { /* Fabricate a numeric name. */
		...
		code = name_ref(font->memory, (const byte*)cid_name,
		                strlen(cid_name), &nref, 1);
		if(code < 0)
			return code;
	} else
...
private
gs_char
gs_font_map_glyph_by_dict(const gs_memory_t* mem, const ref* map,
                          gs_glyph glyph)
{
	ref* v, n;
	if(glyph >= gs_min_cid_glyph) {
		...
		if(dict_find_string(map, "CIDCount", &v) > 0) {
			...
			if(dict_find(map, &n, &v) > 0) {
				...
				if(array_get(mem, v, cid % 256, &vv) == 0 &&
				   r_type(&vv) == t_integer)
...
gs_font_map_glyph_to_unicode(gs_font* font, gs_glyph glyph)
{
	...
	if(r_type(&pdata->GlyphNames2Unicode) == t_dictionary) {
		gs_char c = gs_font_map_glyph_by_dict(
		    font->memory, &pdata->GlyphNames2Unicode, glyph);

		if(c != GS_NO_CHAR)
			return c;
		/*
		 * Fall through, because test.ps for SF bug #529103 requres
		 * to examine both tables. Due to that the Unicode Decoding
		 * resource
	...
	if(UnicodeDecoding != NULL && r_type(UnicodeDecoding) == t_dictionary)
		return gs_font_map_glyph_by_dict(font->memory, UnicodeDecoding,
		                                 glyph);
...
const op_def zbfont_op_defs[] = {{"2.buildfont3", zbuildfont3}, op_def_end(0)};
                     ...
                     const char* bcstr, const char* bgstr)
{
	...
	if(!bcstr)
		...
		if((code = name_ref(mem, (const byte*)bcstr, strlen(bcstr),
		                    &pbuild->BuildChar, 0)) < 0)
			return code;
	...
	if(!bgstr)
		...
		if((code = name_ref(mem, (const byte*)bgstr, strlen(bgstr),
		                    &pbuild->BuildGlyph, 0)) < 0)
			return code;
...
private
int
font_with_same_UID_and_another_metrics(const gs_font* pfont0,
                                       const gs_font* pfont1)
{
	...
	if(uid_equal(&pbfont0->UID, &pbfont1->UID)) {
		...
		if(pbfont0->WMode ||
		   dict_find_string(pfdict0, "Metrics", &pmdict0) <= 0)
			pmdict0 = NULL;
		if(pbfont1->WMode ||
		   dict_find_string(pfdict1, "Metrics", &pmdict1) <= 0)
			pmdict1 = NULL;
		if(!pmdict0 != !pmdict1)
			return 1;
		if(pmdict0 != NULL && !obj_eq(pfont0->memory, pmdict0, pmdict1))
			return 1;
		if(!pbfont0->WMode ||
		   dict_find_string(pfdict0, "Metrics2", &pmdict0) <= 0)
			pmdict0 = NULL;
		if(!pbfont0->WMode ||
		   dict_find_string(pfdict1, "Metrics2", &pmdict1) <= 0)
			pmdict1 = NULL;
		if(!pmdict0 != !pmdict1)
			return 1;
		if(pmdict0 != NULL && !obj_eq(pfont0->memory, pmdict0, pmdict1))
                        ...
                        build_font_options_t options)
{
	...
	if(dict_find_string(op, "CharStrings", &pcharstrings) <= 0) {
		...
		if((options & bf_notdef_required) != 0 &&
		   dict_find_string(pcharstrings, ".notdef", &ignore) <= 0)
			return_error(e_invalidfont);
		/*
		 * Since build_gs_simple_font may resize the dictionary and
		 * cause
	...
	if(uid_is_valid(&pfont->UID) && !dict_check_uid_param(op, &pfont->UID))
		uid_set_invalid(&pfont->UID);
	if(uid_is_valid(&pfont->UID)) {
		const gs_font* pfont0 = (const gs_font*)pfont;

		code = gs_font_find_similar(
		    ifont_dir, &pfont0, font_with_same_UID_and_another_metrics);
		if(code < 0)
...
private
int
build_FDArray_sub_font(i_ctx_t* i_ctx_p, ref* op, gs_font_base** ppfont,
                       font_type ftype, gs_memory_type_ptr_t pstype,
                       const build_proc_refs* pbuild,
...
int
build_gs_FDArray_font(i_ctx_t* i_ctx_p, ref* op, gs_font_base** ppfont,
                      font_type ftype, gs_memory_type_ptr_t pstype,
                      const build_proc_refs* pbuild)
{
	gs_font_base* pfont;
	font_data* pdata;
	int code =
	    build_gs_outline_font(i_ctx_p, op, &pfont, ftype, pstype, pbuild,
	                          bf_options_none, build_FDArray_sub_font);
...
lookup_gs_simple_font_encoding(gs_font_base* pfont)
{
	...
	if(r_type(pfe) == t_array && r_size(pfe) <= 256) {
		...
		for(i = 0; i < esize; ++i) {
			...
			if(array_get(pfont->memory, pfe, (int32_t)i, &fchar) <
			       0 ||
		...
		for(index = 0; index < NUM_KNOWN_REAL_ENCODINGS; ++index) {
			...
			for(i = esize; --i >= 0;) {
				...
				gs_c_glyph_name(
				    gs_c_known_encode((gs_char)i, index),
				    &rstr);
...
private
int
sub_font_params(const gs_memory_t* mem, const ref* op, gs_matrix* pmat,
                gs_matrix* pomat, ref* pfname)
{
	ref* pmatrix;
	ref* pfontname;
	ref* porigfont;
	...
	/* Use the FontInfo/OrigFontName key preferrentially (created by MS
	 * PSCRIPT driver) */
	if((dict_find_string((porigfont != NULL ? porigfont : op), "FontInfo",
	                     &pfontname) > 0) &&
	   (dict_find_string(pfontname, "OrigFontName", &pfontname) > 0)) {
		get_font_name(mem, pfname, pfontname);
	} else if(dict_find_string((porigfont != NULL ? porigfont : op),
	                           ".Alias", &pfontname) > 0) {
		/* If we emulate the font, we want the requested name rather
		 * than a substitute. */
		get_font_name(mem, pfname, pfontname);
	} else if(dict_find_string((porigfont != NULL ? porigfont : op),
              ...
              build_font_options_t options)
{
	...
	if(dict_find_string(op, "FontType", &pftype) <= 0 ||
	   !r_has_type(pftype, t_integer) || pftype->value.intval != (int)ftype)
		return_error(e_invalidfont);
	...
	if((code = dict_int_param(op, "WMode", 0, 1, 0, &wmode)) < 0 ||
	   (code = dict_bool_param(op, "BitmapWidths", false, &bitmapwidths)) <
	       0 ||
	   (code = dict_int_param(op, "ExactSize", 0, 2, fbit_use_bitmaps,
	                          &exactsize)) < 0 ||
	   (code = dict_int_param(op, "InBetweenSize", 0, 2, fbit_use_outlines,
	                          &inbetweensize)) < 0 ||
	   (code = dict_int_param(op, "TransformedChar", 0, 2,
	                          fbit_use_outlines, &transformedchar)) < 0)
		return code;
	...
	if(code > 0) {
		if(!r_has_type(pfid, t_fontID))
			return_error(e_invalidfont);
		/*
		 * If this font has a FID entry already, it might be a scaled
		 * font
		 * made by makefont or scalefont; in a Level 2 environment, it
		 * might
		 * be an existing font being registered under a second name, or
		 * a
		 * re-encoded font (which was invalid in Level 1, but dvips did
		 * it
		 * anyway).
		 */
                  ...
                  ref* fid_op)
{
	...
	pfont = gs_font_alloc(imemory, pstype, &gs_font_procs_default, NULL,
	                      "buildfont(font)");
	pdata = ialloc_struct(font_data, &st_font_data, "buildfont(data)");
	if(pfont == 0 || pdata == 0)
