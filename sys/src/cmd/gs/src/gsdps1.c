...
/* Copyright (C) 1991, 1992, 1994, 1996, 1997, 1998 Aladdin Enterprises.  All
  rights reserved.
...
gs_setbbox(gs_state* pgs, floatp llx, floatp lly, floatp urx, floatp ury)
{
	...
	if(dbox.p.x < fixed2float(min_fixed + box_rounding_slop_fixed) ||
	   ...
	   dbox.q.x >= fixed2float(max_fixed - box_rounding_slop_fixed +
	                           fixed_epsilon) ||
	   dbox.q.y >=
	       fixed2float(max_fixed - box_rounding_slop_fixed + fixed_epsilon))
	...
	if(gx_path_bbox(ppath, &obox) >=
	   0) { /* Take the union of the bboxes. */
...
gs_rectfill(gs_state* pgs, const gs_rect* pr, uint count)
{
	...
	bool hl_color = (hl_color_available &&
	                 dev_proc(pdev, fill_rectangle_hl_color)(
	                     pdev, &empty, pis, pdc, NULL) == 0);
	...
	if((is_fzero2(pgs->ctm.xy, pgs->ctm.yx) ||
	   ...
	   (hl_color || pdc->type == gx_dc_type_pure ||
	    pdc->type == gx_dc_type_ht_binary ||
	    pdc->type == gx_dc_type_ht_colored
	    /* DeviceN todo: add wts case */) &&
	   gs_state_color_load(pgs) >= 0 &&
	   (*dev_proc(pdev, get_alpha_bits))(pdev, go_graphics) <= 1 &&
	   (!pgs->overprint || !pgs->effective_overprint_mode)) {
		...
		for(i = 0; i < count; ++i) {
			...
			if(gs_point_transform2fixed(&pgs->ctm, pr[i].p.x,
			                            pr[i].p.y, &p) < 0 ||
			   gs_point_transform2fixed(&pgs->ctm, pr[i].q.x,
			                            pr[i].q.y, &q) <
			       0) { /* Switch to the slow algorithm. */
				goto slow;
			}
			...
			if(hl_color) {
				...
				if(draw_rect.p.x < draw_rect.q.x &&
				   draw_rect.p.y < draw_rect.q.y) {
					code = dev_proc(
					    pdev, fill_rectangle_hl_color)(
					    pdev, &draw_rect, pis, pdc, pcpath);
					if(code < 0)
						return code;
				}
			} else {
				...
				if(w > 0 && h > 0)
					if(gx_fill_rectangle(x, y, w, h, pdc,
					                     pgs) < 0)
						goto slow;
			}
		}
