...
#define runBuffer 4
private
int swproc(gs_main_instance*, const char*, arg_list*);
private
int argproc(gs_main_instance*, const char*);
private
int run_buffered(gs_main_instance*, const char*);
private
int esc_strlen(const char*);
private
void esc_strcat(char*, const char*);
private
int runarg(gs_main_instance*, const char*, const char*, const char*, int);
private
int run_string(gs_main_instance*, const char*, int);
private
int run_finish(gs_main_instance*, int, int, ref*);
private
int try_stdout_redirect(gs_main_instance* minst, const char* command,
                        const char* filename);

/* Forward references for help printout */
private
void print_help(gs_main_instance*);
private
void print_revision(const gs_main_instance*);
private
void print_version(const gs_main_instance*);
private
void print_usage(const gs_main_instance*);
private
void print_devices(const gs_main_instance*);
private
void print_emulators(const gs_main_instance*);
private
void print_paths(gs_main_instance*);
private
void print_help_trailer(const gs_main_instance*);
...
gs_main_init_with_args(gs_main_instance* minst, int argc, char* argv[])
{
	...
	arg_init(&args, (const char**)argv, argc, gs_main_arg_fopen,
	         (void*)minst);
...
*/
#ifndef __VMS
	{
		...
		if(code < 0) { /* key present, value doesn't fit */
			char* path =
			    (char*)gs_alloc_bytes(minst->heap, len, "GS_LIB");
	...
	/* Prescan the command line for --help and --version. */
	{
		...
		for(i = 1; i < argc; ++i)
			if(!strcmp(argv[i], "--")) {
				/* A PostScript program will be interpreting all
				 * the */
...

	{
		...
		if(code < 0) { /* key present, value doesn't fit */
			char* opts = (char*)gs_alloc_bytes(minst->heap, len,
			                                   "GS_OPTIONS");
...
#define initial_enter_name(nstr, pvalue)                                       \
		...
		if(*arg == '-')
			...
			if(sscanf((const char*)arg, "%u", &bsize) != 1 ||
			   bsize <= 0 || bsize > MAX_BUFFERED_SIZE) {
				outprintf(minst->heap, "-B must be followed by "
				                       "- or size between 1 "
				                       "and %u\n",
				          MAX_BUFFERED_SIZE);
				return e_Fatal;
	...
	case 'K': /* set malloc limit */
	{
		...
		gs_malloc_memory_t* rawheap =
		    gs_malloc_wrapped_contents(minst->heap);
		...
		if(eqp == NULL) {
			...
			*eqp++ = 0;
			...
			if(isd) {
				...
				s_init(&astream, NULL);
				sread_string(&astream, (const byte*)eqp,
				             strlen(eqp));
				scanner_state_init(&state, false);
				code = scan_token(minst->i_ctx_p, &astream,
				                  &value, &state);
				if(code) {
					puts(minst->heap, "-dname= must be "
					                  "followed by a valid "
					                  "token");
					return e_Fatal;
				}
				if(r_has_type_attrs(&value, t_name,
					...
					name_string_ref(minst->heap, &value,
					                &nsref);
...
#undef name_is_string
				...
				int len = strlen(eqp);
				char* str = (char*)gs_alloc_bytes(
				    minst->heap, (uint)len, "-s");
				...
				make_const_string(&value,
				                  a_readonly | avm_foreign, len,
				                  (const byte*)str);
				if((code = try_stdout_redirect(minst, adef,
				                               eqp)) < 0)
					return code;
			}
...
#define start_x() gs_main_run_string_begin(minst, 1, &xec, &xeo)
#define run_x(str)                                                             \
	gs_main_run_string_continue(minst, str, strlen(str), 1, &xec, &xeo)
#define stop_x() gs_main_run_string_end(minst, 1, &xec, &xeo)
			start_x();
			run_x("\216\003abc");
			run_x("== flush\n");
			stop_x();
...
argproc(gs_main_instance* minst, const char* arg)
{
	...
	if(minst->run_buffer_size) {
		...
		return runarg(minst, "", filearg, ".runfile",
		              runInit | runFlush);
	}
}
private
...
run_buffered(gs_main_instance* minst, const char* arg)
{
	...
	if(code < 0)
		return code;
	code = gs_main_run_string_begin(minst, minst->user_errors, &exit_code,
	                                &error_object);
	if(!code) {
		...
		while((count = fread(buf, 1, minst->run_buffer_size, in)) > 0) {
			code = gs_main_run_string_continue(
			    minst, buf, count, minst->user_errors, &exit_code,
			    &error_object);
			if(code != e_NeedInput)
				break;
		}
		if(code == e_NeedInput) {
			code =
			    gs_main_run_string_end(minst, minst->user_errors,
...
private
int
run_finish(gs_main_instance* minst, int code, int exit_code, ref* perror_object)
{
	switch(code) {
	case e_Quit:
	case 0:
                    ...
                    const char* filename)
{
	if(strcmp(command, "stdout") == 0) {
		...
		if(minst->heap->gs_lib_ctx->fstdout2 &&
		   (minst->heap->gs_lib_ctx->fstdout2 !=
		    minst->heap->gs_lib_ctx->fstdout) &&
		   (minst->heap->gs_lib_ctx->fstdout2 !=
		    minst->heap->gs_lib_ctx->fstderr)) {
 ...
 */
private
const char help_usage1[] = "\
Usage: gs [switches] [file1.ps file2.ps ...]\n\
Most frequently used switches: (you can use # in place of =)\n\
 -dNOPAUSE           no pause after page   | -q       `quiet', fewer messages\n\
 -g<width>x<height>  page size in pixels   | -r<res>  pixels/inch resolution\n";
private
const char help_usage2[] = "\
...
private
const char help_devices[] = "Available devices:";
private
const char help_default_device[] = "Default output device:";
private
const char help_emulators[] = "Input formats:";
private
const char help_paths[] = "Search path:";
...
print_revision(const gs_main_instance* minst)
{
	...
	outprintf(minst->heap, " (%d-%02d-%02d)\n%s\n",
	          ...
	          (int)(gs_revisiondate % 100), gs_copyright);
}
...
print_devices(const gs_main_instance* minst)
{
	...
	outprintf(minst->heap, "%s", help_devices);
	{
		...
		for(i = 0; (pdev = gs_getdevice(i)) != 0; i++)
			;
		ndev = (size_t)i;
		names = (const char**)gs_alloc_bytes(
		    minst->heap, ndev * sizeof(const char*), "print_devices");
		if(names ==
		   (const char**)NULL) { /* old-style unsorted device list */
			for(i = 0; (pdev = gs_getdevice(i)) != 0; i++) {
			...
			gs_free(minst->heap, (char*)names,
			        ndev * sizeof(const char*), 1, "print_devices");
		}
	}
	outprintf(minst->heap, "\n");
}
...
print_emulators(const gs_main_instance* minst)
{
	outprintf(minst->heap, "%s", help_emulators);
	{
		...
		for(pes = gs_emulator_name_array; pes->value.const_bytes != 0;
		    pes++)
			/*
			 * Even though gs_emulator_name_array is declared and
			 * used as
			 * an array of string refs, each string is actually a
			 * (null terminated) C string.
			 */
			outprintf(minst->heap, " %s",
			          (const char*)pes->value.const_bytes);
	}
	outprintf(minst->heap, "\n");
}
...
print_paths(gs_main_instance* minst)
{
	...
	gs_main_set_lib_paths(minst);
	{
		...
		for(i = 0; i < count; ++i) {
			 ...
			 * platforms rely on all console output being funneled
			 through
...
print_help_trailer(const gs_main_instance* minst)
{
	...
	if(gp_file_name_combine(gs_doc_directory, strlen(gs_doc_directory),
	                        use_htm, strlen(use_htm), false, buffer,
	                        &blen) != gp_combine_success)
		p = use_htm;
