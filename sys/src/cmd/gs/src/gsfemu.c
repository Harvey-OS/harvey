...
#define extend(lc, ia)                                                         \
	((lc)[msw] =                                                           \
	     ((ia)&0x80000000) + (((ia)&0x7fffffff) >> 3) + 0x38000000,        \
	 (lc)[lsw] = (ia) << 29)
...
__adddf3(double a, double b)
{
	...
	if((la[msw] ^ lb[msw]) >= 0) { /* Adding numbers of the same sign. */
		...
		if(shift == 0 &&
		   (msb > msa ||
		    (msb == msa && lsb >= lsa))) { /* This is the only case
...
#define chop_ls(ulx, j, k)                                                     \
	j = (ulx[lsw] >> 11) & 0x3fff, k = (ulx[lsw] & 0x7ff) << 3
	chop_ms(ula, H, I);
	chop_ms(ulb, h, i);
#undef chop
#define prod(m, n) ((ulong)(m) * (n)) /* force long multiply */
	...
	if((ula[lsw] | ulb[lsw]) & 0x1ffffff) { /* No luck. */
		...
		chop_ls(ulb, j, k);
		{
			...
			uint32_t p4 =
			    prod(I, k) + prod(J, j) + prod(K, i) + (p5 >> 14);
			uint32_t p3 = prod(H, k) + prod(I, j) + prod(J, i) +
			              prod(K, h) + (p4 >> 14);
	...
	expt =
	    (la[msw] & 0x7ff00000) + (lb[msw] & 0x7ff00000) - (dx_bias << 20);
...
__divdf3(double a, double b)
{
	...
	if(!(lb[msw] & 0x7fffffff)) { /* Division by zero. */
		...
		lc[lsw] = 0;
		lc[msw] =
		    (la[msw] & 0x7fffffff ? sign + 0x7ff00000 /* infinity */
		                          : 0x7ff80000 /* NaN */);
		return dc;
	}
	...
	for(i = 0; i < 5; ++i) {
		...
		if(msp > msa ||
		   (lsp > lsa && msp == msa)) { /* q was too large. */
			--q;
			if(lsb > lsp)
				msp--;
			lsp -= lsb;
...
#define upb ((const ulong*)pb)
	if(pa[msw] == pb[msw]) {
		int result =
		    (upa[lsw] < upb[lsw] ? -1 : upa[lsw] > upb[lsw] ? 1 : 0);
...
{ /* This routine does check for overflow. */
	...
	if(expt <= 20)
		i >>= 20 - expt;
	else if(expt >= 31 && (expt > 31 || i != 0x100000 || la[msw] >= 0 ||
	                       ula[lsw] >= 1L << 21)) {
		raise(SIGFPE);
...
{ /* This routine does check for overflow, but it converts */
	...
	if((la[msw] & 0x7ff00000) < 0x38100000)
		...
		lc = (la[msw] & 0xc0000000) + ((la[msw] & 0x07ffffff) << 3) +
		     (ula[lsw] >> 29);
