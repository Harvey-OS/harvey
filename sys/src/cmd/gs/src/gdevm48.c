...
/* Procedures */
declare_mem_procs(mem_true48_copy_mono, mem_true48_copy_color,
                  mem_true48_fill_rectangle);

/* The device descriptor. */
const gx_device_memory mem_true48_device = mem_full_alpha_device(
    "image48", 48, 0, mem_open, gx_default_rgb_map_rgb_color,
    gx_default_rgb_map_color_rgb, mem_true48_copy_mono, mem_true48_copy_color,
    mem_true48_fill_rectangle, gx_default_map_cmyk_color, gx_default_copy_alpha,
...
#else
#define set_color48_cache(color, a, b, c, d, e, f)                             \
	mdev->color48.abcd = abcd = ((bits32)(d) << 24) |                      \
...
private
int
mem_true48_fill_rectangle(gx_device* dev, int x, int y, int w, int h,
...
private
int
mem_true48_copy_mono(gx_device* dev, const byte* base, int sourcex, int sraster,
				...
				if(sbyte & 0xf0) {
					if(sbyte & 0x80)
						put6(pptr, a1, b1, c1, d1, e1,
						     f1);
					if(sbyte & 0x40)
						put6(pptr + 6, a1, b1, c1, d1,
						     e1, f1);
					if(sbyte & 0x20)
						put6(pptr + 12, a1, b1, c1, d1,
						     e1, f1);
					if(sbyte & 0x10)
						put6(pptr + 18, a1, b1, c1, d1,
						     e1, f1);
				}
				if(sbyte & 0xf) {
					if(sbyte & 8)
						put6(pptr + 24, a1, b1, c1, d1,
						     e1, f1);
					if(sbyte & 4)
						put6(pptr + 30, a1, b1, c1, d1,
						     e1, f1);
					if(sbyte & 2)
						put6(pptr + 36, a1, b1, c1, d1,
						     e1, f1);
					if(sbyte & 1)
						put6(pptr + 42, a1, b1, c1, d1,
						     e1, f1);
				}
				pptr += 8 * PIXEL_SIZE;
				count -= 8;
...
private
int
mem_true48_copy_color(gx_device* dev, const byte* base, int sourcex,
...
/* Procedures */
declare_mem_procs(mem48_word_copy_mono, mem48_word_copy_color,
                  mem48_word_fill_rectangle);

/* Here is the device descriptor. */
const gx_device_memory mem_true48_word_device =
    mem_full_device("image48w", 48, 0, mem_open, gx_default_rgb_map_rgb_color,
                    gx_default_rgb_map_color_rgb, mem48_word_copy_mono,
                    mem48_word_copy_color, mem48_word_fill_rectangle,
...
private
int
mem48_word_copy_mono(gx_device* dev, const byte* base, int sourcex, int sraster,
                     gx_bitmap_id id, int x, int y, int w, int h,
                     gx_color_index zero, gx_color_index one)
{
	gx_device_memory* const mdev = (gx_device_memory*)dev;
	...
	mem_true48_copy_mono(dev, base, sourcex, sraster, id, x, y, w, h, zero,
	                     one);
...
private
int
mem48_word_copy_color(gx_device* dev, const byte* base, int sourcex,
                      int sraster, gx_bitmap_id id, int x, int y, int w, int h)
{
	gx_device_memory* const mdev = (gx_device_memory*)dev;
	byte* row;
