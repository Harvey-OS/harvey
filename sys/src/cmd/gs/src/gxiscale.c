...
private
irender_proc(image_render_interpolate);
irender_proc_t
 ...
 */
#if defined(USE_MITCHELL_FILTER) &&                                            \
    defined(USE_CONSERVATIVE_INTERPOLATION_RULES)
	/*
	...
	{
		uint out_size = iss.WidthOut *
		                max(iss.Colors * (iss.BitsPerComponentOut / 8),
...
#endif
	pss = (stream_image_scale_state*)s_alloc_state(mem, template->stype,
	                                               "image scale state");
	if(line == 0 || pss == 0 ||
...
private
int
image_render_interpolate(gx_image_enum* penum, const byte* buffer, int data_x,
                         uint iw, int h, gx_device* dev)
{
...

	{
		...
		for(;;) {
			...
			w.limit = out +
			          width * max(c * sizeofPixelOut,
			                      arch_sizeof_color_index) -
			          1;
			w.ptr = w.limit - width * c * sizeofPixelOut;
			psrc = (const frac*)(w.ptr + 1);
			status = (*pss->template->process)((stream_state*)pss,
			                                   &r, &w, h == 0);
			...
			if(w.ptr == w.limit) {
...
#endif
					code = (*pconcs->type
					             ->remap_concrete_color)(
					    psrc, pcs, &devc, pis, dev,
					    gs_color_select_source);
					if(code < 0)
						return code;
					if(color_is_pure(&devc)) {
						/* Just pack colors into a scan
						 * line. */
				...
				LINE_ACCUM_COPY(dev, out, bpp, xo, x, raster,
				                ry);
				penum->line_xy++;
				if_debug0('B', "\n");
			}
