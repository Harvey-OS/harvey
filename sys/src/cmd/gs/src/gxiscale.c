			 ...
			 int data_x, uint iw, int h, gx_device *dev)
{
...

	{
		...
		for(;;) {
			...
			w.limit = out + width * max(c * sizeofPixelOut, arch_sizeof_color_index) - 1;
			w.ptr = w.limit - width * c * sizeofPixelOut;
			psrc = (const frac *)(w.ptr + 1);
			status = (*pss->template->process)((stream_state *)pss, &r, &w, h == 0);
...
#endif
					code = (*pconcs->type->remap_concrete_color)(psrc, pcs, &devc, pis, dev, gs_color_select_source);
					if(code < 0)
						return code;
