...
const byte ref_type_properties[] = {REF_TYPE_PROPERTIES_DATA};

/* Copy refs from one place to another. */
int
refcpy_to_old(ref* aref, uint index, const ref* from, uint size,
...
void
refcpy_to_new(ref* to, const ref* from, uint size, gs_dual_memory_t* idmemory)
{
	while(size--)
...
obj_eq(const gs_memory_t* mem, const ref* pref1, const ref* pref2)
{
	...
	if(r_type(pref1) != r_type(pref2)) {
		...
		case t_mixedarray:
		case t_shortarray:
			return r_is_array(pref2) && r_size(pref1) == 0 &&
			       r_size(pref2) == 0;
		default:
	...
	case t_fontID: { /*
	                  * In the Adobe implementations, different scalings of
	                  * a
 ...
 */
int
obj_string_data(const gs_memory_t* mem, const ref* op, const byte** pchars,
                uint* plen)
{
	switch(r_type(op)) {
	case t_name: {
 ...
 */
private
void ensure_dot(char*);
int
obj_cvp(const ref* op, byte* str, uint len, uint* prlen, int full_print,
        uint start_pos, const gs_memory_t* mem)
{
	...
	if(full_print) {
		...
		case t_real: {
			 ...
			 * single-precision floats (24 bits of mantissa), the
			 * ANSI %g
			 * default of 6 digits is not enough; 9 are needed.
			 * Unfortunately, using %.9g for floats (as opposed to
			 * doubles)
			 * produces unfortunate artifacts such as 0.01 5 mul
			 * printing as
			 * 0.049999997.  Therefore, we print using %g, and if
			 * the result
			 * isn't accurate enough, print again using %.9g.
			 * Unfortunately, a few PostScript programs 'know' that
			 * the
			 * printed representation of floats fits into 6 digits
			 * (e.g.,
			 * with cvs).  We resolve this by letting cvs, cvrs, and
			 * = do
			 * what the Adobe interpreters appear to do (use %g),
			 * and only
		...
		case t_operator:
		case t_oparray:
			code = obj_cvp(op, (byte*)buf + 2, sizeof(buf) - 4,
			               &size, 0, 0, mem);
			if(code < 0)
				return code;
		...
		case t_name:
			...
			if(start_pos > 0)
				return obj_cvp(op, str, len, prlen, 0,
				               start_pos - 1, mem);
			if(len < 1)
		...
		case t_string:
			...
			{
				bool truncate =
				    (full_print == 1 && size > CVP_MAX_STRING);
				...
				r.limit =
				    r.ptr + (truncate ? CVP_MAX_STRING : size);
				while(skip && status == 1) {
					uint written;

					w.ptr = (byte*)buf - 1;
					w.limit = w.ptr +
					          min(skip + len1, sizeof(buf));
					status = s_PSSE_template.process(
					    NULL, &r, &w, false);
					written = w.ptr - ((byte*)buf - 1);
					if(written > skip) {
						written -= skip;
						memcpy(wstr, buf + skip,
						       written);
						wstr += written;
						skip = 0;
						break;
				...
				/*
				 * We can reach here with status == 0 (and skip
				 * != 0) if
		...
		case t_struct:
			...
			data = (const byte*)gs_struct_type_name_string(
			    gs_object_type(
			        (gs_memory_t*)mem,
	...
	case t_oparray: {
		...
		name_index_ref(mem, opt->nx_table[index - opt->base_index],
		               &nref);
...
{
	int code =
	    obj_cvp(op, str, len, prlen, 0, 0, mem); /* NB: NULL memptr */
...
op_find_index(const ref* pref /* t_operator */)
{
	...
	for(; opp < opend; ++opp) {
		...
		for(; def->oname != 0; ++def)
			if(def->proc == proc)
				return (opp - op_defs_all) * OP_DEFS_MAX_SIZE +
				       (def - *opp);
	}
...
op_index_ref(uint index, ref* pref)
{
	...
	make_tasv(pref, t_oparray, opt->attrs, index, const_refs,
	          (opt->table.value.const_refs + index - opt->base_index));
}
...
int
process_float_array(const gs_memory_t* mem, const ref* parray, int count,
                    float* pval)
{
	int code = 0, indx0 = 0;
	...
	if(r_has_type(parray, t_array))
		return float_params(parray->value.refs + count - 1, count,
		                    pval);
	...
	while(count > 0 && code >= 0) {
		...
		subcount =
		    (count > countof(ref_buff) ? countof(ref_buff) : count);
		for(i = 0; i < subcount && code >= 0; i++)
			code = array_get(mem, parray, (int32_t)(i + indx0),
			                 &ref_buff[i]);
		if(code >= 0)
			code = float_params(ref_buff + subcount - 1, subcount,
			                    pval);
		count -= subcount;
		pval += subcount;
		indx0 += subcount;
                ...
                gs_ref_memory_t* imem)
{
	...
	for(i = 5; i >= 0; i--, aptr++, pel++) {
		if(idmemory) {
			...
			make_tav(aptr, t_real, imemory_new_mask(imem), realval,
			         *pel);
		}
	}
	return 0;
