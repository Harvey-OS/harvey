...
/* $Id: scfe.c,v 1.5 2002/06/16 03:58:14 lpd Exp $ */
...
private
stats_runs_t stats_white_runs, stats_black_runs;
...
print_run_stats(const stats_runs_t* stats)
{
	...
	for(i = 0, total = 0; i < 41; i++)
		dprintf1(" %lu", stats->make_up[i]), total += stats->make_up[i];
	dprintf1(" total=%lu\n\t", total);
...
private
void s_CFE_release(stream_state*);
...
/* Flush the buffer */
private
void cf_encode_1d(stream_CFE_state*, const byte*, stream_cursor_write*);
private
void cf_encode_2d(stream_CFE_state*, const byte*, stream_cursor_write*,
                  const byte*);
private
int
s_CFE_process(stream_state* st, stream_cursor_read* pr, stream_cursor_write* pw,
              bool last)
{
	stream_CFE_state* const ss = (stream_CFE_state*)st;
	...
	for(;;) {
		...
		if_debug6('w', "    pr = 0x%lx(%d)0x%lx, pw = 0x%lx(%d)0x%lx\n",
		          (ulong)pr->ptr, (int)(rlimit - pr->ptr),
		          (ulong)rlimit, (ulong)pw->ptr,
		          (int)(wlimit - pw->ptr), (ulong)wlimit);
		if(ss->write_count) {
			...
			memcpy(pw->ptr + 1,
			       ss->lcode + ss->code_bytes - ss->write_count,
			       ccount);
			pw->ptr += ccount;
		...
		if(ss->read_count) {
			...
			if(rcount == 0 && last)
				break;
			memcpy(ss->lbuf + raster - ss->read_count, pr->ptr + 1,
			       ccount);
			pr->ptr += ccount;
		...
		/*
		 * Write the output directly to the caller's buffer if it's
		 * large
		...
		if(w.limit == wlimit)
			...
			ss->write_count = ss->code_bytes =
			    w.ptr - (ss->lcode - 1);
		if(ss->K != 0) {
			/* In 2-D modes, swap the current and previous scan
			 * lines. */
			byte* temp = ss->lbuf;
...
out:
	if_debug9('w', "[w]CFE exit %d: read_count = %d, write_count = %d,\n   "
	          ...
	          status, ss->read_count, ss->write_count, (ulong)pr->ptr,
	          (int)(rlimit - pr->ptr), (ulong)rlimit, (ulong)pw->ptr,
	          (int)(wlimit - pw->ptr), (ulong)wlimit);
#ifdef DEBUG
             ...
             const byte* lprev)
{
	 ...
	 */
	static const byte initial_count_bit[8] = {0, 1,    2,    4,
	                                          8, 0x10, 0x20, 0x40};
	static const byte further_count_bit[8] = {0x80, 1,    2,    4,
	                                          8,    0x10, 0x20, 0x40};
...
#define b1 (a1 - diff) /* only for printing */
	...
	pass:
		...
		if(diff <= -2) {
			...
			if(prev_count != end_count)
				skip_black_pixels(prev_data, prev_p, prev_count,
				                  invert, rlen);
			if(prev_count > a1) {
				/* Use pass mode. */
				if_debug4('W', "[W]pass: count = %d, a1 = %d, "
				               "b1 = %d, new count = %d\n",
				          a0, a1, b1, prev_count);
		...
		if(diff <= 3 && diff >= -3) {
			...
			if_debug5('W', "[W]vertical %d: count = %d, a1 = %d, "
			               "b1 = %d, new count = %d\n",
			          diff, a0, a1, b1, count);
			hc_put_code(ss, q, cp);
		...
		if(invert == invert_white) {
			if_debug3('W', "[W]horizontal: white = %d, black = %d, "
			               "new count = %d\n",
			          a0, a1, count);
			CF_PUT_WHITE_RUN(ss, a0);
			CF_PUT_BLACK_RUN(ss, a1);
		} else {
			if_debug3('W', "[W]horizontal: black = %d, white = %d, "
			               "new count = %d\n",
			          a0, a1, count);
...
/* Stream template */
const stream_template s_CFE_template = {&st_CFE_state, s_CFE_init,
                                        s_CFE_process, 1, 1, s_CFE_release,
                                        s_CFE_set_defaults};
