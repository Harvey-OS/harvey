	...
	do {
		...
		if(p == NULL) {
			if(n < m)
				...
				m = sizeof(buf) - 1 - l;
				if(!m) {
					eprintf1("The procset %s contains a "
					         "too long line.",
					         fname);
...
copy_procsets(stream* s, const gs_param_string* path, bool HaveTrueTypes)
{
	...
	if(p != NULL) {
		for(;; i++) {
			const byte* c =
			    memchr(p, gp_file_name_list_separator, e - p);
			int k;
			...
			if(l > 0) {
				...
				if(HaveTrueTypes || k < 0) {
					code = copy_ps_file_stripping(
					    s, fname, HaveTrueTypes);
					if(code < 0)
						return code;
				}
			}
...
{
	stream_state* st =
	    s_alloc_state(mem, t->stype, "pdf_open_document.encode");
	...
	if(!is_in_page(pdev) && pdf_stell(pdev) == 0) {
		...
		if(pdev->ForOPDFRead && pdev->OPDFReadProcsetPath.size) {
			...
			if(pdev->params.CompressPages ||
			   pdev->CompressEntireFile) {
				/*  When CompressEntireFile is true and
				   ASCII85EncodePages is false,
				    the ASCII85Encode filter is applied, rather
				   one may expect the opposite.
				    Keeping it so due to no demand for this
				   mode.
				    A right implementation should compute the
				   length of the compressed procset,
				    write out an invocation of SubFileDecode
				   filter, and write the length to
				    there assuming the output file is
				   positionable. */
				stream_write(
				    s,
				    (byte*)"currentfile /ASCII85Decode filter "
				           "/LZWDecode filter cvx exec\r",
				    61);
				code = encode(&s, &s_A85E_template,
				              pdev->pdf_memory);
				if(code < 0)
					return code;
				code = encode(&s, &s_LZWE_template,
				              pdev->pdf_memory);
				if(code < 0)
					return code;
			}
			code = copy_procsets(s, &pdev->OPDFReadProcsetPath,
			                     pdev->HaveTrueTypes);
			if(code < 0)
				return code;
...
private
int none_to_stream(gx_device_pdf*), stream_to_text(gx_device_pdf*),
...
private
const context_proc context_procs[4][4] = {
...
int
pdf_encrypt_init(const gx_device_pdf* pdev, gs_id object_id,
                 stream_arcfour_state* psarc4)
{
	byte key[16];

	return s_arcfour_set_key(psarc4, key,
	                         pdf_object_key(pdev, object_id, key));
}
...
none_to_stream(gx_device_pdf* pdev)
{
	...
	if(pdev->ResourcesBeforeUsage) {
		...
		pdev->strm = s;
		if(pdev->compression ==
		   pdf_compress_Flate) { /* Set up the Flate filter. */
			const stream_template* template =
			    &compression_filter_template;
			stream* es =
			    s_alloc(pdev->pdf_memory, "PDF compression stream");
			byte* buf = gs_alloc_bytes(pdev->pdf_memory, sbuf_size,
			                           "PDF compression buffer");
			compression_filter_state* st = gs_alloc_struct(
			    pdev->pdf_memory, compression_filter_state,
	 ...
	 */
	pprintg2(s, "q %g 0 0 %g 0 0 cm\n", 72.0 / pdev->HWResolution[0],
	         72.0 / pdev->HWResolution[1]);
...
stream_to_none(gx_device_pdf* pdev)
{
	...
	if(pdev->ResourcesBeforeUsage) {
		...
		if(pdev->compression_at_page_start ==
		   pdf_compress_Flate) { /* Terminate the Flate filter. */
			stream* fs = s->strm;

			sclose(s);
			gs_free_object(pdev->pdf_memory, s->cbuf,
			               "zlib buffer");
...
const char* const pdf_resource_type_names[] = {PDF_RESOURCE_TYPE_NAMES};
...
int
pdf_cancel_resource(gx_device_pdf* pdev, pdf_resource_t* pres,
                    pdf_resource_type_t rtype)
{
	...
	if(rtype == resourceXObject || rtype == resourceCharProc ||
	   rtype == resourceOther) {
		int code =
		    cos_stream_release_pieces((cos_stream_t*)pres->object);
...
/* Remove a resource. */
void
pdf_forget_resource(gx_device_pdf* pdev, pdf_resource_t* pres1,
                    pdf_resource_type_t rtype)
{ /* fixme : optimize. */
	...
	for(i = 0; i < NUM_RESOURCE_CHAINS; i++) {
		...
		for(; (pres = *pprev) != 0; pprev = &pres->next)
			if(pres == pres1) {
				*pprev = pres->next;
				COS_RELEASE(pres->object,
				            "pdf_forget_resource");
				gs_free_object(pdev->pdf_memory, pres->object,
				               "pdf_forget_resource");
				gs_free_object(pdev->pdf_memory, pres,
				               "pdf_forget_resource");
				break;
			}
	}
...
pdf_substitute_resource(gx_device_pdf* pdev, pdf_resource_t** ppres,
                        pdf_resource_type_t rtype,
                        int (*eq)(gx_device_pdf* pdev, pdf_resource_t* pres0,
...
pdf_resource_t*
pdf_find_resource_by_resource_id(gx_device_pdf* pdev, pdf_resource_type_t rtype,
                                 gs_id id)
{
	pdf_resource_t** pchain = pdev->resources[rtype].chains;
	pdf_resource_t* pres;
...
/* Find same resource. */
int
pdf_find_same_resource(gx_device_pdf* pdev, pdf_resource_type_t rtype,
	...
	for(i = 0; i < NUM_RESOURCE_CHAINS; i++) {
		for(pres = pchain[i]; pres != 0; pres = pres->next) {
			if(!pres->named && *ppres != pres) {
				...
				int code =
				    pco0->cos_procs->equal(pco0, pco1, pdev);
	...
	for(i = 0; i < NUM_RESOURCE_CHAINS; i++) {
		...
		for(; (pres = *pprev) != 0;) {
			if(cond(pdev, pres)) {
				*pprev = pres->next;
				pres->next =
				    pres; /* A temporary mark - see below */
			} else
				pprev = &pres->next;
		}
	}
...
pdf_write_and_free_all_resource_objects(gx_device_pdf* pdev)
{
	...
	code1 = pdf_finish_font_descriptors(
	    pdev, pdf_release_FontDescriptor_components);
	if(code >= 0)
		code = code1;
	for(i = 0; i < NUM_RESOURCE_TYPES; ++i) {
...
pdf_page_id(gx_device_pdf* pdev, int page_num)
{
	...
	if(page_num >= pdev->num_pages) { /* Grow the pages array. */
		uint new_num_pages = max(page_num + 10, pdev->num_pages << 1);
		pdf_page_t* new_pages = gs_resize_object(
		    pdev->pdf_memory, pdev->pages, new_num_pages,
...
/* Write matrix values. */
void
pdf_put_matrix(gx_device_pdf* pdev, const char* before, const gs_matrix* pmat,
               const char* after)
{
	stream* s = pdev->strm;
...
private
int
pdf_encrypt_encoded_string(const gx_device_pdf* pdev, const byte* str,
                           uint size, gs_id object_id)
{
...
private
int
pdf_put_encoded_string(const gx_device_pdf* pdev, const byte* str, uint size,
                       gs_id object_id)
{
...
private
int
pdf_put_encoded_hex_string(const gx_device_pdf* pdev, const byte* str,
                           uint size, gs_id object_id)
{
...
private
int
pdf_put_composite(const gx_device_pdf* pdev, const byte* vstr, uint size,
                  gs_id object_id)
{
	if(!pdev->KeyLength || object_id == (gs_id)-1) {
		...
		for(; l > 0;) {
			if(*p == '(')
				n = pdf_encrypt_encoded_string(pdev, p, l,
				                               object_id);
			else {
...
#define TEMPLATE_IS(atemp) (template->process == (atemp).process)
		if(TEMPLATE_IS(s_A85E_template))
			...
			decode_parms = cos_dict_alloc(
			    pdev, "pdf_put_image_filters(decode_parms)");
			if(decode_parms == 0)
				return_error(gs_error_VMerror);
			CHECK(cos_param_list_writer_init(&writer, decode_parms,
			                                 0));
			/*
			...
			if(cfs.EndOfBlock)
				cfs.Rows = 0;
			CHECK(s_CF_get_params((gs_param_list*)&writer, &cfs,
			                      false));
			...
			CHECK(cos_dict_put_c_key_int(decode_parms, "/Columns",
			                             ss->Columns));
			if(ss->Colors != 1)
				CHECK(cos_dict_put_c_key_int(
				    decode_parms, "/Colors", ss->Colors));
			if(ss->BitsPerComponent != 8)
				CHECK(cos_dict_put_c_key_int(
				    decode_parms, "/BitsPerComponent",
				    ss->BitsPerComponent));
...
#undef TEMPLATE_IS
	...
	if(filter_name) {
		if(binary_ok) {
			CHECK(cos_dict_put_c_strings(pcd, pfn->Filter,
			                             filter_name));
			if(decode_parms)
				CHECK(cos_dict_put_c_key_object(
				    pcd, pfn->DecodeParms,
				    COS_OBJECT(decode_parms)));
		} else {
			cos_array_t* pca = cos_array_alloc(
			    pdev, "pdf_put_image_filters(Filters)");
			...
			if(decode_parms) {
				pca = cos_array_alloc(
				    pdev, "pdf_put_image_filters(DecodeParms)");
				if(pca == 0)
					return_error(gs_error_VMerror);
				CHECK(cos_array_add_c_string(pca, "null"));
				CHECK(cos_array_add_object(
				    pca, COS_OBJECT(decode_parms)));
				CHECK(cos_dict_put_c_key_object(
				    pcd, pfn->DecodeParms, COS_OBJECT(pca)));
			}
		}
	} else if(!binary_ok)
		CHECK(cos_dict_put_c_strings(pcd, pfn->Filter,
...
{
	const stream_template* template =
	    (pdev->CompatibilityLevel < 1.3 ? &s_LZWE_template
	                                    : &s_zlibE_template);
...
{
	return pdf_begin_data_stream(
	    pdev, pdw, DATA_STREAM_BINARY | DATA_STREAM_COMPRESS, 0);
}
...
#define USE_FLATE 2
	...
	if(!(options & DATA_STREAM_NOLENGTH)) {
		stream_puts(s,
		            (pdev->CompatibilityLevel < 1.3 ? fnames1_2[filters]
		                                            : fnames[filters]));
		if(pdev->ResourcesBeforeUsage) {
...
int
pdf_begin_data_stream(gx_device_pdf* pdev, pdf_data_writer_t* pdw, int options,
                      gs_id object_id)
{
	int code;
	...
	pdw->pdev = pdev; /* temporary for backward compatibility of
	                     pdf_end_data prototype. */
	pdw->binary.target = pdev->strm;
	pdw->binary.dev = (gx_device_psdf*)pdev;
	pdw->binary.strm = 0; /* for GC in case of failure */
	code = pdf_open_aside(pdev, resourceOther, gs_no_id, &pdw->pres,
	                      !object_id, options);
	if(object_id != 0)
...
private
int pdf_function_array(gx_device_pdf* pdev, cos_array_t* pca,
                    ...
                    const gs_range_t* pranges, cos_value_t* pvalue)
{
		...
		return pdf_function(pdev, pfn, pvalue);
	{
		 ...
		 * represent the inverse scaling from what
		 * gs_function_make_scaled
		 * expects.
		 */
		gs_memory_t* mem = pdev->pdf_memory;
		gs_function_t* psfn;
		gs_range_t* ranges = (gs_range_t*)gs_alloc_byte_array(
		    mem, pfn->params.n, sizeof(gs_range_t),
		    "pdf_function_scaled");
		int i, code;
                 ...
                 pdf_resource_t** ppres)
{
	...
	cos_dict_t* pcd;
	int code =
	    pdf_alloc_resource(pdev, resourceFunction, gs_no_id, &pres, -1);
	...
	*ppres = pres;
	...
	if(info.DataSource != 0) {
		...
		if(code >= 0) {
			...
			for(pos = 0; pos < info.data_size; pos += count) {
				...
				data_source_access_only(info.DataSource, pos,
				                        count, buf, &ptr);
	...
	if(info.Functions != 0) {
		...
		if((code = pdf_function_array(pdev, functions, &info)) < 0 ||
		   (code = cos_dict_put_c_key(
		        pcd, "/Functions", COS_OBJECT_VALUE(&v, functions))) <
		       0) {
...
private
int
functions_equal(gx_device_pdf* pdev, pdf_resource_t* pres0,
                pdf_resource_t* pres1)
{
	return true;
}
...
pdf_function(gx_device_pdf* pdev, const gs_function_t* pfn, cos_value_t* pvalue)
{
	...
	if(code < 0)
		return code;
	code = pdf_substitute_resource(pdev, &pres, resourceFunction,
...
private
int
pdf_function_array(gx_device_pdf* pdev, cos_array_t* pca,
                   const gs_function_info_t* pinfo)
{
	int i, code = 0;
	cos_value_t v;
...
int
pdf_write_function(gx_device_pdf* pdev, const gs_function_t* pfn, int32_t* pid)
{
	cos_value_t value;
...
pdf_write_font_bbox(gx_device_pdf* pdev, const gs_int_rect* pbox)
{
	...
	pprintd4(s, "/FontBBox[%d %d %d %d]", pbox->p.x, pbox->p.y, x, y);
	return 0;
}
