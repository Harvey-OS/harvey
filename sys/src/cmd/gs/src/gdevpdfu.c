...
private
int
none_to_stream(gx_device_pdf *),
    stream_to_text(gx_device_pdf *),
...
pdf_write_resource_objects(gx_device_pdf *pdev, pdf_resource_type_t rtype)
{
	...
	for(j = 0; j < NUM_RESOURCE_CHAINS && code >= 0; ++j) {
		...
		for(; pres != 0; pres = pres->next)
			if((!pres->named || pdev->ForOPDFRead) && !pres->object->written)
...
{
	const stream_template *template = (pdev->CompatibilityLevel < 1.3 ? &s_LZWE_template : &s_zlibE_template);
...
#define USE_FLATE 2
	...
	if(!(options & DATA_STREAM_NOLENGTH)) {
		stream_puts(s, (pdev->CompatibilityLevel < 1.3 ? fnames1_2[filters] : fnames[filters]));
		if(pdev->ResourcesBeforeUsage) {
