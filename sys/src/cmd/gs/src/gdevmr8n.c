			       ...
			       int phase_x, int phase_y, gs_logical_operation_t lop)
{
	 ...
	 */
		...
		default:
		df:
			return mem_default_strip_copy_rop(dev,
...
#define rop_body_8(s_pixel, t_pixel)                                                                 \
	if((s_pixel) == strans || /* So = 0, s_tr = 1 */ (t_pixel) == ttrans /* Po = 0, p_tr = 1 */) \
		continue;                                                                            \
		 ...
		 */
				if(bpp == 1) {
					...
					for(; left > 0; dptr += 3, sptr += 3, tptr += 3, ++sx, ++tx, --left) {
						bits32 s_pixel =
						    (scolors ? cbit24(srow, sx, scolors) : get24(sptr));
						bits32 t_pixel =
						    (tcolors ? cbit24(tptr, tx, tcolors) : get24(tptr));
