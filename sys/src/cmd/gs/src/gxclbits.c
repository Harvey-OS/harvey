	     ...
	     int compression_mask, byte **pdp, uint *psize)
{
	...
	if(short_size >= 50 &&
		...
		*psize = try_size;
		code = (pcls != 0 ? set_cmd_put_op(dp, cldev, pcls, 0, try_size) : set_cmd_put_all_op(dp, cldev, 0, try_size));
		if(code < 0)
			return code;
	 ...
	 */
		...
		*psize = op_size + short_size;
		code = (pcls != 0 ? set_cmd_put_op(dp, cldev, pcls, 0, *psize) : set_cmd_put_all_op(dp, cldev, 0, *psize));
		if(code < 0)
			return code;
...
cmd_size_tile_params(const gx_strip_bitmap *tile)
{
	return 2 + cmd_size_w(tile->rep_width) + cmd_size_w(tile->rep_height) +
	       (tile->rep_width == tile->size.x ? 0 : cmd_size_w(tile->size.x / tile->rep_width)) +
	       (tile->rep_height == tile->size.y ? 0 : cmd_size_w(tile->size.y / tile->rep_height)) +
...
top:
	if(clist_find_bits(cldev, tiles->id, &loc)) { /* The bitmap is in the cache.  Check whether this band */
		/* knows about it. */
			...
			{ /*
				...
				int code =
				    cmd_put_bits(cldev, pcls, ts_bits(cldev, loc.tile),
						 ...
						 (cldev->tile_params.size.x > tiles->rep_width ? decompress_elsewhere | decompress_spread : decompress_elsewhere),
						 &dp, &csize);
