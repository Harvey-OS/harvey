...
/* Copyright (C) 1995, 1996, 1997, 1998, 1999 Aladdin Enterprises.  All rights
  reserved.
                   ...
                   uint* width_bytes, uint* raster)
{
	...
	else if(short_raster <= cmd_max_short_width_bytes || height <= 1 ||
	...
	return (height == 0 ? 0
	                    : *width_bytes * (height - 1) + width_bytes_last);
}
                    ...
                    uint raster, uint height, stream_cursor_write* pw)
{
	...
	if(raster == width_bytes) {
		...
		for(y = 1; (r.limit = r.ptr + width_bytes), y < height; ++y) {
			...
			if(status)
				break;
			if(r.ptr != r.limit) { /* We don't attempt to handle
			                          compressors that */
...
cmd_put_bits(gx_device_clist_writer* cldev, gx_clist_state* pcls,
             const byte* data, uint width_bits, uint height, uint raster,
             int op_size, int compression_mask, byte** pdp, uint* psize)
{
	uint short_raster, full_raster;
	uint short_size = clist_bitmap_bytes(
	    width_bits, height, compression_mask & ~cmd_mask_compress_any,
	...
	if(short_size >= 50 &&
		...
		*psize = try_size;
		code = (pcls != 0 ? set_cmd_put_op(dp, cldev, pcls, 0, try_size)
		...
		*psize = op_size + short_size;
		code = (pcls != 0 ? set_cmd_put_op(dp, cldev, pcls, 0, *psize)
...
cmd_size_tile_params(const gx_strip_bitmap* tile)
{
	return 2 + cmd_size_w(tile->rep_width) + cmd_size_w(tile->rep_height) +
	       (tile->rep_width == tile->size.x
	            ? 0
	            : cmd_size_w(tile->size.x / tile->rep_width)) +
	       (tile->rep_height == tile->size.y
	            ? 0
                  ...
                  int comp_num, const gx_transfer_map* map, gs_id* pid)
{
	...
	if(map == 0) {
		...
		if(map->proc == gs_identity_transfer) {
			code =
			    set_cmd_put_all_op(dp, cldev, cmd_opv_set_misc, 3);
			if(code < 0)
				return code;
			dp[1] = cmd_set_misc_map + (cmd_map_identity << 4) +
			        map_index;
			dp[2] = comp_num;
		} else {
			...
			if(code < 0)
				return code;
			dp[1] =
			    cmd_set_misc_map + (cmd_map_other << 4) + map_index;
			dp[2] = comp_num;
...
clist_delete_tile(gx_device_clist_writer* cldev, tile_slot* slot)
{
	...
	while((offset = table[index = tile_hash_next(index) & mask].offset) !=
	      0) {
		tile_slot* tile = (tile_slot*)(cldev->data + offset);
		tile_loc loc;

		if(!clist_find_bits(cldev, tile->id,
		                    &loc)) { /* We didn't find it, so it should
		                                be moved into a slot */
...
#define slot ((tile_slot*)slot_head)
	...
	if(cldev->bits.csize ==
	   cldev->tile_max_count) { /* Don't let the hash table get too full:
	                               delete an entry. */
		...
		if(slot_head == 0) { /* Wrap around and retry. */
			cldev->bits.cnext = 0;
			gx_bits_cache_alloc(
			    &cldev->bits, (ulong)cldev->chunk.size, &slot_head);
#ifdef DEBUG
			if(slot_head == 0) {
	...
	while(gx_bits_cache_alloc(&cldev->bits, (ulong)tsize, &slot_head) < 0) {
		if(slot_head == 0) {                 /* Wrap around. */
			if(cldev->bits.cnext == 0) { /* Too big to fit.  We
			                                should probably detect
			                                this */
				/* sooner, since if we get here, we've cleared
				 * the */
				/* cache. */
				return_error(gs_error_limitcheck);
			}
	...
	bytes_copy_rectangle(ts_bits(cldev, slot), raster, tiles->data, sraster,
...
top:
	if(clist_find_bits(cldev, tiles->id, &loc)) { /* The bitmap is in the
	                                                 cache.  Check whether
	                                                 this band */
		/* knows about it. */
		...
		if(*bptr & bmask) { /* Already known.  Just set the index. */
			if(pcls->tile_index == loc.index)
				return 0;
			if((code = cmd_put_tile_index(cldev, pcls, loc.index)) <
			   0)
				return code;
		} else {
			uint extra = 0;
			...
			if
				tile_params_differ(cldev, tiles, depth)
				{ /*
				   * We have a cached tile whose parameters
				   * differ from
				   * the current ones.  Because of the way tile
				   * IDs are
				   * managed, this is currently only possible
				   * when mixing
				   * Patterns and halftones, but if we didn't
				   * generate new
				   * IDs each time the main halftone cache
				   * needed to be
				   * refreshed, this could also happen simply
				   * from
				   * switching screens.
				   */
					int band;

					clist_new_tile_params(
					    &cldev->tile_params, tiles, depth,
					    cldev);
					cldev->tile_depth = depth;
					/* No band knows about the new
					 * parameters. */
					for(band = cldev->tile_known_min;
					    band <= cldev->tile_known_max;
					    ++band)
						cldev->states[band].known &=
						    ~tile_params_known;
			...
			if(!(
			       pcls->known &
			       tile_params_known)) { /* We're going to have to
			                                write the tile
			                                parameters. */
				extra =
				    cmd_size_tile_params(&cldev->tile_params);
			}
			{ /*
			   * This band doesn't know this tile yet, so output the
			   * bits.  Note that the offset we write is the one
			   * used by
			   * the reading phase, not the writing phase.  Note
			   * also
			   * that the size of the cached and written tile may
			   * differ
			   ...
			   */
				uint32_t offset =
				    (byte*)loc.tile - cldev->chunk.data;
				uint rsize = extra + 1 + cmd_size_w(loc.index) +
				             cmd_size_w(offset);
				byte* dp;
				uint csize;
				int code = cmd_put_bits(
				    cldev, pcls, ts_bits(cldev, loc.tile),
				    tiles->rep_width * depth, tiles->rep_height,
				    loc.tile->cb_raster, rsize,
				    (cldev->tile_params.size.x >
				             tiles->rep_width
				...
				if(code < 0)
					return code;
				if(extra) { /* Write the tile parameters before
				               writing the bits. */
					cmd_store_tile_params(
					    dp, &cldev->tile_params, depth,
					    extra);
					dp += extra;
					...
					if(band_index < cldev->tile_known_min)
						cldev->tile_known_min =
						    band_index;
					if(band_index > cldev->tile_known_max)
						cldev->tile_known_max =
						    band_index;
				}
				*dp = cmd_count_op(cmd_opv_set_tile_bits,
				                   csize - extra);
				dp++;
	...
	/* The tile is not in the cache, add it. */
	{
		...
		if(tile_params_differ(cldev, tiles,
		                      depth)) { /* We'll reset
		                                   cldev->tile_params when we
		                                   write the bits. */
	...
	if(clist_find_bits(cldev, tiles->id, &loc)) { /* The bitmap is in the
		...
		if(*bptr & bmask) { /* Already known.  Just set the index. */
			...
			uint rsize = 2 + cmd_size_w(loc.tile->width) +
			             ...
			             cmd_size_w(loc.index) + cmd_size_w(offset);
			byte* dp;
			uint csize;
			...
			if(loc.tile->num_bands == CHAR_ALL_BANDS_COUNT)
				bit_pcls = NULL;
			code = cmd_put_bits(
			    cldev, bit_pcls, ts_bits(cldev, loc.tile),
			    loc.tile->width * depth, loc.tile->height,
			    loc.tile->cb_raster, rsize,
			    (1 << cmd_compress_cfe) | decompress_elsewhere, &dp,
			    &csize);
