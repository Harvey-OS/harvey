...
int
dict_ranges_param(const gs_memory_t* mem, const ref* pdref, const char* kstr,
                  int count, gs_range* prange)
{
...
int
dict_proc_array_param(const gs_memory_t* mem, const ref* pdict,
                      const char* kstr, uint count, ref* pparray)
{
	ref* pvalue;
	...
	if(dict_find_string(pdict, kstr, &pvalue) > 0) {
		...
		*pparray = *pvalue;
		...
		make_const_array(pparray, a_readonly | avm_foreign, count,
		                 &empty_procs[0]);
	return 0;
}
...
int
dict_range3_param(const gs_memory_t* mem, const ref* pdref, const char* kstr,
                  gs_range3* prange3)
{
...
int
dict_matrix3_param(const gs_memory_t* mem, const ref* pdref, const char* kstr,
                   gs_matrix3* pmat3)
{
	/*
...
cie_points_param(const gs_memory_t* mem, const ref* pdref, gs_cie_wb* pwb)
{
	...
	if((code = dict_floats_param(mem, pdref, "WhitePoint", 3,
	   ...
	   (code = dict_floats_param(mem, pdref, "BlackPoint", 3,
	                             (float*)&pwb->BlackPoint,
	                             (const float*)&BlackPoint_default)) < 0)
		return code;
	if(pwb->WhitePoint.u <= 0 || pwb->WhitePoint.v != 1 ||
	   pwb->WhitePoint.w <= 0 || pwb->BlackPoint.u < 0 ||
	   pwb->BlackPoint.v < 0 || pwb->BlackPoint.w < 0)
		return_error(e_rangecheck);
	return 0;
}
...
private
int cie_3d_table_param(const ref* ptable, uint count, uint nbytes,
                       gs_const_string* strings);
int
cie_table_param(const ref* ptref, gx_color_lookup_table* pclt, gs_memory_t* mem)
{
	...
	if(n == 3) {
		table = gs_alloc_struct_array(
		    mem, pclt->dims[0], gs_const_string,
		    &st_const_string_element, "cie_table_param");
		if(table == 0)
			return_error(e_VMerror);
		code =
		    cie_3d_table_param(pta + 3, pclt->dims[0], nbytes, table);
	} else { /* n == 4 */
		...
		if(r_size(pta + 4) != d0)
			return_error(e_rangecheck);
		table = gs_alloc_struct_array(mem, ntables, gs_const_string,
		 ...
		 * we initialize code to 0 here solely to pacify stupid
		 * compilers.
		 */
               ...
               ref_cie_procs* pcprocs)
{
	...
	if((code = dict_range3_param(mem, pdref, "RangeLMN", &pcie->RangeLMN)) <
	       0 ||
	   (code = dict_proc3_param(mem, pdref, "DecodeLMN",
	                            &pcprocs->DecodeLMN)) < 0 ||
	   (code = dict_matrix3_param(mem, pdref, "MatrixLMN",
	                              &pcie->MatrixLMN)) < 0 ||
...
private
int
cie_abc_param(const gs_memory_t* mem, const ref* pdref, gs_cie_abc* pcie,
              ref_cie_procs* pcprocs)
{
	int code;

	if((code = dict_range3_param(mem, pdref, "RangeABC", &pcie->RangeABC)) <
	       0 ||
	   (code = dict_proc3_param(mem, pdref, "DecodeABC",
	                            &pcprocs->Decode.ABC)) < 0 ||
	   (code = dict_matrix3_param(mem, pdref, "MatrixABC",
	                              &pcie->MatrixABC)) < 0 ||
...
/* Forward references */
private
int cache_common(i_ctx_t*, gs_cie_common*, const ref_cie_procs*, void*,
                 gs_ref_memory_t*);
private
int cache_abc_common(i_ctx_t*, gs_cie_abc*, const ref_cie_procs*, void*,
                     gs_ref_memory_t*);

/* <dict> .setciedefgspace - */
private
int cie_defg_finish(i_ctx_t*);
private
int
zsetciedefgspace(i_ctx_t* i_ctx_p)
{
	...
	pcie->Table.n = 4;
	pcie->Table.m = 3;
	if((code = dict_ranges_param(mem, op, "RangeDEFG", 4,
	                             pcie->RangeDEFG.ranges)) < 0 ||
	   (code = dict_proc_array_param(mem, op, "DecodeDEFG", 4,
	                                 &procs.PreDecode.DEFG)) < 0 ||
	   (code = dict_ranges_param(mem, op, "RangeHIJK", 4,
	                             pcie->RangeHIJK.ranges)) < 0 ||
	   ...
	   (code = cie_cache_joint(i_ctx_p, &istate->colorrendering.procs,
	                           (gs_cie_common*)pcie, igs)) <
	       0 || /* do this last */
	   (code = cie_cache_push_finish(i_ctx_p, cie_defg_finish, imem,
	                                 pcie)) < 0 ||
	   (code = cie_prepare_cache4(i_ctx_p, &pcie->RangeDEFG,
	                              procs.PreDecode.DEFG.value.const_refs,
	                              &pcie->caches_defg.DecodeDEFG[0], pcie,
	                              imem, "Decode.DEFG")) < 0 ||
	   (code = cache_abc_common(i_ctx_p, (gs_cie_abc*)pcie, &procs, pcie,
	                            imem)) < 0)
		DO_NOTHING;
...
zsetciedefspace(i_ctx_t* i_ctx_p)
{
	...
	pcie->Table.n = 3;
	pcie->Table.m = 3;
	if((code = dict_range3_param(mem, op, "RangeDEF", &pcie->RangeDEF)) <
	       0 ||
	   (code = dict_proc3_param(mem, op, "DecodeDEF",
	                            &procs.PreDecode.DEF)) < 0 ||
	   (code = dict_range3_param(mem, op, "RangeHIJ", &pcie->RangeHIJ)) <
	       0 ||
	   (code = cie_table_param(ptref, &pcie->Table, mem)) < 0 ||
	   (code = cie_abc_param(imemory, op, (gs_cie_abc*)pcie, &procs)) < 0 ||
	   (code = cie_cache_joint(i_ctx_p, &istate->colorrendering.procs,
	   ...
	   (code = cie_prepare_cache3(i_ctx_p, &pcie->RangeDEF,
	                              ...
	                              &pcie->caches_def.DecodeDEF[0], pcie,
	                              imem, "Decode.DEF")) < 0 ||
	   (code = cache_abc_common(i_ctx_p, (gs_cie_abc*)pcie, &procs, pcie,
	                            imem)) < 0)
		DO_NOTHING;
...
zsetcieaspace(i_ctx_t* i_ctx_p)
{
	...
	if(code < 0)
		return code;
	pcie = pcs->params.a;
	if((code = dict_floats_param(imemory, op, "RangeA", 2,
	                             (float*)&pcie->RangeA,
	                             (const float*)&RangeA_default)) < 0 ||
	   (code = dict_floats_param(imemory, op, "MatrixA", 3,
	                             (float*)&pcie->MatrixA,
	                             (const float*)&MatrixA_default)) < 0 ||
	   ...
	   (code = cie_cache_push_finish(i_ctx_p, cie_a_finish, imem, pcie)) <
	       0 ||
	   (code = cie_prepare_cache(i_ctx_p, &pcie->RangeA, &procs.Decode.A,
	                             &pcie->caches.DecodeA.floats, pcie, imem,
	                             "Decode.A")) < 0 ||
	   (code = cache_common(i_ctx_p, &pcie->common, &procs, pcie, imem)) <
	       0)
		DO_NOTHING;
...
cache_abc_common(i_ctx_t* i_ctx_p, gs_cie_abc* pcie,
                 const ref_cie_procs* pcprocs, void* container,
                 gs_ref_memory_t* imem)
{
	int code = cie_prepare_cache3(
	    i_ctx_p, &pcie->RangeABC, pcprocs->Decode.ABC.value.const_refs,
	    pcie->caches.DecodeABC.caches, pcie, imem, "Decode.ABC");

	return (code < 0 ? code : cache_common(i_ctx_p, &pcie->common, pcprocs,
	                                       pcie, imem));
}
...
cache_common(i_ctx_t* i_ctx_p, gs_cie_common* pcie,
             ...
             gs_ref_memory_t* imem)
{
	return cie_prepare_cache3(
	    i_ctx_p, &pcie->RangeLMN, pcprocs->DecodeLMN.value.const_refs,
	    &pcie->caches.DecodeLMN[0], container, imem, "Decode.LMN");
}
...
private
int cie_cache_finish1(i_ctx_t*);
private
int cie_cache_finish(i_ctx_t*);
int
...
cie_prepare_caches_4(i_ctx_t* i_ctx_p, const gs_range* domains,
                     const ref* procs, cie_cache_floats* pc0,
                     cie_cache_floats* pc1, cie_cache_floats* pc2,
                     cie_cache_floats* pc3, void* container,
                     gs_ref_memory_t* imem, client_name_t cname)
{
	...
	for(i = 0; i < n && code >= 0; ++i)
		code = cie_prepare_cache(i_ctx_p, domains + i, procs + i,
		                         pcn[i], container, imem, cname);
	return code;
}
...
cie_cache_finish_store(i_ctx_t* i_ctx_p, bool replicate)
{
	...
	if_debug3('c', "[c]cache 0x%lx base=%g, factor=%g:\n", (uint32_t)pcache,
	          pcache->params.base, pcache->params.factor);
	if(replicate ||
...
const op_def zcie_l2_op_defs[] = {op_def_begin_level2(),
