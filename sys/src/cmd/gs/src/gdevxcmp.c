...
set_cmap_values(x11_cmap_values_t* values, int maxv, int mult)
{
	...
	if(maxv < 1 || maxv > 63 || (maxv & (maxv + 1)) || (mult & (mult - 1)))
		return false;
...
set_std_cmap(gx_device_X* xdev, XStandardColormap* map)
{
	...
	xdev->cman.std_cmap.fast =
	    set_cmap_values(&xdev->cman.std_cmap.red, map->red_max,
	                    map->red_mult) &&
	    set_cmap_values(&xdev->cman.std_cmap.green, map->green_max,
	                    map->green_mult) &&
	    set_cmap_values(&xdev->cman.std_cmap.blue, map->blue_max,
	                    map->blue_mult);
}
...
x_get_std_cmap(gx_device_X* xdev, Atom prop)
{
	...
	if(XGetRGBColormaps(xdev->dpy, RootWindowOfScreen(xdev->scr), &scmap,
	                    &nitems, prop))
...
alloc_std_cmap(gx_device_X* xdev, bool colored)
{
	...
	if(colored) {
		for(cmap->green_max = xdev->vinfo->green_mask,
		cmap->green_mult = 1;
		    (cmap->green_max & 1) == 0;) {
			cmap->green_max >>= 1;
			cmap->green_mult <<= 1;
		}
		for(cmap->blue_max = xdev->vinfo->blue_mask,
		cmap->blue_mult = 1;
		    (cmap->blue_max & 1) == 0;) {
			cmap->blue_max >>= 1;
			cmap->blue_mult <<= 1;
...
{
	if(num_colors > 0) {
		xdev->cman.dynamic.colors = (x11_color_t**)gs_malloc(
		    xdev->memory, sizeof(x11_color_t*), xdev->cman.num_rgb,
		    "x11 cman.dynamic.colors");
		if(xdev->cman.dynamic.colors) {
			int i;

			xdev->cman.dynamic.size = xdev->cman.num_rgb;
			xdev->cman.dynamic.shift =
			    16 - xdev->vinfo->bits_per_rgb;
...
x_alloc_color(gx_device_X* xdev, XColor* xcolor)
{
	...
	if(xcolor->pixel < xdev->cman.color_to_rgb.size) {
		x11_rgb_t* pxrgb =
		    &xdev->cman.color_to_rgb.values[xcolor->pixel];
...
setup_cube(gx_device_X* xdev, int ramp_size, bool colors)
{
	...
	xdev->cman.dither_ramp = (x_pixel*)gs_malloc(
	    xdev->memory, sizeof(x_pixel), num_entries, "gdevx setup_cube");
	...
	for(index = 1; index < num_entries - 1; index++) {
		...
		int q = rgb_index / ramp_size, r = q / ramp_size,
		    g = q % ramp_size, b = rgb_index % ramp_size;
		XColor xc;

		xc.red = (X_max_color_value * r / max_rgb) &
		         xdev->cman.color_mask.red;
		xc.green = (X_max_color_value * g / max_rgb) &
		           xdev->cman.color_mask.green;
		xc.blue = (X_max_color_value * b / max_rgb) &
		          xdev->cman.color_mask.blue;
...
gdev_x_setup_colors(gx_device_X* xdev)
{
	...
	if(xdev->ghostview) {
		Atom gv_colors =
		    XInternAtom(xdev->dpy, "GHOSTVIEW_COLORS", False);
		Atom type;
		int format;
		...
		if(XGetWindowProperty(xdev->dpy, xdev->win, gv_colors, 0, 256,
		                      (xdev->dest != 0), XA_STRING, &type,
		   ...
		   type == XA_STRING) {
			...
			if(nitems != 2 ||
			   (*buf != 'M' && *buf != 'G' && *buf != 'C')) {
				eprintf(
				    "Malformed GHOSTVIEW_COLOR property.\n");
	...
	{ /* Set up the reverse map from pixel values to RGB. */
		...
		xdev->cman.color_to_rgb.values =
		    (x11_rgb_t*)gs_malloc(xdev->memory, sizeof(x11_rgb_t),
		                          count, "gdevx color_to_rgb");
		if(xdev->cman.color_to_rgb.values) {
			int i;

			for(i = 0; i < count; ++i)
				xdev->cman.color_to_rgb.values[i].defined =
				    false;
	...
	case 'C':
		...
		xdev->color_info.max_gray = xdev->color_info.max_color =
		    xdev->cman.num_rgb - 1;
#if HaveStdCMap
		/* Get a standard color map if available */
		if(xdev->vinfo->visual == DefaultVisualOfScreen(xdev->scr)) {
			xdev->cman.std_cmap.map =
			    x_get_std_cmap(xdev, XA_RGB_DEFAULT_MAP);
		} else {
			xdev->cman.std_cmap.map =
			    x_get_std_cmap(xdev, XA_RGB_BEST_MAP);
		}
		if(xdev->cman.std_cmap.map ||
		   (xdev->vinfo->class == TrueColor &&
		    alloc_std_cmap(xdev, true))) {
			    ...
			    xdev->color_info.dither_colors =
			        min(xdev->cman.std_cmap.map->red_max,
			            min(xdev->cman.std_cmap.map->green_max,
			                xdev->cman.std_cmap.map->blue_max)) +
			        1;
			if(xdev->cman.std_cmap.map)
...
#define CBRT(r) pow(r, 1.0 / 3.0)
...
#ifdef DEBUG
					eprintf3("Warning: failed to allocate "
					         "%dx%dx%d RGB cube.\n",
					         ramp_size, ramp_size,
					         ramp_size);
#endif
		...
		alloc_dynamic_colors(xdev,
		                     CUBE(xdev->cman.num_rgb) -
...
#undef CBRT
	...
	grayscale:
		...
		{
			int ramp_size =
			    min(xdev->vinfo->colormap_size / 2,
...
#ifdef DEBUG
					eprintf1("Warning: failed to allocate "
					         "%d level gray ramp.\n",
					         ramp_size);
#endif
					ramp_size /= 2;
	...
	default:
		...
		if(xdev->cman.color_to_rgb.values) {
			gs_x_free(xdev->memory, xdev->cman.color_to_rgb.values,
			          "gdevx color_to_rgb");
	...
	if(!gx_device_must_halftone(xdev)) {
		if(xdev->cman.std_cmap.map) {
			xdev->cman.match_mask.red &=
			    X_max_color_value
			    << xdev->cman.std_cmap.red.cv_shift;
			xdev->cman.match_mask.green &=
			    X_max_color_value
			    << xdev->cman.std_cmap.green.cv_shift;
			xdev->cman.match_mask.blue &=
			    X_max_color_value
			    << xdev->cman.std_cmap.blue.cv_shift;
		}
	}
#endif
...
gdev_x_free_dynamic_colors(gx_device_X* xdev)
{
	if(xdev->cman.dynamic.colors) {
		...
		for(i = 0; i < xdev->cman.dynamic.size; i++) {
			for(xcp = xdev->cman.dynamic.colors[i]; xcp;
			    xcp = next) {
				next = xcp->next;
				if(xcp->color.pad)
					x_free_colors(xdev, &xcp->color.pixel,
					              1);
				gs_x_free(xdev->memory, xcp,
				          "x11_dynamic_color");
			}
			xdev->cman.dynamic.colors[i] = NULL;
		}
...
gdev_x_free_colors(gx_device_X* xdev)
{
	...
	if(xdev->cman.dither_ramp)
		gs_x_free(xdev->memory, xdev->cman.dither_ramp,
		          "x11 dither_colors");
	if(xdev->cman.dynamic.colors) {
		gdev_x_free_dynamic_colors(xdev);
		gs_x_free(xdev->memory, xdev->cman.dynamic.colors,
		          "x11 cman.dynamic.colors");
		xdev->cman.dynamic.colors = NULL;
	}
	if(xdev->cman.color_to_rgb.values) {
...
private
const X_color_value cv_tab1[] = {ND(0, 1), ND(1, 1)};
private
const X_color_value cv_tab2[] = {ND(0, 2), ND(1, 2), ND(2, 2)};
private
const X_color_value cv_tab3[] = {ND(0, 3), ND(1, 3), ND(2, 3), ND(3, 3)};
private
const X_color_value cv_tab4[] = {ND(0, 4), ND(1, 4), ND(2, 4), ND(3, 4),
                                 ND(4, 4)};
private
const X_color_value cv_tab5[] = {ND(0, 5), ND(1, 5), ND(2, 5),
                                 ND(3, 5), ND(4, 5), ND(5, 5)};
private
const X_color_value cv_tab6[] = {ND(0, 6), ND(1, 6), ND(2, 6), ND(3, 6),
                                 ND(4, 6), ND(5, 6), ND(6, 6)};
private
const X_color_value cv_tab7[] = {ND(0, 7), ND(1, 7), ND(2, 7), ND(3, 7),
                                 ND(4, 7), ND(5, 7), ND(6, 7), ND(7, 7)};
#undef ND
private
const X_color_value* const cv_tables[] = {0,       cv_tab1, cv_tab2, cv_tab3,
                                          cv_tab4, cv_tab5, cv_tab6, cv_tab7};
...
#define CV_DENOM (gx_max_color_value + 1)
	...
	if(xdev->cman.std_cmap.map) {
		...
		if(gx_device_has_color(xdev)) {
			...
			if((iabs((int)r - (int)cvr) &
			    xdev->cman.match_mask.red) == 0 &&
			   (iabs((int)g - (int)cvg) &
			    xdev->cman.match_mask.green) == 0 &&
			   (iabs((int)b - (int)cvb) &
			    xdev->cman.match_mask.blue) == 0) {
				gx_color_index pixel =
				    (xdev->cman.std_cmap.fast
				         ? (cr << xdev->cman.std_cmap.red
				                      .pixel_shift) +
				               (cg << xdev->cman.std_cmap.green
				                          .pixel_shift) +
				               (cb << xdev->cman.std_cmap.blue
				                          .pixel_shift)
				         : cr * cmap->red_mult +
				               cg * cmap->green_mult +
				               cb * cmap->blue_mult) +
			...
			if((iabs((int)r - (int)cvr) &
			    xdev->cman.match_mask.red) == 0) {
				gx_color_index pixel =
				    cr * cmap->red_mult + cmap->base_pixel;
		...
		if(gx_device_has_color(xdev)) {
			...
			if((iabs((int)r - (int)cvr) &
			   ...
			   (iabs((int)b - (int)cvb) &
				...
				gx_color_index pixel =
				    xdev->cman
				        .dither_ramp[CUBE_INDEX(cr, cg, cb)];

				if_debug4('C',
				          "[cX]%u,%u,%u (dither cube) => %lu\n",
				          r, g, b, pixel);
				return pixel;
			}
	...
	if(xdev->cman.dynamic.colors) {
		...
		for(; xcp; prev = xcp, xcp = xcp->next)
			if(xcp->color.red == dr && xcp->color.green == dg &&
			   xcp->color.blue == db) {
				/* Promote the found entry to the front of the
				 * list. */
				if(prev) {
					prev->next = xcp->next;
					xcp->next =
					    xdev->cman.dynamic.colors[i];
				...
				if(xcp->color.pad) {
					if_debug4(
					    ...
					    "[cX]%u,%u,%u (dynamic) => %lu\n",
					    r, g, b,
					    (uint32_t)xcp->color.pixel);
					return xcp->color.pixel;
				} else {
					if_debug3('C', "[cX]%u,%u,%u (dynamic) "
					               "=> missing\n",
					          r, g, b);
		...
		xcp = (x11_color_t*)gs_malloc(xdev->memory, sizeof(x11_color_t),
		                              1, "x11_dynamic_color");
		if(!xcp)
			return gx_no_color_index;
		...
		if(x_alloc_color(xdev, &xc)) {
			...
			if_debug5(
			    'c',
			    "[cX]0x%x,0x%x,0x%x (dynamic) => added [%d]%lu\n",
			...
			if_debug3(
			    'c',
			    "[cX]0x%x,0x%x,0x%x (dynamic) => can't alloc\n", dr,
			    dg, db);
			return gx_no_color_index;
		}
	}
                     ...
                     gx_color_value prgb[3])
{
	...
	if(cmap) {
		if(color >= cmap->base_pixel) {
			...
			uint g =
			    (value / cmap->green_mult) % (cmap->green_max + 1);
			uint b =
			    (value / cmap->blue_mult) % (cmap->blue_max + 1);

			if(value ==
			   r * cmap->red_mult + g * cmap->green_mult +
			       b * cmap->blue_mult) {
				/* When mapping color buckets back to specific
				 * colors,
				 * we can choose to map them to the darkest
				 * shades
				 * (e.g., 0, 1/3, 2/3), to the lightest shades
				 * (e.g.,
				 * 1/3-epsilon, 2/3-epsilon, 1-epsilon), to the
				 * middle
				 * shades (e.g., 1/6, 1/2, 5/6), or for maximum
				 * range
				 * (e.g., 0, 1/2, 1).  The last of these matches
				 * the
				 * assumptions of the halftoning code, so that
				 * is what
				 * we choose.
				 */
				prgb[0] =
				    r * gx_max_color_value / cmap->red_max;
				prgb[1] =
				    g * gx_max_color_value / cmap->green_max;
				prgb[2] =
				    b * gx_max_color_value / cmap->blue_max;
				return 0;
			}
		}
	}
		...
		if(gx_device_has_color(xdev)) {
			...
			for(i = 0; i < size3; ++i)
				if(xdev->cman.dither_ramp[i] == color) {
					uint max_rgb = size - 1;
					uint q = i / size, r = q / size,
					     g = q % size, b = i % size;

					/*
					 * See above regarding the choice of
					 * color mapping
					 * algorithm.
					 */
					prgb[0] =
					    r * gx_max_color_value / max_rgb;
					prgb[1] =
					    g * gx_max_color_value / max_rgb;
					prgb[2] =
					    b * gx_max_color_value / max_rgb;
					return 0;
				}
		} else {
