			...
			if((iabs((int)r - (int)cvr) & xdev->cman.match_mask.red) == 0 &&
			   ...
			   (iabs((int)b - (int)cvb) & xdev->cman.match_mask.blue) == 0) {
				gx_color_index pixel =
				    (xdev->cman.std_cmap.fast ? (cr << xdev->cman.std_cmap.red.pixel_shift) + (cg << xdev->cman.std_cmap.green.pixel_shift) + (cb << xdev->cman.std_cmap.blue.pixel_shift) : cr * cmap->red_mult + cg * cmap->green_mult +
																										 cb * cmap->blue_mult) +
				    cmap->base_pixel;
		     ...
		     gx_color_value prgb[3])
{
	...
	if(cmap) {
		if(color >= cmap->base_pixel) {
			...
			if(value == r * cmap->red_mult + g * cmap->green_mult + b * cmap->blue_mult) {
