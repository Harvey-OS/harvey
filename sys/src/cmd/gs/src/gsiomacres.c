...
private
int
read_datafork_resource(byte *buf, const char *fname, const uint type, const ushort id)
{
	...
	for(i = 0; i < list->n_resources; i++) {
		if((list->resources[i].type == type) &&
		   (list->resources[i].id == id)) {
			...
			fclose(in);
			if(buf)
				memcpy(buf, list->resources[i].data, list->resources[i].length);
...
const gx_io_device gs_iodev_macresource =
    {
     "%macresource%", "FileSystem", {iodev_macresource_init, iodev_no_open_device, iodev_macresource_open_file, iodev_no_fopen, iodev_no_fclose, iodev_no_delete_file, iodev_no_rename_file, iodev_no_file_status, iodev_no_enumerate_files, NULL, NULL, iodev_no_get_params, iodev_no_put_params}};
