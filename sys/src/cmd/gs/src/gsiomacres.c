...
private
unsigned int
res_string2type(const char* type_string)
{
	unsigned int type = type_string[0] << 24 | type_string[1] << 16 |
	                    type_string[2] << 8 | type_string[3];
	return (type);
}
...
private
int8_t*
res_type2string(const unsigned int type, char* type_string)
{
	if(type_string == NULL)
		return NULL;
...
private
resource_header*
read_resource_header(FILE* in, int offset)
{
...
private
resource_list*
read_resource_map(FILE* in, resource_header* header)
{
	resource_list* list;
	int n_resources;
	...
	if(buf == NULL) {
		if_debug1(
		    's',
		    "error: could not allocate %d bytes for resource map\n",
		    header->map_length);
		return NULL;
	}
	...
	n_types = get_int16(buf + 28);
	n_types++;

	if(type_offset != 30)
		if_debug1(
		    's',
		    "[s] warning! resource type list offset is %d, not 30!\n",
		    type_offset);
	...
	for(i = 0; i < n_types; i++) {
		...
		for(j = 0; j < number[i]; j++) {
			...
			p += 12;
			if_debug4(
			    's', "\tid %d offset 0x%08x flags 0x%02x '%s'\n",
...
private
void
load_resource(FILE* in, resource_header* header, resource* res)
{
	unsigned int len;
	byte* buf;
...
private
int
read_datafork_resource(byte* buf, const char* fname, const uint type,
                       const ushort id)
{
	...
	if(header == NULL) {
		...
		if_debug0('s',
		          "[s] not a serialized data fork resource file?\n");
		return 0;
	}
	...
	for(i = 0; i < list->n_resources; i++) {
		if((list->resources[i].type == type) &&
		   (list->resources[i].id == id)) {
			...
			fclose(in);
			if(buf)
				memcpy(buf, list->resources[i].data,
				       list->resources[i].length);
...
/* prototypes */
private
iodev_proc_init(iodev_macresource_init);
private
iodev_proc_open_file(iodev_macresource_open_file);
...
const gx_io_device gs_iodev_macresource = {
    "%macresource%",
    "FileSystem",
    {iodev_macresource_init, iodev_no_open_device, iodev_macresource_open_file,
     iodev_no_fopen, iodev_no_fclose, iodev_no_delete_file,
     iodev_no_rename_file, iodev_no_file_status, iodev_no_enumerate_files, NULL,
     NULL, iodev_no_get_params, iodev_no_put_params}};
...
private
int
iodev_macresource_open_file(gx_io_device* iodev, const char* fname,
                            uint namelen, const char* access, stream** ps,
                            gs_memory_t* mem)
{
	...
	*res_id_string++ = '\0';
	...
	if_debug3('s',
	          "[s] opening resource fork of '%s' for type '%s' id '%d'\n",
	...
	if(size == 0) {
		...
		if_debug0('s',
		          "[s] trying to open as a datafork file instead...\n");
	...
	if(buf == NULL) {
		if_debug0('s', "macresource: could not allocate buffer for "
		               "resource data\n");
