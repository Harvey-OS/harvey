	...
	if(size >= 2 * NB) {
		...
		if(gs_debug_c('?')) {
			if(prev < str + size && prev + get_uu32(prev) > str) {
				lprintf4("freeing string 0x%lx(%u), overlaps "
				         "0x%lx(%u)!\n",
				         (ulong)str, size, (ulong)prev,
				         get_uu32(prev));
				return;
			}
		}
		...
		for(;;) {
			...
			if(gs_debug_c('?') && next != 0) {
				...
				if(pnext < str + size &&
				   pnext + get_uu32(pnext) > str) {
					lprintf4("freeing string 0x%lx(%u), "
					         "overlaps 0x%lx(%u)!\n",
					         (ulong)str, size, (ulong)pnext,
					         get_uu32(pnext));
					return;
				}
			}
...
sf_consolidate_free(gs_memory_t* mem)
{
	...
	for(cp = imem->clast; cp != 0; cp = cp->cprev) {
		...
		if(cp->ctop == cp->climit && cp->smark_size != 0) {
			/*
			 * No string space is being used.  Since we are not
			 * using the
			 * 'string marking table' for GC, we can recover its
			 * space by
			 * deleting the smark area, setting smark_size = 0, and
			 * sliding
			 * up ctop and climit.  We also need to recompute the
			 * size of
			 * the string freelist area (it will be larger, since
			 * the
			...
			cp->smark = 0;
			/*
			 * Reserve enough space for the string freelist all the
			 * way to
			 * cend even though the climit will be moved to before
			 * the
...
private
void use_string_freelists(gs_ref_memory_t* mem);
void
