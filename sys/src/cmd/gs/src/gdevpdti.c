...
gs_public_st_suffix_add2_string1(st_pdf_char_proc, pdf_char_proc_t,
                                 ...
                                 pdf_char_proc_reloc_ptrs, st_pdf_resource,
                                 font, char_next, char_name);
...
assign_char_code(gx_device_pdf* pdev, int width)
{
	...
	if(pdfont == 0 || pdfont->u.simple.LastChar == 255 ||
	   !pbfs->use_open_font) {
		...
		code = pdf_font_type3_alloc(pdev, &pdfont,
	...
	/* Synthezise ToUnicode CMap :*/
	{
		...
		code = pdf_add_ToUnicode(pdev, font, pdfont,
		                         pte->returned.current_glyph, c);
		if(code < 0)
			return code;
	}
	return c;
...
pdf_write_contents_bitmap(gx_device_pdf* pdev, pdf_font_resource_t* pdfont)
{
	...
	for(pcp = pdfont->u.simple.s.type3.char_procs; pcp;
		...
		if(pdfont->u.simple.s.type3.bitmap_font)
			...
			pdf_put_name(pdev, pcp->char_name.data,
			             pcp->char_name.size);
			pprintld1(s, " %ld 0 R\n", pdf_char_proc_id(pcp));
		}
	}
...
pdf_begin_char_proc_generic(gx_device_pdf* pdev, pdf_font_resource_t* pdfont,
                            gs_id id, gs_char char_code, pdf_char_proc_t** ppcp,
                            pdf_stream_position_t* ppos)
{
...

	{
		 ...
		 * definition we can handle.  (This used to be 10K, but there
		 * was
                    ...
                    pdf_stream_position_t* ppos)
{
	...
	int code =
	    pdf_begin_char_proc_generic(pdev, font, id, char_code, ppcp, ppos);
...
pdf_start_charproc_accum(gx_device_pdf* pdev)
{
	pdf_char_proc_t* pcp;
	pdf_resource_t* pres;
	int code = pdf_enter_substream(pdev, resourceCharProc,
	                               gs_next_ids(pdev->memory, 1), &pres,
...
pdf_set_charproc_attrs(gx_device_pdf* pdev, gs_font* font, const double* pw,
                       int narg, gs_text_cache_control_t control, gs_char ch,
                       gs_const_string* gnstr)
{
	...
	code = pdf_attached_font_resource(pdev, font, &pdfont, NULL, NULL, NULL,
	                                  NULL);
	if(code < 0)
		return code;
	pcp = (pdf_char_proc_t*)pres;
	...
	if(control == TEXT_SET_CHAR_WIDTH) {
		/* PLRM 5.7.1 "BuildGlyph" reads : "Normally, it is unnecessary
		and
		undesirable to initialize the current color parameter, because
		show
		...
		pprintg6(pdev->strm, "%g %g %g %g %g %g d1\n", (float)pw[0],
		         (float)pw[1], (float)pw[2], (float)pw[3], (float)pw[4],
		         (float)pw[5]);
...
pdf_open_aside(gx_device_pdf* pdev, pdf_resource_type_t rtype, gs_id id,
               pdf_resource_t** ppres, bool reserve_object_id, int options)
{
	int code;
	pdf_resource_t* pres;
	...
	code = pdf_alloc_aside(pdev, PDF_RESOURCE_CHAIN(pdev, rtype, id),
	                       pdf_resource_type_structs[rtype], &pres,
	                       reserve_object_id ? 0 : -1);
	if(code < 0)
		return code;
	cos_become(pres->object, cos_type_stream);
	s = cos_write_stream_alloc((cos_stream_t*)pres->object, pdev,
	                           "pdf_enter_substream");
	if(s == 0)
		return_error(gs_error_VMerror);
	pdev->strm = s;
	code = pdf_append_data_stream_filters(
	    pdev, &writer, options | DATA_STREAM_NOLENGTH, pres->object->id);
	if(code < 0) {
		pdev->strm = save_strm;
		return code;
	}
	code = pdf_put_filters((cos_dict_t*)pres->object, pdev,
	                       writer.binary.strm, &fnames);
	if(code < 0) {
		pdev->strm = save_strm;
		return code;
	}
...
{
	/* We should call pdf_end_data here, but we don't want to put
	   pdf_data_writer_t
...
pdf_enter_substream(gx_device_pdf* pdev, pdf_resource_type_t rtype, gs_id id,
                    pdf_resource_t** ppres, bool reserve_object_id,
                    bool compress)
{
	...
	if(pdev->sbstack[sbstack_ptr].text_state == 0) {
		pdev->sbstack[sbstack_ptr].text_state =
		    pdf_text_state_alloc(pdev->pdf_memory);
		if(pdev->sbstack[sbstack_ptr].text_state == 0)
			return_error(gs_error_VMerror);
	}
	...
	pdf_text_state_copy(pdev->sbstack[sbstack_ptr].text_state,
	...
	pdev->sbstack[sbstack_ptr].substream_Resources =
	    pdev->substream_Resources;
	pdev->sbstack[sbstack_ptr].skip_colors = pdev->skip_colors;
	pdev->sbstack[sbstack_ptr].font3 = pdev->font3;
	pdev->sbstack[sbstack_ptr].accumulating_substream_resource =
	    pdev->accumulating_substream_resource;
	pdev->sbstack[sbstack_ptr].charproc_just_accumulated =
	    pdev->charproc_just_accumulated;
	pdev->sbstack[sbstack_ptr].accumulating_a_global_object =
	    pdev->accumulating_a_global_object;
	pdev->sbstack[sbstack_ptr].pres_soft_mask_dict =
	    pdev->pres_soft_mask_dict;
...
pdf_exit_substream(gx_device_pdf* pdev)
{
	...
	pdf_text_state_copy(pdev->text->text_state,
	                    pdev->sbstack[sbstack_ptr].text_state);
...
private
bool
pdf_is_same_charproc1(gx_device_pdf* pdev, pdf_char_proc_t* pcp0,
                      pdf_char_proc_t* pcp1)
{
	...
	if(pcp0->v.y != pcp1->v.y)
		return false;
	if(pcp0->font->u.simple.s.type3.bitmap_font !=
	   pcp1->font->u.simple.s.type3.bitmap_font)
		return false;
	if(memcmp(&pcp0->font->u.simple.s.type3.FontMatrix,
	          &pcp1->font->u.simple.s.type3.FontMatrix,
	          sizeof(pcp0->font->u.simple.s.type3.FontMatrix)))
		return false;
	return pdf_check_encoding_compatibility(pcp1->font, pdev->cgp->s,
	                                        pdev->cgp->num_all_chars);
}

private
int
pdf_is_same_charproc(gx_device_pdf* pdev, pdf_resource_t* pres0,
                     pdf_resource_t* pres1)
{
	return pdf_is_same_charproc1(pdev, (pdf_char_proc_t*)pres0,
	                             (pdf_char_proc_t*)pres1);
}

private
int
pdf_find_same_charproc(gx_device_pdf* pdev, pdf_font_resource_t* pdfont,
                       ...
                       pdf_char_proc_t** ppcp)
{
	...
	pdev->cgp = cgp;
	for(pcp = pdfont->u.simple.s.type3.char_procs; pcp != NULL;
	    pcp = pcp->char_next) {
	...
	pcp = *ppcp;
	code = pdf_find_same_resource(pdev, resourceCharProc,
	                              (pdf_resource_t**)ppcp,
	                              pdf_is_same_charproc);
	pdev->cgp = NULL;
	if(code <= 0)
		return code;
...
private
bool
pdf_is_charproc_defined(gx_device_pdf* pdev, pdf_font_resource_t* pdfont,
                        gs_char ch)
{
	pdf_char_proc_t* pcp;
                       ...
                       const pdf_char_glyph_pairs_t* cgp)
{
	int code;
	pdf_resource_t* pres =
	    (pdf_resource_t*)pdev->accumulating_substream_resource;
	/* We could use pdfont->u.simple.s.type3.char_procs insted the thing
	   above
	...
	if(pdfont->used[ch >> 3] & (0x80 >> (ch & 7))) {
		if(!(pdfont->u.simple.s.type3.cached[ch >> 3] &
		     (0x80 >> (ch & 7)))) {
			...
			if(code < 0)
				return code;
			if(code != 0) {
				code = pdf_cancel_resource(pdev, pres,
				                           resourceCharProc);
				if(code < 0)
					return code;
				pdf_forget_resource(pdev, pres,
				                    resourceCharProc);
				if(pcp->font != pdfont) {
					code = pdf_attach_font_resource(
					    pdev, font, pcp->font);
					if(code < 0)
						return code;
				}
			...
			if(pdf_is_charproc_defined(pdev, pdfont, ch)) {
				...
				while((below = base_font->base) != base_font &&
				      base_font->procs.same_font(
				          base_font, below, FONT_SAME_OUTLINES))
					base_font = below;
				code = pdf_make_font3_resource(pdev, base_font,
				                               &pdfont);
				if(code < 0)
					return code;
				code = pdf_attach_font_resource(pdev, font,
				                                pdfont);
				if(code < 0)
					return code;
			}
	...
	code = pdf_attached_font_resource(pdev, font, &pdfont, &glyph_usage,
	...
	for(i = 0; i < 256; i++) {
		gs_glyph glyph = font->procs.encode_char(
		    font, i, font->FontType == ft_user_defined
		                 ? GLYPH_SPACE_NOGEN
		                 : GLYPH_SPACE_NAME);
                 ...
                 pdf_resource_t* pres)
{
	if(pcd != 0) {
		const cos_value_t* v =
		    cos_dict_find(pcd, (const byte*)key, strlen(key));
		cos_dict_t* list;
		int code;
		char buf[10 + (sizeof(int32_t) * 8 / 3 + 1)],
		    buf1[sizeof(pres->rname) + 1];

		if(pdev->ForOPDFRead && !pres->global &&
		   pdev->accumulating_a_global_object) {
			pres->global = true;
			code = cos_dict_put_c_key_bool(
			    (cos_dict_t*)pres->object, "/.Global", true);
			if(code < 0)
				return code;
		}
		sprintf(buf, "%ld 0 R\n", pres->object->id);
		if(v != NULL) {
			...
			if(list == NULL)
				return_error(gs_error_VMerror);
			code = cos_dict_put_c_key_object((cos_dict_t*)pcd, key,
			                                 (cos_object_t*)list);
			if(code < 0)
				return code;
		}
		...
		return cos_dict_put_string(list, (const byte*)buf1,
		                           strlen(buf1), (const byte*)buf,
		                           strlen(buf));
	}
	return 0;
}
