...
zanchorsearch(i_ctx_t* i_ctx_p)
{
	...
	if(size <= r_size(op1) &&
	   !memcmp(op1->value.bytes, op->value.bytes, size)) {
		os_ptr op0 = op;
...
zstringbreak(i_ctx_t* i_ctx_p)
{
	...
	for(i = 0; i < r_size(op - 1); ++i)
		for(j = 0; j < r_size(op); ++j)
			if(op[-1].value.const_bytes[i] ==
			   op->value.const_bytes[j]) {
				make_int(op - 1, i);
				goto done;
			}
	make_null(op - 1);
...
zstringmatch(i_ctx_t* i_ctx_p)
{
	...
	cmp:
		result = string_match(op1->value.const_bytes, r_size(op1),
		                      op->value.const_bytes, r_size(op), NULL);
		break;
	default:
...
const op_def zstring_op_defs[] = {{"1.bytestring", zbytestring},
