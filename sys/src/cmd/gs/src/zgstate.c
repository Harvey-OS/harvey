...
/* "Client" procedures */
private
void* gs_istate_alloc(gs_memory_t* mem);
private
int gs_istate_copy(void* to, const void* from);
private
void gs_istate_free(void* old, gs_memory_t* mem);
private
const gs_state_client_procs istate_procs = {
    gs_istate_alloc, gs_istate_copy, gs_istate_free, 0, /* copy_for */
};
...
zsetdash(i_ctx_t* i_ctx_p)
{
	...
	n = r_size(op1);
	pattern = (float*)gs_alloc_byte_array(mem, n, sizeof(float), "setdash");
	...
	for(i = 0, code = 0; i < n && code >= 0; ++i) {
		...
		array_get(mem, op1, (int32_t)i, &element);
...
const op_def zgstate1_op_defs[] = {
    ...
    op_def_end(0)};
const op_def zgstate2_op_defs[] = {{"0gsave", zgsave},
