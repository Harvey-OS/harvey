...
typedef void (*TMove_Function)(EXEC_OPS PGlyph_Zone zone, Int point,
                               TT_F26Dot6 distance);

/* Distance projection along one of the proj. vectors, as used */
/* by the interpreter                                          */
typedef TT_F26Dot6 (*TProject_Function)(EXEC_OPS TT_F26Dot6 Vx, TT_F26Dot6 Vy);
...
/* when needed                                                     */
typedef void (*TSet_CVT_Function)(EXEC_OPS Int index, TT_F26Dot6 value);
...
struct _TExecution_Context {
	...
	Int args, new_top; /* new top after exec.    */
...
TT_Error Set_CodeRange(PExecution_Context exec, Int range, void* base,
                       Int length);
...
TT_Error Context_Load(PExecution_Context exec, PInstance ins);

TT_Error Context_Save(PExecution_Context exec, PInstance ins);

TT_Error Context_Run(PExecution_Context exec, Bool debug);

TT_Error Instance_Init(PInstance ins);

TT_Error Instance_Reset(PInstance ins, Bool debug);

TT_Error Instance_Create(void* _instance, void* _face);
