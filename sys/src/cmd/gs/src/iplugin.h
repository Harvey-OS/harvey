...
struct i_plugin_descriptor_s {/* RTTI for plugins */
	...
	void (*finit)(i_plugin_instance* instance,
	              i_plugin_client_memory*
	                  mem); /* Destructor & deallocator for the instance. */
};
...
struct i_plugin_client_memory_s {/* must be copying */
	void* client_data;
	void* (*alloc)(i_plugin_client_memory* mem, unsigned int size,
	               const char* id);
	void (*free)(i_plugin_client_memory* mem, void* data,
	             const char* cname);
};

#define plugin_instantiation_proc(proc)                                        \
	int proc(i_ctx_t*, i_plugin_client_memory* client_mem,                 \
	         i_plugin_instance** instance)
