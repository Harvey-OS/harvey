...
gs_private_st_ptrs1(st_pdf14_ctx, pdf14_ctx, "pdf14_ctx", pdf14_ctx_enum_ptrs,
                    pdf14_ctx_reloc_ptrs, stack);
...
private
int pnga_open(gx_device* pdev);
private
dev_proc_close_device(pnga_close);
private
int pnga_output_page(gx_device* pdev, int num_copies, int flush);
private
dev_proc_fill_rectangle(pnga_fill_rectangle);
private
dev_proc_fill_path(pnga_fill_path);
private
dev_proc_stroke_path(pnga_stroke_path);
private
dev_proc_begin_typed_image(pnga_begin_typed_image);
private
dev_proc_text_begin(pnga_text_begin);
private
dev_proc_begin_transparency_group(pnga_begin_transparency_group);
private
dev_proc_end_transparency_group(pnga_end_transparency_group);
...
private
const gx_device_procs pnga_procs = {
    pnga_open,        /* open */
    ...
    pnga_begin_transparency_group, pnga_end_transparency_group};
...
const gx_device_printer gs_pnga_device = {
    std_device_color_stype_body(pnga_device, &pnga_procs, "pnga",
                                &st_pnga_device, XSIZE, YSIZE, X_DPI, Y_DPI, 24,
                                255, 0),
    {0}};
...
ENUM_PTRS_END
private
RELOC_PTRS_WITH(pnga_device_reloc_ptrs, pnga_device* pdev)
{
	RELOC_VAR(pdev->ctx);
}
RELOC_PTRS_END
...
private
dev_proc_fill_rectangle(pnga_mark_fill_rectangle);
private
dev_proc_fill_rectangle(pnga_mark_fill_rectangle_ko_simple);

private
const gx_device_procs pnga_mark_procs = {
    NULL, /* open */
    NULL, /* get_initial_matrix */
    NULL, /* sync_output */
    NULL, /* output_page */
...
const gx_device_printer gs_pnga_mark_device = {
    std_device_color_stype_body(pnga_mark_device, &pnga_mark_procs, "pnga_mark",
                                &st_pnga_mark_device, XSIZE, YSIZE, X_DPI,
                                Y_DPI, 24, 255, 0),
    {0}};
              ...
              gs_memory_t* memory)
{
	...
	result->data =
	    gs_alloc_bytes(memory, planestride * n_planes, "pdf14_buf_new");
	if(result->data == NULL) {
		...
		for(x = 0; x < width; ++x) {
			...
			if(nos_knockout) {
				...
				art_pdf_composite_knockout_isolated_8(
				    ...
				    n_chan - 1, tos_shape, alpha, shape);
#else
				tos_pixel[3] = tos_ptr[x + tos_shape_offset];
				art_pdf_composite_group_8(
				    nos_pixel, nos_alpha_g_ptr, tos_pixel,
				    n_chan - 1, alpha, blend_mode);
#endif
				...
				art_pdf_recomposite_group_8(
				    nos_pixel, nos_alpha_g_ptr, tos_pixel,
				    tos_alpha_g, n_chan - 1, alpha, blend_mode);
			}
			if(nos_has_shape) {
				nos_ptr[x + nos_shape_offset] =
				    art_pdf_union_mul_8(
				        nos_ptr[x + nos_shape_offset],
				        tos_ptr[x + tos_shape_offset], shape);
			}
			/* todo: knockout cases */
...
pnga_get_marking_device(gx_device* dev, const gs_imager_state* pis)
{
	...
	int code =
	    gs_copydevice((gx_device**)&mdev,
	                  (const gx_device*)&gs_pnga_mark_device, dev->memory);
...
private
int
pnga_fill_path(gx_device* dev, const gs_imager_state* pis, gx_path* ppath,
...
private
int
pnga_stroke_path(gx_device* dev, const gs_imager_state* pis, gx_path* ppath,
                 ...
                 const gx_drawing_color* pdcolor, const gx_clip_path* pcpath)
{
	...
	if(mdev == 0)
		return_error(gs_error_VMerror);
	code =
	    gx_default_stroke_path(mdev, pis, ppath, params, pdcolor, pcpath);
	pnga_release_marking_device(mdev);
	return code;
}
                       ...
                       gx_image_enum_common_t** pinfo)
{
	...
	code = gx_default_begin_typed_image(mdev, pis, pmat, pic, prect,
...
private
const gs_text_enum_procs_t pnga_text_procs = {
    pnga_text_resync,        pnga_text_process,   pnga_text_is_width_only,
    pnga_text_current_width, pnga_text_set_cache, pnga_text_retry,
    pnga_text_release};
                ...
                gs_memory_t* memory, gs_text_enum_t** ppenum)
{
	...
	code = gx_default_text_begin(mdev, pis, text, font, path, pdcolor,
	                             pcpath, memory, &target_enum);
	...
	code =
	    gs_text_enum_init((gs_text_enum_t*)penum, &pnga_text_procs, dev,
	                      pis, text, font, path, pdcolor, pcpath, memory);
	if(code < 0) {
...
private
int
pnga_fill_rectangle(gx_device* dev, int x, int y, int w, int h,
                              ...
                              gs_transparency_state_t** ppts, gs_memory_t* mem)
{
	...
	code = pdf14_push_transparency_group(
	    pdev->ctx, &pdev->ctx->rect, ptgp->Isolated, ptgp->Knockout,
	    floor(255 * alpha + 0.5), floor(255 * pis->shape.alpha + 0.5),
	    pis->blend_mode);
	return code;
}
...
private
int
pnga_end_transparency_group(gx_device* dev, gs_imager_state* pis,
...
private
int
pnga_mark_fill_rectangle(gx_device* dev, int x, int y, int w, int h,
                         gx_color_index color)
{
	...
	for(j = 0; j < h; ++j) {
		...
		for(i = 0; i < w; ++i) {
			...
			art_pdf_composite_pixel_alpha_8(dst, src, 3,
			                                blend_mode);
			for(k = 0; k < 4; ++k)
...
private
int
pnga_mark_fill_rectangle_ko_simple(gx_device* dev, int x, int y, int w, int h,
                                   gx_color_index color)
{
	pnga_mark_device* mdev = (pnga_mark_device*)dev;
	pnga_device* pdev = (pnga_device*)mdev->pnga_dev;
	...
	for(j = 0; j < h; ++j) {
		...
		for(i = 0; i < w; ++i) {
			...
			art_pdf_composite_knockout_simple_8(
			    dst, has_shape ? dst_ptr + shape_off : NULL, src, 3,
			    opacity);
			for(k = 0; k < 4; ++k)
				dst_ptr[k * planestride] = dst[k];
			++dst_ptr;
