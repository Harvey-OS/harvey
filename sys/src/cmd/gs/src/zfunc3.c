...
int
gs_build_function_2(i_ctx_t* i_ctx_p, const ref* op,
                    const gs_function_params_t* mnDR, int depth,
                    gs_function_t** ppfn, gs_memory_t* mem)
{
	...
	*(gs_function_params_t*)&params = *mnDR;
	...
	if((code = dict_float_param(op, "N", 0.0, &params.N)) != 0 ||
	   (code = n0 = fn_build_float_array_forced(op, "C0", false, &params.C0,
	                                            mem)) < 0 ||
	   (code = n1 = fn_build_float_array_forced(op, "C1", false, &params.C1,
	                                            mem)) < 0)
		goto fail;
	if(params.C0 == 0)
		n0 = 1; /* C0 defaulted */
...
int
gs_build_function_3(i_ctx_t* i_ctx_p, const ref* op,
                    const gs_function_params_t* mnDR, int depth,
                    gs_function_t** ppfn, gs_memory_t* mem)
{
	...
	*(gs_function_params_t*)&params = *mnDR;
	...
	params.Encode = 0;
	{
		...
		for(i = 0; i < params.k; ++i) {
			...
			code = fn_build_sub_function(i_ctx_p, &subfn, &ptr[i],
			                             depth, mem);
			if(code < 0)
				goto fail;
		}
	...
	if((code = fn_build_float_array(op, "Bounds", true, false,
	   ...
	   (code = fn_build_float_array(op, "Encode", true, true,
	                                &params.Encode, mem)) != 2 * params.k)
		goto fail;
	if(params.Range == 0)
