...
/* Copyright (C) 1989, 1990, 1991, 1993, 1996 Aladdin Enterprises.  All rights
  reserved.
...
const gx_device_printer far_data gs_nwp533_device = prn_device(
    nwp533_procs, "nwp533", PAPER_XDOTS * 10.0 / DPI, /* width_10ths */
...
analyze_error(int printer_file)
{
	...
	do {
		/* Is there an error */
		if(status.stat[0] &
		   (ST0_CALL | ST0_REPRINT_REQ | ST0_WAIT | ST0_PAUSE)) {
...
nwp533_print_page(gx_device_printer* dev, FILE* prn_stream)
{
	...
	in = (byte*)gs_malloc(dev->memory, line_size, 1,
	                      "nwp533_output_page(in)");
restart:
	if(ioctl(printer_file, LBIOCSTOP, 0) < 0) {
		if(analyze_error(printer_file))
			goto restart;
