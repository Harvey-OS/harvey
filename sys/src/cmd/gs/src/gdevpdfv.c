...
#define ENCODE_MESH_COMPONENT(v, vmin, vmax) ENCODE_VALUE(v, 0xffff, vmin, vmax)
             ...
             const gx_color_tile* m_tile)
{
	...
	uint m_size = (m_tile == 0 ? 0 : tile_size(&m_tile->tmask, 1));
            ...
            cos_stream_t* pcs_image, pdf_resource_t** ppres)
{
	pdf_resource_t* pres;
	int code =
	    pdf_alloc_resource(pdev, resourcePattern, pdc->mask.id, ppres, 0L);
	cos_stream_t* pcos;
	cos_dict_t* pcd;
	cos_dict_t* pcd_Resources =
	    cos_dict_alloc(pdev, "pdf_pattern(Resources)");
	const gx_color_tile* tile = (p_tile ? p_tile : m_tile);
	const gx_strip_bitmap* btile =
	    (p_tile ? &p_tile->tbits : &m_tile->tmask);
	bool mask = p_tile == 0;
	gs_point step;
	gs_matrix smat;
	...
	pres = *ppres;
	{
		cos_dict_t* pcd_XObject =
		    cos_dict_alloc(pdev, "pdf_pattern(XObject)");
		...
		if((code = cos_dict_put(pcd_XObject, (byte*)key, strlen(key),
		                        &v)) < 0 ||
		   (code = cos_dict_put_c_key_object(pcd_Resources, "/XObject",
		                                     COS_OBJECT(pcd_XObject))) <
		       0)
			return code;
	}
	if((code = cos_dict_put_c_strings(
	        pcd_Resources, "/ProcSet",
	        (mask ? "[/PDF/ImageB]" : "[/PDF/ImageC]"))) < 0)
		return code;
	...
	if((code = cos_dict_put_c_key_int(pcd, "/PatternType", 1)) < 0 ||
	   (code = cos_dict_put_c_key_int(pcd, "/PaintType", (mask ? 2 : 1))) <
	       0 ||
	   ...
	   (code = cos_dict_put_c_key_real(pcd, "/XStep",
	                                   step.x / btile->rep_width)) < 0 ||
	   (code = cos_dict_put_c_key_real(pcd, "/YStep",
	                                   step.y / btile->rep_height)) < 0) {
		return code;
	}
                          ...
                          gs_pattern1_instance_t* pinst)
{
	...
	sprintf(buf, "[%g %g %g %g]", t->BBox.p.x, t->BBox.p.y, t->BBox.q.x,
	        t->BBox.q.y);
	...
	if(code >= 0)
		code =
		    cos_dict_put_c_key_int(pcd, "/TilingType", t->TilingType);
	if(code >= 0)
		code = cos_dict_put_string(pcd, (byte*)"/BBox", 5, (byte*)buf,
		                           strlen(buf));
	if(code >= 0)
                     ...
                     cos_stream_t** ppcs_mask)
{
	...
	if((code = pdf_begin_write_image(pdev, &writer, gs_no_id, w, h, NULL,
	                                 false)) < 0 ||
	   (pdev->params.MonoImage.Encode &&
	    (code = psdf_CFE_binary(&writer.binary[0], w, h, true)) < 0) ||
	   (code = pdf_begin_image_data(
	        pdev, &writer, (const gs_pixel_image_t*)&image, NULL, 0)) < 0)
		return code;
	/* Pattern masks are specified in device coordinates, so invert Y. */
	if((code = pdf_copy_mask_bits(writer.binary[0].strm,
	                              m_tile->tmask.data +
	                                  (h - 1) * m_tile->tmask.raster,
	                              0, -m_tile->tmask.raster, w, h, 0)) < 0 ||
		...
		*ppres = 0;
		...
		if(!have_pattern_streams) {
			if((code = pdf_cs_Pattern_uncolored(pdev, &v)) < 0 ||
			   (code = pdf_put_pattern_mask(pdev, m_tile,
			                                &pcs_image)) < 0 ||
			   (code = pdf_pattern(pdev, pdc, NULL, m_tile,
			                       pcs_image, ppres)) < 0)
				return code;
		} else {
			...
			if(code < 0)
				return code;
			*ppres = pdf_find_resource_by_gs_id(
			    pdev, resourcePattern, pdc->mask.id);
			*ppres = pdf_substitute_pattern(*ppres);
			if(!pdev->AR4_save_bug &&
			   pdev->CompatibilityLevel <= 1.3) {
				/* We reconnized AR4 behavior as reserving "q Q"
				 * stack elements
				 * on demand. It looks as processing a pattern
				 * stream
				 * with PaintType 1 AR4 replaces the topmost
				 * stack element
				 * instead allocating a new one, if it was not
				 * previousely allocated.
				 * AR 5 doesn't have this bug. Working around
				 * the AR4 bug here.
				 */
		...
		if(have_pattern_streams)
			return 0;
		set_nonclient_dev_color(&dc_pure, gx_dc_pure_color(pdc));
		return psdf_set_color((gx_device_vector*)pdev, &dc_pure,
		                      &no_scc);
	}
}
                        ...
                        bool have_pattern_streams, pdf_resource_t** ppres)
{
	...
	if(!have_pattern_streams) {
		/*
		 * NOTE: We assume here that the color space of the cached
		 * Pattern
		 * is the same as the native color space of the device.  This
		 * will
		 * have to change in the future!
		 */
		/*
		 * Check whether this colored pattern is actually a masked pure
		 * color,
		 * by testing whether all the colored pixels have the same
		 * color.
		 */
		m_tile = pdc->mask.m_tile;
		if(m_tile) {
			if(p_tile && !(p_tile->depth & 7) &&
			   p_tile->depth <= arch_sizeof_color_index * 8) {
				...
				gx_color_index color =
				    0; /* init is arbitrary if not empty */
				bool first = true;

				for(i = 0, bp = p_tile->tbits.data,
				mp = p_tile->tmask.data;
				    i < p_tile->tbits.rep_height; ++i,
				bp += skip, mp += p_tile->tmask.raster) {
				...
				{
					/* Set the color, then handle as an
					 * uncolored pattern. */
					gx_drawing_color dcolor;

					dcolor = *pdc;
					dcolor.colors.pure = color;
					return pdf_put_uncolored_pattern(
					    pdev, &dcolor, pcs, ppscc,
			...
			if(pdev->CompatibilityLevel < 1.3) {
				/* Masked images are only supported starting in
				 * PDF 1.3. */
				return_error(gs_error_rangecheck);
			}
		}
	...
	if(code < 0)
		return code;
	pdf_cspace_init_Device(pdev->memory, &cs_Device,
	...
	if(!have_pattern_streams) {
		...
		if(m_tile) {
			if((code = pdf_put_pattern_mask(pdev, m_tile,
			                                &pcs_mask)) < 0)
				return code;
		}
		pdf_image_writer_init(&writer);
		if((code = pdf_begin_write_image(pdev, &writer, gs_no_id, w, h,
		                                 NULL, false)) < 0 ||
		   (code = psdf_setup_lossless_filters(
		        (gx_device_psdf*)pdev, &writer.binary[0],
		        (gs_pixel_image_t*)&image)) < 0 ||
		   (code = pdf_begin_image_data(pdev, &writer,
		                                (const gs_pixel_image_t*)&image,
		                                &cs_value, 0)) < 0)
			return code;
		/* Pattern masks are specified in device coordinates, so invert
		 * Y. */
		if((code = pdf_copy_color_bits(
		        writer.binary[0].strm,
		        p_tile->tbits.data + (h - 1) * p_tile->tbits.raster, 0,
		        -p_tile->tbits.raster, w, h,
		        pdev->color_info.depth >> 3)) < 0 ||
		   (code = pdf_end_image_binary(pdev, &writer, h)) < 0)
			return code;
		pcs_image = (cos_stream_t*)writer.pres->object;
		if((pcs_mask != 0 &&
		    (code = cos_dict_put_c_key_object(
		         cos_stream_dict(pcs_image), "/Mask",
		         COS_OBJECT(pcs_mask))) < 0) ||
		   (code = pdf_end_write_image(pdev, &writer)) < 0)
			return code;
		pcs_image =
		    (cos_stream_t*)writer.pres
		        ->object; /* pdf_end_write_image may change it. */
		code = pdf_pattern(pdev, pdc, p_tile, m_tile, pcs_image, ppres);
		if(code < 0)
			return code;
	} else {
		*ppres = pdf_find_resource_by_gs_id(pdev, resourcePattern,
		                                    p_tile->id);
		*ppres = pdf_substitute_pattern(*ppres);
	}
	cos_value_write(&v, pdev);
                       ...
                       const gs_range_t** ppranges)
{
	...
	if(psh->params.Background) {
		...
		code = cos_dict_put_c_key_floats(
		    pscd, "/Background", psh->params.Background->paint.values,
                         ...
                         const gs_range_t* pranges)
{
	...
	if(pfn != 0) {
		...
		if((code = pdf_function_scaled(pscd->pdev, pfn, pranges,
		                               &fn_value)) >= 0)
...
private
int
pdf_put_linear_shading(cos_dict_t* pscd, const float* Coords, int num_coords,
                       const float* Domain /*[2]*/,
                       const gs_function_t* Function,
                       const bool* Extend /*[2]*/, const gs_range_t* pranges)
{
	int code =
	    cos_dict_put_c_key_floats(pscd, "/Coords", Coords, num_coords);

	if(code < 0 || ((Domain[0] != 0 || Domain[1] != 1) &&
	                (code = cos_dict_put_c_key_floats(pscd, "/Domain",
	                                                  Domain, 2)) < 0) ||
	...
	if(Extend[0] | Extend[1]) {
		...
		sprintf(extend_str, "[%s %s]", (Extend[0] ? "true" : "false"),
		        (Extend[1] ? "true" : "false"));
                       ...
                       const gs_range_t* pranges)
{
	...
	case shading_type_Function_based: {
		...
		if((code = cos_dict_put_c_key_floats(pscd, "/Domain",
		                                     params->Domain, 4)) < 0 ||
		   (code = pdf_put_shading_Function(pscd, params->Function,
		                                    pranges)) < 0 ||
		   (code = cos_dict_put_matrix(pscd, "/Matrix",
		                               &params->Matrix)) < 0)
			return code;
		return 0;
	}
...
private
int
put_float_mesh_data(cos_stream_t* pscs, shade_coord_stream_t* cs, int flag,
                    const pdf_mesh_data_params_t* pmdp)
{
	...
	for(i = 0; i < pmdp->num_components; ++i) {
		...
		if(pmdp->is_indexed)
			...
			/*
			 * We don't rescale stream data values, only the Decode
			 * ranges.
			 * (We do have to rescale data values from an array,
			 * unless
			...
			if(domain)
				...
				vmin = 0.0, vmax = 1.0;
			if(pranges) {
				double base = pranges[i].rmin,
				       factor = pranges[i].rmax - base;
                     ...
                     const gs_range_t* pranges)
{
	...
	if(pmp->Function) {
		...
		data_params.Domain =
		    (pmp->Decode != 0 ? pmp->Decode + 4 : NULL);
	...
	if(from_array) {
		cos_array_t* pca =
		    cos_array_alloc(pscd->pdev, "pdf_put_mesh_shading");
		int i;
		...
		if(gs_color_space_get_index(pcs) ==
		   gs_color_space_index_Indexed) {
			data_params.is_indexed = true;
			if((code =
			        pdf_array_add_real2(pca, MIN_MESH_COLOR_INDEX,
			...
			for(i = 0; i < num_comp; ++i) {
				...
				if(pmp->Function || pranges ||
				   data_params.Domain == 0)
					rmin = 0.0, rmax = 1.0;
				else
					rmin = data_params.Domain[2 * i],
					rmax = data_params.Domain[2 * i + 1];
				if((code = pdf_array_add_real2(pca, rmin,
				                               rmax)) < 0)
					return code;
			}
		}
		code =
		    cos_dict_put_c_key_object(pscd, "/Decode", COS_OBJECT(pca));
		if(code < 0)
			return code;
		...
		if(pmp->Function)
			data_params.ranges =
			    0; /* don't scale function parameter */
	} else {
		/****** SCALE Decode ******/
	...
	if(code < 0 ||
	   (code = pdf_put_shading_Function(pscd, pmp->Function, pranges)) <
	       0 ||
	...
	case shading_type_Free_form_Gouraud_triangle: {
		...
		if(from_array) {
			while((flag = shade_next_flag(&cs, 0)) >= 0)
				if((code = put_float_mesh_data(
				        pscs, &cs, flag, &data_params)) < 0)
					return code;
			if(!seofp(cs.s))
...
pdf_put_pattern2(gx_device_pdf* pdev, const gx_drawing_color* pdc,
                 const psdf_set_color_commands_t* ppscc, pdf_resource_t** ppres)
{
	...
	if(ShadingType(psh) >= 4) {
		...
		code = pdf_put_shading_common(
		    cos_stream_dict((cos_stream_t*)psco), psh, &pranges);
		if(code >= 0)
			code1 = pdf_put_mesh_shading((cos_stream_t*)psco, psh,
			                             pranges);
	} else {
                             ...
                             const byte* res_name, int name_length)
{
	...
	return pdf_color_space_named(pdev, &cs_value, NULL, cspace,
	                             &pdf_color_space_names, true, res_name,
	                             name_length);
}
