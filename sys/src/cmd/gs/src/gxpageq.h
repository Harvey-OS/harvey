...
/*	Action codes for each page queue entry. Each page that the interpreter
        ...
        repetition. NOTE that a single ACTION_TERMINATE (followed by nothing)
   can
        also show up at any point in the page queue in lieu of page
   descriptions.
...
#define private_st_gx_page_queue_entry()                                       \
	gs_private_st_ptrs2(st_gx_page_queue_entry, gx_page_queue_entry_t,     \
	                    "gx_page_queue_entry",                             \
	                    gx_page_queue_entry_enum_ptrs,                     \
	                    gx_page_queue_entry_reloc_ptrs, next, queue)
...
gx_page_queue_entry_t* gx_page_queue_entry_alloc(
 ...
 */
/* -ve error code, or 0 */
int gx_page_queue_init(gx_page_queue_t* queue, /* page queue to init */
 ...
 */
void gx_page_queue_dnit(gx_page_queue_t* queue /* page queue to dnit */
                        );
...
int gx_page_queue_wait_one_page(gx_page_queue_t* queue /* queue to wait on */
                                );
 ...
 */
void gx_page_queue_enqueue(gx_page_queue_entry_t* entry /* entry to add */
                           );
