...
typedef enum {
	...
	gs_param_type_string,
	gs_param_type_name,
	gs_param_type_int_array,
	gs_param_type_float_array,
	gs_param_type_string_array,
	gs_param_type_name_array,
...
typedef _param_array_struct(gs_param_string_array_s,
                            gs_param_string) gs_param_string_array;

#define param_string_from_string(ps, str)                                      \
	((ps).data = (const byte*)(str),                                       \
	 (ps).size = strlen((const char*)(ps).data), (ps).persistent = true)

#define param_string_from_transient_string(ps, str)                            \
	((ps).data = (const byte*)(str),                                       \
	 (ps).size = strlen((const char*)(ps).data), (ps).persistent = false)
...
#define GS_PARAM_TYPE_BASE_SIZES(dict_elt_size)                                \
	0, sizeof(bool), sizeof(int), sizeof(long), sizeof(float), 1, 1,       \
	    sizeof(int), sizeof(float), sizeof(gs_param_string),               \
	    sizeof(gs_param_string), (dict_elt_size), (dict_elt_size),         \
	    (dict_elt_size)
...
#define param_proc_request(proc) int proc(gs_param_list*, gs_param_name)
	param_proc_request((*request));

#define param_request(plist, pkey) ((plist)->procs->request(plist, pkey))
...
#define param_proc_requested(proc) int proc(const gs_param_list*, gs_param_name)
	param_proc_requested((*requested));
#define param_requested(plist, pkey) (*(plist)->procs->requested)(plist, pkey)

/* Get the 'policy' associated with an out-of-range parameter value. */
/* (Only used when reading.) */

#define param_proc_get_policy(proc) int proc(gs_param_list*, gs_param_name)
	param_proc_get_policy((*get_policy));
#define param_get_policy(plist, pkey) (*(plist)->procs->get_policy)(plist, pkey)
...
#define param_proc_commit(proc) int proc(gs_param_list*)
	param_proc_commit((*commit));
#define param_commit(plist) (*(plist)->procs->commit)(plist)
...
int param_write_string(gs_param_list*, gs_param_name, const gs_param_string*);
int param_read_name(gs_param_list*, gs_param_name, gs_param_string*);
int param_write_name(gs_param_list*, gs_param_name, const gs_param_string*);
int param_read_int_array(gs_param_list*, gs_param_name, gs_param_int_array*);
int param_write_int_array(gs_param_list*, gs_param_name,
                          const gs_param_int_array*);
int param_write_int_values(gs_param_list*, gs_param_name, const int*, uint,
                           bool);
...
int param_write_float_values(gs_param_list*, gs_param_name, const float*, uint,
                             bool);
 ...
 */
int param_coerce_typed(gs_param_typed_value* pvalue, gs_param_type req_type,
                       gs_memory_t* mem);
...
#define private_st_c_param_list() /* in gsparam.c */                           \
	gs_private_st_ptrs2(st_c_param_list, gs_c_param_list, "c_param_list",  \
	                    c_param_list_enum_ptrs, c_param_list_reloc_ptrs,   \
	                    head, target)
...
#define private_st_gs_param_string() /* in gdevdevn.c */                       \
	gs_private_st_composite(st_gs_param_string, gs_param_string,           \
	                        "gs_param_string", param_string_enum_ptrs,     \
	                        param_string_reloc_ptrs)
...
void gs_c_param_list_write_more(
    gs_c_param_list*); /* switch back to writing, no init */
