...
private
const struct_shared_procs_t refs_shared_procs =
		   ...
		   r_space(pref) >= min_trace) {
			...
			case t_name: {
				...
				SET_RELOC(pref->value.pname,
					  (name *)((char *)rsub + ((char *)pref->value.pname -
								   (char *)psub)));
			} break;
			case t_string: {
				gs_string str;
...
#define RP_REF(rp) ((const ref *)rp)
	...
	for(;;) {
		...
		if(!ref_type_uses_size_or_null(r_type(RP_REF(rp)))) {
			...
			rputc('\n');
			rp = print_reloc(prp, "ref",
					 (const ref_packed *)(r_size(RP_REF(rp)) == 0 ? prp : (const ref_packed *)((const char *)prp -
