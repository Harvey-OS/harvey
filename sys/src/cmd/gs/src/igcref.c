 ...
 */
private
gc_proc_clear_reloc(refs_clear_reloc);
private
gc_proc_set_reloc(refs_set_reloc);
private
gc_proc_compact(refs_compact);
private
const struct_shared_procs_t refs_shared_procs = {refs_clear_reloc,
                                                 refs_set_reloc, refs_compact};
private
struct_proc_clear_marks(refs_clear_marks);
private
struct_proc_reloc_ptrs(refs_do_reloc);
const gs_memory_struct_type_t st_refs = {sizeof(ref), "refs",
                                         &refs_shared_procs, refs_clear_marks,
                                         0, refs_do_reloc};
...
private
void
refs_clear_marks(const gs_memory_t* cmem, void /*obj_header_t */* vptr,
                 uint size, const gs_memory_struct_type_t* pstype)
{
...
#ifdef DEBUG
			if(gs_debug_c('8')) {
				dlprintf1("  [8]unmark packed 0x%lx ",
				          (ulong)rp);
...
refs_clear_reloc(obj_header_t* hdr, uint size)
{
	...
	while(rp < end) {
		if(r_is_packed(rp))
			...
			/* Full-size ref.  Store the relocation here if
			 * possible. */
			ref* const pref = (ref*)rp;
...
#define marked                                                                 \
	((*rp & lp_mark) + (rp[1] & lp_mark) + (rp[2] & lp_mark) +             \
	 (rp[3] & lp_mark))
			...
			case all_marked:
				if_debug2(
				    '8', "  [8]packed refs 0x%lx..0x%lx are "
				         "marked\n",
				    (ulong)rp,
			...
			case 0:
#endif
				if_debug2('8', "  [8]%d packed ref(s) at 0x%lx "
				               "are unmarked\n",
				...
				rp += align_packed_per_ref;
				freed +=
				    sizeof(ref_packed) * align_packed_per_ref;
			}
		} else { /* full-size ref */
			uint rel = reloc + freed;
...
{
	igc_reloc_refs((ref_packed*)vptr, (ref_packed*)((char*)vptr + size),
	               gcst);
}
		...
		if((r_has_attr(pref, l_mark) || do_all) &&
		   r_space(pref) >= min_trace) {
			...
			case t_dictionary:
				...
				SET_RELOC(
				    pref->value.pdict,
				    (dict*)igc_reloc_ref_ptr(
				        (ref_packed*)pref->value.pdict, gcst));
				break;
			case t_array: {
				...
				if(size != 0) { /* value.refs might be NULL */
					...
					/*
					 * If the array is large, we allocated
					 * it in its
					 * own object (at least originally --
					 * this might
					 * be a pointer to a subarray.)  In this
					 * case,
					 * we know it is the only object in its
					 * containing st_refs object, so we know
					 * that
					 * the mark containing the relocation
					 * appears
					 * just after it.
					 */
					if(size <
					   max_size_st_refs / sizeof(ref)) {
						rputc('a');
						SET_RELOC(
						    pref->value.refs,
			...
			case t_mixedarray:
				if(r_size(pref) !=
				   0) { /* value.refs might be NULL */
					rputc('m');
					SET_RELOC(
					    pref->value.packed,
					    igc_reloc_ref_ptr(
					        pref->value.packed, gcst));
				}
				break;
			case t_shortarray: {
				uint size = r_size(pref);
				 ...
				 * elements of this array itself are marked, we
				 * can
				 * save some scanning time by relocating the
				 * pointer
				...
				if(size != 0) { /* value.refs might be NULL */
					rputc('s');
					/*
					 * igc_reloc_ref_ptr has to be able to
					 * determine
					 * whether the pointer points into a
					 * space that
					 * isn't being collected.  It does this
					 * by
					 * checking whether the referent of the
					 * pointer
					 * is marked.  For this reason, we have
					 * to pass
					 * a pointer to the last real element of
					 * the
			...
			case t_name: {
				...
				void* rsub =
				    RELOC_OBJ(psub); /* gcst implicit */

				SET_RELOC(pref->value.pname,
				          (name*)((char*)rsub +
				                  ((char*)pref->value.pname -
				                   (char*)psub)));
			} break;
			case t_string: {
				gs_string str;
...
#define RP_REF(rp) ((const ref*)rp)
	...
	for(;;) {
		...
		if(r_is_packed(rp)) {
			...
			if(!(*rp & lp_mark)) {
				if(*rp !=
				   pt_tag(pt_integer) + packed_max_value) {
					/* This is a stored relocation value. */
					rputc('\n');
					rp = print_reloc(
					    prp, "ref",
					    (const ref_packed*)((const char*)
					                            prp -
					                        (*rp &
					                         packed_value_mask) +
					                        dec));
		...
		if(!ref_type_uses_size_or_null(r_type(RP_REF(rp)))) {
			...
			rp = print_reloc(
			    prp, "ref",
			    (const ref_packed*)(r_size(RP_REF(rp)) == 0
			                            ? prp
			                            : (const ref_packed*)((const char*)
			                                                      prp -
			                                                  r_size(RP_REF(
...
private
void
refs_compact(const gs_memory_t* mem, obj_header_t* pre, obj_header_t* dpre,
             uint size)
{
	ref_packed* dest;
	ref_packed* src;
	...
	for(;;) {
		if(r_is_packed(src)) {
			if(r_has_pmark(src)) {
				if_debug2(
				    '8',
				    "  [8]packed ref 0x%lx copied to 0x%lx\n",
	...
	if((byte*)src - (byte*)dest != r_size((ref*)src - 1) + sizeof(ref)) {
		lprintf3(
		    "Reloc error for refs 0x%lx: reloc = %lu, stored = %u\n",
	...
	while(new_size & (sizeof(ref) - 1))
		*dest++ = pt_tag(pt_integer), new_size += sizeof(ref_packed);
	/* We want to make the newly freed space into a free block, */
	/* but we can only do this if we have enough room. */
	if(size - new_size <
	   sizeof(obj_header_t)) { /* Not enough room.  Pad to original size. */
		while(new_size < size)
