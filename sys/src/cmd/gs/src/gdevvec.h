...
typedef enum {
	...
	gx_path_type_always_close =
	    32, /* include final closepath even if not stroke */
...
typedef struct gx_device_vector_procs_s {
	...
	int (*moveto)(gx_device_vector* vdev, floatp x0, floatp y0, floatp x,
	              floatp y, gx_path_type_t type);
	int (*lineto)(gx_device_vector* vdev, floatp x0, floatp y0, floatp x,
	              floatp y, gx_path_type_t type);
	int (*curveto)(gx_device_vector* vdev, floatp x0, floatp y0, floatp x1,
...
int gdev_vector_dorect(gx_device_vector* vdev, fixed x0, fixed y0, fixed x1,
                       fixed y1, gx_path_type_t type);
...
#define public_st_device_vector() /* in gdevvec.c */                           \
	gs_public_st_suffix_add3_final(                                        \
	    st_device_vector, gx_device_vector, "gx_device_vector",            \
	    device_vector_enum_ptrs, device_vector_reloc_ptrs,                 \
	    gx_device_finalize, st_device, strm, strmbuf, bbox_device)
...
int gdev_vector_open_file_options(gx_device_vector* vdev, uint strmbuf_size,
                                  int open_options);
...
int gdev_vector_prepare_fill(gx_device_vector* vdev, const gs_imager_state* pis,
...
int gdev_vector_prepare_stroke(gx_device_vector* vdev,
                               ...
                               const gx_drawing_color* pdcolor, floatp scale);
...
int gdev_vector_write_rectangle(gx_device_vector* vdev, fixed x0, fixed y0,
                                fixed x1, fixed y1, bool close,
                                gx_rect_direction_t dir);
...
#define gdev_vector_image_enum_common                                          \
	...
	gx_image_enum_common_t*                                                \
	    default_info; /* non-0 iff using default implementation */         \
	gx_image_enum_common_t*                                                \
	    bbox_info; /* non-0 iff passing image data to bbox dev */          \
	int width, height;                                                     \
 ...
 * enumerator.
 */
int gdev_vector_begin_image(gx_device_vector* vdev, const gs_imager_state* pis,
...
int gdev_vector_end_image(gx_device_vector* vdev, gdev_vector_image_enum_t* pie,
