...
int
alloc_function_array(uint count, gs_function_t*** pFunctions, gs_memory_t* mem)
{
	gs_function_t** ptr;
...
fn_common_get_params(const gs_function_t* pfn, gs_param_list* plist)
{
	...
	if(pfn->params.Domain) {
		code = param_write_float_values(plist, "Domain",
		                                pfn->params.Domain,
	...
	if(pfn->params.Range) {
		code =
		    param_write_float_values(plist, "Range", pfn->params.Range,
...
fn_copy_values(const void* pvalues, int count, int size, gs_memory_t* mem)
{
	if(pvalues) {
		void* values =
		    gs_alloc_byte_array(mem, count, size, "fn_copy_values");
               ...
               const gs_range_t* pranges, gs_memory_t* mem)
{
	if(pvalues == 0)
		*ppvalues = 0;
		...
		float* out = (float*)gs_alloc_byte_array(
		    mem, 2 * npairs, sizeof(*pvalues), "fn_scale_pairs");
...
fn_common_serialize(const gs_function_t* pfn, stream* s)
{
	...
	int code =
	    sputs(s, (const byte*)&pfn->head.type, sizeof(pfn->head.type), &n);
	...
	if(code < 0)
		return code;
	code = sputs(s, (const byte*)&p->Domain[0],
	             sizeof(p->Domain[0]) * p->m * 2, &n);
	if(code < 0)
		return code;
