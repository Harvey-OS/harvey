...
#define LOOKUP_INDEX_(vin, pcache, fbits)                                                                                                                                                                     \
	(cie_cached_value)((vin) <= (pcache)->vecs.params.base ? 0 : (vin) >= (pcache)->vecs.params.limit ? (gx_cie_cache_size - 1) << (fbits) : cie_cached_product2int(((vin) - (pcache)->vecs.params.base), \
...
/* Forward references */
private
void cie_lookup_mult3(cie_cached_vector3 *,
		      ...
		      frac *pconc, const gs_imager_state *pis)
{
	...
	for(i = 0; i < 4; ++i) {
		...
		double value =
		    (v0 < rangeDEFG->rmin ? 0.0 : v0 > rangeDEFG->rmax ? factor : (v0 - rangeDEFG->rmin) * factor /
		     ...
		     frac *pconc, const gs_imager_state *pis)
{
	...
	for(i = 0; i < 3; ++i) {
		...
		double value =
		    (v0 < rangeDEF->rmin ? 0.0 : v0 > rangeDEF->rmax ? factor : (v0 - rangeDEF->rmin) * factor /
...
#define SET_TABC(i, t)                                                                             \
	BEGIN                                                                                      \
	tabc[i] = cie_cached2int(vec3 /*ABC*/.t - pcrd->EncodeABC_base[i], _cie_interpolate_bits); \
	if((uint)tabc[i] > (gx_cie_cache_size - 1) << _cie_interpolate_bits)                       \
		tabc[i] = (tabc[i] < 0 ? 0 : (gx_cie_cache_size - 1) << _cie_interpolate_bits);    \
	END
	SET_TABC(0, u);
	SET_TABC(1, v);
	SET_TABC(2, w);
...
#define LOOKUP_INTERPOLATE_BETWEEN(v0, v1, i) \
	((v0) + ((v1) - (v0)) *((ftemp = float_rshift(i, _cie_interpolate_bits)), ftemp - (int)ftemp))
#endif
