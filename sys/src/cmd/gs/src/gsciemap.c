...
#define LOOKUP_INDEX_(vin, pcache, fbits)                                      \
	(cie_cached_value)(                                                    \
	    (vin) <= (pcache)->vecs.params.base                                \
	        ? 0                                                            \
	        : (vin) >= (pcache)->vecs.params.limit                         \
	              ? (gx_cie_cache_size - 1) << (fbits)                     \
	              : cie_cached_product2int(                                \
	                    ((vin) - (pcache)->vecs.params.base),              \
...
private
cie_cached_value
LOOKUP_INDEX(cie_cached_value vin, const gx_cie_vector_cache* pcache, int fbits)
{
	return LOOKUP_INDEX_(vin, pcache, fbits);
}
private
const cie_cached_vector3*
...
/* Forward references */
private
void cie_lookup_mult3(cie_cached_vector3*, const gx_cie_vector_cache3_t*);

#ifdef DEBUG
private
void
cie_lookup_map3(cie_cached_vector3* pvec, const gx_cie_vector_cache3_t* pc,
                const char* cname)
{
	if_debug5('c', "[c]lookup %s 0x%lx [%g %g %g]\n", (const char*)cname,
	          (ulong)pc, cie_cached2float(pvec->u),
	          cie_cached2float(pvec->v), cie_cached2float(pvec->w));
	cie_lookup_mult3(pvec, pc);
	if_debug3('c', "        =[%g %g %g]\n", cie_cached2float(pvec->u),
	          cie_cached2float(pvec->v), cie_cached2float(pvec->w));
}
#else
                      ...
                      frac* pconc, const gs_imager_state* pis)
{
	...
	for(i = 0; i < 4; ++i) {
		...
		double factor =
		    pcie->caches_defg.DecodeDEFG[i].floats.params.factor;
		...
		if(vf != 0 && vi < factor)
			v += vf * (pcie->caches_defg.DecodeDEFG[i]
			               .floats.values[vi + 1] -
			           v);
                     ...
                     frac* pconc, const gs_imager_state* pis)
{
	...
	if_debug3('c', "[c]concretize DEF [%g %g %g]\n", pc->paint.values[0],
	          pc->paint.values[1], pc->paint.values[2]);
	CIE_CHECK_RENDERING(pcs, pconc, pis, return 0);
	...
	for(i = 0; i < 3; ++i) {
		int tdim = pcie->Table.dims[i] - 1;
		double factor =
		    pcie->caches_def.DecodeDEF[i].floats.params.factor;
		double v0 = pc->paint.values[i];
		...
		if(vf != 0 && vi < factor)
			v += vf * (pcie->caches_def.DecodeDEF[i]
			               .floats.values[vi + 1] -
			           v);
		v = (v < 0 ? 0 : v > tdim ? tdim : v);
		hij[i] = float2fixed(v);
                ...
                gx_device* dev, gs_color_select_t select)
{
	...
	case 4:
		if_debug4('c', "[c]=CMYK [%g %g %g %g]\n", frac2float(conc[0]),
		          frac2float(conc[1]), frac2float(conc[2]),
		          frac2float(conc[3]));
		gx_remap_concrete_cmyk(conc[0], conc[1], conc[2], conc[3], pdc,
		                       pis, dev, select);
		goto done;
...
map3:
	if_debug3('c', "[c]=RGB [%g %g %g]\n", frac2float(conc[0]),
	          frac2float(conc[1]), frac2float(conc[2]));
	gx_remap_concrete_rgb(conc[0], conc[1], conc[2], pdc, pis, dev, select);
done:
...
#define SET_TABC(i, t)                                                         \
	...
	if((uint)tabc[i] > (gx_cie_cache_size - 1) << _cie_interpolate_bits)   \
		tabc[i] = (tabc[i] < 0 ? 0 : (gx_cie_cache_size - 1)           \
		                                 << _cie_interpolate_bits);    \
	END
	SET_TABC(0, u);
	SET_TABC(1, v);
 ...
 */
#define EABC(i)                                                                \
	cie_interpolate_fracs(pcrd->caches.EncodeABC[i].fixeds.fracs.values,   \
	                      tabc[i])
		pconc[0] = EABC(0);
		pconc[1] = EABC(1);
		pconc[2] = EABC(2);
...
#define EABC(i)                                                                \
	cie_interpolate_fracs(pcrd->caches.EncodeABC[i].fixeds.ints.values,    \
	                      tabc[i])
#define FABC(i, s) ((s) > 0) ? (EABC(i) << (s)) : (EABC(i) >> -(s))
		rfix[0] = FABC(0, s);
		rfix[1] = FABC(1, s);
		rfix[2] = FABC(2, s);
#undef FABC
...
#define frac2cache_index(v) frac2bits(v, gx_cie_log2_cache_size)
			...
			if(m > 3)
				pconc[3] =
				    RT_LOOKUP(3, frac2cache_index(pconc[3]));
...
#define byte2cache_index(b) ((b) >> (8 - gx_cie_log2_cache_size))
			...
			if(m > 3)
				pconc[3] =
				    RT_LOOKUP(3, byte2cache_index(prtc[3]));
...
private
void
cie_lookup_mult3(cie_cached_vector3* pvec, const gx_cie_vector_cache3_t* pc)
{
...
#define LOOKUP_INTERPOLATE_BETWEEN(v0, v1, i)                                  \
	((v0) +                                                                \
	 ((v1) - (v0))*((ftemp = float_rshift(i, _cie_interpolate_bits)),      \
	                ftemp - (int)ftemp))
#endif
...
#define I_ENTRY(i, j)                                                          \
	&pc->caches[j]                                                         \
	     .vecs.values[(int)cie_cached_rshift(i, _cie_interpolate_bits)]
#define I_ENTRY1(i, p)                                                         \
	...
	if(I_IN_RANGE(0, u)) {
		...
		const cie_cached_vector3* p =
		    LOOKUP_ENTRY(pvec->u, &pc->caches[0]);
	...
	if(I_IN_RANGE(1, v)) {
		...
		const cie_cached_vector3* p =
		    LOOKUP_ENTRY(pvec->v, &pc->caches[1]);
	...
	if(I_IN_RANGE(2, w)) {
		...
		const cie_cached_vector3* p =
		    LOOKUP_ENTRY(pvec->w, &pc->caches[2]);
