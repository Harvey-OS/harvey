...
type1_callsubr(gs_type1_state* pcis, int index)
{
	...
	int code =
	    pfont->data.procs.subr_data(pfont, index, false, &ipsp1->cs_data);
	...
	for(;;) {
		...
		if(c >= c_num1) {
			...
			if(c < c_pos2_0) { /* 1-byte number */
				...
				decode_push_num2(csp, pcis->ostack, c, cip,
				                 state, encrypted);
		...
		case cx_escape:
			...
			case ce1_div:
				csp[-1] =
				    float2fixed((double)csp[-1] / (double)*csp);
				--csp;
				continue;
                           ...
                           gs_font_type1* pfont)
{
	...
	enum { dotsection_in = 0,
	       dotsection_out = -1 } dotsection_flag = dotsection_out;
...
#define CLEAR_OP() (depth = 0, prev_op = -1)
#define CHECK_OP()                                                             \
	BEGIN                                                                  \
	if(prev_op >= 0) {                                                     \
	 ...
	 */
	hstem_hints.count = hstem_hints.replaced_count = hstem_hints.current =
	    0;
	vstem_hints.count = vstem_hints.replaced_count = vstem_hints.current =
	    0;
	...
	for(;;) {
		...
		default:
			...
			continue;
		case c1_hsbw:
			gs_type1_sbw(&cis, cis.ostack[0], fixed_0,
			             cis.ostack[1], fixed_0);
			goto clear;
		case cx_hstem:
			type1_stem1(&cis, &hstem_hints, csp - 1, cis.lsb.y,
			            NULL);
			goto clear;
		case cx_vstem:
			type1_stem1(&cis, &vstem_hints, csp - 1, cis.lsb.x,
			            NULL);
			goto clear;
		...
		case CE_OFFSET + ce1_vstem3:
			type1_stem3(&cis, &vstem_hints, csp - 5, cis.lsb.x,
			            NULL);
			goto clear;
		case CE_OFFSET + ce1_hstem3:
			type1_stem3(&cis, &hstem_hints, csp - 5, cis.lsb.y,
			            NULL);
		clear:
			type1_clear(&cis);
			continue;
	...
	if(replace_hints) {
		hintmask_size = (hstem_hints.count + vstem_hints.count + 7) / 8;
	...
	for(;;) {
		int c = type1_next(&cis);
		fixed* csp = &cis.ostack[cis.os_count - 1];
#define POP(n) (csp -= (n), cis.os_count -= (n))
		int i;
		fixed mx, my;
		...
		case CE_OFFSET + ce1_dotsection:
			if(dotsection_flag == dotsection_out) {
				memcpy(dot_save_hints, active_hints,
				       hintmask_size);
				memset(active_hints, 0, hintmask_size);
				dotsection_flag = dotsection_in;
			} else {
				memcpy(active_hints, dot_save_hints,
				       hintmask_size);
		...
		case CE_OFFSET + ce1_setcurrentpoint:
			if(first) {
				/*  A workaround for fonts which use
				   ce1_setcurrentpoint
				    in an illegal way for shifting a path.
				    See t1_hinter__setcurrentpoint for more
				   information. */
				mx0 = csp[-1], my0 = *csp;
			}
			continue;
		...
		case c1_hsbw:
			gs_type1_sbw(&cis, cis.ostack[0], fixed_0,
			             cis.ostack[1], fixed_0);
			/*
			 * Leave the l.s.b. on the operand stack for the initial
			 * hint,
		...
		case CE_OFFSET + ce1_seac:
			/*
			 * It is an undocumented feature of the Type 2
			 * CharString
			 * format that endchar + 4 or 5 operands is equivalent
			 * to
			 * seac with an implicit asb operand + endchar with 0 or
			 * 1
			 * operands.  Remove the asb argument from the stack,
			 * but
			 * adjust the adx argument to compensate for the fact
			 * that
		...
		case ce1_callothersubr:
			...
			case 0:
				/*
				 * The operand stack contains: delta to
				 * reference point,
				 * 6 deltas for the two curves, fd, final point,
				 * 3, 0.
				 */
		...
		case cx_hlineto: /* hlineto (vlineto hlineto)* [vlineto] =>
		                    hlineto */
			...
			c = prev_op;
			goto put;
		case cx_vlineto: /* vlineto (hlineto vlineto)* [hlineto] =>
		                    vlineto */
			if(depth > MAX_STACK - 1 ||
			...
			c = prev_op;
			goto put;
		case cx_hvcurveto: /* hvcurveto (vhcurveto hvcurveto)* =>
		                      hvcurveto */
			/* (vhcurveto hvcurveto)+ => vhcurveto  */
			/*
			 * We have to check (depth & 1) because the last curve
			 * might
			 * have 5 parameters rather than 4 (see rrcurveto
			 * below).
			 */
			...
			c = prev_op;
			goto put;
		case cx_vhcurveto: /* vhcurveto (hvcurveto vhcurveto)* =>
		                      vhcurveto */
		...
		case cx_rrcurveto:
			if(depth == 0) {
				if(csp[-1] == 0) {
					/* A|0 B C D 0 F rrcurveto => [A] B C D
					 * F vvcurveto */
					c = c2_vvcurveto;
					csp[-1] = csp[0];
					if(csp[-5] == 0) {
					...
					/* A B|0 C D E 0 rrcurveto => [B] A C D
					 * E hhcurveto */
					c = c2_hhcurveto;
					if(csp[-4] == 0) {
						memmove(csp - 4, csp - 3,
				 ...
				 *   0 B C D E F|0 rrcurveto => B C D E [F]
				 * vhcurveto
				 *   A 0 C D E|0 F rrcurveto => A C D F [E]
				 * hvcurveto
				 * but this gets in the way of subsequent
				 * optimization
			...
			if(depth > MAX_STACK - 6)
				goto copy;
			switch(prev_op) {
			case c2_hhcurveto: /* hrcurveto (x1 0 x2 y2 x3 0
			                      rrcurveto)* => */
				/* hhcurveto */
				if(csp[-4] == 0 && *csp == 0) {
					memmove(csp - 4, csp - 3,
				...
				goto copy;
			case c2_vvcurveto: /* rvcurveto (0 y1 x2 y2 0 y3
			                      rrcurveto)* => */
				/* vvcurveto */
				if(csp[-5] == 0 && csp[-1] == 0) {
					memmove(csp - 5, csp - 4,
			...
			vrc: /* (vhcurveto hvcurveto)+ vrcurveto => vhcurveto */
				/* hvcurveto (vhcurveto hvcurveto)* vrcurveto =>
				 * hvcurveto */
				if(csp[-5] != 0)
					goto copy;
			...
			hrc: /* (hvcurveto vhcurveto)+ hrcurveto => hvcurveto */
				/* vhcurveto (hvcurveto vhcurveto)* hrcurveto =>
				 * vhcurveto */
				if(csp[-4] != 0)
					goto copy;
