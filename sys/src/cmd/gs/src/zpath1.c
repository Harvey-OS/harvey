...
/* Forward references */
private
int common_arc(i_ctx_t*,
               int (*)(gs_state*, floatp, floatp, floatp, floatp, floatp));
private
int common_arct(i_ctx_t*, float*);
...
common_arct(i_ctx_t* i_ctx_p, float* tanxy)
{
	...
	if(code < 0)
		return code;
	return gs_arcto(igs, args[0], args[1], args[2], args[3], args[4],
	                tanxy);
}
...
private
int path_continue(i_ctx_t*);
private
int path_cleanup(i_ctx_t*);
private
int
...
private
void pf_push(i_ctx_t*, gs_point*, int);
private
int
...
const op_def zpath1_op_defs[] = {{"5arc", zarc},
                                 {"5arcn", zarcn},
                                 {"5arct", zarct},
                                 {"5arcto", zarcto},
                                 {"0clippath", zclippath},
