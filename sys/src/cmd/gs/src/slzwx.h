...
#define public_st_LZW_state() /* in slzwc.c */                                 \
	gs_public_st_ptrs1(st_LZW_state, stream_LZW_state, "LZWDecode state",  \
	                   lzwd_enum_ptrs, lzwd_reloc_ptrs, table.decode)
#define s_LZW_set_defaults_inline(ss)                                          \
	((ss)->InitialCodeLength = 8, (ss)->FirstBitLowOrder = false,          \
	 (ss)->BlockData = false, (ss)->EarlyChange = 1, /* Clear pointers */  \
