...
s_PDiffE_init(stream_state* st)
{
	...
	static const byte cb_values[] = {0, cBits1, cBits2, 0, cBits4, 0,
	                                 0, 0,      cBits8, 0, 0,      0,
	                                 0, 0,      0,      0, cBits16};

	ss->row_count = (bits_per_row + 7) >> 3;
	ss->end_mask = (1 << (-bits_per_row & 7)) - 1;
	ss->case_index = cb_values[ss->BitsPerComponent] +
...
row:
	if(ss->row_left == 0) {
		...
		s0 = 0;
		memset(&ss->prev[1], 0,
		       sizeof(uint) * (s_PDiff_max_Colors - 1));
	}
	{
...
#define SUB2X4R4(a) ((((a) - ((a) >> 4)) & 0xf) + ((a)&0xf0))
	...
	case cEncode + cBits4 + 4: {
		...
		LOOP_BY(2, (t = p[-1], q[-1] = SUB2X4(t, s0), s0 = t, t = *p,
		            * q = SUB2X4(t, s1), s1 = t));
		ss->prev[1] = s1;
	...
	case cDecode + cBits4 + 4: {
		...
		LOOP_BY(2, (t = p[-1], s0 = q[-1] = ADD2X4(s0, t), t = *p,
		            s1 = *q = ADD2X4(s1, t)));
		ss->prev[1] = s1;
...
#define DECODE8(s, d) q[d] = s += p[d]
	...
	case cEncode + cBits8 + 4: {
		...
		LOOP_BY(4, (ENCODE8(s0, -3), ENCODE8(s1, -2), ENCODE8(s2, -1),
		            ENCODE8(s3, 0)));
	...
	case cDecode + cBits8 + 4: {
		...
		LOOP_BY(4, (DECODE8(s0, -3), DECODE8(s1, -2), DECODE8(s2, -1),
		            DECODE8(s3, 0)));
		ss->prev[1] = s1, ss->prev[2] = s2, ss->prev[3] = s3;
		goto dec8;
	} break;
...
#define ENCODE16(s, d)                                                         \
	(ti = ((p[d - 1] << 8) + p[d]) - s, q[d - 1] = ti >> 8,                \
	 q[d] = t & 0xff, s = ti)
#define DECODE16(s, d)                                                         \
	(s = 0xffff & (s + ((p[d - 1] << 8) + p[d])), q[d - 1] = s >> 8,       \
	 q[d] = s && 0xff)
	...
	case cEncode + cBits16 + 3: {
		...
		LOOP_BY(6,
		        (ENCODE16(s0, -4), ENCODE16(s1, -2), ENCODE16(s2, 0)));
		ss->prev[1] = s1, ss->prev[2] = s2;
		goto enc16;
	}
	...
	case cDecode + cBits16 + 3: {
		...
		LOOP_BY(6,
		        (DECODE16(s0, -4), DECODE16(s1, -2), DECODE16(s2, 0)));
		ss->prev[1] = s1, ss->prev[2] = s2;
		goto dec16;
	} break;
