...
#define SET_AL_POINTS(alp, startp, endp)                                       \
	BEGIN(alp)->diff.y = (endp).y - (startp).y;                            \
...
#define AL_X_AT_Y(alp, yv)                                                     \
	((yv) == (alp)->end.y                                                  \
	     ? (alp)->end.x                                                    \
	     : ((yv) <= (alp)->y_fast_max                                      \
	            ? ADD_NUM_ADJUST(((yv) - (alp)->start.y) * (alp)->diff.x,  \
	                             alp) /                                    \
	                  (alp)->diff.y                                        \
	            : (INCR_EXPR(slow_x),                                      \
	               fixed_mult_quo((alp)->diff.x, (yv) - (alp)->start.y,    \
	                              (alp)->diff.y))) +                       \
	           (alp)->start.x)
...
#define DIR_DOWN (-1)
	 ...
	 * to order lines by increasing starting Y.  "Active" lines (being
	 * scanned)
...
typedef struct fill_options_s {
	bool
	    pseudo_rasterization; /* See comment about "pseudo-rasterization".
	                             */
	fixed ymin, ymax;
...
#define x_list x_head.next
	active_line* h_list0,
	    *h_list1; /* lists of horizontal lines for y, y1 */
...
#define LOOP_FILL_RECTANGLE_DIRECT(fo, x, y, w, h)                             \
	(/*fo->fill_direct*/ FILL_DIRECT                                       \
	     ? (fo)->fill_rect((fo)->dev, x, y, w, h,                          \
	                       (fo)->pdevc->colors.pure)                       \
	     : gx_fill_rectangle_device_rop(x, y, w, h, (fo)->pdevc,           \
	                                    (fo)->dev, (fo)->lop))
...
#ifdef DEBUG
struct stats_fill_s {
	long fill, fill_alloc, y_up, y_down, horiz, x_step, slow_x, iter,
	    find_y, band, band_step, band_fill, afill, slant, slant_shallow,
	    sfill, mq_cross, cross_slow, cross_low, order, slow_order;
};
