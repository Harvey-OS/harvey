...
#define SET_AL_POINTS(alp, startp, endp)                                                                                          \
	...
	SET_NUM_ADJUST(alp);                                                                                                      \
	(alp)->y_fast_max = MAX_MINUS_NUM_ADJUST(alp) / (((alp)->diff.x >= 0 ? (alp)->diff.x : -(alp)->diff.x) | 1) + (startp).y; \
...
#define AL_X_AT_Y(alp, yv)                                                                                                                                                                                                          \
	((yv) == (alp)->end.y ? (alp)->end.x : ((yv) <= (alp)->y_fast_max ? ADD_NUM_ADJUST(((yv) - (alp)->start.y) * (alp)->diff.x, alp) / (alp)->diff.y : (INCR_EXPR(slow_x),                                                      \
...
#define LOOP_FILL_RECTANGLE_DIRECT(fo, x, y, w, h) \
	(/*fo->fill_direct*/ FILL_DIRECT ? (fo)->fill_rect((fo)->dev, x, y, w, h, (fo)->pdevc->colors.pure) : gx_fill_rectangle_device_rop(x, y, w, h, (fo)->pdevc, (fo)->dev, (fo)->lop))
...
struct stats_fill_s {
	long
	    fill,
	    fill_alloc, y_up, y_down, horiz, x_step, slow_x, iter, find_y,
