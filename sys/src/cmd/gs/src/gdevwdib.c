...
/* Copyright (C) 1992, 1995, 1996, 1997, 1999 Aladdin Enterprises.  All rights
  reserved.
...
private
dev_proc_open_device(win_dib_open);
private
dev_proc_get_initial_matrix(win_dib_get_initial_matrix);
private
dev_proc_close_device(win_dib_close);
private
dev_proc_fill_rectangle(win_dib_fill_rectangle);
private
dev_proc_copy_mono(win_dib_copy_mono);
private
dev_proc_copy_color(win_dib_copy_color);
private
dev_proc_get_bits(win_dib_get_bits);
private
dev_proc_put_params(win_dib_put_params);

/* Windows-specific procedures */
private
win_proc_repaint(win_dib_repaint);
private
win_proc_alloc_bitmap(win_dib_alloc_bitmap);
private
win_proc_free_bitmap(win_dib_free_bitmap);
...
private
const gx_device_procs win_dib_procs = {
    win_dib_open, win_dib_get_initial_matrix, win_sync_output, win_output_page,
    win_dib_close, win_map_rgb_color, win_map_color_rgb, win_dib_fill_rectangle,
    NULL,                                        /* tile_rectangle */
    win_dib_copy_mono, win_dib_copy_color, NULL, /* draw_line */
    win_dib_get_bits /* NULL */,                 /* get_bits */
    win_get_params, win_dib_put_params, NULL,    /* map_cmyk_color */
    win_get_xfont_procs, NULL,                   /* get_xfont_device */
...
private
HGLOBAL win_dib_make_dib(gx_device_win* dev, int orgx, int orgy, int wx,
                         int wy);
private
int win_dib_lock_device(unsigned char* device, int flag);
...
#ifdef __WIN32__
	if(!is_win32s)
		wdev->hmtx = CreateMutex(
		    NULL, FALSE, NULL); /* unnamed mutex, initially unowned */
#endif
...
private
int
win_dib_copy_mono(gx_device* dev, const byte* base, int sourcex, int raster,
                  gx_bitmap_id id, int x, int y, int w, int h,
                  gx_color_index zero, gx_color_index one)
{
#if USE_SEGMENTS
	if(single_block(y, h)) {
		wmproc(copy_mono)(wmdev, base, sourcex, raster, id, x, y, w, h,
		                  zero, one);
	} else { /* Divide the transfer into blocks. */
		...
		wmproc(copy_mono)(wmdev, source, sourcex, raster,
		                  gx_no_bitmap_id, x, by, w, bh, zero, one);
		source += bh * raster;
		END_BLOCKS
	}
#else
	wmproc(copy_mono)(wmdev, base, sourcex, raster, id, x, y, w, h, zero,
	                  one);
#endif
	return 0;
}
...
private
int
win_dib_copy_color(gx_device* dev, const byte* base, int sourcex, int raster,
                   gx_bitmap_id id, int x, int y, int w, int h)
{
#if USE_SEGMENTS
	if(single_block(y, h)) {
		wmproc(copy_color)(wmdev, base, sourcex, raster, id, x, y, w,
		                   h);
	} else { /* Divide the transfer into blocks. */
		const byte* source = base;
...
#else
	wmproc(copy_color)(wmdev, base, sourcex, raster, id, x, y, w, h);
#endif
	return 0;
}
...
gsdll_copy_dib(unsigned char* device)
{
	...
	if(!dev || !dev->is_open || dev->mdev.width == 0 ||
	   dev->mdev.height == 0)
		return (HGLOBAL)NULL;
	return win_dib_make_dib((gx_device_win*)dev, 0, 0, dev->width,
	                        dev->height);
}
...
#define max_transfer 2000000
	if(wdev->mdev.raster > 0) { /* just in case! */
		...
		for(; wy > ny; dy += ny, wy -= ny, sy += ny)
			SetDIBitsToDevice(
			    hdc, dx, dy, wx, ny, sx, 0, 0, ny,
			    wdev->mdev.line_ptrs[wdev->height - (sy + ny)],
			    (BITMAPINFO FAR*)&bmi, which_colors);
	}
#undef max_transfer
	SetDIBitsToDevice(hdc, dx, dy, wx, wy, sx, 0, 0, wy,
...
#define xwdev ((gx_device_win_dib*)dev)
	...
	pBits = (BYTE FAR*)(pDIB + sizeof(BITMAPINFOHEADER) +
	                    sizeof(RGBQUAD) * palcount);
	...
	if(dev->BitsPerPixel == 15) { /* 5-5-5 RGB mode */
		...
		for(i = 0; i < palcount; i++) {
			win_map_color_rgb((gx_device*)wdev, (gx_color_index)i,
			                  prgb);
	...
	for(i = orgy; i < orgy + wy; i++) {
		...
		lseg = (UINT)(
		    -OFFSETOF(pLine)); /* remaining bytes in this segment */
		if(lseg >= lwidth) {
			_fmemcpy(pLine, xwdev->mdev.line_ptrs[i] + loffset,
			         lwidth);
			...
			_fmemcpy(pLine, xwdev->mdev.line_ptrs[i] + loffset,
			         lseg);
			_fmemcpy(pLine + lseg,
			         xwdev->mdev.line_ptrs[i] + loffset + lseg,
			         lwidth - lseg);
		}
#else
...
win_dib_alloc_bitmap(gx_device_win* dev, gx_device* param_dev)
{
	...
	if(is_win32s) {
#endif
		/* Round up the width so that the scan line size is a power of
		 * 2. */
...
win_dib_lock_device(unsigned char* device, int flag)
{
	...
	if(!is_win32s) {
		if(flag) {
			if(WaitForSingleObject(wdev->hmtx, 60000) ==
			   WAIT_TIMEOUT)
				return 2;
			return 1;
		}
                     ...
                     LPRGBQUAD prgbquad, LPBYTE* ppbyte, unsigned int row)
{
	...
	if(pbmih) {
		...
		pbmih->biXPelsPerMeter =
		    (DWORD)(dev->x_pixels_per_inch / 25.4 * 1000);
		pbmih->biYPelsPerMeter =
		    (DWORD)(dev->y_pixels_per_inch / 25.4 * 1000);
		pbmih->biClrUsed = palcount;
		pbmih->biClrImportant = palcount;
	}
	if(prgbquad) {
		...
		if(dev->BitsPerPixel == 15) { /* 5-5-5 RGB mode */
			...
			for(i = 0; i < palcount; i++) {
				win_map_color_rgb((gx_device*)wdev,
				                  (gx_color_index)i, prgb);
