...
private
HGLOBAL win_dib_make_dib(gx_device_win *dev, int orgx, int orgy, int wx, int wy);
private
...
#define xwdev ((gx_device_win_dib *)dev)
	...
	hglobal = GlobalAlloc(GHND | GMEM_SHARE, sizeof(BITMAPINFOHEADER) + sizeof(RGBQUAD) * palcount + bitmapsize);
		     ...
		     LPRGBQUAD prgbquad, LPBYTE *ppbyte, unsigned int row)
{
	...
	if((pbmih == NULL) && (prgbquad == NULL) && (ppbyte == NULL))
		return sizeof(BITMAPINFOHEADER) + 256 * sizeof(RGBQUAD) + gdev_mem_raster(&(dev->mdev));
	return 0;
}
