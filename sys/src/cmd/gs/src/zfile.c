...
private
int parse_file_name(const ref* op, gs_parsed_file_name_t* pfn, bool safemode);
private
int parse_real_file_name(const ref* op, gs_parsed_file_name_t* pfn,
                         gs_memory_t* mem, client_name_t cname);
private
int parse_file_access_string(const ref* op, char file_access[4]);

/* Forward references: other. */
private
int execfile_finish(i_ctx_t*);
private
int execfile_cleanup(i_ctx_t*);
private
int zopen_file(i_ctx_t*, const gs_parsed_file_name_t* pfn,
               const char* file_access, stream** ps, gs_memory_t* mem);
private
iodev_proc_open_file(iodev_os_open_file);
private
void file_init_stream(stream* s, FILE* file, const char* fmode, byte* buffer,
                      uint buffer_size);
                               ...
                               const char* permitgroup)
{
	...
	const char* win_sep2 = "\\";
	bool use_windows_pathsep =
	    (gs_file_name_check_separator(win_sep2, 1, win_sep2) == 1);
	uint plen = gp_file_name_parents(fname, len);

	/* Assuming a reduced file name. */

	if(dict_find_string(&(i_ctx_p->userparams), permitgroup, &permitlist) <=
	   0)
	...
	for(i = 0; i < r_size(permitlist); i++) {
		...
		if(permlen == 1 && permstr[0] == '*')
			return 0; /* success */
		                   ...
		                   * the permission element must start with same number of parent
		                   * references.
		                   */
		if(plen != 0 &&
		   plen != gp_file_name_parents((const char*)permstr, permlen))
			continue;
		...
		if(string_match((const unsigned char*)fname, len,
		                ...
		                use_windows_pathsep ? &win_filename_params
		                                    : NULL))
			return 0; /* success */
	}
	/* not found */
                       ...
                       const char* permitgroup)
{
	...
	if(gp_file_name_reduce(fname, len, fname_reduced, &rlen) !=
	   gp_combine_success)
...
zfile(i_ctx_t* i_ctx_p)
{
	...
	if(pname.iodev && pname.iodev->dtype == iodev_dtype_stdio) {
		bool statement =
		    (strcmp(pname.iodev->dname, "%statementedit%") == 0);
		...
		if(statement || lineedit) {
			...
			gx_io_device* indev =
			    gs_findiodevice((const byte*)"%stdin", 6);
			stream* ins;
			if(strcmp(file_access, "r"))
				return_error(e_invalidfileaccess);
			indev->state = i_ctx_p;
			code = (indev->procs.open_device)(indev, file_access,
			                                  &ins, imemory);
			indev->state = 0;
			if(code < 0)
				return code;
		...
		code = (*pname.iodev->procs.open_device)(
		    pname.iodev, file_access, &s, imemory);
...
private
int file_continue(i_ctx_t*);
private
int file_cleanup(i_ctx_t*);
private
int
zfilenameforall(i_ctx_t* i_ctx_p)
{
	...
	/* Push a mark, the iodev, devicenamelen, the scratch string, the
	 * enumerator, */
	...
	if(pname.len == 0 ||
	   iodev->procs.enumerate_files == iodev_no_enumerate_files) {
		pop(3);
...
file_continue(i_ctx_t* i_ctx_p)
{
	...
	code = iodev->procs.enumerate_next(
	    pfen, (char*)pscratch->value.bytes + devlen, len - devlen);
...
zrenamefile(i_ctx_t* i_ctx_p)
{
	...
	int code =
	    parse_real_file_name(op - 1, &pname1, imemory, "renamefile(from)");
	...
	if(code >= 0) {
		...
		if(pname1.iodev != pname2.iodev ||
		   (pname1.iodev == iodev_default &&
		     ...
		     * unless it is a temporary file. Also, we require
		     * FileControl
			...
			code = (*pname1.iodev->procs.rename_file)(
			    pname1.iodev, pname1.fname, pname2.fname);
		}
	}
...
zstatus(i_ctx_t* i_ctx_p)
{
	...
	case t_string: {
		...
		struct stat fstat;
		int code =
		    parse_file_name(op, &pname, i_ctx_p->LockFilePermissions);
		...
		case 0:
			...
			if((double)op[-4].value.intval !=
			   ...
			   (double)op[-3].value.intval != (double)fstat.st_size)
...
zlibfile(i_ctx_t* i_ctx_p)
{
	...
	if(pname.iodev == NULL)
		pname.iodev = iodev_default;
	if(pname.iodev !=
	   iodev_default) { /* Non-OS devices don't have search paths (yet). */
		code = zopen_file(i_ctx_p, &pname, "r", &s, imemory);
		if(code >= 0) {
			code =
			    ssetfilename(s, op->value.const_bytes, r_size(op));
			if(code < 0) {
				sclose(s);
				return_error(e_VMerror);
			}
		...
		code = lib_file_open(i_ctx_p->lib_path, i_ctx_p, pname.fname,
		                     pname.len, cname, sizeof(cname), &clen,
		                     &fref, imemory);
		if(code >= 0) {
			s = fptr(&fref);
           ...
           const char* file_access, stream** ps, gs_memory_t* mem)
{
	...
	if(pfn->fname == NULL) /* just a device */
		...
		if(open_file == iodev_os_open_file) {
			int code = check_file_permissions(
			    i_ctx_p, pfn->fname, pfn->len,
			    file_access[0] == 'r' ? "PermitFileReading"
			                          : "PermitFileWriting");

			if(code < 0)
				return code;
		}
		return open_file(iodev, pfn->fname, pfn->len, file_access, ps,
		                 mem);
	}
}
                   ...
                   const char* file_access, stream** ps, gs_memory_t* mem)
{
	return file_open_stream(fname, len, file_access,
	                        file_default_buffer_size, ps, iodev,
...
{
	uint attrs = (access[1] == '+' ? a_write + a_read + a_execute : 0) |
...
gp_file_name_combine_patch(const char* prefix, uint plen, const char* fname,
                           uint flen, bool no_sibling, char* buffer, uint* blen)
{
	return gp_file_name_combine(prefix, plen, fname, flen, no_sibling,
	                            buffer, blen);
}
                    ...
                    gx_io_device* iodev, gs_memory_t* mem)
{
	...
	/* Allocate the buffer. */
	buffer =
	    gs_alloc_bytes(mem, buffer_size, "file_prepare_stream(buffer)");
	if(buffer == 0)
		return_error(e_VMerror);
	if(fname != 0) {
...
{ /* i_ctx_p is NULL running init files. */
	...
	if(i_ctx_p == NULL)
		return 0;
	if(check_file_permissions_reduced(i_ctx_p, fname, flen,
	                                  "PermitFileReading") < 0)
		return_error(e_invalidfileaccess);
	return 0;
}
...
private
int
lib_fopen_with_libpath(gs_file_path_ptr lib_path, const gs_memory_t* mem,
                       i_ctx_t* i_ctx_p, gx_io_device* iodev, const char* fname,
                       uint flen, char fmode[4], char* buffer, int blen,
                       FILE** file)
{ /* i_ctx_p is NULL running init files.
   ...
   */
	...
	if(search_with_no_combine) {
		...
		if(gp_file_name_reduce(fname, flen, buffer, &blen1) !=
		   gp_combine_success)
			goto skip;
		if(iodev->procs.fopen(iodev, buffer, fmode, file, buffer,
		                      blen) == 0) {
			if(starting_arg_file ||
			   check_file_permissions_aux(i_ctx_p, buffer, blen1) >=
			       0)
				return 0;
	...
	if(search_with_combine) {
		...
		for(pi = 0; pi < r_size(&pfpath->list); ++pi) {
			...
			const char* pstr =
			    (const char*)prdir->value.const_bytes;
			uint plen = r_size(prdir), blen1 = blen;

			gp_file_name_combine_result r =
			    gp_file_name_combine_patch(pstr, plen, fname, flen,
			                               false, buffer, &blen1);
			...
			if(iodev->procs.fopen(iodev, buffer, fmode, file,
			                      buffer, blen) == 0) {
				if(starting_arg_file ||
				   check_file_permissions_aux(i_ctx_p, buffer,
				                              blen1) >= 0)
					return 0;
				iodev->procs.fclose(iodev, *file);
				*file = NULL;
          ...
          const char* fname)
{
	...
	strcat(fmode, gp_fmode_binary_suffix);
	lib_fopen_with_libpath(pfpath, mem, NULL, &iodev_default_copy, fname,
	                       strlen(fname), fmode, buffer, sizeof(buffer),
	                       &file);
	return file;
}
...
int
lib_file_open(const gs_file_path_ptr pfpath, i_ctx_t* i_ctx_p,
              const char* fname, uint len, byte* cname, uint max_clen,
...
{ /* i_ctx_p is NULL running init files. */
	...
	if(fname == 0)
		return 0;
	buffer = (char*)s->cbuf;
	code = lib_fopen_with_libpath(pfpath, mem, i_ctx_p, iodev, fname, len,
	                              fmode, buffer, s->bsize, &file);
	if(code < 0) {
		s->cbuf = NULL;
		s->bsize = s->cbsize = 0;
                 ...
                 iodev_proc_fopen_t fopen_proc, gs_memory_t* mem)
{
	...
	if(!iodev)
		iodev = iodev_default;
	code =
	    file_prepare_stream(fname, len, file_access, buffer_size, ps, fmode,
            ...
            const stream_state* st, gs_memory_t* mem)
{
	...
	if(template->stype != &st_stream_state) {
		sst = s_alloc_state(mem, template->stype,
