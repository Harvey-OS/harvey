...
#define calign_bytes(ct)                                                       \
	(sizeof(ct) == 1 ? 1 : sizeof(ct) == sizeof(short)                     \
...
#define arch_cant_shift_full_chunk                                             \
	(arch_is_big_endian && !arch_ints_are_short &&                         \
	 !arch_can_shift_full_long)

/* Pointer arithmetic macros. */
#define inc_ptr(ptr, delta) (ptr = (void*)((byte*)ptr + (delta)))
 ...
 */
#define set_mono_left_mask(var, bit) (var = chunk_all_bits, var >>= (bit))
#else
 ...
 */
#define set_mono_left_mask(var, bit) (var = mono_masks[bit])
#define set_mono_thin_mask(var, w, bit)                                        \
	(var = ~mono_masks[(w) + (bit)] & mono_masks[bit])
#define set_mono_right_mask(var, ebit) (var = ~mono_masks[ebit])
#endif
