...
/* Forward references */
private
int cmd_put_set_data_x(gx_device_clist_writer *cldev,
		...
		gs_logical_operation_t lop, const gx_clip_path *pcpath)
{
	...
	int data_x_bit;
	byte copy_op =
	    (depth > 1 ? cmd_op_copy_color_alpha : cmd_op_copy_mono + cmd_copy_ht_color);
	bool slow_rop =
	...
	FOR_RECTS
	{
		...
		}
		{
			...
			TRY_RECT
			{
				code = (orig_data_x ? cmd_put_set_data_x(cdev, pcls, orig_data_x) : 0);
				if(code >= 0) {
					byte *dp;
...
private
const gx_image_enum_procs_t clist_image_enum_procs =
    {
     clist_image_plane_data, clist_image_end_image};

/* Forward declarations */
private
bool image_band_box(gx_device *dev, const clist_image_enum *pie,
		    int y, int h, gs_int_rect *pbox);
private
int begin_image_command(byte *buf, uint buf_size,
			const gs_image_common_t *pic);
private
int cmd_image_plane_data(gx_device_clist_writer *cldev,
			 gx_clist_state *pcls,
...
private
uint clist_image_unknowns(gx_device *dev,
			  const clist_image_enum *pie);
private
int write_image_end_all(gx_device *dev,
			const clist_image_enum *pie);
			...
			gs_memory_t *mem, gx_image_enum_common_t **pinfo)
{
	...
	*pinfo = (gx_image_enum_common_t *)pie;
	...
	if(code < 0 ||
	   ...
	   !(cdev->disable_mask & clist_disable_nonrect_hl_image ? (is_xxyy(&mat) || is_xyyx(&mat)) : image_matrix_ok_to_band(&mat)))
		goto use_default;
	{
		       ...
		       int *rows_used)
{
	 ...
	 */
		...
		if(!(pcls->known & begin_image_known)) {
			...
			TRY_RECT
			{
				code = (pie->color_map_is_known ? 0 : cmd_put_color_mapping(cdev, pie->pis));
				...
				if(code >= 0) {
					uint want_known = ctm_known | clip_path_known |
							  ...
							  (pie->color_space.id == gs_no_id ? 0 : color_space_known);
