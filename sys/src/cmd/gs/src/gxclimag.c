...
/* Forward references */
private
int cmd_put_set_data_x(gx_device_clist_writer* cldev, gx_clist_state* pcls,
                       int data_x);
private
bool check_rect_for_trivial_clip(
...
int
clist_fill_mask(gx_device* dev, const byte* data, int data_x, int raster,
                ...
                gs_logical_operation_t lop, const gx_clip_path* pcpath)
{
	...
	int y0;
	int data_x_bit;
	byte copy_op = (depth > 1 ? cmd_op_copy_color_alpha
	...
	if(((cdev->disable_mask & clist_disable_complex_clip) &&
	    !check_rect_for_trivial_clip(pcpath, x, y, x + width,
	                                 y + height)) ||
	   gs_debug_c('`') || id == gx_no_bitmap_id || lop != lop_default ||
	   (depth > 1 && !color_writes_pure(pdcolor, lop)))
	copy:
	return gx_default_fill_mask(dev, data, data_x, raster, id, x, y, width,
	                            height, pdcolor, depth, lop, pcpath);
	...
	FOR_RECTS
	{
		...
		TRY_RECT { code = cmd_update_lop(cdev, pcls, lop); }
		HANDLE_RECT(code);
		if(depth > 1 && !pcls->color_is_alpha) {
			...
			{
				code = set_cmd_put_op(
				    dp, cdev, pcls, cmd_opv_set_copy_alpha, 1);
			}
			HANDLE_RECT(code);
		...
		TRY_RECT
		{
			...
			if(code >= 0)
				code = cmd_do_enable_clip(cdev, pcls,
				                          pcpath != NULL);
		}
		HANDLE_RECT(code);
		TRY_RECT
		...
		if(!cls_has_tile_id(cdev, pcls, id, offset_temp)) {
			...
			{
				code =
				    clist_change_bits(cdev, pcls, &tile, depth);
			}
			HANDLE_RECT_UNLESS(code, (code != gs_error_VMerror ||
			                          !cdev->error_is_retryable));
			if(code < 0) {
		...
		}
		{
			...
			{
				code = (orig_data_x
				            ? cmd_put_set_data_x(cdev, pcls,
				                                 orig_data_x)
				            : 0);
				if(code >= 0) {
					byte* dp;

					code = set_cmd_put_op(dp, cdev, pcls,
					                      op, rsize);
					/*
					 * The following conditional is
					 * unnecessary: the two
					 * statements inside it should go
					 * outside the
					 * HANDLE_RECT.  They are here solely to
					 * pacify
					 * stupid compilers that don't
					 * understand that dp
					 * will always be set if control gets
					 * past the
					 * HANDLE_RECT.
					 */
					if(code >= 0) {
...
private
image_enum_proc_plane_data(clist_image_plane_data);
private
image_enum_proc_end_image(clist_image_end_image);
private
const gx_image_enum_procs_t clist_image_enum_procs = {clist_image_plane_data,
                                                      clist_image_end_image};

/* Forward declarations */
private
bool image_band_box(gx_device* dev, const clist_image_enum* pie, int y, int h,
                    gs_int_rect* pbox);
private
int begin_image_command(byte* buf, uint buf_size, const gs_image_common_t* pic);
private
int cmd_image_plane_data(gx_device_clist_writer* cldev, gx_clist_state* pcls,
                         const gx_image_plane_t* planes,
                         const gx_image_enum_common_t* pie,
                         uint bytes_per_plane, const uint* offsets, int dx,
                         int h);
private
uint clist_image_unknowns(gx_device* dev, const clist_image_enum* pie);
private
int write_image_end_all(gx_device* dev, const clist_image_enum* pie);
                        ...
                        gx_image_enum_common_t** pinfo)
{
	...
	*pinfo = (gx_image_enum_common_t*)pie;
	...
	code = gx_image_enum_common_init(
	    (gx_image_enum_common_t*)pie, (const gs_data_image_t*)pim,
	    &clist_image_enum_procs, dev, num_components, format);
	{
		int i;

		for(i = 1; i < pie->num_planes; ++i)
			varying_depths |=
			    pie->plane_depths[i] != pie->plane_depths[0];
	}
	if(code < 0 || !USE_HL_IMAGES || /* Always use the default. */
		...
		goto use_default;
	{
		...
		if(masked) {
			...
			pie->color_space.byte1 =
			    (base_index << 4) |
			    (indexed ? (pim->ColorSpace->params.indexed.use_proc
			                    ? 12
			                    : 8)
			             : 0);
			pie->color_space.id =
		...
		bytes_per_plane = (pim->Width * pie->bits_per_plane + 7) >> 3;
	...
	if(cdev->disable_mask & clist_disable_complex_clip)
		if(!check_rect_for_trivial_clip(
		       pcpath, (int)floor(dbox.p.x), (int)floor(dbox.p.y),
	...
	if(!masked) {
		 ...
		 * might generate.  For single-component images with up to 4
		 * bits
		 * per pixel, standard Decode values, and no Interpolate, we
		 * generate all the possible colors now; otherwise, we assume
		 * that
		 * any color might be generated.  It is possible to do better
		 * than
		...
		if(bits_per_pixel > 4 || pim->Interpolate || num_components > 1)
			...
			if(dmin != 0 || dmax != (indexed ? max_value : 1)) {
				colors_used = all;
			} else {
				...
				for(i = 0; i <= max_value; ++i) {
					...
					colors_used |= cmd_drawing_colors_used(
					    cdev, &dcolor);
				}
			}
		}
	...
	{
		int y0 =
		    (int)floor(dbox.p.y - 0.51); /* adjust + rounding slop */
...
clist_image_plane_data(gx_image_enum_common_t* info,
                       const gx_image_plane_t* planes, int yh, int* rows_used)
{
	...
	{
		/*
		 * Just transmit the subset of the data that intersects this
		 * band.
		...
		if(!(pcls->known & begin_image_known)) {
			...
			TRY_RECT
			{
				code = (pie->color_map_is_known
				            ? 0
				            : cmd_put_color_mapping(cdev,
				                                    pie->pis));
				pie->color_map_is_known = true;
				if(code >= 0) {
					uint want_known =
					    ctm_known | clip_path_known |
					    op_bm_tk_known |
					    opacity_alpha_known |
				...
				if(code >= 0)
					code = cmd_do_enable_clip(
					    cdev, pcls, pie->pcpath != NULL);
				if(code >= 0)
			...
			if(pie->uses_color) {
				TRY_RECT
				{
					code = cmd_put_drawing_color(
					    cdev, pcls, &pie->dcolor);
				}
				HANDLE_RECT(code);
			}
			if(entire_box.p.x != 0 || entire_box.p.y != 0 ||
			   ...
			   entire_box.q.y != pie->image.Height) {
				...
				cmd_put2w(pie->image.Width - entire_box.q.x,
				          pie->image.Height - entire_box.q.y,
				          bp);
			}
			len = bp - pie->begin_image_command;
			TRY_RECT
			{
				code = set_cmd_put_op(dp, cdev, pcls, image_op,
				                      1 + len);
			}
			HANDLE_RECT(code);
		 ...
		 * actually needed for these particular scan lines if the image
		 * is
		 * rotated.
		 */
		{
			...
			if(by1 > y1)
				by1 = y1;
			/*
			 * Make sure we're skipping an integral number of
			 * pixels, by
			...
			xskip =
			    xoff &
			    -(int)"\001\010\004\010\002\010\004\010"[bpp & 7];
			for(i = 0; i < num_planes; ++i)
				offsets[i] = (by0 - y0) * planes[i].raster +
				             ((xskip * bpp) >> 3);
			bytes_per_plane =
			    ((bx1 - (pie->rect.p.x + xskip)) * bpp + 7) >> 3;
			bytes_per_row = bytes_per_plane * pie->num_planes;
			rows_per_cmd = (cbuf_size - cmd_largest_size) /
			               max(bytes_per_row, 1);

			if(rows_per_cmd == 0) {
				/* The reader will have to buffer a row
				 * separately. */
				rows_per_cmd = 1;
			}
			for(iy = by0, ih = by1 - by0; ih > 0;
				...
				TRY_RECT
				{
					code = cmd_image_plane_data(
					    cdev, pcls, planes, info,
	...
	END_RECTS_ON_ERROR(
	    BEGIN++ cdev->ignore_lo_mem_warnings;
	    NEST_RECT { code = write_image_end_all(dev, pie); } UNNEST_RECT;
	    --cdev->ignore_lo_mem_warnings;
	    /* Update sub-rect */
	    if(!pie->image.Interpolate) pie->rect.p.y +=
	    yh_used; /* interpolate & mem recovery currently incompat */
	    END, (code < 0 ? (band_code = code) : code) >= 0,
	    (cmd_clear_known(cdev, clist_image_unknowns(dev, pie) |
	                               begin_image_known),
	     pie->color_map_is_known = false, cdev->image_enum_id = pie->id,
	     true));
done:
...
clist_image_end_image(gx_image_enum_common_t* info, bool draw_last)
{
	...
	NEST_RECT
	{
		...
		if(code < 0 && cdev->error_is_retryable) {
			...
			++cdev->ignore_lo_mem_warnings;
			retry_code =
			    write_image_end_all(dev, pie); /* force it out */
			--cdev->ignore_lo_mem_warnings;
...
int
clist_create_compositor(gx_device* dev, gx_device** pcdev,
                        ...
                        gs_memory_t* mem)
{
	...
	code = pcte->type->procs.clist_compositor_write_update(pcte, dev, pcdev,
	                                                       pis, mem);
	if(code < 0)
		return code;
...
cmd_put_halftone(gx_device_clist_writer* cldev, const gx_device_halftone* pdht)
{
	 ...
	 *    command code (2 bytes)
	 *    segment size (enc_u_sizew(seg_size) (seg_size <
	 *cbuf_ht_seg_max_size)
	...
	if(ht_size > cbuf_ht_seg_max_size) {
		pht_buff = gs_alloc_bytes(cldev->bandlist_memory, ht_size,
		                          "cmd_put_halftone");
		if(pht_buff == 0)
			return_error(gs_error_VMerror);
	} else {
	...
	if(code < 0) {
		if(ht_size > cbuf_ht_seg_max_size)
			gs_free_object(cldev->bandlist_memory, pht_buff,
			               "cmd_put_halftone");
		else
			cldev->cnext = dp0;
		return code;
	...
	if(ht_size > cbuf_ht_seg_max_size) {
		...
		while(ht_size > 0 && code >= 0) {
			...
			seg_size = (ht_size > cbuf_ht_seg_max_size
			                ? cbuf_ht_seg_max_size
			                : ht_size);
...
int
cmd_put_color_mapping(gx_device_clist_writer* cldev, const gs_imager_state* pis)
{
	int code;
	...
	code = cmd_put_color_map(cldev, cmd_map_black_generation, 0,
	                         pis->black_generation,
	                         &cldev->black_generation_id);
	if(code < 0)
		return code;
	code = cmd_put_color_map(cldev, cmd_map_undercolor_removal, 0,
...
#undef get_id
		 ...
		 * component
		 */
		if(send_default_comp ||
		   cldev->transfer_ids[0] != default_comp_id) {
			gs_id dummy = gs_no_id;

			code =
			    cmd_put_color_map(cldev, cmd_map_transfer, 0,
		...
		if(cldev->transfer_ids[0] != xfer_ids[0]) {
			code = cmd_put_color_map(
			    cldev, cmd_map_transfer_0,
               ...
               gs_int_rect* pbox)
{
	...
	int px = pie->rect.p.x, py = pie->rect.p.y, qx = pie->rect.q.x,
	    qy = pie->rect.q.y;
	...
	if(gs_debug_c('b')) {
		dlprintf6("[b]band box for (%d,%d),(%d,%d), band (%d,%d) =>\n",
		          px, py, qx, qy, y, y + h);
		dlprintf10(
		    "      (%g,%g),(%g,%g), matrix=[%g %g %g %g %g %g]\n",
		    bbox.p.x, bbox.p.y, bbox.q.x, bbox.q.y, pie->matrix.xx,
		    pie->matrix.xy, pie->matrix.yx, pie->matrix.yy,
		    pie->matrix.tx, pie->matrix.ty);
	}
#endif
	if(is_xxyy(&pie->matrix) || is_xyyx(&pie->matrix)) {
		...
		if(gs_point_transform_inverse(bbox.p.x, bbox.p.y, &pie->matrix,
		   ...
		   gs_point_transform_inverse(bbox.p.x, bbox.q.y, &pie->matrix,
		                              &corners[3]) < 0) {
			if_debug0(
			    'b', "[b]can't inverse-transform a band corner!\n");
			return false;
		}
		corners[4] = corners[0];
		...
		for(i = 0; i < 4; ++i) {
			...
			if(pa.x >= px && pa.x <= qx && pa.y >= py && pa.y <= qy)
				box_merge_point(pbox, pa.x, pa.y);
			/* Check for intersections of band edges with image
			 * edges. */
			dx = corners[i + 1].x - pa.x;
			dy = corners[i + 1].y - pa.y;
#define in_range(t, tc, p, q) (0 <= t && t <= 1 && (t = tc) >= p && t <= q)
			if(dx != 0) {
				double t = (px - pa.x) / dx;
...
private
uint /* mask of unknown properties(see pcls->known) */
    clist_image_unknowns(gx_device* dev, const clist_image_enum* pie)
{
	...
	if(pie->color_space.id == gs_no_id) { /* masked image */
		...
		if(cdev->color_space.id == pie->color_space.id) {
			/* The color space pointer might not be valid: update
			 * it. */
...
cmd_image_plane_data(gx_device_clist_writer* cldev, gx_clist_state* pcls,
                     const gx_image_plane_t* planes,
                     const gx_image_enum_common_t* pie, uint bytes_per_plane,
...
private
int /* ret 0 ok, else -ve error status */
    ...
    )
{
	...
	if(gx_cpath_includes_rectangle(pcpath, imgbox.p.x, imgbox.p.y,
