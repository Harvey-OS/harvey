...
/* order matters!             0       1        2        3       4      5     6
 * 7           8 */
const char* media[10] = {"a4", "letter", "legal",   "com10",     "c5",
                         "dl", "b5",     "monarch", "executive", 0};
const int height[2][10] = {
    {3447, 3240, 4140, 5587, 2644, 5083, 2975, 4387, 3090, 0},
    {6894, 6480, 8280, 11167, 5288, 10159, 5950, 8767, 6180, 0}};
const int width[2] = {2528, 5056};
...
private
dev_proc_print_page_copies(lj3100sw_print_page_copies);
private
dev_proc_close_device(lj3100sw_close);
...
gx_device_printer far_data gs_lj3100sw_device = prn_device_margins /*_copies*/ (
    prn_lj3100sw_procs, "lj3100sw", DEFAULT_WIDTH_10THS, DEFAULT_HEIGHT_10THS,
    X_DPI, Y_DPI, XCORRECTION, YCORRECTION, 0.25, 0.2, 0.25, 0.2, 1,
    0 /* lj3100sw_print_page_copies */);
...
private
void
lj3100sw_output_repeated_data_bytes(FILE* prn_stream, char* buffer, char** pptr,
                                    int val, int num)
{
	int size;
	while(num) {
...
private
void
lj3100sw_output_empty_line(FILE* prn_stream, char* buffer, char** pptr,
...
private
int
lj3100sw_print_page_copies(gx_device_printer* pdev, FILE* prn_stream,
                           int num_copies /* ignored */)
{
	int i, j;
	...
	if(gdev_prn_file_is_new(pdev)) {
		...
		lj3100sw_output_repeated_data_bytes(prn_stream, buffer, &ptr,
		                                    0x1b, 12);
		ptr += sprintf(ptr, "\r\nBD");
		lj3100sw_output_repeated_data_bytes(prn_stream, buffer, &ptr, 0,
		                                    5520);
		ptr += sprintf(ptr, "%s\r\n%s %d\r\n%s %d\r\n%s %d\r\n%s "
		                    "%d\r\n%s %d\r\n%s %d\r\n",
		               "NJ", "PQ", -1, "RE", high_resolution ? 6 : 2,
		               "SL", printer_width, "LM", 0, "PS", medium_index,
		               "PC", 0);
	...
	for(i = 0; i < printer_height; i++) {
		if(i < paper_height) {
			...
			for(j = 0; j <= printer_width; j++) {
				...
				if(newcolor == color)
					count++;
				else if(count == printer_width &&
				        color ==
				            0) /* implies j == printer_width */
					lj3100sw_output_empty_line(
					    prn_stream, buffer, &ptr,
					    high_resolution);
				else /* print a sequence of pixels with a
				        uniform color */
					while(newcolor != color) {
						int size = min(count, 64);
						tmp |= code[color][size].bits
						       << bit_index;
						bit_index +=
						    code[color][size].length;
						while(bit_index >= 8) {
			...
			if(bit_index)
				lj3100sw_output_data_byte(prn_stream, buffer,
				                          &ptr, tmp & 0xff);
		} else
			lj3100sw_output_empty_line(prn_stream, buffer, &ptr,
			                           high_resolution);
		lj3100sw_output_newline(prn_stream, buffer, &ptr);
	}
