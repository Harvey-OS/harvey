...
/* Copyright (C) 1989, 1992, 1993, 1999 Aladdin Enterprises.  All rights
  reserved.
...
zaload(i_ctx_t* i_ctx_p)
{
	...
	if(r_has_type(&aref, t_array))
		...
		for(i = 0; i < asize;
		    i++, pdest++, packed = packed_next(packed))
...
zastore(i_ctx_t* i_ctx_p)
{
	...
	if(size > op - osbot) {
		...
		*ref_stack_index(&o_stack, 0) = arr;
		...
		code =
		    refcpy_to_old(op, 0, op - size, size, idmemory, "astore");
		if(code < 0)
			return code;
		op[-(int)size] = *op;
		pop(size);
...
const op_def zarray_op_defs[] = {{"1aload", zaload},
                                 {"1array", zarray},
                                 {"1astore", zastore},
                                 op_def_end(0)};
