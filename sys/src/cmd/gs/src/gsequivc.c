					      ...
					      equivalent_cmyk_color_params *pparams)
{
	...
	for(i = 0; i < pdevn_params->separations.num_separations; i++) {
		if(pparams->color[i].color_info_valid == false) {
			...
			pcs->params.separation.get_colorname_string(pdev->memory, pcs->params.separation.sep_name, &pcs_sep_name,
								    &cs_sep_name_size);
					   ...
					   equivalent_cmyk_color_params *pparams)
{
	...
	for(j = 0; j < pcs->params.device_n.num_components; j++) {
		pcs->params.device_n.get_colorname_string(pdev->memory, pcs->params.device_n.names[j],
							  &pcs_sep_name, &cs_sep_name_size);
	...
	for(i = 0; i < pdevn_params->separations.num_separations; i++) {
		if(pparams->color[i].color_info_valid == false) {
			...
			for(j = 0; j < pcs->params.device_n.num_components; j++) {
				pcs->params.device_n.get_colorname_string(pdev->memory, pcs->params.device_n.names[j], &pcs_sep_name,
									  &cs_sep_name_size);
				if(compare_color_names(dev_sep_name->data, dev_sep_name->size,
						       pcs_sep_name, cs_sep_name_size)) {
					gs_color_space temp_cs = *pcs;
...
private
bool
check_all_colors_known(int num_spot,
		       equivalent_cmyk_color_params *pparams)
{
				   ...
				   gs_devn_params *pdevn_params, equivalent_cmyk_color_params *pparams)
{
	...
	if(pcs != NULL) {
		if(pcs->type->index == gs_color_space_index_Separation) {
			...
			pparams->all_color_info_valid = check_all_colors_known(pdevn_params->separations.num_separations, pparams);
