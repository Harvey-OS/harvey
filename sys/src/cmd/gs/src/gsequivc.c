...
/* Function protypes */
private
void capture_spot_equivalent_cmyk_colors(gx_device* pdev, const gs_state* pgs,
...
private
void
update_Separation_spot_equivalent_cmyk_colors(
    gx_device* pdev, const gs_state* pgs, const gs_color_space* pcs,
    gs_devn_params* pdevn_params, equivalent_cmyk_color_params* pparams)
{
	...
	for(i = 0; i < pdevn_params->separations.num_separations; i++) {
		if(pparams->color[i].color_info_valid == false) {
			...
			if(compare_color_names(dev_sep_name->data,
			                       dev_sep_name->size, pcs_sep_name,
			                       cs_sep_name_size)) {
				gs_color_space temp_cs = *pcs;
				gs_client_color client_color;

				/*
				 * Create a copy of the color space and then
				 * modify it
				 * to force the use of the alternate color
				 * space.
				 */
				temp_cs.params.separation.use_alt_cspace = true;
				client_color.paint.values[0] = 1.0;
				capture_spot_equivalent_cmyk_colors(
				    pdev, pgs, &client_color, &temp_cs, i,
				    pparams);
				break;
			}
		}
...
private
void
update_DeviceN_spot_equivalent_cmyk_colors(
    gx_device* pdev, const gs_state* pgs, const gs_color_space* pcs,
    gs_devn_params* pdevn_params, equivalent_cmyk_color_params* pparams)
{
	int i;
	...
	/*
	 * Check if the color space contains components named 'None'.  If so
	 * then
	 ...
	 * specified as part of the 'color' and not part of the color space.
	 * Thus
	...
	for(j = 0; j < pcs->params.device_n.num_components; j++) {
		pcs->params.device_n.get_colorname_string(
		    ...
		    &cs_sep_name_size);
		if(compare_color_names("None", 4, pcs_sep_name,
		                       cs_sep_name_size))
			return;
	}
	...
	for(i = 0; i < pdevn_params->separations.num_separations; i++) {
		if(pparams->color[i].color_info_valid == false) {
			...
			for(j = 0; j < pcs->params.device_n.num_components;
			    j++) {
				pcs->params.device_n.get_colorname_string(
				    pdev->memory, pcs->params.device_n.names[j],
				    &pcs_sep_name, &cs_sep_name_size);
				if(compare_color_names(
				       dev_sep_name->data, dev_sep_name->size,
				       pcs_sep_name, cs_sep_name_size)) {
					gs_color_space temp_cs = *pcs;
					gs_client_color client_color;

					/*
					 * Create a copy of the color space and
					 * then modify it
					 * to force the use of the alternate
					 * color space.
					 */
					memset(&client_color, 0,
					       sizeof(client_color));
					temp_cs.params.device_n.use_alt_cspace =
					    true;
					client_color.paint.values[j] = 1.0;
					capture_spot_equivalent_cmyk_colors(
					    pdev, pgs, &client_color, &temp_cs,
					    i, pparams);
					break;
				}
			}
...
private
bool
check_all_colors_known(int num_spot, equivalent_cmyk_color_params* pparams)
{
                                   ...
                                   equivalent_cmyk_color_params* pparams)
{
	 ...
	 * space.  If so then when check if the color space contains a
	 * separation
	...
	if(pcs != NULL) {
		if(pcs->type->index == gs_color_space_index_Separation) {
			update_Separation_spot_equivalent_cmyk_colors(
			    pdev, pgs, pcs, pdevn_params, pparams);
 ...
 */
private
cmap_proc_gray(cmap_gray_capture_cmyk_color);
private
cmap_proc_rgb(cmap_rgb_capture_cmyk_color);
private
cmap_proc_cmyk(cmap_cmyk_capture_cmyk_color);
private
cmap_proc_rgb_alpha(cmap_rgb_alpha_capture_cmyk_color);
private
cmap_proc_separation(cmap_separation_capture_cmyk_color);
private
cmap_proc_devicen(cmap_devicen_capture_cmyk_color);

private
const gx_color_map_procs cmap_capture_cmyk_color = {
    cmap_gray_capture_cmyk_color,       cmap_rgb_capture_cmyk_color,
    cmap_cmyk_capture_cmyk_color,       cmap_rgb_alpha_capture_cmyk_color,
    cmap_separation_capture_cmyk_color, cmap_devicen_capture_cmyk_color};
...
private
void
cmap_cmyk_capture_cmyk_color(frac c, frac m, frac y, frac k,
...
cmap_separation_capture_cmyk_color(frac all, gx_device_color* pdc,
                                   ...
                                   gs_color_select_t select)
{
	dprintf("cmap_separation_capture_cmyk_color - this routine should not "
	        "be executed\n");
}
...
cmap_devicen_capture_cmyk_color(const frac* pcc, gx_device_color* pdc,
                                ...
                                gs_color_select_t select)
{
	dprintf("cmap_devicen_capture_cmyk_color - this routine should not be "
	        "executed\n");
}
                                    ...
                                    equivalent_cmyk_color_params* pparams)
{
	 ...
	 * color parameters.  Since we only using this device for a very
	 * specific
