...
struct dict_stack_s {
	 ...
	 * Level 1 mode.  min_dstack_size, the minimum number of entries, does
	 * not
	 ...
	 * r_space(pvalue) <= dsspace.  Note that in order for this trick to
	 * work,
	...
	/*
	 * Cache values for fast name lookup.  If the top entry on the
	 * dictionary
	 * stack is a readable dictionary with packed keys, dtop_keys,
	 * dtop_npairs,
	...
	/*
	 * Cache a copy of the bottom entry on the stack, which is never
	 * deleted.
	 */
	ref system_dict;
};
...
#define public_st_dict_stack() /* in interp.c */                               \
	gs_public_st_suffix_add0(st_dict_stack, dict_stack_t, "dict_stack_t",  \
	                         dict_stack_enum_ptrs, dict_stack_reloc_ptrs,  \
	                         st_ref_stack)
