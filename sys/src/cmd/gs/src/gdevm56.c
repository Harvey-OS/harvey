...
/* Copyright (C) 1994, 1995, 1996, 1997, 1998, 1999 Aladdin Enterprises, 2001
  Artifex Software.  All rights reserved.
...
struct stats_mem56_s {
	int32_t fill, fwide, fgray[101], fsetc, fcolor[101], fnarrow[5],
	    fprevc[257];
	double ftotal;
} stats_mem56;
...
/* Procedures */
declare_mem_procs(mem_true56_copy_mono, mem_true56_copy_color,
                  mem_true56_fill_rectangle);

/* The device descriptor. */
const gx_device_memory mem_true56_device = mem_full_alpha_device(
    "image56", 56, 0, mem_open, gx_default_rgb_map_rgb_color,
    gx_default_rgb_map_color_rgb, mem_true56_copy_mono, mem_true56_copy_color,
    mem_true56_fill_rectangle, gx_default_map_cmyk_color, gx_default_copy_alpha,
...
#define putw(ptr, wxyz) *(bits32*)(ptr) = (wxyz)
...
#else
#define set_color56_cache(color, a, b, c, d, e, f, g)                          \
	mdev->color56.abcd = abcd = ((bits32)(d) << 24) |                      \
	                            ((bits32)(c) << 16) | ((bits16)(b) << 8) | \
	                            (a),                                       \
...
private
int
mem_true56_fill_rectangle(gx_device* dev, int x, int y, int w, int h,
...
private
int
mem_true56_copy_mono(gx_device* dev, const byte* base, int sourcex, int sraster,
                     ...
                     gx_color_index zero, gx_color_index one)
{
	...
	first_bit = 0x80 >> sbit;
	if(zero !=
	   gx_no_color_index) { /* Loop for halftones or inverted masks */
		/* (never used). */
	...
	} else if(one != gx_no_color_index) { /* Loop for character and pattern
	                                         masks. */
				...
				if(sbyte & 0xf0) {
					if(sbyte & 0x80)
						put7(pptr, a1, b1, c1, d1, e1,
						     f1, g1);
					if(sbyte & 0x40)
						put7(pptr + 7, a1, b1, c1, d1,
						     e1, f1, g1);
					if(sbyte & 0x20)
						put7(pptr + 14, a1, b1, c1, d1,
						     e1, f1, g1);
					if(sbyte & 0x10)
						put7(pptr + 21, a1, b1, c1, d1,
						     e1, f1, g1);
				}
				if(sbyte & 0xf) {
					if(sbyte & 8)
						put7(pptr + 28, a1, b1, c1, d1,
						     e1, f1, g1);
					if(sbyte & 4)
						put7(pptr + 35, a1, b1, c1, d1,
						     e1, f1, g1);
					if(sbyte & 2)
						put7(pptr + 42, a1, b1, c1, d1,
						     e1, f1, g1);
					if(sbyte & 1)
						put7(pptr + 49, a1, b1, c1, d1,
						     e1, f1, g1);
				}
				pptr += 8 * PIXEL_SIZE;
				count -= 8;
...
private
int
mem_true56_copy_color(gx_device* dev, const byte* base, int sourcex,
...
/* Procedures */
declare_mem_procs(mem56_word_copy_mono, mem56_word_copy_color,
                  mem56_word_fill_rectangle);

/* Here is the device descriptor. */
const gx_device_memory mem_true56_word_device =
    mem_full_device("image56w", 56, 0, mem_open, gx_default_rgb_map_rgb_color,
                    gx_default_rgb_map_color_rgb, mem56_word_copy_mono,
                    mem56_word_copy_color, mem56_word_fill_rectangle,
...
private
int
mem56_word_copy_mono(gx_device* dev, const byte* base, int sourcex, int sraster,
                     gx_bitmap_id id, int x, int y, int w, int h,
                     gx_color_index zero, gx_color_index one)
{
	gx_device_memory* const mdev = (gx_device_memory*)dev;
	...
	mem_true56_copy_mono(dev, base, sourcex, sraster, id, x, y, w, h, zero,
	                     one);
...
private
int
mem56_word_copy_color(gx_device* dev, const byte* base, int sourcex,
                      int sraster, gx_bitmap_id id, int x, int y, int w, int h)
{
	gx_device_memory* const mdev = (gx_device_memory*)dev;
	byte* row;
	...
	bytes_copy_rectangle(row + x * PIXEL_SIZE, raster,
	                     base + sourcex * PIXEL_SIZE, sraster,
	                     w * PIXEL_SIZE, h);
	mem_swap_byte_rect(row, raster, x * 56, w * 56, h, false);
	return 0;
}
