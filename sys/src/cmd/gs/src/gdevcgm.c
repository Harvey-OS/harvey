...
/* GC descriptor */
gs_private_st_suffix_add1_final(st_device_cgm, gx_device_cgm, "gx_device_cgm",
                                device_cgm_enum_ptrs, device_cgm_reloc_ptrs,
                                gx_device_finalize, st_device, st);

/* Device procedures */
private
dev_proc_open_device(cgm_open);
private
dev_proc_output_page(cgm_output_page);
private
dev_proc_close_device(cgm_close);
private
dev_proc_fill_rectangle(cgm_fill_rectangle);
...
#endif
private
dev_proc_copy_mono(cgm_copy_mono);
private
dev_proc_copy_color(cgm_copy_color);
private
dev_proc_get_params(cgm_get_params);
private
dev_proc_put_params(cgm_put_params);
...
#define cgm_device(dname, depth, max_value, dither, map_rgb_color,             \
                   map_color_rgb)                                              \
	{                                                                      \
		std_device_color_stype_body(gx_device_cgm, 0, dname,           \
		                            &st_device_cgm, 850, 1100, 100,    \
		                            100, depth, max_value, dither),    \
		    {cgm_open, NULL, /* get_initial_matrix */                  \
		     NULL,           /* sync_output */                         \
		     cgm_output_page, cgm_close, map_rgb_color, map_color_rgb, \
		     cgm_fill_rectangle, cgm_tile_rectangle, cgm_copy_mono,    \
		     cgm_copy_color, NULL, /* draw_line */                     \
		     NULL,                 /* get_bits */                      \
		     cgm_get_params, cgm_put_params},                          \
		    {0},        /* fname */                                    \
		    0,          /* file */                                     \
		    0,          /* st */                                       \
		    0 /*false*/ /* in_picture */                               \
...
gx_device_cgm gs_cgmmono_device = cgm_device(
    "cgmmono", 1, 1, 2, gx_default_map_rgb_color, gx_default_w_b_map_color_rgb);

gx_device_cgm gs_cgm8_device =
    cgm_device("cgm8", 8, 5, 6, pc_8bit_map_rgb_color, pc_8bit_map_color_rgb);

gx_device_cgm gs_cgm24_device =
    cgm_device("cgm24", 24, 255, 255, gx_default_rgb_map_rgb_color,
...
#define check_result(result)                                                   \
	if(result != cgm_result_ok)                                            \
	return_error(cgm_error_code(result))
...
cgm_open(gx_device* dev)
{
	...
	result = cgm_set_metafile_elements(
	    cdev->st, &meta,
	    cgm_set_METAFILE_VERSION | cgm_set_VDC_TYPE |
	        cgm_set_INTEGER_PRECISION | cgm_set_INDEX_PRECISION |
	        cgm_set_COLOR_PRECISION | cgm_set_COLOR_INDEX_PRECISION |
	        cgm_set_MAXIMUM_COLOR_INDEX | cgm_set_METAFILE_ELEMENT_LIST);
	check_result(result);
...
cgm_get_params(gx_device* dev, gs_param_list* plist)
{
	...
	if(code < 0)
		return code;
	ofns.data = (const byte*)cdev->fname, ofns.size = strlen(cdev->fname),
...
cgm_put_params(gx_device* dev, gs_param_list* plist)
{
	...
	case 0:
		if(dev->LockSafetyParams &&
		   bytes_compare(ofs.data, ofs.size, (const byte*)cdev->fname,
		                 strlen(cdev->fname))) {
...
#define cgm_set_cell_points(pqr, xo, yo, w, h)                                 \
	pqr[0].integer.x = (xo), pqr[0].integer.y = (yo),                      \
	pqr[1].integer.x = (xo) + (w), pqr[1].integer.y = (yo) + (h),          \
	pqr[2].integer.x = (xo) + (w), pqr[2].integer.y = (yo)
...
cgm_begin_picture(gx_device_cgm* cdev)
{
	...
	pic.color_selection_mode =
	    (cdev->color_info.depth <= 8 ? cgm_color_selection_indexed
	...
	result = cgm_set_picture_elements(
	    cdev->st, &pic,
	    cgm_set_SCALING_MODE | cgm_set_COLOR_SELECTION_MODE |
	        cgm_set_LINE_WIDTH_SPECIFICATION_MODE |
	        cgm_set_EDGE_WIDTH_SPECIFICATION_MODE | cgm_set_VDC_EXTENT);
	check_result(result);
	result = cgm_BEGIN_PICTURE_BODY(cdev->st);
	check_result(result);
	result = cgm_VDC_INTEGER_PRECISION(
	    cdev->st, (cdev->width <= 0x7fff && cdev->height <= 0x7fff
	                   ? 16
	                   : sizeof(cdev->width) * 8));
	check_result(result);
	...
	if(cdev->color_info.depth <= 8) {
		...
		for(i = 0; i < (1 << cdev->color_info.depth); i++) {
			...
			(*dev_proc(cdev, map_color_rgb))(
			    (gx_device*)cdev, (gx_color_index)i, rgb);
			colors[i].rgb.r = rgb[0] >> (gx_color_value_bits - 8);
			colors[i].rgb.g = rgb[1] >> (gx_color_value_bits - 8);
			colors[i].rgb.b = rgb[2] >> (gx_color_value_bits - 8);
		}
...
private
int
cgm_copy_mono(gx_device* dev, const byte* base, int sourcex, int raster,
              ...
              gx_color_index one)
{
	...
	if(zero == 0 && one == 1 && cdev->color_info.depth == 1) {
		...
		result =
		    cgm_CELL_ARRAY(cdev->st, pqr, w, h, 1, cgm_cell_mode_packed,
		                   base, sourcex, raster);
		check_result(result);
	} else {
		result = cgm_INTERIOR_STYLE(cdev->st, cgm_interior_style_solid);
		...
		for(iy = 0; iy < h; iy++)
			for(ix = 0; ix < w; ix++) {
				...
				if(color != gx_no_color_index) {
					...
					cgm_color_from_color_index(&fill_color,
					                           cdev, color);
					cgm_set_rect(points, x, y, 1, 1);
					result = cgm_RECTANGLE(
					    cdev->st, &points[0], &points[1]);
					check_result(result);
				}
			}
	}
...
private
int
cgm_copy_color(gx_device* dev, const byte* base, int sourcex, int raster,
               gx_bitmap_id id, int x, int y, int w, int h)
{
	...
	if(depth == 1)
		return cgm_copy_mono(dev, base, sourcex, raster, id, x, y, w, h,
