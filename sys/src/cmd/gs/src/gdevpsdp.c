...
private
const psdf_image_filter_name Poly_filters[] = {
...
private
const psdf_image_filter_name Mono_filters[] = {
...
#define pi(key, type, memb)                                                    \
	{                                                                      \
		key, type, offset_of(psdf_image_params, memb)                  \
	}
#define psdf_image_param_names(acs, aa, af, de, di, ds, dt, dst, dstd, e, f,   \
                               fns, r)                                         \
	...
	{                                                                      \
		pi(af, gs_param_type_bool, AutoFilter),                        \
		    ...
		    pi(r, gs_param_type_int, Resolution), gs_param_item_end    \
	}

private
const psdf_image_param_names_t Color_names = {psdf_image_param_names(
    "ColorACSImageDict", "AntiAliasColorImages", "AutoFilterColorImages",
    "ColorImageDepth", "ColorImageDict", "DownsampleColorImages",
    "ColorImageDownsampleType", "ColorImageDownsampleThreshold", 1.5,
    "EncodeColorImages", "ColorImageFilter", Poly_filters,
    "ColorImageResolution")};
private
const psdf_image_param_names_t Gray_names = {psdf_image_param_names(
    "GrayACSImageDict", "AntiAliasGrayImages", "AutoFilterGrayImages",
    "GrayImageDepth", "GrayImageDict", "DownsampleGrayImages",
    "GrayImageDownsampleType", "GrayImageDownsampleThreshold", 2.0,
    "EncodeGrayImages", "GrayImageFilter", Poly_filters,
    "GrayImageResolution")};
private
const psdf_image_param_names_t Mono_names = {psdf_image_param_names(
    0, "AntiAliasMonoImages", 0, "MonoImageDepth", "MonoImageDict",
    "DownsampleMonoImages", "MonoImageDownsampleType",
    "MonoImageDownsampleThreshold", 2.0, "EncodeMonoImages", "MonoImageFilter",
    Mono_filters, "MonoImageResolution")};
#undef pi
private
const int8_t* const AutoRotatePages_names[] = {psdf_arp_names, 0};
private
const int8_t* const ColorConversionStrategy_names[] = {psdf_ccs_names, 0};
private
const int8_t* const DownsampleType_names[] = {psdf_ds_names, 0};
private
const int8_t* const Binding_names[] = {psdf_binding_names, 0};
private
const int8_t* const DefaultRenderingIntent_names[] = {psdf_ri_names, 0};
private
const int8_t* const TransferFunctionInfo_names[] = {psdf_tfi_names, 0};
private
const int8_t* const UCRandBGInfo_names[] = {psdf_ucrbg_names, 0};
private
const int8_t* const CannotEmbedFontPolicy_names[] = {psdf_cefp_names, 0};

private
const gs_param_item_t psdf_param_items[] = {
#define pi(key, type, memb)                                                    \
	{                                                                      \
		key, type, offset_of(psdf_distiller_params, memb)              \
	}
    ...
    pi("PreserveOverprintSettings", gs_param_type_bool,
                      ...
                      psdf_image_params* params)
{
	 ...
	 */
	if((code = gs_param_write_items(plist, params, NULL, items)) < 0 ||
	   ...
	   (code = psdf_get_image_dict_param(plist, pnames->Dict,
	                                     params->Dict)) < 0 ||
	   /* (Downsample) */
	   (code = psdf_write_name(
	        plist, pnames->DownsampleType,
...
gdev_psdf_get_params(gx_device* dev, gs_param_list* plist)
{
	...
	if(code < 0 ||
	   (code = gs_param_write_items(plist, &pdev->params, NULL,
	                                psdf_param_items)) < 0 ||

	   /* General parameters */

	   (code = psdf_write_name(
	        plist, "AutoRotatePages",
	   ...
	   (code = psdf_write_name(plist, "Binding",
	       ...
	       0 ||
	   (code = psdf_write_name(
	        plist, "DefaultRenderingIntent",
	        DefaultRenderingIntent_names[(int)pdev->params
	                                         .DefaultRenderingIntent])) <
	       0 ||
	   (code = psdf_write_name(
	        plist, "TransferFunctionInfo",
	        TransferFunctionInfo_names[(int)pdev->params
	                                       .TransferFunctionInfo])) < 0 ||
	   (code = psdf_write_name(
	        plist, "UCRandBGInfo",
	   ...
	   (code = psdf_get_image_params(plist, &Color_names,
	                                 &pdev->params.ColorImage)) < 0 ||
	   (code = psdf_write_name(
	        plist, "ColorConversionStrategy",
	        ColorConversionStrategy_names[(int)pdev->params
	   ...
	   (code = psdf_get_image_params(plist, &Gray_names,
	                                 &pdev->params.GrayImage)) < 0 ||

	   /* Mono sampled image parameters */

	   (code = psdf_get_image_params(plist, &Mono_names,
	                                 &pdev->params.MonoImage)) < 0 ||

	   /* Font embedding parameters */

	   (code = psdf_get_embed_param(plist, ".AlwaysEmbed",
	                                &pdev->params.AlwaysEmbed)) < 0 ||
	   (code = psdf_get_embed_param(plist, ".NeverEmbed",
	                                &pdev->params.NeverEmbed)) < 0 ||
	   (code = psdf_write_name(
	        plist, "CannotEmbedFontPolicy",
	        CannotEmbedFontPolicy_names[(int)pdev->params
	                                        .CannotEmbedFontPolicy])) < 0)
		DO_NOTHING;
	return code;
}
                       ...
                       gs_const_string* pstr, gs_memory_t* mem, int ecode)
{
	...
	case 0: {
		uint size = ps.size;
		byte* data =
		    gs_alloc_string(mem, size, "psdf_read_string_param");
...
{
	return psdf_DCT_filter(plist, st, 8 /*nominal*/, 8 /*ibid.*/,
	                       3 /*ibid.*/, NULL);
}
                     ...
                     gs_memory_t* mem, int ecode)
{
	...
	if(replace) {
		...
		rdata = gs_alloc_struct_array(
		    mem, psa->size + sa.size, gs_param_string,
		    &st_param_string_element, "psdf_put_embed_param(update)");
		if(rdata == 0)
			return_error(gs_error_VMerror);
		...
		if((code = add_embed(&rsa, &sa, mem)) < 0) {
			...
			rsa.data =
			    gs_resize_object(mem, rdata, rsa.size,
			                     "psdf_put_embed_param(resize)");
		}
	} else
		return 0; /* no-op */
	if(code >= 0) {
		gs_free_const_object(mem, psa->data,
		                     "psdf_put_embed_param(free)");
                          ...
                          ss_put_params_t put_params, gs_memory_t* mem)
{
	...
	case 0: {
		...
		if(code < 0) {
			...
			code =
			    param_list_copy((gs_param_list*)plvalue, dict.list);
			if(code < 0) {
                      ...
                      psdf_image_params* params, int ecode)
{
	...
	/* (Downsample) */
	params->DownsampleType = (enum psdf_downsample_type)psdf_put_enum(
	    plist, pnames->DownsampleType, (int)params->DownsampleType,
	    DownsampleType_names, &ecode);
	/* (DownsampleThreshold) */
	/* (Encode) */
	...
	default:
		ecode = code;
	ipe:
		param_signal_error(plist, pnames->Filter, ecode);
	case 1:
		break;
	}
	/* (Resolution) */
...
{
	gx_device_psdf* pdev = (gx_device_psdf*)dev;
	gs_memory_t* mem = (pdev->v_memory ? pdev->v_memory : dev->memory);
	int ecode, code;
	...
	if(!(pdev->params.LockDistillerParams && params.LockDistillerParams)) {
		...
		if(code < 0)
			ecode = code;
		params.AutoRotatePages =
		    (enum psdf_auto_rotate_pages)psdf_put_enum(
		        plist, "AutoRotatePages", (int)params.AutoRotatePages,
		        AutoRotatePages_names, &ecode);
		params.Binding = (enum psdf_binding)psdf_put_enum(
		    plist, "Binding", (int)params.Binding, Binding_names,
		    &ecode);
		params.DefaultRenderingIntent =
		    (enum psdf_default_rendering_intent)psdf_put_enum(
		        plist, "DefaultRenderingIntent",
		        (int)params.DefaultRenderingIntent,
		        DefaultRenderingIntent_names, &ecode);
		params.TransferFunctionInfo =
		    (enum psdf_transfer_function_info)psdf_put_enum(
		        plist, "TransferFunctionInfo",
		        (int)params.TransferFunctionInfo,
		        TransferFunctionInfo_names, &ecode);
		params.UCRandBGInfo = (enum psdf_ucr_and_bg_info)psdf_put_enum(
		    plist, "UCRandBGInfo", (int)params.UCRandBGInfo,
		...
		params.ColorConversionStrategy =
		    (enum psdf_color_conversion_strategy)psdf_put_enum(
		        plist, "ColorConversionStrategy",
		        (int)params.ColorConversionStrategy,
		        ColorConversionStrategy_names, &ecode);
		ecode =
		    psdf_read_string_param(plist, "CalCMYKProfile",
		                           &params.CalCMYKProfile, mem, ecode);
		ecode =
		    psdf_read_string_param(plist, "CalGrayProfile",
		                           &params.CalGrayProfile, mem, ecode);
		ecode = psdf_read_string_param(
		    plist, "CalRGBProfile", &params.CalRGBProfile, mem, ecode);
		...
		ecode =
		    psdf_put_embed_param(plist, "~AlwaysEmbed", ".AlwaysEmbed",
		                         &params.AlwaysEmbed, mem, ecode);
		ecode =
		    psdf_put_embed_param(plist, "~NeverEmbed", ".NeverEmbed",
		                         &params.NeverEmbed, mem, ecode);
		params.CannotEmbedFontPolicy =
		    (enum psdf_cannot_embed_font_policy)psdf_put_enum(
		        plist, "CannotEmbedFontPolicy",
