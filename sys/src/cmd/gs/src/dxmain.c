...
static gboolean window_draw(GtkWidget* widget, GdkEventExpose* event,
                            gpointer user_data);
	...
	if(img && img->window && img->buf) {
		...
		if(event->area.x + event->area.width > img->width) {
			...
			height = min(img->height,
			         ...
			         y;
			gdk_window_clear_area(widget->window, x, y, width,
			                      height);
		}
...
static void
window_resize(IMAGE* img)
{
	gtk_drawing_area_size(GTK_DRAWING_AREA(img->darea), img->width,
	                      img->height);
...
static void
window_separation(IMAGE* img, int sep)
{
...
static void
signal_sep0(GtkWidget* w, gpointer data)
{
	window_separation((IMAGE*)data, 0);
}

static void
signal_sep1(GtkWidget* w, gpointer data)
{
	window_separation((IMAGE*)data, 1);
}

static void
signal_sep2(GtkWidget* w, gpointer data)
{
	window_separation((IMAGE*)data, 2);
}

static void
signal_sep3(GtkWidget* w, gpointer data)
{
	window_separation((IMAGE*)data, 3);
}

static void
signal_sep4(GtkWidget* w, gpointer data)
{
	window_separation((IMAGE*)data, 4);
}

static void
signal_sep5(GtkWidget* w, gpointer data)
{
	window_separation((IMAGE*)data, 5);
}

static void
signal_sep6(GtkWidget* w, gpointer data)
{
	window_separation((IMAGE*)data, 6);
}

static void
signal_sep7(GtkWidget* w, gpointer data)
{
	window_separation((IMAGE*)data, 7);
}

GtkSignalFunc signal_separation[IMAGE_DEVICEN_MAX] = {
    signal_sep0, signal_sep1, signal_sep2, signal_sep3,
    signal_sep4, signal_sep5, signal_sep6, signal_sep7};
...
static void
signal_show_as_gray(GtkWidget* w, gpointer data)
{
	IMAGE* img = (IMAGE*)data;
	...
	if((color == DISPLAY_COLORS_CMYK) ||
	   (color == DISPLAY_COLORS_SEPARATION)) {
		if(!img->cmyk_bar) {
			...
			img->show_as_gray =
			    gtk_check_button_new_with_label("Show as Gray");
			gtk_box_pack_end(GTK_BOX(img->cmyk_bar),
			                 img->show_as_gray, FALSE, FALSE, 5);
			gtk_toggle_button_set_active(
			    GTK_TOGGLE_BUTTON(img->show_as_gray), FALSE);
			gtk_signal_connect(GTK_OBJECT(img->show_as_gray),
			                   "clicked", signal_show_as_gray, img);
...
display_sync(void* handle, void* device)
{
	   ...
	   (color == DISPLAY_COLORS_SEPARATION)) {
		...
		for(i = 0; i < IMAGE_DEVICEN_MAX; i++) {
			...
			if(!img->devicen[i].used)
				...
				gtk_label_set_text(
				    GTK_LABEL(
				        GTK_BIN(img->separation[i])->child),
				    img->devicen[i].name);
	...
	case DISPLAY_COLORS_CMYK:
		if(depth == DISPLAY_DEPTH_8) {
			...
			int show_gray =
			    (vc + vm + vy + vk == 1) && img->devicen_gray;
			for(y = 0; y < img->height; y++) {
				s = img->buf + y * img->rowstride;
				d = img->rgbbuf + y * img->width * 3;
				for(x = 0; x < img->width; x++) {
					...
					*d++ = (255 - cyan) * (255 - black) /
					       255; /* r */
					*d++ = (255 - magenta) * (255 - black) /
					       255; /* g */
					*d++ = (255 - yellow) * (255 - black) /
					       255; /* b */
				}
			}
...
static int
display_separation(void* handle, void* device, int comp_num, const char* name,
                   unsigned short c, unsigned short m, unsigned short y,
                   unsigned short k)
{
	IMAGE* img = image_find(handle, device);
	if(img == NULL)
...
main(int argc, char* argv[])
{
	...
	if((code = gsapi_new_instance(&instance, NULL)) == 0) {
		gsapi_set_stdio(instance, gsdll_stdin, gsdll_stdout,
		                gsdll_stderr);
		if(use_gui)
		...
		if(code == 0)
			code = gsapi_run_string(instance, start_string, 0,
			                        &exit_code);
