...
{
	if(obj == 0)
		return gs_alloc_struct_array(mem, num_elements, void, pstype,
		                             cname);
#ifdef DEBUG
	if(gs_object_type(mem, obj) != pstype) {
		lprintf3("resize_struct_array 0x%lx, type was 0x%lx, expected "
		         "0x%lx!\n",
...
void*
gs_raw_alloc_struct_immovable(gs_memory_t* rmem, gs_memory_type_ptr_t pstype,
                              client_name_t cname)
{
	return gs_alloc_bytes_immovable(rmem, gs_struct_type_size(pstype),
	                                cname);
}
...
int
gs_register_struct_root(gs_memory_t* mem, gs_gc_root_t* root, void** pp,
                        client_name_t cname)
{
...
rc_object_type_name(const void* vp, const rc_header* prc)
{
	...
	if(prc->free != rc_free_struct_only) {
		/*
		 * This object might be stack-allocated or have other unusual
		 * memory
...
ENUM_PTRS_BEGIN_PROC(basic_enum_ptrs)
{
	...
	if(size == 0) {
#ifdef DEBUG
		dprintf2("  basic_enum_ptrs: Attempt to enum 0 size structure "
		         "at 0x%lx, type: %s\n",
		         vptr, pstype->sname);
#endif
		return 0;
	...
	if(index < psd->num_ptrs) {
		...
		if(ppe->offset > size - sizeof(void*)) {
			dprintf4("  basic_enum_ptrs: Attempt to enum ptr with "
			         "offset=%d beyond size=%d: structure at "
			         "0x%lx, type: %s\n",
	...
	return ENUM_USING(
	    *(psd->super_type),
	    (EV_CONST void*)((EV_CONST char*)vptr + psd->super_offset),
