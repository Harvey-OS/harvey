...
/* ---------- Forward refs ----------- */
private
void ptr_align_to(const byte** src,  /* pointer to align */
                  unsigned alignment /* alignment, must be power of 2 */
                  );
private
void wb_put_word(unsigned source,  /* number to put to buffer */
                 WriteBuffer* dest /* destination descriptor */
                 );
private
void wb_put_bytes(const byte* source,     /* bytes to put to buffer */
                  unsigned source_sizeof, /* # bytes to put */
                  WriteBuffer* dest       /* destination descriptor */
                  );
private
void
wb_put_alignment(unsigned alignment, /* alignment to match, must be power 2 */
                 WriteBuffer* dest   /* destination descriptor */
                 );

/* Get word compressed with wb_put_word */
private
unsigned                          /* decompressed word */
    buf_get_word(const byte** src /* UPDATES: ptr to src buf ptr */
                 );
        ...
        )
{
	...
	while((code = param_get_next_key(list, &key_enum, &key)) == 0) {
		...
		if((code = param_read_typed(list, string_key, &value)) != 0) {
			code = code > 0 ? gs_note_error(gs_error_unknownerror)
			                : code;
			break;
		}
		...
		case gs_param_type_float:
			wb_put_bytes((byte*)&value.value, value_top_sizeof,
			             &write_buf);
			break;
		...
		case gs_param_type_float_array:
			...
			wb_put_bytes(value.value.s.data, value_base_sizeof,
			             &write_buf);
			break;
		...
		case gs_param_type_name_array:
			...
			wb_put_bytes((const byte*)&value.value,
			             value_top_sizeof, &write_buf);
			wb_put_alignment(sizeof(void*), &write_buf);

			wb_put_bytes((const byte*)value.value.sa.data,
			             value_base_sizeof, &write_buf);
			{
				int str_count;
				const gs_param_string* sa;

				for(str_count = value.value.sa.size,
				sa = value.value.sa.data;
				    str_count-- > 0; ++sa)
					wb_put_bytes(sa->data, sa->size,
					             &write_buf);
			}
			break;
		...
		case gs_param_type_dict_int_keys:
			...
			{
				int bytes_written = gs_param_list_serialize(
				    value.value.d.list, write_buf.buf,
				    write_buf.buf
				        ? write_buf.buf_end - write_buf.buf
				        : 0);

				temp_code = param_end_read_dict(
				    list, (const char*)key.data,
				    &value.value.d);
				if(bytes_written < 0)
					code = bytes_written;
				else {
        ...
        )
{
	...
	do {
		...
		typed.type = type;
		if(type != gs_param_type_dict &&
		   type != gs_param_type_dict_int_keys) {
		...
		case gs_param_type_dict_int_keys:
			...
			code =
			    gs_param_list_unserialize(typed.value.d.list, buf);
			temp_code =
			    param_end_write_dict(list, key, &typed.value.d);
			if(code >= 0) {
				buf += code;
				code = temp_code;
			}
...
private
void
ptr_align_to(const byte** src,  /* pointer to align */
             unsigned alignment /* alignment, must be power of 2 */
             )
{
...
private
void
wb_put_word(unsigned source,  /* number to put to buffer */
            WriteBuffer* dest /* destination descriptor */
            )
{
	do {
...
private
void
wb_put_bytes(const byte* source,     /* bytes to put to buffer */
             unsigned source_sizeof, /* # bytes to put */
             WriteBuffer* dest       /* destination descriptor */
             )
{
