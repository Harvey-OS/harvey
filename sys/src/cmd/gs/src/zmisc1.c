...
private
int type1crypt(i_ctx_t*, int (*)(byte*, const byte*, uint, uint16_t*));
private
int
...
private
int
type1crypt(i_ctx_t* i_ctx_p, int (*proc)(byte*, const byte*, uint, uint16_t*))
{
	os_ptr op = osp;
	crypt_state state;
	uint ssize;
...
zexE(i_ctx_t* i_ctx_p)
{
	...
	if(code < 0)
		return code;
	return filter_write(i_ctx_p, code, &s_exE_template,
...
zexD(i_ctx_t* i_ctx_p)
{
	...
	if(r_has_type(op, t_dictionary)) {
		...
		if((code = dict_uint_param(op, "seed", 0, 0xffff, 0x10000,
		   ...
		   (code = dict_int_param(op, "lenIV", 0, max_int, 4,
		                          &state.lenIV)) < 0 ||
		   (code = dict_bool_param(op, "eexec", false, &is_eexec)) < 0)
			return code;
		state.cstate = cstate;
	...
	if(r_has_type(op - 1, t_file)) {
		...
		if(s->state != 0 && s->state->template == &s_PFBD_template) {
			 ...
			 * avoid the conversion from binary to hex and back
			 * again.
			 */
			if(pss->record_type == 2) {
				/*
				 * The PFB decoder may have converted some data
				 * to hex
	...
	return filter_read(i_ctx_p, code, &s_exD_template,
	                   (stream_state*)&state, 0);
}

/* ------ Initialization procedure ------ */

const op_def zmisc1_op_defs[] = {{"3.type1encrypt", ztype1encrypt},
