...
#define debug_print_nodes(nodes, n, tag, lengths)                              \
	if(gs_debug_c('W'))                                                    \
		print_nodes_proc(nodes, n, tag, lengths);
private
void
print_nodes_proc(const count_node* nodes, int n, const char* tag, int lengths)
{
	...
	for(i = 0; i < n; ++i)
		dlprintf7(
		    "[w]node %d: f=%ld v=%d len=%d N=%d L=%d R=%d\n", i,
		    nodes[i].freq, nodes[i].value, nodes[i].code_length,
...
{
	return (pn1->value < pn2->value ? -1 : pn1->value > pn2->value ? 1 : 0);
}
#undef pn1
#undef pn2
...
hc_limit_code_lengths(count_node* nodes, uint num_values, int max_length)
{
	 ...
	 */
	for(; needed > 0; --needed) { /* longest points to the first code of
	                                 length max_length. */
...
hc_compute(hc_definition* def, const int32_t* freqs, gs_memory_t* mem)
{
	...
	count_node* nodes = (count_node*)gs_alloc_byte_array(
	    mem, num_values * 2 - 1, sizeof(count_node), "hc_compute");
	int i;
	count_node* lowest;
	count_node* comb;
	...
	for(i = 0; i < num_values; ++i)
		nodes[i].next = &nodes[i - 1], nodes[i].code_length = 0,
...
void
hc_make_decoding(hcd_code* decode, const hc_definition* def, int initial_bits)
