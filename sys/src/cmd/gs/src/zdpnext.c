...
zsizeimageparams(i_ctx_t *i_ctx_p)
{
	 ...
	 */
		gx_color_value max_value =
		    (dev->color_info.num_components == 1 ? dev->color_info.max_gray : max(dev->color_info.max_gray, dev->color_info.max_color));
...
begin_composite(i_ctx_t *i_ctx_p, alpha_composite_state_t *pcp)
{
	...
	code = (*dev_proc(dev, create_compositor))(dev, &pcp->cdev, pcp->pcte, (gs_imager_state *)igs, imemory);
	if(code < 0) {
