...
/* Forward references */
private
int begin_composite(i_ctx_t*, alpha_composite_state_t*);
private
void end_composite(i_ctx_t*, alpha_composite_state_t*);
private
int xywh_param(os_ptr, double[4]);
...
composite_image(i_ctx_t* i_ctx_p, const gs_composite_alpha_params_t* params)
{
	...
	if(code < 0 || (code = num_params(op - 1, 2, dest_pt)) < 0)
		return code;
	...
	if(code >= 0) {
		code = process_non_source_image(
		    i_ctx_p, (const gs_image_common_t*)&image,
		    "composite_image");
		end_composite(i_ctx_p, &cstate);
		if(code >= 0)
			pop(8);
...
private
int device_is_true_color(gx_device* dev);

/* <x> <y> <width> <height> <matrix> .sizeimagebox */
/*   <dev_x> <dev_y> <dev_width> <dev_height> <matrix> */
private
void box_confine(int* pp, int* pq, int wh);
private
int
...
const op_def zdpnext_op_defs[] = {{"0currentalpha", zcurrentalpha},
                                  {"1setalpha", zsetalpha},
...
{
	gx_device* dev = gs_currentdevice(igs);
	int code = gs_create_composite_alpha(&pcp->pcte, &pcp->params, imemory);
	...
	case 4: /* CMYK */
			...
			return false;
		{
			...
			for(i = 0; i <= max_v; ++i) {
				...
				cyan[0] = CV(i);
				cyan[1] = cyan[2] = cyan[3] = CV0;
				magenta[1] = CV(i);
				magenta[0] = magenta[2] = magenta[3] = CV0;
				yellow[2] = CV(i);
				yellow[0] = yellow[1] = yellow[3] = CV0;
				black[3] = CV(i);
				black[0] = black[1] = black[2] = CV0;
				if((*dev_proc(dev, map_cmyk_color))(
				       dev, cyan) != i << cs ||
