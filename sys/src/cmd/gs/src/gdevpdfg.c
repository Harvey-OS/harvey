...
pdf_save_viewer_state(gx_device_pdf* pdev, stream* s)
{
	...
	pdev->vgstack[i].fill_used_process_color =
	    pdev->fill_used_process_color;
	pdev->vgstack[i].stroke_used_process_color =
	...
	pdev->vgstack[i].line_params.dash.pattern =
	    0; /* Use pdev->dash_pattern instead. */
...
pdf_load_viewer_state(gx_device_pdf* pdev, pdf_viewer_state* s)
{
	...
	memcpy(pdev->dash_pattern, s->dash_pattern, sizeof(s->dash_pattern));
}
...
private
void
pdf_viewer_state_from_imager_state_aux(pdf_viewer_state* pvs,
                                       const gs_imager_state* pis)
{
	pvs->transfer_not_identity =
	    (pis->set_transfer.red != NULL
	         ? pis->set_transfer.red->proc != gs_identity_transfer
	         : 0) *
	        1 +
	    (pis->set_transfer.green != NULL
	         ? pis->set_transfer.green->proc != gs_identity_transfer
	         : 0) *
	        2 +
	    (pis->set_transfer.blue != NULL
	         ? pis->set_transfer.blue->proc != gs_identity_transfer
	         : 0) *
	        4 +
	    (pis->set_transfer.gray != NULL
	         ? pis->set_transfer.gray->proc != gs_identity_transfer
	         : 0) *
	        8;
	pvs->transfer_ids[0] =
	    (pis->set_transfer.red != NULL ? pis->set_transfer.red->id : 0);
	pvs->transfer_ids[1] =
	    (pis->set_transfer.green != NULL ? pis->set_transfer.green->id : 0);
	pvs->transfer_ids[2] =
	    (pis->set_transfer.blue != NULL ? pis->set_transfer.blue->id : 0);
	pvs->transfer_ids[3] =
	    (pis->set_transfer.gray != NULL ? pis->set_transfer.gray->id : 0);
	...
	pvs->black_generation_id =
	    (pis->black_generation != 0 ? pis->black_generation->id : 0);
	pvs->undercolor_removal_id =
	    (pis->undercolor_removal != 0 ? pis->undercolor_removal->id : 0);
	...
	pvs->line_params.dot_length_absolute =
...
/* Prepare intitial values for viewer's graphics state parameters. */
void
pdf_prepare_initial_viewer_state(gx_device_pdf* pdev,
                                 const gs_imager_state* pis)
{
	/* Parameter values, which are specified in PDF spec, are set here.
	 * Parameter values, which are specified in PDF spec as "installation
	 * dependent",
	...
	pdf_set_initial_color(pdev, &pdev->vg_initial.saved_fill_color,
	                      &pdev->vg_initial.saved_stroke_color,
	                      &pdev->vg_initial.fill_used_process_color,
	                      &pdev->vg_initial.stroke_used_process_color);
...
pdf_reset_graphics_old(gx_device_pdf* pdev)
{
	...
	pdf_set_initial_color(
	    pdev, &pdev->saved_fill_color, &pdev->saved_stroke_color,
                 ...
                 const gs_client_color* pcc)
{
	...
	for(i = 1; i < n; i++) {
		pprintg1(pdev->strm, " %g",
		         psdf_round(pcc->paint.values[i], 255, 8));
	}
	return 0;
}
                ...
                const psdf_set_color_commands_t* ppscc)
{
	...
	const gs_client_color*
	    pcc; /* fixme: not needed due to gx_hld_get_color_component. */
	...
	case non_pattern_color_space:
		...
		case gs_color_space_index_Indexed:
			if(pdev->CompatibilityLevel <= 1.2) {
				pcs2 = (const gs_color_space*)&pcs->params
				           .indexed.base_space;
				csi = gs_color_space_get_index(pcs2);
				if(csi == gs_color_space_index_Separation) {
					pcs2 =
					    (const gs_color_space*)&pcs2->params
					        .separation.alt_space;
					goto check_pcs2;
				}
				goto check_pcs2;
			}
		...
		scn:
			...
			if(!gx_hld_saved_color_same_cspace(&temp, psc)) {
				code = pdf_color_space(
				    pdev, &cs_value, NULL, pcs,
	...
	case pattern_color_sapce: {
		...
		if(pdc->type == gx_dc_type_pattern)
			code = pdf_put_colored_pattern(
			    pdev, pdc, pcs, ppscc, pis->have_pattern_streams,
			    &pres);
		else if(pdc->type == &gx_dc_pure_masked) {
			code = pdf_put_uncolored_pattern(
			    pdev, pdc, pcs, ppscc, pis->have_pattern_streams,
			    &pres);
			if(code < 0 || pres == 0)
				return code;
...
pdf_set_pure_color(gx_device_pdf* pdev, gx_color_index color,
                   gx_hl_saved_color* psc, bool* used_process_color,
                   const psdf_set_color_commands_t* ppscc)
{
	gx_drawing_color dcolor;

	set_nonclient_dev_color(&dcolor, color);
	return pdf_reset_color(pdev, NULL, &dcolor, psc, used_process_color,
	                       ppscc);
}
 ...
 */
private
data_source_proc_access(transfer_map_access); /* check prototype */
private
int
transfer_map_access(const gs_data_source_t* psrc, ulong start, uint length,
                    byte* buf, const byte** ptr)
{
	const gx_transfer_map* map =
	    (const gx_transfer_map*)psrc->data.str.data;
	uint i;
...
private
int
transfer_map_access_signed(const gs_data_source_t* psrc, ulong start,
                           uint length, byte* buf, const byte** ptr)
{
	/* To prevent numeric errors, we need to map 0 to an integer.
	 * We can't apply a general expression, because Decode isn't accessible
	 * here.
	...
	*ptr = buf;
	for(i = 0; i < length; ++i)
		buf[i] = (byte)((frac2float(map->values[(uint)start + i]) + 1) *
		                127);
	return 0;
}
private
...
pdf_write_transfer_map(gx_device_pdf* pdev, const gx_transfer_map* map,
                       int range0, bool check_identity, const char* key,
                       char* ids)
{
	...
	if(check_identity) {
		...
		if(map->proc == gs_identity_transfer)
			...
			for(i = 0; i < transfer_map_size; ++i) {
				fixed d = map->values[i] -
				          bits2frac(i, log2_transfer_map_size);
				if(any_abs(d) >
				   fixed_epsilon) /* ignore small noise */
					break;
			}
	...
	if(range01[0] < 0 && range01[1] > 0) {
		...
		decode[1] = range01[0] +
		            (range01[1] - range01[0]) *
		                ((1 << params.BitsPerSample) - 1) / (r0 + r1);
	...
	if(code < 0)
		return code;
	sprintf(ids, "%s%s%ld 0 R", key, (key[0] && key[0] != ' ' ? " " : ""),
	        id);
	return 0;
}
private
...
#define HT_FUNC(name, expr)                                                    \
      private                                                                  \
	floatp name(floatp xd, floatp yd)                                      \
	{                                                                      \
                        ...
                        long* pid)
{
	...
	values =
	    gs_alloc_byte_array(mem, num_bits, 2, "pdf_write_spot_function");
	if(values == 0)
		return_error(gs_error_VMerror);
	...
	if(code >= 0 && (code = gs_function_Sd_init(&pfn, &params, mem)) >= 0) {
                        ...
                        const gx_ht_order* porder, long* pid)
{
	 ...
	 */
	{
		...
		for(i = 0; i < countof(ht_functions); ++i) {
			floatp (*spot_proc)(floatp, floatp) =
			    ht_functions[i].proc;
			gs_point pt;

			gs_screen_enum_init_memory(&senum, &order, NULL,
			                           &psht->screen, mem);
			while(
			    (code = gs_screen_currentpoint(&senum, &pt)) == 0 &&
                               ...
                               const gx_device_halftone* pdht, long* pid)
{
	...
	for(i = 0; i < pdht->num_comp; ++i) {
		int code = pdf_write_screen_halftone(
		    pdev, &pcsht->screens.indexed[i],
		    &pdht->components[i].corder, &ht_ids[i]);
		if(code < 0)
			return code;
	}
	*pid = pdf_begin_separate(pdev);
	...
	if(pdht->num_comp > 3)
		pprintld2(s, "/Gray %ld 0 R/Black %ld 0 R", ht_ids[3],
		          ht_ids[3]);
	stream_puts(s, ">>\n");
	return pdf_end_separate(pdev);
}
...
#define CHECK(expr)                                                            \
	BEGIN if((code = (expr)) < 0) return code;                             \
	END
                             ...
                             const gx_ht_order* porder, long* pid)
{
	...
	int code = pdf_write_transfer(pdev, porder->transfer, "", trs);
	...
	*pid = writer.pres->object->id;
	CHECK(cos_dict_put_c_strings((cos_dict_t*)writer.pres->object, "/Type",
	                             "/Halftone"));
	CHECK(cos_dict_put_c_strings((cos_dict_t*)writer.pres->object,
	                             "/HalftoneType", "6"));
	CHECK(cos_dict_put_c_key_int((cos_dict_t*)writer.pres->object, "/Width",
	                             ptht->width));
	...
	if(*trs != 0)
		CHECK(cos_dict_put_c_strings((cos_dict_t*)writer.pres->object,
		                             "/TransferFunction", trs));
	stream_write(writer.binary.strm, ptht->thresholds.data,
	             ptht->thresholds.size);
...
pdf_get_halftone_component_index(const gs_multiple_halftone* pmht,
                                 const gx_device_halftone* pdht, int dht_index)
{
	int j;
	...
	if(j == pmht->num_comp) {
		/* Look for Default. */
		for(j = 0; j < pmht->num_comp; j++)
			if(pmht->components[j].comp_number ==
                            ...
                            const gx_device_halftone* pdht, long* pid)
{
	...
	for(i = 0; i < pdht->num_comp; ++i) {
		...
		if(code < 0)
			return code;
		if(pmht->components[code].comp_number ==
		   GX_DEVICE_COLOR_MAX_COMPONENTS) {
			if(done_Default)
				continue;
			done_Default = true;
		...
		case ht_type_threshold:
			code = pdf_write_threshold_halftone(
			    pdev, &phtc->params.threshold, porder, &ids[i]);
			break;
		case ht_type_threshold2:
			code = pdf_write_threshold2_halftone(
			    pdev, &phtc->params.threshold2, porder, &ids[i]);
			break;
		default:
	...
	*pid = pdf_begin_separate(pdev);
	...
	for(i = 0; i < pdht->num_comp; ++i) {
		...
		phtc = &pmht->components[code];
		if((code = pmht->get_colorname_string(pdev->memory, phtc->cname,
		                                      &str, &len)) < 0 ||
...
private
int
pdf_update_halftone(gx_device_pdf* pdev, const gs_imager_state* pis, char* hts)
{
	...
	case ht_type_screen:
		code =
		    pdf_write_screen_halftone(pdev, &pht->params.screen,
		                              &pdht->components[0].corder, &id);
		break;
	case ht_type_colorscreen:
		code = pdf_write_colorscreen_halftone(
		    pdev, &pht->params.colorscreen, pdht, &id);
		break;
	case ht_type_spot:
		code = pdf_write_spot_halftone(
		    pdev, &pht->params.spot, &pdht->components[0].corder, &id);
		break;
	case ht_type_threshold:
		code = pdf_write_threshold_halftone(
		    pdev, &pht->params.threshold, &pdht->components[0].corder,
		    &id);
		break;
	case ht_type_threshold2:
		code = pdf_write_threshold2_halftone(
		    pdev, &pht->params.threshold2, &pdht->components[0].corder,
		    &id);
		break;
...
pdf_open_gstate(gx_device_pdf* pdev, pdf_resource_t** ppres)
{
	...
	if(pdev->context != PDF_IN_STREAM) {
		/* We apparently use gs_error_interrupt as a request to change
		 * context. */
		return gs_error_interrupt;
	}
	code =
	    pdf_alloc_resource(pdev, resourceExtGState, gs_no_id, ppres, -1L);
	if(code < 0)
		return code;
	cos_become((*ppres)->object, cos_type_dict);
	code = cos_dict_put_c_key_string(resource_dict(*ppres), "/Type",
	                                 (const byte*)"/ExtGState", 10);
	if(code < 0)
		return code;
	return 0;
}
...
{
	if(pres) {
		int code = pdf_substitute_resource(
		    pdev, &pres, resourceExtGState, NULL, true);
...
private
int
pdf_update_transfer(gx_device_pdf* pdev, const gs_imager_state* pis, char* trs)
{
	int i, pi = -1;
	...
	if(update) {
		...
		memcpy(pdev->transfer_ids, transfer_ids,
		       sizeof(pdev->transfer_ids));
                 ...
                 pdf_resource_t** ppres)
{
	...
	if(pdev->state.soft_mask_id != pis->soft_mask_id) {
		...
		if(code < 0)
			return code;
		code = cos_dict_put_c_key_string(
		    resource_dict(*ppres), "/SMask", (byte*)buf, strlen(buf));
		if(code < 0)
			return code;
                    ...
                    pdf_resource_t** ppres)
{
	...
	if(pdev->CompatibilityLevel >= 1.4) {
		if(pdev->state.blend_mode != pis->blend_mode) {
			...
			if(code < 0)
				return code;
			buf[0] = '/';
			strncpy(buf + 1, bm_names[pis->blend_mode],
			        sizeof(buf) - 2);
			code = cos_dict_put_string_copy(resource_dict(*ppres),
			                                "/BM", buf);
			if(code < 0)
				return code;
	...
	if(pdev->sbstack_depth == bottom) {
		...
		char hts[5 + MAX_FN_CHARS + 1],
		    ...
		    bgs[5 + MAX_FN_CHARS + 1], ucrs[6 + MAX_FN_CHARS + 1];

		hts[0] = trs[0] = bgs[0] = ucrs[0] = 0;
		if(pdev->params.PreserveHalftoneInfo &&
		   pdev->halftone_id != pis->dev_ht->id && !pdev->PDFX) {
		...
		if(pdev->params.UCRandBGInfo == ucrbg_Preserve) {
			if(pdev->black_generation_id !=
			   pis->black_generation->id) {
				code = pdf_write_transfer_map(
				    pdev, pis->black_generation, 0, false, "",
				    bgs);
				if(code < 0)
					return code;
				pdev->black_generation_id =
				    pis->black_generation->id;
			}
			if(pdev->undercolor_removal_id !=
			   pis->undercolor_removal->id) {
				code = pdf_write_transfer_map(
				    pdev, pis->undercolor_removal, -1, false,
				    "", ucrs);
				if(code < 0)
					return code;
				pdev->undercolor_removal_id =
				    pis->undercolor_removal->id;
			}
		}
		...
		if(!pdev->PDFX) {
			...
			if(dev_phase.x != phase.x || dev_phase.y != phase.y) {
				...
				code = cos_dict_put_string_copy(
				    resource_dict(*ppres), "/HTP", buf);
				if(code < 0)
					return code;
				gx_imager_setscreenphase(&pdev->state, phase.x,
				                         phase.y,
				                         gs_color_select_all);
			}
		}
	...
	if(pdev->CompatibilityLevel >= 1.3 && pdev->sbstack_depth == bottom) {
		if(pdev->overprint_mode != pdev->params.OPM) {
			...
			if(code < 0)
				return code;
			code = cos_dict_put_c_key_int(resource_dict(*ppres),
			                              "/OPM", pdev->params.OPM);
			if(code < 0)
				return code;
		...
		if(pdev->state.smoothness != pis->smoothness) {
			...
			if(code < 0)
				return code;
			code = cos_dict_put_c_key_real(resource_dict(*ppres),
			                               "/SM", pis->smoothness);
			if(code < 0)
				return code;
		...
		if(pdev->CompatibilityLevel >= 1.4) {
			if(pdev->state.text_knockout != pis->text_knockout) {
				...
				if(code < 0)
					return code;
				code = cos_dict_put_c_key_bool(
				    resource_dict(*ppres), "/TK",
				    pis->text_knockout);
				if(code < 0)
					return code;
...
pdf_try_prepare_fill(gx_device_pdf* pdev, const gs_imager_state* pis)
{
	...
	if(pdev->params.PreserveOverprintSettings &&
	   pdev->fill_overprint != pis->overprint && !pdev->skip_colors) {
		code = pdf_open_gstate(pdev, &pres);
		if(code < 0)
			return code;
		/* PDF 1.2 only has a single overprint setting. */
		if(pdev->CompatibilityLevel < 1.3) {
			code = cos_dict_put_c_key_bool(resource_dict(pres),
			                               "/OP", pis->overprint);
			if(code < 0)
				return code;
...
pdf_try_prepare_stroke(gx_device_pdf* pdev, const gs_imager_state* pis)
{
	...
	if(pdev->params.PreserveOverprintSettings &&
	   pdev->stroke_overprint != pis->overprint && !pdev->skip_colors) {
		code = pdf_open_gstate(pdev, &pres);
		if(code < 0)
			return code;
		code = cos_dict_put_c_key_bool(resource_dict(pres), "/OP",
		                               pis->overprint);
		if(code < 0)
			return code;
		pdev->stroke_overprint = pis->overprint;
		if(pdev->CompatibilityLevel < 1.3) {
			...
			/* According to PDF>=1.3 spec, OP also sets op,
			   ...
			   We don't write /op, so monitor the viewer's state
			   here : */
			pdev->fill_overprint = pis->overprint;
		}
	}
	if(pdev->state.stroke_adjust != pis->stroke_adjust) {
		...
		if(code < 0)
			return code;
		code = cos_dict_put_c_key_bool(resource_dict(pres), "/SA",
		                               pis->stroke_adjust);
		if(code < 0)
			return code;
