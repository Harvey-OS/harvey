...
/* Copyright (C) 1989, 1995, 1997, 1998, 1999 Aladdin Enterprises.  All rights
  reserved.
...
case 1:
ENUM_RETURN((mptr->foreign_line_pointers ? NULL : (void*)mptr->line_ptrs));
...
private
const byte b_w_palette_string[6] = {0xff, 0xff, 0xff, 0, 0, 0};
const gs_const_string mem_mono_b_w_palette = {b_w_palette_string, 6};
private
const byte w_b_palette_string[6] = {0, 0, 0, 0xff, 0xff, 0xff};
...
private
const gx_device_memory* const mem_devices[65] = {
    0, &mem_mono_device, &mem_mapped2_device, 0, &mem_mapped4_device, 0, 0, 0,
    &mem_mapped8_device, 0, 0, 0, 0, 0, 0, 0, &mem_true16_device, 0, 0, 0, 0, 0,
    0, 0, &mem_true24_device, 0, 0, 0, 0, 0, 0, 0, &mem_true32_device, 0, 0, 0,
    0, 0, 0, 0, &mem_true40_device, 0, 0, 0, 0, 0, 0, 0, &mem_true48_device, 0,
    0, 0, 0, 0, 0, 0, &mem_true56_device, 0, 0, 0, 0, 0, 0, 0,
    &mem_true64_device};
...
private
const gx_device_memory* const mem_word_devices[65] = {
    0, &mem_mono_device, &mem_mapped2_word_device, 0, &mem_mapped4_word_device,
    0, 0, 0, &mem_mapped8_word_device, 0, 0, 0, 0, 0, 0, 0,
    0 /*no 16-bit word device*/, 0, 0, 0, 0, 0, 0, 0, &mem_true24_word_device,
    0, 0, 0, 0, 0, 0, 0, &mem_true32_word_device, 0, 0, 0, 0, 0, 0, 0,
    &mem_true40_word_device, 0, 0, 0, 0, 0, 0, 0, &mem_true48_word_device, 0, 0,
    0, 0, 0, 0, 0, &mem_true56_word_device, 0, 0, 0, 0, 0, 0, 0,
...
gs_device_is_memory(const gx_device* dev)
{
	...
	if(mdproto != 0 &&
	   dev_proc(dev, draw_thin_line) == dev_proc(mdproto, draw_thin_line))
		return true;
	mdproto = mem_word_devices[bits_per_pixel];
	return (mdproto != 0 &&
	        dev_proc(dev, draw_thin_line) ==
                   ...
                   gs_memory_t* mem, int page_device, gx_device* target)
{
	...
	case -1:
		...
		break;
	case 1:
		set_dev_proc(dev, get_page_device,
...
gs_make_mem_mono_device(gx_device_memory* dev, gs_memory_t* mem,
                        gx_device* target)
{
	gx_device_init((gx_device*)dev, (const gx_device*)&mem_mono_device, mem,
	               true);
	set_dev_proc(dev, get_page_device, gx_default_get_page_device);
	gx_device_set_target((gx_device_forward*)dev, target);
...
gdev_mem_bits_size(const gx_device_memory* dev, int width, int height)
{
	...
	if(num_planes)
		...
		planes = &plane1, plane1.depth = dev->color_info.depth,
		num_planes = 1;
                    ...
                    bool page_uses_transparency)
{
	...
	if(page_uses_transparency) {
		/*
		 * If the device is using PDF 1.4 transparency then we will need
		 * to
		 * also allocate image buffers for doing the blending
		 * operations.
		 * We can only estimate the space requirements.  However since
		 * it
		 * is only an estimate, we may exceed our desired buffer space
		 * while
		 * processing the file.
		 */
		max_height =
		    size / (bitmap_raster(width * dev->color_info.depth +
		                          ESTIMATED_PDF14_ROW_SPACE(width)) +
...
gdev_mem_open_scan_lines(gx_device_memory* mdev, int setup_height)
{
	...
	if(mdev->bitmap_memory != 0) {
		...
		mdev->base =
		    gs_alloc_bytes(mdev->bitmap_memory, (uint)size, "mem_open");
		if(mdev->base == 0)
		...
		mdev->line_ptrs = (byte**)gs_alloc_byte_array(
		    mdev->line_pointer_memory, mdev->height,
                       ...
                       byte** line_ptrs, int setup_height)
{
	...
	byte* data = (base ? (mdev->raster = raster, mdev->base = base)
                       ...
                       gs_get_bits_params_t* params, gs_int_rect** unread)
{
	...
	int x = prect->p.x, w = prect->q.x - x, y = prect->p.y,
	    h = prect->q.y - y;
		...
		return_error(gs_error_rangecheck);
	{
		...
		if(code >= 0)
			return code;
		return gx_get_bits_copy(dev, x, w, h, params, &copy_params,
		                        base, gx_device_raster(dev, true));
	}
}
...
mem_swap_byte_rect(byte* base, uint raster, int x, int w, int h, bool store)
{
	...
	if(store) {
		if(xbit + w > 64) { /* Operation spans multiple words. */
			...
			if(xbit != 0)
				mem_swap_byte_rect(base, raster, x, 1, h,
				                   false);
			x += w - 1;
			xbit = x & 31;
			if(xbit == 31)
