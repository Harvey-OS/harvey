...
{
	return ((uint)bits_per_pixel > 64 ? (const gx_device_memory *)0 : mem_devices[bits_per_pixel]);
}
		    ...
		    bool page_uses_transparency)
{
	 ...
	 * processing the file.
	 */
		max_height = size / (bitmap_raster(width * dev->color_info.depth + ESTIMATED_PDF14_ROW_SPACE(width)) + sizeof(byte *) * max(dev->num_planes, 1));
		       ...
		       byte **line_ptrs, int setup_height)
{
	...
	byte *data =
	    (base ? (mdev->raster = raster, mdev->base = base) : (raster = mdev->raster, mdev->base));
	int pi;
