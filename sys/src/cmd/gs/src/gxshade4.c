                 ...
                 const shading_vertex_t* vb, const shading_vertex_t* vc)
{
	...
	if(INTERPATCH_PADDING) {
		...
		if(code >= 0)
			code =
			    mesh_padding(&pfs1, &vb->p, &vc->p, &vb->c, &vc->c);
		if(code >= 0)
			code =
			    mesh_padding(&pfs1, &vc->p, &va->p, &vc->c, &va->c);
	}
	if(code >= 0)
                               ...
                               gs_imager_state* pis)
{
	...
	code = mesh_init_fill_state(&state, (const gs_shading_mesh_t*)psh,
	                            rect_clip, dev, pis);
	if(code < 0)
		return code;
	...
	while((flag = shade_next_flag(&cs, num_bits)) >= 0) {
		...
		case 2:
			vb = vc;
		v2:
			if((code = Gt_next_vertex(state.pshm, &cs, &vc)) < 0)
				return code;
                               ...
                               gs_imager_state* pis)
{
	...
	shade_next_init(&cs, (const gs_shading_mesh_params_t*)&psh->params,
	                pis);
	vertex = (shading_vertex_t*)gs_alloc_byte_array(
	    pis->memory, per_row, sizeof(*vertex), "gs_shading_LfGt_render");
	if(vertex == 0)
	...
	while(!seofp(cs.s)) {
		...
		if(code < 0)
			goto out;
		for(i = 1; i < per_row; ++i) {
			code = Gt_fill_triangle(&state, &vertex[i - 1],
			                        &vertex[i], &next);
			if(code < 0)
				goto out;
			vertex[i - 1] = next;
			...
			if(code < 0)
				goto out;
			code = Gt_fill_triangle(&state, &vertex[i],
			                        &vertex[i - 1], &next);
			if(code < 0)
				goto out;
		}
