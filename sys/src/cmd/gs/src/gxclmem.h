...
typedef struct MEMFILE {
	...
	bool ok_to_compress;      /* if true, OK to compress this file */
	                           ...
	                           */
	PHYS_MEMFILE_BLK*
	    reservePhysBlockChain; /* chain of reserve phys blks */
...
#define private_st_MEMFILE() /* in gxclmem.c */                                \
	gs_private_st_ptrs2(st_MEMFILE, MEMFILE, "MEMFILE", MEMFILE_enum_ptrs, \
	                    MEMFILE_reloc_ptrs, compress_state,                \
	                    decompress_state)
...
#define memfile_fclose(cf, fname, delete) clist_fclose(cf, fname, delete)
#define memfile_unlink(fname) clist_unlink(fname)

#define memfile_space_available(req) clist_space_available(req)
#define memfile_fwrite_chars(data, len, cf) clist_fwrite_chars(data, len, cf)

#define memfile_fread_chars(data, len, cf) clist_fread_chars(data, len, cf)

#define memfile_set_memory_warning(cf, nbytes)                                 \
	clist_set_memory_warning(cf, nbytes)
...
#define memfile_fseek(cf, offset, mode, fname)                                 \
	clist_fseek(cf, offset, mode, fname)
