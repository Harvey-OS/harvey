...
wts_cell_calc_h(double inc, int* px1, int* pxwidth, double* pp1, double memw)
{
	...
	e1 = 1e5;
	e2 = 1e5;
	for(uacc = (int)ceil(minrep * inc); uacc <= floor(2 * minrep * inc);
	    uacc++) {
		int mt;
		double et;
...
wts_print_j(const gx_wts_cell_params_j_t* wcpj)
{
	...
	dlprintf6("d: %f, %f; a: %f, %f; err: %g, %g\n", wcpj->base.ufast,
	          wcpj->base.vfast, wcpj->ufast_a, wcpj->vfast_a,
	          wcpj->base.ufast - uf, wcpj->base.vfast - vf);
	dlprintf6("d: %f, %f; a: %f, %f; err: %g, %g\n", wcpj->base.uslow,
	          wcpj->base.vslow, wcpj->uslow_a, wcpj->vslow_a,
...
private
double
wts_set_scr_jxi_try(gx_wts_cell_params_j_t* wcpj, int m, double qb, double jmem)
{
	...
	wts_vec_set(&a, (int)floor(uf * m + 0.5), (int)floor(vf * m + 0.5), 1,
	            0);
	if(a.u == 0 && a.v == 0)
		return qb + 1;
	...
	jumpok = (wcpj->xa == 0 && wcpj->ya == 0) ||
	         (wcpj->xb == 0 && wcpj->yb == 0) || (a.k != 0 && b.k != 0);
	...
	qx = 0.5 *
	     (wts_qart(ra, rbase, pa, pbase) + wts_qart(rb, rbase, pb, pbase));
	if(qx < 1.0 / 4000.0)
		qx *= 0.25;
	else
		qx -= 0.75 / 4000.0;
	...
	for(i = 1; qxl + m * i * jmem < qbi; i++) {
		...
		if(q < qbi && jumpok) {
#ifdef VERBOSE
			dlprintf7("m = %d, n = %d, q = %d, qx = %d, qy = %d, "
			          "qm = %d, qw = %d\n",
			          m, i, (int)(q * 1e6), (int)(qx * 1e6),
			          (int)(qy * 1e6), (int)(qm * 1e6),
			          (int)(qw * 1e6));
#endif
			qbi = q;
			wcpj->base.width = m;
...
wts_pick_cell_size(gs_screen_halftone* ph, const gs_matrix* pmat)
{
	...
	if(fabs(octangle) < 1e-4)
		result =
		    wts_pick_cell_size_h(sratiox, sratioy, sangledeg, memw);
	else
		result =
		    wts_pick_cell_size_j(sratiox, sratioy, sangledeg, memw);
...
private
int
gs_wts_screen_enum_j_currentpoint(gs_wts_screen_enum_t* self, gs_point* ppt)
{
...
private
int
gs_wts_screen_enum_h_currentpoint(gs_wts_screen_enum_t* self, gs_point* ppt)
{
