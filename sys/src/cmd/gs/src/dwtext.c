...
{
	MessageBox((HWND)NULL, message, (LPSTR)NULL,
...
text_to_cursor(TW* tw)
{
	...
	if((cyCursor + tw->CharSize.y > tw->ScrollPos.y + tw->ClientSize.y)
	   ...
	   (cyCursor < tw->ScrollPos.y + tw->ClientSize.y)) {
		nYinc = max(0, cyCursor + tw->CharSize.y - tw->ClientSize.y) -
		        tw->ScrollPos.y;
	...
	if((cxCursor + tw->CharSize.x > tw->ScrollPos.x + tw->ClientSize.x) ||
	   (cxCursor < tw->ScrollPos.x)) {
		nXinc =
		    max(0, cxCursor + tw->CharSize.x - tw->ClientSize.x / 2) -
		    tw->ScrollPos.x;
...
int
text_getpos(TW* tw, int* px, int* py, int* pcx, int* pcy)
{
	*px = tw->x;
	*py = tw->y;
	*pcx = tw->cx;
...
text_register_class(TW* tw, HICON hicon)
{
	...
	wndclass.hIcon =
	    tw->hIcon ? tw->hIcon : LoadIcon(NULL, IDI_APPLICATION);
...
text_create(TW* tw, const char* app_name, int show_cmd)
{
	...
	tw->hwnd =
	    CreateWindow(TextWinClassName, tw->Title,
	                 WS_OVERLAPPEDWINDOW | WS_VSCROLL | WS_HSCROLL, tw->x,
	                 tw->y, tw->cx, tw->cy, NULL, NULL, hInstance, tw);

	if(tw->hwnd == NULL) {
		MessageBox((HWND)NULL, "Couldn't open text window", (LPSTR)NULL,
		           MB_ICONHAND | MB_SYSTEMMODAL);
		return 1;
	}
...
text_write_buf(TW* tw, const char* str, int cnt)
{
	...
	while(cnt > 0) {
		p = tw->ScreenBuffer + tw->CursorPos.y * tw->ScreenSize.x +
		    tw->CursorPos.x;
		limit = tw->ScreenSize.x - tw->CursorPos.x;
		for(count = 0;
		    (count < limit) && (cnt > 0) &&
		        (isprint((unsigned char)(*str)) || *str == '\t');
		    count++) {
			if(*str == '\t') {
				for(n = 8 - ((tw->CursorPos.x + count) % 8);
				    (count < limit) & (n > 0); n--, count++)
					*p++ = ' ';
				str++;
				count--;
			} else {
...
WndTextProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_SIZE:
		...
		tw->ScrollMax.y = max(0, tw->CharSize.y * tw->ScreenSize.y -
		                             tw->ClientSize.y);
		...
		tw->ScrollMax.x = max(0, tw->CharSize.x * tw->ScreenSize.x -
		                             tw->ClientSize.x);
		...
		if(tw->bFocus && tw->bGetCh) {
			SetCaretPos(
			    tw->CursorPos.x * tw->CharSize.x - tw->ScrollPos.x,
			    tw->CursorPos.y * tw->CharSize.y + tw->CharAscent -
			        tw->CaretHeight - tw->ScrollPos.y);
			ShowCaret(hwnd);
		}
	...
	case WM_VSCROLL:
		...
		if((nYinc = max(
		        -tw->ScrollPos.y,
		        min(nYinc, tw->ScrollMax.y - tw->ScrollPos.y))) != 0) {
			tw->ScrollPos.y += nYinc;
	...
	case WM_HSCROLL:
		...
		if((nXinc = max(
		        -tw->ScrollPos.x,
		        min(nXinc, tw->ScrollMax.x - tw->ScrollPos.x))) != 0) {
			tw->ScrollPos.x += nXinc;
	...
	case WM_KEYDOWN:
		...
		break;
	case WM_CHAR: { /* store key in circular buffer */
	...
	case WM_PAINT: {
		...
		source.x =
		    (rect.left + tw->ScrollPos.x) / tw->CharSize.x; /* source */
		source.y = (rect.top + tw->ScrollPos.y) / tw->CharSize.y;
		dest.x = source.x * tw->CharSize.x -
		         tw->ScrollPos.x; /* destination */
		dest.y = source.y * tw->CharSize.y - tw->ScrollPos.y;
		width.x = ((rect.right + tw->ScrollPos.x + tw->CharSize.x - 1) /
		           tw->CharSize.x) -
		          source.x; /* width */
		width.y =
		    ((rect.bottom + tw->ScrollPos.y + tw->CharSize.y - 1) /
		     tw->CharSize.y) -
		    source.y;
		if(source.x < 0)
			source.x = 0;
		...
		while(width.y > 0) {
			TextOut(hdc, dest.x, dest.y,
			        (LPSTR)(tw->ScreenBuffer +
			                source.y * tw->ScreenSize.x + source.x),
			        width.x);
	...
	case WM_CREATE: {
		...
		if((tw->DragPre != (LPSTR)NULL) &&
		   (tw->DragPost != (LPSTR)NULL))
	...
	case WM_CLOSE:
		...
		if(!tw->quitnow) {
			char title[256];
			int count =
			    GetWindowText(hwnd, title, sizeof(title) - 11);
