...
text_to_cursor(TW *tw)
{
	...
	if((cyCursor + tw->CharSize.y > tw->ScrollPos.y + tw->ClientSize.y)
	   ...
	   (cyCursor < tw->ScrollPos.y + tw->ClientSize.y)) {
		nYinc = max(0, cyCursor + tw->CharSize.y - tw->ClientSize.y) - tw->ScrollPos.y;
		nYinc = min(nYinc, tw->ScrollMax.y - tw->ScrollPos.y);
	}
	cxCursor = tw->CursorPos.x * tw->CharSize.x;
	if((cxCursor + tw->CharSize.x > tw->ScrollPos.x + tw->ClientSize.x) || (cxCursor < tw->ScrollPos.x)) {
		nXinc = max(0, cxCursor + tw->CharSize.x - tw->ClientSize.x / 2) - tw->ScrollPos.x;
...
text_update_text(TW *tw, int count)
{
	...
	TextOut(hdc, xpos, ypos,
		(LPSTR)(tw->ScreenBuffer + tw->CursorPos.y * tw->ScreenSize.x + tw->CursorPos.x), count);
...
text_getch(TW *tw)
{
	...
	if(tw->bFocus) {
		SetCaretPos(tw->CursorPos.x * tw->CharSize.x - tw->ScrollPos.x,
			    tw->CursorPos.y * tw->CharSize.y + tw->CharAscent - tw->CaretHeight - tw->ScrollPos.y);
		ShowCaret(tw->hwnd);
	}
