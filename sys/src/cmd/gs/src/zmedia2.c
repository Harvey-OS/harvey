...
private
int zmatch_page_size(const gs_memory_t* mem, const ref* pvreq, const ref* pvmed,
                     int policy, int orient, bool roll, float* best_mismatch,
                     gs_matrix* pmat, gs_point* pmsize);
...
zmatchmedia(i_ctx_t* i_ctx_p)
{
	...
	check_array(*pkeys);
	check_read(*pkeys);
	switch(code = dict_int_null_param(preq, "MediaPosition", 0, 0x7fff, 0,
	                                  &mepos)) {
	default:
		return code;
	case 2:
	...
	switch(code =
	           dict_int_null_param(preq, "Orientation", 0, 3, 0, &orient)) {
	default:
		return code;
	case 2:
	case 1:
		...
		if(r_has_type(&aelt.dict, t_dictionary) &&
		   ...
		   r_has_type(&aelt.key, t_integer)) {
			...
			for(ki = 0; ki < r_size(pkeys); ki++) {
				...
				/*
				 * Match a requested attribute value with the
				 * attribute value in the
				 * description of a medium.  For all attributes
				 * except PageSize,
				 * matching means equality.  PageSize is
				 * special; see match_page_size
				 * below.
				 */
				if(r_has_type(&key, t_name) &&
				   (name_string_ref(imemory, &key, &kstr),
				    r_size(&kstr) == 8 &&
				        !memcmp(kstr.value.bytes, "PageSize",
				                8))) {
					gs_matrix ignore_mat;
					gs_point ignore_msize;

					if(zmatch_page_size(
					       imemory, prvalue, pmvalue,
					       policy, orient, roll,
					       &best_mismatch, &ignore_mat,
					       &ignore_msize) <= 0)
						goto no;
			...
			/* We have a match. Save the match in case no better
			 * match is found */
			if(r_has_type(&match.match_key, t_null))
				match.match_key = aelt.key;
			/*
			 * If it is a better match than the current best it
			 * supersedes it
			 * regardless of priority. If the match is the same,
			 * then update
			...
			if(best_mismatch + mepos_penalty <= mbest) {
				if(best_mismatch + mepos_penalty < mbest ||
				   (r_has_type(&match.match_key, t_integer) &&
				    match.match_key.value.intval >
				        aelt.key.value.intval)) {
					reset_match(&match);
					match.match_key = aelt.key;
			...
			for(pi = match.priority; pi > 0;) {
				...
				if(obj_eq(imemory, &aelt.key,
				          &pri)) { /* Yes, higher priority. */
...
zmatchpagesize(i_ctx_t* i_ctx_p)
{
	...
	code =
	    zmatch_page_size(imemory, op - 5, op - 4, (int)op[-3].value.intval,
...
private
int match_page_size(const gs_point* request, const gs_rect* medium, int policy,
                    int orient, bool roll, float* best_mismatch,
                    gs_matrix* pmat, gs_point* pmsize);
private
int
                 ...
                 gs_matrix* pmat, gs_point* pmsize)
{
		...
		return_error(e_rangecheck);
	{
			...
			return code;
		{
			...
			medium.p.x = v[2], medium.p.y = v[3], medium.q.x = v[4],
			medium.q.y = v[5];
			return match_page_size(&request, &medium, policy,
			                       orient, roll, best_mismatch,
			                       pmat, pmsize);
		}
	}
}
 ...
 */
private
void make_adjustment_matrix(const gs_point* request, const gs_rect* medium,
                ...
                gs_point* pmsize)
{
	...
	if(policy == 7) {
		...
		*pmsize = *request;
		...
		int fit_direct =
		    rx - medium->p.x >= -5 && rx - medium->q.x <= 5 &&
		    ry - medium->p.y >= -5 && ry - medium->q.y <= 5;
		int fit_rotated =
		    rx - medium->p.y >= -5 && rx - medium->q.y <= 5 &&
		    ry - medium->p.x >= -5 && ry - medium->q.x <= 5;

		/* Fudge matches from a non-standard page size match (4 element
		 * array) */
		/* as worse than an exact match from a standard (2 element
		 * array), but */
		/* better than for a rotated match to a standard pagesize. This
		 * should */
		/* prevent rotation unless we have to (particularly for raster
		 * file    */
		/* formats like TIFF, JPEG, PNG, PCX, BMP, etc. and also should
		 * allow  */
		/* exact page size specification when there is a range PageSize
		 * entry. */
		/* As the comment in gs_setpd.ps says "Devices that care will
		 * provide  */
		/* a real InputAttributes dictionary (most without a range
		 * pagesize)   */
		if(fit_direct && fit_rotated) {
			make_adjustment_matrix(request, medium, pmat, false,
			                       orient < 0 ? 0 : orient);
			if(medium->p.x < medium->q.x ||
			   medium->p.y < medium->q.y)
				*best_mismatch =
				    (float)0.001; /* fudge a match to a range as
				                     a small number */
			else /* should be 0 for an exact match */
				*best_mismatch = fabs((rx - medium->p.x) *
				                      (medium->q.x - rx)) +
				                 fabs((ry - medium->p.y) *
				                      (medium->q.y - ry));
		} else if(fit_direct) {
			...
			make_adjustment_matrix(request, medium, pmat, false,
			                       (rotate + 1) & 2);
			*best_mismatch =
			    fabs((medium->p.x - rx) * (medium->q.x - rx)) +
			    fabs((medium->p.y - ry) * (medium->q.y - ry)) +
			    (pmat->xx == 0.0 || (rotate & 1) == 1
			         ? 0.01
			         : 0); /* rotated */
		} else if(fit_rotated) {
			...
			make_adjustment_matrix(request, medium, pmat, false,
			                       rotate | 1);
			*best_mismatch =
			    fabs((medium->p.y - rx) * (medium->q.y - rx)) +
			...
			int rotate =
			    (orient >= 0 ? orient : (rx < ry) ^ (medium->q.x <
			                                         medium->q.y));
			bool larger =
			    (rotate & 1
			         ? medium->q.y >= rx && medium->q.x >= ry
			...
			if(adjust)
				make_adjustment_matrix(request, medium, pmat,
				                       !larger, rotate);
			else {
				gs_rect req_rect;
				if(rotate & 1) {
				...
				make_adjustment_matrix(request, &req_rect, pmat,
				                       false, rotate);
			}
...
const op_def zmedia2_l2_op_defs[] = {op_def_begin_level2(),
