...
private
int zmatch_page_size(const gs_memory_t *mem,
...
zmatchmedia(i_ctx_t *i_ctx_p)
{
	 ...
	 */
			...
			mepos_penalty = (mepos < 0 || aelt.key.value.intval == mepos) ? 0 : .001;
 ...
 */
private
void make_adjustment_matrix(const gs_point *request,
			    const gs_rect *medium,
			    gs_matrix *pmat,
		...
		gs_point *pmsize)
{
	...
	if(policy == 7) {
		...
		*pmsize = *request;
		...
		int fit_direct = rx - medium->p.x >= -5 && rx - medium->q.x <= 5 && ry - medium->p.y >= -5 && ry - medium->q.y <= 5;
		int fit_rotated = rx - medium->p.y >= -5 && rx - medium->q.y <= 5 && ry - medium->p.x >= -5 && ry - medium->q.x <= 5;
		...
		if(fit_direct && fit_rotated) {
			...
			*best_mismatch = fabs((medium->p.y - rx) * (medium->q.y - rx)) +
			...
			int rotate =
			    (orient >= 0 ? orient : (rx < ry) ^ (medium->q.x < medium->q.y));
			bool larger =
			    (rotate & 1 ? medium->q.y >= rx && medium->q.x >= ry : medium->q.x >= rx && medium->q.y >= ry);
			bool adjust = false;
