...
private
dev_proc_print_page(upd_print_page); /** print a page (required) */

private
dev_proc_open_device(upd_open); /** device-initialization (opt) */
private
dev_proc_close_device(upd_close); /** device-release (opt) */

private
dev_proc_get_params(upd_get_params); /** export parameters (opt) */
private
dev_proc_put_params(upd_put_params); /** import parameters (opt) */
...
private
dev_proc_encode_color(upd_rgb_1color); /** Gray-Gray->Index */
private
dev_proc_decode_color(upd_1color_rgb); /** Gray-Index->Gray */

private
dev_proc_encode_color(upd_rgb_3color); /** RGB->RGB-Index */
private
dev_proc_decode_color(upd_3color_rgb); /** RGB-Index->RGB */
...
private
dev_proc_encode_color(upd_rgb_4color); /** RGB->WRGB-Index */
private
dev_proc_decode_color(upd_4color_rgb); /** WRGB-Index->RGB */
...
private
dev_proc_encode_color(upd_cmyk_icolor); /** KCMY->KCMY-Index */
private
dev_proc_decode_color(upd_icolor_rgb); /** KCMY->RGB-Index */
...
private
dev_proc_encode_color(upd_cmyk_kcolor); /** adds black generation */
private
dev_proc_decode_color(upd_kcolor_rgb); /** watches black-gen */
...
private
dev_proc_encode_color(upd_rgb_ovcolor); /** RGB->CMYK-Index */
...
private
dev_proc_encode_color(upd_rgb_novcolor); /** RGB->CMYK-Index */
...
private
int upd_procs_map(upd_device* udev);
...
upd_device far_data gs_uniprint_device =
    {
     /** */
     prn_device_body(
         upd_device, upd_procs, /** The Type and Procedures */
         ...
         0, /**  color_info.max_color     # of distinct color levels -1
               (255/1/0)*/
         2, /**  color_info.dither_grays  size of gray ramp for dithering
               (256/2) */
...
static const char* const upd_mapper[] = {
    "upColorModel",
...
static const char* const upd_render[] = {
    "upRendering",
...
static const char* const upd_format[] = {
    "upOutputFormat",
...
private
int upd_open_map(upd_device* udev);
private
int upd_close_map(upd_device* udev);
...
*/
private
inline uint32_t upd_truncate(upd_pc, int, gx_color_value);

/* ------------------------------------------------------------------- */
/* Return the gx_color_value for a given component                     */
/* ------------------------------------------------------------------- */
private
inline gx_color_value
upd_expand(upd_pc upd, int i, gx_color_index ci0)
{
	const updcmap_pc cmap = upd->cmap + i; /* Writing-Shortcut */
	uint32_t ci = (uint32_t)((ci0 >> cmap->bitshf) &
	                         cmap->bitmsk); /* Extract the component */

	if(!cmap->rise)
		ci = cmap->bitmsk - ci; /* Invert, if necessary */
	/* no Truncation/Expansion on full range */
	if(gx_color_value_bits > cmap->bits)
		return cmap->code[ci];
	else
		return (gx_color_value)ci;
}
...
*/
private
void upd_open_render(upd_device* udev);
private
void upd_close_render(upd_device* udev);

private
void upd_open_fscomp(upd_device* udev);
private
int upd_fscomp(upd_p upd);
private
void upd_close_fscomp(upd_device* udev);

private
void upd_open_fscmyk(upd_device* udev);
private
int upd_fscmyk(upd_p upd);

private
void upd_open_fscmy_k(upd_device* udev);
private
int upd_fscmy_k(upd_p upd);
...
*/
private
int upd_open_writer(upd_device* udev);
private
void upd_close_writer(upd_device* udev);
#if UPD_SIGNAL
private
void upd_signal_handler(int sig);
#endif
...
*/
private
int upd_open_rascomp(upd_device* udev);
private
int upd_start_rascomp(upd_p upd, FILE* out);
private
int upd_rascomp(upd_p upd, FILE* out);
...
private
void upd_limits(upd_p upd, bool check);
private
int upd_open_wrtescp(upd_device* udev);
private
int upd_wrtescp(upd_p upd, FILE* out);
...
*/
private
int upd_rle(byte* out, const byte* in, int nbytes);
private
int upd_open_wrtescp2(upd_device* udev);
private
int upd_wrtescp2(upd_p upd, FILE* out);
private
int upd_wrtescp2x(upd_p upd, FILE* out);
...
private
int upd_open_wrtrtl(upd_device* udev);
private
int upd_wrtrtl(upd_p upd, FILE* out);
...
private
int upd_open_wrtcanon(upd_device* udev);
private
int upd_wrtcanon(upd_p upd, FILE* out);

/**
The seventh writer is for ESC P/2 Nozzle Map Mode (currently Stylus Color 300)
(GR)
*/

private
int upd_wrtescnm(upd_p upd, FILE* out);

/**
Generalized Pixel Get & Read
*/
private
uint32_t upd_pxlfwd(upd_p upd);
private
uint32_t upd_pxlrev(upd_p upd);
#define upd_pxlget(UPD) (*UPD->pxlget)(UPD)

private
void* upd_cast(const void*);
...
#define UPD_MM_GET_ARRAY(mem, Which, Nelts)                                    \
	Which = NULL;                                                          \
	if(0 < (Nelts)) {                                                      \
		byte* tmp = gs_malloc(mem, Nelts, sizeof(Which[0]),            \
		                      "uniprint/params");                      \
		if(tmp) {                                                      \
...
#define UPD_MM_DEL_ARRAY(mem, Which, Nelts, Delete)                            \
	if(Which && 0 < (Nelts)) {                                             \
		uint ii;                                                       \
		for(ii = 0; (Nelts) > ii; ++ii)                                \
			Delete(mem, Which[ii]);                                \
		gs_free(mem, upd_cast(Which), Nelts, sizeof(Which[0]),         \
		        "uniprint/params");                                    \
	}                                                                      \
	Which = 0
...
#define UPD_MM_DEL_PARAM(mem, Which)                                           \
	{                                                                      \
		if(Which.data && Which.size)                                   \
			gs_free(mem, upd_cast(Which.data), Which.size,         \
			        sizeof(Which.data[0]), "uniprint/params");     \
	}
...
#define UPD_MM_CPY_PARAM(mem, To, From)                                        \
	if(From.data && From.size) {                                           \
		...
		if(To.data) {                                                  \
			To.size = From.size;                                   \
			memcpy(upd_cast(To.data), From.data,                   \
			       To.size * sizeof(To.data[0]));                  \
		}                                                              \
	}
...
#define UPD_MM_CPY_APARAM(mem, To, From)                                       \
	if(From.data && From.size) {                                           \
		UPD_MM_GET_ARRAY(mem, To.data, From.size);                     \
		if(To.data) {                                                  \
			gs_param_string* tmp2 =                                \
			    (gs_param_string*)upd_cast(To.data);               \
			uint iii;                                              \
			To.size = From.size;                                   \
			for(iii = 0; To.size > iii; ++iii)                     \
				UPD_MM_CPY_PARAM(mem, tmp2[iii],               \
				                 From.data[iii]);              \
		}                                                              \
	}
...
upd_print_page(gx_device_printer* pdev, FILE* out)
{
	...
	if(!(upd->flags & B_OPEN)) {
		...
		if(0 < upd->strings[S_OPEN].size)
			fwrite(upd->strings[S_OPEN].data, 1,
			       upd->strings[S_OPEN].size, out);
		upd->flags |= B_OPEN;
	}
	/*
	...
	if(0 < upd->strings[S_BEGIN].size)
		fwrite(upd->strings[S_BEGIN].data, 1,
		       upd->strings[S_BEGIN].size, out);
	/*
	...
	if(upd->start_render)
		(*upd->start_render)(upd);
	if(upd->start_writer)
		(*upd->start_writer)(upd, out);
	...
	while(upd->pheight > upd->yscan) { /* Main-Loop */
		 ...
		 *    (this is done in scan-sequence, the printing not
		 * necessarily.)
		 */
		if(ints[I_BEGSKIP] > upd->yscan)
			yfill = 0;
		else
			yfill = upd->yscan - ints[I_BEGSKIP];

		for(yfill += upd->nscnbuf; upd->yscnbuf < yfill;
		    upd->yscnbuf++) {

			if(upd->gsheight > upd->yscnbuf) {

				if(0 > (*dev_proc(udev, get_bits))(
				           (gx_device*)udev, upd->yscnbuf,
				           upd->gsbuf, &upd->gsscan)) {
#if UPD_MESSAGES & UPD_M_WARNING
					errprintf("get_bits aborted with "
					          "error, yscnbuf = %4d\n",
					          upd->yscnbuf);
#endif
					break;
				...
				memset(upd->gsscan = upd->gsbuf, 0,
				       upd->ngsbuf);
			}

			if(0 > (*upd->render)(upd)) {
#if UPD_MESSAGES & UPD_M_WARNING
				errprintf("Rendering aborted with error, "
				          "yscnbuf = %4d\n",
				          upd->yscnbuf);
#endif
				break;
		 ...
		 */
		if((upd->yscnbuf ^ yfill) & upd->scnmsk)
			break;
		/*
		...
		while((upd->yscan - ints[I_BEGSKIP] + need) < upd->yscnbuf) {
			...
			if(upd->yscan >= upd->pheight)
				break;
			if(upd->flags & B_ABORT) {
#if UPD_MESSAGES & UPD_M_WARNING
				errprintf("Printing aborted upon interrupt, "
				          "yscan = %4d\n",
				          upd->yscan);
#endif
				break;
		 ...
		 */
		if((upd->yscan - ints[I_BEGSKIP] + need) < upd->yscnbuf)
			break;
	} /* Main-Loop */
	...
	if((upd->pheight > upd->yscan) &&
	   (0 < upd->strings[S_ABORT].size)) { /* Only This! */
		fwrite(upd->strings[S_ABORT].data, 1,
		       upd->strings[S_ABORT].size, out);
		...
		fwrite(upd->strings[S_END].data, 1, upd->strings[S_END].size,
		       out);
	}
	/*
...
upd_open(gx_device* pdev)
{
	...
	if((NULL != upd) && (NULL != upd->float_a[FA_MARGINS].data) &&
	...
	if(upd) {
		...
		/**
		The following initializations are run, even in case of an error
		in
		...
		*/
		...
		if(gs_error_VMerror == upd_open_map(udev))
		...
		*/
		upd->gswidth = udev->width -
		               (int)((dev_l_margin(udev) + dev_r_margin(udev)) *
		                     udev->x_pixels_per_inch);

		upd->gsheight =
		    udev->height -
		    (int)((dev_t_margin(udev) + dev_b_margin(udev)) *
		...
		if(B_MAP & upd->flags) { /* Only if prerequisites were met */
			...
			upd->gsbuf =
			    gs_malloc(pdev->memory, want, 1, "upd/gsbuf");
		...
		if(gs_error_VMerror == upd_open_writer(udev))
			error = gs_error_VMerror;
		...
		errprintf("\n%sready to print\n\n",
		          B_OK4GO != (upd->flags & (B_OK4GO | B_ERROR)) ? "NOT "
		                                                        : "");
#endif
	}
...
upd_close(gx_device* pdev)
{
	...
	if(upd) {
		...
		if(upd->gsbuf)
			gs_free(pdev->memory, upd->gsbuf, upd->ngsbuf, 1,
			        "uniprint/gsbuf");
		upd->gsbuf = NULL;
		upd->ngsbuf = 0;
		upd->flags &= ~B_BUF;
		...
		UPD_MM_DEL_ARRAY(pdev->memory, upd->choice, countof(upd_choice),
		                 UPD_MM_DEL_VALUE);
		UPD_MM_DEL_ARRAY(pdev->memory, upd->ints, countof(upd_ints),
		                 UPD_MM_DEL_VALUE);
		UPD_MM_DEL_ARRAY(pdev->memory, upd->int_a, countof(upd_int_a),
		                 UPD_MM_DEL_PARAM);
		UPD_MM_DEL_ARRAY(pdev->memory, upd->strings,
		                 countof(upd_strings), UPD_MM_DEL_PARAM);
		UPD_MM_DEL_ARRAY(pdev->memory, upd->string_a,
		                 countof(upd_string_a), UPD_MM_DEL_APARAM);
		UPD_MM_DEL_ARRAY(pdev->memory, upd->float_a,
		                 countof(upd_float_a), UPD_MM_DEL_PARAM);
...
upd_get_params(gx_device* pdev, gs_param_list* plist)
{
	...
	for(i = 0; i < countof(upd_choice); ++i) {
		if(!upd_choice[i])
			continue; /* Choice-Export disabled */
		if(upd && upd->choice && upd->choice[i]) {
			...
			error =
			    param_write_name(plist, upd_choice[i][0], &name);
		} else {
	...
	for(i = 0; i < countof(upd_int_a); ++i) {
		if(!upd_int_a[i])
			continue; /* int-Array-Export disabled */
		if(upd && upd->int_a && upd->int_a[i].size) {
			error = param_write_int_array(plist, upd_int_a[i],
			                              (upd->int_a + i));
		} else {
	...
	for(i = 0; i < countof(upd_strings); ++i) {
		if(!upd_strings[i])
			continue; /* String-Export disabled */
		if(upd && upd->strings && upd->strings[i].size) {
			error = param_write_string(plist, upd_strings[i],
			                           (upd->strings + i));
		} else {
	...
	for(i = 0; i < countof(upd_string_a); ++i) {
		if(!upd_string_a[i])
			continue; /* String-Array-Export disabled */
		if(upd && upd->string_a && upd->string_a[i].size) {
			error = param_write_string_array(plist, upd_string_a[i],
			                                 (upd->string_a + i));
		} else {
	...
	for(i = 0; i < countof(upd_float_a); ++i) {
		if(!upd_float_a[i])
			continue; /* Float-Array-Export disabled */
		if(upd && upd->float_a && upd->float_a[i].size) {
			error = param_write_float_array(plist, upd_float_a[i],
			                                (upd->float_a + i));
		} else {
...
#define UPD_PARAM_READ(Param_read, Name, Object)                               \
	...
	if(!code)                                                              \
		errprintf("upd_put_params: retrieved parameter \"%s\"\n",      \
		          Name);                                               \
	if(0 > code) {                                                         \
...
#define UPD_PARAM_READ(Param_read, Name, Object)                               \
	...
	*/
	...
	if(upd) {
		flags = upd->flags;
		UPD_MM_CPY_ARRAY(udev->memory, choice, upd->choice,
		                 countof(upd_choice), UPD_MM_CPY_VALUE);
		UPD_MM_CPY_ARRAY(udev->memory, ints, upd->ints,
		                 countof(upd_ints), UPD_MM_CPY_VALUE);
		UPD_MM_CPY_ARRAY(udev->memory, int_a, upd->int_a,
		                 countof(upd_int_a), UPD_MM_CPY_PARAM);
		UPD_MM_CPY_ARRAY(udev->memory, strings, upd->strings,
		                 countof(upd_strings), UPD_MM_CPY_PARAM);
		UPD_MM_CPY_ARRAY(udev->memory, string_a, upd->string_a,
		                 countof(upd_string_a), UPD_MM_CPY_APARAM);
		UPD_MM_CPY_ARRAY(udev->memory, float_a, upd->float_a,
		                 countof(upd_float_a), UPD_MM_CPY_PARAM);
	} else {
		flags = 0;
	...
	for(i = 0; countof(upd_choice) > i; ++i) {
		...
		if(0 == code) {
			if(0 <= error)
				error |= UPD_PUT_CHOICE;
			choice[i] = 0;
			if(0 < value.size) {
				int j;
				for(j = 1; upd_choice[i][j]; ++j) {
					if((strlen(upd_choice[i][j]) ==
					    value.size) &&
					   (0 ==
					    strncmp(upd_choice[i][j],
	...
	for(i = 0; countof(upd_strings) > i; ++i) {
		...
		if(0 == code) {
			if(0 <= error)
				error |= UPD_PUT_STRINGS;
			UPD_MM_DEL_PARAM(udev->memory, strings[i]);
			if(!value.size) {
				value.data = NULL;
				strings[i] = value;
			} else {
				UPD_MM_CPY_PARAM(udev->memory, strings[i],
				                 value);
			}
		}
	}
	...
	for(i = 0; countof(upd_string_a) > i; ++i) {
		...
		if(0 == code) {
			if(0 <= error)
				error |= UPD_PUT_STRING_A;
			UPD_MM_DEL_APARAM(udev->memory, string_a[i]);
			if(!value.size) {
				value.data = NULL;
				string_a[i] = value;
			} else {
				UPD_MM_CPY_APARAM(udev->memory, string_a[i],
				                  value);
			}
		}
	}
	...
	for(i = 0; countof(upd_float_a) > i; ++i) {
		...
		if(0 == code) {
			if(0 <= error)
				error |= UPD_PUT_FLOAT_A;
			UPD_MM_DEL_PARAM(udev->memory, float_a[i]);
			if(!value.size) {
				value.data = NULL;
				float_a[i] = value;
			} else {
				UPD_MM_CPY_PARAM(udev->memory, float_a[i],
				                 value);
			}
		}
	}
	...
	*/
	if(0 < error) {
		...
		if(6 > int_a[IA_COLOR_INFO].size) {
			...
			UPD_MM_GET_ARRAY(udev->memory,
			                 int_a[IA_COLOR_INFO].data, 6);
		...
		if(0 == ip[0]) { /* Try to obtain num_components */
			...
			case MAP_RGBNOV:
				...
				break;
			default:
				ip[0] = color_info.num_components;
				break;
			}
		...
		if(UPD_CMAP_MAX < ncomp)
			ncomp = UPD_CMAP_MAX;

		if(ncomp >
		   int_a[IA_COMPBITS].size) { /* Default ComponentBits */
		...
		if(ncomp >
		   int_a[IA_COMPSHIFT].size) { /* Default ComponentShift */
			nbits = 0;
			for(i = 0; i < ncomp; ++i)
		...
		if((0 == param_read_float_array(plist, ".HWMargins", &mfa)) &&
		   (4 == mfa.size) && (0 != mfa.data)) {
			udev->Margins[0] =
			    -mfa.data[0] * udev->MarginsHWResolution[0] / 72.0;
			udev->Margins[1] =
			    -mfa.data[3] * udev->MarginsHWResolution[1] / 72.0;
		}
	...
	*/
	...
	if(0 < error) { /* Actually something loaded without error */
		...
		if(!(upd = udev->upd)) {
			...
			UPD_MM_DEL_ARRAY(udev->memory, upd->choice,
			                 countof(upd_choice), UPD_MM_DEL_VALUE);
			UPD_MM_DEL_ARRAY(udev->memory, upd->ints,
			                 countof(upd_ints), UPD_MM_DEL_VALUE);
			UPD_MM_DEL_ARRAY(udev->memory, upd->int_a,
			                 countof(upd_int_a), UPD_MM_DEL_PARAM);
			UPD_MM_DEL_ARRAY(udev->memory, upd->strings,
			                 countof(upd_strings),
			                 UPD_MM_DEL_PARAM);
			UPD_MM_DEL_ARRAY(udev->memory, upd->string_a,
			                 countof(upd_string_a),
			                 UPD_MM_DEL_APARAM);
			UPD_MM_DEL_ARRAY(udev->memory, upd->float_a,
			                 countof(upd_float_a),
			                 UPD_MM_DEL_PARAM);
		}
		...
		UPD_MM_DEL_ARRAY(udev->memory, choice, countof(upd_choice),
		                 UPD_MM_DEL_VALUE);
		UPD_MM_DEL_ARRAY(udev->memory, ints, countof(upd_ints),
		                 UPD_MM_DEL_VALUE);
		UPD_MM_DEL_ARRAY(udev->memory, int_a, countof(upd_int_a),
		                 UPD_MM_DEL_PARAM);
		UPD_MM_DEL_ARRAY(udev->memory, strings, countof(upd_strings),
		                 UPD_MM_DEL_PARAM);
		UPD_MM_DEL_ARRAY(udev->memory, string_a, countof(upd_string_a),
		                 UPD_MM_DEL_APARAM);
		UPD_MM_DEL_ARRAY(udev->memory, float_a, countof(upd_float_a),
		                 UPD_MM_DEL_PARAM);
	}
...
upd_cmyk_icolor(gx_device* pdev, const gx_color_value cv[])
{
	...
	*/
	...
	if((c == m) && (m == y)) {
		...
		/* It might still become a "gx_no_color_value" due to
		 * truncation, thus: */
	...
	errprintf(
	    "cmyk_icolor: (%5.1f,%5.1f,%5.1f,%5.1f) : "
	    "(%5.1f,%5.1f,%5.1f,%5.1f) : 0x%0*lx\n",
	    ...
	    255.0 *
	        (double)((rv >> upd->cmap[1].bitshf) & upd->cmap[1].bitmsk) /
	        (double)upd->cmap[1].bitmsk,
	    255.0 *
	        (double)((rv >> upd->cmap[2].bitshf) & upd->cmap[2].bitmsk) /
	        (double)upd->cmap[2].bitmsk,
	    255.0 *
	        (double)((rv >> upd->cmap[3].bitshf) & upd->cmap[3].bitmsk) /
	        (double)upd->cmap[3].bitmsk,
	    255.0 *
	        (double)((rv >> upd->cmap[0].bitshf) & upd->cmap[0].bitmsk) /
...
upd_icolor_rgb(gx_device* pdev, gx_color_index color, gx_color_value prgb[3])
{
	...
	errprintf(
	    ...
	    255.0 *
	        (double)((color >> upd->cmap[1].bitshf) & upd->cmap[1].bitmsk) /
	        (double)upd->cmap[1].bitmsk,
	    255.0 *
	        (double)((color >> upd->cmap[2].bitshf) & upd->cmap[2].bitmsk) /
	        (double)upd->cmap[2].bitmsk,
	    255.0 *
	        (double)((color >> upd->cmap[3].bitshf) & upd->cmap[3].bitmsk) /
	        (double)upd->cmap[3].bitmsk,
	    255.0 *
	        (double)((color >> upd->cmap[0].bitshf) & upd->cmap[0].bitmsk) /
	        (double)upd->cmap[0].bitmsk,
	    255.0 * (double)c / (double)gx_max_color_value,
	    255.0 * (double)m / (double)gx_max_color_value,
	    255.0 * (double)y / (double)gx_max_color_value,
...
#if UPD_MESSAGES & UPD_M_MAPCALLS
	errprintf("rgb_1color: (%5.1f) : (%5.1f) : 0x%0*lx\n",
...
upd_rgb_3color(gx_device* pdev, const gx_color_value cv[])
{
	...
	rv = upd_truncate(upd, 0, r) | upd_truncate(upd, 1, g) |
...
#if UPD_MESSAGES & UPD_M_MAPCALLS
	errprintf(
	    "cmyk_kcolor: (%5.1f,%5.1f,%5.1f,%5.1f) : "
	    "(%5.1f,%5.1f,%5.1f,%5.1f) : 0x%0*lx\n",
	    255.0 * (double)c / (double)gx_max_color_value,
	    255.0 * (double)m / (double)gx_max_color_value,
	    255.0 * (double)y / (double)gx_max_color_value,
...
upd_truncate(upd_pc upd, int i, gx_color_value v)
{
	...
	if(!cmap->rise)
		v = cmap->bitmsk - v; /* Again reverse, if necessary */
	...
	if(imap) {
		...
		default:
			...
			if(upd_choice[C_MAPPER][0])
				errprintf("upd_open_map: unsupported %s=%d\n",
				          upd_choice[C_MAPPER][0],
				          upd->choice[C_MAPPER]);
			else
				errprintf(
			...
			if((0 > upd->int_a[IA_COMPBITS].data[imap]) ||
			   ...
			   (upd->int_a[IA_COMPBITS].data[imap] >
...
#if UPD_MESSAGES & UPD_M_WARNING
				errprintf("upd_open_map: %d Bits << %d is "
				          "illegal for %d. Component\n",
				          upd->int_a[IA_COMPBITS].data[imap],
				          upd->int_a[IA_COMPSHIFT].data[imap],
				          imap + 1);
#endif
				...
				if((NULL ==
				   ...
				   (2 >
				    upd->float_a[upd->cmap[imap].xfer].size)) {
					float* fp;
					UPD_MM_DEL_PARAM(
					    udev->memory,
					    upd->float_a[upd->cmap[imap].xfer]);
				...
				if(0 <= n) {
#if UPD_MESSAGES & UPD_M_WARNING
					errprintf("upd_open_map: %d. Component "
					          "has non monoton Xfer\n",
					          imap + 1);
#endif
					success = false;
		...
		for(imap = 0; UPD_CMAP_MAX > imap; ++imap) {
			...
			upd->cmap[imap].rise =
			    upd->float_a[upd->cmap[imap].xfer].data[0] <
			            upd->float_a[upd->cmap[imap].xfer]
			                .data[upd->float_a[upd->cmap[imap].xfer]
			                          .size -
			                      1]
			        ? true
			        : false;
			upd->cmap[imap].code = gs_malloc(
			    udev->memory, upd->cmap[imap].bitmsk + 1,
...
#define XFVAL(I) ((upd->float_a[cmap->xfer].data[I] - offset) * scale)
			...
			scale = upd->float_a[cmap->xfer]
			            .data[upd->float_a[cmap->xfer].size - 1];
			if(0.0 > scale)
				scale = 0.0;
			else if(1.0 < scale)
				scale = 1.0;
			...
			xstep = (float)1.0 /
			        (float)(upd->float_a[cmap->xfer].size - 1);
			...
			for(ly = 0; ly <= cmap->bitmsk; ++ly) {
				...
				fx = fx < 0.0 ? 0.0 : (fx > gx_max_color_value
				                           ? gx_max_color_value
				                           : fx);

				cmap->code[ly] = (gx_color_value)fx;
				if((fx - cmap->code[ly]) >= 0.5)
					cmap->code[ly] += 1;
			}
...
#undef XFVAL
	...
	if(imap) {
		...
		default:
			imap = 0;
#if UPD_MESSAGES & UPD_M_WARNING
			errprintf("upd_open: Mapping %d unknown\n",
			          upd->choice[C_MAPPER]);
#endif
...
upd_procs_map(upd_device* udev)
{
	...
	if(udev->upd && (udev->upd->flags & B_MAP))
		imap = udev->upd->choice[C_MAPPER];
	else
		imap = 0;
...
upd_close_map(upd_device* udev)
{
	...
	if(upd) {
		...
		for(imap = 0; UPD_CMAP_MAX > imap; ++imap) {
			...
			if(upd->cmap[imap].code)
				gs_free(udev->memory, upd->cmap[imap].code,
				        sizeof(upd->cmap[imap].code[0]),
...
upd_open_render(upd_device* udev)
{
	...
	if((B_BUF | B_MAP) == ((B_BUF | B_MAP | B_ERROR) & upd->flags)) {
...
#if UPD_MESSAGES & UPD_M_WARNING
			errprintf(
			    "upd_open_render: Unknown rendering type %d\n",
			    upd->choice[C_RENDER]);
#endif
			break;
		}
...
upd_close_render(upd_device* udev)
{
	...
	if(upd) {
		...
		if((upd->render == upd_fscomp) || (upd->render == upd_fscmyk))
			upd_close_fscomp(udev);

		if((0 < upd->nvalbuf) && upd->valbuf)
			gs_free(udev->memory, upd->valbuf, upd->nvalbuf,
			        sizeof(upd->valbuf[0]), "upd/valbuf");
		upd->valbuf = NULL;
		upd->nvalbuf = 0;
...
upd_open_fscomp(upd_device* udev)
{
	...
	if((0 >= icomp) || (UPD_VALPTR_MAX < icomp) || (UPD_CMAP_MAX < icomp))
		icomp = 0;
	...
	*/
	if(icomp) {
		if(upd->ncomp <=
		   upd->int_a[IA_COMPORDER].size) { /* Reordering */
			bool success = true;
			for(icomp = 0; upd->ncomp > icomp; ++icomp) {
				order[icomp] =
				    upd->int_a[IA_COMPORDER].data[icomp];
				if((0 > order[icomp]) ||
				   (UPD_CMAP_MAX <= order[icomp])) {
					success = false;
#if UPD_MESSAGES & UPD_M_WARNING
					errprintf("upd_open_fscomp: %d is "
			...
			for(icomp = 0; UPD_CMAP_MAX > icomp; ++icomp)
				order[icomp] = icomp;
		} /* Ordering defined */
	}
	...
	*/
	if(icomp) {
		...
		for(icomp = 0; upd->ncomp > icomp; ++icomp) {
			upd->valptr[icomp] = gs_malloc(
			    udev->memory, 1, sizeof(updcomp_t), "upd/fscomp");
			if(NULL == upd->valptr[icomp]) {
#if UPD_MESSAGES & UPD_M_ERROR
				errprintf("upd_open_fscomp: could not allocate "
				          "%d. updcomp\n",
				          icomp);
#endif
				icomp = 0;
	...
	if(icomp) {
		...
		upd->valbuf = gs_malloc(udev->memory, need,
		                        sizeof(upd->valbuf[0]), "upd/valbuf");
...
#if UPD_MESSAGES & UPD_M_ERROR
			errprintf("upd_open_fscomp: could not allocate %u "
			          "words for valbuf\n",
			          need);
#endif
			icomp = 0;
	...
	if(icomp) {
		for(icomp = 0; upd->ncomp > icomp; ++icomp) {
			...
			if(!nsteps)
				continue; /* A 0-Bit component is legal! */

			if(upd->cmap[comp->cmap].rise) {
				ymin = upd->float_a[upd->cmap[comp->cmap].xfer]
				           .data[0];
				ymax =
				    upd->float_a[upd->cmap[comp->cmap].xfer]
				        .data[upd->float_a[upd->cmap[comp->cmap]
				                               .xfer].size -
				              1];
			} else {
				ymax = upd->float_a[upd->cmap[comp->cmap].xfer]
				           .data[0];
				ymin =
				    upd->float_a[upd->cmap[comp->cmap].xfer]
				        .data[upd->float_a[upd->cmap[comp->cmap]
			...
			for(i = 0; i < 32; ++i) { /* Attempt Ideal */
				...
				highval = (int32_t)((ymax - ymin) *
				                        (double)comp->spotsize +
				                    0.5);

				if(!(highmod = highval % nsteps))
					break; /* Gotcha */

				highval += nsteps - highmod;
				comp->spotsize = (int32_t)(
				    (double)highval / (ymax - ymin) + 0.5);
			...
			comp->offset = (int32_t)(ymin * (double)comp->spotsize +
			                         (double)0.5);
			...
			errprintf(
			    ...
			    comp->cmap + 1, i);
			errprintf("steps:     %10ld, Bits: %d\n",
			          (long)comp->bitmsk, comp->bits);
			errprintf("xfer:      %10d Points, %s\n",
			          upd->float_a[upd->cmap[comp->cmap].xfer].size,
			          upd->cmap[comp->cmap].rise ? "rising"
			                                     : "falling");
			errprintf("offset:    %10d 0x%08x\n", comp->offset,
			          comp->offset);
			errprintf("scale:     %10d 0x%08x\n", comp->scale,
			          comp->scale);
			errprintf("threshold: %10d 0x%08x\n", comp->threshold,
			          comp->threshold);
			errprintf("spotsize:  %10d 0x%08x\n", comp->spotsize,
			          comp->spotsize);
#endif
		}
	}
...
upd_close_fscomp(upd_device* udev)
{
	...
	for(icomp = 0; UPD_VALPTR_MAX > icomp; ++icomp) {
		if(!upd->valptr[icomp])
			continue;
		gs_free(udev->memory, upd->valptr[icomp], 1, sizeof(updcomp_t),
		        "upd/fscomp");
		upd->valptr[icomp] = NULL;
	}
}
...
#define FS_M_ROWERR(I)                                                         \
	if(fs_emin[I] > rowerr[I])                                             \
		fs_emin[I] = rowerr[I];                                        \
	if(fs_emax[I] < rowerr[I])                                             \
		fs_emax[I] = rowerr[I];
#else
#define FS_M_ROWERR(I) ;
#endif
...
#define FS_GOAL(Raw, I)                                                        \
	...
	if(pixel[I] < 0)                                                       \
		pixel[I] = 0;                                                  \
	else if(pixel[I] > comp[I]->spotsize)                                  \
		pixel[I] = comp[I]->spotsize;
...
upd_open_fscmyk(upd_device* udev)
{
	...
	if((B_RENDER & upd->flags) && (4 == upd->ncomp) &&
...
upd_fscmyk(upd_p upd)
{
	...
	while(0 < pwidth--) {
		...
		if(data[1] || data[2] || data[3]) {
			...
			FS_M_ROWERR(upd->cmap[1].comp)
			    FS_GOAL(data[1], upd->cmap[1].comp)
			        FS_M_ROWERR(upd->cmap[2].comp)
			            FS_GOAL(data[2], upd->cmap[2].comp)
			                FS_M_ROWERR(upd->cmap[3].comp)
			                    FS_GOAL(data[3], upd->cmap[3].comp)
			    /*
			     *       if black fires, then all other components
			     * fire logically too
			     */
			    if(pixel[upd->cmap[0].comp] >
			       comp[upd->cmap[0].comp]->threshold)
			{
				...
				/*
				 *       if black is below threshold, only
				 * components with larger data-values
				 *       are allowed to fire
				 */
			}
			else
			{ /* Restricted firing */
				...
				if((data[0] < data[1]) &&
				   (pixel[upd->cmap[1].comp] >
				    comp[upd->cmap[1].comp]
				        ->threshold)) { /* "Fire" */
					pixel[upd->cmap[1].comp] -=
					    comp[upd->cmap[1].comp]->spotsize;
					scan[upd->cmap[1].comp].bytes[ibyte] |=
					    bit;
				} /* "Fire" */

				if((data[0] < data[2]) &&
				   (pixel[upd->cmap[2].comp] >
				    comp[upd->cmap[2].comp]
				        ->threshold)) { /* "Fire" */
					pixel[upd->cmap[2].comp] -=
					    comp[upd->cmap[2].comp]->spotsize;
					scan[upd->cmap[2].comp].bytes[ibyte] |=
					    bit;
				} /* "Fire" */

				if((data[0] < data[3]) &&
				   (pixel[upd->cmap[3].comp] >
				    comp[upd->cmap[3].comp]
				        ->threshold)) { /* "Fire" */
					pixel[upd->cmap[3].comp] -=
					    comp[upd->cmap[3].comp]->spotsize;
					scan[upd->cmap[3].comp].bytes[ibyte] |=
					    bit;
				} /* "Fire" */
		...
		} else if(pixel[upd->cmap[0].comp] >
		          comp[upd->cmap[0].comp]->threshold) {
			scan[upd->cmap[0].comp].bytes[ibyte] |= bit;
			pixel[upd->cmap[0].comp] -=
			    comp[upd->cmap[0].comp]->spotsize;
		}
...
upd_fscmy_k(upd_p upd)
{
	...
	while(0 < pwidth--) {
		...
		FS_M_ROWERR(0) FS_GOAL(
		    comp[0]->bitmsk & (ci >> comp[0]->bitshf), 0) FS_M_ROWERR(1)
		    FS_GOAL(comp[1]->bitmsk & (ci >> comp[1]->bitshf),
		            1) FS_M_ROWERR(2)
		        FS_GOAL(comp[2]->bitmsk & (ci >> comp[2]->bitshf), 2)
		            FS_M_ROWERR(3) FS_GOAL(
		                comp[3]->bitmsk & (ci >> comp[3]->bitshf), 3)
		...
		{ /* Colors may fire */
			...
			if((pixel[1] <= comp[1]->threshold) ||
			   ...
			   (pixel[3] <=
			    comp[3]->threshold)) { /* Really a Color */
...
upd_open_writer(upd_device* udev)
{
	...
	upd->ocomp = upd->ncomp;
	if(0 < upd->ints[I_OCOMP])
		upd->ocomp = upd->ints[I_OCOMP];

	/** Massage some Parameters */
	if(success) {

		/*    Make sure, that Pass & Pin-Numbers are at least 1 */
		if(1 > upd->ints[I_NYPASS])
			upd->ints[I_NYPASS] = 1;
		if(1 > upd->ints[I_NXPASS])
			upd->ints[I_NXPASS] = 1;
		if(1 > upd->ints[I_PINS2WRITE])
			upd->ints[I_PINS2WRITE] = 1;

		if((upd->ints[I_NXPASS] * upd->ints[I_NYPASS]) >
		   upd->ints[I_NPASS])
			upd->ints[I_NPASS] =
			    upd->ints[I_NXPASS] * upd->ints[I_NYPASS];
		...
		if(upd->ints[I_NPASS] > upd->int_a[IA_STD_DY].size) {
			...
			for(iy = 1; iy < upd->ints[I_NYPASS]; ++iy) {
				for(ix = 1; ix < upd->ints[I_NXPASS]; ++ix)
					*ip++ = 0;
				*ip++ = 1;
			}
			for(ix = 1; ix < upd->ints[I_NXPASS]; ++ix)
		...
		/*    Adjust END_Y */
		/*    Arrgh, I knew, why I refused to provide defaults for
		 * crucial */
		/*    parameters in uniprint. But o.k. it's nice for
		 * size-changing */
		...
		if(0 >= upd->int_a[IA_STD_IX].size) {
			...
			UPD_MM_GET_ARRAY(udev->memory, ip,
			                 upd->int_a[IA_STD_DY].size);
			upd->int_a[IA_STD_IX].data = ip;
			upd->int_a[IA_STD_IX].size = upd->int_a[IA_STD_DY].size;

			for(i = 0, ix = 0; i < upd->int_a[IA_STD_IX].size;
			    ++i) {
				*ip++ = ix++;
				if(ix == upd->ints[I_NXPASS])
					ix = 0;
			}
		}
...
/** The sum of Values in STD_DY should equal NYPASS * PINS2WRITE (diagnostic) */
	...
	if(success) {
		...
		if((upd->ints[I_NYPASS] * upd->ints[I_PINS2WRITE]) != sum)
			errprintf("upd_open_writer: Sum of normal Feeds is %d "
			          "rather than %d\n",
			          sum, upd->ints[I_NYPASS] *
			                   upd->ints[I_PINS2WRITE]);
	}
#endif
	...
	if(upd->int_a[IA_BEGBOT].size < upd->int_a[IA_BEG_DY].size) {
		...
		int i;
		for(i = 0; i < upd->int_a[IA_BEG_DY].size; ++i)
			if((upd->int_a[IA_BEGBOT].data[i] >
			    upd->ints[I_PINS2WRITE]) ||
			   (upd->int_a[IA_BEGBOT].data[i] < 0))
				break;

		if(i < upd->int_a[IA_BEG_DY].size) {
#if UPD_MESSAGES & UPD_M_WARNING
			errprintf("upd_open_writer: Only %d is invalid initial "
			          "Pins\n",
...
/** The sum of Values in BEG_DY should equal BEG_Y */
	...
	if(success) {
		...
		if(upd->ints[I_BEG_Y] != sum)
			errprintf("upd_open_writer: Sum of initial Feeds is %d "
			          "rather than %d\n",
	...
	if(upd->int_a[IA_ENDTOP].size < upd->int_a[IA_END_DY].size) {
#if UPD_MESSAGES & UPD_M_WARNING
		errprintf("upd_open_writer: Only %d instead of %d Final Pins\n",
		...
		for(i = 0; i < upd->int_a[IA_END_DY].size; ++i)
			if((upd->int_a[IA_ENDTOP].data[i] >
			    upd->ints[I_PINS2WRITE]) ||
			   (upd->int_a[IA_ENDTOP].data[i] < 0))
				break;
	...
	if(success) { /* Compute nscnbuf */
		...
		if(upd->ints[I_NSCNBUF] > want)
			want = upd->ints[I_NSCNBUF];

		if(1 > want)
			want = 1;

		for(use = 1; 0 < use; use <<= 1)
			if(use > want)
				break;
	...
	if(success) { /* Compute pwidth, scnmsk, nbytes, pheight */
		...
		if(0 < upd->ints[I_PWIDTH])
			...
			upd->pwidth = upd->gswidth;

		upd->nbytes =
		    (upd->pwidth + CHAR_BIT * sizeof(upd->scnbuf[0]->bytes[0]) -
		     1) /
		    (CHAR_BIT * sizeof(upd->scnbuf[0]->bytes[0]));

		upd->scnmsk = upd->nscnbuf - 1;

		if(0 < upd->ints[I_PHEIGHT])
			upd->pheight = upd->ints[I_PHEIGHT];
		else
			upd->pheight = upd->gsheight;
	...
	if(success) { /* Determine sizes */
		...
		case FMT_RAS:
			if(0 > upd_open_rascomp(udev))
				success = false;
			break;
		case FMT_EPSON:
			if(0 > upd_open_wrtescp(udev))
				success = false;
			break;
		case FMT_ESCP2Y:
		case FMT_ESCP2XY:
		case FMT_ESCNMY: /* (GR) */
			if(0 > upd_open_wrtescp2(udev))
				success = false;
			break;
		case FMT_RTL:
			if(0 > upd_open_wrtrtl(udev))
				success = false;
			break;
		case FMT_CANON: /* (hr) */
			if(0 > upd_open_wrtcanon(udev))
				success = false;
			break;
		default:
			success = false;
	...
	if(success && (0 < upd->noutbuf)) { /* Allocate outbuf */
		upd->outbuf = gs_malloc(udev->memory, upd->noutbuf,
		                        sizeof(upd->outbuf[0]), "upd/outbuf");
		if(!upd->outbuf)
			success = false;
	} /* Allocate outbuf */
	...
	if(success) {
		upd->scnbuf = gs_malloc(udev->memory, upd->nscnbuf,
		                        sizeof(upd->scnbuf[0]), "upd/scnbuf");
		if(NULL == upd->scnbuf) {
			success = false;
		} else {
			int ibuf;
			for(ibuf = 0; ibuf < upd->nscnbuf; ++ibuf) {
				if(success)
					upd->scnbuf[ibuf] =
					    gs_malloc(udev->memory, upd->ocomp,
					              sizeof(upd->scnbuf[0][0]),
					              "upd/scnbuf[]");
				else
				...
				if(!upd->scnbuf[ibuf]) {
					...
					int icomp;
					for(icomp = 0; icomp < upd->ocomp;
					    ++icomp) {
						if(success)
							upd->scnbuf[ibuf][icomp]
							    .bytes = gs_malloc(
							    udev->memory,
							    upd->nbytes,
							    sizeof(
...
upd_close_writer(upd_device* udev)
{
	...
	if(upd) {
		...
		if((0 < upd->noutbuf) && upd->outbuf)
			gs_free(udev->memory, upd->outbuf, upd->noutbuf,
			        sizeof(upd->outbuf[0]), "upd/outbuf");
		upd->noutbuf = 0;
		upd->outbuf = NULL;
		...
		if((0 < upd->nscnbuf) && upd->scnbuf) {
			for(ibuf = 0; upd->nscnbuf > ibuf; ++ibuf) {
				...
				if(icomp)
					gs_free(udev->memory, upd->scnbuf[ibuf],
					        upd->ocomp,
					        sizeof(upd->scnbuf[0][0]),
					        "upd/scnbuf[]");
				upd->scnbuf[ibuf] = NULL;
			}
			gs_free(udev->memory, upd->scnbuf, upd->nscnbuf,
			        sizeof(upd->scnbuf[0]), "upd/scnbuf");
		}
	...
	if(check) { /* Really check */
		for(icomp = 0; icomp < upd->ocomp;
			...
			if(xs < upd->nbytes) { /* Has Data */
				for(xe = upd->nbytes; xs < xe && !bytes[xe - 1];
				    --xe)
					;

				for(pass = 0; pass < upd->nlimits;
				    ++pass) { /* limit (pass) loop */
...
#define put32(I32, Out) fwrite(&I32, 1, 4, Out)
#else
#define put32(I32, Out)                                                        \
	putc(((I32) >> 24) & 255, Out), putc(((I32) >> 16) & 255, Out),        \
	    putc(((I32) >> 8) & 255, Out), putc((I32)&255, Out)
#endif
		...
		if(1 == upd->ncomp) { /* ??? upd->ocomp */
			...
			for(rgb = 0; rgb < 3; ++rgb) {
				...
				for(entry = 0; entry < 8; ++entry) {
					byte xval =
					    upd->cmap[rgb].rise ? 0x00 : 0xff;
					if(entry & (1 << upd->cmap[rgb].comp))
						xval ^= 0xff;
					putc(xval, out);
				}
			}
			...
			for(rgb = 16; 0 <= rgb; rgb -= 8) {
				...
				for(entry = 0; entry < 16; ++entry) {
					...
					if(!(upd->choice[C_MAPPER] == MAP_RGBW))
						rgbval ^= 0xffffff;
...
upd_rascomp(upd_p upd, FILE* out)
{
	...
	if(1 == upd->ncomp) { /* ??? upd->ocomp */
		...
		if((bits &= 7))
			upd->outbuf[nbytes - 1] &= ((byte)0xff) << (8 - bits);
		...
		while(0 < bits--) {
			...
			case 4:
				if(scan[3].bytes[ibyte] & bit)
					val |= 8;
			case 3:
				if(scan[2].bytes[ibyte] & bit)
					val |= 4;
				if(scan[1].bytes[ibyte] & bit)
					val |= 2;
			case 1:
				if(scan[0].bytes[ibyte] & bit)
					val |= 1;
			}
			*buf++ = val;
			if(!(bit >>= 1)) {
...
upd_open_wrtescp(upd_device* udev)
{
...
#if UPD_MESSAGES & UPD_M_WARNING
		errprintf("ESC/P-Open: Either SETLF- or YMOVE-Command must be "
		          "present\n");
#endif
		error = -1;
	}
	...
	if(((1 < upd->ints[I_XSTEP]) && (0 == upd->strings[S_XSTEP].size)) ||
	   ((1 < upd->ints[I_NXPASS]) && (0 == upd->strings[S_XMOVE].size) &&
	    (0 == upd->strings[S_XSTEP].size))) {
#if UPD_MESSAGES & UPD_M_WARNING
		errprintf("ESC/P-Open: Missing XSTEP- and/or XMOVE-Command\n");
#endif
		error = -1;
	}
...
#if UPD_MESSAGES & UPD_M_WARNING
		errprintf("ESC/P-Open: WRITECOMP-Commands must be given\n");
#endif
		error = -1;
	}

	/**
	If all this is correct, it's time to coumput the size of the
	output-buffer.
	It must hold:
	  1. Y-Positioning
	  2. X-Positioning
	...
	if(0 <= error) {
		...
		if(0 < upd->string_a[SA_SETCOMP].size) {
			...
			for(i = 0; i < upd->ocomp; ++i)
				if(need <
				   upd->string_a[SA_SETCOMP].data[i].size)
					need = upd->string_a[SA_SETCOMP]
					           .data[i]
					           .size;
			noutbuf += need;
		}
		...
		noutbuf += ((upd->ints[I_PINS2WRITE] + 7) / 8) *
		           ((upd->pwidth + upd->ints[I_NXPASS] - 1) /
		            upd->ints[I_NXPASS]);
		...
		if((0 < noutbuf) && (noutbuf <= INT_MAX)) {
			...
			error = -1;
#if UPD_MESSAGES & UPD_M_WARNING
			errprintf("ESC/P-Open: %ld is unreasonable size of "
			          "Outputbuffer\n",
			          (long)noutbuf);
#endif
		}
...
{
	int pinbot, pin, pintop, xbegin, x, xend, icomp, ybegin, yend, y,
	    ioutbuf, n, ixpass;
	byte* obytes, bit;
	updscan_p scan;
	...
	if(xbegin <= xend) { /* Some data to write */
		...
		if(upd->yscan != upd->yprinter) { /* Adjust Y-Position */
			if(B_YABS & upd->flags)
				y = upd->yscan + upd->ints[I_YOFS];
			else
				y = upd->yscan - upd->yprinter;
			...
			if(1 < upd->ints[I_YSTEP]) {
				...
				n = y *
				    -upd->ints[I_YSTEP]; /* May this work? */
				y = 0;
			} else {
				n = y;
				y = 0;
		 ...
		 */
		for(icomp = 0; icomp < upd->ocomp;
		    ++icomp) { /* Component-Print */
			       /*
			        *       First check, wether this Component needs
			        * printing
			        */
			for(y = ybegin; y < yend;
			    y += upd->ints[I_NYPASS]) { /* Comp-Test */
				if(0 > y)
					continue;
				scan = upd->scnbuf[y & upd->scnmsk] + icomp;
				if(0 <= scan->xend[ixpass])
					break;
			} /* Comp-Test */
			if(y >= yend)
				continue; /* Component not required */
			                  /*
			   ...
			   (upd->icomp != icomp)) { /* Selection enabled */
				...
				if(0 <
				   upd->string_a[SA_SETCOMP].data[icomp].size) {
					memcpy(upd->outbuf + ioutbuf,
					       upd->string_a[SA_SETCOMP]
					           .data[icomp]
					           .data,
					       upd->string_a[SA_SETCOMP]
					           .data[icomp]
					           .size);
					ioutbuf += upd->string_a[SA_SETCOMP]
					               .data[icomp]
					               .size;
				}
			} /* Selection enabled */
			...
			if(xbegin != upd->xprinter) {
				...
				if(0 == upd->strings[S_XMOVE].size) {
					...
					if(B_XABS & upd->flags)
						n = x =
						    xbegin + upd->ints[I_XOFS];
					else
						n = x = xbegin - upd->xprinter;
			...
			if(0 < upd->string_a[SA_WRITECOMP].data[icomp].size) {
				memcpy(upd->outbuf + ioutbuf,
				       upd->string_a[SA_WRITECOMP]
				           .data[icomp]
				           .data,
				       upd->string_a[SA_WRITECOMP]
				           .data[icomp]
				           .size);
				ioutbuf += upd->string_a[SA_WRITECOMP]
				               .data[icomp]
				               .size;
			}
			...
			for(x = xbegin; x <= xend; x += upd->ints[I_NXPASS]) {
				...
				obytes +=
				    (upd->ints[I_PINS2WRITE] - pinbot + 7) >> 3;
			}
			/*
	...
	if(upd->yscan < upd->ints[I_BEG_Y]) {
		...
		if(upd->ints[I_BEG_Y] <= upd->yscan)
			upd->ipass = 0;
		else if(upd->int_a[IA_BEG_DY].size <= upd->ipass)
			upd->ipass = 0;
	} else if(upd->yscan >= upd->ints[I_END_Y]) {
		upd->yscan += upd->int_a[IA_END_DY].data[upd->ipass++];
		if(upd->int_a[IA_END_DY].size <= upd->ipass)
			upd->ipass = 0;
	} else {
		upd->yscan += upd->int_a[IA_STD_DY].data[upd->ipass++];
		if(upd->int_a[IA_STD_DY].size <= upd->ipass)
			upd->ipass = 0;
		if(upd->yscan >= upd->ints[I_END_Y])
			upd->ipass = 0;
	}
...
upd_open_wrtescp2(upd_device* udev)
{
	...
	if(0 < upd->strings[S_BEGIN].size) { /* BOP-Checker */
		...
		for(i = 0; i < upd->strings[S_BEGIN].size; ++i) {
			...
			case 2:
				...
				case 'U':
					state = 3;
					break; /* Printer-Resolution */
				case 'C':
					state = 6;
					break; /* Page-Length */
				case 'c':
					state = 10;
					break; /* Top/Bottom Margin */
				default:
					state = 0;
					break;
				}
	...
	if((1 < upd->ints[I_XSTEP]) && (0 == upd->strings[S_XSTEP].size)) {
...
#if UPD_MESSAGES & UPD_M_WARNING
		errprintf("ESC/P2-Open: XSTEP-Command required for XSTEP=%d\n",
		          upd->ints[I_XSTEP]);
#endif
		error = -1;
		...
		ratio = (int)((udev->y_pixels_per_inch + 0.5) /
		              udev->x_pixels_per_inch);

		if(0 == upd->ints[I_XSTEP]) { /* Adjust scale-factor too! */
			if(ratio > 1)
				upd->ints[I_XSTEP] = -ratio;
		...
		for(i = 0; i < upd->ocomp; ++i) {
			...
			*bp++ = 1; /* RLE */
			...
			case FMT_ESCNMY:
				...
				*/
				*bp++ =
				    10; /* needs to always be this for esc300 */
				*bp++ = 10;
				*bp++ = upd->ints[I_ROWS];
				break;
			}
...
#if UPD_MESSAGES & UPD_M_WARNING
		errprintf("ESC/P2-Open: WRITECOMP-Commands must be given\n");
#endif
		error = -1;
	}
	...
	case FMT_ESCP2Y:
		if(1 < upd->ints[I_NXPASS]) {
#if UPD_MESSAGES & UPD_M_WARNING
			errprintf("ESC/P2-Open: FMT_ESCP2Y cannot handle "
			          "multiple X-Passes\n");
#endif
			error = -1;
		} else {
...
#if UPD_MESSAGES & UPD_M_WARNING
		if(1 == upd->ints[I_NXPASS])
			errprintf("ESC/P2-Open: FMT_ESCP2XY should not be used "
			          "with 1X-Pass\n");
#endif
		break;
	case FMT_ESCNMY:
		if(1 < upd->ints[I_NXPASS]) {
#if UPD_MESSAGES & UPD_M_WARNING
			errprintf("ESC/P2-Open: FMT_ESCNMY cannot handle "
			          "multiple X-Passes\n");
#endif
			error = -1;
		} else {
	...
	default:
#if UPD_MESSAGES & UPD_M_WARNING
		errprintf("ESC/P2-Open: %d is not a ESC/P2-Format\n",
		          upd->choice[C_FORMAT]);
#endif
		error = -1;
		break;
	...
	/**
	If all this is correct, it's time to compute the size of the
	output-buffer.
	It must hold:
	  1. Y-Positioning
	  2. X-Positioning
...
upd_wrtescp2(upd_p upd, FILE* out)
{
	...
	for(y = ybegin; y < yend; y += upd->ints[I_NYPASS]) { /* Pin-testloop */
		...
		for(icomp = 0; icomp < upd->ocomp;
			...
			if(x < upd->nbytes) {
				for(x = upd->nbytes - 1; x > xend && !obytes[x];
				    x--)
					;
				if(x > xend)
					xend = x;
		...
		for(icomp = 0; icomp < upd->ocomp;
			...
			for(y = ybegin; y < yend;
				...
				if(0 > y)
					continue;
				obytes =
				    upd->scnbuf[y & upd->scnmsk][icomp].bytes;
			...
			for(pin = 0; pin < pintop; ++pin) {
				ioutbuf +=
				    upd_rle(upd->outbuf + ioutbuf, NULL, n);
				fwrite(upd->outbuf, 1, ioutbuf, out);
				ioutbuf = 0;
			}
			...
			for(pin = pinbot; pin < upd->ints[I_PINS2WRITE];
			    ++pin) {
				ioutbuf +=
				    upd_rle(upd->outbuf + ioutbuf, NULL, n);
				fwrite(upd->outbuf, 1, ioutbuf, out);
	...
	if(xbegin <= xend) { /* Some data to write */
		...
		*/
		...
		if(xbegin != upd->xprinter) {
			...
			if(x &&
			   0 < upd->strings[S_XSTEP].size) { /* Fine-Adjust X */
				while(x--) {
					memcpy(upd->outbuf + ioutbuf,
					       upd->strings[S_XSTEP].data,
					       upd->strings[S_XSTEP].size);
		...
		*/
		...
		for(; y < yend; y += upd->ints[I_NYPASS]) {
			...
			for(i = 0; (i < upd->ints[I_PATRPT]); i++) {
				...
				if(icomp == 0) {
					ioutbuf += upd_rle(
					    upd->outbuf + ioutbuf, NULL, n);
				} else {
					--icomp;
					iyofs =
					    upd->int_a[IA_SCNOFS].data[imask];
					ioutbuf += upd_rle(
					    upd->outbuf + ioutbuf,
					    upd->scnbuf[(y + iyofs) &
					                upd->scnmsk][icomp]
		...
		/*  I think this is the pins at the bottom of the head that
		 * don't print */
	...
	if(xbegin <= xend) { /* Some data to write */
		...
		for(icomp = 0; icomp < upd->ocomp;
			...
			for(pin = 0; pin < pintop; ++pin) {
				ioutbuf +=
				    upd_rle(upd->outbuf + ioutbuf, NULL, n);
				fwrite(upd->outbuf + upd->nbytes, 1,
				       ioutbuf - upd->nbytes, out);
				ioutbuf = upd->nbytes;
			}
...
upd_open_wrtrtl(upd_device* udev)
{
	...
	if(0 < upd->strings[S_BEGIN].size) { /* BOP-Checker */
		...
		for(i = 0; i < upd->strings[S_BEGIN].size; ++i) {
			...
			/* ----- got ESC * */
			case 2:
				j = i; /* This character is not part of the
				          replaced text */
				if(c == 0x72)
					state = 3; /* ESC * r */
				else if(c == 0x74)
				...
				if((B_PAGEWIDTH & upd->flags) &&
				   ((c == 0x73) ||
					...
					if(0 <= j)
						memcpy(
						    bp,
						    upd->strings[S_BEGIN].data,
						    j + 1);
					memcpy(bp + j + 1, cv, ncv);
					memcpy(bp + j + 1 + ncv,
					       upd->strings[S_BEGIN].data + i,
					       upd->strings[S_BEGIN].size - i);
					i = j + 1 + ncv;
					UPD_MM_DEL_PARAM(udev->memory,
					                 upd->strings[S_BEGIN]);
			...
			case 23: /* PJL-Set: S E T ws  */
				if((c == 0x50) || (c == 0x70))
					state = 24; /* set paper... */
				else if((c == 0x52) || (c == 0x72))
					state = 41; /* set resolution */
				else if(c == 0x0a)
					state = 12; /* BOL */
				else
					state = 18;
			...
			case 28: /* PJL-Set: set paper?  */
				if((c == 0x4c) || (c == 0x6c))
					state = 29; /* set paperlength */
				else if((c == 0x57) || (c == 0x77))
					state = 36; /* set paperwidth */
				else if(c == 0x0a)
					state = 12; /* BOL */
				else
					state = 18;
			...
			case 35: /* PJL: set paperlength */
				if((0x30 > c) ||
				   (c > 0x39)) { /* End of number */

					if(B_PAGELENGTH &
					   upd->flags) { /* insert new number */

						sprintf(
						    cv, "%d",
						    (int)(720.0 * udev->height /
						              udev->y_pixels_per_inch +
						          0.5));
						ncv = strlen(cv);
			...
			default:
#if UPD_MESSAGES & UPD_M_ERROR
				errprintf("UNIPRINT-Coding error, wrrtl, state "
				          "= %d\n",
				          state);
#endif
				state = 0;
	...
	if(upd->ocomp > upd->string_a[SA_WRITECOMP].size) {
#if UPD_MESSAGES & UPD_M_WARNING
		errprintf("PCL-Open: WRITECOMP-Commands must be given\n");
#endif
		error = -1;
	}
	...
	*/
	if(0 <= error) {
		...
		if((0 < noutbuf) && (noutbuf <= INT_MAX)) {
			...
			error = -1;
#if UPD_MESSAGES & UPD_M_WARNING
			errprintf("PCL-Open: %ld is unreasonable size of "
			          "Outputbuffer\n",
			          (long)noutbuf);
#endif
		}
					...
					for(icomp = 0; icomp < upd->ocomp;
						...
						sprintf(
						    (char*)upd->outbuf +
						        ioutbuf,
						    (const char*)upd
						        ->string_a[SA_WRITECOMP]
						        .data[icomp]
						        .data,
						    0);
						ioutbuf +=
						    strlen((char*)upd->outbuf +
						           ioutbuf);
					}
					fwrite(upd->outbuf, 1, ioutbuf, out);
					ioutbuf = 0;
		...
		for(icomp = 0; icomp < upd->ocomp;
			...
			for(x = 0; x <= xend; ++x)
				if(data[x])
					break;
			if(x <= xend) {
				ioutbuf = upd_rle(upd->outbuf,
				                  scan[icomp].bytes, xend);
				fprintf(out,
				        (const char*)upd->string_a[SA_WRITECOMP]
				            .data[icomp]
				            .data,
				        ioutbuf);
				fwrite(upd->outbuf, 1, ioutbuf, out);
...
upd_wrtcanon(upd_p upd, FILE* out)
{
	...
	if(0 <= xend) { /* Some data to write */
		...
		for(icomp = 0; icomp < upd->ocomp; ++icomp) {
			...
			/* Are there date to print for the selected color
			 * component */
			data = scan[icomp].bytes;
			for(x = 0; x <= xend; ++x)
				if(data[x])
			...
			/*
			 *      Please Note:
			 *         the validity of the NCOMP-setting should be
			 * checked
			 *         in the put_params-routine, thus the
			 * default-case is
...
private
upd_proc_pxlget(upd_pxlgetnix); /* A Dummy */

private
upd_proc_pxlget(upd_pxlget1f1); /* 1 Bit Forward */
private
upd_proc_pxlget(upd_pxlget1f2);
private
upd_proc_pxlget(upd_pxlget1f3);
private
upd_proc_pxlget(upd_pxlget1f4);
private
upd_proc_pxlget(upd_pxlget1f5);
private
upd_proc_pxlget(upd_pxlget1f6);
private
upd_proc_pxlget(upd_pxlget1f7);
private
upd_proc_pxlget(upd_pxlget1f8);

private
upd_proc_pxlget(upd_pxlget1r1); /* 1 Bit Reverse */
private
upd_proc_pxlget(upd_pxlget1r2);
private
upd_proc_pxlget(upd_pxlget1r3);
private
upd_proc_pxlget(upd_pxlget1r4);
private
upd_proc_pxlget(upd_pxlget1r5);
private
upd_proc_pxlget(upd_pxlget1r6);
private
upd_proc_pxlget(upd_pxlget1r7);
private
upd_proc_pxlget(upd_pxlget1r8);

private
upd_proc_pxlget(upd_pxlget2f1); /* 2 Bit Forward */
private
upd_proc_pxlget(upd_pxlget2f2);
private
upd_proc_pxlget(upd_pxlget2f3);
private
upd_proc_pxlget(upd_pxlget2f4);

private
upd_proc_pxlget(upd_pxlget2r1); /* 2 Bit Reverse */
private
upd_proc_pxlget(upd_pxlget2r2);
private
upd_proc_pxlget(upd_pxlget2r3);
private
upd_proc_pxlget(upd_pxlget2r4);

private
upd_proc_pxlget(upd_pxlget4f1); /* 4 Bit Forward */
private
upd_proc_pxlget(upd_pxlget4f2);

private
upd_proc_pxlget(upd_pxlget4r1); /* 4 Bit Reverse */
private
upd_proc_pxlget(upd_pxlget4r2);

private
upd_proc_pxlget(upd_pxlget8f); /* 8 Bit Forward */
private
upd_proc_pxlget(upd_pxlget8r); /* 8 Bit Reverse */

private
upd_proc_pxlget(upd_pxlget16f); /* 16 Bit Forward */
private
upd_proc_pxlget(upd_pxlget16r); /* 16Bit Reverse */

private
upd_proc_pxlget(upd_pxlget24f); /* 24 Bit Forward */
private
upd_proc_pxlget(upd_pxlget24r); /* 24 Bit Reverse */

private
upd_proc_pxlget(upd_pxlget32f); /* 32 Bit Forward */
private
upd_proc_pxlget(upd_pxlget32r); /* 32 Bit Reverse */
...
upd_pxlfwd(upd_p upd)
{
	if(!(upd->pxlptr = upd->gsscan)) {
		...
		case 4:
			...
			break;
		case 8:
			upd->pxlget = upd_pxlget8f;
			break;
		case 16:
			upd->pxlget = upd_pxlget16f;
			break;
		case 24:
			upd->pxlget = upd_pxlget24f;
			break;
		case 32:
			upd->pxlget = upd_pxlget32f;
			break;
		default:
#if UPD_MESSAGES & UPD_M_ERROR
...
{
	const uint width =
	    upd->pwidth < upd->gswidth ? upd->pwidth : upd->gswidth;
	...
	if(!(upd->pxlptr = upd->gsscan)) {
		...
		uint32_t ofs =
		    (uint32_t)upd->int_a[IA_COLOR_INFO].data[1] * (width - 1);
