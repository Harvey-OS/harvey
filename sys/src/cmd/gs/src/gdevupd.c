...
#define UPD_MM_DEL_PARAM(mem, Which)                                                                              \
	{                                                                                                         \
		if(Which.data && Which.size)                                                                      \
			gs_free(mem, upd_cast(Which.data), Which.size, sizeof(Which.data[0]), "uniprint/params"); \
	}
...
*/
	...
	if((c == m) && (m == y)) {
		...
		rv = upd_truncate(upd, 0, k) | upd_truncate(upd, 1, c) | upd_truncate(upd, 2, m) | upd_truncate(upd, 3, y);
	...
	errprintf(
	    ...
	    255.0 * (double)((rv >> upd->cmap[1].bitshf) & upd->cmap[1].bitmsk) / (double)upd->cmap[1].bitmsk,
	    255.0 * (double)((rv >> upd->cmap[2].bitshf) & upd->cmap[2].bitmsk) / (double)upd->cmap[2].bitmsk,
	    255.0 * (double)((rv >> upd->cmap[3].bitshf) & upd->cmap[3].bitmsk) / (double)upd->cmap[3].bitmsk,
	    255.0 * (double)((rv >> upd->cmap[0].bitshf) & upd->cmap[0].bitmsk) / (double)upd->cmap[0].bitmsk,
 ...
 */
	...
	errprintf(
	    ...
	    255.0 * (double)((color >> upd->cmap[1].bitshf) & upd->cmap[1].bitmsk) / (double)upd->cmap[1].bitmsk,
	    255.0 * (double)((color >> upd->cmap[2].bitshf) & upd->cmap[2].bitmsk) / (double)upd->cmap[2].bitmsk,
	    255.0 * (double)((color >> upd->cmap[3].bitshf) & upd->cmap[3].bitmsk) / (double)upd->cmap[3].bitmsk,
	    255.0 * (double)((color >> upd->cmap[0].bitshf) & upd->cmap[0].bitmsk) / (double)upd->cmap[0].bitmsk,
	    255.0 * (double)c / (double)gx_max_color_value,
	    255.0 * (double)m / (double)gx_max_color_value,
	    255.0 * (double)y / (double)gx_max_color_value,
	    255.0 * (double)k / (double)gx_max_color_value,
...
upd_cmyk_kcolor(gx_device *pdev, const gx_color_value cv[])
{
	...
	if((c == m) && (m == y)) {
		...
		rv = upd_truncate(upd, 0, black) | upd_truncate(upd, 1, c) | upd_truncate(upd, 2, m) | upd_truncate(upd, 3, y);
		...
		for(imap = 0; UPD_CMAP_MAX > imap; ++imap) {
			...
			upd->cmap[imap].rise =
			    upd->float_a[upd->cmap[imap].xfer].data[0] <
				    upd->float_a[upd->cmap[imap].xfer].data[upd->float_a[upd->cmap[imap].xfer].size - 1]
				? true
				: false;
	...
	if(icomp) {
		for(icomp = 0; upd->ncomp > icomp; ++icomp) {
			...
			if(upd->cmap[comp->cmap].rise) {
				...
				ymax = upd->float_a[upd->cmap[comp->cmap].xfer].data[upd->float_a[upd->cmap[comp->cmap].xfer].size - 1];
			} else {
				ymax = upd->float_a[upd->cmap[comp->cmap].xfer].data[0];
				ymin = upd->float_a[upd->cmap[comp->cmap].xfer].data[upd->float_a[upd->cmap[comp->cmap].xfer].size - 1];
			}
...
#define FS_GOAL(Raw, I)                                                                                              \
	pixel[I] = (int32_t)(Raw)*comp[I]->scale + comp[I]->offset + rowerr[I] + colerr[I] - ((colerr[I] + 4) >> 3); \
	if(pixel[I] < 0)                                                                                             \
		pixel[I] = 0;                                                                                        \
...
#define FS_DIST(I)                                                                               \
	...
	rowerr[I] = ((5 * pixel[I]) >> 4) /* 5/16 */ + ((colerr[I] + 4) >> 3); /* 1/16 (rest) */ \
...
upd_open_writer(upd_device *udev)
{
	...
	if(success) {
		...
		if(upd->ints[I_NPASS] > upd->int_a[IA_STD_DY].size) {
			...
			for(ix = 1; ix < upd->ints[I_NXPASS]; ++ix)
				*ip++ = 0;
			*ip = upd->ints[I_NYPASS] * upd->ints[I_PINS2WRITE] - upd->ints[I_NYPASS] + 1;

			upd->ints[I_BEG_Y] = 0;
			upd->ints[I_END_Y] = upd->ints[I_PHEIGHT] ? upd->ints[I_PHEIGHT] : upd->gsheight;
		}
...
/** The sum of Values in BEG_DY should equal BEG_Y */
	...
	if(success) { /* Compute pwidth, scnmsk, nbytes, pheight */
		...
		upd->nbytes = (upd->pwidth + CHAR_BIT * sizeof(upd->scnbuf[0]->bytes[0]) - 1) / (CHAR_BIT * sizeof(upd->scnbuf[0]->bytes[0]));
			...
			for(ibuf = 0; ibuf < upd->nscnbuf; ++ibuf) {
				...
				if(!upd->scnbuf[ibuf]) {
					...
					for(icomp = 0; icomp < upd->ocomp; ++icomp) {
						...
						if(!upd->scnbuf[ibuf][icomp].bytes)
							success = false;
						...
						if(0 < upd->nlimits) {
							...
							if(!upd->scnbuf[ibuf][icomp].xbegin)
								success = false;
			...
			for(rgb = 16; 0 <= rgb; rgb -= 8) {
				...
				for(entry = 0; entry < 16; ++entry) {
					...
					if(entry & (1 << upd->cmap[0].comp)) {
						...
						if(entry & (1 << upd->cmap[1].comp))
							rgbval |= 0xff0000;
						if(entry & (1 << upd->cmap[2].comp))
							rgbval |= 0x00ff00;
						if(entry & (1 << upd->cmap[3].comp))
							rgbval |= 0x0000ff;
					}
	...
	if((B_PAGELENGTH & upd->flags) &&
	   (0 < upd->strings[S_BEGIN].size)) { /* BOP-Checker */
		...
		for(i = 0; i < upd->strings[S_BEGIN].size; ++i) {
			...
			case 2:
				if(bp[i]) {
					value = (int)(0.5 + udev->height * (float)bp[i] / udev->y_pixels_per_inch);
					if(0 >= value)
						bp[i] = 1;
					else if(128 > value)
						bp[i] = value;
...
*/
	if(0 <= error) {
		...
		noutbuf += ((upd->ints[I_PINS2WRITE] + 7) / 8) * ((upd->pwidth + upd->ints[I_NXPASS] - 1) / upd->ints[I_NXPASS]);
			...
			if(xbegin != upd->xprinter) {
				...
				if(0 == upd->strings[S_XMOVE].size) {
					...
					if(1 < upd->ints[I_XSTEP]) {
						...
						if(n)
							n /= upd->ints[I_XSTEP]; /* Major-Steps */
						if(x)
							x %= upd->ints[I_XSTEP]; /* Minor-Steps */
...
upd_open_wrtescp2(upd_device *udev)
{
	...
	if(0 < upd->strings[S_BEGIN].size) { /* BOP-Checker */
		...
		for(i = 0; i < upd->strings[S_BEGIN].size; ++i) {
			...
			case 8:
				if(B_PAGELENGTH & upd->flags) {
					value = (int)(0.5 + udev->height * pixels_per_inch / udev->y_pixels_per_inch);
					bp[i] = value & 0xff;
				}
				state = 9;
				break;
			...
			case 14:
				if(B_BOTTOMMARGIN & upd->flags) {
					value = (int)(0.5 + udev->height * pixels_per_inch / udev->y_pixels_per_inch - dev_b_margin(udev) * pixels_per_inch);
					bp[i] = value & 0xff;
				}
				state = 15;
				break;
			...
			case 3:
				...
				if((B_PAGEWIDTH & upd->flags) &&
				   ((c == 0x73) || (c == 0x53))) { /* esc * r # S */
					...
					if(0 <= j)
						memcpy(bp, upd->strings[S_BEGIN].data, j + 1);
			...
			case 4: /* esc * t */
				...
				if((B_RESOLUTION & upd->flags) &&
				   ((c == 0x72) || (c == 0x52))) { /* esc * t # R */
					...
					sprintf(cv, "%d", (int)((udev->y_pixels_per_inch < udev->x_pixels_per_inch ? udev->x_pixels_per_inch : udev->y_pixels_per_inch) + 0.5));
					ncv = strlen(cv);
			...
			case 35:			       /* PJL: set paperlength */
				if((0x30 > c) || (c > 0x39)) { /* End of number */
					...
					if(B_PAGELENGTH & upd->flags) { /* insert new number */
						...
						sprintf(cv, "%d", (int)(720.0 * udev->height / udev->y_pixels_per_inch + 0.5));
						ncv = strlen(cv);
			...
			case 40:			       /* PJL: set paperlength */
				if((0x30 > c) || (c > 0x39)) { /* End of number */
					...
					if(B_PAGEWIDTH & upd->flags) { /* insert new number */
						...
						sprintf(cv, "%d", (int)(720.0 * udev->width / udev->x_pixels_per_inch + 0.5));
						ncv = strlen(cv);
