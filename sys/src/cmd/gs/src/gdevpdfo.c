 ...
 */
#define cos_element_common(etype) etype* next
...
#define private_st_cos_element() /* in gdevpdfo.c */                           \
	gs_private_st_ptrs1(st_cos_element, cos_element_t, "cos_element_t",    \
	                    cos_element_enum_ptrs, cos_element_reloc_ptrs,     \
	                    next)
...
#define private_st_cos_stream_piece() /* in gdevpdfo.c */                      \
	gs_private_st_suffix_add0_local(                                       \
	    st_cos_stream_piece, cos_stream_piece_t, "cos_stream_piece_t",     \
	    cos_element_enum_ptrs, cos_element_reloc_ptrs, st_cos_element)
...
#define private_st_cos_array_element() /* in gdevpdfo.c */                     \
	gs_private_st_composite(                                               \
	    st_cos_array_element, cos_array_element_t, "cos_array_element_t",  \
	    cos_array_element_enum_ptrs, cos_array_element_reloc_ptrs)
/* dict */
...
#define private_st_cos_dict_element() /* in gdevpdfo.c */                      \
	gs_private_st_composite(                                               \
	    st_cos_dict_element, cos_dict_element_t, "cos_dict_element_t",     \
	    cos_dict_element_enum_ptrs, cos_dict_element_reloc_ptrs)
...
ENUM_PTRS_WITH(cos_array_element_enum_ptrs, cos_array_element_t* pcae)
{
	return (index < cos_element_num_ptrs
	            ...
	            : ENUM_USING(st_cos_value, &pcae->value,
	                         sizeof(cos_value_t),
...
ENUM_PTRS_WITH(cos_dict_element_enum_ptrs, cos_dict_element_t* pcde)
{
	return (index < cos_element_num_ptrs
	            ...
	            : (index -= cos_element_num_ptrs) > 0
	                  ? ENUM_USING(st_cos_value, &pcde->value,
	                               sizeof(cos_value_t), index - 1)
	                  : pcde->owns_key ? ENUM_STRING(&pcde->key)
	                                   : ENUM_OBJ(NULL));
}
ENUM_PTRS_END
private
		...
		if(!pco->id) {
			if(do_space &&
			   !(pco->cos_procs == cos_type_array ||
				...
				/* Arrays and dictionaries (only) are
				 * self-delimiting. */
				stream_putc(s, ' ');
			}
                       ...
                       const cos_value_t* pvalue, bool copy)
{
	*pcv = *pvalue;
	if(pvalue->value_type == COS_VALUE_SCALAR && copy) {
		byte* value_data = gs_alloc_string(
		    mem, pvalue->contents.chars.size, "cos_copy_element_value");
...
private
int
cos_value_equal(const cos_value_t* pcv0, const cos_value_t* pcv1,
                gx_device_pdf* pdev)
{
	...
	case COS_VALUE_CONST:
		if(bytes_compare(
		       pcv0->contents.chars.data, pcv0->contents.chars.size,
...
private
cos_proc_release(cos_generic_release);
private
cos_proc_write(cos_generic_write);
private
cos_proc_equal(cos_generic_equal);
...
private
int
cos_generic_equal(const cos_object_t* pco0, const cos_object_t* pco1,
                  gx_device_pdf* pdev)
{
	return_error(gs_error_Fatal);
}
...
private
cos_proc_release(cos_array_release);
private
cos_proc_write(cos_array_write);
private
cos_proc_equal(cos_array_equal);
const cos_object_procs_t cos_array_procs = {cos_array_release, cos_array_write,
                                            cos_array_equal};
...
private
cos_array_element_t* cos_array_reorder(const cos_array_t* pca,
...
private
int
cos_array_equal(const cos_object_t* pco0, const cos_object_t* pco1,
                gx_device_pdf* pdev)
{
                      ...
                      const cos_value_t* pvalue)
{
	...
	if(next && next->index == index) {
		...
		pcae = gs_alloc_struct(mem, cos_array_element_t,
...
cos_array_add_object(cos_array_t* pca, cos_object_t* pco)
{
	...
	value.contents.chars.size =
	    0; /* Quiet a warning appeared with MSVC6 inline optimization. */
...
private
cos_proc_release(cos_dict_release);
private
cos_proc_write(cos_dict_write);
private
cos_proc_equal(cos_dict_equal);
const cos_object_procs_t cos_dict_procs = {cos_dict_release, cos_dict_write,
                                           cos_dict_equal};
		...
		for(;;) {
			gs_id object_id1 =
			    (pdev->NoEncrypt.size == 0 ||
			             bytes_compare(pdev->NoEncrypt.data,
			                           pdev->NoEncrypt.size,
			                           pcde->key.data,
			                           pcde->key.size)
			         ? object_id
...
cos_dict_objects_write(const cos_dict_t* pcd, gx_device_pdf* pdev)
{
	...
	for(; pcde; pcde = pcde->next)
		if(COS_VALUE_IS_OBJECT(&pcde->value) &&
		   ...
		   !pcde->value.contents.object
		        ->written /* ForOPDFRead only. */)
                  ...
                  const cos_value_t* pvalue, int flags)
{
	...
	if(next) {
		...
		if(flags & DICT_COPY_KEY) {
			copied_key_data =
			    gs_alloc_string(mem, key_size, "cos_dict_put(key)");
			if(copied_key_data == 0)
				return_error(gs_error_VMerror);
			memcpy(copied_key_data, key_data, key_size);
		} else
			copied_key_data =
			    (byte*)key_data; /* OK to break const */
...
int
cos_dict_put_c_key_string(cos_dict_t* pcd, const char* key, const byte* data,
                          uint size)
{
	cos_value_t value;
...
{
	return cos_dict_put_c_key_string(
	    pcd, key, (const byte*)(value ? "true" : "false"), (value ? 4 : 5));
}
int
...
cos_dict_move_all(cos_dict_t* pcdto, cos_dict_t* pcdfrom)
{
	...
	while(pcde) {
		...
		if(cos_dict_find(pcdto, pcde->key.data, pcde->key.size)) {
			...
			cos_dict_element_free(pcdfrom, pcde,
			                      "cos_dict_move_all_from");
		} else {
			/* Move the element. */
			pcde->next = head;
			head = pcde;
...
cos_dict_find(const cos_dict_t* pcd, const byte* key_data, uint key_size)
{
	...
	for(; pcde; pcde = pcde->next)
		if(!bytes_compare(key_data, key_size, pcde->key.data,
		                  pcde->key.size))
			return &pcde->value;
	return 0;
}
...
int
cos_dict_equal(const cos_object_t* pco0, const cos_object_t* pco1,
               gx_device_pdf* pdev)
{
	...
	for(; pcde1; pcde1 = pcde1->next) {
		if(cos_dict_find(pcd0, pcde1->key.data, pcde1->key.size) ==
		   NULL)
			return false;
	}
	for(; pcde0; pcde0 = pcde0->next) {
		const cos_value_t* v =
		    cos_dict_find(pcd1, pcde0->key.data, pcde0->key.size);
		int code;
...
private
param_proc_xmit_typed(cos_param_put_typed);
private
const gs_param_list_procs cos_param_list_writer_procs = {
    cos_param_put_typed,       NULL /* begin_collection */,
    NULL /* end_collection */, NULL /* get_next_key */,
    gs_param_request_default,  gs_param_requested_default};
private
int
cos_param_put_typed(gs_param_list* plist, gs_param_name pkey,
                    gs_param_typed_value* pvalue)
{
	cos_param_list_writer_t* const pclist = (cos_param_list_writer_t*)plist;
	...
	case gs_param_type_float_array: {
		...
		for(i = 0; i < pvalue->value.ia.size; ++i)
			CHECK(
			    cos_array_add_real(pca, pvalue->value.fa.data[i]));
	}
		goto av;
	...
	key_chars[0] = '/';
	return cos_dict_put_no_copy(pclist->pcd, key_chars, key_len + 1,
	                            &value);
}
...
private
cos_proc_release(cos_stream_release);
private
cos_proc_write(cos_stream_write);
private
cos_proc_equal(cos_stream_equal);
...
private
int
cos_stream_equal(const cos_object_t* pco0, const cos_object_t* pco1,
                 gx_device_pdf* pdev)
{
		...
		return false;
	{
		...
		cos_stream_piece_t* pcsp0 = pcs0->pieces,
		                    * pcsp1 = pcs1->pieces;
		long position_save = ftell(sfile);

		for(; pcsp0 && pcsp1;
		    pcsp0 = pcsp0->next, pcsp1 = pcsp1->next) {
			...
			if(size0 != size1)
				goto notequal;
			for(; size0; position0 += size1, position1 += size1,
			             size0 -= size1) {
...
cos_stream_add(cos_stream_t* pcs, uint size)
{
	...
	if(prev != 0 && prev->position + prev->size + size == position) {
		...
		gs_memory_t* mem = pdev->pdf_memory;
		cos_stream_piece_t* pcsp =
		    gs_alloc_struct(mem, cos_stream_piece_t,
		                    &st_cos_stream_piece, "cos_stream_add");
...
/* Warning : this function can't release pieces if another stream is written
 * after them. */
int
...
gs_private_st_suffix_add4(st_cos_write_stream_state, cos_write_stream_state_t,
                          "cos_write_stream_state_t", cos_ws_state_enum_ptrs,
                          cos_ws_state_reloc_ptrs, st_stream_state, pcs, pdev,
                          s, target);
...
private
const stream_procs cos_s_procs = {
    s_std_noavailable, s_std_noseek,           s_std_write_reset,
    s_std_write_flush, cos_write_stream_close, cos_write_stream_process};
private
const stream_template cos_write_stream_template = {
                       ...
                       client_name_t cname)
{
	...
	cos_write_stream_state_t* ss = (cos_write_stream_state_t*)s_alloc_state(
	    mem, &st_cos_write_stream_state, cname);
