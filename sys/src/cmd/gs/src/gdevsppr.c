...
/* Copyright (C) 1992, 1993, 1996, 1998, 1999 Aladdin Enterprises.  All rights
  reserved.
...
private
dev_proc_open_device(sparc_open);
private
dev_proc_print_page(sparc_print_page);
...
const gx_device_printer far_data gs_sparc_device =
    prn_device(prn_sparc_procs, "sparc", DEFAULT_WIDTH_10THS,
               DEFAULT_HEIGHT_10THS, 400, 400, 0, 0, 0, 0, 1, sparc_print_page);
...
char* errmsg[] = {"EMOTOR",  "EROS",    "EFUSER", "XEROFAIL", "ILCKOPEN",
                  "NOTRAY",  "NOPAPR",  "XITJAM", "MISFEED",  "WDRUMX",
                  "WDEVEX",  "NODRUM",  "NODEVE", "EDRUMX",   "EDEVEX",
                  "ENGCOLD", "TIMEOUT", "EDMA",   "ESERIAL"};
...
sparc_print_page(gx_device_printer* pdev, FILE* prn)
{
	...
	lpvipage.resolution =
	    (pdev->x_pixels_per_inch == 300 ? DPI300 : DPI400);
	...
	out_buf =
	    gs_malloc(pdev->memory, out_size, 1, "sparc_print_page: out_buf");
	...
	while(write(fileno(prn), out_buf, out_size) != out_size) {
		...
		case 0:
			if(warning == 0) {
				errprintf("sparc_print_page: Printer Problem "
				          "with unknown reason...");
				dflush();
				warning = 1;
			}
			...
			break;
		case ENGWARN:
			errprintf("sparc_print_page: Printer-Warning: %s...",
			...
			break;
		case ENGFATL:
			errprintf("sparc_print_page: Printer-Fatal: %s\n",
			          err_code_string(lpvierr.err_code));
			return -1;
		case EDRVR:
			errprintf(
			...
			return -1;
		default:
			errprintf("sparc_print_page: Unknown "
			          "err_type=%d(err_code=%d)\n",
	...
	gs_free(pdev->memory, out_buf, out_size, 1,
	        "sparc_print_page: out_buf");
	return 0;
}
