...
#define public_st_device_mask_clip() /* in gxmclip.c */                        \
	gs_public_st_composite_use_final(                                      \
	    st_device_mask_clip, gx_device_mask_clip, "gx_device_mask_clip",   \
	    device_mask_clip_enum_ptrs, device_mask_clip_reloc_ptrs,           \
	    gx_device_finalize)
...
int gx_mask_clip_initialize(gx_device_mask_clip* cdev,
                            ...
                            const gx_bitmap* bits, gx_device* tdev, int tx,
                            int ty, gs_memory_t* mem);
...
#define setup_mask_copy_mono(cdev, color, mcolor0, mcolor1)                    \
	...
	if(cdev->mdev.base == 0) {                                             \
		 ...
		 * copy_mono.                                                  \
		 */                                                            \
		return gx_default_copy_mono(dev, data, sourcex, raster, id, x, \
		                            y, w, h, color0, color1);          \
	}                                                                      \
	if(color1 != gx_no_color_index) {                                      \
		if(color0 != gx_no_color_index) {                              \
			/* Pre-fill with color0. */                            \
			code = (*dev_proc(dev, fill_rectangle))(dev, x, y, w,  \
			                                        h, color0);    \
			if(code < 0)                                           \
				return code;                                   \
		}                                                              \
