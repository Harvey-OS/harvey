...
load_dll(GSDLL* gsdll, char* last_error, int len)
{
	...
	*p = '\0';
	...
	if(gsdll->hmodule < (HINSTANCE)HINSTANCE_ERROR) {
		...
		sprintf(fullname, "Can't load DLL, LoadLibrary error code %ld",
		        err);
	...
	if(gsdll->revision(&rv, sizeof(rv)) != 0) {
		sprintf(fullname, "Unable to identify Ghostscript DLL revision "
		                  "- it must be newer than needed.\n");
		strncpy(last_error, fullname, len - 1);
		unload_dll(gsdll);
		return 1;
	}
	...
	gsdll->new_instance = (PFN_gsapi_new_instance)GetProcAddress(
	    gsdll->hmodule, "gsapi_new_instance");
	...
	gsdll->delete_instance = (PFN_gsapi_delete_instance)GetProcAddress(
	    gsdll->hmodule, "gsapi_delete_instance");
	...
	gsdll->set_stdio = (PFN_gsapi_set_stdio)GetProcAddress(
	    gsdll->hmodule, "gsapi_set_stdio");
	...
	gsdll->set_display_callback =
	    (PFN_gsapi_set_display_callback)GetProcAddress(
	        gsdll->hmodule, "gsapi_set_display_callback");
	...
	gsdll->init_with_args = (PFN_gsapi_init_with_args)GetProcAddress(
	    gsdll->hmodule, "gsapi_init_with_args");
	...
	gsdll->run_string = (PFN_gsapi_run_string)GetProcAddress(
	    gsdll->hmodule, "gsapi_run_string");
	...
	gsdll->exit =
	    (PFN_gsapi_exit)GetProcAddress(gsdll->hmodule, "gsapi_exit");
	if(gsdll->exit == NULL) {
	...
	gsdll->set_visual_tracer = (PFN_gsapi_set_visual_tracer)GetProcAddress(
	    gsdll->hmodule, "gsapi_set_visual_tracer");
