...
gs_public_st_basic_super(st_pdf_sub_font_descriptor, pdf_sub_font_descriptor_t,
                         "pdf_sub_font_descriptor_t",
                         pdf_sub_font_descriptor_ptrs,
                         pdf_sub_font_descriptor_data, &st_pdf_resource, 0);
                            ...
                            const pdf_font_descriptor_common_t* pfd)
{
	...
	pdf_put_name(pdev, pfd->values.FontName.data,
	             pfd->values.FontName.size);
...
#define DESC_INT(str, memb)                                                    \
	{                                                                      \
		str, gs_param_type_int,                                        \
		    offset_of(pdf_font_descriptor_common_t, values.memb)       \
	}
		static const gs_param_item_t required_items[] = {
		    ...
		    DESC_INT("StemV", StemV), gs_param_item_end};
		static const gs_param_item_t optional_items[] = {
		    ...
		    DESC_INT("StemH", StemH), DESC_INT("XHeight", XHeight),
		    gs_param_item_end};
#undef DESC_INT
                          ...
                          gs_font_base* font, bool embed)
{
	...
	int code = pdf_base_font_alloc(
	    pdev, &pbfont, font,
	    (font->orig_FontMatrix.xx == 0 && font->orig_FontMatrix.xy == 0
	         ? &font->FontMatrix
	         : &font->orig_FontMatrix),
	    false, !embed);

	if(code < 0)
		return code;
	code = pdf_alloc_resource(pdev, resourceFontDescriptor, font->id,
	                          (pdf_resource_t**)&pfd, 0L);
	if(code < 0) {
 ...
 */
gs_string*
pdf_font_descriptor_name(pdf_font_descriptor_t* pfd)
{
	return &pfd->common.values.FontName;
}

/*
 * Return the (copied, subset or complete) font associated with a
 * FontDescriptor.
 ...
 */
gs_string*
pdf_font_descriptor_base_name(const pdf_font_descriptor_t* pfd)
{
...
pdf_compute_font_descriptor(pdf_font_descriptor_t* pfd)
{
	...
	int members = (GLYPH_INFO_WIDTH0 << wmode) | GLYPH_INFO_BBOX |
	...
	if(is_cid && bfont->FontBBox.p.x != bfont->FontBBox.q.x &&
	   bfont->FontBBox.p.y != bfont->FontBBox.q.y) {
		int scale = (bfont->FontType == ft_TrueType ||
		                     bfont->FontType == ft_CID_TrueType
		                 ? 1000
		                 : 1);
...
#define CONSIDER_FONT_SYMBOLIC(bfont)                                          \
	 ...
	 */
	notdef = GS_NO_GLYPH;
	for(index = 0; (code = bfont->procs.enumerate_glyph(
	                    (gs_font*)bfont, &index,
	                    (is_cid ? GLYPH_SPACE_INDEX : GLYPH_SPACE_NAME),
	                    &glyph)) >= 0 &&
	                   index != 0;) {
		gs_glyph_info_t info;
		gs_const_string gname;

		code = bfont->procs.glyph_info((gs_font*)bfont, glyph, pmat,
		                               members, &info);
		if(code == gs_error_VMerror)
			return code;
		if(code < 0) {
			/*
			 * Since this function may be indirtectly called from
			 * gx_device_finalize,
			 * we are unable to propagate error code to the
			 * interpreter.
			 * Therefore we skip it here hoping that few errors can
			 * be
			 * recovered by the integration through entire glyph
			 * set.
			 */
			continue;
		}
		if(members & GLYPH_INFO_BBOX) {
			/* rect_merge(desc.FontBBox, info.bbox); Expanding due
			 * to type cast :*/
			if(info.bbox.p.x < desc.FontBBox.p.x)
				desc.FontBBox.p.x = (int)info.bbox.p.x;
			if(info.bbox.q.x > desc.FontBBox.q.x)
				desc.FontBBox.q.x = (int)info.bbox.q.x;
			if(info.bbox.p.y < desc.FontBBox.p.y)
				desc.FontBBox.p.y = (int)info.bbox.p.y;
			if(info.bbox.q.y > desc.FontBBox.q.y)
				desc.FontBBox.q.y = (int)info.bbox.q.y;
			if(!info.num_pieces)
				desc.Ascent =
				    max(desc.Ascent, (int)info.bbox.q.y);
		}
		if(notdef == GS_NO_GLYPH &&
		   gs_font_glyph_is_notdef(bfont, glyph)) {
			notdef = glyph;
		...
		if(gname.data[0] >= 'A' && gname.data[0] <= 'Z') {
			...
			int y0 = (int)(info.bbox.p.y),
			    y1 = (int)(info.bbox.q.y);
	...
	if(!(desc.Flags & FONT_IS_SYMBOLIC)) {
		desc.Flags |=
		    FONT_IS_ADOBE_ROMAN; /* required if not symbolic */
		...
		if(have_colon && have_period) {
			...
			if(angle < -30)
				...
				angle = 30;
			/*
			 * For script or embellished fonts, we can get an angle
			 * that is
		...
		if(have_I) {
			...
			desc.StemV = (int)wdot;
			if(wI > wcolon * 2.5 ||
			   wI > (bbox_period.q.y - bbox_period.p.y) * 0.25)
	...
	if(!(desc.Flags & (FONT_IS_SYMBOLIC | FONT_IS_ALL_CAPS)) &&
	   (small_descent > desc.Descent / 3 ||
	    desc.XHeight > small_height * 0.9))
...
pdf_write_FontDescriptor(gx_device_pdf* pdev, pdf_font_descriptor_t* pfd)
{
	...
	case ft_CID_TrueType:
		if(pdf_do_subset_font(pdev, pfd->base_font, pfd->common.rid)) {
			code =
			    pdf_write_CIDSet(pdev, pfd->base_font, &cidset_id);
			if(code < 0)
				return code;
		}
	default:
...

	{
		...
		if(pfd->embed && pfd->FontType == ft_TrueType &&
		   pdf_do_subset_font(pdev, pfd->base_font, pfd->common.rid))
			fd.values.Flags =
			    (fd.values.Flags & ~(FONT_IS_ADOBE_ROMAN)) |
			    FONT_IS_SYMBOLIC;
	...
	{
		const cos_object_t* pco =
		    (const cos_object_t*)pdf_get_FontFile_object(
		        pfd->base_font);
		if(pco != NULL) {
 ...
 */
int
pdf_release_FontDescriptor_components(gx_device_pdf* pdev,
                                      pdf_font_descriptor_t* pfd)
{
	gs_free_object(pdev->pdf_memory, pfd->base_font,
