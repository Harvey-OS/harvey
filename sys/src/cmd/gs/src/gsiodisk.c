...
/* Function prototypes */
private
iodev_proc_init(iodev_diskn_init);
private
iodev_proc_fopen(iodev_diskn_fopen);
private
iodev_proc_delete_file(diskn_delete);
private
iodev_proc_rename_file(diskn_rename);
private
iodev_proc_file_status(diskn_status);
private
iodev_proc_enumerate_files(diskn_enumerate_files);
private
iodev_proc_enumerate_next(diskn_enumerate_next);
private
iodev_proc_enumerate_close(diskn_enumerate_close);
private
iodev_proc_get_params(diskn_get_params);
private
iodev_proc_put_params(diskn_put_params);
...
gs_private_st_ptrs2(st_map_file_enum, struct map_file_enum_s, "map_file_enum",
                    map_file_enum_enum_ptrs, map_file_enum_reloc_ptrs, pattern,
                    root);

private
void* map_file_enum_init(gs_memory_t*, const char*, const char*);
private
bool map_file_enum_next(void*, char*);
private
void map_file_enum_close(void*);
private
bool map_file_name_get(const char*, const char*, char*);
private
void map_file_name_add(const char*, const char*);
private
void map_file_name_ren(const char*, const char*, const char*);
private
void map_file_name_del(const char*, const char*);
                  ...
                  FILE** pfile, char* rfname, uint rnamelen)
{
	...
	return iodev_os_fopen(iodev_default, realname, access, pfile, rfname,
	                      rnamelen);
}
...
diskn_status(gx_io_device* iodev, const char* fname, struct stat* pstat)
{
	...
	return (stat((char*)realname, pstat) < 0 ? gs_error_undefinedfilename
	                                         : 0);
}
	...
	if(rootstr.data) {
		...
		if(!pstate->root || pstate->root_size <= rootstr.size) {
			if(pstate->root) /* The current storge is too small */
				gs_free_object(pstate->memory, pstate->root,
				               "diskn(rootdir)");
			pstate->root = (char*)gs_alloc_byte_array(
			    pstate->memory, gp_file_name_sizeof, sizeof(char),
			    "diskn(rootdir)");
			if(!pstate->root)
				return gs_error_VMerror;
			pstate->root_size = rootstr.size + 1;
			/* Now allow enumeration of files on the disk */
			iodev->procs.enumerate_files =
			    diskn_enumerate_files_init;
		}
...
private
void
MapFileRename(const char* rootpath, const char* newfilename,
 ...
 * rootpath        char*   string of the base path where in disk0 reside
 *(C:\PS\Disk0\)
 * name            char*   string to search pattern for full match
 *(font\Ryumin-Light)
 ...
 * root_name       char*   string of the base path where in disk0 reside (e.g.
 *C:\PS\Disk0\)
 * search_pattern  char*   string to search pattern (font\*) or full match
 *                          (e.g. font\Ryumin-Light) or NULL
 *                          NULL means all files
 * Returns:		    NULL if error, else pointer to enumeration
 *structure.
 */
private
void*
map_file_enum_init(gs_memory_t* mem, const char* root_name,
                   const char* search_pattern)
{
	int file_version;
	map_file_enum* mapfileenum =
	    gs_alloc_struct(mem, map_file_enum, &st_map_file_enum,
	...
	if(search_pattern) {
		mapfileenum->pattern =
		    (char*)gs_alloc_bytes(mem, strlen(search_pattern) + 1,
...
map_file_enum_close(void* enum_mem)
{
	...
	if(mapfileenum->pattern)
		gs_free_object(mem, mapfileenum->pattern,
		               "diskn_enum_init(pattern)");
 ...
 * Id will be a lowest one if same psname exists more than one in map file. See
 *MapToFile
 * for detail.
...
map_file_name_del(const char* root_name, const char* Fname)
{
	...
	if(d != -1) { /* if the file exists ... */
		...
		if(oldMap != NULL &&
		   (!MapFileReadVersion(oldMap, &file_version) ||
		    file_version != MAP_FILE_VERSION)) {
			fclose(oldMap);
			oldMap = NULL;
		}
		...
		/* Copy every line of the map file except the one with given
		 * name */
 ...
 * map_file_name_ren
 *
 * renames the oldname into newname in the mapping table. newname must not exist
 *and must be
...
private
void
map_file_name_ren(const char* root_name, const char* oldname,
                  const char* newname)
{
	/*  search for target entry */
	...
	if(d != -1) { /* if target exists ... */
		...
		MapFileWriteVersion(
		    newMap, MAP_FILE_VERSION); /* Copy the version number */
		while(MapFileRead(oldMap, name, &d))
