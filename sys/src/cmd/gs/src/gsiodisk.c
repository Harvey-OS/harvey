...
#define diskn(varname, diskname)                                                                                                                                                                                                                          \
	const gx_io_device varname =                                                                                                                                                                                                                      \
	    {                                                                                                                                                                                                                                             \
	     diskname, "FileSystem", {iodev_diskn_init, iodev_no_open_device, NULL /* no longer used */, iodev_diskn_fopen, iodev_os_fclose, diskn_delete, diskn_rename, diskn_status, iodev_no_enumerate_files, /* Only until we have a root location */ \
				      diskn_enumerate_next, diskn_enumerate_close, diskn_get_params, diskn_put_params}}
...
MapToFile(const char *rootpath, const char *name)
{
	...
	if(MapFileReadVersion(mapfile, &file_version) && file_version == MAP_FILE_VERSION) {
...
map_file_enum_init(gs_memory_t *mem, const char *root_name, const char *search_pattern)
{
	...
	if(mapfileenum->stream != NULL && (!MapFileReadVersion(mapfileenum->stream, &file_version) || file_version != MAP_FILE_VERSION)) {
...
map_file_name_del(const char *root_name, const char *Fname)
{
	...
	if(d != -1) { /* if the file exists ... */
		...
		if(oldMap != NULL && (!MapFileReadVersion(oldMap, &file_version) || file_version != MAP_FILE_VERSION)) {
			fclose(oldMap);
			oldMap = NULL;
		}
		if(oldMap == NULL) {
