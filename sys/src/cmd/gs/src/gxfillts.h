...
private
inline int
TEMPLATE_slant_into_trapezoids(const line_list* ll, const active_line* flp,
                               const active_line* alp, fixed y, fixed y1)
{
	 ...
	 * Letting A = _fixed_pixround_v - below and B = _fixed_pixround_v +
	 *above,
	 * we can rewrite this as
	 *	fixed2int(fixed_fraction(y) + A) != fixed2int(fixed_fraction(y)
	 *+ B)
	 * Because of the range constraints given above, this is true precisely
	 *when
	 *	fixed_fraction(y) + A < fixed_1 && fixed_fraction(y) + B >=
	 *fixed_1
	 * or equivalently
	...
	if(le.end.x <= le.start.x) {
		if(re.end.x >= re.start.x) { /* Top wider than bottom. */
			...
			if(ADJUSTED_Y_SPANS_PIXEL(y1)) {
				...
				code = LOOP_FILL_RECTANGLE_DIRECT(
				    fo, xli,
				    fixed2int_pixround(y1 - fo->adjust_below),
				    fixed2int_var_pixround(alp->x_next +
				                           fo->adjust_right) -
				        xli,
				    1);
				vd_rect(flp->x_next - fo->adjust_left,
				        y1 - fo->adjust_below,
				        alp->x_next + fo->adjust_right, y1, 1,
				        VD_TRAP_COLOR);
			}
		...
		if(re.end.x <= re.start.x) { /* Bottom wider than top. */
			if(ADJUSTED_Y_SPANS_PIXEL(y)) {
				INCR(afill);
				xli = fixed2int_var_pixround(flp->x_current -
				                             fo->adjust_left);
				code = LOOP_FILL_RECTANGLE_DIRECT(
				    fo, xli,
				    fixed2int_pixround(y - fo->adjust_below),
				    fixed2int_var_pixround(alp->x_current +
				                           fo->adjust_right) -
				        xli,
				    1);
				vd_rect(flp->x_current - fo->adjust_left,
				        y - fo->adjust_below,
				        alp->x_current + fo->adjust_right, y, 1,
				        VD_TRAP_COLOR);
				if(code < 0)
					return code;
			}
