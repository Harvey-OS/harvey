	...
	for(;;) {
		...
		for(;;) {
			...
			case 1: /* font change */
				...
				char_code =
				    gx_current_char((gs_text_enum_t*)&curr);
				new_font =
				    curr.fstack.items[curr.fstack.depth].font;
				...
				prev_font = new_font;
				psmat =
				    &curr.fstack.items[curr.fstack.depth - 1]
				         .font->FontMatrix;
		...
		if(buf_index) {
			 ...
			 */
			/*
			 * The FontMatrix of leaf descendant fonts is not
			 * updated
			...
			out.fstack.depth = 0;
			out.fstack.items[out.fstack.depth].font =
			    out.current_font = prev_font;

			/* Provide the decoded space character : */
			out.text.space.s_char = space_char;

			gs_matrix_multiply(&prev_font->FontMatrix, psmat,
			                   &fmat);
			code = pdf_encode_process_string(&out, &str, NULL,
			                                 &fmat, &text_state);
			if(code < 0)
				return code;
			curr.xy_index =
			    out.xy_index; /* pdf_encode_process_string advanced
			                     it. */
			gs_text_enum_copy_dynamic(pte, (gs_text_enum_t*)&prev,
			                          true);
			if(return_width) {
 ...
 */
private
const int8_t* const standard_cmap_names[] = {
    /* The following were added in PDF 1.4. */
    "GBKp-EUC-H", "GBKp-EUC-V", "GBK2K-H", "GBK2K-V", "HKscs-B5-H",
    "HKscs-B5-V",
#define END_PDF14_CMAP_NAMES_INDEX 6
    ...
    "GBK-EUC-V", "UniGB-UCS2-H", "UniGB-UCS2-V",
    ...
    "Ext-RKSJ-V", "H", "V", "UniJIS-UCS2-H", "UniJIS-UCS2-V",
                     ...
                     const gs_cmap_t* pcmap, int font_index_only)
{
	...
	if(*pcmn == 0 && !is_identity) { /* not standard */
		pcmres = pdf_find_resource_by_gs_id(
		    pdev, resourceCMap, pcmap->id + font_index_only);
		if(pcmres == 0) {
	...
	if(pcmap->from_Unicode) {
		...
		if(gs_cmap_enum_next_range(&renum) == 0 &&
		   renum.range.size == 2 &&
		   gs_cmap_enum_next_range(&renum) == 1) {
			/*
			 * Exactly one code space range, of size 2.  Add an
			 * identity
			 * ToUnicode CMap.
			 */
			if(!pdev->Identity_ToUnicode_CMaps[pcmap->WMode]) {
				/* Create and write an identity ToUnicode CMap
				 * now. */
				gs_cmap_t* pidcmap;

				code = gs_cmap_create_char_identity(
				    &pidcmap, 2, pcmap->WMode, pdev->memory);
				if(code < 0)
					return code;
				pidcmap->CMapType = 2; /* per PDF Reference */
				code = pdf_cmap_alloc(
				    pdev, pidcmap,
				    &pdev->Identity_ToUnicode_CMaps
				         [pcmap->WMode],
				    -1);
				if(code < 0)
					return code;
			}
			pdfont->res_ToUnicode =
...
scan_cmap_text(pdf_text_enum_t* pte)
{
	...
	gs_font_type0* const font =
	    (gs_font_type0*)pte->orig_font; /* Type 0, fmap_CMap */
	...
	for(;;) {
		...
		do {
			...
			case ft_CID_TrueType:
				break;
			default:
				/* An unsupported case, fall back to default
				 * implementation. */
				return_error(gs_error_rangecheck);
			}
			...
			p.s[0].glyph = glyph;
			p.s[0].chr = cid;
			code = pdf_obtain_cidfont_resource(pdev, subfont,
			                                   &pdsubf, &p);
			if(code < 0)
				return code;
			...
			code = pdf_attached_font_resource(
			    pdev, (gs_font*)subfont, &pdsubf, &glyph_usage,
			    &real_widths, &char_cache_size, &width_cache_size);
			if(code < 0)
				return code;
			pfd = pdsubf->FontDescriptor;
			code =
			    pdf_resize_resource_arrays(pdev, pdsubf, cid + 1);
			if(code < 0)
				return code;
			code = pdf_obtain_cidfont_widths_arrays(
			    pdev, pdsubf, wmode, &w, &w0, &v);
			if(code < 0)
				return code;
			{
				pdf_font_resource_t* pdfont;

				code = pdf_obtain_parent_type0_font_resource(
				    pdev, pdsubf, &font->data.CMap->CMapName,
				    &pdfont);
				if(code < 0)
					return code;
				if(pdf_is_CID_font(subfont)) {
					/* Since PScript5.dll creates
					GlyphNames2Unicode with character codes
					instead CIDs, and with the
					WinCharSetFFFF-H2 CMap
					character codes appears different than
					CIDs (Bug 687954),
					pass the character code intead the CID.
					*/
					code = pdf_add_ToUnicode(
					    pdev, subfont, pdfont,
					    chr + GS_MIN_CID_GLYPH, chr);
				} else
					code = pdf_add_ToUnicode(
					    pdev, subfont, pdfont, glyph, cid);
				if(code < 0)
					return code;
			}
			/* We can't check pdsubf->used[cid >> 3] here,
			   because it mixed data for different values of WMode.
			   Perhaps pdf_font_used_glyph returns fast with reused
			   glyphs.
			 */
			code = pdf_font_used_glyph(pfd, glyph,
			                           (gs_font_base*)subfont);
			if(code == gs_error_rangecheck) {
				if(!(pdsubf->used[cid >> 3] &
				     (0x80 >> (cid & 7)))) {
					char buf[gs_font_name_max + 1];
					int l = min(sizeof(buf) - 1,
					            subfont->font_name.size);

					memcpy(buf, subfont->font_name.chars,
					       l);
					buf[l] = 0;
					eprintf2("Missing glyph CID=%d in the "
					         "font %s . The output PDF may "
					         "fail with some viewers.\n",
					         cid, buf);
					pdsubf->used[cid >> 3] |=
					    0x80 >> (cid & 7);
				}
			...
			if(cid >= char_cache_size || cid >= width_cache_size)
				return_error(
				    gs_error_unregistered); /* Must not happen
				                               */
			if(code == 0 /* just copied */ ||
			   pdsubf->Widths[cid] == 0) {
				pdf_glyph_widths_t widths;

				code = pdf_glyph_widths(
				    pdsubf, wmode, glyph, (gs_font*)subfont,
				    &widths,
				...
				if(wmode) {
					/* Since AR5 use W or DW to compute the
					   x-coordinate of
					   v-vector, comupte and store the glyph
					   width for WMode 0. */
					/* fixme : skip computing real_width
					 * here. */
					code = pdf_glyph_widths(
					    pdsubf, 0, glyph, (gs_font*)subfont,
					    &widths, pte->cdevproc_callout
					                 ? pte->cdevproc_result
					                 : NULL);
					if(code < 0)
						return code;
				...
				if(pdsubf->u.cidfont.CIDToGIDMap != 0) {
					gs_font_cid2* subfont2 =
					    (gs_font_cid2*)subfont;

					pdsubf->u.cidfont.CIDToGIDMap[cid] =
					    subfont2->cidata.CIDMap_proc(
					        subfont2, glyph);
				}
			}
			pdsubf->used[cid >> 3] |= 0x80 >> (cid & 7);
			if(wmode)
				pdsubf->u.cidfont.used2[cid >> 3] |=
				    0x80 >> (cid & 7);
			if(pte->cdevproc_callout) {
				/* Only handle a single character because its
				   width is stored
				   into pte->cdevproc_result, and
				   process_text_modify_width neds it.
		...
		if(break_index > index) {
			...
			int xy_index_step =
			    (pte->text.x_widths !=
			                 NULL && /* see gs_text_replaced_width
			                            */
			             pte->text.x_widths == pte->text.y_widths
			         ? 2
			         : 1);
			...
			if(code < 0)
				return code;
			code =
			    gs_matrix_multiply(&subfont0->FontMatrix, &m1, &m2);
			if(code < 0)
				return code;
			code = gs_matrix_multiply(&m2, &font->FontMatrix, &m3);
			/* We thought that it should be
			 * gs_matrix_multiply(&font->FontMatrix, &m2, &m3); */
			if(code < 0)
				return code;
			code = pdf_obtain_parent_type0_font_resource(
			    pdev, pdsubf0, &font->data.CMap->CMapName, &pdfont);
			if(code < 0)
				return code;
			if(!pdfont->u.type0.Encoding_name[0]) {
				 ...
				 */
				code = attach_cmap_resource(
				    pdev, pdfont, font->data.CMap, font_index0);
				if(code < 0)
					return code;
			}
			pdf_set_text_wmode(pdev, font->WMode);
			code = pdf_update_text_state(
			    &text_state, (pdf_text_enum_t*)pte, pdfont, &m3);
			if(code < 0)
				return code;
			...
			pte->xy_index = 0;
			code = process_text_modify_width(
			    (pdf_text_enum_t*)pte, (gs_font*)font, &text_state,
			    &str, &wxy);
			if(pte->text.x_widths != NULL)
				pte->text.x_widths -= xy_index * xy_index_step;
			if(pte->text.y_widths != NULL)
...
process_cid_text(gs_text_enum_t* pte, void* vbuf, uint bsize)
{
	...
	gs_matrix_multiply(&scale_matrix, &scaled_font->FontMatrix,
	                   &scale_matrix);
	...
	if(pdsubf->u.cidfont.glyphshow_font_id != 0)
		font0 = (gs_font_type0*)gs_find_font_by_id(
		    font->dir, pdsubf->u.cidfont.glyphshow_font_id,
		    &scaled_font->FontMatrix);
	if(font0 == NULL) {
