...
/* Forward references */
private
font_proc_define_font(ztype0_define_font);
private
font_proc_make_font(ztype0_make_font);
private
int ensure_char_entry(i_ctx_t*, os_ptr, const char*, byte*, int);
...
zbuildfont0(i_ctx_t* i_ctx_p)
{
	...
	for(i = 0; i < data.fdep_size; i++) {
		...
		if(psub->FontType == ft_composite) {
			const gs_font_type0* const psub0 =
			    (const gs_font_type0*)psub;
	...
	case fmap_double_escape: /* need EscChar */
		code = ensure_char_entry(i_ctx_p, op, "EscChar", &data.EscChar,
		                         255);
		break;
	case fmap_shift: /* need ShiftIn & ShiftOut */
		code = ensure_char_entry(i_ctx_p, op, "ShiftIn", &data.ShiftIn,
		                         15);
		if(code >= 0)
			code = ensure_char_entry(i_ctx_p, op, "ShiftOut",
			                         &data.ShiftOut, 14);
		break;
	...
	}
	{
		...
		code = build_proc_name_refs(imemory, &build, "%Type0BuildChar",
		                            "%Type0BuildGlyph");
		if(code < 0)
			return code;
		code =
		    build_gs_font(i_ctx_p, op, (gs_font**)&pfont, ft_composite,
		                  &st_gs_font_type0, &build, bf_options_none);
	}
	if(code != 0)
		return code;
	...
	data.Encoding = (uint*)ialloc_byte_array(
	    data.encoding_size, sizeof(uint), "buildfont0(Encoding)");
	if(data.Encoding == 0) {
	...
	data.FDepVector = ialloc_struct_array(data.fdep_size, gs_font*,
...
private
int
ztype0_make_font(gs_font_dir* pdir, const gs_font* pfont, const gs_matrix* pmat,
                 gs_font** ppfont)
{
...
private
int
ensure_char_entry(i_ctx_t* i_ctx_p, os_ptr op, const char* kstr, byte* pvalue,
                  int default_value)
{
	ref* pentry;
...
const op_def zfont0_op_defs[] = {{"2.buildfont0", zbuildfont0}, op_def_end(0)};
