...
/* Forward references */
private
int zcopy_integer(i_ctx_t*);
private
int zcopy_interval(i_ctx_t*);
private
int copy_interval(i_ctx_t*, os_ptr, uint, os_ptr, client_name_t);
...
zget(i_ctx_t* i_ctx_p)
{
	...
	default: {
		...
		if(code <
		   0) { /* Might be a stackunderflow reported as typecheck. */
			if(code == e_typecheck)
...
zputinterval(i_ctx_t* i_ctx_p)
{
	...
	case t_string:
		...
		code =
		    copy_interval(i_ctx_p, opto, (uint)(opindex->value.intval),
		                  op, "putinterval");
		break;
	case t_astruct: {
...
private
int array_continue(i_ctx_t*), dict_continue(i_ctx_t*),
    string_continue(i_ctx_t*), packedarray_continue(i_ctx_t*);
private
int forall_cleanup(i_ctx_t*);
private
int
              ...
              uint index, os_ptr prfrom, client_name_t cname)
{
	...
	if(fromsize > r_size(prto) - index)
		return_error(e_rangecheck);
	switch(fromtype) {
	case t_array: { /* We have to worry about aliasing, */
		/* but refcpy_to_old takes care of it for us. */
		return refcpy_to_old(prto, index, prfrom->value.refs, fromsize,
		                     idmemory, cname);
	}
	case t_string: { /* memmove takes care of aliasing. */
		memmove(prto->value.bytes + index, prfrom->value.bytes,
		        fromsize);
	} break;
	case t_mixedarray:
	case t_shortarray: { /* We don't have to worry about aliasing, because
	                        */
