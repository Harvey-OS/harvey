...
int
param_put_enum(gs_param_list* plist, gs_param_name param_name, int* pvalue,
               const char* const pnames[], int ecode)
{
	gs_param_string ens;
...
/* Put a Boolean value. */
int
param_put_bool(gs_param_list* plist, gs_param_name param_name, bool* pval,
               int ecode)
{
	int code;
...
int
param_put_int(gs_param_list* plist, gs_param_name param_name, int* pval,
              int ecode)
{
	int code;
...
/* Put a long value. */
int
param_put_long(gs_param_list* plist, gs_param_name param_name, int32_t* pval,
               int ecode)
{
	int code;
...
param_list_copy(gs_param_list* plto, gs_param_list* plfrom)
{
	...
	while((code = param_get_next_key(plfrom, &key_enum, &key)) == 0) {
		...
		cc:
			...
			if((code = param_begin_write_collection(
			        plto, string_key, &copy.value.d, coll_type)) <
			       0 ||
			   (code = param_list_copy(copy.value.d.list,
			                           value.value.d.list)) < 0 ||
			   (code = param_end_write_collection(
			        plto, string_key, &copy.value.d)) < 0)
				break;
			code = param_end_read_collection(plfrom, string_key,
			                                 &value.value.d);
			break;
		case gs_param_type_string:
			value.value.s.persistent &= copy_persists;
			goto ca;
		case gs_param_type_name:
			value.value.n.persistent &= copy_persists;
			goto ca;
		case gs_param_type_int_array:
			value.value.ia.persistent &= copy_persists;
			goto ca;
		case gs_param_type_float_array:
			value.value.fa.persistent &= copy_persists;
			goto ca;
