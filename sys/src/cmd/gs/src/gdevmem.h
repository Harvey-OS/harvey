 ...
 */
#define declare_scan_ptr(ptr) DECLARE_SCAN_PTR_VARS(ptr, chunk*, draster)
#define DECLARE_SCAN_PTR_VARS(ptr, ptype, draster)                             \
	register ptype ptr;                                                    \
	uint draster
#define setup_rect(ptr) SETUP_RECT_VARS(ptr, chunk*, draster)
	...
	(rgb_depth >= 8 ? 255 : rgb_depth == 4                                 \
	                            ...
	                            : rgb_depth == 2                           \
	                                  ...
	                                  : rgb_depth == 1                     \
	                                        ? 1                            \
	                                        : (1 << gray_depth) - 1)
#define mem_full_alpha_device(name, rgb_depth, gray_depth, open,               \
                              map_rgb_color, map_color_rgb, copy_mono,         \
                              copy_color, fill_rectangle, map_cmyk_color,      \
                              copy_alpha, strip_tile_rectangle,                \
                              strip_copy_rop, get_bits_rectangle)              \
	{                                                                      \
		std_device_dci_body(                                           \
		    gx_device_memory, 0, name, 0, 0, 72, 72,                   \
		    (rgb_depth ? 3 : 0) +                                      \
		        (gray_depth ? 1 : 0), /* num_components */             \
		    ...
		    {open, /* differs */                                       \
		     mem_get_initial_matrix, gx_default_sync_output,           \
		     gx_default_output_page, mem_close,                        \
		     ...
		     gx_default_get_params, gx_default_put_params,             \
		     ...
		     copy_alpha,                /* differs */                  \
		     gx_default_get_band, gx_default_copy_rop,                 \
		     gx_default_fill_path, gx_default_stroke_path,             \
		     ...
		     gx_default_end_image, strip_tile_rectangle, /* differs */ \
		     ...
		     gx_default_get_hardware_params, gx_default_text_begin,    \
		     gx_default_finish_copydevice},                            \
		    0,                      /* target */                       \
		    mem_device_init_private /* see gxdevmem.h */               \
	}
#define mem_full_device(name, rgb_depth, gray_depth, open, map_rgb_color,      \
                        map_color_rgb, copy_mono, copy_color, fill_rectangle,  \
                        map_cmyk_color, strip_tile_rectangle, strip_copy_rop,  \
                        get_bits_rectangle)                                    \
	mem_full_alpha_device(name, rgb_depth, gray_depth, open,               \
	                      map_rgb_color, map_color_rgb, copy_mono,         \
	                      copy_color, fill_rectangle, map_cmyk_color,      \
	                      gx_default_copy_alpha, strip_tile_rectangle,     \
	                      strip_copy_rop, get_bits_rectangle)
#define mem_device(name, rgb_depth, gray_depth, map_rgb_color, map_color_rgb,  \
                   copy_mono, copy_color, fill_rectangle, strip_copy_rop)      \
...
#define mem_copy_byte_rect(mdev, base, sourcex, sraster, x, y, w, h)           \
	bytes_copy_rectangle(scan_line_base(mdev, y) + x_to_byte(x),           \
	                     (mdev)->raster, base + x_to_byte(sourcex),        \
	                     sraster, x_to_byte(w), h)
