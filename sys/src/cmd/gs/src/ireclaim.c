...
/* Forward references */
private
void gs_vmreclaim(gs_dual_memory_t*, bool);

/* Initialize the GC hook in the allocator. */
private
int ireclaim(gs_dual_memory_t*, int);
private
int
...
gs_vmreclaim(gs_dual_memory_t* dmem, bool global)
{
	...
	i_ctx_t* i_ctx_p = (i_ctx_t*)((char*)dmem - offset_of(i_ctx_t, memory));
	...
	for(i = (global ? i_vm_system : i_vm_local);
	    i < countof(dmem->spaces_indexed); ++i) {
...

	{
		...
		gs_unregister_root((gs_memory_t*)lmem, &context_root,
		                   "i_ctx_p root");
		i_ctx_p = ctxp;
		dmem = &i_ctx_p->memory;
	}
...
const op_def ireclaim_l2_op_defs[] = {op_def_end(ireclaim_init)};
