...
zcurrentdevice(i_ctx_t* i_ctx_p)
{
	...
	make_tav(op, t_device,
	         (mem == 0 ? avm_foreign : imemory_space(mem)) | a_all, pdevice,
	         dev);
	return 0;
}
...
{ /*
   ...
   */
	...
	int w, h;
	gs_get_bits_options_t options = GB_ALIGN_ANY | GB_RETURN_COPY |
	...
	if(r_has_type(op - 1, t_null)) {
		...
		static const gs_get_bits_options_t depths[17] = {
		    0,           GB_DEPTH_1, GB_DEPTH_2, 0, GB_DEPTH_4, 0,
		    0,           0,          GB_DEPTH_8, 0, 0,          0,
		    GB_DEPTH_12, 0,          0,          0, GB_DEPTH_16};
...
zmakewordimagedevice(i_ctx_t* i_ctx_p)
{
	...
	code = gs_makewordimagedevice(
	    &new_dev, &imat, (int)op[-3].value.intval, (int)op[-2].value.intval,
	    colors, colors_size, op->value.boolval, true, imemory);
	if(code == 0) {
		new_dev->memory = imemory;
...
zputdeviceparams(i_ctx_t* i_ctx_p)
{
	...
	if(code > 0 || (code == 0 && (dev->width != old_width ||
		...
		if(gs_currentdevice(igs) == dev) {
			...
			/* If the device wasn't closed, setdevice won't erase
			 * the page. */
			if(was_open && code >= 0)
				code = 1;
		}
...
zsetdevice(i_ctx_t* i_ctx_p)
{
	...
	if(dev->LockSafetyParams) { /* do additional checking if locked  */
		if(op->value.pdevice !=
		   dev) /* don't allow a different device    */
