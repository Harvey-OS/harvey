...
private
ulong
put_table(byte tab[16], const char* tname, uint32_t checksum, uint32_t offset,
          uint length)
{
...
write_range(stream* s, gs_font_type42* pfont, ulong start, uint length)
{
	...
	while(base < limit) {
		...
		while((code = pfont->data.string_proc(pfont, base, size,
		                                      &ptr)) < 0) {
			if(size <= 1)
				return code;
			size >>= 1;
...
{
	gs_glyph glyph =
	    font->procs.encode_char(font, (gs_char)ch, GLYPH_SPACE_NAME);
	int code;
	...
	if(glyph < gs_min_cid_glyph) {
		...
		if(ch >= 32 && ch <= 126)
			...
			return -1;
		mac_glyph =
		    gs_c_known_encode(mac_char, ENCODING_INDEX_MACGLYPH);
...
/* Write the table header and Macintosh sub-table (if any). */
	...
	{
		/*
		 * Punt.  Acrobat Reader 3 can't handle any other Mac table
		 * format.
		 * (AR3 for Linux doesn't seem to be able to handle Windows
		 * format,
          ...
          int wmode)
{
	...
	for(i = 0; i < pmtx->numMetrics; ++i) {
		...
		put_ushort(s,
		           limdbl2ushort(sbw[wmode + 2] * factor)); /* width */
		put_ushort(
		    s, limdbl2ushort(sbw[wmode] * factor)); /* lsb, may be <0 */
	}
...
update_OS_2(ttf_OS_2_t* pos2, uint first_glyph, int num_glyphs)
{
	...
	if(first_glyph >= 0xf000) {
		...
		memset(pos2->ulCodePageRanges, 0,
		       sizeof(pos2->ulCodePageRanges));
...
compute_post(gs_font* font, post_t* post)
{
	...
	for(i = 0, post->length = 32 + 2; i <= 255; ++i) {
		...
		if(mac_index != 0) {
			...
			post->glyphs[post->count].glyph_index =
			    glyph - GS_MIN_GLYPH_INDEX;
			post->count++;
		}
	}
	if(post->count) {
		...
		post->count = j;
		post->glyph_count =
		    post->glyphs[post->count - 1].glyph_index + 1;
	}
...
#define MAX_NUM_TABLES 40
	...
	bool writing_cid = (options & WRITE_TRUETYPE_CID) != 0,
	     ...
	     have_OS_2 = no_generate, have_post = no_generate;
	int have_hvhea[2];
	uint cmap_length = 0;
	ulong OS_2_start = 0;
	...
	for(i = numTables = 0; i < numTables_stored; ++i) {
		...
		start = u32(tab + 8);
		length = u32(tab + 12);
		/* Copy the table data now, since another ACCESS may invalidate
		 * it. */
...
#define W(a, b, c, d) (((a) << 24) + ((b) << 16) + ((c) << 8) + (d))
	...
	for(max_glyph = 0, glyf_length = 0;
		...
		if((code = pfont->data.get_outline(pfont, glyph_index,
			...
			/* Since indexToLocFormat==0 assumes even glyph lengths,
			   round it up here. If later we choose
			   indexToLocFormat==1,
...

	{
		...
		if(!writing_stripped) {
			offset = put_table(tab, "glyf", glyf_checksum, offset,
			                   glyf_length);
			tab += 16;

			offset = put_table(tab, "loca",
			                   loca_checksum[indexToLocFormat],
			                   offset, loca_length);
			tab += 16;
		}
		...
		if(!have_cmap) {
			cmap_length =
			    size_cmap(font, TT_BIAS, 256,
			              GS_MIN_GLYPH_INDEX + max_glyph, options);
			offset = put_table(tab, "cmap",
			                   0L /****** NO CHECKSUM ******/,
			                   offset, cmap_length);
			tab += 16;
		}
			...
			for(i = 0; i < 2; ++i)
				if(have_hvhea[i]) {
					offset = put_table(
					    tab, (i ? "vmtx" : "hmtx"),
					    0L /****** NO CHECKSUM ******/,
					    offset, size_mtx(pfont, &mtx[i],
					                     max_glyph, i));
					tab += 16;
				}

		if(!have_post) {
			offset = put_table(tab, "post",
			                   0L /****** NO CHECKSUM ******/,
			                   offset, post.length);
			tab += 16;
		}
	...
	for(i = 0; i < numTables; ++i) {
		...
		if(tab[8] < 0x40) {
			...
			case W('O', 'S', '/', '2'):
				if(!have_cmap) {
					/*
					 * Adjust the first and last character
					 * indices in the OS/2
					 * table to reflect the values in the
					 * generated cmap.
					 */
					const byte* pos2;
					ttf_OS_2_t os2;
			...
			case W('v', 'h', 'e', 'a'):
				if(generate_mtx) {
					write_range(s, pfont, start,
					            length - 2); /* 34 */
					put_ushort(
					    s, mtx[tab[0] == 'v'].numMetrics);
					break;
				}
			/* falls through */
			default:
	...
	if(!writing_stripped) {
		...
		for(offset = 0;
		    psf_enumerate_glyphs_next(penum, &glyph) != 1;) {
			gs_glyph_data_t glyph_data;

			glyph_data.memory = pfont->memory;
			if((code = pfont->data.get_outline(
			        pfont, glyph & ~GS_GLYPH_TAG, &glyph_data)) >=
			   0) {
				uint l = glyph_data.bits.size, zero = 0;

				if(!indexToLocFormat)
					l = (l + 1) & ~1;
				stream_write(s, glyph_data.bits.data,
				             glyph_data.bits.size);
				if(glyph_data.bits.size < l)
					stream_write(s, &zero, 1);
				offset += l;
				if_debug2('L',
				          "[L]glyf index = %u, size = %u\n", i,
				          glyph_data.bits.size);
				gs_glyph_data_free(&glyph_data,
				                   "psf_write_truetype_data");
			}
...
psf_write_cid2_stripped(stream* s, gs_font_cid2* pfont)
{
	...
	psf_enumerate_bits_begin(&genum, font, &no_subset, 0,
	                         GLYPH_SPACE_INDEX);
	return psf_write_truetype_data(
	    s, (gs_font_type42*)font,
	    WRITE_TRUETYPE_STRIPPED | WRITE_TRUETYPE_CID, &genum, true, NULL);
}
