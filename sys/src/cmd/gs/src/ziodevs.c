...
private
int s_stdin_read_process(stream_state*, stream_cursor_read*,
                     ...
                     stream_cursor_write* pw, bool last)
{
	...
	if(wcount <= 0)
		return 0;
	count =
	    gp_stdin_read((char*)pw->ptr + 1, wcount,
            ...
            gs_memory_t* mem)
{
	...
	if(file_is_invalid(s, &ref_stdout)) {
		...
		buf =
		    gs_alloc_bytes(mem, STDOUT_BUF_SIZE, "stdout_open(buffer)");
		if(s == 0 || buf == 0)
			return_error(e_VMerror);
            ...
            gs_memory_t* mem)
{
	...
	if(file_is_invalid(s, &ref_stderr)) {
		...
		buf =
		    gs_alloc_bytes(mem, STDERR_BUF_SIZE, "stderr_open(buffer)");
		if(s == 0 || buf == 0)
			return_error(e_VMerror);
