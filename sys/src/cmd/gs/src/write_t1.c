...
static void
write_word_entry(FAPI_font* a_fapi_font, WRF_output* a_output,
                 const char* a_name, int a_index, int a_divisor)
{
	...
	WRF_wbyte(a_output, ' ');
	/* Get the value and convert it from unsigned to signed by assigning it
	 * to a short. */
...
static void
write_array_entry_with_count(FAPI_font* a_fapi_font, WRF_output* a_output,
                             const char* a_name, int a_index, int a_count,
                             int a_divisor)
{
	int i;
	...
	for(i = 0; i < a_count; i++) {
		/* Get the value and convert it from unsigned to signed by
		 * assigning it to a short. */
...
static void
write_array_entry(FAPI_font* a_fapi_font, WRF_output* a_output,
                  const char* a_name, int a_index, int a_divisor)
{
	/* NOTE that the feature index must be preceded by the count index for
	 * this to work. */
...
static void
write_subrs(FAPI_font* a_fapi_font, WRF_output* a_output)
{
	int i;
...
static void
write_private_dictionary(FAPI_font* a_fapi_font, WRF_output* a_output)
{
	assert(!a_output->m_encrypt);
	a_output->m_encrypt = true;

	/* Write 4 bytes that must encrypt to at least one character that cannot
	 * be a valid hexadecimal character. */
...
write_main_dictionary(FAPI_font* a_fapi_font, WRF_output* a_output)
{
	...
	for(i = 0; i < 6; i++) {
		WRF_wfloat(a_output,
		           a_fapi_font->get_float(
		               a_fapi_font, FAPI_FONT_FEATURE_FontMatrix, i));
		WRF_wbyte(a_output, (byte)(i == 5 ? ']' : ' '));
	}
	WRF_wbyte(a_output, '\n');

	/* For now, specify standard encoding - I think GS will pass glyph
	 * indices so doesn't matter. */
	WRF_wstring(a_output, "/Encoding StandardEncoding def\n");

	WRF_wstring(a_output, "/FontBBox {");
	for(i = 0; i < 4; i++) {
		int16_t x = a_fapi_font->get_word(
		    a_fapi_font, FAPI_FONT_FEATURE_FontBBox, i);
		WRF_wint(a_output, x);
...
The PostScript is non-standard. The main dictionary contains no /Charstrings
dictionary. This
is supplied to FreeType using the incremental interface, There is also no
/PaintType entry. This is required
by PostScript but FreeType doesn't use it.
*/
int32_t
FF_serialize_type1_font(FAPI_font* a_fapi_font, unsigned char* a_buffer,
