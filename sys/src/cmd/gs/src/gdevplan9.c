...
private
WImage* initwriteimage(FILE* f, Rectangle r, char*, int depth);
private
...
private
dev_proc_map_rgb_color(plan9_rgb2cmap);
private
dev_proc_map_color_rgb(plan9_cmap2rgb);
private
dev_proc_open_device(plan9_open);
private
dev_proc_close_device(plan9_close);
private
dev_proc_print_page(plan9_print_page);
private
dev_proc_put_params(plan9_put_params);
private
dev_proc_get_params(plan9_get_params);
...
private
const gx_device_procs plan9_procs = prn_color_params_procs(
    plan9_open, gdev_prn_output_page, gdev_prn_close, plan9_rgb2cmap,
    plan9_cmap2rgb, gdev_prn_get_params, gdev_prn_put_params);
/*
                plan9_get_params, plan9_put_params);
*/

plan9_device far_data gs_plan9_device = {
    prn_device_body(plan9_device, plan9_procs, "plan9", DEFAULT_WIDTH_10THS,
                    DEFAULT_HEIGHT_10THS, X_DPI, Y_DPI, 0, 0, 0,
                    0,                /* margins */
...
plan9_open(gx_device* dev)
{
	...
	//	printf("plan9_open gs_plan9_device.dither = %d idev->dither =
	//%d\n",
...
initwriteimage(FILE* f, Rectangle r, char* chanstr, int depth)
{
	...
	fprintf(f, "compressed\n%11s %11d %11d %11d %11d ", chanstr, r.min.x,
