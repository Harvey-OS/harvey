...
private
dev_proc_print_page(faxg3_print_page);
private
dev_proc_print_page(faxg32d_print_page);
private
dev_proc_print_page(faxg4_print_page);
...
#define FAX_DEVICE(dname, print_page)                                          \
	{                                                                      \
		FAX_DEVICE_BODY(gx_device_fax, gdev_fax_std_procs, dname,      \
		                print_page)                                    \
	}

const gx_device_fax gs_faxg3_device = FAX_DEVICE("faxg3", faxg3_print_page);

const gx_device_fax gs_faxg32d_device =
    FAX_DEVICE("faxg32d", faxg32d_print_page);

const gx_device_fax gs_faxg4_device = FAX_DEVICE("faxg4", faxg4_print_page);
...
gdev_fax_get_params(gx_device* dev, gs_param_list* plist)
{
	...
	if((code = param_write_int(plist, "AdjustWidth", &fdev->AdjustWidth)) <
	   0)
		ecode = code;
	return ecode;
}
...
gdev_fax_put_params(gx_device* dev, gs_param_list* plist)
{
	...
	switch(code =
	           param_read_int(plist, (param_name = "AdjustWidth"), &aw)) {
	case 0:
		if(aw >= 0 && aw <= 1)
			break;
...
private
void
gdev_fax_init_state_adjust(stream_CFE_state* ss, const gx_device_fax* fdev,
                           int adjust_width)
{
...
gdev_fax_print_strip(gx_device_printer* pdev, FILE* prn_stream,
                     const stream_template* temp, stream_state* ss, int width,
                     int row_first, int row_end /* last + 1 */)
{
	...
	if(code < 0)
		return_error(
		    gs_error_limitcheck); /* bogus, but as good as any */
...
#undef OUT_SIZE
	...
	for(lnum = row_first;;) {
		...
		if_debug7(
		    'w', "[w]lnum=%d r=0x%lx,0x%lx,0x%lx w=0x%lx,0x%lx,0x%lx\n",
		    lnum, (uint32_t)in, (uint32_t)r.ptr, (uint32_t)r.limit,
		    (uint32_t)out, (uint32_t)w.ptr, (uint32_t)w.limit);
		status = temp->process(ss, &r, &w, lnum == row_end);
		if_debug7(
		    'w', "...%d, r=0x%lx,0x%lx,0x%lx w=0x%lx,0x%lx,0x%lx\n",
		    status, (uint32_t)in, (uint32_t)r.ptr, (uint32_t)r.limit,
		    (uint32_t)out, (uint32_t)w.ptr, (uint32_t)w.limit);
		switch(status) {
