...
const char *const cmd_op_names[16] =
    {cmd_op_name_strings};
private
const int8_t *const cmd_misc_op_names[16] =
    {cmd_misc_op_name_strings};
private
const int8_t *const cmd_misc2_op_names[16] =
    {cmd_misc2_op_name_strings};
private
const int8_t *const cmd_segment_op_names[16] =
    {cmd_segment_op_name_strings};
private
const int8_t *const cmd_path_op_names[16] =
...
cmd_put_list_op(gx_device_clist_writer *cldev, cmd_list *pcl, uint size)
{
	...
	if(cldev->ccl == pcl) { /* We're adding another command for the same band. */
		...
		cmd_prefix *cp = (cmd_prefix *)(dp + ((cldev->cbuf - dp) & (arch_align_ptr_mod - 1)));
		   ...
		   int enable)
{
	...
	int code = set_cmd_put_op(dp, cldev, pcls,
				  (byte)(enable ? cmd_opv_enable_lop : cmd_opv_disable_lop),
				  1);
		    ...
		    int enable)
{
	byte *dp;
	int code = set_cmd_put_op(dp, cldev, pcls,
				  (byte)(enable ? cmd_opv_enable_clip : cmd_opv_disable_clip),
				  1);
		   ...
		   gs_param_list *param_list) /* NB open for READ */
{
	...
	if(param_length > 0) {
		...
		if(param_length > sizeof(local_buf)) {
			...
			if(param_length >= 0)
				code = (old_param_length != param_length ? gs_note_error(gs_error_unknownerror) : 0);
			if(code < 0) {
