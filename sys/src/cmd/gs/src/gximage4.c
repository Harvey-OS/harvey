...
/* Forward references */
private
dev_proc_begin_typed_image(gx_begin_image4);
...
private
image_proc_sput(gx_image4_sput);
private
image_proc_sget(gx_image4_sget);
private
image_proc_release(gx_image4_release);
const gx_image_type_t gs_image_type_4 = {
    &st_gs_image4, gx_begin_image4, gx_data_image_source_size, gx_image4_sput,
    gx_image4_sget, gx_image4_release, 4};
/*
...
private
int
gx_begin_image4(gx_device* dev, const gs_imager_state* pis,
                ...
                gx_image_enum_common_t** pinfo)
{
	...
	/* Check that MaskColor values are within the valid range. */
	{
		...
		for(i = 0; i < spp * 2; i += 2) {
			...
			if(c0 > c1) {
				opaque =
				    true; /* pixel can never match mask color */
				break;
			}
...
private
int
gx_image4_sget(gs_image_common_t* pic, stream* s, const gs_color_space* pcs)
{
	...
	num_values = gs_color_space_num_components(pcs) *
