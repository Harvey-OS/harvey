...
struct display_callback_s {
	...
	int (*display_presize)(void* handle, void* device, int width,
	 ...
	 */
	int (*display_update)(void* handle, void* device, int x, int y, int w,
	                      int h);
	 ...
	 */
	void* (*display_memalloc)(void* handle, void* device,
	                          unsigned long size);
	 ...
	 * This function pointer may be set to NULL if not required.
	 */
	int (*display_separation)(void* handle, void* device, int component,
	                          const char* component_name, unsigned short c,
