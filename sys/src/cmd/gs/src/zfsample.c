...
private
int cube_build_func0(const ref* pdict, gs_function_Sd_params_t* params,
                     gs_memory_t* mem);
private
int sampled_data_setup(i_ctx_t* i_ctx_p, gs_function_t* pfn, const ref* pproc,
                       int (*finish_proc)(i_ctx_t*), gs_memory_t* mem);
private
int sampled_data_sample(i_ctx_t* i_ctx_p);
private
int sampled_data_continue(i_ctx_t* i_ctx_p);
private
int sampled_data_finish(i_ctx_t* i_ctx_p);

private
gs_sampled_data_enum* gs_sampled_data_enum_alloc(gs_memory_t* mem,
                                                 client_name_t cname);
 ...
 *    Domain (required)
 *    Range (required)
 *    Size (optional)  If Size is not specified then a default value is
 *determined
...
zbuildsampledfunction(i_ctx_t* i_ctx_p)
{
	 ...
	 */
	return sampled_data_setup(i_ctx_p, pfn, pfunc, sampled_data_finish,
	                          imemory);
}
...
/*
 * This routine will verify that the requested data hypercube parameters will
 * require
 * a data storage size less than or equal to the MAX_DATA_SIZE.
 */
private
bool
valid_cube_size(int num_inputs, int num_outputs, int sample_size,
                const int Size[])
{
...
private
int
determine_sampled_data_size(int num_inputs, int num_outputs, int sample_size,
                            int Size[])
{
                 ...
                 gs_memory_t* mem)
{
	...
	if((code = dict_int_param(pdict, "Order", 1, 3, 1, &params->Order)) <
	       0 ||
	   (code = dict_int_param(pdict, "BitsPerSample", 1, 32, 0,
	                          &params->BitsPerSample)) < 0 ||
	   ((code = params->m = fn_build_float_array(
	         pdict, "Domain", false, true, &params->Domain, mem)) < 0) ||
	   ((code = params->n = fn_build_float_array(
	         pdict, "Range", false, true, &params->Range, mem)) < 0)) {
		goto fail;
	}
	/*
	...
	params->m >>= 1;
	params->n >>= 1;
	if(params->m == 0 || params->n == 0 || params->m > MAX_NUM_INPUTS ||
	   params->n > MAX_NUM_OUTPUTS) {
	 ...
	 */
	{
		int* ptr = (int*)gs_alloc_byte_array(mem, params->m,
		                                     sizeof(int), "Size");
		...
		if(code == 0) {
			/*
			 * The Size array has not been specified.  Determine a
			 * default
			 * set of values.
			 */
			code = determine_sampled_data_size(
			    params->m, params->n, params->BitsPerSample,
			    (int*)params->Size);
			if(code < 0)
				goto fail;
		} else { /* Size array specified - verify valid */
			if(code != params->m ||
			   !valid_cube_size(params->m, params->n,
	 ...
	 */
	bytes =
	    gs_alloc_byte_array(mem, total_size, 1, "cube_build_func0(bytes)");
	if(!bytes) {
		code = gs_note_error(e_VMerror);
		goto fail;
	}
...
private
int
sampled_data_setup(i_ctx_t* i_ctx_p, gs_function_t* pfn, const ref* pproc,
                   int (*finish_proc)(i_ctx_t*), gs_memory_t* mem)
{
	os_ptr op = osp;
	...
	gs_function_Sd_params_t* params =
	    (gs_function_Sd_params_t*)&pfn->params;
	 ...
	 */
	penum =
	    gs_sampled_data_enum_alloc(imemory, "zbuildsampledfuntion(params)");
	if(penum == NULL)
	...
	/*
	 * Note:  As previously mentioned, we are putting some spare (unused)
	 * stack
	 * space under the input values in case the function unbalances the
	 * stack.
	 * It is possible for the function to pop or change values on the stack
	 * outside of the input values.  (This has been found to happen with
	 * some
...
sampled_data_sample(i_ctx_t* i_ctx_p)
{
	...
	for(i = 0; i < num_inputs; i++) {
		...
		make_real(op - num_inputs + i + 1,
		          (float)(penum->indexes[i] * (dmax - dmin) /
		                      (params->Size[i] - 1) +
		                  dmin));
	}
	...
	push_op_estack(
	    sampled_data_continue); /* Put 'save' routine on estack, after
	                               sample proc */
...
sampled_data_continue(i_ctx_t* i_ctx_p)
{
	...
	int code = 0;
	byte* data_ptr;
	double sampled_data_value_max =
	    (double)((1 << params->BitsPerSample) - 1);
	 ...
	 */
	if(num_out + O_STACK_PAD + penum->o_stack_depth !=
	   ref_stack_count(&o_stack)) {
		int stack_depth_adjust =
		    ref_stack_count(&o_stack) - penum->o_stack_depth;

		if(stack_depth_adjust >= 0)
			pop(stack_depth_adjust);
		else {
			/*
			 * If we get to here then there were major problems.
			 * The function
			 * removed too many items off of the stack.  We had
			 * placed extra
			 * (unused) stack stack space to allow for this but the
			 * function
			 * exceeded even that.  Data on the stack may have been
			 * lost.
			 * The only thing that we can do is move the stack
			 * pointer back and
			 * hope.  (We have not seen real Postscript files that
			 * have this
			 * problem.)
			 */
	...
	for(i = 0; i < num_out; i++) {
		...
		if(value < rmin)
			...
			value = rmax;
		value =
		    (value - rmin) / (rmax - rmin); /* Convert to 0 to 1.0 */
		cv = (int)(value * sampled_data_value_max + 0.5);
		for(j = 0; j < bps; j++)
			data_ptr[bps * i + j] =
			    (byte)(cv >> ((bps - 1 - j) * 8)); /* MSB first */
	}
