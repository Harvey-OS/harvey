...
private
int cube_build_func0(const ref *pdict,
		     gs_function_Sd_params_t *params, gs_memory_t *mem);
private
int sampled_data_setup(i_ctx_t *i_ctx_p, gs_function_t *pfn,
...
private
gs_sampled_data_enum *gs_sampled_data_enum_alloc(gs_memory_t *mem, client_name_t cname);
		 ...
		 gs_memory_t *mem)
{
     ...
     */
	{
	     ...
	     */
			...
			if(code != params->m || !valid_cube_size(params->m, params->n, params->BitsPerSample, params->Size))
				code = gs_note_error(e_rangecheck);
			goto fail;
		}
	}
...
sampled_data_sample(i_ctx_t *i_ctx_p)
{
	...
	for(i = 0; i < num_inputs; i++) {
		...
		make_real(op - num_inputs + i + 1, (float)(penum->indexes[i] * (dmax - dmin) / (params->Size[i] - 1) + dmin));
	}
