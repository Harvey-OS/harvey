...
construct_ht_order_short(gx_ht_order* porder, const byte* thresholds)
{
	...
	/* Check whether this is a predefined halftone. */
	{
		...
		for(; *phtrp; ++phtrp) {
			const gx_device_halftone_resource_t* const* pphtr =
			    (*phtrp)();
			...
			while((phtr = *pphtr++) != 0) {
				if(phtr->Width == porder->width &&
				   ...
				   !memcmp(phtr->levels, levels,
				           num_levels * sizeof(*levels)) &&
				   !memcmp(phtr->bit_data, porder->bit_data,
				           size * phtr->elt_size)) {
					/*
					 * This is a predefined halftone.  Free
					 * the levels and
					 * bit_data arrays, replacing them with
					 * the built-in ones.
					 */
					...
					porder->levels =
					    (uint*)phtr
					        ->levels; /* actually const */
					porder->bit_data =
					    (void*)phtr
					        ->bit_data; /* actually const */
					goto out;
				}
			}
		}
...
render_ht_short(gx_ht_tile* pbt, int level, const gx_ht_order* porder)
{
	...
	register const uint16_t* p =
	    (const uint16_t*)porder->bit_data + old_level;
	register byte* data = pbt->tiles.data;
...
#define INVERT(i)                                                              \
	BEGIN                                                                  \
	if_debug3('H', "[H]invert level=%d offset=%u mask=0x%x\n",             \
	          (int)(p + i - (const ushort*)porder->bit_data), p[i] >> 3,   \
	          0x80 >> (p[i] & 7));                                         \
	INVERT_DATA(i);                                                        \
	END
#else
