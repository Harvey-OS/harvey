...
/* Copyright (C) 1994, 1997, 1998, 1999 Aladdin Enterprises.  All rights
  reserved.
...
private
int
s_exE_process(stream_state* st, stream_cursor_read* pr, stream_cursor_write* pw,
              bool last)
{
...
/* Stream template */
const stream_template s_exE_template = {&st_exE_state, NULL, s_exE_process, 1,
                                        2};
...
private
int
s_exD_process(stream_state* st, stream_cursor_read* pr, stream_cursor_write* pw,
              bool last)
{
	stream_exD_state* const ss = (stream_exD_state*)st;
	const byte* p = pr->ptr;
	...
	if(ss->binary < 0) {
		...
		if(ss->pfb_state == 0) {
			 ...
			 * input stream,
			 * because Adobe interpreters do this. Don't skip '\0'
			 * or '\f'.
			 */
			for(; rcount; rcount--, p++) {
				byte c = p[1];
				if(c != '\t' && c != char_CR && c != char_EOL &&
				   c != ' ')
					break;
			}
			pr->ptr = p;
		...
		for(i = min(8, rcount); i > 0; i--)
			if(!(decoder[p[i]] <= 0xf ||
				...
				ss->binary = 1;
				if(ss->pfb_state != 0) {
					/* Stop at the end of the .PFB binary
					 * data. */
					ss->record_left =
					    ss->pfb_state->record_left;
				}
				break;
			}
	}
	...
	hp:
		...
		/*
		 * Check for having finished a prematurely decoded hex section
		 * of
		 * a PFB file.
		 */
		if(ss->hex_left == 0)
	...
	if(skip >= count && skip != 0) {
		...
		gs_type1_decrypt(q + 1, p + 1, skip, (crypt_state*)&ss->cstate);
		count -= skip;
...
const stream_template s_exD_template = {
    &st_exD_state, s_exD_init, s_exD_process, 8, 200, NULL, s_exD_set_defaults};
