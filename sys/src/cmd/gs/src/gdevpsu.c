                     ...
                     const gx_device_pswrite_common_t* pdpc)
{
	...
	pprintd3(s, "_%d_%d_%d", (int)pdpc->LanguageLevel,
...
psw_print_bbox(FILE* f, const gs_rect* pbbox)
{
	fprintf(f, "%%%%BoundingBox: %d %d %d %d\n", (int)floor(pbbox->p.x),
	        (int)floor(pbbox->p.y), (int)ceil(pbbox->q.x),
	        (int)ceil(pbbox->q.y));
	fprintf(f, "%%%%HiResBoundingBox: %f %f %f %f\n", pbbox->p.x,
	        pbbox->p.y, pbbox->q.x, pbbox->q.y);
}

/* ---------------- File level ---------------- */

private
const int8_t* const psw_ps_header[] = {"%!PS-Adobe-3.0", "%%Pages: (atend)", 0};

private
const int8_t* const psw_eps_header[] = {"%!PS-Adobe-3.0 EPSF-3.0", 0};

private
const int8_t* const psw_begin_prolog[] = {
 ...
 */
private
const int8_t* const psw_ps_procset[] = {
...
private
const int8_t* const psw_end_prolog[] = {
    "end readonly def", "%%EndResource", /* ProcSet */
...
psw_write_page_header(stream* s, const gx_device* dev,
                      const gx_device_pswrite_common_t* pdpc, bool do_scale,
                      int32_t page_ord, int dictsize)
{
	...
	if(!pdpc->ProduceEPS) {
		int width =
		    (int)(dev->width * 72.0 / dev->HWResolution[0] + 0.5);
		int height =
		    (int)(dev->height * 72.0 / dev->HWResolution[1] + 0.5);
		typedef struct ps_ {
			const char* size_name;
			int width, height;
		} page_size;
		static const page_size sizes[] = {{"/11x17", 792, 1224},
		                                  {"/a3", 842, 1190},
		                                  {"/a4", 595, 842},
		                                  {"/b5", 501, 709},
		                                  {"/ledger", 1224, 792},
	...
	if(do_scale)
		pprintg2(s, "%g %g scale\n", 72.0 / dev->HWResolution[0],
		         72.0 / dev->HWResolution[1]);
