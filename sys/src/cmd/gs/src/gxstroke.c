...
/* $Id: gxstroke.c,v 1.18 2005/08/17 14:40:11 igor Exp $ */
 ...
 */
private
float join_expansion_factor(const gs_imager_state*, gs_line_join);
int
                         ...
                         gs_fixed_point* ppt)
{
	...
	if(is_fzero2(pis->ctm.xy, pis->ctm.yx) ||
	   is_fzero2(pis->ctm.xx, pis->ctm.yy)) {
		bool must_be_closed = !(pis->line_params.cap == gs_cap_square ||
	...
	if(result) {
		if(!gx_path_has_curves(ppath) &&
		   ...
		   (psub == 0 || (pseg = psub->next) == 0 ||
			...
			float factor =
			    join_expansion_factor(pis, pis->line_params.join);
...
join_expansion_factor(const gs_imager_state* pis, gs_line_join join)
{
	switch(join) {
	case gs_join_miter:
		return pis->line_params.miter_limit;
...
#define ASSIGN_POINT(pp, p) ((pp)->x = (p).x, (pp)->y = (p).y)

/* Other forward declarations */
private
bool width_is_thin(pl_ptr);
private
void adjust_stroke(pl_ptr, const gs_imager_state*, bool);
private
int line_join_points(const gx_line_params* pgs_lp, pl_ptr plp, pl_ptr nplp,
                     gs_fixed_point* join_points, const gs_matrix* pmat,
                     gs_line_join join, bool reflected);
private
void compute_caps(pl_ptr);
private
int add_points(gx_path*, const gs_fixed_point*, int, bool);
private
int add_round_cap(gx_path*, const_ep_ptr);
private
int cap_points(gs_line_cap, const_ep_ptr, gs_fixed_point* /*[3] */);
...
#define stroke_line_proc(proc)                                                 \
	int proc(gx_path*, int, pl_ptr, pl_ptr, const gx_device_color*,        \
	         gx_device*, const gs_imager_state*, const gx_stroke_params*,  \
	         const gs_fixed_rect*, int, gs_line_join, bool)
typedef stroke_line_proc((*stroke_line_proc_t));

private
stroke_line_proc(stroke_add);
private
stroke_line_proc(stroke_fill);
 ...
 */
	...
	gs_line_join curve_join =
	    (pgs_lp->curve_join >= 0
	         ? (gs_line_join)pgs_lp->curve_join
	...
	if(gs_debug_c('o')) {
		...
		dlprintf3("[o]half_width=%f, cap=%d, join=%d,\n",
		          pgs_lp->half_width, (int)pgs_lp->cap,
		          (int)pgs_lp->join);
	...
	/* Expand the path bounding box by the scaled line width. */
	{
		...
		if(gx_stroke_path_expansion(pis, ppath, &expansion) < 0) {
			...
			/*
			 * It's theoretically possible for the following
			 * computations to
			 * overflow, so we need to check for this.
			 */
			ibox.p.x = (ibox.p.x < min_fixed + expansion.x
			                ? min_fixed
			                : ibox.p.x - expansion.x);
			ibox.p.y = (ibox.p.y < min_fixed + expansion.y
			                ? min_fixed
			                : ibox.p.y - expansion.y);
			ibox.q.x = (ibox.q.x > max_fixed - expansion.x
			                ? max_fixed
			                : ibox.q.x + expansion.x);
			ibox.q.y = (ibox.q.y > max_fixed - expansion.y
			                ? max_fixed
	...
	if(is_fzero(line_width))
		...
		sat:
			...
			if(!always_thin &&
			   uniform) { /* Precompute a value we'll need later. */
				device_line_width_scale =
				    line_width_and_scale * xa;
			}
			break;
		default: {
	...
	if_debug7('o', "[o]ctm=(%g,%g,%g,%g,%g,%g) thin=%d\n", xx, xy, yx, yy,
	          pis->ctm.tx, pis->ctm.ty, always_thin);
	...
	if(!gx_path_has_curves(ppath)) { /* don't need to flatten */
		...
		if((code = gx_path_add_flattened_for_stroke(
		        ppath, &fpath, params->flatness, pis)) < 0)
			return code;
		spath = &fpath;
	}
	if(dash_count) {
	...
	for(psub = spath->first_subpath; psub != 0;) {
		...
		d:
			...
			if(!(udx | udy)) { /* degenerate */
				...
				/*
				 * The entire subpath is degenerate, but it
				 * includes
				 * more than one point.  If the dot length is
				 * non-zero,
				...
				if(pgs_lp->cap != gs_cap_round) {
					/* From PLRM, stroke operator :
					   If a subpath is degenerate (consists
					   of a single-point closed path
					   or of two or more points at the same
					   coordinates),
					   stroke paints it only if round line
					   caps have been specified */
					break;
				}
				/*
				 * Orient the dot according to the previous
				 * segment if
				 ...
				 */
				{
					...
					if(end != 0 &&
					   (end->pt.x != x || end->pt.y != y))
						sx = end->pt.x, sy = end->pt.y;
					else if(pseg != 0 && (pseg->pt.x != x ||
					                      pseg->pt.y != y))
						sx = pseg->pt.x,
						sy = pseg->pt.y;
				}
				/*
				 * Compute the properly oriented dot length, and
				 * then
				...
				if((udx | udy) == 0) {
					if(is_fzero(
					       pgs_lp->dot_orientation.xy)) {
						/* Portrait orientation, dot
						 * length = X */
						udx = fixed_1;
					} else {
						/* Landscape orientation, dot
						 * length = Y */
						udy = fixed_1;
					}
				}
				{
					...
					/*
					 * If we're using butt caps, make sure
					 * the "line" is
					 * long enough to show up.
					 */
					if(pgs_lp->cap == gs_cap_butt) {
						fixed dmax = max(any_abs(udx),
						                 any_abs(udy));
				...
				/*
				 * Back up 1 segment to keep the bookkeeping
				 * straight.
				 */
			...
			if(always_thin) {
				...
				if(uniform != 0) {
					/* We can save a lot of work in this
					 * case. */
					...
					gs_imager_idtransform(pis, (float)udx,
					                      (float)udy, &dpt);
					 ...
					 * gs_dtransform.  The second
					 * computation
					 * folds in a 90-degree rotation (in
					 * user
			...
			if(index++) {
				gs_line_join join =
				    (pseg->notes & not_first ? curve_join
				                             : pgs_lp->join);
				int first;
				pl_ptr lptr;
				...
				code = (*line_proc)(
				    to_path, first, &pl_prev, lptr, pdevc, dev,
				    pis, params, &cbox, uniform, join,
				    initial_matrix_reflected);
				if(code < 0)
					goto exit;
		...
		if(index) {
			...
			gs_line_join join =
			    ((pseg == 0 ? (const segment*)spath->first_subpath
			         ...
			         ? curve_join
			         : pgs_lp->join);
			/* For some reason, the Borland compiler requires the
			 * cast */
			/* in the following statement. */
			pl_ptr lptr = (!is_closed || join == gs_join_none
			                   ? (pl_ptr)0
			                   : (pl_ptr)&pl_first);
...
exit:
	if(to_path == &stroke_path_body)
		gx_path_free(
		    &stroke_path_body,
		    "gx_stroke_path_only error"); /* (only needed if error) */
	if(dash_count)
                    ...
                    const gx_device_color* pdevc, const gx_clip_path* pcpath)
{
	...
	code = gx_stroke_path_only_aux(ppath, to_path, pdev, pis, params, pdevc,
	                               pcpath);
	if(vd_allowed('S'))
		vd_release_dc;
	return code;
...
private
int
line_intersect(p_ptr pp1, /* point on 1st line */
...
{ /* return intersection here */
...
#ifdef DEBUG
	if(gs_debug_c('O')) {
		dlprintf4("[o]Intersect %f,%f(%f/%f)", fixed2float(pp1->x),
		          fixed2float(pp1->y), fixed2float(pd1->x),
		          fixed2float(pd1->y));
		dlprintf4(" & %f,%f(%f/%f),\n", fixed2float(pp2->x),
		          fixed2float(pp2->y), fixed2float(pd2->x),
		          fixed2float(pd2->y));
	...
	return (f1 >= 0 && (v1 * xdiff >= u1 * ydiff ? denom >= 0 : denom < 0)
	            ? 0
	            : 1);
}
            ...
            bool reflected)
{
	...
	if(plp->thin) {
		...
		return (*dev_proc(dev, draw_thin_line))(
		    dev, lix, liy, litox, litoy, pdevc, pis->log_op);
	}
	/* Check for being able to fill directly. */
	{
		...
		if(!plp->thin && (nplp == 0 || !nplp->thin) &&
		   ...
		   lop_is_idempotent(pis->log_op)) {
			...
			if(nplp == 0)
				code =
				    cap_points(cap, &plp->e, points + npoints);
			else
				code = line_join_points(
...
#define SUB_OVERFLOWS(r, u, v) (((r = u - v) ^ u) < 0 && (u ^ v) < 0)
...
#undef SUB_OVERFLOWS
			if(nplp != 0) {
				if(join == gs_join_miter) {
					/* Make sure we have a bevel and not a
					 * miter. */
				...
				{
					const gs_fixed_point* bevel =
					    points + 2;

					/* Identify which 3 points define the
					 * bevel triangle. */
					if(points[3].x == nplp->o.p.x &&
					   points[3].y == nplp->o.p.y)
						++bevel;
					/* Fill the bevel. */
					code = (*dev_proc(dev, fill_triangle))(
					    dev, bevel->x, bevel->y,
					    bevel[1].x - bevel->x,
					    bevel[1].y - bevel->y,
					    bevel[2].x - bevel->x,
					    bevel[2].y - bevel->y, pdevc,
					    pis->log_op);
					if(code < 0)
						return code;
				}
			...
			return (*dev_proc(dev, fill_parallelogram))(
			    dev, points[1].x, points[1].y, ax, ay, bx, by,
			    pdevc, pis->log_op);
		fill:
           ...
           bool reflected)
{
	...
	if(first == 0 && pgs_lp->cap == gs_cap_round) {
		...
		if((code = gx_path_add_point(ppath, plp->o.co.x, plp->o.co.y)) <
		       0 ||
		...
		if((npoints =
		        cap_points((first == 0 ? pgs_lp->cap : gs_cap_butt),
		                   &plp->o, points)) < 0)
			return npoints;
	}
	if(nplp == 0) {
		...
		if(pgs_lp->cap == gs_cap_round) {
			...
			++npoints;
			if((code = add_points(ppath, points, npoints,
			                      moveto_first)) < 0)
				return code;
		...
		vd_lineto(points[npoints].x, points[npoints].y);
		++npoints;
		if((code = add_points(ppath, points, npoints, moveto_first)) <
		   0)
			return code;
		code = add_round_cap(ppath, &plp->e);
		goto done;
		...
		code =
		    line_join_points(pgs_lp, plp, nplp, points + npoints,
		                     (uniform ? (gs_matrix*)0 : &ctm_only(pis)),
		                     join, reflected);
	if(code < 0)
		return code;
...
#define jpx join_points[4]
	 ...
	 */
	bool ccw = (double)(plp->width.x) /* x1 */ * (nplp->width.y) /* y2 */ >
	...
	if(join == gs_join_miter &&
	   !(fixed2long(outp->x << 1) == fixed2long(np->x << 1) &&
		...
		if(pmat) {
...
#ifdef DEBUG
			{
				...
				if(dif != 0 && (dif < M_PI) != ccw0)
					lprintf8("ccw wrong: tan(a1=%g)=%g/%g, "
					         "tan(a2=%g)=%g,%g, dif=%g, "
					         "ccw=%d\n",
		...
		if(check > 0 ? (num < 0 || num >= denom * check)
...
compute_caps(pl_ptr plp)
{
	...
	if(gs_debug_c('O')) {
		...
		dlprintf4("\twxy=(%f,%f) lxy=(%f,%f)\n", fixed2float(wx2),
		          fixed2float(wy2), fixed2float(plp->e.cdelta.x),
...
add_round_cap(gx_path* ppath, const_ep_ptr endp)
{
	 ...
	 */
	if((code = gx_path_add_partial_arc(ppath, px + cdx, py + cdy, xo + cdx,
	                                   yo + cdy, quarter_arc_fraction)) <
	       0 ||
	   (code = gx_path_add_partial_arc(ppath, xe, ye, xe + cdx, ye + cdy,
	                                   quarter_arc_fraction)) < 0 ||
	   (code = gx_path_add_partial_arc(ppath, px - cdx, py - cdy, xe - cdx,
	                                   ye - cdy, quarter_arc_fraction)) <
	       0 ||
...
{
#define PUT_POINT(i, px, py) pts[i].x = (px), pts[i].y = (py)
	switch(type) {
	case gs_cap_butt:
		PUT_POINT(0, xo, yo);
		PUT_POINT(1, xe, ye);
