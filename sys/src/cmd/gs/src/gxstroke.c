...
private
int line_join_points(const gx_line_params *pgs_lp,
...
private
int add_points(gx_path *, const gs_fixed_point *,
	       int, bool);
private
int add_round_cap(gx_path *, const_ep_ptr);
private
int cap_points(gs_line_cap, const_ep_ptr,
	       gs_fixed_point * /*[3] */);
			...
			const gx_device_color *pdevc, const gx_clip_path *pcpath)
{
	...
	orientation orient =
		...
		is_fzero2(xy, yx) ? (uniform = (xx == yy ? 1 : xx == -yy ? -1 : 0),
				     reflected = (uniform ? uniform < 0 : (xx < 0) != (yy < 0)),
				     orient_portrait)
				  : is_fzero2(xx, yy) ? (uniform = (xy == yx ? -1 : xy == -yx ? 1 : 0),
...
/*
	...
	gs_line_join curve_join =
	    (pgs_lp->curve_join >= 0 ? (gs_line_join)pgs_lp->curve_join : pgs_lp->join == gs_join_none || pgs_lp->join == gs_join_round ? gs_join_bevel : pgs_lp->join);
	...
	/* Expand the path bounding box by the scaled line width. */
	{
	     ...
	     */
			ibox.p.x = (ibox.p.x < min_fixed + expansion.x ? min_fixed : ibox.p.x - expansion.x);
			ibox.p.y = (ibox.p.y < min_fixed + expansion.y ? min_fixed : ibox.p.y - expansion.y);
			ibox.q.x = (ibox.q.x > max_fixed - expansion.x ? max_fixed : ibox.q.x + expansion.x);
			ibox.q.y = (ibox.q.y > max_fixed - expansion.y ? max_fixed : ibox.q.y + expansion.y);
		}
	}
	...
	if(is_fzero(line_width))
		...
		default: {
			...
			if(cross < 0)
				cross = 0;
			always_thin =
			    ((max(xsq, ysq) + cross) * line_width * line_width < 0.25);
		}
		}
	}
	...
	for(psub = spath->first_subpath; psub != 0;) {
		...
		if(index) {
			...
			gs_line_join join =
			    ((pseg == 0 ? (const segment *)spath->first_subpath : pseg)->notes & not_first ? curve_join : pgs_lp->join);
			/* For some reason, the Borland compiler requires the cast */
			/* in the following statement. */
			pl_ptr lptr =
			    (!is_closed || join == gs_join_none ? (pl_ptr)0 : (pl_ptr)&pl_first);
	    ...
	    bool reflected)
{
	...
	/* Check for being able to fill directly. */
	{
		...
		if(!plp->thin && (nplp == 0 || !nplp->thin) && ((first != 0 && nplp != 0) || cap == gs_cap_butt || cap == gs_cap_square) && (join == gs_join_bevel || join == gs_join_miter ||
																	     join == gs_join_none) &&
		   (pis->fill_adjust.x | pis->fill_adjust.y) == 0 && lop_is_idempotent(pis->log_op)) {
			...
			if(nplp == 0)
				...
				code = line_join_points(pgs_lp, plp, nplp, points + npoints,
							(uniform ? (gs_matrix *)0 : &ctm_only(pis)), join, reflected);
			if(code < 0)
				goto general;
