...
private
const int CoreDistVersion = 5000; /* Distiller 5.0 */
private
const gs_param_item_t pdf_param_items[] = {
#define pi(key, type, memb)                                                    \
	{                                                                      \
		key, type, offset_of(gx_device_pdf, memb)                      \
	}
...
gdev_pdf_get_params(gx_device* dev, gs_param_list* plist)
{
	...
	if(code < 0 ||
	   (code = param_write_int(plist, ".EmbedFontObjects",
	                           &EmbedFontObjects)) < 0 ||
	   ...
	   (code = gs_param_write_items(plist, pdev, NULL, pdf_param_items)) <
	       0)
		;
	return code;
}
...
gdev_pdf_put_params(gx_device* dev, gs_param_list* plist)
{
...

	{
		...
		code = param_read_string_array(plist, (param_name = "pdfmark"),
		                               &ppa);
		switch(code) {
		case 0:
		...
		code =
		    param_read_string_array(plist, (param_name = "DSC"), &ppa);
		switch(code) {
		case 0:
			code = pdf_open_document(pdev);
			if(code < 0)
	...
	if(!(locked && pdev->params.LockDistillerParams)) {
...

		{
			...
			ecode = param_put_int(
			    plist, (param_name = ".EmbedFontObjects"), &efo,
			    ecode);
			if(efo != 1)
				param_signal_error(plist, param_name,
				                   ecode = gs_error_rangecheck);
		}
		{
			int cdv = CoreDistVersion;

			ecode = param_put_int(plist,
			                      (param_name = "CoreDistVersion"),
			                      &cdv, ecode);
		...
		switch(code = param_read_float(
		           plist, (param_name = "CompatibilityLevel"), &cl)) {
		default:
			ecode = code;
			param_signal_error(plist, param_name, ecode);
		case 0:
			/*
			 * Must be 1.2, 1.3, or 1.4.  Per Adobe documentation,
			 * substitute
			...
			if(fon != save_dev.FirstObjectNumber) {
				if(fon <= 0 || fon > 0x7fff0000 ||
				   (pdev->next_id != 0 &&
					...
					ecode = gs_error_rangecheck;
					param_signal_error(
					    plist, "FirstObjectNumber", ecode);
				}
			}
		}
		{
			/*
			 * Set ProcessColorModel now, because
			 * gx_default_put_params checks
			 * it.
			 */
	 ...
	 */
	if(pdev->PDFX)
		cl = (float)1.3; /* Instead pdev->CompatibilityLevel = 1.2; -
		                    see below. */
	...
	if(pdev->ForOPDFRead) {
		...
		cl = (float)1.2; /* Instead pdev->CompatibilityLevel = 1.2; -
		                    see below. */
		pdev->MaxInlineImageSize =
		    max_long; /* Save printer's RAM from saving temporary image
		                 data.
...
#define MAX_EXTENT ((int)(MAX_USER_COORD * 0.9))
	...
	if(dev->height > MAX_EXTENT || dev->width > MAX_EXTENT) {
		double factor = max(dev->height / (double)MAX_EXTENT,
...
pdf_dsc_process(gx_device_pdf* pdev, const gs_param_string_array* pma)
{
	...
	for(i = 0; i + 1 < pma->size && code >= 0; i += 2) {
		 ...
		 *
		 * Acrobat Distiller sets CreationDate and ModDate to the
		 *current
		 * time, not the value of %%CreationDate.  We think this is
		 *wrong,
		...
		if(pdf_key_eq(pkey, "Creator"))
			...
			if((ppdi = &pdev->doc_dsc_info,
			   ...
			   (ppdi = &pdev->page_dsc_info,
				...
				if(sscanf((const char*)pvalue->data,
				          "[%g %g %g %g]", &mat.xx, &mat.xy,
				          &mat.yx, &mat.yy) != 4)
					continue; /* error */
				for(orient = 0; orient < 4; ++orient) {
					if(mat.xx == 1 && mat.xy == 0 &&
					   mat.yx == 0 && mat.yy == 1)
						break;
		...
		if(pdev->ParseDSCCommentsForDocInfo || pdev->PreserveEPSInfo)
			code = cos_dict_put_c_key_string(
			    pdev->Info, key, pvalue->data, pvalue->size);
	}
	return code;
}
