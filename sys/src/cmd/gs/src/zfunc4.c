...
private
int
check_psc_function(i_ctx_t* i_ctx_p, const ref* pref, int depth, byte* ops,
                   int* psize)
{
	int32_t i;
	...
	for(i = 0; i < size; ++i) {
		...
		default: {
			...
			if((code = array_get(imemory, pref, ++i, &elt2)) < 0)
				return code;
			*psize += 3;
			code = check_psc_function(i_ctx_p, &elt, depth + 1, ops,
			                          psize);
			if(code < 0)
				return code;
	...
	(r_btype(pref) == t_operator && r_has_attr(pref, a_executable) &&      \
	 (pref)->value.opproc == proc)
			if(R_IS_OPER(&elt2, zif)) {
				...
				return_error(e_rangecheck);
			else if((code == array_get(imemory, pref, ++i, &elt3)) <
			        0)
				return code;
				...
				*psize += 3;
				code = check_psc_function(
				    i_ctx_p, &elt2, depth + 1, ops, psize);
				if(code < 0)
					return code;
				if(ops)
					psc_fixup(p, ops + *psize);
...
int
gs_build_function_4(i_ctx_t* i_ctx_p, const ref* op,
