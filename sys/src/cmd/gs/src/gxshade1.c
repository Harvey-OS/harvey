...
make_other_poles(patch_curve_t curve[4])
{
	...
	for(i = 0; i < 4; i++) {
		j = (i + 1) % 4;
		curve[i].control[0].x =
		    (curve[i].vertex.p.x * 2 + curve[j].vertex.p.x);
		curve[i].control[0].x /= 3;
		curve[i].control[0].y =
		    (curve[i].vertex.p.y * 2 + curve[j].vertex.p.y);
		curve[i].control[0].y /= 3;
		curve[i].control[1].x =
		    (curve[i].vertex.p.x + curve[j].vertex.p.x * 2);
		curve[i].control[1].y /= 3;
		curve[i].control[1].y =
		    (curve[i].vertex.p.y + curve[j].vertex.p.y * 2);
		curve[i].control[1].y /= 3;
...
Fb_fill_region(Fb_fill_state_t* pfs, const gs_fixed_rect* rect)
{
	...
	pfs1.n_color_args = 2;
	pfs1.rect = *rect;
	gs_point_transform2fixed(&pfs->ptm, fp->region.p.x, fp->region.p.y,
	                         &curve[0].vertex.p);
	gs_point_transform2fixed(&pfs->ptm, fp->region.q.x, fp->region.p.y,
	                         &curve[1].vertex.p);
	gs_point_transform2fixed(&pfs->ptm, fp->region.q.x, fp->region.q.y,
	                         &curve[2].vertex.p);
	gs_point_transform2fixed(&pfs->ptm, fp->region.p.x, fp->region.q.y,
	                         &curve[3].vertex.p);
	make_other_poles(curve);
	curve[0].vertex.cc[0] = fp->region.p.x;
	curve[0].vertex.cc[1] = fp->region.p.y;
	curve[1].vertex.cc[0] = fp->region.q.x;
	curve[1].vertex.cc[1] = fp->region.p.y;
	curve[2].vertex.cc[0] = fp->region.q.x;
	curve[2].vertex.cc[1] = fp->region.q.y;
	curve[3].vertex.cc[0] = fp->region.p.x;
	curve[3].vertex.cc[1] = fp->region.q.y;
                             ...
                             gs_imager_state* pis)
{
	...
	for(xi = 0; xi < 2; ++xi)
		for(yi = 0; yi < 2; ++yi) {
			...
			v[0] = x[xi], v[1] = y[yi];
			gs_function_evaluate(
			    psh->params.Function, v,
...
A_fill_region(A_fill_state_t* pfs, const gs_fixed_rect* rect_clip)
{
	...
	pfs1.rect = *rect_clip;
	pfs1.maybe_self_intersecting = false;
	gs_point_transform2fixed(&pfs->pis->ctm, x0 + pfs->delta.y * h0,
	                         y0 - pfs->delta.x * h0, &curve[0].vertex.p);
	gs_point_transform2fixed(&pfs->pis->ctm, x1 + pfs->delta.y * h0,
	                         y1 - pfs->delta.x * h0, &curve[1].vertex.p);
	gs_point_transform2fixed(&pfs->pis->ctm, x1 + pfs->delta.y * h1,
	                         y1 - pfs->delta.x * h1, &curve[2].vertex.p);
	gs_point_transform2fixed(&pfs->pis->ctm, x0 + pfs->delta.y * h1,
	                         y0 - pfs->delta.x * h1, &curve[3].vertex.p);
	curve[0].vertex.cc[0] = curve[0].vertex.cc[1] =
	    pfs->t0; /* The element cc[1] is set to a dummy value against */
	curve[1].vertex.cc[0] = curve[1].vertex.cc[1] =
	    pfs->t1; /* interrupts while an idle priocessing in gxshade.6.c . */
...
gs_shading_A_fill_rectangle_aux(const gs_shading_t* psh0, const gs_rect* rect,
                                const gs_fixed_rect* clip_rect, gx_device* dev,
                                gs_imager_state* pis)
{
...
private
int
R_tensor_annulus(patch_fill_state_t* pfs, const gs_rect* rect, double x0,
                 double y0, double r0, double t0, double x1, double y1,
                 double r1, double t1)
{
	...
	for(k = 0; k < 4; k++, p0 = p1) {
		...
		for(j = 0; j < 4; j++) {
			...
			code = gs_point_transform2fixed(
			    &pfs->pis->ctm, p[j * 3 + 0].x, p[j * 3 + 0].y,
			    &curve[jj].vertex.p);
			if(code < 0)
				return code;
			code = gs_point_transform2fixed(
			    &pfs->pis->ctm, p[j * 3 + 1].x, p[j * 3 + 1].y,
			    &curve[jj].control[0]);
			if(code < 0)
				return code;
			code = gs_point_transform2fixed(
			    &pfs->pis->ctm, p[j * 3 + 2].x, p[j * 3 + 2].y,
			    &curve[jj].control[1]);
			if(code < 0)
				return code;
		...
		curve[0].vertex.cc[1] = curve[1].vertex.cc[1] =
		    0; /* Initialize against FPE. */
		curve[2].vertex.cc[1] = curve[3].vertex.cc[1] =
		    0; /* Initialize against FPE. */
...
private
void
R_outer_circle(patch_fill_state_t* pfs, const gs_rect* rect, double x0,
...
private
int
R_fill_triangle_new(patch_fill_state_t* pfs, const gs_rect* rect, double x0,
...
private
bool
R_is_covered(double ax, double ay, const gs_point* p0, const gs_point* p1,
             const gs_point* p)
{
...
private
int
R_obtuse_cone(patch_fill_state_t* pfs, const gs_rect* rect, double x0,
              double y0, double r0, double x1, double y1, double r1, double t1,
              double r)
{
	...
	if(!covered[0] && !covered[1] && !covered[2] && !covered[3]) {
		return R_fill_triangle_new(pfs, rect, ax, ay, p0.x, p0.y, p1.x,
		                           p1.y, t1);
	}
	...
	for(i = 0; i < rp_count; i++) {
		code = R_fill_triangle_new(pfs, rect, ax, ay, pb.x, pb.y,
		                           rp[i].x, rp[i].y, t1);
		if(code < 0)
			return code;
		pb = rp[i];
	}
	return R_fill_triangle_new(pfs, rect, ax, ay, pb.x, pb.y, p1.x, p1.y,
	                           t1);
}

private
int
R_tensor_cone_apex(patch_fill_state_t* pfs, const gs_rect* rect, double x0,
                   double y0, double r0, double x1, double y1, double r1,
                   double t)
{
	double as = r0 / (r0 - r1);
...
private
int
R_extensions(patch_fill_state_t* pfs, const gs_shading_R_t* psh,
             const gs_rect* rect, double t0, double t1, bool Extend0,
             bool Extend1)
{
	...
	if(dr >= d - 1e-7 * (d + dr)) {
		...
		if(r0 > r1) {
			if(Extend0) {
				...
				if(r > r0) {
					code = R_tensor_annulus(pfs, rect, x0,
					                        y0, r, t0, x0,
					                        y0, r0, t0);
					if(code < 0)
						return code;
			...
			if(Extend1 && r1 > 0)
				return R_tensor_annulus(pfs, rect, x1, y1, r1,
				                        t1, x1, y1, 0, t1);
		} else {
			if(Extend1) {
				r = R_rect_radius(rect, x1, y1);
				if(r > r1) {
					code = R_tensor_annulus(pfs, rect, x1,
					                        y1, r, t1, x1,
					                        y1, r1, t1);
					if(code < 0)
						return code;
			...
			if(Extend0 && r0 > 0)
				return R_tensor_annulus(pfs, rect, x0, y0, r0,
				                        t0, x0, y0, 0, t0);
		}
	} else if(dr > d / 3) {
		/* Obtuse cone. */
		if(r0 > r1) {
			if(Extend0) {
				r = R_rect_radius(rect, x0, y0);
				code = R_obtuse_cone(pfs, rect, x0, y0, r0, x1,
				                     y1, r1, t0, r);
				if(code < 0)
					return code;
			}
			if(Extend1 && r1 != 0)
				return R_tensor_cone_apex(pfs, rect, x0, y0, r0,
				                          x1, y1, r1, t1);
			return 0;
		} else {
			if(Extend1) {
				r = R_rect_radius(rect, x1, y1);
				code = R_obtuse_cone(pfs, rect, x1, y1, r1, x0,
				                     y0, r0, t1, r);
				if(code < 0)
					return code;
			}
			if(Extend0 && r0 != 0)
				return R_tensor_cone_apex(pfs, rect, x1, y1, r1,
				                          x0, y0, r0, t0);
		}
	} else {
		...
		if(Extend0) {
			R_outer_circle(pfs, rect, x1, y1, r1, x0, y0, r0, &x3,
			               &y3, &r3);
			if(x3 != x1 || y3 != y1) {
				code = R_tensor_annulus(pfs, rect, x0, y0, r0,
				                        t0, x3, y3, r3, t0);
				if(code < 0)
					return code;
			}
		...
		if(Extend1) {
			R_outer_circle(pfs, rect, x0, y0, r0, x1, y1, r1, &x2,
			               &y2, &r2);
			if(x2 != x0 || y2 != y0) {
				code = R_tensor_annulus(pfs, rect, x1, y1, r1,
				                        t1, x2, y2, r2, t1);
				if(code < 0)
					return code;
			}
                                ...
                                gs_imager_state* pis)
{
	...
	state.dd = dd;
	memcpy(&pfs1, (shading_fill_state_t*)&state,
	       sizeof(shading_fill_state_t));
	...
	pfs1.rect = *clip_rect;
	pfs1.maybe_self_intersecting = false;
	code = R_extensions(&pfs1, psh, rect, d0, d1, psh->params.Extend[0],
	                    false);
	if(code < 0)
		return code;
	{
		...
		code = R_tensor_annulus(&pfs1, rect, x0, y0, r0, d0, x1, y1, r1,
		                        d1);
		if(code < 0)
			return code;
	}
	return R_extensions(&pfs1, psh, rect, d0, d1, false,
	                    psh->params.Extend[1]);
}
