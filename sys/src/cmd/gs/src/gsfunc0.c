...
private
int (*const fn_get_samples[])(const gs_function_Sd_t *pfn,
...
#define Power3(a) (a) * (a) * (a)
	...
	pole[pole_step * 1] = q1 * t01 * (-2 * t0 - t1 + 3 * t0 * t1) + t0 * (q2 * t0 + 2 * q2 * t1 - 3 * q2 * t0 * t1 + q3 * t0 * t1) - q0 * t11 * Power2(t01);
		  ...
		  const gs_range_t *pranges, gs_memory_t *mem)
{
	...
	if((code = (psfn->params.Size == 0 ? gs_note_error(gs_error_VMerror) : 0)) < 0 ||
