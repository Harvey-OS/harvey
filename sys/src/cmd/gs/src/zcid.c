...
private
bool
TT_char_code_from_CID_no_subst(const gs_memory_t* mem, const ref* Decoding,
                               const ref* TT_cmap, uint nCID, uint* c)
{
...
int
cid_to_TT_charcode(const gs_memory_t* mem, const ref* Decoding,
                   const ref* TT_cmap, const ref* SubstNWP, uint nCID, uint* c,
                   ref* src_type, ref* dst_type)
{
	...
	for(i = 0; i < SubstNWP_length; i += 5) {
		...
		ns = rs.value.intval;
		if(nCID >= nb && nCID <= ne)
			if(TT_char_code_from_CID_no_subst(
			       mem, Decoding, TT_cmap, ns + (nCID - nb), c)) {
				if((code = array_get(mem, SubstNWP, i + 0,
				                     src_type)) < 0)
					return code;
				if((code = array_get(mem, SubstNWP, i + 4,
				                     dst_type)) < 0)
					return code;
				return 1;
			}
		if(nCID >= ns && nCID <= ns + (ne - nb))
			if(TT_char_code_from_CID_no_subst(
			       mem, Decoding, TT_cmap, nb + (nCID - ns), c)) {
				if((code = array_get(mem, SubstNWP, i + 0,
				                     dst_type)) < 0)
					return code;
				if((code = array_get(mem, SubstNWP, i + 4,
...
private
int
set_CIDMap_element(const gs_memory_t* mem, ref* CIDMap, uint cid,
                   uint glyph_index)
...
int
cid_fill_CIDMap(const gs_memory_t* mem, const ref* Decoding, const ref* TT_cmap,
                const ref* SubstNWP, int GDBytes, ref* CIDMap)
{
	int dict_enum;
	ref el[2];
	int count, i;
	...
	if(r_type(CIDMap) != t_array)
		return_error(e_unregistered); /* Unimplemented. It could be a
		                                 single string. */
	count = r_size(CIDMap);
	...
	for(i = 0; i < count; i++) {
		...
		if(code < 0)
			return code;
		check_type(s, t_string); /* fixme : optimize with moving to
		                            TT_char_code_from_CID. */
	}
	/* Compute the CIDMap : */
	...
	for(;;) {
		...
		for(i = 0; i < count; i++) {
			...
			int code = cid_to_TT_charcode(
			    mem, Decoding, TT_cmap, SubstNWP, cid, &glyph_index,
			    &src_type, &dst_type);

			if(code < 0)
				return code;
			if(code > 0) {
				code = set_CIDMap_element(mem, CIDMap, cid,
				                          glyph_index);
				if(code < 0)
					return code;
			}
