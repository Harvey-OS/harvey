...
int
psf_type1_glyph_data(gs_font_base* pbfont, gs_glyph glyph, gs_glyph_data_t* pgd,
	...
	default: {
		...
		for(i = 0; i < 256; ++i) {
			...
			if(subset_glyphs && subset_size) {
				 ...
				 * subset.  Use binary search to check each
				 * glyph,
				 * since subset_glyphs are sorted.
				 */
				if(!psf_sorted_glyphs_include(
				       subset_glyphs, subset_size, glyph))
					continue;
			}
...
private
int
write_Private(stream* s, gs_font_type1* pfont, gs_glyph* subset_glyphs,
              uint subset_size, gs_glyph notdef, int lenIV,
              ...
              const param_printer_params_t* ppp)
{
	...
	stream_puts(
	    s, "/-|{string currentfile exch readstring pop}executeonly def\n");
	stream_puts(s, "/|-{noaccess def}executeonly def\n");
	stream_puts(s, "/|{noaccess put}executeonly def\n");
	{
	      private
		const gs_param_item_t private_items[] = {
		...
		code = gs_param_write_items(plist, pdata, &defaults,
		                            private_items);
		if(code < 0)
			return code;
		if(lenIV != 4) {
		...
		write_float_array(plist, "FamilyBlues",
		                  pdata->FamilyBlues.values,
		                  pdata->FamilyBlues.count);
		write_float_array(plist, "FamilyOtherBlues",
		                  pdata->FamilyOtherBlues.values,
...

	{
		...
		for(i = 0; i < n; ++i)
			if((code = pdata->procs.subr_data(pfont, i, false,
			                                  &gdata)) >= 0) {
				char buf[50];

				if(gdata.bits.size) {
					sprintf(buf, "dup %d %u -| ", i,
					        gdata.bits.size);
					stream_puts(s, buf);
					write_CharString(s, gdata.bits.data,
					                 gdata.bits.size);
					stream_puts(s, " |\n");
				}
				gs_glyph_data_free(&gdata,
...

	{
		...
		psf_enumerate_glyphs_begin(
		    &genum, (gs_font*)pfont, subset_glyphs,
		    (subset_glyphs ? subset_size : 0), GLYPH_SPACE_NAME);
		for(glyph = gs_no_glyph;
		    (code = psf_enumerate_glyphs_next(&genum, &glyph)) != 1;)
			if(code == 0 &&
			   (code = pdata->procs.glyph_data(pfont, glyph,
			                                   &gdata)) >= 0) {
				++num_chars;
				gs_glyph_data_free(
		...
		for(glyph = gs_no_glyph;
			...
			if(code == 0 &&
			   (code = pdata->procs.glyph_data(pfont, glyph,
				...
				code = pfont->procs.glyph_name((gs_font*)pfont,
				                               glyph, &gstr);
				assert(code >= 0);
				stream_puts(s, "/");
                ...
                int do_write)
{
	if(do_write) {
		pprints1(s, "\n/%s ", key);
		s_write_ps_string(s, pvalue->data, pvalue->size,
		                  PRINT_HEX_NOT_OK);
		stream_puts(s, " def");
	}
}
                     ...
                     const gs_const_string* alt_font_name, int lengths[3])
{
	...
	ppp.item_suffix = " def\n";
	ppp.print_ok = (options & WRITE_TYPE1_ASCIIHEX ? 0 : PRINT_BINARY_OK) |
	               PRINT_HEX_NOT_OK;
	...
	{
		gs_font_info_t info;
		int code = pfont->procs.font_info(
		    (gs_font*)pfont, NULL,
	...
	{
	      private
		const gs_param_item_t font_items[] = {
	...
	}
	{
		...
		write_float_array(plist, "WeightVector",
		                  pdata->WeightVector.values,
	...
	if(options & WRITE_TYPE1_EEXEC) {
		...
		if(options & WRITE_TYPE1_ASCIIHEX) {
			...
			s_init_state((stream_state*)&AXE_state, &s_AXE_template,
			             NULL);
		...
		s_init_filter(&exE_stream, (stream_state*)&exE_state, exE_buf,
		              sizeof(exE_buf), es);
		es = &exE_stream;
		/*
		 * Note: eexec encryption always writes/skips 4 initial bytes,
		 * not
	...
	code =
	    write_Private(es, pfont, glyphs.subset_glyphs, glyphs.subset_size,
