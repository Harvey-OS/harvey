...
#define gs_text_enum_common                                                    \
	...
	cached_fm_pair* pair;           /* corresponds to the current_font and \
	                                   CTM*(1<<log2_scale) */              \
	...
	gs_point                                                               \
	    FontBBox_as_Metrics2; /* used with FontType 9,11 && WMode 1 */     \
...
#define public_st_gs_text_enum() /* in gstext.c */                             \
	gs_public_st_composite(st_gs_text_enum, gs_text_enum_t,                \
	                       "gs_text_enum_t", text_enum_enum_ptrs,          \
	                       text_enum_reloc_ptrs)
 ...
 */
int gs_text_enum_init(gs_text_enum_t* pte, const gs_text_enum_procs_t* procs,
 ...
 */
void gs_text_enum_copy_dynamic(gs_text_enum_t* pto, const gs_text_enum_t* pfrom,
                               bool for_return);
...
#define SHOW_IS(penum, op_mask) (((penum)->text.operation & (op_mask)) != 0)
...
#define SHOW_IS_DO_KERN(penum) /* do_kern */ SHOW_IS(penum, TEXT_INTERVENE)
#define SHOW_IS_SLOW(penum) /* slow_show */                                    \
	SHOW_IS(penum, TEXT_REPLACE_WIDTHS | TEXT_ADD_TO_ALL_WIDTHS |          \
	                   TEXT_ADD_TO_SPACE_WIDTH | TEXT_INTERVENE)
...
#define text_enum_proc_process(proc) int proc(gs_text_enum_t* pte)
...
#define text_enum_proc_is_width_only(proc) bool proc(const gs_text_enum_t* pte)
...
#define text_enum_proc_retry(proc) int proc(gs_text_enum_t* pte)
