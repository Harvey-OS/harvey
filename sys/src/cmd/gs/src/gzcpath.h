...
/* Copyright (C) 1994, 1996, 1997, 1998, 1999 Aladdin Enterprises.  All rights
  reserved.
...
#define private_st_clip_rect_list() /* in gxcpath.c */                         \
	gs_private_st_ptrs_add0(st_clip_rect_list, gx_clip_rect_list,          \
	                        "gx_clip_rect_list", clip_rect_list_enum_ptrs, \
	                        clip_rect_list_reloc_ptrs, st_clip_list, list)
...
#define private_st_cpath_path_list() /* in gxcpath.c */                        \
	gs_private_st_suffix_add1(st_cpath_path_list, gx_cpath_path_list,      \
	                          "gs_cpath_list", cpath_path_list_enum_ptrs,  \
	                          cpath_path_list_reloc_ptrs, st_path, next)
...
/* Inline accessors. */
#define gx_cpath_is_shared(pcpath) ((pcpath)->rect_list->rc.ref_count > 1)

/* Define the structure for enumerating a clipping list. */
typedef enum { visit_left = 1, visit_right = 2 } cpe_visit_t;
typedef enum { cpe_scan, cpe_left, cpe_right, cpe_close, cpe_done } cpe_state_t;
...
#define private_st_cpath_enum() /* in gxcpath.c */                             \
	gs_private_st_suffix_add2(st_cpath_enum, gs_cpath_enum,                \
	                          "gs_cpath_enum", cpath_enum_enum_ptrs,       \
	                          cpath_enum_reloc_ptrs, st_path_enum, visit,  \
	                          rp)
