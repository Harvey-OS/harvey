...
{
	return (format == num_array ? r_size(op) : (r_size(op) - 4) / encoded_number_bytes(format));
}
...
sdecodelong(const byte *p, int format)
{
	...
	int32_t v = (num_is_lsb(format) ? ((int32_t)d << 24) + ((int32_t)c << 16) + (b << 8) + a : ((int32_t)a << 24) + ((int32_t)b << 16) + (c << 8) + d);
