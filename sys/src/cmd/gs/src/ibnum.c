...
/* Copyright (C) 1990, 1996, 1998, 1999 Aladdin Enterprises.  All rights
  reserved.
...
int
num_array_get(const gs_memory_t* mem, const ref* op, int format, uint index,
              ref* np)
{
	if(format == num_array) {
...
#define EXPN2(n) (0.5 / (1L << (n - 1)))
    1.0,       EXPN2(1),  EXPN2(2),  EXPN2(3),  EXPN2(4),  EXPN2(5),  EXPN2(6),
    EXPN2(7),  EXPN2(8),  EXPN2(9),  EXPN2(10), EXPN2(11), EXPN2(12), EXPN2(13),
    EXPN2(14), EXPN2(15), EXPN2(16), EXPN2(17), EXPN2(18), EXPN2(19), EXPN2(20),
    EXPN2(21), EXPN2(22), EXPN2(23), EXPN2(24), EXPN2(25), EXPN2(26), EXPN2(27),
...
sdecode_number(const byte* str, int format, ref* np)
{
	...
	case num_int32 + 16:
		if((format & 31) == 0) {
			...
			np->value.realval = (double)sdecodelong(str, format) *
	...
	case num_int16:
		if((format & 15) == 0) {
			...
			np->value.realval = sdecodeshort(str, format) *
	...
	if((format & ~(num_msb | num_lsb)) == num_float_native) {
...
#if !arch_floats_are_IEEE
		{
			...
			if(expt == 0 && mant == 0)
				...
				mant += 0x800000;
				fnum =
				    (float)ldexp((float)mant, expt - 127 - 23);
			}
			if(sign_expt & 0x100)
				fnum = -fnum;
		}
