...
private
int subpath_expand_dashes(const subpath*, gx_path*, const gs_imager_state*,
...
top:
	...
	while((pseg = pseg->next) != 0 && pseg->type != s_start) {
		...
		if(!(udx | udy)) { /* degenerate */
			if(gs_currentlinecap((const gs_state*)pis) !=
			   gs_cap_round) {
				/* From PLRM, stroke operator :
				   If a subpath is degenerate (consists of a
				   single-point closed path
				   or of two or more points at the same
				   coordinates),
				   stroke paints it only if round line caps have
				   been specified */
				continue;
			}
			dx = 0, dy = 0, length = 0;
		...
		if(ink_on) {
			if(drawing >= 0) {
				code =
				    (pseg->type == s_line_close && drawing > 0
				         ? gx_path_close_subpath_notes(
				               ppath, notes & pseg->notes)
				         : gx_path_add_line_notes(
				               ppath, sx, sy,
				               notes & pseg->notes));
				notes |= sn_not_first;
			}
			...
			if(elt_length < fixed2float(fixed_epsilon) &&
			   (pseg->next == 0 ||
			    pseg->next->type ==
			        s_start)) { /*
			                     * Ink is off, but we're within
			                     * epsilon of the end
			                     * of the dash element, and at the
			                     * end of the
			                     * subpath.  "Stretch" a little so
			                     * we get a dot.
			                     */
				if(code < 0)
					return code;
