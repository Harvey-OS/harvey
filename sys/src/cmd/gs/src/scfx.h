...
#define stream_CF_state_common                                                 \
	...
	int k_left /* number of next rows to encode in 2-D */ /* (only if K >  \
	                                                         0) */
...
#define s_CF_set_defaults_inline(ss)                                           \
	((ss)->Uncompressed = false, (ss)->K = 0, (ss)->EndOfLine = false,     \
	 (ss)->EncodedByteAlign = false, (ss)->Columns = 1728, (ss)->Rows = 0, \
	 (ss)->EndOfBlock = true,                                              \
	 (ss)->BlackIs1 = false, /* Added by Adobe since the Red Book */       \
	 (ss)->DamagedRowsBeforeError =                                        \
	     0,                          /* always set, for s_CF_get_params */ \
	 (ss)->FirstBitLowOrder = false, /* Added by us */                     \
	 (ss)->DecodedByteAlign = 1,     /* Clear pointers */                  \
	 (ss)->lbuf = 0,                                                       \
	 (ss)->lprev = 0)
...
#define private_st_CFE_state() /* in scfe.c */                                 \
	gs_private_st_ptrs3(st_CFE_state, stream_CFE_state,                    \
	                    "CCITTFaxEncode state", cfe_enum_ptrs,             \
	                    cfe_reloc_ptrs, lbuf, lprev, lcode)
...
#define private_st_CFD_state() /* in scfd.c */                                 \
	gs_private_st_ptrs2(st_CFD_state, stream_CFD_state,                    \
	                    "CCITTFaxDecode state", cfd_enum_ptrs,             \
	                    cfd_reloc_ptrs, lbuf, lprev)
