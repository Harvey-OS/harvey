...
type0_from_cidfont_cmap(gs_font_type0** ppfont0, gs_font* font,
                        ...
                        gs_memory_t* mem)
{
	gs_font_type0* font0 = (gs_font_type0*)gs_font_alloc(
	    mem, &st_gs_font_type0, &gs_font_procs_default, NULL,
	    "gs_type0_from_cidfont(font)");
	/* We allocate Encoding dynamically only for the sake of the GC. */
	uint* encoding = (uint*)gs_alloc_bytes(
	    mem, sizeof(uint), "gs_type0_from_cidfont(Encoding)");
	gs_font** fdep =
	...
	if(font0 == 0 || encoding == 0 || fdep == 0) {
		...
		gs_free_object(mem, encoding,
		               "gs_type0_from_cidfont(Encoding)");
                          ...
                          int wmode, bool use_cmap, gs_memory_t* mem)
{
	...
	if(use_cmap) {
		...
		code = gs_font_type0_from_cidfont(&pfont0, (gs_font*)pfcid,
		                                  wmode, NULL, mem);
	}
	if(code < 0) {
