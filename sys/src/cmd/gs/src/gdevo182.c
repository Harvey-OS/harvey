...
private
dev_proc_print_page(oki_print_page);

const gx_device_printer far_data gs_oki182_device =
    prn_device(prn_std_procs, "oki182", 80, /* width_10ths, 8.0" */
               110,                         /* height_10ths, 11" */
               72,                          /* x_dpi */
               72,                          /* y_dpi */
               0, 0, 0, 0,                  /* margins */
...
private
byte*
oki_compress(byte* in, int origWidth, int highRes, int* numSpaces,
             int* newWidth)
{
	int spaces = 0;
	...
	while(in < in_end && in[0] == 0x80 &&
	      memcmp((char*)in, (char*)in + 1, columns_per_space - 1) == 0) {
		spaces++;
...
oki_print_page(gx_device_printer* pdev, FILE* prn_stream)
{
	...
	byte* in =
	    (byte*)gs_malloc(pdev->memory, 16, line_size, "oki_print_page(in)");

	byte* out1 = (byte*)gs_malloc(pdev->memory, 8, line_size,
	                              "oki_print_page(out1)");
	byte* out2 = (byte*)gs_malloc(pdev->memory, 8, line_size,
	                              "oki_print_page(out2)");
	...
	while(lnum < pdev->height) {
		...
		if(skip)
			fprintf(prn_stream, "\033%%5%c", (char)(skip & 0xff));
		skip = 0;

		/* get the rest of the scan lines */
		code =
		    gdev_prn_copy_scan_lines(pdev, lnum + 1, in + line_size,
		                             (bits_per_column - 1) * line_size);
		...
		if(highRes) {
			...
			oki_transpose(in + line_size, out2, pdev->width,
			              2 * line_size);
		} else
			oki_transpose(in, out1, pdev->width, line_size);

		out3 =
		    oki_compress(out1, pdev->width, highRes, &spaces, &width);
		...
		if(highRes) {
			...
			out3 = oki_compress(out2, pdev->width, highRes, &spaces,
			                    &width);
			for(i = 0; i < spaces; i++)
				putc(' ', prn_stream);
			fwrite("\003", 1, 1, prn_stream);
...
bail:
	if(out1 != 0)
		gs_free(pdev->memory, (char*)out1, 8, line_size,
		        "oki_print_page(out1)");

	if(out2 != 0)
		gs_free(pdev->memory, (char*)out2, 8, line_size,
		        "oki_print_page(out2)");

	if(in != 0)
		gs_free(pdev->memory, (char*)in, 16, line_size,
		        "oki_print_page(in)");
