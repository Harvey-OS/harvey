	...
	for(str = *pcont; parse_line(&str, &line);) {
		...
		*end = 0;
		...
		if(!strcmp(line, "HalftoneType")) {
			...
			if(sscanf(end, "%d", &Width) != 1 || Width <= 0 ||
			   Width > 0x4000) {
				fprintf(stderr, "Invalid Width: %s\n", end);
				return -1;
			}
		} else if(!strcmp(line, "Height")) {
			if(sscanf(end, "%d", &Height) != 1 || Height <= 0 ||
			   Height > 0x4000) {
			...
			if(Width < 0 || Height < 0) {
				fprintf(stderr, "Width and Height must precede "
				                "Thresholds.\n");
				return -1;
			}
			...
			phtr->bit_data = malloc(num_bits * sizeof(ushort));
			...
			s_AXD_template.process((stream_state*)&ss, &r, &w,
			                       true);
			str = (char*)r.ptr + 1;
			break;
		}
               ...
               const char* prefix, int index)
{
	...
	fprintf(out, "static const gx_device_halftone_resource_t res_%d = {\n  "
	             "  \"%s\", %d, %d, %d, %d, levels_%d, bit_data_%d, "
	             "%u\n};\n",
	        index, phtr->rname, phtr->HalftoneType, phtr->Width,
	        phtr->Height, phtr->num_levels, index, index,
...
main(int argc, char* argv[])
{
	...
	fprintf(out, "gs_dht_%s(void)\n{\n    static const "
	             "gx_device_halftone_resource_t *const res[] = {\n\t",
	        prefix);
	for(i = 0; i < index; ++i)
