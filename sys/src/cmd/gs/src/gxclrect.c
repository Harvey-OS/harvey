...
int
cmd_write_rect_cmd(gx_device_clist_writer* cldev, gx_clist_state* pcls, int op,
...
#define check_range_w(rmin, rmax) ((unsigned)(dwidth - rmin) <= (rmax - rmin))
	...
	FOR_RECTS
	{
		...
		TRY_RECT
		{
			...
			if(code >= 0 && color != pcls->colors[1])
				code = cmd_put_color(cdev, pcls,
				                     &clist_select_color1,
				                     color, &pcls->colors[1]);
			if(code >= 0)
				code = cmd_write_rect_cmd(cdev, pcls,
				                          cmd_op_fill_rect, x,
				                          y, width, height);
		}
		HANDLE_RECT(code);
...
int
clist_strip_tile_rectangle(gx_device* dev, const gx_strip_bitmap* tile, int x,
                           int y, int width, int height, gx_color_index color0,
                           gx_color_index color1, int px, int py)
{
	gx_device_clist_writer* const cdev = &((gx_device_clist*)dev)->writer;
	int depth = (color1 == gx_no_color_index && color0 == gx_no_color_index
	...
	FOR_RECTS
	{
		...
		TRY_RECT { code = cmd_disable_lop(cdev, pcls); }
		HANDLE_RECT(code);
		if(!cls_has_tile_id(cdev, pcls, tile->id, offset_temp)) {
			...
			if(tile->id != gx_no_bitmap_id) {
				TRY_RECT
				{
					code = clist_change_tile(cdev, pcls,
					                         tile, depth);
				}
			...
			if(code < 0) {
				...
				code = gx_default_strip_tile_rectangle(
				    dev, tile, x, y, width, height, color0,
				    color1, px, py);
				if(code < 0)
					ERROR_RECT(code);
				goto endr;
		...
		TRY_RECT
		{
			...
			if(color0 != pcls->tile_colors[0] ||
			   color1 != pcls->tile_colors[1])
				code = cmd_set_tile_colors(cdev, pcls, color0,
				                           color1);
			if(px != pcls->tile_phase.x ||
			   py != pcls->tile_phase.y) {
				if(code >= 0)
					code = cmd_set_tile_phase(cdev, pcls,
					                          px, py);
			}
			if(code >= 0)
				code = cmd_write_rect_cmd(cdev, pcls,
...
int
clist_copy_mono(gx_device* dev, const byte* data, int data_x, int raster,
                ...
                gx_color_index color0, gx_color_index color1)
{
	...
	copy : {
		...
		{
			code =
			    cmd_put_bits(cdev, pcls, row, w1, height, raster,
		...
		if(code < 0) {
			...
			if(height > 1) {
				...
				NEST_RECT
				{
					code = clist_copy_mono(
					    dev, row, dx, raster,
					    gx_no_bitmap_id, x, y, w2, 1,
					    color0, color1);
					if(code >= 0)
						code = clist_copy_mono(
						    dev, row, dx + w2, raster,
...
int
clist_copy_color(gx_device* dev, const byte* data, int data_x, int raster,
                 gx_bitmap_id id, int x, int y, int width, int height)
{
	gx_device_clist_writer* const cdev = &((gx_device_clist*)dev)->writer;
	...
	FOR_RECTS
	{
		...
		if(pcls->color_is_alpha) {
			...
			{
				code = set_cmd_put_op(
				    dp, cdev, pcls, cmd_opv_set_copy_color, 1);
			}
			HANDLE_RECT(code);
	...
	copy : {
		...
		TRY_RECT
		{
			code = cmd_put_bits(cdev, pcls, row, w1 * depth, height,
			                    raster, rsize,
...
int
clist_copy_alpha(gx_device* dev, const byte* data, int data_x, int raster,
                 gx_bitmap_id id, int x, int y, int width, int height,
                 gx_color_index color, int depth)
{
	...
	FOR_RECTS
	{
		...
		if(color != pcls->colors[1]) {
			TRY_RECT { code = cmd_set_color1(cdev, pcls, color); }
			HANDLE_RECT(code);
		}
	copy : {
		gx_cmd_rect rect;
		...
		if(code < 0) {
			...
			if(height > 1) {
				...
				NEST_RECT
				{
					code = clist_copy_alpha(
					    dev, row, dx, raster,
					    gx_no_bitmap_id, x, y, w2, 1, color,
					    depth);
					if(code >= 0)
						code = clist_copy_alpha(
						    dev, row, dx + w2, raster,
...
int
clist_strip_copy_rop(gx_device* dev, const byte* sdata, int sourcex,
                     ...
                     gs_logical_operation_t lop)
{
	...
	bool subtractive =
	    dev->color_info.num_components == 4; /****** HACK ******/
	gx_color_index S =
	...
	FOR_RECTS
	{
		...
		/* Reducing D, S, T to rop_operand (which apparently is 32 bit)
		   appears safe
		   due to 'all' a has smaller snumber of significant bits. */
		pcls->colors_used.or
		    = ((rop_proc_table[color_rop])(
		           (rop_operand)D, (rop_operand)S, (rop_operand)T) &
		       all) |
		      D;
		...
		if(rop3_uses_T(rop)) {
			if(tcolors == 0 || tcolors[0] != tcolors[1]) {
				...
				if(!cls_has_tile_id(cdev, pcls, tiles->id,
				                    offset_temp)) {
					/* Change tile.  If there is no id,
					 * generate one. */
					if(tiles->id == gx_no_bitmap_id) {
						tile_with_id = *tiles;
						tile_with_id.id =
						    gs_next_ids(dev->memory, 1);
						tiles = &tile_with_id;
					}
					TRY_RECT
					{
						code = clist_change_tile(
						    cdev, pcls, tiles,
						    (tcolors != 0
						         ? 1
						         : dev->color_info
					...
					HANDLE_RECT_UNLESS(
					    code, code == gs_error_limitcheck);
					if(code < 0) {
						/*
						 * The error is a limitcheck: we
						 * have a tile that
						 * is too big to fit in the
						 * command reading buffer.
						 * For now, just divide up the
						 * transfer into scan
						 * lines.  (If a single scan
						 * line won't fit, punt.)
						 * Eventually, we'll need a way
						 * to transfer the tile
						 * in pieces.
						 */
						uint rep_height =
						    tiles->rep_height;
						...
						if(rep_height == 1 ||
						   ...
						   tiles->rep_shift != 0)
							return code;
						/*
						 * Allocate enough fake IDs,
						 * since the inner call on
						 * clist_strip_copy_rop will
						 * need them anyway.
						 */
						ids = gs_next_ids(
						    dev->memory,
						    min(height, rep_height));
						line_tile = *tiles;
						line_tile.size.y = 1;
			...
			{
				code = (tcolors != 0
				            ? cmd_set_tile_colors(cdev, pcls,
				                                  tcolors[0],
				                                  tcolors[1])
				            : cmd_set_tile_colors(
				                  cdev, pcls, gx_no_color_index,
				                  gx_no_color_index));
			}
			HANDLE_RECT(code);
		}
		...
		NEST_RECT
		{
			if(scolors != 0) {
				if(scolors[0] == scolors[1])
					code = clist_fill_rectangle(
					    dev, x, y, width, height,
					    scolors[1]);
				else
					code = clist_copy_mono(
					    dev, row, sourcex, sraster, id, x,
					    y, width, height, scolors[0],
					    scolors[1]);
			} else
				code =
				    clist_copy_color(dev, row, sourcex, sraster,
				                     id, x, y, width, height);
		}
		UNNEST_RECT;
		pcls->lop_enabled = 1;
