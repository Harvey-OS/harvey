			   ...
			   gx_color_index color0, gx_color_index color1, int px, int py)
{
	gx_device_clist_writer *const cdev =
	    &((gx_device_clist *)dev)->writer;
	int depth =
	    (color1 == gx_no_color_index && color0 == gx_no_color_index ? dev->color_info.depth : 1);
	...
	copy : {
		...
		TRY_RECT
		{
			code = cmd_put_bits(cdev, pcls, row, w1, height, raster,
					    rsize, (orig_id == gx_no_bitmap_id ? 1 << cmd_compress_rle : cmd_mask_compress_any),
					    &dp, &csize);
		}
		     ...
		     int phase_x, int phase_y, gs_logical_operation_t lop)
{
     ...
     */
	{
		...
		if(scolors && (scolors[0] == scolors[1]))
			rop_used = (scolors[0] == gx_device_black(dev) ? rop3_know_S_0(rop_used) : scolors[0] == gx_device_white(dev) ? rop3_know_S_1(rop_used) : rop_used);
		if(tcolors && (tcolors[0] == tcolors[1]))
			rop_used = (tcolors[0] == gx_device_black(dev) ? rop3_know_T_0(rop_used) : tcolors[0] == gx_device_white(dev) ? rop3_know_T_1(rop_used) : rop_used);
	...
	FOR_RECTS
	{
	   ...
	   due to 'all' a has smaller snumber of significant bits. */
		...
		if(rop3_uses_T(rop)) {
				...
				if(!cls_has_tile_id(cdev, pcls, tiles->id, offset_temp)) {
					...
					TRY_RECT
					{
						code = clist_change_tile(cdev, pcls, tiles,
									 (tcolors != 0 ? 1 : dev->color_info.depth));
					}
					HANDLE_RECT_UNLESS(code, code == gs_error_limitcheck);
					if(code < 0) {
						/*
			...
			{
				code =
				    (tcolors != 0 ? cmd_set_tile_colors(cdev, pcls, tcolors[0], tcolors[1]) : cmd_set_tile_colors(cdev, pcls, gx_no_color_index,
																  gx_no_color_index));
			}
			HANDLE_RECT(code);
		}
