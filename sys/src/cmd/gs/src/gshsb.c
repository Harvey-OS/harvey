...
/* Forward references */
private
void color_hsb_to_rgb(floatp h, floatp s, floatp b, float rgb[3]);
private
void color_rgb_to_hsb(floatp r, floatp g, floatp b, float hsb[3]);
...
color_hsb_to_rgb(floatp hue, floatp saturation, floatp brightness, float rgb[3])
{
	if(saturation == 0) {
		...
		floatp h6 = hue * 6;
		uint32_t V =
		    float2frac(brightness); /* force arithmetic to long */
