...
/* Forward references */
private
void gx_clip_list_from_rectangle(gx_clip_list*, gs_fixed_rect*);
...
private
ENUM_PTRS_WITH(clip_path_enum_ptrs,
               gx_clip_path* cptr) return ENUM_USING(st_path, &cptr->path,
                                                     sizeof(cptr->path),
                                                     index - 2);

case 0:
return ENUM_OBJ((cptr->rect_list == &cptr->local_list ? 0 : cptr->rect_list));
case 1:
...
ENUM_PTRS_WITH(device_clip_enum_ptrs, gx_device_clip* cptr)
{
	...
	return ENUM_USING(st_device_forward, vptr, sizeof(gx_device_forward),
...
RELOC_PTRS_WITH(device_clip_reloc_ptrs, gx_device_clip* cptr)
{
	if(cptr->current == &cptr->list.single)
		cptr->current =
		    &((gx_device_clip*)RELOC_OBJ(vptr))->list.single;
	else
...
private
const gx_clip_list clip_list_empty = {
    {0, 0, min_int, max_int, 0, 0}, 0, 0, 0, 0, 0};

/* ------ Clipping path memory management ------ */

private
rc_free_proc(rc_free_cpath_list);
private
rc_free_proc(rc_free_cpath_list_local);
private
rc_free_proc(rc_free_cpath_path_list);
...
cpath_init_rectangle(gx_clip_path* pcpath, gs_fixed_rect* pbox)
{
	...
	pcpath->id =
	    gs_next_ids(pcpath->path.memory, 1); /* path changed => change id */
}
private
void
...
int
gx_cpath_init_contained_shared(gx_clip_path* pcpath, const gx_clip_path* shared,
                               gs_memory_t* mem, client_name_t cname)
{
	if(shared) {
		if(shared->path.segments == &shared->path.local_segments) {
			lprintf1("Attempt to share (local) segments of clip "
			         "path 0x%lx!\n",
			         (uint32_t)shared);
                           ...
                           gs_memory_t* mem)
{
	if(shared) {
		...
		rc_init_free(&pcpath->local_list, mem, 1,
		             rc_free_cpath_list_local);
...
gx_cpath_unshare(gx_clip_path* pcpath)
{
	...
	if(rlist->rc.ref_count > 1) {
		int code =
		    cpath_alloc_list(&pcpath->rect_list, pcpath->path.memory,
		                     "gx_cpath_unshare");
...
gx_cpath_assign_preserve(gx_clip_path* pcpto, gx_clip_path* pcpfrom)
{
	...
	if(fromlist == &pcpfrom->local_list) {
		...
		if(tolist == &pcpto->local_list || tolist->rc.ref_count > 1) {
			/* We can't use pcpto's list either.  Allocate a new
			 * one. */
...
private
void
rc_free_cpath_list_local(gs_memory_t* mem, void* vrlist, client_name_t cname)
{
...
gx_cpath_path_list_new(gs_memory_t* mem, gx_clip_path* pcpath, int rule,
                       gx_path* ppfrom, gx_cpath_path_list* next,
                       gx_cpath_path_list** pnew)
{
	int code;
	client_name_t cname = "gx_cpath_path_list_new";
	gx_cpath_path_list* pcplist = gs_alloc_struct(
	    mem, gx_cpath_path_list, &st_cpath_path_list, cname);

	if(pcplist == 0)
		return_error(gs_error_VMerror);
	rc_init_free(pcplist, mem, 1, rc_free_cpath_path_list);
	if(pcpath != NULL && !pcpath->path_valid) {
		code = gx_path_init_contained_shared(&pcplist->path, NULL, mem,
		                                     cname);
		if(code < 0)
			return code;
...
gx_cpath_to_path(gx_clip_path* pcpath, gx_path* ppath)
{
	if(!pcpath->path_valid) {
		...
		while((code = gx_cpath_enum_next(&cenum, pts)) != 0) {
			switch(code) {
			case gs_pe_moveto:
				code = gx_path_add_point(&rpath, pts[0].x,
				                         pts[0].y);
				break;
			case gs_pe_lineto:
				code = gx_path_add_line_notes(
				    &rpath, pts[0].x, pts[0].y,
				    gx_cpath_enum_notes(&cenum));
				break;
			case gs_pe_curveto:
				code = gx_path_add_curve_notes(
				    &rpath, pts[0].x, pts[0].y, pts[1].x,
				    pts[1].y, pts[2].x, pts[2].y,
				    gx_cpath_enum_notes(&cenum));
				break;
			case gs_pe_closepath:
				code = gx_path_close_subpath_notes(
				    &rpath, gx_cpath_enum_notes(&cenum));
				break;
			default:
				if(code >= 0)
					code = gs_note_error(
...
bool
gx_cpath_includes_rectangle(register const gx_clip_path* pcpath, fixed x0,
                            fixed y0, fixed x1, fixed y1)
{
	return (x0 <= x1 ? (pcpath->inner_box.p.x <= x0 &&
	                    x1 <= pcpath->inner_box.q.x)
	                 : (pcpath->inner_box.p.x <= x1 &&
	                    x0 <= pcpath->inner_box.q.x)) &&
	       (y0 <= y1 ? (pcpath->inner_box.p.y <= y0 &&
	                    y1 <= pcpath->inner_box.q.y)
	                 : (pcpath->inner_box.p.y <= y1 &&
	                    y0 <= pcpath->inner_box.q.y));
}
                   ...
                   int rule, gs_imager_state* pis)
{
	...
	if(gx_path_has_curves_inline(ppath)) {
		...
		code = gx_path_add_flattened_accurate(
		    ppath, &fpath, gs_currentflat_inline(pis),
		    pis->accurate_curves);
		if(code < 0)
			return code;
		ppath = &fpath;
	...
	if(gx_cpath_inner_box(pcpath, &old_box) &&
	   ((code = gx_path_is_rectangle(ppath, &new_box)) ||
		...
		bool path_valid = gx_cpath_inner_box(pcpath, &old_box) &&
		                  gx_path_bbox(ppath, &new_box) >= 0 &&
		                  gx_cpath_includes_rectangle(
		                      pcpath, new_box.p.x, new_box.p.y,
		                      new_box.q.x, new_box.q.y);

		if(!path_valid && next == NULL) {
			code = gx_cpath_path_list_new(
			    pcpath->path.memory, pcpath, pcpath->rule,
		...
		if(path_valid) {
			...
			code = gx_cpath_path_list_new(pcpath->path.memory, NULL,
			                              rule, ppath_orig, next,
			                              &pcpath->path_list);
		}
	}
ex:
                           ...
                           bool segments_shared)
{
	int code =
	    (pcpath->path_valid
	         ? gx_path_scale_exp2_shared(&pcpath->path, log2_scale_x,
	                                     log2_scale_y, segments_shared)
	         : 0);
...
gx_cpath_enum_init(gs_cpath_enum* penum, gx_clip_path* pcpath)
{
	if((penum->using_path = pcpath->path_valid)) {
		...
		gx_clip_rect* head =
		    (clp->count <= 1 ? &clp->single : clp->head);
		gx_clip_rect* rp;
...
#define set_pt(xi, yi) (pts[0].x = int2fixed(xi), pts[0].y = int2fixed(yi))
#define set_line(xi, yi)                                                       \
	(penum->line_end.x = (xi), penum->line_end.y = (yi),                   \
	 penum->have_line = true)
	if(penum->have_line) {
	...
	break
		...
		left: /* Trace upward along a left edge. */
			...
			if(look == 0 || look->ymin > rp->ymax ||
			   look->xmin >= rp->xmax) { /* No adjacent rectangle,
			                                switch directions. */
				state =
				    (rp == visit && first_visit == visit_right
				         ? cpe_close
			 ...
			 */
			{
				...
				if(prev != 0 && prev->ymax == rp->ymax &&
				   look->xmin <
				       prev->xmax) { /* There's an adjoining
				                        rectangle as well. */
					/* Switch directions. */
					rp = prev;
					state =
		...
		right: /* Trace downward along a right edge. */
			...
			if(look == 0 || look->ymax < rp->ymin ||
			   look->xmax <= rp->xmin) { /* No adjacent rectangle,
			                                switch directions. */
				state =
				    (rp == visit && first_visit == visit_left
				         ? cpe_close
				         : (set_line(rp->xmin, rp->ymin),
				            cpe_left));
			...
			/* We found an adjacent rectangle. */
			/* See if it also adjoins a rectangle to the right of
			 * rp. */
			{
				...
				if(next != 0 && next->ymin == rp->ymin &&
				   look->xmax >
				       next->xmin) { /* There's an adjoining
				                        rectangle as well. */
					/* Switch directions. */
					rp = next;
					state =
