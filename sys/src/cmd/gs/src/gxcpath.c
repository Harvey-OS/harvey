...
case 0:
ENUM_RETURN((cptr->current == &cptr->list.single ? NULL : (void *)cptr->current));
ENUM_PTRS_END
private
...
{
	return (x0 <= x1 ? (pcpath->inner_box.p.x <= x0 && x1 <= pcpath->inner_box.q.x) : (pcpath->inner_box.p.x <= x1 && x0 <= pcpath->inner_box.q.x)) &&
	       (y0 <= y1 ? (pcpath->inner_box.p.y <= y0 && y1 <= pcpath->inner_box.q.y) : (pcpath->inner_box.p.y <= y1 && y0 <= pcpath->inner_box.q.y));
}
...
{
	int code =
	    (pcpath->path_valid ? gx_path_scale_exp2_shared(&pcpath->path, log2_scale_x, log2_scale_y,
							    segments_shared)
				: 0);
			    ...
			    register gs_fixed_rect *rp)
{
	...
	clp->single.xmax = clp->xmax =
	    (rp->q.x == rp->p.x ? clp->single.xmin : fixed2int_var_ceiling(rp->q.x));
	clp->single.ymax =
	    (rp->q.y == rp->p.y ? clp->single.ymin : fixed2int_var_ceiling(rp->q.y));
	clp->count = 1;
}
...
gx_cpath_enum_init(gs_cpath_enum *penum, gx_clip_path *pcpath)
{
	if((penum->using_path = pcpath->path_valid)) {
		...
		for(rp = head; rp != 0; rp = rp->next)
			rp->to_visit =
			    (rp->xmin < rp->xmax && rp->ymin < rp->ymax ? visit_left | visit_right : 0);
	...
	break
		...
		left: /* Trace upward along a left edge. */
			...
			if(look == 0 || look->ymin > rp->ymax ||
			   look->xmin >= rp->xmax) { /* No adjacent rectangle, switch directions. */
				state =
				    (rp == visit && first_visit == visit_right ? cpe_close : (set_line(rp->xmax, rp->ymax), cpe_right));
			...
			/* See if it also adjoins a rectangle to the left of rp. */
			{
				...
				if(prev != 0 && prev->ymax == rp->ymax &&
				   look->xmin < prev->xmax) { /* There's an adjoining rectangle as well. */
					/* Switch directions. */
					rp = prev;
					state =
					    (rp == visit && first_visit == visit_right ? cpe_close : (set_line(prev->xmax, prev->ymax), cpe_right));
		...
		right: /* Trace downward along a right edge. */
			...
			if(look == 0 || look->ymax < rp->ymin ||
			   look->xmax <= rp->xmin) { /* No adjacent rectangle, switch directions. */
				state =
				    (rp == visit && first_visit == visit_left ? cpe_close : (set_line(rp->xmin, rp->ymin), cpe_left));
			...
			/* See if it also adjoins a rectangle to the right of rp. */
			{
				...
				if(next != 0 && next->ymin == rp->ymin &&
				   look->xmax > next->xmin) { /* There's an adjoining rectangle as well. */
					/* Switch directions. */
					rp = next;
					state =
					    (rp == visit && first_visit == visit_left ? cpe_close : (set_line(next->xmin, next->ymin), cpe_left));
