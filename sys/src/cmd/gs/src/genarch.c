...
main(int argc, char* argv[])
{
	...
	fprintf(f, "/* Parameters derived from machine and compiler "
	           "architecture. */\n");
	...
	floats_are_IEEE =
	    (size_of(float) == size_of(int)
	         ? f0.i == 0 && f1.i == (int)0x3f800000 &&
	               fm1.i == (int)0xbf800000
...
#undef OFFSET_IN
	...
	if(floats_are_IEEE) {
		 ...
		 */
		define_int(f, "ARCH_FLOAT_MANTISSA_BITS",
		           sizeof(float) * 8 - 17);
		define_int(f, "ARCH_DOUBLE_MANTISSA_BITS",
		           sizeof(double) * 8 - 17);
	}
...
#define MAX_NREPS (1 << 10) /* limit the number of reps we try */
		...
		while(nreps < MAX_NREPS &&
		      (t = time_clear(buf, bsize, nreps)) == 0)
			nreps <<= 1;
		t_eps = t;
		while(nreps < MAX_NREPS &&
		      (t = time_clear(buf, bsize, nreps)) < t_eps * 10)
			nreps <<= 1;
