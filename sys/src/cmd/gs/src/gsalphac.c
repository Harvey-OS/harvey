 ...
 */
int composite_values(const pixel_row_t* pdest, const const_pixel_row_t* psource,
 ...
 */
#define alpha_out_notS_notD (1 << composite_Dissolve)
#define _alpha_out_either                                                      \
	(alpha_out_notS_notD | (1 << composite_Satop) |                        \
	 (1 << composite_Datop) | (1 << composite_Xor) |                       \
	 (1 << composite_PlusD) | (1 << composite_PlusL))
...
private
composite_create_default_compositor_proc(c_alpha_create_default_compositor);
private
composite_equal_proc(c_alpha_equal);
private
composite_write_proc(c_alpha_write);
private
composite_read_proc(c_alpha_read);
const gs_composite_type_t gs_composite_alpha_type = {
    GX_COMPOSITOR_ALPHA,
    {c_alpha_create_default_compositor, c_alpha_equal, c_alpha_write,
     c_alpha_read, gx_default_composite_clist_write_update,
                          ...
                          gs_memory_t* mem)
{
	...
	rc_alloc_struct_0(pcte, gs_composite_alpha_t, &st_composite_alpha, mem,
...
c_alpha_write(const gs_composite_t* pcte, byte* data, uint* psize)
{
	...
	if(pacte->params.op == composite_Dissolve) {
		...
		memcpy(data + 1, &pacte->params.delta,
		       sizeof(pacte->params.delta));
	} else {
		used = 1;
		if(size < used) {
			*psize = used;
...
gs_private_st_suffix_add0_final(st_device_composite_alpha,
                                ...
                                device_c_alpha_reloc_ptrs, gx_device_finalize,
                                st_device_forward);
/* The device descriptor. */
private
dev_proc_close_device(dca_close);
private
dev_proc_fill_rectangle(dca_fill_rectangle);
private
dev_proc_map_rgb_color(dca_map_rgb_color);
private
dev_proc_map_color_rgb(dca_map_color_rgb);
private
dev_proc_copy_mono(dca_copy_mono);
private
dev_proc_copy_color(dca_copy_color);
private
dev_proc_map_rgb_alpha_color(dca_map_rgb_alpha_color);
private
dev_proc_map_color_rgb_alpha(dca_map_color_rgb_alpha);
private
dev_proc_copy_alpha(dca_copy_alpha);
private
const gx_device_composite_alpha gs_composite_alpha_device = {
    std_device_std_body_open(gx_device_composite_alpha, 0, "alpha compositor",
                             0, 0, 1, 1),
    {gx_default_open_device, gx_forward_get_initial_matrix,
     gx_default_sync_output, gx_default_output_page, dca_close,
     dca_map_rgb_color, dca_map_color_rgb, dca_fill_rectangle,
     gx_default_tile_rectangle, dca_copy_mono, dca_copy_color,
     gx_default_draw_line, gx_default_get_bits, gx_forward_get_params,
     gx_forward_put_params,
     gx_default_cmyk_map_cmyk_color, /* only called for CMYK */
     gx_forward_get_xfont_procs, gx_forward_get_xfont_device,
     dca_map_rgb_alpha_color, gx_forward_get_page_device,
     gx_forward_get_alpha_bits, dca_copy_alpha, gx_forward_get_band,
     gx_default_copy_rop, gx_default_fill_path, gx_default_stroke_path,
     gx_default_fill_mask, gx_default_fill_trapezoid,
     gx_default_fill_parallelogram, gx_default_fill_triangle,
     ...
     gx_default_strip_copy_rop, gx_forward_get_clipping_box,
     gx_default_begin_typed_image, gx_forward_get_bits_rectangle,
     dca_map_color_rgb_alpha, gx_no_create_compositor}};

/* Create an alpha compositor. */
private
int
c_alpha_create_default_compositor(const gs_composite_t* pcte, gx_device** pcdev,
                                  ...
                                  gs_memory_t* mem)
{
	...
	cdev = gs_alloc_struct_immovable(mem, gx_device_composite_alpha,
...
{
	return dca_map_rgb_alpha_color(dev, cv[0], cv[1], cv[2],
	                               gx_max_color_value);
}
private
gx_color_index
dca_map_rgb_alpha_color(gx_device* dev, gx_color_value red,
...
#define premult_(c) (((c)*a + gx_max_color_value / 2) / gx_max_color_value)
...
#define premult(c) premult_(c)
	...
	if(dev->color_info.num_components == 1) {
		uint lum = (red * lum_red_weight + green * lum_green_weight +
		...
		if(a == 0xff)
			color = ((uint)gx_color_value_to_byte(red) << 16) +
			        ((uint)gx_color_value_to_byte(green) << 8) +
			        gx_color_value_to_byte(blue);
		else /* Premultiplication is necessary. */
			color = (premult(red) << 16) + (premult(green) << 8) +
			        premult(blue);
	}
#undef premult
	return (color << 8) + a;
...
private
int
dca_map_color_rgb(gx_device* dev, gx_color_index color, gx_color_value prgb[3])
{
	gx_color_value red = gx_color_value_from_byte((byte)(color >> 24));
	byte a = (byte)color;

#define postdiv_(c) (((c)*0xff + a / 2) / a)
#ifdef PREMULTIPLY_TOWARDS_WHITE
	byte bias = ~a;
...
#define postdiv(c) postdiv_(c)
	...
	if(dev->color_info.num_components == 1) {
		...
		gx_color_value green = gx_color_value_from_byte(
		                   (byte)(color >> 16)),
		               blue =
		                   gx_color_value_from_byte((byte)(color >> 8));
...
{ /* This is where all the real work gets done! */
	...
	std_row =
	    gs_alloc_bytes(dev->memory, (dev->color_info.depth * w + 7) >> 3,
	                   "dca_fill_rectangle(std)");
	native_row =
	    gs_alloc_bytes(dev->memory, (target->color_info.depth * w + 7) >> 3,
	...
	std_params.options =
	    GB_COLORS_NATIVE |
	    (GB_ALPHA_LAST | GB_DEPTH_8 | GB_PACKING_CHUNKY | GB_RETURN_COPY |
	     GB_RETURN_POINTER | GB_ALIGN_ANY | GB_OFFSET_0 | GB_OFFSET_ANY |
	...
	for(yi = y; yi < y + h; ++yi) {
		...
		dest.initial_x =
		    (std_params.options & GB_OFFSET_ANY ? std_params.x_offset
		                                        : 0);
		dest.alpha = (std_params.options & GB_ALPHA_FIRST
		                  ? gs_image_alpha_first
		                  : std_params.options & GB_ALPHA_LAST
		                        ? gs_image_alpha_last
		                        : gs_image_alpha_none);
		...
		if(std_params.data[0] == std_row) {
			...
			native_params.options =
			    (GB_COLORS_NATIVE | GB_PACKING_CHUNKY |
			     GB_RETURN_COPY | GB_OFFSET_0 | GB_RASTER_ALL |
			     GB_ALIGN_STANDARD);
...
out:
	gs_free_object(dev->memory, native_row, "dca_fill_rectangle(native)");
...
private
int
dca_copy_mono(gx_device* dev, const byte* data, int dx, int raster,
              gx_bitmap_id id, int x, int y, int w, int h, gx_color_index zero,
              gx_color_index one)
{
	/****** TEMPORARY ******/
	return gx_default_copy_mono(dev, data, dx, raster, id, x, y, w, h, zero,
	                            one);
}

private
int
dca_copy_color(gx_device* dev, const byte* data, int dx, int raster,
...
private
int
dca_copy_alpha(gx_device* dev, const byte* data, int data_x, int raster,
               gx_bitmap_id id, int x, int y, int width, int height,
               gx_color_index color, int depth)
{
	/****** TEMPORARY ******/
	return gx_default_copy_alpha(dev, data, data_x, raster, id, x, y, width,
	                             height, color, depth);
}
                 ...
                 const gs_composite_params_t* pcp)
{
	...
	}
	{
		...
		if(!pdest->alpha) {
			...
			if((mask >> pcp->cop) & 1) {
				/*
				 * The operation could produce non-unity alpha
				 * values, but
				 * the destination can't store them.  Return an
				 * error.
				 */
				return_error(gs_error_rangecheck);
			}
...
#define advance(ptr, bit, bpv) sample_next(ptr, bit, bpv)
			...
			if(dest_alpha_j >= 0) {
				...
				get_value(dest_alpha, daptr, dabit & 7,
				          dest_bpv, dest_max);
#ifdef PREMULTIPLY_TOWARDS_WHITE
			...
			if(source_alpha_j >= 0) {
				...
				if(source_alpha_j == 0)
					...
					advance(saptr, sabit,
					        source_bpv * source_alpha_j);
				get_value(source_alpha, saptr, sabit,
				          source_bpv, source_max);
#ifdef PREMULTIPLY_TOWARDS_WHITE
...
#define set_clamped(r, v)                                                      \
	BEGIN if((r = (v)) > dest_max) r = dest_max;                           \
	END
				...
				if(j == 0) {
					...
					if(constant_colors)
						source_v =
						    pcp->source_values[j - 1];
					else {
						get_value(source_v, sptr, sbit,
						          source_bpv,
						          source_max);
						advance(sptr, sbit, source_bpv);
					}
					get_value(dest_v, dptr, dbit, dest_bpv,
					          dest_max);
#ifdef PREMULTIPLY_TOWARDS_WHITE
				...
				case composite_Clear:
					/*
					 * The NeXT documentation doesn't say
					 * this, but the CLEAR
					 * operation sets not only alpha but
					 * also all the color
					 * values to 0.
					 */
					result = 0;
				...
				case composite_Sout:
					result =
					    nfr(source_v, dest_alpha, dest_max);
					break;
				case composite_Dout:
					result = nfr(dest_v, source_alpha,
					             source_max);
					break;
				case composite_Satop:
					set_clamped(result,
					            fr(source_v, dest_alpha) +
					                nfr(dest_v,
					                    source_alpha,
					                    source_max));
					break;
				...
				case composite_Highlight:
					/*
					 * Bizarre but true: this operation
					 * converts white and
					 * light gray into each other, and
					 * leaves all other values
					 * unchanged.  We only implement it
					 * properly for gray-scale
					 * devices.
					 */
					if(j != 0 &&
					   !((source_v ^ highlight_value) & ~1))
						result = source_v ^ 1;
					else
						result = source_v;
					break;
				case composite_Dissolve:
					/*
					 * In this case, and only this case, we
					 * need to worry about
					 * source and dest having different bpv
					 * values.  For the
					 * moment, we wimp out and do everything
					 * in floating point.
					 */
					result =
					    (uint)(source_v * source_delta +
					           dest_v * dest_delta);
					break;
				default:
					return_error(gs_error_rangecheck);
 ...
 */
			...
			if(dest_alpha_j > 0)
				put_value(result_alpha, dptr, dbit, dest_bpv,
				          dbyte);
#undef get_value
#undef put_value
#undef advance
