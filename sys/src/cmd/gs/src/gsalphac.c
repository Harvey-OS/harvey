				  ...
				  gs_memory_t *mem)
{
	...
	*pcdev = (gx_device *)cdev;
	...
	cdev->color_info.depth =
	    (dev->color_info.num_components == 4 ? 32 /* CMYK, no alpha */ : (dev->color_info.num_components + 1) * 8);
...
{ /* This is where all the real work gets done! */
	...
	for(yi = y; yi < y + h; ++yi) {
		...
		code = (*dev_proc(target, get_bits_rectangle))(target, &rect, &std_params, NULL);
		if(code < 0)
			break;
		/* Do the work. */
		...
		dest.alpha =
		    (std_params.options & GB_ALPHA_FIRST ? gs_image_alpha_first : std_params.options & GB_ALPHA_LAST ? gs_image_alpha_last : gs_image_alpha_none);
		...
		if(std_params.data[0] == std_row) {
			...
			if(code < 0)
				break;
			code = (*dev_proc(target, copy_color))(target, native_row, 0, 0 /* raster is irrelevant */,
		 ...
		 int values_per_pixel, uint num_pixels, const gs_composite_params_t *pcp)
{
     ...
     */
	int source_alpha_j =
	    (psource->alpha == gs_image_alpha_last ? values_per_pixel : psource->alpha == gs_image_alpha_first ? 0 : -1);

	/* dest_alpha_j does the same for the destination. */
	int dest_alpha_j =
	    (pdest->alpha == gs_image_alpha_last ? values_per_pixel : pdest->alpha == gs_image_alpha_first ? 0 : -1);
	 ...
	 */
		...
		if(!pdest->alpha) {
			uint mask =
			    (psource->alpha || source_alpha != source_max ? alpha_out_S_notD : alpha_out_notS_notD);
