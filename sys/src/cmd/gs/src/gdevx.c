...
/* Forward references */
private
int x_copy_image(gx_device_X *xdev, const byte *base, int sourcex,
		     ...
		     gs_get_bits_params_t *params, gs_int_rect **unread)
{
	...
	uint raster =
	    (options &GB_RASTER_SPECIFIED ? params->raster : (params->raster = default_raster));
	...
	params->options =
	    ...
	    (options & GB_ALIGN_ALL) |
	    (options & GB_RASTER_SPECIFIED ? GB_RASTER_SPECIFIED : GB_RASTER_STANDARD);
	if(x0 >= x1 || y0 >= y1)
		return 0;
