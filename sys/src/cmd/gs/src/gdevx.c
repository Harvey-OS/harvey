...
/* Forward references */
private
int x_copy_image(gx_device_X* xdev, const byte* base, int sourcex, int raster,
                 int x, int y, int w, int h);
private
int set_tile(gx_device*, const gx_strip_bitmap*);
private
void free_cp(gx_device*);

/* Screen updating machinery */
private
void update_init(gx_device_X*);
private
void update_do_flush(gx_device_X*);

#define flush_text(xdev)                                                       \
	if(IN_TEXT(xdev))                                                      \
	do_flush_text(xdev)
private
void do_flush_text(gx_device_X*);
...
private
dev_proc_begin_typed_image(x_begin_typed_image);
private
...
#define x_device(this_device, dev_body, max_bitmap)                            \
	const gx_device_X this_device = {                                      \
	    dev_body,                                                          \
	    {/* std_procs */                                                   \
	     x_open, x_get_initial_matrix, x_sync, x_output_page, x_close,     \
	     gdev_x_map_rgb_color, gdev_x_map_color_rgb, x_fill_rectangle,     \
	     NULL,                                       /* tile_rectangle */  \
	     x_copy_mono, x_copy_color, NULL,            /* draw_line */       \
	     NULL,                                       /* get_bits */        \
	     gdev_x_get_params, gdev_x_put_params, NULL, /* map_cmyk_color */  \
	     gdev_x_get_xfont_procs, NULL, /* get_xfont_device */              \
	     NULL,                         /* map_rgb_alpha_color */           \
	     x_get_page_device, NULL,      /* get_alpha_bits */                \
	     NULL,                         /* copy_alpha */                    \
	     NULL,                         /* get_band */                      \
	     NULL,                         /* copy_rop */                      \
	     NULL,                         /* fill_path */                     \
	     ...
	     NULL,                         /* image_data */                    \
	     NULL,                         /* end_image */                     \
	     x_strip_tile_rectangle, NULL, /* strip_copy_rop */                \
	     NULL,                         /* get_clipping_box */              \
	     x_begin_typed_image, x_get_bits_rectangle,                        \
	     NULL, /* map_color_rgb_alpha */                                   \
	     NULL, /* create_compositor */                                     \
	     NULL, /* get_hardware_params */                                   \
	     NULL, /* text_begin */                                            \
	     gdev_x_finish_copydevice},                                        \
	    gx_device_bbox_common_initial(0 /*false*/, 1 /*true*/,             \
	                                  1 /*true*/),                         \
	    0 /*false*/, /* is_buffered */                                     \
	    1 /*true*/,  /* IsPageDevice */                                    \
	    ...
	    {                                                                  \
	     ...
	     0,                                                                \
	     XYBitmap,                                                         \
	     NULL, /* xoffset, format, data */                                 \
	     MSBFirst,                                                         \
	     8, /* byte-order, bitmap-unit */                                  \
	     MSBFirst,                                                         \
	     8,                                                                \
	     1, /* bitmap-bit-order, bitmap-pad, depth */                      \
	     0,                                                                \
	     1, /* bytes_per_line, bits_per_pixel */                           \
	     0,                                                                \
	     0,                                                                \
	     0,    /* red_mask, green_mask, blue_mask */                       \
	     NULL, /* *obdata */                                               \
	     {                                                                 \
	      NULL, /* *(*create_image)() */                                   \
	      NULL, /* (*destroy_image)() */                                   \
	    ...
	    NULL,                                                              \
	    NULL, /* dpy, scr */    /* (connection not initialized) */         \
	    NULL,                   /* vinfo */                                \
	    (Colormap)None,         /* cmap */                                 \
	    (Window)None,           /* win */                                  \
	    NULL,                   /* gc */                                   \
	    ...
	    (Pixmap)0, /* dest */                                              \
	    0L,                                                                \
	    (uint32_t)~0L, /* colors_or, colors_and */                         \
	    {                                                                  \
	     /* cp */                                                          \
	     (Pixmap)0, /* pixmap */                                           \
	     NULL,      /* gc */                                               \
	    ...
	    0,         /* font */                                              \
	    0,                                                                 \
	    0, /* back_color, fore_color */                                    \
	    0,                                                                 \
	    0,   /* background, foreground */                                  \
	    {0}, /* cman */                                                    \
	    0,                                                                 \
	    0,    /* borderColor, borderWidth */                               \
	    NULL, /* geometry */                                               \
	    128,                                                               \
	    5,    /* maxGrayRamp, maxRGBRamp */                                \
	    NULL, /* palette */                                                \
	    NULL,                                                              \
	    NULL,                                                              \
	    NULL, /* regularFonts, symbolFonts, dingbatFonts */                \
	    NULL,                                                              \
	    NULL,                                                              \
	    NULL, /* regular_fonts, symbol_fonts, dingbat_fonts */             \
	    1,                                                                 \
	    1, /* useXFonts, useFontExtensions */                              \
	    1,                                                                 \
	    0, /* useScalableFonts, logXFonts */                               \
	    0.0,                                                               \
	    0.0, /* xResolution, yResolution */                                \
	    1,   /* useBackingPixmap */                                        \
	    1,                                                                 \
	    1, /* useXPutImage, useXSetTile */                                 \
                                                                               \
...
x_device(gs_x11_device,
         std_device_color_stype_body(gx_device_X, 0, "x11", &st_device_X,
                                     ...
                                     FAKE_RES* DEFAULT_HEIGHT_10THS /
                                         10, /* x and y extent (nominal) */
                                     FAKE_RES,
                                     FAKE_RES, /* x and y density (nominal) */
                                     24, 255, 256),
         0)

    x_device(gs_x11alpha_device, std_device_dci_alpha_type_body(
                                     gx_device_X, 0, "x11alpha", &st_device_X,
                                     FAKE_RES* DEFAULT_WIDTH_10THS / 10,
                                     FAKE_RES* DEFAULT_HEIGHT_10THS /
                                         10, /* x and y extent (nominal) */
                                     FAKE_RES,
    ...
    private int alt_put_image(gx_device* dev, Display* dpy, Drawable win, GC gc,
                              XImage* pi, int sx, int sy, int dx, int dy,
...
private
int
x_fill_rectangle(gx_device* dev, int x, int y, int w, int h,
                 gx_color_index gscolor)
{
	gx_device_X* xdev = (gx_device_X*)dev;
...
private
int
x_copy_mono(gx_device* dev, const byte* base, int sourcex, int raster,
 ...
 */
{
	gx_device_X* xdev = (gx_device_X*)dev;
	int function = GXcopy;
	unsigned long lzero = zero, lone = one;
	x_pixel bc = lzero, fc = lone;
	...
	if(zero != gx_no_color_index) {
		if(one != gx_no_color_index) {
			/* 2-color case. */
			/* Simply replace existing bits with what's in the
			 * image. */
	...
	put_image(xdev->dpy, xdev->dest, xdev->gc, &xdev->image, sourcex, 0, x,
	          y, w, h);
...
hard: /* Handle the hard 1-color case. */
	...
	put_image(xdev->dpy, xdev->cp.pixmap, xdev->cp.gc, &xdev->image,
	          sourcex, 0, 0, 0, w, h);
...
private
int
x_copy_color(gx_device* dev, const byte* base, int sourcex, int raster,
...
private
int
x_strip_tile_rectangle(gx_device* dev, const gx_strip_bitmap* tiles, int x,
                       int y, int w, int h, gx_color_index zero,
                       gx_color_index one, int px, int py)
{
	gx_device_X* xdev = (gx_device_X*)dev;
	...
	if(h <= 2 && w <= 2) {
		...
		for(j = y + h; --j >= y;) {
			const byte* ptr =
			    tiles->data +
			    (j % tiles->rep_height) * tiles->raster;
			int i;
			...
			for(i = x + w; --i >= x;) {
				...
				byte mask = 0x80 >> (tx & 7);
				x_pixel pixel =
				    (ptr[tx >> 3] & mask ? lone : lzero);

				X_SET_FORE_COLOR(xdev, pixel);
				XDrawPoint(xdev->dpy, xdev->dest, xdev->gc, i,
				           j);
			}
		}
		if(xdev->bpixmap != (Pixmap)0) {
	...
	if(!set_tile(
	       dev,
	       tiles)) { /* Bad news.  Fall back to the default algorithm. */
		return gx_default_strip_tile_rectangle(dev, tiles, x, y, w, h,
		                                       zero, one, px, py);
	}
...
private
int
x_begin_typed_image(gx_device* dev, const gs_imager_state* pis,
                    ...
                    gx_image_enum_common_t** pinfo)
{
	...
	if(dev->dname != sdev->dname ||
	   memcmp(&dev->color_info, &sdev->color_info, sizeof(dev->color_info)))
		goto punt;
	flush_text(xdev);
...
#undef eqe
		...
		goto punt;
	{
		...
		if(pcpath != NULL &&
		   !gx_cpath_includes_rectangle(
		       pcpath, float2fixed(dest.p.x), float2fixed(dest.p.y),
                     ...
                     gs_get_bits_params_t* params, gs_int_rect** unread)
{
	...
	if(~options & (GB_RETURN_COPY | GB_OFFSET_0 | GB_PACKING_CHUNKY |
	               GB_COLORS_NATIVE) ||
	   !(options & GB_ALIGN_ALL) || !(options & GB_RASTER_ALL))
		return gx_default_get_bits_rectangle(dev, prect, params,
		                                     unread);
	params->options = GB_COLORS_NATIVE | GB_ALPHA_NONE | GB_PACKING_CHUNKY |
 ...
 ******/
	...
	for(y = y0; y < y1; y += h) {
		...
		image = XGetImage(xdev->dpy, xdev->dest, x0, y, x1 - x0, h,
		                  plane_mask, ZPixmap);
		for(cy = y; cy < y + h; ++cy) {
			const byte* source = (const byte*)image->data +
			                     (cy - y) * image->bytes_per_line;
			byte* dest = params->data[0] + (cy - y0) * raster;

			/*
			 * XGetImage can return an image with any bit order,
			 * byte order,
			 * unit size (for bitmaps), and bits_per_pixel it wants:
			 * it's up
			 * to us to convert the results.  (It's a major botch in
			 * the X
			 * design that even though the server has to have the
			 * ability to
			 * convert images from any format to any format, there's
			 * no way
			...
			if(image->bits_per_pixel == image->depth &&
			   ...
			   (image->byte_order == MSBFirst ||
			    image->depth <= 8)) {
				/*
				 * The server has been nice enough to return an
				 * image in the
				 * format we use.
				 */
				...
				/*
				 * We need to swap byte order and/or bit order.
				 * What a
				 * totally unnecessary nuisance!  For the
				 * moment, the only
				 * cases we deal with are 16- and 24-bit images
				 * with padding
	...
	if(unread) {
		...
		*unread = 0;
		...
		if(event.type == GraphicsExpose) {
			gs_int_rect* rects = (gs_int_rect*)gs_alloc_bytes(
			    dev->memory, sizeof(gs_int_rect),
...
set_tile(gx_device* dev, const gx_strip_bitmap* tile)
{
	...
	if(tile->size.x != xdev->ht.width || tile->size.y != xdev->ht.height ||
	   xdev->ht.pixmap == (Pixmap)0) {
		if(xdev->ht.pixmap != (Pixmap)0)
			XFreePixmap(xdev->dpy, xdev->ht.pixmap);
		xdev->ht.pixmap =
		    XCreatePixmap(xdev->dpy, xdev->win, tile->size.x,
		                  tile->size.y, xdev->vinfo->depth);
		if(xdev->ht.pixmap == (Pixmap)0)
			return 0;
	...
	if(gs_debug['H']) {
		...
		dlprintf4("[H] 0x%lx: width=%d height=%d raster=%d\n",
		          (ulong)tile->data, tile->size.x, tile->size.y,
		          tile->raster);
		dlputs("");
	...
	X_SET_FUNCTION(xdev, GXcopy);
	put_image(xdev->dpy, xdev->ht.pixmap, xdev->gc, &xdev->image, 0, 0, 0,
	          0, tile->size.x, tile->size.y);
	...
	if(xdev->update.count != 0) {
		...
		if(w > 0 && h > 0) {
			if(xdev->is_buffered) {
				...
				/*
				 * The bbox device may have set the target to
				 * NULL
				 * temporarily.  If this is the case, defer the
				 * screen
				 * update.
				 */
				if(mdev == NULL)
					return; /* don't reset */
				x_copy_image(xdev, mdev->line_ptrs[y], x,
				             mdev->raster, x, y, w, h);
			}
			if(xdev->bpixmap) {
				...
				X_SET_FUNCTION(xdev, GXcopy);
				XCopyArea(xdev->dpy, xdev->bpixmap, xdev->win,
				          xdev->gc, x, y, w, h, x, y);
			}
		}
		update_init(xdev);
...
x_update_add(gx_device_X* xdev, int xo, int yo, int w, int h)
{
	...
	if(!xdev->AlwaysUpdate && xdev->update.count < xdev->MaxBufferedCount &&
	   xdev->update.area < xdev->MaxBufferedArea &&
	   xdev->update.total < xdev->MaxBufferedTotal) {
		/*
		 * Test whether adding this rectangle would result in too much
		 * being
		 * copied unnecessarily.  The fraction of new_up_area used in
		 * the
		 * following test is not particularly critical; using a
		 * denominator
	...
	if(xdev->is_buffered && (xdev->target == NULL))
		xdev->update.box =
		    u; /* update deferred since bbox has target disabled */
	else {
		update_do_flush(xdev);
...
#ifdef DEBUG
		if(gcv.foreground != ~(x_pixel)0 &&
		   gcv.background != ~(x_pixel)0) {
		...
		while(xi < w) {
			if((*dp ^ invert) & mask) {
				...
				if(rp == &rects[NUM_RECTS]) {
					XFillRectangles(dpy, win, gc, rects,
					                NUM_RECTS);
					rp = rects;
				}
