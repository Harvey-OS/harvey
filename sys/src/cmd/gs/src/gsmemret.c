...
private
gs_memory_proc_alloc_bytes(gs_retrying_alloc_bytes_immovable);
private
gs_memory_proc_resize_object(gs_retrying_resize_object);
private
gs_memory_proc_free_object(gs_forward_free_object);
private
gs_memory_proc_stable(gs_retrying_stable);
private
gs_memory_proc_status(gs_forward_status);
private
gs_memory_proc_free_all(gs_forward_free_all);
private
gs_memory_proc_consolidate_free(gs_forward_consolidate_free);

/* Object memory procedures */
private
gs_memory_proc_alloc_bytes(gs_retrying_alloc_bytes);
private
gs_memory_proc_alloc_struct(gs_retrying_alloc_struct);
private
gs_memory_proc_alloc_struct(gs_retrying_alloc_struct_immovable);
private
gs_memory_proc_alloc_byte_array(gs_retrying_alloc_byte_array);
private
gs_memory_proc_alloc_byte_array(gs_retrying_alloc_byte_array_immovable);
private
gs_memory_proc_alloc_struct_array(gs_retrying_alloc_struct_array);
private
gs_memory_proc_alloc_struct_array(gs_retrying_alloc_struct_array_immovable);
private
gs_memory_proc_object_size(gs_forward_object_size);
private
gs_memory_proc_object_type(gs_forward_object_type);
private
gs_memory_proc_alloc_string(gs_retrying_alloc_string);
private
gs_memory_proc_alloc_string(gs_retrying_alloc_string_immovable);
private
gs_memory_proc_resize_string(gs_retrying_resize_string);
private
gs_memory_proc_free_string(gs_forward_free_string);
private
gs_memory_proc_register_root(gs_retrying_register_root);
private
gs_memory_proc_unregister_root(gs_forward_unregister_root);
private
gs_memory_proc_enable_free(gs_forward_enable_free);
private
const gs_memory_procs_t retrying_procs = {
    /* Raw memory procedures */
    gs_retrying_alloc_bytes_immovable, gs_retrying_resize_object,
    gs_forward_free_object, gs_retrying_stable, gs_forward_status,
    gs_forward_free_all, gs_forward_consolidate_free,
    /* Object memory procedures */
    gs_retrying_alloc_bytes, gs_retrying_alloc_struct,
    gs_retrying_alloc_struct_immovable, gs_retrying_alloc_byte_array,
    gs_retrying_alloc_byte_array_immovable, gs_retrying_alloc_struct_array,
    gs_retrying_alloc_struct_array_immovable, gs_forward_object_size,
    gs_forward_object_type, gs_retrying_alloc_string,
    gs_retrying_alloc_string_immovable, gs_retrying_resize_string,
    gs_forward_free_string, gs_retrying_register_root,
    gs_forward_unregister_root, gs_forward_enable_free};
...
int /* -ve error code or 0 */
    gs_memory_retrying_init(gs_memory_retrying_t* rmem, /* allocator to init */
...
{
	RETURN_RETRYING(byte*, target->procs.alloc_bytes(target, size, cname));
}
private
byte*
...
{
	RETURN_RETRYING(
	    byte*, target->procs.alloc_bytes_immovable(target, size, cname));
}
private
void*
...
{
	RETURN_RETRYING(
	    void*, target->procs.alloc_struct_immovable(target, pstype, cname));
}
private
byte*
gs_retrying_alloc_byte_array(gs_memory_t* mem, uint num_elements, uint elt_size,
                             client_name_t cname)
{
	RETURN_RETRYING(byte*, target->procs.alloc_byte_array(
	                           target, num_elements, elt_size, cname));
}
private
byte*
...
{
	RETURN_RETRYING(byte*, target->procs.alloc_byte_array_immovable(
	                           target, num_elements, elt_size, cname));
}
private
void*
...
{
	RETURN_RETRYING(void*, target->procs.alloc_struct_array(
	                           target, num_elements, pstype, cname));
}
private
void*
gs_retrying_alloc_struct_array_immovable(gs_memory_t* mem, uint num_elements,
                                         ...
                                         client_name_t cname)
{
	RETURN_RETRYING(void*, target->procs.alloc_struct_array_immovable(
	                           target, num_elements, pstype, cname));
}
private
void*
gs_retrying_resize_object(gs_memory_t* mem, void* obj, uint new_num_elements,
                          client_name_t cname)
{
	RETURN_RETRYING(void*, target->procs.resize_object(
	                           target, obj, new_num_elements, cname));
}
private
uint
...
{
	RETURN_RETRYING(
	    byte*, target->procs.alloc_string_immovable(target, nbytes, cname));
}
private
byte*
...
{
	RETURN_RETRYING(byte*, target->procs.resize_string(
	                           target, data, old_num, new_num, cname));
}
private
void
...
{
	RETURN_RETRYING(
	    int, target->procs.register_root(target, rp, ptype, up, cname));
}
private
void
...
gs_retrying_stable(gs_memory_t* mem)
{
	if(!mem->stable_memory) {
		...
		if(stable == rmem->target)
			...
			gs_memory_retrying_t* retrying_stable =
			    (gs_memory_retrying_t*)gs_alloc_bytes(
			        stable, sizeof(*rmem), "gs_retrying_stable");

			if(retrying_stable) {
				int code = gs_memory_retrying_init(
				    retrying_stable, stable);

				if(code < 0)
					gs_free_object(stable, retrying_stable,
					               "gs_retrying_stable");
				else
					mem->stable_memory =
