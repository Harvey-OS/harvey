...
#define OP_NUM1(i, f) OP1(E, i, f)
#define OP_MATH1(f) OP1(E, PtCr_int_to_float, f)
#define OP_ANY1(op) OP1(op, op, op)
/* 2-operand operators */
#define OP_NUM2(i, f)                                                          \
	{                                                                      \
		{                                                              \
			E4, E4, E, E, i, PtCr_2nd_int_to_float, E, E,          \
			    PtCr_int_to_float, f                               \
		}                                                              \
	}
...
#define OP_MATH2(f)                                                            \
	{                                                                      \
		{                                                              \
			E4, E4, E, E, PtCr_int2_to_float,                      \
			    PtCr_2nd_int_to_float, E, E, PtCr_int_to_float, f  \
		}                                                              \
	}
#define OP_INT2(i)                                                             \
	{                                                                      \
...
#define OP_REL2(i, f)                                                          \
	{                                                                      \
		{                                                              \
			E4, E, i, E, E, E, E, i, PtCr_2nd_int_to_float, E, E,  \
			    PtCr_int_to_float, f                               \
		}                                                              \
	}
#define OP_ANY2(op)                                                            \
	...
	sw:
		switch(op_defn_table[*p++]
		           .opcode[(vsp[-1].type << 2) + vsp->type]) {
...
#define MAX_SHIFT (ARCH_SIZEOF_INT * 8 - 1)
			if(vsp->value.i < -MAX_SHIFT ||
			   vsp->value.i > MAX_SHIFT)
				vsp[-1].value.i = 0;
#undef MAX_SHIFT
			else if((n = vsp->value.i) < 0)
				vsp[-1].value.i =
				    ((uint)(vsp[-1].value.i)) >> -n;
			else
				vsp[-1].value.i <<= n;
			--vsp;
			continue;
		...
		case PtCr_idiv:
			...
			if((vsp[-1].value.i /= vsp->value.i) == min_int &&
			   vsp->value.i ==
			       -1) /* anomalous boundary case, fail */
		...
		neg_int:
			if(vsp->value.i == min_int)
				store_float(
				    vsp,
				    (floatp)vsp->value.i); /* =self negated */
			else
...
#define DO_REL(rel, m) vsp[-1].value.i = vsp[-1].value.m rel vsp->value.m
...
#undef DO_REL
		...
		case PtCr_roll:
			...
			for(; i > 0; i--) {
				memmove(vsp - n, vsp - (n + 1),
				        n * sizeof(*vsp));
				vsp[-(n + 1)] = vsp[-1];
			}
			for(; i < 0; i++) {
				vsp[-1] = vsp[-(n + 1)];
				memmove(vsp - (n + 1), vsp - n,
				        n * sizeof(*vsp));
			}
			vsp -= 2;
			continue;
...
private
int
fn_PtCr_is_monotonic(const gs_function_t* pfn_common, const float* lower,
	...
	for(p = ops; p < ops + size;)
		...
		default: {                    /* must be < PtCr_NUM_OPS */
			static const char* const op_names[] = {
			    /* Keep this consistent with opcodes in gsfunc4.h!
			       */
			    "abs",      "add", "and",  "atan",  "bitshift", "ce"
			                                                    "il"
			                                                    "in"
			                                                    "g", "cos",  "cvi",
			    "cvr",      "div", "exp",  "floor", "idiv",     "l"
			                                                    "n",      "log",  "mod",
			    "mul",      "neg", "not",  "or",    "round",    "si"
			                                                    "n",     "sqrt", "sub",
			    "truncate", "xor", "eq",   "ge",    "gt",       "l"
			                                                    "e",      "lt",   "ne",
			    "copy",     "dup", "exch", "index", "pop",      "ro"
			                                                    "l"
			                                                    "l"};
            ...
            byte* buf, const byte** ptr)
{
	const gs_function_PtCr_t* const pfn =
	    (const gs_function_PtCr_t*)((const char*)psrc -
	                                offset_of(gs_function_PtCr_t,
	                                          data_source));
	/*
...
{
	gs_function_PtCr_t* psfn = gs_alloc_struct(
	    mem, gs_function_PtCr_t, &st_function_PtCr, "fn_PtCr_make_scaled");
	...
	memcpy(ops, pfn->params.ops.data,
	       pfn->params.ops.size - 1); /* minus return */
	...
	for(i = n; --i >= 0;) {
		...
		if(factor != 1) {
			p[0] = PtCr_float;
			memcpy(p + 1, &factor, sizeof(float));
			p += 1 + sizeof(float);
			*p++ = PtCr_mul;
		}
		if(base != 0) {
			p[0] = PtCr_float;
			memcpy(p + 1, &base, sizeof(float));
			p += 1 + sizeof(float);
			*p++ = PtCr_add;
		}
		if(n != 1) {
	...
	*p++ = PtCr_return;
	...
	psfn->params.ops.data = gs_resize_string(
	    mem, ops, opsize, psfn->params.ops.size, "fn_PtCr_make_scaled");
	*ppsfn = psfn;
	return 0;
}
...
void
gs_function_PtCr_free_params(gs_function_PtCr_params_t* params,
                             gs_memory_t* mem)
{
...
{
	uint n;
	const gs_function_PtCr_params_t* p =
	    (const gs_function_PtCr_params_t*)&pfn->params;
                      ...
                      const gs_function_PtCr_params_t* params, gs_memory_t* mem)
{
	static const gs_function_head_t function_PtCr_head = {
	    function_type_PostScript_Calculator,
	    {
	     ...
	     (fn_get_info_proc_t)fn_PtCr_get_info, fn_common_get_params,
