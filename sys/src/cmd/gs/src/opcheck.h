...
/* Copyright (C) 1993, 1995, 1997, 1998 Aladdin Enterprises.  All rights
  reserved.
...
#define check_type_only(rf, typ)                                               \
	BEGIN if(!r_has_type(&rf, typ)) return_error(e_typecheck);             \
	END
#define check_stype_only(rf, styp)                                             \
	BEGIN if(!r_has_stype(&rf, imemory, styp)) return_error(e_typecheck);  \
	END
/* Check for array */
#define check_array_else(rf, errstat)                                          \
	BEGIN if(!r_has_type(&rf, t_array)) errstat;                           \
	END
#define check_array_only(rf) check_array_else(rf, return_error(e_typecheck))
...
#define check_proc(rf)                                                         \
	BEGIN if(!r_is_proc(&rf)) return_error(check_proc_failed(&rf));        \
	END
#define check_proc_only(rf) check_proc(rf)

/* Check for read, write, or execute access. */
#define check_access(rf, acc1)                                                 \
	BEGIN if(!r_has_attr(&rf, acc1)) return_error(e_invalidaccess);        \
	END
...
#define check_type_access_only(rf, typ, acc1)                                  \
	...
	if(!r_has_type_attrs(&rf, typ, acc1))                                  \
		return_error(                                                  \
		    (!r_has_type(&rf, typ) ? e_typecheck : e_invalidaccess));  \
	END
#define check_read_type_only(rf, typ) check_type_access_only(rf, typ, a_read)
#define check_write_type_only(rf, typ) check_type_access_only(rf, typ, a_write)

/* Check for an integer value within an unsigned bound. */
#define check_int_leu(orf, u)                                                  \
	BEGIN                                                                  \
	check_type(orf, t_integer);                                            \
	if((ulong)(orf).value.intval > (u))                                    \
		return_error(e_rangecheck);                                    \
	END
#define check_int_leu_only(rf, u)                                              \
	BEGIN                                                                  \
	check_type_only(rf, t_integer);                                        \
	if((ulong)(rf).value.intval > (u))                                     \
		return_error(e_rangecheck);                                    \
	END
