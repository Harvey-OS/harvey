...
/* Copyright (C) 1993, 1995, 1997, 1998, 1999 Aladdin Enterprises.  All rights
  reserved.
...
/* Forward references */
private
int z2copy_gstate(i_ctx_t*);
private
int push_callout(i_ctx_t*, const char*);
...
zcallendpage(i_ctx_t* i_ctx_p)
{
	...
	check_type(*op, t_integer);
	if((dev = (*dev_proc(dev, get_page_device))(dev)) != 0) {
		code = (*dev->page_procs.end_page)(dev, (int)op->value.intval,
		                                   igs);
		if(code < 0)
			return code;
		if(code > 1)
...
{
	return (r_has_type(&gs_int_gstate(pgs)->pagedevice, t_null) &&
	        (*dev_proc(gs_currentdevice(pgs), get_page_device))(
	            gs_currentdevice(pgs)) != 0);
}
...
restore_page_device(const gs_state* pgs_old, const gs_state* pgs_new)
{
	...
	gx_device* dev_t2;
	bool samepagedevice =
	    obj_eq(dev_old->memory, &gs_int_gstate(pgs_old)->pagedevice,
	...
	if(dev_old != dev_new) {
		if((dev_t2 = (*dev_proc(dev_new, get_page_device))(dev_new)) ==
		   0)
			return false;
		if(dev_t1 != dev_t2)
			return true;
