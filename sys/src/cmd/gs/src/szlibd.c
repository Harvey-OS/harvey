                ...
                stream_cursor_write* pw, bool ignore_last)
{
	...
	const byte* p = pr->ptr;
	int status;
	static const unsigned char jaws_empty[] = {0x58, 0x85, 1, 0, 0,
	                                           0,    0,    0, 1, 0x0A};
	...
	if(zs->total_in == 0 && zs->avail_in >= 10 &&
	   !memcmp(zs->next_in, jaws_empty, 10)) {
		/* JAWS PDF generator encodes empty stream as jaws_empty[].
		 * The stream declares that the data block length is zero
		 * but zlib routines regard a zero length data block to be an
		 * error.
		 */
		pr->ptr += 10;
		return EOFC;
