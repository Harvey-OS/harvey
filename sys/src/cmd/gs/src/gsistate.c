...
case 0:
return ENUM_OBJ((plp->dash.pattern_size == 0 ? NULL : plp->dash.pattern));
ENUM_PTRS_END
private
RELOC_PTRS_WITH(line_params_reloc_ptrs, gx_line_params* plp)
{
...
ENUM_SUPER(gs_imager_state, st_line_params, line_params,
           st_imager_state_num_ptrs - st_line_params_num_ptrs);
...
#undef E1
    ENUM_PTRS_END private RELOC_PTRS_BEGIN(imager_state_reloc_ptrs)
{
...
gs_imager_state_initialize(gs_imager_state* pis, gs_memory_t* mem)
{
	...
	rc_alloc_struct_n(pis->set_transfer.gray, gx_transfer_map,
	...
	pis->set_transfer.red = pis->set_transfer.green =
...
{
	gs_imager_state* pis_copy = gs_alloc_struct(
	    mem, gs_imager_state, &st_imager_state, "gs_imager_state_copy");
...
#define RCDECR(element) rc_decrement(pis->element, cname)
