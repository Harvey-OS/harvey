...
/* Forward references */
private
int common_to(i_ctx_t*, int (*)(gs_state*, floatp, floatp));
private
int common_curve(i_ctx_t*, int (*)(gs_state*, floatp, floatp, floatp, floatp,
                                   floatp, floatp));
...
private
int
common_to(i_ctx_t* i_ctx_p, int (*add_proc)(gs_state*, floatp, floatp))
{
	os_ptr op = osp;
	double opxy[2];
	int code;
...
private
int
common_curve(i_ctx_t* i_ctx_p, int (*add_proc)(gs_state*, floatp, floatp,
                                               floatp, floatp, floatp, floatp))
{
	...
	if((code = num_params(op, 6, opxy)) < 0)
		return code;
	code = (*add_proc)(igs, opxy[0], opxy[1], opxy[2], opxy[3], opxy[4],
	                   opxy[5]);
	if(code >= 0)
		pop(6);
	return code;
...
const op_def zpath_op_defs[] = {{"0clip", zclip},
