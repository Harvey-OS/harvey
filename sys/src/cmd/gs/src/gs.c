...
/* Copyright (C) 1989, 1995, 1996, 1999 Aladdin Enterprises.  All rights
  reserved.
...
#ifdef RUN_STRINGS
private
const int8_t* run_strings[] = {
    "2 vmreclaim /SAVE save def 2 vmreclaim", "(saved\n) print flush",
...
main(int argc, char* argv[])
{
	...
	{ /* Run a list of strings (for testing). */
		...
		for(; *pstr; ++pstr) {
			...
			fprintf(stdout, "{%s} =>\n", *pstr);
			fflush(stdout);
			code = gs_main_run_string(minst, *pstr, 0, &exit_code,
			                          &error_object);
			zflush(osp);
