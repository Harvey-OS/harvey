...
gx_device_macos gs_macos_device = {
    std_device_color_body(gx_device_macos, &gs_mac_procs, DEV_MAC_NAME,
...
mac_open(register gx_device* dev)
{
	...
	static int16_t picHeader[42] = {
	    0x0000,                         // picture size
	    0x0000, 0x0000, 0x0318, 0x0264, // bounding rect at 72dpi
	    0x0011, 0x02ff, 0x0c00, 0xfffe,
	    0x0000,                         // version/header opcodes
	...
	((int16_t*)(*mdev->pic))[16] = ((int16_t*)(*mdev->pic))[35] =
	    ((int16_t*)(*mdev->pic))[40] = mdev->height;
	((int16_t*)(*mdev->pic))[17] = ((int16_t*)(*mdev->pic))[36] =
	    ((int16_t*)(*mdev->pic))[41] = mdev->width;

	((int16_t*)(*mdev->pic))[10] =
	    (((int32_t)X2Fix(mdev->x_pixels_per_inch)) & 0xFFFF0000) >> 16;
	((int16_t*)(*mdev->pic))[11] =
	    ((int32_t)X2Fix(mdev->x_pixels_per_inch)) & 0x0000FFFF;
	((int16_t*)(*mdev->pic))[12] =
	    (((int32_t)X2Fix(mdev->y_pixels_per_inch)) & 0xFFFF0000) >> 16;
	((int16_t*)(*mdev->pic))[13] =
	    ((int32_t)X2Fix(mdev->y_pixels_per_inch)) & 0x0000FFFF;
...
mac_save_pict(gx_device* dev)
{
	...
	if(mdev->outputFile == NULL) {
		code = gx_device_open_output_file(
		    dev, mdev->outputFileName, true, true, &(mdev->outputFile));
		if(code < 0)
			return code;
	}

	for(i = 0; i < 512; i++)
		fputc(0, mdev->outputFile);
	fwrite(*(mdev->pic), sizeof(char),
	       ((int32_t)mdev->currPicPos - (int32_t)*mdev->pic + 2),
	       mdev->outputFile);

	gx_device_close_output_file(dev, mdev->outputFileName,
	                            mdev->outputFile);
...
mac_close(register gx_device* dev)
{
	...
	HUnlock(
	    (Handle)mdev->pic); // no more changes in the pict -> unlock handle
	if(strcmp(mdev->outputFileName, "")) {
		...
		SetHandleSize((Handle)mdev->pic,
		              len + 10); // +10 just for the case
	}
...
private
int
mac_fill_rectangle(register gx_device* dev, int x, int y, int w, int h,
                   gx_color_index color)
{
	gx_device_macos* mdev = (gx_device_macos*)dev;

	/* ignore a fullpage rect directly after an output_page, this would
	 * clear the pict */
	if(mdev->outputPage && (x == 0) && (y == 0) && (w == mdev->width) &&
	   (h == mdev->height)) {
		return 0;
	}
...
private
int
mac_draw_line(register gx_device* dev, int x0, int y0, int x1, int y1,
              gx_color_index color)
{
	gx_device_macos* mdev = (gx_device_macos*)dev;
...
private
int
mac_copy_mono(register gx_device* dev, const unsigned char* base, int data_x,
              ...
              gx_color_index color_0, gx_color_index color_1)
{
	...
	if(color_0 == gx_no_color_index)
		...
		copyMode = notSrcBic; // this mode is untested ! (no file found
		                      // which is using it)
	else
		copyMode = srcCopy;
...
/* alpha value is simulated by changed the value to white. Full transparency
 * means white color */
               ...
               gx_color_index color, int depth)
{
	...
	int16_t copyMode, shade, maxShade = (1 << depth) - 1,
	                         byteCount = raster * h;
	...
	for(shade = 0; shade <= maxShade; shade++) {
		...
		colHSV.v =
		    value + ((1.0 - value) * (float)shade / (float)maxShade);
	...
	PICTWritePixMap(mdev->currPicPos, 0, 0, raster * 8 / depth, h, raster,
	                0, 0, X2Fix(mdev->x_pixels_per_inch),
	                X2Fix(mdev->y_pixels_per_inch), depth);
...
#define NORMALIZE_RGB(col) ((float)col / (float)0xFFFF)
	...
	float min = 1.0, temp;
	float r = NORMALIZE_RGB(inRGB->red), g = NORMALIZE_RGB(inRGB->green),
	...
	if(HSV->s > 0) {
		float rd = (HSV->v - r) / temp, gd = (HSV->v - g) / temp,
		      bd = (HSV->v - b) / temp;
...
mac_convert_hsv_rgb(colorHSV* inHSV, colorRGB* RGB)
{
	if(inHSV->s == 0) {
		...
		unsigned short t1 = (inHSV->v * (1 - inHSV->s)) * 0xFFFF,
		               t2 =
		                   (inHSV->v * (1 - inHSV->s * fract)) * 0xFFFF,
		               t3 = (inHSV->v * (1 - inHSV->s * (1 - fract))) *
		                    0xFFFF,
		               v = inHSV->v * 0xFFFF;
...
mac_set_colordepth(gx_device* dev, int depth)
{
	...
	*/
	case 24: // 24Bit-Color
		ci->num_components = 3;
		ci->max_gray = 255;
		ci->max_color = 16777215;
		ci->dither_grays = 256;
		ci->dither_colors = 16777216;
		set_dev_proc(dev, map_rgb_color, gx_default_rgb_map_rgb_color);
		set_dev_proc(dev, map_color_rgb, gx_default_rgb_map_color_rgb);
		break;
	}
...
mac_put_params(gx_device* dev, gs_param_list* plist)
{
	...
	if(!code) {
		...
		if(code)
			param_return_error(plist, "BitsPerPixel",
			                   gs_error_rangecheck);
	}
	...
	if(code < 0) {
		...
		if(dev->LockSafetyParams &&
		   bytes_compare(outputFile.data, outputFile.size,
		                 (const byte*)mdev->outputFileName,
		                 strlen(mdev->outputFileName))) {
			param_signal_error(plist, "OutputFile",
		   ...
		   bytes_compare(outputFile.data, outputFile.size,
			...
			if(mdev->outputFile != NULL) {
				gx_device_close_output_file(
				    dev, mdev->outputFileName,
				    mdev->outputFile);
				memcpy(mdev->outputFileName, outputFile.data,
				       outputFile.size);
				mdev->outputFileName[outputFile.size] = 0;
				gx_device_open_output_file(
				    dev, mdev->outputFileName, true, true,
				    &(mdev->outputFile));
			} else {
				memcpy(mdev->outputFileName, outputFile.data,
				       outputFile.size);
...
mac_get_params(gx_device* dev, gs_param_list* plist)
{
	...
	// color depth
	code =
	    param_write_int(plist, "BitsPerPixel", &(mdev->color_info.depth));
