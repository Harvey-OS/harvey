...
/* Copyright (C) 1989, 1996, 1997, 1998, 1999, 2001 Aladdin Enterprises.  All
  rights reserved.
...
gs_main_alloc_instance(gs_memory_t* mem)
{
	...
	if(mem) {
		minst = (gs_main_instance*)gs_alloc_bytes_immovable(
		    mem, sizeof(gs_main_instance), "init_main_instance");
		memcpy(minst, &gs_main_instance_init_values,
		       sizeof(gs_main_instance_init_values));
		minst->heap = mem;
...
private
int gs_run_init_file(gs_main_instance*, int*, ref*);
private
void print_resource_usage(const gs_main_instance*, gs_dual_memory_t*,
                          const char*);
...
/* plmain settings remain in effect */
	...
	paths = (ref*)gs_alloc_byte_array(minst->heap, max_lib_paths,
	                                  sizeof(ref), "lib_path array");
	if(paths == 0) {
		gs_lib_finit(1, e_VMerror, minst->heap);
		return_error(e_VMerror);
	}
	make_array(&minst->lib_path.container, avm_foreign, max_lib_paths,
	           (ref*)gs_alloc_byte_array(minst->heap, max_lib_paths,
	                                     sizeof(ref), "lib_path array"));
...
gs_main_init1(gs_main_instance* minst)
{
	if(minst->init_done < 1) {
		...
		int code =
		    ialloc_init(&idmem, minst->heap, minst->memory_chunk_size,
		                gs_have_level2());
		...
		alloc_save_init(&idmem);
		{
			...
			code = gs_register_struct_root(
			    mem, NULL, (void**)&mem->gs_lib_ctx->gs_name_table,
			    "the_gs_name_table");
			if(code < 0)
				return code;
		}
...
init2_make_string_array(i_ctx_t* i_ctx_p, const ref* srefs, const char* aname)
{
	...
	for(; ifp->value.bytes != 0; ifp++)
		;
	make_tasv(&ifa, t_array, a_readonly | avm_foreign, ifp - srefs,
	          const_refs, srefs);
                  ...
                  int* pexit_code, ref* perror_object)
{
	...
	code = gs_interpret(&minst->i_ctx_p, pref, user_errors, pexit_code,
	                    perror_object);
	while((code == e_NeedStdin) || (code == e_NeedStdout) ||
		...
		if(code == e_NeedStdout) {
			...
			if(r_type(&esp[0]) == t_string) {
				const char* str =
				    (const char*)(esp[0].value.const_bytes);
			 ...
			 *  osp[0] = file
			 */
			gs_push_string(minst, (byte*)minst->stdin_buf, count,
			               false);
			gs_push_integer(minst, 0); /* push integer */
		...
		/*
		 * To resume the interpreter, we call gs_interpret with a null
		 * ref.
		...
		esp += 1;
		*esp = refpop;
		code = gs_interpret(&minst->i_ctx_p, &refnul, user_errors,
...
gs_main_init2(gs_main_instance* minst)
{
	...
	if(minst->init_done < 2) {
		...
		{
			/*
			 * gs_iodev_init has to be called here (late), rather
			 * than
			 * with the rest of the library init procedures, because
			 * of
		...
		init2_make_string_array(i_ctx_p, gs_init_file_array,
		                        "INITFILES");
		/* Set up the array of emulator names. */
		init2_make_string_array(i_ctx_p, gs_emulator_name_array,
		                        "EMULATORS");
		 ...
		 * both minst->display and  display_set_callback() are going
		 * away
		*/
		if(minst->display)
...
gs_main_add_lib_path(gs_main_instance* minst, const char* lpath)
{
	...
	r_set_size(&minst->lib_path.list,
	           minst->lib_path.count + first_is_here);
...
gs_main_set_lib_paths(gs_main_instance* minst)
{
	...
	int first_is_here = (r_size(&minst->lib_path.list) != 0 &&
	                             paths[0].value.bytes ==
	                                 (const byte*)gp_current_directory_name
	                         ? 1
	                         : 0);
                 ...
                 int user_errors, int* pexit_code, ref* perror_object)
{
	...
	if(code < 0)
		return code;
	return gs_main_interpret(minst, &initial_file, user_errors, pexit_code,
	                         perror_object);
}
int
...
gs_run_init_file(gs_main_instance* minst, int* pexit_code, ref* perror_object)
{
	...
	code = scan_token(i_ctx_p, ifile.value.pfile, &first_token, &state);
	...
	*++osp = first_token;
	...
	return gs_main_interpret(minst, &ifile, minst->user_errors, pexit_code,
	                         perror_object);
}
...
gs_main_run_string_with_length(gs_main_instance* minst, const char* str,
                               uint length, int user_errors, int* pexit_code,
                               ref* perror_object)
{
	int code;
	...
	if(code != e_NeedInput)
		return code;
	return gs_main_run_string_end(minst, user_errors, pexit_code,
	                              perror_object);
}
...
private
int8_t*
gs_main_tempnames(gs_main_instance* minst)
{
	i_ctx_t* i_ctx_p = minst->i_ctx_p;
	ref* SAFETY;
	ref* tempfiles;
	...
	if(minst->init_done >= 2) {
		...
		while((idict = dict_next(tempfiles, idict, &keyval[0])) >= 0) {
			if(obj_string_data(minst->heap, &keyval[0], &data,
			                   &size) >= 0)
				len += size + 1;
		}
		if(len != 0)
		...
		if(tempnames) {
			...
			idict = dict_first(tempfiles);
			i = 0;
			while((idict = dict_next(tempfiles, idict,
			                         &keyval[0])) >= 0) {
				if(obj_string_data(minst->heap, &keyval[0],
				                   &data, &size) >= 0) {
					memcpy(tempnames + i, (const char*)data,
					       size);
					i += size;
					tempnames[i++] = '\0';
				}
...
gs_main_finit(gs_main_instance* minst, int exit_status, int code)
{
	...
	if(minst->init_done >= 1) {
		...
		if(idmemory->reclaim != 0) {
			...
			if(code < 0) {
				eprintf1("ERROR %d reclaiming the memory while "
			...
			i_ctx_p =
			    minst
			        ->i_ctx_p; /* interp_reclaim could change it. */
		}
		if(i_ctx_p->pgs != NULL && i_ctx_p->pgs->device != NULL) {
			gx_device* pdev = i_ctx_p->pgs->device;

			/* deactivate the device just before we close it for the
			 * last time */
			gs_main_run_string(minst, ".uninstallpagedevice "
			                          "serverdict /.jobsavelevel "
			                          "get 0 eq {/quit} {/stop} "
			                          "ifelse .systemvar exec",
			                   0, &exit_code, &error_object);
			code = gs_closedevice(pdev);
			if(code < 0)
				eprintf2("ERROR %d closing the device. See "
				         "gs/src/ierrors.h for code "
				         "explanation.\n",
                     ...
                     const int8_t* msg)
{
	...
	gp_get_usertime(utime);
	{
		...
		for(i = 0; i < countof(dmem->spaces_indexed); ++i) {
			...
			if(mem != 0 &&
			   (i == 0 || mem != dmem->spaces_indexed[i - 1])) {
				gs_memory_status_t status;
				gs_ref_memory_t* mem_stable =
				    (gs_ref_memory_t*)gs_memory_stable(
				        (gs_memory_t*)mem);
