...
private
int pdf_char_widths(gx_device_pdf *const pdev,
		  ...
		  pdf_font_resource_t **ppdfont)
{
	...
	for(i = 0; i < pstr->size; ++i) {
		...
		if(font->FontType != ft_user_defined) {
			...
			code = (pdfont->base_font != 0 ? pdf_base_font_copy_glyph(pdfont->base_font, glyph, (gs_font_base *)font) : pdf_font_used_glyph(pdfont->FontDescriptor, glyph, (gs_font_base *)font));
			   ...
			   gs_rect *text_bbox, gs_point *pdpt)
{
	int i;
	int space_char =
	    (pte->text.operation & TEXT_ADD_TO_SPACE_WIDTH ? pte->text.space.s_char : -1);
 ...
 */
private
int process_text_return_width(const pdf_text_enum_t *pte,
			      gs_font_base *font,
			      pdf_text_process_state_t *ppts,
			      const gs_const_string *pstr,
