...
private
int pdf_char_widths(gx_device_pdf* const pdev, pdf_font_resource_t* pdfont,
                    int ch, gs_font_base* font,
                    pdf_glyph_widths_t* pwidths /* may be NULL */);
private
int pdf_encode_string(gx_device_pdf* pdev, pdf_text_enum_t* penum,
                          ...
                          pdf_text_process_state_t* ppts)
{
	...
	code = pdf_encode_string(pdev, penum, pstr, gdata,
	                         &pdfont); /* Must not change penum. */
	if(code < 0)
		return code;
 ...
 */
int
pdf_add_ToUnicode(gx_device_pdf* pdev, gs_font* font,
                  pdf_font_resource_t* pdfont, gs_glyph glyph, gs_char ch)
{
	int code;
	gs_char unicode;
	...
	if(unicode != GS_NO_CHAR) {
		...
		num_codes = pfcid->cidata.common.CIDCount;
				...
				/* Since PScript5.dll creates GlyphNames2Unicode
				   ...
				   instead CIDs, and with the WinCharSetFFFF-H2
				   CMap
				   character codes appears from the range
				   0-0xFFFF (Bug 687954),
				   we must use the maximal character code value
				   for the ToUnicode
				   code count. */
				num_codes = 65536;
#endif
			...
			code = gs_cmap_ToUnicode_alloc(
			    pdev->pdf_memory, pdfont->rid, num_codes, key_size,
			    &pdfont->cmap_ToUnicode);
			if(code < 0)
				return code;
		}
		if(pdfont->cmap_ToUnicode != NULL)
			gs_cmap_ToUnicode_add_pair(pdfont->cmap_ToUnicode, ch,
			                           unicode);
	}
	return 0;
}
 ...
 */
int
pdf_register_charproc_resource(gx_device_pdf* pdev, gs_id id,
                               pdf_resource_type_t type)
{
	if(pdev->font3 != 0) {
		pdf_font_resource_t* pdfont = (pdf_font_resource_t*)pdev->font3;
		pdf_resource_ref_t* used_resources =
		    pdfont->u.simple.s.type3.used_resources;
		int i, used_resources_count =
		           pdfont->u.simple.s.type3.used_resources_count;
		int used_resources_max =
		    pdfont->u.simple.s.type3.used_resources_max;

		for(i = 0; i < used_resources_count; i++)
			if(used_resources[i].id == id &&
			   used_resources[i].type == type)
				return 0;
		if(used_resources_count >= used_resources_max) {
			used_resources_max += 10;
			used_resources = (pdf_resource_ref_t*)gs_alloc_bytes(
			    pdev->pdf_memory,
			...
			if(used_resources_count) {
				memcpy(used_resources,
				       pdfont->u.simple.s.type3.used_resources,
				       sizeof(pdf_resource_ref_t) *
...
pdf_used_charproc_resources(gx_device_pdf* pdev, pdf_font_resource_t* pdfont)
{
	...
	if(pdfont->FontType == ft_user_defined) {
		...
		for(i = 0; i < used_resources_count; i++) {
			pdf_resource_t* pres = pdf_find_resource_by_resource_id(
			    pdev, used_resources[i].type, used_resources[i].id);
			if(pres == NULL)
				return_error(
                  ...
                  pdf_font_resource_t** ppdfont)
{
	 ...
	 * we never re-encode texts due to possible encoding conflict while font
	 * merging.
	 */
	code = pdf_obtain_font_resource(penum, pstr, &pdfont);
	if(code < 0)
		return code;
	code = pdf_add_resource(pdev, pdev->substream_Resources, "/Font",
	                        (pdf_resource_t*)pdfont);
	if(code < 0)
		return code;
	code = pdf_register_charproc_resource(
	    pdev, pdf_resource_id((pdf_resource_t*)pdfont), resourceFont);
	if(code < 0)
		return code;
	...
	for(i = 0; i < pstr->size; ++i) {
		...
		if(font->FontType != ft_user_defined) {
			...
			code =
			    (pdfont->base_font != 0
			         ? pdf_base_font_copy_glyph(pdfont->base_font,
			                                    glyph,
			                                    (gs_font_base*)font)
			         : pdf_font_used_glyph(pdfont->FontDescriptor,
			                               glyph,
			                               (gs_font_base*)font));
			...
			if(code == gs_error_undefined) {
				...
				if(bytes_compare(gnstr.data, gnstr.size,
				                 (const byte*)".notdef", 7)) {
					pet->glyph = glyph;
					pet->str = gnstr;
					pet->is_difference = true;
			          ...
			          (gs_copy_glyph_options(
			               font, glyph, (gs_font*)ccfont,
			               COPY_GLYPH_NO_NEW) != 1 ||
			           gs_copied_font_add_encoding(
			               (gs_font*)ccfont, ch, glyph) < 0)) {
				/*
				 * The "complete" copy of the font appears
				 *incomplete
				 * due to incrementally added glyphs. Drop the
				 *"complete"
				 * copy now and continue with subset font only.
				 *
				 * Note that we need to add the glyph to the
				 *encoding of the
				 * "complete" font, because "PPI-ProPag 2.6.1.4
				 *(archivePg)"
				 * creates multiple font copies with reduced
				 *encodings
				 * (we believe it is poorly designed),
				 * and we can merge the copies back to a single
				 *font (see Bug 686875).
				 * We also check whether the encoding is
				 *compatible.
				 * It must be compatible here due to the
				 *pdf_obtain_font_resource
				 * and ccfont logics, but we want to ensure for
				 *safety reason.
				 */
				ccfont = NULL;
				pdf_font_descriptor_drop_complete_font(
				    pdfont->FontDescriptor);
			}
			/*
			 * We arbitrarily allow the first encoded character in a
			 * given
			 * position to determine the encoding associated with
			 * the copied
			 * font.
			 */
			copied_glyph = cfont->procs.encode_char(
			    (gs_font*)cfont, ch, GLYPH_SPACE_NAME);
			if(glyph != copied_glyph &&
			   gs_copied_font_add_encoding((gs_font*)cfont, ch,
			                               glyph) < 0)
				pet->is_difference = true;
		 ...
		 * we can't detemine in advance, which glyphs the font actually
		 * uses.
		 * The decision about writing it out is deferred until
		 * pdf_write_font_resource.
		 */
...
process_text_estimate_bbox(pdf_text_enum_t* pte, gs_font_base* font,
                           const gs_const_string* pstr, const gs_matrix* pfmat,
                           gs_rect* text_bbox, gs_point* pdpt)
{
	int i;
	int space_char = (pte->text.operation & TEXT_ADD_TO_SPACE_WIDTH
	...
	for(i = 0; i < pstr->size; ++i) {
		...
		int code =
		    font->procs.glyph_info((gs_font*)font, glyph, NULL,
		                           GLYPH_INFO_WIDTH0 << WMode, &info);

		if(code < 0)
			return code;
		gs_point_transform(font->FontBBox.p.x, font->FontBBox.p.y, &m,
		                   &p0);
		gs_point_transform(font->FontBBox.p.x, font->FontBBox.q.y, &m,
		                   &p1);
		gs_point_transform(font->FontBBox.q.x, font->FontBBox.p.y, &m,
		                   &p2);
		gs_point_transform(font->FontBBox.q.x, font->FontBBox.q.y, &m,
		                   &p3);
		...
		if(pte->text.operation & TEXT_REPLACE_WIDTHS) {
			...
			gs_distance_transform(tpt.x, tpt.y, &ctm_only(pte->pis),
			                      &wanted);
		} else {
			gs_distance_transform(info.width[WMode].x,
			                      info.width[WMode].y, &m, &wanted);
 ...
 */
private
int process_text_return_width(const pdf_text_enum_t* pte, gs_font_base* font,
                              pdf_text_process_state_t* ppts,
                   ...
                   pdf_text_process_state_t* ppts)
{
	...
	if(penum->pis->text_rendering_mode != 3 &&
	   !(text->operation & TEXT_DO_NONE)) {
		 ...
		 */
		code = process_text_estimate_bbox(penum, font,
		                                  (gs_const_string*)pstr, pfmat,
		                                  &text_bbox, &width_pt);
		if(code == 0) {
	...
	if(code > 0) {
		...
		if(code & TEXT_ADD_TO_SPACE_WIDTH) {
			if(!memchr(pstr->data, penum->text.space.s_char,
			           pstr->size))
	...
	if(mask == 0) {
		...
		if(code == 0) {
			...
			if(text->operation & TEXT_DO_DRAW ||
			   penum->pis->text_rendering_mode == 3) {
				code = pdf_append_chars(pdev, pstr->data,
				                        accepted, width_pt.x,
				                        width_pt.y, false);
				if(code < 0)
					return code;
				adjust_first_last_char(pdfont, pstr->data,
				                       accepted);
	...
	if(mask) {
		/* process_text_modify_width destroys text parameters, save them
		 * now. */
		int index0 = penum->index, xy_index = penum->xy_index;
		gs_text_params_t text = penum->text;
		int xy_index_step =
		    (penum->text.x_widths !=
		                 NULL && /* see gs_text_replaced_width */
		             penum->text.x_widths == penum->text.y_widths
		         ? 2
		         : 1);
...
finish:
	...
	if(text->operation & TEXT_DO_NONE) {
		...
		gs_distance_transform_inverse(width_pt.x, width_pt.y,
		                              &ctm_only(penum->pis), &p);
 ...
 * and pdf_font_cache_elem::real_widths).  Return 1 if the widths were not
 * cached.
 */
private
int
pdf_char_widths(gx_device_pdf* const pdev, pdf_font_resource_t* pdfont, int ch,
                gs_font_base* font,
                pdf_glyph_widths_t* pwidths /* may be NULL */)
{
	...
	code = pdf_attached_font_resource(pdev, (gs_font*)font, &pdfont1,
	                                  ...
	                                  &char_cache_size, &width_cache_size);
	if(code < 0)
		return code;
	if(pdfont1 != pdfont)
	...
	if(font->FontType != ft_user_defined && real_widths[ch] == 0) {
		...
		code = pdf_glyph_widths(pdfont, font->WMode, glyph,
		                        (gs_font*)font, pwidths, NULL);
		if(code < 0)
			return code;
		if(font->WMode != 0 && code > 0 && !pwidths->replaced_v) {
			 ...
			 */
			code = pdf_glyph_widths(pdfont, 0, glyph,
			                        (gs_font*)font, pwidths, NULL);
		}
		if(pwidths->replaced_v) {
			pdfont->u.simple.v[ch].x =
			    pwidths->real_width.v.x - pwidths->Width.v.x;
			pdfont->u.simple.v[ch].y =
			    pwidths->real_width.v.y - pwidths->Width.v.y;
		} else
		...
		if(font->FontType == ft_user_defined) {
			if(!(pdfont->used[ch >> 3] & 0x80 >> (ch & 7)))
				return gs_error_undefined; /* The charproc was
				                              not accumulated.
				                              */
			if(!pdev->charproc_just_accumulated &&
			   !(pdfont->u.simple.s.type3.cached[ch >> 3] &
			     0x80 >> (ch & 7))) {
				/* The charproc uses setcharwidth.
				   Need to accumulate again to check for a glyph
				   variation. */
                          ...
                          int* accepted)
{
	...
	code = pdf_attached_font_resource(pdev, (gs_font*)font, &pdfont, NULL,
	                                  NULL, NULL, NULL);
	if(code < 0)
		return code;
	if(font->FontType == ft_user_defined) {
	...
	for(i = 0, w.x = w.y = 0; i < pstr->size; ++i) {
		...
		if(font->FontType == ft_user_defined &&
		   ...
		   !(pdfont->u.simple.s.type3.cached[ch >> 3] &
			...
			code =
			    pdf_char_widths((gx_device_pdf*)pte->dev,
	...
	*accepted = i;
	gs_distance_transform(w.x * scale, w.y * scale, &ppts->values.matrix,
	                      &dpt);
	if(pte->text.operation & TEXT_ADD_TO_ALL_WIDTHS) {
                          ...
                          const gs_const_string* pstr, gs_point* pdpt)
{
	...
	for(;;) {
		...
		code =
		    pte1.orig_font->procs.next_char_glyph(&pte1, &chr, &glyph);
		if(code == 2) { /* end of string */
			gs_text_enum_copy_dynamic((gs_text_enum_t*)pte, &pte1,
			                          true);
			break;
		}
		if(code < 0)
		...
		if(composite) { /* from process_cmap_text */
			gs_font* subfont =
			    pte1.fstack.items[pte1.fstack.depth].font;
			pdf_font_resource_t* pdsubf =
			    ppts->values.pdfont->u.type0.DescendantFont;

			FontType = pdsubf->FontType;
			code = pdf_glyph_widths(
			    pdsubf, font->WMode, glyph, subfont, &cw,
			    pte->cdevproc_callout ? pte->cdevproc_result
			                          : NULL);
		} else { /* must be a base font */
			FontType = font->FontType;
			if(chr == GS_NO_CHAR && glyph != GS_NO_GLYPH) {
				/* glyphshow, we have no char code. Bug
				 * 686988.*/
				code = pdf_glyph_widths(ppts->values.pdfont,
				                        font->WMode, glyph,
				                        font, &cw, NULL);
				use_cached_v = false; /* Since we have no chr
				                         and don't call
				                         pdf_char_widths. */
			} else
				code =
				    pdf_char_widths((gx_device_pdf*)pte->dev,
				                    ppts->values.pdfont, chr,
				                    (gs_font_base*)font, &cw);
		}
		if(code < 0) {
			if(index > 0)
				break;
		...
		if(font->WMode) {
			/* With WMode 1 v-vector is (WMode 1 origin) - (WMode 0
			   origin).
...
/* pdf_glyph_origin is not longer used. */
		...
		if((pte->text.operation & TEXT_FROM_SINGLE_GLYPH) ||
		   (pte->text.operation & TEXT_FROM_GLYPHS)) {
			...
			pdf_glyph_origin(ppts->values.pdfont, chr, font->WMode,
			                 &v);
#endif
		if(v.x != 0 || v.y != 0) {
			...
			if(FontType == ft_TrueType ||
			   FontType == ft_CID_TrueType)
				scale0 = (float)0.001;
			else
				scale0 = 1;
#if RIGHT_SBW
			...
			if(composite) {
				...
				gs_distance_transform(
				    glyph_origin_shift.x, glyph_origin_shift.y,
			...
			if(glyph_origin_shift.x != 0 ||
			   glyph_origin_shift.y != 0) {
				ppts->values.matrix.tx =
				    start.x + total.x + glyph_origin_shift.x;
				ppts->values.matrix.ty =
				    start.y + total.y + glyph_origin_shift.y;
				code = pdf_set_text_state_values(pdev,
				                                 &ppts->values);
				if(code < 0)
					break;
			}
		...
		if(pte->text.operation & TEXT_DO_DRAW) {
			...
			if(chr == space_char) {
				gs_distance_transform(
				    (font->WMode ? 0
				                 : ppts->values.word_spacing),
				    (font->WMode ? ppts->values.word_spacing
				                 : 0),
				    &ppts->values.matrix, &tpt);
				did.x += tpt.x;
				did.y += tpt.y;
			}
			code = pdf_append_chars(pdev, pstr->data + index,
			                        pte->index - index, did.x,
			                        did.y, composite);
			if(code < 0)
				break;
		} else
		...
		if(pte->text.operation & TEXT_REPLACE_WIDTHS) {
			...
			code = gs_text_replaced_width(&pte->text,
			                              pte->xy_index++, &dpt);
			if(code < 0)
				return_error(gs_error_unregistered);
			gs_distance_transform(dpt.x, dpt.y, &ctm_only(pte->pis),
			                      &wanted);
		} else {
 ...
 */
int
pdf_encode_glyph(gs_font_base* bfont, gs_glyph glyph0, byte* buf, int buf_size,
...
process_plain_text(gs_text_enum_t* pte, void* vbuf, uint bsize)
{
	...
	if(operation & (TEXT_FROM_STRING | TEXT_FROM_BYTES)) {
		...
		if(bsize < count)
			return_error(
			    gs_error_unregistered); /* Must not happen. */
		memcpy(buf, (const byte*)pte->text.data.bytes + pte->index,
		       count);
	...
	if(count > 1 && (operation & TEXT_INTERVENE)) {
		...
		str.size = 1;
		code = pdf_encode_process_string(penum, &str, gdata, NULL,
		                                 &text_state);
		if(code >= 0) {
