                  ...
                  client_name_t cname)
{
	...
	if(nvals > 0) {
		pimap->values = (float*)gs_alloc_byte_array(
		    pmem, nvals, sizeof(float), cname);
...
gs_private_st_composite(st_color_space_Indexed, gs_paint_color_space,
                        "gs_color_space_Indexed", cs_Indexed_enum_ptrs,
...
private
cs_proc_base_space(gx_base_space_Indexed);
private
cs_proc_restrict_color(gx_restrict_Indexed);
private
cs_proc_concrete_space(gx_concrete_space_Indexed);
private
cs_proc_concretize_color(gx_concretize_Indexed);
private
cs_proc_install_cspace(gx_install_Indexed);
private
cs_proc_set_overprint(gx_set_overprint_Indexed);
private
cs_proc_adjust_cspace_count(gx_adjust_cspace_Indexed);
private
cs_proc_serialize(gx_serialize_Indexed);
const gs_color_space_type gs_color_space_type_Indexed = {
    gs_color_space_index_Indexed, false, false, &st_color_space_Indexed,
    gx_num_components_1, gx_base_space_Indexed, gx_init_paint_1,
    gx_restrict_Indexed, gx_concrete_space_Indexed, gx_concretize_Indexed, NULL,
    gx_default_remap_color, gx_install_Indexed, gx_set_overprint_Indexed,
    gx_adjust_cspace_Indexed, gx_no_adjust_color_count, gx_serialize_Indexed,
...
ENUM_PTRS_END
private
RELOC_PTRS_WITH(cs_Indexed_reloc_ptrs, gs_color_space* pcs)
{
...
private
gs_indexed_map*
alloc_indexed_palette(const gs_color_space* pbase_cspace, int nvals,
                      gs_memory_t* pmem)
{
	int num_comps = gs_color_space_num_components(pbase_cspace);
	gs_indexed_map* pimap;
	int code = alloc_indexed_map(&pimap, nvals * num_comps, pmem,
 ...
 */
int
gs_cspace_build_Indexed(gs_color_space** ppcspace,
                        const gs_color_space* pbase_cspace, uint num_entries,
                        const gs_const_string* ptbl, gs_memory_t* pmem)
{
 ...
 */
int
gs_cspace_indexed_set_proc(gs_color_space* pcspace,
...
gx_restrict_Indexed(gs_client_color* pcc, const gs_color_space* pcs)
{
	...
	pcc->paint.values[0] =
	    (is_fneg(value) ? 0 : value >= pcs->params.indexed.hival
...
private
const gs_color_space*
gx_concrete_space_Indexed(const gs_color_space* pcs, const gs_imager_state* pis)
{
                      ...
                      frac* pconc, const gs_imager_state* pis)
{
	...
	int index = (is_fneg(value) ? 0 : value >= pcs->params.indexed.hival
	                                      ? pcs->params.indexed.hival
	                                      : (int)value);
	const gs_color_space* pbcs =
	    (const gs_color_space*)&pcs->params.indexed.base_space;
...
gx_serialize_Indexed(const gs_color_space* pcs, stream* s)
{
	...
	if(p->use_proc) {
		...
		code = sputs(s, (const byte*)&p->lookup.map->values[0],
		             sizeof(p->lookup.map->values[0]) *
		                 p->lookup.map->num_values,
		             &n);
	} else {
 ...
 * color space into the output. Returns 'rangecheck' if the device can't handle
 * it.
 * The primary reason is to include DefaultGray, DefaultRGB, DefaultCMYK into
 * PDF.
...
{
	return (*dev_proc(pgs->device, include_color_space))(
