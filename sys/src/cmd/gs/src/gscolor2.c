...
indexed_table_size(const gs_color_space *pcs)
{
	return (pcs->params.indexed.hival + 1) *
	       cs_num_components((const gs_color_space *)&pcs->params.indexed.base_space);
}
private
...
{
	return (*pcs->params.indexed.base_space.type->install_cspace)((const gs_color_space *)&pcs->params.indexed.base_space, pgs);
}
...
{
	return (*pcs->params.indexed.base_space.type->set_overprint)((const gs_color_space *)&pcs->params.indexed.base_space, pgs);
}
...
gx_adjust_cspace_Indexed(const gs_color_space *pcs, int delta)
{
	...
	(*pcs->params.indexed.base_space.type->adjust_cspace_count)((const gs_color_space *)&pcs->params.indexed.base_space, delta);
}
...
gx_restrict_Indexed(gs_client_color *pcc, const gs_color_space *pcs)
{
	...
	pcc->paint.values[0] =
	    (is_fneg(value) ? 0 : value >= pcs->params.indexed.hival ? pcs->params.indexed.hival : value);
}
		      ...
		      frac *pconc, const gs_imager_state *pis)
{
	...
	int index =
	    (is_fneg(value) ? 0 : value >= pcs->params.indexed.hival ? pcs->params.indexed.hival : (int)value);
	const gs_color_space *pbcs =
	    (const gs_color_space *)&pcs->params.indexed.base_space;
	gs_client_color cc;
...
{
	if(pip->use_proc) {
		return pip->lookup.map->proc.lookup_index(pip, index, &pcc->paint.values[0]);
	} else {
