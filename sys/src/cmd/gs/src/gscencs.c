...
#endif
	pstr->data = (const byte*)&gs_c_known_encoding_chars
	    [gs_c_known_encoding_offsets[len] + off];
	pstr->size = len;
	return 0;
}
...
gs_c_name_glyph(const byte* str, uint len)
{
	...
	/* Binary search the character table. */
	{
		...
		uint count =
		    (gs_c_known_encoding_offsets[len + 1] - base) / len;
...
main()
{
	...
	c = gs_c_decode(I_notdefined + gs_c_min_std_encoding_glyph,
	                1); /* undef'd */
