...
private
dev_proc_print_page(perm_print_page);
private
dev_proc_get_params(perm_get_params);
private
dev_proc_put_params(perm_put_params);
private
dev_proc_get_color_mapping_procs(perm_get_color_mapping_procs);
private
dev_proc_get_color_comp_index(perm_get_color_comp_index);
private
dev_proc_encode_color(perm_encode_color);
private
dev_proc_decode_color(perm_decode_color);
...
private
const gx_device_procs perm_procs = {
    gdev_prn_open, NULL, NULL, gdev_prn_output_page, gdev_prn_close, NULL, NULL,
    NULL,                           /* fill_rectangle */
    NULL,                           /* tile_rectangle */
    NULL,                           /* copy_mono */
    NULL,                           /* copy_color */
    ...
    perm_get_color_comp_index, perm_encode_color, /* encode_color */
...
const gx_device_perm_t gs_perm_device = {
    prn_device_body_extended(
        gx_device_perm_t, perm_procs, "permute", DEFAULT_WIDTH_10THS,
        DEFAULT_HEIGHT_10THS, 72, 72, 0, 0, 0, 0,
        GX_DEVICE_COLOR_MAX_COMPONENTS, 4, GX_CINFO_POLARITY_SUBTRACTIVE, 32, 0,
        255, 255, 256, 256, GX_CINFO_SEP_LIN, "DeviceN", perm_print_page),
    NULL, 0, 0, 0};
...
perm_print_page(gx_device_printer* pdev, FILE* pstream)
{
	...
	cooked_line =
	    gs_alloc_bytes(pdev->memory, dev->width * 3, "perm_print_page");
...
private
void
rgb_cs_to_perm_cm_0(gx_device* dev, const gs_imager_state* pis, frac r, frac g,
                    frac b, frac out[])
{
...
private
void
rgb_cs_to_perm_cm_1(gx_device* dev, const gs_imager_state* pis, frac r, frac g,
                    frac b, frac out[])
{
	out[0] = frac_1 - r;
	out[1] = frac_1 - g;
...
private
const gx_cm_color_map_procs perm_cmapping_procs_0 = {
    gray_cs_to_perm_cm_0, rgb_cs_to_perm_cm_0, cmyk_cs_to_perm_cm_0};

private
const gx_cm_color_map_procs perm_cmapping_procs_1 = {
    gray_cs_to_perm_cm_1, rgb_cs_to_perm_cm_1, cmyk_cs_to_perm_cm_1};

private
const gx_cm_color_map_procs* perm_cmapping_procs[] = {&perm_cmapping_procs_0,
...
perm_get_color_mapping_procs(const gx_device* dev)
{
	...
	if(pdev->mode < 0 ||
	   pdev->mode >=
	       sizeof(perm_cmapping_procs) / sizeof(perm_cmapping_procs[0]))
		return NULL;
                          ...
                          int name_size, int component_type)
{
	...
	for(i = 0; i < n_separation_names; i++) {
		...
		if(compare_color_names(pname, name_size, sep_name,
		                       strlen(sep_name)))
			return i;
	}
	return -1;
...
perm_get_params(gx_device* pdev, gs_param_list* plist)
{
	...
	/*
	 * We need to specify the SeparationColorNames if we are permuting the
	 * colors.
	 */
	if(code >= 0 && dev->permute == 1) {
		...
		for(i = 0; i < dev->num_std_colorant_names; i++)
			param_string_from_string(scn[i],
			                         dev->std_colorant_names[i]);
		/*
		 * Place the name array in the plist.  This includes allocating
		 * memory for the name array element and the actual string
		 * array.
		 */
...
private
const char* DeviceCMYKComponents[] = {
    "Cyan", "Magenta", "Yellow", "Black", 0 /* List terminator */
};

private
const char* DeviceCMYComponents[] = {
    "Cyan", "Magenta", "Yellow", 0 /* List terminator */
};

private
const char* DeviceNComponents[] = {
    "Yellow", "Cyan",  "Cyan2", "Magenta",
    "Zero",   "Black", 0 /* List terminator */
};
