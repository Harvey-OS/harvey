...
/* Copyright (C) 1991, 1995 to 1999 Aladdin Enterprises. 2001 ArtifexSoftware
...
void
rgb_cs_to_devn_cm(gx_device* dev, int* map, const gs_imager_state* pis, frac r,
...
void
cmyk_cs_to_devn_cm(gx_device* dev, int* map, frac c, frac m, frac y, frac k,
                   frac out[])
{
	int i = dev->color_info.num_components - 1;
...
{
	static const byte depths[4][8] = {{1, 2, 0, 4, 8, 0, 0, 8},
	                                  {2, 4, 0, 8, 16, 0, 0, 16},
	                                  {4, 8, 0, 16, 16, 0, 0, 24},
	                                  {4, 8, 0, 16, 32, 0, 0, 32}};
                          ...
                          const gs_param_string* pstring)
{
	if(plist) {
		...
		while(*plist) {
			if(compare_color_names(*plist, strlen(*plist),
			                       pstring->data, size)) {
				return true;
			}
			plist++;
                               ...
                               int name_size, int component_type)
{
	...
	if(pcolor) {
		while(*pcolor) {
			if(compare_color_names(pname, name_size, *pcolor,
			                       strlen(*pcolor)))
	...
	/* Check if the component is in the separation names list. */
	{
		...
		for(i = 0; i < num_spot; i++) {
			if(compare_color_names(
			       (const char*)separations->names[i].data,
                          ...
                          int auto_spot_colors)
{
	 ...
	 */
	color_component_number = check_pcm_and_separation_names(
	    dev, pdevn_params, pname, name_size, component_type);
	...
	if(color_component_number >= 0) {
		...
		if(num_order)
			color_component_number =
			    pdevn_params
			        ->separation_order_map[color_component_number];
		else
		    /*
		     * We can have more spot colors than we can image.  We
		     * simply
		     * ignore the component (i.e. treat it the same as we would
		     * treat a component that is not in the separation order
		     * map).
		     * Note:  Most device do not allow more spot colors than we
		     * can
		     * image.  (See the options for auto_spot_color in
		     * gdevdevn.h.)
		     */
	...
	if(pdevn_params->separations.num_separations < max_spot_colors) {
		...
		sep_name = gs_alloc_bytes(dev->memory, name_size,
		                          "devn_get_color_comp_index");
		memcpy(sep_name, pname, name_size);
		separations->names[sep_num].size = name_size;
		separations->names[sep_num].data = sep_name;
		color_component_number =
		    sep_num + pdevn_params->num_std_colorant_names;
...
#define set_param_array(a, d, s) (a.data = d, a.size = s, a.persistent = false);
                ...
                equivalent_cmyk_color_params* pequiv_colors)
{
	...
	if((code = sample_device_crd_get_params(pdev, plist, "CRDDefault")) <
	       0 ||
	   (code = param_write_name_array(plist, "SeparationColorNames",
	                                  &scna)) < 0 ||
	   (code = param_write_name_array(plist, "SeparationOrder", &sona)) <
	       0 ||
                ...
                equivalent_cmyk_color_params* pequiv_colors)
{
	...
	BEGIN_ARRAY_PARAM(param_read_name_array, "SeparationOrder", sona,
	                  sona.size, sone)
	{
		break;
	}
	...
	BEGIN_ARRAY_PARAM(param_read_name_array, "SeparationColorNames", scna,
	                  scna.size, scne)
	{
		break;
	}
	...
	if(pdev->color_info.polarity == GX_CINFO_POLARITY_SUBTRACTIVE) {
		/*
		 * Process the SeparationColorNames.  Remove any names that
		 * already
		...
		if(scna.data != 0) {
			...
			for(i = num_spot = 0; i < num_names; i++) {
				/* Verify that the name is not one of our
				 * process colorants */
				if(!check_process_color_names(pcomp_names,
				                              &scna.data[i])) {
					byte* sep_name;
					int name_size = scna.data[i].size;

					/* We have a new separation */
					sep_name = (byte*)gs_alloc_bytes(
					    pdev->memory, name_size,
					    "devicen_put_params_no_sep_order");
					memcpy(sep_name, scna.data[i].data,
					       name_size);
					pdevn_params->separations
					    .names[num_spot]
					    .size = name_size;
					pdevn_params->separations
					    .names[num_spot]
					    .data = sep_name;
					if(pequiv_colors != NULL) {
						/* Indicate that we need to find
						 * equivalent CMYK color. */
						pequiv_colors->color[num_spot]
						    .color_info_valid = false;
						pequiv_colors
						    ->all_color_info_valid =
						    false;
					}
					num_spot++;
				}
		...
		if(sona.data != 0) {
			...
			for(i = 0; i < num_order; i++) {
				/*
				 * Check if names match either the process color
				 * model or
				 * SeparationColorNames.  If not then error.
				 */
				if((comp_num = check_pcm_and_separation_names(
				        pdev, pdevn_params,
				        (const char*)sona.data[i].data,
				        sona.data[i].size, 0)) < 0) {
					return_error(gs_error_rangecheck);
				}
				pdevn_params->separation_order_map[comp_num] =
				    i;
			}
		}
		/*
		 ...
		 * number of separations.  This allows us to then accept
		 * separation
		 * colors in color spaces even if they we not specified at the
		 * start
		 * of the image file.
		 */
		code = param_read_int(plist, param_name = "MaxSeparations",
		                      &max_sep);
		switch(code) {
		default:
		...
		case 0:
			if(max_sep < 1 ||
			   max_sep > GX_DEVICE_COLOR_MAX_COMPONENTS)
				return_error(gs_error_rangecheck);
			{
				int depth = bpc_to_depth(
				    max_sep, pdevn_params->bitspercomponent);
		...
		/*
		 * The DeviceN device can have zero components if nothing has
		 * been
		...
		if(num_spot_changed ||
		   ...
		   pdevn_params->num_separation_order_names != num_order) {
			...
			pdevn_params->max_separations = max_sep;
			/*
			 * If we have SeparationOrder specified then the number
			 * of
			 * components is given by the number of names in the
			 * list.
			 * Otherwise check if the MaxSeparations parameter has
			 * specified
			 * a value.  If so then use that value, otherwise use
			 * the number
			...
			pdev->color_info.depth =
			    bpc_to_depth(pdev->color_info.num_components,
                        ...
                        equivalent_cmyk_color_params* pequiv_colors)
{
	...
	if(memcmp(&pdev->color_info, &save_info,
	          sizeof(gx_device_color_info)) ||
	   memcmp(pdevn_params, &saved_devn_params, sizeof(gs_devn_params)) ||
	   (pequiv_colors != NULL &&
...
private
dev_proc_open_device(spotcmyk_prn_open);
private
dev_proc_get_params(spotcmyk_get_params);
private
dev_proc_put_params(spotcmyk_put_params);
private
dev_proc_print_page(spotcmyk_print_page);
private
dev_proc_get_color_mapping_procs(get_spotcmyk_color_mapping_procs);
private
dev_proc_get_color_mapping_procs(get_devicen_color_mapping_procs);
private
dev_proc_get_color_comp_index(spotcmyk_get_color_comp_index);
private
dev_proc_encode_color(spotcmyk_encode_color);
private
dev_proc_decode_color(spotcmyk_decode_color);
...
ENUM_PTRS_END
private
RELOC_PTRS_WITH(spotcmyk_device_reloc_ptrs, spotcmyk_device* pdev)
{
	RELOC_PREFIX(st_device_printer);
	{
		int i;

		for(i = 0; i < pdev->devn_params.separations.num_separations;
		    ++i) {
			RELOC_PTR(spotcmyk_device,
			          devn_params.separations.names[i].data);
		}
	}
}
RELOC_PTRS_END
...
gs_private_st_composite_final(st_spotcmyk_device, spotcmyk_device,
                              "spotcmyk_device", spotcmyk_device_enum_ptrs,
                              spotcmyk_device_reloc_ptrs,
...
#define device_procs(get_color_mapping_procs)                                  \
	{                                                                      \
		spotcmyk_prn_open, gx_default_get_initial_matrix,              \
		    NULL,                 /* sync_output */                    \
...
fixed_colorant_name DeviceCMYKComponents[] = {
    "Cyan", "Magenta", "Yellow", "Black", 0 /* List terminator */
};

#define spotcmyk_device_body(procs, dname, ncomp, pol, depth, mg, mc, cn)      \
	std_device_full_body_type_extended(                                    \
	    spotcmyk_device, &procs, dname, &st_spotcmyk_device,               \
	    (int)((int32_t)(DEFAULT_WIDTH_10THS) * (X_DPI) / 10),              \
	    (int)((int32_t)(DEFAULT_HEIGHT_10THS) * (Y_DPI) / 10), X_DPI,      \
	    Y_DPI, GX_DEVICE_COLOR_MAX_COMPONENTS, /* MaxComponents */         \
	    ncomp,                                 /* NumComp */               \
	    pol,                                   /* Polarity */              \
 ...
 */
private
const gx_device_procs spot_cmyk_procs =
    device_procs(get_spotcmyk_color_mapping_procs);

const spotcmyk_device gs_spotcmyk_device = {
    spotcmyk_device_body(spot_cmyk_procs, "spotcmyk", 4,
                         GX_CINFO_POLARITY_SUBTRACTIVE, 4, 1, 1, "DeviceCMYK"),
 ...
 */
private
const gx_device_procs devicen_procs =
    device_procs(get_devicen_color_mapping_procs);

const spotcmyk_device gs_devicen_device = {
    spotcmyk_device_body(devicen_procs, "devicen", 4,
                         GX_CINFO_POLARITY_SUBTRACTIVE, 32, 255, 255,
                         "DeviceCMYK"),
    /* DeviceN device specific parameters */
    {
...
private
void
rgb_cs_to_spotcmyk_cm(gx_device* dev, const gs_imager_state* pis, frac r,
                      frac g, frac b, frac out[])
{
	int* map = ((spotcmyk_device*)dev)->devn_params.separation_order_map;
...
private
void
cmyk_cs_to_spotcmyk_cm(gx_device* dev, frac c, frac m, frac y, frac k,
                       frac out[])
{
	int* map = ((spotcmyk_device*)dev)->devn_params.separation_order_map;
...
private
const gx_cm_color_map_procs spotCMYK_procs = {
...
{
	return devn_printer_put_params(
	    pdev, plist, &(((spotcmyk_device*)pdev)->devn_params), NULL);
}
...
private
int
spotcmyk_get_color_comp_index(gx_device* dev, const char* pname, int name_size,
                              int component_type)
{
	return devn_get_color_comp_index(
	    dev, &(((spotcmyk_device*)dev)->devn_params), NULL, pname,
	    name_size, component_type, ENABLE_AUTO_SPOT_COLORS);
}
 ...
 */
int
repack_data(byte* source, byte* dest, int depth, int first_bit, int bit_width,
            int npixel)
{
	...
	if(out_byte_loc != out_bit_start) { /* If partially filled last byte */
		*out =
		    *out&((~0)
		          << out_byte_loc); /* Mask unused part of last byte */
		out++;
	}
	length = out - dest;
	return length;
...
private
int write_pcx_file(gx_device_printer* pdev, char* filename, int ncomp, int bpc,
                   int pcmlinelength);
/*
 * This is an example print page routine for a DeviceN device.  This routine
 * will handle a DeviceN, a CMYK with spot colors, or an RGB process color
 *model.
 *
 ...
 * combinations of bits per pixel and number of colorants that are not
 *supported.
 */
private
int
spotcmyk_print_page(gx_device_printer* pdev, FILE* prn_stream)
{
	...
	byte* in =
	    gs_alloc_bytes(pdev->memory, line_size, "spotcmyk_print_page(in)");
	byte* buf = gs_alloc_bytes(pdev->memory, line_size + 3,
	                           "spotcmyk_print_page(buf)");
	...
	int i, code = 0;
	int first_bit;
	int pcmlinelength = 0; /* Initialize against indeterminizm in case of
	                          pdev->height == 0. */
	...
	/*
	 * Check if the SeparationOrder list has changed the order of the
	 * process
	 * color model colorants. If so then we will treat all colorants as if
	 * they
	 * are spot colors.
	 */
	...
	for(; lnum < bottom; ++lnum) {
		...
		if(npcmcolors) {
			...
			pcmlinelength = repack_data(data, buf, depth, first_bit,
			                            bpc * npcmcolors, width);
			fwrite(buf, 1, pcmlinelength, prn_stream);
		}
		/* Put spot color data into the output files */
		for(i = 0; i < nspot; i++) {
			first_bit = bpc * (nspot - 1 - i);
			linelength[i] = repack_data(data, buf, depth, first_bit,
			                            bpc, width);
	...
	if(npcmcolors) {
		code = write_pcx_file(pdev, (char*)&pdevn->fname, npcmcolors,
		                      bpc, pcmlinelength);
		if(code < 0)
			return code;
	}
...
private
const pcx_header pcx_header_prototype = {
    10, /* manuf */
    0,  /* version (variable) */
    1,  /* encoding */
    0,  /* bpp (variable) */
...
private
void pcx_write_rle(const byte*, const byte*, int, FILE*);
private
int pcx_write_page(gx_device_printer* pdev, FILE* infile, int linesize,
                   FILE* outfile, pcx_header* phdr, bool planar, int depth);
...
private
bool
setup_pcx_header(gx_device_printer* pdev, pcx_header* phdr, int num_planes,
                 int bits_per_plane)
{
	...
	*phdr = pcx_header_prototype;
	...
	case 1:
		...
		case 1:
			...
			memcpy((byte*)phdr->palette, "\000\000\000\377\377\377",
			       6);
			planar = false;
			break;
		case 2: /* Not defined */
		...
		case 4:
			phdr->version = version_2_8_with_palette;
			memcpy((byte*)phdr->palette, pcx_ega_palette,
...
private
int
finish_pcx_file(gx_device_printer* pdev, FILE* file, pcx_header* header,
                int num_planes, int bits_per_plane)
{
	switch(num_planes) {
	case 1:
		switch(bits_per_plane) {
		...
		case 8:
			fputc(0x0c, file);
			return pc_write_mono_palette((gx_device*)pdev, 256,
			                             file);
		case 16: /* Not defined */
			break;
		}
...
private
int
write_pcx_file(gx_device_printer* pdev, char* filename, int ncomp, int bpc,
               int linesize)
{
	pcx_header header;
	int code;
...
private
int
pcx_write_page(gx_device_printer* pdev, FILE* infile, int linesize,
               FILE* outfile, pcx_header* phdr, bool planar, int depth)
{
	int raster = linesize;
	uint rsize = ROUND_UP((pdev->width * phdr->bpp + 7) >> 3,
	                      2); /* PCX format requires even */
	...
	assign_ushort(phdr->bpl,
	              (planar || depth == 1 ? rsize : raster + (raster & 1)));
	...
	for(y = 0; y < height; y++) {
		...
		if(!planar) { /* Just write the bits. */
			if(raster &
			   1) { /* Round to even, with predictable padding. */
				*end = end[-1];
				++end;
			}
			...
			case 4: {
				...
				for(shift = 0; shift < 4; shift++) {
					...
					for(from = row, to = plane; from < end;
					    from += 4) {
						*to++ = (from[0] & bleft ? 0x80
						                         : 0) |
						        (from[0] & bright ? 0x40
						                          : 0) |
						        (from[1] & bleft ? 0x20
			...
			case 24: {
				...
				for(pnum = 0; pnum < 3; ++pnum) {
					pcx_write_rle(row + pnum, row + raster,
					              3, outfile);
					if(pdev->width & 1)
						fputc(
						    0,
