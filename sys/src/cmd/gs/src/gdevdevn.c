 ...
 */
private
const gx_device_procs spot_cmyk_procs = device_procs(get_spotcmyk_color_mapping_procs);
 ...
 */
private
const gx_device_procs devicen_procs = device_procs(get_devicen_color_mapping_procs);
...
private
int write_pcx_file(gx_device_printer *pdev, char *filename, int ncomp,
...
private
int pcx_write_page(gx_device_printer *pdev, FILE *infile,
	       ...
	       pcx_header *phdr, bool planar, int depth)
{
	...
	assign_ushort(phdr->bpl, (planar || depth == 1 ? rsize : raster + (raster & 1)));
