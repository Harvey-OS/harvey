...
int filter_read(
    ...
    uint space);
int filter_write(i_ctx_t* i_ctx_p, int npop, const stream_template* template,
 ...
 */
int filter_read_simple(i_ctx_t* i_ctx_p, const stream_template* template);
int filter_write_simple(i_ctx_t* i_ctx_p, const stream_template* template);
...
#define private_st_stream_proc_state() /* in zfproc.c */                       \
	gs_private_st_complex_only(                                            \
	    st_sproc_state, stream_proc_state, "procedure stream state",       \
	    sproc_clear_marks, sproc_enum_ptrs, sproc_reloc_ptrs, 0)
