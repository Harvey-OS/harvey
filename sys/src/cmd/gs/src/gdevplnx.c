...
ENUM_PTRS_END
private
RELOC_PTRS_WITH(device_plane_extract_reloc_ptrs, gx_device_plane_extract* edev)
{
...
/* Driver procedures */
private
dev_proc_open_device(plane_open_device);
private
dev_proc_fill_rectangle(plane_fill_rectangle);
private
dev_proc_copy_mono(plane_copy_mono);
private
dev_proc_copy_color(plane_copy_color);
private
dev_proc_copy_alpha(plane_copy_alpha);
private
dev_proc_fill_path(plane_fill_path);
private
dev_proc_stroke_path(plane_stroke_path);
private
dev_proc_fill_mask(plane_fill_mask);
private
dev_proc_fill_parallelogram(plane_fill_parallelogram);
private
dev_proc_fill_triangle(plane_fill_triangle);
private
dev_proc_strip_tile_rectangle(plane_strip_tile_rectangle);
private
dev_proc_strip_copy_rop(plane_strip_copy_rop);
private
dev_proc_begin_typed_image(plane_begin_typed_image);
private
dev_proc_get_bits_rectangle(plane_get_bits_rectangle);

/* Device prototype */
private
const gx_device_plane_extract gs_plane_extract_device = {
    std_device_std_body(gx_device_plane_extract, 0, "plane_extract", 0, 0, 72,
                        72),
    {plane_open_device, NULL, NULL, NULL, gx_default_close_device, NULL, NULL,
     plane_fill_rectangle, gx_default_tile_rectangle, plane_copy_mono,
     plane_copy_color, gx_default_draw_line, gx_default_get_bits, NULL, NULL,
     NULL, NULL, NULL, NULL, NULL, NULL, plane_copy_alpha, NULL,
     gx_default_copy_rop, plane_fill_path, plane_stroke_path, plane_fill_mask,
     gx_default_fill_trapezoid, plane_fill_parallelogram, plane_fill_triangle,
     gx_default_draw_thin_line, gx_default_begin_image, gx_default_image_data,
     gx_default_end_image, plane_strip_tile_rectangle, plane_strip_copy_rop,
     NULL, plane_begin_typed_image, plane_get_bits_rectangle, NULL,
     gx_no_create_compositor, /* WRONG */
     NULL, gx_default_text_begin},
...
#define TRANS_COLOR_PIXEL(edev, color)                                         \
	((color) == gx_no_color_index ? gx_no_color_index                      \
	                              : COLOR_PIXEL(edev, color))
...
#define REDUCE_PURE(edev, pixel)                                               \
	((pixel) == (edev)->plane_white && !(edev)->any_marks                  \
	     ? REDUCE_SKIP                                                     \
                     ...
                     gs_logical_operation_t* plop)
{
	...
	if(gx_dc_is_pure(pdevc)) {
		gx_color_index pixel =
		    COLOR_PIXEL(edev, gx_dc_pure_color(pdevc));
		...
		*ppdc = *pdevc;
		...
		if(ppdc->colors.colored.c_level[plane] == 0) {
			...
			ppdc->colors.pure =
			    COLOR_PIXEL(edev, ppdc->colors.pure);
	...
	if(*plop & lop_T_transparent) {
		/*
		 * If the logical operation invokes transparency for the
		 * texture, we
		 * must do some extra work, since a color that was originally
		 * opaque
		 * may become transparent (white) if reduced to a single plane.
		 * If
		 * RasterOp transparency were calculated before halftoning, life
		 * would be easy: we would simply turn off texture transparency
		 * in
		 * the logical operation iff the original (not reduced) color
		 * was
		 * not white.  Unfortunately, RasterOp transparency is
		 * calculated
		 * after halftoning.  (This is arguably wrong, but it's how
		 * we've
		 * defined it.)  Therefore, if transparency is involved with a
		 * white color or a halftone that can include white, we must
		 * keep
		...
		/*
		 * Given that we haven't failed, the only possible colors at
		 * this
    ...
    extract_partial_tile(const tiling_state_t* pts)
{
	...
	bits_extract_plane(&dest, &source, edev->plane.shift, pts->size.x,
	                   pts->size.y);
	return 0;
}
             ...
             byte* local_buffer, uint buffer_size, bool partial_ok)
{
	...
	if(full_size <= buffer_size) {
		...
		if(buffer_size >= width_raster) {
			...
			pts->size.x = pts->buffer.raster *
			              (8 / edev->plane_dev->color_info.depth);
			pts->size.y = 1;
		}
	} else {
		pts->buffer.data =
...
plane_device_init(gx_device_plane_extract* edev, gx_device* target,
                  gx_device* plane_dev, const gx_render_plane_t* render_plane,
                  bool clear)
{
	...
	gx_device_init((gx_device*)edev,
	               (const gx_device*)&gs_plane_extract_device, edev->memory,
	               true);
	...
	if(clear) {
		dev_proc(plane_dev,
		         fill_rectangle)(plane_dev, 0, 0, plane_dev->width,
		                         plane_dev->height, edev->plane_white);
...
private
int
plane_fill_rectangle(gx_device* dev, int x, int y, int w, int h,
                     gx_color_index color)
{
	...
	if(pixel != edev->plane_white)
		...
		return 0;
	return dev_proc(plane_dev, fill_rectangle)(plane_dev, x, y, w, h,
	                                           pixel);
}

private
int
plane_copy_mono(gx_device* dev, const byte* data, int data_x, int raster,
                ...
                gx_color_index color0, gx_color_index color1)
{
	...
	if((pixel0 == edev->plane_white || pixel0 == gx_no_color_index) &&
	   (pixel1 == edev->plane_white || pixel1 == gx_no_color_index)) {
		...
		edev->any_marks = true;
	return dev_proc(plane_dev, copy_mono)(plane_dev, data, data_x, raster,
	                                      id, x, y, w, h, pixel0, pixel1);
}

private
int
plane_copy_color(gx_device* dev, const byte* data, int data_x, int raster,
                 gx_bitmap_id id, int x, int y, int w, int h)
{
	gx_device_plane_extract* const edev = (gx_device_plane_extract*)dev;
	gx_device* const plane_dev = edev->plane_dev;
...
private
int
plane_copy_alpha(gx_device* dev, const byte* data, int data_x, int raster,
                 gx_bitmap_id id, int x, int y, int w, int h,
                 gx_color_index color, int depth)
{
	...
	if(pixel != edev->plane_white)
		...
		return 0;
	return dev_proc(plane_dev, copy_alpha)(plane_dev, data, data_x, raster,
	                                       id, x, y, w, h, pixel, depth);
}

private
int
plane_fill_path(gx_device* dev, const gs_imager_state* pis, gx_path* ppath,
                const gx_fill_params* params, const gx_drawing_color* pdevc,
                const gx_clip_path* pcpath)
{
	...
	default /*REDUCE_FAILED*/:
		return gx_default_fill_path(dev, pis, ppath, params, pdevc,
		                            pcpath);
	}
}
...
private
int
plane_stroke_path(gx_device* dev, const gs_imager_state* pis, gx_path* ppath,
                  ...
                  const gx_clip_path* pcpath)
{
	...
	default /*REDUCE_FAILED*/:
		return gx_default_stroke_path(dev, pis, ppath, params, pdevc,
		                              pcpath);
	}
}
...
private
int
plane_fill_mask(gx_device* dev, const byte* data, int data_x, int raster,
                gx_bitmap_id id, int x, int y, int w, int h,
                ...
                gs_logical_operation_t lop, const gx_clip_path* pcpath)
{
	...
	case REDUCE_DRAW:
		return dev_proc(plane_dev, fill_mask)(
		    plane_dev, data, data_x, raster, gx_no_bitmap_id, x, y, w,
		    h, &dcolor, depth, lop, pcpath);
	default /*REDUCE_FAILED*/:
		return gx_default_fill_mask(dev, data, data_x, raster,
		                            gx_no_bitmap_id, x, y, w, h,
...
private
int
plane_fill_parallelogram(gx_device* dev, fixed px, fixed py, fixed ax, fixed ay,
                         ...
                         gs_logical_operation_t lop)
{
	...
	default /*REDUCE_FAILED*/:
		return gx_default_fill_parallelogram(dev, px, py, ax, ay, bx,
		                                     by, pdcolor, lop);
	}
}
...
private
int
plane_fill_triangle(gx_device* dev, fixed px, fixed py, fixed ax, fixed ay,
                    fixed bx, fixed by, const gx_drawing_color* pdcolor,
                    gs_logical_operation_t lop)
{
	gx_device_plane_extract* const edev = (gx_device_plane_extract*)dev;
...
private
int
plane_strip_tile_rectangle(gx_device* dev, const gx_strip_bitmap* tiles, int x,
                           ...
                           gx_color_index color1, int phase_x, int phase_y)
{
	...
	if(pixel0 == pixel1) {
		 ...
		 */
		{
			...
			int code = begin_tiling(&state, edev, tiles->data, 0,
			                        tiles->raster, tiles->size.x,
			                        tiles->size.y, (byte*)buf,
			                        sizeof(buf), false);

			if(code < 0)
				return gx_default_strip_tile_rectangle(
				    dev, tiles, x, y, w, h, color0, color1,
				    phase_x, phase_y);
			extract_partial_tile(&state);
			plane_tile = *tiles;
...
private
int
plane_strip_copy_rop(gx_device* dev, const byte* sdata, int sourcex,
                     ...
                     int phase_x, int phase_y, gs_logical_operation_t lop)
{
	...
	if(lop & (lop_S_transparent | lop_T_transparent))
		return gx_default_strip_copy_rop(
		    dev, sdata, sourcex, sraster, id, scolors, textures,
		    tcolors, x, y, w, h, phase_x, phase_y, lop);
	if(!rop3_uses_S(rop)) {
		sdata = 0;
		source.colors = 0;
	} else if(scolors) {
	...
	if(sdata) {
		code =
		    begin_tiling(&source.state, edev, sdata, sourcex, sraster,
		                 w, y, (byte*)sbuf, sizeof(sbuf), true);
		if(code < 0)
			return gx_default_strip_copy_rop(
			    dev, sdata, sourcex, sraster, id, scolors, textures,
			    tcolors, x, y, w, h, phase_x, phase_y, lop);
	...
	do {
		...
		code = dev_proc(plane_dev, strip_copy_rop)(
		    plane_dev, plane_source, sourcex, plane_raster,
		    gx_no_bitmap_id, source.colors, plane_textures,
		    texture.colors, x, y, w, h, phase_x, phase_y, lop);
...
gs_private_st_suffix_add3(st_plane_image_enum, plane_image_enum_t,
                          ...
                          plane_image_enum_reloc_ptrs, st_gx_image_enum_common,
                          info, pis, pis_image);
...
plane_cmap_gray(frac gray, gx_device_color* pdc,
                const gs_imager_state* pis_image, gx_device* dev,
                gs_color_select_t select)
{
	...
	gx_remap_concrete_gray(gray, &dcolor, ppie->pis, (gx_device*)edev,
	                       select);
               ...
               gs_color_select_t select)
{
	...
	gx_remap_concrete_rgb(r, g, b, &dcolor, ppie->pis, (gx_device*)edev,
	                      select);
	reduce_drawing_color(pdc, edev, &dcolor, &lop);
}
private
                ...
                gs_color_select_t select)
{
	...
	gx_remap_concrete_cmyk(c, m, y, k, &dcolor, ppie->pis, (gx_device*)edev,
	                       select);
	reduce_drawing_color(pdc, edev, &dcolor, &lop);
}
private
...
private
const gx_color_map_procs plane_color_map_procs = {
...
private
image_enum_proc_plane_data(plane_image_plane_data);
private
image_enum_proc_end_image(plane_image_end_image);
private
const gx_image_enum_procs_t plane_image_enum_procs = {plane_image_plane_data,
                                                      plane_image_end_image};

private
int
plane_begin_typed_image(gx_device* dev, const gs_imager_state* pis,
                        ...
                        gx_image_enum_common_t** pinfo)
{
	...
	*pis_image = *pis;
	...
	code = dev_proc(edev->plane_dev, begin_typed_image)(
	    edev->plane_dev, pis_image, pmat, pic, prect, &dcolor, pcpath,
	    memory, &info->info);
	if(code < 0)
		goto fail;
	...
	if((options & GB_PACKING_PLANAR) && (options & GB_SELECT_PLANES)) {
		...
		for(plane = 0; plane < dev->color_info.num_components; ++plane)
			if(plane != plane_index && params->data[plane] != 0)
				return gx_default_get_bits_rectangle(
				    dev, prect, params, unread);
		...
		if(code >= 0) {
			*params = plane_params;
			params->options =
			    (params->options & ~GB_PACKING_ALL) |
			    (GB_PACKING_PLANAR | GB_SELECT_PLANES);
			params->data[plane_index] = params->data[0];
			for(plane = 0; plane < dev->color_info.num_components;
			    ++plane)
	            ...
	            (GB_COLORS_NATIVE | GB_ALPHA_NONE | GB_PACKING_CHUNKY |
	             GB_RETURN_COPY | GB_ALIGN_STANDARD | GB_OFFSET_0 |
	             GB_RASTER_STANDARD))) {
		/* Expand the plane into chunky pixels. */
		bits_plane_t dest, source;

		dest.data.write = params->data[0];
		dest.raster = bitmap_raster((prect->q.x - prect->p.x) *
		...
		plane_params.options = options &=
		    (~(GB_COLORS_ALL | GB_ALPHA_ALL | GB_PACKING_ALL |
		       GB_RETURN_ALL | GB_ALIGN_ALL | GB_OFFSET_ALL |
		       GB_RASTER_ALL) |
		     GB_COLORS_NATIVE | GB_ALPHA_NONE | GB_PACKING_CHUNKY |
		     /* Try for a pointer return the first time. */
		     GB_RETURN_POINTER | GB_ALIGN_STANDARD |
		...
		if(code >= 0) {
			...
			code = bits_expand_plane(
			    &dest, &source, edev->plane.shift,
			    prect->q.x - prect->p.x, prect->q.y - prect->p.y);
		}
		params->options =
		    (options & ~GB_RETURN_POINTER) | GB_RETURN_COPY;
	} else
		return gx_default_get_bits_rectangle(dev, prect, params,
		                                     unread);
	return code;
