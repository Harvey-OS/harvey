...
#define REDUCE_PURE(edev, pixel) \
	((pixel) == (edev)->plane_white && !(edev)->any_marks ? REDUCE_SKIP : ((edev)->any_marks = true, REDUCE_DRAW))
private
reduced_color_t
		  ...
		  gx_device *plane_dev, const gx_render_plane_t *render_plane, bool clear)
{
	...
	if(clear) {
		dev_proc(plane_dev, fill_rectangle)(plane_dev, 0, 0, plane_dev->width, plane_dev->height,
						    edev->plane_white);
		     ...
		     int x, int y, int w, int h, gx_color_index color)
{
	...
	if(pixel != edev->plane_white)
		...
		return 0;
	return dev_proc(plane_dev, fill_rectangle)(plane_dev, x, y, w, h, pixel);
}
		...
		gx_color_index color0, gx_color_index color1)
{
	...
	if((pixel0 == edev->plane_white || pixel0 == gx_no_color_index) &&
	   (pixel1 == edev->plane_white || pixel1 == gx_no_color_index)) {
		...
		edev->any_marks = true;
	return dev_proc(plane_dev, copy_mono)(plane_dev, data, data_x, raster, id, x, y, w, h, pixel0, pixel1);
}
		 ...
		 int x, int y, int w, int h)
{
	...
	do {
		extract_partial_tile(&state);
		code = dev_proc(plane_dev, copy_color)(plane_dev, state.buffer.data, 0, state.buffer.raster,
		 ...
		 gx_color_index color, int depth)
{
	...
	if(pixel != edev->plane_white)
		...
		return 0;
	return dev_proc(plane_dev, copy_alpha)(plane_dev, data, data_x, raster, id, x, y, w, h, pixel, depth);
}
		...
		const gx_drawing_color *pdevc, const gx_clip_path *pcpath)
{
	...
	case REDUCE_DRAW: {
		...
		return dev_proc(plane_dev, fill_path)(plane_dev, pis_draw, ppath, params, &dcolor, pcpath);
	}
		  ...
		  const gx_drawing_color *pdevc, const gx_clip_path *pcpath)
{
	...
	case REDUCE_DRAW: {
		...
		return dev_proc(plane_dev, stroke_path)(plane_dev, pis_draw, ppath, params, &dcolor, pcpath);
	}
	default /*REDUCE_FAILED*/:
		...
		gs_logical_operation_t lop, const gx_clip_path *pcpath)
{
	...
	case REDUCE_SKIP:
		return 0;
	case REDUCE_DRAW:
		return dev_proc(plane_dev, fill_mask)(plane_dev, data, data_x, raster, gx_no_bitmap_id, x, y, w, h,
			 ...
			 const gx_drawing_color *pdcolor, gs_logical_operation_t lop)
{
	...
	case REDUCE_SKIP:
		return 0;
	case REDUCE_DRAW:
		return dev_proc(plane_dev, fill_parallelogram)(plane_dev, px, py, ax, ay, bx, by, &dcolor, lop);
	default /*REDUCE_FAILED*/:
		    ...
		    const gx_drawing_color *pdcolor, gs_logical_operation_t lop)
{
	...
	case REDUCE_SKIP:
		return 0;
	case REDUCE_DRAW:
		return dev_proc(plane_dev, fill_triangle)(plane_dev, px, py, ax, ay, bx, by, &dcolor, lop);
	default /*REDUCE_FAILED*/:
			   ...
			   int phase_x, int phase_y)
{
	...
	if(pixel0 == pixel1) {
		...
		/* We should use the default implementation if it is small.... */
		{
			...
			code = dev_proc(plane_dev, strip_tile_rectangle)(plane_dev, &plane_tile, x, y, w, h, pixel0, pixel1,
									 phase_x, phase_y);
			end_tiling(&state);
			edev->any_marks = true;
			return code;
	...
	if((pixel0 == edev->plane_white || pixel0 == gx_no_color_index) &&
	   (pixel1 == edev->plane_white || pixel1 == gx_no_color_index)) {
		...
		edev->any_marks = true;
	return dev_proc(plane_dev, strip_tile_rectangle)(plane_dev, tiles, x, y, w, h, pixel0, pixel1, phase_x, phase_y);
}
		     ...
		     int phase_x, int phase_y, gs_logical_operation_t lop)
{
	...
	do {
		...
		code = dev_proc(plane_dev, strip_copy_rop)(plane_dev, plane_source, sourcex, plane_raster, gx_no_bitmap_id,
...
private
const gx_image_enum_procs_t plane_image_enum_procs = {
			...
			gs_memory_t *memory, gx_image_enum_common_t **pinfo)
{
	...
	*pis_image = *pis;
	...
	code = dev_proc(edev->plane_dev, begin_typed_image)(edev->plane_dev, pis_image, pmat, pic, prect,
			 ...
			 gs_get_bits_params_t *params, gs_int_rect **unread)
{
	...
	if((options & GB_PACKING_PLANAR) && (options & GB_SELECT_PLANES)) {
		...
		code = dev_proc(plane_dev, get_bits_rectangle)(plane_dev, prect, &plane_params, unread);
		if(code >= 0) {
			*params = plane_params;
