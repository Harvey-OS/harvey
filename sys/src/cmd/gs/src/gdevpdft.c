...
private
int
pdf_make_soft_mask_dict(gx_device_pdf* pdev,
                        const gs_pdf14trans_params_t* pparams)
{
	...
	code = pdf_alloc_resource(pdev, resourceSoftMaskDict, gs_no_id,
	                          &pres_soft_mask_dict, -1);
	if(code < 0)
		return code;
	...
	code = cos_dict_put_c_key_string(
	    soft_mask_dict, "/S",
	    pparams->subtype == TRANSPARENCY_MASK_Alpha ? (byte*)"/Alpha"
	                                                : (byte*)"/Luminosity",
	...
	if(pparams->Background_components) {
		...
		Background = cos_array_from_floats(
		    pdev, pparams->Background, pparams->Background_components,
		    "pdf_write_soft_mask_dict");
		if(Background == NULL)
			return_error(gs_error_VMerror);
		code = cos_dict_put_c_key_object(soft_mask_dict, "/BC",
		                                 (cos_object_t*)Background);
		if(code < 0)
			return code;
	}
	if(pparams->transfer_function != NULL) {
		...
		code =
		    pdf_write_function(pdev, pparams->transfer_function, &id);
		if(code < 0)
			return code;
		sprintf(buf, " %ld 0 R", id);
		code = cos_dict_put_c_key_string(soft_mask_dict, "/TR",
                    ...
                    const gs_imager_state* pis, cos_dict_t** pdict)
{
	...
	code =
	    pdf_alloc_resource(pdev, resourceGroup, gs_no_id, &pres_group, -1);
	if(code < 0)
		return code;
	cos_become(pres_group->object, cos_type_dict);
	group_dict = (cos_dict_t*)pres_group->object;
	code = cos_dict_put_c_key_string(group_dict, "/Type",
	                                 (const byte*)"/Group", 6);
	if(code < 0)
		return code;
	code = cos_dict_put_c_key_string(group_dict, "/S",
	                                 (const byte*)"/Transparency", 13);
	if(code < 0)
		return code;
	if(pparams->Isolated) {
	...
	code = pdf_substitute_resource(pdev, &pres_group, resourceGroup, NULL,
	                               false);
	if(code < 0)
		return code;
...
pdf_make_form_dict(gx_device_pdf* pdev, const gs_pdf14trans_params_t* pparams,
                   const gs_imager_state* pis, const cos_dict_t* group_dict,
                   cos_dict_t* form_dict)
{
	...
	code = cos_dict_put_c_key_string(form_dict, "/Type",
	                                 (const byte*)"/XObject", 8);
	if(code < 0)
		return code;
	code = cos_dict_put_c_key_string(form_dict, "/Subtype",
	                                 (const byte*)"/Form", 5);
	if(code < 0)
		return code;
	...
	if(code < 0)
		return code;
	code = cos_dict_put_c_key_string(form_dict, "/Matrix",
	                                 (const byte*)"[1 0 0 1 0 0]", 13);
	if(code < 0)
		return code;
	bbox_array = cos_array_from_floats(pdev, bbox, 4,
	                                   "pdf_begin_transparency_group");
	if(bbox_array == NULL)
		return_error(gs_error_VMerror);
	code = cos_dict_put_c_key_object(form_dict, "/BBox",
	                                 (cos_object_t*)bbox_array);
	if(code < 0)
		return code;
	return cos_dict_put_c_key_object(form_dict, "/Group",
	                                 (cos_object_t*)group_dict);
}
                             ...
                             const gs_pdf14trans_params_t* pparams)
{
	...
	if(!in_page)
		...
		if(code < 0)
			return code;
		code =
		    pdf_enter_substream(pdev, resourceXObject, gs_no_id, &pres,
		                        false, pdev->params.CompressPages);
		if(code < 0)
			return code;
		return pdf_make_form_dict(pdev, pparams, pis, group_dict,
...
pdf_end_transparency_group(gs_imager_state* pis, gx_device_pdf* pdev)
{
	...
	if(pdev->sbstack_depth == bottom) {
		...
		if(code < 0)
			return code;
		sputc(pdev->strm, '/');
		sputs(pdev->strm, (const byte*)pres->rname, strlen(pres->rname),
		      &ignore);
                            ...
                            const gs_pdf14trans_params_t* pparams)
{
	if(pparams->mask_is_image) {
		/* HACK :
		    ...
		    Later it will be enumerated once again as a part of SMask-ed
		   image,
		    and the pdfwrite image handler will recognize duplicated
		   images
                          ...
                          const gs_pdf14trans_params_t* pparams)
{
	if(pdev->image_mask_skip)
		...
		code = cos_dict_put_c_key_string(
		    (cos_dict_t*)pdev->pres_soft_mask_dict->object, "/G",
		    (const byte*)buf, strlen(buf));
		if(code < 0)
			return code;
		code =
		    pdf_substitute_resource(pdev, &pdev->pres_soft_mask_dict,
...
int
gdev_pdf_create_compositor(gx_device* dev, gx_device** pcdev,
...
int
gdev_pdf_end_transparency_group(gx_device* dev, gs_imager_state* pis,
                                gs_transparency_state_t** ppts)
{
	return 0;
}
...
int
gdev_pdf_end_transparency_mask(gx_device* dev, gs_transparency_mask_t** pptm)
{
	return 0;
}
