...
#define LASERJET_MARGINS_A4                                                    \
	(float)0.167, (float)0.167, (float)0.167, (float)0.167
#define LASERJET_MARGINS_LETTER                                                \
	(float)0.167, (float)0.167, (float)0.167, (float)0.167
...
private
dev_proc_open_device(hpjet_open);
private
dev_proc_close_device(hpjet_close);
private
dev_proc_print_page_copies(djet_print_page_copies);
private
dev_proc_print_page_copies(djet500_print_page_copies);
private
dev_proc_print_page_copies(fs600_print_page_copies);
private
dev_proc_print_page_copies(ljet_print_page_copies);
private
dev_proc_print_page_copies(ljetplus_print_page_copies);
private
dev_proc_print_page_copies(ljet2p_print_page_copies);
private
dev_proc_print_page_copies(ljet3_print_page_copies);
private
dev_proc_print_page_copies(ljet3d_print_page_copies);
private
dev_proc_print_page_copies(ljet4_print_page_copies);
private
dev_proc_print_page_copies(ljet4d_print_page_copies);
private
dev_proc_print_page_copies(lp2563_print_page_copies);
private
dev_proc_print_page_copies(oce9050_print_page_copies);
private
dev_proc_get_params(hpjet_get_params);
private
dev_proc_put_params(hpjet_put_params);

private
const gx_device_procs prn_hp_procs =
...
#define HPJET_DEVICE(procs, dname, w10, h10, xdpi, ydpi, lm, bm, rm, tm,       \
                     color_bits, print_page_copies)                            \
	{                                                                      \
		prn_device_std_margins_body_copies(                            \
		    gx_device_hpjet, procs, dname, w10, h10, xdpi, ydpi, lm,   \
		    tm, lm, bm, rm, tm, color_bits, print_page_copies),        \
		    0, false, false, false                                     \
	}

const gx_device_hpjet gs_deskjet_device = HPJET_DEVICE(
    prn_hp_procs, "deskjet", DEFAULT_WIDTH_10THS, DEFAULT_HEIGHT_10THS, X_DPI,
    Y_DPI, 0, 0, 0, 0, /* margins filled in by hpjet_open */
    1, djet_print_page_copies);

const gx_device_hpjet gs_djet500_device = HPJET_DEVICE(
    prn_hp_procs, "djet500", DEFAULT_WIDTH_10THS, DEFAULT_HEIGHT_10THS, X_DPI,
    Y_DPI, 0, 0, 0, 0, /* margins filled in by hpjet_open */
    1, djet500_print_page_copies);

const gx_device_hpjet gs_fs600_device = HPJET_DEVICE(
    prn_hp_procs, "fs600", DEFAULT_WIDTH_10THS, DEFAULT_HEIGHT_10THS, X_DPI2,
    Y_DPI2, 0.23, 0.0, 0.23, 0.04, /* margins */
    1, fs600_print_page_copies);

const gx_device_hpjet gs_laserjet_device = HPJET_DEVICE(
    prn_hp_procs, "laserjet", DEFAULT_WIDTH_10THS, DEFAULT_HEIGHT_10THS, X_DPI,
    Y_DPI, 0.05, 0.25, 0.55, 0.25, /* margins */
    1, ljet_print_page_copies);

const gx_device_hpjet gs_ljetplus_device = HPJET_DEVICE(
    prn_hp_procs, "ljetplus", DEFAULT_WIDTH_10THS, DEFAULT_HEIGHT_10THS, X_DPI,
    Y_DPI, 0.05, 0.25, 0.55, 0.25, /* margins */
    1, ljetplus_print_page_copies);

const gx_device_hpjet gs_ljet2p_device = HPJET_DEVICE(
    prn_hp_procs, "ljet2p", DEFAULT_WIDTH_10THS, DEFAULT_HEIGHT_10THS, X_DPI,
    Y_DPI, 0.20, 0.25, 0.25, 0.25, /* margins */
    1, ljet2p_print_page_copies);

const gx_device_hpjet gs_ljet3_device = HPJET_DEVICE(
    prn_hp_procs, "ljet3", DEFAULT_WIDTH_10THS, DEFAULT_HEIGHT_10THS, X_DPI,
    Y_DPI, 0.20, 0.25, 0.25, 0.25, /* margins */
    1, ljet3_print_page_copies);

const gx_device_hpjet gs_ljet3d_device = HPJET_DEVICE(
    prn_hp_procs, "ljet3d", DEFAULT_WIDTH_10THS, DEFAULT_HEIGHT_10THS, X_DPI,
    Y_DPI, 0.20, 0.25, 0.25, 0.25, /* margins */
    1, ljet3d_print_page_copies);

const gx_device_hpjet gs_ljet4_device =
    HPJET_DEVICE(prn_hp_procs, "ljet4", DEFAULT_WIDTH_10THS,
                 DEFAULT_HEIGHT_10THS, X_DPI2, Y_DPI2, 0, 0, 0, 0, /* margins */
                 1, ljet4_print_page_copies);

const gx_device_hpjet gs_ljet4d_device =
    HPJET_DEVICE(prn_hp_procs, "ljet4d", DEFAULT_WIDTH_10THS,
                 DEFAULT_HEIGHT_10THS, X_DPI2, Y_DPI2, 0, 0, 0, 0, /* margins */
                 1, ljet4d_print_page_copies);

const gx_device_hpjet gs_lp2563_device =
    HPJET_DEVICE(prn_hp_procs, "lp2563", DEFAULT_WIDTH_10THS,
                 DEFAULT_HEIGHT_10THS, X_DPI, Y_DPI, 0, 0, 0, 0, /* margins */
                 1, lp2563_print_page_copies);

const gx_device_hpjet gs_oce9050_device =
    HPJET_DEVICE(prn_hp_procs, "oce9050", 24 * 10,
                 24 * 10,    /* 24 inch roll (can print 32" also) */
                 400, 400,   /* 400 dpi */
                 0, 0, 0, 0, /* margins */
...
{ /* Change the margins if necessary. */
	...
	if(ppdev->printer_procs.print_page_copies == djet_print_page_copies ||
	   ppdev->printer_procs.print_page_copies ==
	       djet500_print_page_copies) {
		...
		m = (gdev_pcl_paper_size(pdev) == PAPER_SIZE_A4 ? m_a4
		                                                : m_letter);
	} else if(ppdev->printer_procs.print_page_copies ==
	              oce9050_print_page_copies ||
	          ppdev->printer_procs.print_page_copies ==
	              lp2563_print_page_copies)
		;
	else { /* LaserJet */
...
hpjet_make_init(gx_device_printer* pdev, char* buf, const char* str)
{
	...
	if(dev->ManualFeed_set && dev->ManualFeed)
		paper_source = 2;
                       ...
                       int num_copies)
{
	...
	return dljet_mono_print_page_copies(pdev, prn_stream, num_copies, 300,
	                                    PCL_DJ_FEATURES, init);
}
                          ...
                          int num_copies)
{
	...
	code = dljet_mono_print_page_copies(pdev, prn_stream, num_copies, 400,
	                                    PCL_OCE9050_FEATURES, init);
