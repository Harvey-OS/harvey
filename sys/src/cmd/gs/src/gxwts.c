			   ...
			   const gx_rop_source_t *source)
{
	...
	color0 = dev->color_info.separable_and_linear == GX_CINFO_SEP_LIN ? 0 : pdevc->colors.wts.plane_vector[1];
	...
	if(source == NULL && lop_no_S_is_T(lop))
		code = (*dev_proc(dev, copy_mono))(dev, tile_data, 0, tile_raster, gx_no_bitmap_id,
			   ...
			   const gx_rop_source_t *source)
{
	...
	if(source == NULL && lop_no_S_is_T(lop))
		code = (*dev_proc(dev, copy_color))(dev, ctile_data, 0, ctile_raster, gx_no_bitmap_id,
						    x, y, w, h);
		...
		const gx_device_color *pdevc2)
{
	...
	if(pdevc2->type != pdevc1->type ||
	   ...
	   num_comp != pdevc2->colors.wts.num_components)
		return false;
	return !memcmp(pdevc1->colors.wts.levels,
