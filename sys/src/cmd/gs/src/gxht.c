...
#else
	return (gs_debug_c('.') ? max_cached_tiles_SMALL : max_cached_tiles_LARGE);
#endif
}
uint
...
private
int render_ht(gx_ht_tile *, int, const gx_ht_order *,
	      gx_bitmap_id);
private
gx_ht_tile *
		     ...
		     gx_device *dev, gs_color_select_t select)
{
	...
	const gx_ht_order *porder =
	    (component_index < 0 ? &pdevc->colors.binary.b_ht->order : &pdevc->colors.binary.b_ht->components[component_index].corder);
...
gx_ht_init_cache(const gs_memory_t *mem, gx_ht_cache *pcache, const gx_ht_order *porder)
{
	...
	pcache->render_ht =
	    (pcache->num_tiles == 1 ? gx_render_ht_1_tile : pcache->levels_per_tile == 1 ? gx_render_ht_1_level : gx_render_ht_default);
}
