...
private
dev_color_proc_save_dc(gx_dc_ht_binary_save_dc);
private
dev_color_proc_get_dev_halftone(gx_dc_ht_binary_get_dev_halftone);
private
dev_color_proc_load(gx_dc_ht_binary_load);
private
dev_color_proc_fill_rectangle(gx_dc_ht_binary_fill_rectangle);
private
dev_color_proc_fill_masked(gx_dc_ht_binary_fill_masked);
private
dev_color_proc_equal(gx_dc_ht_binary_equal);
private
dev_color_proc_write(gx_dc_ht_binary_write);
private
dev_color_proc_read(gx_dc_ht_binary_read);
const gx_device_color_type_t gx_dc_type_data_ht_binary = {
    &st_dc_ht_binary, gx_dc_ht_binary_save_dc, gx_dc_ht_binary_get_dev_halftone,
    gx_dc_ht_get_phase, gx_dc_ht_binary_load, gx_dc_ht_binary_fill_rectangle,
    gx_dc_ht_binary_fill_masked, gx_dc_ht_binary_equal, gx_dc_ht_binary_write,
    gx_dc_ht_binary_read, gx_dc_ht_binary_get_nonzero_comps};
...
ENUM_PTRS_END
private
RELOC_PTRS_WITH(dc_ht_binary_reloc_ptrs, gx_device_color* cptr)
{
	gx_ht_tile* tile = cptr->colors.binary.b_tile;
...
private_st_ht_tiles();
private
ENUM_PTRS_BEGIN_PROC(ht_tiles_enum_ptrs) { return 0; }
ENUM_PTRS_END_PROC
private
RELOC_PTRS_BEGIN(ht_tiles_reloc_ptrs)
{
...
#else
	return (gs_debug_c('.') ? max_ht_bits_SMALL : max_ht_bits_LARGE);
#endif
}
...
gx_ht_alloc_cache(gs_memory_t* mem, uint max_tiles, uint max_bits)
{
	...
	byte* tbits = gs_alloc_bytes(mem, max_bits, "alloc_ht_cache(bits)");
	gx_ht_tile* ht_tiles =
...
private
int render_ht(gx_ht_tile*, int, const gx_ht_order*, gx_bitmap_id);
private
gx_ht_tile*
gx_render_ht_default(gx_ht_cache* pcache, int b_level)
{
	...
	if(bt->level != level) {
		int code =
		    render_ht(bt, level, porder, pcache->base_id + b_level);
                     ...
                     gx_device* dev, gs_color_select_t select)
{
	...
	const gx_ht_order* porder =
	    (component_index < 0
	         ...
	         : &pdevc->colors.binary.b_ht->components[component_index]
	                .corder);
                               ...
                               const gx_rop_source_t* source)
{
	...
	if(source == NULL && lop_no_S_is_T(lop))
		return (*dev_proc(dev, strip_tile_rectangle))(
		    dev, &pdevc->colors.binary.b_tile->tiles, x, y, w, h,
		    pdevc->colors.binary.color[0],
		    pdevc->colors.binary.color[1], pdevc->phase.x,
		    pdevc->phase.y);
	...
	return (*dev_proc(dev, strip_copy_rop))(
	    dev, source->sdata, source->sourcex, source->sraster, source->id,
	    (source->use_scolors ? source->scolors : NULL),
	    &pdevc->colors.binary.b_tile->tiles, pdevc->colors.binary.color, x,
	    y, w, h, pdevc->phase.x, pdevc->phase.y, lop);
}

private
int
gx_dc_ht_binary_fill_masked(const gx_device_color* pdevc, const byte* data,
                            int data_x, int raster, gx_bitmap_id id, int x,
                            int y, int w, int h, gx_device* dev,
                            gs_logical_operation_t lop, bool invert)
{
	...
	if(code < 0)
		return code;
	return gx_dc_default_fill_masked(pdevc, data, data_x, raster, id, x, y,
	                                 w, h, dev, lop, invert);
}
 ...
 */
private
const int dc_ht_binary_has_color0 = 0x01;
private
const int dc_ht_binary_has_color1 = 0x02;
private
const int dc_ht_binary_has_level = 0x04;
private
const int dc_ht_binary_has_index = 0x08;
...
private
int
gx_dc_ht_binary_write(const gx_device_color* pdevc,
                      ...
                      byte* pdata, uint* psize)
{
	...
	if(psdc == 0 ||
	   pdevc->colors.binary.color[0] != psdc->colors.binary.b_color[0]) {
		...
		tmp_size = 0;
		(void)gx_dc_write_color(pdevc->colors.binary.color[0], dev,
		                        pdata, &tmp_size);
		req_size += tmp_size;
	}
	if(psdc == 0 ||
	   pdevc->colors.binary.color[1] != psdc->colors.binary.b_color[1]) {
		...
		tmp_size = 0;
		(void)gx_dc_write_color(pdevc->colors.binary.color[1], dev,
		                        pdata, &tmp_size);
		req_size += tmp_size;
	}
	...
	*pdata++ = (byte)flag_bits;
	...
	if((flag_bits & dc_ht_binary_has_color0) != 0) {
		...
		code = gx_dc_write_color(pdevc->colors.binary.color[0], dev,
		                         pdata, &tmp_size);
		if(code < 0)
			return code;
		pdata += tmp_size;
	...
	if((flag_bits & dc_ht_binary_has_color1) != 0) {
		tmp_size = req_size - (pdata - pdata0);
		code = gx_dc_write_color(pdevc->colors.binary.color[1], dev,
		                         pdata, &tmp_size);
		if(code < 0)
			return code;
		pdata += tmp_size;
 ...
 */
private
int gx_dc_ht_binary_read(gx_device_color* pdevc, const gs_imager_state* pis,
                         const gx_device_color* prior_devc,
                         const gx_device* dev, /* ignored */
                         const byte* pdata, uint size,
                         gs_memory_t* mem) /* ignored */
{
	...
	if((flag_bits & dc_ht_binary_has_color0) != 0) {
		code = gx_dc_read_color(&devc.colors.binary.color[0], dev,
		                        pdata, size);
		if(code < 0)
			return code;
		size -= code;
	...
	if((flag_bits & dc_ht_binary_has_color1) != 0) {
		code = gx_dc_read_color(&devc.colors.binary.color[1], dev,
		                        pdata, size);
		if(code < 0)
			return code;
		size -= code;
 ...
 */
int
gx_dc_ht_binary_get_nonzero_comps(const gx_device_color* pdevc,
                                  const gx_device* dev,
...
void
gx_ht_init_cache(const gs_memory_t* mem, gx_ht_cache* pcache,
                 const gx_ht_order* porder)
{
	...
	int width_unit =
	    (width <= ht_mask_bits / 2 ? ht_mask_bits / width * width : width);
          ...
          const gx_ht_order* porder, gx_bitmap_id new_id)
{
	...
	if_debug7('H', "[H]Halftone cache slot 0x%lx: old=%d, new=%d, "
	               "w=%d(%d), h=%d(%d):\n",
	          (ulong)data, pbt->level, level, pbt->tiles.size.x,
	          porder->width, pbt->tiles.size.y,
	          porder->num_bits / porder->width);
#ifdef DEBUG
	if(level < 0 || level > porder->num_bits) {
		lprintf3(
		    "Error in render_ht: level=%d, old level=%d, num_bits=%d\n",
	...
	if(pbt->tiles.raster > porder->raster)
		bits_replicate_horizontally(
		    data, pbt->tiles.rep_width, pbt->tiles.rep_height,
		    porder->raster, pbt->tiles.size.x, pbt->tiles.raster);
	...
	if(gs_debug_c('H')) {
		...
		while(p < ptr) {
			dprintf8(" %d%d%d%d%d%d%d%d", *p >> 7, (*p >> 6) & 1,
			         (*p >> 5) & 1, (*p >> 4) & 1, (*p >> 3) & 1,
			         (*p >> 2) & 1, (*p >> 1) & 1, *p & 1);
			if((++p - data) % wb == 0)
				dputc('\n');
