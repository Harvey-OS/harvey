...
gdev_prn_save_page(gx_device_printer* pdev, gx_saved_page* page, int num_copies)
{
		...
		return_error(gs_error_limitcheck);
	{
		...
		if((code = clist_end_page(pcldev)) < 0 ||
		   (code = clist_fclose(pcldev->page_cfile, pcldev->page_cfname,
		                        false)) < 0 ||
		   (code = clist_fclose(pcldev->page_bfile, pcldev->page_bfname,
		                        false)) < 0)
			return code;
...
int
gdev_prn_render_pages(gx_device_printer* pdev, const gx_placed_page* ppages,
                      int count)
{
	gx_device_clist_reader* const pcldev = (gx_device_clist_reader*)pdev;
	...
	/* Check to make sure the pages are compatible with the device. */
	{
		...
		for(i = 0; i < count; ++i) {
			...
			/* We would like to fully check the color
			 * representation, */
			...
			if(page->info.band_params.BandBufferSpace !=
			       pdev->buffer_space ||
			   page->info.band_params.BandWidth != pdev->width)
				return_error(gs_error_rangecheck);
