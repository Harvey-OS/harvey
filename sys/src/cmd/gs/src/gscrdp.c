		...
		if(code >= 0) {
			...
			if(code >= 0 && !pcrd->caches.RenderTableT_is_identity) {
				...
				for(i = 0; i < m; ++i) {
					...
					for(j = 0; j < size; ++j)
						values[i * size + j] =
						    frac2float((*pcrd->RenderTable.T.procs[i])((byte)(j * scale), pcrd));
				}
				fa.data = values;
				fa.size = size * m;
				fa.persistent = true;
...
{
	return (v <= range->rmin ? values[0] : v >= range->rmax ? values[gx_cie_cache_size - 1] : values[(int)((v - range->rmin) / (range->rmax - range->rmin) *
														   (gx_cie_cache_size - 1) +
													       0.5)]);
}
/*
