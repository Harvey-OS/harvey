...
private
int
write_floats(gs_param_list* plist, gs_param_name key, const float* values,
             int size, gs_memory_t* mem)
{
	float* p = (float*)gs_alloc_byte_array(mem, size, sizeof(float),
	                                       "write_floats");
...
private
int
write_vector3(gs_param_list* plist, gs_param_name key, const gs_vector3* pvec,
              gs_memory_t* mem)
{
	float values[3];
...
private
int
write_matrix3(gs_param_list* plist, gs_param_name key, const gs_matrix3* pmat,
              gs_memory_t* mem)
{
	float values[9];
...
private
int
write_range3(gs_param_list* plist, gs_param_name key, const gs_range3* prange,
             gs_memory_t* mem)
{
	float values[6];
...
private
int
write_proc3(gs_param_list* plist, gs_param_name key, const gs_cie_render* pcrd,
            const gs_cie_render_proc3* procs, const gs_range3* domain,
            gs_memory_t* mem)
{
	float* values;
                      ...
                      gs_memory_t* mem)
{
	...
	if(pcrd->TransformPQR.proc_name) {
		...
		if((code = param_write_name(plist, "TransformPQRName", &pn)) <
		       0 ||
		   (code = param_write_string(plist, "TransformPQRData", &pd)) <
		       0)
			return code;
	} else if(pcrd->TransformPQR.proc != TransformPQR_default.proc) {
		/* We have no way to represent the procedure, so return an
		 * error. */
		return_error(gs_error_rangecheck);
	}
	if((code = param_write_int(plist, "ColorRenderingType", &crd_type)) <
	       0 ||
	   (code = write_vector3(plist, "WhitePoint", &pcrd->points.WhitePoint,
	                         mem)) < 0)
		return code;
	if(memcmp(&pcrd->points.BlackPoint, &BlackPoint_default,
	          sizeof(pcrd->points.BlackPoint))) {
		if((code = write_vector3(plist, "BlackPoint",
		                         &pcrd->points.BlackPoint, mem)) < 0)
			return code;
	}
	if((code = write_matrix3(plist, "MatrixPQR", &pcrd->MatrixPQR, mem)) <
	       0 ||
	   (code = write_range3(plist, "RangePQR", &pcrd->RangePQR, mem)) < 0 ||
	   /* TransformPQR is handled separately */
	   (code = write_matrix3(plist, "MatrixLMN", &pcrd->MatrixLMN, mem)) <
	       0 ||
	   (code = write_proc3(plist, "EncodeLMNValues", pcrd, &pcrd->EncodeLMN,
	                       &pcrd->DomainLMN, mem)) < 0 ||
	   (code = write_range3(plist, "RangeLMN", &pcrd->RangeLMN, mem)) < 0 ||
	   (code = write_matrix3(plist, "MatrixABC", &pcrd->MatrixABC, mem)) <
	       0 ||
	   (code = write_proc3(plist, "EncodeABCValues", pcrd, &pcrd->EncodeABC,
	                       &pcrd->DomainABC, mem)) < 0 ||
	...
	if(pcrd->RenderTable.lookup.table) {
		...
		int* size = (int*)gs_alloc_byte_array(mem, n + 1, sizeof(int),
		                                      "RenderTableSize");
		 ...
		 */
		gs_param_string* table = (gs_param_string*)gs_alloc_byte_array(
		    mem, na, sizeof(gs_param_string), "RenderTableTable");
		gs_param_int_array ia;

		if(size == 0 || table == 0)
			code = gs_note_error(gs_error_VMerror);
		else {
			memcpy(size, pcrd->RenderTable.lookup.dims,
			       sizeof(int) * n);
			...
			ia.size = n + 1;
			ia.persistent = true;
			code = param_write_int_array(plist, "RenderTableSize",
			                             &ia);
		}
		if(code >= 0) {
			...
			for(a = 0; a < na; ++a)
				table[a].data =
				    pcrd->RenderTable.lookup.table[a].data,
				table[a].size =
				    pcrd->RenderTable.lookup.table[a].size,
				table[a].persistent = true;
			sa.data = table;
			sa.size = na;
			sa.persistent = true;
			code = param_write_string_array(
			    plist, "RenderTableTable", &sa);
			if(code >= 0 &&
			   !pcrd->caches.RenderTableT_is_identity) {
				/****** WRITE RenderTableTValues LIKE
				 * write_proc3 ******/
				uint size = gx_cie_cache_size;
				float* values = (float*)gs_alloc_byte_array(
				    mem, size * m, sizeof(float),
				    "write_proc3");
				gs_param_float_array fa;
				int i;
...
private
int
read_vector3(gs_param_list* plist, gs_param_name key, gs_vector3* pvec,
             const gs_vector3* dflt)
{
	float values[3];
                 ...
                 const gs_range* range)
{
	return (v <= range->rmin
	            ? values[0]
	            : v >= range->rmax
	                  ? values[gx_cie_cache_size - 1]
	                  : values[(int)((v - range->rmin) /
...
encode_lmn_0_from_data(floatp v, const gs_cie_render* pcrd)
{
	...
	return encode_from_data(v, &data->lmn[0], &pcrd->DomainLMN.ranges[0]);
}
private
float
...
encode_abc_0_from_data(floatp v, const gs_cie_render* pcrd)
{
	...
	return encode_from_data(v, &data->abc[0], &pcrd->DomainABC.ranges[0]);
}
private
float
...
render_table_t_0_from_data(byte v, const gs_cie_render* pcrd)
{
	...
	return float2frac(encode_from_data(v / 255.0, &data->t[0],
...
render_table_t_1_from_data(byte v, const gs_cie_render* pcrd)
{
	...
	return float2frac(encode_from_data(
	    v / 255.0, &data->t[gx_cie_cache_size], &Range3_default.ranges[0]));
}
private
frac
...
private
const gs_cie_render_proc3 EncodeLMN_from_data = {
    {encode_lmn_0_from_data, encode_lmn_1_from_data, encode_lmn_2_from_data}};
private
const gs_cie_render_proc3 EncodeABC_from_data = {
    {encode_abc_0_from_data, encode_abc_1_from_data, encode_abc_2_from_data}};
private
const gs_cie_render_table_procs RenderTableT_from_data = {
...
int
param_get_cie_render1(gs_cie_render* pcrd, gs_param_list* plist, gx_device* dev)
{
	encode_data_t data;
	...
	if((code = param_read_int(plist, "ColorRenderingType", &crd_type)) <
	       0 ||
	   ...
	   (code_lmn = code = read_proc3(plist, "EncodeLMNValues", data.lmn)) <
	       0 ||
	   (code = read_range3(plist, "RangeLMN", &pcrd->RangeLMN)) < 0 ||
	   (code = read_matrix3(plist, "MatrixABC", &pcrd->MatrixABC)) < 0 ||
	   (code_abc = code = read_proc3(plist, "EncodeABCValues", data.abc)) <
	       0 ||
	...
	case 0: /* specified */
		...
		switch(code = param_read_string(plist, "TransformPQRData",
		                                &pdata)) {
		default: /* error */
			return code;
		case 1: /* missing */
	...
	code_rt = code =
	    param_read_int_array(plist, "RenderTableSize", &rt_size);
	if(code == 1) {
		if(pcrd->RenderTable.lookup.table) {
			gs_free_object(pcrd->rc.memory,
			               (void*)pcrd->RenderTable.lookup
			                   .table, /* break const */
		...
		for(j = 0; j < rt_size.size; ++j)
			if(rt_size.data[j] < 1)
				return_error(gs_error_rangecheck);
		code = param_read_string_array(plist, "RenderTableTable",
		                               &rt_values);
		if(code < 0)
			return code;
		...
		if(n > 4 || m > 4)
			return_error(gs_error_rangecheck);
		memcpy(pcrd->RenderTable.lookup.dims, rt_size.data,
		       n * sizeof(int));
		table = gs_alloc_struct_array(
		    pcrd->rc.memory, pcrd->RenderTable.lookup.dims[0],
