...
const stc_dither_t stc_dither[] = {
    ...
    {"hscmyk", stc_hscmyk, DeviceCMYK | STC_LONG | STC_CMYK10 | STC_DIRECT | 1 * STC_SCAN, 1 + 2 * 4, {0.0, 1023.0}},
 ...
 */
	...
	sd->stc.escp_v = sd->stc.flags & (STCUWEAVE | STCNWEAVE) ? sd->stc.escp_u : 40;
	/*
 * Initialize color
 */
 ...
 ***/
	...
	buf_size = sd->stc.dither->bufadd + alg_size * (sd->stc.dither->flags / STC_SCAN);
	if(buf_size > 0) {
		...
		for(i = 0; i < sd->color_info.num_components; ++i) {
			if((flags & STCCOMP) == STCDELTA) {
				...
				if(sd->stc.seed_row[i] == NULL)
					...
					memset(sd->stc.seed_row[i], 0, sd->stc.seed_size * sizeof(int));
			} else {
 ...
 */
			...
			while(sd->stc.stc_y < sd->stc.prt_scans) { /* Until all scans are processed */
				...
				if(sd->stc.buf_y < need) { /* Nr. 5 (give me input) */
					...
					if(ncolor == 1)
						need = sd->stc.stc_y + sd->stc.prt_buf;
					else
						need = sd->stc.stc_y + (sd->stc.prt_buf >> 2);
 ...
 */
						...
						case 3: /* convert rgb into cmyk */
						{
							...
							for(width = 0; width < sd->stc.prt_size; ++width) {
								...
								for(i = 0; i < 4; ++i) {
									if(cmyk[i] != 0)
										sd->stc.prt_width[buf_i + i] = width + 1;
...
stc_freedata(gs_memory_t *mem, stc_t *stc)
{
	...
	for(i = 0; i < 4; ++i) {
		if(stc->code[i] != NULL) {
			...
			if(i == j)
				gs_free(mem, stc->code[i], 1 << stc->bits, sizeof(gx_color_value),
					"stcolor/code");
		}
		...
		if(stc->vals[i] != NULL) {
			...
			if(i == j)
				gs_free(mem, stc->vals[i], 1 << stc->bits, sd->stc.alg_item,
					"stcolor/transfer");
		}
	}
 ...
 */
					...
					for(ly = 0; ly < (1 << sd->stc.bits); ++ly) {
						...
						while(((iy + 1) < sd->stc.sizc[i]) &&
						      (fy > sd->stc.extc[i][iy + 1]))
							++iy;
						fx = iy + (fy - sd->stc.extc[i][iy]) / (sd->stc.extc[i][iy + 1] - sd->stc.extc[i][iy]);
						fx *= xstep * gx_max_color_value;

						fx = fx < 0.0 ? 0.0 : (fx > gx_max_color_value ? gx_max_color_value : fx);

						sd->stc.code[i][ly] = (gx_color_value)fx;
						if((fx - sd->stc.code[i][ly]) >= 0.5)
							sd->stc.code[i][ly] += 1;
					}
				} /* error || success */
				...
				if(sd->stc.vals[i] == NULL) {
					...
					if(sd->stc.code[i] == NULL) { /* linear */
						...
						for(io = 0; io < (Nout); ++io) {
							...
							if((I + 1) < (Nin))
								Y = In[I] + (In[I + 1] - In[I]) * ((double)io * Ostep - (double)I * Istep) / (double)Istep;
							else
								Y = In[I] + (In[I] - In[I - 1]) * ((double)io * Ostep - (double)I * Istep) / (double)Istep;
							...
							case STC_BYTE:
								Ovb = (byte)Y;
								if(((Y - Ovb) >= 0.5) && ((Ovb + 1) <= Omax))
									Ovb += 1;
								Out[io] = Ovb;
								break;
							case STC_LONG:
								Ovl = (long)Y;
								if(((Y - Ovl) >= 0.5) && ((Ovl + 1) <= Omax))
									Ovl += 1;
								if(((Ovl - Y) >= 0.5) && ((Ovl - 1) >= Omax))
									Ovl -= 1;
								((long *)Out)[io] = Ovl;
								break;
							default:
 ...
 */
						for(o = 0; o < (1 << sd->stc.bits); ++o) { /* code-loop */
							...
							y = sd->stc.dither->minmax[0] + (sd->stc.dither->minmax[1] - sd->stc.dither->minmax[0]) * y;

#define stc_adjvals(T)                                                                                                     \
	((T *)(sd->stc.vals[i]))[o] = (T)y;                                                                                \
                                                                                                                           \
	if(((y - ((T *)(sd->stc.vals[i]))[o]) >= 0.5) && ((1 + ((T *)(sd->stc.vals[i]))[o]) <= sd->stc.dither->minmax[1])) \
		((T *)(sd->stc.vals[i]))[o] += 1;                                                                          \
                                                                                                                           \
	if(((((T *)(sd->stc.vals[i]))[o] - y) >= 0.5) && ((((T *)(sd->stc.vals[i]))[o] - 1) >= sd->stc.dither->minmax[0])) \
		((T *)(sd->stc.vals[i]))[o] -= 1;
...
#define stc_write_null(N)                                                                \
	set_param_array(pfa, defext, countof(defext)) code = param_write_null(plist, N); \
	if(code < 0)                                                                     \
		return code;

#define stc_write_xarray(I, Coding, Transfer)                                                                                                                                    \
	if(sd->stc.sizc[I] > 0) {                                                                                                                                                \
		set_param_array(pfa, sd->stc.extc[I], sd->stc.sizc[I]) code = param_write_float_array(plist, Coding, &pfa);                                                      \
	} else {                                                                                                                                                                 \
	...
	if(code < 0)                                                                                                                                                             \
		return code;                                                                                                                                                     \
                                                                                                                                                                                 \
	if(sd->stc.sizv[I] > 0)                                                                                                                                                  \
		set_param_array(pfa, sd->stc.extv[I], sd->stc.sizv[I]) else set_param_array(pfa, defext, countof(defext)) code = param_write_float_array(plist, Transfer, &pfa); \
	if(code < 0)                                                                                                                                                             \
		return code;
...
#undef stc_write_int
	...
	if(sd->stc.am != NULL) {
		if(nc == 1)
			set_param_array(pfa, sd->stc.am, 3) else if(nc == 3) set_param_array(pfa, sd->stc.am, 9) else set_param_array(pfa, sd->stc.am, 16)
...
#define stc_putcmp(Name) \
	...
	if(code == 0) { /* lookup new value new value */
		...
		for(i = 0; stc_dither[i].name != NULL; ++i)
			if((stc_dither[i].flags & 7) == sd->color_info.num_components)
				break;
 ...
 */
	if(error == 0) {
		...
		sd->color_info.max_gray = sd->stc.bits < gx_color_value_bits ? (1 << sd->stc.bits) - 1 : gx_max_color_value;
		...
		if(((sd->stc.dither->flags & STC_TYPE) != STC_FLOAT) &&
		   ((sd->stc.dither->minmax[1] - sd->stc.dither->minmax[0]) <
			...
			sd->color_info.max_gray = (gx_color_value)(sd->stc.dither->minmax[1] - sd->stc.dither->minmax[0] + 0.5);

		sd->color_info.max_color = sd->color_info.num_components < 3 ? 0 : sd->color_info.max_gray;
		sd->color_info.dither_grays =
		    sd->color_info.max_gray < gx_max_color_value ? sd->color_info.max_gray + 1 : gx_max_color_value;
		sd->color_info.dither_colors = sd->color_info.num_components < 3 ? 0 : sd->color_info.dither_grays;
	}
		...
		if(code < 0) { /* free newly allocated arrays */
			...
			if((sd->stc.escp_release.data != NULL) &&
			   (sd->stc.escp_release.data != oldstc.escp_release.data))
				gs_free(sd->memory, (byte *)sd->stc.escp_release.data, sd->stc.escp_release.size, 1, "stcolor/release");
			...
			for(i = 0; i < sd->color_info.num_components; ++i) { /* components */
				...
				if((sd->stc.extc[i] != NULL) &&
				   ...
				   (sd->stc.extc[i] != oldstc.extc[i])) {
					...
					if(i == j)
						gs_free(sd->memory, sd->stc.extc[i], sd->stc.sizc[i], sizeof(float),
							"stc_put_params");
				}
				...
				if((sd->stc.extv[i] != NULL) &&
				   ...
				   (sd->stc.extv[i] != defext)) {
					...
					for(j = 0; j < i; ++j)
						if(sd->stc.extv[i] == sd->stc.extv[j])
							break;

					if(i == j)
						gs_free(sd->memory, sd->stc.extv[i], sd->stc.sizv[i], sizeof(float),
							"stc_put_params");
				}
			} /* components */
		...
		for(i = 0; i < 4; ++i) {
			...
			if((oldstc.extc[i] != NULL) &&
			   ...
			   (oldstc.extc[i] != defext)) {
				...
				if(i == j)
					gs_free(sd->memory, oldstc.extc[i], oldstc.sizc[i], sizeof(float),
						"stc_put_params");
			}
			...
			if((oldstc.extv[i] != NULL) &&
			   ...
			   (oldstc.extv[i] != defext)) {
				...
				if(i == j)
					gs_free(sd->memory, oldstc.extv[i], oldstc.sizv[i], sizeof(float),
						"stc_put_params");
			}
		}
