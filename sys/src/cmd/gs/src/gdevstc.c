...
private
stc_proc_dither(stc_gscmyk); /* resides in this file */
private
stc_proc_dither(stc_hscmyk); /* resides in this file */

#include <stdlib.h> /* for rand, used in stc_hscmyk */

private
const stc_dither_t stc_dither[] = {
    {"gscmyk", stc_gscmyk, DeviceCMYK | STC_BYTE | STC_DIRECT, 0, {0.0, 1.0}},
    {"hscmyk",
     stc_hscmyk,
     DeviceCMYK | STC_LONG | STC_CMYK10 | STC_DIRECT | 1 * STC_SCAN,
     1 + 2 * 4,
     {0.0, 1023.0}},
    STC_MODI{NULL, NULL, 0, 0, {0.0, 0.0}}};
 ...
 */
private
dev_proc_print_page(stc_print_page);
private
dev_proc_open_device(stc_open);
private
dev_proc_close_device(stc_close);
private
dev_proc_get_params(stc_get_params);
private
dev_proc_put_params(stc_put_params);
...
private
dev_proc_map_rgb_color(stc_map_gray_color);
private
dev_proc_map_color_rgb(stc_map_color_gray);

/* routines for RGB-Modi */
private
dev_proc_map_rgb_color(stc_map_rgb_color);
private
dev_proc_map_color_rgb(stc_map_color_rgb);

/* routines for general CMYK-Modi */
private
dev_proc_map_cmyk_color(stc_map_cmyk_color);
private
dev_proc_map_color_rgb(stc_map_color_cmyk);

/* routines for 10Bit/Component CMYK */
private
dev_proc_map_cmyk_color(stc_map_cmyk10_color);
private
dev_proc_map_color_rgb(stc_map_color_cmyk10);

/***
 *** Table of Device-Procedures
 ***/
private
gx_device_procs stcolor_procs = {
    stc_open, gx_default_get_initial_matrix, gx_default_sync_output,
    gdev_prn_output_page, stc_close, NULL, stc_map_color_cmyk,
    NULL, /* fill_rectangle */
    NULL, /* tile_rectangle */
    NULL, /* copy_mono */
    NULL, /* copy_color */
    NULL, /* draw_line */
    gx_default_get_bits, stc_get_params, stc_put_params, stc_map_cmyk_color};
...
stcolor_device far_data gs_stcolor_device = {
    prn_device_body(stcolor_device, stcolor_procs, "stcolor",
                    ...
                    STC_L_MARGIN, STC_B_MARGIN, STC_R_MARGIN, STC_T_MARGIN, 4,
                    4, 1, 1, 2, 2, /* default: cmyk-direct */
                    stc_print_page),
 ...
 ***/
private
bool stc_iswhite(stcolor_device*, int, byte*);

/***
 *** Functions used for conversion inside the print-loop
 ***/
#define stc_proc_iconvert(Name)                                                \
	byte* Name(stcolor_device* sd, byte* ext_data, int prt_pixels,         \
	           byte* alg_line)

private
stc_proc_iconvert(stc_any_depth); /* general input-conversion */
private
stc_proc_iconvert(stc_rgb24_long); /* 24Bit RGB  -> long's */

private
stc_proc_iconvert(stc_cmyk32_long); /* 32Bit CMYK -> long's */
private
stc_proc_iconvert(stc_any_direct); /* use ext_data as input */

private
stc_proc_iconvert(stc_cmyk10_byte); /* CMYK10->vals-> any type */
private
stc_proc_iconvert(stc_cmyk10_long); /* CMYK10->vals-> any type */
private
stc_proc_iconvert(stc_cmyk10_float); /* CMYK10->vals-> any type */
private
stc_proc_iconvert(stc_cmyk10_dbyte); /* CMYK10 direct bytes */
private
stc_proc_iconvert(stc_cmyk10_dlong); /* CMYK10 direct longs */

/***
 *** Print-functions
 ***/
private
void stc_print_weave(stcolor_device* sd, FILE* prn_stream);
private
void stc_print_bands(stcolor_device* sd, FILE* prn_stream);
private
void stc_print_delta(stcolor_device* sd, FILE* prn_stream);
private
int stc_print_setup(stcolor_device* sd);
...
stc_print_setup(stcolor_device* sd)
{
	...
	sd->stc.escp_v =
	    sd->stc.flags & (STCUWEAVE | STCNWEAVE) ? sd->stc.escp_u : 40;
	/*
	 * Initialize color
	 */
	...
	if((sd->stc.flags & STCBAND) == 0) {
		if(sd->stc.escp_v != sd->stc.escp_u)
			sd->stc.escp_m = 15;
		else if(STCSTCII == (sd->stc.flags & STCMODEL))
			sd->stc.escp_m = 1;
		else if(sd->stc.flags & STCUWEAVE)
			sd->stc.escp_m = 1;
	...
	if((sd->stc.flags & STCWIDTH) == 0)
		sd->stc.escp_width =
		    (int)(sd->width -
		          (dev_l_margin(sd) + dev_r_margin(sd)) *
		              sd->x_pixels_per_inch);
	...
	if((sd->stc.flags & STCTOP) == 0)
		sd->stc.escp_top =
		    (int)(dev_t_margin(sd) * sd->y_pixels_per_inch);
	...
	if((sd->stc.flags & STCINIT) ==
	   0) {                 /* No Initialization-String defined */
		...
		if(need != sd->stc.escp_init.size) { /* Reallocate */
			...
			if(NULL !=
			   (bp = gs_malloc(sd->memory, need, 1,
			                   "stcolor/init"))) { /* Replace */
				if(0 != sd->stc.escp_init.size)
					gs_free(sd->memory,
					        (byte*)sd->stc.escp_init.data,
					        sd->stc.escp_init.size, 1,
					        "stcolor/init");
		...
		memcpy(bp,
		       /*                       1 1 11  1 11  1   1 1  2 22 2  2
		          22  2 22 3  3 3333  3 33*/
		       /* 0 1  2 34  5  6 7  8 90 1 23  4 56  7   8 9  0 12 3  4
		          56  7 89 0  1 2345  6 78*/
		       "\033@\033(G\001\0\1\033(i\1\0w\033(U\001\000u\033("
		       "C\2\000hh\033(c\4\000ttbb\033U",
		       need);

		if((sd->stc.flags & STCUWEAVE) != 0)
			bp[13] = '\1';
		else
			bp[13] = '\0';
	...
	if((sd->stc.flags & STCRELEASE) == 0) { /* No Release-String defined */
		...
		if(need != sd->stc.escp_release.size) { /* Reallocate */
			...
			if(NULL !=
			   (bp = gs_malloc(sd->memory, need, 1,
			                   "stcolor/release"))) { /* Replace */
				if(0 != sd->stc.escp_release.size)
					gs_free(
					    sd->memory,
					    (byte*)sd->stc.escp_release.data,
					    sd->stc.escp_release.size, 1,
					    "stcolor/release");
...
#define SORRY (flags &= ~STCOK4GO)
	...
	ext_line =
	    gs_malloc(sd->memory, ext_size, 1, "stc_print_page/ext_line");
	if(ext_line == NULL)
		SORRY;
	...
	sd->stc.prt_scans = (int)(sd->height -
	                          (dev_t_margin(sd) + dev_b_margin(sd)) *
	                              sd->y_pixels_per_inch);

	col_line =
	    gs_malloc(sd->memory, prt_pixels, 1, "stc_print_page/col_line");
	if(col_line == NULL)
		SORRY;
	...
	if((sd->stc.dither->flags & STC_DIRECT) ||
	   ((sd->stc.bits == 8) && (sd->stc.alg_item == 1))) {
		alg_line = NULL;
	} else {
		alg_line = gs_malloc(sd->memory, alg_size, sd->stc.alg_item,
	...
	if(buf_size > 0) {
		buf = gs_malloc(sd->memory, buf_size, sd->stc.alg_item,
		                "stc_print_page/buf");
		if(buf == NULL)
			SORRY;
	} else {
	...
	if(sd->stc.prt_data == NULL) {
		...
		for(i = 0; i < sd->stc.prt_buf; ++i) {
			sd->stc.prt_data[i] =
			    gs_malloc(sd->memory, sd->stc.prt_size, 1,
			              "stc_print_page/prt");
	...
	sd->stc.seed_size =
	    (sd->stc.prt_size + 2 * sizeof(int) - 1) / sizeof(int);
	{
		int i;
		for(i = 0; i < sd->color_info.num_components; ++i) {
			if((flags & STCCOMP) == STCDELTA) {
				sd->stc.seed_row[i] = gs_malloc(
				    sd->memory, sd->stc.seed_size, sizeof(int),
				    "stc_print_page/seed_row");
				if(sd->stc.seed_row[i] == NULL)
					SORRY;
				else
					memset(sd->stc.seed_row[i], 0,
					       sd->stc.seed_size * sizeof(int));
			} else {
				sd->stc.seed_row[i] = NULL;
			}
		}
		while(i < countof(sd->stc.seed_row))
			sd->stc.seed_row[i++] = NULL;
	}
	...
	default:
		sd->stc.escp_size =
		    64 + sd->stc.prt_size + (sd->stc.prt_size + 127) / 128;
		break;
	}
	...
	if(OK4GO) {
		...
		if((sd->stc.dither->flags & STC_DIRECT) ||
		   ((sd->stc.bits == 8) && (sd->stc.alg_item == 1)))
			...
			else if(((sd->stc.dither->flags & STC_TYPE) ==
			         STC_LONG) &&
			        (sd->stc.dither->minmax[0] == 0.0) &&
		...
		if((*sd->stc.dither->fun)(sd, -prt_pixels, alg_line, buf,
		                          col_line) < 0)
			SORRY;
		...
		if(OK4GO) {
			...
			/*
			 * Decide, wether we Adjust Linefeeds or not. (I hate it
			 * here)
			 */
			if((0 == ((sd->stc.escp_m * sd->stc.escp_u) % 10)) &&
			   (256 > ((sd->stc.escp_m * sd->stc.escp_u) / 10)))
				sd->stc.escp_lf = sd->stc.escp_m;
			else
				sd->stc.escp_lf = 0;
			...
			sd->stc.prt_y =
			    0;     /* physical position of the printer */
			buf_i = 0; /* next free line in buffer */
			sd->stc.flags &= ~STCPRINT; /* no data yet */

			while(sd->stc.stc_y <
			      sd->stc.prt_scans) { /* Until all scans are
			                              processed */
				int need;
				...
				if(sd->stc.buf_y <
				   need) { /* Nr. 5 (give me input) */
					...
					for(; sd->stc.buf_y < need;
						...
						/* initialize output data 1st ->
						 * may take shortcut */

						for(color = 0; color < ncolor;
						    ++color) {
							memset(
							    sd->stc.prt_data
							        [buf_i + color],
						...
						/* convert data for the various
						 * cases */

						alg_data = (*iconvert)(
						    sd, ext_data, prt_pixels,
						    alg_line);
						...
						(*sd->stc.dither->fun)(
						    sd, prt_pixels, alg_data,
						    buf, col_line);
						/*
						 * convert col_line to
						 * printer-format (separate
						 * colors)
						 */
						switch(sd->color_info
						           .num_components) {
						case 1: /* Black & White: just
						           merge into 8 Bytes */
						{
							byte* bytein, *byteout;
							int width;
				...
				/*
				 *    Nr. 5 has got enough input, now we should
				 * print it
				 */
				if((flags & STCCOMP) == STCDELTA)
					stc_print_delta(sd, prn_stream);
				else if(npass > 1)
					stc_print_weave(sd, prn_stream);
				else
					stc_print_bands(sd, prn_stream);
				...
				if(sigismember(&stc_int_pending, SIGINT)) {
					...
					fflush(prn_stream);
					sigprocmask(SIG_SETMASK, &stc_int_save,
					            NULL);
					break;
				}
#endif /* STC_SIGNAL */
			...
			if(sd->stc.flags & STCPRINT) {
				if((flags & STCCOMP) == STCDELTA)
					fputc(0xe3, prn_stream);
				fwrite(sd->stc.escp_release.data, 1,
				       sd->stc.escp_release.size, prn_stream);
				fflush(prn_stream);
			}
#ifdef STC_SIGNAL
	...
	if(ext_line != NULL)
		gs_free(sd->memory, ext_line, ext_size, 1,
		        "stc_print_page/ext_line");

	if(col_line != NULL)
		gs_free(sd->memory, col_line, prt_pixels, 1,
		        "stc_print_page/col_line");
	...
	if(buf != NULL)
		gs_free(sd->memory, buf, buf_size, sd->stc.alg_item,
		        "stc_print_page/buf");

	if(sd->stc.prt_width != NULL)
		gs_free(sd->memory, sd->stc.prt_width, sd->stc.prt_buf,
		        sizeof(int), "stc_print_page/prt_width");
	...
	if(sd->stc.prt_data != NULL) {
		...
		for(i = 0; i < sd->stc.prt_buf; ++i) {
			if(sd->stc.prt_data[i] != NULL)
				gs_free(sd->memory, sd->stc.prt_data[i],
				        sd->stc.prt_size, 1,
				        "stc_print_page/prt");
		}

		gs_free(sd->memory, sd->stc.prt_data, sd->stc.prt_buf,
		        sizeof(byte*), "stc_print_page/prt_data");
	}

	{
		...
		for(i = 0; i < sd->color_info.num_components; ++i) {
			if(sd->stc.seed_row[i] != NULL)
				gs_free(sd->memory, sd->stc.seed_row[i],
				        sd->stc.seed_size, sizeof(int),
				        "stc_print_page/seed_row");
		}
	}
...
private
byte*
stc_any_depth(stcolor_device* sd, byte* ext_data, int prt_pixels,
              byte* alg_line)
{ /* general conversion */
	...
	if(sd->color_info.depth == (sd->color_info.num_components << 3))
		nbits = 8;
...
#define stc_storeapc(T) ((T*)ap)[c] = ((T*)(sd->stc.vals[c]))[ci & cvmsk];
...
private
byte*
stc_rgb24_long(stcolor_device* sd, byte* ext_data, int prt_pixels,
               byte* alg_line)
...
private
byte*
stc_cmyk32_long(stcolor_device* sd, byte* ext_data, int prt_pixels,
                byte* alg_line)
...
private
byte*
stc_cmyk10_byte(stcolor_device* sd, byte* ext_data, int prt_pixels,
                byte* alg_line)
{
	STC_CMYK10_ANY(byte)
}
private
byte*
stc_cmyk10_long(stcolor_device* sd, byte* ext_data, int prt_pixels,
                byte* alg_line)
{
	STC_CMYK10_ANY(long)
}
private
byte*
stc_cmyk10_float(stcolor_device* sd, byte* ext_data, int prt_pixels,
                 byte* alg_line)
{
...
private
byte*
stc_cmyk10_dbyte(stcolor_device* sd, byte* ext_data, int prt_pixels,
                 byte* alg_line)
{
...
private
byte*
stc_cmyk10_dlong(stcolor_device* sd, byte* ext_data, int prt_pixels,
                 byte* alg_line)
{
...
private
byte*
stc_any_direct(stcolor_device* sd, byte* ext_data, int prt_pixels,
               byte* alg_line)
{ /* return ext_data */
	return ext_data;
} /* return ext_data */
...
private
int
stc_print_escpcmd(stcolor_device* sd, FILE* prn_stream, int escp_used,
                  int color, int m, int wbytes)
{
	...
	if(0 == (sd->stc.flags & STCPRINT)) {
		...
		fwrite(sd->stc.escp_init.data, 1, sd->stc.escp_init.size,
		       prn_stream);

		if(0 < sd->stc.escp_lf) { /* Adjust Linefeed */
			fputc('\033', prn_stream);
			fputc('+', prn_stream);
			fputc(((sd->stc.escp_m * sd->stc.escp_u) / 10),
			      prn_stream);
		} /* Adjust Linefeed */
...
stc_bandwidth(stcolor_device* sd, int color, int m, int npass)
{
	...
	while(m-- > 0) { /* check width */
		if(sd->stc.prt_width[buf_a] > w)
			w = sd->stc.prt_width[buf_a];
...
stc_print_weave(stcolor_device* sd, FILE* prn_stream)
{
	...
	while(sd->stc.stc_y < sd->stc.prt_scans) {
		...
		if(sd->stc.stc_y >= sd->stc.escp_m) { /* in normal mode */
			...
			nprint = sd->stc.escp_m -
			         sd->stc.stc_y * ((sd->stc.escp_m + 1) / npass);
			nspace = 1;
		...
		if(sd->stc.buf_y < iprint)
			break;

		escp_used = 0;
		for(color = 0; color < ncolor; ++color) { /* print the colors */

			if(0 == (w = stc_bandwidth(sd, color, nprint, npass)))
				continue;

			escp_used = stc_print_escpcmd(sd, prn_stream, escp_used,
			                              color, sd->stc.escp_m, w);

			buf_a = (sd->stc.prt_buf - 1) &
			        (sd->stc.stc_y * ncolor + color);
			for(iprint = 0; iprint < nprint;
			    ++iprint) { /* send data */

				if((sd->stc.flags & STCCOMP) == STCPLAIN) {
					memcpy(sd->stc.escp_data + escp_used,
				...
				fwrite(sd->stc.escp_data, 1, escp_used,
				       prn_stream);
				escp_used = 0;
			...
			while(iprint++ < sd->stc.escp_m) { /* add empty rows */
				...
				if((sd->stc.flags & STCCOMP) == STCPLAIN) {
					memset(sd->stc.escp_data + escp_used, 0,
					       w);
					escp_used += w;
				} else {
					escp_used += stc_rle(sd->stc.escp_data +
					                         escp_used,
					                     NULL, w);
				}
...
stc_print_bands(stcolor_device* sd, FILE* prn_stream)
{
	...
	while(sd->stc.stc_y < sd->stc.prt_scans) {
		...
		for(w = 0; sd->stc.stc_y < sd->stc.buf_y; ++sd->stc.stc_y) {
			buf_a =
			    (sd->stc.prt_buf - 1) & (sd->stc.stc_y * ncolor);
		...
		if(sd->stc.buf_y < (sd->stc.stc_y + m))
			break;

		escp_used = 0;
		for(color = 0; color < ncolor; ++color) { /* print the colors */

			if(0 == (w = stc_bandwidth(sd, color, m, 1)))
				continue; /* shortcut */
...
stc_print_delta(stcolor_device* sd, FILE* prn_stream)
{
	...
	while(sd->stc.stc_y < sd->stc.prt_scans) {
		...
		if(0 == (sd->stc.flags & STCPRINT)) {
			...
			fwrite(sd->stc.escp_init.data, 1,
			       sd->stc.escp_init.size, prn_stream);
		...
		if(sd->stc.stc_y !=
		   sd->stc.prt_y) { /* really position the printer */
		...
		for(color = 0; color < ncolor; ++color) { /* print the colors */
			...
			if(color == (ncolor - 1)) {
				...
				switch(color) {
				case 1:
					sd->stc.escp_data[escp_used++] = 0x81;
					break; /* M */
				case 2:
					sd->stc.escp_data[escp_used++] = 0x84;
					break; /* Y */
				default:
					sd->stc.escp_data[escp_used++] = 0x82;
					break; /* C */
				}
			}
			...
			w = stc_deltarow(sd->stc.escp_data + escp_used,
			                 sd->stc.prt_data[buf_a],
			                 sd->stc.prt_width[buf_a],
...
stc_freedata(gs_memory_t* mem, stc_t* stc)
{
	...
	for(i = 0; i < 4; ++i) {
		if(stc->code[i] != NULL) {
			...
			for(j = 0; j < i; ++j)
				if(stc->code[i] == stc->code[j])
					break;

			if(i == j)
				gs_free(mem, stc->code[i], 1 << stc->bits,
				        sizeof(gx_color_value), "stcolor/code");
		}

		if(stc->vals[i] != NULL) {

			for(j = 0; j < i; ++j)
				if(stc->vals[i] == stc->vals[j])
					break;

			if(i == j)
				gs_free(mem, stc->vals[i], 1 << stc->bits,
				        sd->stc.alg_item, "stcolor/transfer");
		}
	}
...
private
int stc_open(gx_device* pdev) /* setup margins & arrays */
{
	stcolor_device* sd = (stcolor_device*)pdev;
	int i, j, code;
		   ...
		   (sd->stc.extc[i] != NULL)) { /* code req. */
			...
			if(i == j) { /* new one */
				...
				if(sd->stc.code[i] == NULL) { /* error */
					...
					fx = 1e18;
					fy = -1e18;
					for(ly = 0; ly < sd->stc.sizc[i];
					    ++ly) {
						if(sd->stc.extc[i][ly] < fx)
							fx =
							    sd->stc.extc[i][ly];
						if(sd->stc.extc[i][ly] > fy)
							fy =
							    sd->stc.extc[i][ly];
					}
					if((fx != 0.0) || (fy != 1.0)) {
					...
					ystep =
					    1.0 /
					    (double)((1 << sd->stc.bits) - 1);
					xstep =
					    1.0 / (double)(sd->stc.sizc[i] - 1);

					iy = 0;
					for(ly = 0; ly < (1 << sd->stc.bits);
					    ++ly) {
						fy = ystep * ly;
						while(((iy + 1) <
						       sd->stc.sizc[i]) &&
						...
						fx =
						    ...
						    (fy - sd->stc.extc[i][iy]) /
						        (sd->stc
						         ...
						         sd->stc.extc[i][iy]);
						fx *=
						    xstep * gx_max_color_value;

						fx =
						    fx < 0.0
						        ? 0.0
						        : (fx > gx_max_color_value
						               ? gx_max_color_value
						               : fx);

						sd->stc.code[i][ly] =
						    (gx_color_value)fx;
						if((fx - sd->stc.code[i][ly]) >=
						   0.5)
							sd->stc.code[i][ly] +=
							    1;
					}
				...
				if(sd->stc.vals[i] == NULL) {
					...
					if(sd->stc.code[i] ==
					   NULL) { /* linear */
						...
						double Omin =
						    sd->stc.dither->minmax[0];
						double Omax =
						    sd->stc.dither->minmax[1];
						...
						xstep =
						    1.0 /
						    (double)(sd->stc.sizv[i] -
						             1);

						/*
						 * The following differs in so
						 * far from the previous, that
						 * the desired
						 * X-Values are stored in
						 * another array.
						 */
						for(o = 0;
						    o < (1 << sd->stc.bits);
...
#undef stc_adjvals
	...
	if(code == 0) {
		 ...
		 *        necessary to establish the defaults for omitted
		 * procedures too.
		 */

		switch(
		    sd->color_info.num_components) { /* Establish color-procs */
		case 1:
			set_dev_proc(sd, map_rgb_color, stc_map_gray_color);
			set_dev_proc(sd, map_cmyk_color,
		...
		case 3:
			...
			break;
		default:
			set_dev_proc(sd, map_rgb_color,
			             gx_default_map_rgb_color);
			if(sd->stc.flags & STCCMYK10) {
				set_dev_proc(sd, map_cmyk_color,
				             stc_map_cmyk10_color);
				set_dev_proc(sd, map_color_rgb,
				             stc_map_color_cmyk10);
				set_dev_proc(sd, encode_color,
				             stc_map_cmyk10_color);
				set_dev_proc(sd, decode_color,
				             stc_map_color_cmyk10);
				cv[0] = cv[1] = cv[2] = cv[3] = 0;
				white =
				    stc_map_cmyk10_color((gx_device*)sd, cv);
			} else {
				set_dev_proc(sd, map_cmyk_color,
				             stc_map_cmyk_color);
				set_dev_proc(sd, map_color_rgb,
		 ...
		 */
		if(sd->color_info.depth < 2)
			white |= (white << 1);
		if(sd->color_info.depth < 4)
			white |= (white << 2);
		if(sd->color_info.depth < 8)
			white |= (white << 4);
...
stc_expand(stcolor_device* sd, int i, gx_color_index col)
{
	...
	if(sd->stc.code[i] != NULL) {
		...
		cv += (col & l) / l *
		      ((1 << (gx_color_value_bits - sd->stc.bits)) - 1);
...
stc_map_gray_color(gx_device* pdev, const gx_color_value cv[])
{
	...
	if((r == g) && (g == b)) {
		...
		if(fv < 0.0)
			rv = 0;
		else if((fv + 0.5) > gx_max_color_value)
			rv = gx_max_color_value;
		else
			rv = (gx_color_index)(fv + 0.5);
...
private
int
stc_map_color_gray(gx_device* pdev, gx_color_index color,
	...
	if((c == m) && (m == y)) {
		...
		if(sd->stc.am != NULL) {
			...
			if(fv < 0.0)
				...
				c = gx_max_color_value;
			else
				c = (gx_color_value)(fv + 0.5);
			...
			if(fv < 0.0)
				...
				m = gx_max_color_value;
			else
				m = (gx_color_value)(fv + 0.5);
			...
			if(fv < 0.0)
				...
				y = gx_max_color_value;
			else
				y = (gx_color_value)(fv + 0.5);
...
stc_map_color_cmyk(gx_device* pdev, gx_color_index color, gx_color_value cv[4])
{
	...
	k = stc_expand(sd, 3, color & l);
	color >>= shift;
	y = stc_expand(sd, 2, color & l);
	color >>= shift;
	m = stc_expand(sd, 1, color & l);
	color >>= shift;
...
stc_map_cmyk10_color(gx_device* pdev, const gx_color_value cv[])
{
	...
	switch(mode) {
	case 0:
		rv = (((gx_color_index)m) << 22) | (((gx_color_index)y) << 12) |
		     (((gx_color_index)k) << 2) | mode;
		break;
	case 1:
		rv = (((gx_color_index)c) << 22) | (((gx_color_index)y) << 12) |
		     (((gx_color_index)k) << 2) | mode;
		break;
	case 2:
		rv = (((gx_color_index)c) << 22) | (((gx_color_index)m) << 12) |
		     (((gx_color_index)k) << 2) | mode;
		break;
	default:
...
#define stc_write_null(N)                                                      \
	set_param_array(pfa, defext, countof(defext)) code =                   \
	    param_write_null(plist, N);                                        \
	if(code < 0)                                                           \
		return code;

#define stc_write_xarray(I, Coding, Transfer)                                  \
	if(sd->stc.sizc[I] > 0) {                                              \
		set_param_array(pfa, sd->stc.extc[I], sd->stc.sizc[I]) code =  \
		    param_write_float_array(plist, Coding, &pfa);              \
	} else {                                                               \
	...
	if(sd->stc.sizv[I] > 0)                                                \
		set_param_array(                                               \
		    pfa, sd->stc.extv[I],                                      \
		    sd->stc.sizv[I]) else set_param_array(pfa, defext,         \
		                                          countof(defext))     \
...
#define stc_read_null(N)                                                       \
	...
	if(code < 0)                                                           \
		param_signal_error(plist, N, code);                            \
...
#define stc_read_xarray(I, Coding, Transfer)                                   \
	...
	if(code < 0)                                                           \
		param_signal_error(plist, Coding, code);                       \
	error = error > code ? code : error;                                   \
	...
	if(code < 0)                                                           \
		param_signal_error(plist, Transfer, code);                     \
	error = error > code ? code : error;
...
stc_get_params(gx_device* pdev, gs_param_list* plist)
{
	...
	if(sd->stc.algorithms.size > 0) {
		code = param_write_string_array(plist, "Algorithms",
		                                &sd->stc.algorithms);
	} else {
	...
	case STCPLAIN:
		param_string_from_string(ps, "plain");
		break;
	case STCDELTA:
		param_string_from_string(ps, "deltarow");
		break;
	default:
		param_string_from_string(ps, "runlength");
		break;
	}
	...
	case STCST800:
		param_string_from_string(ps, "st800");
		break;
	case STCSTCII:
		param_string_from_string(ps, "stcii");
		break;
	default:
		param_string_from_string(ps, "stc");
		break;
	}
...
#define stc_write_flag(Mask, Name)                                             \
	...
	stc_write_flag(STCUNIDIR, "Unidirectional")
	    stc_write_flag(STCUWEAVE, "Microweave") btmp =
	        sd->stc.flags & (STCUNIDIR | STCUWEAVE) ? false : true;
	code = param_write_bool(plist, "Softweave", &btmp);
	if(code < 0)
		return code;
	stc_write_flag(STCNWEAVE, "noWeave") stc_write_flag(STCDFLAG0, "Flag0")
...
#undef stc_write_int
	...
	if(sd->stc.am != NULL) {
		if(nc == 1)
			set_param_array(pfa, sd->stc.am, 3) else if(nc == 3)
			    set_param_array(
			        pfa, sd->stc.am,
			        9) else set_param_array(pfa, sd->stc.am, 16)
			        code = param_write_float_array(
			            plist, "ColorAdjustMatrix", &pfa);
	} else {
	...
	if(nc == 1) { /* DeviceGray */
		...
		stc_write_null("Rcoding");
		stc_write_null("Rtransfer");
		stc_write_null("Gcoding");
		stc_write_null("Gtransfer");
		stc_write_null("Bcoding");
		stc_write_null("Btransfer");

		stc_write_null("Ccoding");
		stc_write_null("Ctransfer");
		stc_write_null("Mcoding");
		stc_write_null("Mtransfer");
		stc_write_null("Ycoding");
		stc_write_null("Ytransfer");
		...
		stc_write_null("Ycoding");
		stc_write_null("Ytransfer");
		stc_write_null("Kcoding");
		stc_write_null("Ktransfer");
...
stc_put_params(gx_device* pdev, gs_param_list* plist)
{
	...
	/*
	 * Fetch Major-Parameters (Model, Dithering,
	...
	if(code == 0) { /* Analyze the Model-String */
		        /*
		         * Arrgh: I should have known, that internal strings are not
		         * zero-terminated.
		         */
...
#define stc_putcmp(Name)                                                       \
	((strlen(Name) != l) || (0 != strncmp(Name, (const char*)ps.data, l)))
		...
		if(!stc_putcmp("st800"))
			sd->stc.flags |= STCST800;
		else if(!stc_putcmp("stcii"))
			sd->stc.flags |= STCSTCII;

	} /* Analyze the Model-String */
	if(code < 0)
		param_signal_error(plist, "Model", code);
	error = error > code ? code : error;

	/* If we're running for st800, #components must be 1 */
	if(((sd->stc.flags & STCMODEL) == STCST800) &&
	   ((sd->color_info.num_components > 1) || (sd->stc.dither == NULL) ||
	...
	if(code == 0) { /* lookup new value new value */
		...
		for(i = 0; stc_dither[i].name != NULL; ++i)
			if(!stc_putcmp(stc_dither[i].name))
				break;
...
#undef stc_sizeofitem
		...
		for(i = sd->color_info.num_components; i < 4;
		    ++i) { /* clear unused */
			sd->stc.extv[i] = NULL;
			sd->stc.sizv[i] = 0;
			sd->stc.vals[i] = NULL;
		 ...
		 */
		if((sd->stc.dither != oldstc.dither) ||
		   (oldstc.vals[0] == NULL)) {
			...
			if((sd->stc.dither->flags & STC_CMYK10) != 0) {
				...
				if((sd->stc.dither->flags & STC_FLOAT) !=
				   STC_FLOAT) {
					fv = 2.0;
					for(i = 1;
					    (i < gx_color_value_bits) &&
					        (fv <=
					         (sd->stc.dither->minmax[1] -
					          sd->stc.dither->minmax[0]));
					    ++i)
				...
				if((i * sd->color_info.num_components) >
				   (sizeof(stc_pixel) * 8)) {
	...
	if(code < 0)
		param_signal_error(plist, "Dithering", code);
	error = error > code ? code : error;
	...
	if((error == 0) && (code == 0) && (((sd->stc.flags & STCCMYK10) == 0) ||
	                                   (i != sd->color_info.depth))) {
	...
	if(code == 0) { /* Analyze the OutputCode-String */
		...
		if(!stc_putcmp("plain"))
			sd->stc.flags |= STCPLAIN;
		else if(!stc_putcmp("deltarow"))
			sd->stc.flags |= STCDELTA;
	...
	if(code < 0)
		param_signal_error(plist, "OutputCode", code);
	error = error > code ? code : error;
...
#define stc_read_flag(Mask, Name)                                              \
	...
	stc_read_flag(STCUNIDIR, "Unidirectional")
	    stc_read_flag(STCDFLAG0, "Flag0") stc_read_flag(STCDFLAG1, "Flag1")
...
#undef stc_read_int
	...
	if(code < 0)
		param_signal_error(plist, "ColorAdjustMatrix", code);
	error = error > code ? code : error;
	...
	if(sd->color_info.num_components == 1) { /* DeviceGray */
		...
		stc_read_null("Rcoding");
		stc_read_null("Rtransfer");
		stc_read_null("Gcoding");
		stc_read_null("Gtransfer");
		stc_read_null("Bcoding");
		stc_read_null("Btransfer");

		stc_read_null("Ccoding");
		stc_read_null("Ctransfer");
		stc_read_null("Mcoding");
		stc_read_null("Mtransfer");
		stc_read_null("Ycoding");
		stc_read_null("Ytransfer");
		...
		stc_read_null("Ycoding");
		stc_read_null("Ytransfer");
		stc_read_null("Kcoding");
		stc_read_null("Ktransfer");
	...
	if(error == 0) {
		...
		if(((sd->stc.dither->flags & STC_TYPE) != STC_FLOAT) &&
		   ((sd->stc.dither->minmax[1] - sd->stc.dither->minmax[0]) <
			...
			sd->color_info.max_gray =
			    (gx_color_value)(sd->stc.dither->minmax[1] -
			                     sd->stc.dither->minmax[0] + 0.5);

		sd->color_info.max_color = sd->color_info.num_components < 3
		                               ? 0
		                               : sd->color_info.max_gray;
		sd->color_info.dither_grays =
		    sd->color_info.max_gray < gx_max_color_value
		        ? sd->color_info.max_gray + 1
		        : gx_max_color_value;
		sd->color_info.dither_colors =
		    sd->color_info.num_components < 3
		        ? 0
		        : sd->color_info.dither_grays;
	}
	...
	if(sd->color_info.depth == 3)
		sd->color_info.depth = 4;
	...
	if(error == 0) { /* Allocate new external-arrays */
		...
		for(i = 0; i < sd->color_info.num_components;
		    ++i) { /* Active components */
			int j;
			...
			if((sd->stc.extv[i] != oldstc.extv[i]) &&
			   (sd->stc.extv[i] != defext)) { /* Value-Arrays */
				...
				if(j < i) {
					...
					fp = gs_malloc(
					    sd->memory, sd->stc.sizv[i],
					    sizeof(float), "stc_put_params");
					if(fp != NULL)
						memcpy(fp, sd->stc.extv[i],
						       sd->stc.sizv[i] *
						           sizeof(float));
					else
						code = gs_error_VMerror;
			...
			if((sd->stc.sizc[i] > 1) &&
			   (sd->stc.extc[i] !=
			    oldstc.extc[i])) { /* Code-Arrays */

				for(j = 0; j < i; ++j)
					if((sd->stc.sizc[j] ==
					    sd->stc.sizc[i]) &&
					   (memcmp(sd->stc.extc[j],
					           sd->stc.extc[i],
					           sd->stc.sizc[i] *
					               sizeof(float)) == 0))
						break;
				...
				if(j < i) {
					...
					fp = gs_malloc(
					    sd->memory, sd->stc.sizc[i],
					    sizeof(float), "stc_put_params");
					if(fp != NULL)
						memcpy(fp, sd->stc.extc[i],
						       sd->stc.sizc[i] *
						           sizeof(float));
					else
						code = gs_error_VMerror;
		...
		if((sd->stc.am != NULL) && (sd->stc.am != oldstc.am)) {
			if(sd->color_info.num_components == 1)
				...
				i = 16;
			fp = gs_malloc(sd->memory, i, sizeof(float),
			               "stc_put_params");
			if(fp != NULL)
				memcpy(fp, sd->stc.am, i * sizeof(float));
			else
				code = gs_error_VMerror;
			sd->stc.am = fp;
		}
		...
		if(sd->stc.escp_init.data != oldstc.escp_init.data) {
			...
			if(sd->stc.escp_init.size > 0) {
				...
				if(ip == NULL) {
					...
					memcpy(ip, sd->stc.escp_init.data,
					       sd->stc.escp_init.size);
				}
			}
		...
		if(sd->stc.escp_release.data != oldstc.escp_release.data) {
			...
			if(sd->stc.escp_release.size > 0) {
				...
				if(ip == NULL) {
					...
					memcpy(ip, sd->stc.escp_release.data,
					       sd->stc.escp_release.size);
				}
			}
			...
			if((sd->stc.escp_init.data != NULL) &&
			   (sd->stc.escp_init.data != oldstc.escp_init.data))
				gs_free(
				    ...
				    sd->stc.escp_init.size, 1, "stcolor/init");

			if((sd->stc.escp_release.data != NULL) &&
			   (sd->stc.escp_release.data !=
			    oldstc.escp_release.data))
				gs_free(sd->memory,
				        (byte*)sd->stc.escp_release.data,
				        sd->stc.escp_release.size, 1,
				        "stcolor/release");
	...
	if(error != 0) { /* Undo changes */
		...
		if((oldstc.am != NULL) && (oldstc.am != sd->stc.am)) {
			if(oldcolor.num_components == 1)
				i = 3;
			else if(oldcolor.num_components == 3)
				i = 9;
			else
				i = 16;
			gs_free(sd->memory, oldstc.am, i, sizeof(float),
			        "stc_put_params");
		}
		...
		for(i = 0; i < 4; ++i) {
			...
			if((oldstc.extc[i] != NULL) &&
			   ...
			   (oldstc.extc[i] != defext)) {
				...
				for(j = 0; j < i; ++j)
					if(oldstc.extc[i] == oldstc.extc[j])
						break;

				if(i == j)
					gs_free(sd->memory, oldstc.extc[i],
					        oldstc.sizc[i], sizeof(float),
					        "stc_put_params");
			}
			...
			if((oldstc.extv[i] != NULL) &&
			   ...
			   (oldstc.extv[i] != defext)) {
				...
				for(j = 0; j < i; ++j)
					if(oldstc.extv[i] == oldstc.extv[j])
						break;

				if(i == j)
					gs_free(sd->memory, oldstc.extv[i],
					        oldstc.sizv[i], sizeof(float),
					        "stc_put_params");
			}
		}
		...
		if(sd->is_open) { /* we might need to close it */
			bool doclose = false;
			if((sd->color_info.num_components !=
			    oldcolor.num_components) ||
			...
			for(i = 0; i < sd->color_info.num_components; ++i) {
				if(sd->stc.extv[i] != oldstc.extv[i])
					doclose = true;
				if(sd->stc.extc[i] != oldstc.extc[i])
					doclose = true;
			}
			if(doclose) {
...
stc_gscmyk(stcolor_device* sdev, int npixel, byte* in, byte* buf, byte* out)
{
	...
	if(npixel > 0) { /* npixel >  0 -> scanline-processing       */
		...
		/*    we didn't check for the white-calls above, so this may
		 * cause errors */
	...
	if(npixel < 0) { /* npixel <= 0 -> initialisation            */
		...
		if(((sdev->stc.dither->flags / STC_SCAN) < 1) ||
		   (sdev->stc.dither->bufadd <
		    (1 + 2 * sdev->color_info.num_components)))
			return -3;

		/*
		 * must have STC_CMYK10, STC_DIRECT, but not STC_WHITE
		 */
		if((sdev->stc.dither->flags & STC_CMYK10) == 0)
			return -4;
		if((sdev->stc.dither->flags & STC_DIRECT) == 0)
			return -5;
		if((sdev->stc.dither->flags & STC_WHITE) != 0)
			return -6;
		...
		if((sdev->stc.dither->minmax[0] != 0.0) ||
		   (sdev->stc.dither->minmax[1] != 1023.0))
			return -7;
		/*
		 * initialize buffer
		 */
		...
		while(npixel-- > 0) {
			...
			if(mode == 3) { /* only Black allowed to fire */
				...
				errv[0] = errv[0] < -190 ? -190 : errv[0] < 190
				                                      ? errv[0]
				                                      : 190;
				errv[1] = errv[1] < -190 ? -190 : errv[1] < 190
				                                      ? errv[1]
				                                      : 190;
				errv[2] = errv[2] < -190 ? -190 : errv[2] < 190
				                                      ? errv[2]
				                                      : 190;
