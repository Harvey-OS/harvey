...
{
	gs_transparency_state_t* pts =
	    pgs->transparency_stack; /* known non-0 */
...
gs_state_update_pdf14trans(gs_state* pgs, gs_pdf14trans_params_t* pparams)
{
	...
	/*
	 * Send the PDF 1.4 create compositor action specified by the
	 * parameters.
	 */
	code = send_pdf14trans(pis, dev, &pdf14dev, pparams, pgs->memory);
	/*
	 * If we created a new PDF 1.4 compositor device then we need to install
	 * it
                            ...
                            const gs_rect* pbbox)
{
	...
	if(gs_debug_c('v')) {
		...
		dlprintf5("[v](0x%lx)begin_transparency_group [%g %g %g %g]\n",
		          (uint32_t)pgs, pbbox->p.x, pbbox->p.y, pbbox->q.x,
		          pbbox->q.y);
		if(ptgp->ColorSpace)
			dprintf1("     CS = %s",
			         cs_names[(int)gs_color_space_get_index(
			             ptgp->ColorSpace)]);
		else
			dputs("     (no CS)");
                            ...
                            const gs_pdf14trans_params_t* pparams)
{
	...
	if(gs_debug_c('v')) {
		...
		dlprintf5("[v](0x%lx)begin_transparency_group [%g %g %g %g]\n",
		          (uint32_t)pis, bbox.p.x, bbox.p.y, bbox.q.x,
		          bbox.q.y);
		if(tgp.ColorSpace)
			dprintf1("     CS = %s",
			         cs_names[(int)gs_color_space_get_index(
                           ...
                           const gs_rect* pbbox, bool mask_is_image)
{
	gs_pdf14trans_params_t params = {0};
	const int l =
	    sizeof(params.Background[0]) * ptmp->Background_components;
	int i;

	if_debug8('v', "[v](0x%lx)begin_transparency_mask [%g %g %g %g]\n\
      subtype = %d  Background_components = %d  %s\n",
	          (uint32_t)pgs, pbbox->p.x, pbbox->p.y, pbbox->q.x, pbbox->q.y,
	...
	for(i = 0; i < MASK_TRANSFER_FUNCTION_SIZE; i++) {
		float in =
		    (float)(i * (1.0 / (MASK_TRANSFER_FUNCTION_SIZE - 1)));
		float out;
                           ...
                           const gs_pdf14trans_params_t* pparams)
{
	...
	const int l =
	    sizeof(pparams->Background[0]) * pparams->Background_components;
	...
	if_debug8('v', "[v](0x%lx)begin_transparency_mask [%g %g %g %g]\n\
      subtype = %d  Background_components = %d  %s\n",
	          (uint32_t)pis, pparams->bbox.p.x, pparams->bbox.p.y,
	          pparams->bbox.q.x, pparams->bbox.q.y, (int)tmp.subtype,
	          tmp.Background_components,
	          (tmp.function_is_identity ? "no TR" : "has TR"));
...
int
gs_end_transparency_mask(gs_state* pgs, gs_transparency_channel_selector_t csel)
{
	gs_pdf14trans_params_t params = {0};
