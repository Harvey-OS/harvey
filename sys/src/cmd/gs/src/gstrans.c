...
{
	return (pgs->transparency_stack == 0 ? 0 : pgs->transparency_stack->type);
}
			    ...
			    const gs_pdf14trans_params_t *pparams)
{
	...
	if(gs_debug_c('v')) {
		...
		dlprintf5("[v](0x%lx)begin_transparency_group [%g %g %g %g]\n",
			  (uint32_t)pis, bbox.p.x, bbox.p.y, bbox.q.x, bbox.q.y);
		if(tgp.ColorSpace)
			dprintf1("     CS = %s",
			   ...
			   const gs_rect *pbbox, bool mask_is_image)
{
	...
	if_debug8('v', "[v](0x%lx)begin_transparency_mask [%g %g %g %g]\n\
      subtype = %d  Background_components = %d  %s\n",
		  ...
		  (ptmp->TransferFunction == mask_transfer_identity ? "no TR" : "has TR"));
			   ...
			   const gs_pdf14trans_params_t *pparams)
{
	...
	if(dev_proc(pdev, begin_transparency_mask) != 0)
		return (*dev_proc(pdev, begin_transparency_mask))(pdev, &tmp, &(pparams->bbox), pis, NULL, NULL);
	else
		return 0;
}
