...
write_bmp_depth_header(gx_device_printer* pdev, FILE* file, int depth,
                       const byte* palette /* [4 << depth] */, int raster)
{
	...
	fputc('M', file);
	{
		...
		BMP_ASSIGN_DWORD(fhdr.size, sizeof_bmp_file_header +
		                                sizeof(bmp_info_header) +
		                                quads + bmp_raster * height);
		BMP_ASSIGN_WORD(fhdr.reserved1, 0);
		BMP_ASSIGN_WORD(fhdr.reserved2, 0);
		BMP_ASSIGN_DWORD(fhdr.offBits, sizeof_bmp_file_header +
		                                   sizeof(bmp_info_header) +
		                                   quads);
		if(fwrite((const char*)&fhdr, 1, sizeof(fhdr), file) !=
		   sizeof(fhdr))
...
#undef INCHES_PER_METER
		...
		if(fwrite((const char*)&ihdr, 1, sizeof(ihdr), file) !=
		   sizeof(ihdr))
			return_error(gs_error_ioerror);
	}
	...
	if(depth <= 8) {
		...
		for(i = 0; i != 1 << depth; i++) {
			/* Note that the use of map_color_rgb is deprecated in
			   favor of decode_color. This should work, though,
			   because
