...
#ifdef __WIN32__
	{
		...
		if(!(((HIWORD(version) & 0x8000) != 0) &&
			...
			sprintf(dotversion, "%d.%02d", (int)(gs_revision / 100),
			        (int)(gs_revision % 100));
			sprintf(key, "Software\\%s\\%s", gs_productfamily,
			        dotversion);

			code = gp_getenv_registry(HKEY_CURRENT_USER, key, name,
			                          ptr, plen);
			if(code <= 0)
				return code; /* found it */

			code = gp_getenv_registry(HKEY_LOCAL_MACHINE, key, name,
			                          ptr, plen);
			if(code <= 0)
				return code; /* found it */
		}
...
int
gp_getenv_registry(HKEY hkeyroot, const char* key, const char* name, char* ptr,
                   int* plen)
{
	HKEY hkey;
	...
	if(RegOpenKeyEx(hkeyroot, key, 0, KEY_READ, &hkey) == ERROR_SUCCESS) {
		...
		if(bptr == (char*)NULL)
			bptr =
			    &b; /* Registry API won't return ERROR_MORE_DATA */
		                /* if ptr is NULL */
