...
#define cie_cached_product2int(v, factor, fbits)                                                                 \
	(_cie_product_excess_bits > 0 ? arith_rshift((v)*arith_rshift(factor, _cie_product_excess_bits) +        \
...
#define CIE_CLAMP_INDEX(index) \
	index = (index < 0 ? 0 : index >= gx_cie_cache_size ? gx_cie_cache_size - 1 : index)
