...
/* Copyright (C) 1992, 1995, 1997, 1998, 1999, 2001 Aladdin Enterprises.  All
  rights reserved.
...
#define cie_interpolate_fracs(p, i)                                            \
	((p)[_cix(i)] +                                                        \
	 (frac)arith_rshift((long)((p)[_cix(i) + 1] - (p)[_cix(i)]) * _cif(i), \
	                    _cie_interpolate_bits))
#else
...
#define cie_cached2float(v) float_rshift(v, _cie_fixed_shift)
#define cie_cached2int(v, fbits) arith_rshift(v, _cie_fixed_shift - (fbits))
...
#define _cie_product_excess_bits                                               \
	(_cie_fixed_shift * 2 + gx_cie_log2_cache_size -                       \
	 (arch_sizeof_long * 8 - 1))
#define cie_cached_product2int(v, factor, fbits)                               \
	(_cie_product_excess_bits > 0                                          \
	     ? arith_rshift(                                                   \
	           (v)*arith_rshift(factor, _cie_product_excess_bits) +        \
...
#define cie_cached2int(v, fbits) ((int)float_lshift(v, fbits))
#define cie_cached_product2int(v, factor, fbits)                               \
...
typedef struct cie_vector_cache_params_s {
	...
	cie_interpolation_range_t interpolation_ranges
	    [3]; /* if this cache has an interpolation threshold */
...
typedef struct gx_cie_vector_cache3_s {
	...
	cie_interpolation_range_t
	    interpolation_ranges[3]; /* indexed by output component */
...
#define public_st_cie_common_elements() /* in gscscie.c */                     \
	gs_public_st_suffix_add0_local(                                        \
	    st_cie_common_elements_t, gs_cie_common_elements_t,                \
	    "gs_cie_common_elements_t", cie_common_enum_ptrs,                  \
	    cie_common_reloc_ptrs, st_cie_common)
...
#define private_st_cie_a() /* in gscscie.c */                                  \
	gs_private_st_suffix_add0_local(                                       \
	    st_cie_a, gs_cie_a, "gs_cie_a", cie_common_enum_ptrs,              \
	    cie_common_reloc_ptrs, st_cie_common_elements_t)
...
#define private_st_cie_abc() /* in gscscie.c */                                \
	gs_private_st_suffix_add0_local(                                       \
	    st_cie_abc, gs_cie_abc, "gs_cie_abc", cie_common_enum_ptrs,        \
	    cie_common_reloc_ptrs, st_cie_common_elements_t)
...
#define CIE_CLAMP_INDEX(index)                                                 \
	index = (index < 0 ? 0 : index >= gx_cie_cache_size                    \
	                             ? gx_cie_cache_size - 1                   \
	                             : index)
 ...
 */
extern int gs_cspace_build_CIEA(gs_color_space** ppcspace, void* client_data,
                                gs_memory_t* pmem),
...
extern int gs_cie_defx_set_lookup_table(gs_color_space* pcspace, int* pdims,
