...
/* Copyright (C) 1989, 1995, 1996, 1998, 1999 Aladdin Enterprises.  All rights
  reserved.
...
/* The identity matrix */
private
const gs_matrix gs_identity_matrix = {identity_matrix_body};
...
/* Transform a point. */
int
gs_point_transform(floatp x, floatp y, const gs_matrix* pmat, gs_point* ppt)
{
...
/* Return gs_error_undefinedresult if the matrix is not invertible. */
int
gs_distance_transform_inverse(floatp dx, floatp dy, const gs_matrix* pmat,
                              gs_point* pdpt)
{
	if(is_xxyy(pmat)) {
		if(is_fzero(pmat->xx) || is_fzero(pmat->yy))
...
#define assign_min_max_4(vmin, vmax, v0, v1, v2, v3)                           \
	...
	assign_min_max_4(pbox->p.x, pbox->q.x, pts[0].x, pts[1].x, pts[2].x,
	                 pts[3].x);
	assign_min_max_4(pbox->p.y, pbox->q.y, pts[0].y, pts[1].y, pts[2].y,
	                 pts[3].y);
...
bbox_transform_either_only(const gs_rect* pbox_in, const gs_matrix* pmat,
                           gs_point pts[4],
                           int (*point_xform)(floatp, floatp, const gs_matrix*,
                                              gs_point*))
{
	int code;

	if((code = (*point_xform)(pbox_in->p.x, pbox_in->p.y, pmat, &pts[0])) <
	       0 ||
	   (code = (*point_xform)(pbox_in->p.x, pbox_in->q.y, pmat, &pts[1])) <
	       0 ||
	   (code = (*point_xform)(pbox_in->q.x, pbox_in->p.y, pmat, &pts[2])) <
	       0 ||
	   (code = (*point_xform)(pbox_in->q.x, pbox_in->q.y, pmat, &pts[3])) <
	       0)
		DO_NOTHING;
	return code;
}
                                         ...
                                         gs_point*))
{
	...
	if((code = bbox_transform_either_only(pbox_in, pmat, pts,
	                                      point_xform)) < 0)
		return code;
...
gs_matrix_fixed_from_matrix(gs_matrix_fixed* pfmat, const gs_matrix* pmat)
{
	*(gs_matrix*)pfmat = *pmat;
	if(f_fits_in_fixed(pmat->tx) && f_fits_in_fixed(pmat->ty)) {
		pfmat->tx =
		    fixed2float(pfmat->tx_fixed = float2fixed(pmat->tx));
		pfmat->ty =
		    fixed2float(pfmat->ty_fixed = float2fixed(pmat->ty));
...
int
gs_point_transform2fixed(const gs_matrix_fixed* pmat, floatp x, floatp y,
                         gs_fixed_point* ppt)
{
	fixed px, py, t;
	double xtemp, ytemp;
	...
	if(!is_fzero(pmat->xy)) { /* Hope for 90 degree rotation */
		if((code = CHECK_DFMUL2FIXED_VARS(px, y, pmat->yx, xtemp)) <
		       0 ||
		...
		if(!is_fzero(pmat->xx)) {
			if((code = CHECK_DFMUL2FIXED_VARS(t, x, pmat->xx,
			                                  xtemp)) < 0)
				return code;
		...
		if(!is_fzero(pmat->yy)) {
			if((code = CHECK_DFMUL2FIXED_VARS(t, y, pmat->yy,
			                                  ytemp)) < 0)
				return code;
		...
		if((code = CHECK_DFMUL2FIXED_VARS(px, x, pmat->xx, xtemp)) <
		       0 ||
		...
		FINISH_DFMUL2FIXED_VARS(py, ytemp);
		if(!is_fzero(pmat->yx)) {
			if((code = CHECK_DFMUL2FIXED_VARS(t, y, pmat->yx,
			                                  ytemp)) < 0)
				return code;
			FINISH_DFMUL2FIXED_VARS(t, ytemp);
			if((code = CHECK_SET_FIXED_SUM(px, px, t)) < 0)
...
int
gs_point_transform2fixed_rounding(const gs_matrix_fixed* pmat, floatp x,
                                  floatp y, gs_fixed_point* ppt)
{
	gs_point fpt;
...
int
gs_distance_transform2fixed(const gs_matrix_fixed* pmat, floatp dx, floatp dy,
                            gs_fixed_point* ppt)
{
	fixed px, py, t;
	double xtemp, ytemp;
...
sget_matrix(stream* s, gs_matrix* pmat)
{
	...
	for(; i < 6; ++i, b <<= 1)
		if(b & 0x80) {
			status = sgets(s, (byte*)&coeff[i], sizeof(coeff[0]),
			               &nread);
			if(status < 0 && status != EOFC)
				return_error(gs_error_ioerror);
		} else
