...
ENUM_PTRS_END
private
RELOC_PTRS_WITH(gs_type1_state_reloc_ptrs, gs_type1_state* pcis)
{
	int i;
	...
	for(i = 0; i < pcis->ips_count; i++) {
		...
		RELOC_USING(st_glyph_data, &ipsp->cs_data,
		            sizeof(ipsp->cs_data));
...
gs_type1_interp_init(register gs_type1_state* pcis, gs_imager_state* pis,
                     ...
                     const gs_log2_scale_point* psubpixels,
                     bool no_grid_fitting, int paint_type, gs_font_type1* pfont)
{
	...
	const gs_log2_scale_point* plog2_subpixels =
	    (FORCE_HINTS_TO_BIG_PIXELS
	         ? (psubpixels != NULL ? psubpixels : plog2_scale)
	         : &no_scale);
...
gs_type1_sbw(gs_type1_state* pcis, fixed lsbx, fixed lsby, fixed wx, fixed wy)
{
	...
	if(!pcis->width_set)
		pcis->width.x = wx, pcis->width.y = wy, pcis->width_set = true;
	if_debug4('1', "[1]sb=(%g,%g) w=(%g,%g)\n", fixed2float(pcis->lsb.x),
	          fixed2float(pcis->lsb.y), fixed2float(pcis->width.x),
...
gs_type1_blend(gs_type1_state* pcis, fixed* csp, int num_results)
{
	...
	for(j = 0; j < num_results; j++, base++, deltas += k1)
		for(i = 1; i <= k1; i++)
			*base +=
			    (fixed)(deltas[i] * pdata->WeightVector.values[i]);
              ...
              ip_state_t* ipsp)
{
	...
	code = pfont->data.procs.seac_data(pfont, fixed2int_var(cstack[2]),
	                                   NULL, &gstr, &bgdata);
	if(code < 0)
		return code;
...
gs_type1_endchar(gs_type1_state* pcis)
{
	...
	if(pcis->seac_accent >=
	   0) { /* We just finished the base character of a seac. */
		/* Do the accent. */
		gs_font_type1* pfont = pcis->pfont;
		...
		code = pfont->data.procs.seac_data(pfont, achar, NULL, &gstr,
		                                   &agdata);
		if(code == gs_error_undefined) {
			/*
			 * The font is missing the accent's CharString (due to
			 * bad subsetting).  Just end drawing here without
			 * error.
			 * This is like Acrobat Reader behaves.
			 */
			char buf0[gs_font_name_max + 1], buf1[30];
			int l0 =
			    min(pcis->pfont->font_name.size, sizeof(buf0) - 1);
			...
			buf1[l1] = 0;
			eprintf2("The font '%s' misses the glyph '%s' . "
			         "Continue skipping the glyph.",
			         buf0, buf1);
			return 0;
		}
		if(code < 0)
 ...
 */
int
gs_type1_piece_codes(/*const*/ gs_font_type1* pfont, const gs_glyph_data_t* pgd,
                     gs_char* chars)
{
                      ...
                      int members, gs_glyph_info_t* info)
{
	...
	if(code <= 0)         /* no seac, possibly error */
		return code;
	bcode =
	    pdata->procs.seac_data(pfont, chars[0], &pieces[0], &gstr, NULL);
	acode =
	    pdata->procs.seac_data(pfont, chars[1], &pieces[1], &gstr, NULL);
                    ...
                    int members, gs_glyph_info_t* info)
{
	...
	int default_members =
	    members &
	    ~(piece_members | GLYPH_INFO_WIDTHS | GLYPH_INFO_VVECTOR0 |
	      GLYPH_INFO_VVECTOR1 | GLYPH_INFO_OUTLINE_WIDTHS);
	int code = 0;
	...
	if(width_members) {
		...
		gis.flatness = 0;
		code =
		    gs_type1_interp_init(&cis, &gis, NULL /* no path needed */,
