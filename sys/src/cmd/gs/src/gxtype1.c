	      ...
	      ip_state_t *ipsp)
{
	...
	code = pfont->data.procs.seac_data(pfont, fixed2int_var(cstack[2]), NULL, &gstr, &bgdata);
	if(code < 0)
		return code;
...
#define cnext                      \
	...
	goto top
		...
		case c_callsubr:
			...
			code = pdata->procs.subr_data(pfont, c, false, &ipsp[1].cs_data);
			if(code < 0)
				return_error(code);
			--csp;
		    ...
		    int members, gs_glyph_info_t *info)
{
	...
	int default_members = members & ~(piece_members | GLYPH_INFO_WIDTHS | GLYPH_INFO_VVECTOR0 | GLYPH_INFO_VVECTOR1 | GLYPH_INFO_OUTLINE_WIDTHS);
	int code = 0;
