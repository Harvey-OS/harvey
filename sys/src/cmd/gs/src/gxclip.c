...
struct stats_clip_s {
	int32_t
	    loops,
	    out, in_y, in, in1, down, up, x, no_x;
} stats_clip;
		    ...
		    clip_callback_data_t *pccd)
{
	...
	if(rptr == 0 || (yc = rptr->ymin) >= ye) {
		...
		if(rdev->list.count > 1)
			rdev->current =
			    (rptr != 0 ? rptr : y >= rdev->current->ymax ? rdev->list.tail : rdev->list.head);
		return 0;
	}
	rdev->current = rptr;
	if(yc < y)
...
{
	return (*dev_proc(pccd->tdev, fill_rectangle))(pccd->tdev, xc, yc, xec - xc, yec - yc, pccd->color[0]);
}
private
int
	...
	if((y >= rptr->ymin && ye <= rptr->ymax) ||
	   ((rptr = rptr->next) != 0 &&
		...
		if(x >= rptr->xmin && xe <= rptr->xmax) {
			...
			if(xe > rptr->xmax)
				xe = rptr->xmax;
			return (x >= xe ? 0 : dev_proc(tdev, fill_rectangle)(tdev, x, y, xe - x, h, color));
		}
	}
	ccdata.tdev = tdev;
...
{
	return (*dev_proc(pccd->tdev, copy_mono))(pccd->tdev, pccd->data + (yc - pccd->y) * pccd->raster,
	       ...
	       gx_color_index color0, gx_color_index color1)
{
	...
	if(y >= rptr->ymin && ye <= rptr->ymax) {
		...
		if(x >= rptr->xmin && xe <= rptr->xmax) {
			INCR(in);
			return dev_proc(tdev, copy_mono)(tdev, data, sourcex, raster, id, x, y, w, h, color0, color1);
		}
	}
	ccdata.tdev = tdev;
...
{
	return (*dev_proc(pccd->tdev, copy_color))(pccd->tdev, pccd->data + (yc - pccd->y) * pccd->raster,
						   pccd->sourcex + xc - pccd->x, pccd->raster, gx_no_bitmap_id,
						   xc, yc, xec - xc, yec - yc);
}
private
...
{
	return (*dev_proc(pccd->tdev, copy_alpha))(pccd->tdev, pccd->data + (yc - pccd->y) * pccd->raster,
						   pccd->sourcex + xc - pccd->x, pccd->raster, gx_no_bitmap_id,
...
{
	return (*dev_proc(pccd->tdev, fill_mask))(pccd->tdev, pccd->data + (yc - pccd->y) * pccd->raster,
						  pccd->sourcex + xc - pccd->x, pccd->raster, gx_no_bitmap_id,
...
{
	return (*dev_proc(pccd->tdev, strip_tile_rectangle))(pccd->tdev, pccd->tiles, xc, yc, xec - xc, yec - yc,
...
{
	return (*dev_proc(pccd->tdev, strip_copy_rop))(pccd->tdev, pccd->data + (yc - pccd->y) * pccd->raster,
						       pccd->sourcex + xc - pccd->x, pccd->raster, gx_no_bitmap_id,
			...
			gs_get_bits_params_t *params, gs_int_rect **unread)
{
	...
	code = (*dev_proc(tdev, get_bits_rectangle))(tdev, &rect, params, unread);
	if(code > 0) {
