...
/* Forward references */
private
int zPaintProc(const gs_client_color*, gs_state*);
private
int pattern_paint_prepare(i_ctx_t*);
private
int pattern_paint_finish(i_ctx_t*);
...
{
	gstate_set_pattern_cache(
	    igs,
	    gx_pattern_alloc_cache(imemory_system, gx_pat_cache_default_tiles(),
...
zsetpatternspace(i_ctx_t* i_ctx_p)
{
	...
	case 2:
		...
		if(cs_num_components(&cs) < 0) /* i.e., Pattern space */
			return_error(e_rangecheck);
		{
			/* We can't count on C compilers to recognize the
			 * aliasing */
			/* that would be involved in a direct assignment */
			/* cs.params.pattern.base_space = *(gs_paint_color_space
			 * *)&cs; */
			/* At least MSVC7 chocks with it. */
			memmove(&cs.params.pattern.base_space, &cs,
	...
	make_null(
	    &istate->pattern); /* PLRM: initial color value is a null object */
	pop(1);
	return (ref_stack_count(&e_stack) == edepth
	            ? 0
...
private
int pattern_paint_cleanup(i_ctx_t*);
private
int
zPaintProc(const gs_client_color* pcc, gs_state* pgs)
{
	/* Just schedule a call on the real PaintProc. */
	r_ptr(&gs_int_gstate(pgs)->remap_color_info, int_remap_color_info_t)
	    ->proc = pattern_paint_prepare;
...
pattern_paint_prepare(i_ctx_t* i_ctx_p)
{
	...
	if(pgs->have_pattern_streams) {
		code = dev_proc(cdev, pattern_manage)(
		    cdev, pinst->id, pinst, pattern_manage__can_accum);
		if(code < 0)
			return code;
	...
	if(internal_accum) {
		...
		code = gx_pattern_cache_add_dummy_entry(
		    (gs_imager_state*)igs, pinst, cdev->color_info.depth);
		if(code < 0)
			return code;
	}
	code = gs_gsave(pgs);
	...
	if(internal_accum)
		...
		code = dev_proc(cdev, pattern_manage)(
		    cdev, pinst->id, pinst, pattern_manage__start_accum);
		if(code < 0) {
			gs_grestore(pgs);
			return code;
		}
...
pattern_paint_cleanup(i_ctx_t* i_ctx_p)
{
	...
	if(pdev == NULL) {
		gx_device* cdev = gs_currentdevice_inline(igs);
		int code1 = dev_proc(cdev, pattern_manage)(
		    cdev, gx_no_bitmap_id, NULL, pattern_manage__finish_accum);
