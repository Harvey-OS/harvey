...
#define clamp_coord(xy)                                                        \
	ppt->xy = (xy > fixed2float(max_coord_fixed)                           \
	               ? max_coord_fixed                                       \
...
gs_currentpoint(gs_state* pgs, gs_point* ppt)
{
	...
	return gs_itransform(pgs, pgs->current_point.x, pgs->current_point.y,
	                     ppt);
}

private
inline int
gs_point_transform_compat(floatp x, floatp y, const gs_matrix_fixed* m,
                          gs_point* pt)
{
...
private
inline int
gs_distance_transform_compat(floatp x, floatp y, const gs_matrix_fixed* m,
                             gs_point* pt)
{
#if !PRECISE_CURRENTPOINT
	gs_fixed_point p;
...
{
	if(!f_fits_in_bits(x, fixed_int_bits) ||
	   !f_fits_in_bits(y, fixed_int_bits)) {
...
private
inline int
gs_curveto_aux(gs_state* pgs, floatp x1, floatp y1, floatp x2, floatp y2,
               floatp x3, floatp y3)
{
...
int
gs_curveto(gs_state* pgs, floatp x1, floatp y1, floatp x2, floatp y2, floatp x3,
           floatp y3)
{
...
int
gs_rcurveto(gs_state* pgs, floatp dx1, floatp dy1, floatp dx2, floatp dy2,
            floatp dx3, floatp dy3)
{
	...
	if(code < 0)
		return code;
	/* fixme : check in range. */
	return gs_curveto_aux(
	    pgs, dd1.x + pgs->current_point.x, dd1.y + pgs->current_point.y,
...
{
	gs_id view_clip_id = (pgs->view_clip == 0 || pgs->view_clip->rule == 0
	                          ? gs_no_id
	                          : pgs->view_clip->id);
	...
	if(view_clip_id == gs_no_id) {
		...
		if(rect_within(vcbox, cbox)) {
			...
			if(pgs->effective_clip_shared) {
				npath = gx_cpath_alloc(
				    pgs->memory, "gx_effective_clip_path");
				if(npath == 0)
...
gx_default_clip_box(const gs_state* pgs, gs_fixed_rect* pbox)
{
	...
	if(dev->ImagingBBox_set) { /* Use the ImagingBBox, relative to default
	                              user space. */
