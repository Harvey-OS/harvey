...
#define clamp_coord(xy) \
	ppt->xy = (xy > fixed2float(max_coord_fixed) ? max_coord_fixed : xy < fixed2float(min_coord_fixed) ? min_coord_fixed : float2fixed(xy))
	clamp_coord(x);
	clamp_coord(y);
#undef clamp_coord
}
...
{
	gs_id view_clip_id =
	    (pgs->view_clip == 0 || pgs->view_clip->rule == 0 ? gs_no_id : pgs->view_clip->id);
