...
#define dctp(key, type, stype, memb)                                           \
	{                                                                      \
		key, type, offset_of(stype, memb)                              \
	}
private
const gs_param_item_t s_DCT_param_items[] = {
    ...
    gs_param_item_end};
private
const gs_param_item_t jsd_param_items[] = {
                   ...
                   floatp QFactor, gs_memory_t* mem)
{
	...
	for(i = 0; i < count; ++i) {
		...
		data[i] = (val < 1 ? (code = 1) : val > 255 ? (code = 255)
		                                            : (byte)val);
	}
	pstr->data = data;
	pstr->size = count;
...
s_DCT_get_quantization_tables(gs_param_list* plist,
                              ...
                              const stream_DCT_state* defaults, bool is_encode)
{
	...
	if(is_encode) {
		...
		if(defaults) {
			default_comp_info =
			    defaults->data.compress->cinfo.comp_info;
			default_table_ptrs =
			    defaults->data.compress->cinfo.quant_tbl_ptrs;
		}
	} else {
	...
	if(defaults) {
		...
		for(i = 0; i < num_in_tables; ++i) {
			...
			JQUANT_TBL* default_tbl =
			    (default_comp_info == 0 || default_table_ptrs == 0
			         ? 0
			         : default_table_ptrs[default_comp_info[i]
			                                  .quant_tbl_no]);
	...
	code = param_begin_write_collection(plist, "QuantTables", &quant_tables,
	...
	for(i = 0; i < num_in_tables; ++i) {
		...
		sprintf(key, "%d", i);
		if(QFactor == 1.0) {
			code = quant_param_string(
			    &str, DCTSIZE2,
			    table_ptrs[comp_info[i].quant_tbl_no]->quantval,
			    QFactor, mem);
			switch(code) {
			case 0:
				code = param_write_string(quant_tables.list,
				                          key, &str);
				if(code < 0)
		...
		code = quant_param_array(
		    &fa, DCTSIZE2,
		    table_ptrs[comp_info[i].quant_tbl_no]->quantval, QFactor,
		    mem);
		if(code < 0)
			return code; /* should dealloc */
...
private
int
pack_huff_table(gs_param_string* pstr, const JHUFF_TBL* table, gs_memory_t* mem)
{
	int total;
	int i;
	byte* data;
...
int
s_DCT_get_huffman_tables(gs_param_list* plist, const stream_DCT_state* pdct,
                         const stream_DCT_state* defaults, bool is_encode)
{
	gs_memory_t* mem = pdct->memory;
	...
	/****** byte_array IS WRONG ******/
	huff_data = (gs_param_string*)gs_alloc_byte_array(
	    mem, num_in_tables, sizeof(gs_param_string), "get huffman tables");
	if(huff_data == 0)
		return_error(gs_error_VMerror);
	for(i = 0; i < num_in_tables; i += 2) {
		if((code = pack_huff_table(huff_data + i, ac_table_ptrs[i >> 1],
		                           mem)) < 0 ||
		   (code = pack_huff_table(huff_data + i + 1,
		                           dc_table_ptrs[i >> 1], mem)))
			break;
	}
	if(code < 0)
		return code;
...
{
	int code = gs_param_write_items(plist, ss, defaults, s_DCT_param_items);

	if(code >= 0)
		code = gs_param_write_items(
		    plist, ss->data.common,
		    (defaults ? defaults->data.common : NULL), jsd_param_items);
	return code;
}
 ...
 */
int
s_DCT_byte_params(gs_param_list* plist, gs_param_name key, int start, int count,
                  UINT8* pvals)
{
	int i;
...
private
int
quant_params(gs_param_list* plist, gs_param_name key, int count, UINT16* pvals,
             floatp QFactor)
{
	int i;
	gs_param_string bytes;
	...
	default: /* might be a float array */
		...
		if(!code) {
			...
			for(i = 0; i < count; ++i) {
				...
				pvals[jpeg_order(i)] = (UINT16)(
				    v < 1 ? 1 : v > 255 ? 255 : v + 0.5);
			}
		}
	}
	if(code < 0)
...
{
	int code = gs_param_read_items(plist, pdct, s_DCT_param_items);
                              ...
                              bool is_encode)
{
	...
	for(i = 0; i < num_in_tables; ++i) {
		...
		for(j = 0; j < num_out_tables; j++) {
			if(!memcmp(table_ptrs[j]->quantval, values,
			           sizeof(values)))
				break;
		}
		if(comp_info != NULL)
...
private
int
find_huff_values(JHUFF_TBL** table_ptrs, int num_tables, const UINT8 counts[16],
                 const UINT8* values, int codes_size)
{
	int j;
                         ...
                         bool is_encode)
{
	...
	switch((code = param_begin_read_dict(plist, "HuffTables", &huff_tables,
	                                     true))) {
	case 1:
		return 0;
	default:
	...
	if(is_encode) {
		...
		if(pdct->data.common->Relax)
			max_tables =
			    max(pdct->data.compress->cinfo.input_components, 2);
	} else {
	...
	for(i = 0; i < num_in_tables; ++i) {
		...
		memcpy(this_table->huffval, values,
		       codes_size * sizeof(values[0]));
	}
