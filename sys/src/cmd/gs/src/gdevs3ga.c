...
private
dev_proc_open_device(s3_open);
private
dev_proc_fill_rectangle(s3_fill_rectangle);
private
dev_proc_copy_mono(s3_copy_mono);
private
const gx_device_procs s3_procs = {
    s3_open, NULL, /* get_initial_matrix */
    NULL,          /* sync_output */
    NULL,          /* output_page */
    svga_close, svga_map_rgb_color, svga_map_color_rgb, s3_fill_rectangle,
    NULL, /* tile_rectangle */
...
private
gx_bitmap_id cache_ids[cache_capacity];
...
#define s3_wait_fifo() while(inport(s3_status) & 0xff)
/* Load the parameters for a rectangle operation. */
#define out_s3_rect(x, y, w, h)                                                \
	(outport(s3_x_pos, x), outport(s3_y_pos, y), outport(s3_width, (w)-1), \
	 outport(s3_height, (h)-1))

private
int
s3_open(gx_device* dev)
{
	static const mode_info mode_table[] = {{640, 480, 0x201},
	                                       {800, 600, 0x203},
	                                       {1024, 768, 0x205},
	                                       {-1, -1, -1}};
...
private
int
s3_copy_mono(gx_device* dev, const byte* base, int sourcex, int raster,
             gx_bitmap_id id, int x, int y, int w, int h, gx_color_index czero,
             gx_color_index cone)
{
	int sbit;
	const byte* sptr;
	...
	if(id != gx_no_bitmap_id && w <= cell_width - 7 && h <= cell_height) {
		cache_index = (int)(id & (cache_capacity - 1));
		cache_x = ((cache_index & ((1 << cache_x_bits) - 1))
		           << log2_cell_width) +
		          7;
		cache_y =
		    ((cache_index >> cache_x_bits) << log2_cell_height) + 768;
		...
		cache_index = -1;
		if(lmask !=
		   0xff) { /* The hardware won't do the masking for us. */
			if(czero != gx_no_color_index) {
				if(cone != gx_no_color_index) {
					s3_fill_rectangle(dev, x, y, w, h,
					                  czero);
