...
/* $Id: gdevstc2.c,v 1.5 2002/10/07 08:28:56 ghostgum Exp $*/
...
static const byte rgbvals[8] = {0, RED, GREEN, RED | GREEN, BLUE, BLUE | RED,
                                BLUE | GREEN, BLUE | RED | GREEN};

static const byte cmykvals[16] = {
    0, CYAN, MAGENTA, CYAN | MAGENTA, YELLOW, YELLOW | CYAN, YELLOW | MAGENTA,
    BLACK, BLACK, BLACK, BLACK, BLACK, BLACK, BLACK, BLACK, BLACK};

static const byte* const pixelconversion[5] = {NULL, grayvals, NULL, rgbvals,
                                               cmykvals};
...
stc_fs(stcolor_device* sdev, int npixel, byte* bin, byte* bbuf, byte* out)
{
	...
	if(npixel > 0) { /* npixel >  0 -> scanline-processing       */
		...
		for(p = pstart; p != pstop; p += pstep) { /* loop over pixels */
			...
			for(c = 0; c < sdev->color_info.num_components;
			    c++) {          /* comp */
				int32_t cv; /* component value */

				cv = in[p + c] + errv[p + c] + errc[c] -
				     ((errc[c] + 4) >> 3);
				if(cv > threshold) {
					pixel |= 1 << c;
					cv -= spotsize;
		...
		if(sdev->stc.flags & STCDFLAG0) {
			...
			for(i = 0; i < sdev->color_info.num_components; ++i)
				buf[i + 3] =
				    (int32_t)(0.25000 * scale *
				              (buf[i + 3] - rand_max / 2));

			for(; i < i2do;
			    ++i) /* includes 2 additional pixels ! */
				buf[i + 3] =
				    (int32_t)(0.28125 * scale *
				              (buf[i + 3] - rand_max / 2));
		}
...
stc_fscmyk(stcolor_device* sdev, int npixel, byte* bin, byte* bbuf, byte* out)
{
	...
	if(npixel > 0) { /* npixel >  0 -> scanline-processing       */
		...
		for(p = pstart; p != pstop; p += pstep) { /* loop over pixels */
			...
			/*
			 * Black is treated first, with conventional
			 * Floyd-Steinberg
			 */
			k = in[p + 3];
			...
			if(pixel) {
				...
				/* -------- firing of black causes all colors to
				 * fire too */
				...
				/* -------- if black did not fire, only colors
				 * w. larger values may fire */
		...
		if(sdev->stc.flags & STCDFLAG1) {
			buf[2] = (int32_t)(
			    (sdev->stc.extv[0][sdev->stc.sizv[0] - 1] -
			     sdev->stc.extv[0][0]) *
			        scale / 2.0 +
			    offset);
		} else {
