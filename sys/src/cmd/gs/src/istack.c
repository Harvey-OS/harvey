...
/* Forward references */
private
void init_block(ref_stack_t* pstack, const ref* pblock_array, uint used);
private
int ref_stack_push_block(ref_stack_t* pstack, uint keep, uint add);
...
case 1:
return ENUM_OBJ(sptr->params);
ENUM_PTRS_END
private
RELOC_PTRS_WITH(ref_stack_reloc_ptrs, ref_stack_t* sptr)
{
...
#define RELOC_P(p) sptr->p = (ref*)((ref_packed*)sptr->p - reloc);
	RELOC_P(p);
	RELOC_P(bot);
	RELOC_P(top);
#undef RELOC_P
...
/* Initialize a stack. */
int
ref_stack_init(ref_stack_t* pstack, const ref* pblock_array, uint bot_guard,
               uint top_guard, const ref* pguard_value, gs_ref_memory_t* mem,
...
ref_stack_set_margin(ref_stack_t* pstack, uint margin)
{
	...
	if(margin <= pstack->margin) {
		...
		if(pstack->top - pstack->p < margin) {
			...
			int code =
			    ref_stack_push_block(pstack, keep, used - keep);
	...
	if(space != avm_local) {
		...
		do {
			...
			if(size <= pass)
				...
				left -= size;
				code =
				    refs_check_space(ptr - size, size, space);
				if(code < 0)
					return code;
				if(left == 0)
					break;
...
int
ref_stack_store(const ref_stack_t* pstack, ref* parray, uint count, uint skip,
                int age, bool check, gs_dual_memory_t* idmemory,
                client_name_t cname)
{
	uint left, pass;
	ref* to;
...
ref_stack_pop_block(ref_stack_t* pstack)
{
	...
	ref_stack_block* pnext = (ref_stack_block*)pcur->next.value.refs;
	uint used;
	ref* body;
	ref next;
	...
	if(used + count > pstack->body_size) {
		 ...
		 * elements available, move up the used part of the top block,
		 * and
		...
		/*
		   * The contents of the two blocks will fit into a single
		 * block.
...
ref_stack_push(ref_stack_t* pstack, uint count)
{
	...
	for(; (added = pstack->top - pstack->p) < needed; needed -= added) {
		...
		pstack->p = pstack->top;
		code = ref_stack_push_block(
		    pstack, (pstack->top - pstack->bot + 1) / 3, added);
		if(code < 0) {
			/* Back out. */
...
ref_stack_push_block(ref_stack_t* pstack, uint keep, uint add)
{
	...
	if(pstack->extension_used + (pstack->top - pstack->bot) + add >=
	   ...
	   !params->allow_expansion)
		return_error(params->overflow_error);
	code = gs_alloc_ref_array(pstack->memory, &next, 0, params->block_size,
	                          "ref_stack_push_block");
	if(code < 0)
		return code;
...
{
	ref_stack_block* block = prse->block =
	    (ref_stack_block*)prse->block->next.value.refs;
...
init_block(ref_stack_t* pstack, const ref* psb, uint used)
{
	...
	for(i = params->bot_guard, p = brefs + stack_block_refs; i != 0;
	    i--, p++)
	...
	}
	{
		...
		pblock->used = *psb;
		pblock->used.value.refs =
		    brefs + stack_block_refs + params->bot_guard;
