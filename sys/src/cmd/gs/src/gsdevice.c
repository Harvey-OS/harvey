...
ENUM_PTRS_END
private
RELOC_PTRS_WITH(device_forward_reloc_ptrs, gx_device_forward* fdev)
{
                 ...
                 int* plines_copied, uint* pbytes_copied)
{
	...
	for(i = 0; i < count; i++, dest += line_size) {
		int code =
		    (*dev_proc(dev, get_bits))(dev, start_y + i, dest, NULL);
...
private
void
gx_device_make_struct_type(gs_memory_struct_type_t* st, const gx_device* dev)
{
               ...
               gs_memory_t* mem)
{
	...
	if(dev->stype_is_dynamic) {
		 ...
		 */
		a_std = (gs_memory_struct_type_t*)gs_alloc_bytes_immovable(
		    mem->non_gc_memory, sizeof(*std), "gs_copydevice(stype)");
		if(!a_std)
	...
	if(code < 0) {
		...
		if(a_std)
			gs_free_object(dev->memory->non_gc_memory, a_std,
			               "gs_copydevice(stype)");
		return code;
	}
	*pnew_dev = new_dev;
...
/* Make a null device. */
void
gs_make_null_device(gx_device_null* dev_null, gx_device* dev, gs_memory_t* mem)
{
	...
	if(dev) {
		...
		*/
		...
		set_dev_proc(dn, get_color_mapping_procs,
		             gx_forward_get_color_mapping_procs);
		set_dev_proc(dn, get_color_comp_index,
...
/* Is a null device ? */
bool
gs_is_null_device(gx_device* dev)
{
...
gs_nulldevice(gs_state* pgs)
{
	if(pgs->device == 0 || !gx_device_is_null(pgs->device)) {
		...
		int code = gs_copydevice(
		    &ndev, (const gx_device*)&gs_null_device, pgs->memory);
...
int
gx_device_adjust_resolution(gx_device* dev, int actual_width, int actual_height,
                            int fit)
{
	double width_ratio = (double)actual_width / dev->width;
	double height_ratio = (double)actual_height / dev->height;
	double ratio = (fit ? min(width_ratio, height_ratio)
...
void
gx_device_set_media_size(gx_device* dev, floatp media_width,
                         floatp media_height)
{
                          ...
                          const char* fname, uint fnlen)
{
	...
	*pfmt = 0;
	...
	if(code < 0) {
		if(fname[0] == '%') {
			/* not a recognized iodev -- may be a leading format
			 * descriptor */
			pfn->len = fnlen;
			pfn->fname = fname;
...
int
gx_device_open_output_file(const gx_device* dev, char* fname, bool binary,
                           bool positionable, FILE** pfile)
{
	...
	int code =
	    gx_parse_output_file_name(&parsed, &fmt, fname, strlen(fname));
	...
	if(positionable || (parsed.iodev && parsed.iodev != iodev_default)) {
		...
		if(positionable)
			strcat(fmode, "+");
		code = parsed.iodev->procs.fopen(parsed.iodev, parsed.fname,
		                                 fmode, pfile, NULL, 0);
		if(code)
...
int
gx_device_close_output_file(const gx_device* dev, const char* fname, FILE* file)
{
	gs_parsed_file_name_t parsed;
	const char* fmt;
	int code =
	...
	if(parsed.iodev) {
		if(!strcmp(parsed.iodev->dname, "%stdout%"))
			return 0;
		/* NOTE: fname is unsubstituted if the name has any %nnd
		 * formats. */
