...
#define private_st_cached_fm_pair() /* in gxccman.c */                         \
	gs_private_st_ptrs5(st_cached_fm_pair, cached_fm_pair,                 \
	                    "cached_fm_pair", fm_pair_enum_ptrs,               \
	                    fm_pair_reloc_ptrs, font, UID.xvalues, xfont, ttf, \
	                    ttr)
#define private_st_cached_fm_pair_elt() /* in gxccman.c */                     \
	gs_private_st_element(st_cached_fm_pair_element, cached_fm_pair,       \
	                      "cached_fm_pair[]", fm_pair_element_enum_ptrs,   \
	                      fm_pair_element_reloc_ptrs, st_cached_fm_pair)
/* If font == 0 and UID is invalid, this is a free entry. */
#define fm_pair_is_free(pair) ((pair)->font == 0 && !uid_is_valid(&(pair)->UID))
#define fm_pair_set_free(pair) ((pair)->font = 0, uid_set_invalid(&(pair)->UID))
...
#define cc_set_pair(cc, p) ((cc)->pair_index = ((cc)->pair = (p))->index)
...
struct gs_font_dir_s {
	...
	int (*global_glyph_code)(const gs_memory_t* mem, gs_const_string* gstr,
	                         gs_glyph* pglyph);
};
...
void gx_purge_selected_cached_chars(gs_font_dir*, bool (*)(const gs_memory_t*,
                                                           cached_char*, void*),
                                    void*);
void gx_compute_char_matrix(const gs_matrix* char_tm,
...
void gx_compute_ccache_key(gs_font* pfont, const gs_matrix* char_tm,
                           ...
                           bool design_grid, float* mxx, float* mxy, float* myx,
                           float* myy);
