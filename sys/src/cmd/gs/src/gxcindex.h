...
/* Copyright (C) 1995, 1996, 1997, 1999 Aladdin Enterprises.  All rights
  reserved.
 ...
 * giving error messages.  I hope that was fixed when the value of
 * gx_no_color_index
...
#define LINE_ACCUM_STORE(bpp) sample_store_flush(l_dptr, l_dbit, bpp, l_dbyte)
/*
...
#define LINE_ACCUM_COPY(dev, line, bpp, xo, xe, raster, y)                     \
	if((xe) > l_xprev) {                                                   \
		...
		code = (*dev_proc(dev, copy_color))(                           \
		    dev, line, l_xprev - (xo), raster, gx_no_bitmap_id,        \
		    l_xprev, y, (xe)-l_xprev, 1);                              \
		if(code < 0)                                                   \
			return code;                                           \
	}
