...
#define pbm_prn_device(procs, dev_name, magic, is_raw, num_comp, depth,        \
                       max_gray, max_rgb, optimize, x_dpi, y_dpi, print_page)  \
	{                                                                      \
		prn_device_body(gx_device_pbm, procs, dev_name,                \
		                DEFAULT_WIDTH_10THS, DEFAULT_HEIGHT_10THS,     \
		                x_dpi, y_dpi, 0, 0, 0, 0, num_comp, depth,     \
		                max_gray, max_rgb, max_gray + 1, max_rgb + 1,  \
		                print_page),                                   \
		    magic, {0}, is_raw, optimize, 0, 0, 0                      \
	}

/* For all but PBM, we need our own color mapping and alpha procedures. */
private
dev_proc_map_rgb_color(pgm_map_rgb_color);
private
dev_proc_map_rgb_color(ppm_map_rgb_color);
private
dev_proc_map_color_rgb(pgm_map_color_rgb);
private
dev_proc_map_color_rgb(ppm_map_color_rgb);
private
dev_proc_map_cmyk_color(pkm_map_cmyk_color);
private
dev_proc_map_color_rgb(pkm_map_color_rgb);
private
dev_proc_get_params(ppm_get_params);
private
dev_proc_put_params(ppm_put_params);
private
dev_proc_copy_alpha(pnm_copy_alpha);
private
dev_proc_begin_typed_image(pnm_begin_typed_image);

/* We need to initialize uses_color when opening the device, */
/* and after each showpage. */
private
dev_proc_open_device(ppm_open);
private
dev_proc_output_page(ppm_output_page);

/* And of course we need our own print-page routines. */
private
dev_proc_print_page(pbm_print_page);
private
dev_proc_print_page(pgm_print_page);
private
dev_proc_print_page(ppm_print_page);
private
dev_proc_print_page(pkm_print_page);
private
dev_proc_print_page(psm_print_page);
private
dev_proc_print_page(psm_print_page);
private
dev_proc_print_page(pam_print_page);

private
int pam_print_row(gx_device_printer* pdev, byte* data, int depth,
                  FILE* pstream);
private
int pam_print_page(gx_device_printer* pdev, FILE* pstream);

/* The device procedures */

/* See gdevprn.h for the template for the following. */
#define pgpm_procs(p_open, p_get_params, p_map_rgb_color, p_map_color_rgb,     \
                   p_map_cmyk_color)                                           \
	{                                                                      \
		p_open, NULL, NULL, ppm_output_page, gdev_prn_close,           \
		    p_map_rgb_color, p_map_color_rgb, NULL, NULL, NULL, NULL,  \
		    NULL, NULL, p_get_params, ppm_put_params,                  \
		    p_map_cmyk_color, NULL, NULL, NULL,                        \
		    gx_page_device_get_page_device                             \
	}

private
const gx_device_procs pbm_procs =
    pgpm_procs(gdev_prn_open, gdev_prn_get_params, gdev_prn_map_rgb_color,
               gdev_prn_map_color_rgb, NULL);
private
const gx_device_procs pgm_procs = pgpm_procs(
    ppm_open, gdev_prn_get_params, pgm_map_rgb_color, pgm_map_color_rgb, NULL);
private
const gx_device_procs ppm_procs =
    pgpm_procs(ppm_open, ppm_get_params, gx_default_rgb_map_rgb_color,
               ppm_map_color_rgb, NULL);
private
const gx_device_procs pnm_procs = pgpm_procs(
    ppm_open, ppm_get_params, ppm_map_rgb_color, ppm_map_color_rgb, NULL);
private
const gx_device_procs pkm_procs =
    pgpm_procs(ppm_open, ppm_get_params, NULL, cmyk_1bit_map_color_rgb,
               cmyk_1bit_map_cmyk_color);
private
const gx_device_procs pam_procs =
    pgpm_procs(ppm_open, ppm_get_params, NULL, cmyk_8bit_map_color_rgb,
               cmyk_8bit_map_cmyk_color);

/* The device descriptors themselves */
const gx_device_pbm gs_pbm_device = pbm_prn_device(
    pbm_procs, "pbm", '1', 0, 1, 1, 1, 0, 0, X_DPI, Y_DPI, pbm_print_page);
const gx_device_pbm gs_pbmraw_device = pbm_prn_device(
    pbm_procs, "pbmraw", '4', 1, 1, 1, 1, 1, 0, X_DPI, Y_DPI, pbm_print_page);
const gx_device_pbm gs_pgm_device = pbm_prn_device(
    pgm_procs, "pgm", '2', 0, 1, 8, 255, 0, 0, X_DPI, Y_DPI, pgm_print_page);
const gx_device_pbm gs_pgmraw_device = pbm_prn_device(
    pgm_procs, "pgmraw", '5', 1, 1, 8, 255, 0, 0, X_DPI, Y_DPI, pgm_print_page);
const gx_device_pbm gs_pgnm_device = pbm_prn_device(
    pgm_procs, "pgnm", '2', 0, 1, 8, 255, 0, 1, X_DPI, Y_DPI, pgm_print_page);
const gx_device_pbm gs_pgnmraw_device =
    pbm_prn_device(pgm_procs, "pgnmraw", '5', 1, 1, 8, 255, 0, 1, X_DPI, Y_DPI,
                   pgm_print_page);
const gx_device_pbm gs_ppm_device = pbm_prn_device(
    ppm_procs, "ppm", '3', 0, 3, 24, 255, 255, 0, X_DPI, Y_DPI, ppm_print_page);
const gx_device_pbm gs_ppmraw_device =
    pbm_prn_device(ppm_procs, "ppmraw", '6', 1, 3, 24, 255, 255, 0, X_DPI,
                   Y_DPI, ppm_print_page);
const gx_device_pbm gs_pnm_device = pbm_prn_device(
    pnm_procs, "pnm", '3', 0, 3, 24, 255, 255, 1, X_DPI, Y_DPI, ppm_print_page);
const gx_device_pbm gs_pnmraw_device =
    pbm_prn_device(pnm_procs, "pnmraw", '6', 1, 3, 24, 255, 255, 1, X_DPI,
                   Y_DPI, ppm_print_page);
const gx_device_pbm gs_pkm_device = pbm_prn_device(
    pkm_procs, "pkm", '3', 0, 4, 4, 1, 1, 0, X_DPI, Y_DPI, pkm_print_page);
const gx_device_pbm gs_pkmraw_device = pbm_prn_device(
    pkm_procs, "pkmraw", '6', 1, 4, 4, 1, 1, 0, X_DPI, Y_DPI, pkm_print_page);
const gx_device_pbm gs_pksm_device = pbm_prn_device(
    pkm_procs, "pksm", '1', 0, 4, 4, 1, 1, 0, X_DPI, Y_DPI, psm_print_page);
const gx_device_pbm gs_pksmraw_device = pbm_prn_device(
    pkm_procs, "pksmraw", '4', 1, 4, 4, 1, 1, 0, X_DPI, Y_DPI, psm_print_page);
const gx_device_pbm gs_pam_device = pbm_prn_device(
    pam_procs, "pam", '7', 1, 4, 32, 255, 255, 0, X_DPI, Y_DPI, pam_print_page);

/* Plan 9 bitmaps default to 100 dpi. */
const gx_device_pbm gs_plan9bm_device = pbm_prn_device(
    pbm_procs, "plan9bm", '9', 1, 1, 1, 1, 1, 1, 100, 100, pbm_print_page);
...
/* TO_DO_DEVICEN  - Kludge to emulate pre DeviceN math errors */
	...
	b = cv[0];
	gray =
	    ((r * (uint32_t)lum_red_weight) + (g * (uint32_t)lum_green_weight) +
	     (b * (uint32_t)lum_blue_weight) + (lum_all_weights / 2)) /
	    lum_all_weights * pdev->color_info.max_gray / gx_max_color_value;
#else /* Should be ... */
...
private
int
pgm_map_color_rgb(gx_device* dev, gx_color_index color, gx_color_value prgb[3])
{
	gx_color_value gray =
...
private
gx_color_index
gx_old_default_rgb_map_rgb_color(gx_device* dev, gx_color_value r,
...
private
int
ppm_map_color_rgb(gx_device* dev, gx_color_index color, gx_color_value prgb[3])
{
	...
	prgb[1] = ((color >> bitspercolor) & colormask) *
	          (ulong)gx_max_color_value / max_rgb;
	prgb[2] = (color & colormask) * (ulong)gx_max_color_value / max_rgb;
	return 0;
}
...
pkm_map_cmyk_color(gx_device* pdev, const gx_color_value cv[])
{
	...
	gx_color_index color = (((((cc << bpc) + mc) << bpc) + yc) << bpc) + kc;
...
#define CVALUE(c) ((gx_color_value)((ulong)(c)*gx_max_color_value / max_value))
	   ...
	   (code = param_read_long(plist, (vname = "BlueValues"), &v)) != 1) {
		if(code < 0)
			ecode = code;
		        ...
		        v > (bdev->is_raw || ncomps > 1 ? 256 : 65536L))
		...
		if(ecode >= 0) {
			...
			pdev->color_info.max_gray = pdev->color_info.max_color =
			    (pdev->color_info.dither_grays =
			         pdev->color_info.dither_colors = (int)v) -
			    1;
		}
	}
	if((code = ecode) < 0 ||
	   (code = gdev_prn_put_params_planar(pdev, plist,
	                                      &bdev->UsePlanarBuffer)) < 0)
...
private
int
pnm_copy_alpha(gx_device* pdev, const byte* data, int data_x, int raster,
               gx_bitmap_id id, int x, int y, int width, int height,
               gx_color_index color, int depth)
{
	gx_device_pbm* const bdev = (gx_device_pbm*)pdev;

	if(pdev->color_info.depth < 24 || (color >> 8) == (color & 0xffff))
		bdev->uses_color |= 1;
	else
		bdev->uses_color |= 2;
	return (*bdev->save_copy_alpha)(pdev, data, data_x, raster, id, x, y,
...
private
int
pnm_begin_typed_image(gx_device* dev, const gs_imager_state* pis,
                    ...
                    int (*row_proc)(gx_device_printer*, byte*, int, FILE*))
{
	...
	byte* data =
	    gs_alloc_bytes(pdev->memory, raster, "pbm_print_page_loop");
	int lnum = 0;
	int code = 0;
	...
	if(magic == '9')
		fprintf(pstream, "%11d %11d %11d %11d %11d ", 0, 0, 0,
...
private
int
pbm_print_row(gx_device_printer* pdev, byte* data, int depth, FILE* pstream)
{
	gx_device_pbm* const bdev = (gx_device_pbm*)pdev;
...
private
int
pgm_print_row(gx_device_printer* pdev, byte* data, int depth, FILE* pstream)
...
private
int
pxm_pbm_print_row(gx_device_printer* pdev, byte* data, int depth, FILE* pstream)
...
pgm_print_page(gx_device_printer* pdev, FILE* pstream)
{
	...
	return (bdev->uses_color == 0 && bdev->optimize
	            ? pbm_print_page_loop(pdev, (char)((int)bdev->magic - 1),
	                                  pstream, pxm_pbm_print_row)
...
private
int
ppgm_print_row(gx_device_printer* pdev, byte* data, int depth, FILE* pstream,
               bool color)
...
private
int
ppm_print_row(gx_device_printer* pdev, byte* data, int depth, FILE* pstream)
{
	return ppgm_print_row(pdev, data, depth, pstream, true);
}
private
int
ppm_pgm_print_row(gx_device_printer* pdev, byte* data, int depth, FILE* pstream)
{
...
ppm_print_page(gx_device_printer* pdev, FILE* pstream)
{
	...
	return (
	    bdev->uses_color >= 2 || !bdev->optimize
	        ? pbm_print_page_loop(pdev, bdev->magic, pstream, ppm_print_row)
	        : bdev->uses_color == 1
	              ? pbm_print_page_loop(pdev, (char)((int)bdev->magic - 1),
	                                    pstream, ppm_pgm_print_row)
	              : pbm_print_page_loop(pdev, (char)((int)bdev->magic - 2),
	                                    pstream, pxm_pbm_print_row));
}

private
int
pam_print_row(gx_device_printer* pdev, byte* data, int depth, FILE* pstream)
{
	if(depth == 32)
		fwrite(data, 1, pdev->width * (depth / 8), pstream);
	return 0;
...
pam_print_page(gx_device_printer* pdev, FILE* pstream)
{
	...
	return pbm_print_page_loop(pdev, bdev->magic, pstream, pam_print_row);
}
...
private
int
pkm_print_row_4(gx_device_printer* pdev, byte* data, int depth, FILE* pstream)
{
	gx_device_pbm* const bdev = (gx_device_pbm*)pdev;
	byte* bp;
	uint x;
	...
	for(i = 0; i < 16; ++i) {
		...
		cmyk_1bit_map_color_rgb((gx_device*)pdev, (gx_color_index)i,
		                        rgb);
	...
	if(bdev->is_raw) {
		for(bp = data, x = 0; x < pdev->width;) {
			byte
			    raw[50 * 3]; /* 50 is arbitrary, but must be even */
			...
			for(; x < end; bp++, outp += 6, x += 2) {
				...
				outp[0] = rv[pixel], outp[1] = gv[pixel],
				outp[2] = bv[pixel];
				pixel = b & 0xf;
				outp[3] = rv[pixel], outp[4] = gv[pixel],
				outp[5] = bv[pixel];
			}
		...
		for(bp = data, x = 0, shift = 4; x < pdev->width;) {
			...
			bp += shift >> 2;
			++x;
			fprintf(pstream, "%d %d %d%c", rv[pixel], gv[pixel],
			        bv[pixel],
			        (x == pdev->width || !(x & 7) ? '\n' : ' '));
		}
	}
...
private
int
pkm_print_row(gx_device_printer* pdev, byte* data, int depth, FILE* pstream)
{
	gx_device_pbm* const bdev = (gx_device_pbm*)pdev;
	byte* bp;
	uint x;
...
pkm_print_page(gx_device_printer* pdev, FILE* pstream)
{
	...
	return pbm_print_page_loop(
	    pdev, bdev->magic, pstream,
	    (pdev->color_info.depth < 8 ? pkm_print_row_4 : pkm_print_row));
}
...
psm_print_page(gx_device_printer* pdev, FILE* pstream)
{
	...
	byte* data =
	    gs_alloc_bytes(pdev->memory, max_raster, "pksm_print_page");
	int code = 0;
	int plane;
	...
	for(plane = 0; plane < pdev->color_info.num_components; ++plane) {
		int lnum, band_end;
		/*
		 * The following initialization is unnecessary: lnum == band_end
		 * on
		 * the first pass through the loop below, so marked will always
		 * be
		 * set before it is used.  We initialize marked solely to
		 * suppress
		...
		for(lnum = band_end = 0; lnum < pdev->height; lnum++) {
			...
			if(lnum == band_end) {
				...
				int band_start;
				int band_height = gdev_prn_colors_used(
				    (gx_device*)pdev, lnum, 1, &colors_used,
				    &band_start);

				band_end = band_start + band_height;
				marked =
				    colors_used.or &(plane_mask << plane_shift);
				if(!marked)
					memset(data, 0, raster);
#ifdef DEBUG
				if(plane == 0)
					if_debug4(':', "[:]%4d - %4d mask = "
					               "0x%lx, slow_rop = %d\n",
					          lnum, band_end - 1,
					          (ulong)colors_used.or,
			...
			if(marked) {
				...
				code = gdev_prn_get_lines(
				    pdev, lnum, 1, data, raster, &row,
				    &actual_raster, &render_plane);
				if(code < 0)
					break;
			} else
				row = data;
			code = (plane_depth == 1
			            ? pbm_print_row(pdev, row, plane_depth,
			                            pstream)
			            : pgm_print_row(pdev, row, plane_depth,
			                            pstream));
			if(code < 0)
				break;
		}
