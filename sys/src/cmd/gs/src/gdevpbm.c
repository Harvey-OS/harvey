...
#define pbm_prn_device(procs, dev_name, magic, is_raw, num_comp, depth, max_gray, max_rgb, optimize, x_dpi, y_dpi, print_page)                                                                                                                         \
	{                                                                                                                                                                                                                                              \
		prn_device_body(gx_device_pbm, procs, dev_name, DEFAULT_WIDTH_10THS, DEFAULT_HEIGHT_10THS, x_dpi, y_dpi, 0, 0, 0, 0, num_comp, depth, max_gray, max_rgb, max_gray + 1, max_rgb + 1, print_page), magic, {0}, is_raw, optimize, 0, 0, 0 \
	}
...
#define pgpm_procs(p_open, p_get_params, p_map_rgb_color, p_map_color_rgb, p_map_cmyk_color)                                                                                                                                                \
	{                                                                                                                                                                                                                                   \
		p_open, NULL, NULL, ppm_output_page, gdev_prn_close, p_map_rgb_color, p_map_color_rgb, NULL, NULL, NULL, NULL, NULL, NULL, p_get_params, ppm_put_params, p_map_cmyk_color, NULL, NULL, NULL, gx_page_device_get_page_device \
	}
...
private
const gx_device_procs pgm_procs =
...
private
const gx_device_procs pnm_procs =
    pgpm_procs(ppm_open, ppm_get_params,
		    ...
		    int (*row_proc)(gx_device_printer *, byte *, int, FILE *))
{
	...
	if(magic == '9')
		...
		fprintf(pstream, "TUPLTYPE %s\n",
			(ncomps == 4) ? "CMYK" : ((ncomps == 3) ? "RGB" : "GRAYSCALE"));
		if(bdev->comment[0])
			fprintf(pstream, "# %s\n", bdev->comment);
		else
...
pgm_print_page(gx_device_printer *pdev, FILE *pstream)
{
	...
	return (bdev->uses_color == 0 && bdev->optimize ? pbm_print_page_loop(pdev, (char)((int)bdev->magic - 1), pstream,
									      pxm_pbm_print_row)
...
ppm_print_page(gx_device_printer *pdev, FILE *pstream)
{
	...
	return (bdev->uses_color >= 2 || !bdev->optimize ? pbm_print_page_loop(pdev, bdev->magic, pstream,
									       ppm_print_row)
							 : bdev->uses_color == 1 ? pbm_print_page_loop(pdev, (char)((int)bdev->magic - 1), pstream,
												       ppm_pgm_print_row)
	 ...
	 */
		...
		for(lnum = band_end = 0; lnum < pdev->height; lnum++) {
			...
			if(marked) {
				...
				row = data;
			code =
			    (plane_depth == 1 ? pbm_print_row(pdev, row, plane_depth, pstream) : pgm_print_row(pdev, row, plane_depth, pstream));
			if(code < 0)
				break;
		}
	}
