...
#define private_st_san_trap() /* When GC is invoked, only the buffer link is   \
                                 valid. */                                     \
...
struct gx_san_trap_contact_s {
	...
	gx_san_trap_contact*
	    next; /* Next element of the same relation, a cyclic list. */
	gx_san_trap_contact*
	    prev; /* Prev element of the same relation, a cyclic list. */
	gx_san_trap* upper, *lower; /* A contacting pair. */
};
#define private_st_san_trap_contact() /* When GC is invoked, only the buffer   \
                                         link is valid. */                     \
	gs_private_st_ptrs1(st_san_trap_contact, gx_san_trap_contact,          \
	                    "gx_san_trap_contact", san_trap_contact_enum_ptrs, \
	                    san_trap_contact_reloc_ptrs, link)
...
#define public_st_device_spot_analyzer() /* When GC is invoked, only the       \
                                            buffer links are valid. */         \
	gs_public_st_suffix_add4_final(                                        \
	    st_device_spot_analyzer, gx_device_spot_analyzer,                  \
...
int gx_san_trap_store(gx_device_spot_analyzer* padev, fixed ybot, fixed ytop,
...
/* Generate stems. */
int gx_san_generate_stems(gx_device_spot_analyzer* padev, bool overall_hints,
                          void* client_data,
