...
private
int sreadbuf(stream*, stream_cursor_write*);
private
int swritebuf(stream*, stream_cursor_read*, bool);
private
void stream_compact(stream*, bool);
...
case 5:
return ENUM_CONST_STRING(&st->file_name);
ENUM_PTRS_END
private
RELOC_PTRS_WITH(stream_reloc_ptrs, stream* st)
{
...
stream_finalize(void* vptr)
{
	...
	if_debug2('u', "[u]%s 0x%lx\n",
	          (!s_is_valid(st)
	               ? "already closed:"
	               : st->is_temp ? "is_temp set:" : st->file == 0
	                                                    ? "not file:"
	                                                    : "closing file:"),
	          (ulong)st);
...
private
const stream_template s_no_template = {&st_stream_state, 0, 0, 1, 1, 0};
...
stream_state*
s_alloc_state(gs_memory_t* mem, gs_memory_type_ptr_t stype, client_name_t cname)
{
	stream_state* st = gs_alloc_struct(mem, stream_state, stype, cname);

	if_debug3('s', "[s]alloc_state %s(%s) = 0x%lx\n",
	          client_name_string(cname), client_name_string(stype->sname),
	          (uint32_t)st);
	if(st)
...
ssetfilename(stream* s, const byte* data, uint size)
{
	byte* str =
	    (s->file_name.data == 0
	         ...
	         : gs_resize_string(
	               s->memory, (byte*)s->file_name.data, /* break const */
	               s->file_name.size, size + 1, "ssetfilename"));
...
s_disable(register stream* s)
{
	...
	/* Free the file name. */
	if(s->file_name.data) {
		gs_free_const_string(s->memory, s->file_name.data,
		                     s->file_name.size, "s_disable(file_name)");
		s->file_name.data = 0;
		s->file_name.size = 0;
	}
...
const stream_procs s_filter_read_procs = {s_std_noavailable, s_std_noseek,
                                          s_std_read_reset, s_std_read_flush,
                                          s_filter_close};

const stream_procs s_filter_write_procs = {
    s_std_noavailable, s_std_noseek, s_std_write_reset, s_filter_write_flush,
    s_filter_close};
...
spgetcc(register stream* s, bool close_at_eod)
{
	...
	while(status = s->end_status, left = s->srlimit - s->srptr,
...
sgets(stream* s, byte* buf, uint nmax, uint* pn)
{
	...
	while(cw.ptr < cw.limit) {
		...
		if((left = s->srlimit - s->srptr) > min_left) {
			...
			if(wanted >= s->bsize >> 2 && (st = s->state) != 0 &&
			   wanted >= st->template->min_out_size &&
			   s->end_status == 0 && left == 0) {
				byte* wptr = cw.ptr;

				cw.limit -= min_left;
				status = sreadbuf(s, &cw);
				cw.limit += min_left;
				/* Compact the stream so stell will return the
				 * right result. */
				stream_compact(s, true);
				/*
				 * We know the stream buffer is empty, so it's
				 * safe to
				 * update position.  However, we need to reset
				 * the read
				 * cursor to indicate that there is no data in
				 * the buffer.
				 */
				s->srptr = s->srlimit = s->cbuf - 1;
...
#undef LF
		...
		if(count >= buf->size) { /* filled the string */
			...
			}
			{
				...
				byte* ndata = gs_resize_string(
				    bufmem, buf->data, buf->size, nsize,
				    "sreadline(buffer)");
...
sreadbuf(stream* s, stream_cursor_write* pbuf)
{
	...
	for(;;) {
		...
		for(;;) { /* Descend into the recursion. */
			...
			if_debug4(
			    's',
			    "[s]read process 0x%lx, nr=%u, nw=%u, eof=%d\n",
			    (uint32_t)curr, (uint)(pr->limit - pr->ptr),
			    (uint)(pw->limit - pw->ptr), eof);
			oldpos = pw->ptr;
			status =
			    (*curr->procs.process)(curr->state, pr, pw, eof);
			pr->limit += left;
			if_debug5('s', "[s]after read 0x%lx, nr=%u, nw=%u, "
			               "status=%d, position=%d\n",
			          (uint32_t)curr, (uint)(pr->limit - pr->ptr),
	...
	for(;;) {
		for(;;) {
			...
			if(prev == 0)
				...
				pr = &curr->cursor.r;
			if_debug5(
			    's', "[s]write process 0x%lx(%s), nr=%u, nw=%u, "
			         "end=%d\n",
			    (uint32_t)curr,
			...
			if(status >= 0) {
				status = (*curr->procs.process)(curr->state, pr,
				                                pw, end);
				if_debug5(
				    's', "[s]after write 0x%lx, nr=%u, nw=%u, "
				         "end=%d, status=%d\n",
				    (uint32_t)curr, (uint)(pr->limit - pr->ptr),
			...
			if(status != 1) {
				/*
				 * Keep going if we are closing a filter with a
				 * sub-stream.
		...
		if(status < 0 || prev == 0) {
			 ...
			 * and returned 0 or EOFC (so their end_status is now
			 * EOFC):
			 * finish unwinding and then return.  Change the status
			 * of
...
private
int s_string_available(stream*, int32_t*), s_string_read_seek(stream*, int32_t),
...
{
	s->srptr = s->srlimit =
	    s->cbuf + s->bsize - 1; /* just set to the end */
	return 0;
}
void
...
private
int s_write_position_process(stream_state*, stream_cursor_read*,
                             stream_cursor_write*, bool);
...
stream*
s_add_filter(stream** ps, const stream_template* template, stream_state* ss,
             gs_memory_t* mem)
{
	stream* es;
	stream_state* ess;
	 ...
	 */
	if(bsize > (*ps)->bsize &&
	   template->process != s_NullE_template.process) {
	...
	while(*ps != target) {
		...
		if(mem) {
			...
			if(ss != (stream_state*)s)
				gs_free_object(mem, ss,
				               "s_close_filters(state)");
		}
		*ps = next;
	}
	return 0;
...
/* Stream template */
const stream_template s_NullE_template = {&st_stream_state, NULL,
                                          s_Null_process, 1, 1};
const stream_template s_NullD_template = {&st_stream_state, NULL,
                                          s_Null_process, 1, 1};
