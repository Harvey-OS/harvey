...
stream_finalize(void *vptr)
{
	...
	if_debug2('u', "[u]%s 0x%lx\n",
		  (!s_is_valid(st) ? "already closed:" : st->is_temp ? "is_temp set:" : st->file == 0 ? "not file:" : "closing file:"), (ulong)st);
...
private
const stream_template s_no_template = {
...
{
	byte *str =
	    (s->file_name.data == 0 ? gs_alloc_string(s->memory, size + 1, "ssetfilename") : gs_resize_string(s->memory,
