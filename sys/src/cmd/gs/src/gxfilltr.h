...
TEMPLATE_spot_into_trapezoids(line_list* ll, fixed band_mask)
{
	...
	while(1) {
		...
		while(yll != 0 && yll->start.y == y) {
			active_line* ynext =
			    yll->next; /* insert smashes next/prev links */
		...
		if(vd_enabled) {
			...
			y +=
			    0; /* Just a good place for a debugger breakpoint */
		}
		...
		if(y == y1) {
			...
			if(code > 0) {
				yll =
				    ll->y_list; /* add_y_line_aux in
				                   process_h_segments changes
				                   it. */
				continue;
			}
		}
		...
		covering_pixel_centers = COVERING_PIXEL_CENTERS(
		    y, y1, (!FILL_ADJUST ? 0 : fo.adjust_below),
		    (!FILL_ADJUST ? 0 : fo.adjust_above));
		if(y != y1) {
			intersect_al(ll, y, &y1,
			             (covering_pixel_centers ? 1 : -1),
			             all_bands); /* May change y1. */
			covering_pixel_centers = COVERING_PIXEL_CENTERS(
			    y, y1, (!FILL_ADJUST ? 0 : fo.adjust_below),
			    (!FILL_ADJUST ? 0 : fo.adjust_above));
		}
		...
		if(covering_pixel_centers || all_bands) {
			...
			for(alp = ll->x_list; alp != 0; alp = alp->next) {
				...
				inside += alp->direction;
				if(INSIDE_PATH_P(
				       inside, rule)) /* not about to go out */
					continue;
				...
				if(alp->next != NULL &&
				   ...
				   alp->x_next == alp->next->x_next) {
					/* If the next trapezoid contacts this
					   one, unite them.
					   This simplifies data for the spot
					   analyzer
					   and reduces the number of trapezoids
					   in the rasterization.
					   Note that the topology possibly isn't
					   exactly such
					   as we generate by this uniting :
					   Due to arithmetic errors in
					   x_current, x_next
					   we can unite things, which really are
					   not contacting.
					   But this level of the topology
					   precision is enough for
					   the glyph grid fitting.
					   Also note that
					   while a rasterization with dropout
					   prevention
					   it may cause a shift when choosing a
					   pixel
				...
				/* We just went from inside to outside, so fill
				 * the region. */
				INCR(band_fill);
				if(FILL_ADJUST &&
				   !(flp->end.x == flp->start.x &&
				     alp->end.x == alp->start.x) &&
				   (fo.adjust_below | fo.adjust_above) != 0) {
					/* Assuming pseudo_rasterization =
					 * false. */
					if(FILL_DIRECT)
						code =
						    slant_into_trapezoids__fd(
						        ll, flp, alp, y, y1);
					else
						code =
						    slant_into_trapezoids__nd(
						        ll, flp, alp, y, y1);
				} else {
					...
					if(IS_SPOTAN) {
						/* We can't pass data through
						   the device interface because
						   we need to pass segment
						   pointers. We're unhappy of
						   that. */
						code = gx_san_trap_store(
						    (gx_device_spot_analyzer*)
						        fo.dev,
						    y, y1, flp->x_current,
						    alp->x_current, flp->x_next,
						    alp->x_next, flp->pseg,
						    alp->pseg, flp->direction,
						    alp->direction);
					} else {
						if(flp->end.x == flp->start.x &&
						   alp->end.x == alp->start.x) {
							if(FILL_ADJUST) {
								ybot = max(
								    y - fo.adjust_below,
							...
							if(ytop > ybot) {
								...
								int hi =
								    fixed2int_pixround(
								        ytop) -
								    yi;
								int xli = fixed2int_var_pixround(
								    flp->end.x -
								    (!FILL_ADJUST
								         ? 0
								         : fo.adjust_left));
								int xi = fixed2int_var_pixround(
								    alp->end.x +
								    (!FILL_ADJUST
								         ? 0
								         : fo.adjust_right));
					...
					if(PSEUDO_RASTERIZATION) {
						if(code < 0)
							return code;
						code = complete_margin(
						    ll, flp, alp, y, y1);
						if(code < 0)
							return code;
						code = margin_interior(
						    ll, flp, alp, y, y1);
						if(code < 0)
							return code;
						code = add_margin(ll, flp, alp,
						                  y, y1);
						if(code < 0)
							return code;
						code = process_h_lists(
						    ll, plp, flp, alp, y, y1);
						plp = alp;
					}
				}
			...
			if(PSEUDO_RASTERIZATION) {
				...
				for(alp = ll->x_list; alp != 0;
					...
					if(INSIDE_PATH_P(
					       inside,
					       rule)) /* not about to go out */
						continue;
					code = continue_margin(ll, flp, alp, y,
					                       y1);
					if(code < 0)
						return code;
					code = process_h_lists(ll, plp, flp,
					                       alp, y, y1);
					plp = alp;
					if(code < 0)
						return code;
