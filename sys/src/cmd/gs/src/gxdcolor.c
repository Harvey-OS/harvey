...
const gx_device_color_type_t gx_dc_type_data_none = {
    &st_bytes, gx_dc_no_save_dc, gx_dc_no_get_dev_halftone, gx_dc_no_get_phase,
...
const gx_device_color_type_t gx_dc_type_data_null = {
    ...
    gx_dc_null_equal, gx_dc_no_write, gx_dc_null_read,
...
const gx_device_color_type_t gx_dc_type_data_pure = {
    &st_bytes, gx_dc_pure_save_dc, gx_dc_no_get_dev_halftone,
    gx_dc_no_get_phase, gx_dc_pure_load, gx_dc_pure_fill_rectangle,
    gx_dc_pure_fill_masked, gx_dc_pure_equal, gx_dc_pure_write, gx_dc_pure_read,
...
gx_get_dc_type_index(const gx_device_color* pdevc)
{
	...
	num_types =
	    sizeof(dc_color_type_table) / sizeof(dc_color_type_table[0]);
...
private
int
gx_dc_no_fill_rectangle(const gx_device_color* pdevc, int x, int y, int w,
                        ...
                        const gx_rop_source_t* source)
{
	...
	set_nonclient_dev_color(&filler,
	                        0); /* any valid value for dev will do */
	return gx_dc_pure_fill_rectangle(&filler, x, y, w, h, dev, lop, source);
}

private
int
gx_dc_no_fill_masked(const gx_device_color* pdevc, const byte* data, int data_x,
...
private
int
gx_dc_no_write(const gx_device_color* pdevc,      /* ignored */
...
private
int gx_dc_no_read(gx_device_color* pdevc,
...
private
int
gx_dc_no_get_nonzero_comps(const gx_device_color* pdevc_ignored,
...
private
int
gx_dc_null_fill_rectangle(const gx_device_color* pdevc, int x, int y, int w,
                          int h, gx_device* dev, gs_logical_operation_t lop,
                          const gx_rop_source_t* source)
{
	return 0;
...
gx_dc_null_fill_masked(const gx_device_color* pdevc, const byte* data,
                       int data_x, int raster, gx_bitmap_id id, int x, int y,
...
private
int gx_dc_null_read(gx_device_color* pdevc,
                    const gs_imager_state* pis,        /* ignored */
                    const gx_device_color* prior_devc, /* ignored */
                    const gx_device* dev,              /* ignored */
                    const byte* pdata,                 /* ignored */
...
private
int
gx_dc_pure_fill_rectangle(const gx_device_color* pdevc, int x, int y, int w,
                          int h, gx_device* dev, gs_logical_operation_t lop,
                          const gx_rop_source_t* source)
{
                       ...
                       bool invert)
{
	...
	}
	{
		...
		return (*dev_proc(dev, strip_copy_rop))(
		    dev, data, data_x, raster, id, scolors, NULL, tcolors, x, y,
		    w, h, 0, 0,
...
private
int
gx_dc_pure_write(const gx_device_color* pdevc,
                 const gx_device_color_saved* psdc, /* ignored */
 ...
 */
private
int gx_dc_pure_read(gx_device_color* pdevc,
                    const gs_imager_state* pis,        /* ignored */
                    const gx_device_color* prior_devc, /* ignored */
...
int
gx_dc_pure_get_nonzero_comps(const gx_device_color* pdevc, const gx_device* dev,
                             gx_color_index* pcomp_bits)
{
	int code;
	gx_color_value cvals[GX_DEVICE_COLOR_MAX_COMPONENTS];

	code = dev_proc(dev, decode_color)((gx_device*)dev, pdevc->colors.pure,
	                                   cvals);
	if(code >= 0) {
		int i, ncomps = dev->color_info.num_components;
...
void
gx_complete_halftone(gx_device_color* pdevc, int num_comps,
	...
	for(iy = 0; iy < h; ++iy, row += raster) {
		...
		while(left) {
			...
			if(run < 8) {
				...
				if(run >=
				   left) /* run < 8 unless very last byte */
					left = 0;
				else
					bit = run & 7, left -= run;
			}
			code = gx_device_color_fill_rectangle(
			    pdevc, x + w - l0, y + iy, l0 - left, 1, dev, lop,
			    NULL);
			if(code < 0)
				return code;
		}
 ...
 */
int
gx_dc_write_color(gx_color_index color, const gx_device* dev, byte* pdata,
                  uint* psize)
{
 ...
 */
int
gx_dc_read_color(gx_color_index* pcolor, const gx_device* dev,
                 const byte* pdata, int size)
{
	...
	for(i = (num_bytes >= arch_sizeof_color_index ? 1 : 0); i < num_bytes;
	    i++)
