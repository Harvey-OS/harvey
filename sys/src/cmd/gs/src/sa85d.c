...
/* Process a buffer */
private
int a85d_finish(int, uint32_t, stream_cursor_write*);
private
int
               ...
               stream_cursor_write* pw, bool last)
{
	...
	while(p < rlimit) {
		...
		if(ccode < 85) { /* catches ch < '!' as well */
			if(ccount == 4) {
				/*
				 * We've completed a 32-bit group.  Make sure we
				 * have
				...
				/* Check for overflow condition, throw ioerror
				 * if so */
...
/* Stream template */
const stream_template s_A85D_template = {&st_A85D_state, s_A85D_init,
                                         s_A85D_process, 2, 4};
