...
typedef enum {
	...
	cmd_opv_set_tile_size =
	    0x01, /* rs?(1)nry?(1)nrx?(1)depth(5, encoded), */
...
#define cmd_op_name_strings                                                    \
	"(misc)", "set_color[0]", "set_color[1]", "fill_rect",                 \
	    "fill_rect_short", "fill_rect_tiny", "tile_rect",                  \
	    "tile_rect_short", "tile_rect_tiny", "copy_mono",                  \
	    "copy_color_alpha", "delta_tile_index", "set_tile_index",          \
	    "(misc2)", "(segment)", "(path)"
...
#define cmd_misc_op_name_strings                                               \
	"end_run", "set_tile_size", "set_tile_phase", "set_tile_bits",         \
	    ...
	    "delta2_color0", "delta2_color1", "set_copy_color",                \
	    "set_copy_alpha",
 ...
 */
#define cmd_max_intsize(siz) (((siz)*8 + 6) / 7)
#define cmd_largest_size (2 + (1 + cmd_max_dash) * sizeof(float))
...
#define cmd_code_to_depth(v) (((v)&0x10) != 0 ? ((v)&0xf) << 3 : (v)&0xf)
 ...
 */
uint clist_bitmap_bytes(uint width_bits, uint height, int compression_mask,
...
#define cls_initial_values                                                     \
	...
	, {gx_dc_type_none}, 0, gx_no_bitmap_id, {0, 0},                       \
	    {gx_no_color_index, gx_no_color_index}, {0, 0, 0, 0}, lop_default, \
	    0, 0, 0, initial_known, {0, 0}, 0,                                 \
	{                                                                      \
		0                                                              \
	}
...
#ifdef DEBUG
byte* cmd_put_op(gx_device_clist_writer* cldev, gx_clist_state* pcls,
                 uint size);
#else
...
#define cmd_sizew(w) (w1byte(w) ? 1 : w2byte(w) ? 2 : cmd_size_w((uint)(w)))
#define cmd_size2w(wx, wy)                                                     \
	(w1byte((wx) | (wy)) ? 2 : cmd_size_w((uint)(wx)) +                    \
	                               cmd_size_w((uint)(wy)))
...
extern const clist_select_color_t clist_select_color0, clist_select_color1,
...
#define cmd_set_color0(dev, pcls, color0)                                      \
	cmd_put_color(dev, pcls, &clist_select_color0, color0,                 \
	              &(pcls)->colors[0])
#define cmd_set_color1(dev, pcls, color1)                                      \
	cmd_put_color(dev, pcls, &clist_select_color1, color1,                 \
	              &(pcls)->colors[1])
...
#define cmd_do_enable_lop(cldev, pcls, enable)                                 \
	((pcls)->lop_enabled == ((enable) ^ 1) &&                              \
	     ...
	     ? (cldev)->error_code                                             \
	     : 0)
#define cmd_enable_lop(cldev, pcls) cmd_do_enable_lop(cldev, pcls, 1)
#define cmd_disable_lop(cldev, pcls) cmd_do_enable_lop(cldev, pcls, 0)
...
#define cmd_do_enable_clip(cldev, pcls, enable)                                \
	((pcls)->clip_enabled == ((enable) ^ 1) &&                             \
	     ...
	     ? (cldev)->error_code                                             \
	     : 0)
#define cmd_enable_clip(cldev, pcls) cmd_do_enable_clip(cldev, pcls, 1)
#define cmd_disable_clip(cldev, pcls) cmd_do_enable_clip(cldev, pcls, 0)
...
#define HANDLE_RECT_UNLESS(codevar, unless_clause)                             \
	...
	if(codevar < 0 && !(unless_clause))                                    \
		ERROR_RECT(codevar);                                           \
	END
#define HANDLE_RECT(codevar) HANDLE_RECT_UNLESS(codevar, 0)
#define END_RECTS_ON_ERROR(retry_cleanup, is_error, after_recovering)          \
	continue;                                                              \
	error_in_rect:                                                         \
	if(cdev->error_is_retryable) {                                         \
		retry_cleanup;                                                 \
		if((is_error) && cdev->driver_call_nesting == 0 &&             \
		   (band_code =                                                \
...
int cmd_put_color_map(gx_device_clist_writer* cldev, cmd_map_index map_index,
