                 ...
                 const gx_drawing_color* pdc)
{
	...
	if(!pdev->HaveStrokeColor) {
		/* opdfread.ps assumes same color for stroking and non-stroking
		 * operations. */
		int code = pdf_set_drawing_color(
		    pdev, pis_for_hl_color, pdc, &pdev->saved_stroke_color,
		    &pdev->stroke_used_process_color,
		    &psdf_set_stroke_color_commands);
		if(code < 0)
			return code;
	}
	return pdf_set_drawing_color(
	    pdev, pis_for_hl_color, pdc, &pdev->saved_fill_color,
                   ...
                   const gx_drawing_color* pdc)
{
	...
	if(!pdev->HaveStrokeColor) {
		/* opdfread.ps assumes same color for stroking and non-stroking
		 * operations. */
		int code = pdf_set_drawing_color(pdev, pis_for_hl_color, pdc,
           ...
           gx_path_type_t type)
{
	...
	if(type & gx_path_type_stroke) {
		...
		double xw =
		    w * (fabs(vdev->state.ctm.xx) + fabs(vdev->state.ctm.yx));
	...
	if(!(type & gx_path_type_clip) &&
	   (x0 > xmax || x1 < xmin || y0 > ymax || y1 < ymin || x0 > x1 ||
	    y0 > y1))
...
const gx_device_vector_procs pdf_vector_procs = {
    ...
    NULL,
    /* Imager state */
    pdf_setlinewidth, psdf_setlinecap, psdf_setlinejoin, psdf_setmiterlimit,
    psdf_setdash, psdf_setflat, psdf_setlogop,
    /* Other state */
    pdf_can_handle_hl_color, pdf_setfillcolor, pdf_setstrokecolor,
    /* Paths */
    psdf_dopath, pdf_dorect, psdf_beginpath, psdf_moveto, psdf_lineto,
    psdf_curveto, psdf_closepath, pdf_endpath};
...
{ /* This recursive function provides a reverse order of the list elements. */
	...
	if(e->next != NULL) {
		int code = pdf_put_clip_path_list_elem(pdev, e->next, cenum,
		                                       state, vs);
...
pdf_put_clip_path(gx_device_pdf* pdev, const gx_clip_path* pcpath)
{
	...
	if(new_id != pdev->no_clip_path_id) {
		...
		if(pcpath->path_list == NULL) {
			 ...
			 * This is very unfortunate, but until we can come up
			 * with
			...
			code = pdf_put_clip_path_list_elem(
			    pdev, pcpath->path_list, &cenum, &state, vs);
			if(code < 0)
				return code;
		}
	}
	pdev->clip_path_id = new_id;
	return pdf_remember_clip_path(
	    pdev,
...
private
int
lcvd_fill_rectangle_shifted(gx_device* dev, int x, int y, int width, int height,
                            gx_color_index color)
{
	pdf_lcvd_t* cvd = (pdf_lcvd_t*)dev;

	return cvd->std_fill_rectangle(
	    (gx_device*)&cvd->mdev, x - cvd->mdev.mapped_x,
	    y - cvd->mdev.mapped_y, width, height, color);
}
private
int
lcvd_fill_rectangle_shifted2(gx_device* dev, int x, int y, int width,
                             int height, gx_color_index color)
{
	pdf_lcvd_t* cvd = (pdf_lcvd_t*)dev;
	int code;

	code = (*dev_proc(cvd->mask, fill_rectangle))(
	    (gx_device*)cvd->mask, x - cvd->mdev.mapped_x,
	    y - cvd->mdev.mapped_y, width, height, (gx_color_index)1);
	if(code < 0)
		return code;
	return cvd->std_fill_rectangle(
	    (gx_device*)&cvd->mdev, x - cvd->mdev.mapped_x,
...
private
int
lcvd_fill_rectangle_shifted_from_mdev(gx_device* dev, int x, int y, int width,
                                      int height, gx_color_index color)
{
	pdf_lcvd_t* cvd = (pdf_lcvd_t*)dev;
...
{
	/* Assuming 'mdev' is being closed before 'mask' - see
	 * gx_image3_end_image. */
...
write_image(gx_device_pdf* pdev, gx_device_memory* mdev, gs_matrix* m)
{
	...
	code = pdf_copy_color_data(pdev, mdev->base, sourcex, mdev->raster,
	                           gx_no_bitmap_id, 0, 0, mdev->width,
	                           mdev->height, &image, &writer, 2);
	if(code == 1)
		code = 0; /* Empty image. */
	else if(code == 0)
...
write_mask(gx_device_pdf* pdev, gx_device_memory* mdev, gs_matrix* m)
{
	...
	code = gdev_pdf_copy_mono((gx_device*)pdev, mdev->base, sourcex,
	                          mdev->raster, gx_no_bitmap_id, 0, 0,
	                          mdev->width, mdev->height, gx_no_color_index,
	                          (gx_color_index)0);
...
private
void
max_subimage_width(int width, byte* base, int x0, long count1, int* x1,
                   long* count)
{
...
private
void
compute_subimage(int width, int height, int raster, byte* base, int x0, int y0,
                 long MaxClipPathSize, int* x1, int* y1)
{
	...
	if(x0 != 0) {
		...
		max_subimage_width(width, base + y0 * raster, x0,
		                   MaxClipPathSize / 4, x1, &count);
		*y1 = y0;
	} else {
		int xx, y = y0, yy;
		...
		for(; y < height && count1 > 0;) {
			max_subimage_width(width, base + y * raster, 0, count1,
			                   &xx, &count);
			if(xx < width) {
				if(y == y0) {
			...
			yy = y + 1;
			for(; yy < height; yy++)
				if(memcmp(base + raster * y, base + raster * yy,
				          raster))
					break;
			y = yy;
		}
...
private
int
image_line_to_clip(gx_device_pdf* pdev, byte* base, int x0, int x1, int y0,
                   int y1, bool started)
...
private
int
mask_to_clip(gx_device_pdf* pdev, int width, int height, int raster, byte* base,
             int x0, int y0, int x1, int y1)
{
	...
	for(y = y0; y < y1 && code >= 0;) {
		...
		code = image_line_to_clip(pdev, base + raster * y, x0, x1, y,
		                          yy, has_segments);
		if(code > 0)
			has_segments = true;
		y = yy;
...
private
int
write_subimage(gx_device_pdf* pdev, gx_device_memory* mdev, int x, int y,
               int x1, int y1)
{
	gs_image_t image;
	pdf_image_writer writer;
	...
	code = pdf_copy_color_data(pdev, mdev->base + mdev->raster * Y, X,
	                           mdev->raster, gx_no_bitmap_id, X, Y, X1 - X,
	                           Y1 - Y, &image, &writer, 2);
	if(code < 0)
		return code;
	if(!writer.pres)
...
write_image_with_clip(gx_device_pdf* pdev, pdf_lcvd_t* cvd)
{
	...
	for(;;) {
		...
		compute_subimage(cvd->mask->width, cvd->mask->height,
		                 cvd->mask->raster, cvd->mask->base, x, y,
		                 max(pdev->MaxClipPathSize, 100), &x1, &y1);
		code = mask_to_clip(pdev, cvd->mask->width, cvd->mask->height,
		                    cvd->mask->raster, cvd->mask->base, x, y,
		                    x1, y1);
		if(code < 0)
			return code;
...
pdf_dump_converted_image(gx_device_pdf* pdev, pdf_lcvd_t* cvd)
{
	...
	if(!cvd->path_is_empty || cvd->has_background) {
		if(!cvd->has_background)
			stream_puts(pdev->strm, "W n\n");
		code = write_image(pdev, &cvd->mdev,
		                   (cvd->write_matrix ? &cvd->m : NULL));
		...
		code = (*dev_proc(pdev, pattern_manage))(
		    (gx_device*)pdev, id, &inst, pattern_manage__start_accum);
		if(code >= 0) {
			stream_puts(pdev->strm, "W n\n");
		...
		if(code >= 0) {
			...
			pprintld1(pdev->strm, " cs /R%ld scn ",
			          pdf_resource_id(pres));
		}
		if(code >= 0)
			code = write_mask(pdev, cvd->mask,
			                  (cvd->write_matrix ? &cvd->m : NULL));
	...
	if(code > 0)
		code = (*dev_proc(&cvd->mdev, fill_rectangle))(
		    (gx_device*)&cvd->mdev, 0, 0, cvd->mdev.width,
		    cvd->mdev.height, (gx_color_index)0);
	return code;
}
private
int
	...
	if(gx_path_is_null(ppath)) {
		...
		if(!cvd->path_is_empty) {
			...
			if(code < 0)
				return code;
			stream_puts(pdev->strm, "Q q\n");
			dev_proc(&cvd->mdev, fill_rectangle) =
			    lcvd_fill_rectangle_shifted2;
		}
		...
		code = gdev_vector_dopath(
		    (gx_device_vector*)pdev, ppath,
...
int
pdf_setup_masked_image_converter(gx_device_pdf* pdev, gs_memory_t* mem,
                                 const gs_matrix* m, pdf_lcvd_t** pcvd,
                                 bool need_mask, int x, int y, int w, int h,
                                 bool write_on_close)
{
	int code;
	...
	if(cvd == NULL) {
		cvd = gs_alloc_struct(mem, pdf_lcvd_t, &st_pdf_lcvd_t,
	...
	if(code < 0)
		return code;
	code = (*dev_proc(&cvd->mdev, fill_rectangle))(
	    (gx_device*)&cvd->mdev, 0, 0, cvd->mdev.width, cvd->mdev.height,
	    (gx_color_index)0);
	if(code < 0)
		return code;
	if(need_mask) {
		mask = gs_alloc_struct(mem, gx_device_memory, &st_device_memory,
		                       "pdf_setup_masked_image_converter");
		if(mask == NULL)
			return_error(gs_error_VMerror);
		cvd->mask = mask;
		...
		if(code < 0)
			return code;
		if(write_on_close) {
			code = (*dev_proc(mask, fill_rectangle))(
			    (gx_device*)mask, 0, 0, mask->width, mask->height,
			    (gx_color_index)0);
			if(code < 0)
				return code;
		}
	...
	if(!write_on_close) {
		...
		dev_proc(&cvd->mdev, fill_rectangle) =
		    (need_mask ? lcvd_fill_rectangle_shifted2
		               : lcvd_fill_rectangle_shifted);
	} else
		dev_proc(&cvd->mdev, fill_rectangle) =
		    lcvd_fill_rectangle_shifted_from_mdev;
	dev_proc(&cvd->mdev, get_clipping_box) =
	    lcvd_get_clipping_box_from_target;
	dev_proc(&cvd->mdev, pattern_manage) = lcvd_pattern_manage;
	dev_proc(&cvd->mdev, fill_path) =
	    lcvd_handle_fill_path_as_shading_coverage;
	cvd->m = *m;
	if(write_on_close) {
		cvd->mdev.is_open = true;
		mask->is_open = true;
		dev_proc(&cvd->mdev, close_device) =
		    lcvd_close_device_with_writing;
	}
	return 0;
}

void
pdf_remove_masked_image_converter(gx_device_pdf* pdev, pdf_lcvd_t* cvd,
                                  bool need_mask)
{
	(*dev_proc(&cvd->mdev, close_device))((gx_device*)&cvd->mdev);
	if(cvd->mask) {
		(*dev_proc(cvd->mask, close_device))((gx_device*)cvd->mask);
		gs_free_object(cvd->mask->memory, cvd->mask,
		               "pdf_remove_masked_image_converter");
	}
}
                   ...
                   const gx_drawing_color* pdcolor, const gx_clip_path* pcpath)
{
	...
	code =
	    prepare_fill_with_clip(pdev, pis, &box, have_path, pdcolor, pcpath);
	if(code == gs_error_rangecheck) {
		/* Fallback to the default implermentation for handling
		   a transparency with CompatibilityLevel<=1.3 . */
		return gx_default_fill_path((gx_device*)pdev, pis, ppath,
	...
	if(code == gs_error_rangecheck) {
		...
		if(!convert_to_image) {
			...
			return gx_default_fill_path(dev, pis, ppath, params,
			                            pdcolor, pcpath);
		} else {
			...
			/* double scalex = 1.9, scaley = 1.4; debug purpose
			 * only. */
			double scale, scalex = 1.0, scaley = 1.0;
			gx_drawing_color dc = *pdcolor;
			gs_pattern2_instance_t pi =
			    *(gs_pattern2_instance_t*)dc.ccolor.pattern;
			gs_state* pgs =
			    gs_state_copy(pi.saved, gs_state_memory(pi.saved));
			...
			scale = (double)rect_size.x * rect_size.y *
			        pdev->color_info.num_components /
			        pdev->MaxShadingBitmapSize;
			if(scale > 1) {
				/* This section (together with the call to
				   'path_scale' below)
				   sets up a downscaling when converting the
				   shading into bitmap.
				   We used floating point numbers to debug it,
				   but in production
				   we prefer to deal only with integers being
				   powers of 2
				   in order to avoid possible distorsions when
				   scaling paths.
				*/
				...
				if(scalex * scaley < scale)
					scaley *= 2;
				rect_size.x =
				    (int)floor(rect_size.x / scalex + 0.5);
				rect_size.y =
				    (int)floor(rect_size.y / scaley + 0.5);
				gs_make_scaling(1.0 / scalex, 1.0 / scaley,
				                &ms);
				...
				gs_matrix_multiply(
				    &ctm_only((gs_imager_state*)pgs), &ms, &mm);
			...
			code = pdf_setup_masked_image_converter(
			    pdev, pdev->memory, &m, &pcvd, need_mask, sx, sy,
			    rect_size.x, rect_size.y, false);
			pcvd->has_background =
			    gx_dc_pattern2_has_background(pdcolor);
			stream_puts(pdev->strm, "q\n");
			if(code >= 0) {
				code = gdev_vector_dopath(
				    (gx_device_vector*)pdev, ppath,
			...
			if(code >= 0) {
				...
				gx_path path_intersection, path1,
				    * p = &path_intersection;

				gx_path_init_local(&path_intersection,
				                   pdev->memory);
				gx_path_init_local(&path1, pdev->memory);
				gx_cpath_init_local_shared(
				    &cpath_intersection, pcpath, pdev->memory);
				if((code = gx_cpath_intersect(
				        &cpath_intersection, ppath,
				        params->rule, (gs_imager_state*)pis)) >=
				   0)
					code = gx_cpath_to_path(
					    &cpath_intersection,
					    &path_intersection);
				if(code >= 0 && scale > 1) {
					code = gx_path_copy(&path_intersection,
					                    &path1);
					if(code > 0) {
						p = &path1;
						code = path_scale(
			...
			stream_puts(pdev->strm, "Q Q\n");
			pdf_remove_masked_image_converter(pdev, &cvd,
			                                  need_mask);
	...
	if(have_path) {
		...
		if(make_rect_scaling(pdev, &box1, 1.0, &scale)) {
			gs_make_scaling(pdev->scale.x * scale,
			                pdev->scale.y * scale, &smat);
...
/* Stroke a path. */
int
gdev_pdf_stroke_path(gx_device* dev, const gs_imager_state* pis, gx_path* ppath,
                     ...
                     const gx_clip_path* pcpath)
{
	...
	if(code == gs_error_rangecheck) {
		/* Fallback to the default implermentation for handling
		   a transparency with CompatibilityLevel<=1.3 . */
		return gx_default_stroke_path((gx_device*)dev, pis, ppath,
		                              params, pdcolor, pcpath);
	}
	if(code < 0)
		return code;
	 ...
	 * the coordinates.
	 */
	set_ctm = (bool)gdev_vector_stroke_scaling((gx_device_vector*)pdev, pis,
	                                           &scale, &mat);
	if(set_ctm &&
	   (pis->ctm.xx == 0) + (pis->ctm.xy == 0) + (pis->ctm.yx == 0) +
	           (pis->ctm.yy == 0) >=
	       3) {
		/* Acrobat Reader 5 and Adobe Reader 6 issues
		   the "Wrong operand type" error with such matrices.
		   Besides that, we found that Acrobat Reader 4, Acrobat Reader
		   5
		   and Adobe Reader 6 all store the current path in user space
		   and apply CTM in the time of stroking - See the bug 687901.
		   Therefore a precise conversion of Postscript to PDF isn't
		   possible in this case.
		   Adobe viewers render a line with a constant width instead. */
		set_ctm = false;
		scale = fabs(pis->ctm.xx + pis->ctm.xy + pis->ctm.yx +
		             pis->ctm.yy) /* Using the non-zero coeff. */
		        /
		        sqrt(2); /* Empirically from Adobe. */
	}
	if(set_ctm) {
		/*
		 * We want a scaling factor that will bring the largest
		 * reasonable
		 * user coordinate within bounds.  We choose a factor based on
		 * the
	...
	gx_path_bbox(ppath, &bbox);
	{
		...
		*/
		...
		gs_distance_transform(pis->line_params.half_width, 0,
		                      &ctm_only(pis), &d0);
		gs_distance_transform(0, pis->line_params.half_width,
		                      &ctm_only(pis), &d1);
		...
		if(stroke_bbox.q.x < stroke_bbox.p.x ||
		   stroke_bbox.q.y < stroke_bbox.p.y)
			return 0;
	}
                                 ...
                                 const gx_clip_path* pcpath)
{
	...
	if(rect->p.x == rect->q.x)
		return 0;
	if(!convert_to_image) {
		code = prepare_fill_with_clip(pdev, pis, &box, true, pdcolor,
		                              pcpath);
		if(code < 0)
			return code;
		if(code == 1)
		...
		pprintg4(pdev->strm, "%g %g %g %g re f\n",
		         fixed2float(box1.p.x) * scale,
		         fixed2float(box1.p.y) * scale,
		         fixed2float(box1.q.x - box1.p.x) * scale,
		         fixed2float(box1.q.y - box1.p.y) * scale);
		if(psmat)
			stream_puts(pdev->strm, "Q\n");
		return 0;
	} else {
		...
		params.rule =
		    1; /* Not important because the path is a rectange. */
		...
		gx_path_add_rectangle(&path, rect->p.x, rect->p.y, rect->q.x,
		                      rect->q.y);
		code = gdev_pdf_fill_path(dev, pis, &path, &params, pdcolor,
		                          pcpath);
