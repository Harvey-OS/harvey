...
#define PDF_COLOR_SPACE_NAMES_SHORT "/CMYK", "/G", "/RGB", "/I"
extern const pdf_color_space_names_t pdf_color_space_names,
...
#define public_st_pdf_color_space() /* in gdevpdfc.c */                        \
	gs_public_st_suffix_add2(                                              \
	    st_pdf_color_space, pdf_color_space_t, "pdf_color_space_t",        \
	    pdf_color_space_enum_ptrs, pdf_color_space_reloc_ptrs,             \
	    st_pdf_resource, ranges, serialized)

/*
 * Create a local Device{Gray,RGB,CMYK} color space corresponding to the
 * given number of components.
 */
int pdf_cspace_init_Device(const gs_memory_t* mem, gs_color_space* pcs,
                           int num_components);
...
int pdf_cs_Pattern_uncolored_hl(gx_device_pdf* pdev, const gs_color_space* pcs,
                                cos_value_t* pvalue);

/* Set the ProcSets bits corresponding to an image color space. */
void pdf_color_space_procsets(gx_device_pdf* pdev, const gs_color_space* pcs);
...
void pdf_prepare_initial_viewer_state(gx_device_pdf* pdev,
                                      const gs_imager_state* pis);
...
/* Set initial color. */
void pdf_set_initial_color(gx_device_pdf* pdev,
...
int pdf_prepare_drawing(gx_device_pdf* pdev, const gs_imager_state* pis,
...
#define private_st_pdf_pattern() /* in gdevpdfc.c */                           \
	gs_private_st_suffix_add1(st_pdf_pattern, pdf_pattern_t,               \
	                          ...
	                          pdf_pattern_reloc_ptrs, st_pdf_resource,     \
	                          substitute)
...
#define PDF_IMAGE_PARAM_NAMES                                                  \
	"/BitsPerComponent", "/ColorSpace", "/Decode", "/Height",              \
	    "/ImageMask", "/Interpolate", "/Width"
#define PDF_IMAGE_PARAM_NAMES_SHORT "/BPC", "/CS", "/D", "/H", "/IM", "/I", "/W"
...
typedef struct pdf_image_writer_s {
	...
	pdf_resource_t*
	    pres_mask; /* PS2WRITE only : XObject resource for mask */
} pdf_image_writer;
...
#define pdf_image_writer_max_ptrs                                              \
	(psdf_binary_writer_max_ptrs * pdf_image_writer_num_alt_streams + 4)
...
int pdf_begin_write_image(gx_device_pdf* pdev, pdf_image_writer* piw,
                          gx_bitmap_id id, int w, int h, cos_dict_t* pnamed,
                          bool in_line);
...
int pdf_begin_image_data(gx_device_pdf* pdev, pdf_image_writer* piw,
                         ...
                         const cos_value_t* pcsvalue, int alt_writer_index);

/* Copy the data for a mask or monobit bitmap. */
int pdf_copy_mask_bits(stream* s, const byte* base, int sourcex, int raster,
                       int w, int h, byte invert);

/* Copy the data for a colored image (device pixels). */
int pdf_copy_color_bits(stream* s, const byte* base, int sourcex, int raster,
                        int w, int h, int bytes_per_pixel);

/* Complete image data. */
int pdf_complete_image_data(gx_device_pdf* pdev, pdf_image_writer* piw,
                            int data_h, int width, int bits_per_pixel);
...
int pdf_register_charproc_resource(gx_device_pdf* pdev, gs_id id,
