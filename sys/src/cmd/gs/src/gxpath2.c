			...
			return prt_none;
		{
			...
			if((x0 == pseg1->pt.x && pseg1->pt.y == y2 &&
			   ...
			   (x0 == pseg3->pt.x && pseg3->pt.y == y2 &&
			    x2 == pseg1->pt.x && pseg1->pt.y == y0)) { /* Path is a rectangle.  Return the bounding box. */
				if(x0 < x2)
...
gx_path_is_rectangular(const gx_path *ppath, gs_fixed_rect *pbox)
{
	...
	return (gx_path_subpath_count(ppath) == 1 ? gx_subpath_is_rectangular(ppath->first_subpath, pbox, &pnext) : prt_none);
}
...
nsp:
	if(psub) {
		...
		const segment *pseg;
		segment_notes notes =
		    (prev == (const segment *)psub ? sn_none : psub->next->notes);
