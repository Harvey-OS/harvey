...
#define STORE_BBOX(b) (b).p.x = px, (b).p.y = py, (b).q.x = qx, (b).q.y = qy;
		STORE_BBOX(*pbox);
                          ...
                          const subpath** ppnext)
{
	...
	if(pseg0->curve_count == 0 && (pseg1 = pseg0->next) != 0 &&
	   (pseg2 = pseg1->next) != 0 && (pseg3 = pseg2->next) != 0) {
			...
			return prt_none;
		{
			...
			if((x0 == pseg1->pt.x && pseg1->pt.y == y2 &&
			   ...
			   (x0 == pseg3->pt.x && pseg3->pt.y == y2 &&
			    x2 == pseg1->pt.x &&
			    pseg1->pt.y == y0)) { /* Path is a rectangle.
...
#define update_xy(pt) pt.x += dx, pt.y += dy
	if(ppath->box_last != 0) {
		...
		do {
			...
			case s_curve: {
				...
				code = gx_path_add_curve_notes(
				    ppath, pc->p2.x, pc->p2.y, pc->p1.x,
				    pc->p1.y, prev->pt.x, prev->pt.y,
				    prev_notes);
				break;
			}
			case s_line:
				code = gx_path_add_line_notes(
				    ppath, prev->pt.x, prev->pt.y, prev_notes);
				break;
			case s_line_close:
...
#undef sn_not_end
	 ...
	 */
	if(ppath_old->first_subpath == 0 && path_last_is_moveto(ppath_old)) {
...
gx_path_enum_next(gs_path_enum* penum, gs_fixed_point ppts[3])
{
	...
	if(pseg ==
	   0) { /* We've enumerated all the segments, but there might be */
		/* a trailing moveto. */
		const gx_path* ppath = penum->path;

		if(path_last_is_moveto(ppath) &&
		   !penum->moveto_done) { /* Handle a trailing moveto */
...
gx_path_enum_backup(gs_path_enum* penum)
{
	...
	/* we need to back up over a trailing moveto. */
	{
		...
		if(path_last_is_moveto(ppath) &&
		   penum->moveto_done) { /* Back up over the trailing moveto. */
			penum->moveto_done = false;
			return true;
		}
		{
