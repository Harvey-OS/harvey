                  ...
                  gs_memory_t* mem)
{
	...
	for(i = count; --i >= 0;)
		if(Functions[i])
			gs_function_free((gs_function_t*)Functions[i], true,
			                 mem);
                   ...
                   gs_memory_t* mem)
{
	...
	for(i = 0; i < count; ++i) {
		int code =
		    gs_function_make_scaled(pfns[i], &psfns[i], ranges, mem);

		if(code < 0) {
			fn_free_functions((const gs_function_t* const*)psfns,
			                  count, mem);
			return code;
		}
		if(step_ranges)
...
private
int
fn_ElIn_is_monotonic(const gs_function_t* pfn_common, const float* lower,
                     const float* upper, uint* mask)
{
	const gs_function_ElIn_t* const pfn =
	    (const gs_function_ElIn_t*)pfn_common;

	if(lower[0] > pfn->params.Domain[1] || upper[0] < pfn->params.Domain[0])
...
private
int
fn_ElIn_make_scaled(const gs_function_ElIn_t* pfn, gs_function_ElIn_t** ppsfn,
                    const gs_range_t* pranges, gs_memory_t* mem)
{
	gs_function_ElIn_t* psfn = gs_alloc_struct(
	    mem, gs_function_ElIn_t, &st_function_ElIn, "fn_ElIn_make_scaled");
	float* c0;
	float* c1;
	int code, i;
...
{
	uint n;
	const gs_function_ElIn_params_t* p =
	    (const gs_function_ElIn_params_t*)&pfn->params;
...
gs_function_ElIn_init(gs_function_t** ppfn,
                      const gs_function_ElIn_params_t* params, gs_memory_t* mem)
{
...
fn_1ItSg_evaluate(const gs_function_t* pfn_common, const float* in, float* out)
{
	...
	if(b1 == b0)
		...
		encoded = (arg - b0) * (pfn->params.Encode[2 * i + 1] - e0) /
		              (b1 - b0) +
		          e0;
...
private
int
fn_1ItSg_is_monotonic(const gs_function_t* pfn_common, const float* lower,
                      const float* upper, uint* mask)
{
	const gs_function_1ItSg_t* const pfn =
	    (const gs_function_1ItSg_t*)pfn_common;
	...
	*mask = 0;
	...
	for(i = 0; i < pfn->params.k; ++i) {
		...
		if(w0 > w1)
			return gs_function_is_monotonic(
			    pfn->params.Functions[i], &w1, &w0, mask);
		else
			return gs_function_is_monotonic(
			    pfn->params.Functions[i], &w0, &w1, mask);
	}
...
fn_1ItSg_make_scaled(const gs_function_1ItSg_t* pfn,
                     gs_function_1ItSg_t** ppsfn, const gs_range_t* pranges,
                     gs_memory_t* mem)
{
	...
	psfn->params.Bounds = fn_copy_values(
	    pfn->params.Bounds, pfn->params.k - 1, sizeof(float), mem);
	psfn->params.Encode = fn_copy_values(
	    pfn->params.Encode, 2 * pfn->params.k, sizeof(float), mem);
	if((code = (psfn->params.Bounds == 0 || psfn->params.Encode == 0
	   ...
	   (code = fn_scale_functions((gs_function_t***)&psfn->params.Functions,
	                              pfn->params.Functions, pfn->params.n,
	                              pranges, false, mem)) < 0) {
		gs_function_free((gs_function_t*)psfn, true, mem);
		return code;
	}
...
{
	uint n;
	const gs_function_1ItSg_params_t* p =
	    (const gs_function_1ItSg_params_t*)&pfn->params;
	int code = fn_common_serialize(pfn, s);
	int k;
	...
	if(code < 0)
		return code;
	code = sputs(s, (const byte*)&p->Bounds[0],
	             sizeof(p->Bounds[0]) * (p->k - 1), &n);
	if(code < 0)
		return code;
	return sputs(s, (const byte*)&p->Encode[0],
	             sizeof(p->Encode[0]) * (p->k * 2), &n);
}
                       ...
                       gs_memory_t* mem)
{
	...
	*ppfn = 0; /* in case of error */
	...
	{
		gs_function_1ItSg_t* pfn = gs_alloc_struct(
		    mem, gs_function_1ItSg_t, &st_function_1ItSg,
...
private
int
fn_AdOt_is_monotonic(const gs_function_t* pfn_common, const float* lower,
                     const float* upper, uint* mask)
{
	const gs_function_AdOt_t* const pfn =
	    (const gs_function_AdOt_t*)pfn_common;
	...
	for(i = 0; i < pfn->params.n; ++i) {
		int code = gs_function_is_monotonic(pfn->params.Functions[i],
		                                    lower, upper, mask);
...
fn_AdOt_make_scaled(const gs_function_AdOt_t* pfn, gs_function_AdOt_t** ppsfn,
                    const gs_range_t* pranges, gs_memory_t* mem)
{
	gs_function_AdOt_t* psfn = gs_alloc_struct(
	    mem, gs_function_AdOt_t, &st_function_AdOt, "fn_AdOt_make_scaled");
	int code;
...
{
	const gs_function_AdOt_params_t* p =
	    (const gs_function_AdOt_params_t*)&pfn->params;
	int code = fn_common_serialize(pfn, s);
	int k;
                      ...
                      const gs_function_AdOt_params_t* params, gs_memory_t* mem)
{
	...
	*ppfn = 0; /* in case of error */
		...
		return_error(gs_error_rangecheck);
	{
		...
		float* domain = (float*)gs_alloc_byte_array(
		    mem, 2 * m, sizeof(float), "gs_function_AdOt_init(Domain)");
		int i, j;
