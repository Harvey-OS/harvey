...
/* Copyright (C) 1992, 1993, 1994, 1996 by Aladdin Enterprises.  All rights
  reserved.
...
#define getMagnification                                                       \
	((pdev->x_pixels_per_inch > (MAX_DPI >> 1))                            \
	     ? 0                                                               \
	     : (pdev->x_pixels_per_inch > (MAX_DPI >> 2)) ? 1 : 2)
...
private
dev_proc_print_page(imagen_print_page);
private
dev_proc_open_device(imagen_prn_open);
private
dev_proc_close_device(imagen_prn_close);
...
const gx_device_printer far_data gs_imagen_device =
    prn_device(/*prn_std_procs*/ imagen_procs, "imagen", WIDTH_10THS,
               HEIGHT_10THS, MAX_DPI,          /* x_dpi */
               MAX_DPI,                        /* y_dpi */
...
{ /* iWrite */
	...
	if(BYTE_STREAM && (Val == QUOTE_CHAR || Val == EOF_CHAR ||
...
imagen_print_page(gx_device_printer* pdev, FILE* prn_stream)
{
	...
	byte* in =
	    (byte*)gs_malloc(pdev->memory, BIGSIZE, line_size / BIGSIZE + 1,
	...
	swatchMap =
	    (byte*)gs_malloc(pdev->memory, BIGSIZE, swatchCount / BIGSIZE + 1,
	...
	while(lnum <= pdev->height) {
		...
		/* Check if we don't have a full-height row of swatches at end
		 * of page */
		if(lnum + lastLine > pdev->height) {
			/* back up last row so it overlaps with previous.  Not a
			  problem
			  on a laser printer, because the overlapping part will
			  be identical */
		...
		for(swatchLine = 0; swatchLine <= lastLine; swatchLine++) {
			...
			for(temp = in + line_size;
			    temp < in + line_size + BIGSIZE; temp++) {
				*temp = 0;
			} /* for temp */

			/* get one line */
			gdev_prn_copy_scan_lines(pdev, lnum + swatchLine, in,
			                         line_size);
			DebugMsg(5, "Got scan line %d ", lnum + swatchLine);
			DebugMsg(5, "line %d \n", swatchLine);

			/* interleave scan line into swatch buffer */
			/* a swatch is a 4 byte * 32 byte square.  Swatches are
			   placed
			   next to each other.  The first scan line maps into
			   the first
			   four bytes of the first swatch, then the first four
			   of the second
			   swatch, etc.
			...
			while(temp < in + line_size) {
				...
				*(BIGTYPE*)swatch = *(BIGTYPE*)temp;
				if(*(BIGTYPE*)temp) {
					...
					swatchMap[(swatch - out) /
					          TotalBytesPerSw] = (byte)1;
				} /* if not zero */

				temp += (BIGSIZE > HorzBytesPerSw)
				            ? HorzBytesPerSw
				            : BIGSIZE;
				swatch += (BIGSIZE > HorzBytesPerSw)
				              ? HorzBytesPerSw
				              : BIGSIZE;
				...
				if(((temp - in) % HorzBytesPerSw) == 0) {
					swatch +=
					    (TotalBytesPerSw - HorzBytesPerSw);
				} /* if need to skip */
			}         /* while < line_size */
		...
		while(startSwatch < swatchCount) {
			if(swatchMap[startSwatch] == 0) {
				...
				/* vertical position: scan line, shifted for
				 * magnification */
				iWrite(prn_stream, iABS_V);
				iWrite2(prn_stream, lnum << Magnify);

				/* horizontal position = swatch number * 32
				 * bits/swatch */
				iWrite(prn_stream, iABS_H);
				iWrite2(prn_stream, startSwatch * HorzBitsPerSw
				                        << Magnify);
				iWrite(prn_stream, iBITMAP); /* start bitmap */
				iWrite(prn_stream, 0x07); /* bit OR with page */
				iWrite(prn_stream,
				       (endSwatch - startSwatch)); /* horizontal
			   number of swatches */
				iWrite(prn_stream,
				       1); /* vertical number of swatches */
				/* write out swatch buffer */
				for(swatch =
				        out + startSwatch * TotalBytesPerSw;
				    swatch < out + endSwatch * TotalBytesPerSw;
				    swatch++) {
					iWrite(prn_stream, *swatch);
				} /* for swatch */

				/* swatches have been printed, see if there are
				   still
				   more in this row */
	...
	gs_free(pdev->memory, (char*)swatchMap, BIGSIZE,
	        swatchCount / BIGSIZE + 1, "imagen_print_page(swatchMap)");
	gs_free(pdev->memory, (char*)out, TotalBytesPerSw, swatchCount + 1,
	        "imagen_print_page(out)");
	gs_free(pdev->memory, (char*)in, BIGSIZE, line_size / BIGSIZE + 1,
	        "imagen_print_page(in)");
	/* ----------------------------------------- */
