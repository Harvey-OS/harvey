...
#define getMagnification ( \
    (pdev->x_pixels_per_inch > (MAX_DPI >> 1)) ? 0 : (pdev->x_pixels_per_inch > (MAX_DPI >> 2)) ? 1 : 2)
...
{ /* iWrite */
	...
	if(BYTE_STREAM &&
	   (Val == QUOTE_CHAR || Val == EOF_CHAR || Val == EXTRA_QUOTE1 || Val == EXTRA_QUOTE2 || Val == EXTRA_QUOTE3 || Val == EXTRA_QUOTE4)) {
