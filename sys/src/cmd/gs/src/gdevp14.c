...
#define pdf14_procs(get_color_mapping_procs, get_color_comp_index)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   \
	{                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            \
		pdf14_open, /* open */ NULL, /* get_initial_matrix */ NULL, /* sync_output */ pdf14_output_page, /* output_page */ pdf14_close, /* close */ pdf14_encode_color, /* rgb_map_rgb_color */ pdf14_decode_color, /* gx_default_rgb_map_color_rgb */ pdf14_fill_rectangle, /* fill_rectangle */ NULL, /* tile_rectangle */ NULL, /* copy_mono */ NULL, /* copy_color */ NULL, /* draw_line */ NULL, /* get_bits */ NULL, /* get_params */ pdf14_put_params, /* put_params */ NULL, /* map_cmyk_color */ NULL, /* get_xfont_procs */ NULL, /* get_xfont_device */ NULL, /* map_rgb_alpha_color */ NULL, /* get_page_device */ NULL, /* get_alpha_bits */ NULL, /* copy_alpha */ NULL, /* get_band */ NULL, /* copy_rop */ pdf14_fill_path, /* fill_path */ pdf14_stroke_path, /* stroke_path */ NULL, /* fill_mask */ NULL, /* fill_trapezoid */ NULL, /* fill_parallelogram */ NULL, /* fill_triangle */ NULL, /* draw_thin_line */ NULL, /* begin_image */ NULL, /* image_data */ NULL, /* end_image */ NULL, /* strip_tile_rectangle */ NULL, /* strip_copy_rop, */ NULL, /* get_clipping_box */ pdf14_begin_typed_image, /* begin_typed_image */ NULL, /* get_bits_rectangle */ NULL, /* map_color_rgb_alpha */ pdf14_create_compositor, /* create_compositor */ NULL, /* get_hardware_params */ pdf14_text_begin, /* text_begin */ NULL, /* finish_copydevice */ pdf14_begin_transparency_group, pdf14_end_transparency_group, pdf14_begin_transparency_mask, pdf14_end_transparency_mask, NULL, /* discard_transparency_layer */ get_color_mapping_procs, /* get_color_mapping_procs */ get_color_comp_index, /* get_color_comp_index */ pdf14_encode_color, /* encode_color */ pdf14_decode_color /* decode_color */ \
	}
	...
	if(y0 < y1) {
		...
		for(y = y0; y < y1; ++y) {
			...
			*/
				...
				if(nos_knockout) {
					byte *nos_shape_ptr = nos_has_shape ? &nos_ptr[x + nos_shape_offset] : NULL;
...
c_pdf14trans_write(const gs_composite_t *pct, byte *data, uint *psize)
{
	...
	byte buf[21 + sizeof(pparams->Background) + sizeof(pparams->GrayBackground) + sizeof(pparams->bbox)];
	byte *pbuf = buf;
		...
		if(params.function_is_identity) {
			...
			for(i = 0; i < MASK_TRANSFER_FUNCTION_SIZE; i++) {
				params.transfer_fn[i] = (byte)floor(i *
									(255.0 / (MASK_TRANSFER_FUNCTION_SIZE - 1)) +
								    0.5);
			}
		} else {
...
{
	return (pct->type == &gs_composite_pdf14trans_type || pct->type == &gs_composite_pdf14trans_no_clist_writer_type);
}
...
private
const gx_device_procs pdf14_clist_Gray_procs =
...
private
const gx_device_procs pdf14_clist_RGB_procs =
			      ...
			      const gs_composite_t *pct, gs_imager_state *pis, gs_memory_t *mem)
{
		 ...
		 */
			{
				...
				code = dev_proc(pdev->target, create_compositor)(pdev->target, pcdev, &pctemp, pis, mem);
				*pcdev = dev;
				return code;
			}
		case PDF14_POP_DEVICE:
	...
	code = dev_proc(pdev->target, create_compositor)(pdev->target, pcdev, pct, pis, mem);
				...
				gx_device **pcdev, gs_imager_state *pis, gs_memory_t *mem)
{
	...
	case PDF14_POP_DEVICE:
	     ...
	     */
		code = cmd_put_halftone((gx_device_clist_writer *)(((pdf14_clist_device *)dev)->target), pis->dev_ht);
		break;
	default:
