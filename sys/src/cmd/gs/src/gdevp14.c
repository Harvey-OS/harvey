...
return 0;
case 0:
return ENUM_OBJ(buf->saved);
case 1:
return ENUM_OBJ(buf->data);
case 2:
return ENUM_OBJ(buf->transfer_fn);
ENUM_PTRS_END
...
gs_private_st_ptrs2(st_pdf14_ctx, pdf14_ctx, "pdf14_ctx", pdf14_ctx_enum_ptrs,
                    pdf14_ctx_reloc_ptrs, stack, maskbuf);
...
private
int pdf14_open(gx_device* pdev);
private
dev_proc_close_device(pdf14_close);
private
int pdf14_output_page(gx_device* pdev, int num_copies, int flush);
private
dev_proc_put_params(pdf14_put_params);
private
dev_proc_encode_color(pdf14_encode_color);
private
dev_proc_decode_color(pdf14_decode_color);
private
dev_proc_fill_rectangle(pdf14_fill_rectangle);
private
dev_proc_fill_rectangle(pdf14_mark_fill_rectangle);
private
dev_proc_fill_rectangle(pdf14_mark_fill_rectangle_ko_simple);
private
dev_proc_fill_path(pdf14_fill_path);
private
dev_proc_stroke_path(pdf14_stroke_path);
private
dev_proc_begin_typed_image(pdf14_begin_typed_image);
private
dev_proc_text_begin(pdf14_text_begin);
private
dev_proc_create_compositor(pdf14_create_compositor);
private
dev_proc_create_compositor(pdf14_forward_create_compositor);
private
dev_proc_begin_transparency_group(pdf14_begin_transparency_group);
private
dev_proc_end_transparency_group(pdf14_end_transparency_group);
private
...
private
const gx_color_map_procs* pdf14_get_cmap_procs(const gs_imager_state*,
                                               const gx_device*);
...
gs_private_st_composite_use_final(st_pdf14_device, pdf14_device, "pdf14_device",
                                  pdf14_device_enum_ptrs,
                                  pdf14_device_reloc_ptrs, gx_device_finalize);

const pdf14_device gs_pdf14_Gray_device = {
    std_device_color_stype_body(pdf14_device, &pdf14_Gray_procs, "pdf14gray",
                                &st_pdf14_device, XSIZE, YSIZE, X_DPI, Y_DPI, 8,
                                255, 256),
    {0}};

const pdf14_device gs_pdf14_RGB_device = {
    std_device_color_stype_body(pdf14_device, &pdf14_RGB_procs, "pdf14RGB",
                                &st_pdf14_device, XSIZE, YSIZE, X_DPI, Y_DPI,
                                24, 255, 256),
    {0}};

const pdf14_device gs_pdf14_CMYK_device = {
    std_device_std_color_full_body_type(
        pdf14_device, &pdf14_CMYK_procs, "PDF14cmyk", &st_pdf14_device, XSIZE,
        YSIZE, X_DPI, Y_DPI, 32, 0, 0, 0, 0, 0, 0),
    {0}};
...
case 0:
return ENUM_OBJ(pdev->ctx);
case 1:
ENUM_RETURN(gx_device_enum_ptr(pdev->target));
ENUM_PTRS_END
private
RELOC_PTRS_WITH(pdf14_device_reloc_ptrs, pdf14_device* pdev)
{
	RELOC_VAR(pdev->ctx);
...
private
pdf14_buf*
pdf14_buf_new(gs_int_rect* rect, bool has_alpha_g, bool has_shape, int n_chan,
              gs_memory_t* memory)
{
	pdf14_buf* result;
	...
	result =
	    gs_alloc_struct(memory, pdf14_buf, &st_pdf14_buf, "pdf14_buf_new");
	if(result == NULL)
		return result;
	...
	if(height < 0) {
		...
		if(has_alpha_g) {
			...
			memset(result->data + alpha_g_plane * planestride, 0,
			       planestride);
		}
	}
...
pdf14_ctx_new(gs_int_rect* rect, int n_chan, bool additive, gs_memory_t* memory)
{
	...
	result =
	    gs_alloc_struct(memory, pdf14_ctx, &st_pdf14_ctx, "pdf14_ctx_new");
	if(result == NULL)
		return result;
                              ...
                              gs_blend_mode_t blend_mode)
{
	...
	buf =
	    pdf14_buf_new(rect, !isolated, has_shape, ctx->n_chan, ctx->memory);
	if_debug3('v', "[v]push buf: %d x %d, %d channels\n", buf->rect.p.x,
	          buf->rect.p.y, buf->n_chan);
	if(buf == NULL)
	...
	if(backdrop == NULL) {
		memset(buf->data, 0,
		       buf->planestride *
		           (buf->n_chan + (buf->has_shape ? 1 : 0)));
	} else {
		/* make copy of backdrop for compositing */
		byte* buf_plane = buf->data;
		byte* tos_plane =
		    tos->data + buf->rect.p.x - tos->rect.p.x +
...
pdf14_pop_transparency_group(pdf14_ctx* ctx)
{
	...
	if(y0 < y1) {
		...
		int tos_alpha_g_offset =
		    tos_shape_offset + (tos->has_shape ? tos_planestride : 0);
		...
		if_debug6('v', "pdf14_pop_transparency_group y0 = %d, y1 = %d, "
		               "w = %d, alpha = %d, shape = %d, bm = %d\n",
		...
		if(maskbuf != NULL) {
			mask_ptr =
			    maskbuf->data + x0 - maskbuf->rect.p.x +
		...
		for(y = y0; y < y1; ++y) {
			for(x = 0; x < width; ++x) {
				...
				/* Complement the components for subtractive
				 * color spaces */
				if(additive) {
					for(i = 0; i < n_chan; ++i) {
						tos_pixel[i] = tos_ptr
					...
					tos_pixel[num_comp] =
					    tos_ptr[x +
					            num_comp * tos_planestride];
					nos_pixel[num_comp] =
					    nos_ptr[x +
					            num_comp * nos_planestride];
				}
				...
				if(mask_ptr != NULL) {
					...
					/*
					* The mask data is really monochrome.
					* Thus for additive (RGB)
					* we use the R channel for alpha since R
					* = G = B.  For
					* subtractive (CMYK) we use the K
					* channel.
					*/
					if(mask_alpha == 255) {
						/* todo: rgba->mask */
				...
				if(nos_knockout) {
					...
					art_pdf_composite_knockout_isolated_8(
					    nos_pixel, nos_shape_ptr, tos_pixel,
					    n_chan - 1, tos_shape, pix_alpha,
					    shape);
				} else if(tos_isolated) {
					art_pdf_composite_group_8(
					    nos_pixel, nos_alpha_g_ptr,
					    tos_pixel, n_chan - 1, pix_alpha,
					    blend_mode);
				} else {
					byte tos_alpha_g =
					    tos_ptr[x + tos_alpha_g_offset];
				...
				/* Complement the results for subtractive color
				 * spaces */
				if(additive) {
					for(i = 0; i < n_chan; ++i) {
						nos_ptr[x +
...
private
int
dump_planar_rgba(gs_memory_t* mem, const byte* buf, int width, int height,
                 int rowstride, int planestride)
{
	...
	info_ptr->x_pixels_per_unit = (png_uint_32)(96.0 * (100.0 / 2.54));
	info_ptr->y_pixels_per_unit = (png_uint_32)(96.0 * (100.0 / 2.54));
...
pdf14_put_image(pdf14_device* pdev, gs_imager_state* pis, gx_device* target)
{
...
#ifdef DUMP_TO_PNG
	dump_planar_rgba(pdev->memory, buf_ptr, width, height, buf->rowstride,
	                 buf->planestride);
#endif
	...
	/*
	 * Set color space to either Gray, RGB, or CMYK in preparation for
	 * sending
	 * an image.
	 */
	switch(num_comp) {
	...
	pmat.tx = 0;
	pmat.ty = 0;
	code = dev_proc(target, begin_typed_image)(
	    target, pis, &pmat, (gs_image_common_t*)&image, NULL, NULL, NULL,
	    pis->memory, &info);
	if(code < 0)
		return code;

	linebuf =
	    gs_alloc_bytes(pdev->memory, width * num_comp, "pdf14_put_image");
	for(y = 0; y < height; y++) {
		...
		for(x = 0; x < width; x++) {
			...
			/* composite RGBA (or CMYKA, etc.) pixel with over solid
			 * background */
			a = buf_ptr[x + planestride * num_comp];

			if((a + 1) & 0xfe) {
				a ^= 0xff;
				for(comp_num = 0; comp_num < num_comp;
				    comp_num++) {
					comp =
					    buf_ptr[x + planestride * comp_num];
					tmp = ((bg - comp) * a) + 0x80;
					comp += (tmp + (tmp >> 8)) >> 8;
...
pdf14_output_page(gx_device* dev, int num_copies, int flush)
{
	...
	if(pdev->target != NULL)
		return (*dev_proc(pdev->target, output_page))(
		    pdev->target, num_copies, flush);
	return 0;
}
...
private
int
pdf14_fill_path(gx_device* dev, const gs_imager_state* pis, gx_path* ppath,
                ...
                const gx_clip_path* pcpath)
{
	...
	return gx_default_fill_path(dev, &new_is, ppath, params, pdcolor,
	                            pcpath);
}

private
int
pdf14_stroke_path(gx_device* dev, const gs_imager_state* pis, gx_path* ppath,
...
pdf14_forward_device_procs(gx_device* dev)
{
	...
	/*
	 * We are using gx_device_forward_fill_in_procs to set the various
	 * procs.
	 * This will ensure that any new device procs are also set.  However
	 * that
	 * routine only changes procs which are NULL.  Thus we start by setting
	 * all
	 * procs to NULL.
	 */
...
pdf14_determine_default_blend_cs(gx_device* pdev)
{
	if(pdev->color_info.polarity == GX_CINFO_POLARITY_SUBTRACTIVE)
		...
		/*
		 * Note:  We do not allow the SeparationOrder device parameter
		 * for
		 * additive devices.  Thus we always have 1 colorant for
		 * DeviceGray
		 * and 3 colorants for DeviceRGB.  We do not currently support
		 * blending in a DeviceGray color space.  Thus we oniy use
		 * DeviceRGB.
		 */
		return DeviceRGB;
	}
...
private
int
get_pdf14_device_proto(gx_device* dev, const pdf14_device** pdevproto)
{
...
private
int
pdf14_recreate_device(gs_memory_t* mem, gs_imager_state* pis, gx_device* dev)
{
	pdf14_device* pdev = (pdf14_device*)dev;
                           ...
                           const gs_pdf14trans_t* pdf14pct, gs_memory_t* mem)
{
	...
	case PDF14_BEGIN_TRANS_GROUP:
		code =
		    gx_begin_transparency_group(pis, pdev, &pdf14pct->params);
		break;
	...
	*pcdev = dev;
	if(gs_is_pdf14trans_compositor(pct)) {
		...
		if(pdf14pct->params.pdf14_op == PDF14_PUSH_DEVICE)
			return gx_update_pdf14_compositor(dev, pis, pdf14pct,
			                                  mem);
		return 0;
	}
...
private
int
pdf14_fill_rectangle(gx_device* dev, int x, int y, int w, int h,
                     gx_color_index color)
{
	...
	if(w <= 0 || h <= 0)
		return 0;
	if(buf->knockout)
		return pdf14_mark_fill_rectangle_ko_simple(dev, x, y, w, h,
		                                           color);
	else
...
pdf14_begin_transparency_group(gx_device* dev,
                               ...
                               gs_transparency_state_t** ppts, gs_memory_t* mem)
{
	pdf14_device* pdev = (pdf14_device*)dev;
	...
	if_debug4('v', "[v]begin_transparency_group, I = %d, K = %d, alpha = "
	               "%g, bm = %d\n",
	          ptgp->Isolated, ptgp->Knockout, alpha, pis->blend_mode);
	code = pdf14_push_transparency_group(
	    pdev->ctx, &rect, ptgp->Isolated, ptgp->Knockout,
	    (byte)floor(255 * alpha + 0.5),
	    (byte)floor(255 * pis->shape.alpha + 0.5), pis->blend_mode);
	return code;
}

private
int
pdf14_end_transparency_group(gx_device* dev, gs_imager_state* pis,
                              ...
                              gs_transparency_state_t** ppts, gs_memory_t* mem)
{
	pdf14_device* pdev = (pdf14_device*)dev;
	byte bg_alpha = 0;
	byte* transfer_fn = (byte*)gs_alloc_bytes(
	    pdev->ctx->memory, 256, "pdf14_push_transparency_mask");
	...
	return pdf14_push_transparency_mask(pdev->ctx, &pdev->ctx->rect,
	                                    bg_alpha, transfer_fn);
}

private
int
pdf14_end_transparency_mask(gx_device* dev, gs_transparency_mask_t** pptm)
{
	pdf14_device* pdev = (pdf14_device*)dev;
                          ...
                          gx_color_index color)
{
	...
	if_debug7('v', "[v]pdf14_mark_fill_rectangle, (%d, %d), %d x %d color "
	               "= %lx  bm %d, nc %d,\n",
	...
	line =
	    buf->data + (x - buf->rect.p.x) + (y - buf->rect.p.y) * rowstride;

	for(j = 0; j < h; ++j) {
		dst_ptr = line;
		for(i = 0; i < w; ++i) {
			/* Complement the components for subtractive color
			 * spaces */
			if(additive) {
				for(k = 0; k < num_chan; ++k)
					dst[k] = dst_ptr[k * planestride];
			} else { /* Complement the components for subtractive
			            color spaces */
			...
			if(has_alpha_g) {
				int tmp = (255 - dst_ptr[alpha_g_off]) *
				              (255 - src_alpha) +
				          0x80;
				dst_ptr[alpha_g_off] =
				    255 - ((tmp + (tmp >> 8)) >> 8);
			...
			if(has_shape) {
				int tmp =
				    (255 - dst_ptr[shape_off]) * (255 - shape) +
				    0x80;
				dst_ptr[shape_off] =
				    255 - ((tmp + (tmp >> 8)) >> 8);
			}
                                    ...
                                    gx_color_index color)
{
	...
	if_debug6('v', "[v]pdf14_mark_fill_rectangle_ko_simple, (%d, %d), %d x "
	               "%d color = %lx  bm %d, nc %d,\n",
	...
	for(j = 0; j < h; ++j) {
		...
		for(i = 0; i < w; ++i) {
			...
			art_pdf_composite_knockout_simple_8(
			    dst, has_shape ? dst_ptr + shape_off : NULL, src,
			    num_comp, opacity);
			/* Complement the results for subtractive color spaces
			 */
			if(additive) {
...
private
cmap_proc_gray(pdf14_cmap_gray_direct);
private
cmap_proc_rgb(pdf14_cmap_rgb_direct);
private
cmap_proc_cmyk(pdf14_cmap_cmyk_direct);
private
cmap_proc_rgb_alpha(pdf14_cmap_rgb_alpha_direct);
private
cmap_proc_separation(pdf14_cmap_separation_direct);
private
cmap_proc_devicen(pdf14_cmap_devicen_direct);
private
cmap_proc_is_halftoned(pdf14_cmap_is_halftoned);

private
const gx_color_map_procs pdf14_cmap_many = {
    pdf14_cmap_gray_direct,       pdf14_cmap_rgb_direct,
    pdf14_cmap_cmyk_direct,       pdf14_cmap_rgb_alpha_direct,
    pdf14_cmap_separation_direct, pdf14_cmap_devicen_direct,
...
private
void
pdf14_cmap_gray_direct(frac gray, gx_device_color* pdc,
                      ...
                      gs_color_select_t select)
{
	...
	/* map to the color model */
	dev_proc(dev, get_color_mapping_procs)(dev)
	    ->map_rgb(dev, pis, r, g, b, cm_comps);
...
private
void
pdf14_cmap_rgb_alpha_direct(frac r, frac g, frac b, frac alpha,
                            ...
                            gx_device* dev, gs_color_select_t select)
{
	...
	if(alpha != frac_1) {
		...
		for(i = 0; i < ncomps; i++)
			cm_comps[i] =
			    (frac)((long)cm_comps[i] * alpha) / frac_1 +
			    alpha_bias;
	}
	...
	/* encode as a color index */
	if(dev_proc(dev, map_rgb_alpha_color) !=
	       gx_default_map_rgb_alpha_color &&
	   (cv_alpha = frac2cv(alpha)) != gx_max_color_value)
		color = dev_proc(dev, map_rgb_alpha_color)(dev, cv[0], cv[1],
		                                           cv[2], cv_alpha);
	else
		color = dev_proc(dev, encode_color)(dev, cv);
...
private
void
pdf14_cmap_separation_direct(frac all, gx_device_color* pdc,
                             const gs_imager_state* pis, gx_device* dev,
                             gs_color_select_t select)
{
	int i, ncomps = dev->color_info.num_components;
	...
	if(pis->color_component_map.sep_type == SEP_ALL) {
		...
		/* map to the color model */
		map_components_to_colorants(
		    &comp_value, &(pis->color_component_map), cm_comps);
	}

	/* apply the transfer function(s); convert to color values */
	if(additive)
		for(i = 0; i < ncomps; i++)
			cv[i] = frac2cv(gx_map_color_frac(
			    pis, cm_comps[i], effective_transfer[i]));
	else
		for(i = 0; i < ncomps; i++)
			cv[i] = frac2cv(
			    frac_1 -
			    gx_map_color_frac(pis, (frac)(frac_1 - cm_comps[i]),
...
private
void
pdf14_cmap_devicen_direct(const frac* pcc, gx_device_color* pdc,
                          const gs_imager_state* pis, gx_device* dev,
                          gs_color_select_t select)
{
	int i, ncomps = dev->color_info.num_components;
...
int
gs_pdf14_device_push(gs_memory_t* mem, gs_imager_state* pis, gx_device** pdev,
                     gx_device* target)
{
	const pdf14_device* dev_proto;
	pdf14_device* p14dev;
	...
	if(code < 0)
		return code;
	code = gs_copydevice((gx_device**)&p14dev, (const gx_device*)dev_proto,
	                     mem);
	if(code < 0)
		return code;
...
#define private_st_gs_pdf14trans_t()                                           \
	gs_private_st_ptrs1(st_pdf14trans, gs_pdf14trans_t, "gs_pdf14trans_t", \
	                    st_pdf14trans_enum_ptrs, st_pdf14trans_reloc_ptrs, \
...
{
	const gs_pdf14trans_params_t* pparams =
	    &((const gs_pdf14trans_t*)pct)->params;
	...
	*pbuf++ = opcode; /* 1 byte */
	...
	case PDF14_BEGIN_TRANS_GROUP:
		/*
		 * The bbox data is floating point.  We are not currently using
		 * it.
		 * So we are not currently putting it into the clist.  We are
		 * also
		 * not using the color space.
		 */
		*pbuf++ =
		    (pparams->Isolated & 1) + ((pparams->Knockout & 1) << 1);
	...
	case PDF14_BEGIN_TRANS_MASK:
		...
		*pbuf++ = pparams->Background_components;
		if(pparams->Background_components) {
			...
			pbuf += l;
			memcpy(pbuf, &pparams->GrayBackground,
			       sizeof(pparams->GrayBackground));
...
#undef put_value
	...
	*psize = need;
	...
	if(mask_size) /* Include the transfer mask data if present */
		memcpy(data + need - mask_size, pparams->transfer_fn,
		       mask_size);
...
private
int
c_pdf14trans_read(gs_composite_t** ppct, const byte* data, uint size,
                  gs_memory_t* mem)
{
	...
	case PDF14_BEGIN_TRANS_GROUP:
		/*
		 * We are currently not using the bbox or the colorspace so they
		 * were
	...
	case PDF14_BEGIN_TRANS_MASK:
		...
		if(params.Background_components) {
			...
			data += l;
			memcpy(&params.GrayBackground, data,
			       sizeof(params.GrayBackground));
		...
		if(params.function_is_identity) {
			...
			for(i = 0; i < MASK_TRANSFER_FUNCTION_SIZE; i++) {
				params.transfer_fn[i] = (byte)floor(
				    i * (255.0 /
				         (MASK_TRANSFER_FUNCTION_SIZE - 1)) +
				    0.5);
			}
		} else {
...
c_pdf14trans_create_default_compositor(const gs_composite_t* pct,
                                       gx_device** pp14dev, gx_device* tdev,
...
private
composite_clist_write_update(c_pdf14trans_clist_write_update);
private
composite_clist_read_update(c_pdf14trans_clist_read_update);
 ...
 */
int
gs_create_pdf14trans(gs_composite_t** ppct,
                     const gs_pdf14trans_params_t* pparams, gs_memory_t* mem)
{
	gs_pdf14trans_t* pct;

	rc_alloc_struct_0(pct, gs_pdf14trans_t, &st_pdf14trans, mem,
	                  return_error(gs_error_VMerror),
 ...
 */
int
send_pdf14trans(gs_imager_state* pis, gx_device* dev, gx_device** pcdev,
                gs_pdf14trans_params_t* pparams, gs_memory_t* mem)
{
...
gs_private_st_suffix_add0_final(st_pdf14_clist_device, pdf14_clist_device,
                                "pdf14_clist_device",
                                device_c_pdf14_clist_enum_ptrs,
                                device_c_pdf14_clist_reloc_ptrs,
...
private
dev_proc_create_compositor(pdf14_clist_create_compositor);
private
dev_proc_create_compositor(pdf14_clist_forward_create_compositor);
private
dev_proc_fill_path(pdf14_clist_fill_path);
private
dev_proc_stroke_path(pdf14_clist_stroke_path);
private
dev_proc_text_begin(pdf14_clist_text_begin);
private
dev_proc_begin_image(pdf14_clist_begin_image);
private
dev_proc_begin_typed_image(pdf14_clist_begin_typed_image);

private
const gx_device_procs pdf14_clist_Gray_procs = pdf14_clist_procs(
    gx_default_DevGray_get_color_mapping_procs,
    gx_default_DevGray_get_color_comp_index, gx_default_8bit_map_gray_color,
    gx_default_8bit_map_color_gray);

private
const gx_device_procs pdf14_clist_RGB_procs = pdf14_clist_procs(
    gx_default_DevRGB_get_color_mapping_procs,
    gx_default_DevRGB_get_color_comp_index, gx_default_rgb_map_rgb_color,
    gx_default_rgb_map_color_rgb);

private
const gx_device_procs pdf14_clist_CMYK_procs =
...
const pdf14_clist_device pdf14_clist_Gray_device = {
    std_device_color_stype_body(pdf14_clist_device, &pdf14_clist_Gray_procs,
                                "pdf14clistgray", &st_pdf14_clist_device, XSIZE,
                                YSIZE, X_DPI, Y_DPI, 8, 255, 256),
    {0}};

const pdf14_clist_device pdf14_clist_RGB_device = {
    std_device_color_stype_body(pdf14_clist_device, &pdf14_clist_RGB_procs,
                                "pdf14clistRGB", &st_pdf14_clist_device, XSIZE,
                                YSIZE, X_DPI, Y_DPI, 24, 255, 256),
    {0}};

const pdf14_clist_device pdf14_clist_CMYK_device = {
    std_device_std_color_full_body_type(
        pdf14_clist_device, &pdf14_clist_CMYK_procs, "PDF14clistcmyk",
        &st_pdf14_clist_device, XSIZE, YSIZE, X_DPI, Y_DPI, 32, 0, 0, 0, 0, 0,
        0),
    {0}};
                          ...
                          gx_device** ppdev, gx_device* target)
{
	...
	code =
	    gs_copydevice((gx_device**)&pdev, (const gx_device*)dev_proto, mem);
	if(code < 0)
		return code;
                           ...
                           gx_device* dev)
{
	...
	pdf14_forward_device_procs(dev);
	set_dev_proc(dev, create_compositor,
	             pdf14_clist_forward_create_compositor);
	return 0;
}
                              ...
                              gs_memory_t* mem)
{
	...
	if(gs_is_pdf14trans_compositor(pct)) {
		...
		case PDF14_PUSH_DEVICE:
			...
			pdev->saved_target_color_info =
			    pdev->target->color_info;
			...
			if(code < 0)
				return code;
			/*
			 * This routine is part of the PDF 1.4 clist write
			 * device.
			 * Change the compositor procs to not create another
			 * since we
			 ...
			 */
			{
				...
				code =
				    dev_proc(pdev->target, create_compositor)(
		...
		case PDF14_POP_DEVICE:
			...
			/*
			 * Make sure that the transfer funtions, etc. are
			 * current.
			 */
		...
		case PDF14_BEGIN_TRANS_GROUP:
			/*
			 * Keep track of any changes made in the blending
			 * parameters.
			 */
	...
	code = dev_proc(pdev->target, create_compositor)(pdev->target, pcdev,
	                                                 pct, pis, mem);
	if(*pcdev != pdev->target)
		rc_assign(pdev->target, *pcdev,
                                      ...
                                      gs_imager_state* pis, gs_memory_t* mem)
{
	...
	*pcdev = dev;
	if(gs_is_pdf14trans_compositor(pct)) {
		...
		if(pdf14pct->params.pdf14_op == PDF14_PUSH_DEVICE)
			return pdf14_clist_create_compositor(dev, &ndev, pct,
			                                     pis, mem);
		return 0;
	}
	code = dev_proc(tdev, create_compositor)(tdev, &ndev, pct, pis, mem);
                      ...
                      const gx_clip_path* pcpath)
{
	 ...
	 * blending parameters.  This is needed since the fill_rectangle
	 * routines
...
private
int
pdf14_clist_begin_image(gx_device* dev, const gs_imager_state* pis,
                        ...
                        gx_image_enum_common_t** pinfo)
{
	...
	return gx_default_begin_image(dev, pis, pim, format, prect, pdcolor,
	                              pcpath, memory, pinfo);
}
...
c_pdf14trans_clist_write_update(const gs_composite_t* pcte, gx_device* dev,
                                gx_device** pcdev, gs_imager_state* pis,
                                gs_memory_t* mem)
{
	...
	case PDF14_PUSH_DEVICE:
		...
		/*
		 * Set the color_info of the clist device to match the
		 * compositing
		 ...
		 * imager state to use transfer functions on our color values.
		 * The
		 * transfer functions will be applied at the end after we have
		 * done
	...
	case PDF14_POP_DEVICE:
		/*
		 * Ensure that the tranfer functions, etc.  are current before
		 * we
		 * dump our transparency image to the output device.
		 */
		code = cmd_put_halftone(
		    (gx_device_clist_writer*)(((pdf14_clist_device*)dev)
		                                  ->target),
		    pis->dev_ht);
		break;
	default:
...
c_pdf14trans_clist_read_update(gs_composite_t* pcte, gx_device* cdev,
                               gx_device* tdev, gs_imager_state* pis,
                               gs_memory_t* mem)
{
	...
	/*
	 * We only handle the push/pop operations. Save and restore the
	 * color_info
