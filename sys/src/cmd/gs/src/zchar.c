...
/* Forward references */
private
bool map_glyph_to_char(const gs_memory_t* mem, const ref*, const ref*, ref*);
private
int finish_show(i_ctx_t*);
private
int op_show_cleanup(i_ctx_t*);
private
int op_show_return_width(i_ctx_t*, uint, double*);
...
zashow(i_ctx_t* i_ctx_p)
{
	...
	if(code < 0 || (code = op_show_setup(i_ctx_p, op)) != 0 ||
	   (code = gs_ashow_begin(igs, axy[0], axy[1], op->value.bytes,
	                          r_size(op), imemory, &penum)) < 0)
		return code;
...
zwidthshow(i_ctx_t* i_ctx_p)
{
	...
	if((code = num_params(op - 2, 2, cxy)) < 0 ||
	   ...
	   (code = gs_widthshow_begin(
	        igs, cxy[0], cxy[1], (gs_char)op[-1].value.intval,
	        op->value.bytes, r_size(op), imemory, &penum)) < 0)
		return code;
...
zawidthshow(i_ctx_t* i_ctx_p)
{
	...
	if((code = num_params(op - 4, 2, cxy)) < 0 ||
	   ...
	   (code = op_show_setup(i_ctx_p, op)) != 0 ||
	   (code = gs_awidthshow_begin(
	        igs, cxy[0], cxy[1], (gs_char)op[-3].value.intval, axy[0],
	        axy[1], op->value.bytes, r_size(op), imemory, &penum)) < 0)
		return code;
...
zkshow(i_ctx_t* i_ctx_p)
{
	...
	if((code = op_show_setup(i_ctx_p, op)) != 0 ||
	   (code = gs_kshow_begin(igs, op->value.bytes, r_size(op), imemory,
	                          &penum)) < 0)
		return code;
...
zstringwidth(i_ctx_t* i_ctx_p)
{
	...
	if(code != 0 ||
	   (code = gs_stringwidth_begin(igs, op->value.bytes, r_size(op),
	                                imemory, &penum)) < 0)
		return code;
	if((code = op_show_finish_setup(i_ctx_p, penum, 1,
	                                finish_stringwidth)) < 0) {
		ifree_object(penum, "op_show_enum_setup");
		return code;
	}
	return op_show_continue_pop(i_ctx_p, 1);
...
private
int
zchar_path(i_ctx_t* i_ctx_p, int (*begin)(gs_state*, const byte*, uint, bool,
...
zsetcachedevice2(i_ctx_t* i_ctx_p)
{
	...
	if(code < 0)
		return code;
	if(zchar_show_width_only(penum))
		return op_show_return_width(
		    i_ctx_p, 10,
		    (gs_rootfont(igs)->WMode ? &wbox[6] : &wbox[0]));
...
const op_def zchar_op_defs[] = {{"3ashow", zashow},
                     ...
                     op_proc_t endproc /* end procedure */)
{
	 ...
	 */
	if(osenum && SHOW_IS_ALL_OF(osenum, TEXT_FROM_STRING | TEXT_DO_NONE |
	                                        TEXT_INTERVENE) &&
	...
	if(osenum && osenum->current_font->FontType == ft_user_defined &&
	   ...
	   ((const gs_font_type0*)osenum->fstack.items[0].font)
	           ->data.FMapType == fmap_CMap) {
	...
	if(endproc == NULL)
		endproc = finish_show;
	make_null(&esslot(ep));
	make_int(&esodepth(ep), ref_stack_count_inline(&o_stack) -
	                            npop); /* Save stack depth for */
	make_int(&esddepth(ep),
	         ref_stack_count_inline(
	             &d_stack)); /* correct interrupt processing */
...
op_show_continue(i_ctx_t* i_ctx_p)
{
	...
	if(code >= 0)
		code = op_show_continue_dispatch(i_ctx_p, 0,
		                                 gs_text_process(senum));
	return code;
}
int
...
op_show_continue_dispatch(i_ctx_t* i_ctx_p, int npop, int code)
{
	...
	case TEXT_PROCESS_INTERVENE: {
		...
		push(2);
		make_int(op - 1,
		         gs_text_current_char(penum)); /* previous char */
		...
		if(pfont->FontType == ft_user_defined) {
			...
			if(level2_enabled &&
			   ...
			   glyph != gs_no_glyph) {
				...
				if(!map_glyph_to_char(
				       imemory, &gref, pencoding,
				       (ref*)op)) { /* Not found, try .notdef */
					name_enter_string(imemory, ".notdef",
					                  &gref);
			...
			if(chr != gs_no_char &&
			   ...
			   (glyph == gs_no_glyph ||
			    (array_get(imemory, &pfdata->Encoding,
			               (int32_t)(chr & 0xff), &eref) >= 0 &&
			     (glyph_ref(imemory, glyph, &gref),
			      obj_eq(imemory, &gref, &eref))))) {
				make_int(op, chr & 0xff);
				esp[2] = pfdata->BuildChar;
			} else {
				/* We might not have a glyph: substitute 0.
				 * **HACK** */
	...
	case TEXT_PROCESS_CDEVPROC: {
		...
		if(pfont->FontType == ft_CID_TrueType) {
			...
			uint glyph_index =
			    pfont42->data.get_glyph_index(pfont42, glyph);

			code = zchar42_set_cache(
			    i_ctx_p, (gs_font_base*)pfont42, &cnref,
			    glyph_index, cont, &exec_cont, false);
...
private
bool
map_glyph_to_char(const gs_memory_t* mem, const ref* pgref,
                  const ref* pencoding, ref* pch)
{
...
op_show_restore(i_ctx_t* i_ctx_p, bool for_error)
{
	...
	if(penum->text.operation & TEXT_REPLACE_WIDTHS) {
		gs_free_const_object(penum->memory, penum->text.y_widths,
		                     "y_widths");
		if(penum->text.x_widths != penum->text.y_widths)
			gs_free_const_object(penum->memory,
			                     penum->text.x_widths, "x_widths");
	}
	/*
