...
/* Forward references */
private
int cie_joint_caches_init(gx_cie_joint_caches *,
			  const gs_cie_common *,
			  gs_cie_render *);
private
void cie_joint_caches_complete(gx_cie_joint_caches *,
			       const gs_cie_common *,
			       const gs_cie_abc *,
			       const gs_cie_render *);
private
...
private
void cie_mult3(const gs_vector3 *, const gs_matrix3 *,
	       gs_vector3 *);
private
...
#define CIE_LOAD_CACHE_BODY(pcache, domains, rprocs, dprocs, pcie, cname)                            \
	...
	for(j = 0; j < countof(pcache); j++) {                                                       \
		...
		pcf->params.is_identity = (rprocs)->procs[j] == (dprocs).procs[j];                   \
	}                                                                                            \
	END
...
private
bool cie_cache_mult3(gx_cie_vector_cache3_t *,
...
gs_cie_defx_scale(float *values, const gs_range *range, int dim)
{
	...
	for(i = 0; i < gx_cie_cache_size; ++i) {
		...
		values[i] =
		    (value <= range->rmin ? 0 : value >= range->rmax ? dim - 1 : (value - range->rmin) * scale);
	}
}
 ...
 */
private
void cie_transform_range3(const gs_range3 *, const gs_matrix3 *,
			  gs_range3 *);
int
...
#define SCALED_INDEX(f, n, itemp)                         \
	RESTRICTED_INDEX(f *(1 << _cie_interpolate_bits), \
			 (n) << _cie_interpolate_bits, itemp)
				...
				int k =
				    (c == 0 ? 1 : c == 1 ? m * pcrd->RenderTable.lookup.dims[2] : m);
#define SCALED_INDEX(f, n, itemp) \
