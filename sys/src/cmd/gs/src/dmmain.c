...
/* Ghostscript shlib example wrapper for Macintosh (Classic/Carbon) contributed
   by Nigel Hathaway. Uses the Metrowerks CodeWarrior SIOUX command-line
   library.
 */

#if __ide_target("Ghostscript PPC (Debug)") ||                                 \
    __ide_target("Ghostscript PPC (Release)")
...
static int display_update(void* handle, void* device, int x, int y, int w,
                          int h);
...
static int
display_presize(void* handle, void* device, int width, int height, int raster,
                unsigned int format)
{
	/* Check for correct format (32-bit RGB), fatal error if not */
	if(format != display_format) {
		printf(
		    "DisplayFormat has been set to an incompatible value.\n");
		fflush(stdout);
		return e_rangecheck;
	}
...
static int
display_size(void* handle, void* device, int width, int height, int raster,
...
display_callback display = {
    ...
    display_open, display_preclose, display_close, display_presize,
    display_size, display_sync, display_page, display_update,
    NULL, /* memalloc */
    NULL, /* memfree */
...
/* This function is a fudge which allows the SIOUX window to be waiting for
   input and not be modal at the same time. (Why didn't MetroWerks think of
   that?)
...
*/
static size_t
get_input(void* ptr, size_t size)
{
	EventRecord eventStructure;
	...
	while(!gDone && !stdin_buf) {
#if SIOUX_USE_WASTE
		charswaiting =
		    WEGetTextLength(SIOUXTextWindow->edit) - SIOUXselstart;
#else
		if((*SIOUXTextWindow->edit)->teLength > 0)
			charswaiting =
			    (*SIOUXTextWindow->edit)->teLength - SIOUXselstart;
		else
			charswaiting = ((unsigned short)(*SIOUXTextWindow->edit)
			                    ->teLength) -
			               SIOUXselstart;
#endif
...
#else
			text =
			    (*(*SIOUXTextWindow->edit)->hText) + SIOUXselstart;
#endif
			/* If user has pressed enter, gather up the buffer ready
			 * for returning */
		...
		if(WaitNextEvent(everyEvent, &eventStructure,
		                 SIOUXSettings.sleep, NULL))
			doEvents(&eventStructure);
		else
...
window_create(IMAGE* img)
{
	...
	windowRect.left += 4;
	windowRect.right =
	    ((windowRect.bottom - windowRect.top) * 3) / 4 + windowRect.left;
...
#if TARGET_API_MAC_CARBON
	if(gRunningOnX)
		ChangeWindowAttributes(windowRef, kWindowLiveResizeAttribute,
		                       0);

	CreateScrollBarControl(windowRef, &scrollbarRect, 0, 0, 0, 0, true,
	                       gActionFunctionScrollUPP,
	                       &(img->scrollbarVertRef));

	CreateScrollBarControl(windowRef, &scrollbarRect, 0, 0, 0, 0, true,
	                       gActionFunctionScrollUPP,
	                       &(img->scrollbarHorizRef));
#else
	img->scrollbarVertRef = NewControl(windowRef, &scrollbarRect, "\p",
	                                   false, 0, 0, 0, scrollBarProc, 0);
	img->scrollbarHorizRef = NewControl(windowRef, &scrollbarRect, "\p",
	                                    false, 0, 0, 0, scrollBarProc, 0);
#endif
...
static void
window_invalidate(WindowRef windowRef)
{
	Rect portRect;
...
static void
window_adjust_scrollbars(WindowRef windowRef)
{
	IMAGE* img;
	Rect portRect;
	...
	SizeControl(img->scrollbarHorizRef,
	            portRect.right - portRect.left - kScrollBarWidth + 1,
	            kScrollBarWidth + 1);
	...
	if(img->pixmapHdl) {
		...
		int visibleHeight =
		    portRect.bottom - portRect.top - kScrollBarWidth;
		int visibleWidth =
		...
		if(pixmap->bounds.bottom > visibleHeight) {
			SetControl32BitMaximum(img->scrollbarVertRef,
			                       pixmap->bounds.bottom -
			                           visibleHeight);
			SetControlViewSize(img->scrollbarVertRef,
			                   visibleHeight);
		} else
		...
		if(pixmap->bounds.right > visibleWidth) {
			SetControl32BitMaximum(img->scrollbarHorizRef,
			                       pixmap->bounds.right -
			                           visibleWidth);
			SetControlViewSize(img->scrollbarHorizRef,
			                   visibleWidth);
		} else
...
void
doEvents(EventRecord* eventStrucPtr)
{
	WindowRef windowRef;
...
void
doMouseDown(EventRecord* eventStrucPtr)
{
	WindowRef windowRef;
	...
	case inGrow:
		...
		newSize = GrowWindow(windowRef, eventStrucPtr->where,
		                     &constraintRect);
		if(newSize != 0)
			SizeWindow(windowRef, LoWord(newSize), HiWord(newSize),
			           true);
		window_adjust_scrollbars(windowRef);
	...
	case inZoomOut:
		...
		if(IsWindowInStandardState(windowRef,
		                           &standardStateHeightAndWidth, NULL))
			zoomPart = inZoomIn;
		else
			zoomPart = inZoomOut;

		if(TrackBox(windowRef, eventStrucPtr->where, partCode)) {
			ZoomWindowIdeal(windowRef, zoomPart,
			                &standardStateHeightAndWidth);
			window_adjust_scrollbars(windowRef);
		}
		break;
	}
...
void
doUpdate(EventRecord* eventStrucPtr)
{
	WindowRef windowRef;
...
void
doUpdateWindow(EventRecord* eventStrucPtr)
{
	IMAGE* img;
	WindowRef windowRef;
...
void
doOSEvent(EventRecord* eventStrucPtr)
{
...
void
doInContent(EventRecord* eventStrucPtr, WindowRef windowRef)
{
	...
	if(controlRef = FindControlUnderMouse(eventStrucPtr->where, windowRef,
	                                      &controlPartCode)) {
#if TARGET_API_MAC_CARBON
		TrackControl(controlRef, eventStrucPtr->where,
		             (ControlActionUPP)-1);
#else
...
pascal void
actionFunctionScroll(ControlRef controlRef, ControlPartCode controlPartCode)
{
...
OSErr
quitAppEventHandler(AppleEvent* appEvent, AppleEvent* reply,
                    SInt32 handlerRefcon)
{
	...
	osError =
	    AEGetAttributePtr(appEvent, keyMissedKeywordAttr, typeWildCard,
