...
#define cDecode -4
private
const byte pngp_case_needs_prev[] = {0, 0, 1, 1, 1, 1};
...
s_PNGP_release(stream_state* st)
{
	...
	if(ss->prev_row)
		gs_free_object(st->memory, ss->prev_row,
		               "PNGPredictor prev row");
}
...
private
void
s_pngp_process(stream_state* st, stream_cursor_write* pw, const byte* dprev,
               stream_cursor_read* pr, const byte* upprev, const byte* up,
               uint count)
{
	stream_PNGP_state* const ss = (stream_PNGP_state*)st;
	byte* q = pw->ptr + 1;
	...
	case cEncode + cAverage:
		for(; count; ++q, ++dprev, ++up, ++p, --count)
			*q =
			    (byte)(*p - arith_rshift_1((int)*dprev + (int)*up));
		break;
                ...
                stream_cursor_write* pw, bool last)
{
	...
	while(pr->ptr < pr->limit) {
		...
		if(ss->row_left == 0) {
			...
			predictor = (ss->Predictor == cOptimum
		...
		{
			byte* up =
			    ss->prev_row + bpp + ss->row_count - ss->row_left;
			...
			if(n < bpp) {
				/*
				 * We didn't have both enough input data and
				 * enough output
				 * space to use up all of prev.  Shift more data
				 * into prev
				 * and exit.
				 */
				int prev_left = bpp - n;
				...
				memcpy(ss->prev + prev_left, pr->ptr - (n - 1),
				       n);
			...
			/* and that n = bpp. */
			count -= bpp;
			s_pngp_process(st, pw, pr->ptr - (bpp - 1), pr, up,
			               up + bpp, count);
	...
	while(pr->ptr < pr->limit) {
		...
		}
		{
			...
			if(n < bpp) {
				...
				memmove(ss->prev, ss->prev + n, prev_left);
				memcpy(ss->prev + prev_left, pw->ptr - (n - 1),
				       n);
				if(pw->ptr >= pw->limit && pr->ptr < pr->limit)
					status = 1;
				break;
			...
			/* and that n = bpp. */
			count -= bpp;
			s_pngp_process(st, pw, pw->ptr - (bpp - 1), pr, up,
			               up + bpp, count);
...
/* Stream templates */
const stream_template s_PNGPE_template = {&st_PNGP_state, s_PNGPE_init,
                                          s_PNGPE_process, 1, 1, s_PNGP_release,
                                          s_PNGP_set_defaults, s_PNGP_reinit};
const stream_template s_PNGPD_template = {&st_PNGP_state, s_PNGPD_init,
                                          s_PNGPD_process, 1, 1, s_PNGP_release,
                                          s_PNGP_set_defaults, s_PNGP_reinit};
