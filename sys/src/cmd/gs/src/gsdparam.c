			   ...
			   bool is_hardware)
{
	...
	code = (is_hardware ? (*dev_proc(dev, get_hardware_params))(dev, plist) : (*dev_proc(dev, get_params))(dev, plist));
	if(dev != orig_dev)
...
#define set_param_array(a, d, s) \
	...
	if(
...
#endif
	    (code = (pcms.data == 0 ? 0 : param_write_name(plist, "ProcessColorModel", &pcms))) < 0 ||
	    (code = param_write_float_array(plist, "HWResolution", &hwra)) < 0 ||
	    (code = (dev->ImagingBBox_set ? param_write_float_array(plist, "ImagingBBox", &ibba) : param_write_null(plist, "ImagingBBox"))) < 0 ||
	    ...
	    (code = (dev->NumCopies_set < 0 ||
			 ...
			 ? 0
			 : dev->NumCopies_set ? param_write_int(plist, "NumCopies", &dev->NumCopies) : param_write_null(plist, "NumCopies"))) < 0 ||
...
private
int param_MediaSize(gs_param_list *, gs_param_name,
...
private
int param_check_bytes(gs_param_list *, gs_param_name, const byte *,
		      uint, bool);
