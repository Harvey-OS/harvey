...
/* Forward references */
private
bool param_HWColorMap(gx_device*, byte*);
                           ...
                           bool is_hardware)
{
	...
	code = (is_hardware ? (*dev_proc(dev, get_hardware_params))(dev, plist)
...
#define set_param_array(a, d, s) (a.data = d, a.size = s, a.persistent = false);
	...
	if(
...
#endif
	    (code = (pcms.data == 0 ? 0 : param_write_name(plist,
	                                                   "ProcessColorModel",
	                                                   &pcms))) < 0 ||
	    (code = param_write_float_array(plist, "HWResolution", &hwra)) <
	        0 ||
	    ...
	    (code = param_write_float_array(plist, ".MarginsHWResolution",
	                                    &mhwra)) < 0 ||
	    ...
	    (code = param_write_long(plist, "PageCount", &dev->PageCount)) <
	        0 ||
	    (code = param_write_bool(plist, ".IgnoreNumCopies",
	                             &dev->IgnoreNumCopies)) < 0 ||
	    (code = param_write_int(plist, "TextAlphaBits",
	                            &dev->color_info.anti_alias.text_bits)) <
	        0 ||
	    (code = param_write_int(
	         plist, "GraphicsAlphaBits",
	         &dev->color_info.anti_alias.graphics_bits)) < 0 ||
	    (code = param_write_bool(plist, ".LockSafetyParams",
	                             &dev->LockSafetyParams)) < 0)
		return code;
	...
	if(colors > 1) {
		...
		int32_t ColorValues =
		    (depth >= (8 * arch_sizeof_color_index) ? -1 : 1L << depth);

		if((code = param_write_int(plist, "RedValues", &RGBValues)) <
		       0 ||
		   (code = param_write_int(plist, "GreenValues", &RGBValues)) <
		       0 ||
		   (code = param_write_int(plist, "BlueValues", &RGBValues)) <
		       0 ||
		   (code = param_write_long(plist, "ColorValues",
		                            &ColorValues)) < 0)
			return code;
	}
	if(param_requested(plist, "HWColorMap")) {
		...
		if(param_HWColorMap(dev, palette)) {
			...
			if((code = param_write_string(plist, "HWColorMap",
			                              &hwcms)) < 0)
				return code;
		}
	}
...
int
gdev_begin_input_media(gs_param_list* mlist, gs_param_dict* pdict, int count)
{
	pdict->size = count;
                       ...
                       const gdev_input_media_t* pim)
{
	...
	if((pim->PageSize[0] != 0 && pim->PageSize[1] != 0) ||
	   (pim->PageSize[2] != 0 && pim->PageSize[3] != 0)) {
		...
		psa.size = (pim->PageSize[0] == pim->PageSize[2] &&
		                    pim->PageSize[1] == pim->PageSize[3]
		                ? 2
		                : 4);
		psa.persistent = false;
		code = param_write_float_array(mdict.list, "PageSize", &psa);
		if(code < 0)
			return code;
	}
	if(pim->MediaColor != 0) {
		param_string_from_string(as, pim->MediaColor);
		code = param_write_string(mdict.list, "MediaColor", &as);
		if(code < 0)
			return code;
	}
	if(pim->MediaWeight != 0) {
		...
		code = param_write_float(mdict.list, "MediaWeight", &weight);
		if(code < 0)
			return code;
	}
...
int
gdev_write_input_page_size(int index, gs_param_dict* pdict, floatp width_points,
                           floatp height_points)
{
...
int
gdev_begin_output_media(gs_param_list* mlist, gs_param_dict* pdict, int count)
{
	pdict->size = count;
...
/* Forward references */
private
int param_anti_alias_bits(gs_param_list*, gs_param_name, int*);
private
int param_MediaSize(gs_param_list*, gs_param_name, const float*,
                    gs_param_float_array*);

private
int param_check_bool(gs_param_list*, gs_param_name, bool, bool);
private
int param_check_long(gs_param_list*, gs_param_name, long, bool);
#define param_check_int(plist, pname, ival, is_defined)                        \
	param_check_long(plist, pname, (long)(ival), is_defined)
private
int param_check_bytes(gs_param_list*, gs_param_name, const byte*, uint, bool);
...
#undef max_coord
	...
	BEGIN_ARRAY_PARAM(param_read_float_array, ".MarginsHWResolution", mhwra,
	                  2, mhwre)
	{
	...
	switch(code = param_read_bool(plist, (param_name = ".IgnoreNumCopies"),
	                              &ignc)) {
	default:
		ecode = code;
		param_signal_error(plist, param_name, ecode);
	...
	if(dev->NumCopies_set >= 0 &&
	   (*dev_proc(dev, get_page_device))(dev) != 0) {
		switch(code = param_read_int(plist, (param_name = "NumCopies"),
		                             &nci)) {
		case 0:
			if(nci < 0)
	...
	if((code = param_read_bool(plist, (param_name = "UseCIEColor"), &ucc)) <
	   0) {
		ecode = code;
		param_signal_error(plist, param_name, ecode);
	}
	...
	switch(code = param_read_bool(plist, (param_name = ".LockSafetyParams"),
	                              &locksafe)) {
	case 0:
...
#define IGNORE_INT_PARAM(pname)                                                \
	{                                                                      \
		int igni;                                                      \
		switch(code = param_read_int(plist, (param_name = pname),      \
		                             &igni)) {                         \
		default:                                                       \
			ecode = code;                                          \
			param_signal_error(plist, param_name, ecode);          \
	...
	switch(code = param_read_float_array(
	           plist, (param_name = "ImagingBBox"), &ibba)) {
	case 0:
		if(ibba.size != 4 || ibba.data[2] < ibba.data[0] ||
		   ibba.data[3] < ibba.data[1])
			ecode = gs_note_error(gs_error_rangecheck);
		else
			break;
		goto ibbe;
	 ...
	 */
	{
		 ...
		 */
		if((code = param_check_string(plist, "ProcessColorModel", pcms,
		                              (pcms != NULL))) < 0)
			ecode = code;
	}
	...
	if((code = param_check_string(plist, "OutputDevice", dev->dname,
	                              true)) < 0)
		ecode = code;
	...
	if((code = param_check_int(plist, "GrayValues", GrayValues, true)) < 0)
		ecode = code;
	if((code = param_check_long(plist, "PageCount", dev->PageCount, true)) <
	   0)
		ecode = code;
	...
	if((code = param_check_int(plist, "BlueValues", RGBValues, true)) < 0)
		ecode = code;
	if((code = param_check_long(plist, "ColorValues", ColorValues, true)) <
	   0)
		ecode = code;
	if(param_read_string(plist, "HWColorMap", &cms) != 1) {
		...
		if(param_HWColorMap(dev, palette))
			...
			code =
			    param_check_bytes(plist, "HWColorMap", 0, 0, false);
		if(code < 0)
			ecode = code;
	}
	...
	if(hwra.data != 0 && (dev->HWResolution[0] != hwra.data[0] ||
	...
	if(hwsa.data != 0 &&
	   (dev->width != hwsa.data[0] || dev->height != hwsa.data[1])) {
		if(dev->is_open)
			gs_closedevice(dev);
		gx_device_set_width_height(dev, hwsa.data[0], hwsa.data[1]);
	}
	if(msa.data != 0 && (dev->MediaSize[0] != msa.data[0] ||
...
private
int
param_MediaSize(gs_param_list* plist, gs_param_name pname, const float* res,
                gs_param_float_array* pa)
{
	gs_param_name param_name;
	int ecode = 0;
	int code;
