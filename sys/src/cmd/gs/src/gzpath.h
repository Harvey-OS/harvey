...
#define curve_points_to_coefficients(v0, v1, v2, v3, a, b, c, t01, t12)        \
	(/*d = (v0),*/                                                         \
	 t01 = (v1) - (v0), c = (t01 << 1) + t01, t12 = (v2) - (v1),           \
	 b = (t12 << 1) + t12 - c, a = (v3)-b - c - (v0))
/*
 * or conversely
 */
#define curve_coefficients_to_points(a, b, c, d, v1, v2, v3)                   \
	(/*v0 = (d),*/                                                         \
	 v1 = (d) + ((c) / 3), v2 = v1 + (((b) + (c)) / 3),                    \
	 v3 = (a) + (b) + (c) + (d))

/* Curve segments store the control points. */
typedef struct {
	segment_common gs_fixed_point p1, p2;
} curve_segment;
 ...
 */
struct subpath_s {
	segment_common segment* last; /* last segment of subpath, */
...
gx_path_rectangular_type gx_subpath_is_rectangular(const subpath* pstart,
                                                   gs_fixed_rect* pbox,
...
#define path_is_drawing(ppath) (((ppath)->state_flags & psf_is_drawing) != 0)
 ...
 * Updating operations
 */
#define path_update_newpath(ppath) ((ppath)->state_flags = psf_last_newpath)
#define path_update_moveto(ppath)                                              \
	((ppath)->state_flags = (ppath)->start_flags = psf_last_moveto)
#define path_update_draw(ppath) ((ppath)->state_flags = psf_last_draw)
#define path_update_closepath(ppath) ((ppath)->state_flags = psf_last_closepath)
...
#define private_st_path_segments() /* in gxpath.c */                           \
	gs_private_st_ptrs2(st_path_segments, gx_path_segments,                \
	                    "path segments", path_segments_enum_ptrs,          \
	                    path_segments_reloc_ptrs, contents.subpath_first,  \
	                    contents.subpath_current)
...
typedef struct gx_path_procs_s {
	...
	int (*add_curve)(gx_path*, fixed, fixed, fixed, fixed, fixed, fixed,
	                 segment_notes);
...
#define public_st_path() /* in gxpath.c */                                     \
	gs_public_st_ptrs2(st_path, gx_path, "path", path_enum_ptrs,           \
	                   path_reloc_ptrs, segments, box_last)
...
#define public_st_path_enum() /* in gxpath2.c */                               \
	gs_public_st_ptrs3(st_path_enum, gs_path_enum, "gs_path_enum",         \
	                   path_enum_enum_ptrs, path_enum_reloc_ptrs, pseg,    \
	                   path, copied_path)

/* Inline path accessors. */
#define gx_path_has_curves_inline(ppath) ((ppath)->curve_count != 0)
#define gx_path_has_curves(ppath) gx_path_has_curves_inline(ppath)
#define gx_path_is_void_inline(ppath)                                          \
	((ppath)->segments != 0 && (ppath)->first_subpath == 0)
#define gx_path_is_void(ppath) gx_path_is_void_inline(ppath)
#define gx_path_subpath_count(ppath) ((ppath)->subpath_count)
...
#define gx_path_current_point_inline(ppath, ppt)                               \
	(!path_position_valid(ppath) ? gs_note_error(gs_error_nocurrentpoint)  \
	                             : ((ppt)->x = ppath->position.x,          \
	                                (ppt)->y = ppath->position.y, 0))
...
bool gx_flattened_iterator__init(gx_flattened_iterator* this, fixed x0,
                                 fixed y0, const curve_segment* pc, int k);
bool gx_flattened_iterator__init_line(gx_flattened_iterator* this, fixed x0,
...
bool curve_coeffs_ranged(fixed x0, fixed x1, fixed x2, fixed x3, fixed y0,
                         fixed y1, fixed y2, fixed y3, fixed* ax, fixed* bx,
                         fixed* cx, fixed* ay, fixed* by, fixed* cy, int k);
