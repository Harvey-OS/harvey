...
#define CHECK_PAPER_SIZE(w, h, c)                                              \
	...
	if((new_width_difference > -0.01) &&                                   \
	   (new_height_difference > -0.01) &&                                  \
...
gdev_pcl_3bit_map_rgb_color(gx_device* dev, const gx_color_value cv[])
{
	...
	g = cv[1];
	b = cv[2];
	return (((b >> cv_shift) << 2) + ((g >> cv_shift) << 1) +
	        (r >> cv_shift)) ^
	       7;
}
...
{
	register const word* exam =
	    row; /* word being examined in the row to compress */
	register byte* cptr =
	    compressed; /* output pointer into compressed bytes */

	while(exam <
	      end_row) { /* Search ahead in the input looking for a run */
 ...
 */
int
gdev_pcl_mode9compress(int bytecount, const byte* current, const byte* previous,
                       byte* compressed)
{
	register const byte* cur = current;
