...
/* Copyright (C) 1994, 1995, 1998, 1999 Aladdin Enterprises.  All rights
  reserved.
...
private
void s_BWBS_release(stream_state*);
...
bwbs_init(stream_state* st, uint osize)
{
	...
	if(code != 0)
		return code;
	ss->offsets =
	    (void*)gs_alloc_bytes(st->memory, osize, "BWBlockSort offsets");
	if(ss->offsets == 0) {
		s_BWBS_release(st);
		return ERRC;
		/****** WRONG ******/
...
private
stream_BWBS_state* bwbs_compare_ss;
private
int
...
#define C Cs.v
	...
	for(ch = 0; ch <= 255; sum = C[ch], ch++)
		qsort(indices + sum, C[ch] - sum, sizeof(*indices),
...
/* Stream template */
const stream_template s_BWBSE_template = {&st_BWBS_state, s_BWBSE_init,
                                          s_BWBSE_process, sizeof(int) * 2, 1,
...
#define offset_space(bsize)                                                    \
	(ceil_64k(bsize) * sizeof(offsets_full) +                              \
	 ceil_4k(bsize) * sizeof(offsets_4k) + ((bsize + 1) >> 1) * 3)

#else /* !SHORT_OFFSETS */

#define offset_space(bsize) (bsize * sizeof(int))
...
/* Stream template */
const stream_template s_BWBSD_template = {&st_BWBS_state, s_BWBSD_init,
                                          s_BWBSD_process, 1, sizeof(int) * 2,
                                          s_BWBS_release, s_BWBS_set_defaults};
