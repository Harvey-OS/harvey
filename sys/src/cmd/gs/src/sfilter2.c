               ...
               stream_cursor_write* pw, bool last)
{
	...
	byte* qn =
	    q + (LINE_LIMIT - ss->count); /* value of q before next EOL */
	...
	if_debug3('w',
	          "[w85]initial ss->count = %d, rcount = %d, wcount = %d\n",
	          ss->count, (int)(rlimit - p), (int)(wlimit - q));
	for(; (count = rlimit - p) >= 4; p += 4) {
		uint32_t word = ((uint32_t)(((uint)p[1] << 8) + p[2]) << 16) +
		                (((uint)p[3] << 8) + p[4]);
		...
		put:
			...
			/*
			 * '%%' or '%!' at the beginning of the line will
			 * confuse some
			 * document managers: insert (an) EOL(s) if necessary to
			 * prevent
			 * this.
			 */
			if(q[1] == '%') {
					...
					if(qn - q == LINE_LIMIT - 1) {
						...
						*++q = prev = '\n';
						...
						goto put;
				          ...
				          (q[2] == '%' || q[2] == '!')) {
					/*
					 * We may have to insert more than one
					 * EOL if
...
/* Stream template */
const stream_template s_A85E_template = {&st_A85E_state, s_A85E_init,
                                         s_A85E_process, 4, 6};
...
private
int
s_BT_process(stream_state* st, stream_cursor_read* pr, stream_cursor_write* pw,
             bool last)
{
...
/* Stream template */
const stream_template s_BTE_template = {&st_BT_state, NULL, s_BT_process, 1, 1};
const stream_template s_BTD_template = {&st_BT_state, NULL, s_BT_process, 1, 1};
