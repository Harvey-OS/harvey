...
private
ENUM_PTRS_WITH(copied_glyph_element_enum_ptrs, gs_copied_glyph_t* pcg)
if(index < size / (uint)sizeof(gs_copied_glyph_t))
	return ENUM_CONST_STRING(&pcg[index].gdata);
return 0;
ENUM_PTRS_END
private
RELOC_PTRS_WITH(copied_glyph_element_reloc_ptrs, gs_copied_glyph_t* pcg)
{
 ...
 */
gs_private_st_composite(st_gs_copied_glyph_name_element, gs_copied_glyph_name_t,
                        "gs_copied_glyph_name_t[]", copied_glyph_name_enum_ptrs,
                        copied_glyph_name_reloc_ptrs);
private
ENUM_PTRS_WITH(copied_glyph_name_enum_ptrs, gs_copied_glyph_name_t* pcgn)
if(index < size / (uint)sizeof(gs_copied_glyph_name_t)) {
	const gs_copied_glyph_name_t* const p = &pcgn[index];

	return (p->str.size == 0 || gs_is_c_glyph_name(p->str.data, p->str.size)
...
ENUM_PTRS_END
private
RELOC_PTRS_WITH(copied_glyph_name_reloc_ptrs, gs_copied_glyph_name_t* pcgn)
{
...
gs_private_st_basic(st_gs_copied_glyph_extra_name, gs_copied_glyph_extra_name_t,
...
gs_private_st_composite(st_gs_copied_font_data, gs_copied_font_data_t,
                        "gs_copied_font_data_t", gs_copied_font_data_enum_ptrs,
                        gs_copied_font_data_reloc_ptrs);
...
copied_Encoding_alloc(gs_font* copied)
{
	...
	gs_glyph* Encoding = (gs_glyph*)gs_alloc_byte_array(
	    copied->memory, 256, sizeof(*cfdata->Encoding),
...
copied_data_alloc(gs_font* copied, stream* s, uint extra, int code)
{
	...
	if(code < 0)
		return code;
	fdata =
	    gs_alloc_bytes(copied->memory, len + extra, "copied_data_alloc");
	if(fdata == 0)
		return_error(gs_error_VMerror);
           ...
           gs_memory_t* mem)
{
	...
	if(size == 0)
		...
		/* Copy the data. */
		for(i = 0, size = 0;
		    (code = pfont->data.procs.subr_data(
		         pfont, i, global, &gdata)) != gs_error_rangecheck;
		    ++i) {
			starts[i] = size;
			if(code >= 0) {
				memcpy(data + size, gdata.bits.data,
				       gdata.bits.size);
				size += gdata.bits.size;
				gs_glyph_data_free(&gdata, "copy_subrs");
			}
	...
	*pslot = 0;
	if(glyph >= GS_MIN_GLYPH_INDEX) {
		...
		*pslot = &cfdata->glyphs[glyph - GS_MIN_CID_GLYPH];
		...
		int code =
		    cfdata->procs->named_glyph_slot(cfdata, glyph, pslot);
                ...
                gs_glyph_data_t* pgdata, const byte* prefix, int prefix_bytes)
{
	...
	case 0: /* already defined */
		if((options & COPY_GLYPH_NO_OLD) ||
		   ...
		   memcmp(pcg->gdata.data + prefix_bytes, pgdata->bits.data,
		          size))
	...
	case gs_error_undefined:
		if(options & COPY_GLYPH_NO_NEW)
			...
			if(str == 0)
				...
				memcpy(str + prefix_bytes, pgdata->bits.data,
				       size);
				pcg->gdata.data = str;
                ...
                gs_glyph copied_glyph)
{
	...
	if(code < 0 || (code = font->procs.glyph_name(font, glyph, &str)) < 0)
		return code;
	...
	else if((code = copy_string(copied->memory, &str, "copy_glyph_name")) <
	        0)
		return code;
	...
	if(pcgn->glyph != GS_NO_GLYPH &&
	   (pcgn->str.size != str.size ||
		...
		/* This is a glyph with multiple names.  Add an extra_name
		 * entry. */
		gs_copied_glyph_extra_name_t* extra_name = gs_alloc_struct(
		    copied->memory, gs_copied_glyph_extra_name_t,
...
private
int
copied_enumerate_glyph(gs_font* font, int* pindex, gs_glyph_space_t glyph_space,
                       gs_glyph* pglyph)
{
		...
		if(cfdata->glyphs[*pindex].used) {
			*pglyph =
			    (glyph_space == GLYPH_SPACE_NAME &&
			         ...
			         :
			         /* CIDFontType 0 uses CIDS as slot indices;
			            CIDFontType 2 uses GIDs. */
                  ...
                  gs_char chr, gs_glyph glyph)
{
	...
	double wxy[6];
	double sbw_stub[4]; /* Currently glyph_outline retrieves sbw only with
	                       type 1,2,9 fonts. */
...
#define compare_tables(a, b)                                                   \
	compare_arrays(a.values, a.count, b.values, b.count)
               ...
               int num_glyphs, int glyphs_step, int level)
{
	 ...
	 * separately from outline data. We could skip it for Type 1, which
	 * doesn't.
	 * We don't care of Metrics, Metrics2 because copied font never has
	 * them.
	 */
	int i, WMode = ofont->WMode;
	int members = (GLYPH_INFO_WIDTH0 << WMode) | GLYPH_INFO_OUTLINE_WIDTHS |
	...
	for(i = 0; i < num_glyphs; i++) {
		...
		int code0 = ofont->procs.glyph_info((gs_font*)ofont, glyph,
		                                    &mat, members, &info0);
		int code1 = cfont->procs.glyph_info((gs_font*)cfont, glyph,
		                                    &mat, members, &info1);
		int code2, code;
		...
		if(code1 == gs_error_undefined) {
			num_new_glyphs++;
			if(num_new_glyphs >
			   cfdata->glyphs_size - cfdata->num_glyphs)
				return 0;
			continue;
		}
		if(code0 < 0)
		...
		if(info0.num_pieces > 0) {
			if(level > 5)
				return_error(
				    gs_error_rangecheck); /* abnormal glyph
				                             recursion */
			if(info0.num_pieces > countof(pieces0) / 2) {
				pieces = (gs_glyph*)gs_alloc_bytes(
				    cfont->memory,
				    sizeof(glyphs) * info0.num_pieces * 2,
				    "compare_glyphs");
				if(pieces == 0)
					return_error(gs_error_VMerror);
			}
			...
			code0 = ofont->procs.glyph_info((gs_font*)ofont, glyph,
			                                &mat, GLYPH_INFO_PIECES,
			                                &info0);
			code1 = cfont->procs.glyph_info((gs_font*)cfont, glyph,
			                                &mat, GLYPH_INFO_PIECES,
			                                &info1);
			if(code0 >= 0 && code1 >= 0) {
				code2 =
				    memcmp(info0.pieces, info1.pieces,
				           info0.num_pieces * sizeof(*pieces));
				if(!code2)
					code = compare_glyphs(
					    cfont, ofont, pieces,
					    info0.num_pieces, glyphs_step,
					    level + 1);
				else
				...
				code2 = code = 0;
			if(pieces != pieces0)
				gs_free_object(cfont->memory, pieces,
				               "compare_glyphs");
			if(code0 == gs_error_undefined)
				continue;
			if(code1 == gs_error_undefined) {
			...
			case ft_encrypted2: {
				...
				code0 = font0->data.procs.glyph_data(
				    font0, glyph, &gdata0);
				code1 = font1->data.procs.glyph_data(
				    font1, glyph, &gdata1);
				break;
			}
			case ft_TrueType:
			case ft_CID_TrueType: {
				...
				uint glyph_index0 =
				    font0->data.get_glyph_index(font0, glyph);
				uint glyph_index1 =
				    font1->data.get_glyph_index(font1, glyph);

				gdata0.memory = font0->memory;
				gdata1.memory = font1->memory;
				code0 = font0->data.get_outline(
				    font0, glyph_index0, &gdata0);
				code1 = font1->data.get_outline(
				    font1, glyph_index1, &gdata1);
				break;
			}
			case ft_CID_encrypted: {
				...
				gdata0.memory = font0->memory;
				gdata1.memory = font1->memory;
				code0 = font0->cidata.glyph_data(
				    (gs_font_base*)font0, glyph, &gdata0,
				    &fidx0);
				code1 = font1->cidata.glyph_data(
				    (gs_font_base*)font1, glyph, &gdata1,
				    &fidx1);
				break;
			}
			default:
				return_error(
				    gs_error_unregistered); /* unimplemented */
			}
			if(code0 < 0) {
				if(code1 >= 0)
					gs_glyph_data_free(&gdata1,
					                   "compare_glyphs");
				return code0;
			}
			if(code1 < 0) {
				if(code0 >= 0)
					gs_glyph_data_free(&gdata0,
					                   "compare_glyphs");
				return code1;
			}
			if(gdata0.bits.size != gdata1.bits.size)
				return 0;
			if(memcmp(gdata0.bits.data, gdata0.bits.data,
			          gdata0.bits.size))
				return 0;
                       ...
                       gs_glyph_data_t* pgd)
{
	...
	gs_glyph_data_from_string(
	    pgd, psi->data + psi->starts[subr_num],
	    psi->starts[subr_num + 1] - psi->starts[subr_num], NULL);
	return 0;
}

private
int
copied_type1_seac_data(gs_font_type1* pfont, int ccode, gs_glyph* pglyph,
                       gs_const_string* gstr, gs_glyph_data_t* pgd)
{
	 ...
	 */
	gs_glyph glyph =
	    gs_c_known_encode((gs_char)ccode, ENCODING_INDEX_STANDARD);
	gs_glyph glyph1;
	int code;
...
copy_font_type1(gs_font* font, gs_font* copied)
{
	...
	if(code < 0)
		return code;
	if((code = copy_subrs(font1, false, &cfdata->subrs, copied->memory)) <
	       0 ||
	   (code = copy_subrs(font1, true, &cfdata->global_subrs,
	                      copied->memory)) < 0) {
...
private
const gs_copied_font_procs_t copied_procs_type1 = {
    copy_font_type1,           copy_glyph_type1,   copied_char_add_encoding,
    named_glyph_slot_hashed,   copied_encode_char, gs_type1_glyph_info,
                 ...
                 bool global)
{
	...
	for(i = 0; !exit; i++) {
		...
		*/
		missing0 = (code0 == gs_error_rangecheck ||
		            code0 == gs_error_typecheck);
		missing1 = (code1 == gs_error_rangecheck ||
		            code1 == gs_error_typecheck);
		...
		if(code0 < 0)
			...
			exit = true;
		else if(memcmp(gdata0.bits.data, gdata1.bits.data,
		               gdata0.bits.size))
			exit = true;
		if(code0 > 0)
                          ...
                          gs_glyph_data_t* pgd)
{
	...
	if(!pcg->used)
		...
		gs_glyph_data_from_string(pgd, pcg->gdata.data, pcg->gdata.size,
		                          NULL);
	return 0;
}
...
private
int
copied_type42_get_metrics(gs_font_type42* pfont, uint glyph_index, int wmode,
                          float sbw[4])
{
...
copy_font_type42(gs_font* font, gs_font* copied)
{
	...
	code = (font->FontType == ft_TrueType
	...
	return 0;
fail2:
	gs_free_object(copied->memory, cfdata->data, "copy_font_type42(data)");
fail:
	gs_free_object(copied->memory, cfdata->Encoding,
...
copy_glyph_type42(gs_font* font, gs_glyph glyph, gs_font* copied, int options)
{
	...
	gs_font_type42* const copied42 = (gs_font_type42*)copied;
	uint gid = (options & COPY_GLYPH_BY_INDEX
	...
	DISCARD(copied_glyph_slot(cfdata, gid + GS_MIN_GLYPH_INDEX,
	                          &pcg)); /* can't fail */
	for(i = 0; i < 2; ++i) {
		if(font42->data.get_metrics(font42, gid, i, sbw) >= 0) {
			int sb = (int)(sbw[i] * factor + 0.5);
			uint width = (uint)(sbw[2 + i] * factor + 0.5);
			byte* pmetrics = cfdata->data +
			                 copied42->data.metrics[i].offset +
			                 gid * 4;
...
private
const gs_copied_font_procs_t copied_procs_type42 = {
    copy_font_type42,          copy_glyph_type42,
    copied_char_add_encoding,  named_glyph_slot_linear,
    copied_type42_encode_char, gs_type42_glyph_info,
...
same_type42_hinting(gs_font_type42* font0, gs_font_type42* font1)
{
	...
	for(j = 0; j < 2; j++) {
		...
		for(i = 0; i < numTables; ++i) {
			...
			code =
			    access_type42_data(font[j], 12 + i * 16, 16, &tab);
			if(code < 0)
				return code;
	...
	for(i = 0; i < 3; i++) {
		if(len[0][i] != 0) {
			...
			code = access_type42_data(font0, pos[0][i], len[0][i],
			                          &data0);
			if(code < 0)
				return code;
			code = access_type42_data(font1, pos[1][i], len[1][i],
			                          &data1);
			if(code < 0)
				return code;
...
private
int
copied_cid0_glyph_data(gs_font_base* font, gs_glyph glyph, gs_glyph_data_t* pgd,
                       int* pfidx)
{
...
copied_sub_type1_glyph_data(gs_font_type1* pfont, gs_glyph glyph,
                            gs_glyph_data_t* pgd)
{
	return copied_cid0_glyph_data(
	    (gs_font_base*)cf_data((gs_font*)pfont)->parent, glyph, pgd, NULL);
}
...
cid0_subfont(gs_font* copied, gs_glyph glyph, gs_font_type1** pfont1)
{
	int fidx;
	int code =
	    copied_cid0_glyph_data((gs_font_base*)copied, glyph, NULL, &fidx);
                       ...
                       int members, gs_glyph_info_t* info)
{
	...
	if(members & GLYPH_INFO_WIDTH1) {
		...
		int code = subfont1->procs.font_info(font, NULL, FONT_INFO_BBOX,
		                                     &finfo);
                          ...
                          const gs_matrix* pmat, gx_path* ppath, double sbw[4])
{
	...
	if(code < 0)
		return code;
	return subfont1->procs.glyph_outline((gs_font*)subfont1, WMode, glyph,
	                                     pmat, ppath, sbw);
}
...
copy_font_cid0(gs_font* font, gs_font* copied)
{
	...
	gs_copied_font_data_t* const cfdata = cf_data(copied);
	gs_font_type1** FDArray = gs_alloc_struct_array(
	    copied->memory, copied0->cidata.FDArray_size, gs_font_type1*,
	...
	for(; i < copied0->cidata.FDArray_size; ++i) {
		...
		code = gs_copy_font(subfont, &subfont->FontMatrix,
		                    copied->memory, &subcopy);
		if(code < 0)
			goto fail;
...
fail:
	while(--i >= 0)
		gs_free_object(copied->memory, FDArray[i],
		               "copy_font_cid0(subfont)");
...
copy_glyph_cid0(gs_font* font, gs_glyph glyph, gs_font* copied, int options)
{
	...
	gdata.memory = font->memory;
	code =
	    fcid0->cidata.glyph_data((gs_font_base*)font, glyph, &gdata, &fidx);
	if(code < 0)
		return code;
	...
	if(fidx != 0)
		return_error(gs_error_rangecheck);
	return copy_glyph_data(font, glyph, copied, options, &gdata, prefix,
	                       fdbytes);
}

private
const gs_copied_font_procs_t copied_procs_cid0 = {
    copy_font_cid0,           copy_glyph_cid0,   copied_no_add_encoding,
    named_glyph_slot_none,    gs_no_encode_char, copied_cid0_glyph_info,
    copied_cid0_glyph_outline};
...
copy_font_cid2(gs_font* font, gs_font* copied)
{
	...
	ushort* CIDMap = (ushort*)gs_alloc_byte_array(
	    copied->memory, CIDCount, sizeof(ushort), "copy_font_cid2(CIDMap");

	if(CIDMap == 0)
		return_error(gs_error_VMerror);
	code = copy_font_cid_common(font, copied, &copied2->cidata.common);
	if(code < 0 || (code = copy_font_type42(font, copied)) < 0) {
...
private
int
expand_CIDMap(gs_font_cid2* copied2, uint CIDCount)
{
	ushort* CIDMap;
	...
	if(CIDCount <= copied2->cidata.common.CIDCount)
		return 0;
	CIDMap = (ushort*)gs_alloc_byte_array(
	    copied2->memory, CIDCount, sizeof(ushort), "copy_font_cid2(CIDMap");
	if(CIDMap == 0)
		return_error(gs_error_VMerror);
	memcpy(CIDMap, cfdata->CIDMap,
	       copied2->cidata.common.CIDCount * sizeof(*CIDMap));
...
private
const gs_copied_font_procs_t copied_procs_cid2 = {
    copy_font_cid2,         copy_glyph_cid2,   copied_no_add_encoding,
    named_glyph_slot_none,  gs_no_encode_char, gs_type42_glyph_info,
    gs_type42_glyph_outline};
...
{
	return same_type42_hinting((gs_font_type42*)cfont,
 ...
 */
private
font_proc_font_info(copied_font_info);
private
font_proc_enumerate_glyph(copied_enumerate_glyph);
private
const gs_font_procs copied_font_procs = {
    0, /* define_font, not supported */
    0, /* make_font, not supported */
    copied_font_info, gs_default_same_font,
    0,                         /* encode_char, varies by FontType */
    0,                         /* decode_char, not supported */
    copied_enumerate_glyph, 0, /* glyph_info, varies by FontType */
    0,                         /* glyph_outline, varies by FontType */
    copied_glyph_name, gs_default_init_fstack, gs_default_next_char_glyph,
    copied_build_char};

#if GLYPHS_SIZE_IS_PRIME
private
const int some_primes[] = {
    /* Arbitrary choosen prime numbers, being reasonable for a Type 1|2 font
       size.
    ...
    */
    257,  359,   521,   769,   1031,  2053,  3079,  4099,  5101,  6101, 7109,
    8209, 10007, 12007, 14009, 16411, 20107, 26501, 32771, 48857, 65537};
#endif
 ...
 */
int
gs_copy_font(gs_font* font, const gs_matrix* orig_matrix, gs_memory_t* mem,
             gs_font** pfont_new)
{
	...
	case ft_encrypted2:
		...
		glyphs_size = 0;
		{
			...
			while(font->procs.enumerate_glyph(
			          font, &index, GLYPH_SPACE_NAME, &glyph),
			      index != 0)
				++glyphs_size;
		}
#if GLYPHS_SIZE_IS_PRIME
		/*
		 * Make glyphs_size a prime number to ensure termination of the
		 * loop in
		 * named_glyphs_slot_hashed, q.v.
		 * Also reserve additional slots for the case of font merging
		 * and
...
#else
		/*
		 * Make names_size a power of 2 to ensure termination of the
		 * loop in
		 * named_glyphs_slot_hashed, q.v.
		 */
		glyphs_size = glyphs_size * 3 / 2;
	...
	glyphs = gs_alloc_struct_array(mem, glyphs_size, gs_copied_glyph_t,
	                               ...
	                               "gs_copy_font(glyphs)");
	if(have_names != 0)
		names = gs_alloc_struct_array(
		    mem, glyphs_size, gs_copied_glyph_name_t,
		    &st_gs_copied_glyph_name_element, "gs_copy_font(names)");
	copied =
	    gs_alloc_struct(mem, gs_font, fstype, "gs_copy_font(copied font)");
	cfdata =
	    gs_alloc_struct(mem, gs_copied_font_data_t, &st_gs_copied_font_data,
...
#define MAX_GLYPH_PIECES 64 /* arbitrary, but 32 is too small - bug 687698. */
	...
	/* Copy any sub-glyphs. */
	glyphs[0] = glyph;
	code = psf_add_subset_pieces(glyphs, &count, MAX_GLYPH_PIECES,
	                             MAX_GLYPH_PIECES, font);
	if(code < 0)
		return code;
...
gs_copy_font_complete(gs_font* font, gs_font* copied)
{
	 ...
	 */
	for(;;) {
		for(index = 0; code >= 0 && (font->procs.enumerate_glyph(
		                                 font, &index, space, &glyph),
		                             index != 0);)
			code = gs_copy_glyph(font, glyph, copied);
		/* For Type 42 fonts, if we copied by name, now copy again by
		 * index. */
	...
	if(cf_data(copied)->Encoding != 0)
		for(index = 0; code >= 0 && index < 256; ++index) {
			glyph = font->procs.encode_char(font, (gs_char)index,
			                                GLYPH_SPACE_NAME);
			if(glyph != GS_NO_GLYPH)
				code = gs_copied_font_add_encoding(
				    copied, (gs_char)index, glyph);
		}
...
copied_drop_extension_glyphs(gs_font* copied)
{
	...
	for(i = 0; i < gsize; i++) {
		...
		for(j = 0; j < l; j++)
			if(!memcmp(gx_extendeg_glyph_name_separator,
			           name->str.data + j, sl))
				break;
		if(j >= l)
			continue;
		...
		for(k = 0; k < gsize; k++)
			if(cfdata->glyphs[k].used &&
			   ...
			   !memcmp(cfdata->names[k].str.data, name->str.data,
			           j) &&
			   !bytes_compare(pslot->gdata.data, pslot->gdata.size,
			                  cfdata->glyphs[k].gdata.data,
			                  cfdata->glyphs[k].gdata.size)) {
				i0 = k;
				break;
			}
		...
		for(k = 0; k < gsize; k++)
			if(k != i0 && cfdata->glyphs[k].used &&
			   ...
			   !memcmp(cfdata->names[k].str.data, name->str.data,
			           j) &&
			   !memcmp(gx_extendeg_glyph_name_separator, name + j,
			           sl) &&
			   !bytes_compare(pslot->gdata.data, pslot->gdata.size,
			                  cfdata->glyphs[k].gdata.data,
			                  cfdata->glyphs[k].gdata.size))
