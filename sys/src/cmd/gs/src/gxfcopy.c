		       ...
		       gs_glyph_space_t glyph_space, gs_glyph *pglyph)
{
	...
	for(; *pindex < cfdata->glyphs_size; ++*pindex)
		if(cfdata->glyphs[*pindex].used) {
			*pglyph =
			    (glyph_space == GLYPH_SPACE_NAME && cfdata->names != 0 ? cfdata->names[*pindex].glyph :
...
copy_glyph_type42(gs_font *font, gs_glyph glyph, gs_font *copied, int options)
{
	...
	gs_font_type42 *const copied42 = (gs_font_type42 *)copied;
	uint gid = (options & COPY_GLYPH_BY_INDEX ? glyph - GS_MIN_GLYPH_INDEX : font->FontType == ft_CID_TrueType
...
copied_sub_type1_glyph_data(gs_font_type1 *pfont, gs_glyph glyph,
			    gs_glyph_data_t *pgd)
{
	return copied_cid0_glyph_data((gs_font_base *)cf_data((gs_font *)pfont)->parent,
				      glyph, pgd, NULL);
}
...
copy_font_cid0(gs_font *font, gs_font *copied)
{
	...
	copied0->cidata.FDBytes =
	    (copied0->cidata.FDArray_size <= 1 ? 0 : copied0->cidata.FDArray_size <= 256 ? 1 : 2);
