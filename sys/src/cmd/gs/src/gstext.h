 ...
 */
#define TEXT_HAS_MORE_THAN_ONE_(op, any) (((op)&any) & (((op)&any) - 1))
#define TEXT_OPERATION_IS_INVALID(op)                                          \
	(!((op)&TEXT_FROM_ANY) || !((op)&TEXT_DO_ANY) ||                       \
...
#define TEXT_PARAMS_ARE_INVALID(params)                                        \
	(TEXT_OPERATION_IS_INVALID((params)->operation) ||                     \
	 (((params)->operation & TEXT_FROM_ANY_SINGLE) &&                      \
	  ((params)->size != 1)))
...
#define TEXT_FROM_ANY /* only for testing and masking */                       \
	(TEXT_FROM_STRING | TEXT_FROM_BYTES | TEXT_FROM_CHARS |                \
	 TEXT_FROM_GLYPHS | TEXT_FROM_ANY_SINGLE)
...
#define public_st_gs_text_params() /* in gstext.c */                           \
	gs_public_st_composite(st_gs_text_params, gs_text_params_t,            \
	                       "gs_text_params", text_params_enum_ptrs,        \
	                       text_params_reloc_ptrs)
...
#define dev_t_proc_text_begin(proc, dev_t)                                     \
	int proc(dev_t* dev, gs_imager_state* pis,                             \
...
#define dev_proc_text_begin(proc) dev_t_proc_text_begin(proc, gx_device)
...
#endif
int gs_text_begin(gs_state* pgs, const gs_text_params_t* text, gs_memory_t* mem,
...
int gs_show_begin(gs_state*, const byte*, uint, gs_memory_t*, gs_text_enum_t**),
    gs_ashow_begin(gs_state*, floatp, floatp, const byte*, uint, gs_memory_t*,
                   gs_text_enum_t**),
    gs_widthshow_begin(gs_state*, floatp, floatp, gs_char, const byte*, uint,
                       gs_memory_t*, gs_text_enum_t**),
    gs_awidthshow_begin(gs_state*, floatp, floatp, gs_char, floatp, floatp,
                        const byte*, uint, gs_memory_t*, gs_text_enum_t**),
    gs_kshow_begin(gs_state*, const byte*, uint, gs_memory_t*,
                   gs_text_enum_t**),
    gs_xyshow_begin(gs_state*, const byte*, uint, const float*, const float*,
                    uint, gs_memory_t*, gs_text_enum_t**),
    gs_glyphshow_begin(gs_state*, gs_glyph, gs_memory_t*, gs_text_enum_t**),
    gs_cshow_begin(gs_state*, const byte*, uint, gs_memory_t*,
                   gs_text_enum_t**),
    gs_stringwidth_begin(gs_state*, const byte*, uint, gs_memory_t*,
                         gs_text_enum_t**),
    gs_charpath_begin(gs_state*, const byte*, uint, bool, gs_memory_t*,
                      gs_text_enum_t**),
    gs_glyphpath_begin(gs_state*, gs_glyph, bool, gs_memory_t*,
                       gs_text_enum_t**),
    gs_glyphwidth_begin(gs_state*, gs_glyph, gs_memory_t*, gs_text_enum_t**),
    gs_charboxpath_begin(gs_state*, const byte*, uint, bool, gs_memory_t*,
                         gs_text_enum_t**);
...
int gs_text_set_cache(gs_text_enum_t* pte, const double* values,
