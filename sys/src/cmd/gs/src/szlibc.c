...
/* Copyright (C) 1995, 1997, 1998, 1999 Aladdin Enterprises.  All rights
  reserved.
...
s_zlib_alloc_dynamic_state(stream_zlib_state* ss)
{
	...
	zlib_dynamic_state_t* zds = gs_alloc_struct_immovable(
	    mem, zlib_dynamic_state_t, &st_zlib_dynamic_state,
...
s_zlib_alloc(void* zmem, uint items, uint size)
{
	...
	zlib_block_t* block = gs_alloc_struct(mem, zlib_block_t, &st_zlib_block,
	                                      "s_zlib_alloc(block)");
	void* data = gs_alloc_byte_array_immovable(mem, items, size,
	                                           "s_zlib_alloc(data)");
