...
/* Copyright (C) 1994, 1995, 1996, 1998, 1999, 2000 Aladdin Enterprises.  All
  rights reserved.
...
gc_strings_set_marks(chunk_t* cp, bool mark)
{
	if(cp->smark != 0) {
		...
		if(mark)
			gc_mark_string(cp->sbase, cp->climit - cp->sbase, true,
			               cp);
	}
}
...
#define dprintstr()                                                            \
	...
	if(ptr < cp->ctop) {
		...
		return false;
	          ...
	          cp->climit) { /*
	                         * If this is the bottommost string in a chunk
	                         * that has
	                         * an inner chunk, the string's starting address
	                         * is both
	                         * cp->ctop of the outer chunk and cp->climit of
	                         * the inner;
	                         * gc_locate may incorrectly attribute the
	                         * string to the
	                         * inner chunk because of this.  This doesn't
	                         * affect
	                         * marking or relocation, since the machinery
	                         * for these
		...
		if(ptr + size > scp->climit) {
			lprintf4(
			    "String pointer 0x%lx[%u] outside [0x%lx..0x%lx)\n",
			    (uint32_t)ptr, size, (uint32_t)scp->ctop,
			    (uint32_t)scp->climit);
			return false;
		}
	}
...
#ifdef DEBUG
	if(gs_debug_c('5')) {
		dlprintf4("[5]%s%smarked 0x%lx[%u]", (marks ? "" : "already "),
		          (set ? "" : "un"), (uint32_t)ptr, size);
		dprintstr();
		dputc('\n');
	}
#endif
...
#define byte_count_zero_bits(byt) (uint)(count_zero_bits_table[byt])
#define byte_count_one_bits(byt) (uint)(8 - count_zero_bits_table[byt])
...
gc_strings_set_reloc(chunk_t* cp)
{
	if(cp->sreloc != 0 && cp->smark != 0) {
		byte* bot = cp->ctop;
		byte* top = cp->climit;
		uint count = (top - bot + (string_data_quantum - 1)) >>
...
#undef RELOC_TEST_1S
		...
		while(count--) {
			...
			reloc +=
			    string_data_quantum - byte_count_zero_bits(bitp[0]);
...
#undef R
		...
		while(from > lo) {
			...
			case 0xff:
				to -= 8;
				/*
				 * Since we've seen a byte other than 0xff, we
				 * know
		...
		gs_alloc_fill(cp->ctop, gs_alloc_fill_collected, to - cp->ctop);
		cp->ctop = to;
	}
}
