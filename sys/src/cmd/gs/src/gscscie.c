...
gs_private_st_ptrs1(st_color_space_CIEDEFG, gs_base_color_space,
                    "gs_color_space(CIEDEFG)", cs_CIEDEFG_enum_ptrs,
                    cs_CIEDEFG_reloc_ptrs, params.defg);
private
...
const gs_color_space_type gs_color_space_type_CIEDEFG = {
    gs_color_space_index_CIEDEFG, true, true, &st_color_space_CIEDEFG,
    gx_num_components_4, gx_no_base_space, gx_init_CIE, gx_restrict_CIEDEFG,
    gx_concrete_space_CIE, gx_concretize_CIEDEFG, NULL, gx_default_remap_color,
    gx_install_CIE, gx_spot_colors_set_overprint, gx_adjust_cspace_CIEDEFG,
    gx_no_adjust_color_count, gx_serialize_CIEDEFG,
    gx_cspace_is_linear_default};

/* CIEBasedDEF */
gs_private_st_ptrs1(st_color_space_CIEDEF, gs_base_color_space,
                    "gs_color_space(CIEDEF)", cs_CIEDEF_enum_ptrs,
                    cs_CIEDEF_reloc_ptrs, params.def);
private
...
const gs_color_space_type gs_color_space_type_CIEDEF = {
    gs_color_space_index_CIEDEF, true, true, &st_color_space_CIEDEF,
    gx_num_components_3, gx_no_base_space, gx_init_CIE, gx_restrict_CIEDEF,
    gx_concrete_space_CIE, gx_concretize_CIEDEF, NULL, gx_default_remap_color,
    gx_install_CIE, gx_spot_colors_set_overprint, gx_adjust_cspace_CIEDEF,
    gx_no_adjust_color_count, gx_serialize_CIEDEF, gx_cspace_is_linear_default};

/* CIEBasedABC */
gs_private_st_ptrs1(st_color_space_CIEABC, gs_base_color_space,
                    "gs_color_space(CIEABC)", cs_CIEABC_enum_ptrs,
                    cs_CIEABC_reloc_ptrs, params.abc);
private
...
const gs_color_space_type gs_color_space_type_CIEABC = {
    gs_color_space_index_CIEABC, true, true, &st_color_space_CIEABC,
    gx_num_components_3, gx_no_base_space, gx_init_CIE, gx_restrict_CIEABC,
    gx_concrete_space_CIE, gx_concretize_CIEABC, NULL, gx_remap_CIEABC,
    gx_install_CIE, gx_spot_colors_set_overprint, gx_adjust_cspace_CIEABC,
    gx_no_adjust_color_count, gx_serialize_CIEABC, gx_cspace_is_linear_default};

/* CIEBasedA */
gs_private_st_ptrs1(st_color_space_CIEA, gs_base_color_space,
                    "gs_color_space(CIEA)", cs_CIEA_enum_ptrs,
                    cs_CIEA_reloc_ptrs, params.a);
private
...
const gs_color_space_type gs_color_space_type_CIEA = {
    gs_color_space_index_CIEA, true, true, &st_color_space_CIEA,
    gx_num_components_1, gx_no_base_space, gx_init_CIE, gx_restrict_CIEA,
    gx_concrete_space_CIE, gx_concretize_CIEA, NULL, gx_default_remap_color,
    gx_install_CIE, gx_spot_colors_set_overprint, gx_adjust_cspace_CIEA,
    gx_no_adjust_color_count, gx_serialize_CIEA, gx_cspace_is_linear_default};
	...
	if(pcie == 0 || pcie->RenderTable.lookup.table == 0 ||
	   pcie->RenderTable.lookup.m == 3) {
		gs_cspace_init_DeviceRGB(
		    pis->memory, &rgb_cs); /* idempotent initialization */
		return &rgb_cs;
	} else { /* pcie->RenderTable.lookup.m == 4 */
		gs_cspace_init_DeviceCMYK(
		    pis->memory, &cmyk_cs); /* idempotent initialization */
		return &cmyk_cs;
	}
}
...
gs_cspace_build_CIEA(gs_color_space** ppcspace, void* client_data,
                     gs_memory_t* pmem)
{
	gs_cie_a* pciea = gx_build_cie_space(
	    ppcspace, &gs_color_space_type_CIEA, &st_cie_a, pmem);
...
gs_cspace_build_CIEABC(gs_color_space** ppcspace, void* client_data,
                       gs_memory_t* pmem)
{
	gs_cie_abc* pabc = gx_build_cie_space(
	    ppcspace, &gs_color_space_type_CIEABC, &st_cie_abc, pmem);
...
gs_cspace_build_CIEDEF(gs_color_space** ppcspace, void* client_data,
                       gs_memory_t* pmem)
{
	gs_cie_def* pdef = gx_build_cie_space(
	    ppcspace, &gs_color_space_type_CIEDEF, &st_cie_def, pmem);
...
gs_cspace_build_CIEDEFG(gs_color_space** ppcspace, void* client_data,
                        gs_memory_t* pmem)
{
	gs_cie_defg* pdefg = gx_build_cie_space(
	    ppcspace, &gs_color_space_type_CIEDEFG, &st_cie_defg, pmem);
...
{ /* p->DecodeA : */
	...
	code = sputs(s, (const byte*)&c->params.is_identity,
	             sizeof(c->params.is_identity), &n);
...
gx_serialize_cie_common_elements(const gs_color_space* pcs, stream* s)
{
	...
	for(k = 0; k < 3 && code >= 0; k++)
		code = gx_serialize_cie_cache(
		    &p->common.caches.DecodeLMN[k].floats, s);
	if(code < 0)
		return code;
...
gx_serialize_CIEABC(const gs_color_space* pcs, stream* s)
{
	...
	if(code < 0)
		return code;
	code = sputs(s, (const byte*)&p->caches.skipABC,
	             sizeof(p->caches.skipABC), &n);
	if(code < 0)
		return code;
	if(p->caches.skipABC)
		return 0;
	for(k = 0; k < 3 && code >= 0; k++)
		code = gx_serialize_cie_cache(
		    &p->caches.DecodeABC.caches[k].floats, s);
	if(code < 0)
		return code;
...
gx_serialize_lookup_table(const gx_color_lookup_table* t, stream* s)
{
	...
	if(code < 0)
		return code;
	code =
	    sputs(s, (const byte*)&t->dims[0], sizeof(t->dims[0]) * t->n, &n);
	if(code < 0)
		return code;
	...
	if(code < 0)
		return code;
	code =
	    sputs(s, (const byte*)&t->table->size, sizeof(t->table->size), &n);
	if(code < 0)
		return code;
...
gx_serialize_CIEDEF(const gs_color_space* pcs, stream* s)
{
	...
	for(k = 0; k < 3 && code >= 0; k++)
		code = gx_serialize_cie_cache(
		    &p->caches_def.DecodeDEF[k].floats, s);
	if(code < 0)
		return code;
...
gx_serialize_CIEDEFG(const gs_color_space* pcs, stream* s)
{
	...
	for(k = 0; k < 3 && code >= 0; k++)
		code = gx_serialize_cie_cache(
		    &p->caches_defg.DecodeDEFG[k].floats, s);
	if(code < 0)
		return code;
