...
private
dev_proc_map_rgb_color(tekink_map_rgb_color);
private
dev_proc_map_color_rgb(tekink_map_color_rgb);
private
dev_proc_print_page(tekink_print_page);
private
gx_device_procs tekink_procs =
...
*/
const gx_device_printer far_data gs_tek4696_device = prn_device(
    tekink_procs, "tek4696", 85, 120, /* Page size in 10th of inches */
...
{
	int line_size, color_line_size, scan_line, num_bytes, scan_lines,
	    color_plane;
	...
	for(scan_line = 0; scan_line < scan_lines; scan_line++) {
		...
		for(color_plane = 0; color_plane < 4; color_plane++) {
			outdata =
			    indataend + (color_plane * (color_line_size + 1));
			outdataend = outdata + color_line_size;

			/* Remove trailing spaces and output the color line if
			   it is
			   not blank */
			*outdata = 0xff;
			while(!(*outdataend))
			...
			if(num_bytes = (outdataend - outdata)) {
				line_blank = 0;
				/* On encountering the first non-blank data,
				   output pending
				   blank lines */
				if(blank_lines) {
					pending_micro_lines =
					    ((out_line + blank_lines + 1) / 4) -
					    (out_line / 4);
					for(micro_line = 0;
					    micro_line < pending_micro_lines;
					    micro_line++) {
				...
				fprintf(prn_stream, "\033I%c%03d",
				        '0' + (out_line % 4) + 4 * color_plane,
				        num_bytes);
		...
		if(line_blank && roll_paper) {
			/* Only increment the blank line count, if non blank
			   lines
			   have been encountered previously, i.e. skip leading
			   blank
			   lines. */
			if(out_line)
				blank_lines++;
