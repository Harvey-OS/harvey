...
gx_semaphore_t* /* returns a new semaphore, 0 if error */
    gx_semaphore_alloc(gs_memory_t* memory /* memory allocator to use */
                       )
{
	gx_semaphore_t* sema;

	/* sizeof decl'd sema struct, minus semaphore placeholder's size, +
	 * actual semaphore size */
	unsigned semaSizeof =
	...
	if(gp_semaphore_open(0) == 0) /* see if gp_semaphores are movable */
		...
		/* unmovable */
		sema = (gx_semaphore_t*)gs_alloc_bytes_immovable(
		    memory, semaSizeof, "gx_semaphore (create)");
	if(sema == 0)
		return 0;
...
void
gx_semaphore_free(gx_semaphore_t* sema /* semaphore to delete */
                  )
{
	if(sema) {
...
gx_monitor_t*                            /* returns a new monitor, 0 if error */
    gx_monitor_alloc(gs_memory_t* memory /* memory allocator to use */
                     )
{
	gx_monitor_t* mon;

	/* sizeof decl'd mon struct, minus monitor placeholder's size, + actual
	 * monitor size */
	unsigned monSizeof =
	...
	if(gp_monitor_open(0) == 0) /* see if gp_monitors are movable */
		...
		/* unmovable */
		mon = (gx_monitor_t*)gs_alloc_bytes_immovable(
		    memory, monSizeof, "gx_monitor (create)");
	if(mon == 0)
		return 0;
...
/* Dnit & free a monitor */
void
gx_monitor_free(gx_monitor_t* mon /* monitor to delete */
                )
{
	if(mon) {
