...
/* Forward references */
private
int shading_param(const_os_ptr op, const gs_shading_t** ppsh);
...
zbuildshadingpattern(i_ctx_t* i_ctx_p)
{
	...
	if((code = read_matrix(imemory, op - 1, &mat)) < 0 ||
	   (code = dict_uid_param(op2, &template.uid, 1, imemory, i_ctx_p)) !=
	       1 ||
	...
	code = gs_make_pattern(&cc_instance,
	                       (const gs_pattern_template_t*)&template, &mat,
	                       igs, imemory);
	if(code < 0) {
...
typedef int (*build_shading_proc_t)(i_ctx_t* i_ctx_p, const ref* op,
...
build_shading(i_ctx_t* i_ctx_p, build_shading_proc_t proc)
{
	...
	/* Collect parameters common to all shading types. */
	{
		...
		if(dict_find_string(op, "Background", &pvalue) > 0) {
			gs_client_color* pcc = ialloc_struct(
			    gs_client_color, &st_client_color, "build_shading");
			...
			code = dict_floats_param(
			    imemory, op, "Background",
	...
	else if((code = dict_floats_param(imemory, op, "BBox", 4, box, NULL)) ==
	        4) {
	...
	*ppfn = 0;
	...
	if(r_is_array(pFunction)) {
		...
		for(i = 0; i < size; ++i) {
			...
			code = fn_build_function(i_ctx_p, &rsubfn,
			                         &Functions[i], mem);
			if(code < 0)
				break;
		}
...
private
int
build_shading_1(i_ctx_t* i_ctx_p, const ref* op,
                const gs_shading_params_t* pcommon, gs_shading_t** ppsh,
                gs_memory_t* mem)
{
	...
	*(gs_shading_params_t*)&params = *pcommon;
	...
	params.Function = 0;
	if((code = dict_floats_param(imemory, op, "Domain", 4, params.Domain,
	                             default_Domain)) < 0 ||
	   (dict_find_string(op, "Matrix", &pmatrix) > 0 &&
	    (code = read_matrix(imemory, pmatrix, &params.Matrix)) < 0) ||
	   (code = build_shading_function(i_ctx_p, op, &params.Function, 2,
	                                  mem)) < 0 ||
	   (code = check_indexed_vs_function(params.ColorSpace,
	                                     params.Function)) < 0 ||
...
private
int
build_directional_shading(i_ctx_t* i_ctx_p, const ref* op, float* Coords,
                          int num_Coords, float Domain[2],
                          gs_function_t** pFunction, bool Extend[2],
                          gs_memory_t* mem)
{
	int code =
	    dict_floats_param(imemory, op, "Coords", num_Coords, Coords, NULL);
...
private
int
build_shading_2(i_ctx_t* i_ctx_p, const ref* op,
                const gs_shading_params_t* pcommon, gs_shading_t** ppsh,
                gs_memory_t* mem)
{
...
private
int
build_shading_3(i_ctx_t* i_ctx_p, const ref* op,
                const gs_shading_params_t* pcommon, gs_shading_t** ppsh,
                gs_memory_t* mem)
{
...
build_mesh_shading(i_ctx_t* i_ctx_p, const ref* op,
                   gs_shading_mesh_params_t* params, float** pDecode,
                   gs_function_t** pFunction, gs_memory_t* mem)
{
	int code;
	float* data = 0;
	ref* pDataSource;
	...
	*pFunction = 0;
	...
	if(data_source_is_array(params->DataSource)) {
		...
		int num_decode =
		    4 +
		    (*pFunction != 0 ? 1 : gs_color_space_num_components(
		                               params->ColorSpace)) *
		        2;
		...
		if((code = dict_int_param(op, "BitsPerCoordinate", 1, 32, 0,
		   ...
		   (code = dict_int_param(op, "BitsPerComponent", 1, 16, 0,
		                          &params->BitsPerComponent)) >= 0) {
			*pDecode = (float*)gs_alloc_byte_array(
			    mem, num_decode, sizeof(float),
			    "build_mesh_shading");
			if(*pDecode == 0)
				code = gs_note_error(e_VMerror);
			else {
...
private
int
build_shading_4(i_ctx_t* i_ctx_p, const ref* op,
                const gs_shading_params_t* pcommon, gs_shading_t** ppsh,
                gs_memory_t* mem)
{
	...
	*(gs_shading_params_t*)&params = *pcommon;
	if((code = build_mesh_shading(
	        i_ctx_p, op, (gs_shading_mesh_params_t*)&params, &params.Decode,
	        &params.Function, mem)) < 0 ||
	   (code = check_indexed_vs_function(params.ColorSpace,
	                                     params.Function)) < 0 ||
...
private
int
build_shading_5(i_ctx_t* i_ctx_p, const ref* op,
                const gs_shading_params_t* pcommon, gs_shading_t** ppsh,
                gs_memory_t* mem)
{
...
private
int
build_shading_6(i_ctx_t* i_ctx_p, const ref* op,
                const gs_shading_params_t* pcommon, gs_shading_t** ppsh,
                gs_memory_t* mem)
{
...
private
int
build_shading_7(i_ctx_t* i_ctx_p, const ref* op,
                const gs_shading_params_t* pcommon, gs_shading_t** ppsh,
                gs_memory_t* mem)
{
