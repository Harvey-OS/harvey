...
typedef int (*build_shading_proc_t)(i_ctx_t *i_ctx_p, const ref *op, const gs_shading_params_t *params,
		   ...
		   gs_memory_t *mem)
{
	...
	*pFunction = 0;
	...
	if(data_source_is_array(params->DataSource)) {
		...
		int num_decode = 4 +
				 (*pFunction != 0 ? 1 : gs_color_space_num_components(params->ColorSpace)) * 2;
