...
zsetindexedspace(i_ctx_t* i_ctx_p)
{
	...
	if(r_has_type(&pcsa[2], t_string)) {
		...
		check_read(pcsa[2]);
		/*
		 * The PDF and PS specifications state that the lookup table
		 * must have
		 * the exact number of of data bytes needed.  However we have
		 * found
		 * PDF files from Amyuni with extra data bytes.  Acrobat 6.0
		 * accepts
...
indexed_map1(i_ctx_t* i_ctx_p)
{
	...
	if(i >= 0) { /* i.e., not first time */
		...
		int code =
		    float_params(op, m, &r_ptr(&ep[csme_map], gs_indexed_map)
		                             ->values[i * m]);
...
const op_def zcsindex_l2_op_defs[] = {op_def_begin_level2(),
              ...
              op_proc_t map1)
{
	...
	gs_indexed_map* map;
	int code =
	    alloc_indexed_map(&map, num_values, mem, "setcolorspace(mapped)");
	es_ptr ep;
