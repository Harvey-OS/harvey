...
mem_alpha_map_rgb_color(gx_device *dev, const gx_color_value cv[])
{
	...
	return (color == 0 || color == gx_no_color_index ? color : (gx_color_index)((1 << mdev->log2_alpha_bits) - 1));
}
private
int
...
{
	return gx_forward_map_color_rgb(dev,
			      ...
			      gx_color_value g, gx_color_value b, gx_color_value alpha)
{
	...
	return (color == 0 || color == gx_no_color_index ? color : (gx_color_index)(alpha >> (gx_color_value_bits -
...
{ /* Just use copy_color. */
	return (color == 0 ? (*dev_proc(dev, fill_rectangle))(dev, x, y, width, height,
							      color)
...
private
const gx_device_memory mem_alpha_buffer_device =
