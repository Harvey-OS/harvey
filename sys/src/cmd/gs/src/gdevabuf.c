...
private
dev_proc_map_rgb_color(mem_alpha_map_rgb_color);
private
dev_proc_map_color_rgb(mem_alpha_map_color_rgb);
private
dev_proc_map_rgb_alpha_color(mem_alpha_map_rgb_alpha_color);
private
dev_proc_copy_alpha(mem_alpha_copy_alpha);

void
gs_make_mem_alpha_device(gx_device_memory* adev, gs_memory_t* mem,
                         gx_device* target, int alpha_bits)
{
	gs_make_mem_device(adev, gdev_mem_device_for_bits(alpha_bits), mem, 0,
	                   target);
...
mem_alpha_map_rgb_color(gx_device* dev, const gx_color_value cv[])
{
	...
	return (color == 0 || color == gx_no_color_index
	            ? color
...
{
	return gx_forward_map_color_rgb(
	    dev, (color == 0 ? color : (gx_color_index)1), prgb);
}
private
gx_color_index
...
private
int
mem_alpha_copy_alpha(gx_device* dev, const byte* data, int data_x, int raster,
...
{ /* Just use copy_color. */
	return (color == 0
	            ? (*dev_proc(dev, fill_rectangle))(dev, x, y, width, height,
	                                               color)
	            : (*dev_proc(dev, copy_color))(dev, data, data_x, raster,
	                                           id, x, y, width, height));
}
...
/* Procedures */
private
dev_proc_close_device(mem_abuf_close);
private
dev_proc_copy_mono(mem_abuf_copy_mono);
private
dev_proc_fill_rectangle(mem_abuf_fill_rectangle);
private
dev_proc_get_clipping_box(mem_abuf_get_clipping_box);

/* The device descriptor. */
private
const gx_device_memory mem_alpha_buffer_device = mem_device(
    "image(alpha buffer)", 0, 1, gx_forward_map_rgb_color,
    gx_forward_map_color_rgb, mem_abuf_copy_mono, gx_default_copy_color,
    mem_abuf_fill_rectangle, gx_no_strip_copy_rop);
                        ...
                        int alpha_bits, int mapped_x)
{
	...
	adev->color_info.anti_alias.text_bits =
	    adev->color_info.anti_alias.graphics_bits = alpha_bits;
}
...
abuf_flush_block(gx_device_memory* adev, int y)
{
	...
	int ddepth = (adev->width >> adev->log2_scale.x)
	...
	{ /*
	   ...
	   */
		...
		bits_compress_scaled(bits, bbox.p.x, width, block_height,
		                     adev->raster, bits, draster,
		                     &adev->log2_scale, adev->log2_alpha_bits);
		return (*dev_proc(target, copy_alpha))(
		    target, bits, 0, draster, gx_no_bitmap_id,
		    (adev->mapped_x + bbox.p.x) >> adev->log2_scale.x,
		    y >> adev->log2_scale.y, width >> adev->log2_scale.x, 1,
...
y_transfer_next(y_transfer* pyt, gx_device* dev)
{
	...
	if(ty == my + mh) { /* Add a new block at my1. */
		...
		memset(scan_line_base(mdev, (ms == 0 ? mh : ms) - bh), 0,
		       bh * mdev->raster);
	}
	...
	if_debug7(
	    'V', "[V]abuf: my=%d, mh=%d, ms=%d, ty=%d, th=%d, tby=%d, tbh=%d\n",
...
private
int
mem_abuf_copy_mono(gx_device* dev, const byte* base, int sourcex, int sraster,
                   ...
                   gx_color_index zero, gx_color_index one)
{
	...
	x -= mdev->mapped_x;
	fit_copy_xyw(dev, base, sourcex, sraster, id, x, y, w,
	             h); /* don't limit h */
	if(w <= 0 || h <= 0)
		return 0;
	mdev->save_color = one;
	...
	while(yt.height_left > 0) {
		...
		(*dev_proc(&mem_mono_device, copy_mono))(
		    dev, base + (yt.y_next - y) * sraster, sourcex, sraster,
		    gx_no_bitmap_id, x, yt.transfer_y, w, yt.transfer_height,
                        ...
                        gx_color_index color)
{
	...
	while(yt.height_left > 0) {
		y_transfer_next(&yt, dev);
		(*dev_proc(&mem_mono_device, fill_rectangle))(
		    dev, x, yt.transfer_y, w, yt.transfer_height,
		    (gx_color_index)1);
	}
	return 0;
}
