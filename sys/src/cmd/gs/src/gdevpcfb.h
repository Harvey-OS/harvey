...
#define ega_device(dev_name, procs, fb_raster, screen_height, aspect_ratio,    \
                   video_mode)                                                 \
	{                                                                      \
		std_device_dci_body(                                           \
		    gx_device_ega, &procs, dev_name, fb_raster * 8,            \
		    screen_height, (screen_height * (aspect_ratio)) /          \
		                       PAGE_HEIGHT_INCHES, /* x dpi */         \
		    ...
		    {0}, /* std_procs */                                       \
		    fb_raster, (fb_raster & 15 ? fb_raster : fb_raster >> 4),  \
		    (fb_raster & 15 ? fb_raster : 0), video_mode               \
	}
...
{
	__asm__ volatile("outb %0,%1" ::"a"((unsigned char)data),
	                 "d"((unsigned short)port));
}
static inline void
outport2(int port, int index, int data)
{
	__asm__ volatile("movb %0,%%ah; movb %1,%%al; outw %%ax,%2" ::"qmi"(
	                     (unsigned char)data),
	                 "qmi"((unsigned char)index), "d"((unsigned short)port)
	                 : "eax");
}
#else
