...
/* GC procedures */
ENUM_PTRS_WITH(gs_param_typed_value_enum_ptrs,
               gs_param_typed_value* pvalue) return 0;
case 0:
switch(pvalue->type) {
...
private
const byte xfer_item_sizes[] = {GS_PARAM_TYPE_SIZES(0)};
int
	...
	for(pi = items; pi->key != 0; ++pi) {
		...
		case 0:
			if(typed.type != pi->type) /* shouldn't happen! */
				...
				memcpy(pvalue, &typed.value,
				       xfer_item_sizes[pi->type]);
		}
	}
	return ecode;
}
                     ...
                     const void* default_obj, const gs_param_item_t* items)
{
	...
	for(pi = items; pi->key != 0; ++pi) {
		const char* key = pi->key;
		const void* pvalue =
		    (const void*)((const char*)obj + pi->offset);
...
int
param_write_float(gs_param_list* plist, gs_param_name pkey, const float* pvalue)
{
