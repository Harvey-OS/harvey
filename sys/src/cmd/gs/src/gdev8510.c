...
private
dev_proc_print_page(m8510_print_page);
const gx_device_printer far_data gs_m8510_device =
    prn_device(prn_std_procs, "m8510", 85, /* width_10ths, 8.5" */
               110,                        /* height_10ths, 11" */
               160,                        /* x_dpi */
               144,                        /* y_dpi */
...
private
void m8510_output_run(gx_device_printer* pdev, byte* out, int pass,
                      FILE* prn_stream);
...
m8510_print_page(gx_device_printer* pdev, FILE* prn_stream)
{
	...
	byte* in1 = (byte*)gs_malloc(pdev->memory, 8, line_size,
	                             "m8510_print_page(in1)");
	byte* in2 = (byte*)gs_malloc(pdev->memory, 8, line_size,
	                             "m8510_print_page(in2)");
	byte* out = (byte*)gs_malloc(pdev->memory, 8, line_size,
	                             "m8510_print_page(out)");
	int lnum = 0;
	int code = 0;
	...
	while(lnum < pdev->height) {
		...
		for(i = 7; i >= 0; i--) {
			gdev_prn_copy_scan_lines(
			    pdev, lnum, &in1[i * line_size], line_size);
			lnum++;
			gdev_prn_copy_scan_lines(
			    pdev, lnum, &in2[i * line_size], line_size);
			lnum++;
		}
...
out:
	;
	if(out)
		gs_free(pdev->memory, (char*)out, 8, line_size,
		        "m8510_print_page(out)");
	if(in2)
		gs_free(pdev->memory, (char*)in2, 8, line_size,
		        "m8510_print_page(in2)");
	if(in1)
		gs_free(pdev->memory, (char*)in1, 8, line_size,
		        "m8510_print_page(in1)");
...
private
void
m8510_output_run(gx_device_printer* pdev, byte* out, int pass, FILE* prn_stream)
{
	 ...
	 */
	while(out_end > out && out_end[-1] == 0 && out_end[-2] == 0 &&
	      out_end[-3] == 0 && out_end[-4] == 0 && out_end[-5] == 0 &&
	      out_end[-6] == 0 && out_end[-7] == 0 && out_end[-8] == 0)
		out_end -= 8;
