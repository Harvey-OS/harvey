...
GLOBAL UW16
PCLswapHdr(FSP LPUB8 p, UW16 gifct); /* UFST header doesn't define it. */
...
private
inline fapi_ufst_server*
If_to_I(FAPI_server* If)
{
...
{ /*  The server keeps character raster between calls to get_char_raster_metrics
      ...
      sequence of calls may be interrupted by an error in CDefProc or
     setchachedevice2,
...
open_UFST(fapi_ufst_server* r)
{
	...
	if((code = CGIFinit(&r->IFS)) != 0)
		return code;
	r->IFS.mem_avail[BUFFER_POOL] =
	    6000000L; /* For Asian TT fonts with vertical writing mode. */
...
private
FAPI_retcode
ensure_open(FAPI_server* server)
{
...
get_font_type(FILE* f)
{
	...
	if(fread(buf, 1, sizeof(buf), f) != sizeof(buf))
		return 0;
	if(buf[0] == 0x13 ||
	   buf[0] == 0x14) /* fixme : don't know how to do correctly. */
		return FC_FCO_TYPE;
...
private
int
choose_decoding_PS(fapi_ufst_server* r, ufst_common_font_data* d,
                   const char* cmapId)
{
                   ...
                   const char* cmapId)
{
	...
	UW16 font_access;
	bool failed;
	void* p = (d->is_disk_font ? (void*)(d + 1)
	                           : (void*)((UB8*)d + d->tt_font_body_offset));
...
private
void
scan_xlatmap(fapi_ufst_server* r, ufst_common_font_data* d, const char* xlatmap,
             const char* font_kind,
             int (*choose_proc)(fapi_ufst_server* r, ufst_common_font_data* d,
                                const char* cmapId))
{
	...
	while(*p) {
		...
		while(*p) {
			const char* cmapId = p,
			            * decodingID = p + strlen(p) + 1;
			strncpy(d->decodingID, decodingID,
			        sizeof(d->decodingID));
			if(!decodingID[0])
				break;
                ...
                const char* xlatmap)
{
	if(xlatmap != 0)
		...
		case FC_PST1_TYPE:
			...
			break;
		case FC_TT_TYPE:
			scan_xlatmap(r, d, xlatmap, "TrueType",
			             choose_decoding_TT);
			break;
		case FC_FCO_TYPE:
			scan_xlatmap(r, d, xlatmap, "PostScript",
			             choose_decoding_PS /* fixme */);
			break;
		}
}
...
private
LPUB8
get_TT_glyph(fapi_ufst_server* r, FAPI_font* ff, UW16 chId)
{
	pcleo_glyph_list_elem* g;
	PCLETTO_CHDR* h;
	ufst_common_font_data* d = (ufst_common_font_data*)r->fc.font_hdr - 1;
	LPUB8 q;
	uint16_t glyph_length = ff->get_glyph(ff, chId, 0, 0);
	...
	if(glyph_length == (uint16_t)-1) {
		r->callback_error = e_invalidfont;
		return 0;
	}
	g = (pcleo_glyph_list_elem*)r->client_mem.alloc(
	    &r->client_mem,
	...
	if(use_XL_format) {
		h->h.continuation = 2;
		q = (LPUB8)h + 2;
		store_word(&q, (uint16_t)(glyph_length + 10));
		store_word(
		    &q,
		    (uint16_t)(r->sb_x >>
		               r->If.frac_shift)); /* see can_replace_metrics */
		store_word(&q, (uint16_t)(r->aw_x >> r->If.frac_shift));
		store_word(&q, 0);
		store_word(&q, chId);
	} else {
		h->h.continuation = 0;
		...
		store_word(&q, (uint16_t)(glyph_length + 4));
		store_word(&q, chId);
	}
	if(ff->get_glyph(ff, chId, (LPUB8)q, glyph_length) == (uint16_t)-1) {
		r->callback_error = e_invalidfont;
		return 0;
	}
	q += glyph_length;
...
private
LPUB8
get_T1_glyph(fapi_ufst_server* r, FAPI_font* ff, UW16 chId)
{
	uint16_t glyph_length = ff->get_glyph(ff, chId, 0, 0);
	LPUB8 q;
	pcleo_glyph_list_elem* g = (pcleo_glyph_list_elem*)r->client_mem.alloc(
	    &r->client_mem, sizeof(pcleo_glyph_list_elem) +
	                        sizeof(PS_CHAR_HDR) + 2 + 2 + glyph_length + 1,
	    "PSEO char");
	PS_CHAR_HDR* h;
	ufst_common_font_data* d = (ufst_common_font_data*)r->fc.font_hdr - 1;

	if(g == 0 || glyph_length == (uint16_t)-1) {
		r->callback_error = e_invalidfont;
		return 0;
	}
	g->chId = chId;
...
private
LPUB8
gs_PCLEO_charptr(LPUB8 pfont_hdr, UW16 sym_code)
{       /* never called */
...
private
LPUB8
gs_PCLchId2ptr(IF_STATE* pIFS, UW16 chId)
{
...
private
LPUB8
gs_PCLglyphID2Ptr(IF_STATE* pIFS, UW16 glyphID)
{
...
private
void
pack_pseo_word_array(fapi_ufst_server* r, FAPI_font* ff, UB8** p,
                     UW16 max_count, fapi_font_feature count_id,
                     fapi_font_feature array_id)
{
...
private
void
pack_pseo_fhdr(fapi_ufst_server* r, FAPI_font* ff, UB8* p)
{
	ushort j, n, skip = 0;
	...
	for(j = 0; j < 6; j++)
		pack_float(&p,
		           ff->get_float(ff, FAPI_FONT_FEATURE_FontMatrix, j));
	while((UL32)p & 0x03) /* align to QUADWORD */
		PACK_ZERO(p);
	...
	pack_pseo_word_array(r, ff, &p, 14, FAPI_FONT_FEATURE_BlueValues_count,
	                     FAPI_FONT_FEATURE_BlueValues);
	pack_pseo_word_array(r, ff, &p, 10, FAPI_FONT_FEATURE_OtherBlues_count,
	                     FAPI_FONT_FEATURE_OtherBlues);
	pack_pseo_word_array(r, ff, &p, 14, FAPI_FONT_FEATURE_FamilyBlues_count,
	...
	pack_pseo_word_array(r, ff, &p, 12, FAPI_FONT_FEATURE_StemSnapH_count,
	                     FAPI_FONT_FEATURE_StemSnapH);
	pack_pseo_word_array(r, ff, &p, 12, FAPI_FONT_FEATURE_StemSnapV_count,
...
private
void
enumerate_fco(fapi_ufst_server* r, const char* font_file_path)
         ...
         fco_list_elem** result)
{
	...
	if(e == 0) {
		SW16 fcHandle;
		if((code = CGIFfco_Open(&r->IFS, (UB8*)font_file_path,
		                        &fcHandle)) != 0)
			return code;
		e = (fco_list_elem*)r->client_mem.alloc(
		    &r->client_mem, sizeof(*e), "fco_list_elem");
		if(e == 0) {
...
private
FAPI_retcode
make_font_data(fapi_ufst_server* r, const char* font_file_path, int subfont,
               FAPI_font* ff, ufst_common_font_data** return_data)
{
	...
	*return_data = 0;
	...
	if(ff->font_file_path == NULL) {
		...
		if(ff->is_type1) {
			int subrs_count =
			    ff->get_word(ff, FAPI_FONT_FEATURE_Subrs_count, 0);
			int subrs_length = ff->get_long(
			    ff, FAPI_FONT_FEATURE_Subrs_total_size, 0);
			int subrs_area_size =
			    subrs_count * 5 + subrs_length + 2;
			area_length +=
			    360 + subrs_area_size; /* some inprecise - see
			                              pack_pseo_fhdr */
		} else {
			tt_size =
			    ff->get_long(ff, FAPI_FONT_FEATURE_TT_size, 0);
			if(tt_size == 0)
				return e_invalidfont;
			area_length +=
			    tt_size + (use_XL_format ? 6 : 4) + 4 + 2;
		}
	} else
		area_length += strlen(font_file_path) + 1;
	buf =
	    r->client_mem.alloc(&r->client_mem, area_length, "ufst font data");
	if(buf == 0)
		return e_VMerror;
	...
	if(d->is_disk_font) {
		FILE* f = fopen(font_file_path, "rb"); /* note: gp_fopen isn't
		                                          better since UFST
		                                          calls fopen. */
		if(f == NULL)
		...
		if(d->font_type == FC_FCO_TYPE) {
			...
			if((code = fco_open(r, font_file_path, &e)) != 0)
				return code;
			enumerate_fco(
			    r, font_file_path); /* development perpose only */
		...
		h->fontType = 11;
		    /* wrong */ /*  3- 11=Unicode; 0,1,2 also possible */
		h->style_msb = 0;
		    /* wrong */ /*  4- from PCLT table in TrueType font */
		h->reserved1 = 0;
		h->baselinePosition = 0;
		    /* wrong */ /*  6- from head table in TT font; = 0 */
		h->cellWidth = 1024; /* wrong */ /*  8- head, use xMax - xMin */
		h->cellHeight = 1024;
		    /* wrong */     /* 10- head, use yMax - yMin */
		h->orientation = 0; /* 12- 0 */
		h->spacing = 1;
		    /* wrong */ /* 13- 1=proportional, 0-fixed pitch */
		h->characterSet = 56;
		    /* wrong */ /* 14- same as symSetCode; =56 if unbound. */
		...
		h->textHeight = 102;
		    /* wrong */ /* 32- from OS/2 table in TT font */
		h->textWidth = 1024; /* wrong */ /* 34- OS/2 */
		h->firstCode = 0;                /* 36- set to 0 if unbound */
		h->lastCode = 255;
		    /* wrong */    /* 38- max number of downloadable chars if
		                      unbound */
		...
		h->masterUnderlinePosition = 0;
		    /* wrong */ /* 66- post table, or -20% of em */
		h->masterUnderlineHeight = 0;
		    /* wrong */ /* 68- post table, or 5% of em */
		h->fontScalingTechnology =
		    1;          /* 70- 1=TrueType; 0=Intellifont */
		h->variety = 0; /* 71- 0 if TrueType */
		memset(
		    (LPUB8)h + PCLETTOFONTHDRSIZE, 0,
		    8); /* work around bug in PCLswapHdr : it wants format 10 */
		/*  fixme : Most fields above being marked "wrong" look unused
		   by UFST.
		    Need to check for sure.
		*/
		/*  fixme : This code assumes 1-byte alignment for PCLETTO_FHDR
		   structure.
...
private
FAPI_retcode
get_scaled_font(FAPI_server* server, FAPI_font* ff, int subfont,
	...
	*/
	...
	if(ff->is_cid && ff->is_type1 && ff->font_file_path == NULL &&
	   (dc == FAPI_TOPLEVEL_BEGIN || dc == FAPI_TOPLEVEL_COMPLETE)) {
		/* Don't need any processing for the top level font of a
		   non-disk CIDFontType 0.
	...
	ff->need_decrypt = 1;
	if(d == 0) {
		if((code = make_font_data(r, ff->font_file_path, subfont, ff,
		                          &d)) != 0)
			return code;
		ff->server_font_data = d;
		prepare_typeface(r, d);
		if(ff->font_file_path != NULL ||
		   ff->is_type1) /* such fonts don't use RAW_GLYPH */
			choose_decoding(r, d, xlatmap);
	} else
		prepare_typeface(r, d);
	r->tran_xx = font_scale->matrix[0] / scale,
	r->tran_xy = font_scale->matrix[1] / scale;
	r->tran_yx = font_scale->matrix[2] / scale,
	r->tran_yy = font_scale->matrix[3] / scale;
	...
	fc->format |=
	    FC_NON_Z_WIND; /* NON_ZERO Winding required for TrueType */
...
private
FAPI_retcode
get_font_proportional_feature(FAPI_server* server, FAPI_font* ff, int subfont,
                              bool* bProportional)
{
	fapi_ufst_server* r = If_to_I(server);
	UB8 buf[74];
	...
	*bProportional = false;
	...
	if(ff->font_file_path == NULL || ff->is_type1)
		return 0;
	if(CGIFtt_query(&r->IFS, (UB8*)ff->font_file_path, *(UL32*)"OS/2",
	                (UW16)subfont, &length, buf) != 0)
		return 0; /* No OS/2 table - no chance to get the info. Use
		             default == false. */
...
private
inline void
make_asciiz_char_name(char* buf, int buf_length, FAPI_char_ref* c)
{
...
private
FAPI_retcode
can_retrieve_char_by_name(FAPI_server* server, FAPI_font* ff, FAPI_char_ref* c,
                          int* result)
{
	fapi_ufst_server* r = If_to_I(server);
...
{
	*result =
	    (!ff->is_type1 && ff->font_file_path == NULL &&
...
private
int
export_outline(fapi_ufst_server* r, PIFOUTLINE pol, FAPI_path* p)
{
	...
	for(i = 0; i < num_contrs; i++) {
		...
		for(j = 0; j < num_segmts; j++) {
			...
			if(*segment == 0x00) {
				if((code = p->moveto(p, points->x,
				                     points->y)) != 0)
					return code;
				points++;
			} else if(*segment == 0x01) {
				if((code = p->lineto(p, points->x,
				                     points->y)) != 0)
					return code;
				points++;
				...
				if((code = p->curveto(p, points[0].x,
...
private
inline void
set_metrics(fapi_ufst_server* r, FAPI_metrics* metrics, SL32 design_bbox[4],
            SW16 design_escapement, int escapement, SW16 du_emx, SW16 du_emy)
{
...
private
FAPI_retcode
get_char(fapi_ufst_server* r, FAPI_font* ff, FAPI_char_ref* c, FAPI_path* p,
         FAPI_metrics* metrics, UW16 format)
{
	UW16 code;
	UW16 cc = (UW16)c->char_code;
	SL32 design_bbox[4];
	...
	{ /* hack : Changing UFST internal data. Change to r->fc doesn't help,
	     because Agfa thinks that the "outline/raster" is a property of
	     current font. */
	...
	code = CGIFchar_with_design_bbox(&r->IFS, cc, &result, (SW16)0,
	...
	if(code == ERR_find_cgnum) {
		...
		code =
		    CGIFchar_with_design_bbox(&r->IFS, c1, &result, (SW16)0,
		                              design_bbox, &design_escapement);
	...
	if(format == FC_BITMAP_TYPE) {
		...
		set_metrics(r, metrics, design_bbox, design_escapement,
		            pbm->escapement, pbm->du_emx, pbm->du_emy);
		r->char_data = pbm;
		r->bRaster = true;
	} else {
		IFOUTLINE* pol = (IFOUTLINE*)result;
		set_metrics(r, metrics, design_bbox, design_escapement,
		            pol->escapement, pol->du_emx, pol->du_emy);
...
private
FAPI_retcode
get_char_outline_metrics(FAPI_server* server, FAPI_font* ff, FAPI_char_ref* c,
                         FAPI_metrics* metrics)
{
	fapi_ufst_server* r = If_to_I(server);
	...
	/*	UFST cannot render enough metrics information without generating
	   raster or outline.
...
private
FAPI_retcode
get_char_raster_metrics(FAPI_server* server, FAPI_font* ff, FAPI_char_ref* c,
                        FAPI_metrics* metrics)
{
	fapi_ufst_server* r = If_to_I(server);
	int code;
	...
	if(code == ERR_bm_buff ||
	   code == ERR_bm_too_big) /* Too big character ? */
		return e_limitcheck;
	return code;
	/*	UFST cannot render enough metrics information without generating
	   raster or outline.
...
get_char_raster(FAPI_server* server, FAPI_raster* rast)
{
	...
	if(!r->bRaster)
		...
		rast->line_step = (pbm->width + (1 << (CHUNK_SHIFT - 3)) - 1) >>
		                  (CHUNK_SHIFT - 3);
		if(rast->width != 0) {
	...
	for(; *e != 0;)
		if((*e)->fcHandle == fcHandle && (--(*e)->open_count) == 0) {
			...
			*e = ee->next;
			...
			r->client_mem.free(&r->client_mem, ee->file_path,
			                   "fco_file_path");
...
private
void gs_fapiufst_finit(i_plugin_instance* instance,
                       i_plugin_client_memory* mem);

private
const i_plugin_descriptor ufst_descriptor = {"FAPI", "AgfaUFST",
                                             gs_fapiufst_finit};

private
const FAPI_server If0 = {{&ufst_descriptor},
                         16, /* frac_shift */
                         ensure_open,
                         get_scaled_font,
                         get_decodingID,
...
int
gs_fapiufst_instantiate(i_ctx_t* i_ctx_p, i_plugin_client_memory* client_mem,
                        i_plugin_instance** p_instance)
{
	fapi_ufst_server* r = (fapi_ufst_server*)client_mem->alloc(
	    client_mem, sizeof(fapi_ufst_server), "fapi_ufst_server");
...
private
void
gs_fapiufst_finit(i_plugin_instance* this, i_plugin_client_memory* mem)
{
