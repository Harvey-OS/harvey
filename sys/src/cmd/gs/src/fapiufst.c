...
private
int
choose_decoding_PS(fapi_ufst_server *r, ufst_common_font_data *d, const char *cmapId)
{
...
private
int
choose_decoding_TT(fapi_ufst_server *r, ufst_common_font_data *d, const char *cmapId)
{
	int platId, specId, i;
	CMAP_QUERY q;
	UW16 font_access;
...
private
void
scan_xlatmap(fapi_ufst_server *r, ufst_common_font_data *d, const char *xlatmap, const char *font_kind,
...
private
void
choose_decoding(fapi_ufst_server *r, ufst_common_font_data *d, const char *xlatmap)
{
	if(xlatmap != 0)
		switch(d->font_type) {
		case FC_IF_TYPE: /* fixme */
			break;
		case FC_PST1_TYPE:
			scan_xlatmap(r, d, xlatmap, "PostScript", choose_decoding_PS);
			break;
		case FC_TT_TYPE:
			scan_xlatmap(r, d, xlatmap, "TrueType", choose_decoding_TT);
			break;
		case FC_FCO_TYPE:
			scan_xlatmap(r, d, xlatmap, "PostScript", choose_decoding_PS /* fixme */);
			break;
		}
}
...
get_TT_glyph(fapi_ufst_server *r, FAPI_font *ff, UW16 chId)
{
	...
	if(use_XL_format) {
		...
		store_word(&q, (uint16_t)(r->sb_x >> r->If.frac_shift)); /* see can_replace_metrics */
...
private
void
pack_pseo_word_array(fapi_ufst_server *r, FAPI_font *ff, UB8 **p, UW16 max_count, fapi_font_feature count_id, fapi_font_feature array_id)
{
...
private
FAPI_retcode
make_font_data(fapi_ufst_server *r,
	       const char *font_file_path,
...
private
FAPI_retcode
get_font_proportional_feature(FAPI_server *server, FAPI_font *ff, int subfont, bool *bProportional)
{
	fapi_ufst_server *r = If_to_I(server);
	UB8 buf[74];
...
private
FAPI_retcode
can_retrieve_char_by_name(FAPI_server *server, FAPI_font *ff, FAPI_char_ref *c, int *result)
{
	fapi_ufst_server *r = If_to_I(server);
...
private
FAPI_retcode
can_replace_metrics(FAPI_server *server, FAPI_font *ff, FAPI_char_ref *c, int *result)
{
...
private
FAPI_retcode
get_char_width(FAPI_server *server, FAPI_font *ff, FAPI_char_ref *c, FAPI_metrics *metrics)
{
	fapi_ufst_server *r = If_to_I(server);
	UW16 buffer[2];
...
private
inline void
set_metrics(fapi_ufst_server *r, FAPI_metrics *metrics, SL32 design_bbox[4], SW16 design_escapement, int escapement, SW16 du_emx, SW16 du_emy)
{
...
private
FAPI_retcode
get_char(fapi_ufst_server *r, FAPI_font *ff, FAPI_char_ref *c, FAPI_path *p, FAPI_metrics *metrics, UW16 format)
{
	UW16 code;
	UW16 cc = (UW16)c->char_code;
	SL32 design_bbox[4];
...
private
FAPI_retcode
get_char_outline_metrics(FAPI_server *server, FAPI_font *ff, FAPI_char_ref *c, FAPI_metrics *metrics)
{
	fapi_ufst_server *r = If_to_I(server);
...
private
FAPI_retcode
get_char_raster_metrics(FAPI_server *server, FAPI_font *ff, FAPI_char_ref *c, FAPI_metrics *metrics)
{
	fapi_ufst_server *r = If_to_I(server);
	int code;
...
private
void gs_fapiufst_finit(i_plugin_instance *instance, i_plugin_client_memory *mem);

private
const i_plugin_descriptor ufst_descriptor = {
    "FAPI",
    "AgfaUFST",
    gs_fapiufst_finit};

private
const FAPI_server If0 = {
    {&ufst_descriptor},
    16, /* frac_shift */
    ensure_open,
    get_scaled_font,
...
int
gs_fapiufst_instantiate(i_ctx_t *i_ctx_p, i_plugin_client_memory *client_mem, i_plugin_instance **p_instance)
{
