...
/* Copyright (C) 1994, 1995, 1996, 1997, 1998 Aladdin Enterprises.  All rights
  reserved.
 ...
 */
#define dda_init_state(dstate, init, N) (dstate).Q = (init), (dstate).R = (N)
	...
	(tostep).dQ +=                                                         \
	    ((tostep).dR < (fromstep).NdR                                      \
	         ? ((tostep).dR += (fromstep).dR,                              \
	            (tostep).NdR -= (fromstep).dR, (fromstep).dQ)              \
	         : ((tostep).dR -= (fromstep).NdR,                             \
	            (tostep).NdR += (fromstep).NdR, (fromstep).dQ + 1))
/*
...
#define dda_state_advance(dstate, dstep, nsteps)                               \
	...
	while(n_--)                                                            \
		if((dstate).R > (dstep).dR)                                    \
			(dstate).R -= (dstep).dR;                              \
		else                                                           \
			(dstate).R += (dstep).NdR, (dstate).Q++;               \
	END
 ...
 */
#define dda_state_translate(dstate, delta) ((dstate).Q += (delta))
#define dda_translate(dda, delta) dda_state_translate((dda).state, delta)
