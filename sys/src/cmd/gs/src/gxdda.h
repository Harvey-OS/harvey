...
#define dda_step_add(tostep, fromstep)                                                                      \
	(tostep).dQ +=                                                                                      \
	    ((tostep).dR < (fromstep).NdR ? ((tostep).dR += (fromstep).dR, (tostep).NdR -= (fromstep).dR,   \
					     (fromstep).dQ)                                                 \
...
#define dda_state_next(dstate, dstep) \
	(dstate).Q +=                 \
	    ((dstate).R > (dstep).dR ? ((dstate).R -= (dstep).dR, (dstep).dQ) : ((dstate).R += (dstep).NdR, (dstep).dQ + 1))
...
#define dda_state_previous(dstate, dstep) \
	(dstate).Q -=                     \
	    ((dstate).R <= (dstep).NdR ? ((dstate).R += (dstep).dR, (dstep).dQ) : ((dstate).R -= (dstep).NdR, (dstep).dQ + 1))
