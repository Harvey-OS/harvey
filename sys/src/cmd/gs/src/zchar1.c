...
private
int
type1_exec_init(gs_type1_state* pcis, gs_text_enum_t* penum, gs_state* pgs,
                gs_font_type1* pfont1)
{
	...
	if(color_is_pure(
	       pgs->dev_color)) /* Keep consistency with alpha_buffer_bits() */
		alpha_bits = (*dev_proc(pgs->device, get_alpha_bits))(
		    pgs->device, go_text);
	if(alpha_bits <= 1) {
		/* We render to cache device or the target device has no alpha
		 * bits. */
	...
	return gs_type1_interp_init(
	    pcis, (gs_imager_state*)pgs, pgs->path, &penum->log2_scale,
	    &log2_subpixels,
...
/* Forward references */
private
int bbox_continue(i_ctx_t*);
private
int nobbox_continue(i_ctx_t*);
private
int type1_push_OtherSubr(i_ctx_t*, const gs_type1exec_state*, int (*)(i_ctx_t*),
                         const ref*);
private
int type1_call_OtherSubr(i_ctx_t*, const gs_type1exec_state*, int (*)(i_ctx_t*),
                         const ref*);
private
int type1_callout_dispatch(i_ctx_t*, int (*)(i_ctx_t*), int);
private
int type1_continue_dispatch(i_ctx_t*, gs_type1exec_state*, const ref*, ref*,
                            int);
private
int op_type1_cleanup(i_ctx_t*);
private
void op_type1_free(i_ctx_t*);
private
int bbox_getsbw_continue(i_ctx_t*);
private
int type1exec_bbox(i_ctx_t*, gs_type1exec_state*, gs_font*,
                   op_proc_t* exec_cont);
private
int bbox_finish_fill(i_ctx_t*);
private
int bbox_finish_stroke(i_ctx_t*);
private
int bbox_fill(i_ctx_t*);
private
int bbox_stroke(i_ctx_t*);
private
int nobbox_finish(i_ctx_t*, gs_type1exec_state*);
private
int nobbox_draw(i_ctx_t*, int (*)(gs_state*));
private
int nobbox_fill(i_ctx_t*);
private
int nobbox_stroke(i_ctx_t*);
...
charstring_execchar(i_ctx_t* i_ctx_p, int font_type_mask)
{
	...
	icont:
		code = type1_continue_dispatch(i_ctx_p, &cxs, opstr,
		                               &other_subr, 4);
		...
		case type1_result_callothersubr: /* unknown OtherSubr */
			return type1_call_OtherSubr(
			    i_ctx_p, &cxs, nobbox_continue, &other_subr);
...
/* Returns exec_cont - a function, which must be called by caller after this
 * function. */
private
int
type1exec_bbox(i_ctx_t* i_ctx_p, gs_type1exec_state* pcxs, gs_font* pfont,
               op_proc_t* exec_cont)
{
	os_ptr op = osp;
	...
	gs_font_base* const pbfont = (gs_font_base*)pfont;
	op_proc_t cont =
	    (pbfont->PaintType == 0 ? bbox_finish_fill : bbox_finish_stroke);
...
bbox_getsbw_continue(i_ctx_t* i_ctx_p)
{
	...
	case type1_result_sbw: { /* [h]sbw, done */
		...
		gs_rect bbox;
		op_proc_t cont = (pbfont->PaintType == 0 ? bbox_finish_fill
		                                         : bbox_finish_stroke),
		          exec_cont = 0;
		...
		bbox = pcxs->char_bbox;
		op_type1_free(i_ctx_p);
		code = zchar_set_cache(i_ctx_p, pbfont, op - 1, sbw, sbw + 2,
		                       &bbox, cont, &exec_cont, NULL);
		if(code >= 0 && exec_cont != 0)
			code = (*exec_cont)(i_ctx_p);
		return code;
	}
...
private
int bbox_finish(i_ctx_t* i_ctx_p, op_proc_t cont, op_proc_t* exec_cont);
private
int
                        ...
                        const ref* pcref, ref* pos, int num_args)
{
	...
	case type1_result_callothersubr: {
		...
		code = array_get(imemory, &pfdata->u.type1.OtherSubrs,
		                 (long)value, pos);
		if(code >= 0)
                     ...
                     int (*cont)(i_ctx_t*), const ref* pos)
{
	...
	gs_type1exec_state* hpcxs = ialloc_struct(
	    gs_type1exec_state, &st_gs_type1exec_state, "type1_call_OtherSubr");
...
private
int
type1_callout_dispatch(i_ctx_t* i_ctx_p, int (*cont)(i_ctx_t*), int num_args)
{
	ref other_subr;
	gs_type1exec_state* pcxs = r_ptr(esp, gs_type1exec_state);
	int code;
...
icont:
	code =
	    type1_continue_dispatch(i_ctx_p, pcxs, NULL, &other_subr, num_args);
	switch(code) {
...
nobbox_finish(i_ctx_t* i_ctx_p, gs_type1exec_state* pcxs)
{
		...
		return_error(e_undefined);
	{
		 ...
		 */
		if((*dev_proc(igs->device, get_alpha_bits))(igs->device,
		                                            go_text) > 1) {
			gs_newpath(igs);
			...
			cont = (pbfont->PaintType == 0 ? nobbox_fill
			                               : nobbox_stroke),
			exec_cont = 0;
			code = zchar_set_cache(
			    i_ctx_p, pbfont, op - 1, NULL, pcxs->sbw + 2,
			    &pcxs->char_bbox, cont, &exec_cont,
...
zsetweightvector(i_ctx_t* i_ctx_p)
{
	...
	if(code < 0) {
		/* The font was not defined yet. Just ignore. See
		 * lib/gs_type1.ps . */
		pop(2);
		return 0;
	}
	...
	if(size != pfont1->data.WeightVector.count)
		return_error(e_invalidfont);
	code = process_float_array(imemory, op, size,
	                           pfont1->data.WeightVector.values);
	if(code < 0)
		return code;
	pop(2);
	return 0;
...
private
int
z1_subr_data(gs_font_type1* pfont, int index, bool global, gs_glyph_data_t* pgd)
{
             ...
             gs_const_string* gstr, gs_glyph_data_t* pgd)
{
	...
	if((code = gs_c_glyph_name(glyph, gstr)) < 0 ||
	   (code = name_ref(pfont->memory, gstr->data, gstr->size, &rglyph,
	                    0)) < 0)
		return code;
	if(pglyph)
...
const gs_type1_data_procs_t z1_data_procs = {z1_glyph_data, z1_subr_data,
                                             z1_seac_data, z1_push, z1_pop};
 ...
 */
int
zchar1_glyph_outline(gs_font* font, int WMode, gs_glyph glyph,
...
zcharstring_outline(gs_font_type1* pfont1, int WMode, const ref* pgref,
                    const gs_glyph_data_t* pgd_orig, const gs_matrix* pmat,
...
z1_glyph_info_generic(gs_font* font, gs_glyph glyph, const gs_matrix* pmat,
                      ...
                      font_proc_glyph_info((*proc)), int wmode)
{
	ref gref;
	ref* pcdevproc;
	...
	int default_members =
	    members &
	    ~(width_members + outline_widths + GLYPH_INFO_VVECTOR0 +
	      GLYPH_INFO_VVECTOR1 + GLYPH_INFO_CDEVPROC);
	int done_members = 0;
	int code;
...
int
z1_glyph_info(gs_font* font, gs_glyph glyph, const gs_matrix* pmat, int members,
...
int
z1_set_cache(i_ctx_t* i_ctx_p, gs_font_base* pbfont, ref* cnref, gs_glyph glyph,
...
{ /* This function is similar to zchar42_set_cache. */
	...
	code = gs_default_glyph_info(
	    (gs_font*)pbfont, glyph, &id_matrix,
	    ((GLYPH_INFO_WIDTH0 | GLYPH_INFO_VVECTOR0) << wmode) |
	        GLYPH_INFO_BBOX,
	    &info);
	if(code < 0)
		return code;
	...
	return zchar_set_cache(i_ctx_p, pbfont, cnref, NULL, sbw + 2,
	                       &info.bbox, cont, exec_cont, wmode ? sbw : NULL);
}
