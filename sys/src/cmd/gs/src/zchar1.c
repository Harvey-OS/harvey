...
private
int type1_push_OtherSubr(i_ctx_t *, const gs_type1exec_state *,
			 int (*)(i_ctx_t *), const ref *);
private
int type1_call_OtherSubr(i_ctx_t *, const gs_type1exec_state *,
			 int (*)(i_ctx_t *), const ref *);
private
int type1_callout_dispatch(i_ctx_t *, int (*)(i_ctx_t *), int);
private
int type1_continue_dispatch(i_ctx_t *, gs_type1exec_state *,
			    const ref *, ref *, int);
private
...
private
int type1exec_bbox(i_ctx_t *, gs_type1exec_state *, gs_font *, op_proc_t *exec_cont);
private
	       ...
	       gs_font *pfont, op_proc_t *exec_cont)
{
	...
	if(pcxs->present == metricsNone) {
		...
		return zchar_set_cache(i_ctx_p, pbfont, op - 1,
				       (pcxs->present == metricsSideBearingAndWidth && !pcxs->use_FontBBox_as_Metrics2 ? pcxs->sbw : NULL),
				       pcxs->sbw + 2,
				       &pcxs->char_bbox,
				       cont, exec_cont,
	     ...
	     gs_glyph_data_t *pgd)
{
	...
	code = array_get(pfont->memory, (global ? &pfdata->u.type1.GlobalSubrs : &pfdata->u.type1.Subrs),
			 index, &subr);
	if(code < 0)
		return code;
		      ...
		      int members, gs_glyph_info_t *info, font_proc_glyph_info((*proc)), int wmode)
{
	...
	int default_members = members & ~(width_members + outline_widths + GLYPH_INFO_VVECTOR0 + GLYPH_INFO_VVECTOR1 + GLYPH_INFO_CDEVPROC);
	int done_members = 0;
	int code;
