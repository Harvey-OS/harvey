...
/*  Painting contract :
    ...
    The following functions paint immediately, without vd_beg_path, vd_end_path,
   vd_fill, vd_stroke :
    vd_circle, vd_round, vd_bar, vd_bar_w, vd_curve, vd_curve_w

    The following functions require vd_fill or vd_stroke only if enclosed with
   vd_beg_path, vd_end_path :
...
struct vd_trace_interface_s {
	...
	void (*curveto)(vd_trace_interface* I, double x0, double y0, double x1,
	                double y1, double x2, double y2);
	void (*closepath)(vd_trace_interface* I);
	void (*circle)(vd_trace_interface* I, double x, double y,
	               int r); /* Radius doesn't scale. */
	void (*round)(vd_trace_interface* I, double x, double y,
	              int r); /* Radius doesn't scale. */
	void (*fill)(vd_trace_interface* I);
	void (*stroke)(vd_trace_interface* I);
	void (*setcolor)(vd_trace_interface* I, unsigned long rgbcolor);
	void (*setlinewidth)(vd_trace_interface* I,
	                     unsigned int width); /* Width doesn't scale. */
	void (*text)(vd_trace_interface* I, double x, double y,
	             char* ASCIIZ); /* Font doesn't scale. */
...
extern vd_trace_interface*
    vd_trace1; /* A copy of vd_trace0, or NULL if trace is disabled. */
...
void vd_impl_curveto(double x0, double y0, double x1, double y1, double x2,
                     double y2);
void vd_impl_bar(double x0, double y0, double x1, double y1, int w,
                 unsigned long c); /* unscaled width */
void vd_impl_square(double x0, double y0, int w,
                    unsigned int c); /* unscaled width */
void vd_impl_rect(double x0, double y0, double x1, double y1, int w,
                  unsigned int c); /* unscaled width */
void vd_impl_quad(double x0, double y0, double x1, double y1, double x2,
                  double y2, double x3, double y3, int w,
                  unsigned int c); /* unscaled width */
void vd_impl_curve(double x0, double y0, double x1, double y1, double x2,
                   double y2, double x3, double y3, int w,
                   unsigned long c); /* unscaled width */
void vd_impl_circle(double x, double y, int r,
                    unsigned long c); /* unscaled radius */
void vd_impl_round(double x, double y, int r,
                   unsigned long c); /* unscaled radius */
void vd_impl_text(double x, double y, char* s,
                  unsigned long c); /* unscaled font */
void vd_setflag(char f, char v);

#ifndef RGB
#define RGB(r, g, b)                                                           \
	((unsigned long)(r)*65536L + (unsigned long)(g)*256L +                 \
	 (unsigned long)(b))
#endif
...
#define vd_get_dc(f)                                                           \
	while(vd_trace0 && vd_allowed(f)) {                                    \
		vd_trace0->get_dc(vd_trace0, &vd_trace1);                      \
		break;                                                         \
	}
#define vd_release_dc                                                          \
	BEGIN if(vd_trace1) vd_trace1->release_dc(vd_trace1, &vd_trace1);      \
	END
#define vd_enabled (vd_trace1 != 0)
#define vd_get_size_unscaled_x                                                 \
	(vd_trace1 ? vd_trace1->get_size_x(vd_trace1) : 100)
#define vd_get_size_unscaled_y                                                 \
	(vd_trace1 ? vd_trace1->get_size_y(vd_trace1) : 100)
#define vd_get_size_scaled_x                                                   \
	(vd_trace1 ? vd_trace1->get_size_x(vd_trace1) / vd_trace1->scale_x     \
	           : 100)
#define vd_get_size_scaled_y                                                   \
	(vd_trace1 ? vd_trace1->get_size_y(vd_trace1) / vd_trace1->scale_y     \
	           : 100)
...
#define vd_set_scale(s)                                                        \
	BEGIN if(vd_trace1)                                                    \
	{                                                                      \
		vd_trace1->scale_x = vd_trace1->scale_y = s;                   \
		vd_trace1->set_scale(vd_trace1);                               \
	}                                                                      \
	END
#define vd_set_scaleXY(sx, sy)                                                 \
	BEGIN if(vd_trace1)                                                    \
	{                                                                      \
		vd_trace1->scale_x = sx, vd_trace1->scale_y = sy;              \
		vd_trace1->set_scale(vd_trace1);                               \
	}                                                                      \
	END
#define vd_set_origin(x, y)                                                    \
	BEGIN if(vd_trace1)                                                    \
	{                                                                      \
		vd_trace1->orig_x = x, vd_trace1->orig_y = y;                  \
		vd_trace1->set_origin(vd_trace1);                              \
	}                                                                      \
	END
#define vd_set_shift(x, y)                                                     \
	BEGIN if(vd_trace1)                                                    \
	{                                                                      \
		vd_trace1->shift_x = x, vd_trace1->shift_y = y;                \
		vd_trace1->set_shift(vd_trace1);                               \
	}                                                                      \
	END
#define vd_set_central_shift                                                   \
	BEGIN if(vd_trace1)                                                    \
	    vd_trace1->shift_x = vd_trace1->get_size_x(vd_trace1) / 2,         \
	    vd_trace1->shift_y = vd_trace1->get_size_y(vd_trace1) / 2;         \
	END
#define vd_erase(c)                                                            \
	BEGIN if(vd_trace1) vd_trace1->erase(vd_trace1, c);                    \
	END
#define vd_beg_path                                                            \
	BEGIN if(vd_trace1) vd_trace1->beg_path(vd_trace1);                    \
	END
#define vd_end_path                                                            \
	BEGIN if(vd_trace1) vd_trace1->end_path(vd_trace1);                    \
	END
#define vd_moveto(x, y)                                                        \
	BEGIN if(vd_trace1) vd_impl_moveto(x, y);                              \
	END
#define vd_lineto(x, y)                                                        \
	BEGIN if(vd_trace1) vd_impl_lineto(x, y);                              \
	END
#define vd_lineto_multi(p, n)                                                  \
	BEGIN if(vd_trace1) vd_impl_lineto_multi(p, n);                        \
	END
#define vd_curveto(x0, y0, x1, y1, x2, y2)                                     \
	BEGIN if(vd_trace1) vd_impl_curveto(x0, y0, x1, y1, x2, y2);           \
	END
#define vd_closepath                                                           \
	BEGIN if(vd_trace1) vd_trace1->closepath(vd_trace1);                   \
	END
#define vd_bar(x0, y0, x1, y1, w, c)                                           \
	BEGIN if(vd_trace1) vd_impl_bar(x0, y0, x1, y1, w, c);                 \
	END
#define vd_square(x0, y0, w, c)                                                \
	BEGIN if(vd_trace1) vd_impl_square(x0, y0, w, c);                      \
	END
#define vd_rect(x0, y0, x1, y1, w, c)                                          \
	BEGIN if(vd_trace1) vd_impl_rect(x0, y0, x1, y1, w, c);                \
	END
#define vd_quad(x0, y0, x1, y1, x2, y2, x3, y3, w, c)                          \
	BEGIN if(vd_trace1)                                                    \
	    vd_impl_quad(x0, y0, x1, y1, x2, y2, x3, y3, w, c);                \
	END
#define vd_curve(x0, y0, x1, y1, x2, y2, x3, y3, w, c)                         \
	BEGIN if(vd_trace1)                                                    \
	    vd_impl_curve(x0, y0, x1, y1, x2, y2, x3, y3, w, c);               \
	END
#define vd_circle(x, y, r, c)                                                  \
	BEGIN if(vd_trace1) vd_impl_circle(x, y, r, c);                        \
	END
#define vd_round(x, y, r, c)                                                   \
	BEGIN if(vd_trace1) vd_impl_round(x, y, r, c);                         \
	END
#define vd_fill                                                                \
	BEGIN if(vd_trace1) vd_trace1->fill(vd_trace1);                        \
	END
#define vd_stroke                                                              \
	BEGIN if(vd_trace1) vd_trace1->stroke(vd_trace1);                      \
	END
#define vd_setcolor(c)                                                         \
	BEGIN if(vd_trace1) vd_trace1->setcolor(vd_trace1, c);                 \
	END
#define vd_setlinewidth(w)                                                     \
	BEGIN if(vd_trace1) vd_trace1->setlinewidth(vd_trace1, w);             \
	END
#define vd_text(x, y, s, c)                                                    \
	BEGIN if(vd_trace1) vd_impl_text(x, y, s, c);                          \
	END
#define vd_wait                                                                \
	BEGIN if(vd_trace1) vd_trace1->wait(vd_trace1);                        \
	END
#define vd_save                                                                \
	vd_trace_interface* vd_trace_save;                                     \
	vd_trace_save = vd_trace1; /* NOTE :                                   \
