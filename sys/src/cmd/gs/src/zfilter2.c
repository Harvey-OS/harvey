...
zCFE(i_ctx_t* i_ctx_p)
{
	...
	if(code < 0)
		return code;
	return filter_write(i_ctx_p, 0, &s_CFE_template, (stream_state*)&cfs,
	                    0);
}
                       ...
                       const stream_template* template, stream_state* st)
{
		...
		return filter_write(i_ctx_p, npop, template, st, 0);
	{
		...
		op = osp;
		code = (predictor == 2
		            ? filter_write(i_ctx_p, 0, &s_PDiffE_template,
		                           (stream_state*)&pds, 0)
		            : filter_write(i_ctx_p, 0, &s_PNGPE_template,
		                           (stream_state*)&pps, 0));
		if(code < 0) {
...
const op_def zfilter2_op_defs[] = {op_def_begin_filter(),
