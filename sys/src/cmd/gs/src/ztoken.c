...
private
int ztoken_continue(i_ctx_t*);
private
int token_continue(i_ctx_t*, stream*, scanner_state*, bool);
int
ztoken(i_ctx_t* i_ctx_p)
{
	...
	case t_string: {
		...
		default:
			if(code < 0) {
				/* Clear anything that may have been left on the
				 * ostack */
				if(orig_ostack_depth <
				   ref_stack_count(&o_stack))
					pop(ref_stack_count(&o_stack) -
					    orig_ostack_depth);
				return code;
			}
		}
...
private
int
token_continue(i_ctx_t* i_ctx_p, stream* s, scanner_state* pstate, bool save)
{
	os_ptr op = osp;
	int code;
	ref token;
...
private
int tokenexec_continue(i_ctx_t*, stream*, scanner_state*, bool);
int
...
again:
	...
	case scan_DSC_Comment:
		return ztoken_handle_comment(i_ctx_p, &fref, pstate, esp + 1,
		                             code, save, true,
		                             ztokenexec_continue);
	default: /* error */
		break;
...
ztoken_handle_comment(i_ctx_t* i_ctx_p, const ref* fop, scanner_state* sstate,
                      const ref* ptoken, int scan_code, bool save,
                      bool push_file, op_proc_t cont)
{
	...
	if(ppcproc == 0) {
		 ...
		 * Pop the comment string from the o-stack if needed (see
		 * below).
		 */
		if(pstate->s_pstack)
			--osp;
		 ...
		 * If we were inside { }, the comment string is already on the
		 * stack.
		 */
		if(pstate->s_pstack) {
			op = ++osp;
...
const op_def ztoken_op_defs[] = {{"1token", ztoken},
