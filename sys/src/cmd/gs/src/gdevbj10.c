...
/* Copyright (C) 1990, 1995, 1997, 2000 Aladdin Enterprises.  All rights
  reserved.
...
const gx_device_printer far_data gs_bj200_device =
    prn_device(prn_bj200_procs, "bj200", DEFAULT_WIDTH_10THS,
               DEFAULT_HEIGHT_10THS, 360, /* x_dpi */
               360,                       /* y_dpi */
...
const gx_device_printer far_data gs_bj10e_device =
    prn_device(prn_bj10e_procs, "bj10e", DEFAULT_WIDTH_10THS,
               DEFAULT_HEIGHT_10THS, 360, /* x_dpi */
               360,                       /* y_dpi */
               0, 0, 0, 0,                /* margins */
               1, bj10e_print_page);
	...
	while(lnum < last_row) {
		...
		do {
			...
			if(outl > out_beg) {
				...
				if(xres == 180)
					count <<= 1;
				fprintf(prn_stream, "\033d%c%c", count & 0xff,
				        count >> 8);
			}
			...
			fprintf(prn_stream, "\033[g%c%c%c", count & 0xff,
			        count >> 8, mode);
