...
/* Copyright (C) 1994, 1995, 1997, 1998, 1999 Aladdin Enterprises.  All rights
  reserved.
...
dctd_fill_input_buffer(j_decompress_ptr dinfo)
{
	...
	jddp->faked_eoi =
	    true; /* so process routine doesn't use next_input_byte */
	return TRUE;
}
private
void
               ...
               stream_cursor_write* pw, bool last)
{
	...
	case 2: /* start_decompress */
		...
		if_debug4('w', "[wdd]width=%u, components=%d, "
		               "scan_line_size=%u, min_out_size=%u\n",
		          jddp->dinfo.output_width,
		          jddp->dinfo.output_components, ss->scan_line_size,
		          jddp->template.min_out_size);
		if(ss->scan_line_size > (uint)jddp->template.min_out_size) {
			/* Create a spare buffer for oversize scanline */
			jddp->scanline_buffer = gs_alloc_bytes_immovable(
			    jddp->memory, ss->scan_line_size,
	...
	dumpbuffer:
		if(jddp->bytes_in_scanline != 0) {
			...
			uint tomove = min(jddp->bytes_in_scanline, avail);

			if_debug2('w', "[wdd]moving %u/%u\n", tomove, avail);
			memcpy(pw->ptr + 1, jddp->scanline_buffer +
			                        (ss->scan_line_size -
			                         jddp->bytes_in_scanline),
			       tomove);
			pw->ptr += tomove;
		...
		while(jddp->dinfo.output_height > jddp->dinfo.output_scanline) {
			...
			if_debug3(
			    'w',
			    "[wdd]read returns %d, used=%u, faked_eoi=%d\n",
			    read, (uint)(src->next_input_byte - 1 - pr->ptr),
			    (int)jddp->faked_eoi);
			pr->ptr = (jddp->faked_eoi ? pr->limit
...
const stream_template s_DCTD_template = {
    &st_DCT_state, s_DCTD_init,    s_DCTD_process,     2000,
    4000,          s_DCTD_release, s_DCTD_set_defaults};
