...
{
	return (pcw->options & WRITE_TYPE2_NO_LENIV ? max(pfont->data.lenIV, 0) : 0);
}
private
bool
...
{
	return (count == 0 ? 2 : 3 + offset_size(total + 1) * (count + 1) + total);
}
private
void
			...
			bool full_info, gs_font_info_t *pinfo)
{
	...
	return pbfont->procs.font_info((gs_font *)pbfont, NULL,
				       (full_info ? FONT_INFO_FLAGS | FONT_INFO_ITALIC_ANGLE | FONT_INFO_UNDERLINE_POSITION | FONT_INFO_UNDERLINE_THICKNESS : 0) |
		   ...
		   uint Private_offset, uint Private_size)
{
	...
	{
		int type = (pcw->options & WRITE_TYPE2_CHARSTRINGS ? 2 : pbfont->FontType == ft_encrypted2 ? 2 : 1);
...
/* Compute the size of the (local) Subrs Index. */
	...
	subrs_size =
	    (cff_convert_charstrings(&writer, pbfont) ? 0 : cff_write_Subrs_offsets(&writer, &subrs_count, pfont, false));
#else
...
#ifdef SKIP_EMPTY_SUBRS
		subrs_size[j] =
		    (cff_convert_charstrings(&writer, (gs_font_base *)pfd) ? 0 : cff_write_Subrs_offsets(&writer, &subrs_count[j], pfd, false));
#else
