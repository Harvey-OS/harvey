...
cff_string_sid(cff_writer_t* pcw, const byte* data, uint size)
{
	int index;
	int code =
	    cff_string_index(&pcw->std_strings, data, size, false, &index);

	if(code < 0) {
		code =
		    cff_string_index(&pcw->strings, data, size, true, &index);
		if(code < 0)
			return code;
...
{
	return (pcw->options & WRITE_TYPE2_NO_LENIV ? max(pfont->data.lenIV, 0)
	                                            : 0);
}
private
bool
...
cff_put_Index(cff_writer_t* pcw, const cff_string_table_t* pcst)
{
	...
	for(j = 0; j < pcst->count; ++j)
		put_bytes(pcw->strm, pcst->items[j].key.data,
		          pcst->items[j].key.size);
}
...
private
int
cff_get_Top_info_common(cff_writer_t* pcw, gs_font_base* pbfont, bool full_info,
                        gs_font_info_t* pinfo)
{
                     ...
                     bool write_FontMatrix, const gs_font_info_t* pinfo)
{
	...
	if(pinfo->members & FONT_INFO_NOTICE)
		cff_put_string_value(pcw, pinfo->Notice.data,
		                     pinfo->Notice.size, TOP_Notice);
	if(pinfo->members & FONT_INFO_FULL_NAME)
		cff_put_string_value(pcw, pinfo->FullName.data,
		                     pinfo->FullName.size, TOP_FullName);
	...
	}
	{
		...
		if(write_FontMatrix || pbfont->FontMatrix.xx != fm_default.xx ||
...
private
void
cff_write_Top_font(cff_writer_t* pcw, uint Encoding_offset, uint charset_offset,
                   uint CharStrings_offset, uint Private_offset,
                   uint Private_size)
{
	...
	{
		int type = (pcw->options & WRITE_TYPE2_CHARSTRINGS
		                ? 2
...
#define PUT_FLOAT_TABLE(member, op)                                            \
	...
	if(pfont->data.StdHW.count > 0)
		cff_put_real_value(pcw, pfont->data.StdHW.values[0],
		                   PRIVATE_StdHW);
	if(pfont->data.StdVW.count > 0)
		cff_put_real_value(pcw, pfont->data.StdVW.values[0],
		                   PRIVATE_StdVW);
	if(Subrs_offset)
	...
	if(pfont->FontType != ft_encrypted) {
		if(pfont->data.defaultWidthX != defaultWidthX_DEFAULT)
			cff_put_real_value(
			    pcw, fixed2float(pfont->data.defaultWidthX),
			    PRIVATE_defaultWidthX);
		if(pfont->data.nominalWidthX != nominalWidthX_DEFAULT)
			cff_put_real_value(
			    pcw, fixed2float(pfont->data.nominalWidthX),
                              ...
                              uint* pcount)
{
	...
	for(glyph = gs_no_glyph, count = 0, offset = 1;
	    (code = psf_enumerate_glyphs_next(penum, &glyph)) != 1; ++count) {
		gs_glyph_data_t gdata;
		gs_font_type1* pfd;
		int gcode;
		...
		if(code == 0 &&
		   (gcode = pcw->glyph_data(pfont, glyph, &gdata, &pfd)) >= 0) {
			...
			if(gdata.bits.size >=
			   (extra_lenIV = cff_extra_lenIV(pcw, pfd))) {
				if(cff_convert_charstrings(
				       pcw, (gs_font_base*)pfd)) {
					swrite_position_only(&poss);
					code = psf_convert_type1_to_type2(
					    &poss, &gdata, pfd);
					if(code < 0)
						return code;
					offset += stell(&poss);
	...
	for(glyph = gs_no_glyph;
		...
		if(code == 0 &&
		   (code = pcw->glyph_data(pfont, glyph, &gdata, &pfd)) >= 0) {
			cff_put_CharString(pcw, gdata.bits.data,
			                   gdata.bits.size, pfd);
...
cff_Encoding_size(int num_encoded, int num_encoded_chars)
{
	...
	return 2 + n +
	       (num_encoded_chars > n ? 1 + (num_encoded_chars - n) * 3 : 0);
}
...
cff_write_Encoding(cff_writer_t* pcw, cff_glyph_subset_t* pgsub)
{
	...
	for(j = 0; j < 256; ++j) {
		gs_glyph glyph = pfont->procs.encode_char(
		    (gs_font*)pfont, (gs_char)j, GLYPH_SPACE_NAME);
		int i;
	...
	if(nsupp) {
		...
		for(j = 0; j < nsupp; ++j) {
			...
			sputc(s, chr);
			put_card16(pcw,
			           cff_glyph_sid(pcw, pfont->procs.encode_char(
			                                  (gs_font*)pfont,
			                                  (gs_char)chr,
			                                  GLYPH_SPACE_NAME)));
		}
...
cff_write_charset(cff_writer_t* pcw, cff_glyph_subset_t* pgsub)
{
	...
	for(j = 1; j < pgsub->glyphs.subset_size; ++j)
		put_card16(pcw,
		           cff_glyph_sid(pcw, pgsub->glyphs.subset_data[j]));
	return 0;
}
private
int
...
cff_FDSelect_size(cff_writer_t* pcw, psf_glyph_enum_t* penum, uint* pformat)
{
	...
	while((code = psf_enumerate_glyphs_next(penum, &glyph)) == 0) {
		...
		code =
		    pfont->cidata.glyph_data(pbfont, glyph, NULL, &font_index);
		if(code >= 0) {
			if(font_index != prev)
                   ...
                   int format)
{
	...
	case 3: /* ranges */
		...
		while((code = psf_enumerate_glyphs_next(penum, &glyph)) == 0) {
			...
			code = pfont->cidata.glyph_data(pbfont, glyph, NULL,
			                                &font_index);
			if(code >= 0) {
				if(font_index != prev) {
                     ...
                     gs_int_rect* FontBBox)
{
	 ...
	 * (see below).
	 */
	uint Top_size = 0x7fffff, GSubrs_offset, Encoding_offset,
	     charset_offset, CharStrings_offset, Private_offset,
	     Private_size = 0x7fffff, Subrs_offset, End_offset = 0x7fffff;
	int j;
	...
	while((code = psf_enumerate_glyphs_next(&genum, &glyph)) != 1)
		number_of_glyphs++;
	subset.glyphs.subset_data = (gs_glyph*)gs_alloc_bytes(
	    pfont->memory, number_of_glyphs * sizeof(glyph),
	    "psf_write_type2_font");
	number_of_strings = number_of_glyphs + MAX_CFF_MISC_STRINGS;
	std_string_items = (cff_string_item_t*)gs_alloc_bytes(
	    pfont->memory, (MAX_CFF_STD_STRINGS + number_of_strings) *
	                       sizeof(cff_string_item_t),
	    "psf_write_type2_font");
	...
	if(options & WRITE_TYPE2_CHARSTRINGS) {
		...
		if(pfont->FontType != ft_encrypted2)
			pfont->data.defaultWidthX = pfont->data.nominalWidthX =
			    0;
	}
	...
	psf_enumerate_glyphs_begin(
	    &genum, (gs_font*)pfont, subset.glyphs.subset_glyphs,
	    (subset.glyphs.subset_glyphs ? subset.glyphs.subset_size : 0),
	    GLYPH_SPACE_NAME);
	...
	/* Shuffle the glyphs into the order .notdef, encoded, unencoded. */
	{
		...
		if(!subset.glyphs.subset_glyphs) {
			...
			while((code = psf_enumerate_glyphs_next(&genum,
			                                        &glyph)) != 1)
				if(code == 0) {
					if(num_glyphs == number_of_glyphs)
						return_error(
						    gs_error_limitcheck);
					subset.glyphs
					    .subset_data[num_glyphs++] = glyph;
				}
			subset.glyphs.subset_size = psf_sort_glyphs(
			    subset.glyphs.subset_data, num_glyphs);
			subset.glyphs.subset_glyphs = subset.glyphs.subset_data;
		}

		/* Move the unencoded glyphs to the top of the list. */
		/*
		 * We could do this in time N rather than N log N with a
		 * two-finger
		 ...
		 */
		{
			...
			while(from > 0) {
				...
				if(glyph != subset.glyphs.notdef &&
				   !psf_sorted_glyphs_include(encoded, num_enc,
				                              glyph))
		...
		/* Move .notdef and the encoded glyphs to the bottom of the
		 * list. */
	...
	cff_string_table_init(&writer.std_strings, std_string_items,
	                      MAX_CFF_STD_STRINGS);
	for(j = 0; (glyph = gs_c_known_encode(
	                (gs_char)j, ENCODING_INDEX_CFFSTRINGS)) != gs_no_glyph;
	    ++j) {
		gs_const_string str;
		int ignore;
	...
	psf_enumerate_glyphs_begin(&genum, (gs_font*)pfont,
	                           ...
	                           subset.glyphs.subset_size, GLYPH_SPACE_NAME);
	while((code = psf_enumerate_glyphs_next(&genum, &glyph)) != 1)
		if(code == 0) {
	 ...
	 */
	encoding_size =
	    cff_Encoding_size(subset.num_encoded, subset.num_encoded_chars);
...
/* Compute the size of the (local) Subrs Index. */
	...
	subrs_size = (cff_convert_charstrings(&writer, pbfont)
	                  ? 0
...
iter:
	...
	/* Compute the offsets. */
	GSubrs_offset =
	    4 + cff_Index_size(1, font_name.size) +
	    cff_Index_size(1, Top_size) +
	    cff_Index_size(writer.strings.count, writer.strings.total);
	Encoding_offset =
	    GSubrs_offset + cff_Index_size(gsubrs_count, gsubrs_size);
...
write:
	...
	cff_write_Top_font(&writer, Encoding_offset, charset_offset,
	                   CharStrings_offset, Private_offset, Private_size);
	...
	if(gsubrs_count == 0 || cff_convert_charstrings(&writer, pbfont))
		...
		cff_write_Subrs(&writer, gsubrs_count, gsubrs_size, pfont,
		                true);
	...
	gs_free_object(pfont->memory, subset.glyphs.subset_data,
	               "psf_write_type2_font");
	return 0;
}
                    ...
                    const gs_const_string* alt_font_name)
{
	...
	cff_string_item_t
	    string_items[500 /* character names */ + 40 /* misc. values */];
	gs_const_string font_name;
	stream poss;
	uint charstrings_count, charstrings_size;
	 ...
	 * (see below).
	 */
	uint Top_size = 0x7fffff, GSubrs_offset = 0x7fffff,
	     charset_offset = 0x7fffff, FDSelect_offset = 0x7fffff,
	     CharStrings_offset = 0x7fffff, Font_offset = 0x7fffff,
	     FDArray_offsets[257], Private_offsets[257], Subrs_offsets[257],
	     End_offset = 0x7fffff;
	int j;
	psf_glyph_enum_t genum;
	gs_font_info_t info;
	 ...
	 */
	if((options & WRITE_TYPE2_NO_GSUBRS) != 0 ||
	   cff_convert_charstrings(
	       &writer, (const gs_font_base*)pfont->cidata.FDArray[0])
	   /* we expand all Subrs */
	   )
		gsubrs_count = 0, gsubrs_size = 0;
	else
		gsubrs_size = cff_write_Subrs_offsets(
		    &writer, &gsubrs_count, pfont->cidata.FDArray[0], true);
...
#ifdef SKIP_EMPTY_SUBRS
		subrs_size[j] =
		    (cff_convert_charstrings(&writer, (gs_font_base*)pfd)
		         ? 0
		         : cff_write_Subrs_offsets(&writer, &subrs_count[j],
		                                   pfd, false));
#else
		if(cff_convert_charstrings(&writer, (gs_font_base*)pfd))
			subrs_count[j] = 0; /* we expand all Subrs */
		subrs_size[j] = cff_write_Subrs_offsets(
		    &writer, &subrs_count[j], pfd, false);
#endif
	}
...
iter:
	...
	if_debug4('l', "[l]GSubrs at %u, charset at %u, FDSelect at %u, "
	               "CharStrings at %u\n",
	          GSubrs_offset, charset_offset, FDSelect_offset,
	          CharStrings_offset);
...
write:
	...
	for(j = 0; j < num_fonts; ++j) {
		...
		if(options & WRITE_TYPE2_CHARSTRINGS) {
			...
			if(pfd->FontType != ft_encrypted2)
				pfd->data.defaultWidthX =
				    pfd->data.nominalWidthX = 0;
		}
		cff_write_Top_fdarray(
		    &writer, (gs_font_base*)pfd, Private_offsets[j],
	...
	for(j = 0;; ++j) {
		...
		cff_write_Private(
		    &writer, (subrs_size[j] == 0 ? 0 : Subrs_offsets[j]), pfd);
	}
	...
	for(j = 0;; ++j) {
		...
		if(cff_convert_charstrings(&writer, (gs_font_base*)pfd))
			...
			cff_write_Subrs(&writer, subrs_count[j], subrs_size[j],
			                pfd, false);
	}
