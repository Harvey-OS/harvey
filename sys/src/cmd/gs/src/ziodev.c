...
/* Copyright (C) 1993, 1995, 1997, 1998, 1999, 2001 Aladdin Enterprises.  All
  rights reserved.
...
#define iodev_special(dname, init, open)                                       \
	{                                                                      \
		...
		{                                                              \
			init, open, iodev_no_open_file, iodev_no_fopen,        \
			    iodev_no_fclose, iodev_no_delete_file,             \
			    iodev_no_rename_file, iodev_no_file_status,        \
...
zfilelineedit(i_ctx_t* i_ctx_p)
{
	...
	/* extend string */
	initial_buf_size =
	    statement ? STATEMENTEDIT_BUF_SIZE : LINEEDIT_BUF_SIZE;
...
rd:
	...
	op->value.bytes =
	    buf->data; /* zreadline_from sometimes resizes the buffer. */
	...
	case CALLC: {
		...
		code = s_handle_read_exception(i_ctx_p, code, &rfile, NULL, 0,
		                               zfilelineedit);
	} break;
		...
		if(count + 1 > buf->size) {
			...
			if(nsize > max_string_size) {
				...
				nbuf = gs_resize_string(
				    imemory, buf->data, buf->size, nsize,
				    "zfilelineedit(grow buffer)");
				if(nbuf == 0) {
					code = gs_note_error(e_VMerror);
					return_error(code);
	...
	filename =
	    statement ? gs_iodev_statementedit.dname : gs_iodev_lineedit.dname;
...
const op_def ziodev_op_defs[] = {{"1.getiodevice", zgetiodevice},
                                 op_def_end(0)};
