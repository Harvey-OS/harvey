...
zmakeglyph32(i_ctx_t* i_ctx_p)
{
	...
	if(long_form || metrics[0] != (wx = (int)metrics[0]) ||
	   ...
	   ((wx | width | height | (llx + 128) | (lly + 128)) & ~255) != 0) {
		...
		for(i = 0; i < n; ++i) {
			int v =
			    (int)metrics[i]; /* no floating point widths yet */
...
select_cid_range(const gs_memory_t* mem, cached_char* cc, void* range_ptr)
{
	...
	return (cc->code >= range->cid_min && cc->code <= range->cid_max &&
...
zgetmetrics32(i_ctx_t* i_ctx_p)
{
	...
	if(data[0]) {
		...
		push(2 + n);
		for(i = 0; i < n; ++i)
			make_int(op - n + i, ((int)((data[2 * i + 2] << 8) +
			                            data[2 * i + 3]) ^
			                      0x8000) -
			                         0x8000);
	}
...
const op_def zchar32_op_defs[] = {{"1.getmetrics32", zgetmetrics32},
