...
private
gs_memory_proc_alloc_bytes(gs_heap_alloc_bytes);
private
gs_memory_proc_resize_object(gs_heap_resize_object);
private
gs_memory_proc_free_object(gs_heap_free_object);
private
gs_memory_proc_stable(gs_heap_stable);
private
gs_memory_proc_status(gs_heap_status);
private
gs_memory_proc_free_all(gs_heap_free_all);

/* Object memory procedures */
private
gs_memory_proc_alloc_struct(gs_heap_alloc_struct);
private
gs_memory_proc_alloc_byte_array(gs_heap_alloc_byte_array);
private
gs_memory_proc_alloc_struct_array(gs_heap_alloc_struct_array);
private
gs_memory_proc_object_size(gs_heap_object_size);
private
gs_memory_proc_object_type(gs_heap_object_type);
private
gs_memory_proc_alloc_string(gs_heap_alloc_string);
private
gs_memory_proc_resize_string(gs_heap_resize_string);
private
gs_memory_proc_free_string(gs_heap_free_string);
private
gs_memory_proc_register_root(gs_heap_register_root);
private
gs_memory_proc_unregister_root(gs_heap_unregister_root);
private
gs_memory_proc_enable_free(gs_heap_enable_free);
private
const gs_memory_procs_t gs_malloc_memory_procs = {
    /* Raw memory procedures */
    gs_heap_alloc_bytes, gs_heap_resize_object, gs_heap_free_object,
    gs_heap_stable, gs_heap_status, gs_heap_free_all,
    gs_ignore_consolidate_free,
    /* Object memory procedures */
    gs_heap_alloc_bytes, gs_heap_alloc_struct, gs_heap_alloc_struct,
    gs_heap_alloc_byte_array, gs_heap_alloc_byte_array,
    gs_heap_alloc_struct_array, gs_heap_alloc_struct_array, gs_heap_object_size,
    gs_heap_object_type, gs_heap_alloc_string, gs_heap_alloc_string,
    gs_heap_resize_string, gs_heap_free_string, gs_heap_register_root,
    gs_heap_unregister_root, gs_heap_enable_free};
...
#define _npad                                                                  \
	(-size_of(struct malloc_block_data_s) & (arch_align_memory_mod - 1))
...
#define set_msg(str) DO_NOTHING
	...
	if(size > mmem->limit - sizeof(gs_malloc_block_t)) {
		...
		if(mmem->limit - added < mmem->used)
			...
			/*
			 * We would like to check that malloc aligns blocks at
			 * least as
			 * strictly as the compiler (as defined by
			 * arch_align_memory_mod).
			 * However, Microsoft VC 6 does not satisfy this
			 * requirement.
...
{
	void* ptr = gs_heap_alloc_byte_array(
	    mem, num_elements, gs_struct_type_size(pstype), cname);
                      ...
                      client_name_t cname)
{
	...
	mmem->used += new_size;
	if(new_size > old_size)
		gs_alloc_fill((byte*)new_ptr + old_size, gs_alloc_fill_alloc,
		              new_size - old_size);
	return new_ptr + 1;
}
private
...
gs_heap_free_object(gs_memory_t* mem, void* ptr, client_name_t cname)
{
	...
	if(finalize != 0) {
		if_debug3('u', "[u]finalizing %s 0x%lx (%s)\n",
		          struct_type_name_string(pstype), (ulong)ptr,
		          client_name_string(cname));
		(*finalize)(ptr);
	}
	if(ptr == bp + 1) {
...
gs_malloc_wrap(gs_memory_t** wrapped, gs_malloc_memory_t* contents)
{
	...
	gs_memory_locked_t* lmem =
	    (gs_memory_locked_t*)gs_alloc_bytes_immovable(
	        cmem, sizeof(gs_memory_locked_t), "gs_malloc_wrap(locked)");
	...
	rmem = (gs_memory_retrying_t*)gs_alloc_bytes_immovable(
	    (gs_memory_t*)lmem, sizeof(gs_memory_retrying_t),
	...
	if(code < 0) {
		gs_free_object((gs_memory_t*)lmem, rmem,
		               "gs_malloc_wrap(retrying)");
		gs_memory_locked_release(lmem);
		gs_free_object(cmem, lmem, "gs_malloc_wrap(locked)");
		return code;
