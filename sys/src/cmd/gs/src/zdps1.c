...
/* Copyright (C) 1990, 1996, 1997, 1998, 1999, 2000 Aladdin Enterprises.  All
  rights reserved.
...
/* Forward references */
private
int rect_get(local_rects_t*, os_ptr, gs_memory_t*);
private
void rect_release(local_rects_t*, gs_memory_t*);
...
zrectstroke(i_ctx_t* i_ctx_p)
{
	...
	if(read_matrix(imemory, op, &mat) >= 0) {
		/* Concatenate the matrix to the CTM just before stroking the
		 * path. */
