...
private
dev_proc_open_device(psd_prn_open);
private
dev_proc_get_params(psd_get_params);
private
dev_proc_put_params(psd_put_params);
private
dev_proc_print_page(psd_print_page);
private
dev_proc_map_color_rgb(psd_map_color_rgb);
private
dev_proc_get_color_mapping_procs(get_psdrgb_color_mapping_procs);
private
dev_proc_get_color_mapping_procs(get_psd_color_mapping_procs);
private
dev_proc_get_color_comp_index(psd_get_color_comp_index);
private
dev_proc_encode_color(psd_encode_color);
private
dev_proc_decode_color(psd_decode_color);
private
dev_proc_update_spot_equivalent_colors(psd_update_spot_equivalent_colors);
...
ENUM_PTRS_END
private
RELOC_PTRS_WITH(psd_device_reloc_ptrs, psd_device* pdev)
{
...
gs_private_st_composite_final(st_psd_device, psd_device, "psd_device",
                              psd_device_enum_ptrs, psd_device_reloc_ptrs,
                              psd_device_finalize);
...
#define device_procs(get_color_mapping_procs)                                          \
	{                                                                              \
		psd_prn_open, gx_default_get_initial_matrix,                           \
		    NULL,                 /* sync_output */                            \
 ...
 */
private
const gx_device_procs spot_rgb_procs =
    device_procs(get_psdrgb_color_mapping_procs);

const psd_device gs_psdrgb_device = {
    psd_device_body(spot_rgb_procs, "psdrgb", 3, GX_CINFO_POLARITY_ADDITIVE, 24,
                    255, 255, "DeviceRGB"),
...
const psd_device gs_psdcmyk_device = {
    psd_device_body(spot_cmyk_procs, "psdcmyk", NC,
                    GX_CINFO_POLARITY_SUBTRACTIVE, NC * 8, 255, 255,
                    "DeviceCMYK"),
    /* devn_params specific parameters */
    {
     8, /* Bits per color - must match ncomp, depth, etc. above */
...
private
void
rgb_cs_to_psdrgb_cm(gx_device* dev, const gs_imager_state* pis, frac r, frac g,
                    frac b, frac out[])
{
	int i = ((psd_device*)dev)->devn_params.separations.num_separations;
...
private
void
rgb_cs_to_psdcmyk_cm(gx_device* dev, const gs_imager_state* pis, frac r, frac g,
                     frac b, frac out[])
{
	int* map = ((psd_device*)dev)->devn_params.separation_order_map;
...
private
void
cmyk_cs_to_psdcmyk_cm(gx_device* dev, frac c, frac m, frac y, frac k,
                      frac out[])
{
	int* map = ((psd_device*)dev)->devn_params.separation_order_map;
...
private
void
rgb_cs_to_spotn_cm(gx_device* dev, const gs_imager_state* pis, frac r, frac g,
                   frac b, frac out[])
{
	psd_device* xdev = (psd_device*)dev;
	int n = xdev->devn_params.separations.num_separations;
...
psd_update_spot_equivalent_colors(gx_device* pdev, const gs_state* pgs)
{
	...
	update_spot_equivalent_cmyk_colors(pdev, pgs, &psdev->devn_params,
	                                   &psdev->equiv_cmyk_colors);
	return 0;
}
                 ...
                 int* poutn)
{
	...
	luo = icco->get_luobj(icco, icmFwd, icmDefaultIntent, icmSigDefaultData,
	                      icmLuOrdNorm);
	if(luo == NULL)
		return_error(gs_error_rangecheck);
	*pluo = luo;
...
psd_get_params(gx_device* pdev, gs_param_list* plist)
{
	...
	if(code < 0)
		return code;
	code = devn_get_params(pdev, plist, &(xdev->devn_params),
	                       &(xdev->equiv_cmyk_colors));
	if(code < 0)
		return code;

#if ENABLE_ICC_PROFILE
	pos.data = (const byte*)xdev->profile_out_fn,
	pos.size = strlen(xdev->profile_out_fn), pos.persistent = false;
	code = param_write_string(plist, "ProfileOut", &pos);
	if(code < 0)
		return code;

	prgbs.data = (const byte*)xdev->profile_rgb_fn,
	prgbs.size = strlen(xdev->profile_rgb_fn), prgbs.persistent = false;
	code = param_write_string(plist, "ProfileRgb", &prgbs);
	if(code < 0)
		return code;

	pcmyks.data = (const byte*)xdev->profile_cmyk_fn,
	pcmyks.size = strlen(xdev->profile_cmyk_fn), pcmyks.persistent = false;
...
private
int
psd_param_read_fn(gs_param_list* plist, const char* name, gs_param_string* pstr,
                  uint max_len)
{
...
psd_put_params(gx_device* pdev, gs_param_list* plist)
{
	...
	if(code == 0)
		code =
		    devn_printer_put_params(pdev, plist, &(pdevn->devn_params),
...
private
int
psd_get_color_comp_index(gx_device* dev, const char* pname, int name_size,
                         int component_type)
{
	return devn_get_color_comp_index(
	    dev, &(((psd_device*)dev)->devn_params),
	    &(((psd_device*)dev)->equiv_cmyk_colors), pname, name_size,
...
#define assign_u32(a, v)                                                       \
	a = (((v) >> 24) & 0xff) + (((v) >> 8) & 0xff00) +                     \
...
#define NUM_CMYK_COMPONENTS 4
	 ...
	 * which planes are actually imaged.  For the process color model
	 * channels
	 * we image the channels which are requested.  Non requested process
	 * color
	...
	for(i = 0; i < xc->base_bytes_pp + xc->n_extra_channels; i++) {
		...
		if(sep_order_num != GX_DEVICE_COLOR_MAX_COMPONENTS) {
			if(i < NUM_CMYK_COMPONENTS) /* Do not rearrange CMYK */
				...
				xc->chnl_to_position[xc->num_channels] =
				    sep_order_num;
...
psd_write_header(psd_write_ctx* xc, psd_device* pdev)
{
	...
	psd_write_16(
	    xc,
	    (bits16)
	        bytes_pp); /* Channels (2 Bytes) - Supported range is 1 to 24 */
	...
	/* Channel Names */
	for(chan_idx = NUM_CMYK_COMPONENTS; chan_idx < xc->num_channels;
	    chan_idx++) {
		sep_num = xc->chnl_to_orig_sep[chan_idx] - NUM_CMYK_COMPONENTS;
		separation_name =
		    &(pdev->devn_params.separations.names[sep_num]);
		chan_names_len += (separation_name->size + 1);
	}
	psd_write_32(
	    xc, 12 + (chan_names_len + (chan_names_len % 2)) +
	            (12 + (14 * (xc->num_channels - xc->base_bytes_pp))) + 28);
	...
	psd_write_16(xc, 0);    /* PString */
	psd_write_32(xc,
	             14 * (xc->num_channels - xc->base_bytes_pp)); /* Length */
	for(chan_idx = NUM_CMYK_COMPONENTS; chan_idx < xc->num_channels;
	    chan_idx++) {
		sep_num = xc->chnl_to_orig_sep[chan_idx] - NUM_CMYK_COMPONENTS;
		psd_write_16(xc, 02); /* CMYK */
		/* PhotoShop stores all component values as if they were
		 * additive. */
		if(pdev->equiv_cmyk_colors.color[sep_num].color_info_valid) {
#define convert_color(component)                                               \
	((bits16)(                                                             \
	    (65535 *                                                           \
	     ((double)(frac_1 -                                                \
	               pdev->equiv_cmyk_colors.color[sep_num].component)) /    \
	     frac_1)))
...
private
void
psd_calib_row(psd_write_ctx* xc, byte** tile_data, const byte* row, int channel,
              icmLuBase* luo)
{
	...
	for(x = 0; x < xc->width; x++) {
		if(channel < outn) {
			...
			for(plane_idx = 0; plane_idx < inn; plane_idx++)
				in[plane_idx] =
				    row[x * channels + plane_idx] * (1.0 / 255);

			(*tile_data)[x] = (int)(0.5 + 255 * out[channel]);
			luo->lookup(luo, out, in);
		} else {
			(*tile_data)[x] =
			    255 ^ row[x * channels + base_bytes_pp + channel];
		}
	}
}
...
psd_write_image_data(psd_write_ctx* xc, gx_device_printer* pdev)
{
	...
	sep_line =
	    gs_alloc_bytes(pdev->memory, xc->width, "psd_write_sep_line");
	...
	for(chan_idx = 0; chan_idx < xc->num_channels; chan_idx++) {
		for(j = 0; j < xc->height; ++j) {
			...
			/* Check if the separation is present in the
			 * SeparationOrder */
			if(data_pos >= 0) {
