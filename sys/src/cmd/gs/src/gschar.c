...
private
int show_n_begin(gs_show_enum* penum, gs_state* pgs, int code,
                 gs_text_enum_t* pte);
...
gs_show_enum_release(gs_show_enum* penum, gs_memory_t* emem)
{
	if(penum->text.operation) /* otherwise, never initialized */
		penum->procs->release((gs_text_enum_t*)penum,
...
/* show[_n] */
int
gs_show_n_init(gs_show_enum* penum, gs_state* pgs, const char* str, uint size)
{
	gs_text_enum_t* pte;
	int code =
	    gs_show_begin(pgs, (const byte*)str, size, pgs->memory, &pte);
...
/* ashow[_n] */
int
gs_ashow_n_init(gs_show_enum* penum, gs_state* pgs, floatp ax, floatp ay,
...
/* widthshow[_n] */
int
gs_widthshow_n_init(gs_show_enum* penum, gs_state* pgs, floatp cx, floatp cy,
                    gs_char chr, const char* str, uint size)
{
	gs_text_enum_t* pte;
...
/* awidthshow[_n] */
int
gs_awidthshow_n_init(gs_show_enum* penum, gs_state* pgs, floatp cx, floatp cy,
                     gs_char chr, floatp ax, floatp ay, const char* str,
                     uint size)
{
	gs_text_enum_t* pte;
	int code =
	    gs_awidthshow_begin(pgs, cx, cy, chr, ax, ay, (const byte*)str,
	                        size, pgs->memory, &pte);
...
/* kshow[_n] */
int
gs_kshow_n_init(gs_show_enum* penum, gs_state* pgs, const char* str, uint size)
{
	gs_text_enum_t* pte;
	int code;
...
/* xyshow[_n] */
int
gs_xyshow_n_init(gs_show_enum* penum, gs_state* pgs, const char* str, uint size)
{
	gs_text_enum_t* pte;
                  ...
                  bool stroke_path)
{
	gs_text_enum_t* pte;
	int code =
	    gs_glyphpath_begin(pgs, glyph, stroke_path, pgs->memory, &pte);
...
/* cshow[_n] */
int
gs_cshow_n_init(gs_show_enum* penum, gs_state* pgs, const char* str, uint size)
{
	gs_text_enum_t* pte;
	int code =
	    gs_cshow_begin(pgs, (const byte*)str, size, pgs->memory, &pte);
...
/* stringwidth[_n] */
int
gs_stringwidth_n_init(gs_show_enum* penum, gs_state* pgs, const char* str,
                      uint size)
{
	gs_text_enum_t* pte;
...
/* charpath[_n] */
int
gs_charpath_n_init(gs_show_enum* penum, gs_state* pgs, const char* str,
                   uint size, bool stroke_path)
{
	gs_text_enum_t* pte;
...
/* charboxpath[_n] */
int
gs_charboxpath_n_init(gs_show_enum* penum, gs_state* pgs, const char* str,
                      uint size, bool use_boxes)
{
	gs_text_enum_t* pte;
...
/* Note that this returns 1 if we just set up the cache device. */
int
gs_setcachedevice2_double(gs_show_enum* penum, gs_state* pgs, const double* pw2)
{
	if(penum->pgs != pgs)
		return_error(gs_error_rangecheck);
...
int
gs_setcharwidth(gs_show_enum* penum, gs_state* pgs, floatp wx, floatp wy)
{
	double w[2];
