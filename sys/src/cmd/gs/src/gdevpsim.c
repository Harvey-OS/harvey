...
private
dev_proc_print_page(psmono_print_page);
private
dev_proc_close_device(psmono_close);

const gx_device_printer gs_psmono_device =
    prn_device(prn_std_procs, "psmono", DEFAULT_WIDTH_10THS,
               DEFAULT_HEIGHT_10THS, X_DPI, Y_DPI, 0, 0, 0, 0, /* margins */
               1, psmono_print_page);

private
const gx_device_procs psgray_procs = prn_color_procs(
    gdev_prn_open, gdev_prn_output_page, psmono_close,
    gx_default_gray_map_rgb_color, gx_default_gray_map_color_rgb);

const gx_device_printer gs_psgray_device = {prn_device_body(
    gx_device_printer, psgray_procs, "psgray", DEFAULT_WIDTH_10THS,
    DEFAULT_HEIGHT_10THS, X_DPI, Y_DPI, 0, 0, 0, 0, /* margins */
    1, 8, 255, 0, 256, 1, psmono_print_page)};

private
const char* const psmono_setup[] = {
    /* Initialize the strings for filling runs. */
    "/.ImageFills [ 0 1 255 {",
    "  256 string dup 0 1 7 { 3 index put dup } for { 8 16 32 64 128 } {",
    "    2 copy 0 exch getinterval putinterval dup", "  } forall pop exch pop",
    "} bind for ] def",
    ...
    "    readhexstring pop exch pop 0 exch dup", "  } bind } repeat",
    "  16 { {", /* 0x30-0x3f: prefix hex digit (N-0x30) to next count */
    "    48 sub 3 -1 roll add 4 bitshift exch .ImageItem", "  } bind } repeat",
    "  32 { {", /* 0x40-0x5f: repeat last data byte (N-0x40) times */
    "    64 sub 3 -1 roll add .ImageFills 2 index dup length 1 sub get get",
    "    exch 0 exch getinterval 0 3 1 roll", "  } bind } repeat",
    "  160 { { pop .ImageItem } } repeat", "] readonly def",
    /* Read one item from a compressed image. */
    /* Stack contents: <buffer> <file> <xdigits> <previous> */
    "/.ImageItem {", "  2 index read pop dup .ImageProcs exch get exec",
    "} bind def",
    ...
    "  image pop pop pop pop grestore", "} def", 0};
...
private
void write_data_run(const byte*, int, FILE*, byte);
private
int
psmono_print_page(gx_device_printer* pdev, FILE* prn_stream)
{
	...
	int lnum;
	byte* line =
	    gs_alloc_bytes(pdev->memory, line_size, "psmono_print_page");
	...
	fprintf(prn_stream, "%d %d %d .ImageRead\n", pdev->width, pdev->height,
...
#define min_repeat_run 10
		while(left >= min_repeat_run) { /* Detect a maximal run of
		                                   non-repeated data. */
			const byte* p1 = p;
			int left1 = left;
			byte b;
			...
			while(left1 >= min_repeat_run &&
			      ((b = *p1) != p1[1] || b != p1[2] || b != p1[3] ||
			       b != p1[4] || b != p1[5] || b != p1[6] ||
			       b != p1[7] || b != p1[8] || b != p1[9]))
				++p1, --left1;
...
/* The device descriptor */
private
dev_proc_print_page(psrgb_print_page);
private
dev_proc_close_device(psrgb_close);

private
const gx_device_procs psrgb_procs =
    prn_color_procs(gdev_prn_open, gdev_prn_output_page, psrgb_close,
                    gx_default_rgb_map_rgb_color, gx_default_rgb_map_color_rgb);

const gx_device_printer gs_psrgb_device = {prn_device_body(
    gx_device_printer, psrgb_procs, "psrgb", DEFAULT_WIDTH_10THS,
    DEFAULT_HEIGHT_10THS, X_DPI, Y_DPI, 0, 0, 0, 0, /* margins */
    3, 24, 255, 255, 256, 256, psrgb_print_page)};

private
const char* const psrgb_setup[] = {
    ...
    "  true 3 colorimage grestore", "} bind def", 0};
...
psrgb_print_page(gx_device_printer* pdev, FILE* prn_stream)
{
	...
	byte* lbuf = gs_alloc_bytes(mem, width * 3, "psrgb_print_page(lbuf)");
	int lnum;
	stream fs, a85s, rls;
