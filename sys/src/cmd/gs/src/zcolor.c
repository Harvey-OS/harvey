...
zcolor_remap_one_store(i_ctx_t *i_ctx_p, floatp min_value)
{
	...
	for(i = 0; i < transfer_map_size; i++) {
		...
		if(code < 0)
			return code;
		pmap->values[i] =
		    (v < min_value ? float2frac(min_value) : v >= 1.0 ? frac_1 : float2frac(v));
	}
...
zcolor_test(i_ctx_t *i_ctx_p)
{
	...
	for(i = 0; i < ncomp; i++) {
		if(r_has_type(op + i, t_real))
			cv[i] = (gx_color_value)(op[i].value.realval * gx_max_color_value);
		else if(r_has_type(op + i, t_integer))
			cv[i] = (gx_color_value)(op[i].value.intval * gx_max_color_value);
		else
...
zcolor_test_all(i_ctx_t *i_ctx_p)
{
	...
	dprintf2("Maximum error %g %s\n",
		 ...
		 maxerror <= acceptable_error ? "is Ok" : maxerror <= 3 * acceptable_error / 2 ? "is POOR" : "FAILED");

	if(linsep)
		dprintf2("Maximum linear_and_separable error %g %s\n",
			 (float)lsmaxerror / (float)gx_max_color_value,
			 lsmaxerror <= acceptable_error ? "is Ok" : lsmaxerror <= 3 * acceptable_error / 2 ? "is POOR" : "FAILED");
