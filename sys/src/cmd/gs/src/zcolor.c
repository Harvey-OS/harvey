...
zcurrentcolorspace(i_ctx_t* i_ctx_p)
{
	...
	push(1);
	if(gs_color_space_get_index(igs->color_space) ==
	   gs_color_space_index_DeviceGray) {
		...
		if(array_get(imemory, &csa, 0, &gray) >= 0 &&
			...
			*op = istate->colorspace.array;
			...
			int code =
			    ialloc_ref_array(op, a_all, 1, "currentcolorspace");
			if(code < 0)
				return code;
			return name_enter_string(imemory, "DeviceGray",
			                         op->value.refs);
		}
	} else
		*op = istate->colorspace.array;
...
zsetcolor(i_ctx_t* i_ctx_p)
{
	...
	if((n_comps = cs_num_components(pcs)) < 0) {
		...
		if(r_has_type(op, t_dictionary)) {
			...
			n_numeric_comps =
			    (pattern_instance_uses_base_space(cc.pattern)
			         ? n_comps - 1
			         : 0);
			(void)dict_int_param(op, "PatternType", 1, 2, 1,
			                     &ptype);
			is_ptype2 = ptype == 2;
		} else
			n_numeric_comps = 0;
...
zincludecolorspace(i_ctx_t* i_ctx_p)
{
	...
	code =
	    gs_includecolorspace(igs, nsref.value.const_bytes, r_size(&nsref));
	if(!code)
		pop(1);
	return code;
}
...
zsettransfer(i_ctx_t* i_ctx_p)
{
	...
	pop(1);
	return zcolor_remap_one(i_ctx_p, &istate->transfer_procs.gray,
 ...
 */
int
zcolor_remap_one(i_ctx_t* i_ctx_p, const ref* pproc, gx_transfer_map* pmap,
                 const gs_state* pgs, op_proc_t finish_proc)
{
	os_ptr op;
	...
	if(r_size(pproc) == 0) {
		...
		/*
		 * Even though we don't actually push anything on the e-stack,
		 * all
	...
	*op = *pproc;
	++esp;
	make_struct(esp, imemory_space((gs_ref_memory_t*)pgs->memory), pmap);
...
zcolor_remap_one_store(i_ctx_t* i_ctx_p, floatp min_value)
{
	...
	for(i = 0; i < transfer_map_size; i++) {
		double v;
		int code = real_param(
		    ref_stack_index(&o_stack, transfer_map_size - 1 - i), &v);

		if(code < 0)
			return code;
		pmap->values[i] =
		    (v < min_value ? float2frac(min_value)
		                   : v >= 1.0 ? frac_1 : float2frac(v));
	}
...
zcolor_test(i_ctx_t* i_ctx_p)
{
	...
	for(i = 0; i < ncomp; i++) {
		if(r_has_type(op + i, t_real))
			cv[i] = (gx_color_value)(op[i].value.realval *
			                         gx_max_color_value);
		else if(r_has_type(op + i, t_integer))
			cv[i] = (gx_color_value)(op[i].value.intval *
			                         gx_max_color_value);
		else
...
zcolor_test_all(i_ctx_t* i_ctx_p)
{
	...
	if(ncomp == 1)
		acceptable_error =
		    gx_max_color_value / dev->color_info.max_gray + 1;
	else
		acceptable_error =
		    gx_max_color_value / dev->color_info.max_color + 1;
	...
	dprintf1("separable_and_linear = %s\n",
	         linsep == GX_CINFO_SEP_LIN_NONE
	             ? "No"
	             : linsep == GX_CINFO_SEP_LIN ? "Yes" : "Unknown");
		...
		for(j = 0; j <= steps; j++) {
			...
			if(linsep) {
				...
				if((color != lscolor) && (linsepfailed < 5)) {
					linsepfailed++;
					dprintf(
					    "Failed separable_and_linear for");
					for(i = 0; i < ncomp; i++)
						dprintf1(" %d", cv[i]);
					dprintf("\n");
					dprintf2("encode_color=%x  "
					         "gx_default_encode_color=%x\n",
	...
	dprintf2("Maximum error %g %s\n",
	         ...
	         maxerror <= acceptable_error
	             ? "is Ok"
	             : maxerror <= 3 * acceptable_error / 2 ? "is POOR"
	                                                    : "FAILED");

	if(linsep)
		dprintf2("Maximum linear_and_separable error %g %s\n",
		         (float)lsmaxerror / (float)gx_max_color_value,
		         lsmaxerror <= acceptable_error
		             ? "is Ok"
		             : lsmaxerror <= 3 * acceptable_error / 2
		                   ? "is POOR"
		                   : "FAILED");
