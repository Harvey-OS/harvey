...
#endif
			rc = DosLoadModule(buf, sizeof(buf), dllname,
			                   &gsdll.hmodule);
		}
	}
	if(rc == 0) {
		...
		if(gsdll.revision(&rv, sizeof(rv)) != 0) {
			sprintf(buf, "Unable to identify Ghostscript DLL "
			             "revision - it must be newer than "
			             "needed.\n");
			gs_addmess(buf);
			gs_load_dll_cleanup();
			return FALSE;
		...
		if(rv.revision != GS_REVISION) {
			sprintf(buf, "Wrong version of DLL found.\n  Found "
			             "version %ld\n  Need version  %ld\n",
			        rv.revision, (int32_t)GS_REVISION);
			gs_addmess(buf);
			gs_load_dll_cleanup();
			return FALSE;
		}
		...
		if((rc =
		        DosQueryProcAddr(gsdll.hmodule, 0, "GSAPI_NEW_INSTANCE",
		...
		if((rc = DosQueryProcAddr(
		        gsdll.hmodule, 0, "GSAPI_DELETE_INSTANCE",
		        (PFN*)(&gsdll.delete_instance))) != 0) {
			sprintf(buf,
			        "Can't find GSAPI_DELETE_INSTANCE, rc = %d\n",
			        rc);
			gs_addmess(buf);
			gs_load_dll_cleanup();
			return FALSE;
		...
		if((rc = DosQueryProcAddr(
		        gsdll.hmodule, 0, "GSAPI_SET_DISPLAY_CALLBACK",
		        (PFN*)(&gsdll.set_display_callback))) != 0) {
			sprintf(
			    buf,
			    "Can't find GSAPI_SET_DISPLAY_CALLBACK, rc = %d\n",
			    rc);
			gs_addmess(buf);
			gs_load_dll_cleanup();
			return FALSE;
		...
		if((rc = DosQueryProcAddr(
		        gsdll.hmodule, 0, "GSAPI_INIT_WITH_ARGS",
		        (PFN*)(&gsdll.init_with_args))) != 0) {
			sprintf(buf,
			        "Can't find GSAPI_INIT_WITH_ARGS, rc = %d\n",
			        rc);
			gs_addmess(buf);
			gs_load_dll_cleanup();
			return FALSE;
...
run_gspmdrv(IMAGE* img)
{
	...
	if(DosCreateQueue(&(img->term_queue), QUE_FIFO, term_queue_name)) {
		fprintf(stdout,
		        "run_gspmdrv: failed to create termination queue\n");
		return e_limitcheck;
	}
	/* get full path to gsos2.exe and hence path to gspmdrv.exe */
	if((rc = DosGetInfoBlocks(&pptib, &pppib)) != 0) {
		fprintf(stdout,
		        "run_gspmdrv: Couldn't get module handle, rc = %d\n",
		        rc);
		return e_limitcheck;
	}
	if((rc = DosQueryModuleName(pppib->pib_hmte, sizeof(progname) - 1,
	                            progname)) != 0) {
		fprintf(stdout,
		        "run_gspmdrv: Couldn't get module name, rc = %d\n", rc);
		return e_limitcheck;
	}
	...
	if(rc == ERROR_FILE_NOT_FOUND) {
		...
		rc =
		    DosStartSession(&sdata, &img->session_id, &img->process_id);
	}
	if(rc) {
...
void
image_color(unsigned int format, int index, unsigned char* r, unsigned char* g,
            unsigned char* b)
{
...
display_open(void* handle, void* device)
{
	...
	if(first_image) {
		 ...
		 * A process can have only one child session marked
		 * SSF_RELATED_CHILD.
	...
	if(DosCreateEventSem(name, &(img->sync_event), 0, FALSE)) {
		fprintf(stdout,
		        "display_open: failed to create event semaphore %s\n",
		        name);
		return e_limitcheck;
	}
	...
	if(DosCreateMutexSem(name, &(img->bmp_mutex), 0, FALSE)) {
		...
		fprintf(stdout,
		        "display_open: failed to create mutex semaphore %s\n",
		        name);
		return e_limitcheck;
	}
	...
	if(DosAllocSharedMem((PPVOID)&img->bitmap, name, 13 * 1024 * 1024,
	                     PAG_READ | PAG_WRITE)) {
		fprintf(
		    stdout,
		    "display_open: failed allocating shared BMP memory %s\n",
		    name);
		return e_limitcheck;
	}
                ...
                unsigned int format)
{
	...
	fprintf(stdout, "display_presize(0x%x 0x%x, %d, %d, %d, %d)\n", handle,
	        device, width, height, raster, format);
#endif
	img = image_find(handle, device);
	if(img) {
		...
		if(((color == DISPLAY_COLORS_NATIVE) ||
		   ...
		   ((depth == DISPLAY_DEPTH_1) || (depth == DISPLAY_DEPTH_4) ||
		    (depth == DISPLAY_DEPTH_8)))
			img->format_known = TRUE;
		if((color == DISPLAY_COLORS_RGB) &&
		   (depth == DISPLAY_DEPTH_8) && (alpha == DISPLAY_ALPHA_NONE))
			img->format_known = TRUE;
		if(!img->format_known) {
			fprintf(stdout, "display_presize: format %d = 0x%x is "
			                "unsupported\n",
			        format, format);
			return e_limitcheck;
		}
		...
		/* remember parameters so we can figure out where to allocate
		 * bitmap */
		img->width = width;
		img->height = height;
		img->raster = raster;
             ...
             unsigned int format, unsigned char* pimage)
{
	...
	if(img) {
		...
		case DISPLAY_DEPTH_4:
			...
			break;
		case DISPLAY_DEPTH_8:
			if((img->format & DISPLAY_COLORS_MASK) ==
			   DISPLAY_COLORS_NATIVE)
				bmi->cBitCount = 8;
			else if((img->format & DISPLAY_COLORS_MASK) ==
			        DISPLAY_COLORS_GRAY)
				bmi->cBitCount = 8;
			else
				bmi->cBitCount = 24;
	...
	if(img) {
		...
		if(needed > img->committed) {
			...
			if(rc = DosSetMem(img->bitmap + img->committed,
				...
				fprintf(
				    stdout,
				    "No memory in display_memalloc rc = %d\n",
				    rc);
				return NULL;
			}
...
display_callback display = {sizeof(display_callback), DISPLAY_VERSION_MAJOR,
                            DISPLAY_VERSION_MINOR, display_open,
                            display_preclose, display_close, display_presize,
                            display_size, display_sync, display_page,
                            display_update, display_memalloc, display_memfree};
...
main(int argc, char* argv[])
{
	...
	/* insert -dDisplayFormat=XXXXX as first argument */
	{
		int format = DISPLAY_COLORS_NATIVE | DISPLAY_ALPHA_NONE |
		             DISPLAY_DEPTH_1 | DISPLAY_LITTLEENDIAN |
		             DISPLAY_BOTTOMFIRST;
		int depth;
		...
		if((depth <= 8) && !display_hasPalMan)
			depth =
			    24; /* disaster: limited colours and no palette */
		WinReleasePS(ps);

		if(depth > 8)
			format = DISPLAY_COLORS_RGB | DISPLAY_ALPHA_NONE |
			         DISPLAY_DEPTH_8 | DISPLAY_LITTLEENDIAN |
			         DISPLAY_BOTTOMFIRST;
		else if(depth >= 8)
			...
			format = DISPLAY_COLORS_NATIVE | DISPLAY_ALPHA_NONE |
			         DISPLAY_DEPTH_4 | DISPLAY_LITTLEENDIAN |
			         DISPLAY_BOTTOMFIRST;
	...
	if((code = gsdll.new_instance(&instance, NULL)) == 0) {
		gsdll.set_stdio(instance, gsdll_stdin, gsdll_stdout,
		                gsdll_stderr);
		gsdll.set_display_callback(instance, &display);

		code = gsdll.init_with_args(instance, nargc, nargv);
		if(code == 0)
			code = gsdll.run_string(instance, start_string, 0,
			                        &exit_code);
