                 ...
                 cos_type_t cotype, cos_object_t** ppco, int32_t id)
{
	...
	*ppco = pco = cos_object_alloc(pdev, "pdf_create_named");
	...
	pco->id = (id == -1 ? 0L : id == 0 ? pdf_obj_ref(pdev) : id);
	if(pname) {
		int code =
		    cos_dict_put(pdev->local_named_objects, pname->data,
                ...
                cos_object_t** ppco)
{
	...
	if(pdf_key_eq(pname, "{ThisPage}"))
		...
		code =
		    pdf_create_named(pdev, pname, cos_type_generic, ppco, 0L);
...
pdf_scan_token(const byte** pscan, const byte* end, const byte** ptoken)
{
	...
	while(p < end && scan_char_decoder[*p] == ctype_space) {
		...
		if(p[-1] == 0 && p + 1 < end && *p == 0 && p[1] == '/') {
			/* Special handling for names delimited by a null
			 * character. */
			*ptoken = ++p;
			while(*p != 0)
				if(++p >= end)
					return_error(
					    gs_error_syntaxerror); /* no
					                              terminator
					                              */
			*pscan = p;
			return 1;
	...
	*ptoken = p;
	...
	case '(': {
		...
		do {
			...
			w.ptr = buf;
			w.ptr--;
			status = (*s_PSSD_template.process)((stream_state*)&ss,
			                                    &r, &w, true);
		} while(status == 1);
		*pscan = r.ptr + 1;
		return 1;
	...
	case '<':
		...
		if(p[1] != '<') {
			/*
			 * We need the cast because some compilers declare
			 * memchar as
