...
#define BITS32_PER_COLOR_SAMPLES                                               \
	((GS_IMAGE_MAX_COLOR_COMPONENTS + BYTES_PER_BITS32 - 1) /              \
	 BYTES_PER_BITS32)
...
private
irender_proc(image_render_color);
irender_proc_t
...
#define pdevc_next (&pic_next->dev_color)
	...
	if_debug5('b', "[b]y=%d data_x=%d w=%d xt=%f yt=%f\n", penum->y, data_x,
	          w, fixed2float(xprev), fixed2float(yprev));
	memset(&run, 0, sizeof(run));
	memset(&next, 0, sizeof(next));
	/* Ensure that we don't get any false dev_color_eq hits. */
	if(use_cache) {
		set_nonclient_dev_color(&empty_clue.dev_color,
		                        gx_no_color_index);
		pic = &empty_clue;
	}
...
#define CLUE_HASH4(penum, next)                                                \
	&clues[(next.v[0] + (next.v[1] << 2) + (next.v[2] << 4) +              \
		...
		map4:
			...
			if(device_color) {
				...
				if(penum->alpha) {
					/*
					 * We do not have support for DeviceN
					 * color and alpha.
					 */
					cmap_procs->map_rgb_alpha(
					    ...
					    byte2frac(next.v[1]),
					    byte2frac(next.v[2]),
					    byte2frac(next.v[3]), pdevc_next,
					    pis, dev, gs_color_select_source);
					goto mapped;
				}
				/*
				 * We can call the remap concrete_color for the
				 * colorspace
				 * directly since device_color is only true if
				 * the colorspace
				 * is concrete.
				 */
				...
				remap_concrete_color(frac_color, pcs,
				                     pdevc_next, pis, dev,
		...
		do3:
			...
			if(spp == 2) { /* might be Gray + alpha */
				...
				color_cmyk_to_rgb(byte2frac(psrc[0]),
				                  byte2frac(psrc[1]),
				                  byte2frac(psrc[2]),
				                  byte2frac(psrc[3]), pis, rgb);
				/*
				 * It seems silly to do all this converting
				 * between
	...
	fill:   /* Fill the region between */
		/* xrun/irun and xprev */
		/*
		 * Note;  This section is nearly a copy of a simlar section
		 * below
		 * for processing the last image pixel in the loop.  This would
		 * have been
		 * made into a subroutine except for complications about the
		 * number of
		 * variables that would have been needed to be passed to the
		 * routine.
		 */
		switch(posture) {
		case image_portrait: { /* Rectangle */
			int xi = irun;
			...
			if(wi > 0)
				code = gx_fill_rectangle_device_rop(
				    xi, vci, wi, vdi, pdevc, dev, lop);
		} break;
		case image_landscape: { /* 90 degree rotated rectangle */
			int yi = irun;
			...
			if(hi > 0)
				code = gx_fill_rectangle_device_rop(
				    vci, yi, vdi, hi, pdevc, dev, lop);
		} break;
		default: { /* Parallelogram */
			code = (*dev_proc(dev, fill_parallelogram))(
			    dev, xrun, yrun, xprev - xrun, yprev - yrun, pdyx,
			    pdyy, pdevc, lop);
			xrun = xprev;
			yrun = yprev;
		}
