...
/* GC procedures */
private
ENUM_PTRS_WITH(device_mask_clip_enum_ptrs, gx_device_mask_clip* mcdev)
{
...
ENUM_PTRS_END
private
RELOC_PTRS_WITH(device_mask_clip_reloc_ptrs, gx_device_mask_clip* mcdev)
{
...
gx_mask_clip_initialize(gx_device_mask_clip* cdev,
                        ...
                        gx_device* tdev, int tx, int ty, gs_memory_t* mem)
{
	...
	for(;;) {
		if(buffer_height <= 0) {
			 ...
			 * We could do copy_mono in chunks, but for now, we
			 * punt.
			 */
			cdev->mdev.base = 0;
			return 0;
