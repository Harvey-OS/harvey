...
{
	gs_font_cid2* pfcid = gs_alloc_struct(
	    mem, gs_font_cid2, &st_gs_font_cid2, "gs_font_cid2_from_type42");
...
gs_private_st_suffix_add1(st_cmap_tt_16bit_format4, gs_cmap_tt_16bit_format4_t,
                          ...
                          cmap_tt_16bit_format4_reloc_ptrs, st_cmap, font);
                             ...
                             uint* pfidx, gs_char* pchr, gs_glyph* pglyph)
{
	...
	for(segment2 = 0; segment2 < pcmap->segCount2; segment2 += 2) {
		...
		roff = U16(ttdata);
		if(roff) {
			uint32_t gidoff = pcmap->idRangeOffset + segment2 +
			                  roff + (chr - start) * 2;
...
private
const gs_cmap_ranges_enum_procs_t tt_16bit_format4_range_procs = {
    tt_16bit_format4_next_range};
private
void
tt_16bit_format4_enum_ranges(const gs_cmap_t* pcmap, gs_cmap_ranges_enum_t* pre)
{
...
private
const gs_cmap_lookups_enum_procs_t tt_16bit_format4_lookup_procs = {
...
private
const gs_cmap_procs_t tt_16bit_format4_procs = {
    tt_16bit_format4_decode_next, tt_16bit_format4_enum_ranges,
    tt_16bit_format4_enum_lookups, gs_cmap_compute_identity};

/*
 * Create a CMap from a TrueType Platform 3, Encoding 1, Format 4 cmap.
 */
int
gs_cmap_from_type42_cmap(gs_cmap_t** ppcmap, gs_font_type42* pfont, int wmode,
                         gs_memory_t* mem)
{
	...
	/* Allocate the CMap. */
	{
		static const gs_cid_system_info_t null_cidsi = {
		    {(const byte*)"none", 4}, {(const byte*)"none", 4}, 0};
