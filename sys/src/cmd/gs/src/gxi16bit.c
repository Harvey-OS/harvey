...
private
const byte*
sample_unpack_16(byte* bptr, int* pdata_x, const byte* data, int data_x,
	...
	while(psrc < bufend) {
		...
		case 4: /* may be CMYK */
			...
			if(COLOR_FRACS_4_EQ(next, run))
				goto inc;
			if(use_mask_color &&
			   mask_color16_matches(next.v, penum, 4)) {
		...
		case 3: /* may be RGB */
			...
			if(COLOR_FRACS_4_EQ(next, run))
				goto inc;
			if(use_mask_color &&
			   mask_color16_matches(next.v, penum, 3)) {
				color_set_null(pdevc_next);
				goto f;
			}
			if(device_color) {
		...
		case 1: /* may be Gray */
			...
			if(next.v[0] == run.v[0])
				goto inc;
			if(use_mask_color &&
			   mask_color16_matches(next.v, penum, 1)) {
				color_set_null(pdevc_next);
				goto f;
			}
			if(device_color) {
