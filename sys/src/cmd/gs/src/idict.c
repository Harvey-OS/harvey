...
/* Copyright (C) 1989, 1996, 1997, 1998, 1999, 2000 Aladdin Enterprises.  All
  rights reserved.
...
/* Forward references */
private
int dict_create_contents(uint size, const ref* pdref, bool pack);
...
dict_find(const ref* pdref, const ref* pkey, ref** ppvalue)
{
	...
	if(r_has_type(pkey, t_name) && dict_is_packed(pdict)) {
		...
		uint hash =
		    dict_hash_mod(dict_name_index_hash(nidx), npairs(pdict)) +
		    1;
	...
	if(gs_debug_c('d') && !(stats_dict.lookups % 1000))
		dlprintf3("[d]lookups=%ld probe1=%ld probe2=%ld\n",
		          stats_dict.lookups, stats_dict.probe1,
		          stats_dict.probe2);
	return code;
}
...
{
	ref arr;
	int code = gs_alloc_ref_array(mem, &arr, a_all,
	                              sizeof(dict) / sizeof(ref), "dict_alloc");
	dict* pdict;
	ref dref;
	...
	make_tav(&dref, t_dictionary,
	         r_space(&arr) | imemory_new_mask(mem) | a_all, pdict, pdict);
...
dict_create_contents(uint size, const ref* pdref, bool pack)
{
	...
	if(pack) {
		...
		make_tasv(&pdict->keys, t_shortarray,
		          r_space(&arr) | a_all | new_mask, asize, packed, pkp);
		for(pzp = pkp, i = 0; i < asize || i % packed_per_ref;
		    pzp++, i++)
...
dict_unpack(ref* pdref, dict_stack_t* pds)
{
		...
		return 0; /* nothing to do */
	{
		...
		if(ref_must_save_in(mem, &old_keys))
			ref_do_save_in(mem, pdref, &pdict->keys,
			               "dict_unpack(keys)");
		...
		if(!ref_must_save_in(mem, &old_keys))
			gs_free_ref_array(mem, &old_keys,
          ...
          ref** ppvalue /* result is stored here */)
{
	...
	case t_real:
		 ...
		 */
		{
			...
			if(expt < sizeof(int32_t) * 8 ||
			   pkey->value.realval == min_long)
				i = (int)pkey->value.realval;
			else
				i = (int)(mant * min_long); /* MSVC 6.00.8168.0
				                               cannot compile
				                               this */
			hash = (uint)i *
			       30503; /*   with -O2 as a single expression    */
		}
		goto ih;
	case t_integer:
	...
	if(dict_is_packed(pdict)) {
		...
		packed_search_1(*ppvalue = packed_search_value_pointer,
		                return 1, if(pslot == 0) pslot = kp, goto miss);
	...
	miss: /* Key is missing, not double wrap.  See above re dictfull. */
		...
		*ppvalue = pdict->values.value.refs + (pslot - kbot);
		...
		for(kp = kbot + dict_hash_mod(hash, size) + 2;;) {
			--kp;
			if((etype = r_type(kp)) ==
			   ktype) { /* Fast comparison if both keys are names */
				if(name_index(mem, kp) == nidx) {
					*ppvalue = pdict->values.value.refs +
					...
					return 1;
			          ...
			          t_null) { /* Empty, deleted, or wraparound. */
				/* Figure out which. */
				if(kp == kbot) {     /* wrap */
...
dict_find_string(const ref* pdref, const char* kstr, ref** ppvalue)
{
	...
	if(pdref != 0) {
		...
		if((code = name_ref(dict_mem(pdict), (const byte*)kstr,
		                    strlen(kstr), &kname, -1)) < 0)
			return code;
...
top:
	if((code = dict_find(pdref, pkey, &pvslot)) <=
	   0) { /* not found */ /* Check for overflow */
		ref kname;
		uint index;
		...
		if(dict_is_packed(pdict)) {
			...
			if(!r_has_type(pkey, t_name) ||
			   name_index(pmem, pkey) >
			       packed_name_max_index) { /* Change to unpacked
			                                   representation. */
			...
			if(ref_must_save_in(
			       mem,
			       &pdict->keys)) { /* See initial comment for why
			                           it is safe */
				/* not to save the change if the keys */
				/* array itself is new. */
				ref_do_save_in(mem, &pdict->keys, kp,
				               "dict_put(key)");
			}
		...
		if(r_has_type(pkey, t_name)) {
			...
			if(pname->pvalue == pv_no_defn &&
			   CAN_SET_PVALUE_CACHE(pds, pdref,
			                        mem)) { /* Set the cache. */
	...
	if_debug8('d', "[d]0x%lx: put key 0x%lx 0x%lx\n  value at 0x%lx: old "
	               "0x%lx 0x%lx, new 0x%lx 0x%lx\n",
	          (uint32_t)pdref->value.pdict, ((const uint32_t*)pkey)[0],
	          ((const uint32_t*)pkey)[1], (uint32_t)pvslot,
                ...
                dict_stack_t* pds)
{
	...
	if((code = name_ref(dict_mem(pdict), (const byte*)kstr, strlen(kstr),
	                    &kname, 0)) < 0)
		return code;
...
dict_undef(ref* pdref, const ref* pkey, dict_stack_t* pds)
{
	...
	if(dict_is_packed(pdict)) {
		...
		if(ref_must_save_in(mem, &pdict->keys))
			ref_do_save_in(mem, &pdict->keys, pkp,
			               "dict_undef(key)");
		/*
		...
		if(pkp[-1] == packed_key_empty) {
			/*
			 * In this case we can replace any preceding deleted
			 * keys with
			 * empty ones as well.
			 */
	...
	if(r_has_type(pkey, t_name)) {
		...
		if(pv_valid(pname->pvalue)) {
			...
			if(!(pds && dstack_dict_is_permanent(pds, pdref)))
				lprintf1("dict_undef: cached name value "
				         "pointer 0x%lx is incorrect!\n",
                   ...
                   dict_stack_t* pds)
{
	...
	while((index = dict_next(pdrfrom, index, elt)) >= 0) {
		...
		if((options & COPY_NEW_ONLY) &&
		   dict_find(pdrto, &elt[0], &pvslot) > 0)
			continue;
		if((options & COPY_FOR_RESIZE) && r_has_type(&elt[0], t_name) &&
		...
		if((code = dict_put(pdrto, &elt[0], &elt[1], pds)) < 0) {
			/*
			 * If COPY_FOR_RESIZE is set, the dict_put isn't
			 * supposed to
...
dict_copy_entries(const ref* pdrfrom, ref* pdrto, bool new_only,
                  dict_stack_t* pds)
{
	return dict_copy_elements(pdrfrom, pdrto,
	                          (new_only ? COPY_NEW_ONLY : 0), pds);
}
...
dict_resize(ref* pdref, uint new_size, dict_stack_t* pds)
{
	...
	make_tav(&drto, t_dictionary, r_space(pdref) | a_all | new_mask, pdict,
	         &dnew);
	dnew.memory = pdict->memory;
	if((code = dict_create_contents(new_size, &drto,
	                                dict_is_packed(pdict))) < 0)
		return code;
	/*
	...
	if(ref_must_save_in(mem, &pdict->values))
		ref_do_save_in(mem, pdref, &pdict->values,
		               "dict_resize(values)");
	else
		gs_free_ref_array(mem, &pdict->values,
		...
		if(r_has_type(eltp, t_name) ||
		   (!dict_is_packed(pdict) && !r_has_type(eltp, t_null))) {
			...
			if_debug6('d', "[d]0x%lx: index %d: %lx %lx, %lx %lx\n",
			          (uint32_t)pdict, index, ((uint32_t*)eltp)[0],
			          ((uint32_t*)eltp)[1], ((uint32_t*)vp)[0],
			          ((uint32_t*)vp)[1]);
			return index;
		}
	}
