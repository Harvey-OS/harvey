...
top:
	if((code = dict_find(pdref, pkey, &pvslot)) <= 0) { /* not found */ /* Check for overflow */
		ref kname;
		uint index;
		...
		if(dict_is_packed(pdict)) {
			...
			if(!r_has_type(pkey, t_name) ||
			   name_index(pmem, pkey) > packed_name_max_index) { /* Change to unpacked representation. */
		...
		if(r_has_type(pkey, t_name)) {
			...
			if(pname->pvalue == pv_no_defn &&
			   CAN_SET_PVALUE_CACHE(pds, pdref, mem)) { /* Set the cache. */
