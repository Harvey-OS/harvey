...
/* Forward references */
private
gx_xfont *lookup_xfont_by_name(gx_device *, const gx_xfont_procs *, gs_font_name *, int, const cached_fm_pair *, const gs_matrix *);
private
	...
	((pfn1)->size == (pfn2)->size && (pfn1)->size != 0 && \
	 !memcmp((char *)(pfn1)->chars, (char *)(pfn2)->chars, (pfn1)->size))
		...
		if(xf == 0 && font->FontType != ft_composite &&
		   uid_is_valid(&((gs_font_base *)font)->UID)) { /* Look for an original font with the same UID. */
...
gs_purge_fm_pair(gs_font_dir *dir, cached_fm_pair *pair, int xfont_only)
{
	...
	gx_purge_selected_cached_chars(dir,
				       (xfont_only ? purge_fm_pair_char_xfont : purge_fm_pair_char),
				       pair);
	if(pair->ttr)
