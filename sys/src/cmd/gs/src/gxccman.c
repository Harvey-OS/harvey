...
/* Copyright (C) 1989, 1995, 1996, 1997, 1999 Aladdin Enterprises.  All rights
  reserved.
...
ENUM_PTRS_END
private
RELOC_PTRS_BEGIN(cc_ptr_reloc_ptrs) {}
RELOC_PTRS_END

/* Forward references */
private
gx_xfont* lookup_xfont_by_name(gx_device*, const gx_xfont_procs*, gs_font_name*,
                               int, const cached_fm_pair*, const gs_matrix*);
private
cached_char* alloc_char(gs_font_dir*, uint32_t);
private
cached_char* alloc_char_in_chunk(gs_font_dir*, uint32_t);
private
void hash_remove_cached_char(gs_font_dir*, uint);
private
void shorten_cached_char(gs_font_dir*, cached_char*, uint);
...
gx_char_cache_alloc(gs_memory_t* struct_mem, gs_memory_t* bits_mem,
                    gs_font_dir* pdir, uint bmax, uint mmax, uint cmax,
                    uint upper)
...
gx_char_cache_init(register gs_font_dir* dir)
{
	int i;
	cached_fm_pair* pair;
	char_cache_chunk* cck = (char_cache_chunk*)gs_alloc_bytes_immovable(
	    dir->ccache.bits_memory, sizeof(char_cache_chunk), "initial_chunk");
	...
	for(i = 0, pair = dir->fmcache.mdata; i < dir->fmcache.mmax;
	    i++, pair++) {
		pair->index = i;
		fm_pair_init(pair);
		pair->ttf = 0;
               ...
               bool design_grid, cached_fm_pair** ppair)
{
	...
	gx_compute_ccache_key(font, char_tm, log2_scale, design_grid, &mxx,
	                      &mxy, &myx, &myy);
	if(dir->fmcache.msize == dir->fmcache.mmax) {
		/* cache is full */ /* Prefer an entry with num_chars == 0, if
	                               any. */
		int count;
	...
	if(font->FontType == ft_TrueType || font->FontType == ft_CID_TrueType)
		if(((gs_font_type42*)font)->FAPI == NULL) {
			...
			gs_matrix m;
			gx_compute_char_matrix(char_tm, log2_scale, &cxx, &cxy,
			                       &cyx, &cyy);
			...
			if(!pair->ttr)
				return_error(gs_error_VMerror);
			/*  We could use a single the reader instance for all
			 * fonts ... */
			pair->ttf = ttfFont__create(dir);
			if(!pair->ttf)
				return_error(gs_error_VMerror);
			gx_ttfReader__set_font(pair->ttr,
			                       (gs_font_type42*)font);
			code = ttfFont__Open_aux(pair->ttf, dir->tti, pair->ttr,
			                         (gs_font_type42*)font, &m,
			                         log2_scale, design_grid);
	...
	if_debug8(
	    ...
	    (uint32_t)pair, (uint32_t)font, pair->mxx, pair->mxy, pair->myx,
	    pair->myy, (int32_t)pair->UID.id, (uint32_t)pair->UID.xvalues);
	*ppair = pair;
	return 0;
}
...
gx_lookup_xfont(const gs_state* pgs, cached_fm_pair* pair, int encoding_index)
{
	...
	if(procs != 0 && font->PaintType == 0) {
		...
		if(font->key_name.size != 0)
			xf = lookup_xfont_by_name(fdev, procs, &font->key_name,
	 ...
	 !memcmp((char*)(pfn1)->chars, (char*)(pfn2)->chars, (pfn1)->size))
		if(xf == 0 && font->font_name.size != 0 &&
		   ...
		   !font_name_eq(&font->font_name, &font->key_name))
			xf = lookup_xfont_by_name(fdev, procs, &font->font_name,
			                          encoding_index, pair, &mat);
		if(xf == 0 && font->FontType != ft_composite &&
		   uid_is_valid(
		       &((gs_font_base*)font)->UID)) { /* Look for an original
		                                          font with the same
		                                          UID. */
			...
			for(pfont = pdir->orig_fonts; pfont != 0;
				...
				if(pfont->FontType != ft_composite &&
				   ...
				   !font_name_eq(&font->key_name,
				                 &pfont->key_name)) {
					xf = lookup_xfont_by_name(
					    fdev, procs, &pfont->key_name,
					    encoding_index, pair, &mat);
					if(xf != 0)
						break;
				}
...
gs_purge_fm_pair(gs_font_dir* dir, cached_fm_pair* pair, int xfont_only)
{
	...
	gx_purge_selected_cached_chars(
	    dir, (xfont_only ? purge_fm_pair_char_xfont : purge_fm_pair_char),
	    pair);
	if(pair->ttr)
...
lookup_xfont_by_name(gx_device* fdev, const gx_xfont_procs* procs,
                     gs_font_name* pfstr, int encoding_index,
                     const cached_fm_pair* pair, const gs_matrix* pmat)
{
	gx_xfont* xf;

	if_debug5('k', "[k]lookup xfont %s [%g %g %g %g]\n", pfstr->chars,
	          pmat->xx, pmat->xy, pmat->yx, pmat->yy);
	xf = (*procs->lookup_font)(fdev, &pfstr->chars[0], pfstr->size,
	                           encoding_index, &pair->UID, pmat,
	                           pair->memory);
...
gx_alloc_char_bits(gs_font_dir* dir, gx_device_memory* dev,
                   gx_device_memory* dev2, uint16_t iwidth, uint16_t iheight,
                   const gs_log2_scale_point* pscale, int depth)
{
	...
	if(iraster != 0 &&
	   iheight >> log2_yscale > dir->ccache.upper / iraster) {
		if_debug5('k', "[k]no cache bits: scale=%dx%d, "
		               "raster/scale=%u, height/scale=%u, upper=%u\n",
		          1 << log2_xscale, 1 << log2_yscale, iraster, iheight,
		          dir->ccache.upper);
		return 0; /* too big */
	}
	...
	if(dev2 == 0) {
		...
		rc_decrement_only(target,
		                  "gx_alloc_char_bits"); /* can't go to 0 */
		pdev->rc = rc;
		...
		isize = gdev_mem_bitmap_size(dev) + gdev_mem_bitmap_size(dev2);
	}
...
void
gx_add_cached_char(gs_font_dir* dir, gx_device_memory* dev, cached_char* cc,
                   cached_fm_pair* pair, const gs_log2_scale_point* pscale)
{
	if_debug5('k', "[k]chaining char 0x%lx: pair=0x%lx, glyph=0x%lx, "
	               "wmode=%d, depth=%d\n",
	          (uint32_t)cc, (uint32_t)pair, (uint32_t)cc->code, cc->wmode,
	          cc_depth(cc));
	if(dev != NULL) {
		...
		gx_add_char_bits(
		    dir, cc,
		    (gs_device_is_abuf((gx_device*)dev) ? &no_scale : pscale));
	}
                 ...
                 const gs_log2_scale_point* plog2_scale)
{
...
#ifdef DEBUG
	if(cc->width % (1 << log2_x) != 0 || cc->height % (1 << log2_y) != 0) {
		lprintf4("size %d,%d not multiple of scale %d,%d!\n", cc->width,
		         cc->height, 1 << log2_x, 1 << log2_y);
		cc->width &= -1 << log2_x;
		cc->height &= -1 << log2_y;
	}
#endif
	...
	if((log2_x | log2_y) != 0) {
		...
		bits_compress_scaled(bits + raster * bbox.p.y, bbox.p.x,
		                     cc->width << log2_x, cc->height << log2_y,
		                     raster, bits, nraster, plog2_scale,
		                     log2_depth);
		bbox.p.x >>= log2_x;
		bbox.p.y >>= log2_y;
	} else {
	...
	cc_set_raster(cc, nraster);
	{
		...
		if(diff >= sizeof(cached_char_head)) {
			...
			if_debug2('K',
			          "[K]shortening char 0x%lx by %u (adding)\n",
			          (uint32_t)cc, diff);
		}
	}
...
{ /* Try allocating at the current position first. */
	...
	if(cc == 0) {
		if(dir->ccache.bspace <
		   dir->ccache.bmax) { /* Allocate another chunk. */
			...
			if(icdsize + sizeof(cached_char_head) > cksize) {
				if_debug2('k', "[k]no cache bits: "
				               "cdsize+head=%lu, cksize=%u\n",
				          icdsize + sizeof(cached_char_head),
				          cksize);
				return 0; /* wouldn't fit */
			}
			cck = (char_cache_chunk*)gs_alloc_bytes_immovable(
			    mem, sizeof(*cck), "char cache chunk");
			if(cck == 0)
				return 0;
			cdata = gs_alloc_struct_array_immovable(
			    mem, cksize, byte, &st_font_cache_bytes,
			...
			while((dir->ccache.chunks = cck = cck->next) !=
			      cck_init) {
				dir->ccache.cnext = 0;
...
#define cc ((cached_char*)cch)
	...
	while(gx_bits_cache_alloc((gx_bits_cache*)&dir->ccache, icdsize, &cch) <
	      0) {
		...
		{ /* Free the character */
			...
			if(pair != 0) {
				...
				while(
				    dir->ccache
				        .table[chi & dir->ccache.table_mask] !=
				    cc)
					chi++;
				hash_remove_cached_char(dir, chi);
			}
...
hash_remove_cached_char(gs_font_dir* dir, uint chi)
{
	...
	while((cc = dir->ccache.table[from]) !=
	      0) { /* Loop invariants: chars[chi] == 0; */
		/* chars[chi+1..from] != 0. */
...
{
	gx_bits_cache_shorten((gx_bits_cache*)&dir->ccache, &cc->head, diff,
	                      cc->chunk);
