...
private
int check_image3x_mask(const gs_image3x_t *pim,
		  ...
		  int depth, gs_memory_t *mem)
{
	...
	midev->is_open = true;
	dev_proc(midev, fill_rectangle)((gx_device *)midev, 0, 0, width, height, (gx_color_index)0);
		   ...
		   const gs_image3x_t *pim)
{
	...
	code = dev_proc(bbdev, begin_typed_image)((gx_device *)bbdev, pis, pmat, pic, prect, pdcolor, pcpath, mem,
						  pinfo);
	if(code < 0) {
...
gx_image3x_end_image(gx_image_enum_common_t *info, bool draw_last)
{
	...
	int ocode =
	    (penum->mask[0].info ? gx_image_end(penum->mask[0].info, draw_last) : 0);
	gx_device *mdev1 = penum->mask[1].mdev;
	int scode =
	    (penum->mask[1].info ? gx_image_end(penum->mask[1].info, draw_last) : 0);
