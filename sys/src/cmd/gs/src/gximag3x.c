...
/* Forward references */
private
dev_proc_begin_typed_image(gx_begin_image3x);
private
image_enum_proc_plane_data(gx_image3x_plane_data);
private
image_enum_proc_end_image(gx_image3x_end_image);
private
image_enum_proc_flush(gx_image3x_flush);
private
image_enum_proc_planes_wanted(gx_image3x_planes_wanted);
...
const gx_image_type_t gs_image_type_3x = {
    ...
    IMAGE3X_IMAGETYPE};
private
const gx_image_enum_procs_t image3x_enum_procs = {
    gx_image3x_plane_data, gx_image3x_end_image, gx_image3x_flush,
    gx_image3x_planes_wanted};
...
gs_private_st_suffix_add9(st_image3x_enum, gx_image3x_enum_t,
                          "gx_image3x_enum_t", image3x_enum_enum_ptrs,
                          image3x_enum_reloc_ptrs, st_gx_image_enum_common,
                          pcdev, mask[0].info, mask[0].mdev, mask[0].data,
                          mask[1].info, mask[1].mdev, mask[1].data, pixel.info,
                          pixel.data);
...
private
int check_image3x_mask(const gs_image3x_t* pim, const gs_image3x_mask_t* pimm,
                       const image3x_channel_values_t* ppcv,
                       image3x_channel_values_t* pmcv,
                       image3x_channel_state_t* pmcs, gs_memory_t* mem);
int
gx_begin_image3x_generic(gx_device* dev, const gs_imager_state* pis,
                         ...
                         gx_image_enum_common_t** pinfo)
{
	...
	penum->mask[0].info = 0, penum->mask[0].mdev = 0,
	penum->mask[0].data = 0;
	penum->mask[1].info = 0, penum->mask[1].mdev = 0,
	penum->mask[1].data = 0;
	...
	penum->num_components = gs_color_space_num_components(pim->ColorSpace);
	gx_image_enum_common_init(
	    (gx_image_enum_common_t*)penum, (const gs_data_image_t*)pim,
	    &image3x_enum_procs, dev, 1 + penum->num_components, pim->format);
	...
	if(penum->mask[0].data || penum->mask[1].data) {
		...
		penum->pixel.data = gs_alloc_bytes(
		    mem, (penum->pixel.width * pim->BitsPerComponent *
	...
	for(i = 0; i < NUM_MASKS; ++i) {
		...
		if((code = gs_matrix_multiply(&mask[i].matrix, pmat, &mat)) <
		       0 ||
		   (code = gs_bbox_transform(&mrect, &mat, &mrect)) < 0)
			return code;
		origin[i].x = (int)floor(mrect.p.x);
		origin[i].y = (int)floor(mrect.p.y);
		code = make_mid(&mdev, dev, (int)ceil(mrect.q.x) - origin[i].x,
		...
		mask[i].image.adjust = false;
		{
			...
			*(gs_data_image_t*)&mask[i].image = pixm->MaskDict;
			...
			mask[i].image.BitsPerComponent =
			    pixm->MaskDict.BitsPerComponent;
		}
		{
			gs_matrix m_mat;

			/*
			 * Adjust the translation for rendering the mask to
			 * include a
			...
			m_mat.ty -= origin[i].y;
			/*
			 * Note that pis = NULL here, since we don't want to
			 * have to
			 * create another imager state with default log_op, etc.
			 * dcolor = NULL is OK because this is an opaque image
			 * with
			 * CombineWithColor = false.
			 */
			code = gx_device_begin_typed_image(
			    mdev, NULL, &m_mat,
			    (const gs_image_common_t*)&mask[i].image,
			    &mask[i].rect, NULL, NULL, mem,
			    &penum->mask[i].info);
			if(code < 0)
				goto out2;
		}
	 ...
	 */
	{
		...
		for(i = 0; i < NUM_MASKS; ++i) {
			...
			case interleave_chunky:
				/* Add the mask data to the depth of the image
				 * data. */
		...
		memcpy(&penum->plane_widths[pi],
		       &penum->pixel.info->plane_widths[0],
		       penum->pixel.info->num_planes *
		           sizeof(penum->plane_widths[0]));
		memcpy(&penum->plane_depths[pi],
		       &penum->pixel.info->plane_depths[0],
		       penum->pixel.info->num_planes *
		           sizeof(penum->plane_depths[0]));
...
out1:
	gs_free_object(mem, penum->mask[0].data,
	               "gx_begin_image3x(mask[0].data)");
	gs_free_object(mem, penum->mask[1].data,
	               "gx_begin_image3x(mask[1].data)");
...
check_image3x_mask(const gs_image3x_t* pim, const gs_image3x_mask_t* pimm,
                   ...
                   image3x_channel_state_t* pmcs, gs_memory_t* mem)
{
	int mask_width = pimm->MaskDict.Width,
	    mask_height = pimm->MaskDict.Height;
	int code;
	...
	default:
		return_error(gs_error_rangecheck);
	case interleave_chunky:
		if(mask_width != pim->Width || mask_height != pim->Height ||
	...
	if(!check_image3x_extent(pim->ImageMatrix.xx,
	   ...
	   !check_image3x_extent(pim->ImageMatrix.yy,
		...
		return_error(gs_error_rangecheck);
	if((code = gs_matrix_invert(&pimm->MaskDict.ImageMatrix,
	                            &pmcv->matrix)) < 0 ||
	   (code = gs_point_transform(mask_width, mask_height, &pmcv->matrix,
	                              &pmcv->corner)) < 0)
		return code;
	...
	pmcv->rect.q.x =
	    (ppcv->rect.q.x * mask_width + pim->Width - 1) / pim->Width;
	pmcv->rect.q.y =
	    (ppcv->rect.q.y * mask_height + pim->Height - 1) / pim->Height;
             ...
             const image3x_channel_state_t* pics2)
{
	...
	if(current < 0)
		lprintf4("channel_next invariant fails: %d/%d < %d/%d\n",
		         pics1->y, pics1->full_height, pics2->y,
		         pics2->full_height);
#endif
...
private
IMAGE3X_MAKE_MID_PROC(make_midx_default); /* check prototype */
private
int
                  ...
                  int depth, gs_memory_t* mem)
{
	...
	midev->is_open = true;
	dev_proc(midev, fill_rectangle)((gx_device*)midev, 0, 0, width, height,
	                                (gx_color_index)0);
	*pmidev = (gx_device*)midev;
	return 0;
}
private
IMAGE3X_MAKE_MCDE_PROC(make_mcdex_default); /* check prototype */
private
int
                   ...
                   const gs_int_point origin[2], const gs_image3x_t* pim)
{
	 ...
	 */
	gx_device_bbox* bbdev = gs_alloc_struct_immovable(
	    mem, gx_device_bbox, &st_device_bbox, "make_mcdex_default");
	int code;
	...
	code = dev_proc(bbdev, begin_typed_image)((gx_device*)bbdev, pis, pmat,
	                                          pic, prect, pdcolor, pcpath,
	                                          mem, pinfo);
	if(code < 0) {
...
private
int
gx_begin_image3x(gx_device* dev, const gs_imager_state* pis,
                 const gs_matrix* pmat, const gs_image_common_t* pic,
                 const gs_int_rect* prect, const gx_drawing_color* pdcolor,
                 const gx_clip_path* pcpath, gs_memory_t* mem,
                 gx_image_enum_common_t** pinfo)
                      ...
                      int* rows_used)
{
	...
	*rows_used = 0;
	...
	for(i = 0; i < NUM_MASKS; ++i)
		if(penum->mask[i].InterleaveType ==
		   interleave_separate_source) {
			/*
			 * In order to be able to recover from interruptions, we
			 * must
			 * limit separate-source processing to 1 scan line at a
			 * time.
			 */
			if(h > 1)
				h = 1;
	...
	if(num_chunky) {
		...
		uint bit_x =
		    bpc * (num_components + num_chunky) * planes[pi].data_x;
		sample_load_declare_setup(
		    sptr, sbit, planes[0].data + (bit_x >> 3), bit_x & 7, bpc);
		...
		for(x = 0; x < width; ++x) {
			...
			for(i = 0; i < NUM_MASKS; ++i)
				if(depth[i]) {
					sample_load_next12(value, sptr, sbit,
					                   bpc);
					sample_store_next12(value, dptr[i],
					                    dbit[i], depth[i],
					                    dbbyte[i]);
				}
			for(i = 0; i < num_components; ++i) {
				sample_load_next12(value, sptr, sbit, bpc);
				sample_store_next12(value, pptr, pbit, bpc,
				                    pbbyte);
			}
		}
		for(i = 0; i < NUM_MASKS; ++i)
			if(penum->mask[i].data)
				sample_store_flush(dptr[i], dbit[i], depth[i],
				                   dbbyte[i]);
	...
	for(i = 0; i < NUM_MASKS; ++i)
		if(mask_plane[i].data) {
			 ...
			 * successfully but processing the pixel rows was
			 * interrupted,
			 * we set rows_used to indicate the number of pixel rows
			 * processed (since there is no way to return two
			 * rows_used
			 * values).  If this happened, some mask rows may get
			 * presented
			 * again.  We must skip over them rather than processing
			 * them
			 * again.
			 */
			...
			if(skip >= h) {
				...
				mask_plane[i].data +=
				    skip * mask_plane[i].raster;
				penum->mask[i].skip = 0;
				code = gx_image_plane_data_rows(
				    penum->mask[i].info, &mask_plane[i], mask_h,
				    &mask_used[i]);
				mask_used[i] += skip;
			}
	...
	if(pixel_planes[0].data) {
		/*
		 * If necessary, flush any buffered mask data to the mask
		 * clipping
		 * device.
		 */
		for(i = 0; i < NUM_MASKS; ++i)
		...
		code = gx_image_plane_data_rows(penum->pixel.info, pixel_planes,
		                                h, &pixel_used);
		/*
		...
		*rows_used = pixel_used;
		...
		if(code < 0) {
			/*
			 * We must prevent the mask data from being processed
			 * again.
			 * We rely on the fact that h > 1 is only possible if
			 * the
	...
	if_debug7(
	    'b', "[b]image3x h=%d %sopacity.y=%d %sopacity.y=%d %spixel.y=%d\n",
...
gx_image3x_planes_wanted(const gx_image_enum_common_t* info, byte* wanted)
{
	 ...
	 * pixel data.
	 */
	bool sso = penum->mask[0].InterleaveType == interleave_separate_source,
	...
	if(sso & sss) {
		...
		memset(wanted + 2, (mask_next <= 0 ? 0xff : 0),
		       info->num_planes - 2);
		wanted[1] = (mask_next >= 0 ? 0xff : 0);
		if(wanted[1]) {
			mask_next =
			    channel_next(&penum->mask[0], &penum->mask[1]);
		...
		memset(wanted + 1, (mask_next <= 0 ? 0xff : 0),
		       info->num_planes - 1);
		/*
		 * In principle, wanted will always be true for both mask and
		 * pixel
		 * data if the full_heights are equal.  Unfortunately, even in
		 * this
		 * case, processing may be interrupted after a mask row has been
		 * passed to the underlying image processor but before the data
		 * row
		 * has been passed, in which case pixel data will be 'wanted',
		 * but
