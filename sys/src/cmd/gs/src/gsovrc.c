...
private
int
c_overprint_read(gs_composite_t** ppct, const byte* data, uint size,
                 gs_memory_t* mem)
{
	...
	if(params.retain_any_comps && !params.retain_spot_comps) {
		code =
		    read_color_index(&params.drawn_comps, data + 1, size - 1);
		if(code < 0)
			return code;
		nbytes += code;
	}
...
private
composite_create_default_compositor_proc(c_overprint_create_default_compositor);
 ...
 */
int
gs_create_overprint(gs_composite_t** ppct, const gs_overprint_params_t* pparams,
                    gs_memory_t* mem)
{
	gs_overprint_t* pct;

	rc_alloc_struct_0(pct, gs_overprint_t, &st_overprint, mem,
...
gs_private_st_suffix_add0_final(st_overprint_device_t, overprint_device_t,
                                "overprint_device_t",
 ...
 */
private
dev_proc_open_device(overprint_open_device);
private
dev_proc_put_params(overprint_put_params);
private
dev_proc_get_page_device(overprint_get_page_device);
private
dev_proc_create_compositor(overprint_create_compositor);
 ...
 */
private
dev_proc_fill_rectangle(overprint_generic_fill_rectangle);
private
dev_proc_fill_rectangle(overprint_sep_fill_rectangle);
...
swap_color_index(int depth, gx_color_index color)
{
	...
	color = ((color >> shift) & mask) | ((color & mask) << shift) |
...
private
int
update_overprint_params(overprint_device_t* opdev,
                        const gs_overprint_params_t* pparams)
{
	...
	if(!pparams->retain_any_comps) {
		...
		if(dev_proc(opdev, fill_rectangle) != gx_forward_fill_rectangle)
			memcpy(&opdev->procs, &no_overprint_procs,
	...
	if(opdev->color_info.separable_and_linear == GX_CINFO_SEP_LIN)
		memcpy(&opdev->procs, &sep_overprint_procs,
		       sizeof(sep_overprint_procs));
	else
		memcpy(&opdev->procs, &generic_overprint_procs,
	...
	if(!pparams->retain_spot_comps)
		...
		if((pprocs = dev_proc(opdev, get_color_mapping_procs)(dev)) ==
		       0 ||
		   pprocs->map_gray == 0 || pprocs->map_rgb == 0 ||
		   pprocs->map_cmyk == 0)
...
private
int
overprint_create_compositor(gx_device* dev, gx_device** pcdev,
                            const gs_composite_t* pct, gs_imager_state* pis,
                            gs_memory_t* memory)
{
	if(pct->type != &gs_composite_overprint_type)
		return gx_default_create_compositor(dev, pcdev, pct, pis,
		                                    memory);
	else {
		int code;
...
private
int
overprint_generic_fill_rectangle(gx_device* dev, int x, int y, int width,
                                 int height, gx_color_index color)
{
	...
	if(tdev == 0)
		...
		return gx_overprint_generic_fill_rectangle(
		    tdev, opdev->drawn_comps, x, y, width, height, color,
		    dev->memory);
}

private
int
overprint_sep_fill_rectangle(gx_device* dev, int x, int y, int width,
                             int height, gx_color_index color)
{
	overprint_device_t* opdev = (overprint_device_t*)dev;
	gx_device* tdev = opdev->target;
 ...
 */
		...
		/*
		 * We can handle rectangle filling via
		...
		if(depth <= 8 * sizeof(mono_fill_chunk) &&
		   (depth & (depth - 1)) == 0)
			return gx_overprint_sep_fill_rectangle_1(
			    tdev, opdev->retain_mask, x, y, width, height,
			    color, dev->memory);
		else
			return gx_overprint_sep_fill_rectangle_2(
			    tdev, opdev->retain_mask, x, y, width, height,
			    color, dev->memory);
	}
}
...
fill_in_procs(gx_device_procs* pprocs)
{
	 ...
	 */
	memcpy(&tmpdev.color_info, &gs_overprint_device.color_info,
...
private
int
c_overprint_create_default_compositor(const gs_composite_t* pct,
                                      gx_device** popdev, gx_device* tdev,
