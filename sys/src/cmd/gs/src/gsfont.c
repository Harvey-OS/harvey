...
const gs_font_procs gs_font_procs_default = {
    ...
    gs_default_font_info, gs_default_same_font, gs_no_encode_char,
    gs_no_decode_glyph, gs_no_enumerate_glyph, gs_default_glyph_info,
    gs_no_glyph_outline, gs_no_glyph_name, gs_default_init_fstack,
    gs_default_next_char_glyph, gs_no_build_char};

private_st_font_dir();
private
struct_proc_enum_ptrs(font_enum_ptrs);
private
struct_proc_reloc_ptrs(font_reloc_ptrs);
...
ENUM_PTRS_WITH(font_dir_enum_ptrs, gs_font_dir* dir)
{
	...
	for(; offset <= tmask; ++offset) {
		...
		if(cc != 0 && !--count) {
			(*dir->ccache.mark_glyph)(mem, cc->code,
			                          dir->ccache.mark_glyph_data);
...
/* See gxfcache.h for why we do this here. */
{
	...
	for(chi = dir->ccache.table_mask; chi >= 0; --chi) {
		...
		if(cc != 0)
			cc_set_pair_only(cc, (cached_fm_pair*)RELOC_OBJ(
			                         cc_pair(cc) - cc->pair_index) +
			                         cc->pair_index);
	}
}
    ...
    void gs_font_finalize(void* vptr)
{
	...
	if_debug4(
	    'u',
	    "[u]unlinking font 0x%lx, base=0x%lx, prev=0x%lx, next=0x%lx\n",
...
private
ENUM_PTRS_WITH(font_enum_ptrs,
               gs_font* pfont) return ENUM_USING(st_gs_notify_list,
                                                 &pfont->notify_list,
                                                 sizeof(gs_notify_list_t),
                                                 index - 5);
...
ENUM_PTRS_END
private
RELOC_PTRS_WITH(font_reloc_ptrs, gs_font* pfont);
...
gs_font_dir_alloc2(gs_memory_t* struct_mem, gs_memory_t* bits_mem)
{
	...
	{ /* Try allocating a very large cache. */
		...
		pdir = gs_font_dir_alloc2_limits(
		    struct_mem, bits_mem, smax_LARGE, bmax_LARGE, mmax_LARGE,
		    cmax_LARGE, blimit_LARGE);
	}
	if(pdir == 0)
#endif
		pdir = gs_font_dir_alloc2_limits(
		    struct_mem, bits_mem, smax_SMALL, bmax_SMALL, mmax_SMALL,
...
gs_font_dir_alloc2_limits(gs_memory_t* struct_mem, gs_memory_t* bits_mem,
                          uint smax, uint bmax, uint mmax, uint cmax,
                          uint upper)
{
	gs_font_dir* pdir = gs_alloc_struct(
	    struct_mem, gs_font_dir, &st_font_dir, "font_dir_alloc(dir)");
	int code;

	if(pdir == 0)
		return 0;
	code = gx_char_cache_alloc(struct_mem, bits_mem, pdir, bmax, mmax, cmax,
	                           upper);
	if(code < 0) {
...
gs_font_alloc(gs_memory_t* mem, gs_memory_type_ptr_t pstype,
              const gs_font_procs* procs, gs_font_dir* dir, client_name_t cname)
{
                   ...
                   client_name_t cname)
{
	...
	if(pfont == 0)
		return 0;
	pfont->FontBBox.p.x = pfont->FontBBox.p.y = pfont->FontBBox.q.x =
	    pfont->FontBBox.q.y = 0;
 ...
 */
	...
	if(gs_debug_c('m')) {
		...
		dprintf7(
		    ...
		    pfont->FontType, pmat->xx, pmat->xy, pmat->yx, pmat->yy,
		    pmat->tx, pmat->ty);
	}
#endif
	/*
...
gs_find_font_by_id(gs_font_dir* pdir, gs_id id, gs_matrix* FontMatrix)
{
	...
	for(; pfont != NULL; pfont = pfont->next)
		if(pfont->id == id &&
		   !memcmp(&pfont->FontMatrix, FontMatrix,
		           sizeof(pfont->FontMatrix)))
			return pfont;
	return NULL;
}
...
int
gs_no_make_font(gs_font_dir* pdir, const gs_font* pfont, const gs_matrix* pmat,
                gs_font** ppfont)
{
	return 0;
}
                     ...
                     gs_font_info_t* info)
{
	...
	if((members & FONT_INFO_FLAGS) &&
	   (info->Flags_requested & FONT_IS_FIXED_WIDTH)) {
		...
		for(index = 0;
		    fixed_width >= 0 &&
		        (code = font->procs.enumerate_glyph(
		             font, &index, GLYPH_SPACE_NAME, &glyph)) >= 0 &&
		        index != 0;) {
			gs_glyph_info_t glyph_info;

			code = font->procs.glyph_info(
			    font, glyph, pmat, (GLYPH_INFO_WIDTH0 << wmode),
			    &glyph_info);
			if(code < 0)
				return code;
		...
		if(fixed_width > 0) {
			...
			info->members |= FONT_INFO_AVG_WIDTH |
			                 FONT_INFO_MAX_WIDTH |
			                 FONT_INFO_MISSING_WIDTH;
			info->AvgWidth = info->MaxWidth = info->MissingWidth =
			    fixed_width;
		}
		...
		for(index = 0;
		    font->procs.enumerate_glyph(font, &index, GLYPH_SPACE_NAME,
		                                &glyph) >= 0 &&
		        index != 0;) {
			/*
			 * If this is a CIDFont or TrueType font that uses
			 * integers as
			 * glyph names, check for glyph 0; otherwise, check for
			 * .notdef.
			 */
			...
			{
				gs_glyph_info_t glyph_info;
				int code = font->procs.glyph_info(
				    font, glyph, pmat,
				    (GLYPH_INFO_WIDTH0 << wmode), &glyph_info);
		...
		if(mask & FONT_SAME_ENCODING) {
			if(bfont->encoding_index != ENCODING_INDEX_UNKNOWN ||
			   obfont->encoding_index != ENCODING_INDEX_UNKNOWN) {
				if(bfont->encoding_index ==
				   obfont->encoding_index)
...
{ /* WMode may be inherited from an upper font. */
	...
	if(members & (GLYPH_INFO_WIDTH0 << wmode) && charstrings_font) {
		if(pmat == 0) {
			...
			code = gs_distance_transform(sbw[2], sbw[3], pmat,
			                             &info->width[wmode]);
			if(code < 0)
				return code;
		}
...
int
gs_no_glyph_outline(gs_font* font, int WMode, gs_glyph glyph,
