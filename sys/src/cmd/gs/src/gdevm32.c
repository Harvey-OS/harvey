...
/* Procedures */
declare_mem_procs(mem_true32_copy_mono, mem_true32_copy_color,
                  mem_true32_fill_rectangle);

/* The device descriptor. */
const gx_device_memory mem_true32_device = mem_full_device(
    "image32", 24, 8, mem_open, gx_default_map_rgb_color,
    gx_default_map_color_rgb, mem_true32_copy_mono, mem_true32_copy_color,
    mem_true32_fill_rectangle, gx_default_cmyk_map_cmyk_color,
...
private
int
mem_true32_fill_rectangle(gx_device* dev, int x, int y, int w, int h,
...
#define dest32 ((bits32*)dest)
		...
		case 4:
			do {
				dest32[3] = dest32[2] = dest32[1] = dest32[0] =
				    a_color;
				inc_ptr(dest, draster);
			} while(--h > 0);
			break;
...
private
int
mem_true32_copy_mono(gx_device* dev, const byte* base, int sourcex, int sraster,
...
private
int
mem_true32_copy_color(gx_device* dev, const byte* base, int sourcex,
...
/* Procedures */
declare_mem_procs(mem32_word_copy_mono, mem32_word_copy_color,
                  mem32_word_fill_rectangle);

/* Here is the device descriptor. */
const gx_device_memory mem_true32_word_device = mem_full_device(
    "image32w", 24, 8, mem_open, gx_default_map_rgb_color,
    gx_default_map_color_rgb, mem32_word_copy_mono, mem32_word_copy_color,
    mem32_word_fill_rectangle, gx_default_cmyk_map_cmyk_color,
...
private
int
mem32_word_copy_mono(gx_device* dev, const byte* base, int sourcex, int sraster,
                     gx_bitmap_id id, int x, int y, int w, int h,
                     gx_color_index zero, gx_color_index one)
{
	return mem_true32_copy_mono(dev, base, sourcex, sraster, id, x, y, w, h,
	                            color_swap_bytes(zero),
...
private
int
mem32_word_copy_color(gx_device* dev, const byte* base, int sourcex,
                      int sraster, gx_bitmap_id id, int x, int y, int w, int h)
{
	gx_device_memory* const mdev = (gx_device_memory*)dev;
	byte* row;
