...
zfile_name_combine(i_ctx_t* i_ctx_p)
{
	...
	flen = r_size(op - 1);
	blen = blen0 =
	    plen + flen + 2; /* Inserts separator and ending zero byte. */
	...
	if(gp_file_name_combine((const char*)prefix, plen, (const char*)fname,
	                        flen, no_sibling, (char*)buffer,
	                        &blen) != gp_combine_success) {
		make_bool(op, false);
	} else {
		buffer =
		    iresize_string(buffer, blen0, blen, "zfile_name_combine");
		if(buffer == 0)
			return_error(e_VMerror);
...
push_string(i_ctx_t* i_ctx_p, const char* v)
{
	...
	push(1);
	make_const_string(op, avm_foreign | a_readonly, len, (const byte*)v);
	return 0;
}
