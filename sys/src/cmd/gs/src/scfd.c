...
#define get_run(decode, initial_bits, min_bits, runlen, str, locl, outl)                                                                                                            \
	...
	if((clen = np->code_length) > initial_bits) {                                                                                                                               \
		...
		if_debug4('W', "%s xcode=0x%x,%d rlen=%d\n", str, (init_bits << np->code_length) + peek_var_bits(np->code_length), initial_bits + np->code_length, np->run_length); \
		skip_bits(np->code_length);                                                                                                                                         \
	} else {                                                                                                                                                                    \
	locl:                                                                                                                                                                       \
		if_debug4('W', "%s code=0x%x,%d rlen=%d\n", str, peek_var_bits(clen), clen, np->run_length);                                                                        \
		skip_bits(clen);                                                                                                                                                    \
	}                                                                                                                                                                           \
...
#ifdef DEBUG
{
	...
	if(ss->wpos == wstop && ss->cbit <= (-ss->Columns & 7) &&
	   (k_left == 0 ? !(ss->run_color & ~1) : ss->run_color == 0)) { /* Check for completed data to be copied to the client. */
...
#define vertical_0 (countof(cf2_run_vertical) / 2)
	...
	/* previous ('reference') line. */
	{
		...
		if((prev_data & count_bit[prev_count & 7]) &&
		   (prev_count < init_count || invert != invert_white)) { /* Look for changing white first. */
