...
#define cfd_load_state()                                                       \
	hcd_load_state(), q = ss->lbuf + ss->wpos, qbit = ss->cbit
/* Store the state back in the stream. */
#define cfd_store_state()                                                      \
	hcd_store_state(), ss->wpos = q - ss->lbuf, ss->cbit = qbit
...
#define get_run(decode, initial_bits, min_bits, runlen, str, locl, outl)       \
	...
	if((clen = np->code_length) > initial_bits) {                          \
		...
		if_debug4('W', "%s xcode=0x%x,%d rlen=%d\n", str,              \
		          ...
		          initial_bits + np->code_length, np->run_length);     \
...
private
int cf_decode_eol(stream_CFD_state*, stream_cursor_read*);
private
int cf_decode_1d(stream_CFD_state*, stream_cursor_read*);
private
int cf_decode_2d(stream_CFD_state*, stream_cursor_read*);
private
int cf_decode_uncompressed(stream_CFD_state*, stream_cursor_read*);
private
int
s_CFD_process(stream_state* st, stream_cursor_read* pr, stream_cursor_write* pw,
              bool last)
{
	stream_CFD_state* const ss = (stream_CFD_state*)st;
...
#ifdef DEBUG
{
	...
	if_debug8('w',
	          ...
	          eol_count, k_left, rows_left, (uint32_t)bits, bits_left,
	...
	if(ss->wpos == wstop && ss->cbit <= (-ss->Columns & 7) &&
	   (k_left == 0
	        ? !(ss->run_color & ~1)
	        : ss->run_color == 0)) { /* Check for completed data to be
	...
	ck_eol:
		...
		switch(eol_count) {
		case 0:
			if(ss->EndOfLine) { /* EOL is required, but none is
			                       present. */
				status = ERRC;
				goto check;
			}
...
#define vertical_0 (countof(cf2_run_vertical) / 2)
	...
	/* previous ('reference') line. */
	{
		...
		byte prev_data;
		int dlen;
		static const byte count_bit[8] = {0x80, 1,    2,    4,
		                                  8,    0x10, 0x20, 0x40};
		...
		if((prev_data & count_bit[prev_count & 7]) &&
		   (prev_count < init_count ||
		    invert !=
		        invert_white)) { /* Look for changing white first. */
			if_debug1('W', " data=0x%x", prev_data);
			skip_black_pixels(prev_data, prev_q, prev_count, invert,
			                  plen);
		...
		if(prev_count != end_count) {
			if_debug1('W', " data=0x%x", prev_data);
			skip_white_pixels(prev_data, prev_q, prev_count, invert,
			                  plen);
			if(prev_count < end_count) /* overshot */
				prev_count = end_count;
		...
		if(rlen == run2_pass) { /* Pass mode.  Find b2. */
			if(prev_count != end_count) {
				if_debug1('W', " data=0x%x", prev_data);
				skip_black_pixels(prev_data, prev_q, prev_count,
				                  invert, plen);
				if(prev_count < end_count) /* overshot */
					prev_count = end_count;
			}
...
out3:
	if(bits_left > 0 && peek_bits(1)) {
		/* This is a 1-bit "vertical 0" code, which we can still
		 * process. */
		goto v0;
	}
/* falls through */
...
cf_decode_uncompressed(stream* s)
{
	...
	while(1) {
		...
		np = &cf_uncompressed_decode[peek_bits(
		    cfd_uncompressed_initial_bits)];
		clen = np->code_length;
		rlen = np->run_length;
		if(clen >
		   cfd_uncompressed_initial_bits) { /* Must be an exit code. */
			break;
		}
		if(rlen == cfd_uncompressed_initial_bits) { /* Longest
		                                               representable
		                                               white run */
			if_debug1('W', "[wu]%d\n", rlen);
	...
	if(rlen >=
	   0) { /* Valid exit code, rlen = 2 * run length + next polarity */
		if((qbit -= rlen >> 1) < 0)
			qbit += 8, q++;
		rlen &= 1;
	}
...
/* Stream template */
const stream_template s_CFD_template = {&st_CFD_state, s_CFD_init,
                                        s_CFD_process, 1, 1, s_CFD_release,
                                        s_CFD_set_defaults};
