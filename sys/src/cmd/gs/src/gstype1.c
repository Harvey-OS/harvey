...
#define clear CLEAR_CSTACK(cstack, csp)
	...
	case 0:
		...
		if(code < 0)
			return code;
		code = t1_hinter__set_font_data(h, 1, pdata,
			...
			if(c < c_pos2_0) { /* 1-byte number */
				...
				*++csp = int2fixed(lw);
				if(lw != fixed2long(*csp)) {
					/*
					 * We handle the only case we've ever
					 * seen that
					 * actually uses such large numbers
					 * specially.
					 */
					int32_t denom;

					c0 = *cip++;
					charstring_next(c0, state, c,
					                encrypted);
					if(c < c_num1)
						return_error(
						    gs_error_rangecheck);
					if(c < c_pos2_0)
						...
						decode_num2(denom, c, cip,
						            state, encrypted);
					else if(c == cx_num4)
						decode_num4(denom, cip, state,
						            encrypted);
					else
						return_error(
...
#define inext goto top
		...
		case cx_vlineto:
			...
			goto line;
		case cx_rrcurveto:
			code = t1_hinter__rcurveto(h, cs0, cs1, cs2, cs3, cs4,
			                           cs5);
			goto cc;
		case cx_endchar:
		...
		case c1_hsbw:
			if(!h->seac_flag) {
				fixed sbx = cs0, sby = fixed_0, wx = cs1,
				      wy = fixed_0;
				...
				code = t1_hinter__sbw_seac(h, pcis->adxy.x,
				                           pcis->adxy.y);
			if(code < 0)
				return code;
		...
		rsbw: /* Give the caller the opportunity to intervene. */
			...
			if(pcis->init_done < 0) {
				...
				/*
				 * Accumulate the side bearing now, but don't do
				 * it
				 * a second time for the base character of a
				 * seac.
				 */
				if(pcis->seac_accent >= 0) {
					/*
					 * As a special hack to work around a
					 * bug in
					 * Fontographer, we deal with the
					 * (illegal)
					 * situation in which the side bearing
					 * of the
					 * accented character (save_lsbx) is
					 * different from
					 * the side bearing of the base
					 * character (cs0/cs1).
					 */
		...
		case cx_escape:
			...
			case ce1_dotsection:
				...
				cnext;
			case ce1_vstem3:
				code = t1_hinter__vstem3(h, cs0, cs1, cs2, cs3,
				                         cs4, cs5);
				if(code < 0)
					return code;
				cnext;
			case ce1_hstem3:
				code = t1_hinter__hstem3(h, cs0, cs1, cs2, cs3,
				                         cs4, cs5);
				if(code < 0)
					return code;
				cnext;
			case ce1_seac:
				code = gs_type1_seac(pcis, cstack + 1,
				                     cstack[0], ipsp);
				if(code != 0) {
					*pindex = ics3;
					return code;
			...
			case ce1_sbw:
				if(!h->seac_flag)
					code = t1_hinter__sbw(h, cs0, cs1, cs2,
					                      cs3);
				else
					code = t1_hinter__sbw_seac(
					    h, cs0 + pcis->adxy.x,
					    cs1 + pcis->adxy.y);
				if(code < 0)
					return code;
			...
			case ce1_callothersubr: {
				...
				/* let the client handle it. */
				{
					...
					/* Copy the arguments to the caller's
					 * stack. */
					if(scount < 1 || csp[-1] < 0 ||
					...
					pcis->ips_count =
					    ipsp - &pcis->ipstack[0] + 1;
					if(scount)
						memcpy(pcis->ostack, cstack,
						       scount * sizeof(fixed));
