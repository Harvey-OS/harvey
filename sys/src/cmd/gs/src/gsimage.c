...
/* Copyright (C) 1996, 1997, 1998, 1999, 2000 Aladdin Enterprises.  All rights
  reserved.
...
typedef struct image_enum_plane_s {
	/*
	  ...
	  - A row buffer, aligned and (logically) large enough to hold one scan
	  line
	    for that plane.  (It may have to be reallocated if the plane width
	  or
	    depth changes.)  A row buffer is "full" if it holds exactly a full
	  scan
	    line.
	*/
	gs_string row;
	/*
	  - A position within the row buffer, indicating how many initial bytes
	  are
	    occupied.
	*/
	uint pos;
...
ENUM_PTRS_END
private
RELOC_PTRS_WITH(gs_image_enum_reloc_ptrs, gs_image_enum* eptr)
{
	int i;
                     ...
                     bool uses_color, gx_image_enum_common_t** ppie)
{
	...
	return gx_device_begin_typed_image(dev, (const gs_imager_state*)pgs,
	                                   NULL, pic, NULL, pgs->dev_color,
	                                   pcpath, pgs->memory, ppie);
}
                     ...
                     const gs_data_image_t* pim, gx_device* dev)
{
	...
	for(i = 0; i < pie->num_planes; ++i) {
		...
		penum->image_planes[i].data_x =
		    0; /* just init once, never changes */
	}
...
{
	return gs_image_common_init(
	    penum, pie, pim,
	    (pgs->in_charpath ? NULL : gs_currentdevice_inline(pgs)));
}
...
free_row_buffers(gs_image_enum* penum, int num_planes, client_name_t cname)
{
	...
	for(i = num_planes - 1; i >= 0; --i) {
		if_debug3('b', "[b]free plane %d row (0x%lx,%u)\n", i,
		          (ulong)penum->planes[i].row.data,
		          penum->planes[i].row.size);
		gs_free_string(gs_image_row_memory(penum),
		               penum->planes[i].row.data,
...
int
gs_image_next(gs_image_enum* penum, const byte* dbytes, uint dsize, uint* pused)
{
	int px = penum->plane_index;
                     ...
                     uint* used /*[num_planes]*/)
{
	...
	if(gs_debug_c('b')) {
		...
		for(pi = 0; pi < num_planes; ++pi)
			dprintf6("[b]plane %d source=0x%lx,%u pos=%u "
			         "data=0x%lx,%u\n",
			         pi, (uint32_t)penum->planes[pi].source.data,
			         penum->planes[pi].source.size,
			         penum->planes[pi].pos,
			         (uint32_t)plane_data[pi].data,
			         plane_data[pi].size);
	}
#endif
	for(i = 0; i < num_planes; ++i) {
	...
	for(;;) {
		...
		for(i = 0; i < num_planes; ++i) {
			...
			if(size > 0) {
				if(pos < raster &&
				   (pos != 0 || size < raster)) {
					...
					/* Make sure the row buffer is fully
					 * allocated. */
					if(raster > old_size) {
						gs_memory_t* mem =
						    gs_image_row_memory(penum);
						byte* old_data =
						    penum->planes[i].row.data;
						byte* row =
						    (old_data == 0
					...
					memcpy(penum->planes[i].row.data + pos,
					       penum->planes[i].source.data,
					       copy);
					penum->planes[i].source.data += copy;
					penum->planes[i].source.size = size -=
					    copy;
			...
			if(pos == raster) {
				/*
				 * This plane will be transferred from the row
				 * buffer,
				 * so we can only transfer one row.
				 */
				h = min(h, 1);
				penum->image_planes[i].data =
				    penum->planes[i].row.data;
			} else if(pos == 0 && size >= raster) {
				/* We can transfer 1 or more planes from the
				 * source. */
				h = min(h, size / raster);
				penum->image_planes[i].data =
				    penum->planes[i].source.data;
		...
		if(penum->dev == 0) {
			/*
			 * ****** NOTE: THE FOLLOWING IS NOT CORRECT FOR
			 * ImageType 3
			 * ****** InterleaveType 2, SINCE MASK HEIGHT AND IMAGE
			 * HEIGHT
			 * ****** MAY DIFFER (BY AN INTEGER FACTOR).  ALSO,
			 * plane_depths[0]
			...
			code = gx_image_plane_data_rows(
			    penum->info, penum->image_planes, h, &h);
