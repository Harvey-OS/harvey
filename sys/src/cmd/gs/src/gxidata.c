...
#define BCOUNT(plane) /* bytes per data row */ \
	(((penum->rect.w + (plane).data_x) * penum->spp * penum->bps / num_planes + 7) >> 3)
	...
	for(; penum->y < y_end; penum->y++) {
		...
		if(!penum->interpolate)
			...
			case image_portrait: { /* Precompute integer y and height, */
			...
			case image_landscape: { /* Check for no pixel centers in x. */
