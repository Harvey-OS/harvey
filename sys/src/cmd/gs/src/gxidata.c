...
private
void update_strip(gx_image_enum* penum);
private
void repack_bit_planes(const gx_image_plane_t* src_planes,
                       const uint32_t* offsets, int num_planes, byte* buffer,
                       int width, const sample_lookup_t* ptab, int spread);
private
gx_device* setup_image_device(const gx_image_enum* penum);
...
#define BCOUNT(plane) /* bytes per data row */                                 \
	(((penum->rect.w + (plane).data_x) * penum->spp * penum->bps /         \
	      num_planes +                                                     \
	  7) >>                                                                \
	 3)
	...
	for(; penum->y < y_end; penum->y++) {
		...
		if(bit_planar) {
			...
			for(px = 0; px < num_planes; px += penum->bps)
				repack_bit_planes(planes, offsets, penum->bps,
				                  penum->buffer, penum->rect.w,
				                  &penum->map[px].table,
				                  penum->spread);
			for(px = 0; px < num_planes; ++px)
			 ...
			 * there is only one plane and we don't need to expand
			 * the
			 * input samples, we may use the data directly.
			 */
			sourcex = planes[0].data_x;
			buffer = (*penum->unpack)(
			    penum->buffer, &sourcex,
			    planes[0].data + offsets[0], planes[0].data_x,
			    BCOUNT(planes[0]), &penum->map[0], penum->spread,
			    num_components_per_plane);
		...
		if(!penum->interpolate)
			...
			case image_portrait: { /* Precompute integer y and
			                          height, */
			...
			case image_landscape: { /* Check for no pixel centers in
			                           x. */
		...
		if(x_used) {
			/*
			 * Processing was interrupted by an error.  Skip over
			 * pixels
			 * already processed.
			 */
		...
		code =
		    (*penum->render)(penum, buffer, sourcex + x_used,
                  ...
                  const sample_lookup_t* ptab, int spread)
{
	...
	int any_data_x = 0;
	bool direct =
	    (spread == 1 && ptab->lookup8[0] == 0 && ptab->lookup8[255] == 255);
	int pi, x;
	gx_image_plane_t* pp;
	...
	for(pi = 0, pp = planes; pi < num_planes; ++pi, ++pp)
		if(src_planes[pi].data == 0) {
			...
			pp->data =
			    src_planes[pi].data + (dx >> 3) + offsets[pi];
...
#define MAP_BYTE(v) (ptab->lookup8[(byte)(v)])
			dest[0] = MAP_BYTE(w0 >> 24);
			dest += spread;
			dest[1] = MAP_BYTE(w0 >> 16);
			dest += spread;
			dest[2] = MAP_BYTE(w0 >> 8);
			dest += spread;
			dest[3] = MAP_BYTE(w0);
			dest += spread;
			dest[4] = MAP_BYTE(w1 >> 24);
			dest += spread;
			dest[5] = MAP_BYTE(w1 >> 16);
			dest += spread;
			dest[6] = MAP_BYTE(w1 >> 8);
			dest += spread;
			dest[7] = MAP_BYTE(w1);
			dest += spread;
#undef MAP_BYTE
