...
{ /*
   ...
   */
	...
	params.options = (actual_data ? GB_RETURN_POINTER : 0) |
	                 GB_RETURN_COPY |
                           ...
                           byte* stored_base)
{
	 ...
	 */
	{
		    ...
		    (both & GB_PACKING_CHUNKY
		         ...
		         : both & GB_PACKING_PLANAR
		               ...
		               : both & GB_PACKING_BIT_PLANAR
		                     ...
		                     : 0 /* not possible */);
		uint raster = (options & (GB_RASTER_STANDARD | GB_RASTER_ANY)
		                   ? dev_raster
		                   : params->raster);
		byte* base;
		...
		if(h <= 1 || raster == dev_raster) {
			int x_offset =
			    (options & GB_OFFSET_ANY
			         ? x
			         : options & GB_OFFSET_0 ? 0
			                                 : params->x_offset);
			...
			if(x_offset == x) {
				...
				uint align_mod = (options & GB_ALIGN_ANY
				                      ? 8
				                      : align_bitmap_mod * 8);
				...
				if(depth & (depth - 1)) {
					...
					int step = depth /
					           igcd(depth, align_mod) *
					           align_mod;

					bytes = bit_offset / step * step;
				} else {
					/* Use a faster algorithm if depth is a
					 * power of 2. */
					bytes = bit_offset &
					        (-depth & -(int)align_mod);
				}
			...
			params->options =
			    GB_ALIGN_STANDARD | GB_RETURN_POINTER |
			    GB_RASTER_STANDARD |
			    (stored->options &
			     ~GB_PACKING_ALL) /*see below for PACKING*/ |
			    (params->x_offset == 0 ? GB_OFFSET_0
			                           : GB_OFFSET_SPECIFIED);
			if(both & GB_PACKING_CHUNKY) {
				...
				for(i = 0; i < n; ++i)
					if(!(both & GB_SELECT_PLANES) ||
					   stored->data[i] != 0) {
						params->data[i] = base;
						base +=
						    dev_raster * dev->height;
					}
			}
			return 0;
		}
...
gx_get_bits_copy_cmyk_1bit(byte* dest_line, uint dest_raster,
                           const byte* src_line, uint src_raster, int src_bit,
                           int w, int h)
{
 ...
 */
private
int gx_get_bits_std_to_native(gx_device* dev, int x, int w, int h,
                              gs_get_bits_params_t* params,
                              const gs_get_bits_params_t* stored,
	...
	if(options & GB_PACKING_CHUNKY) {
		...
		uint std_raster =
		    (options & GB_ALIGN_STANDARD ? bitmap_raster(end_bit)
		                                 : (end_bit + 7) >> 3);
		uint raster = (options & GB_RASTER_STANDARD ? std_raster
		                                            : params->raster);
		...
		if(!(skew & 7) && direct_copy) {
			...
			bytes_copy_rectangle(
			    data + (dest_bit_x >> 3), raster,
			    src + (bit_x >> 3), dev_raster,
			    ((bit_x + bit_w + 7) >> 3) - (bit_x >> 3), h);
		} else if(direct_copy) {
			/*
			 * Use the logic already in mem_mono_copy_mono to copy
			 * the
			 * bits to the destination.  We do this one line at a
			 * time,
			...
			for(; h > 0;
			    line_ptr += raster, src += dev_raster, --h) {
				/* Make sure the destination is aligned. */
				int align =
				    ALIGNMENT_MOD(line_ptr, align_bitmap_mod);

				tdev.base = line_ptr - align;
				/* set up parameters required by copy_mono's
				 * fit_copy */
				tdev.width = dest_bit_x + (align << 3) + bit_w;
				tdev.height = 1;
				(*dev_proc(&mem_mono_device, copy_mono))(
				    (gx_device*)&tdev, src, bit_x, dev_raster,
				    gx_no_bitmap_id, dest_bit_x + (align << 3),
				    0, bit_w, 1, (gx_color_index)0,
				    (gx_color_index)1);
			}
		} else if(options & ~stored_options & GB_COLORS_NATIVE) {
			/* Convert standard colors to native. */
			code = gx_get_bits_std_to_native(
			    dev, x, w, h, params, stored, src_base, dev_raster,
			    x_offset, raster);
			options = params->options;
		} else {
			/* Convert native colors to standard. */
			code = gx_get_bits_native_to_std(
			    dev, x, w, h, params, stored, src_base, dev_raster,
			    x_offset, raster, std_raster);
			options = params->options;
		}
		params->options =
		    (options & (GB_COLORS_ALL | GB_ALPHA_ALL)) |
		    GB_PACKING_CHUNKY |
		    (options & GB_COLORS_NATIVE ? 0 : options & GB_DEPTH_ALL) |
		    (options & GB_ALIGN_STANDARD ? GB_ALIGN_STANDARD
		                                 : GB_ALIGN_ANY) |
		    GB_RETURN_COPY |
		    (x_offset == 0 ? GB_OFFSET_0 : GB_OFFSET_SPECIFIED) |
		    (raster == std_raster ? GB_RASTER_STANDARD
		                          : GB_RASTER_SPECIFIED);
	} else if(!(~options & (GB_PACKING_PLANAR | GB_SELECT_PLANES |
	                        GB_ALIGN_STANDARD)) &&
		...
		dest.raster = (options & GB_RASTER_STANDARD
		                   ? bitmap_raster((x_offset + w) * dest_depth)
		                   : params->raster);
...
gx_get_bits_std_to_native(gx_device* dev, int x, int w, int h,
                          gs_get_bits_params_t* params,
                          const gs_get_bits_params_t* stored,
                          const byte* src_base, uint dev_raster, int x_offset,
                          uint raster)
{
	int depth = dev->color_info.depth;
	int dest_bit_offset = x_offset * depth;
	byte* dest_line = params->data[0] + (dest_bit_offset >> 3);
	int ncolors = (stored->options & GB_COLORS_RGB
	                   ? 3
	                   : stored->options & GB_COLORS_CMYK
	                         ? 4
		...
		for(i = 0; i < w; ++i) {
			...
			if(do_alpha) {
				...
				case 1:
					...
					break;
				case 3:
					map_procs->map_rgb(dev, 0, sc[0], sc[1],
					                   sc[2], dc);
					break;
				case 4:
					map_procs->map_cmyk(dev, sc[0], sc[1],
					                    sc[2], sc[3], dc);
					break;
				default:
					return_error(gs_error_rangecheck);
				...
				for(j = 0; j < dev->color_info.num_components;
				    j++)
					v[j] = frac2cv(dc[j]);
                          ...
                          uint raster, uint std_raster)
{
	...
	if(options & GB_COLORS_RGB)
		params->options = options &=
		    ~GB_COLORS_STANDARD_ALL | GB_COLORS_RGB,
		dest_bytes = 3;
	else if(options & GB_COLORS_CMYK)
		params->options = options &=
		    ~GB_COLORS_STANDARD_ALL | GB_COLORS_CMYK,
		dest_bytes = 4;
	else if(options & GB_COLORS_GRAY)
		params->options = options &=
		    ~GB_COLORS_STANDARD_ALL | GB_COLORS_GRAY,
		dest_bytes = 1;
	else
		return_error(gs_error_rangecheck);
	...
	if(options & GB_RASTER_STANDARD) {
		...
		raster = std_raster =
		    (options & GB_ALIGN_STANDARD ? bitmap_raster(end_byte << 3)
		                                 : end_byte);
	}
	...
	if(((options & (GB_COLORS_RGB | GB_ALPHA_FIRST | GB_ALPHA_LAST)) ==
	   ...
	   dev_proc(dev, map_color_rgb) == cmyk_1bit_map_color_rgb) {
		gx_get_bits_copy_cmyk_1bit(dest_line, raster, src_line,
		                           dev_raster, src_bit_offset & 7, w,
		                           h);
		return 0;
	}
	...
	for(; h > 0; dest_line += raster, src_line += dev_raster, --h) {
		...
		for(i = 0; i < w; ++i) {
			...
			if(options & GB_COLORS_RGB) {
				...
				/* with maximum black generation and undercolor
				 * removal. */
				gx_color_value white =
				    max(rgba[0], max(rgba[1], rgba[2]));

				dest[0] =
				    gx_color_value_to_byte(white - rgba[0]);
				dest[1] =
				    gx_color_value_to_byte(white - rgba[1]);
				dest[2] =
				    gx_color_value_to_byte(white - rgba[2]);
				dest[3] = gx_color_value_to_byte(
				    gx_max_color_value - white);
				dest += 4;
			} else { /* GB_COLORS_GRAY */
                              ...
                              gs_int_rect** unread)
{
	...
	if(prect->q.y == prect->p.y + 1 &&
	   ...
	   unread == NULL) {
		...
		code = (*dev_proc(dev, get_bits))(
		    dev, prect->p.y, row, (params->options & GB_RETURN_POINTER)
		                              ? &params->data[0]
		                              : NULL);
		if(code >= 0) {
			if(row != data) {
				if(prect->p.x == 0 && params->data[0] != row &&
				   params->options & GB_RETURN_POINTER) {
					/*
					 * get_bits returned an appropriate
					 * pointer: we can
					 * avoid doing any copying.
					 */
					DO_NOTHING;
				} else {
					/* Copy the partial row into the
					 * supplied buffer. */
					int width_bits =
					    (prect->q.x - prect->p.x) * depth;
					gx_device_memory tdev;
					...
					tdev.line_ptrs = &tdev.base;
					tdev.base = data;
					code = (*dev_proc(&mem_mono_device,
					                  copy_mono))(
					    (gx_device*)&tdev, row,
					    prect->p.x * depth, min_raster,
					    gx_no_bitmap_id, 0, 0, width_bits,
					    1, (gx_color_index)0,
					    (gx_color_index)1);
					params->data[0] = data;
				}
			...
			params->options =
			    ...
			    GB_PACKING_CHUNKY | GB_ALPHA_NONE |
			    GB_COLORS_NATIVE | GB_RASTER_STANDARD |
			    (params->data[0] == data ? GB_RETURN_COPY
			                             : GB_RETURN_POINTER);
			goto ret;
		}
	}
	{
		...
		if(row == 0) {
			...
			uint raster = (options & GB_RASTER_SPECIFIED
			                   ? params->raster
			                   : options & GB_ALIGN_STANDARD
			                         ? bitmap_raster(depth * w)
			                         : (depth * w + 7) >> 3);
			gs_int_rect rect;
			...
			gs_get_bits_options_t copy_options =
			    ...
			    GB_PACKING_CHUNKY | GB_COLORS_NATIVE |
			    (options & (GB_DEPTH_ALL | GB_COLORS_ALL)) |
			    GB_ALPHA_ALL;
			...
			for(y = prect->p.y; y < prect->q.y; ++y) {
				...
				params->data[0] =
				    dest + (y - prect->p.y) * raster;
				code = gx_get_bits_copy(
				    dev, copy_params.x_offset, w, 1, params,
				    &copy_params, copy_params.data[0],
				    dev_raster);
				if(code < 0)
					break;
			}
