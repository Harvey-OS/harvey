			  ...
			  const gs_get_bits_params_t *stored)
{
	 ...
	 */
		int i;
		int n = (stored->options & GB_PACKING_BIT_PLANAR ? dev->color_info.depth : dev->color_info.num_components);
			   ...
			   byte *stored_base)
{
	 ...
	 */
		uint dev_raster =
		    (both & GB_PACKING_CHUNKY ? gx_device_raster(dev, true) : both & GB_PACKING_PLANAR ? bitmap_raster(dev->color_info.depth /
														       dev->color_info.num_components * dev->width)
												       : both & GB_PACKING_BIT_PLANAR ? bitmap_raster(dev->width) : 0 /* not possible */);
		uint raster =
		    (options & (GB_RASTER_STANDARD | GB_RASTER_ANY) ? dev_raster : params->raster);
		byte *base;

		if(h <= 1 || raster == dev_raster) {
			int x_offset =
			    (options & GB_OFFSET_ANY ? x : options & GB_OFFSET_0 ? 0 : params->x_offset);
			...
			if(both & GB_PACKING_CHUNKY) {
				...
				int n =
				    (stored->options &GB_PACKING_BIT_PLANAR ? (params->options |= GB_PACKING_BIT_PLANAR,
		 ...
		 const byte *src_base, uint dev_raster)
{
	...
	if(options & GB_PACKING_CHUNKY) {
		...
		uint std_raster =
		    (options & GB_ALIGN_STANDARD ? bitmap_raster(end_bit) : (end_bit + 7) >> 3);
		uint raster =
	 ...
	 */
	     ...
	     */
			...
			for(; h > 0; line_ptr += raster, src += dev_raster, --h) {
				...
				tdev.height = 1;
				(*dev_proc(&mem_mono_device, copy_mono))((gx_device *)&tdev, src, bit_x, dev_raster, gx_no_bitmap_id,
		...
		dest.raster =
		    (options & GB_RASTER_STANDARD ? bitmap_raster((x_offset + w) * dest_depth) : params->raster);
			  ...
			  int x_offset, uint raster)
{
	...
	int ncolors =
	    (stored->options & GB_COLORS_RGB ? 3 : stored->options & GB_COLORS_CMYK ? 4 : stored->options & GB_COLORS_GRAY ? 1 : -1);
	int ncomp = ncolors +
...
#define v2frac(value) ((long)(value)*frac_1 / src_max)
		...
		for(i = 0; i < w; ++i) {
			...
			if(do_alpha) {
				...
				if(ncolors == 1)
					v[2] = v[1] = v[0];
				pixel = dev_proc(dev, map_rgb_alpha_color)(dev, v[0], v[1], v[2], va);
			} else {
			  ...
			  int x_offset, uint raster, uint std_raster)
{
	...
	if(options & GB_RASTER_STANDARD) {
		...
		raster = std_raster =
		    (options & GB_ALIGN_STANDARD ? bitmap_raster(end_byte << 3) : end_byte);
	}
	/* Check for the one special case we care about. */
	if(((options & (GB_COLORS_RGB | GB_ALPHA_FIRST | GB_ALPHA_LAST)) == GB_COLORS_RGB) &&
			      ...
			      gs_get_bits_params_t *params, gs_int_rect **unread)
{
	...
	if(prect->q.y == prect->p.y + 1 &&
	   ...
	   unread == NULL) {
		...
		if(code >= 0) {
			if(row != data) {
				if(prect->p.x == 0 && params->data[0] != row && params->options & GB_RETURN_POINTER) {
					/*
		     ...
		     */
					...
					tdev.line_ptrs = &tdev.base;
					tdev.base = data;
					code = (*dev_proc(&mem_mono_device, copy_mono))((gx_device *)&tdev, row, prect->p.x * depth,
	...
	}
	{
	     ...
	     */
			...
			int nc =
			    (options & GB_COLORS_CMYK ? 4 : options & GB_COLORS_RGB ? 3 : 1) +
		...
		if(row == 0) {
			...
			uint raster =
			    (options & GB_RASTER_SPECIFIED ? params->raster : options & GB_ALIGN_STANDARD ? bitmap_raster(depth * w) : (depth * w + 7) >> 3);
			gs_int_rect rect;
			...
			for(y = prect->p.y; y < prect->q.y; ++y) {
				...
				code = (*save_get_bits_rectangle)(dev, &rect, &copy_params, NULL);
				if(code < 0)
					break;
