...
private
const byte hash_permutation[256] = {NAME_HASH_PERMUTATION_DATA};

/* Define the data for the 1-character names. */
private
const byte nt_1char_names[NT_1CHAR_SIZE] = {NT_1CHAR_NAMES_DATA};
...
gs_private_st_composite(st_name_string_sub_table, name_string_sub_table_t,
                        "name_string_sub_table_t", name_string_sub_enum_ptrs,
                        name_string_sub_reloc_ptrs);
gs_private_st_composite(st_name_table, name_table, "name_table",
                        name_table_enum_ptrs, name_table_reloc_ptrs);

/* Forward references */
private
int name_alloc_sub(name_table*);
private
void name_free_sub(name_table*, uint);
private
void name_scan_sub(name_table*, uint, bool);
...
names_ref(name_table* nt, const byte* ptr, uint size, ref* pref, int enterflag)
{
	...
	for(nidx = *phash; nidx != 0; nidx = name_next_index(nidx, pnstr)) {
                 ...
                 ref* psref /* result, t_string */)
{
	...
	make_const_string(psref,
	                  ...
	                  pnstr->string_size, (const byte*)pnstr->string_bytes);
}
...
names_unmark_all(name_table* nt)
{
	...
	for(si = 0; si < nt->sub_count; ++si)
		if((ssub = nt->sub[si].strings) != 0) {
			...
			/* We can make the test much more efficient if we
			 * want.... */
			for(i = 0; i < nt_sub_size; ++i)
				if(name_index_to_count(
				       (si << nt_log2_sub_size) + i) >=
				   nt->perm_count)
...
names_restore(name_table* nt, alloc_save_t* save)
{
	...
	for(si = 0; si < nt->sub_count; ++si)
		if(nt->sub[si].strings != 0) {
			...
			for(i = 0; i < nt_sub_size; ++i) {
				name_string_t* pnstr =
				    names_index_string_inline(
				        nt, (si << nt_log2_sub_size) + i);

				if(pnstr->string_bytes == 0)
					pnstr->mark = 0;
				else if(pnstr->foreign_string) {
					/* Avoid storing into a read-only name
					 * string. */
					if(!pnstr->mark)
						pnstr->mark = 1;
				} else
...
ENUM_PTRS_END_PROC
private
RELOC_PTRS_WITH(name_table_reloc_ptrs, name_table* nt)
{
...
private
ENUM_PTRS_BEGIN_PROC(name_string_sub_enum_ptrs) { return 0; }
ENUM_PTRS_END_PROC
private
RELOC_PTRS_BEGIN(name_string_sub_reloc_ptrs)
{
