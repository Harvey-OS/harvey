...
private
const int8_t *const encoding_names[] = {
			  ...
			  const pdf_font_resource_t *pdfont)
{
	...
	pprints1(s, "/Subtype/%s>>\n",
		 (pdfont->FontType == ft_TrueType ? "TrueType" : pdfont->u.simple.s.type1.is_MM_instance ? "MMType1" : "Type1"));
	pdf_end_separate(pdev);
	if(diff_id) {
		...
		continue; /* Don't write for unused glyphs. */
			...
			if(cid == prev + 1) {
				if(wmode) {
				  ...
				  (!wmode || (int)(pdfont->u.cidfont.v[cid * 2 + 0] + 0.5) == (int)(pdfont->Widths[cid] / 2 + 0.5)) &&
