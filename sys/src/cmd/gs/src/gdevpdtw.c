...
private
const int8_t* const encoding_names[] = {KNOWN_REAL_ENCODING_NAMES};
...
private
int
pdf_different_encoding_element(const pdf_font_resource_t* pdfont, int ch,
                               int encoding_index)
{
	if(pdfont->u.simple.Encoding[ch].is_difference)
		...
		if(glyph1 != GS_NO_GLYPH)
			if(!strings_equal(&str,
			                  &pdfont->u.simple.Encoding[ch].str))
				return 1;
	}
	return 0;
}
...
pdf_different_encoding_index(const pdf_font_resource_t* pdfont, int ch0)
{
	...
	for(ch = ch0; ch < 256; ++ch) {
		code =
		    pdf_different_encoding_element(pdfont, ch, base_encoding);
		if(code < 0)
			return code; /* Must not happen */
		if(code)
			break;
...
{
	int ch;
	unsigned char mask = (pdfont->FontType == ft_encrypted ||
	                              pdfont->FontType == ft_encrypted2
	                          ? GS_C_PDF_GOOD_GLYPH_MASK
	                          : GS_C_PDF_GOOD_NON_SYMBOL_MASK);

	if(pdfont->u.simple.Encoding == NULL)
		return true; /* Bitmap Type 3 fonts have no
		                pdfont->u.simple.Encoding . */
	for(ch = 0; ch < 256; ++ch) {
...
int
pdf_write_encoding_ref(gx_device_pdf* pdev, const pdf_font_resource_t* pdfont,
                       int32_t id)
{
	stream* s = pdev->strm;
                          ...
                          const pdf_font_resource_t* pdfont)
{
	...
	pprints1(s, "/Subtype/%s>>\n",
	         (pdfont->FontType == ft_TrueType
	              ? "TrueType"
	              : pdfont->u.simple.s.type1.is_MM_instance ? "MMType1"
	                                                        : "Type1"));
	pdf_end_separate(pdev);
	if(diff_id) {
...
private
bool
pdf_compute_CIDFont_default_widths(const pdf_font_resource_t* pdfont, int wmode,
                                   int* pdw, int* pdv)
{
	...
	if(wmode) {
		...
		while(!psf_enumerate_glyphs_next(&genum, &glyph)) {
			...
			if(i < pdfont->count) { /* safety */
				...
				if(min(any_abs(width), countof(counts) - 1) ==
				   any_abs(dwi)) {
					*pdv = (int)(pdfont->u.cidfont
					                 .v[i * 2 + 1] +
					             0.5);
					break;
				}
...
private
int
pdf_write_CIDFont_widths(gx_device_pdf* pdev, const pdf_font_resource_t* pdfont,
                         int wmode)
{
	/*
	...
	/*
	 * Now write all widths different from the default one.  Currently we
	 * make no
	                         ...
	                         pdfont->count, GLYPH_SPACE_INDEX);
	{
		...
		continue; /* Don't write for unused glyphs. */
			...
			{ /* Check whether copied font really have this glyph.
			     ...
			     */
				gs_font_base* pfont =
				    pdf_font_resource_font(pdfont, false);
				gs_glyph_info_t info;

				if(pdfont != NULL) {
					if(pfont->procs.glyph_info(
					       (gs_font*)pfont, glyph, NULL, 0,
					       &info) < 0)
						continue;
				}
			}
			...
			if(cid == prev + 1) {
				if(wmode) {
					int vx = (int)(pdfont->u.cidfont
					                   .v[cid * 2 + 0] +
					               0.5);
					int vy = (int)(pdfont->u.cidfont
					                   .v[cid * 2 + 1] +
					               0.5);
					...
					pprintd1(s, "\n%d", width);
			          ...
			          (!wmode ||
			           (int)(pdfont->u.cidfont.v[cid * 2 + 0] +
			                 0.5) ==
			               (int)(pdfont->Widths[cid] / 2 + 0.5)) &&
			          (!wmode ||
			           (int)(pdfont->u.cidfont.v[cid * 2 + 1] +
			                 0.5) == dv))
				continue;
			else {
				if(prev >= 0)
...
pdf_write_contents_cid2(gx_device_pdf* pdev, pdf_font_resource_t* pdfont)
{
	...
	if(map_id) {
		...
		pdf_begin_data_stream(pdev, &writer,
		                      ...
		                      /* Don't set DATA_STREAM_ENCRYPT since we
...
pdf_write_font_resource(gx_device_pdf* pdev, pdf_font_resource_t* pdfont)
{
	...
	if(pdfont->BaseFont.size > 0) {
		...
		pdf_put_name(pdev, pdfont->BaseFont.data,
	...
	if(pdfont->res_ToUnicode)
		pprintld1(s, "/ToUnicode %ld 0 R",
		          pdf_resource_id(
		              (const pdf_resource_t*)pdfont->res_ToUnicode));
...
write_font_resources(gx_device_pdf* pdev, pdf_resource_list_t* prlist)
{
	...
	for(j = 0; j < NUM_RESOURCE_CHAINS; ++j)
		for(pres = prlist->chains[j]; pres != 0; pres = pres->next) {
			pdf_font_resource_t* const pdfont =
                                               ...
                                               pdf_font_descriptor_t*))
{
	...
	for(j = 0; j < NUM_RESOURCE_CHAINS; ++j)
		for(pres = pdev->resources[resourceFontDescriptor].chains[j];
			...
			int code =
			    finish_proc(pdev, (pdf_font_descriptor_t*)pres);
...
pdf_close_text_document(gx_device_pdf* pdev)
{
	...
	if((code = pdf_free_font_cache(pdev)) < 0 ||
	   ...
	   (code = pdf_finish_font_descriptors(
	        pdev, pdf_finish_FontDescriptor)) < 0 ||
	   (code = write_font_resources(
	        pdev, &pdev->resources[resourceCIDFont])) < 0 ||
	   (code = write_font_resources(pdev, &pdev->resources[resourceFont])) <
	       0 ||
	   (code = pdf_finish_font_descriptors(pdev,
	                                       pdf_write_FontDescriptor)) < 0)
		return code;
                                    ...
                                    gs_id object_id)
{
	...
	if(pdev->KeyLength && object_id != 0) {
		...
		if(code < 0)
			return code;
		s_arcfour_process_buffer(&sarc4, Registry,
		                         pcidsi->Registry.size);
		code = pdf_encrypt_init(pdev, object_id, &sarc4);
		if(code < 0)
			return code;
		s_arcfour_process_buffer(&sarc4, Ordering,
		                         pcidsi->Ordering.size);
	}
	stream_puts(s, "<<\n/Registry");
...
{
	return pdf_write_cid_system_info_to_stream(pdev, pdev->strm, pcidsi,
	                                           object_id);
}
               ...
               pdf_resource_t** ppres /*CMap*/, int font_index_only)
{
	...
	code = pdf_begin_data_stream(
	    pdev, &writer, DATA_STREAM_NOT_BINARY |
	...
	*ppres = writer.pres;
	...
	if(!pcmap->ToUnicode) {
		...
		code = cos_dict_put_c_key_string(pcd, "/CMapName", buf,
		                                 pcmap->CMapName.size + 1);
		if(code < 0)
			return code;
		s_init(&s, pdev->memory);
		swrite_string(&s, buf, sizeof(buf));
		code = pdf_write_cid_system_info_to_stream(
		    pdev, &s, pcmap->CIDSystemInfo, 0);
		if(code < 0)
			return code;
		code = cos_dict_put_c_key_string(pcd, "/CIDSystemInfo", buf,
		                                 stell(&s));
		if(code < 0)
			return code;
