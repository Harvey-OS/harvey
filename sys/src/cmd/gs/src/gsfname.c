...
gs_parse_file_name(gs_parsed_file_name_t* pfn, const char* pname, uint len)
{
	...
	if(len == 0)
		return_error(
		    gs_error_undefinedfilename); /* null name not allowed */
...
int
gs_parse_real_file_name(gs_parsed_file_name_t* pfn, const char* pname, uint len,
...
{
	if(pfn->fname != 0)
		gs_free_const_string(pfn->memory, (const byte*)pfn->fname,
		                     pfn->len, cname);
}
