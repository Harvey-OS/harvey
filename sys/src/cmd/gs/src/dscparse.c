...
#define DSC_START(dsc)                                                         \
	((dsc)->data_offset + (dsc)->data_index - (dsc)->line_length)
...
dsc_private void dsc_section_join(DSC_OFFSET begin, DSC_OFFSET* pend,
                                  DSC_OFFSET** pplast);
...
dsc_private char* dsc_copy_string(char* str, unsigned int slen, char* line,
...
dsc_private int dsc_get_int(const char* line, unsigned int len,
                            unsigned int* offset);
...
dsc_private int dsc_parse_float_bounding_box(CDSC* dsc, CDSCFBBOX** pfbbox,
                                             int offset);
...
dsc_private int dsc_error(CDSC* dsc, unsigned int explanation, char* line,
...
static const char* const dsc_scan_section_name[15] = {
    "Type",     "Comments",   "pre-Preview", "Preview",   "pre-Defaults",
    "Defaults", "pre-Prolog", "Prolog",      "pre-Setup", "Setup",
    "pre-Page", "Page",       "pre-Trailer", "Trailer",   "EOF"};
...
CDSC*
dsc_init_with_alloc(void* caller_data,
...
dsc_scan_data(CDSC* dsc, const char* data, int length)
{
	...
	do {
		...
		if(length != 0) {
			...
			/* append to buffer */
			bytes_read = min(
			    length, (int)(CDSC_DATA_LENGTH - dsc->data_length));
		...
		if(dsc->scan_section == scan_none) {
			...
			if(code == CDSC_NEEDMORE) {
				/* need more characters before we can identify
				 * type */
				code = CDSC_OK;
				break;
			}
...
dsc_fixup(CDSC* dsc)
{
	...
	if(dsc->page_count && (dsc->begintrailer != 0) &&
	   (dsc->page[dsc->page_count - 1].end != dsc->begintrailer)) {
		...
		dsc_debug_print(
		    dsc, "and extending last page to start of trailer\n");
		dsc->page[dsc->page_count - 1].end = dsc->begintrailer;
	}
	...
	for(i = 0; i < dsc->page_count; i++) {
		if(strlen(dsc->page[i].label) == 0) {
			sprintf(buf, "%d", i + 1);
			if((dsc->page[i].label = dsc_alloc_string(
			        dsc, buf, (int)strlen(buf))) == (char*)NULL)
...
dsc_set_error_function(CDSC* dsc,
                       int (*fn)(void* caller_data, CDSC* dsc,
                                 unsigned int explanation, const char* line,
                                 unsigned int line_len))
{
	dsc->dsc_error_fn = fn;
}
...
dsc_add_page(CDSC* dsc, int ordinal, char* label)
{
	...
	if(dsc->page_count >= dsc->page_chunk_length) {
		CDSCPAGE* new_page = (CDSCPAGE*)dsc_memalloc(
		    dsc,
		    (CDSC_PAGE_CHUNK + dsc->page_count) * sizeof(CDSCPAGE));
		if(new_page == NULL)
			return CDSC_ERROR; /* out of memory */
		memcpy(new_page, dsc->page, dsc->page_count * sizeof(CDSCPAGE));
...
dsc_add_media(CDSC* dsc, CDSCMEDIA* media)
{
	...
	/* extend media array  */
	newmedia_array = (CDSCMEDIA**)dsc_memalloc(dsc, (dsc->media_count + 1) *
	                                                    sizeof(CDSCMEDIA*));
...
int
dsc_set_page_bbox(CDSC* dsc, unsigned int page_number, int llx, int lly,
                  int urx, int ury)
{
	CDSCBBOX* bbox;
...
dsc_init2(CDSC* dsc)
{
	...
	dsc->page =
	    (CDSCPAGE*)dsc_memalloc(dsc, CDSC_PAGE_CHUNK * sizeof(CDSCPAGE));
	if(dsc->page == NULL) {
		dsc_free(dsc);
		return NULL; /* no memory */
	}
...
dsc_reset(CDSC* dsc)
{
	...
	for(i = 0; i < dsc->page_count; i++) {
		/* page media is pointer to an element of media or
		 * dsc_known_media */
		/* do not free it. */
...
dsc_read_line(CDSC* dsc)
{
	...
	do {
		...
		for(p = dsc->line; p < last; p++) {
			if(*p == '\r') {
				...
				if((p < last) && (*p == '\n'))
					...
					dsc->last_cr =
					    TRUE; /* we might need to skip \n */
				dsc->eol =
				    TRUE; /* dsc->line is a complete line */
				break;
			}
			if(*p == '\n') {
				p++;
		...
		if(dsc->eol == FALSE) {
			...
			if(dsc->data_length - dsc->data_index <
			   sizeof(dsc->data) / 2) {
				/* buffer is less than half full, ask for some
				 * more */
				dsc->line_length = 0;
				return 0;
			}
	...
	if((dsc->line[0] == '%') && (dsc->line[1] == '%')) {
		...
		if(COMPARE(dsc->line, "%%BeginData:")) {
			 ...
			 * <bytesorlines> ::= Bytes | Lines (Read in bytes or
			 * lines)
			 */
			...
			if((numberof == NULL) || (bytesorlines == NULL)) {
				...
				int rc =
				    dsc_error(dsc, CDSC_MESSAGE_INCORRECT_USAGE,
	...
	if(!dsc->long_line && (dsc->line_length > DSC_LINE_LENGTH)) {
		dsc_error(dsc, CDSC_MESSAGE_LONG_LINE, dsc->line,
		          dsc->line_length);
		dsc->long_line = TRUE;
	}
...
dsc_unknown(CDSC* dsc)
{
	if(dsc->debug_print_fn) {
		...
		unsigned int length =
		    min(DSC_LINE_LENGTH - 1, dsc->line_length);
		sprintf(line, "Unknown in %s section at line %d:\n  ",
		        dsc_scan_section_name[dsc->scan_section],
		        dsc->line_count);
...
dsc_read_doseps(CDSC* dsc)
{
	...
	if(dsc->file_length && (dsc->doseps->ps_begin + dsc->doseps->ps_length >
	                        dsc->file_length)) {
...
{
	unsigned char* line = (unsigned char*)dsc->line;
	if((dsc->macbin = (CDSCMACBIN*)dsc_memalloc(dsc, sizeof(CDSCMACBIN))) ==
	   NULL)
		return CDSC_ERROR; /* no memory */
	...
	if(dsc->file_length && (((dsc->macbin->resource_begin +
	                          dsc->macbin->resource_length + 127) &
	                         ~127) > dsc->file_length)) {
		return CDSC_ERROR;
	}
...
dsc_parse_pages(CDSC* dsc)
{
	...
	if(COMPARE(p, "atend")) {
		...
		if(i) {
			n += i;
			dsc->page_pages = ip;
			io = dsc_get_int(dsc->line + n, dsc->line_length - n,
			                 &i);
			if(i) {
				/* DSC 2 uses extra integer to indicate page
				 * order */
...
dsc_parse_bounding_box(CDSC* dsc, CDSCBBOX** pbbox, int offset)
{
	...
	if(COMPARE(p, "atend")) {
		...
		n += i;
		if(i)
			lly = dsc_get_int(dsc->line + n, dsc->line_length - n,
			                  &i);
		n += i;
		if(i)
			urx = dsc_get_int(dsc->line + n, dsc->line_length - n,
			                  &i);
		n += i;
		if(i)
			ury = dsc_get_int(dsc->line + n, dsc->line_length - n,
			                  &i);
		if(i) {
			*pbbox = (CDSCBBOX*)dsc_memalloc(dsc, sizeof(CDSCBBOX));
			...
			case CDSC_RESPONSE_OK:
				...
				n = offset;
				n += i;
				fllx = dsc_get_real(dsc->line + n,
				                    dsc->line_length - n, &i);
				n += i;
				if(i)
					flly = dsc_get_real(
					    dsc->line + n, dsc->line_length - n,
					    &i);
				n += i;
				if(i)
					furx = dsc_get_real(
...
dsc_parse_float_bounding_box(CDSC* dsc, CDSCFBBOX** pbbox, int offset)
{
	...
	if(COMPARE(p, "atend")) {
		...
		n += i;
		if(i)
			flly = dsc_get_real(dsc->line + n, dsc->line_length - n,
			                    &i);
		n += i;
		if(i)
			furx = dsc_get_real(dsc->line + n, dsc->line_length - n,
			                    &i);
		n += i;
		if(i)
			fury = dsc_get_real(dsc->line + n, dsc->line_length - n,
			                    &i);
		if(i) {
			*pbbox =
			    (CDSCFBBOX*)dsc_memalloc(dsc, sizeof(CDSCFBBOX));
			if(*pbbox == NULL)
				return CDSC_ERROR; /* no memory */
			(*pbbox)->fllx = fllx;
			(*pbbox)->flly = flly;
...
dsc_parse_media(CDSC* dsc, const CDSCMEDIA** page_media)
{
	...
	if(dsc_copy_string(media_name, sizeof(media_name) - 1, dsc->line + n,
	                   dsc->line_length - n, NULL)) {
		for(i = 0; i < dsc->media_count; i++) {
			if(dsc->media[i]->name &&
			   (dsc_stricmp(media_name, dsc->media[i]->name) ==
			    0)) {
...
dsc_parse_document_media(CDSC* dsc)
{
	...
	if(!blank_line) {
		...
		lmedia.name = dsc_copy_string(name, sizeof(name), dsc->line + n,
		                              dsc->line_length - n, &i);
		n += i;
		if(i)
			lmedia.width = dsc_get_real(dsc->line + n,
			                            dsc->line_length - n, &i);
		n += i;
		if(i)
			lmedia.height = dsc_get_real(dsc->line + n,
			                             dsc->line_length - n, &i);
		n += i;
		if(i)
			lmedia.weight = dsc_get_real(dsc->line + n,
			                             dsc->line_length - n, &i);
		n += i;
		if(i)
			lmedia.colour = dsc_copy_string(
			    colour, sizeof(colour), dsc->line + n,
			    dsc->line_length - n, &i);
		n += i;
		if(i)
			lmedia.type =
			    dsc_copy_string(type, sizeof(type), dsc->line + n,
			                    dsc->line_length - n, &i);
...
dsc_scan_type(CDSC* dsc)
{
	...
	if(line[0] == '\033') {
		...
		if(COMPARE(line, "\033%-12345X")) {
			dsc->skip_pjl =
			    TRUE; /* skip until first PostScript comment */
			dsc->pjl = TRUE;
			dsc->data_index += 9;
	...
	if((line[0] == 0x0) && (line[1] >= 1) && (line[1] <= 63) &&
	   (length < 128))
		return CDSC_NEEDMORE; /* Could be Mac Binary EPSF */
	...
	if((line[0] == 0xc5) && (line[1] == 0xd0) && (line[2] == 0xd3) &&
	   (line[3] == 0xc6)) {
		/* id is "EPSF" with bit 7 set */
		/* read DOS EPS header, then ignore all bytes until the PS
		 * section */
		if(length < 30)
			return CDSC_NEEDMORE;
		dsc->line = (char*)line;
		if(dsc_read_doseps(dsc))
			return CDSC_ERROR;
	} else if((line[0] == 0x0) && (line[1] == 0x05) && (line[2] == 0x16) &&
	          ((line[3] == 0x0) || (line[3] == 0x07))) {
	...
	} else if((line[0] == 0x0) && (line[1] >= 1) && (line[1] <= 63) &&
	          (line[74] == 0x0) && (line[65] == 'E') && (line[66] == 'P') &&
	          (line[67] == 'S') && (line[68] == 'F')) {
		/* Mac Binary EPSF */
		dsc->line = (char*)line;
...
dsc_scan_comments(CDSC* dsc)
{
	...
	if(IS_DSC(line, "%%Pages:")) {
		...
		dsc->id = CDSC_CREATOR;
		dsc->dsc_creator =
		    dsc_add_line(dsc, dsc->line + 10, dsc->line_length - 10);
		if(dsc->dsc_creator == NULL)
			return CDSC_ERROR;
	} else if(IS_DSC(line, "%%CreationDate:")) {
		dsc->id = CDSC_CREATIONDATE;
		dsc->dsc_date =
		    dsc_add_line(dsc, dsc->line + 15, dsc->line_length - 15);
		if(dsc->dsc_date == NULL)
			return CDSC_ERROR;
	} else if(IS_DSC(line, "%%Title:")) {
		dsc->id = CDSC_TITLE;
		dsc->dsc_title =
		    dsc_add_line(dsc, dsc->line + 8, dsc->line_length - 8);
		if(dsc->dsc_title == NULL)
			return CDSC_ERROR;
	} else if(IS_DSC(line, "%%For:")) {
		dsc->id = CDSC_FOR;
		dsc->dsc_for =
		    dsc_add_line(dsc, dsc->line + 6, dsc->line_length - 6);
		if(dsc->dsc_for == NULL)
			return CDSC_ERROR;
		...
		if(dsc_parse_bounding_box(dsc, &(dsc->bbox),
		                          continued ? 3 : 14))
			return CDSC_ERROR;
			...
			if(i && p) {
				...
				if(count >= dsc->media_count) {
					...
					dsc->media[count]->name =
					    dsc_alloc_string(dsc, p,
					                     (int)strlen(p));
				/* find in list of known media */
				while(m && m->name) {
					if(dsc_stricmp(p, m->name) == 0) {
						dsc->media[count]->width =
						    m->width;
						dsc->media[count]->height =
						    m->height;
						break;
					}
					m++;
		...
		while(i && (dsc->line[n] != '\r') && (dsc->line[n] != '\n')) {
			w = dsc_get_real(dsc->line + n, dsc->line_length - n,
			                 &i);
			if(i) {
				if(count >= dsc->media_count) {
					/* set some default values */
...
dsc_scan_preview(CDSC* dsc)
{
	...
	if(dsc->scan_section == scan_pre_preview) {
		if(IS_BLANK(line))
			...
			/* Don't mark the preview as EPSI if a DOS EPS header is
			 * present */
...
dsc_check_match_prompt(CDSC* dsc, const char* str, int count)
{
	if(count != 0) {
		...
		sprintf(buf + strlen(buf), "\n%%%%Begin%.40s: / %%%%End%.40s\n",
		        str, str);
		return dsc_error(dsc, CDSC_MESSAGE_BEGIN_END, buf,
		                 (int)strlen(buf));
	}
...
dsc_check_match(CDSC* dsc)
{
	...
	if(!rc)
		rc = dsc_check_match_type(dsc, feature,
		                          dsc->begin_feature_count);
	if(!rc)
		rc = dsc_check_match_type(dsc, resource,
		                          dsc->begin_resource_count);
	if(!rc)
		rc = dsc_check_match_type(dsc, procset,
...
dsc_scan_page(CDSC* dsc)
{
	...
	if(NOT_DSC_LINE(line)) {
		...
		if(IS_DSC(line, "%%Trailer")) {
				...
				if((!dsc->doseps_end &&
				   ...
				   ((dsc->doseps_end) &&
				    ((DSC_END(dsc) + 32768) <
				     dsc->doseps_end))) {
					int rc = dsc_error(
					    dsc, CDSC_MESSAGE_EARLY_TRAILER,
					    dsc->line, dsc->line_length);
					switch(rc) {
					case CDSC_RESPONSE_OK:
			...
			if(dsc->page_count)
				dsc->page[dsc->page_count - 1].end =
				    DSC_START(dsc);
			if(dsc->file_length) {
				if((!dsc->doseps_end && ((DSC_END(dsc) + 100) <
				                         dsc->file_length)) ||
				   ((dsc->doseps_end) &&
				    ((DSC_END(dsc) + 100) < dsc->doseps_end))) {
					int rc = dsc_error(
					    dsc, CDSC_MESSAGE_EARLY_EOF,
					    dsc->line, dsc->line_length);
					switch(rc) {
					case CDSC_RESPONSE_OK:
		...
		if(dsc->page_count)
			dsc_parse_media(
			    dsc, &(dsc->page[dsc->page_count - 1].media));
	} else if(IS_DSC(line, "%%PaperColor:")) {
		dsc->id = CDSC_PAPERCOLOR;
		/* ignore */
	} else if(IS_DSC(line, "%%PaperForm:")) {
		...
		for(i = 0; i < (int)dsc->media_count; i++) {
			if(dsc->media[i] && dsc->media[i]->name &&
			   (dsc_stricmp(buf, dsc->media[i]->name) == 0)) {
				if(dsc->page_count)
					dsc->page[dsc->page_count - 1].media =
					    dsc->media[i];
				found_media = TRUE;
				break;
			}
		...
		if(dsc->page_count) {
			...
			if(dsc_parse_orientation(
			       dsc,
			       &(dsc->page[dsc->page_count - 1].orientation),
			       18))
				return CDSC_NOTDSC;
		}
	} else if(IS_DSC(line, "%%PageBoundingBox:")) {
		if(dsc->page_count) {
			dsc->id = CDSC_PAGEBOUNDINGBOX;
			if(dsc_parse_bounding_box(
			       dsc, &dsc->page[dsc->page_count - 1].bbox, 18))
				return CDSC_NOTDSC;
		}
	} else if(IS_DSC(line, "%%ViewingOrientation:")) {
		if(dsc->page_count) {
			dsc->id = CDSC_VIEWINGORIENTATION;
			if(dsc_parse_viewing_orientation(
			       dsc, &dsc->page[dsc->page_count - 1]
			                 .viewing_orientation))
				return CDSC_ERROR;
		}
	} else if(IS_DSC(line, "%%PageCropBox:")) {
		if(dsc->page_count) {
			dsc->id = CDSC_PAGECROPBOX;
			if(dsc_parse_float_bounding_box(
			       dsc, &(dsc->page[dsc->page_count - 1].crop_box),
			       14))
				return CDSC_ERROR;
		}
	} else if(IS_DSC(line, "%%BeginFont:")) {
...
dsc_scan_trailer(CDSC* dsc)
{
	...
	if(dsc->scan_section == scan_pre_trailer) {
		if(IS_DSC(line, "%%Trailer")) {
			...
			dsc->endtrailer = DSC_END(dsc);
			dsc->scan_section = scan_trailer;
			/* Continue, in case we found %%EOF in an embedded
			 * document */
			return CDSC_OK;
		} else {
			/* %%Page: didn't follow %%EndSetup
	...
	if(NOT_DSC_LINE(line)) {
		 ...
		 */
		int rc = dsc_error(dsc, CDSC_MESSAGE_PAGE_IN_TRAILER, dsc->line,
		                   dsc->line_length);
		switch(rc) {
		case CDSC_RESPONSE_OK:
...
dsc_alloc_string(CDSC* dsc, const char* str, int len)
{
	...
	if(dsc->string_head == NULL) {
		dsc->string_head =
		    (CDSCSTRING*)dsc_memalloc(dsc, sizeof(CDSCSTRING));
		if(dsc->string_head == NULL)
			return NULL; /* no memory */
		dsc->string = dsc->string_head;
		dsc->string->next = NULL;
...
dsc_private char*
dsc_copy_string(char* str, unsigned int slen, char* line, unsigned int len,
                unsigned int* offset)
{
	int quoted = FALSE;
	int instring = 0;
	...
	while(i < len) {
		...
		if(ch == '\n')
			...
			if((ch >= '0') && (ch <= '9')) {
				...
				int j = 3;
				ch = 0;
				while(j && (i < len) && line[i] >= '0' &&
				      line[i] <= '7') {
					ch = (unsigned char)((ch << 3) +
					                     (line[i] - '0'));
					i++;
					j--;
				}
...
dsc_parse_page(CDSC* dsc)
{
	...
	p = dsc->line + 7;
	pl = dsc_copy_string(page_label, sizeof(page_label), p,
	                     dsc->line_length - 7, &i);
	if(pl == NULL)
		return CDSC_ERROR;
	p += i;
...
dsc_private int
dsc_error(CDSC* dsc, unsigned int explanation, char* line,
          unsigned int line_len)
{
	/* if error function provided, use it */
	if(dsc->dsc_error_fn)
		return dsc->dsc_error_fn(dsc->caller_data, dsc, explanation,
		                         line, line_len);
...
dsc_dcs2_fixup(CDSC* dsc)
{
	...
	if(dsc->dcs2) {
		...
		if(dsc->page_count == 0)
			...
			dsc->page[0].label = dsc_alloc_string(
			    dsc, composite, (int)strlen(composite) + 1);
		if(code != CDSC_OK)
			return code;
		...
		if(*pbegin == *pend) {
			/* no page, so force it to conform to the following
			 * sections */
			*pbegin = 999999999;
			*pend = 0;
		}
		...
		while(pdcs) {
			...
			if((pdcs->begin) && (pdcs->colourname != NULL)) {
				/* Single file DCS 2.0 */
				code = dsc_add_page(dsc, page_number + 1,
				                    pdcs->colourname);
				if(code)
					return code;
				...
				if(end != 0)
					    ...
					    pdcs->begin; /* first separation  */
			} else {
				/* Multiple file DCS 2.0 */
				if((pdcs->location != NULL) &&
				   (pdcs->filetype != NULL) &&
				   (pdcs->colourname != NULL) &&
				   (dsc_stricmp(pdcs->location, "Local") ==
				    0) &&
				   ((dsc_stricmp(pdcs->filetype, "EPS") == 0) ||
				    (dsc_stricmp(pdcs->filetype, "EPSF") ==
				     0))) {
					code =
					    dsc_add_page(dsc, page_number + 1,
					                 pdcs->colourname);
					if(code)
						return code;
		...
		if(end != 0)
			*pend = end;
		/* According to the DCS2 specification, the size of the
		 * composite
...
dsc_parse_platefile(CDSC* dsc)
{
	...
	if(!blank_line) {
		dsc_copy_string(colourname, sizeof(colourname), dsc->line + n,
		                dsc->line_length - n, &i);
		n += i;
		if(i)
		...
		if(dsc->line[n] == '#') {
			...
			n++;
			if(i)
				dcs2.begin = dsc_get_int(
				    dsc->line + n, dsc->line_length - n, &i);
			n += i;
			if(i)
				dcs2.end =
				    dcs2.begin +
				    dsc_get_int(dsc->line + n,
				                dsc->line_length - n, &i);
		} else {
			/* multiple file DCS 2.0 */
			if(i)
		...
		if(i == 0)
			...
			if(strlen(colourname))
				dcs2.colourname = dsc_alloc_string(
				    dsc, colourname, (int)strlen(colourname));
			if(strlen(filetype))
				dcs2.filetype = dsc_alloc_string(
				    dsc, filetype, (int)strlen(filetype));
			if(strlen(location))
				dcs2.location = dsc_alloc_string(
				    dsc, location, (int)strlen(location));
			if(filename)
				dcs2.filename = dsc_add_line(dsc, filename,
				                             filename_length);

			/* Prevent parser from reading separations */
			if(single)
				dsc->file_length =
				    min(dsc->file_length, dcs2.begin);
			/* Allocate it */
...
dsc_parse_dcs1plate(CDSC* dsc)
{
	...
	if(!blank_line) {
		dsc_copy_string(filename, sizeof(filename), dsc->line + n,
		                dsc->line_length - n, &i);
		if(i == 0)
			dsc_unknown(dsc); /* we didn't get all fields */
		else {
			...
			if(strlen(filename))
				dcs2.filename = dsc_alloc_string(
				    dsc, filename, (int)strlen(filename));
			/* Allocate it */
			pdcs2 = (CDCS2*)dsc_memalloc(dsc, sizeof(CDCS2));
			if(pdcs2 == NULL)
				return CDSC_ERROR; /* out of memory */
	...
	if(!blank_line) {
		do {
			...
			if(i && strlen(colourname)) {
				if((pcolour = dsc_find_colour(
				        dsc, colourname)) == NULL) {
					pcolour = (CDSCCOLOUR*)dsc_memalloc(
					    dsc, sizeof(CDSCCOLOUR));
					if(pcolour == NULL)
						return CDSC_ERROR; /* out of
						                      memory */
					memset(pcolour, 0, sizeof(CDSCCOLOUR));
					pcolour->custom =
					    CDSC_CUSTOM_COLOUR_UNKNOWN;
					pcolour->name = dsc_alloc_string(
					    dsc, colourname,
					    (int)strlen(colourname));
					if(dsc->colours == NULL)
						dsc->colours = pcolour;
					else {
				...
				if(dsc_stricmp(colourname, "Cyan") == 0) {
					pcolour->custom =
					    CDSC_CUSTOM_COLOUR_CMYK;
					pcolour->cyan = 1.0;
					pcolour->magenta = pcolour->yellow =
					    pcolour->black = 0.0;
				} else if(dsc_stricmp(colourname, "Magenta") ==
				          0) {
					pcolour->custom =
					    CDSC_CUSTOM_COLOUR_CMYK;
					pcolour->magenta = 1.0;
					pcolour->cyan = pcolour->yellow =
					    pcolour->black = 0.0;
				} else if(dsc_stricmp(colourname, "Yellow") ==
				          0) {
					pcolour->custom =
					    CDSC_CUSTOM_COLOUR_CMYK;
					pcolour->yellow = 1.0;
					pcolour->cyan = pcolour->magenta =
					    pcolour->black = 0.0;
				} else if(dsc_stricmp(colourname, "Black") ==
				          0) {
					pcolour->custom =
					    CDSC_CUSTOM_COLOUR_CMYK;
					pcolour->black = 1.0;
					...
					pcolour->custom =
					    CDSC_CUSTOM_COLOUR_RGB;
					pcolour->red = 1.0;
					pcolour->green = pcolour->blue = 0.0;
				} else if(dsc_stricmp(colourname, "Green") ==
				          0) {
					pcolour->custom =
					    CDSC_CUSTOM_COLOUR_RGB;
					pcolour->green = 1.0;
					pcolour->red = pcolour->blue = 0.0;
				} else if(dsc_stricmp(colourname, "Blue") ==
				          0) {
					pcolour->custom =
					    CDSC_CUSTOM_COLOUR_RGB;
					pcolour->blue = 1.0;
...
dsc_parse_cmyk_custom_colour(CDSC* dsc)
{
	...
	do {
		...
		if(blank_line)
			...
			cyan = dsc_get_real(dsc->line + n, dsc->line_length - n,
			                    &i);
			n += i;
			if(i)
				magenta = dsc_get_real(
				    dsc->line + n, dsc->line_length - n, &i);
			n += i;
			if(i)
				yellow = dsc_get_real(dsc->line + n,
				                      dsc->line_length - n, &i);
			n += i;
			if(i)
				black = dsc_get_real(dsc->line + n,
				                     dsc->line_length - n, &i);
			n += i;
			if(i)
				dsc_copy_string(colourname, sizeof(colourname),
...
dsc_parse_rgb_custom_colour(CDSC* dsc)
{
	...
	do {
		...
		if(blank_line)
			...
			red = green = blue = 0.0;
			red = dsc_get_real(dsc->line + n, dsc->line_length - n,
			                   &i);
			n += i;
			if(i)
				green = dsc_get_real(dsc->line + n,
				                     dsc->line_length - n, &i);
			n += i;
			if(i)
				blue = dsc_get_real(dsc->line + n,
				                    dsc->line_length - n, &i);
			n += i;
			if(i)
				dsc_copy_string(colourname, sizeof(colourname),
