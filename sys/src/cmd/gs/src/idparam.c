...
/* Copyright (C) 1992, 1995, 1997, 1998 Aladdin Enterprises.  All rights
  reserved.
...
/* $Id: idparam.c,v 1.9 2004/08/04 19:36:12 stefan Exp $ */
...
int
dict_bool_param(const ref* pdict, const char* kstr, bool defaultval,
                bool* pvalue)
{
	ref* pdval;
...
int
dict_int_null_param(const ref* pdict, const char* kstr, int minval, int maxval,
                    int defaultval, int* pvalue)
{
	...
	if(pdict == 0 || dict_find_string(pdict, kstr, &pdval) <= 0) {
		...
		case t_real:
			...
			/* needs this. */
			if(pdval->value.realval < minval ||
			   pdval->value.realval > maxval)
...
dict_int_param(const ref* pdict, const char* kstr, int minval, int maxval,
               int defaultval, int* pvalue)
{
	int code = dict_int_null_param(pdict, kstr, minval, maxval, defaultval,
	                               pvalue);
...
/* a missing value will return e_rangecheck rather than 1. */
int
dict_uint_param(const ref* pdict, const char* kstr, uint minval, uint maxval,
                uint defaultval, uint* pvalue)
{
	ref* pdval;
	int code;
	uint ival;
...
/* Return 0 if found, 1 if defaulted, <0 if wrong type. */
int
dict_float_param(const ref* pdict, const char* kstr, floatp defaultval,
                 float* pvalue)
{
	ref* pdval;
...
int
dict_int_array_param(const ref* pdict, const char* kstr, uint maxlen, int* ivec)
{
	return dict_int_array_check_param(pdict, kstr, maxlen, ivec, 0,
	                                  e_limitcheck);
}
int
dict_ints_param(const ref* pdict, const char* kstr, uint len, int* ivec)
{
	return dict_int_array_check_param(pdict, kstr, len, ivec, e_rangecheck,
	                                  e_rangecheck);
}
...
/* and return maxlen. */
int
dict_float_array_check_param(const gs_memory_t* mem, const ref* pdict,
                             const char* kstr, uint len, float* fvec,
                             const float* defaultvec, int under_error,
                             int over_error)
{
	ref* pdval;
	uint size;
	...
	return (code < 0 ? code : size == len || under_error >= 0
	                              ? size
	                              : gs_note_error(under_error));
}
int
dict_float_array_param(const gs_memory_t* mem, const ref* pdict,
                       const char* kstr, uint maxlen, float* fvec,
 ...
 */
int
dict_proc_param(const ref* pdict, const char* kstr, ref* pproc, bool defaultval)
{
	ref* pdval;

	if(pdict == 0 || dict_find_string(pdict, kstr, &pdval) <= 0) {
		if(defaultval)
			make_empty_const_array(pproc,
			                       a_readonly + a_executable);
		else
...
int
dict_matrix_param(const gs_memory_t* mem, const ref* pdict, const char* kstr,
                  gs_matrix* pmat)
{
	ref* pdval;
...
int
dict_uid_param(const ref* pdict, gs_uid* puid, int defaultval, gs_memory_t* mem,
               const i_ctx_t* i_ctx_p)
{
	ref* puniqueid;
	...
	if(level2_enabled && dict_find_string(pdict, "XUID", &puniqueid) > 0) {
		int32_t* xvalues;
		uint size, i;
		...
		if(size == 0)
			return_error(e_rangecheck);
		xvalues = (int32_t*)gs_alloc_byte_array(
		    mem, size, sizeof(int32_t), "get XUID");
...
dict_check_uid_param(const ref* pdict, const gs_uid* puid)
{
	...
	if(uid_is_XUID(puid)) {
		...
		if(dict_find_string(pdict, "XUID", &puniqueid) <= 0)
			return false;
		if(!r_has_type(puniqueid, t_array) || r_size(puniqueid) != size)
			return false;
		for(i = 0; i < size; i++) {
			const ref* pvalue = puniqueid->value.const_refs + i;
