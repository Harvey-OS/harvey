...
s_write_ps_string(stream* s, const byte* str, uint size, int print_ok)
{
	...
	if(added < size || (print_ok & PRINT_HEX_NOT_OK)) {
		/* More efficient, or mandatory, to represent as PostScript
		 * string. */
...

	{
		...
		do {
			/* One picky compiler complains if we initialize to buf
			 * - 1. */
			w.ptr = buf;
			w.ptr--;
...
private
param_proc_xmit_typed(param_print_typed);
...
private
const gs_param_list_procs printer_param_list_procs = {
    param_print_typed,         NULL /* begin_collection */,
...
int
s_alloc_param_printer(gs_param_list** pplist, const param_printer_params_t* ppp,
                      stream* s, gs_memory_t* mem)
{
...
{
	if(plist) {
		printer_param_list_t* const prlist =
		    (printer_param_list_t*)plist;
