...
private
int cmd_put_path(gx_device_clist_writer* cldev, gx_clist_state* pcls,
                 const gx_path* ppath, fixed ymin, fixed ymax, byte op,
                             ...
                             const gx_drawing_color* pdcolor)
{
	 ...
	 * the plane mask.
	 */
	return ((pdcolor->colors.colored.c_base[0] << 3) |
             ...
             const gx_drawing_color* pdcolor)
{
	...
	return !(rop == rop3_0 || rop == rop3_1 || rop == rop3_D ||
                      ...
                      const gx_drawing_color* pdcolor)
{
	const gx_device_halftone* pdht =
	    pdcolor->type->get_dev_halftone(pdcolor);
	int code, di;
	...
	if(pdht != NULL && pdht->id != cldev->device_halftone_id) {
	...
	if(pdcolor->type->get_phase(pdcolor, &color_phase) &&
	   ...
	   (code = cmd_set_tile_phase(cldev, pcls, color_phase.x,
	                              color_phase.y)) < 0)
		return code;
	...
	code =
	    pdcolor->type->write(pdcolor, psdc, (gx_device*)cldev, 0, &dc_size);
	 ...
	 */
	if((code = set_cmd_put_op(dp, cldev, pcls, cmd_opv_extend, req_size)) <
	   0)
		return code;
	dp0 = dp;
	...
	*dp++ = di;
	...
	code = pdcolor->type->write(pdcolor, &pcls->sdc, (gx_device*)cldev, dp,
	                            &dc_size);
	if(code < 0) {
		cldev->cnext = dp0;
		return code;
	...
	pcls->colors_used.or
	    = ((gx_color_index)1 << cldev->color_info.depth) - 1;
                        ...
                        const gx_drawing_color* pdcolor)
{
	if(gx_dc_is_pure(pdcolor))
		...
		return gx_dc_binary_color0(pdcolor) |
 ...
 */
#define FILL_KNOWN                                                             \
	(cj_ac_sa_known | flatness_known | op_bm_tk_known |                    \
	 opacity_alpha_known | shape_alpha_known | fill_adjust_known |         \
                  ...
                  uint must_know)
{
	...
	if(misc2_unknown) {
		...
		if(unknown & cj_ac_sa_known) {
			*bp++ =
			    ((cldev->imager_state.line_params.curve_join + 1)
			     << 2) +
			    (cldev->imager_state.accurate_curves ? 2 : 0) +
			    (cldev->imager_state.stroke_adjust ? 1 : 0);
		}
		if(unknown & flatness_known) {
			memcpy(bp, &cldev->imager_state.flatness,
			       sizeof(float));
			bp += sizeof(float);
		}
		if(unknown & line_width_known) {
			float width = gx_current_line_width(
			    &cldev->imager_state.line_params);
		...
		if(unknown & op_bm_tk_known) {
			*bp++ = ((int)cldev->imager_state.blend_mode << 3) +
		...
		if(unknown & opacity_alpha_known) {
			memcpy(bp, &cldev->imager_state.opacity.alpha,
			       sizeof(float));
			bp += sizeof(float);
		}
		if(unknown & shape_alpha_known) {
			memcpy(bp, &cldev->imager_state.shape.alpha,
			       sizeof(float));
			bp += sizeof(float);
		}
	...
	if(unknown & fill_adjust_known) {
		...
		if(code < 0)
			return code;
		memcpy(dp + 1, &cldev->imager_state.fill_adjust.x,
		       sizeof(fixed));
		memcpy(dp + 1 + sizeof(fixed),
		       &cldev->imager_state.fill_adjust.y, sizeof(fixed));
		pcls->known |= fill_adjust_known;
	}
	if(unknown & ctm_known) {
		int len = cmd_write_ctm_return_length(
		    cldev, &ctm_only(&cldev->imager_state));
	...
	if(unknown & dash_known) {
		...
		dp[1] =
		    n +
		    (cldev->imager_state.line_params.dash.adapt ? 0x80 : 0) +
		    (cldev->imager_state.line_params.dot_length_absolute ? 0x40
		                                                         : 0);
		...
		memcpy(dp + 2 + sizeof(float),
		       ...
		       sizeof(float));
		if(n != 0)
			memcpy(dp + 2 + sizeof(float) * 2, cldev->dash_pattern,
			       n * sizeof(float));
	...
	if(unknown & clip_path_known) {
		...
		if(pcpath->path_valid) {
			if(gx_path_is_rectangle(&pcpath->path, &box) &&
			   fixed_is_int(box.p.x | box.p.y | box.q.x |
			                box.q.y)) {
				/* Write the path as a rectangle. */
				code = cmd_write_rect_cmd(
				    cldev, pcls, cmd_op_fill_rect,
				    fixed2int_var(box.p.x),
				    fixed2int_var(box.p.y),
				    fixed2int(box.q.x - box.p.x),
				    fixed2int(box.q.y - box.p.y));
			} else if(!(cldev->disable_mask &
			            clist_disable_complex_clip)) {
				/* Write the path. */
				code = cmd_put_path(
				    cldev, pcls, &pcpath->path,
				    int2fixed(ymin - 1), int2fixed(ymax + 1),
				    (byte)(pcpath->rule == gx_rule_even_odd
				               ? cmd_opv_eofill
				               : cmd_opv_fill),
				    true, sn_not_first);
			} else {
				/* Complex paths disabled: write outer box as
				 * clip */
		...
		if(punt_to_outer_box) {
			/* Clip is complex, but disabled. Write out the outer
			 * box */
			gs_fixed_rect box;
			...
			code = cmd_write_rect_cmd(
			    cldev, pcls, cmd_op_fill_rect,
			    fixed2int_var(box.p.x), fixed2int_var(box.p.y),
			    fixed2int_ceiling(box.q.x - box.p.x),
			    fixed2int_ceiling(box.q.y - box.p.y));
		}
		{
			int end_code = set_cmd_put_op(dp, cldev, pcls,
			                              cmd_opv_end_clip, 1);

			if(code >= 0)
				code =
				    end_code; /* take the first failure seen */
			if(end_code < 0 && cldev->error_is_retryable) {
				/*
				 * end_clip has to work despite lo-mem to
				 * maintain consistency.
				 * This isn't error recovery, but just to
				 * prevent dangling
				 * cmd_opv_begin_clip's.
				 */
				++cldev->ignore_lo_mem_warnings;
				end_code = set_cmd_put_op(dp, cldev, pcls,
				                          cmd_opv_end_clip, 1);
				--cldev->ignore_lo_mem_warnings;
			}
		}
	...
	if(unknown & color_space_known) {
		...
		if(cldev->color_space.byte1 & 8) { /* indexed */
			...
			uint num_values =
			    (hival + 1) * gs_color_space_num_components(
			                      (const gs_color_space*)&pcs
			                          ->params.indexed.base_space);
			...
			if(use_proc) {
				map_data =
				    pcs->params.indexed.lookup.map->values;
				map_size = num_values *
				           sizeof(pcs->params.indexed.lookup.map
				                      ->values[0]);
			} else {
				map_data =
                ...
                const gx_clip_path* pcpath)
{
	...
	byte op = (byte)(params->rule == gx_rule_even_odd ? cmd_opv_eofill
	                                                  : cmd_opv_fill);
	gs_fixed_point adjust;
	bool slow_rop = cmd_slow_rop(dev, lop_know_S_0(lop), pdcolor);

	if((cdev->disable_mask & clist_disable_fill_path) || gs_debug_c(',')) {
	...
	FOR_RECTS_NO_ERROR
	{
		...
		if(code < 0)
			return code;
		if((code = cmd_do_enable_clip(cdev, pcls, pcpath != NULL)) <
		       0 ||
		...
		code =
		    cmd_put_path(cdev, pcls, ppath, int2fixed(max(y - 1, y0)),
		                 int2fixed(min(y + height + 1, y1)), op, true,
		                 sn_none /* fill doesn't need the notes */);
		if(code < 0)
			return code;
	}
	END_RECTS_NO_ERROR;
                  ...
                  const gx_drawing_color* pdcolor, const gx_clip_path* pcpath)
{
	...
	if(cdev->imager_state.line_params.dash.pattern_size != pattern_size ||
	   ...
	   cdev->imager_state.line_params.dot_length_absolute !=
		 ...
		 */
		cdev->imager_state.line_params.dash.pattern =
		    cdev->dash_pattern;
	...
	if(state_neq(ctm.xx) || state_neq(ctm.xy) || state_neq(ctm.yx) ||
	   state_neq(ctm.yy) ||
	...
	FOR_RECTS_NO_ERROR
	{
		...
		if((code = cmd_do_write_unknown(cdev, pcls, stroke_all_known)) <
		       0 ||
		...
		pcls->colors_used.slow_rop |= slow_rop;
		{
			 ...
			 * outside the clipping region, because that would throw
			 * off
			 * the pattern.
                   ...
                   const gx_drawing_color* pdcolor, gs_logical_operation_t lop)
{
	...
	FOR_RECTS_NO_ERROR
	{
		...
		pcls->colors_used.slow_rop |= slow_rop;
		code = cmd_put_path(
		    cdev, pcls, &path, int2fixed(max(y - 1, y0)),
		    int2fixed(min(y + height + 1, y1)), cmd_opv_polyfill, true,
		    sn_none /* fill doesn't need the notes */);
		if(code < 0)
			goto out;
	}
...
int
clist_fill_parallelogram(gx_device* dev, fixed px, fixed py, fixed ax, fixed ay,
                         ...
                         gs_logical_operation_t lop)
{
	...
	return (code >= 0 ? code : gx_default_fill_parallelogram(dev, px, py,
	                                                         ax, ay, bx, by,
	                                                         pdcolor, lop));
}

int
clist_fill_triangle(gx_device* dev, fixed px, fixed py, fixed ax, fixed ay,
                    fixed bx, fixed by, const gx_drawing_color* pdcolor,
                    gs_logical_operation_t lop)
{
	...
	return (code >= 0 ? code
	                  : gx_default_fill_triangle(dev, px, py, ax, ay, bx,
	                                             by, pdcolor, lop));
}
...
private
int
cmd_put_segment(cmd_segment_writer* psw, byte op, const fixed* operands,
                segment_notes notes)
{
	...
	if(op == cmd_opv_rlineto) {
		if(operands[0] == 0)
			...
			case cmd_opv_rmoveto:
				...
				op = cmd_opv_rmlineto;
			merge:
				cmd_uncount_op(*psw->dp, psw->len);
				cmd_shorten_op(psw->cldev, psw->pcls,
				               psw->len); /* delete it */
				q += psw->len - 1;
				break;
			...
			case cmd_opv_rm2lineto:
				...
				if(operands[0] == -psw->delta_first.x &&
				   operands[1] == -psw->delta_first.y) {
					cmd_uncount_op(cmd_opv_rm2lineto,
					               psw->len);
					*psw->dp = cmd_count_op(
					    cmd_opv_rm3lineto, psw->len);
					return 0;
				}
				break;
			default:
	...
	for(; --i >= 0; ++optr) {
		...
		if(is_bits(d, _fixed_shift + 11) &&
		   !(d & (float2fixed(0.25) - 1))) {
			...
			q += 2;
		} else if(is_bits(d, 19) && i > 0 &&
		          is_bits(d2 = optr[1], 19)) {
	...
	if(notes != psw->notes) {
		byte* dp;
		int code = set_cmd_put_op(dp, psw->cldev, psw->pcls,
		                          cmd_opv_set_misc2, 3);
...
#define which_side(y) ((y) < ymin ? -1 : (y) >= ymax ? 1 : 0)
	...
	segment_notes out_notes =
	    0x7badf00d; /* Initialize against indeterminizm. */
	...
	if_debug4('p', "[p]initial (%g,%g), clip [%g..%g)\n", fixed2float(px),
	          fixed2float(py), fixed2float(ymin), fixed2float(ymax));
...
#define F vs[5]
		...
		case gs_pe_moveto:
			...
			if((start_side = side = which_side(B)) != 0) {
				out.x = A, out.y = B;
				if_debug3('p',
				          "[p]skip moveto (%g,%g) side %d\n",
				          fixed2float(out.x),
				          fixed2float(out.y), side);
				continue;
			}
			C = A - px, D = B - py;
		...
		case gs_pe_lineto: {
			...
			if(next_side == side &&
			   side != 0) { /* Skip a line completely outside the
			                   clip region. */
				if(open < 0)
					start_skip = true;
				out.x = A, out.y = B;
			...
			if(side &&
			   (px != out.x || py != out.y || first_point())) {
				...
				px = out.x, py = out.y;
				if_debug3('p',
				          "[p]catchup %s (%g,%g) for line\n",
				          (open < 0 ? "moveto" : "lineto"),
				          fixed2float(px), fixed2float(py));
			}
			if((side = next_side) != 0) { /* Note a vertex going
			                                 outside the clip
			                                 region. */
				out.x = A, out.y = B;
			}
...
#ifdef DEBUG
		{
			...
			case gs_pe_moveto:
				break;
			default:
				lprintf1("closepath followed by %d, not "
				         "end/moveto!\n",
				         op);
			}
		}
		...
		close:
			if(side != start_side) { /* If we skipped any segments,
			                            put out a moveto/lineto. */
				if(side && (px != out.x || py != out.y ||
				            first_point())) {
					C = out.x - px, D = out.y - py;
					code = cmd_put_rlineto(&writer, &C,
					                       out_notes);
					if(code < 0)
						return code;
					px = out.x, py = out.y;
					if_debug2('p', "[p]catchup line "
					               "(%g,%g) for close\n",
					          fixed2float(px),
					          fixed2float(py));
				}
				if(open > 0 &&
				   start_skip) { /* Draw the closing line back
				                    to the start. */
					...
					if_debug2(
					    'p', "[p]draw close to (%g,%g)\n",
					    fixed2float(px), fixed2float(py));
				}
			}
			/*
			 * We don't bother to update side because we know that
			 * the
			 * next element after a closepath, if any, must be a
			 * moveto.
			 * We must handle explicitly the possibility that the
			 * entire
			 * subpath was skipped.
			 */
			if(implicit_close || open <= 0) {
				/*
				 * Force writing an explicit moveto if the next
				 * subpath
				 * starts with a moveto to the same point where
				 * this one
				 * ends.
				 */
				set_first_point();
		...
		case gs_pe_curveto: {
...

			{
				...
				if(F < bpy)
					...
					bqy = F;
				all_side =
				    (bqy < ymin ? -1 : bpy > ymax ? 1 : 0);
				if(all_side != 0) {
					if(all_side ==
					   side) { /* Skip a curve entirely
				...
				/* If we skipped any segments, put out a
				 * moveto/lineto. */
				if(side && (px != out.x || py != out.y ||
				            first_point())) {
					fixed diff[2];
					...
					if(open < 0) {
						first = out;
						code = cmd_put_rmoveto(&writer,
						                       diff);
					} else
						code = cmd_put_rlineto(
						    &writer, diff, out_notes);
					if(code < 0)
						return code;
					px = out.x, py = out.y;
					if_debug3(
				...
				if((side = out_side) !=
				   0) { /* Note a vertex going outside the clip
				           region. */
					out.x = E, out.y = F;
				}
			}
			{
				...
				open = 1;
				code =
				    cmd_put_segment(&writer, op, optr, notes);
			}
		} break;
		default:
