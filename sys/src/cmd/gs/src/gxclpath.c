...
private
int cmd_put_path(gx_device_clist_writer *cldev,
			...
			if(gx_path_is_rectangle(&pcpath->path, &box) &&
			   fixed_is_int(box.p.x | box.p.y | box.q.x | box.q.y)) {
				...
				code = cmd_put_path(cldev, pcls, &pcpath->path,
						    int2fixed(ymin - 1),
						    int2fixed(ymax + 1),
						    (byte)(pcpath->rule == gx_rule_even_odd ? cmd_opv_eofill : cmd_opv_fill),
						    true, sn_not_first);
			} else {
		...
		const gx_clip_path *pcpath)
{
	...
	byte op = (byte)(params->rule == gx_rule_even_odd ? cmd_opv_eofill : cmd_opv_fill);
			 ...
			 gs_logical_operation_t lop)
{
	...
	return (code >= 0 ? code : gx_default_fill_parallelogram(dev, px, py, ax, ay, bx, by,
								 pdcolor, lop));
}
		    ...
		    gs_logical_operation_t lop)
{
	...
	return (code >= 0 ? code : gx_default_fill_triangle(dev, px, py, ax, ay, bx, by,
							    pdcolor, lop));
}
...
#define F vs[5]
		...
		close:
			if(side != start_side) { /* If we skipped any segments, put out a moveto/lineto. */
				if(side && (px != out.x || py != out.y || first_point())) {
					C = out.x - px, D = out.y - py;
					code = cmd_put_rlineto(&writer, &C, out_notes);
					if(code < 0)
