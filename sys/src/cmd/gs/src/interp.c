...
again:
	...
	case e_Quit:
		...
		*pexit_code = code = osp->value.intval;
		osp -= 2;
		return (code == 0 ? e_Quit : code < 0 && code > -100 ? code : e_Fatal);
...
top:
	...
	if(gs_debug['I'] ||
	   (gs_debug['i'] &&
	    (r_is_packed(iref_packed) ? r_packed_is_name(iref_packed) : r_has_type(IREF, t_name)))) {
...
#define cases_lit_array()        \
	...
	case exec(t_shortarray)
		cases_lit_1() : cases_lit_2() : cases_lit_3() : cases_lit_4() : cases_lit_5() : INCR(lit);
		break;
	...
	x_if:
		...
		if(!r_has_type(iosp - 1, t_boolean))
			return_with_error_iref((iosp <= osbot ? e_stackunderflow : e_typecheck));
		if(!r_is_proc(iosp))
	...
	x_ifelse:
		...
		if(!r_has_type(iosp - 2, t_boolean))
			return_with_error_iref((iosp < osbot + 2 ? e_stackunderflow : e_typecheck));
		if(!r_is_proc(iosp - 1))
	...
	case plain_exec(t_name):
		...
		switch(r_type_xe(pvalue)) {
			...
			cases_lit_1() : cases_lit_2() : cases_lit_3() : cases_lit_4() : cases_lit_5() : INCR(name_lit);
			/* Just push the value */
			if(iosp >= ostop)
	...
	again:
		...
		case 0: /* read a token */
			...
			if(!r_has_attr(&token, a_executable) ||
			   r_is_array(&token)) { /* If scan_token used the o-stack, */
	...
	case exec(t_string): { /* Executable string.  Read a token and interpret it. */
		stream ss;
		scanner_state sstate;
	...
	default: {
		...
		case pt_executable_operator:
			...
			if(!op_index_is_operator(index)) {
				...
				index -= op_def_count;
				pvalue = (const ref *)(index < r_size(&op_array_table_global.table) ? op_array_table_global.table.value.const_refs + index : op_array_table_local.table.value.const_refs +
