...
struct stats_interp_s {
	...
	int32_t x_add, x_def, x_dup, x_exch, x_if, x_ifelse, x_index, x_pop,
	    x_roll, x_sub;
...
/* Forward references */
private
int estack_underflow(i_ctx_t*);
private
int interp(i_ctx_t**, const ref*, ref*);
private
int interp_exit(i_ctx_t*);
private
void set_gc_signal(i_ctx_t*, int*, int);
private
int copy_stack(i_ctx_t*, const ref_stack_t*, ref*);
private
int oparray_pop(i_ctx_t*);
private
int oparray_cleanup(i_ctx_t*);
private
int zsetstackprotect(i_ctx_t*);
private
int zcurrentstackprotect(i_ctx_t*);
...
#define DS_REFS_SIZE(body_size) (stack_block_refs + (body_size))
...
#define REFS_SIZE_DSTACK DS_REFS_SIZE(MAX_DSTACK)
	gs_alloc_ref_array(smem, &stk, 0, REFS_SIZE_OSTACK + REFS_SIZE_ESTACK +
	                                      REFS_SIZE_DSTACK,
	                   "gs_interp_alloc_stacks");

	{
		...
		ref_stack_init(pos, &stk, OS_GUARD_UNDER, OS_GUARD_OVER, NULL,
		               smem, NULL);
		ref_stack_set_error_codes(pos, e_stackunderflow,
		                          e_stackoverflow);
...
gs_interp_make_oper(ref* opref, op_proc_t proc, int idx)
{
	...
	for(i = num_special_ops; i > 0 && proc != interp_op_defs[i].proc; --i)
		DO_NOTHING;
	if(i > 0)
		make_tasv(opref, tx_op + (i - 1), a_executable, i, opproc,
		          proc);
	else
...
interp_reclaim(i_ctx_t** pi_ctx_p, int space)
{
	...
	gs_register_struct_root(imemory_system, &ctx_root, (void**)pi_ctx_p,
	                        "interp_reclaim(pi_ctx_p)");
	code = (*idmemory->reclaim)(idmemory, space);
	i_ctx_p = *pi_ctx_p; /* may have moved */
	gs_unregister_root(imemory_system, &ctx_root,
	                   "interp_reclaim(pi_ctx_p)");
	return code;
}
 ...
 */
private
int gs_call_interp(i_ctx_t**, ref*, int, int*, ref*);
int
...
private
int
gs_call_interp(i_ctx_t** pi_ctx_p, ref* pref, int user_errors, int* pexit_code,
               ref* perror_object)
{
	ref* epref = pref;
	ref doref;
...
again:
	...
	case e_Quit:
		...
		*pexit_code = code = osp->value.intval;
		osp -= 2;
		return (code == 0 ? e_Quit : code < 0 && code > -100 ? code
		                                                     : e_Fatal);
	...
	case e_VMreclaim:
		...
		code = interp_reclaim(
		    pi_ctx_p,
		    (osp->value.intval == 2 ? avm_global : avm_local));
		i_ctx_p = *pi_ctx_p;
		/****** What if code < 0? ******/
	...
	case e_execstackoverflow:
			...
			return ccode;
		{
			...
			uint limit =
			    ref_stack_max_count(&e_stack) - ES_HEADROOM;

			if(count > limit) {
				/*
				 * If there is an e-stack mark within
				 * MIN_BLOCK_ESTACK of
				 * the new top, cut the stack back to remove the
				 * mark.
				 */
				int skip = count - limit;
				int i;

				for(i = skip; i < skip + MIN_BLOCK_ESTACK;
				    ++i) {
					const ref* ep =
					    ref_stack_index(&e_stack, i);

					if(r_has_type_attrs(ep, t_null,
					                    a_executable)) {
						skip = i + 1;
						break;
					}
	...
	case e_stackoverflow:
		if(ref_stack_extend(&o_stack, o_stack.requested) >=
		   0) { /* We can't just re-execute the object, because */
...
set_gc_signal(i_ctx_t* i_ctx_p, int* psignal, int value)
{
	...
	for(i = 0; i < countof(idmemory->spaces_indexed); i++) {
		...
		if(mem == 0)
			continue;
		for(;; mem = mem_stable) {
			mem_stable = (gs_ref_memory_t*)gs_memory_stable(
			    (gs_memory_t*)mem);
...
gs_errorname(i_ctx_t* i_ctx_p, int code, ref* perror_name)
{
	...
	if(dict_find_string(systemdict, "errordict", &perrordict) <= 0 ||
	   dict_find_string(systemdict, "ErrorNames", &pErrorNames) <= 0)
		return_error(
		    e_undefined); /* errordict or ErrorNames not found?! */
	return array_get(imemory, pErrorNames, (int32_t)(-code - 1),
	                 perror_name);
}
...
gs_errorinfo_put_string(i_ctx_t* i_ctx_p, const char* str)
{
	...
	ref* pderror;
	int code =
	    string_to_ref(str, &rstr, iimemory, "gs_errorinfo_put_string");
...
interp(i_ctx_t** pi_ctx_p /* context for execution, updated if resched */,
       const ref* pref /* object to interpret */, ref* perror_object)
{
	i_ctx_t* i_ctx_p = *pi_ctx_p;
	/*
...
#define return_with_error_code_op(nargs) return_with_code_iref()
#define return_with_stackoverflow(objp)                                        \
	{                                                                      \
		o_stack.requested = 1;                                         \
		return_with_error(e_stackoverflow, objp);                      \
	}
...
#define store_state(ep)                                                        \
	(icount > 0                                                            \
	     ? (ep->value.const_refs = IREF + 1, r_set_size(ep, icount))       \
	     : 0)
#define store_state_short(ep)                                                  \
	(icount > 0                                                            \
	     ? (ep->value.packed = iref_packed + 1, r_set_size(ep, icount))    \
	     : 0)
#define store_state_either(ep)                                                 \
	(icount > 0 ? (ep->value.packed = IREF_NEXT_EITHER(iref_packed),       \
	               r_set_size(ep, icount))                                 \
	            : 0)
#define next()                                                                 \
	if(--icount > 0) {                                                     \
...
#define next_either()                                                          \
	...
	iref_packed = IREF_NEXT_EITHER(iref_packed);                           \
	goto top
...
top:
	...
	if(gs_debug['I'] ||
	   (gs_debug['i'] &&
	    (r_is_packed(iref_packed) ? r_packed_is_name(iref_packed)
		...
		osp = iosp;
		esp = iesp;
		dlprintf5("d%u,e%u<%u>0x%lx(%d): ", ref_stack_count(&d_stack),
		          ref_stack_count(&e_stack), ref_stack_count(&o_stack),
		          (uint32_t)IREF, icount);
...
#define cases_invalid()                                                        \
	case plain(t__invalid):                                                \
	case plain_exec(t__invalid)
		cases_invalid() : return_with_error_iref(e_Fatal);
#define cases_nox()                                                            \
	case nox_exec(t_array):                                                \
	case nox_exec(t_dictionary):                                           \
	case nox_exec(t_file):                                                 \
	case nox_exec(t_string):                                               \
	case nox_exec(t_mixedarray):                                           \
	case nox_exec(t_shortarray)
		cases_nox() : return_with_error_iref(e_invalidaccess);
/*
...
#define cases_lit_1()                                                          \
	...
	case plain_exec(t_boolean):                                            \
	case lit(t_dictionary):                                                \
	case nox(t_dictionary)
#define cases_lit_2()                                                          \
	case lit(t_file):                                                      \
	case nox(t_file):                                                      \
	case plain(t_fontID):                                                  \
	case plain_exec(t_fontID):                                             \
	case plain(t_integer):                                                 \
	case plain_exec(t_integer):                                            \
	case plain(t_mark):                                                    \
	case plain_exec(t_mark)
#define cases_lit_3()                                                          \
	case plain(t_name):                                                    \
	case plain(t_null):                                                    \
...
#define cases_lit_4()                                                          \
	case plain(t_real):                                                    \
	case plain_exec(t_real):                                               \
	case plain(t_save):                                                    \
	case plain_exec(t_save):                                               \
	case lit(t_string):                                                    \
	case nox(t_string)
#define cases_lit_5()                                                          \
	case lit(t_mixedarray):                                                \
	case nox(t_mixedarray):                                                \
	case lit(t_shortarray):                                                \
	case nox(t_shortarray):                                                \
	case plain(t_device):                                                  \
	case plain_exec(t_device):                                             \
	case plain(t_struct):                                                  \
	case plain_exec(t_struct):                                             \
	case plain(t_astruct):                                                 \
	case plain_exec(t_astruct)
...
#define cases_lit_array()                                                      \
	...
	case exec(t_shortarray)
		cases_lit_1()
		    ...
		    : INCR(lit);
		break;
		cases_lit_array() : INCR(lit_array);
		break;
	/* Special operators. */
	...
	x_if:
		...
		if(!r_has_type(iosp - 1, t_boolean))
			return_with_error_iref(
			    (iosp <= osbot ? e_stackunderflow : e_typecheck));
		if(!r_is_proc(iosp))
	...
	x_ifelse:
		...
		if(!r_has_type(iosp - 2, t_boolean))
			return_with_error_iref((
			    iosp < osbot + 2 ? e_stackunderflow : e_typecheck));
		if(!r_is_proc(iosp - 1))
	...
	case plain_exec(t_name):
		...
		if(!pv_valid(pvalue)) {
			...
			INCR(find_name);
			if((pvalue = dict_find_name_by_index_inline(
			        nidx, htemp)) == 0)
	...
	case exec(t_file): { /* Executable file.  Read the next token and
	                        interpret it. */
		stream* s;
	...
	again:
		...
		case 0: /* read a token */
			...
			if(!r_has_attr(&token, a_executable) ||
			   r_is_array(
			       &token)) { /* If scan_token used the o-stack, */
		...
		case scan_Refill:
			...
			osp = iosp;
			code =
			    scan_handle_refill(i_ctx_p, &token, &sstate, true,
			                       true, ztokenexec_continue);
		scan_cont:
			iosp = osp;
			iesp = esp;
			switch(code) {
		...
		case scan_DSC_Comment: {
			...
			esp = iesp;
			osp = iosp;
			code = ztoken_handle_comment(
			    i_ctx_p, &file_token, &sstate, &token, code, true,
			    true, ztokenexec_continue);
		}
			goto scan_cont;
		default: /* error */
	...
	case exec(
	    t_string): { /* Executable string.  Read a token and interpret it.
	                    */
		stream ss;
		scanner_state sstate;
...
#undef case_xop
			...
			esp = iesp;
			osp = iosp;
			switch(code = call_operator(op_index_proc(index),
			                            i_ctx_p)) {
			case 0:
			case 1:
				iosp = osp;
		...
		case pt_integer:
			...
			if(iosp >= ostop)
				return_with_stackoverflow_iref();
			++iosp;
			make_int(iosp, ((int)*iref_packed & packed_int_mask) +
			                   packed_min_intval);
			next_short();
		...
		case pt_executable_name:
			INCR(p_exec_name);
			{
				...
				pvalue =
				    name_index_ptr_inline(int_nt, nidx)->pvalue;
				if(!pv_valid(pvalue)) {
					uint htemp;

					INCR(p_find_name);
					if((pvalue =
					        dict_find_name_by_index_inline(
					            nidx, htemp)) == 0) {
						names_index_ref(int_nt, nidx,
						                &token);
						return_with_error(e_undefined,
						                  &token);
					}
				}
				if(r_has_masked_attrs(
				       pvalue, a_execute,
				       a_execute + a_executable)) { /* Literal,
				                                       push it.
				                                       */
					INCR(p_name_lit);
					if(iosp >= ostop)
...
rwe:
	if(!r_is_packed(iref_packed))
		 ...
		 */
		packed_get(imemory, (const ref_packed*)ierror.obj,
		           &ierror.full);
		store_state_short(iesp);
		if(IREF == ierror.obj)
...
error_exit:
	if(ERROR_IS_INTERRUPT(
	       ierror.code)) { /* We must push the current object being
	                          interpreted */
