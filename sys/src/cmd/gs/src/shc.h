...
#define stream_hc_state_common                                                       \
	...
	int bits_left /* # of valid low bits (input) or */ /* unused low bits        \
	                                                      (output) in            \
	                                                      above, */ /* 0         \
	                                                                   <=        \
	                                                                   bits_left \
...
#define hc_print_value(code, clen)                                             \
	(gs_debug_c('W') ? (dlprintf2("[W]0x%x,%d\n", code, clen), 0) : 0)
...
#define hcd_load_state()                                                       \
	p = pr->ptr, rlimit = pr->limit, bits = ss->bits,                      \
...
#define hcd_store_state()                                                      \
	pr->ptr = p -= (bits_left >> 3), ss->bits = bits >>= (bits_left & ~7), \
...
#define hcd_ensure_bits(n, outl)                                               \
	BEGIN HCD_ENSURE_BITS_ELSE(n) goto outl;                               \
	END
...
#define HCD_MORE_BITS_ELSE                                                     \
	if(rlimit - p >= 3) {                                                  \
		if(ss->FirstBitLowOrder)                                       \
			bits = (bits << 24) +                                  \
			       ((uint)byte_reverse_bits[p[1]] << 16) +         \
			       ((uint)byte_reverse_bits[p[2]] << 8) +          \
			       byte_reverse_bits[p[3]];                        \
		else                                                           \
			bits = (bits << 24) + ((uint)p[1] << 16) +             \
			       ((uint)p[2] << 8) + p[3];                       \
		bits_left += 24, p += 3;                                       \
	} else                                                                 \
...
#define hcd_more_bits(outl)                                                    \
	BEGIN HCD_MORE_BITS_ELSE goto outl;                                    \
	END
