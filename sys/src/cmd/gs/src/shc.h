...
#define stream_hc_state_common                                                                                                         \
	...
	int bits_left /* # of valid low bits (input) or */ /* unused low bits (output) in above, */ /* 0 <= bits_left <= 7 */
...
#define hc_put_value(ss, q, code, clen)                                                                                                                   \
	(hc_print_value_then(code, clen)((bits_left -= (clen)) >= 0 ? (bits += (code) << bits_left) : (hc_put_code_proc((ss)->FirstBitLowOrder,           \
															q += hc_bits_size >> 3,           \
															(bits + ((code) >> -bits_left))), \
