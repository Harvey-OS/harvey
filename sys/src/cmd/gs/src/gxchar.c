...
/* Copyright (C) 1989, 1995, 1996, 1997, 1998, 1999 Aladdin Enterprises.  All
  rights reserved.
...
private
const bool CACHE_ROTATED_CHARS = true;

/* Define the maximum size of a full temporary bitmap when rasterizing, */
/* in bits (not bytes). */
private
const uint MAX_TEMP_BITMAP_BITS = 80000;
...
ENUM_PTRS_END
private
RELOC_PTRS_WITH(show_enum_reloc_ptrs, gs_show_enum* eptr)
{
...
private
int continue_kshow(gs_show_enum*);
private
int continue_show(gs_show_enum*);
private
int continue_show_update(gs_show_enum*);
private
void show_set_scale(const gs_show_enum*, gs_log2_scale_point* log2_scale);
private
int show_cache_setup(gs_show_enum*);
private
int show_state_setup(gs_show_enum*);
private
int show_origin_setup(gs_state*, fixed, fixed, gs_show_enum* penum);

/* Accessors for current_char and current_glyph. */
#define CURRENT_CHAR(penum) ((penum)->returned.current_char)
#define SET_CURRENT_CHAR(penum, chr) ((penum)->returned.current_char = (chr))
...
gs_show_enum_alloc(gs_memory_t* mem, gs_state* pgs, client_name_t cname)
{
	...
	rc_alloc_struct_1(penum, gs_show_enum, &st_gs_show_enum, mem, return 0,
	                  cname);
...
private
text_enum_proc_process(gx_show_text_process);
private
text_enum_proc_is_width_only(gx_show_text_is_width_only);
private
text_enum_proc_current_width(gx_show_text_current_width);
private
text_enum_proc_set_cache(gx_show_text_set_cache);
private
text_enum_proc_retry(gx_show_text_retry);
private
text_enum_proc_release(gx_show_text_release); /* not default */

private
const gs_text_enum_procs_t default_text_procs = {
                      ...
                      gs_text_enum_t** ppte)
{
	...
	code =
	    gs_text_enum_init((gs_text_enum_t*)penum, &default_text_procs, dev,
	...
	if(operation & TEXT_DO_ANY_CHARPATH)
		penum->charpath_flag =
		    (operation & TEXT_DO_FALSE_CHARPATH
		         ? cpm_false_charpath
		         : operation & TEXT_DO_TRUE_CHARPATH
		               ? cpm_true_charpath
		               : operation & TEXT_DO_FALSE_CHARBOXPATH
		                     ? cpm_false_charboxpath
		                     : operation & TEXT_DO_TRUE_CHARBOXPATH
		                           ? cpm_true_charboxpath
		                           : operation & TEXT_DO_CHARWIDTH
		                                 ? cpm_charwidth
	...
	if(code < 0)
		return code;
	penum->show_gstate =
	    (propagate_charpath && (pgs->in_charpath != 0) ? pgs->show_gstate
	                                                   : pgs);
	if(!(~operation & (TEXT_DO_NONE | TEXT_RETURN_WIDTH))) {
		 ...
		 */
		gs_make_null_device(dev_null, gs_currentdevice_inline(pgs),
		                    mem);
		pgs->ctm_default_set = false;
		penum->dev_null = dev_null;
		/* Retain this device, since it is referenced from the
		 * enumerator. */
...
private
int set_cache_device(gs_show_enum* penum, gs_state* pgs, floatp llx, floatp lly,
                       ...
                       gs_text_cache_control_t control)
{
	...
	case TEXT_SET_CACHE_DEVICE: {
		int code = set_char_width(penum, pgs, pw[0],
		                          pw[1]); /* default is don't cache */

		if(code < 0)
			return code;
		if(SHOW_IS_ALL_OF(penum,
		                  TEXT_DO_NONE | TEXT_INTERVENE)) /* cshow */
			return code;
	...
	case TEXT_SET_CACHE_DEVICE2: {
		...
		if(gs_rootfont(pgs)->WMode) {
			...
			if((code = gs_point_transform2fixed(&pgs->ctm, -vx, -vy,
			                                    &pvxy)) < 0 ||
			   (code = gs_distance_transform2fixed(&pgs->ctm, vx,
			                                       vy, &dvxy)) < 0)
				return 0; /* don't cache */
			if((code = set_char_width(penum, pgs, pw[6], pw[7])) <
			   0)
				return code;
			...
			code = set_cache_device(penum, pgs, pw[2], pw[3], pw[4],
			                        pw[5]);
			if(code != 1) {
				if(retry) {
					rewind_code = gs_point_transform2fixed(
					    &pgs->ctm, vx, vy, &rewind_pvxy);
					if(rewind_code < 0) {
						/* If the control passes here,
						 * something is wrong. */
						return_error(
						    gs_error_unregistered);
					}
					/* Rewind the origin by (-vx, -vy) if
					 * the cache is failed. */
					gx_translate_to_fixed(
					    pgs, rewind_pvxy.x, rewind_pvxy.y);
				}
				return code;
			}
...
set_char_width(gs_show_enum* penum, gs_state* pgs, floatp wx, floatp wy)
{
	...
	if(penum->fstack.depth > 0 &&
	   penum->fstack.items[penum->fstack.depth].font->FontType ==
	       ft_CID_encrypted) {
		/* We must not convert advance width with a CID font leaf's
		   FontMatrix,
		   because CDevProc is attached to the CID font rather than to
		   its leaf.
		...
		code = gs_distance_transform_inverse(
		    wx, wy,
		    &gs_cid0_indexed_font(pfsi->font, pfsi->index)->FontMatrix,
		    &p);
		if(code < 0)
			return code;
		wx = p.x;
	...
	if((code = gs_distance_transform2fixed(&pgs->ctm, wx, wy,
	                                       &penum->wxy)) < 0)
		return code;
...
private
int
compute_glyph_raster_params(gs_show_enum* penum, bool in_setcachedevice,
                            int* alpha_bits, int* depth,
                            ...
                            gs_log2_scale_point* log2_scale)
{
	...
	*alpha_bits = (*dev_proc(dev, get_alpha_bits))(dev, go_text);
	...
	if(penum->fapi_log2_scale.x != -1)
		*log2_scale = penum->fapi_log2_scale;
		...
		gx_compute_text_oversampling(penum, penum->current_font,
	...
	*depth = (log2_scale->x + log2_scale->y == 0
	              ? 1
	              : min(log2_scale->x + log2_scale->y, *alpha_bits));
	if(gs_currentaligntopixels(penum->current_font->dir) == 0) {
		...
		subpix_origin->y =
		    ((penum->origin.y + rdy) & scy) & (fixed_1 - 1);
#endif
		subpix_origin->x =
		    ((penum->origin.x + rdx) & scx) & (fixed_1 - 1);
	} else
                 ...
                 floatp urx, floatp ury)
{
	...
	pgs->in_cachedevice =
	    CACHE_DEVICE_NOT_CACHING; /* disable color/gray/image operators */
...
#define max_cd(n) (fixed_1 << (arch_sizeof_short * 8 - n)) - (fixed_1 >> n) * 3
...
#undef max_cd
		...
		if((code = gs_distance_transform2fixed(&pgs->ctm, llx, lly,
		                                       &cll)) < 0 ||
		   (code = gs_distance_transform2fixed(&pgs->ctm, llx, ury,
		                                       &clr)) < 0 ||
		   (code = gs_distance_transform2fixed(&pgs->ctm, urx, lly,
		                                       &cul)) < 0 ||
		   (code = gs_distance_transform2fixed(&pgs->ctm, urx, ury,
		                                       &cur)) < 0)
			return 0; /* don't cache */
		{
			fixed ctemp;
...
#undef swap
		...
		code = compute_glyph_raster_params(penum, true, &alpha_bits,
		                                   &depth, &subpix_origin,
		                                   &log2_scale);
		if(code < 0)
			return code;
		...
		if(gs_debug_c('k')) {
			...
			dlprintf6("[p]  ctm=[%g %g %g %g %g %g]\n", pgs->ctm.xx,
			          pgs->ctm.xy, pgs->ctm.yx, pgs->ctm.yy,
		...
		if(cc == 0) {
			...
			pgs->in_cachedevice =
			    CACHE_DEVICE_NONE; /* Provide a correct grestore on
			                          error. */
			...
			code = gx_path_add_rectangle(&box_path, clip_box.p.x,
			                             clip_box.p.y, clip_box.q.x,
			                             clip_box.q.y);
			if(code < 0)
				return code;
			code = gx_cpath_clip(pgs, pgs->clip_path, &box_path,
		...
		pgs->in_cachedevice =
		    CACHE_DEVICE_NONE; /* Provide correct grestore */
		if((code = gs_gsave(pgs)) < 0) {
		...
		gx_translate_to_fixed(
		    pgs, (cc->offset.x + subpix_origin.x) << log2_scale.x,
...
private
int show_update(gs_show_enum* penum);
private
int show_move(gs_show_enum* penum);
private
int show_proceed(gs_show_enum* penum);
private
int show_finish(gs_show_enum* penum);
private
int
...
show_update(gs_show_enum* penum)
{
	...
	case sws_cache:
		...
		}
		{
			...
			code = gx_lookup_fm_pair(
			    pgs->font, &char_tm_only(pgs), &penum->log2_scale,
			    penum->charpath_flag != cpm_show, &pair);
			if(code < 0)
				return code;
			gx_add_cached_char(pgs->font->dir, penum->dev_cache, cc,
			                   pair, &penum->log2_scale);
		}
...
int
gx_current_char(const gs_text_enum_t* pte)
{
	...
	if(fdepth > 0) {
		...
		switch(((gs_font_type0*)(penum->fstack.items[fdepth - 1].font))
		           ->data.FMapType) {
		case fmap_1_7:
		case fmap_9_7:
			chr += fidx << 7;
...
show_move(gs_show_enum* penum)
{
	...
	if(SHOW_IS(penum, TEXT_REPLACE_WIDTHS)) {
		...
		gs_distance_transform2fixed(&pgs->ctm, dpt.x, dpt.y,
		                            &penum->wxy);
	} else {
		double dx = 0, dy = 0;
		...
		if(SHOW_IS_ADD_TO_SPACE(penum)) {
			gs_char chr =
			    gx_current_char((const gs_text_enum_t*)penum);
...
more: /* Proceed to next character */
	...
	if(penum->can_cache >= 0) {
		/* Loop with cache */
		for(;;) {
			switch((code = get_next_char_glyph(
			...
			case 1: /* font change */
				pfont = penum->fstack.items[penum->fstack.depth]
				            .font;
				penum->current_font = pfont;
			...
			case 0: /* plain char */
				/*
				 * We don't need to set penum->current_char in
				 * the
				 * normal cases, but it's needed for widthshow,
				 * kshow, and one strange client, so we may as
				 * well
				 * do it here.
				 */
				SET_CURRENT_CHAR(penum, chr);
				/*
				 * Store glyph now, because pdfwrite needs it
				 * while
				 * synthezising bitmap fonts (see
				 * assign_char_code).
				 */
				if(glyph == gs_no_glyph) {
					glyph = (*penum->encode_char)(
					    pfont, chr, GLYPH_SPACE_NAME);
					SET_CURRENT_GLYPH(penum, glyph);
				} else
					SET_CURRENT_GLYPH(penum, glyph);
				penum->is_pure_color =
				    gs_color_writes_pure(penum->pgs); /* Save
			  ...
			  */
				{
					...
					code = compute_glyph_raster_params(
					    penum, false, &alpha_bits, &depth,
					    &subpix_origin, &log2_scale);
					if(code < 0)
						return code;
					if(pair == 0) {
						code = gx_lookup_fm_pair(
						    pfont, &char_tm_only(pgs),
						    &log2_scale,
						    penum->charpath_flag !=
						        cpm_show,
						    &pair);
					...
					cc = gx_lookup_cached_char(
					    pfont, pair, glyph, wmode, depth,
					    &subpix_origin);
				}
				if(cc == 0) {
					...
					/* If antialiasing is in effect, don't
					 * use xfont */
					if(log2_scale.x + log2_scale.y > 0)
						goto no_cache;
					if(pfont->ExactSize ==
				...
				/* We might be doing .charboxpath or
				 * stringwidth; */
				/* check for these now. */
				if(penum->charpath_flag == cpm_charwidth) {
					/* This is charwidth.  Just move by the
					 * width. */
					DO_NOTHING;
				} else if(penum->charpath_flag != cpm_show) {
					/* This is .charboxpath. Get the
					 * bounding box */
					...
					code = gx_path_current_point(pgs->path,
					                             &pt);
					if(code < 0)
						return code;
					llx =
					    fixed_rounded(pt.x - cc->offset.x) +
					    int2fixed(penum->ftx);
					lly =
					    fixed_rounded(pt.y - cc->offset.y) +
					    int2fixed(penum->fty);
					urx = llx + int2fixed(cc->width),
					ury = lly + int2fixed(cc->height);
					gx_path_init_local(&box_path,
					                   pgs->memory);
					code = gx_path_add_rectangle(
					    &box_path, llx, lly, urx, ury);
					if(code >= 0)
						code = gx_path_add_char_path(
						    pgs->show_gstate->path,
						    &box_path,
						    penum->charpath_flag);
					if(code >= 0)
						code = gx_path_add_point(
						    pgs->path, pt.x, pt.y);
					gx_path_free(&box_path,
					             "show_proceed(box path)");
					if(code < 0)
						return code;
				...
				if(code) {
					/* Might be kshow, glyph is stored
					 * above. */
					return code;
				}
			}
		...
		/* Can't use cache */
		switch((code = get_next_char_glyph((gs_text_enum_t*)penum, &chr,
		                                   &glyph))) {
		default:
			return code;
		case 2:
		...
		case 0: {
			...
			code = compute_glyph_raster_params(
			    penum, false, &alpha_bits, &depth, &subpix_origin,
			    &log2_scale);
			if(code < 0)
				return code;
			if(pair == 0) {
				code = gx_lookup_fm_pair(
				    pfont, &char_tm_only(pgs), &log2_scale,
				    penum->charpath_flag != cpm_show, &pair);
				if(code < 0)
					return code;
			}
		...
		if(glyph == gs_no_glyph) {
			glyph =
			    (*penum->encode_char)(pfont, chr, GLYPH_SPACE_NAME);
		}
		SET_CURRENT_GLYPH(penum, glyph);
		cc = 0;
	}
no_cache:
	 ...
	 * we get here with cc != 0.  penum->current_char and
	 * penum->current_glyph
...
show_state_setup(gs_show_enum* penum)
{
	...
	if(penum->fstack.depth <= 0) {
		...
		if(pfont->FontType == ft_CID_encrypted) {
			...
			gs_matrix_multiply(
			    &mat, &(gs_cid0_indexed_font(pfont, pfsi->index)
			                ->FontMatrix),
			    &mat);
		}
	...
	if(!CACHE_ROTATED_CHARS && is_matrix_good_for_caching(&pgs->char_tm))
		penum->can_cache = 0;
	if(penum->can_cache >= 0 && gx_effective_clip_path(pgs, &pcpath) >= 0) {
		gs_fixed_rect cbox;
...
show_set_scale(const gs_show_enum* penum, gs_log2_scale_point* log2_scale)
{
	...
	if((penum->charpath_flag == cpm_show ||
	   ...
	   is_matrix_good_for_caching(&pgs->char_tm)) {
		const gs_font_base* pfont =
		    (const gs_font_base*)penum->current_font;
		gs_fixed_point extent;
		int code = gs_distance_transform2fixed(
		    &pgs->char_tm, pfont->FontBBox.q.x - pfont->FontBBox.p.x,
		    pfont->FontBBox.q.y - pfont->FontBBox.p.y, &extent);

		if(code >= 0) {
			int sx =
			    (any_abs(extent.x) < int2fixed(60)
			         ? 2
			         : any_abs(extent.x) < int2fixed(200) ? 1 : 0);
			int sy =
			    (any_abs(extent.y) < int2fixed(60)
			         ? 2
			         : any_abs(extent.y) < int2fixed(200) ? 1 : 0);
...
show_origin_setup(gs_state* pgs, fixed cpt_x, fixed cpt_y, gs_show_enum* penum)
{
	if(penum->charpath_flag == cpm_show) {
		...
		if(gs_currentaligntopixels(penum->current_font->dir) == 0) {
			...
			int rdx = 1L
			          << (_fixed_shift - 1 - penum->log2_scale.x);
			int rdy = 1L
			          << (_fixed_shift - 1 - penum->log2_scale.y);
