		      ...
		      gs_memory_t *mem, gs_text_enum_t **ppte)
{
	...
	if(operation & TEXT_DO_ANY_CHARPATH)
		penum->charpath_flag =
		    (operation & TEXT_DO_FALSE_CHARPATH ? cpm_false_charpath : operation & TEXT_DO_TRUE_CHARPATH ? cpm_true_charpath : operation & TEXT_DO_FALSE_CHARBOXPATH ? cpm_false_charboxpath : operation & TEXT_DO_TRUE_CHARBOXPATH ? cpm_true_charboxpath : operation & TEXT_DO_CHARWIDTH ? cpm_charwidth : cpm_show /* can't happen */);
	else
		penum->charpath_flag =
	...
	if(code < 0)
		return code;
	penum->show_gstate =
	    (propagate_charpath && (pgs->in_charpath != 0) ? pgs->show_gstate : pgs);
			    ...
			    gs_fixed_point *subpix_origin, gs_log2_scale_point *log2_scale)
{
	...
	*alpha_bits = (*dev_proc(dev, get_alpha_bits))(dev, go_text);
     ...
     */
	*depth = (log2_scale->x + log2_scale->y == 0 ? 1 : min(log2_scale->x + log2_scale->y, *alpha_bits));
...
{
	penum->encode_char =
	    (SHOW_IS(penum, TEXT_FROM_GLYPHS | TEXT_FROM_SINGLE_GLYPH) ? gs_no_encode_char : gs_show_current_font(penum)->procs.encode_char);
}
...
more: /* Proceed to next character */
	pfont = (penum->fstack.depth < 0 ? pgs->font : penum->fstack.items[penum->fstack.depth].font);
...
show_set_scale(const gs_show_enum *penum, gs_log2_scale_point *log2_scale)
{
	...
	if((penum->charpath_flag == cpm_show ||
	   ...
	   is_matrix_good_for_caching(&pgs->char_tm)) {
		...
		if(code >= 0) {
			int sx =
			    (any_abs(extent.x) < int2fixed(60) ? 2 : any_abs(extent.x) < int2fixed(200) ? 1 : 0);
			int sy =
			    (any_abs(extent.y) < int2fixed(60) ? 2 : any_abs(extent.y) < int2fixed(200) ? 1 : 0);
