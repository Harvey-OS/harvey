                             ...
                             const char* page_init)
{
	...
	uint storage_size_words =
	    line_size_words * 8; /* data, out_row, out_row_alt, prev_row */
	word* storage;
	word* data_words, *out_row_words, *out_row_alt_words, *prev_row_words;
...
#define prev_row ((byte*)prev_row_words)
	...
	if(num_copies != 1 && !(features & PCL_CAN_PRINT_COPIES))
		return gx_default_print_page_copies(pdev, prn_stream,
		                                    num_copies);
	storage = (uint32_t*)gs_alloc_byte_array(
	    pdev->memory, storage_size_words, W, "hpjet_print_page");
	...
	/* Send each scan line in turn */
	{
		...
		for(lnum = 0; lnum < num_rows; lnum++) {
			...
			code = gdev_prn_copy_scan_lines(pdev, lnum, (byte*)data,
			                                line_size);
			if(code < 0)
				break;
			...
			/* Skip blank lines if any */
				 ...
				 * motion
				 * on the DeskJet, so if the number of lines is
				 * small,
				 ...
				 * doesn't
				 * properly clear the seed row if we are in
				 * compression mode
				 * 3.
				 */
			...
			if(features & PCL_MODE_3_COMPRESSION) {
				...
				int count3 = gdev_pcl_mode3compress(
				    line_size, data, prev_row, out_row);
				int count2 = gdev_pcl_mode2compress(
				    data_words, end_data, out_row_alt);
				int penalty3 =
				...
				out_data = out_row;
				out_count = gdev_pcl_mode2compress(
				    data_words, end_data, out_row);
			} else {
				out_data = data;
