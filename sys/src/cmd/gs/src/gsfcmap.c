...
private
const gs_cmap_ranges_enum_procs_t identity_range_procs = {identity_next_range};
private
void
...
identity_next_lookup(gs_cmap_lookups_enum_t* penum)
{
	if(penum->index[0] == 0) {
		...
		memset(penum->entry.key[1], 0,
		       num_bytes - pcimap->varying_bytes);
...
private
const gs_cmap_lookups_enum_procs_t identity_lookup_procs = {
    identity_next_lookup, identity_next_entry};
const gs_cmap_lookups_enum_procs_t gs_cmap_no_lookups_procs = {no_next_lookup,
                                                               0};
private
void
                      ...
                      gs_cmap_lookups_enum_t* pre)
{
	gs_cmap_lookups_enum_setup(
	    pre, pcmap,
	    (which ? &gs_cmap_no_lookups_procs : &identity_lookup_procs));
}
private
bool
...
private
const gs_cmap_procs_t identity_procs = {
                       ...
                       gs_memory_t* mem)
{
	...
	static const gs_cid_system_info_t identity_cidsi = {
	    {(const byte*)"Adobe", 5}, {(const byte*)"Identity", 8}, 0};
	int code;
...
gs_cmap_create_char_identity(gs_cmap_t** ppcmap, int num_bytes, int wmode,
                             gs_memory_t* mem)
{
	return gs_cmap_identity_alloc(
	    ppcmap, num_bytes, 1, num_bytes,
	    (wmode ? "Identity-BF-V" : "Identity-BF-H"), wmode, mem);
}
...
gs_cmap_decode_next(const gs_cmap_t* pcmap, const gs_const_string* str,
                    uint* pindex, uint* pfidx, gs_char* pchr, gs_glyph* pglyph)
{
	return pcmap->procs->decode_next(pcmap, str, pindex, pfidx, pchr,
	                                 pglyph);
}
              ...
              const gs_cmap_procs_t* procs, gs_memory_t* mem)
{
	...
	gs_cid_system_info_t* pcidsi = gs_alloc_struct_array(
	    mem, num_fonts, gs_cid_system_info_t, &st_cid_system_info_element,
 ...
 */
void
gs_cmap_ranges_enum_setup(gs_cmap_ranges_enum_t* penum, const gs_cmap_t* pcmap,
...
gs_cmap_compute_identity(const gs_cmap_t* pcmap, int font_index_only)
{
	...
	for(gs_cmap_lookups_enum_init(pcmap, which, &lenum);
		...
		if(font_index_only >= 0 &&
		   lenum.entry.font_index != font_index_only)
			continue;
...
private
const int gs_cmap_ToUnicode_code_bytes = 2;
...
gs_private_st_suffix_add0(st_cmap_ToUnicode, gs_cmap_ToUnicode_t,
                          "gs_cmap_ToUnicode_t", cmap_ToUnicode_enum_ptrs,
                          cmap_ToUnicode_reloc_ptrs, st_cmap);
...
private
const gs_cmap_ranges_enum_procs_t gs_cmap_ToUnicode_range_procs = {
    gs_cmap_ToUnicode_next_range};

private
int
gs_cmap_ToUnicode_decode_next(const gs_cmap_t* pcmap,
                              const gs_const_string* str, uint* pindex,
...
private
void
gs_cmap_ToUnicode_enum_ranges(const gs_cmap_t* pcmap,
...
private
const gs_cmap_lookups_enum_procs_t gs_cmap_ToUnicode_lookup_procs = {
...
{
	gs_cmap_lookups_enum_setup(
	    pre, pcmap, (which ? &gs_cmap_no_lookups_procs : /* fixme */
...
private
const gs_cmap_procs_t gs_cmap_ToUnicode_procs = {
    gs_cmap_ToUnicode_decode_next, gs_cmap_ToUnicode_enum_ranges,
    gs_cmap_ToUnicode_enum_lookups, gs_cmap_ToUnicode_is_identity};

/*
 * Allocate and initialize a ToUnicode CMap.
 */
int
gs_cmap_ToUnicode_alloc(gs_memory_t* mem, int id, int num_codes, int key_size,
                        gs_cmap_t** ppcmap)
{
	int code;
...
#endif
	code = gs_cmap_alloc(ppcmap, &st_cmap_ToUnicode, 0, cmap_name, name_len,
	                     NULL, 0, &gs_cmap_ToUnicode_procs, mem);
	if(code < 0)
		return code;
	map = (uint8_t*)gs_alloc_bytes(mem,
	                               num_codes * gs_cmap_ToUnicode_code_bytes,
