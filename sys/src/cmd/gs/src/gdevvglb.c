...
/* Copyright (C) 1992, 1993, 1994, 1996, 1997, 1998, 1999 Aladdin Enterprises.
  All rights reserved.
...
private
dev_proc_open_device(vgalib_open);
private
dev_proc_close_device(vgalib_close);
private
dev_proc_map_rgb_color(vgalib_map_rgb_color);
private
dev_proc_map_color_rgb(vgalib_map_color_rgb);
private
dev_proc_fill_rectangle(vgalib_fill_rectangle);
private
dev_proc_tile_rectangle(vgalib_tile_rectangle);
private
dev_proc_copy_mono(vgalib_copy_mono);
private
dev_proc_copy_color(vgalib_copy_color);
private
dev_proc_get_bits(vgalib_get_bits);
private
dev_proc_get_params(vgalib_get_params);
private
dev_proc_put_params(vgalib_put_params);

const gx_device_vgalib gs_vgalib_device = {
    std_device_std_body(gx_device_vgalib, 0, "vgalib", 0, 0, 1, 1),
    {vgalib_open, NULL, /* get_initial_matrix */
     NULL,              /* sync_output */
     NULL,              /* output_page */
     vgalib_close, vgalib_map_rgb_color, vgalib_map_color_rgb,
     vgalib_fill_rectangle, vgalib_tile_rectangle, vgalib_copy_mono,
     vgalib_copy_color, NULL, /* draw_line (obsolete) */
     vgalib_get_bits, vgalib_get_params, vgalib_put_params,
     NULL, /* map_cmyk_color */
	...
	if(vga_getcolors() > 1) {
		...
		for(index = 0; index < 16; ++index) {
			...
			(*dev_proc(dev, map_color_rgb))(
			    dev, (gx_color_index)index, rgb);
#define cv2pv(cv) ((cv) >> (gx_color_value_bits - 8))
			vga_setpalette(index, cv2pv(rgb[0]), cv2pv(rgb[1]),
			               cv2pv(rgb[2]));
#undef cv2pv
		}
	}
...
private
gx_color_index
vgalib_map_rgb_color(gx_device* dev, gx_color_value red, gx_color_value green,
                     gx_color_value blue)
{
...
private
int
vgalib_tile_rectangle(gx_device* dev, const gx_tile_bitmap* tile, int x, int y,
...
private
int
vgalib_copy_mono(gx_device* dev, const byte* base, int sourcex, int raster,
...
private
int
vgalib_copy_color(gx_device* dev, const byte* base, int sourcex, int raster,
                  gx_bitmap_id id, int x, int y, int width, int height)
{
	...
	if(gx_device_has_color(dev)) { /* color device, four bits per pixel */
		...
		vgalib_copy_mono(dev, base, sourcex, raster, id, x, y, width,
		                 height, (gx_color_index)0, (gx_color_index)7);
	}
