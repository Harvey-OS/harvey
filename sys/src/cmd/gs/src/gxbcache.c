...
#define ssize1 ((uint)lsize1)
	...
	if(lsize1 > bck->size - cnext &&
	   lsize != left) { /* Not enough room to allocate in this chunk. */
		*pcbh = 0;
		return -1;
	}
	...
	while(fsize < ssize1 && fsize != ssize) {
		if(!cb_head_is_free(
		       cbh_next)) { /* Ask the caller to free the entry. */
			if(fsize)
				cbh->size = fsize;
			*pcbh = cbh_next;
			return -1;
...
void
gx_bits_cache_shorten(gx_bits_cache* bc, gx_cached_bits_head* cbh, uint diff,
