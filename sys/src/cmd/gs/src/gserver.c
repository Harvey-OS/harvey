...
int gs_server_run_string(const char* str, int* exit_code_ptr, char* errstr,
                     ...
                     const char* init_str)
{
	...
	if((code = gs_init0(c_stdin, c_stdout, c_stderr, 0)) < 0 ||
	   (code = gs_init1()) < 0 || (code = gs_init2()) < 0)
		return code;
	code = gs_server_run_string("/QUIET true def /NOPAUSE true def",
	                            &exit_code, (char*)0, 0, &errstr_len);
	if(code < 0)
		return code;
	return (init_str == NULL ? 0 : gs_server_run_string(
	                                   init_str, &exit_code, (char*)0, 0,
	                                   &errstr_len));
}
...
int
gs_server_run_string(const char* str, int* exit_code_ptr, char* errstr,
                     int errstr_max_len, int* errstr_len_ptr)
{
	ref error_object;
	int code;
...
int
gs_server_run_files(const char** file_names, int permanent, int* exit_code_ptr,
...
errstr_report(ref* perror_object, char* errstr, int errstr_max_len,
              int* errstr_len_ptr)
{
	int code = obj_cvs(perror_object, (byte*)errstr, (uint)errstr_max_len,
	                   (uint*)errstr_len_ptr, false);
