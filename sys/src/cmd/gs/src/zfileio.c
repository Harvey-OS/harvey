...
private
int handle_read_status(i_ctx_t *, int, const ref *, const uint *,
		       op_proc_t);
private
int handle_write_status(i_ctx_t *, int, const ref *, const uint *,
			op_proc_t);
...
zreadline_continue(i_ctx_t *i_ctx_p)
{
	...
	code = (start == 0 ? zreadline_at(i_ctx_p, op - 1, size, true) : zreadline_at(i_ctx_p, op - 1, start, false));
	if(code >= 0)
		pop(1);
	return code;
}
...
zflush(i_ctx_t *i_ctx_p)
{
	...
	return (s_is_writing(s) ? handle_write_status(i_ctx_p, status, &rstdout, NULL, zflush) : handle_read_status(i_ctx_p, status, &rstdout, NULL, zflush));
}
...
zflushfile(i_ctx_t *i_ctx_p)
{
	...
	return (s_is_writing(s) ? handle_write_status(i_ctx_p, status, op, NULL, zflushfile) : handle_read_status(i_ctx_p, status, op, NULL, zflushfile));
}
