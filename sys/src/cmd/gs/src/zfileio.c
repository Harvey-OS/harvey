...
/* Forward references */
private
int write_string(ref*, stream*);
private
int handle_read_status(i_ctx_t*, int, const ref*, const uint*, op_proc_t);
private
int handle_write_status(i_ctx_t*, int, const ref*, const uint*, op_proc_t);
...
zclosefile(i_ctx_t* i_ctx_p)
{
	...
	if(file_is_valid(s, op)) { /* closing a closed file is a no-op */
		...
		if(status != 0 && status != EOFC) {
			if(s_is_writing(s))
				return handle_write_status(i_ctx_p, status, op,
				                           NULL, zclosefile);
			else
				return handle_read_status(i_ctx_p, status, op,
				                          NULL, zclosefile);
		}
	}
	pop(1);
...
private
int zreadhexstring_continue(i_ctx_t*);
...
zreadhexstring_at(i_ctx_t* i_ctx_p, os_ptr op, uint start)
{
	...
	for(;;) {
		status =
		    s_hex_process(&s->cursor.r, &cw, &odd, hex_ignore_garbage);
...
private
int zwritehexstring_continue(i_ctx_t*);
private
int
...
#define MAX_HEX 128
	...
	while(len) {
		...
		case CALLC:
			...
			count &= 1;
			return handle_write_status(i_ctx_p, status, op - 1,
			                           &count,
...
private
int zreadstring_continue(i_ctx_t*);
private
int
...
private
int zreadline(i_ctx_t*);
private
int zreadline_continue(i_ctx_t*);
...
zreadline_at(i_ctx_t* i_ctx_p, os_ptr op, uint count, bool in_eol)
{
	...
	default:
		if(count == 0 && !in_eol)
			...
			return handle_read_status(i_ctx_p, status, op - 1,
			                          &count, zreadline_continue);
		}
	}
	r_set_size(op, count);
	op[-1] = *op;
...
int
zreadline_from(stream* s, gs_string* buf, gs_memory_t* bufmem, uint* pcount,
               bool* pin_eol)
{
	...
	if(zis_stdin(s))
		...
		readline = sreadline;
	return readline(s, NULL, NULL /*WRONG*/, NULL, buf, bufmem, pcount,
	                pin_eol, zis_stdin);
}
...
zprint(i_ctx_t* i_ctx_p)
{
	...
	make_stream_file(&rstdout, s, "w");
	code =
	    handle_write_status(i_ctx_p, status, &rstdout, NULL, zwritestring);
...
zpeekstring(i_ctx_t* i_ctx_p)
{
	...
	while((rlen = sbufavailable(s)) < len) {
		...
		case 0:
			/*
			 * The following is a HACK.  It should reallocate the
			 * buffer to hold
			 * at least len bytes.  However, this raises messy
			 * problems about
			 * which allocator to use and how it should interact
			 * with restore.
			 */
			if(len >= s->bsize)
				return_error(e_rangecheck);
...
private
int zwritecvp_continue(i_ctx_t*);
private
int
...
const op_def zfileio1_op_defs[] = {{"1bytesavailable", zbytesavailable},
...
copy_error_string(i_ctx_t* i_ctx_p, const ref* fop)
{
	...
	if(s->state->error_string[0]) {
		int code =
		    gs_errorinfo_put_string(i_ctx_p, s->state->error_string);
...
private
int
handle_read_status(i_ctx_t* i_ctx_p, int ch, const ref* fop, const uint* pindex,
                   op_proc_t cont)
{
	switch(ch) {
	default: /* error */
	...
	case CALLC:
		if(pindex) {
			...
			return s_handle_read_exception(i_ctx_p, ch, fop, &index,
			                               1, cont);
		} else
			return s_handle_read_exception(i_ctx_p, ch, fop, NULL,
			                               0, cont);
	}
}
                    ...
                    const uint* pindex, op_proc_t cont)
{
	...
	case CALLC:
		if(pindex) {
			...
			make_int(&index, *pindex);
			return s_handle_write_exception(i_ctx_p, ch, fop,
			                                &index, 1, cont);
		} else
			return s_handle_write_exception(i_ctx_p, ch, fop, NULL,
			                                0, cont);
	}
}
