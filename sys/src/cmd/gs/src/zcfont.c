...
/* Forward references */
private
int cshow_continue(i_ctx_t*);
private
int cshow_restore_font(i_ctx_t*);
	...
	/* Push the character code and width, and call the procedure. */
	{
		...
		if(font == root_font)
			...
			/* Construct a scaled version of the leaf font.
			   If font stack is deep enough, get the matrix for
			   scaling
			   from the immediate parent of current font.
			   (The font matrix of root font is not good for the
			   purpose
			   in some case.)
			...
			ialloc_set_space(idmemory, font_space);
			code =
			    gs_makefont(font->dir, font, &root_font->FontMatrix,
			                &scaled_font);
			ialloc_set_space(idmemory, save_space);
			if(code < 0)
				return code;
...
const op_def zcfont_op_defs[] = {{"2cshow", zcshow},
                                 {"0rootfont", zrootfont},
                                 /* Internal operators */
