...
#define MAX_INT_STR 20
	...
	str[size] = 0;
	return (sscanf(str, "%d", pvalue) == 1 ? 0 : gs_note_error(
	                                                 gs_error_rangecheck));
#undef MAX_INT_STR
}
                  ...
                  const gs_param_string* pairs, uint count)
{
	...
	int present = pdfmark_find_key(Page_key, pairs, count, &page_string) +
...
{
	return cos_dict_put_string(pcd, pair->data, pair->size, pair[1].data,
	                           pair[1].size);
}
...
#define MAX_RECT_STRING 100
	...
	strcpy(chars + size, " 0");
	if(sscanf(chars, "[%lg %lg %lg %lg]%d", &v[0], &v[1], &v[2], &v[3],
	          &end_check) != 5)
		return_error(gs_error_rangecheck);
...
pdfmark_make_rect(char str[MAX_RECT_STRING], const gs_rect* prect)
{
	...
	pprintg4(&s, "[%g %g %g %g]", prect->p.x, prect->p.y, prect->q.x,
	         prect->q.y);
	str[stell(&s)] = 0;
}
...
private
int
setup_pdfmark_stream_compression(gx_device_psdf* pdev0, cos_stream_t* pco)
{
	/* This function is for pdfwrite only. */
	gx_device_pdf* pdev = (gx_device_pdf*)pdev0;
	gs_memory_t* mem = pdev->pdf_memory;
	static const pdf_filter_names_t fnames = {PDF_FILTER_NAMES};
	...
	pco->input_strm = cos_write_stream_alloc(
	    pco, pdev, "setup_pdfmark_stream_compression");
	if(pco->input_strm == 0)
	...
	if(!pdev->binary_ok) {
		...
		if(ss == 0)
			return_error(gs_error_VMerror);
		if(s_add_filter(&pco->input_strm, &s_A85E_template, ss, mem) ==
		   0) {
	...
	return pdf_put_filters(cos_stream_dict(pco), pdev, pco->input_strm,
	                       &fnames);
}
                     ...
                     bool for_outline)
{
	...
	for(i = 0; i < count; i += 2) {
		...
		if(pdf_key_eq(pair, "/SrcPg") &&
		   sscanf((const char*)pair[1].data, "%ld", &src_pg) == 1)
			...
			Dest = pair[1];
			coerce_dest = true;
		} else if(pdf_key_eq(pair, "/Page") ||
		          pdf_key_eq(pair, "/View")) {
			/* Make a destination even if this is for an outline. */
			if(Dest.data == 0) {
				code = pdfmark_make_dest(dest, params->pdev,
				                         "/Page", "/View",
				                         pairs, count);
				if(code < 0)
					return code;
		 ...
		 */
			...
			/*
			 * Copy the string into value storage, then update it in
			 * place.
			 */
			pdfmark_put_pair(pcd, pair);
			/* Break const so we can update the (copied) string. */
			pcv =
			    (cos_value_t*)cos_dict_find_c_key(pcd, "/Contents");
			...
			for(i = j = 0; i < csize;)
				if(csize - i >= 2 &&
				   ...
				   (i == 0 || cstr[i - 1] != '\\')) {
					...
					cstr[j++] = cstr[i++];
			if(j != i)
				pcv->contents.chars.data = gs_resize_string(
				    pdev->pdf_memory, cstr, csize, j,
		...
		if((File != 0 || Dest.data != 0) &&
		   (pdf_key_eq(Action + 1, "/Launch") ||
			...
			if(adict == 0)
				return_error(gs_error_VMerror);
			if(!for_outline) {
				/* We aren't sure whether this is really
				 * needed.... */
				cos_dict_put_c_strings(adict, "/Type",
				                       "/Action");
			}
			if(pdf_key_eq(Action + 1, "/Article")) {
				cos_dict_put_c_strings(adict, "/S", "/Thread");
				coerce_dest =
				    false; /* Dest is not a real destination */
			} else
			...
			if(adict == 0)
				return_error(gs_error_VMerror);
			while((code = pdf_scan_token(&scan, end, &key.data)) >
			      0) {
				key.size = scan - key.data;
				if(key.data[0] != '/' ||
				   (code = pdf_scan_token_composite(
				        &scan, end, &value.data)) != 1)
					break;
				value.size = scan - value.data;
				if(pdf_key_eq(&key, "/Dest") ||
				   pdf_key_eq(&key, "/D")) {
					param_string_from_string(key, "/D");
					if(value.data[0] == '(') {
						/****** HANDLE ESCAPES ******/
						pdfmark_coerce_dest(&value,
						                    dest);
					}
	...
	if(Dest.data) {
		...
		cos_dict_put_c_key_string(
		    pcd, "/Dest", (const unsigned char*)dstr, strlen(dstr));
	}
	if(File)
              ...
              const char* subtype)
{
	...
	if(code < 0)
		return code;
	code =
	    pdfmark_put_ao_pairs(pdev, pcd, pairs, count, pctm, &params, false);
	if(code < 0)
		return code;
	if(params.src_pg >= 0)
	...
	return cos_array_add(annots, cos_object_value(&value, COS_OBJECT(pcd)));
}
...
pdfmark_write_bead(gx_device_pdf* pdev, const pdf_bead_t* pbead)
{
	...
	s = pdev->strm;
	pprintld3(s, "<</T %ld 0 R/V %ld 0 R/N %ld 0 R", pbead->article_id,
	          pbead->prev_id, pbead->next_id);
	if(pbead->page_id != 0)
	...
	for(part = pdev->articles; part != 0; part = part->next) {
		...
		if(a_title != 0 && !COS_VALUE_IS_OBJECT(a_title) &&
		   !bytes_compare(a_title->contents.chars.data,
		                  a_title->contents.chars.size, title.data,
		                  title.size))
			break;
	}
             ...
             const gs_matrix* pctm, const gs_param_string* objname)
{
	...
	if(!pdfmark_find_key("/Dest", pairs, count, &key) ||
	   (present = pdfmark_make_dest(dest, pdev, "/Page", "/View", pairs,
	                                count)) < 0)
		return_error(gs_error_rangecheck);
...
start_XObject(gx_device_pdf* pdev, bool compress, cos_stream_t** ppcs)
{
	...
	if(code < 0)
		return code;
	code =
	    pdf_enter_substream(pdev, resourceXObject, gs_no_id, &pres, false,
	...
	if(!pdev->substream_Resources)
		return_error(gs_error_VMerror);
	if(pdev->ForOPDFRead) {
		code = cos_dict_put_c_key_bool((cos_dict_t*)pres->object,
		                               "/.Global", true);
		if(code < 0)
			return code;
	}
	...
	if(level1.data == 0 && source.size <= MAX_PS_INLINE && objname == 0) {
		...
		if(level1.data != 0) {
			...
			code = pdf_enter_substream(
			    pdev, resourceXObject, gs_no_id, &pres, true,
			    pdev->CompressFonts /* Have no better switch*/);
			if(code < 0)
				return code;
			pcs = (cos_stream_t*)pres->object;
		...
		if(level1_id != gs_no_id) {
			...
			code = cos_dict_put_c_key_string(cos_stream_dict(pcs),
			                                 "/Level1", (byte*)r,
			                                 strlen(r));
			if(code < 0)
				return code;
		...
		if(code < 0)
			return code;
		code = pdf_substitute_resource(pdev, &pres, resourceXObject,
		                               NULL, false);
		if(code < 0)
			return code;
		if(objname != 0) {
			code = cos_dict_put(
			    pdev->local_named_objects, objname->data,
			    objname->size,
			    cos_object_value(&value, (cos_object_t*)pcs));
			if(code < 0)
				return code;
		}
...
pdfmark_add_pagelabel(gx_device_pdf* pdev, const gs_param_string* label)
{
	...
	if(label != 0) {
		if(!pdev->PageLabels) {
			pdev->PageLabels = cos_array_alloc(
			    pdev, "pdfmark_add_pagelabel(PageLabels)");
			if(pdev->PageLabels == 0)
				return_error(gs_error_VMerror);
			pdev->PageLabels->id = pdf_obj_ref(pdev);

			/* empty label for unlabled pages before first labled
			 * page */
			pdev->PageLabels_current_page = 0;
			pdev->PageLabels_current_label = cos_dict_alloc(
			    pdev, "pdfmark_add_pagelabel(first)");
		...
		code = cos_dict_put_c_key(
		    dict, "/P",
		    cos_string_value(&value, label->data, label->size));
		if(code < 0) {
	...
	if(label == 0 || pdev->next_page != pdev->PageLabels_current_page) {
		 ...
		 * next labeled page */
		if(pdev->PageLabels) {
			if(pdev->next_page - pdev->PageLabels_current_page >
			   1) {
				cos_dict_t* tmp = cos_dict_alloc(
				    pdev, "pdfmark_add_pagelabel(tmp)");
				if(tmp == 0)
					return_error(gs_error_VMerror);
                ...
                const gs_matrix* pctm, const gs_param_string* no_objname)
{
	...
	for(i = 0; code >= 0 && i < count; i += 2) {
		...
		if(pdf_key_eq(pairs + i, "/Producer")) {
			/*
			 * If the string "Distiller" appears anywhere in the
			 * Producer,
			 * replace the Producer (or the part after a " + ") with
			 * our
			 * own name.
			 */
			...
			if(string_match(vdata, vsize,
			                (const byte*)"*Distiller*", 11,
			                &params) ||
			   string_match(vdata, vsize,
			                (const byte*)"*\000D\000i\000s\000t\000"
			                             "i\000l\000l\000e\000r*",
			                20, &params)) {
				uint j;
				...
				/*
				 * Replace vdata[j .. vsize) with our name.
				 * Note that both
				 * vdata/vstr and the default producer string
				 * are enclosed
				 * in ().
				 */
	...
	if(pdfmark_make_dest(dest, pdev, "/Page", "/View", pairs, count)) {
		...
		for(i = 0; code >= 0 && i < count; i += 2)
			if(!(pdf_key_eq(&pairs[i], "/Page") ||
			     pdf_key_eq(&pairs[i], "/View")))
				code =
				    pdfmark_put_pair(pdev->Catalog, pairs + i);
		return code;
	} else
           ...
           const gs_matrix* pctm, const gs_param_string* objname)
{
	...
	if(code < 0)
		return code;
	if(sscanf((const char*)pairs[1].data, "[%lg %lg %lg %lg]", &bbox.p.x,
	          &bbox.p.y, &bbox.q.x, &bbox.q.y) != 4)
		return_error(gs_error_rangecheck);
	...
	{
		byte* s =
		    gs_alloc_string(pdev->memory, objname->size, "pdfmark_PS");
	...
	pprintg4(&s, "[%g %g %g %g]", bbox.p.x, bbox.p.y, bbox.q.x, bbox.q.y);
	bbox_str_len = stell(&s);
	swrite_string(&s, matrix_str, sizeof(bbox_str));
	pprintg6(&s, "[%g %g %g %g %g %g]", ictm.xx, ictm.xy, ictm.yx, ictm.yy,
	         ictm.tx, ictm.ty);
           ...
           const gs_matrix* pctm, const gs_param_string* no_objname)
{
	...
	if(code < 0)
		return code;
	code =
	    pdf_substitute_resource(pdev, &pres, resourceXObject, NULL, true);
	if(code < 0)
		return code;
	if(objname.size) {
		code = cos_dict_put(
		    pdev->local_named_objects, objname.data, objname.size,
		    cos_object_value(&value, (cos_object_t*)pres->object));
		if(code < 0)
			return code;
	}
	gs_free_const_string(pdev->memory, objname.data, objname.size,
	                     "pdfmark_EP");
	if(code < 0)
		return code;
	return 0;
            ...
            const gs_matrix* pctm, const gs_param_string* objname)
{
	...
	if((code = pdf_make_named(pdev, objname, cotype, &pco, true)) < 0) {
		/*
		 * For Distiller compatibility, allows multiple /OBJ pdfmarks
		 * with
            ...
            const gs_matrix* pctm, const gs_param_string* no_objname)
{
	...
	if(index < 0)
		return_error(gs_error_rangecheck);
	return cos_array_put(
	    (cos_array_t*)pco, index,
                  ...
                  const gs_matrix* pctm, const gs_param_string* no_objname)
{
	...
	if(!pco->is_open)
		return_error(gs_error_rangecheck);
	for(i = 1; i < count; ++i)
		if(sputs(pco->input_strm, pairs[i].data, pairs[i].size, &l) !=
		   0)
...
const pdfmark_name mark_names[] = {
    ...
    {".PUTSTREAM", pdfmark_PUTSTREAM,
     PDFMARK_ODD_OK | PDFMARK_KEEP_NAME | PDFMARK_NO_REFS},
...
pdfmark_process(gx_device_pdf* pdev, const gs_param_string_array* pma)
{
	...
	if(size < 2 ||
	   sscanf((const char*)pts[-1].data, "[%g %g %g %g %g %g]", &ctm.xx,
	          &ctm.xy, &ctm.yx, &ctm.yy, &ctm.tx, &ctm.ty) != 6)
		return_error(gs_error_rangecheck);
	...
	for(pmn = mark_names; pmn->mname != 0; ++pmn)
		if(pdf_key_eq(pts, pmn->mname)) {
			...
			/*
			 * Our coordinate system is scaled so that user space is
			 * always
			...
			if(pmn->options & PDFMARK_NAMEABLE) {
				...
				for(j = 0; j < size; j += 2) {
					if(pdf_key_eq(&data[j], "/_objdef")) {
						objname = &data[j + 1];
						if(!pdf_objname_is_valid(
						       objname->data,
						       objname->size))
							return_error(
							    gs_error_rangecheck);
						/* Save the pairs without the
						 * name. */
						size -= 2;
			...
			/* Save all the pairs. */
			pairs = (gs_param_string*)gs_alloc_byte_array(
			    mem, size, sizeof(gs_param_string),
		...
		copied: /* Substitute object references for names. */
			if(!(pmn->options & PDFMARK_NO_REFS)) {
				for(j = (pmn->options & PDFMARK_KEEP_NAME
				             ? 1
				             : 1 - odd_ok);
				    j < size; j += 2 - odd_ok) {
					code = pdf_replace_names(
					    pdev, &pairs[j], &pairs[j]);
					if(code < 0) {
						gs_free_object(
						    mem, pairs,
						    "pdfmark_process(pairs)");
