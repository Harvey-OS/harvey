...
gs_cspace_set_devn_function(gs_color_space *pcspace, gs_function_t *pfn)
{
	...
	if(gs_color_space_get_index(pcspace) != gs_color_space_index_DeviceN ||
	   ...
	   pfn->params.n !=
	       gs_color_space_num_components((gs_color_space *)&pcspace->params.device_n.alt_space))
		return_error(gs_error_rangecheck);
	pimap = pcspace->params.device_n.map;
		      ...
		      frac *pconc, const gs_imager_state *pis)
{
	...
	if(pis->color_component_map.use_alt_cspace) {
		...
		tcode = (*pcs->params.device_n.map->tint_transform)(pc->paint.values, &cc.paint.values[0],
...
gx_install_DeviceN(const gs_color_space *pcs, gs_state *pgs)
{
	...
	if(pgs->color_space->params.device_n.use_alt_cspace)
		code = (*pcs->params.device_n.alt_space.type->install_cspace)((const gs_color_space *)&pcs->params.device_n.alt_space, pgs);
	/*
...
gx_adjust_cspace_DeviceN(const gs_color_space *pcs, int delta)
{
	...
	(*pcs->params.device_n.alt_space.type->adjust_cspace_count)((const gs_color_space *)&pcs->params.device_n.alt_space, delta);
}
