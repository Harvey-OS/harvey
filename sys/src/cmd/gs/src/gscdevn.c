...
gs_private_st_composite(st_color_space_DeviceN, gs_paint_color_space,
                        "gs_color_space_DeviceN", cs_DeviceN_enum_ptrs,
                        cs_DeviceN_reloc_ptrs);
private_st_device_n_map();

/* Define the DeviceN color space type. */
private
cs_proc_num_components(gx_num_components_DeviceN);
private
cs_proc_base_space(gx_alt_space_DeviceN);
private
cs_proc_init_color(gx_init_DeviceN);
private
cs_proc_restrict_color(gx_restrict_DeviceN);
private
cs_proc_concrete_space(gx_concrete_space_DeviceN);
private
cs_proc_concretize_color(gx_concretize_DeviceN);
private
cs_proc_remap_concrete_color(gx_remap_concrete_DeviceN);
private
cs_proc_install_cspace(gx_install_DeviceN);
private
cs_proc_set_overprint(gx_set_overprint_DeviceN);
private
cs_proc_adjust_cspace_count(gx_adjust_cspace_DeviceN);
private
cs_proc_serialize(gx_serialize_DeviceN);
const gs_color_space_type gs_color_space_type_DeviceN = {
    gs_color_space_index_DeviceN, true, false, &st_color_space_DeviceN,
    gx_num_components_DeviceN, gx_alt_space_DeviceN, gx_init_DeviceN,
    gx_restrict_DeviceN, gx_concrete_space_DeviceN, gx_concretize_DeviceN,
    gx_remap_concrete_DeviceN, gx_default_remap_color, gx_install_DeviceN,
    gx_set_overprint_DeviceN, gx_adjust_cspace_DeviceN,
    gx_no_adjust_color_count, gx_serialize_DeviceN,
...
ENUM_PTRS_END
private
RELOC_PTRS_WITH(cs_DeviceN_reloc_ptrs, gs_color_space* pcs)
{
 ...
 */
int
gs_build_DeviceN(gs_color_space* pcspace, uint num_components,
                 const gs_color_space* palt_cspace, gs_memory_t* pmem)
{
	...
	code =
	    alloc_device_n_map(&pcsdevn->map, pmem, "gs_cspace_build_DeviceN");
	if(code < 0) {
		return code;
	}
	/* Allocate space for color names list. */
	pnames = (gs_separation_name*)gs_alloc_byte_array(
	    pmem, num_components, sizeof(gs_separation_name),
	    ".gs_cspace_build_DeviceN(names)");
	if(pnames == 0) {
		gs_free_object(pmem, pcsdevn->map,
		               ".gs_cspace_build_DeviceN(map)");
 ...
 */
int
gs_cspace_build_DeviceN(gs_color_space** ppcspace, gs_separation_name* psnames,
                        uint num_components, const gs_color_space* palt_cspace,
                        gs_memory_t* pmem)
{
...
int
map_devn_using_function(const float* in, float* out, const gs_imager_state* pis,
                        void* data)
	...
	if(gs_color_space_get_index(pcspace) != gs_color_space_index_DeviceN ||
	   ...
	   pfn->params.n !=
	       gs_color_space_num_components(
	           (gs_color_space*)&pcspace->params.device_n.alt_space))
		return_error(gs_error_rangecheck);
	pimap = pcspace->params.device_n.map;
...
private
const gs_color_space*
gx_concrete_space_DeviceN(const gs_color_space* pcs, const gs_imager_state* pis)
{
#ifdef DEBUG
	/*
                      ...
                      frac* pconc, const gs_imager_state* pis)
{
	 ...
	 * We must preserve tcode for implementing a semi-hack in the
	 * interpreter.
	 */
	if(pis->color_component_map.use_alt_cspace) {
		...
		if(map->cache_valid) {
			...
			for(i = pcs->params.device_n.num_components;
			    --i >= 0;) {
			...
			if(i < 0) {
				int num_out =
				    gs_color_space_num_components(pacs);
		...
		tcode = (*pcs->params.device_n.map->tint_transform)(
		    pc->paint.values, &cc.paint.values[0], pis,
		    pcs->params.device_n.map->tint_transform_data);
		if(tcode < 0)
			return tcode;
                          ...
                          gx_device* dev, gs_color_select_t select)
{
	...
	if(pis->color_component_map.use_alt_cspace) {
		...
		return (*pacs->type->remap_concrete_color)(pconc, pacs, pdc,
		                                           pis, dev, select);
	} else {
...
check_DeviceN_component_names(const gs_color_space* pcs, gs_state* pgs)
{
	...
	byte* pname;
	uint name_size;
	gs_devicen_color_map* pcolor_component_map = &pgs->color_component_map;
	...
	for(i = 0; i < num_comp; i++) {
		...
		pcs->params.device_n.get_colorname_string(dev->memory, names[i],
		                                          &pname, &name_size);
		/*
		 * Postscript does not accept /None as a color component but it
		 * is
		...
		if(name_size == none_size &&
		   (strncmp(none_str, (const char*)pname, name_size) == 0)) {
			...
			/*
			 * Check for duplicated names.  Except for /None, no
			 * components
			...
			/*
			 * Compare the colorant name to the device's.  If the
			 * device's
			 * compare routine returns
			 * GX_DEVICE_COLOR_MAX_COMPONENTS then the
			 * colorant is in the SeparationNames list but not in
			 * the
			 * SeparationOrder list.
			 */
			colorant_number =
			    (*dev_proc(dev, get_color_comp_index))(
			        dev, (const char*)pname, name_size,
			        SEPARATION_NAME);
	...
	if(pcmap->use_alt_cspace)
		...
		if((params.retain_any_comps = pgs->overprint)) {
			...
			for(i = 0; i < ncomps; i++) {
				...
				if(mcomp >= 0)
					gs_overprint_set_drawn_comp(
					    params.drawn_comps, mcomp);
			}
		}
...
int
gx_serialize_device_n_map(const gs_color_space* pcs, gs_device_n_map* m,
                          stream* s)
{
...
gx_serialize_DeviceN(const gs_color_space* pcs, stream* s)
{
	...
	if(code < 0)
		return code;
	code = sputs(s, (const byte*)&p->num_components,
	             sizeof(p->num_components), &n);
	if(code < 0)
		return code;
	code = sputs(s, (const byte*)&p->names[0],
	             sizeof(p->names[0]) * p->num_components, &n);
	if(code < 0)
		return code;
