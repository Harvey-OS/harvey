...
typedef struct gx_device_color_info_s {
	 ...
	 *)
	 *
	 * The comp_mask and comp_bits fields should be left empty by the
	 *client.
...
#define dci_extended_alpha_values(mcmp, nc, p, d, gi, mg, mc, dg, dc, ta, ga,  \
                                  sl, cn)                                      \
	{                                                                      \
		mcmp /* max components */, nc /* number components */,         \
		    p /* polarity */, d /* depth */, gi /* gray index */,      \
		    mg /* max gray */, mc /* max color */,                     \
		    dg /* dither grays */, dc /* dither colors */,             \
		    {ta, ga} /* antialias info text, graphics */,              \
		    sl /* separable_and_linear */, {0} /* component shift */,  \
		    {0} /* component bits */, {0} /* component mask */,        \
		    cn /* process color name */,                               \
		    GX_CINFO_OPMODE_UNKNOWN /* opmode */, 0 /* process_cmps */ \
	}
...
#define dci_std_cm_name(nc)                                                    \
	((nc) == 1 ? "DeviceGray" : ((nc) == 3 ? "DeviceRGB" : "DeviceCMYK"))

#define dci_std_polarity(nc)                                                   \
	((nc) == 4 ? GX_CINFO_POLARITY_SUBTRACTIVE : GX_CINFO_POLARITY_ADDITIVE)
 ...
 */
#define dci_std_gray_index(nc) ((nc) == 3 ? GX_CINFO_COMP_NO_INDEX : (nc)-1)

#define dci_alpha_values(nc, depth, mg, mc, dg, dc, ta, ga)                    \
	dci_extended_alpha_values(                                             \
	    nc, nc, dci_std_polarity(nc), depth, dci_std_gray_index(nc), mg,   \
	    mc, dg, dc, ta, ga, GX_CINFO_UNKNOWN_SEP_LIN, dci_std_cm_name(nc))
...
#define dci_std_color_max_gray(nc, color_bits)                                 \
	((nc) == 3 ? 0 : (1 << dci_std_gray_bits(nc, color_bits)) - 1)

#define dci_std_color_max_color(nc, color_bits)                                \
	((nc) == 1 ? 0 : (1 << dci_std_color_bits(nc, color_bits)) - 1)
...
#define dci_std_color_(nc, color_bits)                                         \
	dci_values(nc, dci_std_color_depth(color_bits),                        \
...
#define dci_std_color(color_bits)                                              \
	dci_std_color_(dci_std_color_num_components(color_bits), color_bits)
 ...
 */
#define get_process_color_model_name(dev) ((dev)->color_info.cm_name)
...
#define dev_page_proc_install(proc) int proc(gx_device* dev, gs_state* pgs)
	dev_page_proc_install((*install));

#define dev_page_proc_begin_page(proc) int proc(gx_device* dev, gs_state* pgs)
...
#define fill_dev_proc(dev, p, dproc)                                           \
	if(dev_proc(dev, p) == 0)                                              \
	set_dev_proc(dev, p, dproc)
#define assign_dev_procs(todev, fromdev) ((todev)->procs = (fromdev)->procs)
...
#define dev_t_proc_open_device(proc, dev_t) int proc(dev_t* dev)
#define dev_proc_open_device(proc) dev_t_proc_open_device(proc, gx_device)
...
#define dev_t_proc_sync_output(proc, dev_t) int proc(dev_t* dev)
#define dev_proc_sync_output(proc) dev_t_proc_sync_output(proc, gx_device)

#define dev_t_proc_output_page(proc, dev_t)                                    \
	int proc(dev_t* dev, int num_copies, int flush)
#define dev_proc_output_page(proc) dev_t_proc_output_page(proc, gx_device)

#define dev_t_proc_close_device(proc, dev_t) int proc(dev_t* dev)
#define dev_proc_close_device(proc) dev_t_proc_close_device(proc, gx_device)

#define dev_t_proc_map_rgb_color(proc, dev_t)                                  \
	gx_color_index proc(dev_t* dev, const gx_color_value cv[])
#define dev_proc_map_rgb_color(proc) dev_t_proc_map_rgb_color(proc, gx_device)

#define dev_t_proc_map_color_rgb(proc, dev_t)                                  \
	int proc(dev_t* dev, gx_color_index color, gx_color_value rgb[3])
#define dev_proc_map_color_rgb(proc) dev_t_proc_map_color_rgb(proc, gx_device)

#define dev_t_proc_fill_rectangle(proc, dev_t)                                 \
	int proc(dev_t* dev, int x, int y, int width, int height,              \
	         gx_color_index color)
#define dev_proc_fill_rectangle(proc) dev_t_proc_fill_rectangle(proc, gx_device)

#define dev_t_proc_tile_rectangle(proc, dev_t)                                 \
	int proc(dev_t* dev, const gx_tile_bitmap* tile, int x, int y,         \
	         int width, int height, gx_color_index color0,                 \
	         gx_color_index color1, int phase_x, int phase_y)
#define dev_proc_tile_rectangle(proc) dev_t_proc_tile_rectangle(proc, gx_device)

#define dev_t_proc_copy_mono(proc, dev_t)                                      \
	int proc(dev_t* dev, const byte* data, int data_x, int raster,         \
	         gx_bitmap_id id, int x, int y, int width, int height,         \
	         gx_color_index color0, gx_color_index color1)
#define dev_proc_copy_mono(proc) dev_t_proc_copy_mono(proc, gx_device)

#define dev_t_proc_copy_color(proc, dev_t)                                     \
	int proc(dev_t* dev, const byte* data, int data_x, int raster,         \
	         gx_bitmap_id id, int x, int y, int width, int height)
#define dev_proc_copy_color(proc) dev_t_proc_copy_color(proc, gx_device)

/* OBSOLETED in release 3.66 */

#define dev_t_proc_draw_line(proc, dev_t)                                      \
	int proc(dev_t* dev, int x0, int y0, int x1, int y1,                   \
	         gx_color_index color)
#define dev_proc_draw_line(proc) dev_t_proc_draw_line(proc, gx_device)

/* Added in release 2.4 */

#define dev_t_proc_get_bits(proc, dev_t)                                       \
	int proc(dev_t* dev, int y, byte* data, byte** actual_data)
#define dev_proc_get_bits(proc) dev_t_proc_get_bits(proc, gx_device)
...
#define dev_proc_get_params(proc) dev_t_proc_get_params(proc, gx_device)

#define dev_t_proc_put_params(proc, dev_t)                                     \
	int proc(dev_t* dev, gs_param_list* plist)
#define dev_proc_put_params(proc) dev_t_proc_put_params(proc, gx_device)

/* Added in release 2.6 */

#define dev_t_proc_map_cmyk_color(proc, dev_t)                                 \
	gx_color_index proc(dev_t* dev, const gx_color_value cv[])
#define dev_proc_map_cmyk_color(proc) dev_t_proc_map_cmyk_color(proc, gx_device)
...
#define dev_t_proc_get_xfont_device(proc, dev_t) gx_device* proc(dev_t* dev)
...
#define dev_t_proc_map_rgb_alpha_color(proc, dev_t)                            \
	gx_color_index proc(dev_t* dev, gx_color_value red,                    \
...
#define dev_t_proc_get_page_device(proc, dev_t) gx_device* proc(dev_t* dev)
...
#define dev_proc_get_alpha_bits(proc) dev_t_proc_get_alpha_bits(proc, gx_device)
...
#define dev_proc_copy_alpha(proc) dev_t_proc_copy_alpha(proc, gx_device)

/* Added in release 3.38 */

#define dev_t_proc_get_band(proc, dev_t)                                       \
	int proc(dev_t* dev, int y, int* band_start)
#define dev_proc_get_band(proc) dev_t_proc_get_band(proc, gx_device)

/* Added in release 3.44 */

#define dev_t_proc_copy_rop(proc, dev_t)                                       \
	int proc(dev_t* dev, const byte* sdata, int sourcex, uint sraster,     \
	         gx_bitmap_id id, const gx_color_index* scolors,               \
	         const gx_tile_bitmap* texture, const gx_color_index* tcolors, \
	         int x, int y, int width, int height, int phase_x,             \
	         int phase_y, gs_logical_operation_t lop)
#define dev_proc_copy_rop(proc) dev_t_proc_copy_rop(proc, gx_device)

/* Added in release 3.60, changed in 3.68. */

#define dev_t_proc_fill_path(proc, dev_t)                                      \
	int proc(dev_t* dev, const gs_imager_state* pis, gx_path* ppath,       \
	         const gx_fill_params* params,                                 \
	         const gx_drawing_color* pdcolor, const gx_clip_path* pcpath)
#define dev_proc_fill_path(proc) dev_t_proc_fill_path(proc, gx_device)

#define dev_t_proc_stroke_path(proc, dev_t)                                    \
	int proc(dev_t* dev, const gs_imager_state* pis, gx_path* ppath,       \
	         const gx_stroke_params* params,                               \
	         const gx_drawing_color* pdcolor, const gx_clip_path* pcpath)
#define dev_proc_stroke_path(proc) dev_t_proc_stroke_path(proc, gx_device)
...
#define dev_proc_fill_mask(proc) dev_t_proc_fill_mask(proc, gx_device)

/* Added in release 3.66, changed in 3.69 */

#define dev_t_proc_fill_trapezoid(proc, dev_t)                                 \
	int proc(dev_t* dev, const gs_fixed_edge* left,                        \
	         const gs_fixed_edge* right, fixed ybot, fixed ytop,           \
	         bool swap_axes, const gx_drawing_color* pdcolor,              \
	         gs_logical_operation_t lop)
#define dev_proc_fill_trapezoid(proc) dev_t_proc_fill_trapezoid(proc, gx_device)

#define dev_t_proc_fill_parallelogram(proc, dev_t)                             \
	int proc(dev_t* dev, fixed px, fixed py, fixed ax, fixed ay, fixed bx, \
...
#define dev_t_proc_fill_triangle(proc, dev_t)                                  \
	int proc(dev_t* dev, fixed px, fixed py, fixed ax, fixed ay, fixed bx, \
	         fixed by, const gx_drawing_color* pdcolor,                    \
	         gs_logical_operation_t lop)
#define dev_proc_fill_triangle(proc) dev_t_proc_fill_triangle(proc, gx_device)

#define dev_t_proc_draw_thin_line(proc, dev_t)                                 \
	int proc(dev_t* dev, fixed fx0, fixed fy0, fixed fx1, fixed fy1,       \
	         const gx_drawing_color* pdcolor, gs_logical_operation_t lop)
#define dev_proc_draw_thin_line(proc) dev_t_proc_draw_thin_line(proc, gx_device)
...
#define dev_t_proc_begin_image(proc, dev_t)                                    \
	int proc(dev_t* dev, const gs_imager_state* pis,                       \
	         const gs_image_t* pim, gs_image_format_t format,              \
	         const gs_int_rect* prect, const gx_drawing_color* pdcolor,    \
	         const gx_clip_path* pcpath, gs_memory_t* memory,              \
	         gx_image_enum_common_t** pinfo)
#define dev_proc_begin_image(proc) dev_t_proc_begin_image(proc, gx_device)

/* OBSOLETED in release 5.23 */

#define dev_t_proc_image_data(proc, dev_t)                                     \
	int proc(dev_t* dev, gx_image_enum_common_t* info,                     \
	         const byte** planes, int data_x, uint raster, int height)
#define dev_proc_image_data(proc) dev_t_proc_image_data(proc, gx_device)

/* OBSOLETED in release 5.23 */

#define dev_t_proc_end_image(proc, dev_t)                                      \
	int proc(dev_t* dev, gx_image_enum_common_t* info, bool draw_last)
#define dev_proc_end_image(proc) dev_t_proc_end_image(proc, gx_device)

/* Added in release 3.68 */

#define dev_t_proc_strip_tile_rectangle(proc, dev_t)                           \
	int proc(dev_t* dev, const gx_strip_bitmap* tiles, int x, int y,       \
	         int width, int height, gx_color_index color0,                 \
	         gx_color_index color1, int phase_x, int phase_y)
...
#define dev_t_proc_strip_copy_rop(proc, dev_t)                                 \
	int proc(dev_t* dev, const byte* sdata, int sourcex, uint sraster,     \
	         ...
	         const gx_color_index* tcolors, int x, int y, int width,       \
	         int height, int phase_x, int phase_y,                         \
	         gs_logical_operation_t lop)
#define dev_proc_strip_copy_rop(proc) dev_t_proc_strip_copy_rop(proc, gx_device)
...
#define dev_t_proc_map_color_rgb_alpha(proc, dev_t)                            \
	int proc(dev_t* dev, gx_color_index color, gx_color_value rgba[4])
...
#define dev_t_proc_create_compositor(proc, dev_t)                              \
	int proc(dev_t* dev, gx_device** pcdev, const gs_composite_t* pcte,    \
...
#define dev_t_proc_begin_transparency_group(proc, dev_t)                       \
	int proc(gx_device* dev, const gs_transparency_group_params_t* ptgp,   \
...
#define dev_t_proc_end_transparency_group(proc, dev_t)                         \
	int proc(gx_device* dev, gs_imager_state* pis,                         \
...
#define dev_t_proc_begin_transparency_mask(proc, dev_t)                        \
	int proc(gx_device* dev, const gx_transparency_mask_params_t* ptmp,    \
	         const gs_rect* pbbox, gs_imager_state* pis,                   \
	         gs_transparency_state_t** ppts, gs_memory_t* mem)
...
#define dev_t_proc_end_transparency_mask(proc, dev_t)                          \
	int proc(gx_device* dev, gs_transparency_mask_t** pptm)
...
#define dev_t_proc_discard_transparency_layer(proc, dev_t)                     \
	int proc(gx_device* dev, gs_transparency_state_t** ppts)
...
#define dev_proc_pattern_manage(proc) dev_t_proc_pattern_manage(proc, gx_device)
...
#define dev_t_proc_include_color_space(proc, dev_t)                            \
	int proc(dev_t* dev, gs_color_space* cspace, const byte* res_name,     \
	         int name_length)
...
typedef struct gs_fill_attributes_s {
	...
	fixed ystart, yend;           /* Only for X-independent gradients. Base
...
#define dev_t_proc_fill_linear_color_scanline(proc, dev_t)                     \
	int proc(                                                              \
	    dev_t* dev, const gs_fill_attributes* fa, int i, int j,            \
	    int w, /* scanline coordinates and width */                        \
	    const frac31*                                                      \
	        c0, /* initial color for the pixel (i,j), the integer part */  \
	    const int32_t* c0_f,   /* initial color for the pixel (i,j), the   \
	                              fraction part numerator */               \
...
#define dev_t_proc_fill_linear_color_triangle(proc, dev_t)                     \
	int proc(dev_t* dev, const gs_fill_attributes* fa,                     \
	         const gs_fixed_point* p0, const gs_fixed_point* p1,           \
	         const gs_fixed_point* p2, const frac31* c0, const frac31* c1, \
	         const frac31* c2)
...
#define gx_device_proc_struct(dev_t)                                           \
	{                                                                      \
		...
		dev_t_proc_strip_tile_rectangle((*strip_tile_rectangle),       \
		                                dev_t);                        \
		...
		dev_t_proc_begin_transparency_group(                           \
		    (*begin_transparency_group), dev_t);                       \
		dev_t_proc_end_transparency_group((*end_transparency_group),   \
		                                  dev_t);                      \
		dev_t_proc_begin_transparency_mask((*begin_transparency_mask), \
		                                   dev_t);                     \
		dev_t_proc_end_transparency_mask((*end_transparency_mask),     \
		                                 dev_t);                       \
		dev_t_proc_discard_transparency_layer(                         \
		    (*discard_transparency_layer), dev_t);                     \
		dev_t_proc_get_color_mapping_procs((*get_color_mapping_procs), \
		                                   dev_t);                     \
		dev_t_proc_get_color_comp_index((*get_color_comp_index),       \
		                                dev_t);                        \
		...
		dev_t_proc_fill_rectangle_hl_color((*fill_rectangle_hl_color), \
		                                   dev_t);                     \
		dev_t_proc_include_color_space((*include_color_space), dev_t); \
		dev_t_proc_fill_linear_color_scanline(                         \
		    (*fill_linear_color_scanline), dev_t);                     \
		dev_t_proc_fill_linear_color_trapezoid(                        \
		    (*fill_linear_color_trapezoid), dev_t);                    \
		dev_t_proc_fill_linear_color_triangle(                         \
		    (*fill_linear_color_triangle), dev_t);                     \
		dev_t_proc_update_spot_equivalent_colors(                      \
		    (*update_spot_equivalent_colors), dev_t);                  \
	}
...
#define gx_device_begin_image(dev, pis, pim, format, prect, pdcolor, pcpath,   \
                              memory, pinfo)                                   \
	((*dev_proc(dev, begin_image))(dev, pis, pim, format, prect, pdcolor,  \
	                               pcpath, memory, pinfo))
#define gx_device_begin_typed_image(dev, pis, pmat, pim, prect, pdcolor,       \
                                    pcpath, memory, pinfo)                     \
	((*dev_proc(dev, begin_typed_image))(dev, pis, pmat, pim, prect,       \
 ...
 */
int gx_image_data(gx_image_enum_common_t* info, const byte** planes, int data_x,
...
#define gx_device_end_image(dev, info, draw_last) gx_image_end(info, draw_last)

/*
 * Get the anti-aliasing parameters for a device.  This replaces the
 * obsolete get_alpha_bits device procedure.
 */
#define gx_device_get_alpha_bits(dev, type) gx_default_get_alpha_bits(dev, type)
...
#define public_st_device() /* in gsdevice.c */                                 \
	gs_public_st_complex_only(st_device, gx_device, "gx_device", 0,        \
	                          gs_no_struct_enum_ptrs,                      \
	                          gs_no_struct_reloc_ptrs, gx_device_finalize)
...
#define public_st_device_forward() /* in gsdevice.c */                         \
	gs_public_st_complex_only(                                             \
	    st_device_forward, gx_device_forward, "gx_device_forward", 0,      \
...
#define gx_device_is_null(dev) ((dev)->dname == gs_null_device.dname)
extern_st(st_device_null);
#define public_st_device_null() /* in gsdevice.c */                            \
	gs_public_st_complex_only(                                             \
	    st_device_null, gx_device_null, "gx_device_null", 0,               \
	    device_forward_enum_ptrs, device_forward_reloc_ptrs,               \
	    gx_device_finalize)
 ...
 */
void gx_device_init(gx_device* dev, const gx_device* proto, gs_memory_t* mem,
                    bool internal);
...
int gx_device_adjust_resolution(gx_device* dev, int actual_width,
                                int actual_height, int fit);
...
void gx_device_set_media_size(gx_device* dev, floatp media_width,
                              floatp media_height);

/****** BACKWARD COMPATIBILITY ******/
#define gx_device_set_page_size(dev, w, h) gx_device_set_media_size(dev, w, h)
...
#define dev_type_proc_initialize(proc) int proc(gx_device*)
...
#define device_type(dtname, stype, initproc)                                   \
      private                                                                  \
	dev_type_proc_initialize(initproc);                                    \
