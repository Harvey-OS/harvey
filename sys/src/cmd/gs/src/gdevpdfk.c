...
pdf_cie_add_ranges(cos_dict_t* pcd, const gs_range* prange, int n, bool clamp)
{
	...
	if(code >= 0)
		code =
		    cos_dict_put_c_key_object(pcd, "/Range", COS_OBJECT(pca));
	if(code < 0)
          ...
          const gs_range* ranges, gs_memory_t* mem)
{
	...
	for(i = 0; i < 1 << ncomp; ++i) {
		...
		for(j = 0; j < ncomp; ++j)
			in[j] =
			    (i & (1 << j) ? ranges[j].rmax : ranges[j].rmin);
		if(cie_to_xyz(in, xyz, pcs, pis) >= 0) {
			...
			for(j = 1; j < 3; ++j) {
				range_out[j].rmin =
				    min(range_out[j].rmin, lab[j]);
				range_out[j].rmax =
				    max(range_out[j].rmax, lab[j]);
			}
		}
	}
                       ...
                       const gs_range* prange)
{
	...
	if(pcd == 0)
		return_error(gs_error_VMerror);
	if((code = lab_range(ranges, pcs, pciec, prange, pdev->pdf_memory)) <
	       0 ||
...
pdf_make_iccbased(gx_device_pdf* pdev, cos_array_t* pca, int ncomps,
                  const gs_range* prange /*[4]*/, const gs_color_space* pcs_alt,
...

{
	...
	for(i = 0; i < ncomps; ++i) {
		...
		if(rmin < 0.0 || rmax > 1.0) {
			...
			if(pprange == 0)
				return_error(gs_error_rangecheck); /* scaling
				                                      not
				                                      allowed */
			*pprange = prange;
			scale_inputs = true;
	...
	if((pcstrm = cos_stream_alloc(pdev, "pdf_make_iccbased(stream)")) ==
	   0) {
	...
	if(!std_ranges && !scale_inputs) {
		code = pdf_cie_add_ranges(cos_stream_dict(pcstrm), prange,
		                          ncomps, true);
		if(code < 0)
			goto fail;
	}
...
private
int write_trc_abc(cos_stream_t*, const profile_table_t*, gs_memory_t*);
private
int write_trc_lmn(cos_stream_t*, const profile_table_t*, gs_memory_t*);
private
profile_table_t*
              ...
              gs_memory_t* ignore_mem)
{
	...
	for(i = 0; i < gx_cie_cache_size; ++i, p += 2)
		set_sample16(
		    p, proc(cache_arg(i, gx_cie_cache_size - 1, pnt->ranges),
		            pabc));
...
private
int write_a2b0(cos_stream_t*, const profile_table_t*, gs_memory_t*);
private
profile_table_t*
         ...
         const gs_color_space* pcs)
{
	...
	pnt = add_table(
	    ppnt, "A2B0", pa2b->header,
	    sizeof(pa2b->header) + ncomps * 2 * NUM_IN_ENTRIES + /* in */
...
private
int
write_a2b0(cos_stream_t* pcstrm, const profile_table_t* pnt, gs_memory_t* mem)
{
...
#define MAX_NCOMPS 4 /* CIEBasedDEFG */
	...
	for(i = 0; i < pa2b->count; ++i) {
		...
		for(n = i, j = ncomps - 1; j >= 0; --j, n /= num_points)
			in[j] =
			    cache_arg(n % num_points, num_points - 1,
...
#undef MAX_ICC_XYZ_VALUE
		if((code = cos_stream_add_bytes(pcstrm, entry, sizeof(entry))) <
		   0)
			break;
	}
	gx_cie_to_xyz_free(pis);
	...
	static const byte header_data[] =
	    {
	     ...
	     0,          0,          0,   3,   /* profile flags (embedded use
	                                          only) */
	     0,          0,          0,   0,   0, 0,
	     0,          0,                  /* device manufacturer */
	     0,          0,          0,   0, /* device model */
	     0,          0,          0,   0,   0, 0,
	     0,          2 /* device attributes */
...
#define MAX_NUM_TABLES 9 /* desc, [rgb]TRC, [rgb]xYZ, wtpt, cprt */
	...
	pdf_cspace_init_Device(pdev->memory, &alt_space,
	                       ncomps); /* can't fail */
	code = pdf_make_iccbased(pdev, pca, ncomps, prange, &alt_space, &pcstrm,
	                         pprange);
	if(code < 0)
		return code;
	...
	if((one_step == ONE_STEP_ABC || one_step == ONE_STEP_LMN) &&
	   pmat != 0) {
		/* Use TRC + XYZ. */
		profile_table_t* tr =
		...
		/* AToB (mft2) */
		profile_table_t* pnt =
		    add_a2b0(&next_table, &a2b0, ncomps, pcs);
	...
	/* Write the profile. */
	{
		...
		if((code = cos_stream_add_bytes(pcstrm, header,
		                                sizeof(header))) < 0 ||
		...
		for(i = 0; i < num_tables; ++i) {
			...
			if((code = cos_stream_add_bytes(pcstrm, tables[i].data,
			                                len)) < 0 ||
			   (tables[i].write != 0 &&
			    (code = tables[i].write(pcstrm, &tables[i],
			                            pdev->pdf_memory)) < 0) ||
			   (code = cos_stream_add_bytes(
			        pcstrm, pad, -(int)(tables[i].length) & 3)) < 0)
				return code;
		}
	}
                         ...
                         const gs_color_space* pcs, cos_array_t* pca)
{
	...
	cos_stream_t* pcstrm;
	int code = pdf_make_iccbased(
	    pdev, pca, picc_info->num_components, picc_info->Range.ranges,
	    (const gs_color_space*)&picc_params->alt_space, &pcstrm, NULL);
                      ...
                      const gs_range_t** pprange)
{
	return (pdev->CompatibilityLevel < 1.3
	            ...
	            pdf_convert_cie_to_iccbased(pdev, pca, pcs, dcsname, pciec,
	                                        prange, one_step, pmat,
	                                        pprange));
}
