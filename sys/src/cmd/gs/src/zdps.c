...
zsetscreenphase(i_ctx_t* i_ctx_p)
{
	...
	x = op[-1].value.intval;
	y = op->value.intval;
	if(x != (int)x || y != (int)y || op[-2].value.intval < -1 ||
...
zcurrentscreenphase(i_ctx_t* i_ctx_p)
{
	...
	check_type(*op, t_integer);
	if(op->value.intval < -1 || op->value.intval >= gs_color_select_count)
		return_error(e_rangecheck);
...
zimage2(i_ctx_t* i_ctx_p)
{
	...
	check_dict_read(*op);
	{
		...
		if((code = dict_matrix_param(imemory, op, "ImageMatrix",
		                             &image.ImageMatrix)) < 0 ||
		   (code = dict_find_string(op, "DataSource", &pDataSource)) <
		       0 ||
		   (code = dict_float_param(op, "XOrigin", 0.0,
		                            &image.XOrigin)) != 0 ||
		   (code = dict_float_param(op, "YOrigin", 0.0,
		                            &image.YOrigin)) != 0 ||
		   (code = dict_float_param(op, "Width", 0.0, &image.Width)) !=
		       0 ||
		   image.Width <= 0 ||
		...
		image.DataSource = igstate_ptr(pDataSource);
		{
			...
			if(dict_find_string(op, "UnpaintedPath", &ignoref) >
			   0) {
				check_dict_write(*op);
				image.UnpaintedPath = gx_path_alloc(
				    imemory, ".image2 UnpaintedPath");
				if(image.UnpaintedPath == 0)
					return_error(e_VMerror);
			} else
				image.UnpaintedPath = 0;
		}
		code = process_non_source_image(
		    i_ctx_p, (const gs_image_common_t*)&image, ".image2");
		if(image.UnpaintedPath) {
			...
			if(gx_path_is_null(image.UnpaintedPath))
				...
				code = make_upath(i_ctx_p, &rupath, igs,
				                  image.UnpaintedPath, false);
			gx_path_free(image.UnpaintedPath,
...
zdefineusername(i_ctx_t* i_ctx_p)
{
	...
	if(array_get(imemory, user_names_p, op[-1].value.intval, &uname) >= 0) {
		...
		if(new_size < 100)
			...
			new_size =
			    (old_size > max_array_size / 2 ? max_array_size
			                                   : old_size << 1);
		else
			new_size <<= 1;
		/*
		 ...
		 */
		{
			...
			code =
			    gs_alloc_ref_array(slmem, &new_array, a_all,
			                       new_size, "defineusername(new)");
			if(code < 0)
				return code;
			refcpy_to_new(new_array.value.refs,
			              user_names_p->value.refs, old_size,
			              idmemory);
			refset_null(new_array.value.refs + old_size,
			            new_size - old_size);
			if(old_size)
				gs_free_ref_array(slmem, user_names_p,
				                  "defineusername(old)");
		}
