...
private
int
check_CBFD(const gs_shading_params_t* params, const gs_function_t* function,
           const float* domain, int m)
{
	int ncomp = gs_color_space_num_components(params->ColorSpace);

	if(ncomp < 0 ||
	   (params->have_BBox && (params->BBox.p.x > params->BBox.q.x ||
	...
	if(function != 0) {
		 ...
		 * be a superset of the domain defined in the shading
		 * dictionary.
...
check_mesh(const gs_shading_mesh_params_t* params)
{
	...
	return check_CBFD((const gs_shading_params_t*)params, params->Function,
	                  domain, 1);
}
...
{
	shading_params_init((gs_shading_params_t*)params);
	data_source_init_floats(&params->DataSource, NULL,
	                        0); /* client must set */
...
private
const gs_shading_procs_t shading_Fb_procs = {gs_shading_Fb_fill_rectangle};
int
gs_shading_Fb_init(gs_shading_t** ppsh, const gs_shading_Fb_params_t* params,
                   gs_memory_t* mem)
{
	gs_shading_Fb_t* psh;
	gs_matrix imat;
	...
	if(code < 0 || (code = gs_matrix_invert(&params->Matrix, &imat)) < 0)
		return code;
...
private
const gs_shading_procs_t shading_A_procs = {gs_shading_A_fill_rectangle};
int
gs_shading_A_init(gs_shading_t** ppsh, const gs_shading_A_params_t* params,
                  gs_memory_t* mem)
{
	gs_shading_A_t* psh;
	int code = check_CBFD((const gs_shading_params_t*)params,
	...
	if(code < 0)
		return code;
	ALLOC_SHADING(&st_shading_A, shading_type_Axial, shading_A_procs,
	              "gs_shading_A_init");
	return 0;
}
...
private
const gs_shading_procs_t shading_R_procs = {gs_shading_R_fill_rectangle};
int
gs_shading_R_init(gs_shading_t** ppsh, const gs_shading_R_params_t* params,
                  gs_memory_t* mem)
{
	gs_shading_R_t* psh;
	int code = check_CBFD((const gs_shading_params_t*)params,
	...
	if((params->Domain != 0 && params->Domain[0] == params->Domain[1]) ||
	   params->Coords[2] < 0 || params->Coords[5] < 0)
		return_error(gs_error_rangecheck);
	ALLOC_SHADING(&st_shading_R, shading_type_Radial, shading_R_procs,
	              "gs_shading_R_init");
	return 0;
}
...
private
const gs_shading_procs_t shading_FfGt_procs = {gs_shading_FfGt_fill_rectangle};
int
...
private
const gs_shading_procs_t shading_LfGt_procs = {gs_shading_LfGt_fill_rectangle};
int
...
private
const gs_shading_procs_t shading_Cp_procs = {gs_shading_Cp_fill_rectangle};
int
gs_shading_Cp_init(gs_shading_t** ppsh, const gs_shading_Cp_params_t* params,
                   gs_memory_t* mem)
{
	gs_shading_Cp_t* psh;
	int code = check_mesh((const gs_shading_mesh_params_t*)params);
...
private
const gs_shading_procs_t shading_Tpp_procs = {gs_shading_Tpp_fill_rectangle};
int
gs_shading_Tpp_init(gs_shading_t** ppsh, const gs_shading_Tpp_params_t* params,
                    gs_memory_t* mem)
{
                     ...
                     const gs_matrix_fixed* pmat)
{
	...
	if((code = gs_point_transform2fixed(pmat, pbox->p.x, pbox->p.y, &pt)) <
	       0 ||
                     ...
                     gs_imager_state* pis, bool fill_background)
{
	...
	if((*dev_proc(dev, pattern_manage))(
	       dev, gs_no_id, NULL, pattern_manage__shading_area) == 0) {
		...
		if(psh->params.have_BBox) {
			...
			if((is_xxyy(pmat) || is_xyyx(pmat)) &&
			   (code = gx_dc_pattern2_shade_bbox_transform2fixed(
			        &psh->params.BBox, pis, &bbox_fixed)) >= 0) {
				...
				if(path_box.p.x >= path_box.q.x ||
				   path_box.p.y >= path_box.q.y)
					goto out; /* empty rectangle */
				box_path = gx_path_alloc(
				    mem, "shading_fill_path(box_path)");
				if(box_path == 0) {
					code = gs_note_error(gs_error_VMerror);
					goto out;
				}
				if((code = gx_cpath_from_rectangle(
				        path_clip, &path_box)) < 0 ||
				   (code = shading_path_add_box(
				        box_path, &psh->params.BBox, pmat)) <
				       0 ||
				   (code = gx_cpath_intersect(
				        path_clip, box_path,
				        gx_rule_winding_number, pis)) < 0)
					DO_NOTHING;
				gx_path_free(box_path,
				             "shading_fill_path(box_path)");
				if(code < 0)
					goto out;
				path_clip_set = true;
			}
		...
		if(!path_clip_set) {
			if(path_box.p.x >= path_box.q.x ||
			   path_box.p.y >= path_box.q.y)
				goto out; /* empty rectangle */
			if((code = gx_cpath_from_rectangle(path_clip,
			                                   &path_box)) < 0)
				goto out;
		}
		if(ppath &&
		   (code = gx_cpath_intersect(path_clip, ppath,
		                              gx_rule_winding_number, pis)) < 0)
			goto out;
	...
	if(psh->params.Background && fill_background) {
		...
		(*pcs->type->remap_color)(&cc, pcs, &dev_color, pis, dev,
		                          gs_color_select_texture);

		/****** WRONG IF NON-IDEMPOTENT RasterOp ******/
		code = gx_shade_background(dev, &path_box, &dev_color,
		                           pis->log_op);
		if(code < 0)
			goto out;
	}
	{
		...
		gs_bbox_transform_inverse(&path_rect, (const gs_matrix*)pmat,
		                          &rect);
	}
...
gs_shading_fill_path_adjusted(const gs_shading_t* psh, /*const*/ gx_path* ppath,
                              ...
                              gs_imager_state* pis, bool fill_background)
{
	return gs_shading_fill_path(psh, ppath, prect, orig_dev, pis,
	                            fill_background);
}
