...
private
dev_proc_get_params(jpeg_get_params);
private
dev_proc_put_params(jpeg_put_params);
private
dev_proc_print_page(jpeg_print_page);
private
dev_proc_map_color_rgb(jpegcmyk_map_color_rgb);
private
dev_proc_map_cmyk_color(jpegcmyk_map_cmyk_color);
...
private
const gx_device_procs jpeg_procs = prn_color_params_procs(
    gdev_prn_open, gdev_prn_output_page, gdev_prn_close,
    gx_default_rgb_map_rgb_color, gx_default_rgb_map_color_rgb, jpeg_get_params,
    jpeg_put_params);

const gx_device_jpeg gs_jpeg_device = {
    prn_device_std_body(gx_device_jpeg, jpeg_procs, "jpeg", DEFAULT_WIDTH_10THS,
                        DEFAULT_HEIGHT_10THS, X_DPI, Y_DPI, 0, 0, 0, 0, 24,
                        jpeg_print_page),
...
private
const gx_device_procs jpeggray_procs = prn_color_params_procs(
    gdev_prn_open, gdev_prn_output_page, gdev_prn_close,
...
const gx_device_jpeg gs_jpeggray_device = {
    prn_device_body(gx_device_jpeg, jpeggray_procs, "jpeggray",
                    DEFAULT_WIDTH_10THS, DEFAULT_HEIGHT_10THS, X_DPI, Y_DPI, 0,
                    0, 0, 0, 1, 8, 255, 0, 256, 0, jpeg_print_page),
    0,  /* JPEGQ: 0 indicates not specified */
    0.0 /* QFactor: 0 indicates not specified */
};
...
private
const gx_device_procs jpegcmyk_procs = {
    gdev_prn_open, gx_default_get_initial_matrix, NULL, /* sync_output */
    gdev_prn_output_page, gdev_prn_close, NULL, jpegcmyk_map_color_rgb,
    NULL, /* fill_rectangle */
    NULL, /* tile_rectangle */
    NULL, /* copy_mono */
    NULL, /* copy_color */
    NULL, /* draw_line */
    NULL, /* get_bits */
    jpeg_get_params, jpeg_put_params, jpegcmyk_map_cmyk_color,
...
{
	int not_k = color & 0xff, r = not_k - ~(color >> 24),
...
jpeg_print_page(gx_device_printer* pdev, FILE* prn_stream)
{
	...
	jpeg_compress_data* jcdp = gs_alloc_struct_immovable(
	    mem, jpeg_compress_data, &st_jpeg_compress_data,
	...
	if(jdev->JPEGQ > 0) {
		...
		code = gs_jpeg_set_linear_quality(
		    &state, (int)(min(jdev->QFactor, 100.0) * 100.0 + 0.5),
		    TRUE);
		if(code < 0)
			goto done;
	}
	...
	state.scan_line_size =
	    jcdp->cinfo.input_components * jcdp->cinfo.image_width;
	...
	if((fbuf = gs_alloc_bytes(mem, fbuf_size, "jpeg_print_page(fbuf)")) ==
	       0 ||
	   (jbuf = gs_alloc_bytes(mem, jbuf_size, "jpeg_print_page(jbuf)")) ==
	       0) {
		code = gs_note_error(gs_error_VMerror);
		goto done;
	}
