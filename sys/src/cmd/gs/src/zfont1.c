...
private
font_proc_same_font(z1_same_font);
...
int
charstring_font_params(const gs_memory_t* mem, const_os_ptr op,
                       charstring_font_refs_t* pfr, gs_type1_data* pdata1)
{
	...
	if((code = dict_int_param(pprivate, "lenIV", -1, 255, pdata1->lenIV,
	                          &pdata1->lenIV)) < 0 ||
	   (code = dict_uint_param(pprivate, "subroutineNumberBias", 0,
	                           max_uint, pdata1->subroutineNumberBias,
	   ...
	   (code = dict_float_param(pprivate, "BlueShift", 7.0,
	                            &pdata1->BlueShift)) < 0 ||
	   (code = pdata1->BlueValues.count = dict_float_array_param(
	        mem, pprivate, "BlueValues", max_BlueValues * 2,
	        &pdata1->BlueValues.values[0], NULL)) < 0 ||
	   (code = dict_float_param(pprivate, "ExpansionFactor", 0.06,
	                            &pdata1->ExpansionFactor)) < 0 ||
	   (code = pdata1->FamilyBlues.count = dict_float_array_param(
	        mem, pprivate, "FamilyBlues", max_FamilyBlues * 2,
	        &pdata1->FamilyBlues.values[0], NULL)) < 0 ||
	   (code = pdata1->FamilyOtherBlues.count = dict_float_array_param(
	        mem, pprivate, "FamilyOtherBlues", max_FamilyOtherBlues * 2,
	    ...
	    */
	   (code = dict_int_param(pprivate, "LanguageGroup", min_int, max_int,
	                          0, &pdata1->LanguageGroup)) < 0 ||
	   (code = pdata1->OtherBlues.count = dict_float_array_param(
	        mem, pprivate, "OtherBlues", max_OtherBlues * 2,
	        &pdata1->OtherBlues.values[0], NULL)) < 0 ||
	   (code = dict_bool_param(pprivate, "RndStemUp", true,
	                           &pdata1->RndStemUp)) < 0 ||
	   (code = pdata1->StdHW.count = dict_float_array_check_param(
	        mem, pprivate, "StdHW", 1, &pdata1->StdHW.values[0], NULL, 0,
	        e_rangecheck)) < 0 ||
	   (code = pdata1->StdVW.count = dict_float_array_check_param(
	        mem, pprivate, "StdVW", 1, &pdata1->StdVW.values[0], NULL, 0,
	        e_rangecheck)) < 0 ||
	   (code = pdata1->StemSnapH.count = dict_float_array_param(
	        mem, pprivate, "StemSnapH", max_StemSnap,
	        &pdata1->StemSnapH.values[0], NULL)) < 0 ||
	   (code = pdata1->StemSnapV.count = dict_float_array_param(
	        mem, pprivate, "StemSnapV", max_StemSnap,
	        &pdata1->StemSnapV.values[0], NULL)) < 0 ||
	   /* The WeightVector is in the font dictionary, not Private. */
	   (code = pdata1->WeightVector.count = dict_float_array_param(
	        mem, op, "WeightVector", max_WeightVector,
                      ...
                      gs_type1_data* pdata1, build_font_options_t options)
{
	...
	if(code < 0)
		return code;
	code =
	    build_gs_primitive_font(i_ctx_p, op, (gs_font_base**)&pfont, ftype,
...
zbuildfont1(i_ctx_t* i_ctx_p)
{
	...
	int code = build_proc_name_refs(imemory, &build, "%Type1BuildChar",
	                                "%Type1BuildGlyph");
...
zbuildfont4(i_ctx_t* i_ctx_p)
{
	...
	if(code < 0)
		return code;
	return buildfont1or4(i_ctx_p, op, &build, ft_disk_based,
	                     bf_options_none);
}

/* ------ Initialization procedure ------ */

const op_def zfont1_op_defs[] = {{"2.buildfont1", zbuildfont1},
...
private
bool
same_font_dict(const font_data* pdata, const font_data* podata, const char* key)
{
	ref* pvalue;
...
z1_same_font(const gs_font* font, const gs_font* ofont, int mask)
{
		...
		return mask;
	{
		...
		if((check & (FONT_SAME_OUTLINES | FONT_SAME_METRICS)) &&
		   !memcmp(&pofont1->data.procs, &z1_data_procs,
		           sizeof(z1_data_procs)) &&
		   obj_eq(font->memory, &pdata->CharStrings,
		          &podata->CharStrings) &&
		   /*
