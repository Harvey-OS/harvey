 ...
 */
void
gx_hld_saved_color_init(gx_hl_saved_color* psc)
{
 ...
 */
const gs_state*
gx_hld_get_gstate_ptr(const gs_imager_state* pis)
{
                  ...
                  gx_hl_saved_color* psc)
{
	...
	if(pdevc == NULL) {
		...
		/*
		 * Have color space, save id,  ccolor, & device color specific
		 * info.
		 * Also save the high level colors since two gx_color_index
		 * values
		 * may be the same but for differing high level colors (due to
		 * the
		...
		i = any_abs(i);
		for(i--; i >= 0; i--)
			psc->ccolor.paint.values[i] =
			    pdevc->ccolor.paint.values[i];
 ...
 */
bool
gx_hld_saved_color_equal(const gx_hl_saved_color* psc1,
 ...
 */
bool
gx_hld_saved_color_same_cspace(const gx_hl_saved_color* psc1,
                               const gx_hl_saved_color* psc2)
{
...
gx_hld_get_color_component(const gs_imager_state* pis,
                           const gx_device_color* pdevc, int comp_num,
                           float* output)
{
