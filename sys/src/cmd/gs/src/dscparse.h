...
typedef struct CDSCPAGE_S {
	...
	CDSCBBOX*
	    bbox; /* PageBoundingBox, also used by GSview for PDF CropBox */
...
CDSC* dsc_init_with_alloc(void* caller_data,
                          void* (*memalloc)(size_t size, void* closure_data),
                          void (*memfree)(void* ptr, void* closure_data),
                          void* closure_data);
...
void dsc_set_debug_function(CDSC* dsc, void (*debug_fn)(void* caller_data,
                                                        const char* str));
...
int dsc_set_page_bbox(CDSC* dsc, unsigned int page_number, int llx, int lly,
                      int urx, int ury);
