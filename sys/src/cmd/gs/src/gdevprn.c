...
#define BACKTRACE(first_arg)                                                   \
	...
	for(space = space_params->BufferSpace;;) {
		base = (reallocate ? (byte *)gs_resize_object(buffer_memory, *the_memory, space,
							      "cmd list buffer")
...
open_c:
	...
	clist_init_params(pclist_dev, base, space, pdev,
			  ...
			  (ppdev->bandlist_memory == 0 ? pdev->memory->non_gc_memory : ppdev->bandlist_memory),
		  ...
		  int new_width, int new_height, bool reallocate)
{
	...
	gs_memory_t *buffer_memory =
	    (ppdev->buffer_memory == 0 ? pdev->memory->non_gc_memory : ppdev->buffer_memory);
	...
	for(pass = 1; pass <= (reallocate ? 2 : 1); ++pass) {
		...
		ppdev->printer_procs.buf_procs.size_buf_device(&buf_space, pdev, NULL, pdev->height, false);
		if(ppdev->page_uses_transparency)
			pdf14_trans_buffer_size = new_height * (ESTIMATED_PDF14_ROW_SPACE(new_width) >> 3);
		mem_space = buf_space.bits + buf_space.line_ptrs + pdf14_trans_buffer_size;
		...
		if(!is_command_list) {
			...
			base =
			    (reallocate ? (byte *)gs_resize_object(buffer_memory, the_memory,
		...
		if(!is_command_list && pass == 1 && PRN_MIN_MEMORY_LEFT != 0 && buffer_memory == pdev->memory->non_gc_memory) {
		...
		if(is_command_list) {
			...
			ppdev->buffer_space = 0;
			if((code = gdev_create_buf_device(ppdev->printer_procs.buf_procs.create_buf_device,
							  &bdev, pdev, NULL, NULL, false)) < 0 ||
			   (code = ppdev->printer_procs.buf_procs.setup_buf_device(bdev, base, buf_space.raster,
...
gdev_prn_get_params(gx_device *pdev, gs_param_list *plist)
{
	...
	if(code < 0 ||
	   ...
	   (ppdev->Duplex_set >= 0 &&
	    (code = (ppdev->Duplex_set ? param_write_bool(plist, "Duplex", &ppdev->Duplex) : param_write_null(plist, "Duplex"))) < 0))
		return code;
    ...
    gdev_prn_output_page(gx_device *pdev, int num_copies, int flush)
{
	...
	endcode = (ppdev->buffer_space && !ppdev->is_async_renderer ? clist_finish_page(pdev, flush) : 0);
			     ...
			     int num_copies)
{
	 ...
	 */
		...
		pdev->PageCount++;
		code = (errcode < 0 ? errcode : closecode < 0 ? closecode : gdev_prn_open_printer((gx_device *)pdev, true));
		if(code < 0) {
			pdev->PageCount -= i;
			return code;
		}
		     ...
		     gx_colors_used_t *colors_used, int *range_start)
{
	...
	return gx_page_info_colors_used(dev, &cldev->page_info,
			   ...
			   int height, bool for_band)
{
	...
	mdev.color_info.depth =
	    (render_plane && render_plane->index >= 0 ? render_plane->depth : target->color_info.depth);
	mdev.width = target->width;
	mdev.num_planes = 0;
...
{
	gx_device_memory *mdev =
	    (gs_device_is_memory(bdev) ? (gx_device_memory *)bdev : (gx_device_memory *)(((gx_device_plane_extract *)bdev)->plane_dev));
	byte **ptrs = line_ptrs;
	 ...
	 */
		ptrs = (byte **)
		    gs_alloc_byte_array(mdev->memory,
					(mdev->num_planes ? full_height * mdev->num_planes : setup_height),
