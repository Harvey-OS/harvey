...
private
int /* returns 0 ok, else -ve error cde */
...
#define BACKTRACE(first_arg)                                                   \
	...
	for(space = space_params->BufferSpace;;) {
		base = (reallocate ? (byte*)gs_resize_object(buffer_memory,
		                                             *the_memory, space,
		                                             "cmd list buffer")
...
open_c:
	...
	clist_init_params(
	    pclist_dev, base, space, pdev, ppdev->printer_procs.buf_procs,
	...
	if(code < 0) {
		...
		if(code == gs_error_limitcheck &&
		   ...
		   !bufferSpace_is_exact) {
			space <<= 1;
			if(reallocate) {
				base = gs_resize_object(
				    buffer_memory, *the_memory, space,
...
private
bool /* ret true if device was cmd list, else false */
                  ...
                  int new_width, int new_height, bool reallocate)
{
	...
	for(pass = 1; pass <= (reallocate ? 2 : 1); ++pass) {
		...
		/* Compute desired space params: never use the space_params
		 * as-is. */
		...
		if(ppdev->is_async_renderer &&
		   space_params.band.BandBufferSpace != 0)
			...
			if(space_params.band.BandBufferSpace > 0)
				...
				space_params.BufferSpace =
				    ppdev->space_params.BufferSpace;
				bufferSpace_is_default = true;
			}
		}

		/* Determine if we can use a full bitmap buffer, or have to use
		 * banding */
		if(pass > 1)
			...
			is_command_list =
			    ...
			    mem_space !=
			        (uint)mem_space; /* too big to allocate */
		}
		if(!is_command_list) {
			/* Try to allocate memory for full memory buffer */
			base =
			    (reallocate ? (byte*)gs_resize_object(
			                      buffer_memory, the_memory,
		...
		if(!is_command_list && pass == 1 && PRN_MIN_MEMORY_LEFT != 0 &&
		   buffer_memory == pdev->memory->non_gc_memory) {
			/* before using full memory buffer, ensure enough
			 * working mem left */
			byte* left =
			    gs_alloc_bytes(buffer_memory, PRN_MIN_MEMORY_LEFT,
			                   "printer mem left");
			if(left == 0)
				is_command_list = true;
			else
				gs_free_object(buffer_memory, left,
				               "printer mem left");
		}
		...
		if(is_command_list) {
			...
			code = gdev_prn_setup_as_command_list(
			    pdev, buffer_memory, &the_memory, &space_params,
			...
			if((code = gdev_create_buf_device(
			   ...
			   (code =
			        ppdev->printer_procs.buf_procs.setup_buf_device(
			            bdev, base, buf_space.raster,
			            (byte**)(base + buf_space.bits), 0,
			            pdev->height, pdev->height)) < 0) {
...
#define COPY_PROC(p) set_dev_proc(ppdev, p, ppdev->orig_procs.p)
		...
		set_dev_proc(ppdev, get_page_device,
...
gdev_prn_allocate_memory(gx_device* pdev,
                         gdev_prn_space_params* new_space_params, int new_width,
                         int new_height)
{
	return gdev_prn_allocate(pdev, new_space_params, new_width, new_height,
	                         false);
}
...
gdev_prn_get_params(gx_device* pdev, gs_param_list* plist)
{
	...
	if(code < 0 ||
	   (code = param_write_long(plist, "MaxBitmap",
	                            &ppdev->space_params.MaxBitmap)) < 0 ||
	   (code = param_write_long(plist, "BufferSpace",
	                            &ppdev->space_params.BufferSpace)) < 0 ||
	   (code = param_write_int(plist, "BandWidth",
	                           &ppdev->space_params.band.BandWidth)) < 0 ||
	   (code = param_write_int(plist, "BandHeight",
	                           &ppdev->space_params.band.BandHeight)) < 0 ||
	   (code = param_write_long(
	        plist, "BandBufferSpace",
	        &ppdev->space_params.band.BandBufferSpace)) < 0 ||
	   (code = param_write_bool(plist, "OpenOutputFile",
	                            &ppdev->OpenOutputFile)) < 0 ||
	   (code = param_write_bool(plist, "ReopenPerPage",
	                            &ppdev->ReopenPerPage)) < 0 ||
	...
	ofns.data = (const byte*)ppdev->fname, ofns.size = strlen(ppdev->fname),
...
gdev_prn_put_params(gx_device* pdev, gs_param_list* plist)
{
	...
	switch(code = param_read_bool(plist, (param_name = "OpenOutputFile"),
	                              &oof)) {
	default:
		ecode = code;
	...
	switch(code = param_read_bool(plist, (param_name = "ReopenPerPage"),
	                              &rpp)) {
	default:
		ecode = code;
		param_signal_error(plist, param_name, ecode);
	...
	switch(code =
	           param_read_bool(plist, (param_name = "PageUsesTransparency"),
...
#define CHECK_PARAM_CASES(member, bad, label)                                  \
	case 0:                                                                \
		if((sp.params_are_read_only ? sp.member != save_sp.member      \
		                            : bad))                            \
	...
	switch(code = param_read_long(plist, (param_name = "MaxBitmap"),
	                              &sp.MaxBitmap)) {
		CHECK_PARAM_CASES(MaxBitmap, sp.MaxBitmap < 10000, mbe);
	}

	switch(code = param_read_long(plist, (param_name = "BufferSpace"),
	                              &sp.BufferSpace)) {
		CHECK_PARAM_CASES(BufferSpace, sp.BufferSpace < 10000, bse);
	}

	switch(code = param_read_int(plist, (param_name = "BandWidth"),
	                             &sp.band.BandWidth)) {
		CHECK_PARAM_CASES(band.BandWidth, sp.band.BandWidth < 0, bwe);
	}

	switch(code = param_read_int(plist, (param_name = "BandHeight"),
	                             &sp.band.BandHeight)) {
		CHECK_PARAM_CASES(band.BandHeight, sp.band.BandHeight < 0, bhe);
	}

	switch(code = param_read_long(plist, (param_name = "BandBufferSpace"),
	                              &sp.band.BandBufferSpace)) {
		CHECK_PARAM_CASES(band.BandBufferSpace,
		                  sp.band.BandBufferSpace < 0, bbse);
	}

	switch(code = param_read_string(plist, (param_name = "OutputFile"),
	                                &ofs)) {
	case 0:
		if(pdev->LockSafetyParams &&
		   bytes_compare(ofs.data, ofs.size, (const byte*)ppdev->fname,
		                 strlen(ppdev->fname))) {
...
#define read_media(pname)                                                      \
	switch(code = param_begin_read_dict(plist, (param_name = pname),       \
	                                    &mdict, true)) {                   \
	case 0:                                                                \
	...
	if(ofs.data != 0 &&
	   bytes_compare(ofs.data, ofs.size, (const byte*)ppdev->fname,
		...
		if(ppdev->file != NULL)
			gx_device_close_output_file(pdev, ppdev->fname,
			                            ppdev->file);
		ppdev->file = NULL;
    ...
    gdev_prn_output_page(gx_device* pdev, int num_copies, int flush)
{
	...
	if(num_copies > 0 || !flush) {
		...
		if(!flush &&
		   (*ppdev->printer_procs.buffer_page)(ppdev, ppdev->file,
		                                       num_copies) >= 0) {
			upgraded_copypage = true;
			flush = true;
		} else if(num_copies > 0)
			/* Print the accumulated page description. */
			outcode = (*ppdev->printer_procs.print_page_copies)(
			    ppdev, ppdev->file, num_copies);
		fflush(ppdev->file);
		errcode =
                             ...
                             const gx_device* dev)
{
	...
	if(first_bit & 7)
		bits_fill_rectangle(data, first_bit, raster,
		                    mono_fill_make_pattern(0), -first_bit & 7,
		                    height);
}
...
int
gdev_prn_open_printer_seekable(gx_device* pdev, bool binary_mode, bool seekable)
{
	gx_device_printer* const ppdev = (gx_device_printer*)pdev;
	...
	{
		int code = gx_device_open_output_file(
		    pdev, ppdev->fname, binary_mode, seekable, &ppdev->file);
		if(code < 0)
			return code;
	}
                     ...
                     gx_colors_used_t* colors_used, int* range_start)
{
	...
	return gx_page_info_colors_used(dev, &cldev->page_info, y, height,
	                                colors_used, range_start);
}
...
gdev_create_buf_device(create_buf_device_proc_t cbd_proc, gx_device** pbdev,
                       gx_device* target, const gx_render_plane_t* render_plane,
                             ...
                             gs_memory_t* mem, bool for_band)
{
	...
	if(target == (gx_device*)mdev) {
		/* The following is a special hack for setting up printer
		 * devices. */
	...
	if(plane_index >= 0) {
		gx_device_plane_extract* edev = gs_alloc_struct(
		    mem, gx_device_plane_extract, &st_device_plane_extract,
		    "create_buf_device");
...
gx_default_size_buf_device(gx_device_buf_space_t* space, gx_device* target,
                           const gx_render_plane_t* render_plane, int height,
                           bool for_band)
{
	gx_device_memory mdev;

	mdev.color_info.depth = (render_plane && render_plane->index >= 0
	                             ? render_plane->depth
	...
	space->line_ptrs =
	    gdev_mem_line_ptrs_size(&mdev, target->width, height);
...
{
	gx_device_memory* mdev =
	    (gs_device_is_memory(bdev)
	         ? (gx_device_memory*)bdev
	         : (gx_device_memory*)(((gx_device_plane_extract*)bdev)
	                                   ->plane_dev));
	byte** ptrs = line_ptrs;
	...
	if(ptrs == 0) {
		/*
		 * Allocate line pointers now; free them when we close the
		 * device.
		 * Note that for multi-planar devices, we have to allocate using
		 * full_height rather than setup_height.
		 */
		ptrs = (byte**)gs_alloc_byte_array(
		    mdev->memory,
		    (mdev->num_planes ? full_height * mdev->num_planes
		                      : setup_height),
 ...
 */
int
gdev_prn_get_lines(gx_device_printer* pdev, int y, int height, byte* buffer,
                   ...
                   const gx_render_plane_t* render_plane)
{
	...
	params.options = GB_RETURN_POINTER | GB_ALIGN_STANDARD | GB_OFFSET_0 |
	                 GB_RASTER_ANY |
	                 /* No depth specified, we always use native colors. */
	                 GB_COLORS_NATIVE | GB_ALPHA_NONE;
	if(render_plane) {
		params.options |= GB_PACKING_PLANAR | GB_SELECT_PLANES;
		memset(params.data, 0, sizeof(params.data[0]) *
	...
	if(code < 0 && actual_buffer) {
		...
		code = dev_proc(pdev, get_bits_rectangle)((gx_device*)pdev,
		                                          &rect, &params, NULL);
	}
	if(code < 0)
		return code;
...
{
	int code =
	    (*dev_proc(pdev, get_bits))((gx_device*)pdev, y, str, actual_data);
...
gdev_prn_maybe_realloc_memory(gx_device_printer* prdev,
                              gdev_prn_space_params* old_sp, int old_width,
                              int old_height, bool old_page_uses_transparency)
{
	int code = 0;
	...
	if(prdev->is_open &&
	   (memcmp(&prdev->space_params, old_sp, sizeof(*old_sp)) != 0 ||
...
#ifdef DEBUGGING_HACKS
		debug_dump_bytes((const byte*)old_sp, (const byte*)(old_sp + 1),
		                 "old");
		debug_dump_bytes((const byte*)&prdev->space_params,
		                 (const byte*)(&prdev->space_params + 1),
		                 "new");
		...
		code = gdev_prn_reallocate_memory(pdev, &new_sp, new_width,
		                                  new_height);
