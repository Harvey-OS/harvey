...
/* Forward references */
private
int sput_word(stream* dest, uint value);
private
int sput_bytes(stream* dest, const byte* data, uint size);
	...
	while((code = param_get_next_key(list, &key_enum, &key)) == 0) {
		...
		string_array:
			sput_word(dest, size);
			{
				...
				for(count = size, sa = data; count-- > 0;
				    ++sa) {
		...
		case gs_param_type_array:
			...
			{
				int end_code = param_end_read_dict(
				    list, key.data, &value.value.d);
...
/* Forward references */
private
int sget_word(stream* src, uint* pvalue);
private
int sget_bytes(stream* src, byte* data, uint size);
...
gs_param_list_gets(stream* src, gs_param_list* list, gs_memory_t* mem)
{
	...
	do {
		...
		case gs_param_type_long:
		case gs_param_type_float:
			code = sget_bytes(src, (byte*)&typed.value,
			                  value_top_sizeof);
		case gs_param_type_null:
			goto put;
		default:
		...
		case gs_param_type_int_array:
		case gs_param_type_float_array:
			data = (value_base_sizeof == 1
			            ? gs_alloc_string(mem, size,
			                              "param string/name")
			            : gs_alloc_byte_array(
			                  mem, size, value_base_sizeof,
			                  "param scalar array"));
			if(data == 0) {
		...
		case gs_param_type_name_array:
			...
			typed.value.sa.size = size;
			{
				...
				for(index = 0; index < size; ++index, ++sa) {
					...
					code =
					    sget_bytes(src, str_data, str_size);
					if(code < 0)
						break;
				}
			}
		...
		case gs_param_type_array:
			...
			{
				int end_code = param_end_write_collection(
				    list, string_key, &typed.value.d);
