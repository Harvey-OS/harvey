...
gs_param_list_gets(stream *src, gs_param_list *list, gs_memory_t *mem)
{
	...
	do {
		...
		case gs_param_type_float_array:
			data =
			    (value_base_sizeof == 1 ? gs_alloc_string(mem, size, "param string/name") : gs_alloc_byte_array(mem, size, value_base_sizeof,
															    "param scalar array"));
			if(data == 0) {
		...
		case gs_param_type_array:
			...
			code = param_begin_write_collection(list, string_key, &typed.value.d,
