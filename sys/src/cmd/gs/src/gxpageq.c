...
struct gx_page_queue_s {
	...
	gx_semaphore_t* render_done_sema; /* semaphore signaled when (partial)
	                                     page rendered */
 ...
 */
private
const gx_band_page_info_t null_page_info = {PAGE_INFO_NULL_VALUES};

#define private_st_gx_page_queue()                                             \
	gs_private_st_ptrs4(st_gx_page_queue, gx_page_queue_t,                 \
	                    "gx_page_queue", gx_page_queue_enum_ptrs,          \
	                    gx_page_queue_reloc_ptrs, monitor, first_in,       \
...
/* ------------ Forward Decl's --------------------------- */
private
gx_page_queue_entry_t* /* removed entry, 0 if none avail */
...
{
	gx_page_queue_entry_t* entry = gs_alloc_struct(
	    queue->memory, gx_page_queue_entry_t, &st_gx_page_queue_entry,
	    "gx_page_queue_entry_alloc");
...
void
gx_page_queue_entry_free(gx_page_queue_entry_t* entry /* entry to free up */
                         )
{
...
int                                            /* -ve error code, or 0 */
    gx_page_queue_init(gx_page_queue_t* queue, /* page queue to init */
                       gs_memory_t* memory /* allocator for dynamic memory */
                       )
{
	queue->memory = memory;
	queue->monitor =
	    gx_monitor_alloc(memory); /* alloc monitor to serialize */
	queue->entry_count = 0;
	...
	if(queue->monitor && queue->render_req_sema &&
	   queue->render_done_sema && queue->reserve_entry)
		return 0;
	else {
...
void
gx_page_queue_dnit(gx_page_queue_t* queue /* page queue to dnit */
                   )
{
...
int /* rets 0 if no pages were waiting for rendering, 1 if actually waited */
    gx_page_queue_wait_one_page(gx_page_queue_t* queue /* queue to wait on */
                                )
{
	int code;
...
void
gx_page_queue_enqueue(gx_page_queue_entry_t* entry /* entry to add */
                      )
{
	gx_page_queue_t* queue = entry->queue;
...
int /* rets 0 ok, gs_error_VMerror if error */
    gx_page_queue_add_page(
        ...
        const gx_band_page_info_t*
            page_info, /* bandinfo incl. bandlist (or 0) */
        ...
        )
{
	...
	gx_page_queue_entry_t* entry = gx_page_queue_entry_alloc(queue);
	...
	while(!queue->reserve_entry) {
		queue->reserve_entry = gx_page_queue_entry_alloc(queue);
		if(!queue->reserve_entry &&
		   !gx_page_queue_wait_one_page(queue)) {
			/* Should never happen: all pages rendered & still can't
			 * get memory: give up! */
    ...
    )
{
	 ...
	 * are managed together, so free the clist just before freeing the
	 * entry.
	 */
	gx_page_queue_entry_free_page_info(entry);
	gx_page_queue_entry_free(entry);
