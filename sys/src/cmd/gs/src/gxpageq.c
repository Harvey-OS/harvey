...
{
	gx_page_queue_entry_t *entry = gs_alloc_struct(queue->memory, gx_page_queue_entry_t,
	...
	)
{
	...
	if(queue->monitor && queue->render_req_sema && queue->render_done_sema && queue->reserve_entry)
		return 0;
	else {
