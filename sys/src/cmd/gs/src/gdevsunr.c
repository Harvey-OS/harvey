...
private
dev_proc_print_page(sunhmono_print_page);

const gx_device_printer gs_sunhmono_device =
    prn_device(prn_std_procs, "sunhmono", DEFAULT_WIDTH_10THS,
...
sunhmono_print_page(gx_device_printer* pdev, FILE* prn_stream)
{
	...
	int lineCnt;
	char* lineStorage; /* Allocated for passing storage to
	                      gdev_prn_get_bits() */
	byte* data;
	...
	*/
	lineStorage = gs_malloc(pdev->memory, gsLineBytes, 1,
	                        "rasterfile_print_page(in)");
	if(lineStorage == 0) {
	...
	for(lineCnt = 0; lineCnt < pdev->height; ++lineCnt) {
		...
		if(gsLineBytes % 2)
			fputc(0,
			      prn_stream); /* pad to even # of bytes with a 0 */
	}
	/* The weird file terminator */
	fwrite("};\n", 1, 3, prn_stream);
out:
	/* Clean up... */
	gs_free(pdev->memory, lineStorage, gsLineBytes, 1,
	        "rasterfile_print_page(in)");
	return code;
}
