...
/* Copyright (C) 1995, 1996, 1998, 1999 Aladdin Enterprises.  All rights
  reserved.
...
bhc_setup(os_ptr op, stream_BHC_state* pbhcs)
{
	...
	if((code = dict_bool_param(op, "FirstBitLowOrder", false,
	   ...
	   (code = dict_bool_param(op, "EndOfData", true, &pbhcs->EndOfData)) <
	       0 ||
	   (code = dict_uint_param(op, "EncodeZeroRuns", 2, 256, 256,
	                           &pbhcs->EncodeZeroRuns)) < 0 ||
	   /* Note: the code returned from the following call */
	   /* is actually the number of elements in the array. */
	   (code = dict_int_array_param(op, "Tables", countof(data), data)) <=
	       0)
	...
	pbhcs->definition.counts = counts = (uint16_t*)ialloc_byte_array(
	    num_counts + 1, sizeof(uint16_t), "bhc_setup(counts)");
	pbhcs->definition.values = values = (uint16_t*)ialloc_byte_array(
	    num_values, sizeof(uint16_t), "bhc_setup(values)");
...
zBHCD(i_ctx_t* i_ctx_p)
{
	...
	if(code < 0)
		return code;
	return filter_read(i_ctx_p, 0, &s_BHCD_template, (stream_state*)&bhcs,
	                   0);
}
...
zcomputecodes(i_ctx_t* i_ctx_p)
{
	...
	data = (uint16_t*)gs_alloc_byte_array(imemory, asize, sizeof(uint16_t),
	                                      "zcomputecodes");
	freqs = (int32_t*)gs_alloc_byte_array(
	    imemory, def.num_values, sizeof(int32_t), "zcomputecodes(freqs)");
...
{
	int code =
	    dict_int_param(op, "BlockSize", 1, max_int / sizeof(int) - 10,
	                   16384, &pbwbss->BlockSize);
...
zBWBSD(i_ctx_t* i_ctx_p)
{
	...
	if(code < 0)
		return code;
	return filter_read(i_ctx_p, 0, &s_BWBSD_template, (stream_state*)&bwbss,
	                   0);
}
