...
private
int gc_trace_chunk(const gs_memory_t *mem, chunk_t *, gc_state_t *, gc_mark_stack *);
private
...
#define for_roots(n, mem, rp) \
	for_spaces(ispace, n) for(mem = space_memories[ispace], rp = mem->roots; rp != 0; rp = rp->next)
...

	{
		...
		for_chunks(max_trace, mem, cp)
		{
			...
			if(avail >= sizeof(gc_mark_stack) + sizeof(ms_entry) * ms_size_min &&
			   !cp->inner_count) {
...
#define mark_name(nidx)                                                                                 \
	...
	ts:
		...
		if(mproc == gs_no_struct_enum_ptrs ||
		   (ptp = (*mproc)(gcst_get_memory_ptr(pstate), ptr, pre_obj_contents_size(ptr - 1),
	...
	tr:
		...
		rr:
			if((sp[1].index = r_size(rptr)) == 0) { /* Set the base pointer to 0, */
...
gc_objects_set_reloc(chunk_t *cp)
{
	...
	if((procs == 0 ? o_is_unmarked(pre) : !(*procs->set_reloc)(pre, reloc, size))) { /* Free object */
...
#endif
			{
				const obj_header_t *pfree = (const obj_header_t *)((const char *)(optr - 1) -
										   (back << obj_back_shift));
				const chunk_head_t *chead = (const chunk_head_t *)((const char *)pfree -
