...
private
const bool I_FORCE_GLOBAL_GC = false;

/* Define whether to bypass the collector entirely. */
private
const bool I_BYPASS_GC = false;
...
private
void gc_init_mark_stack(gc_mark_stack*, uint);
private
void gc_objects_clear_marks(const gs_memory_t* mem, chunk_t*);
private
void gc_unmark_names(name_table*);
private
int gc_trace(gs_gc_root_t*, gc_state_t*, gc_mark_stack*);
private
int gc_rescan_chunk(chunk_t*, gc_state_t*, gc_mark_stack*);
private
int gc_trace_chunk(const gs_memory_t* mem, chunk_t*, gc_state_t*,
                   gc_mark_stack*);
private
bool gc_trace_finish(gc_state_t*);
private
void gc_clear_reloc(chunk_t*);
private
void gc_objects_set_reloc(chunk_t*);
private
void gc_do_reloc(chunk_t*, gs_ref_memory_t*, gc_state_t*);
private
void gc_objects_compact(chunk_t*, gc_state_t*);
private
void gc_free_empty_chunks(gs_ref_memory_t*);

/* Forward references for pointer types */
private
ptr_proc_unmark(ptr_struct_unmark);
private
ptr_proc_mark(ptr_struct_mark);
private
ptr_proc_unmark(ptr_string_unmark);
private
ptr_proc_mark(ptr_string_mark);
/*ptr_proc_unmark(ptr_ref_unmark); */ /* in igc.h */
/*ptr_proc_mark(ptr_ref_mark); */     /* in igc.h */
private
ptr_proc_reloc(igc_reloc_struct_ptr, void);
...
private
const gc_procs_with_refs_t igc_procs = {
...
const gs_ptr_procs_t ptr_string_procs = {ptr_string_unmark, ptr_string_mark,
                                         NULL};
const gs_ptr_procs_t ptr_const_string_procs = {ptr_string_unmark,
                                               ptr_string_mark, NULL};
const gs_ptr_procs_t ptr_ref_procs = {ptr_ref_unmark, ptr_ref_mark, NULL};
...
#define for_spaces(i, n) for(i = 1; i <= n; ++i)
#define for_collected_spaces(i) for(i = min_collect; i <= max_trace; ++i)
#define for_space_mems(i, mem)                                                 \
	for(mem = space_memories[i]; mem != 0; mem = &mem->saved->state)
#define for_mem_chunks(mem, cp) for(cp = (mem)->cfirst; cp != 0; cp = cp->cnext)
...
#define for_roots(n, mem, rp)                                                  \
	for_spaces(ispace, n) for(mem = space_memories[ispace],                \
	                          rp = mem->roots;                             \
	                          rp != 0; rp = rp->next)
	...
	for_spaces(ispace, max_trace) gs_register_struct_root(
	    (gs_memory_t*)space_memories[ispace], &space_roots[ispace],
	    (void**)&space_memories[ispace], "gc_top_level");
...

	{
		...
		for_chunks(max_trace, mem, cp)
		{
			...
			if(avail >= sizeof(gc_mark_stack) +
			                sizeof(ms_entry) * ms_size_min &&
			   !cp->inner_count) {
				gc_mark_stack* pms = (gc_mark_stack*)cp->cbot;

				gc_init_mark_stack(
				    pms, (avail - sizeof(gc_mark_stack)) /
				             sizeof(ms_entry));
				end->next = pms;
				pms->prev = end;
				pms->on_heap = false;
...

	{
		...
		if(!global)
			for_chunks(min_collect - 1, mem, cp) more |=
			    gc_trace_chunk((const gs_memory_t*)mem, cp, &state,
			                   mark_stack);
		...
		while(more < 0) { /* stack overflowed */
			more = 0;
			for_chunks(max_trace, mem, cp) more |=
			    gc_rescan_chunk(cp, &state, mark_stack);
		}
	...
	for_chunks(min_collect - 1, mem, cp) gc_clear_reloc(cp);
	...
	for_chunks(min_collect - 1, mem, cp) gc_do_reloc(cp, mem, &state);
	state.relocating_untraced = false;
	for_collected_chunks(mem, cp) gc_do_reloc(cp, mem, &state);
	...
	for_collected_spaces(ispace)
	{
		for_space_mems(ispace, mem) { gc_free_empty_chunks(mem); }
	}
...
gc_objects_clear_marks(const gs_memory_t* mem, chunk_t* cp)
{
	...
	if_debug3('7', " [7](un)marking %s(%lu) 0x%lx\n",
	          struct_type_name_string(pre->o_type), (ulong)size,
	          (ulong)pre);
	o_set_unmarked(pre);
	if(proc != 0)
...
private
int
gc_trace_chunk(const gs_memory_t* mem, chunk_t* cp, gc_state_t* pstate,
               gc_mark_stack* pmstack)
{
	gs_gc_root_t root;
	void* comp;
	int more = 0;
	...
	DO_ALL
	{
		...
		if(pre->o_type == &st_refs) {
			...
			while((char*)rp < end) {
				comp = rp;
				if(r_is_packed(
				       rp)) { /* No packed refs need tracing. */
					rp++;
				} else {
					ref* const pref = (ref*)rp;
...
private
int gc_extend_stack(gc_mark_stack*, gc_state_t*);
private
int
...
#define mark_name(nidx)                                                        \
	...
	do_struct : {
		...
		if(ptr ==
		   0) { /* We've reached the bottom of a stack segment. */
			pms = pms->prev;
			if(pms == 0)
				break; /* all done */
	...
	ts:
		if_debug4('7', " [7]%smarking %s 0x%lx[%u]",
		          depth_dots(sp, pms),
		          struct_type_name_string(ptr[-1].o_type), (ulong)ptr,
		          sp->index);
		mproc = ptr[-1].o_type->enum_ptrs;
		if(mproc == gs_no_struct_enum_ptrs ||
		   (ptp = (*mproc)(gcst_get_memory_ptr(pstate), ptr,
		                   pre_obj_contents_size(ptr - 1), sp->index,
		                   &nep, ptr[-1].o_type, pstate)) == 0) {
			if_debug0('7', " - done\n");
			sp--;
			continue;
		}
	...
	tr:
		...
		new |= 1;
		if(r_space(rptr) < min_trace) { /* Note that this always picks
		                                   up all scalars. */
			pptr = (ref_packed*)(rptr + 1);
			goto tr;
		}
		...
		rr:
			if((sp[1].index = r_size(rptr)) ==
			   0) { /* Set the base pointer to 0, */
		...
		nr:
			...
			goto tr;
		case t_string:
			if(gc_string_mark(rptr->value.bytes, r_size(rptr), true,
			                  pstate))
				new |= 1;
			goto nr;
		case t_oparray:
...
gc_extend_stack(gc_mark_stack* pms, gc_state_t* pstate)
{
	if(pms->next == 0) { /* Try to allocate another segment. */
		...
		for(count = ms_size_desired; count >= ms_size_min;
		    count >>= 1) {
			pms->next = (gc_mark_stack*)gs_alloc_bytes_immovable(
			    pstate->heap,
			    sizeof(gc_mark_stack) + sizeof(ms_entry) * count,
			    "gc mark stack");
			if(pms->next != 0)
				break;
		}
		if(pms->next == 0) { /* The mark stack overflowed. */
			...
			if(cp == 0) { /* We were tracing outside collectible */
				...
				lprintf1("mark stack overflowed while outside "
				         "collectible space at 0x%lx!\n",
				         (ulong)cptr);
	...
	while((nidx = names_next_valid_index(nt, nidx)) != 0) {
		...
		if(pnstr->mark) {
			...
			if(!pnstr->foreign_string &&
			   gc_string_mark(pnstr->string_bytes,
			                  pnstr->string_size, true, pstate))
				marked = true;
			enst.ptr = names_index_sub_table(nt, nidx);
			ensst.ptr = names_index_string_sub_table(nt, nidx);
			marked |= ptr_struct_mark(&enst, pstate) |
...
gc_init_reloc(chunk_t* cp)
{
	...
	chead->dest = cp->cbase;
	chead->free.o_back = offset_of(chunk_head_t, free) >> obj_back_shift;
...
gc_objects_set_reloc(chunk_t* cp)
{
	...
	if((procs == 0
	        ? o_is_unmarked(pre)
	        : !(*procs->set_reloc)(pre, reloc, size))) { /* Free object */
...
gc_do_reloc(chunk_t* cp, gs_ref_memory_t* mem, gc_state_t* pstate)
{
	...
	if(o_is_untraced(pre) ||
	   pre->o_back << obj_back_shift != (byte*)pre - (byte*)chead) {
		struct_proc_reloc_ptrs((*proc)) = pre->o_type->reloc_ptrs;

		if_debug3('7', " [7]relocating ptrs in %s(%lu) 0x%lx\n",
		          struct_type_name_string(pre->o_type), (ulong)size,
		          (ulong)pre);
		if(proc != 0)
...
igc_reloc_struct_ptr(const void /*obj_header_t */* obj, gc_state_t* gcst)
{
	...
	debug_check_object(optr - 1, NULL, gcst);
	{
		...
		if(back == o_untraced)
			...
			if(cp != 0 && cp->cbase <= (byte*)obj &&
			   (byte*)obj < cp->ctop) {
				if(back > (cp->ctop - cp->cbase) >>
				   obj_back_shift) {
					lprintf2("Invalid back pointer %u at "
					         "0x%lx!\n",
					         back, (ulong)obj);
					gs_abort(NULL);
				}
			} else {
				/* Pointed to unknown chunk. Can't check it,
				 * sorry. */
			}
#endif
			{
				const obj_header_t* pfree =
				    (const obj_header_t*)((const char*)(optr -
				                                        1) -
				                          (back
				                           << obj_back_shift));
				const chunk_head_t* chead =
				    (const chunk_head_t*)((const char*)pfree -
				                          (pfree->o_back
				                           << obj_back_shift));

				robj = chead->dest + ((const char*)obj -
				                      (const char*)(chead + 1) -
				                      pfree->o_nreloc);
			}
		}
	...
	/* Use a severely deprecated pun to remove the const property. */
	{
		...
		u.r = print_reloc(obj, struct_type_name_string(optr[-1].o_type),
		                  robj);
		return u.w;
	}
}
...
gc_objects_compact(chunk_t* cp, gc_state_t* gcst)
{
	...
	const gs_memory_t* cmem =
	    gcst->spaces.memories.named.system->stable_memory;
	...
	if(pre->o_back << obj_back_shift != (byte*)pre - (byte*)chead) {
		...
		if_debug4('7', " [7]compacting %s 0x%lx(%lu) to 0x%lx\n",
		          struct_type_name_string(pre->o_type), (ulong)pre,
		          (ulong)size, (ulong)dpre);
		if(procs == 0) {
			if(dpre != pre)
				memmove(dpre, pre, sizeof(obj_header_t) + size);
		} else
			(*procs->compact)(cmem, pre, dpre, size);
		dpre = (obj_header_t*)((byte*)dpre + obj_size_round(size));
	}
	END_OBJECTS_SCAN
	if(cp->outer == 0 && chead->dest != cp->cbase)
		dpre = (obj_header_t*)
		           cp->cbase; /* compacted this chunk into another */
...
gc_free_empty_chunks(gs_ref_memory_t* mem)
{
	...
	for(cp = mem->clast; cp != 0;
	    cp = csucc) { /* Make sure this isn't an inner chunk, */
...
const gs_memory_t*
gcst_get_memory_ptr(gc_state_t* gcst)
{
