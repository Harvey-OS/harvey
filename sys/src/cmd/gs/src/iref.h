...
typedef enum {
	...
	/*
         ...
         * points to if the binding isn't known.  It never appears on a stack or
         *in
...
#define REF_TYPE_PROPERTIES_DATA                                                       \
	0,     /* t__invalid */                                                        \
	    ...
	    _REF_TYPE_USES_ACCESS |                                                    \
	        _REF_TYPE_IS_DICTIONARY,                 /* t_dictionary */            \
	    ...
	    _REF_TYPE_USES_ACCESS |                                                    \
	        _REF_TYPE_USES_SIZE, /* (unused array type) */                         \
	    0,                       /* t_struct */                                    \
	    ...
	    _REF_TYPE_USES_SIZE, /* t_oparray */         /*                            \
	                                                  ...
	                                                  * the extended pseudo-types  \
	                                                  * used by the                \
	                                                  * interpreter for operators. \
	    ...
	    _REF_TYPE_USES_SIZE, /*28*/                                                \
	    _REF_TYPE_USES_SIZE, _REF_TYPE_USES_SIZE, _REF_TYPE_USES_SIZE,             \
	    _REF_TYPE_USES_SIZE, /*32*/                                                \
	    _REF_TYPE_USES_SIZE, _REF_TYPE_USES_SIZE, _REF_TYPE_USES_SIZE,             \
	    _REF_TYPE_USES_SIZE, /*36*/                                                \
	    _REF_TYPE_USES_SIZE, _REF_TYPE_USES_SIZE, _REF_TYPE_USES_SIZE,             \
	    _REF_TYPE_USES_SIZE, /*40*/                                                \
	    _REF_TYPE_USES_SIZE, _REF_TYPE_USES_SIZE, _REF_TYPE_USES_SIZE,             \
	    _REF_TYPE_USES_SIZE, /*44*/                                                \
	    _REF_TYPE_USES_SIZE, _REF_TYPE_USES_SIZE, _REF_TYPE_USES_SIZE,             \
	    _REF_TYPE_USES_SIZE, /*48*/                                                \
	    _REF_TYPE_USES_SIZE, _REF_TYPE_USES_SIZE, _REF_TYPE_USES_SIZE,             \
	    _REF_TYPE_USES_SIZE, /*52*/                                                \
	    _REF_TYPE_USES_SIZE, _REF_TYPE_USES_SIZE, _REF_TYPE_USES_SIZE,             \
	    _REF_TYPE_USES_SIZE, /*56*/                                                \
	    _REF_TYPE_USES_SIZE, _REF_TYPE_USES_SIZE, _REF_TYPE_USES_SIZE,             \
	    _REF_TYPE_USES_SIZE, /*60*/                                                \
	    _REF_TYPE_USES_SIZE, _REF_TYPE_USES_SIZE, _REF_TYPE_USES_SIZE,             \
	    _REF_TYPE_USES_SIZE /*64 */
#define _REF_TYPE_HAS(rtype, props)                                            \
	((ref_type_properties[rtype] & (props)) != 0)
#define ref_type_uses_access(rtype) _REF_TYPE_HAS(rtype, _REF_TYPE_USES_ACCESS)
#define ref_type_uses_size(rtype) _REF_TYPE_HAS(rtype, _REF_TYPE_USES_SIZE)
...
#define REF_TYPE_DEBUG_PRINT_STRINGS                                           \
	"INVL", "bool", "dict", "file", "arry", "mpry", "spry", "u?ry",        \
	    "STRC", "ASTR", "font", "int ", "mark", "name", "null", "oper",    \
	    "real", "save", "str ", "devc", "opry"
/*
 * Define the type names for the type operator.
 */
#define REF_TYPE_NAME_STRINGS                                                  \
	0, "booleantype", "dicttype", "filetype", "arraytype",                 \
	    "packedarraytype", "packedarraytype", "arraytype", 0, 0,           \
...
#define REF_TYPE_PRINT_STRINGS                                                 \
	0, 0, "-dict-", "-file-", "-array-", "-packedarray-", "-packedarray-", \
	    "-array-", 0, 0, "-fontID-", 0, "-mark-", 0, 0, 0, 0, "-save-",    \
	    "-string-", "-device-", 0
...
#define REF_ATTR_PRINT_MASKS                                                   \
	_REF_ATTR_PRINT_FLAG(l_mark, 'm'), _REF_ATTR_PRINT_FLAG(l_new, 'n'),   \
 ...
 */
struct tas_s {
	/* type_attrs is a single element for fast dispatching in the
	 * interpreter */
	ushort type_attrs;
	ushort rsize;
};
...
#endif
#define r_btype(rp)                                                            \
	((rp)->tas.type_attrs >= (t_next_index << r_type_shift) ? t_operator   \
	                                                        : r_type(rp))
...
#define r_is_proc(rp)                                                          \
	_REF_HAS_MASKED_TYPE_ATTRS(rp, t_array, _REF_T_ARRAY_SPAN,             \
	                           a_execute + a_executable)
...
#define r_type_xe(rp)                                                          \
	_REF_TAS_TYPE_XE(                                                      \
	    ((const ushort*)(rp))[offset_of(ref, tas.type_attrs) /             \
	                          sizeof(ushort)])
...
#define empty_ref_data(type, attrs)                                            \
	{        /*tas*/                                                       \
		{/*type_attrs*/                                                \
			((type) << r_type_shift) | (attrs), /*rsize*/ 0        \
		}                                                              \
	}
