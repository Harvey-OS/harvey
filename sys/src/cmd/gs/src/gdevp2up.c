...
private
dev_proc_open_device(pcx2up_open);
private
dev_proc_print_page(pcx2up_print_page);
...
private
const gx_device_procs pcx2up_procs =
    prn_color_procs(pcx2up_open, gdev_prn_output_page, gdev_prn_close,
                    pc_8bit_map_rgb_color, pc_8bit_map_color_rgb);
gx_device_2up gs_pcx2up_device = {prn_device_body(
    gx_device_2up, pcx2up_procs, "pcx2up", DEFAULT_WIDTH_10THS,
...
#define rdev ((gx_device*)prdev)
		...
		prdev = (gx_device_printer*)gs_alloc_bytes(
		    pdev->memory, prdev_size, "pcx2up_print_page(device)");
		if(prdev == 0)
