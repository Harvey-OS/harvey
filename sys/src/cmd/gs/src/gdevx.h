...
#define private_st_x11fontmap() /* in gdevxini.c */                            \
	gs_private_st_ptrs3(st_x11fontmap, x11fontmap, "x11fontmap",           \
	                    x11fontmap_enum_ptrs, x11fontmap_reloc_ptrs,       \
	                    ps_name, x11_name, next)
...
#define X_SET_FILL_STYLE(xdev, style)                                          \
	...
	if(xdev->fill_style != (style))                                        \
		XSetFillStyle(xdev->dpy, xdev->gc,                             \
		              (xdev->fill_style = (style)));                   \
	END
...
#define NOTE_COLOR(xdev, pixel)                                                \
	(xdev->colors_or |= (pixel), xdev->colors_and &= (pixel))
...
#define DRAW_TEXT(xdev)                                                        \
	XDrawText(xdev->dpy, xdev->dest, xdev->gc, xdev->text.origin.x,        \
	          xdev->text.origin.y, xdev->text.items,                       \
	          xdev->text.item_count)

} gx_device_X;
#define private_st_device_X() /* in gdevx.c */                                 \
	gs_public_st_suffix_add4_final(                                        \
	    st_device_X, gx_device_X, "gx_device_X", device_x_enum_ptrs,       \
	    device_x_reloc_ptrs, gx_device_finalize, st_device_bbox, buffer,   \
	    regular_fonts, symbol_fonts, dingbat_fonts)
