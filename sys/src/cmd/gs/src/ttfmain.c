...
{
	pt->x =
	    MulDiv(dx, m->a, 65536) + MulDiv(dy, m->c, 65536) + (m->tx >> 10);
	pt->y =
	    MulDiv(dx, m->b, 65536) + MulDiv(dy, m->d, 65536) + (m->ty >> 10);
}

private
void
TransformF26Dot6PointFloat(FloatPoint* pt, F26Dot6 dx, F26Dot6 dy,
                           FloatMatrix* m)
{
...
TT_Error
TT_Set_Instance_CharSizes(TT_Instance instance, TT_F26Dot6 charWidth,
...
ttfInterpreter__obtain(ttfMemory* mem, ttfInterpreter** ptti)
{
	...
	tti = mem->alloc_struct(mem,
	                        (const ttfMemoryDescriptor*)&st_ttfInterpreter,
	                        "ttfInterpreter__obtain");
	if(!tti)
		return fMemoryError;
	tti->usage = 0;
	tti->usage_size = 0;
	tti->ttf_memory = mem;
	tti->lock = 1;
	tti->exec = mem->alloc_struct(
	    mem, (const ttfMemoryDescriptor*)&st_TExecution_Context,
	    "ttfInterpreter__obtain");
	if(!tti->exec) {
...
void
ttfFont__init(ttfFont* this, ttfMemory* mem, void (*DebugRepaint)(ttfFont*),
...
#define MAX_SUBGLYPH_NESTING                                                   \
	3 /* Arbitrary. We need this because we don't want                     \
...
ttfFont__Open(ttfInterpreter* tti, ttfFont* this, ttfReader* r,
              unsigned int nTTC, float w, float h, bool design_grid)
{
	...
	r->Seek(r, this->t_head.nPos +
	               offset_of(sfnt_FontHeader, indexToLocFormat));
	this->nIndexToLocFormat = ttfReader__UShort(r);
	r->Seek(r,
	        this->t_maxp.nPos + offset_of(sfnt_maxProfileTable, numGlyphs));
	this->nNumGlyphs = ttfReader__UShort(r);
	r->Seek(r, this->t_maxp.nPos +
	               offset_of(sfnt_maxProfileTable, maxComponentElements));
	this->nMaxComponents = ttfReader__UShort(r);
	if(this->nMaxComponents < 10)
		this->nMaxComponents =
		    10; /* work around DynaLab bug in lgoth.ttf */
	r->Seek(r, this->t_hhea.nPos +
	               offset_of(sfnt_MetricsHeader, numberLongMetrics));
	this->nLongMetricsHorz = ttfReader__UShort(r);
	if(this->t_vhea.nPos != 0) {
		r->Seek(r, this->t_vhea.nPos + offset_of(sfnt_MetricsHeader,
		                                         numberLongMetrics));
	...
	if(tti->usage_size < this->nMaxComponents * MAX_SUBGLYPH_NESTING) {
		tti->ttf_memory->free(tti->ttf_memory, tti->usage,
		                      "ttfFont__Open");
		tti->usage_size = 0;
		tti->usage = mem->alloc_bytes(mem, sizeof(ttfSubGlyphUsage) *
		                                       this->nMaxComponents *
		                                       MAX_SUBGLYPH_NESTING,
		                              "ttfFont__Open");
		if(tti->usage == NULL)
			return fMemoryError;
		tti->usage_size = this->nMaxComponents * MAX_SUBGLYPH_NESTING;
	}
	this->face = mem->alloc_struct(
	    mem, (const ttfMemoryDescriptor*)&st_TFace, "ttfFont__Open");
	if(this->face == NULL)
		return fMemoryError;
	...
	if(code < 0)
		return fBadFontData;
	this->inst = mem->alloc_struct(
	    mem, (const ttfMemoryDescriptor*)&st_TInstance, "ttfFont__Open");
	if(this->inst == NULL)
		return fMemoryError;
	memset(this->inst, 0, sizeof(*this->inst));
	code = Context_Create(this->exec, this->face); /* See comment in the
	                                                  implementation of
	                                                  Context_Create. */
...
private
void
mount_zone(PGlyph_Zone source, PGlyph_Zone target)
{
	Int np, nc;
...
private
void
Init_Glyph_Component(PSubglyph_Record element, PSubglyph_Record original,
...
private
void
MoveGlyphOutline(TGlyph_Zone* pts, int nOffset, ttfGlyphOutline* out,
                 FixMatrix* m)
{
	...
	if(m->a == 65536 && m->b == 0 && m->c == 0 && m->d == 65536 &&
	   m->tx == 0 && m->ty == 0)
		return;
	for(; count != 0; --count) {
                                  ...
                                  FixMatrix* m_orig, ttfGlyphOutline* gOutline)
{
	...
	if(nMtxGlyph < nLongMetrics) {
		...
		nAdvance = ttfReader__Short(r);
		r->Seek(r, nMtxPos + 4 * nLongMetrics +
		               2 * (nMtxGlyph - nLongMetrics));
		sideBearing = ttfReader__Short(r);
	}
	if(r->Error(r))
		goto errex;
	...
	if(gOutline->contourCount == 0)
		...
		/* Some fonts have bad WE_HAVE_INSTRUCTIONS, so use
		 * nNextGlyphPtr : */
		if(r->Error(r))
			goto errex;
		nPos = r->Tell(r);
		n_ins =
		    ((!r->Eof(r) && (bHaveInstructions || nPos < nNextGlyphPtr))
		         ? ttfReader__UShort(r)
		         : 0);
		nPos = r->Tell(r);
		r->ReleaseGlyph(r, glyphIndex);
		...
		for(i = 0; i < nUsage; i++) {
			...
			int j;
			TT_Error code;
			int nPointsStored = this->nPointsTotal,
			    nContoursStored = this->nContoursTotal;
			...
			code = ttfOutliner__BuildGlyphOutlineAux(this, e->index,
			                                         m_orig, &out);
			pts->org_x -= nPointsStored;
			pts->org_y -= nPointsStored;
			pts->cur_x -= nPointsStored;
			...
			if(flags & ARGS_ARE_XY_VALUES) {
				e->m.tx = Scale_X(&exec->metrics, e->arg1)
				          << 10;
				e->m.ty = Scale_Y(&exec->metrics, e->arg2)
				          << 10;
			} else {
				e->m.tx =
				    (pts->org_x[e->arg1] -
				     pts->org_x[gOutline->pointCount + e->arg2])
				    << 10;
				e->m.ty =
				    (pts->org_y[e->arg1] -
				     pts->org_y[gOutline->pointCount + e->arg2])
				    << 10;
			}
			MoveGlyphOutline(pts, nPointsStored, &out, &e->m);
			for(j = nContoursStored;
			    j < out.contourCount + nContoursStored; j++)
		...
		if(!skip_instructions && n_ins &&
		   !(pFont->inst->GS.instruct_control & 1)) {
			...
			if(nPos + n_ins > glyph_size)
				goto errex;
			code = Set_CodeRange(exec, TT_CodeRange_Glyph,
			                     (byte*)glyph + nPos, n_ins);
			if(!code) {
				...
				/* add phantom points : */
				pts->org_x[nPoints - 2] =
				    Scale_X(&exec->metrics, subglyph.pp1.x);
				pts->org_y[nPoints - 2] =
				    Scale_Y(&exec->metrics, subglyph.pp1.y);
				pts->org_x[nPoints - 1] =
				    Scale_X(&exec->metrics, subglyph.pp2.x);
				pts->org_y[nPoints - 1] =
				    Scale_Y(&exec->metrics, subglyph.pp2.y);
				pts->touch[nPoints - 1] = 0;
				pts->touch[nPoints - 2] = 0;
				/* if hinting, round the phantom points (not
				 * sure) : */
		...
		if(this->nContoursTotal + gOutline->contourCount >
		   exec->n_contours) {
			error = fBadFontData;
			goto ex;
		}
		...
		nPoints = gOutline->pointCount =
		    endPoints[gOutline->contourCount - 1] + 1;
		...
		if(r->Error(r))
			goto errex;
		bInsOK = !Set_CodeRange(exec, TT_CodeRange_Glyph,
		                        (byte*)glyph + nPos, n_ins);
		onCurve = pts->touch;
		...
		if(!skip_instructions && !r->Error(r) && n_ins && bInsOK &&
		   !(pFont->inst->GS.instruct_control & 1)) {
			TGlyph_Zone* pts = &exec->pts;
			int k;
			F26Dot6 x;
			...
			gOutline->sideBearing =
			    subglyph.bbox.xMin - subglyph.pp1.x;
...
private
FontError
ttfOutliner__BuildGlyphOutline(ttfOutliner* this, int glyphIndex, float orig_x,
                               float orig_y, ttfGlyphOutline* gOutline)
{
	...
	return ttfOutliner__BuildGlyphOutlineAux(this, glyphIndex, &m_orig,
	                                         gOutline);
}
				...
				if(onCurve[pt] & 1) {
					...
					if(this->ppx != nextX ||
					   this->ppy != nextY) {
						...
						dx3 = p3.x - p0.x,
						dy3 = p3.y - p0.y;
						if(fabs(dx1 * dy3 - dy1 * dx3) >
						       prec * fabs(dx1 * dx3 -
						                   dy1 * dy3) ||
						   fabs(dx2 * dy3 - dy2 * dx3) >
						       prec * fabs(dx2 * dx3 -
						                   dy2 * dy3))
							exp->CurveTo(exp, &p1,
							             &p2, &p3);
...
FontError
ttfOutliner__Outline(ttfOutliner* this, int glyphIndex, float orig_x,
                     float orig_y, FloatMatrix* m1)
{
	ttfFont* pFont = this->pFont;
	FontError error;
	...
	error = ttfOutliner__BuildGlyphOutline(this, glyphIndex, orig_x, orig_y,
	                                       &this->out);
