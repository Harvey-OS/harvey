...
private
int
s_N_8_process(stream_state* st, stream_cursor_read* pr, stream_cursor_write* pw,
              bool last)
{
	BEGIN_1248;
	...
	case 2: {
		static const byte b2[4] = {0x00, 0x55, 0xaa, 0xff};
	...
	case 4: {
		static const byte b4[16] = {0x00, 0x11, 0x22, 0x33, 0x44, 0x55,
...
private
int
s_8_N_process(stream_state* st, stream_cursor_read* pr, stream_cursor_write* pw,
              bool last)
{
	BEGIN_1248;
...
const stream_template s_1_8_template = {&st_1248_state, s_1_init, s_N_8_process,
                                        1, 8};
const stream_template s_2_8_template = {&st_1248_state, s_2_init, s_N_8_process,
                                        1, 4};
const stream_template s_4_8_template = {&st_1248_state, s_4_init, s_N_8_process,
                                        1, 2};
const stream_template s_12_8_template = {&st_1248_state, s_12_init,
                                         s_12_8_process, 1, 2};

const stream_template s_8_1_template = {&st_1248_state, s_1_init, s_8_N_process,
                                        8, 1};
const stream_template s_8_2_template = {&st_1248_state, s_2_init, s_8_N_process,
                                        4, 1};
const stream_template s_8_4_template = {&st_1248_state, s_4_init, s_8_N_process,
                                        2, 1};
...
private
int
s_C2R_process(stream_state* st, stream_cursor_read* pr, stream_cursor_write* pw,
              bool last)
{
	stream_C2R_state* const ss = (stream_C2R_state*)st;
	const byte* p = pr->ptr;
...
case 0:
return ENUM_OBJ(st->Decode);
case 1:
return ENUM_BYTESTRING(&st->Table);
ENUM_PTRS_END
private
...
private
int
s_IE_process(stream_state* st, stream_cursor_read* pr, stream_cursor_write* pw,
             bool last)
{
	stream_IE_state* const ss = (stream_IE_state*)st;
	...
	for(;;) {
		...
		byte_out =
		    (byte_out << ss->BitsPerIndex) + index / num_components;
		next_component = 0;
		if(++(ss->x) == ss->Width) {
...
out:
	...
	ss->Table.data[ss->Table.size - 1] =
	    (ss->next_index == 0 ? 0 : ss->next_index / ss->NumComponents - 1);
	return status;
}

const stream_template s_IE_template = {&st_IE_state, s_IE_init, s_IE_process, 1,
                                       1, 0 /* NULL */, s_IE_set_defaults};
                    ...
                    stream_cursor_write* pw, bool last)
{
	...
	if_debug4('w', "[w]subsample: x=%d, y=%d, rcount=%ld, wcount=%ld\n", x,
	          y, (int32_t)(rlimit - p), (int32_t)(wlimit - q));
	...
	if_debug5(
	    ...
	    y, (int32_t)(p - pr->ptr), (int32_t)(q - pw->ptr), status);
	pr->ptr = p;
	pw->ptr = q;
	ss->x = x, ss->y = y;
	return status;
...
s_Average_init(stream_state* st)
{
	...
	ss->sums = (uint*)gs_alloc_byte_array(st->memory, ss->sum_size,
                               ...
                               int height, int depth, int bits_per_sample)
{
	...
	ss->sample = gs_alloc_bytes(ss->memory, width * depth,
...
{
	/*	This function uses a statistical algorithm being not well
	   defined.
	...
	*/
	...
	for(i = 1; i < ss->width; i++) {
		...
		if(!lower) {
			if(w1 < v) {
				...
				j1 = i - max_gradient_constant; /* inner
				                                   constant
				                                   plateaw */
			else if(upper && w1 - delta > v) {
				/* end of upper plateau at w1-delta...w1 */
				for(j0 = i - 1; j0 > j1 && w1 - delta <= p[j0];
				    j0--)
					DO_NOTHING;
				/* upper plateau j0+1...i-1 */
				if(j0 > 0 &&
				   i < ss->width - 1) /* ignore sides */
					upper_plateaus += i - j0;
				plateau_count++;
				if(j0 > j1) {
					/* upgrade j1...j0 */
		...
		if(!upper) {
			if(w0 > v) {
				...
				for(j0 = i - 1; j0 > j1 && w0 + delta >= p[j0];
				    j0--)
					DO_NOTHING;
	...
	if(plateau_count > ss->width / 6) {
		/*  Possibly a dithering, can't recognize.
		    It would be better to estimate frequency histogram rather
		   than
		    rough quantity, but we hope that the simpler test can work
		   fine.
		*/
		...
		plateaus = min(ss->lower_plateaus,
		               ss->upper_plateaus); /* (fore/back)ground */
                                      ...
                                      const byte* data, int length)
{
	...
	while(l) {
		if(ss->bits_left < 8) {
			uint k =
			    (sizeof(ss->packed_data) * 8 - ss->bits_left) / 8;
...
const stream_template s_compr_chooser_template = {
    &st_compr_chooser_state, s_compr_chooser_init, s_compr_chooser_process, 1,
    1, s_compr_chooser_release, 0 /* NULL */
};
...
s_image_colors_convert_to_device_color(stream_image_colors_state* ss)
{
	...
	for(i = 0; i < ss->depth; i++)
		cc.paint.values[i] =
		    ss->input_color[i] *
		        (ss->Decode[i * 2 + 1] - ss->Decode[i * 2]) / v0 +
		    ss->Decode[i * 2];

	code = ss->pcs->type->remap_color(&cc, ss->pcs, &dc, ss->pis, ss->pdev,
	...
	for(i = 0; i < ss->output_depth; i++) {
		...
		uint w =
		    (dc.colors.pure >> ss->pdev->color_info.comp_shift[i]) & m;
...
/* Set image dimensions. */
void
s_image_colors_set_dimensions(stream_image_colors_state* ss, int width,
                              int height, int depth, int bits_per_sample)
{
	ss->width = width;
	ss->height = height;
	...
	ss->row_alignment_bytes =
	    0; /* (ss->raster * 8 - ss->row_bits) / 8) doesn't work. */
}

void
s_image_colors_set_color_space(stream_image_colors_state* ss, gx_device* pdev,
                               const gs_color_space* pcs,
                               const gs_imager_state* pis, float* Decode)
{
	ss->output_depth = pdev->color_info.num_components;
	ss->output_component_index = ss->output_depth;
	ss->output_bits_per_sample =
	    pdev->color_info
	...
	for(;;) {
		...
		if(ss->row_bits_passed >= ss->row_bits) {
			...
			ss->input_bits_buffer =
			    0; /* Just to simplify the debugging. */
		...
		if(ss->output_component_index < ss->output_depth) {
			for(; ss->output_component_index < ss->output_depth;) {
				uint fitting =
				    (uint)(8 - ss->output_bits_buffered);
				uint v, w, u, n, m;
				...
				n = ss->output_bits_per_sample -
				    ss->output_component_bits_written; /* no. of
				                                          bits
				                                          left
				                                          */
				w = v - ((v >> n) << n); /* the current
				                            component without
				                            written bits. */
				if(fitting > n)
				...
				ss->output_bits_buffer |=
				    u
				    << (8 - ss->output_bits_buffered - fitting);
				...
				if(ss->output_component_bits_written >=
				   ss->output_bits_per_sample) {
					ss->output_component_index++;
					ss->output_component_bits_written = 0;
				}
		...
		if(ss->input_bits_buffered < ss->bits_per_sample) {
			if(pr->ptr >= pr->limit)
				return 0;
			ss->input_bits_buffer =
			    (ss->input_bits_buffer << 8) | *++pr->ptr;
			ss->input_bits_buffered += 8;
			/* fixme: delay shifting the input ptr until
			 * input_bits_buffer is cleaned. */
		}
		if(ss->input_bits_buffered >= ss->bits_per_sample) {
			uint w;

			ss->input_bits_buffered -= ss->bits_per_sample;
			ss->input_color[ss->input_component_index] = w =
			    ss->input_bits_buffer >> ss->input_bits_buffered;
			ss->input_bits_buffer &=
...
const stream_template s__image_colors_template = {
    &st_stream_image_colors_state, s_image_colors_init, s_image_colors_process,
    1, 1, NULL, NULL};
