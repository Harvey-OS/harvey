...
/* Device procedures */
private
dev_proc_open_device(accum_open);
private
dev_proc_close_device(accum_close);
private
dev_proc_fill_rectangle(accum_fill_rectangle);

/* The device descriptor */
/* Many of these procedures won't be called; they are set to NULL. */
private
const gx_device_cpath_accum gs_cpath_accum_device = {
    std_device_std_body(gx_device_cpath_accum, 0, "clip list accumulator", 0, 0,
                        1, 1),
    {accum_open, NULL, NULL, NULL, accum_close, NULL, NULL,
     accum_fill_rectangle, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL,
     NULL, NULL, NULL, NULL, NULL, NULL, NULL, gx_default_fill_path,
     gx_default_stroke_path, NULL, gx_default_fill_trapezoid,
     ...
     gx_default_end_image, NULL, NULL, gx_get_largest_clipping_box,
     gx_default_begin_typed_image, NULL, NULL, NULL, NULL,
     gx_default_text_begin, gx_default_finish_copydevice, NULL, NULL, NULL,
...
void
gx_cpath_accum_set_cbox(gx_device_cpath_accum* padev, const gs_fixed_rect* pbox)
{
...
gx_cpath_accum_end(const gx_device_cpath_accum* padev, gx_clip_path* pcpath)
{
	...
	apath.id =
	    gs_next_ids(padev->list_memory, 1); /* path changed => change id */
...
int
gx_cpath_intersect_path_slow(gx_clip_path* pcpath, gx_path* ppath, int rule,
                             gs_imager_state* pis)
{
	...
	code = gx_fill_path_only(ppath, (gx_device*)&adev, pis, &params, &devc,
	                         pcpath);
...
#ifdef DEBUG
	if(gs_debug_c('q')) {
		gx_clip_rect* rp = (adev->list.count <= 1 ? &adev->list.single
		                                          : adev->list.head);

		dlprintf6("[q]list at 0x%lx, count=%d, head=0x%lx, tail=0x%lx, "
		          "xrange=(%d,%d):\n",
...
static const gx_clip_rect clip_head_rect = {0,       0,       min_int,
                                            min_int, min_int, min_int};
static const gx_clip_rect clip_tail_rect = {0,       0,       max_int,
...
accum_alloc_rect(gx_device_cpath_accum* adev)
{
	...
	if(adev->list.count == 2) {
		...
		gx_clip_rect* tail = gs_alloc_struct(
		    mem, gx_clip_rect, &st_clip_rect, "accum_alloc_rect(tail)");
		gx_clip_rect* single =
...
#define ACCUM_ADD_LAST(ar) ACCUM_ADD_BEFORE(ar, adev->list.tail)
...
#define ACCUM_ADD_BEFORE(ar, rnext)                                            \
	(ar->prev = (rnext)->prev)->next = ar, ar->next = (rnext),             \
	            (rnext)->prev = ar
#define ACCUM_REMOVE(ar) ar->next->prev = ar->prev, ar->prev->next = ar->next
...
top:
	...
	if(adev->list.count == 1) { /* check for Y merging */
		...
		if(x == rptr->xmin && xe == rptr->xmax && y <= rptr->ymax &&
		   ye >= rptr->ymin) {
			if(y < rptr->ymin)
				rptr->ymin = y;
			if(ye > rptr->ymax)
	...
	if(ye < ymax) {
		...
		while(rsplit->ymax == ymax) {
			ACCUM_ALLOC("s.top", ar, rsplit->xmin, ye, rsplit->xmax,
			            ymax);
			ACCUM_ADD_AFTER(ar, rptr);
			rsplit->ymax = ye;
			rsplit = rsplit->prev;
	...
	if(y > ymin) {
		...
		while(rbot->prev->ymin == ymin)
			rbot = rbot->prev;
		for(rsplit = rbot;;) {
			ACCUM_ALLOC("s.bot", ar, rsplit->xmin, ymin,
			            rsplit->xmax, y);
