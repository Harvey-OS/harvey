...
case 0:
	return ENUM_OBJ((cdev->writer.image_enum_id != gs_no_id
	                     ? cdev->writer.clip_path
	                     : 0));
case 1:
	return ENUM_OBJ((cdev->writer.image_enum_id != gs_no_id
	                     ? cdev->writer.color_space.space
	                     : 0));
default:
...
private
dev_proc_get_band(clist_get_band);
/* Driver procedures defined in other files are declared in gxcldev.h. */

/* Other forward declarations */
private
int clist_put_current_params(gx_device_clist_writer* cldev);

/* The device procedures */
const gx_device_procs gs_clist_device_procs = {
    clist_open, gx_forward_get_initial_matrix, gx_default_sync_output,
    clist_output_page, clist_close, gx_forward_map_rgb_color,
    gx_forward_map_color_rgb, clist_fill_rectangle, gx_default_tile_rectangle,
    clist_copy_mono, clist_copy_color, gx_default_draw_line,
    gx_default_get_bits, gx_forward_get_params, gx_forward_put_params,
    gx_forward_map_cmyk_color, gx_forward_get_xfont_procs,
    gx_forward_get_xfont_device, gx_forward_map_rgb_alpha_color,
    gx_forward_get_page_device, gx_forward_get_alpha_bits, clist_copy_alpha,
    clist_get_band, gx_default_copy_rop, clist_fill_path, clist_stroke_path,
    clist_fill_mask, gx_default_fill_trapezoid, clist_fill_parallelogram,
    clist_fill_triangle, gx_default_draw_thin_line, gx_default_begin_image,
    gx_default_image_data, gx_default_end_image, clist_strip_tile_rectangle,
    clist_strip_copy_rop, gx_forward_get_clipping_box, clist_begin_typed_image,
    clist_get_bits_rectangle, gx_forward_map_color_rgb_alpha,
    clist_create_compositor, gx_forward_get_hardware_params,
    ...
    gx_forward_encode_color, gx_forward_decode_color, gx_default_pattern_manage,
    gx_default_fill_rectangle_hl_color, gx_default_include_color_space,
...
clist_tile_cache_size(const gx_device* target, uint data_size)
{
	...
	if(!gx_device_must_halftone(target)) { /* No halftones -- cache holds
	                                          only Patterns & characters. */
                 ...
                 int band_width, int band_height)
{
	...
	if(gs_debug_c('l') | gs_debug_c(':'))
		dlprintf4(
		    "[:]width=%d, band_width=%d, band_height=%d, nbands=%d\n",
...
clist_init_data(gx_device* dev, byte* init_data, uint data_size)
{
	...
	const int band_width = cdev->page_info.band_params.BandWidth =
	...
	const uint band_space = cdev->page_info.band_params.BandBufferSpace =
...
clist_reset(gx_device* dev)
{
	...
	memset(cdev->tile_table, 0,
	       (cdev->tile_hash_mask + 1) * sizeof(*cdev->tile_table));
    ...
    clist_reinit_output_file(gx_device* dev)
{
	...
	if(clist_test_VMerror_recoverable(cdev)) {
		if(cdev->page_bfile != 0)
			code =
			    clist_set_memory_warning(cdev->page_bfile, b_block);
		if(code >= 0 && cdev->page_cfile != 0)
			code =
			    clist_set_memory_warning(cdev->page_cfile, c_block);
	}
	return code;
}
...
clist_finish_page(gx_device* dev, bool flush)
{
	...
	if(flush) {
		...
		if(cdev->page_cfile != 0)
			clist_fseek(cdev->page_cfile, 0L, SEEK_END,
			            cdev->page_cfname);
		if(cdev->page_bfile != 0)
			clist_fseek(cdev->page_bfile, 0L, SEEK_END,
			            cdev->page_bfname);
	}
    ...
    clist_end_page(gx_device_clist_writer* cldev)
{
	...
	if(code >= 0) {
		 ...
		 * Note that because of copypage, there may be many such
		 * entries.
		 */
		cb.band_min = cb.band_max = cmd_band_end;
		cb.pos = (cldev->page_cfile == 0 ? 0 : clist_ftell(
		                                           cldev->page_cfile));
...
clist_compute_colors_used(gx_device_clist_writer* cldev)
{
	...
	int bands_per_colors_used = (nbands + PAGE_INFO_NUM_COLORS_USED - 1) /
...
int /* ret -ve err, >= 0 if recovered w/# = cnt pages left in page queue */
    clist_VMerror_recover(gx_device_clist_writer* cldev, int old_error_code)
{
	int code = old_error_code;
	int pages_remain;

	if(!clist_test_VMerror_recoverable(cldev) ||
	   !cldev->error_is_retryable || old_error_code != gs_error_VMerror)
                                ...
                                int old_error_code)
{
	...
	if(!clist_test_VMerror_recoverable(cldev) ||
	   old_error_code != gs_error_VMerror)
		return old_error_code; /* sorry, don't have any means to recover
		                          this error */
	...
	code =
	    (reset_code < 0 ? reset_code : free_code < 0 ? old_error_code : 0);
    ...
    clist_put_current_params(gx_device_clist_writer* cldev)
{
	...
	code = (*dev_proc(target, get_params))(target,
