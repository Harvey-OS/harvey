...
switch(index) {
case 0:
	return ENUM_OBJ((cdev->writer.image_enum_id != gs_no_id ? cdev->writer.clip_path : 0));
case 1:
	return ENUM_OBJ((cdev->writer.image_enum_id != gs_no_id ? cdev->writer.color_space.space : 0));
default:
...
clist_init_data(gx_device *dev, byte *init_data, uint data_size)
{
	...
	const int band_width =
		...
		(cdev->band_params.BandWidth ? cdev->band_params.BandWidth : target->width);
	...
	const uint band_space =
		...
		(cdev->band_params.BandBufferSpace ? cdev->band_params.BandBufferSpace : data_size);
	byte *data = init_data;
    ...
    clist_put_current_params(gx_device_clist_writer *cldev)
{
	...
	code = (*dev_proc(target, get_params))(target, (gs_param_list *)&param_list);
	if(code >= 0) {
