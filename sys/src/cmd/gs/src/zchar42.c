                  ...
                  bool put_lsb)
{
	...
	if(present == metricsNone) {
		...
		w[1] = sbw[3];
		if(gs_rootfont(igs)
		       ->WMode) { /* for vertically-oriented metrics */
			code = gs_type42_wmode_metrics(
			    (gs_font_type42*)pbfont, glyph_index, true, sbw42);
			if(code < 0) { /* no vertical metrics */
				if(pbfont->FontType == ft_CID_TrueType) {
					sbw[0] = sbw[2] / 2;
					sbw[1] = pbfont->FontBBox.q.y;
					sbw[2] = 0;
					sbw[3] = pbfont->FontBBox.p.y -
					         pbfont->FontBBox.q.y;
				}
			} else {
				sbw[0] = sbw[2] / 2;
				sbw[1] = (pbfont->FontBBox.q.y +
				          pbfont->FontBBox.p.y - sbw42[3]) /
				         2;
				sbw[2] = sbw42[2];
				sbw[3] = sbw42[3];
			}
	...
	return zchar_set_cache(
	    i_ctx_p, pbfont, cnref,
	    (put_lsb && present == metricsSideBearingAndWidth ? sbw : NULL), w,
	    &pbfont->FontBBox, cont, exec_cont,
	    gs_rootfont(igs)->WMode ? sbw : NULL);
}

/* <font> <code|name> <name> <glyph_index> .type42execchar - */
private
int type42_fill(i_ctx_t*);
private
int type42_stroke(i_ctx_t*);
private
int
ztype42execchar(i_ctx_t* i_ctx_p)
{
	...
	op_proc_t cont = (pbfont->PaintType == 0 ? type42_fill : type42_stroke),
	          exec_cont = 0;
	ref* cnref;
	uint glyph_index;
	...
	code = zchar42_set_cache(i_ctx_p, pbfont, cnref, glyph_index, cont,
	                         &exec_cont, true);
...
private
int type42_finish(i_ctx_t* i_ctx_p, int (*cont)(gs_state*));
private
int
type42_fill(i_ctx_t* i_ctx_p)
{
	...
	i_ctx_p->pgs->fill_adjust =
	    fa; /* Not sure whether we need to restore it,
...
type42_finish(i_ctx_t* i_ctx_p, int (*cont)(gs_state*))
{
	 ...
	 */
	code = gs_type42_append(
	    (uint)opc->value.intval, (gs_imager_state*)igs, igs->path,
	    &penum->log2_scale,
...
const op_def zchar42_op_defs[] = {{"4.type42execchar", ztype42execchar},
                                  op_def_end(0)};
