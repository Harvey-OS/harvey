...
private
dev_proc_print_page(t4693d_print_page);
private
dev_proc_map_rgb_color(gdev_t4693d_map_rgb_color);
private
dev_proc_map_color_rgb(gdev_t4693d_map_color_rgb);

private
gx_device_procs t4693d_procs =
    prn_color_procs(gdev_prn_open, gdev_prn_output_page, gdev_prn_close,
                    gdev_t4693d_map_rgb_color, gdev_t4693d_map_color_rgb);

#define t4693d_prn_device(name, depth, max_rgb)                                \
	{                                                                      \
		prn_device_body(gx_device_printer, t4693d_procs, name,         \
		                WIDTH_10THS, HEIGHT_10THS, X_DPI, Y_DPI, 0.25, \
		                0.25, 0.25, 0.25, 3, depth, max_rgb, max_rgb,  \
		                max_rgb + 1, max_rgb + 1, t4693d_print_page)   \
	}

const gx_device_printer gs_t4693d2_device = t4693d_prn_device("t4693d2", 8, 3);
const gx_device_printer gs_t4693d4_device =
    t4693d_prn_device("t4693d4", 16, 15);
const gx_device_printer gs_t4693d8_device =
    t4693d_prn_device("t4693d8", 24, 255);
...
gdev_t4693d_map_color_rgb(gx_device* dev, gx_color_index color, ushort prgb[3])
{
	...
	prgb[0] =
	    (color >> (bitspercolor * 2)) * gx_max_color_value / max_value;
	prgb[1] = ((color >> bitspercolor) & max_value) * gx_max_color_value /
	          max_value;
...
t4693d_print_page(gx_device_printer* dev, FILE* ps_stream)
{
	...
	byte* data =
	    (byte*)gs_malloc(dev->memory, line_size, 1, "t4693d_print_page");
	char* p;
	...
	for(lnum = 0; lnum < prn_dev->height; lnum++) {
		...
		for(i = 0; i < line_size; i += data_size) {
			...
			default:
				...
				gs_free(dev->memory, data, line_size, 1,
				        "t4693d_print_page");
				return_error(gs_error_rangecheck);
			}

			if(fwrite(&data[i], 1, data_size, ps_stream) !=
			   data_size) {
