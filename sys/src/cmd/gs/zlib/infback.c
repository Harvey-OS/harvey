 ...
 */
int ZEXPORT inflateBack(strm, in, in_desc, out, out_desc) z_stream FAR* strm;
in_func in;
void FAR* in_desc;
out_func out;
void FAR* out_desc;
{
	...
	for(;;)
		...
		case DONE:
			/* inflate stream terminated properly -- write leftover
			 * output */
			ret = Z_STREAM_END;
			if(left < state->wsize) {
...
int ZEXPORT inflateBackEnd(strm) z_stream FAR* strm;
{
	if(strm == Z_NULL || strm->state == Z_NULL ||
