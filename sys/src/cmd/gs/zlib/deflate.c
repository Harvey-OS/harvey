...
OF((deflate_state * s, IPos start, IPos match, int length));
#endif
...
int ZEXPORT deflateReset(strm) z_streamp strm;
{
	...
	if(s->wrap < 0) {
		s->wrap =
		    -s->wrap; /* was made negative by deflate(..., Z_FINISH); */
	}
...
int flush;
{
	...
	if(s->pending != 0) {
		...
		if(strm->avail_out == 0) {
			 ...
			 * avail_in equal to zero. There won't be anything to
			 * do,
			 * but this is not an error situation so make sure we
			 * return OK instead of BUF_ERROR at next call of
			 * deflate:
			 */
			s->last_flush = -1;
			return Z_OK;
		 ...
		 * consecutive
		 * flushes. For repeated and useless calls with Z_FINISH, we
		 * keep
	...
	if(strm->avail_in != 0 || s->lookahead != 0 ||
	   (flush != Z_NO_FLUSH && s->status != FINISH_STATE)) {
		...
		if(bstate == need_more || bstate == finish_started) {
			if(strm->avail_out == 0) {
				s->last_flush = -1; /* avoid BUF_ERROR next
				                       call, see above */
			}
			return Z_OK;
			/* If flush != Z_NO_FLUSH && avail_out == 0, the next
			 * call
			 * of deflate should use the same flush parameter to
			 * make sure
			 * that the flush is complete. So we don't have to
			 * output an
			 * empty block here, this will be done at next call.
			 * This also
			 * ensures that for a very small output buffer, we emit
			 * at most
			 * one empty block.
			 */
		}
		if(bstate == block_done) {
			...
			flush_pending(strm);
			if(strm->avail_out == 0) {
				s->last_flush = -1; /* avoid BUF_ERROR at next
				                       call, see above */
				return Z_OK;
			}
		}
 ...
 */
		 ...
		 * keys
		 * are equal and that HASH_BITS >= 8. Compare 2 bytes at a time
		 * at
		 * strstart+3, +5, ... up to strstart+257. We check for
		 * insufficient
		 * lookahead only every 4th comparison; the 128th check will be
		 * made
		 * at strstart+257. If MAX_MATCH-2 is not a multiple of 8, it is
		 * necessary to put more guard bytes at the end of the window,
		 * or
	...
	do {
		...
		if(sizeof(int) <= 2) {
			if(more == 0 && s->strstart == 0 && s->lookahead == 0) {
				 ...
				 * if
				 * strstart == 0 && lookahead == 1 (input done a
				 * byte at time)
				 */
				more--;
			}
		...
		if(s->strstart >= wsize + MAX_DIST(s)) {
			...
			/* Slide the hash table (could be avoided with 32 bit
			   values
			   at the expense of memory usage). We slide even when
			   level == 0
			   to keep the hash table consistent if we switch back
			   to level > 0
			   later. (Using level 0 permanently is not an optimal
			   usage of
 ...
 */
			...
			if((s->strategy < Z_HUFFMAN_ONLY) ||
			   (s->strategy == Z_RLE &&
				...
				s->match_length =
				    longest_match_fast(s, hash_head);
			}
#else
			if(s->strategy < Z_HUFFMAN_ONLY) {
...
#endif
			/* longest_match() or longest_match_fast() sets
			 * match_start */
		}
 ...
 */
			...
			if(s->match_length <= s->max_insert_length &&
			   s->lookahead >= MIN_MATCH) {
				s
				    ->match_length--; /* string at strstart
				                         already in table */
				do {
					s->strstart++;
				...
				Call UPDATE_HASH() MIN_MATCH - 3 more times
#endif
				/* If lookahead < MIN_MATCH, ins_h is garbage,
				 * but it does not
				 * matter since it will be recomputed at next
				 * deflate call.
				 */
			}
		} else {
	...
	for(;;) {
		...
		if(hash_head != NIL && s->prev_length < s->max_lazy_match &&
		   s->strstart - hash_head <= MAX_DIST(s)) {
			 ...
			 * string
			 * of window index 0 (in particular we have to avoid a
			 * match
			 * of the string with itself at the start of the input
			 * file).
			 */
			if(s->strategy < Z_HUFFMAN_ONLY) {
				s->match_length = longest_match(s, hash_head);
		...
		if(s->prev_length >= MIN_MATCH &&
		   s->match_length <= s->prev_length) {
			 ...
			 * match.
			 * strstart-1 and strstart are already inserted. If
			 * there is not
			 * enough lookahead, the last two strings are not
			 * inserted in
			 * the hash table.
			 */
			 ...
			 * match
			 * is longer, truncate the previous match to a single
			 * literal.
			 */
