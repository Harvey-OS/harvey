...
OF((unsigned FAR* have, unsigned char FAR* buf, unsigned len));
...
int ZEXPORT
    inflateInit2_(strm, windowBits, version, stream_size) z_streamp strm;
int windowBits;
const char* version;
int stream_size;
{
...
makefixed()
{
	...
	for(;;) {
		...
		printf("{%u,%u,%d}", state.distcode[low].op,
		       state.distcode[low].bits, state.distcode[low].val);
		if(++low == size)
			break;
		putchar(',');
	}
...
int flush;
{
	...
	static const unsigned short
	    order[19] = /* permutation of code lengths */
	...
	for(;;)
		...
		case TABLE:
			...
			if(state->nlen > 286 || state->ndist > 30) {
				strm->msg = (char*)"too many length or "
				                   "distance symbols";
				state->mode = BAD;
				break;
			}
