...
{	int code = gx_default_get_params(dev, plist);
	...
	code < 0 ||
	(code = param_write_int(plist, "BitsPerPixel", &pmdev->BitsPerPixel)) < 0 ||
...
uie:		param_signal_error(plist, "UpdateInterval", ecode);
	...
	case 0:
		if ( dev->is_open )
		  ...
		  {	code = pm_set_bits_per_pixel(&temp_dev, bpp);
			if ( code < 0 )
			  ecode = code;
			else
			  break;
...
bppe:		param_signal_error(plist, "BitsPerPixel", ecode);
	...
	if ( is_open && (dev->color_info.depth != bpp ||
			 dev->width != width || dev->height != height)
	   )
	{	int ccode;
		int old_bpp = pmdev->color_info.depth;
