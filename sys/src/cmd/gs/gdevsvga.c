...
/* Define the following symbol to use a fixed color table. */
/*#define FIXED_COLORS*/

#ifdef FIXED_COLORS

#define first_dc_index 256
...
/* (dummy) Initialize the dynamic color table. */
void
svga_init_colors()
{
}
...
{	gx_color_index ci;
	...
	for ( ci = 0; ci < 256; ci++ )
	{	pc_8bit_map_color_rgb(NULL, ci, rgb);
...
/* This requires some bizarre contortions! */
gx_color_index
...
/* The 8-color Tseng device */
private dev_proc_map_rgb_color(tseng8_map_rgb_color);
private gx_device_procs tseng8_procs =
	svga_procs_with_map(tseng_open, tseng8_map_rgb_color);
gx_device_svga gs_tseng8_device =
	svga_color_device(tseng8_procs, "tseng8", 8, 1, 2, atiw_get_mode, atiw_set_mode, tseng_set_page);
...
/* Map RGB to 1-bit color. */
gx_color_index
tseng8_map_rgb_color(gx_device *dev, gx_color_value r, gx_color_value g,
  gx_color_value b)
{	return (gx_color_index)
		((r > gx_max_color_value / 2 ? 044 : 0) +
		 (g > gx_max_color_value / 2 ? 022 : 0) +
		 (b > gx_max_color_value / 2 ? 011 : 0));
}
