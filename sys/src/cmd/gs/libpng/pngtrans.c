...
void /* PRIVATE */
    png_do_strip_filler(png_row_infop row_info, png_bytep row,
                        png_uint_32 flags)
{
...
#endif
	{
		...
		if((row_info->color_type == PNG_COLOR_TYPE_RGB ||
		   ...
		   row_info->channels == 4) {
			...
			{
				if(flags & PNG_FLAG_FILLER_AFTER) {
					/* This converts from RRGGBBXX or
					 * RRGGBBAA to RRGGBB */
					sp += 8;
					dp += 6;
					for(i = 1; i < row_width; i++) {
						/* This could be (although
						png_memcpy is probably slower):
						png_memcpy(dp, sp, 6);
						sp += 8;
						dp += 6;
						*/
					...
					/* This converts from XXRRGGBB or
					 * AARRGGBB to RRGGBB */
					for(i = 0; i < row_width; i++) {
						/* This could be (although
						png_memcpy is probably slower):
...
void PNGAPI
png_set_user_transform_info(png_structp png_ptr, png_voidp user_transform_ptr,
...
#else
	if(user_transform_ptr || user_transform_depth ||
	   user_transform_channels)
		png_warning(png_ptr, "This version of libpng does not support "
		                     "user transform info");
#endif
}
#endif
