   ...
   int interlace_type)
{
...

   {
      ...
      rowbytes = ((width * (png_size_t)png_ptr->pixel_depth + 7) >> 3);
      png_ptr->rowbytes = rowbytes;
      if((png_uint_32)png_ptr->rowbytes != rowbytes)
      {
         png_debug1(0,"in pngwutil: rowbytes = (%d)\n", rowbytes);
         png_warning(png_ptr, "rowbytes overflowed in png_write_IHDR.");
         png_ptr->rowbytes = 0;
      }
   }
