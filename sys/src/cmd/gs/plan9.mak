...
# makefile for Plan 9/ANSI C/POSIX configuration.
...
GS_LIB_DEFAULT=/usr/rob/gs/gs3.12:/usr/rob/gs/gs3.12/fonts:/sys/lib/ghostscript/font:/sys/lib/postscript/font
...
# in particular, SCO Xenix, Unix, and ODT just want
#XLIBDIRS=
# X11R6 probably needs SM and ICE added to XLIBS.
...
#DEVICE_DEVS=x11.dev x11alpha.dev x11cmyk.dev
...
DEVICE_DEVS=dfaxlow.dev pbm.dev pbmraw.dev plan9bm.dev 
...
level1.dev: gs.mak bcp.dev type1.dev
	$(SETMOD) level1 -include bcp type1
...
### We shouldn't have to include pattern.dev, but right now we do.
pdf.dev: gs.mak color.dev dctd.dev dps2core.dev dps2read.dev fdecode.dev level1.dev pdffonts.dev psl2core.dev psl2read.dev pdfread.dev   pattern.dev
	$(SETMOD) pdf -include color dctd dps2core dps2read fdecode
	$(ADDMOD) pdf -include level1 pdffonts psl2core psl2read pdfread   pattern
...
pdfread_=spdiff.$(OBJ) zfpdiff.$(OBJ)
pdfread.dev: gs.mak $(pdfread_)
	$(SETMOD) pdfread $(pdfread_)
	$(ADDMOD) pdfread -oper zfpdiff
	$(ADDMOD) pdfread -ps gs_pdf pdf_base pdf_draw pdf_font pdf_main
...
iccfont.$(OBJ): iccfont.c $(GH) gconfigf.h \
  ...
  $(oper_h) $(ostack_h) $(store_h) $(stream_h) $(sfilter_h) $(iscan_h)
...
#	plan9bm	Plan 9 bitmap; pbmraw with different header
...
install-exec: $(GS)
	...
	-mkdir $(scriptdir)
	for f in gsbj gsdj gslj gslp gsnd bdftops font2c ps2ascii ps2epsi ;\
