...
{	int depth;
	...
	m[0] /*left*/ = offset.x * 72.0 / dev->x_pixels_per_inch;
	m[3] /*top*/ = offset.y * 72.0 / dev->y_pixels_per_inch;
	m[2] /*right*/ =
		(size.x - offset.x - GetDeviceCaps(wdev->hdcprn, HORZRES))
		 * 72.0 / dev->x_pixels_per_inch;
	m[1] /*bottom*/ =
		(size.y - offset.y - GetDeviceCaps(wdev->hdcprn, VERTRES))
		 * 72.0 / dev->y_pixels_per_inch
		+ 0.15 * 72.0;  /* hack to add a bit more margin for deskjet printer */
