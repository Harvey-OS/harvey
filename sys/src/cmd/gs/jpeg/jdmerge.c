...
typedef struct {
	...
	/* Pointer to routine to do actual upsampling/conversion of one row
	 * group */
...
/* 2:1 vertical sampling case: may need a spare row. */
{
	...
	if(upsample->spare_full) {
		/* If we have a spare row saved from a previous cycle, just
		 * return it. */
		jcopy_sample_rows(&upsample->spare_row, 0,
		                  output_buf + *out_row_ctr, 0, 1,
