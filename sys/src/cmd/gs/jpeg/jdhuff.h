...
EXTERN(void) jpeg_make_d_derived_tbl JPP((j_decompress_ptr cinfo, boolean isDC,
...
#define CHECK_BIT_BUFFER(state, nbits, action)                                 \
	{                                                                      \
		if(bits_left < (nbits)) {                                      \
			if(!jpeg_fill_bit_buffer(&(state), get_buffer,         \
			                         bits_left, nbits)) {          \
				action;                                        \
			}                                                      \
...
#define HUFF_DECODE(result, state, htbl, failaction, slowlabel)                \
	{                                                                      \
		...
		if((nb = htbl->look_nbits[look]) != 0) {                       \
			...
			nb = HUFF_LOOKAHEAD + 1;                               \
		slowlabel:                                                     \
			if((result = jpeg_huff_decode(&state, get_buffer,      \
			                              bits_left, htbl, nb)) <  \
			   0) {                                                \
				failaction;                                    \
			}                                                      \
