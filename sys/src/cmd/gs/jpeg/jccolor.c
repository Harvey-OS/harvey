...
rgb_ycc_start(j_compress_ptr cinfo)
{
	...
	cconvert->rgb_ycc_tab = rgb_ycc_tab =
	    (INT32*)(*cinfo->mem->alloc_small)((j_common_ptr)cinfo, JPOOL_IMAGE,
                ...
                JSAMPIMAGE output_buf, JDIMENSION output_row, int num_rows)
{
	...
	while(--num_rows >= 0) {
		...
		for(col = 0; col < num_cols; col++) {
			...
			inptr += RGB_PIXELSIZE;
			/* If the inputs are 0..MAXJSAMPLE, the outputs of these
			 * equations
			 * must be too; we do not need an explicit
			 * range-limiting operation.
			 * Hence the value being shifted is never negative, and
			 * we don't
                  ...
                  JSAMPIMAGE output_buf, JDIMENSION output_row, int num_rows)
{
	...
	while(--num_rows >= 0) {
		...
		for(col = 0; col < num_cols; col++) {
			...
			outptr3[col] =
			    inptr[3]; /* don't need GETJSAMPLE here */
			inptr += 4;
			/* If the inputs are 0..MAXJSAMPLE, the outputs of these
			 * equations
			 * must be too; we do not need an explicit
                  ...
                  JSAMPIMAGE output_buf, JDIMENSION output_row, int num_rows)
{
	...
	while(--num_rows >= 0) {
		...
		for(col = 0; col < num_cols; col++) {
			outptr[col] =
			    inptr[0]; /* don't need GETJSAMPLE() here */
			inptr += instride;
		}
	}
}
             ...
             JDIMENSION output_row, int num_rows)
{
	...
	while(--num_rows >= 0) {
		...
		for(ci = 0; ci < nc; ci++) {
			...
			for(col = 0; col < num_cols; col++) {
				outptr[col] = inptr[ci]; /* don't need
				                            GETJSAMPLE() here */
				inptr += nc;
			}
		}
