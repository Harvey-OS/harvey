                 ...
                 JDIMENSION out_row_groups_avail)
{
	...
	while(*in_row_ctr < in_rows_avail &&
		...
		*in_row_ctr += numrows;
		 ...
		 * Note we assume the caller is providing a one-iMCU-height
		 * output buffer!
		 */
		if(prep->rows_to_go == 0 &&
		   *out_row_group_ctr < out_row_groups_avail) {
	...
	while(*out_row_group_ctr < out_row_groups_avail) {
		if(*in_row_ctr < in_rows_avail) {
			...
			*in_row_ctr += numrows;
			...
			/* Return for more data, unless we are at the bottom of
			 * the image. */
			if(prep->rows_to_go != 0)
				break;
			/* When at bottom of image, pad to fill the conversion
			 * buffer. */
		...
		if(prep->next_buf_row == prep->next_buf_stop) {
			(*cinfo->downsample->downsample)(
			    cinfo, prep->color_buf,
			    (JDIMENSION)prep->this_row_group, output_buf,
			    *out_row_group_ctr);
			(*out_row_group_ctr)++;
...
create_context_buffer(j_compress_ptr cinfo)
{
	...
	for(ci = 0, compptr = cinfo->comp_info; ci < cinfo->num_components;
	    ci++, compptr++) {
		/* Allocate the actual buffer space (3 row groups) for this
		 * component.
		...
		/* Copy true buffer row pointers into the middle of the fake row
		 * array */
