...
write_JPEG_file(char* filename, int quality)
{
	 ...
	 * to any one struct (and its associated working data) as a "JPEG
	 * object".
	 */
	struct jpeg_compress_struct cinfo;
	/* This struct represents a JPEG error handler.  It is declared
	 * separately
	 ...
	 * step fails.  (Unlikely, but it could happen if you are out of
	 * memory.)
	 ...
	 * VERY IMPORTANT: use "b" option to fopen() if you are on a machine
	 * that
...
read_JPEG_file(char* filename)
{
	 ...
	 * else,
	 * so that the setjmp() error recovery below can assume the file is
	 * open.
	 * VERY IMPORTANT: use "b" option to fopen() if you are on a machine
	 * that
	...
	while(cinfo.output_scanline < cinfo.output_height) {
		...
		/* Assume put_scanline_someplace wants a pointer and sample
		 * count. */
