                ...
                JDIMENSION input_row, JSAMPARRAY output_buf, int num_rows)
{
	...
	while(--num_rows >= 0) {
		...
		for(col = 0; col < num_cols; col++) {
			...
			/* Range-limiting is essential due to noise introduced
			 * by DCT losses. */
			outptr[RGB_RED] = range_limit[y + Crrtab[cr]];
			outptr[RGB_GREEN] =
			    range_limit[y + ((int)RIGHT_SHIFT(Cbgtab[cb] +
			                                          Crgtab[cr],
			                                      SCALEBITS))];
	...
	while(--num_rows >= 0) {
		for(ci = 0; ci < num_components; ci++) {
			...
			for(count = num_cols; count > 0; count--) {
				*outptr = *inptr++; /* needn't bother with
				                       GETJSAMPLE() here */
                  ...
                  JDIMENSION input_row, JSAMPARRAY output_buf, int num_rows)
{
	...
	while(--num_rows >= 0) {
		...
		for(col = 0; col < num_cols; col++) {
			...
			/* Range-limiting is essential due to noise introduced
			 * by DCT losses. */
			outptr[0] = range_limit[MAXJSAMPLE -
			                        (y + Crrtab[cr])]; /* red */
			outptr[1] = range_limit[MAXJSAMPLE -
			                        (y + /* green */
			                         ((int)RIGHT_SHIFT(
			                             Cbgtab[cb] + Crgtab[cr],
			                             SCALEBITS)))];
			outptr[2] = range_limit[MAXJSAMPLE -
			                        (y + Cbbtab[cb])]; /* blue */
			/* K passes through unchanged */
			outptr[3] =
			    inptr3[col]; /* don't need GETJSAMPLE here */
			outptr += 4;
		}
	}
}
	...
	case JCS_GRAYSCALE:
		...
		if(cinfo->jpeg_color_space == JCS_GRAYSCALE ||
		   cinfo->jpeg_color_space == JCS_YCbCr) {
			...
			/* For color->grayscale conversion, only the Y (0)
			 * component is needed */
