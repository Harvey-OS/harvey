...
jpeg_start_decompress(j_decompress_ptr cinfo)
{
	...
	if(cinfo->global_state == DSTATE_PRELOAD) {
		/* If file has multiple scans, absorb them all into the coef
		 * buffer */
				   ...
				   (retcode == JPEG_ROW_COMPLETED ||
					...
					if(++cinfo->progress->pass_counter >=
					   cinfo->progress->pass_limit) {
						/* jdmaster underestimated
						 * number of scans; ratchet up
						 * one scan */
						cinfo->progress->pass_limit +=
						    (long)
...
output_pass_setup(j_decompress_ptr cinfo)
{
	 ...
	 */
	cinfo->global_state =
	    cinfo->raw_data_out ? DSTATE_RAW_OK : DSTATE_SCANNING;
	return TRUE;
}
...
jpeg_finish_output(j_decompress_ptr cinfo)
{
	if((cinfo->global_state == DSTATE_SCANNING ||
	   ...
	   cinfo->buffered_image) {
		...
		/* BUFPOST = repeat call after a suspension, anything else is
		 * error */
		ERREXIT1(cinfo, JERR_BAD_STATE, cinfo->global_state);
	}
