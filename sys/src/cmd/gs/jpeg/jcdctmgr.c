...
start_pass_fdctmgr(j_compress_ptr cinfo)
{
	...
	for(ci = 0, compptr = cinfo->comp_info; ci < cinfo->num_components;
		...
		case JDCT_ISLOW:
			/* For LL&M IDCT method, divisors are equal to raw
			 * quantization
...
#define CONST_BITS 14
			static const INT16 aanscales[DCTSIZE2] = {
			    ...
			    16384, 22725, 21407, 19266, 16384, 12873, 8867,
			    4520,  22725, 31521, 29692, 26722, 22725, 17855,
			    12299, 6270,  21407, 29692, 27969, 25172, 21407,
			    16819, 11585, 5906,  19266, 26722, 25172, 22654,
			    19266, 15137, 10426, 5315,  16384, 22725, 21407,
			    19266, 16384, 12873, 8867,  4520,  12873, 17855,
			    16819, 15137, 12873, 10114, 6967,  3552,  8867,
			    12299, 11585, 10426, 8867,  6967,  4799,  2446,
			    4520,  6270,  5906,  5315,  4520,  3552,  2446,
			    1247};
			SHIFT_TEMPS
		...
		case JDCT_FLOAT: {
			 ...
			 *   scalefactor[0] = 1
			 *   scalefactor[k] = cos(k*PI/16) * sqrt(2)    for
			 * k=1..7
			 * We apply a further scale factor of 8.
			 * What's actually stored is 1/divisor so that the inner
			 * loop can
	...
	for(bi = 0; bi < num_blocks; bi++, start_col += DCTSIZE) {
		 ...
		 * coef_blocks[] */
		{
			...
			for(i = 0; i < DCTSIZE2; i++) {
				 ...
				 * rounding for negative
				 * quotients, we have to force the dividend
				 * positive for portability.
				 * The maximum coefficient size is +-16K (for
				 * 12-bit data), so this
				 * code should work for either 16-bit or 32-bit
				 * ints.
				 */
				output_ptr[i] = (JCOEF)(
