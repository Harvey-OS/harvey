...
/* Process a SOFn marker */
{
	 ...
	 * specified */
	/* as 0 and is later redefined by DNL.  As long as we have to check
	 * that,  */
...
/* Process a DHT marker */
{
	...
	while(length > 16) {
		...
		/* Here we just do minimal validation of the counts to avoid
		 * walking
...
/* Process a DQT marker */
{
	...
	while(length > 0) {
		...
		for(i = 0; i < DCTSIZE2; i++) {
			...
			/* We convert the zigzag-order table to natural array
			 * order. */
			quant_ptr->quantval[jpeg_natural_order[i]] =
			    (UINT16)tmp;
		}
		...
		if(cinfo->err->trace_level >= 2) {
			for(i = 0; i < DCTSIZE2; i += 8) {
				TRACEMS8(cinfo, 2, JTRC_QUANTVALS,
				         quant_ptr->quantval[i],
				         quant_ptr->quantval[i + 1],
				         quant_ptr->quantval[i + 2],
				         quant_ptr->quantval[i + 3],
				         quant_ptr->quantval[i + 4],
				         quant_ptr->quantval[i + 5],
				         quant_ptr->quantval[i + 6],
 ...
 */
{
	...
	if(datalen >= APP0_DATA_LEN && GETJOCTET(data[0]) == 0x4A &&
	   ...
	   GETJOCTET(data[3]) == 0x46 && GETJOCTET(data[4]) == 0) {
		...
		/* Check version.
		 * Major version must be 1, anything else signals an
		 * incompatible change.
		 * (We used to treat this as an error, but now it's a nonfatal
		 * warning,
		...
		TRACEMS5(cinfo, 1, JTRC_JFIF, cinfo->JFIF_major_version,
		         cinfo->JFIF_minor_version, cinfo->X_density,
		         cinfo->Y_density, cinfo->density_unit);
		...
		if(totallen != ((INT32)GETJOCTET(data[12]) *
			...
			TRACEMS1(cinfo, 1, JTRC_JFIF_BADTHUMBNAILSIZE,
			         (int)totallen);
	...
	if(cur_marker == NULL) {
		...
		if(length >= 0) { /* watch out for bogus length word */
			...
			if(cinfo->unread_marker == (int)M_COM)
				...
				limit =
				    marker->length_limit_APPn
				        [cinfo->unread_marker - (int)M_APP0];
			if((unsigned int)length < limit)
				limit = (unsigned int)length;
			/* allocate and initialize the marker item */
			cur_marker =
			    (jpeg_saved_marker_ptr) (*cinfo->mem->alloc_large)(
			        (j_common_ptr)cinfo, JPOOL_IMAGE,
...
next_marker(j_decompress_ptr cinfo)
{
	...
	for(;;) {
		...
		/* This loop swallows any duplicate FF bytes.  Extra FFs are
		 * legal as
...
read_markers(j_decompress_ptr cinfo)
{
	...
	for(;;) {
		...
		/* At this point cinfo->unread_marker contains the marker code
		 * and the
		 * input point is just past the marker proper, but before any
		 * parameters.
		 * A suspension will cause us to return with this state still
		 * true.
		 */
		switch(cinfo->unread_marker) {
		case M_SOI:
		...
		default: /* must be DHP, EXP, JPGn, or RESn */
			 /* For now, we treat the reserved markers as fatal
			  * errors since they are
			  * likely to be used to signal incompatible JPEG Part 3
			  * extensions.
			  * Once the JPEG 3 version-number marker is well
			  * defined, this code
			  * ought to change!
			  */
...
jpeg_resync_to_restart(j_decompress_ptr cinfo, int desired)
{
	...
	for(;;) {
		if(marker < (int)M_SOF0)
			...
			if(marker == ((int)M_RST0 + ((desired + 1) & 7)) ||
			   marker == ((int)M_RST0 + ((desired + 2) & 7)))
				action = 3; /* one of the next two expected
				               restarts */
		...
		case 1:
			...
			return TRUE;
		case 2:
			/* Scan to the next marker, and repeat the decision
			 * loop. */
			if(!next_marker(cinfo))
				return FALSE;
                  ...
                  unsigned int length_limit)
{
	...
	if(length_limit) {
		...
		/* If saving APP0/APP14, save at least enough for our internal
		 * use. */
		...
		/* If discarding APP0/APP14, use our regular on-the-fly
		 * processor. */
	...
	if(marker_code == (int)M_COM) {
		...
		marker->length_limit_APPn[marker_code - (int)M_APP0] =
		    length_limit;
	} else
