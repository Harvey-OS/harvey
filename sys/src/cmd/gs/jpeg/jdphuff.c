...
METHODDEF(boolean) decode_mcu_DC_first
    JPP((j_decompress_ptr cinfo, JBLOCKROW* MCU_data));
METHODDEF(boolean) decode_mcu_AC_first
    JPP((j_decompress_ptr cinfo, JBLOCKROW* MCU_data));
METHODDEF(boolean) decode_mcu_DC_refine
    JPP((j_decompress_ptr cinfo, JBLOCKROW* MCU_data));
...
decode_mcu_AC_first(j_decompress_ptr cinfo, JBLOCKROW* MCU_data)
{
	...
	if(!entropy->pub.insufficient_data) {
		 ...
		 */
		EOBRUN = entropy->saved
		             .EOBRUN; /* only part of saved state we need */
		...
		if(EOBRUN > 0)    /* if it's a band of zeroes... */
			...
			for(k = cinfo->Ss; k <= Se; k++) {
				HUFF_DECODE(s, br_state, tbl, return FALSE,
				            label2);
				r = s >> 4;
				s &= 15;
				if(s) {
					...
					if(r == 15) {    /* ZRL */
						k += 15; /* skip 15 zeroes in
						            band */
					} else { /* EOBr, run length is 2^r +
					            appended bits */
						EOBRUN = 1 << r;
						if(r) { /* EOBr, r > 0 */
							CHECK_BIT_BUFFER(
...
decode_mcu_AC_refine(j_decompress_ptr cinfo, JBLOCKROW* MCU_data)
{
	...
	if(!entropy->pub.insufficient_data) {
		...
		if(EOBRUN == 0) {
			for(; k <= Se; k++) {
				HUFF_DECODE(s, br_state, tbl, goto undoit,
				            label3);
				r = s >> 4;
				s &= 15;
				if(s) {
					if(s != 1) /* size of new coef should
					              always be 1 */
						WARNMS(cinfo,
						       JWRN_HUFF_BAD_CODE);
					CHECK_BIT_BUFFER(br_state, 1,
					                 goto undoit);
					if(GET_BITS(1))
						s = p1; /* newly nonzero coef is
						           positive */
					else
						s = m1; /* newly nonzero coef is
						           negative */
				} else {
					if(r != 15) {
						EOBRUN = 1 << r; /* EOBr, run
						...
						break; /* rest of block is
						          handled by EOB logic
						          */
					}
					/* note s = 0 for processing ZRL */
				}
				/* Advance over already-nonzero coefs and r
				 * still-zero coefs,
				 * appending correction bits to the nonzeroes.
				 * A correction bit is 1
				 * if the absolute value of the coefficient must
				 * be increased.
				 */
				do {
					thiscoef =
		...
		if(EOBRUN > 0) {
			 ...
			 * end-of-band
			 * (the last newly nonzero coefficient, if any).  Append
			 * a correction
			 * bit to each already-nonzero coefficient.  A
			 * correction bit is 1
			 * if the absolute value of the coefficient must be
			 * increased.
			 */
			for(; k <= Se; k++) {
