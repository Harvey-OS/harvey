...
METHODDEF(void) process_data_simple_main
    JPP((j_decompress_ptr cinfo, JSAMPARRAY output_buf, JDIMENSION* out_row_ctr,
 ...
 */
{
	...
	for(ci = 0, compptr = cinfo->comp_info; ci < cinfo->num_components;
		...
		rgroup =
		    ...
		    cinfo->min_DCT_scaled_size; /* height of a row group of
		                                   component */
		/* Get space for pointer lists --- M+4 row groups in each list.
		 * We alloc both pointer lists with one call to save a few
		 * cycles.
		 */
 ...
 */
{
	...
	for(ci = 0, compptr = cinfo->comp_info; ci < cinfo->num_components;
		...
		/* In the second list, put the last four row groups in swapped
		 * order */
		for(i = 0; i < rgroup * 2; i++) {
			xbuf1[rgroup * (M - 2) + i] = buf[rgroup * M + i];
			xbuf1[rgroup * M + i] = buf[rgroup * (M - 2) + i];
		}
		/* The wraparound pointers at top and bottom will be filled
		 * later
		 * (see set_wraparound_pointers, below).  Initially we want the
		 * "above"
		 * pointers to duplicate the first actual data line.  This only
		 * needs
 ...
 */
{
	...
	for(ci = 0, compptr = cinfo->comp_info; ci < cinfo->num_components;
		...
		/* Duplicate the last real sample row rgroup*2 times; this pads
		 * out the
		 * last partial rowgroup and ensures at least one full rowgroup
		 * of context.
		 */
...
start_pass_main(j_decompress_ptr cinfo, J_BUF_MODE pass_mode)
{
	...
	case JBUF_PASS_THRU:
		if(cinfo->upsample->need_context_rows) {
			...
			make_funny_pointers(
			    cinfo); /* Create the xbuffer[] lists */
			main->whichptr =
                         ...
                         JDIMENSION* out_row_ctr, JDIMENSION out_rows_avail)
{
	...
	if(!main->buffer_full) {
		...
		main->buffer_full =
		    TRUE; /* OK, we have an iMCU row to work with */
	}
	...
	(*cinfo->post->post_process_data)(
	    cinfo, main->buffer, &main->rowgroup_ctr, rowgroups_avail,
                          ...
                          JDIMENSION* out_row_ctr, JDIMENSION out_rows_avail)
{
	 ...
	 * handed
	 * in one call (due to filling the output buffer first).  Must be
	 * prepared
	 * to exit and restart.  This switch lets us keep track of how far we
	 * got.
	 * Note that each case falls through to the next on successful
	 * completion.
	 */
	...
	case CTX_PREPARE_FOR_IMCU:
		 ...
		 * "duplicate"
		 * the last sample row, and adjust rowgroups_avail to ignore
		 * padding rows.
		 */
...
jinit_d_main_controller(j_decompress_ptr cinfo, boolean need_full_buffer)
{
	...
	if(cinfo->upsample->need_context_rows) {
		...
		alloc_funny_pointers(
		    cinfo); /* Alloc space for xbuffer[] lists */
