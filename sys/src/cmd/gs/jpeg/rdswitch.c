...
#define MAX_SCANS 100 /* quite arbitrary limit */
	...
	while(read_scan_integer(fp, &val, &termchar)) {
		...
		while(termchar == ' ') {
			if(ncomps >= MAX_COMPS_IN_SCAN) {
				fprintf(stderr, "Too many components in one "
				                "scan in file %s\n",
				        filename);
				fclose(fp);
				return FALSE;
	...
	if(scanno > 0) {
		 ...
		 * NOTE: for cjpeg's use, JPOOL_IMAGE is the right lifetime for
		 * this data,
		 * but if you want to compress multiple images you'd want
		 * JPOOL_PERMANENT.
		 */
	...
	for(ci = 0; ci < MAX_COMPONENTS; ci++) {
		if(*arg) {
			...
			if(val < 0 || val >= NUM_QUANT_TBLS) {
				fprintf(stderr, "JPEG quantization tables are "
				                "numbered 0..%d\n",
				        NUM_QUANT_TBLS - 1);
				return FALSE;
			}
			...
			/* reached end of parameter, set remaining components to
			 * last table */
			cinfo->comp_info[ci].quant_tbl_no = val;
		}
	}
 ...
 */
{
	...
	for(ci = 0; ci < MAX_COMPONENTS; ci++) {
		if(*arg) {
			...
			if(sscanf(arg, "%d%c%d%c", &val1, &ch1, &val2, &ch2) <
			   3)
				return FALSE;
			if((ch1 != 'x' && ch1 != 'X') ||
			   ch2 != ',') /* syntax check */
