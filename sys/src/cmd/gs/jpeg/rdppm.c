...
start_input_ppm(j_compress_ptr cinfo, cjpeg_source_ptr sinfo)
{
	...
	if(use_raw_buffer) {
		/* For unscaled raw-input case, we can just map it onto the I/O
		 * buffer. */
	...
	if(need_rescale) {
		...
		/* On 16-bit-int machines we have to be careful of maxval =
		 * 65535 */
		...
		for(val = 0; val <= (INT32)maxval; val++) {
			/* The multiplication here must be done in 32 bits to
			 * avoid overflow */
			source->rescale[val] = (JSAMPLE)(
			    (val * MAXJSAMPLE + half_maxval) / maxval);
		}
	}
}
