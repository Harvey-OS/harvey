...
scan_JPEG_header(int keep_COM)
{
	...
	for(;;) {
		...
		default: /* Anything else just gets copied */
			write_marker(marker);
			copy_variable(); /* we assume it has a parameter
			                    count... */
			break;
		}
	} /* end loop */
...
/* complain about bad command line */
{
	...
	fprintf(stderr, "If you do not give either -comment or -cfile on the "
	                "command line,\n");
...
/* On Mac, fetch a command line. */
	...
	for(argn = 1; argn < argc; argn++) {
		...
		if(keymatch(arg, "replace", 1)) {
			...
			/* If the comment text starts with '"', then we are
			 * probably running
			 * under MS-DOG and must parse out the quoted string
			 * ourselves.  Sigh.
			 */
			if(comment_arg[0] == '"') {
				comment_arg =
	...
	if(comment_arg != NULL && comment_file != NULL)
		usage();
	/* If there is neither -comment nor -cfile, we will read the comment
	 * text
...
/* default output file is stdout */
	...
	/* Insert the new COM marker, but only if nonempty text has been
	 * supplied */
	if(comment_length > 0) {
		write_marker(M_COM);
