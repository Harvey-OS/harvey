			...
			if(post->buffer == NULL) {
				post->buffer =
				    (*cinfo->mem->access_virt_sarray)(
				        ...
				        (JDIMENSION)0, post->strip_height,
				        TRUE);
			}
		} else {
			/* For single-pass processing without color
			 * quantization,
			 * I have no work to do; just call the upsampler
			 * directly.
			 */
                   ...
                   JDIMENSION out_rows_avail)
{
	...
	/* Quantize and emit data. */
	(*cinfo->cquantize->color_quantize)(
	    cinfo, post->buffer + post->next_row, output_buf + *out_row_ctr,
	    (int)num_rows);
	*out_row_ctr += num_rows;
...
/* We round up the number of rows to a multiple of the strip height. */
			...
			/* One-pass color quantization: just make a strip
			 * buffer. */
