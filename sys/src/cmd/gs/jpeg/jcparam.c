 ...
 */
{
	/* These are the sample quantization tables given in JPEG spec section
	 * K.1.
...
jpeg_set_defaults(j_compress_ptr cinfo)
{
	 ...
	 * An application that wants to emit JFIF 1.02 extension markers should
	 *set
	 * JFIF_minor_version to 2.  We could probably get away with just
	 *defaulting
	 * to 1.02, but there may still be some decoders in use that will
	 *complain
...
#define SET_COMP(index, id, hsamp, vsamp, quant, dctbl, actbl)                 \
	...
	/* For all colorspaces, we use Q and Huff tables 0 for luminance
	 * components,
	...
	cinfo->write_Adobe_marker =
	    FALSE; /* write no Adobe marker by default */
...
jpeg_simple_progression(j_compress_ptr cinfo)
{
	...
	if(ncomps == 3 && cinfo->jpeg_color_space == JCS_YCbCr) {
		...
		if(ncomps > MAX_COMPS_IN_SCAN)
			...
			nscans = 2 + 4 * ncomps; /* 2 DC scans; 4 AC scans per
			                            component */
	}

	/* Allocate space for script.
	 * We need to put it in the permanent pool in case the application
	 * performs
	 * multiple compressions without changing the settings.  To avoid a
	 * memory
	 * leak if jpeg_simple_progression is called repeatedly for the same
	 * JPEG
