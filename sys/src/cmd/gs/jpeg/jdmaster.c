...
typedef struct {
	...
	boolean
	    using_merged_upsample; /* TRUE if using merged upsample/cconvert */
...
/* Do computations that are needed before master selection phase */
{
	 ...
	 * Note this code assumes that the supported DCT scalings are powers of
	 * 2.
	 */
...
/* Allocate and fill in the sample_range_limit table */
{
	...
	table +=
	    (MAXJSAMPLE + 1); /* allow negative subscripts of simple table */
...
master_selection(j_decompress_ptr cinfo)
{
	...
	if(cinfo->quantize_colors) {
		 ...
		 * active;
		 * this is necessary for starting with quantization to an
		 * external map.
		 */
	}
	...
	if(!cinfo->raw_data_out) {
		if(master->using_merged_upsample) {
			...
			jinit_merged_upsampler(
			    cinfo); /* does color conversion too */
#else
			ERREXIT(cinfo, JERR_NOT_COMPILED);
#endif
		} else {
	...
	use_c_buffer =
	    cinfo->inputctl->has_multiple_scans || cinfo->buffered_image;
	jinit_d_coef_controller(cinfo, use_c_buffer);

	if(!cinfo->raw_data_out)
		jinit_d_main_controller(
		    cinfo, FALSE /* never need full buffer here */);
	...
	if(cinfo->progress != NULL && !cinfo->buffered_image &&
	   cinfo->inputctl->has_multiple_scans) {
		...
		if(cinfo->progressive_mode) {
			...
			/* For a nonprogressive multiscan file, estimate 1 scan
			 * per component. */
...
prepare_for_output_pass(j_decompress_ptr cinfo)
{
	...
	if(cinfo->progress != NULL) {
		...
		/* In buffered-image mode, we assume one more output pass if EOI
		 * not
