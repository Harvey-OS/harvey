...
{	register const byte *p = pr->ptr;
	...
	{	byte b = *++p;
		int i;
		byte prev = b, repl;
		for ( i = 0; (repl = ss->prev[i]) != b; i++ )
		  ss->prev[i] = prev, prev = repl;
		ss->prev[i] = prev;
		*++q = (byte)i;
	}
	pr->ptr = p;
...
{	register const byte *p = pr->ptr;
	...
	byte
	  v0 = ss->prev[0], v1 = ss->prev[1],
	  v2 = ss->prev[2], v3 = ss->prev[3];
	while ( p < rlimit )
	{	byte first;
		...
		default:
		  {	uint b = *p;
			*++q = first = ss->prev[b];
			...
			switch ( b )
			  {
			  case 7: ss->prev[7] = ss->prev[6];
			  case 6: ss->prev[6] = ss->prev[5];
			  case 5: ss->prev[5] = ss->prev[4];
			  /* case 4: ; */
			  }
		  }
			ss->prev[4] = v3;
	...
	ss->prev[0] = v0;  ss->prev[1] = v1;
	ss->prev[2] = v2;  ss->prev[3] = v3;
	pr->ptr = p;
	pw->ptr = q;
	return status;
}
