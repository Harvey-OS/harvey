...
{	
  ...
  if ( pdev->x_pixels_per_inch == 120 )
    dev_type = DMP;
  else if ( pdev->x_pixels_per_inch == 320 )
    dev_type = IWLQ;
 ...
 */
      ...
      case IWLQ:
        ...
        while (prn_end > prn && prn_end[-1] == 0 && prn_end[-2] == 0 &&
               prn_end[-3] == 0)
          prn_end -= 3;
        while (prn_blk < prn_end && prn_blk[0] == 0 && prn_blk[1] == 0 &&
               prn_blk[2] == 0)
          prn_blk += 3;
        if (prn_end != prn_blk) {
          if ((prn_blk - prn) > 7)
            fprintf(prn_stream,"\033U%04d%c%c%c", (int)((prn_blk - prn)/3),
                    0,0,0);
          else
            prn_blk = prn;
          fprintf(prn_stream,"\033C%04d",(int)((prn_end - prn_blk)/3));
          fwrite(prn_blk, 1, (int)(prn_end - prn_blk), prn_stream);
        }
        break;
      case IWHI:
        for (count = 0; count < 2; count++) {
          ...
          if (prn_end != prn_blk) {
            if ((prn_blk - prn_tmp) > 7)
              fprintf(prn_stream,"\033V%04d%c", (int)(prn_blk - prn_tmp), 0);
            else
              prn_blk = prn_tmp;
            fprintf(prn_stream,"\033G%04d",(int)(prn_end - prn_blk));
            fwrite(prn_blk, 1, (int)(prn_end - prn_blk), prn_stream);
          }
      ...
      default:
        ...
        if (prn_end != prn_blk) {
          if ((prn_blk - prn) > 7)
            fprintf(prn_stream,"\033V%04d%c", (int)(prn_blk - prn), 0);
          else
            prn_blk = prn;
          fprintf(prn_stream,"\033G%04d",(int)(prn_end - prn_blk));
          fwrite(prn_blk, 1, (int)(prn_end - prn_blk), prn_stream);
