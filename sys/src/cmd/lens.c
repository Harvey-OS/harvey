...
enum { Edge = 5, Maxmag = 16 };

enum { Mzoom, Munzoom, Mgrid, Mredraw, Mexit };

char* menustr[] = {"zoom", "unzoom", "grid", "redraw", "exit", nil};

Menu menu = {menustr, nil, -1};
...
eresized(int new)
{
	...
	freeimage(tmp);
	tmp = allocimage(display, Rect(0, 0, Dx(screen->r) - Edge,
	                               Dy(screen->r) - Edge + Maxmag),
...
magnify(void)
{
	...
	if(readn(screenfd, screenbuf + bypp * yy * Dx(screenr),
	         bypp * Dx(screenr) * dd) != bypp * Dx(screenr) * dd) {
	...
	for(y = 0; y < dy; y++) {
		...
		if(yy >= 0 && yy < Dy(screenr))
			for(x = 0; x < dx; x++) {
				xx = xoff + x;
				if(xx >= 0 &&
				   xx < Dx(screenr)) /* snarf pixel at xx, yy */
					for(i = 0; i < bypp; i++)
						sp[i] = screenbuf
						    [bypp * (yy * Dx(screenr) +
						             xx) +
		...
		for(yy = 0; yy < mag && y * mag + yy < Dy(tmp->r); yy++) {
			werrstr("no error");
			if(loadimage(tmp, Rect(0, y * mag + yy, Dx(tmp->r),
			                       y * mag + yy + 1),
			             out,
			             bypp * Dx(tmp->r)) != bypp * Dx(tmp->r)) {
				exits("load");
			}
		}
	}
