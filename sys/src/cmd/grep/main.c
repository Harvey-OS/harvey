...
{
	fprint(2,
	       "usage: grep [-%s] [-e pattern] [-f patternfile] [file ...]\n",
	       validflags);
	exits("usage");
}
...
loop0:
	...
	/* if file has no final newline, simulate one to emit matches to last
	 * line */
	if(n > 0) {
		empty = 0;
		nl = u.buf[n - 1] == '\n';
...
loop:
	...
	if(c == '\n') {
		...
		if(!!s->match == !(flag & Vflag)) {
			...
			/* suppress extra newline at EOF unless we are labeling
