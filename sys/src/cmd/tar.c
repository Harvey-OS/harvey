...
{
	fprint(2,
	       "usage: %s {crtx}[PRTfgikmpsuvz] [archive] [file1 file2...]\n",
	       argv0);
	exits("usage");
}
...
refill(int ar, char* bufs, int justhdr)
{
	...
	if(first && usefile && !fixednblock) {
		...
		i = n;
		if(i % Tblock != 0)
			sysfatal("%s: archive block size (%d) error", arname,
			         i);
		i /= Tblock;
		if(i != nblock) {
			nblock = i;
...
name(Hdr* hp)
{
	...
	char* fullname;
	static char
	    fullnamebuf[2 + Maxname + 1]; /* 2+ for ./ on relative names */
...
isdir(Hdr* hp)
{
	...
	return hp->linkflag == LF_DIR || strrchr(name(hp), '\0')[-1] == '/' ||
...
hdrsize(Hdr* hp)
{
	...
	return hdrotoull(hp->size, hp->size + sizeof hp->size, 0, name(hp),
	                 "size");
}
...
parsecksum(char* cksum, char* name)
{
	...
	return hdrotoull(cksum, cksum + sizeof hp->chksum, (uint64_t)-1LL, name,
	                 "checksum");
}
...
readhdr(int ar)
{
	...
	if(hdrcksum == -1 || chksum(hp) != hdrcksum) {
		...
		fprint(2, "%s: skipping past archive header with bad checksum "
		          "in %s...",
		       argv0, arname);
		do {
			hp = getblkrd(ar, Alldata);
			if(hp == nil)
				sysfatal("unexpected EOF looking for archive "
				         "header in %s",
				         arname);
			hdrcksum = parsecksum(hp->chksum, name(hp));
...
/*
 * if name is int32_ter than Namsiz bytes, try to split it at a slash and fit
 * the
...
mkhdr(Hdr* hp, Dir* dir, char* file)
{
	...
	if(dir->length >= (Off)1 << 32) {
		...
		putbe((unsigned char*)hp->size + 1, dir->length,
		      sizeof hp->size - 2);
...
skip(int ar, Hdr* hp, char* msg)
{
	...
	for(blksleft = BYTES2TBLKS(bytes); blksleft > 0; blksleft -= blksread) {
		if(getblkrd(ar, Justnxthdr) == nil)
			sysfatal("unexpected EOF on archive %s %s", arname,
			         msg);
		blksread = gothowmany(blksleft);
...
copyfromar(int ar, int fd, char* fname, uint32_t blksleft, Off bytes)
{
	...
	for(; blksleft > 0; blksleft -= blksread) {
		...
		if(hbp == nil)
			sysfatal(
			    "unexpected EOF on archive extracting %s from %s",
			    fname, arname);
		blksread = gothowmany(blksleft);
		if(blksread <= 0) {
...
static void wrmeta(int fd, Hdr* hp, int32_t mtime,
                   int mode) /* update metadata */
{
	Dir nd;
