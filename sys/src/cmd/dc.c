...
commnds(void)
{
	...
	while(1) {
		...
		case ';':
			...
			q = salloc(
			    1); /*so uninitialized array elt prints as 0*/
			sputc(q, 0);
			pushp(q);
			continue;
		case 'x':
...
{
	int divsign, remsign, offset, divcarry, carry, dig, magic, d, dd, under,
	    first;
	int32_t c, td, cc;
