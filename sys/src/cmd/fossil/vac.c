...
mbCompact(MetaBlock* mb, MetaChunk* mc)
{
	...
	for(i = 0; i < mb->nindex; i++) {
		...
		if(o != oo) {
			...
			U16PUT(mb->buf + MetaHeaderSize +
			           mc[i].index * MetaIndexSize,
			       oo);
		}
		oo += n;
	}
...
deUnpack(DirEntry* dir, MetaEntry* me)
{
	...
	if(0)
		print("deUnpack: got options\n");

	if(p != me->p + me->size)
		goto Err;

	if(0)
		print("deUnpack: correct size\n");
	return 1;
Err:
	if(0)
		print("deUnpack: XXXXXXXXXXXX EBadMeta\n");
	vtSetError(EBadMeta);
	deCleanup(dir);
	return 0;
}
