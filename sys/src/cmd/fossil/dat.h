...
/* well known tags */
enum { BadTag = 0,   /* this tag should not be used */
...
struct Super {
	...
	unsigned char
	    last[VtScoreSize]; /* last snapshot successfully archived */
	char name[128];        /* label */
};
...
struct Entry {
	...
	unsigned char
	    archive; /* archive this snapshot: only valid for snap != 0 */
};
...
struct Source {
	...
	unsigned char
	    score[VtScoreSize]; /* score of block containing this source */
...
/* Block states */
enum { BsFree = 0,   /* available for allocation */
       ...
       BsCopied =
	   1
	   << 1, /* block has been copied (usually in preparation for unlink) */
       BsVenti = 1 << 2, /* block has been stored on Venti */
       BsClosed =
	   1 << 3, /* block has been unlinked on disk from active file system */
...
/* io states */
enum { BioEmpty,      /* label & data are not valid */
...
struct Fsck {
	...
	unsigned char*
	    xmap; /* all blocks in this epoch with parents in this epoch */
