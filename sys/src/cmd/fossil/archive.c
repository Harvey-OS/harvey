...
ventiSend(Arch* a, Block* b, uint8_t* data)
{
	...
	if(!vtSha1Check(score, data, n)) {
		...
		fprint(
		    2,
		    "ventiSend: vtWrite block %#ux failed vtSha1Check %V %V\n",
		    b->addr, score, score2);
		return 0;
	}
	if(!vtSync(a->z))
...
archWalk(Param* p, uint32_t addr, uint8_t type, uint32_t tag)
{
	...
	if(b == nil) {
		...
		if(strcmp(vtGetError(), ELabelMismatch) == 0) {
			/* might as well plod on so we write _something_ to
			 * Venti */
	...
	if(DEBUG)
		fprint(2, "%*sarchive(%ud, %#ux): block label %L\n",
	...
	if((b->l.state & BsVenti) == 0) {
		...
		for(i = 0; nextWalk(&w, score, &type, &tag, &e); i++) {
			...
			switch(x) {
			case ArchFailure:
				fprint(2, "archWalk %#ux failed; ptr is in "
				          "%#ux offset %d\n",
				       addr, b->addr, i);
				ret = ArchFailure;
				goto Out;
			case ArchFaked:
				/*
				 * When we're writing the entry for an archive
				 * directory
				 * (like /archive/2003/1215) then even if we've
				 * faked
				 * any data, record the score unconditionally.
				 * This way, we will always record the Venti
				 * score here.
				 * Otherwise, temporary data or corrupted file
				 * system
				 * would cause us to keep holding onto the
				 * on-disk
				 * copy of the archive.
				 */
				if(e == nil || !e->archive)
			...
			case ArchSuccess:
				if(e) {
					memmove(e->score, p->score,
					        VtScoreSize);
					e->flags &= ~VtEntryLocal;
					entryPack(e, data, w.n - 1);
				} else
					memmove(data + (w.n - 1) * VtScoreSize,
					        p->score, VtScoreSize);
				if(data == b->data) {
					blockDirty(b);
					/*
					 * If b is in the active tree, then we
					 *need to note that we've
					 * just removed addr from the active
					 *tree (replacing it with the
					 * copy we just stored to Venti).  If
					 *addr is in other snapshots,
					 * this will close addr but not free it,
					 *since it has a non-empty
					 * epoch range.
					 *
					 * If b is in the active tree but has
					 *been copied (this can happen
					 * if we get killed at just the right
					 *moment), then we will
					 * mistakenly leak its kids.
					 *
					 * The children of an archive directory
					 *(e.g., /archive/2004/0604)
					 * are not treated as in the active
					 *tree.
					 */
					if((b->l.state & BsCopied) == 0 &&
					   (e == nil || e->snap == 0))
						blockRemoveLink(b, addr,
						                p->l.type,
						                p->l.tag, 0);
				}
				break;
...
archThread(void* v)
{
	...
	for(;;) {
		...
		if(0)
			fprint(2,
			       "archiveSnapshot 0x%#ux: maxdepth %ud nfixed %ud"
			       " send %ud nfailsend %ud nvisit %ud"
			       " nreclaim %ud nfake %ud nreal %ud\n",
			       addr, p.maxdepth, p.nfixed, p.nsend, p.nfailsend,
			       p.nvisit, p.nreclaim, p.nfake, p.nreal);
		if(0)
			fprint(2, "archiveBlock %V (%ud)\n", p.score,
			       p.blockSize);
