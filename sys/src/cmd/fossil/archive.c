		...
		for(i = 0; nextWalk(&w, score, &type, &tag, &e); i++) {
			if(e) {
				if(!(e->flags & VtEntryActive))
					continue;
				if((e->snap && !e->archive) || (e->flags & VtEntryNoArchive)) {
					if(0)
...
archThread(void *v)
{
	...
	for(;;) {
		...
		if(0)
			fprint(2, "archiveSnapshot 0x%#ux: maxdepth %ud nfixed %ud"
		...
		if(!vtWrite(a->z, p.score, VtRootType, rbuf, VtRootSize) || !vtSha1Check(p.score, rbuf, VtRootSize)) {
