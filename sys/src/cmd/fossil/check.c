...
static int walkEpoch(Fsck* chk, Block* b, uint8_t score[VtScoreSize], int type,
          ...
          uint32_t tag, uint32_t epoch)
{
	...
	if(bb == nil) {
		error(chk, "could not load block %V type %d tag %ux: %R", score,
		      type, tag);
		chk->walkdepth--;
		return 0;
	}
	...
	if(b) {
		...
		if(b->l.epoch < bb->l.epoch || bb->l.epochClose <= b->l.epoch) {
			error(chk, "walk: block %#ux [%ud, %ud) points at %#ux "
			           "[%ud, %ud)",
			      b->addr, b->l.epoch, b->l.epochClose, bb->addr,
			      bb->l.epoch, bb->l.epochClose);
			goto Exit;
		}
		...
		if(b->l.epoch == epoch && bb->l.epoch == epoch) {
			if(getBit(chk->emap, addr)) {
				error(chk,
				      "walk: epoch join detected: addr %#ux %L",
				      bb->addr, &bb->l);
				goto Exit;
			}
		...
		if(!(b->l.state & BsCopied) && b->l.epoch == bb->l.epoch) {
			if(getBit(chk->xmap, addr)) {
				error(chk,
				      "walk: copy join detected; addr %#ux %L",
				      bb->addr, &bb->l);
				goto Exit;
			}
	...
	if(epoch == chk->fs->ehi) {
		...
		if(bb->l.state & BsClosed)
			error(chk, "walk: addr %#ux: block is in active tree "
			           "but is closed",
			      addr);
	} else if(!getBit(chk->amap, addr))
		if(!(bb->l.state & BsClosed)) {
			// error(chk, "walk: addr %#ux: block is not in active
			// tree, not closed (%d)",
	...
	if(chk->nseen++ % chk->quantum == 0)
		chk->print("check: visited %d/%d blocks (%.0f%%)\n", chk->nseen,
		           chk->nblocks, chk->nseen * 100. / chk->nblocks);
	...
	case BtDir:
		for(i = 0; i < chk->bsize / VtEntrySize; i++) {
			if(!entryUnpack(&e, bb->data, i)) {
				// error(chk, "walk: could not unpack entry:
				// %ux[%d]: %R",
				//	addr, i);
				setBit(chk->errmap, bb->addr);
				chk->clre(chk, bb, i);
			...
			if(e.snap != 0) {
				if(e.snap >= epoch) {
					// error(chk, "bad snap in entry:
					// %ux[%d] snap = %ud: epoch = %ud",
					//	addr, i, e.snap, epoch);
					setBit(chk->errmap, bb->addr);
					chk->clre(chk, bb, i);
					chk->nclre++;
			...
			if(e.flags & VtEntryLocal) {
				if(e.tag < UserTag)
					if(e.tag != RootTag || tag != RootTag ||
					   i != 1) {
						// error(chk, "bad tag in entry:
						// %ux[%d] tag = %ux",
						//	addr, i, e.tag);
						setBit(chk->errmap, bb->addr);
						chk->clre(chk, bb, i);
				...
				// error(chk, "bad tag in entry: %ux[%d] tag =
				// %ux",
				//	addr, i, e.tag);
				setBit(chk->errmap, bb->addr);
				chk->clre(chk, bb, i);
			...
			if(!walkEpoch(chk, bb, e.score, entryType(&e), e.tag,
			              ep)) {
				setBit(chk->errmap, bb->addr);
				chk->clre(chk, bb, i);
				chk->nclre++;
...
checkLeak(Fsck* chk)
{
	...
	for(a = 0; a < chk->nblocks; a++) {
		...
		if(l.state & BsClosed)
			continue;
		nlost++;
		//		warn(chk, "unreachable block: addr 0x%ux type %d
		//tag 0x%ux "
		//			"state %s epoch %ud close %ud", a, l.type,
		//l.tag,
	...
	chk->print("fsys blocks: total=%ud used=%ud(%.1f%%) free=%ud(%.1f%%) "
	           "lost=%ud(%.1f%%)\n",
	           chk->nblocks, chk->nblocks - nfree - nlost,
	           100. * (chk->nblocks - nfree - nlost) / chk->nblocks, nfree,
	           100. * nfree / chk->nblocks, nlost,
	           100. * nlost / chk->nblocks);
}
           ...
           uint32_t gen, int dir, MetaBlock* mb, int i, Block* b)
{
	...
	r = nil;
	if(getBit(bm, offset)) {
		warn(chk, "multiple references to source: %s -> %d", name,
		     offset);
		goto Err;
	}
	setBit(bm, offset);
...
scanSource(Fsck* chk, char* name, Source* r)
{
	...
	for(o = 0; o < nb; o++) {
		...
		if(b == nil) {
			error(chk, "could not read block in data file %s",
			      name);
			continue;
		}
		if(b->addr != NilBlock && getBit(chk->errmap, b->addr)) {
			warn(chk, "previously reported error in block %ux is "
			          "in file %s",
			     b->addr, name);
		}
		blockPut(b);
...
chkDir(Fsck* chk, char* name, Source* source, Source* meta)
{
	...
	for(o = 0; o < nb; o++) {
		...
		if(b == nil) {
			error(chk,
			      "could not read block in meta file: %s[%ud]: %R",
			      name, o);
			continue;
		}
		if(0)
		...
		if(b->addr != NilBlock && getBit(chk->errmap, b->addr))
			warn(chk,
			     "previously reported error in block %ux is in %s",
			     b->addr, name);
		...
		for(i = mb.nindex - 1; i >= 0; i--) {
			meUnpack(&me, &mb, i);
			if(!deUnpack(&de, &me)) {
				error(chk, "could not unpack dir entry: "
				           "%s[%ud][%d]: %R",
				      name, o, i);
				continue;
			}
			if(s && strcmp(s, de.elem) <= 0)
				error(chk, "dir entry out of order: "
				           "%s[%ud][%d] = %s last = %s",
				      name, o, i, de.elem, s);
			vtMemFree(s);
			s = vtStrDup(de.elem);
			...
			r = openSource(chk, source, nn, bm, de.entry, de.gen, 1,
			               &mb, i, b);
			if(r == nil) {
				deCleanup(&de);
				free(nn);
			...
			mr = openSource(chk, source, nn, bm, de.mentry, de.mgen,
			                0, &mb, i, b);
			if(mr == nil) {
				sourceClose(r);
				deCleanup(&de);
