...
Dfmt(Fmt* fmt)
{
	...
	if(c->namelen == 1 && c->name[0] == '\0' || c->name[0] == '\001') {
		...
		snprint(buf, sizeof buf, "%.*T dloc %.4N dlen %.4N", c->namelen,
		        c->name, c->dloc, c->dlen);
	}
	fmtstrcpy(fmt, buf);
	return 0;
}
...
iso9660init(int xfd, Header* xh, char* xfile9660, int xoff9660)
{
	...
	if(memcmp(sect, sect2, Blocksize) != 0)
		vtFatal("iso9660 offset is a lie %08ux %08ux", *(int32_t*)sect,
		        *(int32_t*)sect2);
}
...
/*
 * The first block used is the first data block of the leftmost file in the
 * tree.
...
iso9660start(Cdir* c)
{
	...
	while(c->flags & 2) {
		...
		/* oops: might happen if leftmost directory is empty or leftmost
		 * file is zero length! */
		if(little(c->dloc, 4) == 0)
			vtFatal(
			    "error parsing cd image or unfortunate cd image");
	}
...
static uint32_t getcdate(uint8_t* p) /* yMdhmsz */
{
	Tm tm;
	int y, M, d, h, m, s, tz;
...
iso9660copyfile(Fs* fs, File* dir, Cdir* c)
{
	...
	if(sysl <= 0)
		vtFatal("missing plan9 directory entry on %d/%d/%.*s",
		        c->namelen, c->name[0], c->namelen, c->name);
	d.name = getname(&p);
	d.uid = getname(&p);
	d.gid = getname(&p);
	if((uintptr)p & 1)
	...
	if(d.mode & DMDIR)
		...
		for(foff = 0; foff < len; foff += h->blockSize) {
			localToGlobal((off + foff - fsoff) / h->blockSize,
			              score);
