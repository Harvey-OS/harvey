 ...
 */
int
dinit(Disk *d, char *mc, int psize, char *expname)
{
	ulong	i;
	uvlong	length;
	char	buf[1024];
	 ...
	 */
	if(bcinit(d, mc, d->bsize) < 0){
		fprint(2, "%s: dinit: couldn't init block cache\n", argv0);
		return -1;
	}
	for(i = 0; i < d->nab; i++){
		b = bcread(d, i);
...
/*
 *  format the allocated memory as a cache
 */
int
dformat(Disk *d, char *mc, char *name, ulong bsize, ulong psize)
{
	int	i;
	uvlong	length;
	Bbuf	*b;
	...
	d->bsize = bsize;
	if((d->bsize % psize) != 0){
		fprint(2, "%s: logical bsize not multiple of physical\n", argv0);
		return -1;
	}
	d->nb = length/d->bsize;
	d->b2b = (d->bsize - sizeof(Dahdr))*8;
