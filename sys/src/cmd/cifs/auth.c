...
static Auth *
auth_ntlmv2(char *windom, char *keyp, u8 *chal, int len)
{
	int i, n;
	Rune r;
	char *p, *u;
	u8 v1hash[MD5dlen], blip[Bliplen], blob[1024], v2hash[MD5dlen];
	u8 c, lm_hmac[MD5dlen], nt_hmac[MD5dlen], nt_sesskey[MD5dlen],
		lm_sesskey[MD5dlen];
	DigestState *ds;
	UserPasswd *up;
	static Auth *ap;
...
{
	DigestState *ds;
	u8 *sig, digest[MD5dlen], theirs[MAClen];
