...
enum { Nomatch,    /* not found in cache */
...
dfscacheinfo(Fmt* f)
{
	...
	for(cp = Cache; cp; cp = cp->next) {
		...
		fmtprint(f, "%-42s %6ld %8.1f %4d %-16s %-24s %s\n", cp->src,
		         ex, (double)cp->rtt / 1000.0L, cp->prox, cp->host,
		         cp->share, cp->path);
	}
	return 0;
}
	...
	if(cp->rtt < rtt && (rtt / labs(rtt - cp->rtt)) < Rtt_tol) {
		if(Debug && strstr(Debug, "dfs") != nil)
			print("	remap bad ping %ld < %ld && %ld < %d\n",
			      cp->rtt, rtt, (rtt / labs(rtt - cp->rtt)),
			      Rtt_tol);
		return -1;
	}
...
redirect(Session* s, Share* sp, char* path)
{
	...
	if(cp && match == Exactmatch) {
		if(cp->expiry >= time(nil)) { /* cache hit */
			if(Debug && strstr(Debug, "dfs") != nil)
				print("redirect cache=hit src=%q => share=%q "
				      "path=%q\n",
			...
			free(unc);
			if(Debug && strstr(Debug, "dfs") != nil)
				print("redirect refresh cache=stale src=%q => "
				      "share=%q path=%q\n",
				      cp->src, cp->share, cp->path);
			return 0;
		}
