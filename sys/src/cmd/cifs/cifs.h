...
enum { Proot = 1,             /* LSBs of qid.path for root dir */
...
typedef struct {
	...
	unsigned char
	    chal[0xff + 1]; /* server's challange for authentication  */
...
int CIFS_NT_opencreate(Session*, Share*, char*, int, int, int, int, int, int,
                       int*, FInfo*);
