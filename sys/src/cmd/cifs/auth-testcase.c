...
auth_plain(char* windom, char* keyp, uint8_t* chal, int len)
{
	...
	up = auth_getuserpasswd(
	    auth_getkey, "windom=%s proto=pass service=cifs %s", windom, keyp);
	if(!up)
...
auth_lm_and_ntlm(char* windom, char* keyp, uint8_t* chal, int len)
{
	...
	err = auth_respond(
	    chal, len, user, sizeof user, &mcr, sizeof mcr, auth_getkey,
	    "windom=%s proto=mschap role=client service=cifs %s", windom, keyp);
	if(err == -1)
		sysfatal("cannot get key - %r");
...
hmac_t64(uint8_t* data, uint32_t dlen, uint8_t* key, uint32_t klen,
         uint8_t* digest, DigestState* state)
{
	if(klen > 64)
		klen = 64;
...
auth_ntlmv2(char* windom, char* keyp, uchar* chal, int len)
{
	...
	up = auth_getuserpasswd(auth_getkey,
	                        "windom=%s proto=pass service=cifs-ntlmv2 %s",
	                        windom, keyp);
	if(!up)
		sysfatal("cannot get key - %r");
...
#ifdef NTLMV2_TEST
	{
		static uchar srvchal[] = {0x52, 0xaa, 0xc8, 0xe8,
		                          0x2c, 0x06, 0x7f, 0xa1};
	...
	{
		uchar v1[] = {
			0x0c, 0xb6, 0x94, 0x88, 0x05, 0xf7, 0x97, 0xbf, 0x2a,
			0x82, 0x80, 0x79, 0x73, 0xb8, 0x95, 0x37;
...
Auth*
getauth(char* name, char* windom, char* keyp, int secmode, uint8_t* chal,
        int len)
{
	int i;
	Auth* ap;
	...
	if(name == nil) {
		...
		if((secmode & SECMODE_PW_ENCRYPT) == 0)
			sysfatal("plaintext authentication required, use '-a "
			         "plain'");
	}
...
macsign(Pkt* p)
{
	...
	for(i = -3; i < 4; i++) {
		...
		if(genmac(buf, len, p->seq + i, p->s->auth->mackey[0], mac) ==
		   0) {
			dmp("got", 0, buf, len);
		...
		if(genmac(buf, len, p->seq + i, p->s->auth->mackey[1], mac) ==
		   0) {
			dmp("got", 0, buf, len);
