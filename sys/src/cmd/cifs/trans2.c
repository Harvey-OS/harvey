...
t2hdr(Session* s, Share* sp, int cmd)
{
	...
	p->tbase =
	    pl16(p, 0); /* 0  Total parameter bytes to be sent, filled later */
...
int T2setfilelength(Session* s, Share* sp, int fh,
                    FInfo* fip) /* FIXME: maybe broken, needs testing */
{
	int rc;
	Pkt* p;
                 ...
                 Refer* re, int nent)
{
	...
	pt2param(p);
	pl16(p, 3); /* max info level we understand, must be >= 3 for domain
	               requests */
	ppath(p, path);
	...
	*gflags = gl32(p);   /* global flags */
	...
	for(i = 0; i < nret && i < nent && i < 16; i++) {
		...
		case 2:
			...
			re[i].path = estrdup9p(tmp);
			goff(p, base, re[i].path,
			     sizeof tmp); /* spurious 8.3 path */
