...
static struct {
    ...
    {2162, "operation cannot be performed on the print destination in its "
           "current state"},
    {2163,
     "operation cannot be performed on the printer queue in its current state"},
    {2164,
     "operation cannot be performed on the print job in its current state"},
    ...
    {2214, "logon server was not specified. computer will be logged on as "
           "STANDALONE"},
    ...
    {2248, "No updates are necessary to this replicant network/local security "
           "database"},
    ...
    {2281,
     "message was sent, but the recipient has paused the Messenger service"},
    ...
    {2318, "This device cannot be shared as both a spooled and a non-spooled "
           "resource"},
    ...
    {2466,
     "server cannot open more files because it has reached its maximum number"},
    ...
    {2483,
     "UPS indicated a line fail or low battery situation. Service not started"},
    ...
    {2610,
     "Service failed to startas no network adapters started with this service"},
    {2611,
    ...
    {2635, "There are workstations, profiles, or configurations using this "
           "boot block"},
    ...
    {2665,
     "indicated server share does not support the indicated DFS namespace"},
    ...
    {2696, "specified computer name is incompatible with the default language "
           "used on the DC"},
    ...
    {2705,
     "password does not meet the requirements of the password filter DLLs"},
};
...
raperrstr(uint err)
{
	...
	if(match != -1)
		...
		snprint(buf, sizeof buf, "rap: %ud/0x%ux - unknown error", err,
		        err);
	return buf;
}
