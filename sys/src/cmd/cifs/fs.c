...
shareinfo(Fmt* f)
{
	...
	for(i = 0; i < n; i++) {
		...
		if(RAPshareinfo(Sess, sp, sp->name, &si2) != -1) {
			...
			fmtprint(f, "%-8s %5d/%-5d %s", type, si2.activeusrs,
			         si2.maxusrs, si2.comment);
			free(si2.name);
			free(si2.comment);
			free(si2.path);
			free(si2.passwd);
...
openfileinfo(Fmt* f)
{
	...
	for(i = 0; i < got; i++) {
		fmtprint(f, "%c%c%c %-4d %-24q %q ",
		         (fi[i].perms & 1) ? 'r' : '-',
		         (fi[i].perms & 2) ? 'w' : '-',
		         (fi[i].perms & 4) ? 'c' : '-', fi[i].locks, fi[i].user,
		         fi[i].path);
		free(fi[i].path);
		free(fi[i].user);
	}
...
conninfo(Fmt* f)
{
	...
	fmtprint(f, "%q %q %q %q %+ldsec %dmtu %s\n", Sess->auth->user,
	         Sess->cname, Sess->auth->windom, Sess->remos, Sess->slip,
	         Sess->mtu, Sess->isguest ? "as guest" : "");
...
dfsredir(Fmt* f, char* path, int depth)
{
	...
	n = T2getdfsreferral(Sess, &Ipc, path, &flags, &used, retab,
	                     nelem(retab));
	if(n == -1)
		return;
...
groupinfo(Fmt* f)
{
	...
	for(i = 0; i < got1; i++) {
		...
		usrs = nil;
		if((got2 = RAPgroupusers(Sess, &Ipc, grps[i].name, &usrs)) !=
		   -1) {
			for(j = 0; j < got2; j++) {
...
nodelist(Fmt* f, int type)
{
	...
	Serverinfo* si;
	static char* types[] = {
	        [0] "workstation", [1] "server", [2] "SQL server", [3] "DC",
	        [4] "backup DC", [5] "time source", [6] "Apple server",
	        [7] "Novell server", [8] "domain member", [9] "printer server",
	        [10] "dial-up server", [11] "Unix", [12] "NT", [13] "WFW",
	        [14] "MFPN (?)", [15] "NT server", [16] "potential browser",
	        [17] "backup browser", [18] "LMB", [19] "DMB", [20] "OSF Unix",
	        [21] "VMS", [22] "Win95", [23] "DFS", [24] "NT cluster",
	        [25] "Terminal server", [26] "[26]", [27] "[27]",
	        [28] "IBM DSS",
	};
...
period(int32_t sec)
{
	...
	sec -= min * 60L;
	if(days)
		snprint(when, sizeof(when), "%d %d:%d:%ld ", days, hrs, min,
		        sec);
	else
