...
cifsrpc(Pkt* p)
{
	...
	if(memcmp(m, magic, nelem(magic)) != 0) {
		werrstr(
		    "cifsrpc: bad magic number in packet %20ux%02ux%02ux%02ux",
		    m[0], m[1], m[2], m[3]);
		return -1;
	}
              ...
              char* cname, int cnamlen)
{
	...
	*svrtime = gvtime(p); /* fileserver time */
	s->tz = (int16_t)gl16(p) *
	        60;         /* TZ in mins, is signed (SNIA doc is wrong) */
...
CIFSsession(Session* s)
{
	...
	Rune r;
	Pkt* p;
	enum { mycaps = CAP_UNICODE | CAP_LARGE_FILES | CAP_NT_SMBS |
	...
	if((s->secmode & SECMODE_PW_ENCRYPT) == 0) {
		...
		pl16(p, utflen(Sess->auth->resp[0]) * 2 +
		            2); /* passwd size (UPPER CASE) */
		pl32(p, 0);     /* Reserved */
		pl32(p, mycaps);
		pbytes(p);
...
CIFSrename(Session* s, Share* sp, char* old, char* new)
{
	...
	pl16(p, ATTR_HIDDEN | ATTR_SYSTEM |
	            ATTR_DIRECTORY); /* search attributes */
	pbytes(p);
	p8(p, STR_ASCII);
	ppath(p, old);
	p8(p, STR_ASCII);
...
CIFS_NT_opencreate(Session* s, Share* sp, char* name, int flags, int options,
                   int attrs, int access, int share, int action, int* result,
                   FInfo* fi)
{
	Pkt* p;
	int fh;
	...
	p8(p, SECURITY_CONTEXT_TRACKING |
	          SECURITY_EFFECTIVE_ONLY); /* security flags */
	pbytes(p);
...
/* for Win95/98/ME */
CIFS_SMB_opencreate(Session* s, Share* sp, char* name, int access, int attrs,
