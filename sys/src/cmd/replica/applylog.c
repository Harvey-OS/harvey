...
resolve(char* name)
{
	...
	for(i = 0; i < nres; i++) {
		...
		if(len == 0)
			return res[i].c;
		if(strncmp(name, res[i].name, len) == 0 &&
		   (name[len] == '/' || name[len] == 0))
			return res[i].c;
	}
	return '?';
...
usage(void)
{
	fprint(2, "usage: replica/applylog [-cnSstuv] [-T timefile] clientdb "
...
main(int argc, char** argv)
{
	...
	int checkedmatch1, checkedmatch2, checkedmatch3, checkedmatch4;
	ulong now;
	Biobuf bin;
	Dir dbd, ld, nd, rd;
	Avlwalk* w;
		...
		case 'd': /* delete file */
			...
			if(!havedb) {
				if(resolve1 == 's')
					...
					goto DoRemoveDb;
				conflict(name,
				         "locally created; will not remove");
				skip = 1;
				continue;
			}
			assert(havelocal && havedb);
			if(dbd.mtime > rd.mtime) /* we have a newer file than
			                            what was deleted */
				break;
			if(samecontents(local, remote) >
			   0) { /* going to get recreated */
				chat("= %q %luo %q %q %lud\n", name, rd.mode,
				     rd.uid, rd.gid, rd.mtime);
				break;
			}
			if(!(dbd.mode & DMDIR) &&
			   (dbd.mtime != ld.mtime ||
			    dbd.length != ld.length)) { /* locally modified
			                                   since we downloaded
			                                   it */
				if(resolve1 == 's')
					goto DoRemove;
					...
					break;
				conflict(name,
				         "locally modified; will not remove");
				skip = 1;
				continue;
			}
		DoRemove:
		...
		case 'a': /* add file */
			if(!havedb) {
				...
				if(samecontents(local, remote) > 0) {
					chat("= %q %luo %q %q %lud\n", name,
					     rd.mode, rd.uid, rd.gid, rd.mtime);
					goto DoCreateDb;
				}
				if(resolve1 == 's')
					goto DoCreate;
					...
					goto DoCreateDb;
				conflict(name,
				         "locally created; will not overwrite");
				skip = 1;
				continue;
			}
			assert(havedb);
			if(dbd.mtime >=
			   rd.mtime) /* already created this file; ignore */
				break;
			if(havelocal) {
				if((ld.mode & DMDIR) && (rd.mode & DMDIR))
					break;
				...
				if(dbd.mtime == ld.mtime &&
				   dbd.length == ld.length)
					goto DoCreate;
				if(resolve1 == 's')
					goto DoCreate;
				else if(resolve1 == 'c')
				...
				conflict(
				    name,
				    "locally modified; will not overwrite");
				skip = 1;
				continue;
			}
			if(!ismatch(name)) {
		...
		DoCreate:
			...
			if(rd.mode & DMDIR) {
				...
				if(dirfwstat(fd, &nd) < 0)
					fprint(
					    2,
					    "warning: cannot set gid on %q\n",
					    local);
				if(douid) {
					nulldir(&nd);
					nd.uid = rd.uid;
					if(dirfwstat(fd, &nd) < 0)
						fprint(2, "warning: cannot set "
						          "uid on %q\n",
						       local);
				}
				close(fd);
				...
				if(copyfile(local, remote, name, &rd, 1, &k) <
				   0) {
					if(k)
						addce(local);
					skip = 1;
		...
		case 'c': /* change contents */
			if(!havedb) {
				...
				if(havelocal)
					conflict(
					    name,
					    "locally created; will not update");
				else
					conflict(
					    name,
					    "not replicated; will not update");
				skip = 1;
				continue;
			}
			if(dbd.mtime >=
			   rd.mtime) /* already have/had this version; ignore */
				break;
			if(!ismatch(name)) {
				skip = prstopped(skip, name);
				continue;
			...
			if(!havelocal) {
				...
				if(resolve1 == 's')
					...
					break;
				conflict(name,
				         "locally removed; will not update");
				skip = 1;
				continue;
			}
			...
			if(dbd.mtime != ld.mtime || dbd.length != ld.length) {
				...
				if(resolve1 == 's')
					...
					goto DoCopyDb;
				conflict(
				    name, "locally modified; will not update "
				          "[%llud %lud -> %llud %lud]",
				    dbd.length, dbd.mtime, ld.length, ld.mtime);
				skip = 1;
				continue;
			}
		DoCopy:
		...
		case 'm': /* change metadata */
			if(!havedb) {
				...
				if(resolve1 == 's') {
					...
					goto DoMetaDb;
				if(havelocal)
					conflict(name, "locally created; will "
					               "not update metadata");
				else
					conflict(name, "not replicated; will "
					               "not update metadata");
				skip = 1;
				continue;
			}
			if(!(dbd.mode & DMDIR) &&
			   dbd.mtime >
			       rd.mtime) /* have newer version; ignore */
				break;
			if((dbd.mode & DMDIR) && dbd.mtime > now)
				break;
			if(havelocal &&
			   (!douid || strcmp(ld.uid, rd.uid) == 0) &&
			   strcmp(ld.gid, rd.gid) == 0 && ld.mode == rd.mode)
				break;
			if(!havelocal) {
				if(notexists(remote)) {
					addce(local);
				...
				if(resolve1 == 's') {
					...
					break;
				conflict(name, "locally removed; will not "
				               "update metadata");
				skip = 1;
				continue;
			}
			if(!(dbd.mode & DMDIR) &&
			   (dbd.mtime != ld.mtime ||
			    dbd.length !=
			        ld.length)) { /* this check might be overkill */
				if(notexists(remote)) {
					addce(local);
					/* no skip=1 */
					break;
				...
				SET(checkedmatch4);
				if(resolve1 == 's' ||
				   samecontents(local, remote) > 0)
					goto DoMeta;
				else if(resolve1 == 'c')
					break;
				conflict(name, "contents locally modified "
				               "(%s); will not update metadata "
				               "to %s %s %luo",
				         dbd.mtime != ld.mtime
				             ? "mtime"
				             : dbd.length != ld.length
			...
			if((douid && strcmp(ld.uid, dbd.uid) != 0) ||
			   strcmp(ld.gid, dbd.gid) != 0 ||
			   ld.mode != dbd.mode) {
				if(notexists(remote)) {
					addce(local);
					/* no skip=1 */
				...
				if(resolve1 == 's')
					...
					break;
				conflict(name, "metadata locally changed; will "
				               "not update metadata to %s %s "
				               "%luo",
				         rd.uid, rd.gid, rd.mode);
				skip = 1;
				continue;
...
copytotemp(char* remote, int rfd, Dir* d0)
{
	...
	if(d0->qid.path != d1->qid.path || d0->qid.vers != d1->qid.vers ||
	   d0->mtime != d1->mtime || d0->length != d1->length) {
...
DoCopy:
	 ...
	 */
	p = strchr(name, '/');
	if(safeinstall && p && strncmp(p, "/bin/", 5) == 0 &&
	   access(local, AEXIST) >= 0) {
		/*
		 * remove bin/_targ
		 */
	...
	if((dl = dirstat(local)) == nil) {
		...
		if((wfd = open(local, OTRUNC | OWRITE)) >= 0) {
			...
			if(dirfwstat(wfd, &nd) < 0)
				fprint(2, "warning: set mode on %s to 0660 to "
				          "open; cannot set back to %luo: %r\n",
				       local, nd.mode);
			goto okay;
		}
		nd.mode = dl->mode;
		if(dirwstat(local, &nd) < 0)
			fprint(2, "warning: set mode on %s to %luo to open; "
			          "open failed; cannot set mode back to %luo: "
			          "%r\n",
