...
walk(char *new, char *old, Dir *xd, void *)
{
	...
	if(markdb(db, new, &od) < 0) {
		...
		if((!uid && strcmp(od.uid, d.uid) != 0) || strcmp(od.gid, d.gid) != 0 || od.mode != d.mode) {
			xlog('m', new, &d);
			change = 1;
		}
	}
...
warn(char *msg, void *)
{
	 ...
	 */
	if(p) {
		if(cistrstr(p, "hungup") || cistrstr(p, "Hangup") || cistrstr(p, "rpc error") || cistrstr(p, "shut down") || cistrstr(p, "i/o") || cistrstr(p, "connection"))
