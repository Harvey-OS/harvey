 ...
 *	If the name is "screen" or "display" or "vga", reads the colormap from
...
getcmap(int id, char* f, unsigned char* buf)
{
	...
	if(cmap == -1) {
		if(strcmp(name, "screen") == 0 ||
		   strcmp(name, "display") == 0 || strcmp(name, "vga") == 0) {
...
main(int argc, char* argv[])
{
	...
	if(strncmp(buf + 2 * 12, "         m8 ", 12) != 0)
		sysfatal("can't set colormap except on CMAP8 (m8) displays; "
		         "this one is %.12s",
		         buf + 2 * 12);
