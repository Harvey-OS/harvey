...
struct {
    ...
    "domain", offsetof(Cookie, dom), 1, "path", offsetof(Cookie, path), 1,
    "name", offsetof(Cookie, name), 0, "value", offsetof(Cookie, value), 0,
    "comment", offsetof(Cookie, comment), 1, "version",
    offsetof(Cookie, version), 1,
};
...
struct {
    ...
    "expire", offsetof(Cookie, expire), "secure", offsetof(Cookie, secure),
    "explicitdomain", offsetof(Cookie, explicitdom), "explicitpath",
    offsetof(Cookie, explicitpath), "netscapestyle",
    offsetof(Cookie, netscapestyle),
};
...
jarfmt(Fmt* fmt)
{
	...
	for(i = 0; i < jar->nc; i++)
		fmtprint(fmt, "%s%s=%s", i ? "; " : "", jar->c[i].name,
		         jar->c[i].value);
	fmtprint(fmt, "\r\n");
	return 0;
}
...
expirejar(Jar* jar, int exiting)
{
	...
	n = 0;
	for(i = 0; i < jar->nc; i++) {
		if(jar->c[i].expire < now ||
		   (exiting && jar->c[i].expire == ~0)) {
...
isdomainmatch(char* name, char* pattern)
{
	...
	if(strcmp(ipattr(name), "dom") == 0 && pattern[0] == '.') {
		...
		if(lname >= lpattern &&
		   cistrcmp(name + lname - lpattern, pattern) == 0)
			return 1;
	}
...
iscookiematch(Cookie* c, char* dom, char* path, uint now)
{
	return isdomainmatch(dom, c->dom) &&
	       strncmp(c->path, path, strlen(c->path)) == 0 && c->expire >= now;
}
...
cookiesearch(Jar* jar, char* dom, char* path, int issecure)
{
	...
	qsort(j->c, j->nc, sizeof(j->c[0]),
	      (int (*)(const void*, const void*))cookiecmp);
	return j;
}
...
parsehttp(Jar* jar, char* hdr, char* dom, char* path)
{
	...
	for(p = hdr; p; p = nextp) {
		...
		for(; *p; p = skipspace(p)) {
			if((e = parsecookie(&c, p, &p, isns, dom, path)) !=
			   nil) {
				if(debug)
...
fswrite(Req* r)
{
	...
	case Xhttp:
		if(a->state == NeedUrl) {
			...
			memmove(a->inhttp + strlen(a->inhttp), r->ifcall.data,
			        r->ifcall.count);
		}
	...
	case Xcookies:
		...
		memmove(a->ctext + r->ifcall.offset, r->ifcall.data,
		        r->ifcall.count);
		r->ofcall.count = r->ifcall.count;
		respond(r, nil);
		return;
...
main(int argc, char** argv)
{
	...
	closefile(
	    createfile(fs.tree->root, "http", "cookie", 0666, (void*)Xhttp));
	closefile(createfile(fs.tree->root, "cookies", "cookie", 0666,
	                     (void*)Xcookies));
