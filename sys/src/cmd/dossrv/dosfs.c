...
rwalk(void)
{
	...
	for(; rep->nwqid < req->nwname && rep->nwqid < MAXWELEM; rep->nwqid++) {
		...
		if(strcmp(req->wname[rep->nwqid], ".") == 0) {
			 ...
			 */
			r = searchdir(f, req->wname[rep->nwqid], dp, 0,
			              longtype);
			putfile(f);
			if(r < 0)
				goto error;
 ...
 */
static int
mkdentry(Xfs* xf, Dosptr* ndp, char* name, char* sname, int longtype, int nattr,
...
rcreate(void)
{
	...
	if(mkdentry(f->xf, ndp, req->name, sname, longtype, nattr, start, 0) <
	   0) {
		if(ndp->p != nil)
			putsect(ndp->p);
		free(ndp);
...
doremove(Xfs* xf, Dosptr* dp)
{
	...
	dp->p->flags |= BMOD;
	for(prevdo = dp->offset - DOSDIRSIZE; prevdo >= 0;
	    prevdo -= DOSDIRSIZE) {
	...
	if(prevdo < 0 && dp->prevaddr != -1) {
		...
		for(prevdo = ((Dosbpb*)xf->ptr)->sectsize - DOSDIRSIZE;
		    prevdo >= 0; prevdo -= DOSDIRSIZE) {
...
dostat(Xfile* f, Dir* d)
{
	...
	if(isroot(dp->addr)) {
		...
		for(prevdo = dp->offset - DOSDIRSIZE; prevdo >= 0;
			...
			if(dp->p->iobuf[prevdo + 11] != 0xf)
				break;
			name = getnamesect(namebuf, name, &dp->p->iobuf[prevdo],
			                   &islong, &sum, -1);
		}
		if(prevdo < 0 && dp->prevaddr != -1) {
			p = getsect(f->xf, dp->prevaddr);
			for(prevdo =
			        ((Dosbpb*)f->xf->ptr)->sectsize - DOSDIRSIZE;
			    prevdo >= 0; prevdo -= DOSDIRSIZE) {
				if(p->iobuf[prevdo + 11] != 0xf)
					break;
				name = getnamesect(namebuf, name,
				                   &p->iobuf[prevdo], &islong,
				                   &sum, -1);
			}
			putsect(p);
		}
...
rwstat(void)
{
	...
	dir.name = repdata;
	dostat(f, &dir);
	if(convM2D(req->stat, req->nstat, &wdir, (char*)statbuf) !=
	   req->nstat) {
		errno = Ebadstat;
		goto out;
	}
	...
	if(wdir.name[0] != '\0' && strcmp(dir.name, wdir.name) != 0) {
		...
		/*
		 * grab parent directory of file to be changed and check for
		 * write perm
		...
		/*
		 * search for dir entry again, since we may be able to use the
		 * old slot,
		 * and we need to set up the naddr field if a long name spans
		 * the block.
		 * create new entry.
		 */
		if(searchdir(&pf, wdir.name, dp, 1, longtype) < 0 ||
		   mkdentry(pf.xf, dp, wdir.name, sname, longtype, attr, start,
		            length) < 0) {
			putsect(parp);
			errno = Eio;
			goto out;
