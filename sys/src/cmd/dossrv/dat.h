 ...
 */
enum { NOFATMIRROR = 0x0080, /* masks for extflags */
       ACTFATMASK = 0x000f,
};
...
struct Dosptr {
	...
	uint32_t clust;
	uint32_t naddr; /* next block in directory (for writing multi entry
	                   elements) */
	uint32_t prevaddr;
	Iosect* p;
	Dosdir* d;
...
#define QIDPATH(p)                                                             \
	((p)->addr * (Sectorsize / DOSDIRSIZE) + (p)->offset / DOSDIRSIZE)
...
enum { Asis, Clean, Clunk };

enum { Invalid, Short, ShortLower, Long };
