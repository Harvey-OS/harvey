...
enum { Maxdoms = 10, /* max domains in a path */
       Timeout =
	   2 * 60 * 60, /* seconds until temporarily trusted addr times out */
};
...
ipwalk(char* name, Node* np)
{
	...
	if(debugfd >= 0)
		fprint(debugfd, "%d.%d.%d.%d - ", addr[0] & 0xff,
...
/*
 * match path components to prohibited domain & user specifications.  patterns
 *include:
 *	domain, domain! or domain!*	  - all users in domain
 *	*.domain, *.domain! or *.domain!* - all users in domain and its
 *subdomains
 *	!user or *!user			  - user in all domains
 *	domain!user			  - user in domain
 *	*.domain!user			  - user in domain and its subdomains
 *
 *	if "user" has a trailing '*', it matches all user names beginning with
 *"user"
 *
 * there are special semantics for the "domain, domain! or domain!*"
 *specifications:
 * the first two forms match when the domain is anywhere in at list of
 *source-routed
 * domains while the latter matches only when the domain is the last hop.  the
 *same is
