...
enum { But1 = Button_bit(1), /* mouse buttons for events */
       But2 = Button_bit(2),
       But3 = Button_bit(3),
};
...
/* Prepend string s to null-terminated string in n-byte buffer buf[], truncating
   if
   necessary and using a space to separate s from the rest of buf[].
*/
char*
str_insert(char* buf, char* s, int n)
{
...
/* Alter string smain (without lengthening it) so as to remove the first
   occurrence of
   ssub, assuming ssub is ASCII.  Return nonzero (true) if string smain had to
   be changed.
   In spite of the ASCII-centric appearance, I think this can handle UTF in
   smain.
*/
int
remove_substr(char* smain, char* ssub)
{
...
/* Wait for a mouse click and return 0 for failue if not button but (curs can be
 * 0) */
int
get_1click(int but, Mouse* m, Cursor* curs)
{
	if(curs)
		esetcursor(curs);
	while(m->buttons == 0)
...
/* Wait for a mouse click or keyboard event from the string of expected
   characters.  Return
   the character code or -1 for a button-but mouse event or 0 for wrong button.
*/
int
get_click_or_kbd(int but, Mouse* m, const char* expected)
{
	Event ev;
	...
	do
		ty = eread(Emouse | Ekeyboard, &ev);
	while((ty & Emouse)
	          ? ev.mouse.buttons == 0
	          : (ev.kbdc & ~127) ||
	                !(expbits[(ev.kbdc >> 5) & 3] & (1 << (ev.kbdc & 31))));
	if(ty & Ekeyboard)
		return ev.kbdc;
	*m = ev.mouse;
...
*/
int
lift_button(int but, Mouse* m, int tlimit)
{
	do {
		*m = emouse();
		if(m->msec >= tlimit)
...
enum { DOrange = 0xffaa00FF,
       Dgray = 0xbbbbbbFF,
       DDkgreen = 0x009900FF,
       DDkred = 0xcc0000FF,
       DViolet = 0x990099FF,
       DDkyellow = 0xaaaa00FF,
       DLtblue = 0xaaaaffFF,
       DPink = 0xffaaaaFF,
...
color_ref clrtab[] = {
    DRed,    "Red",    'R', 0, DPink,     "Pink",     'P', 0,
    DDkred,  "Dkred",  'r', 0, DOrange,   "Orange",   'O', 0,
    DYellow, "Yellow", 'Y', 0, DDkyellow, "Dkyellow", 'y', 0,
    DGreen,  "Green",  'G', 0, DDkgreen,  "Dkgreen",  'g', 0,
    DCyan,   "Cyan",   'C', 0, DBlue,     "Blue",     'B', 0,
    DLtblue, "Ltblue", 'b', 0, DMagenta,  "Magenta",  'M', 0,
    DViolet, "Violet", 'V', 0, Dgray,     "Gray",     'A', 0,
    DBlack,  "Black",  'K', 0, DWhite,    "White",    'W', 0,
...
/* Return an allocated array that describes the color letters (clrtab[].nam1
   values,
   optionally prefixed by 'T') in the string that starts at c0 and ends just
   before
   fin.  The first entry is a dummy whose thick field tells how many data
   entries follow.
   If buf!=0, it should point to an array of length 2 that is to hold the output
   (truncated to a dummy and one data entry).  The error indication is 1 data
   entry
   of default color and thickness; e.g., "Multi(xxbadxx)" in a label prevents
   gview
   from recognizing anything that follows.
*/
thick_color*
parse_color_chars(const char* c0, const char* fin, thick_color* buf)
{
	thick_color* tc; /* Pending return value */
	int i, j,
	    n = fin - c0; /* n is an upper bound on how many data members */
	const char* c;
	for(c = c0; c < fin - 1; c++)
		if(*c == 'T')
			n--;
...
/* Decide what color and thickness to use for a polyline based on the label it
   has in the
   input file.  The winner is whichever color name comes first, or otherwise
   black; and
   thickness is determined by the presence of "Thick" in the string.  Place the
   result
   in *r1 and return 0 unless a Multi(...) spec is found, in which case the
   result is
   an allocated array of alternative color and thickness values.  A nonzero
   idxdest
   requests the clrtab[] index in *idxdest and no allocated array.
*/
thick_color*
nam2thclr(const char* nam, thick_color* r1, int* idxdest)
{
	char* c, * cbest = 0, * rp = 0;
	int i, ibest = -1;
	thick_color* tc = 0;
	...
	if(ibest == is_Multi) {
		tc = parse_color_chars(cbest + 6, rp,
		                       (idxdest == 0 ? 0 : &buf[0]));
	...
	r1->thick =
	    (tc != 0) ? tc[1].thick : (strstr(nam, "Thick") == 0 ? 0 : 1);
	return tc;
}
...
/* Alter string nam so that nam2thick() and nam2clr() agree with *tc, using
   ...
   We go to great pains to perform this alteration in a manner that will seem
   natural
   to the user, i.e., we try removing a suitably isolated color name before
   inserting
   a new one.
*/
char*
nam_with_thclr(char* nam, const thick_color* tc, char* buf, int bufn)
{
	thick_color c0;
	int clr0i;
...
*/
int
fintersects(const frectangle* r1, const frectangle* r2, double slant)
{
...
int
fcontains(const frectangle* r, fpoint p)
{
	return r->min.x <= p.x && p.x <= r->max.x && r->min.y <= p.y &&
	       p.y <= r->max.y;
}

void
grow_bb(frectangle* dest, const frectangle* r)
{
	if(r->min.x < dest->min.x)
		dest->min.x = r->min.x;
	if(r->min.y < dest->min.y)
		dest->min.y = r->min.y;
	if(r->max.x > dest->max.x)
		dest->max.x = r->max.x;
	if(r->max.y > dest->max.y)
		dest->max.y = r->max.y;
}

void
slant_frect(frectangle* r, double sl)
{
	r->min.y += sl * r->min.x;
	r->max.y += sl * r->max.x;
}
...
fpolygons univ = {/* everything there is to display */
                  0, 1e30, 1e30, -1e30, -1e30, 0, 0, 0, 0, 2 * 1e30};

void
free_fp_etc(fpolygon* fp)
{
	if(fp->ct != 0)
		free(fp->ct);
	free(fp->p);
...
void
set_default_clrs(fpolygons* fps, fpolygon* fpstop)
{
	fpolygon* fp;
...
void
fp_remove(fpolygons* fps, fpolygon* fp)
{
	fpolygon* q, ** p = &fps->p;
	while(*p != fp)
		if(*p == 0)
...
/* The transform maps abstract fpoint coordinates (the ones used in the input)
   to the current screen coordinates.  The do_untransform() macros reverses
   this.
   ...
   left and upper right corners of univ.disp; slant_ht<0 refers to a
   parallelogram
...
#define do_transform(d, tr, s)                                                 \
	((d)->x = (tr)->o.x + (tr)->sc.x * (s)->x,                             \
	 (d)->y = (tr)->o.y + (tr)->sc.y * (s)->y + (tr)->sl * (s)->x)
#define do_untransform(d, tr, s)                                               \
	((d)->x = (.5 + (s)->x - (tr)->o.x) / (tr)->sc.x,                      \
	 (d)->y = (.5 + (s)->y - (tr)->sl * (d)->x - (tr)->o.y) / (tr)->sc.y)
...
cur_trans(void)
{
	...
	if(sh > 0) {
		t.sl = -t.sc.y * (s->max.y - s->min.y - sh) /
		       (s->max.x - s->min.x);
		t.o.y = d.min.y - t.sc.y * s->max.y - t.sl * s->max.x;
	} else {
		t.sl =
		    t.sc.y * (s->max.y - s->min.y + sh) / (s->max.x - s->min.x);
...
/* Set *y0 and *y1 to the lower and upper bounds of the set of y-sl*x values
  *that
   *u says to display, where sl is the amount of slant.
*/
double
set_unslanted_y(fpolygons* u, double* y0, double* y1)
{
...
void
nontrivial_interval(double* lo, double* hi)
{
	if(*lo >= *hi) {
		double mid = .5 * (*lo + *hi);
...
*/
double
untransform_corners(double rminx, double rminy, double rmaxx, double rmaxy,
                    fpoint* ul, fpoint* lr)
{
...
void
disp_dozoom(double rminx, double rminy, double rmaxx, double rmaxy)
{
	fpoint ul, lr;
...
void
disp_zoomin(Rectangle r)
{
	disp_dozoom(r.min.x, r.min.y, r.max.x, r.max.y);
}

void
disp_zoomout(Rectangle r)
{
...
void
expand2(double* a, double* b, double f)
{
	double mid = .5 * (*a + *b);
	*a = mid + f*(*a - mid);
	*b = mid + f*(*b - mid);
...
disp_squareup(void)
{
	...
	if(dx > dy)
		...
		expand2(&univ.disp.min.x, &univ.disp.max.x, dy / dx);
...
*/
void
slant_disp(fpoint p, fpoint q)
{
	double yll, ylr, yul,
	    yur; /* corner y coords of displayed parallelogram */
	double sh, dy;
	if(p.x == q.x)
		return;
	sh = univ.slant_ht;
	if(sh > 0) {
		yll = yul = univ.disp.min.y;
		yul += sh;
		ylr = yur = univ.disp.max.y;
		ylr -= sh;
	} else {
		yll = yul = univ.disp.max.y;
		yll += sh;
		ylr = yur = univ.disp.min.y;
		yur -= sh;
	}
...
int
is_valid_label(char* lab)
{
	char* t;
	if(lab[0] == '"')
		return (t = strrchr(lab, '"')) != 0 && t != lab &&
		       strspn(t + 1, " \t\r\n") == strlen(t + 1);
	return strcspn(lab, " \t") == strlen(lab);
}

/* Read a polyline and update the number of lines read.  A zero result indicates
   bad
   syntax if *lineno increases; otherwise it indicates end of file.
*/
fpolygon*
rd_fpoly(FILE* fin, int* lineno)
{
...
/* Read input into *fps and return 0 or a line number where there's a syntax
 * error */
int
rd_fpolys(FILE* fin, fpolygons* fps)
{
...
void
wr_fpoly(FILE* fout, const fpolygon* fp)
{
	char buf[256];
	int i;
	for(i = 0; i <= fp->n; i++)
...
void
wr_fpolys(FILE* fout, fpolygons* fps)
{
	fpolygon* fp;
...
*/
int
do_xory(double x0, double x1, double xlo, double xhi, double* t0, double* t1)
{
	*t1 = 1.0;
	if(x0 < xlo) {
		if(x1 < xlo)
...
*/
double
frac_outside(const fpoint* p, const fpoint* q, const frectangle* r,
             double slope)
{
	...
	if(!do_xory(px, qx, r->min.x, r->max.x, &t0, &t1))
		return 1;
	if(!do_xory(p->y - slope * px, q->y - slope * qx, r->min.y, r->max.y,
	            &tt0, &tt1))
		return 1;
	if(tt0 > t0)
		t0 = tt0;
...
*/
double
in_length(const fpoint* p0, const fpoint* pn, frectangle r, double slope)
{
	const fpoint* p = p0;
	double px, py;
	do
...
*/
double
out_length(const fpoint* p0, const fpoint* pn, frectangle r, double slope)
{
	const fpoint* p = p0;
	double fr;
	do {
...
char*
abbrev_num(double x, const lab_interval* iv)
{
	static char buf[16];
	double dx = x - iv->off;
...
lab_interval
next_larger(double s0, double xlo, double xhi)
{
	double nlo, nhi;
	lab_interval r;
	...
	if(nhi >= digs10pow)
		...
		r.off = 0;
	r.sep = (s0 <= r.unit) ? r.unit
	                       : (s0 < 2 * r.unit ? 2 * r.unit : 5 * r.unit);
	switch(r.logunit % 3) {
	case 1:
		r.unit *= .1;
		r.logunit--;
...
lab_interval
mark_x_axis(const transform* tr)
{
	...
	for(seps = 0, nseps = floor(x1 / iv.sep) - seps0; seps <= nseps;
	    seps += 1) {
...
lab_interval
mark_y_axis(const transform* tr)
{
	Font* f = display->defaultfont;
	fpoint p = univ.disp.min;
	Point q, qrt, qlft;
	...
	for(seps = 0, nseps = floor(y1 / iv.sep) - seps0; seps <= nseps;
	    seps += 1) {
...
void
lab_iv_info(const lab_interval* iv, double slant, char* buf, int* n)
{
	if(iv->off > 0)
...
void
draw_xy_ranges(const lab_interval* xiv, const lab_interval* yiv)
{
	Point p;
...
#define mydist(p, o, sl, xwt, ywt)                                             \
	(myx = (p).x - (o).x, myy = (p).y - sl * (p).x - (o).y,                \
	 xwt * myx * myx + ywt * myy * myy)

/* At what fraction of the way from p0[0] to p0[1] is
   mydist(p,ctr,slant,xwt,ywt)
   minimized?
*/
double
closest_time(const fpoint* p0, const fpoint* ctr, double slant, double xwt,
             double ywt)
{
	double p00y = p0[0].y - slant * p0[0].x,
	       p01y = p0[1].y - slant * p0[1].x;
...
*/
int
improve_pt(fpoint* p0, double len, const frectangle* r, double slant,
           pt_on_fpoly* psel)
{
	fpoint ctr = fcenter(r);
	double x_wt = 2 / (r->max.x - r->min.x),
	       y_wt = 2 / (r->max.y - r->min.y);
	double xwt = x_wt * x_wt, ywt = y_wt * y_wt;
	double d, dbest = (psel->t < 0) ? 1e30
	                                : mydist(psel->p, ctr, slant, xwt, ywt);
...
/* Test *fp against *r after transforming by y=y-x*slope, and set *psel
 * accordingly.
*/
void
select_in_fpoly(fpolygon* fp, const frectangle* r, double slant,
                pt_on_fpoly* psel)
{
	fpoint* p0 = fp->p, * pn = fp->p + fp->n;
	double l1, l2;
	if(p0 == pn) {
		improve_pt(p0, 0, r, slant, psel);
		psel->fp = fp;
		return;
	}
...
/* Test all the fpolygons against *r after transforming by y=y-x*slope, and
   return
   the resulting selection, if any.
*/
pt_on_fpoly*
select_in_univ(const frectangle* r, double slant)
{
...
Rectangle
sel_dot_box(const transform* tr)
{
	Point ctr;
	Rectangle r;
	if(tr == 0)
		...
		do_transform(&ctr, tr, &cur_sel.p);
	r.min.x = ctr.x - Dotrad;
	r.max.x = ctr.x + Dotrad + 1;
	r.min.y = ctr.y - Dotrad;
	r.max.y = ctr.y + Dotrad + 1;
	return r;
}

void
unselect(const transform* tr)
{
	transform tra;
	if(sel_bkg == 0)
...
/* Text at top right is written first and this low-level routine clobbers it if
   the new top-left text would overwrite it.  However, users of this routine
   should
   try to keep the new text short enough to avoid this.
*/
void
show_mytext(char* msg)
{
...
say_where(const transform* tr)
{
	...
	if(nmax >= 100)
		nmax = 100 - 1;
	n = sprintf(buf, "(%.14g,%.14g) at t=%.14g", rnd(cur_sel.p.x, xtol),
	            rnd(cur_sel.p.y, ytol), rnd(cur_sel.t, t_tol(xtol, ytol)));
...
void reselect(const transform* tr) /* uselect(); set cur_sel; call this */
{
	...
	if(fabs(p2.x) + fabs(p2.y) > 1e8 ||
	   (pt2.x = p2.x, pt2.y = p2.y, is_off_screen(pt2))) {
		cur_sel.t = -1;
		return;
	}
	pt3.x = pt2.x - Dotrad;
...
/* Show the given prompt and read a line of user input.  The text appears at the
   top left.  If it runs into the top right text, we stop echoing but let the
   user
...
*/
void
draw_fpts(const fpoint* p0, double n1, const transform* tr, int thick,
          Image* clr)
{
	int n = (int)n1;
	const fpoint* p = p0 + n;
...
void
draw_1fpoly(const fpolygon* fp, const transform* tr, Image* clr,
            const frectangle* udisp, double slant)
{
	fpoint* p0 = fp->p, * pn = fp->p + fp->n;
	double l1, l2;
	if(p0 == pn && fcontains(udisp, *p0)) {
		draw_fpts(p0, 0, tr, fp->c.thick, clr);
		return;
	}
...
double
get_clip_data(const fpolygons* u, frectangle* r)
{
...
void
draw_fpoly(const fpolygon* fp, const transform* tr, Image* clr)
{
	frectangle r;
...
Image*
palette_color(Point pt, int dy, int n)
...
void
all_set_clr(fpolygons* fps, Image* clr)
{
	fpolygon* p;
...
void
all_set_scheme(fpolygons* fps, int scheme)
{
	fpolygon* p;
	for(p = fps->p; p != 0; p = p->link)
...
void
do_recolor(int but, Mouse* m, int alluniv)
{
...
void
prepare_mv(const fpolygon* fp)
{
	Rectangle r = screen->r;
	Image* scr0;
	int dt = 1 + fp->c.thick;
	r.min.x += lft_border - dt;
	r.min.y += top_border - dt;
	r.max.x -= rt_border - dt;
	r.max.y -= bot_border - dt;
	if(mv_bkgd != 0 && mv_bkgd->repl == 0)
		freeimage(mv_bkgd);
...
void
move_fp(fpolygon* fp, double dx, double dy)
{
...
void
rotate_fp(fpolygon* fp, fpoint o, double theta)
{
...
double
dir_angle(const Point* pt, const transform* tr)
{
	fpoint p;
	double dy, dx;
...
init_e_menu(void)
{
	...
	if(unact != 0)
		...
		case Erecolor:
			...
			break;
		case Ethick:
			u = (unact->fp->c.thick == 0) ? "unthin" : "unthicken";
			break;
		case Edelete:
			u = "undelete";
			break;
...
e_action*
save_act(e_action* a0, e_index typ)
...
/* This would be trivial except it's nice to preserve the selection in order to
   make
   it easy to undo a series of moves.  (There's no do_unrotate() because it's
   harder
   and less important to preserve the selection in that case.)
*/
void
do_unmove(e_action* a)
{
	double tsav = cur_sel.t;
	unselect(0);
...
usage(void)
{
	...
	fprintf(stderr,
	        ...
	        "\n"
	        "Read a polygonal line graph in an ASCII format (one x y pair "
	        "per line, delimited\n"
	        "by spaces with a label after each polyline), and view it "
	        "interactively.  Use\n"
	        "standard input if no infile is specified.\n"
	        "Option -l specifies a file in which to log the coordinates of "
	        "each point selected.\n"
	        "(Clicking a point with button one selects it and displays its "
	        "coordinates and\n"
	        "the label of its polylone.)  Option -m allows polylines to be "
	        "moved and rotated.\n"
