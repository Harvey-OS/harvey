...
is_off_screen(Point p)
{
	...
	return p.x - r->min.x < lft_border || r->max.x - p.x < rt_border || p.y - r->min.y <= top_border || r->max.y - p.y <= bot_border;
}
...
*/
thick_color *
parse_color_chars(const char *c0, const char *fin,
		  thick_color *buf)
{
...
*/
char *
nam_with_thclr(char *nam, const thick_color *tc, char *buf,
	       int bufn)
{
	thick_color c0;
	int clr0i;
...
in_length(const fpoint *p0, const fpoint *pn, frectangle r, double slope)
{
	...
	do
		if(++p > pn)
			return pn - p0;
	while(r.min.x <= (px = p->x) && px <= r.max.x && r.min.y <= (py = p->y - slope * px) && py <= r.max.y);
...
*/
double
closest_time(const fpoint *p0, const fpoint *ctr, double slant,
