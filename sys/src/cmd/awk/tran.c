...
void syminit(void) /* initialize symbol table with builtin vars */
{
	...
	nullloc = setsymtab("$zero&null", "", 0.0, NUM | STR | CON | DONTFREE,
	                    symtab);
	...
	CONVFMT =
	    &setsymtab("CONVFMT", "%.6g", 0.0, STR | DONTFREE, symtab)->sval;
	FILENAME =
	    &setsymtab("FILENAME", "", 0.0, STR | DONTFREE, symtab)->sval;
	...
	FNR = &fnrloc->fval;
	SUBSEP =
	    &setsymtab("SUBSEP", "\034", 0.0, STR | DONTFREE, symtab)->sval;
...
Cell*
setsymtab(char* n, char* s, Awkfloat f, unsigned t, Array* tp)
{
	int h;
	Cell* p;
...
funnyvar(Cell* vp, char* rw)
{
	...
	WARNING("funny variable %p: n=%s s=\"%s\" f=%g t=%o", vp, vp->nval,
