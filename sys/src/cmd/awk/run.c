...
{                              /* a[0] = BEGIN, a[1] = body, a[2] = END */
	...
	if(a[0]) { /* BEGIN */
		...
		if(isexit(x))
			return (True);
		if(isjump(x))
			FATAL("illegal break, continue, next or nextfile from "
			      "BEGIN");
		tempfree(x);
	}
	if(a[1] || a[2])
...
{
	static Cell newcopycell = {OCELL, CCOPY, 0,
	                           "",    0.0,   NUM | STR | DONTFREE};
	int i, ncall, ndef;
	Node* x;
	...
	if(frame == NULL) {
		fp = frame =
		    (struct Frame*)calloc(nframe += 100, sizeof(struct Frame));
		if(frame == NULL)
	...
	dprintf(("calling %s, %d args (%d in defn), fp=%d\n", s, ncall, ndef,
	         (int)(fp - frame)));
	if(ncall > ndef)
		WARNING("function %s called with %d args, uses only %d", s,
		        ncall, ndef);
	if(ncall + ndef > NARGS)
		FATAL("function %s has %d arguments, limit %d", s, ncall + ndef,
		      NARGS);
...
Cell* copycell(Cell* x) /* make a copy of a cell in a temp */
{
	...
	y->tval =
	    x->tval &
	    ~(CON | FLD | REC | DONTFREE); /* copy is not constant or field */
	                                   /* is DONTFREE right? */
	return y;
}
...
Cell* arg(Node** a, int n) /* nth argument of a function */
{
	...
	if(n + 1 > fp->nargs)
		FATAL("argument #%d of function %s was not supplied", n + 1,
		      fp->fcncell->nval);
	return fp->args[n];
}
...
Cell* array(Node** a, int n) /* a[0] is symtab, a[1] is list of subscripts */
{
	...
	for(np = a[1]; np; np = np->nnext) {
		...
		s = getsval(y);
		if(!adjbuf(&buf, &bufsz, strlen(buf) + strlen(s) + nsub + 1,
		           recsize, 0, 0))
...
Cell* awkdelete(Node** a,
                int n) /* a[0] is symtab, a[1] is list of subscripts */
{
	Cell* x, *y;
	Node* np;
	char* s;
	...
	if(a[1] == 0) { /* delete the elements, not the table */
		...
		for(np = a[1]; np; np = np->nnext) {
			...
			s = getsval(y);
			if(!adjbuf(&buf, &bufsz,
			           strlen(buf) + strlen(s) + nsub + 1, recsize,
			           0, 0))
...
int format(char** pbuf, int* pbufsize, char* s,
           Node* a) /* printf-like conversions */
{
	char* fmt;
	char* p, *t, *os;
	Cell* x;
	...
	while(*s) {
		...
		/* have to be real careful in case this is a huge number, eg,
		 * %100000d */
		fmtwd = atoi(s + 1);
		if(fmtwd < 0)
			fmtwd = -fmtwd;
		...
		for(t = fmt; (*t++ = *s) != '\0'; s++) {
			if(!adjbuf(&fmt, &fmtsz, MAXNUMSIZE + 1 + t - fmt,
			           recsize, &t, 0))
				FATAL("format item %.30s... ran format() out "
				      "of memory",
				      os);
			...
			if(*s == '*') {
				...
				if(fmtwd < 0)
					fmtwd = -fmtwd;
				adjbuf(&buf, &bufsize, fmtwd + 1 + p - buf,
				       recsize, &p, "format");
				t = fmt + strlen(fmt);
				tempfree(x);
			}
		...
		*t = '\0';
		...
		case 0:
			...
			if(fmtwd > n)
				n = fmtwd;
			adjbuf(&buf, &bufsize, 1 + strlen(p) + n + p - buf,
			       recsize, &p, "format");
			p += strlen(p);
			sprintf(p, "%s", t);
			break;
		case 1:
			...
			break;
		case 2:
			sprintf(p, fmt, (int32_t)getfval(x));
			break;
		case 3:
		...
		case 4:
			...
			if(!adjbuf(&buf, &bufsize, 1 + n + p - buf, recsize, &p,
			           0))
				FATAL("huge string/format (%d chars) in printf "
				      "%.30s... ran format() out of memory",
				      n, t);
			sprintf(p, fmt, t);
			break;
		case 5:
...
double ipow(double x,
            int n) /* x**n.  ought to be done by pow, but isn't always */
{
	double v;
...
Cell* split(Node** a, int nnn) /* split(a[0], a[1], a[2]); a[3] is type */
{
	   ...
	   arg3type == REGEXPR) { /* reg expr */
		...
		if(nematch(p, s, t)) {
			do {
				...
				*patbeg = '\0';
				if(is_number(t))
					setsymtab(num, t, atof(t), STR | NUM,
					          (Array*)ap->sval);
				else
					setsymtab(num, t, 0.0, STR,
					          (Array*)ap->sval);
				*patbeg = temp;
				t = patbeg + patlen;
				if(t[-1] == 0 || *t == 0) {
	...
	spdone:
		...
		for(n = 0; *s != 0; s += nb) {
			...
			buf[nb] = '\0';
			if(isdigit(buf[0]))
				setsymtab(num, buf, atof(buf), STR | NUM,
				          (Array*)ap->sval);
			else
...
Cell* bltin(Node** a,
            int n) /* builtin functions. a[0] is type, a[1] is arg list */
{
	Cell* x, *y;
	Awkfloat u;
	int t;
	...
	if(pmatch(p, t, t)) {
		...
		while(*sptr != 0) {
			...
			if(*sptr == '\\') {
				...
				sptr++;
				adjbuf(&buf, &bufsz, 1 + patlen + pb - buf,
				       recsize, &pb, "sub");
		...
		*pb = '\0';
		...
		if((patlen == 0 && *patbeg) || (patlen && *(sptr - 1))) {
			adjbuf(&buf, &bufsz, 1 + strlen(sptr) + pb - buf, 0,
			       &pb, "sub");
			while((*pb++ = *sptr++) != 0)
				;
		}
		...
		do {
			if(patlen == 0 &&
			   *patbeg != 0) {       /* matched empty string */
				...
				*pb++ = *c++;
				if(pb >
				   buf + bufsz) /* BUG: not sure of this test */
				...
				sptr = c;
				adjbuf(&buf, &bufsz,
				       1 + (patbeg - sptr) + pb - buf, recsize,
				       &pb, "gsub");
				while(sptr < patbeg)
					*pb++ = *sptr++;
				sptr = rptr;
				while(*sptr != 0) {
					adjbuf(&buf, &bufsz, 5 + pb - buf,
					       recsize, &pb, "gsub");
					if(*sptr == '\\') {
						backsub(&pb, &sptr);
					} else if(*sptr == '&') {
