...
double GAMMA =
    1.0; /* theory tells me this should be 2.2, but 1.0 sure looks better */
enum { Left = 0,
       Right,
       Top,
       Bottom,
...
geometry(Rectangle r)
{
	...
	for(i = 0; i < 9; i++)
		fr[i] = rectaddpt(Rect(0, 0, 48, 48),
		                  Pt(rramp.min.x + 48 + 56 * (i % 3),
		                     rramp.max.y + 6 + 56 * (i / 3)));
...
y2gamma(int y)
{
	...
	g = (double)y / 128.0;
	return 0.5 +
	       g * g; /* gamma from 0.5 to 4.5, with 1.0 near the middle */
}
...
drawscreen(int clear)
{
	...
	border(screen, rectaddpt(state.selr, subpt(rbig.min, orig->r.min)), -2,
	       red, ZP);
	if(clear) {
		drawface(-1);
...
{
	border(screen, rectaddpt(old, subpt(rbig.min, orig->r.min)), -2, orig,
	       old.min);
	border(screen, rectaddpt(new, subpt(rbig.min, orig->r.min)), -2, red,
	       ZP);
}
...
{
	double black, center, delta, *k, shrink, sum, *tmp[48], *tt, w, white,
	    x;
	...
	if(small->chan != GREY8) {
		...
		for(i = 0; i < 48; i++) {
			...
			for(j = 0; j < 48; j++) {
				...
				error = sd - sv;
				if(ii + 49 <
				   48 * 48) { /* one test is enough, really */
					sdata[ii + 1] =
					    sdata[ii + 1] + ((3 * error) >> 4);
					sdata[ii + 48] =
					    sdata[ii + 48] + ((3 * error) >> 4);
					sdata[ii + 49] =
					    sdata[ii + 49] + ((3 * error) >> 3);
				}

				/* produce correct color map value by copying
				 * bits */
				switch(dp) {
				case 1:
					sv |= sv >> 1;
		...
		for(i = 0; i < nelem(bdata); i++)
			bdata[i] = sdata[i];
		if(loadimage(tmp8, tmp8->r, bdata, sizeof bdata) !=
		   sizeof bdata)
			sysfatal("loadimage: %r");
		...
		for(i = 0; i < nelem(bdata); i++)
			bdata[i] = sdata[i];
		if(loadimage(small, small->r, bdata, sizeof bdata) !=
		   sizeof bdata)
			sysfatal("loadimage: %r");
	}
...
setgtab(State* s)
{
	...
	for(i = 0; i <= 1000; i++)
		s->gtab[i] =
		    val2cmap[(int)(255.0 * pow((i / 1000.0), 1.0 / s->gamma))];
}
...
writeface(char* outfile, Image* image)
{
	...
	case GREY1:
		if(unloadimage(image, image->r, data, 48 * 48 / 8) !=
		   48 * 48 / 8)
	...
	case GREY2:
		if(unloadimage(image, image->r, data, 48 * 48 / 4) !=
		   48 * 48 / 4)
			sysfatal("unloadimage: %r");
		for(y = 0; y < 48; y++) {
			for(i = 0; i < 3; i++)
...
move(Rectangle r, Rectangle picr, Point d, int k, Rectangle* rp)
{
	...
	case RTop:
		...
		if(i < 0) {
			/*
			 * should really check i/2, but this is safe and
			 * feedback
...
Cursor boxcursor = {
    ...
    {
     0x00, 0x00, 0x7F, 0xFE, 0x7F, 0xFE, 0x7F, 0xFE, 0x70, 0x0E, 0x70, 0x0E,
...
Cursor* corners[10] = {
    &tl, &t, &tr, &l, &boxcursor, &r, &bl, &b, &br, nil, /* default arrow */
};

char* item[] = {"Reset", "Depth", "Undo", "Write", "Exit", nil};

Menu menu = {item, nil, 2};

/*BUG make less flashy */
void
moveface(Image* back, Point lastp, Image* face, Point p, Point d)
{
	draw(screen, rectaddpt(back->r, subpt(lastp, d)), back, nil,
	     back->r.min);
	draw(back, back->r, screen, nil, addpt(back->r.min, subpt(p, d)));
	border(screen, rectaddpt(face->r, subpt(p, d)), -1, display->black, ZP);
	draw(screen, rectaddpt(face->r, subpt(p, d)), face, nil, face->r.min);
}
...
dragface(Mouse* m, Image* im, Point d, int x)
{
	...
	if(back == nil) {
		back = allocimage(display, Rect(-1, -1, 49, 49),
		                  display->image->chan, 0, DNofill);
		if(back == nil)
...
main(int argc, char** argv)
{
	...
	red = allocimage(display, Rect(0, 0, 1, 1), display->image->chan, 1,
	                 DRed);
	green = allocimage(display, Rect(0, 0, 1, 1), display->image->chan, 1,
	                   DGreen);
	blue = allocimage(display, Rect(0, 0, 1, 1), display->image->chan, 1,
	                  DBlue);
	if(bkgd == nil || small == nil || tmp8 == nil || red == nil ||
	   green == nil || blue == nil)
		sysfatal("allocimage: %r");
	...
	for(;;) {
		...
		if(m.buttons & 4) {
			...
			case 0: /* Reset */
				mark();
				initstate();
				small = allocimage(display, Rect(0, 0, 48, 48),
				                   CHAN1(CGrey, state.depth), 0,
				                   DWhite);
				if(small == nil)
					sysfatal("allocimage: %r");
				process(rdata, orig->r, state.selr, small);
		...
		if(ptinrect(m.xy, rbig)) {
			rlist(
			    rectaddpt(state.selr, subpt(rbig.min, orig->r.min)),
			    rbig9);
			for(i = 0; i < 9; i++)
			...
			if(m.buttons & 1) {
				...
				lastp = m.xy;
				while(m = emouse(), m.buttons & 1) {
					if(move(state.selr, orig->r,
					        subpt(m.xy, lastp), i,
					        &nselr)) {
						moveframe(state.selr, nselr);
						state.selr = nselr;
						lastp = m.xy;
						process(rdata, orig->r,
						        state.selr, small);
						drawface(-1);
					}
				}
		...
		if(i < nelem(face) && face[i] != nil) {
			if(m.buttons != 1)
				continue;
			n = dragface(&m, face[i]->small,
			             subpt(m.xy, rface[i].min), i);
			if(n == i)
				continue;
			saveface(face[i], n);
			continue;
