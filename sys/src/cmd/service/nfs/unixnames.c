...
Unixidmap *
pair2idmap(char *server, char *client)
{
	Resub match;
	Unixscmap *m, *mp;
	Unixidmap *r;
	...
	for(mp=0,m=scmap; m; mp=m,m=m->next){
		if(m->server[0] != server[0] || m->client[0] != client[0])
			continue;
		if(strcmp(m->server, server) || strcmp(m->client, client))
			continue;
		if(mp){
			mp->next = m->next;
			m->next = scmap;
	...
	for(r=idhead; r; r=r->next){
		...
		if(match.sp != server || match.ep <= match.sp || *match.ep)
			continue;
		match.sp = match.ep = 0;
		if(regexec(r->cexp, client, &match, 1) == 0)
			continue;
		if(match.sp != client || match.ep <= match.sp || *match.ep)
			continue;
		m = malloc(sizeof(Unixscmap));
		m->next = scmap;
		scmap = m;
		m->server = strstore(server);
		m->client = strstore(client);
		m->map = r;
		break;
	}
	return r;
...
readunixidmaps(char *file)
{
	...
	Biobuf *in;
	Unixidmap *m;
	int i, arc; char *arv[16], buf[2*NAMELEN];
	char *l;
	long savalarm;
