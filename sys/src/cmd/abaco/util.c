...
istextfield(Item* i)
{
	...
	if(ff->ftype == Ftext || ff->ftype == Ftextarea ||
	   ff->ftype == Fpassword)
		return TRUE;
...
frdims(Dimen* d, int n, int t, int** ret)
{
	...
	if(tt < t) {
		...
		if(totpixrel < t)
			spcnt = (double)((t - totpixrel) * 100) /
			        (double)(t * totpcnt);
		else {
			trest = t - totrel * minrelu;
			if(trest > 0)
				spcnt = (double)trest /
				        (double)(totpix + (t * totpcnt / 100));
			else {
...
rect3d(Image* im, Rectangle r, int i, Image** c, Point sp)
{
	...
	draw(im, Rect(r.min.x + i, r.min.y + i, r.max.x - i, r.max.y - i), c[2],
	     nil, sp);
	p[0] = r.min;
...
getcolor(int rgb)
{
	...
	c->i = eallocimage(display, Rect(0, 0, 1, 1), screen->chan, 1,
	                   (rgb << 8) | 0xFF);
	c->rgb = rgb;
	c->next = colortab[h];
	colortab[h] = c;
...
#include "tcs.h"
    ...
    "euc_jp", "jis", "euc_kr", "euc-k", "windows-874", "tis", nil, nil,
};
...
tcs(char* cs, char* s, int32_t* np)
{
	...
	if(cistrcmp(tcstab[i].tcs, "8859-1") == 0 ||
	   cistrcmp(tcstab[i].tcs, "ascii") == 0) {
	latin1:
		n = 0;
...
fixtext1(Item** list)
{
	...
	for(it = *list; it != nil; it = prev->next) {
		...
		if(*s) {
			...
			ntext->state =
			    text->state &
			    ~(IFbrk | IFbrksp | IFnobrk | IFcleft | IFcright);
			ntext->tag = text->tag;
			ntext->fnt = text->fnt;
			ntext->fg = text->fg;
			ntext->ul = text->ul;
...
makenewwindow(Page* p)
{
	...
	for(i = 1; i < c->nw; i++) {
		...
		if(Dy(w->page.all) >= Dy(bigw->page.all))
			bigw = w;
		if(w->page.lay == nil &&
		   Dy(w->page.all) >= Dy(emptyw->page.all))
			emptyw = w;
	}
	emptyp = &emptyw->page;
	el = Dy(emptyp->all);
