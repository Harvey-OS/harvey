...
execute(Text* t, uint aq0, uint aq1, Text*)
{
	...
	if(q1 == q0) { /* expand to find word (actually file name) */
		...
		if(t->q1 > t->q0 && t->q0 <= q0 && q0 <= t->q1) {
			...
			while(q1 < t->rs.nr && isexecc(c = t->rs.r[q1]) &&
			      c != ':')
				q1++;
			while(q0 > 0 && isexecc(c = t->rs.r[q0 - 1]) &&
			      c != ':')
				q0--;
			if(q1 == q0)
				return;
...
get(Text* t, Text*, int, int, Rune*, int)
{
	...
	dohist = FALSE;
	if(w->page.url == nil ||
	   runestreq(w->page.url->act, w->url.rs) == FALSE)
		dohist = TRUE;
...
lookpage(Rune* s, int n)
{
	...
	for(j = 0; j < row.ncol; j++) {
		...
		for(i = 0; i < c->nw; i++) {
			...
			p = &w->page;
			if(p->url &&
			   runeeq(p->url->src.r, p->url->src.nr, s, n))
				if(w->col != nil)
					return w;
		}
	}
...
openpage(Page* p, Runestr* rs)
{
	...
	if(w) {
		p = &w->page;
		if(!p->col->safe &&
		   Dy(p->r) == 0) /* window is obscured by full-column window */
...
plumblook(Plumbmsg* m)
{
	...
	if(m->ndata >= BUFSIZE) {
		fprint(2, "insanely long file name (%d bytes) in plumb message "
		          "(%.32s...)\n",
		       m->ndata, m->data);
		return;
	}
...
void new(Text* et, Text*, int, int, Rune*, int)
{
	if(et->col != nil)
...
google(Text*, Text*, int, int, Rune* arg, int narg)
{
	...
	s = ucvt(arg);
	rs.r = runesmprint("http://www.google.com/search?hl=en&ie=UTF-8&q=%.*S",
	                   narg, s);
