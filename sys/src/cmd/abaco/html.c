...
sizetextfield(Lay*, Iformfield* i)
{
	...
	f = getfont(WFont);
	i->width =
	    runestringnwidth(f, L"0", 1) * w + 2 * (Space + Border + Margin);
...
drawtext(Box* b, Page* p, Image* im)
{
	...
	if(istextsel(p, b->r, &q0, &q1, i->s, f)) {
		...
		if(q1 > 0)
			r1.max.x =
			    r1.min.x + runestringnwidth(f, i->s + q0, q1 - q0);
...
drawcheck(Image* im, Rectangle r, Formfield* f)
{
	...
	if(f->flags & FFchecked)
		colarray(c, getcolor(Dark), getcolor(Light), getcolor(Red),
		         TRUE);
	else
		colarray(c, getcolor(Dark), getcolor(Light), getcolor(Back),
		         FALSE);
...
drawbutton(Image* im, Rectangle r, Formfield* f, int checked)
{
	...
	runestringbg(im, r.min, display->black, ZP, getfont(WFont), f->value,
	             c[2], ZP);
}
...
submit(Page* p, Formfield* formfield, int subfl)
{
	...
	for(f = form->fields; f != nil; f = f->next) {
		...
		if(f->ftype == Fsubmit && (f != formfield || !subfl))
			continue;
		if(f->value == nil || f->name == nil ||
		   runestrcmp(f->name, L"_no_name_submit_") == 0)
			continue;
...
setradios(Formfield* formfield)
{
	...
	for(f = formfield->form->fields; f != nil; f = f->next)
		if(f->ftype == Fradio && f != formfield &&
		   runestrcmp(f->name, formfield->name) == 0)
			f->flags &= ~FFchecked;
}
...
selectmouse(Box* b, Page* p, int but)
{
	...
	if(i >= 0) {
		...
		drawselect(p->b, rectaddpt(rectsubpt(b->r, p->pos), p->r.min),
		           (Iformfield*)b->i);
		if(f->value != nil)
			free(f->value);
...
keyform(Box* b, Page* p, Rune r)
{
	...
	if(t->b != p->b)
		drawtextfield(p->b,
		              rectaddpt(rectsubpt(b->r, p->pos), p->r.min),
		              (Iformfield*)b->i);
	texttype(t, r);
	if(f->value)
		free(f->value);
...
layitem(Lay* lay, Item* i)
{
	...
	if(i->state & IFbrk || i->state & IFbrksp)
		newline(lay, i->state);
	else if(lay->lastline->r.max.x + i->width > lay->xwall &&
	        forceitem(i) == FALSE)
		newline(lay, i->state);

	l = lay->lastline;
	r = Rect(l->r.max.x, l->r.min.y, l->r.max.x + i->width,
	         l->r.min.y + i->height);
	l->r.max.x = r.max.x;
	...
	for(l = lay->lines; l != nil; l = l->next)
		for(b = l->boxes; b != nil; b = b->next) {
			...
			if(b->i->tag == Itexttag) {
				...
				for(c = ((Itable*)b->i)->table->cells; c != nil;
				    c = c->next)
					if(c->lay)
						laysnarf(p, c->lay, rs);
		}
