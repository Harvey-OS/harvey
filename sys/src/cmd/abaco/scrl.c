...
scrlresize(void)
{
	freeimage(vscrtmp);
	freeimage(hscrtmp);
	vscrtmp = eallocimage(display, Rect(0, 0, 32, screen->r.max.y),
	                      screen->chan, 0, DNofill);
	hscrtmp = eallocimage(display, Rect(0, 0, screen->r.max.x, 32),
	                      screen->chan, 0, DNofill);
}
...
textscroll(Text* t, int but)
{
	...
	do {
		...
		if(!eqpt(mouse->xy, Pt(x, my))) {
			...
			readmouse(
			    mousectl); /* absorb event generated by moveto() */
		}
		if(but == 2) {
			y = my;
			p0 = (int64_t)t->rs.nr * (y - s.min.y) / h;
			if(p0 >= t->q1)
		...
		if(but == 1)
			p0 = textbacknl(t, t->org,
			                (my - s.min.y) / t->font->height);
		else
