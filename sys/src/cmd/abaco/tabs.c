...
settable(Table* t)
{
	...
	for(c = t->cells; c != nil; c = c->next) {
		...
		if(dimenkind(c->wspec) == Dnone) {
			lay = layitems(c->content, Rect(0, 0, Tablemax, 0),
			               FALSE);
			c->maxw = Dx(lay->r);
			layfree(lay);
		}
...
tablewidth(Table* t, int tw, int sep)
{
	...
	for(c = t->cells; c != nil; c = c->next) {
		if(dimenkind(c->wspec) != Dnone) {
			d = c->minw;
			c->minw = c->maxw =
			    max(dimwidth(c->wspec, tw), c->minw);
			c->minw = d;
		}
		if(c->colspan != 1)
			continue;
	...
	if(tmax <= tw) {
		d = 0;
		if(tw > tmax && dimenkind(t->width) != Dnone &&
		   t->availw != Tablemax)
			d = (tw - tmax) / t->ncol;
		for(i = 0; i < t->ncol; i++) {
...
fixrows(Table* t, int sep)
{
	...
	for(c = t->cells; c != nil; c = c->next) {
		...
		lay = layitems(c->content, Rect(0, 0, cellwidth(t, c, sep), 0),
		               FALSE);
...
tableheight(Table* t, int sep)
{
	...
	for(i = 0; i < t->nrow; i++) {
		...
		for(c = t->rows[i].cells; c != nil; c = c->nextinrow) {
			if(c->rowspan != 1)
				continue;
			lay = layitems(c->content,
			               Rect(0, 0, cellwidth(t, c, sep), 0),
			               FALSE);
