...
run(void)
{
	...
	do {
		...
		reg.ip = &itab[_armclass(((reg.ir>>25)&0x7), reg.ir)];
...
undef(ulong inst)
{
	Bprint(bioout, "Undefined Instruction Trap IR %.8lux Op %d\n",
		inst, _armclass(((reg.ir>>25)&0x7), reg.ir));
	longjmp(errjmp, 0);
}
...
Imem(ulong inst)
{
	...
	if(trace) {
		if (lbit) {
			if (!bit25)
				itrace("MOV%c%s%s\t#%x(R%d),R%d #%lux=#%x",
					(bbit ? 'B' : 'W'), (pbit ? "" : ".P"), cond[reg.cond], 
					off, rn, rd,
					address, value);
			else
				itrace("MOV%c%s%s\t(R%d%s%d)(R%d),R%d  #%lux=#%x",
					(bbit ? 'B' : 'W'), (pbit ? "" : ".P"), cond[reg.cond], 
					rm, shtype[st], sc, rn, rd,
					address, value);
		} else {
			if (!bit25)
				itrace("MOV%c%s%s\tR%d,#%x(R%d) #%lux=#%x",
					(bbit ? 'B' : 'W'), (pbit ? "" : ".P"), cond[reg.cond], 
					rd, off, rn,
					address, value);
			else
				itrace("MOV%c%s%s\tR%d,(R%d%s%d)(R%d) #%lux=#%x",
					(bbit ? 'B' : 'W'), (pbit ? "" : ".P"), cond[reg.cond], 
