...
Inst itab[] = {{Idp0, "AND", Iarith}, /* 00 - r,r,r */
               {Idp0, "EOR", Iarith}, /* 01 */
               {Idp0, "SUB", Iarith}, /* 02 */
               {Idp0, "RSB", Iarith}, /* 03 */
               {Idp0, "ADD", Iarith}, /* 04 */
...
runcmp(void)
{
	switch(reg.cond) {
	case 0x0: /* eq */
		return (reg.cc1 == reg.cc2);
	case 0x1: /* ne */
		return (reg.cc1 != reg.cc2);
	case 0x2: /* hs */
		return ((uint32_t)reg.cc1 >= (uint32_t)reg.cc2);
	case 0x3: /* lo */
		return ((uint32_t)reg.cc1 < (uint32_t)reg.cc2);
	case 0x4: /* mi */
		return (reg.cc1 - reg.cc2 < 0);
	case 0x5: /* pl */
		return (reg.cc1 - reg.cc2 >= 0);
	case 0x8: /* hi */
		return ((uint32_t)reg.cc1 > (uint32_t)reg.cc2);
...
run(void)
{
	...
	do {
		...
		if(execute) {
			...
			if(trace)
				itrace("%s%s	IGNORED", reg.ip->name,
				       cond[reg.cond]);
		}
		reg.r[REGPC] += 4;
		if(bplist)
...
{
	Bprint(bioout,
	       "undefined instruction trap pc #%lux inst %.8lux class %d\n",
...
dpex(long inst, long o1, long o2, int rd)
{
	...
	case 4: /* add */
		...
		reg.r[rd] = o1 + o2;
		if(inst & Sbit) {
			if(((uvlong)(ulong)o1 + (uvlong)(ulong)o2) &
			   (1LL << 32))
				reg.cbit = 1;
			else
				reg.cbit = 0;
...
Idp0(uint32_t inst)
{
	...
	dpex(inst, o1, o2, rd);
	if(trace)
		itrace("%s%s\tR%d,R%d,R%d =#%x", reg.ip->name, cond[reg.cond],
		       rm, rn, rd, reg.r[rd]);
	if(rd == REGPC)
		reg.r[rd] -= 4;
}
...
Idp1(uint32_t inst)
{
	...
	dpex(inst, o1, o2, rd);
	if(trace)
		itrace("%s%s\tR%d%s%d,R%d,R%d =#%x", reg.ip->name,
		       cond[reg.cond], rm, shtype[st], sc, rn, rd, reg.r[rd]);
	if(rd == REGPC)
		reg.r[rd] -= 4;
}
...
Idp2(uint32_t inst)
{
	...
	dpex(inst, o1, o2, rd);
	if(trace)
		itrace("%s%s\tR%d%sR%d=%d,R%d,R%d =#%x", reg.ip->name,
		       cond[reg.cond], rm, shtype[st], rs, o3, rn, rd,
		       reg.r[rd]);
	if(rd == REGPC)
		reg.r[rd] -= 4;
}
...
Idp3(uint32_t inst)
{
	...
	dpex(inst, o1, o2, rd);
	if(trace)
		itrace("%s%s\t#%x,R%d,R%d =#%x", reg.ip->name, cond[reg.cond],
		       o2, rn, rd, reg.r[rd]);
	if(rd == REGPC)
		reg.r[rd] -= 4;
}
...
Imull(uint32_t inst)
{
	...
	if(inst & (1 << 22)) {
		...
		v = (uint64_t)(uint32_t)reg.r[rm] *
		    (uint64_t)(uint32_t)reg.r[rs];
		if(inst & (1 << 21))
			v += (uint32_t)reg.r[rn];
	}
	reg.r[rd] = v >> 32;
	...
	if(trace)
		itrace("%s%s\tR%d,R%d,(R%d,R%d) =#%llx", reg.ip->name,
		       cond[reg.cond], rs, rm, rn, rd, v);
}
...
Imula(uint32_t inst)
{
	...
	if(trace)
		itrace("%s%s\tR%d,R%d,R%d,R%d =#%x", reg.ip->name,
		       cond[reg.cond], rs, rm, rn, rd, reg.r[rd]);
}
...
Iswap(uint32_t inst)
{
	...
	if(trace) {
		...
		itrace("SWP%s%s\t#%x(R%d),R%d #%lux=#%x", bw, dotc, rn, rd,
		       address, value);
	}
}
...
Imem1(uint32_t inst)
{
	...
	if(trace) {
		...
		if(lbit) {
			if(!bit25)
				itrace("MOV%s%s%s\t#%x(R%d),R%d #%lux=#%x", bw,
				       dotp, dotc, off, rn, rd, address, value);
			else
				itrace(
				    "MOV%s%s%s\t(R%d%s%d)(R%d),R%d  #%lux=#%x",
				    bw, dotp, dotc, rm, shtype[st], sc, rn, rd,
				    address, value);
		} else {
			if(!bit25)
				itrace("MOV%s%s%s\tR%d,#%x(R%d) #%lux=#%x", bw,
				       dotp, dotc, rd, off, rn, address, value);
			else
				itrace(
				    "MOV%s%s%s\tR%d,(R%d%s%d)(R%d) #%lux=#%x",
				    bw, dotp, dotc, rd, rm, shtype[st], sc, rn,
				    address, value);
		}
	}
}
...
Imem2(uint32_t inst)
{
	...
	if(trace) {
		...
		if(lbit) {
			if(bit22)
				itrace("MOV%s%s%s\t#%x(R%d),R%d #%lux=#%x", hb,
				       dotp, dotc, off, rn, rd, address, value);
			else
				itrace("MOV%s%s%s\t(R%d)(R%d),R%d  #%lux=#%x",
				       hb, dotp, dotc, rm, rn, rd, address,
				       value);
		} else {
			if(bit22)
				itrace("MOV%s%s%s\tR%d,#%x(R%d) #%lux=#%x", hb,
				       dotp, dotc, rd, off, rn, address, value);
			else
				itrace("MOV%s%s%s\tR%d,(R%d)(R%d) #%lux=#%x",
				       hb, dotp, dotc, rd, rm, rn, address,
				       value);
		}
	}
}
...
Ilsm(uint32_t inst)
{
	...
	if(trace) {
		itrace("%s.%c%c\tR%d=%lux%s, <%lux>", (lbit ? "LDM" : "STM"),
		       (ubit ? 'I' : 'D'), (pbit ? 'B' : 'A'), rn, reg.r[rn],
		       (wbit ? "!" : ""), reglist);
	}
}
