...
void text(int t, char* p1) /* convert text string p1 of type t */
{
	...
	if(t == QTEXT) {
		...
		for(psp = p1; (c = textc()) != '\0';) {
			...
			if(csp - cs > CSSIZE)
				ERROR "converted token %.25s... too long",
				    p1 FATAL;
		}
		sadd("\\fP");
		*csp = '\0';
	...
	dprintf(".\t%dtext: S%d <- %s; b=%g,h=%g,lf=%c,rf=%c,ps=%d\n", t, yyval,
	        p, ebase[yyval], eht[yyval], lfont[yyval], rfont[yyval], ps);
	printf(".ds %d \"%s\n", yyval, p);
}
...
trans(int c, char*)
{
	...
	case '\\': /* troff - pass only \(xx without comment */
		...
		if(c == '(' && *psp && *(psp + 1)) {
			...
			fprintf(stderr, "eqn warning: unquoted troff command "
			                "\\%c, file %s:%d\n",
...
void shim(int lc,
          int rc) /* add padding space suitable to left and right classes */
{
