...
bshiftb(int p1, int dir, int p2)
{
	...
	printf(".as %d \\v'%gm'%s%s\\*(%d%s%s\\v'%gm'\n", yyval, REL(shval, ps),
	       DPS(ps, subps), sh1, p2, DPS(subps, ps), sh2, REL(-shval, ps));
	rfont[p1] = 0;
	sfree(p2);
}
...
shift2(int p1, int p2, int p3)
{
	...
	eht[yyval] =
	    h1 + subsh + b2 - b1 + max(0, h3 - (1 - Supshift) * (h1 - b1));
	...
	*/
	...
	printf(".as %d %s\\v'%gm'\\*(%d\\v'%gm'\\h'-\\n(%du'\\\n", p1,
	       DPS(ps, subps), REL(subsh, subps), p2, REL(-subsh, subps), p2);
	printf("\\v'%gm'\\*(%d\\v'%gm'\\h'-\\n(%du+\\n(%du'%s%s\n",
	       REL(supsh, subps), p3, REL(-supsh, subps), p3, treg,
	       DPS(subps, ps), Sub2space);
	if(rfont[p2] == ITAL)
		rfont[yyval] = 0;   /* lie */
