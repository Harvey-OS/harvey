...
main(int argc, char* argv[])
{
	...
	while(argc > 1 && argv[1][0] == '-') {
		...
		case 'd':
			...
			break;
		case 's':
			szstack[0] = gsize = atoi(&argv[1][2]);
			break;
		case 'p':
			deltaps = atoi(&argv[1][2]);
			dps_set = 1;
			break;
		case 'm':
			...
			break;
		case 'f':
			strcpy(ftstack[0].name, &argv[1][2]);
			break;
		case 'e':
			noeqn++;
			break;
...
putout(int p1)
{
	...
	eqnht = eht[p1];
	before = eht[p1] - ebase[p1] -
	         BeforeSub; /* leave room for sub or superscript */
...
{
	printf(".nr %d 0\\w'%s\\*(%d'\n", n1, DPS(gsize, p),
	       n2); /* 0 defends against - width */
}

char*
ABSPS(int dn) /* absolute size dn in printable form \sd or \s(dd (dd >= 40) */
{
...
char* DPS(int f,
          int t) /* delta ps (t-f) in printable form \s+d or \s-d or \s+-(dd */
{
	static char buf[100], * lb = buf;
	char* p;
	int dn;
