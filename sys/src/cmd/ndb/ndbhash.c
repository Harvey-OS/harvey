...
hfopen(Ndb *db, char *attr)
{
	...
	/* warn about going serially through huge files */
	if(d.length > 128*1024){
		print("Missing or out of date hash file %s. Not searching.\n", buf);
		syslog(0, "ndb", "Missing or out of date hash file %s.  Not searching", buf);
		close(Bfildes(&db->b));
		Bterm(&db->b);
		db->mtime = 0;
	}
...
extern int debug;
if(debug)
print("file %s %s=%s ptr %lux type %d\n", db->file, attr, val, s->ptr, s->type);
if(debug)
print("offet %d icount %d\n", db->b.offset, db->b.icount);
}
		if(s->type == Dptr){
