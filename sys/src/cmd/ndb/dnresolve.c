...
enum { Udp,
       ...
       //	Maxretries=	3, /* cname+actual resends: was 32; have pity
       //on user */
       //	Maxwaitms=	1000,	/* wait no longer for a remote dns
       //query */
       //	Minwaitms=	100,	/* willing to wait for a remote dns
       //query */
...
int likely[] = {
        [Ta] 95,   [Taaaa] 10, [Tcname] 15, [Tmx] 60,  [Tns] 90,  [Tnull] 5,
        [Tptr] 35, [Tsoa] 90,  [Tsrv] 60,   [Ttxt] 15, [Tall] 95,
};
          ...
          int recurse, int rooted, int* status)
{
	...
	if(rp == nil) {
		...
		if(type != Tptr && dp->respcode != Rname)
			for(loops = 0; rp == nil && loops < Maxretries;
			    loops++) {
				/* retry cname, then the actual type */
				rp = dnresolve1(name, class, Tcname, req, depth,
				                recurse);
				if(rp == nil)
					break;
				...
				rp = dnresolve1(name, class, type, req, depth,
				                recurse);
			}
           ...
           int recurse)
{
	...
	if(rp)
		if(rp->db) {
			...
			if(rp->auth) {
				noteinmem();
				if(debug)
					dnslog("[%d] dnresolve1 %s %d %d: auth "
					       "rr in db",
	...
	if(type != Tcname) {
		...
		if(rp) {
			if(debug)
				dnslog("[%d] dnresolve1 %s %d %d: rr from "
				       "rrlookup for non-cname",
				       getpid(), name, type, class);
			return nil;
		}
	}
	...
	if(rp) {
		...
		if(debug)
			dnslog("[%d] dnresolve1 %s %d %d: no rr from dblookup; "
			       "crapped out",
			       getpid(), name, type, class);
	}
	return rp;
	...
	if(medium == Udp)
		...
		if(fd <= 0)
			...
			if(readn(fd, ibuf, len) != len) {
				dnslog(
				    "readnet: short read of tcp data from %I",
				    qp->tcpip);
				/* probably a time-out */
				notestats(startns, 1, qp->type);
				len = -1;
          ...
          uint64_t endms)
{
	...
	for(; timems() < endms &&
		...
		if(mp->id != req)
			...
			dnslog("%d: qp->type %d instead of %d: %I", qp->req->id,
			       mp->qd->type, qp->type, srcip);
		else {
	...
	if(timems() >= endms) {
		...
		dnslog("readreply: %s type %s: ns %I read error or eof "
		       "(returned %d): %r",
		       qp->dp->name, rrname(qp->type, tbuf, sizeof tbuf), srcip,
		       len);
		if(medium == Udp)
...
xmitquery(Query* qp, int medium, int depth, uint8_t* obuf, int inns, int len)
{
	...
	if(qp->ndest == 0)
		if(cfg.straddle && cfg.inside) {
			...
			if(n == 0)
				dnslog(
				    "xmitquery: %s: no outside-ns nameservers",
				    qp->dp->name);
		} else
	...
	if(medium == Tcp) {
		...
		mydnsquery(qp, medium, obuf,
		           len); /* sets qp->tcpip from obuf */
		if(debug)
...
static int lckindex[Maxlcks] = {
    ...
    Ta, Tns, Tcname, Tsoa, Tptr, Tmx, Ttxt, Taaaa,
};

static int qtype2lck(int qtype) /* map query type to querylck index */
{
	int i;
...
procansw(Query* qp, DNSmsg* mp, uint8_t* srcip, int depth, Dest* p)
{
	 ...
	 */
	//	lcktype = qtype2lck(qp->type);		/* someday try this
	//again */
...
tcpquery(Query* qp, DNSmsg* mp, int depth, uint8_t* ibuf, uint8_t* obuf,
         int len, uint32_t waitms, int inns, uint16_t req)
{
	int rv = 0;
	uint64_t endms;
        ...
        int inns)
{
	...
	for(ndest = 1; ndest < Maxdest; ndest++) {
		...
		for(replywaits = 0; replywaits < ndest; replywaits++) {
			...
			procsetname(
			    "reading %sside reply from %I: %s %s from %s",
...
udpquery(Query* qp, char* mntpt, int depth, int patient, int inns)
{
	...
	if(fd < 0) {
		dnslog("can't get udpport for %s query of name %s: %r", mntpt,
		       qp->dp->name);
...
netquery(Query* qp, int depth)
{
	...
	if(!cfg.straddle || inname) {
		rv = udpquery(qp, mntpt, depth, Hurry,
	...
	if(rv == Answnone && cfg.inside && !inname) {
		if(triedin)
			dnslog("[%d] netquery: internal nameservers failed for "
			       "%s; trying external",
			       getpid(), dp->name);
	...
	//	if (rv == Answnone)		/* could ask /net.alt/dns
	//directly */
