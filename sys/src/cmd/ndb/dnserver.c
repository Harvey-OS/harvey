...
dnserver(DNSmsg* reqp, DNSmsg* repp, Request* req, uint8_t* srcip, int rcode)
{
	...
	if(rcode) {
		...
		if(rcode >= 0 && rcode < nrname)
			errmsg = rname[rcode];
		dnslog("server: response code 0%o (%s), req from %I", rcode,
		       errmsg, srcip);
	...
	if(myarea != nil) {
		if(repp->qd->type == Tixfr || repp->qd->type == Taxfr) {
			...
			repp->flags =
			    Runimplimented | Fresp | recursionflag | Oquery;
			return;
		}
	} else if(norecursion) {
	...
	if(myarea == nil)
		...
		for(cp = repp->qd->owner->name; cp; cp = walkup(cp)) {
			...
			if(strncmp(nsdp->name, "local#", 6) == 0)
				dnslog("returning %s as nameserver",
				       nsdp->name);
			repp->ns =
			    dblookup(cp, repp->qd->owner->class, Tns, 0, 0);
			if(repp->ns)
				break;
		}
