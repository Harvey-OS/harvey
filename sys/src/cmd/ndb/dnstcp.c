...
main(int argc, char* argv[])
{
	...
	for(;; putactivity(0)) {
		...
		if(debug)
			dnslog("[%d] %d: serve (%s) %d %s %s", getpid(), req.id,
			       caller, reqmsg.id, reqmsg.qd->owner->name,
...
reply(int fd, DNSmsg* rep, Request* req)
{
	...
	if(debug) {
		dnslog("%d: reply (%s) %s %s %ux", req->id, caller,
		       rep->qd->owner->name,
		       rrname(rep->qd->type, tname, sizeof tname), rep->flags);
...
logsend(int id, int subid, uint8_t* addr, char* sname, char* rname, int type)
{
	...
	dnslog("%d.%d: sending to %I/%s %s %s", id, subid, addr, sname, rname,
