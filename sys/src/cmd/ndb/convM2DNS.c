...
{
	snprint(sp->errbuf, sizeof sp->errbuf, "negative len %d: %R", actual,
	        rp);
	sp->err = sp->errbuf;
	return 0;
}
...
mstypehack(Scan* sp, uint16_t type, char* where)
{
	if((uint8_t)type == 0 && (type >> 8) != 0) {
		USED(where);
		//		dnslog("%s: byte-swapped type field in ptr rr from
		//win2k",
		//			where);
		if(sp->rcode == Rok)
			sp->rcode = Rformat;
...
retry:
	...
	if(sp->p - data != len) {
		...
		if(len > sp->p - data) {
			dnslog(
			    "bad %s RR len (%d bytes nominal, %lud actual): %R",
 ...
 *
 *  ideally would note if len == Maxpayload && query was via UDP, for
 *errtoolong.
 */
char*
