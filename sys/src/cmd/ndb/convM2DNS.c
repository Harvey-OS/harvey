...
struct Scan
{
	u8	*base;		/* input buffer */
	u8	*p;		/* current position */
	u8	*ep;		/* byte after the end */
 ...
 */
static int
gbytes(RR *rp, Scan *sp, u8 **p, int n)
{
 ...
 */
static u16
mstypehack(Scan *sp, u16 type, char *where)
{
	if ((u8)type == 0 && (type>>8) != 0) {
		USED(where);
 ...
 */
char*
convM2DNS(u8 *buf, int len, DNSmsg *m, int *codep)
{
	char *err = nil;
	RR *rp = nil;
	Scan scan;
