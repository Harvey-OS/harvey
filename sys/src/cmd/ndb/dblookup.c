...
static int implemented[Tall] = {
        [Ta] 1,    [Taaaa] 1, [Tcname] 1, [Tmx] 1,  [Tns] 1,
        [Tnull] 1, [Tptr] 1,  [Tsoa] 1,   [Tsrv] 1, [Ttxt] 1,
};
...
soarr(Ndbtuple* entry, Ndbtuple* pair)
{
	...
	if(mb)
		if(strchr(mb->val, '.')) {
			...
			snprint(mailbox, sizeof mailbox, "%s.%s", mb->val,
			        ns->val);
	...
	for(; rp; rp = rp->next) {
		...
		if(nsrp)
			if(rp->owner != nsrp->owner)
				if(subsume(rp->owner->name,
				           nsrp->owner->name) &&
				   strcmp(nsrp->owner->name, localservers) !=
				       0) {
					dnslog(
					    "delegation loop %R -> %R from %I",
					    nsrp, rp, addr);
					return 1;
				}
			...
			if(sp) {
				*sp = '\0';
				if(strcmp(addr, line) == 0) {
					dnslog("rejecting my ip %s as local "
					       "dns server",
					       addr);
					return 1;
				}
...
addlocaldnsserver(DN* dp, int class, char* ipaddr, int i)
{
	...
	if(n < nelem(locdns))
		if(locdns[n] == nil || ++n < nelem(locdns))
			locdns[n] =
			    strdup(ipaddr); /* remember 1st few local ns */
	qunlock(&locdnslck);
...
createv6ptrs(void)
{
	...
	for(s = owned; s; s = s->next) {
		 ...
		 * in this network and create ptrs.
		 */
		dnptr(net, mask, dom, Taaaa, V6maxrevdomdepth - pfxnibs,
		      Ptrttl);
	}
}
