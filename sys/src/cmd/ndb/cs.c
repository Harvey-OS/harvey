...
int ipv6lookups = 1;
jmp_buf
    masterjmp; /* return through here after a slave process has been created */
...
rwrite(Job* job, Mfile* mf)
{
	...
	if(paranoia)
		syslog(0, paranoiafile, "write %s by %s", job->request.data,
		       mf->user);
...
ipid(void)
{
	...
	if(mysysname == 0) {
		...
		if(mysysname == 0) {
			...
			if(t == nil) {
				for(f = 0; f < 3; f++) {
					snprint(buf, sizeof buf, "%s/ether%d",
					        mntpt, f);
					if(myetheraddr(addr, buf) >= 0) {
						snprint(eaddr, sizeof(eaddr),
						        "%E", addr);
						free(ndbgetvalue(db, &s,
						                 "ether", eaddr,
						                 "sys", &t));
						if(t != nil)
							break;
...
lookup(Mfile* mf)
{
	...
	if(strcmp(mf->net, "net") == 0) {
		...
		for(np = mf->nextnet; np; np = np->next) {
			...
			if(nt == nil)
				continue;
			hack =
			    np->fasttimeouthack && !lookforproto(nt, np->net);
			for(t = nt; mf->nreply < Nreply && t; t = t->entry) {
				...
				if(cp) {
					/* avoid duplicates */
					for(i = 0; i < mf->nreply; i++)
						if(strcmp(mf->reply[i], cp) ==
						   0)
							break;
					if(i == mf->nreply) {
						/* save the reply */
						mf->replylen[mf->nreply] =
						    strlen(cp);
	...
	if(np && np->net != nil) {
		...
		if(mf->serv)
			...
			snprint(reply, sizeof(reply), "%s/%s/clone %s", mntpt,
			        mf->net, mf->host);
...
iptrans(Ndbtuple* t, Network* np, char* serv, char* rem, int hack)
{
	...
	if(*t->val == '*')
		snprint(reply, sizeof(reply), "%s/%s/clone %s%s", mntpt,
		        np->net, ts, x);
	else
		snprint(reply, sizeof(reply), "%s/%s/clone %s!%s%s%s", mntpt,
...
telcotrans(Ndbtuple* t, Network* np, char* serv, char* rem, int)
{
	...
	if(rem != nil)
		...
		*x = 0;
	if(serv)
		snprint(reply, sizeof(reply), "%s/%s/clone %s!%s%s", mntpt,
		        np->net, t->val, serv, x);
	else
		snprint(reply, sizeof(reply), "%s/%s/clone %s%s", mntpt,
		        np->net, t->val, x);
