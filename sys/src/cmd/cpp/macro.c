...
expand(Tokenrow* trp, Nlist* np, int inmacro)
{
	...
	if(np->ap == NULL)          /* parameterless */
		...
		ntokc = gatherargs(trp, atr,
		                   (np->flag & ISVARMAC) ? rowlen(np->ap) : 0,
		                   &narg);
		if(narg < 0) { /* not actually a call (no '(') */
			       /* error(WARNING, "%d %r\n", narg, trp); */
			/* gatherargs has already pushed trp->tr to the next
			 * token */
			return;
		}
	...
	for(trp->tp = trp->bp; trp->tp < trp->lp; trp->tp++) {
		if(trp->tp->type == DSHARP1)
			...
			if(ntr.lp - ntr.bp != 2 || ntr.bp->type == UNCLASS)
				error(WARNING, "Bad token %r produced by ##",
				      &ntr);
			ntr.lp = ntr.bp + 1;
			trp->tp = ltp;
			makespace(&ntr);
...
builtin(Tokenrow* trp, int biname)
{
	...
	*op++ = '"';
	...
	case KDATE:
		...
		strncpy(op + 7, curtime + 24,
		        4); /* Plan 9 asctime disobeys standard */
		op += 11;
		break;
