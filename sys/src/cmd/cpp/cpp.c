		...
		if(trp->tp->type == END) {
			if(--incdepth >= 0) {
				if(cursource->ifdepth)
					error(ERROR, "Unterminated conditional "
					             "in #include");
				unsetsource();
...
control(Tokenrow* trp)
{
	...
	case KUNDEF:
		...
		if((np = lookup(tp, 0))) {
			if(np->flag & ISUNCHANGE) {
				error(ERROR,
				      "#defined token %t can't be undefined",
				      tp);
				return;
			}
			np->flag &= ~ISDEFINED;
	...
	kline:
		...
		if(tp + 1 < trp->lp)
			cursource->filename =
			    (char*)newstring(tp->t + 1, tp->len - 2, 0);
		return;
	...
	default:
		error(ERROR, "Preprocessor control `%t' not yet implemented",
		      tp);
		break;
	}
	setempty(trp);
