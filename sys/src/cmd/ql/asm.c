...
elf32jmp(Putl putl)
{
	...
	elf32phdr(putl, PT_LOAD, HEADR + textsize - JMPSZ, 0xFFFFFFFC,
	          0xFFFFFFFC, JMPSZ, JMPSZ, R | X, 0); /* text */
}
...
asmb(void)
{
	...
	for(p = firstp; p != P; p = p->link) {
		...
		if(prevpc & (1 << 31) && (pc & (1 << 31)) == 0) {
			...
			Bprint(&bso,
			       "%s: warning: text segment wrapped past 0\n",
			       tn);
		}
		prevpc = pc;
	}
	...
	if(!debug['s']) {
		...
		if(dlm)
			asmdyn();
		if(HEADTYPE == 0 ||
		   HEADTYPE == 1) /* round up file length for boot image */
			if((symsize + lcsize) & 1)
				CPUT(0);
		cflush();
	} else if(dlm) {
