/* read Portable Network Graphics format */
#include <u.h>
#include <libc.h>
#include <ctype.h>
#include <bio.h>
...
zread(void *va)
{
	...
	if(z->b >= z->e)
		for (;;) {
			...
			else if(type[0] & PropertyBit)
				continue;  /* skip auxiliary chunks for now */
...
unfilter(int alg, uchar *buf, uchar *up, int len, int bypp)
{
	...
	case FilterPaeth:
		...
		break;
	default:
		sysfatal("unknown filtering scheme %d", alg);
	}
}
...
convertpix(ZlibW *z, uchar *pixel, uchar *r, uchar *g, uchar *b)
{
	...
	case 2:			/* gray+alpha */
		...
		if (pixel[0] > z->palsize)
			sysfatal("index %d out of bounds %d", pixel[0],
				z->palsize);
		off = 3*pixel[0];
		*r = z->palette[off];
		*g = z->palette[off+1];
 ...
 */
	...
	for (i = 1; i < z->scanl; ++i)
		for (bit = 128; bit > 0; bit /= 2) {
			...
			if (ch == nch) {
				if (z->npix++ < z->chanl)
					convertpix(z, pixel, z->chan[0]+pd,
						z->chan[1]+pd, z->chan[2]+pd);
				pd++;
				if (pd % z->scanpix == 0)
					goto out;
				ch = 0;
