	...
	while(iy >= 0) {
		...
		while(ix < width) {
			...
			if(0 != val) {
				...
				default
				    : /* Transparent data sequence detected */
					p = &buf[ix + iy * width];
					if((1 == (val & 3)) || (2 == (val & 3)))
						skip = 1;
					else
		...
		while(ix < width) {
			...
			if(0 != val) {
				...
				case 2: /* Position change detected */
					...
					iy -= val;
					break;
				default: /* Transparent (not compressed)
				            sequence detected */
					p = &buf[ix + iy * width];
					if(val & 1)
						skip = 1;
...
load_16(Biobuf* b, int32_t width, int32_t height, Rgb* buf, Rgb* clut)
{
	...
	if(clut) {
		unsigned mask_blue =
		    (unsigned)clut[0].blue + ((unsigned)clut[0].green << 8);
		unsigned mask_green =
		    (unsigned)clut[1].blue + ((unsigned)clut[1].green << 8);
		unsigned mask_red =
		    (unsigned)clut[2].blue + ((unsigned)clut[2].green << 8);
		...
		for(iy = height; iy; iy--, i += step_up)
			for(ix = 0; ix < width; ix++, i++) {
				...
				if(shft_blue >= 0)
					buf[i].blue = (uint8_t)(
					    (val & mask_blue) >> shft_blue);
				else
					buf[i].blue = (uint8_t)(
					    (val & mask_blue) << -shft_blue);
				if(shft_green >= 0)
					buf[i].green = (uint8_t)(
					    (val & mask_green) >> shft_green);
				else
					buf[i].green = (uint8_t)(
					    (val & mask_green) << -shft_green);
				if(shft_red >= 0)
					buf[i].red = (uint8_t)(
					    (val & mask_red) >> shft_red);
				else
					buf[i].red = (uint8_t)((val & mask_red)
					                       << -shft_red);
			}
	} else
		for(iy = height; iy; iy--, i += step_up)
