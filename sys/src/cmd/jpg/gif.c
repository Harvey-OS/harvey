...
main(int argc, char* argv[])
{
	...
	ARGBEGIN
	{
	case '3': /* produce encoded, compressed, three-color bitmap file; no
	             display by default */
		threeflag++;
	/* fall through */
	case 't': /* produce encoded, compressed, true-color bitmap file; no
	             display by default */
		cflag++;
		dflag++;
		output++;
		...
		outchan = RGB24;
		break;
	case 'c': /* produce encoded, compressed, bitmap file; no display by
	             default */
		cflag++;
		dflag++;
		output++;
	...
	case 'v': /* force RGBV */
		...
		outchan = CMAP8;
		break;
	case '9': /* produce plan 9, uncompressed, bitmap file; no display by
	             default */
		nineflag++;
		dflag++;
		output++;
...
/* interleave alpha values of 0xFF in data stream. alpha value comes first, then
 * b g r */
uint8_t*
...
show(int fd, char* name)
{
	...
	for(k = 0; k < n; k++) {
		if(outchan == CMAP8)
			...
			if(outchan == GREY8 ||
			   (images[k]->chandesc == CY && threeflag == 0))
		...
		if(rgbv[k] == nil) {
			fprint(
			    2,
			    "gif: converting %s to local format failed: %r\n",
			    name);
			err = "torgbv";
			goto Return;
		}
		if(!dflag) {
			...
			if(rgbv[k]->chandesc == CY)
				ims[k] = allocimage(display, rgbv[k]->r, GREY8,
				                    0, 0);
			else
				ims[k] = allocimage(display, rgbv[k]->r,
				                    outchan, 0, 0);
			if(ims[k] == nil) {
			...
			if(loadimage(ims[k], ims[k]->r, rgbv[k]->chans[0],
			             rgbv[k]->chanlen) < 0) {
		...
		do {
			for(k = 0; k < n; k++) {
				...
				if(dt < 50)
					dt = 50;
				while(n == 1 || ecankbd()) {
					if((ch = ekbd()) == 'q' || ch == 0x7F ||
					   ch == 0x04) /* an odd, democratic
					                  list */
						exits(nil);
					if(ch == '\n')
						goto Out;
			...
			/* loopcount -1 means no loop (this code's rule),
			 * loopcount 0 means loop forever (netscape's rule)*/
	...
	if(n > 1 && output)
		fprint(2, "gif: warning: only writing first image in %d-image "
		          "GIF %s\n",
		       n, name);
	if(nineflag) {
		...
		chantostr(buf, outchan);
		print("%11s %11d %11d %11d %11d ", buf, rgbv[0]->r.min.x,
		      rgbv[0]->r.min.y, rgbv[0]->r.max.x, rgbv[0]->r.max.y);
		if(write(1, rgbv[0]->chans[0], rgbv[0]->chanlen) !=
		   rgbv[0]->chanlen) {
