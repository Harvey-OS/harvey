...
int threeflag = 0;
int colorspace =
    CYCbCr; /* default for 8-bit displays: combine color rotation with dither */
int output = 0;
...
main(int argc, char* argv[])
{
	...
	if(yflag == 0 && dflag == 0 &&
	   colorspace == CYCbCr) { /* see if we should convert right to RGB */
		fd = open("/dev/screen", OREAD);
		if(fd > 0) {
			buf[12] = '\0';
			if(read(fd, buf, 12) == 12 &&
			   chantodepth(strtochan(buf)) > 8)
				colorspace = CRGB;
			close(fd);
		}
	}
...
vidmerge(Rawimage** aa1, Rawimage** aa2)
{
	...
	for(i = 0; aa1[i]; i++) {
		...
		if(a1->cmaplen != a2->cmaplen) {
			fprint(2,
			       "jpg: vidmerge: cmaplen different in img %d\n",
			       i);
			return nil;
		}
...
rpt:
	...
	if(nineflag) {
		...
		print("%11s %11d %11d %11d %11d ", buf, c->r.min.x, c->r.min.y,
		      c->r.max.x, c->r.max.y);
