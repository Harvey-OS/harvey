...
torgbv(Rawimage* i, int errdiff)
{
	...
	int dx, dy;
	char err[ERRMAX];
	uint8_t* cmap, *cm, *in, *out, *inp, *outp, cmap1[3 * 256], map[256],
	    *rpic, *bpic, *gpic;
	...
	case CRGB1:
		...
		if(j > 8)
			return _remaperror("remap: can't do colormap size 3*%d",
			                   i->cmaplen / 3);
		if(i->cmaplen != 3 * 256) {
			/* to avoid a range check in inner loop below, make a
			 * full-size cmap */
		...
		if(errdiff == 0) {
			...
			/* modified floyd steinberg, coefficients (1 0) 3/16,
			 * (0, 1) 3/16, (1, 1) 7/16 */
			for(y = 0; y < dy; y++) {
				er = 0;
				eg = 0;
	...
	case CYCbCr:
		...
		map3 = ycbcrmap;
		if(i->nchans != 3)
			return _remaperror("remap: RGB image has %d channels",
			                   i->nchans);
		goto Threecolor;
		...
		if(errdiff == 0) {
			...
			for(y = 0; y < dy; y++) {
				...
				for(x = 0; x < dx; x++) {
					...
					bpic += bpc;
					/*
					 * Errors can be uncorrectable if
					 * converting from YCbCr,
					 * since we can't guarantee that an
					 * extremal value of one of
					 * the components selects a color with
					 * an extremal value.
					 * If we don't, the errors accumulate
					 * without bound.  This
					 * doesn't happen in RGB because the
					 * closest table can guarantee
					 * a color on the edge of the gamut,
					 * producing a zero error in
					 * that component.  For the rotation
					 * YCbCr space, there may be
					 * no color that can guarantee zero
					 * error at the edge.
					 * Therefore we must clamp explicitly
					 * rather than by assuming
					 * an upper error bound of CLAMPOFF.
