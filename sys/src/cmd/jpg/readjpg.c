...
static void huffmantables(Header*, uint8_t*, int);
...
static void colormap(Header*, int, Rawimage*, int**, int**, int**, int, int,
                     int, int, int*, int*);
...
readslave(Header* header, int colorspace)
{
	...
	while(header->err[0] == '\0') {
		...
		case APPn + 0:
			if(nseg == 1 &&
			   strncmp((char*)b, "JFIF", 4) ==
			       0) /* JFIF header; check version */
				if(b[5] > 1 || b[6] > 2)
					sprint(header->err, "ReadJPG: can't "
					                    "handle JFIF "
					                    "version %d.%2d",
					       b[5], b[6]);
			break;
		...
		case SOS:
			...
			default:
				sprint(
				    header->err,
				    "unrecognized or unspecified encoding %d",
				    header->mode);
				break;
			}
			break;
...
marker(Header* h)
{
	...
	if(c != 0xFF)
		jpgerror(
		    h, "ReadJPG: expecting marker; found 0x%x at offset %lld\n",
		    c, Boffset(h->fd));
	while(c == 0xFF)
		c = readbyte(h);
	return c;
...
readsegment(Header* h, int* markerp)
{
	...
	case 0:
		jpgerror(h,
		         "ReadJPG: expecting marker; saw %.2x at offset %lld",
		         m, Boffset(h->fd));
	}
...
baselinescan(Header* h, int colorspace)
{
	...
	for(comp = 0; comp < Ns; comp++) {
		 ...
		 * frame, */
		/* so if both have 3 we know scan is Y Cb Cr and there's no need
		 * to */
		/* reorder */
	...
	for(mcu = 0; mcu < nmcu;) {
		...
		if(Ns == 1) /* very easy */
			colormap1(h, colorspace, image, data[0][0], mcu,
			          nacross);
		else if(allHV1) /* fairly easy */
			colormapall1(h, colorspace, image, data[0][0],
			             data[1][0], data[2][0], mcu, nacross);
		else /* miserable general case */
			colormap(h, colorspace, image, data[0], data[1],
			         data[2], mcu, nacross, Hmax, Vmax, H, V);
...
restart(Header* h, int mcu)
{
	...
	if(rst < 0)
		jpgerror(h, readerr);
	if((rst & 7) != (rest & 7))
		jpgerror(h, "ReadJPG: expected RST%d got %d", rest & 7,
		         rst & 7);
	h->cnt = 0;
	h->sr = 0;
}
...
progressiveIDCT(Header* h, int colorspace)
{
	...
	for(mcu = 0; mcu < nmcu; mcu++) {
		...
		/* rotate colors to RGB and assign to bytes */
		if(Nf == 1) /* very easy */
			colormap1(h, colorspace, h->image, data[0][0], mcu,
			          h->nacross);
		else if(allHV1) /* fairly easy */
			colormapall1(h, colorspace, h->image, data[0][0],
			             data[1][0], data[2][0], mcu, h->nacross);
		else /* miserable general case */
			colormap(h, colorspace, h->image, data[0], data[1],
			         data[2], mcu, h->nacross, h->Hmax, h->Vmax, H,
			         V);
	}
...
progressiveinit(Header* h, int colorspace)
{
	...
	for(k = 0; k < Nf; k++) {
		h->dccoeff[k] =
		    jpgmalloc(h, h->nblock[k] * nmcu * sizeof(int), 1);
		h->accoeff[k] =
		    jpgmalloc(h, h->nblock[k] * nmcu * sizeof(int*), 1);
...
progressivedc(Header* h, int comp, int Ah, int Al)
{
	...
	if(Ns != h->Nf)
		jpgerror(
		    h,
		    "ReadJPG: can't handle progressive with Nf!=Ns in DC scan");
...
progressiveacinc(Header* h, int comp, int Al)
{
	...
	int ri, mcu, nacross, ndown, nhor, nver, eobrun, nzeros, pending, x, y,
	    tmcu, blockno, q, nmcu;
	Huffman* acht;
	...
	for(y = 0; y < nver; y++) {
		for(x = 0; x < nhor; x++) {
			...
			if(eobrun > 0) {
				if(nzeros > 0)
					jpgerror(h, "ReadJPG: zeros pending at "
					            "block start");
				for(k = Ss; k <= Se; k++)
...
static void
colormap1(Header* h, int colorspace, Rawimage* image, int data[8 * 8], int mcu,
          int nacross)
{
	uint8_t* pic;
...
static void
colormapall1(Header* h, int colorspace, Rawimage* image, int data0[8 * 8],
             int data1[8 * 8], int data2[8 * 8], int mcu, int nacross)
{
...
colormap(Header* h, int colorspace, Rawimage* image, int* data0[8 * 8],
         int* data1[8 * 8], int* data2[8 * 8], int mcu, int nacross, int Hmax,
         int Vmax, int* H, int* V)
{
	...
	for(y = 0; y < dy; y++) {
		...
		for(x = 0; x < dx; x++) {
			if(colorspace == CYCbCr) {
				rpic[pici + x] =
				    clamp[data0[b0][y0 + x0++ * H0 / Hmax] +
				          128 + CLAMPOFF];
				gpic[pici + x] =
				    clamp[data1[b1][y1 + x1++ * H1 / Hmax] +
				          128 + CLAMPOFF];
				bpic[pici + x] =
				    clamp[data2[b2][y2 + x2++ * H2 / Hmax] +
				          128 + CLAMPOFF];
			} else {
...
decode(Header* h, Huffman* t)
{
	...
	if(i >= 17) {
		if(badcode == 0)
			fprint(2, "badly encoded %dx%d JPEG file; ignoring bad "
			          "value\n",
			       h->X, h->Y);
		badcode = 1;
		i = 0;
...
nextbyte(Header* h, int marker)
{
	...
	if(b == 0xFF) {
		...
		if(b2 != 0) {
			...
			if(b2 == DNL)
				jpgerror(h,
				         "ReadJPG: DNL marker unimplemented");
			/* decoder is reading into marker; satisfy it and
			 * restore state */
			Bungetc(h->fd);
			h->peek = b;
		}
...
idct(int b[8 * 8])
{
	...
	for(y = 0; y < 8; y++) {
		eighty = y << 3;
		/* if all non-DC components are zero, just propagate the DC
		 * term*/
		p = b + eighty;
		if(p[1] == 0)
			if(p[2] == 0 && p[3] == 0)
