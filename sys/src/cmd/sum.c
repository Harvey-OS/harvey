...
uint32_t
sum5(uint32_t sum, void* buf, uint64_t uvn)
{
	uint8_t* s, *send;
	int n;
...
uint32_t
sumr(uint32_t sum, void* buf, uint64_t uvn)
{
	uint8_t* s, *send;
	int n;
...
uint32_t
sum32(uint32_t lcrc, void* buf, uint64_t uvn)
{
	uint8_t* s = buf;
	uint32_t crc = lcrc;
	int n;
	...
	if(buf == 0) {
		...
		x[0] = (n >> 24) ^
		       0xCC; /* encode the length but make n==0 not 0 */
		x[1] = (n >> 16) ^ 0x55;
		x[2] = (n >> 8) ^ 0xCC;
		x[3] = (n) ^ 0x55;
		crc = sum32(lcrc, x, 4);
