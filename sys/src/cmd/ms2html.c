...
/* if arg0 is set, read up to (and expand) to the next whitespace, else to the
 * end of line */
char*
...
doconvert(void)
{
	...
	Bprint(&bout, "Copyright</A> &#169; %d Alcatel-Lucent Inc.  All rights "
	              "reserved.\n",
	       t->year + 1900);
...
g_TL(int i, char** c)
{
	...
	if(!titleseen) {
		...
		Bprint(&bout, "%s\n", title);
		Bprint(&bout, "</title>\n");
		Bprint(&bout, "<body BGCOLOR=\"#FFFFFF\" TEXT=\"#000000\" "
		              "LINK=\"#0000FF\" VLINK=\"#330088\" "
		              "ALINK=\"#FF0044\">\n");
		titleseen = 1;
	}
...
g_BP(int argc, char** argv)
{
	...
	ext = strrchr(p, '.');
	if(ext) {
		if(strcmp(ext, ".jpeg") == 0 || strcmp(ext, ".gif") == 0) {
			Bprint(&bout, "<br><img src=\"%s\"><br>\n", argv[1]);
			return;
		}
	}
