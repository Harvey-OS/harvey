...
Cipher* cryptos[] = {
    &cipheraes128, &cipheraes192, &cipheraes256,
    //	&cipherblowfish,
    &cipher3des,   &cipherrc4,
};
...
char* st_names[] = {
        [Empty] "Empty", [Allocated] "Allocated", [Initting] "Initting",
        [Listening] "Listening", [Opening] "Opening",
        [Negotiating] "Negotiating", [Authing] "Authing",
        [Established] "Established", [Eof] "Eof", [Closing] "Closing",
        [Closed] "Closed",
};
...
stopen(Req* r)
{
	...
	case Qlisten:
		r->aux = (void*)threadcreate(
		    (lev == Connection ? stlisconn : stlischan), r, Defstk);
		break;
	case Qclone:
		switch(lev) {
		case Top:
...
readreqrem(void* a)
{
	...
	case Connection:
		if(c->ctlfd >= 0) {
			...
			buf[n] = '\0';
			snprint(path, NETPATHLEN, "%s/tcp/%s/remote", mntpt,
			        buf);
...
readdata(void* a)
{
	...
	sc = c->chans[cnum];
	if(sc->dataq == nil &&
	   (sc->state == Closed || sc->state == Closing || sc->state == Eof)) {
...
stwrite(Req* r)
{
	...
	case Qdata:
		...
		if(lev == Top || lev == Connection || c->state == Closed ||
		   c->state == Closing || ch->state == Closed ||
		   ch->state == Closing) {
			respond(r, nil);
			break;
		}
...
dialbyhand(Conn* c, int ntok, char* toks[])
{
	...
	return fprint(c->ctlfd, "connect %s %s", toks[1],
	              ntok > 3 ? toks[2] : "");
}
...
writectlproc(void* a)
{
	...
	case Connection:
		...
		if(strcmp(toks[0], "ssh-userauth") == 0)
			...
			snprint(path, NETPATHLEN, "%s/tcp/%s/data", mntpt,
			        tcpconn);
		...
		if(strcmp(toks[0], "connect") == 0) {
			...
			if(ch->state != Closed && ch->state != Closing) {
				...
				if(ch->otherid != -1) {
					...
					n = finish_packet(p);
					iowrite(c->dio, c->datafd, p->nlength,
					        n);
					free(p);
				}
				qlock(&c->l);
...
writereqremproc(void* a)
{
	...
	r->ofcall.count = r->ifcall.count;
	if(c->state == Closed || c->state == Closing || ch->state == Closed ||
	   ch->state == Closing)
...
stclunk(Fid* f)
{
	...
	c = connections[cnum];
	sshdebug(c, "got clunk on file: %#llux %d %d %d: %s", qidpath, lev,
	         cnum, chnum, f->file->name);
	...
	if(lev == Connection && (qidpath & Qtypemask) == Qctl &&
	   (c->state == Opening || c->state == Negotiating ||
	    c->state == Authing)) {
		for(n = 0; n < MAXCONN &&
		               (!c->chans[n] || c->chans[n]->state == Empty ||
		                c->chans[n]->state == Closed ||
		                c->chans[n]->state == Closing);
		    ++n)
			;
	...
	for(n = 0;
	    n < MAXCONN &&
	        (!c->chans[n] || c->chans[n]->state == Empty ||
	         c->chans[n]->state == Closed || c->chans[n]->state == Closing);
	    ++n)
		;
	if(n >= MAXCONN) {
		if(c->rpid >= 0)
...
stflush(Req* r)
{
	...
	sshdebug(nil, "got flush on file %#llux %lld %lld %lld: %s %#p", argv0,
	         qidpath, qidpath >> Levshift,
	...
	if(! or->aux)
		respond(or, "interrupted");
	        ...
	        or
	            ->ifcall.type == Tread && (qidpath & Qtypemask) == Qdata &&
...
alloc_conn(void)
{
	...
	c->cscrypt = c->csmac = c->ctlfd = c->datafd = c->decrypt = c->encrypt =
	    c->inmac = c->ncscrypt = c->ncsmac = c->nsccrypt = c->nscmac =
	        c->outmac = c->rpid = c->sccrypt = c->scmac = c->tcpconn = -1;
	if(c->e) {
		mpfree(c->e);
		c->e = nil;
	}
...
alloc_chan(Conn* c)
{
	...
	if(sc->dir == nil) {
		...
		sc->dir = createfile(c->dir, buf, uid, 0555 | DMDIR,
		                     (void*)(slcn | Qroot));
		sc->ctl =
		    createfile(sc->dir, "ctl", uid, 0666, (void*)(slcn | Qctl));
		...
		sc->status = createfile(sc->dir, "status", uid, 0444,
		                        (void*)(slcn | Qstatus));
		sc->tcp =
		    createfile(sc->dir, "tcp", uid, 0444, (void*)(slcn | Qtcp));
	}
	c->nchan++;
...
exchids(Conn* c, Ioproc* io, char* remid, int remsz)
{
	...
	if(c->role == Server) {
		...
		sendmyid(c, io);
		sshdebug(c, "dohandshake: client, got `%s' sent `%s'", remid,
		         MYID);
		if(remid[0] == '\0') {
...
dohandshake(Conn* c, char* tcpconn)
{
	...
	if(c->idstring[0] == '\0' && strncmp(path, "SSH-2", 5) != 0 &&
...
established(Conn* c, Packet* p, Packet* p2, char* buf, int size)
{
	...
	case SSH_MSG_KEXINIT:
		...
		if(validatekex(c, p) < 0) {
			sshdebug(c,
			         "kex crypto algorithm mismatch (Established)");
			return -1;
		}
	...
	case SSH_MSG_CHANNEL_OPEN:
		...
		sshdebug(c, "searching for a listener for channel type %s",
		         buf);
		ch = alloc_chan(c);
		if(ch == nil) {
			nochans(c, p, p2);
		...
		if(i >= c->nchan) {
			...
			sshdebug(c, "waiting for someone to announce %s",
			         ch->ann);
			rsleep(&ch->r);
		} else {
	...
	case SSH_MSG_CHANNEL_WINDOW_ADJUST:
		...
		sshdebug(c, "new twindow for channel: %d: %lud", cnum,
		         ch->twindow);
		qlock(&ch->xmtlock);
	...
	case SSH_MSG_CHANNEL_CLOSE:
		...
		qunlock(&c->l);
		for(i = 0; i < MAXCONN &&
		               (!c->chans[i] || c->chans[i]->state == Empty ||
		                c->chans[i]->state == Closed);
		    ++i)
			;
		if(i >= MAXCONN)
			return -1;
	...
	case SSH_MSG_CHANNEL_REQUEST:
		...
		pl = emalloc9p(sizeof(Plist));
		pl->pack = emalloc9p(sizeof(Packet));
		n = snprint((char*)pl->pack->payload, Maxpayload, "%s %c", buf,
		            *q ? 't' : 'f');
		sshdebug(c, "request message begins: %s",
		         (char*)pl->pack->payload);
		memmove(pl->pack->payload + n, q + 1,
		        p->rlength - (11 + n - 2));
...
reader0(Conn* c, Packet* p, Packet* p2)
{
	...
	if(p->rlength > Maxpktpay) {
		sshdebug(
		    c,
		    "absurd packet length: %ld, unrecoverable decrypt failure",
		    p->rlength);
	...
	n = nl + np + nm;
	if(debug) {
		sshdebug(c, "got message of %d bytes %d padding", n,
		         p->pad_len);
		if(p->payload[0] > SSH_MSG_CHANNEL_OPEN) {
			i = nhgetl(p->payload + 1);
			if(c->chans[i])
				sshdebug(c, " for channel %d win %lud", i,
				         c->chans[i]->rwindow);
			else
	...
	if(np != p->rlength + 4 - nb || c->inmac != -1 && nm != SHA1dlen) {
		sshdebug(c, "got EOF/error on connection read: %d %d %r", np,
		         nm);
	...
	if(c->state == Initting) {
		if(p->payload[0] != SSH_MSG_KEXINIT) {
			sshdebug(c, "missing KEX init packet: %d",
			         p->payload[0]);
			bail(c, p, p2, "bad kex");
		}
		if(c->rkexinit)
		...
		case SSH_MSG_DISCONNECT:
			if(debug) {
				get_string(p, p->payload + 5, buf, Arbbufsz,
				           nil);
				sshdebug(c, "got disconnect: %s", buf);
			}
		...
		sshdebug(c, "connection %d in bad state, reader exiting",
		         c->id);
...
reader(void* a)
{
	...
	c = a;
	c->rpid = threadid();
	sshdebug(c, "starting reader for connection %d, pid %d", c->id,
	         c->rpid);
	threadsetname("reader");
	p = new_packet(c);
	p2 = new_packet(c);
...
static int
authreqpk(Packet* p, Packet* p2, Conn* c, char* user, uint8_t* q, char* alg,
          char* blob, char* sig, char* service, char* me)
{
...
client_auth(Conn* c, Ioproc* io)
{
	...
	if(c->password) {
		...
		sshdebug(c, "client_auth using password for svc %s",
		         c->service);
	} else {
		...
		if((p3 = pkas[i]->sign(c, p4->payload, p4->rlength - 1)) ==
		   nil) {
	...
	if(c->clonefile->ref <= 2 && c->ctlfile->ref <= 2 &&
	   ...
	   c->statusfile->ref <= 2)
		...
		sshdebug(c, "clone %ld ctl %ld data %ld listen %ld "
		         ...
		         c->listenfile->ref, c->localfile->ref,
