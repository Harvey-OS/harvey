...
newtunnel(char *myname)
{
	...
	if(debug)
		fprint(2, "%s: starting new sshtun for key access\n", argv0);
	kid = rfork(RFPROC|RFFDG|RFNOTEG|RFENVG);
	if (kid == 0) {
		for (fd = 3; fd < 40; fd++)
			close(fd);
		execl("/bin/sshtun", "sshtun", "-m", netdir, "-s", myname, nil);
		sysfatal("no /bin/sshtun: %r");
	} else if (kid < 0)
