...
dh_init(PKA *pkas[])
{
	...
	if (p != nil) {
		...
		if (st == nil) {
			syslog(0, "ssh", "sshtun no proto=rsa key in factotum");
			goto norsa;
		}
		end = st;
		for (; st > buf && *st != '\n'; --st)
	...
	p = strstr(st, " n=");
	if (p == nil || p > end) {
		syslog(0, "ssh", "sshtun no key (n) found");
		free(buf);
		return;
	}
	...
	p = strstr(st, " ek=");
	if (p == nil || p > end) {
		syslog(0, "ssh", "sshtun no key (ek) found");
		free(buf);
		return;
	}
	pkas[k++] = &rsa_pka;
	...
	if (p == nil) {
		p = strstr(st, "!dk?");
		if (p == nil || p > end) {
			// syslog(0, "ssh", "sshtun no key (dk) found");
			free(buf);
			return;
		}
		goto norsa;
...
norsa:
	...
	if (p != nil) {
		...
		if (st == nil) {
			syslog(0, "ssh", "sshtun no proto=dsa key in factotum");
			free(buf);
			return;
		}
		end = st;
	...
	p = strstr(buf, " p=");
	if (p == nil || p > end) {
		syslog(0, "ssh", "sshtun no key (p) found");
		free(buf);
		return;
	}
	...
	p = strstr(buf, " q=");
	if (p == nil || p > end) {
		syslog(0, "ssh", "sshtun no key (q) found");
		free(buf);
		return;
	}
	...
	p = strstr(buf, " alpha=");
	if (p == nil || p > end) {
		syslog(0, "ssh", "sshtun no key (g) found");
		free(buf);
		return;
	}
	...
	p = strstr(buf, " key=");
	if (p == nil || p > end) {
		syslog(0, "ssh", "sshtun no key (y) found");
		free(buf);
		return;
	}
	...
	if (p == nil) {
		p = strstr(buf, "!secret?");
		if (p == nil || p > end)
			syslog(0, "ssh", "sshtun no key (x) found");
		free(buf);
		return;
	}
...
rsa_ks(Conn *c)
{
	...
	if (myrsakey.pub.ek == nil || myrsakey.pub.n == nil) {
		syslog(0, "ssh", "sshtun no public RSA key info");
		return nil;
	}
	ks = new_packet(c);
...
rsa_sign(Conn *c, uchar *m, int nm)
{
	...
	if (myrsakey.dk) {
		...
		if (ar == nil ||
		    ...
		    ar->arg == nil) {
			if (debug)
				fprint(2, "%s: got error in factotum: %r\n",
					argv0);
			auth_freerpc(ar);
			close(fd);
			return nil;
...
rsa_verify(Conn *c, uchar *m, int nm, char *user, char *sig, int)
{
	...
	if (debug)
		fprint(2, "%s: in rsa_verify for connection: %d\n", argv0, c->id);
	...
	if (0 && rsa_exponent) {
		...
		if (ar == nil || auth_rpc(ar, "start", p, strlen(p)) != ARok ||
			...
			if (debug)
				fprint(2, "%s: factotum returned %s\n",
					argv0, ar->ibuf);
...
{
	if (debug)
		fprint(2, "%s: in dss_verify for connection: %d\n", argv0, c->id);
...
dh_server(Conn *c, Packet *pack1, mpint *grp, int nbit)
{
	...
	pack2 = new_packet(c);
	if (debug)
		fprint(2, "%s: ID strings: %s---%s\n", argv0, c->otherid, MYID);
	...
	if (sig == nil) {
		syslog(0, "ssh", "sshtun failed to generate signature: %r");
		mpfree(f);
		mpfree(e);
		mpfree(k);
		mpfree(y);
	...
	if (debug > 1) {
		fprint(2, "%s: writing %d bytes: len %d\n", argv0, n,
			nhgetl(pack2->nlength));
		dump_packet(pack2);
	}
...
dh_client12(Conn *c, Packet *p)
{
	...
	if (debug)
		fprint(2, "%s: verifying server signature\n", argv0);
	...
	if (newkey == nil) {
		syslog(0, "ssh", "sshtun out of memory");
		exits("out of memory");
	}
	 ...
	 * and do so using the stock factotum.
	 */
	if (nokeyverify)
		n = 1;
