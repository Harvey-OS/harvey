...
auth(char *buf, int n, int ctlfd)
{
	...
	if (fd < 0) {
		syslog(0, "ssh", "can't open capuse: %r");
		hangup(ctlfd);
		exits("capuse");
	}
	if (write(fd, buf, n) != n) {
		syslog(0, "ssh", "write to capuse failed: %r");
		hangup(ctlfd);
		exits("capuse");
	}
	close(fd);
...
main(int argc, char *argv[])
{
	...
	errfd = -1;
	if (debug)
		errfd = create("/tmp/ssh.err", OWRITE, 0666);
	...
	syslog(0, "ssh", "server netdir is %s", netdir);	// TODO
	...
	syslog(0, "ssh", "server listen is %s", listfile); // TODO
