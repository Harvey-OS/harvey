...
auth(char* buf, int n, int ctlfd)
{
	...
	if(write(fd, buf, n) != n) {
		syslog(0, "ssh", "server write `%.*s' to capuse failed: %r", n,
		       buf);
		hangup(ctlfd);
		exits("capuse");
	}
...
main(int argc, char* argv[])
{
	...
	if(0 && idstring) { /* was for coexistence */
		syslog(0, "ssh", "server conn %s, writing \"id %s\" to %s", buf,
		       idstring, path);
...
forkcmd(char* cmd, char* p, int reqfd, int datafd, int want_reply)
{
	...
	case 0:
		...
		if(!prevent ||
		   (q = getenv("sshsession")) && strcmp(q, "allow") == 0)
			get_string(p + 1, cmd);
		else
			confine(p + 1, cmd);
