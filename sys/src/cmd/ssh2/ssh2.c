...
static int chpid;
static int reqfd, dfd1, cfd1, dfd2, cfd2, consfd, kconsfd, cctlfd, notefd,
    keyfd;
...
main(int argc, char* argv[])
{
	...
	quotefmtinstall();
	reqfd = dfd1 = cfd1 = dfd2 = cfd2 = consfd = kconsfd = cctlfd = notefd =
	    keyfd = -1;
	whichkey = nil;
	ARGBEGIN
	{
...
doauth(int cfd1, char* whichkey)
{
	...
	if(!nopka) {
		if(whichkey)
			n = fprint(cfd1, "ssh-userauth K %q %q", user,
			           whichkey);
		else
	...
	up = auth_getuserpasswd(iflag ? auth_getkey : nil,
	                        "proto=pass service=ssh server=%q user=%q",
	                        remote, user);
	if(up == nil) {
	...
	fprint(2,
	       "%s: auth ctl msg `ssh-userauth k %q <password>' for %q: %r\n",
	       argv0, user, path);
	return -1;
}
