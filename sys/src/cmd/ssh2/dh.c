...
enum { Errnokey = -2, /* no key on keyring */
 ...
 */
static char* group1p = "FFFFFFFFFFFFFFFFC90FDAA22168C234C4C6628B80DC1CD1"
...
*/
static char* group14p = "FFFFFFFFFFFFFFFFC90FDAA22168C234C4C6628B80DC1CD1"
                        "29024E088A67CC74020BBEA63B139B22514A08798E3404DD"
                        "EF9519B3CD3A431B302B0A6DF25F14374FE1356D6D51C245"
                        "E485B576625E7EC6F44C42E9A637ED6B0BFF5CB6F406B7ED"
...
rsa_sign(Conn* c, uint8_t* m, int nm)
{
	...
	if(myrsakey.dk) {
		...
		if(ar == nil ||
		   ...
		   auth_rpc(ar, "read", nil, 0) != ARok || ar->arg == nil) {
...
rsa_verify(Conn* c, uint8_t* m, int nm, char* user, char* sig, int)
{
	...
	if(0 && rsa_exponent) {
		...
		p = (char*)get_string(nil, (uint8_t*)sig, buf, Blobsz / 2, nil);
...
dh_server(Conn* c, Packet* pack1, mpint* grp, int nbit)
{
	...
	if(debug > 1) {
		sshdebug(c, "writing %d bytes: len %d", n,
...
findkeyinuserring(Conn* c, RSApub* srvkey)
{
	...
	if((n = findkey(r, c->remote, srvkey)) != KeyOk) {
		...
		free(keymbox.msg);
		keymbox.msg = smprint("%c%04ld%s",
		                      n == NoKeyFile || n == NoKey ? 'c' : 'b',
		...
		if(keymbox.msg == nil || keymbox.msg[0] == 'n') {
			...
			keymbox.msg =
			    smprint("f%04ld%s", strlen(newkey), newkey);
			return -1;
		}
...
verifyhostkey(Conn* c, RSApub* srvkey, Packet* sig)
{
	...
	if(fd >= 0) {
		...
		newkey =
		    smprint("delkey proto=rsa role=verify sys=%s", c->remote);
		if(newkey) {
			seek(fd, 0, 0);
			write(fd, newkey, strlen(newkey));
			free(newkey);
...
Kex dh1sha1 = {"diffie-hellman-group1-sha1", dh_server1, dh_client11,
               dh_client12};

Kex dh14sha1 = {"diffie-hellman-group14-sha1", dh_server14, dh_client141,
                dh_client142};

PKA rsa_pka = {"ssh-rsa", rsa_ks, rsa_sign, rsa_verify};

PKA dss_pka = {"ssh-dss", dss_ks, dss_sign, dss_verify};
