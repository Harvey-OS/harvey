...
enum { STnomem = -4,        /* buffer allocation failed */
...
#define PUTBELONG(p, ul)                                                       \
	((p)[0] = (ul) >> 24, (p)[1] = (ul) >> 16, (p)[2] = (ul) >> 8,         \
	 (p)[3] = (ul))
...
int32_t SRblank(ScsiReq*, unsigned char,
                unsigned char); /* MMC CD-R/CD-RW commands */
...
int32_t SRfwaddr(ScsiReq*, unsigned char, unsigned char, unsigned char,
                 unsigned char*);
