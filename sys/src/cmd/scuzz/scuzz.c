...
Biobuf bin, bout;
i32 maxiosize = MaxIOsize;
int exabyte = 0;
int force6bytecmds = 0;

typedef struct {
	char *name;
	i32 (*f)(ScsiReq *, int, char *[]);
	int open;
	char *help;
} ScsiCmd;
...
{
	u8 list[MaxDirData], *lp, page;
	i32 i, n, nbytes, status;
	char *p;
...
static i32
start(ScsiReq *rp, int argc, char *argv[], u8 code)
{
	char *p;
