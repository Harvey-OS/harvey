...
int history = 0;  /* use old interface, showing history of mailbox rather than
                     current state */
int initload = 0; /* initialize program with contents of mail box */

enum { Facesep = 6, /* must be even to avoid damaging background stipple */
       Infolines = 9,

       HhmmTime = 18 * 60 * 60, /* max age of face to display hh:mm time */
};

enum { Mainp, Timep, Mousep, NPROC };

int pids[NPROC];
char* procnames[] = {"main", "time", "mouse"};

Rectangle leftright = {0, 0, 20, 15};

uint8_t leftdata[] = {0x00, 0x80, 0x00, 0x01, 0x80, 0x00, 0x03, 0x80, 0x00,
                      0x07, 0x80, 0x00, 0x0f, 0x00, 0x00, 0x1f, 0xff, 0xf0,
                      0x3f, 0xff, 0xf0, 0xff, 0xff, 0xf0, 0x3f, 0xff, 0xf0,
                      0x1f, 0xff, 0xf0, 0x0f, 0x00, 0x00, 0x07, 0x80, 0x00,
                      0x03, 0x80, 0x00, 0x01, 0x80, 0x00, 0x00, 0x80, 0x00};

uint8_t rightdata[] = {0x00, 0x10, 0x00, 0x00, 0x18, 0x00, 0x00, 0x1c, 0x00,
                       0x00, 0x1e, 0x00, 0x00, 0x0f, 0x00, 0xff, 0xff, 0x80,
                       0xff, 0xff, 0xc0, 0xff, 0xff, 0xf0, 0xff, 0xff, 0xc0,
                       0xff, 0xff, 0x80, 0x00, 0x0f, 0x00, 0x00, 0x1e, 0x00,
                       0x00, 0x1c, 0x00, 0x00, 0x18, 0x00, 0x00, 0x10, 0x00};
...
setdate(void)
{
	...
	date[4 + 4 + 3 + 5] = '\0'; /* change from Thu Jul 22 14:28:43 EDT
	                               1999\n to Thu Jul 22 14:28 */
}
...
drawtime(void)
{
	...
	if(eqpt(enddate, ZP)) {
		enddate = r.min;
		enddate.x += stringwidth(
		    datefont, "Wed May 30 22:54"); /* nice wide string */
...
timeproc(void)
{
	for(;;) {
		...
		now = time(nil);
		sleep(((60 - now % 60) + 1) *
		      1000); /* wait for minute to change */
		setdate();
	}
}
...
alreadyseen(char* digest)
{
	...
	for(i = 0; i < nfaces; i++) {
		f = faces[i];
		if(f->str[Sdigest] != nil &&
		   strcmp(digest, f->str[Sdigest]) == 0)
			return 1;
	}
	return 0;
}
...
facerect(int index) /* index is geometric; 0 is always upper left face */
{
	...
	/* simple fix to avoid drawing off screen, allowing customers to use
	 * position */
...
drawface(Face* f, int i)
{
	...
	if(f->unknown) {
		...
		for(p.x = -1; p.x <= 1; p.x++)
			for(p.y = -1; p.y <= 1; p.y++)
				center(tinyfont, addpt(r.min, p),
				       f->str[Sdomain], display->white);
...
drawarrows(void)
{
	...
	draw(screen, rightr, last < nfaces ? blue : bgrnd, right,
	     leftright.min);
}

void addface(Face* f) /* always adds at 0 */
{
	Face** ofaces;
	Rectangle r0, r1, r;
	int y, nx, ny;
	...
	if(j < first)
		...
		for(y = (j - first) / nx; y < ny; y++) {
			if(x != nx - 1) {
				...
				r = r0;
				r.max.x = r.min.x +
				          (nx - x - 1) * (Facesize + Facesep);
				draw(screen, r, screen, nil, r1.min);
			}
			if(y != ny - 1) {
				/* copy one up from row below */
				r = facerect((y + 1) * nx);
				draw(screen, facerect(y * nx + nx - 1), screen,
				     nil, r.min);
			}
			x = 0;
		}
...
void delete(char* s, char* digest)
{
	int i;
	Face* f;
...
resized(void)
{
	...
	nacross =
	    (Dx(screen->r) - 2 * facep.x + Facesep) / (Facesize + Facesep);
	for(ndown = 1; rectinrect(facerect(ndown * nacross), screen->r);
	    ndown++)
		;
	setlast();
