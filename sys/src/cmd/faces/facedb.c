		...
		*ebuf = 0;
		...
		if(regexec(exp, dom, 0, 0)) {
			...
			if(ep - q + 2 > sizeof buf) {
				fprint(
				    2,
				    "huge replacement in machinelist: %.*s\n",
				    utfnlen(q, ep - q), q);
...
tryfindfiledir(char* dom, char* user, char* dir)
{
	...
	for(i = 8; i > 0; i >>= 1) {
		...
		if(access(ndir, AEXIST) >= 0 &&
		   (x = tryfindfiledir(dom, user, ndir)) != nil) {
			free(ndir);
			free(dom);
			return x;
		}
...
{
	Image* img;
	img = allocimage(display, Rect(0, 0, Facesize, Facesize), chan, 0,
	                 DNofill);
	if(img == nil) {
		clearsaved();
		img = allocimage(display, Rect(0, 0, Facesize, Facesize), chan,
		                 0, DNofill);
		if(img == nil)
			return nil;
	}
...
readbit(int fd, uint32_t chan)
{
	...
	if(n < ndata)
		return allocimage(display, Rect(0, 0, Facesize, Facesize),
		                  CMAP8, 0, 0x88888888);
...
readface(char* fn)
{
	...
	if(buf[0] == '0' && buf[1] == 'x') {
		...
		if(face == nil)
			goto Done;
		else if(face->chan == GREY4 ||
		        face->chan ==
		            GREY8) { /* greyscale: use inversion as mask */
			mask = myallocimage(face->chan);
			/* okay if mask is nil: that will copy the image white
			 * background and all */
			if(mask == nil)
				goto Done;
			...
			gendraw(mask, mask->r, display->black, ZP, face,
			        face->r.min);
			freeimage(face);
			face = display->black;
		} else if(face->depth ==
		          8) { /* snarf the bytes back and do a fill. */
			mask = myallocimage(GREY1);
			if(mask == nil)
				goto Done;
			if(unloadimage(face, face->r, data,
			               Facesize * Facesize) !=
			   Facesize * Facesize) {
				freeimage(mask);
				goto Done;
			...
			if(loadimage(mask, mask->r, mdata, sizeof mdata) !=
			   sizeof mdata) {
				freeimage(mask);
				goto Done;
			}
...
Done:
	/* always add at beginning of list, so updated files don't collide in
	 * cache */
	if(f == nil) {
...
findbit(Face* f)
{
	...
	if(f->file) {
		...
		f->bit =
		    allocimage(display, Rect(0, 0, 1, 1), CMAP8, 1, DYellow);
