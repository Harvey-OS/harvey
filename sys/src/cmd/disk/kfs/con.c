...
cmd_rename(void)
{
	...
	if(cons.arg[0] == '/') {
		...
		while(nextelem()) {
			if(noelem[0])
				if(con_walk(FID3, noelem)) {
					cprint("target path %s does not exist",
					       noelem);
					return;
				}
		...
		if(err = con_create(FID3, elem, d.uid, d.gid, perm,
		                    (d.mode & DDIR) ? OREAD : ORDWR)) {
			cprint("can't create %s: %s\n", elem, errstring[err]);
			return;
		}
		...
		cname(nxelem);
		if(strchr(nxelem, '/')) {
			cprint("bad rename target: not full path, but contains "
			       "slashes\n");
			return;
		}
...
adduser(char* user, int isgroup)
{
	 ...
	 */
	if(con_clone(FID1, FID2) || con_path(FID2, "/adm/users") ||
	   con_open(FID2, 1)) {
...
cmd_checkuser(void)
{
	...
	if(con_clone(FID1, FID2) || con_path(FID2, "/adm/users") ||
	   con_open(FID2, 0) || con_stat(FID2, (char*)buf))
		return;
	p = buf + 3 * NAMELEN + 4 * 4;
...
Command command[] = {
    "allow", cmd_allow, "", "allowoff", cmd_disallow, "", "atime", cmd_atime,
    "", "cfs", cmd_cfs, "[filesys]", "chat", cmd_chat, "", "check", cmd_check,
    "[cdfpPqrtw]", "clri", cmd_clri, "filename", "create", cmd_create,
    "filename user group perm [ald]", "disallow", cmd_disallow, "", "halt",
    cmd_halt, "", "help", cmd_help, "", "listen", cmd_listen, "[address]",
    "newuser", cmd_newuser, "username", "noneattach", cmd_noneattach, "",
    "nowritegroup", cmd_nowritegroup, "", "remove", cmd_remove, "filename",
    "rename", cmd_rename, "file newname", "start", cmd_start, "", "stats",
    cmd_stats, "[fw]", "sync", cmd_sync, "", "user", cmd_user, "", 0};
