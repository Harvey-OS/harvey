 ...
 */
int nbootprog =
    188; /* no. of bytes of boot program, including the first 0x3E */
unsigned char bootprog[512] = {
        [0x000] 0xEB, 0x3C, 0x90, 0x00, 0x00, 0x00, 0x00, 0x00,         0x00,
        0x00,         0x00, 0x00, 0x00, 0x00, 0x00, 0x00, [0x03E] 0xFA, 0xFC,
        0x8C,         0xC8, 0x8E, 0xD8, 0x8E, 0xD0, 0xBC, 0x00,         0x7C,
        0xBE,         0x77, 0x7C, 0xE8, 0x19, 0x00, 0x33, 0xC0,         0xCD,
        0x16,         0xBB, 0x40, 0x00, 0x8E, 0xC3, 0xBB, 0x72,         0x00,
        0xB8,         0x34, 0x12, 0x26, 0x89, 0x07, 0xEA, 0x00,         0x00,
        0xFF,         0xFF, 0xEB, 0xD6, 0xAC, 0x0A, 0xC0, 0x74,         0x09,
        ...
        'o',          's',  't',  ' ',  'a',  'n',  'y',  ' ',          'k',
        'e',          'y',  ' ',  't',  'o',  ' ',  'r',  'e',          'b',
        'o',          'o',  't',  '.',  '.',  '.',  0x00, 0x00,         0x00,
        [0x1F0] 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,         0x00,
...
sanitycheck(Disk* disk)
{
	...
	bad = 0;
	if(dos && nresrv < 2 &&
	   seek(disk->fd, disk->secsize, 0) == disk->secsize &&
	   read(disk->fd, buf, sizeof(buf)) >= 5 &&
	   strncmp(buf, "part ", 5) == 0) {
		fprint(2, "there's a plan9 partition on the disk\n"
	...
	if(disk->type == Tsd && disk->offset == 0LL) {
		fprint(
		    ...
		    "this is likely a mistake.  specify -x to disable this "
		    "check.\n");
		bad = 1;
	}
...
void
dosfs(int dofat, int dopbs, Disk* disk, char* label, int argc, char* argv[],
      int commit)
{
	char r[16];
	Dosboot* b;
	...
	if(dofat) {
		if(commit) {
			...
			print("type %s, %d tracks, %d heads, %d sectors/track, "
			      "%lld bytes/sec\n",
			      t->name, t->tracks, t->heads, t->sectors,
			      secsize);
		}
		 ...
		 * over after you subtract out the space taken up by the fat
		 * tables.
	...
	Tryagain:
		 ...
		 * but of course we don't know yet.  maybe iterating will get us
		 * there.
		 * or maybe it will cycle.
		 */
		clusters = 0;
		for(i = 0;; i++) {
			fatsecs = (fatbits * clusters + 8 * secsize - 1) /
			          (8 * secsize);
			...
			if(rootfiles > 512) {
				rootfiles = 512;
				rootsecs =
				    rootfiles / (secsize / sizeof(Dosdir));
			}
			data = nresrv + 2 * fatsecs +
			       (rootfiles * sizeof(Dosdir) + secsize - 1) /
			           secsize;
			...
			if(i > 10)
				fatal("can't decide how many clusters to use "
				      "(%d? %d?)",
				      clusters, newclusters);
			if(chatty)
				print("clusters %d\n", clusters);
		...
		if(chatty)
			print("try %d fatbits => %d clusters of %d\n", fatbits,
...
clustalloc(int flag)
{
	...
	if(flag == Eof)
		...
		++fatlast;
		if(fatlast >= clusters)
			sysfatal("data does not fit on disk (%d %d)", fatlast,
			         clusters);
		return fatlast;
	}
}
