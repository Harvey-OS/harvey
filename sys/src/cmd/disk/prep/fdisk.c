...
{
	fprint(2,
	       "usage: disk/fdisk [-abfprvw] [-s sectorsize] /dev/sdC0/data\n");
	exits("usage");
}
...
struct Tentry {
	...
	uint8_t
	    xlba[4]; /* starting LBA from beginning of disc or ext. partition */
...
enum { Active = 0x80,  /* partition is active */
...
static Type types[256] = {
    [TypeEMPTY] { "EMPTY", "" }, [TypeFAT12] { "FAT12", "dos" },
    [TypeFAT16] { "FAT16", "dos" }, [TypeFAT32] { "FAT32", "dos" },
    [TypeFAT32LBA] { "FAT32LBA", "dos" }, [TypeFAT16X] { "FAT16X", "dos" },
    [TypeEXTHUGE] { "EXTHUGE", "" }, [TypeIBMRecovery] { "IBMRECOVERY", "ibm" },
    [TypeEXTENDED] { "EXTENDED", "" }, [TypeFATHUGE] { "FATHUGE", "dos" },
    [TypeBB] { "BB", "bb" },

    [TypeXENIX] { "XENIX", "xenix" },
    [TypeXENIXUSR] { "XENIX USR", "xenixusr" }, [TypeHPFS] { "HPFS", "ntfs" },
    [TypeAIXBOOT] { "AIXBOOT", "aixboot" },
    [TypeAIXDATA] { "AIXDATA", "aixdata" },
    [TypeOS2BOOT] { "OS/2BOOT", "os2boot" },
    [TypeUNFORMATTED] { "UNFORMATTED", "" }, [TypeHPFS2] { "HPFS2", "hpfs2" },
    [TypeCPM0] { "CPM0", "cpm0" }, [TypeDMDDO] { "DMDDO", "dmdd0" },
    [TypeGB] { "GB", "gb" }, [TypeSPEEDSTOR] { "SPEEDSTOR", "speedstor" },
    [TypeSYSV386] { "SYSV386", "sysv386" },
    [TypeNETWARE] { "NETWARE", "netware" }, [TypePCIX] { "PCIX", "pcix" },
    [TypeMINIX13] { "MINIXV1.3", "minix13" },
    [TypeMINIX] { "MINIXV1.5", "minix15" },
    [TypeLINUXSWAP] { "LINUXSWAP", "linuxswap" },
    [TypeLINUX] { "LINUX", "linux" }, [TypeLINUXEXT] { "LINUXEXTENDED", "" },
    [TypeLINUXLVM] { "LINUXLVM", "linuxlvm" },
    [TypeAMOEBA] { "AMOEBA", "amoeba" },
    [TypeAMOEBABB] { "AMOEBABB", "amoebaboot" },
    [TypeBSD386] { "BSD386", "bsd386" }, [TypeNETBSD] { "NETBSD", "netbsd" },
    [TypeBSDI] { "BSDI", "bsdi" }, [TypeBSDISWAP] { "BSDISWAP", "bsdiswap" },
    [TypeOTHER] { "OTHER", "other" }, [TypeCPM] { "CPM", "cpm" },
    [TypeDellRecovery] { "DELLRECOVERY", "dell" },
    [TypeSPEEDSTOR12] { "SPEEDSTOR12", "speedstor" },
    [TypeSPEEDSTOR16] { "SPEEDSTOR16", "speedstor" },
    [TypeEFIProtect] { "EFIPROTECT", "efiprotect" }, [TypeEFI] { "EFI", "efi" },
...
{
	if(seek(disk->fd, (int64_t)sec * disk->secsize + off, 0) !=
	   (int64_t)sec * disk->secsize + off)
...
{
	written = 1;
	if(seek(disk->wfd, (int64_t)sec * disk->secsize + off, 0) !=
	   (int64_t)sec * disk->secsize + off)
		goto Error;
...
mkpart(char* name, int primary, int64_t lba, int64_t size, Tentry* t)
{
	...
	if(p->lba != lba)
		fprint(2,
		       "%s: start of partition (%lld) won't fit in MBR table\n",
		       argv0, lba);
	p->size = size;
	if(p->size != size)
		fprint(2,
		       "%s: size of partition (%lld) won't fit in MBR table\n",
		       argv0, size);
	p->primary = primary;
	return p;
}
...
recover(Edit* edit)
{
	...
	err = 0;
	for(i = 0; i < nrtab; i++)
		if(diskwrite(edit->disk, &rtab[i].table, Tablesz, rtab[i].lba,
		             Toffset) < 0)
			err = 1;
	if(err) {
		fprint(2, "warning: some writes failed during restoration of "
	...
	if(ctlfd >= 0) {
		for(i = 0; i < edit->npart; i++)
			if(edit->part[i]->ctlname &&
			   fprint(ctlfd, "delpart %s", edit->part[i]->ctlname) <
			       0)
				fprint(2, "delpart failed: %s: %r",
				       edit->part[i]->ctlname);
		for(i = 0; i < edit->nctlpart; i++)
			if(edit->part[i]->name &&
			   fprint(ctlfd, "delpart %s", edit->ctlpart[i]->name) <
			       0)
				fprint(2, "delpart failed: %s: %r",
				       edit->ctlpart[i]->name);
		for(i = 0; i < edit->nctlpart; i++) {
			if(fprint(ctlfd, "part %s %lld %lld",
			          edit->ctlpart[i]->name,
			          edit->ctlpart[i]->start + offset,
			          edit->ctlpart[i]->end + offset) < 0) {
				fprint(2, "restored disk partition table but "
				          "not kernel; reboot\n");
				exits("inconsistent");
			}
		}
...
rdpart(Edit* edit, uint64_t lba, uint64_t xbase)
{
	...
	for(tp = table.entry, ep = tp + NTentry; tp < ep && npart < Mpart;
	    tp++) {
		switch(tp->type) {
		case TypeEMPTY:
			break;
		...
		case TypeLINUXEXT:
			...
			break;
		default:
			p = mkpart(nil, lba == 0, lba + getle32(tp->xlba),
			           getle32(tp->xsize), tp);
...
autopart(Edit* edit)
{
	...
	if(bigsize < 1) {
		fprint(2, "couldn't find space or partition slot for plan 9 "
		          "partition\n");
		return;
	}
	...
	active = Active;
	for(i = 0; i < edit->npart; i++)
		if(((Dospart*)edit->part[i])->primary &&
		   (((Dospart*)edit->part[i])->active & Active))
			active = 0;
...
cmdsum(Edit* edit, Part* vp, int64_t a, int64_t b)
{
	...
	if(div == 1)
		print("%s %-12s %*lld %-*lld (%lld cylinders, %lld %s) %s\n",
		      buf, name, edit->disk->width, a, edit->disk->width, b,
		      b - a, sz, suf, ty);
	else
		print(
		    "%s %-12s %*lld %-*lld (%lld cylinders, %lld.%.2d %s) %s\n",
		    buf, name, edit->disk->width, a, edit->disk->width, b,
		    b - a, sz / div, (int)(((sz % div) * 100) / div), suf, ty);
}
...
static char* help = "A name - set partition active\n"
...
static int
wrextend(Edit* edit, int i, int64_t xbase, int64_t startlba, int64_t* endlba)
{
	int ni;
	Table table;
	Tentry* tp, *ep;
	...
	Finish:
		if(startlba < *endlba) {
			disk = edit->disk;
			diskread(disk, &table, Tablesz, mbroffset + startlba,
			         Toffset);
			tp = table.entry;
			ep = tp + NTentry;
			for(; tp < ep; tp++)
			...
			if(diskwrite(edit->disk, &table, Tablesz,
			             mbroffset + startlba, Toffset) < 0)
				recover(edit);
		}
		return i;
	}
	...
	*tp = p->Tentry;
	wrtentry(disk, tp, p->type, startlba, startlba + disk->s,
	         p->end * sec2cyl);
	tp++;

	if(p->end * sec2cyl != *endlba) {
		memset(tp, 0, sizeof *tp);
		wrtentry(disk, tp, TypeEXTENDED, xbase, p->end * sec2cyl,
		         *endlba);
		tp++;
	}
	...
	if(diskwrite(edit->disk, &table, Tablesz, mbroffset + startlba,
	             Toffset) < 0)
		recover(edit);
	return ni;
}
	...
	for(i = 0; i < edit->npart && tp < ep;) {
		...
		if(p->primary) {
			*tp = p->Tentry;
			...
			ni = wrextend(edit, i, p->start * sec2cyl,
			              p->start * sec2cyl, &endlba);
			memset(tp, 0, sizeof *tp);
