 ...
 */
static void
_writedirs(Cdimg* cd, Direc* d, int (*put)(Cdimg*, Direc*, int, int, int),
           int level)
{
	int i, l, ll;
	uint32_t start, next;
 ...
 */
static void
_writedumpdirs(Cdimg* cd, Direc* d, int (*put)(Cdimg*, Direc*, int, int, int),
               int level)
{
	int i;
	uint32_t start;
	...
	case 0:
		...
		for(i = 0; i < d->nchild; i++)
			if(d->child[i].mode & DMDIR)
				_writedumpdirs(cd, &d->child[i], put,
				               level + 1);
