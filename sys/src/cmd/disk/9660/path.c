...
static u32
writepathtable(Cdimg *cd, u32 vdblock, int size)
{
	int rp, wp;
	u8 buf[Blocksize];
	u32 bk, i, *len, n;
	...
	wp = 0;
	len = nil;
	start = (i64)cd->nextblock * Blocksize;
	Cwseek(cd, start);
	Crseek(cd, start);
...
static void
writepathtablepair(Cdimg *cd, u32 vdblock)
{
	u32 bloc, lloc, sz, sz2;
