...
opencd(char *file, Cdinfo info)
{
	...
	if(strstr(cd->iso.systemid, "iso9660") == nil && strstr(cd->iso.systemid, "utf8") == nil) {
	 ...
	 char *(*cvtname)(uint8_t *, int))
{
	...
	/* plan 9 use fields */
	if((cd->flags & CDplan9) && cvtname == isostring && (c->namelen != 1 || c->name[0] > 1)) {
		p = buf + 33 + c->namelen;
		if((p - buf) & 1)
			p++;
		assert(p < buf + c->len);
	     ...
	     uint32_t bloc)
{
	...
	Cputnm(cd, bloc, 4);
	Cputnm(cd, 0, 4);
	assert(Cwoffset(cd) == (int64_t)block * Blocksize + offsetof(Cvoldesc, rootdir[0]));
}
...
readjolietdesc(Cdimg *cd, Voldesc *v)
{
	...
	for(i = 16; i < 24; i++) {
		...
		if(memcmp(cv.magic, magic, sizeof magic) != 0)
			continue;
		if(cv.charset[0] != 0x25 || cv.charset[1] != 0x2F || (cv.charset[2] != 0x40 && cv.charset[2] != 0x43 && cv.charset[2] != 0x45))
			continue;
		break;
	}
