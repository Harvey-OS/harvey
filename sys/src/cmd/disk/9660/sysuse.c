           ...
           int dowrite)
{
	...
	for(; p[0] != '\0'; p = q) {
		...
		ensurespace(cd, 5 + strlen(p), cp, nil,
		            dowrite); /* nil: better not use this. */
		Cputrripname(cd, nm, flags | (q[0] ? NMcontinue : 0), p,
		             dowrite);
	}
	return cp;
}
...
Cputsysuse(Cdimg* cd, Direc* d, int dot, int dowrite, int initlen)
{
	...
	if(what & RR_SL) {
		for(path = d->symlink; path[0] != '\0'; path = nextpath) {
			...
			do {
				...
				ensurespace(cd, 7 + strlen(p), cp, nil,
				            dowrite);
				CputrripSL(cd, nextpath[0],
				           flags | (q[0] ? NMcontinue : 0), p,
				           dowrite);
				p = q;
			} while(p[0] != '\0');
		}
	...
	if(what & RR_TF) {
		m = CputrripTF(
		    cd, d, TFcreation | TFmodify | TFaccess | TFattributes, 0);
		cp = ensurespace(cd, m, cp, &cn, dowrite);
		CputrripTF(cd, d,
		           TFcreation | TFmodify | TFaccess | TFattributes,
		           dowrite);
	}
	assert(dowrite == 0 || cp != &co ||
	       Cwoffset(cd) == o + co.len - initlen);
