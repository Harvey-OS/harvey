...
main(int argc, char** argv)
{
	...
	case 'o':
		...
		if(blocksize % Blocksize)
			sysfatal(
			    "bad block size %d -- must be multiple of 2048",
			    blocksize);
		blocksize /= Blocksize;
		break;
	case 'p':
	...
	Dofix:
		 ...
		 * overwritten at the next dump.  Because of this, it needs to
		 * be
		 * reconstructable.  We reconstruct the _conform.map and dump
		 * trees
	...
	if(cd->flags & (CDconform | CDjoliet)) {
		...
		if(length) {
			/* The ISO9660 name will get turned into uppercase when
			 * written. */
			if((iconform = walkdirec(&idumproot, "_conform.map")) ==
			   nil)
				iconform =
				    adddirec(&idumproot, "_conform.map", &dir);
			jconform = nil;
			if(cd->flags & CDjoliet) {
				if((jconform = walkdirec(
				        &jdumproot, "_conform.map")) == nil)
					jconform = adddirec(
					    &jdumproot, "_conform.map", &dir);
			}
	...
	if(mk9660) {
		 ...
		 */
		convertnames(&idumproot, (info.flags & CDconform)
		                             ? (void*)struprcpy
		                             : (void*)strcpy);
		if(cd->nulldump) {
		...
		setvolsize(cd, cd->iso9660pvd, cd->nextblock);
		if(cd->flags & CDjoliet) {
			setroot(cd, cd->jolietsvd, jdumproot.block,
			        jdumproot.length);
			setvolsize(cd, cd->jolietsvd, cd->nextblock);
		}
	}
	...
	if(!mk9660) {
		/*
		 * If we've gotten too big, truncate back to what we started
		 * with,
		...
		if(cd->nulldump) {
			...
			sprint(buf + strlen(buf),
			       "%s %lud %lud %lud %llud %lud %lud", dumpname,
			       now, newnull, cblock, clength, iroot.block,
			       iroot.length);
			if(cd->flags & CDjoliet)
