{2:}program VFtoVP(vffile,tfmfile,vplfile,output);label{3:}9999;{:3}
const{4:}tfmsize=30000;vfsize=10000;maxfonts=300;ligsize=5000;
hashsize=5003;maxstack=50;{:4}{144:}charcodeascii=0;charcodeoctal=1;
charcodedefault=2;{:144}type{5:}byte=0..255;{:5}{22:}
indextype=0..tfmsize;{:22}{143:}
charcodeformattype=charcodeascii..charcodedefault;{:143}var{7:}
vffile:packed file of byte;{:7}{10:}tfmfile:packed file of byte;{:10}
{12:}lf,lh,bc,ec,nw,nh,nd,ni,nl,nk,ne,np:0..32767;{:12}{20:}
vplfile:text;{:20}{23:}tfm:array[-1000..tfmsize]of byte;{:23}{26:}
...
b0,b1,b2,b3:byte;fontlh:0..32767;fontbc,fontec:0..32767;{:37}{42:}
defaultdirectory:packed array[1..9]of char;{:42}{50:}
ASCII04,ASCII10,ASCII14:cstring;xchr:packed array[0..255]of char;
MBLstring,RIstring,RCEstring:cstring;{:50}{52:}dig:array[0..11]of 0..9;
{:52}{55:}level:0..5;{:55}{68:}charsonline:0..8;perfect:boolean;{:68}
{70:}i:0..32767;c:0..256;d:0..3;k:indextype;r:0..65535;{:70}{86:}
labeltable:array[0..258]of record cc:0..256;rr:0..ligsize;end;
labelptr:0..257;sortptr:0..257;boundarychar:0..256;bcharlabel:0..32767;
{:86}{88:}activity:array[0..ligsize]of 0..2;ai,acti:0..ligsize;{:88}
{112:}hash:array[0..hashsize]of 0..66048;
...
vflimit:0..vfsize;o:byte;{:124}{140:}verbose:cinttype;{:140}{145:}
charcodeformat:charcodeformattype;{:145}{148:}
vfname,tfmname,vplname:cstring;{:148}{136:}procedure parsearguments;
...
longoptions[currentoption].val:=0;currentoption:=currentoption+1;{:137}
{138:}longoptions[currentoption].name:='version';
longoptions[currentoption].hasarg:=0;longoptions[currentoption].flag:=0;
longoptions[currentoption].val:=0;currentoption:=currentoption+1;{:138}
{139:}longoptions[currentoption].name:='verbose';
longoptions[currentoption].hasarg:=0;
longoptions[currentoption].flag:=addressof(verbose);
longoptions[currentoption].val:=1;currentoption:=currentoption+1;{:139}
{142:}longoptions[currentoption].name:='charcode-format';
longoptions[currentoption].hasarg:=1;longoptions[currentoption].flag:=0;
longoptions[currentoption].val:=0;currentoption:=currentoption+1;{:142}
{147:}longoptions[currentoption].name:=0;
longoptions[currentoption].hasarg:=0;longoptions[currentoption].flag:=0;
...
end else if getoptreturnval=63 then begin usage(1,'vftovp');
...
charcodeoctal else writeln(stderr,'Bad character code format',optarg,'.'
...
usage(1,'vftovp');end;vfname:=cmdline(optind);
...
begin kpsesetprogname(argv[0]);kpseinitprog('VFTOVP',0,nil,nil);
parsearguments;{11:}vffile:=kpseopenfile(vfname,kpsevfformat);
tfmfile:=kpseopenfile(tfmname,kpsetfmformat);
if verbose then begin write('This is VFtoVP, Version 1.2');
writeln(versionstring);end;{:11}{21:}
if optind+3>argc then begin vplfile:=stdout;
end else begin vplname:=extendfilename(cmdline(optind+2),'vpl');
rewrite(vplfile,vplname);end;{:21}{51:}
ASCII04:='  !"#$%&''()*+,-./0123456789:;<=>?';
...
RIstring:=' RI ';RCEstring:=' RCE';{:51}{56:}level:=0;{:56}{69:}
charsonline:=0;perfect:=true;{:69}{87:}boundarychar:=256;
bcharlabel:=32767;labelptr:=0;labeltable[0].rr:=0;{:87}end;{:2}{38:}
...
if eof(tfmfile)then b3:=0 else read(tfmfile,b3);end;{:38}{46:}
...
a:=256*a+b;k:=k-1;end;vfread:=a;end;{:46}{48:}
function tfmwidth(c:byte):integer;var a:integer;k:indextype;
begin k:=4*(widthbase+tfm[4*(charbase+c)]);a:=tfm[k];
if a>=128 then a:=a-256;
tfmwidth:=((256*a+tfm[k+1])*256+tfm[k+2])*256+tfm[k+3];end;{:48}{53:}
procedure outdigs(j:integer);begin repeat j:=j-1;
write(vplfile,dig[j]:1);until j=0;end;procedure printdigs(j:integer);
begin repeat j:=j-1;write(dig[j]:1);until j=0;end;{:53}{54:}
procedure printoctal(c:byte);var j:0..2;begin write('''');
for j:=0 to 2 do begin dig[j]:=c mod 8;c:=c div 8;end;printdigs(3);end;
{:54}{57:}procedure outln;var l:0..5;begin writeln(vplfile);
for l:=1 to level do write(vplfile,'   ');end;procedure left;
begin level:=level+1;write(vplfile,'(');end;procedure right;
begin level:=level-1;write(vplfile,')');outln;end;{:57}{58:}
procedure outBCPL(k:indextype);var l:0..39;begin write(vplfile,' ');
l:=tfm[k];while l>0 do begin k:=k+1;l:=l-1;write(vplfile,xchr[tfm[k]]);
end;end;{:58}{59:}procedure outoctal(k,l:indextype);var a:0..1023;
b:0..32;j:0..11;begin write(vplfile,' O ');a:=0;b:=0;j:=0;
...
,' C ',xchr[c])else begin tfm[0]:=c;outoctal(0,1);end;end;{:61}{62:}
procedure outface(k:indextype);var s:0..1;b:0..8;
...
repeat dig[j]:=a mod 10;a:=a div 10;j:=j+1;until a=0;outdigs(j);end{:64}
;{65:}begin write(vplfile,'.');f:=10*f+5;delta:=10;
repeat if delta>1048576 then f:=f+524288-(delta div 2);
write(vplfile,f div 1048576:1);f:=10*(f mod 1048576);delta:=delta*10;
until f<=delta;end;{:65};end;{:63}{75:}
procedure checkBCPL(k,l:indextype);var j:indextype;c:byte;
...
key:integer;t:integer;begin if hashptr=hashsize then goto 10;{116:}
...
ligz[h]:=zz;hashptr:=hashptr+1;hashlist[hashptr]:=h;10:end;{:115}{117:}
ifdef('notdef')function ligf(h,x,y:indextype):indextype;begin end;
endif('notdef')function eval(x,y:indextype):indextype;var key:integer;
begin key:=256*x+y+1;h:=(1009*key)mod hashsize;
while hash[h]>key do if h>0 then h:=h-1 else h:=hashsize;
if hash[h]<key then eval:=y else eval:=ligf(h,x,y);end;{:117}{118:}
...
45:stringbalance:=false;10:end;{:119}{121:}
...
getbytes:=a;end;{:126}{132:}function vfinput:boolean;label 9999,10;
...
writeln('pt'){:36};end;{39:}fontchars[fontptr]:=vfptr;{44:}{:44}{45:}
...
tfmname:=kpsefindtfm(curname);free(curname);
if not tfmname then writeln(stderr,
'---not loaded, TFM file can''t be opened!')else begin resetbin(tfmfile,
tfmname);free(tfmname);fontbc:=0;fontec:=256;readtfmword;
...
function organize:boolean;label 9999,10;var tfmptr:indextype;begin{24:}
...
organize:=vfinput;goto 10;9999:organize:=false;10:end;{:132}{133:}
...
end{:120};{71:}begin fonttype:=0;if lh>=12 then begin{76:}
...
outln{:74};{72:}left;write(vplfile,'CHECKSUM');outoctal(24,4);right{:72}
;{80:}if(lh>17)and(tfm[92]>127)then begin left;
write(vplfile,'SEVENBITSAFEFLAG TRUE');right;end{:80};end{:71};{81:}
...
writeln('I have set it to zero.');end;{84:}
...
outfix(4*(parambase+i));right;end{:83};right;end;{82:}
...
outfix(8);right;right;end{:122};{85:}
...
writeln('I have set it to zero.');end;{:85};end;{:133}{134:}
...
vfptr:=vfptr+1;
if((o<=127))or((o>=128)and(o<=131))or((o>=133)and(o<=136))then begin if
o>=128 then if o>=133 then c:=getbytes(o-132,false)else c:=getbytes(o
-127,false)else c:=o;if f=fontptr then begin perfect:=false;
if charsonline>0 then writeln(' ');charsonline:=0;
...
write(vplfile,')');outln;end;{:128}{129:}
...
repeat write(vplfile,'(POP)');top:=top-1;until top=0;end;right;end{:125}
;domap:=true;goto 10;9999:domap:=false;10:end;
function docharacters:boolean;label 9999,10;var c:byte;k:indextype;
ai:0..ligsize;begin{101:}sortptr:=0;
...
if(tfm[4*(charbase+c)+1]div 16)>0 then{103:}
...
if(tfm[4*(charbase+c)+1]mod 16)>0 then{104:}
...
if(tfm[4*(charbase+c)+2]div 4)>0 then{105:}
...
end else if tfm[k+2]>=128 then{99:}
...
write(vplfile,' R 0.0');end else outfix(4*(kernbase+r));right;end{:99}
else{100:}
begin if((tfm[k+1]<bc)or(tfm[k+1]>ec)or(tfm[4*(charbase+tfm[k+1])]=0))
then if tfm[k+1]<>boundarychar then begin perfect:=false;
if charsonline>0 then writeln(' ');charsonline:=0;
...
if tfm[k]>0 then if level=1 then{98:}
...
if tfm[k]>=128 then i:=nl else i:=i+1+tfm[k];until i>=nl;right;end{:106}
;2:{107:}begin r:=tfm[4*(charbase+c)+3];
...
outchar(tfm[4*(charbase+c)+3]);right;end;end;end{:107};3:{108:}
...
tfm[4*(charbase+c)+2]:=4*(tfm[4*(charbase+c)+2]div 4)+0;
end else begin left;write(vplfile,'VARCHAR');outln;{109:}
...
if not organize then goto 9999;dosimplethings;{89:}
if nl>0 then begin for ai:=0 to nl-1 do activity[ai]:=0;{92:}
...
end;activity[nl-1]:=1;end{:92};end;{90:}
...
tfm[4*(charbase+c)+2]:=4*(tfm[4*(charbase+c)+2]div 4)+0;end else{91:}
...
labeltable[sortptr+1].rr:=r;labelptr:=labelptr+1;activity[r]:=2;end{:91}
;end;labeltable[labelptr+1].rr:=ligsize;{:90};if nl>0 then begin left;
write(vplfile,'LIGTABLE');outln;{93:}
...
end else activity[r]:=2;end;end{:93};{94:}sortptr:=1;
for acti:=0 to nl-1 do if activity[acti]<>1 then begin i:=acti;{96:}
if activity[i]=0 then begin if level=1 then begin left;
write(vplfile,'COMMENT THIS PART OF THE PROGRAM IS NEVER USED!');outln;
end end else if level=2 then right{:96};{95:}
...
outchar(labeltable[sortptr].cc);right;sortptr:=sortptr+1;end{:95};{97:}
...
end;if yligcycle<256 then begin write(
...
write(vplfile,'SO THE DATA HAS BEEN CHANGED!)');end;9999:end.{:135}
