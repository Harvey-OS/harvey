{4:}program PKtype(input,output);type{9:}ASCIIcode=32..126;{:9}{10:}
...
pkfile:bytefile;{:31}{33:}pkname:^char;curloc:integer;{:33}{37:}
...
turnon:boolean;hbit:integer;count:integer;{:51}{56:}
...
longoptions[currentoption].val:=0;currentoption:=currentoption+1;{:57}
{58:}longoptions[currentoption].name:='version';
longoptions[currentoption].hasarg:=0;longoptions[currentoption].flag:=0;
longoptions[currentoption].val:=0;currentoption:=currentoption+1;{:58}
{59:}longoptions[currentoption].name:=0;
longoptions[currentoption].hasarg:=0;longoptions[currentoption].flag:=0;
...
end else if getoptreturnval='?'then begin usage(1,'pktype');
...
'pktype: Need exactly one file argument.');usage(1,'pktype');end;end;
{:56}procedure initialize;var i:integer;begin kpsesetprogname(argv[0]);
...
procedure openpkfile;
begin pkfile:=kpseopenfile(cmdline(1),kpsepkformat);curloc:=0;end;{:32}
...
if getbyte<>247 then begin writeln(output,
...
if hppp<>vppp then writeln(output,'Warning:  aspect ratio not 1:1!'){:38
