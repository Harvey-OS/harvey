{3:}program DVItype(dvifile,output);label{4:}9999,30;{:4}const{5:}
maxfonts=500;maxwidths=25000;linelength=79;terminallinelength=150;
stacksize=100;namesize=10000;{:5}type{8:}ASCIIcode=0..255;{:8}{9:}
...
curname:^char;{:24}{25:}b0,b1,b2,b3:eightbits;{:25}{30:}
...
startvals:0..9;count:array[0..9]of integer;{:42}{57:}
...
k,m,n,p,q:integer;{:108}{122:}showopcodes:cinttype;{:122}{112:}
procedure parsearguments;const noptions=8;
...
longoptions[currentoption].val:=0;currentoption:=currentoption+1;{:113}
{114:}longoptions[currentoption].name:='version';
longoptions[currentoption].hasarg:=0;longoptions[currentoption].flag:=0;
longoptions[currentoption].val:=0;currentoption:=currentoption+1;{:114}
{115:}longoptions[currentoption].name:='output-level';
...
{118:}longoptions[currentoption].name:='max-pages';
longoptions[currentoption].hasarg:=1;longoptions[currentoption].flag:=0;
longoptions[currentoption].val:=0;currentoption:=currentoption+1;
maxpages:=1000000;{:118}{119:}longoptions[currentoption].name:='dpi';
longoptions[currentoption].hasarg:=1;longoptions[currentoption].flag:=0;
longoptions[currentoption].val:=0;currentoption:=currentoption+1;
resolution:=300.0;{:119}{120:}
...
end else if getoptreturnval=63 then begin usage(1,'dvitype');
...
printversionandexit('This is DVItype, Version 3.6',nil,'D.E. Knuth');
end else if(strcmp(longoptions[optionindex].name,'output-level')=0)then
begin outmode:=atou(optarg);
if(outmode=0)or(outmode>4)then begin writeln(stderr,
'Value for --output-level must be >= 1 and <= 4.');uexit(1);end;
...
'dvitype: More than ten count registers specified.');uexit(1);end;
m:=m+1;
end else begin startcount[k]:=strtol(optarg+m,addressof(endnum),10);
if endnum=optarg+m then begin writeln(stderr,
'dvitype: -page-start values must be numeric or *.');uexit(1);end;
...
'dvitype: Need exactly one file argument.');usage(1,'dvitype');end;end;
{:112}procedure initialize;var i:integer;begin kpsesetprogname(argv[0]);
parsearguments;write(stdout,'This is DVItype, Version 3.6');
...
xchr[126]:='~';for i:=127 to 255 do xchr[i]:='?';{:11}{12:}
...
oldbackpointer:=-1;started:=false;{:98}end;{:3}{23:}
procedure opendvifile;
begin curname:=extendfilename(cmdline(optind),'dvi');
resetbin(dvifile,curname);curloc:=0;end;procedure opentfmfile;
var fullname:^char;begin fullname:=kpsefindtfm(curname);
...
function dvilength:integer;begin xfseek(dvifile,0,2,'dvitype');
curloc:=xftell(dvifile,'dvitype');dvilength:=curloc;end;
procedure movetobyte(n:integer);begin xfseek(dvifile,n,0,'dvitype');
...
then match:=false;startmatch:=match;end;{:44}{59:}
procedure definefont(e:integer);var f:0..maxfonts;p:integer;n:integer;
c,q,d,m:integer;r:0..namesize;j,k:0..namesize;mismatch:boolean;
...
r:=fontname[nf+1]-fontname[nf];curname:=xmalloc(r+1);
...
;end;if outmode=0 then writeln(stdout,' ');
if tfmfile then xfclose(tfmfile,curname);free(curname);end{:62}else{60:}
...
showing:=true;write(stdout,a:1,': ','down',o-156:1,' ',p:1);
...
flushtext;showing:=true;write(stdout,a:1,': ','fntnum',p:1);
...
showing:=true;write(stdout,a:1,': ','set',o-127:1,' ',p:1);
if showopcodes and(o>=128)then write(stdout,' {',o:1,'}');end;goto 41;
...
9998:writeln(stdout,'!');dopage:=false;9999:end;{:79}{95:}{99:}
...
repeat if eof(dvifile)then begin writeln(stderr,'Bad DVI file: ',
'the file ended prematurely','!');uexit(1);end;k:=getbyte;
...
end;others:end;until k=140;{:96};bopseen:=false;end;9999:end;{:95}{103:}
...
end;{:103}{107:}begin initialize;{56:}
writeln(stdout,'Options selected:');write(stdout,'  Starting page = ');
...
printreal(newmag/1000.0,8,3);writeln(stdout,'')end{:56};{109:}
opendvifile;p:=getbyte;
