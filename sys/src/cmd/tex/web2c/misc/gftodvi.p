...
dvibufsize=800;widestrow=8192;liglookahead=20;{:5}type{9:}
...
nameoffile:^ASCIIcode;{:48}{49:}b0,b1,b2,b3:eightbits;{:49}{53:}
...
{:220}{226:}verbose:cinttype;{:226}{229:}overflowlabeloffset:integer;
offsetinpoints:real;{:229}{222:}procedure parsearguments;
...
longoptions[currentoption].val:=0;currentoption:=currentoption+1;{:223}
{224:}longoptions[currentoption].name:='version';
longoptions[currentoption].hasarg:=0;longoptions[currentoption].flag:=0;
longoptions[currentoption].val:=0;currentoption:=currentoption+1;{:224}
{225:}longoptions[currentoption].name:='verbose';
longoptions[currentoption].hasarg:=0;
longoptions[currentoption].flag:=addressof(verbose);
longoptions[currentoption].val:=1;currentoption:=currentoption+1;{:225}
{228:}longoptions[currentoption].name:='overflow-label-offset';
longoptions[currentoption].hasarg:=1;longoptions[currentoption].flag:=0;
longoptions[currentoption].val:=0;currentoption:=currentoption+1;{:228}
{231:}longoptions[currentoption].name:=0;
longoptions[currentoption].hasarg:=0;longoptions[currentoption].flag:=0;
...
end else if getoptreturnval=63 then begin usage(1,'gftodvi');
...
printversionandexit('This is GFtoDVI, Version 3.0',nil,'D.E. Knuth');
...
until getoptreturnval=-1;if(optind+1<>argc)then begin writeln(stderr,
'gftodvi: Need exactly one file argument.');usage(1,'gftodvi');end;end;
{:222}procedure initialize;var i,j,m,n:integer;
begin kpsesetprogname(argv[0]);kpseinitprog('GFTODVI',0,nil,nil);
parsearguments;
...
xchr[123]:='{';xchr[124]:='|';xchr[125]:='}';xchr[126]:='~';{:13}{14:}
for i:=1 to 31 do xchr[i]:=chr(i);for i:=127 to 255 do xchr[i]:=chr(i);
...
for k:=1 to 5 do begin fontarea[k]:=0;fontat[k]:=0;end;{:97}{103:}
totalpages:=0;maxv:=0;maxh:=0;lastbop:=-1;{:103}{106:}
halfbuf:=dvibufsize div 2;dvilimit:=dvibufsize;dviptr:=0;dvioffset:=0;
{:106}{118:}dummyinfo.B0:=0;dummyinfo.B1:=0;dummyinfo.B2:=0;
dummyinfo.B3:=0;{:118}{126:}c[1]:=1;d[1]:=2;twotothe[0]:=1;m:=1;
for k:=1 to 13 do twotothe[k]:=2*twotothe[k-1];for k:=2 to 6 do{128:}
begin n:=twotothe[k-1];for j:=0 to n-1 do begin m:=m+1;c[m]:=m;
d[m]:=n+n;end;end{:128};for k:=7 to 12 do{129:}begin n:=twotothe[k-1];
...
5:begin writeln(stderr,'METAFONT logo!');uexit(1);end;end;30:end;{:58}
...
firststring(34);{:88};startgf;{221:}
if getbyte<>247 then begin writeln(stderr,'Bad GF file: ','No preamble',
