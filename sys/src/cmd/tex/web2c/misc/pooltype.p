{2:}program POOLtype(poolfile,output);type{5:}ASCIIcode=0..255;{:5}
var{7:}xord:array[ASCIIcode]of ASCIIcode;
xchr:array[ASCIIcode]of ASCIIcode;{:7}{12:}k,l:0..255;m,n:ASCIIcode;
s:integer;{:12}{13:}count:integer;{:13}{18:}
poolfile:packed file of ASCIIcode;poolname:^char;xsum:boolean;{:18}{21:}
...
longoptions[currentoption].val:=0;currentoption:=currentoption+1;{:22}
{23:}longoptions[currentoption].name:='version';
longoptions[currentoption].hasarg:=0;longoptions[currentoption].flag:=0;
longoptions[currentoption].val:=0;currentoption:=currentoption+1;{:23}
{24:}longoptions[currentoption].name:=0;
longoptions[currentoption].hasarg:=0;longoptions[currentoption].flag:=0;
...
end else if getoptreturnval='?'then begin usage(1,'pooltype');
...
printversionandexit('This is POOLtype, Version 3.0',nil,'D.E. Knuth');
end;until getoptreturnval=-1;
if(optind+1<>argc)then begin writeln(stderr,
'pooltype: Need exactly one file argument.');usage(1,'pooltype');end;
poolname:=extendfilename(cmdline(optind),'pool');
reset(poolfile,poolname);end;{:21}procedure initialize;var{6:}i:integer;
{:6}begin kpsesetprogname(argv[0]);parsearguments;{8:}xchr[32]:=' ';
...
'! I can''t read the POOL file.');uexit(1);end;repeat{20:}
if eof(poolfile)then begin writeln(stderr,
