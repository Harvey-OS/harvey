{3:}program GFtype(gffile,output);const{5:}linelength=79;maxrow=79;
maxcol=79;{:5}type{8:}ASCIIcode=32..126;{:8}{9:}
...
gffile:bytefile;{:21}{23:}curloc:integer;{:23}{25:}
wantsmnemonics:cinttype;wantspixels:cinttype;{:25}{35:}m,n:integer;
...
pixratio:real;{:62}{67:}a:integer;b,c,l,o,p,q,r:integer;{:67}{73:}
procedure parsearguments;const noptions=4;
...
longoptions[currentoption].val:=0;currentoption:=currentoption+1;{:74}
{75:}longoptions[currentoption].name:='version';
longoptions[currentoption].hasarg:=0;longoptions[currentoption].flag:=0;
longoptions[currentoption].val:=0;currentoption:=currentoption+1;{:75}
{76:}longoptions[currentoption].name:='mnemonics';
longoptions[currentoption].hasarg:=0;
longoptions[currentoption].flag:=addressof(wantsmnemonics);
longoptions[currentoption].val:=1;currentoption:=currentoption+1;{:76}
{77:}longoptions[currentoption].name:='images';
longoptions[currentoption].hasarg:=0;
longoptions[currentoption].flag:=addressof(wantspixels);
longoptions[currentoption].val:=1;currentoption:=currentoption+1;{:77}
{78:}longoptions[currentoption].name:=0;
longoptions[currentoption].hasarg:=0;longoptions[currentoption].flag:=0;
...
end else if getoptreturnval=63 then begin usage(1,'gftype');
...
printversionandexit('This is GFtype, Version 3.1',nil,'D.R. Fuchs');end;
until getoptreturnval=-1;if(optind+1<>argc)then begin writeln(stderr,
'gftype: Need exactly one file argument.');usage(1,'gftype');end;end;
{:73}procedure initialize;var i:integer;begin kpsesetprogname(argv[0]);
...
for i:=32 to 126 do xord[xchr[i]]:=i;{:12}{26:}{:26}{47:}
for i:=0 to 255 do charptr[i]:=-1;totalchars:=0;{:47}{63:}
minmoverall:=maxint;maxmoverall:=-maxint;minnoverall:=maxint;
maxnoverall:=-maxint;{:63}end;{:3}{22:}procedure opengffile;
begin gffile:=kpseopenfile(cmdline(optind),kpsegfformat);curloc:=0;{34:}
