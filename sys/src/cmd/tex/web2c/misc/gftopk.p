...
maxrow=100000;{:6}type{9:}ASCIIcode=32..126;{:9}{10:}
textfile=packed file of char;{:10}{37:}eightbits=0..255;
bytefile=packed file of eightbits;{:37}var{11:}
xord:array[char]of ASCIIcode;xchr:array[0..255]of char;{:11}{38:}
gffile:bytefile;pkfile:bytefile;{:38}{41:}pkloc:integer;gfloc:integer;
pkopen:boolean;{:41}{46:}bitweight:integer;outputbyte:integer;{:46}{48:}
gflen:integer;{:48}{49:}tfmwidth:array[0..255]of integer;
dx,dy:array[0..255]of integer;status:array[0..255]of 0..2;
row:array[0..maxrow]of integer;{:49}{56:}gfch:integer;
gfchmod256:integer;predpkloc:integer;maxn,minn:integer;
maxm,minm:integer;rowptr:integer;{:56}{79:}power:array[0..8]of integer;
{:79}{83:}comment:packed array[1..0]of char;{:83}{87:}checksum:integer;
designsize:integer;hmag:integer;i:integer;{:87}{92:}verbose:cinttype;
{:92}{95:}gfname,pkname,vplname:cstring;{:95}{88:}
...
longoptions[currentoption].val:=0;currentoption:=currentoption+1;{:89}
{90:}longoptions[currentoption].name:='version';
longoptions[currentoption].hasarg:=0;longoptions[currentoption].flag:=0;
longoptions[currentoption].val:=0;currentoption:=currentoption+1;{:90}
{91:}longoptions[currentoption].name:='verbose';
longoptions[currentoption].hasarg:=0;
longoptions[currentoption].flag:=addressof(verbose);
longoptions[currentoption].val:=1;currentoption:=currentoption+1;{:91}
{94:}longoptions[currentoption].name:=0;
longoptions[currentoption].hasarg:=0;longoptions[currentoption].flag:=0;
...
end else if getoptreturnval=63 then begin usage(1,'gftopk');
...
printversionandexit('This is GFtoPK, Version 2.3',nil,'Tomas Rokicki');
end;until getoptreturnval=-1;
if(optind+1<>argc)and(optind+2<>argc)then begin writeln(stderr,
'gftopk: Need one or two file arguments.');usage(1,'gftopk');end;
...
{:88}procedure initialize;var i:integer;begin kpsesetprogname(argv[0]);
kpseinitprog('GFTOPK',0,nil,nil);parsearguments;
if verbose then write(stdout,'This is GFtoPK, Version 2.3');
if verbose then writeln(stdout,versionstring);{12:}
...
for i:=32 to 126 do xord[xchr[i]]:=i;{:13}{42:}pkopen:=false;{:42}{50:}
for i:=0 to 255 do status[i]:=0;{:50}{80:}power[0]:=1;
for i:=1 to 8 do power[i]:=power[i-1]+power[i-1];{:80}end;{:4}{30:}
...
{39:}procedure opengffile;
begin gffile:=kpseopenfile(gfname,kpsegfformat);gfloc:=0;end;{:39}{40:}
procedure openpkfile;begin rewritebin(pkfile,pkname);pkloc:=0;
...
((a-256)*256+b)*256+c)*256+d;gfloc:=gfloc+4;end;{:43}{45:}
...
begin xfseek(gffile,0,2,'gftopk');gflength:=xftell(gffile,'gftopk');end;
procedure movetobyte(n:integer);begin xfseek(gffile,n,0,'gftopk');end;
{:47}{52:}{63:}procedure packandsendcharacter;var i,j,k:integer;{66:}
...
firston:boolean;flagbyte:integer;state:boolean;on:boolean;{:71}{78:}
...
repeatflag+1;rowptr:=i+1;end;i:=i+1;j:=j+1;end;end{:67};{68:}
...
pkword(height);pkword(xoffset);pkword(yoffset);end{:73}
else if(dx[gfch]>16777215)or(width>255)or(height>255)or(xoffset>127)or(
yoffset>127)or(xoffset<-128)or(yoffset<-128)or(compsize>1015)then{75:}
...
end else begin rcount:=count;ri:=i;ron:=on;end;{81:}
...
pkloc:=pkloc+1 end;end{:77}{:69};end{:63};procedure convertgffile;
var i,j,k:integer;gfcom:integer;{59:}dotherows:boolean;on:boolean;
state:boolean;extra:integer;bad:boolean;{:59}{62:}hppp,vppp:integer;
q:integer;postloc:integer;{:62}begin opengffile;
...
'Identification byte is incorrect','!');uexit(1);end;{61:}
...
tfmwidth[gfch]:=gfsignedquad;i:=gfsignedquad;status[gfch]:=1;end;{54:}
...
dotherows:=false;case gfcom of 67,68:{55:}
...
'no character locator for character ',gfch:1,'!');uexit(1);end;{58:}
...
rowptr]:=extra;rowptr:=rowptr+1;end;end;extra:=i;end;on:=not on;end{:60}
...
rowptr:=rowptr+1;end;end;if bad then begin writeln(stderr,
...
until gfcom=69;end{:58};end{:55};{54:}
239,240,241,242:begin begin putbyte(gfcom+1,pkfile);pkloc:=pkloc+1 end;
i:=0;for j:=0 to gfcom-239 do begin k:=gfbyte;begin putbyte(k,pkfile);
pkloc:=pkloc+1 end;i:=i*256+k;end;
for j:=1 to i do begin putbyte(gfbyte,pkfile);pkloc:=pkloc+1 end;end;
