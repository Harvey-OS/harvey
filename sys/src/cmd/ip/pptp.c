...
usage(void)
{
	fprint(2, "usage: ip/pptp [-Pd] [-k keyspec] [-x pppnetmntpt] [-w "
	          "window] server\n");
	exits("usage");
}
...
grereadproc(void* v)
{
	...
	while((n = read(grefd, pkt, sizeof pkt)) > 0) {
		...
		if(nhgets(pkt + 10) != GrePPP)
			myfatal("%I: gre read bad protocol 0x%x", remoteip,
			        nhgets(pkt + 10));
		...
		if(debug)
			fprint(2, "got gre callid %d len %d flag 0x%x pass %d "
			          "seq %d rseq %d\n",
			       nhgets(pkt + 8 + 6), len, flags, pass,
			       nhgetl(pkt + 8 + 8), rseq);
	}
	threadexits(nil);
}
...
pppreadproc(void* v)
{
	...
	while((n = read(topppfd, pkt + Hdr, sizeof pkt - Hdr)) > 0) {
		...
		rack = myrseq;
		if(debug)
			fprint(2, "wrote gre callid %d len %d flag 0x%x seq %d "
			          "rseq %d\n",
			       nhgets(pkt + 8 + 6), n, nhgets(pkt + 8),
...
dumpctlpkt(uint8_t* pkt)
{
	fprint(2, "pkt len %d mtype %d cookie 0x%.8ux type %d\n", nhgets(pkt),
	       nhgets(pkt + 2), nhgetl(pkt + 4), nhgets(pkt + 8));
	...
	default:
		...
		break;
	case Tstart:
		fprint(2, "\tTstart proto %d framing %d bearer %d maxchan %d "
		          "firmware %d\n",
		       nhgets(pkt + 12), nhgetl(pkt + 16), nhgetl(pkt + 20),
		       nhgets(pkt + 24), nhgets(pkt + 26));
		fprint(2, "\thost %.64s\n", (char*)pkt + 28);
		fprint(2, "\tvendor %.64s\n", (char*)pkt + 92);
		break;
	case Rstart:
		fprint(2, "\tRstart proto %d res %d err %d framing %d bearer "
		          "%d maxchan %d firmware %d\n",
		       nhgets(pkt + 12), pkt[14], pkt[15], nhgetl(pkt + 16),
		       nhgetl(pkt + 20), nhgets(pkt + 24), nhgets(pkt + 26));
		fprint(2, "\thost %.64s\n", (char*)pkt + 28);
		fprint(2, "\tvendor %.64s\n", (char*)pkt + 92);
		break;
	...
	case Tcallout:
		fprint(2, "\tTcallout id %d serno %d bps %d-%d\n",
		       nhgets(pkt + 12), nhgets(pkt + 14), nhgetl(pkt + 16),
		       nhgetl(pkt + 20));
		fprint(2, "\tbearer 0x%x framing 0x%x recvwin %d delay %d\n",
		       nhgetl(pkt + 24), nhgetl(pkt + 28), nhgets(pkt + 32),
		       nhgets(pkt + 34));
	...
	case Rcallout:
		fprint(2, "\tRcallout id %d peerid %d res %d err %d cause %d\n",
		       nhgets(pkt + 12), nhgets(pkt + 14), pkt[16], pkt[17],
		       nhgets(pkt + 18));
	...
	case Rcallreq:
		fprint(2, "\tRcallout id %d peerid %d res %d err %d recvwin %d "
		          "delay %d\n",
		       nhgets(pkt + 12), nhgets(pkt + 14), pkt[16], pkt[17],
		       nhgets(pkt + 18), nhgets(pkt + 20));
		break;

	case Acallcon:
		fprint(2, "\tAcallcon peerid %d connect %d recvwin %d delay %d "
		          "framing 0x%x\n",
		       nhgets(pkt + 12), nhgetl(pkt + 16), nhgets(pkt + 20),
		       nhgets(pkt + 22), nhgetl(pkt + 24));
		break;
	...
	case Acalldis:
		fprint(2, "\tAcalldis callid %d res %d err %d cause %d\n",
		       nhgets(pkt + 12), pkt[14], pkt[15], nhgets(pkt + 16));
		fprint(2, "\tstats %.128s\n", (char*)pkt + 20);
		break;
	...
	case Alinkinfo:
		fprint(2,
		       "\tAlinkinfo peerid %d sendaccm 0x%ux recvaccm 0x%ux\n",
		       nhgets(pkt + 12), nhgetl(pkt + 16), nhgetl(pkt + 20));
		break;
	}
}
