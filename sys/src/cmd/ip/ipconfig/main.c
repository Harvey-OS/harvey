 ...
 */
Option option[256] = {
    [OBmask] { "ipmask", Taddr }, [OBtimeoff] { "timeoff", Tulong },
    [OBrouter] { "ipgw", Taddrs }, [OBtimeserver] { "time", Taddrs },
    [OBnameserver] { "name", Taddrs }, [OBdnserver] { "dns", Taddrs },
    [OBlogserver] { "log", Taddrs }, [OBcookieserver] { "cookie", Taddrs },
    [OBlprserver] { "lpr", Taddrs }, [OBimpressserver] { "impress", Taddrs },
    [OBrlserver] { "rl", Taddrs }, [OBhostname] { "sys", Tstr },
    [OBbflen] { "bflen", Tulong }, [OBdumpfile] { "dumpfile", Tstr },
    [OBdomainname] { "dom", Tstr }, [OBswapserver] { "swap", Taddrs },
    [OBrootpath] { "rootpath", Tstr }, [OBextpath] { "extpath", Tstr },
    [OBipforward] { "ipforward", Taddrs }, [OBnonlocal] { "nonlocal", Taddrs },
    [OBpolicyfilter] { "policyfilter", Taddrs },
    [OBmaxdatagram] { "maxdatagram", Tulong }, [OBttl] { "ttl", Tulong },
    [OBpathtimeout] { "pathtimeout", Taddrs },
    [OBpathplateau] { "pathplateau", Taddrs }, [OBmtu] { "mtu", Tulong },
    [OBsubnetslocal] { "subnetslocal", Taddrs }, [OBbaddr] { "baddr", Taddrs },
    [OBdiscovermask] { "discovermask", Taddrs },
    [OBsupplymask] { "supplymask", Taddrs },
    [OBdiscoverrouter] { "discoverrouter", Taddrs },
    [OBrsserver] { "rs", Taddrs }, [OBstaticroutes] { "staticroutes", Taddrs },
    [OBtrailerencap] { "trailerencap", Taddrs },
    [OBarptimeout] { "arptimeout", Tulong },
    [OBetherencap] { "etherencap", Taddrs }, [OBtcpttl] { "tcpttl", Tulong },
    [OBtcpka] { "tcpka", Tulong }, [OBtcpkag] { "tcpkag", Tulong },
    [OBnisdomain] { "nisdomain", Tstr }, [OBniserver] { "ni", Taddrs },
    [OBntpserver] { "ntp", Taddrs }, [OBnetbiosns] { "netbiosns", Taddrs },
    ...
    [OBhomeagent] { "homeagent", Taddrs }, [OBsmtpserver] { "smtp", Taddrs },
    [OBpop3server] { "pop3", Taddrs }, [OBnntpserver] { "nntp", Taddrs },
    [OBwwwserver] { "www", Taddrs }, [OBfingerserver] { "finger", Taddrs },
    [OBircserver] { "irc", Taddrs }, [OBstserver] { "st", Taddrs },
    [OBstdaserver] { "stdar", Taddrs },

    [ODipaddr] { "ipaddr", Taddr }, [ODlease] { "lease", Tulong },
    [ODoverload] { "overload", Taddr }, [ODtype] { "type", Tbyte },
    [ODserverid] { "serverid", Taddr }, [ODparams] { "params", Tvec },
    [ODmessage] { "message", Tstr }, [ODmaxmsg] { "maxmsg", Tulong },
    [ODrenewaltime] { "renewaltime", Tulong },
    [ODrebindingtime] { "rebindingtime", Tulong },
    [ODvendorclass] { "vendorclass", Tvec }, [ODclientid] { "clientid", Tvec },
    [ODtftpserver] { "tftp", Taddr }, [ODbootfile] { "bootfile", Tstr },
};
...
char* verbs[] = {
        ...
        [Vether] "ether",       [Vgbe] "gbe",       [Vppp] "ppp",
        [Vloopback] "loopback", [Vaddpref6] "add6", [Vra6] "ra6",
        [Vtorus] "torus",       [Vtree] "tree",     [Vpkt] "pkt",
};
...
parse6ra(int argc, char** argv)
{
	...
	if(conf.maxraint < conf.minraint)
		sysfatal("maxraint %d < minraint %d", conf.maxraint,
		         conf.minraint);
}
...
doadd(int retry)
{
	...
	if(ipv6auto && !ppp) {
		...
		if(ip6cfg(0) < 0)
			sysfatal("can't start IPv6 on %s, address %I", conf.dev,
			         conf.laddr);
		//		return;
	}
...
doremove(void)
{
	...
	for(nifc = ifc; nifc != nil; nifc = nifc->next) {
		...
		for(lifc = nifc->lifc; lifc != nil; lifc = lifc->next) {
			...
			if(fprint(cfd, "remove %I %M", lifc->ip, lifc->mask) <
			   0)
...
binddevice(void)
{
	...
	if(strcmp(conf.type, "ppp") == 0)
		...
		if(fprint(conf.cfd, "bind %s %s", conf.type, conf.dev) < 0)
			sysfatal("%s: bind %s %s: %r", buf, conf.type,
			         conf.dev);
	} else {
...
dhcpsend(int type)
{
	...
	case Discover:
		...
		if(plan9) {
			n = snprint((char*)vendor, sizeof vendor, "plan9_%s",
			            conf.cputype);
...
dhcprecv(void)
{
	...
	case Ack:
		...
		/* get names */
		optgetstr(bp->optdata, OBhostname, conf.hostname,
...
int32_t jitter(void) /* compute small pseudo-random delay in ms */
{
...
optgetp9addrs(uint8_t* ap, int op, uint8_t* ip, int n)
{
	...
	for(i = 0; i < n && i < addrs && len > 0; i++) {
		...
		DEBUG("got plan 9 option %d addr %I (%s)", op,
		      &ip[i * IPaddrlen], p);
		p += slen;
		len -= slen;
	}
...
parsebootp(uint8_t* p, int n)
{
	...
	if(memcmp(optmagic, p, 4) != 0) {
		warning("parsebootp: bad opt magic %ux %ux %ux %ux", p[0], p[1],
		        p[2], p[3]);
		return nil;
	}
	p += 4;
