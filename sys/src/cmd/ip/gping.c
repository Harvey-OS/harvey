...
struct Graph {
	...
	int ndata;
	char* label;
	void (*newvalue)(Machine*, int32_t*, int32_t*, int32_t*);
...
int present[Nmenu2];
void (*newvaluefn[Nmenu2])(Machine*, int32_t*, int32_t*, int32_t*) = {
    rttval, lostval,
};
...
hashmark(Point p, int dy, int32_t v, int32_t vmax, char* label)
{
	...
	draw(screen, Rect(p.x, y - 1, p.x + Labspace, y + 1), display->black,
	     nil, ZP);
...
drawmark(Graph* g, int x)
{
	...
	draw(screen, Rect(x, g->r.min.y, x + 1, g->r.max.y), cols[c][2], nil,
	     ZP);
}
...
drawcursor(Graph* g, int x)
{
	...
	draw(screen, Rect(x, g->r.min.y, x + 1, g->r.max.y),
	     cols[g->colindex][2], nil, ZP);
}
...
lostval(Machine* m, int32_t* v, int32_t* vmax, int32_t* mark)
{
	...
	if(m->rcvdmsgs + m->lostmsgs > 0)
		x = (m->lostavg >> 1) +
		    (((m->lostmsgs * 100) / (m->lostmsgs + m->rcvdmsgs)) >> 1);
	else
		x = m->lostavg;
	m->lostavg = x;
...
resize(void)
{
	...
	for(x = startx, i = 0; i < nmach; i++, x += dx) {
		...
		if(n > 1 &&
		   j >= 1 + 3 + (n > 10) +
		            (n > 100)) { /* first char of name + (n) */
			j -= 3 + (n > 10) + (n > 100);
			if(j <= 0)
				j = 1;
			snprint(buf, sizeof buf, "%.*s(%d)", j, mach[i].name,
			        n);
		} else
	...
	draw(screen, Rect(screen->r.max.x - hashdx - 1, starty - 1,
	                  screen->r.max.x - hashdx, screen->r.max.y),
	     display->black, nil, ZP);

	/* create graphs */
	for(i = 0; i < nmach; i++) {
		machr = Rect(startx + i * dx, starty, screen->r.max.x,
		             screen->r.max.y);
		if(i < nmach - 1)
		...
		for(j = 0; j < ngraph; j++, y += dy) {
			...
			*g->msg = 0;
			...
			n = (g->r.max.x - r.min.x) /
			    stringwidth(mediumfont, "9");
			if(n > 4) {
				if(n > Gmsglen)
					n = Gmsglen;
				r.max.x =
				    r.min.x + stringwidth(mediumfont, "9") * n;
				g->overtmplen = n;
				g->overtmp =
