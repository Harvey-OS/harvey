...
rlogin(char* rsys, char* keyspec)
{
	...
	for(;;) {
		if(up == nil && os != Plan9)
			up = auth_getuserpasswd(
			    auth_getkey, "proto=pass server=%s service=ftp %s",
			    rsys, keyspec);
		if(up != nil) {
...
cracktime(char* month, char* day, char* yr, char* hms)
{
	...
	if(yr) {
		...
		if(tm.mon > now.mon ||
		   (tm.mon == now.mon && tm.mday > now.mday + 1))
			tm.year--;
	}
...
crackdir(char* p, String** remname)
{
	...
	case OS2:
		...
		case 5:
			...
			if(dn == 3)
				d.atime = cracktime(dfield[0], dfield[1],
				                    dfield[2], field[3]);
			break;
		}
		break;
	case Tops:
		...
		if(dn == 3)
			d.atime = cracktime(dfield[1], dfield[0], dfield[2],
			                    field[2]);
		else
			d.atime = time(0);
		break;
	...
	case VMS:
		...
		case 6:
			...
			dn = getfields(field[2], dfield, 4, 1, "/-");
			if(dn == 3)
				d.atime = cracktime(dfield[1], dfield[0],
				                    dfield[2], field[3]);
			else
				d.atime = time(0);
			break;
	...
	case NetWare:
		...
		case 8: /* New style */
			...
			if(strchr(field[6], ':'))
				d.atime = cracktime(field[4], field[5], nil,
				                    field[6]);
			else
				d.atime = cracktime(field[4], field[5],
				                    field[6], nil);
			break;
		case 9:
			s = s_copy(field[8]);
			...
			d.length = atoll(field[3]);
			d.atime =
			    cracktime(field[4], field[5], field[6], field[7]);
			break;
		case 1:
			s = s_copy(field[0]);
			d.uid = "none";
	...
	default:
		...
		case 8: /* ls -l */
			...
			if(strchr(field[6], ':'))
				d.atime =
				    cracktime(field[4], field[5], 0, field[6]);
			else
				d.atime =
				    cracktime(field[4], field[5], field[6], 0);
			break;
		case 9: /* ls -lg */
			s = s_copy(field[8]);
			d.uid = field[2];
			...
			if(strchr(field[7], ':'))
				d.atime =
				    cracktime(field[5], field[6], 0, field[7]);
			else
				d.atime =
				    cracktime(field[5], field[6], field[7], 0);
			break;
		case 10: /* plan 9 */
			s = s_copy(field[9]);
			d.uid = field[3];
			...
			if(strchr(field[8], ':'))
				d.atime =
				    cracktime(field[6], field[7], 0, field[8]);
			else
				d.atime =
				    cracktime(field[6], field[7], field[8], 0);
			break;
...
port(void)
{
	...
	/* tell remote side */
	sprint(buf, "PORT %d,%d,%d,%d,%d,%d", ipaddr[IPv4off + 0],
	       ipaddr[IPv4off + 1], ipaddr[IPv4off + 2], ipaddr[IPv4off + 3],
	       port >> 8, port & 0xff);
	sendrequest(buf, nil);
	if(getreply(&ctlin, msg, sizeof(msg), 0) != Success)
		return seterr(msg);
...
passive(int mode, Biobuf** bpp, char* cmda, char* cmdb)
{
	...
	snprint(ds, sizeof(ds), "%s!%s.%s.%s.%s!%d", net, f[0], f[1], f[2],
	        f[3], ((atoi(f[4]) & 0xff) << 8) + (atoi(f[5]) & 0xff));
...
static int nw_mode(char dirlet, char* s) /* NetWare file mode mapping */
{
	int mode = 0777;
	...
	if(s[2] == '-') /* can't write file */
		mode &= ~0222;
	if(dirlet == 'd' && s[7] == '-' &&
	   s[3] == '-') /* cannot create in, or modify dir */
		mode &= ~0222;
