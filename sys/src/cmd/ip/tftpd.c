...
static Opt option[] = {
    ...
    /* see "hack" below */
    "blksize", &blksize, 8, Maxsegsize, "tsize", &tsize, 0, (~0UL >> 1) & 0xFF,
};
...
static char* opnames[] = {
        [Tftp_READ] "read", [Tftp_WRITE] "write", [Tftp_DATA] "data",
        [Tftp_ACK] "ack",   [Tftp_ERROR] "error", [Tftp_OACK] "oack",
};
		...
		if(cistrcmp(name, op->name) == 0) {
			...
			*op->valp = n;
			...
			if(cistrcmp("tsize", op->name) != 0)
				syslog(dbg, flog,
				       "tftpd %d setting %s to client's %d",
				       pid, name, n);
			return op;
		}
	return nil;
 ...
 */
static int
options(int fd, char* buf, int bufsz, char* file, uint16_t oper, char* p,
        int dlen)
{
	...
	for(; dlen > 0 && *p != '\0'; p = val + vallen, bp += olen) {
		...
		if(oper == Tftp_READ && cistrcmp(p, "tsize") == 0) {
			...
			*op->valp = size;
			...
			syslog(dbg, flog, "tftpd %d %s tsize is %,lld", pid,
			       file, size);
	...
	if(restricted) {
		if(file[0] == '#' || strncmp(file, "../", 3) == 0 ||
		   ...
		   (file[0] == '/' && strncmp(file, dirsl, dirsllen) != 0)) {
			...
			close(fd);
			syslog(dbg, flog,
			       "tftpd %d bad request %d from %s file %s", pid,
			       op, raddr, file);
			return;
		}
	}
...
sendfile(int net, char* name, char* mode, int opts)
{
	...
	file = -1;
	failed = 1;
	syslog(dbg, flog, "tftpd %d send file '%s' %s to %s", pid, name, mode,
	       raddr);
	name = sunkernel(name);
	if(name == 0) {
	...
	for(txtry = 0; txtry < timeout;) {
		if(rexmit == Ackok) {
			...
			syslog(dbg, flog, "tftpd %d rexmit %d %s:%d to %s", pid,
			       Hdrsize + n, name, block, raddr);
			txtry++;
		}
...
error:
	syslog(dbg, flog, "tftpd %d %s file '%s' %s to %s", pid,
...
sunkernel(char* name)
{
	...
	if(suffix - name != 8 ||
	   (strcmp(suffix, "") != 0 && strcmp(suffix, ".SUN") != 0))
		return name;
