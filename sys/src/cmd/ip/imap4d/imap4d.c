...
static ParseCmd SNonAuthed[] = {{"capability", capabilityCmd},
                                {"logout", logoutCmd},
                                {"x-exit", logoutCmd},
                                {"noop", noopCmd},
...
static ParseCmd SAuthed[] = {{"capability", capabilityCmd},
                             {"logout", logoutCmd},
                             {"x-exit", logoutCmd},
                             {"noop", noopCmd},
...
static ParseCmd SSelected[] = {{"capability", capabilityCmd},
                               {"logout", logoutCmd},
                               {"x-exit", logoutCmd},
                               {"noop", noopCmd},
...
main(int argc, char* argv[])
{
	...
	case 'v':
		...
		break;
	default:
		fprint(2, "usage: ip/imap4d [-acpv] [-d site] [-r remotehost] "
		          "[-s servername]\n");
		bye("usage");
		break;
	}
...
imap4(int preauth)
{
	...
	if(preauth) {
		Bprint(&bout, "* preauth %s IMAP4rev1 server ready user %s "
		              "authenticated\r\n",
		       servername, username);
		imapState = SAuthed;
	} else {
 ...
 * rfc2060 5.5:	servers prohibited from sending expunge while fetch, store,
 *search in progress
 * rfc2060 7:	in selected state, server checks mailbox for new messages as
 *part of every command
 * 		sends untagged EXISTS and RECENT respsonses reflecting new size
 *of the mailbox
 * 		should also send appropriate untagged FETCH and EXPUNGE messages
 *if another agent
 * 		changes the state of any message flags or expunges any messages
 * rfc2060 7.4.1	expunge server response must not be sent when no command
 *is in progress,
 * 		nor while responding to a fetch, stort, or search command (uid
 *versions are ok)
...
authenticateCmd(char* tg, char* cmd)
{
	...
	if(cistrcmp(s, "cram-md5") == 0) {
		...
		Bprint(&bout,
		       "%s NO %s unsupported authentication protocol\r\n", tg,
		       cmd);
}
...
capabilityCmd(char* tg, char* cmd)
{
	...
	//	Bprint(&bout, "* CAPABILITY IMAP4 IMAP4REV1 NAMESPACE IDLE SCAN
...
idleCmd(char* tg, char* cmd)
{
	...
	if(idlepid < 0) {
		...
		if(pid == 0) {
			for(;;) {
				...
				/*
				 * parent may have changed curDir, but it
				 * doesn't change our .
				 */
				resetCurDir();
	...
	/*
	 * clear out the next line, which is supposed to contain
	 * (case-insensitive)
	 * done\n
...
listCmd(char* tg, char* cmd)
{
	...
	if(ref == nil || mbox == nil) {
		Bprint(&bout,
		       "%s BAD %s mailbox name not in modified utf-7\r\n", tg,
		       cmd);
		return;
	}
...
namespaceCmd(char* tg, char* cmd)
{
	 ...
	 * send back nil or descriptions of (prefix heirarchy-delim) for each
	 * case
	 */
	...
	if(rock.next != nil && rock.next->key == SKCharset) {
		if(cistrcmp(rock.next->s, "utf-8") != 0 &&
		   cistrcmp(rock.next->s, "us-ascii") != 0) {
			Bprint(&bout, "%s NO [BADCHARSET] (\"US-ASCII\" "
			              "\"UTF-8\") %s%s failed\r\n",
			       tg, uid, cmd);
			checkBox(selected, 0);
			status(uids, uids);
...
selectCmd(char* tg, char* cmd)
{
	...
	Bprint(&bout, "* OK [PERMANENTFLAGS (\\Seen \\Answered \\Flagged "
	              "\\Draft \\Deleted)]\r\n");
...
static NamedInt statusItems[] = {{"MESSAGES", SMessages},
                                 {"RECENT", SRecent},
                                 {"UIDNEXT", SUidNext},
...
statusCmd(char* tg, char* cmd)
{
	...
	if(box == nil) {
		check();
		Bprint(&bout,
		       "%s NO [TRYCREATE] %s can't open mailbox %s: %r\r\n", tg,
		       cmd, mbox);
		return;
	}
	...
	for(i = 0; statusItems[i].name != nil; i++) {
		if(si & statusItems[i].v) {
			...
			default:
				Bprint(&bout, ")");
				bye("internal error: status item not "
				    "implemented");
				break;
			}
...
fetchWhat(void)
{
	...
	s = atomString(fetchAtom, "");
	if(cistrcmp(s, "all") == 0)
		f = mkFetch(FFlags, mkFetch(FInternalDate,
		                            mkFetch(FRfc822Size,
		                                    mkFetch(FEnvelope, nil))));
	else if(cistrcmp(s, "fast") == 0)
		f = mkFetch(FFlags,
		            mkFetch(FInternalDate, mkFetch(FRfc822Size, nil)));
	else if(cistrcmp(s, "full") == 0)
		f = mkFetch(
		    FFlags,
		    mkFetch(FInternalDate,
		            mkFetch(FRfc822Size,
		                    mkFetch(FEnvelope, mkFetch(FBody, nil)))));
	else
		f = fetchAtt(s, nil);
	return f;
}
 ...
 * searchkey	: "ALL" | "ANSWERED" | "DELETED" | "FLAGGED" | "NEW" | "OLD" |
 *"RECENT"
 *		| "SEEN" | "UNANSWERED" | "UNDELETED" | "UNFLAGGED" | "DRAFT" |
 *"UNDRAFT"
 *		| astrkey ' ' astring
 *		| datekey ' ' date
 ...
 * datekey	: "BEFORE" | "ON" | "SINCE" | "SENTBEFORE" | "SENTON" |
 *"SENTSINCE"
 */
static NamedInt searchMap[] = {{"ALL", SKAll},
                               {"ANSWERED", SKAnswered},
                               {"DELETED", SKDeleted},
                               {"FLAGGED", SKFlagged},
                               {"NEW", SKNew},
...
static NamedInt searchMapStr[] = {{"CHARSET", SKCharset},
                                  {"BCC", SKBcc},
                                  {"BODY", SKBody},
                                  {"CC", SKCc},
                                  {"FROM", SKFrom},
...
static NamedInt searchMapDate[] = {{"BEFORE", SKBefore},
                                   {"ON", SKOn},
                                   {"SINCE", SKSince},
...
static NamedInt searchMapFlag[] = {
    {"KEYWORD", SKKeyword}, {"UNKEYWORD", SKUnkeyword}, {nil, 0}};
