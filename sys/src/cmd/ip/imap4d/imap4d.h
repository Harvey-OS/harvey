...
enum { StrAlloc = 32,      /* characters allocated at a time */
...
struct Msg {
	...
	uint8_t
	    expunged;    /* message actually expunged, but not yet reported to
	                    client */
	uint8_t matched; /* search succeeded? */
	uint8_t
	    bogus;    /* implies the message is invalid, ie contains nulls; see
	                 flags above */
	...
	uint32_t
	    size; /* size of fs/rawbody, in bytes, with \r added before \n */
 ...
 * however, it means these structures cannot be mixed with the mailbox
 *structures
 */
