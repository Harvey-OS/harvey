...
fetchBody(Msg* m, Fetch* f)
{
	...
	case FPHeadFieldsNot:
		n = m->head.size + 3;
		s = emalloc(n);
		n = selectFields(s, n, m->head.buf, f->hdrs,
		                 f->part == FPHeadFields);
		fetchBodyStr(f, s, n);
		free(s);
		return;
...
findMsgSect(Msg* m, NList* sect)
{
	...
	for(; sect != nil; sect = sect->next) {
		id = sect->n;
#ifdef HACK
		/* HACK to solve extra level of structure not visible from
		 * upas/fs  */
		if(m->kids == 0 && id == 1 && sect->next == nil) {
			if(m->mime.type->s &&
			   strcmp(m->mime.type->s, "message") == 0)
				if(m->mime.type->t &&
				   strcmp(m->mime.type->t, "rfc822") == 0)
					if(m->head.type->s &&
					   strcmp(m->head.type->s, "text") == 0)
						if(m->head.type->t &&
						   strcmp(m->head.type->t,
						          "plain") == 0)
							break;
		}
/* end of HACK */
...
fetchBodyStruct(Msg* m, Header* h, int extensions)
{
	 ...
	 * except in the case of a multipart message, when return lengths for a
	 * body[] response
	 */
	len = m->size;
	if(h == &m->mime)
	...
	if(h->type == nil || cistrcmp(h->type->s, "text") == 0) {
		...
		if(h != &m->mime)
			k = m->kids;
		if(k == nil)
			Bprint(&bout, "(NIL NIL NIL NIL NIL NIL NIL NIL NIL "
			              "NIL) (\"text\" \"plain\" NIL NIL NIL "
			              "NIL 0 0) 0");
		else {
			fetchEnvelope(k);
			Bputc(&bout, ' ');
