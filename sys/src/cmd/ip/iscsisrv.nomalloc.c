...
appsensedata(Resppkt *rp, uchar *data, uint len)
{
	...
	/* we assume that resppkt is vast */
...
appdata(Datainpkt *rp, uchar *data, uint len)
{
	...
	/* we assume that datapkt is vast */
...
//	pk->datapkt = nil;
	pk->datalen = Bhdrsz;	/* discard any data to be returned */
	return Reply;
}
...
targread(Pkts *pk, vlong blockno, int nblks)
{
	...
	/* read data directly into outgoing data-in pkt */
	assert(nblks*Blksz <= 1024*1024);
	n = read(targfd, (uchar *)pk->datapkt + pk->datalen, nblks*Blksz);
	if (n < 0)
		n = 0;
	pk->datalen += ROUNDUP(n, 4);
	/* dseglen excludes padding */
	putbe3(pk->datapkt->dseglen, getbe3(pk->datapkt->dseglen) + n);
}
...
icmd(Pkts *pk)
{
	...
	Iscsicmdreq *req;
	Iscsidatain *dpkt;
	static uchar datapkt[1024*1024];
...
//		pk->datapkt = dpkt = emalloc(pk->datalen);
		pk->datapkt = dpkt = (Iscsidatain *)datapkt;
...
process(int net, Iscsijustbhdr *bhdr)
{
	...
	Pkts pkts;
	Pkts *pk;
	static uchar reqpkt[1024*1024], resppkt[1024*1024];
...
//	pk->resppkt = emalloc(pk->resplen);
	pk->resppkt = (Iscsicmdresp *)resppkt;
