/*
 * iscsifs - iscsi initiator (client), presents sd(3)-like view of remote device
 */
#include <u.h>
#include <libc.h>
#include <auth.h>
 ...
 */
void
ilogin(int net, char *targ)
{
	int n;
	Iscsiloginreq reqpkt;
	Iscsiloginresp resppkt;

	memset(&reqpkt, 0, sizeof reqpkt);
	// TODO format ilogin pkt with targ

	strcpy((char *)&reqpkt, targ);		// fake
	reqpkt.op = Ioplogin;
//	uchar	op;		\
//	uchar	opspfc[3];	\
//	uchar	totahslen;	\
...
//	uchar	dseg[];		/* dseg[dseglen] */
	...
	if (write(net, &reqpkt, sizeof reqpkt) != sizeof reqpkt)
		sysfatal("error sending login request: %r");

	n = read(net, &resppkt, sizeof resppkt);
	if (n <= 0)
		sysfatal("bad response to login request");
...
rootgen(int off, Dir *d, void*)
{
	...
	if(off == 0){
		...
		d->qid.path = Qdir;
		d->qid.type = QTDIR;
		d->uid = estrdup9p("iscsifs");
		d->gid = estrdup9p("iscsifs");
		d->muid = estrdup9p("");
		return 0;
	}
	return -1;
...
rdwrpart(Req *r)
{
	...
	/* TODO: instead, format+send req pkt, read resp pkt */
...
usage(void)
{
	fprint(2, "usage: %s [-Dr] [-m mtpt] [-p 9parts] [-s srvname] "
		"dialstring target\n", argv0);
...
main(int argc, char **argv)
{
	...
	addr = netmkaddr(dialstr, "tcp", "3260");
	net = dial(addr, nil, nil, nil);
	if(net < 0)
		sysfatal("dial %s: %r", addr);

	ilogin(net, targ);

	/* TODO: rewrite this; get geometry from the target */
