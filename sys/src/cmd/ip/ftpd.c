...
main(int argc, char** argv)
{
	...
	while(cmd = Brdline(&in, '\n')) {
		...
		if(t->name == 0) {
			 ...
			 *  This is a major hack to avoid the problem. --
			 * presotto
			 */
			i = strlen(cmd);
			if(i > 4 && strcmp(cmd + i - 4, "abor") == 0) {
				...
				logit("%s (%s) command not implemented", cmd,
				      arg ? arg : "");
...
transfer(char* cmd, char* a1, char* a2, char* a3, int image)
{
	...
	case -1:
		return reply("450 Out of processes: %r");
	case 0:
		logit("running %s %s %s %s pid %d", cmd, a1 ? a1 : "",
		      a2 ? a2 : "", a3 ? a3 : "", getpid());
		close(pfd[1]);
		close(dfd);
		dup(pfd[0], 1);
	...
	if(w != nil && w->msg != nil && w->msg[0] != 0) {
		bytes = -1;
		logit("%s", w->msg);
		logit("%s %s %s %s failed %s", cmd, a1 ? a1 : "", a2 ? a2 : "",
		      a3 ? a3 : "", w->msg);
	}
	free(w);
...
passcmd(char* response)
{
	...
	if(isnoworld) {
		...
		if(ch == nil)
			return reply(
			    "531 Send user id before encrypted challenge");
		ch->resp = response;
		...
		snprint(namefile, sizeof(namefile), "/usr/%s/lib/namespace.ftp",
		        user);
...
portcmd(char* arg)
{
	...
	snprint(data, sizeof data, "tcp!%.3s.%.3s.%.3s.%.3s!%d", field[0],
...
listfile(Biobufhdr* b, char* name, int lflag, char* dname)
{
	...
	if(lflag) {
		...
		Bprint(b, "%s %3d %-8s %-8s %7lld %s ", mode2asc(d->mode),
		       links, d->uid, d->gid, d->length, ts + 4);
	}
	if(Cflag && maxnamelen < 40) {
		n = strlen(name);
		pad =
		    ((col + maxnamelen) / (maxnamelen + 1)) * (maxnamelen + 1);
	...
	for(i = 0; i < argc; i++) {
		...
		if(Cflag)
			for(g = gl->first; g; g = g->next)
				if(g->glob &&
				   (n = strlen(s_to_c(g->glob))) > maxnamelen)
					maxnamelen = n;
		while(s = globiter(gl)) {
			if(debug)
				logit("glob %s", s);
...
mdtmcmd(char* arg)
{
	...
	if(d == nil)
		return reply("501 %r accessing %s", arg);
	t = gmtime(d->mtime);
	rv = reply("213 %4.4d%2.2d%2.2d%2.2d%2.2d%2.2d", t->year + 1900,
...
retrieve(char* arg, int arg2)
{
	...
	if(type != Timage)
		for(p = buf, ep = &buf[n]; p < ep; p++)
			if(*p & 0x80) {
				close(fd);
				reply(
				    "550 This file requires type binary/image");
				return;
			}
	...
	do {
		...
		if(i != n) {
			close(fd);
			close(dfd);
			logit("get %s %r to data connection after %d", arg,
			      bytes);
...
storecmd(char* arg)
{
	...
	if(offset) {
		...
		if(seek(fd, offset, 0) == -1)
			return reply("550 Error seeking %s to %d: %r", arg,
			             offset);
	} else {
...
rntocmd(char* to)
{
	...
	if((tp && fp == 0) || (fp && tp == 0) ||
	   (fp && tp && (fp - s_to_c(filepath) != tp - to ||
	                 memcmp(s_to_c(filepath), to, tp - to))))
