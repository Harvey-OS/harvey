...
enum { IP_IPV6PROTO = 41, /* IPv4 protocol number for IPv6 */
...
usage(void)
{
	fprint(
	    2,
	    "usage: %s [-ag] [-x mtpt] [local6[/mask]] [remote4 [remote6]]\n",
	    argv0);
	exits("Usage");
}
...
defv6addr(void)
{
	...
	return smprint("%ux:%2.2x%2.2x:%2.2x%2.2x::1/48", V6to4pfx, ipv4[0],
...
ip2tunnel(int in, int out)
{
	...
	while((n = read(in, ip, sizeof buf - STFHDR)) > 0) {
		...
		if(write(out, op, n) != n) {
			syslog(0, "6in4",
			       "error writing to tunnel (%r), giving up");
			break;
		}
	}
}
...
tunnel2ip(int in, int out)
{
	...
	for(;;) {
		...
		if(n <= 0) {
			syslog(0, "6in4",
			       "error reading from tunnel (%r), giving up");
			break;
		}
		...
		if(write(out, op, n) != n) {
			syslog(0, "6in4", "error writing to packet interface "
			                  "(%r), giving up");
			break;
		}
	}
