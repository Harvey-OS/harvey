...
urlinit(void)
{
	...
	if(b == nil) {
		...
		if(b == nil) {
			syslog(0, HTTPLOG, "no %s, abandon prefetch hints",
			       file);
			filelen = -1;
			return;
		}
	...
	syslog(0, HTTPLOG, "prefetch-hints url=%d (%.1fMB)", nurl,
	       1.e-6 * (URLmax * sizeof(*urlname) + filelen));
...
statsinit(void)
{
	...
	syslog(0, HTTPLOG, "prefetch-hints stats=%d (%.1fMB)", nstats,
	       1.e-6 * ((filelen / 3) * sizeof(Hint)));
}
...
hintprint(HConnect* hc, Hio* hout, char* uri, int thresh, int havej)
{
	...
	for(i = 0; i < nhint[j]; i++) {
		...
		close(fd);
		snprint(etag, sizeof(etag), "\"%lluxv%lux\"", dir->qid.path,
		        dir->qid.vers);
		siz = (int)(log((double)dir->length) * RECIPLOG2 + 0.9999);
		free(dir);
		if(strncmp(uri, sf, prefix) == 0 &&
		   strchr(sf + prefix, '/') == 0 && sf[prefix] != 0)
			sf = sf + prefix;
