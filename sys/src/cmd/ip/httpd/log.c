...
writelog(HConnect* c, char* fmt, ...)
{
	...
	if(logfd > 0) {
		...
		*c->hstop = '\0';
		...
		if(c->req.uri != nil && c->req.uri[0] != 0)
			bufp =
			    seprint(bufp, bufe, "FinalURI: %s\n", c->req.uri);
		bufp =
		    seprint(bufp, bufe, "----------\n%s\n", (char*)c->header);
		write(logfd, buf, bufp - buf); /* append-only file */
	}

	/* another log, with less information but formatted for common analysis
	 * tools */
	if(logall[2] > 0 && strncmp(fmt, "Reply: ", 7) == 0) {
		...
		if(fmt[7] == '%') {
			...
			va_end(arg);
		} else if(strcmp(fmt + 7, "200 file %lld %lld\n") == 0 ||
		          strcmp(fmt + 7, "206 partial content %lld %lld\n") ==
		              0 ||
		          strcmp(fmt + 7, "206 partial content, early "
		                          "termination %lld %lld\n") == 0) {
			va_start(arg, fmt);
			objectsize =
			    va_arg(arg, int64_t); /* length in sendfd.c */
			USED(objectsize);
			objectsize =
			    va_arg(arg, int64_t); /* wrote in sendfd.c */
			va_end(arg);
		}
		bufp = seprint(buf, bufe, "%s - -", p->remotesys);
		bufp = seprint(bufp, bufe, " [%.2d/%s/%d:%.2d:%.2d:%.2d +0000]",
		               tm->mday, monname[tm->mon], tm->year + 1900,
		               tm->hour, tm->min, tm->sec);
		if(c->req.uri == nil || c->req.uri[0] == 0) {
			bufp = seprint(
			    bufp, bufe, " \"%.*s\"",
			    (int)utfnlen((char*)c->header,
			                 strcspn((char*)c->header, "\r\n")),
			    (char*)c->header);
		} else {
			/* use more canonical form of URI, if available */
			bufp = seprint(bufp, bufe, " \"%s %s HTTP/%d.%d\"",
			               c->req.meth, c->req.uri, c->req.vermaj,
			               c->req.vermin);
		}
		bufp =
		    seprint(bufp, bufe, " %s %lld\n", statuscode, objectsize);
