...
doctype(void)
{
	hprint(hout, "<!DOCTYPE html PUBLIC \"-//W3C//DTD XHTML 1.0 "
	             "Transitional//EN\"\n");
	hprint(hout, "    "
...
maxwidths(Dir* dp, int32_t n)
{
	...
	for(i = 0; i < n; i++) {
		if(snprint(scratch, sizeof scratch, "%ud", dp[i].dev) >
		   devwidth)
		...
		if(snprint(scratch, sizeof scratch, "%lld", dp[i].length) >
		   lenwidth)
...
dols(char* dir)
{
	...
	cleanname(
	    dir); //  expands "" to "."; ``dir+1'' access below depends on that
	if(!allowed(dir)) {
		error("Permission denied",
		      "<p>Cannot list directory %s: Access prohibited</p>",
		      dir);
		return;
	}
	fd = open(dir, OREAD);
	if(fd < 0) {
		error("Cannot read directory",
		      "<p>Cannot read directory %s: %r</p>", dir);
		return;
	}
	if(vermaj) {
		hokheaders(connect);
	...
	while((p = strchr(nm, '/')) != nil) {
		*p = '\0';
		...
		if(!(*dir == '\0' && *(dir + 1) == '\0') && allowed(f))
			hprint(hout, "<a href=\"/magic/webls?dir=%H\">%s/</a>",
			       f, nm);
		else
			hprint(hout, "%s/", nm);
		*p = '/';
	...
	for(i = 0; i < n; i++) {
		...
		hprint(hout,
		       "%M %C %*ud %-*s %-*s %*lld %s <a href=\"%s\">%s</a>\n",
		       d[i].mode, d[i].type, devwidth, d[i].dev, uidwidth,
		       d[i].uid, gidwidth, d[i].gid, lenwidth, d[i].length,
	...
	if(strcmp(f, dir) != 0 && allowed(f))
		hprint(hout, "\nGo to <a "
		             "href=\"/magic/webls?dir=%H\">parent</a> "
		             "directory\n",
		       f);
	else
...
main(int argc, char** argv)
{
	...
	if(strcmp(connect->req.meth, "GET") != 0 &&
	   strcmp(connect->req.meth, "HEAD") != 0) {
