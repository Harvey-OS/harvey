...
enum { MaxLog = 24 * 1024, /* limit on length of any one log request */
       LockSecs =
	   MaxLog /
	   500, /* seconds to wait before giving up on opening the data file */
};
...
main(int argc, char** argv)
{
	...
	if(strcmp(c->req.meth, "POST") == 0) {
		...
		if(t != nil)
			*t = '\0';
	} else if(strcmp(c->req.meth, "GET") != 0 &&
