...
main(int argc, char** argv)
{
	...
	dolisten(
	    netmkaddr(address, "tcp", certificate == nil ? "http" : "https"));
	exits(nil);
}
...
hashsys(char* rsys)
{
	...
	if(sys->rsys != nil) {
		...
		for(;
		    notme = (strcmp(sys->rsys, rsys) != 0) && sys->next != nil;
		    sys = sys->next)
			;
		if(notme) {
	...
	for(;;) {
		...
		case 0:
			...
			if(data >= 0 && certificate != nil) {
				...
				scheme = "http";
			if(data < 0) {
				syslog(0, HTTPLOG, "can't open %s/data: %r",
				       ndir);
				exits(nil);
			}
			dup(data, 0);
...
top:
	...
	if(newuri != nil) {
		...
		if(hparseheaders(c, 15 * 60 * 1000) < 0)
			exits("failed");
		if(flags & Redirperm) {
			logit(c, "%s: permanently moved to %s", origuri,
			      newuri);
...
magic:
	if(magic != nil && strcmp(magic, "httpd") != 0) {
		...
		snprint(vers, sizeof(vers), "HTTP/%d.%d", c->req.vermaj,
		        c->req.vermin);
		hb = hunload(&c->hin);
		if(hb == nil) {
			hfail(c, HInternal);
		...
		hp = c->private;
		execl(c->xferbuf, magic, "-d", hmydomain, "-w", webroot, "-s",
		      c->scheme, "-p", c->port, "-r", hp->remotesys, "-N",
		      netdir, "-b", hb, "-L", logfd0, logfd1, "-R", c->header,
	...
	if(origuri[0] == '/' && origuri[1] == 0) {
		snprint(virtualhost, sizeof virtualhost, "http://%s/",
		        c->head.host);
...
send(HConnect* c)
{
	...
	fd = open(w, OREAD);
	if(fd < 0 && strlen(masque) > 0 &&
	   strncmp(c->req.uri, masque, strlen(masque)) == 0) {
		// may be a URI from before virtual hosts;  try again without
		// masque
		strcpy(w, webroot);
		strcat(w, c->req.uri);
		fd = open(w, OREAD);
