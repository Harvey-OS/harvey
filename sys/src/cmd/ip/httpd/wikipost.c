...
enum { MaxLog = 100 * 1024, /* limit on length of any one log request */
};
...
char*
dowiki(HConnect* c, char* title, char* author, char* comment, char* base,
       uint32_t version, char* text)
{
	...
	if((fd = open("/mnt/wiki/new", ORDWR)) < 0) {
		syslog(0, LOG, "%s open /mnt/wiki/new failed: %r",
		       hp->remotesys);
		hfail(c, HNotFound);
		exits("failed");
	}
	...
	if((i++, fprint(fd, "%s\nD%lud\nA%s (%s)\n", title, version, author,
	   ...
	   (i++, (text[0] && write(fd, text, strlen(text)) != strlen(text)))) {
		syslog(0, LOG, "%s write failed %d %ld fd %d: %r",
		       hp->remotesys, i, strlen(text), fd);
		hfail(c, HInternal);
		exits("failed");
	}
...
main(int argc, char** argv)
{
	...
	syslog(0, LOG, "%s post s %s t '%s' v %ld a %s c %s b %s t 0x%p",
	       hp->remotesys, service, title, (int32_t)version, author, comment,
	       base, text);
	...
	if(title == nil || version == ~0 || text == nil || text[0] == '\0' ||
	   base == nil || service == nil || strchr(title, '\n') ||
	   strchr(comment, '\n') || dangerous(service) ||
	   strchr(service, '/') || strlen(service) > 20) {
		syslog(0, LOG, "%s failed dangerous", hp->remotesys);
		hfail(hc, HSyntax);
		exits("failed");
	}

	syslog(0, LOG, "%s post s %s t '%s' v %ld a %s c %s", hp->remotesys,
	       service, title, (int32_t)version, author, comment);
