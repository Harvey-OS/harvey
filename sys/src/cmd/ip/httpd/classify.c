...
char* gov[] = {
    "gov", "gouv", "mil", "government", 0,
};

Country allc[] = {{"ad", "andorra"},
...
{
	for(; cp->code != 0; cp++)
		if(cistrcmp(s, cp->code) == 0 || cistrcmp(s, cp->name) == 0)
			return 1;
	return 0;
}
...
classify(char* ip, Ndbtuple* t)
{
	...
	for(nt = t; nt != nil; nt = nt->entry) {
		...
		/* domain names can always hurt, even without forward
		 * verification */
