...
char* ospftype[] = {
        [OSPFhello] "hello", [OSPFdd] "data definition",
        [OSPFlsrequest] "link state request",
        [OSPFlsupdate] "link state update", [OSPFlsack] "link state ack",
};
...
ospfauth(Ospfpkt* ospf)
{
	...
	case 2:
		...
		break;
	default:
		sprint(auth, "auth%d(%8.8ux %8.8ux)", NetS(ospf->autype),
		       NetL(ospf->auth), NetL(ospf->auth + 4));
	}
	return auth;
}
...
seprintospfhello(char* p, char* e, void* a)
{
	...
	return seprint(p, e, "%s(mask %V interval %d opt %ux pri %ux deadt %d "
	                     "designated %V bdesignated %V)",
	               ospftype[OSPFhello], h->mask, NetS(h->interval),
	               h->options, h->pri, NetL(h->deadint), h->designated,
	               h->bdesignated);
}
...
char* lsatype[] = {
        [LSARouter] "Router LSA",           [LSANetwork] "Network LSA",
        [LSASummN] "Summary LSA (Network)", [LSASummR] "Summary LSA (Router)",
...
seprintospflsaheader(char* p, char* e, struct OspfLSAhdr* h)
{
	return seprint(
	    p, e,
	    "age %d opt %ux type %ux lsid %V adv_rt %V seqno %ux c %4.4ux l %d",
	    NetS(h->lsage), h->options & 0xff, h->lstype, h->lsid, h->advtrt,
	    NetL(h->lsseqno), NetS(h->lscksum), NetS(h->lsalen));
}
...
seprintospflsupdate(char* p, char* e, void* a, int len)
{
	...
	case LSANetwork: {
		...
		for(i = 0; i < nlsa; i++) {
			...
			p = seprint(p, e, " mask %V attrt %V)", h->netmask,
			            h->attrt);
		}
	} break;
	case LSASummN:
	case LSASummR: {
		...
		for(i = 0; i < nlsa; i++) {
			...
			p = seprint(p, e, "lsa%d(", i);
			p = seprintospflsaheader(p, e, &(h->hdr));
			p = seprint(p, e, " mask %V met %d)", h->netmask,
			            Net3(h->lsa.metric));
		}
	} break;
	case LSAASext: {
		...
		for(i = 0; i < nlsa; i++) {
			...
			p = seprint(p, e, " lsa%d(", i);
			p = seprintospflsaheader(p, e, &(h->hdr));
			p = seprint(p, e, " mask %V extflg %1.1ux met %d "
			                  "fwdaddr %V extrtflg %ux)",
			            h->netmask, h->lsa.flag,
			            Net3(h->lsa.metric), h->lsa.fwdaddr,
			            NetL(h->lsa.exrttag));
		}
	} break;
	default:
...
p_seprint(Msg* m)
{
	...
	p = seprint(p, e, "ver=%d type=%d len=%d r=%V a=%V c=%4.4ux %s ",
	            ospf->version, ospf->type, x, ospf->router, ospf->area,
	            NetS(ospf->sum), ospfauth(ospf));
...
Proto ospf = {
    "ospf", nil, nil, p_seprint, nil, nil, nil, defaultframer,
};
