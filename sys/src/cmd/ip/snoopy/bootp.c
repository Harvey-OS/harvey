...
enum { OfferTimeout = 60,     /* when an offer times out */
...
struct Hdr {
	...
	uint8_t flags[2];
	uint8_t
	    ciaddr[IPv4addrlen]; /* client IP address (client tells server) */
	uint8_t
	    yiaddr[IPv4addrlen]; /* client IP address (server tells client) */
...
p_seprint(Msg* m)
{
	...
	m->p = seprint(
	    m->p, m->e, "t=%s ht=%d hl=%d hp=%d xid=%ux sec=%d fl=%4.4ux ca=%V "
	                "ya=%V sa=%V ga=%V cha=%E magic=%lux",
	    op(h->op), h->htype, h->hlen, h->hops, NetL(h->xid), NetS(h->secs),
	    NetS(h->flags), h->ciaddr, h->yiaddr, h->siaddr, h->giaddr,
	    h->chaddr, (uint32_t)NetL(h->optmagic));
...
Proto bootp = {
    "bootp", p_compile, p_filter, p_seprint,
    p_mux,   "%#.8lux", p_fields, defaultframer,
};
