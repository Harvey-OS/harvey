...
char* pktnames[] = {"Sync",  "Data",  "Dataquery", "Ack",
                    "Query", "State", "Close"};
...
p_seprint(Msg* m)
{
	...
	m->p = seprint(
	    m->p, m->e,
	    "s=%d d=%d t=%s id=%lud ack=%lud spec=%d ck=%4.4ux ln=%d", sport,
	    dport, pkttype(h->type), (uint32_t)NetL(h->id),
	    (uint32_t)NetL(h->ack), h->spec, NetS(h->sum), NetS(h->len));
	return 0;
}

Proto il = {
    "il",  p_compile, p_filter, p_seprint,
