...
{
	return 4 + (flags & GRE_chksum || flags & GRE_routing)
	           ? 4
	           : 0 + flags & GRE_key ? 4 : 0 + flags & GRE_seq
	                                           ? 4
	                                           : 0 + flags & GRE_ack ? 4
	                                                                 : 0;
}
...
p_seprint(Msg* m)
{
	...
	m->p = seprint(m->p, m->e, "version=%d proto=%#ux flags=%#.4ux",
	               h.version, h.proto, h.flags);
	...
	if(h.flags & GRE_routing)
		m->p =
		    seprint(m->p, m->e, " offset=%#ux haverouting", h.offset);
	if(h.version == 0)
		m->p = seprint(m->p, m->e, " recursion=%ud",
		               (h.flags & GRE_recur) >> 8);
...
Proto gre = {
    "gre", p_compile, p_filter, p_seprint,
    p_mux, "%#.4ux",  p_fields, defaultframer,
};
