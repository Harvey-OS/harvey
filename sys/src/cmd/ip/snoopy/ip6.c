		...
		case FRAG_HDR:
			m->p =
			    seprint(m->p, m->e,
			            ...
			            NetL(pkt + 4), NetS(pkt + 2) & ~7,
			            (int)*pkt, (int)(*(pkt + 3) & 0x1),
			            (int)*(pkt + 1), (int)(*(pkt + 3) & 0x6));
			len = FRAG_HSZ;
			break;
...
p_seprint(Msg* m)
{
	...
	m->p = seprint(m->p, m->e, "s=%I d=%I ttl=%3d pr=%d ln=%d", h->src,
	               h->dst, h->ttl, h->proto, NetS(h->length));
	v6hdr_seprint(m);
	return 0;
}

Proto ip6 = {
    "ip6", p_compile, p_filter, p_seprint,
