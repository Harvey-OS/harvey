...
ps(char* s)
{
	...
	char args[256], *argv[16], buf[64], pbuf[8], rbuf[20], rbuf1[20],
	    status[4096];
	...
	if(rflag) {
		if(rtime >= 86400)
			sprint(rbuf, " %lud:%02lud:%02lud:%02lud",
			       rtime / 86400, (rtime / 3600) % 24,
			       (rtime / 60) % 60, rtime % 60);
		else if(rtime >= 3600)
			sprint(rbuf, " %lud:%02lud:%02lud", rtime / 3600,
			       (rtime / 60) % 60, rtime % 60);
		else
	...
	Bprint(&bout, "%-10s %8s%s %4lud:%.2lud %3lud:%.2lud %s %7ludK %-8.8s ",
	       argv[1], s, rbuf1, utime / 60, utime % 60, stime / 60,
	       stime % 60, pbuf, size, argv[2]);
