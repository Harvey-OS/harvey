...
void first(int v) /* calculate set of positions with v as root which can be
                     active initially */
{
	int i;
	uint8_t* p;
...
cgoto(void)
{
	...
	for(s = 0; s <= stnum; s++) {
		...
		for(i = 1; i <= npos; i++) {
			...
			if(name[curpos] < NCH)
				...
				case S2FINAL:
					break;
				default:
					warning("bad switch cgoto %d state %d",
					        curpos, s);
					break;
#endif
				}
		...
		for(i = 1; i < NCH; i++) {
			if(symbol[i]) {
				nextstate(s, i); /* executed for each state,
				                    transition pair */
				xstate = notin(stnum);
				if(xstate == -2)
...
packtrans(int st, uint8_t* tch, int* tst, int cnt, int tryit)
{
...
#ifdef DEBUG
			if(debug)
				print("use compression  %d,  %d vs %d\n", st, k,
				      cnt);
#endif
			k = 0;
			for(i = 1; i < NCH; i++)
	...
	for(i = 0; i < st; i++) { /* get most similar state */
		/* reject state with more transitions, state already represented
		   by a third state,
		        and state which is compressed by char if ours is not to
		   be */
		if(sfall[i] != -1)
			continue;
		if(cpackflg[st] == 1)
...
stprt(int i)
{
	...
	print("\n");
	if(cpackflg[i] == TRUE)
		print("backup char in use\n");
	if(sfall[i] != -1)
		print("fall back state %d\n", sfall[i]);
	p = gotof[i];
	if(p == -1)
		return;
	print("(%d transitions)\n", nexts[p]);
...
#ifdef DEBUG
		if(debug)
			print("bot,top %d, %d startup begins %d\n", bot, top,
			      startup);
#endif
		do {
			startup += 1;
