...
static double tpoleinit[4][2] = {1., 0., 1., 180., -1., 90., -1., -90.};
static struct tproj {
	...
	struct coord postrot; /*same as trot*/
                 ...
                 {/*30*/ {0., -45., 45., -150.},
                  ...
                  /*33*/ {0.}}};
static double tx[4] = {
    /*where to move facet after final rotation*/
    0., 0., -1., 1. /*-1,1 to be sqrt(3)*/
};
static double ty[4] = {0., 2., -1., -1.};
static double root3;
...
twhichp(struct place* g, int* p, int* q)
{
	...
	for(i = 0; i < 4; i++) {
		...
		cosdist[i] = g->nlat.s * tp->nlat.s +
		             g->nlat.c * tp->nlat.c * (g->wlon.s * tp->wlon.s +
		                                       g->wlon.c * tp->wlon.c);
	}
	j = 0;
	for(i = 1; i < 4; i++)
...
Xtetra(struct place* place, double* x, double* y)
{
	...
	if(br < 0) {
		...
		vr = fpir - vr;
		vi = fpii - vi;
	} else if(!elco2(br, bi, tk, 1., 1., &vr, &vi))
		return 0;
	if(si >= 0) {
		tr = f0r - vi;
		ti = f0i + vr;
	...
	tpp = &tproj[i][j];
	*x = tr * tpp->postrot.c + ti * tpp->postrot.s + tx[i];
	*y = ti * tpp->postrot.c - tr * tpp->postrot.s + ty[i];
	return (1);
}
...
tetracut(struct place* g, struct place* og, double* cutlon)
{
	...
	if((g->nlat.s <= -rt3inv && og->nlat.s <= -rt3inv) &&
	   (ckcut(g, og, * cutlon = 0.) == 2 ||
	    ckcut(g, og, * cutlon = PI) == 2))
		return (2);
	twhichp(g, &i, &k);
	twhichp(og, &j, &k);
	if(i == j || i == 0 || j == 0)
