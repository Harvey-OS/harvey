...
norm(struct place* gg, struct place* pp, struct coord* tw)
{
	...
	if(p->nlat.s == 1.) {
		...
		m.nlat.s =
		    p->nlat.s * g->nlat.s + p->nlat.c * g->nlat.c * g->wlon.c;
		m.nlat.c = sqrt(1. - m.nlat.s * m.nlat.s);
		m.nlat.l = atan2(m.nlat.s, m.nlat.c);
		m.wlon.s = g->nlat.c * g->wlon.s;
		m.wlon.c =
		    p->nlat.c * g->nlat.s - p->nlat.s * g->nlat.c * g->wlon.c;
		m.wlon.l = atan2(m.wlon.s, -m.wlon.c) - tw->l;
		*g = m;
	}
	sincos(&g->wlon);
	if(g->wlon.l > PI)
...
printp(struct place* g)
{
	printf("%.3f %.3f %.3f %.3f %.3f %.3f\n", g->nlat.l, g->nlat.s,
	       g->nlat.c, g->wlon.l, g->wlon.s, g->wlon.c);
}
