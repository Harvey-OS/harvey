...
static struct vert {
	double x, y;
} v[NVERT + 2]; /*clipping polygon*/
static struct edge {
	double a, b, c;
} e[NVERT];       /* coeffs for linear inequality */
...
found:
	...
	while(argc > 0 && option(argv[0])) {
		...
		case 'm':
			...
			while(argc && !option(*argv)) {
				file =
				    realloc(file, (nfile + 1) * sizeof(*file));
	...
	limcase = limits[2] < -180. ? 0 : limits[3] > 180. ? 2 : 1;
	if(window[0] >= window[1] || window[2] >= window[3] ||
	   window[0] > 90. || window[1] < -90. || window[2] > 180. ||
	   window[3] < -180.)
	...
	if(nvert <= 0) {
		for(lat = klimits[0]; lat < klimits[1] + dd - FUZZ; lat += dd) {
			if(lat > klimits[1])
				lat = klimits[1];
			for(lon = klimits[2]; lon < klimits[3] + dd - FUZZ;
			    lon += dd) {
				i = (kflag ? posproj : normproj)(
				    lat,
				    lon + (lon < klimits[3] ? FUZZ : -FUZZ), &x,
				    &y);
				if(i * vflag <= 0)
					continue;
				if(x < xmin)
	...
	if(position[2] != 0) {
		if(posproj(position[0] - .5, position[1], &xcent, &ycent) ==
		       0 ||
	...
	pen(DOTTED);
	if(grid[0] > 0.)
		for(lat = ceil(lolat / grid[0]) * grid[0]; lat <= hilat;
		    lat += grid[0])
			dogrid(lat, lat, lolon, hilon);
	if(grid[1] > 0.)
		for(lon = ceil(lolon / grid[1]) * grid[1]; lon <= hilon;
		    lon += grid[1])
...
inwindow(struct place* geog)
{
	if(geog->nlat.l < rwindow[0] - FUZZ ||
	   ...
	   geog->wlon.l < rwindow[2] - FUZZ || geog->wlon.l > rwindow[3] + FUZZ)
		return (0);
	else
		return (1);
}
...
{
	if(rlimits[0] - FUZZ > g->nlat.l || rlimits[1] + FUZZ < g->nlat.l)
		return (0);
	switch(limcase) {
	case 0:
...
clipinit(void)
{
	...
	if(nvert == 2) { /*rectangle with diag specified*/
		nvert = 4;
		v[2] = v[1];
		v[1].x = v[0].x, v[1].y = v[2].y, v[3].x = v[2].x,
		v[3].y = v[0].y;
	}
	v[nvert] = v[0];
	v[nvert + 1] = v[1];
