...
usage(void)
{
	...
	fprint(2,
	       "\twhere size is an integer or a percentage in the form 25%%\n");
	exits("usage");
}
...
resample(int xsize, int ysize, Memimage* m)
{
	...
	for(i = 0; i < Dy(m->r); i++) {
		...
		if(oscan[i] == nil)
			sysfatal("can't allocate: %r");
		j = unloadmemimage(m, Rect(m->r.min.x, m->r.min.y + i,
		                           m->r.max.x, m->r.min.y + i + 1),
		                   oscan[i], bpl);
		if(j != bpl)
	...
	for(i = 0; i < Dy(m->r); i++) {
		for(j = 0; j < nchan; j++) {
			if(j == 0 && m->chan == XRGB32)
				continue;
			resamplex(oscan[i], j, nchan, Dx(m->r), nscan[i],
			          xsize);
		}
		free(oscan[i]);
		oscan[i] = nscan[i];
