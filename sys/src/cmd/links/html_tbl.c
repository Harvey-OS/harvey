...
{
	char *al;
	if ((al = (char *)get_attr_val((unsigned char *)attr, (unsigned char *)"align"))) {
		if (!(strcasecmp(al, ( char *)"left"))) *a = AL_LEFT;
		if (!(strcasecmp(al, ( char *)"right"))) *a = AL_RIGHT;
		if (!(strcasecmp(al, ( char *)"center"))) *a = AL_CENTER;
		if (!(strcasecmp(al, ( char *)"justify"))) *a = AL_BLOCK;
		if (!(strcasecmp(al, ( char *)"char"))) *a = AL_RIGHT; /* NOT IMPLEMENTED */
		mem_free(al);
	}
}
...
{
	char *al;
	if ((al = (char *)get_attr_val((unsigned char *)attr, (unsigned char *)"valign"))) {
...
{
	char *al;
	if ((al = (char *)get_attr_val((unsigned char *)attr, (unsigned char *)"width"))) {
		if (*al && al[strlen(al) - 1] == '*') {
			...
			int p = get_width((unsigned char *)attr, (unsigned char *)"width", sh);
			if (p >= 0) *w = p;
		}
		mem_free(al);
	}
...
#ifdef DEBUG
#define CELL(t, x, y) (((x) < 0 || (x) >= (t)->rx || (y) < 0 || (y) >= (t)->ry) ? (internal((unsigned char *)"accessing cell out of table (%d,%d) - limit (%d,%d)", (x), (y), (t)->rx, (t)->ry), (t)->cells) : &(t)->cells[(y) * (t)->rx + (x)])
#else
...
struct table *parse_table(unsigned char *html, unsigned char *eof, unsigned char **end, struct rgb *bgcolor, int sh, struct s_e **bad_html, int *bhp)
{
	...
	*end = html;
	...
	if (t_namelen == 5 && !casecmp(t_name, (unsigned char *)"TABLE", 5)) {
		en = skip_element(en, eof, (unsigned char *)"TABLE", 1);
		goto see;
	}
	if (t_namelen == 6 && !casecmp(t_name, (unsigned char *)"SCRIPT", 5)) {
		en = skip_element(en, eof, (unsigned char *)"SCRIPT", 0);
		goto see;
	}
	if (t_namelen == 6 && !casecmp(t_name, (unsigned char *)"/TABLE", 6)) {
		if (c_span) new_columns(t, c_span, c_width, c_al, c_val, 1);
		if (p) CELL(t, x, y)->end = html;
		if (lbhp) (*bad_html)[*bhp-1].e = html;
		goto scan_done;
	}
	if (t_namelen == 8 && !casecmp(t_name, (unsigned char *)"COLGROUP", 8)) {
		if (c_span) new_columns(t, c_span, c_width, c_al, c_val, 1);
		if (lbhp) (*bad_html)[*bhp-1].e = html, lbhp = NULL;
		c_al = AL_TR;
		c_val = VAL_TR;
		c_width = W_AUTO;
		get_align((char *)t_attr, &c_al);
		get_valign((char *)t_attr, &c_val);
		get_c_width((char *)t_attr, &c_width, sh);
		if ((c_span = get_num(t_attr, (unsigned char *)"span")) == -1) c_span = 1;
		goto see;
	}
	if (t_namelen == 9 && !casecmp(t_name, (unsigned char *)"/COLGROUP", 9)) {
		if (c_span) new_columns(t, c_span, c_width, c_al, c_val, 1);
		if (lbhp) (*bad_html)[*bhp-1].e = html, lbhp = NULL;
		c_span = 0;
		c_al = AL_TR;
	...
	if (t_namelen == 3 && !casecmp(t_name, (unsigned char *)"COL", 3)) {
		int sp, wi, al, val;
		if (lbhp) (*bad_html)[*bhp-1].e = html, lbhp = NULL;
		if ((sp = get_num(t_attr,  (unsigned char *)"span")) == -1) sp = 1;
		wi = c_width;
		al = c_al;
		val = c_val;
		get_align((char *)t_attr, &al);
		get_valign((char *)t_attr, &val);
		get_c_width((char *)t_attr, &wi, sh);
		new_columns(t, sp, wi, al, val, !!c_span);
		c_span = 0;
		goto see;
	}
	if (t_namelen == 3 && (!casecmp(t_name, (unsigned char *)"/TR", 3) || !casecmp(t_name, (unsigned char *)"/TD", 3) || !casecmp(t_name, (unsigned char *)"/TH", 3))) {
		if (c_span) new_columns(t, c_span, c_width, c_al, c_val, 1);
		if (p) CELL(t, x, y)->end = html, p = 0;
		if (lbhp) (*bad_html)[*bhp-1].e = html, lbhp = NULL;
	}
	if (t_namelen == 2 && !casecmp(t_name, (unsigned char *)"TR", 2)) {
		if (c_span) new_columns(t, c_span, c_width, c_al, c_val, 1);
		if (p) CELL(t, x, y)->end = html, p = 0;
		if (lbhp) (*bad_html)[*bhp-1].e = html, lbhp = NULL;
		if (group) group--;
		...
		l_val = VAL_MIDDLE;
		memcpy(&l_col, bgcolor, sizeof(struct rgb));
		get_align((char *)t_attr, &l_al);
		get_valign((char *)t_attr, &l_val);
		get_bgcolor(t_attr, &l_col);
		y++, x = 0;
		goto see;
	}
	if (t_namelen == 5 && ((!casecmp(t_name, (unsigned char *)"THEAD", 5)) || (!casecmp(t_name, (unsigned char *)"TBODY", 5)) || (!casecmp(t_name, (unsigned char *)"TFOOT", 5)))) {
		if (c_span) new_columns(t, c_span, c_width, c_al, c_val, 1);
		if (lbhp) (*bad_html)[*bhp-1].e = html, lbhp = NULL;
		group = 2;
	}
	if (t_namelen != 2 || (casecmp(t_name, (unsigned char *)"TD", 2) && casecmp(t_name, (unsigned char *)"TH", 2))) goto see;
	if (c_span) new_columns(t, c_span, c_width, c_al, c_val, 1);
	if (lbhp) (*bad_html)[*bhp-1].e = html, lbhp = NULL;
	if (p) CELL(t, x, y)->end = html, p = 0;
	if (y == -1) y = 0, x = 0;
	...
	get_align((char *)t_attr, &cell->align);
	get_valign((char *)t_attr, &cell->valign);
	get_bgcolor(t_attr, &cell->bgcolor);
#ifdef G
	sprintf((char *)cell->bgcolor_str, "#%02x%02x%02x", cell->bgcolor.r & 0xff, cell->bgcolor.g & 0xff, cell->bgcolor.b & 0xff);
#endif
	if ((csp = get_num(t_attr,  (unsigned char *)"colspan")) == -1) csp = 1;
	if (!csp) csp = -1;
	if ((rsp = get_num(t_attr,  (unsigned char *)"rowspan")) == -1) rsp = 1;
	if (!rsp) rsp = -1;
	cell->colspan = csp;
	cell->rowspan = rsp;
	if (csp == 1) {
		int w = W_AUTO;
		get_c_width((char *)t_attr, &w, sh);
...
void get_cell_width(char *start, char *end, int cellpd, int w, int a, int *min, int *max, int n_link, int *n_links, unsigned char *bgc)
{
	...
	if (!F) {
		if (!(p = format_html_part((unsigned char *)start, (unsigned char *)end, AL_LEFT, cellpd, w, NULL, !!a, !!a, NULL, n_link))) return;
		if (min) *min = p->x;
		if (max) *max = p->xmax;
		...
		if (!(gp = g_format_html_part((unsigned char *)start, (unsigned char *)end, AL_LEFT, 0, w, NULL, n_link, NULL, bgc, NULL))) return;
		if (min) *min = gp->x;
		if (max) *max = gp->xmax;
	...
	if (min && max && *min > *max) internal((unsigned char *)"get_cell_width: %d > %d", *min, *max);
}
...
static inline void check_cell_widths(struct table *t)
{
	...
	for (j = 0; j < t->y; j++) for (i = 0; i < t->x; i++) {
		...
		struct table_cell *c = CELL(t, i, j);
		if (!c->start) continue;
		get_cell_width((char *)c->start, (char *)c->end, t->cellpd, 0, 0, &min, &max, c->link_num, NULL, gf_val(NULL, c->bgcolor_str));
		if (min != c->min_width || max < c->max_width) internal((unsigned char *)"check_cell_widths failed");
	}
}
...
do {									\
		...
		if (!c->start) continue;				\
		c->link_num = nl;					\
		get_cell_width((char *)c->start, (char *)c->end, t->cellpd, 0, 0, &c->min_width, &c->max_width, nl, &nl, gf_val(NULL, c->bgcolor_str));\
} while (0)
...
void dst_width(int *p, int n, int w, int *lim)
{
	...
	if (w) {
		if (!lim) internal((unsigned char *)"bug in dst_width");
		lim = NULL;
		s = 0;
		goto again;
	}
...
int get_column_widths(struct table *t)
{
	...
	do {
		...
		for (i = 0; i < t->x; i++) for (j = 0; j < t->y; j++) {
			...
			if (c->colspan + i > t->x) {
				internal((unsigned char *)"colspan out of table");
				return -1;
			}
			if (c->colspan == s) {
				int k, p = 0;
...
void get_table_width(struct table *t)
{
	...
	t->min_t = min;
	t->max_t = max;
	if (min > max) internal((unsigned char *)"min(%d) > max(%d)", min, max);
}
...
void distribute_widths(struct table *t, int width)
{
	...
	if (!t->x) return;
	if (d < 0) {
		internal((unsigned char *)"too small width %d, required %d", width, t->min_t);
		return;
	}
	...
	while (d) {
		...
		for (i = 0; i < t->x; i++) {
			switch (om) {
				...
				case 6:
					...
					break;
				default:
					internal((unsigned char *)"could not expand table");
					goto end2;
			}
			p += w[i];
		}
		...
		if (mii != -1) {
			...
			if (t->w_c[mii] < q) {
				internal((unsigned char *)"shrinking cell");
				t->w_c[mii] = q;
			}
			wq = 1;
			if (d) goto a;
...
void check_table_widths(struct table *t)
{
	...
	for (j = 0; j < t->y; j++) for (i = 0; i < t->x; i++) {
		...
		get_cell_width((char *)c->start, (char *)c->end, t->cellpd, p, 1, &c->x_width, NULL, c->link_num, NULL, NULL);
		if (c->x_width > p) {
			/*int min, max;
			get_cell_width(c->start, c->end, t->cellpd, 0, 0, &min, &max, c->link_num, NULL, NULL);
			internal((unsigned char *)"cell is now wider (%d > %d) min = %d, max = %d, now_min = %d, now_max = %d", c->x_width, p, t->min_c[i], t->max_c[i], min, max);*/
		...
		for (i = 0; i < t->x; i++) for (j = 0; j < t->y; j++) {
			...
			if (c->colspan == s) {
				...
				/*for (k = i; k < i + s; k++) if (w[k] > t->w_c[k]) {
					...
					if (w[k] > t->w_c[k]) {
						if (!c) internal((unsigned char *)"can't shrink cell");
						else goto ag;
					}
				}*/
			} else if (c->colspan > s && c->colspan < ns) ns = c->colspan;
	...
	if (ns > s) {
		/*internal((unsigned char *)"new width(%d) is larger than previous(%d)", ns, s);*/
		mem_free(w);
		return;
	}
	m = -1;
...
void format_table(unsigned char *attr, unsigned char *html, unsigned char *eof, unsigned char **end, void *f)
{
	...
	if ((border = get_num(attr,  (unsigned char *)"border")) == -1) border = has_attr(attr, (unsigned char *)"border") || has_attr(attr, (unsigned char *)"rules") || has_attr(attr, (unsigned char *)"frame");
	/*if (!border) border = 1;*/
	if ((cellsp = get_num(attr,  (unsigned char *)"cellspacing")) == -1) cellsp = gf_val(1, 2);
	if ((cellpd = get_num(attr,  (unsigned char *)"cellpadding")) == -1) {
		vcellpd = gf_val(0, 1);
	...
	if ((al = (char *)get_attr_val(attr, (unsigned char *)"align"))) {
	...
	frame = F_BOX;
	if ((al = (char *)get_attr_val(attr, (unsigned char *)"frame"))) {
	...
	if ((al = (char *)get_attr_val(attr, (unsigned char *)"rules"))) {
	...
	wf = 0;
	if ((width = get_width(attr, (unsigned char *)"width", gf_val(p->data || p->xp, !!gp->data))) == -1) {
	...
	t->bordercolor = get_attr_val(attr, (unsigned char *)"bordercolor");
	t->align = align;
	t->border = border;
	t->cellpd = cellpd;
	t->vcellpd = vcellpd;
	...
	if (p->cy + t->rh != cye) internal((unsigned char *)"size does not match; 1:%d, 2:%d", p->cy + t->rh, cye);
	p->cy = cye;
...
void process_g_table(struct g_part *gp, struct table *t)
{
	...
	if (t->bordercolor && !decode_color(t->bordercolor, &dummy)) {
		...
		if (ta->bg.r + ta->bg.g * 3 + ta->bg.b * 5 > 9 * 128) strcpy((char *)bgstr, "#000000");
			...
			max &= 0xff;
			sprintf((char *)bgstr, "#%02x%02x%02x", max, max, max);
		} else sprintf((char *)bgstr, "#%02x%02x%02x", G_HTML_TABLE_FRAME_COLOR, G_HTML_TABLE_FRAME_COLOR, G_HTML_TABLE_FRAME_COLOR);
