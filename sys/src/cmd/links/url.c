...
struct protocol {
	...
	void	(*nc_func)(struct session *, uchar *);
	int	free_syntax;
	int	need_slashes;
...
int
check_protocol(uchar *p, int l)
{
	int i;

	for (i = 0; protocols[i].prot; i++)
		if (!casecmp((uchar *)protocols[i].prot, p, l))
			return i;
	return -1;
}

int
get_prot_info(uchar *prot, int *port, void (**func)(struct connection *),
	void (**nc_func)(struct session *ses, uchar *))
{
	int i;
...
int
parse_url(uchar *url, int *prlen, uchar **user, int *uslen,
	uchar **pass, int *palen, uchar **host, int *holen,
	uchar **port, int *polen, uchar **data, int *dalen, uchar **post)
{
	unsigned char *p, *q;
	unsigned char p_c[2];
	int a;
	...
	if (dalen) *dalen = 0;
	if (post) *post = NULL;
	if (!url || !(p = (unsigned char *)strchr(( char *)url, ':'))) return -1;
	...
	if (*q == '@') {
		unsigned char *pp =(unsigned char *) strchr(( char *)p, ':');
		if (!pp || pp > q) {
			if (user) *user = p;
			if (uslen) *uslen = q - p;
		} else {
...
unsigned char *get_host_name(unsigned char *url)
{
	unsigned char *h;
	int hl;
	if (parse_url(url, NULL, NULL, NULL, NULL, NULL, &h, &hl, NULL, NULL, NULL, NULL, NULL)) return stracpy((unsigned char *)"");
	return memacpy(h, hl);
}
...
translate_directories(unsigned char *url)
{
	...
	int lo = !casecmp(url, (unsigned char *)"file://", 7);
	if (!casecmp(url, (unsigned char *)"javascript:", 11)) return;
...
void insert_wd(unsigned char **up, unsigned char *cwd)
{
	...
	if (casecmp(url, (unsigned char *)"file://", 7)) return;
	if (dir_sep(url[7])) return;
#ifdef DOS_FS
	if (upcase(url[7]) >= 'A' && upcase(url[7]) <= 'Z' && url[8] == ':' && dir_sep(url[9])) return;
#endif
	if (!(url = mem_alloc(strlen(( char *)*up) + strlen(( char *)cwd) + 2))) return;
	memcpy(url, *up, 7);
	strcpy(( char *)url + 7, ( char *)cwd);
	if (!dir_sep(cwd[strlen(( char *)cwd) - 1])) strcat(( char *)url, "/");
	strcat(( char *)url, ( char *)*up + 7);
	mem_free(*up);
	*up = url;
}
...
unsigned char *join_urls(unsigned char *base, unsigned char *rel)
{
	...
	int l;
	int lo = !casecmp(base, (unsigned char *)"file://", 7);
	if (rel[0] == '#') {
	...
	if (rel[0] == '/' && rel[1] == '/') {
		unsigned char *s, *n;
		if (!(s =(unsigned char *) strstr(( char *)base, "//"))) {
			internal((unsigned char *)"bad base url: %s", base);
			return NULL;
		}
	...
	if (!casecmp((unsigned char *)"proxy://", rel, 8)) goto prx;
...
prx:
	if (parse_url(base, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, &p, NULL, NULL) || !p) {
		internal((unsigned char *)"bad base url");
		return NULL;
	}
	if (!dsep(*p)) p--;
	...
	if (!(n = mem_alloc(p - base + strlen(( char *)rel) + 1))) return NULL;
	memcpy(n, base, p - base);
	strcpy(( char *)n + (p - base), ( char *)rel);
	translate_directories(n);
	return n;
}
...
unsigned char *translate_url(unsigned char *url, unsigned char *cwd)
{
	...
	unsigned char *nu, *da;
	while(*url == ' ') url++;
	if (!casecmp((unsigned char *)"proxy://", url, 8)) goto prx;
	...
	if (strstr((char *)url, "//") && (nu = stracpy(url))) {
		add_to_strn(&nu, (unsigned char *)"/");
...
prx:
	...
	if (*ch != ':' || *(url + strcspn(( char *)url, "/@")) == '@') {
		unsigned char *prefix = (unsigned char *)"file://";
		int sl = 0;
		...
		if (*url != '.' && *ch == '.') {
			...
			if (f - e == 2)
http:
				prefix = (uchar *)"http://", sl = 1;
			else {
				static char *tld[] = {
					"com", "edu", "net", "org", "gov",
					"mil", "int", "arpa", "aero", "biz",
					"coop", "info", "museum", "name",
					"pro", NULL
				};
				for (i = 0; tld[i]; i++)
					if (f - e == strlen(tld[i]) &&
					    !casecmp((uchar *)tld[i], e, f - e))
						goto http;
			}
		}
		if (*ch == '@' || *ch == ':' || !cmpbeg(url, (unsigned char *)"ftp.")) prefix = (unsigned char *)"ftp://", sl = 1;
		if (!(nu = stracpy(prefix))) return NULL;
		add_to_strn(&nu, url);
		if (sl && !strchr(( char *)url, '/')) add_to_strn(&nu,(unsigned char *) "/");
...
unsigned char *extract_position(unsigned char *url)
{
	...
	if (!(u = (unsigned char *)strchr(( char *)url, POST_CHAR))) u = url + strlen(( char *)url);
	uu = u;
	while (--uu >= url && *uu != '#') ;
