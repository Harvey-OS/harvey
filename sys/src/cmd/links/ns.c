...
{	lns*current=(pns->ns)[key%HASHNUM],*pom=0;
	...
	{       debug("promenna nalezena\n");
		...
		if(!zamecnictvi_u_sperhaku)
			my_internal((unsigned char *)"Out of namespace!",context);
		...
		{	current->mid=sperhaky[0];
			...
			{	case JS_OBJ_T_TEXT:	current->value=Ctextptr;
				...
				break;
				default:	my_internal((unsigned char *)"Semerad! to je typek!\n",context); 
...
{	lns *current=(pns->ns)[key%HASHNUM],*pom=0;
	...
	{	debug("promenna nalezena\n");
		...
		current->identifier=key;
		current->mid=pns->mid;
		if(pns->handler<0)internal((unsigned char *)"Handler<0\n");
		current->handler=pns->handler;
		current->next=pns->ns[key%HASHNUM];
		pns->ns[key%HASHNUM]=current;
		current->type=UNDEFINED;
...
{	lns ** ns=pns->ns,*pom,*p1;
	int i=0;
	js_mem_free(pns);
	if(!ns)	my_internal((unsigned char *)"Freeing NULL addrspace!\n",context);
	while(i<HASHNUM)
	{	pom=ns[i++];
	...
	{	pom=ns[i++];
		while(pom)			
		{	if(pom->type==PARLIST) 
				my_internal((unsigned char *)"Too many parentlists!\n",context) /*;*/
			else
	...
	{	if(pom->parent==parent)
		...
		{	prev=pom;
			...
			{	prev->next=pom->next;
				...
				internal((unsigned char *)"Internal: Parent not in list\n");
		}
	} else 	internal((unsigned char *)"Internal: Performing delete_from_parlist to empty list!\n");
}
