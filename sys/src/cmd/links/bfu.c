...
#else
#define txtlen(x) (F ? g_text_width(bfu_style_wb, (unsigned char *)x) : strlen((char *)x))
#endif
...
void init_bfu(void)
{
	...
	bfu_style_wb = g_get_style(G_BFU_BG_COLOR, G_BFU_FG_COLOR, G_BFU_FONT_SIZE, (unsigned char *)G_BFU_DEFAULT_FONT, 0);
	bfu_style_wb_b = g_get_style(G_BFU_BG_COLOR, G_BFU_FG_COLOR, G_BFU_FONT_SIZE, (unsigned char *)G_BFU_DEFAULT_FONT, 0);
	bfu_style_bw = g_get_style(G_BFU_FG_COLOR, G_BFU_BG_COLOR, G_BFU_FONT_SIZE, (unsigned char *)G_BFU_DEFAULT_FONT, 0);
	bfu_style_bw_u = g_get_style(G_BFU_FG_COLOR, G_BFU_BG_COLOR, G_BFU_FONT_SIZE, (unsigned char *)G_BFU_DEFAULT_FONT, FF_UNDERLINE);
	bfu_style_bw_mono = g_get_style(G_BFU_FG_COLOR, G_BFU_BG_COLOR, G_BFU_FONT_SIZE, (unsigned char *)"monospaced", 0);
	bfu_style_wb_mono = g_get_style(G_BFU_BG_COLOR, G_BFU_FG_COLOR, G_BFU_FONT_SIZE, (unsigned char *)"monospaced", 0);
	bfu_style_wb_mono_u = g_get_style(G_BFU_BG_COLOR, G_BFU_FG_COLOR, G_BFU_FONT_SIZE, (unsigned char *)"monospaced", FF_UNDERLINE);
	...
	if ((menu = mem_alloc(sizeof(struct menu)))) {
		...
		if (F) {
			...
			for (i = 0; i < n; i++) {
				...
				menu->hktxt2[i] = stracpy((unsigned char *)"");
				menu->hktxt3[i] = stracpy((unsigned char *)"");
				x:;
			}
		}
#endif
...
void count_menu_size(struct terminal *term, struct menu *menu)
{
	...
	for (my = 0; menu->items[my].text; my++) {
		int s = txtlen((char *)_(menu->items[my].text, term)) + txtlen((char *)_(menu->items[my].rtext, term)) + gf_val(MENU_HOTKEY_SPACE, G_MENU_HOTKEY_SPACE) * (_(menu->items[my].rtext, term)[0] != 0) + gf_val(4, 2 * (G_MENU_LEFT_BORDER + G_MENU_LEFT_INNER_BORDER));
		if (s > mx) mx = s;
	}
	menu->ni = my;
...
void display_menu_txt(struct terminal *term, struct menu *menu)
{
	...
	for (p = menu->view, s = menu->y + 1; p < menu->ni && p < menu->view + menu->yw - 2; p++, s++) {
		...
		if (menu->items[p].hotkey != M_BAR || (tmptext[0])) {
			int l = strlen((char *)_(menu->items[p].rtext, term));
			...
			for (x = 0; x < menu->xw - 4 && (c = tmptext[x]); x++)
				set_char(term, menu->x + x + 2, s, !h && strchr((char *)_(menu->items[p].hotkey, term), upcase(c)) ? h = 1, COLOR_MENU_HOTKEY | c : co | c);
		} else {
	...
	switch (ev->ev) {
		...
		case EV_KBD:
			...
			if (ev->x > ' ' && ev->x < 256) {
				int i;
				for (i = 0; i < menu->ni; i++)
					if (strchr((char *)_(menu->items[i].hotkey, win->term), upcase(ev->x))) {
						menu->selected = i;
						scroll_menu(menu, 0);
						s = 1;
					}
	...
	if (!F) {
		...
		for (i = 0; menu->items[i].text; i++) {
			...
			if (s) {
				...
				fill_area(term, p+strlen((char *)tmptext)+2, 0, 2, 1, co);
				menu->sp = p;
			...
			for (j = 0; (c = tmptext[j]); j++, p++)
				set_char(term, p, 0, (!s && strchr((char *)_(menu->items[i].hotkey, term), upcase(c)) ? s = 1, COLOR_MAINMENU_HOTKEY : co) | c);
			p += 2;
		}
		menu->ni = i;
#ifdef G
		...
		case EV_MOUSE:
			...
			if ((ev->b & BM_ACT) == B_DOWN && ev->y >= gf_val(1, G_BFU_FONT_SIZE)) delete_window_ev(win, ev);
				...
				for (i = 0; i < menu->ni; i++) {
					...
					p += txtlen((char *)tmptext) + gf_val(4, 2 * G_MAINMENU_BORDER);
					if (ev->x >= o && ev->x < p) {
						menu->selected = i;
						draw_to_window(win, (void (*)(struct terminal *, void *))display_mainmenu, menu);
...
void display_dlg_item(struct dialog_data *dlg, struct dialog_item_data *di, int sel)
{
	...
	if (!F) switch (di->item->type) {
		...
		case D_CHECKBOX:
			...
			{
				if (di->checked) print_text(term, di->x, di->y, 3, (unsigned char *)"[X]", COLOR_DIALOG_CHECKBOX);
				else print_text(term, di->x, di->y, 3, (unsigned char *)"[ ]", COLOR_DIALOG_CHECKBOX);
			}
			else	/* checkbox */
			{
				if (di->checked) print_text(term, di->x, di->y, 3, (unsigned char *)"[X]", COLOR_DIALOG_CHECKBOX);
		...
		case D_FIELD_PASS:
			...
			print_text(term, di->x, di->y, strlen((char *)(t + di->vpos)) <= di->l ? strlen((char *)(t + di->vpos)) : di->l, t + di->vpos, COLOR_DIALOG_FIELD_TEXT);
		...
		case D_BUTTON:
			...
			print_text(term, di->x, di->y, 2, (unsigned char*)"[ ", co);
			print_text(term, di->x + 2, di->y, strlen((char *)text), text, co);
			print_text(term, di->x + 2 + strlen((char*)text), di->y, 2, (unsigned char*)" ]", co);
			if (sel) {
			...
			break;
		default:
			internal((unsigned char *)"display_dlg_item: unknown item: %d", di->item->type);
#ifdef G
	} else {
		struct rect rr;
		struct graphics_device *dev = term->dev;
		...
		switch (di->item->type) {
			...
			case D_CHECKBOX:
				p = di->x;
				if (di->checked) {
					if (!sel) g_print_text(drv, dev, di->x, di->y, bfu_style_bw, (unsigned char *)(di->item->gid?(G_DIALOG_RADIO_L G_DIALOG_RADIO_X G_DIALOG_RADIO_R):(G_DIALOG_CHECKBOX_L G_DIALOG_CHECKBOX_X G_DIALOG_CHECKBOX_R)), &p);
					else {
						g_print_text(drv, dev, di->x, di->y, bfu_style_bw, di->item->gid?(unsigned char *)G_DIALOG_RADIO_L:(unsigned char *)G_DIALOG_CHECKBOX_L, &p);
						g_print_text(drv, dev, p, di->y, bfu_style_bw_u, di->item->gid?(unsigned char *)G_DIALOG_RADIO_X:(unsigned char *)G_DIALOG_CHECKBOX_X, &p);
						g_print_text(drv, dev, p, di->y, bfu_style_bw, di->item->gid?(unsigned char *)G_DIALOG_RADIO_R:(unsigned char *)G_DIALOG_CHECKBOX_R, &p);
					}
				} else {
					int s = g_text_width(bfu_style_bw, di->item->gid?(unsigned char *)G_DIALOG_RADIO_X:(unsigned char *)G_DIALOG_CHECKBOX_X);
					g_print_text(drv, dev, di->x, di->y, bfu_style_bw, di->item->gid?(unsigned char *)G_DIALOG_RADIO_L:(unsigned char *)G_DIALOG_CHECKBOX_L, &p);
					if (!sel) drv->fill_area(dev, p, di->y, p + s, di->y + G_BFU_FONT_SIZE, bfu_bg_color), p += s;
						...
						g_print_text(drv, dev, p, di->y, bfu_style_bw_u, (unsigned char *)"          ", NULL);
						p += s;
						drv->set_clip_area(dev, &r);
					}
					g_print_text(drv, dev, p, di->y, bfu_style_bw, di->item->gid?(unsigned char *)G_DIALOG_RADIO_R:(unsigned char *)G_DIALOG_CHECKBOX_R, &p);
			...
			case D_BUTTON:
				...
				text = _(di->item->text, term);
				if ((text2 = mem_alloc(strlen((char *)text) + 5))) {
					strcpy((char *)text2, G_DIALOG_BUTTON_L);
					strcpy((char *)text2 + 2, (char *)text);
					strcat((char *)text2, G_DIALOG_BUTTON_R);
					di->l = 0;
...
void dlg_set_history(struct dialog_item_data *di)
{
	...
	if ((l = strlen((char *)s)) > di->item->dlen) l = di->item->dlen - 1;
	memcpy(di->cdata, s, l);
	di->cdata[l] = 0;
	di->cpos = l;
}
...
int dlg_mouse(struct dialog_data *dlg, struct dialog_item_data *di, struct event *ev)
{
	switch (di->item->type) {
		case D_BUTTON:
			if (gf_val(ev->y != di->y, ev->y < di->y || ev->y >= di->y + G_BFU_FONT_SIZE) || ev->x < di->x || ev->x >= di->x + gf_val(strlen((char *)_(di->item->text, dlg->win->term)) + 4, di->l)) return 0;
		...
		case D_FIELD_PASS:
			...
			if (!F) {
				if ((di->cpos = di->vpos + ev->x - di->x) > strlen((char *)di->cdata)) di->cpos = strlen((char *)di->cdata);
#ifdef G
			} else {
				int p, u;
...
void do_tab_compl(struct terminal *term, struct list_head *history, struct window *win)
{
	...
	int l = strlen((char *)cdata), n = 0;
	struct history_item *hi;
	struct menu_item *items = DUMMY, *i;
	foreach(hi, *history) if (!strncmp((char *)cdata, (char *)hi->d, l)) {
		if (!(n & (ALLOC_GR - 1))) {
		...
		items[n].text = hi->d;
		items[n].rtext = (unsigned char *)"";
		items[n].hotkey = (unsigned char *)"";
	...
	switch (ev->ev) {
				...
				if (di->item->type == D_FIELD || di->item->type == D_FIELD_PASS) {
					if (di->item->history) {
						...
						foreach(j, di->item->history->items) {
							struct history_item *hi;
							if (!(hi = mem_alloc(sizeof(struct history_item) + strlen((char *)j->d) + 1))) continue;
							strcpy((char *)hi->d, (char *)j->d);
							add_to_list(di->history, hi);
						}
					}
					di->cpos = strlen((char *)di->cdata);
				}
			}
			dlg->selected = 0;
		case EV_RESIZE:
		...
		case EV_KBD:
			...
			if (di->item->type == D_FIELD || di->item->type == D_FIELD_PASS) {
				...
				if (ev->x == KBD_RIGHT) {
					if (di->cpos < strlen((char *)di->cdata)) {
						if (!F) di->cpos++;
#ifdef G
						else {
							int u;
...
#endif
					if (strlen((char *)di->cdata) < di->item->dlen - strlen((char *)u)) {
						memmove(di->cdata + di->cpos + strlen((char *)u), di->cdata + di->cpos, strlen((char *)di->cdata) - di->cpos + 1);
						memcpy(&di->cdata[di->cpos], u, strlen((char *)u));
						di->cpos += strlen((char *)u);
					}
					goto dsp_f;
				}
				if (ev->x == KBD_BS) {
...
#endif
						memmove(di->cdata + di->cpos - s, di->cdata + di->cpos, strlen((char *)di->cdata) - di->cpos + s);
						di->cpos -= s;
					}
					goto dsp_f;
				}
...
#endif
						memmove(di->cdata + di->cpos, di->cdata + di->cpos + s, strlen((char *)di->cdata) - di->cpos + s);
					}
					goto dsp_f;
				}
				if (upcase(ev->x) == 'U' && ev->y == KBD_CTRL) {
					memmove(di->cdata, di->cdata + di->cpos, strlen((char *)di->cdata + di->cpos) + 1);
					di->cpos = 0;
					goto dsp_f;
				}
				/* Copy to clipboard */
				...
				if ((ev->x == KBD_INS && ev->y == KBD_SHIFT) || (upcase(ev->x) == 'V' && ev->y == KBD_CTRL)) {
					...
					if (clipboard) {
						strncpy((char *)di->cdata , (char *)clipboard, di->item->dlen);
...
{
	unsigned char *end;
	double d = strtod((char *)di->cdata, (char **)(void *)&end);
	if (!*di->cdata || *end) {
...
{
	unsigned char *end;
	long l = strtol((char *)di->cdata, (char **)(void *)&end, 10);
	if (!*di->cdata || *end) {
		msg_box(dlg->win->term, NULL, TEXT(T_BAD_NUMBER), AL_CENTER, TEXT(T_NUMBER_EXPECTED), NULL, 1, TEXT(T_CANCEL), NULL, B_ENTER | B_ESC);
		return 1;
	}
...
{
	unsigned char *end;
	long l = strtol((char *)di->cdata, (char **)(void *)&end, 16);
	if (!*di->cdata || *end) {
		msg_box(dlg->win->term, NULL, TEXT(T_BAD_NUMBER), AL_CENTER, TEXT(T_NUMBER_EXPECTED), NULL, 1, TEXT(T_CANCEL), NULL, B_ENTER | B_ESC);
		return 1;
	}
...
void draw_dlg(struct dialog_data *dlg)
{
	if (!F) {
		...
		i = strlen((char *)_(dlg->dlg->title, term));
		print_text(term, (dlg->xw - i) / 2 + dlg->x - 1, dlg->y + DIALOG_TOP_BORDER, 1, (unsigned char *)" ", COLOR_DIALOG_TITLE);
		print_text(term, (dlg->xw - i) / 2 + dlg->x, dlg->y + DIALOG_TOP_BORDER, i, _(dlg->dlg->title, term), COLOR_DIALOG_TITLE);
		print_text(term, (dlg->xw - i) / 2 + dlg->x + i, dlg->y + DIALOG_TOP_BORDER, 1, (unsigned char *)" ", COLOR_DIALOG_TITLE);
#ifdef G
	} else {
	...
	if (!F) do {
...
#ifdef G
	else if ((tx2 = (unsigned char *)strchr(( char *)text, '\n'))) {
		...
		do {
			*tx2 = 0;
			...
			tx1 = tx2 + 1;
		} while ((tx2 = (unsigned char *)strchr((char *)tx1, '\n')));
		dlg_format_text(dlg, term, tx1, x, y, w, rw, co, align);
		mem_free(txt);
	} else {
		int www;
		...
		if (!r) {
			...
			text = ww.last_wrap;
			if (*text == ' ') text++;
			else if (term) add_to_strn(&txt, (unsigned char *)"-");
		} else {
			www = ww.pos;
			txt = text;
		}
...
void max_buttons_width(struct terminal *term, struct dialog_item_data *butt, int n, int *width)
{
	...
	int i;
	for (i = 0; i < n; i++) w += txtlen((char*)_((butt++)->item->text, term)) + gf_val(6, G_DIALOG_BUTTON_SPACE + txtlen(G_DIALOG_BUTTON_L) + txtlen(G_DIALOG_BUTTON_R));
	if (w > *width) *width = w;
}
...
void min_buttons_width(struct terminal *term, struct dialog_item_data *butt, int n, int *width)
{
	int i;
	for (i = 0; i < n; i++) {
		int w = txtlen((char*)_((butt++)->item->text, term)) + gf_val(4, txtlen(G_DIALOG_BUTTON_L G_DIALOG_BUTTON_R));
		if (w > *width) *width = w;
	}
}
	...
	while (i1 < n) {
		...
		if (term) {
			...
			for (i = i1; i < i2; i++) {
				butt[i].x = p;
				butt[i].y = *y;
				p += (butt[i].l = txtlen((char*)_(butt[i].item->text, dlg->win->term)) + gf_val(4, txtlen(G_DIALOG_BUTTON_L G_DIALOG_BUTTON_R))) + gf_val(2, G_DIALOG_BUTTON_SPACE);
			}
		}
...
void max_group_width(struct terminal *term, unsigned char **texts, struct dialog_item_data *item, int n, int *w)
{
	...
	while (n--) {
		int wx = item->item->type == D_CHECKBOX ? gf_val(4, txtlen(G_DIALOG_CHECKBOX_L G_DIALOG_CHECKBOX_X G_DIALOG_CHECKBOX_R) + G_DIALOG_CHECKBOX_SPACE) :
			item->item->type == D_BUTTON ? txtlen((char*)_(item->item->text, term)) + (gf_val(4, txtlen((char*)G_DIALOG_BUTTON_L G_DIALOG_BUTTON_R))) :
			gf_val(item->item->dlen + 1, (item->item->dlen + 1) * G_DIALOG_FIELD_WIDTH);
		wx += txtlen((char*)_(texts[0], term));
...
void min_group_width(struct terminal *term, unsigned char **texts, struct dialog_item_data *item, int n, int *w)
{
	while (n--) {
		int wx = item->item->type == D_CHECKBOX ? gf_val(4, txtlen(G_DIALOG_CHECKBOX_L G_DIALOG_CHECKBOX_X G_DIALOG_CHECKBOX_R) + G_DIALOG_CHECKBOX_SPACE) :
			item->item->type == D_BUTTON ? txtlen((char*)_(item->item->text, term)) + (gf_val(4, txtlen(G_DIALOG_BUTTON_L G_DIALOG_BUTTON_R))) :
			gf_val(item->item->dlen + 1, (item->item->dlen + 1) * G_DIALOG_FIELD_WIDTH);
		wx += txtlen((char*)_(texts[0], term));
		if (wx > *w) *w = wx;
		texts++;
...
void dlg_format_group(struct dialog_data *dlg, struct terminal *term, unsigned char **texts, struct dialog_item_data *item, int n, int x, int *y, int w, int *rw)
{
	int nx = 0;
	while (n--) {
		int wx = item->item->type == D_CHECKBOX ? gf_val(3, txtlen((char*)G_DIALOG_CHECKBOX_L G_DIALOG_CHECKBOX_X G_DIALOG_CHECKBOX_R)) :
			item->item->type == D_BUTTON ? txtlen((char*)_(item->item->text, dlg->win->term)) + (gf_val(4, txtlen((char*)G_DIALOG_BUTTON_L G_DIALOG_BUTTON_R))) :
			gf_val(item->item->dlen, item->item->dlen * G_DIALOG_FIELD_WIDTH);
		int sl;
		if (_(texts[0], dlg->win->term)[0]) sl = txtlen((char*)_(texts[0], dlg->win->term)) + gf_val(1, G_DIALOG_GROUP_TEXT_SPACE);
		else sl = 0;
		wx += sl;
		if (nx && nx + wx > w) {
			nx = 0;
		...
		if (term) {
			if (!F) print_text(term, x + nx + 4 * (item->item->type == D_CHECKBOX), *y, strlen((char *)_(texts[0], dlg->win->term)), _(texts[0], dlg->win->term), COLOR_DIALOG_TEXT);
#ifdef G
			else {
				int l, ll;
				l = ll = x + nx + (item->item->type == D_CHECKBOX ? txtlen((char*)G_DIALOG_CHECKBOX_L G_DIALOG_CHECKBOX_X G_DIALOG_CHECKBOX_R) + G_DIALOG_GROUP_TEXT_SPACE : 0);
...
void add_to_history(struct history *h, unsigned char *t)
{
	...
	memcpy(hi->d, t, l);
	foreach(hs, h->items) if (!strcmp((char*)hs->d, (char*)t)) {
	...
	while (h->n > MAX_HISTORY_ITEMS) {
		...
		if ((void *)hd == &h->items) {
			internal((unsigned char*)"history is empty");
			h->n = 0;
			return;
		}
		del_from_list(hd);
...
void input_field(struct terminal *term, struct memory_list *ml, unsigned char *title, unsigned char *text, unsigned char *okbutton, unsigned char *cancelbutton, void *data, struct history *history, int l, unsigned char *def, int min, int max, int (*check)(struct dialog_data *, struct dialog_item_data *), void (*fn)(void *, unsigned char *), void (*cancelfn)(void *))
{
	...
	*(field = (unsigned char *)dlg + sizeof(struct dialog) + 4 * sizeof(struct dialog_item)) = 0;
	if (def) {
		if (strlen((char *)def) + 1 > l) memcpy(field, def, l - 1);
		else strcpy((char*)field, (char*)def);
	}
	dlg->title = title;
