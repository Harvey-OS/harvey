...
struct h_conn *is_host_on_list(struct connection *c)
{
	...
	foreach(h, h_conns) if (!strcmp(( char *)h->host, ( char *)ho)) {
		mem_free(ho);
		return h;
	}
	mem_free(ho);
...
struct k_conn *is_host_on_keepalive_list(struct connection *c)
{
	...
	foreach(h, keepalive_connections)
		if (h->protocol == ph && h->port == po && !strcmp(( char *)h->host, ( char *)ho)) {
			mem_free(ho);
			return h;
		}
	mem_free(ho);
...
void
free_connection_data(Connection *c)
{
	struct h_conn *h;
	...
	if (!c->running)
		internal((uchar *)"connection already suspended");
	c->running = 0;
	if (c->dnsquery)
	...
	if (c->timer != -1)
		kill_timer(c->timer), c->timer = -1;
	if (--active_connections < 0) {
		internal((unsigned char *)"active connections underflow");
	...
	if (c->state != S_WAIT)
		if ((h = is_host_on_list(c))) {
			...
			internal((uchar *)"suspending connection that is not on the list (state %d)",
				c->state);
}
...
void add_keepalive_socket(struct connection *c, ttime timeout)
{
	...
	if (c->sock1 == -1) {
		internal((unsigned char *)"keepalive connection not connected");
		goto del;
	}
...
void check_keepalive_connections(void)
{
	...
	for (; p > MAX_KEEPALIVE_CONNECTIONS; p--)
		...
		else internal((unsigned char *)"keepalive list empty");
...
static void
interrupt_connection(Connection *c)
{
#ifdef HAVE_SSL
	if (c->ssl == (void *)-1)
		c->ssl = NULL;
...
run_connection(struct connection *c)
{
	...
	if (c->running) {
		internal((unsigned char *)"connection already running");
		return;
	}
...
void
retry_connection(Connection *c)
{
	interrupt_connection(c);
	if (c->unrestartable >= 2 ||
	    ++c->tries >= (max_tries? max_tries: 1000)) {
		/* send_connection_info(c); */
		del_connection(c);
#ifdef DEBUG
		check_queue_bugs();
...
again:
	...
	foreach(d, queue) {
		...
		cc += d->running;
		if (q < p) if (!ps) {
			internal((unsigned char *)"queue is not sorted");
			sort_queue();
			ps = 1;
			goto again;
		} else {
			internal((unsigned char *)"queue is not sorted even after sort_queue!");
			break;
		} else p = q;
		if (d->state < 0) {
			internal((unsigned char *)"interrupted connection on queue (conn %s, state %d)", d->url, d->state);
			d = d->prev;
			abort_connection(d->next);
		}
	}
	if (cc != active_connections) {
		internal((unsigned char *)"bad number of active connections (counted %d, stored %d)", cc, active_connections);
...
#endif
	check_keepalive_connections();
	while (c != (Connection *)&queue) {
		Connection *d;
		int cp = getpri(c);

		for (d = c; d != (Connection *)&queue && getpri(d) == cp; ) {
			Connection *dd = d;
...
get_proxy(unsigned char *url)
{
	...
	unsigned char *u;
	if (*http_proxy && l >= 7 && !casecmp(url, (unsigned char *)"http://", 7)) proxy = http_proxy;
	if (*ftp_proxy && l >= 6 && !casecmp(url, (unsigned char *)"ftp://", 6)) proxy = ftp_proxy;
	if (!(u = mem_alloc(l + 1 + (proxy ? strlen(( char *)proxy) + 9 : 0)))) return NULL;
	if (proxy) strcpy(( char *)u, "proxy://"), strcat(( char *)u, ( char *)proxy), strcat(( char *)u, "/");
	else *u = 0;
	strcat(( char *)u, ( char *)url);
	return u;
}
	...
	int pri, int no_cache)
{
	...
	if (stat)
		stat->c = NULL, stat->ce = NULL, stat->pri = pri;
#ifdef DEBUG
	foreach(c, queue) {
		struct status *st;
		...
		foreach (st, c->statuss) {
			if (st == stat) {
				internal((unsigned char *)"status already assigned to '%s'", c->url);
...
#endif
	if (stat)
		stat->state = S_OUT_OF_MEM, stat->prev_error = 0;
...
skip_cache:
	...
	foreach(c, queue)
		if (!c->detached && !strcmp(( char *)c->url, ( char *)u)) {
			mem_free(u);
			if (getpri(c) > pri) {
				del_from_list(c);
				c->pri[pri]++;
	...
	c->running = 0;
	c->prev_error = 0;
	c->from = no_cache >= NC_IF_MOD || !e || e->frag.next == &e->frag ||
		((struct fragment *)e->frag.next)->offset?
		0: ((struct fragment *)e->frag.next)->length;
...
{		/* !!! FIXME: one object in more connections */
	...
	int oldpri;
	if (!oldstat) {
		internal((unsigned char *)"change_connection: oldstat == NULL");
		return;
	}
	oldpri = oldstat->pri;
	if (oldstat->state < 0) {
	...
	c = oldstat->c;
	if (--c->pri[oldpri] < 0) {
		internal((unsigned char *)"priority counter underflow");
		c->pri[oldpri] = 0;
	}
	c->pri[newpri]++;
...
void detach_connection(struct status *stat, int pos)
{
	...
	if (!l) internal((unsigned char *)"detaching free connection");
...
void add_blacklist_entry(unsigned char *host, int flags)
{
	...
	foreach(b, blacklist) if (!strcasecmp(( char *)host, ( char *)b->host)) {
		b->flags |= flags;
		return;
	}
	if (!(b = mem_alloc(sizeof(struct blacklist_entry) + strlen(( char *)host) + 1))) return;
	b->flags = flags;
	strcpy(( char *)b->host, ( char *)host);
...
{
	struct blacklist_entry *b;
	foreach(b, blacklist) if (!strcasecmp(( char *)host, ( char *)b->host)) return b->flags;
	return 0;
}
