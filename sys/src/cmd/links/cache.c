...
{
	unsigned char *a;
	if (strlen((char *)url) < 8 || casecmp(url, (unsigned char *)"proxy://", 8)) return url;
	if (!(a = (unsigned char *)strchr((char *)url + 8, '/'))) return url;
	return a + 1;
}
...
int find_in_cache(unsigned char *url, struct cache_entry **f)
{
	...
	foreach(e, cache) if (!strcmp((char *)e->url, (char *)url)) {
		del_from_list(e);
		add_to_list(cache, e);
		*f = e;
		return 0;
...
int get_cache_entry(unsigned char *url, struct cache_entry **f)
{
	...
	if (!(e->url = mem_alloc(strlen((char *)url)+1))) {
		mem_free(e);
		return -1;
	}
...
void defrag_entry(struct cache_entry *e)
{
	...
	for (g = f->next; g != (void *)&e->frag && g->offset <= g->prev->offset+g->prev->length; g = g->next) if (g->offset < g->prev->offset+g->prev->length) {
		internal((unsigned char*)"fragments overlay");
		return;
	}
	if (g == f->next) return;
...
void delete_entry_content(struct cache_entry *e)
{
	...
	if ((cache_size -= e->data_size) < 0) {
		internal((unsigned char*)"cache_size underflow: %ld", cache_size);
		cache_size = 0;
	}
	e->data_size = 0;
	if (e->last_modified) {
...
{
	if (e->refcount) internal((unsigned char*)"deleteing locked cache entry");
#ifdef DEBUG
	if (is_entry_used(e)) internal((unsigned char*)"deleting loading cache entry");
#endif
...
int shrink_file_cache(int u)
{
	...
	foreach(e, cache) {
		if (e->refcount || is_entry_used(e)) {
			if ((ncs -= e->data_size) < 0) {
				internal((unsigned char*)"cache_size underflow: %ld", ncs);
				ncs = 0;
			}
	...
	if (ccs != cache_size) internal((unsigned char*)"cache size badly computed: %ld != %ld", cache_size, ccs), cache_size = ccs;
	...
	if (/*!no &&*/ ncs) internal((unsigned char*)"cache_size(%ld) is larger than size of all objects(%ld)", cache_size, cache_size - ncs);
	g:
	...
	/*if (!no && cache_size > memory_cache_size * MEMORY_CACHE_GC_PERCENT) {
		internal((unsigned char*)"garbage collection doesn't work, cache size %ld", cache_size);
	}*/
	ret:
...
{
	register_cache_upcall(shrink_file_cache, (unsigned char *)"file");
}
