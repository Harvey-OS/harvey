...
int draw_bfu_element(struct terminal * term, int x, int y, unsigned c, long b, long f, unsigned char type, unsigned char selected)
{
	if (!F){
		...
		switch (type)
		{
			...
			default:
			internal((unsigned char *)"draw_bfu_element: unknown BFU element type %d.\n",type);
		}
...
int list_item_button(struct dialog_data *dlg, struct dialog_item_data *useless)
{
	...
	if (!(ld->button_fn))internal((unsigned char *)"Links got schizophrenia! Call "BOHNICE".\n");
...
int list_item_delete(struct dialog_data *dlg,struct dialog_item_data *useless)
{
	...
	if (!txt)
	{
		...
		if (!txt)internal((unsigned char *)"Cannot allocate memory.\n");
		*txt=0;
	}
		...
		for (a=0,l=ld->win_offset;a<ld->n_items;)
		{
			...
			if (l->depth>=0)  /* everything except head */
			{
				switch((l->type)&1)
				{
					...
					internal((unsigned char *)"=8-Q  lunacy level too high! Call "BOHNICE".\n");
...
void scroll_list(struct terminal *term, void *bla)
{
	...
	else
	{
		...
		switch(direction)
		{
			...
			default:
			internal((unsigned char *)"Wrong direction %d in function scroll_list.\n",direction);
		}
...
int list_event_handler(struct dialog_data *dlg, struct event *ev)
{
	...
	switch(ev->ev)
	{
		...
		if (ev->x=='/') /* search forward */
		{
			...
			input_field(ses->term, getml(r,NULL), TEXT(T_SEARCH), TEXT(T_SEARCH_FOR_TEXT), TEXT(T_OK), TEXT(T_CANCEL), r, ld->search_history, MAX_INPUT_URL_LEN, (unsigned char *)"", 0, 0, NULL, (void (*)(void *, unsigned char *)) list_search_for, NULL);
			return EVENT_PROCESSED;
		}
		if (ev->x=='?') /* search back */
		{
			...
			input_field(ses->term, getml(r,NULL), TEXT(T_SEARCH_BACK), TEXT(T_SEARCH_FOR_TEXT), TEXT(T_OK), TEXT(T_CANCEL), r, ld->search_history, MAX_INPUT_URL_LEN, (unsigned char *)"", 0, 0, NULL, (void (*)(void *, unsigned char *)) list_search_for_back, NULL);
			return EVENT_PROCESSED;
		}
		if (ev->x=='n') /* find next */
		{
		...
		default:
		internal((unsigned char *)"Unknown event received: %d", ev->ev);
