...
int get_ftp_response(struct connection *c, struct read_buffer *rb, int part)
{
	...
	for (l = 0; l < rb->len; l++) if (rb->data[l] == 10) {
		unsigned char *e;
		int k = strtoul((char *)rb->data, (char **)(void *)&e, 10);
...
void ftp_login(struct connection *c)
{
	...
	add_to_str(&login, &logl, (unsigned char *)"USER ");
	if ((u = get_user_name(c->url)) && *u) add_to_str(&login, &logl, u);
	else add_to_str(&login, &logl, (unsigned char *)"anonymous");
	if (u) mem_free(u);
	if (fast_ftp) {
		struct ftp_connection_info *fi;
		add_to_str(&login, &logl, (unsigned char *)"\r\nPASS ");
	...
	} else add_to_str(&login, &logl, (unsigned char *)"\r\n");
	write_to_socket(c, c->sock1, login, strlen((char *)login), ftp_logged);
	mem_free(login);
	setcstate(c, S_SENT);
}
...
void ftp_got_user_info(struct connection *c, struct read_buffer *rb)
{
	...
	if (g >= 200 && g < 300) {
		...
		if (fast_ftp) ftp_pass_info(c, rb);
			...
			add_to_str(&login, &logl, (unsigned char *)"PASS ");
			if ((u = get_pass(c->url)) && *u) add_to_str(&login, &logl, u);
			else add_to_str(&login, &logl, default_anon_pass);
			if (u) mem_free(u);
			add_to_str(&login, &logl, (unsigned char *)"\r\n");
			write_to_socket(c, c->sock1, login, strlen((char *)login), ftp_sent_passwd);
			mem_free(login);
			setcstate(c, S_LOGIN);
		}
	}
...
struct ftp_connection_info *add_file_cmd_to_str(struct connection *c)
{
	...
	if (!d) {
		internal((unsigned char *)"get_url_data failed");
		setcstate(c, S_INTERNAL);
		abort_connection(c);
		return NULL;
	}
	if (!(de = (unsigned char *)strchr(( char *)d, POST_CHAR))) de = d + strlen(( char *)d);
	if (d == de || de[-1] == '/') {
		inf->dir = 1;
		inf->pending_commands = 4;
		add_to_str(&s, &l, (unsigned char *)"TYPE A\r\n");
		if (!inf->pasv) {
			add_to_str(&s, &l, (unsigned char *)"PORT ");
			...
			add_to_str(&s, &l, (unsigned char *)"PASV\r\n");
		}
		add_to_str(&s, &l, (unsigned char *)"CWD /");
		...
		add_to_str(&s, &l, (unsigned char *)"TYPE I\r\n");
		if (!inf->pasv) {
			add_to_str(&s, &l, (unsigned char *)"PORT ");
			add_num_to_str(&s, &l, pc[0]);
			add_chr_to_str(&s, &l, ',');
...
void ftp_send_retr_req(struct connection *c, int state)
{
	...
	if (fast_ftp) a:add_to_str(&login, &logl, fi->cmdbuf);
		...
		unsigned char *nl = (unsigned char *)strchr(( char *)fi->cmdbuf, '\n');
		if (!nl) goto a;
		nl++;
		add_bytes_to_str(&login, &logl, fi->cmdbuf, nl - fi->cmdbuf);
		memmove(fi->cmdbuf, nl, strlen(( char *)nl) + 1);
	}
	write_to_socket(c, c->sock1, login, strlen(( char *)login), ftp_retr_1);
	mem_free(login);
	setcstate(c, state);
}
...
void ftp_retr_file(struct connection *c, struct read_buffer *rb)
{
			...
			case 2:		/* PORT */
				...
				if (inf->pasv) {
					...
					if (connect(c->sock2, (struct sockaddr *)&sa, sizeof sa)) {
						if (errno != EINPROGRESS && errno != 144) {
							setcstate(c, -errno);
							retry_connection(c);
							return;
						}
	...
	if (g >= 100 && g < 200) {
		...
		if (casecmp(&d[i], (unsigned char *)"byte", 4)) goto nol;
		est = strtol(( char *)&d[p], NULL, 10);
	...
	if (*d && *d < p && WHITECHAR(buf[*d - 1])) {
		...
		if (!f) {
			if ((ee - *d != 1 || buf[*d] != '.') &&
				...
				add_to_str(&str, &sl, (unsigned char *)"<a href=\"..\"/>..</a>\n");
			}
		}
		add_conv_str(&str, &sl, buf, *d);
		add_to_str(&str, &sl, (unsigned char *)"<a href=\"");
		add_conv_str(&str, &sl, buf + *d, ee - *d);
		if (buf[0] == 'd') add_chr_to_str(&str, &sl, '/');
		add_to_str(&str, &sl, (unsigned char *)"\">");
		add_conv_str(&str, &sl, buf + *d, ee - *d);
		add_to_str(&str, &sl, (unsigned char *)"</a>");
		add_conv_str(&str, &sl, buf + ee, p - ee);
	} else {
		int pp;
		if (p > 5 && !casecmp(buf, (unsigned char *)"total", 5)) goto raw;
...
#define A(s) add_fragment(c->cache, c->from, s, strlen(( char *)s)), c->from += strlen(( char *)s)
		A(ftp_head);
