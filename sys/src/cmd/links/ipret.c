	...
	{	if(js_verbose_errors)		      
		{/*	snprintf(txt,MAX_STR_LEN,"%s in line %d!!!\b\b\n",a,(int)context->current->lineno);*/
			if(context->current) {
				...
				if (strlen((char *)txt)>MAGICKA_KONSTANTA_NA_MAXIMALNI_DYLKU_JS_KODU_PRI_ERRORU)
				{
				...
				if (fd->f_data)
				{
					...
					txt2=convert_string(ct,txt,strlen((char *)txt),NULL);
				}
				else
					txt2=stracpy(txt);
				mem_free(txt);
				...
				if(i)	context->code[k]='\n';
			} else	txt=stracpy((unsigned char *)a);
...
{	lns*zrusit;
	...
	{	case FLOAT:
		...
		case VARIABLE:
			...
			} else	my_internal((unsigned char *)"Killing null variable!\n",context);
			js_mem_free(pom);
		break;
		case ARGUMENTY:
		...
		case ARRAY:
			...
			{	zrusit=lookup(MIN1KEY,(plns*)pom->argument,context);
				...
				{	my_internal((unsigned char *)"Internal: Parentlist corrupted!\n",context); return;
				}
		...
		case INTVAR:
			js_mem_free(pom);
		break;
		default: my_internal((unsigned char *)"Error! Rusim divny typ!\n",context);
	}
}
...
/*			kam->argument=(long)js_mem_alloc(strlen("function f(){ [native code] }")+1);*/
		...
		break;
		default: my_internal((unsigned char *)"Error! Tahle konverze jeste nefunguje!\n",context);
			kam->typ=INTEGER;
			kam->argument=0;
		break;
	}
...
{	lns*zrusit;
	...
	{	case UNDEFINED:
		...
		case ARRAY:
			...
			if(zrusit->type!=PARLIST)
			{	my_internal((unsigned char *)"Internal: Parent list corrupted!!\n",context);	return;}
		...
		break;
		default:
			my_internal((unsigned char *)"Aiee! Unknown variable type!\n",context);
...
{	abuf*pomocny;
	...
	{	case UNDEFINED: if(!js_all_conversions)
		...
		break;
		default:
			my_internal((unsigned char *)"Tato konverze jeste neni napsana!\n",context);
		break;
	}
	return retval;
}
...
{	int retval=0;
	...
	{	case UNDEFINED:	if(!js_all_conversions)
		...
		case ARRAY:
			...
			buffer->argument=0;
		break;
		default:	my_internal((unsigned char *)"Aiee! Unknown conversion!\n",context);
...
/*			if(!strcmp(a,"nan"))
		...
		break;
		default:
			my_internal((unsigned char *)"Tato konverze jeste neni hotova",context);
			a=stracpy1("null");
...
{	float vysl=0;
	...
	{	case UNDEFINED:
		...
		break;
		default:
			my_internal((unsigned char *)"Tato konverze jeste neni hotova!\n",context);
			return 0;
		break;
	}
	js_mem_free(arg);
...
{	printf("Prosel jsem moc argumentu ve funkci: %s\n",a);
	internal((unsigned char *)"exiting!");
}
...
{	abuf*objekt;
	...
	{	case 0:	debug("zjistuji koho\n");
		...
		case 1:	debug("nicim ho...\n");
			...
			if(!variable->value)
			{	my_internal((unsigned char *)"Deleting null object!",context);
...
/*				clearvar(vysl,context); */
				...
				{	case UNDEFINED:
					...
					default:
						...
						my_internal((unsigned char *)"Internal: Unknown type turned up!\n",context);
					break;
				}
			}
			js_mem_free(kam);
...
{       abuf*retval=js_mem_alloc(sizeof(abuf));
	...
	{	case CELE:
		...
		break;
		default: my_internal((unsigned char *)"Bug! Cislo neni ani cele, ani necele!\n",context);
			retval->typ=INTEGER;
			retval->argument=0;
		break;
	}
		...
		case 2: debug("pocitam... \n");
			...
			if(pna->type==INTVAR)
				...
				{	case STRING:
					...
					break;
					default:
						my_internal((unsigned char *)"Internal: Unknown types in plusassign!\n",context);
						pna->type=INTEGER;
						pna->value=0;
					break;
				}
	...
	{	case 0:	debug("zjistuji argument\n");
		...
		case 1:	debug("zjistuji typ ");
			...
			{	case INTEGER:
				...
				break;
				default:
					my_internal((unsigned char *)"Internal: Detecting strange type!\n",context);
					strcpy((char*)arg->argument,"undefined");
				break;
			}
			pusha(arg,context);
	...
	{	case 0:	debug("hledam ji\n");
		...
		case 1:	debug("uz ji mam!\n");
			...
			if(var->typ==VARIABLE)
				...
				{	
					my_internal((unsigned char *)"Divny typ v inicializaci promenne!\n",context);
...
{	my_internal((unsigned char *)"Ale h0vn0!!!\n",context); 
...
/* Basnik: context->ptr->arg[3] je typu lns*, jeho value je plns* a my
   ...
   a to konkretne do context->current->arg[5]-te pozice. */
			...
			if(!pna){ my_internal((unsigned char *)"Kalim mimo misu!\n",context);
				retval=js_mem_alloc(sizeof(abuf));
				retval->typ=UNDEFINED;
				retval->argument=0;
				pusha(retval,context);
...
{	vrchol*parenti;
	...
	{	if(parenti->opcode==TWITH)
		{	debug("kucham with, ");
			...
			{	my_internal((unsigned char *)"Internal: Lost addrspace by continue!!\n",context);
...
{	vrchol*parenti;
	...
	{	if(parenti->opcode==TWITH)
		{	debug("kucham with, ");
			arg1=pulla(context);
			if((arg1->typ!=ADDRSPACE)&&(arg1->typ!=MAINADDRSPC))
			{	my_internal((unsigned char *)"Internal: Lost addrspace by break!!\n",context);
		...
		case 1:	debug("returnim, ");
			...
			{	if(parenti->opcode==TWITH)
				{	debug("kucham with, ");
					arg1=pulla(context);
					if((arg1->typ!=ADDRSPACE)&&(arg1->typ!=MAINADDRSPC))
					{	my_internal((unsigned char *)"Internal: Lost addrspace by return!!\n",context);
...
{	abuf* arg,*prevpar;
	...
	{	case 0:	debug("zjistuji argument\n");
		...
		case 2:	debug("vracim namespace\n");
			...
			{	my_internal((unsigned char *)"Internal: Lost addrspace in with statement!!!\n",context);
...
sto_jenda:		sezvykan=0;
		...
		case 102:
			...
			if(pom->type!=PARLIST) my_internal((unsigned char *)"Parentlist je podplacen!\n",context);
			delete_from_parlist(&fotr_je_lotr,pom);
			if((arg->typ==ADDRSPACE)||(arg->typ==ADDRSPACEP)||(arg->typ==ARRAY))
			{	pom=lookup(MIN1KEY,(plns*)arg->argument,context);
				if(pom->type!=PARLIST)	my_internal((unsigned char *)"Parentlist podplacen!\n",context);
				add_to_parlist(&fotr_je_lotr,pom);
			} else 	pom=0;
			...
			{	my_internal((unsigned char *)"Internal: Bacha! Podtekl namespace!!\n",context);
...
{	abuf*pomarg=0,*p1=0;
	...
	{	case 0:	debug("pass1, sekam namespace\n");
		...
		case 1:	debug("pass2, uklizim namespace\n");
			...
			{	my_internal((unsigned char *)"FATAL Error! Buffer leak implies namespace leak!\n",context);
...
{	my_internal((unsigned char *)"Funkce conscall byla preci obsoletizovana!!!\n",context); 
	js_error("Unsupported constructor call ",context);
}
...
/*				printf("Odpal! "); */
		...
		case 2:	debug("vracim zemetras!\n");
			...
			{	my_internal((unsigned char *)"Internal: Namespace lost by function member!!!\n",context);
			...
			{	if(retval->typ==INTVAR)
				{	debug("Nazdar!\n");/*	pomvar1=context->lnamespace->ns[((lns*)retval->argument)->identifier%HASHNUM];
					...
					{	while((pomvar1)&&(pomvar1->next)&&(pomvar1->next->identifier!=((lns*)retval->argument)->identifier))pomvar1=pomvar1->next;	
						if(!pomvar1)
						{	my_internal((unsigned char *)"Lost variable!\n",context);
							js_error("Invalid member operation ",context);
							return;
						}
						if(!pomvar1->next)
   ...
   pri testu delky se udela atoi B-) */
			...
			{	if(variable->type!=INTVAR)	
				...
				if(je_to_cislo){
					...
					{	my_internal((unsigned char *)"Internal:Strange type of \"length\" property!\n",context);
...
{	abuf*vysl;
	...
	if(context->current->in==0){
		...
		{
			my_internal((unsigned char *)"Bug! Fce convert nema syna ve strome!\n",context);
			...
			{	case INTEGER:
				...
				case VARIABLE:
					...
					switch(vysl->typ=variable->type){
						...
						break;
						default: 
							my_internal((unsigned char *)"Divna konverze variable!\n",context);
	...
	{	case 0:	debug("hledam ji\n");
		...
		case 1:	debug("uz ji mam!\n");
			...
			if(var->typ==VARIABLE)
				...
				my_internal((unsigned char *)"Internal:Divny typ v inicializaci promenne!\n",context);
...
/*		case TCONVERT:	convert(context);
		...
		break;
		default:
			my_internal((unsigned char *)"Error! Udelal jsem si spatny intercode; tento opcode neznam!\n",context);
