...
{
	if (path) while (chdir(( char *)path) && errno == EINTR) ;
}
...
#ifdef DEBUG
			if (is_rect_valid(&r1) && do_rects_intersect(&r1, r)) internal((unsigned char *)"bad intersection 1");
			if (is_rect_valid(&r2) && do_rects_intersect(&r2, r)) internal((unsigned char *)"bad intersection 2");
			if (is_rect_valid(&r3) && do_rects_intersect(&r3, r)) internal((unsigned char *)"bad intersection 3");
			if (is_rect_valid(&r4) && do_rects_intersect(&r4, r)) internal((unsigned char *)"bad intersection 4");
#endif
...
struct term_spec *get_term_spec(unsigned char *term)
{
	struct term_spec *t;
	NO_GFX;
	foreach(t, term_specs) if (!strcasecmp(( char *)t->term, ( char *)term)) return t;
	return &dumb_term;
}
...
struct term_spec *new_term_spec(unsigned char *term)
{
	...
	if (strlen(( char *)term) < MAX_TERM_LEN) strcpy(( char *)t->term, ( char *)term);
...
struct terminal *init_gfx_term(void (*root_window)(struct window *, struct event *, int), void *info, int len)
{
	...
	if (!strcasecmp(( char *)drv->name, "x")) term->environment |= ENV_XWIN;
	term->spec = &gfx_term;
	if ((cwd = get_cwd())) {
		safe_strncpy(term->cwd, cwd, MAX_CWD_LEN);
		mem_free(cwd);
	}
	gfx_term.charset = get_cp_index((unsigned char *)"utf-8");
...
void in_term(struct terminal *term)
{
	...
	if ((r = read(term->fdin, iq + term->qlen, ALLOC_GR)) <= 0) {
		if (r == -1 && errno != ECONNRESET) error((unsigned char *)"ERROR: error %d on terminal: could not read event", errno);
		destroy_terminal(term);
		return;
	}
	term->qlen += r;
	...
	if (ev->ev != EV_INIT && ev->ev != EV_RESIZE && ev->ev != EV_REDRAW && ev->ev != EV_KBD && ev->ev != EV_MOUSE && ev->ev != EV_ABORT) {
		error((unsigned char *)"ERROR: error on terminal: bad event %d", ev->ev);
		goto mm;
	}
	if (ev->ev == EV_INIT) {
		int init_len;
	...
	if (ev->ev == EV_REDRAW || ev->ev == EV_RESIZE || ev->ev == EV_INIT) {
		...
		send_redraw:
		if (ev->x < 0 || ev->y < 0) {
			error((unsigned char *)"ERROR: bad terminal size: %d, %d", (int)ev->x, (int)ev->y);
			goto mm;
		}
...
{											\
	...
	if (s->mode == TERM_LINUX) {							\
		if (s->m11_hack) {							\
			if (ch >> 15 != mode) {						\
				if (!(mode = ch >> 15)) add_to_str(&a, &l, (unsigned char *)"\033[10m");	\
				else add_to_str(&a, &l, (unsigned char *)"\033[11m");			\
			}								\
		}									\
		...
		if (ch >> 15 != mode) {							\
			if (!(mode = ch >> 15)) add_to_str(&a, &l, (unsigned char *)"\x0f");		\
			else add_to_str(&a, &l, (unsigned char *)"\x0e");				\
		}									\
	...
	if (A != attrib) {								\
		attrib = A;								\
		add_to_str(&a, &l, (unsigned char *)"\033[0");						\
		if (s->col) {								\
			unsigned char m[4];						\
			m[0] = ';'; m[1] = '3'; m[3] = 0;				\
			m[2] = (attrib & 7) + '0';					\
			...
			add_to_str(&a, &l, (unsigned char *)";7");					\
		if (attrib & 0100) add_to_str(&a, &l, (unsigned char *)";1");				\
		add_to_str(&a, &l, (unsigned char *)"m");						\
	}										\
...
void redraw_screen(struct terminal *term)
{
	...
	if (l) {
		if (s->col) add_to_str(&a, &l,(unsigned char *) "\033[37;40m");
		add_to_str(&a, &l, (unsigned char *)"\033[0m");
		if (s->mode == TERM_LINUX && s->m11_hack) add_to_str(&a, &l,(unsigned char *) "\033[10m");
		if (s->mode == TERM_VT100) add_to_str(&a, &l, (unsigned char *)"\x0f");
	}
...
#endif
	exe((char *)path + 1, path[0]);
	close(p);
	if (path[1 + strlen(( char *)path + 1) + 1]) unlink((char *)path + 1 + strlen(( char *)path + 1) + 1);
}
...
void exec_on_terminal(struct terminal *term, unsigned char *path, unsigned char *delete, int fg)
{
	...
	if (term->master) {
		if (!*path) {
			...
			if (!(param = mem_alloc(strlen(( char *)path) + strlen(( char *)delete) + 3))) return;
			param[0] = fg;
			strcpy(( char *)param + 1, ( char *)path);
			strcpy(( char *)param + 1 + strlen(( char *)path) + 1, ( char *)delete);
			if (fg == 1) {
			...
			if ((blockh = start_thread((void (*)(void *, int))exec_thread, param, strlen(( char *)path) + strlen(( char *)delete) + 3)) == -1) {
				if (fg == 1) {
		...
		unsigned char *data;
		if ((data = mem_alloc(strlen(( char *)path) + strlen(( char *)delete) + 4))) {
			data[0] = 0;
			data[1] = fg;
			strcpy(( char *)data + 2, ( char *)path);
			strcpy(( char *)data + 3 + strlen(( char *)path), ( char *)delete);
			hard_write(term->fdout, data, strlen(( char *)path) + strlen(( char *)delete) + 4);
			mem_free(data);
		}
		/*char x = 0;
		...
		x = fg;
		hard_write(term->fdout, &x, 1);
		hard_write(term->fdout, path, strlen(( char *)path) + 1);
		hard_write(term->fdout, delete, strlen(( char *)delete) + 1);*/
	}
}
...
void do_terminal_function(struct terminal *term, unsigned char code, unsigned char *data)
{
	...
	NO_GFX;
	if (!(x_data = mem_alloc(strlen(( char *)data) + 2))) return;
	x_data[0] = code;
	strcpy(( char *)x_data + 1, ( char *)data);
...
{
	unsigned char *a;
	while ((a = (unsigned char *)strchr((char *)title, 1))) memmove(a, a + 1, strlen(( char *)a + 1) + 1);
	if (term->title && !strcmp((char *)title, (char *)term->title)) goto ret;
	if (term->title) mem_free(term->title);
