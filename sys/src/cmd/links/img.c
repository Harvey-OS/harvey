...
void img_destruct_cached_image(struct cached_image *cimg)
{
	switch (cimg->state){
		...
		internal((unsigned char *)"Invalid state in struct cached_image");
#endif /* #ifdef DEBUG */
	}
	mem_free(cimg->url);
	mem_free(cimg);
...
void header_dimensions_known(struct cached_image *cimg)
{
	...
	if ((cimg->state^8)&13){
		...
		internal((unsigned char *)"Invalid state in header_dimensions_known");
	}
	if (cimg->width<1||cimg->height<1){
		fprintf(stderr,"width=%d height=%d\n",cimg->width, cimg->height);
		internal((unsigned char *)"Zero dimensions in header_dimensions_known");
	}
#endif /* #ifdef DEBUG */
	if (cimg->wanted_xw<0){
		/* Unspecified width */
	...
	if (cimg->strip_optimized){
		...
		if (cimg->height<=0){
			...
			internal((unsigned char *)"Invalid cimg->height in strip_optimized section of\
	...
	,unsigned char *buffer, int height)
{
	...
	if (cimg->state!=12&&cimg->state!=14){
		fprintf(stderr,"cimg->state=%d\n",cimg->state);
		internal((unsigned char *)"invalid state in buffer_to_16");
	}
#endif /* #ifdef DEBUG */
...
#ifdef DEBUG
		default:
		internal((unsigned char *)"buffer_to_16: unknown mem organization");
#endif /* #ifdef DEBUG */
...
void buffer_to_bitmap(struct cached_image *cimg)
{
	...
	if(cimg->state!=12&&cimg->state!=14){
		fprintf(stderr,"cimg->state=%d\n",cimg->state);
		internal((unsigned char *)"buffer_to_bitmap called in invalid state");
	}
	if (cimg->strip_optimized) internal((unsigned char *)"strip_optimized in buffer_to_bitmap");
	if (cimg->width<1||cimg->height<1){
		fprintf(stderr,"cimg->width=%d, cimg->height=%d\n",cimg->width,
				cimg->height);
		internal((unsigned char *)"Invalid cimg->width x cimg->height in\
buffer_to_bitmap");
	}
#endif /* #ifdef DEBUG */
...
#ifdef DEBUG
		if (ox<=0||oy<=0){
			internal((unsigned char *)"ox or oy <=0 before resampling image");
		}
#endif /* #ifdef DEBUG */
		if (ix!=ox||iy!=oy){
...
void img_end(struct cached_image *cimg)
{
	switch(cimg->state){
		...
		internal((unsigned char *)"Invalid state encountered in end");
#endif /* #ifdef DEBUG */
	}
	cimg->state|=1;
}
...
void r3l0ad(struct cached_image *cimg, struct g_object_image *goi)
{
	...
	switch(cimg->state){
		...
		default:
		fprintf(stderr,"cimg->state=%d\n",cimg->state);
		internal((unsigned char *)"Invalid state in r3l0ad()");
#endif /* #ifdef DEBUG */
	}
	cimg->state&=2;
}
...
{
	if (strcmp((char *)template,(char *)test)) return 0;
	else{
		mem_free(test);
		return 1;
	}
...
void type(struct cached_image *cimg, unsigned char *content_type)
{
	...
	if (cimg->state!=0&&cimg->state!=2){
		fprintf(stderr,"cimg->state=%d\n",cimg->state);
		internal((unsigned char *)"Invalid state encountered in type()");
	}
#endif /* #ifdef DEBUG */
#ifdef HAVE_JPEG
	if (dtest((unsigned char *)"image/jpeg",content_type)){
		cimg->image_type=IM_JPG;
		jpeg_start(cimg);
	}else if (dtest((unsigned char *)"image/jpg",content_type)){
		cimg->image_type=IM_JPG;
		jpeg_start(cimg);
	}else if (dtest((unsigned char *)"image/jpe",content_type)){
		cimg->image_type=IM_JPG;
		jpeg_start(cimg);
	}else if (dtest((unsigned char *)"image/pjpe",content_type)){
		cimg->image_type=IM_JPG;
		jpeg_start(cimg);
	}else if (dtest((unsigned char *)"image/pjpeg",content_type)){
		cimg->image_type=IM_JPG;
		jpeg_start(cimg);
	}else if (dtest((unsigned char *)"image/pjpg",content_type)){
		cimg->image_type=IM_JPG;
		jpeg_start(cimg);
	}else 
#endif /* #ifdef HAVE_JPEG */
		if (dtest((unsigned char *)"image/png",content_type)){
		cimg->image_type=IM_PNG;
		png_start(cimg);
	}else if (dtest((unsigned char *)"image/gif",content_type)){
		cimg->image_type=IM_GIF;
		gif_start(cimg);
	}else if (dtest((unsigned char *)"image/x-xbitmap",content_type)){
...
int img_process_download(struct g_object_image *goi, struct f_data_c *fdatac)
{
...
#ifdef DEBUG
	if (!goi->af) internal((unsigned char *)"NULL goi->af in process_download\n");
	if (cimg->state>=16){ /* Negative don't occur becaus it's unsigned char */
		fprintf(stderr,"cimg->state=%d\n",cimg->state);
		internal((unsigned char *)"Invalid cimg->state in img_process_download\n");
	}
#endif /* #ifdef DEBUG */
...
#ifdef DEBUG
		if (!ctype)
			internal((unsigned char *)"NULL ctype in process_download()");
#endif /* #ifdef DEBUG */
		type(cimg,ctype);
	}
	...
	if (!((cimg->state^8)&9)){
		...
		default:
			...
			internal((unsigned char *)"Invalid image_type encountered when processing data in\
...
{
#ifdef DEBUG
	if (xw<1||yw<1) internal((unsigned char *)"zero dimension in draw_frame_mark");
#endif /* #ifdef DEBUG */
	if (broken == 1){
		...
		int x, int y, int bg)
{
	...
	if (goi->cimg->state<12||goi->cimg->state>=16){
		fprintf(stderr,"cimg->state=%d\n",cimg->state);
		internal((unsigned char *)"Invalid cimg->state in draw_picture");
	}
#endif /* #ifdef DEBUG */
	if (!(cimg->state&1)){
		if (!cimg->bmp.user)
		...
		fprintf(stderr,"cimg->state=%d\n",cimg->state);
		internal((unsigned char *)"Nonexistent bitmap in said cimg->state in draw_picture");
	}
#endif /* #ifdef DEBUG */
...
void update_bitmap(struct cached_image *cimg)
{
	...
	if (cimg->state<12||cimg->state>=16){
		fprintf(stderr,"cimg->state=%d\n",cimg->state);
		internal((unsigned char *)"Invalid state in update_bitmap");
	}
#endif /* #ifdef DEBUG */
	if (!(cimg->state&1)&&
	...
	int x, int y)
{
...
#else
	{
		...
		fprintf(stderr,"cimg->state\%d\n",cimg->state);
		internal((unsigned char *)"Invalid state in img_draw_image");
	}
...
#ifdef DEBUG
		if (!url)
			internal ((unsigned char *)"NULL url as argument of\
