...
unsigned char *get_text_translation(unsigned char *text, struct terminal *term)
{
	...
	if (term->spec) charset = term->spec->charset;
		...
		if (utf8_charset == -1) utf8_charset = get_cp_index((unsigned char *)"UTF-8");
	...
	if ((current_tra = translation_array[current_language][charset])) {
		...
		if (!(tt = translations[current_language].t[text - dummyarray].name)) {
			...
			trn = convert_string(conv_table, tt, strlen(( char *)tt), &l_opt);
		}
...
int language_index(unsigned char *name)
{
	...
	for (a=0;a<N_LANGUAGES;a++)
		if (!strcmp(( char *)name, ( char *)translations[a].t[T__LANGUAGE].name))return a;
	return 0;
}
...
void set_language(int l)
{
	...
	for (i = 0; i < T__N_TEXTS; i++) if (translations[l].t[i].code != i) {
		internal((unsigned char *)"Bad table for language %s. Run script synclang.", translations[l].t[T__LANGUAGE].name);
		return;
	}
	current_language = l;
	...
	i = get_cp_index(cp);
	if (i == -1) {
		internal((unsigned char *)"Unknown charset for language %s.", translations[l].t[T__LANGUAGE].name);
		i = 0;
	}
