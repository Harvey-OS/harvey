...
long (*get_color_fn(int depth))(int rgb)
{
	switch(depth)
	{
		...
		default:
			printf("depth=%d\n",depth);
			internal((unsigned char *)"Unsupported depth in get_color_fn.");
			return NULL;
			break;
...
void
make_8_table(int *table, double gamma)
{
	int i, light0;

	for (i = 0; i < 256; i++) {
		/* Long live the Nipkow Disk */
		light0 = 65535 * pow((double)i / 255, gamma);
		if (light0 < 0) 
			light0 = 0;
		else if (light0 > 65535) 
			light0 = 65535;
		table[i] = light0;
	}
}
 ...
 */
void
make_16_table(int *table, int bits, int pos, double gamma, int dump_t2c,
	int bigendian)
{
	int j, light_val;
	int grades = (1 << bits)-1, grade, last_grade = -1, last_content = 0;
	double voltage, rev_gamma = 1 / gamma;
	const double t = 1.0 / 65535;
	...
	for (j = 0; j < 65536; j++) {
		...
		/*
		 * Find nearest voltage to this voltage.  Finding nearest
		 * voltage, not nearest photon flux ensures the dithered
		 * pixels will be perceived to be near.  The voltage input
		 * into the monitor was intentionally chosen by
		 * generations of television engineers to roughly comply
		 * with eye's response, thus minimizing and unifying noise
		 * impact on transmitted signal.  This is only marginal
		 * enhancement however it sounds kool ;-) (and is kool).
		 */
		...
		if (dump_t2c) {
			...
			if (bigendian) {
				...
				last_content = light_val | (val2 << 16);
			} else
				last_content = light_val | (grade<<(pos + 16));
		table[j] = last_content;  /* Save index and photon flux. */
	}
}
...
void init_dither(int depth)
{
	switch(depth){
		...
		internal((unsigned char *)"Graphics driver returned unsupported \
