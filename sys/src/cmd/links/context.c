...
/*			fotri=((plns*)pna->value)->next;*/
		...
		break;
		default:
			internal((unsigned char *)"Aiee! Unknown variable type!\n");
		break;
	}
/*	js_mem_free(pna);*/
}
...
{	int i=0;
	...
	lns*a,*b;
	ns=pns->ns;
	if(!ns)	my_internal((unsigned char *)"Del_ns is trying to free NULL addrspace!\n",context);
	while(i<HASHNUM)
	{	if((a=ns[i++]))
	...
	{	if((a=ns[i]))
			...
			{	b=a->next;
				if(a->type==PARLIST) 
					my_internal((unsigned char *)"Too many parlists!\n",context) /* ;*/
...
/*	js_durchfall=0;	*/
	...
	if(promenna->type!=PARLIST)
		my_internal((unsigned char *)"Parentlist corrupted!",context);
	if(!promenna->value)
