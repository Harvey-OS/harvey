...
#ifdef DEBUG
	if (!deco) internal((unsigned char *)"NULL decoder in jpeg_restart");
#endif /* #ifdef DEBUG */
...
#ifdef DEBUG
			if (deco->scanlines[0])
				internal((unsigned char *)"Prebouchavam deco->scanlines[0]");
#endif /* #ifdef DEBUG */
...
#ifdef DEBUG
				if (deco->scanlines[0])
					internal((unsigned char *)"Prebouchavam dec->scanlines v barevne sekci");
#endif /* #ifdef DEBUG */
