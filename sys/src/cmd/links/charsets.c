...
unsigned char *no_str = (unsigned char*)"*";

void new_translation_table(struct conv_table *p)
{
	int i;
	for (i = 0; i < 256; i++) if (p[i].t) free_translation_table(p[i].u.tbl);
	for (i = 0; i < 128; i++) p[i].t = 0, p[i].u.str = (unsigned char *)strings[i];
...
{
	int j, s;
	if (u < 128) return (unsigned char*)strings[u];
	if (u == 0xa0) return (unsigned char*)"\001";
	if (u == 0xad) return (unsigned char*)"";
	...
	for (j = 0; codepages[to].table[j].c; j++)
		if (codepages[to].table[j].u == u)
			return (unsigned char*)strings[codepages[to].table[j].c];
	BIN_SEARCH(unicode_7b, N_UNICODE_7B, U_EQUAL, U_ABOVE, u, s);
	if (s != -1) return (unsigned char*)unicode_7b[s].s;
	return NULL;
}
...
void add_utf_8(struct conv_table *ct, int u, unsigned char *str)
{
	...
	while (p[1]) {
		if (ct[*p].t) ct = ct[*p].u.tbl;
			...
			if (ct[*p].u.str != no_str) {
				internal((unsigned char*)"bad utf encoding #1");
				return;
			}
	...
	if (ct[*p].t) {
		internal((unsigned char*)"bad utf encoding #2");
		return;
	}
...
struct conv_table *get_translation_table_to_utf_8(int from)
{
	...
	for (i = 0; i < 128; i++) utf_table[i].u.str = (unsigned char*)strings[i];
	if (codepages[from].table == table_utf_8) {
		for (i = 128; i < 256; i++) utf_table[i].u.str = stracpy((unsigned char*)strings[i]);
		return utf_table;
	}
...
struct conv_table *get_translation_table(int from, int to)
{
	...
	if (codepages[from].table == table_utf_8) {
		int j;
		for (j = 0; codepages[to].table[j].c; j++) add_utf_8(table, codepages[to].table[j].u, codepages[to].table[j].u == 0xa0 ? (unsigned char*)"\001" : codepages[to].table[j].u == 0xad ? (unsigned char*)"" : (unsigned char*)strings[codepages[to].table[j].c]);
		for (i = 0; unicode_7b[i].x != -1; i++) if (unicode_7b[i].x >= 0x80) add_utf_8(table, unicode_7b[i].x, (unsigned char*)unicode_7b[i].s);
...
unsigned char *get_entity_string(unsigned char *st, int l, int encoding)
{
	...
	if (st[0] == '#') {
		...
		while (s <= e) {
			int c;
			int m = (s + e) / 2;
			c = xxstrcmp((unsigned char *)entities[m].s, st, l);
			if (!c) {
				n = entities[m].c;
				goto f;
			}
		...
		for (a = 0; codepages[i].aliases[a]; a++) {
			for (p = 0; n[p]; p++) {
				if (upcase(n[p]) == upcase(codepages[i].aliases[a][0])) {
					...
					if (strlen((char *)codepages[i].aliases[a]) > ll) {
						ll = strlen((char *)codepages[i].aliases[a]);
						ii = i;
					}
				}
				fail:;
...
{
	if (index < 0) return (unsigned char *)"none";
