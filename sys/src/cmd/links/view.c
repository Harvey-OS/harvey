...
void destroy_vs(struct view_state *vs)
{
	int i;
	if (--vs->refcount) {
		if (vs->refcount < 0) internal((unsigned char *)"destroy_vs: view_state refcount underflow");
		return;
	}
...
{
	if (!fn) return;
	if (*l) add_to_str(str, l, (unsigned char *)", ");
...
unsigned char *print_js_event_spec(struct js_event_spec *j)
{
	...
	int l = 0;
	if (!j) return str;
	add_event_desc(&str, &l, j->click_code, (unsigned char *)"onclick");
	add_event_desc(&str, &l, j->dbl_code, (unsigned char *)"ondblclick");
	add_event_desc(&str, &l, j->down_code, (unsigned char *)"onmousedown");
	add_event_desc(&str, &l, j->up_code, (unsigned char *)"onmouseup");
	add_event_desc(&str, &l, j->over_code, (unsigned char *)"onmouseover");
	add_event_desc(&str, &l, j->out_code, (unsigned char *)"onmouseout");
	add_event_desc(&str, &l, j->move_code, (unsigned char *)"onmousemove");
	add_event_desc(&str, &l, j->focus_code, (unsigned char *)"onfocus");
	add_event_desc(&str, &l, j->blur_code, (unsigned char *)"onblur");
	add_event_desc(&str, &l, j->change_code, (unsigned char *)"onchange");
	add_event_desc(&str, &l, j->keypress_code, (unsigned char *)"onkeypress");
	add_event_desc(&str, &l, j->keyup_code, (unsigned char *)"onkeyup");
	add_event_desc(&str, &l, j->keydown_code, (unsigned char *)"onkeydown");
	return str;
}
...
{
	struct tag *tag;
	foreach(tag, f->tags) if (!strcasecmp(( char *)tag->name, ( char *)name)) return tag->y;
	return -1;
}
...
void sort_links(struct f_data *f)
{
	...
	for (i = 0; i < f->nlinks; i++) {
		...
		for (j = p; j <= q; j++) {
			if (j >= f->y) {
				internal((unsigned char *)"link out of screen");
				continue;
			}
...
void draw_link(struct terminal *t, struct f_data_c *scr, int l)
{
	...
	vy = vs->view_pos;
	if (scr->link_bg) {
		internal((unsigned char *)"link background not empty");
	...
	switch (link->type) {
		...
		case L_AREA:
			...
			if (link->type == L_FIELD) {
				...
				/*else internal((unsigned char *)"link has no form control");*/
			...
			break;
		default: internal((unsigned char *)"bad link type");
	}
}
...
{
	int i;
	if (fs->state >= 0 && fs->state < fc->nvalues && !strcmp(( char *)fc->values[fs->state], ( char *)fs->value)) return;
	for (i = 0; i < fc->nvalues; i++) {
		if (!strcmp(( char *)fc->values[i], ( char *)fs->value)) {
			fs->state = i;
			return;
		}
	}
	...
	else fs->value = stracpy((unsigned char *)"");
}
...
void init_ctrl(struct form_control *form, struct form_state *fs)
{
	...
	switch (form->type) {
		...
		case FC_TEXTAREA:
			...
			fs->state = strlen(( char *)form->default_value);
			fs->vpos = 0;
			break;
		case FC_FILE:
			fs->value = stracpy((unsigned char *)"");
			fs->state = 0;
			fs->vpos = 0;
			break;
		case FC_CHECKBOX:
...
void draw_form_entry(struct terminal *t, struct f_data_c *f, struct link *l)
{
	...
	int i, x, y;
	if (!form) {
		internal((unsigned char *)"link %d has no form", (int)(l - f->f_data->links));
		return;
	}
	...
	switch (form->type) {
		...
		case FC_FILE:
			...
			for (i = 0; i < form->size; i++, x++)
				if (x >= xp && y >= yp && x < xp+xw && y < yp+yw) {
					if (fs->value && i >= -fs->vpos && i < strlen(( char *)fs->value) - fs->vpos) set_only_char(t, x, y, form->type != FC_PASSWORD ? fs->value[i + fs->vpos] : '*');
...
void draw_forms(struct terminal *t, struct f_data_c *f)
{
	...
	if (!l1 || !l2) {
		if (l1 || l2) internal((unsigned char *)"get_first_link == %p, get_last_link == %p", l1, l2);
		return;
	}
	do {
...
int has_form_submit(struct f_data *f, struct form_control *form)
{
	...
	if (!q) internal((unsigned char *)"form is not on list");
	return 0;
}
...
unsigned char *encode_textarea(unsigned char *t)
{
	...
	for (; *t; t++) {
		...
		else add_to_str(&o, &len, (unsigned char *)"\r\n");
	}
	return o;
}
		...
		if (form->form_num == fc->form_num && ((form->type != FC_SUBMIT && form->type != FC_IMAGE && form->type != FC_RESET && form->type != FC_BUTTON ) || form == fc) && form->name && form->name[0]) {
			...
			switch (form->type) {
				...
				case FC_SELECT:
					...
					break;
				case FC_IMAGE:
					add_to_strn(&sub->name, fi ? (unsigned char *)".x" : (unsigned char *)".y");
					/*sub->value = stracpy((unsigned char *)"0");*/
					sub->value = init_str();
					svl = 0;
					add_num_to_str(&sub->value, &svl, fi ? ismap_x : ismap_y);
					break;
				default:
					internal((unsigned char *)"bad form control type");
					mem_free(sub);
					continue;
			}
		     ...
		     int cp_from, int cp_to)
{
	...
	*data = init_str();
	foreach(sv, *l) {
		...
		if (lst) add_to_str(data, len, (unsigned char *)"&"); else lst = 1;
		encode_string(sv->name, data, len);
		add_to_str(data, len, (unsigned char *)"=");
		if (sv->type == FC_TEXT || sv->type == FC_PASSWORD || sv->type == FC_TEXTAREA)
			p2 = ( char *)convert_string(convert_table, p, strlen(( char *)p), NULL);
		else p2 = ( char *)stracpy(p);
		encode_string((unsigned char *)p2, data, len);
		mem_free(p2);
	}
}
		      ...
		      unsigned char *bound, int cp_from, int cp_to)
{
	...
	*data = init_str();
	foreach(sv, *l) {
		...
		if (sv->type == FC_FILE) {
			add_to_str(data, len, (unsigned char *)"\"; filename=\"");
		...
		if (sv->type != FC_FILE) {
			if (sv->type == FC_TEXT || sv->type == FC_PASSWORD || sv->type == FC_TEXTAREA)
				p = ( char *)convert_string(convert_table, sv->value, strlen(( char *)sv->value), NULL);
			else p = ( char *)stracpy(sv->value);
			add_to_str(data, len, (unsigned char *)p);
			mem_free(p);
		} else {
			int fh, rd;
#define F_BUFLEN 1024
			...
			if (*sv->value) {
				...
				if ((fh = open(( char *)sv->value, O_RDONLY)) == -1) goto error;
				set_bin(fh);
				do {
	...
	for (i = 0; i <= *len - BL; i++) {
		...
		internal((unsigned char *)"Counld not assing boundary");
		nb:;
	}
...
unsigned char *get_form_url(struct session *ses, struct f_data_c *f, struct form_control *form, int *onsubmit)
{
	...
	if (form->onsubmit)
	{
		jsint_execute_code(f,form->onsubmit,strlen(( char *)form->onsubmit),-1,form->form_num,form->form_num);
	...
	if (!data) goto ff;
	if (!strncasecmp(( char *)form->action,"javascript:",11))
	{
		go=stracpy(form->action);
		goto x;
	}
	if (form->method == FM_GET) {
		if ((go = mem_alloc(strlen(( char *)form->action) + 1 + len + 1))) {
			unsigned char *pos;
			strcpy(( char *)go, ( char *)form->action);
			if ((pos = (unsigned char *)strchr(( char *)go, '#'))) {
				unsigned char *poss = pos;
				pos = stracpy(pos);
				*poss = 0;
			}
			if (strchr(( char *)go, '?')) strcat(( char *)go, "&");
			else strcat(( char *)go, "?");
			strcat(( char *)go, ( char *)data);
			if (pos) strcat(( char *)go, ( char *)pos), mem_free(pos);
		}
	} else {
		int l = 0;
		int i;
		...
		if (form->method == FM_POST) add_to_str(&go, &l, (unsigned char *)"application/x-www-form-urlencoded\n");
		else {
			add_to_str(&go, &l, (unsigned char *)"multipart/form-data; boundary=");
			add_bytes_to_str(&go, &l, bound, BL);
			add_to_str(&go, &l, (unsigned char *)"\n");
		}
		for (i = 0; i < len; i++) {
			unsigned char p[3];
			sprintf(( char *)p, "%02x", (int)data[i]);
			add_to_str(&go, &l, p);
		}
	}
	x:
...
unsigned char *get_link_url(struct session *ses, struct f_data_c *f, struct link *l, int *onsubmit)
{
	if (l->type == L_LINK) {
		...
		if (ismap_link && strlen(( char *)l->where) >= 4 && !strcmp(( char *)l->where + strlen(( char *)l->where) - 4, "?0,0")) {
			unsigned char *nu = init_str();
			int ll = 0;
			add_bytes_to_str(&nu, &ll, l->where, strlen(( char *)l->where) - 3);
...
int enter(struct session *ses, struct f_data_c *f, int a)
{
	...
	if (link->js_event&&link->js_event->click_code)
		jsint_execute_code(f,link->js_event->click_code,strlen(( char *)link->js_event->click_code),-1,(link->type==L_BUTTON&&link->form&&link->form->type==FC_SUBMIT)?link->form->form_num:-1,-1);
#endif
...
#endif
			if (strlen(( char *)u) >= 4 && !casecmp(u, (unsigned char *)"MAP@", 4))
	...
	if (link->type == L_CHECKBOX) {
		...
		if (link->form->type == FC_CHECKBOX) fs->state = !fs->state;
			...
			foreach(fc, f->f_data->forms)
				if (fc->form_num == link->form->form_num && fc->type == FC_RADIO && !xstrcmp((unsigned char *)fc->name, (unsigned char *)link->form->name)) {
	...
	if (link->type == L_SELECT) {
		...
		{
			jsint_execute_code(f,link->js_event->focus_code,strlen(( char *)link->js_event->focus_code),-1,-1,-1);
		}
#endif
	...
	if (link->type == L_FIELD || link->type == L_AREA) {
		...
		if (!ses->locked_link&&f->vs&&f->f_data&&f->vs->current_link>=0&&f->vs->current_link<f->f_data->nlinks)
		{
			...
			if (lnk->js_event&&lnk->js_event->focus_code)
				jsint_execute_code(f,lnk->js_event->focus_code,strlen(( char *)lnk->js_event->focus_code),-1,-1,-1);
		}
#endif
#ifdef G
		if (F && a) {
	...
	internal((unsigned char *)"bad link type %d", link->type);
	return 1;
}
...
void selected_item(struct terminal *term, void *pitem, struct session *ses)
{
	...
	if (old_item!=item&&l->js_event&&l->js_event->change_code)
		jsint_execute_code(f,l->js_event->change_code,strlen(( char *)l->js_event->change_code),-1,-1,-1);
#endif
	/* execute onblur handler */
#ifdef JS
	if (l->js_event&&l->js_event->blur_code)
		jsint_execute_code(f,l->js_event->blur_code,strlen(( char *)l->js_event->blur_code),-1,-1,-1);
#endif
...
static void field_op_changed(struct f_data_c *f, struct link *lnk)
{
	if (lnk->js_event&&lnk->js_event->keydown_code)
		jsint_execute_code(f,lnk->js_event->keydown_code,strlen(( char *)lnk->js_event->keydown_code),-1,-1,-1);
	if (lnk->js_event&&lnk->js_event->keypress_code)
		jsint_execute_code(f,lnk->js_event->keypress_code,strlen(( char *)lnk->js_event->keypress_code),-1,-1,-1);
	if (lnk->js_event&&lnk->js_event->keyup_code)
		jsint_execute_code(f,lnk->js_event->keyup_code,strlen(( char *)lnk->js_event->keyup_code),-1,-1,-1);
}
#endif
...
int field_op(struct session *ses, struct f_data_c *f, struct link *l, struct event *ev, int rep)
{
	...
	if (!form) {
		internal((unsigned char *)"link has no form control");
		return 0;
	}
	...
	if (ev->ev == EV_KBD) {
		if (ev->x == KBD_LEFT) {
			...
			if (fs->state < strlen(( char *)fs->value)) {
...
#endif
			} else fs->state = strlen(( char *)fs->value);
		}
			...
			if (form->type == FC_TEXTAREA) {
				...
				if ((ln = format_text(fs->value, form->cols, form->wrap))) {
					...
					for (y = 0; ln[y].st; y++) if (fs->value + fs->state >= ln[y].st && fs->value + fs->state < ln[y].en + (ln[y+1].st != ln[y].en)) {
						...
						if (fs->state && fs->state < strlen(( char *)fs->value) && ln[y+1].st == ln[y].en) fs->state--;
						goto yyyy;
					}
			...
			if (!form->ro && strlen(( char *)fs->value) < form->maxlength) {
				unsigned char *v;
				if ((v = mem_realloc(fs->value, strlen(( char *)fs->value) + 12))) {
					unsigned char a_[2];
					unsigned char *nw;
					int ll;
					if (!F) {
					...
					if (ll > 10) goto bad;
					fs->value = v;
					memmove(v + fs->state + ll, v + fs->state, strlen(( char *)v + fs->state) + 1);
			...
			char * clipboard = ( char *)get_clipboard_text();
			if (!clipboard) goto brk;
			if (!form->ro && strlen(( char *)clipboard) <= form->maxlength) {
				unsigned char *v;
				if ((v = mem_realloc(fs->value, strlen(( char *)clipboard) +1))) {
					fs->value = v;
					memmove(v , clipboard, strlen(( char *)clipboard) +1);
					fs->state = strlen(( char *)fs->value);
				}
			}
			mem_free(clipboard);
			...
			if (!form->ro && strlen(( char *)fs->value) < form->maxlength) {
				unsigned char *v;
				if ((v = mem_realloc(fs->value, strlen(( char *)fs->value) + 2))) {
					fs->value = v;
					memmove(v + fs->state + 1, v + fs->state, strlen(( char *)v + fs->state) + 1);
					v[fs->state++] = '\n';
#ifdef JS
					fs->changed=1;
					field_op_changed(f,l);
...
#endif
				memmove(fs->value + fs->state - ll, fs->value + fs->state, strlen(( char *)fs->value + fs->state) + 1), fs->state -= ll
#ifdef JS
...
#endif
			if (!form->ro && fs->state < strlen(( char *)fs->value)) memmove(fs->value + fs->state, fs->value + fs->state + ll, strlen(( char *)fs->value + fs->state + ll) + 1)
#ifdef JS
				, fs->changed=1, field_op_changed(f,l)
#endif
				;
		} else if (upcase(ev->x) == 'U' && ev->y == KBD_CTRL) {
			if (!form->ro) memmove(fs->value, fs->value + fs->state, strlen(( char *)fs->value + fs->state) + 1);
			fs->state = 0;
#ifdef JS
			fs->changed=1;
			field_op_changed(f,l);
		...
		if (l) {
			...
			if (l->form&&(l->type==L_AREA||l->type==L_FIELD)&&(fs=find_form_state(fd,l->form)))
			{
				...
				if (l->type==L_FIELD)
				{
					...
					{
						xx+=fs->vpos;
						fs->state=xx < strlen(( char *)fs->value) ? (xx<0?0:xx) : strlen(( char *)fs->value);
					}
				}
			}
		}
	...
	if (!F && fd->vs) {
		...
		if (previous_link!=fd->vs->current_link&&fd->f_data&&previous_link>=0&&previous_link<fd->f_data->nlinks) /* link has changed */
		{
			...
			if ((l->type==L_FIELD||l->type==L_AREA) && (fs=find_form_state(fd,l->form)) && fs->changed && l->js_event && l->js_event->change_code)
				fs->changed=0,jsint_execute_code(fd,l->js_event->change_code,strlen(( char *)l->js_event->change_code),-1,-1,-1);
			...
			if (l->js_event&&l->js_event->out_code)
				jsint_execute_code(fd,l->js_event->out_code,strlen(( char *)l->js_event->out_code),-1,-1,-1);
		}
		if (previous_link!=fd->vs->current_link&&fd->f_data&&fd->vs->current_link>=0&&fd->vs->current_link<fd->f_data->nlinks)
		{
			...
			if (l->js_event&&l->js_event->over_code)
				jsint_execute_code(fd,l->js_event->over_code,strlen(( char *)l->js_event->over_code),-1,-1,-1);
		}
#endif
	}
	return r;
...
void send_event(struct session *ses, struct event *ev)
{
	if (ev->ev == EV_KBD) {
		...
		if (ev->x == 'G' && !ev->y) {
			...
			if (!s) goto quak;
			if (strchr(( char *)s, POST_CHAR)) *strchr(( char *)s, POST_CHAR) = 0;
			dialog_goto_url(ses, ( char *)s);
			mem_free(s);
			goto x;
		}
		if (upcase(ev->x) == 'G' && ev->y == KBD_CTRL) {
			if (!ses->screen->vs||ses->screen->vs->current_link == -1)goto quak;
			dialog_goto_url(ses, ( char *)ses->screen->f_data->links[ses->screen->vs->current_link].where);
			goto x;
		}
		/*
		if (upcase(ev->x) == 'A') {
			...
			if ((ev->b & BM_ACT) != B_MOVE) {
				...
				if (ses->screen&&ses->screen->vs&&ses->screen->f_data&&ses->screen->vs->current_link>=0&&ses->screen->vs->current_link<ses->screen->f_data->nlinks)
				{
					...
					if (lnk->type!=L_SELECT&&lnk->js_event&&lnk->js_event->blur_code)
						jsint_execute_code(current_frame(ses),lnk->js_event->blur_code,strlen(( char *)lnk->js_event->blur_code),-1,-1,-1);
					...
					if ((lnk->type==L_AREA||lnk->type==L_FIELD)&&lnk->js_event&&lnk->js_event->change_code&&(fs=find_form_state(ses->screen,lnk->form))&&fs->changed)
						fs->changed=0,jsint_execute_code(current_frame(ses),lnk->js_event->change_code,strlen(( char *)lnk->js_event->change_code),-1,-1,-1);
...
void frm_download(struct session *ses, struct f_data_c *fd)
{
	...
	if ((ses->dn_url = get_link_url(ses, fd, link, NULL))) {
		if (!casecmp(ses->dn_url, (unsigned char *)"MAP@", 4)) {
			mem_free(ses->dn_url);
			ses->dn_url = NULL;
			return;
		}
...
void send_open_in_new_xterm_target(struct terminal *term, void (*open_window)(struct terminal *term, unsigned char *, unsigned char *), struct session *ses)
{
		...
		if (ses->wtd_target&&*ses->wtd_target)
		{
			...
			params=stracpy((unsigned char *)"-target ");
			add_to_strn(&params, tgt);
			add_to_strn(&params,(unsigned char *)" ");
...
void send_open_new_xterm(struct terminal *term, void (*open_window)(struct terminal *, unsigned char *, unsigned char *), struct session *ses)
{
	...
	ses->dn_url = init_str();
	l = 0;
        add_to_str(&ses->dn_url, &l, (unsigned char *)"-base-session ");
...
void open_in_new_window(struct terminal *term, void (*xxx)(struct terminal *, void (*)(struct terminal *, unsigned char *, unsigned char *), struct session *ses), struct session *ses)
{
	...
	for (oi = oin; oi->text; oi++) add_to_menu(&mi, oi->text, (unsigned char *)"", oi->hk, MENU_FUNC xxx, oi->fn, 0);
	mem_free(oin);
	do_menu(term, mi, ses);
}
...
void send_plumb(struct terminal *term, void *xxx, struct session *ses)
{
	struct f_data_c *fd = current_frame(ses);
	if (!fd) return;
	if (fd->vs->current_link == -1) return;
...
void link_menu(struct terminal *term, void *xxx, struct session *ses)
{
	...
	if (link->type == L_LINK && link->where) {
		if (strlen(( char *)link->where) >= 4 && !casecmp(link->where, (unsigned char *)"MAP@", 4)) {
			if (!F) {
				add_to_menu(&mi, TEXT(T_DISPLAY_USEMAP), (unsigned char *)">", TEXT(T_HK_DISPLAY_USEMAP), MENU_FUNC send_enter, NULL, 1);
			}
		}
		else {
			int c = can_open_in_new(term);
			if (!F) add_to_menu(&mi, TEXT(T_FOLLOW_LINK), (unsigned char *)"", TEXT(T_HK_FOLLOW_LINK), MENU_FUNC send_enter, NULL, 0);
			if (c) add_to_menu(&mi, TEXT(T_OPEN_IN_NEW_WINDOW), c - 1 ? (unsigned char *)">" : (unsigned char *)"", TEXT(T_HK_OPEN_IN_NEW_WINDOW), MENU_FUNC open_in_new_window, send_open_in_new_xterm, c - 1);
			if (!anonymous) add_to_menu(&mi, TEXT(T_DOWNLOAD_LINK), (unsigned char *)"d", TEXT(T_HK_DOWNLOAD_LINK), MENU_FUNC send_download, NULL, 0);
			if (!anonymous) add_to_menu(&mi, (unsigned char *)"Plumb", (unsigned char *)"p",(unsigned char *)"Plumb", MENU_FUNC send_plumb, NULL, 0);
#ifdef G
			if(F && term && term->dev && term->dev->drv && !strcmp((char *)term->dev->drv->name,"x")) 
				add_to_menu(&mi, TEXT(T_COPY_LINK_LOCATION), (unsigned char *)"c", TEXT(T_HK_COPY_LINK_LOCATION), MENU_FUNC copy_link_location, NULL, 0);
#endif
	...
	if ((link->type == L_CHECKBOX || link->type == L_SELECT || link->type == L_FIELD || link->type == L_AREA) && link->form){
		int c = can_open_in_new(term);
		add_to_menu(&mi, TEXT(T_SUBMIT_FORM), (unsigned char *)"", TEXT(T_HK_SUBMIT_FORM), MENU_FUNC send_submit, NULL, 0);
		if (c && link->form->method == FM_GET) add_to_menu(&mi, TEXT(T_SUBMIT_FORM_AND_OPEN_IN_NEW_WINDOW), c - 1 ? (unsigned char *)">" : (unsigned char *)"", TEXT(T_HK_SUBMIT_FORM_AND_OPEN_IN_NEW_WINDOW), MENU_FUNC open_in_new_window, send_open_in_new_xterm, c - 1);
		if (!anonymous) add_to_menu(&mi, TEXT(T_SUBMIT_FORM_AND_DOWNLOAD), (unsigned char *)"d", TEXT(T_HK_SUBMIT_FORM_AND_DOWNLOAD), MENU_FUNC send_download, NULL, 0);
		add_to_menu(&mi, TEXT(T_RESET_FORM), (unsigned char *)"", TEXT(T_HK_RESET_FORM), MENU_FUNC send_reset, NULL, 0);
	}
	if (link->type == L_BUTTON && link->form) {
		if (link->form->type == FC_RESET) add_to_menu(&mi, TEXT(T_RESET_FORM), (unsigned char *)"", TEXT(T_HK_RESET_FORM), MENU_FUNC send_enter, NULL, 0);
		else if (link->form->type==FC_BUTTON);
		else if (link->form->type == FC_SUBMIT || link->form->type == FC_IMAGE) {
			int c = can_open_in_new(term);
			add_to_menu(&mi, TEXT(T_SUBMIT_FORM), (unsigned char *)"", TEXT(T_HK_SUBMIT_FORM), MENU_FUNC send_enter, NULL, 0);
			if (c && link->form->method == FM_GET) add_to_menu(&mi, TEXT(T_SUBMIT_FORM_AND_OPEN_IN_NEW_WINDOW), c - 1 ? (unsigned char *)">" : (unsigned char *)"", TEXT(T_HK_SUBMIT_FORM_AND_OPEN_IN_NEW_WINDOW), MENU_FUNC open_in_new_window, send_open_in_new_xterm, c - 1);
			if (!anonymous) add_to_menu(&mi, TEXT(T_SUBMIT_FORM_AND_DOWNLOAD), (unsigned char *)"d", TEXT(T_HK_SUBMIT_FORM_AND_DOWNLOAD), MENU_FUNC send_download, NULL, 0);
		}
	}
	if (link->where_img) {
		if (!F || f->f_data->opt.plain != 2) add_to_menu(&mi, TEXT(T_VIEW_IMAGE), (unsigned char *)"i", TEXT(T_HK_VIEW_IMAGE), MENU_FUNC send_image, NULL, 0);
		if (!anonymous) add_to_menu(&mi, TEXT(T_DOWNLOAD_IMAGE), (unsigned char *)"I", TEXT(T_HK_DOWNLOAD_IMAGE), MENU_FUNC send_download_image, NULL, 0);
	}
	x:
	no_l:
	if (!mi->text) add_to_menu(&mi, TEXT(T_NO_LINK_SELECTED), (unsigned char *)"", M_BAR, NULL, NULL, 0);
	do_menu(term, mi, ses);
}
...
unsigned char *print_current_titlex(struct f_data_c *fd, int w)
{
	...
	if (fd->yw < fd->f_data->y) {
		...
		add_to_str(&p, &pl, (unsigned char *)"(p");
		add_num_to_str(&p, &pl, pp);
		add_to_str(&p, &pl, (unsigned char *)" of ");
...
unsigned char *print_current_linkx(struct f_data_c *fd, struct terminal *term)
{
	...
	if (l->type == L_LINK) {
		if (!l->where && l->where_img) {
			...
			if (l->img_alt)
			{
				...
				txt = convert_string(ct, l->img_alt, strlen(( char *)l->img_alt), &fd->f_data->opt);
				add_to_str(&m, &ll, txt);
				mem_free(txt);
			}
			else
			{
				...
				add_to_str(&m, &ll, (unsigned char *)" ");
				add_to_str(&m, &ll, l->where_img);
			}
			goto p;
		}
		if (l->where && strlen(( char *)l->where) >= 4 && !casecmp(l->where, (unsigned char *)"MAP@", 4)) {
			m = init_str();
			ll = 0;
	...
	if (l->type == L_BUTTON) {
		if (l->form->type == FC_BUTTON){
			...
			if (fd->f_data)
			{
				...
				ct=get_translation_table(fd->f_data->cp,fd->f_data->opt.cp);
				txt=convert_string(ct,n,strlen(( char *)n),NULL);
				mem_free(n);
			}
			else
				txt=n;
	...
	if (l->type == L_CHECKBOX || l->type == L_SELECT || l->type == L_FIELD || l->type == L_AREA) {
		...
		if (l->form->name && l->form->name[0]) add_to_str(&m, &ll, (unsigned char *)", "), add_to_str(&m, &ll, _(TEXT(T_NAME), term)), add_to_str(&m, &ll, (unsigned char *)" "), add_to_str(&m, &ll, l->form->name);
		if ((l->form->type == FC_CHECKBOX || l->form->type == FC_RADIO) && l->form->default_value && l->form->default_value[0]) add_to_str(&m, &ll, (unsigned char *)", "), add_to_str(&m, &ll, _(TEXT(T_VALUE), term)), add_to_str(&m, &ll, (unsigned char *)" "), add_to_str(&m, &ll, l->form->default_value);
...
unsigned char *print_current_linkx_plus(struct f_data_c *fd, struct terminal *term)
{
	...
	if (l->type == L_LINK) {
		...
		if (l->where_img) {
			add_to_str(&m, &ll, (unsigned char *)"\n");
			add_to_str(&m, &ll, _(TEXT(T_IMAGE), term));
			add_to_str(&m, &ll, (unsigned char *)": src='");
			add_to_str(&m, &ll, l->where_img);
			add_to_str(&m, &ll, (unsigned char *)"'");
			...
			if (l->img_alt)
			{
				...
				add_to_str(&m, &ll, (unsigned char *)" alt='");
				ct=get_translation_table(fd->f_data->cp,fd->f_data->opt.cp);
				txt = convert_string(ct, l->img_alt, strlen(( char *)l->img_alt), &fd->f_data->opt);
				add_to_str(&m, &ll, txt);
				add_to_str(&m, &ll, (unsigned char *)"'");
			...
			if (F&&l->obj)
			{
				add_to_str(&m, &ll, (unsigned char *)" size='");
				add_num_to_str(&m, &ll, l->obj->xw);
				add_to_str(&m, &ll, (unsigned char *)"x");
...
void loc_msg(struct terminal *term, struct location *lo, struct f_data_c *frame)
{
	...
	if (strchr((char *)lo->url, POST_CHAR)) add_bytes_to_str(&s, &l, lo->url, (char *)strchr((char *)lo->url, POST_CHAR) - (char *)lo->url);
	...
	if (!get_cache_entry(lo->url, &ce)) {
		...
		if (frame->f_data->ass == 1) add_to_str(&s, &l, (unsigned char *)" ("), add_to_str(&s, &l, _(TEXT(T_ASSUMED), term)), add_to_str(&s, &l,(unsigned char *) ")");
		if (frame->f_data->ass == 2) add_to_str(&s, &l, (unsigned char *)" ("), add_to_str(&s, &l, _(TEXT(T_IGNORING_SERVER_SETTING), term)), add_to_str(&s, &l, (unsigned char *)")");
		if ((a = parse_http_header(ce->head, (unsigned char *)"Content-Type", NULL))) {
			add_to_str(&s, &l, (unsigned char *)"\n");
		...
		if ((a = parse_http_header(ce->head,(unsigned char *) "Server", NULL))) {
			add_to_str(&s, &l, (unsigned char *)"\n");
		...
		if ((a = parse_http_header(ce->head, (unsigned char *)"Date", NULL))) {
			add_to_str(&s, &l, (unsigned char *)"\n");
...
void head_msg(struct session *ses)
{
	...
	if (!find_in_cache(cur_loc(ses)->url, &ce)) {
		...
		else s = ss = stracpy((unsigned char *)"");
		len = strlen(( char *)s) - 1;
		if (len > 0) {
			while ((ss = (unsigned char *)strstr(( char *)s, "\r\n"))) memmove(ss, ss + 1, strlen(( char *)ss));
