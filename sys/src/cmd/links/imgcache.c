...
int image_size(struct cached_image *cimg)
{
	...
	switch(cimg->state){
		...
		internal((unsigned char *)"Invalid cimg->state in image_size\n");
		break;
#endif /* #ifdef DEBUG */
	}
	return siz;
	...
	foreach(i, image_cache) {
		switch (type) {
			...
			case CI_FILES:
				...
				break;
			default:
				internal((unsigned char *)"imgcache_info: query %d", type);
		}
	}
	return n;
}
...
{
	register_cache_upcall(shrink_image_cache, (unsigned char *)"imgcache");
}
