...
{
	if (scr->fd) {
		internal((unsigned char *)"trying to free locked formatted data");
		return;
	}
	clear_formatted(scr);
	/*del_from_list(scr);*/
...
#ifdef DEBUG
	if (y >= p->data->y) {
		internal((unsigned char *)"line does not exist");
		return -1;
	}
#endif
...
int split_line(struct part *p)
{
	...
	if (p->data) {
#ifdef DEBUG
		if ((POS(i, p->cy) & 0xff) != ' ') internal((unsigned char *)"bad split: %c", (char)POS(i, p->cy));
#endif
...
void html_tag(struct f_data *f, unsigned char *t, int x, int y)
{
	struct tag *tag;
	if (!f) return;
	if ((tag = mem_alloc(sizeof(struct tag) + strlen(( char *)t) + 1))) {
		tag->x = x;
		tag->y = y;
...
void put_chars_conv(struct part *p, unsigned char *c, int l)
{
	...
	while (pp < l) {
		...
		while (*e) {
			...
			if (bp < CH_BUF) continue;
			flush1:
			put_chars(p, (unsigned char *)buffer, bp);
			bp = 0;
		}
	}
	if (bp) put_chars(p, (unsigned char *)buffer, bp);
}
...
void html_form_control(struct part *p, struct form_control *fc)
{
	...
	if (fc->type == FC_TEXT || fc->type == FC_PASSWORD || fc->type == FC_TEXTAREA) {
		...
		/*
		for (i = 0; i < fc->nvalues; i++) if ((dv = convert_string(convert_table, fc->values[i], strlen(( char *)fc->values[i]), d_opt))) {
...
struct frameset_desc *create_frameset(struct f_data *fda, struct frameset_param *fp)
{
	...
	if (!fp->x || !fp->y) {
		internal((unsigned char *)"zero size of frameset");
		return NULL;
	}
...
{
	pr(
	parse_html((unsigned char *)start, (unsigned char *)end, (void (*)(void *, unsigned char *, int)) put_chars_conv, (void (*)(void *)) line_break, (void *(*)(void *, int, ...)) html_special, part, head);
...
struct part *format_html_part(unsigned char *start, unsigned char *end, int align, int m, int width, struct f_data *data, int xs, int ys, unsigned char *head, int link_num)
{
	...
	if (ys < 0) {
		internal((unsigned char *)"format_html_part: ys == %d", ys);
		return NULL;
	}
	if (data) {
		struct node *n;
	...
	p->cx = -1;
	p->cy = 0;
	do_format(( char *)start, ( char *)end, p, head);
...
void push_base_format(unsigned char *url, struct document_options *opt, int frame)
{
	...
	if (html_stack.next != &html_stack) {
		internal((unsigned char *)"something on html stack");
...
struct conv_table *get_convert_table(unsigned char *head, int to, int def, int *frm, int *aa, int hard)
{
	...
	while (from == -1 && p && (a = parse_http_header(p, (unsigned char *)"Content-Type", &p))) {
		if ((b = parse_header_param(a, (unsigned char *)"charset"))) {
			from = get_cp_index(b);
			mem_free(b);
		}
		mem_free(a);
	}
	if (from == -1 && head && (a = parse_http_header(head,(unsigned char *) "Content-Charset", NULL))) {
		from = get_cp_index(a);
		mem_free(a);
	}
	if (from == -1 && head && (a = parse_http_header(head, (unsigned char *)"Charset", NULL))) {
		from = get_cp_index(a);
		mem_free(a);
	}
	if (aa) {
...
void really_format_html(struct cache_entry *ce, unsigned char *start, unsigned char *end, struct f_data *screen, int frame)
{
	...
	screen->title = convert_string(convert_table, t, strlen(( char *)t), d_opt);
	d_opt->plain = i;
	mem_free(t);
	...
	if (html_stack.next != &html_stack) {
		internal((unsigned char *)"html stack not empty after operation");
		init_list(html_stack);
	}
...
int compare_opt(struct document_options *o1, struct document_options *o2)
{
	...
	if (o1->xw == o2->xw &&
	    ...
	    kdo_si_hraje_nezlobi____a_nebo_to_je_PerM>=-0.0001 &&
	    ((o1->framename && o2->framename && !strcasecmp((char *)o1->framename, (char *)o2->framename)) || (!o1->framename && !o2->framename))) return 0;
	return 1;
}
