...
int do_real_lookup(unsigned char *name, ip__address *host)
{
	unsigned char *n;
	struct hostent *hst;
	for (n = (unsigned char *)name; *n; n++) if (*n != '.' && (*n < '0' || *n > '9')) goto nogethostbyaddr;
#ifdef HAVE_GETHOSTBYADDR
	if (!(hst = gethostbyaddr (name, strlen(( char *)name), AF_INET)))
#endif
...
int do_lookup(struct dnsquery *q, int force_async)
{
	...
	if (!async_lookup && !force_async) {
		...
		int r;
		sync_lookup:
		r = do_real_lookup((unsigned char *)q->name, q->addr);
		q->xfn(q, r);
		return 0;
#ifndef NO_ASYNC_LOOKUP
	} else {
		if ((q->h = start_thread((void (*)(void *, int))lookup_fn, q->name, strlen(( char *)q->name) + 1)) == -1) goto sync_lookup;
...
void end_dns_lookup(struct dnsquery *q, int a)
{
	...
	if (a) goto e;
	if ((dnsentry = mem_alloc(sizeof(struct dnsentry) + strlen(( char *)q->name) + 1))) {
...
{
	struct dnsquery *q;
	if (!(q = malloc(sizeof(struct dnsquery) + strlen(( char *)name) + 1))) {
		fn(data, -1);
		return 0;
	}
	q->fn = fn;
...
int find_host(unsigned char *name, ip__address *addr, void **qp, void (*fn)(void *, int), void *data)
{
	struct dnsentry *dnsentry;
	if (qp) *qp = NULL;
	if (!find_in_dns_cache(( char *)name, &dnsentry)) {
...
void init_dns(void)
{
	register_cache_upcall(shrink_dns_cache, (unsigned char *)"dns");
}
