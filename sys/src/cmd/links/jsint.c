...
void jsint_set_cookies(struct f_data_c *fd, int final_flush)
{
	...
	if(!(fd->js)||!(fd->js->ctx))internal((unsigned char *)"jsint_set_cookies called with NULL context.\n");
	if (!(fd->js->ctx->cookies)||!(fd->rq))return;  /* cookies string is empty, nothing to set */
	str=fd->js->ctx->cookies;
	
a_znova:
	eq1=(unsigned char *)strchr((char *)str,'=');
	semic1=(unsigned char *)strchr((char *)str,';');
	...
	if (!*str||(!final_flush&&!semic1))  /* na konci neni strednik a skript jeste bezi, takze to musime vratit do stringu a vypadnout */
	{
		...
		if (*str)bla=(unsigned char *)stracpy1(str);
	...
	next=semic1?semic1+1:str+strlen((char *)str);
	...
	if (semic1!=NULL)
	{
		eq2=(unsigned char *)strchr((char *)semic1+1,'=');
		semic2=(unsigned char *)strchr((char *)semic1+1,';');
	}
	
	if (eq2&&semic1&&(final_flush||semic2))
	{
		unsigned char *p=(unsigned char *)strstr((char *)semic1+1,"expires");
		if (!p)p=(unsigned char *)strstr((char *)semic1+1,"EXPIRES");
		if (p&&p>semic1&&p<eq2)  /* za 1. prirazenim nasleduje "expires=", takze to je porad jedna susenka */
			next=semic2?semic2+1:str+strlen((char *)str);
	}
...
int jsint_create(struct f_data_c *fd)
{
	...
	if (fd->js) internal((unsigned char *)"javascript state present");
...
void javascript_func(struct session *ses, unsigned char *hlavne_ze_je_vecirek)
{
	...
	jsint_execute_code(current_frame(ses),code,strlen((char *)code),-1,-1,-1);
}
...
void jsint_execute_code(struct f_data_c *fd, unsigned char *code, int len, int write_pos, int onclick_submit, int onsubmit)
{
	...
	*/
	...
	if (len >= 11 && !casecmp(code, (unsigned char *)"javascript:", 11)) code += 11, len -= 11;
...
void jsint_done_execution(struct f_data_c *fd)
{
	...
	struct js_state *js = fd->js;
	if (!js) {
		internal((unsigned char *)"no js in frame");
		return;
	}
	if (!js->active) {
		internal((unsigned char *)"jsint_done_execution: completion function called on inactive js");
		return;
	}
...
struct f_data_c *jsint_find_document(long doc_id)
{
	...
	if (type!=JS_OBJ_T_DOCUMENT&&type!=JS_OBJ_T_FRAME)
		{unsigned char txt[256]; snprintf((char *)txt,256,"jsint_find_document called with type=%d\n",type);internal(txt);}
...
void jsint_scan_script_tags(struct f_data_c *fd)
{
	...
	if (s >= eof || fd->script_t < 0)
	{
		...
		{
			jsint_execute_code(fd,onload_code,strlen((char *)onload_code),-1,-1,-1);
		}
		fd->script_t = -1;
		goto ret;
	}
	...
	if (!onload_code&&namelen==4&&!casecmp(name,(unsigned char *)"BODY",4))
	{
		onload_code=get_attr_val(attr,(unsigned char *)"onload");	/* if the element doesn't have onload attribute get_attr_val returns NULL */
		goto se;
	}
		
	if (!onload_code&&namelen==3&&!casecmp(name,(unsigned char *)"IMG",3))
	{
		onload_code=get_attr_val(attr,(unsigned char *)"onload");	/* if the element doesn't have onload attribute get_attr_val returns NULL */
		goto se;
	}
		
	if (namelen != 6 || casecmp(name, (unsigned char *)"SCRIPT", 6) || s - ss < fd->script_t) goto se;
	start = end = NULL;
	if ((val = get_attr_val(attr, (unsigned char *)"src"))) {
		unsigned char *url;
	...
	if (e + 8 <= eof) {
		if (/*uv ||*/ casecmp(e, (unsigned char *)"</SCRIPT", 8)) goto es;
	} else e = eof;
	ee = e;
	while (ee < eof && *ee != '>') ee++;
	if (ee < eof) ee++;
...
void *jsint_find_object(struct f_data_c *document, long obj_id)
{
	...
	switch (type)
	{
		...
		default:
		internal((unsigned char *)"jsint_find_object: unknown type %d.",type);
...
static long *__find_in_subframes(struct f_data_c *js_ctx, struct f_data_c *fd, long *pole_vole, int *n_items, unsigned char *takhle_tomu_u_nas_nadavame)
{
	...
	foreach(ff,fd->subframes)
		if (ff->loc&&ff->loc->name&&!strcmp((char *)ff->loc->name,(char *)takhle_tomu_u_nas_nadavame)&&jsint_can_access(js_ctx,ff))	/* to je on! */
	...
	foreach(fi,fd->f_data->images)
	{
		...
		gi = (struct g_object_image *)((char *)fi + ((char *)(&goi) - (char *)(&(goi.image_list))));
		if (gi->name&&!strcmp((char *)gi->name, (char *)takhle_tomu_u_nas_nadavame))
			if (!(pole_vole=__add_id(pole_vole,n_items,JS_OBJ_T_IMAGE+((gi->id)<<JS_OBJ_MASK_SIZE))))return NULL;
	}
#endif
	/* search forms */
	foreachback(f,fd->f_data->forms)
		if (f->form_name&&!strcmp((char *)f->form_name,(char *)takhle_tomu_u_nas_nadavame))   /* tak tohle JE Jim Beam */
			if (!(pole_vole=__add_id(pole_vole,n_items,((f->form_num)<<JS_OBJ_MASK_SIZE)+JS_OBJ_T_FORM)))return NULL;

	/* search form elements */
	foreachback(f,fd->f_data->forms)
		if (f->name&&!strcmp((char *)f->name,(char *)takhle_tomu_u_nas_nadavame))   /* tak tohle JE Jim Beam */
		{
			...
			switch (f->type)
			{
				...
				default: /* internal((unsigned char *)"Invalid form element type.\n"); */
...
long *jsint_resolve(void *context, long obj_id, char *takhle_tomu_u_nas_nadavame,int *n_items)
{
	...
	*n_items=0;
	...
	switch(jsint_object_type(obj_id))
	{
		...
		pole_vole=__find_in_subframes(js_ctx, fd, pole_vole, n_items, (unsigned char *)takhle_tomu_u_nas_nadavame);
		break;	
		...
		case JS_OBJ_T_FORM:	
		{
			...
			foreachback(f,js_ctx->f_data->forms)
			{
				if (f->form_num==fc->form_num)	/* this form */
					if (f->name&&!strcmp((char *)f->name,(char *)takhle_tomu_u_nas_nadavame))   /* this IS Jim Beam */
					{
						long tak_mu_to_ukaz=0;
						tak_mu_to_ukaz=(f->g_ctrl_num)<<JS_OBJ_MASK_SIZE;
						switch (f->type)
						{
							...
							/* internal((unsigned char *)"Invalid form element type.\n"); */
						}
...
{
	struct f_data_c *fd;
	if (!data)internal((unsigned char *)"js_upcall_get_document_id called with NULL pointer!");
...
void js_upcall_document_write(void *p, unsigned char *str, int len)
{
	...
	struct js_state *js = fd->js;
	if (!js)return;
	if (!js->active) internal((unsigned char *)"js_upcall_document_write: no request active");
	if (js->active->write_pos == -1) return;
	if (js->active->write_pos < 0) internal((unsigned char *)"js_upcall_document_write: js->active trashed");
	if (!js->src) {
		unsigned char *s, *eof;
		struct fragment *f;
...
unsigned char *js_upcall_get_title(void *data)
{
	...
	if (!data)internal((unsigned char *)"js_upcall_get_title called with NULL pointer!");
	fd=(struct f_data_c *)data;

	title=mem_alloc(MAX_STR_LEN*sizeof(unsigned char));
	if (!title)internal((unsigned char *)"Cannot allocate memory in js_upcall_get_title");
	...
	if (fd->f_data)
	{
		...
		t = convert_string(ct, title, strlen((char *)title), NULL);
		mem_free(title);
		title=t;
	}
	return title;
...
unsigned char *js_upcall_get_location(void *data)
{
	...
	if (!data)internal((unsigned char *)"js_upcall_get_location called with NULL pointer!");
	fd=(struct f_data_c *)data;

	loc=mem_alloc(MAX_STR_LEN*sizeof(unsigned char));
	if (!loc)internal((unsigned char *)"Cannot allocate memory in js_upcall_get_location.\n");
...
unsigned char *js_upcall_document_last_modified(void *data, long document_id)
{
	...
	if (!data)internal((unsigned char *)"js_upcall_document_last_modified called with NULL pointer!");
	fd=(struct f_data_c *)data;
...
unsigned char *js_upcall_get_useragent(void *data)
{
	...
	if (!data)internal((unsigned char *)"js_upcall_get_useragent called with NULL pointer!");
	fd=(struct f_data_c *)data;

	if (!fake_useragent||!(*fake_useragent)) {
		add_to_str(&retval, &l, (unsigned char *)"Links (" VERSION_STRING "; ");
		add_to_str(&retval, &l, system_name);
		add_to_str(&retval, &l, (unsigned char *)")");
	}
	else {
...
{
	if (!fake_useragent||!(*fake_useragent))
		return stracpy((unsigned char *)"Links");
	else
...
unsigned char *js_upcall_get_appversion(void)
{
	...
	str=init_str();
	add_to_str(&str,&l,(unsigned char *)VERSION_STRING);
...
unsigned char *js_upcall_get_referrer(void *data)
{
	...
	if (!data)internal((unsigned char *)"js_upcall_get_referrer called with NULL pointer!");
	fd=(struct f_data_c *)data;
	...
	switch (referer)
	{
		...
		case REFERER_REAL:
		{
			...
			post=(unsigned char *)strchr((char *)fd->rq->prev_url, POST_CHAR);
...
void js_upcall_confirm(void *data)
{
	...
	if (!s)internal((unsigned char *)"js_upcall_confirm called with NULL pointer\n");  /* to jenom kdyby na mne PerM zkousel naky oplzlosti... */
	...
	if (!jsid)internal((unsigned char *)"Cannot allocate memory in js_upcall_confirm.\n");
	...
	if (fd->f_data)
	{
		...
		ct=get_translation_table(fd->f_data->cp,fd->f_data->opt.cp);
		txt=convert_string(ct,s->string,strlen((char *)s->string),NULL);
	}
	else
...
void js_upcall_alert(void * data)
{
	...
	if (!s)internal((unsigned char *)"Alert called with NULL pointer.\n"); /* to jenom kdyby na mne PerM zkousel naky oplzlosti... */
	...
	if (!fd->js) return;
	jsid=mem_alloc(sizeof(struct gimme_js_id));
	if (!jsid)internal((unsigned char *)"Cannot allocate memory in js_upcall_alert.\n");
...
void js_upcall_close_window(void *data)
{
	...
	if (!s)internal((unsigned char *)"js_upcall_close_window called with NULL pointer\n");  /* to jenom kdyby na mne PerM zkousel naky oplzlosti... */
	...
	if (js_manual_confirmation)
	{
		...
		jsid=mem_alloc(sizeof(struct gimme_js_id));
		if (!jsid)internal((unsigned char *)"Cannot allocate memory in js_upcall_close_window\n");
...
{
	struct f_data_c *fd;
	if (!data)internal((unsigned char *)"js_upcall_get_window_id called with NULL pointer!");
...
void js_upcall_get_string(void *data)
{
	...
	if (!s)internal((unsigned char *)"js_upcall_get_string called with NULL pointer\n");  /* to jenom kdyby na mne PerM zkousel naky oplzlosti... */
	...
	if (!fd->js) return;
	jsid=mem_alloc(sizeof(struct gimme_js_id));
	if (!jsid)internal((unsigned char *)"Cannot allocate memory in js_upcall_get_string\n");
...
long *js_upcall_get_links(void *data, long document_id, int *len)
{
	...
	if (!js_ctx)internal((unsigned char *)"js_upcall_get_links called with NULL context pointer\n");
	...
	*len=fd->f_data->nlinks;
	...
	if (!to_je_Ono)internal((unsigned char *)"Cannot allocate memory in js_upcall_get_links\n");
...
unsigned char *js_upcall_get_link_target(void *data, long document_id, long link_id)
{
	...
	if (!js_ctx)internal((unsigned char *)"js_upcall_get_link_target called with NULL context pointer\n");
...
long *js_upcall_get_forms(void *data, long document_id, int *len)
{
	...
	if (!js_ctx)internal((unsigned char *)"js_upcall_get_forms called with NULL context pointer\n");
	fd=jsint_find_document(document_id);
	if (!fd||!jsint_can_access(js_ctx,fd))return NULL;
	...
	if (!to_je_Ono)internal((unsigned char *)"Cannot allocate memory in js_upcall_get_forms\n");
	...
	*len=0;
	...
	foreachback(fc, fd->f_data->forms)
	{
		...
		if (!p)internal((unsigned char *)"Cannot reallocate memoru in js_upcall_get_forms\n");
		to_je_Ono=p;
...
unsigned char *js_upcall_get_form_action(void *data, long document_id, long form_id)
{
	...
	if (!js_ctx)internal((unsigned char *)"js_upcall_get_form_action called with NULL context pointer\n");
	fd=jsint_find_document(document_id);
	if (!fd||!jsint_can_access(js_ctx,fd))return NULL;
...
unsigned char *js_upcall_get_form_target(void *data, long document_id, long form_id)
{
	...
	if (!js_ctx)internal((unsigned char *)"js_upcall_get_form_target called with NULL context pointer\n");
	fd=jsint_find_document(document_id);
	if (!fd||!jsint_can_access(js_ctx,fd))return NULL;
...
unsigned char *js_upcall_get_form_method(void *data, long document_id, long form_id)
{
	...
	if (!js_ctx)internal((unsigned char *)"js_upcall_get_form_method called with NULL context pointer\n");
	fd=jsint_find_document(document_id);
	if (!fd||!jsint_can_access(js_ctx,fd))return NULL;
	...
	{
		case FM_GET:
		return stracpy((unsigned char *)"GET");

		case FM_POST:
		case FM_POST_MP:
		return stracpy((unsigned char *)"POST");

		default:
		internal((unsigned char *)"Invalid form method!\n");
		return NULL;  /* never called, but GCC likes it */
	}
}
...
unsigned char *js_upcall_get_form_encoding(void *data, long document_id, long form_id)
{
	...
	if (!js_ctx)internal((unsigned char *)"js_upcall_get_form_encoding called with NULL context pointer\n");
	fd=jsint_find_document(document_id);
	if (!fd||!jsint_can_access(js_ctx,fd))return NULL;
	...
	switch (fc->method)
	{
		case FM_GET:
		case FM_POST:
		return stracpy((unsigned char *)"application/x-www-form-urlencoded");

		case FM_POST_MP:
		return stracpy((unsigned char *)"multipart/form-data");
...
unsigned char *js_upcall_get_location_protocol(void *data)
{
	...
	loc=mem_alloc(MAX_STR_LEN*sizeof(unsigned char));
	if (!loc)internal((unsigned char *)"Cannot allocate memory in js_upcall_get_location_protocol\n");
...
unsigned char *js_upcall_get_location_port(void *data)
{
	...
	loc=mem_alloc(MAX_STR_LEN*sizeof(unsigned char));
	if (!loc)internal((unsigned char *)"Cannot allocate memory in js_upcall_get_location_port\n");
...
unsigned char *js_upcall_get_location_hostname(void *data)
{
	...
	loc=mem_alloc(MAX_STR_LEN*sizeof(unsigned char));
	if (!loc)internal((unsigned char *)"Cannot allocate memory in js_upcall_get_location_hostname\n");
...
unsigned char *js_upcall_get_location_host(void *data)
{
	...
	loc=mem_alloc(MAX_STR_LEN*sizeof(unsigned char));
	if (!loc)internal((unsigned char *)"Cannot allocate memory in js_upcall_get_location_host\n");
...
unsigned char *js_upcall_get_location_pathname(void *data)
{
	...
	loc=mem_alloc(MAX_STR_LEN*sizeof(unsigned char));
	if (!loc)internal((unsigned char *)"Cannot allocate memory in js_upcall_get_location_pathname\n");
...
unsigned char *js_upcall_get_location_search(void *data)
{
	...
	loc=mem_alloc(MAX_STR_LEN*sizeof(unsigned char));
	if (!loc)internal((unsigned char *)"Canot allocate memory in js_upcall_get_location_search\n");
	
	if (!(get_current_url(fd->ses,loc,MAX_STR_LEN))){mem_free(loc);return NULL;}

	if (parse_url(loc, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, &d, NULL, NULL)){mem_free(loc);return NULL;}
	if (!d){mem_free(loc);return NULL;}
	p=stracpy((unsigned char *)strchr((char *)d,'?'));
	mem_free(loc);
	return p;
}
...
unsigned char *js_upcall_get_location_hash(void *data)
{
	...
	loc=mem_alloc(MAX_STR_LEN*sizeof(unsigned char));
	if (!loc)internal((unsigned char *)"Cannot allocate memory in js_upcall_get_location_hash.");
	
	if (!(get_current_url(fd->ses,loc,MAX_STR_LEN))){mem_free(loc);return NULL;}

	if (parse_url(loc, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, &d, NULL, NULL)){mem_free(loc);return NULL;}
	if (!d){mem_free(loc);return NULL;}
	d=(unsigned char *)strchr((char *)d,'#');
	if (!d){mem_free(loc);return NULL;}
	d++;
	p=memacpy(d,strcspn((char *)d,"?"));
	mem_free(loc);
...
long *js_upcall_get_form_elements(void *data, long document_id, long form_id, int *len)
{
	...
	if (!js_ctx)internal((unsigned char *)"js_upcall_get_form_elements called with NULL context pointer\n");
	if ((form_id&JS_OBJ_MASK)!=JS_OBJ_T_FORM)return NULL;   /* this isn't form */
	fd=jsint_find_document(document_id);
	...
	*len=0;
	...
	if (!pole_Premysla_Zavorace)internal((unsigned char *)"Cannot allocate memory in js_upcall_get_form_elements.");
...
long *js_upcall_get_anchors(void *hej_Hombre, long document_id, int *len)
{
	...
	*len=0;
	...
	if (!js_ctx)internal((unsigned char *)"js_upcall_get_anchors called with NULL context pointer\n");
	fd=jsint_find_document(document_id);
	if (!fd||!jsint_can_access(js_ctx,fd))return NULL;
	...
	if (!(*len))return NULL;
	to_je_Ono=mem_alloc((*len)*sizeof(long));
	if (!to_je_Ono)internal((unsigned char *)"Cannot allocate memory in js_upcall_get_anchors\n");
...
int js_upcall_get_checkbox_radio_checked(void *smirak, long document_id, long radio_tv_id)
{
	...
	if (!js_ctx)internal((unsigned char *)"js_upcall_get_checkbox_radio_checked called with NULL context pointer\n");
	fd=jsint_find_document(document_id);
...
void js_upcall_set_checkbox_radio_checked(void *smirak, long document_id, long radio_tv_id, int value)
{
	...
	if (!js_ctx)internal((unsigned char *)"js_upcall_set_checkbox_radio_checked called with NULL context pointer\n");
...
int js_upcall_get_checkbox_radio_default_checked(void *bidak_smirak, long document_id, long radio_tv_id)
{
	...
	if (!js_ctx)internal((unsigned char *)"js_upcall_get_checkbox_radio_default_checked called with NULL context pointer\n");
	if ((radio_tv_id&JS_OBJ_MASK)!=JS_OBJ_T_RADIO&&(radio_tv_id&JS_OBJ_MASK)!=JS_OBJ_T_CHECKBOX)return -1;   /* this isn't radio nor TV */
	fd=jsint_find_document(document_id);
	if (!fd||!jsint_can_access(js_ctx,fd))return -1;
...
void js_upcall_set_checkbox_radio_default_checked(void *bidak_smirak, long document_id, long radio_tv_id, int value)
{
	...
	if (!js_ctx)internal((unsigned char *)"js_upcall_set_checkbox_radio_default_checked called with NULL context pointer\n");
	if ((radio_tv_id&JS_OBJ_MASK)!=JS_OBJ_T_RADIO&&(radio_tv_id&JS_OBJ_MASK)!=JS_OBJ_T_CHECKBOX)return;   /* this isn't radio nor TV */
	fd=jsint_find_document(document_id);
	if (!fd||!jsint_can_access(js_ctx,fd))return;
...
unsigned char *js_upcall_get_form_element_name(void *bidak, long document_id, long ksunt_id)
{
	...
	if (!js_ctx)internal((unsigned char *)"js_upcall_get_form_element_name called with NULL context pointer\n");
	fd=jsint_find_document(document_id);
	if (!fd||!jsint_can_access(js_ctx,fd))return NULL;
...
void js_upcall_set_form_element_name(void *bidak, long document_id, long ksunt_id, unsigned char *name)
{
	...
	if (!js_ctx)internal((unsigned char *)"js_upcall_set_form_element_name called with NULL context pointer\n");
	fd=jsint_find_document(document_id);
	...
	if ((name||(hopla->fc->name))&&strcmp((char *)name,(char *)hopla->fc->name))
	{
...
unsigned char *js_upcall_get_form_element_default_value(void *bidak, long document_id, long ksunt_id)
{
	...
	if (!js_ctx)internal((unsigned char *)"js_upcall_get_form_element_default_value called with NULL context pointer\n");
	fd=jsint_find_document(document_id);
	if (!fd||!jsint_can_access(js_ctx,fd))return NULL;
...
void js_upcall_set_form_element_default_value(void *bidak, long document_id, long ksunt_id, unsigned char *name)
{
	...
	if (!js_ctx)internal((unsigned char *)"js_upcall_set_form_element_default_value called with NULL context pointer\n");
	fd=jsint_find_document(document_id);
	if (!fd||!jsint_can_access(js_ctx,fd)){if (name)mem_free(name);return;}
	...
	if ((name||(hopla->fc->default_value))&&strcmp((char *)name,(char *)hopla->fc->default_value))
	{
...
unsigned char *js_upcall_get_form_element_value(void *bidak, long document_id, long ksunt_id)
{
	...
	if (!js_ctx)internal((unsigned char *)"js_upcall_get_form_element_value called with NULL context pointer\n");
	fd=jsint_find_document(document_id);
	if (!fd||!jsint_can_access(js_ctx,fd))return NULL;
...
void js_upcall_set_form_element_value(void *bidak, long document_id, long ksunt_id, unsigned char *name)
{
	...
	if (!js_ctx)internal((unsigned char *)"js_upcall_set_form_element_value called with NULL context pointer\n");
	fd=jsint_find_document(document_id);
	if (!fd||!jsint_can_access(js_ctx,fd)){if (name)mem_free(name);return;}
	...
	if (hopla->fs->state > strlen((char *)hopla->fs->value))
		hopla->fs->state = strlen((char *)hopla->fs->value);
	if ((ksunt_id&JS_OBJ_MASK) != JS_OBJ_T_TEXTAREA) {
		if (hopla->fs->vpos > strlen((char *)hopla->fs->value))
			hopla->fs->vpos = strlen((char *)hopla->fs->value);
	}
	mem_free(hopla);
	if (name)mem_free(name);
	redraw_document(fd);
...
void js_upcall_click(void *bidak, long document_id, long elem_id)
{
	...
	if (!js_ctx)internal((unsigned char *)"js_upcall_click called with NULL context pointer\n");
	fd=jsint_find_document(document_id);
	if (!fd||!jsint_can_access(js_ctx,fd))return;
...
void js_upcall_focus(void *bidak, long document_id, long elem_id)
{
	...
	if (!js_ctx)internal((unsigned char *)"js_upcall_focus called with NULL context pointer\n");
	fd=jsint_find_document(document_id);
	if (!fd||!jsint_can_access(js_ctx,fd))return;
			...
			for (a=0;a<fd->f_data->nlinks;a++)
			{
				...
				if (l->form&&l->form==hopla->fc)	/* to je on! */
				{
					...
					if (l->js_event&&l->js_event->focus_code)
						jsint_execute_code(fd,l->js_event->focus_code,strlen((char *)l->js_event->focus_code),-1,-1,-1);
...
void js_upcall_blur(void *bidak, long document_id, long elem_id)
{
	...
	if (!js_ctx)internal((unsigned char *)"js_upcall_blur called with NULL context pointer\n");
	fd=jsint_find_document(document_id);
	if (!fd||!jsint_can_access(js_ctx,fd))return;
				...
				for (a=0;a<fd->f_data->nlinks;a++)
				{
					...
					if (l->form&&l->form==hopla->fc)	/* to je on! */
					{
						...
						if (l->js_event&&l->js_event->blur_code)
							jsint_execute_code(fd,l->js_event->blur_code,strlen((char *)l->js_event->blur_code),-1,-1,-1);
...
void js_upcall_submit(void *bidak, long document_id, long form_id)
{
	...
	if (!js_ctx)internal((unsigned char *)"js_upcall_submit called with NULL context pointer\n");
	fd=jsint_find_document(document_id);
	if (!fd||!jsint_can_access(js_ctx,fd))return;
...
void js_upcall_reset(void *bidak, long document_id, long form_id)
{
	...
	if (!js_ctx)internal((unsigned char *)"js_upcall_reset called with NULL context pointer\n");
	fd=jsint_find_document(document_id);
	if (!fd||!jsint_can_access(js_ctx,fd))return;
	if ((form_id&JS_OBJ_MASK)!=JS_OBJ_T_FORM)return;
	if (!fd->f_data)return;
...
int js_upcall_get_radio_length(void *p, long document_id, long radio_id)
{
	...
	if (!js_ctx)internal((unsigned char *)"js_upcall_get_radio_length called with NULL context pointer\n");
	...
	foreachback(f,fd->f_data->forms)
		if (f->type==radio->type&&f->form_num==radio->form_num&&!strcmp((char *)radio->name,(char *)f->name))count++;
	mem_free(hopla);
	return count;
}
...
int js_upcall_get_select_length(void *p, long document_id, long select_id)
{
	...
	if (!js_ctx)internal((unsigned char *)"js_upcall_get_select_length called with NULL context pointer\n");
...
struct js_select_item* js_upcall_get_select_options(void *p, long document_id, long select_id, int *n)
{
	...
	*n=hopla->fc->nvalues;
	...
	if (!elektricke_pole)internal((unsigned char *)"Cannot allocate memory in js_upcall_get_select_options\n");
...
void js_upcall_goto_url(void * data)
{
	...
	if (!s)internal((unsigned char *)"js_upcall_goto_url called with NULL pointer\n");
	...
	if (js_manual_confirmation)
	{
		...
		/* goto the same url */
		{
			...
			if (p&&fd->loc&&fd->loc->url&&!strcmp((char *)txt,(char *)dest_url))
			{
				mem_free(dest_url);
				js_mem_free(data);
		...
		if (!jsid)internal((unsigned char *)"Cannot allocate memory in js_upcall_goto_url.\n");
...
int js_upcall_get_history_length(void *context)
{
	...
	if (!fd)internal((unsigned char *)"PerMe, PerMe, ja si te podam!\n");
...
void js_upcall_goto_history(void * data)
{
	...
	if (!s)internal((unsigned char *)"Hele, tyhle prasarny si zkousej na nekoho jinyho, jo?!\n");
	if (!(s->num)&&!(s->string))internal((unsigned char *)"Tak tohle na mne nezkousej, bidaku!\n");
	if ((s->num)&&(s->string))internal((unsigned char *)"Ta sedla!\n");
	...
	else	/* goto given url */
	{
		...
		foreach(loc,fd->ses->history)
		{
			if (!strcmp((char *)s->string,(char *)loc->url)){url=stracpy(s->string);history_num=a;break;}
			a++;
		}
	}
	...
	if (js_manual_confirmation)
	{
		...
		jsid=mem_alloc(sizeof(struct gimme_js_id_string));
		if (!jsid)internal((unsigned char *)"Can't allocate memory in js_upcall_goto_history\n");
...
void js_upcall_set_default_status(void *context, unsigned char *tak_se_ukaz_Kolbene)
{
	...
	if (!fd)internal((unsigned char *)"Tak tohle teda ne, bobanku!\n");
	...
	if (fd->f_data&&tak_se_ukaz_Kolbene)
	{
		...
		ct=get_translation_table(fd->f_data->cp,fd->f_data->opt.cp);
		trouba=convert_string(ct,tak_se_ukaz_Kolbene,strlen((char *)tak_se_ukaz_Kolbene),NULL); /* Taky to mate levnejsi - jinak by to stalo deset! */
...
unsigned char* js_upcall_get_default_status(void *context)
{
	...
	if (!fd)internal((unsigned char *)"Ale hovno!\n");
	...
	if (fd->f_data&&tak_se_ukaz_Danku)
	{
		...
		ct=get_translation_table(fd->f_data->opt.cp,fd->f_data->cp);
		trouba=convert_string(ct,tak_se_ukaz_Danku,strlen((char *)tak_se_ukaz_Danku),NULL);
...
void js_upcall_set_status(void *context, unsigned char *tak_se_ukaz_Kolbene)
{
	...
	if (!fd)internal((unsigned char *)"To leda tak -PRd!\n");
	...
	if (fd->f_data&&tak_se_ukaz_Kolbene)
	{
		...
		ct=get_translation_table(fd->f_data->cp,fd->f_data->opt.cp);
		trouba=convert_string(ct,tak_se_ukaz_Kolbene,strlen((char *)tak_se_ukaz_Kolbene),NULL);
		mem_free(tak_se_ukaz_Kolbene);
		/* a je to v troube... */
	}
	else
...
unsigned char* js_upcall_get_status(void *context)
{
	...
	if (!fd)internal((unsigned char *)"To leda tak hovno!\n");
...
unsigned char * js_upcall_get_cookies(void *context)
{
	...
	if (!fd)internal((unsigned char *)"Tak tomu rikam selhani komunikace...\n");
...
ty_uz_se_nevratis:
	...
	if (fd->js->ctx->cookies)
	{
		...
		else {add_to_str(&s,&l,(unsigned char *)"; ");add_to_str(&s,&l,fd->js->ctx->cookies);}
	}
	return s;
}
...
long * js_upcall_get_all(void *chuligane, long document_id, int *len)
{
	...
	if (!js_ctx)internal((unsigned char *)"js_upcall_get_all called with NULL context pointer\n");
	fd=jsint_find_document(document_id);
	if (!fd||!jsint_can_access(js_ctx,fd))return NULL;
...
long *js_upcall_get_images(void *chuligane, long document_id, int *len)
{
	...
	if (F)
	{
		if (!js_ctx)internal((unsigned char *)"js_upcall_get_images called with NULL context pointer\n");
		fd=jsint_find_document(document_id);
		if (!fd||!fd->f_data||!jsint_can_access(js_ctx,fd))return NULL;
		...
		*len=0;
		...
		pole_Premysla_Zavorace=mem_alloc((*len)*sizeof(long));
		if (!pole_Premysla_Zavorace)internal((unsigned char *)"Cannot allocate memory in js_upcall_get_images.");
...
int js_upcall_get_image_width(void *chuligane, long document_id, long image_id)
{
	...
	if (F)
	{
		if (!js_ctx)internal((unsigned char *)"js_upcall_get_image_width called with NULL context pointer\n");
...
int js_upcall_get_image_height(void *chuligane, long document_id, long image_id)
{
	...
	if (F)
	{
		if (!js_ctx)internal((unsigned char *)"js_upcall_get_image_height called with NULL context pointer\n");
		if ((image_id&JS_OBJ_MASK)!=JS_OBJ_T_IMAGE)return -1;
		fd=jsint_find_document(document_id);
		if (!fd||!jsint_can_access(js_ctx,fd))return -1;
...
int js_upcall_get_image_border(void *chuligane, long document_id, long image_id)
{
	...
	if (F)
	{
		if (!js_ctx)internal((unsigned char *)"js_upcall_get_image_border called with NULL context pointer\n");
		if ((image_id&JS_OBJ_MASK)!=JS_OBJ_T_IMAGE)return -1;
		fd=jsint_find_document(document_id);
		if (!fd||!jsint_can_access(js_ctx,fd))return -1;
...
int js_upcall_get_image_vspace(void *chuligane, long document_id, long image_id)
{
	...
	if (F)
	{
		if (!js_ctx)internal((unsigned char *)"js_upcall_get_image_vspace called with NULL context pointer\n");
		if ((image_id&JS_OBJ_MASK)!=JS_OBJ_T_IMAGE)return -1;
		fd=jsint_find_document(document_id);
		if (!fd||!jsint_can_access(js_ctx,fd))return -1;
...
int js_upcall_get_image_hspace(void *chuligane, long document_id, long image_id)
{
	...
	if (F)
	{
		if (!js_ctx)internal((unsigned char *)"js_upcall_get_image_hspace called with NULL context pointer\n");
		if ((image_id&JS_OBJ_MASK)!=JS_OBJ_T_IMAGE)return -1;
		fd=jsint_find_document(document_id);
		if (!fd||!jsint_can_access(js_ctx,fd))return -1;
...
unsigned char * js_upcall_get_image_name(void *chuligane, long document_id, long image_id)
{
	...
	if (F)
	{
		if (!js_ctx)internal((unsigned char *)"js_upcall_get_image_name called with NULL context pointer\n");
...
unsigned char * js_upcall_get_image_alt(void *chuligane, long document_id, long image_id)
{
	...
	if (F)
	{
		if (!js_ctx)internal((unsigned char *)"js_upcall_get_image_alt called with NULL context pointer\n");
		if ((image_id&JS_OBJ_MASK)!=JS_OBJ_T_IMAGE)return NULL;
		fd=jsint_find_document(document_id);
		if (!fd||!jsint_can_access(js_ctx,fd))return NULL;
...
void js_upcall_set_image_name(void *chuligane, long document_id, long image_id, unsigned char *name)
{
	...
	if (F)
	{
		if (!js_ctx)internal((unsigned char *)"js_upcall_set_image_name called with NULL context pointer\n");
...
void js_upcall_set_image_alt(void *chuligane, long document_id, long image_id, unsigned char *alt)
{
	...
	if (F)
	{
		if (!js_ctx)internal((unsigned char *)"js_upcall_set_image_alt called with NULL context pointer\n");
		if ((image_id&JS_OBJ_MASK)!=JS_OBJ_T_IMAGE)return;
		fd=jsint_find_document(document_id);
...
unsigned char * js_upcall_get_image_src(void *chuligane, long document_id, long image_id)
{
	...
	if (F)
	{
		if (!js_ctx)internal((unsigned char *)"js_upcall_get_image_src called with NULL context pointer\n");
		if ((image_id&JS_OBJ_MASK)!=JS_OBJ_T_IMAGE)return NULL;
		fd=jsint_find_document(document_id);
		if (!fd||!jsint_can_access(js_ctx,fd))return NULL;
...
void js_upcall_set_image_src(void *chuligane)
{
	...
	if (F)
	{
		...
		if (!chuligane)internal((unsigned char *)"js_upcall_set_image_src called with NULL argument\n");
		if (!js_ctx)internal((unsigned char *)"js_upcall_set_image_src called with NULL context pointer\n");
		image_id=fax->obj_id;
...
int js_upcall_image_complete(void *chuligane, long document_id, long image_id)
{
	...
	if (F)
	{
		if (!js_ctx)internal((unsigned char *)"js_upcall_image_complete called with NULL context pointer\n");
		if ((image_id&JS_OBJ_MASK)!=JS_OBJ_T_IMAGE)return -1;
		fd=jsint_find_document(document_id);
		if (!fd||!jsint_can_access(js_ctx,fd))return -1;
...
long js_upcall_get_parent(void *chuligane, long frame_id)
{
	...
	if (!js_ctx)internal((unsigned char *)"js_upcall_get_parent called with NULL context pointer\n");
...
long js_upcall_get_frame_top(void *chuligane, long frame_id)
{
	...
	if (!js_ctx)internal((unsigned char *)"js_upcall_get_frame_top called with NULL context pointer\n");
	if ((frame_id&JS_OBJ_MASK)!=JS_OBJ_T_FRAME&&(frame_id&JS_OBJ_MASK)!=JS_OBJ_T_DOCUMENT)return -1;
	fd=jsint_find_document(frame_id);
	if (!fd||!jsint_can_access(js_ctx,fd))return -1;
