...
void g_put_chars_conv(struct g_part *p, unsigned char *c, int l)
{
	...
	while (pp < l) {
		...
		while (*e) {
			...
			if (bp < CH_BUF) continue;
			flush1:
			g_put_chars(p, (unsigned char *)buffer, bp);
			bp = 0;
		}
	}
	if (bp) g_put_chars(p, (unsigned char *)buffer, bp);
}
...
unsigned char *make_html_font_name(int attr)
{
	...
	str=init_str();len=0;
	add_to_str(&str, &len, (unsigned char *)G_HTML_DEFAULT_FAMILY);
	add_to_str(&str, &len, attr & AT_BOLD ? (unsigned char *)"-bold" : (unsigned char *)"-medium");
	add_to_str(&str, &len, attr & AT_ITALIC ? (unsigned char *)"-italic-serif" :
			(unsigned char *)"-roman-serif");
	add_to_str(&str, &len, attr & AT_FIXED ? (unsigned char *)"-mono" : (unsigned char *)"-vari");
	return str;
}
...
void add_object_to_line(struct g_part *pp, struct g_object_line **lp, struct g_object *go)
{
	...
	if (go && (go->xw < 0 || go->yw < 0)) {
		internal((unsigned char *)"object has negative size: %d,%d", go->xw, go->yw);
		return;
	}
	if (!*lp) {
...
void split_line_object(struct g_part *p, struct g_object_text *text, unsigned char *ptr)
{
	...
	if (!(t2 = mem_calloc(sizeof(struct g_object_text) + strlen((char *)ptr) + 1))) return;
	...
	esp = 1;
	if (*ptr == ' ') {
		strcpy((char *)t2->text, (char *)ptr + 1);
		*ptr = 0;
		/*debug("split: (%s)(%s)", text->text, ptr + 1);*/
	} else {
		strcpy((char *)t2->text, (char *)ptr);
		ptr[0] = '-';
		ptr[1] = 0;
		esp = 0;
	}
	...
	if (text != p->text) {
		internal((unsigned char *)"split_line_object: bad wrap");
		t2->destruct(t2);
		mem_free(t2);
		return;
	}
	...
	p->w.last_wrap_obj = NULL;
	t2 = p->text;
	if (t2 && *t2->text && t2->text[strlen((char *)t2->text) - 1] == ' ') {
		p->w.last_wrap = &t2->text[strlen((char *)t2->text) - 1];
...
void g_html_form_control(struct g_part *p, struct form_control *fc)
{
	...
	if (fc->type == FC_TEXT || fc->type == FC_PASSWORD || fc->type == FC_TEXTAREA) {
		unsigned char *dv = convert_string(convert_table, fc->default_value, strlen((char *)fc->default_value), d_opt);
		if (dv) {
...
int is_in_area(struct map_area *a, int x, int y)
{
	...
	switch (a->shape) {
		...
		case SHAPE_POLY:
			...
			return over & 1;
		default:
			internal((unsigned char *)"is_in_area: bad shape: %d", a->shape);
	}
	return 0;
}
	...
	if (im->usemap && p->data) {
		...
		if (af->rq && (af->rq->state == O_LOADING || af->rq->state == O_INCOMPLETE || af->rq->state == O_OK) && af->rq->ce) {
			...
			for (i = 0; menu[i].text; i++) {
				...
				struct link *link;
				int shape = !ld->shape || !*ld->shape ? SHAPE_RECT : !strcasecmp((char *)ld->shape, "default") ? SHAPE_DEFAULT : !strcasecmp((char *)ld->shape, "rect") ? SHAPE_RECT : !strcasecmp((char *)ld->shape, "circle") ? SHAPE_CIRCLE : !strcasecmp((char *)ld->shape, "poly") ? SHAPE_POLY : -1;
				if (shape == -1) continue;
...
void *g_html_special(struct g_part *p, int c, ...)
{
	...
	switch (c) {
		case SP_TAG:
			...
			if ((tag = mem_calloc(sizeof(struct g_object_tag) + strlen((char *)t) + 1))) {
				tag->mouse_event = g_dummy_mouse;
				tag->draw = g_dummy_draw;
				tag->destruct = g_tag_destruct;
				strcpy((char *)tag->name, (char *)t);
				flush_pending_text_to_line(p);
		...
		default:
			va_end(l);
			internal((unsigned char *)"html_special: unknown code %d", c);
	}
	return NULL;
}
...
void g_put_chars(struct g_part *p, unsigned char *s, int l)
{
	...
	xstrcmp(format.link, last_link) || xstrcmp(format.target, last_target) ||
	    ...
	    || ((format.js_event || last_js_event) && compare_js_event_spec(format.js_event, last_js_event))	) {
			/*if (!html_format_changed) internal((unsigned char *)"html_format_changed not set");*/
			flush_pending_text_to_line(p);
	...
	if (p->pending_text_len == -1) {
		p->pending_text_len = strlen((char *)p->text->text);
		goto a1;
	}
	...
	if ((last_link /*|| last_target*/ || last_image || last_form) &&
		...
		if (!p->data->nlinks) {
			internal((unsigned char *)"no link");
			goto back_link;
		}
...
{
	pr(
	parse_html((unsigned char *)start, (unsigned char *)end, (void (*)(void *, unsigned char *, int)) g_put_chars_conv, (void (*)(void *)) g_line_break, (void *(*)(void *, int, ...)) g_html_special, part, head);
...
struct g_part *g_format_html_part(unsigned char *start, unsigned char *end, int align, int m, int width, unsigned char *head, int link_num, unsigned char *bg, unsigned char *bgcolor, struct f_data *f_d)
{
	...
	p->cx = -1;
	g_do_format((char *)start, (char *)end, p, head);
	/*nobreak = 0;*/
	line_breax = 1;
	while (&html_top != e) {
		kill_html_stack_item(&html_top);
		if (!&html_top || (void *)&html_top == (void *)&html_stack) {
			internal((unsigned char *)"html stack trashed");
			break;
		}
	}
	html_top.dontkill = 0;
