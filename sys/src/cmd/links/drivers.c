...
#endif
#ifdef GRDRV_PLAN9
extern struct graphics_driver plan9_driver;
#endif
...
{ 
	...
	for (gd = graphics_drivers; *gd; gd++) {
		if (l) add_to_str(&d, &l, (unsigned char *)" ");
...
unsigned char *init_graphics_driver(struct graphics_driver *gd, unsigned char *param, unsigned char *display)
{
	...
	if (dp->shell) strncpy((char *)gd->shell,(char *)dp->shell,MAX_STR_LEN);
...
unsigned char *init_graphics(unsigned char *driver, unsigned char *param, unsigned char *display)
{
	...
	struct graphics_driver **gd;
	for (gd = graphics_drivers; *gd; gd++) {
		if (!driver || !*driver || !strcasecmp((char *)(*gd)->name, (char *)driver)) {
			unsigned char *r;
			...
			if (!l) {
				if (!driver || !*driver) add_to_str(&s, &l, (unsigned char *)"Could not initialize any graphics driver. Tried the following drivers:\n");
				else add_to_str(&s, &l, (unsigned char *)"Could not initialize graphics driver ");
			}
	...
	if (!l) {
		add_to_str(&s, &l, (unsigned char *)"Unknown graphics driver ");
		if (driver) add_to_str(&s, &l, driver);
		add_to_str(&s, &l, (unsigned char *)".\nThe following graphics drivers are supported:\n");
		for (gd = graphics_drivers; *gd; gd++) {
			if (gd != graphics_drivers) add_to_str(&s, &l, (unsigned char *)", ");
			add_to_str(&s, &l, (*gd)->name);
		}
		add_to_str(&s, &l, (unsigned char *)"\n");
	}
