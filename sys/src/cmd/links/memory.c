...
{
	struct cache_upcall *c;
	if ((c = mem_alloc(sizeof(struct cache_upcall) + strlen(( char *)name) + 1))) {
		c->upcall = upcall;
...
void free_all_caches(void)
{
	...
	if (!(a & ST_CACHE_EMPTY)) {
		...
		foreach(c, cache_upcalls) if (!(c->upcall(SH_FREE_ALL) & ST_CACHE_EMPTY)) {
			if (l) add_to_str(&m, &l, (unsigned char *)", ");
			add_to_str(&m, &l, c->name);
		}
		internal((unsigned char *)"could not release entries from caches: %s", m);
		mem_free(m);
	}
