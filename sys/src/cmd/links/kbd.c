...
{
	int l;
	if (!itrm->eqlen) internal((unsigned char *)"event queue empty");
...
unsigned char *init_seq = (unsigned char *)"\033)0\0337";
unsigned char *init_seq_x_mouse = (unsigned char *)"\033[?1000h";
unsigned char *init_seq_tw_mouse = (unsigned char *)"\033[?9h";
unsigned char *term_seq = (unsigned char *)"\033[2J\0338\r \b";
unsigned char *term_seq_x_mouse = (unsigned char *)"\033[?1000l";
unsigned char *term_seq_tw_mouse = (unsigned char *)"\033[?9l";

/*unsigned char *term_seq = "\033[2J\033[?1000l\0338\b \b";*/

void send_init_sequence(int h,int flags)
{
	hard_write(h, init_seq, strlen(( char *)init_seq));
	if (flags & USE_TWIN_MOUSE) {
		hard_write(h, init_seq_tw_mouse, strlen(( char *)init_seq_tw_mouse));
	} else if (flags & USE_X_MOUSE) {
		hard_write(h, init_seq_x_mouse, strlen(( char *)init_seq_x_mouse));
	}
}

void send_term_sequence(int h,int flags)
{
	hard_write(h, term_seq, strlen(( char *)term_seq));
	if (flags & USE_TWIN_MOUSE) {
		hard_write(h, term_seq_tw_mouse, strlen(( char *)term_seq_tw_mouse));
	} else if (flags & USE_X_MOUSE) {
		hard_write(h, term_seq_x_mouse, strlen(( char *)term_seq_x_mouse));
	}
}
...
void handle_trm(int std_in, int std_out, int sock_in, int sock_out, int ctl_in, void *init_string, int init_len)
{
	...
	if (get_terminal_size(ctl_in, &x, &y)) {
		error((unsigned char *)"ERROR: could not get terminal size");
		return;
	}
	...
	itrm->flags = 0;
	if (!(ts = (unsigned char *)getenv("TERM"))) ts = (unsigned char *)"";
	if ((xwin & ENV_TWIN) && !strcmp(( char *)ts, "linux")) itrm->flags |= USE_TWIN_MOUSE;
	if ((xwin & ENV_XWIN)) itrm->flags |= USE_X_MOUSE;
	if (strlen(( char *)ts) >= MAX_TERM_LEN) queue_event(itrm, ts, MAX_TERM_LEN);
	else {
		unsigned char *mm;
		int ll = MAX_TERM_LEN - strlen(( char *)ts);
		queue_event(itrm, ts, strlen(( char *)ts));
		if (!(mm = mem_alloc(ll))) {
			itrm->free_trm(itrm);
			return;
		}
	...
	if (!(ts = get_cwd()) && !(ts = stracpy((unsigned char *)""))) goto neni_pamet;
	if (strlen(( char *)ts) >= MAX_CWD_LEN) queue_event(itrm, ts, MAX_CWD_LEN);
	else {
		unsigned char *mm;
		int ll = MAX_CWD_LEN - strlen(( char *)ts);
		queue_event(itrm, ts, strlen(( char *)ts));
		if (!(mm = mem_alloc(ll))) {
			itrm->free_trm(itrm);
			return;
	...
	set_window_title((unsigned char *)"Links");
...
/*	NO_GFX;*/
	...
	/*if (ditrm->sock_out != fd) {
		internal((unsigned char *)"unblock_itrm: bad fd: %d", fd);
		return -1;
	}*/
...
void block_itrm(int fd)
{
	...
	if (!itrm) return;
	/*if (ditrm->sock_out != fd) {
		internal((unsigned char *)"block_itrm: bad fd: %d", fd);
		return;
	}*/
	itrm->blocked = 1;
	block_stdin();
...
void resize_terminal_x(unsigned char *text)
{
	int x, y;
	unsigned char *p;
	if (!(p = (unsigned char *)strchr(( char *)text, ','))) return;
	*p++ = 0;
	x = atoi(( char *)text);
	y = atoi(( char *)p);
	resize_window(x, y);
...
void in_sock(struct itrm *itrm)
{
	...
	if (!*path) {
		...
		unsigned char *param;
		if (is_blocked() && fg) {
			if (*delete) unlink(( char *)delete);
			goto to_je_ale_hnus;
		}
		if (!(param = mem_alloc(strlen(( char *)path) + strlen(( char *)delete) + 3))) goto to_je_ale_hnus;
		param[0] = fg;
...
void kbd_timeout(struct itrm *itrm)
{
	...
	if (!itrm->qlen) {
		internal((unsigned char *)"timeout on empty queue");
		return;
	}
...
int process_queue(struct itrm *itrm)
{
	...
	if (itrm->kqueue[0] == '\033') {
		...
		if (itrm->kqueue[1] == '[' || itrm->kqueue[1] == 'O') {
			...
			if (itrm->qlen >= 4 && itrm->kqueue[2] == '[') {
				...
				el = 4;
			} else if (get_esc_code(( char *)itrm->kqueue, itrm->qlen, &c, &v, &el)) goto ret;
			else switch (c) {
	...
	l1:
	if (itrm->qlen < el) {
		internal((unsigned char *)"event queue underflow");
		itrm->qlen = el;
	}
	if (ev.x != -1) {
		itrm->queue_event(itrm, (unsigned char *)&ev, sizeof(struct event));
...
void in_kbd(struct itrm *itrm)
{
	...
	if ((itrm->qlen += r) > IN_BUF_SIZE) {
		error((unsigned char *)"ERROR: too many bytes read");
