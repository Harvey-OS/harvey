...
restart_again:
	...
	if (deco->width<0||deco->height<0)	/* decoding header */
	{
		...
		p=my_memmem(deco->buffer,deco->buffer_pos,(unsigned char *)"width",5);
		q=my_memmem(deco->buffer,deco->buffer_pos,(unsigned char *)"height",6);
...
cycle_again:
	...
	switch(deco->state)
	{
		...
		case 1: /* za 1. lomitkem */
		{
			...
			xbm_decode(cimg, (unsigned char *)"/", 1);
