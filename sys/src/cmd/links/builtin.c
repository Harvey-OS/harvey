			...
			{	case UNDEFINED: \
				...
				case ARRAY: \
					...
					if(pomvar1->type!=PARLIST) \
						my_internal((unsigned char *)"Parent list corrupted!\n",context); \
					add_to_parlist(pomvar,pomvar1); \
				break; \
				default: \
					my_internal((unsigned char *)"Internal: Strange array-element assignment!\n",context); \
				break; \
			} \
			js_mem_free(b); 
...
{	struct fax_me_tender_2_stringy *faxoff = data;
	...
	char* kod=(char *)faxoff->string1;
	void*fdatac=context->ptr;
	*(int*)faxoff->string2=-1;
	js_mem_free(data);
	jsint_execute_code(fdatac,(unsigned char *)kod,strlen(kod),-1,-1,-1);
	if(!na_bordel)internal((unsigned char *)"Ztraceny text pri interpretovani!\n");
	...
	{	pom=na_bordel->next;
		...
		if(!na_bordel->next)
			internal((unsigned char *)"Ztraceny text pri interpretovani!\n");
		pom=na_bordel->next;
...
static char* call_setTimeout(char*string,int time,js_context*context)
{
	...
	faxoff->string2=(unsigned char *)&context->t[timerno];
	context->bordely[timerno]=(js_bordylek*)faxoff;
	faxoff->ident=context;
	faxoff->string1=(unsigned char *)string;
...
{	struct fax_me_tender_int_string *faxoff=js_mem_alloc(sizeof(struct fax_me_tender_int_string));
	...
	if(!(faxoff->num=integer))
		faxoff->string=(unsigned char *)string;
	else
		faxoff->string=0;
	context->upcall_data=faxoff;
...
{	struct fax_me_tender_string_2_longy * to_je_ale_woprusz=js_mem_alloc(sizeof(struct fax_me_tender_string_2_longy));
	to_je_ale_woprusz->ident=(char *)context->ptr;
	to_je_ale_woprusz->string=(unsigned char *)stracpy1((char *)string);
...
static void call_prompt(js_context*context,char*string1,char*string2)
{
	struct fax_me_tender_2_stringy *faxoff=js_mem_alloc(sizeof(struct fax_me_tender_2_stringy));
	faxoff->ident=context->ptr;
	faxoff->string1=(unsigned char *)string1;
	faxoff->string2=(unsigned char *)string2;
	context->upcall_data=faxoff;
...
/*			js_mem_free(vysl);*/
		...
		case Cwrite:
			...
			pomstr=tostring(force_getarg(&argy),context);
			js_upcall_document_write((unsigned char *)context->ptr,(unsigned char *)pomstr,strlen(pomstr));
			js_mem_free(pomstr);
		...
		case Cwriteln: 
			...
			js_upcall_document_write((unsigned char *)context->ptr,(unsigned char *)pomstr1,strlen(pomstr1));
			js_mem_free(pomstr);
			js_mem_free(pomstr1);
		...
		case Cjoin:
			...
			if(pomvar->type!=INTVAR)my_internal((unsigned char *)"Internal:Strange type of \"length\" property!\n",context);
...
/*		case Cshit:
		...
		case Cunshit:
			...
			{	case UNDEFINED:
				...
				break;
				default:
					my_internal((unsigned char *)"Internal: Unknown types assigned!\n",context);
				break;
			}
			js_mem_free(cislo);
			js_mem_free(pomarg);
		...
		case Cshift:
			...
			if(pomvar->type!=INTVAR)
				internal((unsigned char *)"Internal: Strange type of \"length\" property!\n");
			if(pomvar->handler<=0)
		...
		case Cunshift:
			idebug("Cunshit called ");
			pomvar=lookup(js_lengthid,(plns*)variable->handler,context);
			if(pomvar->type!=INTVAR)internal((unsigned char *)"Internal: Strange type of \"length\" property!\n");
			cislo=js_mem_alloc(DELKACISLA);
			cislo[DELKACISLA-1]='\0';
			snprintf(cislo,DELKACISLA-1,"%d",(int)(pomint=pomvar->handler++));
			pomvar=llookup(cislo,context->namespace,(plns*)variable->handler,context);
			...
			{	case UNDEFINED:
				...
				case ADDRSPACEP:
					...
					pomvar1=lookup(MIN1KEY,(plns*)pomarg->argument,context);
					if(pomvar1->type!=PARLIST)
					internal((unsigned char *)"Parent list corrupted!!!!\n");
					add_to_parlist(pomvar,pomvar1);
				break;
				default:
					internal((unsigned char *)"Internal: Unknown types assigned!\n");
				break;
			}
			js_mem_free(cislo);
			js_mem_free(pomarg);
...
/*			js_mem_free(pomstr2);*/
		...
		case CIntMETFUN: /* but not written */
			...
			my_internal((unsigned char *)"Blby operator vestavene metody!!\n",context);
/*			printf("dobehl!\n");*/
		...
		case CtoString:
			...
			{	case C_OBJ_Objekt:
				...
				break;
				case C_OBJ_location:
					pomstr=(char *)js_upcall_get_location((unsigned char *)context->ptr);
				...
				break;
				default:
					internal((unsigned char *)"Strange type of object asked for toString method!\n");
				break;
			}
		break;
		case Cfloor:
			...
			{	
				if (f1 <= -MAXINT+1 || f1 >= MAXINT-1) *(float *)retval=f1;
					...
					*(float*)retval=floor(f1);
					...
					if(((float)(pomint=(int)(*(float*)retval)))==*(double*)retval)
					{	rettype=INTEGER;
						js_mem_free((float*)retval);
						retval=pomint;
						idebug("je to cele!\n");
		...
		case Cmd5:
			rettype=STRING;
			pomstr=tostring(force_getarg(&argy),context);
			retval=(long)MD5Data((unsigned char *)pomstr,strlen(pomstr),0);
			if(pomstr)js_mem_free(pomstr);
		break;
		case Ceval:
		...
		default:
			zrusargy(argy,context);
/*			js_mem_free(vysl);*/
			internal((unsigned char *)"Internal: Strange internal function number!!\n");
		break;	
	}
	zrusargy(argy,context);
	vysl->typ=rettype;
...
{	long* policko;
	...
	{	case Cmatika:
		...
		case Ctitle:
			*typ=STRING;
			pomstr=(char *)js_upcall_get_title(context->ptr);
			*value=(long)stracpy1(pomstr);
			if(pomstr)mem_free(pomstr);
		break;
		case ClastModified:
			*typ=STRING;
			pomstr=(char *)js_upcall_document_last_modified(context->ptr,pna->mid);
			*value=(long)stracpy1(pomstr);
			if(pomstr)mem_free(pomstr);
		break;
		case CuserAgent:
			*typ=STRING;
			pomstr=(char *)js_upcall_get_useragent(context->ptr);
			*value=(long)stracpy1(pomstr);
                        if(pomstr)mem_free(pomstr);
		break;
		case Creferer:
			*typ=STRING;
			pomstr=(char *)js_upcall_get_referrer(context->ptr);
			*value=(long)stracpy1(pomstr);
                        if(pomstr)mem_free(pomstr);
		break;
		case CappCodeName:
			*typ=STRING;
			pomstr=(char *)js_upcall_get_appcodename();
			*value=(long)stracpy1(pomstr);
                        if(pomstr)mem_free(pomstr);
		break;
		case CappName:
			*typ=STRING;
			pomstr=(char *)js_upcall_get_appname();
			*value=(long)stracpy1(pomstr);
                        if(pomstr)mem_free(pomstr);
		break;
		case CappVersion:
			*typ=STRING;
			pomstr=(char *)js_upcall_get_appversion();
			*value=(long)stracpy1(pomstr);
                        if(pomstr)mem_free(pomstr);
		break;
		case CIntMETFUN:
			...
			{	*typ=FLOAT;
                       ...
                       {       *typ=FLOAT;
                               ...
                               *(float*)(*value)=MY_NAN; 
                               ...
                               internal((unsigned char *)"Strange internal property name!!\n");
/*		       printf("dobehl\n");*/
                                        
                break;	
		case Chash:
			*typ=STRING;
			pomstr=(char *)js_upcall_get_location_hash(context->ptr);
			*value=(long)stracpy1(pomstr);
                        if(pomstr)mem_free(pomstr);
		break;
		case Chost:
			*typ=STRING;
			pomstr=(char *)js_upcall_get_location_host(context->ptr);
			*value=(long)stracpy1(pomstr);
                        if(pomstr)mem_free(pomstr);
		break;
		case Chostname:
			*typ=STRING;
			pomstr=(char *)js_upcall_get_location_hostname(context->ptr);
			*value=(long)stracpy1(pomstr);
                        if(pomstr)mem_free(pomstr);
		break;
		case Chref:
			*typ=STRING;
			pomstr=(char *)js_upcall_get_location(context->ptr);
			*value=(long)stracpy1(pomstr);
                        if(pomstr)mem_free(pomstr);
		break;
		case Cpathname:
			*typ=STRING;
			pomstr=(char *)js_upcall_get_location_pathname(context->ptr);
			*value=(long)stracpy1(pomstr);
                        if(pomstr)mem_free(pomstr);
		break;
		case Cport:
			*typ=STRING;
			pomstr=(char *)js_upcall_get_location_port(context->ptr);
			*value=(long)stracpy1(pomstr);
                        if(pomstr)mem_free(pomstr);
		break;
		case Cprotocol:
			*typ=STRING;
			pomstr=(char *)js_upcall_get_location_protocol(context->ptr);
			*value=(long)stracpy1(pomstr);
                        if(pomstr)mem_free(pomstr);
		break;
		case Csearch:
			*typ=STRING;
			pomstr=(char *)js_upcall_get_location_search(context->ptr);
			*value=(long)stracpy1(pomstr);
                        if(pomstr)mem_free(pomstr);
		break;
		case Cname:
			*typ=STRING;
			pomstr=(char *)js_upcall_get_window_name(context->ptr);
			*value=(long)stracpy1(pomstr);
                        if(pomstr)mem_free(pomstr);
		break;
		case Clinks:
...
/*			BIVAR1("text",Cselecttext,pna->mid,pna->handler);
		...
		case Cselectname:
			...
			*typ=STRING;
			pomstr=(char *)js_upcall_get_form_element_name(context->ptr,pna->handler,pna->mid);
			*value=(long)stracpy1(pomstr);
			if(pomstr)	mem_free(pomstr);
			idebug("and exited!\n");
		break;
		...
		case Cselectmrchatext:
			...
			*typ=STRING; /* Lepsi by bylo boobean na pocest pana GNU - be-e-e-e! */
			ale_te_lipy_se_nevzdam=js_upcall_get_select_options(context->ptr,pna->handler,pna->mid,&pomint);
			*value=(long)stracpy1((char *)ale_te_lipy_se_nevzdam[pna->index].text);
			while(pomint--){
				if(ale_te_lipy_se_nevzdam[pomint].text)mem_free(ale_te_lipy_se_nevzdam[pomint].text);
				if(ale_te_lipy_se_nevzdam[pomint].value)mem_free(ale_te_lipy_se_nevzdam[pomint].value);
			}
...
/*		case Cselectindex:
		...
		case Cvalue:
			...
			*typ=STRING;
			pomstr=(char *)js_upcall_get_form_element_value(context->ptr,pna->handler,pna->mid);
			*value=(long)stracpy1(pomstr);
                        if(pomstr)mem_free(pomstr);
			idebug("and exited\n");
		...
		case CdefaultValue:
			...
			*typ=STRING;
			pomstr=(char *)js_upcall_get_form_element_default_value(context->ptr,pna->handler,pna->mid);
			*value=(long)stracpy1(pomstr);
                        if(pomstr)mem_free(pomstr);
			idebug("and exited\n");
		break;
		case Ctarget:
			...
			*typ=STRING;
			pomstr=(char *)js_upcall_get_link_target(context->ptr,pna->handler,pna->mid);
			*value=(long)stracpy1(pomstr);
                        if(pomstr)mem_free(pomstr);
		...
		case Caction:
			...
			*typ=STRING;
			pomstr=(char *)js_upcall_get_form_action(context->ptr,pna->handler,pna->mid);
			*value=(long)stracpy1(pomstr);
                        if(pomstr)mem_free(pomstr);
			idebug("and exited\n");
		break;
			...
			*value=(long)(pomns=js_mem_alloc(sizeof(plns)));
			...
			{	snprintf(pomstr,DELKACISLA,"%d",i);
				...
				{	case JS_OBJ_T_TEXT:
					...
					break;
					default: my_internal((unsigned char *)"GNU! To je typek!\n",context);
					break;
				}
				i++;
			}
		...
		case Cencoding:
			...
			*typ=STRING;
			pomstr=(char *)js_upcall_get_form_encoding(context->ptr,pna->handler,pna->mid);
			*value=(long)stracpy1(pomstr);
                        if(pomstr)mem_free(pomstr);
			idebug("and exited!\n");
		break;
		case Cmethod:
			...
			*typ=STRING;
			pomstr=(char *)js_upcall_get_form_method(context->ptr,pna->handler,pna->mid);
			*value=(long)stracpy1(pomstr);
                        if(pomstr)mem_free(pomstr);
			idebug("and exited!\n");
		break;
		case Cftarget:
			...
			*typ=STRING;
			pomstr=(char *)js_upcall_get_form_target(context->ptr,pna->handler,pna->mid);
			*value=(long)stracpy1(pomstr);
                        if(pomstr)mem_free(pomstr);
			idebug("and exited!\n");
		break;
		...
		case Cstatus:
			...
			*typ=STRING;
			pomstr=(char *)js_upcall_get_status(context->ptr);
			*value=(long)stracpy1(pomstr);
                        if(pomstr)mem_free(pomstr);
			idebug("and exited!\n");
		break;
		case CdefaultStatus:
			...
			*typ=STRING;
			pomstr=(char *)js_upcall_get_default_status(context->ptr);
			*value=(long)stracpy1(pomstr);
                        if(pomstr)mem_free(pomstr);
			idebug("and exited!\n");
		break;
		case Ccookie:
			...
			*typ=STRING;
			pomstr=(char *)js_upcall_get_cookies(context->ptr);
			*value=(long)stracpy1(pomstr);
                        if(pomstr)mem_free(pomstr);
			idebug("and exited!\n");
		break;
		...
		case Cimagealt:
			...
			*typ=STRING;
			pomstr=(char *)js_upcall_get_image_alt(context->ptr,pna->handler,pna->mid);
			*value=(long)stracpy1(pomstr);
			if(pomstr)mem_free(pomstr);
			idebug("and exited!\n");
		break;
		...
		case Csrc:
			idebug("Csrc called ");
                        *typ=STRING;
                        pomstr=(char *)js_upcall_get_image_src(context->ptr,pna->handler,pna->mid);
			*value=(long)stracpy1(pomstr);
                        if(pomstr)mem_free(pomstr);
                        idebug("and exited!\n");
		break;
...
/*		case Clowsrc:
		...
		case Cimagename:
			idebug("Cname called ");
                        *typ=STRING;
                        pomstr=(char *)js_upcall_get_image_name(context->ptr,pna->handler,pna->mid);
			*value=(long)stracpy1(pomstr);
                        if(pomstr)mem_free(pomstr);
                        idebug("and exited!\n");
		break;
	...
	{	case UNDEFINED:
		...
		case ARRAY:
			...
			if(fotri->type!=PARLIST)
				my_internal((unsigned char *)"Parentlist corrupted!\n",context);
			...
			pomstr1=(char *)stracpy((unsigned char *)pomstr);
			if(pomstr)js_mem_free(pomstr);
			return pomstr1;
			idebug("iatostring: Pozor! objekt->string dosud neni vyladen\n");
		break;
		default:
			my_internal((unsigned char *)"Unknown type for assign to intern. var.!\n",context);
		break;
	}
	pombuf=js_mem_alloc(sizeof(abuf));
	pombuf->typ=typ;
...
/*			idebug("iatobool: Pozor, objekt->boolean neni vyladeno!\n");*/
		break;
		default:
			internal((unsigned char *)"Unknown type for assign to intern. var.!\n");
		break;
	}
	pombuf=js_mem_alloc(sizeof(abuf));
	pombuf->typ=typ;
...
/*	float*pomfloat;*/
	...
	{	case Cmatika:
		...
		break;
		case Ctitle:
			js_upcall_set_title((unsigned char *)context->ptr,(unsigned char *)iatostring(typ,value,context));
		...
		break;
		case Cfename:
			js_upcall_set_form_element_name((unsigned char *)context->ptr,pna->handler,pna->mid,(unsigned char *)iatostring(typ,value,context));
		...
		break;
		case Cvalue:
			js_upcall_set_form_element_value((unsigned char *)context->ptr,pna->handler,pna->mid,(unsigned char *)iatostring(typ,value,context)); 
			idebug("Nastavena promenna Cvalue\n");
		break;
		case CdefaultValue:
			js_upcall_set_form_element_default_value(context->ptr,pna->handler,pna->mid,(unsigned char *)iatostring(typ,value,context));
		...
		break;
		case Cstatus:
			js_upcall_set_status((unsigned char *)context->ptr,(unsigned char *)iatostring(typ,value,context));
			idebug("Nastaveny status\n");
		break;
		case CdefaultStatus:
			js_upcall_set_default_status((unsigned char *)context->ptr,(unsigned char *)iatostring(typ,value,context));
		...
		case Ccookie:
			...
			pomstr=iatostring(typ,value,context);
			context->cookies=(unsigned char *)stracpy1(pomstr);
			if(pomstr)mem_free(pomstr);
		...
		break;
		case Cimagename:
			js_upcall_set_image_name(context->ptr,pna->handler,pna->mid,(unsigned char *)iatostring(typ,value,context));
			idebug("Nastaveno jmeno obrazku\n");
		break;
		case Cimagealt:
			js_upcall_set_image_alt(context->ptr,pna->handler,pna->mid,(unsigned char *)iatostring(typ,value,context));
...
/*		case Cforms:
		...
		default:
			printf("%d - \n",(int)pna->value);
			internal((unsigned char *)"Killing unknown variable!\n");
		break;
	}
}
...
{	js_context*context=jezis_to_je_ale_krawina;
	...
	if(context->running)
	{	bufet=pulla(context);
		if(!bufet)my_internal((unsigned char *)"Downcall narazil hlavou do skaly!\n",context);
		if(bufet->typ!=UNDEFINED)my_internal((unsigned char *)"Nevyzadany downcall!!\n",context);
...
{	js_context*context=jezis_to_je_ale_krawina;
	...
	{	bufet=pulla(context);
		if(!bufet)my_internal((unsigned char *)"Downcall narazil hlavou do skaly!\n",context);
		if(bufet->typ!=UNDEFINED)my_internal((unsigned char *)"Nevyzadany downcall!!\n",context);
		idebug((unsigned char *)"Vracim se z upcallu s pravdivym vysledkem!\n");
		if(context->current)
		{	pusha(bufet,context);
			bufet->typ=BOOLEAN;
			bufet->argument=TRUE;
...
{	js_context*context=jezis_to_je_ale_krawina;
	...
	if(context->running)
	{	bufet=pulla(context);
		if(!bufet)my_internal((unsigned char *)"Baudys narazil hlavou do skaly! B-)\n",context);
		if(bufet->typ!=UNDEFINED)my_internal((unsigned char *)"Nevyzadany downcall!!\n",context);
...
{	js_context*context=jezis_to_je_ale_krawina;
	...
	if(context->running)
	{	bufet=pulla(context);
		if(!bufet)my_internal((unsigned char *)"Downcall narazil hlavou do futra!\n",context);
		if(bufet->typ!=UNDEFINED)my_internal((unsigned char *)"Nevyzadany downcall!!\n",context);
