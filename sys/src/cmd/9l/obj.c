...
char	symname[]	= SYMDEF;
char	thechar		= '9';
char	*thestring 	= "29000";

/*
 *	-H0 -T0 -R4			is start at 0 text
...
main(int argc, char *argv[])
{
	...
	if(!debug['m']) {
		...
		lookup("_mulul", 0)->type = SXREF;
		lookup("_mulml", 0)->type = SXREF;
		lookup("_mulmul", 0)->type = SXREF;
	}
	if(!debug['d']) {
