...
properand(Instr *i, int and)
{
	...
	case I:
	case O:
		if (findsym(o->val, CANY, &s) && o->val-s.value < 0x1000000)
...
i386regval(int reg)
{
	...
	case SI:
	case DI:
		return rget(regname[reg-AX+EAX]) & 0xffff;
	case AL:
	case CL:
	case DL:
	case BL:
		return rget(regname[reg-AL+EAX]) & 0xff;
	case AH:
	case CH:
	case DH:
	case BH:
		return (rget(regname[reg-AH+EAX]) & 0xff00)>>8;
	}
}
...
i386foll(ulong pc, ulong *foll)
{
	...
	if(strcmp(in.op.name, "RET") == 0){
		get4(cormap, rget("SP"), SEGDATA, (long *) &l);
		foll[0] = l;
		return 1;
	}
	foll[0] = mydot+in.n;
