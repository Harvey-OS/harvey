...
rread(Req* r)
{
	...
	chat("read(fid=%d,offset=%lld,count=%d)...", thdr.fid, thdr.offset,
	     thdr.count);
	errno = 0;
...
rwrite(Req* r)
{
	...
	chat("write(fid=%d,offset=%lld,count=%d)...", thdr.fid, thdr.offset,
	     thdr.count);
...
rcreate(Req* r)
{
	...
	chat("create(fid=%d,name=\"%s\",perm=%uo,mode=%d)...", thdr.fid,
	     thdr.name, thdr.perm, thdr.mode);
	...
	errno = 0;
	if(thdr.perm & DMDIR) {
		perm = (thdr.perm & ~0777) | (getmode(f) & thdr.perm & 0777);
		perm |= S_IFDIR;
