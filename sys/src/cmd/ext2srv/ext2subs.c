...
ext2fs(Xfs* xf)
{
	...
	if(xf->block_size == OFFSET_SUPER_BLOCK)
		...
		xf->superaddr = 0, xf->superoff = OFFSET_SUPER_BLOCK,
		xf->grpaddr = 1;
	else {
		chat(" blocks of %d bytes are not supported...",
		     xf->block_size);
		errno = Eformat;
		return -1;
	}
	...
	xf->ngroups =
	    (superblock.s_blocks_count - superblock.s_first_data_block +
...
getext2(Xfs* xf, char type, int n)
{
	...
	case EXT2_BBLOCK:
		...
		if(!bd)
			goto error;
		e.buf =
		    getbuf(xf, DESC_OFFSET(xf, bd->iobuf, n)->bg_block_bitmap);
		if(!e.buf) {
			putbuf(bd);
			goto error;
		}
	...
	case EXT2_BINODE:
		...
		if(!bd)
			goto error;
		e.buf =
		    getbuf(xf, DESC_OFFSET(xf, bd->iobuf, n)->bg_inode_bitmap);
		if(!e.buf) {
			putbuf(bd);
			goto error;
		}
...
get_inode(Xfile* file, uint nr)
{
	...
	block = ed.u.gd->bg_inode_table +
	        (((nr - 1) % xf->inodes_per_group) / xf->inodes_per_block);
	putext2(ed);
...
getname(Xfile* f, char* str)
{
	...
	for(i = 0; (i < nblock) && (i < EXT2_NDIR_BLOCKS); i++) {
		...
		for(offset = 0; offset < xf->block_size;) {
			dir = (DirEntry*)(buf->iobuf + offset);
			if(f->inbr == dir->inode) {
				len = (dir->name_len < EXT2_NAME_LEN)
				          ? dir->name_len
				          : EXT2_NAME_LEN;
				if(str == 0)
					str = malloc(len + 1);
...
readdir(Xfile* f, void* vbuf, int64_t offset, int32_t count)
{
	...
	for(i = 0, rcount = 0; (i < nblock) && (i < EXT2_NDIR_BLOCKS); i++) {
		...
		for(off = 0; off < xf->block_size;) {
			...
			pdir.name = name;
			len = (edir->name_len < EXT2_NAME_LEN) ? edir->name_len
			                                       : EXT2_NAME_LEN;
...
repeat:
	...
	if(goal < es.u.sb->s_first_data_block ||
	   goal >= es.u.sb->s_blocks_count)
	...
	if(ed.u.gd->bg_free_blocks_count > 0) {
		block =
		    (goal - es.u.sb->s_first_data_block) % xf->blocks_per_group;
...
search_back:
	...
	*/
	for(k = 0; k < 7 && block > 0 && !test_bit(block - 1, eb.u.bmp);
	    k++, block--)
		;
...
getblk(Xfile* f, int block)
{
	...
	baddr =
	    block_getblk(f, baddr, block / (addr_per_block * addr_per_block));
	baddr = block_getblk(f, baddr,
...
new_inode(Xfile* f, int mode)
{
	...
	if((j = find_first_zero_bit(eb.u.bmp, xf->inodes_per_group >> 3)) <
	   xf->inodes_per_group) {
		if(set_bit(j, eb.u.bmp)) {
			chat("inode %d of group %d is already allocated...", j,
			     group);
			putext2(ed);
			putext2(eb);
			putext2(es);
	...
	buf = getbuf(
	    xf, ed.u.gd->bg_inode_table +
	            (((j - 1) % xf->inodes_per_group) / xf->inodes_per_block));
	if(!buf)
		goto error;
	inode = ((struct Inode*)buf->iobuf) + ((j - 1) % xf->inodes_per_block);
...
free_inode(Xfs* xf, int inr)
{
	...
	buf = getbuf(xf, ed.u.gd->bg_inode_table + (b / xf->inodes_per_block));
	if(!buf) {
		putext2(ed);
		return;
	}
	inode =
	    ((struct Inode*)buf->iobuf) + ((inr - 1) % xf->inodes_per_block);
	...
	for(;;) {
		...
		if((de->inode == 0 && de->rec_len >= rec_len) ||
		   (de->rec_len >= DIR_REC_LEN(de->name_len) + rec_len)) {
			if(de->inode) {
				de1 = (DirEntry*)((char*)de +
				                  DIR_REC_LEN(de->name_len));
				de1->rec_len =
				    de->rec_len - DIR_REC_LEN(de->name_len);
				de->rec_len = DIR_REC_LEN(de->name_len);
				de = de1;
...
unlink(Xfile* file)
{
	...
	b = ed.u.gd->bg_inode_table +
	    (((file->pinbr - 1) % xf->inodes_per_group) / xf->inodes_per_block);
	putext2(ed);
	buf = getbuf(xf, b);
	if(!buf) {
		putext2(es);
...
void
free_block(Xfs* xf, uint32_t block)
{
	uint32_t bg;
	Ext2 ed, es, eb;
