...
ext2fs(Xfs *xf)
{
	...
	seek(xf->dev, OFFSET_SUPER_BLOCK, 0);
	if(!rdonly && sizeof(SuperBlock) != write(xf->dev, &superblock, sizeof(SuperBlock))) {
		...
		if(block) {
			...
			*/
			...
			if(block < xf->blocks_per_group - 32)
				lmap |= (((uint32_t *)eb.u.bmp)[(block >> 5) + 1]) << (31 - (block & 31));
			else
		...
		for(i = 0; i < xf->ngroups; i++) {
			...
			if(ed.u.gd->bg_free_inodes_count &&
			   ed.u.gd->bg_free_inodes_count >= ave) {
				if(group < 0 || ed.u.gd->bg_free_inodes_count > ed.u.gd->bg_free_inodes_count)
					group = i;
			}
			putext2(ed);
		}
