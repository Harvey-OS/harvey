...
struct	Wren
{
	i32	block;			/* size of a block -- from config */
	Devsize	nblock;			/* number of blocks -- from config */
	i32	mult;			/* multiplier to get physical blocks */
...
wrenread(Device *d, Off b, void *c)
{
	...
	if(b >= dr->max) {
		...
		r = 0x040;
	} else if (pread(d->wren.fd, c, RBUFSIZE, (i64)b*RBUFSIZE) != RBUFSIZE) {
...
wrenwrite(Device *d, Off b, void *c)
{
	...
	if(b >= dr->max) {
		print("wrenwrite: block out of range %Z(%lld)\n",
			d, (Wideoff)b);
		r = 0x040;
	} else if (pwrite(d->wren.fd, c, RBUFSIZE, (i64)b*RBUFSIZE) != RBUFSIZE) {
