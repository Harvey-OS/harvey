 ...
 */
static int
doscsi(Target* tp, int rw, uint8_t* cmd, int cbytes, void* data, int* dbytes)
{
	int lun, db = 0;
	...
	if(scsicmd(sc, reqcmd, sizeof reqcmd, reqdata, sizeof reqdata, Sread) <
	   0)
		return STharderr;
...
static int
scsiexec(Target* tp, int rw, uint8_t* cmd, int cbytes, void* data, int* dbytes)
{
	int s;
	...
	default:
		 ...
		 * are 'ok' but for which the returned status code is not
		 *'STok'.
...
buggery:
	if(quiet == 0) {
		...
		print("%s: reqsense: '%s' code #%2.2ux #%2.2ux\n", tp->id, s,
		      sense[12], sense[13]);
		print("%s: byte 2: #%2.2ux, bytes 15-17: #%2.2ux #%2.2ux "
		      "#%2.2ux\n",
