 ...
 */
enum { SUPER_ADDR = 2, /* block address of superblock */
...
enum { MAXDAT = 8192, /* max allowable data message */
 ...
 * tunable parameters
 */
enum { Maxword = 256, /* max bytes per command-line word */
...
enum { BUFSIZE = RBUFSIZE - sizeof(Tag),
       ...
       BKPERBLK = 10,
       CEPERBK =
	   (BUFSIZE - BKPERBLK * sizeof(Off)) / (sizeof(Centry) * BKPERBLK),
};
 ...
 * flags to getbuf
 */
enum { Brd = (1 << 0),    /* read the block if miss */
