...
wormlabel(Device* d, Side* v)
{
	...
	if(wormread(d, labelblk, labelbuf)) {
		...
		if(label->magic == Labmagic) {
			print("side %d's label magic byte-swapped; filsys "
			      "should be configured with xD",
			      vord);
			swab2(&label->ord);
	...
	if(v->ord != vord)
		panic("wormlabel: side %d switched ordinal to %d underfoot",
		      vord, v->ord);
	if(label->ord != vord) {
		print("labelled worm side %Z has wrong ordinal in label (%d, "
		      "want %d)",
		      d, label->ord, vord);
		qunlock(v);
 ...
 *	locks the structure
 */
static Side* wormunit(Device* d) /* d is l0 or r2 (e.g.) */
{
	int p, drive;
...
findside(Device* juke, int side, Device* top)
{
	...
	for(x = mcat->cat.first; x != nil; x = x->link) {
		...
		i = x->wren.targ;
		if(i < 0 || i >= w->nside)
			panic("wormsizeside: side %d in %Z out of range", i,
			      mcat);
		if(i == side)
			break;
	}
...
wormread(Device* d, Off b, void* c)
{
	...
	if(b >= max) {
		...
		r = 0x071;
	} else if(pread(dr->wren.fd, c, RBUFSIZE, (int64_t)b * RBUFSIZE) !=
	          RBUFSIZE) {
		fd2path(dr->wren.fd, name, sizeof name);
		print("wormread: error on %Z(%lld) on %s in %s: %r\n", d,
		      (Wideoff)b, name, dr->wren.sddir);
		cons.nwormre++;
		r = 1;
	}
	qunlock(v);
...
wormwrite(Device* d, Off b, void* c)
{
	...
	if(b >= max) {
		print("wormwrite: block out of range %Z(%lld)\n", d,
		      (Wideoff)b);
		r = 0x071;
	} else if(pwrite(dr->wren.fd, c, RBUFSIZE, (int64_t)b * RBUFSIZE) !=
	          RBUFSIZE) {
		fd2path(dr->wren.fd, name, sizeof name);
...
geometry(Juke* w)
{
	...
	s = scsiio(w->juke, SCSIread, cmd, sizeof cmd, buf,
	           sizeof buf); /* mode sense elem addrs */
	if(s)
		panic("geometry #%x", s);
	...
	s = scsiio(w->juke, SCSIread, cmd, sizeof cmd, buf,
	           sizeof buf); /* mode sense geometry */
	if(s)
		panic("geometry #%x", s);
	...
	case 4: /* data transfer */
		...
		if(buf[8 + 8 + 2] & 1) {
			...
			if(t < 0 || t >= w->nse || t >= MAXSIDE ||
			   s >= MAXDRIVE) {
				print("element: juke %Z lies; claims side %d "
				      "is in drive %d\n",
...
wormi(char* arg)
{
	...
	if(j < w->nside) {
		...
		qlock(v);
		if(v->status == Sstart) {
			if(mmove(w, w->mt0, w->dt0 + v->drive, v->elem,
			         v->rot)) {
				qunlock(v);
				return 0;
			}
...
jukeinit(Device* d)
{
	...
	while(xdev = xdev->link) {
		...
		if(w->drive[i]->type != Devnone && xdev != w->drive[i]) {
		...
		for(i = 0; i < w->nside; i++) {
			...
			if(v->status == Sstart && t > v->time) {
				...
				print("\ttime   r%ld drive %Z\n", v - w->side,
				      w->drive[drive]);
				mmove(w, w->mt0, w->dt0 + drive, v->elem,
				      v->rot);
				v->status = Sunload;
			}
			qunlock(v);
