...
map(Device* d)
{
	...
	if(access(map->to, AEXIST) >= 0) {
		...
		*d = *map->tdev; /* wren -> wren mapping */
		...
		print("bad mapping %Z to %s; no such file or device", d,
		      map->to);
	d->wren.mapped = 1;
}
...
mergeconf(Iobuf* p)
{
	...
	for(cp = p->iobuf; *cp != '\0'; cp++) {
		...
		if(word[0] == '\0')
			...
			cp = getwrd(word, cp);
			for(fs = filsys;
			    fs < filsys + nelem(filsys) - 1 && fs->name; fs++)
...
dowormcopy(void)
{
	...
	if(from->type != Devfworm && from->type != Devcw) {
		print("main file system is not a worm; copyworm may not do "
		      "what you want!\n");
	...
	if(f2 == nil) {
		...
		to = f2->dev;
		print("\ncopying worm from %Z (worm %Z) to %Z, starting in 8 "
		      "seconds\n",
		      fdev, from, to);
		delay(8000);
	}
...
arginit(void)
{
	...
	for(;;) {
		 ...
		 */
		if(strcmp(word, "ipauth") != 0 && astrcmp(word, "ip") != 0 &&
		   astrcmp(word, "ipgw") != 0 && astrcmp(word, "ipmask") != 0 &&
