...
enum { Slop = 256, /* room at the start of a message buf for proto hdrs */
};
...
Zfmt(Fmt* fmt)
{
	...
	case Devmirr:
		...
		if(d->cat.first == d->cat.last)
			...
			sprint(s, "%c%Z-%Z%c", c, d->cat.first, d->cat.last,
			       c1);
		break;
	case Devro:
	...
	case Devfworm:
		...
		break;
	case Devpart:
		sprint(s, "p(%Z)%ld.%ld", d->part.d, d->part.base,
		       d->part.size);
		break;
	case Devswab:
...
rootream(Device* dev, Off addr)
{
	...
	d->uid = -1;
	d->gid = -1;
	d->mode = DALLOC | DDIR | ((DREAD | DEXEC) << 6) |
	          ((DREAD | DEXEC) << 3) | ((DREAD | DEXEC) << 0);
...
devread(Device* d, Off b, void* c)
{
	...
	for(;;)
		...
		case Devnone:
			...
			return 1;
		default:
			panic("illegal device in devread: %Z %lld", d,
			      (Wideoff)b);
			return 1;
		}
}
...
devwrite(Device* d, Off b, void* c)
{
	...
	for(;;)
		...
		case Devnone:
			...
			return 0;
		default:
			panic("illegal device in devwrite: %Z %lld", d,
			      (Wideoff)b);
			return 1;
		}
}
