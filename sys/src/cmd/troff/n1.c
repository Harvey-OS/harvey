...
dwbinit dwbpaths[] = {&DWBfontdir, NULL, 0, &DWBntermdir, NULL, 0,
                      &DWBalthyphens, NULL, 0, &DWBhomedir, NULL, 0, NULL,
                      nextf, NS, NULL, NULL, 0};
...
main(int argc, char* argv[])
{
	...
	while(--argc > 0 && (++argv)[0][0] == '-')
		...
		case 'm':
			if(mflg++ >= NMF) {
				ERROR "Too many macro packages: %s",
				    argv[0] WARN;
				break;
			}
...
init2(void)
{
	...
	for(i = 1; i < NEV; i++) /* propagate the environment */
		envcopy(&env[i], &env[0]);
	for(i = 0; i < NEV; i++) {
		if((env[i]._word._bufp =
		        (Tchar*)calloc(WDSIZE, sizeof(Tchar))) == NULL) {
			ERROR "not enough room for word buffers" WARN;
			done2(1);
		}
		env[i]._word._size = WDSIZE;
		if((env[i]._line._bufp =
		        (Tchar*)calloc(LNSIZE, sizeof(Tchar))) == NULL) {
...
control(int a, int b)
{
	...
	if(contabp[j].f == 0) {
		...
		if(dip != d)
			for(k = dilev; k; k--)
				if(d[k].curd == a) {
					ERROR "diversion %s invokes itself "
					      "during diversion",
					    unpair(a) WARN;
					edone(0100);
				}
		...
		if(b)
			collect();
		flushi();
		return pushi(contabp[j].mx,
		             a); /* BUG??? all that matters is 0/!0 */
	}
	if(b) {
		if(trace & TRREQ)
...
char gchtab[NCHARS] = {
    ...
    000, 000, 000, 000, 000, 000, 000, 000, 000, 001, 000, 001, 000, 000, 000,
    000, /* FLSS, ESC */
    000, 000, 000, 000, 000, 000, 000, 000, 000, 000, 000, 000, 000, 000, 000,
    000, 000, 000, 000, 000, 000, 000, 000, 000, 000, 000, 000, 000, 000, 000,
    000, 000, 000, 000, 000, 000, 000, 000, 000, 000, 000, 000, 000, 000, 000,
    000, 000, 000, 000, 000, 000, 000, 000, 000, 000, 000, 000, 000, 000, 000,
    000, 000, 000, 000, 000, 000, 000, 000, 000, 000, 001, 000, /* f */
    000, 000, 000, 000, 000, 000, 000, 000, 000, 000, 000, 000, 000, 000, 000,
...
g0:
	...
	if(ismot(i))
		return (i);
	k = cbits(i);
	if(k >= sizeof(gchtab) / sizeof(gchtab[0]) ||
	   gchtab[k] == 0) /* nothing special */
		return (i);
	if(k != ESC) {
		if(k == '\n') {
			nlflg++;
...
#ifdef UNICODE
Tchar
get1ch(FILE* fp) /* get one "character" from input, figure out what alphabet */
{
	wchar_t wc;
	char buf[100], *p;
	int i, n, c;
	...
	if(c == EOF)
		return EOF;
	*p = 0;
	return chadd(buf, MBchar,
	             Install); /* add name even if haven't seen it */
}
#endif /*UNICODE*/
...
getach(void)
{
	...
	lgf++;
	j = cbits(i = getch());
	if(ismot(i) || j > SHORTMASK || (j <= 040 && j != 002 /*STX*/
	                                 && j != 003          /*ETX*/
	                                 && j != 005          /*ENQ*/
	                                 && j != 006          /*ACK*/
	                                 && j != 007)) {      /*BELL*/
...
caseso(void)
{
	...
	nextf[0] = 0;
	fp = NULL;
	if(skip() || !getname() || (fp = fopen(nextf, "r")) == NULL ||
	   ifi >= NSO) {
