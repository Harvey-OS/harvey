...
again:
	...
	while(s > s0) {
		...
		mark:
			hyend = w - 1;
			if(*s0 & 0100) /* 0100 used in suftab to encode
			                  something too */
				continue;
			if(!chkvow(w))
				return (0);
...
static int texit(Tchar* start,
                 Tchar* end) /* hyphenate as in tex, return # found */
{
