...
t_width(Tchar j)
{
	...
	if(sfbits(j) == oldbits) {
		...
		xbits(j, 0);
	if(i < nchnames + ALPHABET &&
	   widcache[i].fontpts == (xfont << 8) + xpts && !setwdf)
		k = widcache[i].width;
	else {
		k = getcw(i);
		if(bd)
...
t_setch(int c)
{
	...
	if(c == '(') { /* \(xx */
		...
		c = getach();
		while((*s = getach()) != c && *s != 0 &&
		      s < temp + sizeof(temp) - 1)
			s++;
	}
	*s = '\0';
#ifdef UNICODE
	return chadd(temp, Troffchar, Install) |
	       chbits; /* add name even if haven't seen it */
#else
	if(NROFF) {
		int j;
...
caseps(void)
{
	...
	if(TROFF) {
		if(skip())
			...
			noscale++;
			i = inumb(&apts); /* this is a disaster for fractional
			                     point sizes */
			noscale = 0;
			if(nonumb)
				i = apts1;
...
t_setps(void)
{
	...
	if(isdigit(i)) { /* \sd or \sdd */
		...
		if(i == 0) /* \s0 */
			j = apts1;
		else if(i <= 3 && (ch = getch()) &&
		        isdigit(j = cbits(ch))) { /* \sdd */
			j = 10 * i + j - '0';
			ch = 0;
		} else /* \sd */
			j = i;
...
t_setfont(int a)
{
	...
	if(/* i == 'S' || */ i ==
	   '0') /* an experiment -- why can't we change to it? */
		return;
	if((j = findft(i)) == -1)
		if((j = setfp(0, i, (char*)0, 1)) ==
		   -1) /* try to put it in position 0 */
			return;
s0:
	font1 = font;
	font = j;
...
setfp(int pos, int f, char* truename,
      int print) /* mount font f at position pos[0...nfonts] */
{
	...
	if(truename &&
	   strrchr(truename, '/')) { /* .fp 1 R dir/file: use verbatim */
