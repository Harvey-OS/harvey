...
mnspace(void)
{
	...
	contabp =
	    (Contab*)memcpy((char*)contabp, (char*)contab, sizeof(contab));
	if(contabp == NULL) {
...
g0:
	while(!skip() && (j = getrq()) != 0) {
		if(dip != d)
			for(k = dilev; k; k--)
				if(d[k].curd == j) {
					ERROR "cannot remove diversion %s "
					      "during definition",
					    unpair(j) WARN;
					goto g0;
				}
...
growcontab(void)
{
	...
	if(contabp == NULL) {
		...
		memset((char*)(contabp) + (nm - MDELTA) * sizeof(Contab), 0,
		       MDELTA * sizeof(Contab));
		mrehash();
	}
}
	...
	if(app && oldmn >= 0 && contabp[oldmn].mx) {
		...
		if((nextb = alloc()) == -1) {
			...
			if(macerr++ > 1)
				done2(02);
			if(nextb == 0)
				ERROR "Not enough space for string/macro "
				      "names" WARN;
			edone(04);
			return (offset = 0);
		}
...
Offset alloc(void) /* return free Offset in nextb */
{
	...
	if(i == nblist) {
		blist =
		    (Blockp*)realloc((char*)blist, 2 * nblist * sizeof(Blockp));
		if(blist == NULL) {
	...
	while(argpp != argppend && !skip()) {
		*argpp++ = strp;
		...
		while(1) {
			...
			*strp++ = i;
			if(strflg && strp >= lim) {
				/* ERROR "strp=0x%x, lim = 0x%x", strp, lim
				 * WARN; */
...
casedi(void)
{
	...
	if(skip() || (i = getrq()) == 0) {
		...
		if(dilev > 0) {
			...
			FINDDIV(j);
			if((contabp[j].divsiz =
			        (Divsiz*)malloc(sizeof(Divsiz))) == NULL) {
