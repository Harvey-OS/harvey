...
getfont(char* name, int pos) /* create width tab for font */
{
	...
	ftemp->defaultwidth = ftemp->spacewidth =
	    Inch * Unitwidth / 72 / 3; /* should be rounded */
	nw = code = 0;
	while(fscanf(fin, "%s", cmd) != EOF) {
		if(strcmp(cmd, "name") == 0)
			...
			while(fgets(buf, sizeof buf, fin) != NULL) {
				...
				chtemp[n].kern = kern;
				chtemp[n].code = code;
				/*fprintf(stderr, "font %2.2s char %4.4s num %3d
				   wid %2d code %3d\n",
	 ...
	 *{
	 *			printf("%c %d ", ftemp->wp[i].num,
	 *ftemp->wp[i].wid);
	 *		else if (i >= ALPHABET)
	 *			printf("%d (%s) %d ", ftemp->wp[i].num,
	 *				chnames[ftemp->wp[i].num-ALPHABET],
	 *ftemp->wp[i].wid);
	 *	}
	 *	printf("\n");
	 */
...
chadd(char* s, int type,
      int install) /* add s to global character name table; */
{                  /* or just look it up */
	...
	for(i = 0; i < nchnames; i++)
		if(type == chnames[i][0] &&
		   eq(s, chnames[i] + 1)) /* +1 since type at front */
			break;
	...
	chnames[nchnames] = p =
	    (char*)malloc(strlen(s) + 1 + 1); /* type + \0 */
	if(p == NULL) {
