...
t_ptout(Tchar i)
{
	...
	if(cbits(i) != '\n') {
		if(olinep >= oline + olnsize) {
			diff = olinep - oline;
			olnsize += OLNSIZE;
			if((oline = (Tchar*)realloc((char*)oline,
			                            olnsize * sizeof(Tchar))) !=
			   NULL) {
				if(diff && olinep)
					olinep = oline + diff;
			} else {
...
ptfont(void)
{
	...
	if(ascii)
		return;
	if(xfont > nfonts) {
		ptfpcmd(0, fonts[xfont].longname,
...
void
ptfpcmd(int f, char* s, char* longname)
{
	...
	/*	OUT "f%d\n", xfont PUT;	/* need this for buggy version of
	 * adobe transcript */
