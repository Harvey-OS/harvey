...
nnspace(void)
{
	...
	numtabp =
	    (Numtab*)memcpy((char*)numtabp, (char*)numtab, sizeof(numtab));
	if(numtabp == NULL) {
...
grownumtab(void)
{
	...
	if(numtabp == NULL) {
		...
		memset((char*)(numtabp) + (ncnt - NDELTA) * sizeof(Numtab), 0,
		       NDELTA * sizeof(Numtab));
		nrehash();
	}
}
