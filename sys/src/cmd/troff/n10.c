...
static char*
parse(char* s, int typeit) /* convert \0, etc to nroff driving table format */
...
static int getnrfont(FILE* fp) /* read the nroff description file */
{
	...
	for(i = 0; i < ALPHABET; i++) {
		...
		chtemp[i].num = chtemp[i].code =
		    i;             /* every alphabetic character is itself */
	...
	while(fgets(buf, sizeof buf, fp) != NULL) {
		...
		if(strlen(ch) == 1) { /* it's ascii */
			...
			if(strcmp(ch, "---") == 0) { /* no name */
				...
				type = Troffchar;
			/* BUG in here somewhere when same character occurs
			 * twice in table */
...
n_ptinit(void)
{
	...
	if(!is(devname) && trace)
		ERROR "wrong terminal name: saw %s, wanted %s", cmd,
		    devname WARN;
	for(;;) {
		fscanf(fp, "%s", cmd);
		if(is("charset"))
		...
		if(is("bset"))
			...
			t.twinit = strdupl(parse(opt, Notype));
		else if(is("twrest"))
			t.twrest = strdupl(parse(opt, Notype));
		else if(is("twnl"))
			t.twnl = strdupl(parse(opt, Notype));
		else if(is("hlr"))
			t.hlr = strdupl(parse(opt, Notype));
		else if(is("hlf"))
			t.hlf = strdupl(parse(opt, Notype));
		else if(is("flr"))
			t.flr = strdupl(parse(opt, Notype));
		else if(is("bdon"))
			t.bdon = strdupl(parse(opt, Notype));
		else if(is("bdoff"))
			t.bdoff = strdupl(parse(opt, Notype));
		else if(is("iton"))
			t.iton = strdupl(parse(opt, Notype));
		else if(is("itoff"))
			t.itoff = strdupl(parse(opt, Notype));
		else if(is("ploton"))
			t.ploton = strdupl(parse(opt, Notype));
		else if(is("plotoff"))
			t.plotoff = strdupl(parse(opt, Notype));
		else if(is("up"))
			t.up = strdupl(parse(opt, Notype));
		else if(is("down"))
			t.down = strdupl(parse(opt, Notype));
		else if(is("right"))
			t.right = strdupl(parse(opt, Notype));
		else if(is("left"))
			t.left = strdupl(parse(opt, Notype));
		else
...
ptout1(void)
{
	...
	for(q = oline; q < olinep; q++) {
		...
		if(!(*t.bdon & 0377) &&
		   ((j = bdtab[xfont]) || xfont == BDFONT || xfont == BIFONT))
			j++;
		else
		...
		if(k < ALPHABET) { /* ordinary ascii */
			...
			while(--j > 0) {
				...
				oput(k);
		          ...
		          t.tfont.nchars) { /* BUG -- not really understood */
			/* fprintf(stderr, "big char %d, name %s\n", k,
			 * chname(k)); /* */
			oputs(chname(k) +
			      1); /* BUG: should separate Troffchar and
			             MBchar... */
			 ...
			 * chname(k)); /* */
			codep =
			    t.tfont.wp[k].str + 1; /* Troffchar by default */
			while(*codep != 0) {
				if(*codep & 0200) {
					codep = plot(codep);
					oput(' ');
