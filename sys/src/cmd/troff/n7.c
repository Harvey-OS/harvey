...
storeline(Tchar c, int w)
{
	...
	if(linep >= line + lnsize - 2) {
		lnsize += LNSIZE;
		diff = linep - line;
		if((line = (Tchar*)realloc((char*)line,
		                           lnsize * sizeof(Tchar))) != NULL) {
			if(linep && diff)
				linep = line + diff;
		} else {
			if(over) {
...
newline(int a)
{
	...
	if(dip != d) {
		...
		if(dip->ditrap && !dip->ditf && dip->dnl >= dip->ditrap &&
		   dip->dimac)
...
nlpn:
	...
	if(print)
		ptpage(numtabp[PN]
		           .val); /* supposedly in a clean state so can pause */
	if(stop && print) {
		dpn++;
		if(dpn >= stop) {
			dpn = 0;
		...
		if(hyoff != 1 && *hyp == wp) {
			...
			if(!wdstart ||
			   (wp > wdstart + 1 && wp < wdend &&
			    (!(hyf & 04) ||
			     wp < wdend - 1) && /* 04 => last 2 */
			    (!(hyf & 010) ||
			     wp > wdstart + 2))) { /* 010 => 1st 2 */
				nhyp++;
...
storeword(Tchar c, int w)
{
	...
	if(wordp >= word + wdsize - 2) {
		wdsize += WDSIZE;
		savp = word;
		if((word = (Tchar*)realloc((char*)word,
		                           wdsize * sizeof(Tchar))) != NULL) {
			if(wordp)
