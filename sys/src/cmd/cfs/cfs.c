...
char* mname[] = {
        [Tversion] "Tversion", [Tauth] "Tauth", [Tflush] "Tflush",
        [Tattach] "Tattach", [Twalk] "Twalk", [Topen] "Topen",
        [Tcreate] "Tcreate", [Tclunk] "Tclunk", [Tread] "Tread",
        [Twrite] "Twrite", [Tremove] "Tremove", [Tstat] "Tstat",
        [Twstat] "Twstat", [Rversion] "Rversion", [Rauth] "Rauth",
        [Rerror] "Rerror", [Rflush] "Rflush", [Rattach] "Rattach",
        [Rwalk] "Rwalk", [Ropen] "Ropen", [Rcreate] "Rcreate",
        [Rclunk] "Rclunk", [Rread] "Rread", [Rwrite] "Rwrite",
        [Rremove] "Rremove", [Rstat] "Rstat", [Rwstat] "Rwstat", 0,
};
...
usage(void)
{
	...
	fprint(2,
	       "\tcfs [-a netaddr | -F srv] [-dknrS] [-f partition] [mntpt]\n");
	exits("usage");
}
...
void rflush(void) /* synchronous so easy */ { sendreply(0); }
...
rwalk(Mfile* mf)
{
	...
	nmf = nil;
	if(statson && mf->qid.type == rootqid.type &&
	   mf->qid.path == rootqid.path && c.thdr.nwname == 1 &&
	   strcmp(c.thdr.wname[0], "cfsctl") == 0) {
...
rread(Mfile* mf)
{
	...
	while(cnt > 0 && !done) {
		...
		if(n <= 0) {
			...
			if(n == 0 || n > cnt)
				n = cnt;
			DPRINT(2, "fetch %ld bytes of data from server at "
			          "offset %lld\n",
			       n, off);
...
rwrite(Mfile* mf)
{
	...
	/* don't modify our cache for append-only data; always read from
	 * server*/
...
rcvmsg(P9fs* p, Fcall* f)
{
	...
	if(p->len <= 0) {
		snprint(buf, sizeof buf, "read9pmsg(%d)->%ld: %r", p->fd[0],
		        p->len);
		error(buf);
	}

	if((rlen = convM2S(datarcv, p->len, f)) != p->len)
		error("rcvmsg format error, expected length %d, got %d", rlen,
		      p->len);
	if(f->fid >= Nfid) {
		fprint(2, "<-%s: %d %s on %d\n", p->name, f->type,
...
genstats(void)
{
	...
	for(i = 0; i < nelem(cfsstat.cm); i++)
		if(cfsstat.cm[i].n || cfsstat.sm[i].n) {
			...
			if(cfsstat.cm[i].n - cfsprev.cm[i].n)
				p += snprint(
				    p, sizeof statbuf + statbuf - p, "%7.3f ",
				    0.000001 *
	...
	p += snprint(p, sizeof statbuf + statbuf - p,
	             "%7lud %7lud ndelegateread\n", cfsstat.ndelegateread,
	             cfsstat.ndelegateread - cfsprev.ndelegateread);
	...
	p += snprint(p, sizeof statbuf + statbuf - p,
	             "%7llud %7llud bytesread\n", cfsstat.bytesread,
	             cfsstat.bytesread - cfsprev.bytesread);
	p += snprint(p, sizeof statbuf + statbuf - p,
	             "%7llud %7llud byteswritten\n", cfsstat.byteswritten,
	             cfsstat.byteswritten - cfsprev.byteswritten);
	p += snprint(p, sizeof statbuf + statbuf - p,
	             "%7llud %7llud bytesfromserver\n", cfsstat.bytesfromserver,
	             cfsstat.bytesfromserver - cfsprev.bytesfromserver);
	p += snprint(p, sizeof statbuf + statbuf - p,
	             "%7llud %7llud bytesfromdirs\n", cfsstat.bytesfromdirs,
	             cfsstat.bytesfromdirs - cfsprev.bytesfromdirs);
	p += snprint(p, sizeof statbuf + statbuf - p,
	             "%7llud %7llud bytesfromcache\n", cfsstat.bytesfromcache,
	             cfsstat.bytesfromcache - cfsprev.bytesfromcache);
	p += snprint(p, sizeof statbuf + statbuf - p,
	             "%7llud %7llud bytestocache\n", cfsstat.bytestocache,
	             cfsstat.bytestocache - cfsprev.bytestocache);
	statlen = p - statbuf;
	cfsprev = cfsstat;
}
