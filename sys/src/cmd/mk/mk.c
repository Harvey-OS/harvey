...
work(Node* node, Node* p, Arc* parc)
{
	...
	if((node->flags & MADE) && (node->flags & PRETENDING) && p &&
	   outofdate(p, parc, 0)) {
		if(explain)
			fprint(1, "unpretending %s(%lud) because %s is out of "
			          "date(%lud)\n",
	...
	if(node->prereqs == 0) {
		if(node->time == 0) {
			if(getwd(cwd, sizeof cwd))
				fprint(2, "mk: don't know how to make '%s' in "
				          "directory %s\n",
				       node->name, cwd);
			else
	...
	*/
	if((iflag == 0) && (node->time == 0) &&
	   (node->flags & (PRETENDING | CANPRETEND)) && p && ra->n &&
	   !outofdate(p, ra, 0)) {
	...
	*/
	for(a = node->prereqs; a; a = a->next)
		if(a->n && (a->n->flags & PRETENDING)) {
			if(explain)
				Bprint(&bout, "unpretending %s because of %s "
				              "because of %s\n",
				       a->n->name, node->name,
				       ra->n ? ra->n->name
...
update(int fake, Node* node)
{
	 ...
	 * node->time, node->flags);/**/
}

static pcmp(char* prog, char* p, char* q)
{
...
outofdate(Node* node, Arc* arc, int eval)
{
	...
	if(arc->prog) {
		...
		if(sym == 0 || eval) {
			...
			ret = sym->u.value;
		return (ret - 1);
	} else if(strchr(arc->n->name, '(') &&
	          arc->n->time == 0) /* missing archive member */
		return 1;
	else
		/*
