...
gunzipf(char* file, int stdout)
{
	...
	Binit(&bin, ifd, OREAD);
	if(Bgetc(&bin) != GZMAGIC1 || Bgetc(&bin) != GZMAGIC2 ||
	   Bgetc(&bin) != GZDEFLATE) {
...
header(Biobuf* bin, GZHead* h)
{
	...
	if(flag & ~(GZFTEXT | GZFEXTRA | GZFNAME | GZFCOMMENT | GZFHCRC))
		fprint(2, "gunzip: reserved flags set, data may not be "
...
error(char* fmt, ...)
{
	...
	if(gzok)
		fprint(2,
		       "gunzip: %s: corrupted data after byte %lld ignored\n",
		       infile, gzok);
	else {
		fprint(2, "gunzip: ");
		if(infile)
