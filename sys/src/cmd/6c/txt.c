...
naddr(Node* n, Adr* a)
{
	...
	case OCONST:
		...
		if(typev[n->type->etype] || n->type->etype == TIND)
			...
			a->offset = convvtox(
			    n->vconst, typeu[n->type->etype] ? TULONG : TLONG);
		break;
...
gmove(Node* f, Node* t)
{
	...
	if(f->op == ONAME || f->op == OINDREG || f->op == OIND ||
	   f->op == OINDEX)
		...
		fld:
			regalloc(&nod, f, t);
			if(tt != TDOUBLE &&
			   tt != TFLOAT) { /* TO DO: why is this here */
				prtree(f, "odd tree");
				nod.type = t64 ? types[TVLONG] : types[TINT];
			}
			gins(a, f, &nod);
	...
	if(a == AMOVQ || a == AMOVSD || a == AMOVSS ||
	   a == AMOVL && ewidth[ft] == ewidth[tt]) /* TO DO: check AMOVL */
		if(samaddr(f, t))
			return;
	gins(a, f, t);
}
...
gopcode(int o, Type* ty, Node* f, Node* t)
{
	...
	case OASMUL:
	case OMUL:
		if(f->op == OREGISTER && t != Z && isreg(t, D_AX) &&
		   reg[D_DX] == 0)
			t = Z;
		a = AIMULL;
		if(et == TVLONG || et == TUVLONG || et == TIND)
