 ...
 * 	double-height lines, nor the ``fill the screen with E's'' confidence
 *check.
 *
...
emulate(void)
{
	...
	for(;;) {
		...
		case '\026': /* SYN */
		case '\027': /* ETB */
		case '\030': /* CAN: cancel escape sequence, display
		                checkerboard (not implemented) */
		case '\031': /* EM */
		...
		case '\033':
			...
			/*
			 * 1 - graphic processor option on (no-op; not
			 * installed)
			 */
			case '1':
				break;
			...
			/*
			 * 2 - graphic processor option off (no-op; not
			 * installed)
			 */
			case '2':
				break;
			 ...
			 * margin.
			 * (Original VT100 had a bug: tracked new-line/line-feed
			 * mode.)
			 */
			case 'D':
				if(++y > yscrmax) {
					y = yscrmax;
					scroll(yscrmin + 1, yscrmax + 1,
					       yscrmin, yscrmax);
				}
				break;

			/*
			 * E - active position to start of next line, scroll if
			 * at bottom margin.
			 */
			case 'E':
				x = 0;
			...
			/*
			 * M - active position up a line, scroll if at top
			 * margin..
			 */
			case 'M':
				if(--y < yscrmin) {
					y = yscrmin;
					scroll(yscrmin, yscrmax, yscrmin + 1,
					       yscrmin);
				}
				break;
			...
			Ident:
				sendnchars2(
				    7,
				    "\033[?1;2c"); /* VT100 with AVO option */
				                   //				sendnchars2(5,
				                   //"\033[?6c");	/* VT102
				//(insert/delete-char, etc.) */
				break;
			...
			case '[':
				...
				/*
				* s - some dec private stuff. actually [ ? num
				* s, but we can't detect it.
				*/
				case 's':
					break;
				...
				/*
				 * H and f - cursor motion.  operand[0] is row
				 * and
				...
				case 'J':
					...
					case 2:
						...
						break;
					/*
					 * operand 1: start of screen to active
					 * position, inclusive.
					 */
					case 1:
						clear(Rpt(pt(0, 0),
						...
						break;
					/*
					 * Default:  active position to end of
					 * screen, inclusive.
					 */
					default:
						clear(Rpt(pt(x, y),
				...
				case 'K':
					...
					case 2:
						...
						break;
					/*
					 * operand 1: start of line to active
					 * position, inclusive.
					 */
					case 1:
						clear(Rpt(pt(0, y),
				...
				/*
				 *	P - delete character(s) from right of
				 *cursor (xterm)
				 */
				case 'P':
					fixops(operand);
					i = x + operand[0];
					draw(screen,
					     Rpt(pt(x, y), pt(xmax + 1, y + 1)),
					     screen, nil, pt(i, y));
					clear(Rpt(pt(xmax - operand[0], y),
					          pt(xmax + 1, y + 1)));
					break;

				/*
				 *	@ - insert blank(s) to right of cursor
				 *(xterm)
				 */
				case '@':
					fixops(operand);
					i = x + operand[0];
					draw(screen,
					     Rpt(pt(i, y), pt(xmax + 1, y + 1)),
					     screen, nil, pt(x, y));
					clear(Rpt(pt(x, y), pt(i, y + 1)));
					break;

				/*
				 *	X - erase character(s) at cursor and to
				 *the right (xterm)
				 */
				case 'X':
					fixops(operand);
				...
				/*
				 * L - insert a line at cursor position (VT102
				 * and later)
				 */
				case 'L':
					fixops(operand);
				...
				/*
				 * M - delete a line at cursor position (VT102
				 * and later)
				 */
				case 'M':
					fixops(operand);
					for(i = 0; i < operand[0]; ++i)
						scroll(y + 1, yscrmax + 1, y,
						       yscrmax);
					break;
		...
		Default:
			...
			n = 1;
			c = 0;
			while(!cs->raw && host_avail() && x + n <= xmax &&
			      n < BUFS && (c = get_next_char()) >= ' ' &&
			      c < '\177') {
				buf[n++] = c;
				c = 0;
			}
			buf[n] = 0;
			//			clear(Rpt(pt(x,y), pt(x+n,
			//y+1)));
