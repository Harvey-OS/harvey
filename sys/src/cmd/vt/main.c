...
char* menutext2[] = {"backup", "forward", "reset", "clear", "send", "page", 0};

char* menutext3[] = {"24x80", "crnl", "nl", "raw", "exit", 0};
...
initialize(int argc, char** argv)
{
	...
	bordercol =
	    allocimage(display, Rect(0, 0, 1, 1), screen->chan, 1, 0xCCCCCCCC);
	cursback = allocimage(display, Rect(0, 0, CW + 1, NS + 1), screen->chan,
	                      0, DNofill);

	for(i = 0; i < 8; i++) {
		colors[i] = allocimage(display, Rect(0, 0, 1, 1), screen->chan,
		                       1, rgbacolors[i]);
		hicolors[i] = allocimage(display, Rect(0, 0, 1, 1),
		                         screen->chan, 1, rgbahicolors[i]);
	}
...
cursoff(void)
{
	draw(screen, Rpt(pt(x, y), addpt(pt(x, y), Pt(CW, NS))), cursback, nil,
	     cursback->r.min);
}
		...
		do {
			...
			case Emouse:
				...
				if(button2() || button3())
					...
					/* eresized() is triggered by special
					 * mouse event */
					newmouse = 1;
				}
				break;
...
setdim(int ht, int wid)
{
	...
	r.min = screen->r.min;
	r.max =
	    addpt(screen->r.min, Pt((xmax + 1) * CW + 2 * XMARGIN + 2 * INSET,
...
void scroll(int sy, int ly, int dy,
            int cy) /* source, limit, dest, which line to clear */
{
	draw(screen, Rpt(pt(0, dy), pt(xmax + 1, dy + ly - sy)), screen, nil,
	     pt(0, sy));
	clear(Rpt(pt(0, cy), pt(xmax + 1, cy + 1)));
	flushimage(display, 1);
}

void bigscroll(void) /* scroll up half a page */
{
	int half = ymax / 3;
	...
	draw(screen, Rpt(pt(0, 0), pt(xmax + 1, ymax + 1)), screen, nil,
	     pt(0, half));
