...
servername(char* addr)
{
	...
	if(strncmp(addr, "tcp!", 4) == 0 || strncmp(addr, "net!", 4) == 0)
		addr += 4;
	addr = estrdup(addr);
	p = addr + strlen(addr);
	if(p > addr && *(p - 1) == 's')
...
wwwauthenticate(HttpState* hs, char* line)
{
	...
	if(user == nil || pass == nil) {
		...
		fmtprint(&fmt, "proto=pass service=http server=%q realm=%q",
		         name, realm);
		free(name);
		if(hs->c->url->user)
...
httpopen(Client* c, Url* url)
{
	...
	if(httpdebug)
		fprint(2, "<- %s %s HTTP/1.0\n<- Host: %s\n",
		       c->havepostbody ? "POST" : "GET", url->http.page_spec,
		       url->host);
	...
	if(authenticate) {
		if(!hs->credentials) {
			if(hs->autherror[0])
				...
				werrstr("unauthorized; no www-authenticate: "
				        "header");
			goto Error;
		}
