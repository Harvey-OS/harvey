...
static char pc_vers[] =
    "@(#)pcnfsd_v2.c	1.6 - rpc.pcnfsd V2.0 (c) 1994 P9, GmbH";
static char pc_home[] = "merrimack:/";

static Procmap pcproc[] = {/* pcnfsd v2 */
                           0, pcnull, 1, pcinfo, 13, pcauth, 0, 0};

static Procmap pc1proc[] = {/* pc-nfsd v1 */
                            0, pcnull, 1, pc1auth, 0, 0};

int myport = 1111;

Progmap progmap[] = {
    150001, 2, pcinit, pcproc, 150001, 1, 0, pc1proc, 0, 0, 0,
};
...
pcinfo(int n, Rpccall* cmd, Rpccall* reply)
{
	...
	chat("host=%I, port=%ld: pcinfo...", cmd->host, cmd->port);
	if(n <= 16)
	...
	chat("\"%.*s\",\"%.*s\"\n", utfnlen(vers.s, vers.n), vers.s,
	     utfnlen(cm.s, cm.n), cm.s);
...
pc1auth(int n, Rpccall* cmd, Rpccall* reply)
{
	...
	chat("host=%I, port=%ld: pcauth...", cmd->host, cmd->port);
	if(n <= 8)
		return garbage(reply, "count too small");
	...
	if(uid < 0)
		uid = 1;
	chat("\"%.*s\",\"%.*s\" uid=%d\n", utfnlen(id.s, id.n), id.s,
	     utfnlen(pw.s, pw.n), pw.s, uid);
	PLONG(0);   /* status */
	PLONG(uid); /* uid */
	PLONG(uid); /* gid */
	return dataptr - (uint8_t*)reply->results;
...
pcauth(int n, Rpccall* cmd, Rpccall* reply)
{
	...
	if(uid < 0)
		uid = 1;
	chat("\"%.*s\",\"%.*s\",\"%.*s\",\"%.*s\"\n", utfnlen(sys.s, sys.n),
	     sys.s, utfnlen(id.s, id.n), id.s, utfnlen(pw.s, pw.n), pw.s,
	     utfnlen(cm.s, cm.n), cm.s);
	PLONG(0); /* status - OK */
	PLONG(uid);
	PLONG(uid); /* gid */
