...
rpc2xfid(Rpccall* cmd, Dir* dp)
{
	...
	if(auth2unix(&cmd->cred, &au) != 0) {
		 ...
		 *			au.stamp, utfnlen(au.mach.s, au.mach.n),
		 *au.mach.s, au.uid, au.gid);
		...
		chat("%ld@%.*s...", au.uid,
		     utfnlen(au.mach.s, (p ? p - au.mach.s : au.mach.n)),
		     au.mach.s);
	}
	...
	if(m == 0) {
		...
		/*chat("getdom %d.%d.%d.%d", cmd->host&0xFF,
		   (cmd->host>>8)&0xFF,
...
setuser(Xfile* xp, char* user)
{
	...
	if(xf->urfid)
		return xf;
	if(xp->parent == xp ||
	   !(xpf = setuser(xp->parent, user))) /* assign = */
...
xfstat(Xfid* xf, Dir* dp)
{
	...
	if(xmesg(s, Tstat) == 0) {
		...
		/* not reached ? */
		chat("xp->qid.path=0x%.16llux, dp->qid.path=0x%.16llux "
		     "name=%s...",
...
xfwalkcr(int type, Xfid* xf, String* elem, int32_t perm)
{
	...
	chat("fid=%d,qid=0x%llux,%ld,%.2ux...", s->f.fid, s->f.qid.path,
	     s->f.qid.vers, s->f.qid.type);
