...
Procmap nfsproc[] = {0, nfsnull,       /* void */
                     ...
                     8,
                     nfswrite, /* Fhandle, int32_t, int32_t, int32_t, String */
...
Progmap progmap[] = {
    100005, 1, mntinit, mntproc, 100003, 2, nfsinit, nfsproc, 0, 0, 0,
};
...
nfssetattr(int n, Rpccall* cmd, Rpccall* reply)
{
	...
	chat("mode=0%lo,u=%ld,g=%ld,size=%ld,atime=%ld,mtime=%ld...",
	     sattr.mode, sattr.uid, sattr.gid, sattr.size, sattr.atime,
	     sattr.mtime);
	if(xf == 0)
		return error(reply, NFSERR_STALE);
	...
	chat("sattr.mode=%luo dir.mode=%luo nd.mode=%luo...", sattr.mode,
	     dir.mode, nd.mode);
	if(r) {
		r = xfwstat(xf, &nd);
		if(r < 0)
...
creat(int n, Rpccall* cmd, Rpccall* reply, int chdir)
{
	...
	if(newxf == 0) {
		newxf =
		    xfwalkcr(Tcreate, xf, &elem, chdir | (sattr.mode & 0777));
		if(newxf)
			trunced = 1;
		else
			newxf = xfwalkcr(Twalk, xf, &elem, 0);
...
nfsreaddir(int n, Rpccall* cmd, Rpccall* reply)
{
	...
	if(xfopen(xf, Oread) < 0)
		return error(reply, NFSERR_PERM);
	while(xf->offset <
	      offset) { /* if we reopened, xf->offset will be zero */
	...
	entries = 0;
	while(count >
	      16) { /* at least 16 bytes required; we don't know size of name */
		chat("top of loop\n");
		setfid(s, xf->opfid);
		s->f.offset = xf->offset;
		...
		rdata = s->f.data;
		/* now have a buffer of Plan 9 directories; unpack into NFS
		 * thingies */
		while(sfcount >= 0) {
