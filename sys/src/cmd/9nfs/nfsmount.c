...
Procmap mntproc[] = {0, mntnull,    1, mntmnt,    2, mntdump, 3, mntumnt,
                     4, mntumntall, 5, mntexport, 0, 0};
...
mntmnt(int n, Rpccall* cmd, Rpccall* reply)
{
	...
	clog("host=%I, port=%ld, root=\"%.*s\"...", cmd->host, cmd->port,
	     utfnlen(root.s, root.n), root.s);
	...
	clog("auth: %ld %.*s u=%ld g=%ld", au.stamp,
	     utfnlen(au.mach.s, au.mach.n), au.mach.s, au.uid, au.gid);
...
mntexport(int n, Rpccall* cmd, Rpccall* reply)
{
	...
	if(auth2unix(&cmd->cred, &au) != 0) {
		...
		chat("%ld@%.*s...", au.uid, utfnlen(au.mach.s, au.mach.n),
		     au.mach.s);
	PLONG(TRUE);
	PLONG(1);
	PPTR("/", 1);
