...
enum { NARG = 15,              /* max number of arguments */
...
famount(int fd, AuthRpc* rpc, char* mntpt, int flags, char* aname)
{
	...
	afd = fauth(fd, aname);
	if(afd >= 0) {
		ai = fauth_proxy(afd, rpc, amount_getkey,
...
nsop(char* fn, int argc, char* argv[], AuthRpc* rpc)
{
	...
	if(strcmp(argv0, ".") == 0 && argc == 1) {
		...
		if(argc == 2) {
			if(famount(fd, rpc, argv[1], flags, "") < 0 &&
			   newnsdebug)
				fprint(2, "%s: mount: %s %s: %r\n", fn, argv[0],
				       argv[1]);
		} else if(argc == 3) {
			if(famount(fd, rpc, argv[1], flags, argv[2]) < 0 &&
			   newnsdebug)
...
static int catch(void* x, char* m)
{
	USED(x);
...
callexport(char* sys, char* tree)
{
	...
	if((fd = dial(na, 0, 0, 0)) < 0)
		return -1;
	if((ai = auth_proxy(fd, auth_getkey, "proto=p9any role=client")) ==
	       nil ||
	   write(fd, tree, strlen(tree)) < 0 || read(fd, buf, 3) != 2 ||
	   buf[0] != 'O' || buf[1] != 'K') {
		close(fd);
		auth_freeAI(ai);
		return -1;
...
expandarg(char* arg, char* buf)
{
	...
	while(p = nextdollar(arg)) {
		...
		if(fd >= 0) {
			...
			/* some singleton environment variables have trailing
			 * NULs */
