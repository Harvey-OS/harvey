	     ...
	     uint nresp, AuthGetkey *getkey, char *fmt, ...)
{
	...
	if(p == nil || dorpc(rpc, "start", p, strlen(p), getkey) != ARok || dorpc(rpc, "write", chal, nchal, getkey) != ARok || dorpc(rpc, "read", nil, 0, getkey) != ARok) {
		free(p);
		close(afd);
		auth_freerpc(rpc);
		return -1;
	...
	if((a = auth_attr(rpc)) != nil && (s = _strfindattr(a, "user")) != nil && strlen(s) < nuser)
		strcpy(user, s);
	else if(nuser > 0)
		user[0] = '\0';
