...
sopen(Srv *srv, Req *r)
{
	...
	if(r->fid->file) {
		...
		/* BUG RACE */
		if((r->ifcall.mode & ORCLOSE) && !hasperm(r->fid->file->parent, r->fid->uid, AWRITE)) {
			respond(r, Eperm);
			return;
		}
		r->ofcall.qid = r->fid->file->qid;
		if((r->ofcall.qid.type & QTDIR) && (r->fid->rdir = opendirfile(r->fid->file)) == nil) {
...
sread(Srv *srv, Req *r)
{
	...
	if(r->ifcall.offset < 0 || ((r->fid->qid.type & QTDIR) && r->ifcall.offset != 0 && r->ifcall.offset != r->fid->diroffset)) {
