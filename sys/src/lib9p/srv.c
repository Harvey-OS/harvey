...
getreq(Srv* s)
{
	...
	if((r = allocreq(s->rpool, f.tag)) ==
	   nil) { /* duplicate tag: cons up a fake Req */
...
rflush(Req* r, char* error)
{
	...
	if(or ) {
		...
		if(or->responded == 0) {
			or->flush = erealloc9p(
			    or->flush, (or->nflush + 1) * sizeof(or->flush[0]));
...
swalk(Srv* srv, Req* r)
{
	...
	if(r->ifcall.fid != r->ifcall.newfid) {
		if((r->newfid = allocfid(srv->fpool, r->ifcall.newfid)) ==
		   nil) {
			respond(r, Edupfid);
			return;
		}
...
rwalk(Req* r, char* error)
{
	if(error || r->ofcall.nwqid < r->ifcall.nwname) {
		...
		if(r->ofcall.nwqid == 0) {
			...
			/* if file trees are in use, filewalk took care of the
			 * rest */
...
sread(Srv* srv, Req* r)
{
	...
	if(r->ifcall.offset < 0 ||
	   ((r->fid->qid.type & QTDIR) && r->ifcall.offset != 0 &&
	    r->ifcall.offset != r->fid->diroffset)) {
	...
	if((r->fid->qid.type & QTDIR) && r->fid->file) {
		r->ofcall.count =
		    readdirfile(r->fid->rdir, r->rbuf, r->ifcall.count);
		respond(r, nil);
		return;
	}
	if(srv->read)
...
swrite(Srv* srv, Req* r)
{
	...
	o = r->fid->omode & 3;
	if(o != OWRITE && o != ORDWR) {
		snprint(e, sizeof e, "write on fid with open mode 0x%ux",
		        r->fid->omode);
		respond(r, e);
		return;
	}
...
swstat(Srv* srv, Req* r)
{
	...
	if(convM2D(r->ifcall.stat, r->ifcall.nstat, &r->d,
	           (char*)r->ifcall.stat) != r->ifcall.nstat) {
		respond(r, Ebaddir);
		return;
	}
	...
	if((uint8_t)~r->d.qid.type || (uint32_t)~r->d.qid.vers ||
	   (uint64_t)~r->d.qid.path) {
	...
	if((uint32_t)~r->d.mode &&
	   ((r->d.mode & DMDIR) >> 24) != (r->fid->qid.type & QTDIR)) {
...
respond(Req* r, char* error)
{
	...
	if(m != n)
		sysfatal("lib9p srv: write %d returned %d on fd %d: %r", n, m,
		         srv->outfd);
	qunlock(&srv->wlock);
