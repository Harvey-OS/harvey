...
#define DBG if(0)
#define RGB2K(r, g, b)                                                         \
	((299 * ((ulong)(r)) + 587 * ((ulong)(g)) + 114 * ((ulong)(b))) / 1000)
...
dumpimage(char* name, Memimage* img, void* vdata, Point labelpt)
{
	...
	for(y = 0; y < Yrange; y++) {
		...
		v = 0;
		p = data +
		    (byteaddr(img, Pt(0, y)) - (uint8_t*)img->data->bdata);
		Bprint(&b, "%-4d\t", y);
...
checkone(Point p, Point sp, Point mp)
{
	...
	if(memcmp(dp, sdp, (dst->depth + 7) / 8) != 0) {
		fprint(2, "dtest: one bad pixel drawing at dst %P from source "
		          "%P mask %P\n",
		       p, sp, mp);
		fprint(2, " %.2ux %.2ux %.2ux %.2ux should be %.2ux %.2ux "
		          "%.2ux %.2ux\n",
...
void
checkline(Rectangle r, Point sp, Point mp, int y, Memimage* stmp,
          Memimage* mtmp)
{
	uint32_t* dp;
	int nb;
...
verifyonemask(void)
{
	...
	memmove(src->data->bdata, srcbits,
	        src->width * sizeof(uint32_t) * Yrange);
...
verifyrectmaskrepl(int srcrepl, int maskrepl)
{
	...
	for(tp.y = sp.y, up.y = mp.y, y = dr.min.y;
	    y < dr.max.y && tp.y < Yrange && up.y < Yrange; y++, tp.y++, up.y++)
...
getpixel(Memimage* img, Point pt)
{
	...
	if(bpp < 8) {
		 ...
		 *what it says.
		 * In all channels but the color map, we replicate to 8 bits
		 *because that's the
...
putpixel(Memimage* img, Point pt, uint32_t nv)
{
	...
	if(bpp < 8) {
		 ...
		 * pt.x%npack pixels,
		 * which is equivalent to skipping the rightmost npack -
		 * pt.x%npack - 1 pixels.
		 */
		npack = 8 / bpp;
             ...
             Point mp)
{
	...
	if(dst->flags & Fgrey) {
		 ...
		 * calculation
		 * because the draw operator does this, and we need to be
		 * operating
