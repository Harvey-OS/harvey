/*
 * this implementation interacts badly with the vgavesa driver.
 * zeroing hwaccel causes dregs to be left behind where the (software)
 * cursor was.  setting hwaccel non-zero results instead in failure to
 * redraw behind deleted windows.
 */
#include <u.h>
#include <libc.h>
#include <draw.h>
...
#define pixelbits _pixelbits		/* no X server here */
...
void	_memfillcolor(Memimage *i, ulong val);
...
/* function from plan9ports */
void
memimagedraw(Memimage *dst, Rectangle r, Memimage *src, Point sp, Memimage *mask, Point mp, int op)
{
	Memdrawparam *par;

	// TODO if (swcursor in use) turn swcursor off();
	if((par = _memimagedrawsetup(dst, r, src, sp, mask, mp, op)) != nil)
		_memimagedraw(par);
	// TODO if (swcursor in use) restore swcursor();
}
