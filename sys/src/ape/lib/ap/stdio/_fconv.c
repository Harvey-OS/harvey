...
double _tens[] = {1e0, 1e1, 1e2, 1e3, 1e4, 1e5, 1e6, 1e7, 1e8, 1e9, 1e10, 1e11,
...
_Balloc(int k)
{
	...
	if(rv = freelist[k]) {
		...
		x = 1 << k;
		rv =
		    (Bigint*)malloc(sizeof(Bigint) + (x - 1) * sizeof(int32_t));
		rv->k = k;
		rv->maxwds = x;
	}
	rv->sign = rv->wds = 0;
...
Bigint* _multadd(Bigint* b, int m, int a) /* multiply by m and add a */
{
	int i, wds;
	unsigned long* x, y;
#ifdef Pack_32
