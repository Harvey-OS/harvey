...
regexec1(Reprog* progp, /* program to run */
         ...
         char* starts, char* eol, wchar_t startchar)
{
	int flag = 0;
	Reinst* inst;
	Relist* tlp;
		...
		for(tlp = tl; tlp->inst; tlp++) { /* assignment = */
			...
			for(inst = tlp->inst;; inst = inst->l.next) {
				...
				case ANY:
					...
					break;
				case ANYNL:
					if(_renewthread(nl, inst->l.next,
					                &tlp->se) == nle)
						return -1;
					break;
				case BOL:
				...
				case OR:
					...
					if(_renewthread(tlp, inst->r.right,
					                &tlp->se) == tle)
						return -1;
...
extern int regexec(Reprog* progp, /* program to run */
                   char* bol,     /* string to run machine on */
                   Resub* mp,     /* subexpression elements */
                   int ms)        /* number of elements at mp */
{
	...
	startchar =
	    (progp->startinst->type == RUNE && progp->startinst->r.r < Runeself)
	        ? progp->startinst->r.r
	        : 0;
