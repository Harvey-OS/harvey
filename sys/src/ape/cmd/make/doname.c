...
doname(nameblkp p, int reclevel, time_t* tval, int nowait)
{
	...
	for(lp = p->linep; lp; lp = lp->nxtlineblock) {
		...
		for(q = lp->depp; q; q = q->nxtdepblock)
			if(q->depname) {
				errstat += doname(q->depname, reclevel + 1,
				                  &td1, q->nowait);
				if(dbgflag)
					printf("TIME(%s)=%ld\n",
					       q->depname->namep, td1);
			...
			if(lp->shp)
				if(ptime < td || (ptime == 0 && td == 0) ||
				   lp->depp == 0) {
					...
					setvar("^", mkqlist(allchain, tgsbuf),
					       YES);
					qchain = NULL;
					if(!questflag)
						errstat += docom(
						    lp->shp, nowait, nproc1);
					set3var("@", CHNULL);
					okdel = okdel1;
					ptime1 = prestime();
					didwork = YES;
	...
	for(lp = sufflist; lp; lp = lp->nxtlineblock)
		for(suffp = lp->depp; suffp; suffp = suffp->nxtdepblock) {
			...
			if(suffix(p->namep, pnamep, prefix)) {
				(void)srchdir(concat(prefix, "*", temp), NO,
				              (depblkp)NULL);
				for(lp1 = sufflist; lp1;
				    lp1 = lp1->nxtlineblock)
					for(suffp1 = lp1->depp; suffp1;
					    suffp1 = suffp1->nxtdepblock) {
						p1namep =
						    suffp1->depname->namep;
						if((p1 = srchname(
						        concat(p1namep, pnamep,
						               concsuff))) &&
						   (p2 = srchname(
						        concat(prefix, p1namep,
						               sourcename)))) {
							errstat += doname(
							    p2, reclevel + 1,
							    &td, NO);
		...
		if(stem = wildmatch(wp, p->namep, len)) {
			...
			for(q = lp->depp; q; q = q->nxtdepblock) {
				...
				if(q->depname == NULL)
					continue;
				tamep =
				    srchname(wildsub(q->depname->namep, stem));
				/*TEMP fprintf(stderr,"check dep %s on %s
				 * =>%s\n",p->namep,q->depname->namep,tamep->namep);*/
				/*TEMP*/ if(dbgflag)
					printf("%s depends on %s. stem=%s\n",
					       p->namep, tamep->namep, stem);
				errstat +=
				    doname(tamep, reclevel + 1, &td, q->nowait);
				if(ptime < td)
	...
	if(errstat == 0 && (ptime < tdep || (ptime == 0 && tdep == 0))) {
		...
		if(explcom)
			...
			if(p1 = srchname(".DEFAULT")) {
				set3var("<", p->namep);
				for(lp2 = p1->linep; lp2;
				    lp2 = lp2->nxtlineblock)
					if(implcom = lp2->shp) {
						errstat += docom(
						    implcom, nowait, nproc1);
						break;
					}
			} else if(keepgoing) {
...
docom(struct shblock* q, int nowait, int nproc1)
{
	...
	for(; q; q = q->nxtshblock) {
		...
		if(docom1(s, ign, nopr, doit || !noexflag,
		          nowait && !q->nxtshblock) &&
		   !ign)
			return YES;
	}
