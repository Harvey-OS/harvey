...
char* dfltmacro[] = {".SUFFIXES : .o .c .y .l .a .sh .f", "MAKE=make", "AR=ar",
                     "ARFLAGS=rv", "YACC=yacc", "YFLAGS=", "LEX=lex", "LFLAGS=",
                     "LDFLAGS=", "CC=c89", "CFLAGS=-O", "FC=fort77",
                     "FFLAGS=-O 1", 0};

char* dfltpat[] = {
    "%.o : %.c", "\t$(CC) $(CFLAGS) -c $<",

    "%.o : %.y", "\t$(YACC) $(YFLAGS) $<", "\t$(CC) $(CFLAGS) -c y.tab.c",
    "\trm y.tab.c", "\tmv y.tab.o $@",

    "%.o : %.l", "\t$(LEX) $(LFLAGS) $<", "\t$(CC) $(CFLAGS) -c lex.yy.c",
    "\trm lex.yy.c", "\tmv lex.yy.o $@",

    "%.c : %.y", "\t$(YACC) $(YFLAGS) $<", "\tmv y.tab.c $@",

    "%.c : %.l", "\t$(LEX) $(LFLAGS) $<", "\tmv lex.yy.c $@",

    "% : %.o", "\t$(CC) $(CFLAGS) $(LDFLAGS) -o $@ $<",
...
char* dfltsuff[] = {
    ".SUFFIXES : .o .c .y .l .a .sh .f", ".c.o :", "\t$(CC) $(CFLAGS) -c $<",

    ".f.o :", "\t$(FC) $(FFLAGS) -c $<",

    ".y.o :", "\t$(YACC) $(YFLAGS) $<", "\t$(CC) $(CFLAGS) -c y.tab.c",
    "\trm -f y.tab.c", "\tmv y.tab.o $@",

    ".l.o :", "\t$(LEX) $(LFLAGS) $<", "\t$(CC) $(CFLAGS) -c lex.yy.c",
...
opdir(char* dirname, int stopifbad)
{
	...
	if(od->dirfc == NULL && (od->dirfc = opendir(dirname)) == NULL &&
	   stopifbad) {
	...
	*p++ = '(';
	...
	while(getarch()) {
		if(!strncmp(arfname, s, nc)) {
			clarch();
			/*TEMP fprintf(stderr, "found archive member %14s,
			 * time=%d\n", s, arfdate); */
			return arfdate;
		}
	}
