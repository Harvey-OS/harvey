...
static int metas(char* s) /* Are there are any  Shell meta-characters? */
{
	char c;
...
static void doclose(void) /* Close open directory files before exec'ing */
{
	struct dirhd* od;
...
waitproc(int* statp)
{
	...
	if(pid == -1) {
		...
		{
			if(proclive) {
				for(i = 0, procp = procstack; i < nproc;
				    ++i, ++procp)
					procp->done = YES;
				proclive = nproc = 0;
			}
