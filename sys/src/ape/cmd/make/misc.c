...
hashloc(char* s)
{
	...
	for(i = hashval; hashtab[i] != 0 && !equal(s, hashtab[i]->namep);
...
int suffix(char* a, char* b,
           char* p) /* is b the suffix of a?  if so, set p = prefix */
{
	char* a0, *b0;
	a0 = a;
	b0 = b;
...
subst(char* a, char* b, char* e)
{
	...
	if(a)
		while(*a) {
			if(*a != '$' || a[1] == '\0' || *++a == '$')
				/* if a non-macro character copy it.  if $$ or
				 * $\0, copy $ */
				*b++ = *a++;
			else {
				s = vname;
...
wildmatch(wildp p, char* name, int len)
{
	...
	if(len < p->totlen || strncmp(name, p->left, p->llen) ||
