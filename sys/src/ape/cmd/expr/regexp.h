...
char* compile(instring, ep, endbuf, seof) register char* ep;
...
step(p1, p2) register char* p1, *p2;
{
	register c;
...
advance(lp, ep) register char* lp, *ep;
{
	register char* curlp;
	char c;
	char* bbeg;
...
getrnge(str) register char* str;
{
	low = *str++ & 0377;
	size = *str == 255 ? 20000 : (*str & 0377) - low;
}

ecmp(a, b, count) register char* a, *b;
register count;
{
	while(count--)
		if(*a++ != *b++)
