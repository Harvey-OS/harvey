...
static void
    print_context_label(mark, inf, label) char const *mark;
...
void
    print_context_header(inf, unidiff_flag) struct file_data inf[];
int unidiff_flag;
{
	if(unidiff_flag) {
...
void
    print_context_script(script, unidiff_flag) struct change *script;
int unidiff_flag;
{
...
static void
    print_context_number_range(file, a, b) struct file_data const *file;
int a, b;
{
	int trans_a, trans_b;
...
static void
    pr_context_hunk(hunk) struct change *hunk;
{
...
static void
    print_unidiff_number_range(file, a, b) struct file_data const *file;
int a, b;
{
	int trans_a, trans_b;
	translate_range(file, a, b, &trans_a, &trans_b);
...
static void
    pr_unidiff_hunk(hunk) struct change *hunk;
{
...
static struct change *
    find_hunk(start) struct change *start;
{
	struct change *prev;
	int top0, top1;
	int thresh;
...
static void
    mark_ignorable(script) struct change *script;
{
	while(script) {
...
static void
    find_function(file, linenum, linep, lenp) struct file_data const *file;
int linenum;
char const **linep;
size_t *lenp;
{
