...
PARAMS((struct file_data const*, int, char const**, size_t*));
...
static void print_context_label
PARAMS((char const*, struct file_data*, char const*));
static void print_context_number_range
PARAMS((struct file_data const*, int, int));
static void print_unidiff_number_range
PARAMS((struct file_data const*, int, int));
...
static void print_context_label(mark, inf, label) char const* mark;
...
void print_context_header(inf, unidiff_flag) struct file_data inf[];
int unidiff_flag;
{
	if(unidiff_flag) {
...
void print_context_script(script, unidiff_flag) struct change* script;
int unidiff_flag;
{
...
static void print_context_number_range(file, a, b) struct file_data const* file;
int a, b;
{
	int trans_a, trans_b;
...
static void pr_context_hunk(hunk) struct change* hunk;
{
	...
	analyze_hunk(hunk, &first0, &last0, &first1, &last1, &show_from,
	             &show_to);
		...
		for(i = first0; i <= last0; i++) {
			...
			if(next && next->line0 <= i)
				/* The change NEXT covers this line.
				   If lines were inserted here in file 1, this
				   is "changed".
	...
	if(show_to) {
		...
		for(i = first1; i <= last1; i++) {
			...
			if(next && next->line1 <= i)
				/* The change NEXT covers this line.
				   If lines were deleted here in file 0, this is
				   "changed".
...
static void print_unidiff_number_range(file, a, b) struct file_data const* file;
int a, b;
{
	int trans_a, trans_b;
	translate_range(file, a, b, &trans_a, &trans_b);
...
static void pr_unidiff_hunk(hunk) struct change* hunk;
{
...
static struct change* find_hunk(start) struct change* start;
{
	struct change* prev;
	int top0, top1;
	int thresh;
...
static void mark_ignorable(script) struct change* script;
{
	while(script) {
		...
		analyze_hunk(script, &first0, &last0, &first1, &last1, &deletes,
		             &inserts);
...
static void find_function(file, linenum, linep, lenp) struct file_data const
    * file;
int linenum;
char const** linep;
size_t* lenp;
