...
#define SIGCHLD SIGCLD
	...
	if(edscript)
		conflicts_found = output_diff3_edscript(stdout, diff3, mapping, rev_mapping,
							tag_strings[0], tag_strings[1], tag_strings[2]);
	else if(merge) {
		if(!freopen(file[rev_mapping[FILE0]], "r", stdin))
			perror_with_exit(file[rev_mapping[FILE0]]);
		conflicts_found = output_diff3_merge(stdin, stdout, diff3, mapping, rev_mapping,
						     tag_strings[0], tag_strings[1], tag_strings[2]);
		if(ferror(stdin))
			fatal("read error");
	} else {
...
static struct diff3_block *
    make_3way_diff(thread0, thread1) struct diff_block *thread0,
    *thread1;
{
	/*
 ...
 */
	...
	while(current[0] || current[1]) {
      ...
      base_water_mark = D_LOWLINE (high_water_diff, FC);
		...
		using[high_water_thread] = last_using[high_water_thread] = high_water_diff;
	 ...
	 needs to be incorporated.  */
		while(other_diff && D_LOWLINE(other_diff, FC) <= high_water_mark + 1) {
 ...
 */
static struct diff3_block *
    using_to_diff3_block(using, last_using, low_thread, high_thread, last_diff3) struct diff_block
    *using[2],
    *last_using[2];
...
struct diff3_block const *last_diff3;
{
	...
	if(!using[0])
		...
		if(nl0 != nl1 || !compare_line_list(D_LINEARRAY(result, FILE0), D_LENARRAY(result, FILE0), D_LINEARRAY(result, FILE1), D_LENARRAY(result, FILE1), nl0))
 ...
 */
static int
    copy_stringlist(fromptrs, fromlengths, toptrs, tolengths, copynum) char *const fromptrs[];
char *toptrs[];
 ...
 */
static struct diff3_block *
    create_diff3_block(low0, high0, low1, high1, low2, high2) register int low0,
 ...
 */
static int
    compare_line_list(list1, lengths1, list2, lengths2, nl) char *const list1[],
    *const list2[];
...
int nl;
{
	...
	while(nl--)
		if(!*l1 || !*l2 || *lgths1 != *lgths2++ || memcmp(*l1++, *l2++, *lgths1++))
			return 0;
	return 1;
}
...
static struct diff_block *
    process_diff(filea, fileb, last_block) char const *filea,
    *fileb;
...
static enum diff_type
    process_diff_control(string, db) char **string;
...
static char *
    read_diff(filea, fileb, output_placement) char const *filea,
    *fileb;
...
#define INT_STRLEN_BOUND(type) ((sizeof(type) * CHAR_BIT - 1) * 302 / 1000 + 2)
	...
	*ap = 0;
	...
	FILE *fpipe;
	char *command = xmalloc(sizeof(diff_program) + 30 + INT_STRLEN_BOUND(int)+4 * (strlen(filea) + strlen(fileb)));
	char *p;
 ...
 */
static char *
    scan_diff_line(scan_ptr, set_start, set_length, limit, leadingchar) char *scan_ptr,
    **set_start;
size_t *set_length;
char *limit;
int leadingchar;
...
char const *file0, *file1, *file2;
{
	...
	for(b = reverse_diff3_blocklist(diff); b; b = b->next) {
		...
		enum diff_type type = ((b->correspond == DIFF_ALL) ? DIFF_ALL : ((enum diff_type)(((int)DIFF_1ST) + rev_mapping[(int)b->correspond - (int)DIFF_1ST])));
		...
		if(conflict) {
			...
			undotlines(outputfile, leading_dot,
				   ...
				   (D_NUMLINES(b, mapping[FILE1]) + D_NUMLINES(b, mapping[FILE2]) + 1));
 ...
 */
static struct diff3_block *
    reverse_diff3_blocklist(diff) struct diff3_block *diff;
{
...
static void
    fatal(string) char const *string;
{
	fprintf(stderr, "%s: %s\n", program_name, string);
	exit(2);
}
...
static void
    perror_with_exit(string) char const *string;
{
	int e = errno;
	fprintf(stderr, "%s: ", program_name);
	errno = e;
