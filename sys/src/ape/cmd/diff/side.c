...
void
    print_sdiff_script(script) struct change *script;
{
	begin_output();
...
static unsigned
    tab_from_to(from, to) unsigned from,
    to;
{
	FILE *out = outfile;
	unsigned tab;
 ...
 */
static unsigned
    print_half_line(line, indent, out_bound) char const *const *line;
...
static void
    print_1sdiff_line(left, sep, right) char const *const *left;
int sep;
...
static void
    print_sdiff_common_lines(limit0, limit1) int limit0,
    limit1;
{
...
static void
    print_sdiff_hunk(hunk) struct change *hunk;
{
