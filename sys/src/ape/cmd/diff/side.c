...
static unsigned print_half_line
PARAMS((char const* const*, unsigned, unsigned));
static unsigned tab_from_to PARAMS((unsigned, unsigned));
static void print_1sdiff_line
PARAMS((char const* const*, int, char const* const*));
...
void print_sdiff_script(script) struct change* script;
{
	begin_output();
...
static unsigned tab_from_to(from, to) unsigned from, to;
{
	FILE* out = outfile;
	unsigned tab;

	if(!tab_expand_flag)
		for(tab = from + TAB_WIDTH - from % TAB_WIDTH; tab <= to;
		    tab += TAB_WIDTH) {
			putc('\t', out);
			from = tab;
		}
 ...
 */
static unsigned print_half_line(line, indent, out_bound) char const* const
    * line;
unsigned indent, out_bound;
{
	FILE* out = outfile;
	register unsigned in_position = 0, out_position = 0;
	register char const* text_pointer = line[0], * text_limit = line[1];
		...
		case '\b':
			if(in_position != 0 && --in_position < out_bound)
				if(out_position <= in_position)
					/* Add spaces to make up for suppressed
					 * tab past out_bound.  */
					for(; out_position < in_position;
					    out_position++)
						putc(' ', out);
				else {
...
static void print_1sdiff_line(left, sep, right) char const* const* left;
int sep;
...
static void print_sdiff_common_lines(limit0, limit1) int limit0, limit1;
{
...
static void print_sdiff_hunk(hunk) struct change* hunk;
{
	...
	if(inserts && deletes) {
		for(i = first0, j = first1; i <= last0 && j <= last1; ++i, ++j)
			print_1sdiff_line(&files[0].linbuf[i], '|',
			                  &files[1].linbuf[j]);
		deletes = i <= last0;
		inserts = j <= last1;
		next0 = first0 = i;
