...
static int
    diag(xoff, xlim, yoff, ylim, minimal, part) int xoff,
    xlim, yoff, ylim, minimal;
...
static void
    compareseq(xoff, xlim, yoff, ylim, minimal) int xoff,
    xlim, yoff, ylim, minimal;
{
...
static void
    discard_confusing_lines(filevec) struct file_data filevec[];
{
	unsigned int f, i;
	char *discarded[2];
	int *equiv_count[2];
	...
	p = (int *)xmalloc((filevec[0].buffered_lines + filevec[1].buffered_lines) * (2 * sizeof(int)));
	for(f = 0; f < 2; f++) {
	...
	discarded[0] = xmalloc(sizeof(char) * (filevec[0].buffered_lines + filevec[1].buffered_lines));
	discarded[1] = discarded[0] + filevec[0].buffered_lines;
	bzero(discarded[0], sizeof(char) * (filevec[0].buffered_lines + filevec[1].buffered_lines));
...
static void
    shift_boundaries(filevec) struct file_data filevec[];
{
	int f;
...
static struct change *
    add_change(line0, line1, deleted, inserted, old) int line0,
...
static struct change *
    build_script(filevec) struct file_data const filevec[];
{
	struct change *script = 0;
	char *changed0 = filevec[0].changed_flag;
	char *changed1 = filevec[1].changed_flag;
...
static void
    briefly_report(changes, filevec) int changes;
...
/* Report the differences of two files.  DEPTH is the current directory
   depth. */
int
    diff_2_files(filevec, depth) struct file_data filevec[];
int depth;
{
	int diags;
	int i;
	...
	if(read_files(filevec, no_details_flag & ~ignore_some_changes)) {
		...
		if(filevec[0].stat.st_size != filevec[1].stat.st_size && (filevec[0].desc < 0 || S_ISREG(filevec[0].stat.st_mode)) && (filevec[1].desc < 0 || S_ISREG(filevec[1].stat.st_mode)))
			changes = 1;
		...
		/* Standard input equals itself.  */
			...
			for(;; filevec[0].buffered_chars = filevec[1].buffered_chars = 0) {
				...
				if(filevec[0].buffered_chars != filevec[1].buffered_chars || (filevec[0].buffered_chars != 0 && memcmp(filevec[0].buffer, filevec[1].buffer, filevec[0].buffered_chars) != 0)) {
					changes = 1;
					break;
				}
	 ...
	 Allocate an extra element, always zero, at each end of each vector.  */
		...
		filevec[1].changed_flag = filevec[0].changed_flag + filevec[0].buffered_lines + 2;
