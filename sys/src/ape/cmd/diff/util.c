...
void
    perror_with_name(text) char const *text;
{
	int e = errno;
...
void
    pfatal_with_name(text) char const *text;
{
	int e = errno;
	print_message_queue();
	fprintf(stderr, "%s: ", program_name);
...
void
    error(format, arg, arg1) char const *format,
    *arg, *arg1;
{
	fprintf(stderr, "%s: ", program_name);
...
void
    message(format, arg1, arg2) char const *format,
    *arg1, *arg2;
{
...
void
    message5(format, arg1, arg2, arg3, arg4) char const *format,
    *arg1, *arg2, *arg3, *arg4;
{
	if(paginate_flag) {
...
void
    setup_output(name0, name1, depth) char const *name0,
    *name1;
int depth;
{
	current_name0 = name0;
...
begin_output()
{
	...
	name = xmalloc(strlen(current_name0) + strlen(current_name1) + strlen(switch_string) + 7);
		...
		if(c1 != c2) {
		 ...
		 if it is at the end of the line.  */
		     ...
		     character in both sides and try again.  */
					if(c2 == ' ' && c1 != '\n' && (unsigned char const *)s1 + 1 < t1 && ISSPACE(t1[-2])) {
						--t1;
						continue;
					}
					if(c1 == ' ' && c2 != '\n' && (unsigned char const *)s2 + 1 < t2 && ISSPACE(t2[-2])) {
						--t2;
						continue;
					}
				}
...
struct change *
    find_change(start) struct change *start;
{
	return start;
}

struct change *
    find_reverse_change(start) struct change *start;
{
	return start;
}
...
void
    print_script(script, hunkfun, printfun) struct change *script;
...
void
    print_1_line(line_flag, line) char const *line_flag;
...
void
    output_1_line(text, limit, flag_format, line_flag) char const *text,
...
int
    change_letter(inserts, deletes) int inserts,
    deletes;
{
	if(!inserts)
		return 'd';
...
int
    translate_line_number(file, lnum) struct file_data const *file;
int lnum;
{
...
void
    translate_range(file, a, b, aptr, bptr) struct file_data const *file;
int a, b;
int *aptr, *bptr;
{
...
void
    print_number_range(sepchar, file, a, b) int sepchar;
...
void
    analyze_hunk(hunk, first0, last0, first1, last1, deletes, inserts) struct change *hunk;
...
char *
    dir_file_pathname(dir, file) char const *dir,
    *file;
{
...
void
    debug_script(sp) struct change *sp;
{
	fflush(stdout);
	for(; sp; sp = sp->link)
