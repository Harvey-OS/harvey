...
static int
    dir_sort(dir, dirdata) struct file_data const *dir;
struct dirdata *dirdata;
{
	...
	dirdata->names = names = (char const **)xmalloc(sizeof(char *) * (nnames + 1));
	for(i = 0; i < nnames; i++) {
		names[i] = data;
		data += strlen(data) + 1;
	}
...
int
    diff_dirs(filevec, handle_file, depth) struct file_data const filevec[];
