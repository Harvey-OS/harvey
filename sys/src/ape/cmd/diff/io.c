...
int
    sip(current, skip_test) struct file_data *current;
int skip_test;
{
...
void
    slurp(current) struct file_data *current;
{
	size_t cc;
...
static void
    find_and_hash_each_line(current) struct file_data *current;
{
	unsigned h;
	...
	hashing_done:
		...
		if((char const *)p == bufend && current->missing_newline && ROBUST_OUTPUT_STYLE(output_style)) {
...
static void
    prepare_text_end(current) struct file_data *current;
{
...
static void
    find_identical_ends(filevec) struct file_data filevec[];
{
	word *w0, *w1;
	...
	if(p0 == p1)
	 ...
	 to make the equality test false, and thus terminate the loop.  */
		...
		if(ROBUST_OUTPUT_STYLE(output_style) && (buffer0 + n0 - filevec[0].missing_newline < p0) != (buffer1 + n1 - filevec[1].missing_newline < p1))
			--p0, --p1;
	}
	...
	if(!ROBUST_OUTPUT_STYLE(output_style) || filevec[0].missing_newline == filevec[1].missing_newline) {
	...
	if(no_diff_means_no_output && !function_regexp_list) {
		...
		alloc_lines0 = prefix_count + GUESS_LINES(0, 0, p0 - filevec[0].prefix_end) + context;
	} else {
		prefix_count = 0;
		prefix_mask = ~0;
	...
	if(!(no_diff_means_no_output && filevec[0].prefix_end == p0 && filevec[1].prefix_end == p1)) {
		...
		while(p0 != end0) {
			...
			if(l == alloc_lines0)
				linbuf0 = (char const **)xrealloc(linbuf0, (alloc_lines0 *= 2) * sizeof(*linbuf0));
			linbuf0[l] = p0;
			while(*p0++ != '\n')
				;
		}
	...
	alloc_lines1 = (buffered_prefix + GUESS_LINES(lines, filevec[1].prefix_end - buffer1, tem) + context);
...
int
    read_files(filevec, pretend_binary) struct file_data filevec[];
int pretend_binary;
{
	int i;
