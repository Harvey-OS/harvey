...
int sip(current, skip_test) struct file_data* current;
int skip_test;
{
...
#endif
			size_t n = read(current->desc, current->buffer,
			                current->bufsize);
			if(n == -1)
...
#if HAVE_SETMODE
			if(oldmode != O_BINARY) {
				if(lseek(current->desc, -(off_t)n, SEEK_CUR) ==
				   -1)
					pfatal_with_name(current->name);
...
void slurp(current) struct file_data* current;
{
	size_t cc;
	...
	/* It's not a regular file; read it, growing the buffer as needed.  */
		...
		for(;;) {
			if(current->buffered_chars == current->bufsize) {
				...
				current->buffer =
				    xrealloc(current->buffer, current->bufsize);
			}
			cc = read(current->desc,
			          current->buffer + current->buffered_chars,
...
static void find_and_hash_each_line(current) struct file_data* current;
{
	unsigned h;
	...
	hashing_done:
		...
		if((char const*)p == bufend && current->missing_newline &&
		   ROBUST_OUTPUT_STYLE(output_style)) {
			...
			/* Omit the inserted newline when computing linbuf
			 * later.  */
			p--;
		...
		for(i = *bucket;; i = eqs[i].next)
			if(!i) {
				/* Create a new equivalence class in this
				 * bucket. */
				i = eqs_index++;
				if(i == eqs_alloc)
					eqs = (struct equivclass*)xrealloc(
					    eqs,
					    (eqs_alloc *= 2) * sizeof(*eqs));
				eqs[i].next = *bucket;
				eqs[i].hash = h;
				eqs[i].line = ip;
				eqs[i].length = length;
				*bucket = i;
				...
				/* Reuse existing equivalence class if the lines
				   are identical.
				   This detects the common case of exact
				   identity
				   faster than complete comparison would.  */
				if(eqs[i].length == length &&
				   memcmp(eqline, ip, length) == 0)
					break;

				/* Reuse existing class if line_cmp reports the
				 * lines equal.  */
		...
		if(line == alloc_lines) {
			...
			cureqs = (int*)xrealloc(cureqs,
			                        alloc_lines * sizeof(*cureqs));
			linbuf =
			    (char const**)xrealloc(linbuf + linbuf_base,
...
static void prepare_text_end(current) struct file_data* current;
{
...
static void find_identical_ends(filevec) struct file_data filevec[];
{
	word* w0, *w1;
	...
	if(!ROBUST_OUTPUT_STYLE(output_style) ||
	   filevec[0].missing_newline == filevec[1].missing_newline) {
		...
		while(p0 != beg0)
			if(*--p0 != *--p1) {
				/* Point at the first char of the matching
				 * suffix.  */
				++p0, ++p1;
				beg0 = p0;
				break;
		...
		/* Are we at a line-beginning in both files?  If not, add the
		   rest of
		   this line to the main body.  Discard up to HORIZON_LINES
		   lines from
	...
	/* Calculate number of lines of prefix to save.
	   ...
	   Handle 1 more line than the context says (because we count 1 too
	   many),
	   rounded up to the next power of 2 to speed index computation.  */

	if(no_diff_means_no_output && !function_regexp_list) {
		for(prefix_count = 1; prefix_count < context + 1;
		    prefix_count *= 2)
			;
	...
	if(!(no_diff_means_no_output && filevec[0].prefix_end == p0 &&
	     filevec[1].prefix_end == p1)) {
		p0 = buffer0;
		...
		while(p0 != end0) {
			...
			if(l == alloc_lines0)
				linbuf0 = (char const**)xrealloc(
				    linbuf0,
				    (alloc_lines0 *= 2) * sizeof(*linbuf0));
			linbuf0[l] = p0;
			while(*p0++ != '\n')
				;
		}
	...
	if(buffered_prefix != lines) {
		...
		for(i = 0; i < buffered_prefix; i++)
			linbuf1[i] =
			    linbuf0[(lines - context + i) & prefix_mask];
		for(i = 0; i < buffered_prefix; i++)
			linbuf0[i] = linbuf1[i];
	}
...
static int const primes[] = {
    509,       1021,      2039,      4093,       8191,       16381,   32749,
#if 32767 < INT_MAX
    65521,     131071,    262139,    524287,     1048573,    2097143, 4194301,
    8388593,   16777213,  33554393,  67108859, /* Preposterously large . . . */
...
int read_files(filevec, pretend_binary) struct file_data filevec[];
int pretend_binary;
{
	int i;
	...
	equivs = (struct equivclass*)xmalloc(equivs_alloc *
	                                     sizeof(struct equivclass));
