...
#include <sys/cdefs.h>
__FBSDID("$FreeBSD: src/contrib/diff/sdiff.c,v 1.1.1.1.12.1 2002/01/28 "
         "01:26:35 nectar Exp $");
...
static int edit
PARAMS((struct line_filter*, int, struct line_filter*, int, FILE*));
static int interact
PARAMS((struct line_filter*, struct line_filter*, struct line_filter*, FILE*));
...
static void fatal(msg) char const* msg;
{
	fprintf(stderr, "%s: %s\n", program_name, msg);
	exiterr();
}

static void perror_fatal(msg) char const* msg;
{
	int e = errno;
	checksigs();
...
static FILE* ck_fopen(fname, type) char const* fname, *type;
{
...
static void ck_fclose(f) FILE* f;
{
	if(fclose(f))
		perror_fatal("input/output error");
}

static size_t ck_fread(buf, size, f) char* buf;
size_t size;
FILE* f;
{
...
static void ck_fwrite(buf, size, f) char const* buf;
size_t size;
FILE* f;
{
...
static void ck_fflush(f) FILE* f;
{
	if(fflush(f) != 0)
		perror_fatal("output error");
}

static char const* expand_name(name, is_dir, other_name) char* name;
int is_dir;
...
static void lf_init(lf, infile) struct line_filter* lf;
FILE* infile;
{
	lf->infile = infile;
...
static size_t lf_refill(lf) struct line_filter* lf;
{
...
static void lf_copy(lf, lines, outfile) struct line_filter* lf;
int lines;
FILE* outfile;
{
...
static void lf_skip(lf, lines) struct line_filter* lf;
int lines;
{
	while(lines) {
		lf->bufpos =
...
static int lf_snarf(lf, buffer, bufsize) struct line_filter* lf;
char* buffer;
size_t bufsize;
{
	char* start = lf->bufpos;
...
char* argv[];
{
	...
	while((opt = getopt_long(argc, argv, "abBdHiI:lo:stvw:W", longopts,
	                         0)) != EOF) {
		switch(opt) {
		case 'a':
			diffarg("-a");
	...
	if(!out_file) {
		...
		left = ck_fopen(
		    expand_name(argv[optind], leftdir, argv[optind + 1]), "r");
		;
		right = ck_fopen(
		    expand_name(argv[optind + 1], rightdir, argv[optind]), "r");
...
#else  /* HAVE_FORK */
		{
			...
			if(!diffpid) {
				signal(SIGINT, SIG_IGN); /* in case user
				                            interrupts editor */
...
static void diffarg(a) char const* a;
{
	...
	if(diffargs == diffargsmax) {
		...
		diffargsmax *= 2;
		diffargv = (char const**)realloc(
		    diffargv, diffargsmax * sizeof(char const*));
		if(!diffargv)
			fatal("out of memory");
	}
	diffargv[diffargs++] = a;
...
static RETSIGTYPE catchsig(s) int s;
{
#if !HAVE_SIGACTION
	signal(s, SIG_IGN);
#endif
...
static int edit(left, lenl, right, lenr, outfile) struct line_filter* left;
int lenl;
	...
	for(;;) {
		...
		case 'e': {
...

			{
				int wstatus;
#if !HAVE_FORK
				char* command = xmalloc(strlen(edbin) +
				                        strlen(tmpname) + 2);
				...
				perror_fatal("fseek");
			{
				/* SDIFF_BUFSIZE is too big for a local var
				   in some compilers, so we allocate it
				   dynamically.  */
				char* buf = xmalloc(SDIFF_BUFSIZE);
				size_t size;

				while((size = ck_fread(buf, SDIFF_BUFSIZE,
				                       tmp)) != 0) {
					checksigs();
...
static int interact(diff, left, right, outfile) struct line_filter* diff;
...
static int diraccess(dir) char const* dir;
{
	struct stat buf;
