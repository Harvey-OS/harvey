...
void print_ed_script(script) struct change* script;
{
...
static void print_ed_hunk(hunk) struct change* hunk;
{
	int f0, l0, f1, l1;
	int deletes, inserts;
  ...
  hunk = flip_script (hunk);
	...
	if(inserts) {
		...
		for(i = f1; i <= l1; i++) {
			...
			if(!inserting)
				fprintf(outfile, "%da\n",
				        i - f1 + translate_line_number(
				                     &files[0], f0) -
				            1);
			inserting = 1;

			/* If the file's line is just a dot, it would confuse
			   `ed'.
...
void pr_forward_ed_script(script) struct change* script;
{
	print_script(script, find_change, pr_forward_ed_hunk);
}

static void pr_forward_ed_hunk(hunk) struct change* hunk;
{
	int i;
	int f0, l0, f1, l1;
	int deletes, inserts;
...
void print_rcs_script(script) struct change* script;
{
...
static void print_rcs_hunk(hunk) struct change* hunk;
{
	int i;
	int f0, l0, f1, l1;
	int deletes, inserts;
