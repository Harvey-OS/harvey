...
void print_ifdef_script(script) struct change* script;
{
	...
	if(next_line < files[0].valid_lines) {
		begin_output();
		format_ifdef(
		    group_format[UNCHANGED], next_line, files[0].valid_lines,
...
static void print_ifdef_hunk(hunk) struct change* hunk;
{
	int first0, last0, first1, last1, deletes, inserts;
	char* format;

	/* Determine range of line numbers involved in each file.  */
	analyze_hunk(hunk, &first0, &last0, &first1, &last1, &deletes,
	             &inserts);
	if(inserts)
...
static void format_ifdef(format, beg0, end0, beg1, end1) char* format;
...
static char* format_group(out, format, endchar, groups) register FILE* out;
char* format;
int endchar;
struct group const* groups;
{
	...
	while((c = *f) != endchar && c != 0) {
		...
		if(c == '%') {
			...
			case '(':
				/* Print if-then-else format e.g.
				 * `%(n=1?thenpart:elsepart)'.  */
				{
					int i, value[2];
					...
					f = format_group(thenout, f, ':',
					                 groups);
					if(*f) {
						f = format_group(elseout, f + 1,
						                 ')', groups);
						if(*f)
							f++;
					}
			...
			case '<':
				...
				print_ifdef_lines(out, line_format[OLD],
				                  &groups[0]);
				continue;

			case '=':
				/* Print common lines.  */
				print_ifdef_lines(out, line_format[UNCHANGED],
				                  &groups[0]);
				continue;

			case '>':
				/* Print lines inserted from second file.  */
				print_ifdef_lines(out, line_format[NEW],
				                  &groups[1]);
				continue;
			...
			default: {
				...
				if(out) {
					/* Temporarily replace e.g. "%3dnx" with
					 * "%3d\0x".  */
					*speclim = 0;
...
static int groups_letter_value(g, letter) struct group const* g;
int letter;
{
	if(ISUPPER(letter)) {
		g++;
	...
	switch(letter) {
	case 'e':
		return translate_line_number(g->file, g->from) - 1;
	case 'f':
		return translate_line_number(g->file, g->from);
	case 'l':
		return translate_line_number(g->file, g->upto) - 1;
	case 'm':
...
static void print_ifdef_lines(out, format, group) register FILE* out;
char* format;
...
static char* scan_char_literal(lit, intptr) char* lit;
int* intptr;
{
	register char* p = lit;
	int value, digits;
...
static char* scan_printf_spec(spec) register char* spec;
{
