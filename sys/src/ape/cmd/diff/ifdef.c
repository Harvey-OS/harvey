...
void
    print_ifdef_script(script) struct change *script;
{
...
static void
    print_ifdef_hunk(hunk) struct change *hunk;
{
...
static void
    format_ifdef(format, beg0, end0, beg1, end1) char *format;
...
static char *
    format_group(out, format, endchar, groups) register FILE *out;
char *format;
int endchar;
...
static int
    groups_letter_value(g, letter) struct group const *g;
int letter;
{
	if(ISUPPER(letter)) {
		g++;
...
static void
    print_ifdef_lines(out, format, group) register FILE *out;
char *format;
...
static char *
    scan_char_literal(lit, intptr) char *lit;
int *intptr;
{
	register char *p = lit;
	int value, digits;
...
static char *
    scan_printf_spec(spec) register char *spec;
{
