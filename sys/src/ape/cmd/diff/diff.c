...
static char* option_list(
    optionvec,
    count) char** optionvec; /* Was `vector', but that collides on Alliant.  */
int count;
{
	int i;
	size_t length = 0;
...
int excluded_filename(f) char const* f;
{
	int i;
...
static void add_exclude(pattern) char const* pattern;
{
	if(exclude_alloc <= exclude_count)
		exclude = (char const**)(exclude_alloc == 0
		                             ? xmalloc((exclude_alloc = 64) *
		                                       sizeof(*exclude))
		                             : xrealloc(exclude,
		                                        (exclude_alloc *= 2) *
		                                            sizeof(*exclude)));
...
static int add_exclude_file(name) char const* name;
{
	struct file_data f;
	char* p, *q, *lim;
	...
	f.name = optarg;
	f.desc =
	    (strcmp(name, "-") == 0 ? STDIN_FILENO : open(name, O_RDONLY, 0));
	...
	while((c = getopt_long(
		...
		case '0':
			...
			/* If a context length has already been specified,
			   more digits allowed only if they follow right after
			   the others.
			   Reject two separate runs of digits, or digits after
			   -C.  */
		...
		case 'c':
			...
			specify_style(c == 'U' ? OUTPUT_UNIFIED
			                       : OUTPUT_CONTEXT);
			break;

		case 'd':
			/* Don't discard lines.  This makes things slower
			   (sometimes much
			   slower) but will find a guaranteed minimal set of
			   changes.  */
			no_discards = 1;
			break;
			...
			specify_style(OUTPUT_IFDEF);
			{
				...
				char* b =
				    xmalloc(sizeof(C_ifdef_group_formats) +
				            ...
				            - 8 /* 5*"%%" + 3*"%c" */);
				sprintf(b, C_ifdef_group_formats, optarg,
				        optarg, 0, optarg, optarg, 0, 0, optarg,
				        optarg, optarg);
				for(i = 0; i < 4; i++) {
					err |=
					    specify_format(&group_format[i], b);
					b += strlen(b) + 1;
				}
				if(err)
					error("conflicting #ifdef formats", 0,
					      0);
			}
			break;
		...
		case 'h':
			/* Split the files into chunks of around 1500 lines
			   for faster processing.  Usually does not change the
			   result.
		...
		case 'H':
			/* Turn on heuristics that speed processing of large
			   files
...
#define SIGCHLD SIGCLD
			...
			/* Pagination requires forking and waiting, and
			   System V fork+wait does not work if SIGCHLD is
			   ignored.  */
		...
		case 'n':
			/* Output RCS-style diffs, like `-f' except that each
			   command
		...
		case 'N':
			/* When comparing directories, if a file appears only in
			   one
			   directory, treat it as present but empty in the
			   other.  */
		...
		case 'p':
			/* Make context-style output and show name of last C
			 * function.  */
			show_c_function = 1;
		...
		case 'S':
			/* When comparing directories, start with the specified
			   file name.  This is used for resuming an aborted
			   comparison.  */
			dir_start_file = optarg;
			break;

		case 't':
			/* Expand tabs to spaces in the output so that it
			   preserves
		...
		case 'T':
			/* Use a tab in the output, rather than a space, before
			   the
			   text of an input line, so as to keep the proper
			   alignment
			   in the input line without changing the characters in
			   it.  */
			tab_align_flag = 1;
			break;
		...
		case 135:
			...
			{
				int i, err = 0;
				for(i = 0; i < sizeof(line_format) /
				                   sizeof(*line_format);
				    i++)
					err |= specify_format(&line_format[i],
					                      optarg);
				if(err)
					error("conflicting line format", 0, 0);
			}
...
/* Use binary I/O when reading and writing data.
   On Posix hosts, this has no effect.  */
	...
	if(argc - optind != 2)
		try_help(argc - optind < 2 ? "missing operand"
		                           : "extra operand");

	{
		/*
		 *	We maximize first the half line width, and then the
		 *gutter width,
		 *	according to the following constraints:
		 *	1.  Two half lines plus a gutter must fit in a line.
		 *	2.  If the half line width is nonzero:
		 *	    a.  The gutter width is at least
		 *GUTTER_WIDTH_MINIMUM.
		 *	    b.  If tabs are not expanded to spaces,
		 *		a half line plus a gutter is an integral number of
		 *tabs,
		...
		sdiff_half_width =
		    max(0, min(off - GUTTER_WIDTH_MINIMUM, width - off)),
	...
	if(output_style == OUTPUT_IFDEF) {
		/* Format arrays are char *, not char const *,
		   ...
		   But it is safe to assign a constant like "%=" to a format
		   array,
		...
		for(i = 0; i < sizeof(line_format) / sizeof(*line_format); i++)
			if(!line_format[i])
				line_format[i] = "%l\n";
		if(!group_format[OLD])
			group_format[OLD] = group_format[UNCHANGED]
			                        ? group_format[UNCHANGED]
			                        : "%<";
		if(!group_format[NEW])
			group_format[NEW] = group_format[UNCHANGED]
			                        ? group_format[UNCHANGED]
			                        : "%>";
		if(!group_format[UNCHANGED])
			group_format[UNCHANGED] = "%=";
		if(!group_format[CHANGED])
			group_format[CHANGED] =
			    concat(group_format[OLD], group_format[NEW], "");
	}
...
static void add_regexp(reglist, pattern) struct regexp_list** reglist;
char const* pattern;
{
...
static char const* const option_help[] = {
    ...
    "-e  --ed  Output an ed script.", "-n  --rcs  Output an RCS format diff.",
    "-y  --side-by-side  Output in two columns.", "  -w NUM  --width=NUM  "
                                                  "Output at most NUM (default "
    ...
    "  GFMT may contain:", "    %<  lines from FILE1",
    "    %>  lines from FILE2", "    %=  lines common to FILE1 and FILE2",
    "    %[-][WIDTH][.[PREC]]{doxX}LETTER  printf-style spec for LETTER",
    "      LETTERs are as follows for new group, lower case for old group:",
    "        F  first line number", "        L  last line number",
    "        N  number of lines = L-F+1", "        E  F-1", "        M  L+1",
    "  LFMT may contain:", "    %L  contents of line",
    ...
    "changes.\n",
    "-v  --version  Output version info.", "--help  Output this help.", 0};
...
static int specify_format(var, value) char** var;
char* value;
{
...
static void specify_style(style) enum output_style style;
{
	if(output_style != OUTPUT_NORMAL && output_style != style)
		error("conflicting specifications of output style", 0, 0);
	output_style = style;
}

static char const* filetype(st) struct stat const* st;
{
	...
	if(S_ISREG(st->st_mode)) {
		...
		/* Posix.2 section 5.14.2 seems to suggest that we must read the
		   file
		   and guess whether it's C, Fortran, etc., but this is somewhat
		   useless
		   and doesn't reflect historical practice.  We're allowed to
		   guess
...
static int compare_files(dir0, name0, dir1, name1, depth) char const* dir0,
    *dir1;
...
int depth;
{
	...
	if(!((name0 != 0 && name1 != 0) ||
		...
		message("Only in %s: %s\n", dir, name);
		/* Return 1 so that diff_dirs will return 1 ("some files
		 * differ").  */
		return 1;
	}
	...
	inf[0].name =
	    dir0 == 0 ? name0 : (free0 = dir_file_pathname(dir0, name0));
	inf[1].name =
	    dir1 == 0 ? name1 : (free1 = dir_file_pathname(dir1, name1));
	...
	for(i = 0; i <= 1; i++) {
		if(inf[i].desc != -1) {
			...
			if(i && filename_cmp(inf[i].name, inf[0].name) == 0) {
				...
				if(stat_result == 0 &&
				   S_ISREG(inf[i].stat.st_mode)) {
					off_t pos = lseek(STDIN_FILENO,
					                  (off_t)0, SEEK_CUR);
					if(pos == -1)
						stat_result = -1;
					else {
						...
						/* Posix.2 4.17.6.1.4 requires
						 * current time for stdin.  */
			...
			if(stat_result != 0) {
				...
				inf[i].dir_p = S_ISDIR(inf[i].stat.st_mode) &&
				               inf[i].desc != 0;
				if(inf[1 - i].desc == -1) {
					inf[1 - i].dir_p = inf[i].dir_p;
					inf[1 - i].stat.st_mode =
					    inf[i].stat.st_mode;
				}
			}
		}
	...
	if(failed) {
		...
		if(depth > 0 && !recursive) {
			...
			val = diff_dirs(inf, compare_files, depth);
	          ...
	          (depth > 0 && (!S_ISREG(inf[0].stat.st_mode) ||
	                         !S_ISREG(inf[1].stat.st_mode)))) {
		/* Perhaps we have a subdirectory that exists only in one
		   directory.
		   If so, just print a message to that effect.  */

		if(inf[0].desc == -1 || inf[1].desc == -1) {
			if((inf[0].dir_p | inf[1].dir_p) && recursive &&
			   (entire_new_file_flag ||
				...
				char const* dir =
				    (inf[0].desc == -1) ? dir1 : dir0;
				/* See Posix.2 section 4.17.6.1.1 for this
				 * format.  */
				message("Only in %s: %s\n", dir, name0);
				val = 1;
			}
		...
		if(inf[1].desc == -2)
			...
			else if((inf[1].desc = open(inf[1].name, O_RDONLY, 0)) <
			        0) {
