...
static char *
    option_list(optionvec, count) char **optionvec; /* Was `vector', but that collides on Alliant.  */
int count;
{
	int i;
	size_t length = 0;
...
static int
    ck_atoi(str, out) char const *str;
int *out;
{
	char const *p;
	for(p = str; *p; p++)
...
int
    excluded_filename(f) char const *f;
{
	int i;
...
static void
    add_exclude(pattern) char const *pattern;
{
...
static int
    add_exclude_file(name) char const *name;
{
	struct file_data f;
	char *p, *q, *lim;
		...
		case 'D':
			...
			specify_style(OUTPUT_IFDEF);
			{
				...
				static char const C_ifdef_group_formats[] =
				    "#ifndef %s\n%%<#endif /* not %s */\n%c#ifdef %s\n%%>#endif /* %s */\n%c%%=%c#ifndef %s\n%%<#else /* %s */\n%%>#endif /* %s */\n";
				char *b = xmalloc(sizeof(C_ifdef_group_formats) + 7 * strlen(optarg) - 14 /* 7*"%s" */
						  - 8 /* 5*"%%" + 3*"%c" */);
...
/* Use binary I/O when reading and writing data.
	 ...
	 since "%=" does not format any integers.  */
		...
		for(i = 0; i < sizeof(line_format) / sizeof(*line_format); i++)
			if(!line_format[i])
				line_format[i] = "%l\n";
		if(!group_format[OLD])
			group_format[OLD] = group_format[UNCHANGED] ? group_format[UNCHANGED] : "%<";
		if(!group_format[NEW])
			group_format[NEW] = group_format[UNCHANGED] ? group_format[UNCHANGED] : "%>";
	...
	no_diff_means_no_output =
	    (output_style == OUTPUT_IFDEF ? (!*group_format[UNCHANGED] || (strcmp(group_format[UNCHANGED], "%=") == 0 && !*line_format[UNCHANGED]))
...
static void
    add_regexp(reglist, pattern) struct regexp_list **reglist;
char const *pattern;
{
...
static int
    specify_format(var, value) char **var;
char *value;
{
...
static void
    specify_style(style) enum output_style style;
{
...
static char const *
    filetype(st) struct stat const *st;
{
...
static int
    compare_files(dir0, name0, dir1, name1, depth) char const *dir0,
    *dir1;
...
int depth;
{
	...
	if(!((name0 != 0 && name1 != 0) || (unidirectional_new_file_flag && name1 != 0) || entire_new_file_flag)) {
	...
	if(!failed && depth == 0 && inf[0].dir_p != inf[1].dir_p) {
	 ...
	 use the file in that dir with the other file's basename.  */
		...
		char const *filename = inf[dir_arg].name = dir_file_pathname(dir, p ? p + 1 : fnm);
	...
	} else if((same_files = inf[0].desc != -1 && inf[1].desc != -1 && 0 < same_file(&inf[0].stat, &inf[1].stat)) && no_diff_means_no_output) {
	...
	} else if((inf[0].dir_p | inf[1].dir_p) || (depth > 0 && (!S_ISREG(inf[0].stat.st_mode) || !S_ISREG(inf[1].stat.st_mode)))) {
		/* Perhaps we have a subdirectory that exists only in one directory.
	 If so, just print a message to that effect.  */

		if(inf[0].desc == -1 || inf[1].desc == -1) {
			if((inf[0].dir_p | inf[1].dir_p) && recursive && (entire_new_file_flag || (unidirectional_new_file_flag && inf[0].desc == -1)))
				val = diff_dirs(inf, compare_files, depth);
			else {
	...
	} else if((no_details_flag & ~ignore_some_changes) && inf[0].stat.st_size != inf[1].stat.st_size && (inf[0].desc == -1 || S_ISREG(inf[0].stat.st_mode)) && (inf[1].desc == -1 || S_ISREG(inf[1].stat.st_mode))) {
