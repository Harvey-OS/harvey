...
#ifdef DEBUG
static void
    extract_number(dest, source) int *dest;
...
#ifdef DEBUG
static void
    extract_number_and_incr(destination, source) int *destination;
unsigned char **source;
{
	extract_number(destination, *source);
	*source += 2;
...
#define EXTRACT_CHARACTER(destination, source)                                            \
	do {                                                                              \
		(destination) = ((source)[0] | ((source)[1] << 8) | ((source)[2] << 16)); \
	} while(0)
...
#define CHARSET_LOOKUP_BITMAP(p, c) \
	((c) < CHARSET_BITMAP_SIZE(p) * BYTEWIDTH && (p)[2 + (c) / BYTEWIDTH] & (1 << ((c) % BYTEWIDTH)))
...
#define CHARSET_LOOKUP_RANGE_TABLE_RAW(not, c, range_table, count)                                \
	do {                                                                                      \
		...
		unsigned char *p;                                                                 \
		unsigned char *range_table_end = CHARSET_RANGE_TABLE_END((range_table), (count)); \
                                                                                                  \
...
void
    print_fastmap(fastmap) char *fastmap;
{
...
void
    print_partial_compiled_pattern(start, end) unsigned char *start;
unsigned char *end;
{
	int mcnt, mcnt2;
...
void
    print_compiled_pattern(bufp) struct re_pattern_buffer *bufp;
{
...
#define INIT_FAIL_STACK()                                                                                                                          \
	do {                                                                                                                                       \
		fail_stack.stack = (fail_stack_elt_t *)REGEX_ALLOCATE_STACK(INIT_FAILURE_ALLOC * TYPICAL_FAILURE_SIZE * sizeof(fail_stack_elt_t)); \
                                                                                                                                                   \
...
#define GROW_FAIL_STACK(fail_stack)                                                                                          \
	(((fail_stack).size * sizeof(fail_stack_elt_t) >= re_max_failures * TYPICAL_FAILURE_SIZE)                            \
	     ? 0                                                                                                             \
	     : ((fail_stack).stack = (fail_stack_elt_t *)                                                                    \
		    REGEX_REALLOCATE_STACK((fail_stack).stack,                                                               \
					   (fail_stack).size * sizeof(fail_stack_elt_t),                                     \
					   MIN(re_max_failures * TYPICAL_FAILURE_SIZE,                                       \
					       ((fail_stack).size * sizeof(fail_stack_elt_t) * FAIL_STACK_GROWTH_FACTOR))),  \
                                                                                                                             \
		(fail_stack).stack == NULL                                                                                   \
		    ? 0                                                                                                      \
		    : ((fail_stack).size = (MIN(re_max_failures * TYPICAL_FAILURE_SIZE,                                      \
						((fail_stack).size * sizeof(fail_stack_elt_t) * FAIL_STACK_GROWTH_FACTOR)) / \
...
#define NUM_FAILURE_ITEMS                                      \
	(((0                                                   \
	       ? 0                                             \
	       : highest_active_reg - lowest_active_reg + 1) * \
	  NUM_REG_ITEMS) +                                     \
	 NUM_NONREG_ITEMS)
...
#define EXTEND_RANGE_TABLE_WORK_AREA(work_area, n)                                                            \
	do {                                                                                                  \
		if(((work_area).used + (n)) * sizeof(int) > (work_area).allocated) {                          \
			...
			if((work_area).table)                                                                 \
				(work_area).table = (int *)realloc((work_area).table, (work_area).allocated); \
			else                                                                                  \
				(work_area).table = (int *)malloc((work_area).allocated);                     \
	...
	while(p != pend) {
		...
		case '*':
		   ...
		   for null bytes, because of RE_DOT_NOT_NULL?	*/
					if(TRANSLATE((unsigned char)*(p - 2)) == TRANSLATE('.') && zero_times_ok && p < pend && TRANSLATE((unsigned char)*p) == TRANSLATE('\n') && !(syntax & RE_DOT_NEWLINE)) {/* We have .*\n.  */
		...
		case '[': {
		   ...
		   class.  */
			   ...
			   Instead, just set it to C and handle below.	*/
				...
				if(p < pend && p[0] == '-' && p[1] != ']') {
					...
					if(SINGLE_BYTE_CHAR_P(c) && !SINGLE_BYTE_CHAR_P(c1)) {
		   ...
		   bytes for COUNT and three bytes for each character.	*/
				...
				for(i = 0; i < used; i++)
					STORE_CHARACTER_AND_INCR(b, RANGE_TABLE_WORK_ELT(range_table_work, i));
			}
		} break;
		...
		normal_char:
			...
			if(!pending_exact
			   ...
			   *pending_exact >= (1 << BYTEWIDTH) - (p - p1)
			   ...
			   ||
			   (p != pend && (*p == '*' || *p == '^')) || ((syntax & RE_BK_PLUS_QM)
									   ? p + 1 < pend && *p == '\\' && (p[1] == '+' || p[1] == '?')
									   : p != pend && (*p == '+' || *p == '?')) ||
     ...
     the search and match routines.  */
	{
		...
		if(fail_stack.size < re_max_failures * TYPICAL_FAILURE_SIZE) {
			...
			if(!fail_stack.stack)
				...
				fail_stack.stack = (fail_stack_elt_t *)xrealloc(fail_stack.stack,
										(fail_stack.size * sizeof(fail_stack_elt_t)));
#else  /* not emacs */
			if(!fail_stack.stack)
				fail_stack.stack = (fail_stack_elt_t *)malloc(fail_stack.size * sizeof(fail_stack_elt_t));
			else
				fail_stack.stack = (fail_stack_elt_t *)realloc(fail_stack.stack,
									       (fail_stack.size * sizeof(fail_stack_elt_t)));
#endif /* not emacs */
		}
...
int
    re_compile_fastmap(bufp) struct re_pattern_buffer *bufp;
{
	int i, j, k;
#ifdef MATCH_MAY_ALLOCATE
	...
	while(1) {
	 ...
	 with `break'.	*/
		...
		case charset:
			...
			if(CHARSET_RANGE_TABLE_EXISTS_P(&p[-2]) && match_any_multibyte_characters == false) {
...
void
    re_set_registers(bufp, regs, num_regs, starts, ends) struct re_pattern_buffer *bufp;
...
int
    re_search(bufp, string, size, startpos, range, regs) struct re_pattern_buffer *bufp;
const char *string;
...
int
    re_search_2(bufp, string1, size1, string2, size2, startpos, range, regs, stop) struct re_pattern_buffer *bufp;
...
int stop;
{
	...
	for(;;) {
	 ...
	 because that case doesn't repeat.  */
		if(anchored_start && startpos > 0) {
			if(!(bufp->newline_anchor && ((startpos <= size1 ? string1[startpos - 1]
		 ...
		 inside the loop.  */
				if(RE_TRANSLATE_P(translate)) {
					if(multibyte)
						...
						while(range > lim && !fastmap[(unsigned char)RE_TRANSLATE(translate, (unsigned char)*d)]) {
							d++;
							range--;
						}
				} else
	...
	advance:
		if(!range)
			...
			if(multibyte) {
				const unsigned char *p = (const unsigned char *)POS_ADDR_VSTRING(startpos);
				const unsigned char *pend = (const unsigned char *)STOP_ADDR_VSTRING(startpos);
...
int
    re_match(bufp, string, size, pos, regs) struct re_pattern_buffer *bufp;
const char *string;
int size, pos;
struct re_registers *regs;
{
...
int
    re_match_2(bufp, string1, size1, string2, size2, pos, regs, stop) struct re_pattern_buffer *bufp;
const char *string1, *string2;
int size1, size2;
int pos;
struct re_registers *regs;
...
/* This is a separate function so that we can force an alloca cleanup
   afterwards.	*/
static int
    re_match_2_internal(bufp, string1, size1, string2, size2, pos, regs, stop) struct re_pattern_buffer *bufp;
const char *string1, *string2;
int size1, size2;
int pos;
struct re_registers *regs;
	...
	for(;;) {
	   ...
	   are the characters to match.	 */
			...
			if(RE_TRANSLATE_P(translate)) {
				...
				if(multibyte)
					do {
						...
						if(RE_TRANSLATE(translate, buf_ch) != pat_ch)
							goto fail;
					...
					do {
						PREFETCH();
						if((unsigned char)RE_TRANSLATE(translate, (unsigned char)*d) != (unsigned char)*p++)
							goto fail;
						d++;
					} while(--mcnt);
			} else {
		...
		case anychar: {
			...
			if((!(bufp->syntax & RE_DOT_NEWLINE) && buf_ch == '\n') || ((bufp->syntax & RE_DOT_NOT_NULL) && buf_ch == '\000'))
				goto fail;
		...
		case charset_not: {
		   ...
		   case the bit list is a full 32 bytes long.  */
				if(c < (unsigned)(CHARSET_BITMAP_SIZE(&p[-1]) * BYTEWIDTH) && p[1 + c / BYTEWIDTH] & (1 << (c % BYTEWIDTH)))
					not = !not;
	   ...
	   followed by the numeric value of <digit> as the register number.  */
		case duplicate: {
	       ...
	       the end of the first string.  */
			...
			dend2 = ((FIRST_STRING_P(regstart[regno]) == FIRST_STRING_P(regend[regno]))
				     ? regend[regno]
				     : end_match_1);
			for(;;) {
	   ...
	   We change it to either `pop_failure_jump' or `jump'.	 */
		case maybe_pop_jump:
		   ...
		   yet.	 */
					...
					register unsigned int c = *p2 == (unsigned char)endline ? '\n' : p2[2];

					if((re_opcode_t)p1[3] == exactn) {
						if(!(multibyte /* && (c != '\n') */
						     && BASE_LEADING_CODE_P(c))
						       ? c != p1[5]
						       : (STRING_CHAR(&p2[2], pend - &p2[2]) != STRING_CHAR(&p1[5], pend - &p1[5]))) {
							p[-3] = (unsigned char)pop_failure_jump;
		       ...
		       at `&p1[3]'.  */
						if(SINGLE_BYTE_CHAR_P(c)) {
							if(c < CHARSET_BITMAP_SIZE(&p1[3]) * BYTEWIDTH && p1[5 + c / BYTEWIDTH] & (1 << (c % BYTEWIDTH)))
								not = !not;
			...
			that we can't change to pop_failure_jump.  */
					...
					if((re_opcode_t)p1[3] == exactn) {
						...
						if(SINGLE_BYTE_CHAR_P(c)) {
							if(c < CHARSET_BITMAP_SIZE(p2) * BYTEWIDTH && (p2[2 + c / BYTEWIDTH] & (1 << (c % BYTEWIDTH))))
								not = !not;
		   ...
		   such case.  */
			   ...
			   every character listed in the charset after.	 */
							for(idx = 0; idx < (int)p2[1]; idx++)
								if(!(p2[2 + idx] == 0 || (idx < CHARSET_BITMAP_SIZE(&p1[3]) && ((p2[2 + idx] & ~p1[5 + idx]) == 0))))
									break;
		       ...
		       has no overlap with the one after the loop.  */
							for(idx = 0;
							    (idx < (int)p2[1] && idx < CHARSET_BITMAP_SIZE(&p1[3]));
							    idx++)
								if((p2[2 + idx] & p1[5 + idx]) != 0)
									break;

							if(idx == p2[1] || idx == CHARSET_BITMAP_SIZE(&p1[3])) {
								p[-3] = (unsigned char)pop_failure_jump;
								DEBUG_PRINT1("	 No match => pop_failure_jump.\n");
							}
						}
...
static boolean
    group_match_null_string_p(p, end, reg_info) unsigned char **p,
    *end;
register_info_type *reg_info;
{
	int mcnt;
...
static boolean
    alt_match_null_string_p(p, end, reg_info) unsigned char *p,
    *end;
register_info_type *reg_info;
{
	int mcnt;
...
static boolean
    common_op_match_null_string_p(p, end, reg_info) unsigned char **p,
    *end;
register_info_type *reg_info;
{
	int mcnt;
...
static int
    bcmp_translate(s1, s2, len, translate) unsigned char *s1,
    *s2;
register int len;
...
size_t
    regerror(errcode, preg, errbuf, errbuf_size) int errcode;
const regex_t *preg;
char *errbuf;
size_t errbuf_size;
{
	...
	if(errcode < 0 || errcode >= (sizeof(re_error_msgid) / sizeof(re_error_msgid[0])))
