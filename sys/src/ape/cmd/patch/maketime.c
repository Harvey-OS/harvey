...
static int
    month_days(tm) struct tm const *tm;
{
	int m = tm->tm_mon;
	return (month_yday[m + 1] - month_yday[m] + (m == 1 && isleap(tm->tm_year + TM_YEAR_ORIGIN)));
}
...
struct tm const *b;
{
	...
	int intervening_leap_days = (((ay >> 2) - (by >> 2)) - (ac - bc) + ((ac >> 2) - (bc >> 2)));
	time_t difference_in_years = ay - by;
	time_t difference_in_days = (difference_in_years * 365 + (intervening_leap_days + difference_in_day_of_year));
	return (((((difference_in_days * 24 + (a->tm_hour - b->tm_hour)) * 60) + (a->tm_min - b->tm_min)) * 60) + (a->tm_sec - b->tm_sec));
}
 ...
 */
void
    adjzone(t, seconds) register struct tm *t;
int32_t seconds;
{
	/*
 ...
 */
time_t
    tm2time(tm, localzone) struct tm *tm;
int localzone;
{
   ...
   */
	...
	tm->tm_yday = month_yday[tm->tm_mon] + tm->tm_mday - (tm->tm_mon < 2 || !isleap(tm->tm_year + TM_YEAR_ORIGIN));
 ...
 */
static time_t
    maketime(pt, default_time) struct partime const *pt;
time_t default_time;
{
	int localzone, wday;
	struct tm tm;
...
time_t
    str2time(source, default_time, default_zone) char const *source;
time_t default_time;
