...
static struct name_val const month_names[] = {{"jan", 0},
                                              {"feb", 1},
                                              {"mar", 2},
                                              {"apr", 3},
                                              {"may", 4},
...
static struct name_val const weekday_names[] = {{"sun", 0},
                                                {"mon", 1},
                                                {"tue", 2},
                                                {"wed", 3},
                                                {"thu", 4},
...
static int lookup(s, table) char const* s;
...
static void undefine(t) struct partime* t;
{
	t->tm.tm_sec = t->tm.tm_min = t->tm.tm_hour = t->tm.tm_mday =
	    t->tm.tm_mon = t->tm.tm_year = t->tm.tm_wday = t->tm.tm_yday =
...
static char const* parse_prefix(str, t, pi) char const* str;
struct partime* t;
int* pi;
{
	int i = *pi;
...
static char const* parse_fixed(s, digits, res) char const* s;
int digits, *res;
{
	int n = 0;
...
/* Parse an initial prefix of S of length DIGITS;
   ...
   Store the parsed number into *RES.
   Return the first character after the prefix, or 0 if it wasn't parsed.  */
static char const* parse_ranged(s, digits, lo, hi, res) char const* s;
...
/* Parse an initial prefix of S of length DIGITS;
   ...
   Return the first character after the prefix, or 0 if it wasn't parsed.  */
static char const* parse_decimal(s, digits, lo, hi, resolution, res,
                                 fres) char const* s;
...
/* Parse an initial prefix of S; it must denote a time zone.
   ...
   Return the first character after the prefix, or 0 if it wasn't parsed.  */
char* parzone(s, zone) char const* s;
int32_t* zone;
{
	char sign;
	int hh, mm, ss;
...
/* Parse an initial prefix of S, matching the pattern whose code is C.
   Set *T accordingly.
   Return the first character after the prefix, or 0 if it wasn't parsed.  */
static char const* parse_pattern_letter(s, c, t) char const* s;
int c;
struct partime* t;
{
	switch(c) {
	...
	case 'm': /* minute [00-59 followed by optional fraction] */
		s = parse_decimal(s, 2, 0, 59, 60, &t->tm.tm_min,
		                  &t->tm.tm_sec);
		break;
...
static int merge_partime(t, u) struct partime* t;
...
#define conflict(a, b) ((a) != (b) && TM_DEFINED(a) && TM_DEFINED(b))
	if(conflict(t->tm.tm_sec, u->tm.tm_sec) ||
	   ...
	   conflict(t->ymodulus, u->ymodulus) || conflict(t->yweek, u->yweek) ||
	   (t->zone != u->zone && t->zone != TM_UNDEFINED_ZONE &&
...
#define merge_(a, b)                                                           \
	if(TM_DEFINED(b))                                                      \
		(a) = (b);
	merge_(t->tm.tm_sec, u->tm.tm_sec) merge_(t->tm.tm_min, u->tm.tm_min)
