...
int8_t* find_backup_file_name(file) const int8_t* file;
{
	...
	s = malloc(file_len + backup_suffix_size_max +
	...
	if(s) {
		...
		if(backup_type != simple) {
			...
			if(!(backup_type == numbered_existing &&
			     highest_backup == 0)) {
				int8_t* numbered_suffix =
				    s + (file_len + backup_suffix_size_max);
				sprintf(numbered_suffix, ".~%d~",
				        highest_backup + 1);
...
static int max_backup_version(file, dir) const char* file;
const char* dir;
{
	DIR* dirp;
	struct dirent* dp;
...
static int version_number(base, backup, base_length) const char* base;
const char* backup;
size_t base_length;
{
	int version;
	...
	if(strncmp(base, backup, base_length) == 0 &&
	   backup[base_length] == '.' && backup[base_length + 1] == '~') {
...
enum backup_type get_version(version) const char* version;
{
	int i;
