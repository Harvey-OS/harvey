...
void open_patch_file(filename) char const* filename;
{
	...
	if(!filename || !*filename || strEQ(filename, "-")) {
		...
		if(binary_transput) {
			if(isatty(STDIN_FILENO))
				fatal("cannot read binary data from tty on "
				      "this platform");
...
there_is_another_patch()
{
	...
	if(diff_type == NO_DIFF) {
		if(verbosity == VERBOSE)
			say(p_base ? "  Ignoring the trailing garbage.\ndone\n"
	...
	if(verbosity != SILENT) {
		...
		if(!inname) {
			say("can't find file to patch at input line %ld\n",
			    p_sline);
			say(strippath == INT_MAX ? "Perhaps you should have "
			                           "used the -p or --strip "
			                           "option?\n"
...
intuit_diff_type()
{
	...
	for(;;) {
		...
		if(!stars_last_line && strnEQ(s, "*** ", 4))
			name[OLD] =
			    fetchname(s + 4, strippath, &p_timestamp[OLD]);
		else if(strnEQ(s, "+++ ", 4))
			/* Swap with NEW below.  */
			name[OLD] =
			    fetchname(s + 4, strippath, &p_timestamp[OLD]);
			...
			if(strnEQ(t, "--- ", 4)) {
				...
				name[NEW] =
				    fetchname(t + 4, strippath, &timestamp);
		...
		if((diff_type == NO_DIFF || diff_type == CONTEXT_DIFF ||
		    diff_type == NEW_CONTEXT_DIFF) &&
		   stars_last_line && strnEQ(s, "*** ", 4)) {
			...
			retval =
			    (*(s - 1) == '*' ? NEW_CONTEXT_DIFF : CONTEXT_DIFF);

			{
				/* Scan the first hunk to see whether the file
				   contents
				...
				p_input_line -= 2;
				if(another_hunk(retval, 0) && !p_repl_lines &&
				   p_newfirst == 1)
...
scan_exit:
	...
	if(!inname) {
		...
		if(!posixly_correct && (name[OLD] || name[NEW]) &&
		   name[INDEX]) {
			free(name[INDEX]);
			name[INDEX] = 0;
		}
		...
		for(i = OLD; i <= INDEX; i++)
			if(name[i]) {
				if(i0 != NONE &&
				   strcmp(name[i0], name[i]) == 0) {
					/* It's the same name as before; reuse
					 * stat results.  */
		...
		if(!posixly_correct) {
			...
			if(p_says_nonexistent[reverse ^ (i == NONE ||
				...
				assert(i0 != NONE);
				if(ok_to_reverse(
				       "The next patch%s would %s the file "
				       "`%s',\nwhich %s!",
...
static int prefix_components(filename, checkdirs) char* filename;
int checkdirs;
{
	int count = 0;
...
static enum nametype best_name(name, ignore) char* const* name;
int const* ignore;
{
	enum nametype i;
	int components[3];
	...
	for(i = OLD; i <= INDEX; i++)
		if(name[i] && !ignore[i] && components[i] == components_min &&
		   basename_len[i] == basename_len_min && len[i] == len_min)
			break;
...
static void next_intuit_at(file_pos, file_line) file_offset file_pos;
LINENUM file_line;
{
	p_base = file_pos;
	p_bline = file_line;
...
static void skip_to(file_pos, file_line) file_offset file_pos;
LINENUM file_line;
{
	register FILE* i = pfp;
	...
	if((verbosity == VERBOSE || !inname) && p_base < file_pos) {
		...
		say("The text leading up to this "
		    "was:\n--------------------------\n");
...
int another_hunk(difftype, rev) enum diff difftype;
int rev;
{
	register char* s;
	...
	if(difftype == CONTEXT_DIFF || difftype == NEW_CONTEXT_DIFF) {
		...
		bool ptrn_spaces_eaten =
		    FALSE;                 /* ptrn was slightly misformed */
		...
		LINENUM ptrn_prefix_context =
		    -1; /* lines in pattern prefix context */
		LINENUM ptrn_suffix_context =
		    -1; /* lines in pattern suffix context */
		LINENUM repl_prefix_context =
		    -1; /* lines in replac. prefix context */
		...
		chars_read = get_line();
		if(chars_read == (size_t)-1 || chars_read <= 8 ||
		   strncmp(buf, "********", 8) != 0) {
		...
		while(p_end < p_max) {
			...
			p_end++;
			if(p_end == hunkmax)
				fatal("unterminated hunk starting at line %ld; "
				      "giving up at line %ld: %s",
				...
				if(repl_beginning ||
				   (p_end !=
					...
					if(p_end == 1) {
						...
						fatal("duplicate `---' at line "
						      "%ld; check line numbers "
						      "at line %ld",
						      p_input_line,
						      p_hunk_beg +
						          repl_beginning);
	...
	hunk_done:
		...
		if(fillcnt) {
			...
			while(fillcnt-- > 0) {
				...
				if(p_end < fillsrc || fillsrc == repl_beginning)
					fatal("replacement text or line "
					      "numbers mangled in hunk at line "
					      "%ld",
					      p_hunk_beg);
			...
			if(debug & 64)
				printf("fillsrc %ld, filldst %ld, rb %ld, e+1 "
				       "%ld\n",
				       fillsrc, filldst, repl_beginning,
				       p_end + 1);
			assert(fillsrc == p_end + 1 ||
			       fillsrc == repl_beginning);
			assert(filldst == p_end + 1 ||
		...
		chars_read = get_line();
		if(chars_read == (size_t)-1 || chars_read <= 4 ||
		   strncmp(buf, "@@ -", 4) != 0) {
			next_intuit_at(line_beginning, p_input_line);
			return chars_read == (size_t)-1 ? -1 : 0;
		}
		...
		while(fillsrc <= p_ptrn_lines || filldst <= p_end) {
			...
			case '-':
				...
				chars_read -= fillsrc == p_ptrn_lines &&
				              incomplete_line();
				p_Char[fillsrc] = ch;
				p_line[fillsrc] = s;
		...
		for(i = 1; i <= p_ptrn_lines; i++) {
			...
			if(!chars_read)
				fatal("unexpected end of file in patch at line "
				      "%ld",
				      p_input_line);
			if(buf[0] != '<' || (buf[1] != ' ' && buf[1] != '\t'))
				fatal("`<' expected at line %ld of patch",
				      p_input_line);
			chars_read -=
			    2 + (i == p_ptrn_lines && incomplete_line());
			p_len[i] = chars_read;
...
static size_t pget_line(indent, rfc934_nesting) int indent;
int rfc934_nesting;
{
...
bool pch_says_nonexistent(which) int which;
{
...
time_t pch_timestamp(which) int which;
{
...
LINENUM
pch_first() { return p_first; }

/* Return the number of lines of old context. */

LINENUM
pch_ptrn_lines() { return p_ptrn_lines; }

/* Return the probable line position in the new file of the first line. */

LINENUM
pch_newfirst() { return p_newfirst; }

/* Return the number of lines in the replacement text including context. */

LINENUM
pch_repl_lines() { return p_repl_lines; }

/* Return the number of lines in the whole hunk. */

LINENUM
pch_end() { return p_end; }

/* Return the number of context lines before the first changed line. */

LINENUM
pch_prefix_context() { return p_prefix_context; }

/* Return the number of context lines after the last changed line. */

LINENUM
pch_suffix_context() { return p_suffix_context; }

/* Return the length of a particular patch line. */

size_t pch_line_len(line) LINENUM line;
{
	return p_len[line];
}
...
char pch_char(line) LINENUM line;
{
	return p_Char[line];
}
...
bool pch_write_line(line, file) LINENUM line;
FILE* file;
{
...
LINENUM
pch_hunk_beg() { return p_hunk_beg; }

/* Apply an ed script by feeding ed itself. */

void do_ed_script(ofp) FILE* ofp;
{
	...
	for(;;) {
		...
		this_line_is_command =
		    (ISDIGIT(*buf) && (*t == 'd' || *t == 'c' || *t == 'a' ||
		                       *t == 'i' || *t == 's'));
		if(this_line_is_command) {
			if(pipefp)
				if(!fwrite(buf, sizeof *buf, chars_read,
				           pipefp))
					write_fatal();
			if(*t != 'd' && *t != 's') {
