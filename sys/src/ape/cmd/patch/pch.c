...
void
    open_patch_file(filename) char const *filename;
{
	...
	if(!filename || !*filename || strEQ(filename, "-")) {
		...
		if(S_ISREG(st.st_mode) && (stdin_pos = file_tell(stdin)) != -1) {
			...
			if(ferror(stdin) || fclose(stdin) != 0)
				read_fatal();
			if(fflush(pfp) != 0 || file_seek(pfp, (file_offset)0, SEEK_SET) != 0)
				write_fatal();
		}
	} else {
...
grow_hunkmax()
{
	...
	if((p_line = (char **)realloc(p_line, hunkmax * sizeof(*p_line))) && (p_len = (size_t *)realloc(p_len, hunkmax * sizeof(*p_len))) && (p_Char = realloc(p_Char, hunkmax * sizeof(*p_Char))))
		return TRUE;
	if(!using_plan_a)
		memory_fatal();
...
there_is_another_patch()
{
	...
	if(verbosity == VERBOSE)
		say("  %sooks like %s to me...\n",
		    ...
		    diff_type == UNI_DIFF ? "a unified diff" : diff_type == CONTEXT_DIFF ? "a context diff" : diff_type == NEW_CONTEXT_DIFF ? "a new-style context diff" : diff_type == NORMAL_DIFF ? "a normal diff" : "an ed script");
...
intuit_diff_type()
{
	...
	for(;;) {
		...
		if((diff_type == NO_DIFF || diff_type == UNI_DIFF) && strnEQ(s, "@@ -", 4)) {
			...
			p_sline = p_input_line;
			retval = UNI_DIFF;
			if(!((name[OLD] || !p_timestamp[OLD]) && (name[NEW] || !p_timestamp[NEW])))
				say("missing header for unified diff at line %ld of patch\n",
				    p_sline);
			goto scan_exit;
		}
		stars_this_line = strnEQ(s, "********", 8);
		if((diff_type == NO_DIFF || diff_type == CONTEXT_DIFF || diff_type == NEW_CONTEXT_DIFF) && stars_last_line && strnEQ(s, "*** ", 4)) {
			s += 4;
			if(!atol(s))
				p_says_nonexistent[OLD] = 1 + !p_timestamp[OLD];
		 ...
		 appear to have been deleted.  */
				...
				p_input_line -= 2;
				if(another_hunk(retval, 0) && !p_repl_lines && p_newfirst == 1)
					p_says_nonexistent[NEW] = 1 + !p_timestamp[NEW];
	...
	if(!inname) {
		...
		if(!posixly_correct) {
				...
				for(i = OLD; i <= INDEX; i++)
					if(name[i]) {
						...
						if(nope == NONE || strcmp(name[nope], name[i]) != 0) {
							cs = (version_controller(name[i], readonly, (struct stat *)0,
										 &getbuf, &diffbuf));
			...
			if(p_says_nonexistent[reverse ^ (i == NONE || st[i].st_size == 0)]) {
				assert(i0 != NONE);
				if(ok_to_reverse("The next patch%s would %s the file `%s',\nwhich %s!",
			...
			if(i == NONE && p_says_nonexistent[reverse]) {
				...
				for(i = OLD; i <= INDEX; i++)
					if(name[i]) {
						newdirs[i] = (prefix_components(name[i], 0) - prefix_components(name[i], 1));
...
static int
    prefix_components(filename, checkdirs) char *filename;
int checkdirs;
{
	int count = 0;
...
static enum nametype
    best_name(name, ignore) char *const *name;
int const *ignore;
{
	enum nametype i;
	int components[3];
	...
	for(i = OLD; i <= INDEX; i++)
		if(name[i] && !ignore[i] && components[i] == components_min && basename_len[i] == basename_len_min && len[i] == len_min)
			break;
...
int
    another_hunk(difftype, rev) enum diff difftype;
int rev;
{
	register char *s;
	...
	if(difftype == CONTEXT_DIFF || difftype == NEW_CONTEXT_DIFF) {
		...
		chars_read = get_line();
		if(chars_read == (size_t)-1 || chars_read <= 8 || strncmp(buf, "********", 8) != 0) {
		...
		while(p_end < p_max) {
			...
			case '-':
			   ...
			   them in from `new' context lines.  */
				...
				while(p_max >= hunkmax)
					if(!grow_hunkmax())
						return -1;
				if(p_repl_lines != ptrn_copiable && (p_prefix_context != 0 || context != 0 || p_repl_lines != 1))
			...
			change_line:
				...
				chars_read -=
				    (1 < chars_read && p_end == (repl_beginning ? p_max : p_ptrn_lines) && incomplete_line());
	...
	hunk_done:
		...
		p_prefix_context = ((repl_prefix_context == -1 || (ptrn_prefix_context != -1 && ptrn_prefix_context < repl_prefix_context))
					? ptrn_prefix_context
					: repl_prefix_context);
		p_suffix_context = ((ptrn_suffix_context != -1 && ptrn_suffix_context < context)
					? ptrn_suffix_context
					: context);
		assert(p_prefix_context != -1 && p_suffix_context != -1);

		if(difftype == CONTEXT_DIFF && (fillcnt || (p_first > 1 && p_prefix_context + p_suffix_context < ptrn_copiable))) {
			if(verbosity == VERBOSE)
				say("%s\n%s\n%s\n",
		...
		chars_read = get_line();
		if(chars_read == (size_t)-1 || chars_read <= 4 || strncmp(buf, "@@ -", 4) != 0) {
			next_intuit_at(line_beginning, p_input_line);
			return chars_read == (size_t)-1 ? -1 : 0;
		}
		s = buf + 4;
...
static size_t
    pget_line(indent, rfc934_nesting) int indent;
int rfc934_nesting;
{
...
bool
    pch_says_nonexistent(which) int which;
{
	...
	FILE *ofp;
{
	...
	if(!pipefp)
		return;
	if(fwrite("w\nq\n", sizeof(char), (size_t)4, pipefp) == 0 || fflush(pipefp) != 0)
		write_fatal();
	if(pclose(pipefp) != 0)
