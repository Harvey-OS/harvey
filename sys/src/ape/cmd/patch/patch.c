...
char** argv;
{
	...
	for(open_patch_file(patchname); there_is_another_patch();
		...
		if(diff_type == ED_DIFF) {
			...
			while(0 <
				...
				LINENUM context =
				    (prefix_context < suffix_context
				         ...
				         : prefix_context);
				LINENUM mymaxfuzz =
				    (maxfuzz < context ? maxfuzz : context);
				hunk++;
				if(!skip_rest_of_patch) {
					do {
					...
					if(verbosity == VERBOSE)
						say("Hunk #%d ignored at "
				          ...
				          (where == 1 &&
				           ...
				           instat.st_size)) {
					if(where)
						say("Patch attempted to create "
						    "file `%s', which already "
						    "exists.\n",
						    inname);
					abort_hunk();
					failed++;
			...
			if(got_hunk < 0 && using_plan_a) {
				...
				say("\n\nRan out of memory using Plan A -- "
				    "trying again...\n\n");
				if(outstate.ofp) {
					fclose(outstate.ofp);
					outstate.ofp = 0;
			   ...
			   (remove_empty_files ||
				...
				if(verbosity == VERBOSE)
					say("Removing file `%s'%s.\n", outname,
					    dry_run ? " and any empty ancestor "
					              "directories"
					            : "");
				if(!dry_run) {
				...
				if(!dry_run) {
					...
					if(!inerrno &&
					   chmod(outname, instat.st_mode) != 0)
						pfatal("can't set permissions "
						       "on file `%s'",
						       outname);
				}
			}
		...
		if(diff_type != ED_DIFF) {
			...
			if(failed) {
				somefailed = TRUE;
				say("%d out of %d hunk%s %s", failed, hunk,
				    "s" + (hunk == 1),
				    skip_rest_of_patch ? "ignored" : "FAILED");
				if(outname) {
					char* rej = rejname;
					if(!rejname) {
						rej = xmalloc(strlen(outname) +
						              5);
						strcpy(rej, outname);
						addext(rej, ".rej", '#');
					}
...
static char const* const option_help[] = {
    ...
    "matching.",
    "  -l  --ignore-whitespace  Ignore white space changes between patch and "
    "input.",
    ...
    "applied.",
    "  -R  --reverse  Assume patches were created with old and new files "
    "swapped.",
    ...
    "patching.",
    "", "  -Z  --set-utc  Set times of patched files, assuming diff uses UTC "
        "(GMT).",
    ...
    "  -g NUM  --get=NUM  Get files from RCS or SCCS if positive; ask if "
    "negative.",
    "", "Miscellaneous options:", "", "  -t  --batch  Ask no questions; skip "
                                      "bad-Prereq patches; assume reversed.",
    "  -f  --force  Like -t, but ignore bad-Prereq patches, and assume "
    "unreversed.",
    "  -s  --quiet  --silent  Work silently unless an error occurs.",
    "  --verbose  Output extra information about the work being done.",
    "  --dry-run  Do not actually change any files; just print what would "
    "happen.",
    ...
    "platform).",
#endif
    "", "  -v  --version  Output version info.", "  --help  Output this help.",
    "", "Report bugs to <bug-gnu-utils@prep.ai.mit.edu>.", 0};

static void usage(stream, status) FILE* stream;
int status;
{
	char const* const* p;
	...
	while((optc = getopt_long(Argc, Argv, shortopts, longopts, (int*)0)) !=
		...
		case 'b':
			...
			/* Special hack for backward compatibility with CVS 1.9.
			   If the last 4 args are `-b SUFFIX ORIGFILE
			   PATCHFILE',
		...
		case 'd':
			if(chdir(optarg) < 0)
				pfatal("can't change directory to `%s'",
				       optarg);
			break;
		case 'D':
		...
		case 'g':
			patch_get =
			    numeric_string(optarg, 1, "get option value");
			break;
		case 'i':
		...
		case 'o':
			if(strcmp(optarg, "-") == 0)
				fatal(
				    "can't output patches to standard output");
...
static int numeric_string(string, negative_allowed, argtype_msgid) char const
    * string;
...
static LINENUM locate_hunk(fuzz) LINENUM fuzz;
{
	...
	LINENUM suffix_context = pch_suffix_context();
	LINENUM context =
	    (prefix_context < suffix_context ? suffix_context : prefix_context);
	...
	LINENUM min_where =
	    last_frozen_line + 1 - (prefix_context - prefix_fuzz);
	LINENUM max_pos_offset = max_where - first_guess;
	LINENUM max_neg_offset = first_guess - min_where;
	LINENUM max_offset =
	    (max_pos_offset < max_neg_offset ? max_neg_offset : max_pos_offset);
	...
	if(prefix_fuzz < 0) {
		...
		if(suffix_fuzz < 0)
			...
			if(pat_lines != input_lines ||
			   prefix_context < last_frozen_line)
				return 0;
	...
	for(offset = 0; offset <= max_offset; offset++) {
		...
		if(0 < offset && offset <= max_neg_offset &&
		   patch_match(first_guess, -offset, prefix_fuzz,
		               suffix_fuzz)) {
			if(debug & 1)
				say("Offset changing from %ld to %ld\n",
				    last_offset, -offset);
...
static bool apply_hunk(outstate, where) struct outstate* outstate;
LINENUM where;
{
	...
	while(old <= lastline) {
		if(pch_char(old) == '-') {
			...
			fatal("Out-of-sync patch, lines %ld,%ld -- mangled "
			...
			if(R_do_defines) {
				fprintf(fp,
				        outstate->after_newline + else_defined);
				if(ferror(fp))
					write_fatal();
				def_state = IN_ELSE;
			}
			...
			if(R_do_defines && def_state != OUTSIDE) {
				fprintf(fp,
				        outstate->after_newline + end_defined,
				        R_do_defines);
				if(ferror(fp))
					write_fatal();
	...
	if(new <= pat_end && pch_char(new) == '+') {
		...
		if(R_do_defines) {
			if(def_state == OUTSIDE) {
				fprintf(fp,
				        outstate->after_newline + if_defined,
				        R_do_defines);
				def_state = IN_IFDEF;
...
static FILE* create_output_file(name) char const* name;
{
...
static void init_output(name, outstate) char const* name;
struct outstate* outstate;
{
...
static void init_reject(name) char const* name;
{
...
static bool copy_till(outstate, lastline) register struct outstate* outstate;
...
static bool spew_output(outstate) struct outstate* outstate;
{
	if(debug & 256)
...
static bool patch_match(base, offset, prefix_fuzz, suffix_fuzz) LINENUM base;
LINENUM offset;
LINENUM prefix_fuzz;
LINENUM suffix_fuzz;
{
	...
	for(iline = base + offset + prefix_fuzz; pline <= pat_lines;
	    pline++, iline++) {
		p = ifetch(iline, offset >= 0, &size);
		if(canonicalize) {
			if(!similar(p, size, pfetch(pline),
			            pch_line_len(pline)))
				return FALSE;
...
static bool similar(a, alen, b, blen) register char const* a;
...
static char const* make_temp(letter) int letter;
{
	char* r;
#if HAVE_MKTEMP
