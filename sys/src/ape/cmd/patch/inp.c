...
static void report_revision(found_revision) int found_revision;
{
	if(found_revision) {
		...
		if(verbosity != SILENT)
			say("Warning: this file doesn't appear to be the %s "
			    "version -- patching anyway.\n",
			    revision);
	} else if(batch) {
		fatal("This file doesn't appear to be the %s version -- "
		      "aborting.",
		      revision);
	} else {
		ask("This file doesn't appear to be the %s version -- patch "
		    "anyway? [n] ",
		    revision);
		if(*buf != 'y')
			fatal("aborted");
...
static void too_many_lines(filename) char const* filename;
{
	fatal("File `%s' has too many lines.", filename);
}

void get_input_file(filename, outname) char const* filename;
char const* outname;
{
	...
	if(patch_get && invc != 0 &&
	   ...
	   (invc =
	        !!(cs = (version_controller(filename, elsewhere,
	                                    inerrno ? (struct stat*)0 : &instat,
	                                    &getbuf, &diffbuf))))) {

		if(!inerrno) {
			if(!elsewhere &&
			   (instat.st_mode & (S_IWUSR | S_IWGRP | S_IWOTH)) !=
			       0)
				/* Somebody can write to it.  */
				fatal("file `%s' seems to be locked by "
				      "somebody else under %s",
				      filename, cs);
			/* It might be checked out unlocked.  See if it's safe
			   to
			   check out the default version locked.  */
			if(verbosity == VERBOSE)
				say("Comparing file `%s' to default %s "
				    "version...\n",
				    filename, cs);
			if(systemic(diffbuf) != 0) {
				say("warning: patching file `%s', which does "
				    "not match default %s version\n",
				    filename, cs);
				cs = 0;
			}
		}
...
static bool plan_a(filename) char const* filename;
{
	...
	if(size) {
		...
		while(size - buffered != 0) {
			...
			if(n == 0) {
				/* Some non-POSIX hosts exaggerate st_size in
				   text mode;
	...
	lim = buffer + size;
	iline =
	    3; /* 1 unused, 1 for SOF, 1 for EOF if last line is incomplete */
	...
	if(revision) {
		...
		if(revlen <= size) {
			...
			for(s = buffer;
			    (s = (char*)memchr(s, rev0, limrev - s)); s++)
				if(memcmp(s, rev, revlen) == 0 &&
				   (s == buffer ||
				    ISSPACE((unsigned char)s[-1])) &&
				   (s + 1 == limrev ||
				    ISSPACE((unsigned char)s[revlen]))) {
					found_revision = 1;
					break;
				}
		}
...
static void plan_b(filename) char const* filename;
{
	register FILE* ifp;
	register int c;
	register size_t len;
...
char const* ifetch(line, whichbuf, psize) register LINENUM line;
	...
	if(using_plan_a) {
		...
		*psize = i_ptr[line + 1] - p;
		...
		if(tiline[0] == baseline)
			...
			if(lseek(tifd,
			         (off_t)(baseline / lines_per_buf * tibufsize),
			         SEEK_SET) == -1 ||
