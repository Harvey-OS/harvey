...
void
    scan_input(filename) char *filename;
{
...
static void
    report_revision(found_revision) int found_revision;
{
	if(found_revision) {
...
static void
    too_many_lines(filename) char const *filename;
{
	fatal("File `%s' has too many lines.", filename);
}

void
    get_input_file(filename, outname) char const *filename;
char const *outname;
{
	...
	if(patch_get && invc != 0 && (inerrno || (!elsewhere && (/* No one can write to it.  */
								 (instat.st_mode & (S_IWUSR | S_IWGRP | S_IWOTH)) == 0
								 /* Only the owner (who's not me) can write to it.  */
								 ||
								 ((instat.st_mode & (S_IWGRP | S_IWOTH)) == 0 && instat.st_uid != geteuid())))) &&
	   (invc = !!(cs = (version_controller(filename, elsewhere,
					       inerrno ? (struct stat *)0 : &instat,
					       &getbuf, &diffbuf))))) {

		if(!inerrno) {
			if(!elsewhere && (instat.st_mode & (S_IWUSR | S_IWGRP | S_IWOTH)) != 0)
...
static bool
    plan_a(filename) char const *filename;
{
	...
	if(!(size == instat.st_size && (buffer = malloc(size ? size : (size_t)1))))
		return FALSE;
	...
	if(!(iline == (size_t)iline && (size_t)iline *sizeof *ptr / sizeof *ptr == (size_t)iline && (ptr = (char const **)malloc((size_t)iline * sizeof *ptr)))) {
		free(buffer);
		return FALSE;
	}
	iline = 0;
	...
	if(revision) {
		...
		if(revlen <= size) {
			...
			for(s = buffer; (s = (char *)memchr(s, rev0, limrev - s)); s++)
				if(memcmp(s, rev, revlen) == 0 && (s == buffer || ISSPACE((unsigned char)s[-1])) && (s + 1 == limrev || ISSPACE((unsigned char)s[revlen]))) {
					found_revision = 1;
					break;
				}
		}
...
static void
    plan_b(filename) char const *filename;
{
	register FILE *ifp;
	register int c;
	register size_t len;
...
char const *
    ifetch(line, whichbuf, psize) register LINENUM line;
