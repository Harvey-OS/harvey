...
uint8_t bittab[] = {1, 2, 4, 8, 16, 32, 64, 128};
	...
	swit:
		...
		case 's':
			...
			if(*cp == 'w') {
				...
				if(nfiles >= MAXFILES) {
					fprintf(stderr, "sed: Too many files "
					                "in w commands 1 \n");
					exit(2);
				}
		...
		case 'w':
			...
			if((rep->r1.fcode = fopen(fname[nfiles], "w")) ==
			   NULL) {
