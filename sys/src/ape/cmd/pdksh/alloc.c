...
static void* asplit
ARGS((Area * ap, Block* bp, Cell* fp, Cell* fpp, int cells));

/* create empty Area */
Area* ainit(ap) register Area* ap;
{
	ap->freelist = &aempty;
	ACHECK(ap);
	return ap;
...
void afreeall(ap) register Area* ap;
{
	register Block* bp;
	register Block* tmp;
...
void* alloc(size, ap) size_t size;
register Area* ap;
{
	int cells, acells;
	Block* bp = 0;
	...
	if(cells <= ICELLS) {
		...
		for(bp = ap->freelist;; bp = bp->next) {
			for(fpp = NULL, fp = bp->freelist; fp != bp->last;
			    fpp = fp, fp = fp->next) {
				if((fp - 1)->size >= cells)
					goto Found;
			}
 ...
 */
static void* asplit(ap, bp, fp, fpp, cells) Area* ap;
Block* bp;
Cell* fp;
Cell* fpp;
int cells;
...
void* aresize(ptr, size, ap) register void* ptr;
size_t size;
Area* ap;
{
	int cells;
	...
	if(dp && cells > oldcells && cells <= ICELLS) {
		...
		for(fpp = NULL, fp = bp->freelist;
		    fp != bp->last && dp + oldcells + NOBJECT_FIELDS <= fp;
		    fpp = fp, fp = fp->next) {
...
static void ablockfree(bp, ap) Block* bp;
Area* ap;
{
	/* NOTE: If this code changes, similar changes may be
...
void acheck(ap) Area* ap;
{
	...
	while(1) {
		...
		for(dp = &bp->cell[NOBJECT_FIELDS]; dp != bp->last;) {
			...
			if((dp - 1)->size > ICELLS && !isfree &&
			   (dp != &bp->cell[NOBJECT_FIELDS] ||
			    dp + (dp - 1)->size != bp->last)) {
				shellf("acheck: big cell doesn't make up whole "
				       "block\n");
				ok = 0;
			}
			if(isfree) {
				if(dp->next <= dp) {
					shellf("acheck: free fragment's next "
					       "<= self\n");
					ok = 0;
				}
				if(dp->next > bp->last) {
			...
			if(dptmp > bp->last) {
				...
				if(dptmp > bp->last) {
					shellf("acheck: next frag just out of "
					       "range\n");
					ok = 0;
					break;
				}
...
void aprint(ap, ptr, size) register Area* ap;
void* ptr;
size_t size;
{
	Block* bp;
	...
	if(!ap)
		...
		for(i = 0;; i++) {
			...
			if(bp->prev != bpp || bp->next->prev != bp)
				shellf("aprint: BAD prev pointer: bp %p, "
				       "bp->prev %p, bp->next %p, bpp=%p\n",
				       bp, bp->prev, bp->next, bpp);
			shellf("aprint: block %2d (p=%p,%p,n=%p): 0x%p .. 0x%p "
			       "(%ld)\n",
			       i, bp->prev, bp, bp->next, bp->cell, bp->last,
			       (int32_t)((char*)bp->last - (char*)bp->cell));
			fp = bp->freelist;
			if(bp->last <= bp->cell + NOBJECT_FIELDS)
				shellf("aprint: BAD bp->last too small: %p <= "
				       "%p\n",
				       bp->last, bp->cell + NOBJECT_FIELDS);
			if(bp->freelist < bp->cell + NOBJECT_FIELDS ||
			   bp->freelist > bp->last)
				shellf("aprint: BAD bp->freelist %p out of "
				       "range: %p .. %p\n",
				       bp->freelist, bp->cell + NOBJECT_FIELDS,
				       bp->last);
			for(dp = bp->cell; dp != bp->last;) {
				...
				if((dp - 2)->block != bp)
					shellf("aprint: BAD dp->block %p != bp "
					       "%p\n",
					       (dp - 2)->block, bp);
				if(dp > bp->last)
					shellf("aprint: BAD dp gone past "
					       "block: %p > %p\n",
					       dp, bp->last);
				if(dp > fp)
					shellf("aprint: BAD dp gone past free: "
					       "%p > %p\n",
					       dp, fp);
				if(dp == fp) {
					fp = fp->next;
					if(fp < dp || fp > bp->last)
						shellf("aprint: BAD free "
						       "object %p out of "
						       "range: %p .. %p\n",
						       fp, dp, bp->last);
				}
