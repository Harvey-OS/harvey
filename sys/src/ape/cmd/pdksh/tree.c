...
static void vfptreef
ARGS((struct shf * shf, int indent, const char* fmt, va_list va));
...
static void ptree(t, indent, shf) register struct op* t;
int indent;
...
Chain:
	...
	case TOR:
	case TAND:
		fptreef(shf, indent, "%T%s %T", t->left,
		        (t->type == TOR) ? "||" : "&&", t->right);
		break;
	case TBANG:
		fptreef(shf, indent, "! ");
...
/* fall through */
	...
	case TUNTIL:
		...
		fptreef(shf, indent + 6, "%s %T",
		        (t->type == TWHILE) ? "while" : "until", t->left);
	...
	case TFUNCT:
		fptreef(shf, indent,
		        t->u.ksh_func ? "function %s %T" : "%s() %T", t->str,
		        t->left);
		break;
	case TTIME:
...
static void pioact(shf, indent, iop) register struct shf* shf;
int indent;
register struct ioword* iop;
{
	...
	expected =
	    (type == IOREAD || type == IORDWR || type == IOHERE)
	        ? 0
	        : (type == IOCAT || type == IOWRITE)
	              ? 1
...
static void tputC(c, shf) register int c;
register struct shf* shf;
{
	if((c & 0x60) == 0) {/* C0|C1 */
		tputc((c & 0x80) ? '$' : '^', shf);
...
static void tputS(wp, shf) register char* wp;
register struct shf* shf;
{
...
#else
    fptreef(shf, indent, fmt, va_alist) struct shf* shf;
int indent;
const char* fmt;
va_dcl
#endif
...
#else
    snptreef(s, n, fmt, va_alist) char* s;
int n;
const char* fmt;
va_dcl
#endif
...
static void vfptreef(shf, indent, fmt, va) register struct shf* shf;
int indent;
const char* fmt;
register va_list va;
{
...
struct op* tcopy(t, ap) register struct op* t;
Area* ap;
{
	...
	if(t->vars == NULL)
		...
		for(tw = t->vars; *tw++ != NULL;)
			;
		rw = r->vars =
		    (char**)alloc((int)(tw - t->vars) * sizeof(*tw), ap);
		for(tw = t->vars; *tw != NULL;)
			*rw++ = wdcopy(*tw++, ap);
		*rw = NULL;
	}
	...
	if(t->args == NULL)
		...
		for(tw = t->args; *tw++ != NULL;)
			;
		rw = r->args =
		    (char**)alloc((int)(tw - t->args) * sizeof(*tw), ap);
		for(tw = t->args; *tw != NULL;)
			*rw++ = wdcopy(*tw++, ap);
		*rw = NULL;
	}
...
char* wdscan(wp, c) register const char* wp;
register int c;
{
	register int nest = 0;
	...
	while(1)
		...
		default:
			internal_errorf(
			    0, "wdscan: unknown char 0x%x (carrying on)",
			    wp[-1]);
		}
}
...
static struct ioword** iocopy(iow, ap) register struct ioword** iow;
Area* ap;
{
...
void tfree(t, ap) register struct op* t;
Area* ap;
{
	register char** w;
