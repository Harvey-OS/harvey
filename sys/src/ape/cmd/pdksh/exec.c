...
static int comexec
ARGS((struct op * t, struct tbl* volatile tp, char** ap, int volatile flags));
...
static const char* dbteste_getopnd
...
int volatile flags;
{
	...
	while(tp && tp->type == CSHELL) {
		fcflags =
		    FC_BI | FC_FUNC | FC_PATH; /* undo effects of command */
	...
	case CFUNC: /* function call */
		...
		if(!(tp->flag & ISSET)) {
			...
			if(include(tp->u.fpath, 0, (char**)0, 0) < 0) {
				warningf(TRUE, "%s: can't open function "
				               "definition file %s - %s",
			...
			if(!(ftp = findfunc(cp, hash(cp), FALSE)) ||
			   !(ftp->flag & ISSET)) {
				warningf(TRUE, "%s: function not defined by %s",
				         cp, tp->u.fpath);
				rv = 127;
				break;
			}
...
static void scriptexec(tp, ap) register struct op* tp;
register char** ap;
{
	char* shell;
		   ...
		   || (strncmp(buf, "extproc", 7) == 0 && isspace(buf[7]) &&
			...
			if(*cp && *cp != '\n') {
				...
				while(*cp && *cp != '\n' && *cp != ' ' &&
				      *cp != '\t') {
#ifdef OS2
					/* Allow shell search without prepended
					 * path
					 * if shell with / in pathname cannot be
					 * found.
					 * Use / explicitly so \ can be used if
					 * explicit
					 * needs to be forced.
					 */
					if(*cp == '/')
...
#ifdef OS2
					if(a0 != a2) {
						char* tmp_a0 = str_nsave(
						    a0, strlen(a0) + 5, ATEMP);
...
int shcomexec(wp) register char** wp;
{
	...
	if(!tp && (flags & FC_FUNC)) {
		...
		if(tp && !(tp->flag & ISSET)) {
			if((fpath = str_val(global("FPATH"))) == null) {
				...
				tp->u.fpath =
				    search(name, fpath, R_OK, &tp->u2.errno_);
		}
	}
...
Search:
	if((!tp || (tp->type == CTALIAS && !(tp->flag & ISSET))) &&
	   (flags & FC_PATH)) {
		...
		npath = search(name, flags & FC_DEFPATH ? def_path : path, X_OK,
		               &tp->u2.errno_);
		if(npath) {
			tp->val.s =
			    tp == &temp ? npath : str_save(npath, APERM);
			tp->flag |= ISSET | ALLOC;
		} else if((flags & FC_FUNC) &&
		          (fpath = str_val(global("FPATH"))) != null &&
		          (npath = search(name, fpath, R_OK, &tp->u2.errno_)) !=
		              (char*)0) {
...
int search_access(path, mode, errnop) const char* path;
int mode;
int* errnop; /* set if candidate found, but not suitable */
{
	 ...
	 *	 exec.c(search())).
	 */
	static char* xsuffixes[] = {".ksh", ".exe", ".",    ".sh",
	                            ".cmd", ".com", ".bat", (char*)0};
	static char* rsuffixes[] = {".ksh", ".",    ".sh",
	                            ".cmd", ".bat", (char*)0};
	int i;
...
#ifdef OS2
static int search_access1(path, mode, errnop) const char* path;
int mode;
int* errnop; /* set if candidate found, but not suitable */
{
	int ret, err = 0;
 ...
 */
char* search(name, path, mode, errnop) const char* name;
const char* path;
 ...
 */
static int iosetup(iop, tp) register struct ioword* iop;
struct tbl* tp;
{
	register int u = -1;
	char* cp = iop->name;
	...
	case IODUP: {
		...
		if(*cp == '-' && !cp[1]) {
			...
			do_close = 1;
		               ...
		               cp, X_OK | ((iop->flag & IORDUP) ? R_OK : W_OK),
		               &emsg)) < 0) {
			warningf(TRUE, "%s: %s",
	...
	if(u < 0) {
		...
		if(u == -1)
			warningf(TRUE, "cannot %s %s: %s",
			         iotype == IODUP ? "dup" : (iotype == IOREAD ||
			                                    iotype == IOHERE)
			                                       ? "open"
			                                       : "create",
			         cp, strerror(errno));
...
int sub;
{
	...
	if(!(shf = h->shf) || (fd = open(h->name, O_RDONLY, 0)) < 0) {
		warningf(TRUE, "can't %s temporary file %s: %s",
		         !shf ? "create" : "open", h->name, strerror(errno));
		if(shf)
			shf_close(shf);
		return -2 /* special to iosetup(): don't print error */;
	}
 ...
 */
static char* do_selectargs(ap, print_menu) register char** ap;
bool_t print_menu;
{
	static const char* const read_args[] = {"read", "-r", "REPLY",
	                                        (char*)0};
	char* s;
	int i, argct;
...
int buflen;
{
	...
	shf_snprintf(buf, buflen, "%*d) %s", smi->num_width, i + 1,
	             smi->args[i]);
	return buf;
}
