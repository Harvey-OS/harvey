 ...
 */
int
    execute(t, flags) struct op *volatile t;
volatile int flags; /* if XEXEC don't fork */
{
	...
	case TELIF:
		...
		rv = execute(t->left, XERROK) == 0 ? execute(t->right->left, flags & XERROK) : execute(t->right->right, flags & XERROK);
		break;
...
static int
    comexec(t, tp, ap, flags) struct op *t;
...
static void
    scriptexec(tp, ap) register struct op *tp;
register char **ap;
{
	char *shell;
...
#ifdef OS2
		   || (strncmp(buf, "extproc", 7) == 0 && isspace(buf[7]) && (cp = &buf[7]))
#endif /* OS2 */
		       ) {
			while(*cp && (*cp == ' ' || *cp == '\t'))
				cp++;
...
Search:
	if((!tp || (tp->type == CTALIAS && !(tp->flag & ISSET))) && (flags & FC_PATH)) {
		...
		if(npath) {
			...
			tp->flag |= ISSET | ALLOC;
		} else if((flags & FC_FUNC) && (fpath = str_val(global("FPATH"))) != null && (npath = search(name, fpath, R_OK, &tp->u2.errno_)) != (char *)0) {
 ...
 */
void
    flushcom(all) int all; /* just relative or all */
{
	struct tbl *tp;
	struct tstate ts;
...
static int
    call_builtin(tp, wp) struct tbl *tp;
char **wp;
{
	int rv;
 ...
 */
static int
    iosetup(iop, tp) register struct ioword *iop;
struct tbl *tp;
{
	register int u = -1;
	char *cp = iop->name;
	...
	case IOWRITE:
		 ...
		 */
		if(Flag(FNOCLOBBER) && !(iop->flag & IOCLOB) && (stat(cp, &statb) < 0 || S_ISREG(statb.st_mode)))
			flags |= O_EXCL;
		break;
	...
	if(u < 0) {
		...
		if(u == -1)
			warningf(TRUE, "cannot %s %s: %s",
				 iotype == IODUP ? "dup"
						 : (iotype == IOREAD || iotype == IOHERE) ? "open" : "create",
				 cp, strerror(errno));
		return -1;
	}
 ...
 */
static char *
    do_selectargs(ap, print_menu) register char **ap;
bool_t print_menu;
{
...
static char *
    select_fmt_entry(arg, i, buf, buflen) void *arg;
int i;
char *buf;
int buflen;
{
...
Test_meta meta;
{
	...
	if(meta == TM_UNOP || meta == TM_BINOP) {
		if(uqword) {
			...
			char *q = buf;
			for(p = *te->pos.wp; *p == CHAR && q < &buf[sizeof(buf) - 1];
			    p += 2)
				*q++ = p[1];
			*q = '\0';
