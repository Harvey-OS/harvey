	...
	if(func) {
		...
		for(l = e->loc; l; l = l->next) {
			for(p = tsort(&l->vars); (vp = *p++);) {
				...
				int any_set = 0;
				/*
				 * See if the parameter is set (for arrays, if
				 * any
				 * element is set).
				 */
				...
				/*
				 * Check attributes - note that all array
				 *elements
				 * have (should have?) the same attributes, so
				 *checking
				 * the first is sufficient.
				 *
				 * Report an unset param only if the user has
				 * explicitly given it some attribute (like
				 *export);
				 * otherwise, after "echo $FOO", we would report
				 *FOO...
				 */
				...
				for(; vp; vp = vp->u.array) {
					/* Ignore array elements that aren't set
					 * unless there
					 * are no set elements, in which case
					 * the first is
					 * reported on
					 */
					...
					/* no arguments */
					if(thing == 0 && flag == 0) {
						/* at&t ksh prints things like
						 * export, integer,
						 * leftadj, zerofill, etc., but
						 * POSIX says must
...
int c_alias(wp) char** wp;
{
	...
	if(rflag) {
		static const char* const args[] = {"unalias", "-ta",
		                                   (const char*)0};

		if(!tflag || *wp) {
			shprintf("alias: -r flag can only be used with -t and "
			         "without arguments\n");
			return 1;
		}
	...
	for(; *wp != NULL; wp++) {
		...
		if((val && !tflag) || (!val && tflag && !Uflag)) {
			...
			newval =
			    tflag ? search(alias, path, X_OK, (int*)0) : val;
			if(newval) {
...
int buflen;
{
	...
	i++;
	if(sigtraps[i].name)
		shf_snprintf(buf, buflen, "%*d %*s %s", ki->num_width, i,
		             ki->name_width, sigtraps[i].name,
		             sigtraps[i].mess);
	else
		shf_snprintf(buf, buflen, "%*d %*d %s", ki->num_width, i,
...
int c_kill(wp) char** wp;
{
	...
	if(lflag) {
		if(wp[i]) {
			...
			print_columns(shl_stdout, SIGNALS - 1, kill_fmt_entry,
			              (void*)&ki, ki.num_width + ki.name_width +
			                              mess_width + 3);
		}
		return 0;
	}
