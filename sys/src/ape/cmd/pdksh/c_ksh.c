    ...
    c_whence(wp) char **wp;
{
	...
	while((vflag || ret == 0) && (id = *wp++) != NULL) {
		...
		if(vflag || (tp->type != CALIAS && tp->type != CEXEC && tp->type != CTALIAS))
			shprintf("%s", id);
		switch(tp->type) {
		case CKEYWD:
			if(vflag)
    ...
    c_typeset(wp) char **wp;
{
	...
	if(!(builtin_opt.info & GI_MINUSMINUS) && wp[builtin_opt.optind] && (wp[builtin_opt.optind][0] == '-' || wp[builtin_opt.optind][0] == '+') && wp[builtin_opt.optind][1] == '\0') {
...
static char *
    kill_fmt_entry(arg, i, buf, buflen) void *arg;
int i;
char *buf;
int buflen;
{
