...
static struct op* block
ARGS((int type, struct op* t1, struct op* t2, char** wp));
static struct op* newtp ARGS((int type));
static void syntaxerr ARGS((const char* what)) GCC_FUNC_ATTR(noreturn);
...
static const char* dbtestp_getopnd
	...
	if(t != NULL) {
		while(token(0) == '|') {
			...
			if(tl == NULL)
				...
				tl = tl->right =
				    block(TPIPE, tl->right, p, NOWORDS);
		}
		REJECT;
	}
	return (t);
...
static struct op* c_list(multi) int multi;
{
	...
	while(1) {
		...
		if(c == '\n' && (multi || inalias(source))) {
			...
			p = block(c == '&' ? TASYNC : TCOPROC, p, NOBLOCK,
			          NOWORDS);
		else if(c != ';')
			have_sep = 0;
		if(!t)
...
static struct op* get_command(cf) int cf;
{
	...
	iops =
	    (struct ioword**)alloc(sizeofN(struct ioword*, NUFILE + 1), ATEMP);
	XPinit(args, 16);
	XPinit(vars, 16);
		...
		while(1) {
			...
			case LWORD:
				...
				if(iopn == 0 && XPsize(vars) == 0 &&
				   XPsize(args) == 0 && assign_command(ident))
	...
	if(iopn == 0) {
		...
		iops[iopn++] = NULL;
		iops = (struct ioword**)aresize(
		    (void*)iops, sizeofN(struct ioword*, iopn), ATEMP);
		t->ioact = iops;
	}
...
const char* const dbtest_tokens[] = {dbtest_or, dbtest_and, dbtest_not,
	...
	if(meta == TM_OR)
		...
		if(meta == TM_BINOP && c == REDIR &&
		   (yylval.iop->flag == IOREAD ||
			...
			ret = 1;
			save = wdcopy(yylval.iop->flag == IOREAD ? db_lthan
			                                         : db_gthan,
			              ATEMP);
