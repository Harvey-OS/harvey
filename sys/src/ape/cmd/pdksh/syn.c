...
static struct op *
    nested(type, smark, emark) int type,
    smark, emark;
{
	...
	case REDIR:
		...
		while(1) {
			...
			case LWORD:
				 ...
				 */
				if(iopn == 0 && XPsize(vars) == 0 && XPsize(args) == 0 && assign_command(ident))
...
static struct op *
    function_body(name, ksh_func) char *name;
...
static void
    nesting_push(save, tok) struct nesting_state *save;
int tok;
{
	*save = nesting;
...
static void
    nesting_pop(saved) struct nesting_state *saved;
{
	nesting = *saved;
}
 ...
 */
static int
    assign_command(s) char *s;
{
	char c = *s;

	if(Flag(FPOSIX) || !*s)
		return 0;
	return (c == 'a' && strcmp(s, "alias") == 0) || (c == 'e' && strcmp(s, "export") == 0) || (c == 'r' && strcmp(s, "readonly") == 0) || (c == 't' && strcmp(s, "typeset") == 0);
}

/* Check if we are in the middle of reading an alias */
static int
    inalias(s) struct source *s;
{
...
Test_meta meta;
{
	...
	if(meta == TM_OR)
		...
		if(meta == TM_BINOP && c == REDIR && (yylval.iop->flag == IOREAD || yylval.iop->flag == IOWRITE)) {
			ret = 1;
