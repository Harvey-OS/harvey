 ...
 */
char *
    evalonestr(cp, f) register char *cp;
int f;
{
	XPtrV w;
...
void
    expand(cp, wp, f) char *cp; /* input word */
 ...
 */
static int
    comsub(xp, cp) register Expand *xp;
char *cp;
{
	Source *s, *sold;
...
static char *
    trimsub(str, pat, how) register char *str;
char *pat;
int how;
{
 ...
 */
int
    glob_str(cp, wp, markdirs) char *cp;
XPtrV *wp;
int markdirs;
{
	int oldsize = XPsize(*wp);
...
#define stat_check() (stat_done ? stat_done : (stat_done = stat(Xstring(*xs, xp), &statb) < 0 \
							       ? -1                           \
							       : 1))
			 ...
			 */
			if((check & GF_EXCHECK) && xp > Xstring(*xs, xp) && ISDIRSEP(xp[-1]) && !S_ISDIR(lstatb.st_mode)
#ifdef S_ISLNK
			 ...
			 * directory
			 */
			if(((check & GF_MARKDIR) && (check & GF_GLOBBED)) && xp > Xstring(*xs, xp) && !ISDIRSEP(xp[-1]) && (S_ISDIR(lstatb.st_mode)
#ifdef S_ISLNK
															    || (S_ISLNK(lstatb.st_mode) && stat_check() > 0 && S_ISDIR(statb.st_mode))
#endif /* S_ISLNK */
																)) {
 ...
 */
static char *
    maybe_expand_tilde(p, dsp, dpp, isassign) char *p;
XString *dsp;
char **dpp;
int isassign;
{
