...
int x_bind ARGS((const char* a1, const char* a2, int macro, int list));
/* eval.c */
...
char* search ARGS((const char* name, const char* path, int mode, int* errnop));
...
int findhist ARGS((int start, int fwd, const char* str, int anchored));
#endif /* HISTORY */
/* io.c */
...
void bi_errorf ARGS((const char* fmt, ...)) GCC_FUNC_ATTR(format(printf, 1, 2));
void internal_errorf ARGS((int jump, const char* fmt, ...))
    GCC_FUNC_ATTR(format(printf, 2, 3));
void error_prefix ARGS((int fileline));
void shellf ARGS((const char* fmt, ...)) GCC_FUNC_ATTR(format(printf, 1, 2));
void shprintf ARGS((const char* fmt, ...)) GCC_FUNC_ATTR(format(printf, 1, 2));
#ifdef KSH_DEBUG
...
#endif /* KSH */
/* main.c */
int include ARGS((const char* name, int argc, char** argv, int intr_ok));
...
void aerror ARGS((Area * ap, const char* msg)) GCC_FUNC_ATTR(noreturn);
/* misc.c */
...
const unsigned char* pat_scan
ARGS((const unsigned char* p, const unsigned char* pe, int match_sep));
...
void print_columns
ARGS((struct shf * shf, int n, char* (*func)(void*, int, char*, int), void* arg,
      int max_width));
...
/* path.c */
int make_path ARGS((const char* cwd, const char* file, char** pathlist,
...
struct tbl* typeset
ARGS((const char* var, Tflag set, Tflag clr, int field, int base));
