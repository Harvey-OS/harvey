...
j_exit()
{
	...
	for(j = job_list; j != (Job *)0; j = j->next) {
		if(j->ppid == procpid && (j->state == PSTOPPED || (j->state == PRUNNING && ((j->flags & JF_FG) || (Flag(FLOGIN) && !Flag(FNOHUP) && procpid == kshpid))))) {
			killed = 1;
...
int
    exchild(t, flags, close_fd) struct op *t;
int flags;
int close_fd; /* used if XPCLOSE or XCCLOSE */
{
	...
	if(Flag(FMONITOR) && !(flags & XXCOM)) {
		...
		if(first_child_sync) {
			char c;
			while(read(j_sync_pipe[0], &c, 1) == -1 && errno == EINTR)
				;
			close(j_sync_pipe[0]);
			j_sync_open = 0;
		}
	...
	if(close_fd >= 0 && (((flags & XPCLOSE) && !ischild) || ((flags & XCCLOSE) && ischild)))
		close(close_fd);
	if(ischild) {/* child */
#ifdef KSH
...
const char *where;
{
	...
	while((volatile int)j->state == PRUNNING || ((flags & JW_STOPPEDWAIT) && (volatile int)j->state == PSTOPPED)) {
#ifdef JOB_SIGS
	...
	if(j->flags & JF_FG) {
		...
		if(Flag(FMONITOR) && j->state == PSIGNALLED && WIFSIGNALED(status) && (sigtraps[WTERMSIG(status)].flags & TF_TTY_INTR))
...
struct shf *shf;
{
	...
	for(p = j->proc_list; p != (Proc *)0;) {
		...
		case PSIGNALLED:
			 ...
			 * (ie, SIGINT, SIGPIPE)
			 */
			if(how == JP_SHORT && !coredumped && (WTERMSIG(p->status) == SIGINT || WTERMSIG(p->status) == SIGPIPE)) {
				buf[0] = '\0';
			} else
		...
		status = p->status;
		p = p->next;
		while(p && p->state == state && WSTATUS(p->status) == WSTATUS(status)) {
			if(how == JP_LONG)
