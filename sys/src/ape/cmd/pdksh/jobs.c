...
int tcsetpgrp(fd, grp) int fd;
pid_t grp;
{
	return ioctl(fd, TIOCSPGRP, &grp);
}

int tcgetpgrp(fd) int fd;
{
	int r, grp;
...
static const char* const lookup_msgs[] = {null, "no such job", "ambiguous",
	...
	if(Flag(FMONITOR)) {
		...
		if(ttypgrp_ok && our_pgrp != kshpid) {
			if(setpgid(0, kshpid) < 0) {
				warningf(FALSE, "j_init: setpgid() failed: %s",
				         strerror(errno));
				ttypgrp_ok = 0;
			} else {
...
#if defined(NTTYDISC) && defined(TIOCSETD) && !defined(HAVE_TERMIOS_H) &&      \
...
int close_fd; /* used if XPCLOSE or XCCLOSE */
{
	...
	if(flags & XPIPEI) {/* continuing with a pipe */
		if(!last_job)
			internal_errorf(
			    1, "exchild: XPIPEI and no last_job - pid %d",
			    (int)procpid);
		j = last_job;
		last_proc->next = p;
		last_proc = p;
...
static void j_set_async(j) Job* j;
{
	...
	if(async_job &&
	   (async_job->flags & (JF_KNOWN | JF_ZOMBIE)) == JF_ZOMBIE)
	...
	while(nzombie > child_max) {
		...
		if(!oldest) {
			...
			if(!(async_job->flags & JF_ZOMBIE) || nzombie != 1) {
				internal_errorf(0, "j_async: bad nzombie (%d)",
				                nzombie);
				nzombie = 0;
			}
			break;
		...
		if(Flag(FMONITOR) && ttypgrp_ok && j->pgrp) {
			...
			if(tcsetpgrp(tty_fd, our_pgrp) < 0) {
				warningf(
				    ...
				    tty_fd, (int)our_pgrp, strerror(errno));
			}
			if(j->state == PSTOPPED) {
				j->flags |= JF_SAVEDTTY;
...
static void fill_command(c, len, t) char* c;
int len;
struct op* t;
{
	int alen;
