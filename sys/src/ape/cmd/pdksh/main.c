...
/* set posix flag just before environment so that it will have
	...
	/* Figure out the current working directory and set $PWD */
	{
		...
		if(!ISABSPATH(pwd) || stat(pwd, &s_pwd) < 0 || stat(".", &s_dot) < 0 || s_pwd.st_dev != s_dot.st_dev || s_pwd.st_ino != s_dot.st_ino)
			pwdx = (char *)0;
		set_current_wd(pwdx);
		if(current_wd[0])
	...
	if(Flag(FLOGIN)) {
		 ...
		 */
		if(!Flag(FPRIVILEGED) && strcmp(profile = substitute("$INIT/profile.ksh", 0),
						"/profile.ksh"))
    ...
    unwind(i) int i;
{
	...
	if(i == LEXIT || (Flag(FERREXIT) && (i == LERROR || i == LINTR) && sigtraps[SIGEXIT_].trap)) {
...
static void
    remove_temps(tp) struct temp *tp;
{
#ifdef OS2
