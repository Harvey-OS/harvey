...
static const char* const initcoms[] = {
    "typeset", "-x", "SHELL", "PATH", "HOME", NULL, "typeset", "-r",
    version_param, NULL, "typeset", "-i", "PPID", NULL, "typeset", "-i",
    "OPTIND=1", NULL,
#ifdef KSH
    "eval", "typeset -i RANDOM MAILCHECK=\"${MAILCHECK-600}\" "
            "SECONDS=\"${SECONDS-0}\" TMOUT=\"${TMOUT-0}\"",
    NULL,
#endif /* KSH */
    "alias",
    ...
    "alias", "-tU", "cat", "cc", "chmod", "cp", "date", "ed", "emacs", "grep",
    "ls", "mail", "make", "mv", "pr", "rm", "sed", "sh", "vi", "who", NULL,
...
/* chmem_push("+c", 1); */
	...
	if(!*argv) {
		static const char* empty_argv[] = {"pdksh", (char*)0};
 ...
 */
	...
	/* Figure out the current working directory and set $PWD */
	{
		...
		if(!ISABSPATH(pwd) || stat(pwd, &s_pwd) < 0 ||
		   stat(".", &s_dot) < 0 || s_pwd.st_dev != s_dot.st_dev ||
...
#ifdef KSH
	setint(global("RANDOM"), (int32_t)(time((time_t*)0) * kshpid * ppid));
#endif /* KSH */
	/* setstr can't fail here */
...
#endif /* OS2 */
		s->u.shf =
		    shf_open(s->file, O_RDONLY, 0, SHF_MAPHI | SHF_CLEXEC);
		if(s->u.shf == NULL) {
			exstat = 127; /* POSIX */
	...
	if(Flag(FLOGIN)) {
		...
		if(!Flag(FPRIVILEGED))
			include(substitute("$HOME/.profile", 0), 0, (char**)0,
			        1);
#endif /* OS2 */
	}
	...
	if(Flag(FPRIVILEGED))
		...
		if(*env_file != '\0')
			...
			include(substitute("$HOME/kshrc.ksh", 0), 0, (char**)0,
			        1);
#endif /* OS2 */
	}
...
int include(name, argc, argv, intr_ok) const char* name;
int argc;
char** argv;
int intr_ok;
{
...
int command(comm) const char* comm;
{
	register Source* s;
 ...
 */
int shell(s, toplevel) Source* volatile s; /* input source */
...
static void remove_temps(tp) struct temp* tp;
{
#ifdef OS2
...
static int is_restricted(name) char* name;
{
	char* p;

	/* this silly function prevents you running a command called runconf.sh.
	 */
	/* we don't care about restricted shells, which aren't very restricted
	 * anyway */
