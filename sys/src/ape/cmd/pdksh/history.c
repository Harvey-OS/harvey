...
static int hist_replace
ARGS((char** hp, const char* pat, const char* rep, int global));
...
int c_fc(wp) char** wp;
{
	...
	if(lflag) {
		...
		for(hp = rflag ? hlast : hfirst; hp >= hfirst && hp <= hlast;
		    hp += rflag ? -1 : 1) {
	...
	if(!(shf = tf->shf)) {
		bi_errorf("cannot create temp file %s - %s", tf->name,
		          strerror(errno));
		return 1;
	}
	for(hp = rflag ? hlast : hfirst; hp >= hfirst && hp <= hlast;
...

	{
		...
		if(n < 0) {
			bi_errorf("error reading temp file %s - %s", tf->name,
...
static int hist_replace(hp, pat, rep, global) char** hp;
const char* pat;
const char* rep;
int global;
{
...
void histsave(
    lno, cmd,
    dowrite) int lno; /* ignored (compatibility with COMPLEX_HISTORY) */
const char* cmd;
...
retry:
	...
	if(hsize == 0) {
		 ...
		 * we have some data
		 */
		base = (unsigned char*)mmap(0, hsize, PROT_READ, MAP_FLAGS,
		                            histfd, 0);
		/*
...
static int hist_count_lines(base, bytes) register unsigned char* base;
register int bytes;
{
	State state = shdr;
	register lines = 0;
 ...
 */
static int hist_shrink(oldbase, oldbytes) unsigned char* oldbase;
int oldbytes;
{
	int fd;
	char nfile[1024];
 ...
 */
static unsigned char* hist_skip_back(base, bytes, no) unsigned char* base;
int* bytes;
int no;
{
 ...
 */
static void histload(s, base, bytes) Source* s;
 ...
 */
static void histinsert(s, lno, line) Source* s;
int lno;
unsigned char* line;
{
	register char** hp;
 ...
 */
static void writehistfile(lno, cmd) int lno;
char* cmd;
{
	int sizenow;
	unsigned char* base;
	...
	if(sizenow != hsize) {
		...
		if(sizenow > hsize) {
			...
			bytes = sizenow - hsize;
			base = (unsigned char*)mmap(0, sizenow, PROT_READ,
			                            MAP_FLAGS, histfd, 0);
			if((int)base == -1)
				goto bad;
			new = base + hsize;
