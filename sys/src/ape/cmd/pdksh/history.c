...
static int
    hist_replace(hp, pat, rep, global) char **hp;
const char *pat;
const char *rep;
int global;
{
...
static char **
    hist_get_newest(allow_cur) int allow_cur;
{
 ...
 */
int
    findhist(start, fwd, str, anchored) int start;
int fwd;
const char *str;
int anchored;
{
 ...
 */
void
    histsave(lno, cmd, dowrite) int lno; /* ignored (compatibility with COMPLEX_HISTORY) */
const char *cmd;
 ...
 */
void
    histsave(lno, cmd, dowrite) int lno;
const char *cmd;
int dowrite;
{
	register char **hp;
...
static int
    hist_count_lines(base, bytes) register unsigned char *base;
register int bytes;
{
	State state = shdr;
	register lines = 0;
 ...
 */
static int
    hist_shrink(oldbase, oldbytes) unsigned char *oldbase;
int oldbytes;
{
	int fd;
	char nfile[1024];
...
static unsigned char *
    hist_skip_back(base, bytes, no) unsigned char *base;
int *bytes;
int no;
{
