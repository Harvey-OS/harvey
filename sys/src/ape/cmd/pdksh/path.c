 ...
 */
void simplify_path(path) char* path;
{
	char* cur;
	char* t;
	int isrooted;
...
void set_current_wd(path) char* path;
{
	int len;
	char* p = path;
...
#ifdef S_ISLNK
char* get_phys_path(path) const char* path;
{
	XString xs;
	char* xp;
...
static char* do_phys_path(xsp, xp, path) XString* xsp;
char* xp;
const char* path;
{
	const char* p, *q;
