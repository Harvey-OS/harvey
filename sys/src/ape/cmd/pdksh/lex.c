...
/* optimized getsc_bn() */
#define getsc() (*source->str != '\0' && *source->str != '\\' && !backslash_skip ? *source->str++ : getsc_bn())
/* optimized getsc__() */
...
Again:
	...
	while(!((c = getsc()) == 0 || ((state == SBASE || state == SHEREDELIM) && ctype(c, C_LEX1)))) {
		Xcheck(ws, wp);
		switch(state) {
		case SBASE:
...
Done:
	...
	dp = Xstring(ws, wp);
	if((c == '<' || c == '>') && state == SBASE && ((c2 = Xlength(ws, wp)) == 0 || (c2 == 2 && dp[0] == CHAR && digit(dp[1])))) {
		struct ioword *iop =
	...
	*wp++ = EOS; /* terminate word */
	...
	if(*ident != '\0' && (cf & (KEYWORD | ALIAS))) {
		...
		/* { */
		if((cf & KEYWORD) && (p = tsearch(&keywords, ident, h)) && (!(cf & ESACONLY) || p->val.i == ESAC || p->val.i == '}')) {
			afree(yylval.cp, ATEMP);
			return p->val.i;
		}
		if((cf & ALIAS) && (p = tsearch(&aliases, ident, h)) && (p->flag & ISSET)) {
			register Source *s;
...
static void
    readhere(iop) struct ioword *iop;
{
	register int c;
	char *volatile eof;
	char *eofp;
	...
	while((c = *s->str++) == 0) {
		...
		case SALIAS:
			if(s->flags & SF_ALIASEND) {
				...
				s = source;
			} else if(*s->u.tblp->val.s && isspace(strchr(s->u.tblp->val.s, 0)[-1])) {
