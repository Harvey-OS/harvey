 ...
 */
struct tbl *
    global(n) register const char *n;
{
	register struct block *l = e->loc;
 ...
 */
struct tbl *
    local(n, copy) register const char *n;
bool_t copy;
{
	register struct block *l = e->loc;
	register struct tbl *vp;
	...
	if(copy && !(vp->flag & DEFINED)) {
		...
		while((ll = ll->next) && !(vq = tsearch(&ll->vars, n, h)))
			;
		if(vq) {
			vp->flag |= vq->flag & (EXPORT | INTEGER | RDONLY | LJUST | RJUST | ZEROFIL | LCASEV | UCASEV_AL | INT_U | INT_L);
			if(vq->flag & INTEGER)
				vp->type = vq->type;
...
char *
    str_val(vp) register struct tbl *vp;
{
	char *s;
...
int32_t
    intval(vp) register struct tbl *vp;
{
	int32_t num;
	int base;
...
int
    setstr(vq, s, error_ok) register struct tbl *vq;
const char *s;
int error_ok;
{
	if(vq->flag & RDONLY) {
...
void
    setint(vq, n) register struct tbl *vq;
int32_t n;
{
	if(!(vq->flag & INTEGER)) {
 ...
 */
struct tbl *
    setint_v(vq, vp) register struct tbl *vq,
    *vp;
{
	int base;
	int32_t num;
 ...
 */
static void export(vp, val) register struct tbl *vp;
const char *val;
{
	register char *xp;
 ...
 */
struct tbl *
    typeset(var, set, clr, field, base) register const char *var;
Tflag clr, set;
int field, base;
{
	register struct tbl *vp;
	...
	if(Flag(FRESTRICTED) && (strcmp(tvar, "PATH") == 0 || strcmp(tvar, "ENV") == 0 || strcmp(tvar, "SHELL") == 0))
	...
	if(set | clr) {
		...
		for(t = vpbase; t; t = t->u.array) {
			...
			fake_assign = (t->flag & ISSET) && (!val || t != vp) && ((set & (UCASEV_AL | LCASEV | LJUST | RJUST | ZEROFIL)) || ((t->flag & INTEGER) && (clr & INTEGER)) || (!(t->flag & INTEGER) && (set & INTEGER)));
			if(fake_assign) {
				if(t->flag & INTEGER) {
					s = str_val(t);
					free_me = (char *)0;
 ...
 */
void
    unset(vp, array_ref) register struct tbl *vp;
int array_ref;
{
	if(vp->flag & ALLOC)
...
static int
    special(name) register const char *name;
{
...
static void
    unspecial(name) register const char *name;
{
	register struct tbl *tp;
...
static void
    getspec(vp) register struct tbl *vp;
{
...
static void
    setspec(vp) register struct tbl *vp;
{
	char *s;
	...
	case V_TMPDIR:
		 ...
		 */
		{
			struct stat statb;
			s = str_val(vp);
			if(ISABSPATH(s) && eaccess(s, W_OK | X_OK) == 0 && stat(s, &statb) == 0 && S_ISDIR(statb.st_mode))
...
static void
    unsetspec(vp) register struct tbl *vp;
{
	switch(special(vp->name)) {
	case V_PATH:
		if(path)
 ...
 */
static struct tbl *
    arraysearch(vp, val) struct tbl *vp;
int val;
{
