 ...
 */
struct tbl* global(n) register const char* n;
{
	register struct block* l = e->loc;
 ...
 */
struct tbl* local(n, copy) register const char* n;
bool_t copy;
{
	register struct block* l = e->loc;
	register struct tbl* vp;
...
char* str_val(vp) register struct tbl* vp;
{
	char* s;
...
int32_t intval(vp) register struct tbl* vp;
{
	int32_t num;
	int base;
...
int setstr(vq, s, error_ok) register struct tbl* vq;
const char* s;
int error_ok;
{
	if(vq->flag & RDONLY) {
	...
	if(!(vq->flag & INTEGER)) {/* string dest */
		if((vq->flag & ALLOC)) {
			...
			if(s >= vq->val.s && s <= vq->val.s + strlen(vq->val.s))
				internal_errorf(
				    TRUE, "setstr: %s=%s: assigning to self",
				    vq->name, s);
...
void setint(vq, n) register struct tbl* vq;
int32_t n;
{
	if(!(vq->flag & INTEGER)) {
 ...
 */
struct tbl* setint_v(vq, vp) register struct tbl* vq, *vp;
{
	int base;
	int32_t num;
	...
	if(vp->flag & (RJUST | LJUST)) {
		...
		if(vp->flag & RJUST) {
			...
			if(vp->flag & ZEROFIL)
				while(*s == '0')
					s++;
			shf_snprintf(p, nlen + 1, "%-*.*s", vp->u2.field,
			             vp->u2.field, s);
		}
	} else
		memcpy(p, s, olen + 1);
 ...
 */
struct tbl* typeset(var, set, clr, field, base) register const char* var;
Tflag clr, set;
int field, base;
{
	register struct tbl* vp;
	...
	if(Flag(FRESTRICTED) &&
	   (strcmp(tvar, "PATH") == 0 || strcmp(tvar, "ENV") == 0 ||
	    strcmp(tvar, "SHELL") == 0))
	...
	if(set | clr) {
		...
		for(t = vpbase; t; t = t->u.array) {
			...
			fake_assign =
			    (t->flag & ISSET) && (!val || t != vp) &&
			    ((set &
			      (UCASEV_AL | LCASEV | LJUST | RJUST | ZEROFIL)) ||
 ...
 */
void unset(vp, array_ref) register struct tbl* vp;
int array_ref;
{
	if(vp->flag & ALLOC)
...
static int special(name) register const char* name;
{
...
static void unspecial(name) register const char* name;
{
	register struct tbl* tp;
...
static void getspec(vp) register struct tbl* vp;
{
...
static void setspec(vp) register struct tbl* vp;
{
	char* s;
...
static void unsetspec(vp) register struct tbl* vp;
{
	switch(special(vp->name)) {
	case V_PATH:
		if(path)
	...
	case V_TMOUT: /* at&t ksh leaves previous value in place */
		 ...
		 *meaning,
		 * but OPTARG does not (still set by getopts) and _ is also
		 *still
...
int val;
{
	...
	if(curr && curr->index == val) {
		...
		new = (struct tbl*)alloc(
		    sizeof(struct tbl) + strlen(vp->name) + 1, vp->areap);
