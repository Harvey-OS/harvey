...
static const struct t_op u_ops[] = {{"-a", TO_FILAXST},
                                    {"-b", TO_FILBDEV},
                                    {"-c", TO_FILCDEV},
                                    {"-d", TO_FILID},
                                    {"-e", TO_FILEXST},
                                    ...
                                    {"-z", TO_STZER},
                                    {"", TO_NONOP}};
static const struct t_op b_ops[] = {{"=", TO_STEQL},
#ifdef KSH
                                    {"==", TO_STEQL},
#endif /* KSH */
                                    {"!=", TO_STNEQ},
		...
		while(--argc >= 0) {
			...
			if(argc == 1) {
				...
				if(!Flag(FPOSIX) && strcmp(opnd1, "-t") == 0)
					break;
				res = (*te.eval)(&te, TO_STNZE, opnd1, (char*)0,
				                 1);
				if(invert & 1)
					res = !res;
				return !res;
...
static int test_stat(path, statb) const char* path;
struct stat* statb;
{
 ...
 */
static int test_eaccess(path, mode) const char* path;
int mode;
{
	int res;
	...
	if(strncmp(path, "/dev/fd/", 8) == 0 && getn(path + 8, &fd)) {
		...
		if((flags = fcntl(fd, F_GETFL, 0)) < 0 || (mode & X_OK) ||
...
const char* msg;
{
	const char* op = te->pos.wp + offset >= te->wp_end ? (const char*)0
	                                                   : te->pos.wp[offset];
