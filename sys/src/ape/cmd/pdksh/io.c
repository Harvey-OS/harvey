...
#else
    errorf(fmt, va_alist) const char* fmt;
va_dcl
#endif
{
	va_list va;
...
#else
    warningf(fileline, fmt, va_alist) int fileline;
const char* fmt;
va_dcl
#endif
{
...
#else
    bi_errorf(fmt, va_alist) const char* fmt;
va_dcl
#endif
{
	va_list va;
...
#else
    internal_errorf(jump, fmt, va_alist) int jump;
const char* fmt;
va_dcl
#endif
{
...
void error_prefix(fileline) int fileline;
{
	...
	if(fileline && source && source->file != NULL) {
		shf_fprintf(shl_out, "%s[%d]: ", source->file,
		            source->errline > 0 ? source->errline
		                                : source->line);
		source->errline = 0;
	}
}
...
#else
    shellf(fmt, va_alist) const char* fmt;
va_dcl
#endif
{
	va_list va;
...
#else
    shprintf(fmt, va_alist) const char* fmt;
va_dcl
#endif
{
	va_list va;
...
kshdebug_init_()
{
	...
	kshdebug_shf =
	    shf_open("/tmp/ksh-debug.log", O_WRONLY | O_APPEND | O_CREAT, 0600,
	             SHF_WR | SHF_MAPHI);
	if(kshdebug_shf) {
...
#else
    kshdebug_printf_(fmt, va_alist) const char* fmt;
va_dcl
#endif
{
	va_list va;
...
void kshdebug_dump_(str, mem, nbytes) const char* str;
const void* mem;
int nbytes;
{
	int i, j;
...
int can_seek(fd) int fd;
{
	...
	return fstat(fd, &statb) == 0 && !S_ISREG(statb.st_mode) ? SHF_UNBUF
	                                                         : 0;
}
...
void openpipe(pv) register int* pv;
{
	if(pipe(pv) < 0)
...
void closepipe(pv) register int* pv;
{
	close(pv[0]);
	close(pv[1]);
}
...
struct temp** tlist;
{
	...
	while(1) {
		...
		shf_snprintf(path, len, "%s/sh%05u.%03x", dir,
		             (unsigned)procpid, inc++);
