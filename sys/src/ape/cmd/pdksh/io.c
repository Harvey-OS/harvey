...
void
    error_prefix(fileline) int fileline;
{
	/* Avoid foo: foo[2]: ... */
...
int
    ksh_dup2(ofd, nfd, errok) int ofd;
int nfd;
int errok;
{
 ...
 */
int
    check_fd(name, mode, emsgp) char *name;
int mode;
const char **emsgp;
{
	int fd, fl;
	...
	if(isdigit(name[0]) && !name[1]) {
		 ...
		 * posix has to say).
		 */
		if(!(mode & X_OK) && fl != O_RDWR && (((mode & R_OK) && fl != O_RDONLY) || ((mode & W_OK) && fl != O_WRONLY))) {
			if(emsgp)
				*emsgp = (fl == O_WRONLY) ? "fd not open for reading"
 ...
 */
int
    coproc_getfd(mode, emsgp) int mode;
const char **emsgp;
{
 ...
 */
void
    coproc_cleanup(reuse) int reuse;
{
