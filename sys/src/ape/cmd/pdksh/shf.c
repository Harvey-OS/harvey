...
struct shf *
    shf_fdopen(fd, sflags, shf) int fd;
int sflags;
struct shf *shf;
{
	int bsize = sflags & SHF_UNBUF ? (sflags & SHF_RD ? 1 : 0) : SHF_BSIZE;
...
struct shf *
    shf_reopen(fd, sflags, shf) int fd;
int sflags;
struct shf *shf;
{
	int bsize = sflags & SHF_UNBUF ? (sflags & SHF_RD ? 1 : 0) : SHF_BSIZE;
 ...
 */
struct shf *
    shf_sopen(buf, bsize, sflags, shf) char *buf;
int bsize;
int sflags;
struct shf *shf;
{
...
int
    shf_fdclose(shf) struct shf *shf;
{
	int ret = 0;
 ...
 */
static int
    shf_emptybuf(shf, flags) struct shf *shf;
int flags;
{
	int ret = 0;
...
static int
    shf_fillbuf(shf) struct shf *shf;
{
	if(shf->flags & SHF_STRING)
		return 0;
 ...
 */
int
    shf_seek(shf, where, from) struct shf *shf;
off_t where;
int from;
{
	if(shf->fd < 0) {
 ...
 */
int
    shf_read(buf, bsize, shf) char *buf;
int bsize;
struct shf *shf;
{
 ...
 */
char *
    shf_getse(buf, bsize, shf) char *buf;
int bsize;
struct shf *shf;
{
	unsigned char *end;
...
int
    shf_getchar(shf) struct shf *shf;
{
	if(!(shf->flags & SHF_RD))
...
#else
    shf_fprintf(shf, fmt, va_alist) struct shf *shf;
const char *fmt;
va_dcl
#endif
{
...
#else
    shf_snprintf(buf, bsize, fmt, va_alist) char *buf;
int bsize;
const char *fmt;
va_dcl
#endif
...
#else
    shf_smprintf(fmt, va_alist) char *fmt;
va_dcl
#endif
{
	struct shf shf;
 ...
 */
#define POP_INT(f, s, a) (((f)&FL_LONG) ? va_arg((a), unsigned long)                                        \
					: (sizeof(int) < sizeof(int32_t) ? ((s) ? (int32_t)va_arg((a), int) \
										: va_arg((a), unsigned))    \
									 : va_arg((a), unsigned)))
...
int
    shf_vfprintf(shf, fmt, args) struct shf *shf;
const char *fmt;
va_list args;
{
	char c, *s;
