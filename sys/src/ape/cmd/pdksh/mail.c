...
void mcset(interval) int32_t interval;
{
	mailcheck_interval = interval;
}

void mbset(p) register char* p;
{
	struct stat stbuf;
...
void mpset(mptoparse) register char* mptoparse;
{
	register mbox_t* mbp;
...
static void munset(mlist) register mbox_t* mlist;
{
	register mbox_t* mbp;
