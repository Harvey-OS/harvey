	   ...
	   int *setargsp)
{
	...
	if(what == OF_CMDLINE) {
		 ...
		 * flag using +l.
		 */
		Flag(FLOGIN) = (argv[0][0] == '-' || ((p = ksh_strrchr_dirsep(argv[0])) && *++p == '-'));
		opts = cmd_opts;
	} else
		opts = set_opts;
	...
	while((optc = ksh_getopt(argv, &go, opts)) != EOF) {
		...
		default:
			...
			for(i = 0; i < NELEM(options); i++)
				if(optc == options[i].c && (what & options[i].flags)) {
	...
	if(!(go.info & GI_MINUSMINUS) && argv[go.optind] && (argv[go.optind][0] == '-' || argv[go.optind][0] == '+') && argv[go.optind][1] == '\0') {
...
ksh_getopt(char **argv, Getopt *go, const char *options)
{
	...
	if(go->p == 0 || (c = argv[go->optind - 1][go->p]) == '\0') {
		...
		if(arg == (char *)0 || ((flag != '-') /* neither a - nor a + (if + allowed) */
