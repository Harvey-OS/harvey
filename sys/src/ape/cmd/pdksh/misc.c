...
static int do_gmatch
ARGS((const unsigned char* s, const unsigned char* p, const unsigned char* se,
      const unsigned char* pe, int isfile));
 ...
 */
int
parse_args(char** argv, int what, /* OF_CMDLINE or OF_SET */
           int* setargsp)
{
	...
	if(setargsp)
		...
		*setargsp =
		    !arrayset && ((go.info & GI_MINUSMINUS) || argv[go.optind]);
...
gmatch(const char* s, const char* p, int isfile)
{
	...
	if(!isfile && !has_globbing(p, pe)) {
		int len = pe - p + 1;
		char tbuf[64];
		char* t = len <= sizeof(tbuf) ? tbuf : (char*)alloc(len, ATEMP);
		debunk(t, p);
		return !strcmp(t, s);
	}
...
#ifdef EWOULDBLOCK || errno == EWOULDBLOCK
#endif /* EWOULDBLOCK */)) {
			int oerrno = errno;
