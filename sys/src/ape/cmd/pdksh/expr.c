...
ARGS((Expr_state * es, enum token op, struct tbl* vasn, bool_t is_prefix));
static void assign_check
ARGS((Expr_state * es, enum token op, struct tbl* vasn));
...
/* Do a ++ or -- operation */
static struct tbl* do_ppmm(es, op, vasn, is_prefix) Expr_state* es;
enum token op;
struct tbl* vasn;
bool_t is_prefix;
{
