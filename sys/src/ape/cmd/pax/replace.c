...
void add_replstr(pattern) char* pattern;
...
#endif
{
	...
	strcpy(buff, name);
	for(rptr = rplhead; !found && rptr != (Replstr*)NULL;
	    rptr = rptr->next) {
		do {
...
int
get_disposition(char* mode, char* name)

#else

int get_disposition(mode, name) char* mode;
char* name;
...
int get_newname(name, size) char* name;
int size;
