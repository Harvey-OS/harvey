...
int do_pax(ac, av) int ac; /* argument counter */
char** av;                 /* arguments */
...
static OFFSET pax_optsize(str) char* str; /* pointer to string to interpret */
...
#endif
{
	fprintf(stderr, "Usage: %s -[cimopuvy] [-f archive] [-s replstr] [-t "
	                "device] [pattern...]\n",
	        myname);
	fprintf(stderr, "       %s -r [-cimopuvy] [-f archive] [-s replstr] "
	                "[-t device] [pattern...]\n",
	        myname);
	fprintf(stderr, "       %s -w [-adimuvy] [-b blocking] [-f archive] "
	                "[-s replstr]\n              [-t device] [-x format] "
	                "[pathname...]\n",
	        myname);
	fprintf(stderr, "       %s -r -w [-ilmopuvy] [-s replstr] "
	                "[pathname...] directory\n",
	        myname);
	exit(1);
}
