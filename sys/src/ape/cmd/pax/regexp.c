...
regexp* regcomp(exp) char* exp;
{
	...
	if(OP(regnext(scan)) == END) {/* Only one top-level choice. */
		 ...
		 * Resolve
		 * ties in favor of later strings, since the regstart check
		 * works
		 * with the beginning of the r.e. and avoiding duplication
		 * strengthens checking.  Not a strong reason, but sufficient in
		 * the
		 * absence of others.
		 */
		if(flags & SPSTART) {
...
/*
 - regnode - emit a node
 */
static char* regnode(op) char op;
{
	register char* ret;
	register char* ptr;
...
static int regtry(prog, string) regexp* prog;
char* string;
...
static int regmatch(prog) char* prog;
...
#endif
{
	...
	while(scan != (char*)NULL) {
		...
		case OPEN + 9: {
			...
			if(regmatch(nxt)) {
				/*
				 * Don't set startp if some later invocation of
				 * the same
		...
		case CLOSE + 9: {
			...
			if(regmatch(nxt)) {
				/*
				 * Don't set endp if some later invocation of
				 * the same
				 * parentheses already has.
				 */
