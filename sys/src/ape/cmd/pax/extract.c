...
#ifndef lint
static char* ident =
    "$Id: extract.c,v 1.3 89/02/12 10:29:43 mark Exp Locker: mark $";
...
#endif
{
	...
	while(get_header(name, &sb) == 0) {
		...
		if(f_list) { /* only wanted a table of contents */
			...
			if(((ar_format == TAR)
			        ? buf_skip(
			              ROUNDUP((OFFSET)sb.sb_size, BLOCKSIZE))
			...
			if(ar_format == TAR && sb.sb_nlink > 1) {
				/*
				 * This kludge makes sure that the link table is
				 * cleared
...
#endif
{
	...
	for(;;) {
		...
		if(skipped) {
			warnarch("Apparently resynchronized",
			         (OFFSET)sizeof(magic));
...
#endif
{
	...
	memcpy((char*)&binary, magic + sizeof(uint16_t),
...
#endif
{
	...
	header[H_STRLEN] = '\0';
	if(sscanf(header, H_SCAN, &asb->sb_dev, &asb->sb_ino, &asb->sb_mode,
	          &asb->sb_uid,
#ifdef _POSIX_SOURCE
