...
#define DIRSIZ(dp)                                                             \
	((sizeof(struct direct) - (MAXNAMELEN + 1) + sizeof(int32_t) +         \
	  (dp)->d_namlen) /                                                    \
	...
	for(rewind = true;;) {
		...
		if((dirp->dd_loc < dirp->dd_size /* valid index */
		   ...
		   (dp = (struct dirent*)&dirp->dd_buf[dirp->dd_loc])->d_off <=
		    ...
		    /* whole buffer past matching entry */ if(!rewind) { /* no
		                                                          * point
		                                                          * in
		                                                          * searching
		                                                          * further
...
OFFSET telldir(dirp) DIR* dirp; /* stream from opendir() */
...
/* directory file block buffer */
	...
	for(bp = (struct dirent*)buf; bp == (struct dirent*)buf;) {
		...
		for(dp = (struct direct*)u.dblk; (char*)dp < &u.dblk[size];
			...
			if(dp->d_fileno !=
			   0) { /* non-empty; copy to user buffer */
				register int reclen =
				...
				{
#ifdef UFS
					/* Is the following kludge ugly?  You
					 * bet. */
