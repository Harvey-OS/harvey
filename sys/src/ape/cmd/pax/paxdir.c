...
#define DIRSIZ(dp) ((sizeof(struct direct) - (MAXNAMELEN + 1) + sizeof(int32_t) + (dp)->d_namlen) / sizeof(int32_t) * sizeof(int32_t))
...
DIR *opendir(dirname) char *dirname; /* name of directory */
...
#endif
{
	...
	if((dirp = (DIR *)malloc(sizeof(DIR))) == (DIR *)NULL || (dirp->dd_buf = (char *)malloc((unsigned)DIRBUF)) == (char *)NULL) {
...
int closedir(dirp) register DIR *dirp; /* stream from opendir() */
...
struct dirent *readdir(dirp) register DIR *dirp; /* stream from opendir() */
...
void
seekdir(register DIR *dirp, register OFFSET loc)

#else

void seekdir(dirp, loc) register DIR *dirp; /* stream from opendir() */
...
#endif
{
	...
	for(rewind = true;;) {
		...
		if((dirp->dd_loc < dirp->dd_size	      /* valid index */
		   ...
		   (dp = (struct dirent *)&dirp->dd_buf[dirp->dd_loc])->d_off <= loc /* match possible in this buffer */
		   ) {
			...
			if(lseek(dirp->dd_fd, (OFFSET)0, SEEK_SET) != 0)
...
static int NameLen(name) char *name; /* -> name embedded in struct direct */
...
static void sig_catch(sig) int sig; /* must be SIGSYS */
...
int
getdents(int fildes, char *buf, unsigned nbyte)

#else

int getdents(fildes, buf, nbyte) int fildes; /* directory file descriptor */
char *buf;				     /* where to put the (struct dirent)s */
unsigned nbyte;				     /* size of buf[] */
