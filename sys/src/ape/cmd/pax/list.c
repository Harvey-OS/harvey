...
int read_header(name, asb) char* name;
Stat* asb;
...
#endif
{
	...
	if(sum == 8 * ' ') {
		...
		/*
		 * This is a zeroed record...whole record is 0's except for the
		 * 8
	...
	if(sum == recsum) {
		...
		case AREGTYPE:
			/*
			 * Berkeley tar stores directories as regular files with
			 * a
			 * trailing /
			 */
...
void print_entry(name, asb) char* name;
Stat* asb;
...
static void cpio_entry(name, asb) char* name;
Stat* asb;
...
static void tar_entry(name, asb) char* name;
Stat* asb;
	...
	if(f_extract) {
		...
		if(f_verbose) {
			...
			fprintf(msgfile, " %d/%d %6d %3s %2d %02d:%02d %4d %s",
			        ...
			        atm->tm_hour, atm->tm_min, atm->tm_year + 1900,
			        name);
		} else {
			fprintf(msgfile, "%s", name);
		}
		...
		case S_IFLNK: /* This file is a symbolic link */
			...
			if(i < 0) { /* Could not find symbolic link */
				...
				symnam[i] = '\0';
				fprintf(msgfile, " symbolic link to %s",
				        symnam);
			}
			break;
#endif
...
static void pax_entry(name, asb) char* name;
Stat* asb;
...
#endif
{
	...
	if(f_list && f_verbose) {
		...
		case S_IFBLK:
		case S_IFCHR:
			fprintf(msgfile, "\t%3d, %3d", major(asb->sb_rdev),
			        minor(asb->sb_rdev));
			break;
		case S_IFREG:
		...
		fprintf(msgfile, " %3s %2d %02d:%02d ", monnames[atm->tm_mon],
...
static void
print_mode(uint16_t mode)

#else

static void print_mode(mode) ushort mode;
...
#endif
{
	...
	putc(mode & 0100 ? mode & 04000 ? 's' : 'x' : mode & 04000 ? 'S' : '-',
	     msgfile);
	putc(mode & 0040 ? 'r' : '-', msgfile);
	putc(mode & 0020 ? 'w' : '-', msgfile);
	putc(mode & 0010 ? mode & 02000 ? 's' : 'x' : mode & 02000 ? 'S' : '-',
	     msgfile);
	putc(mode & 0004 ? 'r' : '-', msgfile);
	putc(mode & 0002 ? 'w' : '-', msgfile);
	putc(mode & 0001 ? mode & 01000 ? 't' : 'x' : mode & 01000 ? 'T' : '-',
	     msgfile);
}
...
static long from_oct(digs,
                     where) int digs; /* number of characters to convert */
char* where;                          /* character representation of octal number */
