...
connproc(void* v)
{
	...
	while((p = vtrecv(c)) != nil) {
		...
		if(vtfcallunpack(&r->tx, p) < 0) {
			vtlog(VtServerLog, "<font size=-1>%T %s:</font> recv "
			                   "bad packet %p: %r<br>\n",
			      c->addr, p);
		...
		vtlog(VtServerLog,
		      "<font size=-1>%T %s:</font> recv packet %p (%F)<br>\n",
		      c->addr, p, &r->tx);
		if(chattyventi)
			fprint(2, "%s <- %F\n", argv0, &r->tx);
		packetfree(p);
...
vtgetreq(VtSrv* srv)
{
	...
	if(r != nil)
		vtlog(VtServerLog,
		      "<font size=-1>%T %s:</font> vtgetreq %F<br>\n",
...
vtrespond(VtReq* r)
{
	...
	vtlog(VtServerLog,
	      "<font size=-1>%T %s:</font> send packet %p (%F)<br>\n",
	      sc->c->addr, p, &r->rx);
	if(vtsend(sc->c, p) < 0)
		fprint(2, "vtsend %F: %r\n", &r->rx);
	scdecref(sc);
