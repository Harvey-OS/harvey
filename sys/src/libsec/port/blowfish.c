...
void
setupBFstate(BFstate *s, u8 key[], int keybytes, u8 *ivec)
{
	int i, j;
	u32 n, buf[2];
...
void
bfCBCencrypt(u8 *buf, int n, BFstate *s)
{
	int i;
	u8 *p;
	u32 bo[2], bi[2], b;
...
void
bfCBCdecrypt(u8 *buf, int n, BFstate *s)
{
	int i;
	u8 *p;
...
void
bfECBencrypt(u8 *buf, int n, BFstate *s)
{
	int i;
	u32 b[2];
...
void
bfECBdecrypt(u8 *buf, int n, BFstate *s)
{
	int i;
	u32 b[2];
...
bfencrypt(u32 *b, BFstate *s)
{
	...
	for(i=1; i<16; i += 2) {
		r ^= pb[i];
		r ^= ( (sb[ (u8) (l>>24)] + sb[256 + ((u8) (l>>16))]) ^
			sb[512 + ((u8) (l>>8))]) + sb[768 +((u8) l)];

		l ^= pb[i+1];
		l ^= ( (sb[ (u8) (r>>24)] + sb[256 + ((u8) (r>>16))]) ^
			sb[512 + ((u8) (r>>8))]) + sb[768 +((u8) r)];
	}
