...
static int (*ocvt[])(FILE*, va_list*, int, int, int) = {
    ...
    ocvt_X, 0,      0, 0,      0,      0,      0,      0, /*  X  Y  Z  [  \  ]
                                                             ^  _ */
    0,      0,      0, ocvt_c, ocvt_d, ocvt_e, ocvt_f, ocvt_g, /*  `  a  b  c  d
                                                                  e  f  g */
    0,      ocvt_d, 0, 0,      0,      0,      ocvt_n, ocvt_o, /*  h  i  j  k  l
                                                                  m  n  o */
    ocvt_p, 0,      0, ocvt_s, 0,      ocvt_u, 0,      0, /*  p  q  r  s  t  u
                                                             v  w */
    ocvt_x, 0,      0, 0,      0,      0,      0,      0, /*  x  y  z  {  |  }
                                                             ~ ^? */
...
vfprintf(FILE* f, const char* s, va_list args)
{
	...
	while(*s) {
		...
		s++;
		flags = 0;
		while(lflag[*s & _IO_CHMASK])
			flags |= lflag[*s++ & _IO_CHMASK];
		if(*s == '*') {
		...
		if(*s == '.') {
			...
			precision = -1;
		while(tflag[*s & _IO_CHMASK])
			flags |= tflag[*s++ & _IO_CHMASK];
		if(ocvt[*s])
			nprint +=
			    (*ocvt[*s++])(f, &args, flags, width, precision);
		else if(*s) {
			putc(*s++, f);
			nprint++;
		}
           ...
           int radix, int sgned, char alphabet[], char* prefix)
{
	...
	if(sgned) {
		if(flags & PTR)
			snum = (int32_t)va_arg(*args, void*);
		else if(flags & SHORT)
			snum = va_arg(*args, int16_t);
		else if(flags & LONG)
			snum = va_arg(*args, int32_t);
		else
...
{
	return ocvt_fixed(f, args, flags, width, precision, 16, 0,
	                  "0123456789ABCDEF", "0X");
}
...
{
	return ocvt_fixed(f, args, flags, width, precision, 8, 0, "01234567",
	                  "0");
}
...
ocvt_flt(FILE* f, va_list* args, int flags, int width, int precision, char afmt)
{
	...
	case 'g':
		if(precision > 0)
			digits =
			    dtoa(d, 2, precision, &exponent, &sign, &edigits);
		else {
			digits =
			    dtoa(d, 0, precision, &exponent, &sign, &edigits);
	...
	ndig = edigits - digits;
	if((afmt == 'g' || afmt == 'G') &&
	   !(flags & ALT)) { /* knock off trailing zeros */
		if(fmt == 'f') {
