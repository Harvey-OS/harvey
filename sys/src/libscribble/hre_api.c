...
#define RI_CHECK_MAGIC(rec)                                                    \
	((rec != nil) && (((recognizer)rec)->recognizer_magic == REC_MAGIC) && \
...
recognizer_load(char* directory, char* name, char** subset)
{
	...
	if(rec->recognizer_load_state == nil ||
	   ...
	   rec->recognizer_error == nil || rec->recognizer_set_context == nil ||
	   rec->recognizer_get_context == nil || rec->recognizer_clear == nil ||
...
recognizer_unload(recognizer rec)
{
	...
	if(!RI_CHECK_MAGIC(rec)) {
		the_last_error =
		    dgettext(INTL_DOMAIN, "Bad recognizer object.");
		return (-1);
	}
...
wordset
recognizer_load_dictionary(recognizer rec, char* dir, char* name)
{
	/*Make sure magic numbers right.*/
...
const char*
recognizer_manager_version(recognizer rec)
{
	/*Make sure magic numbers right.*/
...
int
recognizer_get_buffer(recognizer rec, uint* nstrokes, Stroke** strokes)
{
...
int
recognizer_set_buffer(recognizer rec, uint nstrokes, Stroke* strokes)
{
...
int
recognizer_translate(recognizer rec, uint nstrokes, Stroke* strokes,
                     bool correlate_p, int* nret, rec_alternative** ret)
{
	int retval;
	char msg[80];
	/*Make sure magic numbers right.*/
	 ...
	 *	fprint(2, "\nrecognizer_translate: ari_pts = %ld, sizeof(Time) =
	 *%d, sizeof(ari_pts[0] = %d, %d points are...\n", ari_pts,
	 *sizeof(Time), sizeof(ari_pts[0]), ari_pstr.ps_npts);
	 *	for (ari = 0; ari < ari_pstr.ps_npts; ari++)
	 *	   fprint(2, "%ld -- (%d, %d)  ", ari_pts[ari], ari_pts[ari].x,
	 *ari_pts[ari].y);
	 *      }
	 *    }
	*/
	...
	retval = rec->recognizer_translate(rec, nstrokes, strokes, correlate_p,
	                                   nret, ret);
	return (retval);
}
...
static char*
shared_library_name(char* directory, char* locale, char* name)
{
	char* ret;
	int len = strlen(name);
	...
	if(directory != nil) {
		...
		ret =
		    (char*)safe_malloc(strlen(dir) + strlen(locale) + len + 3);
		/*Form the pathname.*/
		strcpy(ret, dir);
		strcat(ret, "/");
		strcat(ret, locale);
...
static rec_info*
make_rec_info(char* c, char* d, char** subset)
{
	int i, len;
	rec_info* ri;
	char* locale;
	...
	if(subset != nil) {
		...
		for(i = 0; i < len; i++) {
			if(subset[i] != nil) {
				if((ri->ri_subset[i] = strdup(subset[i])) ==
				   nil) {
					delete_rec_info(ri);
					return (nil);
				}
...
rec_alternative*
make_rec_alternative_array(uint size)
{
	int i;
	rec_alternative* ri;
...
void
delete_rec_alternative_array(uint nalter, rec_alternative* ra,
...
rec_element*
initialize_rec_element(rec_element* re, char type, uint size, void* trans,
                       rec_confidence conf)
{
	if(re != nil) {
		...
		case REC_GESTURE:
			if(size > 0 && trans != nil) {
				...
				memcpy((void*)re->re_result.gval, trans,
				       sizeof(gesture));
			}
			break;
		...
		case REC_OTHER:
			if(size > 0 && trans != nil) {
				re->re_result.aval = (char*)safe_malloc(
				    (size + 1) * sizeof(char));
		...
		case REC_WCHAR:
			if(size > 0 && trans != nil) {
				re->re_result.wval = (wchar_t*)safe_malloc(
		...
		case REC_CORR:
			if(size > 0 && trans != nil) {
				re->re_result.rcval =
				    (rec_correlation*)safe_malloc(
				        sizeof(rec_correlation));
				memcpy((void*)re->re_result.rcval, trans,
				       sizeof(rec_correlation));
			}
			break;
...
rec_correlation*
make_rec_correlation(char type, uint size, void* trans, rec_confidence conf,
                     uint ps_size)
{
	rec_correlation* rc;
	...
	if(initialize_rec_element(&(rc->ro_elem), type, size, trans, conf) ==
	   nil) {
		return (nil);
	}
...
delete_rec_correlation(rec_correlation* rc, bool delete_points_p)
{
	if(rc != nil) {
		...
		delete_Stroke_array(rc->ro_nstrokes, rc->ro_strokes,
		                    delete_points_p);
...
Stroke*
initialize_Stroke(Stroke* ps, uint npts, pen_point* pts)
{
	if(ps != nil) {
		ps->npts = npts;
		ps->pts = pts;
...
void
delete_Stroke_array(uint size, Stroke* ps, bool delete_points_p)
{
	int i;
...
gesture*
initialize_gesture(gesture* g, char* name, uint nhs, pen_point* hspots,
...
{
	initialize_Stroke(ps1, ps2->npts, ps2->pts);
	return (ps1);
}

Stroke*
copy_Stroke_array(uint nstrokes, Stroke* strokes)
{
	int i;
...
Stroke*
concatenate_Strokes(int nstrokes1, Stroke* strokes1, int nstrokes2,
                    Stroke* strokes2, int* nstrokes3, Stroke** strokes3)
{
	int i;
	int ns;
	Stroke* ps;
