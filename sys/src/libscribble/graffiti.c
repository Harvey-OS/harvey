...
char* cl_name[3] = {DEFAULT_LETTERS_FILE, DEFAULT_DIGITS_FILE,
                    DEFAULT_PUNC_FILE};
...
recognize(Scribble* s)
{
	...
	c = recognizer_translate(
	    graf->rec[s->puncShift ? CS_PUNCTUATION : s->curCharSet], 1, ps,
	    false, &nr, &ret);
	if(c != -1)
...
graffiti_load_recognizers(struct graffiti* pg)
{
	...
	for(i = 0; i < NUM_RECS; i++) {
		...
		if(pg->rec[i] == nil) {
			fprint(2, "Error loading recognizer from %s.",
			       DEFAULT_REC_DIR);
			return 0;
		}
	...
	if((homedir = (char*)getenv("home")) == nil) {
		...
		strecpy(pg->cldir, pg->cldir + sizeof pg->cldir,
		        REC_DEFAULT_USER_DIR);
		usingDefault = true;
	} else {
		if(0)
			fprint(2, "homedir = %s\n", homedir);
		snprint(pg->cldir, sizeof pg->cldir, "%s/%s", homedir,
		        CLASSIFIER_DIR);
		usingDefault = false;
	}
	...
	for(i = 0; i < NUM_RECS; i++) {
		...
		rec_return =
		    recognizer_load_state(pg->rec[i], pg->cldir, cl_name[i]);
		if((rec_return == -1) && (usingDefault == false)) {
			if(0)
				fprint(2, "Unable to load custom classifier "
				          "file %s/%s.\nTrying default "
				          "classifier file instead.\nOriginal "
				          "error: %s\n ",
				       pg->cldir, cl_name[i],
				       (s = recognizer_error(pg->rec[i]))
				           ? s
				           : "(none)");
			rec_return = recognizer_load_state(
			    pg->rec[i], REC_DEFAULT_USER_DIR, cl_name[i]);
		}
		if(rec_return == -1) {
			fprint(2, "Unable to load default classifier file "
			          "%s.\nOriginal error: %s\n",
			       cl_name[i],
			       (s = recognizer_error(pg->rec[i])) ? s
			                                          : "(none)");
			return 0;
		}
	}
	...
	if((pg->rec_getClasses =
	        (li_recognizer_getClasses)fns[LI_GET_CLASSES]) == nil) {
		fprint(2, "LI Recognizer Training:li_recognizer_getClasses() "
		          "not found!");
		if(fns != nil) {
			free(fns);
		}
