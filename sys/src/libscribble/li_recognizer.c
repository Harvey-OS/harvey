...
static void free_rClassifier(rClassifier* rc);
	...
	for(k = 0; k < nclss; k++) {
		...
		for(i = 0; i < nex; i++) {
			...
			if(fscanf(fd, "%d", &npts) != 1)
				goto unallocate; /*Boy would I like
				                    exceptions!*/
			...
			if((examples[k] =
			        add_example(examples[k], npts, pts)) == nil) {
				delete_pen_point_array(pts);
				goto unallocate;
			}
...
static int
read_classifier(FILE* fd, rClassifier* rc)
{
...
recognizer_getClasses(recognizer r, char*** list, int* nc)
{
	...
	*nc = nclasses = rec->li_rc.nclasses;
	...
	for(i = 0; i < nclasses; i++)
		ret[i] =
		    rec->li_rc.cnames[i]; /* only the 1st char of the cname */
	*list = ret;
	return 0;
}
...
static int
recognizer_train(recognizer r, rc* rc, uint u, Stroke* s, rec_element* re,
                 bool b)
{
	/*This operation isn't supported by the LI recognizer.*/
...
int
li_recognizer_get_example(recognizer r, int class, int instance, char** name,
                          pen_point** points, int* npts)
{
...
static int
li_recognizer_load(recognizer r, char* dir, char* filename)
{
	FILE* fd;
	char* pathname;
	li_recognizer* rec;
...
static int
li_recognizer_save(recognizer r, char* c, char* d)
{
	/*This operation isn't supported by the LI recognizer.*/
...
static int
li_recognizer_translate(recognizer r, uint ncs, Stroke* tps, bool b, int* nret,
                        rec_alternative** ret)
{
	char* clss;
	li_recognizer* rec;
...
RECOGNIZER_INITIALIZE(ri)
{
	...
	*/
	...
	r->recognizer_delete_from_dictionary =
	    li_recognizer_delete_from_dictionary;
	...
	r->recognizer_set_gesture_action = li_recognizer_set_gesture_action;
...
static void
lialg_initialize(rClassifier* rec)
{
	int i;
...
lialg_compute_dominant_points(point_list* points)
{
	...
	if(lidebug) {
		...
		for(j = 0; j < dpts->npts; j++) {
			fprint(2, "%P (%lud)  ", dpts->pts[j].Point,
			       dpts->pts[j].chaincode);
		}
		fprint(2, "\n");
		fflush(stderr);
	}
...
static void
lialg_bresline(pen_point* startpt, pen_point* endpt, point_list* newpts, int* j)
{
	...
	dX = abs(Bx -
	         Ax); /* store the change in X and Y of the line endpoints */
	dY = abs(By - Ay);
	...
	/* DETERMINE INDEPENDENT VARIABLE (ONE THAT ALWAYS INCREMENTS BY 1 (OR
	 * -1) ) */
	/* AND INITIATE APPROPRIATE LINE DRAWING ROUTINE (BASED ON FIRST OCTANT
	 */
	/* ALWAYS). THE X AND Y'S MAY BE FLIPPED IF Y IS THE INDEPENDENT
	 * VARIABLE. */
	if(dX >= dY) {             /* if X is the independent variable */
		int dPr = dY << 1; /* amount to increment decision if right is
		                      chosen (always) */
		int dPru =
		    dPr -
		    (dX
		...
		for(; dX >= 0; dX--) {
			...
			if(P > 0) { /* is the pixel	going right AND	up? */
				Ax +=
				    Xincr; /* increment independent variable */
		...
		int dPr = dX << 1; /* amount to increment decision if right is
		                      chosen (always) */
		int dPru =
		    dPr -
		    (dY
		...
		for(; dY >= 0; dY--) {
			...
			if(P > 0) { /* is the pixel going up AND right? */
				...
				Ay +=
				    Yincr; /* increment independent variable */
				P += dPru; /* increment decision (for up) */
...
lialg_compute_regions(point_list* pts)
{
	...
	/* Allocate a 2 x pts->npts array for use in computing the (filtered)
	 * Angle set, A_n. */
	...
	for(j = 0; j < LP_FILTER_ITERS; j++, curr = R[j], next = R[j + 1]) {
		for(i = 0; i < pts->npts; i++) {
			...
			for(k = i - LP_FILTER_WIDTH; k <= i + LP_FILTER_WIDTH;
			    k++) {
				int oldval =
				    (k < 0 || k >= pts->npts) ? 18000 : curr[k];
				next[i] +=
				    oldval *
				    lialg_lpfwts
				        [k -
				         (i - LP_FILTER_WIDTH)]; /* overflow? */
			}
	...
	for(i = 0; i < pts->npts; i++)
		next[i] =
		    (abs(curr[i] - 18000) < LP_FILTER_THLD) ? 18000 : curr[i];
	curr = next;

	/* Debugging. */
	if(lidebug > 1) {
		for(i = 0; i < pts->npts; i++) {
			fprint(2, "%3d:  (%P)  %lud  ", i, pts->pts[i].Point,
			       pts->pts[i].chaincode);
...
#define RGN_TYPE(val)                                                          \
	(((val) == 18000) ? RGN_PLAIN                                          \
		...
		for(curr_reg = regions; curr_reg; curr_reg = curr_reg->next)
			if(curr_reg->type == RGN_PLAIN) {
				...
				for(next_reg = curr_reg->next;
				    next_reg != nil &&
				        (next_reg->end - next_reg->start) <
				            LP_FILTER_MIN;
				    next_reg = curr_reg->next) {
					/* next_reg must not be plain, and it
					 * must be followed by a plain */
					/* assert(next_reg->type != RGN_PLAIN);
					 */
					/* assert(next_reg->next != nil &&
					 * (next_reg->next)->type == RGN_PLAIN);
					 */
		...
		/* Add-in pseudo-extremes. */
		{
			...
			regions = nil;
			prev_reg = nil;
			for(curr_reg = tmp; curr_reg;
			    curr_reg = curr_reg->next) {
				if(curr_reg->type == RGN_PLAIN) {
					int arclen =
					    lialg_compute_pathlen_subset(
					        pts, curr_reg->start,
					        curr_reg->end);
					...
					if(arclen >= PE_AL_THLD &&
					   atcr >= PE_ATCR_THLD) {
						int mid = curr_reg->start +
						          (curr_reg->end -
						           curr_reg->start) /
						              2;
						int end = curr_reg->end;
						region_list* saved_next =
						    curr_reg->next;
						...
						/* curr_reg = (region_list
						 * *)safe_malloc(sizeof(region_list));*/
						curr_reg =
						    malloc(sizeof(region_list));
						curr_reg->start = mid;
						curr_reg->end = mid;
...
static point_list*
lialg_compute_dompts(point_list* pts, region_list* regions)
{
	point_list* dpts;
	int ndpts;
	int* cas;
	...
	/* Dominant points include:  start_pt, end_pt,
	 * extrema_of_non_plain_regions, midpts of the preceding. */
	nonplain = 0;
	...
	for(curr = regions; curr != nil; curr = curr->next)
		if(curr->type != RGN_PLAIN) {
			...
			/* Record midpoint. */
			dpts->pts[dp++] =
			    pts->pts[previx + (currix - previx) / 2];
...
static int*
lialg_compute_contour_angle_set(point_list* pts, region_list* regions)
{
	int* V;
	region_list* curr_reg;
	int i;
 ...
 */
static void
lialg_score_stroke(point_list* input_dompts, point_list* curr_dompts, int* sim,
                   int* dist)
{
	*sim = MIN_SIM;
	*dist = MAX_DIST;
...
static int
lialg_compute_similarity(point_list* input_dompts, point_list* curr_dompts)
{
	int sim;
	point_list* A, *B;
	int N, M;
...
static int
lialg_compute_distance(point_list* input_dompts, point_list* curr_dompts)
{
	int dist;
	point_list* A, *B;
	int N, M;
...
static int
lialg_read_classifier_digest(rClassifier* rec)
{
	int nclasses;
	FILE* fp;
...
static int
lialg_canonicalize_examples(rClassifier* rec)
{
	int i;
	int nclasses;
	...
	for(nclasses = 0;
	    nclasses < MAXSCLASSES && rec->cnames[nclasses] != nil; nclasses++)
		;
	...
	for(i = 0; i < nclasses; i++) {
		...
		for(nex = 0; tmp != nil; nex++, tmp = tmp->next) {
			if((pts = add_example(pts, tmp->npts, tmp->pts)) ==
			   nil) {
				delete_examples(pts);
				return (-1);
			}
		...
		for(j = 0, tmp = pts; j < nex; j++, tmp = tmp->next) {
			if(lialg_canonicalize_example_stroke(tmp) != 0) {
				if(lidebug) {
					fprint(2, "lialg_canonicalize_example_"
					          "stroke returned error\n");
				}
				return (-1);
			}

			if(tmp->xrange > maxxrange)
				maxxrange = tmp->xrange;
			if(tmp->yrange > maxyrange)
				maxyrange = tmp->yrange;
		}

		/* Normalize max ranges. */
		if(((100 * maxxrange + CANONICAL_X / 2) / CANONICAL_X) >
		   ((100 * maxyrange + CANONICAL_Y / 2) / CANONICAL_Y)) {
			maxyrange = (maxyrange * CANONICAL_X + maxxrange / 2) /
			            maxxrange;
			maxxrange = CANONICAL_X;
		} else {
			maxxrange = (maxxrange * CANONICAL_Y + maxyrange / 2) /
			            maxyrange;
		...
		for(j = 0, tmp = pts; j < nex; j++, tmp = tmp->next) {
			int scalex = (tmp->xrange == 0)
			                 ? 100
			                 : (100 * maxxrange + tmp->xrange / 2) /
			                       tmp->xrange;
			int scaley = (tmp->yrange == 0)
			                 ? 100
			                 : (100 * maxyrange + tmp->yrange / 2) /
			                       tmp->yrange;
			if(lialg_translate_points(tmp, 0, 0, scalex, scaley) !=
			   0) {
				delete_examples(pts);
				return (-1);
			}
		...
		if(lialg_translate_points(avg, avgxoff, avgyoff, 100, 100) !=
		   0) {
			delete_examples(pts);
			return (-1);
		}

		/* Create a point list to serve as the ``canonical
		 * representation. */
		if((rec->canonex[i] = add_example(nil, avg->npts, avg->pts)) ==
		   nil) {
			delete_examples(pts);
			return (-1);
		}
...
static int
lialg_canonicalize_example_stroke(point_list* points)
{
	int minx, miny, maxx, maxy, xrange, yrange, scale;

	/* Filter out points that are too close. */
	if(lialg_filter_points(points) != 0)
		return (-1);
	...
	if(points->npts < 2) {
		if(lidebug) {
			fprint(2,
			       "lialg_canonicalize_example_stroke: npts=%d\n",
			       points->npts);
		}
		return (-1);
	}
	...
	if(lialg_translate_points(points, minx, miny, scale, scale) != 0) {
		if(lidebug) {
			fprint(2, "lialg_translate_points "
			          "(minx=%d,miny=%d,scale=%d) returned error\n",
			       minx, miny, scale);
		}
		return (-1);
	}
	...
	if(lialg_compute_equipoints(points) != 0)
		return (-1);
	...
	if(lialg_translate_points(points, minx, miny, 100, 100) != 0) {
		if(lidebug) {
			fprint(2, "lialg_translate_points (minx=%d,miny=%d) "
			          "returned error\n",
			       minx, miny);
		}
		return (-1);
...
static int
lialg_compute_equipoints(point_list* points)
{
	...
	if(lidebug) {
		fprint(2, "compute_equipoints:  npts = %d, pathlen = %d, "
		          "equidist = %d\n",
	...
	for(i = 1; i < points->npts; i++) {
		...
		while(remaining_seglen >= dist_to_next_eqpt) {
			if(dx1 == 0) {
				...
				int dx =
				    (100 * dist_to_next_eqpt + tmp / 2) / tmp;
	...
	if(nequipoints == NCANONICAL) {
		...
		if(lidebug) {
			fprint(2,
			       "lialg_compute_equipoints: nequipoints = %d\n",
			       nequipoints);
		}
		/*	assert(false);*/
		return (-1);
...
/* Result is x 100. */
static int
lialg_compute_pathlen(point_list* points)
{
...
/* Result is x 100. */
static int
lialg_compute_pathlen_subset(point_list* points, int start, int end)
{
	int pathlen;
	int i;
...
static int
lialg_filter_points(point_list* points)
{
	int filtered_npts;
...
/* Note that this does NOT update points->xrange and points->yrange! */
static int
lialg_translate_points(point_list* points, int minx, int miny, int scalex,
                       int scaley)
{
	int i;
	...
	for(i = 0; i < points->npts; i++) {
		points->pts[i].x =
		    ((points->pts[i].x - minx) * scalex + 50) / 100;
		points->pts[i].y =
		    ((points->pts[i].y - miny) * scaley + 50) / 100;
	}
...
static void
lialg_get_bounding_box(point_list* points, int* pminx, int* pminy, int* pmaxx,
                       int* pmaxy)
{
	int minx, miny, maxx, maxy;
	int i;
...
static void
lialg_compute_lpf_parameters(void)
{
	int i;
...
static int
likeatan(int tantop, int tanbot)
{
	int t;
