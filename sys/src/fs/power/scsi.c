...
struct Target
{
	...
	int	status;
	int	probe;
	Drive	drv;			/* Kens idea of this device */
...
static  int	scsiexec(Target*, int, uchar*, int, void*, int);
...
reqsense(Target *tp)
{
	...
	status = scsiexec(tp, SCSIread, cmd, sizeof(cmd), sense, sizeof(sense));
	if(status != 0x6000)
		return status;
	...
	print("scsi reqsense %D: key #%2.2ux code #%2.2ux #%2.2ux\n",
		tp->drv.dev, sense[2], sense[12], sense[13]);
...
scsiio(Device d, int rw, uchar *cmd, int clen, void *data, int dlen)
{
	...
	s = scsiexec(tp, rw, cmd, clen, data, dlen);
/*	print("%D: cmd #%2.2lux #%4.4lux\n", d, cmd[0], s);	/**/
	if(s == 0x6000)
		s = 0;
	return s;
}
...
scsiprobe(Target *c, Device d)
{
	...
	for(i = 0; i < NTarget; i++) {
		if(i == CtlrID) {
			c[i].drv.status = Dself;
			continue;
		}
		tp = &c[i];
		tp->drv.status = Dready;

		/* Test unit ready */
		memset(cmd, 0, sizeof(cmd));
		d.unit = i;
		status = scsiexec(tp, SCSIread, cmd, sizeof(cmd), 0, 0);
		if(status == 0x6000)
			continue;
		status = scsiexec(tp, SCSIread, cmd, sizeof(cmd), 0, 0);
		if(status == 0x6000)
		 ...
		 * is simply not there
		 */
		tp->drv.status = Dunavail;
		if(reqsense(tp) != 0)
			tp->drv.status = Dnready;
	}
}
...
scsidrive(Device d)
{
	...
	if(d.ctrl >= NCtlr || d.unit >= NTarget || d.unit == CtlrID)
		return 0;

	c = &target[d.ctrl][d.unit];
	c->drv.dev = d;
	if(c->probe == 0) {
		scsiprobe(c, d);
		c->probe = 1;
 ...
 */
void
resetscsi(int ctlrnr, uchar *data, uchar *addr, ulong *cnt, uchar *fls, ulong map)
{
	int i;
	uchar status;
	Target *ctlr, *tp;
...
scsiinit(void)
{
	...
	resetscsi(0, SCSI0DATA, SCSI0ADDR, SCSI0CNT, SCSI0FLS, 0x1fc0);
	resetscsi(1, SCSI1DATA, SCSI1ADDR, SCSI1CNT, SCSI1FLS, 0x1f80);

	cmd_install("statd", "-- scsi stats", cmd_statd);
}
