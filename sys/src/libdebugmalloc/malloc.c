...
static void		plogstack(Pool*);

typedef struct Private Private;
struct Private {
	char		msg[128];	/* a rock under which to leave messages to be printed at unlock */
	Lock		lk;
...
static Pool sbrkmem = {
	...
	.alloc=		sbrkalloc,
	.merge=		sbrkmerge,
	.flags=		POOL_PARANOIA|POOL_LOGGING|POOL_ANTAGONISM,
...
plogstack(Pool *p)
{
	static char buf[8*8+20];
	char *q;
	static ulong pc[8];
	int i, n;
	extern int _malloc_getcallers(ulong*, int);	/* arch-specific */

	n = _malloc_getcallers(pc, nelem(pc));
	q = buf;
	for(i=0; i<n; i++) 
		q += sprint(q, " %.8lux", pc[i]);
	p->print(p, "#stack%s\n", buf);
}
