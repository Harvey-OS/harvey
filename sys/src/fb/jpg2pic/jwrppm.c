 ...
 *
 * This file used to contain routines to write output images in PPM/PGM format.
 * Mangled by td to write TYPE=dump files, which are very similar
 */
...
output_init (decompress_info_ptr cinfo)
{
  if (cinfo->out_color_space == CS_GRAYSCALE) {
    ...
    fprintf(cinfo->output_file, "TYPE=dump\n");
    fprintf(cinfo->output_file, "WINDOW=0 0 %d %d\n",
	cinfo->image_width, cinfo->image_height);
    fprintf(cinfo->output_file, "NCHAN=1\n");
    fprintf(cinfo->output_file, "CHAN=m\n");
    if(outputname!=0) fprintf(cinfo->output_file, "NAME=%s\n", outputname);
    fprintf(cinfo->output_file, "\n");
#ifndef USE_PUTC_OUTPUT
    ...
    fprintf(cinfo->output_file, "WINDOW=0 0 %d %d\n",
	cinfo->image_width, cinfo->image_height);
    fprintf(cinfo->output_file, "NCHAN=3\n");
    fprintf(cinfo->output_file, "CHAN=rgb\n");
    if(outputname!=0) fprintf(cinfo->output_file, "NAME=%s\n", outputname);
    fprintf(cinfo->output_file, "\n");
#ifndef USE_PUTC_OUTPUT
