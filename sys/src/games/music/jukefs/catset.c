...
{
	if(debug)
		fprint(2, "catsetrealloc %p %d (%p %d)", cs, sz, cs->bitpiece, cs->nbitpiece);
	if(sz > cs->nbitpiece) {
...
{
	if(debug)
		fprint(2, "catsetcopy %p %p ", dst, src);
	if(debug)
		catsetprint(2, dst);
	if(debug)
		fprint(2, " ");
...
catsetorset(Catset *dst, Catset *src)
{
	...
	if(debug)
		fprint(2, "catsetorset %p %p ", dst, src);
	if(debug)
		catsetprint(2, dst);
	if(debug)
		fprint(2, " ");
