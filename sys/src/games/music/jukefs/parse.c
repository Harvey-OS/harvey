	...
	for(;;) {
		...
		if(t >= 0)
			...
			catcase:
				nt = gettoken(token);
				if(nt != Equals)
					sysfatal("Expected Equals, not %s",
					         token);
				nt = gettoken(token);
				if(nt != Txt)
					sysfatal("Expected Text, not %s",
					         token);
				if((p = strchr(token, '\n')))
					*p = 0;
				p = token;
				if(o->type == Category) {
					if(catsetisset(&o->categories)) {
						fprint(2, "Category object "
						          "must have one "
						          "category\n");
					}
					catsetcopy(&o->categories,
					           &tokenlist[t].categories);
					strncpy(o->key, p, KEYLEN);
					if(catobjects[t] == 0)
						sysfatal(
						    "Class %s not yet defined",
						    tokenlist[t].name);
					for(i = 0; i < catobjects[t]->nchildren;
					    i++)
						if(strcmp(catobjects[t]
						              ->children[i]
						              ->key,
						          p) == 0)
							break;
					if(i == catobjects[t]->nchildren) {
						/* It's a new key for the
						 * category */
						addchild(catobjects[t], o,
						         "new key for cat");
					} else {
						/* Key already existed */
					...
					if(i == catobjects[t]->nchildren) {
						...
						*/
						...
						catsetcopy(
						    &oo->categories,
						    &tokenlist[t].categories);
						addchild(catobjects[t], oo,
						         "catobjects[t],oo");
					}
					addchild(catobjects[t]->children[i], o,
					         "children[i]");
				}
				break;
			}
			...
			case Eof:
				...
				sysfatal("Unexpected Eof in %s, file %s",
				         tokenlist[o->type].name, file);
			case Newcat:
				/* New category, make an entry in the tokenlist
				 */
				tokenlist = realloc(
				    tokenlist, (ntoken + 1) * sizeof(Token));
				ot = &tokenlist[ntoken];
			...
			case BraceC:
				while(tp > textbuf && tp[-1] == '\n')
					*--tp = 0;
				if((o->type == File || o->type == Include) &&
				   o->path) {
					o->value =
					    smprint("%s/%s", o->path, textbuf);
				} else if(tp > textbuf) {
				...
				case Path:
					...
					if(p[0] == '/' || o->path == nil) {
						...
						setmalloctag(o->parent->path,
						             0x100004);
					} else {
						o->parent->path = smprint(
						    "%s/%s", o->path, p);
						setmalloctag(o->parent->path,
						             0x100005);
					}
...
getinclude(Object* o)
{
	...
	while((t = gettoken(token)) != Eof) {
		if(t < 0) {
			if(*dirname)
				sysfatal(
				    "Bad include file %s/%s, token %s, str %d",
				    dirname, filename, token, str);
			else
				sysfatal(
				    "Bad include file %s, token %s, str %d",
				    filename, token, str);
		}
		free(o->path);
...
addchild(Object* parent, Object* child, char* where)
{
	...
	if(0)
		fprint(2,
		       "addcatparent %s parent %d type %d child %d type %d\n",
		       where, parent->tabno, parent->type, child->tabno,
		       child->type);
