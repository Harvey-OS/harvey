...
printparentage(char* lp, int ln, Object* o)
{
	...
	*lp = '\0';
	...
	if(0)
		fprint(2,
		       "parentage 0x%p %d type %d value 0x%p parent 0x%p %d\n",
		       o, o->tabno, o->type, o->value, o->parent,
		       o->parent->tabno);
	if(o->orig) {
		if(0)
			fprint(2, "parentage 0x%p %d type %d orig %d type %d "
			          "parent 0x%p %d\n",
			       o, o->tabno, o->type, o->orig->tabno,
			       o->orig->type, o->orig->parent,
			       o->orig->parent->tabno);
		o = o->orig;
	}
	sln = ln;
	...
	if(o->value && *o->value && ln > 0) {
		if(o->type == Category) {
			if(o->parent == root) {
				...
				for(k = Ntoken; k < ntoken; k++)
					if(catseteq(&o->categories,
					            &tokenlist[k].categories)) {
						r = snprint(lp, ln, "%s: ",
						            tokenlist[k].name);
						lp += r;
						ln -= r;
		...
		if(0)
			fprint(2, "parentage 0x%p %d type %d no value\n", o,
			       o->tabno, o->type);
	}
	for(i = 0; i < o->nchildren && ln > 0; i++)
		...
		case Lyrics:
			r = snprint(lp, ln, "%s: ",
			            tokenlist[o->children[i]->type].name);
			lp += r;
			ln -= r;
			if(ln <= 0)
				break;
