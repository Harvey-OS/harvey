	...
	for(;;) {
		...
		while(q = strchr(p, '\n')) {
			*q = 0;
			...
			if(nf) {
				playlist.entry =
				    realloc(playlist.entry,
				            (playlist.nentries + 1) *
				                sizeof playlist.entry[0]);
				...
				updateplaylist(
				    0); // this will also update nentries
			}
			q++;
			n -= q - p;
			p = q;
...
playctlproc(void* a)
{
	...
	for(;;) {
		...
		case 3:
			chanprint(chan, "playctlproc: error %lud %q",
			          strtoul(fields[1], nil, 0), fields[2]);
			if(strcmp(fields[0], "error") == 0)
				break;
		// fall through
		case 2:
			chanprint(chan, "playctlproc: %s %lud", fields[0],
			          strtoul(fields[1], nil, 0));
		}
	}
}
...
volumeproc(void* arg)
{
	...
	for(;;) {
		...
		for(i = 0; i < nlines; i++) {
			...
			if(nf == 0)
				continue;
			if(nf != 6 || strcmp(fields[0], "volume") ||
			   strcmp(fields[1], "out"))
				continue;
