...
char* playprog[] = {
        [Pac] = "/bin/games/pac4dec", [Mp3] = "/bin/games/mp3dec",
        [Pcm] = "/bin/cp", [Ogg] = "/bin/games/vorbisdec",
};
...
pac4dec(void* a)
{
	...
	if(fd < 0) {
		...
		pb->cmd = Error;
		pb->off = 0;
		pb->len = snprint(pb->data, sizeof(pb->data),
		                  "startplay: %s: %r", pfd->filename);
		sendp(full, pb);
		threadexits("open");
	}
	dup(pfd->fd, 1);
	...
	pb->cmd = Error;
	pb->off = 0;
	pb->len = snprint(pb->data, sizeof(pb->data), "startplay: %s: exec",
	                  playprog[type]);
	sendp(full, pb);
	...
	for(;;) {
		...
		case 0:
			...
			if(pb->len < 0) {
				...
				/* Simple end of file */
				sendp(empty,
				      pb); /* Don't need buffer after all */
			}
			close(fd);
			fd = -1;
			if(debug & DbgPlayer)
		...
		case 1:
			if((debug & DbgPac) && newstate.cmd)
				fprint(2, "Pacproc: newstate %s-%d, playstate "
				          "%s-%d\n",
			...
			switch(newstate.cmd) {
			default:
				sysfatal("pacproc: unexpected newstate %d",
				         newstate.cmd);
			case Stop:
...
pcmproc(void*)
{
	...
	Pacbuf* pb, *b;
	Alt a[3] = {
	    {full, &pb, CHANRCV}, {playout, &pb, CHANRCV}, {nil, nil, CHANEND},
	};

	/*
	 * This is the real-time proc.
	 * It gets its input from two sources, full data/control buffers from
	 * the pacproc
	 * which mixes decoded data with control messages, and data buffers from
	 * the pcmproc's
	 * (*this* proc's) own internal playout buffer.
	 * When a command is received on the `full' channel containing a command
	 * that warrants
	 * an immediate change of audio source (e.g., to silence or to another
	 * number), we just
	 * toss everything in the pipeline -- i.e., the playout channel
	 * Finally, we report all state changes using `playupdate' (another
	 * message channel)
	 */
	...
	for(;;) {
		...
		case 0:
			...
			if((debug & DbgPcm) && localstate.m != prevstate.m) {
				fprint(
				    2,
				    "pcm, full: %s-%d, local state is %s-%d\n",
			...
			switch(pb->cmd) {
			default:
				sysfatal("pcmproc: unknown newstate: %s-%d",
				         statetxt[pb->cmd], pb->off);
			case Resume:
				a[1].op = CHANRCV;
				newstate.cmd = Play;
			...
			case Stop:
				...
				break;
			case Skip:
				/* Dump all data in the buffer, then fall
				 * through */
				while(b = nbrecvp(playout))
					if(b->cmd == Error) {
		...
		case 1:
			...
			if((debug & DbgPlayer) && localstate.m != prevstate.m) {
				fprint(2, "pcm, playout: %s-%d, local state is "
				          "%s-%d\n",
				       statetxt[pb->cmd], pb->off,
				       statetxt[localstate.cmd],
				       localstate.off);
				prevstate.m = localstate.m;
			}
			switch(pb->cmd) {
