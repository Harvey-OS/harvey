...
File files[] = {
        ...
        [Qplayctl] =
            {.dir = {0, 0, {Qplayctl, 0, QTFILE}, 0666, 0, 0, 0, "playctl"}},
        [Qplaylist] = {.dir = {0,
                               0,
                               {Qplaylist, 0, QTFILE},
                               0666 | DMAPPEND,
                               ...
                               "playlist"}},
        [Qplayvol] =
            {.dir = {0, 0, {Qplayvol, 0, QTFILE}, 0666, 0, 0, 0, "playvol"}},
        [Qplaystat] =
            {.dir = {0, 0, {Qplaystat, 0, QTFILE}, 0444, 0, 0, 0, "playstat"}},
};
...
char* statetxt[] = {[Nostate] = "panic!", [Error] = "error",
                    [Stop] = "stop",      [Pause] = "pause",
                    [Play] = "play",      [Resume] = "resume",
                    [Skip] = "skip",      nil};

// low-order bits: position in play list, high-order: play state:
Pmsg playstate = {Stop, 0};

char* rflush(Worker*), *rauth(Worker*), *rattach(Worker*), *rwalk(Worker*),
    *ropen(Worker*), *rcreate(Worker*), *rread(Worker*), *rwrite(Worker*),
    *rclunk(Worker*), *rremove(Worker*), *rstat(Worker*), *rwstat(Worker*),
    *rversion(Worker*);
...
rread(Worker* w)
{
	...
	if(f->file == files + Qplaystat) {
		p = getplaystat(r->ofcall.data,
		                r->ofcall.data + sizeof r->indata);
	...
	while(f->vers == f->file->dir.qid.vers && (f->flags & Eof)) {
		/* Wait until file state changes (f->file->dir.qid.vers is
		 * incremented) */
...
rwrite(Worker* w)
{
	...
	if(f->file == &files[Qplayctl]) {
		...
		playlist.data =
		    realloc(playlist.data, playlist.ndata + cnt + 2);
		if(playlist.data == 0)
			sysfatal("realloc: %r");
		...
		while(*p) {
			playlist.lines = realloc(playlist.lines,
			                         (playlist.nlines + 1) *
			                             sizeof(playlist.lines[0]));
...
rstat(Worker* w)
{
	...
	if(r->fid == nil)
		return Enotexist;
	r->ofcall.nstat =
	    convD2M(&r->fid->file->dir, r->indata, messagesize - IOHDRSZ);
...
playlistsrv(void*)
{
	...
	while(recv(playlistreq, &m)) {
		...
		if(m.off < playlist.nlines) {
			...
			if(q == nil)
				sysfatal(
				    "playlistsrv: no newline character found");
			n = q - p;
			r = malloc(n + 1);
			memmove(r, p, n);
			r[n] = 0;
