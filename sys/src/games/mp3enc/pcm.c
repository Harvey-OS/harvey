...
octetstream_t*
octetstream_open(OUT size_t size)
{
...
int
octetstream_resize(INOUT octetstream_t* const os, OUT size_t size)
{
...
int
octetstream_close(INOUT octetstream_t* const os)
{
...
static inline int
lame_encode_frame(INOUT lame_t* lame, OUTTR sample_t** inbuf,
                  IN uint8_t* mp3buf, OUT size_t mp3buf_size)
{
	int ret;
#if 1
	int i;
	...
	case coding_MPEG_Layer_1:
		ret = lame_encode_mp1_frame(lame->global_flags, inbuf[0],
		                            inbuf[1], mp3buf, mp3buf_size);
		break;
#endif
#ifdef HAVE_MPEG_LAYER2
	case coding_MPEG_Layer_2:
		ret = lame_encode_mp2_frame(lame->global_flags, inbuf[0],
		                            inbuf[1], mp3buf, mp3buf_size);
		break;
#endif
	case coding_MPEG_Layer_3:
		ret = lame_encode_mp3_frame(lame->global_flags, inbuf[0],
		                            inbuf[1], mp3buf, mp3buf_size);
		break;
#ifdef HAVE_MPEG_PLUS
	case coding_MPEG_plus:
		ret = lame_encode_mpp_frame(lame->global_flags, inbuf[0],
		                            inbuf[1], mp3buf, mp3buf_size);
		break;
#endif
#ifdef HAVE_AAC
	case coding_MPEG_AAC:
		ret = lame_encode_aac_frame(lame->global_flags, inbuf[0],
		                            inbuf[1], mp3buf, mp3buf_size);
		break;
#endif
#ifdef HAVE_VORBIS
	case coding_Ogg_Vorbis:
		ret = lame_encode_ogg_frame(lame->global_flags, inbuf[0],
		                            inbuf[1], mp3buf, mp3buf_size);
		break;
#endif
	default:
...
/*
 * Now there are following the so called peek functions. They got a pointer and
 *returning
 * the data to this pointer as a sample. The size of the memory object can be
 *from
 * 8 up to 80 bits. One sample must be fully determined by it's bits, not by the
 *history
 * or by other channels or things like that. Also the input must have a integral
 *byte size.
 *
 * That means:
 *
...
/*
 *  These macros get 1...4 bytes and calculating a value between -32768 and
 * +32768.
...
#define BYTES3(x1, x2, x3)                                                     \
	((((int8_t)(x1)) << 8) + (uint8_t)(x2) + 1 / 256. * (uint8_t)(x3))
#define BYTES4(x1, x2, x3, x4)                                                 \
	((((int8_t)(x1)) << 8) + (uint8_t)(x2) + 1 / 256. * (uint8_t)(x3) +    \
	 1 / 65536. * (uint8_t)(x4))
 ...
 * Also note: peek function got their data from a variable called src, which is
 * a pointer
 * to const unsigned char.
 */

static inline sample_t
read_opposite_float32(OUT uint8_t* const src)
{
	uint8_t tmp[4];
	tmp[0] = src[3];
	tmp[1] = src[2];
...
static inline sample_t
read_opposite_float64(OUT uint8_t* const src)
{
	uint8_t tmp[8];
	tmp[0] = src[7];
	tmp[1] = src[6];
...
static sample_t
read_float80_le(OUT uint8_t* const src)
{
	int sign = src[9] & 128 ? -1 : +1;
	int32_t exponent = (src[9] & 127) * 256 + src[8] - 16384 - 62;
	long double mantisse =
	    src[7] * 72057594037927936.L + src[6] * 281474976710656.L +
	    src[5] * 1099511627776.L + src[4] * 4294967296.L +
	    src[3] * 16777216.L + src[2] * 65536.L + src[1] * 256.L + src[0];
	return sign * ldexp(mantisse, exponent);
}

static sample_t
read_float80_be(OUT uint8_t* const src)
{
	int sign = src[0] & 128 ? -1 : +1;
	int32_t exponent = (src[0] & 127) * 256 + src[1] - 16384 - 62;
	long double mantisse =
	    src[2] * 72057594037927936.L + src[3] * 281474976710656.L +
	    src[4] * 1099511627776.L + src[5] * 4294967296.L +
	    src[6] * 16777216.L + src[7] * 65536.L + src[8] * 256.L + src[9];
	return sign * ldexp(mantisse, exponent);
}

static inline sample_t
read_opposite_float80(OUT uint8_t* const src)
{
	uint8_t tmp[10];
	tmp[0] = src[9];
	tmp[1] = src[8];
...
/*
 *  Now we are building all resorting functions. The macro wants the name of the
 *function
 *  and a peek function or macro. The created function wants the following
 *input:
 *
 *     destination: Where to store the result as sample_t vector? The distance
 *between
 *                  the elements is sizeof(sample_t)
 *     source:      Where to get the bytes which should converted to destination
 *     step size:   The distance between the input samples. This is at least the
 *size
...
typedef void (*demux_t)(IN sample_t* dst, OUT uint8_t* src, OUT ssize_t step,
                        OUT size_t len);

#define FUNCTION(name, expr)                                                   \
	static void name(IN sample_t* dst, OUT uint8_t* src, OUT ssize_t step, \
	                 OUT size_t len)                                       \
	{                                                                      \
		size_t i = len;                                                \
		do {                                                           \
...
/* ... and some are not or there are faster but byte order dependent
 * possiblities */
 ...
 *  The demultiplexers
 *    - 1st argument is the destination of the data converted to sample_t (and
 * normalized to +/-32768)
 *    - 2nd argument is a pointer to the source data, in any format
 *    - 3rd argument is 1st argument of the table (multiplied by the channel
...
static inline int
select_demux(OUT uint32_t mode, IN demux_t* retf, IN ssize_t* size)
{
	int big = mode >> 24;
	...
	*size = tabptr->size;
	*retf = big & 1 ? tabptr->demultiplexer_be : tabptr->demultiplexer_le;
...
static inline int
internal_lame_encoding_pcm(INOUT lame_t* const lame,
                           INOUT octetstream_t* const os,
                           OUT sample_t* const* const data, OUT size_t len)
{
	size_t i;
	double ampl;
	double dampl;
...
#error FFTOFFSET is greater than BLKSIZE, see <encoder.h>
	...
	mf_needed =
	    BLKSIZE + lame->frame_size - FFTOFFSET; // amount needed for FFT
	mf_needed = MAX(
	    mf_needed,
	    286 +
	        576 * (1 + lame->mode_gr)); // amount needed for MDCT/filterbank
	...
	if(lame->ampl != ampl) {
		if(remaining <= 1) { // constant amplification
			...
			ampl_on = 2;
			dampl = (lame->ampl - ampl) / remaining *
			        lame->sampfreq_out / lame->sampfreq_in;
		}
	...
	while((int)remaining > 0) {
		...
		if(lame->resample_in != NULL) {
			...
			for(ch = 0; ch < lame->channels_out; ch++) {
				...
				// resample filter virtually should have no
				// delay !
				ret = resample_buffer(lame->resample_in,
				                      mfbuf[ch] + lame->mf_size,
				                      &n_out, pdata[ch], &n_in,
				                      ch);
				if(ret < 0)
					return ret;
			...
			for(ch = 0; ch < lame->channels_out; ch++) {
				memcpy(mfbuf[ch] + lame->mf_size, pdata[ch],
				       n_out * sizeof(**mfbuf));
				pdata[ch] += n_in;
			}
		...
		fprintf(stderr,
		        "n_in=%d, n_out=%d, remaining=%d, remaining=%d\n", n_in,
		        n_out, remaining, remaining - n_in);
		...
		if(lame->mf_size >= mf_needed) {
			// Enlarge octetstream buffer if (possibly) needed by
			// (25% + 16K)
			if(os->size < 16384 + os->length)
				octetstream_resize(os, os->size + os->size / 4 +
				                           16384);
			// Encode one frame
			...
			for(ch = 0; ch < lame->channels_out; ch++)
				memmove(mfbuf[ch] + 0,
				        mfbuf[ch] + lame->frame_size,
				        lame->mf_size * sizeof(**mfbuf));
...
static inline void
average(sample_t* dst, const sample_t* src1, const sample_t* src2, size_t len)
{
	FN("average");
...
int
lame_encode_pcm(lame_t* const lame, octetstream_t* os, const void* pcm,
                size_t len, uint32_t flags)
{
	...
	ret = internal_lame_encoding_pcm(lame, os, (sample_t const* const*)data,
	                                 len);
...
int
lame_encode_pcm_flush(lame_t* const lame, octetstream_t* const os)
{
	int ret;
	int ret_cb;
	...
	flush_bitstream(lame->global_flags); // mp3 related stuff. bit buffer
	                                     // might still contain some mp3
	                                     // data
...
static lame_t*
pointer2lame(void* const handle)
{
...
int
lame_encode_buffer(void* const gfp, const int16_t buffer_l[],
                   const int16_t buffer_r[], const size_t nsamples,
                   void* const mp3buf, const size_t mp3buf_size)
{
	const int16_t* pcm[2];
	octetstream_t* os;
	int ret;
...
int
lame_encode_buffer_interleaved(void* const gfp, const int16_t buffer[],
                               size_t nsamples, void* const mp3buf,
                               const size_t mp3buf_size)
{
	...
	ret = lame_encode_pcm(lame, os, buffer, nsamples,
	                      LAME_INTERLEAVED | LAME_NATIVE_ENDIAN |
	                          LAME_INT16 | lame->channels_in);
...
int
lame_encode_flush(void* const gfp, void* const mp3buf, const size_t mp3buf_size)
{
	octetstream_t* os;
	int ret;
	lame_t* lame;
...
sinpi(long double x)
{
	...
	default:
		assert(0);
	case 0:
		return +SIN(M_PIl * (0.0 + x));
	case 1:
		return +COS(M_PIl * (0.5 - x));
	case 2:
		return +COS(M_PIl * (x - 0.5));
	case 3:
		return +SIN(M_PIl * (1.0 - x));
	case 4:
		return -SIN(M_PIl * (x - 1.0));
	case 5:
		return -COS(M_PIl * (1.5 - x));
	case 6:
		return -COS(M_PIl * (x - 1.5));
	case 7:
		return -SIN(M_PIl * (2.0 - x));
	}
}
...
cospi(long double x)
{
	...
	default:
		assert(0);
	case 0:
		return +COS(M_PIl * (x - 0.0));
	case 1:
		return +SIN(M_PIl * (0.5 - x));
	case 2:
		return -SIN(M_PIl * (x - 0.5));
	case 3:
		return -COS(M_PIl * (1.0 - x));
	case 4:
		return -COS(M_PIl * (x - 1.0));
	case 5:
		return -SIN(M_PIl * (1.5 - x));
	case 6:
		return +SIN(M_PIl * (x - 1.5));
	case 7:
		return +COS(M_PIl * (2.0 - x));
	}
}
...
blackman(double x)
{
	...
	x = cospi(x);
	return (0.16 * x + 0.50) * x +
	       0.34; // using addition theorem of arc functions
}
...
static inline double
blackmanharris_nuttall(double x)
{
	if(fabs(x) >= 1)
		return 0.;

	x += 1.;
	return (10 - 15 * cospi(x) + 6 * cospi(2 * x) - cospi(3 * x)) *
	       (1. / 32);
}

static inline double
blackmanharris_min4(double x)
{
	if(fabs(x) >= 1)
		return 0.;

	x += 1.;
	return 0.355768 - 0.487396 * cospi(x) + 0.144232 * cospi(2 * x) -
	       0.012604 * cospi(3 * x);
}
 ...
 *
 * When Nuttall set the ak coefficients to the "minimum 4-term" values of a0 =
 *0.355768, a1 =
 * 0.487396, a2 = 0.144232, and a3 = 0.012604, he obtained the window and the
 *frequency
 * response shown in Figure 8. The minimum-4-term window loses some frequency
 * resolution because it has a wide main lobe, but the first side lobe drops to
 *-93 db.
 *
 * If you require fast side-lobe rolloff in an application, consider Nuttall's
 *Equation (30). It
 * applies the following coefficients to the general equation above: a0 = 10/32,
 *a1 = 15/32, a2 =
 * 6/32, and a3 = 1/32. Figure 8 also includes the window and frequency response
 *for Nuttall's
 * Equation (30). This window has a first side-lobe level of -61 dB and a
 *significant side-lobe
 * rolloff of -42 dB/octave.
 *
 * The first investigators of windowing functions determined that window
 *functions should
 * have zero values at their boundaries and so should their successive
 *derivatives. If a
 * window's kth derivative is zero at the boundaries, the peaks of the window's
 *side lobes will
...
/*
 * The scalarxx versions with xx=04,08,12,16,20,24 are fixed size for xx element
 *scalars
 * The scalar1n version is suitable for every non negative length
 * The scalar4n version is only suitable for positive lengths which are a
 *multiple of 4
 *
...
float_t
scalar04_float32(const sample_t* p, const sample_t* q)
{
	return p[0] * q[0] + p[1] * q[1] + p[2] * q[2] + p[3] * q[3];
}

float_t
scalar08_float32(const sample_t* p, const sample_t* q)
{
	return p[0] * q[0] + p[1] * q[1] + p[2] * q[2] + p[3] * q[3] +
...
float_t
scalar12_float32(const sample_t* p, const sample_t* q)
{
	return p[0] * q[0] + p[1] * q[1] + p[2] * q[2] + p[3] * q[3] +
	       p[4] * q[4] + p[5] * q[5] + p[6] * q[6] + p[7] * q[7] +
...
float_t
scalar16_float32(const sample_t* p, const sample_t* q)
{
	return p[0] * q[0] + p[1] * q[1] + p[2] * q[2] + p[3] * q[3] +
	       p[4] * q[4] + p[5] * q[5] + p[6] * q[6] + p[7] * q[7] +
	       p[8] * q[8] + p[9] * q[9] + p[10] * q[10] + p[11] * q[11] +
...
float_t
scalar20_float32(const sample_t* p, const sample_t* q)
{
	return p[0] * q[0] + p[1] * q[1] + p[2] * q[2] + p[3] * q[3] +
	       p[4] * q[4] + p[5] * q[5] + p[6] * q[6] + p[7] * q[7] +
	       p[8] * q[8] + p[9] * q[9] + p[10] * q[10] + p[11] * q[11] +
...
float_t
scalar24_float32(const sample_t* p, const sample_t* q)
{
	return p[0] * q[0] + p[1] * q[1] + p[2] * q[2] + p[3] * q[3] +
	       p[4] * q[4] + p[5] * q[5] + p[6] * q[6] + p[7] * q[7] +
	       p[8] * q[8] + p[9] * q[9] + p[10] * q[10] + p[11] * q[11] +
...
float_t
scalar32_float32(const sample_t* p, const sample_t* q)
{
	return p[0] * q[0] + p[1] * q[1] + p[2] * q[2] + p[3] * q[3] +
	       p[4] * q[4] + p[5] * q[5] + p[6] * q[6] + p[7] * q[7] +
	       p[8] * q[8] + p[9] * q[9] + p[10] * q[10] + p[11] * q[11] +
...
float_t
scalar4n_float32(const sample_t* p, const sample_t* q, size_t len)
{
...
float_t
scalar1n_float32(const sample_t* p, const sample_t* q, size_t len)
{
	float_t sum;
...
void
init_scalar_functions(OUT lame_t* const lame)
{
...
static double
factorize(OUT long double f1, OUT long double f2, IN int* const x1,
          IN int* const x2)
{
	unsigned i;
	long ltmp;
...
/*
 *  Some programmers and some file format only supporting integral sampling
 * frequencies
 *  This patch tries to find the right sampling frequency for some know rounding
 * victims.
 *  Better is to support directly 64 or 80 bit FPU precision.
 */

long double
unround_samplefrequency(OUT long double freq)
{
	...
	case 48663:
		return 1411200 /
		       29.L; // SB 128 "48 kHz" 48662.06896551724137931034
	case 44055:
	case 44056:
		return 2863636 /
		       65.L; // NTSC PCM/LD PCM 44055.93846153846153846153
	case 32072:
	case 32073:
		return 1411200 /
		       44.L; // SB 128 "32 kHz" 32072.72727272727272727272
	case 31468:
	case 31469:
		return 2863636 / 91.L; // NTSC Hi8 Digital Audio
		                       // 31468.52747252747252747252
	case 23918:
	case 23919:
		return 1411200 /
		       59.L; // SB 128 "24 kHz" 23918.64406779661016949152
	case 22254:
	case 22255:
		return 244800 / 11.L; // MAC HQ 22254.54545454545454545454
	case 16036:
	case 16037:
		return 1411200 /
		       88.L; // SB 128 "16 kHz" 16036.36363636363636363636
	case 11959:
	case 11960:
		return 1411200 /
		       118.L; // SB 128 "12 kHz" 11959.32203389830508474576
	case 11127:
	case 11128:
		return 122400 / 11.L; // MAC LQ 11127.27272727272727272727
	case 8018:
	case 8019:
		return 1411200 /
		       176.L; // SB 128  "8 kHz" 8018.18181818181818181818
	case 8012:
	case 8013:
		return 312500 / 39.L; // NeXT/Telco 8012.82051282051282051282
	case 5512:
	case 5513:
		return 44100 / 8.L; // 1/8 CD 5512.50000000000000000000
	}
...
static inline void Calc_Coeffs(
    IN sample_t* Coeff, OUT int iNew, OUT int iOld, OUT long i, OUT long k,
    OUT double bandwidth) // 0.0 ... 1.0: used bandwidth from 0...fs_out/2
{
	...
	for(j = 0; j <= 2 * WINDOW_SIZE; j++) {
		tmp1 =
		    ((k + j) * iNew - i * iOld) / (long double)iNew * bandwidth;
...
resample_t* resample_open(OUT long double sampfreq_in,  // [Hz]
                          OUT long double sampfreq_out, // [Hz]
                          OUT double lowpass_freq, // [Hz] or <0 for auto mode
                          OUT int quality) // Proposal: 0: default, 1: sample
                                           // select, 2: linear interpolation
//           4: 4-point interpolation, 32: 32-point interpolation
{
	...
	err = factorize(sampfreq_out, sampfreq_in, &(ret->scale_out),
	                &(ret->scale_in));
	fprintf(stderr, "(%.6g => %.6g Hz, Ratio %d:%d", (double)sampfreq_in,
	        (double)sampfreq_out, ret->scale_in, ret->scale_out);
	...
	ret->taps = TAPS;
	ret->lowpass_freq = lowpass_freq < 0.
	                        ? 0.50 * MIN(sampfreq_in, sampfreq_out)
	                        : lowpass_freq;
	ret->in_old[0] = calloc(sizeof(sample_t*), 2 * TAPS);
	ret->in_old[1] = calloc(sizeof(sample_t*), 2 * TAPS);
	ret->src_step =
	    calloc(sizeof(unsigned char*), (unsigned)ret->scale_out);
	ret->fir = calloc(sizeof(sample_t*), (unsigned)ret->scale_out);
	ret->firfree = calloc(sizeof(sample_t),
	                      TAPS * ret->scale_out + 64 / sizeof(sample_t));
	p = (sample_t*)(((ptrdiff_t)(ret->firfree) | 63) +
	                1); /* 64 byte alignment */

	for(i = 0; i < ret->scale_out; i++, p += TAPS) {
		ret->src_step[i] =
		    round_nearest((i + 1.L) * ret->scale_in / ret->scale_out) -
		    round_nearest((i + 0.L) * ret->scale_in / ret->scale_out);

		Calc_Coeffs(
		    ret->fir[i] = p, ret->scale_out, ret->scale_in, i,
		    round_nearest((double)i * ret->scale_in / ret->scale_out -
		                  WINDOW_SIZE),
...
int
resample_close(INOUT resample_t* const r)
{
	FN("resample_close");
...
int resample_buffer(           // return code, 0 for success
    ...
    INOUT size_t* const
        out_req_len, // requested output data len/really written output data len
    OUT sample_t* const in, // where are the input data?
    INOUT size_t* const
        in_avail_len,   // available input data len/consumed input data len
