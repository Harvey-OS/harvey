...
putbits2(lame_global_flags* gfp, int val, int j)
{
	...
	while(j > 0) {
		...
		if(bs->buf_bit_idx == 0) {
			...
			//		assert(gfc->header[gfc->w_ptr].write_timing
			//>= bs->totbit);
		...
		//	assert (j < MAX_LENGTH); /* 32 too large on 32 bit
		//machines */
...
drain_into_ancillary(lame_global_flags* gfp, int remainingBits)
{
	...
	if(remainingBits >= 32) {
		...
		if(remainingBits >= 32)
			for(i = 0;
			    i < (int)strlen(version) && remainingBits >= 8;
			    ++i) {
				remainingBits -= 8;
...
writeheader(lame_internal_flags* gfc, int val, int j)
{
	...
	while(j > 0) {
		...
		j -= k;
		assert(j <
		       MAX_LENGTH); /* >> 32  too large for 32 bit machines */
		gfc->header[gfc->h_ptr].buf[ptr >> 3] |= ((val >> j))
		                                         << (8 - (ptr & 7) - k);
		ptr += k;
	}
...
encodeSideInfo2(lame_global_flags* gfp, int bitsPerFrame)
{
	...
	if(gfp->version == 1) {
		...
		for(ch = 0; ch < gfc->channels_out; ch++) {
			...
			for(band = 0; band < 4; band++) {
				CRC_writeheader(gfc, l3_side->scfsi[ch][band],
				                1, &crc);
			}
		}
		...
		for(gr = 0; gr < 2; gr++) {
			for(ch = 0; ch < gfc->channels_out; ch++) {
				...
				CRC_writeheader(gfc, gi->part2_3_length, 12,
				                &crc);
				CRC_writeheader(gfc, gi->big_values / 2, 9,
				                &crc);
				CRC_writeheader(gfc, gi->global_gain, 8, &crc);
				CRC_writeheader(gfc, gi->scalefac_compress, 4,
				                &crc);
				CRC_writeheader(gfc, gi->window_switching_flag,
				                1, &crc);

				if(gi->window_switching_flag) {
					CRC_writeheader(gfc, gi->block_type, 2,
					                &crc);
					CRC_writeheader(
					    gfc, gi->mixed_block_flag, 1, &crc);

					if(gi->table_select[0] == 14)
						gi->table_select[0] = 16;
					CRC_writeheader(
					    gfc, gi->table_select[0], 5, &crc);
					if(gi->table_select[1] == 14)
						gi->table_select[1] = 16;
					CRC_writeheader(
					    gfc, gi->table_select[1], 5, &crc);

					CRC_writeheader(
					    gfc, gi->subblock_gain[0], 3, &crc);
					CRC_writeheader(
					    gfc, gi->subblock_gain[1], 3, &crc);
					CRC_writeheader(
					    gfc, gi->subblock_gain[2], 3, &crc);
				} else {
					...
					CRC_writeheader(
					    gfc, gi->table_select[2], 5, &crc);

					assert(gi->region0_count < 16U);
					assert(gi->region1_count < 8U);
					CRC_writeheader(gfc, gi->region0_count,
					                4, &crc);
					CRC_writeheader(gfc, gi->region1_count,
					                3, &crc);
				}
				CRC_writeheader(gfc, gi->preflag, 1, &crc);
				CRC_writeheader(gfc, gi->scalefac_scale, 1,
				                &crc);
				CRC_writeheader(gfc, gi->count1table_select, 1,
				                &crc);
			}
		}
	} else {
		...
		CRC_writeheader(gfc, l3_side->private_bits, gfc->channels_out,
		                &crc);
		...
		for(ch = 0; ch < gfc->channels_out; ch++) {
			...
			CRC_writeheader(gfc, gi->window_switching_flag, 1,
			                &crc);

			if(gi->window_switching_flag) {
				CRC_writeheader(gfc, gi->block_type, 2, &crc);
				CRC_writeheader(gfc, gi->mixed_block_flag, 1,
				                &crc);

				if(gi->table_select[0] == 14)
					gi->table_select[0] = 16;
				CRC_writeheader(gfc, gi->table_select[0], 5,
				                &crc);
				if(gi->table_select[1] == 14)
					gi->table_select[1] = 16;
				CRC_writeheader(gfc, gi->table_select[1], 5,
				                &crc);

				CRC_writeheader(gfc, gi->subblock_gain[0], 3,
				                &crc);
				CRC_writeheader(gfc, gi->subblock_gain[1], 3,
				                &crc);
				CRC_writeheader(gfc, gi->subblock_gain[2], 3,
				                &crc);
			} else {
				if(gi->table_select[0] == 14)
					gi->table_select[0] = 16;
				...
				if(gi->table_select[2] == 14)
					gi->table_select[2] = 16;
				CRC_writeheader(gfc, gi->table_select[2], 5,
				                &crc);

				assert(gi->region0_count < 16U);
				assert(gi->region1_count < 8U);
				CRC_writeheader(gfc, gi->region0_count, 4,
				                &crc);
				CRC_writeheader(gfc, gi->region1_count, 3,
				                &crc);
			}
...

	{
		...
		if(gfc->h_ptr == gfc->w_ptr) {
			...
			ERRORF(gfc, "Error: MAX_HEADER_BUF too small in "
			            "bitstream.c \n");
		}
	}
}
...
#ifdef DEBUG
	DEBUGF("%ld %d %d %d\n", gfc->bs.totbit - gegebo, gi->count1bits,
	       gi->big_values, gi->count1);
#endif
	return bits;
}
...
static int
Huffmancodebits(lame_global_flags* gfp, int tableindex, int start, int end,
                int* ix)
{
	int i, bits;
...
ShortHuffmancodebits(lame_global_flags* gfp, int* ix, gr_info* gi)
{
	...
	bits += Huffmancodebits(gfp, gi->table_select[1], region1Start,
	                        gi->big_values, ix);
	return bits;
}
...
LongHuffmancodebits(lame_global_flags* gfp, int* ix, gr_info* gi)
{
	...
	bits += Huffmancodebits(gfp, gi->table_select[1], region1Start,
	                        region2Start, ix);
	bits += Huffmancodebits(gfp, gi->table_select[2], region2Start,
	                        bigvalues, ix);
	return bits;
}

inline static int
writeMainData(lame_global_flags* const gfp, int l3_enc[2][2][576],
			...
			for(ch = 0; ch < gfc->channels_out; ch++) {
				...
				if(gi->block_type == SHORT_TYPE) {
					for(sfb = 0; sfb < SBPSY_s; sfb++) {
						...
						putbits2(
						    gfp,
						    scalefac[gr][ch].s[sfb][0],
						    slen);
						putbits2(
						    gfp,
						    scalefac[gr][ch].s[sfb][1],
						    slen);
						putbits2(
						    gfp,
						    scalefac[gr][ch].s[sfb][2],
						    slen);
						scale_bits += 3 * slen;
					}
					data_bits += ShortHuffmancodebits(
					    gfp, l3_enc[gr][ch], gi);
				} else {
					int i;
					for(i = 0; i < sizeof(scfsi_band) /
					                       sizeof(int) -
					                   1;
					    i++) {
						if(gr != 0 &&
					...
					data_bits += LongHuffmancodebits(
					    gfp, l3_enc[gr][ch], gi);
				}
				data_bits += huffman_coder_count1(
				    gfp, l3_enc[gr][ch], gi);
#ifdef DEBUG
				DEBUGF("<%ld> ", gfc->bs.totbit - hogege);
#endif
				/* does bitcount in quantize.c agree with actual
				 * bit count?*/
				assert(data_bits ==
		...
		for(ch = 0; ch < gfc->channels_out; ch++) {
			...
			if(gi->block_type == SHORT_TYPE) {
				for(; sfb_partition < 4; sfb_partition++) {
					int sfbs = gi->sfb_partition_table
					               [sfb_partition] /
					           3;
			...
			data_bits +=
			    huffman_coder_count1(gfp, l3_enc[gr][ch], gi);

			/* does bitcount in quantize.c agree with actual bit
			 * count?*/
			assert(data_bits ==
			       gi->part2_3_length - gi->part2_length);
			assert(scale_bits == gi->part2_length);
...
flush_bitstream(lame_global_flags* gfp)
{
	...
	if(flushbits >= 0) {
		...
		if(last_ptr < first_ptr)
			remaining_headers =
			    1 + last_ptr - first_ptr + MAX_HEADER_BUF;
	...
	assert(gfc->header[last_ptr].write_timing + bitsPerFrame ==
	       gfc->bs.totbit);
}

void
add_dummy_byte(lame_global_flags* const gfp, unsigned char val)
{
	lame_internal_flags* gfc = gfp->internal_flags;
	int i;
                 ...
                 int l3_enc[2][2][576], III_scalefac_t scalefac[2][2])
{
	...
	if(gfc->bs.totbit > 1000000000) {
		/* to avoid totbit overflow, (at 8h encoding at 128kbs) lets
		 * reset bit counter*/
		int i;
		for(i = 0; i < MAX_HEADER_BUF; ++i)
...
int
copy_buffer(unsigned char* buffer, int size, Bit_stream_struc* bs)
{
