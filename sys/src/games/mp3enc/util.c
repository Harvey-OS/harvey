...
ATHformula_old(FLOAT8 f)
{
	...
	ath = 3.640 * pow(f, -0.8) - 6.500 * exp(-0.6 * pow(f - 3.3, 2.0)) +
	      0.001 * pow(f, 4.0);
	return ath;
}

FLOAT8
ATHformula_GB(FLOAT8 f)
{
	FLOAT8 ath;
	f /= 1000; // convert to khz
	f = Max(0.01, f);
...
ATHformula_Frank(FLOAT8 freq)
{
	 ...
	 * third = 1/12
	 * octave = 1/40 decade
	 * rest is linear interpolated, values are currently in decibel rel. 20
	 * µPa
	 */
	static FLOAT tab[] = {
	...
	freq_log =
	    40. * log10(0.1 * freq); /* 4 steps per third, starting at 10 Hz */
	index = (unsigned)freq_log;
	assert(index < sizeof(tab) / sizeof(*tab));
	return tab[index] * (1 + index - freq_log) +
	       tab[index + 1] * (freq_log - index);
}

FLOAT8
ATHformula(FLOAT8 f, lame_global_flags* gfp)
{
	switch(gfp->ATHtype) {
	case 0:
...
getframebits(lame_global_flags* gfp, int* bitsPerFrame, int* mean_bits)
{
	...
	// bytes_per_frame = bitrate * 1000 / ( gfp->out_samplerate /
	// (gfp->version == 1  ?  1152  :  576 )) / 8;
	// bytes_per_frame = bitrate * 1000 / gfp->out_samplerate *
	// (gfp->version == 1  ?  1152  :  576 ) / 8;
	// bytes_per_frame = bitrate * ( gfp->version == 1  ?  1152/8*1000  :
	// 576/8*1000 ) / gfp->out_samplerate;
...
int FindNearestBitrate(int bRate,      /* legal rates from 32 to 448 */
                       ...
                       int samplerate) /* convert bitrate in kbps to index */
{
	...
	for(i = 1; i <= 14; i++)
		if(ABS(bitrate_table[version][i] - bRate) <
		   ABS(bitrate - bRate))
...
*/
void
freorder(int scalefac_band[], FLOAT8 ix_orig[576])
{
...
blackman(FLOAT8 x, FLOAT8 fcn, int l)
{
	/* This algorithm from:
...
void
fill_buffer(lame_global_flags* gfp, sample_t* mfbuf[2], sample_t* in_buffer[2],
            int nsamples, int* n_in, int* n_out)
{
	...
	if(gfc->resample_ratio != 1.0) {
		for(ch = 0; ch < gfc->channels_out; ch++) {
			*n_out = fill_buffer_resample(
			    gfp, &mfbuf[ch][gfc->mf_size], gfp->framesize,
...
int
fill_buffer_resample(lame_global_flags* gfp, sample_t* outbuf, int desired_len,
                     sample_t* inbuf, int len, int* num_used, int ch)
{
	...
	intratio = (fabs(gfc->resample_ratio -
	                 floor(.5 + gfc->resample_ratio)) < .0001);
	...
	if(gfc->fill_buffer_resample_init == 0) {
		gfc->inbuf_old[0] =
		    calloc(BLACKSIZE, sizeof(gfc->inbuf_old[0][0]));
		gfc->inbuf_old[1] =
		    calloc(BLACKSIZE, sizeof(gfc->inbuf_old[0][0]));
		for(i = 0; i <= 2 * bpc; ++i)
			gfc->blackfilt[i] =
			    calloc(BLACKSIZE, sizeof(gfc->blackfilt[0][0]));
	...
	for(k = 0; k < desired_len; k++) {
		...
		time0 =
		    k * gfc->resample_ratio; /* time of k'th output sample */
		...
		/* blackman filter.  by default, window centered at
		 * j+.5(filter_l%2) */
...
#define PRECOMPUTE
...
#else
			xvalue += y * blackman(i - offset, fcn,
			                       filter_l); /* very slow! */
#endif
		}
		outbuf[k] = xvalue;
	...
	*num_used = Min(len, filter_l + j - filter_l / 2);
	...
	if(*num_used >= BLACKSIZE) {
		...
		int n_shift =
		    BLACKSIZE - *num_used; /* number of samples to shift */
...
lame_msgf(const lame_internal_flags* gfc, const char* format, ...)
{
	...
	if(gfc->report.msgf != NULL) {
		...
		(void)vfprintf(stderr, format, args);
		fflush(
		    stderr); /* we print to stderr, so me may want to flush */
	}
...
updateStats(lame_internal_flags* const gfc)
{
	...
	if(gfc->channels_out == 2)
		gfc->bitrate_stereoMode_Hist[gfc->bitrate_index]
		                            [gfc->mode_ext]++;
}
...
#include <float.h>
		unsigned int mask;
		mask = _controlfp(0, 0);
		mask &= ~(_EM_OVERFLOW | _EM_UNDERFLOW | _EM_ZERODIVIDE |
		          _EM_INVALID);
