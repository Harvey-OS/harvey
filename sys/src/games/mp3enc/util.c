...
ATHformula_GB(FLOAT8 f)
{
	...
	ath = 3.640 * pow(f, -0.8) - 6.800 * exp(-0.6 * pow(f - 3.4, 2.0)) + 6.000 * exp(-0.15 * pow(f - 8.7, 2.0)) + 0.6 * 0.001 * pow(f, 4.0);
	return ath;
}
...
ATHformula_GBtweak(FLOAT8 f)
{
	...
	/* from Painter & Spanias, 1997 */
	/* modified by Gabriel Bouvigne to better fit to the reality */
	ath = 3.640 * pow(f, -0.8) - 6.800 * exp(-0.6 * pow(f - 3.4, 2.0)) + 6.000 * exp(-0.15 * pow(f - 8.7, 2.0)) + 0.57 * 0.001 * pow(f, 4.0) //0.57 to maximize HF importance
...
void
fill_buffer(lame_global_flags *gfp,
	    sample_t *mfbuf[2],
...
#define _FPU_GETCW(cw) __asm__("fnstcw %0" \
			       : "=m"(*&cw))
