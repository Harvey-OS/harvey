...
int
ResvFrameBegin(lame_global_flags* gfp, III_side_info_t* l3_side, int mean_bits,
               int frameLength)
{
	 ...
	 *          Number of bits allowed to encode one frame (you can take
	 *8*511 bit
	 *          from the bit reservoir and at most 8*1440 bit from the
	 *current
	 *          frame (320 kbps, 32 kHz), so 8*1951 bit is the largest
	 *possible
	...
	if(gfc->pinfo != NULL) {
		gfc->pinfo->mean_bits = mean_bits / 2; /* expected bits per
		                                          channel per granule
		                                          [is this also right
		                                          for mono/stereo,
		                                          MPEG-1/2 ?] */
...
*/
void
ResvMaxBits(lame_global_flags* gfp, int mean_bits, int* targ_bits,
            int* extra_bits)
{
	lame_internal_flags* gfc = gfp->internal_flags;
	int add_bits;
	...
	*targ_bits = mean_bits;
	...
	if(gfc->ResvSize > ((gfc->ResvMax * full_fac) / 10)) {
		...
		*targ_bits += add_bits;
		...
		add_bits = 0;
		/* build up reservoir.  this builds the reservoir a little
		 * slower
		 * than FhG.  It could simple be mean_bits/15, but this was
		 * rigged
	...
	*extra_bits =
	    (gfc->ResvSize < (gfc->ResvMax * 6) / 10 ? gfc->ResvSize
	                                             : (gfc->ResvMax * 6) / 10);
	*extra_bits -= add_bits;
...
*/
void
ResvAdjust(lame_internal_flags* gfc, gr_info* gi, III_side_info_t* l3_side,
           int mean_bits)
{
...
#define NEW_DRAINXX
	...
	{
		int mdb_bytes =
		    Min(l3_side->main_data_begin * 8, stuffingBits) / 8;
		l3_side->resvDrain_pre += 8 * mdb_bytes;
		stuffingBits -= 8 * mdb_bytes;
		gfc->ResvSize -= 8 * mdb_bytes;
		l3_side->main_data_begin -= mdb_bytes;

		/* drain just enough to be byte aligned.  The remaining bits
		 * will
		 * be added to the reservoir, and we will deal with them next
		 * frame.
		 * If the next frame is at a lower bitrate, it may have a larger
		 * ResvMax,
