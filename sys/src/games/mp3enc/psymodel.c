...
*/
int
L3psycho_anal(lame_global_flags *gfp,
 ...
 */
     ...
     **********************************************************************/
		for(j = 0; j < gfc->cw_lower_index; j++) { /* calculate unpredictability measure cw */
			FLOAT an, a1, a2;
			FLOAT bn, b1, b2;
			FLOAT rn, r1, r2;
     ...
     **********************************************************************/
		for(j = gfc->cw_lower_index; j < gfc->cw_upper_index; j += 4) { /* calculate unpredictability measure cw */
			FLOAT rn, r1, r2;
			FLOAT numre, numim, den;
...
#define CONV2 (-.43)
			...
			if(tbb != 0) {
				tbb = ctb / tbb;
				if(tbb <= exp((1 - CONV1) / CONV2)) { /* tonality near 1 */
					tbb = exp(-LN_TO_LOG10 * TMN);
				} else if(tbb >= exp((0 - CONV1) / CONV2)) { /* tonality near 0 */
		...
		{ /* compute PE for all 4 channels */
			...
			if(chn < 2) {
				...
				if(mx > 30 * mn) { /* big surge of energy - always use short blocks */
					uselongblock[chn] = 0;
				} else if((mx > 10 * mn) && (gfc->pe[chn] > 1000)) { /* medium surge, medium pe - use short blocks */
					uselongblock[chn] = 0;
				}
...
int
L3psycho_anal_ns(lame_global_flags *gfp,
		 const sample_t *buffer[2], int gr_out,
		 FLOAT8 *ms_ratio,
		 FLOAT8 *ms_ratio_next,
		 III_psy_ratio masking_ratio[2][2],
...
#define NSATTACKTHRE_S 300
			...
			for(i = 0; i < 12; i++) {
				if(!ns_attacks[i / 3] && attack_intensity[i] > (chn == 3 ? NSATTACKTHRE_S : NSATTACKTHRE))
   ...
   *********************************************************************/
	for(chn = 0; chn < numchn; chn++) {
		if(chn < 2) {
			...
			if(gfp->analysis)
				gfc->pinfo->pe[gr_out][chn] = percep_entropy[chn];
		} else {
			...
			if(gfp->analysis)
				gfc->pinfo->pe[gr_out][chn] = percep_MS_entropy[chn - 2];
		}
	}
...
int
L3para_read(lame_global_flags *gfp, FLOAT8 sfreq, int *numlines_l, int *numlines_s,
	    FLOAT8 *minval,
