...
percep_entropy_MS[2]  mid and side channel values (prev granule) of
percep_entropy
...
*/
int
L3psycho_anal(lame_global_flags* gfp, const sample_t* buffer[2], int gr_out,
              FLOAT8* ms_ratio, FLOAT8* ms_ratio_next,
              III_psy_ratio masking_ratio[2][2],
              III_psy_ratio masking_MS_ratio[2][2], FLOAT8 percep_entropy[2],
              FLOAT8 percep_MS_entropy[2], FLOAT8 energy[4], int blocktype_d[2])
{
	...
	for(chn = 0; chn < numchn; chn++) {
		...
		/* there is a one granule delay.  Copy maskings computed last
		 * call
		...
		gfc->tot_ener[chn] = gfc->energy[0]; /* sum total energy at
		                                        nearly no extra cost */

		for(j = BLKSIZE / 2 - 1; j >= 0; --j) {
			FLOAT re = (*wsamp_l)[BLKSIZE / 2 - j];
			FLOAT im = (*wsamp_l)[BLKSIZE / 2 + j];
			gfc->energy[BLKSIZE / 2 - j] =
			    (re * re + im * im) * (FLOAT)0.5;
		 ...
		 **********************************************************************/
		for(j = 0; j < gfc->cw_lower_index;
		    j++) { /* calculate unpredictability measure cw */
			FLOAT an, a1, a2;
			FLOAT bn, b1, b2;
			FLOAT rn, r1, r2;
			...
			bn = gfc->bx_sav[chn][0][j] =
			    j == 0 ? (*wsamp_l)[0] : (*wsamp_l)[BLKSIZE - j];
		 ...
		 **********************************************************************/
		for(j = gfc->cw_lower_index; j < gfc->cw_upper_index;
		    j += 4) { /* calculate unpredictability measure cw */
			FLOAT rn, r1, r2;
			FLOAT numre, numim, den;
			...
			gfc->cw[j + 1] = gfc->cw[j + 2] = gfc->cw[j + 3] =
			    gfc->cw[j] = den;
		}
	...
	gfc->energy[j+1] = gfc->energy[j+2] = gfc->energy[j+3] =  gfc->energy[j]=tot;
		...
		/**********************************************************************
		 *    Calculate the energy and the unpredictability in the
		 *threshold   *
...
#define CONV2 (-.43)
			...
			if(tbb != 0) {
				tbb = ctb / tbb;
				if(tbb <= exp((1 - CONV1) /
				              CONV2)) { /* tonality near 1 */
					tbb = exp(-LN_TO_LOG10 * TMN);
				} else if(tbb >=
				          exp((0 - CONV1) /
				              CONV2)) { /* tonality near 0 */
			...
			/* at this point, tbb represents the amount the
			 * spreading function
			 * will be reduced.  The smaller the value, the less
			 * masking.
			 * minval[] = 1 (0db)     says just use tbb.
			 * minval[]= .01 (-20db)  says reduce spreading function
			 * by
			 *                        at least 20db.
			 */
			...
			/* note: all surges in PE are because of this pre-echo
			 * formula
			 * for thr[b].  If it this is not used, PE is always
			 * around 600
			 */
			 ...
			 * frame2:  regular frame.  looks like pre-echo when
			 * compared to
			...
			*/
			...
			if(vbr_mtrh == gfp->VBR) {
				thr[b] = Min(rpelev * gfc->nb_1[chn][b],
				             rpelev2 * gfc->nb_2[chn][b]);
				thr[b] = Max(thr[b], gfc->ATH->adjust *
				                         gfc->ATH->cb[b]);
				thr[b] = Min(thr[b], ecb);
			} else if(gfc->blocktype_old[chn > 1 ? chn - 2 : chn] ==
			          SHORT_TYPE)
				thr[b] = ecb; /* Min(ecb,
				                 rpelev*gfc->nb_1[chn][b]); */
			else
				thr[b] =
				    Min(ecb, Min(rpelev * gfc->nb_1[chn][b],
				                 rpelev2 * gfc->nb_2[chn][b]));
...

			{
				...
				*/
				if(thrpe < eb[b])
					gfc->pe[chn] -= gfc->numlines_l[b] *
					                log(thrpe / eb[b]);
			}
		}

		/***************************************************************
		 * determine the block type (window type) based on L & R
		 *channels
		 *
		 ***************************************************************/
		{ /* compute PE for all 4 channels */
			...
			if(chn < 2) {
				...
				/* tuned for t1.wav.  doesnt effect most other
				 * samples */
				if(gfc->pe[chn] > 3000)
					uselongblock[chn] = 0;

				if(mx > 30 * mn) { /* big surge of energy -
				                      always use short blocks */
					uselongblock[chn] = 0;
				} else if((mx > 10 * mn) &&
				          (gfc->pe[chn] > 1000)) { /* medium
				                                      surge,
				                                      medium pe
				                                      - use
				                                      short
		...
		for(sb = 0; sb < NBPSY_l; sb++) {
			FLOAT8 enn = gfc->w1_l[sb] * eb[gfc->bu_l[sb]] +
			             gfc->w2_l[sb] * eb[gfc->bo_l[sb]];
			FLOAT8 thmm = gfc->w1_l[sb] * thr[gfc->bu_l[sb]] +
			              gfc->w2_l[sb] * thr[gfc->bo_l[sb]];
		...
		for(sblock = 0; sblock < 3; sblock++) {
			...
			for(sb = 0; sb < NBPSY_s; sb++) {
				FLOAT8 enn = gfc->w1_s[sb] * eb[gfc->bu_s[sb]] +
				             gfc->w2_s[sb] * eb[gfc->bo_s[sb]];
				FLOAT8 thmm =
				    gfc->w1_s[sb] * thr[gfc->bu_s[sb]] +
				    gfc->w2_s[sb] * thr[gfc->bo_s[sb]];

				for(b = gfc->bu_s[sb] + 1; b < gfc->bo_s[sb];
				    b++) {
					enn += eb[b];
					thmm += thr[b];
				}
	...
	if(gfp->mode == JOINT_STEREO) {
		...
		for(sb = 0; sb < NBPSY_l; sb++) {
			...
			if(gfc->thm[0].l[sb] <= 1.58 * gfc->thm[1].l[sb] &&
			   gfc->thm[1].l[sb] <= 1.58 * gfc->thm[0].l[sb]) {
				...
				rmid = Max(gfc->thm[chmid].l[sb],
				           Min(gfc->thm[chside].l[sb], mld));

				mld = gfc->mld_l[sb] * gfc->en[chmid].l[sb];
				rside = Max(gfc->thm[chside].l[sb],
				            Min(gfc->thm[chmid].l[sb], mld));
		...
		for(sb = 0; sb < NBPSY_s; sb++) {
			for(sblock = 0; sblock < 3; sblock++) {
				if(gfc->thm[0].s[sb][sblock] <=
				       1.58 * gfc->thm[1].s[sb][sblock] &&
				   gfc->thm[1].s[sb][sblock] <=
				       1.58 * gfc->thm[0].s[sb][sblock]) {

					mld = gfc->mld_s[sb] *
					      gfc->en[chside].s[sb][sblock];
					rmid = Max(
					    gfc->thm[chmid].s[sb][sblock],
					    Min(gfc->thm[chside].s[sb][sblock],
					        mld));

					mld = gfc->mld_s[sb] *
					      gfc->en[chmid].s[sb][sblock];
					rside = Max(
					    gfc->thm[chside].s[sb][sblock],
					    Min(gfc->thm[chmid].s[sb][sblock],
					        mld));
	...
	if(gfp->mode == JOINT_STEREO) {
		...
		ms_ratio_l =
		    (sidetot / tot) * 0.7; /* was .35*(sidetot/tot)/5.0*10 */
		...
		for(sblock = 0; sblock < 3; sblock++)
			for(sb = NBPSY_s / 4; sb < NBPSY_s; sb++) {
				x1 = Min(gfc->thm[0].s[sb][sblock],
				         gfc->thm[1].s[sb][sblock]);
				x2 = Max(gfc->thm[0].s[sb][sblock],
				         gfc->thm[1].s[sb][sblock]);
				/* thresholds difference in db */
				if(x2 >= 1000 * x1)
					db = 3;
	...
	if(gfc->channels_out == 2) {
		if(!gfp->allow_diff_short || gfp->mode == JOINT_STEREO) {
			...
			/* this is necessary if the frame is to be encoded in
			 * ms_stereo.  */
	...
	for(chn = 0; chn < gfc->channels_out; chn++) {
		if(uselongblock[chn]) { /* no attack : use long blocks */
		...
		blocktype_d[chn] =
		    gfc->blocktype_old[chn]; /* value returned to calling
		                                program */
		gfc->blocktype_old[chn] =
		    blocktype[chn]; /* save for next call to l3psy_anal */
	}
...
mask_add(FLOAT8 m1, FLOAT8 m2, int k, int b, lame_internal_flags* const gfc)
{
	static const FLOAT8 table1[] = {
	    3.3246 * 3.3246,   3.23837 * 3.23837, 3.15437 * 3.15437,
	    3.00412 * 3.00412, 2.86103 * 2.86103, 2.65407 * 2.65407,
	    2.46209 * 2.46209, 2.284 * 2.284,     2.11879 * 2.11879,
	    1.96552 * 1.96552, 1.82335 * 1.82335, 1.69146 * 1.69146,
	    1.56911 * 1.56911, 1.46658 * 1.46658, 1.37074 * 1.37074,
	    1.31036 * 1.31036, 1.25264 * 1.25264, 1.20648 * 1.20648,
	    1.16203 * 1.16203, 1.12765 * 1.12765, 1.09428 * 1.09428,
	    1.0659 * 1.0659,   1.03826 * 1.03826, 1.01895 * 1.01895,
	    1};
	...
	static const FLOAT8 table3[] = {
	    2.35364 * 2.35364, 2.29259 * 2.29259, 2.23313 * 2.23313,
	    2.12675 * 2.12675, 2.02545 * 2.02545, 1.87894 * 1.87894,
	    1.74303 * 1.74303, 1.61695 * 1.61695, 1.49999 * 1.49999,
	    1.39148 * 1.39148, 1.29083 * 1.29083, 1.19746 * 1.19746,
...
int
L3psycho_anal_ns(lame_global_flags* gfp, const sample_t* buffer[2], int gr_out,
                 FLOAT8* ms_ratio, FLOAT8* ms_ratio_next,
                 III_psy_ratio masking_ratio[2][2],
                 III_psy_ratio masking_MS_ratio[2][2], FLOAT8 percep_entropy[2],
                 FLOAT8 percep_MS_entropy[2], FLOAT8 energy[4],
                 int blocktype_d[2])
{
	/* to get a good cache performance, one has to think about
	 * the sequence, in which the variables are used.
	...
	if(gfp->VBR == vbr_off)
		pcfact = gfc->ResvMax == 0 ? 0 : ((FLOAT)gfc->ResvSize) /
		                                     gfc->ResvMax * 0.5;
	else if(gfp->VBR == vbr_rh || gfp->VBR == vbr_mtrh ||
	        gfp->VBR == vbr_mt) {
		static const FLOAT8 pcQns[10] = {1.0, 1.0, 1.0, 0.8, 0.6,
		                                 0.5, 0.4, 0.3, 0.2, 0.1};
	...
	for(chn = 0; chn < numchn; chn++) {
		...
		if(chn < 2) {
			/* LR maskings  */
			// percep_entropy            [chn]       = gfc -> pe
			// [chn];
			masking_ratio[gr_out][chn].en = gfc->en[chn];
			masking_ratio[gr_out][chn].thm = gfc->thm[chn];
		} else {
			/* MS maskings  */
			// percep_MS_entropy         [chn-2]     = gfc -> pe
			// [chn];
			masking_MS_ratio[gr_out][chn - 2].en = gfc->en[chn];
			masking_MS_ratio[gr_out][chn - 2].thm = gfc->thm[chn];
		}
	...
	if (init) {
	  ...
	  init = 0;
...
#else
			static FLOAT8 tab[20] = {
			    1,       0.79433, 0.63096, 0.63096, 0.63096,
			    0.63096, 0.63096, 0.25119, 0.11749, 0.11749,
			    0.11749, 0.11749, 0.11749, 0.11749, 0.11749,
			    0.11749, 0.11749, 0.11749, 0.11749, 0.11749};
#endif
		...
		for(b = 0; b < gfc->npart_l; b++) {
			...
			/****   convolve the partitioned energy with the
			 * spreading function   ****/
...
#if 1
			for(k = gfc->s3ind[b][0]; k <= gfc->s3ind[b][1]; k++) {
				ecb = mask_add(ecb, gfc->s3_l[b][k] * eb2[k], k,
				               k - b, gfc);
			}
...
#define NS_INTERP(x, y, r) (pow((x), (r)) * pow((y), 1 - (r)))
			...
			if(gfc->blocktype_old[chn > 1 ? chn - 2 : chn] ==
			   SHORT_TYPE)
				thr[b] = ecb; /* Min(ecb,
				                 rpelev*gfc->nb_1[chn][b]); */
			else
...

		{
			...
			for(i = 0; i < 12; i++) {
				en_subshort[i] = 0;
				for(j = 0; j < 576 / 9; j++) {
					en_subshort[i] += ns_hpfsmpl[chn][k] *
					                  ns_hpfsmpl[chn][k];
					k++;
				}

				if(en_subshort[i] < 100)
					en_subshort[i] = 100;
			}
...
#define NSATTACKTHRE_S 300
			...
			for(i = 0; i < 2; i++) {
				attack_intensity[i] =
				    en_subshort[i] /
				    gfc->nsPsy.last_en_subshort[chn][7 + i];
			}

			for(; i < 12; i++) {
				attack_intensity[i] =
				    en_subshort[i] / en_subshort[i - 2];
			}

			ns_attacks[0] = ns_attacks[1] = ns_attacks[2] =
			    ns_attacks[3] = 0;

			for(i = 0; i < 12; i++) {
				if(!ns_attacks[i / 3] &&
				   attack_intensity[i] > (chn == 3
				                              ? NSATTACKTHRE_S
				                              : NSATTACKTHRE))
			...
			for(i = 0; i < 9; i++) {
				gfc->nsPsy.last_en_subshort[chn][i] =
				    en_subshort[i];
				gfc->nsPsy.last_attack_intensity[chn][i] =
				    attack_intensity[i];
			}
...
#define NS_PREECHO_ATT2 0.3
				...
				if(ns_attacks[sblock] == 1) {
					...
					thmm = Min(thmm, p);
				           ...
				           ns_attacks[sblock - 1] == 3) ||
				          (sblock == 0 &&
				           gfc->nsPsy.last_attacks[chn][2] ==
				               3)) {
					double p =
					    sblock <= 1
					        ? gfc->nsPsy.last_thm
...
#define NS_MSFIX 3.5
	...
	if(numchn == 4) {
		...
		if(gfc->nsPsy.safejoint)
			msfix = 1;

		for(sb = 0; sb < NBPSY_l; sb++) {
			FLOAT8 thmL, thmR, thmM, thmS, ath;
			ath = (gfc->ATH
			           ->cb[(gfc->bu_l[sb] + gfc->bo_l[sb]) / 2]) *
			      pow(10, -gfp->ATHlower / 10.0);
		...
		for(sb = 0; sb < NBPSY_s; sb++) {
			for(sblock = 0; sblock < 3; sblock++) {
				...
				if(thmL * msfix < (thmM + thmS) / 2) {
					FLOAT8 f =
					    thmL * msfix / ((thmM + thmS) / 2);
					thmM *= f;
					thmS *= f;
				}
				if(thmR * msfix < (thmM + thmS) / 2) {
					FLOAT8 f =
					    thmR * msfix / ((thmM + thmS) / 2);
					thmM *= f;
					thmS *= f;
				}

				gfc->thm[2].s[sb][sblock] =
				    Min(gfc->thm[2].s[sb][sblock], thmM);
				gfc->thm[3].s[sb][sblock] =
				    Min(gfc->thm[3].s[sb][sblock], thmS);
			}
		}
	}
	...
	for(chn = 0; chn < numchn; chn++) {
		{
			static FLOAT8 regcoef[] = {
			    1124.23, 10.0583, 10.7484, 7.29006,  16.2714,
			    6.2345,  4.09743, 3.05468, 3.33196,  2.54688,
			    3.68168, 5.83109, 2.93817, -8.03277, -10.8458,
			    8.48777, 9.13182, 2.05212, 8.6674,   50.3937,
			    73.267,  97.5664, 0};
			...
			for(sb = 0; sb < NBPSY_l; sb++) {
				...
				if(gfc->thm[chn].l[sb] * gfc->masking_lower !=
				       0 &&
				   gfc->en[chn].l[sb] / (gfc->thm[chn].l[sb] *
				                         gfc->masking_lower) >
				       1)
					t = log(gfc->en[chn].l[sb] /
					        (gfc->thm[chn].l[sb] *
					         gfc->masking_lower));
				else
					t = 0;
		...
		{
			static FLOAT8 regcoef[] = {
			    1236.28, 0, 0,       0,       0.434542, 25.0738, 0,
			    0,       0, 19.5442, 19.7486, 60,       100,     0};
...
int
L3para_read(lame_global_flags* gfp, FLOAT8 sfreq, int* numlines_l,
            int* numlines_s, FLOAT8* minval, FLOAT8 s3_l[CBANDS][CBANDS],
            FLOAT8 s3_s[CBANDS][CBANDS], FLOAT8* SNR, int* bu_l, int* bo_l,
            FLOAT8* w1_l, FLOAT8* w2_l, int* bu_s, int* bo_s, FLOAT8* w1_s,
            FLOAT8* w2_s, int* npart_l_orig, int* npart_l, int* npart_s_orig,
            int* npart_s)
{
	lame_internal_flags* gfc = gfp->internal_flags;
	...
	for(i = 0; i < CBANDS; i++) {
		...
		do {
			/* find smallest j2 >= j so that  (bark - bark_l[i-1]) <
			 * DELBARK */
			ji = j;
	...
	*npart_l_orig = i + 1;
	...
	/* compute which partition bands are in which scalefactor bands */
	{
		...
		for(sfb = 0; sfb < SBMAX_l; sfb++) {
			...
			//      DEBUGF(gfc,"longblock:  old: (%i,%i)  new:
			//      (%i,%i) %i %i
	...
	for(i = 0; i < *npart_l_orig; i++) {
		...
		// Calculating the medium bark scaled frequency of the spectral
		// lines
		// from j ... j + numlines[i]-1  (=spectral lines in parition
		// band i)
		...
		for(k = 0; k < numlines_l[i]; k++, j++) {
			...
			//	freq = Min(.1,freq);       // ATH below 100 Hz
			//constant, not further climbing
			level =
			    ATHformula(freq * 1000, gfp) -
			    20; // scale to FFT units; returned value is in dB
			level =
			    pow(10., 0.1 * level); // convert from dB -> energy
			level *= numlines_l[i];
	...
	for(i = 0; i < CBANDS; i++) {
		...
		do {
			/* find smallest j2 >= j so that  (bark - bark_s[i-1]) <
			 * DELBARK */
			ji = j;
	...
	*npart_s_orig = i + 1;
	...
	/* compute which partition bands are in which scalefactor bands */
	{
		...
		for(sfb = 0; sfb < SBMAX_s; sfb++) {
			...
			// DEBUGF(gfc,"shortblock: old: (%i,%i)  new: (%i,%i) %i
			// %i \n",bu_s[sfb],bo_s[sfb],
	...
	/************************************************************************
	 * Now compute the spreading function, s[j][i], the value of the
	 *spread-*
	...
	for(i = 0; i < *npart_l_orig; i++) {
		for(j = 0; j < *npart_l_orig; j++) {
			s3_l[i][j] =
			    s3_func(bval_l[i] - bval_l[j]) * bval_l_width[j];
		}
	}
	for(i = 0; i < *npart_s_orig; i++) {
		for(j = 0; j < *npart_s_orig; j++) {
			s3_s[i][j] =
			    s3_func(bval_s[i] - bval_s[j]) * bval_s_width[j];
		}
	}
...
#define temporalmask_sustain_sec 0.01
	...
	gfc->decay =
	    exp(-1.0 * LOG10 / (temporalmask_sustain_sec * sfreq / 192.0));
...
psymodel_init(lame_global_flags* gfp)
{
	...
	gfc->cw_upper_index =
	    Min(HBLKSIZE - 4, gfc->cw_upper_index); /* j+3 < HBLKSIZE-1 */
	gfc->cw_upper_index = Max(6, gfc->cw_upper_index);

	for(j = 0; j < HBLKSIZE; j++)
		gfc->cw[j] = 0.4f;

	i = L3para_read(gfp, (FLOAT8)samplerate, gfc->numlines_l,
	                gfc->numlines_s, gfc->minval, gfc->s3_l, gfc->s3_s,
	                gfc->SNR_s, gfc->bu_l, gfc->bo_l, gfc->w1_l, gfc->w2_l,
	                gfc->bu_s, gfc->bo_s, gfc->w1_s, gfc->w2_s,
	                &gfc->npart_l_orig, &gfc->npart_l, &gfc->npart_s_orig,
	                &gfc->npart_s);
	if(i != 0)
		return -1;
	...
	*/
	...
	if(gfc->nsPsy.use) {
		...
		for(b = 0; b < gfc->npart_l; b++) {
			for(k = gfc->s3ind[b][0]; k <= gfc->s3ind[b][1]; k++) {
				// spreading function has been properly
				// normalized by
		...
		for(b = 0; b < gfc->npart_s; b++) {
			FLOAT8 norm = 0;
			for(k = gfc->s3ind_s[b][0]; k <= gfc->s3ind_s[b][1];
			    k++) {
				norm += gfc->s3_s[b][k];
			}
			for(k = gfc->s3ind_s[b][0]; k <= gfc->s3ind_s[b][1];
	...
	if(gfc->nsPsy.use) {
#if 1
		/* spread only from npart_l bands.  Normally, we use the
		 * spreading
