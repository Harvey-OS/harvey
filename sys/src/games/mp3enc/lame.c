 ...
 */
	...
	if(samplefreq >= 32000)
		br =
		    bitrate - (channels == 1 ? (17 + 4) * 8 : (32 + 4) * 8) * samplefreq / 1152;
	else
		br =
		    bitrate - (channels == 1 ? (9 + 4) * 8 : (17 + 4) * 8) * samplefreq / 576;
...
lame_init_params(lame_global_flags *const gfp)
{
	...
	if(gfp->lowpassfreq == 0) {
		...
		if(0 && gfp->out_samplerate != optimum_samplefreq(lowpass, gfp->in_samplerate)) {
			MSGF(gfc,
...
lame_print_config(const lame_global_flags *gfp)
{
	...
	if(gfc->CPU_features.MMX || gfc->CPU_features.AMD_3DNow || gfc->CPU_features.SIMD || gfc->CPU_features.SIMD2) {
...
lame_init_old(lame_global_flags *gfp)
{
	...
	if(NULL ==
	   (gfc = gfp->internal_flags =
		calloc(1, sizeof(lame_internal_flags))))
		return -1;
