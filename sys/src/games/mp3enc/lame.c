...
lame_init_params_ppflt_lowpass(FLOAT8 amp_lowpass[32], FLOAT lowpass1,
                               FLOAT lowpass2, int* lowpass_band, int* minband,
                               int* maxband)
{
	int band;
	FLOAT8 freq;
	...
	for(band = 0; band <= 31; band++) {
		...
		if(lowpass1 < freq && freq < lowpass2) {
			...
			*maxband = Max(*maxband, band);
			amp_lowpass[band] = cos((PI / 2) * (lowpass1 - freq) /
			                        (lowpass2 - lowpass1));
		}
		/*
...
lame_init_params_ppflt(lame_global_flags* gfp)
{
	...
	if(gfc->lowpass1 > 0) {
		...
		lame_init_params_ppflt_lowpass(
		    gfc->amp_lowpass, gfc->lowpass1, gfc->lowpass2,
...
optimum_bandwidth(double* const lowerlimit, double* const upperlimit,
                  const unsigned bitrate, const int samplefreq,
                  const double channels, lame_global_flags* gfp)
{
	 ...
	 *                  epsilon is the percentage of LR frames for typical
	 *audio
	...
	if(samplefreq >= 32000)
		br = bitrate -
		     (channels == 1 ? (17 + 4) * 8 : (32 + 4) * 8) *
		         samplefreq / 1152;
	else
		br = bitrate -
		     (channels == 1 ? (9 + 4) * 8 : (17 + 4) * 8) * samplefreq /
		         576;

	if(channels >= 2.)
		br /= 1.75 + 0.25 * (channels - 2.); // MS needs 1.75x mono, LR
		                                     // needs 2.00x mono
	 ...
	 *
	 *   What can we see?
	 *       Value for 8 kbps is nonsense (although 8 kbps and stereo is
	 *nonsense)
	 *       Values are between 2.05 and 2.24 for 16...192 kbps
	 *       Some bitrate lack the following bitrates have: 16, 40, 80, 160
	 *kbps
	 *       A lot of bits per spectral line have: 24, 48, 96 kbps
	 *
	 *   What I propose?
	 *       A slightly with the bitrate increasing bits/line function. It
	 *is
	 *       better to decrease NMR for low bitrates to get a little bit
	 *more
	...
	f_low =
	    br /
	    log10(br * 4.425e-3); // Tests with 8, 16, 32, 64, 112 and 160 kbps
 ...
 */
	 ...
	 *  18 kHz =>   0 Hz
	 *
	 *  These are ad hoc values and these can be optimized if a high pass is
	 *available.
	 */
	if(f_low <= 16000)
	...
	/*
	 *  When we sometimes have a good highpass filter, we can add the
	 * highpass
...
optimum_samplefreq(int lowpassfreq, int input_samplefreq)
{
	 ...
	 * Rules:
	 *
	 *  - output sample frequency should NOT be decreased by more than 3% if
	 *lowpass allows this
...
lame_init_qval(lame_global_flags* gfp)
{
	...
	case 8:
		gfp->quality = 7;
	case 7: /* use psymodel (for short block and m/s switching), but no
	           noise shapping */
		gfc->filter_type = 0;
		gfc->psymodel = 1;
		gfc->quantization = 0;
...
lame_init_params(lame_global_flags* const gfp)
{
	...
	if(gfp->mode == MONO)
		gfp->force_ms =
		    0; // don't allow forced mid/side stereo for mono output
	...
	if(gfp->VBR == vbr_off && gfp->brate == 0) {
		/* no bitrate or compression ratio specified, use a compression
		 * ratio of 11.025 */
	...
	if(gfp->VBR == vbr_off && gfp->compression_ratio > 0) {
		...
		if(gfp->out_samplerate == 0)
			gfp->out_samplerate =
			    map2MP3Frequency(0.97 * gfp->in_samplerate);
		/* round up with a margin of 3% */

		/* choose a bitrate for the output samplerate which achieves
		 * specified compression ratio
		 */
		gfp->brate = gfp->out_samplerate * 16 * gfc->channels_out /
		             (1.e3 * gfp->compression_ratio);

		/* we need the version for the bitrate table look up */
		gfc->samplerate_index =
		    SmpFrqIndex(gfp->out_samplerate, &gfp->version);

		if(!gfp->free_format) /* for non Free Format find the nearest
		                         allowed bitrate */
	...
	if(gfp->out_samplerate == 0) {
		...
		/* check if user specified bitrate requires downsampling, if
		 * compression    */
		/* ratio is > 13, choose a new samplerate to get the ratio down
		 * to about 10 */
		...
		if(gfp->VBR == vbr_abr) {
			...
			if(gfp->compression_ratio > 13.)
				gfp->out_samplerate = map2MP3Frequency(
				    (10. * 1.e3 * gfp->VBR_mean_bitrate_kbps) /
				    (16 * gfc->channels_out));
		}
	}
	...
	if(gfp->ogg) {
		...
		gfc->mode_gr = gfp->out_samplerate <= 24000
		                   ? 1
		                   : 2; // Number of granules per frame
	...
	if(gfp->lowpassfreq == 0) {
		...
		default:
			channels =
			    1.; // just to make data flow analysis happy :-)
			assert(0);
			break;
		}
		...
		optimum_bandwidth(&lowpass, &highpass,
		                  gfp->out_samplerate * 16 * gfc->channels_out /
		                      gfp->compression_ratio,
		                  gfp->out_samplerate, channels, gfp);
		...
		if(0 &&
		   gfp->out_samplerate !=
			...
			MSGF(gfc, "I would suggest to use %u Hz instead of %u "
			          "Hz sample frequency\n",
			     optimum_samplefreq(lowpass, gfp->in_samplerate),
			     gfp->out_samplerate);
		}
	...
	if(gfp->lowpassfreq > 0) {
		...
		if(gfp->lowpasswidth >= 0) {
			gfc->lowpass1 = 2. *
			                (gfp->lowpassfreq - gfp->lowpasswidth) /
			                gfp->out_samplerate;
	...
	if(gfp->VBR == vbr_off) {
		...
		gfc
		    ->VBR_min_bitrate = 1; /* default: allow   8 kbps (MPEG-2)
		                              or  32 kbps (MPEG-1) */
		gfc->VBR_max_bitrate = 14; /* default: allow 160 kbps (MPEG-2)
		                              or 320 kbps (MPEG-1) */

		if(gfp->VBR_min_bitrate_kbps)
			if((gfc->VBR_min_bitrate = BitrateIndex(
			        gfp->VBR_min_bitrate_kbps, gfp->version,
			        gfp->out_samplerate)) < 0)
				return -1;
		if(gfp->VBR_max_bitrate_kbps)
			if((gfc->VBR_max_bitrate = BitrateIndex(
	...
	j = gfc->samplerate_index + (3 * gfp->version) +
	    6 * (gfp->out_samplerate < 16000);
...
lame_print_config(const lame_global_flags* gfp)
{
	...
	if(gfp->num_channels == 2 && gfc->channels_out == 1 /* mono */) {
		MSGF(gfc, "Autoconverting from stereo to mono. Setting "
		          "encoding to mono mode.\n");
	}
	...
	if(gfc->filter_type == 0) {
		if(gfc->highpass2 > 0.)
			MSGF(gfc, "Using polyphase highpass filter, transition "
			          "band: %5.0f Hz - %5.0f Hz\n",
			     0.5 * gfc->highpass1 * out_samplerate,
			     0.5 * gfc->highpass2 * out_samplerate);
		if(gfc->lowpass1 > 0.) {
			MSGF(gfc, "Using polyphase lowpass  filter, transition "
			          "band: %5.0f Hz - %5.0f Hz\n",
	...
	if(gfp->free_format) {
		MSGF(gfc, "Warning: many decoders cannot handle free format "
		          "bitstreams\n");
		if(gfp->brate > 320) {
			MSGF(gfc, "Warning: many decoders cannot handle free "
			          "format bitrates >320 kbps (see "
			          "documentation)\n");
		}
	}
}
...
*/
int
lame_encode_frame(lame_global_flags* gfp, sample_t inbuf_l[],
                  sample_t inbuf_r[], unsigned char* mp3buf, int mp3buf_size)
{
	int ret;
	if(gfp->ogg) {
#ifdef HAVE_VORBIS
		ret = lame_encode_ogg_frame(gfp, inbuf_l, inbuf_r, mp3buf,
		                            mp3buf_size);
#else
		return -5; /* wanna encode ogg without vorbis */
#endif
	} else {
		ret = lame_encode_mp3_frame(gfp, inbuf_l, inbuf_r, mp3buf,
		                            mp3buf_size);
	}
...
*/
int
lame_encode_buffer_sample_t(lame_global_flags* gfp, sample_t buffer_l[],
                            sample_t buffer_r[], int nsamples,
...
#error FFTOFFSET is greater than BLKSIZE, see encoder.h
	...
	mf_needed =
	    BLKSIZE + gfp->framesize - FFTOFFSET; /* amount needed for FFT */
	mf_needed = Max(
	    mf_needed,
	    286 +
	        576 *
	            (1 + gfc->mode_gr)); /* amount needed for MDCT/filterbank */
	...
	while(nsamples > 0) {
		int n_in =
		    0; /* number of input samples processed with fill_buffer */
...
int
lame_encode_buffer(lame_global_flags* gfp, const short int buffer_l[],
                   const short int buffer_r[], int nsamples,
                   unsigned char* mp3buf, const int mp3buf_size)
{
	lame_internal_flags* gfc = gfp->internal_flags;
	int ret, i;
...
int
lame_encode_buffer_float(lame_global_flags* gfp, const float buffer_l[],
                         const float buffer_r[], int nsamples,
                         unsigned char* mp3buf, const int mp3buf_size)
{
	lame_internal_flags* gfc = gfp->internal_flags;
	int ret, i;
...
int
lame_encode_buffer_long(lame_global_flags* gfp, const int32_t buffer_l[],
                        const int32_t buffer_r[], int nsamples,
                        unsigned char* mp3buf, const int mp3buf_size)
{
	lame_internal_flags* gfc = gfp->internal_flags;
	int ret, i;
...
int
lame_encode_buffer_interleaved(lame_global_flags* gfp, short int buffer[],
                               ...
                               int mp3buf_size)
{
	...
	ret = lame_encode_buffer(gfp, buffer_l, buffer_r, nsamples, mp3buf,
	                         mp3buf_size);
	free(buffer_l);
	free(buffer_r);
	return ret;
...
int
lame_encode(lame_global_flags* const gfp, const short int in_buffer[2][1152],
            unsigned char* const mp3buf, const int size)
{
	...
	return lame_encode_buffer(gfp, in_buffer[0], in_buffer[1],
	                          gfp->framesize, mp3buf, size);
}
...
int
lame_encode_flush(lame_global_flags* gfp, unsigned char* mp3buffer,
                  int mp3buffer_size)
{
	...
	while(gfc->mf_samples_to_encode > 0) {
		...
		/* send in a frame of 0 padding until all internal sample
		 * buffers
		 * are flushed
		 */
		imp3 = lame_encode_buffer(gfp, buffer[0], buffer[1],
		                          gfp->framesize, mp3buffer,
...
#ifdef HAVE_VORBIS
		/* ogg related stuff */
		imp3 = lame_encode_ogg_finish(gfp, mp3buffer,
		                              mp3buffer_size_remaining);
#endif
	} else {
		/* mp3 related stuff.  bit buffer might still contain some mp3
		 * data */
		flush_bitstream(gfp);
		/* write a id3 tag to the bitstream */
		id3tag_write_v1(gfp);
		imp3 =
		    copy_buffer(mp3buffer, mp3buffer_size_remaining, &gfc->bs);
	}
...
int
lame_encode_finish(lame_global_flags* gfp, unsigned char* mp3buffer,
                   int mp3buffer_size)
{
...
lame_init_old(lame_global_flags* gfp)
{
	 ...
	 * 1152 samples.  To synthesize the 576 samples centered under this
	 * granule
	 * we need the previous granule for the first 288 samples (no problem),
	 * and
	 * the next granule for the next 288 samples (not possible if this is
	 * last
	...
	gfc->mf_size =
	    ENCDELAY - MDCTDELAY; /* we pad input with this many 0's */
	...
	for(j = 0; j < 14; j++)
		for(i = 0; i < 4; i++)
			bitrate_stmode_count[j][i] =
			    gfc->bitrate_stereoMode_Hist[j + 1][i];
}
