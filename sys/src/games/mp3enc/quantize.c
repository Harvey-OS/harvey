...
init_outer_loop(gr_info* const cod_info, III_scalefac_t* const scalefac,
                const int is_mpeg1, const FLOAT8 xr[576], FLOAT8 xrpow[576])
{
	FLOAT8 tmp, sum = 0;
	int i;
                    ...
                    const FLOAT8 xrpow[576], int l3enc[576])
{
	...
	do {
		...
		if(nBits > desired_rate) {
			...
			break; /* nBits == desired_rate;; most unlikely to
			          happen.*/
...
int
inner_loop(lame_internal_flags* const gfc, gr_info* const cod_info,
           const int max_bits, const FLOAT8 xrpow[576], int l3enc[576])
{
	int bits;
...
inline static int
quant_compare(const int experimentalX, const calc_noise_result* const best,
 ...
 *************************************************************************/
static void
amp_scalefac_bands(lame_global_flags* gfp, const gr_info* const cod_info,
                   III_scalefac_t* const scalefac, III_psy_xmin* distort,
                   FLOAT8 xrpow[576])
{
...
static void
inc_scalefac_scale(const lame_internal_flags* const gfc,
                   gr_info* const cod_info, III_scalefac_t* const scalefac,
                   FLOAT8 xrpow[576])
{
	int start, end, l, i, j;
	int sfb;
...
static int
inc_subblock_gain(const lame_internal_flags* const gfc, gr_info* const cod_info,
                  III_scalefac_t* const scalefac, FLOAT8 xrpow[576])
{
	int window;
	...
	for(window = 0; window < 3; window++) {
		...
		for(sfb = cod_info->sfb_smin; sfb < SBMAX_s; sfb++) {
			...
			width = gfc->scalefac_band.s[sfb] -
			        gfc->scalefac_band.s[sfb + 1];
			i = gfc->scalefac_band.s[sfb] * 3 + width * window;
			amp =
			    IPOW20(210 + (s << (cod_info->scalefac_scale + 1)));
			for(l = 0; l < width; l++) {
				xrpow[l] *= amp;
			}
		}
 ...
 ********************************************************************/
inline static int
balance_noise(lame_global_flags* const gfp, gr_info* const cod_info,
              III_scalefac_t* const scalefac, III_psy_xmin* distort,
              FLOAT8 xrpow[576])
{
	lame_internal_flags* const gfc =
	    (lame_internal_flags*)gfp->internal_flags;
	int status;
	...
	if(gfc->noise_shaping > 1) {
		if(!cod_info->scalefac_scale) {
			...
			if(cod_info->block_type == SHORT_TYPE) {
				status = inc_subblock_gain(gfc, cod_info,
				                           scalefac, xrpow) ||
...
static int
outer_loop(lame_global_flags* gfp, gr_info* const cod_info,
           const FLOAT8 xr[576], /* magnitudes of spectral values */
           const III_psy_xmin* const
	...
	do {
		...
		if(iteration == 1) {
			if(bits_found > huff_bits) {
				cod_info->global_gain++;
				real_bits = inner_loop(gfc, cod_info, huff_bits,
				                       xrpow, l3_enc_w);
			} else {
		...
		if(gfc->noise_shaping)
			/* coefficients and thresholds both l/r (or both
			 * mid/side) */
		...
		if(0 == gfc->noise_shaping_stop && 0 == gfp->experimentalX &&
		   (over == 0 || best_noise_info.over_count == 0))
			break;
		/* Otherwise, allow up to 3 unsuccesful tries in serial, then
		 * stop
		 * if our best quantization so far had no distorted bands. This
		 * gives us more possibilities for different quant_compare
		 * modes.
		 * Much more than 3 makes not a big difference, it is only
		 * slower.
		 */
		...
		if(gfc->sfb21_extra) {
			if(cod_info->block_type == SHORT_TYPE) {
				if(distort.s[SBMAX_s - 1][0] > 1 ||
				   ...
				   distort.s[SBMAX_s - 1][2] > 1)
					break;
			} else {
				if(distort.l[SBMAX_l - 1] > 1)
					break;
			}
		}
		...
		notdone =
		    balance_noise(gfp, cod_info, scalefac, &distort, xrpow);
...
static void
iteration_finish(lame_internal_flags* gfc, FLOAT8 xr[2][2][576],
                 int l3_enc[2][2][576], III_psy_ratio ratio[2][2],
	...
	for(gr = 0; gr < gfc->mode_gr; gr++) {
		for(ch = 0; ch < gfc->channels_out; ch++) {
			...
			if(gfc->use_best_huffman == 1)
				best_huffman_divide(gfc, gr, ch, cod_info,
				                    l3_enc[gr][ch]);

			/*  update reservoir status after FINAL
...
static void
VBR_encode_granule(
    lame_global_flags* gfp, gr_info* const cod_info,
    ...
    int min_bits, int max_bits)
{
	...
	do {
		...
		over = outer_loop(gfp, cod_info, xr, l3_xmin, scalefac, xrpow,
		                  l3_enc, ch, this_bits);
...
static void
get_framebits(lame_global_flags* gfp, int* const analog_mean_bits,
              int* const min_mean_bits, int frameBits[15])
{
	lame_internal_flags* gfc = gfp->internal_flags;
 ...
 ************************************************************************/
inline static int
calc_min_bits(lame_global_flags* gfp, const gr_info* const cod_info,
              const int pe, const FLOAT8 ms_ener_ratio, const int bands,
              const int mch_bits, const int analog_mean_bits,
 ...
 ************************************************************************/
inline static int
calc_max_bits(const lame_internal_flags* const gfc, const int frameBits[15],
              const int min_bits)
{
	int max_bits;
	...
	max_bits =
	    Min(1200 + max_bits, MAX_BITS - 195 * (gfc->channels_out - 1));
...
static int
VBR_prepare(lame_global_flags* gfp, FLOAT8 pe[2][2], FLOAT8 ms_ener_ratio[2],
            FLOAT8 xr[2][2][576], III_psy_ratio ratio[2][2],
            III_psy_xmin l3_xmin[2][2], int frameBits[16],
            int* analog_mean_bits, int* min_mean_bits, int min_bits[2][2],
            int max_bits[2][2], int bands[2][2])
{
	lame_internal_flags* gfc = gfp->internal_flags;
	static const FLOAT8 dbQ[10] = {-2.,  -1.0, -.66, -.33, 0.,
	                               0.33, .66,  1.0,  1.33, 1.66};
	static const FLOAT8 dbQns[10] = {-4,  -3,  -2,  -1,  0,
	                                 0.7, 1.4, 2.1, 2.8, 3.5};
	/*static const FLOAT8 atQns[10]={-16,-12,-8,-4,0,  1,  2,  3,  4,  5};*/

	static const FLOAT8 dbQmtrh[10] = {-4., -3., -2., -1., 0.,
	                                   0.5, 1.,  1.5, 2.,  2.5};
	...
	for(gr = 0; gr < gfc->mode_gr; gr++) {
		...
		for(ch = 0; ch < gfc->channels_out; ch++) {
			...
			if(cod_info->block_type == SHORT_TYPE)
				adjust =
				    5 / (1 + exp(3.5 - pe[gr][ch] / 300.)) -
				    0.14;
			else
				adjust =
				    2 / (1 + exp(3.5 - pe[gr][ch] / 300.)) -
				    0.05;
			...
			min_bits[gr][ch] = calc_min_bits(
			    gfp, cod_info, (int)pe[gr][ch], ms_ener_ratio[gr],
			    bands[gr][ch], 0, *analog_mean_bits, *min_mean_bits,
			    analog_silence, ch);

			max_bits[gr][ch] =
			    calc_max_bits(gfc, frameBits, min_bits[gr][ch]);
                   ...
                   III_scalefac_t scalefac[2][2])
{
	...
	analog_silence = VBR_prepare(gfp, pe, ms_ener_ratio, xr, ratio, l3_xmin,
	...
	do {
		...
		for(gr = 0; gr < gfc->mode_gr; gr++) {
			for(ch = 0; ch < num_chan; ch++) {
				...
				/*  init_outer_loop sets up cod_info, scalefac
				 * and xrpow
				 */
				ret = init_outer_loop(
				    cod_info, &scalefac[gr][ch], gfc->is_mpeg1,
				    xr[gr][ch], xrpow);
				if(ret == 0) {
					/*  xr contains no energy
					 *  l3_enc, our encoding data, will be
					 * quantized to zero
					 */
					memset(l3_enc[gr][ch], 0,
					       sizeof(int) * 576);
					save_bits[gr][ch] = 0;
            ...
            if (gfc->mode_ext == MPG_MD_MS_LR && ch == 1)  
				...
				if(gfp->VBR == vbr_mtrh) {
					...
					VBR_encode_granule(
					    gfp, cod_info, xr[gr][ch],
					    &l3_xmin[gr][ch], &scalefac[gr][ch],
					    xrpow, l3_enc[gr][ch], ch,
					    min_bits[gr][ch], max_bits[gr][ch]);

				used_bits += cod_info->part2_3_length;
				save_bits[gr][ch] =
				    Min(MAX_BITS, cod_info->part2_3_length);
				used_bits2 +=
				    Min(MAX_BITS, cod_info->part2_3_length);
			} /* for ch */
		}         /* for gr */
		 ...
		 */
		if(reduce_s_ch) {
			/*  number of bits needed was found for MID channel
			 * above.  Use formula
			 *  (fixed bitrate code) to set the side channel bits */
			for(gr = 0; gr < gfc->mode_gr; gr++) {
				FLOAT8 fac =
				    .33 * (.5 - ms_ener_ratio[gr]) / .5;
				save_bits[gr][1] =
				    (int)(((1 - fac) / (1 + fac)) *
				          save_bits[gr][0]);
				save_bits[gr][1] =
				    Max(analog_mean_bits, save_bits[gr][1]);
		 ...
		 */
		if(analog_silence && !gfp->VBR_hard_min)
			/*  we detected analog silence and the user did not
			 * specify
			 *  any hard framesize limit, so start with smallest
			 * possible frame
			 */
			gfc->bitrate_index = 1;
		else
		...
		for(; gfc->bitrate_index < gfc->VBR_max_bitrate;
		    gfc->bitrate_index++) {
		...
		if(used_bits > bits) {
			// printf("# %d used %d have
			// %d\n",gfp->frameNum,used_bits,bits);
			if(gfp->VBR == vbr_mtrh) {
				for(gr = 0; gr < gfc->mode_gr; gr++) {
					for(ch = 0; ch < gfc->channels_out;
					    ch++) {
						max_bits[gr][ch] =
						    save_bits[gr][ch];
						max_bits[gr][ch] *= frameBits
						    [gfc->bitrate_index];
	...
	for(gr = 0; gr < gfc->mode_gr; gr++) {
		for(ch = 0; ch < gfc->channels_out; ch++) {
			 ...
			 */
			ret = init_outer_loop(cod_info, &scalefac[gr][ch],
			                      gfc->is_mpeg1, xr[gr][ch], xrpow);
			if(ret == 0) {
				/*  xr contains no energy
				 *  l3_enc, our encoding data, will be quantized
				 * to zero
				 */
				 ...
				 */
				outer_loop(gfp, cod_info, xr[gr][ch],
				           &l3_xmin[gr][ch], &scalefac[gr][ch],
				           xrpow, l3_enc[gr][ch], ch,
				           save_bits[gr][ch]);
			}
...
static void
calc_target_bits(lame_global_flags* gfp, FLOAT8 pe[2][2],
                 FLOAT8 ms_ener_ratio[2], int targ_bits[2][2],
                 int* analog_silence_bits, int* max_frame_bits)
{
	lame_internal_flags* gfc = gfp->internal_flags;
	III_side_info_t* l3_side = &gfc->l3_side;
	FLOAT8 res_factor;
	...
	*analog_silence_bits = mean_bits / gfc->channels_out;
	...
	for(gr = 0; gr < gfc->mode_gr; gr++) {
		for(ch = 0; ch < gfc->channels_out; ch++) {
			targ_bits[gr][ch] =
			    res_factor * (mean_bits / gfc->channels_out);
			...
			if(pe[gr][ch] > 700) {
				...
				/* short blocks use a little extra, no matter
				 * what the pe */
				if(cod_info->block_type == SHORT_TYPE) {
...
void
iteration_loop(lame_global_flags* gfp, FLOAT8 pe[2][2], FLOAT8 ms_ener_ratio[2],
               FLOAT8 xr[2][2][576], III_psy_ratio ratio[2][2],
               int l3_enc[2][2][576], III_scalefac_t scalefac[2][2])
{
	...
	for(gr = 0; gr < gfc->mode_gr; gr++) {
		...
		for(ch = 0; ch < gfc->channels_out; ch++) {
			...
			/*  init_outer_loop sets up cod_info, scalefac and xrpow
			 */
			i = init_outer_loop(cod_info, &scalefac[gr][ch],
			                    gfc->is_mpeg1, xr[gr][ch], xrpow);
			if(i == 0) {
				/*  xr contains no energy, l3_enc will be
				 * quantized to zero
				 */
				memset(l3_enc[gr][ch], 0, sizeof(int) * 576);
			} else {
