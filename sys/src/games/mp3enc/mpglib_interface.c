...
int
lame_decode1_headers(unsigned char* buffer, int len, int16_t pcm_l[],
                     int16_t pcm_r[], mp3data_struct* mp3data)
{
	...
	if(mp.header_parsed) {
		...
		if(mp.fsizeold > 0) /* works for free format and fixed, no
		                       overrun, temporal results are < 400.e6 */
			mp3data->bitrate = 8 * (4 + mp.fsizeold) *
			                       mp3data->samplerate /
			                       (1.e3 * mp3data->framesize) +
			                   0.5;
		else
			mp3data->bitrate =
			    tabsel_123[mp.fr.lsf][mp.fr.lay -
			                          1][mp.fr.bitrate_index];
...
int
lame_decode1(unsigned char* buffer, int len, int16_t pcm_l[], int16_t pcm_r[])
{
...
int
lame_decode_headers(unsigned char* buffer, int len, int16_t pcm_l[],
                    int16_t pcm_r[], mp3data_struct* mp3data)
{
	int ret;
	...
	while(1) {
		switch(ret = lame_decode1_headers(buffer, len, pcm_l + totsize,
		...
		default:
			totsize += ret;
			len =
			    0; /* future calls to decodeMP3 are just to flush
			          buffers */
			break;
		}
	}
...
int
lame_decode(unsigned char* buffer, int len, int16_t pcm_l[], int16_t pcm_r[])
{
	mp3data_struct mp3data;
