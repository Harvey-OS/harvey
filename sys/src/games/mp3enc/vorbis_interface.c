...
short int
    convbuffer[4096]; /* take 8 KByte out of the data segment, not the stack */
int convsize;
...
int
lame_decode_ogg_initfile(lame_global_flags* gfp, FILE* fd,
	...
	while(i < 2) {
		while(i < 2) {
			...
			if(result == 1) {
				ogg_stream_pagein(
				    &os,
				    &og); /* we can ignore any errors here
				...
				while(i < 2) {
					...
					if(result == 0)
						break;
					if(result == -1) {
						/* Uh oh; data at some point was
						   corrupted or missing!
						   We can't tolerate that in a
						   header.  Die. */
						ERRORF(gfc, "Corrupt secondary "
						            "header.  "
						            "Exiting.\n");
						return -1;
					}
...
*/
int
lame_decode_ogg_fromfile(lame_global_flags* gfp, FILE* fd, short int pcm_l[],
	...
	while(1) {
		...
		**pcm is a multichannel double vector.  In stereo, for
		...
		if(samples >= convsize || eos || eof) {
			/* read 1024 samples, or if eos, read what ever is in
			 * buffer */
			int clipflag = 0;
		...
		if(result == 0) {
			/* need more data */
		} else if(result == -1) { /* missing or corrupt data at this
		                             page position */
			ERRORF(gfc, "Corrupt or missing data in bitstream; "
			            "continuing...\n");
		} else {
			/* decode this page */
			ogg_stream_pagein(&os,
			                  &og); /* can safely ignore errors at
			                             this point */
			do {
				result = ogg_stream_packetout(&os, &op);
				if(result == 0) {
...
ogg_stream_state
    os2;        // take physical pages, weld into a logical stream of packets
...
int
lame_encode_ogg_init(lame_global_flags* gfp)
{
	lame_internal_flags* gfc = gfp->internal_flags;
	...
	vorbis_comment_add(&vc2,
	                   "Track encoded using L.A.M.E. libvorbis interface.");
...
/* Add ID3-style comments to the output using (for the time being) the
   ...
   from a patch by Ralph Giles <giles@a3a32260.sympatico.bconnected.net> */
	...
	if(gfp->tag_spec.title) {
		...
		strncat(comment, gfp->tag_spec.title,
		        MAX_COMMENT_LENGTH - strlen(comment));
		vorbis_comment_add(&vc2, comment);
	}
	if(gfp->tag_spec.artist) {
		strcpy(comment, "ARTIST=");
		strncat(comment, gfp->tag_spec.artist,
		        MAX_COMMENT_LENGTH - strlen(comment));
		vorbis_comment_add(&vc2, comment);
	}
	if(gfp->tag_spec.album) {
		strcpy(comment, "ALBUM=");
		strncat(comment, gfp->tag_spec.album,
		        MAX_COMMENT_LENGTH - strlen(comment));
		vorbis_comment_add(&vc2, comment);
	}
	...
	if(gfp->tag_spec.comment) {
		...
		strncat(comment, gfp->tag_spec.comment,
		        MAX_COMMENT_LENGTH - strlen(comment));
		vorbis_comment_add(&vc2, comment);
	}
/* TODO -- support for track and genre */
...

	{
		...
		vorbis_analysis_headerout(&vd2, &vc2, &header, &header_comm,
		                          &header_code);
		ogg_stream_packetin(&os2,
		                    &header); /* automatically placed in its own
		                                page */
		ogg_stream_packetin(&os2, &header_comm);
		ogg_stream_packetin(&os2, &header_code);

		/* no need to write out here.  We'll get to that in the main
		 * loop */
	}
...
int
lame_encode_ogg_finish(lame_global_flags* gfp, char* mp3buf, int mp3buf_size)
{
	int eos = 0, bytes = 0;
...
int
lame_encode_ogg_frame(lame_global_flags* gfp, const sample_t* inbuf_l,
                      const sample_t* inbuf_r, unsigned char* mp3buf,
                      size_t mp3buf_size)
{
	lame_internal_flags* gfc = gfp->internal_flags;
	int i;
