...
typedef long double
    longdouble_t; // temporarly results of long double operations
...
/*
 * These are data stream direction attributes like used in Ada83/Ada95 and in
 *RPC
 * The data direction is seen from the caller to the calling function.
 * Examples:
 *
 *    size_t  fread  ( void INOUT* buffer, size_t items, size_t itemsize, FILE
 *INOUT* fp );
 *    size_t  fwrite ( void OUT  * buffer, size_t items, size_t itemsize, FILE
 *INOUT* fp );
...
#define INOUT /* [inout] */
#define IN    /* [in]    */
#define OUTTR /* [out]: data is modified like [inout], but you don't get any   \
                 useful back */
...
/* The current code doesn't work on machines with non 8 bit char's in any way,
 * so abort */
 ...
 *    - You know the absolute byte order of the data (LAME_LITTLE_ENDIAN,
 * LAME_BIG_ENDIAN)
...
#define LAME_STEREO_65535_CHANNELS 0x0000FFFF
...
float_t scalar4n_float32_i387(const float32_t* p, const float32_t* q,
                              const size_t len);
float_t scalar1n_float32_i387(const float32_t* p, const float32_t* q,
                              const size_t len);
...
float_t scalar4n_float32_3DNow(const float32_t* p, const float32_t* q,
                               const size_t len);
float_t scalar1n_float32_3DNow(const float32_t* p, const float32_t* q,
                               const size_t len);
...
float_t scalar4n_float32_SIMD(const float32_t* p, const float32_t* q,
                              const size_t len);
float_t scalar1n_float32_SIMD(const float32_t* p, const float32_t* q,
                              const size_t len);
...
float_t scalar4n_float32(const float32_t* p, const float32_t* q,
                         const size_t len);
float_t scalar1n_float32(const float32_t* p, const float32_t* q,
                         const size_t len);
...
resample_t* resample_open(OUT long double sampfreq_in,  // [Hz]
                          ...
                          OUT int quality); // Proposal: 0 default, 1 sample
                                            // select, 2 linear interpol, 4
                                            // 4-point interpolation, 32
                                            // 32-point interpolation
