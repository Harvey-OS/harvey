...
static const int genre_alpha_map[] = {
    123, 34,  74,  73,  99,  20,  40,  26,  145, 90,  116, 41,  135, 85,  96,
    138, 89,  0,   107, 132, 65,  88,  104, 102, 97,  136, 61,  141, 32,  1,
    112, 128, 57,  140, 2,   139, 58,  3,   125, 50,  22,  4,   55,  127, 122,
    120, 98,  52,  48,  54,  124, 25,  84,  80,  115, 81,  119, 5,   30,  36,
    59,  126, 38,  49,  91,  6,   129, 79,  137, 7,   35,  100, 131, 19,  33,
    46,  47,  8,   29,  146, 63,  86,  71,  45,  142, 9,   77,  82,  64,  133,
    10,  66,  39,  11,  103, 12,  75,  134, 13,  53,  62,  109, 117, 23,  108,
    92,  67,  93,  43,  121, 15,  68,  14,  16,  76,  87,  118, 17,  78,  143,
    114, 110, 69,  21,  111, 95,  105, 42,  37,  24,  56,  44,  101, 83,  94,
    106, 147, 113, 18,  51,  130, 144, 60,  70,  31,  72,  27,  28};
...
id3tag_set_track(lame_global_flags* gfp, const char* track)
{
	...
	if(track && *track) {
		...
		/* limit a track to 255 so it fits in a version 1 tag even
		 * though CD
          ...
          size_t length)
{
	if(length) {
		...
		/* Set frame size = total size - header size.  Frame header and
		 * field
		 * bytes include 2-byte header flags, 1 encoding descriptor
		 * byte, and
		 * for comment frames: 3-byte language descriptor and 1 content
		 * descriptor byte */
		frame = set_4_byte_value(
		    frame, ((id == COMMENT_FRAME_ID) ? 5 : 1) + length);
		/* clear 2-byte header flags */
		*frame++ = 0;
		*frame++ = 0;
		/* clear 1 encoding descriptor byte to indicate ISO-8859-1
		 * format */
		*frame++ = 0;
...
id3tag_write_v2(lame_global_flags* gfp)
{
	...
	if((gfc->tag_spec.flags & CHANGED_FLAG) &&
	   !(gfc->tag_spec.flags & V1_ONLY_FLAG)) {
		/* calculate length of four fields which may not fit in verion 1
		 * tag */
		size_t title_length =
		    gfc->tag_spec.title ? strlen(gfc->tag_spec.title) : 0;
		size_t artist_length =
		    gfc->tag_spec.artist ? strlen(gfc->tag_spec.artist) : 0;
		size_t album_length =
		    gfc->tag_spec.album ? strlen(gfc->tag_spec.album) : 0;
		size_t comment_length =
		    gfc->tag_spec.comment ? strlen(gfc->tag_spec.comment) : 0;
		...
		if((gfc->tag_spec.flags & (ADD_V2_FLAG | V2_ONLY_FLAG)) ||
		   ...
		   (gfc->tag_spec.track && (comment_length > 28))) {
			...
			tag_size = 10;
			if(title_length) {
				/* add 10-byte frame header, 1 encoding
				 * descriptor byte ... */
			...
			if(comment_length) {
				/* add 10-byte frame header, 1 encoding
				 * descriptor byte,
				 * 3-byte language descriptor, 1 content
				 * descriptor byte ... */
				tag_size += 15 + comment_length;
			}
			if(gfc->tag_spec.track) {
				track_length =
				    sprintf(track, "%d", gfc->tag_spec.track);
			...
			if(gfc->tag_spec.genre != GENRE_NUM_UNKNOWN) {
				genre_length =
				    sprintf(genre, "(%d)", gfc->tag_spec.genre);
			...
			*p++ = 0;
			...
			/* encode adjusted size into four bytes where most
			 * significant
			...
			*p++ = adjusted_tag_size & 0x7fu;
			...
			/*
			 * NOTE: The remainder of the tag (frames and padding,
			 * if any)
			 * are not "unsynchronized" to prevent false MPEG audio
			 * headers
			 * from appearing in the bitstream.  Why?  Well, most
			 * players
			 * and utilities know how to skip the ID3 version 2 tag
			 * by now
			 * even if they don't read its contents, and it's
			 * actually
			 * very unlikely that such a false "sync" pattern would
			 * occur
			 * in just the simple text frames added here.
			 */

			/* set each frame in tag */
			p = set_frame(p, TITLE_FRAME_ID, gfc->tag_spec.title,
			              title_length);
			p = set_frame(p, ARTIST_FRAME_ID, gfc->tag_spec.artist,
			              artist_length);
			p = set_frame(p, ALBUM_FRAME_ID, gfc->tag_spec.album,
			              album_length);
			p = set_frame(p, YEAR_FRAME_ID, year, year_length);
			p = set_frame(p, COMMENT_FRAME_ID,
			              gfc->tag_spec.comment, comment_length);
...
id3tag_write_v1(lame_global_flags* gfp)
{
	...
	if((gfc->tag_spec.flags & CHANGED_FLAG) &&
		...
		*p++ = 'G';
		...
		p = set_text_field(p, gfc->tag_spec.comment,
		                   gfc->tag_spec.track ? 28 : 30, pad);
		if(gfc->tag_spec.track) {
