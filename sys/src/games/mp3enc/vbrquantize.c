...
static FLOAT8
calc_sfb_noise_ave(const FLOAT8* xr, const FLOAT8* xr34, const int bw,
                   const int sf)
{
	double xp;
	double xe;
...
static const int max_range_short[SBPSY_s] = {15, 15, 15, 15, 15, 15,
                                             7,  7,  7,  7,  7,  7};

static const int max_range_long[SBPSY_l] = {
    15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 7, 7, 7, 7, 7, 7, 7, 7, 7, 7};

static const int max_range_short_lsf[SBPSY_s] = {15, 15, 15, 15, 15, 15,
                                                 7,  7,  7,  7,  7,  7};
...
static int
compute_scalefacs_short_lsf(int sf[SBPSY_s][3], gr_info* cod_info,
                            int scalefac[SBPSY_s][3], int sbg[3])
{
	...
	for(i = 0; i < 3; ++i) {
		 ...
		 * 8*sbg >= maxsf1
		 */
		maxsf1 = Max(maxsf1 - maxrange1 * ifqstep,
		             maxsf2 - maxrange2 * ifqstep);
		sbg[i] = 0;
		if(minsf > 0)
		...
		for(sfb = 0; sfb < SBPSY_s; sfb++) {
			...
			if(sf[sfb][i] < 0) {
				...
				scalefac[sfb][i] = -sf[sfb][i] / ifqstep +
				                   (-sf[sfb][i] % ifqstep != 0);
				...
				if(maxover <
				   -(sf[sfb][i] + scalefac[sfb][i] * ifqstep))
					maxover = -(sf[sfb][i] +
					            scalefac[sfb][i] * ifqstep);
			}
		}
	}
...
static int
compute_scalefacs_long_lsf(int sf[SBPSY_l], const gr_info* const cod_info,
                           int scalefac[SBPSY_l])
{
	...
	for(sfb = 0; sfb < SBPSY_l; sfb++) {
		...
		if(sf[sfb] < 0) {
			...
			scalefac[sfb] =
			    -sf[sfb] / ifqstep + (-sf[sfb] % ifqstep != 0);
	...
	for(i = 0; i < 3; ++i) {
		...
		for(sfb = 0; sfb < SBPSY_s; sfb++) {
			...
			if(sf[sfb][i] < 0) {
				...
				if(-(sf[sfb][i] + scalefac[sfb][i] * ifqstep) >
				   maxover) {
					maxover = -(sf[sfb][i] +
					            scalefac[sfb][i] * ifqstep);
				}
...
*/
static int
compute_scalefacs_long(int sf[SBPSY_l], gr_info* cod_info,
                       int scalefac[SBPSY_l])
{
	int sfb;
	int maxover;
...
static int
VBR_quantize_granule(lame_global_flags* gfp, FLOAT8 xr34[576], int l3_enc[576],
                     const III_psy_ratio* const ratio,
                     III_scalefac_t* const scalefac, const int gr, const int ch)
{
...
static int
short_block_vbr_sf(const lame_internal_flags* const gfc,
                   const III_psy_xmin* const l3_xmin,
                   const FLOAT8 xr34_orig[576], const FLOAT8 xr34[576],
                   III_scalefac_t* const vbrsf)
{
	...
	for(j = 0, sfb = 0; sfb < SBMAX_s; sfb++) {
		for(b = 0; b < 3; b++) {
			...
			vbrsf->s[sfb][b] =
			    find_scalefac_ave(&xr34[j], &xr34_orig[j], sfb,
...
static int
long_block_vbr_sf(const lame_internal_flags* const gfc,
                  const III_psy_xmin* const l3_xmin,
                  const FLOAT8 xr34_orig[576], const FLOAT8 xr34[576],
                  III_scalefac_t* const vbrsf)
{
	...
	for(sfb = 0; sfb < SBMAX_l; sfb++) {
		...
		vbrsf->l[sfb] =
		    find_scalefac_ave(&xr34[start], &xr34_orig[start], sfb,
		                      l3_xmin->l[sfb], width);
	}
...
static int
short_block_sf(const lame_internal_flags* const gfc,
               const III_psy_xmin* const l3_xmin, const FLOAT8 xr34_orig[576],
               const FLOAT8 xr34[576], III_scalefac_t* const vbrsf)
{
	int j, sfb, b;
	...
	for(j = 0, sfb = 0; sfb < SBMAX_s; sfb++) {
		for(b = 0; b < 3; b++) {
			...
			if(0 == gfc->noise_shaping_amp) {
				/*  the faster and sloppier mode to use at lower
				 * quality
				 */
				vbrsf->s[sfb][b] =
				    find_scalefac(&xr34[j], &xr34_orig[j], sfb,
				                  l3_xmin->s[sfb][b], width);
			} else {
				/*  the slower and better mode to use at higher
				 * quality
				 */
				vbrsf->s[sfb][b] = find_scalefac_ave(
				    &xr34[j], &xr34_orig[j], sfb,
				    l3_xmin->s[sfb][b], width);
			}
			j += width;
		}
	...
	for(b = 0; b < 3; b++) {
		...
		vbrsf->s[SBPSY_s][b] =
		    Max(vbrsf->s[SBPSY_s][b], vbrmin - (vbrmean - vbrmin));
...
/* a variation for vbr-mtrh */
static int
long_block_sf(const lame_internal_flags* const gfc,
              const III_psy_xmin* const l3_xmin, const FLOAT8 xr34_orig[576],
              const FLOAT8 xr34[576], III_scalefac_t* const vbrsf)
{
	int sfb;
	...
	for(sfb = 0; sfb < SBMAX_l; sfb++) {
		...
		if(0 == gfc->noise_shaping_amp) {
			 ...
			 */
			vbrsf->l[sfb] =
			    find_scalefac(&xr34[start], &xr34_orig[start], sfb,
			                  l3_xmin->l[sfb], width);
		} else {
			/*  the slower and better mode to use at higher quality
			 */
			vbrsf->l[sfb] =
			    find_scalefac_ave(&xr34[start], &xr34_orig[start],
...
static void
short_block_scalefacs(lame_global_flags* gfp, gr_info* const cod_info,
                      III_scalefac_t* const scalefac,
                      III_scalefac_t* const vbrsf, int* const VBRmax)
{
	lame_internal_flags* gfc = gfp->internal_flags;
	const int* max_range;
	int sfb, b;
	...
	for(sfb = 0; sfb < SBPSY_s; sfb++) {
		for(b = 0; b < 3; b++) {
			v0 = (vbrmax - vbrsf->s[sfb][b]) -
			     (4 * 14 + 2 * max_range[sfb]);
			v1 = (vbrmax - vbrsf->s[sfb][b]) -
			     (4 * 14 + 4 * max_range[sfb]);
			if(maxover0 < v0)
				maxover0 = v0;
			if(maxover1 < v1)
				maxover1 = v1;
	...
	if(gfc->is_mpeg1)
		maxover = compute_scalefacs_short(
		    vbrsf->s, cod_info, scalefac->s, cod_info->subblock_gain);
	else
		maxover = compute_scalefacs_short_lsf(
		    vbrsf->s, cod_info, scalefac->s, cod_info->subblock_gain);
...
static void
long_block_scalefacs(lame_global_flags* gfp, gr_info* const cod_info,
                     III_scalefac_t* const scalefac,
                     III_scalefac_t* const vbrsf, int* const VBRmax)
{
	lame_internal_flags* gfc = gfp->internal_flags;
	...
	for(sfb = 0; sfb < SBPSY_l; sfb++) {
		...
		v0p = (vbrmax - vbrsf->l[sfb]) -
		      2 * (max_rangep[sfb] + pretab[sfb]);
		v1p = (vbrmax - vbrsf->l[sfb]) -
		      4 * (max_rangep[sfb] + pretab[sfb]);
		if(maxover0 < v0)
			maxover0 = v0;
		if(maxover1 < v1)
			maxover1 = v1;
	...
	if(gfc->is_mpeg1 == 1)
		maxover =
		    compute_scalefacs_long(vbrsf->l, cod_info, scalefac->l);
	else
		maxover =
		    compute_scalefacs_long_lsf(vbrsf->l, cod_info, scalefac->l);
...
static void
short_block_xr34(const lame_internal_flags* const gfc,
                 ...
                 const FLOAT8 xr34_orig[576], FLOAT8 xr34[576])
{
	...
	for(j = 0, sfb = 0; sfb < SBMAX_s; sfb++) {
		...
		for(b = 0; b < 3; b++) {
			ifac = 8 * cod_info->subblock_gain[b] +
			       ifqstep * scalefac->s[sfb][b];
			fac = calc_fac(ifac);
			/*
			 *  loop unrolled into "Duff's Device".  Robert Hegemann
			 */
...
static void
long_block_xr34(const lame_internal_flags* const gfc,
                const gr_info* const cod_info,
                const III_scalefac_t* const scalefac,
                const FLOAT8 xr34_orig[576], FLOAT8 xr34[576])
{
 ...
 ************************************************************************/
static int
VBR_noise_shaping(lame_global_flags* gfp, FLOAT8 xr[576], FLOAT8 xr34orig[576],
                  III_psy_ratio* ratio, int l3_enc[576], int digital_silence,
                  int minbits, int maxbits, III_scalefac_t* scalefac,
                  III_psy_xmin* l3_xmin, int gr, int ch)
{
	...
	do {
		...
		if(shortblock) {
			short_block_scalefacs(gfp, cod_info, scalefac, &vbrsf,
			                      &vbrmax);
			short_block_xr34(gfc, cod_info, scalefac, xr34orig,
			                 xr34);
		} else {
			long_block_scalefacs(gfp, cod_info, scalefac, &vbrsf,
			                     &vbrmax);
			long_block_xr34(gfc, cod_info, scalefac, xr34orig,
			                xr34);
		}
		VBR_quantize_granule(gfp, xr34, l3_enc, ratio, scalefac, gr,
		                     ch);
		...
		if(cod_info->part2_3_length < minbits) {
			if(digital_silence)
				break;
			// if (cod_info->part2_3_length ==
			// cod_info->part2_length) break;
			if(vbrmax + 210 == 0)
				break;
	...
	while(cod_info->part2_3_length > Min(maxbits, MAX_BITS)) {
		...
		if(cod_info->global_gain > 255)
			ERRORF(gfc,
			       "%ld impossible to encode ??? frame! bits=%d\n",
...
int
VBR_noise_shaping2(lame_global_flags* gfp, FLOAT8 xr[576], FLOAT8 xr34orig[576],
                   III_psy_ratio* const ratio, int l3_enc[576],
                   int digital_silence, int minbits, int maxbits,
                   III_scalefac_t* const scalefac, III_psy_xmin* const l3_xmin,
                   int gr, int ch)
{
	lame_internal_flags* gfc = gfp->internal_flags;
	III_scalefac_t vbrsf;
	...
	if(cod_info->part2_3_length > maxbits) {
		...
		if(bits > huffbits) {
			bits =
			    inner_loop(gfc, cod_info, huffbits, xr34, l3_enc);
			cod_info->part2_3_length = bits;
		}
	...
	if(cod_info->part2_length >=
	   LARGE_BITS) /* Houston, we have a problem */
		return -2;
...
void
VBR_quantize(lame_global_flags* gfp, FLOAT8 pe[2][2], FLOAT8 ms_ener_ratio[2],
             FLOAT8 xr[2][2][576], III_psy_ratio ratio[2][2],
             int l3_enc[2][2][576], III_scalefac_t scalefac[2][2])
{
	lame_internal_flags* gfc = gfp->internal_flags;
	...
	FLOAT8 xr34[2][2][576];
	// static const FLOAT8 dbQ[10]={-6.0,-5.0,-4.0,-3.0, -2.0, -1.0, -.25,
	// .5, 1.25, 2.0};
	/* from quantize.c VBR algorithm */
	/*static const FLOAT8 dbQ[10]=
	 {-5.5,-4.25,-3.0,-2.50, -1.75, -.75, -.5, -.25, .25, .75};*/
	/* a third dbQ table ?!? */
	static const FLOAT8 dbQ[10] = {-6.06, -4.4, -2.9, -1.57, -0.4,
	                               0.61,  1.45, 2.13, 2.65,  3.0};

	qadjust =
	    0; /* start with -1 db quality improvement over quantize.c VBR */
	...
	for(gr = 0; gr < gfc->mode_gr; gr++) {
		...
		for(ch = 0; ch < gfc->channels_out; ch++) {
			/* if in the following sections the quality would not be
			 * adjusted
			 * then we would only have to call calc_xmin once here
			 * and
			...
			if(pe[gr][ch] > 750) {
				masking_lower_db -=
				    Min(10, 4 * (pe[gr][ch] - 750.) / 750.);
			}
			gfc->masking_lower = pow(10.0, masking_lower_db / 10);

			/* masking thresholds */
			over_ath = calc_xmin(gfp, xr[gr][ch], &ratio[gr][ch],
...

	{
		 ...
		 */
		int bit_rate = gfp->out_samplerate * 16 * gfc->channels_out /
		               (1000.0 * gfp->compression_ratio);
		bitsPerFrame =
		    (bit_rate * gfp->framesize * 1000) / gfp->out_samplerate;
		mean_bits =
		    (bitsPerFrame - 8 * gfc->sideinfo_len) / gfc->mode_gr;
	}
	...
	/*
	 * loop over all ch,gr, encoding anything with bits >
	 *.5*(max_frame_bits/4)
	 *
      ...
      if (gfc->mode_ext==MPG_MD_MS_LR) {
        ...
        minbits_lr[1] = Max(125,(1-fac)*minbits);
			...
			for(ch = 0; ch < gfc->channels_out; ch++) {
				...
				/* ENCODE this data first pass, and on future
				 * passes unless it uses
				 * a very small percentage of the max_frame_bits
				 */
				if(cod_info->part2_3_length >
				   (max_frame_bits /
				    (2 * gfc->channels_out * gfc->mode_gr))) {

					shortblock = (cod_info->block_type ==
					              SHORT_TYPE);

					/* Adjust allowed masking based on
					 * quality setting */
					if(qadjust != 0 /*|| shortblock*/) {
						masking_lower_db =
						    dbQ[gfp->VBR_q] + qadjust;

						/*
						if (shortblock) masking_lower_db
						-= 4;
						*/

						if(pe[gr][ch] > 750)
							masking_lower_db -= Min(
							    10,
							    4 * (pe[gr][ch] -
							         750.) /
							        750.);
						gfc->masking_lower =
						    pow(10.0,
						        masking_lower_db / 10);
						calc_xmin(gfp, xr[gr][ch],
						          ratio[gr] + ch,
						          cod_info,
						          l3_xmin[gr] + ch);
					}

					/* digital silent granules do not need
					 * the full round trip,
					 * but this can be optimized later on
					 */
					adjusted = VBR_noise_shaping(
					    gfp, xr[gr][ch], xr34[gr][ch],
					    ratio[gr] + ch, l3_enc[gr][ch],
					    digital_silence[gr][ch],
					    minbits_lr[ch], maxbits,
					    scalefac[gr] + ch, l3_xmin[gr] + ch,
					    gr, ch);
					if(adjusted > 10) {
						/* global_gain was changed by a
						 * large amount to get bits <
						 * maxbits */
						/* quality is set to high.  we
						 * could set bits = LARGE_BITS
						 * to force re-encoding.  But
						 * most likely the other
						 * channels/granules
						 * will also use too many bits,
						 * and the entire frame will
						 * be > max_frame_bits, forcing
						 * re-encoding below.
						 */
						// cod_info->part2_3_bits =
						// LARGE_BITS;
					}
				}
	...
	for(gr = 0; gr < gfc->mode_gr; gr++) {
		for(ch = 0; ch < gfc->channels_out; ch++) {
			best_scalefac_store(gfc, gr, ch, l3_enc, l3_side,
			                    scalefac);
