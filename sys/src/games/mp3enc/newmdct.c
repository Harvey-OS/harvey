...
static const FLOAT8 enwindow[] = {
    ...
    -1.4782e-05 * 0.740951125354959 / 2.384e-06, 9.063471690191471e-01,
    ...
    -1.6689e-05 * 0.773010453362737 / 2.384e-06, 8.206787908286602e-01,
    ...
    -1.8120e-05 * 0.803207531480645 / 2.384e-06, 7.416505462720353e-01,
    ...
    -1.9550e-05 * 0.831469612302545 / 2.384e-06, 6.681786379192989e-01,
    ...
    -2.1458e-05 * 0.857728610000272 / 2.384e-06, 5.993769336819237e-01,
    ...
    -2.3365e-05 * 0.881921264348355 / 2.384e-06, 5.345111359507916e-01,
    ...
    -2.5272e-05 * 0.903989293123443 / 2.384e-06, 4.729647758913199e-01,
    ...
    -3.0041e-05 * 0.941544065183021 / 2.384e-06, 3.578057213145241e-01,
    ...
    -3.2425e-05 * 0.956940335732209 / 2.384e-06, 3.033466836073424e-01,
    ...
    -3.4809e-05 * 0.970031253194544 / 2.384e-06, 2.504869601913055e-01,
    ...
    -3.7670e-05 * 0.98078528040323 / 2.384e-06, 1.989123673796580e-01,
    ...
    -4.0531e-05 * 0.989176509964781 / 2.384e-06, 1.483359875383474e-01,
    ...
    -4.3392e-05 * 0.995184726672197 / 2.384e-06, 9.849140335716425e-02,
    ...
    -4.6253e-05 * 0.998795456205172 / 2.384e-06, 4.912684976946725e-02,
    ...
    7.47204e-04 / 2.384e-06, 4.9591e-05 / 2.384e-06, 4.756451e-03 / 2.384e-06,
    2.1458e-05 / 2.384e-06, -6.9618e-05 / 2.384e-06, /*    2.384e-06/2.384e-06*/
};
#endif
 ...
 */
static const int order[] = {0,  1,  16, 17, 8,  9,  24, 25, 4,  5,  20,
                            21, 12, 13, 28, 29, 2,  3,  18, 19, 10, 11,
...
window_subband(const sample_t* x1, FLOAT8 a[SBLIMIT])
{
	...
	for(i = -15; i < 0; i++) {
		 ...
		 * data.
		 * thinking about the data cache performance, I think we should
		 * not
...
mdct_short(FLOAT8* inout)
{
	...
	for(l = 0; l < 3; l++) {
		...
		inout[3 * 0] =
		    tc1 * 1.907525191737280e-11 /* tritab_s[2] */ + tc0;
		inout[3 * 5] =
		    -ts1 * 1.907525191737280e-11 /* tritab_s[0] */ + ts0;

		tc2 = tc2 * 0.86602540378443870761 *
		      1.907525191737281e-11 /* tritab_s[2] */;
		...
		ts2 = ts2 * 0.86602540378443870761 *
		      1.907525191737281e-11 /* tritab_s[0] */;
		inout[3 * 3] = tc1 + ts2;
		inout[3 * 4] = tc1 - ts2;
...
inline static void
mdct_long(FLOAT8* out, FLOAT8* in)
{
	FLOAT8 ct, st;
	{
	...
	for(ch = 0; ch < gfc->channels_out; ch++) {
		for(gr = 0; gr < gfc->mode_gr; gr++) {
			...
			for(k = 0; k < 18 / 2; k++) {
				...
				wk += 64;
				/*
				 * Compensate for inversion in the analysis
				 * filter
				 */
			...
			if(gfc->filter_type == 0) {
				for(band = gfc->highpass_start_band;
				    band <= gfc->highpass_end_band; band++) {
					for(k = 0; k < 18; k++)
						gfc->sb_sample[ch][1 - gr][k]
						              [order[band]] *=
						    gfc->amp_highpass[band];
				}
				for(band = gfc->lowpass_start_band;
				    band <= gfc->lowpass_end_band; band++) {
					for(k = 0; k < 18; k++)
						gfc->sb_sample[ch][1 - gr][k]
						              [order[band]] *=
			...
			for(band = 0; band < 32; band++, mdct_enc += 18) {
				...
				band1 =
				    gfc->sb_sample[ch][1 - gr][0] + order[band];
				if(gi->mixed_block_flag && band < 2)
					type = 0;
				if(band >= gfc->lowpass_band ||
				   band <= gfc->highpass_band) {
					memset((char*)mdct_enc, 0,
					       18 * sizeof(FLOAT8));
				} else {
					if(type == SHORT_TYPE) {
						for(k = -NS / 4; k < 0; k++) {
		...
		wk = w1 + 286;
		if(gfc->mode_gr == 1) {
			memcpy(gfc->sb_sample[ch][0], gfc->sb_sample[ch][1],
			       576 * sizeof(FLOAT8));
		}
	}
}
