...
int
count_bits_long(lame_internal_flags *const gfc, const int ix[576], gr_info *const gi)
{
	int i, a1, a2;
	int bits = 0;
    ...
    III_scalefac_t scalefac[2][2])
{
	...
	if(gfc->mode_gr == 2 && gr == 1 && l3_side->gr[0].ch[ch].tt.block_type != SHORT_TYPE && l3_side->gr[1].ch[ch].tt.block_type != SHORT_TYPE) {
