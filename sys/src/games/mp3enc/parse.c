           ...
           const char* ProgramName)
{
	...
	fprintf(
	    ...
	    "\n"
	    "    --preset type   type must be phone, voice, fm, tape, hifi, cd "
	    "or studio\n"
	    "                    \"--preset help\" gives some more infos on "
	    "these\n"
	    "\n"
          ...
          int lessmode)
{
	...
	fprintf(fp, "usage: %s [options] <infile> [outfile]\n\n"
	            ...
	            "    -s sfreq        sampling frequency of input file "
	            "(kHz) - default 44.1 kHz\n"
	...
	fprintf(
	    ...
	    "                    auto = jstereo, with varialbe mid/side "
	    "threshold\n"
	    "    -a              downmix from stereo to mono file for mono "
	    "encoding\n"
	    ...
	    "    -t              disable writing wav header when using "
	    "--decode\n"
	    "    --comp  <arg>   choose bitrate to achive a compression ratio "
	    "of <arg>\n"
	...
	fprintf(
	    ...
	    "\n"
	    "  ABR options:\n"
	    "    --abr <bitrate> specify average bitrate desired (instead of "
	    "quality)\n"
	    "\n"
	    "  VBR options:\n"
	    ...
	    "    -b <bitrate>    specify minimum allowed bitrate, default  32 "
	    "kbps\n"
	    "    -B <bitrate>    specify maximum allowed bitrate, default 320 "
	    "kbps\n"
	    "    -F              strictly enforce the -b option, for use with "
	    "players that\n"
	    "                    do not support low bitrate mp3 (Apex AD600-A "
	    "DVD/mp3 player)\n"
	...
	fprintf(
	    ...
	    "    -p              error protection.  adds 16 bit checksum to "
	    "every frame\n"
	    "                    (the checksum is computed correctly)\n"
	    "    --nores         disable the bit reservoir\n"
	    "    --strictly-enforce-ISO   comply as much as possible to ISO "
	    "MPEG spec\n"
	    "\n"
	    "  Filter options:\n"
	    "    -k              keep ALL frequencies (disables all filters),\n"
	    "                    Can cause ringing and twinkling\n"
	    "  --lowpass <freq>        frequency(kHz), lowpass filter cutoff "
	    "above freq\n"
	    "  --lowpass-width <freq>  frequency(kHz) - default 15%% of "
	    "lowpass freq\n"
	    "  --highpass <freq>       frequency(kHz), highpass filter cutoff "
	    "below freq\n"
	    "  --highpass-width <freq> frequency(kHz) - default 15%% of "
	    "highpass freq\n"
	    "  --resample <sfreq>  sampling frequency of output file(kHz)- "
	    "default=automatic\n"
	    "  --cwlimit <freq>    compute tonality up to freq (in kHz) "
	    "default 8.8717");
	...
	fprintf(
	    fp,
	    "  ID3 tag options:\n"
	    "    --tt <title>    audio/song title (max 30 chars for version 1 "
	    "tag)\n"
	    "    --ta <artist>   audio/song artist (max 30 chars for version 1 "
	    "tag)\n"
	    "    --tl <album>    audio/song album (max 30 chars for version 1 "
	    "tag)\n"
	    "    --ty <year>     audio/song year of issue (1 to 9999)\n"
	    "    --tc <comment>  user-defined text (max 30 chars for v1 tag, "
	    "28 for v1.1)\n"
	    "    --tn <track>    audio/song track number (1 to 255, creates "
	    "v1.1 tag)\n"
	    "    --tg <genre>    audio/song genre (name or number in list)\n"
	    "    --add-id3v2     force addition of version 2 tag\n"
	    "    --id3v1-only    add only a version 1 tag\n"
	    "    --id3v2-only    add only a version 2 tag\n"
	    "    --space-id3v1   pad version 1 tag with spaces instead of "
	    "nulls\n"
	    "    --pad-id3v2     pad version 2 tag with extra 128 bytes\n"
	    "    --genre-list    print alphabetically sorted ID3 genre list "
	    "and exit\n"
	    "\n"
	    "    Note: A version 2 tag will NOT be added unless one of the "
	    "input fields\n"
	    "    won't fit in a version 1 tag (e.g. the title string is longer "
	    "than 30\n"
	    "    characters), or the '--add-id3v2' or '--id3v2-only' options "
	    "are used,\n"
	    "    or output is redirected to stdout."
#if defined(HAVE_VORBIS)
	    "\n\n"
	    "    Note: All '--t*' options (except those for track and genre) "
	    "work for Ogg\n"
...
typedef struct {
	...
	int32_t
	    resample; /* resample frequency in Hz, or -1 for no resampling */
	int16_t highpass_freq; /* highpass frequency in Hz, or -1 for no
	                          highpass filtering */
	int16_t lowpass_freq;  /* lowpass frequency in Hz, or -1 for no lowpass
	                          filtering */
...
const preset_t Presets[] = {
    /* name       fs     fu    fo    dfo shrt qual  mode       cbr
       vbr_mode/min/max */
    {"phone", 8000, 125, 3400, 0, 1, 5, MONO, 16, 6, 8,
     24}, /* phone standard 300-3400 */
    {"phon+", 11025, 100, 4000, 0, 1, 5, MONO, 24, 4, 16,
     32}, /* phone theoretical limits */
    {"lw", 11025, -1, 4000, 0, 0, 5, MONO, 24, 3, 16, 56},    /* LW */
    {"mw-eu", 11025, -1, 4000, 0, 0, 5, MONO, 24, 3, 16, 56}, /* MW in europe */
    {"mw-us", 16000, -1, 7500, 0, 0, 5, MONO, 40, 3, 24,
     112},                                                 /* MW in U.S.A. */
             ...
             const char* ProgramName)
{
	...
	fprintf(fp, "    equal to: -mj -b112 --resample 32 --lowpass 15 "
	            "--lowpass-width 0\n");
              ...
              const char* ProgramName)
{
	...
	for(i = 0; i < sizeof Presets / sizeof *Presets; i++)
		if(0 ==
		   strncmp(preset_name, Presets[i].name, strlen(preset_name))) {
			if(Presets[i].resample >= 0)
				(void)lame_set_out_samplerate(
				    gfp, Presets[i].resample);
			if(Presets[i].highpass_freq >= 0)
...
#define T_ELIF2(str1, str2)                                                    \
	...
	else if(0 == local_strcasecmp(token, str1) ||                          \
...
parse_args(lame_global_flags* gfp, int argc, char** argv)
{
	...
	for(i = 0, err = 0; ++i < argc && !err;) {
		...
		if(*token++ == '-') {
			...
			if(*token == '-') { /* GNU style */
				...
				argUsed = 1;
				(void)lame_set_out_samplerate(
				    gfp, resample_rate(atof(nextArg)));
				...
				if(gfp->VBR_mean_bitrate_kbps >= 8000)
					gfp->VBR_mean_bitrate_kbps =
					    (gfp->VBR_mean_bitrate_kbps + 500) /
					    1000;
				gfp->VBR_mean_bitrate_kbps =
				    Min(gfp->VBR_mean_bitrate_kbps, 320);
				gfp->VBR_mean_bitrate_kbps =
				    Max(gfp->VBR_mean_bitrate_kbps, 8);
				...
				input_format = sf_ogg;
#else
				fprintf(stderr, "Error: mp3enc not compiled "
				                "with Vorbis support\n");
				return -1;
#endif
				T_ELIF("ogg")
				...
				val = atof(nextArg);
				argUsed = 1;
				/* useful are 0.001 kHz...50 kHz, 50 Hz...50000
				 * Hz */
				gfp->lowpassfreq =
				    val * (val < 50. ? 1.e3 : 1.e0) + 0.5;
				if(val < 0.001 || val > 50000.) {
					fprintf(stderr, "Must specify lowpass "
					                "with --lowpass freq, "
					                "freq >= 0.001 kHz\n");
					return -1;
				}
				...
				val = atof(nextArg);
				argUsed = 1;
				/* useful are 0.001 kHz...16 kHz, 16 Hz...50000
				 * Hz */
				gfp->highpassfreq =
				    val * (val < 16. ? 1.e3 : 1.e0) + 0.5;
				if(val < 0.001 || val > 50000.) {
					fprintf(stderr, "Must specify highpass "
					                "with --highpass freq, "
					                "freq >= 0.001 kHz\n");
					return -1;
				}
				...
				if(gfp->highpasswidth < 0) {
					fprintf(stderr,
					        "Must specify highpass width "
					        "with --highpass-width freq, "
					        "freq >= 0 kHz\n");
					return -1;
				}
				...
				if(gfp->cwlimit <= 0) {
					fprintf(stderr, "Must specify cwlimit "
					                "with --cwlimit freq, "
					                "freq >= 0.001 kHz\n");
					return -1;
				}
		 ...
		 */
				...
				silent =
				    10; /* on a scale from 1 to 10 be very
				           silent */

				T_ELIF("verbose")
				silent =
				    0; /* print a lot on screen, the default */
				...
				T_ELIF("longhelp")
				long_help(gfp, stdout, ProgramName,
				          0 /* lessmode=NO */);
				return -2;
				...
				while((c = *token++) != '\0') {
					...
					case 't': /* dont write VBR tag */
						(void)lame_set_bWriteVbrTag(gfp,
						                            0);
						(void)
						    lame_set_disable_waveheader(
						        gfp, 1);
						break;
					...
					case 'x': /* force byte swapping */
						swapbytes = 1;
						break;
					case 'p': /* (jo) error_protection: add
					             crc16 information to stream
					             */
						gfp->error_protection = 1;
						break;
					case 'a': /* autoconvert input file from
					             stereo to mono - for mono
					             mp3 encoding */
						autoconvert = 1;
	...
	if(input_format == sf_mp1 || input_format == sf_mp2 ||
	   input_format == sf_mp3) {
		fprintf(stderr, "Error: libmp3lame not compiled with mpg123 "
		                "*decoding* support \n");
		return -1;
	}
#endif
	...
	if(input_format == sf_ogg) {
		fprintf(stderr,
		        "Error: mp3enc not compiled with Vorbis support\n");
		return -1;
	}
#endif
	...
	if(gfp->free_format) {
		if(gfp->brate < 8 || gfp->brate > 640) {
			fprintf(stderr, "For free format, specify a bitrate "
			                "between 8 and 640 kbps\n");
			return -1;
		}
	}
	return 0;
