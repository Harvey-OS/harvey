...
static const FLOAT costab[TRI_SIZE * 2] = {
    9.238795325112867e-01, 3.826834323650898e-01, 9.951847266721969e-01,
    9.801714032956060e-02, 9.996988186962042e-01, 2.454122852291229e-02,
...
void
fft_short(lame_internal_flags* gfc, FLOAT x_real[3][BLKSIZE_s], int chn,
...
void
fft_long(lame_internal_flags* const gfc, FLOAT x[BLKSIZE], int chn,
         const sample_t* buffer[2])
{
...
init_fft(lame_internal_flags* const gfc)
{
	...
	for(i = 0; i < BLKSIZE_s / 2; i++)
		window_s[i] =
		    0.5 * (1.0 - cos(2.0 * PI * (i + 0.5) / BLKSIZE_s));
}
