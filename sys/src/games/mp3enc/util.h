...
typedef struct {
	...
	long double
	    sample_freq_out; /* requested Output sample frequency in Hz */
 ...
 * Other remark:
 *     If you set a flag to 0 for uninit data and 1 for init data, the right
 *test
...
#define LAME_ID 0xFFF88E3B
	...
	int channels_in;  /* number of channels in the input data stream (PCM or
	                     decoded PCM) */
	int channels_out; /* number of channels in the output data stream (not
	                     used for decoding) */
	resample_t* resample_in; /* context for coding (PCM=>MP3) resampling */
	resample_t*
	    resample_out; /* context for decoding (MP3=>PCM) resampling */
...
#define MFSIZE (3 * 1152 + ENCDELAY - MDCTDELAY)
	...
	lame_global_flags* gfp; /* needed as long as the frame encoding
	                           functions must access gfp (all needed
	                           information can be added to gfc) */
	coding_t coding; /* MPEG Layer 1/2/3, Ogg Vorbis, MPEG AAC, ... */
	unsigned long frame_count; /* Number of frames coded, 2^32 > 3 years */
	int mf_samples_to_encode;
	int mf_size;
	float
	    ampl; /* amplification at the end of the current chunk (1. = 0 dB)
	             */
	float last_ampl; /* amplification at the end of the last chunk    (1. =
	                    0 dB) */
	...
	float highpass1,
	    highpass2; /* normalized frequency bounds of passband */

	/* polyphase filter (filter_type=0)  */
	int lowpass_band;        /* zero bands >= lowpass_band in the polyphase
	                            filterbank */
	...
	int filter_type;   /* 0=polyphase filter, 1= FIR filter 2=MDCT
	                      filter(bad)*/
	...
	int noise_shaping_stop; /* 0 = stop at over=0, all scalefacs amplified
	                           or
...
#define MAX_HEADER_LEN 40 /* max size of header is 38 */
	...
	/* functions to replace with CPU feature optimized versions in
	 * takehiro.c */
...
extern void getframebits(lame_global_flags* gfp, int* bitsPerFrame,
                         int* mean_bits);

void fill_buffer(lame_global_flags* gfp, sample_t* mfbuf[2],
                 sample_t* in_buffer[2], int nsamples, int* n_in, int* n_out);

int fill_buffer_resample(lame_global_flags* gfp, sample_t* outbuf,
                         int desired_len, sample_t* inbuf, int len,
                         int* num_used, int channels);
