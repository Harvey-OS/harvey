 ...
 *				nMode	: Channel Mode: 0=STEREO 1=JS 2=DS
 *3=MONO
 ****************************************************************************
*/
int
	...
	*/
	{
		...
		gfp->TotalFrameSize = ((gfp->version + 1) * 72000 * bitrate) /
		                      gfp->out_samplerate;
 ...
 *				nVbrScale	: encoder quality indicator
 *(0..100)
 ****************************************************************************
*/
int
PutVbrTag(lame_global_flags* gfp, FILE* fpStream, int nVbrScale)
{
	...
	if(gfp->version == 1) {
		...
		abyte = pbtStreamBuffer[2] &
		        (char)0x0d; /* AF keep also private bit */
		pbtStreamBuffer[2] =
		    (char)bbyte | abyte; /* 64kbs MPEG1 frame */
	} else {
		/* MPEG2 */
		...
		abyte = pbtStreamBuffer[2] &
		        (char)0x0d; /* AF keep also private bit */
		pbtStreamBuffer[2] =
		    (char)bbyte | abyte; /* 64kbs MPEG2 frame */
	}
	...
	if(fwrite(pbtStreamBuffer, (unsigned int)gfp->TotalFrameSize, 1,
	          fpStream) != 1) {
		return -1;
	}
