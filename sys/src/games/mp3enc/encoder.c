 ...
 */
void
adjust_ATH(lame_global_flags* const gfp, FLOAT8 tot_ener[2][4])
{
	...
	if(gfc->ATH->use_adjust) {
		 ...
		 */
		if(vbr_mtrh == gfp->VBR) {
			/*  this code reduces slowly the ATH (speed of 12 dB per
			 * second)
			...
			if(0.5 < max_val / 32768) {     /* value above 50 % */
				gfc->ATH->adjust = 1.0; /* do not reduce ATH */
			          ...
			          max_val / 32768) { /* value above 30 % */
				gfc->ATH->adjust *=
				    0.955; /* reduce by ~0.2 dB */
				if(gfc->ATH->adjust <
				   0.3) /* but ~5 dB in maximum */
					gfc->ATH->adjust = 0.3;
			} else { /* value below 30 % */
				gfc->ATH->adjust *=
				    0.93; /* reduce by ~0.3 dB */
				if(gfc->ATH->adjust <
				   0.01) /* but 20 dB in maximum */
					gfc->ATH->adjust = 0.01;
			}
#else /* jd - 27 feb 2001 */
			...
			if(max_val_n >
			   0.25) { /* sqrt((1 - 0.01) / 15.84) from curve
			              below*/
				if(gfc->ATH->adjust >= 1.0) {
					gfc->ATH->adjust = 1.0;
				} else { /* preceding frame has lower ATH
				            adjust; */
					/* ascend only to the preceding
					 * adjust_limit */
					/* in case there is leading low volume
					 */
					if(gfc->ATH->adjust <
					   gfc->ATH->adjust_limit) {
						gfc->ATH->adjust =
						    gfc->ATH->adjust_limit;
					}
				}
				...
				adj_lim_new =
				    15.84 * (max_val_n * max_val_n) + 0.01;
				if(gfc->ATH->adjust >=
				   adj_lim_new) { /* descend gradually */
					gfc->ATH->adjust *=
					    adj_lim_new * 0.075 + 0.925;
					if(gfc->ATH->adjust <
					   adj_lim_new) { /* stop descent */
						gfc->ATH->adjust = adj_lim_new;
					}
				} else { /* ascend */
					if(gfc->ATH->adjust_limit >=
					   adj_lim_new) {
						gfc->ATH->adjust = adj_lim_new;
					} else { /* preceding frame has lower
					            ATH adjust; */
						/* ascend only to the preceding
						 * adjust_limit */
						if(gfc->ATH->adjust <
    ...
    int mp3buf_size)              // Output
{
	...
	if(gfc->lame_encode_frame_init == 0) {
		...
		gfc->frac_SpF = ((gfp->version + 1) * 72000L * gfp->brate) %
		                gfp->out_samplerate;
...
#error FFTOFFSET greater than 576: FFT uses a negative offset
		...
		/* prime the MDCT/polyphase filterbank with a short block */
		{
			...
			for(i = 0, j = 0; i < 286 + 576 * (1 + gfc->mode_gr);
			    ++i) {
	...
	default:
		if(gfp->VBR != vbr_off) {
			...
			if(gfp->disable_reservoir) {
				gfc->padding = 0;
				/* if the user specified --nores, dont very
				 * gfc->padding either */
				/* tiny changes in frac_SpF rounding will cause
				 * file differences */
			} else {
				/* padding method as described in
				 * "MPEG-Layer3 / Bitstream Syntax and Decoding"
				 * by Martin Sieler, Ralph Sperschneider
				 *
				 * note: there is no padding for the very first
				 *frame
				 *
				 * Robert.Hegemann@gmx.de 2000-06-22
				 */
	...
	if(gfc->psymodel) {
		 ...
		 * psy model has a 1 granule (576) delay that we must compensate
		 * for
		 * (mt 6/99).
		 */
		int ret;
		const sample_t*
		    bufp[2]; /* address of beginning of left & right granule */
		int blocktype[2];
		...
		for(gr = 0; gr < gfc->mode_gr; gr++) {
			...
			for(ch = 0; ch < gfc->channels_out; ch++)
				bufp[ch] =
				    &inbuf[ch][576 + gr * 576 - FFTOFFSET];

			if(gfc->nsPsy.use) {
				ret = L3psycho_anal_ns(
				    gfp, bufp, gr, &gfc->ms_ratio[gr],
				    &ms_ratio_next, masking_LR, masking_MS,
	...
	for(gr = 0; gr < gfc->mode_gr; gr++) {
		for(ch = 0; ch < gfc->channels_out; ch++) {
			...
			cod_info->mixed_block_flag =
			    0; /* never used by this model */
	...
	if(check_ms_stereo) {
		...
		check_ms_stereo = (gfc->l3_side.gr[gr0].ch[0].tt.block_type ==
		                   gfc->l3_side.gr[gr0].ch[1].tt.block_type) &&
		                  (gfc->l3_side.gr[gr1].ch[0].tt.block_type ==
		                   gfc->l3_side.gr[gr1].ch[1].tt.block_type);
	}
	...
	if(gfp->force_ms) {
		...
		*/
		...
		if(gfc->mode_gr == 1) {
			...
			ms_ratio_ave1 = 0.33 * (gfc->ms_ratio[0] +
			                        ms_ratio_prev + ms_ratio_next);
			ms_ratio_ave2 = gfc->ms_ratio[0];
		} else {
			ms_ratio_ave1 =
			    0.25 * (gfc->ms_ratio[0] + gfc->ms_ratio[1] +
			            ms_ratio_prev + ms_ratio_next);
			ms_ratio_ave2 =
			    0.50 * (gfc->ms_ratio[0] + gfc->ms_ratio[1]);
		}

		if(gfp->mode_automs) {
			if(gfp->compression_ratio < 11.025) {
				/* 11.025 => 1, 6.3 => 0 */
				double thr = (gfp->compression_ratio - 6.3) /
				             (11.025 - 6.3);
				if(thr < 0)
					thr = 0;
				threshold1 *= thr;
		...
		if((ms_ratio_ave1 < threshold1 && ms_ratio_ave2 < threshold2) ||
		   gfc->nsPsy.use) {
			...
			/* based on PE: M/S coding would not use much more bits
			 * than L/R coding */

			if(sum_pe_MS <= 1.07 * sum_pe_LR && !gfc->nsPsy.use)
				gfc->mode_ext = MPG_MD_MS_LR;
			if(sum_pe_MS <= 1.00 * sum_pe_LR && gfc->nsPsy.use)
				gfc->mode_ext = MPG_MD_MS_LR;
		}
	}
	...
	if(gfp->analysis && gfc->pinfo != NULL) {
		for(gr = 0; gr < gfc->mode_gr; gr++) {
			for(ch = 0; ch < gfc->channels_out; ch++) {
				...
				gfc->pinfo->ms_ener_ratio[gr] =
				    ms_ener_ratio[gr];
				gfc->pinfo->blocktype[gr][ch] =
				    gfc->l3_side.gr[gr].ch[ch].tt.block_type;
				memcpy(gfc->pinfo->xr[gr][ch], xr[gr][ch],
				       sizeof(xr[gr][ch]));
				/* in psymodel, LR and MS data was stored in
				   pinfo.
				   switch to MS data: */
				if(gfc->mode_ext == MPG_MD_MS_LR) {
					gfc->pinfo->pe[gr][ch] =
					    gfc->pinfo->pe[gr][ch + 2];
					gfc->pinfo->ers[gr][ch] =
					    gfc->pinfo->ers[gr][ch + 2];
					memcpy(
	...
	if(gfc->nsPsy.use && (gfp->VBR == vbr_off || gfp->VBR == vbr_abr)) {
		static FLOAT fircoef[19] = {
		    -0.0207887, -0.0378413, -0.0432472, -0.031183,  7.79609e-18,
		    0.0467745,  0.10091,    0.151365,   0.187098,   0.2,
		    0.187098,   0.151365,   0.10091,    0.0467745,  7.79609e-18,
		    -0.031183,  -0.0432472, -0.0378413, -0.0207887,
		};
		int i;
		FLOAT8 f;
		...
		f = 0;
		for(i = 0; i < 19; i++)
			f += gfc->nsPsy.pefirbuf[i] * fircoef[i];
	...
	default:
	case vbr_off:
		iteration_loop(gfp, *pe_use, ms_ener_ratio, xr, *masking,
		               l3_enc, scalefac);
		break;
	case vbr_mt:
		VBR_quantize(gfp, *pe_use, ms_ener_ratio, xr, *masking, l3_enc,
		             scalefac);
		break;
	case vbr_rh:
	case vbr_mtrh:
		VBR_iteration_loop(gfp, *pe_use, ms_ener_ratio, xr, *masking,
		                   l3_enc, scalefac);
		break;
	case vbr_abr:
		ABR_iteration_loop(gfp, *pe_use, ms_ener_ratio, xr, *masking,
		                   l3_enc, scalefac);
		break;
	}
