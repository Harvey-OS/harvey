 ...
 * Another nasty thing is that the rest of the code doesn't work for 16 bit
 * ints,
...
#define FloatToUnsigned(f)                                                     \
	((unsigned long)(((int32_t)((f)-2147483648.0)) + 2147483647L + 1))
#define UnsignedToFloat(u)                                                     \
	(((double)((int32_t)((u)-2147483647L - 1))) + 2147483648.0)
#endif /* applec */
       /****************************************************************
...
ConvertFromIeeeExtended(char* bytes)
{
...
#ifdef TEST
	printf(
	    "ConvertFromIEEEExtended(%lx,%lx,%lx,%lx,%lx,%lx,%lx,%lx,%lx,%lx\r",
	    (int32_t)bytes[0], (int32_t)bytes[1], (int32_t)bytes[2],
	    (int32_t)bytes[3], (int32_t)bytes[4], (int32_t)bytes[5],
	    (int32_t)bytes[6], (int32_t)bytes[7], (int32_t)bytes[8],
	    (int32_t)bytes[9]);
#endif
	...
	if(expon == 0 && hiMant == 0 && loMant == 0) {
		...
		/* This case should also be called if the number is too large to
		 * fit into
		 * a double variable */
