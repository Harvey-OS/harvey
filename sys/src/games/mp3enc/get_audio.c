...
/* and for Vorbis: */
int lame_decode_ogg_initfile(lame_global_flags* gfp, FILE* fd,
                             mp3data_struct* mp3data);
int lame_decode_ogg_fromfile(lame_global_flags* gfc, FILE* fd,
                             short int pcm_l[], short int pcm_r[],
                             mp3data_struct* mp3data);

static int read_samples_pcm(FILE* musicin, int16_t sample_buffer[2304],
...
fskip(FILE* fp, int32_t offset, int whence)
{
	...
	if(whence != SEEK_CUR || offset < 0) {
		fprintf(stderr, "fskip problem: Mostly the return status of "
		                "functions is not evaluated so it is more "
		                "secure to pollute <stderr>.\n");
		return -1;
	}
...
init_outfile(char* outPath, int decode)
{
	...
	else if((outf = fopen(outPath, "wb+")) == NULL)
		return NULL;
	return outf;
}
...
void
SwapBytesInWords(int16_t* ptr, int short_words)
{ /* Some speedy code */
	unsigned long val;
...
************************************************************************/
int
get_audio(lame_global_flags* const gfp, int16_t buffer[2][1152])
{
	int num_channels = lame_get_num_channels(gfp);
	int16_t insamp[2 * 1152];
	int samples_read;
	int framesize;
	int samples_to_read;
	...
	if(count_samples_carefully) {
		remaining =
		    tmp_num_samples - Min(tmp_num_samples, num_samples_read);
	...
	case sf_mp3:
		samples_read =
		    read_samples_mp3(gfp, musicin, buffer, num_channels);
		break;
	case sf_ogg:
		samples_read =
		    read_samples_ogg(gfp, musicin, buffer, num_channels);
		break;
	default:
		samples_read =
...
int
read_samples_ogg(lame_global_flags* const gfp, FILE* const musicin,
                 short int oggpcm[2][1152], const int stereo)
{
	...
	out = lame_decode_ogg_fromfile(gfp, musicin, oggpcm[0], oggpcm[1],
	                               &mp3input_data);
	/*
	...
	if(lame_get_num_channels(gfp) != mp3input_data.stereo)
		fprintf(stderr, "Error: number of channels has changed in %s - "
		                "not supported\n",
		        type_name);
	if(lame_get_in_samplerate(gfp) != mp3input_data.samplerate)
		fprintf(stderr, "Error: sample frequency has changed in %s - "
		                "not supported\n",
		        type_name);
...
int
read_samples_mp3(lame_global_flags* const gfp, FILE* const musicin,
...
static int
WriteWaveHeader(FILE* const fp, const int pcmbytes, const int freq,
                const int channels, const int bits)
{
	...
	Write32BitsLowHigh(fp,
	                   pcmbytes + 44 - 8); // length in bytes without header
	fwrite("WAVEfmt ", 2, 4, fp);          // 2 labels
	Write32BitsLowHigh(fp, 2 + 2 + 4 + 4 + 2 +
	                           2); // length of PCM format declaration area
             ...
             char* outPath)
{
	...
	fprintf(stderr, "\rinput:  %s%s(%g kHz, %i channel%s, ",
	        ...
	        strlen(inPath) > 26 ? "\n\t" : "  ",
	        lame_get_in_samplerate(gfp) / 1.e3, tmp_num_channels,
	        tmp_num_channels != 1 ? "s" : "");

	switch(input_format) {
	case sf_mp3:
		skip += 528 + 1; /* mp3 decoder has a 528 sample delay, plus
		                    user supplied "skip" */
	...
	case sf_ogg:
		...
		skip = 0; /* other formats have no delay */ /* is += 0 not
		                                               better ??? */
		break;
	case sf_raw:
	...
	do {
		...
		skip -=
		    (i = skip < iread ? skip : iread); /* 'i' samples are to
		                                          skip in this frame */
	...
	if(0 == lame_get_disable_waveheader(gfp))
		if(!fseek(outf, 0l, SEEK_SET)) /* if outf is seekable, rewind
		                                  and adjust length */
			WriteWaveHeader(outf, wavsize,
...
OpenSndFile(lame_global_flags* gfp, char* inPath)
{
	...
	if(input_format == sf_mp1 || input_format == sf_mp2 ||
	   input_format == sf_mp3) {
		...
		if(-1 ==
		   lame_decode_ogg_initfile(gfp, musicin, &mp3input_data)) {
			fprintf(stderr,
			        "Error reading headers in mp3 input file %s.\n",
			        lpszFileName);
			exit(1);
...
#else
		fprintf(stderr,
		        "mp3enc not compiled with libvorbis support.\n");
		exit(1);
#endif
		...
		gs_wfInfo.seekable =
		    (input_format !=
		     sf_raw); /* if user specified -r, set to not seekable */
...
static int
read_samples_pcm(FILE* const musicin, int16_t sample_buffer[2304],
...
int
read_samples_pcm(FILE* musicin, int16_t sample_buffer[2304], int frame_size,
                 int samples_to_read)
{
	int samples_read;
	...
	if(16 == pcmbitwidth) {
		samples_read =
		    fread(sample_buffer, 2, samples_to_read, musicin);
...
parse_wave_header(lame_global_flags* gfp, FILE* sf)
{
	...
	int is_wav = 0;
	int32_t data_length = 0, file_length, subSize = 0;
	int loop_sanity = 0;
	...
	for(loop_sanity = 0; loop_sanity < 20; ++loop_sanity) {
		...
		if(type == WAV_ID_FMT) {
			...
			if(subSize < 16) {
				/*DEBUGF(
				   "'fmt' chunk too short (only %ld bytes)!",
				   subSize);  */
				return 0;
			}
			...
			if(subSize > 0) {
				if(fskip(sf, (int32_t)subSize, SEEK_CUR) != 0)
					return 0;
			};
...
aiff_check2(const char* file_name, IFF_AIFF* const pcm_aiff_data)
{
	...
	if(pcm_aiff_data->sampleSize != sizeof(int16_t) * CHAR_BIT) {
		fprintf(stderr, "Sound data is not %i bits in '%s'\n",
		        sizeof(int16_t) * CHAR_BIT, file_name);
		return 1;
	}
...
parse_aiff_header(lame_global_flags* gfp, FILE* sf)
{
	...
	while(chunkSize > 0) {
		...
		/* don't use a switch here to make it easier to use 'break' for
		 * SSND */
		if(type == IFF_ID_COMM) {
			...
			if(fskip(sf, (int32_t)aiff_info.blkAlgn.offset,
			         SEEK_CUR) != 0)
				return 0;
...
parse_file_header(lame_global_flags* gfp, FILE* sf)
{
	...
	if(input_format == sf_raw) {
		/*
		   ** Assume it's raw PCM.  Since the audio data is assumed to
		  *begin
...
OpenSndFile(lame_global_flags* gfp, char* inPath)
{
	...
	if(!strcmp(inPath, "-")) {
		lame_set_stream_binary_mode(
		    musicin = stdin); /* Read from standard input. */
	} else {
	...
	if(input_format == sf_mp1 || input_format == sf_mp2 ||
	   input_format == sf_mp3) {
		...
		if(-1 ==
		   lame_decode_ogg_initfile(gfp, musicin, &mp3input_data)) {
			fprintf(stderr,
			        "Error reading headers in ogg input file %s.\n",
			        inPath);
			exit(1);
		}
		if(-1 == lame_set_num_channels(gfp, mp3input_data.stereo)) {
	...
	if(lame_get_num_samples(gfp) == MAX_U_32_NUM && musicin != stdin) {
		double flen = lame_get_file_size(
		    inPath); /* try to figure out num_samples */
 ...
 * This is a (nearly?) complete header analysis for a MPEG-1/2/2.5 Layer I, II
 * or III
 * data stream
 */
...
is_syncword_mp123(const void* const headerptr)
{
	...
	static const char abl2[16] = {0, 7, 7, 7, 0, 7, 0, 0,
	                              0, 0, 0, 8, 8, 8, 8, 8};
...
is_syncword_mp3(const void* const headerptr)
{
	...
	if((p[1] & 0x18) == 0x08)
		return 0; // no MPEG-1, -2 or -2.5
	if((p[1] & 0x06) != 0x02)
		return 0; // no Layer III (can be merged with 'next 3 bits are
		          // also' test, but don't do this, this decreases
		          // readability)
	if((p[2] & 0xF0) == 0xF0)
		return 0; // bad bitrate
	if((p[2] & 0x0C) == 0x0C)
...
lame_decode_initfile(FILE* fd, mp3data_struct* mp3data)
{
	...
	if(aid_header) {
		if(fread(&buf, 1, 2, fd) != 2)
			return -1; /* failed */
		aid_header =
		    (unsigned char)buf[0] + 256 * (unsigned char)buf[1];
	...
	if(mp3data->totalframes > 0) {
		/* mpglib found a Xing VBR header and computed nsamp &
		 * totalframes */
	} else {
...
*/
int
lame_decode_fromfile(FILE* fd, int16_t pcm_l[], int16_t pcm_r[],
                     mp3data_struct* mp3data)
{
	int ret = 0, len;
	unsigned char buf[100];
