              ...
              const int framesize)        // Size of a frame [bps/kbps]
{
	...
	if(frameNum > 0 && tstime->elapsed_time > 0) {
		tstime->estimated_time =
		    tstime->elapsed_time * totalframes / frameNum;
		tstime->speed_index =
...
ts_time_decompose(const unsigned long time_in_sec, const char padded_char)
{
	...
	if(hour == 0)
		...
		fprintf(stderr, "%2lu:%02u:%02u%c", hour, min, sec,
		        padded_char);
	else
...
void
timestatus(const int samp_rate, const int frameNum, const int totalframes,
           const int framesize)
{
	...
	int percent;
	static int init =
	    0; /* What happens here? A work around instead of a bug fix ??? */
	...
	fprintf(stderr, proc_time.speed_index <= 1. ? "%9.4f" SPEED_CHAR "|"
	                                            : "%#9.5g" SPEED_CHAR "|",
	        SPEED_MULT * proc_time.speed_index);
	ts_time_decompose(
	    (unsigned long)(real_time.estimated_time - real_time.elapsed_time),
	    ' ');
	fflush(stderr);
}
...
timestatus_klemm(const lame_global_flags* const gfp)
{
	...
	if(!silent)
		if(gfp->frameNum == 0 || gfp->frameNum == 9 ||
		   GetRealTime() - last_time >= update_interval ||
		   GetRealTime() - last_time < 0) {
#ifdef BRHIST
			brhist_jump_back();
#endif
			timestatus(lame_get_out_samplerate(gfp), gfp->frameNum,
			           gfp->totalframes, gfp->framesize);
#ifdef BRHIST
			if(brhist) {
				brhist_disp(gfp);
			}
#endif
			last_time =
			    GetRealTime(); /* from now! disp_time seconds */
		}
}
...
void
decoder_progress(const lame_global_flags* const gfp,
                 const mp3data_struct* const mp3data)
{
	static int last;
