...
main(int argc, char** argv)
{
	...
	if(lame_get_decode_only(gf))
		...
		if(mp3_delay_set)
			...
			lame_decoder(gf, outf, gf->encoder_delay, inPath,
			             outPath);
	else {
		...
		do {
			...
			/* encode */
			imp3 =
			    lame_encode_buffer(gf, Buffer[0], Buffer[1], iread,
			                       mp3buffer, sizeof(mp3buffer));

			/* was our output buffer big enough? */
			if(imp3 < 0) {
				if(imp3 == -1)
					fprintf(stderr, "mp3 buffer is not big "
					                "enough... \n");
				else
					fprintf(stderr, "mp3 internal error:  "
					                "error code=%i\n",
					        imp3);
				return 1;
			}
		...
		if(!silent) {
			...
			brhist_jump_back();
#endif
			timestatus(lame_get_out_samplerate(gf), gf->frameNum,
			           gf->totalframes, gf->framesize);
#ifdef BRHIST
			if(brhist)
				brhist_disp(gf);
