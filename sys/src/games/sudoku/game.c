...
int allowbits[Brdsize] = {0x00100, 0x00200, 0x00400, 0x00800, 0x01000,
		...
		for(i = 0; i < Psize; i++) {
			...
			if((0 <= c) && (c < 9)) {
				if(!setallowed(pboard, i, c)) {
					print("starting position impossible... "
					      "failed at cell %d char: %d\n",
					      i, c + 1);
					return;
				}
			}
