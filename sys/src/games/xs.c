...
Cursor whitearrow = {
    {0, 0},
    {
     0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFE, 0xFF, 0xFC, 0xFF, 0xF0, 0xFF, 0xF0,
     0xFF, 0xF8, 0xFF, 0xFC, 0xFF, 0xFE, 0xFF, 0xFF, 0xFF, 0xFE, 0xFF, 0xFC,
     0xF3, 0xF8, 0xF1, 0xF0, 0xE0, 0xE0, 0xC0, 0x40,
    },
    {
     0xFF, 0xFF, 0xFF, 0xFF, 0xC0, 0x06, 0xC0, 0x1C, 0xC0, 0x30, 0xC0, 0x30,
     0xC0, 0x38, 0xC0, 0x1C, 0xC0, 0x0E, 0xC0, 0x07, 0xCE, 0x0E, 0xDF, 0x1C,
...
enum { TIMER, MOUSE, RESHAPE, KBD, SUSPEND, NALT };
...
uint8_t txbits[NCOL][32] = {
    {0xDD, 0xDD, 0xFF, 0xFF, 0x77, 0x77, 0xFF, 0xFF, 0xDD, 0xDD, 0xFF, 0xFF,
     0x77, 0x77, 0xFF, 0xFF, 0xDD, 0xDD, 0xFF, 0xFF, 0x77, 0x77, 0xFF, 0xFF,
...
{
	mouse.xy =
	    Pt(rboard.min.x + Dx(rboard) / 2, rboard.min.y + Dy(rboard) / 2);
...
score(int p)
{
	...
	draw(screen, Rpt(pscore, addpt(pscore, scoresz)), display->white, nil,
	     ZP);
...
drawboard(void)
{
	...
	draw(screen,
	     Rect(rboard.min.x, rboard.min.y - 2, rboard.max.x, rboard.min.y),
	     display->white, nil, ZP);
	for(i = 0; i < NY; i++)
		for(j = 0; j < NX; j++)
			if(board[i][j])
				drawsq(screen, Pt(rboard.min.x + j * pcsz,
				                  rboard.min.y + i * pcsz),
				       board[i][j] - 16);
	score(0);
	if(suspended)
	...
	for(;;)
		...
		case TIMER:
			...
			if(dt < 0) {
				i = 1;
				dt =
				    16 * (points + nrand(10000) - 5000) / 10000;
				if(dt >= 32) {
					i += (dt - 32) / 16;
					dt = 32;
				}
...
redraw(int new)
{
	...
	bb = allocimage(display, Rect(0, 0, N * pcsz, N * pcsz), screen->chan,
	                0, 0);
	bbmask =
	    allocimage(display, Rect(0, 0, N * pcsz, N * pcsz), GREY1, 0, 0);
	bb2 = allocimage(display, Rect(0, 0, N * pcsz, N * pcsz + DY),
	                 screen->chan, 0, 0);
...
threadmain(int argc, char* argv[])
{
	...
	for(i = 0; i < NCOL; i++) {
		tx[i] = allocimage(display, Rect(0, 0, 16, 16), screen->chan, 1,
		                   txpix[i]);
		if(tx[i] == 0)
	...
	whitemask = allocimage(display, Rect(0, 0, 1, 1), CMAP8, 1,
	                       setalpha(DWhite, 0x7F));
	if(whitemask == 0)
