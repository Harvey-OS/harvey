...
drawhand(int length, int width, double theta)
{
	...
	Point vhand[4];
	vhand[0] = add(screen->r.min,
	               add(catoffs, Pt(CATWID / 2 + round(length * s),
	                               CATHGT / 2 - round(length * c))));
	vhand[1] =
	    add(screen->r.min, add(catoffs, Pt(CATWID / 2 - round(ws + wc),
	                                       CATHGT / 2 + round(wc - ws))));
	vhand[2] =
	    add(screen->r.min, add(catoffs, Pt(CATWID / 2 - round(ws - wc),
	                                       CATHGT / 2 + round(wc + ws))));
	vhand[3] = vhand[0];
...
{
	Image* bp;
	double theta = .4 * sin(t + 3. * PIO2) -
	               .08; /* an assymetric tail leans to one side */
	...
	for(i = 0; i != NTP; i++)
		rtp[i] = add(
		    Pt(tp[i].x * c + tp[i].y * s, -tp[i].x * s + tp[i].y * c),
		    toffs);
...
draweye(double t)
{
	...
	double u;
	double angle =
	    0.7 * sin(t + 3 * PIO2) + PI / 2.0; /* direction eyes point */
	Point pts[100];
	int i, j;
	struct {
		double x, y, z;
	...
	if(eyes == 0) {
		...
		//		wrbitmap(eyes, eyes->r.min.y, eyes->r.max.y,
		//eyes_bits);
	}
	...
	if(crosseyed) {
		...
		for(i = 0, u = -PI / 2.0; u < PI / 2.0; i++, u += 0.25) {
			...
			pt.z = 2. + cos(u) * sin(angle + PI / 7.0);
			pts[i].x =
			    (pt.z == 0.0 ? pt.x : pt.x / pt.z) * 23.0 + 12.0;
			pts[i].y =
			    (pt.z == 0.0 ? pt.y : pt.y / pt.z) * 23.0 + 11.0;
		}
		for(u = PI / 2.0; u > -PI / 2.0; i++, u -= 0.25) {
			pt.x = cos(u) * cos(angle - PI / 7.0);
			pt.y = sin(u);
...
drawclock(void)
{
	...
	if(xredraw || tm.min != otm.min || tm.hour != otm.hour) {
		if(xredraw) {
			...
			// bitblt(&screen, screen.r.min, &screen, screen.r,
			// Zero);
		...
		drawhand(HOURLEN, HANDWID,
		         2. * PI * (tm.hour + tm.min / 60.) / 12.);
		xredraw = 0;
	}
...
#ifdef NOTDEF
void
drawpoly(Bitmap* dst, Point p[], int np, int v, Fcode f)
{
	int i;
	Point q = p[np - 1];
	for(i = 0; i != np; i++) {
...
int
myfillpoly(Bitmap* b, Point vert[], int nvert, Windrule w, int v, Fcode f)
{
	...
	for(p = evert - 1, q = vert, ep = edges; q != evert; p = q, q++, ep++) {
		...
		if(p10.x >= 0) {
			...
			ep->dx =
			    -(p10.x /
			      p10.y); /* this nonsense rounds toward zero */
			ep->dx1 = ep->dx - 1;
		}
		ep->x = 0;
		ep->num = p10.x % p10.y;
		...
		for(ep = *yp; ep; ep = nextep) {
			...
			if(nwind & w) { /* inside */
				...
				if(right >= b->r.max.x)
					right = b->r.max.x;
				if(right > left)
					segment(b, Pt(left, y), Pt(right, y), v,
					        f);
			}
			wind = nwind;
			nextep = ep->next;
