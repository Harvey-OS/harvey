...
{
	return sqrt(
	    (double)(sqr(q->pos.x - p->pos.x) + sqr(q->pos.y - p->pos.y)));
}
...
undraw(Dot* d)
{
	...
	/*
	        if (track_width > 0)
	        {
	                ...
	                {
	                        bitblt(&screen, d->track[d->next_clear], track,
	   track->r, D ^ ~S);
	                        d->next_clear = (d->next_clear + 1) %
	   track_length;
	                        d->ntracks--;
	                }

	                d->track[d->next_write] = Pt(d->pos.x + d->width / 2 -
	   track_width / 2, d->pos.y + d->height / 2 - track_width / 2);
	                bitblt(&screen, d->track[d->next_write], track,
	   track->r, D ^ ~S);
...
reflect1_lr(Image* b)
{
	...
	for(x = 0; x < PDUP * NPJW; x++)
		draw(im, Rect(x, 0, x, PDUP * NPJW), b, nil,
		     Pt(b->r.max.x - x, 0));
	draw(b, b->r, im, nil, ZP);
}
...
reflect1_ud(Image* b)
{
	...
	for(y = 0; y < PDUP * NPJW; y++)
		draw(im, Rect(0, y, PDUP * NPJW, y), b, nil,
		     Pt(0, b->r.max.y - y));
	draw(b, b->r, im, nil, ZP);
}
...
upd(Dot* d)
{
	...
	if(iflag) {
		 ...
		 * Going off-screen?
		 */
		if(d->pos.x + d->width < 0 || d->pos.x >= Dx(screen->r) ||
		   d->pos.y + d->height < 0 || d->pos.y >= Dy(screen->r))
			restart(d);
...
setup(Dot* d, char* who, uint8_t* face, int n_els)
{
	...
	if(im == nil) {
		im = eallocimage(display, Rect(0, 0, NPJW, NPJW), CMAP8, 0,
		                 DNofill);
		imask = eallocimage(display, Rect(0, 0, NPJW, NPJW), CMAP8, 0,
		                    DNofill);
	}
	...
	for(i = 0; i < nels(d->faces); i++) {
		d->faces[i] =
		    eallocimage(display, Rect(0, 0, PDUP * NPJW, PDUP * NPJW),
...
main(int argc, char* argv[])
{
	...
	default:
		fprint(2, "Usage: %s [-i] [-k k_floor] [-n track_length] [-w "
		          "track_width] [-x gravityx] [-y gravityy]\n",
		       argv0);
		exits("usage");
	}
	ARGEND
	...
	initdraw(0, 0, 0);
	im = eallocimage(display, Rect(0, 0, PDUP * NPJW, PDUP * NPJW), CMAP8,
	                 0, DNofill);
