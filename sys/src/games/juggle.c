...
enum { NSTEP = 10, /* number of steps between throws */
...
void throw(int t, int hgt)
{
	...
	b = n = 0;
	for(i = 0; i != nball; i++)
		if(ball[i].hand == hand && ball[i].time <= t) {
			n++;
			b = i;
		}
	if(hgt == 0) {
...
move(int t)
{
	...
	for(i = 0; i != NSTEP; i++) {
		...
		for(j = 0; j != nball; j++)
			fillellipse(image, bpos(j, i, t), RBALL, RBALL,
			            disk[j % ndisk], ZP);
