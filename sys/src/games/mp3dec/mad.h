...
#define MAD_VERSION                                                            \
	MAD_VERSION_STRING(MAD_VERSION_MAJOR) "." MAD_VERSION_STRING(          \
	    MAD_VERSION_MINOR) "." MAD_VERSION_STRING(MAD_VERSION_PATCH)       \
	    MAD_VERSION_EXTRA
...
#define mad_f_mul(x, y)                                                        \
	((mad_fixed_t)(                                                        \
	    (((mad_fixed64_t)(x) * (y)) + (1L << (MAD_F_SCALEBITS - 1))) >>    \
	    MAD_F_SCALEBITS))
#else
#define mad_f_mul(x, y)                                                        \
...
static __forceinline mad_fixed_t
mad_f_mul_inline(mad_fixed_t x, mad_fixed_t y)
{
...
#define mad_f_scale64(hi, lo)                                                  \
	({                                                                     \
		...
		asm("addl %4,%2\n\t"                                           \
		    "adcl %5,%3"                                               \
		    : "=rm"(__lo_), "=rm"(__hi_)                               \
		    : "0"(lo), "1"(hi), "ir"(1L << (MAD_F_SCALEBITS - 1)),     \
		      "ir"(0)                                                  \
		    : "cc");                                                   \
		asm("shrdl %3,%2,%1"                                           \
		    : "=rm"(__result)                                          \
...
enum { MAD_OPTION_IGNORECRC = 0x0001,     /* ignore CRC errors */
...
enum { MAD_FLAG_NPRIVATE_III = 0x0007, /* number of Layer III private bits */
...
enum { MAD_PRIVATE_HEADER = 0x0100, /* header private bit */
...
void mad_decoder_init(
    struct mad_decoder*, void*, enum mad_flow (*)(void*, struct mad_stream*),
