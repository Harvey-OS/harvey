	      ...
	      unsigned char const *sfbwidth, signed int exponents[39])
{
	...
	if(channel->block_type == 2) {
		...
		if(channel->flags & mixed_block_flag) {
			...
			while(l < 36) {
				exponents[sfbi] = gain -
						  (signed int)((channel->scalefac[sfbi] + (pretab[sfbi] & premask)) << scalefac_multiplier);
 ...
 */
static enum mad_error
III_stereo(mad_fixed_t xr[2][576],
 ...
 */
static void
III_imdct_l(mad_fixed_t const X[18], mad_fixed_t z[36],
 ...
 */
static inline void
III_overlap_z(mad_fixed_t overlap[18],
	      mad_fixed_t sample[18][32], unsigned int sb)
{
	unsigned int i;
 ...
 */
static enum mad_error
III_decode(struct mad_bitptr *ptr, struct mad_frame *frame,
