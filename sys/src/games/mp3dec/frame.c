 ...
 */
void
mad_header_init(struct mad_header* header)
{
	header->layer = 0;
	header->mode = 0;
 ...
 */
void
mad_frame_init(struct mad_frame* frame)
{
 ...
 */
static int
decode_header(struct mad_header* header, struct mad_stream* stream)
{
	unsigned int index;
	...
	if(header->flags & MAD_FLAG_LSF_EXT)
		header->bitrate =
		    bitrate_table[3 + (header->layer >> 1)][index];
	else
 ...
 */
static int
free_bitrate(struct mad_stream* stream, struct mad_header const* header)
{
 ...
 */
int
mad_header_decode(struct mad_header* header, struct mad_stream* stream)
{
...
sync:
	...
	if(header->layer == MAD_LAYER_I)
		N = ((12 * header->bitrate / header->samplerate) + pad_slot) *
		    4;
	else {
		...
		N = (slots_per_frame * header->bitrate / header->samplerate) +
		    pad_slot;
	}
 ...
 */
int
mad_frame_decode(struct mad_frame* frame, struct mad_stream* stream)
{
 ...
 */
void
mad_frame_mute(struct mad_frame* frame)
{
	unsigned int s, sb;
