 ...
 */
void
mad_bit_init(struct mad_bitptr* bitptr, unsigned char const* byte)
{
	bitptr->byte = byte;
	bitptr->cache = 0;
 ...
 */
unsigned int
mad_bit_length(struct mad_bitptr const* begin, struct mad_bitptr const* end)
{
	return begin->left + CHAR_BIT * (end->byte - (begin->byte + 1)) +
	       (CHAR_BIT - end->left);
}
 ...
 */
unsigned char const*
mad_bit_nextbyte(struct mad_bitptr const* bitptr)
{
 ...
 */
void
mad_bit_skip(struct mad_bitptr* bitptr, unsigned int len)
{
 ...
 */
unsigned long
mad_bit_read(struct mad_bitptr* bitptr, unsigned int len)
{
 ...
 */
unsigned short
mad_bit_crc(struct mad_bitptr bitptr, unsigned int len, unsigned short init)
{
	...
	for(crc = init; len >= 32; len -= 32) {
		...
		crc =
		    (crc << 8) ^ crc_table[((crc >> 8) ^ (data >> 24)) & 0xff];
		crc =
		    (crc << 8) ^ crc_table[((crc >> 8) ^ (data >> 16)) & 0xff];
