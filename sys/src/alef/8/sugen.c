...
fungen(Node *code, Node *args)
{
	...
	if(opt('q') == 0 && nerr == 0) {
		...
		switch(t->type) {
		case TVOID:
			instruction(ANOP, ZeroN, regn(Freg));
			instruction(ANOP, ZeroN, regn(Retireg));
			break;
		case TFLOAT:
			instruction(ANOP, ZeroN, regn(Retireg));
			break;
...
stmnt(Node *n)
{
	...
	case OIF:
		...
		if(r && r->type == OELSE) {
			stmnt(r->left);
			truedone = instruction(AJMP, ZeroN, ZeroN);
	...
	case ODWHILE:
		instruction(AJMP, ZeroN, ZeroN);
		enter = ipc;
		instruction(AJMP, ZeroN, ZeroN);	/* Continue */
		pushjmp(&contstack);
		instruction(AJMP, ZeroN, ZeroN); 	/* Break */
		pushjmp(&brkstack);
		...
		instruction(AJMP, ZeroN, ZeroN); 	/* Loop */
		label(ipc, loop->pc+1);
...
switchcode(Node *n)
{
	...
	instruction(AJMP, ZeroN, ZeroN);		/* Entry */
	enter = ipc;
	instruction(AJMP, ZeroN, ZeroN); 		/* Break */
	pushjmp(&brkstack);
	...
	instruction(AJMP, ZeroN, ZeroN); 		/* Done break */
	label(ipc, brkstack->i->pc);
...
parcode(Node *n)
{
	...
	for(i = 0; i < cnt-1; i++) {
		...
		instruction(ACMPL, &retr, con(i));
		instruction(AJNE, ZeroN, ZeroN);
		loop = ipc;
		regfree(&retr);
		...
		/* ensure enough space for args to ALEF_pexit */
