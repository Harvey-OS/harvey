 ...
 */
int
closept3(Point3 p, Point3 q, double eps)
{
	return 2. * dist3(p, q) < eps * (len3(p) + len3(q));
}
double
dot3(Point3 p, Point3 q)
{
	return p.x * q.x + p.y * q.y + p.z * q.z;
}
Point3
cross3(Point3 p, Point3 q)
{
	Point3 r;
	r.x = p.y * q.z - p.z * q.y;
	r.y = p.z * q.x - p.x * q.z;
...
Point3
lerp3(Point3 p, Point3 q, double alpha)
{
	p.x += (q.x - p.x) * alpha;
	p.y += (q.y - p.y) * alpha;
	p.z += (q.z - p.z) * alpha;
 ...
 */
Point3
reflect3(Point3 p, Point3 p0, Point3 p1)
{
	Point3 a, b;
	a = sub3(p, p0);
	b = sub3(p1, p0);
 ...
 */
Point3
nearseg3(Point3 p0, Point3 p1, Point3 testp)
{
	double num, den;
	Point3 q, r;
	q = sub3(p1, p0);
...
double
pldist3(Point3 p, Point3 p0, Point3 p1)
{
	Point3 d, e;
	double dd, de, dsq;
	d = sub3(p1, p0);
 ...
 */
double
vdiv3(Point3 a, Point3 b)
{
	return (a.x * b.x + a.y * b.y + a.z * b.z) /
	       (b.x * b.x + b.y * b.y + b.z * b.z);
}
Point3
vrem3(Point3 a, Point3 b)
{
	double quo = (a.x * b.x + a.y * b.y + a.z * b.z) /
	             (b.x * b.x + b.y * b.y + b.z * b.z);
	a.x -= b.x * quo;
	a.y -= b.y * quo;
	a.z -= b.z * quo;
 ...
 */
Point3
ppp2f3(Point3 p0, Point3 p1, Point3 p2)
{
	Point3 p01, p02;
	p01 = sub3(p1, p0);
	p02 = sub3(p2, p0);
 ...
 */
Point3
fff2p3(Point3 f0, Point3 f1, Point3 f2)
{
	double det;
	Point3 p;
	...
	p.x = (f0.w * (f2.y * f1.z - f1.y * f2.z) +
	       f1.w * (f0.y * f2.z - f2.y * f0.z) +
	       f2.w * (f1.y * f0.z - f0.y * f1.z)) /
	      det;
	p.y = (f0.w * (f2.z * f1.x - f1.z * f2.x) +
	       f1.w * (f0.z * f2.x - f2.z * f0.x) +
	       f2.w * (f1.z * f0.x - f0.z * f1.x)) /
	      det;
	p.z = (f0.w * (f2.x * f1.y - f1.x * f2.y) +
	       f1.w * (f0.x * f2.y - f2.x * f0.y) +
	       f2.w * (f1.x * f0.y - f0.x * f1.y)) /
	      det;
	p.w = 1.;
	return p;
}
/*
 * pdiv4 does perspective division to convert a projective point to affine
 * coordinates.
 */
Point3
pdiv4(Point3 a)
