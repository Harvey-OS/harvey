...
#define EPS 1.387778780781445675529539585113525e-17 /* 2^-56 */
	 ...
	 * component -- first r, then i, j, or k.  When the trace is greater
	 * than zero,
	 * |r| is greater than 1/2, which is as small as a largest component can
	 * be.
	 * Otherwise, the largest diagonal entry corresponds to the largest of
	 * |i|,
	...
	if(tr >= 0.0) {
		...
		switch(i) {
		case 0:
			s = sqrt((mat[0][0] - (mat[1][1] + mat[2][2])) +
			         mat[3][3]);
			qu.i = s * 0.5;
			s = 0.5 / s;
			...
			break;
		case 1:
			s = sqrt((mat[1][1] - (mat[2][2] + mat[0][0])) +
			         mat[3][3]);
			qu.j = s * 0.5;
			s = 0.5 / s;
			...
			break;
		case 2:
			s = sqrt((mat[2][2] - (mat[0][0] + mat[1][1])) +
			         mat[3][3]);
			qu.k = s * 0.5;
			s = 0.5 / s;
...
Quaternion
qadd(Quaternion q, Quaternion r)
{
	q.r += r.r;
	q.i += r.i;
	q.j += r.j;
...
Quaternion
qsub(Quaternion q, Quaternion r)
{
	q.r -= r.r;
	q.i -= r.i;
	q.j -= r.j;
...
Quaternion
qmul(Quaternion q, Quaternion r)
{
	Quaternion s;
...
Quaternion
qdiv(Quaternion q, Quaternion r)
{
...
Quaternion
slerp(Quaternion q, Quaternion r, double a)
{
	double u, v, ang, s;
	double dot = q.r * r.r + q.i * r.i + q.j * r.j + q.k * r.k;
	ang =
	    dot < -1 ? PI : dot > 1 ? 0 : acos(dot); /* acos gives NaN for dot
 ...
 */
Quaternion
qmid(Quaternion q, Quaternion r)
{
	double l;
	q = qadd(q, r);
	l = qlen(q);
