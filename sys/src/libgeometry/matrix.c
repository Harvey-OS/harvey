...
adjoint(Matrix m, Matrix madj)
{
	...
	madj[0][0] = m11 * (m22 * m33 - m23 * m32) +
	             m21 * (m13 * m32 - m12 * m33) +
	             m31 * (m12 * m23 - m13 * m22);
	madj[0][1] = m01 * (m23 * m32 - m22 * m33) +
	             m21 * (m02 * m33 - m03 * m32) +
	             m31 * (m03 * m22 - m02 * m23);
	madj[0][2] = m01 * (m12 * m33 - m13 * m32) +
	             m11 * (m03 * m32 - m02 * m33) +
	             m31 * (m02 * m13 - m03 * m12);
	madj[0][3] = m01 * (m13 * m22 - m12 * m23) +
	             m11 * (m02 * m23 - m03 * m22) +
	             m21 * (m03 * m12 - m02 * m13);
	madj[1][0] = m10 * (m23 * m32 - m22 * m33) +
	             m20 * (m12 * m33 - m13 * m32) +
	             m30 * (m13 * m22 - m12 * m23);
	madj[1][1] = m00 * (m22 * m33 - m23 * m32) +
	             m20 * (m03 * m32 - m02 * m33) +
	             m30 * (m02 * m23 - m03 * m22);
	madj[1][2] = m00 * (m13 * m32 - m12 * m33) +
	             m10 * (m02 * m33 - m03 * m32) +
	             m30 * (m03 * m12 - m02 * m13);
	madj[1][3] = m00 * (m12 * m23 - m13 * m22) +
	             m10 * (m03 * m22 - m02 * m23) +
	             m20 * (m02 * m13 - m03 * m12);
	madj[2][0] = m10 * (m21 * m33 - m23 * m31) +
	             m20 * (m13 * m31 - m11 * m33) +
	             m30 * (m11 * m23 - m13 * m21);
	madj[2][1] = m00 * (m23 * m31 - m21 * m33) +
	             m20 * (m01 * m33 - m03 * m31) +
	             m30 * (m03 * m21 - m01 * m23);
	madj[2][2] = m00 * (m11 * m33 - m13 * m31) +
	             m10 * (m03 * m31 - m01 * m33) +
	             m30 * (m01 * m13 - m03 * m11);
	madj[2][3] = m00 * (m13 * m21 - m11 * m23) +
	             m10 * (m01 * m23 - m03 * m21) +
	             m20 * (m03 * m11 - m01 * m13);
	madj[3][0] = m10 * (m22 * m31 - m21 * m32) +
	             m20 * (m11 * m32 - m12 * m31) +
	             m30 * (m12 * m21 - m11 * m22);
	madj[3][1] = m00 * (m21 * m32 - m22 * m31) +
	             m20 * (m02 * m31 - m01 * m32) +
	             m30 * (m01 * m22 - m02 * m21);
	madj[3][2] = m00 * (m12 * m31 - m11 * m32) +
	             m10 * (m01 * m32 - m02 * m31) +
	             m30 * (m02 * m11 - m01 * m12);
	madj[3][3] = m00 * (m11 * m22 - m12 * m21) +
	             m10 * (m02 * m21 - m01 * m22) +
	             m20 * (m01 * m12 - m02 * m11);
}
/*
 ...
 */
double
invertmat(Matrix m, Matrix minv)
{
	double d, dinv;
	int i, j;
	d = determinant(m);
