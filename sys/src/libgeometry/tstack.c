...
void
rot(Space* t, double theta, int axis)
{
...
void
scale(Space* t, double x, double y, double z)
{
	Matrix m, inv;
	ident(m);
	m[0][0] = x;
...
void
xform(Space* t, Matrix m)
{
	Matrix inv;
	if(invertmat(m, inv) == 0)
		return;
	ixform(t, m, inv);
}
void
ixform(Space* t, Matrix m, Matrix inv)
{
	matmul(t->t, m);
	matmulr(t->tinv, inv);
}
 ...
 */
void
look(Space* t, Point3 e, Point3 l, Point3 u)
{
	Matrix m, inv;
	Point3 r;
	l = unit3(sub3(l, e));
 ...
 */
int
persp(Space* t, double fov, double n, double f)
{
	Matrix m;
	double z;
	if(n <= 0 || f <= n || fov <= 0 ||
	   180 <= fov) /* really need f!=n && sin(v)!=0 */
		return -1;
 ...
 * The whole window is transformed to fit centered inside the viewport with
 * equal
 * slop on either top and bottom or left and right, depending on the viewport's
 * aspect ratio.
 * The window is viewed down the y axis, with x to the left and z up.  The
 * viewport
 * has x increasing to the right and y increasing down.  The window's y
 * coordinates
 * are mapped, unchanged, into the viewport's z coordinates.
 */
void
viewport(Space* t, Rectangle r, double aspect)
{
	Matrix m;
