...
enum { Dptr,  /* pointer to database file */
...
hfopen(Ndb* db, char* attr)
{
	...
	if((d = dirfstat(Bfildes(&db->b))) == nil ||
	   db->qid.path != d->qid.path || db->qid.vers != d->qid.vers) {
		if(ndbreopen(db) < 0) {
			free(d);
			return 0;
		}
...
ndbsearch(Ndb* db, Ndbs* s, char* attr, char* val)
{
	...
	if(s->hf) {
		...
		print("Missing or out of date hash file %s.%s.\n", db->file,
		      attr);
		syslog(0, "ndb", "Missing or out of date hash file %s.%s.",
		       db->file, attr);
