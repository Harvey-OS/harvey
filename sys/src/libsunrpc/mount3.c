...
int
nfsMount3TNullUnpack(uint8_t* a, uint8_t* ea, uint8_t** pa, NfsMount3TNull* x)
{
	USED(ea);
	USED(x);
	*pa = a;
...
int
nfsMount3RNullUnpack(uint8_t* a, uint8_t* ea, uint8_t** pa, NfsMount3RNull* x)
{
	USED(ea);
	USED(x);
	*pa = a;
...
nfsMount3RMntPack(uint8_t* a, uint8_t* ea, uint8_t** pa, NfsMount3RMnt* x)
{
	...
	switch(x->status) {
	case 0:
		if(sunVarOpaquePack(a, ea, &a, &x->handle, &x->len,
		                    NfsMount3MaxHandleSize) < 0)
			goto Err;
...
nfsMount1RMntPack(uint8_t* a, uint8_t* ea, uint8_t** pa, NfsMount3RMnt* x)
{
	...
	case 0:
		if(x->len != NfsMount1HandleSize)
			goto Err;
		if(sunFixedOpaquePack(a, ea, &a, x->handle,
		                      NfsMount1HandleSize) < 0)
			goto Err;
		if(x->nauth != 0)
			goto Err;
		break;
...
nfsMount1RMntUnpack(uint8_t* a, uint8_t* ea, uint8_t** pa, NfsMount3RMnt* x)
{
	...
	case 0:
		...
		x->nauth = 0;
		x->auth = 0;
		if(sunFixedOpaqueUnpack(a, ea, &a, x->handle,
		                        NfsMount1HandleSize) < 0)
			goto Err;
		if(x->nauth != 0)
			goto Err;
...
nfsMount3RMntUnpack(uint8_t* a, uint8_t* ea, uint8_t** pa, NfsMount3RMnt* x)
{
	...
	switch(x->status) {
	case 0:
		if(sunVarOpaqueUnpack(a, ea, &a, &x->handle, &x->len,
		                      NfsMount3MaxHandleSize) < 0)
			goto Err;
...
int
nfsMount3TDumpUnpack(uint8_t* a, uint8_t* ea, uint8_t** pa, NfsMount3TDump* x)
{
	USED(ea);
	USED(x);
	*pa = a;
...
int
nfsMount3EntryUnpack(uint8_t* a, uint8_t* ea, uint8_t** pa, NfsMount3Entry* x)
{
	u1int one;
...
int
nfsMount3RDumpUnpack(uint8_t* a, uint8_t* ea, uint8_t** pa, NfsMount3RDump* x)
{
	int i;
	uint8_t* oa;
	u1int u1;
	...
	for(i = 0;; i++) {
		...
		if(sunUint32Unpack(a, ea, &a, &u32) < 0 ||
		   u32 > NfsMount3MaxNameSize || (a += u32) >= ea ||
		   sunUint32Unpack(a, ea, &a, &u32) < 0 ||
		   u32 > NfsMount3MaxPathSize || (a += u32) >= ea)
			goto Err;
	}
	x->count = (a - 4) - oa;
	x->data = oa;
...
int
nfsMount3TUmntUnpack(uint8_t* a, uint8_t* ea, uint8_t** pa, NfsMount3TUmnt* x)
{
	if(sunStringUnpack(a, ea, &a, &x->path, 1024) < 0)
		goto Err;
	*pa = a;
...
int
nfsMount3RUmntUnpack(uint8_t* a, uint8_t* ea, uint8_t** pa, NfsMount3RUmnt* x)
{
	USED(ea);
	USED(x);
	*pa = a;
...
int
nfsMount3TExportPack(uint8_t* a, uint8_t* ea, uint8_t** pa, NfsMount3TExport* x)
{
	USED(ea);
	USED(x);
	*pa = a;
...
int
nfsMount3RExportPack(uint8_t* a, uint8_t* ea, uint8_t** pa, NfsMount3RExport* x)
{
	u1int zero;
                      ...
                      char*** pgp, NfsMount3Export* x)
{
	...
	for(;;) {
		...
		if(u1 == 0)
			break;
		if(sunStringUnpack(a, ea, &a, &gp[ng++], NfsMount3MaxNameSize) <
		   0)
			goto Err;
	}
	x->ng = ng;
...
int
nfsMount3ExportPack(uint8_t* a, uint8_t* ea, uint8_t** pa, NfsMount3Export* x)
{
	int i;
	u1int u1;
...
static SunProc proc3[] = {
    (P)nfsMount3TNullPack,      (P)nfsMount3TNullUnpack,
    (S)nfsMount3TNullSize,      (F)nfsMount3TNullPrint,
    sizeof(NfsMount3TNull),     (P)nfsMount3RNullPack,
    (P)nfsMount3RNullUnpack,    (S)nfsMount3RNullSize,
    (F)nfsMount3RNullPrint,     sizeof(NfsMount3RNull),
    (P)nfsMount3TMntPack,       (P)nfsMount3TMntUnpack,
    (S)nfsMount3TMntSize,       (F)nfsMount3TMntPrint,
    sizeof(NfsMount3TMnt),      (P)nfsMount3RMntPack,
    (P)nfsMount3RMntUnpack,     (S)nfsMount3RMntSize,
    (F)nfsMount3RMntPrint,      sizeof(NfsMount3RMnt),
    (P)nfsMount3TDumpPack,      (P)nfsMount3TDumpUnpack,
    (S)nfsMount3TDumpSize,      (F)nfsMount3TDumpPrint,
    sizeof(NfsMount3TDump),     (P)nfsMount3RDumpPack,
    (P)nfsMount3RDumpUnpack,    (S)nfsMount3RDumpSize,
    (F)nfsMount3RDumpPrint,     sizeof(NfsMount3RDump),
    (P)nfsMount3TUmntPack,      (P)nfsMount3TUmntUnpack,
    (S)nfsMount3TUmntSize,      (F)nfsMount3TUmntPrint,
    sizeof(NfsMount3TUmnt),     (P)nfsMount3RUmntPack,
    (P)nfsMount3RUmntUnpack,    (S)nfsMount3RUmntSize,
    (F)nfsMount3RUmntPrint,     sizeof(NfsMount3RUmnt),
    (P)nfsMount3TUmntallPack,   (P)nfsMount3TUmntallUnpack,
    (S)nfsMount3TUmntallSize,   (F)nfsMount3TUmntallPrint,
    sizeof(NfsMount3TUmntall),  (P)nfsMount3RUmntallPack,
    (P)nfsMount3RUmntallUnpack, (S)nfsMount3RUmntallSize,
    (F)nfsMount3RUmntallPrint,  sizeof(NfsMount3RUmntall),
    (P)nfsMount3TExportPack,    (P)nfsMount3TExportUnpack,
    (S)nfsMount3TExportSize,    (F)nfsMount3TExportPrint,
    sizeof(NfsMount3TExport),   (P)nfsMount3RExportPack,
    (P)nfsMount3RExportUnpack,  (S)nfsMount3RExportSize,
    (F)nfsMount3RExportPrint,   sizeof(NfsMount3RExport),
};
...
static SunProc proc1[] = {
    ...
    (P)nfsMount3TMntPack,       (P)nfsMount3TMntUnpack,
    (S)nfsMount3TMntSize,       (F)nfsMount3TMntPrint,
    sizeof(NfsMount3TMnt),      (P)nfsMount1RMntPack,
    (P)nfsMount1RMntUnpack,     (S)nfsMount1RMntSize,
    (F)nfsMount3RMntPrint,      sizeof(NfsMount3RMnt),
    (P)nfsMount3TDumpPack,      (P)nfsMount3TDumpUnpack,
    (S)nfsMount3TDumpSize,      (F)nfsMount3TDumpPrint,
    sizeof(NfsMount3TDump),     (P)nfsMount3RDumpPack,
...
SunProg nfsMount3Prog = {
    NfsMount3Program, NfsMount3Version, proc3, nelem(proc3),
};

SunProg nfsMount1Prog = {
    NfsMount1Program, NfsMount1Version, proc1, nelem(proc1),
};
