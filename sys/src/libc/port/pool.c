...
#define PSHORT(p, x)                                                           \
	(((uint8_t*)(p))[0] = ((x) >> 8) & 0xFF, ((uint8_t*)(p))[1] = (x)&0xFF)
 ...
 * slow.
 *
 * Verbosity induces the dumping of the pool via p->print at each lock
 *operation.
...
#define antagonism                                                             \
	if(!(p->flags & POOL_ANTAGONISM)) {                                    \
	} else
#define paranoia                                                               \
	if(!(p->flags & POOL_PARANOIA)) {                                      \
	} else
#define verbosity                                                              \
...
#define DPRINT                                                                 \
	if(!(p->flags & POOL_DEBUGGING)) {                                     \
	} else                                                                 \
	p->print
#define LOG                                                                    \
	if(!(p->flags & POOL_LOGGING)) {                                       \
	} else                                                                 \
	p->print
...
treedelete(Free* tree, Free* node)
{
	...
	if(node->left == nil)
		...
		*loc = node->left;
		...
		for(lsucc = &node->right; (*lsucc)->left;
		    lsucc = &(*lsucc)->left)
			;
		succ = *lsucc;
		*lsucc = succ->right;
...
pooladd(Pool* p, Alloc* anode)
{
	...
	antagonism
	{
		memmark(_B2D(anode), 0xF7,
		        anode->size - sizeof(Bhdr) - sizeof(Btail));
	}
...
pooldel(Pool* p, Free* node)
{
	...
	antagonism
	{
		memmark(_B2D(node), 0xF9,
		        node->size - sizeof(Bhdr) - sizeof(Btail));
	}
...
trim(Pool* p, Alloc* b, uint32_t dsize)
{
	...
	if(b->size - dsize >= 0x10000 ||
	   (extra >= bsize >> 2 && extra >= MINBLOCKSIZE &&
	    extra >= p->minblock)) {
...
freefromfront(Pool* p, Alloc* b, uint32_t skip)
{
	...
	if(skip >= 0x1000 || (skip >= b->size >> 2 && skip >= MINBLOCKSIZE &&
	                      skip >= p->minblock)) {
...
dumpblock(Pool* p, Bhdr* b)
{
	...
	if(b->magic == ALLOC_MAGIC) {
		...
		p->print(p, "%.2ux %.2ux %.2ux %.2ux  %.2ux %.2ux %.2ux %.2ux",
		         cp[-8], cp[-7], cp[-6], cp[-5], cp[-4], cp[-3], cp[-2],
		         cp[-1]);
		p->print(
		    p, " | %.2ux %.2ux %.2ux %.2ux  %.2ux %.2ux %.2ux %.2ux\n",
...
blockcheck(Pool* p, Bhdr* b)
{
	...
	case ALLOC_MAGIC:
		...
		if(t->magic0 != TAIL_MAGIC0) {
			...
			if((p->flags & POOL_TOLERANCE) && bq == eq &&
			   t->magic0 == 0)
...
/* poolallocl: attempt to allocate block to hold dsize user bytes; assumes lock
 * held */
static void*
                ...
                uint32_t span)
{
	...
	if(span && (uintptr)c / span != (uintptr)(c + dsize - 1) / span) {
		...
		if((uintptr)c / span != (uintptr)(c + dsize - 1) / span) {
			poolfreel(p, v);
			werrstr("cannot satisfy dsize %lud span %lud with "
			        "align %lud+%ld",
			        dsize, span, align, offset);
			return nil;
		}
	}
	skip = c - (char*)v;
...
poolmsize(Pool* p, void* v)
{
	...
	if(v == nil) /* consistency with other braindead ANSI-ness */
		...
		b = D2B(p, v);
		dsize = (b->size & ~(p->quantum - 1)) - sizeof(Bhdr) -
		        sizeof(Btail);
