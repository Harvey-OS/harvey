...
static char* cmds[] = {[EAlign] = "align", [EBorder] = "border",
                       [EBordercolor] = "bordercolor", [EData] = "data",
                       [EFocus] = "focus", [EFont] = "font",
                       [EFormat] = "format", [EHide] = "hide",
...
entryshow(Entry* e)
{
	...
	if(e->border > 0) {
		border(e->screen, r, e->border, e->bordercolor->image,
		       e->bordercolor->image->r.min);
		dr = insetrect(r, e->border);
	} else
		dr = r;
	p = entrypoint(e, 0);
	_string(e->screen, p, e->textcolor->image, ZP, e->font->font, nil,
	        e->text, e->ntext, dr, nil, ZP, SoverD);
	if(e->hasfocus) {
...
entrykey(Entry* e, Rune r)
{
	...
	default:
		e->text =
		    ctlrealloc(e->text, (e->ntext + 1 + 1) * sizeof(Rune));
	...
	case 0x16: /* control V: paste (append snarf buffer) */
		...
		if(s != nil) {
			n = runestrlen(s);
			e->text = ctlrealloc(e->text,
			                     (e->ntext + n + 1) * sizeof(Rune));
