...
static char* cmds[] = {[EAdd] = "add", [EAlign] = "align", [EBorder] = "border",
                       [EBordercolor] = "bordercolor", [EFocus] = "focus",
                       [EFont] = "font", [EFormat] = "format", [EHide] = "hide",
                       [EImage] = "image", [ERect] = "rect",
                       [EReveal] = "reveal", [ESelectcolor] = "selectcolor",
                       [ESelecttextcolor] = "selecttextcolor", [EShow] = "show",
...
menushow(Menu0* m)
{
	...
	if(m->border > 0)
		border(m->window, m->rect, m->border, m->bordercolor->image,
		       ZP);
	/* text goes here */
	dx = 0;
	for(i = 0; i < m->nline; i++) {
	...
	for(i = 0; i < m->nline; i++) {
		...
		q = _ctlalignpoint(r, stringwidth(f, m->line[i]), f->height,
		                   m->align % 3);
		_string(m->window, q, c, ZP, f, m->line[i], nil,
		        strlen(m->line[i]), clipr, nil, ZP, SoverD);
		p.y += f->height;
	}
	//	if(m->pressed)
	//		draw(m->screen, m->rect, m->lighm->image, m->mask->image,
	//m->mask->image->r.min);
...
menuctl(Control* c, CParse* cp)
{
	...
	case EWindow:
		...
		moveto(m->controlset->mousectl,
		       Pt(m->rect.min.x + Dx(m->rect) / 2,
		          m->rect.min.y + up * h + h / 2));
		//		_ctlfocus(m, 1);
		break;
	}
}
