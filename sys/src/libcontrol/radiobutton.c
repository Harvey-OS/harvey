...
static char* cmds[] = {[EAdd] = "add",       [EButton] = "button",
                       [EFocus] = "focus",   [EFormat] = "format",
                       [EHide] = "hide",     [ERect] = "rect",
                       [EReveal] = "reveal", [EShow] = "show",
...
radiomouse(Control* c, Mouse* m)
{
	...
	for(i = 0; i < r->nbuttons; i++)
		if(ptinrect(m->xy, r->buttons[i]->rect) &&
		   r->buttons[i]->mouse) {
...
radioctl(Control* c, CParse* cp)
{
	...
	case EAdd:
		...
		r->buttons = ctlrealloc(r->buttons,
		                        (r->nbuttons + 1) * sizeof(Control*));
	...
	case EButton:
		if(cp->sender == nil)
			ctlerror("%q: senderless buttonevent: %q", r->name,
			         cp->str);
	...
	case ESize:
		...
		rect.min.x = cp->iargs[1];
		rect.min.y = cp->iargs[2];
		if(rect.min.x <= 0 || rect.min.y <= 0 || rect.max.x <= 0 ||
		   rect.max.y <= 0 || rect.max.x < rect.min.x ||
