...
/* use button 3 for a proper function to the application, that is not for
 * plumber
...
static char* cmds[] = {[EAlign] = "align", [EBorder] = "border",
                       ...
                       [EFont] = "font", [EFormat] = "format", [EHide] = "hide",
                       [EImage] = "image", [ELight] = "light", [EMask] = "mask",
                       [EPressedtextcolor] = "pressedtextcolor",
                       [ERect] = "rect", [EReveal] = "reveal", [EShow] = "show",
                       [ESize] = "size", [EText] = "text",
                       [ETextcolor] = "textcolor", [EEnable] = "enable",
                       [EDisable] = "disable", [EToggle] = "toggle",
                       [EGettext] = "gettext", [EValue] = "value", nil};
...
textbutton3mouse(Control* c, Mouse* m)
{
	...
	if(t->left == 1) {
		if((m->buttons & 1) == 1 && (t->lastbut & 1) == 0) {
			...
			if(t->gettextflg == 0)
				chanprint(t->event, t->format, t->name,
				          t->pressed, m->xy.x, m->xy.y);
			else
				chanprint(t->event, "%q: value %q", t->name,
				          t->line[0]);
			t->pressed ^= 1;
			textbutton3show(t);
			t->lastbut = m->buttons & 1;
...
textbutton3show(Textbutton3* t)
{
	...
	if(t->pressed || t->toggle)
		draw(t->screen, t->rect, t->light->image, t->mask->image,
		     t->mask->image->r.min);
	if(t->border > 0)
		border(t->screen, t->rect, t->border, t->bordercolor->image,
		       ZP);
	/* text goes here */
	dx = 0;
	for(i = 0; i < t->nline; i++) {
	...
	for(i = 0; i < t->nline; i++) {
		...
		q = _ctlalignpoint(r, stringwidth(f, t->line[i]), f->height,
		                   t->align % 3);
		_string(t->screen, q, im, ZP, f, t->line[i], nil,
		        strlen(t->line[i]), clipr, nil, ZP, SoverD);
		p.y += f->height;
	}
...
createtextbutton3(Controlset* cs, char* name)
{
	...
	t = (Textbutton3*)_createctl(cs, "textbutton3", sizeof(Textbutton3),
	                             name);
