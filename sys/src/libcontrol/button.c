...
static char* cmds[] = {[EAlign] = "align", [EBorder] = "border",
                       ...
                       [EPale] = "pale", [ERect] = "rect", [EReveal] = "reveal",
                       [EShow] = "show", [ESize] = "size", [EValue] = "value",
                       nil};
...
buttonshow(Button* b)
{
	...
	if(b->off)
		draw(b->screen, r, b->pale->image, b->mask->image,
		     b->mask->image->r.min);
	else if(b->pressed)
		draw(b->screen, r, b->light->image, b->mask->image,
		     b->mask->image->r.min);
