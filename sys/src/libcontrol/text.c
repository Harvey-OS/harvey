		...
		if(m->buttons & 7) {
			...
			if(sel >= 0) {
				if(t->selectstyle == Seldown) {
					...
					if(debug)
						fprint(2, "textmouse Seldown event %q: select %d %d\n",
						       t->name, sel, t->selected[sel] ? (m->buttons & 7) : 0);
				} else {
					if(debug)
						fprint(2, "textmouse Selup no event yet %q: select %d %d\n",
						       t->name, sel, t->selected[sel] ? (m->buttons & 7) : 0);
					t->sel = sel;
			...
			if((sel >= 0) && (sel == t->sel)) {
				chanprint(t->event, "%q: select %d %d",
					  t->name, sel, t->but);
				if(debug)
					fprint(2, "textmouse Selup event %q: select %d %d\n",
					       t->name, sel, t->but);
			} else if(sel != t->sel) {
				...
				if(debug)
					fprint(2, "textmouse Selup cancel %q: select %d %d\n",
					       t->name, sel, t->but);
			}
			t->sel = -1;
			t->but = 0;
