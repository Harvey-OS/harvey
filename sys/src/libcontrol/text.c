...
static char* cmds[] = {[EAccumulate] = "accumulate", [EAdd] = "add",
                       [EAlign] = "align", [EBorder] = "border",
                       [EBordercolor] = "bordercolor", [EClear] = "clear",
                       [EDelete] = "delete", [EFocus] = "focus",
                       [EFont] = "font", [EHide] = "hide", [EImage] = "image",
                       [ERect] = "rect", [EReplace] = "replace",
                       [EReveal] = "reveal", [EScroll] = "scroll",
                       [ESelect] = "select", [ESelectcolor] = "selectcolor",
                       [ESelectingcolor] = "selectingcolor",
                       [ESelectmode] = "selectmode",
                       [ESelectstyle] = "selectstyle", [EShow] = "show",
                       [ESize] = "size", [ETextcolor] = "textcolor",
                       [ETopline] = "topline", [EValue] = "value",
                       [EWarp] = "warp", nil};
...
textmouse(Control* c, Mouse* m)
{
	...
	t = (Text*)c;
	if(debug)
		fprint(2, "textmouse %s t->lastbut %d; m->buttons %d\n",
	...
	if((t->selectstyle == Selup) && (m->buttons & 7)) {
		sel = textline(t, m->xy);
		if(t->sel >= 0) {
			//			if (debug) fprint(2, "textmouse
			//Selup %q sel=%d t->sel=%d t->but=%d\n",
			//						t->name, sel,
			//t->sel, t->but);
	...
	if(t->lastbut != (m->buttons & 7)) {
		if(m->buttons & 7) {
			...
			t->offsel = 0;
			if((sel >= 0) && (sel == t->sel)) {
				chanprint(t->event, "%q: select %d %d", t->name,
				          sel, t->but);
				if(debug)
					fprint(2, "textmouse Selup event %q: "
					          "select %d %d\n",
					       t->name, sel, t->but);
			} else if(sel != t->sel) {
				...
				if(debug)
					fprint(2, "textmouse Selup cancel %q: "
					          "select %d %d\n",
					       t->name, sel, t->but);
			}
			t->sel = -1;
...
textshow(Text* t)
{
	...
	if(t->border > 0) {
		border(t->screen, r, t->border, t->bordercolor->image,
		       t->bordercolor->image->r.min);
	...
	for(i = t->topline; i < t->nline && i < t->topline + t->nvis; i++) {
		...
		p = _ctlalignpoint(r, runestringnwidth(f, text, ntext),
		                   f->height, t->align);
		if(t->warp == i) {
			Point p2;
		...
		_string(t->screen, p, t->textcolor->image, ZP, f, nil, text,
		        ntext, tr, nil, ZP, SoverD);
		r.min.y += f->height;
	}
...
textctl(Control* c, CParse* cp)
{
	...
	case EDelete:
		...
		i = cp->iargs[1];
		if(i < 0 || i >= t->nline)
			ctlerror("%q: line number out of range: %s", t->name,
			         cp->str);
		free(t->line[i]);
		memmove(t->line + i, t->line + i + 1,
		        (t->nline - (i + 1)) * sizeof(Rune*));
		memmove(t->selected + i, t->selected + i + 1,
		        t->nline - (i + 1));
		t->nline--;
		textshow(t);
		break;
	...
	case ESelect:
		...
		if(cp->iargs[1] < 0 || cp->iargs[1] >= t->nline)
			ctlerror(
			    "%q: selection index out of range (nline %d): %s",
			    t->name, t->nline, cp->str);
	...
	case EAdd:
		switch(cp->nargs) {
		default:
			ctlerror("%q: wrong argument count in '%s'", t->name,
			         cp->str);
		case 2:
			n = t->nline;
			break;
