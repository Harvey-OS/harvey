...
static void	textctl(Control*, char*);
static void	textshow(Text*);
static void	texttogglei(Text*, int);
static void	textfree(Text*);
...
textthread(void *v)
{
	...
	snprint(buf, sizeof buf, "text-%s-0x%p", t->name, t);
	threadsetname(buf);
	...
	for(;;){
		...
		case AKey:
			...
			break;
		case AMouse:
			if(lastbut != (t->m.buttons&1)){
				if(t->m.buttons & 1){
					sel = texttoggle(t, t->m.xy);
					if(sel >= 0)
						printctl(t->event, "%q: select %d %d",
							t->name, sel, t->selected[sel]);
				}
				lastbut = t->m.buttons & 1;
			}
			break;
		case ACtl:
			_ctlcontrol(t, t->str, textctl);
			free(t->str);
			break;
		case AWire:
			_ctlrewire(t);
...
{
	return _createctl(cs, "text", sizeof(Text), name, textthread, 0);
}
...
textctl(Control *c, char *str)
{
	...
	case ERect:
		...
		t->rect = r;
		t->nvis = (Dy(r)-2*t->border)/font->height;
		break;
	case EReplace:
		_ctlargcount(t, &cp, 3);
		i = cp.iargs[1];
	...
	case EAdd:
		...
		t->selected[n] = (t->selectmode == Selmulti);
		t->nline++;
		if(t->scroll) {
			if (n > t->topline + t->nvis)
				t->topline = n - t->nvis;
			if (n < t->topline)
				t->topline = n;
		}
