...
static char* cmds[] = {[EBorder] = "border", [EBordercolor] = "bordercolor",
...
struct {
    ...
    "Shift", 0,      "Ctrl",  0,       "Alt",  0,       "Caps", 0,
    "Del",   '\177', "Enter", '\n',    "Esc",  '\033',  "<-",   '\b',
    "->",    '\t',   "Scrib", 0x10000, "Menu", 0x10001, nil,    0,
};

static char** keyset[Nstate] = {
    keyregular, keyshift, keycaps, keycapsshift,
};
	...
	for(row = 0; row < Nrow; row++) {
		...
		for(col = 0; *kp; col++, kp += n + 1) {
			...
			if(n > 1)
				...
				f = f1;
			q = _ctlalignpoint(r, stringnwidth(f, kp, n), f->height,
			                   Acenter);
			_string(im, q, k->textcolor->image, ZP, f, kp, nil, n,
			        r, nil, ZP, SoverD);
			x = r.max.x;
			if(kp[n + 1])
