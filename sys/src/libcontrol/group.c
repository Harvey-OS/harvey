...
static char* cmds[] = {
        ...
        [EFocus] = "focus", [EHide] = "hide", [EImage] = "image",
        [ERect] = "rect", [ERemove] = "remove", [EReveal] = "reveal",
        [ESeparation] = "separation", [EShow] = "show", [ESize] = "size",
};
...
groupctl(Control* c, CParse* cp)
{
	...
	case EAdd:
		for(i = 1; i < cp->nargs; i++) {
			...
			if(c == nil)
				ctlerror("%q: no such control: %s", g->name,
				         cp->args[i]);
			_ctladdgroup(g, c);
		}
		if(g->setsize)
	...
	case ERemove:
		...
		for(n = 0; n < g->nkids; n++)
			if(strcmp(cp->args[1], g->kids[n]->name) == 0)
				break;
		if(n == g->nkids)
			ctlerror("%s: remove nonexistent control: %q", g->name,
			         cp->args[1]);
		removegroup(g, n);
		if(g->setsize)
			g->setsize((Control*)g);
	...
	case EReveal:
		...
		if(g->type == Ctlstack) {
			if(cp->nargs == 2) {
				if(cp->iargs[1] < 0 || cp->iargs[1] >= g->nkids)
					ctlerror("%s: control out of range: %q",
					         g->name, cp->str);
		...
		_ctlargcount(g, cp, 1);
		if(debug)
			fprint(2, "reveal %s: border %R/%d\n", g->name, g->rect,
			       g->border);
		border(g->screen, g->rect, g->border, g->bordercolor->image,
		       g->bordercolor->image->r.min);
		r = insetrect(g->rect, g->border);
		if(debug)
			fprint(2, "reveal %s: draw %R\n", g->name, r);
		draw(g->screen, r, g->image->image, nil,
		     g->image->image->r.min);
		for(i = 0; i < g->nkids; i++)
			if(g->kids[i]->ctl)
				_ctlprint(g->kids[i], "reveal");
	...
	case EShow:
		...
		if(debug)
			fprint(2, "show %s: border %R/%d\n", g->name, g->rect,
			       g->border);
		border(g->screen, g->rect, g->border, g->bordercolor->image,
		       g->bordercolor->image->r.min);
		r = insetrect(g->rect, g->border);
		if(debug)
			fprint(2, "show %s: draw %R\n", g->name, r);
		draw(g->screen, r, g->image->image, nil,
		     g->image->image->r.min);
		for(i = 0; i < g->nkids; i++)
			if(g->kids[i]->ctl) {
				if(debug)
					fprint(2, "show %s: kid %s: %q\n",
	...
	case ESeparation:
		if(g->type != Ctlstack) {
			_ctlargcount(g, cp, 2);
			if(cp->iargs[1] < 0)
				ctlerror("%q: illegal value: %c", g->name,
				         cp->str);
...
groupmouse(Control* c, Mouse* m)
{
		...
		if(i >= 0 && g->kids[i]->mouse &&
		   ((((m->buttons == 0) || (g->lastbut == 0)) &&
		    ...
		    (((m->buttons != 0) || (g->lastbut != 0)) &&
		     (g->lastkid == i)))) {
			if(debugm)
				fprint(2, "groupmouse %s mouse kid %s i=%d "
				          "lastkid=%d buttons=%d lastbut=%d "
				          "inrect=%d\n",
				       g->name, g->kids[i]->name, i, g->lastkid,
				       m->buttons, g->lastbut,
				       ptinrect(m->xy, g->kids[i]->rect) ? 1
				                                         : 0);
			...
			if(debugm)
				fprint(2, "groupmouse %s skip kid %s i=%d "
				          "lastkid=%d buttons=%d lastbut=%d "
				          "inrect=%d\n",
				       g->name, g->kids[i]->name, i, g->lastkid,
				       m->buttons, g->lastbut,
	...
	if(g->lastbut == 0 && m->buttons != 0) {
		...
		if(!eqpt(s->m.xy, p)) {
			...
			if(Dx(r) > 5 && Dy(r) > 5) {
				i = allocwindow(wscreen, r, Refnone,
				                0xEEEEEEFF); /* grey */
				freeimage(oi);
				if(i == nil)
					goto Rescue;
...
groupsize(Control* c)
{
	...
	g = (Group*)c;
	assert(g->type == Ctlcolumn || g->type == Ctlrow ||
	       g->type == Ctlstack);
	if(g->mansize)
		return;
	r = Rect(1, 1, 1, 1);
	if(debug)
		fprint(2, "groupsize %q\n", g->name);
	for(i = 0; i < g->nkids; i++) {
		q = g->kids[i];
		if(q->setsize)
			q->setsize(q);
		if(q->size.min.x == 0 || q->size.min.y == 0 ||
		   q->size.max.x == 0 || q->size.max.y == 0)
			ctlerror("%q: bad size %R", q->name, q->size);
		if(debug)
			fprint(2, "groupsize %q: [%d %q]: %R\n", g->name, i,
			       q->name, q->size);
		switch(g->type) {
		case Ctlrow:
			if(i)
			...
			if(r.min.y < q->size.min.y)
				r.min.y = q->size.min.y;
			if(r.max.y < q->size.max.y)
				r.max.y = q->size.max.y;
			break;
		case Ctlcolumn:
			if(r.min.x < q->size.min.x)
				r.min.x = q->size.min.x;
			if(r.max.x < q->size.max.x)
				r.max.x = q->size.max.x;
			if(i)
...
boxboxresize(Group* g, Rectangle r)
{
	...
	if(debug)
		fprint(2,
		       "boxboxresize %q %R (%d×%d) min/max %R separation %d\n",
	...
	rr = rectaddpt(Rect(0, 0, wid, ht),
	               addpt(r.min, Pt(g->separation / 2, g->separation / 2)));
	if(debug)
		fprint(2, "boxboxresize rows %d, cols %d, wid %d, ht %d, wpad "
		          "%d, hpad %d\n",
		       rows, cols, wid, ht, wpad, hpad);
	for(i = 0; i < g->nkids; i++) {
		if(debug)
			fprint(2, "	%d %q: %R (%d×%d)\n", i,
			       g->kids[i]->name, rr, Dx(rr), Dy(rr));
		_ctlprint(
		    g->kids[i], "rect %R",
		    rectaddpt(rr,
		              Pt((wpad + wid + g->separation) * (i / rows),
		                 (hpad + ht + g->separation) * (i % rows))));
	}
	g->nseparators = rows + cols - 2;
	g->separators =
	    realloc(g->separators, g->nseparators * sizeof(Rectangle));
	rr = r;
	rr.max.y = rr.min.y + g->separation + hpad;
	for(i = 1; i < rows; i++) {
		g->separators[i - 1] =
		    rectaddpt(rr, Pt(0, (hpad + ht + g->separation) * i -
		                            g->separation - hpad));
		if(debug)
			fprint(2, "row separation %d [%d]: %R\n", i, i - 1,
			       rectaddpt(rr,
			                 Pt(0, (hpad + ht + g->separation) * i -
			                           g->separation)));
	}
	rr = r;
	rr.max.x = rr.min.x + g->separation + wpad;
	for(i = 1; i < cols; i++) {
		g->separators[i + rows - 2] =
		    rectaddpt(rr, Pt((wpad + wid + g->separation) * i -
		                         g->separation - wpad,
		                     0));
		if(debug)
			fprint(
			    2, "col separation %d [%d]: %R\n", i, i + rows - 2,
			    rectaddpt(rr, Pt((wpad + wid + g->separation) * i -
			                         g->separation,
			                     0)));
	}
...
columnresize(Group* g, Rectangle r)
{
	...
	if(debug)
		fprint(2,
		       "columnresize %q %R (%d×%d) min/max %R separation %d\n",
		       g->name, r, Dx(r), Dy(r), g->size, g->separation);
	if(x < g->size.min.x) {
		werrstr("resize %s: too narrow: need %d, have %d", g->name,
		        g->size.min.x, x);
		r.max.x = r.min.x + g->size.min.x;
	}
	if(y < g->size.min.y) {
		werrstr("resize %s: too short: need %d, have %d", g->name,
		        g->size.min.y, y);
	...
	for(i = 0; i < g->nkids; i++) {
		q = g->kids[i];
		if(debug)
			fprint(2, "[%q]: %d⋯%d\t", q->name, q->size.min.y,
			       q->size.max.y);
		d[i] = q->size.min.y;
		y -= d[i];
...
rowresize(Group* g, Rectangle r)
{
	...
	y = Dy(r);
	if(debug)
		fprint(2, "rowresize %q %R (%d×%d), separation %d\n", g->name,
		       r, Dx(r), Dy(r), g->separation);
	if(x < g->size.min.x) {
		werrstr("resize %s: too narrow: need %d, have %d", g->name,
		        g->size.min.x, x);
		r.max.x = r.min.x + g->size.min.x;
	...
	for(i = 0; i < g->nkids; i++) {
		q = g->kids[i];
		if(debug)
			fprint(2, "[%q]: %d⋯%d\t", q->name, q->size.min.x,
			       q->size.max.x);
		d[i] = q->size.min.x;
		x -= d[i];
	...
	if(x < 0) {
		if(debug)
			fprint(2, "rowresize: x == %d\n", x);
		x = 0;
	}
	if(x >= g->size.max.x - g->size.min.x) {
		if(debug)
			fprint(2, "max: %d > %d - %d", x, g->size.max.x,
			       g->size.min.x);
		// all rects can be maximum width
		for(i = 0; i < g->nkids; i++)
			d[i] += p[i];
		...
		if(debug)
			fprint(2, "divvie up: %d < %d - %d", x, g->size.max.x,
			       g->size.min.x);
		// rects can't be max width, divide up the rest
		j = x;
		for(i = 0; i < g->nkids; i++) {
