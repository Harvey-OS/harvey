...
print("dclmodule: %s\n", h->name);
	...
	module[nmodule+1].pin = module[nmodule].pin;
	nmodule++;
}
...
dclinst(Hshtab *inst, Hshtab *master)
{
	...
	Hshtab **p;
	char buf[NCPS];
print("dclinst: %s %s nmodule %d\n", inst->name, master->name, nmodule);
	for (i = 0; module[i].name != master; i++)
		if (i == nmodule)
			exits("undefined master");
print("i %d nmodule %d\n", i, nmodule);
	for (p = module[i].pin; p < module[i+1].pin; p++) {
		sprintf(buf, "%sDOT%s", inst->name, (*p)->name);
