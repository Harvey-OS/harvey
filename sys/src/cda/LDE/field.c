...
setiused(Hshtab *h, int mask)
{
	...
	case EQN:
		if(!h->assign)
			fprintf(stderr, "used and not set: %s\n", h->name);
		break;
	}
}
...
setoused(Hshtab *h, Node *tp)
{
	...
	if(h->assign) {
		fprintf(stderr, "set more then once: %s\n", h->name);
		return;
	}
	h->assign = tp;
}
...
setdcused(Hshtab *h, Node *tp)
{
	...
	if(h->dontcare) {
		fprintf(stderr, "dc: set more then once: %s\n", h->name);
		return;
	}
	h->dontcare = tp;
}
	...
	for(h = &hshtab[0]; h < &hshtab[HSHSIZ]; h++) {
		...
		case BOTH:
			if(h->code != OUTPUT && !h->iused)
				fprintf(stderr, "input not used: %s\n", h->name);
			if(h->code != INPUT && !h->assign)
				fprintf(stderr, "output not set: %s\n", h->name);
			break;

		case CEQN:
			if(h->assign && !h->iused)
				fprintf(stderr, "temp set and not used: %s\n", h->name);
			break;
		}
	}
}
