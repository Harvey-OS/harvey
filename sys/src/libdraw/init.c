...
int
geninitdraw(char* devdir, void (*error)(Display*, char*), char* fontname,
            char* label, char* windir, int ref)
{
	...
	if(fontname == nil) {
		snprint(buf, sizeof buf, "%d %d\n0 %d\t%s\n", df->height,
		        df->ascent, df->n - 1, deffontname);
		// BUG: Need something better for this
	...
	if(label) {
		...
		fd = open(buf, OREAD);
		if(fd >= 0) {
			read(fd, display->oldlabel,
			     (sizeof display->oldlabel) - 1);
			close(fd);
...
initdraw(void (*error)(Display*, char*), char* fontname, char* label)
{
	...
	if(access("/dev/draw/new", AEXIST) < 0 &&
	   bind("#i", "/dev", MAFTER) < 0) {
