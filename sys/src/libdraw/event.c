...
extract(void)
{
	...
	/* this test isn't perfect, though; could do flushimage(display, 0) then
	 * call extract */
	/* also: make sure we don't interfere if we're multiprocessing the
	 * display */
	if(display->locking) {
		/* if locking is being done by program, this means it can't
		 * depend on automatic flush in emouse() etc. */
		...
		flushimage(display, 1);
loop:
	if((n = read(epipe[0], ebuf, EMAXMSG + 1)) < 0 || ebuf[0] >= MAXSLAVE)
	...
	if(i == Smouse) {
		...
		if((eb = s->tail) &&
		   memcmp(eb->buf + 1 + 2 * 12, ebuf + 1 + 1 + 2 * 12, 12) ==
		       0) {
