...
#define NPROC 50
RWLock afterAWhile;	/* held for a while by main, then writers will take it */
...
waiter(int index)
{
	...
	if(wlockt(&afterAWhile, 100000)){
		...
		if(verbose)
			print("writer %d failed: wlockt timedout in %lld ms\n", getpid(), (nsec() - start) / (1000*1000));
		postnote(PNGROUP, getpid(), smprint("fail: writer %d timedout", getpid()));
	}
...
main(void)
{
	...
	rlock(&afterAWhile);
	if(verbose)
		print("main: got the afterAWhile: wakeup all writers...\n");
	rwakeupall(&rStart);
	if(verbose)
		print("main: got the afterAWhile: wakeup all writers... done\n");
	qunlock(&rl);
	...
	qlock(&rl);
	if(verbose)
		print("main: waiting all writers to take the lock...\n");
	...
	qunlock(&rl);
	if(verbose)
		print("main: waiting all writers to take the lock... done\n");
