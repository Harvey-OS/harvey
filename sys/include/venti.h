 ...
 */
enum { VtEntryActive = 1 << 0,     /* entry is in use */
...
enum { VtRootSize = 300, VtRootVersion = 2 };
 ...
 * Crypto suites
 */
enum { VtCryptoNone, VtCryptoSSL3, VtCryptoTLS1, VtCryptoMax };

/*
 * Codecs
 */
enum { VtCodecNone, VtCodecDeflate, VtCodecThwack, VtCodecMax };
...
enum { VtStateAlloc, VtStateConnected, VtStateClosed };
...
int vthello(VtConn*);
int vtread(VtConn*, uint8_t score[VtScoreSize], uint type, uint8_t* buf, int n);
int vtwrite(VtConn*, uint8_t score[VtScoreSize], uint type, uint8_t* buf,
            int n);
Packet* vtreadpacket(VtConn*, uint8_t score[VtScoreSize], uint type, int n);
int vtwritepacket(VtConn*, uint8_t score[VtScoreSize], uint type, Packet* p);
int vtsync(VtConn*);
int vtping(VtConn*);
...
void vtcachesetwrite(VtCache*, int (*)(VtConn*, uint8_t[VtScoreSize], uint,
                                       uint8_t*, int));
