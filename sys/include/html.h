...
/* Charsets */
enum { UnknownCharset, US_Ascii, ISO_8859_1, UTF_8, Unicode, NCHARSETS };

/* Frame Target IDs */
enum { FTtop, FTself, FTparent, FTblank };
...
struct Itext {
	...
	uint8_t
	    voff;   /* Voffbias+vertical offset from baseline, in pixels (+ve ==
	               down) */
...
enum { IFbrk = 0x80000000,    /* forced break before this item */
...
/* Spacer kinds */
enum { ISPnull,   /* 0 height and width */
...
enum { FFchecked = (1 << 7), FFmultiple = (1 << 6) };
...
enum { TFparsing = (1 << 7), TFnowrap = (1 << 6), TFisth = (1 << 5) };
...
/* Font sizes */
enum { Tiny, Small, Normal, Large, Verylarge, NumSize };

enum { NumFnt = NumStyle * NumSize,
       DefFnt = FntR * NumSize + Normal,
};

/* Lines are needed through some text items, for underlining or strikethrough */
enum { ULnone, ULunder, ULmid };
...
struct Docinfo {
	...
	Iimage*
	    backgrounditem; /* Image Item for doc background image, or nil */
...
extern Item* parsehtml(uint8_t* data, int datalen, Rune* src, int mtype,
