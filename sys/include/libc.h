...
extern void
cycles(uint64_t*); /* 64-bit value of the cycle counter if there is one, 0 if
                      there isn't */
...
extern void qsort(void*, int32_t, int32_t, int (*)(const void*, const void*));
...
#define ARGBEGIN                                                               \
	for((argv0 ? 0 : (argv0 = *argv)), argv++, argc--;                     \
		...
		while(*_args && (_args += chartorune(&_argc, _args)))          \
			switch(_argc)
/* #define	ARGEND		SET(_argt);USED(_argt,_argc,_args);}USED(argv,
 * argc); */
#define ARGEND                                                                 \
	SET(_argt);                                                            \
	USED(_argt);                                                           \
