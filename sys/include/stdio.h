 ...
 *	If setvbuf(., ., _IOLBF, .) is called, _IO_putc leaves wp and rp pointed
 *at the
 *	end of the buffer so that it can be called on each putc to check whether
 *it's got
 *	a newline.  This nonsense is in order to avoid impacting performance of
 *the other
 *	buffering modes more than necessary -- putting the test in putc adds
 *many
 *	instructions that are wasted in non-_IOLBF mode:
 *	#define putc(c, f)	(_IO_ctmp=(c),\
 *				(f)->wp>=(f)->rp || (f)->flags&LINEBUF &&
 *_IO_ctmp=='\n'\
 *					?_IO_putc(_IO_ctmp, f)\
 *					:*(f)->wp++=_IO_ctmp)
 *
...
int putc(int, FILE*);
#define putc(c, f)                                                             \
	((f)->wp >= (f)->rp ? _IO_putc(c, f) : (*(f)->wp++ = c) & _IO_CHMASK)
