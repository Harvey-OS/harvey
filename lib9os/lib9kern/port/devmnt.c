...
mntversion(Chan *c, char *version, int msize, int returnlen)
{
	...
	/* validity */
	if(msize < 0)
		p9error("bad iounit in version call");
	if(strncmp(v, VERSION9P, strlen(VERSION9P)) != 0)
		p9error("bad 9P version specification");
	...
	if(k == 0)
		p9error("bad fversion conversion on send");
	...
	if(l < k){
		...
		c->offset -= k - l;
		unlock(&c->l);
		p9error("short write in fversion");
	}

	/* message sent; receive and decode reply */
	k = devtab[c->type]->read(c, msg, 8192+IOHDRSZ, c->offset);
	if(k <= 0)
		p9error("EOF receiving fversion reply");
	...
	l = convM2S(msg, k, &f);
	if(l != k)
		p9error("bad fversion conversion on reply");
	if(f.type != Rversion){
		if(f.type == Rerror)
			p9error(f.ename);
		p9error("unexpected reply type in fversion");
	}
	if(f.msize > msize)
		p9error("server tries to increase msize in fversion");
	if(f.msize<256 || f.msize>1024*1024)
		p9error("nonsense value of msize in fversion");
	if(strncmp(f.version, v, strlen(f.version)) != 0)
		p9error("bad 9P version returned from server");
...
mntwalk(Chan *c, Chan *nc, char **name, int nname)
{
	...
	if(nname > MAXWELEM)
		p9error("devmnt: too many name elements");
	alloc = 0;
	...
	if(r->reply.nwqid > nname)
		p9error("too many QIDs returned by walk");
	if(r->reply.nwqid < nname){
		if(alloc)
			cclose(nc);
		wq->clone = nil;
...
mntchk(Chan *c)
{
	...
	/*
	 * Was it closed and reused (was p9error(Eshutdown); now, it can't happen)
	 */
