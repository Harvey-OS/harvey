...
#define va_start(list, start) list = \
				  (sizeof(start) < 8 ? (char *)((vlong *)&(start) + 1) : (char *)(&(start) + 1))
#define va_end(list) \
	USED(list)
#define va_arg(list, mode) \
	((sizeof(mode) == 1) ? ((mode *)(list += 8))[-1] : (sizeof(mode) == 2) ? ((mode *)(list += 8))[-1] : (sizeof(mode) == 4) ? ((mode *)(list += 8))[-1] : ((mode *)(list += sizeof(mode)))[-1])
