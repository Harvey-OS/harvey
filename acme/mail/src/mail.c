...
usage(void)
{
	fprint(
	    2,
	    "usage: Mail [-sS] [-o outgoing] [mailboxname [directoryname]]\n");
...
threadmain(int argc, char* argv[])
{
	...
	/* open these early so we won't miss notification of new mail messages
	 * while we read mbox */
	...
	fsname = estrdup(name);
	if(newdir && argc > 0) {
		s = emalloc(5 + strlen(mailboxdir) + strlen(mboxname) +
		            strlen(name) + 10 + 1);
		for(i = 0; i < 10; i++) {
			sprint(s, "open %s/%s %s", mailboxdir, mboxname,
			       fsname);
	...
	if(strcmp(name, "mbox") == 0) {
		/*
		 * Avoid creating multiple windows to send mail by only
		 * accepting
...
plumbthread(void)
{
	...
	while((m = recvp(cplumb)) != nil) {
		...
		if(type == nil)
			fprint(
			    2,
			    "Mail: plumb message with no mailtype attribute\n");
...
mboxcommand(Window* w, char* s)
{
	...
	if(strcmp(s, "Delmesg") == 0) {
		if(nargs > 1) {
			for(i = 1; i < nargs; i++) {
				snprint(buf, sizeof buf, "%s%s", mbox.name,
				        args[i]);
				delmesg(buf, nil, 1);
			}
		}
		...
		targs = emalloc(
		    nargs *
		    sizeof(char*)); /* could be too many for a local array */
	...
	for(;;) {
		...
		case 'M':
			...
			case 'X':
				...
				/* if it's a long message, it can't be for us
				 * anyway */
