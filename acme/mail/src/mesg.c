...
struct {
	...
	char* suffix;
             ...
             "image/gif", "image", ".gif", "image/jpeg", "image", ".jpg",
             "image/jpeg", "image", ".jpeg", "image/png", "image", ".png",
             "image/tiff", "image", ".tif", "application/postscript",
             "postscript", ".ps", "application/pdf", "postscript", ".pdf",
             "application/msword", "msword", ".doc", "application/rtf",
             "msword", ".rtf", "audio/x-wav", "wav", ".wav", nil, nil};

char* goodtypes[] = {
    "text", "text/plain", "message/rfc822", "text/richtext",
    "text/tab-separated-values", "text/calendar", "application/octet-stream",
    nil,
};
...
struct {
	char* type;
	char* ext;
} exts[] = {"image/gif", ".gif", "image/jpeg", ".jpg", nil, nil};

char* okheaders[] = {"From:", "Date:", "To:", "CC:", "Subject:", nil};

char* extraheaders[] = {
    "Resent-From:", "Resent-To:", "Sort:", nil,
};
...
mesgadd(Message* mbox, char* dir, Dir* d, char* digest)
{
	 ...
	 * accepting message */
	if(loaded == 0 || (digest != nil && m->digest != nil &&
	                   strcmp(digest, m->digest) != 0)) {
		mesgfreeparts(m);
		free(m);
		return 0;
	}
...
stripdate(char* as)
{
	...
	if(n > 5) {
		...
		if(strlen(fld[1]) < 4 && isnumeric(fld[1]))
			sprint(as + strlen(as), "%.3s %.3s ", fld[1],
			       fld[2]); /* date, month */
		else
			sprint(as + strlen(as), "%.3s %.3s ", fld[2],
			       fld[1]); /* date, month */
		/* do we use time or year?  depends on whether year matches this
		 * one */
		if(thisyear(fld[5])) {
			if(strchr(fld[3], ':') != nil)
				sprint(as + strlen(as), "%.5s ",
				       fld[3]); /* time */
...
info(Message* m, int ind, int ogf)
{
	...
	if(ind == 0 && shortmenu) {
		...
		if(ind == 0 && m->subject[0] == '\0') {
			...
			snprint(fmt, sizeof fmt, " %%-%d.%ds  %%-%d.%ds", len,
			        len, lens, lens);
	...
	if(ind == 0) {
		if(strcmp(m->type, "text") != 0 &&
		   strncmp(m->type, "text/", 5) != 0 &&
...
void
mesgmenu0(Window* w, Message* mbox, char* realdir, char* dir, int ind,
          Biobuf* fd, int onlyone, int dotail)
{
	int i;
	Message* m;
	char* name, *tmp;
...
mesgmenunew(Window* w, Message* mbox)
{
	...
	if(!mbox->dirty)
		winclean(w);
	/* select tag line plus following indented lines, but not final newline
	 * (it's distinctive) */
	winselect(w, "0/.*\\n((\t.*\\n)*\t.*)?/", 1);
	close(w->addr);
	close(w->data);
...
name2regexp(char* prefix, char* s)
{
	...
	buf = emalloc(strlen(prefix) + 2 * strlen(s) +
	              50); /* leave room to append more */
	p = buf;
	*p++ = '0';
	*p++ = '/';
	*p++ = '^';
...
mesgmenumark(Window* w, char* which, char* mark)
{
	...
	if(winsetaddr(w, buf, 1) &&
	   winsetaddr(w, "+0-#1", 1)) /* go to end of line */
...
mesgcommand(Message* m, char* cmd)
{
	...
	if(strcmp(args[0], "Q") == 0) {
		...
		if(nargs >= 3 && strcmp(args[1], "Reply") == 0 &&
		   strcmp(args[2], "all") == 0)
...
replytoaddr(Window* w, Message* m, Event* e, char* s)
{
	...
	if(isemail(s)) {
		...
		if(m->subject && m->subject[0]) {
			...
			if(tolower(m->subject[0]) != 'r' ||
			   tolower(m->subject[1]) != 'e' ||
			   m->subject[2] != ':')
				pm->attr->value =
				    estrstrdup("Re: ", m->subject);
			else
		...
		case 'M':
			...
			case 'L':
				...
				do {
					...
					nopen += mesgopen(&mbox, mbox.name, s,
					                  m, 0, nil);
					while(*s != 0 && *s++ != '\n')
						;
				} while(*s);
...
printheader(char* dir, Biobuf* b, char** okheaders)
{
	...
	for(i = 0; i < n; i++)
		for(j = 0; okheaders[j]; j++)
			if(cistrncmp(lines[i], okheaders[j],
			             strlen(okheaders[j])) == 0)
				Bprint(b, "%s\n", lines[i]);
	free(s);
}
...
mesgload(Message* m, char* rootdir, char* file, Window* w)
{
	...
	if(strcmp(m->type, "message/rfc822") !=
	   0) { /* suppress headers of envelopes */
		if(strlen(m->from) > 0) {
	...
	if(m->head == nil) { /* single part message */
		...
		if(strcmp(m->type, "multipart/alternative") == 0) {
			...
			if(thisone == nil) {
				thisone = m->head; /* in case we can't find a
				                      good one */
		...
		for(mp = m->head; mp != nil; mp = mp->next) {
			...
			/* skip first element in name because it's already in
			 * window name */
			if(mp != m->head)
				Bprint(w->body, "\n===> %s (%s) [%s]\n",
...
int
mesgopen(Message* mbox, char* dir, char* s, Message* mesg, int plumbed,
         char* digest)
{
	char* t, *u, *v;
	Message* m;
	...
	if(m == nil)
		goto Error;
	if(mesg != nil && m != mesg) /* string looked like subpart but isn't
	...
	if(m->opened == 0) {
		...
		if(!reuse) {
			if(m->deleted)
				wintagwrite(m->w, "Q Reply all UnDelmesg Save ",
				            2 + 6 + 4 + 10 + 5);
			else
				wintagwrite(m->w, "Q Reply all Delmesg Save ",
				            2 + 6 + 4 + 8 + 5);
		}
...
rewritembox(Window* w, Message* mbox)
{
	...
	for(m = mbox->head; m != nil; m = next) {
		...
		if(m->writebackdel) {
			/* messages deleted by plumb message are not removed
			 * again */
			t = estrdup(m->name);
			if(strlen(t) > 0)
				t[strlen(t) - 1] = '\0';
	...
	if(write(mbox->ctlfd, deletestr, strlen(deletestr)) < 0)
		fprint(
		    2,
		    "Mail: warning: error removing mail message files: %r\n");
	free(deletestr);
	winselect(w, "0", 0);
	if(nopen == 0)
		winclean(w);
...
mesglookupfile(Message* mbox, char* name, char* digest)
{
	...
	if(k == 0 || strncmp(name, mbox->name, n) != 0) {
		//		fprint(2, "Mail: message %s not in this
		//mailbox\n", name);
		return nil;
	}
