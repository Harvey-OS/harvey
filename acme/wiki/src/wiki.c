...
expand(Window* w, Event* e, Event* eacme)
{
	...
	if(q1 < 0)
		return eacme;
	if((x = eval(w, "#%lud-/\\[/", q1)) == -1 ||
	   x > e->q1) /* ] opens after us */
		return eacme;
	e->q0 = q0 + 1;
	e->q1 = q1;
	return e;
...
acmeevent(Wiki* wiki, Event* e)
{
	...
	case 'M':               /* mouse event */
		...
		case 'L': /* mouse: button 3 in body */
			...
			eq = e;
			if(e->flag &
			   2) { /* we do our own expansion for loads */
				e2 = recvp(w->cevent);
				eq = expand(w, eq, e2);
			}
			s = eq->b;
...
wikiopen(char* arg, char* addr)
{
	...
	/*
	        if(arg==nil){
	                ...
	                || seek(mapfd, 0, 0) < 0 || (n=read(mapfd, tmp,
	   sizeof(tmp)-2)) < 0){
	...
	*/
	...
	if(strncmp(arg, dir, strlen(dir)) == 0 && arg[strlen(dir)] == '/' &&
	   arg[strlen(dir) + 1])
		arg += strlen(dir) + 1;
	else if(arg[0] == '/')
		return -1;
