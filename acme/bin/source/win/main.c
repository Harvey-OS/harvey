	...
	if(!command(w, s) && s[0] != '\0') { /* send it as typed text */
		...
		if(e->flag & 1)
			...
			if(len > 0 && s[len - 1] != '\n' &&
			   s[len - 1] != '\004') {
				if(!needfree) {
					/* if(needfree), we left room for a
					 * newline before */
					t = emalloc(len + 2);
					strcpy(t, s);
					s = t;
	...
	for(;;) {
		...
		if(debug)
			fprint(2, "msg: %C %C %d %d %d %d %q\n",
			       e->c1 ? e->c1 : ' ', e->c2 ? e->c2 : ' ', e->q0,
			       e->q1, e->flag, e->nb, e->b);
		switch(e->c1) {
		default:
		Unknown:
		...
		case 'E': /* write to tag or body; body happens due to sendit */
			...
			if(!islower(e->c2))
				fprint(2, "win msg: %C %C %d %d %d %d %q\n",
				       e->c1, e->c2, e->q0, e->q1, e->flag,
				       e->nb, e->b);
			break;

		case 'F': /* generated by our actions (specifically case 'S'
		             above) */
			delta = e->q1 - e->q0;
			if(e->c2 == 'D') {
			...
			case 'I':
				...
				if(e->q0 >= hostpt &&
				   hasboundary(e->r, e->nr)) {
					/*
					 * If we are between the S message
					 * (which
					 * we processed by inserting text in the
					 * window) and the F message notifying
					 * us
					 * that the text has been inserted, then
					 * our
					 * impression of the hostpt and acme's
					 * may be different.  This could be seen
					 * if you
					 * hit enter a bunch of times in a con
					 * session.  To work around the
					 * unreliability,
					 * only send input if we don't have an S
					 * pending.
					 * The same race occurs between when a
					 * character
					 * is typed and when we get notice of
					 * it, but
					 * since characters tend to be typed at
					 * the end
					 * of the buffer, we don't run into it.
					 * There's
					 * no workaround possible for this
					 * typing race,
					 * since we can't tell when the user has
					 * typed
					 * something but we just haven't been
					 * notified.
					 */
					if(pendingS)
						pendingK = 1;
					else
						hostpt += sendinput(w, hostpt,
						                    &endpt);
				}
				break;
			}
