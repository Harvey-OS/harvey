...
openwin(char* name, char* buttons, Win* twin, int wintype)
{
	...
	if(wintype == Dictwin)
		...
		sprint(buf, "%s/%s/%s/%d", name, dict, format(pattern),
		       curindex + 1);
	else
...
command(char* com, Win* w, int wintype)
{
	...
	if(strncmp(com, "Nmatch", 6) == 0) {
		...
		curone = curaddr[curindex];
		if(curone != nil) {
			sprint(buffer, "%s/%s/%s/%d", prog, dict,
			       format(pattern), curindex + 1);
			wname(w, buffer);
			dispentry(w);
		}
		return True;
	...
	while(True) {
		...
		case 'X': /* Execute command. */
			...
			if(na) {
				...
				snprint(t, strlen(s) + 1 + na + 1, "%s %s", s,
				        ea.b);
				s = t;
			}
		...
		case 'L': /* Look for something. */
			...
			if(wintype == Entrywin) {
				...
				procpexec(
				    lprog,
				    args); /* New adict with chosen pattern. */
			}
			if(wintype == Matchwin) {
				tmp = atoi(e.b) - 1;
				if((tmp >= 0) && (tmp < MAXMATCH) &&
				   (curaddr[tmp] != nil)) {
					curindex = tmp;
					curone = curaddr[curindex];
					/* Display selected match. */
					if(Eopen) {
						sprint(buf, "%s/%s/%s/%d", prog,
						       dict, format(pattern),
						       curindex + 1);
						wname(&Ewin, buf);
						dispentry(&Ewin);
